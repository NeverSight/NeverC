...
static int h2_expect_headers_on(int fd, uint32_t expect_stream) {
    for (;;) {
        uint8_t header[9];
        if (sock_read_all(fd, header, sizeof(header)) != 0) return -1;
        uint32_t length = ((uint32_t)header[0] << 16) |
        ...
        if (header[3] == NC_H2_FRAME_GOAWAY)
            return -1;
        if (length > 0 && h2_drain_frame_payload(fd, length) != 0)
            return -1;
        if (header[3] == NC_H2_FRAME_HEADERS && stream_id == expect_stream)
            return 0;
    }
}
...
TEST(h2c_rejects_invalid_path) {
    ...
    neverc_hpack_header_t scheme_rel[] = {
        { .name = ":method", .value = "GET" },
        { .name = ":path", .value = "//evil.example/" },
        { .name = ":scheme", .value = "http" },
        { .name = ":authority", .value = "localhost" },
    };
    ...
    neverc_hpack_header_t scheme_rel_nul[] = {
        { .name = ":method", .value = "GET" },
        { .name = ":path", .value = "/%00//evil.example/" },
        { .name = ":scheme", .value = "http" },
        { .name = ":authority", .value = "localhost" },
    };
    ...
    ASSERT_EQ(h2c_expect_protocol_rst(scheme_rel_nul, 4), 0);
...
TEST(h2c_unfinished_request_leftover_headers_keep_conn) {
    neverc_tcp_conn_t *client = NULL;
    pid_t child = -1;
    ASSERT_EQ(h2_pipe_handshake(&client, &child, 1), 0);
    int fd = neverc_tcp_conn_fd(client);
    ...
    neverc_hpack_header_t trailers[] = {
        { .name = "x-trail", .value = "1" },
    };
    ASSERT_EQ(h2_send_headers_on(fd, 1, trailers, 1, 1), 0);
    neverc_hpack_header_t get[] = {
        { .name = ":method", .value = "GET" },
        { .name = ":path", .value = "/" },
        { .name = ":scheme", .value = "http" },
        { .name = ":authority", .value = "localhost" },
    };
    ASSERT_EQ(h2_send_headers_on(fd, 3, get, 4, 1), 0);
    ASSERT_EQ(h2_expect_headers_on(fd, 3), 0);
    neverc_tcp_close(client);
    int status = 0;
    ASSERT_EQ(h2_reap_child(child, &status), 0);
    ASSERT_TRUE(WIFEXITED(status));
...
TEST(h2c_stream_flow_violation_resets_only_stream) {
    neverc_tcp_conn_t *client = NULL;
    neverc_tcp_conn_t *server = NULL;
    ASSERT_EQ(neverc_tcp_pipe(&client, &server), 0);
    h2_serve_ctx_t ctx = {
    ...
    ASSERT_EQ(sock_write_all(fd, settings, sizeof(settings)), 0);
    ASSERT_EQ(h2_client_handshake(fd), 0);
    neverc_hpack_header_t post[] = {
        { .name = ":method", .value = "POST" },
        { .name = ":path", .value = "/" },
        { .name = ":scheme", .value = "http" },
        { .name = ":authority", .value = "localhost" },
    };
    ASSERT_EQ(h2_send_headers_on(fd, 1, post, 4, 0), 0);
    ASSERT_EQ(h2_send_data_on(fd, 1, "123456789", 9U, 1), 0);
    uint32_t error_code = 0xffffffffU;
    ASSERT_EQ(h2_read_rst_on(fd, 1, &error_code), 0);
    ASSERT_EQ(error_code, NC_H2_FLOW_CONTROL_ERROR);
...
static void h2_run_adversarial_response_child(
    ...
    if (response_kind == 2 || response_kind == 16) {
        ...
        uint8_t enable_push[] = {
            0, NC_H2_SETTINGS_ENABLE_PUSH, 0, 0, 0,
            (uint8_t)(response_kind == 2 ? 1 : 0)};
    ...
    if (response_kind == 10) {
        /* 103 then 200. Each block is well under a 50-byte list limit;
         * accumulating across blocks would reject the 200. */
        uint8_t informational[14] = {
            0, 0, 5, NC_H2_FRAME_HEADERS, NC_H2_FLAG_END_HEADERS,
            0, 0, 0, 1,
            0x48, 0x03, '1', '0', '3'};
        uint8_t final_headers[10] = {
            ...
            (uint8_t)(NC_H2_FLAG_END_HEADERS | NC_H2_FLAG_END_STREAM),
            0, 0, 0, 1, 0x88};
        if (sock_write_all(fd, informational, sizeof(informational)) != 0 ||
            sock_write_all(fd, final_headers, sizeof(final_headers)) != 0)
            _exit(1);
        char drain[256];
        while (read(fd, drain, sizeof(drain)) > 0) { }
        neverc_tcp_close(connection);
    ...
    if (response_kind == 11) {
        /* HEADERS+END_STREAM then leftover DATA must not clobber status. */
        uint8_t final_headers[10] = {
            0, 0, 1, NC_H2_FRAME_HEADERS,
            (uint8_t)(NC_H2_FLAG_END_HEADERS | NC_H2_FLAG_END_STREAM),
            0, 0, 0, 1, 0x88};
        uint8_t extra_data[10] = {
            0, 0, 1, NC_H2_FRAME_DATA, 0, 0, 0, 0, 1, 'x'};
        if (sock_write_all(fd, final_headers, sizeof(final_headers)) != 0 ||
            sock_write_all(fd, extra_data, sizeof(extra_data)) != 0)
            _exit(1);
        char drain[256];
        while (read(fd, drain, sizeof(drain)) > 0) { }
        neverc_tcp_close(connection);
    ...
    if (response_kind == 17) {
        ...
        uint8_t data_frame[18] = {
            0, 0, 9, NC_H2_FRAME_DATA, NC_H2_FLAG_END_STREAM,
            0, 0, 0, 1,
            'o', 'v', 'e', 'r', '-', 'f', 'l', 'o', 'w'};
        if (sock_write_all(fd, final_headers, sizeof(final_headers)) != 0 ||
            sock_write_all(fd, data_frame, sizeof(data_frame)) != 0)
            _exit(1);
        char drain[256];
        while (read(fd, drain, sizeof(drain)) > 0) { }
        neverc_tcp_close(connection);
...
TEST(h2_client_data_after_end_stream_keeps_status) {
    const char *error = NULL;
    neverc_tcp_listener_t *listener =
        neverc_tcp_listen("127.0.0.1:0", &error);
    ASSERT_TRUE(listener != NULL);
    ...
    ASSERT_TRUE(child >= 0);
    if (child == 0)
        h2_run_adversarial_response_child(listener, 11, -1);
    neverc_tcp_listener_close(listener);
    ...
    int second = first == 1
        ? neverc_h2_client_stream_receive(stream, context, &terminal) : -1;
    int header_type = headers ? (int)headers->type : -1;
    int header_status = headers ? headers->status_code : -1;
    int terminal_type = terminal ? (int)terminal->type : -1;
    const char *terminal_error = terminal ? terminal->error : "missing";
    uint32_t terminal_error_code = terminal ? terminal->error_code : 0xffffU;
...
static neverc_h2_response_t *h2_client_do_kind_configured(
    int kind, const char *method, uint32_t initial_window_size) {
    const char *error = NULL;
    neverc_tcp_listener_t *listener =
        neverc_tcp_listen("127.0.0.1:0", &error);
...
static neverc_h2_response_t *h2_client_do_kind(int kind, const char *method) {
    return h2_client_do_kind_configured(kind, method, 0U);
}
...
TEST(h2_client_stream_flow_error_uses_flow_control_code) {
    neverc_h2_response_t *response =
        h2_client_do_kind_configured(17, "GET", 8U);
    ASSERT_TRUE(response != NULL);
    ASSERT_TRUE(response->error != NULL);
    ASSERT_EQ(response->stream_error, NC_H2_FLOW_CONTROL_ERROR);
    neverc_h2_response_free(response);
}

TEST(h2_client_rejects_non_token_method) {
    neverc_h2_response_t *response = h2_client_do_kind(18, "GET{");
    ASSERT_TRUE(response != NULL);
    ASSERT_TRUE(response->error != NULL);
    neverc_h2_response_free(response);
}
...
TEST(h2_client_header_list_size_is_per_block) {
    ...
    ASSERT_TRUE(child >= 0);
    if (child == 0)
        h2_run_adversarial_response_child(listener, 10, -1);
    neverc_tcp_listener_close(listener);
    ...
    neverc_h2_client_config_t config = neverc_h2_client_config_default();
    config.timeout_ms = 3000;
    config.max_response_header_list_size = 50;
    neverc_h2_client_t *client = neverc_h2_client_dial(
        dial_address, "localhost", 0, &config, &error);
    neverc_h2_client_stream_t *stream = client
        ? neverc_h2_client_stream_open(client, NULL, "GET", "/",
    ...
    int first = stream && context
        ? neverc_h2_client_stream_receive(stream, context, &headers) : -1;
    int header_type = headers ? (int)headers->type : -1;
    int status_code = headers ? headers->status_code : -1;
...
static void h2_client_assert_rejects_server_enable_push(int response_kind) {
    const char *error = NULL;
    neverc_tcp_listener_t *listener =
        neverc_tcp_listen("127.0.0.1:0", &error);
    ASSERT_TRUE(listener != NULL);
    ...
    if (child == 0) {
        close(notified[0]);
        h2_run_adversarial_response_child(listener, response_kind,
                                          notified[1]);
    }
    close(notified[1]);
    neverc_tcp_listener_close(listener);
...
TEST(h2_client_rejects_server_enable_push) {
    h2_client_assert_rejects_server_enable_push(2);
}

TEST(h2_client_rejects_server_enable_push_zero) {
    h2_client_assert_rejects_server_enable_push(16);
}
...
int main(void) {
    ...
    run_test_h2c_unfinished_request_leftover_headers_keep_conn();
    ...
    run_test_h2c_stream_flow_violation_resets_only_stream();
    ...
    run_test_h2_client_rejects_server_enable_push_zero();
