...
static int test_valid_alabel_reaches_controlled_dial(void) {
    static const char label[] = "valid-http";
    reset_dial_probe();
    neverc_http_response_t *response =
        neverc_http_get("http://xn--bcher-kva.de/");
    int passed = response != NULL && controlled_dial_calls == 1 &&
        strcmp(controlled_dial_address, "xn--bcher-kva.de:80") == 0;
...
static int test_invalid_alabel_is_rejected_before_dial(const char *label,
                                                        const char *url) {
    reset_dial_probe();
...
int main(void) {
    int failed = 0;
    failed |= test_valid_alabel_reaches_controlled_dial();
    failed |= test_invalid_alabel_is_rejected_before_dial(
