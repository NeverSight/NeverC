...
TEST_F(StdLibTest, HpackHeaderListBudget) {
  auto r = compileAndRunStdTest("hpack_list_budget", {},
                                {"-fno-builtin-std"});
  ASSERT_TRUE(r.ok()) << "stdout: " << r.out << "\nstderr: " << r.err;
  EXPECT_TRUE(r.contains("passed")) << "stdout: " << r.out;
}
...
TEST_F(StdLibTest, TimeSleepBounds) {
  auto r =
      compileAndRunStdTest("time_sleep_bounds", {}, {"-fno-builtin-std"});
  ASSERT_TRUE(r.ok()) << "stdout: " << r.out << "\nstderr: " << r.err;
  EXPECT_TRUE(r.contains("passed")) << "stdout: " << r.out;
}
...
TEST_F(StdLibTest, ContextClockAndCancelOrdering) {
  auto r = compileAndRunStdTest("context_clock", {}, {"-fno-builtin-std"});
  ASSERT_TRUE(r.ok()) << "stdout: " << r.out << "\nstderr: " << r.err;
  EXPECT_TRUE(r.contains("passed")) << "stdout: " << r.out;
}
...
TEST_F(StdLibTest, MapsTombstoneLoadInvariant) {
  auto r =
      compileAndRunStdTest("maps_tombstone_load", {}, {"-fno-builtin-std"});
  ASSERT_TRUE(r.ok()) << "stdout: " << r.out << "\nstderr: " << r.err;
  EXPECT_TRUE(r.contains("passed")) << "stdout: " << r.out;
}
...
TEST_F(StdLibTest, SlicesIntSpanBounds) {
  auto r =
      compileAndRunStdTest("slices_int_bounds", {}, {"-fno-builtin-std"});
  ASSERT_TRUE(r.ok()) << "stdout: " << r.out << "\nstderr: " << r.err;
  EXPECT_TRUE(r.contains("passed")) << "stdout: " << r.out;
}
...
// ===== Native RPC / gRPC =====
STD_TEST(rpc)
STD_TEST(rpc_client_epoch, "src/thread/thread.c", "src/context/context.c")
