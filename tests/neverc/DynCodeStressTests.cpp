...
class DynCodeStressTest : public NeverCTest {};

TEST_F(DynCodeStressTest, VolatileBarrier) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
)");
  dyncodeCrossCompile("volatile_barrier", src.string());
}

TEST_F(DynCodeStressTest, FnptrDispatch) {
...
int dyncode_entry(int k, int x) {
    BinOp f = kOps[k & 3];
    return f(x, k);
}
int main(int a, int b) { return dyncode_entry(a, b); }
)");
  dyncodeCrossCompile("fnptr_dispatch", src.string());
}

TEST_F(DynCodeStressTest, BitfieldsUnion) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("bitfields_union", src.string());
}

TEST_F(DynCodeStressTest, Int64Div) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("int64_div", src.string());
}

TEST_F(DynCodeStressTest, Builtins) {
  auto src = tmpFile("builtins.c");
  writeFile(src, R"(
int dyncode_entry(unsigned a, unsigned b) {
    unsigned s = 0;
...
int main(int a, int b) { return dyncode_entry((unsigned)a, (unsigned)b); }
)");
  dyncodeCrossCompile("builtins", src.string());
}

TEST_F(DynCodeStressTest, BigSwitch) {
  auto src = tmpFile("big_switch.c");
  std::string code = R"(
int dyncode_entry(int k, int v) {
    switch (k & 0x1F) {
)";
  for (int i = 0; i < 31; ++i)
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  writeFile(src, code);
  dyncodeCrossCompile("big_switch", src.string());
}

TEST_F(DynCodeStressTest, AtomicOps) {
  auto src = tmpFile("atomic_ops.c");
  writeFile(src, R"(
static volatile int g_counter;
int dyncode_entry(int iters, int step) {
    int seen = 0;
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("atomic_ops", src.string());
}

TEST_F(DynCodeStressTest, Varargs) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("varargs", src.string());
}

TEST_F(DynCodeStressTest, FloatMath) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("float_math", src.string());
}

TEST_F(DynCodeStressTest, SretReturn) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("sret_return", src.string());
}

TEST_F(DynCodeStressTest, VLA) {
  auto src = tmpFile("vla.c");
  writeFile(src, R"(
int dyncode_entry(int n, int seed) {
    int count = (n & 0x1F) + 1;
    int buf[count];
    int sum = 0;
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("vla", src.string());
}

TEST_F(DynCodeStressTest, LongDouble) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("long_double", src.string());
}

TEST_F(DynCodeStressTest, Unreachable) {
...
int main(int a, int b) { return dyncode_entry(a, b); }
  ...
  dyncodeCrossCompile("unreachable", src.string());
}
