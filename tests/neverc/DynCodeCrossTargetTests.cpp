...
class DynCodeCrossTargetTest : public NeverCTest {};

TEST_F(DynCodeCrossTargetTest, PureComputation) {
...
int dyncode_entry(int a, int b) {
  ...
  dyncodeCrossCompile("pure", src.string());
}

TEST_F(DynCodeCrossTargetTest, Echo) {
  auto src = tmpFile("cross_echo.c");
  writeFile(src, R"(
#include <unistd.h>
int main(void) { write(1, "OK\n", 3); _exit(0); return 0; }
)");
  dyncodeCrossCompile("echo", src.string());
}

TEST_F(DynCodeCrossTargetTest, KRDeclarations) {
...
int main(void) { write(1, "KR\n", 3); _exit(0); return 0; }
  ...
  dyncodeCrossCompile("kr", src.string());
}

TEST_F(DynCodeCrossTargetTest, Heavy) {
...
int main(void) {
  ...
  dyncodeCrossCompile("heavy", src.string());
}

TEST_F(DynCodeCrossTargetTest, R0PureCompute) {
  auto src = tmpFile("cross_r0_pure.c");
  writeFile(src, R"(
int dyncode_entry(int seed) {
    int acc = seed;
    for (int i = 0; i < 5; ++i) acc = acc * 17 + 3;
    return acc;
}
)");
  dyncodeCrossCompileKernel("r0_pure", src.string());
}

TEST_F(DynCodeCrossTargetTest, R0Extern) {
...
NEVERC_KERNEL_ENTRY
int dyncode_entry(int seed) {
    platform_log("hello from dyncode");
    return seed * 31;
}
)");
  dyncodeCrossCompileKernel("r0_extern", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3Atomics) {
...
int dyncode_entry(int seed) {
  ...
  dyncodeCrossCompile("r3_atomic", src.string());
  dyncodeCrossCompileKernel("r0_atomic", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3Float) {
...
int dyncode_entry(int a, int b) {
  ...
  dyncodeCrossCompile("r3_float", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3Struct) {
...
int dyncode_entry(int a, int b) {
  ...
  dyncodeCrossCompile("r3_struct", src.string());
  dyncodeCrossCompileKernel("r0_struct", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3Bitops) {
  auto src = tmpFile("cross_bitops.c");
  writeFile(src, R"(
int dyncode_entry(unsigned a, unsigned long long b) {
    int r = 0;
    r += __builtin_clz(a | 1);
  ...
  dyncodeCrossCompile("r3_bitops", src.string());
  dyncodeCrossCompileKernel("r0_bitops", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3ThreadLocal) {
  auto src = tmpFile("cross_tls.c");
  writeFile(src, R"(
_Thread_local int counter = 0;
int dyncode_entry(int x) { counter += x; return counter * 3; }
)");
  dyncodeCrossCompile("r3_tls", src.string());
  dyncodeCrossCompileKernel("r0_tls", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3DesignatedInit) {
...
int dyncode_entry(int a, int b) {
  ...
  dyncodeCrossCompile("r3_desig", src.string());
  dyncodeCrossCompileKernel("r0_desig", src.string());
}

TEST_F(DynCodeCrossTargetTest, R3VLA) {
  auto src = tmpFile("cross_vla.c");
  writeFile(src, R"(
int dyncode_entry(int n) {
    int buf[n];
  ...
  dyncodeCrossCompile("r3_vla", src.string());
}

TEST_F(DynCodeCrossTargetTest, OptMatrix) {
  auto src = tmpFile("cross_opt.c");
  writeFile(src, R"(
#include <unistd.h>
int dyncode_entry(void) {
  ...
  for (auto *opt : {"-O0", "-O1", "-O2", "-O3", "-Os", "-Oz"}) {
    for (auto *triple : triples) {
      ...
      auto r = ncc({"-fdyncode", opt, "-target", triple, src.string(), "-o",
                    bin.string()});
...
TEST_F(DynCodeCrossTargetTest, RejectLibm) {
  auto src = tmpFile("cross_libm.c");
  writeFile(src, R"(
double sqrt(double); double sin(double);
int dyncode_entry(int a) {
    return (int)(sqrt((double)a) + sin((double)a));
}
)");
  auto r = ncc({"-fdyncode", "-target", "aarch64-linux-gnu", src.string(),
...
TEST_F(DynCodeCrossTargetTest, RejectStdio) {
  auto src = tmpFile("cross_stdio.c");
  writeFile(src, R"(
int printf(const char *fmt, ...);
int dyncode_entry(int a) { printf("a=%d\n", a); return a; }
)");
  auto r = ncc({"-fdyncode", "-target", "aarch64-linux-gnu", src.string(),
...
TEST_F(DynCodeCrossTargetTest, RejectHeap) {
...
int dyncode_entry(int n) {
  ...
  auto r = ncc({"-fdyncode", "-fno-dyncode-heap-arena", "-target",
...
TEST_F(DynCodeCrossTargetTest, RejectConstructor) {
  auto src = tmpFile("cross_ctor.c");
  writeFile(src, R"(
__attribute__((constructor)) static void init(void) {}
int dyncode_entry(int x) { return x; }
)");
  auto r = ncc({"-fdyncode", "-target", "aarch64-linux-gnu", src.string(),
...
TEST_F(DynCodeCrossTargetTest, PrintOnlyModes) {
  auto src = tmpFile("cross_print_only.c");
  writeFile(src, "int dyncode_entry(int x) { return x + 1; }");
  for (auto *flag : {"-###", "-fdriver-only", "-ccc-print-phases",
                     "-ccc-print-bindings"}) {
    SCOPED_TRACE(flag);
    auto bin = tmpFile(std::string("print_only_") + flag + ".bin");
    auto r = ncc({"-fdyncode", flag, "-target", "aarch64-linux-gnu",
