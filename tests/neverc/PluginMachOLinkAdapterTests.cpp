...
TEST_F(PluginMachOLinkAdapterTest,
       IcfDistinguishesReferentOffsetsBeyondHashWidth) {
  const fs::path DataSource = tmpFile("macho-icf-wide-referent-data.s");
  const fs::path TextSource = tmpFile("macho-icf-wide-referent-text.s");
  const fs::path DataObject = tmpFile("macho-icf-wide-referent-data.o");
  const fs::path TextObject = tmpFile("macho-icf-wide-referent-text.o");
  const fs::path Image = tmpFile("macho-icf-wide-referent.dylib");

  // The two symbols share one zero-fill input section, so the 4 GiB virtual
  // gap does not enlarge the object or output file. Mach-O ICF hashes a
  // referent offset into 32 bits; the offsets therefore collide exactly even
  // though the 64-bit addresses named by the relocations are different.
  writeFile(DataSource, ".globl _near_data, _far_data\n"
                        ".zerofill __DATA,__bss,_near_data,0x100000000,3\n"
                        ".zerofill __DATA,__bss,_far_data,8,3\n");
  ...
  EXPECT_NE(*F1, *F2)
      << "ICF folded referent offsets that differ above its hash width";
}
