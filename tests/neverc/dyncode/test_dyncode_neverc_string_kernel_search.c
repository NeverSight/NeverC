...
/* NeverC builtin string in kernel-context dyncode mode: search + resize.
 *
 * Compile-only guard for `-mdyncode-context=kernel`: ensures the
 ...
 * only asserts that the dyncode pipeline accepts every helper.
 *
