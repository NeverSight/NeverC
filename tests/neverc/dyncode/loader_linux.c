...
/*
 * DynCode loader for Linux / Android (arm64 and x86_64).
 *
 * Reads a flat `.bin` dyncode file, maps it with W^X discipline
 ...
 *
 *   ./loader <dyncode.bin> <int0> <int1>
 *       -> treats entry as `int f(int, int)` and uses its return
 *          value as the process exit code.  Useful for the
 *          computation-only dyncodes that do not self-exit.
 *
 *   ./loader <dyncode.bin>
 *       -> treats entry as `void f(void)`; the dyncode is expected
