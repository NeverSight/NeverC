 ...
 *
 * Compile-only guard for `-mdyncode-context=kernel`: every UTF-8 /
 ...
 * `int dyncode_entry(int seed)` and is driven exclusively by
 * `-mdyncode-context=kernel` codegen.  The two are not mergable into
