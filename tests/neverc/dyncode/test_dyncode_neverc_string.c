...
/* NeverC builtin string in dyncode mode.
 *
 * Exercises owned string operations (`+`, `+=`, assignment copy, `substr`),
 * std::string-like mutation helpers, and utility helpers so the dyncode
...
int main(int a, int b) {
    ...
    if (!neverc_string_eq(joined, "protocode"))
        return 1;
    if (joined != "protocode" || joined == "proto")
        return 2;
    ...
    if (joined_cstr[0] != 'p' || joined_cstr[9] != 0)
        return 11;
    const char *joined_data = joined.data();
    if (joined_data[4] != 'o' || joined_data[9] != 0)
        return 23;
    ...
    if (joined.size() != 9 || joined.length() != 9 || joined.empty())
        return 20;
    if (!joined.starts_with("proto") || !joined.contains("toco") ||
        joined.compare("protocode") != 0)
        return 21;
    if (joined.front() != 'p' || joined.back() != 'e' || joined.at(5) != 'c')
        return 22;

    string mut = "proto";
    mut = mut.append("code");
    mut = mut.push_back('!');
    if (!neverc_string_eq(mut, "protocode!"))
        return 29;
    mut = mut.pop_back();
    mut = mut.insert(5, "-");
    if (!neverc_string_eq(mut, "proto-code"))
        return 30;
    mut = mut.erase(5, 1);
