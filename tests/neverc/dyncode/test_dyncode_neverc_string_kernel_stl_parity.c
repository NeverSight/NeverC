 ...
 *
 * Compile-only guard for `-mdyncode-context=kernel`: every helper the
 * user-mode `test_dyncode_neverc_string_stl_parity.c` exercises must
 ...
 * kernel-mode test uses `int dyncode_entry(int seed)` and is driven
 * exclusively by `-mdyncode-context=kernel` codegen.  The two are
