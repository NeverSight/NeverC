...
static NevercStatus get_image(const NevercPhaseFrame *Frame,
  ...
  Status = Trace->PhaseAPI->GetImage(Trace->PhaseAPI->Context, Frame, Artifact,
                                     OutPhase);
...
static void record_mutation_statuses(const NevercMutableBinaryAPI *Binary,
                                     NevercTaskHandle Task,
                                     NevercMutableBinaryBuilderHandle Builder,
                                     uint64_t Size,
                                     NevercStatusCode OutStatuses[6]) {
  NevercByteView Empty = {0, 0};
  OutStatuses[0] = Binary->Reserve(Binary->Context, Task, Builder, 0).Code;
  OutStatuses[1] = Binary->Write(Binary->Context, Task, Builder, Empty).Code;
  OutStatuses[2] =
      Binary->WriteAt(Binary->Context, Task, Builder, 0, Empty).Code;
  OutStatuses[3] =
      Binary->Insert(Binary->Context, Task, Builder, 0, Empty).Code;
  OutStatuses[4] = Binary->Append(Binary->Context, Task, Builder, Empty).Code;
...
NevercStatus NEVERC_CALL neverc_test_post_emit_interceptor(
  ...
  NevercTestPostEmitTrace *Trace = (NevercTestPostEmitTrace *)UserData;
  NevercLinkPhaseImageInfo Phase;
  NevercBinaryImageInfo Image;
  NevercPhaseResult Downstream;
  NevercStatus Status;
  NevercByteView Patch;
  if (!Frame || !Continuation || !OutResult || !Trace || !Trace->PhaseAPI)
  ...
  if (Trace->AppendByte)
    ...
    Status = Image.Binary->WriteAt(Image.Binary->Context, Frame->Task,
                                   Image.Builder, Trace->PatchOffset, Patch);
...
NevercStatus NEVERC_CALL neverc_test_after_commit_observer(
  ...
  NevercStatus Status;
  NevercByteView Patch;
  if (!Frame || !Trace || !Trace->PhaseAPI || Point != NEVERC_OBSERVER_AFTER)
    return invalid_status();
  Status = get_image(Frame, Frame->Input, Trace, &Phase, &Image);
  if (!neverc_status_is_ok(Status))
    return Status;
...
NevercStatus NEVERC_CALL neverc_test_post_emit_observer(
  ...
  NevercBinaryImageInfo Image;
  NevercStatus Status;
  if (!Frame || !Trace || !Trace->PhaseAPI || Point != NEVERC_OBSERVER_BEFORE)
    return invalid_status();
  Status = get_image(Frame, Frame->Input, Trace, &Phase, &Image);
  if (!neverc_status_is_ok(Status))
    return Status;
  ++Trace->ObserverCalls;
  cache_image(Frame, &Phase, &Image, Trace);
  record_mutation_statuses(Image.Binary, Frame->Task, Image.Builder, Image.Size,
                           Trace->ObserverMutationStatuses);
  return neverc_status_ok();
}
...
NevercStatus NEVERC_CALL neverc_test_cached_post_emit_capability_observer(
  ...
  NevercBinaryImageInfo Image;
  NevercStatus Status;
  if (!Frame || !Trace || !Trace->CachedLink || Point != NEVERC_OBSERVER_BEFORE)
    return invalid_status();
