...
static const NevercIRGenAPI *IRGenAPI;
...
provide_ir_module(const NevercPhaseFrame *Frame, NevercPhaseResult *OutResult,
                  void *UserData) {
  const NevercIRCoreAPI *Core = NULL;
  const NevercIRBuilderAPI *BuilderAPI = NULL;
  NevercIRGeneratePhaseInput Input;
  ...
  Input.Header = (NevercABITableHeader){sizeof(Input), NEVERC_IR_GEN_API_MAJOR,
                                        NEVERC_IR_GEN_API_MINOR, 0};
  Status = IRGenAPI->GetGeneratePhaseInput(IRGenAPI->Context, Frame,
                                           Frame->Input, &Input);
  if (Status.Code != NEVERC_STATUS_OK)
    return Status;

  Status = IRGenAPI->CreateModule(IRGenAPI->Context, Frame,
                                  STRING_VIEW("plugin-fixed-main"), &Core,
                                  &BuilderAPI);
  if (Status.Code == NEVERC_STATUS_OK)
  ...
  Descriptor.Header = (NevercABITableHeader){
      sizeof(Descriptor), NEVERC_IR_GEN_API_MAJOR, NEVERC_IR_GEN_API_MINOR, 0};
  Descriptor.Product =
      (NevercInterfaceID){NEVERC_PHASE_IR_GENERATE_OUTPUT_HIGH,
                          NEVERC_PHASE_IR_GENERATE_OUTPUT_LOW};
  Descriptor.DependencyDigest = Input.SourceDigest;
  Status = IRGenAPI->PublishModule(IRGenAPI->Context, Frame, &Descriptor,
                                   &Output);
  if (Status.Code != NEVERC_STATUS_OK)
    return Status;
...
register_plugin(const NevercCoreAPI *Core, const NevercRegistrarAPI *Registrar,
  ...
  Provider.Phase =
      (NevercInterfaceID){NEVERC_PHASE_IR_GENERATE_HIGH,
                          NEVERC_PHASE_IR_GENERATE_LOW};
  Provider.ProviderID = STRING_VIEW("neverc.test.irgen-provider");
  ...
  Provider.Callback = provide_ir_module;
...
NEVERC_EXPORT NevercStatus NEVERC_CALL neverc_plugin_entry(
  ...
  NevercInterfaceID Interface = {NEVERC_INTERFACE_IR_GEN_HIGH,
                                 NEVERC_INTERFACE_IR_GEN_LOW};
  ...
  Status = Bootstrap->QueryInterface(
      Bootstrap->Context, Interface, NEVERC_IR_GEN_API_MAJOR,
      NEVERC_IR_GEN_API_MINOR, &Table, &Minor, &StructSize);
  if (Status.Code != NEVERC_STATUS_OK)
    return Status;
  if (!Table ||
      StructSize < offsetof(NevercIRGenAPI, GetModuleArtifactInfo) +
                       sizeof(((NevercIRGenAPI *)0)->GetModuleArtifactInfo))
    return failure(NEVERC_STATUS_ABI_MISMATCH);
  IRGenAPI = (const NevercIRGenAPI *)Table;

  Capacity = OutPlugin->Header.StructSize;
  memset(&Descriptor, 0, sizeof(Descriptor));
  Descriptor.Header = (NevercABITableHeader){
      sizeof(Descriptor), NEVERC_PLUGIN_ABI_MAJOR, NEVERC_PLUGIN_ABI_MINOR, 0};
  Descriptor.PluginID = STRING_VIEW("org.neverc.test.irgen-provider");
  Descriptor.DisplayName = STRING_VIEW("NeverC IRGen provider test");
