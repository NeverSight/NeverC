...
// ============================================================================
// Variable Introspection: origin, value, flavor
// ============================================================================
...
TEST_F(BuildTest, OriginMultipleSources) {
  writeMakefile(
      "FILE_VAR = from-file\n"
      "all:\n"
      "\t@echo file=$(origin FILE_VAR) cmd=$(origin CMD_VAR) undef=$(origin UNDEF)\n"
      ".PHONY: all\n");
  auto R = runMake({"-n", "CMD_VAR=from-cmd"});
  ASSERT_TRUE(R.ok()) << R.err;
...
TEST_F(BuildTest, ValueReturnsUnexpanded) {
  writeMakefile(
      ...
      "RAW = $(value X)\n"
      "all:\n"
      "\t@echo raw=$(RAW) expanded=$(X)\n"
      ".PHONY: all\n");
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("expanded=hello")) << R.out;
}
...
TEST_F(BuildTest, FlavorReturnsType) {
  writeMakefile(
      ...
      "SIM := simple\n"
      "all:\n"
      "\t@echo rec=$(flavor REC) sim=$(flavor SIM) undef=$(flavor NOPE)\n"
      ".PHONY: all\n");
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
...
// ============================================================================
// MAKE_VERSION and MAKEFILE_LIST
// ============================================================================
...
TEST_F(BuildTest, MakefileListTracksIncludes) {
  writeFile(tmp() / "sub.mk", "SUB_VAR = from-sub\n");
  writeMakefile(
      "include sub.mk\n"
      "all:\n"
      "\t@echo list=$(MAKEFILE_LIST)\n"
  ...
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("sub.mk")) << R.out;
}
...
TEST_F(BuildTest, ExportBeforeDefine) {
  writeMakefile(
      "export LATE_VAR\n"
      "LATE_VAR = late-value\n"
      "all:\n"
  ...
  auto R = runMake();
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("late-value")) << R.out;
}
...
TEST_F(BuildTest, PosixDoubleColonAssign) {
  writeMakefile(
      "X ::= immediate-value\n"
      "all:\n"
      "\t@echo $(X)\n"
  ...
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("immediate-value")) << R.out;
}
...
TEST_F(BuildTest, FuncFileWriteAndRead) {
  writeMakefile(
      "$(file >tmpfile.txt,hello-file)\n"
      "CONTENT = $(file <tmpfile.txt)\n"
      "all:\n"
      "\t@echo $(CONTENT)\n"
      ".PHONY: all\n");
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("hello-file")) << R.out;
}

// ============================================================================
// Recipe Continuation and Edge Cases
// ============================================================================

TEST_F(BuildTest, RecipeCommentHashPreserved) {
  writeMakefile(
      "all:\n"
      "\t@echo hello # this is in recipe\n"
      ".PHONY: all\n");
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("hello #")) << "# in recipe should be preserved. out: " << R.out;
}
...
TEST_F(BuildTest, RecipeContinuationLine) {
  ...
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("hello") || R.contains("world")) << R.out;
}

// ============================================================================
// Computed Variable Names (Linux Kernel Pattern)
// ============================================================================

TEST_F(BuildTest, ComputedVarName) {
  writeMakefile(
      "quiet_cmd_cc = [CC] $@\n"
      "cmd_cc = gcc -c $< -o $@\n"
      "quiet := quiet_\n"
...
TEST_F(BuildTest, ComputedVarNameVerbose) {
  writeMakefile(
      "quiet_cmd_cc = [CC] $@\n"
      "cmd_cc = gcc -c $< -o $@\n"
      "quiet :=\n"
  ...
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("gcc -c")) << R.out;
}

// ============================================================================
// Foreach Variable Restore
// ============================================================================

TEST_F(BuildTest, ForeachVarRestore) {
  writeMakefile(
      "i = original\n"
      "X = $(foreach i,a b c,$(i)!)\n"
      "all:\n"
  ...
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("a! b! c!")) << R.out;
  EXPECT_TRUE(R.contains("i=original")) << "foreach should restore var. out: " << R.out;
}
...
TEST_F(BuildTest, OverrideAppendCmdLineVar) {
  writeMakefile(
      "override CFLAGS += -Wall\n"
      "all:\n"
      "\t@echo $(CFLAGS)\n"
      ".PHONY: all\n");
  auto R = runMake({"-n", "CFLAGS=-O2"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("-Wall")) << "override += should append. out: " << R.out;
}
...
TEST_F(BuildTest, AppendIgnoredOnCmdlineVar) {
  ...
  EXPECT_FALSE(R.contains("-Wall")) << "Normal += should not override cmd. out: " << R.out;
}
...
TEST_F(BuildTest, Stress30ModuleForeachEval) {
  std::string Mk;
  Mk += "MODULES =";
  for (int I = 0; I < 30; ++I)
    Mk += " m" + std::to_string(I);
  ...
  auto R = runMake({"-n"});
  ASSERT_TRUE(R.ok()) << R.err;
  EXPECT_TRUE(R.contains("build-m0")) << R.out;
  EXPECT_TRUE(R.contains("build-m29")) << R.out;
}
...
TEST_F(BuildTest, StressParallelFanout20) {
  ...
  ASSERT_TRUE(R.ok()) << R.err;
  for (int I = 1; I <= 20; ++I)
    EXPECT_TRUE(R.contains("done" + std::to_string(I))) << "Missing done" << I;
}

// ============================================================================
// Comprehensive Linux 5.10 Kernel Makefile Simulation
// ============================================================================

TEST_F(BuildTest, Linux510KernelKbuildSimulation) {
  std::string mk;
  mk += "VERSION = 5\n";
  mk += "PATCHLEVEL = 10\n";
  mk += "SUBLEVEL = 191\n";
  ...
  mk += "CONFIG_NET := y\n";
  mk += "\n";
  mk += "obj-y := init/ kernel/ mm/\n";
  mk += "obj-$(CONFIG_NET) += net/\n";
  mk += "obj-$(CONFIG_MODULES) += modules/\n";
  mk += "\n";
  mk += "vmlinux-dirs := $(patsubst %/,%,$(obj-y))\n";
  mk += "vmlinux-objs := $(addsuffix /built-in.o,$(vmlinux-dirs))\n";
  mk += "\n";
  ...
  mk += "\n";
  mk += "all: vmlinux\n";
  mk += "\t@echo Build complete [skip=$(SKIP_BUILD)]\n";
  mk += "\n";
  mk += "define build_dir\n";
  mk += "$(1)/built-in.o:\n";
  ...
  EXPECT_TRUE(R.contains("DIRS=5")) << R.out;
  EXPECT_TRUE(R.contains("OBJS=5")) << R.out;
  ...
  EXPECT_TRUE(R_verbose.contains("ld -o vmlinux")) << "verbose should show full link cmd: " << R_verbose.out;
}
