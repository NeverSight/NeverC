...
protected:
  ...
  MappingEvidence mapping(const std::string &ID = "cpp.math.fabs.f64.v1") {
    ...
    M.ID = ID;
    M.DeclarationID =
        "7e6cf33a987d325f52d0ab3b1874f755bfcaf33da0a63ca1e89ea904cf24bf61";
...
TEST_F(TranslateCppSdkTest, BuiltinSdkLoadsBothTargetsWithoutExternalInputs) {
  for (const std::string &Triple :
    ...
    EXPECT_EQ(Context.ApprovedFiles.size(), 533u);
    EXPECT_TRUE(
...
TEST_F(TranslateCppSdkTest,
  ...
  EXPECT_EQ(Object->getArray("headers")->size(), 533u);
...
TEST_F(TranslateCppSdkTest, BuiltinSdkApprovesOnlyCanonicalMathMappings) {
  ...
  Floor.DeclarationID =
      "7f892b44aee930e1cdff735d6fb3f45cca7cf518b7c1e82384b67d3d5d53e522";
