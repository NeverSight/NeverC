/* RUN: %clang_cc1 -std=c89 -verify=expected,c89only -pedantic -Wno-c11-extensions %s
   RUN: %clang_cc1 -std=c99 -verify=expected,c99andup -pedantic -Wno-c11-extensions %s
   RUN: %clang_cc1 -std=c11 -verify=expected,c99andup -pedantic %s
   RUN: %clang_cc1 -std=c17 -verify=expected,c99andup -pedantic %s
   RUN: %clang_cc1 -std=c2x -verify=expected,c99andup -pedantic %s
 */
...
_Static_assert(!__builtin_types_compatible_p(__typeof__(dr423_const), __typeof__(dr423_nonconst)), "const int(void) != int(void)");
...
void dr496(void) {
   ...
   */
   (void)__builtin_offsetof(struct S { int a; }, a); /* expected-warning{{defining a type within '__builtin_offsetof' is a NeverC extension}} */
}
