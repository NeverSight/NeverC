...
dr010_t dr010_a = {1};
dr010_t dr010_b = {1, 2};
int dr010_c = sizeof(dr010_t); /* expected-warning {{invalid application of 'sizeof' to an incomplete type 'dr010_t' (aka 'int[]')}} */
...
void dr011(void) {
  extern int i[];
  {
    ...
    extern int i[10];
    (void)sizeof(i);
    _Static_assert(sizeof(i) == 10 * sizeof(int), "fail"); /* c89only-warning {{mixing declarations and code is a C99 extension}} */
  }
  (void)sizeof(i); /* expected-warning {{invalid application of 'sizeof' to an incomplete type 'int[]'}} */

  extern int dr011_a[10]; /* c89only-warning {{mixing declarations and code is a C99 extension}} */
  (void)sizeof(dr011_a);
  ...
  extern int j[10];
  {
    extern int j[];
    (void)sizeof(j);
    _Static_assert(sizeof(j) == 10 * sizeof(int), "fail"); /* c89only-warning {{mixing declarations and code is a C99 extension}} */
  }
}
 ...
 */
_Static_assert(!__builtin_types_compatible_p(struct S { int a; }, union U { int a; }), "struct and union are not compatible");
...
void dr053(void) {
  ...
  fp1 = f;
  fp2 = fp1;  /* expected-warning {{incompatible function pointer types assigning to 'int (*)()' from 'int (*)(int)'}} */
