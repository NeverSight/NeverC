 ...
 *
 * NeverC adaption:
 *  - "auto int" gives "'auto' storage class specifier is not allowed"
 *  - "NeverC extension" → "NeverC extension"
 */
void test_auto_int(void) {
  auto int auto_int = 12; // expected-error {{'auto' storage class specifier is not allowed}}
}
...
void test_typedefs(void) {
  ...
  typedef void (*fp2)(auto); // expected-error {{'auto' not allowed in function prototype}}
...
void test_misc(void) {
  ...
  auto test_char_ptr2[] = "another test";   // expected-warning {{type inference of a declaration other than a plain identifier with optional trailing attributes is a NeverC extension}}
...
void test_pointers(void) {
  int a;
  auto *ptr = &a; // expected-warning {{type inference of a declaration other than a plain identifier with optional trailing attributes is a NeverC extension}}
  auto *ptr2 = a; // expected-error {{variable 'ptr2' with type 'auto *' has incompatible initializer of type 'int'}} \
                     expected-warning {{type inference of a declaration other than a plain identifier with optional trailing attributes is a NeverC extension}}
  auto nptr = nullptr;
