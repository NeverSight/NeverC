/*===-- ReplacementConformancePlugin.c - custom backend conformance ------===*\
|*                                                                            *|
|* A full custom target ("neverc-test-none") plus ".nobj" object format,      *|
|* compiled by the *system* C compiler against ONLY the public SDK single     *|
|* header. It proves, through the public CLI, that a plugin can replace the    *|
|* whole codegen backend (C -> custom Object) and the assembly parser         *|
|* (.s -> custom Object) with a deterministic, black-box byte format the      *|
|* built-in backend would never emit. The body is the shared registry fixture *|
|* used by the in-tree tests, retargeted to the single public header so the   *|
|* same proven backend runs from the installed SDK.                           *|
\*===----------------------------------------------------------------------===*/
...
/* Hard-wire the custom backend identity so no -D toggles are required. */
#define NEVERC_TEST_PLUGIN_ID "com.neverc.conformance.replacement"
#define NEVERC_TEST_PLUGIN_DISPLAY_NAME "Conformance Replacement Backend"
#define NEVERC_TEST_REGISTER_TARGET 1
#define NEVERC_TEST_REGISTER_NOBJ_BACKEND 1
#define NEVERC_TEST_TARGET_ID_LOW 42
#define NEVERC_TEST_TARGET_NAME "neverc-test"
#define NEVERC_TEST_TARGET_RAW_TRIPLE "neverc-test-none"
#define NEVERC_TEST_TARGET_ARCHITECTURE "neverc"
#define NEVERC_TEST_CONCURRENCY NEVERC_CONCURRENCY_THREAD_SAFE
