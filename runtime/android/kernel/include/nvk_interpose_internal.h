...
/*
 * nvk_interpose_internal.h — Advanced interpose API: context interposes, batch ops,
 * kCFI thunks, fptr replacement, ftrace/kprobe fallbacks, and scan.
 *
 * Included automatically via <nvk.h>.  For the basic interpose API
 * (install/remove, register/unregister, re-entry guard), see
 * <nvk_interpose.h>.
 */
#ifndef NEVERC_KRT_INTERPOSE_INTERNAL_H
#define NEVERC_KRT_INTERPOSE_INTERNAL_H
...
/*
 * ftrace-based interpose (fallback).
 *
 * 6.18+: register_ftrace_function / unregister_ftrace_function /
 * ftrace_set_filter_ip were removed from the kernel symbol table.
 * neverc_krt_ftrace_init() returns -1 and neverc_krt_interpose_auto()
 * skips the ftrace path.  Inline patching and kprobes remain
 * available as interpose methods on 6.18.
 */
...
/* ---- kprobe-based interpose (fallback) ---- */

int neverc_krt_kprobe_interpose_init(void);

#endif /* NEVERC_KRT_INTERPOSE_INTERNAL_H */
