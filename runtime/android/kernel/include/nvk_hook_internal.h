...
/*
 * nvk_hook_internal.h — Advanced hook API: context hooks, batch ops,
 * kCFI thunks, fptr replacement, ftrace/kprobe fallbacks, and scan.
 *
 * Included automatically via <nvk.h>.  For the basic hook API
...
/* ---- Context hook (probe-style with full register context) ---- */
 ...
 * neverc_krt_ftrace_init() returns -1 and neverc_krt_hook_auto()
 * skips the ftrace path.  Inline patching and kprobes remain
 * available as hook methods on 6.18.
 */
