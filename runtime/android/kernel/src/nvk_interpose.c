	...
	if (h->stub) {
		...
		if (h->tramp_code)
			_neverc_krt_poison_tramp(h->tramp_code,
					  NEVERC_KRT_INTERPOSE_TRAMP_CAP);
		_neverc_krt_quiesce();
				      ...
				      int count)
{
	int i, ok = 0;
	for (i = 0; i < count; i++) {
		batch[i].result = neverc_krt_interpose_install_ctx(
	...
	if (ok > 0 && ok < count) {
		for (i = 0; i < count; i++) {
			if (batch[i].result == NEVERC_KRT_INTERPOSE_OK)
				neverc_krt_interpose_remove_ctx(batch[i].interpose);
		}
		return -1;
	}
	return ok == count ? 0 : -1;
...
int neverc_krt_interpose_install_batch(struct neverc_krt_interpose_batch *batch, int count)
{
	int i, ok = 0;
	for (i = 0; i < count; i++) {
		batch[i].result = neverc_krt_interpose_install(
			batch[i].interpose, batch[i].target,
	...
	if (ok > 0 && ok < count) {
		for (i = 0; i < count; i++) {
			if (batch[i].result == NEVERC_KRT_INTERPOSE_OK)
				neverc_krt_interpose_remove(batch[i].interpose);
		}
		return -1;
	}
	return ok == count ? 0 : -1;
...
{
	if (h && h->active)
		neverc_krt_interpose_remove(h);
	if (ft_fallback && ft_fallback->active)
		neverc_krt_ftrace_interpose_remove(ft_fallback);
}
			     ...
			     void **orig, struct neverc_krt_interpose_ref *ref)
{
	...
	ret = neverc_krt_interpose_install_ctx(&_neverc_krt_reg_ctxs[slot],
					   target,
...
static int _neverc_krt_probe_install_slot(int slot)
{
	...
	return neverc_krt_interpose_install_ctx(&_neverc_krt_probe_ctxs[slot],
					   ps->addr,
...
{
	neverc_krt_interpose_remove_ctx(&_neverc_krt_probe_ctxs[slot]);
}
