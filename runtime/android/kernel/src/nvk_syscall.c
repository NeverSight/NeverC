			       ...
			       neverc_krt_syscall_fn_t *orig)
{
	...
	*orig = _neverc_krt_sys_call_table[nr];
	unsigned long entry = (unsigned long)&_neverc_krt_sys_call_table[nr];

	return neverc_krt_mem_write_protected(entry, &new_fn, sizeof(new_fn));
}
...
int neverc_krt_syscall_restore(int nr, neverc_krt_syscall_fn_t orig)
{
	...
	return neverc_krt_mem_write_protected(entry, &orig, sizeof(orig));
}
