...
/* nvk_hook.c — implementations extracted from nvk_hook.h. */
#include <nvk.h>
...
void _nvk_pool_free(u32 *ptr)
{
	...
	unsigned long flags;
	if (!ptr) return;
	if (!_nvk_is_kern_ptr((unsigned long)ptr)) return;
...
void _nvk_write_insn(void *addr, u32 insn)
{
	...
	need_prot = nvk_mem_make_rw((unsigned long)addr);
...
int _nvk_patch_multi(u32 *target, u32 *insns, int count)
{
	...
	if (_nvk_patchtext && count <= 16) {
		...
		if (ret == 0 && _nvk_verify_patch(target, insns, count) == 0)
			return 0;
	}
			    ...
			    void *replace, void **orig)
{
	...
	if (!_nvk_is_kern_ptr((unsigned long)target))
	...
	if (_nvk_flushic)
		...
		_nvk_icache_inval((unsigned long)h->trampoline,
				  (unsigned long)&h->trampoline[tidx]);
				...
				nvk_ctx_handler_t handler, void **call_orig)
{
...

	{
		...
		if (_nvk_flushic)
			...
			_nvk_icache_inval((unsigned long)page, flush_end);
	}
			    ...
			    void **new_orig)
{
...

	{
		int tampered = 0;
		u32 cur;
		int vslot = nvk_a64_is_bti(h->orig_insns[0]) ? 1 : 0;
		if (!nvk_mem_read(&cur, &code[vslot], 4)) {
			if (cur != 0x58000050U) {
				long b_off = (long)(unsigned long)h->replace -
					(long)(unsigned long)&code[vslot];
				u32 expected_b = nvk_a64_gen_b(b_off);
	...
	if (h->stub) {
		...
		if (h->tramp_code)
			_nvk_poison_tramp(h->tramp_code,
					  NVK_HOOK_TRAMP_CAP);
		_nvk_quiesce();
		if (_nvk_modfree) _nvk_modfree(h->stub);
		h->stub = (void *)0;
		h->tramp_code = (void *)0;
		h->base.trampoline = (void *)0;
	}
...
{
	if (!ops || !regs) return;
	struct nvk_ftrace_hook *h = (struct nvk_ftrace_hook *)(
		(char *)ops - __builtin_offsetof(struct nvk_ftrace_hook, ops));
	if (!h->replace) return;
