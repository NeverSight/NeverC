...
static unsigned long _nvk_off_vm_flags;
...
static void _nvk_detect_vm_flags_off(const void *vma)
{
...
static void _nvk_read_vma_info(const void *vma, struct nvk_vma_info *info)
{
	const unsigned long *v = (const unsigned long *)vma;
	info->start = v[0];
	info->end   = v[1];
