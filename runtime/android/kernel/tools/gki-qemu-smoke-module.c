 ...
 * the init/exit pointers at the configured struct module offsets.  NeverC
 * emits the release-derived KCFI type-id word immediately before each entry
 * symbol on profiles whose pinned module carries that entry ABI.
 */
#include <nvkmod.h>
