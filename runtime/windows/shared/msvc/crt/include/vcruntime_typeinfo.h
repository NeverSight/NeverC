...
extern "C++" { // attach declarations to the global module, see N4910
               // [module.unit]/7
...
_VCRTIMP int __cdecl
__std_type_info_compare(_In_ const __std_type_info_data *_Lhs,
                        _In_ const __std_type_info_data *_Rhs);

_VCRTIMP size_t __cdecl
__std_type_info_hash(_In_ const __std_type_info_data *_Data);

_VCRTIMP const char *__cdecl
__std_type_info_name(_Inout_ __std_type_info_data *_Data,
...
_VCRT_EXPORT_STD class type_info // Exported because for typeid, MSVC looks for
                                 // type_info in the global namespace
{
public:
...
#ifdef _M_CEE_PURE
    return __std_type_info_name(&_Data, static_cast<__type_info_node *>(
                                            __type_info_root_node.ToPointer()));
#else
...
#pragma warning(                                                               \
    disable                                                                    \
    : 4577) // 'noexcept' used with no exception handling mode specified
_VCRT_EXPORT_STD class _NODISCARD bad_cast : public exception {
public:
  bad_cast() noexcept : exception("bad cast", 1) {}

  static bad_cast
  __construct_from_string_literal(const char *const _Message) noexcept {
...
_VCRT_EXPORT_STD class _NODISCARD bad_typeid : public exception {
public:
...
class _NODISCARD __non_rtti_object : public bad_typeid {
public:
  static __non_rtti_object
  __construct_from_string_literal(const char *const _Message) noexcept {
