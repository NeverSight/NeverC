...
public:
  _CancellationTokenRegistration(long _InitialRefs = 1)
      : _RefCounter(_InitialRefs), _M_state(_STATE_CALLED), _M_signaled(false),
...
private:
  ...
  void _Invoke() {
    ...
    _ASSERTE((_Tid & 0x3) ==
             0); // If this ever fires, we need a different encoding for this.
...
class CancellationTokenRegistration_TaskProc
    : public _CancellationTokenRegistration {
public:
  CancellationTokenRegistration_TaskProc(TaskProc_t _Proc, _In_ void *_PData,
                                         int _InitialRefs)
      : _CancellationTokenRegistration(_InitialRefs), _M_proc(_Proc),
        _M_pData(_PData) {}
  ...
  private:
    typedef struct _Node {
      ...
      _Node(_CancellationTokenRegistration *_Token)
          : _M_token(_Token), _M_next(nullptr) {}
    } Node;
...
public:
  ...
  _CancellationTokenRegistration *_RegisterCallback(TaskProc_t _PCallback,
                                                    _In_ void *_PData,
                                                    int _InitialRefs = 1) {
    _CancellationTokenRegistration *_PRegistration =
        new CancellationTokenRegistration_TaskProc(_PCallback, _PData,
                                                   _InitialRefs);
  ...
  void
  _DeregisterCallback(_In_ _CancellationTokenRegistration *_PRegistration) {
...

    {
      ...
      //
      // If a cancellation has occurred, the registration list is guaranteed to
      // be empty if we've observed it under the auspices of the lock.  In this
      // case, we must synchronize with the cancelling thread to guarantee that
      // the cancellation is finished by the time we return from this method.
      //
    ...
    // - The callback is about to be made           --> flag it so it doesn't
    // happen and return
    ...
    if (_Synchronize) {
      ...
      default: {
        if (_Result == ::Concurrency::details::platform::GetCurrentThreadId()) {
          //
          // It is entirely legal for a caller to Deregister during a callback
          // instead of having to provide their own synchronization mechanism
          // between the two.  In this case, we do *NOT* need to explicitly
          // synchronize with the callback as doing so would deadlock.  If the
          // call happens during, skip any extra synchronization.
          //
          break;
        }

        const long _Result_1 =
            atomic_exchange(_PRegistration->_M_state,
                            _CancellationTokenRegistration::_STATE_SYNCHRONIZE);

        if (_Result_1 != _CancellationTokenRegistration::_STATE_CALLED) {
          ::std::unique_lock<::std::mutex> _Lock(_PRegistration->_M_Mutex);
          _PRegistration->_M_CondVar.wait(
              _Lock, [_PRegistration] { return _PRegistration->_M_signaled; });
...
/// <summary>
///     The <c>cancellation_token_registration</c> class represents a callback
///     notification from a <c>cancellation_token</c>.  When the <c>register</c>
///     method on a <c>cancellation_token</c> is used to receive notification of
///     when cancellation occurs, a <c>cancellation_token_registration</c>
///     object is returned as a handle to the callback so that the caller can
///     request a specific callback no longer be made through use of the
...
public:
  ...
  cancellation_token_registration(
      const cancellation_token_registration &_Src) noexcept {
    _Assign(_Src._M_pRegistration);
  }

  cancellation_token_registration(
      cancellation_token_registration &&_Src) noexcept {
    _Move(_Src._M_pRegistration);
  }

  cancellation_token_registration &
  operator=(const cancellation_token_registration &_Src) noexcept {
    if (this != &_Src) {
      _Clear();
      _Assign(_Src._M_pRegistration);
    }
  ...
  cancellation_token_registration &
  operator=(cancellation_token_registration &&_Src) noexcept {
    if (this != &_Src) {
      _Clear();
      _Move(_Src._M_pRegistration);
    }
...
private:
  ...
  cancellation_token_registration(
      _In_ details::_CancellationTokenRegistration *_PRegistration) noexcept
...
///     whether some operation has been requested to cancel.  A given token can
///     be associated with a <c>task_group</c>, <c>structured_task_group</c>, or
///     <c>task</c> to provide implicit cancellation.  It can also be polled for
///     cancellation or have a callback registered for if and when the
///     associated <c>cancellation_token_source</c> is canceled.
/// </summary>
class cancellation_token {
public:
  ...
  /// <summary>
  ///     Registers a callback function with the token.  If and when the token
  ///     is canceled, the callback will be made.  Note that if the token is
  ///     already canceled at the point where this method is called, the
  ///     callback will be made immediately and synchronously.
  /// </summary>
  /// <typeparam name="_Function">
  ///     The type of the function object that will be called back when this
  ///     <c>cancellation_token</c> is canceled.
  /// </typeparam>
  /// <param name="_Func">
  ///     The function object that will be called back when this
  ///     <c>cancellation_token</c> is canceled.
  /// </param>
  /// <returns>
  ///     A <c>cancellation_token_registration</c> object which can be utilized
  ///     in the <c>deregister</c> method to deregister a previously registered
  ///     callback and prevent it from being made. The method will throw an <see
  ///     cref="invalid_operation Class">invalid_operation </see> exception if
  ///     it is called on a <c>cancellation_token</c> object that was created
  ///     using the <see cref="cancellation_token::none
  ///     Method">cancellation_token::none </see> method.
  /// </returns>
  template <typename _Function>
  ::Concurrency::cancellation_token_registration
  register_callback(const _Function &_Func) const {
    if (_M_Impl == nullptr) {
      // A callback cannot be registered if the token does not have an
      // associated source.
...
#pragma warning(suppress : 28197)
  ...
  /// <summary>
  ///     Removes a callback previously registered via the <c>register</c>
  ///     method based on the <c>cancellation_token_registration</c> object
  ///     returned at the time of registration.
  /// </summary>
  /// <param name="_Registration">
  ///     The <c>cancellation_token_registration</c> object corresponding to the
  ///     callback to be deregistered.  This token must have been previously
  ///     returned from a call to the <c>register</c> method.
  /// </param>
  void deregister_callback(
      const cancellation_token_registration &_Registration) const {
...
public:
  ...
  cancellation_token_source &
  operator=(cancellation_token_source &&_Src) noexcept {
    if (this != &_Src) {
      _Clear();
      _Move(_Src._M_Impl);
    }
  ...
  /// <summary>
  ///     Creates a <c>cancellation_token_source</c> which is canceled when the
  ///     provided token is canceled.
  /// </summary>
  /// <param name="_Src">
  ///     A token whose cancellation will cause cancellation of the returned
  ///     token source.  Note that the returned token source can also be
  ///     canceled independently of the source contained in this parameter.
  /// </param>
  /// <returns>
  ///     A <c>cancellation_token_source</c> which is canceled when the token
  ///     provided by the <paramref name="_Src"/> parameter is canceled.
  /// </returns>
  static cancellation_token_source
  create_linked_source(cancellation_token &_Src) {
  ...
  /// <summary>
  ///     Creates a <c>cancellation_token_source</c> which is canceled when one
  ///     of a series of tokens represented by an STL iterator pair is canceled.
  /// </summary>
  /// <param name="_Begin">
  ///     The STL iterator corresponding to the beginning of the range of tokens
  ///     to listen for cancellation of.
  /// </param>
  /// <param name="_End">
  ///     The STL iterator corresponding to the ending of the range of tokens to
  ///     listen for cancellation of.
  /// </param>
  /// <returns>
  ///     A <c>cancellation_token_source</c> which is canceled when any of the
  ///     tokens provided by the range described by the STL iterators contained
  ///     in the <paramref name="_Begin"/> and <paramref name="_End"/>
  ///     parameters is canceled.
  /// </returns>
  template <typename _Iter>
  static cancellation_token_source create_linked_source(_Iter _Begin,
                                                        _Iter _End) {
    cancellation_token_source _NewSource;
