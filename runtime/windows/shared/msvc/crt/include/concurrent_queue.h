...
private:
  ...
  virtual void _Assign_and_destroy_item(void *_Dst, _Page &_Src,
                                        size_t _Index) = 0;
...
struct _Micro_queue {
  ...
  void _Push(void *_Item, _Ticket _K, _Concurrent_queue_base &_Base,
             void (_Concurrent_queue_base::*moveOp)(
                 _Concurrent_queue_base_v4::_Page &, size_t, void *));
...
public:
  ...
  static size_t _Index(_Ticket _K) { return _K * _Phi % _N_queue; }
...
class _Concurrent_queue_iterator_base_v4 {
  ...
  friend bool operator==(const _Concurrent_queue_iterator<_C, _Ty> &,
                         const _Concurrent_queue_iterator<_C, _U> &);
...
protected:
  ...
  // Default constructor
  _Concurrent_queue_iterator_base_v4() : _My_rep(nullptr), _My_item(nullptr) {}

  // Copy constructor
  _Concurrent_queue_iterator_base_v4(
      const _Concurrent_queue_iterator_base_v4 &_I)
...
public:
  ...
  /** If _Value==_Container::value_type, then this routine is the copy
     constructor. If _Value==const _Container::value_type, then this routine is
     a conversion constructor. */
  _Concurrent_queue_iterator(
      const _Concurrent_queue_iterator<_Container,
                                       typename _Container::value_type> &_Other)
      : _Concurrent_queue_iterator_base_v4(_Other) {}

  // Iterator assignment
  _Concurrent_queue_iterator &
  operator=(const _Concurrent_queue_iterator &_Other) {
    _Assign(_Other);
    return *this;
  }

  // Reference to current item
  _Value &operator*() const { return *static_cast<_Value *>(_My_item); }
...
template <typename _C, typename _Ty, typename _U>
bool operator==(const _Concurrent_queue_iterator<_C, _Ty> &_I,
...
/// <summary>
///     The <c>concurrent_queue</c> class is a sequence container class that
///     allows first-in, first-out access to its elements. It enables a limited
///     set of concurrency-safe operations, such as <c>push</c> and
///     <c>try_pop</c>.
/// </summary>
/// <typeparam name="_Ty">
...
///     details about the allocation and deallocation of memory for this
///     concurrent queue. This argument is optional and the default value is
...
template <typename _Ty, class _Ax>
class concurrent_queue
    : public ::Concurrency::details::_Concurrent_queue_base_v4 {
  template <typename _Container, typename _Value>
  friend class ::Concurrency::details::_Concurrent_queue_iterator;

  // allocator type
  typedef typename ::std::allocator_traits<_Ax>::template rebind_alloc<char>
  ...
  private:
    ...
    void operator=(const _Destroyer &); // prevent warning: assign operator
                                        // can't be generated
  public:
    _Destroyer(_Ty &_Value) : _My_value(_Value) {}

    ~_Destroyer() { _My_value.~_Ty(); }
  };
  ...
  /*override*/ virtual void _Copy_item(_Page &_Dst, size_t _Index,
                                       const void *_Src) {
  ...
  /*override*/ virtual void _Assign_and_destroy_item(void *_Dst, _Page &_Src,
                                                     size_t _Index) {
...
public:
  ...
  ///     concurrent queue for reading and performing <c>const</c> operations.
  /// </summary>
  /**/
  ...
  ///     initialize the queue. <para>The first constructor specifies an empty
  ///     initial queue and explicitly specifies the allocator type to be
  ///     used.</para> <para>The second constructor specifies a copy of the
  ///     concurrent queue <paramref name="_OtherQ"/>.</para> <para>The third
  ///     constructor specifies a move of the concurrent queue <paramref
  ///     name="_OtherQ"/>.</para> <para>The fourth constructor specifies values
  ///     supplied by the iterator range
  ...
  /// </summary>
  /// <param name="_OtherQ">
  ///     The source <c>concurrent_queue</c> object to copy or move elements
  ///     from.
  /// </param>
  /// <param name="_Al">
  ///     The allocator class to use with this object.
  ...
  /// </remarks>
  /**/
  concurrent_queue(const concurrent_queue &_OtherQ,
  ...
  /// </remarks>
  /**/
  concurrent_queue(concurrent_queue &&_OtherQ,
                   const allocator_type &_Al = allocator_type());
  ...
  concurrent_queue(_InputIterator _Begin, _InputIterator _End)
      : _Concurrent_queue_base_v4(sizeof(_Ty)),
        _My_allocator(allocator_type()) {
    while (_Begin != _End) {
      this->push(*_Begin);
      ++_Begin;
    }
  ...
  /// <summary>
  ///     Enqueues an item at tail end of the concurrent queue. This method is
  ///     concurrency-safe.
  /// </summary>
  /// <param name="_Src">
  ...
  /// </param>
  /// <remarks>
  ///     <c>push</c> is concurrency-safe with respect to calls to the methods
  ///     <c>push</c>, <c>try_pop</c>, and <c>empty</c>.
  /// </remarks>
  /**/
  void push(const _Ty &_Src) { _Internal_push(&_Src); }
  ...
  /// </remarks>
  /**/
  void push(_Ty &&_Src) { _Internal_move_push(&_Src); }

  /// <summary>
  ///     Dequeues an item from the queue if one is available. This method is
  ///     concurrency-safe.
  /// </summary>
  /// <param name="_Dest">
  ...
  /// </param>
  /// <returns>
  ///     <c>true</c> if an item was successfully dequeued,<c>false</c>
  ///     otherwise.
  /// </returns>
  /// <remarks>
  ///     If an item was successfully dequeued, the parameter <paramref
  ///     name="_Dest"/> receives the dequeued value, the original value held in
  ///     the queue is destroyed, and this function returns <c>true</c>. If
  ///     there was no item to dequeue, this function returns <c>false</c>
  ///     without blocking, and the contents of the <paramref name="_Dest"/>
  ///     parameter are undefined. <para><c>try_pop</c> is concurrency-safe with
  ///     respect to calls to the methods <c>push</c>, <c>try_pop</c>, and
  ///     <c>empty</c>.</para>
  /// </remarks>
  /**/
  bool try_pop(_Ty &_Dest) { return _Internal_pop_if_present(&_Dest); }

  /// <summary>
  ///     Returns the number of items in the queue. This method is not
  ///     concurrency-safe.
  /// </summary>
  /// <returns>
  ...
  /// </returns>
  /// <remarks>
  ///     <c>unsafe_size</c> is not concurrency-safe and can produce incorrect
  ///     results if called concurrently with calls to the methods <c>push</c>,
  ///     <c>try_pop</c>, and <c>empty</c>.
  /// </remarks>
  /**/
  size_type unsafe_size() const { return _Internal_size(); }

  /// <summary>
  ///     Tests if the concurrent queue is empty at the moment this method is
  ///     called. This method is concurrency-safe.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the concurrent queue was empty at the moment we looked,
  ///     <c>false</c> otherwise.
  /// </returns>
  /// <remarks>
  ///     While this method is concurrency-safe with respect to calls to the
  ///     methods <c>push</c>, <c>try_pop</c>, and <c>empty</c>, the value
  ///     returned might be incorrect by the time it is inspected by the calling
  ///     thread.
  /// </remarks>
  /**/
  bool empty() const { return _Internal_empty(); }

  /// <summary>
  ///     Returns a copy of the allocator used to construct the concurrent
  ///     queue. This method is concurrency-safe.
  /// </summary>
  /// <returns>
  ...
  /// <summary>
  ///     Clears the concurrent queue, destroying any currently enqueued
  ///     elements. This method is not concurrency-safe.
  /// </summary>
  /**/
  void clear();

  /// <summary>
  ///     A type that represents a non-thread-safe iterator over the elements in
  ///     a concurrent queue.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     A type that represents a non-thread-safe <c>const</c> iterator over
  ///     elements in a concurrent queue.
  /// </summary>
  /**/
  typedef details::_Concurrent_queue_iterator<concurrent_queue, const _Ty>
      const_iterator;

  /// <summary>
  ///     Returns an iterator of type <typeparamref name="iterator"/> or
  ///     <typeparamref name="const_iterator"/> to the beginning of the
  ///     concurrent queue. This method is not concurrency-safe.
  /// </summary>
  /// <returns>
  ///     An iterator of type <typeparamref name="iterator"/> or <typeparamref
  ///     name="const_iterator"/> to the beginning of the  concurrent queue
  ///     object.
  /// </returns>
  /// <remarks>
  ///     The iterators for the <c>concurrent_queue</c> class are primarily
  ///     intended for debugging, as they are slow, and iteration is not
  ///     concurrency-safe with respect to other queue operations.
  /// </remarks>
  /**/
  ...
  ///     queue. This method is not concurrency-safe.
  /// </summary>
  /// <returns>
  ///     An iterator of type <typeparamref name="iterator"/> or <typeparamref
  ///     name="const_iterator"/> to the end of the concurrent queue.
  /// </returns>
  /// <remarks>
  ///     The iterators for the <c>concurrent_queue</c> class are primarily
  ///     intended for debugging, as they are slow, and iteration is not
...
/**/
template <typename _Ty, class _Ax>
concurrent_queue<_Ty, _Ax>::concurrent_queue(const concurrent_queue &_Queue,
                                             const allocator_type &_Al)
    : _Concurrent_queue_base_v4(sizeof(_Ty)), _My_allocator(_Al) {
  concurrent_queue::const_iterator _QEnd = _Queue.unsafe_end();
  for (concurrent_queue::const_iterator _It = _Queue.unsafe_begin();
       _It != _QEnd; ++_It)
    this->push(*_It);
}
...
/**/
template <typename _Ty, class _Ax>
concurrent_queue<_Ty, _Ax>::concurrent_queue(concurrent_queue &&_Queue,
                                             const allocator_type &_Al)
    : _Concurrent_queue_base_v4(sizeof(_Ty)), _My_allocator(_Al) {
...
/// </summary>
/**/
template <typename _Ty, class _Ax> void concurrent_queue<_Ty, _Ax>::clear() {
  while (!empty()) {
