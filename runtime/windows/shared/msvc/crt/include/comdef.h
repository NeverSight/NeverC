...
#endif /* defined WINAPI_FAMILY_PHONE_APP &&                                   \
          WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_PHONE_APP) */
#endif /* WINAPI_FAMILY */
...
public:
  _com_error(HRESULT hr, const wchar_t *message)
  ...
  ~_com_error() throw() { delete[] m_message; }
  ...
  HRESULT Error() const throw() { return m_hr; }

  const wchar_t *ErrorMessage() const throw() {
    if (m_message == nullptr) {
      wchar_t buffer[4096];
      if (::FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, m_hr,
                           MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), buffer,
...
inline void __declspec(noreturn) __stdcall
_com_raise_error(HRESULT hr, const wchar_t *message) {
  ...
  ::Windows::Foundation::Diagnostics::OriginateError(
      hr, static_cast<unsigned int>(length), message);
...
inline void __stdcall _com_issue_error(HRESULT hr) { __errorPfn(hr, nullptr); }

inline void __stdcall
_set_com_error_handler(void(__stdcall *pHandler)(HRESULT, const wchar_t *)) {
  __errorPfn = pHandler;
}
...
HRESULT __stdcall _com_dispatch_raw_propget(IDispatch *, DISPID, VARTYPE,
                                            void *) throw();
  ...
  if (m_pszMsg == NULL) {
    ...
    if (m_pszMsg != NULL) {
      ...
      if (m_pszMsg != NULL) {
        WORD wCode = WCode();
        if (wCode != 0) {
          _COM_PRINTF_S_1(m_pszMsg, 32, TEXT("IDispatch error #%d"),
                          (int)wCode);
        } else {
