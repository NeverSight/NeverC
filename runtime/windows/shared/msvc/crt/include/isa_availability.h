 ...
 * these defines identify performance features that are enabled in the
 * processor. The compiler backend can use these to enable processor specific
 * optimizations.
 */
...
enum ISA_AVAILABILITY {
  ...
  __ISA_AVAILABLE_NEON_ARM64 =
      2, // support for 128-bit NEON instructions for ARM64. The distinction
         // between ARM32 and ARM64 NEON is temporary. They may eventually be
         // merged.
};

#if defined(_M_IX86) || defined(_M_X64)

// Inverted ISA extension bits are defined by macros rather than enum so that
// types are correct The lowest byte has flags that should be especially useful
// for code that must run on legacy processors.
#define __IA_SUPPORT_VECTOR128                                                 \
  0x00000001 // Vector lengths up to 128 bits supported (SSE2    if
             // __avx10_version = 0)
#define __IA_SUPPORT_VECTOR256                                                 \
  0x00000002 // Vector lengths up to 256 bits supported (AVX2    if
             // __avx10_version = 0)
#define __IA_SUPPORT_VECTOR512                                                 \
  0x00000004 // Vector lengths up to 512 bits supported (AVX-512 if
             // __avx10_version = 0)
#define __IA_SUPPORT_AVX10 0x00000008 // AVX10 support
#define __IA_SUPPORT_SSE42 0x00000010 // SSE4.2 support (scalar and 128-bit)
#define __IA_SUPPORT_SV128X                                                    \
  0x00000020 // AVX512 or AVX10.1 support (scalar and 128-bit)
#define __IA_SUPPORT_AVX10_2                                                   \
  0x00000040 // AVX10.2 128-bit support (scalar and 128-bit)
