...
//     (the std::invoke function only; other components like bind and
//     reference_wrapper are C++20 only)
// P1164R1 Making create_directory() Intuitive
// P1165R1 Consistently Propagating Stateful Allocators In basic_string's
// operator+() P1902R1 Missing Feature-Test Macros 2017-2019 P2013R5
...
// P0218R1 <filesystem>
// P0220R1 <any>, <memory_resource>, <optional>, <string_view>, apply(),
// sample(), Boyer-Moore search() P0226R1 Mathematical Special Functions P0253R1
// Fixing Searcher Return Types P0254R2 Integrating string_view And std::string
...
// P0510R0 Rejecting variants Of Nothing, Arrays, References, And Incomplete
// Types P0602R4 Propagating Copy/Move Triviality In variant/optional P0604R0
// invoke_result, is_invocable, is_nothrow_invocable P0607R0 Inline Variables
// For The STL P0608R3 Improving variant's Converting Constructor/Assignment
...
// P2407R5 Freestanding Library: Partial Classes
//     (including __cpp_lib_freestanding_optional,
//     __cpp_lib_freestanding_string_view, and
...
// Implemented when char8_t is available (C++14/17 with /Zc:char8_t, C++20
// without /Zc:char8_t-): P0482R6 Library Support For char8_t
...
// P1870R1 Rename forwarding-range To borrowed_range (Was safe_range before
// LWG-3379) P1871R1 disable_sized_sentinel_for P1872R0 span Should Have
// size_type, Not index_type P1878R1 Constraining Readable Types P1907R2
// ranges::ssize P1956R1 <bit> has_single_bit(), bit_ceil(), bit_floor(),
// bit_width() P1959R0 Removing weak_equality And strong_equality P1960R0
// atomic_ref Cleanup P1964R2 Replacing boolean With boolean-testable P1973R1
// Renaming default_init To for_overwrite P1976R2 Explicit Constructors For
// Fixed-Extent span From Dynamic-Extent Ranges P1983R0 Fixing Minor Ranges
// Issues P1994R1 elements_view Needs Its Own sentinel P2017R1 Conditionally
// Borrowed Ranges P2091R0 Fixing Issues With Range Access CPOs P2102R0 Making
// "Implicit Expression Variations" More Explicit P2106R0 Range Algorithm Result
// Types P2116R0 Removing tuple-Like Protocol Support From Fixed-Extent span
...
// Invocable Concepts P3136R1 Retiring Niebloids P3323R1 Forbid atomic<cv T>,
// Specify atomic_ref<cv T>
//     (for atomic_ref<cv T>)
...
//     (only the <stdfloat> header; we don't support any optional extended
...
// P2166R1 Prohibiting basic_string And basic_string_view Construction From
// nullptr P2186R2 Removing Garbage Collection Support P2273R3 constexpr
// unique_ptr P2278R4 cbegin Should Always Return A Constant Iterator P2286R8
// Formatting Ranges P2291R3 constexpr Integral <charconv> P2302R4
// ranges::contains, ranges::contains_subrange P2321R2 zip P2322R6
// ranges::fold_left, ranges::fold_right, Etc. P2374R4 views::cartesian_product
...
// P2836R1 basic_const_iterator Should Follow Its Underlying Type's
// Convertibility P3107R5 Permit An Efficient Implementation Of <print> P3142R0
// Printing Blank Lines With println() P3235R3 std::print More Types Faster With
// Less Memory
//     (partial implementation; see GH-4924)

// _HAS_CXX23 and _SILENCE_ALL_CXX23_DEPRECATION_WARNINGS control:
// P1413R3 Deprecate aligned_storage And aligned_union
// P2614R2 Deprecating float_denorm_style, numeric_limits::has_denorm,
// numeric_limits::has_denorm_loss Other C++23 deprecation warnings
...
// calls, but not all.
//
// std::execution::unseq has no direct analogue for any optimizer we target as
// of 2020-07-29, though we will map it to #pragma loop(ivdep) for the for_each
// algorithms only as these are the only algorithms where the library does not
// need to introduce inter-loop-body dependencies to accomplish the algorithm's
// goals.
//
...
#define _EMIT_STL_MESSAGE(MESSAGE)                                             \
  _STL_PRAGMA_MESSAGE(__FILE__ "(" _STL_STRINGIZE(__LINE__) "): " MESSAGE)
...
#else // ^^^ __has_cpp_attribute(nodiscard) >= 201907L /
      // __has_cpp_attribute(nodiscard) < 201907L vvv
#define _NODISCARD_CTOR
#define _NODISCARD_CTOR_MSG(_Msg)
...
#define _NODISCARD_REMOVE_ALG                                                  \
  _NODISCARD_MSG("The 'remove' and 'remove_if' algorithms return the "         \
                 "iterator past the last element "                             \
                 "that should be kept. You need to call "                      \
                 "container.erase(result, container.end()) afterwards. "       \
                 "In C++20, 'std::erase' and 'std::erase_if' are simpler "     \
                 "replacements for these two steps.")
...
#define _NODISCARD_EMPTY_MEMBER                                                \
  _NODISCARD_MSG(                                                              \
      "This member function returns a bool indicating whether the collection " \
      "is empty and has no other effects. "                                    \
      "It is not useful to call this member function and discard the return "  \
      "value. "                                                                \
      "Use the 'clear()' member function if you want to erase all elements.")

#define _NODISCARD_EMPTY_ARRAY_MEMBER                                          \
  _NODISCARD_MSG("This member function returns a bool indicating whether the " \
                 "array is empty and has no other effects. "                   \
                 "It is not useful to call this member function and discard "  \
                 "the return value. "                                          \
...
#define _NODISCARD_EMPTY_NON_MEMBER                                            \
  _NODISCARD_MSG("This function returns a bool indicating whether the "        \
                 "collection is empty and "                                    \
                 "has no other effects. It is not useful to call this "        \
                 "function and discard the return value.")

#define _NODISCARD_BARRIER_TOKEN                                               \
  _NODISCARD_MSG("The token from 'arrive()' should not be discarded; it "      \
                 "should be passed to 'wait()'.")

#define _NODISCARD_TRY_WAIT                                                    \
  _NODISCARD_MSG("This member function returns the state of the "              \
                 "synchronization object and does not do anything else; "      \
                 "it is not useful to call this member function and discard "  \
                 "the return value.")

#define _NODISCARD_TRY_CHANGE_STATE                                            \
  _NODISCARD_MSG("This function returns whether the operation succeeded in "   \
                 "modifying object state. "                                    \
                 "It is dangerous to ignore the return value.")

#define _NODISCARD_SMART_PTR_ALLOC                                             \
  _NODISCARD_MSG(                                                              \
      "This function constructs an object wrapped by a smart pointer and has " \
      "no other effects; "                                                     \
...
#define _NODISCARD_ASSUME_ALIGNED                                              \
  _NODISCARD_MSG("'std::assume_aligned' has a potential effect on the return " \
                 "value (not on the passed argument). "                        \
                 "It is not useful to call 'std::assume_aligned' and discard " \
                 "the return value.")

#define _NODISCARD_LAUNDER                                                     \
  _NODISCARD_MSG(                                                              \
      "'std::launder' has a potential effect on the return value (not on the " \
      "passed argument). "                                                     \
...
#else // ^^^ defined(_SILENCE_NODISCARD_LOCK_WARNINGS) /
      // !defined(_SILENCE_NODISCARD_LOCK_WARNINGS) vvv

#define _NODISCARD_LOCK                                                        \
  _NODISCARD_MSG("A lock should be stored in a variable to protect the "       \
                 "scope. If you're intentionally constructing "                \
                 "a temporary to protect the rest of the current expression "  \
                 "using the comma operator, you can cast "                     \
                 "the temporary to void or define "                            \
                 "_SILENCE_NODISCARD_LOCK_WARNINGS to suppress this warning.")

#define _NODISCARD_CTOR_LOCK                                                   \
  _NODISCARD_CTOR_MSG(                                                         \
      "A lock should be stored in a variable to protect the scope. If you're " \
      "intentionally constructing "                                            \
      "a temporary to protect the rest of the current expression using the "   \
      "comma operator, you can cast "                                          \
      "the temporary to void or define _SILENCE_NODISCARD_LOCK_WARNINGS to "   \
...
#define _NODISCARD_CTOR_THREAD                                                 \
  _NODISCARD_CTOR_MSG(                                                         \
      "This temporary 'std::thread' is not joined or detached, "               \
      "so 'std::terminate' will be called at the end of the statement.")

#define _NODISCARD_CTOR_JTHREAD                                                \
  _NODISCARD_CTOR_MSG(                                                         \
      "This temporary 'std::jthread' is implicitly joined at the end of the "  \
      "statement. "                                                            \
      "If this is intentional, you can add '.join()' to suppress this "        \
      "warning. "                                                              \
      "Otherwise, this 'std::jthread' should be stored in a variable.")

#define _NODISCARD_ASYNC                                                       \
  _NODISCARD_MSG(                                                              \
      "The result of 'std::async' should be stored in a variable. If the "     \
      "return value is discarded, "                                            \
      "the temporary 'std::future' is destroyed, waiting for an async result " \
      "or evaluating "                                                         \
      "a deferred result, thus defeating the purpose of 'std::async'.")

#define _NODISCARD_GET_FUTURE                                                  \
  _NODISCARD_MSG("Since 'get_future' may be called only once, discarding the " \
                 "result is likely a mistake.")
...
// Should we enable [[msvc::lifetimebound]] or [[clang::lifetimebound]]
// warnings?
...
// warning C5053: support for 'explicit(<expr>)' in C++17 and earlier is a
// vendor extension
#if !_HAS_CXX20
...
// warning C4412: function signature contains type 'meow'; C++ objects are
// unsafe to pass between pure code
//                and mixed or native. (/Wall)
// warning C4455: literal suffix identifiers that do not start with an
// underscore are reserved warning C4494: Ignoring __declspec(allocator) because
// the function return type is not a pointer or reference warning C4514:
// unreferenced inline function has been removed (/Wall) warning C4574: 'MACRO'
// is defined to be '0': did you mean to use '#if MACRO'? (/Wall) warning C4582:
// 'union': constructor is not implicitly called (/Wall) warning C4583: 'union':
// destructor is not implicitly called (/Wall) warning C4587: behavior change:
// constructor is no longer implicitly called (/Wall) warning C4588: behavior
// change: destructor is no longer implicitly called (/Wall) warning C4619:
// #pragma warning: there is no warning number 'number' (/Wall) warning C4623:
// default constructor was implicitly defined as deleted (/Wall) warning C4625:
// copy constructor was implicitly defined as deleted (/Wall) warning C4626:
// assignment operator was implicitly defined as deleted (/Wall) warning C4643:
// Forward declaring 'meow' in namespace std is not permitted by the C++
// Standard. (/Wall) warning C4648: standard attribute 'meow' is ignored warning
// C4702: unreachable code warning C4793: function compiled as native warning
// C4820: 'N' bytes padding added after data member 'meow' (/Wall) warning
...
// class/function scope (/Wall /d1WarnOnGlobals) warning C5026: move constructor
// was implicitly defined as deleted (/Wall) warning C5027: move assignment
// operator was implicitly defined as deleted (/Wall) warning C5045: Compiler
// will insert Spectre mitigation for memory load if /Qspectre switch specified
// (/Wall) warning C5220: a non-static data member with a volatile qualified
// type no longer implies that compiler generated
//                copy/move constructors and copy/move assignment operators are
//                not trivial (/Wall)
// warning C5246: 'member': the initialization of a subobject should be wrapped
// in braces (/Wall) warning C5278: adding a specialization for 'type trait' has
// undefined behavior warning C5280: a static operator '()' requires at least
// '/std:c++23preview' warning C5281: a static lambda requires at least
// '/std:c++23preview' warning C6294: Ill-defined for-loop: initial condition
...
// warning: declaring overloaded 'operator()' as 'static' is a C++23 extension
// [-Wc++23-extensions] warning: static lambdas are a C++23 extension
// [-Wc++23-extensions] warning: ignoring __declspec(allocator) because the
// function return type '%s' is not a pointer or reference type
//     [-Wignored-attributes]
// warning: '#pragma float_control' is not supported on this target - ignored
// [-Wignored-pragmas] warning: user-defined literal suffixes not starting with
// '_' are reserved [-Wuser-defined-literals] warning: unknown pragma ignored
// [-Wunknown-pragmas]
...
_EMIT_STL_ERROR(STL1002,
                "Unexpected compiler version, expected CUDA 12.4 or newer.");
#endif // ^^^ old CUDA ^^^
#elif defined(__EDG__)
// not attempting to detect __EDG_VERSION__ being less than expected
#elif defined(__clang__)
#if __clang_major__ < 19
_EMIT_STL_ERROR(STL1000,
...
_EMIT_STL_ERROR(STL1001,
                "Unexpected compiler version, expected MSVC 19.44 or newer.");
#endif              // ^^^ old MSVC ^^^
...
// constexpr or consteval specifier is implicitly an inline function or
// variable"
...
#else // ^^^ constexpr in C++20 and later / inline (not constexpr) in C++17 and
      // earlier vvv
...
#else // ^^^ constexpr in C++23 and later / inline (not constexpr) in C++20 and
      // earlier vvv
...
_EMIT_STL_ERROR(STL1004,
                "C++98 unexpected() is incompatible with C++23 unexpected<E>.");
...
#define _DEPRECATE_TR1_NAMESPACE                                               \
  [[deprecated("warning STL4002: "                                             \
               "The non-Standard std::tr1 namespace and TR1-only machinery "   \
               "are deprecated and will be REMOVED. You can "                  \
               "define _SILENCE_TR1_NAMESPACE_DEPRECATION_WARNING to "         \
               "suppress this warning.")]]
#endif // ^^^ warning enabled ^^^
#endif // _HAS_TR1_NAMESPACE

// STL4003 was "The non-Standard std::identity struct is deprecated and will be
// REMOVED."
...
#define _MISMATCHED_ALLOCATOR_MESSAGE(_CONTAINER, _VALUE_TYPE)                 \
  _CONTAINER " requires that Allocator's value_type match " _VALUE_TYPE        \
             " (See N4950 [container.alloc.reqmts]/5)"                         \
             " Either fix the allocator value_type or define "                 \
...
#define _FACET_SPECIALIZATION_MESSAGE                                          \
  ...
  "Either use a Standard specialization or define "                            \
  "_ENFORCE_FACET_SPECIALIZATIONS=0 "                                          \
  "to suppress this error."

// To improve compiler throughput, use 'hidden friend' operators in
// <system_error> instead of non-members that are depicted in the Standard.
...
// Controls whether the STL will force /fp:fast to enable vectorization of
// algorithms defined in the standard as special cases; such as reduce,
...
#else // ^^^ floating-point exceptions enabled / floating-point exceptions
      // disabled (default) vvv
...
#define _CXX17_DEPRECATE_C_HEADER                                              \
  [[deprecated("warning STL4004: "                                             \
               "<ccomplex>, <cstdalign>, <cstdbool>, and <ctgmath> are "       \
               "deprecated in C++17. "                                         \
               "You can define _SILENCE_CXX17_C_HEADER_DEPRECATION_WARNING "   \
               "or _SILENCE_ALL_CXX17_DEPRECATION_WARNINGS to suppress this "  \
               "warning.")]]
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_UNCAUGHT_EXCEPTION_DEPRECATION_WARNING) &&         \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_ADAPTOR_TYPEDEFS_DEPRECATION_WARNING) &&           \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
#define _CXX17_DEPRECATE_ADAPTOR_TYPEDEFS                                      \
  [[deprecated(                                                                \
      "warning STL4007: Many result_type typedefs "                            \
      "and all argument_type, first_argument_type, and second_argument_type "  \
...
#define _CXX17_DEPRECATE_NEGATORS                                              \
  [[deprecated("warning STL4008: "                                             \
               "std::not1(), std::not2(), std::unary_negate, and "             \
               "std::binary_negate are deprecated in C++17. "                  \
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_OLD_ALLOCATOR_MEMBERS_DEPRECATION_WARNING) &&      \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
#define _CXX17_DEPRECATE_OLD_ALLOCATOR_MEMBERS                                 \
  [[deprecated("warning STL4010: "                                             \
               ...
               "Use std::allocator_traits instead of accessing these members " \
               "directly. "                                                    \
               "You can define "                                               \
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_RAW_STORAGE_ITERATOR_DEPRECATION_WARNING) &&       \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
#define _CXX17_DEPRECATE_RAW_STORAGE_ITERATOR                                  \
  [[deprecated("warning STL4011: "                                             \
               "std::raw_storage_iterator is deprecated in C++17. "            \
               "Consider using the std::uninitialized_copy() family of "       \
               "algorithms instead. "                                          \
               "You can define "                                               \
               "_SILENCE_CXX17_RAW_STORAGE_ITERATOR_DEPRECATION_WARNING "      \
               "or _SILENCE_ALL_CXX17_DEPRECATION_WARNINGS to suppress this "  \
               "warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _CXX17_DEPRECATE_RAW_STORAGE_ITERATOR
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_TEMPORARY_BUFFER_DEPRECATION_WARNING) &&           \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
#define _CXX17_DEPRECATE_TEMPORARY_BUFFER                                      \
  [[deprecated(                                                                \
      "warning STL4012: "                                                      \
      "std::get_temporary_buffer() and std::return_temporary_buffer() are "    \
      "deprecated in C++17. "                                                  \
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_IS_LITERAL_TYPE_DEPRECATION_WARNING) &&            \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
#define _CXX17_DEPRECATE_IS_LITERAL_TYPE                                       \
  [[deprecated(                                                                \
      "warning STL4013: "                                                      \
      "std::is_literal_type and std::is_literal_type_v are deprecated in "     \
      "C++17. "                                                                \
...
#define _CXX17_DEPRECATE_ITERATOR_BASE_CLASS                                   \
  [[deprecated(                                                                \
      ...
      "(The <iterator> header is NOT deprecated.) The C++ Standard has never " \
      "required user-defined iterators to "                                    \
      "derive from std::iterator. To fix this warning, stop deriving from "    \
      "std::iterator and start providing "                                     \
      "publicly accessible typedefs named iterator_category, value_type, "     \
...
#if _HAS_CXX17 &&                                                              \
    !defined(_SILENCE_CXX17_SHARED_PTR_UNIQUE_DEPRECATION_WARNING) &&          \
    !defined(_SILENCE_ALL_CXX17_DEPRECATION_WARNINGS)
...
#define _CXX17_DEPRECATE_CODECVT_HEADER                                        \
  [[deprecated(                                                                \
      ...
      "(containing std::codecvt_mode, "                                        \
      "std::codecvt_utf8, std::codecvt_utf16, and std::codecvt_utf8_utf16) "   \
      "are deprecated in C++17. "                                              \
      "(The std::codecvt class template is NOT deprecated.) "                  \
      "The C++ Standard doesn't provide equivalent non-deprecated "            \
      "functionality; "                                                        \
...
// STL4018 was "The non-Standard std::tr2::sys namespace is deprecated and will
// be REMOVED."

// STL4019 was "The member std::fpos::seekpos() is non-Standard, and [...] will
// be removed"
...
#if _HAS_CXX20 &&                                                              \
    !defined(_SILENCE_CXX20_CODECVT_FACETS_DEPRECATION_WARNING) &&             \
...
#define _CXX20_DEPRECATE_CODECVT_FACETS                                        \
  [[deprecated("warning STL4020: "                                             \
               ...
               "are deprecated in C++20. You can define "                      \
               "_SILENCE_CXX20_CODECVT_FACETS_DEPRECATION_WARNING "            \
               "or _SILENCE_ALL_CXX20_DEPRECATION_WARNINGS to suppress this "  \
               "warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _CXX20_DEPRECATE_CODECVT_FACETS
#endif // ^^^ warning disabled ^^^
...
// N4868 D.24 [depr.fs.path.factory]
    ...
    !defined(_SILENCE_CXX20_U8PATH_DEPRECATION_WARNING) &&                     \
    !defined(_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS)
#define _CXX20_DEPRECATE_U8PATH                                                \
  [[deprecated(                                                                \
      "warning STL4021: "                                                      \
      "The std::filesystem::u8path() overloads are deprecated in C++20. "      \
      "The constructors of std::filesystem::path provide equivalent "          \
      "functionality via construction from "                                   \
...
// STL4022 warned about "The hash_meow and unordered_meow containers'
// non-Standard lower_bound() member" STL4023 warned about "The hash_meow and
// unordered_meow containers' non-Standard upper_bound() member"

// P0966R1 [depr.string.capacity]
#if _HAS_CXX20 &&                                                              \
    !defined(                                                                  \
        _SILENCE_CXX20_STRING_RESERVE_WITHOUT_ARGUMENT_DEPRECATION_WARNING) && \
    !defined(_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS)
#define _CXX20_DEPRECATE_STRING_RESERVE_WITHOUT_ARGUMENT                       \
  [[deprecated(                                                                \
      "warning STL4024: "                                                      \
      "std::string::reserve() without an argument is deprecated in C++20. "    \
      "To shrink the string's capacity, use std::string::shrink_to_fit() "     \
      "instead. Otherwise, provide an "                                        \
...
#define _CXX20_DEPRECATE_IS_POD                                                \
  [[deprecated("warning STL4025: "                                             \
               ...
               "The std::is_trivially_copyable and/or "                        \
               "std::is_standard_layout traits likely suit your use case. "    \
...
// by Microsoft and will be REMOVED."
...
#define _CXX20_DEPRECATE_REL_OPS                                               \
  [[deprecated(                                                                \
      ...
      "Their use is superseded by C++20's <=> operator and automatic "         \
      "rewrites of relational expressions. "                                   \
...
#define _CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT                         \
  [[deprecated(                                                                \
      ...
      "The shared_ptr specialization of std::atomic provides superior "        \
      "functionality. "                                                        \
      "You can define "                                                        \
      "_SILENCE_CXX20_OLD_SHARED_PTR_ATOMIC_SUPPORT_DEPRECATION_WARNING "      \
      "or _SILENCE_ALL_CXX20_DEPRECATION_WARNINGS to suppress this "           \
...
#define _CXX20_DEPRECATE_VOLATILE                                              \
  [[deprecated("warning STL4030: "                                             \
               "Some operations on volatile-qualified types in the STL are "   \
               "deprecated in C++20. "                                         \
...
#if _HAS_CXX20 &&                                                              \
    !defined(_SILENCE_CXX20_MOVE_ITERATOR_ARROW_DEPRECATION_WARNING) &&        \
    !defined(_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS)
...
// STL4032 was "std::pmr::polymorphic_allocator::destroy() is deprecated in
// C++17 by LWG-3036." (reverted by P2875R4)

#if _HAS_CXX20 &&                                                              \
    !defined(_SILENCE_CXX20_IS_ALWAYS_EQUAL_DEPRECATION_WARNING) &&            \
    !defined(_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS)
...
#if _HAS_CXX23 &&                                                              \
    !defined(_SILENCE_CXX23_ALIGNED_UNION_DEPRECATION_WARNING) &&              \
    !defined(_SILENCE_ALL_CXX23_DEPRECATION_WARNINGS)
...
#if _HAS_CXX20 && !defined(_SILENCE_CXX20_CISO646_REMOVED_WARNING) &&          \
    !defined(_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS)
...
// STL4038 is used to warn that "The contents of <meow> are available only with
// C++NN or later."

// STL4039 is used to warn that "The contents of <coroutine> are not available
// with /await."
...
#define _CXX23_DEPRECATE_UNIX_STREAMS                                          \
  [[deprecated(                                                                \
      "warning STL4041: "                                                      \
      "std::errc enumerators std::errc::no_message_available, "                \
      "std::errc::no_stream_resources, "                                       \
      "std::errc::not_a_stream, and std::errc::stream_timeout and their "      \
      "corresponding errno macros ENODATA, ENOSR, "                            \
      "ENOSTR, and ETIME are deprecated in C++23 by LWG-3869. These errno "    \
      "macros are deprecated in POSIX 2008 and "                               \
      "removed in POSIX 202x. You can define "                                 \
      "_SILENCE_CXX23_UNIX_STREAMS_DEPRECATION_WARNING or "                    \
...
#define _CXX23_DEPRECATE_DENORM                                                \
  [[deprecated(                                                                \
      ...
      "are deprecated in C++23. You can define "                               \
...
#if !defined(_SILENCE_STDEXT_ARR_ITERS_DEPRECATION_WARNING) &&                 \
    !defined(_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS)
#define _DEPRECATE_STDEXT_ARR_ITERS                                            \
  [[deprecated(                                                                \
      "warning STL4043: stdext::checked_array_iterator, "                      \
      "stdext::unchecked_array_iterator, and related factory "                 \
      "functions are non-Standard extensions and will be removed in the "      \
      "future. std::span (since C++20) and "                                   \
      "gsl::span can be used instead. You can define "                         \
      "_SILENCE_STDEXT_ARR_ITERS_DEPRECATION_WARNING or "                      \
      "_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS to suppress this warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _DEPRECATE_STDEXT_ARR_ITERS
#endif // ^^^ warning disabled ^^^

// STL4044 was "The contents of the stdext::cvt namespace are non-Standard
// extensions and will be removed"

#if !defined(_SILENCE_IO_PFX_SFX_DEPRECATION_WARNING) &&                       \
    !defined(_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS)
#define _DEPRECATE_IO_PFX_SFX                                                  \
  [[deprecated(                                                                \
      "warning STL4045: The ipfx(), isfx(), opfx(), and osfx() functions are " \
      "removed before C++98 (see WG21-N0794) "                                 \
      "but kept as non-Standard extensions. They will be removed in the "      \
      "future, and the member classes sentry "                                 \
      "should be used instead. You can define "                                \
      "_SILENCE_IO_PFX_SFX_DEPRECATION_WARNING or "                            \
      "_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS to suppress this warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _DEPRECATE_IO_PFX_SFX
#endif // ^^^ warning disabled ^^^

#if !defined(_SILENCE_TR1_NAMESPACE_DEPRECATION_WARNING) &&                    \
    !defined(_SILENCE_TR1_RANDOM_DEPRECATION_WARNING) &&                       \
    !defined(_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS)
#define _DEPRECATE_TR1_RANDOM                                                  \
  [[deprecated(                                                                \
      "warning STL4046: Non-Standard TR1 components in <random> are "          \
      "deprecated and will be REMOVED. You "                                   \
      "can define _SILENCE_TR1_NAMESPACE_DEPRECATION_WARNING, "                \
      "_SILENCE_TR1_RANDOM_DEPRECATION_WARNING, or "                           \
      "_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS to suppress this warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _DEPRECATE_TR1_RANDOM
#endif // ^^^ warning disabled ^^^
...
#if _HAS_CXX20 && defined(__cpp_char8_t) &&                                    \
    !defined(_SILENCE_CXX20_CODECVT_CHAR8_T_FACETS_DEPRECATION_WARNING) &&     \
    !defined(_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS)
#define _CXX20_DEPRECATE_CODECVT_CHAR8_T_FACETS                                \
  [[deprecated(                                                                \
      "warning STL4047: std::codecvt<char16_t, char8_t, mbstate_t>, "          \
      ...
      "are deprecated by LWG-3767. You can define "                            \
      "_SILENCE_CXX20_CODECVT_CHAR8_T_FACETS_DEPRECATION_WARNING or "          \
      "_SILENCE_ALL_CXX20_DEPRECATION_WARNINGS to suppress this warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _CXX20_DEPRECATE_CODECVT_CHAR8_T_FACETS
#endif // ^^^ warning disabled ^^^

#if !defined(_SILENCE_LOCALE_EMPTY_DEPRECATION_WARNING) &&                     \
    !defined(_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS)
#define _DEPRECATE_LOCALE_EMPTY                                                \
  [[deprecated(                                                                \
      "warning STL4048: locale::empty() is a non-Standard extension and will " \
      "be removed in the future. A "                                           \
      "default-constructed locale can be used instead. You can define "        \
      "_SILENCE_LOCALE_EMPTY_DEPRECATION_WARNING or "                          \
      "_SILENCE_ALL_MS_EXT_DEPRECATION_WARNINGS to suppress this warning.")]]
#else // ^^^ warning enabled / warning disabled vvv
#define _DEPRECATE_LOCALE_EMPTY
#endif // ^^^ warning disabled ^^^
...
#else // ^^^ _HAS_DEPRECATED_ADAPTOR_TYPEDEFS /
      // !_HAS_DEPRECATED_ADAPTOR_TYPEDEFS vvv
...
#define _HAS_STREAM_INSERTION_OPERATORS_DELETED_IN_CXX20                       \
...
#define _HAS_GARBAGE_COLLECTION_SUPPORT_DELETED_IN_CXX23                       \
  (_HAS_FEATURES_REMOVED_IN_CXX23)
...
#define __cpp_lib_math_constants 201907L
    ...
    !defined(__EDG__) // TRANSITION, Clang and EDG support for modules
...
#else
#define __cpp_lib_chrono                                                       \
  201510L // P0092R1 <chrono> floor(), ceil(), round(), abs()
#endif

#if _HAS_CXX23
#define __cpp_lib_concepts                                                     \
  202207L // P2404R3 Move-Only Types For Comparison Concepts
#elif _HAS_CXX20
#define __cpp_lib_concepts                                                     \
  202002L // P1964R2 Replacing boolean With boolean-testable
#endif
...
#define __cpp_lib_constexpr_memory                                             \
  201811L // P1006R1 constexpr For pointer_traits<T*>::pointer_to()
#endif
...
#elif _HAS_CXX17
#define __cpp_lib_optional                                                     \
  201606L // P0307R2 Making Optional Greater Equal Again
#endif
...
#define __cpp_lib_shared_ptr_arrays                                            \
  201611L // P0497R0 Fixing shared_ptr For Arrays
#endif

#if _HAS_CXX23
#define __cpp_lib_shift                                                        \
  202202L // P2440R1 ranges::shift_left, ranges::shift_right
#elif _HAS_CXX20
...
// extern "C++" attaches declarations to the global module, see N4964
// [module.unit]/7.2. It has no effect in C++14/17.

// In the STL's headers (which might be used to build the named module std), we
// unconditionally and directly mark declarations of our separately compiled
// machinery as extern "C++", allowing the named module to work with the
// separately compiled code (which is always built classically).

// TRANSITION: _USE_EXTERN_CXX_EVERYWHERE_FOR_STL controls whether we also wrap
// the STL's header-only code in this linkage-specification, as a temporary
// workaround to allow importing the named module in a translation unit with
// classic includes.
...
#else // ^^^ _USE_EXTERN_CXX_EVERYWHERE_FOR_STL /
      // !_USE_EXTERN_CXX_EVERYWHERE_FOR_STL vvv
...
// We use the stdext (standard extension) namespace to contain non-standard
// extensions
...
// Note that the STL DLL builds will set this to XP for ABI compatibility with
// VS2015 which supported XP.
...
#endif // ^^^ !defined(_M_ARM) && !defined(_M_ARM64) && !defined(_ONECORE) &&
       // !defined(_CRT_APP) ^^^
#endif // !defined(_STL_WIN32_WINNT)

#ifdef __cpp_noexcept_function_type
#define _NOEXCEPT_FNPTR noexcept
#else // ^^^ defined(__cpp_noexcept_function_type) /
      // !defined(__cpp_noexcept_function_type) vvv
...
#else // ^^^ defined(_ENABLE_STL_INTERNAL_CHECK) /
      // !defined(_ENABLE_STL_INTERNAL_CHECK) vvv
#define _STL_INTERNAL_STATIC_ASSERT(...)
#endif // ^^^ !defined(_ENABLE_STL_INTERNAL_CHECK) ^^^

#if defined(__CUDACC__) || (defined(__clang__) && __clang_major__ < 16)
// TRANSITION, CUDA 12.4 doesn't have downlevel support for static call
// operators. TRANSITION, VSO-2397560, temporary workaround for Real World Code
// relying on ancient Clang versions.
...
#ifdef __CUDACC__ // TRANSITION, CUDA 12.4 doesn't recognize MSVC __restrict;
                  // CUDA __restrict__ is not usable in C++
#define _RESTRICT
