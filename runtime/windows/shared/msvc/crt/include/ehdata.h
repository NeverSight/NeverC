...
#define CT_ISSIMPLETYPE(ct)                                                    \
  (CT_PROPERTIES(ct) & CT_IsSimpleType) // Is it a simple type?
#define CT_BYREFONLY(ct)                                                       \
  (CT_PROPERTIES(ct) & CT_ByReferenceOnly) // Must it be caught by reference?
#define CT_HASVB(ct)                                                           \
  (CT_PROPERTIES(ct) &                                                         \
   CT_HasVirtualBase) // Is this type a class with virtual bases?
#define CT_ISWINRTHANDLE(ct)                                                   \
  (CT_PROPERTIES(ct) & CT_IsWinRTHandle) // Is it a winrt handle?
#define CT_ISSTDBADALLOC(ct)                                                   \
  (CT_PROPERTIES(ct) & CT_IsStdBadAlloc) // Is it a std::bad_alloc?

#define THROW_ATTRS(t) ((t).attributes)
#if _EH_RELATIVE_TYPEINFO
#define THROW_UNWINDFUNC_IB(t, ib) ((void (*)(void *))((ib) + (t).pmfnUnwind))
#define THROW_FORWARDCOMPAT_IB(t, ib)                                          \
  ((int(__cdecl *)(...))((ib) + (t).pForwardCompat))
#define THROW_CTARRAY_IB(t, ib)                                                \
  ((CatchableTypeArray *)((ib) + (t).pCatchableTypeArray))
...
// -1 is the 'blank' state, i.e. there is nothing to unwind, no try blocks
// active.
//
...
typedef const struct _s_HandlerType {
  ...
  int dispFrame; // displacement of address of function frame wrt establisher
                 // frame
#endif
...
#define HT_ISCONST(ht)                                                         \
  (HT_ADJECTIVES(ht) & HT_IsConst) // Is the type referenced 'const' qualified
#define HT_ISVOLATILE(ht)                                                      \
  (HT_ADJECTIVES(ht) &                                                         \
   HT_IsVolatile) // Is the type referenced 'volatile' qualified
#define HT_ISUNALIGNED(ht)                                                     \
  (HT_ADJECTIVES(ht) &                                                         \
   HT_IsUnaligned) // Is the type referenced 'unaligned' qualified
#define HT_ISREFERENCE(ht)                                                     \
  (HT_ADJECTIVES(ht) & HT_IsReference) // Is the catch type by reference
#define HT_ISRESUMABLE(ht)                                                     \
  (HT_ADJECTIVES(ht) &                                                         \
   HT_IsResumable) // Might the catch choose to resume (Reserved)
...
#if _EH_RELATIVE_FUNCINFO
  int dispHandlerArray; // Image relative offset of list of handlers for this
                        // try
#else
...
#if _EH_RELATIVE_TYPEINFO
#define EST_ARRAY_IB(estl, ib, n)                                              \
  (&(((HandlerType *)((ib) + (estl)->dispTypeArray))[n]))
#else
...
#define FUNC_PHANDLERMAP(fi, ib)                                               \
  ((TryBlockMapEntry *)((ib) + (fi).dispTryBlockMap))
...
struct EHRegistrationNode {
  /* void *			stackPtr */ // Stack ptr at entry to try (below
                                            // address point)
...
} DispatcherContext; // changed the case of the name to conform to EH
                     // conventions

//
// On ARM we don't have a registration node, just a pointer to the stack frame
// base
//
...
//
// On P7 we don't have a registration node, just a pointer to the stack frame
// base
//
...
//
// On ARM64 we don't have a registration node, just a pointer to the stack frame
// base
//
typedef ULONG_PTR EHRegistrationNode;
...
typedef struct EHExceptionRecord {
  ...
  struct _EXCEPTION_RECORD
      *ExceptionRecord;           // An extra exception record (not used)
  void *ExceptionAddress;         // Address at which exception occurred
  unsigned long NumberParameters; // Number of extended parameters. (=
...
// PER_EXCEPTINFO is used to access the full ExceptionInformation field of
// EXCEPTION_RECORD which is only partially covered by
// EHExceptionRecord->params.
#define PER_EXCEPTINFO(per)                                                    \
  (*(ULONG_PTR(*)[EXCEPTION_MAXIMUM_PARAMETERS]) & (PER_MAGICNUM(per)))
#if _EH_RELATIVE_TYPEINFO
...
#define PER_IS_MSVC_EH(per)                                                    \
  ((PER_CODE(per) == EH_EXCEPTION_NUMBER) &&                                   \
   ...
   ((PER_MAGICNUM(per) == EH_PURE_MAGIC_NUMBER1) || PER_PTHROW(per) == NULL))
#else
#define PER_IS_MSVC_EH(per)                                                    \
  ((PER_CODE(per) == EH_EXCEPTION_NUMBER) &&                                   \
   (PER_NPARAMS(per) == EH_EXCEPTION_PARAMETERS) &&                            \
