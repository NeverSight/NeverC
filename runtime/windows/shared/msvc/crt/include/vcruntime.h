...
#define _CRT_BEGIN_C_HEADER                                                    \
  cpp_quote("__pragma(pack(push, _CRT_PACKING))") cpp_quote("extern \"C\" {")
...
#if defined(_M_IX86) && defined(_CRT_LEGACY_X86_FLT_EXCEPTIONS) &&             \
    !defined(_M_CEE_PURE)
...
#define _CRT_INSECURE_DEPRECATE(_Replacement)                                  \
  _CRT_DEPRECATE_TEXT(                                                         \
      "This function or variable may be unsafe. Consider using " #_Replacement \
...
__report_gsfailure(_In_ uintptr_t _StackCookie);
#else
