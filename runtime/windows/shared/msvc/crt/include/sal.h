...
/*==========================================================================
   ...
   for _In_ parameters the buffer must have the specified stringtype before the
call for _Out_ parameters the buffer must have the specified stringtype after
the call for _Inout_ parameters both conditions apply
...
#define _Inout_defensive_(annotes)                                             \
  _Pre_defensive_ _Post_defensive_ _Group_(annotes)
...
#define _Const_                                                                \
  _SAL2_Source_(_Const_, (), _Pre1_impl_(__readaccess_impl_notref))

// Input parameters --------------------------

//   _In_ - Annotations for parameters where data is passed into the function,
//   but not modified.
//          _In_ by itself can be used with non-pointer types (although it is
//          redundant).
...
#define _In_opt_                                                               \
  _SAL2_Source_(_In_opt_, (),                                                  \
                _Pre1_impl_(__maybenull_impl_notref)                           \
                    _Pre_valid_impl_ _Deref_pre_readonly_)

// nullterminated 'in' parameters.
// e.g. void CopyStr( _In_z_ const char* szFrom, _Out_z_cap_(cchTo) char* szTo,
// size_t cchTo );
#define _In_z_ _SAL2_Source_(_In_z_, (), _In_ _Pre1_impl_(__zterm_impl))
#define _In_opt_z_                                                             \
  _SAL2_Source_(_In_opt_z_, (), _In_opt_ _Pre1_impl_(__zterm_impl))

// 'input' buffers with given size

#define _In_reads_(size)                                                       \
  _SAL2_Source_(_In_reads_, (size), _Pre_count_(size) _Deref_pre_readonly_)
#define _In_reads_opt_(size)                                                   \
  _SAL2_Source_(_In_reads_opt_, (size),                                        \
                _Pre_opt_count_(size) _Deref_pre_readonly_)
#define _In_reads_bytes_(size)                                                 \
  _SAL2_Source_(_In_reads_bytes_, (size),                                      \
                _Pre_bytecount_(size) _Deref_pre_readonly_)
#define _In_reads_bytes_opt_(size)                                             \
  _SAL2_Source_(_In_reads_bytes_opt_, (size),                                  \
                _Pre_opt_bytecount_(size) _Deref_pre_readonly_)
#define _In_reads_z_(size)                                                     \
  _SAL2_Source_(_In_reads_z_, (size), _In_reads_(size) _Pre_z_)
#define _In_reads_opt_z_(size)                                                 \
  _SAL2_Source_(_In_reads_opt_z_, (size),                                      \
                _Pre_opt_count_(size) _Deref_pre_readonly_ _Pre_opt_z_)
#define _In_reads_or_z_(size)                                                  \
  _SAL2_Source_(_In_reads_or_z_, (size),                                       \
                _In_ _When_(_String_length_(_Curr_) < (size), _Pre_z_)         \
                    _When_(_String_length_(_Curr_) >= (size),                  \
                           _Pre1_impl_(__count_impl(size))))
...
#define _In_reads_to_ptr_(ptr)                                                 \
  _SAL2_Source_(_In_reads_to_ptr_, (ptr),                                      \
                _Pre_ptrdiff_count_(ptr) _Deref_pre_readonly_)
#define _In_reads_to_ptr_opt_(ptr)                                             \
  _SAL2_Source_(_In_reads_to_ptr_opt_, (ptr),                                  \
                _Pre_opt_ptrdiff_count_(ptr) _Deref_pre_readonly_)
...
//   _Out_ - Annotations for pointer or reference parameters where data passed
//   back to the caller.
//           These are mostly used where the pointer/reference is to a
//           non-pointer type. _Outptr_/_Outref) (see below) are typically used
//           to return pointers via parameters.
...
#define _Out_writes_(size)                                                     \
  _SAL2_Source_(_Out_writes_, (size), _Pre_cap_(size) _Post_valid_impl_)
#define _Out_writes_opt_(size)                                                 \
  _SAL2_Source_(_Out_writes_opt_, (size), _Pre_opt_cap_(size) _Post_valid_impl_)
#define _Out_writes_bytes_(size)                                               \
  _SAL2_Source_(_Out_writes_bytes_, (size),                                    \
                _Pre_bytecap_(size) _Post_valid_impl_)
#define _Out_writes_bytes_opt_(size)                                           \
  _SAL2_Source_(_Out_writes_bytes_opt_, (size),                                \
                _Pre_opt_bytecap_(size) _Post_valid_impl_)
#define _Out_writes_z_(size)                                                   \
  _SAL2_Source_(_Out_writes_z_, (size),                                        \
                _Pre_cap_(size) _Post_valid_impl_ _Post_z_)
#define _Out_writes_opt_z_(size)                                               \
  _SAL2_Source_(_Out_writes_opt_z_, (size),                                    \
                _Pre_opt_cap_(size) _Post_valid_impl_ _Post_z_)
...
#define _Out_writes_all_(size)                                                 \
  _SAL2_Source_(_Out_writes_all_, (size),                                      \
                _Out_writes_to_(_Old_(size), _Old_(size)))
#define _Out_writes_all_opt_(size)                                             \
  _SAL2_Source_(_Out_writes_all_opt_, (size),                                  \
                _Out_writes_to_opt_(_Old_(size), _Old_(size)))
...
#define _Out_writes_bytes_to_opt_(size, count)                                 \
  _SAL2_Source_(_Out_writes_bytes_to_opt_, (size, count),                      \
                _Pre_opt_bytecap_(size)                                        \
                    _Post_valid_impl_ _Post_bytecount_(count))
#define _Out_writes_bytes_all_(size)                                           \
  _SAL2_Source_(_Out_writes_bytes_all_, (size),                                \
                _Out_writes_bytes_to_(_Old_(size), _Old_(size)))
...
#define _Out_writes_to_ptr_(ptr)                                               \
  _SAL2_Source_(_Out_writes_to_ptr_, (ptr),                                    \
                _Pre_ptrdiff_cap_(ptr) _Post_valid_impl_)
#define _Out_writes_to_ptr_opt_(ptr)                                           \
  _SAL2_Source_(_Out_writes_to_ptr_opt_, (ptr),                                \
                _Pre_opt_ptrdiff_cap_(ptr) _Post_valid_impl_)
#define _Out_writes_to_ptr_z_(ptr)                                             \
  _SAL2_Source_(_Out_writes_to_ptr_z_, (ptr),                                  \
                _Pre_ptrdiff_cap_(ptr) _Post_valid_impl_ Post_z_)
...
//   _Inout_ - Annotations for pointer or reference parameters where data is
//   passed in and
...
#define _Inout_updates_(size)                                                  \
  _SAL2_Source_(_Inout_updates_, (size),                                       \
                _Pre_cap_(size) _Pre_valid_impl_ _Post_valid_impl_)
...
#define _Inout_updates_z_(size)                                                \
  _SAL2_Source_(_Inout_updates_z_, (size),                                     \
                _Pre_cap_(size)                                                \
                    _Pre_valid_impl_ _Post_valid_impl_ _Pre1_impl_(            \
                        __zterm_impl) _Post1_impl_(__zterm_impl))
...
#define _Inout_updates_all_(size)                                              \
  _SAL2_Source_(_Inout_updates_all_, (size),                                   \
                _Inout_updates_to_(_Old_(size), _Old_(size)))
#define _Inout_updates_all_opt_(size)                                          \
  _SAL2_Source_(_Inout_updates_all_opt_, (size),                               \
...
#define _Inout_updates_bytes_to_(size, count)                                  \
  _SAL2_Source_(_Inout_updates_bytes_to_, (size, count),                       \
                _Out_writes_bytes_to_(size, count)                             \
                    _Pre_valid_impl_ _Pre1_impl_(__bytecount_impl(size)))
...
//      The caller passes the address of an LPWSTR variable as ppwsz, and
//      SHStrDupW allocates and initializes memory and returns the pointer to
//      the new LPWSTR in *ppwsz.
//
//    _Outptr_opt_ - describes parameters that are allowed to be NULL.
//    _Outptr_*_result_maybenull_ - describes parameters where the called
//    function might return NULL to the caller.
//
//    Example:
//       void MyFunc(_Outptr_opt_ int **ppData1, _Outptr_result_maybenull_ int
//       **ppData2);
//    Callers:
//       MyFunc(NULL, NULL);           // error: parameter 2, ppData2, should
//       not be NULL MyFunc(&pData1, &pData2);     // ok: both non-NULL if
//       (*pData1 == *pData2) ...   // error: pData2 might be NULL after call
...
#define _Outptr_opt_                                                           \
  _SAL2_Source_(_Outptr_opt_, (),                                              \
                _Out_opt_impl_ _Deref_post2_impl_(__notnull_impl_notref,       \
                                                  __count_impl(1)))
#define _Outptr_opt_result_maybenull_                                          \
  _SAL2_Source_(_Outptr_opt_result_maybenull_, (),                             \
                _Out_opt_impl_ _Deref_post2_impl_(__maybenull_impl_notref,     \
                                                  __count_impl(1)))

// Annotations for _Outptr_ parameters returning pointers to null terminated
// strings.

#define _Outptr_result_z_                                                      \
  _SAL2_Source_(_Outptr_result_z_, (), _Out_impl_ _Deref_post_z_)
...
#define _Outptr_opt_result_maybenull_z_                                        \
  _SAL2_Source_(_Outptr_opt_result_maybenull_z_, (),                           \
...
#define _Outptr_result_nullonfailure_                                          \
  _SAL2_Source_(_Outptr_result_nullonfailure_, (),                             \
...
// Annotations for _Outptr_ parameters which return a pointer to a ref-counted
// COM object, following the COM convention of setting the output to NULL on
// failure. The current implementation is identical to
// _Outptr_result_nullonfailure_. For pointers to types that are not COM
// objects, _Outptr_result_nullonfailure_ is preferred.

#define _COM_Outptr_                                                           \
  _SAL2_Source_(_COM_Outptr_, (), _Outptr_ _On_failure_(_Deref_post_null_))
#define _COM_Outptr_result_maybenull_                                          \
  _SAL2_Source_(_COM_Outptr_result_maybenull_, (),                             \
                _Outptr_result_maybenull_ _On_failure_(_Deref_post_null_))
#define _COM_Outptr_opt_                                                       \
  _SAL2_Source_(_COM_Outptr_opt_, (),                                          \
                _Outptr_opt_ _On_failure_(_Deref_post_null_))
#define _COM_Outptr_opt_result_maybenull_                                      \
  _SAL2_Source_(_COM_Outptr_opt_result_maybenull_, (),                         \
                _Outptr_opt_result_maybenull_ _On_failure_(_Deref_post_null_))

// Annotations for _Outptr_ parameters returning a pointer to buffer with a
// specified number of elements/bytes
...
#define _Outptr_opt_result_buffer_to_(size, count)                             \
  _SAL2_Source_(_Outptr_opt_result_buffer_to_, (size, count),                  \
                _Out_opt_impl_ _Deref_post3_impl_(__notnull_impl_notref,       \
                                                  __cap_impl(size),            \
                                                  __count_impl(count)))

#define _Outptr_result_buffer_all_(size)                                       \
  _SAL2_Source_(_Outptr_result_buffer_all_, (size),                            \
                _Out_impl_ _Deref_post2_impl_(__notnull_impl_notref,           \
                                              __count_impl(size)))
...
#define _Outptr_result_buffer_maybenull_(size)                                 \
  _SAL2_Source_(_Outptr_result_buffer_maybenull_, (size),                      \
                _Out_impl_ _Deref_post2_impl_(__maybenull_impl_notref,         \
                                              __cap_impl(size)))
...
#define _Outptr_result_buffer_to_maybenull_(size, count)                       \
  _SAL2_Source_(_Outptr_result_buffer_to_maybenull_, (size, count),            \
                _Out_impl_ _Deref_post3_impl_(__maybenull_impl_notref,         \
                                              __cap_impl(size),                \
                                              __count_impl(count)))
#define _Outptr_opt_result_buffer_to_maybenull_(size, count)                   \
  _SAL2_Source_(_Outptr_opt_result_buffer_to_maybenull_, (size, count),        \
                _Out_opt_impl_ _Deref_post3_impl_(__maybenull_impl_notref,     \
                                                  __cap_impl(size),            \
                                                  __count_impl(count)))
...
#define _Outref_result_buffer_(size)                                           \
  _SAL2_Source_(_Outref_result_buffer_, (size),                                \
                _Outref_ _Post1_impl_(__cap_impl(size)))
#define _Outref_result_bytebuffer_(size)                                       \
  _SAL2_Source_(_Outref_result_bytebuffer_, (size),                            \
                _Outref_ _Post1_impl_(__bytecap_impl(size)))
#define _Outref_result_buffer_to_(size, count)                                 \
  _SAL2_Source_(_Outref_result_buffer_to_, (size, count),                      \
                _Outref_result_buffer_(size)                                   \
                    _Post1_impl_(__count_impl(count)))
...
#define _Outref_result_buffer_all_(size)                                       \
  _SAL2_Source_(_Outref_result_buffer_all_, (size),                            \
                _Outref_result_buffer_to_(size, _Old_(size)))
...
#define _Outref_result_nullonfailure_                                          \
  _SAL2_Source_(_Outref_result_nullonfailure_, (),                             \
                _Outref_ _On_failure_(_Post_null_))

// Generic annotations to set output value of a by-pointer or by-reference
// parameter to null/zero on failure.
#define _Result_nullonfailure_                                                 \
  _SAL2_Source_(_Result_nullonfailure_, (),                                    \
                _On_failure_(_Notref_impl_ _Deref_impl_ _Post_null_))
...
#define _Ret_z_                                                                \
  _SAL2_Source_(_Ret_z_, (),                                                   \
                _Ret2_impl_(__notnull_impl, __zterm_impl) _Ret_valid_impl_)
...
#define _Ret_notnull_                                                          \
  _SAL2_Source_(_Ret_notnull_, (), _Ret1_impl_(__notnull_impl))
#define _Ret_maybenull_                                                        \
  _SAL2_Source_(_Ret_maybenull_, (), _Ret1_impl_(__maybenull_impl))
...
#define _Must_inspect_result_                                                  \
  _SAL2_Source_(_Must_inspect_result_, (),                                     \
...
#define _Scanf_format_string_                                                  \
  _SAL2_Source_(_Scanf_format_string_, (), _Scanf_format_string_impl_)
...
#define _Printf_format_string_params_(x)                                       \
  _SAL2_Source_(_Printf_format_string_params_, (x),                            \
                _Format_string_impl_("printf", x))
#define _Scanf_format_string_params_(x)                                        \
  _SAL2_Source_(_Scanf_format_string_params_, (x),                             \
                _Format_string_impl_("scanf", x))
#define _Scanf_s_format_string_params_(x)                                      \
  _SAL2_Source_(_Scanf_s_format_string_params_, (x),                           \
                _Format_string_impl_("scanf_s", x))
...
#define _Pre_equal_to_(expr)                                                   \
  _SAL2_Source_(_Pre_equal_to_, (expr), _In_range_(==, expr))
#define _Post_equal_to_(expr)                                                  \
  _SAL2_Source_(_Post_equal_to_, (expr), _Out_range_(==, expr))
...
#define _Unchanged_(e)                                                         \
  _SAL2_Source_(_Unchanged_, (e), _At_(e, _Post_equal_to_(_Old_(e)) _Const_))

// Annotations to allow expressing generalized pre and post conditions.
// 'cond' may be any valid SAL expression that is considered to be true as a
// precondition or postcondition (respectively).
#define _Pre_satisfies_(cond)                                                  \
  _SAL2_Source_(_Pre_satisfies_, (cond), _Pre_satisfies_impl_(cond))
#define _Post_satisfies_(cond)                                                 \
  _SAL2_Source_(_Post_satisfies_, (cond), _Post_satisfies_impl_(cond))

// Annotations to express struct, class and field invariants
#define _Struct_size_bytes_(size)                                              \
  _SAL2_Source_(_Struct_size_bytes_, (size), _Writable_bytes_(size))

#define _Field_size_(size)                                                     \
  _SAL2_Source_(_Field_size_, (size), _Notnull_ _Writable_elements_(size))
...
#define _Field_size_bytes_full_(size)                                          \
  _SAL2_Source_(_Field_size_bytes_full_, (size),                               \
                _Field_size_bytes_part_(size, size))
#define _Field_size_bytes_full_opt_(size)                                      \
  _SAL2_Source_(_Field_size_bytes_full_opt_, (size),                           \
                _Field_size_bytes_part_opt_(size, size))
...
#define _Readable_bytes_(size)                                                 \
  _SAL2_Source_(_Readable_bytes_, (size), _Readable_bytes_impl_(size))
...
#define _Writable_bytes_(size)                                                 \
  _SAL2_Source_(_Writable_bytes_, (size), _Writable_bytes_impl_(size))
#define _Writable_elements_(size)                                              \
  _SAL2_Source_(_Writable_elements_, (size), _Writable_elements_impl_(size))

#define _Null_terminated_                                                      \
  _SAL2_Source_(_Null_terminated_, (), _Null_terminated_impl_)
#define _NullNull_terminated_                                                  \
  _SAL2_Source_(_NullNull_terminated_, (), _NullNull_terminated_impl_)

// Expressing buffer size as pre or post condition
#define _Pre_readable_size_(size)                                              \
  _SAL2_Source_(_Pre_readable_size_, (size),                                   \
                _Pre1_impl_(__count_impl(size)) _Pre_valid_impl_)
...
#define _Pre_writable_byte_size_(size)                                         \
  _SAL2_Source_(_Pre_writable_byte_size_, (size),                              \
                _Pre1_impl_(__bytecap_impl(size)))

#define _Post_readable_size_(size)                                             \
  _SAL2_Source_(_Post_readable_size_, (size),                                  \
                _Post1_impl_(__count_impl(size)) _Post_valid_impl_)
...
#define _Post_writable_byte_size_(size)                                        \
  _SAL2_Source_(_Post_writable_byte_size_, (size),                             \
                _Post1_impl_(__bytecap_impl(size)))
...
#define _Pre_z_                                                                \
  _SAL2_Source_(_Pre_z_, (), _Pre1_impl_(__zterm_impl) _Pre_valid_impl_)

// valid size unknown or indicated by type (e.g.:LPSTR)
#define _Pre_valid_                                                            \
  _SAL2_Source_(_Pre_valid_, (),                                               \
                _Pre1_impl_(__notnull_impl_notref) _Pre_valid_impl_)
#define _Pre_opt_valid_                                                        \
  _SAL2_Source_(_Pre_opt_valid_, (),                                           \
                _Pre1_impl_(__maybenull_impl_notref) _Pre_valid_impl_)

#define _Pre_invalid_                                                          \
  _SAL2_Source_(_Pre_invalid_, (), _Deref_pre1_impl_(__notvalid_impl))

// Overrides recursive valid when some field is not yet initialized when using
// _Inout_
#define _Pre_unknown_                                                          \
  _SAL2_Source_(_Pre_unknown_, (), _Pre1_impl_(__maybevalid_impl))

// used with allocated but not yet initialized objects
#define _Pre_notnull_                                                          \
  _SAL2_Source_(_Pre_notnull_, (), _Pre1_impl_(__notnull_impl_notref))
#define _Pre_maybenull_                                                        \
  _SAL2_Source_(_Pre_maybenull_, (), _Pre1_impl_(__maybenull_impl_notref))
#define _Pre_null_                                                             \
  _SAL2_Source_(_Pre_null_, (), _Pre1_impl_(__null_impl_notref))
...
// void CopyStr( _In_z_ const char* szFrom, _Pre_cap_(cch) _Post_z_ char*
// szFrom, size_t cchFrom ); buffer will be a zero-terminated string after the
// call
#define _Post_z_                                                               \
  _SAL2_Source_(_Post_z_, (), _Post1_impl_(__zterm_impl) _Post_valid_impl_)

// e.g. HRESULT InitStruct( _Post_valid_ Struct* pobj );
#define _Post_valid_ _SAL2_Source_(_Post_valid_, (), _Post_valid_impl_)
#define _Post_invalid_                                                         \
  _SAL2_Source_(_Post_invalid_, (), _Deref_post1_impl_(__notvalid_impl))

// e.g. void free( _Post_ptr_invalid_ void* pv );
#define _Post_ptr_invalid_                                                     \
  _SAL2_Source_(_Post_ptr_invalid_, (), _Post1_impl_(__notvalid_impl))

// e.g. void ThrowExceptionIfNull( _Post_notnull_ const void* pv );
#define _Post_notnull_                                                         \
  _SAL2_Source_(_Post_notnull_, (), _Post1_impl_(__notnull_impl))
...
#define _Post_maybenull_                                                       \
  _SAL2_Source_(_Post_maybenull_, (), _Post1_impl_(__maybenull_impl))
...
the call for _Inout_ parameters both conditions apply
   ...
   'cap' (capacity) describes the writable size of the buffer and is typically
used with _Out_. The default unit is elements. Use 'bytecap' if the size is
given in bytes 'count' describes the readable size of the buffer and is
typically used with _In_. The default unit is elements. Use 'bytecount' if the
size is given in bytes.
   ...
   If the buffer size is neither a parameter nor a constant expression use the
x_ postfix. e.g. bytecount_x_(num*size) x_ annotations accept any arbitrary
string. No analysis can be done for x_ annotations but they at least tell the
tool that the buffer has some sort of extent description. x_ annotations might
be supported by future compiler versions.
...
#define _In_count_(size)                                                       \
  _SAL1_1_Source_(_In_count_, (size), _Pre_count_(size) _Deref_pre_readonly_)
#define _In_opt_count_(size)                                                   \
  _SAL1_1_Source_(_In_opt_count_, (size),                                      \
                  _Pre_opt_count_(size) _Deref_pre_readonly_)
#define _In_bytecount_(size)                                                   \
  _SAL1_1_Source_(_In_bytecount_, (size),                                      \
                  _Pre_bytecount_(size) _Deref_pre_readonly_)
#define _In_opt_bytecount_(size)                                               \
  _SAL1_1_Source_(_In_opt_bytecount_, (size),                                  \
                  _Pre_opt_bytecount_(size) _Deref_pre_readonly_)

// valid buffer extent described by a constant extression
#define _In_count_c_(size)                                                     \
  _SAL1_1_Source_(_In_count_c_, (size),                                        \
                  _Pre_count_c_(size) _Deref_pre_readonly_)
#define _In_opt_count_c_(size)                                                 \
  _SAL1_1_Source_(_In_opt_count_c_, (size),                                    \
                  _Pre_opt_count_c_(size) _Deref_pre_readonly_)
#define _In_bytecount_c_(size)                                                 \
  _SAL1_1_Source_(_In_bytecount_c_, (size),                                    \
                  _Pre_bytecount_c_(size) _Deref_pre_readonly_)
#define _In_opt_bytecount_c_(size)                                             \
  _SAL1_1_Source_(_In_opt_bytecount_c_, (size),                                \
                  _Pre_opt_bytecount_c_(size) _Deref_pre_readonly_)
...
#define _In_z_count_(size)                                                     \
  _SAL1_1_Source_(_In_z_count_, (size),                                        \
                  _Pre_z_ _Pre_count_(size) _Deref_pre_readonly_)
...
#define _In_z_bytecount_(size)                                                 \
  _SAL1_1_Source_(_In_z_bytecount_, (size),                                    \
                  _Pre_z_ _Pre_bytecount_(size) _Deref_pre_readonly_)
...
#define _In_z_count_c_(size)                                                   \
  _SAL1_1_Source_(_In_z_count_c_, (size),                                      \
                  _Pre_z_ _Pre_count_c_(size) _Deref_pre_readonly_)
...
#define _In_opt_z_bytecount_c_(size)                                           \
  _SAL1_1_Source_(_In_opt_z_bytecount_c_, (size),                              \
                  _Pre_opt_z_ _Pre_opt_bytecount_c_(size)                      \
                      _Deref_pre_readonly_)

// buffer capacity is described by another pointer
// e.g. void Foo( _In_ptrdiff_count_(pchMax) const char* pch, const char* pchMax
// ) { while pch < pchMax ) pch++; }
#define _In_ptrdiff_count_(size)                                               \
  _SAL1_1_Source_(_In_ptrdiff_count_, (size),                                  \
                  _Pre_ptrdiff_count_(size) _Deref_pre_readonly_)
...
// 'x' version for complex expressions that are not supported by the current
// compiler version e.g. void Set3ColMatrix( _In_count_x_(3*cRows) const Elem*
// matrix, int cRows );
#define _In_count_x_(size)                                                     \
  _SAL1_1_Source_(_In_count_x_, (size),                                        \
                  _Pre_count_x_(size) _Deref_pre_readonly_)
#define _In_opt_count_x_(size)                                                 \
  _SAL1_1_Source_(_In_opt_count_x_, (size),                                    \
                  _Pre_opt_count_x_(size) _Deref_pre_readonly_)
#define _In_bytecount_x_(size)                                                 \
  _SAL1_1_Source_(_In_bytecount_x_, (size),                                    \
                  _Pre_bytecount_x_(size) _Deref_pre_readonly_)
#define _In_opt_bytecount_x_(size)                                             \
  _SAL1_1_Source_(_In_opt_bytecount_x_, (size),                                \
                  _Pre_opt_bytecount_x_(size) _Deref_pre_readonly_)

// 'out' with buffer size
// e.g. void GetIndices( _Out_cap_(cIndices) int* rgIndices, size_t cIndices );
// buffer capacity is described by another parameter
#define _Out_cap_(size)                                                        \
  _SAL1_1_Source_(_Out_cap_, (size), _Pre_cap_(size) _Post_valid_impl_)
...
#define _Out_opt_bytecap_(size)                                                \
  _SAL1_1_Source_(_Out_opt_bytecap_, (size),                                   \
                  _Pre_opt_bytecap_(size) _Post_valid_impl_)

// buffer capacity is described by a constant expression
#define _Out_cap_c_(size)                                                      \
  _SAL1_1_Source_(_Out_cap_c_, (size), _Pre_cap_c_(size) _Post_valid_impl_)
#define _Out_opt_cap_c_(size)                                                  \
  _SAL1_1_Source_(_Out_opt_cap_c_, (size),                                     \
                  _Pre_opt_cap_c_(size) _Post_valid_impl_)
#define _Out_bytecap_c_(size)                                                  \
  _SAL1_1_Source_(_Out_bytecap_c_, (size),                                     \
                  _Pre_bytecap_c_(size) _Post_valid_impl_)
#define _Out_opt_bytecap_c_(size)                                              \
  _SAL1_1_Source_(_Out_opt_bytecap_c_, (size),                                 \
                  _Pre_opt_bytecap_c_(size) _Post_valid_impl_)

// buffer capacity is described by another parameter multiplied by a constant
// expression
#define _Out_cap_m_(mult, size)                                                \
  _SAL1_1_Source_(_Out_cap_m_, (mult, size),                                   \
                  _Pre_cap_m_(mult, size) _Post_valid_impl_)
#define _Out_opt_cap_m_(mult, size)                                            \
  _SAL1_1_Source_(_Out_opt_cap_m_, (mult, size),                               \
                  _Pre_opt_cap_m_(mult, size) _Post_valid_impl_)
#define _Out_z_cap_m_(mult, size)                                              \
  _SAL1_1_Source_(_Out_z_cap_m_, (mult, size),                                 \
...
// buffer capacity is described by another pointer
// e.g. void Foo( _Out_ptrdiff_cap_(pchMax) char* pch, const char* pchMax ) {
// while pch < pchMax ) pch++; }
#define _Out_ptrdiff_cap_(size)                                                \
  _SAL1_1_Source_(_Out_ptrdiff_cap_, (size),                                   \
                  _Pre_ptrdiff_cap_(size) _Post_valid_impl_)
#define _Out_opt_ptrdiff_cap_(size)                                            \
  _SAL1_1_Source_(_Out_opt_ptrdiff_cap_, (size),                               \
                  _Pre_opt_ptrdiff_cap_(size) _Post_valid_impl_)

// buffer capacity is described by a complex expression
#define _Out_cap_x_(size)                                                      \
  _SAL1_1_Source_(_Out_cap_x_, (size), _Pre_cap_x_(size) _Post_valid_impl_)
#define _Out_opt_cap_x_(size)                                                  \
  _SAL1_1_Source_(_Out_opt_cap_x_, (size),                                     \
                  _Pre_opt_cap_x_(size) _Post_valid_impl_)
#define _Out_bytecap_x_(size)                                                  \
  _SAL1_1_Source_(_Out_bytecap_x_, (size),                                     \
                  _Pre_bytecap_x_(size) _Post_valid_impl_)
#define _Out_opt_bytecap_x_(size)                                              \
  _SAL1_1_Source_(_Out_opt_bytecap_x_, (size),                                 \
                  _Pre_opt_bytecap_x_(size) _Post_valid_impl_)

// a zero terminated string is filled into a buffer of given capacity
// e.g. void CopyStr( _In_z_ const char* szFrom, _Out_z_cap_(cchTo) char* szTo,
// size_t cchTo ); buffer capacity is described by another parameter
#define _Out_z_cap_(size)                                                      \
  _SAL1_1_Source_(_Out_z_cap_, (size),                                         \
                  _Pre_cap_(size) _Post_valid_impl_ _Post_z_)
#define _Out_opt_z_cap_(size)                                                  \
  _SAL1_1_Source_(_Out_opt_z_cap_, (size),                                     \
                  _Pre_opt_cap_(size) _Post_valid_impl_ _Post_z_)
#define _Out_z_bytecap_(size)                                                  \
  _SAL1_1_Source_(_Out_z_bytecap_, (size),                                     \
                  _Pre_bytecap_(size) _Post_valid_impl_ _Post_z_)
#define _Out_opt_z_bytecap_(size)                                              \
  _SAL1_1_Source_(_Out_opt_z_bytecap_, (size),                                 \
                  _Pre_opt_bytecap_(size) _Post_valid_impl_ _Post_z_)

// buffer capacity is described by a constant expression
#define _Out_z_cap_c_(size)                                                    \
  _SAL1_1_Source_(_Out_z_cap_c_, (size),                                       \
                  _Pre_cap_c_(size) _Post_valid_impl_ _Post_z_)
#define _Out_opt_z_cap_c_(size)                                                \
  _SAL1_1_Source_(_Out_opt_z_cap_c_, (size),                                   \
                  _Pre_opt_cap_c_(size) _Post_valid_impl_ _Post_z_)
#define _Out_z_bytecap_c_(size)                                                \
  _SAL1_1_Source_(_Out_z_bytecap_c_, (size),                                   \
                  _Pre_bytecap_c_(size) _Post_valid_impl_ _Post_z_)
...
#define _Out_z_cap_x_(size)                                                    \
  _SAL1_1_Source_(_Out_z_cap_x_, (size),                                       \
                  _Pre_cap_x_(size) _Post_valid_impl_ _Post_z_)
#define _Out_opt_z_cap_x_(size)                                                \
  _SAL1_1_Source_(_Out_opt_z_cap_x_, (size),                                   \
                  _Pre_opt_cap_x_(size) _Post_valid_impl_ _Post_z_)
#define _Out_z_bytecap_x_(size)                                                \
  _SAL1_1_Source_(_Out_z_bytecap_x_, (size),                                   \
                  _Pre_bytecap_x_(size) _Post_valid_impl_ _Post_z_)
...
// a zero terminated string is filled into a buffer of given capacity
// e.g. size_t CopyCharRange( _In_count_(cchFrom) const char* rgchFrom, size_t
// cchFrom, _Out_cap_post_count_(cchTo,return)) char* rgchTo, size_t cchTo );
...
#define _Out_opt_bytecap_post_bytecount_(cap, count)                           \
  _SAL1_1_Source_(_Out_opt_bytecap_post_bytecount_, (cap, count),              \
                  _Pre_opt_bytecap_(cap)                                       \
                      _Post_valid_impl_ _Post_bytecount_(count))

// a zero terminated string is filled into a buffer of given capacity
// e.g. size_t CopyStr( _In_z_ const char* szFrom,
// _Out_z_cap_post_count_(cchTo,return+1) char* szTo, size_t cchTo );
...
#define _Out_z_capcount_(capcount)                                             \
  _SAL1_1_Source_(_Out_z_capcount_, (capcount),                                \
                  _Pre_cap_(capcount)                                          \
                      _Post_valid_impl_ _Post_z_count_(capcount))
...
#define _Out_z_bytecapcount_(capcount)                                         \
  _SAL1_1_Source_(_Out_z_bytecapcount_, (capcount),                            \
                  _Pre_bytecap_(capcount)                                      \
                      _Post_valid_impl_ _Post_z_bytecount_(capcount))
...
// e.g. void ModifyIndices( _Inout_count_(cIndices) int* rgIndices, size_t
// cIndices );
#define _Inout_count_(size)                                                    \
  _SAL1_1_Source_(_Inout_count_, (size), _Prepost_count_(size))
...
#define _Inout_count_c_(size)                                                  \
  _SAL1_1_Source_(_Inout_count_c_, (size), _Prepost_count_c_(size))
...
#define _Inout_opt_bytecount_c_(size)                                          \
  _SAL1_1_Source_(_Inout_opt_bytecount_c_, (size),                             \
                  _Prepost_opt_bytecount_c_(size))

// nullterminated 'inout' buffers with initialized elements before and after the
// call e.g. void ModifyIndices( _Inout_count_(cIndices) int* rgIndices, size_t
// cIndices );
...
#define _Inout_opt_z_count_(size)                                              \
  _SAL1_1_Source_(_Inout_opt_z_count_, (size),                                 \
                  _Prepost_z_ _Prepost_opt_count_(size))
#define _Inout_z_bytecount_(size)                                              \
  _SAL1_1_Source_(_Inout_z_bytecount_, (size),                                 \
                  _Prepost_z_ _Prepost_bytecount_(size))
#define _Inout_opt_z_bytecount_(size)                                          \
  _SAL1_1_Source_(_Inout_opt_z_bytecount_, (size),                             \
                  _Prepost_z_ _Prepost_opt_bytecount_(size))

#define _Inout_z_count_c_(size)                                                \
  _SAL1_1_Source_(_Inout_z_count_c_, (size),                                   \
                  _Prepost_z_ _Prepost_count_c_(size))
#define _Inout_opt_z_count_c_(size)                                            \
  _SAL1_1_Source_(_Inout_opt_z_count_c_, (size),                               \
                  _Prepost_z_ _Prepost_opt_count_c_(size))
#define _Inout_z_bytecount_c_(size)                                            \
  _SAL1_1_Source_(_Inout_z_bytecount_c_, (size),                               \
                  _Prepost_z_ _Prepost_bytecount_c_(size))
#define _Inout_opt_z_bytecount_c_(size)                                        \
  _SAL1_1_Source_(_Inout_opt_z_bytecount_c_, (size),                           \
                  _Prepost_z_ _Prepost_opt_bytecount_c_(size))
...
#define _Inout_opt_ptrdiff_count_(size)                                        \
  _SAL1_1_Source_(_Inout_opt_ptrdiff_count_, (size),                           \
                  _Pre_opt_ptrdiff_count_(size))

#define _Inout_count_x_(size)                                                  \
  _SAL1_1_Source_(_Inout_count_x_, (size), _Prepost_count_x_(size))
...
#define _Inout_opt_bytecount_x_(size)                                          \
  _SAL1_1_Source_(_Inout_opt_bytecount_x_, (size),                             \
                  _Prepost_opt_bytecount_x_(size))

// e.g. void AppendToLPSTR( _In_ LPCSTR szFrom, _Inout_cap_(cchTo) LPSTR* szTo,
// size_t cchTo );
#define _Inout_cap_(size)                                                      \
  _SAL1_1_Source_(_Inout_cap_, (size), _Pre_valid_cap_(size) _Post_valid_)
#define _Inout_opt_cap_(size)                                                  \
  _SAL1_1_Source_(_Inout_opt_cap_, (size),                                     \
                  _Pre_opt_valid_cap_(size) _Post_valid_)
#define _Inout_bytecap_(size)                                                  \
  _SAL1_1_Source_(_Inout_bytecap_, (size),                                     \
                  _Pre_valid_bytecap_(size) _Post_valid_)
...
#define _Inout_opt_cap_c_(size)                                                \
  _SAL1_1_Source_(_Inout_opt_cap_c_, (size),                                   \
                  _Pre_opt_valid_cap_c_(size) _Post_valid_)
#define _Inout_bytecap_c_(size)                                                \
  _SAL1_1_Source_(_Inout_bytecap_c_, (size),                                   \
                  _Pre_valid_bytecap_c_(size) _Post_valid_)
#define _Inout_opt_bytecap_c_(size)                                            \
  _SAL1_1_Source_(_Inout_opt_bytecap_c_, (size),                               \
                  _Pre_opt_valid_bytecap_c_(size) _Post_valid_)
...
#define _Inout_opt_cap_x_(size)                                                \
  _SAL1_1_Source_(_Inout_opt_cap_x_, (size),                                   \
                  _Pre_opt_valid_cap_x_(size) _Post_valid_)
#define _Inout_bytecap_x_(size)                                                \
  _SAL1_1_Source_(_Inout_bytecap_x_, (size),                                   \
                  _Pre_valid_bytecap_x_(size) _Post_valid_)
#define _Inout_opt_bytecap_x_(size)                                            \
  _SAL1_1_Source_(_Inout_opt_bytecap_x_, (size),                               \
                  _Pre_opt_valid_bytecap_x_(size) _Post_valid_)

// inout string buffers with writable size
// e.g. void AppendStr( _In_z_ const char* szFrom, _Inout_z_cap_(cchTo) char*
// szTo, size_t cchTo );
#define _Inout_z_cap_(size)                                                    \
  _SAL1_1_Source_(_Inout_z_cap_, (size), _Pre_z_cap_(size) _Post_z_)
...
#define _Inout_opt_z_bytecap_(size)                                            \
  _SAL1_1_Source_(_Inout_opt_z_bytecap_, (size),                               \
                  _Pre_opt_z_bytecap_(size) _Post_z_)

#define _Inout_z_cap_c_(size)                                                  \
  _SAL1_1_Source_(_Inout_z_cap_c_, (size), _Pre_z_cap_c_(size) _Post_z_)
...
#define _Inout_opt_z_bytecap_c_(size)                                          \
  _SAL1_1_Source_(_Inout_opt_z_bytecap_c_, (size),                             \
                  _Pre_opt_z_bytecap_c_(size) _Post_z_)

#define _Inout_z_cap_x_(size)                                                  \
  _SAL1_1_Source_(_Inout_z_cap_x_, (size), _Pre_z_cap_x_(size) _Post_z_)
...
#define _Inout_opt_z_bytecap_x_(size)                                          \
  _SAL1_1_Source_(_Inout_opt_z_bytecap_x_, (size),                             \
                  _Pre_opt_z_bytecap_x_(size) _Post_z_)
...
#define _Deref_in_bound_                                                       \
  _SAL1_1_Source_(_Deref_in_bound_, (), _Deref_in_bound_impl_)
#define _Deref_out_bound_                                                      \
  _SAL1_1_Source_(_Deref_out_bound_, (), _Deref_out_bound_impl_)
#define _Deref_inout_bound_                                                    \
  _SAL1_1_Source_(_Deref_inout_bound_, (), _Deref_in_bound_ _Deref_out_bound_)
#define _Deref_ret_bound_                                                      \
  _SAL1_1_Source_(_Deref_ret_bound_, (), _Deref_ret_bound_impl_)

// e.g.  HRESULT HrCreatePoint( _Deref_out_opt_ POINT** ppPT );
#define _Deref_out_ _SAL1_1_Source_(_Deref_out_, (), _Out_ _Deref_post_valid_)
#define _Deref_out_opt_                                                        \
  _SAL1_1_Source_(_Deref_out_opt_, (), _Out_ _Deref_post_opt_valid_)
#define _Deref_opt_out_                                                        \
  _SAL1_1_Source_(_Deref_opt_out_, (), _Out_opt_ _Deref_post_valid_)
#define _Deref_opt_out_opt_                                                    \
  _SAL1_1_Source_(_Deref_opt_out_opt_, (), _Out_opt_ _Deref_post_opt_valid_)

// e.g.  void CloneString( _In_z_ const wchar_t* wzFrom, _Deref_out_z_ wchar_t**
// pWzTo );
#define _Deref_out_z_ _SAL1_1_Source_(_Deref_out_z_, (), _Out_ _Deref_post_z_)
#define _Deref_out_opt_z_                                                      \
  _SAL1_1_Source_(_Deref_out_opt_z_, (), _Out_ _Deref_post_opt_z_)
#define _Deref_opt_out_z_                                                      \
  _SAL1_1_Source_(_Deref_opt_out_z_, (), _Out_opt_ _Deref_post_z_)
...
// _Deref_pre_ ---
//
// describing conditions for array elements of dereferenced pointer parameters
// that must be met before the call

// e.g. void SaveStringArray( _In_count_(cStrings) _Deref_pre_z_ const wchar_t*
// const rgpwch[] );
#define _Deref_pre_z_                                                          \
  _SAL1_1_Source_(_Deref_pre_z_, (),                                           \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__zterm_impl) _Pre_valid_impl_)
#define _Deref_pre_opt_z_                                                      \
  _SAL1_1_Source_(_Deref_pre_opt_z_, (),                                       \
                  _Deref_pre1_impl_(__maybenull_impl_notref)                   \
                      _Deref_pre1_impl_(__zterm_impl) _Pre_valid_impl_)

// e.g. void FillInArrayOfStr32( _In_count_(cStrings) _Deref_pre_cap_c_(32)
// _Deref_post_z_ wchar_t* const rgpwch[] ); buffer capacity is described by
// another parameter
...
#define _Deref_pre_z_cap_(size)                                                \
  _SAL1_1_Source_(_Deref_pre_z_cap_, (size),                                   \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre2_impl_(__zterm_impl, __cap_impl(size))        \
                          _Pre_valid_impl_)
...
#define _Deref_pre_z_bytecap_(size)                                            \
  _SAL1_1_Source_(_Deref_pre_z_bytecap_, (size),                               \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre2_impl_(__zterm_impl, __bytecap_impl(size))    \
                          _Pre_valid_impl_)
...
#define _Deref_pre_z_cap_c_(size)                                              \
  _SAL1_1_Source_(_Deref_pre_z_cap_c_, (size),                                 \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre2_impl_(__zterm_impl, __cap_c_impl(size))      \
                          _Pre_valid_impl_)
...
#define _Deref_pre_z_bytecap_c_(size)                                          \
  _SAL1_1_Source_(_Deref_pre_z_bytecap_c_, (size),                             \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre2_impl_(__zterm_impl, __bytecap_c_impl(size))  \
                          _Pre_valid_impl_)
...
#define _Deref_pre_z_cap_x_(size)                                              \
  _SAL1_1_Source_(_Deref_pre_z_cap_x_, (size),                                 \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre2_impl_(__zterm_impl, __cap_x_impl(size))      \
                          _Pre_valid_impl_)
...
#define _Deref_pre_z_bytecap_x_(size)                                          \
  _SAL1_1_Source_(_Deref_pre_z_bytecap_x_, (size),                             \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre2_impl_(__zterm_impl, __bytecap_x_impl(size))  \
                          _Pre_valid_impl_)
...
#define _Deref_pre_valid_cap_(size)                                            \
  _SAL1_1_Source_(_Deref_pre_valid_cap_, (size),                               \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__cap_impl(size)) _Pre_valid_impl_)
...
// e.g. void SaveMatrix( _In_count_(n) _Deref_pre_count_(n) const Elem** matrix,
// size_t n ); valid buffer extent is described by another parameter
...
#define _Deref_pre_bytecount_(size)                                            \
  _SAL1_1_Source_(_Deref_pre_bytecount_, (size),                               \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__bytecount_impl(size))                \
                          _Pre_valid_impl_)
...
#define _Deref_pre_count_c_(size)                                              \
  _SAL1_1_Source_(_Deref_pre_count_c_, (size),                                 \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__count_c_impl(size))                  \
                          _Pre_valid_impl_)
...
#define _Deref_pre_bytecount_c_(size)                                          \
  _SAL1_1_Source_(_Deref_pre_bytecount_c_, (size),                             \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__bytecount_c_impl(size))              \
                          _Pre_valid_impl_)
...
#define _Deref_pre_count_x_(size)                                              \
  _SAL1_1_Source_(_Deref_pre_count_x_, (size),                                 \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__count_x_impl(size))                  \
                          _Pre_valid_impl_)
...
#define _Deref_pre_bytecount_x_(size)                                          \
  _SAL1_1_Source_(_Deref_pre_bytecount_x_, (size),                             \
                  _Deref_pre1_impl_(__notnull_impl_notref)                     \
                      _Deref_pre1_impl_(__bytecount_x_impl(size))              \
                          _Pre_valid_impl_)
...
// e.g. void PrintStringArray( _In_count_(cElems) _Deref_pre_valid_ LPCSTR
// rgStr[], size_t cElems );
...
#define _Deref_pre_notnull_                                                    \
  _SAL1_1_Source_(_Deref_pre_notnull_, (),                                     \
                  _Deref_pre1_impl_(__notnull_impl_notref))
#define _Deref_pre_maybenull_                                                  \
  _SAL1_1_Source_(_Deref_pre_maybenull_, (),                                   \
                  _Deref_pre1_impl_(__maybenull_impl_notref))
...
#define _Deref_pre_readonly_                                                   \
  _SAL1_1_Source_(_Deref_pre_readonly_, (),                                    \
                  _Deref_pre1_impl_(__readaccess_impl_notref))
#define _Deref_pre_writeonly_                                                  \
  _SAL1_1_Source_(_Deref_pre_writeonly_, (),                                   \
                  _Deref_pre1_impl_(__writeaccess_impl_notref))

//
// _Deref_post_ ---
//
// describing conditions for array elements or dereferenced pointer parameters
// that hold after the call

// e.g. void CloneString( _In_z_ const Wchar_t* wzIn _Out_ _Deref_post_z_
// wchar_t** pWzOut );
#define _Deref_post_z_                                                         \
  _SAL1_1_Source_(_Deref_post_z_, (),                                          \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__zterm_impl) _Post_valid_impl_)
#define _Deref_post_opt_z_                                                     \
  _SAL1_1_Source_(_Deref_post_opt_z_, (),                                      \
                  _Deref_post1_impl_(__maybenull_impl_notref)                  \
                      _Deref_post1_impl_(__zterm_impl) _Post_valid_impl_)

// e.g. HRESULT HrAllocateMemory( size_t cb, _Out_ _Deref_post_bytecap_(cb)
// void** ppv ); buffer capacity is described by another parameter
...
#define _Deref_post_z_cap_(size)                                               \
  _SAL1_1_Source_(_Deref_post_z_cap_, (size),                                  \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post2_impl_(__zterm_impl, __cap_impl(size))       \
                          _Post_valid_impl_)
...
#define _Deref_post_z_bytecap_(size)                                           \
  _SAL1_1_Source_(_Deref_post_z_bytecap_, (size),                              \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post2_impl_(__zterm_impl, __bytecap_impl(size))   \
                          _Post_valid_impl_)
...
#define _Deref_post_z_cap_c_(size)                                             \
  _SAL1_1_Source_(_Deref_post_z_cap_c_, (size),                                \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post2_impl_(__zterm_impl, __cap_c_impl(size))     \
                          _Post_valid_impl_)
...
#define _Deref_post_z_bytecap_c_(size)                                         \
  _SAL1_1_Source_(_Deref_post_z_bytecap_c_, (size),                            \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post2_impl_(__zterm_impl, __bytecap_c_impl(size)) \
                          _Post_valid_impl_)
...
#define _Deref_post_z_cap_x_(size)                                             \
  _SAL1_1_Source_(_Deref_post_z_cap_x_, (size),                                \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post2_impl_(__zterm_impl, __cap_x_impl(size))     \
                          _Post_valid_impl_)
...
#define _Deref_post_z_bytecap_x_(size)                                         \
  _SAL1_1_Source_(_Deref_post_z_bytecap_x_, (size),                            \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post2_impl_(__zterm_impl, __bytecap_x_impl(size)) \
                          _Post_valid_impl_)
...
// e.g. HRESULT HrAllocateZeroInitializedMemory( size_t cb, _Out_
// _Deref_post_bytecount_(cb) void** ppv ); valid buffer extent is described by
// another parameter
#define _Deref_post_count_(size)                                               \
  _SAL1_1_Source_(_Deref_post_count_, (size),                                  \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__count_impl(size))                   \
                          _Post_valid_impl_)
...
#define _Deref_post_bytecount_(size)                                           \
  _SAL1_1_Source_(_Deref_post_bytecount_, (size),                              \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__bytecount_impl(size))               \
                          _Post_valid_impl_)
...
#define _Deref_post_count_c_(size)                                             \
  _SAL1_1_Source_(_Deref_post_count_c_, (size),                                \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__count_c_impl(size))                 \
                          _Post_valid_impl_)
...
#define _Deref_post_bytecount_c_(size)                                         \
  _SAL1_1_Source_(_Deref_post_bytecount_c_, (size),                            \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__bytecount_c_impl(size))             \
                          _Post_valid_impl_)
...
#define _Deref_post_count_x_(size)                                             \
  _SAL1_1_Source_(_Deref_post_count_x_, (size),                                \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__count_x_impl(size))                 \
                          _Post_valid_impl_)
...
#define _Deref_post_bytecount_x_(size)                                         \
  _SAL1_1_Source_(_Deref_post_bytecount_x_, (size),                            \
                  _Deref_post1_impl_(__notnull_impl_notref)                    \
                      _Deref_post1_impl_(__bytecount_x_impl(size))             \
                          _Post_valid_impl_)
...
// e.g. void GetStrings( _Out_count_(cElems) _Deref_post_valid_ LPSTR const
// rgStr[], size_t cElems );
...
#define _Deref_post_notnull_                                                   \
  _SAL1_1_Source_(_Deref_post_notnull_, (),                                    \
                  _Deref_post1_impl_(__notnull_impl_notref))
#define _Deref_post_maybenull_                                                 \
  _SAL1_1_Source_(_Deref_post_maybenull_, (),                                  \
                  _Deref_post1_impl_(__maybenull_impl_notref))
...
#define _Deref_ret_z_                                                          \
  _SAL1_1_Source_(_Deref_ret_z_, (),                                           \
                  _Deref_ret1_impl_(__notnull_impl_notref)                     \
                      _Deref_ret1_impl_(__zterm_impl))
#define _Deref_ret_opt_z_                                                      \
  _SAL1_1_Source_(_Deref_ret_opt_z_, (),                                       \
                  _Deref_ret1_impl_(__maybenull_impl_notref)                   \
                      _Ret1_impl_(__zterm_impl))

//
// special _Deref_ ---
//
#define _Deref2_pre_readonly_                                                  \
  _SAL1_1_Source_(_Deref2_pre_readonly_, (),                                   \
                  _Deref2_pre1_impl_(__readaccess_impl_notref))
...
#define _Ret_opt_valid_                                                        \
  _SAL1_1_Source_(_Ret_opt_valid_, (),                                         \
                  _Ret1_impl_(__maybenull_impl_notref) _Ret_valid_impl_)
#define _Ret_opt_z_                                                            \
  _SAL1_1_Source_(_Ret_opt_z_, (),                                             \
                  _Ret2_impl_(__maybenull_impl, __zterm_impl)                  \
                      _Ret_valid_impl_)
...
#define _Ret_bytecap_(size)                                                    \
  _SAL1_1_Source_(_Ret_bytecap_, (size),                                       \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__bytecap_impl(size)))
...
#define _Ret_bytecap_c_(size)                                                  \
  _SAL1_1_Source_(_Ret_bytecap_c_, (size),                                     \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__bytecap_c_impl(size)))
...
#define _Ret_bytecap_x_(size)                                                  \
  _SAL1_1_Source_(_Ret_bytecap_x_, (size),                                     \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__bytecap_x_impl(size)))
...
#define _Ret_z_cap_(size)                                                      \
  _SAL1_1_Source_(_Ret_z_cap_, (size),                                         \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret2_impl_(__zterm_impl, __cap_impl(size))              \
                          _Ret_valid_impl_)
...
#define _Ret_z_bytecap_(size)                                                  \
  _SAL1_1_Source_(_Ret_z_bytecap_, (size),                                     \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret2_impl_(__zterm_impl, __bytecap_impl(size))          \
                          _Ret_valid_impl_)
...
#define _Ret_count_(size)                                                      \
  _SAL1_1_Source_(_Ret_count_, (size),                                         \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__count_impl(size)) _Ret_valid_impl_)
...
#define _Ret_bytecount_(size)                                                  \
  _SAL1_1_Source_(_Ret_bytecount_, (size),                                     \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__bytecount_impl(size)) _Ret_valid_impl_)
...
#define _Ret_count_c_(size)                                                    \
  _SAL1_1_Source_(_Ret_count_c_, (size),                                       \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__count_c_impl(size)) _Ret_valid_impl_)
...
#define _Ret_bytecount_c_(size)                                                \
  _SAL1_1_Source_(_Ret_bytecount_c_, (size),                                   \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__bytecount_c_impl(size)) _Ret_valid_impl_)
...
#define _Ret_count_x_(size)                                                    \
  _SAL1_1_Source_(_Ret_count_x_, (size),                                       \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__count_x_impl(size)) _Ret_valid_impl_)
...
#define _Ret_bytecount_x_(size)                                                \
  _SAL1_1_Source_(_Ret_bytecount_x_, (size),                                   \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret1_impl_(__bytecount_x_impl(size)) _Ret_valid_impl_)
...
#define _Ret_z_count_(size)                                                    \
  _SAL1_1_Source_(_Ret_z_count_, (size),                                       \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret2_impl_(__zterm_impl, __count_impl(size))            \
                          _Ret_valid_impl_)
...
#define _Ret_z_bytecount_(size)                                                \
  _SAL1_1_Source_(_Ret_z_bytecount_, (size),                                   \
                  _Ret1_impl_(__notnull_impl_notref)                           \
                      _Ret2_impl_(__zterm_impl, __bytecount_impl(size))        \
                          _Ret_valid_impl_)
...
#define _Pre_opt_z_                                                            \
  _SAL1_1_Source_(_Pre_opt_z_, (),                                             \
                  _Pre1_impl_(__maybenull_impl_notref)                         \
                      _Pre1_impl_(__zterm_impl) _Pre_valid_impl_)

// restrict access rights
#define _Pre_readonly_                                                         \
  _SAL1_1_Source_(_Pre_readonly_, (), _Pre1_impl_(__readaccess_impl_notref))
#define _Pre_writeonly_                                                        \
  _SAL1_1_Source_(_Pre_writeonly_, (), _Pre1_impl_(__writeaccess_impl_notref))

// e.g. void FreeMemory( _Pre_bytecap_(cb) _Post_ptr_invalid_ void* pv, size_t
// cb ); buffer capacity described by another parameter
#define _Pre_cap_(size)                                                        \
  _SAL1_1_Source_(_Pre_cap_, (size),                                           \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__cap_impl(size)))
...
#define _Pre_bytecap_c_(size)                                                  \
  _SAL1_1_Source_(_Pre_bytecap_c_, (size),                                     \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecap_c_impl(size)))
...
#define _Pre_cap_c_one_                                                        \
  _SAL1_1_Source_(_Pre_cap_c_one_, (),                                         \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__cap_c_one_notref_impl))
...
#define _Pre_cap_m_(mult, size)                                                \
  _SAL1_1_Source_(_Pre_cap_m_, (mult, size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__mult_impl(mult, size)))
#define _Pre_opt_cap_m_(mult, size)                                            \
  _SAL1_1_Source_(_Pre_opt_cap_m_, (mult, size),                               \
                  _Pre1_impl_(__maybenull_impl_notref)                         \
                      _Pre1_impl_(__mult_impl(mult, size)))

// buffer capacity described by size of other buffer, only used by dangerous
// legacy APIs e.g. int strcpy(_Pre_cap_for_(src) char* dst, const char* src);
#define _Pre_cap_for_(param)                                                   \
  _SAL1_1_Source_(_Pre_cap_for_, (param),                                      \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__cap_for_impl(param)))
...
#define _Pre_bytecap_x_(size)                                                  \
  _SAL1_1_Source_(_Pre_bytecap_x_, (size),                                     \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecap_x_impl(size)))
...
// e.g. void AppendStr( _Pre_z_ const char* szFrom, _Pre_z_cap_(cchTo) _Post_z_
// char* szTo, size_t cchTo );
#define _Pre_z_cap_(size)                                                      \
  _SAL1_1_Source_(_Pre_z_cap_, (size),                                         \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre2_impl_(__zterm_impl, __cap_impl(size))              \
                          _Pre_valid_impl_)
...
#define _Pre_z_bytecap_(size)                                                  \
  _SAL1_1_Source_(_Pre_z_bytecap_, (size),                                     \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre2_impl_(__zterm_impl, __bytecap_impl(size))          \
                          _Pre_valid_impl_)
...
#define _Pre_z_cap_c_(size)                                                    \
  _SAL1_1_Source_(_Pre_z_cap_c_, (size),                                       \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre2_impl_(__zterm_impl, __cap_c_impl(size))            \
                          _Pre_valid_impl_)
...
#define _Pre_z_bytecap_c_(size)                                                \
  _SAL1_1_Source_(_Pre_z_bytecap_c_, (size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre2_impl_(__zterm_impl, __bytecap_c_impl(size))        \
                          _Pre_valid_impl_)
...
#define _Pre_z_cap_x_(size)                                                    \
  _SAL1_1_Source_(_Pre_z_cap_x_, (size),                                       \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre2_impl_(__zterm_impl, __cap_x_impl(size))            \
                          _Pre_valid_impl_)
...
#define _Pre_z_bytecap_x_(size)                                                \
  _SAL1_1_Source_(_Pre_z_bytecap_x_, (size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre2_impl_(__zterm_impl, __bytecap_x_impl(size))        \
                          _Pre_valid_impl_)
...
#define _Pre_valid_cap_(size)                                                  \
  _SAL1_1_Source_(_Pre_valid_cap_, (size),                                     \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__cap_impl(size)) _Pre_valid_impl_)
...
#define _Pre_valid_bytecap_(size)                                              \
  _SAL1_1_Source_(_Pre_valid_bytecap_, (size),                                 \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecap_impl(size)) _Pre_valid_impl_)
...
#define _Pre_valid_cap_c_(size)                                                \
  _SAL1_1_Source_(_Pre_valid_cap_c_, (size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__cap_c_impl(size)) _Pre_valid_impl_)
...
#define _Pre_valid_bytecap_c_(size)                                            \
  _SAL1_1_Source_(_Pre_valid_bytecap_c_, (size),                               \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecap_c_impl(size)) _Pre_valid_impl_)
...
#define _Pre_valid_cap_x_(size)                                                \
  _SAL1_1_Source_(_Pre_valid_cap_x_, (size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__cap_x_impl(size)) _Pre_valid_impl_)
...
#define _Pre_valid_bytecap_x_(size)                                            \
  _SAL1_1_Source_(_Pre_valid_bytecap_x_, (size),                               \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecap_x_impl(size)) _Pre_valid_impl_)
#define _Pre_opt_valid_bytecap_x_(size)                                        \
  _SAL1_1_Source_(_Pre_opt_valid_bytecap_x_, (size),                           \
                  _Pre1_impl_(__maybenull_impl_notref)                         \
                      _Pre1_impl_(__bytecap_x_impl(size)) _Pre_valid_impl_)

// e.g. void AppendCharRange( _Pre_count_(cchFrom) const char* rgFrom, size_t
// cchFrom, _Out_z_cap_(cchTo) char* szTo, size_t cchTo ); Valid buffer extent
// described by another parameter
...
#define _Pre_count_c_(size)                                                    \
  _SAL1_1_Source_(_Pre_count_c_, (size),                                       \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__count_c_impl(size)) _Pre_valid_impl_)
...
#define _Pre_bytecount_c_(size)                                                \
  _SAL1_1_Source_(_Pre_bytecount_c_, (size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecount_c_impl(size)) _Pre_valid_impl_)
...
#define _Pre_count_x_(size)                                                    \
  _SAL1_1_Source_(_Pre_count_x_, (size),                                       \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__count_x_impl(size)) _Pre_valid_impl_)
...
#define _Pre_bytecount_x_(size)                                                \
  _SAL1_1_Source_(_Pre_bytecount_x_, (size),                                   \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__bytecount_x_impl(size)) _Pre_valid_impl_)
...
#define _Pre_ptrdiff_count_(ptr)                                               \
  _SAL1_1_Source_(_Pre_ptrdiff_count_, (ptr),                                  \
                  _Pre1_impl_(__notnull_impl_notref)                           \
                      _Pre1_impl_(__count_x_impl(__ptrdiff(ptr)))              \
                          _Pre_valid_impl_)
...
// char * _Source, _In_ size_t _Count) buffer maybe zero-terminated after the
// call
#define _Post_maybez_                                                          \
  _SAL_L_Source_(_Post_maybez_, (), _Post1_impl_(__maybezterm_impl))

// e.g. SIZE_T HeapSize( _In_ HANDLE hHeap, DWORD dwFlags, _Pre_notnull_
// _Post_bytecap_(return) LPCVOID lpMem );
#define _Post_cap_(size)                                                       \
  _SAL1_1_Source_(_Post_cap_, (size), _Post1_impl_(__cap_impl(size)))
...
#define _Post_count_(size)                                                     \
  _SAL1_1_Source_(_Post_count_, (size),                                        \
                  _Post1_impl_(__count_impl(size)) _Post_valid_impl_)
#define _Post_bytecount_(size)                                                 \
  _SAL1_1_Source_(_Post_bytecount_, (size),                                    \
                  _Post1_impl_(__bytecount_impl(size)) _Post_valid_impl_)
#define _Post_count_c_(size)                                                   \
  _SAL1_1_Source_(_Post_count_c_, (size),                                      \
                  _Post1_impl_(__count_c_impl(size)) _Post_valid_impl_)
...
#define _Post_count_x_(size)                                                   \
  _SAL1_1_Source_(_Post_count_x_, (size),                                      \
                  _Post1_impl_(__count_x_impl(size)) _Post_valid_impl_)
...
#define _Prepost_opt_z_                                                        \
  _SAL1_1_Source_(_Prepost_opt_z_, (), _Pre_opt_z_ _Post_z_)
...
#define _Prepost_opt_count_(size)                                              \
  _SAL1_1_Source_(_Prepost_opt_count_, (size),                                 \
                  _Pre_opt_count_(size) _Post_count_(size))
#define _Prepost_bytecount_(size)                                              \
  _SAL1_1_Source_(_Prepost_bytecount_, (size),                                 \
                  _Pre_bytecount_(size) _Post_bytecount_(size))
...
#define _Prepost_count_c_(size)                                                \
  _SAL1_1_Source_(_Prepost_count_c_, (size),                                   \
                  _Pre_count_c_(size) _Post_count_c_(size))
#define _Prepost_opt_count_c_(size)                                            \
  _SAL1_1_Source_(_Prepost_opt_count_c_, (size),                               \
                  _Pre_opt_count_c_(size) _Post_count_c_(size))
#define _Prepost_bytecount_c_(size)                                            \
  _SAL1_1_Source_(_Prepost_bytecount_c_, (size),                               \
                  _Pre_bytecount_c_(size) _Post_bytecount_c_(size))
...
#define _Prepost_count_x_(size)                                                \
  _SAL1_1_Source_(_Prepost_count_x_, (size),                                   \
                  _Pre_count_x_(size) _Post_count_x_(size))
#define _Prepost_opt_count_x_(size)                                            \
  _SAL1_1_Source_(_Prepost_opt_count_x_, (size),                               \
                  _Pre_opt_count_x_(size) _Post_count_x_(size))
#define _Prepost_bytecount_x_(size)                                            \
  _SAL1_1_Source_(_Prepost_bytecount_x_, (size),                               \
                  _Pre_bytecount_x_(size) _Post_bytecount_x_(size))
...
#define _Prepost_valid_                                                        \
  _SAL1_1_Source_(_Prepost_valid_, (), _Pre_valid_ _Post_valid_)
#define _Prepost_opt_valid_                                                    \
  _SAL1_1_Source_(_Prepost_opt_valid_, (), _Pre_opt_valid_ _Post_valid_)
...
// describing conditions for array elements or dereferenced pointer parameters
// that hold before and after the call

#define _Deref_prepost_z_                                                      \
  _SAL1_1_Source_(_Deref_prepost_z_, (), _Deref_pre_z_ _Deref_post_z_)
#define _Deref_prepost_opt_z_                                                  \
  _SAL1_1_Source_(_Deref_prepost_opt_z_, (),                                   \
                  _Deref_pre_opt_z_ _Deref_post_opt_z_)

#define _Deref_prepost_cap_(size)                                              \
  _SAL1_1_Source_(_Deref_prepost_cap_, (size),                                 \
                  _Deref_pre_cap_(size) _Deref_post_cap_(size))
...
#define _Deref_prepost_cap_x_(size)                                            \
  _SAL1_1_Source_(_Deref_prepost_cap_x_, (size),                               \
                  _Deref_pre_cap_x_(size) _Deref_post_cap_x_(size))
...
#define _Deref_prepost_z_cap_(size)                                            \
  _SAL1_1_Source_(_Deref_prepost_z_cap_, (size),                               \
                  _Deref_pre_z_cap_(size) _Deref_post_z_cap_(size))
...
#define _Deref_prepost_count_(size)                                            \
  _SAL1_1_Source_(_Deref_prepost_count_, (size),                               \
                  _Deref_pre_count_(size) _Deref_post_count_(size))
...
#define _Deref_prepost_valid_                                                  \
  _SAL1_1_Source_(_Deref_prepost_valid_, (),                                   \
                  _Deref_pre_valid_ _Deref_post_valid_)
#define _Deref_prepost_opt_valid_                                              \
  _SAL1_1_Source_(_Deref_prepost_opt_valid_, (),                               \
                  _Deref_pre_opt_valid_ _Deref_post_opt_valid_)
...
#define _Deref_out_z_cap_c_(size)                                              \
  _SAL1_1_Source_(_Deref_out_z_cap_c_, (size),                                 \
                  _Deref_pre_cap_c_(size) _Deref_post_z_)
#define _Deref_inout_z_cap_c_(size)                                            \
  _SAL1_1_Source_(_Deref_inout_z_cap_c_, (size),                               \
                  _Deref_pre_z_cap_c_(size) _Deref_post_z_)
#define _Deref_out_z_bytecap_c_(size)                                          \
  _SAL1_1_Source_(_Deref_out_z_bytecap_c_, (size),                             \
                  _Deref_pre_bytecap_c_(size) _Deref_post_z_)
#define _Deref_inout_z_bytecap_c_(size)                                        \
  _SAL1_1_Source_(_Deref_inout_z_bytecap_c_, (size),                           \
                  _Deref_pre_z_bytecap_c_(size) _Deref_post_z_)
...
#define _Always_impl_(annos)                                                   \
  _Group_(annos _SAL_nop_impl_) _On_failure_impl_(annos _SAL_nop_impl_)
...
#define _NullNull_terminated_impl_                                             \
  _Group_(_SA_annotes1(SAL_nullTerminated, __yes) _SA_annotes1(                \
...
#define _Points_to_data_impl_                                                  \
  _At_(*_Curr_, _SA_annotes1(SAL_mayBePointer, __no))
...
#define _Readable_bytes_impl_(size)                                            \
  _SA_annotes1(SAL_readableTo, byteCount(size))
#define _Readable_elements_impl_(size)                                         \
  _SA_annotes1(SAL_readableTo, elementCount(size))
...
#define _Writable_bytes_impl_(size)                                            \
  _SA_annotes1(SAL_writableTo, byteCount(size))
#define _Writable_elements_impl_(size)                                         \
  _SA_annotes1(SAL_writableTo, elementCount(size))
...
#define _Deref_out_range_impl_(min, max)                                       \
  _Deref_post_impl_ _Range_impl_(min, max)
...
#define _SA_annotes2(n, pp1, pp2)                                              \
  __declspec(#n "(" _SA_SPECSTRIZE(pp1) "," _SA_SPECSTRIZE(pp2) ")")
#define _SA_annotes3(n, pp1, pp2, pp3)                                         \
  __declspec(#n "(" _SA_SPECSTRIZE(pp1) "," _SA_SPECSTRIZE(                    \
      pp2) "," _SA_SPECSTRIZE(pp3) ")")
...
__ANNOTATION(SAL_adt(__AuToQuOtE __In_impl_ char *,
                     __AuToQuOtE __In_impl_ char *));
__ANNOTATION(SAL_add_adt_property(__AuToQuOtE __In_impl_ char *,
                                  __AuToQuOtE __In_impl_ char *));
__ANNOTATION(SAL_remove_adt_property(__AuToQuOtE __In_impl_ char *,
                                     __AuToQuOtE __In_impl_ char *));
...
__ANNOTATION(SAL_entrypoint(__AuToQuOtE __In_impl_ char *,
                            __AuToQuOtE __In_impl_ char *));
...
#define _On_failure_impl_(annos)                                               \
  [SAL_context(p1 = "SAL_failed")] _Group_(                                    \
...
#define _Use_decl_anno_impl_                                                   \
  _SA_annotes0(SAL_useHeader) // this is a special case!
...
#define _On_failure_impl_(annos)                                               \
  _SA_annotes1(SAL_context, SAL_failed)                                        \
      _Group_(_Post_impl_ _Group_(_SAL_nop_impl_ annos))
...
#define __maybenull_impl                                                       \
  _SA_annotes0(SAL_maybenull) // _SA_annotes1(SAL_null, __maybe)

#define __valid_impl _SA_annotes0(SAL_valid) // _SA_annotes1(SAL_valid, __yes)
#define __notvalid_impl                                                        \
  _SA_annotes0(SAL_notvalid) // _SA_annotes1(SAL_valid, __no)
#define __maybevalid_impl                                                      \
  _SA_annotes0(SAL_maybevalid) // _SA_annotes1(SAL_valid, __maybe)
...
#define __cap_c_one_notref_impl                                                \
  _Notref_ _SA_annotes1(SAL_writableTo, elementCount(1))
#define __cap_for_impl(param)                                                  \
  _SA_annotes1(SAL_writableTo, inexpressibleCount(sizeof(param)))
#define __cap_x_impl(size)                                                     \
  _SA_annotes1(SAL_writableTo, inexpressibleCount(#size))
...
#define __count_x_impl(size)                                                   \
  _SA_annotes1(SAL_readableTo, inexpressibleCount(#size))
...
#define __bytecount_x_impl(size)                                               \
  _SA_annotes1(SAL_readableTo, inexpressibleCount(#size))
...
#define _Use_decl_anno_impl_                                                   \
  __declspec("SAL_useHeader()") // this is a special case!
...
#define _Deref_pre3_impl_(p1, p2, p3)                                          \
  _Deref_pre_impl_ p1 _Deref_pre_impl_ p2 _Deref_pre_impl_ p3
...
#define __inner_exceptthat _SA_annotes0(SAL_except)
    ...
    !defined(RC_INVOKED) && defined(_PFT_VER) // ][
...
/*
 ...
 Annotations may be placed before either a function parameter's type or its
 return type, and describe the function's behavior regarding the parameter or
 return value. There are two classes of annotations: buffer annotations and
 advanced annotations. Buffer annotations describe how functions use their
 pointer parameters, and advanced annotations either describe complex/unusual
 buffer behavior, or provide additional information about a parameter that is
 ...
 buffer parameters or return values for a function. Each of these annotations
 describes a single buffer (which could be a string, a fixed-length or
 variable-length array, or just a pointer) that the function interacts with:
 where it is, how large it is, how much is initialized, and what the function
 does with it.

 The appropriate macro for a given buffer can be constructed using the table
 below. Just pick the appropriate values from each category, and combine them
 together with a leading underscore. Some combinations of values do not make
 sense as buffer annotations. Only meaningful annotations can be added to your
 code; for a list of these, see the buffer annotation definitions section.
 ...
 _deref, the function will provide the buffer, and it will be uninitialized at
 exit. Otherwise, the caller must provide the buffer. This should only be used
            for alloc and free functions.
 _in    : The function will only read from the buffer. The caller must provide
 the buffer and initialize it. Cannot be used with _deref. _out   : The function
 will only write to the buffer. If used on the return value or with _deref, the
 function will provide the buffer and initialize it. Otherwise, the caller must
 provide the buffer, and the function will initialize it. _inout : The function
 may freely read from and write to the buffer. The caller must provide the
 buffer and initialize it. If used with _deref, the buffer may be reallocated by
 the function.

 Size: Describes the total size of the buffer. This may be less than the space
 actually allocated for the buffer, in which case it describes the accessible
 amount.

 <>      : No buffer size is given. If the type specifies the buffer size (such
 as with LPSTR and LPWSTR), that amount is used. Otherwise, the buffer is one
             element long. Must be used with _in, _out, or _inout.
 _ecount : The buffer size is an explicit element count.
 _bcount : The buffer size is an explicit byte count.

 Output: Describes how much of the buffer will be initialized by the function.
 For _inout buffers, this also describes how much is initialized at entry. Omit
 this category for _in buffers; they must be fully initialized by the caller.

 <>    : The type specifies how much is initialized. For instance, a function
 initializing an LPWSTR must NULL-terminate the string. _full : The function
 initializes the entire buffer. _part : The function initializes part of the
 buffer, and explicitly indicates how much.

 NullTerm: States if the present of a '\0' marks the end of valid elements in
 the buffer. _z    : A '\0' indicated the end of the buffer _nz     : The buffer
 may not be null terminated and a '\0' does not indicate the end of the buffer.
 Optional: Describes if the buffer itself is optional.

 <>   : The pointer to the buffer must not be NULL.
 _opt : The pointer to the buffer might be NULL. It will be checked before being
 dereferenced.
 ...
 with _ecount or _bcount but not _part. (size,length) : The buffer's total size
 and initialized length are given. Use with _ecount_part and _bcount_part.
 ...
 LWSTDAPI_(BOOL) StrToIntExA(
     ...
     DWORD dwFlags,
     __out int *piRet                     -- A pointer whose dereference will be
 filled in.
 );
 ...
 that might be NULL.
     __inout IPropertyStore *ppsStore     -- An initialized parameter that may
 be freely used
                                          --   and modified.
 );
 ...
 set to an
                                          --   uninitialized buffer with cb
 bytes.
 );

 __inout_bcount_full(cb) : A buffer with cb elements that is fully initialized
 at entry and exit, and may be written to by this function.
 ...
 buffer macros. These may be used either to annotate buffer parameters that
 involve complex or conditional behavior, or to enrich existing annotations with
 additional information.

 __success(expr) f :
     <expr> indicates whether function f succeeded or not. If <expr> is true at
 exit, all the function's guarantees (as given by other annotations) must hold.
 If <expr> is false at exit, the caller should not expect any of the function's
 guarantees to hold. If not used, the function must always satisfy its
 guarantees. Added automatically to functions that indicate success in standard
 ways, such as by returning an HRESULT.

 __nullterminated p :
     Pointer p is a buffer that may be read or written up to and including the
 first NULL character or pointer. May be used on typedefs, which marks valid
 (properly initialized) instances of that type as being NULL-terminated.

 __nullnullterminated p :
     Pointer p is a buffer that may be read or written up to and including the
 first sequence of two NULL characters or pointers. May be used on typedefs,
 which marks valid instances of that type as being double-NULL terminated.
 ...
 __typefix(ctype) v :
     Value v should be treated as an instance of ctype, rather than its declared
 type.
 ...
 __fallthrough :
     Annotates switch statement labels where fall-through is desired, to
 distinguish from forgotten break statements.
 ...
 typedef __nullterminated WCHAR* LPWSTR : Initialized LPWSTRs are
 NULL-terminated strings.

 __out_ecount(cch) __typefix(LPWSTR) void *psz : psz is a buffer parameter which
 will be a NULL-terminated WCHAR string at exit, and which initially contains
 cch WCHARs.
...
#define __inner_control_entrypoint(category)                                   \
  _SA_annotes2(SAL_entrypoint, controlEntry, category)

/*
 Pre-defined data entry point categories include: Registry, File, Network.
*/
#define __inner_data_entrypoint(category)                                      \
  _SA_annotes2(SAL_entrypoint, dataEntry, category)
...
#define __post_deref_except_maybenull                                          \
  __post __deref __inner_exceptthat _Maybenull_impl_
#define __pre_deref_except_maybenull                                           \
  __pre __deref __inner_exceptthat _Maybenull_impl_

#define __inexpressible_readableTo(size)                                       \
  _Readable_elements_impl_(_Inexpressible_(size))
#define __inexpressible_writableTo(size)                                       \
  _Writable_elements_impl_(_Inexpressible_(size))
...
#define __in_bcount(size)                                                      \
  _SAL1_Source_(__in_bcount, (size), _In_reads_bytes_(size))
#define __in_z _SAL1_Source_(__in_z, (), _In_z_)
#define __in_ecount_z(size)                                                    \
  _SAL1_Source_(__in_ecount_z, (size), _In_reads_z_(size))
...
#define __in_ecount_nz(size)                                                   \
  _SAL1_Source_(__in_ecount_nz, (size), __in_ecount(size))
#define __in_bcount_nz(size)                                                   \
  _SAL1_Source_(__in_bcount_nz, (size), __in_bcount(size))
#define __out _SAL1_Source_(__out, (), _Out_)
#define __out_ecount(size)                                                     \
  _SAL1_Source_(__out_ecount, (size), _Out_writes_(size))
#define __out_bcount(size)                                                     \
  _SAL1_Source_(__out_bcount, (size), _Out_writes_bytes_(size))
#define __out_ecount_part(size, length)                                        \
  _SAL1_Source_(__out_ecount_part, (size, length),                             \
                _Out_writes_to_(size, length))
#define __out_bcount_part(size, length)                                        \
  _SAL1_Source_(__out_bcount_part, (size, length),                             \
                _Out_writes_bytes_to_(size, length))
#define __out_ecount_full(size)                                                \
  _SAL1_Source_(__out_ecount_full, (size), _Out_writes_all_(size))
#define __out_bcount_full(size)                                                \
  _SAL1_Source_(__out_bcount_full, (size), _Out_writes_bytes_all_(size))
#define __out_z                                                                \
  _SAL1_Source_(__out_z, (), __post __valid __refparam __post __nullterminated)
#define __out_z_opt                                                            \
  _SAL1_Source_(__out_z_opt, (),                                               \
                __post __valid __refparam __post __nullterminated              \
...
#define __out_ecount_full_z(size)                                              \
  _SAL1_Source_(__out_ecount_full_z, (size),                                   \
                __out_ecount_full(size) __post __nullterminated)
#define __out_bcount_full_z(size)                                              \
  _SAL1_Source_(__out_bcount_full_z, (size),                                   \
                __out_bcount_full(size) __post __nullterminated)
#define __out_nz _SAL1_Source_(__out_nz, (), __post __valid __refparam)
#define __out_nz_opt                                                           \
  _SAL1_Source_(__out_nz_opt, (),                                              \
                __post __valid __refparam __post_except_maybenull_)
#define __out_ecount_nz(size)                                                  \
  _SAL1_Source_(__out_ecount_nz, (size),                                       \
                __ecount(size) __post __valid __refparam)
#define __out_bcount_nz(size)                                                  \
  _SAL1_Source_(__out_bcount_nz, (size),                                       \
                __bcount(size) __post __valid __refparam)
#define __inout _SAL1_Source_(__inout, (), _Inout_)
#define __inout_ecount(size)                                                   \
  _SAL1_Source_(__inout_ecount, (size), _Inout_updates_(size))
#define __inout_bcount(size)                                                   \
  _SAL1_Source_(__inout_bcount, (size), _Inout_updates_bytes_(size))
#define __inout_ecount_part(size, length)                                      \
  _SAL1_Source_(__inout_ecount_part, (size, length),                           \
                _Inout_updates_to_(size, length))
#define __inout_bcount_part(size, length)                                      \
  _SAL1_Source_(__inout_bcount_part, (size, length),                           \
...
#define __inout_ecount_z(size)                                                 \
  _SAL1_Source_(__inout_ecount_z, (size), _Inout_updates_z_(size))
...
#define __inout_ecount_nz(size)                                                \
  _SAL1_Source_(__inout_ecount_nz, (size), __inout_ecount(size))
#define __inout_bcount_nz(size)                                                \
  _SAL1_Source_(__inout_bcount_nz, (size), __inout_bcount(size))
...
#define __in_ecount_opt(size)                                                  \
  _SAL1_Source_(__in_ecount_opt, (size), _In_reads_opt_(size))
#define __in_bcount_opt(size)                                                  \
  _SAL1_Source_(__in_bcount_opt, (size), _In_reads_bytes_opt_(size))
#define __in_z_opt _SAL1_Source_(__in_z_opt, (), _In_opt_z_)
#define __in_ecount_z_opt(size)                                                \
  _SAL1_Source_(__in_ecount_z_opt, (size),                                     \
                __in_ecount_opt(size) __pre __nullterminated)
#define __in_bcount_z_opt(size)                                                \
  _SAL1_Source_(__in_bcount_z_opt, (size),                                     \
                __in_bcount_opt(size) __pre __nullterminated)
#define __in_nz_opt _SAL1_Source_(__in_nz_opt, (), __in_opt)
#define __in_ecount_nz_opt(size)                                               \
  _SAL1_Source_(__in_ecount_nz_opt, (size), __in_ecount_opt(size))
#define __in_bcount_nz_opt(size)                                               \
  _SAL1_Source_(__in_bcount_nz_opt, (size), __in_bcount_opt(size))
#define __out_opt _SAL1_Source_(__out_opt, (), _Out_opt_)
#define __out_ecount_opt(size)                                                 \
  _SAL1_Source_(__out_ecount_opt, (size), _Out_writes_opt_(size))
#define __out_bcount_opt(size)                                                 \
  _SAL1_Source_(__out_bcount_opt, (size), _Out_writes_bytes_opt_(size))
...
#define __out_ecount_full_opt(size)                                            \
  _SAL1_Source_(__out_ecount_full_opt, (size),                                 \
                __out_ecount_full(size) __pre_except_maybenull)
#define __out_bcount_full_opt(size)                                            \
  _SAL1_Source_(__out_bcount_full_opt, (size),                                 \
                __out_bcount_full(size) __pre_except_maybenull)
#define __out_ecount_z_opt(size)                                               \
  _SAL1_Source_(__out_ecount_z_opt, (size),                                    \
                __out_ecount_opt(size) __post __nullterminated)
#define __out_bcount_z_opt(size)                                               \
  _SAL1_Source_(__out_bcount_z_opt, (size),                                    \
                __out_bcount_opt(size) __post __nullterminated)
...
#define __out_ecount_nz_opt(size)                                              \
  _SAL1_Source_(__out_ecount_nz_opt, (size),                                   \
                __out_ecount_opt(size) __post __nullterminated)
#define __out_bcount_nz_opt(size)                                              \
  _SAL1_Source_(__out_bcount_nz_opt, (size),                                   \
                __out_bcount_opt(size) __post __nullterminated)
#define __inout_opt _SAL1_Source_(__inout_opt, (), _Inout_opt_)
#define __inout_ecount_opt(size)                                               \
  _SAL1_Source_(__inout_ecount_opt, (size),                                    \
                __inout_ecount(size) __pre_except_maybenull)
#define __inout_bcount_opt(size)                                               \
  _SAL1_Source_(__inout_bcount_opt, (size),                                    \
                __inout_bcount(size) __pre_except_maybenull)
...
#define __inout_ecount_full_opt(size)                                          \
  _SAL1_Source_(__inout_ecount_full_opt, (size),                               \
                __inout_ecount_full(size) __pre_except_maybenull)
#define __inout_bcount_full_opt(size)                                          \
  _SAL1_Source_(__inout_bcount_full_opt, (size),                               \
                __inout_bcount_full(size) __pre_except_maybenull)
#define __inout_z_opt                                                          \
  _SAL1_Source_(__inout_z_opt, (),                                             \
                __inout_opt __pre __nullterminated __post __nullterminated)
...
#define __deref_ecount(size)                                                   \
  _SAL1_Source_(__deref_ecount, (size),                                        \
                _Notref_ __ecount(1) __post _Notref_ __elem_readableTo(1)      \
                    __post _Notref_ __deref _Notref_ __notnull __post __deref  \
                        __elem_writableTo(size))
...
#define __deref_out_ecount_part(size, length)                                  \
  _SAL1_Source_(__deref_out_ecount_part, (size, length),                       \
...
#define __deref_out_ecount_full(size)                                          \
  _SAL1_Source_(__deref_out_ecount_full, (size),                               \
                __deref_out_ecount_part(size, size))
#define __deref_out_bcount_full(size)                                          \
  _SAL1_Source_(__deref_out_bcount_full, (size),                               \
                __deref_out_bcount_part(size, size))
...
#define __deref_inout_ecount_part(size, length)                                \
  _SAL1_Source_(__deref_inout_ecount_part, (size, length),                     \
                __deref_inout_ecount(size)                                     \
                    __pre __deref __elem_readableTo(length)                    \
...
#define __deref_inout_bcount_part(size, length)                                \
  _SAL1_Source_(__deref_inout_bcount_part, (size, length),                     \
                __deref_inout_bcount(size)                                     \
                    __pre __deref __byte_readableTo(length)                    \
                        __post __deref __byte_readableTo(length))
#define __deref_inout_ecount_full(size)                                        \
  _SAL1_Source_(__deref_inout_ecount_full, (size),                             \
                __deref_inout_ecount_part(size, size))
#define __deref_inout_bcount_full(size)                                        \
  _SAL1_Source_(__deref_inout_bcount_full, (size),                             \
                __deref_inout_bcount_part(size, size))
...
#define __deref_ecount_opt(size)                                               \
  _SAL1_Source_(__deref_ecount_opt, (size),                                    \
                __deref_ecount(size) __post_deref_except_maybenull)
#define __deref_bcount_opt(size)                                               \
  _SAL1_Source_(__deref_bcount_opt, (size),                                    \
                __deref_bcount(size) __post_deref_except_maybenull)
...
#define __deref_out_z_opt                                                      \
  _SAL1_Source_(__deref_out_z_opt, (), _Outptr_result_maybenull_z_)
...
#define __deref_out_nz_opt                                                     \
  _SAL1_Source_(__deref_out_nz_opt, (), __deref_out_opt)
...
#define __deref_inout_opt                                                      \
  _SAL1_Source_(__deref_inout_opt, (),                                         \
                __deref_inout __pre_deref_except_maybenull                     \
                    __post_deref_except_maybenull)
...
#define __deref_inout_z_opt                                                    \
  _SAL1_Source_(__deref_inout_z_opt, (),                                       \
                __deref_inout_opt __pre __deref __nullterminated __post        \
                    __deref __nullterminated)
...
#define __deref_inout_ecount_nz_opt(size)                                      \
  _SAL1_Source_(__deref_inout_ecount_nz_opt, (size),                           \
                __deref_inout_ecount_opt(size))
#define __deref_inout_bcount_nz_opt(size)                                      \
  _SAL1_Source_(__deref_inout_bcount_nz_opt, (size),                           \
                __deref_inout_bcount_opt(size))
#define __deref_opt_ecount(size)                                               \
  _SAL1_Source_(__deref_opt_ecount, (size),                                    \
                __deref_ecount(size) __pre_except_maybenull)
#define __deref_opt_bcount(size)                                               \
  _SAL1_Source_(__deref_opt_bcount, (size),                                    \
                __deref_bcount(size) __pre_except_maybenull)
#define __deref_opt_out _SAL1_Source_(__deref_opt_out, (), _Outptr_opt_)
#define __deref_opt_out_z                                                      \
  _SAL1_Source_(__deref_opt_out_z, (), _Outptr_opt_result_z_)
#define __deref_opt_out_ecount(size)                                           \
  _SAL1_Source_(__deref_opt_out_ecount, (size),                                \
                __deref_out_ecount(size) __pre_except_maybenull)
#define __deref_opt_out_bcount(size)                                           \
  _SAL1_Source_(__deref_opt_out_bcount, (size),                                \
                __deref_out_bcount(size) __pre_except_maybenull)
...
#define __deref_opt_inout_z                                                    \
  _SAL1_Source_(__deref_opt_inout_z, (),                                       \
                __deref_opt_inout __pre __deref __nullterminated __post        \
                    __deref __nullterminated)
...
#define __deref_opt_inout_nz                                                   \
  _SAL1_Source_(__deref_opt_inout_nz, (), __deref_opt_inout)
#define __deref_opt_inout_ecount_nz(size)                                      \
  _SAL1_Source_(__deref_opt_inout_ecount_nz, (size),                           \
                __deref_opt_inout_ecount(size))
#define __deref_opt_inout_bcount_nz(size)                                      \
  _SAL1_Source_(__deref_opt_inout_bcount_nz, (size),                           \
                __deref_opt_inout_bcount(size))
#define __deref_opt_ecount_opt(size)                                           \
  _SAL1_Source_(__deref_opt_ecount_opt, (size),                                \
                __deref_ecount_opt(size) __pre_except_maybenull)
#define __deref_opt_bcount_opt(size)                                           \
  _SAL1_Source_(__deref_opt_bcount_opt, (size),                                \
                __deref_bcount_opt(size) __pre_except_maybenull)
#define __deref_opt_out_opt                                                    \
  _SAL1_Source_(__deref_opt_out_opt, (), _Outptr_opt_result_maybenull_)
...
#define __deref_opt_out_z_opt                                                  \
  _SAL1_Source_(__deref_opt_out_z_opt, (),                                     \
                __post __deref __valid __refparam __pre_except_maybenull       \
                    __pre_deref_except_maybenull __post_deref_except_maybenull \
                        __post __deref __nullterminated)
...
#define __deref_opt_out_ecount_nz_opt(size)                                    \
  _SAL1_Source_(__deref_opt_out_ecount_nz_opt, (size),                         \
                __deref_opt_out_ecount_opt(size))
#define __deref_opt_out_bcount_nz_opt(size)                                    \
  _SAL1_Source_(__deref_opt_out_bcount_nz_opt, (size),                         \
                __deref_opt_out_bcount_opt(size))
#define __deref_opt_inout_opt                                                  \
  _SAL1_Source_(__deref_opt_inout_opt, (),                                     \
                __deref_inout_opt __pre_except_maybenull)
...
#define __deref_opt_inout_z_opt                                                \
  _SAL1_Source_(__deref_opt_inout_z_opt, (),                                   \
                __deref_opt_inout_opt __pre __deref __nullterminated __post    \
                    __deref __nullterminated)
...
#define __deref_opt_inout_nz_opt                                               \
  _SAL1_Source_(__deref_opt_inout_nz_opt, (), __deref_opt_inout_opt)
#define __deref_opt_inout_ecount_nz_opt(size)                                  \
  _SAL1_Source_(__deref_opt_inout_ecount_nz_opt, (size),                       \
                __deref_opt_inout_ecount_opt(size))
#define __deref_opt_inout_bcount_nz_opt(size)                                  \
  _SAL1_Source_(__deref_opt_inout_bcount_nz_opt, (size),                       \
                __deref_opt_inout_bcount_opt(size))
...
#define __nullnullterminated                                                   \
  _SAL1_Source_(__nullnulltermiated, (), _SAL_nop_impl_)
#define __reserved _SAL1_Source_(__reserved, (), _Reserved_)
#define __checkReturn _SAL1_Source_(__checkReturn, (), _Check_return_)
#define __typefix(ctype)                                                       \
  _SAL1_Source_(__typefix, (ctype), __inner_typefix(ctype))
#define __override __inner_override
#define __callback __inner_callback
#define __format_string                                                        \
  _SAL1_1_Source_(__format_string, (), _Printf_format_string_)
#define __blocksOn(resource)                                                   \
  _SAL_L_Source_(__blocksOn, (resource), __inner_blocksOn(resource))
#define __control_entrypoint(category)                                         \
  _SAL_L_Source_(__control_entrypoint, (category),                             \
                 __inner_control_entrypoint(category))
#define __data_entrypoint(category)                                            \
  _SAL_L_Source_(__data_entrypoint, (category),                                \
...
#else  // ][
#define __on_failure(annotes)                                                  \
  _SAL1_1_Source_(__on_failure, (annotes),                                     \
                  _On_failure_impl_(annotes _SAL_nop_impl_))
#endif // ]
...
#define _Analysis_noreturn_                                                    \
  _SAL2_Source_(_Analysis_noreturn_, (), _SA_annotes0(SAL_terminates))
...
#define _Analysis_mode_impl_(mode)                                             \
  _SAL2_Source_(_Analysis_mode_impl_, (mode),                                  \
                _SA_annotes1(SAL_analysisMode, #mode))
...
#define _Analysis_mode_(mode)                                                  \
  __pragma(warning(                                                            \
      disable                                                                  \
      : 28110 28111 28161 28162)) typedef _Analysis_mode_impl_(mode) int       \
...
#define _Function_class_(x)                                                    \
  _SAL2_Source_(_Function_class_, (x),                                         \
                _SA_annotes1(SAL_functionClassNew, _SA_SPECSTRIZE(x)))

#define _Enum_is_bitflag_                                                      \
  _SAL2_Source_(_Enum_is_bitflag_, (), _SA_annotes0(SAL_enumIsBitflag))
#define _Strict_type_match_                                                    \
  _SAL2_Source_(_Strict_type_match, (), _SA_annotes0(SAL_strictType2))

#define _Maybe_raises_SEH_exception_                                           \
  _SAL2_Source_(_Maybe_raises_SEH_exception_, (x),                             \
                _Pre_ _SA_annotes1(SAL_inTry, __yes))
#define _Raises_SEH_exception_                                                 \
  _SAL2_Source_(_Raises_SEH_exception_, (x),                                   \
                _Maybe_raises_SEH_exception_ _Analysis_noreturn_)
