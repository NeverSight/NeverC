 ...
 * agents.h
 *
 * Main public header file for ConcRT's asynchronous agents layer. This is the
 * only header file a C++ program must include to use asynchronous agents.
 *
 * The core runtime, Parallel Patterns Library (PPL), and resource manager are
 * defined in separate header files.
 * =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 ****/
...
#pragma warning(                                                               \
    disable : 4702) // Unreachable code - needed for retail version code path
...
///     provide access to the Concurrency Runtime, a concurrent programming
...
/// <summary>
///     A lock holder that acquires a non-reentrant lock on instantiation and
...
/// <summary>
///     A lock holder that acquires a reentrant lock on instantiation and
///     releases it on destruction
/// </summary>
/**/
...
// Internal namespace:
//
// ::Concurrency::details contains definitions to support routines in the public
// namespaces and macros. Clients should not directly interact with this
// namespace.
//***************************************************************************
...
template <class _Message> class _Queue : public _AllocBase {
protected:
...
public:
  ...
  size_t _Count() const { return _M_count; }
  ...
  //
  // Returns a Boolean indicating whether the operation succeeded, that is, the
  // message was found in the queue.
  ...
  //
  // Returns a pointer to the message found at the head of the queue.
  _Message *_Peek() { return _M_pHead; }
...
// thread-safe.
//
template <class _Type> class _Dynamic_array {
public:
  ...
  //
  // Index operation. Retrieve an element at the specified index. No bounds
  // check is done.
  //
  ...
  //
  size_t _Size() const { return _M_index; }
...
//
template <class _Type> __int64 _Trace_agents_get_id(_Type *_PObject) {
...
public:
  ...
  _Network_link_iterator(_MyContainer *_PNetwork_link, size_t _Index)
      : _M_pNetwork_link(_PNetwork_link), _M_index(_Index), _M_value(nullptr) {
...
/// <summary>
///     The <c>network_link_registry</c> abstract base class manages the links
...
public:
  /// <summary>
  ///     A type that represents the block type stored in the
  ...
  /// <summary>
  ///     A type that represents an element pointer stored in the
  ///     <c>network_link_registry</c> object.
  /// </summary>
  /**/
  typedef _Block *_EType;
  ...
  ///     <c>network_link_registry</c> object for reading and performing const
  ///     operations.
  /// </summary>
  /**/
  typedef _EType const &const_reference;
  ...
  /// <summary>
  ///     A type that provides an iterator that can read or modify any element
  ///     in a <c>network_link_registry</c> object.
  /// </summary>
  /**/
  typedef _Network_link_iterator<_Block> iterator;

  /// <summary>
  ///     When overridden in a derived class, adds a link to the
  ///     <c>network_link_registry</c> object.
  /// </summary>
  /// <param name="_Link">
  ...
  /// <summary>
  ///     When overridden in a derived class, searches the
  ///     <c>network_link_registry</c> object for a specified block.
  /// </summary>
  /// <param name="_Link">
  ///     A pointer to a block that is being searched for in the
  ///     <c>network_link_registry</c> object.
  /// </param>
  /// <returns>
  ...
  /// <summary>
  ///     When overridden in a derived class, returns an iterator to the first
  ///     element in the <c>network_link_registry</c> object.
  /// </summary>
  /// <remarks>
  ///     The end state of the iterator is indicated by a <c>NULL</c> link.
  /// </remarks>
  /// <returns>
  ///     An iterator addressing the first element in the
  ///     <c>network_link_registry</c> object.
  /// </returns>
  /**/
...
protected:
  ...
  /// <summary>
  ///     Retrieves the element at the given index. If the index is out of
  ///     bounds, <c>NULL</c> is returned. Users need to use the iterator to
  ///     access the links.
  /// </summary>
  /// <param name="_Index">
  ...
  /// </param>
  /// <returns>
  ///     The element in the registry at  the index specified by the <paramref
  ///     name="_Index"/> parameter.
  /// </returns>
  /**/
...
/// <summary>
///     The <c>single_link_registry</c> object is a <c>network_link_registry</c>
///     that manages only a single source or target block.
/// </summary>
/// <typeparam name="_Block">
///     The block data type being stored in the <c>single_link_registry</c>
///     object.
/// </typeparam>
/// <seealso cref="multi_link_registry Class"/>
/**/
...
public:
  ...
  /// </summary>
  /// <remarks>
  ///     The method throws an <see cref="invalid_operation
  ///     Class">invalid_operation</see> exception if it is called before the
  ///     link is removed.
  /// </remarks>
  /**/
  ...
  /// </param>
  /// <remarks>
  ///     The method throws an <see cref="invalid_link_target
  ///     Class">invalid_link_target</see> exception if there is already a link
  ///     in this registry.
  /// </remarks>
  /**/
  ...
  /// </summary>
  /// <param name="_Link">
  ///     A pointer to a block that is to be searched for in the
  ///     <c>single_link_registry</c> object.
  /// </param>
  /// <returns>
  ...
  /// </returns>
  /**/
  virtual size_t count() { return (_M_connectedLink == nullptr) ? 0 : 1; }
...
protected:
  ...
  /// </returns>
  /**/
  virtual typename network_link_registry<_Block>::_EType
  _Get_element(size_t _Index) const {
    if (_Index == 0) {
...
/// <summary>
///     The <c>multi_link_registry</c> object is a <c>network_link_registry</c>
///     that manages multiple source blocks or multiple target blocks.
/// </summary>
/// <typeparam name="_Block">
///     The block data type being stored in the <c>multi_link_registry</c>
///     object.
/// </typeparam>
/// <seealso cref="single_link_registry Class"/>
/**/
...
public:
  ...
  /// <remarks>
  ///     The method throws an <see cref="invalid_operation
  ///     Class">invalid_operation</see> exception if called before all links
  ///     are removed.
  /// </remarks>
  /**/
  ...
  /// <summary>
  ///     Sets an upper bound on the number of links that the
  ///     <c>multi_link_registry</c> object can hold.
  /// </summary>
  /// <param name="_MaxLinks">
  ///     The maximum number of links that the <c>multi_link_registry</c> object
  ///     can hold.
  /// </param>
  /// <remarks>
  ///     After a bound is set, unlinking an entry will cause the
  ///     <c>multi_link_registry</c> object to enter an immutable state where
  ///     further calls to <c>add</c> will throw an <c>invalid_link_target</c>
  ///     exception.
  /// </remarks>
  /**/
  ...
  /// <remarks>
  ///     The method throws an <see cref="invalid_link_target
  ///     Class">invalid_link_target</see> exception if the link is already
  ///     present in the registry, or if a bound has already been set with the
  ///     <c>set_bound</c> function and a link has since been removed.
  /// </remarks>
  /**/
  virtual void add(typename network_link_registry<_Block>::_EType _Link) {
    if (_Link == nullptr) {
  ...
  /// <param name="_Link">
  ///     A pointer to a block that is to be searched for in the
  ///     <c>multi_link_registry</c> object.
  /// </param>
  /// <returns>
  ...
  /// </returns>
  /**/
  virtual size_t count() { return _Count(); }
...
private:
  ...
  /// </param>
  /// <returns>
  ///     <c>true</c> if the specified link was found and removed, <c>false</c>
  ///     otherwise.
  /// </returns>
  /**/
  ...
  ::Concurrency::details::_Dynamic_array<
      typename network_link_registry<_Block>::_EType>
      _M_vector;
};
...
/// </typeparam>
/**/
template <class _LinkRegistry> class _Source_link_iterator {
public:
  ...
  _Source_link_iterator(_MyContainer *_PNetwork_link, size_t _Index)
      : _M_pNetwork_link(_PNetwork_link), _M_index(_Index),
        _M_sentinel(nullptr) {
  ...
  /// </returns>
  /**/
  const_reference operator*() { return _Get(0); }
  ...
  /// </summary>
  /**/
  const_reference operator[](size_t _Pos) const { return _Get(_Pos); }

  /// <summary>
  ///     Pre-increment the iterator to point to the next element
  /// </summary>
  /// <returns>
  ///     Reference to the object pointer to by the iterator after incrementing
  ///     it
  /// </returns>
  /**/
  _Myt &operator++() {
  ...
  /// </summary>
  /// <returns>
  ///     Reference to the object pointer to by the iterator before incrementing
  ///     it
  /// </returns>
  /**/
  _Myt operator++(int) {
...
/// <summary>
///     The <c>source_link_manager</c> object manages messaging block network
///     links to <c>ISource</c> blocks.
/// </summary>
/// <typeparam name="_LinkRegistry">
///     The network link registry.
/// </typeparam>
/// <remarks>
///     Currently, the source blocks are reference counted. This is a wrapper on
///     a <c>network_link_registry</c> object that allows concurrent access to
///     the links and provides the ability to reference the links through
///     callbacks. Message blocks (<c>target_block</c>s or
///     <c>propagator_block</c>s) should use this class for their source links.
/// </remarks>
/// <seealso cref="single_link_registry Class"/>
/// <seealso cref="multi_link_registry Class"/>
/**/
...
public:
  /// <summary>
  ///     The type of link registry being managed by the
  ...
  /// <summary>
  ///     The type of the blocks being managed by the <c>source_link_manager</c>
  ///     object.
  /// </summary>
  /**/
  typedef typename _LinkRegistry::type _Block;
  ...
  /// <summary>
  ///     The method signature for a callback method for this
  ///     <c>source_link_manager</c> object.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     A type that represents a pointer to an element stored in the
  ///     <c>source_link_manager</c> object.
  /// </summary>
  /**/
  typedef _Block *_EType;
  ...
  /// <summary>
  ///     A type that provides a reference to a <c>const</c> element stored in a
  ///     <c>source_link_manager</c> object for reading and performing const
  ///     operations.
  /// </summary>
  /**/
  typedef _EType const &const_reference;
  ...
  /// <summary>
  ///     A type that provides an iterator that can read or modify any element
  ///     in the <c>source_link_manager</c> object.
  /// </summary>
  /**/
  typedef _Source_link_iterator<_LinkRegistry> iterator;

  /// <summary>
  ///     A type that provides a reentrant lock for the
  ///     <c>source_link_manager</c> object.
  /// </summary>
  /**/
  ...
  /// </summary>
  /**/
  ~source_link_manager() { _CONCRT_ASSERT(_M_pendingRemove._Size() == 0); }

  /// <summary>
  ///     Registers the target block that holds this <c>source_link_manager</c>
  ///     object.
  /// </summary>
  /// <param name="_PTarget">
  ...
  /// </param>
  /**/
  void set_bound(size_t _MaxLinks) { _M_links.set_bound(_MaxLinks); }
  ...
  bool remove(_EType _Link) {
    ...
    // NOTE: touching "this" pointer is dangerous as soon as the above lock is
    // released
  ...
  /// <summary>
  ///     Searches the <c>network_link_registry</c> within this
  ///     <c>source_link_manager</c> object for a specified block.
  /// </summary>
  /// <param name="_Link">
  ///     A pointer to a block that is to be searched for in the
  ///     <c>source_link_manager</c> object.
  ...
  /// <summary>
  ///     Counts the number of linked blocks in the <c>source_link_manager</c>
  ///     object.
  /// </summary>
  /// <returns>
...
enum message_status {
  ...
  postponed,
  /// <summary>
  ///     The target tried to accept the message, but it was no longer
  ///     available.
  /// </summary>
  /**/
  missed
...
/// <summary>
///     The basic message envelope containing the data payload being passed
///     between messaging blocks.
/// </summary>
...
public:
  ...
  /// </param>
  /// <remarks>
  ///     The constructor that takes a pointer to a <c>message</c> object as an
  ///     argument throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if the parameter <paramref
  ///     name="_Msg"/> is <c>NULL</c>.
  /// </remarks>
  /**/
  ...
  message(_Type const &_P, runtime_object_identity _Id)
      : ::Concurrency::details::_Runtime_object(_Id), payload(_P),
        _M_pNext(nullptr), _M_refCount(0) {}
  ...
  message(message const &_Msg)
      : payload(_Msg.payload), _M_pNext(nullptr), _M_refCount(0) {}
  ...
  /// </param>
  /// <remarks>
  ///     This method throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if the parameter <paramref
  ///     name="_Msg"/> is <c>NULL</c>.
  /// </remarks>
  /**/
  message(_In_ message const *_Msg)
      : payload((_Msg == nullptr) ? nullptr : _Msg->payload), _M_pNext(nullptr),
        _M_refCount(0) {
    if (_Msg == nullptr) {
  ...
  /// </returns>
  /**/
  runtime_object_identity msg_id() const { return _M_id; }
  ...
  /// <summary>
  ///     Adds to the reference count for the <c>message</c> object.  Used for
  ///     message blocks that need reference counting to determine message
  ///     lifetimes.
  /// </summary>
  /// <returns>
  ...
  /// <summary>
  ///     Subtracts from the reference count for the <c>message</c> object. Used
  ///     for message blocks that need reference counting to determine message
  ///     lifetimes.
  /// </summary>
  /// <returns>
  ///     The new value of the reference count.
  /// </returns>
  /**/
  long remove_ref() { return _InterlockedDecrement(&_M_refCount); }
...
/// <summary>
///     The <c>message_processor</c> class is the abstract base class for
///     processing of <c>message</c> objects.  There is no guarantee on the
///     ordering of the messages.
/// </summary>
/// <typeparam name="_Type">
///     The data type of the payload within messages handled by this
///     <c>message_processor</c> object.
/// </typeparam>
/// <seealso cref="ordered_message_processor Class"/>
/**/
template <class _Type> class message_processor {
public:
  /// <summary>
  ///     A type alias for <typeparamref name="_Type"/>.
  /// </summary>
  ...
  /// <summary>
  ///     When overridden in a derived class, places messages into the block
  ///     asynchronously.
  /// </summary>
  /// <param name="_Msg">
  ...
  /// <summary>
  ///     When overridden in a derived class, waits for all asynchronous
...
protected:
  ...
  ///     messages into the block. Called once every time a new message is added
  ///     and the queue is found to be empty.
  /// </summary>
  /// <remarks>
  ...
  /// <summary>
  ///     Wrapper for <c>process_incoming_message</c> suitable for use as a
  ///     argument to <c>CreateThread</c> and other similar methods.
  /// </summary>
  /// <param name="_Data">
  ...
  static void __cdecl _Process_incoming_message_wrapper(void *_Data) {
    message_processor<_Type> *_PMessageProcessor =
        (message_processor<_Type> *)_Data;
    _PMessageProcessor->process_incoming_message();
  }
};

/// <summary>
///     An <c>ordered_message_processor</c> is a <c>message_processor</c> that
///     allows message blocks to process messages in the order they were
///     received.
/// </summary>
/// <typeparam name="_Type">
...
public:
  /// <summary>
  ///     The signature of the callback method invoked while processing
  ///     messages.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     The signature of the callback method invoked while propagating
  ///     messages.
  /// </summary>
  /**/
  ...
  /// </summary>
  /// <remarks>
  ///     This <c>ordered_message_processor</c> will not schedule asynchronous
  ///     or synchronous handlers until the <c>initialize</c> function is
  ///     called.
  /// </remarks>
  /**/
  ordered_message_processor()
      : _M_queuedDataCount(0), _M_pScheduler(nullptr),
        _M_pScheduleGroup(nullptr), _M_stopProcessing(1), _M_lwtCount(0),
        _M_handler(nullptr), _M_processor(nullptr), _M_propagator(nullptr) {}
  ...
  /// </remarks>
  /**/
  virtual ~ordered_message_processor() { wait(); }

  /// <summary>
  ///     Initializes the <c>ordered_message_processor</c> object with the
  ///     appropriate callback function, scheduler and schedule group.
  /// </summary>
  /// <param name="_PScheduler">
  ///     A pointer to the scheduler to be used for scheduling light-weight
  ///     tasks.
  /// </param>
  /// <param name="_PScheduleGroup">
  ///     A pointer to the schedule group to be used for scheduling light-weight
  ///     tasks.
  /// </param>
  /// <param name="_Handler">
  ...
  /**/
  void initialize(_Inout_opt_ Scheduler *_PScheduler,
  ...
  /// </param>
  virtual void
  initialize_batched_processing(_Handler_method const &_Processor,
  ...
  /// <summary>
  ///     Synchronously queues up messages and starts a processing task, if this
  ...
  /// <summary>
  ///     Asynchronously queues up messages and starts a processing task, if
  ...
  virtual void async_send(_Inout_opt_ message<_Type> *_Msg) {
    ...
    if (_InterlockedIncrement(&_M_queuedDataCount) == 1) {
      ...
      if (_M_stopProcessing == 0) {
        ...
        _Trace_agents(AGENTS_EVENT_SCHEDULE,
                      ::Concurrency::details::_Trace_agents_get_id(this));

        TaskProc _Proc = &::Concurrency::ordered_message_processor<
            _Type>::_Process_incoming_message_wrapper;
      ...
      // If we get here then no task was scheduled. Decrement LWT count to
      // reflect this fact
  ...
  /// <summary>
  ///     A processor-specific spin wait used in destructors of message blocks
  ///     to make sure that all asynchronous processing tasks have time to
  ///     finish before destroying the block.
  /// </summary>
  /**/
  virtual void wait() {
...
protected:
  /// <summary>
  ///     The processing function that is called asynchronously.  It dequeues
  ///     messages and begins processing them.
  /// </summary>
  /**/
  virtual void process_incoming_message() {
    _Trace_agents(AGENTS_EVENT_START,
                  ::Concurrency::details::_Trace_agents_get_id(this));
    long _Count = _Process_message_helper();
    _Trace_agents(AGENTS_EVENT_END,
                  ::Concurrency::details::_Trace_agents_get_id(this), _Count);
...
private:
  ...
  void _Sync_send_helper(message<_Type> *_Msg) {
    ...
    if (_M_stopProcessing == 0) {
      if (_M_queuedDataCount > 0) {
        long _Count =
            _InterlockedExchange((volatile long *)&_M_queuedDataCount, 0);
  ...
  long _Process_message_helper() {
    ...
    // Read off the number of messages to process in this iteration by snapping
    // a count
    ...
    // send occurring.  One of them could have sent all of the messages for the
    // other
    while (_Count > 0) {
      ...
      volatile long _Orig = _InterlockedExchangeAdd(
          (volatile long *)&_M_queuedDataCount, -_Count);
      _CONCRT_ASSERT(_Orig >= _Count);
      if (_Orig == _Count) {
        // Because _Count did not change, we processed everything there is to
        // process
        break;
      }
  ...
  void _Invoke_handler(long _Count) {
    ...
    for (int _I = 0; _I < _Count; _I++) {
      ...
      if (_M_processor == nullptr) {
        // If a processor function does not exist, the message processor is
        // using single message processing rather than batched processing. There
        // should also be no propagator function defined in this case.
  ...
  void _Invoke_handler(message<_Type> *_Msg) {
    if (_M_processor == nullptr) {
      // If a processor function does not exist, the message processor is using
      // single message processing rather than batched processing.  There should
      // also be no propagator function defined in this case.
      _CONCRT_ASSERT(_M_propagator == nullptr);
      _M_handler(_Msg);
    } else {
      // Use the batched message processing function
...
private:
  ...
  /// <summary>
  ///     A flag set in the destructor of a block to cease processing of new
  ///     messages. This is required to guarantee that _M_queuedDataCount will
  ///     get to 0 eventually.
  /// </summary>
  /**/
...
/// <summary>
///     The <c>ITarget</c> class is the interface for all target blocks.  Target
///     blocks consume messages offered to them by <c>ISource</c> blocks.
/// </summary>
/// <typeparam name="_Type">
///     The data type of the payload within the messages accepted by the target
///     block.
/// </typeparam>
/// <remarks>
///     For more information, see <see cref="Asynchronous Message Blocks"/>.
...
public:
  ...
  // internal message structure that is used by Consume and the LWT.  Doing so
  // could result in a deadlock with the Consume call.

  /// <summary>
  ///     When overridden in a derived class, asynchronously passes a message
  ///     from a source block to this target block.
  /// </summary>
  ...
  /// </param>
  /// <returns>
  ///     A <see cref="message_status Enumeration">message_status</see>
  ///     indication of what the target decided to do with the message.
  /// </returns>
  /// <remarks>
  ///     The method throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if either the <paramref
  ///     name="_PMessage"/> or <paramref name="_PSource"/> parameter is
  ///     <c>NULL</c>.
  /// </remarks>
  /**/
  virtual message_status propagate(_Inout_opt_ message<_Type> *_PMessage,
                                   _Inout_opt_ ISource<_Type> *_PSource) = 0;

  /// <summary>
  ///     When overridden in a derived class, synchronously passes a message to
  ///     the target block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  ///     Class">invalid_argument</see> exception if either the <paramref
  ///     name="_PMessage"/> or <paramref name="_PSource"/> parameter is
  ///     <c>NULL</c>. <para>Using the <c>send</c> method outside of message
  ///     initiation and to propagate messages within a network is dangerous and
  ///     can lead to deadlock.</para> <para>When <c>send</c> returns, the
  ///     message has either already been accepted, and transferred into the
  ///     target block, or it has been declined by the target.</para>
  /// </remarks>
  /**/
  virtual message_status send(_Inout_ message<_Type> *_PMessage,
                              _Inout_ ISource<_Type> *_PSource) = 0;

  /// <summary>
  ///     When overridden in a derived class, returns true or false depending on
  ///     whether the message block accepts messages offered by a source that is
  ///     not linked to it. If the overridden method returns <c>true</c>, the
  ///     target cannot postpone an offered message, as consumption of a
  ///     postponed message at a later time requires the source to be identified
  ///     in its source link registry.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the block can accept message from a source that is not
  ///     linked to it <c>false</c> otherwise.
  /// </returns>
  /**/
  virtual bool supports_anonymous_source() { return false; }
  ...
  /// <summary>
  ///     The signature of any method used by the block that returns a
  ///     <c>bool</c> value to determine whether an offered message should be
  ///     accepted.
  /// </summary>
  /**/
...
protected:
  /// <summary>
  ///     When overridden in a derived class, links a specified source block to
  ///     this <c>ITarget</c> block.
  /// </summary>
  /// <param name="_PSource">
  ///     The <c>ISource</c> block being linked to this <c>ITarget</c> block.
  /// </param>
  /// <remarks>
  ///     This function should not be called directly on an <c>ITarget</c>
  ///     block. Blocks should be connected together using the
  ///     <c>link_target</c> method on <c>ISource</c> blocks, which will invoke
  ///     the <c>link_source</c> method on the corresponding target.
  /// </remarks>
  /**/
  virtual void link_source(_Inout_ ISource<_Type> *_PSource) = 0;

  /// <summary>
  ///     When overridden in a derived class, unlinks a specified source block
  ///     from this <c>ITarget</c> block.
  /// </summary>
  /// <param name="_PSource">
  ///     The <c>ISource</c> block being unlinked from this <c>ITarget</c>
  ///     block.
  /// </param>
  /// <remarks>
  ///     This function should not be called directly on an <c>ITarget</c>
  ///     block. Blocks should be disconnected using the <c>unlink_target</c> or
  ///     <c>unlink_targets</c> methods on <c>ISource</c> blocks, which will
  ///     invoke the <c>unlink_source</c> method on the corresponding target.
  /// </remarks>
  /**/
  virtual void unlink_source(_Inout_ ISource<_Type> *_PSource) = 0;

  /// <summary>
  ///     When overridden in a derived class, unlinks all source blocks from
  ///     this <c>ITarget</c> block.
  /// </summary>
  /**/
...
/// <summary>
///     The <c>ISource</c> class is the interface for all source blocks.  Source
///     blocks propagate messages to <c>ITarget</c> blocks.
/// </summary>
/// <typeparam name="_Type">
///     The data type of the payload within the messages produced by the source
///     block.
/// </typeparam>
/// <remarks>
///     For more information, see <see cref="Asynchronous Message Blocks"/>.
...
public:
  ...
  /// <summary>
  ///     When overridden in a derived class, links a target block to this
  ///     <c>ISource</c> block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block being linked to this <c>ISource</c>
  ///     block.
  /// </param>
  /**/
  ...
  /// <summary>
  ///     When overridden in a derived class, unlinks a target block from this
  ///     <c>ISource</c> block, if found to be previously linked.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block being unlinked from this <c>ISource</c>
  ///     block.
  /// </param>
  /**/
  ...
  /// <summary>
  ///     When overridden in a derived class, unlinks all target blocks from
  ///     this <c>ISource</c> block.
  /// </summary>
  /**/
  virtual void unlink_targets() = 0;

  /// <summary>
  ///     When overridden in a derived class, accepts a message that was offered
  ///     by this <c>ISource</c> block, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ///     object.
  /// </param>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling the <c>accept</c>
  ///     method.
  /// </param>
  /// <returns>
  ...
  /// </returns>
  /// <remarks>
  ///     The <c>accept</c> method is called by a target while a message is
  ///     being offered by this <c>ISource</c> block. The message pointer
  ///     returned may be different from the one passed into the
  ///     <c>propagate</c> method of the <c>ITarget</c> block, if this source
  ///     decides to make a copy of the message.
  /// </remarks>
  /**/
  virtual message<_Type> *accept(runtime_object_identity _MsgId,
                                 _Inout_ ITarget<_Type> *_PTarget) = 0;

  /// <summary>
  ///     When overridden in a derived class, reserves a message previously
  ///     offered by this <c>ISource</c> block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ///     object.
  /// </param>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling the <c>reserve</c>
  ///     method.
  /// </param>
  /// <returns>
  ///     <c>true</c> if the message was successfully reserved, <c>false</c>
  ///     otherwise. Reservations can fail for many reasons, including: the
  ///     message was already reserved or accepted by another target, the source
  ///     could deny reservations, and so forth.
  /// </returns>
  /// <remarks>
  ///     After you call <c>reserve</c>, if it succeeds, you must call either
  ///     <c>consume</c> or <c>release</c> in order to take or give up
  ///     possession of the message, respectively.
  /// </remarks>
  /**/
  virtual bool reserve(runtime_object_identity _MsgId,
                       _Inout_ ITarget<_Type> *_PTarget) = 0;

  /// <summary>
  ///     When overridden in a derived class, consumes a message previously
  ///     offered by this <c>ISource</c> block and successfully reserved by the
  ///     target, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the reserved <c>message</c>
  ///     object.
  /// </param>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling the <c>consume</c>
  ///     method.
  /// </param>
  /// <returns>
  ///     A pointer to the <c>message</c> object that the caller now has
  ///     ownership of.
  /// </returns>
  /// <remarks>
  ///     The <c>consume</c> method is similar to <c>accept</c>, but must always
  ///     be preceded by a call to <c>reserve</c> that returned <c>true</c>.
  /// </remarks>
  /**/
  virtual message<_Type> *consume(runtime_object_identity _MsgId,
                                  _Inout_ ITarget<_Type> *_PTarget) = 0;

  /// <summary>
  ///     When overridden in a derived class, releases a previous successful
  ///     message reservation.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the reserved <c>message</c>
  ...
  /// </param>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling the <c>release</c>
  ///     method.
  /// </param>
  /**/
  virtual void release(runtime_object_identity _MsgId,
                       _Inout_ ITarget<_Type> *_PTarget) = 0;

  /// <summary>
  ///     When overridden in a derived class, acquires a reference count on this
  ///     <c>ISource</c> block, to prevent deletion.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling this method.
  /// </param>
  /// <remarks>
  ///     This method is called by an <c>ITarget</c> object that is being linked
  ///     to this source during the <c>link_target</c> method.
  /// </remarks>
  /**/
  virtual void acquire_ref(_Inout_ ITarget<_Type> *_PTarget) = 0;

  /// <summary>
  ///     When overridden in a derived class, releases a reference count on this
  ///     <c>ISource</c> block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling this method.
  /// </param>
  /// <remarks>
  ///     This method is called by an <c>ITarget</c> object that is being
  ///     unlinked from this source. The source block is allowed to release any
  ///     resources reserved for the target block.
  /// </remarks>
  /**/
...
protected:
  ...
  /// </param>
  /// <remarks>
  ///     This function definition is required because ISource blocks need to
  ///     call Target->link_source(), which is a private member of ITarget.
  ///     ISource is declared as a friend class, so this is a way for derived
  ///     classes of ISource to properly link/unlink their targets during
  ///     link_target(), unlink_target() and unlink_targets()
  /// </remarks>
  /**/
  ...
  /// <remarks>
  ///     This function definition is required because ISource blocks need to
  ///     call Target->unlink_source(), which is a private member of ITarget.
  ///     ISource is declared as a friend class, so this is a way for derived
  ///     classes of ISource to properly link/unlink their targets during
  ///     link_target(), unlink_target() and unlink_targets()
  /// </remarks>
...
/// <summary>
///     A general receive implementation, allowing a context to wait for data
///     from exactly one source and filter the values that are accepted.  If the
///     specified timeout is not COOPERATIVE_TIMEOUT_INFINITE, an exception
///     (operation_timed_out) will be thrown if the specified amount of time
///     expires before a message is received.  Note that zero length timeouts
///     should likely use try_receive as opposed to receive with a timeout of
...
/// </param>
/// <param name="_Timeout">
///     The maximum time for which the method should for the data, in
///     milliseconds.
/// </param>
/// <param name="_Filter_proc">
///     A pointer to a filter which will indicate whether to accept the data or
///     not.
/// </param>
/// <returns>
...
template <class _Type>
_Type _Receive_impl(
    ISource<_Type> *_Src, unsigned int _Timeout,
    typename ITarget<_Type>::filter_method const *_Filter_proc) {
  // The Blocking Recipient messaging block class is internal to the receive
  // function
  ...
  public:
    ...
    _Blocking_recipient(ISource<_Type> *_PSource,
        ...
        : _M_fState(_NotInitialized), _M_pConnectedTo(nullptr),
          _M_pMessage(nullptr), _M_timeout(_Timeout), _M_pFilter(nullptr) {
      _Connect(_PSource);
    }
    ...
    // Cleans up any resources that may have been created by the
    // BlockingRecipient.
    ~_Blocking_recipient() {
      _Disconnect();
    ...
    // block, generally within an asynchronous task to send messages to its
    // targets.
    virtual message_status propagate(message<_Type> *_PMessage,
    ...
    // Synchronously sends a message to this block.  When this function
    // completes the message will already have propagated into the block.
    virtual message_status send(message<_Type> *_PMessage,
                                ISource<_Type> *_PSource) {
      if (_PMessage == nullptr) {
        throw ::std::invalid_argument("_PMessage");
      }
  ...
  private:
    ...
    virtual void unlink_source(ISource<_Type> *_PSource) {
      if (_InterlockedCompareExchangePointer(
              reinterpret_cast<void *volatile *>(&_M_pConnectedTo),
              (void *)nullptr, _PSource) == _PSource) {
    ...
    virtual void unlink_sources() {
      ISource<_Type> *_PSource =
          reinterpret_cast<ISource<_Type> *>(_InterlockedExchangePointer(
              reinterpret_cast<void *volatile *>(&_M_pConnectedTo),
              (void *)nullptr));
      if (_PSource != nullptr) {
    ...
    // Cleanup the connection to the blocking recipient's source. There is no
    // need to do anything about the associated context.
    void _Disconnect() { unlink_sources(); }
    ...
    void _Wait_for_message() {
      ...
      if (_InterlockedCompareExchange(&_M_fState, _Blocked, _NotInitialized) ==
          _NotInitialized) {
      ...
      // Unlinking from our source guarantees that there are no threads in
      // propagate
      _Disconnect();
    ...
    enum { _NotInitialized, _Blocked, _Initialized };
...
/// <summary>
///     A general receive implementation, allowing a context to wait for data
///     from exactly one source and filter the values that are accepted.
/// </summary>
/// <typeparam name="_Type">
///     The payload type.
/// </typeparam>
...
/// </returns>
/// <remarks>
///     If the parameter <paramref name="_Timeout"/> has a value other than the
///     constant <c>COOPERATIVE_TIMEOUT_INFINITE</c>, the exception <see
///     cref="operation_timed_out Class">operation_timed_out</see> is thrown if
///     the specified amount of time expires before a message is received.  If
///     you want a zero length timeout, you should use the <see
///     cref="try_receive Function">try_receive</see> function, as opposed to
///     calling <c>receive</c> with a timeout of <c>0</c> (zero), as it is more
///     efficient and does not throw exceptions on timeouts. <para>For more
///     information, see <see cref="Message Passing Functions"/>.</para>
/// </remarks>
...
/**/
template <class _Type>
_Type receive(_Inout_ ISource<_Type> *_Src,
              unsigned int _Timeout = COOPERATIVE_TIMEOUT_INFINITE) {
...
/**/
template <class _Type>
_Type receive(ISource<_Type> &_Src,
              unsigned int _Timeout = COOPERATIVE_TIMEOUT_INFINITE) {
  return _Receive_impl(&_Src, _Timeout, nullptr);
}
...
///     A general try-receive implementation, allowing a context to look for
///     data from exactly one source and filter the values that are accepted. If
///     the data is not ready, try_receive will return false.
/// </summary>
/// <typeparam name="_Type">
///     The payload type
/// </typeparam>
...
/// </param>
/// <returns>
///     A bool indicating whether a payload was placed in <paramref
///     name="_value"/> or not.
/// </returns>
/**/
template <class _Type>
bool _Try_receive_impl(
    ISource<_Type> *_Src, _Type &_value,
    typename ITarget<_Type>::filter_method const *_Filter_proc) {
  // The Immediate Recipient messaging block class is internal to the receive
  // function
  class _Immediate_recipient : public ITarget<_Type> {
  public:
    // Create an Immediate Recipient
    _Immediate_recipient(ISource<_Type> *_PSource)
        : _M_pConnectedTo(nullptr), _M_pMessage(nullptr), _M_isInitialized(0),
          _M_pFilter(nullptr) {
      _Connect(_PSource);
    }
...
///     A general try-receive implementation, allowing a context to look for
///     data from exactly one source and filter the values that are accepted. If
///     the data is not ready, the method will return false.
/// </summary>
/// <typeparam name="_Type">
///     The payload type.
/// </typeparam>
...
/// </param>
/// <returns>
///     A <c>bool</c> value indicating whether or not a payload was placed in
...
/// <seealso cref="asend Function"/>
/**/
template <class _Type> bool try_receive(ISource<_Type> &_Src, _Type &_value) {
  return _Try_receive_impl(&_Src, _value, nullptr);
}
...
template <class _Type> class _AnonymousOriginator : public ISource<_Type> {
public:
  ...
  virtual ~_AnonymousOriginator() { delete _M_pMessage; }
  ...
  virtual message<_Type> *accept(runtime_object_identity _MsgId,
    ...
    // The ownership of this message has changed.  Set the internal pointer to
    // NULL so it won't be deleted in the destructor
    _M_pMessage = nullptr;
  ...
  // Release needs to be defined for ISource blocks, but Originator doesn't need
  // to do anything for reservation release because there can only be one target
  // block to read the data at a later time.
...
template <class _Type> class _SyncOriginator : public ISource<_Type> {
public:
  typedef single_link_registry<ITarget<_Type>> _Target_registry;

  // Create an Originator
  _SyncOriginator()
      : _M_pMessage(nullptr), _M_fStatus(postponed), _M_referenceCount(0) {}
  ...
  virtual void unlink_targets() {
    ...
    for (typename _Target_registry::iterator _Iter =
             _M_connectedTargets.begin();
         *_Iter != nullptr; ++_Iter) {
  ...
  virtual message<_Type> *accept(runtime_object_identity _MsgId,
    ...
    // The message has been accepted/consumed, propagate indication that it has
    // succeeded
    _Done(accepted);
  ...
  // Reserve needs to be defined for ISource blocks, but Originator doesn't need
  // to do anything for reservation because there can only be one target block
...
// The Originator messaging block class is internal to the send function.
template <class _Type> class _AsyncOriginator : public ISource<_Type> {
public:
  typedef single_link_registry<ITarget<_Type>> _Target_registry;
  ...
  // Reserve needs to be defined for ISource blocks, but AsyncOriginator doesn't
  // need to do anything for reservation because there can only be one target
  // block to read the data at a later time.
  virtual bool reserve(runtime_object_identity _MsgId,
                       ITarget<_Type> *_PTarget) {
    if (_PTarget == nullptr) {
  ...
  virtual message<_Type> *consume(runtime_object_identity _MsgId,
    ...
    // We are done. Unlink from the target. DO NOT TOUCH "this" pointer after
    // unlink
  ...
  // Release needs to be defined for ISource blocks, but AsyncOriginator doesn't
  // need to do anything for reservation release because there can only be one
  // target block to read the data at a later time.
  virtual void release(runtime_object_identity _MsgId,
                       ITarget<_Type> *_PTarget) {
    if (_PTarget == nullptr) {
      throw ::std::invalid_argument("_PTarget");
  ...
  virtual void acquire_ref(_Inout_ ITarget<_Type> *) { _Acquire_ref(); }

  virtual void release_ref(_Inout_ ITarget<_Type> *) { _Release_ref(); }
...
private:
  ...
  _AsyncOriginator() : _M_pMessage(nullptr), _M_refcount(0) {}
  ...
  void _Acquire_ref() { _InterlockedIncrement(&_M_refcount); }
...
public:
  ...
  static bool _asend(ITarget<_Type> *_Trg, const _Type &_Data) {
    // If the block can participate in posting messages without requiring a call
    // back, use that method of initiating the message rather for efficiency
    // purposes.
    if (_Trg != nullptr && _Trg->supports_anonymous_source()) {
...
/// <summary>
///     A synchronous send operation, which waits until the target either
///     accepts or declines the message.
/// </summary>
/// <typeparam name="_Type">
///     The payload type.
/// </typeparam>
...
/// <seealso cref="asend Function"/>
/**/
template <class _Type> bool send(ITarget<_Type> &_Trg, const _Type &_Data) {
  return ::Concurrency::send(&_Trg, _Data);
}

/// <summary>
///     An asynchronous send operation, which schedules a task to propagate the
///     data to the target block.
/// </summary>
/// <typeparam name="_Type">
...
/// </param>
/// <returns>
///     <c>true</c> if the message was accepted before the method returned,
///     <c>false</c> otherwise.
/// </returns>
/// <remarks>
///     For more information, see <see cref="Message Passing Functions"/>.
...
/// <seealso cref="send Function"/>
/**/
template <class _Type> bool asend(ITarget<_Type> &_Trg, const _Type &_Data) {
...
/// <summary>
///     The <c>target_block</c> class is an abstract base class that provides
///     basic link management functionality and error checking for target only
///     blocks.
/// </summary>
...
template <class _SourceLinkRegistry,
          class _MessageProcessorType = ordered_message_processor<
              typename _SourceLinkRegistry::type::source_type>>
class target_block
    : public ITarget<typename _SourceLinkRegistry::type::source_type> {
public:
  /// <summary>
  ///     The type of the payload for the incoming messages to this
  ///     <c>target_block</c> object.
  /// </summary>
  /**/
  typedef typename _SourceLinkRegistry::type::source_type _Source_type;

  /// <summary>
  ///     The type of the <c>source_link_manager</c> this <c>target_block</c>
  ///     object.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     The type of the iterator for the <c>source_link_manager</c> for this
  ///     <c>target_block</c> object.
  /// </summary>
  /**/
  ...
  target_block() : _M_pFilter(nullptr), _M_fDeclineMessages(false) {
    _Trace_agents(
        AGENTS_EVENT_CREATE, ::Concurrency::details::_Trace_agents_get_id(this),
  ...
  virtual ~target_block() {
    ...
    _Trace_agents(AGENTS_EVENT_DESTROY,
                  ::Concurrency::details::_Trace_agents_get_id(this));
  }

  /// <summary>
  ///     Asynchronously passes a message from a source block to this target
  ///     block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// </returns>
  /// <remarks>
  ///     <para> The method throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if either the <paramref
  ///     name="_PMessage"/> or <paramref name="_PSource"/> parameter is
  ///     <c>NULL</c>.</para>
  /// </remarks>
  /**/
  virtual message_status
  propagate(_Inout_opt_ message<_Source_type> *_PMessage,
            _Inout_opt_ ISource<_Source_type> *_PSource) {
    // It is important that calls to propagate do *not* take the same lock on
    // the internal structure that is used by <c>consume</c> and the LWT.  Doing
    // so could result in a deadlock.
  ...
  /// <summary>
  ///     Synchronously passes a message from a source block to this target
  ///     block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// </remarks>
  /**/
  virtual message_status send(_Inout_ message<_Source_type> *_PMessage,
...
protected:
  /// <summary>
  ///     When overridden in a derived class, this method asynchronously passes
  ///     a message from an <c>ISource</c> block to this <c>target_block</c>
  ///     object. It is invoked by the <c>propagate</c> method, when called by a
  ///     source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /**/
  virtual message_status
  propagate_message(_Inout_ message<_Source_type> *_PMessage,
                    _Inout_ ISource<_Source_type> *_PSource) = 0;

  /// <summary>
  ///     When overridden in a derived class, this method synchronously passes a
  ///     message from an <c>ISource</c> block to this <c>target_block</c>
  ///     object. It is invoked by the <c>send</c> method, when called by a
  ///     source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// </returns>
  /// <remarks>
  ///     By default, this block returns <c>declined</c> unless overridden by a
  ///     derived class.
  /// </remarks>
  /**/
  virtual message_status send_message(_Inout_ message<_Source_type> *,
  ...
  /// </param>
  /// <remarks>
  ///     This function should not be called directly on a <c>target_block</c>
  ///     object. Blocks should be connected together using the
  ///     <c>link_target</c> method on <c>ISource</c> blocks, which will invoke
  ///     the <c>link_source</c> method on the corresponding target.
  /// </remarks>
  /**/
  ...
  /// </param>
  ///     This function should not be called directly on n <c>target_block</c>
  ///     object. Blocks should be disconnected using the <c>unlink_target</c>
  ///     or <c>unlink_targets</c> methods on <c>ISource</c> blocks, which will
  ///     invoke the <c>unlink_source</c> method on the corresponding target.
  /**/
  ...
  /**/
  virtual void unlink_sources() {
    for (source_iterator _Iter = _M_connectedSources.begin(); *_Iter != nullptr;
         ++_Iter) {
  ...
  /// <summary>
  ///     When overridden in a derived class, processes a message that was
  ///     accepted by this <c>target_block</c> object.
  /// </summary>
  /// <param name="_PMessage">
  ...
  /// </summary>
  /// <remarks>
  ///     This method is called by the destructor to ensure that new messages
  ///     are declined while destruction is in progress.
  /// </remarks>
  /**/
  void decline_incoming_messages() { _M_fDeclineMessages = true; }

  /// <summary>
  ///     Initializes the base object. Specifically, the
  ///     <c>message_processor</c> object needs to be initialized.
  /// </summary>
  /// <param name="_PScheduler">
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  void initialize_target(_Inout_opt_ Scheduler *_PScheduler = nullptr,
                         _Inout_opt_ ScheduleGroup *_PScheduleGroup = nullptr) {
    // Register a callback with the processor
    _M_messageProcessor.initialize(_PScheduler, _PScheduleGroup,
                                   // Processing and Propagating function used
                                   // by ordered_message_processors
                                   [this](message<_Source_type> *_PMessage) {
                                     // Handle message by calling
                                     // process_message to maintain CRT100
                                     // compatibility
  ...
  void enable_batched_processing() {
    _M_messageProcessor.initialize_batched_processing(
        ...
        [this](message<_Source_type> *_PMessage) {
          // Handle message through new process_input_message to use CRT110
          // batch processing
  ...
  /// </summary>
  /// <remarks>
  ///     This method is used by message block destructors to ensure all
  ///     asynchronous operations have had time to finish before destroying the
  ///     block.
  /// </remarks>
  /**/
  void wait_for_async_sends() {
    // Decline new messages to ensure that messages are not dropped during the
    // wait
  ...
  /// <summary>
  ///     Unlinks all sources after waiting for outstanding asynchronous send
  ///     operations to complete.
  /// </summary>
  /// <remarks>
  ///     All target blocks should call this routine to remove the sources in
  ///     their destructor.
  /// </remarks>
  /**/
  void remove_sources() {
  ...
  virtual void process_input_messages(_Inout_ message<_Source_type> *) {
    throw invalid_operation("To use batched processing, you must override "
                            "process_input_messages in the message block.");
  }
  ...
  /// <summary>
  ///     The filter function which determines whether offered messages should
  ///     be accepted.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     A <c>bool</c> that is set to indicate that all messages should be
  ///     declined in preparation for deleting the block
  /// <summary>
  /**/
...
/// <summary>
///     The <c>source_block</c> class is an abstract base class for source-only
///     blocks. The class provides basic link management functionality as well
///     as common error checks.
/// </summary>
...
/// </typeparam>
/// <remarks>
///     Message blocks should derive from this block to take advantage of link
///     management and synchronization provided by this class.
/// </remarks>
/// <seealso cref="ISource Class"/>
/**/
template <class _TargetLinkRegistry,
          class _MessageProcessorType = ordered_message_processor<
              typename _TargetLinkRegistry::type::type>>
...
public:
  ...
  /**/
  source_block()
      : _M_pReservedFor(nullptr), _M_reservedId(-1), _M_referenceCount(0) {
    _Trace_agents(
        AGENTS_EVENT_CREATE, ::Concurrency::details::_Trace_agents_get_id(this),
        ::Concurrency::details::_Trace_agents_get_id(&_M_messageProcessor));
  }
  ...
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to an <c>ITarget</c> block to link to this
  ///     <c>source_block</c> object.
  /// </param>
  /// <remarks>
  ///     The method throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if the parameter <paramref
  ///     name="_PTarget"/> is <c>NULL</c>.
  /// </remarks>
  /**/
  ...
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to an <c>ITarget</c> block to unlink from this
  ///     <c>source_block</c> object.
  /// </param>
  /// <remarks>
  ///     The method throws an <see cref="invalid_argument
  ...
  virtual void unlink_targets() {
    ...
    for (target_iterator _Iter = _M_connectedTargets.begin(); *_Iter != nullptr;
         ++_Iter) {
  ...
  /// <summary>
  ///     Accepts a message that was offered by this <c>source_block</c> object,
  ...
  ///     The method throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if the parameter <paramref
  ///     name="_PTarget"/> is <c>NULL</c>. <para> The <c>accept</c> method is
  ///     called by a target while a message is being offered by this
  ///     <c>ISource</c> block. The message pointer returned may be different
  ///     from the one passed into the <c>propagate</c> method of the
  ///     <c>ITarget</c> block, if this source decides to make a copy of the
  ///     message.
  ///     </para>
  /// </remarks>
  /**/
  virtual message<_Target_type> *
  accept(runtime_object_identity _MsgId,
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>source_block</c>
  ///     object.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  ///     The method throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if the parameter <paramref
  ///     name="_PTarget"/> is <c>NULL</c>. <para> After you call
  ///     <c>reserve</c>, if it succeeds, you must call either <c>consume</c> or
  ///     <c>release</c> in order to take or give up possession of the message,
  ///     respectively.
  ///     </para>
  /// </remarks>
  /**/
  ...
  /// <summary>
  ///     Consumes a message previously offered by this <c>source_block</c>
  ///     object and successfully reserved by the target, transferring ownership
  ///     to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the reserved <c>message</c>
  ...
  ///     </para>
  ///     <para>
  ///     The method throws a <see cref="bad_target Class">bad_target</see>
  ///     exception if the parameter <paramref name="_PTarget"/> does not
  ///     represent the target that called <c>reserve</c>.
  ///     </para>
  ///     <para>
  ///     The <c>consume</c> method is similar to <c>accept</c>, but must always
  ///     be preceded by a call to <c>reserve</c> that returned <c>true</c>.
  ...
  /**/
  virtual message<_Target_type> *
  consume(runtime_object_identity _MsgId,
          _Inout_ ITarget<_Target_type> *_PTarget) {
    _R_lock _Lock(_M_internalLock);
    ...
    if (_Msg != nullptr) {
      ...
      // _M_pReservedId is intentionally not reset so that it can assist in
      // debugging
      _M_pReservedFor = nullptr;

      // Reservation is assumed to block propagation. Notify that propagation
      // can now be resumed
      resume_propagation();
    }
  ...
  virtual void release(runtime_object_identity _MsgId,
    ...
    // Reservation is assumed to block propagation. Notify that propagation can
    // now be resumed
    resume_propagation();
  }

  /// <summary>
  ///     Acquires a reference count on this <c>source_block</c> object, to
  ///     prevent deletion.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling this method.
  ...
  virtual void release_ref(_Inout_ ITarget<_Target_type> *_PTarget) {
    if (_PTarget != nullptr) {
      ...
      // We assume that each target would keep a single reference on its source,
      // so we call unlink target notification on every release. Otherwise, we
      // would be required to keep a reference count per target. Note:
      // unlink_target_notification can check the value of this _PTarget
      // pointer, but must not dereference it, as it may have already been
      // deleted.
    ...
    // It is *unsafe* to touch the "this" pointer after decrementing the
    // reference count
  }
...
protected:
  ...
  /// <summary>
  ///     A callback that notifies that a new target has been linked to this
  ///     <c>source_block</c> object.
  /// </summary>
  /// <param name="_PTarget">
  ...
  /// <summary>
  ///     A callback that notifies that a target has been unlinked from this
  ///     <c>source_block</c> object.
  /// </summary>
  /// <param name="_PTarget">
  ...
  /**/
  virtual void
  unlink_target_notification(_Inout_ ITarget<_Target_type> *_PTarget) {
  ...
  /// <summary>
  ///     When overridden in a derived class, accepts an offered message by the
  ///     source. Message blocks should override this method to validate the
  ///     <paramref name="_MsgId"/> and return a message.
  /// </summary>
  /// <param name="_MsgId">
  ...
  /// </returns>
  /// <remarks>
  ///     To transfer ownership, the original message pointer should be
  ///     returned. To maintain ownership, a copy of message payload needs to be
  ///     made and returned.
  /// </remarks>
  /**/
  virtual message<_Target_type> *
  accept_message(runtime_object_identity _MsgId) = 0;

  /// <summary>
  ///     When overridden in a derived class, reserves a message previously
  ///     offered by this <c>source_block</c> object.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ///     reserved.
  /// </param>
  /// <returns>
  ///     <c>true</c> if the message was successfully reserved, <c>false</c>
  ...
  /// </returns>
  /// <remarks>
  ///     After <c>reserve</c> is called, if it returns <c>true</c>, either
  ///     <c>consume</c> or <c>release</c> must be called to either take or
  ...
  /// <summary>
  ///     When overridden in a derived class, consumes a message that was
  ///     previously reserved.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// </returns>
  /// <remarks>
  ///     Similar to <c>accept</c>, but is always preceded by a call to
  ///     <c>reserve</c>.
  /// </remarks>
  /**/
  virtual message<_Target_type> *
  consume_message(runtime_object_identity _MsgId) = 0;

  /// <summary>
  ///     When overridden in a derived class, releases a previous message
  ///     reservation.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     When overridden in a derived class, resumes propagation after a
  ...
  /// <summary>
  ///     Process input messages.  This is only useful for propagator blocks,
  ///     which derive from source_block
  /// </summary>
  /**/
  ...
  virtual void propagate_output_messages() {
    throw invalid_operation("To use batched processing, you must override "
                            "propagate_output_messages in the message block.");
  }

  /// <summary>
  ///     When overridden in a derived class, propagates the given message to
  ///     any or all of the linked targets. This is the main propagation routine
  ///     for message blocks.
  /// </summary>
  /// <param name="_PMessage">
  ...
  /**/
  virtual void
  propagate_to_any_targets(_Inout_opt_ message<_Target_type> *_PMessage) {
    (void)_PMessage;
    throw invalid_operation(
        "To use ordered message processing, you must override "
        "propagate_to_any_targets in the message block.");
  }
  ...
  //
  /// <summary>
  ///     Initializes the <c>message_propagator</c> within this
  ///     <c>source_block</c>.
  /// </summary>
  /// <param name="_PScheduler">
  ///     The scheduler to be used for scheduling tasks.
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  void initialize_source(_Inout_opt_ Scheduler *_PScheduler = nullptr,
                         _Inout_opt_ ScheduleGroup *_PScheduleGroup = nullptr) {
    // Register a callback
    _M_messageProcessor.initialize(_PScheduler, _PScheduleGroup,
  ...
  void enable_batched_processing() {
    ...
    _M_messageProcessor.initialize_batched_processing(
        ...
        [this]() { this->_Propagate_message(); });
  }

  /// <summary>
  ///     Synchronously queues up messages and starts a propagation task, if
  ///     this has not been done already.
  /// </summary>
  /// <param name="_Msg">
  ...
  /// <summary>
  ///     Asynchronously queues up messages and starts a propagation task, if
  ///     this has not been done already
  /// </summary>
  /// <param name="_Msg">
  ...
  /// <summary>
  ///     Waits for all asynchronous propagations to complete. This
  ///     propagator-specific spin wait is used in destructors of message blocks
  ///     to make sure that all asynchronous propagations have time to finish
  ///     before destroying the block.
  /// </summary>
  /**/
  void wait_for_outstanding_async_sends() { _M_messageProcessor.wait(); }

  /// <summary>
  ///     Removes all target links for this source block. This should be called
  ///     from the destructor.
  /// </summary>
  /**/
  void remove_targets() {
...
private:
  ...
  // Message handler callback for the propagator. Invokes
  // propagate_to_any_targets which derived classes should implement.
  /**/
  ...
  // Message handler callback for the propagator. Invokes
  // propagate_output_messages which derived classes should implement.
  /**/
...
//**************************************************************************
/// <summary>
///     The <c>propagator_block</c> class is an abstract base class for message
///     blocks that are both a source and target. It combines the functionality
///     of both the <c>source_block</c> and <c>target_block</c> classes.
/// </summary>
/// <typeparam name="_TargetLinkRegistry">
...
/// </typeparam>
/// <remarks>
///     To avoid multiple inheritance, the <c>propagator_block</c> class
///     inherits from the <c>source_block</c> class and <c>ITarget</c> abstract
///     class.  Most of the functionality in the <c>target_block</c> class is
///     replicated here.
/// </remarks>
...
class propagator_block
    : public source_block<_TargetLinkRegistry, _MessageProcessorType>,
      public ITarget<typename _SourceLinkRegistry::type::source_type> {
public:
  /// <summary>
  ///     The type of the payload for the incoming message to this
  ///     <c>propagator_block</c>.
  /// </summary>
  /**/
  typedef typename _SourceLinkRegistry::type::source_type _Source_type;

  /// <summary>
  ///     The type of the <c>source_link_manager</c> this
  ///     <c>propagator_block</c>.
  /// </summary>
  /**/
  typedef source_link_manager<_SourceLinkRegistry> _SourceLinkManager;
  ...
  using typename source_block<_TargetLinkRegistry,
                              _MessageProcessorType>::_Target_type;
  using typename ITarget<_Source_type>::filter_method;
  ...
  /// </returns>
  /// <remarks>
  ///     The <c>propagate</c> method is invoked on a target block by a linked
  ///     source block. It queues up an asynchronous task to handle the message,
  ///     if one is not already queued or executing. <para> The method throws an
  ///     <see cref="invalid_argument Class">invalid_argument</see> exception if
  ///     either the <paramref name="_PMessage"/> or <paramref name="_PSource"/>
  ...
  /// <summary>
  ///     Synchronously initiates a message to this block.  Called by an
  ///     <c>ISource</c> block. When this function completes, the message will
...
protected:
  /// <summary>
  ///     When overridden in a derived class, this method asynchronously passes
  ///     a message from an <c>ISource</c> block to this <c>propagator_block</c>
  ///     object. It is invoked by the <c>propagate</c> method, when called by a
  ///     source block.
  /// </summary>
  /// <param name="_PMessage">
  ...
  /// <summary>
  ///     When overridden in a derived class, this method synchronously passes a
  ///     message from an <c>ISource</c> block to this <c>propagator_block</c>
  ///     object. It is invoked by the <c>send</c> method, when called by a
  ///     source block.
  /// </summary>
  /// <param name="_PMessage">
  ...
  /// <summary>
  ///     Unlinks a specified source block from this <c>propagator_block</c>
  ///     object.
  /// </summary>
  /// <param name="_PSource">
  ///     A pointer to the <c>ISource</c> block that is to be unlinked.
  ...
  void initialize_source_and_target(
      _Inout_opt_ Scheduler *_PScheduler = nullptr,
      _Inout_opt_ ScheduleGroup *_PScheduleGroup = nullptr) {
  ...
  /// <summary>
  ///     Registers a filter method that will be invoked on every received
  ///     message.
  /// </summary>
  /// <param name="_Filter">
  ///     The filter method.
  ...
  /// <summary>
  ///     Removes all the source and target network links from this
  ///     <c>propagator_block</c> object.
  /// </summary>
  /**/
...
/// <summary>
///     An <c>unbounded_buffer</c> messaging block is a multi-target,
///     multi-source, ordered <c>propagator_block</c> capable of storing an
...
public:
  ...
  using typename ITarget<typename propagator_block<
      _TargetLinkRegistry, _SourceLinkRegistry>::_Source_type>::filter_method;

  /// <summary>
  ///     Constructs an <c>unbounded_buffer</c> messaging block.
  /// </summary>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
  ///     parameters. <para>The type <typeparamref name="filter_method"/> is a
  ///     functor with signature <c>bool (_Type const &amp;)</c> which is
  ///     invoked by this <c>unbounded_buffer</c> messaging block to determine
  ///     whether or not it should accept an offered message.</para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  unbounded_buffer() : _M_fForceRepropagation(false) {
  ...
  /// </summary>
  /// <param name="_Filter">
  ///     A filter function which determines whether offered messages should be
  ///     accepted.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ...
  /// </summary>
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>unbounded_buffer</c> object is scheduled.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  unbounded_buffer(Scheduler &_PScheduler) : _M_fForceRepropagation(false) {
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>unbounded_buffer</c> messaging block is scheduled.
  /// </param>
  /// <param name="_Filter">
  ///     A filter function which determines whether offered messages should be
  ///     accepted.
  ...
  /// </summary>
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>unbounded_buffer</c> messaging block is scheduled. The
  ///     <c>Scheduler</c> object used is implied by the schedule group.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
...
protected:
  ...
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>unbounded_buffer</c> messaging block. It is invoked by the
  ///     <c>propagate</c> method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  /// </param>
  ...
  /// </returns>
  /**/
  virtual message_status propagate_message(_Inout_ message<_Type> *_PMessage,
  ...
  /// <summary>
  ///     Synchronously passes a message from an <c>ISource</c> block to this
  ///     <c>unbounded_buffer</c> messaging block. It is invoked by the
  ///     <c>send</c> method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  /// </param>
  ...
  /// </returns>
  /**/
  virtual message_status send_message(_Inout_ message<_Type> *_PMessage,
                                      _Inout_ ISource<_Type> *_PSource) {
    _PMessage = _PSource->accept(_PMessage->msg_id(), this);
  ...
  /// <summary>
  ///     Overrides the <c>supports_anonymous_source</c> method to indicate that
  ///     this block can accept messages offered to it by a source that is not
  ///     linked.
  /// </summary>
  /// <returns>
  ...
  /// </returns>
  /**/
  virtual bool supports_anonymous_source() { return true; }

  /// <summary>
  ///     Accepts a message that was offered by this <c>unbounded_buffer</c>
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>unbounded_buffer</c>
  ///     messaging block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>unbounded_buffer</c>
  ///     messaging block and reserved by the target, transferring ownership to
  ///     the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  virtual void resume_propagation() {
    ...
    if (_M_messageBuffer._Count() > 0) {
      // Set the flag to force a repropagation. This flag is cleared when a
      // propagation happens The only functions that call this are release,
      // consume, and link_target, all of which hold the internal lock, so the
      // flag is guaranteed to be read by propagation, which also holds the same
      // lock.
  ...
  /// <summary>
  ///     A callback that notifies that a new target has been linked to this
  ///     <c>unbounded_buffer</c> messaging block.
  /// </summary>
  /// <param name="_PTarget">
  ...
  /// <summary>
  ///     Places the <c>message</c> <paramref name="_PMessage"/> in this
  ///     <c>unbounded_buffer</c> messaging block and tries to offer it to all
  ///     of the linked targets.
  /// </summary>
  ...
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to a <c>message</c> object that this <c>unbounded_buffer</c>
  ///     has taken ownership of.
  /// </param>
  /// <remarks>
  ///     If another message is already ahead of this one in the
  ///     <c>unbounded_buffer</c>, propagation to linked targets will not occur
  ///     until any earlier messages have been accepted or consumed. The first
  ///     linked target to successfully <c>accept</c> or <c>consume</c> the
  ///     message takes ownership, and no other target can then get the message.
  /// </remarks>
  /**/
  virtual void propagate_output_messages() {
    // Move the messages from the processedMessages queue to the internal
    // storage to make them ready for propagating out

    // If there are messages in the message queue, the queue is blocked and a
    // propagation should not happen unless it has been forced using
    // resume_propagation
...
private:
  ...
  void _Propagate_priority_order(
      ::Concurrency::details::_Queue<message<_Target_type>> &_MessageBuffer) {
    ...
    while (_Msg != nullptr) {
      ...
      for (target_iterator _Iter = this->_M_connectedTargets.begin();
           *_Iter != nullptr; ++_Iter) {
        ITarget<_Target_type> *_PTarget = *_Iter;
  ...
  /// <summary>
  ///     Deletes all messages currently stored in this message block.  Should
  ///     be called by the destructor to ensure any messages propagated in are
  ///     cleaned up.
  /// </summary>
  /**/
...
private:
  ...
  //
  unbounded_buffer const &
  operator=(unbounded_buffer const &);        // no assignment operator
...
/// <summary>
///     An <c>overwrite_buffer</c> messaging block is a multi-target,
///     multi-source, ordered <c>propagator_block</c> capable of storing a
///     single message at a time. New messages overwrite previously held ones.
/// </summary>
/// <typeparam name="_Type">
///     The payload type of the messages stored and propagated by the buffer.
/// </typeparam>
/// <remarks>
///     An <c>overwrite_buffer</c> messaging block propagates out copies of its
///     stored message to each of its targets. <para>For more information, see
///     <see cref="Asynchronous Message Blocks"/>.</para>
/// </remarks>
...
public:
  ...
  ///     parameters. <para>The type <typeparamref name="filter_method"/> is a
  ///     functor with signature <c>bool (_Type const &amp;)</c> which is
  ///     invoked by this <c>overwrite_buffer</c> messaging block to determine
  ///     whether or not it should accept an offered message.</para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  overwrite_buffer()
      : _M_pMessage(nullptr), _M_pReservedMessage(nullptr),
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>overwrite_buffer</c> messaging block is scheduled.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>overwrite_buffer</c> messaging block is scheduled. The
  ///     <c>Scheduler</c> object used is implied by the schedule group.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ...
  /// <summary>
  ///     Checks whether this <c>overwrite_buffer</c> messaging block has a
  ///     value yet.
  /// </summary>
  /// <returns>
  ...
  /// </returns>
  /**/
  bool has_value() const { return _M_fIsInitialized != 0; }

  /// <summary>
  ///     Gets a reference to the current payload of the message being stored in
  ///     the <c>overwrite_buffer</c> messaging block.
  /// </summary>
  /// <returns>
  ///     The payload of the currently stored message.
  /// </returns>
  /// <remarks>
  ///     The value stored in the <c>overwrite_buffer</c> could change
  ///     immediately after this method returns. This method will wait until a
  ///     message arrives if no message is currently stored in the
  ///     <c>overwrite_buffer</c>.
  /// </remarks>
  /**/
  _Type value() { return ::Concurrency::receive<_Type>(this); }
...
protected:
  ...
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>overwrite_buffer</c> messaging block. It is invoked by the
  ///     <c>propagate</c> method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  virtual message_status propagate_message(_Inout_ message<_Type> *_PMessage,
                                           _Inout_ ISource<_Type> *_PSource) {
    // It is important that calls to propagate do *not* take the same lock on
    // the internal structure that is used by Consume and the LWT.  Doing so
    // could result in a deadlock with the Consume call.
  ...
  /// <summary>
  ///     Accepts a message that was offered by this <c>overwrite_buffer</c>
  ///     messaging block, returning a copy of the message to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ///     object.
  ...
  /// </returns>
  /// <remarks>
  ///     The <c>overwrite_buffer</c> messaging block returns copies of the
  ///     message to its targets, rather than transferring ownership of the
  ///     currently held message.
  /// </remarks>
  /**/
  virtual message<_Type> *accept_message(runtime_object_identity _MsgId) {
    //
    ...
    // message stored.
    //
    // Because we are returning a copy, the accept routine for an
    // overwritebuffer does not need to grab the internalLock
    //
    message<_Type> *_Msg = nullptr;
    if (_M_pMessage->msg_id() == _MsgId) {
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>overwrite_buffer</c>
  ///     messaging block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>overwrite_buffer</c>
  ///     messaging block and reserved by the target, returning a copy of the
  ...
  virtual message<_Type> *consume_message(runtime_object_identity _MsgId) {
    ...
    if (_M_pReservedMessage != nullptr &&
        _M_pReservedMessage->msg_id() != _MsgId) {
      return nullptr;
    }
    // This redundant assert is specifically to make the /analyze switch happy,
    // which cannot recognize the same assertion above in if stmnt.
    _CONCRT_ASSERT(_M_pReservedMessage != nullptr);
  ...
  /// <summary>
  ///     A callback that notifies that a new target has been linked to this
  ///     <c>overwrite_buffer</c> messaging block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the newly linked target.
  /// </param>
  ...
  /// <summary>
  ///     Places the <c>message</c> <paramref name="_PMessage"/> in this
  ///     <c>overwrite_buffer</c> messaging block and offers it to all of the
  ///     linked targets.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to a <c>message</c> object that this <c>overwrite_buffer</c>
  ///     has taken ownership of.
  /// </param>
  /// <remarks>
  ///     This method overwrites the current message in the
  ///     <c>overwrite_buffer</c> with the newly accepted message <paramref
  ///     name="_PMessage"/>.
  /// </remarks>
  /**/
...
private:
  ...
  // Hide assignment operator and copy constructor
  //
  overwrite_buffer const &
  operator=(overwrite_buffer const &);        // no assignment operator
...
/// <summary>
///     A <c>call</c> messaging block is a multi-source, ordered
///     <c>target_block</c> that invokes a specified function when receiving a
///     message.
/// </summary>
/// <typeparam name="_Type">
...
/**/
template <class _Type,
          class _FunctorType = ::std::function<void(_Type const &)>>
class call : public target_block<multi_link_registry<ISource<_Type>>> {
private:
  /// <summary>
  ///     The function type that this block executes upon receiving a
  ///     <c>message</c>.
  /// </summary>
  /**/
...
public:
  using typename ITarget<
      typename target_block<_SourceLinkRegistry>::_Source_type>::filter_method;
  ...
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
  ///     parameters. <para>The type <typeparamref name="_Call_method"/> is a
  ///     functor with signature <c>void (_Type const &amp;)</c> which is
  ///     invoked by this <c>call</c> messaging block to process a
  ///     message.</para> <para>The type <typeparamref name="filter_method"/> is
  ///     a functor with signature <c>bool (_Type const &amp;)</c> which is
  ///     invoked by this <c>call</c> messaging block to determine whether or
  ///     not it should accept an offered message.</para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  call(_Call_method const &_Func) : _M_pFunc(_Func) {
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>call</c> messaging block is scheduled.
  /// </param>
  /// <param name="_Func">
  ///     A function that will be invoked for each accepted message.
  /// </param>
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  call(Scheduler &_PScheduler, _Call_method const &_Func) : _M_pFunc(_Func) {
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  call(Scheduler &_PScheduler, _Call_method const &_Func,
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>call</c> messaging block is scheduled. The <c>Scheduler</c>
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  call(ScheduleGroup &_PScheduleGroup, _Call_method const &_Func)
      : _M_pFunc(_Func) {
...
protected:
  ...
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>call</c> messaging block. It is invoked by the <c>propagate</c>
  ...
  /// <summary>
  ///     Synchronously passes a message from an <c>ISource</c> block to this
  ///     <c>call</c> messaging block. It is invoked by the <c>send</c> method,
  ///     when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  /// </param>
  ...
  /// <summary>
  ///     Processes a message that was accepted by this <c>call</c> messaging
  ///     block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the message that is to be handled.
...
/// <summary>
///     A <c>transformer</c> messaging block is a single-target, multi-source,
///     ordered <c>propagator_block</c> which can accept messages of one type
///     and is capable of storing an unbounded number of messages of a different
///     type.
/// </summary>
...
///     The payload type of the messages stored and propagated out by the
///     buffer.
/// </typeparam>
/// <remarks>
///     For more information, see <see cref="Asynchronous Message Blocks"/>.
...
class transformer
    : public propagator_block<single_link_registry<ITarget<_Output>>,
...
public:
  ...
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
  ///     parameters. <para>The type <typeparamref name="_Transform_method"/> is
  ///     a functor with signature <c>_Output (_Input const &amp;)</c> which is
  ///     invoked by this <c>transformer</c> messaging block to process a
  ///     message.</para> <para>The type <typeparamref name="filter_method"/> is
  ///     a functor with signature <c>bool (_Input const &amp;)</c> which is
  ///     invoked by this <c>transformer</c> messaging block to determine
  ///     whether or not it should accept an offered message.</para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>transformer</c> messaging block is scheduled.
  /// </param>
  /// <param name="_Func">
  ///     A function that will be invoked for each accepted message.
  /// </param>
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  transformer(Scheduler &_PScheduler, _Transform_method const &_Func,
              _Inout_opt_ ITarget<_Output> *_PTarget = nullptr)
      : _M_pFunc(_Func) {
    this->initialize_source_and_target(&_PScheduler);
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>transformer</c> messaging block is scheduled. The
  ///     <c>Scheduler</c> object used is implied by the schedule group.
  /// </param>
  /// <param name="_Func">
  ///     A function that will be invoked for each accepted message.
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  transformer(ScheduleGroup &_PScheduleGroup, _Transform_method const &_Func,
              _Inout_opt_ ITarget<_Output> *_PTarget = nullptr)
      : _M_pFunc(_Func) {
    this->initialize_source_and_target(nullptr, &_PScheduleGroup);
...
protected:
  ...
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>transformer</c> messaging block. It is invoked by the
  ///     <c>propagate</c> method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// </returns>
  /**/
  virtual message_status propagate_message(_Inout_ message<_Input> *_PMessage,
  ...
  /// <summary>
  ///     Synchronously passes a message from an <c>ISource</c> block to this
  ///     <c>transformer</c> messaging block. It is invoked by the <c>send</c>
  ///     method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// </returns>
  /**/
  virtual message_status send_message(_Inout_ message<_Input> *_PMessage,
                                      _Inout_ ISource<_Input> *_PSource) {
    _PMessage = _PSource->accept(_PMessage->msg_id(), this);
  ...
  /// <summary>
  ///     Accepts a message that was offered by this <c>transformer</c>
  ///     messaging block, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>transformer</c>
  ///     messaging block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>transformer</c> and
  ///     reserved by the target, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ///     consumed.
  ...
  /// <summary>
  ///     A callback that notifies that a new target has been linked to this
  ///     <c>transformer</c> messaging block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the newly linked target.
  /// </param>
...
private:
  ...
  void _Delete_stored_messages() {
    ...
    // Because the transformer uses its own input queue, it's possible there are
    // messages in this queue and no LWT will be executed to handle them.
...
//**************************************************************************
/// <summary>
///     A <c>timer</c> messaging block is a single-target <c>source_block</c>
///     capable of sending a message to its target after a specified time period
///     has elapsed or at specific intervals.
/// </summary>
/// <typeparam name="_Type">
...
public:
  /// <summary>
  ///     Constructs a <c>timer</c> messaging block that will fire a given
  ///     message after a specified interval.
  /// </summary>
  /// <param name="_Ms">
  ///     The number of milliseconds that must elapse after the call to start
  ///     for the specified message to be propagated downstream.
  /// </param>
  /// <param name="_Value">
  ...
  ///     If true, indicates that the timer will fire periodically every
  ///     <paramref name="_Ms"/> milliseconds.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_Scheduler"/> or <paramref name="_ScheduleGroup"/>
  ///     parameters.
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  timer(unsigned int _Ms, _Type const &_Value,
        ITarget<_Type> *_PTarget = nullptr, bool _Repeating = false)
  ...
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>timer</c> messaging block is scheduled is scheduled.
  /// </param>
  /// <param name="_Ms">
  ///     The number of milliseconds that must elapse after the call to start
  ///     for the specified message to be propagated downstream.
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  timer(Scheduler &_Scheduler, unsigned int _Ms, _Type const &_Value,
        _Inout_opt_ ITarget<_Type> *_PTarget = nullptr, bool _Repeating = false)
      : _Timer(_Ms, _Repeating) {
  ...
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>timer</c> messaging block is scheduled. The <c>Scheduler</c>
  ///     object used is implied by the schedule group.
  /// </param>
  /// <param name="_Ms">
  ///     The number of milliseconds that must elapse after the call to start
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  timer(ScheduleGroup &_ScheduleGroup, unsigned int _Ms, _Type const &_Value,
        _Inout_opt_ ITarget<_Type> *_PTarget = nullptr, bool _Repeating = false)
      : _Timer(_Ms, _Repeating) {
  ...
  ~timer() {
    //
    // Make sure there are no more outstanding timer fires.  Note that this does
    // not mean that the LWT that was queued is finished, it only means that no
    // more timers will fire after the return from _Stop.  We still *MUST* wait
    // on any outstanding LWTs.
    //
    if (_M_state == Started)
      _Stop();
  ...
  /// <summary>
  ///     Starts the <c>timer</c> messaging block.  The specified number of
  ///     milliseconds after this is called, the specified value will be
  ///     propagated downstream as a <c>message</c>.
  /// </summary>
  /**/
  void start() {
  ...
  /// <summary>
  ///     Stops the <c>timer</c> messaging block.  If it is a repeating
  ///     <c>timer</c> messaging block, it can be restarted with a subsequent
  ///     <c>start()</c> call.  For non-repeating timers, this has the same
  ///     effect as a <c>stop</c> call.
  /// </summary>
  /**/
  void pause() {
    //
    // Non repeating timers cannot pause.  They go to a final stopped state on
    // pause.
    //
    if (!_M_fRepeating) {
      stop();
...
protected:
  /// <summary>
  ///     Accepts a message that was offered by this <c>timer</c> messaging
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>timer</c> messaging
  ///     block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  virtual bool reserve_message(runtime_object_identity _MsgId) {
    //
    // Semantically, every timer tick is the same value -- it doesn't matter the
    // message ID.  Because we can only have one target as well, we do not need
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>timer</c> and reserved
  ///     by the target, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ///     consumed.
  ...
  /**/
  virtual void link_target_notification(_Inout_ ITarget<_Type> *_PTarget) {
    // If there is a timer message sitting around, it must be propagated to the
    // target now.
  ...
  /// <summary>
  ///     Tries to offer the message produced by the <c>timer</c> block to all
  ///     of the linked targets.
  /// </summary>
  /**/
...
private:
  ...
  /// </summary>
  /**/
  message<_Type> *_NewMessage() const { return new message<_Type>(_M_value); }

  /// <summary>
  ///     Called when the timer fires.
  /// </summary>
  /**/
  virtual void _Fire() { this->async_send(nullptr); }
  ...
  /// </param>
  /**/
  void _Initialize(const _Type &_Value, _Inout_ ITarget<_Type> *_PTarget,
                   bool _Repeating,
                   _Inout_opt_ Scheduler *_PScheduler = nullptr,
                   _Inout_opt_ ScheduleGroup *_PScheduleGroup = nullptr) {
    _M_pMessage = nullptr;
    ...
    //
    // If we are going to utilize the current scheduler for timer firing, we
    // need to capture it now.  Otherwise, the timer threads fired from Windows
    // (what _Fire executes within) will wind up with a default scheduler
    // attached -- probably not the semantic we want.
    //
    if (_PScheduleGroup == nullptr && _PScheduler == nullptr) {
      ::Concurrency::details::_Scheduler _sched =
          ::Concurrency::details::_CurrentScheduler::_Get();
...
/// <summary>
///     A <c>single_assignment</c> messaging block is a multi-target,
///     multi-source, ordered <c>propagator_block</c> capable of storing a
///     single, write-once <c>message</c>.
/// </summary>
/// <typeparam name="_Type">
///     The payload type of the message stored and propagated by the buffer.
/// </typeparam>
/// <remarks>
///     A <c>single_assignment</c> messaging block propagates out copies of its
///     message to each target. <para>For more information, see <see
///     cref="Asynchronous Message Blocks"/>.</para>
/// </remarks>
/// <seealso cref="overwrite_buffer Class"/>
/// <seealso cref="unbounded_buffer Class"/>
/**/
...
public:
  ...
  ///     parameters. <para>The type <typeparamref name="filter_method"/> is a
  ///     functor with signature <c>bool (_Type const &amp;)</c> which is
  ///     invoked by this <c>single_assignment</c> messaging block to determine
  ///     whether or not it should accept an offered message.</para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  single_assignment() : _M_pMessage(nullptr), _M_fIsInitialized(false) {
    this->initialize_source_and_target();
  }
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>single_assignment</c> messaging block is scheduled.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PScheduleGroup"/>
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>single_assignment</c> messaging block is scheduled. The
  ///     <c>Scheduler</c> object used is implied by the schedule group.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ...
  /// <summary>
  ///     Checks whether this <c>single_assignment</c> messaging block has been
  ///     initialized with a value yet.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the block has received a value, <c>false</c> otherwise.
  /// </returns>
  /**/
  bool has_value() const { return (_M_pMessage != nullptr); }

  /// <summary>
  ///     Gets a reference to the current payload of the message being stored in
  ///     the <c>single_assignment</c> messaging block.
  /// </summary>
  /// <returns>
  ///     The payload of the stored message.
  /// </returns>
  /// <remarks>
  ///     This method will wait until a message arrives if no message is
  ///     currently stored in the <c>single_assignment</c> messaging block.
  /// </remarks>
  /**/
  _Type const &value() {
    if (_M_pMessage == nullptr) {
...
protected:
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>single_assignment</c> messaging block. It is invoked by the
  ///     <c>propagate</c> method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// <summary>
  ///     Accepts a message that was offered by this <c>single_assignment</c>
  ///     messaging block, returning a copy of the message to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  /// </returns>
  /// <remarks>
  ///     The <c>single_assignment</c> messaging block returns copies of the
  ///     message to its targets, rather than transferring ownership of the
  ///     currently held message.
  /// </remarks>
  /**/
  virtual message<_Type> *accept_message(runtime_object_identity _MsgId) {
    // This check is to prevent spoofing and verify that the propagated message
    // is the one that is accepted at the end.
    if (_M_pMessage == nullptr || _MsgId != _M_pMessage->msg_id()) {
      return nullptr;
    }
    ...
    // message passed in.
    //
    // Because we are returning a copy, the accept routine for a
    // single_assignment does not need to grab the internal lock.
    //
    return (new message<_Type>(_M_pMessage->payload));
  }

  /// <summary>
  ///     Reserves a message previously offered by this <c>single_assignment</c>
  ///     messaging block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>single_assignment</c>
  ///     and reserved by the target, returning a copy of the message to the
  ///     caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     A callback that notifies that a new target has been linked to this
  ///     <c>single_assignment</c> messaging block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the newly linked target.
  /// </param>
  ...
  /// <summary>
  ///     Places the <c>message</c> <paramref name="_PMessage"/> in this
  ///     <c>single_assignment</c> messaging block and offers it to all of the
  ///     linked targets.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to a <c>message</c> that this <c>single_assignment</c>
  ///     messaging block has taken ownership of.
  /// </param>
  /**/
  virtual void propagate_to_any_targets(_Inout_opt_ message<_Type> *_PMessage) {
    // Initialized flag should have been set by the propagate function using
...
private:
  ...
  // Hide assignment operator and copy constructor
  //
  single_assignment const &
  operator=(single_assignment const &);         // no assignment operator
...
enum join_type {
  /// <summary>
  ///     Greedy <c>join</c> messaging blocks immediately accept a message upon
  ///     propagation.  This is more efficient,
  ///     but has the possibility for live-lock, depending on the network
  ///     configuration.
  /// </summary>
  /**/
  greedy = 0,
  /// <summary>
  ///     Non-greedy <c>join</c> messaging blocks postpone messages and try and
  ///     consume them after all have arrived.
...
///     <c>propagator_block</c> which combines together messages of type
///     <typeparamref name="_Type"/> from each of its sources.
/// </summary>
/// <typeparam name="_Type">
///     The payload type of the messages joined and propagated by the block.
/// </typeparam>
/// <typeparam name="_Jtype">
///     The kind of <c>join</c> block this is, either <c>greedy</c> or
///     <c>non_greedy</c>
/// </typeparam>
/// <remarks>
///     For more information, see <see cref="Asynchronous Message Blocks"/>.
...
class join : public propagator_block<
                 single_link_registry<ITarget<::std::vector<_Type>>>,
                 multi_link_registry<ISource<_Type>>> {
private:
  typedef single_link_registry<ITarget<::std::vector<_Type>>>
      _TargetLinkRegistry;
  typedef multi_link_registry<ISource<_Type>> _SourceLinkRegistry;
...
public:
  ...
  using typename ITarget<typename propagator_block<
      _TargetLinkRegistry, _SourceLinkRegistry>::_Source_type>::filter_method;
  using typename propagator_block<_TargetLinkRegistry,
                                  _SourceLinkRegistry>::source_iterator;
  ...
  ///     parameters. <para>The type <typeparamref name="filter_method"/> is a
  ///     functor with signature <c>bool (_Type const &amp;)</c> which is
  ///     invoked by this <c>join</c> messaging block to determine whether or
  ///     not it should accept an offered message.</para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  ...
  join(size_t _NumInputs)
      : _M_messageArray(_NumInputs), _M_savedMessageIdArray(_NumInputs) {
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>join</c> messaging block is scheduled.
  /// </param>
  /// <param name="_NumInputs">
  ///     The number of inputs this <c>join</c> block will be allowed.
  /// </param>
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>join</c> messaging block is scheduled. The <c>Scheduler</c>
  ///     object used is implied by the schedule group.
  /// </param>
  /// <param name="_NumInputs">
  ///     The number of inputs this <c>join</c> block will be allowed.
...
protected:
  ...
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>join</c> messaging block. It is invoked by the <c>propagate</c>
  ///     method, when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// </returns>
  /**/
  message_status propagate_message(_Inout_ message<_Type> *_PMessage,
                                   _Inout_ ISource<_Type> *_PSource) {
    // It is important that calls to propagate do *not* take the same lock on
    // the internal structure that is used by Consume and the LWT.  Doing so
    // could result in a deadlock with the Consume call.
    ...
    size_t _Slot = 0;
    bool _Found = false;
    for (source_iterator _Iter = this->_M_connectedSources.begin();
         *_Iter != nullptr; ++_Iter) {
      if (*_Iter == _PSource) {
        _Found = true;
        break;
    ...
    if (!_Found) {
      // If this source was not found in the array, this is not a connected
      // source decline the message
      return declined;
    }
    ...
    if (fIsGreedy) {
      ...
      if (_Ret_val != postponed) {
        _M_messageArray._M_messages[_Slot] =
      ...
      if (_InterlockedExchange(
              (volatile long *)&_M_savedMessageIdArray._M_savedIds[_Slot],
              _PMessage->msg_id()) == -1) {
  ...
  /// <summary>
  ///     Accepts a message that was offered by this <c>join</c> messaging
  ///     block, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>join</c> messaging
  ///     block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>join</c> messaging
  ///     block and reserved by the target, transferring ownership to the
  ///     caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// </remarks>
  /**/
  virtual message<_OutputType> *
  consume_message(runtime_object_identity _MsgId) {
    // By default, accept the message
    return accept_message(_MsgId);
  }
  ...
  /**/
  virtual void
  link_target_notification(_Inout_ ITarget<::std::vector<_Type>> *) {
    // If the message queue is blocked due to reservation
    // there is no need to do any message propagation
    if (this->_M_pReservedFor != nullptr) {
      return;
  ...
  /// <summary>
  ///     Constructs an output message containing an input message from each
  ///     source when they have all propagated a message.  Sends this output
  ///     message out to each of its targets.
  /// </summary>
  /**/
  void propagate_to_any_targets(_Inout_opt_ message<_OutputType> *) {
    message<_OutputType> *_Msg = nullptr;
    // Create a new message from the input sources
    // If messagesRemaining == 0, we have a new message to create.  Otherwise,
    // this is coming from a consume or release from the target.  In that case
    // we don't want to create a new message.
    ...
    // Check to see if multiple messages have been passed in on any of the
    // links, and postponed. If so, try and reserve/consume them now
    if (fIsGreedy) {
      // Look at the saved IDs and reserve/consume any that have passed in while
      // this join was waiting to complete
      _CONCRT_ASSERT(_M_messageArray._M_count ==
                     _M_savedMessageIdArray._M_count);

      for (size_t i = 0; i < _M_messageArray._M_count; i++) {
        for (;;) {
          runtime_object_identity _Saved_id;
          // Grab the current saved ID value.  This value could be changing from
          // based on any calls of source->propagate(this).  If the message ID
          // is different than what is snapped here, that means, the reserve
          // below must fail.  This is because reserve is trying to get the same
          // source lock the propagate(this) call must be holding.
          {
            _NR_lock lockHolder(_M_propagationLock);
          ...
          if (_Saved_id != -1) {
            ...
            if ((_PSource != nullptr) && _PSource->reserve(_Saved_id, this)) {
              _M_messageArray._M_messages[i] =
                  _PSource->consume(_Saved_id, this);
      ...
      // If messages have all been received, async_send again, this will start
      // the LWT up to create a new message
      if (_M_messagesRemaining == 0) {
        this->async_send(nullptr);
      }
    }
    ...
    if (!_M_messageBuffer._Is_head(_Msg->msg_id())) {
      // another message is at the head of the outbound message queue and
      // blocked simply return
      return;
    }
...
private:
  ...
  message<::std::vector<_Type>> *__cdecl _Create_new_message() {
    ...
    // If this is a non-greedy join, check each source and try to consume their
    // message
    if (fIsNonGreedy) {
      ...
      for (size_t i = 0; i < _M_savedMessageIdArray._M_count; i++) {
        // Snap the current saved ID into a buffer.  This value can be changing
        // behind the scenes from other source->propagate(msg, this) calls, but
        // if so, that just means the reserve below will fail.
        _InterlockedIncrementSizeT(&_M_messagesRemaining);
        _M_savedIdBuffer[i] = _InterlockedExchange(
            (volatile long *)&_M_savedMessageIdArray._M_savedIds[i], -1);
        ...
        if (!_Sources[i]->reserve(_M_savedIdBuffer[i], this)) {
          ...
          for (size_t j = 0; j < i; j++) {
            ...
            if (_InterlockedCompareExchange(
                    (volatile long *)&_M_savedMessageIdArray._M_savedIds[j],
                    _M_savedIdBuffer[j], -1) == -1) {
              if (_InterlockedDecrementSizeT(&_M_messagesRemaining) == 0) {
                this->async_send(nullptr);
              }
            }
      ...
      for (size_t i = 0; i < _M_messageArray._M_count; i++) {
        _M_messageArray._M_messages[i] =
            _Sources[i]->consume(_M_savedIdBuffer[i], this);
    ...
    if (!fIsNonGreedy) {
      ...
      // This is safe because all messages have been received, thus no new
      // async_sends for greedy joins can be called.
  ...
  /// </param>
  /// <param name="_PScheduler">
  ///     The scheduler onto which the task to propagate the <c>join</c> block's
  ///     message will be scheduled. If unspecified, the <c>join</c> messaging
  ///     block uses the default scheduler.
  /// </param>
  /// <param name="_PScheduleGroup">
  ///     The schedule group into which the task to propagate the <c>join</c>
  ///     block's message will be scheduled. The scheduler used is implied by
  ///     the schedule group.  If unspecified, the <c>join</c> uses a schedule
  ///     group of the scheduler's choosing.
  /// </param>
  /**/
  void _Initialize(size_t _NumInputs, Scheduler *_PScheduler = nullptr,
                   ScheduleGroup *_PScheduleGroup = nullptr) {
  ...
  struct _MessageArray {
    ...
    _MessageArray(size_t _NumInputs)
        : _M_count(_NumInputs), _M_messages(new message<_Type> *[_NumInputs]) {
  ...
  struct _SavedMessageIdArray {
    ...
    ~_SavedMessageIdArray() { delete[] _M_savedIds; }
  };
  ...
  ::Concurrency::details::_Queue<message<::std::vector<_Type>>>
      _M_messageBuffer;
};
...
class _Order_node_base
    : public propagator_block<single_link_registry<ITarget<size_t>>,
                              multi_link_registry<ISource<_Type>>> {
public:
  /// <summary>
  ///     Constructs a _Order_node_base within the default scheduler, and places
  ///     it on any schedule group of the scheduler's choosing.
  /// </summary>
  /**/
  _Order_node_base()
      : _M_pReceiveMessage(nullptr), _M_pSendMessage(nullptr), _M_index(0) {}
  ...
  /// </returns>
  /**/
  bool has_value() const { return (_M_pReceiveMessage != nullptr); }
  ...
  /// </summary>
  /// <remarks>
  ///     _Reset is called from Populate_destination_tuple through
  ///     propagate_to_any_targets() thus, it always has the internal lock held.
  ///     This is only used for _Greedy_node and _Non_greedy_node.
  /// </remarks>
  /**/
  ...
  /// </returns>
  /// <remarks>
  ///     After 'reserve' is called, either 'consume' or 'release' must be
  ///     called.
  /// </remarks>
  /**/
  ...
  /// <summary>
  ///     Consumes a message previously offered by the source and reserved by
  ///     the target, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The runtime object identity of the message.
  /// </param>
...
protected:
  ...
  /// <summary>
  ///     Create a message that contains an index used to determine the source
  ///     message
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     Validate constructor arguments and fully connect this
  ///     _Order_node_base.
  /// </summary>
  /**/
  void _Initialize_order_node(ISource<_Type> *_PSource, size_t _Index,
                              ITarget<size_t> *_PTarget,
                              Scheduler *_PScheduler = nullptr,
                              ScheduleGroup *_PScheduleGroup = nullptr) {
    if (_PSource == nullptr) {
      throw ::std::invalid_argument("_PSource");
    }
...
private:
  ...
  // Hide assignment operator and copy constructor
  //
  _Order_node_base const &
  operator=(_Order_node_base const &);        // no assignment operator
...
/// </typeparam>
/**/
template <class _Type> class _Reserving_node : public _Order_node_base<_Type> {
private:
...
public:
  ...
  /// <summary>
  ///     Constructs a _Reserving_node within the default scheduler, and places
  ///     it on any schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PSource">
  ...
  /// </param>
  /// <param name="_PTarget">
  ///     The target to which the node will signal about having received its
  ///     input data
  /// </param>
  /**/
  _Reserving_node(ISource<_Type> *_PSource, size_t _Index,
                  ITarget<size_t> *_PTarget = nullptr)
      : _M_fIsInitialized(false), _M_savedId(-1), _M_pReservedSource(nullptr) {
  ...
  /**/
  _Reserving_node(ISource<_Type> *_PSource, size_t _Index,
                  ITarget<size_t> *_PTarget, filter_method const &_Filter)
      : _M_fIsInitialized(false), _M_savedId(-1), _M_pReservedSource(nullptr) {
    this->register_filter(_Filter);
    this->_Initialize_order_node(_PSource, _Index, _PTarget);
  }
  ...
  /// <summary>
  ///     Constructs a _Reserving_node within the specified scheduler, and
  ///     places it on any schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PScheduler">
  ...
  /// </param>
  /**/
  _Reserving_node(Scheduler &_PScheduler, ISource<_Type> *_PSource,
                  size_t _Index, ITarget<size_t> *_PTarget = nullptr)
      : _M_fIsInitialized(false), _M_savedId(-1), _M_pReservedSource(nullptr) {
  ...
  /**/
  _Reserving_node(Scheduler &_PScheduler, ISource<_Type> *_PSource,
                  size_t _Index, ITarget<size_t> *_PTarget,
                  filter_method const &_Filter)
      : _M_fIsInitialized(false), _M_savedId(-1), _M_pReservedSource(nullptr) {
    this->register_filter(_Filter);
    this->_Initialize_order_node(_PSource, _Index, _PTarget, &_PScheduler);
  ...
  /// <summary>
  ///     Constructs a _Order_node within the specified schedule group.  The
  ///     scheduler is implied by the schedule group.
  /// </summary>
  /// <param name="_PScheduleGroup">
  ...
  /// </param>
  /**/
  _Reserving_node(ScheduleGroup &_PScheduleGroup, ISource<_Type> *_PSource,
                  size_t _Index, ITarget<size_t> *_PTarget = nullptr)
      : _M_fIsInitialized(false), _M_savedId(-1), _M_pReservedSource(nullptr) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget, nullptr,
                                 &_PScheduleGroup);
  }
  ...
  /// </summary>
  /// <remarks>
  ///     This function is not used in a _Reserving_node, which is only used for
  ///     choice blocks
  /// </remarks>
  /**/
  virtual void _Reset() {}
...
protected:
  ...
  /// <summary>
  ///     Asynchronously passes a message from an <c>ISource</c> block to this
  ///     <c>ITarget</c> block. It is invoked by the <c>propagate</c> method,
  ///     when called by a source block.
  /// </summary>
  /// <param name="_PMessage">
  ///     A pointer to the <c>message</c> object.
  ...
  /// <remarks>
  ///     It is important that calls to propagate do *not* take the same lock on
  ///     the internal structure that is used by Consume and the light-weight
  ///     task.  Doing so could result in a deadlock with the Consume call.
  /// </remarks>
  /**/
  virtual message_status propagate_message(message<_Type> *_PMessage,
    ...
    // _Order_node messaging block can be initialized only once, just like
    // single_assignment.
    if (_M_fIsInitialized) {
      return declined;
    }

    // Reserve a message on the source until this _Order_node gets the feedback
    // from the single_assignment on whether it has been selected.
  ...
  virtual message<size_t> *accept_message(runtime_object_identity _MsgId) {
    ...
    if (this->_M_pSendMessage == nullptr ||
        _MsgId != this->_M_pSendMessage->msg_id()) {
      return nullptr;
    }
  ...
  /// <summary>
  ///     Takes the message and propagates it to all the targets of this
  ///     _Order_node
  /// </summary>
  /// <param name="_PMessage">
  ...
  /// </param>
  /// <remarks>
  ///     This function packages its _M_index into a message and immediately
  ///     sends it to the targets.
  /// </remarks>
  /**/
...
private:
  ...
  message_status _Propagate_to_target(ITarget<size_t> *_PTarget) {
    ...
    // If the message got rejected we have to release the hold on the source
    // message.
    if (_Status != accepted) {
      if (_M_savedId != -1) {
        // Release the reservation
  ...
  // For greedy order-nodes, the message ID of subsequent messages sent to this
  // node For non-greedy order nodes, the message ID of the message to
  // reserve/consume
...
private:
  ...
  // Hide assignment operator and copy constructor
  //
  _Reserving_node const &
  operator=(_Reserving_node const &);       // no assignment operator
...
/// </typeparam>
/**/
template <class _Type> class _Greedy_node : public _Order_node_base<_Type> {
private:
  typedef single_link_registry<ITarget<size_t>> _TargetLinkRegistry;
  typedef multi_link_registry<ISource<_Type>> _SourceLinkRegistry;
...
public:
  ...
  /// <summary>
  ///     Constructs a _Greedy_node within the default scheduler, and places it
  ///     on any schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PSource">
  ///     The source of data passed into the node
  /// </param>
  ...
  /// </param>
  /**/
  _Greedy_node(ISource<_Type> *_PSource, size_t _Index,
               ITarget<size_t> *_PTarget = nullptr)
      : _M_pGreedyMessage(nullptr), _M_savedId(-1) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget);
  }
  ...
  /// <summary>
  ///     Constructs a _Greedy_node within the specified scheduler, and places
  ///     it on any schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PScheduler">
  ///     A reference to a scheduler instance.
  ...
  /// </param>
  /**/
  _Greedy_node(Scheduler &_PScheduler, ISource<_Type> *_PSource, size_t _Index,
               ITarget<size_t> *_PTarget = nullptr)
      : _M_pGreedyMessage(nullptr), _M_savedId(-1) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget, &_PScheduler);
  }
  ...
  /// <summary>
  ///     Constructs a _Greedy_node within the specified schedule group.  The
  ///     scheduler is implied by the schedule group.
  /// </summary>
  /// <param name="_PScheduleGroup">
  ///     A reference to a schedule group.
  ...
  /// </param>
  /**/
  _Greedy_node(ScheduleGroup &_PScheduleGroup, ISource<_Type> *_PSource,
               size_t _Index, ITarget<size_t> *_PTarget = nullptr)
      : _M_pGreedyMessage(nullptr), _M_savedId(-1) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget, nullptr,
                                 &_PScheduleGroup);
  ...
  void _Reset() {
    ...
    //
    for (;;) {
      // Set the current saved ID as -1.  Check to see if something was ready
      // for consumption (if _Saved_id != -1) and consume it if possible.
      runtime_object_identity _Saved_id;
...
protected:
  ...
  virtual void propagate_to_any_targets(_Inout_opt_ message<size_t> *) {
    ...
    if (this->_M_pSendMessage == nullptr) {
      // Save the incoming message so that it can be consumed in the accept
      // function
...
/// </typeparam>
/**/
template <class _Type> class _Non_greedy_node : public _Order_node_base<_Type> {
private:
  typedef single_link_registry<ITarget<size_t>> _TargetLinkRegistry;
  typedef multi_link_registry<ISource<_Type>> _SourceLinkRegistry;
...
public:
  ...
  /// <summary>
  ///     Constructs a _Non_greedy_node within the default scheduler, and places
  ///     it on any schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PSource">
  ///     The source of data passed into the node
  ...
  /// </param>
  /**/
  _Non_greedy_node(ISource<_Type> *_PSource, size_t _Index,
                   ITarget<size_t> *_PTarget = nullptr)
      : _M_savedId(-1), _M_reservedId(-1), _M_pReservedSource(nullptr) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget);
  }
  ...
  /// <summary>
  ///     Constructs a _Non_greedy_node within the specified scheduler, and
  ///     places it on any schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PScheduler">
  ///     A reference to a scheduler instance.
  ...
  /// </param>
  /**/
  _Non_greedy_node(Scheduler &_PScheduler, ISource<_Type> *_PSource,
                   size_t _Index, ITarget<size_t> *_PTarget = nullptr)
      : _M_savedId(-1), _M_reservedId(-1), _M_pReservedSource(nullptr) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget, &_PScheduler);
  }
  ...
  /// <summary>
  ///     Constructs a _Non_greedy_node within the specified schedule group. The
  ///     scheduler is implied by the schedule group.
  /// </summary>
  /// <param name="_PScheduleGroup">
  ///     A reference to a schedule group.
  ...
  /// </param>
  /**/
  _Non_greedy_node(ScheduleGroup &_PScheduleGroup, ISource<_Type> *_PSource,
                   size_t _Index, ITarget<size_t> *_PTarget = nullptr)
      : _M_savedId(-1), _M_reservedId(-1), _M_pReservedSource(nullptr) {
    this->_Initialize_order_node(_PSource, _Index, _PTarget, nullptr,
                                 &_PScheduleGroup);
  ...
  /// <summary>
  ///     Called for a non_greedy type join block in order to reserve the
  ///     message in this join block
  /// </summary>
  /// <returns>
  ...
  bool _Reserve_received_message() {
    ...
    // Obtain an iterator to the first source. It will guarantee that the
    // reference count on the source is maintained
    source_iterator _Iter = this->_M_connectedSources.begin();
    ISource<_Type> *_PSource = *_Iter;

    if (_PSource != nullptr) {
      // CAS out the current saved ID, in order to try and reserve it
      runtime_object_identity _SavedId =
          _InterlockedExchange((volatile long *)&_M_savedId, -1);

      _Ret_val = _PSource->reserve(_SavedId, this);
      //
      // If this reserved failed, that means we need to wait for another message
      // to come in on this link.  _M_savedID was set to -1 to indicate to the
      // _Order_node that it needs to async_send when that next message comes
      // through
      //
      // If the reserve succeeds, save away the reserved ID.  This will be use
      // later in consume
      //
      if (_Ret_val) {
  ...
  /// <summary>
  ///     Called for a non_greedy type join block in order to consume the
  ///     message in this join block that has been reserved
  /// </summary>
  /**/
  void _Consume_received_message() {
    if (_M_pReservedSource != nullptr) {
      ...
      runtime_object_identity _OldId =
          _InterlockedExchange((volatile long *)&_M_reservedId, -1);
  ...
  /// <summary>
  ///     Called for a non_greedy type join block release a reservation on this
  ///     block
  /// </summary>
  /**/
  bool _Release_received_message() {
    ...
    if (_M_pReservedSource != nullptr) {
      ...
      if (_InterlockedCompareExchange((volatile long *)&_M_savedId, _SavedId,
                                      -1) == -1) {
        retVal = true;
      }
...
protected:
  ...
  virtual message_status propagate_message(message<_Type> *_PMessage,
                                           ISource<_Type> *) {
    // Change the message ID.  If it was -1, that means an async-send needs to
    // occur
    if (_InterlockedExchange((volatile long *)&_M_savedId,
                             _PMessage->msg_id()) == -1) {
      this->async_send(nullptr);
    }
  ...
  virtual message<size_t> *accept_message(runtime_object_identity _MsgId) {
    ...
    //
    // Because we are returning a copy, the accept routine for a
    // _Non_greedy_node does not need to grab the internal lock.
    //
    return (new message<size_t>(this->_M_pSendMessage->payload));
  }
...
private:
  ...
  // For non-greedy order nodes, the message ID of the message to
  // reserve/consume
  runtime_object_identity _M_savedId;

  // For non-greedy order nodes, the reserved ID of the message that was
  // reserved
  runtime_object_identity _M_reservedId;
...
private:
  ...
  // Hide assignment operator and copy constructor
  //
  _Non_greedy_node const &
  operator=(_Non_greedy_node const &);        // no assignment operator
...
/// <summary>
///     A <c>choice</c> messaging block is a multi-source, single-target block
///     that represents a control-flow interaction with a set of sources. The
///     choice block will wait for any one of multiple sources to produce a
///     message and will propagate the index of the source that produced the
///     message.
/// </summary>
/// <typeparam name="_Type">
///     A <c>tuple</c>-based type representing the payloads of the input
///     sources.
/// </typeparam>
/// <remarks>
///     The choice block ensures that only one of the incoming messages is
///     consumed. <para>For more information, see <see cref="Asynchronous
///     Message Blocks"/>.</para>
/// </remarks>
...
public:
  ...
  ///     <para>
  ///         The runtime uses the default scheduler if you do not specify the
  ///         <paramref name="_PScheduler"/> or <paramref
  ///         name="_PScheduleGroup"/> parameters.
  ///     </para>
  ///     <para>
  ///         Move construction is not performed under a lock, which means that
  ///         it is up to the user to make sure that there are no light-weight
  ///         tasks in flight at the time of moving. Otherwise, numerous races
  ///         can occur, leading to exceptions or inconsistent state.
  ///     </para>
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  explicit choice(_Type _Tuple)
      : _M_sourceTuple(_Tuple), _M_pScheduler(nullptr),
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>choice</c> messaging block is scheduled.
  /// </param>
  /// <param name="_Tuple">
  ///     A <c>tuple</c> of sources for the choice.
  /// </param>
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  choice(Scheduler &_PScheduler, _Type _Tuple)
      : _M_sourceTuple(_Tuple), _M_pScheduler(&_PScheduler),
        _M_pScheduleGroup(nullptr) {
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>choice</c> messaging block is scheduled. The <c>Scheduler</c>
  ///     object used is implied by the schedule group.
  /// </param>
  /// <param name="_Tuple">
  ///     A <c>tuple</c> of sources for the choice.
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  choice(ScheduleGroup &_PScheduleGroup, _Type _Tuple)
      : _M_sourceTuple(_Tuple), _M_pScheduler(nullptr),
        _M_pScheduleGroup(&_PScheduleGroup) {
  ...
  ///     Note that the original object is orphaned, making this a move
  ///     constructor.
  /// </param>
  /// <remarks>
  ///     <para>
  ...
  choice(choice &&_Choice) {
    ...
    // Single assignment is heap allocated, so simply copy the pointer. If it
    // already has a value, it will be preserved.
    ...
    // Copy the pointers to order nodes to a new object and zero out in the old
    // object.
    memcpy(_M_pSourceChoices, _Choice._M_pSourceChoices,
           sizeof(_M_pSourceChoices));
  ...
  /// <summary>
  ///     Checks whether this <c>choice</c> messaging block has been initialized
  ///     with a value yet.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the block has received a value, <c>false</c> otherwise.
  /// </returns>
  /**/
  bool has_value() const { return _M_pSingleAssignment->has_value(); }

  /// <summary>
  ///     Returns an index into the <c>tuple</c> representing the element
  ///     selected by the <c>choice</c> messaging block.
  /// </summary>
  /// <returns>
  ///     The message index.
  /// </returns>
  ...
  /// </remarks>
  /**/
  size_t index() { return _M_pSingleAssignment->value(); }

  /// <summary>
  ///     Gets the message whose index was selected by the <c>choice</c>
  ///     messaging block.
  /// </summary>
  ...
  /// </returns>
  /// <remarks>
  ///     Because a <c>choice</c> messaging block can take inputs with different
  ///     payload types, you must specify the type of the payload at the point
  ///     of retrieval. You can determine the type based on the result of the
  ///     <c>index</c> method.
  /// </remarks>
  /**/
  template <typename _Payload_type> _Payload_type const &value() {
    return static_cast<_Reserving_node<_Payload_type> *>(
               _M_pSourceChoices[_M_pSingleAssignment->value()])
        ->value();
  }
  ...
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to an <c>ITarget</c> block to link to this <c>choice</c>
  ///     messaging block.
  /// </param>
  /**/
  ...
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to an <c>ITarget</c> block to unlink from this <c>choice</c>
  ///     messaging block.
  /// </param>
  /**/
  ...
  ///     destructor for the internal <c>single_assignment</c> block will unlink
  ///     properly.
  /// </remarks>
  /**/
  virtual void unlink_targets() { _M_pSingleAssignment->unlink_targets(); }

  /// <summary>
  ///     Accepts a message that was offered by this <c>choice</c> block,
  ///     transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  /// </returns>
  /**/
  virtual message<size_t> *accept(runtime_object_identity _MsgId,
                                  _Inout_ ITarget<size_t> *_PTarget) {
    return _M_pSingleAssignment->accept(_MsgId, _PTarget);
  }

  /// <summary>
  ///     Reserves a message previously offered by this <c>choice</c> messaging
  ///     block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     Consumes a message previously offered by this <c>choice</c> messaging
  ///     block and successfully reserved by the target, transferring ownership
  ///     to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the reserved <c>message</c>
  ...
  /// </remarks>
  /**/
  virtual message<size_t> *consume(runtime_object_identity _MsgId,
                                   _Inout_ ITarget<size_t> *_PTarget) {
  ...
  /// <summary>
  ///     Acquires a reference count on this <c>choice</c> messaging block, to
  ///     prevent deletion.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling this method.
...
private:
  template <int _Index>
  using _Reserving_node_source_type =
      _Reserving_node<typename ::std::remove_pointer_t<
          ::std::tuple_element_t<_Index, _Type>>::source_type>;

  /// <summary>
  ///     Constructs and initializes a _Reserving_node for each tuple messaging
  ///     block passed in.
  /// </summary>
  /// <typeparam>The highest-number index of the choice's sources</typeparam>
  /**/
  template <int _Index> void _Initialize_choices() {
    ::std::tuple_element_t<_Index, _Type> _Item =
        ::std::get<_Index>(_M_sourceTuple);
    _Reserving_node_source_type<_Index> *_Order_node_element = nullptr;

    if (_M_pScheduleGroup != nullptr) {
      _Order_node_element = new _Reserving_node_source_type<_Index>(
      ...
      _Order_node_element =
          new _Reserving_node_source_type<_Index>(_Item, _Index);
    }
  ...
  /// <summary>
  ///     Provides a sentinel template specialization for _Initialize_choices
  ...
  /// <summary>
  ///     Deletes all _Reserving_node elements that were created in
  ///     _Initialize_choices.
  /// </summary>
  /// <typeparam>The highest-number index of the choice's sources</typeparam>
  /**/
  template <int _Index> void _Delete_choices() {
    delete static_cast<_Reserving_node_source_type<_Index> *>(
        _M_pSourceChoices[_Index]);
    _M_pSourceChoices[_Index] = nullptr;
    _Delete_choices<_Index + 1>();
  }

  /// <summary>
  ///     Provides a sentinel template specialization for _Delete_choices
  ///     recursive template expansion.
  /// </summary>
  /**/
...
#ifdef _CRT_USE_WINAPI_FAMILY_DESKTOP_APP
/// <summary>
///     Constructs a <c>choice</c> messaging block from an optional
///     <c>Scheduler</c> or <c>ScheduleGroup</c> and two or more input sources.
/// </summary>
...
make_choice(Scheduler &_PScheduler, _Type1 _Item1, _Type2 _Item2,
            _Types... _Items) {
  return choice<::std::tuple<_Type1, _Type2, _Types...>>(
      _PScheduler, ::std::make_tuple(_Item1, _Item2, _Items...));
}
...
make_choice(ScheduleGroup &_PScheduleGroup, _Type1 _Item1, _Type2 _Item2,
            _Types... _Items) {
  return choice<::std::tuple<_Type1, _Type2, _Types...>>(
      _PScheduleGroup, ::std::make_tuple(_Item1, _Item2, _Items...));
}
...
/// </typeparam>
/**/
template <typename... _Types> struct _Unwrap<::std::tuple<_Types...>> {
  typedef ::std::tuple<typename ::std::remove_pointer_t<_Types>::source_type...>
      type;
};
...
class _Join_node
    : public propagator_block<single_link_registry<ITarget<_Destination_type>>,
                              multi_link_registry<ISource<size_t>>> {
private:
...
public:
  ...
  /// <summary>
  ///     Constructs a join within the default scheduler, and places it on any
  ///     schedule group of the scheduler's choosing.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     Constructs a join within the specified scheduler, and places it on any
  ///     schedule group of the scheduler's choosing.
  /// </summary>
  /// <param name="_PScheduler">
  ///     A reference to a scheduler instance.
  ...
  _Join_node(Scheduler &_PScheduler)
      : _M_counter(::std::tuple_size_v<_Destination_type>) {
    this->initialize_source_and_target(&_PScheduler);
  }

  /// <summary>
  ///     Constructs a join within the specified schedule group.  The scheduler
  ///     is implied by the schedule group.
  /// </summary>
  /// <param name="_PScheduleGroup">
  ///     A reference to a schedule group.
  /// </param>
...
protected:
  ...
  /// </returns>
  /**/
  virtual message_status propagate_message(message<size_t> *_PMessage,
                                           ISource<size_t> *_PSource) {
    // This join block is connected to the _Order_node sources, which know not
    // to send any more messages until join propagates them further. That is why
    // join can always accept the incoming messages.
  ...
  /// <summary>
  ///     Accepts an offered message by the source, transferring ownership to
  ///     the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The runtime object identity of the message.
  ...
  /// </returns>
  /**/
  virtual message<_Destination_type> *
  accept_message(runtime_object_identity _MsgId) {
    //
    // Peek at the head message in the message buffer.  If the IDs match
    // dequeue and transfer ownership
    //
  ...
  /// </returns>
  /// <remarks>
  ///     <c>consume_message</c> is similar to <c>accept</c>, but is always
  ///     preceded by a call to <c>reserve</c>.
  /// </remarks>
  /**/
  virtual message<_Destination_type> *
  consume_message(runtime_object_identity _MsgId) {
  ...
  /// </param>
  /// <remarks>
  ///     This function packages source payloads into a tuple message and
  ///     immediately sends it to the targets.
  /// </remarks>
  /**/
  virtual void
  propagate_to_any_targets(_Inout_opt_ message<_Destination_type> *) {
    message<_Destination_type> *_Msg = nullptr;
    ...
    if (_M_counter == 0) {
      ...
      if (!fIsNonGreedy) {
        ...
        _InterlockedExchange(&_M_counter,
                             ::std::tuple_size_v<_Destination_type>);
      }
...
private:
  /// <summary>
  ///     Tries to reserve from all sources.  If successful, it will consume all
  ///     the messages
  /// </summary>
  /// <returns>
  ...
  /**/
  template <int _Index>
  bool _Try_consume_source_messages(_Destination_type &_Destination_tuple,
                                    ISource<size_t> **_Sources) {
    typedef _Non_greedy_node<typename ::std::remove_pointer_t<
        ::std::tuple_element_t<_Index, _Type>>::source_type>
        _Non_greedy_node_source_type;
    _Non_greedy_node_source_type *_Node =
        static_cast<_Non_greedy_node_source_type *>(_Sources[_Index]);

    // Increment the counter once for each reservation
    _InterlockedIncrement(&_M_counter);

    if (_Node->_Reserve_received_message()) {
      bool _Ret_val = _Try_consume_source_messages<_Index + 1>(
          _Destination_tuple, _Sources);

      if (_Ret_val) {
        _Node->_Consume_received_message();
      } else {
        if (_Node->_Release_received_message()) {
          // If _Release_received_message() restored the ID, decrement the count
  ...
  /// <summary>
  ///     Provides a sentinel template specialization for
  ///     _Try_consume_source_messages recursive template expansion.
  /// </summary>
  /// <returns>
  ///     A bool indicating whether the consumption attempt worked.
  /// </returns>
  ...
  template <>
  bool
  _Try_consume_source_messages<::std::tuple_size_v<_Type>>(_Destination_type &,
                                                           ISource<size_t> **) {
    return true;
  }

  /// <summary>
  ///     Tries to acquire all of the messages from the _Non_greedy_nodes.  Each
  ///     node has already indicated that it has received a message that it can
  ///     try to reserve.  This function starts the reservation and consume
  ///     process.
  /// </summary>
  /// <returns>
  ///     A bool indicating whether the reserve/consume of all messages
  ///     succeeded.
  /// </returns>
  /**/
  bool _Non_greedy_acquire_messages() {
    ...
    bool _IsAcquireSuccessful =
        _Try_consume_source_messages<0>(_Destination_tuple, _Sources);
  ...
  /// <summary>
  ///     Called when all the source messaging blocks have received their
  ///     messages. The payloads are copied into local tuple and then packaged
  ///     into a message to be propagated: _M_pSendMessage.
  /// </summary>
  /**/
  ...
  /**/
  template <int _Index>
  void _Populate_destination_tuple(_Destination_type &_Destination_tuple,
                                   ISource<size_t> **_Sources) {
    typedef _Order_node_base<typename ::std::remove_pointer_t<
        ::std::tuple_element_t<_Index, _Type>>::source_type>
        _Order_node_base_source_type;
    _Order_node_base_source_type *_Node =
        static_cast<_Order_node_base_source_type *>(_Sources[_Index]);
  ...
  /// <summary>
  ///     Provides a sentinel template specialization for
  ///     _Populate_destination_tuple recursive template expansion.
  /// </summary>
  /**/
  template <>
  void
  _Populate_destination_tuple<::std::tuple_size_v<_Type>>(_Destination_type &,
                                                          ISource<size_t> **) {}

  // A tuple containing a collection of source messaging blocks
  _Type _M_sourceTuple;

  // Counts messages received by sources of this node and is used to trigger
  // propagation to targets This value starts at the total number of inputs and
  // counts down to zero.  When it reaches zero, a join of the inputs is
  // started.
...
/// <summary>
///     A <c>multitype_join</c> messaging block is a multi-source, single-target
///     messaging block that combines together messages of different types from
///     each of its sources and offers a tuple of the combined messages to its
///     targets.
/// </summary>
/// <typeparam name="_Type">
///     The <c>tuple</c> payload type of the messages joined and propagated by
///     the block.
/// </typeparam>
/// <typeparam name="_Jtype">
///     The kind of <c>join</c> block this is, either <c>greedy</c> or
...
public:
  ...
  /// </summary>
  /// <param name="_Tuple">
  ///     A <c>tuple</c> of sources for this <c>multitype_join</c> messaging
  ///     block.
  /// </param>
  /// <remarks>
  ///     <para>
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  explicit multitype_join(_Type _Tuple)
      : _M_sourceTuple(_Tuple), _M_pScheduler(nullptr),
        _M_pScheduleGroup(nullptr) {
  ...
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the propagation task for the
  ///     <c>multitype_join</c> messaging block is scheduled.
  /// </param>
  /// <param name="_Tuple">
  ///     A <c>tuple</c> of sources for this <c>multitype_join</c> messaging
  ///     block.
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  multitype_join(Scheduler &_PScheduler, _Type _Tuple)
      : _M_sourceTuple(_Tuple), _M_pScheduler(&_PScheduler),
        _M_pScheduleGroup(nullptr) {
    _M_pJoinNode =
        new _Join_node<_Type, _Destination_type, _Jtype>(_PScheduler);
    _Initialize_joins<0>();
  }
  ...
  /// <param name="_PScheduleGroup">
  ///     The <c>ScheduleGroup</c> object within which the propagation task for
  ///     the <c>multitype_join</c> messaging block is scheduled. The
  ///     <c>Scheduler</c> object used is implied by the schedule group.
  /// </param>
  /// <param name="_Tuple">
  ///     A <c>tuple</c> of sources for this <c>multitype_join</c> messaging
  ...
  /// <seealso cref="ScheduleGroup Class"/>
  /**/
  multitype_join(ScheduleGroup &_PScheduleGroup, _Type _Tuple)
      : _M_sourceTuple(_Tuple), _M_pScheduler(nullptr),
        _M_pScheduleGroup(&_PScheduleGroup) {
    _M_pJoinNode =
  ...
  /// <param name="_PTarget">
  ///     A pointer to an <c>ITarget</c> block to link to this
  ///     <c>multitype_join</c> messaging block.
  /// </param>
  /**/
  ...
  /// <summary>
  ///     Unlinks a target block from this <c>multitype_join</c> messaging
  ///     block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to an <c>ITarget</c> block to unlink from this
  ...
  /// </summary>
  /**/
  virtual void unlink_targets() { _M_pJoinNode->unlink_targets(); }

  /// <summary>
  ///     Accepts a message that was offered by this <c>multitype_join</c>
  ///     block, transferring ownership to the caller.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the offered <c>message</c>
  ...
  /// <summary>
  ///     Reserves a message previously offered by this <c>multitype_join</c>
  ///     messaging block.
  /// </summary>
  /// <param name="_MsgId">
  ///     The <c>runtime_object_identity</c> of the <c>message</c> object being
  ...
  /// <summary>
  ///     Consumes a message previously offered by the <c>multitype_join</c>
  ///     messaging block and successfully reserved by the target, transferring
  ...
  /// <summary>
  ///     Acquires a reference count on this <c>multitype_join</c> messaging
  ...
  /// <summary>
  ///     Releases a reference count on this <c>multiple_join</c> messaging
  ///     block.
  /// </summary>
  /// <param name="_PTarget">
  ///     A pointer to the target block that is calling this method.
...
private:
  template <int _Index>
  using _Source_type = typename ::std::remove_pointer_t<
      ::std::tuple_element_t<_Index, _Type>>::source_type;
  ...
  /// <summary>
  ///     Constructs and initializes a _Order_node for each tuple messaging
  ///     block passed in.
  /// </summary>
  /// <typeparam name="_Index">
  ...
  /// </typeparam>
  /**/
  template <int _Index> void _Initialize_joins() {
    ::std::tuple_element_t<_Index, _Type> _Item =
        ::std::get<_Index>(_M_sourceTuple);
    _Order_node_base_source_type<_Index> *_Order_node_element = nullptr;

    if (_Jtype == non_greedy) {
      typedef _Non_greedy_node<_Source_type<_Index>>
          _Non_greedy_node_source_type;

      if (_M_pScheduleGroup != nullptr) {
        _Order_node_element =
            new _Non_greedy_node_source_type(*_M_pScheduleGroup, _Item, _Index);
      } else if (_M_pScheduler != nullptr) {
        _Order_node_element =
            new _Non_greedy_node_source_type(*_M_pScheduler, _Item, _Index);
      } else {
      ...
      if (_M_pScheduleGroup != nullptr) {
        _Order_node_element =
            new _Greedy_node_source_type(*_M_pScheduleGroup, _Item, _Index);
      } else if (_M_pScheduler != nullptr) {
        _Order_node_element =
            new _Greedy_node_source_type(*_M_pScheduler, _Item, _Index);
      } else {
  ...
  /// <summary>
  ///     Provides a sentinel template specialization for _Initialize_joins
  ///     recursive template expansion.
  /// </summary>
  /**/
  ...
  /// <summary>
  ///     Deletes all _Order_node elements that were created in
  ///     _Initialize_joins.
  /// </summary>
  /// <typeparam name="_Index">
  ///     The highest-number index of the multitype_join's sources
  /// </typeparam>
  /**/
  template <int _Index> void _Delete_joins() {
    delete static_cast<_Order_node_base_source_type<_Index> *>(
        _M_pSourceJoins[_Index]);
    _M_pSourceJoins[_Index] = nullptr;
    _Delete_joins<_Index + 1>();
  }

  /// <summary>
  ///     Provides a sentinel template specialization for _Delete_joins
  ///     recursive template expansion.
  /// </summary>
  /**/
...
private:
  ...
  // Hide assignment operator
  //
  multitype_join const &
  operator=(multitype_join const &);      // no assignment operator
...
#ifdef _CRT_USE_WINAPI_FAMILY_DESKTOP_APP
/// <summary>
///     Constructs a <c>non_greedy multitype_join</c> messaging block from an
///     optional <c>Scheduler</c> or <c>ScheduleGroup</c> and two or more input
///     sources.
/// </summary>
/// <typeparam name="_Type1">
///     The message block type of the first source.
...
/// </param>
/// <returns>
///     A <c>non_greedy multitype_join</c> message block with two or more input
///     sources.
/// </returns>
/// <seealso cref="multitype_join Class"/>
/// <seealso cref="Scheduler Class"/>
...
make_join(Scheduler &_PScheduler, _Type1 _Item1, _Type2 _Item2,
          _Types... _Items) {
  return multitype_join<::std::tuple<_Type1, _Type2, _Types...>>(
      _PScheduler, ::std::make_tuple(_Item1, _Item2, _Items...));
}
...
#ifdef _CRT_USE_WINAPI_FAMILY_DESKTOP_APP
/// <summary>
///     Constructs a <c>greedy multitype_join</c> messaging block from an
///     optional <c>Scheduler</c> or <c>ScheduleGroup</c> and two or more input
///     sources.
/// </summary>
/// <typeparam name="_Type1">
...
/// </param>
/// <returns>
///     A <c>greedy multitype_join</c> message block with two or more input
///     sources.
/// </returns>
/// <seealso cref="multitype_join Class"/>
/// <seealso cref="Scheduler Class"/>
...
make_greedy_join(Scheduler &_PScheduler, _Type1 _Item1, _Type2 _Item2,
                 _Types... _Items) {
  return multitype_join<::std::tuple<_Type1, _Type2, _Types...>, greedy>(
      _PScheduler, ::std::make_tuple(_Item1, _Item2, _Items...));
}
...
enum agent_status {
  ...
  agent_created,
  /// <summary>
  ///     The <c>agent</c> has been started, but not entered its <c>run</c>
  ///     method.
  /// </summary>
  /**/
  agent_runnable,
...
/// <summary>
///     A class intended to be used as a base class for all independent agents.
///     It is used to hide state from other agents and interact using
///     message-passing.
/// </summary>
/// <remarks>
///     For more information, see <see cref="Asynchronous Agents"/>.
...
public:
  ...
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ///     <paramref name="_PScheduler"/> or <paramref name="_PGroup"/>
  ///     parameters.
  /// </remarks>
  /// <seealso cref="Scheduler Class"/>
  /// <seealso cref="ScheduleGroup Class"/>
  ...
  /// </summary>
  /// <param name="_PScheduler">
  ///     The <c>Scheduler</c> object within which the execution task of the
  ///     agent is scheduled.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ...
  /// </summary>
  /// <param name="_PGroup">
  ///     The <c>ScheduleGroup</c> object within which the execution task of the
  ///     agent is scheduled. The <c>Scheduler</c> object used is implied by the
  ///     schedule group.
  /// </param>
  /// <remarks>
  ///     The runtime uses the default scheduler if you do not specify the
  ...
  /// </summary>
  /// <remarks>
  ///     It is an error to destroy an agent that is not in a terminal state
  ///     (either <c>agent_done</c> or <c>agent_canceled</c>). This can be
  ///     avoided by waiting for the agent to reach a terminal state in the
  ///     destructor of a class that inherits from the <c>agent</c> class.
  /// </remarks>
  /**/
  ...
  /// </summary>
  /// <returns>
  ///     Returns a message source that can send messages about the current
  ///     state of the agent.
  /// </returns>
  /**/
  ...
  /// </summary>
  /// <returns>
  ///     Returns the current state of the agent.  Note that this returned state
  ///     could change immediately after being returned.
  /// </returns>
  ...
  /// <summary>
  ///     Moves an agent from the <c>agent_created</c> state to the
  ///     <c>agent_runnable</c> state, and schedules it for execution.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the agent started correctly, <c>false</c> otherwise. An
  ///     agent that has been canceled cannot be started.
  /// </returns>
  /// <seealso cref="agent_status Enumeration"/>
  /**/
  ...
  /// <summary>
  ///     Moves an agent from either the <c>agent_created</c> or
  ///     <c>agent_runnable</c> states to the <c>agent_canceled</c> state.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the agent was canceled, <c>false</c> otherwise. An
  ///     agent cannot be canceled if it has already started running or has
  ///     already completed.
  /// </returns>
  /// <seealso cref="agent_status Enumeration"/>
  /**/
  ...
  /// </param>
  /// <returns>
  ///     The <c>agent_status</c> of the agent when the wait completes. This can
  ///     either be <c>agent_canceled</c> or <c>agent_done</c>.
  /// </returns>
  /// <remarks>
  ///     An agent task is completed when the agent enters the
  ///     <c>agent_canceled</c> or <c>agent_done</c> states. <para>If the
  ///     parameter <paramref name="_Timeout"/> has a value other than the
  ///     constant <c>COOPERATIVE_TIMEOUT_INFINITE</c>, the exception <see
  ///     cref="operation_timed_out Class">operation_timed_out</see> is thrown
  ///     if the specified amount of time expires before the agent has completed
  ///     its task.</para>
  /// </remarks>
  ...
  /// <seealso cref="agent_status Enumeration"/>
  /**/
  _CONCRTIMP static agent_status __cdecl
  wait(_Inout_ agent *_PAgent,
       unsigned int _Timeout = COOPERATIVE_TIMEOUT_INFINITE);
  ...
  /// </summary>
  /// <param name="_Count">
  ///     The number of agent pointers present in the array <paramref
  ///     name="_PAgents"/>.
  /// </param>
  /// <param name="_PAgents">
  ...
  /// </param>
  /// <param name="_PStatus">
  ///     A pointer to an array of agent statuses. Each status value will
  ///     represent the status of the corresponding agent when the method
  ///     returns.
  /// </param>
  /// <param name="_Timeout">
  ///     The maximum time for which to wait, in milliseconds.
  ...
  wait_for_all(size_t _Count, _In_reads_(_Count) agent **_PAgents,
               _Out_writes_opt_(_Count) agent_status *_PStatus = nullptr,
               unsigned int _Timeout = COOPERATIVE_TIMEOUT_INFINITE);
  ...
  /**/
  _CONCRTIMP static void __cdecl
  wait_for_one(size_t _Count, _In_reads_(_Count) agent **_PAgents,
               agent_status &_Status, size_t &_Index,
               unsigned int _Timeout = COOPERATIVE_TIMEOUT_INFINITE);

protected:
  /// <summary>
  ///     Represents the main task of an agent. <c>run</c> should be overridden
  ///     in a derived class, and specifies what the agent should do after it
  ///     has been started.
  /// </summary>
  /// <remarks>
  ///     The agent status is changed to <c>agent_started</c> right before this
  ///     method is invoked. The method should invoke <c>done</c> on the agent
  ///     with an appropriate status before returning, and may not throw any
  ///     exceptions.
  /// </remarks>
  /**/
  virtual void run() = 0;
  ...
  /// <summary>
  ///     Moves an agent into the <c>agent_done</c> state, indicating that the
  ///     agent has completed.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the agent is moved to the <c>agent_done</c> state,
  ///     <c>false</c> otherwise. An agent that has been canceled cannot be
  ///     moved to the <c>agent_done</c> state.
  /// </returns>
  /// <remarks>
  ///     This method should be called at the end of the <c>run</c> method, when
  ///     you know the execution of your agent has completed.
  /// </remarks>
  /// <seealso cref="agent_status Enumeration"/>
  /**/
...
private:
  ...
  // This is initialized to TRUE and there is a race between Start() and
  // Cancel() to set it to FALSE.  Once Started or Canceled, further calls to
  // Start() or Cancel() will return false.
  /**/
  volatile long _M_fStartable;

  // A flag to check of whether the agent can be canceled
  // This is initialized to TRUE and there is a race between Cancel() and the
  // LWT executing a task that has been started to set it to FALSE.  If Cancel()
  // wins, the task will not be executed.  If the LWT wins, Cancel() will return
  // false.
  /**/
  volatile long _M_fCancelable;

  // A static wrapper function that calls the Run() method.  Used for scheduling
  // of the task
  /**/
...
/// <summary>
///     Associates the given name to the message block or agent in the ETW
///     trace.
/// </summary>
/// <typeparam name="_Type">
...
/// </typeparam>
/// <param name="_PObject">
///     A pointer to the message block or agent that is being named in the
///     trace.
/// </param>
/// <param name="_Name">
...
/// </param>
template <class _Type>
void Trace_agents_register_name(_Inout_ _Type *_PObject,
                                _In_z_ const wchar_t *_Name) {
  _Trace_agents(AGENTS_EVENT_NAME,
                ::Concurrency::details::_Trace_agents_get_id(_PObject), _Name);
}
