...
// explicitly or implicitly.  Therefore, this header also has several
// definitions that are used throughout the CRT.
//
#pragma once
#define _INC_VADEFS
...
// C4339: '__type_info_node': use of undefined type detected in CLR meta-data
// (/Wall)
...
// Use _VCRUNTIME_EXTRA_DISABLED_WARNINGS to add additional warning suppressions
// to VCRuntime headers.
...
#define _VCRUNTIME_DISABLED_WARNINGS                                           \
  ...
  _VCRUNTIME_DISABLED_WARNING_4412 4514 4820 _VCRUNTIME_EXTRA_DISABLED_WARNINGS
#endif
...
#define _APALIGN(t, ap) (__alignof(t))
    ...
    (defined _M_CEE && !defined _M_ARM && !defined _M_ARM64)
...
#define __crt_va_start_a(ap, v)                                                \
  ((void)(__va_start(&ap, _ADDRESSOF(v), _SLOTSIZEOF(v), __alignof(v),         \
                     _ADDRESSOF(v))))
#define __crt_va_arg(ap, t)                                                    \
  (*(t *)__va_arg(&ap, _SLOTSIZEOF(t), _APALIGN(t, ap), (t *)0))
...
void __cdecl __va_start(va_list *, ...);
#define __crt_va_start_a(ap, v)                                                \
  ((void)(__va_start(&ap, _ADDRESSOF(v), _SLOTSIZEOF(v), _ADDRESSOF(v))))
#else
#define __crt_va_start_a(ap, v)                                                \
  ((void)(ap = (va_list)_ADDRESSOF(v) + _SLOTSIZEOF(v)))
#endif
...
#define __crt_va_start_a(ap, v)                                                \
  ((void)(__va_start(&ap, _ADDRESSOF(v), _SLOTSIZEOF(v), __alignof(v),         \
                     _ADDRESSOF(v))))
// a hybrid va arg, to account for the shift in calling convention, with the
// alignment of ARM64
#define __crt_va_arg(ap, t)                                                    \
...
template <typename _Ty> struct __vcrt_assert_va_start_is_not_reference {
  static_assert(!__vcrt_va_list_is_reference<_Ty>::__the_value,
                "va_start argument must not have reference type and must not "
                "be parenthesized");
};
} // extern "C++"
