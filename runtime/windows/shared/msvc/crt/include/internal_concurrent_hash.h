...
public:
  ...
  _Hash_compare(hasher _Hasharg, _Key_equality _Keyeqarg)
      : _M_hash_object(_Hasharg), _M_key_compare_object(_Keyeqarg) {}
...
// An efficient implementation of the _Reverse function utilizes a 2^8 or 2^16
// lookup table holding the bit-reversed values of [0..2^8 - 1] or [0..2^16 - 1]
// respectively. Those values can also be computed on the fly at a slightly
// higher cost.
...
inline unsigned char _Reverse_byte(unsigned char _Original_byte) {
  // return ((_Original_byte * 0x80200802ULL) & 0x0884422110ULL) *
  // 0x0101010101ULL >> 32;
...
template <typename _Traits> class _Concurrent_hash : public _Traits {
public:
  // Type definitions
  typedef _Concurrent_hash<_Traits> _Mytype;
  typedef typename _Split_ordered_list<typename _Traits::value_type,
  ...
  typedef ::std::conditional_t<::std::is_same_v<key_type, value_type>,
  ...
  typedef typename _Al_traits::template rebind_traits<_Full_iterator>
      _Alfi_traits;

  static constexpr size_type _Initial_bucket_number =
      8; // Initial number of buckets
  static constexpr size_type _Initial_bucket_load =
      4; // Initial maximum number of elements per bucket
  static constexpr size_type _Pointers_per_table =
      sizeof(size_type) * 8; // One bucket segment per bit

  // Constructors/Destructors
  _Concurrent_hash(size_type _Number_of_buckets = _Initial_bucket_number,
                   const _Key_compare &_Parg = _Key_compare(),
                   const allocator_type &_Allocator = allocator_type())
      : _Traits(_Parg), _M_split_ordered_list(_Allocator),
        _M_allocator(_Allocator), _M_number_of_buckets(_Number_of_buckets),
        _M_maximum_bucket_size((float)_Initial_bucket_load) {
    _Init();
  }

  _Concurrent_hash(const _Concurrent_hash &_Right,
                   const allocator_type &_Allocator)
      : _Traits(_Right._M_comparator), _M_split_ordered_list(_Allocator),
        _M_allocator(_Allocator) {
    _Copy(_Right);
  }

  _Concurrent_hash(const _Concurrent_hash &_Right)
      : _Traits(_Right._M_comparator),
        _M_split_ordered_list(_Right.get_allocator()),
        _M_allocator(_Right.get_allocator()) {
    _Init();
    _Copy(_Right);
  }
  ...
  _Concurrent_hash(_Concurrent_hash &&_Right)
      : _Traits(_Right._M_comparator),
        _M_split_ordered_list(_Right.get_allocator()),
        _M_allocator(_Right.get_allocator()),
        _M_number_of_buckets(_Initial_bucket_number),
        _M_maximum_bucket_size((float)_Initial_bucket_load) {
    _Init();
    swap(_Right);
  }
  ...
  /// <summary>
  ///     Returns the stored allocator object for this concurrent container.
  ...
  /// <summary>
  ///     Tests whether no elements are present. This method is concurrency
  ///     safe.
  /// </summary>
  /// <remarks>
  ///     In the presence of concurrent inserts, whether or not the concurrent
  ///     container is empty may change immediately after calling this function,
  ///     before the return value is even read.
  /// </remarks>
  /// <returns>
  ///     <c>true</c> if the concurrent container is empty, <c>false</c>
  ///     otherwise.
  /// </returns>
  /**/
  bool empty() const { return _M_split_ordered_list.empty(); }

  /// <summary>
  ///     Returns the number of elements in this concurrent container. This
  ///     method is concurrency safe.
  /// </summary>
  /// <remarks>
  ///     In the presence of concurrent inserts, the number of elements in the
  ///     concurrent container may change immediately after calling this
  ///     function, before the return value is even read.
  /// </remarks>
  /// <returns>
  ///     The number of items in the container.
  /// </returns>
  /**/
  size_type size() const { return _M_split_ordered_list.size(); }

  /// <summary>
  ///     Returns the maximum size of the concurrent container, determined by
  ///     the allocator. This method is concurrency safe.
  /// </summary>
  /// <remarks>
  ///     This upper bound value may actually be higher than what the container
  ///     can actually hold.
  /// </remarks>
  /// <returns>
  ///     The maximum number of elements that can be inserted into this
  ///     concurrent container.
  /// </returns>
  /**/
  size_type max_size() const { return _M_split_ordered_list.max_size(); }

  /// <summary>
  ///     Returns an iterator pointing to the first element in the concurrent
  ///     container. This method is concurrency safe.
  /// </summary>
  /// <returns>
  ///     An iterator to the first element in the concurrent container.
  /// </returns>
  /**/
  iterator begin() { return _M_split_ordered_list.begin(); }
  ...
  /// </returns>
  /**/
  const_iterator begin() const { return _M_split_ordered_list.begin(); }

  /// <summary>
  ///     Returns an iterator pointing to the location succeeding the last
  ///     element in the concurrent container. This method is concurrency safe.
  /// </summary>
  /// <returns>
  ///     An iterator to the location succeeding the last element in the
  ///     concurrent container.
  /// </returns>
  /**/
  iterator end() { return _M_split_ordered_list.end(); }

  /// <summary>
  ///     Returns a const_iterator pointing to the location succeeding the last
  ///     element in the concurrent container. This method is concurrency safe.
  /// </summary>
  /// <returns>
  ///     A const_iterator to the location succeeding the last element in the
  ///     concurrent container.
  /// </returns>
  /**/
  const_iterator end() const { return _M_split_ordered_list.end(); }

  /// <summary>
  ///     Returns a const iterator pointing to the first element in the
  ///     concurrent container. This method is concurrency safe.
  /// </summary>
  /// <returns>
  ///     A const iterator to the first element in the concurrent container.
  /// </returns>
  /**/
  const_iterator cbegin() const { return _M_split_ordered_list.cbegin(); }

  /// <summary>
  ///     Returns a const iterator pointing to the location succeeding the last
  ///     element in the concurrent container. This method is concurrency safe.
  /// </summary>
  /// <returns>
  ///     A const iterator to the location succeeding the last element in the
  ///     concurrent container.
  /// </returns>
  /**/
  const_iterator cend() const { return _M_split_ordered_list.cend(); }

  /// <summary>
  ///     Removes elements from the container at specified positions. This
  ...
  ///     function removes the elements in the range [<paramref name="_First"/>,
  ///     <paramref name="_Last"/>). <para>The third member function removes the
  ///     elements in the range delimited by equal_range(<paramref
  ///     name="_Keyval"/>)</see>. </para>
  /// </remarks>
  /// <returns>
  ...
  ///     such element exists. The third member function returns the number of
  ///     elements it removes.
  /// </returns>
  /**/
  ...
  /// <summary>
  ///     Swaps the contents of two concurrent containers. This function is not
  ///     concurrency safe.
  /// </summary>
  /// <param name="_Right">
  ...
  /// </param>
  /// <remarks>
  ///     The member function throws an <see cref="invalid_argument
  ///     Class">invalid_argument</see> exception if the swap is being performed
  ///     on unequal allocators.
  /// </remarks>
  /**/
  ...
  /// <summary>
  ///     Erases all the elements in the concurrent container. This function is
  ...
  /// <summary>
  ///     Finds an element that matches a specified key. This function is
  ///     concurrency safe.
  /// </summary>
  /// <param name="_Keyval">
  ...
  /// </param>
  /// <returns>
  ///     An iterator pointing to the location of the first element that matched
  ///     the key provided, or the iterator <c>end()</c> if no such element
  ///     exists.
  /// </returns>
  /**/
  iterator find(const key_type &_Keyval) { return _Find(_Keyval); }
  ...
  /// </returns>
  /**/
  const_iterator find(const key_type &_Keyval) const { return _Find(_Keyval); }

  /// <summary>
  ///     Counts the number of elements matching a specified key. This function
  ///     is concurrency safe.
  /// </summary>
  /// <param name="_Keyval">
  ...
  size_type count(const key_type &_Keyval) const {
    ...
    for (; _It != end() &&
           !this->_M_comparator(this->_Key_function(*_It), _Keyval);
         ++_It) {
      ++_Count;
    }
    return _Count;
  ...
  /// <summary>
  ///     Finds a range that matches a specified key. This function is
  ///     concurrency safe.
  /// </summary>
  /// <param name="_Keyval">
  ///     The key value to search for.
  /// </param>
  /// <returns>
  ///     A <see cref="pair Class">pair</see> where the first element is an
  ///     iterator to the beginning and the second element is an iterator to the
  ///     end of the range.
  /// </returns>
  /// <remarks>
  ///     <para>It is possible for concurrent inserts to cause additional keys
  ///     to be inserted after the begin iterator and before the end
  ///     iterator.</para>
  /// </remarks>
  /**/
  ...
  /// </returns>
  /**/
  size_type unsafe_bucket_count() const { return _M_number_of_buckets; }
  ...
  size_type unsafe_bucket_size(size_type _Bucket) {
    ...
    for (; _Iterator != _M_split_ordered_list._End() &&
           !_Iterator._Ptr->_Is_dummy();
         ++_Iterator) {
      ++_Count;
    }
  ...
  /// <summary>
  ///     Returns the bucket index that a specific key maps to in this
  ///     container.
  /// </summary>
  /// <param name="_Keyval">
  ...
  size_type unsafe_bucket(const key_type &_Keyval) const {
    _Split_order_key _Order_key =
        (_Split_order_key)this->_M_comparator(_Keyval);
    size_type _Bucket = _Order_key % _M_number_of_buckets;
    return _Bucket;
  }

  /// <summary>
  ///     Returns an iterator to the first element in this container for a
  ///     specific bucket.
  /// </summary>
  /// <param name="_Bucket">
  ///     The bucket index.
  ...
  /// <summary>
  ///     Returns an iterator to the last element in this container for a
  ///     specific bucket.
  /// </summary>
  /// <param name="_Bucket">
  ///     The bucket index.
  ...
  local_iterator unsafe_end(size_type _Bucket) {
    // If we've increased the number of buckets, there's a chance the
    // intermediate dummy node marking the end of this bucket has not yet been
    // lazily initialized. Inserting from _M_number_of_buckets/2 to
    // _M_number_of_buckets will recursively initialize all the dummy nodes in
    // the map.
    for (size_type _Bucket_index = _M_number_of_buckets >> 1;
         _Bucket_index < _M_number_of_buckets; ++_Bucket_index) {
    ...
    do {
      ++_Iterator;
    } while (_Iterator != _M_split_ordered_list._End() &&
             !_Iterator._Ptr->_Is_dummy());
  ...
  /// <summary>
  ///     Computes and returns the current load factor of the container. The
  ///     load factor is the number of elements in the container divided by the
  ///     number of buckets.
  /// </summary>
  /// <returns>
  ...
  /// <summary>
  ///     Gets or sets the maximum load factor of the container. The maximum
  ///     load factor is the largest number of elements than can be in any
  ///     bucket before the container grows its internal table.
  /// </summary>
  /// <returns>
  ///     The first member function returns the stored maximum load factor. The
  ///     second member function does not return a value but throws an <see
  ///     cref="out_of_range Class">out_of_range</see> exception if the supplied
  ///     load factor is invalid..
  /// </returns>
  /**/
  float max_load_factor() const { return _M_maximum_bucket_size; }
  ...
  /// </param>
  /// <remarks>
  ///     The member function alters the number of buckets to be at least
  ///     <paramref name="_Buckets"/> and rebuilds the hash table as needed. The
  ///     number of buckets must be a power of 2. If not a power of 2, it will
  ///     be rounded up to the next largest power of 2. <para>It throws an <see
  ///     cref="out_of_range Class">out_of_range</see> exception if the number
  ///     of buckets is invalid (either 0 or greater than the maximum number of
  ///     buckets).</para>
  /// </remarks>
  /**/
...
protected:
  ...
  ::std::pair<iterator, bool> _Insert(_ValTy &&_Value) {
    _Split_order_key _Order_key =
        (_Split_order_key)this->_M_comparator(this->_Key_function(_Value));
    size_type _Bucket = _Order_key % _M_number_of_buckets;
    ...
    _Nodeptr _New_node = _M_split_ordered_list._Buynode(
        _Order_key, ::std::forward<_ValTy>(_Value));
    ...
    for (;;) {
      if (_Where == _Last || _Mylist::_Get_key(_Where) > _Order_key) {
        ...
        ::std::pair<iterator, bool> _Result = _M_split_ordered_list._Insert(
            _Iterator, _Where, _New_node, &_New_count);
        ...
        if (_Result.second) {
          ...
          // Insertion failed: either the same node was inserted by another
          // thread, or another element was inserted at exactly the same place
          // as this node. Proceed with the search from the previous location
          // where order key was known to be larger (note: this is legal only
          // because there is no safe concurrent erase operation supported).
          _Where = _Iterator;
          ++_Where;
          continue;
        }
      } else if (!this->_M_allow_multimapping &&
                 _Mylist::_Get_key(_Where) == _Order_key &&
                 this->_M_comparator(this->_Key_function(*_Where),
                                     this->_Key_function(_New_node->_Myval)) ==
                     0) {
        ...
        return ::std::pair<iterator, bool>(
            _M_split_ordered_list._Get_iterator(_Where), false);
      }
  ...
  template <class _Iterator> void _Insert(_Iterator _First, _Iterator _Last) {
...
private:
  ...
  iterator _Find(const key_type &_Keyval) {
    ...
    for (_Full_iterator _Iterator = _Get_bucket(_Bucket); _Iterator != _Last;
         ++_Iterator) {
      if (_Mylist::_Get_key(_Iterator) > _Order_key) {
        ...
        // found. Key function comparison has to be performed to check whether
        // this is the right element. If not, keep searching while order key is
        // the same.
  ...
  const_iterator _Find(const key_type &_Keyval) const {
    ...
    // If _Bucket has not been initialized, keep searching up for a parent
    // bucket that has been initialized.  Worst case is the entire map will be
    // read.
    ...
    for (_Full_const_iterator _Iterator = _Get_bucket(_Bucket);
         _Iterator != _Last; ++_Iterator) {
      if (_Mylist::_Get_key(_Iterator) > _Order_key) {
        // If the order key is smaller than the current order key, the element
        // is not in the hash.
        return end();
  ...
  iterator _Erase(const_iterator _Iterator) {
    _Split_order_key _Order_key =
        (_Split_order_key)this->_M_comparator(this->_Key_function(*_Iterator));
    size_type _Bucket = _Order_key % _M_number_of_buckets;
    ...
    for (_Full_iterator _Iterator = _Get_bucket(_Bucket); _Iterator != _Last;
      ...
      if (_Mylist::_Get_key(_Iterator) > _Order_key) {
        ...
        return ::std::pair<iterator, iterator>(end(), end());
                 ...
                 !this->_M_comparator(this->_Key_function(*_Iterator),
                                      _Keyval)) {
        iterator _Begin = _M_split_ordered_list._Get_iterator(_Iterator);
        iterator _End = _Begin;

        for (; _End != end() &&
               !this->_M_comparator(this->_Key_function(*_End), _Keyval);
             ++_End) {
        }
  ...
  // This operation makes sense only if mapping is many-to-one.
  ::std::pair<const_iterator, const_iterator>
  _Equal_range(const key_type &_Keyval) const {
    _Split_order_key _Order_key =
        (_Split_order_key)this->_M_comparator(_Keyval);
    size_type _Bucket = _Order_key % _M_number_of_buckets;
  ...
  void _Initialize_bucket(size_type _Bucket) {
    ...
    _Full_iterator _Dummy_node = _M_split_ordered_list._Insert_dummy(
        _Parent, _Split_order_dummy_key(_Bucket));
  ...
  void _Adjust_table_size(size_type _Total_elements, size_type _Current_size) {
    ...
    if (((float)_Total_elements / (float)_Current_size) >
        _M_maximum_bucket_size) {
      // Double the size of the hash only if size has not changed inbetween
      // loads
      _InterlockedCompareExchangeSizeT(&_M_number_of_buckets, 2 * _Current_size,
                                       _Current_size);
    }
  }
  ...
  void _Set_bucket(size_type _Bucket, _Full_iterator _Dummy_head) {
    ...
    if (_M_buckets[_Segment] == nullptr) {
      ...
      ::std::_Uninitialized_value_construct_n(_New_segment, _Seg_size,
                                              _M_allocator);
      if (_InterlockedCompareExchangePointer(
              (void *volatile *)&_M_buckets[_Segment], _New_segment, nullptr) !=
          nullptr) {
  ...
  typename _Alfi_traits::allocator_type
      _M_allocator;               // Allocator object for segments
  size_type _M_number_of_buckets; // Current table size
  float _M_maximum_bucket_size;   // Maximum size of the bucket
};

#pragma warning(                                                               \
    pop) // Warning 4127 -- while (true) has a constant expression in it
