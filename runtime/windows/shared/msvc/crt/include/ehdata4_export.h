...
//
// Defines the on-disk data format and higher-level logical classes to interact
// with __CxxFrameHandler4 metadata. Valid EH state values are -1, 0 ...
// MAX_INT. States are always encoded as compressed integers which assumes
// positive values. Thus, all state values are encoded as EHstate + 1 to avoid
// encoding a negative (a state of -1 is encoded as a compressed 0, 0 as a
// compressed 1 etc.)
//
// The on-disk format is as follows; fields are listed in order as they would
// appear on the disk. When reading, fields that are not used means the next
// field that exists will be stored next:
//
// FuncInfo4: Top-level metadata structure for a function
//   header:           1 byte, bitfield fully defined in FuncInfoHeader
//   structure bbtFlags:         compressed int, only exists when header.BBT ==
//   1 dispUnwindMap:    4 bytes, RVA to UnwindMap which only exists when
//   header.UnwindMap == 1 dispTryBlockMap:  4 bytes, RVA to TryBlockMap which
//   only exists when header.TryBlockMap == 1 dispIPtoStateMap: 4 bytes, RVA to
//   IPtoStateMap OR SepIPtoStateMap that is always assumed to exist.
//                     Is RVA to IPtoStateMap if header.isSeparated == 0,
//                     otherwise RVA to SepIPtoStateMap.
...
//     nextOffset << 2 | UnwindEntryType: compressed int, nextOffset represents
//     the byte offset from the start of this entry to the previous entry.
//                                        Note that the entry for state -1
//                                        doesn't explicitly exist in the table
//                                        as there's never any unwind actions
//                                        associated with it. Instead entries
//                                        that go to state -1 encode an offset
//                                        that points to before the start of the
//                                        UnwindMapEntry4 buffer. See 'enum
//                                        Type' in UnwindMapEntry4 for a
//                                        description of what the different
//                                        UnwindEntryType represent.
//     action:                            4 bytes, RVA to the destructor action.
//     object:                            compressed int, offset to object
//     directly or to stack location containing its pointer depending on
//     UnwindEntryType.
//
...
//     tryHigh:              compressed int, final state of the try--the range
//     of try states is [tryLow, tryHigh] catchHigh:            compressed int,
//     final state of all catches--the range of catch states is (tryHigh,
//     catchHigh] RVA to Handler Array: 4 bytes, RVA to metadata about the
//     various catch handlers for this try
//
// HandlerMap4: Describes Catch(es) associated with a specific Try
//   NumEntries: compressed int, number of entries in the map
//   HandlerType4[numEntries]:
//     header:              1 byte, bitfield fully defined in HandlerTypeHeader
//     structure adjectives:          compressed int, only present if
//     header.adjectives == 1 dispType:            4 bytes, RVA to type
//     descriptor only present if header.dispType == 1 dispCatchObj: compressed
//     int, displacement from establisher from to catch object only present if
//     header.dispCatchObj == 1 dispOfHandler:       4 bytes, RVA to 'catch'
//     code continuationAddress: between 0 (no continuation) and up to 2
//     compressed ints (up to 2 continuation addresses),
//                          used by the runtime to figure out the next normal
//                          flow instruction to execute after the catch
//
...
//     Ip:    compressed int, function-relative AND delta-encoded from the
//     previous entry.
//            For example, entries of 0,5,15 map to 0,5,20 bytes from the start
//            of the function
//     State: compressed int, stored as the EHstate+1 to avoid encoding a
//     negative
//
// SepIPtoStateMap4: Used when code is separated out (e.g. POGO) and contains
// RVAs to IP2StateMap(s) for each function contribution
//  NumEntries: compressed int, number of entries in map
//  SepIPtoStateMapEntry4[NumEntries]:
//    addrStartRVA: 4 bytes, RVA to start address of function contribution, used
//    to differentiate between segments dispOfIPMap:  4 bytes, RVA to
//    IP2StateMap structure corresponding to the function contribution with this
//    addrStartRVA
//
// Header Usage:
// Reading in data:
//   DecompFuncInfo:                 entry-point that provides a FuncInfo4 used
//   to build other structures. UWMap4/TryBlockMap4:            build up
//   high-level representation of UnwindMap/TryBlockMap from FuncInfo4.
//   IPtoStateMap4/SepIPtoStateMap4: build up high-level representation of
//   IPtoStateMap/SepIPtoStateMap from FuncInfo4.
//                                   Note that SepIPtoStateMap4 has support for
//                                   non-separated IptoStateMap and can be used
//                                   for both cases.
//   HandlerMap4:                    build up high-level representation of
//   HandlerMap from a TryBlockMapEntry4 in a TryBlockMap4.
...
struct UnwindMapEntry4 {
  // To save space, we encode common dtor patterns that used to require a
  // compiler-generated stub to execute into an offset + RVA to the destructor
  // method itself.
  enum Type {
    NoUW = 0b00,             // No unwind action associated with this state
    DtorWithObj = 0b01,      // Dtor with an object offset
    DtorWithPtrToObj = 0b10, // Dtor with an offset that contains a pointer to
                             // the object to be destroyed
    RVA = 0b11 // Dtor that has a direct function that is called that knows
               // where the object is and can perform more exotic destruction
  };
  uint32_t
      nextOffset; // State this action takes us to (in offset form, unlike FH3!)
  Type type;      // Type of entry
  int32_t action; // Image-relative offset of action, exists for all NoUW entry
                  // types
  uint32_t object; // Frame offset of object pointer to be destroyed, exists for
                   // DtorWithObj and DtorWithPtrToObj types
};
...
struct HandlerTypeHeader {
  ...
  enum contType { NONE = 0b00, ONE = 0b01, TWO = 0b10, RESERVED = 0b11 };
  union {
#pragma warning(push)
#pragma warning(disable                                                        \
    ...
    struct {
      ...
      uint8_t dispType : 1;   // Existence of Image relative offset of the
                              // corresponding type descriptor
      uint8_t dispCatchObj
          : 1; // Existence of Displacement of catch object from base
      uint8_t contIsRVA : 1; // Continuation addresses are RVAs rather than
                             // function relative, used for separated code
      uint8_t contAddr
          : 2; // 1.   00: no continuation address in metadata, use what the
               // catch funclet returns
...
static_assert(sizeof(HandlerTypeHeader) == sizeof(uint8_t),
              "Size of HandlerTypeHeader not 1 Byte");

struct HandlerType4 {
  HandlerTypeHeader header;
  uint32_t adjectives; // Handler Type adjectives (bitfield)
  int32_t
      dispType; // Image relative offset of the corresponding type descriptor
  uint32_t dispCatchObj; // Displacement of catch object from base
  int32_t dispOfHandler; // Image relative offset of 'catch' code
  uintptr_t continuationAddress[MAX_CONT_ADDRESSES]; // Continuation address(es)
                                                     // of catch funclet
...
struct TryBlockMapEntry4 {
  ...
  int32_t dispHandlerArray; // Image relative offset of list of handlers for
                            // this try
};
...
struct SepIPtoStateMapEntry4 {
  ...
  int32_t
      dispOfIPMap; // RVA to IP map corresponding to this function contribution
};

// Note: with FH4 there is no magic number before FuncInfoHeader in the image.
// In runtime anything with __CxxFrameHandler4 handler is treated as having
// EH_MAGIC_NUMBER3. If we'll want to do any updates we won't have any way to
// mark a version. The only possibility would be to use 'reserved' bit for
// 'version', with 0 meaning current version, 1 for all future versions if
// needed.
...
#pragma warning(disable                                                        \
    ...
    struct {
      ...
      uint8_t isSeparated
          : 1; // 1 if this function has separated code segments, 0 otherwise
...
#pragma warning(pop)
  ...
  FuncInfoHeader() { value = 0; }
};

static_assert(sizeof(FuncInfoHeader) == sizeof(uint8_t),
              "Size of FuncInfoHeader not 1 Byte");
...
struct FuncInfo4 {
  ...
  uint32_t dispFrame;       // displacement of address of function frame wrt
                            // establisher frame, only used for catch funclets
...
inline uint32_t ReadUnsigned(uint8_t **pbEncoding) {
  ...
  uint32_t result =
      *(reinterpret_cast<uint32_t *>(*pbEncoding - negLength - 4));
...
// Read function info from the buffer. If the function is separated into
// segments (by PGO or by BBT) 'functionStart' is actually a segment start, not
// the start of the whole function. Find correct IPtoStateMap for that segment.
inline ptrdiff_t DecompFuncInfo(uint8_t *buffer, FuncInfo4 &FuncInfoDe,
                                uintptr_t imageBase, int32_t functionStart,
                                bool rawIP2StateRVA) {
...
namespace Compress {
static constexpr uint32_t maxRVAs =
    4; // From HandlerMap (dispType, handlerAddr, 2 continuation addresses)
static constexpr uint32_t maxBufferSize =
    27; // Maximum size of a single structure, from HandlerType
static constexpr uint32_t compIntSize =
    5; // Maximum compressed integer buffer size
struct RVAoffsets {
  uint32_t count;
...
inline uint32_t compFuncInfo4(FuncInfo4 funcInfo, buffer buffer,
                              RVAoffsets *offsets) {
  uint32_t index = 0;
  offsets->count = 0;
...
inline uint32_t compUWMapEntry(UnwindMapEntry4 UWEntry, buffer buffer,
                               RVAoffsets *offsets) {
  uint32_t index = 0;
  offsets->count = 0;
  // Pack type with next offset
  uint32_t fieldValue =
      (UWEntry.nextOffset << 2) | static_cast<uint32_t>(UWEntry.type);
  index += getNETencoded(&buffer[index], fieldValue);

  if (UWEntry.type == UnwindMapEntry4::Type::DtorWithObj ||
      UWEntry.type == UnwindMapEntry4::Type::DtorWithPtrToObj) {
...
// Assumes the Ip in ipMap has already been delta-encoded and made function
// relative. State passed in is the real state.
...
inline uint32_t compTryBlockMap(TryBlockMapEntry4 tryBlockEntry, buffer buffer,
                                RVAoffsets *offsets) {
  uint32_t index = 0;
  offsets->count = 0;
  ...
  *(reinterpret_cast<int32_t *>(&buffer[index])) =
      tryBlockEntry.dispHandlerArray;
  addOffset(offsets, index);
...
inline uint32_t compHandlerMap(HandlerType4 handlerEntry, buffer buffer,
                               RVAoffsets *offsets) {
  uint32_t index = 0;
  offsets->count = 0;
  ...
  *(reinterpret_cast<int32_t *>(&buffer[index])) = handlerEntry.dispOfHandler;
  ...
  if (handlerEntry.header.contIsRVA) {
    if (handlerEntry.header.contAddr == HandlerTypeHeader::contType::ONE) {
      *(reinterpret_cast<int32_t *>(&buffer[index])) =
          (int32_t)handlerEntry.continuationAddress[0];
      addOffset(offsets, index);
    } else if (handlerEntry.header.contAddr ==
               HandlerTypeHeader::contType::TWO) {
      *(reinterpret_cast<int32_t *>(&buffer[index])) =
          (int32_t)handlerEntry.continuationAddress[0];
      addOffset(offsets, index);

      *(reinterpret_cast<int32_t *>(&buffer[index])) =
          (int32_t)handlerEntry.continuationAddress[1];
      addOffset(offsets, index);
    } else {
    ...
    if (handlerEntry.header.contAddr == HandlerTypeHeader::contType::ONE) {
      index += getNETencoded(&buffer[index],
                             (uint32_t)handlerEntry.continuationAddress[0]);
    } else if (handlerEntry.header.contAddr ==
               HandlerTypeHeader::contType::TWO) {
      index += getNETencoded(&buffer[index],
                             (uint32_t)handlerEntry.continuationAddress[0]);
      index += getNETencoded(&buffer[index],
                             (uint32_t)handlerEntry.continuationAddress[1]);
    } else {
      // no encoded cont addresses or unknown
    }
  }
...
// High level representations for easy processing and traversal of low level
// structures
class UWMap4 {
public:
  UWMap4(const FuncInfo4 *pFuncInfo, uintptr_t imageBase) {
    if (pFuncInfo->dispUnwindMap != 0) {
      uint8_t *buffer =
          imageRelToByteBuffer(imageBase, pFuncInfo->dispUnwindMap);
  ...
  public:
    ...
    void WalkBack() { _UWMap.WalkBack(&_currEntry); }
  ...
  iterator begin() { return iterator(*this, _bufferStart); }
  ...
  void getStartStop(__ehstate_t start, __ehstate_t stop, iterator &startIter,
                    iterator &stopIter) {
  ...
  // Using known offsets/state values, find the state of an unknown iterator
  // known to be between lowState and highState
  static __ehstate_t getStateFromIterators(const iterator &lowStateIter,
                                           const __ehstate_t lowState,
                                           const iterator &highStateIter,
                                           const __ehstate_t highState,
                                           const iterator &toStateIter) {
    __ehstate_t state = -1;
  ...
  uint32_t getNumEntries() { return _numEntries; }
...
private:
  ...
  void ReadEntry(uint8_t **currOffset) {
    ...
    _UWEntry.type =
        static_cast<UnwindMapEntry4::Type>(_UWEntry.nextOffset & 0b11);
    _UWEntry.nextOffset >>= 2;

    if (_UWEntry.type == UnwindMapEntry4::Type::DtorWithObj ||
        _UWEntry.type == UnwindMapEntry4::Type::DtorWithPtrToObj) {
  ...
  void WalkForward(uint8_t **currOffset) { ReadEntry(currOffset); }
};
...
public:
  ...
  public:
    ...
    TryBlockMapEntry4 operator*() { return _tryBlockMap._tryBlock; }
    ...
    void incrementToSentinel() { _currBlock++; }
  ...
  iterator end() { return iterator(*this, _numTryBlocks); }

  uint32_t getNumTryBlocks() { return _numTryBlocks; }
...
class HandlerMap4 {
public:
  HandlerMap4(const TryBlockMapEntry4 *tryMap, uintptr_t imageBase,
              int32_t functionStart)
      : _imageBase(imageBase), _functionStart(functionStart) {
  ...
  public:
    ...
    HandlerType4 operator*() { return _handlerMap._handler; }
  ...
  iterator end() { return iterator(*this, _numHandlers); }
  ...
  void resetBuffer() { setBuffer(0); }

  uint32_t getNumHandlers() { return _numHandlers; }
...
private:
  ...
  void DecompHandler() {
    ...
    if (_handler.header.contIsRVA) {
      ...
      if (_handler.header.contAddr == HandlerTypeHeader::contType::ONE) {
        _handler.continuationAddress[0] =
            _functionStart + ReadUnsigned(&_buffer);
      } else if (_handler.header.contAddr == HandlerTypeHeader::contType::TWO) {
        _handler.continuationAddress[0] =
            _functionStart + ReadUnsigned(&_buffer);
        _handler.continuationAddress[1] =
            _functionStart + ReadUnsigned(&_buffer);
      } else {
        // no encoded cont addresses or unknown
      }
...
class IPtoStateMap4 {
public:
  IPtoStateMap4(const FuncInfo4 *pFuncInfo, uintptr_t imageBase,
                uint32_t funcStart)
      : _imageBase(imageBase), _funcStart(funcStart) {
    if (pFuncInfo->dispIPtoStateMap) {
      uint8_t *buffer =
          imageRelToByteBuffer(imageBase, pFuncInfo->dispIPtoStateMap);
      _numEntries = ReadUnsigned(&buffer);
      _bufferStart = buffer;
    } else {
      _numEntries = 0;
  ...
  class iterator {
  public:
    iterator(IPtoStateMap4 &IptoStateMap, uint32_t entry,
             uint8_t *offset = nullptr)
        : _IPtoStateMap(IptoStateMap), _entry(entry), _offset(offset),
          _prevIp(0) {}

    iterator &operator++() {
      IPtoStateMapEntry4 mapEntry =
          _IPtoStateMap.decompIP2State(&_offset, _prevIp);
      _entry++;
  ...
  iterator begin() { return iterator(*this, 0, _bufferStart); }

  iterator end() { return iterator(*this, _numEntries); }
...
public:
  SepIPtoStateMap4(const FuncInfo4 *pFuncInfo, uintptr_t imageBase,
                   int32_t functionStart)
      : _imageBase(imageBase) {
  ...
  class iterator {
  public:
    iterator(SepIPtoStateMap4 &SepIPtoStateMap, uint32_t entry,
             uint8_t *offset = nullptr)
        : _SepIPtoStateMap(SepIPtoStateMap), _entry(entry), _offset(offset) {}

    iterator &operator++() {
      SepIPtoStateMapEntry4 mapEntry =
          _SepIPtoStateMap.decompSepIp2State(&_offset);
      _entry++;
      return *this;
    }
