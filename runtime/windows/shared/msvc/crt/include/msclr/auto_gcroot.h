...
// Forward Declaration
template <typename _element_type> class auto_gcroot;
...
template <typename _element_type> struct _auto_gcroot_ref {
  // construct from compatible auto_gcroot
  _auto_gcroot_ref(auto_gcroot<_element_type> &ref) : m_ref(ref) {}
...
public:
  ...
  auto_gcroot(_element_type _ptr = nullptr) : m_ptr(_ptr) {}

  // construct by assuming pointer from _right auto_gcroot
  auto_gcroot(auto_gcroot<_element_type> &_right) : m_ptr(_right.release()) {}
  ...
  auto_gcroot(auto_gcroot<_other_type> &_right) : m_ptr(_right.release()) {}
  ...
  auto_gcroot<_element_type> &
  attach(_detail::_auto_gcroot_ref<_element_type> &_right) {
  ...
  // assign compatible _right.ref
  auto_gcroot<_element_type> &
  operator=(_detail::_auto_gcroot_ref<_element_type> &_right) {
    return attach(_right);
  }
  ...
  _element_type get() const { return m_ptr; }

  // return pointer to class object (assume pointer)
  _element_type operator->() const { return m_ptr; }
  ...
  template <typename _other_type> operator auto_gcroot<_other_type>() {
  ...
  void swap(auto_gcroot<_element_type> &_right) { m_ptr.swap(_right.m_ptr); }
