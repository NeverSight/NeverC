...
#define BEGIN_DELEGATE_MAP(CLASS)                                              \
  ...
  msclr::delegate_map::internal::delegate_proxy_factory<CLASS>                 \
      m_delegate_map_proxy;                                                    \
                                                                               \
  ref class delegate_proxy_type {                                              \
    CLASS *m_p_native_target;                                                  \
                                                                               \
  public:                                                                      \
    delegate_proxy_type(CLASS *pNativeTarget)                                  \
  ...
  void MEMBER(ARG0 arg0, ARG1 arg1) {                                          \
    if (m_p_native_target == NULL)                                             \
      throw gcnew System::ArgumentNullException(                               \
          "Delegate call failed: Native sink was not attached or has already " \
          "detached from the managed proxy (m_p_native_target == NULL). "      \
          "Hint: see if native sink was destructed or not constructed "        \
          "properly");                                                         \
                                                                               \
...
#define MAKE_DELEGATE(DELEGATE, MEMBER)                                        \
  gcnew DELEGATE(m_delegate_map_proxy.get_proxy(this),                         \
                 &delegate_proxy_type::MEMBER)
