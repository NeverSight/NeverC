...
#define _EXCEPTION_MB2WC                                                       \
  "Conversion from MultiByte to WideChar failed.  Please check the content "   \
  "of the string and/or locale settings."
#define _EXCEPTION_WC2MB                                                       \
  "Conversion from WideChar to MultiByte failed.  Please check the content "   \
  "of the string and/or locale settings."

// helper class to allocate/deallocate a buffer with new[]/delete[]
template <class T> class char_buffer {
public:
  explicit char_buffer(size_t _size) { _ptr = new T[_size]; }

  ~char_buffer() { delete[] _ptr; }
...
inline void WriteAnsiString(_Out_writes_all_(_size) _Post_z_ char *_buf,
                            size_t _size, System::String ^ _str) {
  cli::pin_ptr<const wchar_t> _pinned_ptr = PtrToStringChars(_str);
  ...
  if (_size > INT_MAX) {
    ...
    throw gcnew System::ArgumentOutOfRangeException(
        _EXCEPTION_GREATER_THAN_INT_MAX);
  }

  const auto _written = static_cast<size_t>(::WideCharToMultiByte(
      CP_THREAD_ACP, WC_NO_BEST_FIT_CHARS, _pinned_ptr, _str->Length, _buf,
      static_cast<int>(_size), NULL, NULL));
  if (_written >= _size ||
...
inline _Check_return_ size_t GetUnicodeStringSize(_In_reads_z_(_count + 1)
                                                      const char *_str,
                                                  size_t _count) {
  if (_count > INT_MAX) {
    throw gcnew System::ArgumentOutOfRangeException(
  ...
  auto _size = static_cast<size_t>(::MultiByteToWideChar(
      CP_THREAD_ACP, 0, _str, static_cast<int>(_count), NULL, 0));
  if (_size == 0 && _count != 0) {
...
inline void WriteUnicodeString(_Out_writes_all_(_size) _Post_z_ wchar_t *_dest,
                               size_t _size,
  ...
  size_t _written = static_cast<size_t>(
      ::MultiByteToWideChar(CP_THREAD_ACP, 0, _src, static_cast<int>(_count),
                            _dest, static_cast<int>(_size)));
  if (_written >= _size ||
      (_written == 0 && _size != 1)) // allowing empty string
  {
    throw gcnew System::ArgumentException(_EXCEPTION_MB2WC);
...
inline System::String ^
    InternalAnsiToStringHelper(_In_reads_z_(_count + 1) const char *_src,
                               size_t _count) {
...
inline System::String ^
    InternalUnicodeToStringHelper(_In_reads_z_(_count + 1) const wchar_t *_src,
                                  size_t _count) {
  if (_count > INT_MAX) {
    throw gcnew System::ArgumentOutOfRangeException(
        _EXCEPTION_GREATER_THAN_INT_MAX);
...
template <class _To_Type, class _From_Type,
          bool _Needs_Context =
              context_node<_To_Type, _From_Type>::_Needs_Context>
...
public:
  __declspec(deprecated(
      ...
      "needed for this conversion is not included.  Please refer to the "
      "documentation on 'How to: Extend the Marshaling Library' for adding "
      "your own marshaling method.")) static _To_Type
  marshal_as(const _From_Type &) = delete;
};
...
public:
  __declspec(deprecated(
      ...
      "marshal_context for this conversion.")) static _To_Type
  marshal_as(const _From_Type &) = delete;
};

template <class _To_Type, class _From_Type>
inline _To_Type _Marshal_as_impl(::std::false_type,
                                 const _From_Type &_from_object) {
  return error_reporting_helper<_To_Type, _From_Type>::marshal_as(_from_object);
}

template <class _From_Type>
    inline System::String ^
    _Marshal_as_impl(::std::true_type, const char *_from_object) {
  if (!_from_object) {
    return nullptr;
  }

  return details::InternalAnsiToStringHelper(_from_object,
                                             strlen(_from_object));
}

template <class _From_Type>
    inline System::String ^
    _Marshal_as_impl(::std::true_type, const wchar_t *_from_object) {
  if (!_from_object) {
    return nullptr;
  }
...
inline _To_Type marshal_as(const _From_Type &_from_object) {
  ...
  return _Marshal_as_impl<_To_Type>(
      typename ::std::conjunction<
          ...
          ::std::disjunction<::std::is_same<char *, _Decayed>,
                             ::std::is_same<const char *, _Decayed>,
                             // Note: BSTR is actually a typedef for wchar_t*.
                             // As a result, BSTR will get accepted as a null
                             // terminated character type sequence here.
...
public:
  context_node(_Outref_result_maybenull_ _Post_z_ const char *&_to_object,
               System::String ^ _from_object) {
    _ptr = NULL;
...
public:
  context_node(_Outref_ _Post_z_ const wchar_t *&_to_object,
               System::String ^ _from_object) {
    _ip = System::Runtime::InteropServices::Marshal::StringToHGlobalUni(
        _from_object);
...
ref class marshal_context {
  internal :
      // The list of objects that need to be cleaned up. Basically all those
      // that have a destructor
  ...
  public:
    ...
    static inline _To_Type marshal_as(
        const _From_Type &_from,
        System::Collections::Generic::LinkedList<Object ^> % _clean_up_list) {
      typedef context_node<_To_Type, _From_Type> _cn;

      _To_Type _to_object;
      // Create a context node and assign marshalling result to the 'to'
      // variable
...
public:
  ...
  inline _To_Type marshal_as(const _From_Type &_from) {
    ...
    return internal_marshaler<_To_Type, _From_adjusted,
                              _cn::_Needs_Context>::marshal_as(_from,
                                                               _clean_up_list);
  }
  ...
  inline _To_Type marshal_as(_From_Type ^ _from) {
    typedef context_node<_To_Type, _From_Type ^> _cn;
    return internal_marshaler<_To_Type, _From_Type ^,
                              _cn::_Needs_Context>::marshal_as(_from,
                                                               _clean_up_list);
  }
