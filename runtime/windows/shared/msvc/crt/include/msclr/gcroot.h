 ...
 *
 *       Note that we cannot currently embed a GCHandle directly in an unmanaged
 * C++ class.  We therefore store a void*, and use the conversion methods of
...
template <class T> struct gcroot {
  ...
  [System::Diagnostics::DebuggerStepThroughAttribute]
      [System::Security::SecuritySafeCritical] gcroot() {
  ...
  gcroot(const gcroot &r) {
    ...
    _handle = __GCHANDLE_TO_VOIDPTR(
        GCHandle::Alloc(__VOIDPTR_TO_GCHANDLE(r._handle).Target));
  }
  ...
  //
  [System::Diagnostics::DebuggerStepThroughAttribute]
      [System::Security::SecurityCritical] ~gcroot() {
  ...
  // The managed object is not a secret or protected resource, so it's okay to
  // expose to anyone who has access to the gcroot object
