...
public:
  ...
  // Accept an optional timespan (in milliseconds) and throw on failure or
  // timeout.
  ...
  template <class T>
  lock(T ^ _object, int _timeout) : m_object(_object), m_locked(false) {
    // ensure that T is not a ReaderWriterLock.
    typedef is_not<T,
                   System::Threading::ReaderWriterLock>::__dont_use_this_type__
        _Diagnostic;
  ...
  lock(T ^ _object, System::TimeSpan _timeout)
      : m_object(_object), m_locked(false) {
    // ensure that T is not a ReaderWriterLock.
    typedef is_not<T,
                   System::Threading::ReaderWriterLock>::__dont_use_this_type__
        _Diagnostic;
  ...
  template <class T>
  lock(T ^ _object, lock_when) : m_object(_object), m_locked(false) {
    // ensure that T is not a ReaderWriterLock.
    typedef is_not<T,
                   System::Threading::ReaderWriterLock>::__dont_use_this_type__
        _Diagnostic;
...
#if !defined(_M_CEE_SAFE)
  template <class T>
  lock(gcroot<T ^> _object) : m_object(_object), m_locked(false) {
    // ensure that T is not a ReaderWriterLock.
    typedef is_not<T,
                   System::Threading::ReaderWriterLock>::__dont_use_this_type__
        _Diagnostic;
  ...
  template <class T>
  lock(gcroot<T ^> _object, int _timeout) : m_object(_object), m_locked(false) {
    // ensure that T is not a ReaderWriterLock.
    typedef is_not<T,
                   System::Threading::ReaderWriterLock>::__dont_use_this_type__
        _Diagnostic;
  ...
  // to defer taking the lock.
  template <class T>
  lock(gcroot<T ^> _object, lock_when) : m_object(_object), m_locked(false) {
    // ensure that T is not a ReaderWriterLock.
    typedef is_not<T,
                   System::Threading::ReaderWriterLock>::__dont_use_this_type__
        _Diagnostic;
  ...
  template <class T> bool operator==(T t) {
    ...
    typedef is_not<T, _detail_class::_safe_bool>::__dont_use_this_type__
        _Diagnostic;
  ...
  void acquire() {
    if (!m_locked) {
      System::Threading::Monitor::TryEnter(
          m_object, System::Threading::Timeout::Infinite, m_locked);
      if (!m_locked) {
        static const long _hresult_wait_timeout = 0x80070102;
        throw System::Runtime::InteropServices::Marshal::GetExceptionForHR(
            _hresult_wait_timeout);
  ...
  bool try_acquire(
      int _timeout) // noexcept, but error C2353 prohibits on managed classes
  {
    if (!m_locked) {
      System::Threading::Monitor::TryEnter(m_object, _timeout, m_locked);
      if (!m_locked) {
  ...
  // Try to take the lock within the specified timespan. In the case
  // of timeout, return false.
  bool try_acquire(System::TimeSpan _timeout) // noexcept, but error C2353
                                              // prohibits on managed classes
  {
    if (!m_locked) {
      System::Threading::Monitor::TryEnter(m_object, _timeout, m_locked);
      if (!m_locked) {
