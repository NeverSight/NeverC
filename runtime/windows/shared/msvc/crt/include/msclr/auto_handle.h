...
template <typename _element_type> ref class auto_handle {
private:
...
public:
  ...
  auto_handle() : m_handle(nullptr) {}

  // construct from object pointer
  auto_handle(_element_type ^ _ptr) : m_handle(_ptr) {}
  ...
  auto_handle(auto_handle<_other_type> % _right) : m_handle(_right.release()) {}
  ...
  template <typename _other_type>
      auto_handle<_element_type> % operator=(auto_handle<_other_type> %
                                             _right) {
    reset(_right.release());
    return *this;
  }

  _element_type ^ get() { return m_handle; }

  // return pointer to class object (assume pointer)
  _element_type ^ operator->() { return m_handle; }
