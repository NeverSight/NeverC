...
template <class _interface_type> ref class smart_com_ptr {
public:
  smart_com_ptr(_interface_type *p) { ptr = p; }
  ...
  _interface_type *operator->() { return ptr; }

  ~smart_com_ptr() { ptr->Release(); }
...
public:
  ...
  void CreateInstance(REFCLSID rclsid, LPUNKNOWN pouter, DWORD cls_context) {
    ...
    System::Runtime::InteropServices::Marshal::ThrowExceptionForHR(
        CoCreateInstance(rclsid, pouter, cls_context, __uuidof(_interface_type),
                         (void **)&interface_ptr));
  ...
  _interface_type *GetInterface() {
    ...
    _interface_type *interface_ptr = NULL;
    System::Runtime::InteropServices::Marshal::ThrowExceptionForHR(
        iunk->QueryInterface(__uuidof(_interface_type),
                             (void **)&interface_ptr));
  ...
  template <class _other_type> void QueryInterface(ptr<_other_type> % other) {
...
private:
  ...
  bool valid() { return (obj_rcw != nullptr); }

  void assign(_interface_type *p) {
    if (p) {
      obj_rcw =
          System::Runtime::InteropServices::Marshal::GetUniqueObjectForIUnknown(
              System::IntPtr(p));
    }
  }
