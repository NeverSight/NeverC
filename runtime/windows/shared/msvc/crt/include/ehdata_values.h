...
#define EH_EXCEPTION_NUMBER                                                    \
  ('msc' | 0xE0000000) // The NT Exception # that we use
...
#define EH_MAGIC_HAS_ES                                                        \
  EH_MAGIC_NUMBER2 // Magic number is greater or equal than that
...
#define HT_IsStdDotDot                                                         \
  0x00000040 // the catch is std C++ catch(...) which is supposed to catch only
             // C++ exceptions
#define HT_IsBadAllocCompat                                                    \
  0x00000080                      // the WinRT type can catch a std::bad_alloc
