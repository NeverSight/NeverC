...
void _Uses_alloc_construct_non_pair(_Ty *const _Ptr, _Outer_alloc &_Outer,
                                    _Inner_alloc &_Inner, _Types &&..._Args) {
  // uses-allocator construction of *_Ptr by alloc _Outer propagating alloc
  // _Inner, non-pair case
  if constexpr (uses_allocator_v<remove_cv_t<_Ty>, _Inner_alloc>) {
    if constexpr (is_constructible_v<_Ty, allocator_arg_t, _Inner_alloc &,
                                     _Types...>) {
      allocator_traits<_Outer_alloc>::construct(
          _Outer, _Ptr, allocator_arg, _Inner, _STD forward<_Types>(_Args)...);
    } else {
      static_assert(
          is_constructible_v<_Ty, _Types..., _Inner_alloc &>,
          "N4950 [allocator.uses.trait]/1 requires "
          "is_constructible_v<T, Args..., Alloc&> when "
          "uses_allocator_v<remove_cv_t<T>, Alloc> is true and "
          "is_constructible_v<T, allocator_arg_t, Alloc&, Args...> is false");
      allocator_traits<_Outer_alloc>::construct(
          _Outer, _Ptr, _STD forward<_Types>(_Args)..., _Inner);
    }
  } else {
    static_assert(is_constructible_v<_Ty, _Types...>,
                  "N4950 [allocator.uses.trait]/1 requires "
                  "is_constructible_v<T, Args...> when "
                  "uses_allocator_v<remove_cv_t<T>, Alloc> is false");
    allocator_traits<_Outer_alloc>::construct(_Outer, _Ptr,
...
decltype(auto) _Uses_alloc_piecewise(_Alloc &_Al, tuple<_Types...> &&_Tuple) {
  if constexpr (uses_allocator_v<remove_cv_t<_Ty>, _Alloc>) {
    if constexpr (is_constructible_v<_Ty, allocator_arg_t, _Alloc &,
                                     _Types...>) {
      return _STD tuple_cat(
          tuple<allocator_arg_t, _Alloc &>(allocator_arg, _Al),
          _STD move(_Tuple));
    } else {
...
template <class _CvPair, class _Outer_alloc, class _Inner_alloc,
          class... _Types1, class... _Types2>
void _Uses_alloc_construct_pair_piecewise(_CvPair *const _Ptr,
                                          _Outer_alloc &_Outer,
                                          _Inner_alloc &_Inner,
                                          tuple<_Types1...> &&_Val1,
                                          tuple<_Types2...> &&_Val2) {
  // uses-allocator construction of pair from _Val1 and _Val2 by alloc _Outer
  // propagating alloc _Inner
  allocator_traits<_Outer_alloc>::construct(
      _Outer, _Ptr, piecewise_construct,
      _STD _Uses_alloc_piecewise<typename _CvPair::first_type>(
          _Inner, _STD move(_Val1)),
      _STD _Uses_alloc_piecewise<typename _CvPair::second_type>(
          _Inner, _STD move(_Val2)));
}

template <class _CvPair, class _Outer_alloc, class _Inner_alloc,
          class... _Types1, class... _Types2>
void _Uses_alloc_construct_pair(_CvPair *const _Ptr, _Outer_alloc &_Outer,
                                _Inner_alloc &_Inner, piecewise_construct_t,
                                tuple<_Types1...> _Val1,
                                tuple<_Types2...> _Val2) {
  // uses-allocator construction of pair by alloc _Outer propagating alloc
  // _Inner, piecewise case
  _STD _Uses_alloc_construct_pair_piecewise(_Ptr, _Outer, _Inner,
                                            _STD move(_Val1), _STD move(_Val2));
}
...
void _Uses_alloc_construct_pair(_CvPair *const _Ptr, _Outer_alloc &_Outer,
                                _Inner_alloc &_Inner) {
  // uses-allocator construction of pair by alloc _Outer propagating alloc
  // _Inner, zero-argument case
  _STD _Uses_alloc_construct_pair_piecewise(_Ptr, _Outer, _Inner, tuple<>{},
                                            tuple<>{});
}
...
template <class _CvPair, class _Outer_alloc, class _Inner_alloc, class _Uty,
          class _Vty>
void _Uses_alloc_construct_pair(_CvPair *const _Ptr, _Outer_alloc &_Outer,
                                _Inner_alloc &_Inner, _Uty &&_Arg1,
                                _Vty &&_Arg2) {
  // uses-allocator construction of pair by alloc _Outer propagating alloc
  // _Inner, two-argument case
  _STD _Uses_alloc_construct_pair_piecewise(
      _Ptr, _Outer, _Inner, _STD forward_as_tuple(_STD forward<_Uty>(_Arg1)),
...
void _Uses_alloc_construct_pair(_CvPair *const _Ptr, _Outer_alloc &_Outer,
  ...
  // uses-allocator construction of pair by alloc _Outer propagating alloc
  // _Inner, lvalue pair argument
  _STD _Uses_alloc_construct_pair_piecewise(
      _Ptr, _Outer, _Inner, _STD forward_as_tuple(_Pair.first),
...
void _Uses_alloc_construct_pair(_CvPair *const _Ptr, _Outer_alloc &_Outer,
                                _Inner_alloc &_Inner, _Uty &&_Ux) {
  // uses-allocator construction of pair by alloc _Outer propagating alloc
  // _Inner, non-pair argument
  static_assert(
      _Is_normally_bindable<remove_cv_t<_CvPair>, _Uty>,
  ...
  if constexpr (is_lvalue_reference_v<_Pair_ref_t>) {
    _STD _Uses_alloc_construct_pair_piecewise(
        _Ptr, _Outer, _Inner, _STD forward_as_tuple(_Pair_ref.first),
        _STD forward_as_tuple(_Pair_ref.second));
  } else {
    _STD _Uses_alloc_construct_pair_piecewise(
        _Ptr, _Outer, _Inner,
        _STD forward_as_tuple(
            _STD forward<typename _CvPair::first_type>(_Pair_ref.first)),
        _STD forward_as_tuple(
            _STD forward<typename _CvPair::second_type>(_Pair_ref.second)));
  }
}
#endif // !_HAS_CXX20
...
public:
  ...
  allocate(_CRT_GUARDOVERFLOW const size_t _Bytes,
           const size_t _Align =
               alignof(max_align_t)) { // allocate _Bytes bytes of memory with
                                       // alignment _Align
    _STL_ASSERT(
        _STD _Is_pow_2(_Align),
        "memory_resource::allocate(): Alignment must be a power of two.");
  ...
  void deallocate(void *const _Ptr, const size_t _Bytes,
    ...
    _STL_ASSERT(
        _STD _Is_pow_2(_Align),
        "memory_resource::deallocate(): Alignment must be a power of two.");
    return do_deallocate(_Ptr, _Bytes, _Align);
  }

  _NODISCARD bool is_equal(const memory_resource &_That) const noexcept {
    // determine if *this and _That can both deallocate memory allocated by
    // either
...
_EXPORT_STD _NODISCARD inline bool
operator==(const memory_resource &_Left,
           const memory_resource &_Right) noexcept {
  return &_Left == &_Right || _Left.is_equal(_Right);
}

#if !_HAS_CXX20
_NODISCARD inline bool operator!=(const memory_resource &_Left,
                                  const memory_resource &_Right) noexcept {
  return !(_Left == _Right);
}
#endif // !_HAS_CXX20

extern "C" _CRT_SATELLITE_1 memory_resource *__cdecl
_Aligned_get_default_resource() noexcept;
extern "C" _CRT_SATELLITE_1 memory_resource *__cdecl
_Unaligned_get_default_resource() noexcept;
...
#else  // ^^^ _HAS_CXX20 && defined(__cpp_lib_byte) / !_HAS_CXX20 ||
       // !defined(__cpp_lib_byte) vvv
...
public:
  ...
  /* implicit */ polymorphic_allocator(
      memory_resource *const _Resource_) noexcept // strengthened
      : _Resource{_Resource_} {                   // initialize with _Resource_
    _STL_ASSERT(_Resource, "Cannot initialize polymorphic_allocator with null "
                           "resource (N4950 [mem.poly.allocator.ctor]/2)");
  }
  ...
  ~polymorphic_allocator() noexcept {
    const auto _Tombstone{
        reinterpret_cast<memory_resource *>(_MSVC_STL_UINTPTR_TOMBSTONE_VALUE)};
    _Resource = _Tombstone;
  }
#endif // _MSVC_STL_DESTRUCTOR_TOMBSTONES

  _NODISCARD_RAW_PTR_ALLOC __declspec(allocator)
  _Ty *allocate(_CRT_GUARDOVERFLOW const size_t _Count) {
    // get space for _Count objects of type _Ty from _Resource
    void *const _Vp =
        _Resource->allocate(_Get_size_of_n<sizeof(_Ty)>(_Count), alignof(_Ty));
...
#if _HAS_CXX20
  _NODISCARD_RAW_PTR_ALLOC __declspec(allocator) void *
  allocate_bytes(const size_t _Bytes,
                 const size_t _Align = alignof(max_align_t)) {
  ...
  template <class _Uty>
  _NODISCARD_RAW_PTR_ALLOC __declspec(allocator) _Uty *
  allocate_object(_CRT_GUARDOVERFLOW const size_t _Count = 1) {
    void *const _Vp =
        allocate_bytes(_Get_size_of_n<sizeof(_Uty)>(_Count), alignof(_Uty));
  ...
  void construct(_Uty *const _Ptr, _Types &&..._Args) {
    // propagate allocator *this if uses_allocator_v<remove_cv_t<_Uty>,
    // polymorphic_allocator>
#if _HAS_CXX20
    // equivalent to calling uninitialized_construct_using_allocator except for
    // handling of cv-qualification
    _STD apply(
        ...
        _STD uses_allocator_construction_args<_Uty>(
            *this, _STD forward<_Types>(_Args)...));
#else  // ^^^ _HAS_CXX20 / !_HAS_CXX20 vvv
    allocator<char> _Al{};
    if constexpr (_Is_cv_pair<_Uty>) {
      _STD _Uses_alloc_construct_pair(_Ptr, _Al, *this,
                                      _STD forward<_Types>(_Args)...);
    } else {
      _STD _Uses_alloc_construct_non_pair(_Ptr, _Al, *this,
                                          _STD forward<_Types>(_Args)...);
    }
#endif // ^^^ !_HAS_CXX20 ^^^
  }
  ...
  _NODISCARD polymorphic_allocator
  select_on_container_copy_construction() const noexcept /* strengthened */ {
  ...
  _NODISCARD friend bool
  operator==(const polymorphic_allocator &_Lhs,
             const polymorphic_allocator &_Rhs) noexcept {
...
_NODISCARD bool operator==(const polymorphic_allocator<_Ty1> &_Left,
...
struct _Has_no_alloc_destroy<pmr::polymorphic_allocator<_Ty>, _Ptr, void>
    : true_type {
  // polymorphic_allocator technically _does_ have a destroy member, but it's
  // equivalent to the default implementation in allocator_traits so we can
  // optimize it away.
};
