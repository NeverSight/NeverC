...
public:
  ...
  static _Interface *GetInterfacePtr() noexcept { return nullptr; }

  static _Interface &GetInterface() noexcept { return *GetInterfacePtr(); }
...
public:
  ...
  // CLSID.
  //
  static const IID &GetIID() noexcept { return ThisIIID::GetIID(); }

  // Constructs a smart-pointer from any other smart pointer.
  //
  template <
      typename _OtherIID,
      ::std::enable_if_t<!::std::is_same<_IIID, _OtherIID>::value, int> = 0>
  _com_ptr_t(const _com_ptr_t<_OtherIID> &p) : m_pInterface(nullptr) {
  ...
  template <
      ...
      ::std::enable_if_t<
          !(::std::is_same<_InterfaceType, Interface>::value // call Interface*
                                                             // pInterface
            || ::std::is_same<_InterfaceType, _com_ptr_t>::value // call
                                                                 // _com_ptr_t*
                                                                 // p
            || ::std::is_same<_InterfaceType, char>::value // call PCSTR str,
                                                           // IUnknown* pOuter,
                                                           // DWORD dwClsContext
            || ::std::is_same<_InterfaceType, const char>::value ||
            ::std::is_same<_InterfaceType, wchar_t>::value // call LPCWSTR str,
                                                           // IUnknown* pOuter,
                                                           // DWORD dwClsContext
            || ::std::is_same<_InterfaceType, const wchar_t>::value),
          int> = 0>
  _com_ptr_t(_InterfaceType *p) : m_pInterface(nullptr) {
    HRESULT hr = _QueryInterface(p);
  ...
  explicit _com_ptr_t(_com_ptr_t *p) : m_pInterface(nullptr) {
    if (p == nullptr) {
  ...
  _com_ptr_t(const _com_ptr_t &cp) noexcept : m_pInterface(cp.m_pInterface) {
    _AddRef();
  }

  _com_ptr_t(_com_ptr_t &&cp) noexcept : m_pInterface(cp.m_pInterface) {
    cp.m_pInterface = nullptr;
  }

  _com_ptr_t(Interface *pInterface) noexcept : m_pInterface(pInterface) {
    _AddRef();
  }
  ...
  explicit _com_ptr_t(const CLSID &clsid, IUnknown *pOuter = nullptr,
  ...
  explicit _com_ptr_t(LPCWSTR str, IUnknown *pOuter = nullptr,
                      DWORD dwClsContext = CLSCTX_ALL) {
  ...
  explicit _com_ptr_t(LPCSTR str, IUnknown *pOuter = nullptr,
                      DWORD dwClsContext = CLSCTX_ALL) {
    HRESULT hr = CreateInstance(str, pOuter, dwClsContext);
  ...
  // Allows an instance of this class to act as though it were the actual
  // interface.
  ...
  // Returns the address of the interface pointer contained in this class. This
  // is useful when using the COM/OLE interfaces to create this interface.
  ...
  template <typename _InterfaceType,
            ::std::enable_if_t<
                !::std::is_same<_InterfaceType, Interface>::value, int> = 0>
  ...
  bool operator!=(const _com_ptr_t &p) const { return !(*this == p); }
  ...
  bool operator!=(Interface *p) const { return !(*this == p); }
  ...
  bool operator<(Interface *p) const { return _CompareUnknown(p) < 0; }
  ...
  Interface *&GetInterfacePtr() noexcept { return m_pInterface; }

  // Loads an interface for the provided CLSID.
  // Returns an HRESULT.  Any previous interface is unconditionally released.
  HRESULT CreateInstance(const CLSID &rclsid, IUnknown *pOuter = nullptr,
  ...
  // Creates the class specified by clsidString. clsidString may contain a class
  // id, or a prog id string.
  HRESULT CreateInstance(LPCWSTR clsidString, IUnknown *pOuter = nullptr,
                         DWORD dwClsContext = CLSCTX_ALL) noexcept {
  ...
  // Creates the class specified by SBCS clsidString. clsidString may contain a
  // class id, or a prog id string.
  HRESULT CreateInstance(LPCSTR clsidStringA, IUnknown *pOuter = nullptr,
                         DWORD dwClsContext = CLSCTX_ALL) noexcept {
    ...
    int const destSize = MultiByteToWideChar(
        CP_ACP, 0, clsidStringA, static_cast<int>(size), nullptr, 0);
    ...
    if (MultiByteToWideChar(CP_ACP, 0, clsidStringA, static_cast<int>(size),
                            clsidStringW, destSize) == 0) {
      _freea(clsidStringW);
      return HRESULT_FROM_WIN32(GetLastError());
    }
  ...
  //
  template <typename _InterfaceType>
  HRESULT QueryInterface(const IID &iid, _InterfaceType *&p) noexcept {
    if (m_pInterface) {
...
private:
  ...
  HRESULT _QueryInterface(_InterfacePtr p) noexcept {
    HRESULT hr;
