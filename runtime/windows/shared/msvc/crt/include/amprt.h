...
//  A reference counter to be used as the base class for all reference counted
//  types.
...
public:
  ...
  size_t _Remove_reference() {
    ...
    size_t refCount =
        InterlockedDecrement(reinterpret_cast<LONG volatile *>(&_M_rc));
  ...
  size_t _Get_reference_count() { return _M_rc; }
...
template <class T> class _Reference_counted_obj_ptr {
public:
  // Constructor
  _Reference_counted_obj_ptr(T *_Ptr = NULL) : _M_obj_ptr(_Ptr) { _Init(); }
  ...
  // Move constructor
  _Reference_counted_obj_ptr(_Reference_counted_obj_ptr &&_Other) noexcept
      : _M_obj_ptr(_Other._M_obj_ptr) {
  ...
  // Assignment operator
  _Reference_counted_obj_ptr &
  operator=(const _Reference_counted_obj_ptr &_Other) {
  ...
  _Reference_counted_obj_ptr &
  operator=(_Reference_counted_obj_ptr &&_Other) noexcept {
    if (_M_obj_ptr != _Other._M_obj_ptr) {
      T *oldPtr = _M_obj_ptr;
      _M_obj_ptr = _Other._M_obj_ptr;
      _Other._M_obj_ptr = nullptr;
  ...
  _Ret_ T *operator->() const { return _M_obj_ptr; }

  T &operator*() const { return *_M_obj_ptr; }

  operator T *() const { return _M_obj_ptr; }

  _Ret_ T *_Get_ptr() const { return _M_obj_ptr; }
...
typedef details::_Reference_counted_obj_ptr<details::_Accelerator_view_impl>
    _Accelerator_view_impl_ptr;
typedef details::_Reference_counted_obj_ptr<details::_Accelerator_impl>
    _Accelerator_impl_ptr;
...
typedef details::_Reference_counted_obj_ptr<details::_Ubiquitous_buffer>
    _Ubiquitous_buffer_ptr;
typedef details::_Reference_counted_obj_ptr<details::_Event_impl>
    _Event_impl_ptr;
typedef details::_Reference_counted_obj_ptr<details::_View_shape>
    _View_shape_ptr;
...
public:
  ...
  /// <summary>
  ///     Poll whether the _Event has completed or not and throws any exceptions
  ///     that occur
  /// </summary>
  /// <returns>
  ///     true, if the _Event has completed, false otherwise
  ...
  /// <summary>
  ///     Creates an event which is an ordered collection of this and a
  ///     continuation task
  /// </summary>
  /// <returns>
  ///     The composite event
  /// </returns>
  _AMPIMP _Event
  _Add_continuation(const std::function<_Event __cdecl()> &_Continuation_task);

  /// <summary>
  ///     Return true if the other _Event is same as this _Event; false
  ///     otherwise
  /// </summary>
  _AMPIMP bool operator==(const _Event &_Other) const;

  /// <summary>
  ///     Return false if the other _Event is same as this _Event; true
  ///     otherwise
  /// </summary>
  _AMPIMP bool operator!=(const _Event &_Other) const;
...
_Ret_ _Accelerator_view_impl *
_Get_accelerator_view_impl_ptr(const accelerator_view &_Accl_view);
_Ret_ _Accelerator_impl *_Get_accelerator_impl_ptr(const accelerator &_Accl);
_Event _Get_access_async(const _View_key _Key, accelerator_view _Av,
...
inline bool _Is_valid_access_mode(_Access_mode _Mode) {
  if ((_Mode != _Read_access) && (_Mode != _Write_access) &&
...
typedef struct _Buffer_descriptor {
  friend _Event _Get_access_async(const _View_key _Key, accelerator_view _Av,
                                  _Access_mode _Mode, _Buffer_ptr &_Buf_ptr);

  // _M_data_ptr points to the raw data underlying the buffer for accessing on
  // host
  mutable void *_M_data_ptr;

private:
  // _M_buffer_ptr points to a _Ubiquitous_buffer that holds the data in an 1D
  // array. This is private to ensure that all assignments to this data member
...
public:
  // _M_curr_cpu_access_mode specifies the current access mode of the data on
  // the cpu accelerator_view specified at the time of registration of this view
...
public:
  ...
  // Default constructor
  _Buffer_descriptor() __GPU : _M_data_ptr(NULL),
                               _M_buffer_ptr(NULL),
                               _M_curr_cpu_access_mode(_No_access),
                               _M_type_access_mode(_Is_array_mode) {}

  _Buffer_descriptor(_In_ void *_Data_ptr, _In_ _Ubiquitous_buffer *_Buffer_ptr,
                     _Access_mode _Curr_cpu_access_mode,
                     _Access_mode _Type_mode) __GPU
      : _M_data_ptr(_Data_ptr),
        _M_buffer_ptr(NULL),
        _M_curr_cpu_access_mode(_Curr_cpu_access_mode),
        _M_type_access_mode(_Type_mode) {
    _Set_buffer_ptr(_Buffer_ptr);
  }

  // Destructor
  ~_Buffer_descriptor() __GPU { _Set_buffer_ptr(NULL); }
  ...
  void _Set_buffer_ptr(_In_opt_ _Ubiquitous_buffer *_Buffer_ptr) __CPU_ONLY {
    if (_M_buffer_ptr != _Buffer_ptr) {
      if (_M_buffer_ptr != NULL) {
        reinterpret_cast<_Reference_counter *>(_M_buffer_ptr)
            ->_Remove_reference();
      }
  ...
  bool _Is_array() const { return (_M_type_access_mode == _Is_array_mode); }

  _Ret_ _View_key _Get_view_key() { return this; }

  const _View_key _Get_view_key() const { return ((const _View_key)(this)); }
...
private:
  ...
  // The index of the most detailed (largest in size) mipmap level for the
  // texture (or texture view) This value is always zero for the texture and
  // might be non-zero for the texture views
  ...
  // then read-only texture view with most detailed mipmap level equal to 1, can
  // have 1 or 2 mipmap levels ([1] or [1, 2]). Further texture_views created on
  // top of the texture view defined above can only narrow down the range of
  // accessible mipmap levels.
...
public:
  ...
  // Default constructor
  _Texture_descriptor() __GPU : _M_data_ptr(NULL),
                                _M_texture_ptr(NULL),
                                _M_most_detailed_mipmap_level(0),
                                _M_view_mipmap_levels(0) {
    // Enables move constructor
  }

  // Constructor for the texture
  _Texture_descriptor(unsigned int _Most_detailed_mipmap_level,
                      unsigned int _View_mipmap_levels) __GPU
      : _M_data_ptr(NULL),
        _M_texture_ptr(NULL),
        _M_most_detailed_mipmap_level(_Most_detailed_mipmap_level),
  ...
  // Destructor
  ~_Texture_descriptor() __GPU { _Set_texture_ptr(NULL); }
  ...
  _Texture_descriptor(const _Texture_descriptor &_Other,
  ...
  bool operator==(const _Texture_descriptor &_Other) const __GPU {
    return _M_texture_ptr == _Other._M_texture_ptr &&
           ...
           _M_most_detailed_mipmap_level ==
               _Other._M_most_detailed_mipmap_level &&
           _M_view_mipmap_levels == _Other._M_view_mipmap_levels;
  }
  ...
  void _Set_texture_ptr(_In_opt_ _Texture *_Texture_ptr) __CPU_ONLY {
    if (_M_texture_ptr != _Texture_ptr) {
      if (_M_texture_ptr != NULL) {
        reinterpret_cast<_Reference_counter *>(_M_texture_ptr)
            ->_Remove_reference();
      }
  ...
  bool _Are_mipmap_levels_overlapping(const _Texture_descriptor *_Other) const
      __CPU_ONLY {
    _ASSERTE(_Other);
    ...
    return !(
        (_M_most_detailed_mipmap_level < _Other->_M_most_detailed_mipmap_level)
            ? ((_M_most_detailed_mipmap_level + _M_view_mipmap_levels - 1) <
               _Other->_M_most_detailed_mipmap_level)
            : ((_Other->_M_most_detailed_mipmap_level +
                _Other->_M_view_mipmap_levels - 1) <
               _M_most_detailed_mipmap_level));
  }
...
private:
  ...
  // representation. This is private to ensure that all assignments to this data
  // member only happen through public functions which properly manage the ref
  // count of the underlying _Sampler object.
...
public:
  ...
  // Default constructor
  _Sampler_descriptor() __GPU : _M_data_ptr(NULL), _M_sampler_ptr(NULL) {}
  ...
  // Destructor
  ~_Sampler_descriptor() __GPU { _Set_sampler_ptr(NULL); }
  ...
  bool operator==(const _Sampler_descriptor &_Other) const __GPU {
    return _M_sampler_ptr == _Other._M_sampler_ptr &&
           _M_data_ptr == _Other._M_data_ptr;
  }

  _Ret_ _Sampler *_Get_sampler_ptr() const __CPU_ONLY { return _M_sampler_ptr; }

  void _Set_sampler_ptr(_In_opt_ _Sampler *_Sampler_ptr) __CPU_ONLY {
    if (_M_sampler_ptr != _Sampler_ptr) {
      if (_M_sampler_ptr != NULL) {
        reinterpret_cast<_Reference_counter *>(_M_sampler_ptr)
            ->_Remove_reference();
      }
...
_AMPIMP void __cdecl
_Register_async_event(const _Event &_Ev,
                      const std::shared_future<void> &_Shared_future);
_AMPIMP _Access_mode __cdecl
_Get_recommended_buffer_host_access_mode(const accelerator_view &_Av);
} // namespace details
...
/// </summary>
/// <param name="_Av">
///     The D3D accelerator_view for which the underlying D3D device interface
///     is returned.
/// </param>
/// <returns>
///     The IUnknown interface pointer of the D3D device underlying the
///     accelerator_view.
/// </returns>
...
/// </returns>
_AMPIMP accelerator_view __cdecl
create_accelerator_view(_In_ IUnknown *_D3D_device,
...
///     D3D11_CREATE_DEVICE_DISABLE_GPU_TIMEOUT flag for Direct3D device
///     creation and is used to indicate if the operating system should allow
///     workloads that take more than 2 seconds to execute, without resetting
///     the device per the Windows timeout detection and recovery mechanism. Use
///     of this flag is recommended if you need to perform time consuming tasks
///     on the accelerator_view.
/// </param>
/// <param name="_Qmode">
///     The queuing_mode to be used for the newly created accelerator_view.
///     This parameter has a default value of queuing_mode_automatic.
...
/// </returns>
_AMPIMP accelerator_view __cdecl
create_accelerator_view(accelerator &_Accelerator, bool _Disable_timeout,
                        queuing_mode _Qmode = queuing_mode_automatic);
...
/// </param>
/// <returns>
///     A boolean flag indicating if timeout is disabled for the specified
///     accelerator_view.
/// </returns>
_AMPIMP bool __cdecl
is_timeout_disabled(const accelerator_view &_Accelerator_view);

/// <summary>
///     Acquire a lock on an accelerator_view for the purpose of safely
///     performing D3D operations on resources shared with the accelerator_view.
///     The accelerator_view and all C++ AMP resources associated with this
///     accelerator_view internally take this lock when performing operations
///     and will block while another thread holds the D3D access lock.
///
///     This lock is non-recursive: It is undefined behavior to call this
///     function from a thread that already holds the lock. It is undefined
///     behavior to perform operations on the accelerator_view or any data
///     container associated with the accelerator_view from the thread that
///     holds the D3D access lock.
///
///     See also scoped_d3d_access_lock, a RAII-style class for a scope-based
///     D3D access lock.
/// </summary>
/// <param name="_Av">
...
/// <summary>
///     Attempt to acquire the D3D access lock on an accelerator_view without
///     blocking.
/// </summary>
/// <param name="_Av">
///     The accelerator_view to lock.
/// </param>
/// <returns>
///     true if the lock was acquired, or false if it is currently held by
///     another thread.
/// </returns>
_AMPIMP bool __cdecl d3d_access_try_lock(accelerator_view &_Av);

/// <summary>
///     Release the D3D access lock on the given accelerator_view.  If the
///     calling thread does not hold the lock on the accelerator_view the
...
public:
  /// <summary>
  ///     Acquire a D3D access lock on the given accelerator_view.  The lock is
  ///     released when this object goes out of scope.  Construction will block
  ...
  /// <summary>
  ///     Construct a scoped_d3d_access_lock on an accelerator_view for which
  ///     the lock is already held (e.g. was acquired by d3d_access_try_lock).
  ///     The D3D access lock must already be held by the calling thread and not
  ///     controlled by any other scoped_d3d_access_lock.
  /// </summary>
  /// <param name="_Av">
  ...
  /// </returns>
  _AMPIMP scoped_d3d_access_lock &
  operator=(scoped_d3d_access_lock &&_Other) noexcept;
...
/// <summary>
///  Class represents a accelerator abstraction for C++ AMP data-parallel
///  devices
/// </summary>
class accelerator {
  friend class accelerator_view;
  ...
  _AMPIMP friend accelerator_view __cdecl direct3d::create_accelerator_view(
      accelerator &_Accelerator, bool _Disable_timeout,
      queuing_mode _Qmode /* = queuing_mode_automatic */);

  friend _Ret_ details::_Accelerator_impl *
  details::_Get_accelerator_impl_ptr(const accelerator &_Accl);
...
public:
  ...
  /// <summary>
  ///     Returns the vector of accelerator objects representing all available
  ///     accelerators
  /// </summary>
  /// <returns>
  ...
  ///     for executing the parallel_for_each kernel to be automatically
  ///     selected by the runtime. For all other purposes, the accelerator_view
  ///     returned by this method is the same as the default accelerator_view of
  ///     the default accelerator
  /// </summary>
  ...
  /// </summary>
  std::wstring get_device_path() const { return _Get_device_path(); }
  ...
  __declspec(property(
      get = get_version)) unsigned int version; // hiword=major, loword=minor

  /// <summary>
  ///     Returns the device description as a std::wstring
  /// </summary>
  std::wstring get_description() const { return _Get_description(); }
  ...
  ///     emulated. This is true, for example, with the direct3d reference and
  ///     WARP accelerators.
  /// </summary>
  ...
  __declspec(property(
      get = get_supports_double_precision)) bool supports_double_precision;
  ...
  __declspec(property(get = get_supports_limited_double_precision)) bool
  ...
  __declspec(property(
      get = get_supports_cpu_shared_memory)) bool supports_cpu_shared_memory;
  ...
  __declspec(property(
      get = get_default_cpu_access_type)) access_type default_cpu_access_type;
  ...
  ///     on this accelerator. This method only succeeds if the
  ///     default_cpu_access_type for the accelerator has not already been
  ///     overridden by a previous call to this method and the runtime selected
  ///     default_cpu_access_type for this accelerator has not yet been used for
  ///     allocating an array or for an implicit memory allocation backing an
  ///     array_view accessed on this accelerator.
  /// </summary>
  /// <param name="_Default_cpu_access_type">
  ///     The default cpu access_type to be used for array/array_view memory
  ///     allocations on this accelerator.
  /// </param>
  /// <returns>
  ///     A boolean value indicating if the default cpu access_type for the
  ///     accelerator was successfully set.
  /// </returns>
  _AMPIMP bool
  set_default_cpu_access_type(access_type _Default_cpu_access_type);
  ...
  create_view(queuing_mode qmode = queuing_mode_automatic);

  /// <summary>
  ///     Return true if the other accelerator is same as this accelerator;
  ///     false otherwise
  /// </summary>
  _AMPIMP bool operator==(const accelerator &_Other) const;

  /// <summary>
  ///     Return false if the other accelerator is same as this accelerator;
  ///     true otherwise
  /// </summary>
  _AMPIMP bool operator!=(const accelerator &_Other) const;
...
public:
  ...
  completion_future(const completion_future &_Other)
      : _M_shared_future(_Other._M_shared_future), _M_task(_Other._M_task) {}
  ...
  ///     and false otherwise
  /// </returns>
  bool valid() const { return _M_shared_future.valid(); }
  ...
  /// </summary>
  /// <returns>
  ///     - future_status::deferred if the associated asynchronous operation is
  ///     not running
  ///     - future_status::ready if the associated asynchronous operation is
  ///     finished
  ///     - future_status::timeout if the time period specified has elapsed
  /// </returns>
  template <class _Rep, class _Period>
  std::future_status
  wait_for(const std::chrono::duration<_Rep, _Period> &_Rel_time) const {
  ...
  /// </summary>
  /// <returns>
  ///     A std::shared_future&lt;void&gt; object corresponding to the
  ///     associated asynchronous operation
  /// </returns>
  operator std::shared_future<void>() const { return _M_shared_future; }
  ...
  /// </summary>
  template <typename _Functor> void then(const _Functor &_Func) const {
  ...
  ///     asynchronous operation
  /// </returns>
  concurrency::task<void> to_task() const { return _M_task; }
...
/// <summary>
///  Class represents a virtual device abstraction on a C++ AMP data-parallel
///  accelerator
/// </summary>
class accelerator_view {
  ...
  _AMPIMP friend _Ret_ IUnknown *__cdecl direct3d::get_device(
      const accelerator_view &_Av);

  _AMPIMP friend accelerator_view __cdecl direct3d::create_accelerator_view(
      _In_ IUnknown *_D3D_device,
  ...
  _AMPIMP friend bool __cdecl direct3d::is_timeout_disabled(
      const accelerator_view &_Accelerator_view);

  friend _Ret_ details::_Accelerator_view_impl *
  details::_Get_accelerator_view_impl_ptr(const accelerator_view &_Accl_view);
...
public:
  ...
  __declspec(property(
      get = get_accelerator)) Concurrency::accelerator accelerator;
  ...
  __declspec(property(
      get = get_queuing_mode)) Concurrency::queuing_mode queuing_mode;
  ...
  /// <summary>
  ///     Return true if the other accelerator view is same as this accelerator
  ///     view; false otherwise
  /// </summary>
  _AMPIMP bool operator==(const accelerator_view &_Other) const;

  /// <summary>
  ///     Return false if the other accelerator view is same as this accelerator
  ///     view; true otherwise
  /// </summary>
  _AMPIMP bool operator!=(const accelerator_view &_Other) const;

  /// <summary>
  ///     Waits for completion of all commands submitted so far to this
  ///     accelerator_view
  /// </summary>
  _AMPIMP void wait();

  /// <summary>
  ///     Submit all pending commands queued to this accelerator_view to the
  ///     accelerator for execution.
  /// </summary>
  _AMPIMP void flush();

  /// <summary>
  ///     Return a future to track the completion of all commands submitted so
  ///     far to this accelerator_view
  /// </summary>
...
namespace details {
inline _Ret_ _Accelerator_view_impl *
_Get_accelerator_view_impl_ptr(const accelerator_view &_Accl_view) {
  return _Accl_view._M_impl;
}

inline _Ret_ _Accelerator_impl *
_Get_accelerator_impl_ptr(const accelerator &_Accl) {
...
typedef std::unordered_set<accelerator_view, _Accelerator_view_hasher>
    _Accelerator_view_unordered_set;

// Describes the N dimensional shape of a view in a buffer
class _View_shape : public _Reference_counter {
public:
  _AMPIMP static _Ret_ _View_shape *__cdecl _Create_view_shape(
      unsigned int _Rank, unsigned int _Linear_offset,
  ...
  inline unsigned int _Get_rank() const { return _M_rank; }

  inline unsigned int _Get_linear_offset() const { return _M_linear_offset; }

  inline const unsigned int *_Get_base_extent() const { return _M_base_extent; }

  inline const unsigned int *_Get_view_offset() const { return _M_view_offset; }
  inline const unsigned int *_Get_view_extent() const { return _M_view_extent; }

  inline const bool *_Get_projection_info() const { return _M_projection_info; }

  inline bool _Is_projection() const { return _M_projection_info[0]; }

  inline bool _Is_valid(size_t _Buffer_size) const {
    // The end point of the base shape should not be greater than the size of
    // the buffer
    size_t endLinearOffset =
        _M_linear_offset + _Get_extent_size(_M_rank, _M_base_extent);
  ...
  static inline bool _Compare_extent_with_elem_size(
      unsigned int _Rank, const unsigned int *_Extent1, size_t _Elem_size1,
      const unsigned int *_Extent2, size_t _Elem_size2) {
    _ASSERTE((_Rank >= 1) && (_Extent1 != NULL) && (_Extent2 != NULL));

    // The extents should match accounting for the element sizes of the
    // respective buffers
    if ((_Extent1[_Rank - 1] * _Elem_size1) !=
        (_Extent2[_Rank - 1] * _Elem_size2)) {
      return false;
    }
  ...
  static inline bool _Compare_extent(unsigned int _Rank,
  ...
  inline bool _Is_view_linear(unsigned int &_Linear_offset,
                              unsigned int &_Linear_size) const {
    ...
    while ((_First_dim_with_non_unit_extent < _M_rank) &&
    ...
    if ((_Effective_rank <= 1) ||
        (_Compare_extent(
            _Effective_rank - 1,
            &_M_base_extent[_First_dim_with_non_unit_extent + 1],
            &_M_view_extent[_First_dim_with_non_unit_extent + 1]))) {
    ...
    if (_Compare_base_shape(_Other)) {
      ...
      for (size_t _I = 0; _I < _M_rank; ++_I) {
        if (!_Intervals_overlap(
                _M_view_offset[_I], _M_view_offset[_I] + _M_view_extent[_I] - 1,
                _Other->_M_view_offset[_I],
                _Other->_M_view_offset[_I] + _Other->_M_view_extent[_I] - 1)) {
          return false;
        }
      }
  ...
  inline bool _Subsumes(const _View_shape *_Other) const {
    // Subsumption test can only be done for shapes that have the same base
    // shape or when both have a rank of 1
    ...
    for (size_t _I = 0; _I < _M_rank; ++_I) {
      otherEndPointIndex[_I] =
          _Other->_Get_view_offset()[_I] + _Other->_Get_view_extent()[_I] - 1;
    }
...
private:
  // Private constructor to force construction through the _Create_view_shape
  // method
  ...
  static unsigned int _Get_extent_size(unsigned int _Rank,
  ...
  inline bool _Is_valid() const {
    ...
    // Ensure the _M_view_offset + _M_view_extent is within the bounds of
    // _M_base_extent
    size_t viewSize = 1;
  ...
  inline unsigned int
  _Get_linear_offset(const unsigned int *_Element_index) const {
...
// This function creates a new _View_shape object from an existing _View_shape
// object when the data underlying the view needs to be reinterpreted to use a
// different element size than the one used by the original view.
inline _Ret_ _View_shape *
_Create_reinterpreted_shape(const _View_shape *_Source_shape,
                            size_t _Curr_elem_size, size_t _New_elem_size) {
  unsigned int _Rank = _Source_shape->_Get_rank();
  size_t _LinearOffsetInBytes =
      _Source_shape->_Get_linear_offset() * _Curr_elem_size;
  size_t _BaseLSDExtentInBytes =
      (_Source_shape->_Get_base_extent())[_Rank - 1] * _Curr_elem_size;
  size_t _ViewLSDOffsetInBytes =
      (_Source_shape->_Get_view_offset())[_Rank - 1] * _Curr_elem_size;
  size_t _ViewLSDExtentInBytes =
      (_Source_shape->_Get_view_extent())[_Rank - 1] * _Curr_elem_size;
  ...
  return _View_shape::_Create_view_shape(
      _Rank, _New_linear_offset, _New_base_extent.data(),
      _New_view_offset.data(), _New_view_extent.data());
}
...
inline access_type _Get_cpu_access_type(_Access_mode _Cpu_access_mode) {
  ...
  if (_Cpu_access_mode & _Read_access) {
    _Cpu_access_type =
        static_cast<access_type>(_Cpu_access_type | access_type_read);
  }

  if (_Cpu_access_mode & _Write_access) {
    _Cpu_access_type =
        static_cast<access_type>(_Cpu_access_type | access_type_write);
  }
...
public:
  // Force construction through these static public method to ensure that
  // _Buffer objects are allocated in the runtime

  // Allocate a new buffer on the specified accelerator_view
  _AMPIMP static _Ret_ _Buffer *__cdecl
  _Create_buffer(accelerator_view _Accelerator_view,
  ...
  // Create a buffer object from a pre-allocated storage on the specified
  // accelerator_view. This can be thought of as the accelerator_view "adopting"
  // the passed data buffer.
  _AMPIMP static _Ret_ _Buffer *__cdecl
  _Create_buffer(_In_ void *_Data_ptr, accelerator_view _Accelerator_view,
                 size_t _Num_elems, size_t _Elem_size);

  // Create a staging buffer on the specified accelerator_view which can be
  // accessed on the cpu upon mapping.
  _AMPIMP static _Ret_ _Buffer *__cdecl
  _Create_stage_buffer(accelerator_view _Accelerator_view,
                       accelerator_view _Access_on_accelerator_view,
                       size_t _Num_elems, size_t _Elem_size,
                       bool _Is_temp = false);

  // Creates a temp staging buffer of the requested size. This function may
  // create a staging buffer smaller than the requested size.
  _AMPIMP static _Ret_ _Buffer *__cdecl
  _Get_temp_staging_buffer(accelerator_view _Av, size_t _Requested_num_elems,
                           size_t _Elem_size);
  ...
  _AMPIMP _Event _Copy_to_async(_Inout_ _Buffer *_Dest, size_t _Num_elems,
                                size_t _Src_offset = 0,
                                size_t _Dest_offset = 0);

  //  Copy data to _Dest asynchronously.
  _AMPIMP _Event _Copy_to_async(_Inout_ _Buffer *_Dest,
  ...
  // Return the raw data ptr - only a accelerator view implementation can
  // interpret this raw pointer. This method should usually not be used in the
  // AMP header files The _Get_host_ptr is the right way for accessing the host
  // accessible ptr for a buffer
  _Ret_ void *_Get_data_ptr() const { return _M_data_ptr; }
  ...
  _Ret_ void *_Get_host_ptr() const { return _M_host_ptr; }

  size_t _Get_elem_size() const { return _M_elem_size; }

  size_t _Get_num_elems() const { return _M_num_elems; }
  ...
  bool _Owns_data() const { return _M_owns_data; }

  _AMPIMP bool _Exclusively_owns_data();

  bool _Is_staging() const { return _M_is_staging; }
  ...
  bool _Is_host_accessible(_Access_mode _Requested_access_mode) const {
    return ((_Get_allowed_host_access_mode() & _Requested_access_mode) ==
  ...
  bool _Is_temp() const { return _M_is_temp; }
  ...
  bool _Is_buffer() const { return _M_is_buffer; }

  _AMPIMP bool _Is_mappable() const;

protected:
  // The _Buffer constructor is protected to force construction through the
  // static _Create_buffer method to ensure the object is allocated in the
  // runtime
  _Buffer(
      _In_ _Accelerator_view_impl *_Av,
      _Pre_writable_byte_size_(_Elem_size *_Num_elems) void *_Buffer_data_ptr,
      void *_Host_ptr, _Access_mode _Allowed_host_access_mode,
      _Access_mode _Current_host_access_mode, size_t _Num_elems,
      size_t _Elem_size, bool _Owns_data, bool _Is_staging, bool _Is_temp,
      bool _Is_adopted);
  ...
  void _Set_host_ptr(_In_opt_ void *_Host_ptr,
  ...
  void _Set_data_ptr(_In_opt_ IUnknown *_Data_ptr) { _M_data_ptr = _Data_ptr; }
...
protected:
  ...
  // Used to determine how to map the staging buffer after its involved in a
  // copy
  bool _M_is_temp;
...
private:
  // A set of view_keys to invalidate whenever the host ptr of a staging buffer
  // is invalidated
...
public:
  ...
  _AMPIMP static _Ret_ _Texture *__cdecl
  _Create_texture(accelerator_view _Accelerator_view, unsigned int _Rank,
                  size_t _Width, size_t _Height, size_t _Depth,
                  unsigned int _Mip_levels, _Short_vector_base_type_id _Type_id,
                  unsigned int _Num_channels, unsigned int _Bits_per_channel,
                  bool _Is_temp = false);

  // Create a texture object from a pre-allocated storage on the specified
  // accelerator_view. This can be thought of as the accelerator_view "adopting"
  // the passed data buffer.
  _AMPIMP static _Ret_ _Texture *__cdecl
  _Adopt_texture(unsigned int _Rank, _Texture_base_type_id _Id,
                 _In_ IUnknown *_Data_ptr, accelerator_view _Accelerator_view,
                 unsigned int _View_format);

  // Create a staging texture on the specified accelerator_view which can be
  // accessed on the cpu upon mapping.
  _AMPIMP static _Ret_ _Texture *__cdecl
  _Create_stage_texture(accelerator_view _Accelerator_view,
                        accelerator_view _Access_on_accelerator_view,
                        unsigned int _Rank, size_t _Width, size_t _Height,
                        size_t _Depth, unsigned int _Mip_levels,
                        unsigned int _Format, bool _Is_temp = false);

  // Create a staging texture on the specified accelerator_view which can be
  // accessed on the cpu upon mapping.
  _AMPIMP static _Ret_ _Texture *__cdecl _Create_stage_texture(
      accelerator_view _Accelerator_view,
      accelerator_view _Access_on_accelerator_view, unsigned int _Rank,
      size_t _Width, size_t _Height, size_t _Depth, unsigned int _Mip_levels,
      _Short_vector_base_type_id _Type_id, unsigned int _Num_channels,
  ...
  _AMPIMP static _Ret_ _Texture *__cdecl
  _Get_temp_staging_texture(accelerator_view _Accelerator_view,
                            unsigned int _Rank, size_t _Width, size_t _Height,
                            size_t _Depth, unsigned int _Mip_levels,
  ...
  _AMPIMP static _Ret_ _Texture *__cdecl
  _Clone_texture(const _Texture *_Src,
                 const accelerator_view &_Accelerator_view,
                 const accelerator_view &_Associated_av);

  //  Copy data to _Dest asynchronously for textures. The two textures must have
  //  been created with compatible physical formats.
  _AMPIMP _Event _Copy_to_async(_Inout_ _Texture *_Dest,
  ...
  unsigned int _Get_rank() const { return _M_rank; }

  unsigned int _Get_texture_format() const { return _M_texture_format; }

  unsigned int _Get_view_format() const { return _M_view_format; }

  unsigned int _Get_num_channels() const { return _M_num_channels; }
  ...
  unsigned int
  _Get_data_length(unsigned int _Most_detailed_mipmap_level,
                   unsigned int _View_mipmap_levels,
                   const size_t *_Extents = nullptr) const // in bytes
  {
    ...
    for (unsigned int _Mip_offset = 0; _Mip_offset < _View_mipmap_levels;
         ++_Mip_offset) {
      ...
      if (_Extents) {
        ...
        case 3:
          _Depth =
              (_Extents[2] >> _Mip_level) ? (_Extents[2] >> _Mip_level) : 1U;
          // deliberately fall through
        case 2:
          _Height =
              (_Extents[1] >> _Mip_level) ? (_Extents[1] >> _Mip_level) : 1U;
          // deliberately fall through
        case 1:
          _Width =
              (_Extents[0] >> _Mip_level) ? (_Extents[0] >> _Mip_level) : 1U;
          break;
        default:
      ...
      _Total_bytes +=
          ((_Width * _Height * _Depth *
            static_cast<unsigned long long>(_Get_bits_per_element())) +
           _Bits_per_byte - 1) /
          _Bits_per_byte;
  ...
  unsigned int _Get_mip_levels() const { return _M_mip_levels; }

  size_t _Get_row_pitch() const { return _M_row_pitch; }

  void _Set_row_pitch(size_t _Val) { _M_row_pitch = _Val; }

  size_t _Get_depth_pitch() const { return _M_depth_pitch; }

  void _Set_depth_pitch(size_t _Val) { _M_depth_pitch = _Val; }

private:
  // The _Texture constructor is private to force construction through the
  // static _Create_texture method to ensure the object is allocated in the
  // runtime
  _Texture(_In_ _Accelerator_view_impl *_Av, _In_ void *_Texture_data_ptr,
           _In_opt_ void *_Host_ptr, _Access_mode _Allowed_host_access_mode,
           _Access_mode _Current_host_access_mode, unsigned int _Rank,
           size_t _Width, size_t _Height, size_t _Depth,
           unsigned int _Mip_levels, unsigned int _Texture_format,
           unsigned int _View_format, unsigned int _Num_channels,
           unsigned int _Bits_per_channel, bool _Owns_data, bool _Is_staging,
           bool _Is_temp, bool _Is_adopted);
...
public:
  ...
  _AMPIMP static _Ret_ _Sampler *__cdecl
  _Create(unsigned int _Filter_mode, unsigned int _Address_mode,
          float _Border_r, float _Border_g, float _Border_b, float _Border_a);
  ...
  // Return the raw data ptr - only an accelerator view implementation can
  // interpret this raw pointer. This method should usually not be used in the
  // AMP header files
  _Ret_ void *_Get_data_ptr() const { return _M_data_ptr; }
  ...
  unsigned int _Get_filter_mode() const { return _M_filter_mode; }

  unsigned int _Get_address_mode() const { return _M_address_mode; }

  const float *_Get_border_color() const { return &_M_border_color[0]; }

private:
  // The _Sampler constructor is private to force construction through the
  // static _Create method to ensure the object is allocated in the runtime
  _Sampler(unsigned int _Filter_mode, unsigned int _Address_mode,
           float _Border_r, float _Border_g, float _Border_b, float _Border_a);
...
_AMPIMP _Event __cdecl
_Copy_async_impl(_In_ _Texture *_Src_tex, const size_t *_Src_offset,
                 unsigned int _Src_mipmap_level, _Inout_ _Texture *_Dst_tex,
                 const size_t *_Dst_offset, unsigned int _Dst_mipmap_level,
                 const size_t *_Copy_extent,
                 const size_t *_Preferred_copy_chunk_extent = NULL);

inline bool _Get_chunked_staging_texture(_In_ _Texture *_Tex,
                                         _In_reads_(3)
                                             const size_t *_Copy_chunk_extent,
                                         _Inout_ size_t *_Remaining_copy_extent,
                                         _Out_writes_(3)
                                             size_t *_Curr_copy_extent,
                                         _Out_ _Texture_ptr *_Staging_texture) {
  bool _Truncated_copy = false;
  size_t _Allocation_extent[3] = {_Copy_chunk_extent[0], _Copy_chunk_extent[1],
                                  _Copy_chunk_extent[2]};

  unsigned int _Most_sig_idx = _Tex->_Get_rank() - 1;

  if (_Allocation_extent[_Most_sig_idx] >
      _Remaining_copy_extent[_Most_sig_idx]) {
    _Allocation_extent[_Most_sig_idx] = _Remaining_copy_extent[_Most_sig_idx];
  }

  _Texture_ptr _Stage = _Texture::_Get_temp_staging_texture(
      _Tex->_Get_accelerator_view(), _Tex->_Get_rank(), _Allocation_extent[0],
      _Allocation_extent[1], _Allocation_extent[2],
      /*_Mip_levels=*/1, _Tex->_Get_texture_format());

  std::copy(&_Allocation_extent[0], &_Allocation_extent[3],
            &_Curr_copy_extent[0]);
  size_t _Staging_tex_extent[3] = {_Stage->_Get_width(), _Stage->_Get_height(),
                                   _Stage->_Get_depth()};
  ...
  // The truncation can however happen only in the most significant dimension
  // and lower dimensions should not get truncated
  if (_Curr_copy_extent[_Most_sig_idx] <
      _Remaining_copy_extent[_Most_sig_idx]) {
...
// Suppress "warning C6101: Returning uninitialized memory '*_Dst'.:  A
// successful" "path through the function does not set the named _Out_
// parameter." The callers to _Copy_data_on_host all have static_assert that
// _Rank has to be 1, 2, or 3 dimensions for texture
//
template <typename _Input_iterator, typename _Value_type>
inline void _Copy_data_on_host_src_iter(int _Rank, _Input_iterator _Src,
                                        _Out_ _Value_type *_Dst, size_t _Width,
  ...
  case 2: {
    ...
    for (size_t _I = 0; _I < _Height; _I++) {
      ...
      std::copy(_Src_start, _Src_end,
                reinterpret_cast<_Value_type *>(_Dst_ptr));
  ...
  case 3: {
    unsigned char *_Dst_ptr_slice_start =
...
inline void _Copy_data_on_host_dst_iter(int _Rank, const _Value_type *_Src,
                                        _Output_iterator _Dst, size_t _Width,
                                        size_t _Height, size_t _Depth,
  ...
  case 2: {
    const unsigned char *_Src_ptr =
        reinterpret_cast<const unsigned char *>(_Src);
    _Output_iterator _Dst_iter = _Dst;
    for (size_t _I = 0; _I < _Height; _I++) {
      const _Value_type *_Src_end =
          reinterpret_cast<const _Value_type *>(_Src_ptr) + _Width;

      std::copy(reinterpret_cast<const _Value_type *>(_Src_ptr), _Src_end,
                _Dst_iter);
  ...
  case 3: {
    const unsigned char *_Src_ptr_slice_start =
        reinterpret_cast<const unsigned char *>(_Src);
...
_AMPIMP size_t __cdecl
_Get_preferred_copy_chunk_size(size_t _Total_copy_size_in_bytes);

inline size_t _Get_preferred_copy_chunk_num_elems(size_t _Total_num_elems,
                                                  size_t _Elem_size) {
  size_t preferredChunkSize =
      _Get_preferred_copy_chunk_size(_Total_num_elems * _Elem_size);

  return (preferredChunkSize / _Elem_size);
}

inline void _Get_preferred_copy_chunk_extent(
    unsigned int _Rank, size_t _Width, size_t _Height, size_t _Depth,
    size_t _Bits_per_element,
    _Out_writes_(3) size_t *_Preferred_copy_chunk_extent) {
  _ASSERTE(_Preferred_copy_chunk_extent != nullptr);

  size_t requestedByteSize = static_cast<size_t>(
      (static_cast<unsigned long long>(_Width) *
       static_cast<unsigned long long>(_Height) *
       static_cast<unsigned long long>(_Depth) *
       static_cast<unsigned long long>(_Bits_per_element)) >>
      3);
  ...
  size_t preferredCopyChunkNumElems = static_cast<size_t>(
      (static_cast<unsigned long long>(preferredChunkSize) * 8U) /
      _Bits_per_element);
  ...
  case 2:
    ...
    break;
  case 3:
    _Depth = (preferredCopyChunkNumElems + (_Height * _Width) - 1) /
             (_Height * _Width);
    break;
  default:
    _ASSERTE(false);
...
// Finds the greatest common divisor of 2 unsigned integral numbers using
// Euclid's algorithm
template <typename _T> inline _T _Greatest_common_divisor(_T _M, _T _N) {
...
// Finds the least common multiple of 2 unsigned integral numbers using their
// greatest_common_divisor
template <typename _T> inline _T _Least_common_multiple(_T _M, _T _N) {
  static_assert(std::is_unsigned<_T>::value,
...
inline _Event _Copy_impl(InputIterator _SrcFirst, InputIterator _SrcLast,
                         size_t _NumElemsToCopy, _Out_ _Buffer *_Dst,
                         size_t _Dest_offset,
                         size_t _Preferred_copy_chunk_num_elems = 0) {
...
#pragma warning(disable : 6001) // Using uninitialized memory '*_Dst'
  if (((_NumElemsToCopy * sizeof(_Value_type)) +
       (_Dest_offset * _Dst->_Get_elem_size())) >
...
#pragma warning(pop)
  ...
  if (_Dst->_Is_host_accessible(_Write_access)) {
    ...
    // The _Dest is accessible on host. We just need to do a std::copy using a
    // raw pointer as OutputIterator
    _Buffer_ptr _PDestBuf = _Dst;
    _Ev = _Ev._Add_continuation(std::function<_Event()>(
        [_PDestBuf, _Dest_offset, _SrcFirst, _SrcLast]() mutable -> _Event {
          _Value_type *_DestPtr = reinterpret_cast<_Value_type *>(
              reinterpret_cast<char *>(_PDestBuf->_Get_host_ptr()) +
              (_Dest_offset * _PDestBuf->_Get_elem_size()));
    ...
    // _Dest is on a device. Lets create a temp staging buffer on the _Dest
    // accelerator_view and copy the input over We may create a staging buffer
    // of size smaller than the copy size and in that case we will perform the
    // copy as a series of smaller copies
    _Buffer_ptr _PDestBuf = _Dst;
    ...
    if (_PreferredNumElemsToCopyPerChunk == 0) {
      // If a preferred copy chunk size was not specified, lets pick one based
      // on the size of the copy
      _PreferredNumElemsToCopyPerChunk = _Get_preferred_copy_chunk_num_elems(
          _NumElemsToCopy, sizeof(_Value_type));
    }
    ...
    size_t _Lcm =
        _Least_common_multiple(_Dst->_Get_elem_size(), sizeof(_Value_type));
    ...
    do {
      ...
      _Buffer_ptr _PDestStagingBuf = _Buffer::_Get_temp_staging_buffer(
          _Dst->_Get_accelerator_view(), _AllocationNumElems,
          sizeof(_Value_type));
      ...
      if (_NumElemsToCopyRemaining <= _CurrNumElemsToCopy) {
        ...
        // least common multiple of the destination buffer's element size and
        // sizeof(_Value_type).
        _CurrNumElemsToCopy =
            (_CurrNumElemsToCopy / _AdjustmentRatio) * _AdjustmentRatio;
        std::advance(_CurrEndIter, _CurrNumElemsToCopy);
      }

      _ASSERTE((_CurrNumElemsToCopy % _AdjustmentRatio) == 0);

      // This would not actually never block since we just created this staging
      // buffer or are using a cached one that is not in use
      ...
      std::copy(
          ...
          reinterpret_cast<_Value_type *>(_PDestStagingBuf->_Get_host_ptr()));

      _Ev = _Ev._Add_event(_PDestStagingBuf->_Copy_to_async(
          _PDestBuf, _CurrNumElemsToCopy, 0, _CurrDstOffset));

      // Adjust the iterators and offsets
      _NumElemsToCopyRemaining -= _CurrNumElemsToCopy;
      _CurrDstOffset +=
          (_CurrNumElemsToCopy * sizeof(_Value_type)) / _Dst->_Get_elem_size();
      _CurrStartIter = _CurrEndIter;
...
// The std::advance method is only supported for InputIterators and hence we
// have a custom implementation which forwards to the std::advance if the
// iterator is an input iterator and uses a loop based advance implementation
// otherwise
template <typename _InputIterator, typename _Distance>
typename std::enable_if<std::is_base_of<
    std::input_iterator_tag, typename std::iterator_traits<_InputIterator>::
...
typename std::enable_if<!std::is_base_of<
    std::input_iterator_tag, typename std::iterator_traits<_OutputIterator>::
                                 iterator_category>::value>::type
...
inline _Event _Copy_impl(_In_ _Buffer *_Src, size_t _Src_offset,
                         size_t _Num_elems, OutputIterator _DestIter,
                         size_t _Preferred_copy_chunk_num_elems = 0) {
  ...
  size_t _NumElemsToCopy =
      (_Num_elems * _Src->_Get_elem_size()) / sizeof(_Value_type);
  ...
  if (_Src->_Is_host_accessible(_Read_access)) {
    ...
    // The _Src is accessible on host. We just need to do a std::copy using a
    // raw pointer as OutputIterator
    _Buffer_ptr _PSrcBuf = _Src;
    _Ev = _Ev._Add_continuation(
        std::function<_Event()>([_PSrcBuf, _Src_offset, _DestIter,
                                 _NumElemsToCopy]() mutable -> _Event {
          // The _Src is accessible on host. We just need to do a std::copy
          const _Value_type *_PFirst = reinterpret_cast<const _Value_type *>(
              reinterpret_cast<char *>(_PSrcBuf->_Get_host_ptr()) +
              (_Src_offset * _PSrcBuf->_Get_elem_size()));
    ...
    // The _Src is on the device. We need to copy it out to a temporary staging
    // array We may create a staging buffer of size smaller than the copy size
    // and in that case we will perform the copy as a series of smaller copies
    ...
    _Buffer_ptr _PSrcStagingBuf = _Buffer::_Get_temp_staging_buffer(
        _Src->_Get_accelerator_view(), _AllocationNumElems,
        sizeof(_Value_type));

    _ASSERTE(_PSrcStagingBuf != NULL);
    _ASSERTE(_PSrcStagingBuf->_Get_elem_size() == sizeof(_Value_type));

    // The total byte size of a copy chunk must be an integral multiple of both
    // the source buffer's element size and sizeof(_Value_type).
    size_t _Lcm =
        _Least_common_multiple(_Src->_Get_elem_size(), sizeof(_Value_type));
    size_t _AdjustmentRatio = _Lcm / sizeof(_Value_type);
    ...
    if (_NumElemsToCopy <= _CurrNumElemsToCopy) {
      ...
      // least common multiple of the source buffer's element size and
      // sizeof(_Value_type).
      _CurrNumElemsToCopy =
          (_CurrNumElemsToCopy / _AdjustmentRatio) * _AdjustmentRatio;
    }
    ...
    _Ev = _PSrcBuf->_Copy_to_async(_PSrcStagingBuf,
                                   (_CurrNumElemsToCopy * sizeof(_Value_type)) /
                                       _PSrcBuf->_Get_elem_size(),
                                   _Src_offset, 0);

    if (_NumElemsToCopyRemaining != 0) {
      _Ev = _Ev._Add_continuation(std::function<_Event()>(
          [_DestIter, _PSrcBuf, _PSrcStagingBuf, _CurrNumElemsToCopy,
           _NumElemsToCopyRemaining, _Src_offset,
           _PreferredNumElemsToCopyPerChunk]() mutable -> _Event {
            // Initiate an asynchronous copy of the remaining part so that this
            // part of the copy makes progress while we consummate the copying
            // of the first part
            size_t _CurrSrcOffset =
                _Src_offset + ((_CurrNumElemsToCopy * sizeof(_Value_type)) /
                               _PSrcBuf->_Get_elem_size());
            OutputIterator _CurrDestIter = _DestIter;
            _Advance_output_iterator<decltype(_CurrDestIter), size_t>(
                _CurrDestIter, _CurrNumElemsToCopy);
            _Event _Ret_ev = _Copy_impl<OutputIterator, _Value_type>(
                _PSrcBuf._Get_ptr(), _CurrSrcOffset,
            ...
            _Value_type *_PFirst = reinterpret_cast<_Value_type *>(
      ...
      _Ev = _Ev._Add_continuation(
          std::function<_Event()>([_DestIter, _PSrcStagingBuf,
...
_AMPIMP _Event __cdecl _Copy_impl(_In_ _Buffer *_Src,
                                  _View_shape_ptr _Src_shape,
                                  _Out_ _Buffer *_Dst,
                                  _View_shape_ptr _Dst_shape);

struct _Array_copy_desc {
  _Array_copy_desc(const unsigned int _Rank,
...
// Declaration
_AMPIMP HRESULT __cdecl _Recursive_array_copy(
    const _Array_copy_desc &_Desc, unsigned int _Native_copy_rank,
    std::function<HRESULT(const _Array_copy_desc &_Reduced)> _Native_copy_func);

_AMPIMP std::pair<accelerator_view, accelerator_view> __cdecl
_Get_src_dest_accelerator_view(
    _In_opt_ const _Buffer_descriptor *_SrcBuffDescPtr,
...
inline _Event _Copy_impl_iter(_InputInterator _SrcFirst,
...
// Iterator based copy function
template <typename InputIterator, typename _Value_type>
inline _Event _Copy_impl(InputIterator _SrcFirst, _View_shape_ptr _Src_shape,
                         _Inout_ _Buffer *_Dst, _View_shape_ptr _Dst_shape) {
  _ASSERTE(_Dst != NULL);
  ...
  _ASSERTE(_View_shape::_Compare_extent_with_elem_size(
      _Src_shape->_Get_rank(), _Src_shape->_Get_view_extent(),
      sizeof(_Value_type), _Dst_shape->_Get_view_extent(),
      _Dst->_Get_elem_size()));
  ...
  if (_Dst->_Is_host_accessible(_Write_access)) {
    ...
    return _Ev._Add_continuation(std::function<_Event()>(
        [_SrcFirst, _Src_shape, _PDestBuf, _Dst_shape]() mutable -> _Event {
          return _Copy_impl_iter(
              _SrcFirst, _Src_shape,
              reinterpret_cast<_Value_type *>(_PDestBuf->_Get_host_ptr()),
              _Create_reinterpreted_shape(_Dst_shape,
                                          _PDestBuf->_Get_elem_size(),
                                          sizeof(_Value_type)));
        }));
  } else {
    ...
    _Buffer_ptr _PTempStagingBuf = _Buffer::_Create_stage_buffer(
        _Dst->_Get_accelerator_view(),
        accelerator(accelerator::cpu_accelerator).default_view,
        _Src_shape->_Get_view_size(), sizeof(_Value_type), true /* _Is_temp */);

    _PTempStagingBuf->_Map_buffer(_Write_access, true /* _Wait */);
    _Value_type *_Dst_ptr =
        reinterpret_cast<_Value_type *>(_PTempStagingBuf->_Get_host_ptr());
    ...
    _Buffer_ptr _PDestBuf = _Dst;
    return _Ev._Add_continuation(
        std::function<_Event()>([_PTempStagingBuf, _Src_shape, _PDestBuf,
                                 _Dst_shape]() mutable -> _Event {
          return _Copy_impl(_PTempStagingBuf, _Src_shape, _PDestBuf,
                            _Dst_shape);
        }));
  }
}
...
inline _Event _Copy_impl(_In_ _Buffer *_Src, _View_shape_ptr _Src_shape,
                         OutputIterator _DestIter, _View_shape_ptr _Dst_shape) {
  _ASSERTE(_Src != NULL);
  _ASSERTE(_Src_shape != NULL);
  _ASSERTE(_Dst_shape != NULL);
  ...
  _ASSERTE(_View_shape::_Compare_extent_with_elem_size(
      _Src_shape->_Get_rank(), _Src_shape->_Get_view_extent(),
      _Src->_Get_elem_size(), _Dst_shape->_Get_view_extent(),
      sizeof(_Value_type)));
  ...
  if (_Src->_Is_host_accessible(_Read_access)) {
    ...
    return _Ev._Add_continuation(std::function<_Event()>(
        [_PSrcBuf, _Src_shape, _DestIter, _Dst_shape]() mutable -> _Event {
          return _Copy_impl_iter(
              reinterpret_cast<_Value_type *>(_PSrcBuf->_Get_host_ptr()),
              _Create_reinterpreted_shape(
    ...
    // staging buffer on the source buffer's accelerator_view and initiate a
    // copy from the source buffer to the temporary staging buffer
    _Buffer_ptr _PTempStagingBuf = _Buffer::_Create_stage_buffer(
        _Src->_Get_accelerator_view(),
        accelerator(accelerator::cpu_accelerator).default_view,
        _Dst_shape->_Get_view_size(), sizeof(_Value_type), true);

    _Event _Ev = _Src->_Copy_to_async(_PTempStagingBuf, _Src_shape, _Dst_shape);
    return _Ev._Add_continuation(std::function<_Event()>(
        [_PTempStagingBuf, _Dst_shape, _DestIter]() mutable -> _Event {
          return _Copy_impl_iter(reinterpret_cast<_Value_type *>(
...
template <typename _InputInterator, typename _OutputIterator>
inline _Event
_Copy_impl_iter(_InputInterator _SrcIter, _View_shape_ptr _Src_shape,
  ...
  _ASSERTE(_Src_shape->_Get_rank() == _Dst_shape->_Get_rank());
  _ASSERTE(_View_shape::_Compare_extent(_Src_shape->_Get_rank(),
                                        _Src_shape->_Get_view_extent(),
                                        _Dst_shape->_Get_view_extent()));

  // If both the _Src_shape and _Dst_shape are linear we can be more efficient
  unsigned int _Src_linear_offset, _Src_linear_size, _Dst_linear_offset,
      _Dst_linear_size;
  ...
  _Array_copy_desc _Desc(
      _Src_shape->_Get_rank(), _Src_shape->_Get_linear_offset(),
      _Src_extent.data(), _Src_offset.data(), _Dst_shape->_Get_linear_offset(),
      _Dst_extent.data(), _Dst_offset.data(), _Copy_extent.data());

  // Note: Capturing shape pointers would be incorrect, they are valid for
  // setting up the call. They might be deleted right after this call completes.
  HRESULT hr = _Recursive_array_copy(
      _Desc, 1,
      [_SrcIter, _DstIter](const _Array_copy_desc &_Reduced) -> HRESULT {
        auto _SrcFirst = _SrcIter;
        auto _DstFirst = _DstIter;

        std::advance(_DstFirst, _Reduced._Dst_linear_offset +
                                    _Reduced._Dst_copy_offset[0]);
        std::advance(_SrcFirst, _Reduced._Src_linear_offset +
                                    _Reduced._Src_copy_offset[0]);
  ...
  if (FAILED(hr)) {
    throw Concurrency::runtime_exception("Failed to copy between buffers",
                                         E_FAIL);
  }
...
class _Ubiquitous_buffer : public _Reference_counter {
  friend _Event _Get_access_async(const _View_key _Key, accelerator_view _Av,
                                  _Access_mode _Mode, _Buffer_ptr &_Buf_ptr);
  friend _AMPIMP accelerator_view __cdecl _Select_copy_src_accelerator_view(
      _In_ _View_key _Src_view_key,
      const accelerator_view &_Dest_accelerator_view);
  friend struct _DPC_call_handle;

public:
  _AMPIMP static _Ret_ _Ubiquitous_buffer *__cdecl
  _Create_ubiquitous_buffer(size_t _Num_elems, size_t _Elem_size);

  _AMPIMP static _Ret_ _Ubiquitous_buffer *__cdecl
  _Create_ubiquitous_buffer(_Buffer_ptr _Master_buffer);

  // Register a new view on top of this _Ubiquitous_buffer
  _AMPIMP void
  _Register_view(_In_ _View_key _Key, accelerator_view _Cpu_av,
  ...
  _AMPIMP void _Register_view_copy(_In_ _View_key _New_view_key,
                                   _In_ _View_key _Existing_view_key);

  // Unregister a view currently registered with this _Ubiquitous_buffer
  _AMPIMP void _Unregister_view(_In_ _View_key _Key);

  // Obtain a specified mode of access to the specified view on the specified
  // target accelerator_view. This method also serves the purpose of determining
  // the amount of data copy expected to happen as part of this _Get_access
  // request without actually performing the copies or state updates in the
  // _Ubiquitous_buffer. This is used for reporting the implicit data copies
  // that happen when accessing array_views in C++ AMP ETW events
  _AMPIMP _Event _Get_access_async(_In_ _View_key _Key,
  ...
  bool _Has_data_source() const { return _M_has_data_source; }

private:
  // The _Ubiquitous_buffer constructors are private to force construction
  // through the static _Create_ubiquitous_buffer method to ensure the object is
  ...
  // Get access to a buffer on a specified accelerator for a specified
  // pre-registered view. If _Sync_size parameter is not null, then function
  // calculates number of bytes that we need to synchronize to get desired
  // access.
  _AMPIMP _Event _Get_access_async(_In_ _View_key _Key, accelerator_view _Av,
                                   _Access_mode _Mode, _Buffer_ptr &_Buf_ptr,
                                   _Inout_opt_ ULONGLONG *_Sync_size = NULL);

  // Commit a view to the main buffer if needed. When the _Sync_size parameter
  // is non-null this method just returns the amount of data to be copied as
  // part of the commit, without actually performing the commit
  _Event _Commit_view_async(_In_ _View_info *_Info,
                            _Inout_opt_ ULONGLONG *_Sync_size = nullptr);

  // Get the _Buffer_ptr corresponding to a specified accelerator_view. When the
  // _Create parameter is true, it creates a new _Buffer if one does not already
  // exist for that accelerator_view
  _Ret_ _Buffer *_Get_buffer(_In_ _Accelerator_view_impl *_Av,
                             bool _Create = true);
  ...
  // Determines whether the data underlying the specified view has been
  // discarded based on whether a subsuming view has the discard flag set.
  bool
  _Should_discard(const _View_info *_Info,
                  _In_opt_ const _View_key _Source_view_key = nullptr) const;
  ...
  // does the specified view require a data update on the target
  // accelerator_view to fulfil an access request
  bool _Requires_update_on_target_accelerator_view(
      const _View_info *_Info, _Access_mode _Requested_mode,
  ...
  // and flags them as "commit not needed" if that view is subsumed by another
  // view present in the commit list
  static void _Flag_redundant_commits(
      std::vector<std::pair<_View_info *, bool>> &_Commit_list);

  // This method returns the list of accelerator_views where the specified view
  // already has a valid cached copy of the data and getting read access  would
  // not incur any data movement. The _Can_access_anywhere parameter is an
  // output parameter used to indicate to the caller that the specified view can
  // be accessed on any accelerator_view without incurring any data movement.
  // This is true when there are no modified overlapping views that require
  // synchronization and the specified view has the discard_data flag set.
  // This method is used for determining the source accelerator_view for copy
  // and p_f_e operations involving array_views
  _Accelerator_view_unordered_set
  _Get_caching_info(_In_ _View_key _Key,
                    _Out_opt_ bool *_Can_access_anywhere = NULL);

  _Accelerator_view_unordered_set
  _Get_caching_info_impl(_In_ _View_key _Key,
                         _Out_opt_ bool *_Can_access_anywhere);

  _Ret_ _Accelerator_view_impl *_Determine_alternate_target_accelerator_view(
      _In_ _View_key _Key, _In_ _Accelerator_view_impl *_Original_av,
      _Access_mode _Mode);
...
private:
  ...
  // caching protocol. Also, note that some view_info may not have any live
  // registered and hence does not exist in the _M_view_map but may exist here
  // since it has uncommitted data which needs to be considered as part of the
  // cache coherence protocol to prevent modifications underlying this view from
  // being lost
...
class _D3D_interop {
public:
  _AMPIMP static _Ret_ IUnknown *__cdecl
  _Get_D3D_buffer(_In_ _Buffer *_Buffer_ptr);
  _AMPIMP static _Ret_ IUnknown *__cdecl
  _Get_D3D_texture(_In_ _Texture *_Texture_ptr);
  _AMPIMP static _Ret_ void *__cdecl
  _Get_D3D_sampler_data_ptr(_In_ IUnknown *_D3D_sampler);
  _AMPIMP static void __cdecl
  _Release_D3D_sampler_data_ptr(_In_ void *_Sampler_data_ptr);
  _AMPIMP static _Ret_ IUnknown *__cdecl
  _Get_D3D_sampler(const Concurrency::accelerator_view &_Av,
                   _In_ _Sampler *_Sampler_ptr);
};

inline _Event _Get_access_async(const _View_key _Key, accelerator_view _Av,
                                _Access_mode _Mode, _Buffer_ptr &_Buf_ptr) {
  return _Key->_Get_buffer_ptr()->_Get_access_async(_Key->_Get_view_key(), _Av,
                                                    _Mode, _Buf_ptr);
}

inline _Ret_ _View_shape *
_Get_buffer_view_shape(const _Buffer_descriptor &_Descriptor) {
  return _Descriptor._Get_buffer_ptr()->_Get_view_shape(
      _Descriptor._Get_view_key());
}

inline bool _Is_cpu_accelerator(const accelerator &_Accl) {
...
// This structure is used for storing information about resources required by
// the kernel.
enum _Resource_kind {
  RESOURCE_BUFFER = 0,
  RESOURCE_TEXTURE = 1,
...
struct _Device_resource_info {
  ...
  void *_M_desc; // Pointer to the
                 // _Buffer_descriptor/_Texture_descriptor/_Sampler_descriptor
                 // instance which underlies all device resource
  ...
  BOOL _Is_buffer() const { return (_M_resource_kind == RESOURCE_BUFFER); }

  BOOL _Is_texture() const { return (_M_resource_kind == RESOURCE_TEXTURE); }

  BOOL _Is_sampler() const { return (_M_resource_kind == RESOURCE_SAMPLER); }
...
struct _Device_const_buffer_info {
  ...
  unsigned int
      _M_is_debug_data; // Is this debug data which will be
...
struct _DPC_call_handle {
  ...
  bool _Is_buffer_aliased(_In_ void *const _Buffer_ptr) const {
    return ((_M_aliased_buffer_set != nullptr) &&
            (_M_aliased_buffer_set->find(_Buffer_ptr) !=
             _M_aliased_buffer_set->end()));
  }

  bool _Is_buffer_unaccessed(size_t const _Buffer_idx) const {
    return ((_M_is_device_buffer_unaccessed != nullptr) &&
            _M_is_device_buffer_unaccessed->operator[](_Buffer_idx));
  }

  void _Set_buffer_unaccessed(size_t _Buffer_idx) {
    if (_M_is_device_buffer_unaccessed == nullptr) {
      _M_is_device_buffer_unaccessed = std::unique_ptr<std::vector<bool>>(
          new std::vector<bool>(_M_num_resources, false));
    }
...
_AMPIMP _Ret_ _DPC_call_handle *HELPERAPI
__dpc_create_call_handle(_In_ _Host_Scheduling_info *_Sch_info) noexcept(false);

// Destroy the call handle
_AMPIMP void HELPERAPI
__dpc_release_call_handle(_In_ _DPC_call_handle *_Handle) noexcept(false);

_AMPIMP void HELPERAPI
__dpc_set_device_resource_info(_In_ _DPC_call_handle *_Handle,
                               _In_ _Device_resource_info *_DeviceResourceInfo,
                               size_t _NumResources) noexcept(false);

// Set const buffer info.
_AMPIMP void HELPERAPI __dpc_set_const_buffer_info(
    _In_ _DPC_call_handle *_Handle,
    _In_ _Device_const_buffer_info *_DeviceConstBufferInfo,
    size_t _NumConstBuffers) noexcept(false);

// Set the kernel shader info
_AMPIMP void HELPERAPI __dpc_set_kernel_shader_info(
    _In_ _DPC_call_handle *_Handle, _Inout_ void *_ShaderBlobs) noexcept(false);
// Set kernel dispatch info
_AMPIMP void HELPERAPI __dpc_set_kernel_dispatch_info(
    _In_ _DPC_call_handle *_Handle, unsigned int _ComputeRank,
    _In_ int *_Extents, unsigned int _GroupRank,
    const unsigned int *_GroupExtents, unsigned int &_GroupCountX,
    unsigned int &_GroupCountY, unsigned int &_GroupCountZ) noexcept(false);

// Dispatch the kernel
_AMPIMP void HELPERAPI
__dpc_dispatch_kernel(_In_ _DPC_call_handle *_Handle) noexcept(false);
...
_AMPIMP void HELPERAPI __dpc_dispatch_kernel_test(
    _In_ _DPC_call_handle *_Handle, _In_ WCHAR *szFileName,
    LPCSTR szEntryPoint) noexcept(false);
#endif
}
...
class _Amp_runtime_trace {
  ...
  friend BOOL CALLBACK _Init_amp_runtime_trace(PINIT_ONCE _Init_once,
                                               PVOID _Param,
...
public:
  ...
  // End event is triggered by multiple other events such us StartComputeEvent
  // to show exactly when given activity completed
  _AMPIMP void _Write_end_event(ULONG _Span_id);

  // Add accelerator configuration information
  // Note: This member function does not have to be exported, it is used by C++
  // AMP runtime factory
  void _Add_accelerator_config_event(PVOID _Accelerator_id,
                                     LPCWSTR _Device_path,
                                     LPCWSTR _Device_description);

  // Used by callback function, to write all configuration data when new session
  // is detected Note: This member function does not have to be exported, it is
  // used by C++ AMP runtime factory
  void _Write_all_accelerator_config_events();

  // Started accelerator_view::wait operation
  // Note: This member function does not have to be exported, it is used by C++
  // AMP runtime factory
  ULONG _Start_accelerator_view_wait_event(PVOID _Accelerator_id,
  ...
  // Below are set of helpers that take various types that were available at
  // event injection point and extract all necessary data
  _AMPIMP ULONG
  _Start_parallel_for_each_event_helper(_In_ _DPC_call_handle *_Handle);

  // This helper wraps functor with wait start and wait end events
  inline concurrency::completion_future
  _Start_async_op_wait_event_helper(ULONG _Async_op_id, _Event _Ev) {
    std::shared_future<void> retFuture;
    concurrency::task_completion_event<void> retTaskCompletionEvent;

    // Create a std::shared_future by creating a deferred task through
    // std::async that waits for the event _Ev to finish. Wrap functor with
    ...
    retFuture = std::async(std::launch::deferred, [=]() mutable {
      try {
        if (_Async_op_id == _Amp_runtime_trace::_M_event_disabled) {
          ...
          auto _Span_id = details::_Get_amp_trace()->_Start_async_op_wait_event(
              _Async_op_id);
          _Ev._Get();
          details::_Get_amp_trace()->_Write_end_event(_Span_id);
        }
      } catch (...) {
        // If an exception is encountered when executing the asynchronous
        // operation we should set the exception on the retTaskCompletionEvent
        // so that it is appropriately canceled and the exception is propagated
        // to continuations
    ...
    // Lets issue a continuation just to swallow any exceptions that are
    // encountered during the async operation and are never observed by the user
    // or are just observed through the shared_future and not through the task
  ...
  _AMPIMP ULONG _Start_array_view_synchronize_event_helper(
      const _Buffer_descriptor &_Buff_desc);
  _AMPIMP ULONG _Launch_array_view_synchronize_event_helper(
      const _Buffer_descriptor &_Buff_desc);

  // Helpers for buffers (array, array_view)
  _AMPIMP ULONG _Start_copy_event_helper(const _Buffer_descriptor &_Src,
                                         const _Buffer_descriptor &_Dest,
                                         ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Start_copy_event_helper(nullptr_t,
                                         const _Buffer_descriptor &_Dest,
                                         ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Start_copy_event_helper(const _Buffer_descriptor &_Src,
                                         nullptr_t,
                                         ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Launch_async_copy_event_helper(const _Buffer_descriptor &_Src,
                                                const _Buffer_descriptor &_Dest,
                                                ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Launch_async_copy_event_helper(nullptr_t,
                                                const _Buffer_descriptor &_Dest,
                                                ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Launch_async_copy_event_helper(const _Buffer_descriptor &_Src,
                                                nullptr_t,
  ...
  // Helper for textures
  _AMPIMP ULONG _Start_copy_event_helper(const _Texture_descriptor &_Src,
                                         nullptr_t,
                                         ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Start_copy_event_helper(nullptr_t,
  ...
  _AMPIMP ULONG _Start_copy_event_helper(const _Texture_descriptor &_Src,
                                         const _Texture_descriptor &_Dest,
                                         ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Launch_async_copy_event_helper(const _Texture_descriptor &_Src,
                                                nullptr_t,
                                                ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG
  _Launch_async_copy_event_helper(nullptr_t, const _Texture_descriptor &_Dest,
                                  ULONGLONG _Num_bytes_for_copy);
  _AMPIMP ULONG _Launch_async_copy_event_helper(
      const _Texture_descriptor &_Src, const _Texture_descriptor &_Dest,
      ULONGLONG _Num_bytes_for_copy);
...
private:
  ...
  void _Write_accelerator_config_event(
  ...
  ULONG _Start_parallel_for_each_event(
      PVOID _Accelerator_id, PVOID _Accelerator_view_id,
      BOOL _Is_tiled_explicitly, ULONGLONG _Num_of_tiles,
      ULONG _Num_of_threads_per_tile, BOOL _Is_aliased,
      ULONG _Num_read_only_resources, ULONG _Num_read_write_resources,
      ULONGLONG _Size_of_all_resouces, ULONG _Size_of_const_data,
      ULONGLONG _Size_of_data_for_copy);

  // Synchronous copy operation has started
  ULONG _Start_copy_event(PVOID _Src_accelerator_id,
                          PVOID _Src_accelerator_view_id,
                          PVOID _Dst_accelerator_id,
                          PVOID _Dst_accelerator_view_id,
                          ULONGLONG _Num_bytes_for_copy, BOOL _Is_src_staging,
                          BOOL _Is_dst_staging);

  // Asynchronous copy operation has been launched
  ULONG _Launch_async_copy_event(PVOID _Src_accelerator_id,
                                 PVOID _Src_accelerator_view_id,
                                 PVOID _Dst_accelerator_id,
                                 PVOID _Dst_accelerator_view_id,
                                 ULONGLONG _Num_bytes_for_copy,
                                 BOOL _Is_src_staging, BOOL _Is_dst_staging);
  ...
  ULONG
  _Start_array_view_synchronize_event(ULONGLONG _Num_bytes_to_synchronize);

  // Async array_view::synchronize operation has been launched
  ULONG
  _Launch_array_view_synchronize_event(ULONGLONG _Num_bytes_to_synchronize);

  // Helper function that extracts information from buffer descriptor
  std::tuple<PVOID, PVOID, BOOL>
  _Get_resource_diagnostic_info(const _Buffer_descriptor &_Buff_desc,
                                accelerator_view _Accl_view) const;

  // Helper function that extracts information from texture descriptor
  std::tuple<PVOID, PVOID, BOOL>
  _Get_resource_diagnostic_info(const _Texture_descriptor &_Tex_desc) const;

  // Generates unique identifiers for span_id and async_op_id
  ULONG _Get_unique_identifier();

  // Critical section object used by callback function to synchronize following
  // situations: a) multiple sessions have started at the same time b) C++ AMP
  // Runtime factory adds new accelerator config event to the collection
  Concurrency::critical_section _M_critical_section;

  // Collection of all configuration events at the time of C++ AMP Runtime
  // initialization
