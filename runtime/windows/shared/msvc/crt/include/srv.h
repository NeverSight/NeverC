...
typedef struct srv_datetime { // Format for SRVDATETIME
  long dtdays;                // number of days since 1/1/1900
  unsigned long dttime;       // number 300th second since mid
} DBDATETIME;

typedef struct srv_money { // Format for SRVMONEY
  long mnyhigh;
...
#define SRV_CONNECT_V7                                                         \
  16 // TDS type for TDS 7 clients.  Overloaded with SRV_ATTENTION_ACK
#define SRV_SKIP 17
...
#define SRV_DUPLICATE_HANDLER                                                  \
  2 // additional return value for srv_pre/post_handle
...
int __cdecl srv_describe(SRV_PROC *, int, char *, int, long int, long int,
...
int __cdecl srv_convert(SRV_PROC *, int, void *, long int, int, void *,
                        long int);
...
long __cdecl srv_langcpy(SRV_PROC *srvproc, long start, long nbytes,
                         char *buffer);
long __cdecl srv_langlen(SRV_PROC *srvproc);
void *__cdecl srv_langptr(SRV_PROC *srvproc);

int __cdecl srv_log(SRV_SERVER *server, BOOL datestamp, char *msg, int msglen);
...
int __cdecl srv_returnval(SRV_PROC *srvproc, char *valuename, int len,
                          BYTE status, long int type, long int maxlen,
                          long int datalen, void *value);
...
int __cdecl srv_wsendmsg(SRV_PROC *srvproc, long int msgnum, BYTE msgclass,
                         WCHAR *message, // message text
                         int msglen);    // length of message text

int __cdecl srv_sendmsg(SRV_PROC *srvproc, int msgtype, long int msgnum,
...
int __cdecl srv_pre_handle(SRV_SERVER *server, SRV_PROC *srvproc,
                           long int event, EventHandler handler, BOOL remove);
