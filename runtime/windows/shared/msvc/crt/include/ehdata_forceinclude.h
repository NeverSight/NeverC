...
// This file is preprocessed and then the preprocessed results are
...
#pragma warning(disable                                                        \
                : 4200) // nonstandard extension used: array of runtime bound
...
//				be shared between objects thrown by reference
// but with varying 				qualifiers.
...
#endif
  PMD thisDisplacement; // Pointer to instance of catch type within thrown
                        // object.
...
typedef const struct _s_ThrowInfo {
  ...
  PMFN pmfnUnwind; // Destructor to call when exception has been handled or
                   // aborted
#if _EH_RELATIVE_TYPEINFO && !defined(BUILDING_C1XX_FORCEINCLUDE)
  int pForwardCompat; // Image relative offset of Forward compatibility frame
                      // handler
  ...
  CatchableTypeArray
      *pCatchableTypeArray; // Pointer to list of pointers to types
#endif
} ThrowInfo;
...
// compiler (see macro above); since this prototype is known to the FE along
// with the pre-injected types, it has to match exactly.
//
extern "C" __declspec(noreturn) void __stdcall
_CxxThrowException(void *pExceptionObject,
                   _ThrowInfo *pThrowInfo) noexcept(false);

extern "C" int __cdecl __CxxExceptionFilter(void *ppExcept, void *pType,
                                            int adjectives, void *pBuildObj);

#ifdef prepifdef
prepifdef _MANAGED int __clrcall ___CxxExceptionFilter(void *ppExcept,
                                                       void *pType,
                                                       int adjectives,
                                                       void *pBuildObj);
prependif // _MANAGED
#else
#ifdef _MANAGED
int __clrcall ___CxxExceptionFilter(void *ppExcept, void *pType, int adjectives,
                                    void *pBuildObj);
#endif // _MANAGED
#endif

    // Returns true if the object is really a C++ exception
    // If it is, stores the previous exception in *storage, and saves the
    // current one This is needed to keep track of the current exception object
    // (used for rethrow & destruction)
    extern "C" int __cdecl __CxxRegisterExceptionObject(void *exception,
                                                        void *storage);

#ifdef prepifdef
prepifdef _MANAGED int __clrcall ___CxxRegisterExceptionObject(void *exception,
                                                               void *storage);
prependif // _MANAGED
#else
#ifdef _MANAGED
int __clrcall ___CxxRegisterExceptionObject(void *exception, void *storage);
    ...
    // This is needed, so Unregister can know whether or not to destroy the
    // object
    extern "C" int __cdecl __CxxDetectRethrow(void *exception);

#ifdef prepifdef
prepifdef _MANAGED int __clrcall ___CxxDetectRethrow(void *exception);
prependif // _MANAGED
#else
#ifdef _MANAGED
int __clrcall ___CxxDetectRethrow(void *exception);
    ...
    // Returns the byte count of stack space required to store the exception
    // info
    extern "C" int __cdecl __CxxQueryExceptionSize(void);

#ifdef prepifdef
prepifdef _MANAGED int __clrcall ___CxxQueryExceptionSize(void);
prependif // _MANAGED
#else
#ifdef _MANAGED
int __clrcall ___CxxQueryExceptionSize(void);
    ...
    extern "C" void __cdecl __CxxUnregisterExceptionObject(void *storage,
                                                           int rethrow);

#ifdef prepifdef
prepifdef _MANAGED void __clrcall ___CxxUnregisterExceptionObject(void *storage,
                                                                  int rethrow);
prependif // _MANAGED
#else
#ifdef _MANAGED
