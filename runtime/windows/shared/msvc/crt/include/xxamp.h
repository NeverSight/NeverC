...
#define _CPP_AMP_VERIFY_RANK(_Rank, _Type_name)                                \
  static_assert(((_Rank) > 0) && ((_Rank) <= CPP_AMP_MAX_RANK),                \
                "The _Rank of " #_Type_name                                    \
                " should be greater than 0 and <= 128.")

// This macro is used to determine whether a specified iterator is a mutable
// iterator by ensuring that the iterator_category iterator_trait is or inherits
// from either the output_iterator_tag or forward_iterator_tag. We use a
// static_assert to emit a compilation error when the iterator does not meet
// this requirement. This macro is used for verifying that the destination
// iterator passed to the concurrency::copy overloads is of the right type.
#define _CPP_AMP_VERIFY_MUTABLE_ITERATOR(_Type_name)                           \
  static_assert((std::is_base_of<std::output_iterator_tag,                     \
                                 typename std::iterator_traits<                \
                                     _Type_name>::iterator_category>::value || \
...
//=============================================================================
void __dp_read_texture(const void * /* pTex */, _Out_ void * /* pRet */,
                       unsigned int /* x */, unsigned int /* y */,
                       unsigned int /* z */,
                       unsigned int /* Miplevel */) __GPU_ONLY;

void __dp_write_texture(_Out_ void * /* pTex */, const void * /* pValue */,
                        unsigned int /* x */, unsigned int /* y */,
                        unsigned int /* z */) __GPU_ONLY;

void __dp_sample_texture(
    const void * /* pTex */, const void * /* pSampler */,
    _Out_ void * /* pRet */, float /* x */, float /* y */, float /* z */,
    unsigned int /* kind: 0 - GatherRed, 1 - GatherGreen, 2 - GatherBlue, 3 -
                    GatherAlpha, 4 - Sample */
    ,
    float /* LevelOfDetail */) __GPU_ONLY;

void __dp_sample_texture_predefined(
    const void * /* pTex */, _Out_ void * /* pRet */, float /* x */,
    float /* y */, float /* z */,
...
// forward decls
template <typename _Value_type, int _Rank> class array;

template <typename _Value_type, int _Rank> class array_view;

template <int _Dim0, int _Dim1 = 0, int _Dim2 = 0> class tiled_extent;
...
namespace details {
// forward decls
template <int _Rank, int _Element_size> class _Array_view_shape;

template <int _Rank, int _Element_size> class _Array_view_base;

// Helper class to avoid static_assertion errors on uninstantiated
// templates.
template <typename _T> struct _Falsifier {
  static const bool value = false;
};

// Helper classes for array and array_view projection
template <typename _T, int _R> class _Projection_result_type {
...
template <typename _T> class _Projection_result_type<_T, 1> {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");
...
template <typename _T, int _R> class _Const_array_view_projection_helper {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");
  static_assert(_R >= 2, "Rank must be greater than or equal to 2");

public:
  static typename _Projection_result_type<_T, _R>::_Const_result_type
  _Project0(const array_view<const _T, _R> *_Arr_view, int _I) __GPU;
};

template <typename _T> class _Const_array_view_projection_helper<_T, 1> {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");

public:
  static typename _Projection_result_type<_T, 1>::_Const_result_type
  _Project0(const array_view<const _T, 1> *_Arr_view, int _I) __GPU;
};

template <typename _T, int _R> class _Array_view_projection_helper {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");
  static_assert(_R >= 2, "Rank must be greater than or equal to 2");

public:
  static typename _Projection_result_type<_T, _R>::_Result_type
  _Project0(const array_view<_T, _R> *_Arr_view, int _I) __GPU;
};

template <typename _T> class _Array_view_projection_helper<_T, 1> {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");

public:
  static typename _Projection_result_type<_T, 1>::_Result_type
  _Project0(const array_view<_T, 1> *_Arr_view, int _I) __GPU;
};

template <typename _T, int _R> class _Const_array_projection_helper {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");
  static_assert(_R >= 2, "Rank must be greater than or equal to 2");

public:
  static typename _Projection_result_type<_T, _R>::_Const_result_type
  _Project0(const array<_T, _R> *_Array, int _I) __GPU;
};

template <typename _T> class _Const_array_projection_helper<_T, 1> {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");

public:
  static typename _Projection_result_type<_T, 1>::_Const_result_type
  _Project0(const array<_T, 1> *_Array, int _I) __GPU;
};

template <typename _T, int _R> class _Array_projection_helper {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");
  static_assert(_R >= 2, "Rank must be greater than or equal to 2");

public:
  static typename _Projection_result_type<_T, _R>::_Result_type
  _Project0(_In_ array<_T, _R> *_Array, int _I) __GPU;
};

template <typename _T> class _Array_projection_helper<_T, 1> {
  static_assert(!std::is_const<_T>::value, "const _T is not supported");

public:
  static typename _Projection_result_type<_T, 1>::_Result_type
  _Project0(_In_ array<_T, 1> *_Array, int _I) __GPU;
};
...
template <class _Tuple_type> _Tuple_type _Create_uninitialized_tuple() __GPU {
...
template <typename _T, _op_kind _Kind, int _Rank = _T::rank,
...
template <typename _T, int _Rank = _T::rank,
          bool _Unroll = (_Rank > 1 && _Rank <= LOOP_UNROLL_THRESHOLD)>
struct _product_helper;

// comparison operations
template <typename _T, _op_kind _Kind> struct _cmp_op_helper;

template <typename _T> struct _cmp_op_helper<_T, opEq> {
  static bool func(const _T &a, const _T &b) __GPU { return a == b; }
};

template <typename _T> struct _cmp_op_helper<_T, opNeq> {
  static bool func(const _T &a, const _T &b) __GPU { return a != b; }
};

template <typename _T, _op_kind _Kind> struct _cmp_op_helper {
  static bool func(const _T &a, const _T &b) __GPU {
    static_assert(_Falsifier<_T>::value, "invalid op");
    return false;
  }
};

// operations:  a op= b
template <typename _T, _op_kind _Kind> struct _compound_assign_op_helper;

template <typename _T> struct _compound_assign_op_helper<_T, opAssign> {
  static void func(_T &a, const _T &b) __GPU { a = b; }
};

template <typename _T> struct _compound_assign_op_helper<_T, opAddEq> {
  static void func(_T &a, const _T &b) __GPU { a += b; }
};

template <typename _T> struct _compound_assign_op_helper<_T, opSubEq> {
  static void func(_T &a, const _T &b) __GPU { a -= b; }
};

template <typename _T> struct _compound_assign_op_helper<_T, opMulEq> {
  static void func(_T &a, const _T &b) __GPU { a *= b; }
};

template <typename _T> struct _compound_assign_op_helper<_T, opDivEq> {
  static void func(_T &a, const _T &b) __GPU { a /= b; }
};

template <typename _T> struct _compound_assign_op_helper<_T, opModEq> {
  static void func(_T &a, const _T &b) __GPU { a %= b; }
};
...
// operations:  a = b + c
template <typename _T, _op_kind _Kind> struct _arithmetic_op_helper;

template <typename _T> struct _arithmetic_op_helper<_T, opAdd> {
  static void func(_T &a, const _T &b, const _T &c) __GPU { a = b + c; }
};

template <typename _T> struct _arithmetic_op_helper<_T, opSub> {
  static void func(_T &a, const _T &b, const _T &c) __GPU { a = b - c; }
};

template <typename _T> struct _arithmetic_op_helper<_T, opMul> {
  static void func(_T &a, const _T &b, const _T &c) __GPU { a = b * c; }
};

template <typename _T> struct _arithmetic_op_helper<_T, opDiv> {
  static void func(_T &a, const _T &b, const _T &c) __GPU { a = b / c; }
};

template <typename _T> struct _arithmetic_op_helper<_T, opMod> {
  static void func(_T &a, const _T &b, const _T &c) __GPU { a = b % c; }
};
...
template <typename _T1> struct _index_helper {
  template <typename _T2>
  static typename _T1::value_type func(const _T2 &a, int i) __GPU {
    return a[i];
  }

  static typename _T1::value_type func(typename _T1::value_type a[_T1::rank],
                                       int i) __GPU {
    return a[i];
  }

  static typename _T1::value_type func(typename _T1::value_type a,
                                       int i) __GPU {
    return a;
  }
};
...
struct _cmp_op_loop_helper<_T, _Kind, _Rank, true /* _Unroll */> {
  static bool func(const _T &a, const _T &b) __GPU {
    if (!_cmp_op_helper<typename _T::value_type, _Kind>::func(a[_Rank - 1],
                                                              b[_Rank - 1]))
      return false;
    else
...
struct _compound_assign_op_loop_helper<_T, _Kind, 1, false /* _Unroll */> {
  template <typename _T2> static void func(_T &a, const _T2 &b) __GPU {
    _compound_assign_op_helper<typename _T::value_type, _Kind>::func(
        a[0], _index_helper<_T>::func(b, 0));
  }
};
...
struct _compound_assign_op_loop_helper<_T, _Kind, _Rank, true /* _Unroll */> {
  template <typename _T2> static void func(_T &a, const _T2 &b) __GPU {
    ...
    _compound_assign_op_helper<typename _T::value_type, _Kind>::func(
        a[_Rank - 1], _index_helper<_T>::func(b, _Rank - 1));
  }
};
...
struct _compound_assign_op_loop_helper<_T, opAssign, 1, false /* _Unroll */> {
  ...
  static void func(_T &a, typename _T::value_type b) __GPU { a[0] = b; }
};
...
struct _arithmetic_op_loop_helper<_T, _Kind, 1, false /* _Unroll */> {
  ...
  static void func(_T &a, const _T1 &b, const _T2 &c) __GPU {
    _arithmetic_op_helper<typename _T::value_type, _Kind>::func(
        a[0], _index_helper<_T>::func(b, 0), _index_helper<_T>::func(c, 0));
  }
};
...
struct _arithmetic_op_loop_helper<_T, _Kind, _Rank, false /* _Unroll */> {
  ...
  static void func(_T &a, const _T1 &b, const _T2 &c) __GPU {
    for (int i = 0; i < _Rank; i++) {
      _arithmetic_op_helper<typename _T::value_type, _Kind>::func(
          a[i], _index_helper<_T>::func(b, i), _index_helper<_T>::func(c, i));
    }
  }
};
...
template <typename _T> struct _product_helper<_T, 1, false /* _Unroll */> {
...
template <typename _T1, int _Rank = _T1::rank> struct _map_index;

#pragma warning(push)
#pragma warning(disable : 4100) // unreferenced formal parameter
template <typename _T1> struct _map_index<_T1, 1> {
  template <typename _T2>
...
template <typename _T1> struct _map_index<_T1, 2> {
  template <typename _T2>
  static _T1 func(int _Flat_index, const _T2 _Base) __GPU {
    _T1 _index = _Create_uninitialized_tuple<_T1>();
    _index[1] = static_cast<unsigned int>(_Flat_index) %
                static_cast<unsigned int>(_Base[1]);
    _index[0] = static_cast<unsigned int>(_Flat_index) /
                static_cast<unsigned int>(_Base[1]);
    return _index;
  }
};

template <typename _T1> struct _map_index<_T1, 3> {
  template <typename _T2>
  static _T1 func(int _Flat_index, const _T2 _Base) __GPU {
    _T1 _index = _Create_uninitialized_tuple<_T1>();

    _index[2] = static_cast<unsigned int>(_Flat_index) %
                static_cast<unsigned int>(_Base[2]);
    _Flat_index = static_cast<unsigned int>(_Flat_index) /
                  static_cast<unsigned int>(_Base[2]);
    _index[1] = static_cast<unsigned int>(_Flat_index) %
                static_cast<unsigned int>(_Base[1]);
    _index[0] = static_cast<unsigned int>(_Flat_index) /
...
template <typename _T1> struct _map_index<_T1, 4> {
  template <typename _T2>
  static _T1 func(int _Flat_index, const _T2 _Base) __GPU {
    _T1 _index = _Create_uninitialized_tuple<_T1>();
    _index[3] = static_cast<unsigned int>(_Flat_index) %
                static_cast<unsigned int>(_Base[3]);
    _Flat_index = static_cast<unsigned int>(_Flat_index) /
                  static_cast<unsigned int>(_Base[3]);
    _index[2] = static_cast<unsigned int>(_Flat_index) %
                static_cast<unsigned int>(_Base[2]);
...
template <typename _T1, int _Rank> struct _map_index {
  template <typename _T2>
  static _T1 func(int _Flat_index, const _T2 _Base) __GPU {
    _T1 _index = _Create_uninitialized_tuple<_T1>();
    for (int i = _Rank - 1; i > 0; --i) {
      _index[i] = static_cast<unsigned int>(_Flat_index) %
                  static_cast<unsigned int>(_Base[i]);
      _Flat_index = static_cast<unsigned int>(_Flat_index) /
                    static_cast<unsigned int>(_Base[i]);
    }
    _index[0] = _Flat_index;
...
template <typename _EXT, typename _IDX, int _R> struct _contains {
...
template <typename _EXT, typename _IDX> struct _contains<_EXT, _IDX, 1> {
  static bool func(const _EXT &_Ext, const _IDX &_Idx) __GPU {
    return (_Idx[0] >= 0) & (_Idx[0] < _Ext[0]);
  }
};

template <typename _EXT, typename _IDX> struct _contains<_EXT, _IDX, 2> {
  static bool func(const _EXT &_Ext, const _IDX &_Idx) __GPU {
    return (_Idx[0] >= 0) & (_Idx[0] < _Ext[0]) & (_Idx[1] >= 0) &
           (_Idx[1] < _Ext[1]);
  }
};

template <typename _EXT, typename _IDX> struct _contains<_EXT, _IDX, 3> {
  static bool func(const _EXT &_Ext, const _IDX &_Idx) __GPU {
    return (_Idx[0] >= 0) & (_Idx[0] < _Ext[0]) & (_Idx[1] >= 0) &
           (_Idx[1] < _Ext[1]) & (_Idx[2] >= 0) & (_Idx[2] < _Ext[2]);
  }
};
...
template <typename _RES_EXT, typename _SRC_EXT, typename _RES_IDX,
          typename _SRC_IDX, int _R>
struct _project0 {
...
#pragma warning(push)
#pragma warning(disable : 4100) // unreferenced formal parameter
template <typename _T1, typename _T2> struct _Array_init_helper<_T1, _T2, 1> {
  static void func(unsigned int &_Total_extent, _T1 &_Multiplier,
                   const _T2 &_Extent) __GPU {
    return;
  }
};
#pragma warning(pop)

template <typename _T1, typename _T2> struct _Array_init_helper<_T1, _T2, 2> {
  static void func(unsigned int &_Total_extent, _T1 &_Multiplier,
                   const _T2 &_Extent) __GPU {
...
template <typename _T1, typename _T2> struct _Array_init_helper<_T1, _T2, 3> {
  static void func(unsigned int &_Total_extent, _T1 &_Multiplier,
                   const _T2 &_Extent) __GPU {
...
template <typename _T1, typename _T2> struct _Array_init_helper<_T1, _T2, 4> {
  static void func(unsigned int &_Total_extent, _T1 &_Multiplier,
                   const _T2 &_Extent) __GPU {
...
template <typename _T1, typename _T2, int _Rank> struct _Array_init_helper {
  static void func(unsigned int &_Total_extent, _T1 &_Multiplier,
                   const _T2 &_Extent) __GPU {
...
template <int _Rank, typename _T1, typename _T2> struct _Array_flatten_helper;
...
struct _Array_flatten_helper<3, _T1, _T2> {
  static _T2 func(const _T1 *_Multiplier, const _T2 *_Index) __GPU {
    return ((_Multiplier[0] * _Index[0]) + (_Multiplier[1] * _Index[1]) +
            _Index[2]);
  }
};
...
template <typename _T, int _Rank> struct _Texture_read_helper;

// rank == 1
template <typename _T> struct _Texture_read_helper<_T, 1> {
  static void func(const void *_Tex_data, _Out_ void *_Val, const _T &_Index,
                   unsigned int _Mip_level) __GPU_ONLY {
    __dp_read_texture(_Tex_data, _Val, static_cast<unsigned int>(_Index[0]), 1,
                      1, _Mip_level);
  }
};

template <typename _T> struct _Texture_read_helper<_T, 2> {
  static void func(const void *_Tex_data, _Out_ void *_Val, const _T &_Index,
                   unsigned int _Mip_level) __GPU_ONLY {
    __dp_read_texture(_Tex_data, _Val, static_cast<unsigned int>(_Index[1]),
                      static_cast<unsigned int>(_Index[0]), 1, _Mip_level);
  }
};

template <typename _T> struct _Texture_read_helper<_T, 3> {
  static void func(const void *_Tex_data, _Out_ void *_Val, const _T &_Index,
                   unsigned int _Mip_level) __GPU_ONLY {
    __dp_read_texture(_Tex_data, _Val, static_cast<unsigned int>(_Index[2]),
                      static_cast<unsigned int>(_Index[1]),
                      static_cast<unsigned int>(_Index[0]), _Mip_level);
  }
};

template <typename _T, int _Rank> struct _Texture_write_helper;

// rank == 1
template <typename _T> struct _Texture_write_helper<_T, 1> {
  static void func(_Out_ void *_Tex_data, const void *_Ret,
                   const _T &_Index) __GPU_ONLY {
    __dp_write_texture(_Tex_data, _Ret, static_cast<unsigned int>(_Index[0]), 1,
                       1);
  }
};

template <typename _T> struct _Texture_write_helper<_T, 2> {
  static void func(_Out_ void *_Tex_data, const void *_Ret,
                   const _T &_Index) __GPU_ONLY {
    __dp_write_texture(_Tex_data, _Ret, static_cast<unsigned int>(_Index[1]),
                       static_cast<unsigned int>(_Index[0]), 1);
  }
};

template <typename _T> struct _Texture_write_helper<_T, 3> {
  static void func(_Out_ void *_Tex_data, const void *_Ret,
                   const _T &_Index) __GPU_ONLY {
    __dp_write_texture(_Tex_data, _Ret, static_cast<unsigned int>(_Index[2]),
                       static_cast<unsigned int>(_Index[1]),
                       static_cast<unsigned int>(_Index[0]));
  }
};

template <typename _T, int _Rank> struct _Texture_sample_helper;

// rank == 1
template <typename _T> struct _Texture_sample_helper<_T, 1> {
  static void func(const void *_Tex_data, const void *_Sampler,
                   _Out_ void *_Val, const _T &_Coord, unsigned int _Kind,
                   float _Level_of_detail) __GPU_ONLY {
    __dp_sample_texture(_Tex_data, _Sampler, _Val, _Coord, 1.0f, 1.0f, _Kind,
                        _Level_of_detail);
  }
};

template <typename _T> struct _Texture_sample_helper<_T, 2> {
  static void func(const void *_Tex_data, const void *_Sampler,
                   _Out_ void *_Val, const _T &_Coord, unsigned int _Kind,
                   float _Level_of_detail) __GPU_ONLY {
    __dp_sample_texture(_Tex_data, _Sampler, _Val, _Coord.x, _Coord.y, 1.0f,
                        _Kind, _Level_of_detail);
  }
};

template <typename _T> struct _Texture_sample_helper<_T, 3> {
  static void func(const void *_Tex_data, const void *_Sampler,
                   _Out_ void *_Val, const _T &_Coord, unsigned int _Kind,
                   float _Level_of_detail) __GPU_ONLY {
    __dp_sample_texture(_Tex_data, _Sampler, _Val, _Coord.x, _Coord.y, _Coord.z,
                        _Kind, _Level_of_detail);
  }
};

template <typename _T, int _Rank> struct _Texture_predefined_sample_helper;

// rank == 1
template <typename _T> struct _Texture_predefined_sample_helper<_T, 1> {
  static void func(const void *_Tex_data, _Out_ void *_Val, const _T &_Coord,
                   unsigned int _Predefined_sampler_id, unsigned int _Kind,
                   float _Level_of_detail) __GPU_ONLY {
    __dp_sample_texture_predefined(_Tex_data, _Val, _Coord, 1.0f, 1.0f,
                                   _Predefined_sampler_id, _Kind,
                                   _Level_of_detail);
  }
};

template <typename _T> struct _Texture_predefined_sample_helper<_T, 2> {
  static void func(const void *_Tex_data, _Out_ void *_Val, const _T &_Coord,
                   unsigned int _Predefined_sampler_id, unsigned int _Kind,
                   float _Level_of_detail) __GPU_ONLY {
    __dp_sample_texture_predefined(_Tex_data, _Val, _Coord.x, _Coord.y, 1.0f,
                                   _Predefined_sampler_id, _Kind,
                                   _Level_of_detail);
  }
};

template <typename _T> struct _Texture_predefined_sample_helper<_T, 3> {
  static void func(const void *_Tex_data, _Out_ void *_Val, const _T &_Coord,
                   unsigned int _Predefined_sampler_id, unsigned int _Kind,
                   float _Level_of_detail) __GPU_ONLY {
    __dp_sample_texture_predefined(_Tex_data, _Val, _Coord.x, _Coord.y,
                                   _Coord.z, _Predefined_sampler_id, _Kind,
                                   _Level_of_detail);
  }
};
...
_Get_buffer_descriptor(const _Array_type &_Array) __GPU {
...
// object which gets access to the underlying buffer on the array's
// accelerator_view by synchronizing any pending modifications on other
// accelerator_views made through array_views on the array
template <typename _Array_type>
_Event _Get_access_async(const _Array_type &_Array, _Access_mode _Mode,
...
const _Texture_descriptor &
_Get_texture_descriptor(const _Texture_type &_Tex) __GPU {
...
static void _Is_positive(const _T<_Rank> &_Tuple) __CPU_ONLY {
  ...
  if (!valid) {
    throw runtime_exception("Invalid - values for each dimension must be > 0",
                            E_INVALIDARG);
  }
}

// The GPU version is a no-op for since there is no good exception-like
// mechanism on GPU
template <int _Rank, template <int> class _T>
...
static void _Is_valid_section(const _T2<_Rank> &_Base_extent,
  ...
  for (int i = 0; i < _Rank; ++i) {
    if ((_Section_origin[i] + _Section_extent[i]) > _Base_extent[i]) {
      throw runtime_exception(
          "the specified section index and extent are out of bound",
          E_INVALIDARG);
    }
  }
}
...
template <int _Rank, template <int> class _T1, template <int> class _T2>
static void
_Is_valid_section(const _T2<_Rank> & /*_Base_extent*/,
                  const _T1<_Rank> & /*_Section_origin*/,
                  const _T2<_Rank> & /*_Section_extent*/) __GPU_ONLY {};

template <int _Rank, template <int> class _T1>
static void _Is_valid_projection(int _I,
                                 const _T1<_Rank> &_Base_extent) __CPU_ONLY {
  if ((_I < 0) || (_I >= _Base_extent[0])) {
    throw runtime_exception("the specified projection index is out of bound",
                            E_INVALIDARG);
  }
}
...
template <int _Rank, template <int> class _T1>
static void
_Is_valid_projection(int /*_I*/,
                     const _T1<_Rank> & /*_Base_extent*/) __GPU_ONLY {}
...
#pragma warning(pop)
  ...
  if (!totalSizeValid) {
    throw runtime_exception("Invalid - extent size exceeds UINT_MAX",
                            E_INVALIDARG);
  }
}
...
inline unsigned int _Get_max_mipmap_levels(const extent<_Rank> &_Extent) {
  ...
  for (int _I = 1; _I < _Rank; ++_I) {
    _Max_dim = static_cast<unsigned int>(_Extent[_I]) > _Max_dim
                   ? static_cast<unsigned int>(_Extent[_I])
                   : _Max_dim;
  }
...
inline void
_Are_valid_mipmap_parameters(unsigned int _Most_detailed_mipmap_level,
                             unsigned int _Mipmap_levels = 0) {
  if (_Most_detailed_mipmap_level >= 32) {
    throw runtime_exception(
        "The most detailed mipmap level cannot be greater than or equal to 32",
        E_INVALIDARG);
  }
...
template <int _Rank>
inline extent<_Rank>
_Get_extent_at_level_unsafe(const extent<_Rank> &_Base_extent,
                            unsigned int _Level) __GPU;

template <int _Rank>
inline extent<_Rank> _Get_extent_at_level(const extent<_Rank> &_Base_extent,
                                          unsigned int _Level);

// Validate most detailed level and mipmap levels of the new view, given number
// of mipmap levels on the source
inline void _Is_valid_mipmap_range(unsigned int _Src_view_mipmap_levels,
                                   unsigned int _Dst_most_detailed_level,
                                   unsigned int _Dst_view_mipmap_levels) {
  _Are_valid_mipmap_parameters(_Dst_most_detailed_level,
                               _Dst_view_mipmap_levels);

  if (_Dst_view_mipmap_levels == 0 ||
      _Src_view_mipmap_levels <
          _Dst_most_detailed_level + _Dst_view_mipmap_levels) {
    throw runtime_exception("Invalid texture mipmap range", E_INVALIDARG);
  }
}

// _Parallel_for_each declarations used by the Concurrency::parallel_for_each
// functions
template <int _Rank, typename _Kernel_type>
void _Parallel_for_each(_In_ _Host_Scheduling_info *_Sch_info,
                        extent<_Rank> _Compute_domain, const _Kernel_type &_F);
...
void __dp_d3d_msad4(_Out_ unsigned int * /* pointer to the return value */,
                    unsigned int /* reference */, unsigned int /* source.x */,
                    unsigned int /* source.y */, unsigned int /* accum.x */,
                    unsigned int /* accum.y */, unsigned int /* accum.z */,
...
void __dp_stub_info(unsigned int /* x */, unsigned int /* y */,
                    unsigned int /* z */,
...
_Ret_ void *
__dp_get_texture_info(bool /* read write? */, unsigned int, /* rank */
...
void __dp_const_buffer_info(unsigned int /* unique id */,
                            unsigned int /* size */) __GPU_ONLY;

unsigned int __dp_read_const_buffer(unsigned int /* const buffer id */,
                                    unsigned int /* index */) __GPU_ONLY;

unsigned int
__dp_get_physical_id(unsigned int /* 0 - gid, 1 - tid, 2 - dtid */,
                     unsigned int /* 0 - x, 1 - y, 2 - z */) __GPU_ONLY;

// This intrinsic is used to aid line number debug info generation for C++ AMP
// kernels
...
namespace Concurrency {
namespace details {
// This function is used to aid line number debug info generation for C++ AMP
// kernels and is called by the C++ AMP FE in the generated kernel_stub for
// parallel_for_each calls.
inline void __dp_no_source_func() __GPU_ONLY { __dp_no_source_stub(); }
} // namespace details
} // namespace Concurrency
