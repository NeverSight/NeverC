...
template <class _Node, class _Alloc, template <class...> class _Base,
          class... _Types>
class _Node_handle
...
private:
  ...
  void _Clear() noexcept { // destroy any contained node and return to the empty
                           // state
    if (_Ptr != nullptr) {
      _Alloc &_Al = _Getal();
...
public:
  ...
  _Node_handle(_Node_handle &&_That) noexcept
      : _Ptr{_That._Ptr} { // steal node and allocator (if any) from _That
    if (_Ptr != nullptr) {
      _That._Ptr = nullptr;
