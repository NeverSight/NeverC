...
public:
  class _PPLTaskChore {
    ...
    static void __cdecl _Callback(void *_Args) {
      auto _Chore =
          ::std::unique_ptr<_PPLTaskChore>(static_cast<_PPLTaskChore *>(_Args));
...
#pragma warning(                                                               \
    ...
    _PPLTaskChore(TaskProc_t _Proc, void *_Param)
        : _M_Chore{&_Callback, this}, _M_proc(_Proc), _M_param(_Param) {}
#pragma warning(pop)
...
inline ::std::shared_ptr<scheduler_interface> *
_GetStaticAmbientSchedulerStorage() {
  // thread-unsafe local static OK here because we're guarded by
  // ::std::call_once in the caller
...
inline void set_ambient_scheduler(
    const ::std::shared_ptr<scheduler_interface> &_Scheduler) {
...
struct _TaskProcHandle {
  ...
  static void __cdecl _RunChoreBridge(void *_Parameter) {
    ::std::unique_ptr<_TaskProcHandle>{
        static_cast<_TaskProcHandle *>(_Parameter)}
        ->invoke();
  }
};
...
public:
  ...
  void _ScheduleTask(_TaskProcHandle_t *_Parameter,
    ...
    if (_InliningMode == _ForceInline) {
      ...
      if (_M_pScheduler) {
        ...
        _DefaultPPLTaskScheduler().schedule(_TaskProcHandle_t::_RunChoreBridge,
                                            _Parameter);
      }
    }
  }
  ...
  // Fire and forget
  static void _RunTask(TaskProc_t _Proc, void *_Parameter,
                       _TaskInliningMode _InliningMode) {
    if (_InliningMode == _ForceInline) {
      _Proc(_Parameter);
    } else {
      // Schedule the work on the default scheduler if the ambient scheduler is
      // not set
