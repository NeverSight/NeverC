...
#pragma warning(disable                                                        \
                : 4714) // forceinline function 'YieldProcessor' not inlined
#pragma warning(                                                               \
    disable : 28112) // Disable Prefast warning about variables being accessed
                     // through both interlocked and non-interlocked functions
...
// Routine Description:
//
//     The DLOAD_BYTE_OFFSET macro takes a virtual address and returns the byte
...
#define DLOAD_BYTE_OFFSET(Va, PageSize)                                        \
  ((ULONG)((LONG_PTR)(Va) & (PageSize - 1)))
...
#define DLOAD_ADDRESS_AND_SIZE_TO_SPAN_PAGES(Va, Size, PageSize)               \
  ((ULONG)((((ULONG_PTR)(Size)) / PageSize) +                                  \
           ((DLOAD_BYTE_OFFSET(Va, PageSize) +                                 \
             DLOAD_BYTE_OFFSET(Size, PageSize) + PageSize - 1) /               \
            PageSize)))
...
typedef VOID(NTAPI *AcquireSRWLockExclusiveProc)(
    _Inout_ _Acquires_exclusive_lock_(*SRWLock) SRWLOCK_TYPE *SRWLock);

typedef VOID(NTAPI *ReleaseSRWLockExclusiveProc)(
...
Done:
  OldValue = (HMODULE)InterlockedCompareExchangePointer(
      (PVOID *)&DloadKernel32, (PVOID)Kernel32, SRWLOCK_UNINITIALIZED);
...
_Acquires_lock_(DloadSrwLock) DLOAD_INLINE VOID DloadLock(VOID)
...

{
  ...
  for (;;) {
    ...
    if (InterlockedCompareExchangePointer((PVOID *)&DloadSrwLock, (PVOID)1,
                                          0) == 0) {
      break;
    }
  }
...
_Releases_lock_(DloadSrwLock) DLOAD_INLINE VOID DloadUnlock(VOID)
...
#pragma warning(suppress : 6387) // Passing 0 is valid, but WritePointerRelease
                                 // isn't annotated properly
...
_Success_(return != nullptr) DLOAD_INLINE PVOID
    DloadObtainSection(_Out_ PULONG SectionSize,
...
DECLSPEC_NOINLINE
DLOAD_INLINE
VOID DloadMakePermanentImageCommit(_Inout_updates_(Length) PVOID BaseAddress,
                                   _In_ SIZE_T Length)
...

{
  ...
  PageCount =
      DLOAD_ADDRESS_AND_SIZE_TO_SPAN_PAGES(BaseAddress, Length, PageSize);
...
DLOAD_INLINE
BOOLEAN
IsBufferInBounds(_In_ PVOID OuterPointer, _In_ SIZE_T OuterSize,
                 _In_ PVOID InnerPointer, _In_ SIZE_T InnerSize)
/*++
...
DLOAD_INLINE
VOID DloadProtectSection(_In_opt_ PVOID RangeStart, _In_ SIZE_T RangeSize,
...

{
  ...
  if (VirtualProtect(RangeStart, RangeSize, Protection, OldProtection) ==
      FALSE) {
    DLOAD_ASSERT(FALSE);
    __fastfail(FAST_FAIL_DLOAD_PROTECTION_FAILURE);
  }
...

{
  ...
  if (DloadSectionLockCount == 1) {
    DloadProtectSection(nullptr, 0, PAGE_READWRITE, &DloadSectionOldProtection);
  }
...

{
  ...
  if (DloadSectionLockCount == 0) {
    DloadProtectSection(nullptr, 0, DloadSectionOldProtection, &OldProtect);
  }
...

{
  ...
  DloadResolveDelayLoadedAPI = (ResolveDelayLoadedAPIProc)GetProcAddress(
  ...
  DloadResolveDelayLoadsFromDll = (ResolveDelayLoadsFromDllProc)GetProcAddress(
...
PVOID
WINAPI
Dload__delayLoadHelper2(_In_ PCIMAGE_DELAYLOAD_DESCRIPTOR DelayloadDescriptor,
...

{
  ...
  *NativeHandled = TRUE;
  ...
  Symbol = DloadResolveDelayLoadedAPI(
      (PVOID)&__ImageBase, DelayloadDescriptor,
      (PDELAYLOAD_FAILURE_DLL_CALLBACK)__pfnDliFailureHook2,
      DelayLoadFailureHook, ThunkAddress, 0);
...
HRESULT
WINAPI
Dload__HrLoadAllImportsForDll(_In_ LPCSTR DllName, _Out_ PBOOLEAN NativeHandled)
...

{
  ...
  *NativeHandled = TRUE;
  ...
  Status = DloadResolveDelayLoadsFromDll((PVOID)&__ImageBase, DllName, 0);
