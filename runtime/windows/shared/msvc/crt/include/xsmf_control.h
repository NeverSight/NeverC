...
using _SMF_control_copy = conditional_t<
    ...
    conditional_t<conjunction_v<is_copy_constructible<_Types>...>,
                  _Non_trivial_copy<_Base>, _Deleted_copy<_Base>>>;

template <class _Base, class... _Types>
struct _Non_trivial_move
    : _SMF_control_copy<_Base,
                        _Types...> { // non-trivial move construction facade
...
using _SMF_control_move =
    conditional_t<conjunction_v<is_trivially_move_constructible<_Types>...>,
                  _SMF_control_copy<_Base, _Types...>,
                  conditional_t<conjunction_v<is_move_constructible<_Types>...>,
                                _Non_trivial_move<_Base, _Types...>,
                                _Deleted_move<_Base, _Types...>>>;

template <class _Base, class... _Types>
struct _Non_trivial_copy_assign
    : _SMF_control_move<_Base,
                        _Types...> { // non-trivial copy assignment facade
  ...
  _CONSTEXPR20 _Non_trivial_copy_assign &
  operator=(const _Non_trivial_copy_assign &_That) noexcept(
...
struct _Non_trivial_move_assign
    : _SMF_control_copy_assign<_Base, _Types...> { // non-trivial move
                                                   // assignment facade
  ...
  _Non_trivial_move_assign &
  operator=(const _Non_trivial_move_assign &) = default;

  _CONSTEXPR20 _Non_trivial_move_assign &
  operator=(_Non_trivial_move_assign &&_That) noexcept(
...
struct _Deleted_move_assign
    : _SMF_control_copy_assign<_Base,
                               _Types...> { // deleted move assignment facade
  using _Mybase = _SMF_control_copy_assign<_Base, _Types...>;
  using _Mybase::_Mybase;
