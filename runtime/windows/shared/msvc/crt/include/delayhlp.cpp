...
//
//
// Then, you can either link directly with this new object file, or replace the
// one in delayimp.lib with your new one, eg: lib /out:delayimp.lib delayhlp.obj
//
...
static inline VOID GuardedWrite(_In_opt_ PVOID RangeStart,
  ...
  if (bGuardedWrite) {
    ...
    DloadProtectSection(RangeStart, RangeSize, PAGE_READWRITE,
                        &dwOldProtection);
  }
  ...
  if (bGuardedWrite) {
    DloadProtectSection(RangeStart, RangeSize, dwOldProtection,
                        &dwOldProtection);
...
inline unsigned IndexFromPImgThunkData(PCImgThunkData pitdCur,
...
template <class X> X PFromRva(RVA rva) { return X(PBYTE(&__ImageBase) + rva); }
...
static inline void WINAPI OverlayIAT(PImgThunkData pitdDst,
                                     PCImgThunkData pitdSrc,
                                     unsigned countOfImports) {
  unsigned i = 0;
...
static inline bool WINAPI FLoadedAtPreferredAddress(PIMAGE_NT_HEADERS pinh,
                                                    HMODULE hmod) {
...
static PCImgDelayDescr PiddFromDllName(LPCSTR szDll) {
  ...
  //
  if (pinh->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT]
          .Size != 0) {
...
#define InterlockedExchangePointer(Target, Value)                              \
  (PVOID)(LONG_PTR)                                                            \
      InterlockedExchange((PLONG)(Target), (LONG)(LONG_PTR)(Value))

typedef unsigned long *PULONG_PTR;

#endif

extern "C" FARPROC WINAPI __delayLoadHelper2(PCImgDelayDescr pidd,
  ...
  // our own use
  //
  InternalImgDelayDescr idd = {pidd->grAttrs,
  ...
  if (0 == (idd.grAttrs & dlattrRva)) {
    ...
    RaiseException(VcppException(ERROR_SEVERITY_ERROR, ERROR_INVALID_PARAMETER),
  ...
  //
  const unsigned iIAT =
      IndexFromPImgThunkData(PCImgThunkData(ppfnIATEntry), idd.pIAT);
  ...
  if (dli.dlp.fImportByName) {
    dli.dlp.szProcName = LPCSTR(
        PFromRva<PIMAGE_IMPORT_BY_NAME>(RVA(UINT_PTR(pitd->u1.AddressOfData)))
            ->Name);
  } else {
  ...
  if (hmod == 0) {
    ...
    if (hmod == 0) {
      ...
      if (hmod == 0) {
        ...
        // If we get to here, we blindly assume that the handler of the
        // exception has magically fixed everything up and left the function
        // pointer in dli.pfnCur.
        //
        return dli.pfnCur;
      }
    }
    ...
    //
    HMODULE hmodT =
        HMODULE(InterlockedExchangePointer((PVOID *)idd.phmod, PVOID(hmod)));
    if (hmodT == hmod) {
      ::FreeLibrary(hmod);
    }
  }
...
SetEntryHookBypass:
  GuardedWrite(ppfnIATEntry, sizeof(*ppfnIATEntry),
...
extern "C" BOOL WINAPI __FUnloadDelayLoadedDLL2(LPCSTR szDll) {
  ...
  if ((pidd != NULL) && (pidd->rvaUnloadIAT != 0)) {
    HMODULE *phmod = PFromRva<HMODULE *>(pidd->rvaHmod);
    HMODULE hmod = *phmod;
    if (hmod != NULL) {

      unsigned countOfImports =
          CountOfImports(PFromRva<PImgThunkData>(pidd->rvaIAT));
      ...
      GuardedWrite(PFromRva<PImgThunkData>(pidd->rvaIAT),
                   ...
                   [pidd, countOfImports]() {
                     OverlayIAT(PFromRva<PImgThunkData>(pidd->rvaIAT),
                                PFromRva<PCImgThunkData>(pidd->rvaUnloadIAT),
                                countOfImports);
                   });
