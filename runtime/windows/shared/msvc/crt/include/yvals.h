...
// This header is used to compile the import library (via locale0_implib.cpp =>
// locale0.cpp => xfacet => yvals.h). MAJOR LIMITATIONS apply to what can be
// included here! Before editing this file, read: /docs/import_library.md
...
_EMIT_STL_ERROR(STL1005,
                "Tried to include a non-core C++ Standard Library header file "
                "with _ENFORCE_ONLY_CORE_HEADERS defined.");
...
// Code in MSVCM is built with /clr, but is used by user code built with
// /clr:pure so it must conform to the expectations of /clr:pure clients. Use
// __PURE_APPDOMAIN_GLOBAL when a global needs to be appdomain-global for pure
...
#ifdef _HAS_ITERATOR_DEBUGGING // A2i. _HAS_ITERATOR_DEBUGGING is already
                               // defined, validate it.
...
#ifdef _HAS_ITERATOR_DEBUGGING // B1i. _HAS_ITERATOR_DEBUGGING is already
                               // defined, validate it.
...
#pragma detect_mismatch("_ITERATOR_DEBUG_LEVEL",                               \
                        _STL_STRINGIZE(_ITERATOR_DEBUG_LEVEL))
...
_EMIT_STL_ERROR(STL1006, "_CONTAINER_DEBUG_LEVEL has been removed. It was "
                         "superseded by _MSVC_STL_HARDENING.");
#endif
...
_EMIT_STL_ERROR(
    STL1008, "_STL_CALL_ABORT_INSTEAD_OF_INVALID_PARAMETER has been removed. "
...
// * Uncommon case: If it returns, the STL will attempt to "continue on error",
// behaving as if no checking was done.
//   + For example, a legacy codebase with a long startup time might want to log
//   errors for investigation later.
//   + WARNING: If you replace the STL's "doom function" to "continue on error",
//   you do so at your own risk!
//     After the STL has detected a precondition violation, undefined behavior
//     is imminent. The STL will support "continue on error" by proceeding to do
//     what it would have done anyways (instead of falling off the end of a
//     non-void function, etc.), but it will not attempt to replace undefined
//     behavior with implementation-defined behavior. (For example, we will not
//     transform `pop_back()` of an empty `vector` to be a no-op.)
#ifndef _MSVC_STL_DOOM_FUNCTION
#ifdef _MSVC_STL_USE_ABORT_AS_DOOM_FUNCTION
#define _MSVC_STL_DOOM_FUNCTION(mesg) _CSTD abort()
#else // ^^^ defined(_MSVC_STL_USE_ABORT_AS_DOOM_FUNCTION) /
      // !defined(_MSVC_STL_USE_ABORT_AS_DOOM_FUNCTION) vvv
// TRANSITION, GH-4858: after dropping Win7 support, we can directly call
...
#define _STL_INTERNAL_CHECK(...)                                               \
  _STL_VERIFY(__VA_ARGS__, "STL internal check: " #__VA_ARGS__)
...
#else // ^^^ defined(_DLL) && !defined(_STATIC_CPPLIB) / !defined(_DLL) ||
      // defined(_STATIC_CPPLIB) vvv
...
extern "C++" class _CRTIMP2_PURE_IMPORT
    _Lockit { // lock while object in existence -- MUST NEST
public:
#ifdef _M_CEE_PURE
...
class _CRTIMP2_PURE_IMPORT
    _EmptyLockit { // empty lock class used for bin compat
private:
  int _Locktype;
};
#endif // defined(_M_CEE_PURE)
