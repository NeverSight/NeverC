...
public:
  ...
  reference operator*() const { return ((reference) * *(_Mybase *)this); }
...
public:
  typedef typename ::std::allocator_traits<
      _Element_allocator_type>::template rebind_alloc<_Element_type>
      _Allocator_type;
  ...
  struct _Node {
    ...
    _Split_order_key _Get_order_key() const { return _M_order_key; }
    ...
    _Nodeptr _Atomic_set_next(_Nodeptr _New_node, _Nodeptr _Current_node) {
      // Try to change the next pointer on the current element to a new element,
      // only if it still points to the cached next
      _Nodeptr _Exchange_node = (_Nodeptr)_InterlockedCompareExchangePointer(
          (void *volatile *)&_Next, _New_node, _Current_node);
    ...
    // Dummy nodes are used by buckets in the hash table to quickly index into
    // the right subsection of the split-ordered list.
    bool _Is_dummy() const { return (_M_order_key & 0x1) == 0; }
  ...
  _Split_order_list_node(_Allocator_type _Allocator)
      : _M_node_allocator(_Allocator), _M_value_allocator(_Allocator) {}
#else  /* _ITERATOR_DEBUG_LEVEL == 0 */
  _Split_order_list_node(_Allocator_type _Allocator)
      : _M_node_allocator(_Allocator), _M_value_allocator(_Allocator) {
    using _Alproxy_traits =
        typename _Al_traits::template rebind_traits<::std::_Container_proxy>;
...
class _Split_order_list_value
    : public _Split_order_list_node<_Element_type, _Element_allocator_type> {
public:
  typedef _Split_order_list_node<_Element_type, _Element_allocator_type>
      _Mybase;
  ...
  _Nodeptr _Buynode(_Split_order_key _Order_key, _ValTy &&_Value) {
    ...
    try {
      _Al_traits::construct(this->_M_value_allocator,
                            ::std::addressof(_Pnode->_Myval),
                            ::std::forward<_ValTy>(_Value));
...
class _Split_ordered_list
    : _Split_order_list_value<_Element_type, _Element_allocator_type> {
public:
  typedef _Split_ordered_list<_Element_type, _Element_allocator_type> _Mytype;
  typedef _Split_order_list_value<_Element_type, _Element_allocator_type>
      _Mybase;
  using typename _Mybase::_Al_traits;
  using typename _Mybase::_Allocator_type;
  using typename _Mybase::_Alnode_traits;
  ...
  _Split_ordered_list(allocator_type _Allocator = allocator_type())
      : _Mybase(_Allocator), _M_element_count(0) {}
  ...
  ~_Split_ordered_list() {
    ...
    _ASSERT_EXPR(_Pnode != nullptr && _Pnode->_Next == nullptr,
                 L"Invalid head list node");
  ...
  allocator_type get_allocator() const { return (this->_M_value_allocator); }
  ...
  const_iterator cbegin() const { return (((const _Mytype *)this)->begin()); }

  const_iterator cend() const { return (((const _Mytype *)this)->end()); }
  ...
  _Full_iterator _Begin() { return _Full_iterator(this->_Myhead, this); }
  ...
  _Full_iterator _End() { return _Full_iterator(0, this); }

  _Full_const_iterator _End() const { return _Full_const_iterator(0, this); }

  static _Split_order_key _Get_key(const _Full_const_iterator &_Iterator) {
    return _Iterator._Ptr->_Get_order_key();
  }

  // Returns a public iterator version of the internal iterator. Public iterator
  // must not be a dummy private iterator.
  iterator _Get_iterator(_Full_iterator _Iterator) {
    _ASSERT_EXPR(_Iterator._Ptr != nullptr && !_Iterator._Ptr->_Is_dummy(),
                 L"Invalid user node (dummy)");
  ...
  // Returns a public iterator version of a first non-dummy internal iterator at
  // or after the passed in internal iterator.
  ...
  // or after the passed in internal iterator.
  const_iterator
  _Get_first_real_iterator(_Full_const_iterator _Iterator) const {
    // Skip all dummy, internal only iterators
    while (_Iterator != _End() && _Iterator._Ptr->_Is_dummy()) {
      ++_Iterator;
    }
  ...
  // Try to insert a new element in the list. If insert fails, return the node
  // that was inserted instead.
  _Nodeptr _Insert(_Nodeptr _Previous, _Nodeptr _New_node,
                   _Nodeptr _Current_node) {
  ...
  _Pairib _Insert(_Full_iterator _Iterator, _Full_iterator _Next,
                  _Nodeptr _List_node, long *_New_count) {
  ...
  _Full_iterator _Insert_dummy(_Full_iterator _Iterator,
    ...
    // Create a dummy element up front, even though it may be discarded (due to
    ...
    for (;;) {
      ...
      // If the head iterator is at the end of the list, or past the point where
      // this dummy node needs to be inserted, then try to insert it.
      if (_Where == _Last || _Get_key(_Where) > _Order_key) {
        _ASSERT_EXPR(_Get_key(_Iterator) < _Order_key,
                     L"Invalid node order in the list");

        // Try to insert it in the right place
        _Nodeptr _Inserted_node =
            _Insert(_Iterator._Ptr, _Dummy_node, _Where._Ptr);

        if (_Inserted_node == _Dummy_node) {
          // Insertion succeeded
          return _Full_iterator(_Dummy_node, this);
        } else {
          // Insertion failed: either dummy node was inserted by another thread,
          // or a real element was inserted at exactly the same place as dummy
          // node. Proceed with the search from the previous location where
          // order key was known to be larger (note: this is legal only because
  ...
  void _Erase(_Full_iterator _Previous, _Full_const_iterator &_Where) {
#if _ITERATOR_DEBUG_LEVEL == 2
    _STL_VERIFY(_Where._Getcont() == this && _Where._Ptr != this->_Myhead,
    ...
    _ASSERT_EXPR(_Prevnode->_Next == _Pnode,
                 L"Erase must take consecutive iterators");
  ...
  void _Move_all(_Mytype &_Source_list) {
    ...
    for (_Full_const_iterator _Iterator = _First; _Iterator != _Last;) {
      ...
      _Nodeptr _Dummy_node =
          _Node->_Is_dummy() ? _Buynode(_Node->_Get_order_key())
                             : _Buynode(_Node->_Get_order_key(), _Node->_Myval);
