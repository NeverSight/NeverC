...
#if _SECURE_COMPILER_COM && defined(__GOT_SECURE_LIB__) &&                     \
    __GOT_SECURE_LIB__ >= 200402L
...
// Destructor
//
inline _bstr_t::~_bstr_t() noexcept { _Free(); }
...
//
inline _bstr_t::Data_t::~Data_t() noexcept { _Free(); }
...
public:
  ...
  _variant_t(short sSrc,
             VARTYPE vtSrc = VT_I2); // Creates a VT_I2, or a VT_BOOL
  _variant_t(long lSrc, VARTYPE vtSrc =
                            VT_I4); // Creates a VT_I4, a VT_ERROR, or a VT_BOOL
  _variant_t(float fltSrc) throw(); // Creates a VT_R4
  _variant_t(double dblSrc,
             VARTYPE vtSrc = VT_R8);   // Creates a VT_R8, or a VT_DATE
  ...
  _variant_t(IDispatch *pSrc,
             bool fAddRef = true) throw(); // Creates a VT_DISPATCH
  _variant_t(bool boolSrc) throw();        // Creates a VT_BOOL
  _variant_t(IUnknown *pSrc,
             bool fAddRef = true) throw();   // Creates a VT_UNKNOWN
  ...
  operator unsigned __int64()
      const; // Extracts a unsigned __int64 from a VT_UI8
#endif
...
inline _variant_t &_variant_t::operator=(const _bstr_t &bstrSrc) {
  _COM_ASSERT(V_VT(this) != VT_BSTR || (BSTR)bstrSrc == NULL ||
...
inline _variant_t &_variant_t::operator=(const char *pSrc) {
  _COM_ASSERT(V_VT(this) != (VT_I1 | VT_BYREF) || pSrc == NULL ||
              V_I1REF(this) != pSrc);
...
inline _variant_t &_variant_t::operator=(IUnknown *pSrc) {
  _COM_ASSERT(V_VT(this) != VT_UNKNOWN || pSrc == NULL ||
              V_UNKNOWN(this) != pSrc);
...
inline _variant_t::~_variant_t() noexcept { ::VariantClear(this); }
