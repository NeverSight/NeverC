...
public:
  ...
  F32vec4(float _F3, float _F2, float _F1, float _F0) {
    vec = _mm_set_ps(_F3, _F2, _F1, _F0);
  }
  ...
  F32vec4 &operator=(const __m128 &_Avec) {
    vec = _Avec;
    return *this;
  }
  ...
  friend F32vec4 rcp_nr(const F32vec4 &_A) {
    ...
    return _mm_sub_ps(_mm_add_ps(_Ra0, _Ra0),
                      _mm_mul_ps(_mm_mul_ps(_Ra0, _A), _Ra0));
  }
...
#define Fvec32s4_COMP(op)                                                      \
  friend F32vec4 cmp##op(const F32vec4 &_A, const F32vec4 &_B) {               \
    return _mm_cmp##op##_ps(_A, _B);                                           \
  }
...
#undef Fvec32s4_COMP
      ...
      /* Min and Max */
      friend F32vec4 simd_min(const F32vec4 &_A, const F32vec4 &_B) {
  ...
  friend F32vec4 abs(const F32vec4 &_A) {
    return _mm_and_ps(_A, _f32vec4_abs_mask);
  }
  ...
  const float &operator[](int _I) const {
    ...
    _VEC_ASSERT((0 <= _I) &&
                (_I <= 3)); /* User should only access elements 0-3 */
    float *_Fp = (float *)&vec;
    return *(_Fp + _I);
  }
 ...
 */
inline int move_mask(const F32vec4 &_A) { return _mm_movemask_ps(_A); }

/* Data Motion Functions */

/* Load Unaligned loadu_ps: Unaligned */
inline void loadu(F32vec4 &_A, float *_P) { _A = _mm_loadu_ps(_P); }

/* Store Temporal storeu_ps: Unaligned */
inline void storeu(float *_P, const F32vec4 &_A) { _mm_storeu_ps(_P, _A); }

/* Cacheability Support */

/* Non-Temporal Store */
inline void store_nta(float *_P, const F32vec4 &_A) { _mm_stream_ps(_P, _A); }

/* Conditional Selects:*/
/*(a OP b)? c : d; where OP is any compare operator
Macros expand to conditional selects which use all compare intrinsics.
Example:
friend F32vec4 select_eq(const F32vec4 &_A, const F32vec4 &_B, const F32vec4
&_C, const F32vec4 &_D)
{
...
#define Fvec32s4_SELECT(op)                                                    \
  inline F32vec4 select_##op(const F32vec4 &_A, const F32vec4 &_B,             \
                             const F32vec4 &_C, const F32vec4 &_D) {           \
...
#if defined(_M_IX86)
    /* Max and Min */
    inline Is16vec4 simd_max(const Is16vec4 &_A, const Is16vec4 &_B) {
  return _m_pmaxsw(_A, _B);
}
inline Is16vec4 simd_min(const Is16vec4 &_A, const Is16vec4 &_B) {
  return _m_pminsw(_A, _B);
}
inline Iu8vec8 simd_max(const Iu8vec8 &_A, const Iu8vec8 &_B) {
  return _m_pmaxub(_A, _B);
}
inline Iu8vec8 simd_min(const Iu8vec8 &_A, const Iu8vec8 &_B) {
  return _m_pminub(_A, _B);
}

/* Average */
inline Iu16vec4 simd_avg(const Iu16vec4 &_A, const Iu16vec4 &_B) {
  return _mm_avg_pu16(_A, _B);
}
inline Iu8vec8 simd_avg(const Iu8vec8 &_A, const Iu8vec8 &_B) {
...
inline Iu16vec4 mul_high(const Iu16vec4 &_A, const Iu16vec4 &_B) {
  return _m_pmulhuw(_A, _B);
}

/* Byte Mask Write: Write bytes if most significant bit in each corresponding
 * byte is set */
inline void mask_move(const I8vec8 &_A, const I8vec8 &_B, char *_Addr) {
  _m_maskmovq(_A, _B, _Addr);
}
...
inline int F32vec4ToInt(const F32vec4 &_A) { return _mm_cvtt_ss2si(_A); }
...
/* Convert the 32-bit int i to an SP FP value; the upper three SP FP values are
...
/* Convert the two 32-bit integer values in b to two SP FP values; the upper two
 * SP FP values are passed from a. */
...
public:
  ...
  /* Logical Operators */
  friend F32vec1 operator&(const F32vec1 &_A, const F32vec1 &_B) {
    return _mm_and_ps(_A, _B);
  }
  friend F32vec1 operator|(const F32vec1 &_A, const F32vec1 &_B) {
    return _mm_or_ps(_A, _B);
  ...
  /* Arithmetic Operators */
  friend F32vec1 operator+(const F32vec1 &_A, const F32vec1 &_B) {
    return _mm_add_ss(_A, _B);
  }
  friend F32vec1 operator-(const F32vec1 &_A, const F32vec1 &_B) {
    return _mm_sub_ss(_A, _B);
  }
  friend F32vec1 operator*(const F32vec1 &_A, const F32vec1 &_B) {
    return _mm_mul_ss(_A, _B);
  }
  friend F32vec1 operator/(const F32vec1 &_A, const F32vec1 &_B) {
  ...
  friend F32vec1 rcp_nr(const F32vec1 &_A) {
    ...
    return _mm_sub_ss(_mm_add_ss(_Ra0, _Ra0),
                      _mm_mul_ss(_mm_mul_ss(_Ra0, _A), _Ra0));
  }
...
#define Fvec32s1_COMP(op)                                                      \
  friend F32vec1 cmp##op(const F32vec1 &_A, const F32vec1 &_B) {               \
    return _mm_cmp##op##_ss(_A, _B);                                           \
  }
...
#undef Fvec32s1_COMP
      ...
      /* Min and Max */
      friend F32vec1 simd_min(const F32vec1 &_A, const F32vec1 &_B) {
    return _mm_min_ss(_A, _B);
  }
  friend F32vec1 simd_max(const F32vec1 &_A, const F32vec1 &_B) {
...
Macros expand to conditional selects which use all compare intrinsics.
Example:
friend F32vec1 select_eq(const F32vec1 &_A, const F32vec1 &_B, const F32vec1
&_C, const F32vec1 &_D)
{
...
#define Fvec32s1_SELECT(op)                                                    \
  inline F32vec1 select_##op(const F32vec1 &_A, const F32vec1 &_B,             \
                             const F32vec1 &_C, const F32vec1 &_D) {           \
