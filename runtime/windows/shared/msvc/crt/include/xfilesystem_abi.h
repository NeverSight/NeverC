...
#pragma warning(disable                                                        \
                : 4061) // enumerator not explicitly handled by switch label
_NODISCARD inline bool
__std_is_file_not_found(const __std_win_error _Error) noexcept {
  switch (_Error) {
...
enum class __std_fs_file_attr : unsigned long {
  ...
  _Reparse_point =
      0x00000400, // #define FILE_ATTRIBUTE_REPARSE_POINT        0x00000400
...
enum class __std_fs_reparse_tag : unsigned long {
  _None = 0,
  _Mount_point =
      (0xA0000003L),        // #define IO_REPARSE_TAG_MOUNT_POINT (0xA0000003L)
...
struct __std_fs_find_data {            // typedef struct _WIN32_FIND_DATAW {
  ...
  wchar_t _File_name[__std_fs_max_path];   //     _Field_z_ WCHAR  cFileName[
                                           //     MAX_PATH ];
  wchar_t
      _Short_file_name[14]; //     _Field_z_ WCHAR  cAlternateFileName[ 14 ];
}; // } WIN32_FIND_DATAW, ... ;
...
enum class __std_fs_stats_flags : unsigned long {
  ...
  _Reparse_tag =
      0x04, // read/has reparse_tag; may not be combined with _Follow_symlinks
  _File_size = 0x08,       // read/has file size
  _Link_count = 0x10,      // read/has link count
  _Last_write_time = 0x20, // read/has last write time

  _All_data =
      _Attributes | _Reparse_tag | _File_size | _Link_count | _Last_write_time
};
} // extern "C"
...
struct __std_fs_stats {
  ...
  _NODISCARD __std_fs_file_attr _Symlink_hint_attributes() const noexcept {
    if (_STD _Bitmask_includes_any(_Available,
...
enum class __std_access_rights : unsigned long {
  ...
  _File_read_attributes =
      0x0080, // #define FILE_READ_ATTRIBUTES      ( 0x0080 )
  _File_write_attributes =
      0x0100, // #define FILE_WRITE_ATTRIBUTES     ( 0x0100 )
...
enum class __std_fs_file_flags : unsigned long {
  _None = 0,
  _Backup_semantics =
      0x02000000, // #define FILE_FLAG_BACKUP_SEMANTICS      0x02000000
  _Open_reparse_point =
      0x00200000, // #define FILE_FLAG_OPEN_REPARSE_POINT    0x00200000
};
} // extern "C"
...
_NODISCARD __std_ulong_and_error __stdcall __std_fs_get_full_path_name(
    _In_z_ const wchar_t *_Source, _In_ unsigned long _Target_size,
    _Out_writes_z_(_Target_size) wchar_t *_Target) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_open_handle(_Out_ __std_fs_file_handle *_Handle,
...
_NODISCARD __std_ulong_and_error __stdcall
...
_NODISCARD __std_fs_copy_file_result __stdcall
__std_fs_copy_file(_In_z_ const wchar_t *_Source, _In_z_ const wchar_t *_Target,
                   _In_ __std_fs_copy_options _Options) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_directory_iterator_open(_In_z_ const wchar_t *_Path_spec,
                                 _Inout_ __std_fs_dir_handle *_Handle,
                                 _Out_ __std_fs_find_data *_Results) noexcept;

void __stdcall
__std_fs_directory_iterator_close(_In_ __std_fs_dir_handle _Handle) noexcept;

_NODISCARD _Success_(return == __std_win_error::_Success) __std_win_error
    __stdcall
    __std_fs_get_stats(_In_z_ const wchar_t *_Path, __std_fs_stats *_Stats,
                       _In_ __std_fs_stats_flags _Flags,
                       _In_ __std_fs_file_attr _Symlink_attribute_hint =
                           __std_fs_file_attr::_Invalid) noexcept;
...
_NODISCARD __std_fs_convert_result __stdcall
...
_NODISCARD __std_fs_equivalent_result __stdcall
__std_fs_equivalent(_In_z_ const wchar_t *_Path1,
                    _In_z_ const wchar_t *_Path2) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_set_last_write_time(_In_ long long _Last_write_filetime,
                             _In_z_ const wchar_t *_Path) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_change_permissions(_In_z_ const wchar_t *_Path,
                            _In_ bool _Follow_symlinks,
                            _In_ bool _Readonly) noexcept;

_NODISCARD _Success_(return._Error ==
                           __std_win_error::_Success) __std_ulong_and_error
    __stdcall
    __std_fs_get_temp_path(_Out_writes_z_(__std_fs_temp_path_max)
                               wchar_t *_Target) noexcept;

_NODISCARD _Success_(return._Error ==
                           __std_win_error::_Success) __std_ulong_and_error
    __stdcall
    __std_fs_get_current_path(_In_ unsigned long _Target_size,
                              _Out_writes_z_(_Target_size)
                                  wchar_t *_Target) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_set_current_path(_In_z_ const wchar_t *_Target) noexcept;
...
_NODISCARD __std_win_error __stdcall
__std_fs_create_hard_link(_In_z_ const wchar_t *_File_name,
                          _In_z_ const wchar_t *_Existing_file_name) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_create_symbolic_link(_In_z_ const wchar_t *_Symlink_file_name,
                              _In_z_ const wchar_t *_Target_file_name) noexcept;
...
_NODISCARD _Success_(return == __std_win_error::_Success) __std_win_error
    ...
    __std_fs_read_name_from_reparse_data_buffer(
        _In_ __std_fs_reparse_data_buffer *_Handle, _Out_ wchar_t **_Offset,
...
_NODISCARD __std_fs_create_directory_result __stdcall
__std_fs_create_directory(_In_z_ const wchar_t *_New_directory) noexcept;
...
_NODISCARD __std_fs_remove_result __stdcall
__std_fs_remove(_In_z_ const wchar_t *_Target) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_rename(_In_z_ const wchar_t *_Source,
                _In_z_ const wchar_t *_Target) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_resize_file(_In_z_ const wchar_t *_Target,
                     uintmax_t _New_size) noexcept;

_NODISCARD __std_win_error __stdcall
__std_fs_space(_In_z_ const wchar_t *_Target, _Out_ uintmax_t *_Available,
...
struct _Fs_file {
  ...
  _Fs_file(const wchar_t *const _File_name,
           const __std_access_rights _Desired_access,
