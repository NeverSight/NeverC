...
GetProxyImpl(::IUnknown *, REFIID, ::IUnknown *, ::IUnknown **);
__declspec(dllimport) HRESULT __stdcall ReleaseInContextImpl(::IUnknown *,
                                                             ::IUnknown *);

template <typename T>
__declspec(no_refcount) inline HRESULT
GetProxy(T ^ ObjectIn, ::IUnknown *ContextCallBack, T ^ *Proxy) {
  return GetProxyImpl(*reinterpret_cast<IUnknown **>(&ObjectIn), __uuidof(T ^),
                      ContextCallBack, reinterpret_cast<::IUnknown **>(Proxy));
}
...
template <typename T> class AgileHelper {
  __abi_IUnknown *_p;
  bool _release;

public:
  AgileHelper(__abi_IUnknown *p, bool release = true)
      : _p(p), _release(release) {}
...
template <typename T> struct __remove_hat {
  typedef T type;
};
template <typename T> struct __remove_hat<T ^> {
  typedef T type;
};
template <typename T> struct AgileTypeHelper {
...
#pragma warning(disable                                                        \
                : 4451) // Usage of ref class inside this context can lead to
                        // invalid marshaling of object across contexts

template <typename T,
          bool TIsNotAgile =
              (__is_win_class(typename Details::AgileTypeHelper<T>::type) &&
               !__is_winrt_agile(typename Details::AgileTypeHelper<T>::type)) ||
              __is_win_interface(typename Details::AgileTypeHelper<T>::type)>
class Agile {
  static_assert(
      __is_win_class(typename Details::AgileTypeHelper<T>::type) ||
          __is_win_interface(typename Details::AgileTypeHelper<T>::type),
      "Agile can only be used with ref class or interface class types");
  ...
  enum class AgileState { NonAgilePointer = 0, AgilePointer = 1, Unknown = 2 };
  ...
  void SetObject(TypeT object) {
    ...
    if (object != nullptr) {
      ...
      HRESULT hr = reinterpret_cast<::IUnknown *>(object)->QueryInterface(
          __uuidof(::IAgileObject), &checkIfAgile);
...
public:
  Agile() throw()
      : _object(nullptr), _contextToken(0), _agileState(AgileState::Unknown) {}

  Agile(nullptr_t) throw()
      : _object(nullptr), _contextToken(0), _agileState(AgileState::Unknown) {}

  explicit Agile(TypeT object) throw()
      : _object(nullptr), _contextToken(0), _agileState(AgileState::Unknown) {
  ...
  TypeT Get() const {
    ...
    TypeT localObject;
    __abi_ThrowIfFailed(
        Details::GetProxy(_object, _contextCallback.Get(), &localObject));

    if (_agileState == AgileState::Unknown) {
      // Object is agile if it implements IAgileObject
      // GetAddressOf captures the context with out knowing the type of object
      // that it will hold
      ::Microsoft::WRL::ComPtr<::IAgileObject> checkIfAgile;
      HRESULT hr =
          reinterpret_cast<IUnknown *>(localObject)
  ...
  TypeT operator->() const throw() { return Get(); }
  ...
  void Release() throw() {
    if (_object) {
      ...
      *(IUnknown **)(&_object) = nullptr;
      ...
      ULONG_PTR currentContextToken;
      __abi_ThrowIfFailed(CoGetContextToken(&currentContextToken));
      if (_contextToken == 0 || _contextCallback == nullptr ||
          _contextToken == currentContextToken) {
        pObject->Release();
      } else {
  ...
  bool operator==(nullptr_t) const throw() { return _object == nullptr; }
  ...
  bool operator<(const Agile &other) const throw() {
    if (reinterpret_cast<void *>(_object) <
        reinterpret_cast<void *>(other._object)) {
      return true;
    }
...
template <typename T> class Agile<T, false> {
  static_assert(
      __is_win_class(typename Details::AgileTypeHelper<T>::type) ||
          __is_win_interface(typename Details::AgileTypeHelper<T>::type),
      "Agile can only be used with ref class or interface class types");
...
public:
  ...
  Agile(Agile &&object) throw() : _object(nullptr) { Swap(object); }

  ~Agile() throw() { Release(); }

  TypeT Get() const { return _object; }
  ...
  // Release the interface and set to NULL
  void Release() throw() { _object = nullptr; }

  void Swap(Agile &object) { std::swap(_object, object._object); }
...
template <class U> bool operator==(nullptr_t, const Agile<U> &a) throw() {
  return a == nullptr;
}

template <class U> bool operator!=(const Agile<U> &a, nullptr_t) throw() {
  return !(a == nullptr);
}
