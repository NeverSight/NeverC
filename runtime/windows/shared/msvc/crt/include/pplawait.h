...
struct coroutine_traits<::concurrency::task<_Ty>, _Whatever...> {
  struct promise_type {
    ...
    void return_value(const _Ty &_Val) { _M_tce.set(_Val); }
...
// When GetResults is called for an operation in the Canceled state, an
// E_ILLEGAL_METHOD_CALL exception will be thrown. We preempt that by throwing
// an OperationCanceled exception, which better meets caller expectations.
inline void
_VerifyStateForResultsCall(Windows::Foundation::AsyncStatus _Status) {
...
template <typename _Ty> struct _Ppltask_awaiter {
  task<_Ty> &_Task;

  bool await_ready() const { return _Task.is_done(); }
  ...
  void await_suspend(_STD
                         ...
                         experimental::
                             ...
                             coroutine_handle<>
                                 _ResumeCb) {
    _Task.then([_ResumeCb](const task<_Ty> &) { _ResumeCb(); },
               task_continuation_context::get_current_winrt_context());
  }

  decltype(auto) await_resume() { return _Task.get(); }
};

template <typename _Ty> auto operator co_await(task<_Ty> &&_Task) {
...
struct [[deprecated(
    "warning PPL4001: "
    "The await_resume_context class is deprecated and will be removed in a "
    "future release. "
    "You can define _RESUMABLE_ENABLE_LEGACY_AWAIT_ADAPTERS and add /await to "
    "the command line "
...
// The Windows-internal CXAwait.h provides alternative bindings for IAsyncXXX
// and coroutines. These defines ensure the headers are included in the correct
// order, cxawait.h should be first.
...
namespace Windows {
namespace Foundation {
template <typename _TaskTy, typename _HandlerTy> struct _IAsync_awaiter {
  _TaskTy ^ _Task;

  bool await_ready() const { return _Task->Status >= AsyncStatus::Completed; }
  ...
  void await_suspend(_STD
                         ...
                         experimental::
                             ...
                             coroutine_handle<>
                                 _ResumeCb) {
    _Task->Completed =
        ref new _HandlerTy([_ResumeCb](_TaskTy ^, AsyncStatus) { _ResumeCb(); },
...
auto operator co_await(IAsyncActionWithProgress<_Pr> ^ _Expr) {
  return _IAsync_awaiter<IAsyncActionWithProgress<_Pr>,
                         AsyncActionWithProgressCompletedHandler<_Pr>>{_Expr};
}

template <typename _Ty> auto operator co_await(IAsyncOperation<_Ty> ^ _Expr) {
  return _IAsync_awaiter<IAsyncOperation<_Ty>,
                         AsyncOperationCompletedHandler<_Ty>>{_Expr};
}
...
auto operator co_await(IAsyncOperationWithProgress<_Ty, _Pr> ^ _Expr) {
  return _IAsync_awaiter<IAsyncOperationWithProgress<_Ty, _Pr>,
                         AsyncOperationWithProgressCompletedHandler<_Ty, _Pr>>{
      _Expr};
}
} // namespace Foundation
} // namespace Windows
#endif // defined(__cplusplus_winrt) && (defined(__cpp_impl_coroutine) ||
       // defined(_RESUMABLE_FUNCTIONS_SUPPORTED))
...
inline bool
_TryAwaitReady(_Ty &_Obj, decltype(_STD declval<_Ty>().await_ready(), 0), int) {
  return _Obj.await_ready();
}
template <typename _Ty>
inline bool _TryAwaitReady(_Ty &_Obj,
                           decltype(await_ready(_STD declval<_Ty>()), 0), ...) {
  return await_ready(_Obj);
}

template <typename _Ty, typename _Handle>
inline void _TryAwaitSuspend(
    _Ty &_Obj, _Handle _Hnd,
    decltype(_STD declval<_Ty>().await_suspend(_STD declval<_Handle>()), 0),
    int) {
...
template <typename _Ty>
inline auto _TryAwaitResume(_Ty &_Obj,
                            decltype(_STD declval<_Ty>().await_resume(), 0),
                            int) {
  return _Obj.await_resume();
}
template <typename _Ty>
inline auto _TryAwaitResume(_Ty &_Obj,
                            decltype(await_resume(_STD declval<_Ty>()), 0),
                            ...) {
...
inline true_type
_TryAwaitable(_Ty,
              decltype(_TryAwaitReady(_STD declval<decay_t<_Ty>>(), 0, 0),
...
struct awaitable_traits
    : decltype(details::awaitabletraitsimpl::_TryAwaitable(
          _STD declval<_Ty>(), 0)){static bool invoke_await_ready(_Ty & _Obj){
...
template <typename _Ty> using is_awaitable = awaitable_traits<_Ty>;
...
template <typename _Ty> bool await_ready(const task<_Ty> &_Task) {
...
template <typename _Ty> auto await_resume(const task<_Ty> &_Task) {
  return _Task.get();
}
...
class await_resume_context {
  ...
  public:
    template <typename _Handle> void _CallInContext(_Handle _Hnd) {
      _M_func = _Hnd;
    ...
    ThreadpoolContext() { _M_chore._M_work = nullptr; }
...
public:
  ...
  static await_resume_context any() { return await_resume_context{}; }

  template <typename _Ty> class _CallbackChain {
    _Ty _M_awaitable;
  ...
  public:
    ...
    template <typename _Handle> void await_suspend(_Handle _Hnd) {
      _STD experimental::awaitable_traits<_Ty>::invoke_await_suspend(
          _M_awaitable, [=] {
    ...
    auto await_resume() {
      return _STD experimental::awaitable_traits<_Ty>::invoke_await_resume(
          _M_awaitable);
    }
  };
  ...
  public:
    explicit _Callback(await_resume_context *_ContextPtr)
        : _M_contextPtr(_ContextPtr) {}
  ...
  template <typename _Ty> auto get_awaitable(_Ty _Awaitable) {
    return _CallbackChain<_STD decay_t<_Ty>>(_Awaitable, this);
  }

  auto get_awaitable() { return _Callback(this); }
};
} // namespace Concurrency
...
void await_suspend(IAsyncAction ^ _Task, _Handle _ResumeCb) {
  _Task->Completed = ref new AsyncActionCompletedHandler(
      [_ResumeCb](IAsyncAction ^, AsyncStatus) { _ResumeCb(); },
      ::Platform::CallbackContext::Same);
}
...
void await_suspend(IAsyncOperation<_Ty> ^ _Task, _Handle _ResumeCb) {
  _Task->Completed = ref new AsyncOperationCompletedHandler<_Ty>(
      [_ResumeCb](IAsyncOperation<_Ty> ^, AsyncStatus) { _ResumeCb(); },
      ::Platform::CallbackContext::Same);
}
...
void await_suspend(IAsyncActionWithProgress<_Pr> ^ _Task, _Handle _ResumeCb) {
  _Task->Completed = ref new AsyncActionWithProgressCompletedHandler<_Pr>(
      [_ResumeCb](IAsyncActionWithProgress<_Pr> ^, AsyncStatus) {
        _ResumeCb();
      },
      ::Platform::CallbackContext::Same);
}
...
void await_suspend(IAsyncOperationWithProgress<_Ty, _Pr> ^ _Task,
                   _Handle _ResumeCb) {
  _Task->Completed =
      ref new AsyncOperationWithProgressCompletedHandler<_Ty, _Pr>(
          [_ResumeCb](IAsyncOperationWithProgress<_Ty, _Pr> ^, AsyncStatus) {
            _ResumeCb();
          },
          ::Platform::CallbackContext::Same);
}
...
template <typename _Ty, typename _Pr>
_Ty await_resume(Windows::Foundation::IAsyncOperationWithProgress<_Ty, _Pr> ^
                 _Task) {
  _VerifyStateForResultsCall(_Task->Status);
  return _Task->GetResults();
}
