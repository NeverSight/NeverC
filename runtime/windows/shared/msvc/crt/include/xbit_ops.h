...
_NODISCARD inline unsigned long
_Floor_of_log_2(size_t _Value) noexcept { // returns floor(log_2(_Value))
...
#ifdef _WIN64
  // CodeQL [SM02313] _Result is always initialized: the code above guarantees
  // that _Value is non-zero.
...
_NODISCARD inline unsigned long
_Ceiling_of_log_2(const size_t _Value) noexcept { // returns ceil(log_2(_Value))
                                                  // pre: _Value > 1
