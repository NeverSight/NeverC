...
//
typedef int(WINAPI *WSPIAPI_PGETADDRINFO)(_In_ const char *nodename,
                                          _In_ const char *servname,
                                          _In_ const struct addrinfo *hints,
                                          _Out_ struct addrinfo **res);

typedef int(WINAPI *WSPIAPI_PGETNAMEINFO)(
    _In_reads_bytes_(salen) const struct sockaddr *sa, _In_ socklen_t salen,
    _Out_writes_bytes_(hostlen) char *host, _In_ size_t hostlen,
    _Out_writes_bytes_(servlen) char *serv, _In_ size_t servlen,
    _In_ int flags);

typedef void(WINAPI *WSPIAPI_PFREEADDRINFO)(_In_ struct addrinfo *ai);
...
__inline char *WINAPI WspiapiStrdup(_In_z_ const char *pszString)
/*++
...
__inline BOOL WINAPI WspiapiParseV4Address(_In_z_ const char *pszAddress,
...
__inline struct addrinfo *WINAPI WspiapiNewAddrInfo(_In_ int iSocketType,
                                                    _In_ int iProtocol,
                                                    _In_ WORD wPort,
...
__inline int WINAPI WspiapiQueryDNS(
    _In_z_ const char *pszNodeName, _In_ int iSocketType, _In_ int iProtocol,
    _In_ WORD wPort, _Out_writes_bytes_(NI_MAXHOST) char pszAlias[NI_MAXHOST],
...
__inline int WINAPI WspiapiLookupNode(_In_z_ const char *pszNodeName,
                                      _In_ int iSocketType, _In_ int iProtocol,
                                      _In_ WORD wPort, _In_ BOOL bAI_CANONNAME,
                                      _Outptr_ struct addrinfo **pptResult)
/*++
...
--*/
{
  ...
  for (;;) {
    iError = WspiapiQueryDNS(pszNodeName, iSocketType, iProtocol, wPort,
                             pszAlias, pptResult);
    if (iError)
      break;
...
__inline int WINAPI WspiapiClone(_In_ WORD wPort,
...
--*/
{
  ...
  for (ptNext = ptResult; ptNext != NULL;) {
    ...
    ptNew = WspiapiNewAddrInfo(
        SOCK_DGRAM, ptNext->ai_protocol, wPort,
...
__inline void WINAPI WspiapiLegacyFreeAddrInfo(_In_ struct addrinfo *ptHead)
/*++
...
__inline int WINAPI WspiapiLegacyGetAddrInfo(
    _In_ const char *pszNodeName, _In_ const char *pszServiceName,
    _In_ const struct addrinfo *ptHints, _Outptr_ struct addrinfo **pptResult)
/*++
...
__inline int WINAPI WspiapiLegacyGetNameInfo(
    _In_reads_bytes_(tSocketLength) const struct sockaddr *ptSocketAddress,
    _In_ socklen_t tSocketLength,
    _Out_writes_bytes_(tNodeLength) char *pszNodeName, _In_ size_t tNodeLength,
    _Out_writes_bytes_(tServiceLength) char *pszServiceName,
    _In_ size_t tServiceLength, _In_ int iFlags)
/*++
...
--*/
{
  ...
  if ((!ptSocketAddress) ||
      (tSocketLength < (socklen_t)sizeof(struct sockaddr)))
    return EAI_FAIL;
  ...
  if (pszServiceName && tServiceLength) {
    ...
    if (iFlags & NI_NUMERICSERV) {
      ...
      _WSPIAPI_SPRINTF_S_1(szBuffer, _WSPIAPI_COUNTOF(szBuffer), "%u",
                           ntohs(wPort));
    } else {
...
#pragma warning(suppress : 4996 38026)
      ptHost =
          gethostbyaddr((char *)&tAddress, sizeof(struct in_addr), AF_INET);
...
__inline FARPROC WINAPI WspiapiLoad(_In_ WORD wFunction)
/*++
...
--*/
{
  ...
  __analysis_assume((sizeof(rgtLocal) / sizeof(WSPIAPI_FUNCTION)) ==
                    iNumGlobal);
...
__inline int WINAPI WspiapiGetAddrInfo(_In_opt_ const char *nodename,
...
__inline void WINAPI WspiapiFreeAddrInfo(_In_ struct addrinfo *ai) {
