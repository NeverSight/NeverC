...
enum tagSFBS_FLAGS {
  SFBS_FLAGS_ROUND_TO_NEAREST_DISPLAYED_DIGIT =
      0x0001, // round to the nearest displayed digit
  SFBS_FLAGS_TRUNCATE_UNDISPLAYED_DECIMAL_DIGITS =
      0x0002, // discard undisplayed digits
};
typedef int SFBS_FLAGS;

LWSTDAPI StrFormatByteSizeEx(ULONGLONG ull, SFBS_FLAGS flags,
                             _Out_writes_(cchBuf) PWSTR pszBuf,
                             _In_range_(>, 0) UINT cchBuf);
#endif // (NTDDI_VERSION >= NTDDI_VISTASP1)

LWSTDAPI_(PSTR)
StrFormatByteSizeA(DWORD dw, _Out_writes_(cchBuf) PSTR pszBuf, UINT cchBuf);
LWSTDAPI_(PSTR)
StrFormatByteSize64A(LONGLONG qdw, _Out_writes_(cchBuf) PSTR pszBuf,
                     UINT cchBuf);
LWSTDAPI_(PWSTR)
StrFormatByteSizeW(LONGLONG qdw, _Out_writes_(cchBuf) PWSTR pszBuf,
                   UINT cchBuf);
LWSTDAPI_(PWSTR)
StrFormatKBSizeW(LONGLONG qdw, _Out_writes_(cchBuf) PWSTR pszBuf, UINT cchBuf);
LWSTDAPI_(PSTR)
StrFormatKBSizeA(LONGLONG qdw, _Out_writes_(cchBuf) PSTR pszBuf, UINT cchBuf);
LWSTDAPI_(int)
StrFromTimeIntervalA(_Out_writes_(cchMax) PSTR pszOut, UINT cchMax,
                     DWORD dwTimeMS, int digits);
LWSTDAPI_(int)
StrFromTimeIntervalW(_Out_writes_(cchMax) PWSTR pszOut, UINT cchMax,
                     DWORD dwTimeMS, int digits);
LWSTDAPI_(BOOL)
StrIsIntlEqualA(BOOL fCaseSens, _In_ PCSTR pszString1, _In_ PCSTR pszString2,
                int nChar);
LWSTDAPI_(BOOL)
StrIsIntlEqualW(BOOL fCaseSens, _In_ PCWSTR pszString1, _In_ PCWSTR pszString2,
                int nChar);
LWSTDAPI_(PSTR)
StrNCatA(_Inout_updates_(cchMax) PSTR psz1, PCSTR psz2, int cchMax);
LWSTDAPI_(PWSTR)
StrNCatW(_Inout_updates_(cchMax) PWSTR psz1, PCWSTR psz2, int cchMax);
#ifdef USE_STRICT_CONST
...
LWSTDAPI_(PCWSTR)
StrRChrW(_In_ PCWSTR pszStart, _In_opt_ PCWSTR pszEnd, WCHAR wMatch);
LWSTDAPI_(PCSTR)
StrRChrIA(_In_ PCSTR pszStart, _In_opt_ PCSTR pszEnd, WORD wMatch);
LWSTDAPI_(PCWSTR)
StrRChrIW(_In_ PCWSTR pszStart, _In_opt_ PCWSTR pszEnd, WCHAR wMatch);
LWSTDAPI_(PCSTR)
StrRStrIA(_In_ PCSTR pszSource, _In_opt_ PCSTR pszLast, _In_ PCSTR pszSrch);
LWSTDAPI_(PCWSTR)
StrRStrIW(_In_ PCWSTR pszSource, _In_opt_ PCWSTR pszLast, _In_ PCWSTR pszSrch);
#else
...
LWSTDAPI_(BOOL)
StrToIntExW(_In_ PCWSTR pszString, STIF_FLAGS dwFlags, _Out_ int *piRet);
#if (_WIN32_IE >= _WIN32_IE_IE60)
LWSTDAPI_(BOOL)
StrToInt64ExA(_In_ PCSTR pszString, STIF_FLAGS dwFlags, _Out_ LONGLONG *pllRet);
LWSTDAPI_(BOOL)
StrToInt64ExW(_In_ PCWSTR pszString, STIF_FLAGS dwFlags,
...
LWSTDAPI_(PWSTR)
StrCatBuffW(_Inout_updates_(cchDestBuffSize) PWSTR pszDest, _In_ PCWSTR pszSrc,
            int cchDestBuffSize);
LWSTDAPI_(PSTR)
StrCatBuffA(_Inout_updates_(cchDestBuffSize) PSTR pszDest, _In_ PCSTR pszSrc,
            int cchDestBuffSize);
LWSTDAPI_(BOOL) ChrCmpIA(WORD w1, WORD w2);
LWSTDAPI_(BOOL) ChrCmpIW(WCHAR w1, WCHAR w2);

LWSTDAPI_(int)
wvnsprintfA(_Out_writes_(cchDest) PSTR pszDest, _In_ int cchDest,
            _In_ _Printf_format_string_ PCSTR pszFmt, _In_ va_list arglist);
LWSTDAPI_(int)
wvnsprintfW(_Out_writes_(cchDest) PWSTR pszDest, _In_ int cchDest,
            _In_ _Printf_format_string_ PCWSTR pszFmt, _In_ va_list arglist);
LWSTDAPIV_(int)
wnsprintfA(_Out_writes_(cchDest) PSTR pszDest, _In_ int cchDest,
           _In_ _Printf_format_string_ PCSTR pszFmt, ...);
LWSTDAPIV_(int)
wnsprintfW(_Out_writes_(cchDest) PWSTR pszDest, _In_ int cchDest,
           _In_ _Printf_format_string_ PCWSTR pszFmt, ...);
...
LWSTDAPI StrRetToStrA(_Inout_ STRRET *pstr, _In_opt_ PCUITEMID_CHILD pidl,
                      _Outptr_ LPSTR *ppsz);
LWSTDAPI StrRetToStrW(_Inout_ STRRET *pstr, _In_opt_ PCUITEMID_CHILD pidl,
...
#endif // !UNICODE
LWSTDAPI StrRetToBufA(_Inout_ STRRET *pstr, _In_opt_ PCUITEMID_CHILD pidl,
                      _Out_writes_(cchBuf) LPSTR pszBuf, UINT cchBuf);
LWSTDAPI StrRetToBufW(_Inout_ STRRET *pstr, _In_opt_ PCUITEMID_CHILD pidl,
                      _Out_writes_(cchBuf) LPWSTR pszBuf, UINT cchBuf);
#ifdef UNICODE
...
LWSTDAPI SHStrDupA(_In_ LPCSTR psz,
                   _Outptr_result_nullonfailure_ LPWSTR *ppwsz);
...
inline HRESULT SHLocalStrDupW(_In_ PCWSTR psz,
                              _Outptr_result_maybenull_ PWSTR *ppsz) {
  *ppsz = StrDupW(psz);
  return *ppsz ? S_OK : E_OUTOFMEMORY;
}

inline HRESULT SHLocalStrDupA(_In_ PCSTR psz,
                              _Outptr_result_maybenull_ PSTR *ppsz) {
  *ppsz = StrDupA(psz);
  return *ppsz ? S_OK : E_OUTOFMEMORY;
}
...
LWSTDAPI_(DWORD)
StrCatChainW(_Out_writes_(cchDst) PWSTR pszDst, DWORD cchDst, DWORD ichAt,
             _In_ PCWSTR pszSrc);
LWSTDAPI StrRetToBSTR(_Inout_ STRRET *pstr, _In_opt_ PCUITEMID_CHILD pidl,
                      _Outptr_ BSTR *pbstr);
...
LWSTDAPI SHLoadIndirectString(_In_ PCWSTR pszSource,
...
LWSTDAPI_(int)
StrCmpNICW(_In_ LPCWSTR pszStr1, _In_ LPCWSTR pszStr2, int nChar);
#ifdef UNICODE
...
//
LWSTDAPI_(BOOL)
IntlStrEqWorkerA(BOOL fCaseSens, _In_reads_(nChar) LPCSTR lpString1,
                 _In_reads_(nChar) LPCSTR lpString2, int nChar);
LWSTDAPI_(BOOL)
IntlStrEqWorkerW(BOOL fCaseSens, _In_reads_(nChar) LPCWSTR lpString1,
                 _In_reads_(nChar) LPCWSTR lpString2, int nChar);
...
#endif // !UNICODE
LWSTDAPI_(BOOL)
PathAddExtensionA(_Inout_updates_(MAX_PATH) LPSTR pszPath,
                  _In_opt_ LPCSTR pszExt);
LWSTDAPI_(BOOL)
PathAddExtensionW(_Inout_updates_(MAX_PATH) LPWSTR pszPath,
...
#endif // !UNICODE
LWSTDAPI_(BOOL)
PathAppendA(_Inout_updates_(MAX_PATH) LPSTR pszPath, _In_ LPCSTR pszMore);
LWSTDAPI_(BOOL)
PathAppendW(_Inout_updates_(MAX_PATH) LPWSTR pszPath, _In_ LPCWSTR pszMore);
...
#endif // !UNICODE
LWSTDAPI_(BOOL)
PathCanonicalizeA(_Out_writes_(MAX_PATH) LPSTR pszBuf, _In_ LPCSTR pszPath);
LWSTDAPI_(BOOL)
PathCanonicalizeW(_Out_writes_(MAX_PATH) LPWSTR pszBuf, _In_ LPCWSTR pszPath);
LWSTDAPI_(LPSTR)
PathCombineA(_Out_writes_(MAX_PATH) LPSTR pszDest, _In_opt_ LPCSTR pszDir,
             _In_opt_ LPCSTR pszFile);
LWSTDAPI_(LPWSTR)
PathCombineW(_Out_writes_(MAX_PATH) LPWSTR pszDest, _In_opt_ LPCWSTR pszDir,
...
#endif // !UNICODE
LWSTDAPI_(BOOL)
PathCompactPathA(_In_opt_ HDC hDC, _Inout_updates_(MAX_PATH) LPSTR pszPath,
                 _In_ UINT dx);
LWSTDAPI_(BOOL)
PathCompactPathW(_In_opt_ HDC hDC, _Inout_updates_(MAX_PATH) LPWSTR pszPath,
                 _In_ UINT dx);
LWSTDAPI_(BOOL)
PathCompactPathExA(_Out_writes_(cchMax) LPSTR pszOut, _In_ LPCSTR pszSrc,
                   _In_ UINT cchMax, _In_ DWORD dwFlags);
LWSTDAPI_(BOOL)
PathCompactPathExW(_Out_writes_(cchMax) LPWSTR pszOut, _In_ LPCWSTR pszSrc,
                   _In_ UINT cchMax, _In_ DWORD dwFlags);
LWSTDAPI_(int)
PathCommonPrefixA(_In_ LPCSTR pszFile1, _In_ LPCSTR pszFile2,
                  _Out_writes_opt_(MAX_PATH) LPSTR achPath);
LWSTDAPI_(int)
PathCommonPrefixW(_In_ LPCWSTR pszFile1, _In_ LPCWSTR pszFile2,
                  _Out_writes_opt_(MAX_PATH) LPWSTR achPath);
...
#endif
LWSTDAPI_(BOOL)
PathFindOnPathA(_Inout_updates_(MAX_PATH) LPSTR pszPath,
                _In_opt_ PZPCSTR ppszOtherDirs);
LWSTDAPI_(BOOL)
PathFindOnPathW(_Inout_updates_(MAX_PATH) LPWSTR pszPath,
                _In_opt_ PZPCWSTR ppszOtherDirs);
LWSTDAPI_(LPCSTR)
PathFindSuffixArrayA(_In_ LPCSTR pszPath,
                     _In_reads_(iArraySize) const LPCSTR *apszSuffix,
                     _In_ int iArraySize);
LWSTDAPI_(LPCWSTR)
PathFindSuffixArrayW(_In_ LPCWSTR pszPath,
                     _In_reads_(iArraySize) const LPCWSTR *apszSuffix,
                     _In_ int iArraySize);
#ifdef UNICODE
...
LWSTDAPI_(BOOL)
PathIsContentTypeW(_In_ LPCWSTR pszPath, _In_ LPCWSTR pszContentType);
...
LWSTDAPI PathMatchSpecExA(_In_ LPCSTR pszFile, _In_ LPCSTR pszSpec,
                          _In_ DWORD dwFlags);
LWSTDAPI PathMatchSpecExW(_In_ LPCWSTR pszFile, _In_ LPCWSTR pszSpec,
                          _In_ DWORD dwFlags);
...
LWSTDAPI_(BOOL)
PathRelativePathToA(_Out_writes_(MAX_PATH) LPSTR pszPath, _In_ LPCSTR pszFrom,
                    _In_ DWORD dwAttrFrom, _In_ LPCSTR pszTo,
                    _In_ DWORD dwAttrTo);
LWSTDAPI_(BOOL)
PathRelativePathToW(_Out_writes_(MAX_PATH) LPWSTR pszPath, _In_ LPCWSTR pszFrom,
                    _In_ DWORD dwAttrFrom, _In_ LPCWSTR pszTo,
                    _In_ DWORD dwAttrTo);
...
LWSTDAPI_(BOOL)
PathRenameExtensionA(_Inout_updates_(MAX_PATH) LPSTR pszPath,
                     _In_ LPCSTR pszExt);
LWSTDAPI_(BOOL)
PathRenameExtensionW(_Inout_updates_(MAX_PATH) LPWSTR pszPath,
                     _In_ LPCWSTR pszExt);
LWSTDAPI_(BOOL)
PathSearchAndQualifyA(_In_ LPCSTR pszPath, _Out_writes_(cchBuf) LPSTR pszBuf,
                      _In_ UINT cchBuf);
LWSTDAPI_(BOOL)
PathSearchAndQualifyW(_In_ LPCWSTR pszPath, _Out_writes_(cchBuf) LPWSTR pszBuf,
                      _In_ UINT cchBuf);
...
#endif // !UNICODE
LWSTDAPI_(BOOL)
PathIsSystemFolderA(_In_opt_ LPCSTR pszPath, _In_ DWORD dwAttrb);
LWSTDAPI_(BOOL)
PathIsSystemFolderW(_In_opt_ LPCWSTR pszPath, _In_ DWORD dwAttrb);
#ifdef UNICODE
...
#endif // !UNICODE
LWSTDAPI_(BOOL)
PathUnExpandEnvStringsA(_In_ LPCSTR pszPath, _Out_writes_(cchBuf) LPSTR pszBuf,
                        _In_ UINT cchBuf);
LWSTDAPI_(BOOL)
PathUnExpandEnvStringsW(_In_ LPCWSTR pszPath,
                        _Out_writes_(cchBuf) LPWSTR pszBuf, _In_ UINT cchBuf);
#ifdef UNICODE
...
#define URL_ESCAPE_AS_UTF8                                                     \
  0x00040000 // Percent-encode all non-ASCII characters as their UTF-8
             // equivalents.
#endif       // (NTDDI_VERSION >= NTDDI_WIN7)
#if (NTDDI_VERSION >= NTDDI_WIN8)
#define URL_UNESCAPE_AS_UTF8 URL_ESCAPE_AS_UTF8
#define URL_ESCAPE_ASCII_URI_COMPONENT                                         \
  0x00080000 // Percent-encode all ASCII characters outside of the unreserved
             // set from URI RFC 3986 (a-zA-Z0-9-.~_) (i.e.) No need for
             // URL_ESCAPE_PERCENT along with this.
...
#define URL_ESCAPE_SEGMENT_ONLY                                                \
  0x00002000 // Treat the entire URL param as one URL segment.
...
LWSTDAPI_(int)
UrlCompareW(_In_ PCWSTR psz1, _In_ PCWSTR psz2, BOOL fIgnoreSlash);
LWSTDAPI UrlCombineA(_In_ PCSTR pszBase, _In_ PCSTR pszRelative,
                     _Out_writes_to_opt_(*pcchCombined, *pcchCombined)
                         PSTR pszCombined,
                     _Inout_ DWORD *pcchCombined, DWORD dwFlags);
LWSTDAPI UrlCombineW(_In_ PCWSTR pszBase, _In_ PCWSTR pszRelative,
                     _Out_writes_to_opt_(*pcchCombined, *pcchCombined)
                         PWSTR pszCombined,
                     _Inout_ DWORD *pcchCombined, DWORD dwFlags);
LWSTDAPI UrlCanonicalizeA(_In_ PCSTR pszUrl,
                          _Out_writes_to_(*pcchCanonicalized,
                                          *pcchCanonicalized)
                              PSTR pszCanonicalized,
                          _Inout_ DWORD *pcchCanonicalized, DWORD dwFlags);
LWSTDAPI UrlCanonicalizeW(_In_ PCWSTR pszUrl,
                          _Out_writes_to_(*pcchCanonicalized,
                                          *pcchCanonicalized)
...
LWSTDAPI UrlUnescapeA(_Inout_ PSTR pszUrl,
                      _Out_writes_to_opt_(*pcchUnescaped, *pcchUnescaped)
                          PSTR pszUnescaped,
                      _Inout_opt_ DWORD *pcchUnescaped, DWORD dwFlags);
LWSTDAPI UrlUnescapeW(_Inout_ PWSTR pszUrl,
                      _Out_writes_to_opt_(*pcchUnescaped, *pcchUnescaped)
                          PWSTR pszUnescaped,
                      _Inout_opt_ DWORD *pcchUnescaped, DWORD dwFlags);
LWSTDAPI UrlEscapeA(_In_ PCSTR pszUrl,
                    _Out_writes_to_(*pcchEscaped, *pcchEscaped) PSTR pszEscaped,
                    _Inout_ DWORD *pcchEscaped, DWORD dwFlags);
LWSTDAPI UrlEscapeW(_In_ PCWSTR pszUrl,
                    _Out_writes_to_(*pcchEscaped, *pcchEscaped)
                        PWSTR pszEscaped,
                    _Inout_ DWORD *pcchEscaped, DWORD dwFlags);
LWSTDAPI UrlCreateFromPathA(_In_ PCSTR pszPath,
                            _Out_writes_to_(*pcchUrl, *pcchUrl) PSTR pszUrl,
                            _Inout_ DWORD *pcchUrl, DWORD dwFlags);
LWSTDAPI UrlCreateFromPathW(_In_ PCWSTR pszPath,
                            _Out_writes_to_(*pcchUrl, *pcchUrl) PWSTR pszUrl,
                            _Inout_ DWORD *pcchUrl, DWORD dwFlags);
LWSTDAPI PathCreateFromUrlA(_In_ PCSTR pszUrl,
                            _Out_writes_to_(*pcchPath, *pcchPath) PSTR pszPath,
                            _Inout_ DWORD *pcchPath, DWORD dwFlags);
LWSTDAPI PathCreateFromUrlW(_In_ PCWSTR pszUrl,
                            _Out_writes_to_(*pcchPath, *pcchPath) PWSTR pszPath,
                            _Inout_ DWORD *pcchPath, DWORD dwFlags);
#if (_WIN32_IE >= _WIN32_IE_IE70)
LWSTDAPI PathCreateFromUrlAlloc(_In_ PCWSTR pszIn, _Outptr_ PWSTR *ppszOut,
                                DWORD dwFlags);
#endif // _WIN32_IE_IE70
LWSTDAPI UrlHashA(_In_ PCSTR pszUrl, _Out_writes_bytes_(cbHash) BYTE *pbHash,
                  DWORD cbHash);
LWSTDAPI UrlHashW(_In_ PCWSTR pszUrl, _Out_writes_bytes_(cbHash) BYTE *pbHash,
                  DWORD cbHash);
LWSTDAPI UrlGetPartW(_In_ PCWSTR pszIn, _Out_writes_(*pcchOut) PWSTR pszOut,
                     _Inout_ DWORD *pcchOut, DWORD dwPart, DWORD dwFlags);
LWSTDAPI UrlGetPartA(_In_ PCSTR pszIn, _Out_writes_(*pcchOut) PSTR pszOut,
                     _Inout_ DWORD *pcchOut, DWORD dwPart, DWORD dwFlags);
LWSTDAPI UrlApplySchemeA(_In_ PCSTR pszIn, _Out_writes_(*pcchOut) PSTR pszOut,
                         _Inout_ DWORD *pcchOut, DWORD dwFlags);
LWSTDAPI UrlApplySchemeW(_In_ PCWSTR pszIn, _Out_writes_(*pcchOut) PWSTR pszOut,
                         _Inout_ DWORD *pcchOut, DWORD dwFlags);
LWSTDAPI HashData(_In_reads_bytes_(cbData) BYTE *pbData, DWORD cbData,
                  _Out_writes_bytes_(cbHash) BYTE *pbHash, DWORD cbHash);
LWSTDAPI UrlFixupW(_In_ PCWSTR pcszUrl,
                   _Out_writes_(cchMax) PWSTR pszTranslatedUrl, DWORD cchMax);
...
LWSTDAPI_(LSTATUS)
SHDeleteEmptyKeyW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubKey);
#ifdef UNICODE
...
LWSTDAPI_(LSTATUS)
SHDeleteValueA(_In_ HKEY hkey, _In_opt_ LPCSTR pszSubKey, _In_ LPCSTR pszValue);
LWSTDAPI_(LSTATUS)
SHDeleteValueW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubKey,
...
#endif // !UNICODE
LWSTDAPI_(LSTATUS)
SHGetValueA(_In_ HKEY hkey, _In_opt_ LPCSTR pszSubKey, _In_opt_ LPCSTR pszValue,
            _Out_opt_ DWORD *pdwType,
            _Out_writes_bytes_opt_(*pcbData) void *pvData,
            _Inout_opt_ DWORD *pcbData);
LWSTDAPI_(LSTATUS)
SHGetValueW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubKey,
            _In_opt_ LPCWSTR pszValue, _Out_opt_ DWORD *pdwType,
            _Out_writes_bytes_opt_(*pcbData) void *pvData,
            _Inout_opt_ DWORD *pcbData);
#ifdef UNICODE
...
#endif // !UNICODE
LWSTDAPI_(LSTATUS)
SHSetValueA(_In_ HKEY hkey, _In_opt_ LPCSTR pszSubKey, _In_opt_ LPCSTR pszValue,
            _In_ DWORD dwType, _In_reads_bytes_opt_(cbData) LPCVOID pvData,
            _In_ DWORD cbData);
LWSTDAPI_(LSTATUS)
SHSetValueW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubKey,
            _In_opt_ LPCWSTR pszValue, _In_ DWORD dwType,
...
#define SRRF_RT_REG_NONE                                                       \
  0x00000001 // restrict type to REG_NONE      (other data types will not return
             // ERROR_SUCCESS)
#define SRRF_RT_REG_SZ                                                         \
  0x00000002 // restrict type to REG_SZ        (other data types will not return
             // ERROR_SUCCESS) (automatically converts REG_EXPAND_SZ to REG_SZ
             // unless SRRF_NOEXPAND is specified)
#define SRRF_RT_REG_EXPAND_SZ                                                  \
  0x00000004 // restrict type to REG_EXPAND_SZ (other data types will not return
             // ERROR_SUCCESS) (must specify SRRF_NOEXPAND or SHRegGetValue will
             // fail with ERROR_INVALID_PARAMETER)
#define SRRF_RT_REG_BINARY                                                     \
  0x00000008 // restrict type to REG_BINARY    (other data types will not return
             // ERROR_SUCCESS)
#define SRRF_RT_REG_DWORD                                                      \
  0x00000010 // restrict type to REG_DWORD     (other data types will not return
             // ERROR_SUCCESS)
#define SRRF_RT_REG_MULTI_SZ                                                   \
  0x00000020 // restrict type to REG_MULTI_SZ  (other data types will not return
             // ERROR_SUCCESS)
#define SRRF_RT_REG_QWORD                                                      \
  0x00000040 // restrict type to REG_QWORD     (other data types will not return
             // ERROR_SUCCESS)

#define SRRF_RT_DWORD                                                          \
  (SRRF_RT_REG_BINARY |                                                        \
   SRRF_RT_REG_DWORD) // restrict type to *32-bit* SRRF_RT_REG_BINARY or
                      // SRRF_RT_REG_DWORD (other data types will not return
                      // ERROR_SUCCESS)
#define SRRF_RT_QWORD                                                          \
  (SRRF_RT_REG_BINARY |                                                        \
   SRRF_RT_REG_QWORD) // restrict type to *64-bit* SRRF_RT_REG_BINARY or
                      // SRRF_RT_REG_DWORD (other data types will not return
                      // ERROR_SUCCESS)
#define SRRF_RT_ANY 0x0000ffff // no type restriction

#define SRRF_RM_ANY                                                            \
  0x00000000 // no mode restriction (default is to allow any mode)
#define SRRF_RM_NORMAL                                                         \
  0x00010000 // restrict system startup mode to "normal boot" (other startup
             // modes will not return ERROR_SUCCESS)
#define SRRF_RM_SAFE                                                           \
  0x00020000 // restrict system startup mode to "safe mode" (other startup modes
             // will not return ERROR_SUCCESS)
#define SRRF_RM_SAFENETWORK                                                    \
  0x00040000 // restrict system startup mode to "safe mode with networking"
             // (other startup modes will not return ERROR_SUCCESS)

#define SRRF_NOEXPAND                                                          \
  0x10000000 // do not automatically expand environment strings if value is of
             // type REG_EXPAND_SZ
#define SRRF_ZEROONFAILURE                                                     \
  0x20000000 // if pvData is not NULL, set content to all zeros on failure
#define SRRF_NOVIRT                                                            \
  0x40000000 // if the requested key is virtualized, then fail with
...
//                       handling semantics:
//
//                         SRRF_RT_DWORD == SRRF_RT_REG_BINARY |
//                         SRRF_RT_REG_DWORD SRRF_RT_QWORD == SRRF_RT_REG_BINARY
//                         | SRRF_RT_REG_QWORD
//
...
// Examples:
//
//  1) read REG_SZ (or REG_EXPAND_SZ as REG_SZ) "string" data from the (default)
//  value of an open hkey
//
//      TCHAR szData[128]
//      DWORD cbData = sizeof(pszData)
//      if (ERROR_SUCCESS == SHRegGetValue(hkey, NULL, NULL, SRRF_RT_REG_SZ,
//      NULL, szData, &cbData))
//      {
//          // use sz (successful read)
//      }
//
//  2) read REG_SZ (or REG_EXPAND_SZ as REG_SZ) "string" data of unknown size
//  from the "MyValue" value of an open hkey
//
//      DWORD cbData
//      if (ERROR_SUCCESS == SHRegGetValue(hkey, NULL, TEXT("MyValue"),
//      SRRF_RT_REG_SZ, NULL, NULL, &cbData))
//      {
//          TCHAR *pszData = new TCHAR[cbData/sizeof(TCHAR)]
//          if (pszData)
//          {
//              if (ERROR_SUCCESS == SHRegGetValue(hkey, NULL, TEXT("MyValue"),
//              SRRF_RT_REG_SZ, NULL, pszData, &cbData))
//              {
...
//      }
//
//  3) read "dword" data from the "MyValue" value of the "MySubKey" subkey of an
//  open hkey
//
//      DWORD dwData
//      DWORD cbData = sizeof(dwData)
//      if (ERROR_SUCCESS == SHRegGetValue(hkey, TEXT("MySubKey"),
//      TEXT("MyValue"), SRRF_RT_REG_DWORD, NULL, &dwData, &cbData))
//      {
//          // use dwData (successful read)
//      }
//
//  4) read "dword" data from the "MyValue" value of the "MySubKey" subkey of an
//  open hkey (32-bit binary data also ok)
//
//      DWORD dwData
//      DWORD cbData = sizeof(dwData)
//      if (ERROR_SUCCESS == SHRegGetValue(hkey, TEXT("MySubKey"),
//      TEXT("MyValue"), SRRF_RT_DWORD, NULL, &dwData, &cbData))
//      {
//          // use dwData (successful read)
//      }
//
...
//
//      BOOL bExists = ERROR_SUCCESS == SHRegGetValue(hkey, NULL,
//      TEXT("MyValue"), SRRF_RT_ANY, NULL, NULL, NULL)

LWSTDAPI_(LSTATUS)
SHRegGetValueA(_In_ HKEY hkey, _In_opt_ LPCSTR pszSubKey,
               _In_opt_ LPCSTR pszValue, _In_ SRRF srrfFlags,
               _Out_opt_ DWORD *pdwType,
               _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
               _Inout_opt_ DWORD *pcbData);
LWSTDAPI_(LSTATUS)
SHRegGetValueW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubKey,
               _In_opt_ LPCWSTR pszValue, _In_ SRRF srrfFlags,
               _Out_opt_ DWORD *pdwType,
               _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
               _Inout_opt_ DWORD *pcbData);
#ifdef UNICODE
...
LWSTDAPI_(LSTATUS)
SHRegSetValue(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubKey,
              _In_opt_ LPCWSTR pszValue, _In_ SRRF srrfFlags, _In_ DWORD dwType,
              _In_reads_bytes_opt_(cbData) LPCVOID pvData,
              _In_opt_ DWORD cbData);

LWSTDAPI_(LSTATUS)
SHRegGetValueFromHKCUHKLM(_In_ PCWSTR pwszKey, _In_opt_ PCWSTR pwszValue,
                          _In_ SRRF srrfFlags, _Out_opt_ DWORD *pdwType,
                          _Out_writes_bytes_to_opt_(*pcbData,
                                                    *pcbData) void *pvData,
                          _Inout_opt_ _When_(pvData != 0, _Pre_notnull_)
                              DWORD *pcbData);
STDAPI_(BOOL)
SHRegGetBoolValueFromHKCUHKLM(_In_ PCWSTR pszKey, _In_opt_ PCWSTR pszValue,
                              _In_ BOOL fDefault);
...
// to be REG_SZ.
LWSTDAPI_(LSTATUS)
SHQueryValueExA(_In_ HKEY hkey, _In_opt_ LPCSTR pszValue,
                _Reserved_ DWORD *pdwReserved, _Out_opt_ DWORD *pdwType,
                _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                _Inout_opt_ DWORD *pcbData);
// These functions behave just like RegQueryValueEx(), except if the data
// type is REG_SZ, REG_EXPAND_SZ or REG_MULTI_SZ then the string is
...
// to be REG_SZ.
LWSTDAPI_(LSTATUS)
SHQueryValueExW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszValue,
                _Reserved_ DWORD *pdwReserved, _Out_opt_ DWORD *pdwType,
                _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                _Inout_opt_ DWORD *pcbData);
#ifdef UNICODE
...
LWSTDAPI_(LSTATUS)
SHEnumKeyExA(_In_ HKEY hkey, _In_ DWORD dwIndex,
             _Out_writes_(*pcchName) LPSTR pszName, _Inout_ LPDWORD pcchName);
LWSTDAPI_(LSTATUS)
SHEnumKeyExW(_In_ HKEY hkey, _In_ DWORD dwIndex,
             _Out_writes_(*pcchName) LPWSTR pszName, _Inout_ LPDWORD pcchName);
LWSTDAPI_(LSTATUS)
SHEnumValueA(_In_ HKEY hkey, _In_ DWORD dwIndex,
             _Out_writes_opt_(*pcchValueName) PSTR pszValueName,
             _Inout_opt_ LPDWORD pcchValueName, _Out_opt_ LPDWORD pdwType,
             _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
             _Inout_opt_ LPDWORD pcbData);
LWSTDAPI_(LSTATUS)
SHEnumValueW(_In_ HKEY hkey, _In_ DWORD dwIndex,
             _Out_writes_opt_(*pcchValueName) PWSTR pszValueName,
             _Inout_opt_ LPDWORD pcchValueName, _Out_opt_ LPDWORD pdwType,
             _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
             _Inout_opt_ LPDWORD pcbData);
LWSTDAPI_(LSTATUS)
SHQueryInfoKeyA(_In_ HKEY hkey, _Out_opt_ LPDWORD pcSubKeys,
                _Out_opt_ LPDWORD pcchMaxSubKeyLen, _Out_opt_ LPDWORD pcValues,
                _Out_opt_ LPDWORD pcchMaxValueNameLen);
LWSTDAPI_(LSTATUS)
SHQueryInfoKeyW(_In_ HKEY hkey, _Out_opt_ LPDWORD pcSubKeys,
                _Out_opt_ LPDWORD pcchMaxSubKeyLen, _Out_opt_ LPDWORD pcValues,
                _Out_opt_ LPDWORD pcchMaxValueNameLen);

// recursive key copy
LWSTDAPI_(LSTATUS)
SHCopyKeyA(_In_ HKEY hkeySrc, _In_opt_ LPCSTR pszSrcSubKey, _In_ HKEY hkeyDest,
           _Reserved_ DWORD fReserved);
LWSTDAPI_(LSTATUS)
SHCopyKeyW(_In_ HKEY hkeySrc, _In_opt_ LPCWSTR pszSrcSubKey, _In_ HKEY hkeyDest,
           _Reserved_ DWORD fReserved);

// Getting and setting file system paths with environment variables

LWSTDAPI_(LSTATUS)
SHRegGetPathA(_In_ HKEY hKey, _In_opt_ LPCSTR pcszSubKey,
              _In_opt_ LPCSTR pcszValue, _Out_writes_(MAX_PATH) LPSTR pszPath,
              _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegGetPathW(_In_ HKEY hKey, _In_opt_ LPCWSTR pcszSubKey,
              _In_opt_ LPCWSTR pcszValue, _Out_writes_(MAX_PATH) LPWSTR pszPath,
              _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegSetPathA(_In_ HKEY hKey, _In_opt_ LPCSTR pcszSubKey,
              _In_opt_ LPCSTR pcszValue, _In_ LPCSTR pcszPath,
              _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegSetPathW(_In_ HKEY hKey, _In_opt_ LPCWSTR pcszSubKey,
              _In_opt_ LPCWSTR pcszValue, _In_ LPCWSTR pcszPath,
              _In_ DWORD dwFlags);
...
typedef enum {
  ...
  SHREGENUM_BOTH = 0x00000011,    // Enumerates both HKCU and HKLM without
                                  // duplicates. This option is NYI.
} SHREGENUM_FLAGS;
...
#define SHREGSET_DEFAULT                                                       \
  (SHREGSET_FORCE_HKCU |                                                       \
   SHREGSET_HKLM) // Default is SHREGSET_FORCE_HKCU | SHREGSET_HKLM.

typedef HANDLE HUSKEY; // HUSKEY is a Handle to a User Specific KEY.
typedef HUSKEY *PHUSKEY;

LWSTDAPI_(LSTATUS)
SHRegCreateUSKeyA(_In_ LPCSTR pszPath, _In_ REGSAM samDesired,
                  _In_opt_ HUSKEY hRelativeUSKey, _Out_ PHUSKEY phNewUSKey,
                  _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegCreateUSKeyW(_In_ LPCWSTR pwzPath, _In_ REGSAM samDesired,
                  _In_opt_ HUSKEY hRelativeUSKey, _Out_ PHUSKEY phNewUSKey,
                  _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegOpenUSKeyA(_In_ LPCSTR pszPath, _In_ REGSAM samDesired,
                _In_opt_ HUSKEY hRelativeUSKey, _Out_ PHUSKEY phNewUSKey,
                _In_ BOOL fIgnoreHKCU);
LWSTDAPI_(LSTATUS)
SHRegOpenUSKeyW(_In_ LPCWSTR pwzPath, _In_ REGSAM samDesired,
                _In_opt_ HUSKEY hRelativeUSKey, _Out_ PHUSKEY phNewUSKey,
                _In_ BOOL fIgnoreHKCU);
LWSTDAPI_(LSTATUS)
SHRegQueryUSValueA(_In_ HUSKEY hUSKey, _In_opt_ LPCSTR pszValue,
                   _Inout_opt_ DWORD *pdwType,
                   _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                   _Inout_opt_ DWORD *pcbData, _In_ BOOL fIgnoreHKCU,
                   _In_reads_bytes_opt_(dwDefaultDataSize) void *pvDefaultData,
                   _In_opt_ DWORD dwDefaultDataSize);
LWSTDAPI_(LSTATUS)
SHRegQueryUSValueW(_In_ HUSKEY hUSKey, _In_opt_ LPCWSTR pszValue,
                   _Inout_opt_ DWORD *pdwType,
                   _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                   _Inout_opt_ DWORD *pcbData, _In_ BOOL fIgnoreHKCU,
                   _In_reads_bytes_opt_(dwDefaultDataSize) void *pvDefaultData,
                   _In_opt_ DWORD dwDefaultDataSize);
LWSTDAPI_(LSTATUS)
SHRegWriteUSValueA(_In_ HUSKEY hUSKey, _In_ LPCSTR pszValue, _In_ DWORD dwType,
                   _In_reads_bytes_(cbData) const void *pvData,
                   _In_ DWORD cbData, _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegWriteUSValueW(_In_ HUSKEY hUSKey, _In_ LPCWSTR pwzValue, _In_ DWORD dwType,
                   _In_reads_bytes_(cbData) const void *pvData,
                   _In_ DWORD cbData, _In_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegDeleteUSValueA(_In_ HUSKEY hUSKey, _In_ LPCSTR pszValue,
                    _In_ SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LSTATUS)
SHRegDeleteUSValueW(_In_ HUSKEY hUSKey, _In_ LPCWSTR pwzValue,
                    _In_ SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LSTATUS)
SHRegDeleteEmptyUSKeyW(_In_ HUSKEY hUSKey, _In_ LPCWSTR pwzSubKey,
                       _In_ SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LSTATUS)
SHRegDeleteEmptyUSKeyA(_In_ HUSKEY hUSKey, _In_ LPCSTR pszSubKey,
                       _In_ SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LSTATUS)
SHRegEnumUSKeyA(_In_ HUSKEY hUSKey, _In_ DWORD dwIndex,
                _Out_writes_to_(*pcchName, *pcchName) LPSTR pszName,
                _Inout_ LPDWORD pcchName, _In_ SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LSTATUS)
SHRegEnumUSKeyW(_In_ HUSKEY hUSKey, _In_ DWORD dwIndex,
                _Out_writes_to_(*pcchName, *pcchName) LPWSTR pwzName,
                _Inout_ LPDWORD pcchName, _In_ SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LSTATUS)
SHRegEnumUSValueA(_In_ HUSKEY hUSkey, _In_ DWORD dwIndex,
                  _Out_writes_to_(*pcchValueName, *pcchValueName)
                      LPSTR pszValueName,
                  _Inout_ LPDWORD pcchValueName, _Out_opt_ LPDWORD pdwType,
                  _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                  _Inout_opt_ LPDWORD pcbData,
                  _In_ SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LSTATUS)
SHRegEnumUSValueW(_In_ HUSKEY hUSkey, _In_ DWORD dwIndex,
                  _Out_writes_to_(*pcchValueName, *pcchValueName)
                      LPWSTR pszValueName,
                  _Inout_ LPDWORD pcchValueName, _Out_opt_ LPDWORD pdwType,
                  _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                  ...
                  _In_ SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LSTATUS)
SHRegQueryInfoUSKeyA(_In_ HUSKEY hUSKey, _Out_opt_ LPDWORD pcSubKeys,
                     _Out_opt_ LPDWORD pcchMaxSubKeyLen,
                     _Out_opt_ LPDWORD pcValues,
                     _Out_opt_ LPDWORD pcchMaxValueNameLen,
                     _In_ SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LSTATUS)
SHRegQueryInfoUSKeyW(_In_ HUSKEY hUSKey, _Out_opt_ LPDWORD pcSubKeys,
                     _Out_opt_ LPDWORD pcchMaxSubKeyLen,
                     _Out_opt_ LPDWORD pcValues,
                     _Out_opt_ LPDWORD pcchMaxValueNameLen,
                     _In_ SHREGENUM_FLAGS enumRegFlags);
...
// These calls are equal to an SHRegOpenUSKey, SHRegQueryUSValue, and then a
// SHRegCloseUSKey.
STDAPI_(LSTATUS)
SHRegGetUSValueA(_In_ LPCSTR pszSubKey, _In_opt_ LPCSTR pszValue,
                 _Inout_opt_ DWORD *pdwType,
                 _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                 _Inout_opt_ DWORD *pcbData, _In_ BOOL fIgnoreHKCU,
                 _In_reads_bytes_opt_(dwDefaultDataSize) void *pvDefaultData,
...
STDAPI_(LSTATUS)
SHRegGetUSValueW(_In_ LPCWSTR pszSubKey, _In_opt_ LPCWSTR pszValue,
                 _Inout_opt_ DWORD *pdwType,
                 _Out_writes_bytes_to_opt_(*pcbData, *pcbData) void *pvData,
                 _Inout_opt_ DWORD *pcbData, _In_ BOOL fIgnoreHKCU,
                 _In_reads_bytes_opt_(dwDefaultDataSize) void *pvDefaultData,
                 _In_ DWORD dwDefaultDataSize);
LWSTDAPI_(LSTATUS)
SHRegSetUSValueA(_In_ LPCSTR pszSubKey, _In_ LPCSTR pszValue, _In_ DWORD dwType,
                 _In_reads_bytes_opt_(cbData) const void *pvData,
                 _In_opt_ DWORD cbData, _In_opt_ DWORD dwFlags);
LWSTDAPI_(LSTATUS)
SHRegSetUSValueW(_In_ LPCWSTR pwzSubKey, _In_ LPCWSTR pwzValue,
                 _In_ DWORD dwType,
                 _In_reads_bytes_opt_(cbData) const void *pvData,
                 _In_opt_ DWORD cbData, _In_opt_ DWORD dwFlags);
LWSTDAPI_(int)
SHRegGetIntW(_In_ HKEY hk, _In_opt_ PCWSTR pwzKey, _In_ int iDefault);
...
LWSTDAPI_(BOOL)
SHRegGetBoolUSValueA(_In_ LPCSTR pszSubKey, _In_opt_ LPCSTR pszValue,
                     _In_ BOOL fIgnoreHKCU, _In_ BOOL fDefault);
LWSTDAPI_(BOOL)
SHRegGetBoolUSValueW(_In_ LPCWSTR pszSubKey, _In_opt_ LPCWSTR pszValue,
                     _In_ BOOL fIgnoreHKCU, _In_ BOOL fDefault);
#ifdef UNICODE
...
enum {
  ...
  ASSOCF_REMAPRUNDLL =
      0x00000080, //  actually gets info about rundlls target if applicable
  ...
  ASSOCF_INIT_IGNOREUNKNOWN =
      0x00000400, //  dont use the "Unknown" progid, instead fail
#if (NTDDI_VERSION >= NTDDI_WIN8)
  ASSOCF_INIT_FIXED_PROGID =
      0x00000800, //  the Init() pszAssoc value is a ProgId that should not be
                  //  mapped using the current user defaults
  ASSOCF_IS_PROTOCOL =
      0x00001000, //  the Init() pszAssoc value is an uri scheme (not including
                  //  the ":") that should be mapped using the current user
                  //  defaults
  ASSOCF_INIT_FOR_FILE =
      0x00002000, //  use this flag when specifying ASSOCF_INIT_FIXED_PROGID if
                  //  the ProgId corresponds with a file extension based
                  //  association
#endif
#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)
  ASSOCF_IS_FULL_URI =
      0x00004000, //  Used to specify that full http/https URI is being passed
                  //  for target resolution Only one of
                  //  ASSOCF_INIT_FIXED_PROGID, ASSOCF_IS_PROTOCOL or
                  //  ASSOCF_IS_FULL_URI can be specified at a time.
  ASSOCF_PER_MACHINE_ONLY = 0x00008000, //  Enforces per-machine association
                                        //  look-up only and avoid HKCU.
#endif
#if (NTDDI_VERSION >= NTDDI_WIN10_RS4)
  // For http/https uri associations, this enables selecting the OS default
  // browser, that is a controlled by policy settings, instead of the default
  // browser.
...
typedef enum {
  ...
  ASSOCSTR_INFOTIP, //  info tip for an item, or list of properties to create
                    //  info tip from
#if (_WIN32_IE >= _WIN32_IE_IE60)
  ASSOCSTR_QUICKTIP,    //  same as ASSOCSTR_INFOTIP, except, this list contains
                        //  only quickly retrievable properties
  ASSOCSTR_TILEINFO,    //  similar to ASSOCSTR_INFOTIP - lists important
                        //  properties for tileview
  ASSOCSTR_CONTENTTYPE, //  MIME Content type
  ASSOCSTR_DEFAULTICON, //  Default icon source
  ASSOCSTR_SHELLEXTENSION, //  Guid string pointing to the
                           //  Shellex\Shellextensionhandler value.
#endif                     // _WIN32_IE_IE60
  ...
  // a string value of the uri protocol schemes, for example
  // "http:https:ftp:file:" or "*" indicating all
  ASSOCSTR_SUPPORTED_URI_PROTOCOLS,
#if (NTDDI_VERSION >= NTDDI_WIN10)
  ASSOCSTR_PROGID, // The ProgId provided by the app associated with the file
                   // type or uri scheme based on user default settings.
  ASSOCSTR_APPID, // The AppUserModelID of the app associated with the file type
                  // or uri scheme based on user default settings.
  ASSOCSTR_APPPUBLISHER, // THe publisher of the app associated with the file
                         // type or uri scheme based on user default settings.
  ASSOCSTR_APPICONREFERENCE, // The icon reference of the app associated with
                             // the file type or uri scheme based on user
                             // default settings.
#endif                       // NTDDI_WIN10
...
typedef enum {
  ASSOCKEY_SHELLEXECCLASS =
      1,              //  the key that should be passed to ShellExec(hkeyClass)
...
// queryAssoc->GetData(nullptr, ASSOCDATA_EDITFLAGS, nullptr, &editFlags,
// &size);
//
// Some of these flags are no longer used since editing file type associations
// has been removed from Explorer's folder options UI.

typedef enum {
  FTA_None = 0x00000000,
  FTA_Exclude = 0x00000001, // used to exclude (hide) types like drvfile
  FTA_Show =
      0x00000002, // used to show types like folder that don't have associations
  ...
  FTA_NoEditVerbCmd =
      0x00000800, // no modification of the commnds associated with the verbs
  FTA_NoEditVerbExe = 0x00001000, // no editing of the verb's exe
  FTA_NoDDE = 0x00002000,         // no editing of the DDE fields

  FTA_NoEditMIME =
      0x00008000, // no editing of the Content Type or Default Extension fields
  FTA_OpenIsSafe = 0x00010000, // the open verb should be invoked automaticaly
                               // for downloaded files
  FTA_AlwaysUnsafe =
      0x00020000, // don't allow the "Never ask me" checkbox to be enabled; File
                  // Type dialog still allows user to turn this off

  FTA_NoRecentDocs =
      0x00100000, // don't add this file type to the Recent Documents folder
  FTA_SafeForElevation = 0x00200000, // Win8: can be launched in medium IL by a
                                     // process running in AppContainer
  FTA_AlwaysUseDirectInvoke =
      0x00400000, // Win8: when downloading use the direct invoke feature even
                  // if the server headers are not provided
...
DECLARE_INTERFACE_IID_(IQueryAssociations, IUnknown,
                       "c46ca590-3c3f-11d2-bee6-0000f805ca57") {
  // IQueryAssociations methods
  STDMETHOD(Init)(THIS_ _In_ ASSOCF flags, _In_opt_ LPCWSTR pszAssoc,
                  _In_opt_ HKEY hkProgid, _In_opt_ HWND hwnd) PURE;
  STDMETHOD(GetString)(
      THIS_ _In_ ASSOCF flags, _In_ ASSOCSTR str, _In_opt_ LPCWSTR pszExtra,
      _Out_writes_opt_(*pcchOut) LPWSTR pszOut, _Inout_ DWORD * pcchOut) PURE;
  STDMETHOD(GetKey)(THIS_ _In_ ASSOCF flags, _In_ ASSOCKEY key,
                    _In_opt_ LPCWSTR pszExtra, _Out_ HKEY * phkeyOut) PURE;
  STDMETHOD(GetData)(THIS_ _In_ ASSOCF flags, _In_ ASSOCDATA data,
                     _In_opt_ LPCWSTR pszExtra,
                     _Out_writes_bytes_opt_(*pcbOut) void *pvOut,
                     _Inout_opt_ DWORD *pcbOut) PURE;
  STDMETHOD(GetEnum)(THIS_ _In_ ASSOCF flags, _In_ ASSOCENUM assocenum,
                     _In_opt_ LPCWSTR pszExtra, _In_ REFIID riid,
                     _Outptr_ void **ppvOut) PURE;
};
...
STDAPI_(DWORD)
SHGetAssocKeys(_In_ IQueryAssociations *pqa,
               _Out_writes_to_(cKeys, return) HKEY *rgKeys, DWORD cKeys);

//  wrappers for the interface
LWSTDAPI AssocQueryStringA(_In_ ASSOCF flags, _In_ ASSOCSTR str,
                           _In_ LPCSTR pszAssoc, _In_opt_ LPCSTR pszExtra,
                           _Out_writes_opt_(*pcchOut) LPSTR pszOut,
                           _Inout_ DWORD *pcchOut);
//  wrappers for the interface
LWSTDAPI AssocQueryStringW(_In_ ASSOCF flags, _In_ ASSOCSTR str,
                           _In_ LPCWSTR pszAssoc, _In_opt_ LPCWSTR pszExtra,
...
#endif // !UNICODE
LWSTDAPI AssocQueryStringByKeyA(_In_ ASSOCF flags, _In_ ASSOCSTR str,
                                _In_ HKEY hkAssoc, _In_opt_ LPCSTR pszExtra,
                                _Out_writes_opt_(*pcchOut) LPSTR pszOut,
                                _Inout_ DWORD *pcchOut);
LWSTDAPI AssocQueryStringByKeyW(_In_ ASSOCF flags, _In_ ASSOCSTR str,
                                _In_ HKEY hkAssoc, _In_opt_ LPCWSTR pszExtra,
                                _Out_writes_opt_(*pcchOut) LPWSTR pszOut,
                                _Inout_ DWORD *pcchOut);
#ifdef UNICODE
...
#endif // !UNICODE
LWSTDAPI AssocQueryKeyA(_In_ ASSOCF flags, _In_ ASSOCKEY key,
                        _In_ LPCSTR pszAssoc, _In_opt_ LPCSTR pszExtra,
                        _Out_ HKEY *phkeyOut);
LWSTDAPI AssocQueryKeyW(_In_ ASSOCF flags, _In_ ASSOCKEY key,
                        _In_ LPCWSTR pszAssoc, _In_opt_ LPCWSTR pszExtra,
                        _Out_ HKEY *phkeyOut);
#ifdef UNICODE
...
//      dangerous file types should be handled more carefully than other file
//      types.
//
//  Parameter:  pszAssoc - type to check.  may be an extension or progid.
//  (".exe" or "exefile" would both be valid)
//
...
//      then it checks the editflags for the file type looking for the
//      FTA_AlwaysUnsafe bit. then it checks Safer policies.
//
...
#if (_WIN32_IE >= _WIN32_IE_IE60SP2)
LWSTDAPI AssocGetPerceivedType(_In_ PCWSTR pszExt, _Out_ PERCEIVED *ptype,
                               _Out_ PERCEIVEDFLAG *pflag,
                               _Outptr_opt_ PWSTR *ppszType);
#endif
...
LWSTDAPI_(IStream *)
SHOpenRegStreamA(_In_ HKEY hkey, _In_opt_ LPCSTR pszSubkey,
                 _In_opt_ LPCSTR pszValue, _In_ DWORD grfMode);
LWSTDAPI_(IStream *)
SHOpenRegStreamW(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubkey,
...
#endif // !UNICODE
LWSTDAPI_(IStream *)
SHOpenRegStream2A(_In_ HKEY hkey, _In_opt_ LPCSTR pszSubkey,
                  _In_opt_ LPCSTR pszValue, _In_ DWORD grfMode);
LWSTDAPI_(IStream *)
SHOpenRegStream2W(_In_ HKEY hkey, _In_opt_ LPCWSTR pszSubkey,
                  _In_opt_ LPCWSTR pszValue, _In_ DWORD grfMode);
#ifdef UNICODE
...
LWSTDAPI SHCreateStreamOnFileA(_In_ LPCSTR pszFile, _In_ DWORD grfMode,
                               _Outptr_ IStream **ppstm);
LWSTDAPI SHCreateStreamOnFileW(_In_ LPCWSTR pszFile, _In_ DWORD grfMode,
                               _Outptr_ IStream **ppstm);
#ifdef UNICODE
...
LWSTDAPI SHCreateStreamOnFileEx(_In_ LPCWSTR pszFile, _In_ DWORD grfMode,
                                _In_ DWORD dwAttributes, _In_ BOOL fCreate,
...
LWSTDAPI_(IStream *)
SHCreateMemStream(_In_reads_bytes_opt_(cbInit) const BYTE *pInit,
                  _In_ UINT cbInit);
...
#if (_WIN32_IE >= 0x0603)
LWSTDAPI GetAcceptLanguagesA(_Out_writes_to_(*pcchLanguages, *pcchLanguages)
                                 LPSTR pszLanguages,
                             _Inout_ DWORD *pcchLanguages);
LWSTDAPI GetAcceptLanguagesW(_Out_writes_to_(*pcchLanguages, *pcchLanguages)
                                 LPWSTR pszLanguages,
                             _Inout_ DWORD *pcchLanguages);
#ifdef UNICODE
...
LWSTDAPI IUnknown_GetSite(_In_ IUnknown *punk, _In_ REFIID riid,
                          _Outptr_ void **ppv);
LWSTDAPI IUnknown_QueryService(_In_opt_ IUnknown *punk,
                               _In_ REFGUID guidService, _In_ REFIID riid,
...
LWSTDAPI IStream_Read(_In_ IStream *pstm, _Out_writes_bytes_all_(cb) void *pv,
                      _In_ ULONG cb);
LWSTDAPI IStream_Write(_In_ IStream *pstm, _In_reads_bytes_(cb) const void *pv,
                       _In_ ULONG cb);
...
LWSTDAPI ConnectToConnectionPoint(_In_opt_ IUnknown *punk,
                                  _In_ REFIID riidEvent, BOOL fConnect,
                                  _In_ IUnknown *punkTarget,
                                  _Out_ DWORD *pdwCookie,
                                  _Outptr_opt_ IConnectionPoint **ppcpOut);

#if (NTDDI_VERSION >= NTDDI_VISTA)

LWSTDAPI IStream_ReadPidl(_In_ IStream *pstm,
                          _Outptr_ PIDLIST_RELATIVE *ppidlOut);
LWSTDAPI IStream_WritePidl(_In_ IStream *pstm,
                           _In_ PCUIDLIST_RELATIVE pidlWrite);
...
#define SHGVSPB_PERFOLDER                                                      \
  0x00000004 // must have one of PERFOLDER ALLFOLDERS or INHERIT
...
#define SHGVSPB_NOAUTODEFAULTS                                                 \
  0x80000000 // turns off read delegation to more general property bags
...
LWSTDAPI SHGetViewStatePropertyBag(_In_opt_ PCIDLIST_ABSOLUTE pidl,
                                   _In_opt_ PCWSTR pszBagName, DWORD dwFlags,
...
#define FDTF_NOAUTOREADINGORDER                                                \
  0x00000400 // Don't detect reading order automatically. Useful if you will be
             // converting to Ansi and don't want Unicode reading order
             // characters

LWSTDAPI_(int)
SHFormatDateTimeA(_In_ const FILETIME UNALIGNED *pft,
                  _Inout_opt_ DWORD *pdwFlags,
                  _Out_writes_(cchBuf) LPSTR pszBuf, UINT cchBuf);
LWSTDAPI_(int)
SHFormatDateTimeW(_In_ const FILETIME UNALIGNED *pft,
                  _Inout_opt_ DWORD *pdwFlags,
                  _Out_writes_(cchBuf) LPWSTR pszBuf, UINT cchBuf);
#ifdef UNICODE
...
_Success_(return > 0) LWSTDAPI_(int)
    SHAnsiToUnicode(_In_ PCSTR pszSrc, _Out_writes_(cwchBuf) PWSTR pwszDst,
                    _In_ int cwchBuf);
_Success_(return > 0) LWSTDAPI_(int)
    SHAnsiToAnsi(_In_ PCSTR pszSrc, _Out_writes_(cchBuf) PSTR pszDst,
                 _In_ int cchBuf);
_Success_(return > 0) LWSTDAPI_(int)
    SHUnicodeToAnsi(_In_ PCWSTR pwszSrc, _Out_writes_(cchBuf) PSTR pszDst,
                    _In_ int cchBuf);
_Success_(return > 0) LWSTDAPI_(int)
    SHUnicodeToUnicode(_In_ PCWSTR pwzSrc, _Out_writes_(cwchBuf) PWSTR pwzDst,
                       _In_ int cwchBuf);

// The return value from all SH<Type>To<Type> is the size of szDest including
// the terminater.
#ifdef UNICODE
...
LWSTDAPI_(int)
SHMessageBoxCheckA(_In_opt_ HWND hwnd, _In_ LPCSTR pszText,
                   _In_ LPCSTR pszCaption, UINT uType, int iDefault,
                   _In_ LPCSTR pszRegVal);
LWSTDAPI_(int)
SHMessageBoxCheckW(_In_opt_ HWND hwnd, _In_ LPCWSTR pszText,
                   _In_ LPCWSTR pszCaption, UINT uType, int iDefault,
                   _In_ LPCWSTR pszRegVal);
#ifdef UNICODE
...
LWSTDAPI_(LRESULT)
SHSendMessageBroadcastA(_In_ UINT uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam);
// Prevents hang do to hung window on broadcast
LWSTDAPI_(LRESULT)
SHSendMessageBroadcastW(_In_ UINT uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam);
#ifdef UNICODE
...
#define OS_WIN2000TERMINAL                                                     \
  12 // Windows 2000 Terminal Server in "Application Server" mode (now simply
     // called "Terminal Server")

#define OS_EMBEDDED 13 // Embedded Windows Edition
#define OS_TERMINALCLIENT                                                      \
  14 // Windows Terminal Client (eg user is comming in via tsclient)
#define OS_TERMINALREMOTEADMIN                                                 \
  15                      // Terminal Server in "Remote Administration" mode
...
#define OS_HOME                                                                \
  19 // Home Edition (eg NOT Professional, Server, Advanced Server, or
     // Datacenter)
#define OS_PROFESSIONAL                                                        \
  20 // Professional     (aka Workstation; eg NOT Server, Advanced Server, or
     // Datacenter)
#define OS_DATACENTER                                                          \
  21 // Datacenter       (eg NOT Server, Advanced Server, Professional, or
     // Personal)
#define OS_ADVSERVER                                                           \
  22 // Advanced Server  (eg NOT Datacenter, Server, Professional, or Personal)
#define OS_SERVER                                                              \
  23 // Server           (eg NOT Datacenter, Advanced Server, Professional, or
     // Personal)
#define OS_TERMINALSERVER                                                      \
  24 // Terminal Server - server running in what used to be called "Application
     // Server" mode (now simply called "Terminal Server")
#define OS_PERSONALTERMINALSERVER                                              \
  25 // Personal Terminal Server - per/pro machine running in single user TS
     // mode
...
#define OS_DOMAINMEMBER                                                        \
  28 // Is this machine a member of a domain (eg NOT a workgroup)
#define OS_ANYSERVER                                                           \
  29 // is this machine any type of server? (eg datacenter or advanced server or
     // server)?
#define OS_WOW6432                                                             \
  30 // Is this process a 32-bit process running on an 64-bit platform?
...
#define OS_SERVERADMINUI                                                       \
  34 // Should defaults lean towards those preferred by server administrators?
...
typedef enum {
  ...
  GLOBALCOUNTER_ASSOCCHANGED, // throttles reading of the registry value
                              // "GlobalAssocChangedCounter" from
...
#if (_WIN32_IE >= 0x0603)
LWSTDAPI_(HANDLE)
SHAllocShared(_In_reads_bytes_opt_(dwSize) const void *pvData,
              _In_ DWORD dwSize, _In_ DWORD dwProcessId);
...
LWSTDAPI_(BOOL)
SHUnlockShared(_In_reads_(_Inexpressible_("header-preceded")) void *pvData);
#endif // _WIN32_IE >= 0x0603
...
STDAPI QISearch(_Inout_ void *that, _In_ LPCQITAB pqit, _In_ REFIID riid,
...
//      dwFlags - Flags to indicate what to AutoAppend or AutoSuggest for the
//      editbox.
//
// WARNING:
...
#define SHACF_FILESYSTEM                                                       \
  0x00000001 // This includes the File System as well as the rest of the shell
...
#define SHACF_USETAB                                                           \
  0x00000008 // Use the tab to move thru the autocomplete possibilities instead
...
#define SHACF_FILESYS_DIRS                                                     \
  0x00000020 // Same as SHACF_FILESYS_ONLY except it only includes directories,
...
#define SHACF_AUTOSUGGEST_FORCE_ON                                             \
  0x10000000 // Ignore the registry default and force the feature on.
#define SHACF_AUTOSUGGEST_FORCE_OFF                                            \
  0x20000000 // Ignore the registry default and force the feature off.
#define SHACF_AUTOAPPEND_FORCE_ON                                              \
  0x40000000 // Ignore the registry default and force the feature on. (Also know
             // as AutoComplete)
#define SHACF_AUTOAPPEND_FORCE_OFF                                             \
  0x80000000 // Ignore the registry default and force the feature off. (Also
             // know as AutoComplete)
...
LWSTDAPI_(BOOL)
SHSkipJunction(_In_opt_ IBindCtx *pbc, _In_ const CLSID *pclsid);

enum {
  CTF_INSIST =
      0x00000001, // call pfnThreadProc synchronously if CreateThread() fails
...
#if (_WIN32_IE >= _WIN32_IE_IE60)
  CTF_FREELIBANDEXIT = 0x00000010, // hold a ref to the DLL and call
                                   // FreeLibraryAndExitThread() when done
  CTF_REF_COUNTED = 0x00000020,    // thread supports ref counting via
                                   // SHGetThreadRef() or CTF_THREAD_REF so that
                                   // child threads can keep this thread alive
  CTF_WAIT_ALLOWCOM = 0x00000040,  // while waiting for pfnCallback, allow COM
                                   // marshaling to the blocked calling thread
#endif                             // _WIN32_IE_IE60
#if (_WIN32_IE >= _WIN32_IE_IE70)
  CTF_UNUSED = 0x00000080,
  CTF_INHERITWOW64 = 0x00000100, // new thread should inherit the wow64 disable
                                 // state for the file system redirector
#endif                           // _WIN32_IE_IE70
#if (NTDDI_VERSION >= NTDDI_VISTA)
  CTF_WAIT_NO_REENTRANCY =
      0x00000200, // don't allow re-entrancy when waiting for the sync proc,
                  // this won't work with marshalled objects or SendMessages()
                  // from the sync proc
#endif            // (NTDDI_VERSION >= NTDDI_VISTA)
#if (NTDDI_VERSION >= NTDDI_WIN7)
  CTF_KEYBOARD_LOCALE =
      0x00000400, // carry the keyboard locale from creating to created thread
  CTF_OLEINITIALIZE = 0x00000800, // init OLE on the created thread (this will
                                  // also init COM as STA)
  CTF_COINIT_MTA = 0x00001000,    // init COM as MTA for the created thread
  CTF_NOADDREFLIB =
      0x00002000, // this flag is the opposite of CTF_FREELIBANDEXIT that is now
                  // implicit as of Vista this avoids the
                  // LoadLibrary/FreeLibraryAndExitThread calls that result in
                  // contention for the loader lock only use this when the
                  // thread being created has some other means to ensure that
                  // the code of the thread proc will remain loaded. This should
                  // not be used in the context of COM objects as those need to
                  // ensure that the DLL stays loaded as COM will unload DLLs
#endif            // (NTDDI_VERSION >= NTDDI_WIN7)
};
...
LWSTDAPI_(BOOL)
SHCreateThread(_In_ LPTHREAD_START_ROUTINE pfnThreadProc, _In_opt_ void *pData,
               _In_ SHCT_FLAGS flags,
               _In_opt_ LPTHREAD_START_ROUTINE pfnCallback);
LWSTDAPI_(BOOL)
SHCreateThreadWithHandle(_In_ LPTHREAD_START_ROUTINE pfnThreadProc,
                         _In_opt_ void *pData, _In_ SHCT_FLAGS flags,
                         _In_opt_ LPTHREAD_START_ROUTINE pfnCallback,
                         _Out_opt_ HANDLE *pHandle);
...
#if (_WIN32_IE >= _WIN32_IE_IE60)
LWSTDAPI SHReleaseThreadRef(void); // release a CTF_THREAD_REF reference earlier
...
LWSTDAPI_(void)
ColorRGBToHLS(COLORREF clrRGB, _Out_ WORD *pwHue, _Out_ WORD *pwLuminance,
              _Out_ WORD *pwSaturation);
...
#if (_WIN32_IE >= 0x0602)
// Function to see if Internet Explorer Enhanced Security Configuration is
...
__inline PWSTR StrChrNW(_In_ PWSTR pszStart, WCHAR wMatch, UINT cchMax) {
  return const_cast<PWSTR>(
      StrChrNW(const_cast<PCWSTR>(pszStart), wMatch, cchMax));
}
__inline PWSTR StrChrNIW(_In_ PWSTR pszStart, WCHAR wMatch, UINT cchMax) {
  return const_cast<PWSTR>(
      StrChrNIW(const_cast<PCWSTR>(pszStart), wMatch, cchMax));
}
#endif // _WIN32_IE_IE60
...
__inline PSTR StrRChrA(_In_ PSTR pszStart, _In_opt_ PCSTR pszEnd, WORD wMatch) {
  return const_cast<PSTR>(
      StrRChrA(const_cast<PCSTR>(pszStart), pszEnd, wMatch));
}
__inline PWSTR StrRChrW(_In_ PWSTR pszStart, _In_opt_ PCWSTR pszEnd,
                        WCHAR wMatch) {
  return const_cast<PWSTR>(
      StrRChrW(const_cast<PCWSTR>(pszStart), pszEnd, wMatch));
}
__inline PSTR StrRChrIA(_In_ PSTR pszStart, _In_opt_ PCSTR pszEnd,
                        WORD wMatch) {
  return const_cast<PSTR>(
      StrRChrIA(const_cast<PCSTR>(pszStart), pszEnd, wMatch));
}
__inline PWSTR StrRChrIW(_In_ PWSTR pszStart, _In_opt_ PCWSTR pszEnd,
                         WCHAR wMatch) {
  return const_cast<PWSTR>(
      StrRChrIW(const_cast<PCWSTR>(pszStart), pszEnd, wMatch));
}
__inline PSTR StrRStrIA(_In_ PSTR pszSource, _In_opt_ PCSTR pszLast,
                        _In_ PCSTR pszSrch) {
  return const_cast<PSTR>(
      StrRStrIA(const_cast<PCSTR>(pszSource), pszLast, pszSrch));
}
__inline PWSTR StrRStrIW(_In_ PWSTR pszSource, _In_opt_ PCWSTR pszLast,
                         _In_ PCWSTR pszSrch) {
  return const_cast<PWSTR>(
      StrRStrIW(const_cast<PCWSTR>(pszSource), pszLast, pszSrch));
}
...
__inline PWSTR StrStrNW(_In_ PWSTR pszFirst, _In_ PCWSTR pszSrch, UINT cchMax) {
  return const_cast<PWSTR>(
      StrStrNW(const_cast<PCWSTR>(pszFirst), pszSrch, cchMax));
}
__inline PWSTR StrStrNIW(_In_ PWSTR pszFirst, _In_ PCWSTR pszSrch,
                         UINT cchMax) {
  return const_cast<PWSTR>(
      StrStrNIW(const_cast<PCWSTR>(pszFirst), pszSrch, cchMax));
}
#endif
