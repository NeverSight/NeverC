...
#define SQL_COPT_SS_REMOTE_PWD                                                 \
  (SQL_COPT_SS_BASE + 1) // dbrpwset SQLSetConnectOption only
#define SQL_COPT_SS_USE_PROC_FOR_PREP                                          \
  (SQL_COPT_SS_BASE + 2) // Use create proc for SQLPrepare
...
#define SQL_COPT_SS_USER_DATA                                                  \
  (SQL_COPT_SS_BASE + 5) // dbgetuserdata/dbsetuserdata
#define SQL_COPT_SS_ENLIST_IN_DTC                                              \
  SQL_ATTR_ENLIST_IN_DTC // Enlist in a DTC transaction
#define SQL_COPT_SS_ENLIST_IN_XA                                               \
  SQL_ATTR_ENLIST_IN_XA // Enlist in a XA transaction
// SQL_ATTR_CONNECTION_DEAD 1209 (It will return current connection status, it
// will not go to server)
#define SQL_COPT_SS_FALLBACK_CONNECT                                           \
  (SQL_COPT_SS_BASE + 10) // Enables FallBack connections
#define SQL_COPT_SS_PERF_DATA                                                  \
  (SQL_COPT_SS_BASE +                                                          \
   11) // Used to access SQL Server ODBC driver performance data
#define SQL_COPT_SS_PERF_DATA_LOG                                              \
  (SQL_COPT_SS_BASE +                                                          \
   12) // Used to set the logfile name for the Performance data
#define SQL_COPT_SS_PERF_QUERY_INTERVAL                                        \
  (SQL_COPT_SS_BASE +                                                          \
   13) // Used to set the query logging threshold in milliseconds.
#define SQL_COPT_SS_PERF_QUERY_LOG                                             \
  (SQL_COPT_SS_BASE + 14) // Used to set the logfile name for saving queryies.
#define SQL_COPT_SS_PERF_QUERY                                                 \
  (SQL_COPT_SS_BASE + 15) // Used to start and stop query logging.
#define SQL_COPT_SS_PERF_DATA_LOG_NOW                                          \
  (SQL_COPT_SS_BASE + 16) // Used to make a statistics log entry to disk.
#define SQL_COPT_SS_QUOTED_IDENT                                               \
  (SQL_COPT_SS_BASE + 17) // Enable/Disable Quoted Identifiers
...
#define SQL_COPT_SS_TRANSLATE                                                  \
  (SQL_COPT_SS_BASE + 20) // Perform code page translation
#define SQL_COPT_SS_ATTACHDBFILENAME                                           \
  (SQL_COPT_SS_BASE + 21) // File name to be attached as a database
#define SQL_COPT_SS_CONCAT_NULL                                                \
  (SQL_COPT_SS_BASE + 22) // Enable/Disable CONCAT_NULL_YIELDS_NULL
#define SQL_COPT_SS_ENCRYPT                                                    \
  (SQL_COPT_SS_BASE + 23) // Allow strong encryption for data

#define SQL_COPT_SS_MAX_USED SQL_COPT_SS_ENCRYPT

//	Statement Options
#define SQL_SOPT_SS_BASE 1225
#define SQL_SOPT_SS_TEXTPTR_LOGGING                                            \
  (SQL_SOPT_SS_BASE + 0) // Text pointer logging
#define SQL_SOPT_SS_CURRENT_COMMAND                                            \
  (SQL_SOPT_SS_BASE + 1) // dbcurcmd SQLGetStmtOption only
#define SQL_SOPT_SS_HIDDEN_COLUMNS                                             \
  (SQL_SOPT_SS_BASE + 2) // Expose FOR BROWSE hidden columns
#define SQL_SOPT_SS_NOBROWSETABLE                                              \
  (SQL_SOPT_SS_BASE + 3) // Set NOBROWSETABLE option
#define SQL_SOPT_SS_REGIONALIZE                                                \
  (SQL_SOPT_SS_BASE + 4) // Regionalize output character conversions
#define SQL_SOPT_SS_CURSOR_OPTIONS                                             \
  (SQL_SOPT_SS_BASE + 5) // Server cursor options
#define SQL_SOPT_SS_NOCOUNT_STATUS                                             \
  (SQL_SOPT_SS_BASE + 6) // Real vs. Not Real row count indicator
#define SQL_SOPT_SS_DEFER_PREPARE                                              \
  (SQL_SOPT_SS_BASE + 7) // Defer prepare until necessary

#define SQL_SOPT_SS_MAX_USED SQL_SOPT_SS_DEFER_PREPARE

#define SQL_COPT_SS_BASE_EX 1240
#define SQL_COPT_SS_BROWSE_CONNECT                                             \
  (SQL_COPT_SS_BASE_EX + 1) // Browse connect mode of operation
#define SQL_COPT_SS_BROWSE_SERVER                                              \
  (SQL_COPT_SS_BASE_EX + 2) // Single Server browse request.
#define SQL_COPT_SS_WARN_ON_CP_ERROR                                           \
  (SQL_COPT_SS_BASE_EX + 3) // Issues warning when data from the server
                            // had a loss during code page conversion.

#define SQL_COPT_SS_CONNECTION_DEAD                                            \
  (SQL_COPT_SS_BASE_EX + 4) // dbdead SQLGetConnectOption only
                            // It will try to ping the server.
                            // Expensive connection check

#define SQL_COPT_SS_BROWSE_CACHE_DATA                                          \
  (SQL_COPT_SS_BASE_EX +                                                       \
   5) // Determines if we should cache browse info
      // Used when returned buffer is greater then ODBC limit (32K)

#define SQL_COPT_SS_RESET_CONNECTION                                           \
  (SQL_COPT_SS_BASE_EX + 6) // When this option is set, we will perform
                            // connection reset on next packet
...
#define SQL_RE_OFF                                                             \
  0L //	No regionalization occurs on output character conversions
...
#define SQL_CO_FFO_AF                                                          \
  (SQL_CO_FFO | SQL_CO_AF)    //	Fast-forward cursor with autofetch
...
#define SQL_CURSOR_FAST_FORWARD_ONLY                                           \
  8 //	Only returned by SQLGetStmtAttr/Option
...
#define SQL_CA_SS_COLUMN_UTYPE                                                 \
  (SQL_CA_SS_BASE + 1)                                //	dbcolutype/dbaltutype
...
#define SQL_CA_SS_COMPUTE_ID                                                   \
  (SQL_CA_SS_BASE + 6)                                //	dbnextrow status return
...
#define SQL_CA_SS_COLUMN_HIDDEN                                                \
  (SQL_CA_SS_BASE + 11) //	Column is hidden (FOR BROWSE)
#define SQL_CA_SS_COLUMN_KEY                                                   \
  (SQL_CA_SS_BASE + 12) //	Column is key column (FOR BROWSE)
// #define SQL_DESC_BASE_COLUMN_NAME_OLD	(SQL_CA_SS_BASE+13) //This is
// defined at another location.
#define SQL_CA_SS_COLUMN_COLLATION                                             \
  (SQL_CA_SS_BASE + 14) //	Column collation (only for chars)
...
#define DBMAXCHAR                                                              \
  (8000 + 1) // Max length of DBVARBINARY and DBVARCHAR, etc. +1 for zero byte
#define MAXNAME                                                                \
  (SQL_MAX_SQLSERVERNAME +                                                     \
   1) // Max server identifier length including zero byte
...
typedef struct dbdatetime4 { // Internal representation of SMALLDATETIME data
                             // type
...
#define BCPTEXTFILE                                                            \
  14 // Sets BCP mode to expect a text file and to detect Unicode or ANSI
     // automatically
...
#define BCPFILECP_RAW                                                          \
  (-1) // Data in file is in Server code page (no conversion)
...
HANDLE SQL_API SQLInitEnumServers(_In_ LPWSTR pwchServerName,
                                  _In_ LPWSTR pwchInstanceName);
