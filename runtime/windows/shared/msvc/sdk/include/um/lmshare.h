...
_Check_return_ NET_API_STATUS NET_API_FUNCTION NetShareAdd(
    _In_opt_ LMSTR servername, _In_ DWORD level,
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetShareEnum(
        _In_opt_ LMSTR servername, _In_ DWORD level,
        _When_(level == 0,
               _Outptr_result_bytebuffer_(*entriesread * sizeof(SHARE_INFO_0)))
            _When_(level == 1, _Outptr_result_bytebuffer_(*entriesread *
                                                          sizeof(SHARE_INFO_1)))
                _When_(level == 2,
                    ...
                    _When_(level == 502,
                        ...
                        _When_(level == 503,
                               _Outptr_result_bytebuffer_(
                                   *entriesread * sizeof(SHARE_INFO_503)))
                            LPBYTE *bufptr,
        _In_ DWORD prefmaxlen, _Out_ LPDWORD entriesread,
        _Out_ LPDWORD totalentries, _Inout_opt_ LPDWORD resume_handle);
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetShareGetInfo(
        _In_opt_ LMSTR servername, _In_ LMSTR netname, _In_ DWORD level,
        _When_(level == 0, _Outptr_result_bytebuffer_(sizeof(SHARE_INFO_0)))
            _When_(level == 1, _Outptr_result_bytebuffer_(sizeof(SHARE_INFO_1)))
                _When_(level == 2,
                       _Outptr_result_bytebuffer_(sizeof(SHARE_INFO_2)))
                    _When_(level == 501,
                           _Outptr_result_bytebuffer_(sizeof(SHARE_INFO_501)))
                        _When_(level == 502, _Outptr_result_bytebuffer_(
                                                 sizeof(SHARE_INFO_502)))
                            _When_(level == 503, _Outptr_result_bytebuffer_(
                                                     sizeof(SHARE_INFO_503)))
                                _When_(level == 1005,
                                       _Outptr_result_bytebuffer_(sizeof(
                                           SHARE_INFO_1005))) LPBYTE *bufptr);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetShareSetInfo(
    _In_opt_ LMSTR servername, _In_ LMSTR netname, _In_ DWORD level,
    _When_(level == 1, _In_reads_bytes_(sizeof(SHARE_INFO_1)))
        _When_(level == 2, _In_reads_bytes_(sizeof(SHARE_INFO_2))) _When_(
            level == 502, _In_reads_bytes_(sizeof(SHARE_INFO_502)))
            _When_(level == 503, _In_reads_bytes_(sizeof(SHARE_INFO_503)))
                _When_(level == 1004, _In_reads_bytes_(sizeof(SHARE_INFO_1004)))
                    _When_(level == 1005,
                           _In_reads_bytes_(sizeof(SHARE_INFO_1005)))
                        _When_(level == 1006,
                               _In_reads_bytes_(sizeof(SHARE_INFO_1006)))
                            _When_(level == 1501,
                                   _In_reads_bytes_(sizeof(SHARE_INFO_1501)))
                                LPBYTE buf,
    _Out_opt_ LPDWORD parm_err);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetShareDel(
    _In_opt_ LMSTR servername, _In_ LMSTR netname, _Reserved_ DWORD reserved);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetShareDelSticky(
    _In_opt_ LMSTR servername, _In_ LMSTR netname, _Reserved_ DWORD reserved);

_Check_return_ NET_API_STATUS NET_API_FUNCTION
NetShareCheck(_In_opt_ LMSTR servername, _In_ LMSTR device, _Out_ LPDWORD type);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetShareDelEx(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _When_(level == 0, _In_reads_bytes_(sizeof(SHARE_INFO_0)))
        _When_(level == 1, _In_reads_bytes_(sizeof(SHARE_INFO_1)))
            _When_(level == 2, _In_reads_bytes_(sizeof(SHARE_INFO_2)))
                _When_(level == 502, _In_reads_bytes_(sizeof(SHARE_INFO_502)))
                    _When_(level == 503,
                           _In_reads_bytes_(sizeof(SHARE_INFO_503)))
                        LPBYTE buf);
...
//
_Check_return_ NET_API_STATUS NET_API_FUNCTION
NetServerAliasAdd(_In_opt_ LMSTR servername, _In_ DWORD level,
                  _In_reads_bytes_(sizeof(SERVER_ALIAS_INFO_0)) LPBYTE buf);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerAliasDel(
    _In_opt_ LMSTR servername, _In_ DWORD level,
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetServerAliasEnum(
        ...
        _When_(level == 0,
               _Outptr_result_bytebuffer_(
                   *entriesread * sizeof(SERVER_ALIAS_INFO_0_CONTAINER)))
            LPBYTE *bufptr,
        _In_ DWORD prefmaxlen, _Out_ LPDWORD entriesread,
        _Out_ LPDWORD totalentries, _Inout_opt_ LPDWORD resumehandle);
...
#define CSC_MASK_EXT                                                           \
  0x2030                // Used to mask off the following states (including
...
#define SHI1005_FLAGS_RESTRICT_EXCLUSIVE_OPENS                                 \
  0x000100 // Used to disallow read-deny read behavior
#define SHI1005_FLAGS_FORCE_SHARED_DELETE                                      \
  0x000200 // Used to allows force shared delete
#define SHI1005_FLAGS_ALLOW_NAMESPACE_CACHING                                  \
  0x000400 // The clients may cache the namespace
#define SHI1005_FLAGS_ACCESS_BASED_DIRECTORY_ENUM                              \
  0x000800 // Trim visible files in enumerations based on access
#define SHI1005_FLAGS_FORCE_LEVELII_OPLOCK 0x001000 // Only issue level2 oplock
#define SHI1005_FLAGS_ENABLE_HASH                                              \
  0x002000 // Enable hash generation and retrieval requests from share
...
#define SHI1005_FLAGS_DISABLE_CLIENT_BUFFERING                                 \
  0x020000 // Used to set the allowed client buffering
#define SHI1005_FLAGS_IDENTITY_REMOTING 0x040000 // Allows auth tunneling
#define SHI1005_FLAGS_CLUSTER_MANAGED                                          \
  0x080000 // Used to prevent share from being modified by users
#define SHI1005_FLAGS_COMPRESS_DATA 0x100000 // Request compression
#define SHI1005_FLAGS_ISOLATED_TRANSPORT                                       \
  0x200000 // Request asymmetric connections for this share
#define SHI1005_FLAGS_DISABLE_DIRECTORY_HANDLE_LEASING                         \
  0x400000 // Used to disable handle caching for directories
...
#define SHI1005_VALID_FLAGS_SET                                                \
  (CSC_MASK | SHI1005_FLAGS_RESTRICT_EXCLUSIVE_OPENS |                         \
   ...
   SHI1005_FLAGS_FORCE_LEVELII_OPLOCK | SHI1005_FLAGS_ENABLE_HASH |            \
   SHI1005_FLAGS_ENABLE_CA | SHI1005_FLAGS_ENCRYPT_DATA |                      \
   SHI1005_FLAGS_DISABLE_CLIENT_BUFFERING | SHI1005_FLAGS_IDENTITY_REMOTING |  \
   SHI1005_FLAGS_CLUSTER_MANAGED | SHI1005_FLAGS_RESERVED |                    \
   SHI1005_FLAGS_COMPRESS_DATA | SHI1005_FLAGS_ISOLATED_TRANSPORT |            \
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetSessionEnum(
        _In_opt_ LMSTR servername, _In_opt_ LMSTR UncClientName,
        _In_opt_ LMSTR username, _In_ DWORD level,
        _When_(level == 0, _Outptr_result_bytebuffer_(*entriesread *
            ...
            _When_(level == 1,
                ...
                _When_(level == 2,
                       _Outptr_result_bytebuffer_(*entriesread *
                                                  sizeof(SESSION_INFO_2)))
                    _When_(level == 10,
                           _Outptr_result_bytebuffer_(*entriesread *
                                                      sizeof(SESSION_INFO_10)))
                        _When_(level == 502,
                               _Outptr_result_bytebuffer_(
                                   *entriesread * sizeof(SESSION_INFO_502)))
                            LPBYTE *bufptr,
        _In_ DWORD prefmaxlen, _Out_ LPDWORD entriesread,
        _Out_ LPDWORD totalentries, _Inout_opt_ LPDWORD resume_handle);

_Check_return_ NET_API_STATUS NET_API_FUNCTION
NetSessionDel(_In_opt_ LMSTR servername, _In_opt_ LMSTR UncClientName,
              _In_opt_ LMSTR username);

_Check_return_ _Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetSessionGetInfo(
        _In_opt_ LMSTR servername, _In_ LMSTR UncClientName,
        _In_ LMSTR username, _In_ DWORD level,
        _When_(level == 0, _Outptr_result_bytebuffer_(sizeof(SESSION_INFO_0)))
            _When_(level == 1,
                   _Outptr_result_bytebuffer_(sizeof(SESSION_INFO_1)))
                _When_(level == 2,
                       _Outptr_result_bytebuffer_(sizeof(SESSION_INFO_2)))
                    _When_(level == 10,
                           _Outptr_result_bytebuffer_(sizeof(SESSION_INFO_10)))
                        LPBYTE *bufptr);
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetConnectionEnum(
        _In_opt_ LMSTR servername, _In_ LMSTR qualifier, _In_ DWORD level,
        _When_(level == 0,
               _Outptr_result_bytebuffer_(*entriesread *
...
_Check_return_ NET_API_STATUS NET_API_FUNCTION
NetFileClose(_In_opt_ LMSTR servername, _In_ DWORD fileid);

_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetFileEnum(
        _In_opt_ LMSTR servername, _In_opt_ LMSTR basepath,
        _In_opt_ LMSTR username, _In_ DWORD level,
        _When_(level == 2,
               _Outptr_result_bytebuffer_(*entriesread * sizeof(FILE_INFO_2)))
            _When_(level == 3, _Outptr_result_bytebuffer_(*entriesread *
                                                          sizeof(FILE_INFO_3)))
                LPBYTE *bufptr,
        _In_ DWORD prefmaxlen, _Out_ LPDWORD entriesread,
        _Out_ LPDWORD totalentries, _Inout_opt_ PDWORD_PTR resume_handle);

_Check_return_ _Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetFileGetInfo(
        _In_opt_ LMSTR servername, _In_ DWORD fileid, _In_ DWORD level,
