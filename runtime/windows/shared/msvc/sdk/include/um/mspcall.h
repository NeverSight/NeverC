...
public:
  ...
  //
  // Private addref and release for the MSP call. See Platform SDK
  // documentation.
  //
  ...
  STDMETHOD(CreateStream)(IN long lMediaType, IN TERMINAL_DIRECTION Direction,
  ...
  virtual HRESULT Init(IN CMSPAddress *pMSPAddress, IN MSP_HANDLE htCall,
                       IN DWORD dwReserved, IN DWORD dwMediaType) = 0;
...
protected:
  virtual HRESULT InternalCreateStream(IN DWORD dwMediaType,
                                       IN TERMINAL_DIRECTION Direction,
                                       IN OUT ITStream **ppStream) = 0;

  virtual HRESULT CreateStreamObject(IN DWORD dwMediaType,
                                     IN TERMINAL_DIRECTION Direction,
...
public:
  ...
  HRESULT Init(IN CMSPAddress *pMSPAddress, IN MSP_HANDLE htCall,
  ...
  virtual HRESULT ProcessGraphEvent(IN ITStream *pITStream, IN long lEventCode,
                                    IN LONG_PTR lParam1, IN LONG_PTR lParam2);

protected:
  // helper function:
  HRESULT RegisterWaitEvent(IN IMediaEvent *pIMediaEvent,
