...
// MsiOpenDatabase persist predefine values, otherwise output database path is
// used
#define MSIDBOPEN_READONLY                                                     \
  (LPCTSTR)0 // database open read-only, no persistent changes
#define MSIDBOPEN_TRANSACT (LPCTSTR)1 // database read/write in transaction mode
#define MSIDBOPEN_DIRECT                                                       \
  (LPCTSTR)2 // database direct read/write without transaction
#define MSIDBOPEN_CREATE                                                       \
  (LPCTSTR)3 // create new database, transact mode read/write
#define MSIDBOPEN_CREATEDIRECT                                                 \
  (LPCTSTR)4 // create new database, direct mode read/write
#define MSIDBOPEN_PATCHFILE                                                    \
  32 / sizeof(*MSIDBOPEN_READONLY) // add flag to indicate patch file
...
typedef enum tagMSICOLINFO {
  ...
  MSICOLINFO_TYPES =
      1, // return column definitions, datatype code followed by width
} MSICOLINFO;
...
typedef enum tagMSIDBERROR {
  ...
  MSIDBERROR_DUPLICATEKEY =
      1, //  new record duplicates primary keys of existing record in table
  ...
  MSIDBERROR_NOTINSET =
      6, //  data not a member of the values permitted in the set
  MSIDBERROR_BADVERSION = 7, //  invalid version string
  MSIDBERROR_BADCASE =
      8, //  invalid case, must be all upper-case or all lower-case
  ...
  MSIDBERROR_BADDEFAULTDIR =
      18, //  bad string in DefaultDir column of Directory table
  MSIDBERROR_BADREGPATH = 19, //  bad registry path string
  MSIDBERROR_BADCUSTOMSOURCE =
      20, //  bad string in CustomSource column of CustomAction table
  ...
  MSIDBERROR_BADCATEGORY =
      23, //  Category column of _Validation table for column is invalid
  MSIDBERROR_BADKEYTABLE = 24, //  table in KeyTable column of _Validation table
                               //  could not be found/loaded
  MSIDBERROR_BADMAXMINVALUES =
      25, //  value in MaxValue column of _Validation table is less than value
          //  in MinValue column
  MSIDBERROR_BADCABINET = 26,  //  bad cabinet name
  MSIDBERROR_BADSHORTCUT = 27, //  bad shortcut target
  MSIDBERROR_STRINGOVERFLOW =
      28, //  string overflow (greater than length allowed in column def)
  MSIDBERROR_BADLOCALIZEATTRIB =
      29 //  invalid localization attribute (primary keys cannot be localized)
...
typedef enum tagMSIRUNMODE {
  ...
  MSIRUNMODE_ADVERTISE =
      1, // installing advertisements, else installing or updating product
  MSIRUNMODE_MAINTENANCE =
      2, // modifying an existing installation, else new installation
  MSIRUNMODE_ROLLBACKENABLED = 3, // rollback is enabled
  MSIRUNMODE_LOGENABLED =
      4, // log file active, enabled prior to install session
  MSIRUNMODE_OPERATIONS =
      5, // spooling execute operations, else in determination phase
  MSIRUNMODE_REBOOTATEND =
      6, // reboot needed after successful installation (settable)
  MSIRUNMODE_REBOOTNOW = 7, // reboot needed to continue installation (settable)
  MSIRUNMODE_CABINET =
      8, // installing files from cabinets and files using Media table
  MSIRUNMODE_SOURCESHORTNAMES =
      9, // source LongFileNames suppressed via PID_MSISOURCE summary property
  MSIRUNMODE_TARGETSHORTNAMES =
      10, // target LongFileNames suppressed via SHORTFILENAMES property
...
// Note: INSTALLMESSAGE_ERROR, INSTALLMESSAGE_WARNING, INSTALLMESSAGE_USER are
// to or'd with a message box style to indicate the buttons to display and
// return:
...
// else ERROR_INVALID_HANDLE, ERROR_INVALID_HANDLE_STATE,
// ERROR_BAD_QUERY_SYNTAX, ERROR_GEN_FAILURE Execution of this function sets the
...
// Similar to a GetLastError function, but for the view. NOT the same as
// MsiGetLastErrorRecord Returns errors of MsiViewModify.

MSIDBERROR WINAPI
MsiViewGetErrorA(MSIHANDLE hView,
                 _Out_writes_opt_(*pcchBuf)
                     LPSTR szColumnNameBuffer, // buffer to hold column name
                 _Inout_opt_ LPDWORD pcchBuf); // size of buffer
MSIDBERROR WINAPI
MsiViewGetErrorW(MSIHANDLE hView,
                 _Out_writes_opt_(*pcchBuf)
...
// ERROR_GEN_FAILURE Execution of this function sets the error record,
// accessible via MsiGetLastErrorRecord
...
// Returns ERROR_SUCCESS, ERROR_INVALID_HANDLE, ERROR_INVALID_HANDLE_STATE,
// ERROR_GEN_FAILURE, ERROR_ACCESS_DENIED Execution of this function sets the
// error record, accessible via MsiGetLastErrorRecord
...
// Returns ERROR_SUCCESS, ERROR_INVALID_HANDLE, ERROR_INVALID_PARAMETER, or
// ERROR_INVALID_HANDLE_STATE

UINT WINAPI MsiViewGetColumnInfo(
    MSIHANDLE hView,
    MSICOLINFO eColumnInfo, // retrieve columns names or definitions
    MSIHANDLE
        *phRecord); // returned data record containing all names or definitions
...
// Return a record containing the names of all primary key columns for a given
// table Returns an MSIHANDLE for a record containing the name of each column.
// The field count of the record corresponds to the number of primary key
// columns. Field [0] of the record contains the table name. Returns
// ERROR_SUCCESS, ERROR_INVALID_HANDLE, ERROR_INVALID_TABLE

UINT WINAPI MsiDatabaseGetPrimaryKeysA(
    MSIHANDLE hDatabase,
...
// Return an enum defining the state of the table (temporary, unknown, or
// persistent). Returns MSICONDITION_ERROR, MSICONDITION_FALSE,
// MSICONDITION_TRUE, MSICONDITION_NONE
...
// Execution of this function sets the error record, accessible via
// MsiGetLastErrorRecord

UINT WINAPI MsiGetSummaryInformationA(
    MSIHANDLE hDatabase,   // 0 if not open
    LPCSTR szDatabasePath, // path to database, 0 if database handle supplied
    UINT
        uiUpdateCount, // maximium number of updated values, 0 to open read-only
    MSIHANDLE *phSummaryInfo); // returned handle to summary information data
UINT WINAPI MsiGetSummaryInformationW(
    MSIHANDLE hDatabase,    // 0 if not open
...
UINT WINAPI MsiSummaryInfoGetPropertyCount(
    MSIHANDLE hSummaryInfo,
    PUINT
        puiPropertyCount); // pointer to location to return total property count

// Set a single summary information property
// Returns ERROR_SUCCESS, ERROR_INVALID_HANDLE, ERROR_UNKNOWN_PROPERTY

UINT WINAPI MsiSummaryInfoSetPropertyA(
    MSIHANDLE hSummaryInfo,
    UINT uiProperty, // property ID, one of allowed values for summary
                     // information
    UINT uiDataType, // VT_I4, VT_LPSTR, VT_FILETIME, or VT_EMPTY
    INT iValue,      // integer value, used only if integer property
    FILETIME
        *pftValue, // pointer to filetime value, used only if datetime property
    LPCSTR szValue); // text value, used only if string property
UINT WINAPI MsiSummaryInfoSetPropertyW(
    MSIHANDLE hSummaryInfo,
    UINT uiProperty, // property ID, one of allowed values for summary
                     // information
    UINT uiDataType, // VT_I4, VT_LPSTR, VT_FILETIME, or VT_EMPTY
...
UINT WINAPI MsiSummaryInfoGetPropertyA(
    ...
    UINT uiProperty, // property ID, one of allowed values for summary
                     // information
    _Out_ PUINT
        puiDataType, // returned type: VT_I4, VT_LPSTR, VT_FILETIME, VT_EMPTY
...
// Open an installer database, specifying the persistance mode, which is a
// pointer. Predefined persist values are reserved pointer values, requiring
// pointer arithmetic. Execution of this function sets the error record,
// accessible via MsiGetLastErrorRecord
...
UINT WINAPI MsiDatabaseGenerateTransformA(
    MSIHANDLE hDatabase,
...
UINT WINAPI MsiDatabaseGenerateTransformW(
    MSIHANDLE hDatabase,
    MSIHANDLE hDatabaseReference, // base database to reference changes
...
UINT WINAPI MsiDatabaseApplyTransformA(
    MSIHANDLE hDatabase,
...
// Create summary information of existing transform to include validation and
// error conditions Execution of this function sets the error record, accessible
...
UINT WINAPI MsiCreateTransformSummaryInfoA(
    ...
    MSIHANDLE hDatabaseReference, // base database to reference changes
    LPCSTR szTransformFile,       // name of generated transform file
    int iErrorConditions,         // errors to suppress when applied, from
                                  // MSITRANSFORM_ERROR
    int iValidation);             // properties validated when applied,
                                  // MSITRANSFORM_VALIDATE
...
UINT WINAPI MsiRecordGetStringA(
    MSIHANDLE hRecord, UINT iField,
    _Out_writes_opt_(*pcchValueBuf)
        LPSTR szValueBuf,              // buffer for returned value
    _Inout_opt_ LPDWORD pcchValueBuf); // in/out buffer character count
UINT WINAPI MsiRecordGetStringW(
    MSIHANDLE hRecord, UINT iField,
    _Out_writes_opt_(*pcchValueBuf)
...
UINT WINAPI
MsiRecordSetStreamA(MSIHANDLE hRecord, UINT iField,
                    LPCSTR szFilePath); // path to file containing stream data
UINT WINAPI
MsiRecordSetStreamW(MSIHANDLE hRecord, UINT iField,
...
UINT WINAPI MsiRecordReadStream(
    ...
    _Out_writes_bytes_opt_(
        *pcbDataBuf) char *szDataBuf, // buffer to receive bytes from stream
...
MSIHANDLE WINAPI MsiGetActiveDatabase(
    MSIHANDLE hInstall); // returns handle to database, 0 if none active
...
// If the property is not defined, it is equivalent to a 0-length value, not
// error Returns ERROR_SUCCESS, ERROR_MORE_DATA, ERROR_INVALID_HANDLE,
// ERROR_BAD_ARGUMENTS
...
// Format record data using a format string containing field markers and/or
// properties Record field 0 must contain the format string Other fields must
// contain data that may be referenced by the format string.

UINT WINAPI MsiFormatRecordA(
    MSIHANDLE hInstall, // non-zero for property expansion
    MSIHANDLE hRecord,  // handle to record, field 0 contains format string
    _Out_writes_opt_(*pcchResultBuf)
        LPSTR szResultBuf,              // buffer to return formatted string
    _Inout_opt_ LPDWORD pcchResultBuf); // in/out buffer character count
UINT WINAPI MsiFormatRecordW(
    MSIHANDLE hInstall, // non-zero for property expansion
    MSIHANDLE hRecord,  // handle to record, field 0 contains format string
    _Out_writes_opt_(*pcchResultBuf)
...
UINT WINAPI MsiGetFeatureStateA(
    MSIHANDLE hInstall,
...
UINT WINAPI MsiGetFeatureStateW(
    MSIHANDLE hInstall,
...
UINT WINAPI MsiSetFeatureAttributesW(
    MSIHANDLE hInstall,
    LPCWSTR szFeature,   // feature name within product
    DWORD dwAttributes); // attributes bits to set for this feature
#ifdef UNICODE
...
UINT WINAPI MsiGetComponentStateA(
    MSIHANDLE hInstall,
    LPCSTR szComponent,        // component name within product
    INSTALLSTATE *piInstalled, // returned current install state
    INSTALLSTATE *piAction);   // action taken during install session
UINT WINAPI MsiGetComponentStateW(
    MSIHANDLE hInstall,
...
UINT WINAPI MsiGetFeatureCostA(
    MSIHANDLE hInstall,
...
UINT WINAPI MsiEnumComponentCostsA(
    MSIHANDLE hInstall,
...
UINT WINAPI MsiEnumComponentCostsW(
    MSIHANDLE hInstall,
...
// Setting the value to 0 initialized components and features to the default
// level Execution of this function sets the error record, accessible via
// MsiGetLastErrorRecord
...
UINT WINAPI MsiGetTargetPathA(
    MSIHANDLE hInstall,
    LPCSTR szFolder, // folder identifier, primary key into Directory table
    _Out_writes_opt_(*pcchPathBuf)
        LPSTR szPathBuf,              // buffer to return full path
    _Inout_opt_ LPDWORD pcchPathBuf); // in/out buffer character count
UINT WINAPI MsiGetTargetPathW(
    MSIHANDLE hInstall,
    LPCWSTR szFolder, // folder identifier, primary key into Directory table
    _Out_writes_opt_(*pcchPathBuf)
        LPWSTR szPathBuf,             // buffer to return full path
...
UINT WINAPI MsiSetTargetPathW(
    MSIHANDLE hInstall,
    LPCWSTR szFolder, // folder identifier, primary key into Directory table
    LPCWSTR
        szFolderPath); // full path for folder, ending in directory separator
#ifdef UNICODE
...
// Returns ERROR_SUCCESS, ERROR_DISK_FULL, ERROR_INVALID_HANDLE_STATE, or
...
UINT WINAPI MsiEnableUIPreview(
    MSIHANDLE hDatabase,
