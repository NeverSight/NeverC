...
//
// The property GUID is the same as for the Imaging device class:
...
//
// This property can be set to any of the STI_DEVICE_TYPE enumeration values
// (see sti.h), including:
//
...
// 3 - StiDeviceTypeStreamingVideo (video streaming device such as webcam, still
// of the Imaging class,
//     formerly serviced by WIA Video but no longer STI/WIA compliant)
//
// For example a modern app can use this property to enumerate WIA scanner
// devices:
//
// System.Devices.InterfaceClassGuid:="{6bdd1fc6-810f-11d0-bec7-08002be2092f}"
// AND System.Devices.WiaDeviceType:=1
//
// ... where {6bdd1fc6-810f-11d0-bec7-08002be2092f} is the GUID of the Imaging
// device class and 1 is StiDeviceTypeScanner. Also note in this example that
// the app accesses the property via its namespace
// (System.Devices.WiaDeviceType), not via its internal DEVPKEY definition.
//

#include <devpropdef.h>

DEFINE_DEVPROPKEY(DEVPKEY_WIA_DeviceType, 0x6bdd1fc6, 0x810f, 0x11d0, 0xbe,
...
// With DEVPKEY_WIA_USDClassId, a scan class monitor can create a devnode and
// register its interface with this property, allowing the WIA service to find
// the associated minidriver without a kernel mode device driver and INF file.
// Previously, a scan class monitor had to use a combination of a kernel mode
// device driver plus an INF file to set the device registry value USDClass in
// order to point the WIA service to the associated minidriver DLL.
//
DEFINE_DEVPROPKEY(DEVPKEY_WIA_USDClassId, 0x6bdd1fc6, 0x810f, 0x11d0, 0xbe,
                  0xc7, 0x08, 0x00, 0x2b, 0xe2, 0x09, 0x2f, 3);
