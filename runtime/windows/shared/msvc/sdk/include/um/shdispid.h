...
#define DISPID_VERBINVOKED                                                     \
  202 // A verb (either from the main or context menu) was invoked in the folder
      // view
#define DISPID_DEFAULTVERBINVOKED                                              \
  203 // default verb (either from the main or context menu) was invoked in the
      // folder view
#define DISPID_BEGINDRAG 204       // user clicked on an item
#define DISPID_VIEWMODECHANGED 205 // The ListViewMode Changed
#define DISPID_NOITEMSTATE_CHANGED                                             \
  206 // We went from 0->some or some->0 items in the view
...
#define DISPID_VIEWPAINTDONE                                                   \
  211 // The enumerated items have been inserted into the view and painted
#define DISPID_COLUMNSCHANGED 212 // The set of visible details columns changed
#define DISPID_CTRLMOUSEWHEEL                                                  \
  213 // The mousewheel has been moved while the CTRL key was down
...
#define DISPID_SELECTEDITEMCHANGED                                             \
  220 // One of the selected items has changed (not the same as a selection
      // change)
#define DISPID_EXPLORERWINDOWREADY                                             \
  221 // Explorer window is open, been painted and is ready
#define DISPID_UPDATEIMAGE 222 // A SHCNE_UPDATEIMAGE notification was received
#define DISPID_INITIALENUMERATIONDONE                                          \
  223 // Used internally by specialized views like the start menu. Not fired
      // when the data source finishes enumeration. To detect when the data
      // source is done enumerating, use DISPID_FILELISTENUMDONE.
#define DISPID_ENTERPRISEIDCHANGED                                             \
  224 // Fired when enterprise id is changed in Common File Dialog during save
      // as
