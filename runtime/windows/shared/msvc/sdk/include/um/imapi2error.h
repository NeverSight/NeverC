...
// MessageText:
//
// The requested write speed was not supported by the drive and the speed was
// adjusted.
//
...
// MessageText:
//
// The requested rotation type was not supported by the drive and the rotation
// type was adjusted.
//
...
// MessageText:
//
// The requested write speed and rotation type were not supported by the drive
// and they were both adjusted.
//
...
// MessageText:
//
// The disc did not pass burn verification and may contain corrupt data or be
// unusable.
//
...
// MessageText:
//
// The device accepted the command, but returned sense data, indicating an
// error.
//
...
// MessageText:
//
// The drive reported that it is in the process of becoming ready.  Please try
...
// MessageText:
//
// The media is currently being formatted.  Please wait for the format to
// complete before attempting to use the media.
//
...
// MessageText:
//
// The drive reported that it is performing a long-running operation, such as
// finishing a write.  The drive may be unusable for a long period of time.
//
...
// MessageText:
//
// The drive reported that the combination of parameters provided in the mode
// page for a MODE SELECT command were not supported.
//
...
// MessageText:
//
// The device failed to accept the command within the timeout period. This may
// be caused by the device having entered an inconsistent state, or the timeout
// value for the command may need to be increased.
//
...
// MessageText:
//
// The DVD structure is not present. This may be caused by incompatible
// drive/medium used.
//
...
// MessageText:
//
// The media's speed is incompatible with the device.  This may be caused by
// using higher or lower speed media than the range of speeds supported by the
// device.
//
...
// MessageText:
//
// The device associated with this recorder during the last operation has been
// exclusively locked, causing this operation to failed.
//
...
// MessageText:
//
// The write failed because the drive did not receive data quickly enough to
// continue writing. Moving the source data to the local computer, reducing the
// write speed, or enabling a "buffer underrun free" setting may resolve this
// issue.
//
...
//
#define E_IMAPI_DF2DATA_STREAM_TOO_LARGE_FOR_CURRENT_MEDIA                     \
  ((HRESULT)0xC0AA0404L)
...
// MessageText:
//
// Overwriting non-blank media is not allowed without the ForceOverwrite
// property set to VARIANT_TRUE.
//
...
// MessageText:
//
// The requested operation is not valid when media has been "prepared" but not
// released.
//
...
// MessageText:
//
// The drive did not report sufficient data for a MODE SENSE (page 0x2A)
// command. The drive may not be supported, or the media may not be correct.
//
...
// MessageText:
//
// The drive did not complete the erase in one hour.  The drive may require a
// power cycle, media removal, or other manual intervention to resume proper
// operation.
//
...
// MessageText:
//
// The drive returned an unexpected error during the erase.  The the media may
// be unusable, the erase may be complete, or the drive may still be in the
// process of erasing the disc.
//
#define E_IMAPI_ERASE_UNEXPECTED_DRIVE_RESPONSE_DURING_ERASE                   \
  ((HRESULT)0x80AA0907L)
...
// MessageText:
//
// The drive returned an error for a START UNIT (spinup) command.  Manual
...
// MessageText:
//
// Adding the track would result in exceeding the limit for the start of the
// leadout.
//
...
//
#define E_IMAPI_RAW_IMAGE_TRACK_INDEX_OFFSET_ZERO_CANNOT_BE_CLEARED            \
  ((HRESULT)0x80AA0A09L)
...
//
#define E_IMAPI_RAW_IMAGE_TRACK_INDEX_TOO_CLOSE_TO_OTHER_INDEX                 \
  ((HRESULT)0x80AA0A0AL)
