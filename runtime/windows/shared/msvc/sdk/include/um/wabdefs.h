...
#define PT_SYSTIME                                                             \
  ((ULONG)64) /* FILETIME 64-bit int w/ number of 100ns periods since Jan      \
                 1,1601 */
...
typedef struct _SPropTagArray {
  ULONG cValues;
  _Field_size_(cValues) ULONG aulPropTag[MAPI_DIM];
...
typedef struct _ADRLIST {
  ULONG cEntries;
  _Field_size_(cEntries) ADRENTRY aEntries[MAPI_DIM];
...
#define DoesCbNewSRowSetOverflow(_crow)                                        \
  (((_crow) >= (ULONG_MAX / (ULONG)sizeof(SRow))) ||                           \
   ((CbNewSRowSet(_crow)) < ((CbNewSRowSet(0)))))
...
typedef SCODE(STDMETHODCALLTYPE ALLOCATEBUFFER)(ULONG cbSize,
                                                LPVOID *lppBuffer);
...
typedef long(STDAPICALLTYPE NOTIFCALLBACK)(LPVOID lpvContext,
                                           ULONG cNotification,
...
#define MAPI_IMAPIPROP_METHODS(IPURE)                                          \
  ...
  (THIS_ HRESULT hResult, ULONG ulFlags, LPMAPIERROR FAR * lppMAPIError)       \
      IPURE;                                                                   \
  ...
  (THIS_ ULONG ulPropTag, LPCIID lpiid, ULONG ulInterfaceOptions,              \
   ULONG ulFlags, LPUNKNOWN FAR * lppUnk) IPURE;                               \
  MAPIMETHOD(SetProps)                                                         \
  (THIS_ ULONG cValues, LPSPropValue lpPropArray,                              \
   LPSPropProblemArray FAR * lppProblems) IPURE;                               \
  MAPIMETHOD(DeleteProps)                                                      \
  (THIS_ LPSPropTagArray lpPropTagArray,                                       \
   LPSPropProblemArray FAR * lppProblems) IPURE;                               \
  MAPIMETHOD(CopyTo)                                                           \
  (THIS_ ULONG ciidExclude, LPCIID rgiidExclude,                               \
   LPSPropTagArray lpExcludeProps, ULONG_PTR ulUIParam,                        \
   LPMAPIPROGRESS lpProgress, LPCIID lpInterface, LPVOID lpDestObj,            \
   ULONG ulFlags, LPSPropProblemArray FAR * lppProblems) IPURE;                \
  MAPIMETHOD(CopyProps)                                                        \
  (THIS_ LPSPropTagArray lpIncludeProps, ULONG_PTR ulUIParam,                  \
   LPMAPIPROGRESS lpProgress, LPCIID lpInterface, LPVOID lpDestObj,            \
   ULONG ulFlags, LPSPropProblemArray FAR * lppProblems) IPURE;                \
  MAPIMETHOD(GetNamesFromIDs)                                                  \
  (THIS_ LPSPropTagArray FAR * lppPropTags, LPGUID lpPropSetGuid,              \
   ULONG ulFlags, ULONG FAR * lpcPropNames,                                    \
...
#define MAPI_IMAPITABLE_METHODS(IPURE)                                         \
  ...
  (THIS_ HRESULT hResult, ULONG ulFlags, LPMAPIERROR FAR * lppMAPIError)       \
      IPURE;                                                                   \
  MAPIMETHOD(Advise)                                                           \
  (THIS_ ULONG ulEventMask, LPMAPIADVISESINK lpAdviseSink,                     \
  ...
  (THIS_ ULONG FAR * lpulRow, ULONG FAR * lpulNumerator,                       \
   ULONG FAR * lpulDenominator) IPURE;                                         \
  MAPIMETHOD(FindRow)                                                          \
  (THIS_ LPSRestriction lpRestriction, BOOKMARK bkOrigin, ULONG ulFlags)       \
      IPURE;                                                                   \
  MAPIMETHOD(Restrict)                                                         \
  ...
  (THIS_ ULONG cbInstanceKey, LPBYTE pbInstanceKey, ULONG ulRowCount,          \
   ULONG ulFlags, LPSRowSet FAR * lppRows, ULONG FAR * lpulMoreRows) IPURE;    \
...
#define MAPI_IMAPISTATUS_METHODS(IPURE)                                        \
  ...
  (THIS_ _In_opt_ ULONG_PTR ulUIParam, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(SettingsDialog)                                                   \
  (THIS_ _In_opt_ ULONG_PTR ulUIParam, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(ChangePassword)                                                   \
  (THIS_ _In_ LPTSTR lpOldPass, _In_ LPTSTR lpNewPass, ULONG ulFlags) IPURE;   \
  MAPIMETHOD(FlushQueues)                                                      \
  (THIS_ _In_opt_ ULONG_PTR ulUIParam, ULONG cbTargetTransport,                \
...
#define MAPI_IMAPICONTAINER_METHODS(IPURE)                                     \
  ...
  (THIS_ ULONG ulFlags, _Outptr_ LPMAPITABLE FAR * lppTable) IPURE;            \
  MAPIMETHOD(GetHierarchyTable)                                                \
  (THIS_ ULONG ulFlags, _Outptr_ LPMAPITABLE FAR * lppTable) IPURE;            \
  MAPIMETHOD(OpenEntry)                                                        \
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   LPCIID lpInterface, ULONG ulFlags, _Out_ ULONG FAR * lpulObjType,           \
   _Outptr_ LPUNKNOWN FAR * lppUnk) IPURE;                                     \
  MAPIMETHOD(SetSearchCriteria)                                                \
  (THIS_ _In_opt_ LPSRestriction lpRestriction,                                \
   _In_opt_ LPENTRYLIST lpContainerList, ULONG ulSearchFlags) IPURE;           \
 ...
 *  If the AB_RECIPIENTS flag is set and neither AB_MODIFIABLE or
 * AB_UNMODIFIABLE bits are set, it is an error.
 */

typedef struct _flaglist {
  ULONG cFlags;
  _Field_size_(cFlags) ULONG ulFlag[MAPI_DIM];
...
#define MAPI_IABCONTAINER_METHODS(IPURE)                                       \
  ...
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   ULONG ulCreateFlags, _Outptr_ LPMAPIPROP FAR * lppMAPIPropEntry) IPURE;     \
  MAPIMETHOD(CopyEntries)                                                      \
  (THIS_ _In_ LPENTRYLIST lpEntries, _In_opt_ ULONG_PTR ulUIParam,             \
   _In_opt_ LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(DeleteEntries)                                                    \
  (THIS_ _In_ LPENTRYLIST lpEntries, ULONG ulFlags) IPURE;                     \
  MAPIMETHOD(ResolveNames)                                                     \
  (THIS_ _In_opt_ LPSPropTagArray lpPropTagArray, ULONG ulFlags,               \
   _In_ LPADRLIST lpAdrList, _Out_ LPFlagList lpFlagList) IPURE;
...
#define MAPI_IMAPIFOLDER_METHODS(IPURE)                                        \
  ...
  (THIS_ LPCIID lpInterface, ULONG ulFlags,                                    \
   _Outptr_ LPMESSAGE FAR * lppMessage) IPURE;                                 \
  MAPIMETHOD(CopyMessages)                                                     \
  (THIS_ _In_ LPENTRYLIST lpMsgList, _In_opt_ LPCIID lpInterface,              \
   _In_ LPVOID lpDestFolder, _In_opt_ ULONG_PTR ulUIParam,                     \
   _In_opt_ LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(DeleteMessages)                                                   \
  (THIS_ _In_ LPENTRYLIST lpMsgList, _In_opt_ ULONG_PTR ulUIParam,             \
   _In_opt_ LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(CreateFolder)                                                     \
  (THIS_ ULONG ulFolderType, _In_ LPTSTR lpszFolderName,                       \
   _In_opt_ LPTSTR lpszFolderComment, _In_opt_ LPCIID lpInterface,             \
   ULONG ulFlags, _Outptr_ LPMAPIFOLDER FAR * lppFolder) IPURE;                \
  MAPIMETHOD(CopyFolder)                                                       \
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   _In_opt_ LPCIID lpInterface, _In_ LPVOID lpDestFolder,                      \
   _In_ LPTSTR lpszNewFolderName, _In_opt_ ULONG_PTR ulUIParam,                \
   _In_opt_ LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(DeleteFolder)                                                     \
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   _In_opt_ ULONG_PTR ulUIParam, _In_opt_ LPMAPIPROGRESS lpProgress,           \
   ULONG ulFlags) IPURE;                                                       \
  ...
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   ULONG ulFlags, _Out_ ULONG FAR * lpulMessageStatus) IPURE;                  \
  MAPIMETHOD(SetMessageStatus)                                                 \
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   ULONG ulNewStatus, ULONG ulNewStatusMask, _Out_ ULONG FAR * lpulOldStatus)  \
      IPURE;                                                                   \
  MAPIMETHOD(SaveContentsSort)                                                 \
  (THIS_ _In_ LPSSortOrderSet lpSortCriteria, ULONG ulFlags) IPURE;            \
  MAPIMETHOD(EmptyFolder)                                                      \
  (THIS_ _In_opt_ ULONG_PTR ulUIParam, _In_opt_ LPMAPIPROGRESS lpProgress,     \
   ULONG ulFlags) IPURE;
...
#define MAPI_IMSGSTORE_METHODS(IPURE)                                          \
  MAPIMETHOD(Advise)                                                           \
  (THIS_ ULONG cbEntryID, _In_reads_bytes_opt_(cbEntryID) LPENTRYID lpEntryID, \
   ULONG ulEventMask, _In_ LPMAPIADVISESINK lpAdviseSink,                      \
   _Out_ ULONG FAR * lpulConnection) IPURE;                                    \
  MAPIMETHOD(Unadvise)                                                         \
  (THIS_ _In_ ULONG ulConnection) IPURE;                                       \
  MAPIMETHOD(CompareEntryIDs)                                                  \
  (THIS_ ULONG cbEntryID1, _In_reads_bytes_(cbEntryID1) LPENTRYID lpEntryID1,  \
   ULONG cbEntryID2, _In_reads_bytes_(cbEntryID2) LPENTRYID lpEntryID2,        \
   ULONG ulFlags, _Out_ ULONG FAR * lpulResult) IPURE;                         \
  MAPIMETHOD(OpenEntry)                                                        \
  (THIS_ ULONG cbEntryID, _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID,     \
   _In_opt_ LPCIID lpInterface, ULONG ulFlags, _Out_ ULONG FAR * lpulObjType,  \
   _Outptr_ LPUNKNOWN FAR * ppUnk) IPURE;                                      \
  MAPIMETHOD(SetReceiveFolder)                                                 \
  (THIS_ _In_opt_ LPTSTR lpszMessageClass, ULONG ulFlags, ULONG cbEntryID,     \
   _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID) IPURE;                     \
  MAPIMETHOD(GetReceiveFolder)                                                 \
  (THIS_ _In_opt_ LPTSTR lpszMessageClass, ULONG ulFlags,                      \
  ...
  (THIS_ _In_ LPMESSAGE lpMessage, ULONG ulLockState) IPURE;                   \
  MAPIMETHOD(FinishedMsg)                                                      \
  (THIS_ ULONG ulFlags, ULONG cbEntryID,                                       \
   _In_reads_bytes_(cbEntryID) LPENTRYID lpEntryID) IPURE;                     \
...
#define MAPI_IMESSAGE_METHODS(IPURE)                                           \
  ...
  (THIS_ ULONG ulAttachmentNum, _In_opt_ LPCIID lpInterface, ULONG ulFlags,    \
   _Outptr_ LPATTACH FAR * lppAttach) IPURE;                                   \
  MAPIMETHOD(CreateAttach)                                                     \
  (THIS_ _In_opt_ LPCIID lpInterface, ULONG ulFlags,                           \
   _Out_ ULONG FAR * lpulAttachmentNum, _Outptr_ LPATTACH FAR * lppAttach)     \
      IPURE;                                                                   \
  MAPIMETHOD(DeleteAttach)                                                     \
  (THIS_ ULONG ulAttachmentNum, _In_opt_ ULONG_PTR ulUIParam,                  \
   _In_opt_ LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                   \
  MAPIMETHOD(GetRecipientTable)                                                \
  (THIS_ ULONG ulFlags, _Outptr_ LPMAPITABLE FAR * lppTable) IPURE;            \
  MAPIMETHOD(ModifyRecipients)                                                 \
  (THIS_ ULONG ulFlags, _In_ LPADRLIST lpMods) IPURE;                          \
...
typedef SCODE(STDMETHODCALLTYPE FAR *LPFNBUTTON)(ULONG_PTR ulUIParam,
                                                 LPVOID lpvContext,
                                                 ULONG cbEntryID,
...
#define MAPI_IMAPICONTROL_METHODS(IPURE)                                       \
  MAPIMETHOD(GetLastError)                                                     \
  (THIS_ HRESULT hResult, ULONG ulFlags,                                       \
   _Outptr_ LPMAPIERROR FAR * lppMAPIError) IPURE;                             \
  MAPIMETHOD(Activate)                                                         \
  (THIS_ ULONG ulFlags, _In_opt_ ULONG_PTR ulUIParam) IPURE;                   \
  MAPIMETHOD(GetState)                                                         \
...
#define MAPI_IPROVIDERADMIN_METHODS(IPURE)                                     \
  ...
  (THIS_ _In_ LPTSTR lpszProvider, ULONG cValues,                              \
   _In_reads_(cValues) LPSPropValue lpProps, _In_opt_ ULONG_PTR ulUIParam,     \
   ULONG ulFlags, _Out_ MAPIUID FAR * lpUID) IPURE;                            \
  MAPIMETHOD(DeleteProvider)                                                   \
  (THIS_ _In_ LPMAPIUID lpUID) IPURE;                                          \
  MAPIMETHOD(OpenProfileSection)                                               \
  (THIS_ _In_opt_ LPMAPIUID lpUID, _In_opt_ LPCIID lpInterface, ULONG ulFlags, \
