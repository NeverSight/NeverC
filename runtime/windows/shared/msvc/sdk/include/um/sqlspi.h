...
// SQL_SPI is just a marker for "Service Provider Interface", otherwise it is
// the same as API Application should not call functions that are marked as
// SQL_SPI directly
...
#define SQL_CONN_POOL_RATING_GOOD_ENOUGH                                       \
  99 // the rating is good enough and we can stop rating
#define SQL_CONN_POOL_RATING_USELESS                                           \
  0 // the candidate connection must not be reused for the current request

// SQLSetConnectAttr
#define SQL_ATTR_DBC_INFO_TOKEN                                                \
  118 // reset the pooled connection in case it is not a perfect match

// Set connection attributes into DBC info token
SQLRETURN SQL_SPI SQLSetConnectAttrForDbcInfoW(
    SQLHDBC_INFO_TOKEN hDbcInfoToken, SQLINTEGER Attribute,
    _In_reads_bytes_opt_(StringLength) SQLPOINTER Value,
    SQLINTEGER StringLength);

// Set connection information for SQLDriverConnect
SQLRETURN SQL_SPI SQLSetDriverConnectInfoW(SQLHDBC_INFO_TOKEN hDbcInfoToken,
...
SQLRETURN SQL_SPI SQLSetConnectInfoW(
    SQLHDBC_INFO_TOKEN hDbcInfoToken, _In_reads_(cchDSN) SQLWCHAR *szDSN,
    SQLSMALLINT cchDSN, _In_reads_(cchUID) SQLWCHAR *szUID, SQLSMALLINT cchUID,
    _In_reads_(cchAuthStr) SQLWCHAR *szAuthStr, SQLSMALLINT cchAuthStr);

// Get the pool ID for the token
SQLRETURN SQL_SPI SQLGetPoolID(SQLHDBC_INFO_TOKEN hDbcInfoToken,
                               _Out_ POOLID *pPoolID);

// Return how close hCandidateConnection matches with hRequest
// *pRating must be between SQL_CONN_POOL_RATING_USELESS and
// SQL_CONN_POOL_RATING_BEST (inclusively) If return value is not SQL_SUCCESS or
// *pRating > SQL_CONN_POOL_RATING_BEST, the candidate connection will not be
// used any more in any other connection request If
// fRequiresTransactionEnlistment is TRUE, transId represents the DTC
// transaction ID that should be enlisted to (transId == 0 means unenlistment).
// Otherwise, transId should be ignored
SQLRETURN SQL_SPI SQLRateConnection(SQLHDBC_INFO_TOKEN hRequest,
...
// If application is calling SQLDriverConnect, szConnStrOut is non-NULL at
// input. Otherwise, it will be set to NULL
SQLRETURN SQL_SPI SQLPoolConnectW(SQLHDBC hdbc,
...
SQLRETURN SQL_SPI SQLCleanupConnectionPoolID(SQLHENV EnvironmentHandle,
                                             POOLID poolID);
...
// Set connection information for SQLDriverConnect
SQLRETURN SQL_SPI SQLSetDriverConnectInfoA(SQLHDBC_INFO_TOKEN hDbcInfoToken,
                                           _In_reads_(cchConnStrIn)
                                               SQLCHAR *szConnStrIn,
                                           SQLSMALLINT cchConnStrIn);
...
SQLRETURN SQL_SPI SQLSetConnectInfoA(
    SQLHDBC_INFO_TOKEN hDbcInfoToken, _In_reads_(cchDSN) SQLCHAR *szDSN,
    SQLSMALLINT cchDSN, _In_reads_(cchUID) SQLCHAR *szUID, SQLSMALLINT cchUID,
    _In_reads_(cchAuthStr) SQLCHAR *szAuthStr, SQLSMALLINT cchAuthStr);

// Create a physical connection
// If application is calling SQLDriverConnect, szConnStrOut is non-NULL at
// input. Otherwise, it will be set to NULL
SQLRETURN SQL_SPI SQLPoolConnectA(SQLHDBC hdbc,
                                  SQLHDBC_INFO_TOKEN hDbcInfoToken,
                                  _Out_writes_opt_(cchConnStrOutMax)
...
typedef SQLRETURN(SQL_API *SQL_ASYNC_NOTIFICATION_CALLBACK)(
    _In_ SQLPOINTER pContext, BOOL fLast);
