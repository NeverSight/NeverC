...
// MessageText:
//
// The shadow copy provider had an error. Check the System and Application event
...
// shadow copied. This is probably due to excessive activity on the volume. Try
// again later when the volume is not being used so heavily.
//
...
// MessageText:
//
// The shadow copy provider timed out while holding writes to the volume being
// shadow copied. This is probably due to excessive activity on the volume by an
// application or a system service. Try again later when activity on the volume
// is reduced.
//
...
// MessageText:
//
// Another shadow copy creation is already in progress. Wait a few moments and
// try again.
//
...
// MessageText:
//
// An error was detected in the Volume Shadow Copy Service (VSS). The problem
// occurred while trying to contact VSS writers. Verify that the Event System
// service and the VSS service are running and check for associated errors in
// the event logs.
//
...
// MessageText:
//
// A writer did not respond to a GatherWriterStatus call.  The writer may either
// have terminated or it may be stuck.  Check the System and Application event
// logs for more information.
//
...
// MessageText:
//
// The writer has already successfully called the Subscribe function.  It cannot
...
// MessageText:
//
// The specified shadow copy storage association is in use and so can't be
// deleted.
//
...
// MessageText:
//
// The remote server is running a version of the Volume Shadow Copy Service that
// does not support remote shadow-copy creation.
//
...
// MessageText:
//
// The writer experienced a non-transient error.  If the backup process is
// retried, the error is likely to reoccur.
//
...
// MessageText:
//
// The writer experienced an error while trying to recover the shadow-copy
// volume.
//
...
// MessageText:
//
// The shadow copy set break operation failed because the disk/partition
// identities could not be reverted. The target identity already exists on the
// machine or cluster and must be masked before this operation can succeed.
//
...
// MessageText:
//
// An expected hidden volume did not arrive in the system. Check the Application
...
// MessageText:
//
// An expected volume did not arrive in the system. Check the Application event
...
// MessageText:
//
// The given Backup Components Document is for a non-transportable shadow copy.
// This operation can only be done on transportable shadow copies.
//
...
// MessageText:
//
// The MBR signature or GPT ID for one or more disks could not be set to the
// intended value. Check the Application event log for more information.
//
...
// MessageText:
//
// The LUN resynchronization operation could not be started because another
// resynchronization operation is already in progress.
//
...
// MessageText:
//
// The clustered disks could not be enumerated or could not be put into cluster
// maintenance mode. Check the System event log for cluster related events and
// the Application event log for VSS related events.
//
...
// MessageText:
//
// The requested operation would overwrite a volume that is not explicitly
// selected. For more information, check the Application event log.
//
...
// MessageText:
//
// The shadow copy ID was not found in the backup components document for the
// shadow copy set.
//
...
// MessageText:
//
// The specified volume is nested too deeply to participate in the VSS
// operation.
//
...
// MessageText:
//
// The writer experienced a partial failure. Check the component level error
...
// MessageText:
//
// There are too few disks on this computer or one or more of the disks is too
// small. Add or change disks so they match the disks in the backup, and try the
// restore again.
//
...
// MessageText:
//
// Windows cannot create a disk on this computer needed to restore from the
// backup. Make sure the disks are properly connected, or add or change disks,
...
// MessageText:
//
// The computer needs to be restarted to finish preparing a hard disk for
// restore. To continue, restart your computer and run the restore again.
//
...
// MessageText:
//
// Automated System Recovery failed the shadow copy, because a selected critical
// volume is located on a cluster shared disk. This is an unsupported
// configuration.
//
...
// MessageText:
//
// A data disk is currently set as active in BIOS. Set some other disk as active
// or use the DiskPart utility to clean the data disk, and then retry the
// restore operation.
//
...
// MessageText:
//
// The disk that is set as active in BIOS is too small to recover the original
// system disk. Replace the disk with a larger one and retry the restore
// operation.
//
...
// MessageText:
//
// Failed to find enough suitable disks for recreating all critical disks. The
// number of available disks should be same or greater than the number of
// critical disks at time of backup, and each one of the disks must be of same
// or greater size.
//
...
// MessageText:
//
// Writer status is not available for one or more writers.  A writer may have
// reached the limit to the number of available backup-restore session states.
//
...
// MessageText:
//
// A critical dynamic disk is a Virtual Hard Disk (VHD). This is an unsupported
// configuration. Check the Application event log for more details.
//
...
// MessageText:
//
// A critical volume selected for backup exists on a disk which cannot be backed
// up by ASR.
//
...
// present in backup. Ensure disks are online, and disk drivers are installed to
// access the disk(s). 'diskpart.exe' tool with list disks command can be used
// to see the list of available fixed disks on the system.
//
...
// MessageText:
//
// Windows did not find any disk which it can use for recreating volumes present
// in backup. Offline disks, cluster shared disks or disks explicitly excluded
// by user will not be used by Windows. Ensure that disks are online and no
// disks are excluded by mistake.
//
