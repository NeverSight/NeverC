...
public:
  ...
  *
  ...
  _Check_return_ HRESULT ConvertToBmp(_In_ BYTE *pSource, INT iSourceSize,
                                      _Outptr_result_buffer_(*piDestSize)
                                          BYTE **ppDest,
                                      _Inout_ INT *piDestSize,
                                      _Out_ BMP_IMAGE_INFO *pBmpImageInfo,
...
#else
  HRESULT SetValidValues(INT index, _In_ BSTR defaultValue,
                         _In_ BSTR currentValue, INT numValues,
                         _In_reads_(numValues) BSTR *pValues);
#endif
...
#if (_WIN32_WINNT == 0x0501)
  void SetValidValues(INT index, _In_ CLSID *defaultValue,
                      _In_ CLSID *currentValue, INT numValues,
                      _In_reads_(numValues) CLSID **pValues);
#else
  HRESULT SetValidValues(INT index, _In_ CLSID *defaultValue,
                         _In_ CLSID *currentValue, INT numValues,
                         _In_reads_(numValues) CLSID **pValues);
#endif
...
#else
  HRESULT SetCurrentValue(INT index, _In_reads_opt_(size) BYTE *value,
                          INT size);
#endif
...
#if (_WIN32_WINNT == 0x0501)
_Check_return_
    HRESULT wiauGetDrvItemContext(_In_ BYTE *pWiasContext,
                                  _Inout_ VOID **ppItemCtx,
                                  _Inout_opt_ IWiaDrvItem **ppDrvItem = NULL);
#else
_Check_return_ HRESULT _stdcall
wiauGetDrvItemContext(_In_ BYTE *pWiasContext, _Inout_ VOID **ppItemCtx,
                      _Inout_opt_ IWiaDrvItem **ppDrvItem = NULL);
#endif
...
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauSetImageItemSize(_In_ BYTE *pWiasContext,
                                            LONG lWidth, LONG lHeight,
                                            LONG lDepth, LONG lSize,
                                            _In_opt_ PWSTR pwszExt = NULL);
#else
_Check_return_ HRESULT _stdcall
wiauSetImageItemSize(_In_ BYTE *pWiasContext, LONG lWidth, LONG lHeight,
                     LONG lDepth, LONG lSize, _In_opt_ PWSTR pwszExt = NULL);
#endif
...
_Check_return_ BOOL wiauPropsInPropSpec(LONG NumPropSpecs,
                                        _In_reads_(NumPropSpecs)
                                            const PROPSPEC *pPropSpecs,
                                        int NumProps,
                                        _In_reads_(NumProps) PROPID *pProps);
#else
_Check_return_ BOOL _stdcall
wiauPropsInPropSpec(LONG NumPropSpecs,
...
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauGetValidFormats(
    _In_ IWiaMiniDrv *pDrv, _In_ BYTE *pWiasContext, LONG TymedValue,
    _Out_ int *pNumFormats, _Out_writes_(*pNumFormats) GUID **ppFormatArray);
#else
_Check_return_ HRESULT _stdcall
wiauGetValidFormats(_In_ IWiaMiniDrv *pDrv, _In_ BYTE *pWiasContext,
                    LONG TymedValue, _Out_ int *pNumFormats,
                    _Out_writes_(*pNumFormats) GUID **ppFormatArray);
#endif
...
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauGetResourceString(HINSTANCE hInst, LONG lResourceID,
                                             _Out_ BSTR *pbstrStr);
#else
_Check_return_ HRESULT _stdcall
wiauGetResourceString(HINSTANCE hInst, LONG lResourceID, _Out_ BSTR *pbstrStr);
#endif
...
\**************************************************************************/
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauRegOpenDataW(_In_ HKEY hkeyAncestor,
                                        _Inout_ HKEY *phkeyDeviceData);
#else
_Check_return_ HRESULT _stdcall wiauRegOpenDataW(_In_ HKEY hkeyAncestor,
                                                 _Inout_ HKEY *phkeyDeviceData);
#endif
...
\**************************************************************************/
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauRegOpenDataA(_In_ HKEY hkeyAncestor,
                                        _Inout_ HKEY *phkeyDeviceData);
#else
_Check_return_ HRESULT _stdcall wiauRegOpenDataA(_In_ HKEY hkeyAncestor,
                                                 _Inout_ HKEY *phkeyDeviceData);
#endif
...
\**************************************************************************/
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauRegGetStrW(_In_ HKEY hkKey,
                                      _In_ PCWSTR pwszValueName,
                                      _Out_writes_bytes_(*pdwLength)
                                          PWSTR pwszValue,
                                      _Inout_ WORD *pdwLength);
#else
_Check_return_ HRESULT _stdcall wiauRegGetStrW(_In_ HKEY hkKey,
                                               _In_ PCWSTR pwszValueName,
                                               _Out_writes_bytes_(*pdwLength)
                                                   PWSTR pwszValue,
...
_Check_return_ HRESULT wiauRegGetStrA(_In_ HKEY hkKey, _In_ PCSTR pszValueName,
                                      ...
                                      _Inout_ DWORD *pdwLength);
#else
_Check_return_ HRESULT _stdcall wiauRegGetStrA(_In_ HKEY hkKey,
                                               _In_ PCSTR pszValueName,
                                               _Out_writes_bytes_(*pdwLength)
                                                   PSTR pszValue,
                                               _Inout_ DWORD *pdwLength);
#endif
...
\**************************************************************************/
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauRegGetDwordW(_In_ HKEY hkKey,
                                        _In_ PCWSTR pwszValueName,
                                        _Out_ DWORD *pdwValue);
#else
_Check_return_ HRESULT _stdcall wiauRegGetDwordW(_In_ HKEY hkKey,
                                                 _In_ PCWSTR pwszValueName,
                                                 _Out_ DWORD *pdwValue);
#endif
...
\**************************************************************************/
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauRegGetDwordA(_In_ HKEY hkKey,
                                        _In_ PCSTR pszValueName,
                                        _Out_ DWORD *pdwValue);
#else
_Check_return_ HRESULT _stdcall wiauRegGetDwordA(_In_ HKEY hkKey,
                                                 _In_ PCSTR pszValueName,
                                                 _Out_ DWORD *pdwValue);
#endif
...
\**************************************************************************/
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauStrW2C(_In_reads_bytes_(iSize)
                                      _Pre_z_ WCHAR *pwszSrc,
                                  _Out_writes_(iSize) CHAR *pszDst, INT iSize);
#else
_Check_return_ HRESULT _stdcall wiauStrW2C(_In_reads_bytes_(iSize)
                                               _Pre_z_ WCHAR *pwszSrc,
                                           _Out_writes_(iSize) CHAR *pszDst,
                                           INT iSize);
#endif
...
_Check_return_ HRESULT wiauStrC2W(_In_reads_bytes_(iSize) _Pre_z_ CHAR *pszSrc,
                                  _Out_writes_bytes_(iSize) WCHAR *pwszDst,
                                  INT iSize);
#else
_Check_return_ HRESULT _stdcall
wiauStrC2W(_In_reads_bytes_(iSize) _Pre_z_ CHAR *pszSrc,
...
#if (_WIN32_WINNT == 0x0501)
_Check_return_ HRESULT wiauStrW2W(_In_reads_bytes_(iSize)
                                      _Pre_z_ WCHAR *pwszSrc,
                                  _Out_writes_bytes_(iSize) WCHAR *pwszDst,
                                  INT iSize);
#else
_Check_return_ HRESULT _stdcall
wiauStrW2W(_In_reads_bytes_(iSize) _Pre_z_ WCHAR *pwszSrc,
           _Out_writes_bytes_(iSize) WCHAR *pwszDst, INT iSize);
#endif
...
*   1. Set registry
HKLM\System\CurrentControlSet\Control\StillImage\Debug\<ModuleName>,
...
#define _STIDEBUG_H_ // WIA debugging is incompatible with stidebug.h, so don't
                     // include it
...
// Don't log at all
//
const DWORD WIAUDBG_DONT_LOG =
    WIAUDBG_DONT_LOG_TO_FILE | WIAUDBG_DONT_LOG_TO_DEBUGGER;
...
void __stdcall wiauDbgHelper(_In_ LPCSTR prefix, _In_ LPCSTR fname,
                             _In_ LPCSTR fmt, va_list marker);
void __stdcall wiauDbgHelper2(_In_ LPCSTR prefix, _In_ LPCSTR fname,
                              _In_ LPCSTR fmt, ...);
...
inline void __stdcall wiauDbgErrorHr(HRESULT hr, _In_ LPCSTR fname,
                                     _In_ LPCSTR fmt, ...) {
  va_list marker;
...
inline void __stdcall wiauDbgLegacyError2(_In_ HINSTANCE hInstance,
                                          _In_ LPCSTR fmt, ...) {
...
inline void __stdcall wiauDbgLegacyTrace2(_In_ HINSTANCE hInstance,
                                          _In_ LPCSTR fmt, ...) {
  UNREFERENCED_PARAMETER(hInstance);
  va_list marker;
...
inline void __stdcall wiauDbgLegacyHresult2(_In_ HINSTANCE hInstance,
                                            HRESULT hr) {
  UNREFERENCED_PARAMETER(hInstance);
  wiauDbgErrorHr(hr, "", "");
}
...
#define REQUIRE_SUCCESS(hr, fname, msg)                                        \
  if (FAILED(hr)) {                                                            \
    if (g_dwDebugFlags & WIAUDBG_PRINT_INFO) {                                 \
      ...
      wiauDbgError(fname, "[%s(%d): Thread 0x%X (%d)]", __FILE__, __LINE__,    \
                   threadId, threadId);                                        \
    }                                                                          \
