...
#define SECPKG_ATTR_ISSUER_LIST                                                \
  0x50 // (OBSOLETE) returns SecPkgContext_IssuerListInfo
#define SECPKG_ATTR_REMOTE_CRED                                                \
  0x51 // (OBSOLETE) returns SecPkgContext_RemoteCredentialInfo
#define SECPKG_ATTR_LOCAL_CRED                                                 \
  0x52 // (OBSOLETE) returns SecPkgContext_LocalCredentialInfo
...
#define SECPKG_ATTR_ISSUER_LIST_EX                                             \
  0x59 // returns SecPkgContext_IssuerListInfoEx
...
#define SECPKG_ATTR_MAPPED_CRED_ATTR                                           \
  0x5c                                // returns SecPkgContext_MappedCredAttr
...
#define SECPKG_ATTR_REMOTE_CERTIFICATES                                        \
  0x5F // returns SecPkgContext_Certificates
#define SECPKG_ATTR_CLIENT_CERT_POLICY                                         \
  0x60 // sets    SecPkgCred_ClientCertCtlPolicy
#define SECPKG_ATTR_CC_POLICY_RESULT                                           \
  0x61 // returns SecPkgContext_ClientCertPolicyResult
#define SECPKG_ATTR_USE_NCRYPT                                                 \
  0x62 // Sets the CRED_FLAG_USE_NCRYPT_PROVIDER FLAG on cred group
...
#define SECPKG_ATTR_SUPPORTED_SIGNATURES                                       \
  0x66 // returns SecPkgContext_SupportedSignatures
...
#define SECPKG_ATTR_KEYING_MATERIAL_INFO                                       \
  0x6a // sets SecPkgContext_KeyingMaterialInfo
...
#define SECPKG_ATTR_CONNECTION_INFO_EX                                         \
  0x6e // returns SecPkgContext_ConnectionInfoEx
#define SECPKG_ATTR_KEYING_MATERIAL_TOKEN_BINDING                              \
  0x6f // returns SecPkgContext_KeyingMaterial specific to Token Binding
#define SECPKG_ATTR_KEYING_MATERIAL_INPROC                                     \
  0x70 // returns SecPkgContext_KeyingMaterial_Inproc
#define SECPKG_ATTR_CERT_CHECK_RESULT                                          \
  0x71 // returns SecPkgContext_CertificateValidationResult, use during and
       // after SSPI handshake loop
#define SECPKG_ATTR_CERT_CHECK_RESULT_INPROC                                   \
  0x72 // returns SecPkgContext_CertificateValidationResult, use only after SSPI
       // handshake loop
#define SECPKG_ATTR_SESSION_TICKET_KEYS                                        \
  0x73 // sets    SecPkgCred_SessionTicketKeys
#define SECPKG_ATTR_SERIALIZED_REMOTE_CERT_CONTEXT_INPROC                      \
  0x74 // returns CERT_BLOB, use only after SSPI handshake loop
#define SECPKG_ATTR_SERIALIZED_REMOTE_CERT_CONTEXT                             \
  0x75 // returns CERT_BLOB, use during and after SSPI handshake loop
...
// An IDL struct _SecPkgCred_ClientCertPolicy_RPC is defined in
// minio/security/base/lsa/idl/sspi/sspirpc.idl for rpc calls. The IDL struct
// should also be updated if there is any change on struct
// _SecPkgCred_ClientCertPolicy.
...
typedef struct _SecPkgCred_SessionTicketKey {
  DWORD TicketInfoVersion; // Set to SESSION_TICKET_INFO_VERSION for the current
                           // session ticket protection method.
  BYTE KeyId[16]; // Uniquely identifies each session ticket key issued by a TLS
                  // server.
  BYTE KeyingMaterial[64]; // Must be generated using a cryptographic RNG.
  BYTE KeyingMaterialSize; // Size in bytes of the keying material in the
                           // KeyingMaterial array. Must be between 32 and 64.
