...
struct ParsedObjectPath {
  ...
  _Field_size_part_(m_dwAllocNamespaces, m_dwNumNamespaces)
  ...
  _Field_size_part_(m_dwAllocKeys,
                    m_dwNumKeys) KeyRef **m_paKeys; // NULL if no keys specified
...
public:
  ...
  BOOL IsInstance();
  BOOL IsObject();
  BOOL IsValid() { return (m_paNamespaces && m_paKeys); };
};

// NOTE:
// The m_vValue in the KeyRef may not be of the expected type, i.e., the parser
// cannot distinguish 16 bit integers from 32 bit integers if they fall within
// the legal subrange of a 16 bit value.  Therefore, the parser only uses the
// following types for keys:
//      VT_I4, VT_R8, VT_BSTR
// If the underlying type is different, the user of this parser must do
...
public:
  ...
  int Parse(_In_opt_ LPCWSTR RawPath, _Outptr_ ParsedObjectPath **pOutput);
  static int WINAPI Unparse(_In_ ParsedObjectPath *pInput,
