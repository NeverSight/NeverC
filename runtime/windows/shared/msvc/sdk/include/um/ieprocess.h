//+-------------------------------------------------------------------------
//
//  IEProcess.h -- This module defines the IE APIs that only exist in the
//  IExplore process.
//
...
// This struct encapsulate the tabdata that will not be changed during the
// lifetime of the tab thread so that this data can be copied and store in the
// TLS of the worker threads associated with the original tab thread
...
struct IE80TabWindowExports {
  ...
  /// this call will return. In addition to do the show/hide/enable logic
  /// correctly, a different parent may need to be substituted.
  /// </summary>
  /// <param name="allowUnknownThread">Threads in a tab process are associated
  /// with
  ///     a tab window where possible.  If this flag is false and the thread is
  ///     not associated with a tab window, the call will fail.  Callers should
  ///     default to false.  Pass true to work even if the thread is
  ///     unnassociated.  </param>
  ...
  ///     of the proposed parent.  Where possible this will be the proposed
  ///     parent.</param>
  /// <returns>S_OK if the caller should continue, a failure code
  /// otherwise.</returns> <remarks> Method can block in it's own modal loop
  /// waiting for the parent window to be available.
  /// </remarks>
  HRESULT(WINAPI *WaitForTabWindow)(
      _In_ bool allowUnknownThread, _In_ HWND proposedParentWindow,
      _Out_ HWND *pActualParentWindow); //  actual parent to use in following
                                        //  calls that require a parent.
  ...
  /// <param name="pActualParentWindow">Window that should be used instead
  ///     of the proposed parent.  Can be the same as the proposed
  ///     parent.</param>
  ...
  /// <returns>S_OK if the caller should continue, a failure code
  /// otherwise.</returns> <remarks> This function uses COM and will
  /// CoInitialize() if necessary. It can also block in it's own modal loop
  /// making x-process COM calls.
  /// </remarks>
  ...
  /// <summary>
  /// Free and cleanup any resources initialized in
...
public:
  ...
  /// </summary>
  /// <returns>Non-NULL if any IE process exports have been
  /// initialized.</returns>
  static HMODULE GetProcessModule() {
    HMODULE moduleHandle = ::GetModuleHandleW(IEPROCESS_MODULE_NAME);
    if (moduleHandle) {
      IEGetProcessModule_t IEGetProcessModule =
          (IEGetProcessModule_t)::GetProcAddress(moduleHandle,
  ...
  /// </summary>
  /// <returns>Non-NULL if the tab window exports have been
  /// initialized.</returns>
  static const IETabWindowExports *
  GetTabWindowExports(HMODULE ieProcessModule) {
    if (ieProcessModule) {
      IEGetTabWindowExports_t IEGetTabWindowExports =
          (IEGetTabWindowExports_t)::GetProcAddress(
              ieProcessModule, IEGetTabWindowExports_PROC_NAME);
