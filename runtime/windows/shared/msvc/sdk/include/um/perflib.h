...
// This is used in generated PERF_COUNTERSET_INFO structure to declare provider
// type. Kernel provider is reserved for Microsoft internal use. Driver provider
// and user-mode provider literals will be published.
//
#define PERF_PROVIDER_USER_MODE 0
#define PERF_PROVIDER_KERNEL_MODE 1
#define PERF_PROVIDER_DRIVER 2

// These are used for PERF_COUNTERSET_INFO::InstanceType value. That is, whether
// the CounterSet allows multiple instances (for example, Process, PhysicalDisk,
// etc) or only single default instance (for example, Memory, TCP, etc).
//
...
#define PERF_COUNTERSET_MULTI_INSTANCES                                        \
...
#define PERF_COUNTERSET_SINGLE_AGGREGATE                                       \
...
#define PERF_COUNTERSET_MULTI_AGGREGATE                                        \
...
"2", and "5". */
#define PERF_COUNTERSET_SINGLE_AGGREGATE_HISTORY                               \
...
#define PERF_COUNTERSET_INSTANCE_AGGREGATE                                     \
...
Note that only certain combinations of the preceding possible values are
allowed:
...
#define PERF_ATTRIB_BY_REFERENCE                                               \
  0x0000000000000001 /* Reference.                                             \
The query on the server MUST dereference the counter to obtain the value. */
#define PERF_ATTRIB_NO_DISPLAYABLE                                             \
  0x0000000000000002 /* No display.                                            \
Instructs the client consumer querying for performance counter data not to     \
display the counter value. */
#define PERF_ATTRIB_NO_GROUP_SEPARATOR                                         \
  0x0000000000000004 /* No group separator.                                    \
...
#define PERF_ATTRIB_DISPLAY_AS_REAL                                            \
  0x0000000000000008 /* Display as real.                                       \
Instructs the client consumer querying performance counter to display the      \
counter value as a real number. */
#define PERF_ATTRIB_DISPLAY_AS_HEX                                             \
  0x0000000000000010 /* Display as hexadecimal.                                \
Instructs the client consumer querying performance counter to display the      \
counter value as a hexadecimal number. */

// Provider counterset is defined as a leading PERF_COUNTERSET_INFO structure
// followed by a sequence of PERF_COUNTER_INFO structures. Note that the
// structure block will be automatically generated by schema generation/parsing
// tool.
//
...
// PERF_COUNTERSET_INSTANCE block is returned from PerfCreateInstance() API call
// to identify specific instance of a counterset. The returned block is formed
// by PERF_COUNTERSET_INSTANCE structure followed by counter data block (layout
// defined by provider counterset template) and instance name string (if
// exists).
//
...
// PERF_COUNTER_IDENTITY structure is used in customized notification callback.
// Whenever PERFLIB V2 invokes customized notification callback, it passes wnode
// datablock (which contains WNODE_HEADER structure followed by other binary
// data) that contains the information providers can use.
//
// For PERF_ADD_COUNTER and PERF_REMOVE_COUNTER request, PERFLIB will pass
// PERF_COUNTER_IDENTITY block so that providers know which counter is
// added/removed. For other requests, currently only machine name is passed (so
// that providers can determine whether the request is for physical node or
// virtual node).
//
...
// Prototype for service request callback. Data providers register with PERFLIB
// V2 by passing a service request callback function that is called for all
// PERFLIB requests.
//
typedef ULONG(
#ifndef MIDL_PASS
    WINAPI
#endif
        *PERFLIBREQUEST)(IN ULONG RequestCode, IN PVOID Buffer,
                         IN ULONG BufferSize);

// Usually PerfSetCounterSetInfo() calls is automatically generated
// PerfAutoStartUp() function (generated by schema generation/parsing tool) to
// inform PERFLIB the layout of specific counterset.
//
ULONG WINAPI PerfStartProvider(_In_ LPGUID ProviderGuid,
...
//
typedef LPVOID(CALLBACK *PERF_MEM_ALLOC)(IN SIZE_T AllocSize,
                                         IN LPVOID pContext);
...
ULONG WINAPI PerfStartProviderEx(
    _In_ LPGUID ProviderGuid, _In_opt_ PPERF_PROVIDER_CONTEXT ProviderContext,
...
ULONG WINAPI PerfStopProvider(_In_ HANDLE ProviderHandle);

ULONG WINAPI PerfSetCounterSetInfo(_In_ HANDLE ProviderHandle,
                                   _Inout_updates_bytes_(TemplateSize)
                                       PPERF_COUNTERSET_INFO Template,
                                   _In_ ULONG TemplateSize);

PPERF_COUNTERSET_INSTANCE WINAPI PerfCreateInstance(_In_ HANDLE ProviderHandle,
                                                    _In_ LPCGUID CounterSetGuid,
                                                    _In_ PCWSTR Name,
                                                    _In_ ULONG Id);

ULONG WINAPI PerfDeleteInstance(_In_ HANDLE Provider,
                                _In_ PPERF_COUNTERSET_INSTANCE InstanceBlock);

PPERF_COUNTERSET_INSTANCE WINAPI PerfQueryInstance(_In_ HANDLE ProviderHandle,
                                                   _In_ LPCGUID CounterSetGuid,
                                                   _In_ PCWSTR Name,
                                                   _In_ ULONG Id);

ULONG WINAPI PerfSetCounterRefValue(_In_ HANDLE Provider,
                                    _Inout_ PPERF_COUNTERSET_INSTANCE Instance,
                                    _In_ ULONG CounterId, _In_ PVOID Address);

ULONG WINAPI PerfSetULongCounterValue(
    _In_ HANDLE Provider, _Inout_ PPERF_COUNTERSET_INSTANCE Instance,
    _In_ ULONG CounterId, _In_ ULONG Value);

ULONG WINAPI PerfSetULongLongCounterValue(
    _In_ HANDLE Provider, _Inout_ PPERF_COUNTERSET_INSTANCE Instance,
    _In_ ULONG CounterId, _In_ ULONGLONG Value);
...
typedef struct _PERF_INSTANCE_HEADER {
  ULONG Size;       // = sizeof(PERF_INSTANCE_HEADER) + sizeof(InstanceName) +
                    // sizeof(Padding)
...
typedef struct _PERF_COUNTERSET_REG_INFO {
  ...
  ULONG
  NumCounters; // Number of PERF_COUNTER_REG_INFO structures in this block.
  ULONG InstanceType; // PERF_COUNTERSET_* type (SINGLE_INSTANCE,
                      // MULTI_INSTANCES, etc.)
                      // Followed by:
...
typedef struct _PERF_COUNTER_REG_INFO {
  ULONG CounterId;   // Counter ID is unique within the counter set. Max of 64K
                     // counters per counter set.
  ULONG Type;        // Counter type from winperf.h, e.g. PERF_COUNTER_COUNTER,
                     // PERF_SAMPLE_COUNTER, etc.
  ULONGLONG Attrib;  // PERF_ATTRIB value.
  ULONG DetailLevel; // PERF_DETAIL_NOVICE or PERF_DETAIL_ADVANCED.
  LONG DefaultScale; // -10 to 10. ScaledValue = RawValue * (10^DefaultScale).
  ULONG
  BaseCounterId;    // The counter ID of the base counter, 0xffffffff for none.
  ...
  ULONG AggregateFunc; // PERF_AGGREGATE value, used if counter set type is
                       // aggregate.
...
typedef struct _STRING_BUFFER_HEADER {
  DWORD dwSize;     // = sizeof(PERF_STRING_BUFFER_HEADER) + sizeof(Counters) +
                    // sizeof(StringData)
  DWORD dwCounters; // Number of PERF_STRING_COUNTER_HEADER structures in this
                    // block.
                    // Followed by:
...
typedef struct _STRING_COUNTER_HEADER {
  ...
  DWORD dwOffset;    // Byte offset from PERF_STRING_BUFFER_HEADER to string.
                     // Offset can be 0xffffffff to indicate NULL.
...
typedef struct _PERF_COUNTER_IDENTIFIER {
  ...
  ULONG Status; // Win32 error code indicating success/failure of the add/delete
                // operation.
  ULONG Size;   // sizeof(PERF_COUNTER_IDENTIFIER) + sizeof(InstanceName) +
                // sizeof(Padding)
  ...
  ULONG Index;      // Set by PerfQueryCounterInfo to the position in which the
                    // corresponding counter data is returned.
  ULONG Reserved;   // Reserved.
                    // Followed by:
                    // WCHAR InstanceName[];
                    // WCHAR Padding[];
...
typedef struct _PERF_DATA_HEADER {
  ULONG dwTotalSize; // = sizeof(PERF_DATA_HEADER) + sizeof(PERF_COUNTER_HEADER
                     // blocks...)
  ...
  LONGLONG
  PerfTime100NSec;   // The number of 100 nanosecond intervals since January
                     // 1, 1601, in Coordinated Universal Time (UTC).
  LONGLONG PerfFreq; // The frequency of a high-resolution clock.
  SYSTEMTIME
  SystemTime; // The time at which data is collected on the provider side.
              // Followed by:
...
typedef enum _PerfCounterDataType {
  PERF_ERROR_RETURN =
      0, /* An error occurred when the performance counter value was queried. */
  PERF_SINGLE_COUNTER =
      1, /* Query returned a single counter from a single-instance. */
  PERF_MULTIPLE_COUNTERS =
      2, /* Query returned multiple counters from a single instance. */
  PERF_MULTIPLE_INSTANCES =
      4, /* Query returned a single counter from each of multiple instances. */
  PERF_COUNTERSET =
      6 /* Query returned multiple counters from each of multiple instances. */
} PerfCounterDataType;
...
typedef struct _PERF_COUNTER_HEADER {
  ULONG dwStatus; // Win32 error code indicating success/failure of the query
                  // operation.
  PerfCounterDataType dwType; // Result type - error, single/single,
                              // multi/single, single/multi, multi/multi.
  ...
  // If dwType == PERF_MULTIPLE_COUNTERS:  PERF_MULTI_COUNTERS block +
  // PERF_COUNTER_DATA blocks. If dwType == PERF_MULTIPLE_INSTANCES:
  // PERF_MULTI_INSTANCES block. If dwType == PERF_COUNTERSET:
  // PERF_MULTI_COUNTERS block + PERF_MULTI_INSTANCES block.
...
typedef struct _PERF_MULTI_INSTANCES {
  ULONG dwTotalSize; // = sizeof(PERF_MULTI_INSTANCES) + sizeof(instance data
                     // blocks...)
...
- Other: On return, the value *pcCounterSetIdsActual is undefined and should
  not be used.
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfEnumerateCounterSet(_In_opt_z_ LPCWSTR szMachine,
                            _Out_opt_cap_post_count_(cCounterSetIds,
                                                     *pcCounterSetIdsActual)
...
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfEnumerateCounterSetInstances(
        _In_opt_z_ LPCWSTR szMachine, _In_ LPCGUID pCounterSetId,
        _Out_opt_bytecap_post_bytecount_(cbInstances, *pcbInstancesActual)
            PPERF_INSTANCE_HEADER pInstances,
        DWORD cbInstances, _Out_ LPDWORD pcbInstancesActual);
...
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfQueryCounterSetRegistrationInfo(
        _In_opt_z_ LPCWSTR szMachine, _In_ LPCGUID pCounterSetId,
        PerfRegInfoType requestCode, DWORD requestLangId,
        _Out_opt_bytecap_post_bytecount_(cbRegInfo, *pcbRegInfoActual)
            LPBYTE pbRegInfo,
        DWORD cbRegInfo, _Out_ LPDWORD pcbRegInfoActual);
...
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfOpenQueryHandle(_In_opt_z_ LPCWSTR szMachine, _Out_ HANDLE *phQuery);
...
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfCloseQueryHandle(_In_ HANDLE hQuery);
...
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfQueryCounterInfo(_In_ HANDLE hQuery,
                         _Out_opt_bytecap_post_bytecount_(cbCounters,
                                                          *pcbCountersActual)
                             PPERF_COUNTER_IDENTIFIER pCounters,
                         DWORD cbCounters, _Out_ LPDWORD pcbCountersActual);
...
details.
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI PerfQueryCounterData(
    _In_ HANDLE hQuery,
    _Out_opt_bytecap_post_bytecount_(cbCounterBlock, *pcbCounterBlockActual)
        PPERF_DATA_HEADER pCounterBlock,
    DWORD cbCounterBlock, _Out_ LPDWORD pcbCounterBlockActual);
...
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfAddCounters(_In_ HANDLE hQuery,
                    _Inout_bytecount_(cbCounters)
                        PPERF_COUNTER_IDENTIFIER pCounters,
                    DWORD cbCounters);
...
*/
_Success_(return == ERROR_SUCCESS) ULONG WINAPI
    PerfDeleteCounters(_In_ HANDLE hQuery,
                       _Inout_bytecount_(cbCounters)
                           PPERF_COUNTER_IDENTIFIER pCounters,
                       DWORD cbCounters);
