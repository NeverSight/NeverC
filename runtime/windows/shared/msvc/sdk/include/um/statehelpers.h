...
GetRegistryValueWithFallbackW(
    __in_opt HKEY hkeyPrimary, __in_opt LPCWSTR pwszPrimarySubKey,
    __in_opt HKEY hkeyFallback, __in_opt LPCWSTR pwszFallbackSubKey,
    __in PCWSTR pwszValue, __in DWORD dwFlags, __out_opt LPDWORD pdwType,
    _When_((dwFlags & 0x7F) == RRF_RT_REG_SZ ||
           ...
           __out_z) _When_((dwFlags & 0x7F) == RRF_RT_REG_MULTI_SZ ||
                               *pdwType == REG_MULTI_SZ,
                           __post __nullnullterminated)
        __out_bcount_part_opt(cbDataIn, *pcbDataOut) PVOID pvData,
    __in DWORD cbDataIn, __out_opt LPDWORD pcbDataOut);
