...
//
//  These are the different types of replication records that may be read from
//  the log.
//
...
//
//  The TXF_ID is exposed as a 128 bit structure.  This should NOT be treated as
//  a GUID, in that this is a unique identifier only within the context of the
//  RM.  These may collide between different RMs.
//
...
//
//  This is the basic record.  If the entire record cannot be copied into the
//  user's buffer, at least this much may be copied if the buffer is big enough,
//  so that the user can determine how large a buffer he needs.
//
...
//
//  This indicates a write.  If this write goes beyond the end of the file, that
//  implies an extension of the file.
//
...
typedef struct _TXF_LOG_RECORD_WRITE {
  ...
  //
  //  Beginning location of the write, as an offset from the beginning of the
  //  file.
  //
...
BOOL WINAPI TxfLogCreateFileReadContext(_In_ LPCWSTR LogPath,
...
/*
  ...
  Description:
    ...
    This context will be required to read any replication records.  In order to
  recover resources, the context must later be closed by calling
  TxfLogDestroyReadContext. Since the resources are allocated by a user-mode
  process, if that routine is not called, the resources will be recovered
  automatically when the process hosting the DLL terminates.
...
BOOL WINAPI TxfLogCreateRangeReadContext(
    _In_ LPCWSTR LogPath, _In_ CLFS_LSN BeginningLsn, _In_ CLFS_LSN EndingLsn,
    _In_ PLARGE_INTEGER BeginningVirtualClock,
    _In_ PLARGE_INTEGER EndingVirtualClock, _In_ ULONG RecordTypeMask,
    _Outptr_ PVOID *TxfLogContext);
/*
...
BOOL WINAPI TxfLogDestroyReadContext(_In_ PVOID TxfLogContext);
/*
...
BOOL WINAPI TxfLogReadRecords(_In_ PVOID TxfLogContext, _In_ ULONG BufferLength,
                              _Out_writes_bytes_to_(BufferLength, *BytesUsed)
                                  PVOID Buffer,
                              _Out_ PULONG BytesUsed, _Out_ PULONG RecordCount);
/*
  Description:

    If all of the available records were copied into the buffer, TRUE will be
  returned. Otherwise, the return value will be FALSE.  If FALSE is returned,
  LastError should be checked.
    ...
    --------------------------------------------------------------------------------------
    NOTE: This routine will return all redo records in the log except for CLRs.
  This means that the replication client must be intelligent enough to discard
  records for transactions that will ultimately abort.  We provide the KTM GUID
  in each replication record to assist in this filtering.  Alternatively, the
  replication client could query the last LSN after each abort and commit are
  complete.  The scan could then be done only for ranges in which all the
  records are for committed transactions.
    --------------------------------------------------------------------------------------
...
BOOL WINAPI TxfReadMetadataInfo(_In_ HANDLE FileHandle, _Out_ PTXF_ID TxfFileId,
...
/*
  Description:
    ...
    This routine will retrieve the TxfFileId, most recent LSN, and locking
  transaction (if applicable) for the specified file.  If the file has not had
  any transacted work done on it, it will have none of these properties and this
  call will return failure.
  ...
  Parameter Definition:
    ...
    LockingTransaction  - If TransactionState != TXFS_TRANSACTION_STATE_NONE,
  the GUID of the transaction locking the file.  Otherwise, this value has no
                          meaning.

*/

BOOL WINAPI TxfLogRecordGetFileName(_In_reads_bytes_(RecordBufferLengthInBytes)
                                        PVOID RecordBuffer,
                                    _In_ ULONG RecordBufferLengthInBytes,
                                    _Out_writes_bytes_(*NameBufferLengthInBytes)
                                        PWSTR NameBuffer,
...
BOOL WINAPI TxfLogRecordGetGenericType(_In_ PVOID RecordBuffer,
                                       _In_ ULONG RecordBufferLengthInBytes,
...
VOID WINAPI TxfSetThreadMiniVersionForCreate(_In_ USHORT MiniVersion);
/*
  Description:

    This routine is used to set the MiniVersion that a subsequent create should
  open. It should be returned to its previous state after calling create.
  Therefore, prior to calling this routine, the caller should invoke
...
VOID WINAPI TxfGetThreadMiniVersionForCreate(_Out_ PUSHORT MiniVersion);
/*
  Description:
