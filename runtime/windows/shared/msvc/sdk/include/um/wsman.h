...
#else
#error                                                                         \
    "Define WSMAN_API_VERSION_1_0 or WSMAN_API_VERSION_1_1 before including wsman.h"
#endif
...
//  The extended error information is the soap fault description from the
//  protocol layer including the machine where the fault was reported as well as
//  plug-in specific error text.
//
//  This error structure is used by callbacks to return detailed error
//  information.
//
...
typedef struct _WSMAN_ERROR {
  ...
  _In_opt_ PCWSTR
      errorDetail; // extended error description from the fault;
                   // it can be NULL, for example in out of memory conditions
  _In_opt_ PCWSTR language; // language for error description (RFC 3066 language
                            // code); it can be NULL
  _In_opt_ PCWSTR machineName; // machine id; it can be NULL
  _In_opt_ PCWSTR pluginName;  // Plug-in name for errors generated by plug-ins.
                               // Otherwise NULL.
} WSMAN_ERROR;
...
//
// -----------------------------------------------------------------------------
// flags used by the authenticationMechanism member of
...
enum WSManAuthenticationFlags {
  ...
  WSMAN_FLAG_NO_AUTHENTICATION =
      0x1, // Use no authentication for a remote operation
  WSMAN_FLAG_AUTH_DIGEST =
      0x2, // Use digest authentication for a remote operation
  WSMAN_FLAG_AUTH_NEGOTIATE = 0x4, // Use negotiate authentication for a remote
                                   // operation (may use kerberos or ntlm)
  WSMAN_FLAG_AUTH_BASIC =
      0x8, // Use basic authentication for a remote operation
  WSMAN_FLAG_AUTH_KERBEROS =
      0x10, // Use kerberos authentication for a remote operation
#if (WINVER >= 0x600)
  WSMAN_FLAG_AUTH_CREDSSP =
      0x80, // Use CredSSP authentication for a remote operation
#endif
  WSMAN_FLAG_AUTH_CLIENT_CERTIFICATE =
      0x20 // Use client certificate authentication for a remote operation
};
...
typedef struct _WSMAN_AUTHENTICATION_CREDENTIALS {
  DWORD authenticationMechanism; // can be 0 (the user did not specify an
                                 ...
                                 // will choose between Kerberos and Negotiate
                                 // only); if it is not 0, it must be one of the
...
typedef struct _WSMAN_FILTER {
  _In_ PCWSTR filter; // filter enumeration/subscription - allows ad-hoc queries
                      // using quey languages like SQL
...
typedef struct _WSMAN_OPERATION_INFO {
  _In_opt_ WSMAN_FRAGMENT
      fragment; // optional element to support Fragment transfer or
...
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManInitialize(DWORD flags, _Out_ WSMAN_API_HANDLE *apiHandle);
...
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManDeinitialize(_Inout_opt_ WSMAN_API_HANDLE apiHandle,
                               DWORD flags);
...
DWORD WINAPI WSManGetErrorMessage(
    ...
    _Out_ DWORD *
        messageLengthUsed // effective message length, including NULL terminator
);
...
//  application has to explicitly load the user profile prior to using this
//  option.
// IMPORTANT: proxy configuration is supported for HTTPS only; for HTTP, the
// direct
...
enum WSManProxyAccessType {
  WSMAN_OPTION_PROXY_IE_PROXY_CONFIG =
      1, // use the Internet Explorer proxy configuration for the current user
  WSMAN_OPTION_PROXY_WINHTTP_PROXY_CONFIG =
      2, // default: proxy settings configured for WinHTTP, using the
         // ProxyCfg.exe utility
  WSMAN_OPTION_PROXY_AUTO_DETECT = 4, // Force autodetection of proxy
  WSMAN_OPTION_PROXY_NO_PROXY_SERVER =
      8, // do not use a proxy server - resolves all host names locally
};
...
typedef struct _WSMAN_PROXY_INFO {
  DWORD accessType;
  _In_opt_ WSMAN_AUTHENTICATION_CREDENTIALS
      authenticationCredentials; // credentials and authentication scheme used
                                 // for proxy
} WSMAN_PROXY_INFO;
...
DWORD WINAPI WSManCreateSession(
    _In_ WSMAN_API_HANDLE apiHandle,
    _In_opt_ PCWSTR
        connection, // if NULL, then connection will default to localhost
    DWORD flags,
    _In_opt_ WSMAN_AUTHENTICATION_CREDENTIALS *serverAuthenticationCredentials,
    _In_opt_ WSMAN_PROXY_INFO *proxyInfo, _Out_ WSMAN_SESSION_HANDLE *session);
...
// completed or cancelled before calling this API Returns a non zero error code
// upon failure
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManCloseSession(_Inout_opt_ WSMAN_SESSION_HANDLE session,
                               DWORD flags);
...
enum WSManSessionOption {
  ...
  WSMAN_OPTION_DEFAULT_OPERATION_TIMEOUTMS =
      1, // DWORD - default timeout in ms that applies to all operations on the
         // client side

  WSMAN_OPTION_MAX_RETRY_TIME =
      11, // DWORD (read only) - maximum time for Robust connection retries
  WSMAN_OPTION_TIMEOUTMS_CREATE_SHELL =
      12, // DWORD - timeout in ms for WSManCreateShell operations
  WSMAN_OPTION_TIMEOUTMS_RUN_SHELL_COMMAND =
      13, // DWORD - timeout in ms for WSManRunShellCommand operations
  WSMAN_OPTION_TIMEOUTMS_RECEIVE_SHELL_OUTPUT =
      14, // DWORD - timeout in ms for WSManReceiveShellOutput operations
  WSMAN_OPTION_TIMEOUTMS_SEND_SHELL_INPUT =
      15, // DWORD - timeout in ms for WSManSendShellInput operations
  WSMAN_OPTION_TIMEOUTMS_SIGNAL_SHELL =
      16, // DWORD - timeout in ms for WSManSignalShell and WSManCloseCommand
          // operations
  WSMAN_OPTION_TIMEOUTMS_CLOSE_SHELL =
      17, // DWORD - timeout in ms for WSManCloseShell operations

  //
  // connection options
  //

  WSMAN_OPTION_SKIP_CA_CHECK = 18, // DWORD  - 1 to not validate the CA on the
                                   // server certificate; 0 - default
  WSMAN_OPTION_SKIP_CN_CHECK = 19, // DWORD  - 1 to not validate the CN on the
                                   // server certificate; 0 - default
  WSMAN_OPTION_UNENCRYPTED_MESSAGES =
      20, // DWORD  - 1 to not encrypt the messages; 0 - default
  WSMAN_OPTION_UTF16 = 21, // DWORD  - 1 Send all network packets for remote
                           // operatons in UTF16; 0 - default is UTF8
  WSMAN_OPTION_ENABLE_SPN_SERVER_PORT =
      22, // DWORD  - 1 When using negotiate, include port number in the
          // connection SPN; 0 - default Used when not talking to the main OS on
          // a machine but, for instance, a BMC
  WSMAN_OPTION_MACHINE_ID =
      23, // DWORD  - 1 Identify this machine to the server by including the
          // MachineID header; 0 - default
  ...
  WSMAN_OPTION_MAX_ENVELOPE_SIZE_KB =
      28, // DWORD - max SOAP envelope size (kb) - default 150kb from winrm
          // config (see 'winrm help config' for more details); the client SOAP
          // packet size cannot surpass
          //  this value; this value will be also sent to the server in the SOAP
          //  request as a MaxEnvelopeSize header; the server will use
          //  min(MaxEnvelopeSizeKb from server configuration, MaxEnvelopeSize
          //  value from SOAP).
  WSMAN_OPTION_SHELL_MAX_DATA_SIZE_PER_MESSAGE_KB =
      29, // DWORD (read only) - max data size (kb) provided by the client,
          // guaranteed by
          //  the winrm client implementation to fit into one SOAP packet; this
          //  is an
          // approximate value calculated based on the
          // WSMAN_OPTION_MAX_ENVELOPE_SIZE_KB (default 500kb), the maximum
          // possible size of the SOAP headers and the overhead of the base64
          // encoding which is specific to WSManSendShellInput API; this option
          // can be used with WSManGetSessionOptionAsDword API; it cannot be
          // used with WSManSetSessionOption API.
  WSMAN_OPTION_REDIRECT_LOCATION = 30, // string - read-only, cannot set
  WSMAN_OPTION_SKIP_REVOCATION_CHECK =
      31, // DWORD  - 1 to not validate the revocation status on the server
          // certificate; 0 - default
  WSMAN_OPTION_ALLOW_NEGOTIATE_IMPLICIT_CREDENTIALS =
      32, // DWORD  - 1 to allow default credentials for Negotiate (this is for
          // SSL only); 0 - default
  WSMAN_OPTION_USE_SSL =
      33, // DWORD - When using just a machine name in the connection string use
          // an SSL connection. 0 means HTTP, 1 means HTTPS.  Default is 0.
  WSMAN_OPTION_USE_INTEARACTIVE_TOKEN =
      34 // DWORD - When creating connection on local machine, use interactive
         // token feature. 1 - default
};
...
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManSetSessionOption(_In_ WSMAN_SESSION_HANDLE session,
...
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManGetSessionOptionAsDword(_In_ WSMAN_SESSION_HANDLE session,
                                          WSManSessionOption option,
                                          _Inout_ DWORD *value);
...
// -----------------------------------------------------------------------------
//
_Success_(return == NO_ERROR) DWORD WINAPI WSManGetSessionOptionAsString(
    _In_ WSMAN_SESSION_HANDLE session, WSManSessionOption option,
    DWORD stringLength,
...
enum WSManCallbackFlags {
  ...
  //  shell knows that no more output will occur for this stream; in some
  //  conditions this cannot be determined.
  //
  WSMAN_FLAG_CALLBACK_END_OF_STREAM = 0x8,

  // Flag that if present on CreateShell callback indicates that it supports
  // disconnect
  ...
  // Flag indicates that client shell is retrying to establish network
  ...
  // Flag indicates that the client shell got into broken state in the middle of
  // retry notification sequence due to some internal error at wsman layer
  WSMAN_FLAG_CALLBACK_RETRY_ABORTED_DUE_TO_INTERNAL_ERROR = 0x1000,

  // Flag that indicates for a receive operation that a delay stream request has
  // been processed
...
//
// -----------------------------------------------------------------------------
// Closes an asynchronous operation; if the callback associated with the
// operation
//  is pending and have not completed when WSManCloseOperation is called, then
//  the function marks the operation for deletion and returns; If the callback
//  was not called, the operation is cancelled and the operation callback is
//  called with WSMAN_ERROR_OPERATION_ABORTED error; the operation handle is
//  freed in all cases after the callback returns.
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManCloseOperation(
    _Inout_opt_ WSMAN_OPERATION_HANDLE operationHandle, DWORD flags);
...
//    WSMAN_FLAG_CALLBACK_END_OF_OPERATION flag to mark the end of the
//    operation. See WSManCloseCommand/WSManCloseShell API description for more
//    info.
// -----------------------------------------------------------------------
//
...
enum WSManShellFlag {
  ...
  // is turned on, but if communicating with a down-level box it may be
  // necessary to do this.  Other reasons for turning it off is due to the extra
  // memory consumption and CPU utilization that is used as a result of
  // compression.
  ...
  // Enable receive call to not immediately retrieve results. Only applicable
  // for Receive calls on commands
...
//
// -----------------------------------------------------------------------
// to be used with WSManRunShellCommand API to not use cmd.exe /c prefix when
...
//
// -----------------------------------------------------------------------
// Data structure returned after a Receive operation; each
// WSMAN_RECEIVE_DATA_RESULT
//  has a stream element with data that can be text (ANSI/UNICODE), binary
//  content, objects, or partial or full XML. In addition, the command state is
//  reported to the client and exitCode if the command is done.
//
...
typedef struct _WSMAN_RECEIVE_DATA_RESULT {
  ...
  WSMAN_DATA
  streamData; // always data as binary; can be stream text (ANSI/UNICODE),
              // binary content or objects or partial or full XML

  // command state string values (shell specific) - the state of the command for
  // which status is being reported
  PCWSTR commandState; // if WSMAN_COMMAND_STATE_DONE then the command should be
...
//  more streams will be received from the command, so the command can be closed
...
typedef void(CALLBACK *WSMAN_SHELL_COMPLETION_FUNCTION)(
    ...
    _In_ WSMAN_ERROR *error, // error allocated and owned by the winrm stack;
                             // valid in the callback only;
    _In_ WSMAN_SHELL_HANDLE
        shell, // shell handle associated with the user context; must be closed
               // using WSManCloseShell
    _In_opt_ WSMAN_COMMAND_HANDLE
        command, // command handle associated with the user context; must be
                 // closed using WSManCloseCommand
    _In_opt_ WSMAN_OPERATION_HANDLE
        operationHandle, // valid only for Send/Receive/Signal operations; must
                         // be closed using WSManCloseOperation
    _In_opt_ WSMAN_RESPONSE_DATA
        *data // output data from command/shell; allocated internally and owned
              // by the winrm stack valid only within this function. Currently
              // used to send back receive stream data and open Xml content in
              // rsp:ConnectResponse
);
#else
...
typedef void(CALLBACK *WSMAN_SHELL_COMPLETION_FUNCTION)(
    ...
    _In_opt_ WSMAN_OPERATION_HANDLE
        operationHandle, // valid only for Send/Receive/Signal operations; must
                         // be closed using WSManCloseOperation
    _In_opt_ WSMAN_RECEIVE_DATA_RESULT
        *data // output data from command/shell; allocated internally and owned
              // by the winrm stack valid only within this function
);
...
void WINAPI WSManCreateShell(
    ...
    _Inout_ WSMAN_SESSION_HANDLE session, DWORD flags,
...
void WINAPI WSManRunShellCommand(
    ...
    _Inout_ WSMAN_SHELL_HANDLE shell, DWORD flags, _In_ PCWSTR commandLine,
    _In_opt_ WSMAN_COMMAND_ARG_SET *args, _In_opt_ WSMAN_OPTION_SET *options,
    _In_ WSMAN_SHELL_ASYNC *async,
    _Out_ WSMAN_COMMAND_HANDLE
        *command // should be closed using WSManCloseCommand
);
...
void WINAPI WSManSignalShell(
    ...
    _In_opt_ WSMAN_COMMAND_HANDLE
        command, // if NULL, the Signal will be sent to the shell
    DWORD flags,
    _In_ PCWSTR code, // signal code
    _In_ WSMAN_SHELL_ASYNC *async,
    _Out_ WSMAN_OPERATION_HANDLE
        *signalOperation // should be closed using WSManCloseOperation
);
...
void WINAPI WSManReceiveShellOutput(
    ...
    _Inout_ WSMAN_SHELL_HANDLE shell, _In_opt_ WSMAN_COMMAND_HANDLE command,
    DWORD flags,
    _In_opt_ WSMAN_STREAM_ID_SET
        *desiredStreamSet, // request output from a particular stream or list of
                           // streams
    _In_ WSMAN_SHELL_ASYNC *async,
    _Out_ WSMAN_OPERATION_HANDLE
        *receiveOperation // should be closed using WSManCloseOperation
);
...
void WINAPI WSManSendShellInput(
    ...
    _In_ WSMAN_SHELL_HANDLE shell, _In_opt_ WSMAN_COMMAND_HANDLE command,
    DWORD flags,
    _In_ PCWSTR streamId, // input stream name
    _In_ WSMAN_DATA
        *streamData, // data as binary - that can contain text (ANSI/UNICODE),
                     // binary content or objects or partial or full XML
    BOOL endOfStream, _In_ WSMAN_SHELL_ASYNC *async,
    _Out_ WSMAN_OPERATION_HANDLE
        *sendOperation // should be closed using WSManCloseOperation
);
...
//
// -----------------------------------------------------------------------------
// Closes a command (signals the termination of a command); the
// WSManCloseCommand callback
//  is called with WSMAN_FLAG_CALLBACK_END_OF_OPERATION flag as result of this
//  operation
// -----------------------------------------------------------------------------
//
void WINAPI WSManCloseCommand(_Inout_opt_ WSMAN_COMMAND_HANDLE commandHandle,
                              DWORD flags, _In_ WSMAN_SHELL_ASYNC *async);
...
// -----------------------------------------------------------------------------
//
void WINAPI WSManCloseShell(_Inout_opt_ WSMAN_SHELL_HANDLE shellHandle,
                            DWORD flags, _In_ WSMAN_SHELL_ASYNC *async);
...
void WINAPI WSManCreateShellEx(
    _Inout_ WSMAN_SESSION_HANDLE session, DWORD flags,
    _In_ PCWSTR resourceUri, // shell resource URI
    _In_ PCWSTR shellId, _In_opt_ WSMAN_SHELL_STARTUP_INFO *startupInfo,
    _In_opt_ WSMAN_OPTION_SET *options,
    _In_opt_ WSMAN_DATA *createXml, // open content for create shell
    _In_ WSMAN_SHELL_ASYNC *async,
    _Out_ WSMAN_SHELL_HANDLE *shell // should be closed using WSManCloseShell
...
void WINAPI WSManRunShellCommandEx(
    ...
    _Inout_ WSMAN_SHELL_HANDLE shell, DWORD flags, _In_ PCWSTR commandId,
    _In_ PCWSTR commandLine, _In_opt_ WSMAN_COMMAND_ARG_SET *args,
    _In_opt_ WSMAN_OPTION_SET *options, _In_ WSMAN_SHELL_ASYNC *async,
    _Out_ WSMAN_COMMAND_HANDLE
        *command // should be closed using WSManCloseCommand
);
...
//
void WINAPI
WSManDisconnectShell(_Inout_ WSMAN_SHELL_HANDLE shell, DWORD flags,
...
// -----------------------------------------------------------------------------
//
void WINAPI WSManReconnectShell(_Inout_ WSMAN_SHELL_HANDLE shell, DWORD flags,
                                _In_ WSMAN_SHELL_ASYNC *async);
...
//
void WINAPI
WSManReconnectShellCommand(_Inout_ WSMAN_COMMAND_HANDLE commandHandle,
                           DWORD flags, _In_ WSMAN_SHELL_ASYNC *async);
...
void WINAPI WSManConnectShell(
    _Inout_ WSMAN_SESSION_HANDLE session, DWORD flags, _In_ PCWSTR resourceUri,
...
void WINAPI WSManConnectShellCommand(
    _Inout_ WSMAN_SHELL_HANDLE shell, DWORD flags,
...
typedef struct _WSMAN_CERTIFICATE_DETAILS {
  PCWSTR subject; // Certificate subject in distinguished form
                  // Ex: "CN=xyz.com, OU=xyz management, O=Microsoft, L=Redmond,
                  // S=Washington, C=US"
  PCWSTR issuerName; // Certificate issuer in distinguished form
                     // Ex: "CN=Microsoft Secure Server Authority, DC=redmond,
                     // DC=corp, DC=microsoft, DC=com"
  PCWSTR issuerThumbprint; // Thumbprint of Certificate issuer
  PCWSTR subjectName; // Certificate Subject Alternative Name (SAN) if available
                      // or subject Common Name (CN) Ex: "xyz.com"
...
typedef struct _WSMAN_SENDER_DETAILS {
  ...
  WSMAN_CERTIFICATE_DETAILS
  *certificateDetails; // valid only if the authentication is client
                       // certificates
  HANDLE clientToken;
  PCWSTR httpURL;
...
// structure will stay valid until the plug-in calls
// WSManPluginOperationComplete on the operation.  A plug-in has two ways of
// handling cancellation of an operation.  The plug-in can check the
// shutdownNotification boolean value if it iterates through a set of results,
// or if the plug-in is more asynchronous in nature then the
// shutdownNotificationHandle can be used when queuing up asynchronous
...
// need to return a context to that shell or command before data can be
// channelled into the plug-in, or before data can be returned to the called.
// WS-Man calls the WSMAN_PLUGIN_RELEASE_SHELL_CONTEXT entry point during shell
// shutdown when it is safe to delete the plug-in shell context, and WS-Man
// calls the WSMAN_PLUGIN_RELEASE_COMMAND_CONTEXT entry point when it is safe to
// delete the plug-in command context.  Any context reported through
...
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManPluginReportContext(_In_ WSMAN_PLUGIN_REQUEST *requestDetails,
                                      _In_ DWORD flags, _In_ PVOID context);
...
typedef VOID(WINAPI *WSMAN_PLUGIN_RELEASE_COMMAND_CONTEXT)(
    _In_ PVOID shellContext, _In_ PVOID commandContext);
...
//          Data reported is at a boundary. Plugins usually serialize and
//          fragment output data objects and push them along the receive byte
//          stream. If the current data chunk being reported is an end fragment
//          of the data object current processed, plugins would set this flag.
// -----------------------------------------------------------------------------
//
...
DWORD WINAPI WSManPluginReceiveResult(_In_ WSMAN_PLUGIN_REQUEST *requestDetails,
                                      _In_ DWORD flags, _In_opt_ PCWSTR stream,
...
// - to supply open Xml content in certain wsman responses like
// rsp:ConnectResponse
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManPluginOperationComplete(
    _In_ WSMAN_PLUGIN_REQUEST *requestDetails, _In_ DWORD flags,
    _In_ DWORD errorCode, _In_opt_ PCWSTR extendedInformation);
...
#define WSMAN_PLUGIN_PARAMS_GET_REQUESTED_LOCALE                               \
  5 /* Returns WSMAN_DATA_TEXT                                                 \
     */
#define WSMAN_PLUGIN_PARAMS_GET_REQUESTED_DATA_LOCALE                          \
  6 /* Returns WSMAN_DATA_TEXT */

DWORD WINAPI
WSManPluginGetOperationParameters(_In_ WSMAN_PLUGIN_REQUEST *requestDetails,
                                  _In_ DWORD flags, _Out_ WSMAN_DATA *data);

//
// -----------------------------------------------------------------------------
// This API is used to get plugin configuration information from within the
// plugin. The plugin can ask for specific data by specifying a different flag
// for each as follows: WSMAN_PLUGIN_PARAMS_SHAREDHOST 1    Specifies if the
// plugin is configured to run as a shared host or separate host.
// WSMAN_PLUGIN_PARAMS_RUNAS_USER                           2    Specifies the
// RunAs User configured for the plugin. If no RunAs user is configured, this
// will return an empty string. WSMAN_PLUGIN_PARAMS_AUTORESTART 3    Specifies
// if the plugin is configured to be autorestarted.
...
DWORD WINAPI WSManPluginGetConfiguration(_In_ PVOID pluginContext,
                                         _In_ DWORD flags,
...
// flags are reserved
// pluginContext MUST be the same context that plugin provided to the
// WSManPluginStartup method
// -----------------------------------------------------------------------------
//
DWORD WINAPI WSManPluginReportCompletion(_In_ PVOID pluginContext,
                                         _In_ DWORD flags);
...
//
DWORD WINAPI
WSManPluginFreeRequestDetails(_In_ WSMAN_PLUGIN_REQUEST *requestDetails);
...
typedef DWORD(WINAPI *WSMAN_PLUGIN_STARTUP)(
    _In_ DWORD flags, _In_ PCWSTR applicationIdentification,
    _In_opt_ PCWSTR extraInfo, _Out_ PVOID *pluginContext);
...
typedef DWORD(WINAPI *WSMAN_PLUGIN_SHUTDOWN)(_In_opt_ PVOID pluginContext,
                                             _In_ DWORD flags,
                                             _In_ DWORD reason);
...
typedef VOID(WINAPI *WSMAN_PLUGIN_SHELL)(
    _In_ PVOID
        pluginContext, // Relates to context returned from WSMAN_PLUGIN_STARTUP
    _In_ WSMAN_PLUGIN_REQUEST *requestDetails, _In_ DWORD flags,
    _In_opt_ WSMAN_SHELL_STARTUP_INFO *startupInfo,
...
// report the command context.  Once the command is completed or when it is
// closed via the operationClosed boolean value or operationClosedHandle in the
// requestDetails the plug-in needs to call WSManPluginOperationComplete.
...
typedef VOID(WINAPI *WSMAN_PLUGIN_COMMAND)(
    _In_ WSMAN_PLUGIN_REQUEST *requestDetails, _In_ DWORD flags,
    _In_ PVOID shellContext, _In_ PCWSTR commandLine,
...
typedef VOID(WINAPI *WSMAN_PLUGIN_SEND)(
    _In_ WSMAN_PLUGIN_REQUEST *requestDetails, _In_ DWORD flags,
    _In_ PVOID shellContext, _In_opt_ PVOID commandContext, _In_ PCWSTR stream,
...
// all data has been send, when the stream is terminated via some internal
// means, or if the receive call is cancelled through the operationClosed
// boolean value or operationClosedHandle, the plug-in needs to call
...
typedef VOID(WINAPI *WSMAN_PLUGIN_RECEIVE)(
    _In_ WSMAN_PLUGIN_REQUEST *requestDetails, _In_ DWORD flags,
    _In_ PVOID shellContext, _In_opt_ PVOID commandContext,
...
// of this callback may be many completion calls for the Signal, Receive,
// Command and Shell operations.
// -----------------------------------------------------------------------------
//
typedef VOID(WINAPI *WSMAN_PLUGIN_SIGNAL)(
    _In_ WSMAN_PLUGIN_REQUEST *requestDetails, _In_ DWORD flags,
    _In_ PVOID shellContext, _In_opt_ PVOID commandContext, _In_ PCWSTR code);
...
typedef VOID(WINAPI *WSMAN_PLUGIN_AUTHORIZE_USER)(
    _In_ PVOID pluginContext, _In_ WSMAN_SENDER_DETAILS *senderDetails,
    _In_ DWORD flags);

//
// -----------------------------------------------------------------------------
// WSMAN_PLUGIN_AUTHORIZE_USER
// Plug-in needs to have a DLL export of WSManPluginAuthzOperation to handle
// this request.  The plug-in must call WSManPluginAuthzUserComplete to report
// either the user is not authorized with ERROR_ACCESS_DENIED or for successful
...
typedef VOID(WINAPI *WSMAN_PLUGIN_AUTHORIZE_OPERATION)(
    _In_ PVOID pluginContext, _In_ WSMAN_SENDER_DETAILS *senderDetails,
    _In_ DWORD flags, _In_ DWORD operation, _In_ PCWSTR action,
...
DWORD WINAPI WSManPluginAuthzUserComplete(
    _In_ WSMAN_SENDER_DETAILS *senderDetails, _In_ DWORD flags,
    _In_opt_ PVOID userAuthorizationContext, _In_opt_ HANDLE impersonationToken,
    _In_ BOOL userIsAdministrator, _In_ DWORD errorCode,
    _In_opt_ PCWSTR extendedErrorInformation);

DWORD WINAPI WSManPluginAuthzOperationComplete(
    _In_ WSMAN_SENDER_DETAILS *senderDetails, _In_ DWORD flags,
    _In_opt_ PVOID userAuthorizationContext, _In_ DWORD errorCode,
    _In_opt_ PCWSTR extendedErrorInformation);

DWORD WINAPI WSManPluginAuthzQueryQuotaComplete(
    _In_ WSMAN_SENDER_DETAILS *senderDetails, _In_ DWORD flags,
    _In_opt_ WSMAN_AUTHZ_QUOTA *quota, _In_ DWORD errorCode,
    _In_opt_ PCWSTR extendedErrorInformation);
