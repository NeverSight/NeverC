...
#define SPC_MARKER_CHECK_CURRENTLY_SUPPORTED_FLAGS                             \
...
//                                      pcbSignedDataMsg contains the required
//                                      size.
//
extern BOOL WINAPI CryptSIPGetSignedDataMsg(IN SIP_SUBJECTINFO *pSubjectInfo,
...
typedef BOOL(WINAPI *pCryptSIPGetSignedDataMsg)(
    IN SIP_SUBJECTINFO *pSubjectInfo, OUT DWORD *pdwEncodingType,
...
//                                      'put' function again.
//
extern BOOL WINAPI CryptSIPPutSignedDataMsg(IN SIP_SUBJECTINFO *pSubjectInfo,
...
typedef BOOL(WINAPI *pCryptSIPPutSignedDataMsg)(
    IN SIP_SUBJECTINFO *pSubjectInfo, IN DWORD dwEncodingType,
    OUT DWORD *pdwIndex, IN DWORD cbSignedDataMsg, IN BYTE *pbSignedDataMsg);
...
extern BOOL WINAPI CryptSIPCreateIndirectData(
    IN SIP_SUBJECTINFO *pSubjectInfo, IN OUT DWORD *pcbIndirectData,
...
extern BOOL WINAPI CryptSIPVerifyIndirectData(
    IN SIP_SUBJECTINFO *pSubjectInfo, IN SIP_INDIRECT_DATA *pIndirectData);
...
//                                      for the requested SIP.
//
extern BOOL WINAPI CryptSIPRemoveSignedDataMsg(IN SIP_SUBJECTINFO *pSubjectInfo,
                                               IN DWORD dwIndex);
...
//
extern BOOL WINAPI
CryptSIPLoad(IN const GUID *pgSubject, // GUID for the requried sip
...
//      FALSE:                          Errors occured.  See GetLastError()
//
extern BOOL WINAPI CryptSIPRetrieveSubjectGuid(
    IN LPCWSTR FileName,        // wide file name
...
// SIP's object ID is right for the file type.
//
// NOTE:  This function only supports SIPs that are used for catalog files
// (either PE, CAB, or flat).
//
// Returns:
//      TRUE:                           No fatal errors
//      FALSE:                          Errors occured.  See GetLastError()
//
extern BOOL WINAPI CryptSIPRetrieveSubjectGuidForCatalogFile(
    IN LPCWSTR FileName,        // wide file name
    IN OPTIONAL HANDLE hFileIn, // or handle of open file
    OUT GUID *pgSubject);       // defined SIP's GUID
...
extern _Success_(return != FALSE) BOOL WINAPI
    CryptSIPGetSealedDigest(_In_ SIP_SUBJECTINFO *pSubjectInfo,
                            _In_opt_count_(dwSig) const BYTE *pSig,
                            _In_ DWORD dwSig,
                            _Out_writes_opt_(*pcbDigest) BYTE *pbDigest,
                            _Inout_ DWORD *pcbDigest);
typedef _Success_(return != FALSE) BOOL(WINAPI *pCryptSIPGetSealedDigest)(
    _In_ SIP_SUBJECTINFO *pSubjectInfo, _In_opt_count_(dwSig) const BYTE *pSig,
    _In_ DWORD dwSig, _Out_writes_opt_(*pcbDigest) BYTE *pbDigest,
    _Inout_ DWORD *pcbDigest);
