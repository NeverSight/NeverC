...
//                    the flags are 0, then we scan this list looking for a
//                    match for the default form specified.
//
...
// Query Forms
// ===========
//  Query forms are registered and have query pages added to them, a form
//  without pages is not displayed.  Each form has a unique CLSID to allow it to
//  be selected by invoking the query dialog.
//

#define CQFF_NOGLOBALPAGES 0x0000001 // = 1 => doesn't have global pages added
#define CQFF_ISOPTIONAL                                                        \
  0x0000002 // = 1 => form is hidden, unless optional forms requested
...
DECLARE_INTERFACE_IID_(IQueryForm, IUnknown,
...
#define CQPM_ENABLE                                                            \
  0x00000003 // wParam = TRUE/FALSE (enable, disable), lParam = 0
#define CQPM_GETPARAMETERS                                                     \
  0x00000005 // wParam = 0, lParam = -> receives the LocalAlloc
...
#define CQPM_SETDEFAULTPARAMETERS                                              \
  0x00000009 // wParam = 0, lParam -> OPENQUERYWINDOW
...
DECLARE_INTERFACE_IID_(IPersistQuery, IPersist,
  ...
  STDMETHOD(ReadString)(THIS_ LPCWSTR pSection, LPCWSTR pValueName,
                        _Out_ LPWSTR pBuffer, INT cchBuffer) PURE;
...
#define OQWF_SINGLESELECT                                                      \
  0x00000004 // = 1 => view to have single selection (depends on viewer)
#define OQWF_LOADQUERY                                                         \
  0x00000008 // = 1 => use the IPersistQuery to load the given query
...
#define OQWF_SAVEQUERYONOK                                                     \
  0x00000200 // = 1 => use the IPersistQuery to write the query on close
#define OQWF_HIDEMENUS 0x00000400 // = 1 => no menu bar displayed
#define OQWF_HIDESEARCHUI                                                      \
  0x00000800 // = 1 => dialog is filter, therefore start, stop, new search etc

#define OQWF_PARAMISPROPERTYBAG                                                \
  0x80000000 // = 1 => the form parameters ptr is an IPropertyBag
             // (ppbFormParameters)
...
typedef struct {
  ...
  CLSID clsidDefaultForm; // default form to be selected (if OQF_DEFAULTFORM ==
                          // 1 )
...
DECLARE_INTERFACE_IID_(ICommonQuery, IUnknown,
