...
#include <pshpack8.h>
template <VARTYPE T> struct TUIPROPERTYKEY {
  GUID fmtid;
  DWORD pid;
  inline operator const PROPERTYKEY &() const {
    return reinterpret_cast<const PROPERTYKEY &>(*this);
  }
  inline const PROPERTYKEY *operator&() const {
    return reinterpret_cast<const PROPERTYKEY *>(this);
  }
};
#include <poppack.h>
...
// UIBreakCheckType is being used to do both Compilation and Runtime checking of
// types on PROPVARIANTs

struct UIWrongType {};

template <class A, VARTYPE B> struct UIBreakCheckType {
  // If a PROPERTYKEY/type combination is invalid, compilation will break (it
  // will try to cast a bool to UIWrongType)
  typedef UIWrongType _Type;
  static bool Validate(REFPROPERTYKEY key) {
...
#define UI_REGISTER_TYPE(type)                                                 \
  ...
  template <> struct UIBreakCheckType<PROPERTYKEY, type> {                     \
    typedef bool _Type;                                                        \
    /* we cannot catch at compilation time when PROPERTYKEYs are by reference, \
     * so check at runtime */                                                  \
...
HRESULT UIInitPropertyFromBoolean(const T &propertyKey, BOOL fVal,
  ...
  typename UIBreakCheckType<T, VT_BOOL>::_Type valid =
      UIBreakCheckType<T, VT_BOOL>::Validate(propertyKey);
  return valid ? InitPropVariantFromBoolean(fVal, pPropVar) : E_INVALIDARG;
}

template <class T>
HRESULT UIInitPropertyFromUInt32(const T &propertyKey, UINT ulVal,
                                 _Out_ PROPVARIANT *pPropVar) {
  // If this fails to compile, it means this property is not of type VT_UI4
  typename UIBreakCheckType<T, VT_UI4>::_Type valid =
      UIBreakCheckType<T, VT_UI4>::Validate(propertyKey);
  return valid ? InitPropVariantFromUInt32(ulVal, pPropVar) : E_INVALIDARG;
}

template <class T>
HRESULT UIInitPropertyFromString(const T &propertyKey, _In_ PCWSTR psz,
                                 _Out_ PROPVARIANT *pPropVar) {
  // If this fails to compile, it means this property is not of type VT_LPWSTR
  typename UIBreakCheckType<T, VT_LPWSTR>::_Type valid =
      UIBreakCheckType<T, VT_LPWSTR>::Validate(propertyKey);
  return valid ? InitPropVariantFromString(psz, pPropVar) : E_INVALIDARG;
}

template <class T>
HRESULT UIInitPropertyFromDecimal(const T &propertyKey, const DECIMAL &decValue,
                                  _Out_ PROPVARIANT *pPropVar) {
  // If this fails to compile, it means this property is not of type VT_DECIMAL
  typename UIBreakCheckType<T, VT_DECIMAL>::_Type valid =
      UIBreakCheckType<T, VT_DECIMAL>::Validate(propertyKey);
  if (valid) {
...
template <class T>
HRESULT UIInitPropertyFromInterface(const T &propertyKey, _In_ IUnknown *pUnk,
                                    _Out_ PROPVARIANT *pPropVar) {
  // If this fails to compile, it means this property is not of type VT_UNKNOWN
  typename UIBreakCheckType<T, VT_UNKNOWN>::_Type valid =
      UIBreakCheckType<T, VT_UNKNOWN>::Validate(propertyKey);
  if (valid) {
...
template <class T>
HRESULT UIInitPropertyFromImage(const T &propertyKey, _In_ IUIImage *pImage,
                                _Out_ PROPVARIANT *pPropVar) {
  return UIInitPropertyFromInterface(propertyKey, pImage, pPropVar);
}

template <class T>
HRESULT UIInitPropertyFromIUnknownArray(const T &propertyKey,
                                        _In_ SAFEARRAY *psa,
                                        _Out_ PROPVARIANT *pPropVar) {
  // If this fails to compile, it means this property is not of type VT_ARRAY
  typename UIBreakCheckType<T, VT_ARRAY | VT_UNKNOWN>::_Type valid =
      UIBreakCheckType<T, VT_ARRAY | VT_UNKNOWN>::Validate(propertyKey);
...
template <class T>
HRESULT UIPropertyToBoolean(const T &propertyKey, REFPROPVARIANT propvarIn,
                            _Out_ BOOL *pfRet) {
  // If this fails to compile, it means this property is not of type VT_BOOL
  typename UIBreakCheckType<T, VT_BOOL>::_Type valid =
      UIBreakCheckType<T, VT_BOOL>::Validate(propertyKey);
...
template <class T>
HRESULT UIPropertyToUInt32(const T &propertyKey, REFPROPVARIANT propvarIn,
                           _Out_ UINT *pulVal) {
  // If this fails to compile, it means this property is not of type VT_UI4
  typename UIBreakCheckType<T, VT_UI4>::_Type valid =
      UIBreakCheckType<T, VT_UI4>::Validate(propertyKey);
...
template <class T>
HRESULT UIPropertyToStringAlloc(const T &propertyKey, REFPROPVARIANT propvarIn,
                                _Outptr_ PWSTR *ppszOut) {
  // If this fails to compile, it means this property is not of type VT_LPWSTR
  typename UIBreakCheckType<T, VT_LPWSTR>::_Type valid =
      UIBreakCheckType<T, VT_LPWSTR>::Validate(propertyKey);
...
template <class T>
HRESULT UIPropertyToDecimal(const T &propertyKey, REFPROPVARIANT propvarIn,
                            _Out_ DECIMAL *pDecValue) {
  // If this fails to compile, it means this property is not of type VT_DECIMAL
  typename UIBreakCheckType<T, VT_DECIMAL>::_Type valid =
      UIBreakCheckType<T, VT_DECIMAL>::Validate(propertyKey);
...
HRESULT UIPropertyToInterface(const T &propertyKey, REFPROPVARIANT propvarIn,
                              _Outptr_ TInterface **ppObj) {
  *ppObj = NULL;
  // If this fails to compile, it means this property is not of type VT_UNKNOWN
  typename UIBreakCheckType<T, VT_UNKNOWN>::_Type valid =
      UIBreakCheckType<T, VT_UNKNOWN>::Validate(propertyKey);
  if (valid && propvarIn.vt == VT_UNKNOWN) {
    if (propvarIn.punkVal) {
      return propvarIn.punkVal->QueryInterface(__uuidof(TInterface),
                                               (void **)ppObj);
    }
    return S_OK;
  }
...
template <class T>
HRESULT UIPropertyToImage(const T &propertyKey, REFPROPVARIANT propvarIn,
                          _Outptr_ IUIImage **ppImage) {
  return UIPropertyToInterface(propertyKey, propvarIn, ppImage);
}

template <class T>
HRESULT UIPropertyToIUnknownArrayAlloc(const T &propertyKey,
