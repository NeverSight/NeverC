...
DECLARE_INTERFACE_(IVPBaseConfig, IUnknown) {
  ...
  STDMETHOD(GetConnectInfo)(
      THIS_ _Inout_ LPDWORD pdwNumConnectInfo,
      _Out_writes_to_opt_(*pdwNumConnectInfo, *pdwNumConnectInfo)
          LPDDVIDEOPORTCONNECT pddVPConnectInfo) PURE;
  ...
  STDMETHOD(GetVideoFormats)(THIS_ _Inout_ LPDWORD pdwNumFormats,
                             _Out_writes_to_opt_(*pdwNumFormats, *pdwNumFormats)
                                 LPDDPIXELFORMAT pddPixelFormats) PURE;
  ...
  STDMETHOD(SetInvertPolarity)(THIS) PURE;
  ...
  STDMETHOD(GetOverlaySurface)(THIS_ OUT LPDIRECTDRAWSURFACE *
                               ppddOverlaySurface) PURE;

  // sets the direct draw kernel handle
  STDMETHOD(SetDirectDrawKernelHandle)(THIS_ IN ULONG_PTR dwDDKernelHandle)
      PURE;
