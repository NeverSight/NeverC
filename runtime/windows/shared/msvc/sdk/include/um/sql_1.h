...
struct SQL_LEVEL_1_TOKEN {
  ...
  BOOL bConstIsStrNumeric; // True if the vConstValue is a BSTR and is a UINT32
                           // or any 64bit number
...
struct SQL_LEVEL_1_RPN_EXPRESSION {
  ...
  int nCurPropSize;
  _Field_size_part_(nCurPropSize, nNumberOfProperties)
      BSTR *pbsRequestedPropertyNames; // Array of property names which values
                                       // are to be returned if
