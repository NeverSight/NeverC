...
Abstract:
    ...
    This module contains the user-mode APIs for the Projected File System
(ProjFS) library.

    ProjFS enables an application, the "provider", to use these APIs to present
a store of hierarchical data as a projection into the file system.  The on-disk
location of these files are rooted in a directory called a "virtualization
root".  The provider chooses this directory and manages all files within it. The
virtualization root together with its descendants comprises a "virtualization
    instance".

    For example, a hypothetical ProjFS registry provider could present the keys
and values in the Windows Registry as a tree of directories and files rooted at
C:\Registry.  A single provider may manage multiple virtualization instances,
each with its own virtualization root.  To extend the example, a ProjFS registry
provider could choose to establish a separate virtualization instance for each
    registry hive, resulting in directory trees rooted at
C:\Registry\HKEY_CLASSES_ROOT, C:\Registry\HKEY_CURRENT_USER, etc.  Note that a
virtualization root cannot be a descendant of another virtualization root.
...
STDAPI
PrjStartVirtualizing(_In_ PCWSTR virtualizationRootPath,
                     ...
                     _Outptr_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT
...
STDAPI
PrjClearNegativePathCache(_In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT
                              namespaceVirtualizationContext,
...
PrjMarkDirectoryAsPlaceholder(
    _In_ PCWSTR rootPathName, _In_opt_ PCWSTR targetPathName,
...
STDAPI
PrjWritePlaceholderInfo(_In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT
                            namespaceVirtualizationContext,
                        _In_ PCWSTR destinationFileName,
                        _In_reads_bytes_(placeholderInfoSize)
                            const PRJ_PLACEHOLDER_INFO *placeholderInfo,
                        _In_ UINT32 placeholderInfoSize);

STDAPI
PrjWritePlaceholderInfo2(_In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT
                             namespaceVirtualizationContext,
                         _In_ PCWSTR destinationFileName,
                         _In_reads_bytes_(placeholderInfoSize)
                             const PRJ_PLACEHOLDER_INFO *placeholderInfo,
...
PrjUpdateFileIfNeeded(
    ...
    _In_reads_bytes_(placeholderInfoSize)
        const PRJ_PLACEHOLDER_INFO *placeholderInfo,
    _In_ UINT32 placeholderInfoSize, _In_opt_ PRJ_UPDATE_TYPES updateFlags,
    _Out_opt_ PRJ_UPDATE_FAILURE_CAUSES *failureReason);

STDAPI
PrjDeleteFile(
    _In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT namespaceVirtualizationContext,
    _In_ PCWSTR destinationFileName, _In_opt_ PRJ_UPDATE_TYPES updateFlags,
    _Out_opt_ PRJ_UPDATE_FAILURE_CAUSES *failureReason);

STDAPI
PrjWriteFileData(
    _In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT namespaceVirtualizationContext,
    _In_ const GUID *dataStreamId, _In_reads_bytes_(length) void *buffer,
    _In_ UINT64 byteOffset, _In_ UINT32 length);
...
STDAPI
PrjGetOnDiskFileState(_In_ PCWSTR destinationFileName,
                      _Out_ PRJ_FILE_STATE *fileState);

STDAPI_(void *)
PrjAllocateAlignedBuffer(_In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT
                             namespaceVirtualizationContext,
                         _In_ size_t size);

STDAPI_(void)
PrjFreeAlignedBuffer(_In_ void *buffer);
...
typedef _Function_class_(PRJ_START_DIRECTORY_ENUMERATION_CB)
    HRESULT(CALLBACK PRJ_START_DIRECTORY_ENUMERATION_CB)(
        _In_ const PRJ_CALLBACK_DATA *callbackData,
        _In_ const GUID *enumerationId);

typedef _Function_class_(PRJ_GET_DIRECTORY_ENUMERATION_CB)
    HRESULT(CALLBACK PRJ_GET_DIRECTORY_ENUMERATION_CB)(
        _In_ const PRJ_CALLBACK_DATA *callbackData,
        _In_ const GUID *enumerationId, _In_opt_ PCWSTR searchExpression,
        _In_ PRJ_DIR_ENTRY_BUFFER_HANDLE dirEntryBufferHandle);

typedef _Function_class_(PRJ_END_DIRECTORY_ENUMERATION_CB)
    HRESULT(CALLBACK PRJ_END_DIRECTORY_ENUMERATION_CB)(
        _In_ const PRJ_CALLBACK_DATA *callbackData,
        _In_ const GUID *enumerationId);

typedef _Function_class_(PRJ_GET_PLACEHOLDER_INFO_CB)
    HRESULT(CALLBACK PRJ_GET_PLACEHOLDER_INFO_CB)(
        _In_ const PRJ_CALLBACK_DATA *callbackData);

typedef _Function_class_(PRJ_GET_FILE_DATA_CB) HRESULT(
    CALLBACK PRJ_GET_FILE_DATA_CB)(_In_ const PRJ_CALLBACK_DATA *callbackData,
                                   _In_ UINT64 byteOffset, _In_ UINT32 length);

typedef _Function_class_(PRJ_QUERY_FILE_NAME_CB)
    HRESULT(CALLBACK PRJ_QUERY_FILE_NAME_CB)(
        _In_ const PRJ_CALLBACK_DATA *callbackData);
...
typedef _Function_class_(PRJ_NOTIFICATION_CB)
    HRESULT(CALLBACK PRJ_NOTIFICATION_CB)(
        _In_ const PRJ_CALLBACK_DATA *callbackData, _In_ BOOLEAN isDirectory,
        _In_ PRJ_NOTIFICATION notification, _In_opt_ PCWSTR destinationFileName,
        _Inout_ PRJ_NOTIFICATION_PARAMETERS *operationParameters);

typedef _Function_class_(PRJ_CANCEL_COMMAND_CB) VOID(
    CALLBACK PRJ_CANCEL_COMMAND_CB)(_In_ const PRJ_CALLBACK_DATA *callbackData);
...
PrjCompleteCommand(
    _In_ PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT namespaceVirtualizationContext,
    _In_ INT32 commandId, _In_ HRESULT completionResult,
...
STDAPI
PrjFillDirEntryBuffer(_In_ PCWSTR fileName,
                      _In_opt_ PRJ_FILE_BASIC_INFO *fileBasicInfo,
                      _In_ PRJ_DIR_ENTRY_BUFFER_HANDLE dirEntryBufferHandle);

STDAPI
PrjFillDirEntryBuffer2(_In_ PRJ_DIR_ENTRY_BUFFER_HANDLE dirEntryBufferHandle,
                       _In_ PCWSTR fileName,
                       _In_opt_ PRJ_FILE_BASIC_INFO *fileBasicInfo,
                       _In_opt_ PRJ_EXTENDED_INFO *extendedInfo);

STDAPI_(BOOLEAN)
PrjFileNameMatch(_In_ PCWSTR fileNameToCheck, _In_ PCWSTR pattern);

STDAPI_(int)
PrjFileNameCompare(_In_ PCWSTR fileName1, _In_ PCWSTR fileName2);

STDAPI_(BOOLEAN)
PrjDoesNameContainWildCards(_In_ LPCWSTR fileName);
