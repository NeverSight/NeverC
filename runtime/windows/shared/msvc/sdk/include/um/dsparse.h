...
_Check_return_ _Success_(return == ERROR_SUCCESS) DSPARSE DWORD WINAPI
    DsMakeSpnW(_In_ LPCWSTR ServiceClass, _In_ LPCWSTR ServiceName,
               _In_opt_ LPCWSTR InstanceName, _In_ USHORT InstancePort,
               _In_opt_ LPCWSTR Referrer, _Inout_ DWORD *pcSpnLength,
               _Out_writes_to_opt_(*pcSpnLength, *pcSpnLength) LPWSTR pszSpn);

_Check_return_ _Success_(return == ERROR_SUCCESS) DSPARSE DWORD WINAPI
    DsMakeSpnA(_In_ LPCSTR ServiceClass, _In_ LPCSTR ServiceName,
               _In_opt_ LPCSTR InstanceName, _In_ USHORT InstancePort,
               _In_opt_ LPCSTR Referrer, _Inout_ DWORD *pcSpnLength,
...
_Check_return_ DSPARSE DWORD WINAPI DsCrackSpnA(
    _In_ LPCSTR pszSpn, _Inout_opt_ LPDWORD pcServiceClass,
...
_Check_return_ DSPARSE DWORD WINAPI DsCrackSpnW(
    _In_ LPCWSTR pszSpn, _Inout_opt_ DWORD *pcServiceClass,
...
DsQuoteRdnValue(
    ...
    _Out_writes_to_(*pcQuotedRdnValueLength, *pcQuotedRdnValueLength)    LPTCH
psQuotedRdnValue
    )
/*++
...
Arguments:
    ...
    pcQuotedRdnValueeLength - IN, maximum length of psQuotedRdnValue, in chars
                        ...
                        OUT ERROR_BUFFER_OVERFLOW, chars needed in
psQuotedRdnValue
...
_Check_return_ _Success_(return == 0) DSPARSE DWORD WINAPI DsQuoteRdnValueW(
    DWORD cUnquotedRdnValueLength,
    _In_reads_(cUnquotedRdnValueLength) IN LPCWCH psUnquotedRdnValue,
    _Inout_ DWORD *pcQuotedRdnValueLength,
    _Out_writes_to_(*pcQuotedRdnValueLength, *pcQuotedRdnValueLength)
        LPWCH psQuotedRdnValue);

_Check_return_ _Success_(return == 0) DSPARSE DWORD WINAPI DsQuoteRdnValueA(
    DWORD cUnquotedRdnValueLength,
...
DsUnquoteRdnValue(
    ...
    _Out_writes_to_(*pcUnquotedRdnValueLength,*pcUnquotedRdnValueLength) LPTCH
psUnquotedRdnValue
    )
...
Arguments:
    ...
    pcUnquotedRdnValueLength - IN, maximum length of psUnquotedRdnValue, in
chars OUT ERROR_SUCCESS, chars used in psUnquotedRdnValue OUT
ERROR_BUFFER_OVERFLOW, chars needed for psUnquotedRdnValue
...
_Check_return_ _Success_(return == 0) DSPARSE DWORD WINAPI DsUnquoteRdnValueW(
    DWORD cQuotedRdnValueLength,
    _In_reads_(cQuotedRdnValueLength) LPCWCH psQuotedRdnValue,
    _Inout_ DWORD *pcUnquotedRdnValueLength,
    _Out_writes_to_(*pcUnquotedRdnValueLength, *pcUnquotedRdnValueLength)
        LPWCH psUnquotedRdnValue);

_Check_return_ _Success_(return == 0) DSPARSE DWORD WINAPI
    DsUnquoteRdnValueA(DWORD cQuotedRdnValueLength,
                       _In_reads_(cQuotedRdnValueLength) LPCCH psQuotedRdnValue,
                       _Inout_ DWORD *pcUnquotedRdnValueLength,
                       _Out_writes_to_(*pcUnquotedRdnValueLength,
...
--*/
_Check_return_ _Success_(return == 0) DSPARSE DWORD WINAPI
    DsGetRdnW(_Inout_ _At_(*ppDN, _Pre_readable_size_(*pcDN)
                                      _Post_readable_size_(*pcDN)) LPCWCH *ppDN,
              _Inout_ DWORD *pcDN, _Outptr_result_buffer_(*pcKey) LPCWCH *ppKey,
              _Out_ DWORD *pcKey, _Outptr_result_buffer_(*pcVal) LPCWCH *ppVal,
              _Out_ DWORD *pcVal);
...
DSPARSE
BOOL WINAPI DsCrackUnquotedMangledRdnW(_In_reads_(cchRDN) LPCWSTR pszRDN,
                                       DWORD cchRDN, _Out_opt_ GUID *pGuid,
                                       _Out_opt_ DS_MANGLE_FOR *peDsMangleFor);

DSPARSE
BOOL WINAPI DsCrackUnquotedMangledRdnA(_In_reads_(cchRDN) LPCSTR pszRDN,
                                       DWORD cchRDN, _Out_opt_ GUID *pGuid,
                                       _Out_opt_ DS_MANGLE_FOR *peDsMangleFor);
...
Description
    ...
    Determine if the given RDN Value is mangled, and of the given type. Note
that the key portion of an RDN should not be supplied.

    The name may be quoted or unquoted.  This routine tries to unquote the
value.  If the unquote operation fails, the routine proceeds to attempt the
unmangle.

    A change was made in the default quoting behavior of DNs returned from the
DS between Windows 2000 and Windows XP. This routine transparently handles RDNs
with special characters in either form.
...
DSPARSE
BOOL WINAPI DsIsMangledRdnValueW(_In_reads_(cRdn) LPCWSTR pszRdn, DWORD cRdn,
                                 DS_MANGLE_FOR eDsMangleForDesired);

DSPARSE
BOOL WINAPI DsIsMangledRdnValueA(_In_reads_(cRdn) LPCSTR pszRdn, DWORD cRdn,
                                 DS_MANGLE_FOR eDsMangleForDesired);
...
DSPARSE
BOOL WINAPI DsIsMangledDnA(_In_ LPCSTR pszDn, DS_MANGLE_FOR eDsMangleFor);

DSPARSE
BOOL WINAPI DsIsMangledDnW(_In_ LPCWSTR pszDn, DS_MANGLE_FOR eDsMangleFor);
...
//
// DWORD DsCrackSpn2(
//      IN LPTSTR pszSPN,           // the SPN to parse (does not have to be
//      NULL-terminated) IN DWORD cSpn,            // length of pszSPN IN OUT
//      PUSHORT pcServiceClass OPTIONAL,
...
DWORD
WINAPI
DsCrackSpn2A(_In_reads_(cSpn) LPCSTR pszSpn, _In_ DWORD cSpn,
             _Inout_opt_ LPDWORD pcServiceClass,
             _Out_writes_to_opt_(*pcServiceClass, *pcServiceClass)
                 LPSTR ServiceClass,
             _Inout_opt_ LPDWORD pcServiceName,
             _Out_writes_to_opt_(*pcServiceName, *pcServiceName)
                 LPSTR ServiceName,
             _Inout_opt_ LPDWORD pcInstanceName,
             _Out_writes_to_opt_(*pcInstanceName, *pcInstanceName)
                 LPSTR InstanceName,
             _Out_opt_ USHORT *pInstancePort);
...
DWORD
WINAPI
DsCrackSpn2W(_In_reads_(cSpn) LPCWSTR pszSpn, _In_ DWORD cSpn,
...
DWORD
WINAPI
DsCrackSpn3W(_In_ LPCWSTR pszSpn, IN DWORD cSpn, IN OUT DWORD *pcHostName,
             _Out_writes_to_(*pcHostName, *pcHostName) LPWSTR HostName,
             IN OUT DWORD *pcInstanceName,
             _Out_writes_to_(*pcInstanceName, *pcInstanceName)
                 LPWSTR InstanceName,
...
DWORD
WINAPI
DsCrackSpn4W(_In_ LPCWSTR pszSpn, IN DWORD cSpn, IN OUT DWORD *pcHostName,
             _Out_writes_to_(*pcHostName, *pcHostName) LPWSTR HostName,
             IN OUT DWORD *pcInstanceName,
             _Out_writes_to_(*pcInstanceName, *pcInstanceName)
                 LPWSTR InstanceName,
