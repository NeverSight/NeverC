...
//
// The V2 information is used in most cases of logon.  The structure is
// identical to the V1 token information structure, with the exception that the
// memory allocation is handled differently.  The LSA_TOKEN_INFORMATION_V2
// structure is intended to be allocated monolithiclly, with the privileges,
// DACL, sids, and group array either part of same allocation, or allocated and
// freed externally.
//
...
//
// The V3 token information structure adds claims support to the LSA token.  LSA
// assumes that the first fields in this structure are identical to those in
// LSA_TOKEN_INFORMATION_V1, so no changes should be made that are not reflected
// there as well.
//
...
typedef struct _LSA_TOKEN_INFORMATION_V3 {
  ...
  // assigned SIDs.  NULL DeviceGroups is valid, and indicates that no
  // compounding should occur.  If DeviceGroups are present, LSA will add WORLD
  // and other assigned SIDs.
  //
...
typedef NTSTATUS(NTAPI LSA_CREATE_LOGON_SESSION)(_Inout_ PLUID LogonId);

typedef NTSTATUS(NTAPI LSA_DELETE_LOGON_SESSION)(_In_ PLUID LogonId);

typedef NTSTATUS(NTAPI LSA_ADD_CREDENTIAL)(_In_ PLUID LogonId,
                                           _In_ ULONG AuthenticationPackage,
                                           _In_ PLSA_STRING PrimaryKeyValue,
                                           _In_ PLSA_STRING Credentials);

typedef NTSTATUS(NTAPI LSA_GET_CREDENTIALS)(_In_ PLUID LogonId,
                                            _In_ ULONG AuthenticationPackage,
...
typedef NTSTATUS(NTAPI LSA_DELETE_CREDENTIAL)(_In_ PLUID LogonId,
                                              _In_ ULONG AuthenticationPackage,
                                              _In_ PLSA_STRING PrimaryKeyValue);

typedef PVOID(NTAPI LSA_ALLOCATE_LSA_HEAP)(_In_ ULONG Length);

typedef VOID(NTAPI LSA_FREE_LSA_HEAP)(_In_ PVOID Base);

typedef PVOID(NTAPI LSA_ALLOCATE_PRIVATE_HEAP)(_In_ SIZE_T Length);

typedef VOID(NTAPI LSA_FREE_PRIVATE_HEAP)(_In_ PVOID Base);

typedef NTSTATUS(NTAPI LSA_ALLOCATE_CLIENT_BUFFER)(
    _In_ PLSA_CLIENT_REQUEST ClientRequest, _In_ ULONG LengthRequired,
    _Outptr_result_bytebuffer_(LengthRequired) PVOID *ClientBaseAddress);

typedef NTSTATUS(NTAPI LSA_FREE_CLIENT_BUFFER)(
    _In_ PLSA_CLIENT_REQUEST ClientRequest, _In_ PVOID ClientBaseAddress);

typedef NTSTATUS(NTAPI LSA_COPY_TO_CLIENT_BUFFER)(
    _In_ PLSA_CLIENT_REQUEST ClientRequest, _In_ ULONG Length,
...
typedef NTSTATUS(NTAPI LSA_AP_INITIALIZE_PACKAGE)(
    ...
    _In_ PLSA_DISPATCH_TABLE LsaDispatchTable, _In_opt_ PLSA_STRING Database,
...
typedef NTSTATUS(NTAPI LSA_AP_LOGON_USER)(
    ...
    _Outptr_ PVOID *TokenInformation, _Out_ PLSA_UNICODE_STRING *AccountName,
...
typedef NTSTATUS(NTAPI LSA_AP_LOGON_USER_EX)(
    ...
    _Out_ PNTSTATUS SubStatus,
    _Out_ PLSA_TOKEN_INFORMATION_TYPE TokenInformationType,
    _Outptr_ PVOID *TokenInformation, _Out_ PUNICODE_STRING *AccountName,
...
typedef VOID(NTAPI LSA_AP_LOGON_TERMINATED)(_In_ PLUID LogonId);
...
typedef NTSTATUS (*PSAM_CREDENTIAL_UPDATE_NOTIFY_ROUTINE)(
    ...
    _In_ ULONG OldCredentialSize, _In_ ULONG UserAccountControl,
    _In_opt_ PUNICODE_STRING UPN, _In_ PUNICODE_STRING UserName,
    _In_ PUNICODE_STRING NetbiosDomainName, _In_ PUNICODE_STRING DnsDomainName,
...
typedef VOID (*PSAM_CREDENTIAL_UPDATE_FREE_ROUTINE)(_In_ PVOID p);
...
typedef struct _SECPKG_CLIENT_INFO {
  ...
  ImpersonationLevel; // Impersonation level of client
...
#define SECPKG_CLIENT_PROCESS_TERMINATED                                       \
  0x01 // The client process has terminated
...
#define SECPKG_CALL_THREAD_TERM                                                \
  0x00000080 // Call is from a thread that has term'd
#define SECPKG_CALL_PROCESS_TERM                                               \
  0x00000100                          // Call is from a process that has term'd
#define SECPKG_CALL_IS_TCB 0x00000200 // Call is from TCB
#define SECPKG_CALL_NETWORK_ONLY                                               \
  0x00000400 // Call asks for network logon only, no cached logons
#define SECPKG_CALL_WINLOGON                                                   \
  0x00000800 // the caller of LsaLogonuser() is Winlogon
#define SECPKG_CALL_ASYNC_UPDATE 0x00001000 // asynchronous update for unlock
#define SECPKG_CALL_SYSTEM_PROC                                                \
  0x00002000                        // Call originated from the System process
#define SECPKG_CALL_NEGO 0x00004000 // Called by SPNEGO
#define SECPKG_CALL_NEGO_EXTENDER 0x00008000 // Called by NEGO extender
#define SECPKG_CALL_BUFFER_MARSHAL                                             \
  0x00010000                          // Buffer passed is marshaled (by RPC)
#define SECPKG_CALL_UNLOCK 0x00020000 // Unlock
#define SECPKG_CALL_CLOUDAP_CONNECT                                            \
  0x00040000 // the caller of LsaLogonuser() is CloudAP during connection flow

//
// WOWXX: Additional defines to determine which type of WoW guest we are dealing
// with.
//
#define SECPKG_CALL_WOWX86 0x00000040
#define SECPKG_CALL_WOWA32 0x00040000

// Whenever Negotiate goes through different packages, it chooses which packages
// to try and if the error returned from the packages warrants a breakout. The
// packages themselves, however, do not know the reason why the previous
// packages could not handle the logon. This is good for most cases, but it's
// possible that package A wants package B to know why it failed so that package
// B can either change its behavior or log it. The ledger here is optional and
// controlled via LSA. The contract is simple:
// 1. To set the information, a package calls the LSA function
// LsaSetSecpkgFailureReason().
// 2. To get the information, a package calls the LSA function
// LsaGetSecpkgFailureReason().

// These enums are for special cases that might not be apparent based on the
// returned NTSTATUS;
typedef enum _SECPKG_FAILURE_SPECIAL_REASON {
  ...
  SecpkgFailureReason_NullTarget,    // The targetname used during
                                     // InitializeSecurityContext was null.
  SecpkgFailureReason_UnknownTarget, // The targetname used during
                                     // InitializeSecurityContext could not be
                                     // resolved.
  SecpkgFailureReason_IpAddress,     // The targetname used during
                                 // InitializeSecurityContext contained an IP
                                 // Address.
  SecpkgFailureReason_DupTarget, // The targetname used during
                                 // InitializeSecurityContext has duplicates.
                                 // E.g. duplicate SPS in AD.
  SecpkgFailureReason_NoLineOfSight, // The secpkg needed a line-of-sight to a
                                     // Domain Controller, but none could be
                                     // found.
  SecpkgFailureReason_Loopback,      // The secpkg does not support loopback
                                     // authentication.
  SecpkgFailureReason_NullSession, // The secpkg does not handle null sessions.
} SECPKG_FAILURE_SPECIAL_REASON,
    *PSECPKG_FAILURE_SPECIAL_REASON;
...
typedef struct _SECPKG_CREDENTIAL {
  ...
  SECPKG_BYTE_VECTOR
  MarshaledSuppliedCreds; // contains a SECPKG_SUPPLIED_CREDENTIAL structure
...
#define PRIMARY_CRED_INTERNET_USER                                             \
  0x00000100 // online identity credential, consumer accounts like MSA
#define PRIMARY_CRED_AUTH_ID                                                   \
  0x00000200 // credential is unencrypted SEC_WINNT_AUTH_IDENTITY_EX2
...
#define PRIMARY_CRED_PACKED_CREDS                                              \
  0x00008000 // PSEC_WINNT_AUTH_PACKED_CREDENTIALS
#define PRIMARY_CRED_ENTERPRISE_INTERNET_USER                                  \
  0x00010000 // online identity credential, enterprise accounts like AAD
#define PRIMARY_CRED_ENCRYPTED_CREDGUARD_PASSWORD                              \
  0x00020000 // password is encrypted by CredGuard
#define PRIMARY_CRED_CACHED_INTERACTIVE_LOGON                                  \
  0x00040000 // the actual logon type seen by the SSP was CachedInteractive
             // Nego does optimized logon and async online logon for Interactive
             // Logon type sent via Winlogon. LSA distributes the original
             // LogonType to SSPs for their SpAcceptCredentials() function, so
             // the SSPs don't know if the logon was optimized or online.
             // However if the logon package adds this flag then SSPs know for
             // sure that the logon was cached interactive(no network) and the
             // absence of this flag denotes network logon was attempted.
...
#define PRIMARY_CRED_SUPPLEMENTAL                                              \
  0x00400000 // The update is only to move supplemental credentials around
             // all primary credentials fields except the LogonId should be
             // ignored
#define PRIMARY_CRED_FOR_PASSWORD_CHANGE                                       \
  0x00800000 // The credential will be used for a password change
...
typedef struct _SECPKG_PRIMARY_CRED {
  ...
  ULONG Flags;
  UNICODE_STRING
  DnsDomainName;      // DNS domain name where account is located (if known)
...
//
// SECPKG_PRIMARY_CRED_EX has the same layout of SECPKG_PRIMARY_CRED for
// existing fields. this is designed to work with existing code transparently.
//
...
typedef enum _SECPKG_CALL_PACKAGE_MESSAGE_TYPE {
  ...
  SecPkgCallPackageMaxMessage =
      SecPkgCallPackageTransferCredMessage, // update when adding new messages
...
// associated with the TSPkg logon session. This is intended to bind the NLA
// session to the interactive logon session
...
typedef NTSTATUS(NTAPI LSA_DUPLICATE_HANDLE)(_In_ HANDLE SourceHandle,
                                             _Out_ PHANDLE DestionationHandle);

typedef NTSTATUS(NTAPI LSA_SAVE_SUPPLEMENTAL_CREDENTIALS)(
    _In_ PLUID LogonId, _In_ ULONG SupplementalCredSize,
    _In_reads_bytes_(SupplementalCredSize) PVOID SupplementalCreds,
    _In_ BOOLEAN Synchronous);

typedef HANDLE(NTAPI LSA_CREATE_THREAD)(_In_ SEC_ATTRS SecurityAttributes,
...
typedef NTSTATUS(NTAPI LSA_GET_CLIENT_INFO_EX)(
    _Out_ PSECPKG_CLIENT_INFO_EX ClientInfo, _In_ ULONG StructSize);

typedef HANDLE(NTAPI LSA_REGISTER_NOTIFICATION)(
    _In_ SEC_THREAD_START StartFunction, _In_opt_ PVOID Parameter,
    _In_ ULONG NotificationType, _In_ ULONG NotificationClass,
    _In_ ULONG NotificationFlags, _In_ ULONG IntervalMinutes,
    _In_opt_ HANDLE WaitEvent);

typedef NTSTATUS(NTAPI LSA_CANCEL_NOTIFICATION)(_In_ HANDLE NotifyHandle);

typedef NTSTATUS(NTAPI LSA_MAP_BUFFER)(_In_ PSecBuffer InputBuffer,
                                       _Out_ PSecBuffer OutputBuffer);

typedef NTSTATUS(NTAPI LSA_CREATE_TOKEN)(
    _In_ PLUID LogonId, _In_ PTOKEN_SOURCE TokenSource,
    _In_ SECURITY_LOGON_TYPE LogonType,
    _In_ SECURITY_IMPERSONATION_LEVEL ImpersonationLevel,
    _In_ LSA_TOKEN_INFORMATION_TYPE TokenInformationType,
    _In_ PVOID TokenInformation, _In_opt_ PTOKEN_GROUPS TokenGroups,
    _In_ PUNICODE_STRING AccountName, _In_ PUNICODE_STRING AuthorityName,
    _In_opt_ PUNICODE_STRING Workstation, _In_opt_ PUNICODE_STRING ProfilePath,
...
typedef NTSTATUS(NTAPI LSA_CREATE_TOKEN_EX)(
    ...
    _In_ SECPKG_SESSIONINFO_TYPE SessionInformationType, _Out_ PHANDLE Token,
    _Out_ PNTSTATUS SubStatus);

typedef VOID(NTAPI LSA_AUDIT_LOGON)(
    _In_ NTSTATUS Status, _In_ NTSTATUS SubStatus,
    _In_opt_ PUNICODE_STRING AccountName,
    _In_opt_ PUNICODE_STRING AuthenticatingAuthority,
    _In_opt_ PUNICODE_STRING WorkstationName, _In_opt_ PSID UserSid,
    _In_ SECURITY_LOGON_TYPE LogonType, _In_ PTOKEN_SOURCE TokenSource,
    _In_ PLUID LogonId);
...
typedef NTSTATUS(NTAPI LSA_CALL_PACKAGEEX)(
    _In_ PUNICODE_STRING AuthenticationPackage, _In_ PVOID ClientBufferBase,
    _In_reads_bytes_(SubmitBufferLength) PVOID ProtocolSubmitBuffer,
    _In_ ULONG SubmitBufferLength,
    _Outptr_result_bytebuffer_(*ReturnBufferLength) PVOID *ProtocolReturnBuffer,
    _Out_ PULONG ReturnBufferLength, _Out_ PNTSTATUS ProtocolStatus);
...
typedef BOOLEAN(NTAPI LSA_GET_CALL_INFO)(_Out_ PSECPKG_CALL_INFO Info);

typedef PVOID(NTAPI LSA_CREATE_SHARED_MEMORY)(_In_ ULONG MaxSize,
                                              _In_ ULONG InitialSize);

typedef PVOID(NTAPI LSA_ALLOCATE_SHARED_MEMORY)(_In_ PVOID SharedMem,
                                                _In_ ULONG Size);

typedef VOID(NTAPI LSA_FREE_SHARED_MEMORY)(_In_ PVOID SharedMem,
                                           _Inout_opt_ PVOID Memory);

typedef BOOLEAN(NTAPI LSA_DELETE_SHARED_MEMORY)(_In_ PVOID SharedMem);

typedef NTSTATUS(NTAPI LSA_GET_APP_MODE_INFO)(_Out_opt_ PULONG UserFunction,
                                              _Out_opt_ PULONG_PTR Argument1,
                                              _Out_opt_ PULONG_PTR Argument2,
                                              _Out_opt_ PSecBuffer UserData,
                                              _Out_opt_ PBOOLEAN ReturnToLsa);

typedef NTSTATUS(NTAPI LSA_SET_APP_MODE_INFO)(_In_ ULONG UserFunction,
                                              _In_opt_ ULONG_PTR Argument1,
                                              _In_opt_ ULONG_PTR Argument2,
                                              _In_opt_ PSecBuffer UserData,
                                              _In_opt_ BOOLEAN ReturnToLsa);

typedef NTSTATUS(NTAPI LSA_GET_SECPKG_FAILURE_REASON)(
    _In_ const ULONG_PTR PackageID, _Out_ SECPKG_FAILURE_REASON *Reason);

typedef NTSTATUS(NTAPI LSA_SET_SECPKG_FAILURE_REASON)(
...
typedef NTSTATUS(NTAPI LSA_OPEN_SAM_USER)(_In_ PSECURITY_STRING Name,
...
typedef NTSTATUS(NTAPI LSA_CLOSE_SAM_USER)(_In_ PVOID UserHandle);

typedef NTSTATUS(NTAPI LSA_GET_AUTH_DATA_FOR_USER)(
    _In_ PSECURITY_STRING Name, _In_ SECPKG_NAME_TYPE NameType,
    _In_opt_ PSECURITY_STRING Prefix,
    _Outptr_result_bytebuffer_(*UserAuthDataSize) PUCHAR *UserAuthData,
    _Out_ PULONG UserAuthDataSize, _Out_opt_ PUNICODE_STRING UserFlatName);

typedef NTSTATUS(NTAPI LSA_CONVERT_AUTH_DATA_TO_TOKEN)(
    _In_ PVOID UserAuthData, _In_ ULONG UserAuthDataSize,
    _In_ SECURITY_IMPERSONATION_LEVEL ImpersonationLevel,
    _In_ PTOKEN_SOURCE TokenSource, _In_ SECURITY_LOGON_TYPE LogonType,
    _In_ PUNICODE_STRING AuthorityName, _Out_ PHANDLE Token,
    _Out_ PLUID LogonId, _Out_ PUNICODE_STRING AccountName,
    _Out_ PNTSTATUS SubStatus);

typedef NTSTATUS(NTAPI LSA_CRACK_SINGLE_NAME)(
    _In_ ULONG FormatOffered, _In_ BOOLEAN PerformAtGC,
    _In_ PUNICODE_STRING NameInput, _In_opt_ PUNICODE_STRING Prefix,
    _In_ ULONG RequestedFormat, _Out_ PUNICODE_STRING CrackedName,
    _Out_ PUNICODE_STRING DnsDomainName, _Out_ PULONG SubStatus);

typedef NTSTATUS(NTAPI LSA_AUDIT_ACCOUNT_LOGON)(
    _In_ ULONG AuditId, _In_ BOOLEAN Success, _In_opt_ PUNICODE_STRING Source,
    _In_opt_ PUNICODE_STRING ClientName, _In_opt_ PUNICODE_STRING MappedName,
    _In_ NTSTATUS Status);

typedef NTSTATUS(NTAPI LSA_CLIENT_CALLBACK)(_In_ PCHAR Callback,
...
typedef NTSTATUS(NTAPI LSA_REGISTER_CALLBACK)(
    _In_ ULONG CallbackId, _In_ PLSA_CALLBACK_FUNCTION Callback);

typedef NTSTATUS(NTAPI LSA_GET_EXTENDED_CALL_FLAGS)(_Out_ PULONG Flags);
...
typedef VOID(NTAPI LSA_PROTECT_MEMORY)(_Inout_updates_bytes_(BufferSize)
                                           PVOID Buffer,
                                       _In_ ULONG BufferSize);

typedef NTSTATUS(NTAPI LSA_OPEN_TOKEN_BY_LOGON_ID)(
    _In_ PLUID LogonId, _Out_ HANDLE *RetTokenHandle);

typedef NTSTATUS(NTAPI LSA_EXPAND_AUTH_DATA_FOR_DOMAIN)(
    _In_reads_bytes_(UserAuthDataSize) PUCHAR UserAuthData,
    _In_ ULONG UserAuthDataSize, _In_ PVOID Reserved,
...
typedef NTSTATUS(NTAPI LSA_GET_SERVICE_ACCOUNT_PASSWORD)(
    _In_ PUNICODE_STRING AccountName, _In_opt_ PUNICODE_STRING DomainName,
    _In_ CRED_FETCH CredFetch, _Inout_ FILETIME *FileTimeExpiry,
...
typedef VOID(NTAPI LSA_AUDIT_LOGON_EX)(
    ...
    _In_ SECURITY_LOGON_TYPE LogonType,
    _In_ SECURITY_IMPERSONATION_LEVEL ImpersonationLevel,
    _In_ PTOKEN_SOURCE TokenSource, _In_ PLUID LogonId);

typedef NTSTATUS(NTAPI LSA_CHECK_PROTECTED_USER_BY_TOKEN)(
    _In_ HANDLE UserToken, _Out_ PBOOLEAN ProtectedUser);

typedef NTSTATUS(NTAPI LSA_QUERY_CLIENT_REQUEST)(
    _In_ PLSA_CLIENT_REQUEST ClientRequest, _In_ ULONG QueryType,
...
typedef struct _ENCRYPTED_CREDENTIALW {
  ...
  // The CredentialBlobSize field is the length (in bytes) of the encrypted
  // credential
  //
...
#define CREDP_FLAGS_IN_PROCESS                                                 \
  0x01 // Caller is in-process. Password data may be returned
#define CREDP_FLAGS_USE_MIDL_HEAP                                              \
  0x02 // Allocated buffer should use MIDL_user_allocte
#define CREDP_FLAGS_DONT_CACHE_TI                                              \
  0x04 // TargetInformation shouldn't be cached for CredGetTargetInfo
#define CREDP_FLAGS_CLEAR_PASSWORD                                             \
  0x08 // Credential blob is passed in in-the-clear
#define CREDP_FLAGS_USER_ENCRYPTED_PASSWORD                                    \
  0x10 // Credential blob is passed protected by RtlEncryptMemory
#define CREDP_FLAGS_TRUSTED_CALLER                                             \
  0x20 // Caller is a trusted process (eg. logon process).
#define CREDP_FLAGS_VALIDATE_PROXY_TARGET                                      \
  0x40 // Validate that TargetName is proxy

typedef NTSTATUS(NTAPI CredReadFn)(_In_ PLUID LogonId, _In_ ULONG CredFlags,
                                   _In_ LPWSTR TargetName, _In_ ULONG Type,
                                   _In_ ULONG Flags,
                                   _Outptr_ PENCRYPTED_CREDENTIALW *Credential);

typedef NTSTATUS(NTAPI CredReadDomainCredentialsFn)(
    _In_ PLUID LogonId, _In_ ULONG CredFlags,
    _In_ PCREDENTIAL_TARGET_INFORMATIONW TargetInfo, _In_ ULONG Flags,
    _Out_ PULONG Count,
...
typedef NTSTATUS(NTAPI CredWriteFn)(_In_ PLUID LogonId, _In_ ULONG CredFlags,
                                    _In_ PENCRYPTED_CREDENTIALW Credential,
                                    _In_ ULONG Flags);

typedef NTSTATUS(NTAPI CrediUnmarshalandDecodeStringFn)(
    _In_ LPWSTR MarshaledString,
    _Outptr_result_bytebuffer_(*BlobSize) LPBYTE *Blob, _Out_ ULONG *BlobSize,
...
NTSTATUS
CredMarshalTargetInfo(_In_ PCREDENTIAL_TARGET_INFORMATIONW InTargetInfo,
                      _Outptr_result_bytebuffer_(*BufferSize) PUSHORT *Buffer,
                      _Out_ PULONG BufferSize);

NTSTATUS
CredUnmarshalTargetInfo(
    _In_reads_bytes_(BufferSize) PUSHORT Buffer, _In_ ULONG BufferSize,
...
typedef PVOID(NTAPI LSA_LOCATE_PKG_BY_ID)(_In_ ULONG PackgeId);
...
typedef NTSTATUS(NTAPI SpInitializeFn)(
    _In_ ULONG_PTR PackageId, _In_ PSECPKG_PARAMETERS Parameters,
    _In_ PLSA_SECPKG_FUNCTION_TABLE FunctionTable);

typedef NTSTATUS(NTAPI SpShutdownFn)(VOID);

typedef NTSTATUS(NTAPI SpGetInfoFn)(_Out_ PSecPkgInfo PackageInfo);
...
typedef NTSTATUS(LSA_AP_LOGON_USER_EX2)(
    ...
    _In_ PVOID ClientBufferBase, _In_ ULONG SubmitBufferSize,
    _Outptr_result_bytebuffer_(*ProfileBufferSize) PVOID *ProfileBuffer,
    _Out_ PULONG ProfileBufferSize, _Out_ PLUID LogonId,
    _Out_ PNTSTATUS SubStatus,
    _Out_ PLSA_TOKEN_INFORMATION_TYPE TokenInformationType,
    _Outptr_ PVOID *TokenInformation, _Out_ PUNICODE_STRING *AccountName,
    _Out_ PUNICODE_STRING *AuthenticatingAuthority,
...
typedef NTSTATUS(LSA_AP_PRE_LOGON_USER_SURROGATE)(
    ...
    _In_reads_bytes_(SubmitBufferSize) PVOID ProtocolSubmitBuffer,
    _In_ PVOID ClientBufferBase, _In_ ULONG SubmitBufferSize,
    _Inout_ PSECPKG_SURROGATE_LOGON SurrogateLogon, _Out_ PNTSTATUS SubStatus);
...
typedef NTSTATUS(LSA_AP_POST_LOGON_USER_SURROGATE)(
    ...
    _In_ ULONG ProfileBufferSize, _In_ PLUID LogonId, _In_ NTSTATUS Status,
    _In_ NTSTATUS SubStatus,
    _In_ LSA_TOKEN_INFORMATION_TYPE TokenInformationType,
    _In_ PVOID TokenInformation, _In_ PUNICODE_STRING AccountName,
...
typedef NTSTATUS(NTAPI SpAcceptCredentialsFn)(
    _In_ SECURITY_LOGON_TYPE LogonType, _In_ PUNICODE_STRING AccountName,
...
typedef NTSTATUS(NTAPI SpAcquireCredentialsHandleFn)(
    _In_opt_ PUNICODE_STRING PrincipalName, _In_ ULONG CredentialUseFlags,
    _In_opt_ PLUID LogonId, _In_opt_ PVOID AuthorizationData,
    _In_opt_ PVOID GetKeyFunciton, _In_opt_ PVOID GetKeyArgument,
...
typedef NTSTATUS(NTAPI SpQueryCredentialsAttributesFn)(
    _In_ LSA_SEC_HANDLE CredentialHandle, _In_ ULONG CredentialAttribute,
...
typedef NTSTATUS(NTAPI SpAddCredentialsFn)(
    _In_ LSA_SEC_HANDLE CredentialHandle,
    _In_opt_ PUNICODE_STRING PrincipalName, _In_ PUNICODE_STRING Package,
    _In_ ULONG CredentialUseFlags, _In_ PVOID AuthorizationData,
    _In_ PVOID GetKeyFunciton, _In_ PVOID GetKeyArgument,
    _Out_ PTimeStamp ExpirationTime);

typedef NTSTATUS(NTAPI SpSaveCredentialsFn)(
    _In_ LSA_SEC_HANDLE CredentialHandle, _In_ PSecBuffer Credentials);

typedef NTSTATUS(NTAPI SpGetCredentialsFn)(_In_ LSA_SEC_HANDLE CredentialHandle,
                                           _Inout_ PSecBuffer Credentials);

typedef NTSTATUS(NTAPI SpDeleteCredentialsFn)(
    _In_ LSA_SEC_HANDLE CredentialHandle, _In_ PSecBuffer Key);

typedef NTSTATUS(NTAPI SpInitLsaModeContextFn)(
    _In_opt_ LSA_SEC_HANDLE CredentialHandle,
    _In_opt_ LSA_SEC_HANDLE ContextHandle, _In_opt_ PUNICODE_STRING TargetName,
    _In_ ULONG ContextRequirements, _In_ ULONG TargetDataRep,
    _In_ PSecBufferDesc InputBuffers, _Out_ PLSA_SEC_HANDLE NewContextHandle,
    _Inout_ PSecBufferDesc OutputBuffers, _Out_ PULONG ContextAttributes,
    _Out_ PTimeStamp ExpirationTime, _Out_ PBOOLEAN MappedContext,
    _Out_ PSecBuffer ContextData);

typedef NTSTATUS(NTAPI SpDeleteContextFn)(_In_ LSA_SEC_HANDLE ContextHandle);

typedef NTSTATUS(NTAPI SpApplyControlTokenFn)(_In_ LSA_SEC_HANDLE ContextHandle,
                                              _In_ PSecBufferDesc ControlToken);

typedef NTSTATUS(NTAPI SpAcceptLsaModeContextFn)(
    _In_opt_ LSA_SEC_HANDLE CredentialHandle,
    _In_opt_ LSA_SEC_HANDLE ContextHandle, _In_ PSecBufferDesc InputBuffer,
    _In_ ULONG ContextRequirements, _In_ ULONG TargetDataRep,
    _Out_ PLSA_SEC_HANDLE NewContextHandle, _Inout_ PSecBufferDesc OutputBuffer,
    _Out_ PULONG ContextAttributes, _Out_ PTimeStamp ExpirationTime,
    _Out_ PBOOLEAN MappedContext, _Out_ PSecBuffer ContextData);

typedef NTSTATUS(NTAPI SpGetUserInfoFn)(_In_ PLUID LogonId, _In_ ULONG Flags,
                                        _Outptr_ PSecurityUserData *UserData);

typedef NTSTATUS(NTAPI SpQueryContextAttributesFn)(
    _In_ LSA_SEC_HANDLE ContextHandle, _In_ ULONG ContextAttribute,
    _Inout_ PVOID Buffer);
...
typedef NTSTATUS(NTAPI SpChangeAccountPasswordFn)(
    _In_ PUNICODE_STRING pDomainName, _In_ PUNICODE_STRING pAccountName,
    _In_ PUNICODE_STRING pOldPassword, _In_ PUNICODE_STRING pNewPassword,
    _In_ BOOLEAN Impersonating, _Inout_ PSecBufferDesc pOutput);

typedef NTSTATUS(NTAPI SpQueryMetaDataFn)(
    _In_opt_ LSA_SEC_HANDLE CredentialHandle,
    _In_opt_ PUNICODE_STRING TargetName, _In_ ULONG ContextRequirements,
...
typedef NTSTATUS(NTAPI SpExchangeMetaDataFn)(
    _In_opt_ LSA_SEC_HANDLE CredentialHandle,
    _In_opt_ PUNICODE_STRING TargetName, _In_ ULONG ContextRequirements,
    _In_ ULONG MetaDataLength, _In_reads_bytes_(MetaDataLength) PUCHAR MetaData,
    _Inout_ PLSA_SEC_HANDLE ContextHandle);

typedef NTSTATUS(NTAPI SpGetCredUIContextFn)(
    _In_ LSA_SEC_HANDLE ContextHandle, _In_ GUID *CredType,
    _Out_ PULONG FlatCredUIContextLength,
    _Outptr_result_bytebuffer_(*FlatCredUIContextLength)
...
typedef NTSTATUS(NTAPI SpExtractTargetInfoFn)(
    _In_opt_ PLSA_CLIENT_REQUEST ClientRequest,
    _In_reads_bytes_(SubmitBufferLength) PVOID ProtocolSubmitBuffer,
    _In_opt_ PVOID ClientBufferBase, _In_ ULONG SubmitBufferLength,
    _Result_nullonfailure_ _Outptr_result_bytebuffer_(*pcbTargetInfo)
        PVOID *ppvTargetInfo,
...
typedef NTSTATUS(NTAPI SpGetRemoteCredGuardLogonBufferFn)(
    _In_ LSA_SEC_HANDLE CredHandle, _In_ LSA_SEC_HANDLE ContextHandle,
    _In_ const UNICODE_STRING *TargetName, _Out_ PHANDLE RedirectedLogonHandle,
...
typedef NTSTATUS(NTAPI SpGetRemoteCredGuardSupplementalCredsFn)(
    _In_ LSA_SEC_HANDLE CredHandle, _In_ const UNICODE_STRING *TargetName,
...
typedef NTSTATUS(NTAPI SpGetTbalSupplementalCredsFn)(
    _In_ LUID LogonId, _Out_ PULONG SupplementalCredsSize,
    _Outptr_result_bytebuffer_(*SupplementalCredsSize)
        PVOID *SupplementalCreds);
...
typedef struct _SECPKG_FUNCTION_TABLE {
  ...
  CallPackageUntrusted; // SECPKG_INTERFACE_VERSION
  PLSA_AP_CALL_PACKAGE_PASSTHROUGH
  CallPackagePassthrough;                   // SECPKG_INTERFACE_VERSION
  ...
  SpAcquireCredentialsHandleFn
      *AcquireCredentialsHandle; // SECPKG_INTERFACE_VERSION
  SpQueryCredentialsAttributesFn
      *QueryCredentialsAttributes;                  // SECPKG_INTERFACE_VERSION
  ...
  SpGetExtendedInformationFn
      *GetExtendedInformation; // SECPKG_INTERFACE_VERSION
  SpQueryContextAttributesFn
      *QueryContextAttributes;        // SECPKG_INTERFACE_VERSION
  SpAddCredentialsFn *AddCredentials; // SECPKG_INTERFACE_VERSION
  SpSetExtendedInformationFn
      *SetExtendedInformation; // SECPKG_INTERFACE_VERSION

  SpSetContextAttributesFn *SetContextAttributes; // SECPKG_INTERFACE_VERSION_2

  SpSetCredentialsAttributesFn
      *SetCredentialsAttributes; // SECPKG_INTERFACE_VERSION_3

  SpChangeAccountPasswordFn
      *ChangeAccountPassword; // SECPKG_INTERFACE_VERSION_4
  ...
  SpGetRemoteCredGuardLogonBufferFn
      *GetRemoteCredGuardLogonBuffer; // SECPKG_INTERFACE_VERSION_8
  SpGetRemoteCredGuardSupplementalCredsFn
      *GetRemoteCredGuardSupplementalCreds; // SECPKG_INTERFACE_VERSION_8

  SpGetTbalSupplementalCredsFn
      *GetTbalSupplementalCreds; // SECPKG_INTERFACE_VERSION_9

  PLSA_AP_LOGON_USER_EX3 LogonUserEx3; // SECPKG_INTERFACE_VERSION_10
  PLSA_AP_PRE_LOGON_USER_SURROGATE
  PreLogonUserSurrogate; // SECPKG_INTERFACE_VERSION_10
  PLSA_AP_POST_LOGON_USER_SURROGATE
  PostLogonUserSurrogate; // SECPKG_INTERFACE_VERSION_10
...
typedef NTSTATUS(NTAPI SpInstanceInitFn)(
    _In_ ULONG Version, _In_ PSECPKG_DLL_FUNCTIONS FunctionTable,
    _Outptr_ PVOID *UserFunctions);

typedef NTSTATUS(NTAPI SpInitUserModeContextFn)(
    _In_ LSA_SEC_HANDLE ContextHandle, _In_ PSecBuffer PackedContext);

typedef NTSTATUS(NTAPI SpMakeSignatureFn)(_In_ LSA_SEC_HANDLE ContextHandle,
...
typedef NTSTATUS(NTAPI SpVerifySignatureFn)(
    _In_ LSA_SEC_HANDLE ContextHandle, _In_ PSecBufferDesc MessageBuffers,
    _In_ ULONG MessageSequenceNumber, _Out_opt_ PULONG QualityOfProtection);

typedef NTSTATUS(NTAPI SpSealMessageFn)(_In_ LSA_SEC_HANDLE ContextHandle,
                                        _In_ ULONG QualityOfProtection,
                                        _In_ PSecBufferDesc MessageBuffers,
                                        _In_ ULONG MessageSequenceNumber);

typedef NTSTATUS(NTAPI SpUnsealMessageFn)(_In_ LSA_SEC_HANDLE ContextHandle,
                                          _In_ PSecBufferDesc MessageBuffers,
                                          _In_ ULONG MessageSequenceNumber,
                                          _Out_opt_ PULONG QualityOfProtection);

typedef NTSTATUS(NTAPI SpGetContextTokenFn)(_In_ LSA_SEC_HANDLE ContextHandle,
...
typedef NTSTATUS(NTAPI SpCompleteAuthTokenFn)(_In_ LSA_SEC_HANDLE ContextHandle,
                                              _In_ PSecBufferDesc InputBuffer);

typedef NTSTATUS(NTAPI SpFormatCredentialsFn)(
    _In_ PSecBuffer Credentials, _Out_ PSecBuffer FormattedCredentials);
...
// AttributeInfo flag
//
// The highest order bit of AttributeInfo indicates whether the attribute data
// is ANSI (0) or UNICODE (1). The remaining bits store an enumeration,
// indicating which call the attribute came from:
...
//
// On success, MarshaledAttributeData contains MarshaledAttributeDataSize bytes
// of packed data. If no work needs to be done, returns STATUS_NOT_SUPPORTED and
// leaves MarshaledAttributeData NULL.
//
typedef NTSTATUS(NTAPI SpMarshalAttributeDataFn)(
    _In_ DWORD AttributeInfo, _In_ ULONG Attribute,
    ...
    _Outptr_result_bytebuffer_(*MarshaledAttributeDataSize)
...
typedef NTSTATUS(SEC_ENTRY *SpLsaModeInitializeFn)(
    _In_ ULONG LsaVersion, _Out_ PULONG PackageVersion,
...
//
// * Passed by the LSA to SpLsaModeInitializeFn to indicate the version of the
// LSA.
//      All packages currently expect the LSA to pass SECPKG_INTERFACE_VERSION.
//
// * Passed by secur32.dll to SpUserModeInitialzeFn to indicate the version of
// the secur32 DLL.
//      All packages currently expect secur32 to pass SECPKG_INTERFACE_VERSION.
//
// * Returned from SpLsaModeInitializeFn to indicate the version of
// SECPKG_FUNCTION_TABLE.
//      SECPKG_INTERFACE_VERSION indicates all fields through
//      SetExtendedInformation are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_2 indicates all fields through
//      SetContextAttributes are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_3 indicates all fields through
//      SetCredentialsAttributes are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_4 indicates all fields through
//      ChangeAccountPassword are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_5 indicates all fields through
//      UpdateCredentials are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_6 indicates all fields through
//      ValidateTargetInfo are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_7 indicates all fields through
//      PostLogonUserInfo are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_8 indicates all fields through
//      GetRemoteSupplementalCreds are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_9 indicates all fields through
//      GetTbalSupplementalCreds are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_10 indicates all fields through
//      PostLogonUserSurrogate are defined (potentially to NULL)
//      SECPKG_INTERFACE_VERSION_11 indicates all fields through
//      ExtractTargetInfo are defined (potentially to NULL)
//
// * Returned from SpUserModeInitializeFn to indicate the version of the auth
// package.
//      SECPKG_INTERFACE_VERSION indicates all fields through ImportContext are
//      defined (potentially to NULL) SECPKG_INTERFACE_VERSION_2 indicates all
//      fields through MarshalAttributeData are defined (potentially to NULL)
//
...
typedef PVOID(SEC_ENTRY KSEC_CREATE_CONTEXT_LIST)(_In_ KSEC_CONTEXT_TYPE Type);

typedef VOID(SEC_ENTRY KSEC_INSERT_LIST_ENTRY)(_In_ PVOID List,
                                               _In_ PKSEC_LIST_ENTRY Entry);

typedef NTSTATUS(SEC_ENTRY KSEC_REFERENCE_LIST_ENTRY)(
    _In_ PKSEC_LIST_ENTRY Entry, _In_ ULONG Signature,
    _In_ BOOLEAN RemoveNoRef);

typedef VOID(SEC_ENTRY KSEC_DEREFERENCE_LIST_ENTRY)(_In_ PKSEC_LIST_ENTRY Entry,
                                                    _Out_opt_ BOOLEAN *Delete);

typedef NTSTATUS(SEC_ENTRY KSEC_SERIALIZE_WINNT_AUTH_DATA)(
    _In_ PVOID pvAuthData, _Out_ PULONG Size,
...
typedef PVOID(SEC_ENTRY KSEC_LOCATE_PKG_BY_ID)(_In_ ULONG PackageId);
...
typedef NTSTATUS(NTAPI KspDeleteContextFn)(_In_ LSA_SEC_HANDLE ContextId,
                                           _Out_ PLSA_SEC_HANDLE LsaContextId);

typedef NTSTATUS(NTAPI KspInitContextFn)(_In_ LSA_SEC_HANDLE ContextId,
                                         _In_ PSecBuffer ContextData,
                                         _Out_ PLSA_SEC_HANDLE NewContextId);

typedef NTSTATUS(NTAPI KspMakeSignatureFn)(_In_ LSA_SEC_HANDLE ContextId,
                                           _In_ ULONG fQOP,
                                           _In_ PSecBufferDesc Message,
                                           _In_ ULONG MessageSeqNo);

typedef NTSTATUS(NTAPI KspVerifySignatureFn)(_In_ LSA_SEC_HANDLE ContextId,
                                             _In_ PSecBufferDesc Message,
                                             _In_ ULONG MessageSeqNo,
                                             _Out_opt_ PULONG pfQOP);

typedef NTSTATUS(NTAPI KspSealMessageFn)(_In_ LSA_SEC_HANDLE ContextId,
                                         _In_ ULONG fQOP,
                                         _In_ PSecBufferDesc Message,
                                         _In_ ULONG MessageSeqNo);

typedef NTSTATUS(NTAPI KspUnsealMessageFn)(_In_ LSA_SEC_HANDLE ContextId,
                                           _In_ PSecBufferDesc Message,
                                           _In_ ULONG MessageSeqNo,
                                           _Out_opt_ PULONG pfQOP);

typedef NTSTATUS(NTAPI KspGetTokenFn)(_In_ LSA_SEC_HANDLE ContextId,
                                      _Out_opt_ PHANDLE ImpersonationToken,
                                      _Outptr_opt_ PACCESS_TOKEN *RawToken);

typedef NTSTATUS(NTAPI KspQueryAttributesFn)(_In_ LSA_SEC_HANDLE ContextId,
                                             _In_ ULONG Attribute,
                                             _Inout_ PVOID Buffer);

typedef NTSTATUS(NTAPI KspCompleteTokenFn)(_In_ LSA_SEC_HANDLE ContextId,
                                           _In_ PSecBufferDesc Token);

typedef NTSTATUS(NTAPI KspMapHandleFn)(_In_ LSA_SEC_HANDLE ContextId,
                                       _Out_ PLSA_SEC_HANDLE LsaContextId);

typedef NTSTATUS(NTAPI KspSetPagingModeFn)(_In_ BOOLEAN PagingMode);

typedef NTSTATUS(NTAPI KspSerializeAuthDataFn)(_In_ PVOID pvAuthData,
                                               _Out_ PULONG Size,
...
SECURITY_STATUS
SEC_ENTRY
KSecRegisterSecurityProvider(_In_ PSECURITY_STRING ProviderName,
                             _In_ PSECPKG_KERNEL_FUNCTION_TABLE Table);

SECURITY_STATUS
SEC_ENTRY
KSecLocatePackage(_In_ PUNICODE_STRING PackageName,
