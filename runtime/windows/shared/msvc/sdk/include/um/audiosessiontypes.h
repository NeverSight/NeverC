...
//
// Description: Type definitions used by the audio session manager RPC/COM
// interfaces
//
#pragma once
...
//     and use the
//                                   WAS format.
//     AUDCLNT_SHAREMODE_EXCLUSIVE - The device will be opened in exclusive mode
//     and use the
...
//
//
//     AUDCLNT_STREAMFLAGS_CROSSPROCESS -             Audio policy control for
//     this stream will be shared with
//                                                    with other process
//                                                    sessions that use the same
//                                                    audio session GUID.
//
//     AUDCLNT_STREAMFLAGS_LOOPBACK -                 Initializes a renderer
//     endpoint for a loopback audio application.
//                                                    In this mode, a capture
//                                                    stream will be opened on
//                                                    the specified renderer
//                                                    endpoint. Shared mode and
//                                                    a renderer endpoint is
//                                                    required. Otherwise the
//                                                    IAudioClient::Initialize
//                                                    call will fail. If the
//                                                    initialize is successful,
//                                                    a capture stream will be
//                                                    available from the
//                                                    IAudioClient object.
//
//     AUDCLNT_STREAMFLAGS_EVENTCALLBACK -            An exclusive mode client
//     will supply an event handle that will be
//                                                    signaled when an IRP
//                                                    completes (or a waveRT
//                                                    buffer completes) telling
//                                                    it to fill the next buffer
//
//     AUDCLNT_STREAMFLAGS_NOPERSIST -                Session state will not be
//     persisted
//
//     AUDCLNT_STREAMFLAGS_RATEADJUST -               The sample rate of the
//     stream is adjusted to a rate specified by an application.
//
//     AUDCLNT_STREAMFLAGS_SRC_DEFAULT_QUALITY -      When used with
//     AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM, a sample rate
//                                                    converter with better
//                                                    quality than the default
//                                                    conversion but with a
//                                                    higher performance cost is
//                                                    used. This should be used
//                                                    if the audio is ultimately
//                                                    intended to be heard by
//                                                    humans as opposed to other
//                                                    scenarios such as pumping
//                                                    silence or populating a
//                                                    meter.
//
...
//                                                    session controls.
//
//     AUDCLNT_SESSIONFLAGS_DISPLAY_HIDE -            Don't show volume control
//     in the Volume Mixer.
//
//     AUDCLNT_SESSIONFLAGS_DISPLAY_HIDEWHENEXPIRED - Don't show volume control
//     in the Volume Mixer after the
//                                                    session expires.
//
//
