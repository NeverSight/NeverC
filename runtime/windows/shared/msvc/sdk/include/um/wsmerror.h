...
// MessageText:
//
// The WS-Management service cannot process the request. The service cannot find
// the resource identified by the resource URI and selectors.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The WS-Addressing
// action URI is invalid. Check the documentation for information on how to
// construct an action URI.
//
#define ERROR_WSMAN_INVALID_ACTIONURI 0x80338001L

// Same text as ERROR_WSMAN_INVALID_RESOURCE_URI, but some code in R2 uses this
// error code
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The resource URI is
// missing or it has an incorrect format. Check the documentation or use the
// following command for information on how to construct a resource URI: "winrm
// help uris".
//
...
// MessageText:
//
// The WS-Management service cannot complete the request. The WSManEnumerator
// object is full and no more items can be added.
//
...
// MessageText:
//
// The WS-Management configuration is corrupted. Use the following command to
// restore defaults: %n%n winrm invoke Restore
// http://schemas.microsoft.com/wbem/wsman/1/config @{} %n%n Then add any custom
...
// MessageText:
//
// The WS-Management service cannot process a pull request because a pull
...
// MessageText:
//
// The WS-Management enumeration session is finished or cancelled and cannot be
// used. Start a new enumeration.
//
...
// MessageText:
//
// The event subscription is already closed and cannot be used. Start a new
// subscription.
//
...
// MessageText:
//
// The event subscription session is closing and cannot be used. Start a new
// subscription.
//
...
// MessageText:
//
// The application or script that has an event subscription did not request a
// pull operation within the heartbeat interval. The subscription session was
// closed. Start a new subscription.
//
#define ERROR_WSMAN_SUBSCRIPTION_CLIENT_DID_NOT_CALL_WITHIN_HEARTBEAT          \
  0x8033800AL
...
// The value specified is smaller than the minimum allowed value for this
// setting. Change the timeout value and try the request again.
//
...
// MessageText:
//
// The WS-Management service does not support the SOAP version specified in the
// request.
//
...
// MessageText:
//
// The WS-Management service does not support the encoding specified in the
// request.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The request contains
// one or more invalid SOAP headers.
//
...
// MessageText:
//
// The WS-Management service cannot process a SOAP header in the request that is
// marked as mustUnderstand by the client. This could be caused by the use of a
// version of the protocol which is not supported, or may be an incompatibility
// between the client and server implementations.
//
#define ERROR_WSMAN_SOAP_FAULT_MUST_UNDERSTAND 0x80338010L

// wsa, code=Sender, subcode=WS-Addressing MessageInformationHeaderRequired,
...
// MessageText:
//
// The WS-Management service cannot process the request. The request does not
// have all the expected SOAP headers.
//
...
// the WinRM service, run the following command on the destination to analyze
...
// MessageText:
//
// The WS-Management service does not support the action specified in the
// request.
//
#define ERROR_WSMAN_ACTION_NOT_SUPPORTED 0x80338013L

// This is probably what will wrap all other Windows error codes and so should
// not explicitly be used wsa, code=Receiver, subcode=WS-Addressing
// EndpointUnavailable, details=
//
// MessageId: ERROR_WSMAN_ENDPOINT_UNAVAILABLE
//
// MessageText:
//
// The WS-Management service cannot process the request because the resource is
...
// MessageText:
//
// The WS-Management service cannot identify the format of the object passed to
// a Put or Create method. The input XML may not be appropriate for the resource
// or uses the wrong schema for the resource. Change the input XML in the
// request.
//
...
// MessageText:
//
// The expiration time passed to the WS-Management Enumerate method is not
// valid. The time value may be zero or refer to a time in the past. Change the
// expiration time and try the request again.
//
//
#define ERROR_WSMAN_ENUMERATE_INVALID_EXPIRATION_TIME 0x80338016L

// wsman, code=Sender, subcode=WS-Management UnsupportedFeature - WS-Management
// faultDetail/ExpirationTime, details=
//
// MessageId: ERROR_WSMAN_ENUMERATE_UNSUPPORTED_EXPIRATION_TIME
//
// MessageText:
//
// The data source does not support expiration time. Remove the expiration time
// from the request and try the request again.
//
...
// MessageText:
//
// The data source does not support filtering. Remove the filter from the
...
// details=supported_dialects
//
// MessageId: ERROR_WSMAN_ENUMERATE_FILTER_DIALECT_REQUESTED_UNAVAILABLE
//
// MessageText:
//
// The filter dialect (the type associated with the filter) was not supported
// for this resource. Change the filter dialect or remove it from the request
...
// MessageText:
//
// The data source could not process the filter. The filter might be missing or
// it might be invalid. Change the filter and try the request again.
//
#define ERROR_WSMAN_ENUMERATE_CANNOT_PROCESS_FILTER 0x8033801AL

// wsen, code=Receiver, subcode=WS-Enumeration InvalidEnumerationContext,
// details=
//
...
// MessageText:
//
// The WS-Enumeration context in the enumeration is not valid. Enumeration may
// have been completed or canceled. You cannot use this enumeration context
// anymore. Start a new enumeration.
//
...
// MessageText:
//
// The pull operation did not get any data in the MaxTime duration. But the
// enumeration is still valid. The client can attempt to do another pull request
// to retrieve data.
//
...
// MessageText:
//
// The WS-Management service cannot renew the enumeration. Start a new
// enumeration.
//
#define ERROR_WSMAN_ENUMERATE_UNABLE_TO_RENEW 0x8033801DL

// wse, code=Sender, subcode=WS-Eventing DeliveryModeRequestedUnavailable,
// details=List of delivery modes that are supported
//
// MessageId: ERROR_WSMAN_EVENTING_DELIVERY_MODE_REQUESTED_UNAVAILABLE
//
// MessageText:
//
// The WS-Management service does not support the delivery mode for the
// specified resource. The client should change the subscription to use one of
...
// MessageText:
//
// The expiration time of the subscription is invalid. The time is either not
// supported, zero or a time that happened in the past. Change the expiration
// time and try the request again.
//
...
// MessageText:
//
// The expiration time specified for subscription was invalid. Specify the
// expiration time as a duration.
//
...
// MessageText:
//
// The event source does not support filtering. Remove the filter from the
// request and try the request again.
//
...
// MessageText:
//
// The WS-Management service cannot renew the event subscription. Create a new
// subscription.
//
...
// MessageText:
//
// The WS-Management service cannot complete the WS-Eventing request because the
// request had some unknown or invalid content and could not be processed.
//
...
// MessageText:
//
// The WS-Management service cannot process the response because it is larger
...
// packet does not have a valid SOAP body.
//
...
// MessageText:
//
// The resumption context specified in the subscription is invalid. It may have
// expired, or be in the wrong format.
//
...
// wsman, code=Sender, subcode=WS-Management ResumptionTypeNotSupported,
// details=
//
...
// MessageText:
//
// The WS-Management service does not support the type of resumption requested
// by the subscription.
//
...
// MessageText:
//
// The request contains character encoding that is unsupported. WS-Management
// only supports requests that are encoded in UTF-8 or UTF-16. Change the
// character encoding in the request and try the request again.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the subscription
// ID is invalid.
//
...
// MessageText:
//
// The WS-Management service cannot process the batch request. The request must
// specify either MaxItems, MaxCharacters, or MaxTime.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the
// WS-Addressing Action URI in the request is not compatible with the resource.
//
...
// MessageText:
//
// The WS-Management service cannot complete the WS-Addressing Action URI in the
// request because the resource was already in use.
//
...
// MessageText:
//
// The WS-Management service cannot create the resource because it already
// exists.
//
...
// MessageText:
//
// The WS-Management service cannot complete the request because the receiver
// does not accept the delivery of events. The receiver requests that the
// subscription be cancelled. Event receivers return this message to force the
...
// MessageText:
//
// The WS-Management service cannot process the request because the encoding of
// the request exceeds an internal encoding limit. Reconfigure the client to
// send messages which fit the encoding limits of the service.
//
...
// MessageText:
//
// The WS-Management service does not support the format of the WS-Addressing
// Endpoint Reference.
//
...
// MessageText:
//
// The bookmark in the subscription is invalid. The bookmark may be expired or
// corrupted. Issue a new subscription without any bookmarks or locate the
// correct bookmark.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because one or more
// options are not valid. The option names or values may not be valid or they
// are used in incorrect combinations. Retrieve the catalog entry for the
// resource and determine how to correct the invalid option values.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because a parameter for
...
// MessageText:
//
// The WS-Management service cannot process the request because the selectors
// for the resource are not valid.
//
...
// MessageText:
//
// The requested metadata is not available at the current address. Retry the
// request with a new address.
//
...
// MessageText:
//
// The WS-Management service cannot rename the resource. The selectors for the
// resource are not correct. The resource may exist already, the address may be
// incorrect, or the resource URI may be invalid. Change the request and retry.
//
...
// MessageText:
//
// The SOAP XML in the message does not match the corresponding XML schema
// definition. Change the XML and retry.
//
...
// MessageText:
//
// The WS-Management service does not support the specified feature. Remove the
// unsupported feature from the request and retry.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the XML is
// invalid.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the URI contains
...
// MessageText:
//
// The event source is attempting to deliver an event when a delivery is in
// progress already.
//
...
// MessageText:
//
// The response that the WS-Management service computed exceeds the maximum
// envelope size in the request.
//
...
// MessageText:
//
// The response that the WS-Management service computed exceed the internal
// limit for envelope size.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because it contains too
// many options.
//
...
// MessageText:
//
// The WS-Management service does not support the character set used in the
// request. Change the request to use UTF-8 or UTF-16.
//
...
// MessageText:
//
// The operation succeeded and cannot be reversed but the result is too large to
// send.
//
...
// The filter dialect is the type of filter, such as XPath or WQL.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because it contains a
// bookmark that is expired.
//
...
// MessageText:
//
// The WS-Management provider does not support the specified option set because
// mustComply for one of the options is set to true. Change mustComply for one
// of the options to false.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because one or more of
// the options has an invalid name.
//
...
//
// The WS-Management service cannot process the request because one or more of
// the options has an invalid value.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. A parameter that is
// required for the operation is not the correct type.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. A parameter name is
// invalid.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the XML content
// has invalid values.
//
...
//
// The WS-Management service cannot identify the format of the object passed to
// a Put or Create method. The XML namespace for the input XML is invalid.
// Change the XML namespace for the input XML in the request.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because an XML fragment
// in the URI is invalid.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the request did
// not contain all required selectors.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because a value for a
// selector is of the wrong type.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because a value for the
// selector is invalid.
//
...
//
// The WS-Management service cannot process the request because the selectors
// for the resource are ambiguous.
//
...
//
// The WS-Management service cannot process the request because the request
// contains invalid selectors for the target resource.
//
...
//
// The WS-Management service cannot process the request because the request
// contains an invalid URI for the target resource.
//
...
//
// The WS-Management service cannot process the request because the request
// contains an invalid target system.
//
...
// MessageText:
//
// The WS-Management service cannot process a Create request because the target
// already exists.
//
...
// MessageText:
//
// The WS-Management service does not support the value in the configuration for
// MaxEnvelopeSize.
//
...
// MessageText:
//
// The service returned a response that indicates that the method is
// unsupported.
//
...
// MessageText:
//
// The WS-Management service failed to locate the component that can process the
// request.
//
...
// MessageText:
//
// The MAC that is configured is not in the list of enabled DHCP adapters on the
// computer.
//
...
// MessageText:
//
// The WS-Management service cannot find the dynamic IP address on the adapter
...
// MessageText:
//
// The WS-Management service cannot process the request because the envelope
// size in the request is too small.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The EndPointReference
// contains more nested EndPointReferences than WS-Management supports.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the timeout
...
// MessageText:
//
// The WS-Management service cannot process the request. The data source failed
// to return results for the request.
//
...
// MessageText:
//
// The WS-Management service cannot guarantee that all data is returned in the
// requested locale as some data sources may not be able to comply. Resend the
// remote request with locale as a hint (the SOAP header should have
...
// MessageText:
//
// The listener address specified is invalid. The address can be specified in
// one of the following formats: *, IP:<ip_address>, MAC:<mac_address>. Change
// the listener address and try the request again.
//
...
// MessageText:
//
// The client is attempting to concurrently receive events from a single
// subscription session.This is not supported.
//
...
// MessageText:
//
// The source is sending events in a connection that did not match the security
...
// MessageText:
//
// The WS-Management client cannot process the request. The event source
// identity does not match the identity of the machine that the client
// subscribed to.
//
...
// MessageText:
//
// The client could not start a valid listener to receive subscription events
// based on the specified input settings.
//
...
// MessageText:
//
// Cannot execute the Fragment-Level operation. The fragment path cannot be
// missing if the fragment dialect is specified.
//
...
// MessageText:
//
// Cannot execute the Fragment-Level operation because of invalid value for the
// fragment dialect.
//
...
// MessageText:
//
// Cannot execute the Fragment-Level operation because the fragment path is
// invalid. Check the syntax of the fragment path string. Also check the
// spelling and the case of the property names in the fragment path string: they
// have to match the spelling and the case of the resource properties.
//
...
// MessageText:
//
// The specified batch parameter is incompatible with the specified event
// delivery mode. This can happen if batchSettings for a specific mode are
...
//
#define ERROR_WSMAN_EVENTING_INCOMPATIBLE_BATCHPARAMS_AND_DELIVERYMODE         \
  0x80338094L

// wsman, code=Sender, subcode=WS-Eventing EventSourceUnableToProcess, details=
// /UnusableAddress
//
...
// MessageText:
//
// The connectivity test from the push subscription source to the client failed.
// This can happen if the client machine initiating the push subscription is
// unreachable from the server machine where the event source is located.
// Possible reasons include firewall or some other network boundary. Modify
// subscription to use Pull based subscription.
//
...
// MessageText:
//
// The subscribe packet had an EndTo element address that does not match the
// NotifyTo element address or it was invalid. For subscription the EndTo
// element need not be present in the subscription request. If it exists then
// it's address should match the address specified in NotifyTo element.
//
...
// MessageText:
//
// The event source sent an event packet whose header could not be processed by
// the client. This can happen if it was malformed or if the header had a
// mustUnderstand attribute that could not be understood by the client.
//
...
// MessageText:
//
// An operation is being attempted on a session that is being closed.This can
// happen if the session that is being used is also being closed by another
// thread.
//
...
// MessageText:
//
// The listener on which the subscription session was established is no longer
// valid. This can happen if the WSMAN service listener configuration has been
// changed and a subscription was already active and using one of the
// configurations that was deleted.
//
...
// MessageText:
//
// The WS-Management service on the remote machine with which this subscription
// had been set up has requested that the subscription be closed. This can
// happen if the WS-Management service on the remote machine was being shutdown.
// To correct this problem restart the WS-Management service on the remote
// machine and re-create the subscription.
//
...
// MessageText:
//
// The event source was unable to deliver events to the client.This can happen
// due to network issues preventing the source from connecting to the client.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. This request is valid
// only when the -remote option is specified.
//
#define ERROR_WSMAN_MACHINE_OPTION_REQUIRED 0x803380A5L

// wsman, code=Sender, subcode=WS-Management UnsupportedFeature,
// details=/OptionSet
//
...
// MessageText:
//
// The WS-Management service does not support the options feature for the
// specified resource. Remove the options from the request and retry.
//
...
// MessageText:
//
// The subscribe packet contains batch size value which is smaller than
// supported value.
//
...
// MessageText:
//
// The WS-Management service cannot process the subscribe request. The delivery
// mode is either invalid or missing.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The provider method was
// not found.
//
...
// MessageText:
//
// The WinRM client could not create a push subscription because there are no
// listeners configured that match the specified hostname and transport, or
// because there is no enabled firewall exception on the port used by the
// selected listener. Change the hostname and transport, create an appropriate
// firewall exception, or run winrm quickconfig.
//
...
// specified along with the 'no authentication' flag. No user name, password or
// client certificate should be specified with the 'no authentication' option.
//
...
// MessageText:
//
// The WinRM client cannot process the request. An invalid flag was specified
// for this request. Remove or change the invalid flag and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request must specify only
// one authentication mechanism. If the No Authentication flag is set, no
// authentication mechanism should be specified. Change the request to specify
// only one authentication mechanism or 'no authentication' and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The SPN Server Port can only be
// used when the authentication mechanism is Negotiate or Kerberos. Remove the
// SPN Server Port or change the authentication mechanism and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request must not include
// credentials when using a smart card or default certificate. Remove the
// credentials or change the authentication mechanism and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Requests must include user name
// and password when Basic or Digest authentication mechanism is used. Add the
// user name and password or change the authentication mechanism and try the
// request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Requests must not include user
// name and password when a certificate is used for authentication. Remove the
// user name and password or change the authentication mechanism and try the
// request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Requests must include
// credentials if they specify the following flag:
// WSManFlagCredUsernamePassword. Add the credentials or remove the
// WSManFlagCredUsernamePassword flag and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Requests with credentials must
// include the following flag: WSManFlagCredUsernamePassword. Add the flag or
...
// MessageText:
//
// The WinRM client cannot process the request. Requests must include the
// certificate thumbprint when a certificate is used for authentication. Change
// the request to include the certificate thumbprint and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Requests must include the type
// of certificate to use for authentication. Change the request to include the
...
//
// The WinRM client cannot process the request. Requests must include the
// location (machine or user certificate store) of the certificate used for
// authentication. Change the request to include the location of the certificate
// and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The certificate structure was
// incomplete. Change the certificate structure and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Credentials must not be provided
// for local requests. Remove the credentials and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Connection options must not be
// provided for local requests. Remove the connection options and try the
// request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManCreateSession function is null or zero. Change the request to
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManEnumerate function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManSubscribe function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The parameter that should
// contain the result of the request is null. Change the request to include the
...
// MessageText:
//
// The WinRM client cannot process the request. The request is missing the
// session or enumeration handle. Change the request to include the missing
...
// MessageText:
//
// The WinRM client cannot process the request. The resource URI must not be ""
// (blank or empty string) or NULL. Change the resource URI and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The resource locator was
// invalid. Change the resource locator and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The input XML must not be ""
// (blank or empty string) or NULL. Change the input XML and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The maximum number of elements
// to be retrieved in a batch is too small. Change the value for the maximum
// number of elements in a batch and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The maximum number of characters
// to be retrieved in a batch is too small. Change the value for the maximum
// number of characters in a batch and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The action URI must not be ""
// (blank or empty string) or NULL. Change the action URI and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The heartbeat interval must be
// greater than 0. Change the heartbeat interval and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request must contain one and
// only one delivery mode. Change the request to contain only one delivery mode
// and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request contained multiple
// settings for the policy regarding the maximum envelope size. Change the
// request to contain only one setting for the policy and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request contained an
// expiration time, but did not specify if it was absolute or relative. Change
// the request to specify the type of the expiration time (absolute or relative)
// and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request specified the type
// of the expiration time (absolute or relative) but it did not contain an
// expiration time. Change the request to include the expiration time and try
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The pull subscription request
// contained flags related to a push subscription. Change the flags and try the
// request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the push subscription
// request contained an unsupported delivery transport. HTTP and HTTPS are the
// only currently supported transports. Change the delivery transport and try
// the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The delivery address for push
// subscriptions was too long. Change the delivery address and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The request contained the
// compression option but contained an unrecognized value. Change the value for
// the compression option and try the request again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManDeliverEndSubscriptionNotification function is null or zero.
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManDeliverEvents function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManGetBookmark function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManDecodeObject function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManEncodeObject(Ex) function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManEnumeratorNextObject function is null or zero. Change the
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManConstructError function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRM service cannot process the request. Push subscriptions are not
// supported for local session. Change subscription type to Pull and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The unencrypted flag only
// applies to the HTTP transport. Remove the unencrypted flag or change the
// transport and try again the request.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Certificate parameters are not
// valid when the HTTP transport is also specified. Remove the certificate
// parameters or change the transport and try again the request.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The connection string should be
// of the form [<transport>://]<host>[:<port>][/<suffix>] where transport is one
// of "http" or "https". Transport, port and suffix are optional. The host may
// be a hostname or an IP address. For IPv6 addresses, enclose the address in
// brackets - e.g. "http://[1::2]:80/wsman". Change the connection string and
...
// MessageText:
//
// The WinRM client cannot process the request. The connection string contains
// an unsupported transport. Valid transports are "http" or "https". Change the
// connection string and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the port specified in the
// connection string is not valid. Verify the port and retry the request. Valid
// values are between 1 and 65535. Change the value for port and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The port specified in the
// configuration is invalid. Valid values are between 1 and 65535. Change the
// value for port and try the request again.
//
...
// MessageText:
//
// The WinRM service cannot process the request. WSMAN_FLAG_SEND_HEARTBEAT flag
// requires the event enumerator to be empty. Change the flag or change the
// event enumerator and try the request again.
//
...
// disabled in the client configuration. Change the client configuration and try
// the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Basic authentication is
...
// MessageText:
//
// The WinRM client cannot process the request. Digest authentication is
// currently disabled in the client configuration. Change the client
// configuration and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Negotiate authentication is
// currently disabled in the client configuration. Change the client
// configuration and try the request again. If this is a request for the local
// configuration, use one of the enabled authentication mechanisms still
// enabled. To use Kerberos, specify the local computer name as the remote
// destination. To use Basic, specify the local computer name as the remote
// destination, specify Basic authentication and provide user name and password.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Kerberos authentication is
// currently disabled in the client configuration. Change the client
// configuration and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Certificate authentication is
// currently disabled in the client configuration. Change the client
// configuration and try the request again.
//
...
// Default credentials with Negotiate over HTTP can be used only if the target
// machine is part of the TrustedHosts list or the Allow implicit credentials
// for Negotiate option is specified.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The CertificateThumbprint
// property must be empty when the SSL configuration will be shared with another
// service.
//
...
//
// The WinRM client cannot process the request. The CertificateThumbprint
// property must not be "" (blank or empty string) or NULL.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The WinRM client tried to create
// an SSL configuration for a pair of IP address and port according to the
// request, but the SSL configuration for that pair is owned by another service
// and cannot be shared. Use a different IP address and port combination and try
// the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The certificate CN and the
// hostname that were provided do not match.
//
...
// MessageText:
//
// The WinRM client cannot process the request. When HTTP is the transport, the
// Certificate thumbprint must be blank. HTTP does not use the Certificate
// thumbprint.
//
...
// Ranges are specified using the syntax IP1-IP2. Multiple ranges are separated
// using , as delimiter.
// * is used to indicate that the service should listen on all available IPs on
// the machine. When * is used, other ranges in the filter are ignored. If
// filter is blank, the service doesn't listen on any address. For example, if
// service should be restricted to listen on only IPv4 addresses, IPv6 filter
// should be left empty. %nExample IPv4 filters:
...
// MessageText:
//
// The WinRM client cannot process the request. The input XML modifies selectors
// or keys for the instance. You cannot create a new instance or change the
// identity of an instance by changing the keys. Change the input XML and try
// the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The Enhanced Key Usage (EKU)
// field of the certificate is not set to "Server Authentication". Retry the
// request with a certificate that has the correct EKU.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The response from the
// destination computer does not include any results.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The response to a create request
// did not contain a valid end point reference. The ResourceCreated element was
// not found or did not contain valid content.
//
...
//
// The WinRM client cannot process the request. The response from the
// destination computer does not contain a valid SOAP enumeration context.
//
...
//
// The WinRM client cannot process the request. The response from the
// destination computer contains a WS-Management FragmentTransfer header but the
// content of the body is not wrapped by the WS-Management XmlFragment wrapper.
//
...
//
// The WinRM client cannot process the request. The response from the
// destination computer contains one or more invalid SOAP headers.
//
...
// MessageText:
//
// The WinRM client cannot process the request. It cannot find any SOAP Headers
// or Body elements in the response from the destination computer.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The destination computer
...
//
// The WinRM client cannot process the request. The destination computer
// returned an invalid SOAP fault.
//
...
// type of the HTTP response from the destination computer. The content type is
// absent or invalid.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The HTTP response from the
// destination computer was not in the same format as the request. A Unicode
// request packet may have been sent and an ANSI packet received.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The encrypted message body has
// an invalid format and cannot be decrypted. Ensure that the service is
// encrypting the message body according to the specifications.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The resource URI is not valid:
// it does not contain keys, but the class selected is not a singleton. To
// access an instance which is not a singleton, keys must be provided. Use the
// following command to get more information in how to construct a resource URI:
// "winrm help uris".
//
...
// MessageText:
//
// The WinRM client cannot process the request. The resource URI for an
// enumeration operation with WQL filter must not contain keys and the class
// name must be '*' (star). Use the following command to get more information in
...
// MessageText:
//
// The WS-Management identification operation is only available on remote
// sessions.
//
...
// MessageText:
//
// Subscribe operation with Push delivery mode is only available on remote
// sessions.
//
...
// MessageText:
//
// The subscription manager address is invalid. The response was not received
// from the address to which the subscription request was sent.
//
...
// MessageText:
//
// WinRM cannot process the request because the WMI object contains too many
// levels of nested embedded objects.
//
...
// It does not support retrieving a WMI object that contains a property of type
// CIM_REFERENCE and the value of that property contains a remote machine name.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The WMI service
// reported that the WMI provider could not perform the requested operation.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. A value retrieved from
// the WMI service or the WMI provider is invalid.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The WMI provider
// returned an 'access denied' error.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. An 'access denied'
// error was received when connecting to the WMI service on the computer
// specified.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the filter XML
// is invalid.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The resource URI for an
// Enumerate operation must not contain keys.
//
...
// MessageText:
//
// Cannot execute the Fragment-Level operation because the fragment path
// contains either "" (blank or empty string) or NULL. Change the value of the
// fragment path and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The response received from the
// destination machine contains invalid characters and cannot be processed.
//
...
// Specify a DNS or NetBIOS destination or specify Basic or Negotiate
// authentication.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Kerberos authentication cannot
// be used with implicit credentials if the client computer is not joined to a
// domain. Use explicit credentials or specify a different authentication
// mechanism than Kerberos.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The batch settings parameter is
// invalid.
//
...
// MessageText:
//
// The WinRM client cannot process the request. If you do not specify an
// authentication mechanism or you specify Kerberos, then you cannot use
// "localhost" or "127.0.0.1" or "[::1]" for the remote host name. You can
// explicitly specify a different authentication mechanism than Kerberos or
// specify the remote host as a DNS name or NetBIOS name.
//
...
// MessageText:
//
// The WinRM client received an unknown HTTP status code from the remote
...
// MessageText:
//
// The WinRM client received a HTTP redirect status code from the remote
// WS-Management service. WinRM does not support redirects.
//
...
// MessageText:
//
// The WinRM client sent a request to the remote WS-Management service and was
// notified that the request size exceeded the configured MaxEnvelopeSize quota.
//
...
// Verify that the WS-Management service is running on the remote host and
...
// the requested HTTP URL was not available. This is usually returned by a HTTP
// server that does not support the WS-Management protocol.
//
...
// MessageText:
//
// The subscribe packet had a Locale element with missing lang attribute. The
// lang attribute is required for the Locale element.
//
...
// appropriate certificate. To be used for SSL, a certificate must have a CN
// matching the hostname, be appropriate for Server Authentication, and not be
// expired, revoked, or self-signed.
//
...
// MessageText:
//
// The Windows Remote Management (WinRM) service cannot be configured for remote
// access because Group Policy does not allow local firewall changes. Check the
// Group Policy settings to allow local firewall exceptions and add WinRM to the
// firewall exceptions.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the selector name is not
// valid. Change the selector name and retry the request.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the selector
// values do not match a known resource, or the resource is offline. Retry the
// request later when the resource is online, or try a different selector.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the a header in
...
// MessageText:
//
// The expiration time specified for enumeration was invalid. Specify the
// expiration time as a duration.
//
...
// MessageText:
//
// The WS-Management service received a request which specified a maximum number
// of elements, but the service does not support this feature. Retry the request
...
//
// The WinRM client cannot process the request. The request must contain one and
// only one enumeration mode. Change the request to contain only one enumeration
// mode and try again.
//
...
// MessageText:
//
// The WinRS client cannot process the request. An invalid flag was specified
// for this request. Remove or change the invalid flag and try the request
// again.
//
...
// MessageText:
//
// The WinRS client cannot process the request. One of the parameters required
// is null or zero. Change the request to include the missing parameter and try
// again.
//
...
// MessageText:
//
// The data source could not process the filter. The filter might be missing,
// invalid or too complex to process. If a service only supports a subset of a
// filter dialect (such as XPath level 1), it may return this fault for valid
...
// MessageText:
//
// The WinRM client cannot process the request. The object parameter for the
// WSManEnumeratorAddObject function is null or zero, but the enumeration mode
// is Object or ObjectAndEPR.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The EPR parameter for the
// WSManEnumeratorAddObject function is null or zero, but the enumeration mode
// is EPR or ObjectAndEPR.
//
...
// MessageText:
//
// The WinRS client cannot process the Receive request because the shell plugin
// returned an empty response to the request.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManCreateShell function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WinrsCloseShell function is null or zero. Change the request
// to include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRS client cannot process the request. The parameter required for the
// WinrsFreeCreateShellResult function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManRunShellCommand function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRS client cannot process the request. The parameter required for the
// WinrsFreeRunCommandResult function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManSignalShell function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSMansReceiveShellOutput function is null or zero. Change
...
//
// The WinRS client cannot process the request. The parameter required for the
// WinrsFreePullResult function is null or zero. Change the request to include
// the missing parameter and try again.
//
...
//
// The WinRS client cannot process the request. One of the parameters required
// for the WinrsPull function is null or zero. Change the request to include the
// missing parameter and try again.
//
...
//
// The WinRS client cannot process the request. The parameter required for the
// WinrsCloseReceiveHandle function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManSendShellInput function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRS client cannot process the request. One of the parameters required
// for the WinrsPush function is null or zero. Change the request to include the
// missing parameter and try again.
//
...
//
// The WinRS client cannot process the request. The parameter required for the
// WinrsCloseSendHandle function is null or zero. Change the request to include
// the missing parameter and try again.
//
...
//
// The WinRS client cannot process the request. One of the parameters required
// for the WinrsGet function is null or zero. Change the request to include the
// missing parameter and try again.
//
...
// MessageText:
//
// The WS-Management service does not support the specified polymorphism mode.
...
// MessageText:
//
// The WS-Management service cannot process the request because the specified
// URI is not supported on the service side. Retry the request with local
// session.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. A DMTF resource URI was
// used to access a non-DMTF class. Try again using a non-DMTF resource URI.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The DMTF class in the
// repository uses a different major version number from the requested class.
...
// MessageText:
//
// The WS-Management service cannot process the request. The resource URI and
// __cimnamespace selector attempted to use different namespaces. Try removing
// the __cimnamespace selector or using a DMTF resource URI. If a non-DMTF
// resource URI is used with a __cimnamespace selector, the namespaces must
// match.
//
...
// MessageText:
//
// The WS-Management client cannot process the request. To use the
// WSManSubscribe API the user has to be running under Network Service account.
// No other account is supported currently for push subscriptions.
//
...
// MessageText:
//
// The WS-Management client cannot process the request. The event source machine
// is not joined to a domain. To set up a push subscription session to an event
// source the source has to be connected to a domain. To fix this problem either
// join the event source machine to a domain or use PULL as the delivery mode
...
// MessageText:
//
// The WS-Management client cannot process the request. The subscriber machine
// is not joined to a domain. To set up a push subscription session to an event
// source, the subscriber machine has to be connected to a domain. To fix this
// problem either join the subscriber machine to a domain or use PULL as the
...
// MessageText:
//
// The WinRM client cannot process the request because it is trying to update a
// read-only setting. Remove this setting from the command and try again.
//
...
// You may want to use the CHCP command to change the client Code Page to 437
// and receive the results in English.
//
...
// Not used. To be removed.
//
#define ERROR_WSMAN_CLIENT_DISABLE_LOOPBACK_WITH_EXPLICIT_CREDENTIALS          \
  0x80338141L
...
// The server implementation should never return more items than are specified
// by the client.
//
...
// MessageText:
//
// The WinRM client cannot process the request. A certificate thumbprint was
// specified together with a user name or password. Only one credentials type
// can be specified. Remove the credentials type that does not correspond to the
// intended authentication mechanism and retry the request.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The flag that specifies the
// authentication mechanism to use is incorrect. Remove or change the invalid
// flag and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. When an authentication mechanism
// is not specified, only user name and password credentials are allowed. If you
// want to use a different type of credentials then you need to specify the
// authentication mechanism. Specify the authentication mechanism or the correct
// credentials and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. For authentication mechanisms
// that require the credentials of an user account, both user name and password
// must be specified. Specify the missing user name or password and try the
// request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. If you are using a machine
// certificate, it must contain a DNS name in the Subject Alternative Name
// extension or in the Subject Name field, and no UPN name. If you are using a
// user certificate, the Subject Alternative Name extension must contain a UPN
// name and must not contain a DNS name. Change the certificate structure and
// try the request again.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. One of the environment
// variable name passed to the WSManCreateShell function is null or zero. Change
// the request to include the missing parameter and try again.
//
...
// MessageText:
//
// An operation is being attempted on a shell that is being closed. This can
// happen if the shell that is being used is also being closed by another
// thread.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. One of the stream id name
// passed to the WSManCreateShell function is null or zero. Change the request
// to include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. The shell handle passed to
// the WSMan Shell function is not valid. The shell handle is valid only when
// WSManCreateShell function completes successfully. Change the request
// including a valid shell handle and try again.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. The command handle passed
// to the WSMan Shell function is not valid. The command handle is valid only
// when WSManRunShellCommand function completes successfully. Change the request
// including a valid shell handle and try again.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. One of the argument value
// passed to the WSManRunShellCommand function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
// MessageText:
//
// An operation is being attempted on a command that is being closed. This can
// happen if the command handle that is being used is also being freed by
// another thread.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. The stream id index from
// within WSMAN_STREAM_ELEMENT passed to the WSManSendShellInput function is
// invalid. The stream id index should be an index from within inputStreamSet
// array passed to the WSManCreateShell function. Change the request with a
// valid index and try again.
//
...
// MessageText:
//
// The WS-Management operations to update the certificate mapping store of the
// WINRM service config can only be done remotely.
//
...
// MessageText:
//
// The WINRM certificate mapping configuration store has reached an internal
// limit and cannot create any more entries. Remove some entries and try again.
//
...
// MessageText:
//
// The WINRM certificate mapping configuration operation cannot be completed
// because the user credentials could not be verified. Please check the username
// and password used for mapping this certificate and verify that it is a
...
//
// The WinRM client cannot process the request. The Enhanced Key Usage (EKU)
// field of the certificate is not set to "Client Authentication". Retry the
// request with a certificate that has the correct EKU.
//
...
//
// The WinRM client cannot process the request. A certificate thumbprint was
// specified, but the following flag is missing: WSManFlagUseClientCertificate.
...
// MessageText:
//
// The WinRM client cannot process the request. The following flags cannot be
// specified together: WSManFlagUseClientCertificate and
// WSManFlagCredUsernamePassword. Remove one of the flags and try the request
// again.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the CustomRemoteShell URI
// specified is invalid. CustomRemoteShell URI should start with WinRM shell
// resource URI prefix:
// "http://schemas.microsoft.com/wbem/wsman/1/windows/shell". The URI should not
// contain invalid characters including '*', '?', white spaces and tabs. The
// CustomRemoteShell URI cannot be longer than 1023 characters.
//
...
//
// The WinRM client cannot process the request because the CustomRemoteShell URI
// specified is invalid. Windows command shell URI
// ("http://schemas.microsoft.com/wbem/wsman/1/windows/shell/cmd") cannot be a
// CustomRemoteShell URI.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the process path
// specified for the CustomRemoteShell table entry is invalid. The process path
// should be absolute and should point to an existing executable.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the provided security
...
// MessageText:
//
// The WinRM service cannot process the request because the WS-Policy contained
// in the DeliverTo is too complex or uses a structure not understood by the
// service. The WinRM service supports a single layer of policy assertions
// underneath a wsp:ExactlyOne element.
//
...
//
// The WinRM service cannot process the request because the WS-Policy contained
// in the DeliverTo does not contain any options that the service can comply
// with. The WinRM service supports the following profiles: Negotiate or
// Kerberos over HTTP, Negotiate or Kerberos over HTTPS, and mutual certificate
// authentication over HTTPS using issuer thumbprints.
//
...
// MessageText:
//
// The WinRM service cannot process the request because the
// wsman:ConnectionRetry element in the DeliverTo is invalid.
//
...
// It must contain at least one character.
// It must contain at most one "*" character which should be the first
// character. (This may be the only character in which case it matches all
// subjects). The Subject cannot be longer than 1023 characters.
//
...
// MessageText:
//
// WinRM cannot make the configuration change because the Issuer used for the
// certificate mapping operation is not valid. The certificate identified by the
// issuer thumbprint must be present in the machine "Trusted Root Certification
// Authorities" or "Intermediate Certification Authorities" store. The
// certificate must have key usage that allows it to sign other certificates.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the type field in the
// WSMAN_ALLOWED_PUBLISHERS argument is invalid. Collector-initiated
// subscriptions must use WSMAN_SINGLE_PUBLISHER and Source-initiated
// subscriptions must use WSMAN_MULTIPLE_PUBLISHERS.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the delivery retry
// parameters are invalid. If delivery retry is requested, the
// deliveryRetryInterval and deliveryRetryAttempts fields must both be nonzero.
//
...
// MessageText:
//
// The WinRM client cannot process the request because client certificate
// subject filters were specified without any issuers. When using client
// certificate authentication, specify at least one issuer thumbprint.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the subscription contains
// no domain or non-domain sources. Subscriptions using
// WSMAN_MULTIPLE_PUBLISHERS must specify either a security descriptor or an
// issuer list or both.
//
...
// MessageText:
//
// The WinRM service cannot process the request because the subscription manager
// returned invalid enumeration results. The m:Subscription XML object or
...
// types on this machine is set to Public. Change the network connection type to
...
//
// WinRM cannot make the configuration change.
// The Password used for updating the certificate mapping configuration is not
// valid. It cannot be longer than 255 characters.
//
...
// WinRM cannot make the configuration change.
// The Password used for updating the certificate mapping configuration is not
// valid. A user account used for configuring a certificate mapping cannot have
// a blank password.
//
...
//
// WinRM cannot make the configuration change.
// The credential used for updating or creating the certificate mapping
// configuration is not valid. The credential consists of both Password and
// UserName being supplied together in a pair.
//
...
// MessageText:
//
// The WinRM service executed an operation and the provider returned
// inconclusive information regarding success or failure of the operation. The
// status was marked as failed, but no error code was given.
//
...
// MessageText:
//
// The WS-Management service on the remote machine cannot process the shell
// request. This can happen if the WS-Management service on the remote machine
// was being shutdown. To correct this problem restart the WS-Management service
// on the remote machine and re-send the shell request.
//
...
// The URI parameter is the key to CustomRemoteShell table and cannot be
// modified.
//
...
// The selector keys of Subject, URI or Issuer cannot be changed by overriding
// the selector key value in the body.
//
...
// MessageText:
//
// The WinRM client cannot process the request because it received an HTML error
// packet.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManInitialize function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManInitialize API call. Remove or change the invalid flag and try
// the call again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManDeinitialize API call. Remove or change the invalid flag and try
// the call again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManSetSessionOption function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManSetSessionOption function is invalid. Change the invalid
// parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// to get a session option is invalid. Change the invalid parameter and try
// again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManCreateShell API call. Remove or change the invalid flag and try
// the call again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManCloseShell API call. Remove or change the invalid flag and try
// the call again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManCloseCommand API call. Remove or change the invalid flag and try
// the call again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManCloseShell function is null or zero. Change the request
// to include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManCloseCommand function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManRunShellCommand API call. Remove or change the invalid flag and
// try the call again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. You must wait for the
// WSManRunShellCommand API call to complete before calling
...
// MessageText:
//
// The WinRM client cannot process the request. The response to a Command
// request did not contain a valid CommandResponse element. The CommandResponse
...
// MessageText:
//
// The WinRM client cannot process the request. The OptionSet element is
// invalid. Change the request to include a valid OptionSet element and try
// again.
//
...
//
// The WinRM client cannot process the request. The response to a Command
// request did not contain a valid CommandResponse element. The CommandId
// element was not found or did not contain valid content.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManSignalShell API call. Remove or change the invalid flag and try
// the call again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManSendShellInput API call. Remove or change the invalid flag and
// try the call again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. An invalid parameter was
// specified for the WSManSendShellInput API call. streamData parameter should
// be specified in binary format using WSMAN_DATA_TYPE_BINARY type. Change the
// invalid parameter and try the call again.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. The stream name passed to
// the WSManSendShellInput function is not valid. The input stream name should
// be specified as part of the input streams during shell creation using
// WSManCreateShell function. Change the request including a valid input stream
// name and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManReceiveShellOutput function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request. The stream or list of
// streams passed to the WSManReceiveShellOutput function is not valid. The
// desired stream names should be specified as part of the output streams during
// shell creation using WSManCreateShell function. Change the request including
// valid desired streams and try again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManReceiveShellOutput API call. Remove or change the invalid flag
...
// MessageText:
//
// The WinRM client cannot process the request. The response to a Receive
// request did not contain a valid ReceiveResponse element. The ReceiveResponse
// element was not found or did not contain valid content.
//
...
// MessageText:
//
// The file path specified is either not absolute, not in the system32
// directory, or not valid.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The UI language option is
// invalid. Change the UI language and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManGetErrorMessage function is null or zero. Change the request to
// include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The language code parameter is
// invalid. The language code parameter should be either NULL or a valid RFC
// 3066 language code. Change the language code and try the request again.
//
...
//
// The WinRM client cannot process the request. An invalid flag was specified
// for the WSManGetErrorMessage API call. Remove or change the invalid flag and
// try the call again.
//
...
// sent to a different machine. Use the redirect information to send the request
// to a new machine.
//
...
//
// The WinRM client cannot process the request. The flag that specifies the
// proxy authentication mechanism to use is incorrect. Remove or change the
// invalid flag and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The credentials for proxy
// authentication are not specified correctly. Both user name and password
// credentials must be valid. Specify the correct credentials and try the
// request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The proxy access type is
// incorrect. Use one of the proxy access type flags; the flags cannot be
// combined. Change the invalid proxy access type and try the request again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The direct connection to the
// server option cannot be used with non empty proxy authentication data. Change
// the invalid proxy access type or use empty proxy authentication data and try
// the request again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManGetSessionOptionAsDword function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManGetSessionOptionAsDword function is invalid. Change the invalid
// parameter and try again.
//
...
//
// The WinRM client cannot process the request. One of the parameters required
// for the WSManGetSessionOptionAsString function is invalid. Change the invalid
// parameter and try again.
//
...
//
// The WinRM client cannot process the request. Requests must include user name
// and password when CredSSP authentication mechanism is used. Add the user name
// and password or change the authentication mechanism and try the request
// again.
//
...
// configured with an SPN appropriate for the target computer. For example, for
// a target computer name "myserver.domain.com", the SPN can be one of the
...
// and look at the following policy: Computer Configuration -> Administrative
// Templates -> System -> Credentials Delegation -> Allow Delegating Fresh
// Credentials. Verify that it is enabled and configured with an SPN appropriate
...
//
// The WinRM client cannot process the request. A computer policy does not allow
// the delegation of the user credentials to the target computer because the
// computer is not trusted. The identity of the target computer can be verified
// if you configure the WSMAN service to use a valid certificate using the
// following command: winrm set winrm/config/service
// @{CertificateThumbprint="<thumbprint>"} Or you can check the Event Viewer for
// an event that specifies that the following SPN could not be created:
// WSMAN/<computerFQDN>. If you find this event, you can manually create the SPN
// using setspn.exe . If the SPN exists, but CredSSP cannot use Kerberos to
// validate the identity of the target computer and you still want to allow the
// delegation of the user credentials to the target computer, use gpedit.msc and
// look at the following policy: Computer Configuration -> Administrative
// Templates -> System -> Credentials Delegation -> Allow Fresh Credentials with
// NTLM-only Server Authentication. Verify that it is enabled and configured
// with an SPN appropriate for the target computer. For example, for a target
// computer name "myserver.domain.com", the SPN can be one of the following:
// WSMAN/myserver.domain.com or WSMAN/*.domain.com. Try the request again after
// these changes.
//
...
// MessageText:
//
// The WS-Management service cannot complete the authorization under the given
// token. A previous authorization attempt for the same user resulted in a
// different token. The user record will be revoked and the next request will
// reauthorize.
//
...
// MessageText:
//
// An application tried to retrieve the HTTP Redirect location from the session
// when no redirect error (ERROR_WSMAN_REDIRECT_REQUESTED) was returned. The
// application needs to be updated so as to only retrieve the location after
...
// MessageText:
//
// The WS-Management service cannot complete the Pull operation for the
// enumeration because the wsman:MaxEnvelopeSize, wsen:MaxCharacters or
// wsen:MaxElements parameters differ from those specified to the enumeration.
// The application needs to specify the same parameters for Pull as were
// specified for the enumeration.
//
...
// MessageText:
//
// The WinRM service cannot process the request because it is trying to update a
// deprecated setting. Remove this setting from the command and try again.
//
...
// A Security element contains a URI that does not match its parent Resource
// element.
//
...
// MessageText:
//
// The WinRM client cannot process the request. Allow implicit credentials for
// Negotiate authentication option is only valid for HTTPS transport. Remove the
// allow implicit credentials for Negotiate authentication option and try the
// request again.
//
#define ERROR_WSMAN_CANNOT_USE_ALLOW_NEGOTIATE_IMPLICIT_CREDENTIALS_FOR_HTTP   \
  0x803381B0L
...
// MessageText:
//
// The WinRM client cannot process the request. Setting proxy information is not
// valid when the HTTP transport is specified. Remove the proxy information or
// change the transport and try the request again.
//
...
//
// The WinRM client cannot process the request. Setting proxy information is not
// valid when the authentication mechanism with the remote machine is Kerberos.
// Remove the proxy information or change the authentication mechanism and try
// the request again.
//
...
//
// The WinRM client cannot process the request. Setting proxy information is not
// valid when the authentication mechanism with the remote machine is CredSSP.
// Remove the proxy information or change the authentication mechanism and try
// the request again.
//
...
//
// The WinRM client cannot process the request. The request must specify only
// one authentication mechanism for proxy. Change the request to specify only
// one authentication mechanism and try again.
//
...
// MessageText:
//
// The WinRM client received a redirect error from the server when it is not
// appropriate. The only time a redirect error can be reported correctly is
// during the authorization of a user. This would result in a properly formatted
// redirect response from the server that includes the redirect endpoint.
//
...
// MessageText:
//
// The WinRM service received a redirect error from an authorization plug-in
// where the redirect location was too long.
//
...
// MessageText:
//
// The WinRM service received a HTTP redirect message redirecting the client but
...
// MessageText:
//
// The WinRM service cannot process the request. The Channel Binding Token
// Hardening Level (CbtHardeningLevel) value is invalid. The valid values are
// "None", "Relaxed" and "Strict". Change the CbtHardeningLevel value and try
// again.
//
...
// Verify that the service on the remote host is properly configured to listen
// for HTTPS requests. Consult the logs and documentation for the WS-Management
...
// MessageText:
//
// The WinRM client used a parameter to specify the use of SSL while specifying
// http in the connection string.
//
...
// MessageText:
//
// The WinRM service cannot process the request because the security for this
// resource URI cannot be changed.
//
...
// MessageText:
//
// The WinRM service cannot process the request. The enumeration request expects
// a selector based filter to specify the shell identifier.
//
...
// MessageText:
//
// The WinRM service cannot process the request. The enumeration of end point
// resources for shell commands is not supported.
//
...
// MessageText:
//
// The WinRM Shell client cannot process the request because the shell name has
...
// MessageText:
//
// The WinRM runAs configuration operation cannot be completed because the user
// credentials could not be verified. Verify that the username and password used
// for configuration are valid and retry the operation.
//
...
// MessageText:
//
// The WinRM service cannot process the request because the WinRS shell instance
...
// MessageText:
//
// The WinRM service cannot process the request. This WinRS shell instance does
// not support disconnect and reconnect operations because it was created by an
// older WinRS client or its provider does not support the disconnect operation.
//
...
//
// The WinRM service cannot process the request because the WinRS shell instance
// is connected to a different client.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManDisconnectShell function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManReconnectShell function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManConnectShell function is null or zero. Change the
// request to include the missing parameter and try again.
//
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManConnectShellCommand function is null or zero. Change
// the request to include the missing parameter and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request. The body response is not a valid
...
// MessageText:
//
// The WinRM Shell client cannot process the request. The command is currently
// terminating or was terminated.
//
...
// MessageText:
//
// The WinRM service cannot process the request. The WinRS shell instance is
// currently connected to a different client.
//
...
// MessageText:
//
// The WinRM client encountered an error while communicating with the WinRM
// service during the disconnect operation. The shell has been disconnected and
// the streams were possibly suspended abruptly.
//
...
// MessageText:
//
// The WinRM client cannot process the request. A disconnect operation cannot be
// performed on a WinRS shell instance that is already disconnected.
//
...
// MessageText:
//
// The WinRM client cannot process the request. A reconnect operation cannot be
// performed on a WinRS shell instance that is currently connected.
//
...
// MessageText:
//
// An error was encountered while subscribing to the Group Policy change
// notification.
//
#define ERROR_WSMAN_CONFIG_GROUP_POLICY_CHANGE_NOTIFICATION_SUBSCRIPTION_FAILED \
  0x803381D1L
...
//
// The WinRM Shell client cannot process the request. One of the parameters
// required for the WSManReconnectShellCommand function is null or zero. Change
// the request to include the missing parameter and try again.
//
...
//
// The WinRM client cannot process the request. A reconnect operation cannot be
// performed on a WinRS shell command instance that is currently connected.
//
...
// MessageText:
//
// The WinRM service cannot process the request because the command ID specified
// by the client is not a valid GUID. Modify the request and retry the request.
//
...
// MessageText:
//
// The WinRM service cannot process the request because the shell ID specified
// by the client is not a valid GUID. Provide a valid ID and try again.
//
...
// MessageText:
//
// The WinRM service cannot process the request. A command already exists with
// the command ID specified by the client.
//
...
// MessageText:
//
// The WinRM service cannot process the request. A resource already exists with
// the shell ID specified by the client.
//
...
//
// The WinRM client cannot process the request. A disconnect operation cannot be
// performed on a WinRS shell command instance that is disconnected.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. The resource URI for
// the Subscribe operation must not contain keys.
//
...
// MessageText:
//
// The WS-Management service did not receive a response for an extended
// semantics operation within the timeframe specified in the OperationTimeout
// setting.
//
...
// MessageText:
//
// The WS-Management service cannot process the request because the stream is
...
// MessageText:
//
// The creation of a new Shell failed. Verify that the RunAsPassword value is
// correctly configured and that the Group Policy setting "Disallow WinRM from
// storing RunAs credentials" is Disabled or Not Configured. To enable WinRM to
// store RunAs credentials, change this Group Policy setting to Disabled.
//
...
// To enable WinRM to store RunAs credentials, change the "Disallow WinRM from
...
// MessageText:
//
// The WinRM client cannot process the request because the XML instance does not
// match the class schema provided by the server.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the XML contains an
// unsupported type. Verify the XML and retry the operation.
//
...
//
// The WS-Management service cannot process the request.
// The service is configured to reject remote connection requests for this
// plugin.
//
...
// MessageText:
//
// The WS-Management service cannot process the request. This user has exceeded
// the maximum number of concurrent shells allowed for this plugin. Close at
// least one open shell or raise the plugin quota for this user.
//
...
// users executing remote operations has been exceeded for this plugin. Retry
// the request later or raise the quota for concurrent users.
//
...
//
// The WS-Management service cannot process the request. The maximum number of
// concurrent shells allowed for this plugin has been exceeded. Retry the
// request later or raise the Maximum Shells per Plugin quota.
//
...
//
// The WS-Management service cannot process the request. The maximum number of
// concurrent operations allowed for this plugin has been exceeded. Retry the
// request later or raise the Maximum Operations per Plugin quota.
//
...
//
// The WS-Management service cannot process the request. This user has exceeded
// the maximum number of allowed concurrent operations. Retry the request later
// or raise the Maximum Operations per User quota.
//
...
//
// The WS-Management service cannot process the request. The maximum number of
// concurrent commands per shell has been exceeded. Retry the request later or
// raise the Maximum Commands per Shell quota.
//
...
// Retry the operation later or close one or more of the currently running
// operations.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the MI Deserializer
// cannot be created.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the metadata could not be
// deserialized.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the metadata failed to be
...
// MessageText:
//
// The WinRM client cannot process the request because a WinRM session could not
// be created.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the target object has a
// key property set to NULL. Incomplete objects cannot be used as the target of
// an operation.
//
...
// MessageText:
//
// The WinRM client cannot process the request as the server identity could not
// be verified. If the identity of the server is trusted, add it to the
// TrustedHosts list and retry the request. Use winrm.cmd to configure
// TrustedHosts. Note that computers in the TrustedHosts list might not be
// authenticated. For more information on how to set TrustedHosts, run the
// following command: winrm help config
//
...
// MessageText:
//
// The WinRM client cannot process the request because the octet string array
...
// MessageText:
//
// The WS-Management service cannot process the request. The requested
// IdleTimeout is outside the allowed range.
//
...
// MessageText:
//
// The WinRM client cannot process the request because insufficient metadata is
// available. The application does not allow all properties to be returned as
// strings, but the server does not support correctly typing the properties.
// Change the request to allow all properties to be returned as strings and
// retry the request.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the MI_OperationOptions
// contained both a Resource URI and a Resource URI Prefix. Specify only one of
// these two options and try again.
//
...
// MessageText:
//
// The WinRM client cannot process the request because keysOnly and
// WSMAN_ENUMERATIONMODE_OBJECTONLY were specified at the same time. These two
// settings are incompatible. Remove the WSMAN_ENUMERATIONMODE_OBJECTONLY
// option, or set keysOnly to MI_FALSE, and retry the request.
//
...
// MessageText:
//
// The WinRM client cannot process the request because the class name is not
// valid. Supply a valid class name or set the Resource URI option and retry the
// request.
//
...
// MessageText:
//
// The subscribe packet contains an Encoding value that is not valid in the
// delivery section.
//
...
//
// The WinRM client cannot process the request.
// The destination computer name must be a hostname or an IP address, and must
// not be a URL. To use an IPv6 address, enclose the address in brackets, like
// the following: "[::1]". The transport, port number, and URL prefix may be
// controlled by setting the appropriate destination options. Change the
// destination computer name string and retry the operation.
//
...
// MessageText:
//
// The server does not support WS-Management Identify operations. Skip the
// TestConnection part of the request and try again.
//
...
// MessageText:
//
// The WS-Management service cannot process the operation. The operation is
// being attempted on a client session that is unusable. This may be related to
// a recent restart of the WS-Management service. Please create a new client
// session and retry the operation if re-executing the operation does not have
// undesired behavior.
//
...
// MessageText:
//
// The WS-Management service cannot process the operation. An attempt to create
// a virtual account failed. Ensure that WinRM service is running as Local
// System and that it has TCB privilege enabled.
//
...
// MessageText:
//
// The WS-Management service cannot process the operation. Virtual account
// feature is only available in Windows 7, Server 2008 R2 and above.
//
...
// MessageText:
//
// The WS-Management service cannot process the operation. An attempt to logon
// using the configured RunAs Managed Service Account failed.
//
...
// MessageText:
//
// The WS-Management service cannot process the operation. An attempt to query
// mapped credential failed. This will happen if the security context associated
// with WinRM service has changed since the credential was originally mapped.
//
...
// MessageText:
//
// The event source of the push subscription is in disable or inactive on the
// Event controller server.
//
#define ERROR_WSMAN_EVENTING_PUSH_SUBSCRIPTION_NOACTIVATE_EVENTSOURCE          \
  0x803381FFL
