...
//
// LevelFlags : The lower 16 bits describe the use level while the upper 16 bits
// are flags.
//
...
NET_API_STATUS NET_API_FUNCTION NetUseAdd(
    _In_opt_ LPTSTR servername, _In_ DWORD LevelFlags,
    _When_(USE_LEVEL(LevelFlags) == 0, _In_reads_bytes_(sizeof(USE_INFO_0)))
        _When_(USE_LEVEL(LevelFlags) == 1, _In_reads_bytes_(sizeof(USE_INFO_1)))
            _When_(USE_LEVEL(LevelFlags) == 2,
                   _In_reads_bytes_(sizeof(USE_INFO_2)))
                _When_(USE_LEVEL(LevelFlags) == 3,
                       _In_reads_bytes_(sizeof(USE_INFO_3)))
                    _When_(USE_LEVEL(LevelFlags) == 4,
                           _In_reads_bytes_(sizeof(USE_INFO_4)))
                        _When_(USE_LEVEL(LevelFlags) == 5,
                               _In_reads_bytes_(sizeof(USE_INFO_5))) LPBYTE buf,
    _Out_opt_ LPDWORD parm_err);

NET_API_STATUS NET_API_FUNCTION NetUseDel(_In_opt_ LMSTR UncServerName OPTIONAL,
                                          _In_ LMSTR UseName,
                                          _In_ DWORD ForceLevelFlags);

NET_API_STATUS NET_API_FUNCTION NetUseEnum(
    _In_opt_ LMSTR UncServerName, _In_ DWORD LevelFlags,
    _Outptr_opt_result_buffer_(_Inexpressible_(EntriesRead)) LPBYTE *BufPtr,
    _In_ DWORD PreferedMaximumSize, _Out_opt_ LPDWORD EntriesRead,
    _Out_ LPDWORD TotalEntries, _Inout_opt_ LPDWORD ResumeHandle);

NET_API_STATUS NET_API_FUNCTION NetUseGetInfo(
    _In_opt_ LMSTR UncServerName, _In_ LMSTR UseName, _In_ DWORD LevelFlags,
    _When_(USE_LEVEL(LevelFlags) == 0,
           _Outptr_opt_result_bytebuffer_(sizeof(USE_INFO_0)))
        _When_(USE_LEVEL(LevelFlags) == 1,
               _Outptr_opt_result_bytebuffer_(sizeof(USE_INFO_1)))
            _When_(USE_LEVEL(LevelFlags) == 2,
                   _Outptr_opt_result_bytebuffer_(sizeof(USE_INFO_2)))
                _When_(USE_LEVEL(LevelFlags) == 3,
                       _Outptr_opt_result_bytebuffer_(sizeof(USE_INFO_3)))
                    _When_(USE_LEVEL(LevelFlags) == 4,
                           _Outptr_opt_result_bytebuffer_(sizeof(USE_INFO_4)))
                        _When_(USE_LEVEL(LevelFlags) == 5,
                               _Outptr_opt_result_bytebuffer_(
                                   sizeof(USE_INFO_5))) LPBYTE *bufptr);
...
#define USE_DEFAULT_CREDENTIALS                                                \
  0x4 // No explicit credentials passed to NetUseAdd
...
typedef struct _USE_OPTION_TRANSPORT_PARAMETERS {
  ULONG Tag;       // 'ParT'
  USHORT Length;   // sizeof(USE_OPTION_TRANSPORT_PARAMETERS) +
                   // sizeof(TRANSPORT_INFO)
  USHORT Reserved; // 0
...
typedef struct _SMB_USE_OPTION_COMPRESSION_PARAMETERS {
  ...
  USHORT Reserved;
    ...
    *PSMB_USE_OPTION_COMPRESSION_PARAMETERS;
