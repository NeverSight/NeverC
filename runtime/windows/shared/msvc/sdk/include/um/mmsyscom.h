...
#define MAX_JOYSTICKOEMVXDNAME                                                 \
  260 /* max oem vxd name length (including NULL) */
...
typedef _Return_type_success_(
    return == 0) UINT MMRESULT; /* error return code, 0 means no error */
...
#define MMSYSERR_INVALHANDLE                                                   \
  (MMSYSERR_BASE + 5)                         /* device handle is invalid      \
                                               */
#define MMSYSERR_NODRIVER (MMSYSERR_BASE + 6) /* no device driver present */
#define MMSYSERR_NOMEM (MMSYSERR_BASE + 7)    /* memory allocation error */
#define MMSYSERR_NOTSUPPORTED                                                  \
  (MMSYSERR_BASE + 8)                           /* function isn't supported    \
                                                 */
#define MMSYSERR_BADERRNUM (MMSYSERR_BASE + 9)  /* error value out of range */
#define MMSYSERR_INVALFLAG (MMSYSERR_BASE + 10) /* invalid flag passed */
#define MMSYSERR_INVALPARAM                                                    \
  (MMSYSERR_BASE + 11)                           /* invalid parameter passed   \
                                                  */
#define MMSYSERR_HANDLEBUSY (MMSYSERR_BASE + 12) /* handle being used */
/* simultaneously on another */
/* thread (eg callback) */
#define MMSYSERR_INVALIDALIAS                                                  \
  (MMSYSERR_BASE + 13)                      /* specified alias not found */
...
#define MMSYSERR_VALNOTFOUND                                                   \
  (MMSYSERR_BASE + 19) /* registry value not found                             \
                        */
#define MMSYSERR_NODRIVERCB                                                    \
  (MMSYSERR_BASE + 20) /* driver does not call DriverCallback */
