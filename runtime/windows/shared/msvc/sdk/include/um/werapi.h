...
//
// Fault reporting UI should always be shown. This is only applicable for
...
// The upper 16 bits are reserved for internal use and get cleared by
// WerAddFile.
//
...
#define WER_SUBMIT_ADD_REGISTERED_DATA                                         \
  16                               // Add registered data to the WER report
#define WER_SUBMIT_OUTOFPROCESS 32 // Force the report to go out of process
#define WER_SUBMIT_NO_CLOSE_UI                                                 \
  64 // Do not show the close dialog for the critical report
#define WER_SUBMIT_NO_QUEUE 128   // Do not queue the report
#define WER_SUBMIT_NO_ARCHIVE 256 // Do not archive the report
#define WER_SUBMIT_START_MINIMIZED                                             \
  512 // The initial reporting UI is minimized and will flash
#define WER_SUBMIT_OUTOFPROCESS_ASYNC                                          \
  1024 // Force the report to go out of process and do not wait for it to finish
#define WER_SUBMIT_BYPASS_DATA_THROTTLING                                      \
  2048 // Bypass data throttling for the report
#define WER_SUBMIT_ARCHIVE_PARAMETERS_ONLY                                     \
  4096 // Archive only the parameters; the cab is discarded
#define WER_SUBMIT_REPORT_MACHINE_ID                                           \
  8192 // Always send the machine ID, regardless of the consent the report was
       // submitted with
#define WER_SUBMIT_BYPASS_POWER_THROTTLING                                     \
  16384 // Bypass power-related throttling (when on battery)
#define WER_SUBMIT_BYPASS_NETWORK_COST_THROTTLING                              \
  32768 // Bypass network-related throttling (when on restricted networks)
...
HRESULT
WINAPI
WerReportCreate(_In_ PCWSTR pwzEventType, _In_ WER_REPORT_TYPE repType,
                _In_opt_ PWER_REPORT_INFORMATION pReportInformation,
                _Out_ HREPORT *phReportHandle);

HRESULT
WINAPI
WerReportSetParameter(_In_ HREPORT hReportHandle, _In_ DWORD dwparamID,
                      _In_opt_ PCWSTR pwzName, _In_ PCWSTR pwzValue);

HRESULT
WINAPI
WerReportAddFile(_In_ HREPORT hReportHandle, _In_ PCWSTR pwzPath,
                 _In_ WER_FILE_TYPE repFileType, _In_ DWORD dwFileFlags);

HRESULT
WINAPI
WerReportSetUIOption(_In_ HREPORT hReportHandle, _In_ WER_REPORT_UI repUITypeID,
                     _In_ PCWSTR pwzValue);

HRESULT
WINAPI
WerReportSubmit(_In_ HREPORT hReportHandle, _In_ WER_CONSENT consent,
                _In_ DWORD dwFlags, _Out_opt_ PWER_SUBMIT_RESULT pSubmitResult);
...
HRESULT
WINAPI
WerReportAddDump(_In_ HREPORT hReportHandle, _In_ HANDLE hProcess,
                 _In_opt_ HANDLE hThread, _In_ WER_DUMP_TYPE dumpType,
...
HRESULT
WINAPI
WerReportCloseHandle(_In_ HREPORT hReportHandle);
...
HRESULT
WINAPI
WerRegisterFile(_In_ PCWSTR pwzFile, _In_ WER_REGISTER_FILE_TYPE regFileType,
                _In_ DWORD dwFlags);
...
STDAPI
WerRegisterExcludedMemoryBlock(_In_ const void *address, _In_ DWORD size);

STDAPI
WerUnregisterExcludedMemoryBlock(_In_ const void *address);

STDAPI
WerRegisterCustomMetadata(_In_ PCWSTR key, _In_ PCWSTR value);

STDAPI
WerUnregisterCustomMetadata(_In_ PCWSTR key);
...
STDAPI
WerRegisterAdditionalProcess(_In_ DWORD processId,
                             _In_ DWORD captureExtraInfoForThreadId);

STDAPI
WerUnregisterAdditionalProcess(_In_ DWORD processId);

STDAPI
WerRegisterAppLocalDump(_In_ PCWSTR localAppDataRelativePath);

STDAPI
WerUnregisterAppLocalDump();

STDAPI
WerSetMaxProcessHoldMilliseconds(_In_ DWORD dwMilliseconds);
...
HRESULT
WINAPI
WerAddExcludedApplication(_In_ PCWSTR pwzExeName, _In_ BOOL bAllUsers);

HRESULT
WINAPI
WerRemoveExcludedApplication(_In_ PCWSTR pwzExeName, _In_ BOOL bAllUsers);
...
HRESULT
WINAPI
WerRegisterRuntimeExceptionModule(_In_ PCWSTR pwszOutOfProcessCallbackDll,
                                  _In_ PVOID pContext);

HRESULT
WINAPI
WerUnregisterRuntimeExceptionModule(_In_ PCWSTR pwszOutOfProcessCallbackDll,
                                    _In_ PVOID pContext);
...
typedef HRESULT (*PFN_WER_RUNTIME_EXCEPTION_EVENT)(
    _In_ PVOID pContext,
    _In_ const PWER_RUNTIME_EXCEPTION_INFORMATION pExceptionInformation,
    _Out_ BOOL *pbOwnershipClaimed, _Out_writes_(*pchSize) PWSTR pwszEventName,
    _Inout_ PDWORD pchSize, _Out_ PDWORD pdwSignatureCount);

typedef HRESULT (*PFN_WER_RUNTIME_EXCEPTION_EVENT_SIGNATURE)(
    _In_ PVOID pContext,
    _In_ const PWER_RUNTIME_EXCEPTION_INFORMATION pExceptionInformation,
    _In_ DWORD dwIndex, _Out_writes_(*pchName) PWSTR pwszName,
    _Inout_ PDWORD pchName, _Out_writes_(*pchValue) PWSTR pwszValue,
    _Inout_ PDWORD pchValue);

typedef HRESULT (*PFN_WER_RUNTIME_EXCEPTION_DEBUGGER_LAUNCH)(
    _In_ PVOID pContext,
    _In_ const PWER_RUNTIME_EXCEPTION_INFORMATION pExceptionInformation,
    _Out_ PBOOL pbIsCustomDebugger,
    _Out_writes_(*pchDebuggerLaunch) PWSTR pwszDebuggerLaunch,
    _Inout_ PDWORD pchDebuggerLaunch, _Out_ PBOOL pbIsDebuggerAutolaunch);
...
Members:
    ...
    CreationTime       - See WER_REPORT_METADATA_V1.CreationTime for
explanation. SizeInBytes        - See WER_REPORT_METADATA_V1.SizeInBytes for
explanation. CabId              - Cab id of the report if present. (E.g:
125446340080) ReportStatus       - The detailed status of the report. Useful for
tracing the steps a report has gone through during submission attempt.
    ReportIntegratorId - The report integrator id of the report.
    NumberOfFiles      - The number of files included in the WER report.
    SizeOfFileNames    - Total size of the file name, in count of WCHARS,
including terminating character for each name and one more in the end. FileNames
- A pointer to hold the names of the files included in the report. It is
...
Structure Description:
    ...
    Friendly Event Name, Application Name, Application Path, Description,
BucketIdString and LegacyBucketId.
...
Members:
    ...
    CreationTime       - See WER_REPORT_METADATA_V1.CreationTime for
explanation. SizeInBytes        - See WER_REPORT_METADATA_V1.SizeInBytes for
explanation. CabId              - See WER_REPORT_METADATA_V2.CabId for
explanation. ReportStatus       - See WER_REPORT_METADATA_V2.ReportStatus for
explanation. ReportIntegratorId - See WER_REPORT_METADATA_V2.ReportIntegrationId
for explanation. NumberOfFiles      - See WER_REPORT_METADATA_V2.NumberOfFiles
for explanation. SizeOfFileNames    - See WER_REPORT_METADATA_V2.SizeOfFileNames
for explanation. FileNames          - See WER_REPORT_METADATA_V2.FileNames for
explanation. FriendlyEventName  - Represents friendly event name.  E.g. "Windows
Update installation problem". ApplicationName    - Represents application name.
E.g. "Host Process for Windows Services". ApplicationPath    - Represents
application path. Description        - Represents failure description.  E.g. "A
Windows update did not install properly. Sending the following information to
Microsoft can help improve the software." BucketIdString     - Represents Bucket
Id string (possibly truncated).  In user mode it looks like failure hash. For
kernel failures it looks like faulting frame name. LegacyBucketId     -
Represents Legacy Bucket Id integer value.  E.g. 2221943892575779112.
 --*/
...
__control_entrypoint(DllExport) HRESULT
    WerStoreOpen(_In_ REPORT_STORE_TYPES repStoreType,
                 _Out_ PHREPORTSTORE phReportStore);

__control_entrypoint(DllExport) VOID
    WerStoreClose(_In_opt_ _Post_invalid_ HREPORTSTORE hReportStore);

__control_entrypoint(DllExport) HRESULT WerStoreGetFirstReportKey(
    _In_ HREPORTSTORE hReportStore,
    _Outptr_result_maybenull_z_ PCWSTR *ppszReportKey);

__control_entrypoint(DllExport) HRESULT
    WerStoreGetNextReportKey(_In_ HREPORTSTORE hReportStore,
                             _Outptr_result_maybenull_z_ PCWSTR *ppszReportKey);

__control_entrypoint(DllExport) HRESULT WerStoreQueryReportMetadataV2(
    _In_ HREPORTSTORE hReportStore, _In_ PCWSTR pszReportKey,
    _Out_ PWER_REPORT_METADATA_V2 pReportMetadata);

__control_entrypoint(DllExport) HRESULT WerStoreQueryReportMetadataV3(
    _In_ HREPORTSTORE hReportStore, _In_ PCWSTR pszReportKey,
    _Out_ PWER_REPORT_METADATA_V3 pReportMetadata);

__control_entrypoint(DllExport) VOID WerFreeString(_In_ PCWSTR pwszStr);

__control_entrypoint(DllExport) HRESULT WerStorePurge();
...
__control_entrypoint(DllExport) HRESULT
    WerStoreGetReportCount(_In_ HREPORTSTORE hReportStore,
                           _Out_ DWORD *pdwReportCount);

__control_entrypoint(DllExport) HRESULT
    WerStoreGetSizeOnDisk(_In_ HREPORTSTORE hReportStore,
...
__control_entrypoint(DllExport) HRESULT WerStoreQueryReportMetadataV1(
    _In_ HREPORTSTORE hReportStore, _In_ PCWSTR pszReportKey,
    _Out_ PWER_REPORT_METADATA_V1 pReportMetadata);

__control_entrypoint(DllExport) HRESULT
    WerStoreUploadReport(_In_ HREPORTSTORE hReportStore,
                         _In_ PCWSTR pszReportKey, _In_ DWORD dwFlags,
