...
// - which is best effort - from network equipment. This may lead to lost
...
__inline INT QOS_HEADER_OVERHEAD(_In_ INT af, _In_ INT protocol) {
  UINT32 overhead;
...
//  socket with.
//
__inline UINT64 QOS_ADD_OVERHEAD(_In_ INT af, _In_ INT protocol,
...
//  socket with.
//
__inline UINT64 QOS_SUBTRACT_OVERHEAD(_In_ INT af, _In_ INT protocol,
                                      _In_ UINT32 targetDataPacketSize,
                                      _In_ UINT64 dataRate) {
  UINT32 overhead;
  double d;
...
//
ExternC BOOL WINAPI QOSCreateHandle(_In_ PQOS_VERSION Version,
...
//
ExternC BOOL WINAPI QOSCloseHandle(_In_ HANDLE QOSHandle);
...
//
ExternC BOOL WINAPI QOSStartTrackingClient(_In_ HANDLE QOSHandle,
...
//  information, call GetLastError.
//
ExternC BOOL WINAPI QOSStopTrackingClient(_In_ HANDLE QOSHandle,
                                          _In_ PSOCKADDR DestAddr,
                                          _Reserved_ DWORD Flags);
...
//
ExternC BOOL WINAPI QOSEnumerateFlows(_In_ HANDLE QOSHandle,
                                      _Inout_ PULONG Size,
...
//          have the required components or that it is not on the same link.
//
ExternC BOOL WINAPI QOSAddSocketToFlow(_In_ HANDLE QOSHandle,
                                       _In_ SOCKET Socket,
...
//
ExternC BOOL WINAPI QOSRemoveSocketFromFlow(_In_ HANDLE QOSHandle,
...
//          to perform this call.
//
ExternC BOOL WINAPI QOSSetFlow(_In_ HANDLE QOSHandle, _In_ QOS_FLOWID FlowId,
                               _In_ QOS_SET_FLOW Operation, _In_ ULONG Size,
...
//          before ascertaining the state of the network.
//
ExternC BOOL WINAPI QOSQueryFlow(_In_ HANDLE QOSHandle, _In_ QOS_FLOWID FlowId,
...
//          information on your network is currently not supported.
//
ExternC BOOL WINAPI QOSNotifyFlow(_In_ HANDLE QOSHandle, _In_ QOS_FLOWID FlowId,
...
//  information, call GetLastError.
//
ExternC BOOL WINAPI QOSCancel(_In_ HANDLE QOSHandle,
