...
// FDM_TYPE_CHAR_INC_EQUAL_BM_BASE  // base width == cx for horiz, == cy for
// vert. FDM_TYPE_MAXEXT_EQUAL_BM_SIDE    // side width == cy for horiz, == cx
// for vert.
...
#define SIZEOFDV(cAxes)                                                        \
  (offsetof(DESIGNVECTOR, dvValues) + (cAxes) * sizeof(LONG))
#define SIZEOFAXIW(cAxes)                                                      \
  (offsetof(AXESLISTW, axlAxisInfo) + (cAxes) * sizeof(AXISINFOW))
#define SIZEOFAXIA(cAxes)                                                      \
  (offsetof(AXESLISTA, axlAxisInfo) + (cAxes) * sizeof(AXISINFOA))
...
#define GCAPS_FONT_RASTERIZER                                                  \
  0x40000000 // the device has built in tt rasterizer
...
typedef struct tagCDDDXGK_REDIRBITMAPPRESENTINFO {
  ...
  UINT NumContexts;   // in
  HANDLE
  hContext[WINDDI_MAX_BROADCAST_CONTEXT + 1]; // in  Presenting DXGCONTEXTs
...
typedef struct _GDIINFO {
  ...
  //
  // The user defined pHTPatA, pHTPatB, pHTPatC pointers correspond to the
  // primary color order defined in ulPrimaryOrder as PRIMARY_ORDER_xxx.
  //
  ...
  // location is ignored (always black), while 1 to 255 threshold values give
  // the dither pattern 255 level of grays.
...
ENGAPI
ULONG APIENTRY CLIPOBJ_cEnumStart(CLIPOBJ *pco, BOOL bAll,
...
// SO_REVERSED               // set if horiz & "right to left" or if vert &
// "bottom to top" SO_ZERO_BEARINGS          // all glyphs have zero a and c
// spaces SO_CHAR_INC_EQUAL_BM_BASE // base == cx for horiz, == cy for vert.
...
typedef struct _STROBJ {
  ...
  ULONG
  ulCharInc; // non-zero only if fixed pitch font, equal to advanced width.
...
VOID APIENTRY FONTOBJ_vGetInfo(FONTOBJ *pfo,
                               __in_data_source(USER_MODE) ULONG cjSize,
                               FONTINFO *pfi);

ULONG APIENTRY FONTOBJ_cGetGlyphs(FONTOBJ *pfo,
                                  __in_data_source(USER_MODE) ULONG iMode,
                                  ULONG cGlyph, HGLYPH *phg, PVOID *ppvGlyph);

#if (NTDDI_VERSION <= NTDDI_WINXP)
GAMMA_TABLES *APIENTRY FONTOBJ_pGetGammaTables(FONTOBJ *pfo);
#endif // (NTDDI_VERSION <= NTDDI_WINXP)

XFORMOBJ *APIENTRY FONTOBJ_pxoGetXform(_In_ FONTOBJ *pfo);

IFIMETRICS *APIENTRY FONTOBJ_pifi(_In_ FONTOBJ *pfo);

FD_GLYPHSET *APIENTRY FONTOBJ_pfdg(FONTOBJ *pfo);

PVOID APIENTRY FONTOBJ_pvTrueTypeFontFile(FONTOBJ *pfo, ULONG *pcjFile);

PBYTE APIENTRY FONTOBJ_pjOpenTypeTablePointer(FONTOBJ *pfo, ULONG ulTag,
                                              ULONG *pcjTable);
...
PFD_GLYPHATTR APIENTRY FONTOBJ_pQueryGlyphAttrs(FONTOBJ *pfo,
...
ULONG APIENTRY PALOBJ_cGetColors(PALOBJ *ppalo, ULONG iStart, ULONG cColors,
                                 ULONG *pulColors);
...
#define PD_ALL                                                                 \
  (PD_BEGINSUBPATH | PD_ENDSUBPATH | PD_RESETSTYLE | PD_CLOSEFIGURE |          \
   PD_BEZIERS)
...
ENGAPI
BOOL APIENTRY PATHOBJ_bMoveTo(_In_ PATHOBJ *ppo, _In_ POINTFIX ptfx);

ENGAPI
BOOL APIENTRY PATHOBJ_bPolyLineTo(_In_ PATHOBJ *ppo,
                                  _In_reads_(cptfx) POINTFIX *pptfx,
                                  _In_ ULONG cptfx);

ENGAPI
BOOL APIENTRY PATHOBJ_bPolyBezierTo(_In_ PATHOBJ *ppo,
                                    _In_reads_(cptfx) POINTFIX *pptfx,
                                    _In_ ULONG cptfx);

ENGAPI
BOOL APIENTRY PATHOBJ_bCloseFigure(_In_ PATHOBJ *ppo);
...
ENGAPI
BOOL APIENTRY STROBJ_bEnumPositionsOnly(STROBJ *pstro, ULONG *pc,
                                        PGLYPHPOS *ppgpos);
...
ENGAPI
BOOL APIENTRY STROBJ_bGetAdvanceWidths(STROBJ *pso,
                                       __in_data_source(USER_MODE) ULONG iFirst,
                                       ULONG c, POINTQF *pptqD);
...
#endif // (NTDDI_VERSION >= NTDDI_WINXP)
#if (NTDDI_VERSION >= NTDDI_WIN7)
// BMF_TEMP_ALPHA is set to the SURFOBJ by the GDI display driver (CDD)
// indicating that the output of the Clear Type or antialiased text rendering
// should be alpha values only. The display driver will use the alpha values to
// blend the foreground color and the destination.
...
ENGAPI
ULONG APIENTRY XFORMOBJ_iGetXform(_In_ XFORMOBJ *pxo, _Out_opt_ XFORML *pxform);

ENGAPI
BOOL APIENTRY XFORMOBJ_bApplyXform(XFORMOBJ *pxo,
                                   __in_data_source(USER_MODE)
                                       __out_validated(GRAPHIC) ULONG iMode,
                                   ULONG cPoints, PVOID pvIn, PVOID pvOut);
...
ENGAPI
ULONG APIENTRY XLATEOBJ_cGetPalette(XLATEOBJ *pxlo,
...
ENGAPI
HBITMAP APIENTRY EngCreateBitmap(SIZEL sizl, LONG lWidth,
                                 __in_data_source(USER_MODE) ULONG iFormat,
                                 __in_data_source(USER_MODE) FLONG fl,
                                 __in_data_source(USER_MODE) PVOID pvBits);

ENGAPI
HSURF APIENTRY EngCreateDeviceSurface(__in_data_source(USER_MODE) DHSURF dhsurf,
                                      __in_data_source(USER_MODE) SIZEL sizl,
                                      ULONG iFormatCompat);

ENGAPI
HBITMAP APIENTRY EngCreateDeviceBitmap(__in_data_source(USER_MODE)
                                           DHSURF dhsurf,
                                       __in_data_source(USER_MODE) SIZEL sizl,
                                       ULONG iFormatCompat);
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
ENGAPI
HBITMAP APIENTRY EngCreateRedirectionDeviceBitmap(__in_data_source(USER_MODE)
                                                      DHSURF dhsurf,
                                                  __in_data_source(USER_MODE)
                                                      SIZEL sizl,
                                                  ULONG iFormatCompat);
...
ENGAPI
BOOL APIENTRY EngDeleteSurface(__in_data_source(USER_MODE) HSURF hsurf);

ENGAPI
SURFOBJ *APIENTRY EngLockSurface(HSURF hsurf);

ENGAPI
VOID APIENTRY EngUnlockSurface(__in_data_source(USER_MODE) SURFOBJ *pso);

ENGAPI
BOOL APIENTRY EngEraseSurface(SURFOBJ *pso, RECTL *prcl,
...
ENGAPI
BOOL APIENTRY EngModifySurface(HSURF hsurf, HDEV hdev, FLONG flHooks,
                               FLONG flSurface, DHSURF dhsurf, VOID *pvScan0,
                               LONG lDelta, VOID *pvReserved);

ENGAPI
BOOL APIENTRY EngMarkBandingSurface(__in_data_source(USER_MODE) HSURF hsurf);
...
ENGAPI
HPALETTE APIENTRY EngCreatePalette(__in_data_source(USER_MODE) ULONG iMode,
...
ENGAPI
ULONG APIENTRY EngQueryPalette(HPALETTE hpal, ULONG *piMode, ULONG cColors,
                               ULONG *pulColors);

ENGAPI
BOOL APIENTRY EngDeletePalette(__in_data_source(USER_MODE) HPALETTE hpal);
...
ENGAPI
VOID APIENTRY EngDeleteClip(_In_opt_ CLIPOBJ *pco);
...
BOOL APIENTRY APIENTRY DrvQueryDriverInfo(DWORD dwMode,
...
BOOL APIENTRY DrvEnableDriver(ULONG iEngineVersion, ULONG cj,
...
DHPDEV APIENTRY
DrvEnablePDEV(_In_ DEVMODEW *pdm, _In_ LPWSTR pwszLogAddress, ULONG cPat,
              _In_reads_opt_(cPat) HSURF *phsurfPatterns, ULONG cjCaps,
              _Out_writes_bytes_(cjCaps) ULONG *pdevcaps, ULONG cjDevInfo,
              _Out_writes_bytes_(cjDevInfo) DEVINFO *pdi, HDEV hdev,
              _In_ LPWSTR pwszDeviceName, HANDLE hDriver);

#define HS_DDI_MAX 6

#if (NTDDI_VERSION >= NTDDI_VISTA)
LONG APIENTRY DrvRenderHint(DHPDEV dhpdev, ULONG NotifyCode, SIZE_T Length,
...
__drv_preferredFunction("(see documentation)", "Obsolete") BOOL APIENTRY
    DrvOffset( // Obsolete
...
ULONG_PTR APIENTRY DrvSaveScreenBits(SURFOBJ *pso, ULONG iMode, ULONG_PTR ident,
                                     RECTL *prcl);
...
ULONG APIENTRY DrvGetModes(_In_ HANDLE hDriver, ULONG cjSize,
                           _Out_writes_bytes_opt_(cjSize) DEVMODEW *pdm);

DECLSPEC_DEPRECATED_DDK
VOID APIENTRY DrvMovePanning(LONG x, LONG y, FLONG fl);

BOOL APIENTRY DrvPlgBlt(_Inout_ SURFOBJ *psoTrg, _Inout_ SURFOBJ *psoSrc,
                        _In_opt_ SURFOBJ *psoMsk, _In_ CLIPOBJ *pco,
                        _In_opt_ XLATEOBJ *pxlo, _In_opt_ COLORADJUSTMENT *pca,
                        _In_opt_ POINTL *pptlBrushOrg, _In_ POINTFIX *pptfx,
                        _In_ RECTL *prcl, _In_opt_ POINTL *pptl,
                        _In_ ULONG iMode);
...
HBITMAP APIENTRY DrvCreateDeviceBitmap(DHPDEV dhpdev, SIZEL sizl,
                                       ULONG iFormat);
...
#if (NTDDI_VERSION >= NTDDI_WIN10_RS2)
#define CDBEX_REUSE                                                            \
  0x00000010 // This flag indicates dhsurfGroup is the dhsurf from old PDEV
             // during dynamic mode change.
#endif       // (NTDDI_VERSION >= NTDDI_WIN10_RS2)

HBITMAP APIENTRY DrvCreateDeviceBitmapEx(DHPDEV dhpdev, SIZEL sizl,
                                         ULONG iFormat, DWORD Flags,
                                         DHSURF dhsurfGroup, DWORD DxFormat,
#if (NTDDI_VERSION >= NTDDI_WIN7)
                                         DWORD SubresourceIndex,
#endif // (NTDDI_VERSION >= NTDDI_WIN8)
                                         HANDLE *phSharedSurface);

VOID APIENTRY DrvDeleteDeviceBitmapEx(DHSURF dhsurf);

BOOL APIENTRY DrvAssociateSharedSurface(_In_ SURFOBJ *psoSurf,
                                        _In_ HANDLE hPhysicalSurface,
                                        _In_ HANDLE hLogicalSurface,
                                        _In_ SIZEL sizl);

LONG APIENTRY DrvSynchronizeRedirectionBitmaps(_In_ DHPDEV dhpdev,
                                               _Out_ UINT64 *puiFenceID);

BOOL APIENTRY DrvAccumulateD3DDirtyRect(
    _In_ SURFOBJ *psoSurf, _In_ CDDDXGK_REDIRBITMAPPRESENTINFO *pDirty);

BOOL APIENTRY DrvStartDxInterop(_In_ SURFOBJ *psoSurf, _In_ BOOL bDiscard,
                                _In_ PVOID KernelModeDeviceHandle);

BOOL APIENTRY DrvEndDxInterop(_In_ SURFOBJ *psoSurf, _In_ BOOL bDiscard,
                              _Out_ BOOL *bDeviceLost,
                              _In_ PVOID KernelModeDeviceHandle);

VOID APIENTRY DrvLockDisplayArea(_In_ DHPDEV dhpdev, _In_opt_ RECTL *prcl);

VOID APIENTRY DrvUnlockDisplayArea(_In_ DHPDEV dhpdev, _In_opt_ RECTL *prcl);

#endif // (NTDDI_VERSION >= NTDDI_WIN7)

#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)

BOOL APIENTRY DrvSurfaceComplete(_In_ DHPDEV dhpdev,
                                 _In_ HANDLE hLogicalSurface);
...
BOOL APIENTRY DrvSetPalette(DHPDEV dhpdev, PALOBJ *ppalo, FLONG fl,
                            _In_range_(0, WINDDI_MAXSETPALETTECOLORINDEX)
                                ULONG iStart,
                            _In_range_(0, WINDDI_MAXSETPALETTECOLORS - iStart)
                                ULONG cColors);
...
ULONG APIENTRY DrvDitherColor(_In_ DHPDEV dhpdev, _In_ ULONG iMode,
                              _In_ ULONG rgb,
                              _Inout_updates_(8 * 8) ULONG *pul);

BOOL APIENTRY DrvRealizeBrush(_In_ BRUSHOBJ *pbo, _Inout_ SURFOBJ *psoTarget,
                              _In_ SURFOBJ *psoPattern,
                              _In_opt_ SURFOBJ *psoMask, _In_ XLATEOBJ *pxlo,
                              _In_ ULONG iHatch);
...
PIFIMETRICS APIENTRY DrvQueryFont(DHPDEV dhpdev, ULONG_PTR iFile, ULONG iFace,
#if (NTDDI_VERSION >= NTDDI_WINXP)
                                  ULONG_PTR *pid
#else
                                  ULONG *pid
...
PVOID APIENTRY DrvQueryFontTree(DHPDEV dhpdev, ULONG_PTR iFile, ULONG iFace,
                                ULONG iMode,
#if (NTDDI_VERSION >= NTDDI_WINXP)
                                ULONG_PTR *pid
#else
...
LONG APIENTRY DrvQueryFontData(DHPDEV dhpdev, FONTOBJ *pfo, ULONG iMode,
                               HGLYPH hg, GLYPHDATA *pgd,
                               _Out_writes_bytes_(cjSize) PVOID pv,
                               ULONG cjSize);

VOID APIENTRY DrvFree(PVOID pv,
#if (NTDDI_VERSION >= NTDDI_WINXP)
                      ULONG_PTR id);
#else
                      ULONG id);
...
_Success_(return != DDI_ERROR) LONG APIENTRY
    DrvQueryTrueTypeTable(ULONG_PTR iFile, ULONG ulFont, ULONG ulTag,
                          PTRDIFF dpStart, ULONG cjBuf,
                          _Out_writes_opt_(cjBuf) BYTE *pjBuf,
                          _Outptr_opt_result_buffer_(*pcjTable) PBYTE *ppjTable,
                          _Out_opt_ ULONG *pcjTable);

BOOL APIENTRY DrvQueryAdvanceWidths(DHPDEV dhpdev, FONTOBJ *pfo, ULONG iMode,
                                    _In_reads_(cGlyphs) HGLYPH *phg,
                                    _Out_writes_bytes_(cGlyphs * sizeof(USHORT))
                                        PVOID pvWidths,
                                    ULONG cGlyphs);
...
LONG APIENTRY DrvQueryTrueTypeOutline(DHPDEV dhpdev, FONTOBJ *pfo,
                                      HGLYPH hglyph, BOOL bMetricsOnly,
...
BOOL APIENTRY DrvBitBlt(_Inout_ SURFOBJ *psoTrg, _In_opt_ SURFOBJ *psoSrc,
                        _In_opt_ SURFOBJ *psoMask, _In_ CLIPOBJ *pco,
                        _In_opt_ XLATEOBJ *pxlo, _In_ RECTL *prclTrg,
                        _In_opt_ POINTL *pptlSrc, _In_opt_ POINTL *pptlMask,
                        _In_opt_ BRUSHOBJ *pbo, _In_opt_ POINTL *pptlBrush,
                        _In_ ROP4 rop4);

BOOL APIENTRY DrvStretchBlt(_Inout_ SURFOBJ *psoDest, _Inout_ SURFOBJ *psoSrc,
                            _In_opt_ SURFOBJ *psoMask, _In_ CLIPOBJ *pco,
                            _In_opt_ XLATEOBJ *pxlo,
                            _In_opt_ COLORADJUSTMENT *pca,
                            _In_ POINTL *pptlHTOrg, _In_ RECTL *prclDest,
                            _In_ RECTL *prclSrc, _In_opt_ POINTL *pptlMask,
                            _In_ ULONG iMode);

BOOL APIENTRY DrvStretchBltROP(
    _Inout_ SURFOBJ *psoDest, _Inout_ SURFOBJ *psoSrc,
    _In_opt_ SURFOBJ *psoMask, _In_ CLIPOBJ *pco, _In_opt_ XLATEOBJ *pxlo,
    _In_opt_ COLORADJUSTMENT *pca, _In_ POINTL *pptlHTOrg, _In_ RECTL *prclDest,
    _In_ RECTL *prclSrc, _In_opt_ POINTL *pptlMask, _In_ ULONG iMode,
    _In_ BRUSHOBJ *pbo, _In_ DWORD rop4);

BOOL APIENTRY DrvAlphaBlend(_Inout_ SURFOBJ *psoDest, _In_ SURFOBJ *psoSrc,
                            _In_ CLIPOBJ *pco, _In_opt_ XLATEOBJ *pxlo,
                            _In_ RECTL *prclDest, _In_ RECTL *prclSrc,
                            _In_ BLENDOBJ *pBlendObj);

BOOL APIENTRY DrvGradientFill(_Inout_ SURFOBJ *psoDest, _In_ CLIPOBJ *pco,
                              _In_opt_ XLATEOBJ *pxlo, _In_ TRIVERTEX *pVertex,
                              _In_ ULONG nVertex, _In_ PVOID pMesh,
                              _In_ ULONG nMesh, _In_ RECTL *prclExtents,
                              _In_ POINTL *pptlDitherOrg, _In_ ULONG ulMode);

BOOL APIENTRY DrvTransparentBlt(_Inout_ SURFOBJ *psoDst, _In_ SURFOBJ *psoSrc,
                                _In_ CLIPOBJ *pco, _In_opt_ XLATEOBJ *pxlo,
                                _In_ RECTL *prclDst, _In_ RECTL *prclSrc,
                                _In_ ULONG iTransColor, _In_ ULONG ulReserved);

BOOL APIENTRY DrvCopyBits(SURFOBJ *psoDest, SURFOBJ *psoSrc, CLIPOBJ *pco,
                          XLATEOBJ *pxlo, RECTL *prclDest, POINTL *pptlSrc);

/*
 * Text Output
 */

BOOL APIENTRY DrvTextOut(SURFOBJ *pso, STROBJ *pstro, FONTOBJ *pfo,
                         CLIPOBJ *pco,
                         RECTL *prclExtra, // Obsolete, always NULL
                         RECTL *prclOpaque, BRUSHOBJ *pboFore,
...
BOOL APIENTRY DrvLineTo(_Inout_ SURFOBJ *pso, _In_ CLIPOBJ *pco,
                        _In_ BRUSHOBJ *pbo, _In_ LONG x1, _In_ LONG y1,
                        _In_ LONG x2, _In_ LONG y2, _In_opt_ RECTL *prclBounds,
                        _In_ MIX mix);

BOOL APIENTRY DrvStrokePath(_Inout_ SURFOBJ *pso, _In_ PATHOBJ *ppo,
                            _In_ CLIPOBJ *pco, _In_opt_ XFORMOBJ *pxo,
                            _In_ BRUSHOBJ *pbo, _In_ POINTL *pptlBrushOrg,
                            _In_ LINEATTRS *plineattrs, _In_ MIX mix);

#define FP_ALTERNATEMODE 1L
#define FP_WINDINGMODE 2L

BOOL APIENTRY DrvFillPath(_Inout_ SURFOBJ *pso, _In_ PATHOBJ *ppo,
                          _In_ CLIPOBJ *pco, _In_ BRUSHOBJ *pbo,
                          _In_ POINTL *pptlBrushOrg, _In_ MIX mix,
                          _In_ FLONG flOptions);

BOOL APIENTRY DrvStrokeAndFillPath(_Inout_ SURFOBJ *pso, _Inout_ PATHOBJ *ppo,
                                   _In_ CLIPOBJ *pco, _In_opt_ XFORMOBJ *pxo,
                                   _In_ BRUSHOBJ *pboStroke,
                                   _In_ LINEATTRS *plineattrs,
                                   _In_ BRUSHOBJ *pboFill,
                                   _In_ POINTL *pptlBrushOrg, _In_ MIX mixFill,
                                   _In_ FLONG flOptions);

BOOL APIENTRY DrvPaint(_Inout_ SURFOBJ *pso, _In_ CLIPOBJ *pco,
                       _In_ BRUSHOBJ *pbo, _In_ POINTL *pptlBrushOrg,
                       _In_ MIX mix);
...
ULONG APIENTRY DrvSetPointerShape(SURFOBJ *pso, SURFOBJ *psoMask,
                                  SURFOBJ *psoColor, XLATEOBJ *pxlo, LONG xHot,
                                  LONG yHot, LONG x, LONG y, RECTL *prcl,
                                  FLONG fl);
...
BOOL APIENTRY DrvSendPage(_In_ SURFOBJ *pso);

BOOL APIENTRY DrvStartPage(_In_ SURFOBJ *pso);

ULONG APIENTRY DrvEscape(_In_ SURFOBJ *pso, _In_ ULONG iEsc, _In_ ULONG cjIn,
                         _In_reads_bytes_(cjIn) PVOID pvIn, _In_ ULONG cjOut,
                         _Out_writes_bytes_(cjOut) PVOID pvOut);

BOOL APIENTRY DrvStartDoc(_In_ SURFOBJ *pso, _In_ LPWSTR pwszDocName,
                          _In_ DWORD dwJobId);

#define ED_ABORTDOC 1

BOOL APIENTRY DrvEndDoc(_In_ SURFOBJ *pso, _In_ FLONG fl);

DECLSPEC_DEPRECATED_DDK
BOOL APIENTRY DrvQuerySpoolType(_In_ DHPDEV, _In_ LPWSTR);

ULONG APIENTRY DrvDrawEscape(_In_ SURFOBJ *pso, _In_ ULONG iEsc,
                             _In_ CLIPOBJ *pco, _In_ RECTL *prcl,
                             _In_ ULONG cjIn,
                             _In_reads_bytes_(cjIn) PVOID pvIn);

ULONG APIENTRY DrvGetGlyphMode(_In_ DHPDEV, _In_ FONTOBJ *);

_Success_(return == 1) ULONG APIENTRY
    DrvFontManagement(_In_ SURFOBJ *pso, _In_opt_ FONTOBJ *pfo,
                      _In_ ULONG iMode, _In_ ULONG cjIn,
                      _In_reads_bytes_(cjIn) PVOID pvIn, _In_ ULONG cjOut,
                      _Out_writes_bytes_(cjOut) PVOID pvOut);

BOOL APIENTRY DrvStartBanding(_In_ SURFOBJ *pso, _In_ POINTL *pptl);

BOOL APIENTRY DrvNextBand(_In_ SURFOBJ *pso, _In_ POINTL *pptl);
...
ULONG APIENTRY DrvQueryPerBandInfo(_In_ SURFOBJ *pso, _Inout_ PERBANDINFO *pbi);
...
BOOL APIENTRY DrvGetDirectDrawInfo(DHPDEV dhpdev, DD_HALINFO *pHalInfo,
                                   DWORD *pdwNumHeaps, VIDEOMEMORY *pvmList,
                                   DWORD *pdwNumFourCCCodes, DWORD *pdwFourCC);

HBITMAP APIENTRY DrvDeriveSurface(DD_DIRECTDRAW_GLOBAL *pDirectDraw,
...
HANDLE APIENTRY DrvIcmCreateColorTransform(
    _In_ DHPDEV dhpdev, _In_ LPLOGCOLORSPACEW pLogColorSpace,
    ...
    _In_ ULONG cjTargetProfile, _In_ DWORD dwReserved);

BOOL APIENTRY DrvIcmDeleteColorTransform(_In_ DHPDEV dhpdev,
                                         _In_ HANDLE hcmXform);

BOOL APIENTRY DrvIcmCheckBitmapBits(
    DHPDEV dhpdev, HANDLE hColorTransform, SURFOBJ *pso,
...
BOOL APIENTRY DrvIcmSetDeviceGammaRamp(DHPDEV dhpdev, ULONG iFormat,
                                       LPVOID lpRamp);
...
BOOL APIENTRY DrvQueryDeviceSupport(SURFOBJ *pso, XLATEOBJ *pxlo, XFORMOBJ *pxo,
                                    ULONG iType, ULONG cjIn,
                                    _In_reads_bytes_(cjIn) PVOID pvIn,
                                    ULONG cjOut,
                                    _Out_writes_bytes_(cjOut) PVOID pvOut);
...
ENGAPI
ULONG APIENTRY WNDOBJ_cEnumStart(WNDOBJ *pwo, ULONG iType, ULONG iDirection,
                                 ULONG cLimit);
...
VOID APIENTRY EngRenderHint(DHPDEV dhpdev, ULONG NotifyCode, SIZE_T Length,
                            _In_reads_bytes_opt_(Length) PVOID Data);
...
ENGAPI
HDRVOBJ APIENTRY EngCreateDriverObj(PVOID pvObj, FREEOBJPROC pFreeObjProc,
                                    HDEV hdev);
...
ENGAPI
BOOL APIENTRY EngBitBlt(_In_ SURFOBJ *psoTrg, _In_ SURFOBJ *psoSrc,
                        _In_ SURFOBJ *psoMask, _In_ CLIPOBJ *pco,
                        _In_ XLATEOBJ *pxlo, _In_ RECTL *prclTrg,
                        _In_ POINTL *pptlSrc, _In_ POINTL *pptlMask,
                        _In_ BRUSHOBJ *pbo, _In_ POINTL *pptlBrush,
                        __in_data_source(USER_MODE) ROP4 rop4);

ENGAPI
BOOL APIENTRY EngLineTo(SURFOBJ *pso, CLIPOBJ *pco, BRUSHOBJ *pbo,
                        __in_data_source(USER_MODE) LONG x1,
                        __in_data_source(USER_MODE) LONG y1,
                        __in_data_source(USER_MODE) LONG x2,
                        __in_data_source(USER_MODE) LONG y2, RECTL *prclBounds,
                        __in_data_source(USER_MODE) MIX mix);

ENGAPI
BOOL APIENTRY EngStretchBlt(SURFOBJ *psoDest, SURFOBJ *psoSrc, SURFOBJ *psoMask,
...
ENGAPI
BOOL APIENTRY EngStretchBltROP(SURFOBJ *psoDest, SURFOBJ *psoSrc,
                               SURFOBJ *psoMask, CLIPOBJ *pco, XLATEOBJ *pxlo,
                               COLORADJUSTMENT *pca, POINTL *pptlHTOrg,
                               __in_data_source(USER_MODE) RECTL *prclDest,
                               __in_data_source(USER_MODE) RECTL *prclSrc,
                               POINTL *pptlMask,
                               __in_data_source(USER_MODE) ULONG iMode,
...
ENGAPI
BOOL APIENTRY EngAlphaBlend(SURFOBJ *psoDest, SURFOBJ *psoSrc, CLIPOBJ *pco,
                            XLATEOBJ *pxlo, RECTL *prclDest, RECTL *prclSrc,
                            BLENDOBJ *pBlendObj);

ENGAPI
BOOL APIENTRY EngGradientFill(SURFOBJ *psoDest, CLIPOBJ *pco, XLATEOBJ *pxlo,
                              TRIVERTEX *pVertex, ULONG nVertex, PVOID pMesh,
                              ULONG nMesh, RECTL *prclExtents,
                              POINTL *pptlDitherOrg,
                              __in_data_source(USER_MODE) ULONG ulMode);

ENGAPI
BOOL APIENTRY EngTransparentBlt(_In_ SURFOBJ *psoDst, _In_ SURFOBJ *psoSrc,
                                _In_opt_ CLIPOBJ *pco, _In_opt_ XLATEOBJ *pxlo,
                                _In_ RECTL *prclDst, _In_ RECTL *prclSrc,
...
ENGAPI
BOOL APIENTRY EngTextOut(SURFOBJ *pso, STROBJ *pstro, FONTOBJ *pfo,
                         CLIPOBJ *pco, RECTL *prclExtra, RECTL *prclOpaque,
                         BRUSHOBJ *pboFore, BRUSHOBJ *pboOpaque,
                         POINTL *pptlOrg, __in_data_source(USER_MODE) MIX mix);

ENGAPI
BOOL APIENTRY EngStrokePath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco,
                            XFORMOBJ *pxo, BRUSHOBJ *pbo, POINTL *pptlBrushOrg,
                            __in_data_source(USER_MODE) LINEATTRS *plineattrs,
                            __in_data_source(USER_MODE) MIX mix);

ENGAPI
BOOL APIENTRY EngFillPath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco,
                          BRUSHOBJ *pbo, POINTL *pptlBrushOrg,
                          __in_data_source(USER_MODE) MIX mix,
                          __in_data_source(USER_MODE) FLONG flOptions);

ENGAPI
BOOL APIENTRY EngStrokeAndFillPath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco,
...
ENGAPI
BOOL APIENTRY EngPaint(SURFOBJ *pso, CLIPOBJ *pco, BRUSHOBJ *pbo,
                       POINTL *pptlBrushOrg,
                       __in_data_source(USER_MODE) MIX mix);

ENGAPI
BOOL APIENTRY EngCopyBits(SURFOBJ *psoDest, SURFOBJ *psoSrc, CLIPOBJ *pco,
                          XLATEOBJ *pxlo,
                          __in_data_source(USER_MODE) RECTL *prclDest,
                          __in_data_source(USER_MODE) POINTL *pptlSrc);

ENGAPI
ULONG APIENTRY EngSetPointerShape(SURFOBJ *pso, SURFOBJ *psoMask,
                                  SURFOBJ *psoColor, XLATEOBJ *pxlo, LONG xHot,
                                  LONG yHot, LONG x, LONG y, RECTL *prcl,
                                  FLONG fl);
...
ENGAPI
BOOL APIENTRY EngPlgBlt(SURFOBJ *psoTrg, SURFOBJ *psoSrc, SURFOBJ *psoMsk,
                        CLIPOBJ *pco, XLATEOBJ *pxlo, COLORADJUSTMENT *pca,
                        POINTL *pptlBrushOrg, POINTFIX *pptfx, RECTL *prcl,
                        POINTL *pptl, __in_data_source(USER_MODE) ULONG iMode);

ENGAPI
ULONG APIENTRY EngDitherColor(HDEV hdev, ULONG iMode, ULONG rgb, ULONG *pul);
...
//
// The function are called by the display driver during acceleration of the
// ClearType or antialiased text rendering.
//

VOID APIENTRY EngCTGetGammaTable(ULONG ulGamma, CONST BYTE **pGammaTable,
                                 CONST BYTE **pInverseGammaTable);

ULONG APIENTRY EngCTGetCurrentGamma(HDEV hdev);
...
LONG APIENTRY HT_ComputeRGBGammaTable(
    _In_ USHORT GammaTableEntries, _In_ USHORT GammaTableType,
    _In_ USHORT RedGamma, _In_ USHORT GreenGamma, _In_ USHORT BlueGamma,
    _Out_writes_bytes_(GammaTableEntries * 3) LPBYTE pGammaTable);

LONG APIENTRY HT_Get8BPPFormatPalette(_Out_opt_ LPPALETTEENTRY pPaletteEntry,
                                      _In_ USHORT RedGamma,
                                      _In_ USHORT GreenGamma,
                                      _In_ USHORT BlueGamma);

LONG APIENTRY HT_Get8BPPMaskPalette(_Out_opt_ LPPALETTEENTRY pPaletteEntry,
                                    _In_ BOOL Use8BPPMaskPal, _In_ BYTE CMYMask,
                                    _In_ USHORT RedGamma,
                                    _In_ USHORT GreenGamma,
                                    _In_ USHORT BlueGamma);
...
LONG APIENTRY HTUI_DeviceColorAdjustment(_In_opt_ LPSTR pDeviceName,
...
__analysis_noreturn ENGAPI VOID APIENTRY EngDebugBreak(VOID);

#if (NTDDI_VERSION >= NTDDI_WINXPSP2)
ENGAPI
VOID APIENTRY EngBugCheckEx(_In_ ULONG BugCheckCode, _In_ ULONG_PTR P1,
                            _In_ ULONG_PTR P2, _In_ ULONG_PTR P3,
                            _In_ ULONG_PTR P4);
#endif // (NTDDI_VERSION >= NTDDI_WINXPSP2)

ENGAPI
VOID APIENTRY EngDebugPrint(_In_z_ PCHAR StandardPrefix,
                            _In_z_ PCHAR DebugMessage, _In_ va_list ap);

ENGAPI
VOID APIENTRY EngQueryPerformanceCounter(_Out_ LONGLONG *pPerformanceCount);

ENGAPI
VOID APIENTRY EngQueryPerformanceFrequency(_Out_ LONGLONG *pFrequency);

__drv_preferredFunction("(see documentation)",
                        "Obsolete, always returns false. ") ENGAPI BOOL APIENTRY
    EngSetPointerTag(_In_ HDEV hdev, _In_ SURFOBJ *psoMask,
                     _In_ SURFOBJ *psoColor, _In_ XLATEOBJ *pxlo,
                     _In_ FLONG fl);
...
#define EngAllocMem(flags, cj, tag)                                            \
  ((PVOID)GlobalAlloc(((flags) & FL_ZERO_MEMORY) ? GPTR : GMEM_FIXED, cj))
...
#define EngAllocPrivateUserMem(psl, cj, tag)                                   \
  ((PVOID)GlobalAlloc(GMEM_FIXED, cj))
...
ENGAPI
_Post_writable_byte_size_(MemSize) PVOID APIENTRY
    EngAllocMem(_In_ ULONG Flags, _In_ ULONG MemSize, _In_ ULONG Tag);

ENGAPI
VOID APIENTRY EngFreeMem(_In_ PVOID Mem);

ENGAPI
_Post_writable_byte_size_(cj) PVOID APIENTRY
    EngAllocUserMem(_In_ SIZE_T cj, _In_ ULONG tag);

ENGAPI
VOID APIENTRY EngFreeUserMem(_In_ PVOID pv);

ENGAPI
_Post_writable_byte_size_(cj) PVOID APIENTRY
    EngAllocPrivateUserMem(_In_ PDD_SURFACE_LOCAL psl, _In_ SIZE_T cj,
                           _In_ ULONG tag);

ENGAPI
VOID EngFreePrivateUserMem(_In_ PDD_SURFACE_LOCAL psl, _In_ PVOID pv);

#if (NTDDI_VERSION >= NTDDI_WINXP)
ENGAPI
HRESULT
EngDxIoctl(ULONG ulIoctl, PVOID pBuffer, ULONG ulBufferSize);
#endif // (NTDDI_VERSION >= NTDDI_WINXP)

ENGAPI
int APIENTRY EngMulDiv(_In_ int a, _In_ int b, _In_ int c);
...
ENGAPI
VOID APIENTRY EngProbeForRead(_In_reads_bytes_(Length) PVOID Address,
#if (NTDDI_VERSION <= NTDDI_WINXP)
                              _In_ ULONG Length,
#else
                              _In_ SIZE_T Length,
...
ENGAPI
VOID APIENTRY EngProbeForReadAndWrite(_Inout_updates_bytes_(Length)
                                          PVOID Address,
#if (NTDDI_VERSION <= NTDDI_WINXP)
                                      _In_ ULONG Length,
#else
...
ENGAPI
HANDLE APIENTRY EngSecureMem(_In_reads_bytes_(Length) PVOID Address,
                             _In_ ULONG Length);

ENGAPI
VOID APIENTRY EngUnsecureMem(_In_ HANDLE hSecure);

ENGAPI
DWORD APIENTRY
EngDeviceIoControl(_In_ HANDLE hDevice, _In_ DWORD dwIoControlCode,
                   _In_reads_bytes_opt_(nInBufferSize) LPVOID lpInBuffer,
                   _In_ DWORD nInBufferSize,
                   _Out_writes_bytes_opt_(nOutBufferSize) LPVOID lpOutBuffer,
                   _In_ DWORD nOutBufferSize, _Out_ LPDWORD lpBytesReturned);
...
#define EngFindImageProcAddress(h, procname)                                   \
  ((PVOID)GetProcAddress(h, procname))
#define EngUnloadImage(h) FreeLibrary((HMODULE)(h))

#else // !USERMODE_DRIVER

ENGAPI
HANDLE APIENTRY EngLoadImage(_In_ LPWSTR pwszDriver);

ENGAPI
PVOID APIENTRY EngFindImageProcAddress(HANDLE hModule, _In_ LPSTR lpProcName);

ENGAPI
VOID APIENTRY EngUnloadImage(_In_ HANDLE hModule);

ENGAPI
PVOID APIENTRY EngMapModule(_In_ HANDLE h, _In_ PULONG pSize);
...
ENGAPI
LPWSTR APIENTRY EngGetPrinterDataFileName(_In_ HDEV hdev);

ENGAPI
LPWSTR APIENTRY EngGetDriverName(_In_ HDEV hdev);
...
ENGAPI
BOOL APIENTRY EngGetType1FontList(_In_ HDEV hdev,
                                  _Out_writes_bytes_opt_(cjType1Buffer)
...
ENGAPI
PVOID APIENTRY EngFindResource(_In_ HANDLE h, _In_ int iName, _In_ int iType,
                               _Out_ PULONG pulSize);

ENGAPI
VOID APIENTRY EngFreeModule(_In_ HANDLE h);

ENGAPI
BOOL APIENTRY EngDeleteFile(_In_ LPWSTR pwszFileName);

ENGAPI
_Success_(return != 0)
    _When_(cjSize != 0, _At_(return, _Out_writes_bytes_(cjSize))) PVOID APIENTRY
    EngMapFile(_In_ LPWSTR pwsz, _In_ ULONG cjSize, _Out_ ULONG_PTR *piFile);
...
__drv_preferredFunction("EngUnmapFontFileFD", "Obsolete") ENGAPI VOID APIENTRY
    EngUnmapFontFile(_In_ ULONG_PTR iFile);

__drv_preferredFunction("EngMapFontFileFD", "Obsolete") ENGAPI BOOL APIENTRY
    EngMapFontFile(_In_ ULONG_PTR iFile,
...
ENGAPI
BOOL APIENTRY EngMapFontFileFD(_In_ ULONG_PTR iFile,
                               _Outptr_result_bytebuffer_(*pcjBuf)
                                   PULONG *ppjBuf,
                               _Out_ ULONG *pcjBuf);
...
ENGAPI
VOID APIENTRY EngAcquireSemaphore(_In_ HSEMAPHORE hsem);

ENGAPI
VOID APIENTRY EngReleaseSemaphore(_In_ HSEMAPHORE hsem);

ENGAPI
VOID APIENTRY EngDeleteSemaphore(_In_ HSEMAPHORE hsem);

#if (NTDDI_VERSION >= NTDDI_LONGHORN)
ENGAPI
VOID APIENTRY EngAcquireSemaphoreShared(_In_ HSEMAPHORE hsem);

ENGAPI
BOOL APIENTRY EngAcquireSemaphoreNoWait(_In_ HSEMAPHORE hsem);

ENGAPI
BOOL APIENTRY EngAcquireSemaphoreSharedNoWait(_In_ HSEMAPHORE hsem);
...
#if (NTDDI_VERSION >= NTDDI_LONGHORN)
ENGAPI
BOOL APIENTRY EngIsSemaphoreSharedByCurrentThread(_In_ HSEMAPHORE hsem);
#endif
...
ENGAPI
BOOL APIENTRY EngInitializeSafeSemaphore(_Out_ ENGSAFESEMAPHORE *pssem);

ENGAPI
VOID APIENTRY EngDeleteSafeSemaphore(_Inout_ ENGSAFESEMAPHORE *pssem);

ENGAPI
VOID APIENTRY EngMultiByteToUnicodeN(
    _Out_writes_bytes_to_(MaxBytesInUnicodeString, *BytesInUnicodeString)
        LPWSTR UnicodeString,
    _In_ ULONG MaxBytesInUnicodeString, _Out_opt_ PULONG BytesInUnicodeString,
    _In_reads_bytes_(BytesInMultiByteString) PCHAR MultiByteString,
    _In_ ULONG BytesInMultiByteString);

ENGAPI
VOID APIENTRY EngUnicodeToMultiByteN(
    _Out_writes_bytes_to_(MaxBytesInMultiByteString, *BytesInMultiByteString)
...
ENGAPI
BOOL APIENTRY EngGetPrinterDriver(_In_ HANDLE hPrinter,
                                  _In_opt_ LPWSTR pEnvironment,
                                  _In_ DWORD dwLevel,
                                  _Out_writes_bytes_opt_(cbBuf)
                                      BYTE *lpbDrvInfo,
                                  _In_ DWORD cbBuf, _Out_ DWORD *pcbNeeded);

ENGAPI
DWORD APIENTRY EngGetPrinterData(_In_ HANDLE hPrinter, _In_ LPWSTR pValueName,
                                 _Out_opt_ LPDWORD pType,
                                 _Out_writes_bytes_opt_(nSize) LPBYTE pData,
                                 _In_ DWORD nSize, _Out_ LPDWORD pcbNeeded);

ENGAPI
DWORD APIENTRY EngSetPrinterData(_In_ HANDLE hPrinter, _In_ LPWSTR pType,
                                 _In_ DWORD dwType,
                                 _In_reads_bytes_(cjPrinterData)
                                     LPBYTE lpbPrinterData,
                                 _In_ DWORD cjPrinterData);

ENGAPI
BOOL APIENTRY EngGetForm(_In_ HANDLE hPrinter, _In_ LPWSTR pFormName,
                         _In_ DWORD Level,
                         _In_reads_bytes_opt_(cbBuf) LPBYTE pForm,
                         _In_ DWORD cbBuf, _Out_ LPDWORD pcbNeeded);

ENGAPI
BOOL APIENTRY EngWritePrinter(_In_ HANDLE hPrinter,
                              _In_reads_bytes_(cbBuf) LPVOID pBuf,
                              _In_ DWORD cbBuf, _Out_ LPDWORD pcWritten);

ENGAPI
BOOL APIENTRY EngGetPrinter(_In_ HANDLE hPrinter, _In_ DWORD dwLevel,
                            _Out_writes_bytes_opt_(cbBuf) LPBYTE pPrinter,
                            _In_ DWORD cbBuf, _Out_ LPDWORD pcbNeeded);

ENGAPI
BOOL APIENTRY EngEnumForms(_In_ HANDLE hPrinter, _In_ DWORD Level,
...
ENGAPI
VOID APIENTRY EngSort(_Inout_updates_bytes_(c *cjElem) PBYTE pjBuf,
                      _In_ ULONG c, _In_ ULONG cjElem, _In_ SORTCOMP pfnComp);
...
ENGAPI
VOID APIENTRY EngQueryLocalTime(_Out_ PENG_TIME_FIELDS);

_Success_(return != 0) ENGAPI FD_GLYPHSET *APIENTRY
    EngComputeGlyphSet(_In_ INT nCodePage, _In_ INT nFirstChar,
                       _In_ INT cChars);

ENGAPI
INT APIENTRY
EngMultiByteToWideChar(_In_ UINT CodePage,
                       _Out_writes_bytes_opt_(BytesInWideCharString)
                           _Null_terminated_ LPWSTR WideCharString,
                       _In_ INT BytesInWideCharString,
                       _In_reads_bytes_opt_(BytesInMultiByteString)
                           _Null_terminated_ LPSTR MultiByteString,
                       _In_ INT BytesInMultiByteString);

ENGAPI
INT APIENTRY
EngWideCharToMultiByte(_In_ UINT CodePage,
                       _In_reads_bytes_opt_(BytesInWideCharString)
                           _Null_terminated_ LPWSTR WideCharString,
                       _In_ INT BytesInWideCharString,
                       _Out_writes_bytes_opt_(BytesInMultiByteString)
                           _Null_terminated_ LPSTR MultiByteString,
                       _In_ INT BytesInMultiByteString);
...
ENGAPI
LARGE_INTEGER APIENTRY EngQueryFileTimeStamp(_In_ LPWSTR pwsz);

_Success_(return) ENGAPI BOOL APIENTRY
    EngGetFileChangeTime(_In_ HANDLE h, _Out_ LARGE_INTEGER *pChangeTime);

_Success_(return) ENGAPI BOOL APIENTRY
    EngGetFilePath(_In_ HANDLE h, _At_((PWSTR)pDest, _Out_writes_(MAX_PATH + 1))
                                      WCHAR (*pDest)[MAX_PATH + 1]);

#if defined(_M_AMD64) && (NTDDI_VERSION >= NTDDI_VISTA) &&                     \
    !defined(_CONTRACT_GEN)

_Check_return_ _Success_(((pBuffer != NULL && cjBufferSize != 0) &&
                          return == 1) ||
                         ((pBuffer == NULL || cjBufferSize == 0) && return > 0))
    _When_(pBuffer != NULL && cjBufferSize != 0 && return == 1,
           _Kernel_float_saved_ _At_(
               *pBuffer,
               _Post_valid_ _Kernel_acquires_resource_(EngFloatState)))
        _On_failure_(_Post_satisfies_(return == 0)) __forceinline ULONG
#pragma warning(                                                               \
    suppress : 6001 6101 28104) // Suppress PREfast warnings for not acquiring
                                // EngFloatState and filling out pBuffer
    EngSaveFloatingPointState(
        _At_(*pBuffer, _Kernel_requires_resource_not_held_(EngFloatState))

            _Out_writes_bytes_opt_(cjBufferSize) VOID *pBuffer,
        _In_ ULONG cjBufferSize) {
  return ((((pBuffer) == NULL) || ((cjBufferSize) == 0)) ? 8 : TRUE);
}

_Check_return_ _Success_(return) _Kernel_float_restored_
    _At_(*pBuffer,
         _Kernel_requires_resource_held_(EngFloatState)
             _Kernel_releases_resource_(EngFloatState)) __forceinline BOOL
#pragma warning(                                                               \
    suppress                                                                   \
    : 28103) // Suppress PREfast warning for not releasing EngFloatState
    EngRestoreFloatingPointState(_In_reads_(_Inexpressible_(statesize))
                                     VOID *pBuffer) {
...
_Check_return_ _Success_(((pBuffer != NULL && cjBufferSize != 0) &&
    ...
    _When_(pBuffer != NULL && cjBufferSize != 0 && return == 1,
           _Kernel_float_saved_ _At_(
               *pBuffer,
               _Post_valid_ _Kernel_acquires_resource_(EngFloatState)))
        _On_failure_(_Post_satisfies_(return == 0)) ENGAPI ULONG APIENTRY
    EngSaveFloatingPointState(
        _At_(*pBuffer, _Kernel_requires_resource_not_held_(EngFloatState))
            _Out_writes_bytes_opt_(cjBufferSize) VOID *pBuffer,
...
_Check_return_ _Success_(return) _Kernel_float_restored_
    _At_(*pBuffer,
         _Kernel_requires_resource_held_(EngFloatState)
             _Kernel_releases_resource_(EngFloatState)) ENGAPI BOOL APIENTRY
    EngRestoreFloatingPointState(_In_reads_(_Inexpressible_(statesize))
                                     VOID *pBuffer);
...
ENGAPI
PDD_SURFACE_LOCAL APIENTRY EngLockDirectDrawSurface(_In_ HANDLE hSurface);

ENGAPI
BOOL APIENTRY EngUnlockDirectDrawSurface(_In_ PDD_SURFACE_LOCAL pSurface);
...
ENGAPI
BOOL APIENTRY EngDeleteEvent(_In_ PEVENT pEvent);

ENGAPI
BOOL APIENTRY EngCreateEvent(_Out_ PEVENT *ppEvent);

ENGAPI
BOOL APIENTRY EngUnmapEvent(_In_ PEVENT pEvent);

ENGAPI
PEVENT APIENTRY EngMapEvent(_In_ HDEV hDev, _In_ HANDLE hUserObject,
                            _Reserved_ PVOID Reserved1,
                            _Reserved_ PVOID Reserved2,
                            _Reserved_ PVOID Reserved3);

ENGAPI
BOOL APIENTRY EngWaitForSingleObject(_In_ PEVENT pEvent,
                                     _In_ PLARGE_INTEGER pTimeOut);

ENGAPI
LONG APIENTRY EngSetEvent(_In_ PEVENT pEvent);

#if (NTDDI_VERSION >= NTDDI_WINXP)
ENGAPI
VOID APIENTRY EngClearEvent(_In_ PEVENT pEvent);

ENGAPI
LONG APIENTRY EngReadStateEvent(_In_ PEVENT pEvent);
#endif // (NTDDI_VERSION >= NTDDI_WINXP)
...
_Success_(return != nullptr) ENGAPI
    _Post_writable_byte_size_(*pulSize) PVOID APIENTRY
    EngFntCacheLookUp(_In_ ULONG FastCheckSum, _Out_ ULONG *pulSize);

#ifdef USERMODE_DRIVER
ENGAPI
BOOL WINAPI EngFntCacheFlush(_In_ PVOID OffsetBuffer, _In_ BOOL DiscardContent);
#endif

ENGAPI
_Post_writable_byte_size_(ulSize) PVOID APIENTRY
    EngFntCacheAlloc(_In_ ULONG FastCheckSum, _In_ ULONG ulSize);

ENGAPI
VOID APIENTRY EngFntCacheFault(_In_ ULONG ulFastCheckSum,
...
BOOL APIENTRY EngQueryDeviceAttribute(_In_ HDEV hdev,
                                      ...
                                      _In_ ULONG ulInSize, _Out_ VOID *pvOut,
                                      _Out_writes_bytes_(ulOutSize)
                                          ULONG ulOutSize);
...
DECLSPEC_DEPRECATED_DDK
BOOL APIENTRY EngQueryEMFInfo(_In_ HDEV hdev, _Out_ EMFINFO *pEMFInfo);
...
typedef DHPDEV(APIENTRY *PFN_DrvEnablePDEV)(PDEVMODEW, LPWSTR, ULONG, HSURF *,
                                            ULONG, GDIINFO *, ULONG, PDEVINFO,
                                            HDEV, LPWSTR, HANDLE);
...
typedef BOOL(APIENTRY *PFN_DrvTextOut)(SURFOBJ *, STROBJ *, FONTOBJ *,
                                       CLIPOBJ *, RECTL *, RECTL *, BRUSHOBJ *,
                                       BRUSHOBJ *, POINTL *, MIX);
typedef BOOL(APIENTRY *PFN_DrvStretchBlt)(SURFOBJ *, SURFOBJ *, SURFOBJ *,
                                          CLIPOBJ *, XLATEOBJ *,
                                          COLORADJUSTMENT *, POINTL *, RECTL *,
                                          RECTL *, POINTL *, ULONG);
typedef BOOL(APIENTRY *PFN_DrvStretchBltROP)(SURFOBJ *, SURFOBJ *, SURFOBJ *,
                                             CLIPOBJ *, XLATEOBJ *,
                                             COLORADJUSTMENT *, POINTL *,
                                             RECTL *, RECTL *, POINTL *, ULONG,
                                             BRUSHOBJ *, ROP4);
typedef BOOL(APIENTRY *PFN_DrvTransparentBlt)(SURFOBJ *, SURFOBJ *, CLIPOBJ *,
                                              XLATEOBJ *, RECTL *, RECTL *,
                                              ULONG, ULONG);
typedef BOOL(APIENTRY *PFN_DrvPlgBlt)(SURFOBJ *, SURFOBJ *, SURFOBJ *,
                                      CLIPOBJ *, XLATEOBJ *, COLORADJUSTMENT *,
                                      POINTL *, POINTFIX *, RECTL *, POINTL *,
                                      ULONG);
typedef BOOL(APIENTRY *PFN_DrvBitBlt)(SURFOBJ *, SURFOBJ *, SURFOBJ *,
                                      CLIPOBJ *, XLATEOBJ *, RECTL *, POINTL *,
                                      POINTL *, BRUSHOBJ *, POINTL *, ROP4);
typedef BOOL(APIENTRY *PFN_DrvRealizeBrush)(BRUSHOBJ *, SURFOBJ *, SURFOBJ *,
                                            SURFOBJ *, XLATEOBJ *, ULONG);
typedef BOOL(APIENTRY *PFN_DrvCopyBits)(SURFOBJ *, SURFOBJ *, CLIPOBJ *,
...
typedef BOOL(APIENTRY *PFN_DrvSetPalette)(DHPDEV, PALOBJ *, FLONG, ULONG,
                                          ULONG);
typedef ULONG(APIENTRY *PFN_DrvEscape)(SURFOBJ *, ULONG, ULONG, PVOID, ULONG,
                                       PVOID);
typedef ULONG(APIENTRY *PFN_DrvDrawEscape)(SURFOBJ *, ULONG, CLIPOBJ *, RECTL *,
                                           ULONG, PVOID);
#if (NTDDI_VERSION >= NTDDI_WINXP)
typedef PIFIMETRICS(APIENTRY *PFN_DrvQueryFont)(DHPDEV, ULONG_PTR, ULONG,
                                                ULONG_PTR *);
typedef PVOID(APIENTRY *PFN_DrvQueryFontTree)(DHPDEV, ULONG_PTR, ULONG, ULONG,
                                              ULONG_PTR *);
#else
typedef PIFIMETRICS(APIENTRY *PFN_DrvQueryFont)(DHPDEV, ULONG_PTR, ULONG,
                                                ULONG *);
typedef PVOID(APIENTRY *PFN_DrvQueryFontTree)(DHPDEV, ULONG_PTR, ULONG, ULONG,
                                              ULONG *);
#endif // (NTDDI_VERSION >= NTDDI_WINXP)
typedef LONG(APIENTRY *PFN_DrvQueryFontData)(DHPDEV, FONTOBJ *, ULONG, HGLYPH,
...
#if (_WIN32_WINNT >= _WIN32_WINNT_WIN2K)
typedef HFF(APIENTRY *PFN_DrvLoadFontFile)(ULONG, ULONG_PTR *, PVOID *, ULONG *,
                                           DESIGNVECTOR *, ULONG, ULONG);
#else
typedef HFF(APIENTRY *PFN_DrvLoadFontFile)(ULONG, ULONG_PTR *, PVOID *, ULONG *,
                                           ULONG);
#endif
typedef BOOL(APIENTRY *PFN_DrvUnloadFontFile)(ULONG_PTR);
typedef ULONG(APIENTRY *PFN_DrvSetPointerShape)(SURFOBJ *, SURFOBJ *, SURFOBJ *,
                                                XLATEOBJ *, LONG, LONG, LONG,
                                                LONG, RECTL *, FLONG);
...
typedef BOOL(APIENTRY *PFN_DrvStartDoc)(SURFOBJ *pso, LPWSTR pwszDocName,
                                        DWORD dwJobId);
...
typedef BOOL(APIENTRY *PFN_DrvLineTo)(SURFOBJ *, CLIPOBJ *, BRUSHOBJ *, LONG,
                                      LONG, LONG, LONG, RECTL *, MIX);
typedef BOOL(APIENTRY *PFN_DrvStrokePath)(SURFOBJ *, PATHOBJ *, CLIPOBJ *,
                                          XFORMOBJ *, BRUSHOBJ *, POINTL *,
                                          LINEATTRS *, MIX);
typedef BOOL(APIENTRY *PFN_DrvFillPath)(SURFOBJ *, PATHOBJ *, CLIPOBJ *,
                                        BRUSHOBJ *, POINTL *, MIX, FLONG);
typedef BOOL(APIENTRY *PFN_DrvStrokeAndFillPath)(SURFOBJ *, PATHOBJ *,
                                                 CLIPOBJ *, XFORMOBJ *,
                                                 BRUSHOBJ *, LINEATTRS *,
                                                 BRUSHOBJ *, POINTL *, MIX,
                                                 FLONG);
typedef BOOL(APIENTRY *PFN_DrvPaint)(SURFOBJ *, CLIPOBJ *, BRUSHOBJ *, POINTL *,
                                     MIX);
...
typedef ULONG_PTR(APIENTRY *PFN_DrvSaveScreenBits)(SURFOBJ *, ULONG, ULONG_PTR,
                                                   RECTL *);
typedef ULONG(APIENTRY *PFN_DrvGetModes)(HANDLE, ULONG, DEVMODEW *);
typedef LONG(APIENTRY *PFN_DrvQueryTrueTypeTable)(ULONG_PTR, ULONG, ULONG,
                                                  PTRDIFF, ULONG, BYTE *,
                                                  PBYTE *, ULONG *);
typedef LONG(APIENTRY *PFN_DrvQueryTrueTypeSection)(ULONG, ULONG, ULONG,
                                                    HANDLE *, PTRDIFF *);
typedef LONG(APIENTRY *PFN_DrvQueryTrueTypeOutline)(DHPDEV, FONTOBJ *, HGLYPH,
                                                    BOOL, GLYPHDATA *, ULONG,
                                                    TTPOLYGONHEADER *);
...
typedef BOOL(APIENTRY *PFN_DrvQueryAdvanceWidths)(DHPDEV, FONTOBJ *, ULONG,
                                                  HGLYPH *, PVOID, ULONG);
typedef ULONG(APIENTRY *PFN_DrvFontManagement)(SURFOBJ *, FONTOBJ *, ULONG,
                                               ULONG, PVOID, ULONG, PVOID);
typedef BOOL(APIENTRY *PFN_DrvSetPixelFormat)(SURFOBJ *, LONG, HWND);
typedef LONG(APIENTRY *PFN_DrvDescribePixelFormat)(DHPDEV, LONG, ULONG,
...
typedef BOOL(APIENTRY *PFN_DrvEnableDirectDraw)(DHPDEV, DD_CALLBACKS *,
                                                DD_SURFACECALLBACKS *,
                                                DD_PALETTECALLBACKS *);
typedef VOID(APIENTRY *PFN_DrvDisableDirectDraw)(DHPDEV);
typedef BOOL(APIENTRY *PFN_DrvGetDirectDrawInfo)(DHPDEV, DD_HALINFO *, DWORD *,
                                                 VIDEOMEMORY *, DWORD *,
                                                 DWORD *);
typedef HANDLE(APIENTRY *PFN_DrvIcmCreateColorTransform)(DHPDEV,
                                                         LPLOGCOLORSPACEW,
                                                         LPVOID, ULONG, LPVOID,
                                                         ULONG, LPVOID, ULONG,
                                                         DWORD);
typedef BOOL(APIENTRY *PFN_DrvIcmDeleteColorTransform)(DHPDEV, HANDLE);
typedef BOOL(APIENTRY *PFN_DrvIcmCheckBitmapBits)(DHPDEV, HANDLE, SURFOBJ *,
                                                  PBYTE);
typedef BOOL(APIENTRY *PFN_DrvIcmSetDeviceGammaRamp)(DHPDEV, ULONG, LPVOID);
typedef BOOL(APIENTRY *PFN_DrvAlphaBlend)(SURFOBJ *, SURFOBJ *, CLIPOBJ *,
                                          XLATEOBJ *, PRECTL, PRECTL,
                                          BLENDOBJ *);
typedef BOOL(APIENTRY *PFN_DrvGradientFill)(SURFOBJ *, CLIPOBJ *, XLATEOBJ *,
                                            TRIVERTEX *, ULONG, PVOID, ULONG,
                                            RECTL *, POINTL *, ULONG);
typedef BOOL(APIENTRY *PFN_DrvQueryDeviceSupport)(SURFOBJ *, XLATEOBJ *,
                                                  XFORMOBJ *, ULONG, ULONG,
                                                  PVOID, ULONG, PVOID);
typedef HBITMAP(APIENTRY *PFN_DrvDeriveSurface)(DD_DIRECTDRAW_GLOBAL *,
                                                DD_SURFACE_LOCAL *);
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
typedef LONG(APIENTRY *PFN_DrvRenderHint)(DHPDEV dhpdev, ULONG NotifyCode,
                                          SIZE_T Length,
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
#if (NTDDI_VERSION >= NTDDI_WIN7)
typedef HBITMAP(APIENTRY *PFN_DrvCreateDeviceBitmapEx)(DHPDEV, SIZEL, ULONG,
                                                       DWORD, DHSURF, DWORD,
                                                       DWORD, HANDLE *);
#else
typedef HBITMAP(APIENTRY *PFN_DrvCreateDeviceBitmapEx)(DHPDEV, SIZEL, ULONG,
...
typedef BOOL(APIENTRY *PFN_DrvAssociateSharedSurface)(SURFOBJ *, HANDLE, HANDLE,
                                                      SIZEL);
typedef NTSTATUS(APIENTRY *PFN_DrvSynchronizeRedirectionBitmaps)(DHPDEV,
                                                                 UINT64 *);
typedef BOOL(APIENTRY *PFN_DrvAccumulateD3DDirtyRect)(
    SURFOBJ *, CDDDXGK_REDIRBITMAPPRESENTINFO *);
typedef BOOL(APIENTRY *PFN_DrvStartDxInterop)(SURFOBJ *, BOOL,
                                              PVOID KernelModeDeviceHandle);
