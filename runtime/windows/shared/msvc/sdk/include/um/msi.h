...
#define NTDDI_WINTHRESHOLD 0x06040000
     ...
     (defined(NTDDI_VERSION) && NTDDI_VERSION >= NTDDI_WIN7))
...
#define _WIN32_MSI 200
       ...
       (defined(NTDDI_VERSION) && NTDDI_VERSION >= NTDDI_WIN2K))
...
typedef enum tagINSTALLMESSAGE {
  INSTALLMESSAGE_FATALEXIT =
      0x00000000L, // premature termination, possibly fatal OOM
  ...
  INSTALLMESSAGE_FILESINUSE =
      0x05000000L, // list of files in use that need to be replaced
  INSTALLMESSAGE_RESOLVESOURCE =
      0x06000000L, // request to determine a valid source location
  INSTALLMESSAGE_OUTOFDISKSPACE =
      0x07000000L, // insufficient disk space message
  INSTALLMESSAGE_ACTIONSTART =
      0x08000000L, // start of action: action name & description
  INSTALLMESSAGE_ACTIONDATA =
      0x09000000L, // formatted data associated with individual action item
  INSTALLMESSAGE_PROGRESS =
      0x0A000000L, // progress gauge info: units so far, total
  INSTALLMESSAGE_COMMONDATA =
      0x0B000000L, // product info for dialog: language Id, dialog caption
  INSTALLMESSAGE_INITIALIZE =
      0x0C000000L, // sent prior to UI initialization, no string data
  INSTALLMESSAGE_TERMINATE =
      0x0D000000L, // sent after UI termination, no string data
  INSTALLMESSAGE_SHOWDIALOG =
      0x0E000000L, // sent prior to display or authored dialog or wizard
#if (_WIN32_MSI >= 500)
  INSTALLMESSAGE_PERFORMANCE =
      0x0F000000L, // log only, to log performance number like action time
#endif             // (_WIN32_MSI >= 500)
#if (_WIN32_MSI >= 400)
  INSTALLMESSAGE_RMFILESINUSE =
      0x19000000L, // the list of apps that the user can request Restart Manager
                   // to shut down and restart
#endif             // (_WIN32_MSI >= 400)
#if (_WIN32_MSI >= 450)
  INSTALLMESSAGE_INSTALLSTART =
      0x1A000000L, // sent prior to server-side install of a product
...
typedef enum tagINSTALLUILEVEL {
  ...
  INSTALLUILEVEL_ENDDIALOG =
      0x80, // display success/failure dialog at end of install
  INSTALLUILEVEL_PROGRESSONLY = 0x40, // display only progress dialog
  INSTALLUILEVEL_HIDECANCEL =
      0x20, // do not display the cancel button in basic UI
  INSTALLUILEVEL_SOURCERESONLY =
      0x100, // force display of source resolution even if quiet
#if (_WIN32_MSI >= 500)
...
typedef enum tagINSTALLSTATE {
  ...
  INSTALLSTATE_REMOVED =
      1, // component being removed (action state, not settable)
  INSTALLSTATE_ABSENT =
      2, // uninstalled (or action state absent but clients remain)
...
{ REINSTALLMODE_REPAIR = 0x00000001,      // Reserved bit - currently ignored
  ...
  REINSTALLMODE_FILEOLDERVERSION =
      0x00000004, // Reinstall if file is missing, or older version
  REINSTALLMODE_FILEEQUALVERSION =
      0x00000008, // Reinstall if file is missing, or equal or older version
  REINSTALLMODE_FILEEXACT =
      0x00000010, // Reinstall if file is missing, or not exact version
  REINSTALLMODE_FILEVERIFY =
      0x00000020, // checksum executables, reinstall if missing or corrupt
  REINSTALLMODE_FILEREPLACE =
      0x00000040, // Reinstall all files, regardless of version
...
typedef enum tagINSTALLOGMODE // bit flags for use with MsiEnableLog and
                              // MsiSetExternalUI
{ INSTALLLOGMODE_FATALEXIT = (1 << (INSTALLMESSAGE_FATALEXIT >> 24)),
  ...
  INSTALLLOGMODE_PROPERTYDUMP =
      (1 << (INSTALLMESSAGE_PROGRESS >> 24)),                        // log only
  INSTALLLOGMODE_VERBOSE = (1 << (INSTALLMESSAGE_INITIALIZE >> 24)), // log only
  INSTALLLOGMODE_EXTRADEBUG =
      (1 << (INSTALLMESSAGE_TERMINATE >> 24)), // log only
  INSTALLLOGMODE_LOGONLYONERROR =
      (1 << (INSTALLMESSAGE_SHOWDIALOG >> 24)), // log only
#if (_WIN32_MSI >= 500)
  INSTALLLOGMODE_LOGPERFORMANCE =
      (1 << (INSTALLMESSAGE_PERFORMANCE >> 24)), // log only
#endif                                           // (_WIN32_MSI >= 500)
  INSTALLLOGMODE_PROGRESS =
      (1 << (INSTALLMESSAGE_PROGRESS >> 24)), // external handler only
  INSTALLLOGMODE_INITIALIZE =
      (1 << (INSTALLMESSAGE_INITIALIZE >> 24)), // external handler only
  INSTALLLOGMODE_TERMINATE =
      (1 << (INSTALLMESSAGE_TERMINATE >> 24)), // external handler only
  INSTALLLOGMODE_SHOWDIALOG =
      (1 << (INSTALLMESSAGE_SHOWDIALOG >> 24)), // external handler only
  INSTALLLOGMODE_FILESINUSE =
      (1 << (INSTALLMESSAGE_FILESINUSE >> 24)), // external handler only
#if (_WIN32_MSI >= 400)
  INSTALLLOGMODE_RMFILESINUSE =
      (1 << (INSTALLMESSAGE_RMFILESINUSE >> 24)), // external handler only
#endif                                            // (_WIN32_MSI >= 400)
#if (_WIN32_MSI >= 450)
  INSTALLLOGMODE_INSTALLSTART =
      (1 << (INSTALLMESSAGE_INSTALLSTART >> 24)), // external/embedded only
  INSTALLLOGMODE_INSTALLEND =
      (1 << (INSTALLMESSAGE_INSTALLEND >> 24)), // external/embedded only
#endif                                          // (_WIN32_MSI >= 450)
} INSTALLLOGMODE;
...
#if (_WIN32_MSI >= 150)
  INSTALLMODE_NODETECTION_ANY = -4, // provide any, if available, supported
                                    // internally for MsiProvideAssembly
#endif
...
typedef enum tagMSIINSTALLCONTEXT {
  ...
  MSIINSTALLCONTEXT_ALL =
      (MSIINSTALLCONTEXT_USERMANAGED | MSIINSTALLCONTEXT_USERUNMANAGED |
       MSIINSTALLCONTEXT_MACHINE),      // All contexts. OR of all valid values
...
#define MAX_FEATURE_CHARS                                                      \
  38 // maximum chars in feature name (same as string GUID)
...
typedef enum tagSCRIPTFLAGS {
  SCRIPTFLAGS_CACHEINFO =
      0x00000001L, // set if the icons need to be created/ removed
  SCRIPTFLAGS_SHORTCUTS =
      0x00000004L, // set if the shortcuts needs to be created/ deleted
  SCRIPTFLAGS_MACHINEASSIGN =
      0x00000008L, // set if product to be assigned to machine
  SCRIPTFLAGS_REGDATA_CNFGINFO =
      0x00000020L, // set if the product cnfg mgmt. registry data needs to be
                   // written/ removed
  SCRIPTFLAGS_VALIDATE_TRANSFORMS_LIST = 0x00000040L,
#if (_WIN32_MSI >= 110)
  SCRIPTFLAGS_REGDATA_CLASSINFO =
      0x00000080L, // set if COM classes related app info needs to be  created/
                   // deleted
  SCRIPTFLAGS_REGDATA_EXTENSIONINFO =
      0x00000100L, // set if extension related app info needs to be  created/
                   // deleted
  SCRIPTFLAGS_REGDATA_APPINFO =
      SCRIPTFLAGS_REGDATA_CLASSINFO |
      SCRIPTFLAGS_REGDATA_EXTENSIONINFO, // for source level backward
                                         // compatibility
#else                                    //_WIN32_MSI == 100
...
typedef enum tagADVERTISEFLAGS {
  ADVERTISEFLAGS_MACHINEASSIGN =
      0,                         // set if the product is to be machine assigned
...
typedef enum tagMSIARCHITECTUREFLAGS {
  MSIARCHITECTUREFLAGS_X86 =
      0x00000001L, // set if creating the script for i386 platform
  MSIARCHITECTUREFLAGS_IA64 =
      0x00000002L, // set if creating the script for IA64 platform
  MSIARCHITECTUREFLAGS_AMD64 =
      0x00000004L, // set if creating the script for AMD64 platform
  MSIARCHITECTUREFLAGS_ARM =
      0x00000008L // set if creating the script for ARM platform
} MSIARCHITECTUREFLAGS;

typedef enum tagMSIOPENPACKAGEFLAGS {
  MSIOPENPACKAGEFLAGS_IGNOREMACHINESTATE =
      0x00000001L, // ignore the machine state when creating the engine
} MSIOPENPACKAGEFLAGS;

typedef enum tagMSIADVERTISEOPTIONFLAGS {
  MSIADVERTISEOPTIONFLAGS_INSTANCE =
      0x00000001L, // set if advertising a new instance
...
INSTALLUILEVEL WINAPI
MsiSetInternalUI(_In_ INSTALLUILEVEL dwUILevel, // UI level
...
UINT WINAPI MsiSetExternalUIRecord(
    _In_opt_ INSTALLUI_HANDLER_RECORD
        puiHandler,             // for progress and error handling
    _In_ DWORD dwMessageFilter, // bit flags designating messages to handle
    _In_opt_ LPVOID pvContext,  // application context
    _Out_opt_ PINSTALLUI_HANDLER_RECORD
        ppuiPrevHandler); // pointer to the previously set handler
...
INSTALLSTATE WINAPI MsiQueryProductStateA(_In_ LPCSTR szProduct);
INSTALLSTATE WINAPI MsiQueryProductStateW(_In_ LPCWSTR szProduct);
#ifdef UNICODE
...
UINT WINAPI MsiConfigureProductA(
    ...
    _In_ INSTALLSTATE
        eInstallState); // local/source/default/absent/lock/uncache
...
// Output reg and shortcut info to script file for specified architecture for
// Assign or Publish If dwPlatform is 0, then the script is created based on the
// current platform (behavior of MsiAdvertiseProduct) If dwPlatform specifies a
// platform, then the script is created as if the current platform is the
//    platform specified in dwPlatform
// If dwOptions includes MSIADVERTISEOPTIONFLAGS_INSTANCE, then a new instance
// is advertised. Use of
//    this option requires that szTransforms include the instance transform that
...
UINT WINAPI MsiAdvertiseProductExA(
    ...
    _In_ DWORD
        dwPlatform, // the MSIARCHITECTUREFLAGS that control for which platform
                    //   to create the script, ignored if szScriptfilePath is
                    //   NULL
    _In_ DWORD dwOptions); // the MSIADVERTISEOPTIONFLAGS that specify extra
...
UINT WINAPI MsiProcessAdvertiseScriptA(
    ...
    _In_opt_ LPCSTR
        szIconFolder, // optional path to folder for icon files and transforms
...
UINT WINAPI MsiGetProductInfoFromScriptA(
    ...
    _Out_writes_opt_(MAX_GUID_CHARS + 1)
...
USERINFOSTATE WINAPI MsiGetUserInfoA(
    ...
    _Out_writes_opt_(*pcchSerialBuf)
...
USERINFOSTATE WINAPI MsiGetUserInfoW(
    ...
    _Inout_opt_ LPDWORD pcchUserNameBuf, // in/out buffer character count
    _Out_writes_opt_(*pcchOrgNameBuf)
...
UINT WINAPI
MsiCollectUserInfoA(_In_ LPCSTR szProduct); // product code, string GUID
UINT WINAPI
MsiCollectUserInfoW(_In_ LPCWSTR szProduct); // product code, string GUID
#ifdef UNICODE
...
UINT WINAPI MsiApplyPatchA(
    ...
    _In_opt_ LPCSTR
        szInstallPackage, // location of package for install to patch <optional>
...
UINT WINAPI MsiEnumPatchesA(_In_ LPCSTR szProduct, _In_ DWORD iPatchIndex,
                            _Out_writes_(MAX_GUID_CHARS + 1) LPSTR lpPatchBuf,
                            _Out_writes_(*pcchTransformsBuf)
                                LPSTR lpTransformsBuf,
                            _Inout_ LPDWORD pcchTransformsBuf);
UINT WINAPI MsiEnumPatchesW(_In_ LPCWSTR szProduct, _In_ DWORD iPatchIndex,
...
UINT WINAPI MsiRemovePatchesA(
    _In_ LPCSTR
        szPatchList, // semi-colon delimited list of patches to remove; patch
                     // can be referenced by patch package path or Patch GUID
    _In_ LPCSTR
        szProductCode, // ProductCode GUID of product with patch to remove
    _In_ INSTALLTYPE eUninstallType, // type of patch uninstall to perform. Must
                                     // be INSTALLTYPE_SINGLE_INSTANCE.
    _In_opt_ LPCSTR szPropertyList); // command line property settings using
...
UINT WINAPI MsiRemovePatchesW(
    ...
    _In_ INSTALLTYPE eUninstallType, // type of patch uninstall to perform. Must
                                     // be INSTALLTYPE_SINGLE_INSTANCE.
    _In_opt_ LPCWSTR szPropertyList); // command line property settings using
                                      // PROPERTY=VALUE pairs
#ifdef UNICODE
...
// patch files, XML files, and/or XML blobs. Patches already applied to the
...
UINT WINAPI MsiDeterminePatchSequenceA(
    ...
    _In_ MSIINSTALLCONTEXT
        dwContext,         // Installation context we're interested in
    _In_ DWORD cPatchInfo, // Number of patches in the array
    _Inout_updates_(cPatchInfo) PMSIPATCHSEQUENCEINFOA
        pPatchInfo); // Array of patch sequence information data
...
// Determines which patches apply and what sequence for the provided set of
// patch files, XML files, and/or XML blobs. Does not take into account either
// products or patches already installed on the machine.

UINT WINAPI MsiDetermineApplicablePatchesA(
    _In_ LPCSTR
        szProductPackagePath, // Fully qualified path to a product MSI database
    _In_ DWORD cPatchInfo,    // Number of patches in the array
    _Inout_updates_(cPatchInfo) PMSIPATCHSEQUENCEINFOA
        pPatchInfo); // Array of patch sequence information data
...
// Enumerates all patches applied for a single product or all products based
// upon enumeration context and filter.

UINT WINAPI MsiEnumPatchesExA(
    _In_opt_ LPCSTR
        szProductCode,         // Enumerate patches on instances of this product
    ...
    _Out_opt_ MSIINSTALLCONTEXT
        *pdwTargetProductContext, // Enumerated patch's context
    _Out_writes_opt_(*pcchTargetUserSid)
        LPSTR szTargetUserSid, // Enumerated patch's user account
    _Inout_opt_ LPDWORD
        pcchTargetUserSid); // in/out character count of szTargetUserSid
...
INSTALLSTATE WINAPI MsiQueryFeatureStateA(_In_ LPCSTR szProduct,
                                          _In_ LPCSTR szFeature);
INSTALLSTATE WINAPI MsiQueryFeatureStateW(_In_ LPCWSTR szProduct,
...
#if (_WIN32_MSI >= 300)
// Retrieve the installed state for a product feature in a particular product
// instance
...
INSTALLSTATE WINAPI MsiUseFeatureA(_In_ LPCSTR szProduct,
                                   _In_ LPCSTR szFeature);
INSTALLSTATE WINAPI MsiUseFeatureW(_In_ LPCWSTR szProduct,
                                   _In_ LPCWSTR szFeature);
#ifdef UNICODE
...
INSTALLSTATE WINAPI
MsiUseFeatureExA(_In_ LPCSTR szProduct,    // product code
...
INSTALLSTATE WINAPI
MsiUseFeatureExW(_In_ LPCWSTR szProduct,   // product code
...
UINT WINAPI MsiConfigureFeatureA(
    _In_ LPCSTR szProduct, _In_ LPCSTR szFeature,
    _In_ INSTALLSTATE
        eInstallState); // local/source/default/absent/lock/uncache
UINT WINAPI MsiConfigureFeatureW(
    _In_ LPCWSTR szProduct, _In_ LPCWSTR szFeature,
    _In_ INSTALLSTATE
        eInstallState); // local/source/default/absent/lock/uncache
#ifdef UNICODE
...
UINT WINAPI MsiProvideComponentA(
    ...
    _In_ DWORD dwInstallMode, // either of type INSTALLMODE or a combination of
                              // the REINSTALLMODE flags
    _Out_writes_opt_(*pcchPathBuf)
...
// Return full component path for a qualified component, performing any
// necessary installation. Prompts for source if necessary and increments the
// usage count for the feature.
...
// Return full component path for a qualified component, performing any
// necessary installation. Prompts for source if necessary and increments the
// usage count for the feature. The szProduct parameter specifies the product to
// match that has published the qualified component. If null, this API works the
// same as MsiProvideQualifiedComponent.
...
INSTALLSTATE WINAPI MsiGetComponentPathExW(
    ...
    _In_opt_ MSIINSTALLCONTEXT dwContext, // Install context
    _Out_writes_opt_(*pcchOutPathBuffer)
        LPWSTR lpOutPathBuffer,             // returned path
    _Inout_opt_ LPDWORD pcchOutPathBuffer); // in/out buffer character count
#ifdef UNICODE
...
// Return full component path for an assembly installed via the WI, performing
// any necessary installation. Prompts for source if necessary and increments
// the usage count for the feature. The szAssemblyName parameter specifies the
// stringized assembly name. The szAppContext is the full path to the .cfg file
// or the app exe to which the assembly being requested has been privatised to,
// which is null for global assemblies

UINT WINAPI MsiProvideAssemblyA(
    _In_ LPCSTR szAssemblyName,   // stringized assembly name
    _In_opt_ LPCSTR szAppContext, // specifies the full path to the parent asm's
                                  // .cfg file, null for global assemblies
    _In_ DWORD dwInstallMode,  // either of type INSTALLMODE or a combination of
                               // the REINSTALLMODE flags
...
UINT WINAPI MsiProvideAssemblyW(
    ...
    _In_opt_ LPCWSTR
        szAppContext, // specifies the full path to the parent asm's .cfg file,
                      // null for global assemblies
    _In_ DWORD dwInstallMode,  // either of type INSTALLMODE or a combination of
                               // the REINSTALLMODE flags
    _In_ DWORD dwAssemblyInfo, // assembly info, including assembly type
    _Out_writes_opt_(*pcchPathBuf)
...
UINT WINAPI MsiEnumProductsExA(
    ...
    _Out_opt_ MSIINSTALLCONTEXT
        *pdwInstalledContext,               // Enumerated product's context
...
UINT WINAPI MsiEnumFeaturesA(
    ...
    _Out_writes_(MAX_FEATURE_CHARS + 1)
        LPSTR lpFeatureBuf, // feature name buffer,   size=MAX_FEATURE_CHARS+1
    _Out_writes_opt_(MAX_FEATURE_CHARS + 1)
...
UINT WINAPI MsiEnumComponentsW(
    ...
    _Out_writes_(MAX_GUID_CHARS + 1) LPWSTR
        lpComponentBuf); // buffer of char count: 39 (size of string GUID)
#ifdef UNICODE
...
UINT WINAPI MsiEnumComponentsExA(
    ...
    _Out_writes_opt_(MAX_GUID_CHARS + 1) CHAR
        szInstalledComponentCode[39], // Enumerated component's ComponentCode
    _Out_opt_ MSIINSTALLCONTEXT
        *pdwInstalledContext, // Enumerated component's context
    _Out_writes_opt_(*pcchSid)
        LPSTR szSid,              // Enumerated component's user account
    _Inout_opt_ LPDWORD pcchSid); // in/out character count of szSid
UINT WINAPI MsiEnumComponentsExW(
    _In_opt_ LPCWSTR szUserSid, // Account for enumeration
    _In_ DWORD dwContext,       // Contexts for enumeration
    _In_ DWORD dwIndex,         // 0-based index into installed components
    _Out_writes_opt_(MAX_GUID_CHARS + 1) WCHAR
        szInstalledComponentCode[39], // Enumerated component's ComponentCode
    _Out_opt_ MSIINSTALLCONTEXT
        *pdwInstalledContext, // Enumerated component's context
    _Out_writes_opt_(*pcchSid)
        LPWSTR szSid,             // Enumerated component's user account
    _Inout_opt_ LPDWORD pcchSid); // in/out character count of szSid
#ifdef UNICODE
...
UINT WINAPI MsiEnumClientsExA(
    ...
    _Out_writes_opt_(MAX_GUID_CHARS + 1)
        CHAR szProductBuf[39], // Enumerated component's client ProductCode
    _Out_opt_ MSIINSTALLCONTEXT
        *pdwInstalledContext, // Enumerated component's client product's context
...
UINT WINAPI MsiEnumClientsExW(
    ...
    _In_ DWORD dwProductIndex,  // 0-based index into component clients
    _Out_writes_opt_(MAX_GUID_CHARS + 1)
        WCHAR szProductBuf[39], // Enumerated component's client ProductCode
    _Out_opt_ MSIINSTALLCONTEXT
        *pdwInstalledContext, // Enumerated component's client product's context
...
UINT WINAPI MsiEnumComponentQualifiersA(
    ...
    _Out_writes_opt_(*pcchApplicationDataBuf)
        LPSTR lpApplicationDataBuf, // description buffer
    _Inout_opt_ LPDWORD
        pcchApplicationDataBuf); // in/out description buffer character count
...
UINT WINAPI MsiOpenPackageExA(
    _In_ LPCSTR szPackagePath, // path to package, or database handle: #nnnn
    _In_ DWORD dwOptions, // options flags to indicate whether or not to ignore
                          // machine state
    _Out_ MSIHANDLE *hProduct); // returned product handle, must be closed
...
UINT WINAPI MsiGetPatchFileListA(_In_ LPCSTR szProductCode,
                                 _In_ LPCSTR szPatchPackages,
                                 _Out_ LPDWORD pcFiles,
                                 _Outptr_result_buffer_maybenull_(*pcFiles)
                                     MSIHANDLE **pphFileRecords);
UINT WINAPI MsiGetPatchFileListW(_In_ LPCWSTR szProductCode,
...
UINT WINAPI MsiVerifyPackageA(_In_ LPCSTR szPackagePath); // location of package
UINT WINAPI
MsiVerifyPackageW(_In_ LPCWSTR szPackagePath); // location of package
#ifdef UNICODE
...
//  >1 = decreasing priority, feature installation based on InstallLevel
//  property

UINT WINAPI MsiGetFeatureInfoA(
    _In_ MSIHANDLE hProduct, // product handle obtained from MsiOpenProduct
    _In_ LPCSTR szFeature,   // feature name
    _Out_opt_ LPDWORD lpAttributes, // attribute flags for the feature, using
                                    // INSTALLFEATUREATTRIBUTE
    _Out_writes_opt_(*pcchTitleBuf)
...
UINT WINAPI
MsiInstallMissingFileA(_In_ LPCSTR szProduct, // product code
                       _In_ LPCSTR szFile);   // file name, without path
UINT WINAPI
MsiInstallMissingFileW(_In_ LPCWSTR szProduct, // product code
...
UINT WINAPI MsiSourceListAddSourceExA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Source will be added to this
                                          // product or patch
...
UINT WINAPI MsiSourceListAddSourceExW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Source will be added to this
                                           // product or patch
...
UINT WINAPI MsiSourceListAddMediaDiskA(
    _In_ LPCSTR
        szProductCodeOrPatchCode, // Disk will be added to this product or patch
    _In_opt_ LPCSTR szUserSid,    // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
UINT WINAPI MsiSourceListClearSourceA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Source will be cleared for this
                                          // product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
UINT WINAPI MsiSourceListClearSourceW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Source will be cleared for this
                                           // product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
// Remove an existing registered disk under the media source for the specified
// product or patch instance

UINT WINAPI MsiSourceListClearMediaDiskA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Disk will be cleared for this
                                          // product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
UINT WINAPI MsiSourceListClearMediaDiskW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Disk will be cleared for this
                                           // product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
// Remove all existing sources of the given source type for the specified
// product or patch instance

UINT WINAPI MsiSourceListClearAllExA(
    _In_ LPCSTR szProductCodeOrPatchCode, // All sources will be cleared for
                                          // this product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions);            // Additional qualifiers
UINT WINAPI MsiSourceListClearAllExW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // All sources will be cleared for
                                           // this product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions);            // Additional qualifiers
...
UINT WINAPI MsiSourceListForceResolutionExA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Source Resolution will be forced
                                          // for this product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions);            // Additional qualifiers
UINT WINAPI MsiSourceListForceResolutionExW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Source Resolution will be forced
                                           // for this product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions);            // Additional qualifiers
...
// Set the source list property to the provided value for the specified product
// or patch instance

UINT WINAPI MsiSourceListSetInfoA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Source Info will be set for this
                                          // product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
UINT WINAPI MsiSourceListSetInfoW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Source Info will be set for this
                                           // product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
// Query the given source list property for the specified product or patch
// instance

UINT WINAPI MsiSourceListGetInfoA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Source Info will be queried for
                                          // this produt or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
    _In_ LPCSTR szProperty,           // Property being queried
...
UINT WINAPI MsiSourceListGetInfoW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Source Info will be queried for
                                           // this produt or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
UINT WINAPI MsiSourceListEnumSourcesA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Sources will be enumerated for this
                                          // product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
    _In_ DWORD dwIndex,               // Index value for enumeration
    _Out_writes_opt_(*pcchSource)
...
UINT WINAPI MsiSourceListEnumSourcesW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Sources will be enumerated for
                                           // this product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
// Enumerate all registered media disks for the specified product or patch
// instance

UINT WINAPI MsiSourceListEnumMediaDisksA(
    _In_ LPCSTR szProductCodeOrPatchCode, // Disks will be enumerated for this
                                          // product or patch
    _In_opt_ LPCSTR szUserSid,        // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
    ...
    _Out_writes_opt_(*pcchVolumeLabel)
        LPSTR szVolumeLabel, // Volume label of the disk being enumerated
    _Inout_opt_ LPDWORD
        pcchVolumeLabel, // in/out character count of szVolumeLabel
    _Out_writes_opt_(*pcchDiskPrompt)
        LPSTR szDiskPrompt, // DiskPrompt of the disk being enumerated
    _Inout_opt_ LPDWORD
        pcchDiskPrompt); // in/out character count of szDiskPrompt
UINT WINAPI MsiSourceListEnumMediaDisksW(
    _In_ LPCWSTR szProductCodeOrPatchCode, // Disks will be enumerated for this
                                           // product or patch
    _In_opt_ LPCWSTR szUserSid,       // Account of this product/patch instance
    _In_ MSIINSTALLCONTEXT dwContext, // Context of this product/patch instance
    _In_ DWORD dwOptions,             // Additional qualifiers
...
UINT WINAPI MsiGetFileVersionA(
    ...
    _Out_writes_opt_(*pcchVersionBuf)
...
UINT WINAPI
MsiGetFileHashA(_In_ LPCSTR szFilePath,          // path to the file
                _In_ DWORD dwOptions,            // options
                _Inout_ PMSIFILEHASHINFO pHash); // returned file hash info
UINT WINAPI
MsiGetFileHashW(_In_ LPCWSTR szFilePath,         // path to the file
                _In_ DWORD dwOptions,            // options
                _Inout_ PMSIFILEHASHINFO pHash); // returned file hash info
#ifdef UNICODE
...
HRESULT WINAPI MsiGetFileSignatureInformationA(
    ...
    _Out_writes_bytes_opt_(*pcbHashData)
        LPBYTE pbHashData, // returned hash buffer, NULL if not desired
...
// user context
UINT WINAPI MsiIsProductElevatedA(_In_ LPCSTR szProduct,   // product code
                                  _Out_ BOOL *pfElevated); // result
// checks to see if a product is managed
// checks per-machine if called from system context, per-user if from
// user context
UINT WINAPI MsiIsProductElevatedW(_In_ LPCWSTR szProduct,  // product code
                                  _Out_ BOOL *pfElevated); // result
#ifdef UNICODE
...
UINT WINAPI MsiNotifySidChangeA(_In_ LPCSTR pOldSid, _In_ LPCSTR pNewSid);
UINT WINAPI MsiNotifySidChangeW(_In_ LPCWSTR pOldSid, _In_ LPCWSTR pNewSid);
#ifdef UNICODE
...
UINT WINAPI MsiBeginTransactionA(
    ...
    _In_ DWORD dwTransactionAttributes,   // Attributes to control the nature of
                                          // the transaction
...
UINT WINAPI MsiEndTransaction(
    _In_ DWORD
        dwTransactionState); // Flags to indicate how to end a transaction

// Join an already created multi package transaction
UINT WINAPI MsiJoinTransaction(
    _In_ MSIHANDLE hTransactionHandle,   // Transaction handle returned by
                                         // MsiBeginTransaction
...
#define ERROR_UNKNOWN_PRODUCT                                                  \
  1605L // This action is only valid for products that are currently installed.
// LOCALIZE END
...
#define ERROR_BAD_CONFIGURATION                                                \
  1610L // The configuration data for this product is corrupt.  Contact your
        // support personnel.
// LOCALIZE END:
#define ERROR_INDEX_ABSENT 1611L // Component qualifier not present.
// LOCALIZE BEGIN:
#define ERROR_INSTALL_SOURCE_ABSENT                                            \
  1612L // The installation source for this product is not available.  Verify
...
#define ERROR_INSTALL_SERVICE_FAILURE                                          \
  1601L // The Windows Installer Service could not be accessed. This can occur
...
#define ERROR_INSTALL_PACKAGE_VERSION                                          \
  1613L // This installation package cannot be installed by the Windows
...
#define ERROR_INSTALL_ALREADY_RUNNING                                          \
  1618L // Another program is being installed. Please wait until that
        // installation is complete, and then try installing this software
        // again.
#define ERROR_INSTALL_PACKAGE_OPEN_FAILED                                      \
  1619L // This installation package could not be opened.  Verify that the
...
#define ERROR_INSTALL_PACKAGE_INVALID                                          \
  1620L // This installation package could not be opened.  Contact the
        // application vendor to verify that this is a valid Windows Installer
        // package.
#define ERROR_INSTALL_UI_FAILURE                                               \
  1621L // There was an error starting the Windows Installer service user
        // interface.  Contact your support personnel.
#define ERROR_INSTALL_LOG_FAILURE                                              \
  1622L // Error opening installation log file.  Verify that the specified log
        // file location exists and is writable.
#define ERROR_INSTALL_LANGUAGE_UNSUPPORTED                                     \
  1623L // This language of this installation package is not supported by your
        // system.
#define ERROR_INSTALL_PACKAGE_REJECTED                                         \
  1625L // The system administrator has set policies to prevent this
        // installation.
// LOCALIZE END
...
#define ERROR_CREATE_FAILED                                                    \
  1631L // The Windows Installer service failed to start.  Contact your support
        // personnel.
// LOCALIZE END:
#endif
...
#define ERROR_INSTALL_TEMP_UNWRITABLE                                          \
  1632L // The Temp folder is on a drive that is full or is inaccessible. Free
        // up space on the drive or verify that you have write permission on the
        // Temp folder.
#endif

#ifndef ERROR_INSTALL_PLATFORM_UNSUPPORTED
#define ERROR_INSTALL_PLATFORM_UNSUPPORTED                                     \
  1633L // This installation package is not supported by this processor type.
...
#define ERROR_INSTALL_TRANSFORM_FAILURE                                        \
  1624L // Error applying transforms.  Verify that the specified transform paths
        // are valid.
#endif

#ifndef ERROR_PATCH_PACKAGE_OPEN_FAILED
#define ERROR_PATCH_PACKAGE_OPEN_FAILED                                        \
  1635L // This patch package could not be opened.  Verify that the patch
        // package exists and that you can access it, or contact the application
        // vendor to verify that this is a valid Windows Installer patch
        // package.
#define ERROR_PATCH_PACKAGE_INVALID                                            \
  1636L // This patch package could not be opened.  Contact the application
        // vendor to verify that this is a valid Windows Installer patch
        // package.
#define ERROR_PATCH_PACKAGE_UNSUPPORTED                                        \
  1637L // This patch package cannot be processed by the Windows Installer
        // service.  You must install a Windows service pack that contains a
        // newer version of the Windows Installer service.
#endif

#ifndef ERROR_PRODUCT_VERSION
#define ERROR_PRODUCT_VERSION                                                  \
  1638L // Another version of this product is already installed.  Installation
        // of this version cannot continue.  To configure or remove the existing
        // version of this product, use Add/Remove Programs on the Control
        // Panel.
#endif

#ifndef ERROR_INVALID_COMMAND_LINE
#define ERROR_INVALID_COMMAND_LINE                                             \
  1639L // Invalid command line argument.  Consult the Windows Installer SDK for
...
#define ERROR_INSTALL_REMOTE_DISALLOWED                                        \
  1640L // Only administrators have permission to add, remove, or configure
        // server software during a Terminal services remote session. If you
        // want to install or configure software on the server, contact your
...
#define ERROR_SUCCESS_REBOOT_INITIATED                                         \
  1641L // The requested operation completed successfully.  The system will be
...
#define ERROR_PATCH_TARGET_NOT_FOUND                                           \
  1642L // The upgrade patch cannot be installed by the Windows Installer
        ...
        // that the program to be upgraded exists on your computer and that you
        // have the correct upgrade patch.
#endif
// LOCALIZE END

// The following two error codes are not returned from MSI version 1.0, 1.1.
// or 1.2

// LOCALIZE BEGIN:
#ifndef ERROR_PATCH_PACKAGE_REJECTED
#define ERROR_PATCH_PACKAGE_REJECTED                                           \
  1643L // The patch package is not permitted by software restriction policy.
#endif

#ifndef ERROR_INSTALL_TRANSFORM_REJECTED
#define ERROR_INSTALL_TRANSFORM_REJECTED                                       \
  1644L // One or more customizations are not permitted by software restriction
        // policy.
#endif
// LOCALIZE END
...
#define ERROR_INSTALL_REMOTE_PROHIBITED                                        \
  1645L // The Windows Installer does not permit installation from a Remote
        // Desktop Connection.
#endif
// LOCALIZE END
...
#define ERROR_PATCH_REMOVAL_UNSUPPORTED                                        \
  1646L // Uninstallation of the patch package is not supported.
#endif
// LOCALIZE END
...
#define ERROR_PATCH_NO_SEQUENCE                                                \
  1648L // No valid sequence could be found for the set of patches.
#endif
// LOCALIZE END

// LOCALIZE BEGIN:
#ifndef ERROR_PATCH_REMOVAL_DISALLOWED
#define ERROR_PATCH_REMOVAL_DISALLOWED                                         \
  1649L // Patch removal was disallowed by policy.
#endif
// LOCALIZE END
...
#define ERROR_PATCH_MANAGED_ADVERTISED_PRODUCT                                 \
  1651L // Windows Installer does not permit patching of managed advertised
        // products. At least one feature of the product must be installed
...
#define ERROR_INSTALL_SERVICE_SAFEBOOT                                         \
  1652L // The Windows Installer service is not accessible in Safe Mode. Please
        // try again when your computer is not in Safe Mode or you can use
        // System Restore to return your machine to a previous good state.
#endif
// LOCALIZE END
...
#define ERROR_ROLLBACK_DISABLED                                                \
  1653L // Multi-package transaction cannot be run when rollback is disabled.
#endif
// LOCALIZE END
...
#define ERROR_INSTALL_REJECTED                                                 \
  1654L // The app that you are trying to run is not supported on this version
        // of Windows
#endif
// LOCALIZE END
