...
STDAPI DdqCreateSession(_In_ DdqAccessLevel accessLevel,
                        _Out_ HDIAGNOSTIC_DATA_QUERY_SESSION *hSession);

STDAPI DdqCloseSession(_In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession);

STDAPI DdqGetSessionAccessLevel(_In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
                                _Out_ DdqAccessLevel *accessLevel);

STDAPI
DdqGetDiagnosticDataAccessLevelAllowed(_Out_ DdqAccessLevel *accessLevel);
...
STDAPI DdqGetDiagnosticRecordStats(
    ...
    _Out_ UINT32 *recordCount, _Out_ INT64 *minRowId, _Out_ INT64 *maxRowId);

// Caller is expected to free payload with CoTaskMemFree()
STDAPI
DdqGetDiagnosticRecordPayload(_In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
                              _In_ INT64 rowId, _Out_ PCWSTR *payload);

// Invoke DdqFreeDiagnosticRecordLocaleTags() to free tagDescription
STDAPI DdqGetDiagnosticRecordLocaleTags(
    _In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession, _In_ PCWSTR locale,
...
STDAPI DdqGetDiagnosticRecordLocaleTagAtIndex(
    _In_ HDIAGNOSTIC_EVENT_TAG_DESCRIPTION hTagDescription, _In_ UINT32 index,
...
// Invoke DdqFreeDiagnosticRecordProducerCategories() to free
// categoryDescription
STDAPI DdqGetDiagnosticRecordProducerCategories(
    _In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession, _In_ PCWSTR producerName,
...
STDAPI DdqIsDiagnosticRecordSampledIn(
    _In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
    _In_ const GUID *providerGroup, _In_opt_ const GUID *providerId,
    _In_ PCWSTR providerName, _In_opt_ const UINT32 *eventId,
    _In_ PCWSTR eventName, _In_opt_ const UINT32 *eventVersion,
    _In_opt_ const UINT64 *eventKeywords, _Out_ BOOL *isSampledIn);

// Invoke DdqFreeDiagnosticRecordPage() to free record
STDAPI DdqGetDiagnosticRecordPage(
    _In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
    _In_ DIAGNOSTIC_DATA_SEARCH_CRITERIA *const searchCriteria,
    _In_ UINT32 offset, _In_ UINT32 pageRecordCount, _In_ INT64 baseRowId,
    _Out_ HDIAGNOSTIC_RECORD *hRecord);

STDAPI DdqFreeDiagnosticRecordPage(_In_ HDIAGNOSTIC_RECORD hRecord);

STDAPI DdqGetDiagnosticRecordAtIndex(_In_ HDIAGNOSTIC_RECORD hRecord,
                                     _In_ UINT32 index,
                                     _Out_ DIAGNOSTIC_DATA_RECORD *record);

STDAPI DdqGetDiagnosticRecordCount(_In_ HDIAGNOSTIC_RECORD hRecord,
                                   _Out_ UINT32 *recordCount);

STDAPI DdqGetDiagnosticReportStoreReportCount(
    _In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession, _In_ UINT32 reportStoreType,
...
STDAPI DdqGetDiagnosticReport(_In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
...
STDAPI DdqGetDiagnosticReportCount(_In_ HDIAGNOSTIC_REPORT hReport,
                                   _Out_ UINT32 *reportCount);

STDAPI DdqExtractDiagnosticReport(_In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
                                  _In_ UINT32 reportStoreType,
...
STDAPI DdqGetDiagnosticRecordTagDistribution(
    ...
    _Outptr_result_buffer_all_(*statCount)
...
STDAPI DdqGetDiagnosticRecordBinaryDistribution(
    _In_ HDIAGNOSTIC_DATA_QUERY_SESSION hSession,
    _In_reads_(producerNameCount) PCWSTR *producerNames,
    _In_ UINT32 producerNameCount, _In_ UINT32 topNBinaries,
    _Outptr_result_buffer_all_(*statCount)
