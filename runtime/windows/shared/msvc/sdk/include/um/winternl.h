...
//     CloseHandle
//
__kernel_entry NTSTATUS NTAPI NtClose(IN HANDLE Handle);

//
// use the Win32 API instead
//     CreateFile
//
__kernel_entry NTSTATUS NTAPI NtCreateFile(
...
//     CreateFile
//
__kernel_entry NTSTATUS NTAPI NtOpenFile(OUT PHANDLE FileHandle,
...
//     N/A
//
__kernel_entry NTSTATUS NTAPI NtRenameKey(_In_ HANDLE KeyHandle,
...
__kernel_entry NTSTATUS NTAPI NtNotifyChangeMultipleKeys(
    _In_ HANDLE MasterKeyHandle, _In_opt_ ULONG Count,
    _In_reads_opt_(Count) OBJECT_ATTRIBUTES SubordinateObjects[],
    _In_opt_ HANDLE Event, _In_opt_ PIO_APC_ROUTINE ApcRoutine,
...
__kernel_entry NTSTATUS NTAPI NtQueryMultipleValueKey(
...
__kernel_entry NTSTATUS NTAPI NtSetInformationKey(
    _In_ HANDLE KeyHandle,
...
//     DeviceIoControl
//
__kernel_entry NTSTATUS NTAPI NtDeviceIoControlFile(
...
NTSTATUS
NTAPI
NtWaitForSingleObject(IN HANDLE Handle, IN BOOLEAN Alertable,
...
BOOLEAN
NTAPI
RtlIsNameLegalDOS8Dot3(IN PUNICODE_STRING Name,
...
_When_(Status < 0, _Out_range_(>, 0))
    _When_(Status >= 0, _Out_range_(==, 0)) ULONG NTAPI
...
//
__kernel_entry NTSTATUS NTAPI NtSetInformationThread(
...
//
__kernel_entry NTSTATUS NTAPI NtQuerySystemTime(OUT PLARGE_INTEGER SystemTime);

__kernel_entry NTSTATUS NTAPI NtQueryTimerResolution(_Out_ PULONG MaximumTime,
...
NTSTATUS
NTAPI
RtlInitStringEx(PSTRING DestinationString, PCSZ SourceString);
...
//
// this function is implemented in winsta.dll (you need to loadlibrary to call
// this function) this internal function retrives the LogonId (also called
// SessionId) for the current process You should avoid using this function as it
// can change. you can retrieve the same information Using public api
// WTSQuerySessionInformation. Pass WTSSessionId as the WTSInfoClass parameter
//
typedef BOOLEAN(WINAPI *PWINSTATIONQUERYINFORMATIONW)(HANDLE, ULONG,
                                                      WINSTATIONINFOCLASS,
                                                      PVOID, ULONG, PULONG);
