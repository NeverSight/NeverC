...
// use; this includes legacy EAP Methods too. Returns non zero return code up on
// error.
DWORD WINAPI EapHostPeerGetMethods(
    // API fills the structure to have installed EAP Methods. Caller should free
    // the inner pointers using EapHostPeerFreeMemory starting at the inner most
    // pointer.
    _Out_ EAP_METHOD_INFO_ARRAY *pEapMethodInfoArray,
    // In case of error, API fills ppEapError if possible. Caller should free
    // ppEapError using EapHostPeerFreeErrorMemory
    _Outptr_ EAP_ERROR **ppEapError);

// This API is used to retrieve the method properties for a specific
// configuration It returns non zero return code upon error.
DWORD WINAPI EapHostPeerGetMethodProperties(
    ...
    // behavior.
    _In_ DWORD dwFlags,
    // An EAP_METHOD_TYPE structure that identifies the EAP method the
    // supplicant is to use.
    ...
    // A pointer to the method properties array. Caller should free the inner
    // pointers using EapHostPeerFreeMemory starting at the inner most pointer.
    // The caller should free empvString value only when the type is
    // empvtString.
...
DWORD WINAPI EapHostPeerInvokeConfigUI(
    // window handle of the parent window under which configuration dialog will
    // show up
    ...
    _In_ EAP_METHOD_TYPE eapMethodType,
    // Size of input configuration; this could be 0 when there is no
    // configuration
...
// for showing UI in long screen. The structures returned have details on how to
// show the fields. API Returns non zero return code up on error.
DWORD WINAPI EapHostPeerQueryCredentialInputFields(
    ...
    // the user. Caller should free the inner pointers using
    // EapHostPeerFreeMemory starting at the inner most pointer.
...
// to obtain credential blob that could used start authentication API Returns
// non zero return code up on error.
DWORD WINAPI EapHostPeerQueryUserBlobFromCredentialInputFields(
    ...
    // pointer to DWORD that receives the size of credential blob, if supplicant
    // passes in non-zero size and non-NULL data below, EAPHost will just
    // attempt to Update the blob with passed in values (if method supports)
    // instead of creating a new one.
    _Inout_ DWORD *pdwUserBlobSize,
    // Pointer that receives the credential blob that can be used in
    // authentication. For incoming data caller should always allocate this
    // memory using LocalAlloc() Caller should free the memory using
    // EapHostPeerFreeMemory.
    _Inout_ _At_(*ppbUserBlob, _When_(*ppbUserBlob != NULL,
                                      _Pre_writable_size_(*pdwUserBlobSize))
...
// of EapHostPeerResponseInvokeUi supplicant should call EapHostPeerGetUIContext
// to get UI context data. After that. it then should call this API from a
// process where UI can be brought up. API Returns non zero return code up on
// error.
DWORD WINAPI EapHostPeerInvokeInteractiveUI(
    ...
    // pointer to DWORD that receives data from interactive UI that is used for
    // authentication to continue
    _Out_ DWORD *pdwSizeOfDataFromInteractiveUI,
    // pointer that receives buffer filled with interactive UI that is used for
    // authentication to continue. Caller should free the memory using
    // EapHostPeerFreeMemory.
...
// This API will enable supplicants to collect necessary information from
// EAPHost to raise interactive UI and collect user information supplicant
// should call EapHostPeerQueryInteractiveUIInputFields() API first after it
// receives EapHostPeerResponseInvokeUi actionCode from EAPHost. If the return
// value from this API is EAP_E_EAPHOST_METHOD_OPERATION_NOT_SUPPORTED, then
// supplicant should fall back to traditional model of invoking method
// interactive UI, i.e. by calling EapHostPeerInvokeInteractiveUI() API Returns
// non zero return code up on error.
...
// which can be consumed by EAPHost runtime APIs API Returns non zero return
// code up on error.
DWORD WINAPI EapHostPeerQueryUIBlobFromInteractiveUIInputFields(
    ...
    // pointer to DWORD that receives size of credential blob, if supplicant
    // passes in non-zero size and non-NULL data below, EAPHost will just
    // attempt to Update the blob with passed in values (if method supports)
    // instead of creating a new one.
    _Out_ DWORD *pdwSizeOfDataFromInteractiveUI,
    // Pointer that receives the credential blob that can be used in
    // authentication. Caller should free the memory using
    // EapHostPeerFreeMemory.
    _Outptr_result_buffer_(*pdwSizeOfDataFromInteractiveUI)
        BYTE **ppDataFromInteractiveUI,
    // In case of error, API fills ppEapError if possible.  Caller should free
...
// Supplicant can utilize XML based EAP configuration storing, managing,
// editing. When it wants to call EapHostPeerConfigureUI or when it has to start
// authentication, it calls this API to convert XML configuration into blob. API
// Returns non zero return code up on error.
DWORD WINAPI EapHostPeerConfigXml2Blob(
    ...
    _Out_ DWORD *pdwSizeOfConfigOut,
    // pointer that receives configuration blob. Caller should free the memory
    // using EapHostPeerFreeMemory.
    _Outptr_result_buffer_(*pdwSizeOfConfigOut) BYTE **ppConfigOut,
...
// When it wants to start authentication, it calls this API to convert XML based
// credentials into blob. API Returns non zero return code up on error.
DWORD WINAPI EapHostPeerCredentialsXml2Blob(
    ...
    // pointer that receives credential blob buffer.  Caller should free the
    // memory using EapHostPeerFreeMemory.
...
// Supplicant can utilize XML based EAP credentials storing, managing, editing.
// When it wants to start convert configuration blob to XML, it can call this
// API. API Returns non zero return code up on error.
DWORD WINAPI EapHostPeerConfigBlob2Xml(
    ...
    _In_reads_(dwSizeOfConfigIn) BYTE *pConfigIn,
    // XML document that contains XML form of the blob. If the EAP Method does
    // not support EapPeerConfigBlob2Xml function, the XML contains ConfigBlob
    // node with blob in string form
...
// should not use this API for freeing EAP_ERROR structure.
VOID WINAPI EapHostPeerFreeMemory(BYTE *pData);

// Supplicant calls this API to free EAP_ERROR memory, which will get filled
// when an API fails.
