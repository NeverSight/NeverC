...
#define GET_KERNDATA(pUFM) ((PKERNDATA)((PBYTE)(pUFM) + (pUFM)->loKernPair))
...
//
// _PRINTIFI32 and _PRINTIFI64 are fixed-size versions of the IFIMETRICS
// structures that are gauranteed not to change accross architectures or OS
// versions, and can be used for binary file layouts.  _PRINTIFI32 is the format
// used for Unidrv UFM files on all platformts.  PScript NTF files use the
// platform-specific version of the code.
//
...
#define DF_NO_DOUBLE_UNDERLINE                                                 \
  0x0010                        // Cannot double underline via
                                // FONTSIMU ATION
#define DF_NO_STRIKETHRU 0x0020 // Cannot strikethru via FONTSIMULATION
#define DF_BKSP_OK                                                             \
  0x0040 // Can use backspace char, see spec
         // for details
...
#define MTYPE_COMPOSE                                                          \
  0x01                    // wCode is an array of 16-bit offsets from the
...
#define MTYPE_SINGLE                                                           \
  0x08 // wCode contains a single byte character code in
       // multi byte character string.
#define MTYPE_DOUBLE                                                           \
  0x10 // wCode contains a double byte character code in
       // multi byte character string.
//
