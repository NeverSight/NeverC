...
#define DISPID_BEFORENAVIGATE                                                  \
  100 // this is sent before navigation to give a chance to abort
#define DISPID_NAVIGATECOMPLETE                                                \
  101 // in async, this is sent when we have enough to show
...
#define DISPID_TITLEICONCHANGE                                                 \
  114 // sent when the top level window icon may have changed.
...
#define DISPID_ONADDRESSBAR                                                    \
  261 // sent when the address bar should be shown/hidden
#define DISPID_WINDOWSETRESIZABLE                                              \
  262 // sent to set the style of the host window frame
#define DISPID_WINDOWCLOSING                                                   \
  263 // sent before script window.close closes the window
#define DISPID_WINDOWSETLEFT                                                   \
  264 // sent when the put_left method is called on the WebOC
#define DISPID_WINDOWSETTOP                                                    \
  265 // sent when the put_top method is called on the WebOC
#define DISPID_WINDOWSETWIDTH                                                  \
  266 // sent when the put_width method is called on the WebOC
#define DISPID_WINDOWSETHEIGHT                                                 \
  267 // sent when the put_height method is called on the WebOC
#define DISPID_CLIENTTOHOSTWINDOW                                              \
  268 // sent during window.open to request conversion of dimensions
#define DISPID_SETSECURELOCKICON                                               \
  269 // sent to suggest the appropriate security icon to show
#define DISPID_FILEDOWNLOAD                                                    \
  270 // Fired to indicate the File Download dialog is opening
#define DISPID_NAVIGATEERROR                                                   \
  271 // Fired to indicate the a binding error has occured
#define DISPID_PRIVACYIMPACTEDSTATECHANGE                                      \
  272 // Fired when the user's browsing experience is impacted
#define DISPID_NEWWINDOW3 273
#define DISPID_VIEWUPDATE                                                      \
  281 // Fired when the contents of a shell browser window change
#define DISPID_SETPHISHINGFILTERSTATUS                                         \
  282 // Fired by the Phishing Filter API to signal what state the analysis is
      // in
#define DISPID_WINDOWSTATECHANGED                                              \
  283 // Fired to indicate that the browser window's visibility or enabled state
      // has changed
#define DISPID_NEWPROCESS                                                      \
  284 // Fired when a navigation must be redirected due to Protected Mode
#define DISPID_THIRDPARTYURLBLOCKED                                            \
  285 // Fired when a third-party url is blocked due to Privacy Advisor
#define DISPID_REDIRECTXDOMAINBLOCKED                                          \
  286 // Fired when a x-domain redirect is blocked due to browser nav constant
#define DISPID_WEBWORKERSTARTED 288
#define DISPID_WEBWORKERFINISHED 289
#define DISPID_BEFORESCRIPTEXECUTE                                             \
  290 // Fired prior to any of a page's script is executed

// Printing events
#define DISPID_PRINTTEMPLATEINSTANTIATION                                      \
  225 // Fired to indicate that a print template is instantiated
#define DISPID_PRINTTEMPLATETEARDOWN                                           \
  226 // Fired to indicate that a print templete is completely gone
#define DISPID_UPDATEPAGESTATUS                                                \
  227 // Fired to indicate that the spooling status has changed
