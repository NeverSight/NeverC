...
protected:
  ...
  virtual HRESULT DeleteInstance(const CInstance &newInstance,
                                 long lFlags = 0L);
  ...
  virtual HRESULT ExecMethod(const CInstance &cInstance,
                             ...
                             _Out_ CInstance *pOutParams, long lFlags = 0L);

  // find and create all instances of your class
  virtual HRESULT EnumerateInstances(_In_ MethodContext *pMethodContext,
                                     long lFlags = 0L);
  ...
  // to see what properties are required.  If you don't implement this method,
  // the GetObject(CInstance, long) method will be called instead.
  virtual HRESULT GetObject(_Inout_ CInstance *pInstance, long lFlags,
                            _In_ CFrameworkQuery &Query);
  ...
  virtual HRESULT ExecQuery(_In_ MethodContext *pMethodContext,
                            _In_ CFrameworkQuery &cQuery, long lFlags = 0L);
  ...
  bool GetLocalInstancePath(const CInstance *pInstance,
                            _Out_ CHString &strPath);
  ...
  enum FlagDefs {
    ...
    PutInstanceFlags = (WBEM_FLAG_CREATE_ONLY | WBEM_FLAG_CREATE_OR_UPDATE |
                        WBEM_FLAG_UPDATE_ONLY | WBEM_FLAG_SEND_STATUS),
...
private:
  ...
  IWbemClassObject
      *m_piClassObject; // holds class object from which others are cloned.
  ...
  HRESULT ExecuteQuery(_In_ MethodContext *pContext,
                       _In_ CFrameworkQuery &pQuery, long lFlags = 0L);

  HRESULT CreateInstanceEnum(_In_ MethodContext *pContext, long lFlags = 0L);

  HRESULT PutInstance(_In_ IWbemClassObject __RPC_FAR *pInst, long lFlags,
                      _In_ MethodContext *pContext);

  HRESULT DeleteInstance(_In_ ParsedObjectPath *pParsedObjectPath, long lFlags,
                         _In_ MethodContext *pContext);

  HRESULT ExecMethod(_In_ ParsedObjectPath *pParsedObjectPath,
                     _In_ BSTR bstrMethodName, long lFlags,
                     _In_ CInstance *pInParams, _Out_ CInstance *pOutParams,
                     _In_ MethodContext *pContext);
