...
STDAPI
CreateBindLink(_In_ PCWSTR virtualPath, _In_ PCWSTR backingPath,
               CREATE_BIND_LINK_FLAGS createBindLinkFlags,
               UINT32 exceptionCount,
               _In_reads_opt_(exceptionCount) PCWSTR *const exceptionPaths);

STDAPI
RemoveBindLink(_In_ PCWSTR virtualPath);

#pragma endregion

#endif /* #if defined(BINDLINK_API_IMPL) || (defined(NTDDI_WIN10_CU) &&        \
