...
//
//      AuthenticatorDllPath             (REG_EXPAND_SZ)
//      %SystemRoot%\system32\sample.dll AuthenticatorFriendlyName (REG_SZ)
//      Sample EAP Protocol
...
#define EAP_AUTHENTICATOR_VALUENAME_FRIENDLY_NAME                              \
  TEXT("AuthenticatorFriendlyName")
...
typedef struct _EAP_AUTHENTICATOR_METHOD_ROUTINES {
  ...
  DWORD(APIENTRY *EapMethodAuthenticatorInitialize)(
      _In_ EAP_METHOD_TYPE *pEapType, _Outptr_ EAP_ERROR **ppEapError);
  ...
  DWORD(APIENTRY *EapMethodAuthenticatorBeginSession)(
      ...
      // This is the maximum size of an eap packet that the authenticator can
      // send.
  ...
  DWORD(APIENTRY *EapMethodAuthenticatorUpdateInnerMethodParams)(
      ...
      _In_ EAP_SESSION_HANDLE sessionHandle, _In_ DWORD dwFlags,
      _In_ LPCWSTR pwszIdentity,
      // Pointer to an array of attributes. This array contains attributes that
      // describe the entity being authenticated.
      _In_ const EapAttributes *const pAttributeArray,
  ...
  // EapHostAuthenticatorProcessRequestPacket returns
  // EapHostAuthenticatorResponseRespond.
  DWORD(APIENTRY *EapMethodAuthenticatorGetAttributes)(
      // context handle as returned from a successful call to
      // EapHostAuthenticatorBeginSession
      _In_ EAP_SESSION_HANDLE sessionHandle,
      // Array of attributes that the caller needs to act on.
      _Out_ EapAttributes *pAttribs, _Outptr_ EAP_ERROR **ppEapError);
  ...
  //    EAP_HOST_AUTHENTICATOR_RESPONSE_SUCCESS or
  //    EAP_HOST_AUTHENTICATOR_RESPONSE_FAILURE Even if the action returned
  //    above was a success, the authenticator can choose to call this method
  //    with a failure.
  // 2. The server can choose to terminate an authentication with a failure in
  // the middle of
  //    an authentication.
...
DWORD EapMethodAuthenticatorGetInfo(
    ...
    _Out_ EAP_AUTHENTICATOR_METHOD_ROUTINES *pEapInfo,
    _Outptr_ EAP_ERROR **ppEapError);

DWORD EapMethodAuthenticatorInitialize(_In_ EAP_METHOD_TYPE *pEapType,
                                       _Outptr_ EAP_ERROR **ppEapError);
...
// 1. A call to EapHostAuthenticatorReceivePacket returned
//    EAP_HOST_AUTHENTICATOR_RESPONSE_SUCCESS or
//    EAP_HOST_AUTHENTICATOR_RESPONSE_FAILURE Even if the action returned above
//    was a success, the authenticator can choose to call this method with a
//    failure.
// 2. The server can choose to terminate an authentication with a failure in the
// middle of
//    an authentication.
...
DWORD EapMethodAuthenticatorShutdown(_In_ EAP_METHOD_TYPE *pEapType,
                                     _Outptr_ EAP_ERROR **ppEapError);
...
DWORD WINAPI EapMethodAuthenticatorInvokeConfigUI(
    _In_ EAP_METHOD_TYPE *pEapMethodType, _In_ HWND hwndParent,
    _In_ DWORD dwFlags, _In_ LPCWSTR pwszMachineName,
    _In_ DWORD dwSizeOfConfigIn, _In_reads_(dwSizeOfConfigIn) BYTE *pConfigIn,
