...
public:
    ...
    if (pSrc) {
      ...
      if ((ULONG)cbNeeded == cbNeeded) {
        ...
        if (m_psz == NULL) {
          ...
          ::MultiByteToWideChar(CP_ACP, 0, pSrc, -1, m_psz,
                                (ULONG)cbNeeded / sizeof(WCHAR));
        }
      }
    }
    return m_psz;
  ...
  ~CSpDynamicString() { ::CoTaskMemFree(m_psz); }
  ...
  operator WCHAR *() const { return m_psz; }
  ...
  HRESULT AppendHR(const WCHAR *pszSrc, const ULONG lenSrc) {
    ...
    if (pszSrc && (lenSrc || !m_psz)) {
      ...
      if (ullcbNew > ((ULONGLONG)lenMe + lenSrc + 1) &&
          ((ULONG)ullcbNew == ullcbNew)) {
  ...
  HRESULT Append2HR(const WCHAR *pszSrc1, const WCHAR *pszSrc2) {
    ...
    if (lenSrc1 || lenSrc2 || !m_psz) {
      ...
      if (pszNew) {
        ...
        // In both of these cases, we copy the trailing NULL so that we're sure
        // it gets there (if lenSrc2 is 0 then we better copy it from pszSrc1).
        if (lenSrc1) {
          memcpy(pszNew + lenMe, pszSrc1, (lenSrc1 + 1) * sizeof(WCHAR));
        }
        if (lenSrc2) {
          memcpy(pszNew + lenMe + lenSrc1, pszSrc2,
                 (lenSrc2 + 1) * sizeof(WCHAR));
        }
        m_psz = pszNew;
      } else {
        hr = E_OUTOFMEMORY;
  ...
  CHAR *CopyToChar() const {
    if (m_psz) {
      CHAR *psz;
      ULONG cbNeeded =
          ::WideCharToMultiByte(CP_ACP, 0, m_psz, -1, NULL, NULL, NULL, NULL);
      psz = (CHAR *)::CoTaskMemAlloc(cbNeeded);
      if (psz == NULL) {
        SetLastError(ERROR_OUTOFMEMORY);
      } else {
        ::WideCharToMultiByte(CP_ACP, 0, m_psz, -1, psz,
                              cbNeeded / sizeof(CHAR), NULL, NULL);
      }
      return psz;
    }
    return NULL;
  ...
  bool operator!() const { return (m_psz == NULL); }
...
FORCEINLINE
DWORD
SpIsRunningInAppContainer(__out PBOOL IsAppContainer)
...
--*/
{
  ...
  *IsAppContainer = FALSE;
  ...
  if (OpenThreadToken(
          GetCurrentThread(), TOKEN_QUERY,
          TRUE, // Needs to be true if the token has only the SecurityIdentify
                // impersonation level, see
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &EffectiveToken) ==
        FALSE) {
      return GetLastError();
    }
  }
  ...
  if (GetTokenInformation(EffectiveToken, TokenIsAppContainer, &AppContainer,
                          sizeof(DWORD), &ReturnLength) != FALSE) {
...
inline HRESULT SpGetTokenFromId(const WCHAR *pszTokenId,
...
inline HRESULT SpGetCategoryFromId(const WCHAR *pszCategoryId,
...
inline HRESULT SpGetDefaultTokenIdFromCategoryId(const WCHAR *pszCategoryId,
...
inline HRESULT SpSetDefaultTokenIdForCategoryId(const WCHAR *pszCategoryId,
...
inline HRESULT
SpGetDefaultTokenFromCategoryId(const WCHAR *pszCategoryId,
                                ISpObjectToken **ppToken,
...
inline HRESULT SpSetDefaultTokenForCategoryId(const WCHAR *pszCategoryId,
...
inline HRESULT SpSetCommonTokenData(ISpObjectToken *pToken, const CLSID *pclsid,
...
inline HRESULT SpCreateNewToken(const WCHAR *pszTokenId,
                                ISpObjectToken **ppToken) {
  HRESULT hr;
...
inline HRESULT SpCreateNewToken(const WCHAR *pszCategoryId,
...
inline HRESULT
SpCreateNewTokenEx(const WCHAR *pszCategoryId, const WCHAR *pszTokenKeyName,
                   const CLSID *pclsid, const WCHAR *pszLangIndependentName,
                   LANGID langid, const WCHAR *pszLangDependentName,
  ...
  if (SUCCEEDED(hr)) {
    hr = SpSetCommonTokenData(*ppToken, pclsid, pszLangIndependentName, langid,
                              pszLangDependentName, ppDataKeyAttribs);
  }

  return hr;
}

inline HRESULT SpCreateNewTokenEx(const WCHAR *pszTokenId, const CLSID *pclsid,
                                  const WCHAR *pszLangIndependentName,
                                  LANGID langid,
                                  const WCHAR *pszLangDependentName,
                                  ISpObjectToken **ppToken,
  ...
  if (SUCCEEDED(hr)) {
    ...
    HRESULT hr2 =
        HRESULT_FROM_WIN32(SpIsRunningInAppContainer(&isAppContainer));
    // only set the extra data when not running in app chamber, if we failed the
    // call to check if in chamber default to old sapi behavior.
    if (FAILED(hr2) || !isAppContainer) {
      // Now set the extra data
      hr = SpSetCommonTokenData(*ppToken, pclsid, pszLangIndependentName,
                                langid, pszLangDependentName, ppDataKeyAttribs);
    }
  }
...
inline HRESULT SpEnumTokens(const WCHAR *pszCategoryId,
...
inline HRESULT SpFindBestToken(const WCHAR *pszCategoryId,
                               const WCHAR *pszReqAttribs,
                               const WCHAR *pszOptAttribs,
  ...
  if (pszOptAttribs) {
    ...
    if (SUCCEEDED(hr)) {
      hr = ULongAdd(ulLen, 1 + 1,
                    &ulLen); // including 1 char here for null terminator
    }
  } else {
    hr = ULongAdd(ulLenVendorPreferred, 1,
                  &ulLen); // including 1 char here for null terminator
  }
  if (FAILED(hr)) {
    hr = E_INVALIDARG;
    ...
    if (SUCCEEDED(hr)) {
      hr = SpEnumTokens(pszCategoryId, pszReqAttribs, pszOptAttribsVendorPref,
                        &cpEnum);
    }
...
HRESULT SpCreateObjectFromToken(ISpObjectToken *pToken, T **ppObject,
  ...
  hr = pToken->CreateInstance(pUnkOuter, dwClsCtxt, __uuidof(T),
                              (void **)ppObject);
...
template <class T>
HRESULT SpCreateDefaultObjectFromCategoryId(const WCHAR *pszCategoryId,
                                            T **ppObject,
                                            IUnknown *pUnkOuter = NULL,
                                            DWORD dwClsCtxt = CLSCTX_ALL) {
...
template <class T>
HRESULT
SpCreateBestObject(const WCHAR *pszCategoryId, const WCHAR *pszReqAttribs,
                   const WCHAR *pszOptAttribs, T **ppObject,
...
inline HRESULT SpCreatePhoneConverter(LANGID LangID, const WCHAR *pszReqAttribs,
                                      const WCHAR *pszOptAttribs,
  ...
  if (LangID != 0) {
    ...
    hr = SpCreateBestObject(SPCAT_PHONECONVERTERS, dstrReqAttribs,
                            pszOptAttribs, ppPhoneConverter);
  }

#if _SAPI_VER >= 0x053
  // If we cannot find a phone converter, use the Universal Phone Converter as
  // default
  if (hr == SPERR_NOT_FOUND) {
    hr = CoCreateInstance(CLSID_SpPhoneConverter, NULL, CLSCTX_ALL,
                          IID_ISpPhoneConverter, (LPVOID *)ppPhoneConverter);

    if (SUCCEEDED(hr)) {
      ISpPhoneticAlphabetSelection *pAlphabetSelection = NULL;
      hr = (*ppPhoneConverter)
               ->QueryInterface(IID_ISpPhoneticAlphabetSelection,
                                (LPVOID *)&pAlphabetSelection);
      if (SUCCEEDED(hr)) {
...
inline HRESULT SpGetLanguageFromToken(ISpObjectToken *pToken, LANGID *plangid) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    // check if the langid is in the format "409;9" - extract the "409" in this
    // case
    const WCHAR *pSemiColon = wcschr(pszLangId, L';');
    if (pSemiColon) {
      size_t cLen = pSemiColon - dstrLanguage;
      if (cLen <= 4) {
        StringCbCopyNW(achHexNumber, sizeof(achHexNumber), dstrLanguage,
                       cLen * sizeof(achHexNumber[0]));
...
inline HRESULT SpGetLanguageFromVoiceToken(ISpObjectToken *pToken,
                                           LANGID *plangid) {
...
inline HRESULT
SpGetDescription(_In_ ISpObjectToken *pObjToken,
                 _Outptr_ PWSTR *ppszDescription,
  ...
  // When running on Windows Vista query the localized engine name from a
  // resource dll
  OSVERSIONINFO ver;
  ...
  *ppszDescription = NULL;
  ...
  if ((::GetVersionEx(&ver) == TRUE) &&
    ...
    typedef HRESULT(WINAPI * LPFN_RegLoadMUIStringW)(
        HKEY, LPCWSTR, LPWSTR, DWORD, LPDWORD, DWORD, LPCWSTR);
    LPFN_RegLoadMUIStringW pfnRegLoadMUIStringW = NULL;

    // Delay bind with RegLoadMUIStringW since this function is not supported on
    // previous versions of advapi32.dll RegLoadMUIStringW is supported only on
    // advapi32.dll that ships with Windows Vista  and above Calling
    // RegLoadMUIStringW directly makes the loader try to resolve the function
    // reference at load time which breaks, hence we manually load advapi32.dll,
    // query for the function pointer and invoke it.
    hmodAdvapi32Dll = ::LoadLibrary(TEXT("advapi32.dll"));
    if (hmodAdvapi32Dll) {
      pfnRegLoadMUIStringW = (LPFN_RegLoadMUIStringW)::GetProcAddress(
          hmodAdvapi32Dll, "RegLoadMUIStringW");
    ...
    if (SUCCEEDED(hr)) {
      ...
      pRegKeyPath = wcschr(pszTemp, L'\\'); // Find the first occurance of '\\'
                                            // in the absolute registry key path
      if (pRegKeyPath) {
        *pRegKeyPath = L'\0';
        pRegKeyPath++; // pRegKeyPath now points to the path to the recognizer
                       // token under the HKLM or HKCR hive
        *ppszDescription = 0;

        // Open the registry key for read and get the handle
        if (wcsncmp(pszTemp, L"HKEY_LOCAL_MACHINE", MAX_PATH) == 0) {
          lErrorCode = RegOpenKeyExW(HKEY_LOCAL_MACHINE, pRegKeyPath, 0,
                                     KEY_QUERY_VALUE, &Handle);
        } else if (wcsncmp(pszTemp, L"HKEY_CURRENT_USER", MAX_PATH) == 0) {
          lErrorCode = RegOpenKeyExW(HKEY_CURRENT_USER, pRegKeyPath, 0,
                                     KEY_QUERY_VALUE, &Handle);
        } else {
        ...
        if (ERROR_SUCCESS == lErrorCode) {
          *ppszDescription = (WCHAR *)CoTaskMemAlloc(
              MAX_PATH * sizeof(WCHAR)); // This should be enough memory to
                                         // allocate the localized Engine Name
          lErrorCode = (*pfnRegLoadMUIStringW)(
              Handle, SR_LOCALIZED_DESCRIPTION, *ppszDescription,
              MAX_PATH * sizeof(WCHAR), NULL, REG_MUI_STRING_TRUNCATE, NULL);
        }
      } else {
        // pRegKeyPath should never be 0 if we are querying for relative hkey
        // path
        lErrorCode = ERROR_BAD_ARGUMENTS;
      }
  ...
  // If running on OSes released before Windows Vista query the localized string
  // from the registry If RegLoadMUIStringW failed to retrieved the localized
  // Engine name retrieve the localized string from the fallback (Default)
  // attribute
#else
  hr = E_FAIL;
...
inline HRESULT SpSetDescription(ISpObjectToken *pObjToken,
 ...
 *
 *****************************************************************************/
inline HRESULT SpConvertStreamFormatEnum(SPSTREAMFORMAT eFormat,
                                         GUID *pFormatId,
  ...
  if (eFormat >= SPSF_8kHz8BitMono && eFormat <= SPSF_48kHz16BitStereo) {
    ...
    *ppCoMemWaveFormatEx = pwfex;
    if (pwfex) {
      ...
      static const DWORD adwKHZ[] = {8000,  11025, 12000, 16000, 22050,
                                     24000, 32000, 44100, 48000};
      pwfex->wFormatTag = WAVE_FORMAT_PCM;
      pwfex->nChannels = pwfex->nBlockAlign = (WORD)(bIsStereo ? 2 : 1);
      pwfex->nSamplesPerSec = (dwKHZ < sizeof(adwKHZ) / sizeof(adwKHZ[0]))
                                  ? adwKHZ[dwKHZ]
                                  : adwKHZ[0];
    ...
    *ppCoMemWaveFormatEx = pwfex;
    if (pwfex) {
      ...
      static const DWORD adwKHZ[] = {8000, 11025, 22050, 44100};
      static const DWORD BytesPerSec[] = {4096,  8192,  5644,  11289,
                                          11155, 22311, 22179, 44359};
      ...
      pwfex->nAvgBytesPerSec =
          (dwIndex < sizeof(BytesPerSec) / sizeof(BytesPerSec[0]))
              ? BytesPerSec[dwIndex]
              : BytesPerSec[0];
      pwfex->nBlockAlign =
          (WORD)(((dwKHZ < sizeof(BlockAlign) / sizeof(BlockAlign[0]))
                      ? BlockAlign[dwKHZ]
                      : BlockAlign[0]) *
                 pwfex->nChannels);
      ...
      BYTE *pExtra = ((BYTE *)pwfex) + sizeof(WAVEFORMATEX);
      memcpy(pExtra,
             (dwKHZ < sizeof(Extra) / sizeof(Extra[0])) ? Extra[dwKHZ]
                                                        : Extra[0],
             32);
      pFmtGuid = &SPDFID_WaveFormatEx;
    } else {
    ...
    *ppCoMemWaveFormatEx = pwfex;
    if (pwfex) {
      ...
      pwfex->nChannels = 1;
      pwfex->nSamplesPerSec = (dwIndex < sizeof(adwKHZ) / sizeof(adwKHZ[0]))
                                  ? adwKHZ[dwIndex]
                                  : adwKHZ[0];
...
public:
  ...
  static HRESULT CoMemCopyWFEX(const WAVEFORMATEX *pSrc,
                               WAVEFORMATEX **ppCoMemWFEX) {
    if (pSrc == NULL || pSrc->nAvgBytesPerSec == 0 || pSrc->nBlockAlign == 0 ||
        pSrc->nChannels == 0) // There are other fields like wBitsPerSample but
                              // these can be zero in some formats.
    {
      return E_INVALIDARG;
    }
    ...
    if (pSrc->wFormatTag != WAVE_FORMAT_PCM) {
      // Add the extra data size in but ignore for WAVE_FORMAT_PCM {accoring to
      // MSDN this should be ignored}.
      cb += pSrc->cbSize;
      if (cb < pSrc->cbSize) {
        return E_INVALIDARG;
      }
    ...
    *ppCoMemWFEX = (WAVEFORMATEX *)::CoTaskMemAlloc(cb);
    if (*ppCoMemWFEX) {
      ...
      if ((*ppCoMemWFEX)->wFormatTag == WAVE_FORMAT_PCM) {
        (*ppCoMemWFEX)->cbSize =
            0; // Always set cbSize to zero for WAVE_FORMAT_PCM.
      }
      return S_OK;
    } else {
      return E_OUTOFMEMORY;
  ...
  CSpStreamFormat(SPSTREAMFORMAT eFormat, HRESULT *phr) {
    *phr = SpConvertStreamFormatEnum(eFormat, &m_guidFormatId,
  ...
  HRESULT AssignFormat(SPSTREAMFORMAT eFormat) {
    ::CoTaskMemFree(m_pCoMemWaveFormatEx);
    return SpConvertStreamFormatEnum(eFormat, &m_guidFormatId,
                                     &m_pCoMemWaveFormatEx);
  }
  ...
  HRESULT AssignFormat(REFGUID rguidFormatId,
    ...
    if (rguidFormatId == m_guidFormatId) {
      if (m_pCoMemWaveFormatEx) {
        if (pwfex && pwfex->cbSize == m_pCoMemWaveFormatEx->cbSize &&
            memcmp(m_pCoMemWaveFormatEx, pwfex,
                   sizeof(WAVEFORMATEX) + pwfex->cbSize) == 0) {
          return TRUE;
        }
      } else {
        return (pwfex == NULL);
  ...
  HRESULT ParamValidateAssignFormat(REFGUID rguidFormatId,
                                    const WAVEFORMATEX *pWaveFormatEx,
  ...
  HRESULT AssignFormat(const CSpStreamFormat &Src) { return Src.CopyTo(*this); }

  HRESULT ParamValidateCopyTo(GUID *pFormatId,
                              WAVEFORMATEX **ppCoMemWFEX) const {
  ...
  ULONG SerializeSize() const {
    ...
    if (m_pCoMemWaveFormatEx) {
      if (m_pCoMemWaveFormatEx->cbSize != 0 &&
          m_pCoMemWaveFormatEx->wFormatTag == WAVE_FORMAT_PCM) {
        _ASSERT(!"PCM wave format");
        SetLastError(ERROR_INVALID_STATE);
        return 0;
      }
      cb += sizeof(WAVEFORMATEX) + m_pCoMemWaveFormatEx->cbSize +
            3;      // Add 3 to round up
  ...
  ULONG Serialize(_Out_writes_bytes_(cbBuffer) BYTE *pBuffer,
                  _In_ ULONG cbBuffer) const {
    ...
    *((UNALIGNED GUID *)pBuffer) = m_guidFormatId;
    if (m_pCoMemWaveFormatEx) {
      ...
      memcpy(pBuffer, m_pCoMemWaveFormatEx,
             sizeof(WAVEFORMATEX) + m_pCoMemWaveFormatEx->cbSize);
    }
    return cb;
  }
...
// Note some of the newer locales do not have associated Windows codepages.  For
// these, we return UTF-8.

inline UINT SpCodePageFromLcid(LCID lcid) {
  WCHAR achCodePage[6];

  return (0 != GetLocaleInfoW(lcid, LOCALE_IDEFAULTANSICODEPAGE, achCodePage,
                              sizeof(achCodePage) / sizeof(*achCodePage)))
             ? _wtoi(achCodePage)
             : 65001;
}

inline HRESULT SPBindToFile(LPCWSTR pFileName, SPFILEMODE eMode,
                            ISpStream **ppStream, const GUID *pFormatId = NULL,
  ...
  if (SUCCEEDED(hr)) {
    hr = (*ppStream)->BindToFile(pFileName, eMode, pFormatId, pWaveFormatEx,
                                 ullEventInterest);
    if (FAILED(hr)) {
      (*ppStream)->Release();
      *ppStream = NULL;
...
inline HRESULT SPBindToFile(const CHAR *pFileName, SPFILEMODE eMode,
                            ISpStream **ppStream, const GUID *pFormatId = NULL,
                            const WAVEFORMATEX *pWaveFormatEx = NULL,
                            ULONGLONG ullEventInterest = SPFEI_ALL_EVENTS) {
  WCHAR szWcharFileName[MAX_PATH];
  ::MultiByteToWideChar(CP_ACP, 0, pFileName, -1, szWcharFileName,
                        sp_countof(szWcharFileName));
  return SPBindToFile(szWcharFileName, eMode, ppStream, pFormatId,
 ...
 *--------------*
 *   Description:
 *       Checks a string is not longer that the supplied size {IN BYTES},
 * including the terminating char. If pcb is non-null then it is set to the size
 * of the string {IN BYTES}, including the terminating char.
 *
 *****************************************************************************/
inline HRESULT CheckStringSizeBytes(WCHAR const *psz, size_t cbMax,
                                    size_t *pcb) {
  HRESULT hr = S_OK;
 ...
 *--------------*
 *   Description:
 *       Helper function that can be used by clients that do not use the
 * CSpEvent class.
 *
 *   Returns:
 *
 ...
 *----------------------*
 *   Description:
 *       Computes the required size of a buffer to serialize an event.  The
 * caller must specify which type of serialized event is desired -- either
 * SPSERIALIZEDEVENT or SPSERIALIZEDEVENT64.
 *
 *   Returns:
...
template <class T> inline ULONG SpEventSerializeSize(const SPEVENT *pEvent) {
  ...
  } else if ((pEvent->elParamType == SPET_LPARAM_IS_STRING) &&
             pEvent->lParam != NULL) {
 ...
 *   pass a pointer to either a SPSERIAILZEDEVENT or SPSERIALIZEDEVENT64
 * structure.
 *
 *   Returns:
...
template <class T> inline ULONG SpSerializedEventSize(const T *pSerEvent) {
  ULONG ulSize = sizeof(T);

  if ((pSerEvent->elParamType == SPET_LPARAM_IS_POINTER) &&
      pSerEvent->SerializedlParam) {
    ulSize += ULONG(pSerEvent->SerializedwParam);
  } else if ((pSerEvent->elParamType == SPET_LPARAM_IS_STRING ||
              pSerEvent->elParamType == SPET_LPARAM_IS_TOKEN) &&
...
public:
  CSpEvent() { SpInitEvent(this); }
  ~CSpEvent() { SpClearEvent(this); }
  // If you need to take the address of a CSpEvent that is not const, use the
  // AddrOf() method which will do debug checking of parameters.  If you
  // encounter this problem when calling GetEvents from an event source, you may
  // want to use the GetFrom() method of this class.
  const SPEVENT *operator&() { return this; }
  CSpEvent *AddrOf() {
    // Note:  This method does not ASSERT since we assume the caller knows what
    // they are doing.
    return this;
  }
  void Clear() { SpClearEvent(this); }
  ...
  template <class T> HRESULT Serialize(T *pSerEvent) const {
    ...
    if (lParam) {
      ...
      case SPET_LPARAM_IS_STRING:
        StringCchCopyW((WCHAR *)(pSerEvent + 1), wcslen((WCHAR *)lParam) + 1,
                       (WCHAR *)lParam);
        pSerEvent->SerializedlParam = sizeof(T);
        break;
      ...
      case SPET_LPARAM_IS_TOKEN: {
        ...
        if (SUCCEEDED(hr)) {
          pSerEvent->SerializedwParam =
              (dstrObjectId.Length() + 1) * sizeof(WCHAR);
          ;
  ...
  ISpRecoResult *RecoResult() const { return (ISpRecoResult *)Object(); }
  ...
  BOOL IsPaused() { return (BOOL)(wParam & SPREF_AutoPause); }
  BOOL IsEmulated() { return (BOOL)(wParam & SPREF_Emulated); }

#if _SAPI_VER >= 0x053
  BOOL IsSMLTimeout() { return (BOOL)(wParam & SPREF_SMLTimeout); }
#endif
  ...
  const WCHAR *PropertyName() const {
    _ASSERT((eEventId == SPEI_PROPERTY_NUM_CHANGE &&
            ...
            (eEventId == SPEI_PROPERTY_STRING_CHANGE &&
             elParamType == SPET_LPARAM_IS_POINTER));
    // Note: Don't use String() method here since in the case of string
    // attributes, the elParamType field specifies LPARAM_IS_POINTER, but the
    // attribute name IS the first string in this buffer
    return (const WCHAR *)lParam;
  }
...
public:
  ...
  ~CSpPhrasePtr() { ::CoTaskMemFree(m_pPhrase); }
  // The assert on operator& usually indicates a bug.  If this is really
  // what is needed, however, take the address of the m_pPhrase member
  // explicitly.
  SPPHRASE **operator&() {
    _ASSERT(m_pPhrase == NULL);
    return &m_pPhrase;
  }
  operator SPPHRASE *() const { return m_pPhrase; }
  SPPHRASE &operator*() const {
    _ASSERT(m_pPhrase);
    return *m_pPhrase;
  }
  SPPHRASE *operator->() const { return m_pPhrase; }
  bool operator!() const { return (m_pPhrase == NULL); }
  void Clear() {
    if (m_pPhrase) {
...
public:
  ...
  ~CSpCoTaskMemPtr() { ::CoTaskMemFree(m_pT); }
  void Clear() {
    if (m_pT) {
      ::CoTaskMemFree(m_pT);
      m_pT = NULL;
...
/**** Helper function used to create a new phrase object from an array of
    ...
    If prgDispAttribs is NULL then the DisplayAttribs for each element default
   to SPAF_ONE_TRAILING_SPACE. ****/
inline HRESULT
CreatePhraseFromWordArray(const WCHAR **ppWords, ULONG cWords,
  ...
  ULONG i;
  WCHAR **pStringPtrArray =
      (WCHAR **)::CoTaskMemAlloc(cWords * sizeof(WCHAR *));
  if (!pStringPtrArray) {
    return E_OUTOFMEMORY;
  }
  for (i = 0; i < cWords; i++) {
  ...
  for (i = 0; SUCCEEDED(hr) && i < cWords; i++) {
    ...
    if (ppWords[i][0] == L'/' && !fNoSpecialCharacters) {
      ...
      *p = L'\0';
      ...
      pPhraseElement[i].pszLexicalForm =
          pszSecondPart ? pszSecondPart : pszFirstPart;
    ...
    pPhraseElement[i].bDisplayAttributes =
        (BYTE)(prgDispAttribs ? prgDispAttribs[i] : SPAF_ONE_TRAILING_SPACE);
    pPhraseElement[i].RequiredConfidence = SP_NORMAL_CONFIDENCE;
    pPhraseElement[i].ActualConfidence = SP_NORMAL_CONFIDENCE;
    pPhraseElement[i].SREngineConfidence =
        1.0f; // Emulated results give confidence of 1.0
  }
...
/**** Helper function used to create a new phrase object from a
    ...
    ****/
inline HRESULT
CreatePhraseFromText(const WCHAR *pszOriginalText,
                     ISpPhraseBuilder **ppResultPhrase, LANGID LangId = 0,
                     CComPtr<ISpPhoneConverter> cpPhoneConv = NULL,
                     BOOL fNoSpecialCharacters = FALSE) {
  HRESULT hr = S_OK;
  ...
  *p = L'\0';
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = CreatePhraseFromWordArray((const WCHAR **)pStringPtrArray, cWords,
                                   NULL, ppResultPhrase, LangId, cpPhoneConv,
                                   fNoSpecialCharacters);
