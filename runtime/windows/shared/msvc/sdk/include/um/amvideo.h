...
DECLARE_INTERFACE_(IDirectDrawVideo, IUnknown) {
  ...
  STDMETHOD(GetFourCCCodes)(THIS_ _Out_ DWORD * pCount, _Out_ DWORD * pCodes)
      PURE;
...
DECLARE_INTERFACE_(IFullScreenVideo, IUnknown) {
  ...
  STDMETHOD(GetModeInfo)(THIS_ long Mode, _Out_ long *pWidth,
                         _Out_ long *pHeight, _Out_ long *pDepth) PURE;
...
DECLARE_INTERFACE_(IFullScreenVideoEx, IFullScreenVideo) {
  ...
  STDMETHOD(GetAcceleratorTable)(THIS_ _Out_ HWND * phwnd, HACCEL * phAccel)
      PURE;
...
// Different from DIBSIZE, RAWSIZE does NOT align the width to be multiple of 4
// bytes. Given width, height, and bitCount, RAWSIZE calculates the image size
// without any extra padding at the end of each row.
#define WIDTHBYTES_RAW(bits) ((DWORD)((bits) + 7) / 8)
#define RAWWIDTHBYTES(bi)                                                      \
  (DWORD) WIDTHBYTES_RAW((DWORD)(bi).biWidth *(DWORD)(bi).biBitCount)
...
__inline HRESULT SAFE_DIBWIDTHBYTES(_In_ const BITMAPINFOHEADER *pbi,
                                    _Out_ DWORD *pcbWidth) {
  DWORD dw;
  HRESULT hr;
...
__inline HRESULT SAFE_DIBSIZE(_In_ const BITMAPINFOHEADER *pbi,
                              _Out_ DWORD *pcbSize) {
  DWORD dw;
  DWORD dwWidthBytes;
  HRESULT hr;
