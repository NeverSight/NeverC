...
// MessageText:
//
// The Windows Search Service is starting up and attempting to remove the old
// search index {Reason: %2}. %1
//
...
// MessageText:
//
// The Windows Search Service has failed to remove the old search index.
// Internal error <%2,%3>. %1
//
...
// MessageText:
//
// Event ID %2 for the Windows Search Service has been suppressed %3 time(s)
// since %4. This event is used to suppress Windows Search Service events that
// have occurred frequently within a short period of time.  See Event ID %2 for
// further details on this event.%1
//
...
// MessageText:
//
// The Windows Search Service failed to move Index files from %2 to %3 with the
// following error: <%4,%5>. This might be because the target directory is not
// empty, or because the SYSTEM account doesn't have write access to the target
// directory. %1
//
...
// MessageText:
//
// While rolling back the index, the Windows Search Service encountered the
// following error: <%4,%5>. Index files were not moved from %2 to %3. %1
//
...
// MessageText:
//
// Windows Search Service failed to process the list of included and excluded
// locations with the error <%2, %3, "%4">. %1
//
...
// If this causes problems, contact Microsoft Product Support Services and
// include the stack trace in the event. %2.
//
...
// MessageText:
//
// Performance monitoring cannot be initialized for the gatherer service,
// because the counters are not loaded or the shared memory object cannot be
// opened. This only affects availability of the perfmon counters. Restart the
// computer.%1
//
...
// MessageText:
//
// Performance monitoring cannot be initialized for the gatherer object, because
// the counters are not loaded or the shared memory object cannot be opened.
// This only affects availability of the perfmon counters. Restart the
// computer.%1
//
...
// MessageText:
//
// An exception occurred. ID: %2. This is an internal error. Reproduce the error
// with the debugger attached and enable exceptions, then contact product
// support. One of the components loaded in your system is bad. You may be able
// to avoid the problem by recreating the index.%1
//
...
// MessageText:
//
// The update is complete. The gatherer successfully processed %2 documents
// totaling %3K. It failed to filter %4 documents. %5 documents were modified.
// %6 URLs were not found or denied access.%1
//
...
// MessageText:
//
// Internal gatherer error %2 occurred. Please contact Microsoft Product Support
// Services.%1
//
...
// MessageText:
//
// The update cannot be started because all of the content sources were excluded
// by site path rules, or removed from the index configuration.%1
//
...
// Fix the errors and try the update again.%1
//
...
// MessageText:
//
// Critical error %2 occurred, and the index was shut down. The system is
// probably low on resources. Free up resources and restart the service.%1
//
...
// MessageText:
//
// Advise Status Change failed. The system is probably low on resources. Free up
// resources and restart the service.%1
//
...
// MessageText:
//
// The registry version does not match with the expected <%2>, or the registry
// cannot be accessed because the service account does not have the correct
// permissions.  Uninstall the previous version before installing the new one.%1
//
...
// MessageText:
//
// A request to start the update has been ignored because the update is already
// in progress or is scheduled on one or more content sources.%1
//
...
// MessageText:
//
// The update for the index cannot be started because the specified content
...
// MessageText:
//
// The incremental update is complete. The gatherer successfully processed %2
// documents totaling %3K. It failed to filter %4 documents. %5 documents were
// modified. %6 URLs were not found or denied access.%1
//
...
// MessageText:
//
// No documents were accessed because no e-mail address is specified in the
// content index server properties. Specify the e-mail address in the service
// configuration.%1
//
...
// MessageText:
//
// The previous update was reset, or was otherwise interrupted. A full update of
// all content sources will be automatically started. %1
//
...
// MessageText:
//
// The update has been delayed because a disk is full. Check the system default
// temp location and the drive on which search catalog is created. The system
// default temp location is used for creation of temporary files during
// crawling. If it is full, crawling pauses. If the system default temp location
// is full, change the location to a disk with more free space and restart the
// computer. Changes to the system temp location do not take effect for system
// services until the computer is restarted.%1
//
...
// MessageText:
//
// The gatherer property mapping file cannot be opened. The default values are
// being used. You may have to copy the property mapping file from the setup CD,
// or reinstall the application.%1
//
...
// MessageText:
//
// The automatic description encoding tag value is invalid. The gatherer is
// setting this value to "yes"". Fix the gthrprm.txt file.%1
//
...
// MessageText:
//
// An update cannot begin because the content source <%2> is in use by another
// update already in progress. The update will start as soon as all its content
// sources are released by updates already in progress.%1
//
...
// MessageText:
//
// The adaptive update is complete. The gatherer successfully processed %2
// documents totaling %3K. It failed to filter %4 documents. %5 documents were
// modified. %6 URLs were not found or denied access.%1
//
...
// MessageText:
//
// The gatherer is using the word breaker for language id <%2> for text in
// language id <%3>. The corresponding language resources are not installed on
// your computer.%1
//
...
// MessageText:
//
// The index cannot be restored. You may need to delete and recreate the
// index.%1
//
...
// MessageText:
//
// Performance monitoring cannot be initialized for the topic assistant object,
// because the counters are not loaded or the shared memory object cannot be
// opened. This only affects availability of the perfmon counters. Restart the
// computer.%1
//
...
// MessageText:
//
// The gatherer is recovering after an improper shutdown.  This will delay
// availability of gathering functions, and may result in some noncritical data
// loss.%1
//
...
// MessageText:
//
// The gatherer detected pages in the history during recovery that cannot be
// read, and repaired them.  However, statistical data for some URLs may have
// been lost.  This can be caused by restarting a computer without first
// shutting down Windows, or by disk failure.%1
//
...
// MessageText:
//
// The application restore failed. This can be caused by system errors
// (indicated by previously logged events), or a corrupt backup image (retry
// restore with valid backup image). %1
//
...
// MessageText:
//
// The application cannot be backed up. You may be unable to restore from the
// backup image.%1
//
...
// MessageText:
//
// The Windows Search service stopped the Protocol Host process because it was
// consuming too many resources.  A new Protocol Host process will be started.
// No user action is required.%1
//
...
// MessageText:
//
// Notifications for the scope %2 are not active. The event logging threshold
// for this scope was exceeded.  No further events will be sent for one hour. %1
//
...
// MessageText:
//
// Notifications for the scope %2 were reactivated. The event logging threshold
// for this scope was exceeded.  No further events will be sent for one hour. %1
//
...
// MessageText:
//
// The application network access account is invalid.  Update the account with a
...
// MessageText:
//
// The system locale has changed. Existing data will be deleted and the index
// must be recreated.%1
//
...
// MessageText:
//
// The gatherer files cannot be flushed, and this action cannot be completed.
// The gatherer will attempt to flush files again. If the problem persists,
// restart the service, free system resources or verify that your hardware is
// working properly. %1
//
...
// MessageText:
//
// The checkpoint record cannot be updated, and this action cannot be completed.
// The gatherer will attempt to update the checkpoint record again. If the
// problem persists, restart the service, free system resources or verify that
// your hardware is working properly. %1
//
...
// MessageText:
//
// The gatherer files cannot be saved, and this action cannot be completed. The
// gatherer will attempt to save the files again. If the problem persists,
// restart the service, free system resources or verify that your hardware is
// working properly. %1
//
...
// MessageText:
//
// The gatherer files from the previous checkpoint cannot be restored, and this
// action cannot be completed. The gatherer will attempt to restore the files
// again. If the problem persists, restart the service, free system resources or
// verify that your hardware is working properly. %1
//
...
// MessageText:
//
// The checkpoint record cannot be read, and this action cannot be completed. If
// the problem persists, restart the service, free system resources or verify
// that your hardware is working properly. %1
//
...
// MessageText:
//
// The project cannot be initialized, because the checkpoint record cannot be
// read. The data structures on the disk will be reset.  Verify that your
// hardware is working properly. %1
//
...
// MessageText:
//
// The project cannot be initialized, because one of the checkpoint files is
// missing. The data structures on the disk will be reset.  Check to see if
// someone is manually deleting files, and verify that your hardware is working
// properly. %1
//
...
// MessageText:
//
// The group %2\%3 contains %4 members. Groups over %5 members are not expanded.
// %1
//
...
// MessageText:
//
// Unable to create file share to accept topic assistant training data. Reason:
// %2.%1
//
...
// MessageText:
//
// Unable to terminate notifications normally.  Restart the service or contact
// Product Support.%1
//
...
// MessageText:
//
// The per-user filter pool for session %2 and user context token %3 could not
// be added <%4,%5>.%1
//
...
// MessageText:
//
// Error ID %2 happened in Windows Search recovery stage, please restart the
// service. If this error persists, please recreate the index.%1
//
...
// MessageText:
//
// Directory location <%2> is invalid. The application configuration cannot be
// read.  Reinstall the application.%1
//
...
// MessageText:
//
// The update was paused because the disk <%2> is full. Free up disk space to
// continue crawling the index.%1
//
...
// MessageText:
//
// Index propagation to search server <%2> succeeded.  The service is now
// waiting for the search server to accept the propagation.%1
//
...
// MessageText:
//
// There are no search servers selected.  Reconfigure the index to identify a
// suitable search server.  If the problem persists, delete and recreate the
// index.%1
//
...
// MessageText:
//
// The search service has detected corrupted data files in the index {id=%2}.
// The service will attempt to automatically correct this problem by rebuilding
// the index.%1
//
...
// MessageText:
//
// The Windows Search Service is being stopped because there is a problem with
// the indexer: %2.%1
//
...
// MessageText:
//
// Propagation stopped because the propagation state for search server <%2>
// cannot be read. Try propagation again.  If this error persists, delete and
// recreate the propagated index.%1
//
...
// MessageText:
//
// The index is being copied to the drop location in preparation for
// propagation.%1
//
...
// MessageText:
//
// The content index server received an "Access Denied"" error while propagating
// to the <%2> search server. This usually results from an incorrect
// configuration of the propagation account. Check that the propagation account
// is valid.%1
//
...
// MessageText:
//
// Search server <%2> cannot be contacted. However, <%2> was successfully added
// as a propagation recipient.%1
//
...
// MessageText:
//
// Performance monitoring cannot be initialized because the counters are not
// loaded or the shared memory object cannot be opened. Stop and restart the
// search service.  If this error continues, reinstall the application.%1
//
...
// MessageText:
//
// Configuration directory %2 is missing, and disaster recovery must be
// performed. If there are existing indexes, they must be restored from the last
// backup. If there is no backup of index data, then delete the catalogs and
// recreate them.%1
//
...
// MessageText:
//
// The registry cannot be read, possibly because the registry keys for this
// index are missing. You may have to delete and recreate the index %1.
//
...
// MessageText:
//
// Index propagation failed to commit to any of the search servers.  The Windows
// Search Service is now trying to revert the search servers to the previous
// index. %1
//
...
// MessageText:
//
// The Windows Search Service cannot initialize multi-instancing in Jet. If the
// application is used in a cluster environment, all applications using Jet will
// fail in the same group.%1
//
...
// uebug Unless it is absolutely necessary, the "Unexpected Win32  rror:" part
// of the message should be removed.
//
...
// MessageText:
//
// Performance Counters could not be loaded for %1 for instance %2 %3 due to the
// following error: %4.
//
...
// MessageText:
//
// Could not get performance counter registry info for %1 for instance %2 %3 due
// to the following error: %4.
//
...
// MessageText:
//
// Performance counters will not be loaded because the named objects (shared
// memory or events) are in use for %1 for instance %2 %3.
//
...
// MessageText:
//
// The protocol host process %2 did not respond and is being forcibly terminated
// {filter host process %3}. %1
//
...
// MessageText:
//
// The filter host process %2 did not respond and is being forcibly terminated.
// %1
//
...
// MessageText:
//
// The search service has failed to create database instance for the index {%1}
// due to maximum number of instance reached. Please re-configure maximum value
...
// MessageText:
//
// The search service has failed to configure maximum number of database
// instance. {%1}. Please re-configure maximum value and restart the service.
//
...
// MessageText:
//
// The search service has failed to create or load catalog for an user with SID
// {%2}. Please inspect a profile directory for the user is accessible. You
// should re-start the search service after fixing any issue found from the
// profile directory. %1
//
...
// MessageText:
//
// This scope has no default namespace.  Add a default namespace before
// continuing.
//
#define XML_E_NODEFAULTNS 0x80042200L

// uebug SXQL is jargon.  Use a more familiar term, if possible, and/or suggest
// an action that makes the error clear.
//
...
//  MessageText:
//
//  The specified application name is not valid. Check to see whether a search
//  application with that name already exists.
//
...
//  MessageText:
//
//  The specified search application name was not found. It may have already
//  been deleted.
//
...
// MessageText:
//
// The specified catalog was not found. Check to see if it was deleted, or if
// there are errors in your application code.
//
#define MSS_E_CATALOGNOTFOUND 0x80042103L

// uebug The phrase "it is stopping" is unclear.  What sort of action is
// "stopping?  Use clearer terminology.
//
//  MessageId: MSS_E_CATALOGSTOPPING
//
//  MessageText:
//
//  The content index cannot be deleted while the search service is stopped.
//  Restart the search service and try again.
//
...
// MessageText:
//
// This file is shorter than 2 bytes. Unicode text file must begin with a wide
// character that indicates byte order.
//
...
// MessageText:
//
// The search service is running as a local System service. To access Exchange
// documents, check that the search service is running in a user account with
...
// MessageText:
//
// The Exchange search provider cannot be initialized. You may have to reinstall
// the  application.
//
...
// MessageText:
//
// The security provider was not initialized. You may have to reinstall the
// application.
//
...
// MessageText:
//
// Access is denied. You may not have sufficient privileges to perform the
// operation.
//
...
//  MessageText:
//
//  The requested content index name is not valid. Use a valid content index
//  name.
//
...
// MessageText:
//
// The requested content index is disabled on the search server. Contact the
// system administrator to enable the content index. Check the event log for
// possible errors.
//
...
// MessageText:
//
// The request cannot be processed, because the system is low on resources. Try
// again later, or contact your system administrator to free up system
// resources.
//
...
//  MessageText:
//
//  The request or function call is out of sequence. Check for programming
//  errors.
//
...
// MessageText:
//
// The search server cannot find one or more catalog files. Check the event log
...
// MessageText:
//
// The SetStatus operation cannot be completed, because another SetStatus
// operation is already in progress. Try again after the current operation is
// completed.
//
...
// MessageText:
//
// The content index cannot be created, because another content index with the
// same name already exists. Create the content index using a different name.
//
#define DS_E_DUPLICATEID 0x80040412L

// uebug suggest: "Increase the number of possible catalogs, or remove another
// catalog."
//
...
//  MessageText:
//
//  The content index cannot be created, because the limit on the number of
//  indexes was reached.
//
...
// MessageText:
//
// The search server cannot read or write to the registry. Check that the search
// server is running with administrative privileges.
//
...
// MessageText:
//
// The content index cannot be removed in the current state. Disable and remove
// the content index.
//
...
// MessageText:
//
// The content index name is in the wrong format. Use a name without spaces or
// punctuation.
//
...
// MessageText:
//
// There is a mismatch in the protocol between the search service and the
// client. Install the correct version of the client.
//
...
// MessageText:
//
// The content index directory structure cannot be created. See the event log
// for related errors.
//
#define DS_E_INDEXDIRECTORY 0x8004041CL

// uebug The value of what?  Point out the maximum length in the suggested
// action. this is not in use.
//
...
// uebug It's unclear what operation the buffer is too small for, or whether
// that buffer can be changed.
//
...
// MessageText:
//
// The operation cannot be completed, because one or more queries failed to
...
// MessageText:
//
// The registry data cannot be written. Check that the search service is running
...
// MessageText:
//
// The content index cannot be created because an index with the same name
// already exists on the search server. Create the content index using a
// different name.
//
...
// MessageText:
//
// The content index cannot be enabled because its propagation version cannot be
// verified against the dedicated index server.  This can occur when either the
// dedicated index server is offline or the content index is out-of-date.
//
...
// MessageText:
//
// The query cannot be collated, because columns using the same name have
// different types.
//
...
// MessageText:
//
// The requested content index is empty or has not been added to the search
// server.
//
...
// MessageText:
//
// The column type in the search query does not match the column type in the
// index.
//
...
// uebug "query start hit" might not be clear to users.  Is there a clearer way
// of saying this?
//
...
// MessageText:
//
// The query found some matching items, but is unable to compile the entire
// result set.
//
...
// uebug What is meant by "the current context"?  Be specific.  How is it
// invalid?  Provide a valid operation for the context.
//
...
// MessageText:
//
// The configuration file attempted to load a reserved column. This error is
// obsolete and should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The tag nesting configuration for the application is not valid.  Reinstall
// the application.
//
...
// MessageText:
//
// The configuration file for the application contains an invalid tag. Reinstall
// the application.
//
...
// MessageText:
//
// The configuration file version number for the application is invalid.
...
// MessageText:
//
// The output configuration file cannot be created. Stop and restart the system,
...
// MessageText:
//
// Data cannot be written to the output configuration file. This error is
// obsolete and should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The configuration file for the application contains invalid information.
// Reinstall the application.
//
...
// MessageText:
//
// The configuration file type for the application is not recognized. Reinstall
// the application.
//
...
// MessageText:
//
// The configuration file column data type for the application is not
// recognized. Reinstall the application.
//
...
// MessageText:
//
// The noise word file cannot be created. This error is obsolete and should no
// longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// There was an error writing to the temporary noise word file. This error is
// obsolete and should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The specified column name already exists. The application's configuration was
// corrupted.  Reinstall the application.
//
...
// MessageText:
//
// The value for the attribute is invalid. The application's configuration was
// corrupted.  Reinstall the application.
//
...
// MessageText:
//
// The property set GUID is invalid. The application's configuration was
// corrupted.  Reinstall the application.
//
...
// MessageText:
//
// No more columns.  This is an internal error code and should not be reported
// to users.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The system cannot write to the configuration file, because it was modified
// since it was last read. Stop and restart the search service.  If this problem
...
// MessageText:
//
// The specified column name is invalid. The application's configuration has
// been corrupted.  Reinstall the application.
//
...
// MessageText:
//
// The property ID specified is invalid. The application's configuration has
// been corrupted.  Reinstall the application.
//
...
// MessageText:
//
// An invalid attribute was specified for this tag, or a required attribute is
// missing. The application's configuration was corrupted.  Reinstall the
// application.
//
...
// MessageText:
//
// The specified file name is invalid. This error is obsolete and should no
// longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// A column having the specified property set and PID already exists. The
// application's configuration has been corrupted.  Reinstall the application.
//
...
// MessageText:
//
// The configuration describing a language and sublanguage contains unexpected
// duplicate information. The application's configuration may be corrupted.
// Though basic functionality is unlikely to be impacted, it is recommended that
...
// MessageText:
//
// The object you are trying to create already exists. Try again using a
// different name.
//
...
// MessageText:
//
// The value cannot be set, because the object was already deleted or was not
// initialized properly. Check that the object reference is still valid,
// increase the registry size, or recreate the content index configuration.
//
...
// MessageText:
//
// Performance monitoring cannot be initialized. This affects performance
// counters only. Try restarting the service or restarting the server.
//
...
// MessageText:
//
// A duplicate index entry exists in the registry. No other indexes are
// affected, but it is recommended that the duplicate entry be deleted.
//
...
// MessageText:
//
// A duplicate extension entry exists in the registry. The duplicate extension
// should be deleted, but all other extensions are unaffected.
//
...
// MessageText:
//
// The URL was already processed during this update. If you received this
// message while processing alerts, then the alerts are redundant, or else
// Modify should be used instead of Add.
//
...
// MessageText:
//
// The gatherer attempted to create more gatherer plug-in objects than are
// allowed. Remove another plug-in before adding this one.
//
...
// MessageText:
//
// The function is not implemented in this context, and cannot be called.  This
// error is internal, and cannot normally occur. Reinstall the application.
//
...
// MessageText:
//
// The plug-in did not provide a filter sink object. You may have installed a
// custom plug-in that is not supported.
//
...
// MessageText:
//
// The filtering process ended and is now unavailable. The address will be
// retried, but it is possible that the system is low in resources. Free some
// system resources, or restart the update when resources are available.
//
...
// MessageText:
//
// An unexpected message was received from the filtering process. The address
// will be retried, but if this occurs frequently, your system is low on
// resources. Free some system resources, or restart the update when resources
// are available.
//
...
// MessageText:
//
// The filtering process returned an unsupported property type. Fix the filter
// to allow successful use of the document.
//
...
// MessageText:
//
// The specified name for this object exceeds the maximum length, which is
// usually 2047 characters. Use a shorter name for the object.
//
...
// MessageText:
//
// The user agent or e-mail address was not specified. Specify the e-mail
// address to use in the protocol headers.
//
...
// MessageText:
//
// A filter for the document cannot be created. Install the corresponding filter
...
// MessageText:
//
// The ProgID for a protocol handler cannot be obtained, and the protocol
// handler registration is invalid. Reregister the protocol handlers.
//
...
// MessageText:
//
// The gatherer index was not initialized. The content index must be remounted.
// If the index is still not initialized, remove it.
//
...
// MessageText:
//
// The status change request was ignored, because the same status change is
// already pending.
//
...
// MessageText:
//
// The catalog name is invalid. Choose a catalog name that is shorter or does
...
// MessageText:
//
// The requested value was not set by the filter process. This is a recoverable
// internal error. The document may still succeed.
//
...
// MessageText:
//
// This content source cannot be crawled, because its is set up to accept
// alerts.
//
...
// MessageText:
//
// The property mapping is duplicated in the schema. Remove the duplicate
// property schema.
//
...
// MessageText:
//
// The update cannot begin without content sources. Add at least one content
// source that is not used for alerts.
//
...
// MessageText:
//
// The specified account information is incorrect or invalid.  Check that the
// correct account and password are being used.
//
...
// MessageText:
//
// The document cannot be filtered, because the document and filter versions do
// not appear to match. Install a matching document filter.
//
...
// MessageText:
//
// The specified account name cannot be used because it contains invalid
// characters, such as '\'.  Use a valid account name.
//
...
// MessageText:
//
// Failed to load document into IFilter. Possibly caused by unrecognized
// document format or document corruption. Only metadata obtained from the
// document storage was indexed.
//
...
// MessageText:
//
// The server is unavailable and cannot be accessed. The server is probably
// disconnected from the network. Access attempts will be delayed for ten
// minutes.
//
...
// MessageText:
//
// This path does not correspond to the selected site. Use a path name that
// matches the site, or use a wildcard (*).
//
...
// MessageText:
//
// The document cannot be filtered, because the required code page is not
// installed.  Install the code page for this document.
//
...
// MessageText:
//
// No protocol handler is available. Install a protocol handler that can process
// this URL type.
//
...
// MessageText:
//
// Internet Explorer is set to offline mode. Change Internet Explorer to online
// mode, and continue.
//
...
// MessageText:
//
// The filter process cannot be started. The system is most likely low on
// resources, or the filter process binary was modified. If the resources are
// available, check the search binaries with an antivirus program.
//
...
// MessageText:
//
// The specified URL mapping is invalid. The "from"" mapping is empty, or the
// ""from"" mapping is the same as the ""to"" mapping. Specify a valid ""from""
// mapping that differs from the ""to"" mapping.
//
...
// MessageText:
//
// The user agent string was not specified or contains invalid characters.
// Specify a user agent which is not empty and does not contain double quotation
// marks.
//
...
// MessageText:
//
// The specified number of logs to keep is invalid. Set the number of logs to be
// greater than zero.
//
...
// MessageText:
//
// The extension string is not specified or is invalid. Specify an extension
// string that does not contain the following characters: [./?*:\#] or spaces.
//
...
// MessageText:
//
// The specified maximum grow factor is invalid. Set the maximum grow factor to
...
// MessageText:
//
// The specified time out value is invalid. Set the time out value to be greater
...
// MessageText:
//
// The specified retry limit value is invalid. Set the retry limit value to be
...
// MessageText:
//
// The site name is not specified or is invalid. Specify a site name that does
// not contain the following characters: [/\\@#|] or spaces.
//
...
// MessageText:
//
// The content source is not specified or is invalid. Type a host name that does
...
// MessageText:
//
// A catalog with the name specified already exists. Create the catalog using a
// different name.
//
...
// MessageText:
//
// The path is not specified or contains invalid characters, such as ["" ?*].
// Specify a path that does not contain these characters.
//
...
// MessageText:
//
// The filtering was stopped because of a user action, such as stopping the
// crawl.
//
...
// MessageText:
//
// The specified port for the HTTP proxy is invalid. Specify a port between 0
// and 0xffff
//
...
// MessageText:
//
// The path expression cannot contain the reserved escape character '|'. Use an
// expression without reserved characters.
// |' - a reserved escape character. Use an expression without reserved
// characters.
//
...
// MessageText:
//
// The specification for the content source is missing the host name. Specify a
// valid URL.
//
...
// MessageText:
//
// The path specification for the content source cannot contain wildcard
// characters, such as [?*]. Remove all wildcards from the path specification.
//
...
// MessageText:
//
// The specified gatherer application cannot be found. Use the name of an
// existing application, or reinstall the application.
//
...
// MessageText:
//
// The application plug-in manager cannot be removed, because one or more
// content indexes was configured to use the plug-in.  Remove all indexes, and
// then remove the plug-in manager.
//
...
// MessageText:
//
// The application name is invalid, because it contains special characters.
...
// MessageText:
//
// The data size returned by the filter is greater than the allocated buffer.
// Download and install an update for the filter.
//
...
// MessageText:
//
// The object cannot be created on a non-fixed drive. Create the object on
// another drive.
//
...
// MessageText:
//
// The content was not likely to be modified during the adaptive update, so the
// gatherer is not going to check the document for changes.
//
...
// MessageText:
//
// The update will start as soon as all content sources are released by other
// updates that are already in progress.
//
...
// MessageText:
//
// The transaction was ignored as redundant, because another transaction for the
// same URL was active at the same time.
//
...
// MessageText:
//
// The gatherer started getting properties using the filter from the protocol
// handler.
//
...
// MessageText:
//
// The system attempted to load a filter marked as apartment in a multi-threaded
// filter daemon. The document will be retried in a single-threaded filter.
// Since multithreaded filtering is more efficient, try to obtain the version of
// the filter that is multi-threaded.
//
...
// MessageText:
//
// The identified content was not crawled for this address. Recreate the content
// index.
//
...
// MessageText:
//
// The gatherer cannot handle the alert stream. All alert sources will be
// crawled again.
//
...
// MessageText:
//
// A duplicate of this document was detected, and the document will be retried.
// If this problem persists, recreate the content index.
//
...
// MessageText:
//
// The administrative data from the Exchange store or registry cannot be read.
// Check that the Exchange store is started, and that it was properly restored.
//
...
// MessageText:
//
// Read access to the Exchange store is temporarily unavailable. If this message
// persists, restart the Exchange store.
//
...
// MessageText:
//
// The function is unavailable, because it was called while the word breaker was
// attempting to fill the text buffer. Download and install an update for the
// word breaker.
//
...
// MessageText:
//
// The property list for the content class of this document cannot be loaded,
// and the schema cannot be accessed. Check that the schema is correctly
// configured.
//
...
// MessageText:
//
// Some parts of the document were modified, and others were not. The gatherer
// will refilter the entire document.
//
...
// MessageText:
//
// Crawling cannot continue because the local server is not responding. Restart
...
// MessageText:
//
// Schema/schema cache error (%1) occurred. Schema collection ref: %2, content
// class: %3. This error is internal, for debugging.
//
...
// MessageText:
//
// Document filtering cannot be completed because the document server did not
// respond within the specified timeout. Try crawling the server later, or
...
// MessageText:
//
// Alerts for this content source cannot be established because information
// about the file share cannot be obtained.
//
...
// MessageText:
//
// Alerts for a content source with a local address are only supported for a
// fixed media drive type. Use the UNC name for remote file shares.
//
...
// MessageText:
//
// The new content index cannot be added because there is not enough disk space.
// Free up at least 10 megabytes of free space, and then try again.
//
...
// MessageText:
//
// The document received an invalid document resource ID.  The document must be
// crawled again.
//
...
// MessageText:
//
// Hierarchical content sources such as those in the file system cannot be
// nested.
//
...
// MessageText:
//
// Backup and restore failed during an ESE database phase. For more detailed
// information, check the event log.
//
...
// MessageText:
//
// This document is a child of another document.  It will not be cataloged
// separately.
//
...
// MessageText:
//
// The plug-in is not properly registered on this computer.  Check that the
// application is properly installed, or contact Microsoft Product Support.
//
...
// MessageText:
//
// Alerts for this scope were interrupted so that authentication tokens can be
// updated.
//
...
// MessageText:
//
// File system alerts were not initialized successfully   If the problem
// persists after restarting the service, try rerunning Setup.
//
...
// MessageText:
//
// File system alerts for this scope are stopped because the scope is being
// removed.
//
...
// MessageText:
//
// Alerts failed because of network failures or because the internal cache size
// was exceeded.  If the problem persists, change the content source type to
// adaptive or crawl.
//
...
// MessageText:
//
// The operation could not be completed because the specified path is invalid.
// Try again using a valid path.
//
...
// MessageText:
//
// There were not enough keywords in the sample documents to train the Topic
// Assistant.  Try again when more sample documents are available.
//
...
// MessageText:
//
// The Topic Assistant requires sample documents from at least two categories.
// Try again when sample documents for more categories are available.
//
...
// MessageText:
//
// There were not enough sample documents to train the Topic Assistant.  Try
// again when more sample documents are available.
//
...
// MessageText:
//
// An error occurred using the topic assistant.  Check the event log for related
// failures.
//
...
// MessageText:
//
// The system attempted to load a filter marked as apartment from an embedded
// component in a multi-threaded filter daemon. The document will be retried in
// a single-threaded filter. Since multithreaded filtering is more efficient,
// try to obtain the version of the filter that is multi-threaded.
//
...
// MessageText:
//
// The document contains text that can not be broken into words. The document
// won't be indexed.
//
...
// MessageText:
//
// This is a success code that indicates that the MIME filter should be used to
// filter the document stream. This is an internal error code and should not be
// reported to users.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The data for this folder will not be crawled because it is configured to be
// crawled by (%1).
//
...
// MessageText:
//
// The security descriptor for this document only contained built-in access
// control entries. The built-in ACEs have to be removed because they won't be
// valid for this document during queries.
//
...
// MessageText:
//
// The context for this object is no longer valid.  Refresh the context for this
// object.
//
...
// MessageText:
//
// The checkpoint cannot be enabled, because the content index is not idle. Wait
// until the index is idle, and then try again.
//
...
// MessageText:
//
// The maximum number of search scopes was reached. Use one of the existing
// scopes.
//
...
// MessageText:
//
// Maximum number of properties per document has been reached. The document is
// most likely corrupt, or it represents a denial of service threat, the
// indexing of this document has been aborted.
//
...
// MessageText:
//
// The start address is invalid because it is a local path.  Please enter a
// valid start address.
//
...
// MessageText:
//
// The backup image cannot be verified because the image may be corrupted. To
// preserve your data, it is recommended that you run Data Backup and Restore
// again.
//
...
// MessageText:
//
// The filter host process cannot be started. The system is most likely low on
// resources, or the filter host process binary was modified. If the resources
// are available, check the search binaries with an antivirus program.
//
...
// MessageText:
//
// Some of the links were not successfully added to the queue. Parent
...
// MessageText:
//
// The tag is invalid. This error is obsolete and should no longer be reported.
...
// MessageText:
//
// The propagation configuration file cannot be read.  Check that the same
// versions of search are used on both the content index and search servers.  If
// the problem persists, recrawl and repropagate the content index.
//
...
// MessageText:
//
// The number of search servers exceeded the limit.  Ensure that the number of
// propagated content indexes is less than the documented maximum.  If so, start
// and restart the search service, and then try again.
//
...
// MessageText:
//
// The specified search server already exists. This error is obsolete and should
// no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// An update is already in progress. This error is obsolete and should no longer
// be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The index list file cannot be read. This error is obsolete and should no
// longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// A registry entry required for content index creation cannot be read or
// created. Delete and recreate the content index.  If the problem persists, you
// may need to reinstall search.
//
...
// MessageText:
//
// One or more files required for content index creation were not found. Stop
// and restart the search service.  If the problem persists, delete and recreate
// the content index.
//
...
// MessageText:
//
// The search server was not found. Stop and restart the search service.  If
// propagation is being used, stop and restart the search service on the remote
// computers.
//
...
// MessageText:
//
// A word breaker was not found for the given language. Check your current
// language settings and ensure that search supports the current language.  If
// the problem persists, reinstall search.
//
...
// MessageText:
//
// A noise word list was not found for the given language. This error is
// obsolete and should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// A stemmer was not found for the given language. This error is obsolete and
// should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// Index propagation was stopped.  This error is obsolete and should no longer
// be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The disk is full. This error is obsolete and should no longer be reported.
// Call Microsoft Product Support.
//
...
// MessageText:
//
// The index data is invalid. This error is obsolete and should no longer be
// reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The index data is corrupt. This error is obsolete and should no longer be
// reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The property store cannot be initialized. This error is obsolete and should
// no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The propagation state information cannot be read. This error is obsolete and
// should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// There is no update in progress.  This error is obsolete and should no longer
// be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The search server does not exist.  This error is obsolete and should no
// longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The content index is not loaded. Stop and restart the search service.  If the
// problem persists, review other errors in the event log. If necessary, delete
...
// MessageText:
//
// There is a mismatch in the major version number. Check that the same versions
// of search are used on both the content index and search servers.  If
// necessary, crawl the index again and repropagate the index.
//
...
// MessageText:
//
// The index is a mismatch in the minor version number. This error is obsolete
// and should no longer be reported.  Call Microsoft Product Support.
//
...
// MessageText:
//
// A connection with the Search service on a remote machine could not be
// established. Check the connectivity with the remote machine.  If necessary,
// stop and restart the Search service on the remote machine.
//
...
// MessageText:
//
// Crawling of documents was stopped. This is an internal error code and should
// not be reported to users.  Call Microsoft Product Support.
//
...
// MessageText:
//
// The index was dismounted because it is being removed or the search service
// stopped.  If the problem persists, restart the search service.
//
...
// MessageText:
//
// The search server was added successfully, but cannot be contacted.  Check
// that the search service is executing on the remote computer, and that you can
...
// MessageText:
//
// No index level information exists for this index. Use application level
// tables for this index.
//
...
// MessageText:
//
// Information for this content class was not found. Use the default content
// class.  This error is obsolete and should no longer be reported.  Call
...
// necessary to delete and recreate the content index.
//
...
// MessageText:
//
// The content index server detected a previously crawled document with the same
// content. This is an internal error code and should not be reported to users.
// Call Microsoft Product Support.
//
...
// MessageText:
//
// The content index server cannot find a description of the content index in
// its database. Search will automatically attempt to recreate the content index
// description.  If this problem persists, stop and restart the search service
// and, if necessary, delete  and recreate the content index.
//
...
// MessageText:
//
// The content index server cannot find needed information in its database.
// Reset and recrawl the content index.
//
...
// MessageText:
//
// The content index server cannot update or access its database, because the
// version store has insufficient resources.  Increase the system resource usage
// setting for the search service.  If the problem persists, stop and restart
// the search service.
//
...
// MessageText:
//
// The content index server cannot update or access its database because
// sessions are unavailable. Increase the system resource usage setting for the
// search service.  If the problem persists, stop and restart the search
// service.
//
...
//
// The content index server cannot update or access its database because
// insufficient system resources are available. Increase the system resource
// usage setting for the search service.  If the problem persists, stop and
...
// MessageText:
//
// The content index server cannot update an unrecognized or recently defined
// property attribute. Reset and recrawl the content index.
//
...
// MessageText:
//
// The property store was unable to open, close, or delete a data file used in
// propagation.  Check that your index volume has enough disk space.
//
...
// MessageText:
//
// The property store encountered propagation data files that cannot be used.
...
// MessageText:
//
// There is a mismatch in the property store version numbers. Ensure that the
// same versions of search are used on both the content index and search
// servers.  If necessary, recrawl the content index and try to propagate the
// content index again.
//
...
//
// The content index server cannot update or access information because of a
// sharing violation.  Stop and restart the search service.  If the problem
// persists, reset and recrawl the content index.  In some cases it may be
// necessary to delete and recreate the content index.
//
...
// MessageText:
//
// The configuration properties for Microsoft Exchange 5.5 were not set. Specify
// these properties, and then try again.
//
...
// MessageText:
//
// The name of the Microsoft Exchange 5.5 server specified in the content source
// does not match the Exchange server specified in the Microsoft Exchange 5.5
// configuration properties. Change the name of the Exchange server in the
// content source or configuration properties to match, and then try again.
//
...
// MessageText:
//
// There is insufficient privilege to access the Microsoft Exchange 5.5 server.
// Check that the Microsoft Exchange 5.5 configuration is valid and that the
// search service is running in the context of a user having administrative
// privileges on the Exchange server configuration node.
//
...
// MessageText:
//
// The Microsoft Exchange 5.5 server name specified in the address does not
// match the one specified in the Microsoft Exchange 5.5 configuration.
//
...
//
// There is insufficient privilege to access the Microsoft Exchange 5.5 Server.
// Verify that the Microsoft Exchange 5.5 configuration is valid and that the
// Search Service is running in the context of a user having administrative
// privileges on the Exchange Server configuration node.
//
...
// MessageText:
//
// The service cannot access the Exchange 5.5 server. The configured Microsoft
// Exchange 5.5 server name may be invalid, the Exchange 5.5 server may be
// temporarily unavailable, or the account used by the search service may not
// have administrative access to the Exchange 5.5 server. Check all of these
...
// MessageText:
//
// The service cannot access the Exchange 5.5 service. The account used by the
// search service may not be an Exchange administrator, or the site or
// organization settings on the search host may be invalid.
//
...
// MessageText:
//
// Outlook is not installed on this machine, or is missing the CDO or MSEMS
...
// MessageText:
//
// The account information specified for accessing the Exchange 5.5 server
// cannot be used. Verify the account and password information.
//
#define EXCI_E_INVALID_ACCOUNT_INFO 0x800411A8L

// The following PRTH errors are here because we don't want to expose them to
// Protocol handler developers. Those we can afford to expose are in prtherr.mc.
//
...
// I beleive we don't expose setting the maximum download limit in the UI. But
// is it a good idea to put the limit in the error message? It is 16Mb unless
// the setting in the registry is changed.
//
//  MessageId: PRTH_S_MAX_GROWTH
//
//  MessageText:
//
//  Filtering of this file ended because the file reached the maximum filter
//  output limit. Check that the filter does not generate a large amount of data
...
// MessageText:
//
// The address cannot be accessed. Check that the address can be accessed from
// Internet Explorer.
//
...
// MessageText:
//
// The file reached the maximum download limit. Check that the full text of the
...
// MessageText:
//
// The URL was excluded because its content type (multipart/x-mixed-replace) is
// not supported.
//
...
// MessageText:
//
// Search was unable to convert the Access Control List of the crawled store
// into a Windows NT Access Control List. Check that local information is not
// used in the store.
//
...
// MessageText:
//
// An object cannot be processed because search cannot convert a Notes
// Access-Denied entry in the Access Control List into a Windows NT identity.
// Check the Notes to Windows NT identity mapping.
//
...
// MessageText:
//
// The item cannot be processed further because search failed to find one of its
// properties. Check that the item is valid in the store.
//
...
// MessageText:
//
// Filtering was interrupted because of a sharing conflict with another process.
// Crawl the file again when it is not in use.
//
...
// MessageText:
//
// Accessing this site requires a client certificate.  Specify a client
// certificate in the site path rules.
//
...
// MessageText:
//
// The item should be accessed while impersonating a user. Protocol handler is
// expected to implement IUrlAccessor3.
//
...
// uebug Is this an internal warning only?  If not, then why the reference to a
// function call?
//
...
// MessageText:
//
// At least one content index definition was not initialized.  Check the amount
// of free disk space, and verify that the registry configuration is correct.
//
...
// MessageText:
//
// The account cannot be granted a privilege to "log on as service"" on the
// domain controller.  The domain security administrator needs to grant the
// default account this privilege.  All of the default accounts specified for
// the server need this account privilege.
//
...
// MessageText:
//
// The selected content index definition did not initialize and cannot be used.
// Check server status, content index configuration, and user access to the
// server.
//
...
// uebug Is this jargon (chunk, IGatherScriptFilterSink) something users will
// understand?
//
...
//  MessageText:
//
//  The chunk does not allow changes. To set properties on a chunk, create it
//  using IGatherScriptFilterSink::CreateChunk
//
...
// mapi errors
// uebug What is not supported?  Also, for all MAPI bugs, we should capitalize
// MAPI, and consider removing it from the beginning of the message and
// inserting into the text instead.
//
...
// uebug Avoid "bad".  Where is the character width "bad"?  Avoid prefacing bug
// with "MAPI:".
//
...
// uebug Is the caller MAPI, or the parameters?  In other words, is this
// correct?: "The MAPI caller passed an invalid parameter or flag."
//
...
// MessageText:
//
// The version of OLE installed on the workstation is not compatible with this
// version of MAPI.
//
...
// MessageText:
//
// The calling process is an NT service, and registry keys required by MAPI
// could not be initialized.
//
#define _MAPI_E_TOO_COMPLEX 0x80040117L

// uebug Avoid "bad".  Rewrite so MAPI is part of the text, and not preceding it
// with a colon.
//
...
// uebug What is an extended error?  It is vague terminology, which we should
// avoid, if possible.
//
...
// uebug Is "entry ID" capitalized? If not, previous references should be made
// lowercase. If so, this should be capitalized.
//
...
// MessageText:
//
// An unexpected error occurred in the filtering process. Contact Microsoft
// Product Support.
//
...
// MessageText:
//
// The document IFilter cannot provide the needed interface. The IFilter may
// contain errors.
//
...
// MessageText:
//
// A document IFilter cannot be initialized. The document or IFilter may contain
// errors.
//
...
// MessageText:
//
// The password for the content access account cannot be decrypted because it
// was stored with different credentials.  Re-type the password for the account
// used to crawl this content.
//
...
// MessageText:
//
// An unexpected error occurred in the Notes protocol handler while processing
// the URL.
//
...
// MessageText:
//
// The Notes data cannot be accessed. Check that the server is properly
// configured for accessing Notes data.
//
...
// MessageText:
//
// Access to %1 is denied to user %1. To allow access, contact your Notes
// administrator.
//
...
// MessageText:
//
// The user name mapping cannot be read. Check that the database, view, and
// column names are correct, that the Notes database is accessible, and that the
// sort option is enabled. For more information, see the Administrator's Guide.
//
...
// MessageText:
//
// One or more Lotus Notes database forms cannot be read. Some fields might not
// be available.
//
...
// MessageText:
//
// An unexpected error occurred in the exstore protocol handler. Contact
// Microsoft Product Support.
//
...
// MessageText:
//
// Cannot find or access the client certificate specified for crawling this
// site.
//
...
// MessageText:
//
// Parallel vectors for property mapping information on the content source have
...
// MessageText:
//
// An unknown data type was found when reading content class information from
// the content source.
//
...
// MessageText:
//
// This is an internal error code. A method was called when it should not be.
// Call Microsoft Product Support.
//
...
// MessageText:
//
// A security update was attempted on an address that was redirected to a
// different server.
//
...
// MessageText:
//
// A duplicate property was found. Call Microsoft support if this error is
// encountered.
//
...
// MessageText:
//
// Data source mapping cannot be retrieved. Check to see that the mapping entry
// is valid.
//
...
// MessageText:
//
// A zero-length cookie was retrieved at the end of a Dirsync search. The
// account may not have the appropriate rights.
//
...
// MessageText:
//
// The user account name can not be resolved within the Active Directory. You
// would need to verify whether the user is a real user or just a temporary user
// object generated for some application purpose. You might be able to refine
// the LDAP search filter to avoid this error.
//
...
// MessageText:
//
// Error (%1) occurred, which may have resulted from the unavailability of
// directory service server (%2). If the auto discovery option is turned on,
// import will re-try it with the newly discovered domain controller. Otherwise,
// verify your import settings and make sure the server is still available.
//
...
// MessageText:
//
// The attempt to discover domain controller for domain (%1) was failed with
// error (%2). Please make sure whether there is problem in network connectivity
// or the domain is renamed.
//
...
// not imported. Check the user name and password of the access account
// specified on the Configure Profile Import page. If incremental import is
// enabled and you are importing from a Windows 2000 domain, check that the
// access account has the Replicate Changes permission for Active Directory
// directory services.
//
...
// MessageText:
//
// You do not have sufficient permissions for incremental import, or the DirSync
// cookie stored in the user profile database is corrupt. If you are importing
// from a Windows 2000 domain, check that the import account has the Replicate
// Changes permission for Active Directory directory services. If the import
// account has the permission, start a full import to refresh the cookie.
//
...
// MessageText:
//
// The named pipe used to communicate with the filter daemon has not been
// connected.
//
...
// MessageText:
//
// The system attempted to load an apartment threading model filter marked in a
// multi-threaded filter daemon. The document will be retried in a
// single-threaded filter daemon process. Since multithreaded filtering is more
// efficient, try to obtain the version of the filter that is multi-threaded.
//
...
//
// The value cannot be set, because the object was already deleted or was not
// initialized properly. Make sure the object reference is still valid, increase
// the registry size, or recreate the catalog configuration.
//
...
// MessageText:
//
// An internal interface is being used after the corresponding catalog has been
// shutdown. The operation will be aborted.
//
...
// MessageText:
//
// An internal interface is being used prior to being initialized. The operation
// will be aborted.
//
...
// MessageText:
//
// Data transferred between the MSFTESQL service and a filter daemon process is
// corrupted. This is an internal error.
//
...
// MessageText:
//
// This is an internal error: Document IDs should be greater than 0 and less
// than or equal to 0x7fffffff.
//
...
// MessageText:
//
// A status change is occurring or the project is in a force paused state, so
// MSFTESQL cannot accept input at this time.
//
...
// MessageText:
//
// The MSFTEFD process was launched but did not connect with the MSFTESQL
// service.
//
...
// MessageText:
//
// This is an internal error: Initialization of the datasink is incorrect.  At
// least one protocol handler PROGID is required.
//
...
// MessageText:
//
// This is an internal error:  A static thread has gotten invalid arguments and
// will force batches to be aborted and retried.
//
...
// MessageText:
//
// A catalog already exists with this name, so another can be created or
// mounted.
//
...
// MessageText:
//
// The Full Text Engine's input queue is getting full.  This batch has been
// accepted for processing.  This success code is intended to help pause input
// until the queue is less full.
//
...
// MessageText:
//
// A file path exceeds the maximum limit for paths in Windows, so it can't be
// used.
//
...
// MessageText:
//
// Signature validation cannot be performed on modules loaded by COM, so the
// object will not be created. The object is likely a filter, wordbreaker,
// stemmer, or protocol handler.
//
...
// MessageText:
//
// This is an internal error that should be handled. The FD has been killed and
// this chunk buffer has already been reassigned.
//
...
// MessageText:
//
// The maximum number of plug-ins has been exceeded, so a new plug-in can't be
// loaded.
//
...
// MessageText:
//
// The Full Text Engine's input queue is full.  This batch has been accepted for
// processing; however, the Full Text Engine will soon go into a forced paused
// state until the queue is less full.
//
...
// MessageText:
//
// An object could not be inserted because it was a duplicate of an existing
// object. The object may be a catalog or other named entity.
//
...
// MessageText:
//
// This transaction was superseded by a subsequent transaction, so it will not
// be completed.
//
...
// MessageText:
//
// This is an internal error: This request cannot be completed because the
// Filter Daemon has been shutdown.
//
...
// MessageText:
//
// There are no plug-in components in the indexing pipeline, so the data
// collected will not be used.
//
...
// MessageText:
//
// A status change is active on another thread. Since only one status change is
// allowed at a time this request can't be handled.
//
...
// MessageText:
//
// The filter daemon process MSFTEFD timed out for an unknown reason. This may
// indicate a bug in a filter, wordbreaker, or protocol handler.
//
...
// MessageText:
//
// This is an internal error: This activity is no longer valid because the
...
// MessageText:
//
// The plug-in specified is not known likely because it isn't loaded, so the
// operation can't succeed. Only specify plug-ins that are loaded.
//
...
// MessageText:
//
// The filter daemon process MSFTEFD was not able to create an accessor object
// for the batch.
//
...
// MessageText:
//
// The property type specified is incorrect for this property. Please see the
// product documentation for the correct data type for this property.
//
...
// MessageText:
//
// The value specified is out of range. Please see the product documentation for
// the valid range.
//
...
// MessageText:
//
// The upgrade interface has already been instantiated. Only one instance of the
// upgrade interface is allowed.
//
...
// MessageText:
//
// The stack for a thread in this process is corrupted due to a programming bug.
// This may be a security threat and indicate your system has been attacked. The
// process will be shutdown.
//
...
// MessageText:
//
// The catalog is opened for read-only operations. Writing is prohibited to this
// catalog.
//
...
// MessageText:
//
// The performance monitor counters cannot be unloaded because they are not
...
// MessageText:
//
// This huge document will be retried in dedicated filter daemon MSFTEFD
// process.
//
...
// MessageText:
//
// This is an internal error: The filter daemon MSFTEFD type is unknown or
// invalid.
//
...
// MessageText:
//
// This is an internal error: Datatype misalignment was detected likely due to a
// programming error.
//
...
// MessageText:
//
// The filter daemon process MSFTEFD used too much memory and will be
// terminated.
//
...
// MessageText:
//
// msftesql should reprocess this document in an isolated fashion to confirm the
// error.
//
...
// MessageText:
//
// A failure occurred when tracking the completion of wordlists which will cause
// current activity to be aborted.
//
...
// MessageText:
//
// Internal Success Code.  Crawl is done.  Chunk buffers can be flushed; there
// is no additional input data.
//
...
// MessageText:
//
// The property requested from the catalog blob received doesn't match the
// current value of the catalog property. The property value used by the catalog
// was successfully returned. The caller should either call SetProperty with the
// value returned or dismount the catalog and mount it back with the blob.
//
...
// MessageText:
//
// Property Store is not found in the indexer. The possible causes are that the
// call came during a catalog reset or that the property store plug-in failed to
// load during indexer initialization.
//
...
// MessageText:
//
// There is not enough available physical or virtual memory for chunk buffers.
// Chunk buffers are needed to index data. Please free up memory.
//
...
// MessageText:
//
// There is not enough occurrence buffer memory available. Possible causes
// include too many processors, too many filter threads, or the occurrence
// buffer is not being returned.
//
...
// MessageText:
//
// The in-memory wordlist is corrupted. This is due to a faulty filter,
// wordbreaker, or other indexing component.
//
...
// MessageText:
//
// The IPersistStream and IPersistFile interfaces were unavailable from an
// IFilter filter DLL to load data for indexing.
//
...
// MessageText:
//
// The filter daemon MSFTEFD failed to load an IFilter interface for document,
...
// MessageText:
//
// The document being indexed timed out for an unknown reason. This may be due
// to a bug in a filter or wordbreaker.
//
...
// MessageText:
//
// The system failed to load noise words stream into text filter because text
// filter does not support IPersistStream.
//
...
// MessageText:
//
// The system failed to load noise words stream into text filter because the
// IPersistStream::Load() function failed.
//
...
// MessageText:
//
// The system failed to load noise words stream into text filter because
// IFilter::Init() function failed.
//
...
// MessageText:
//
// The options used are not supported. Check that your version of Internet
...
// MessageText:
//
// Access is denied. Check that the Default Content Access Account in Windows
// Search Central Administration is correct, or follow the "Exclude and Include
// Content" link to add a rule to specify the proper crawling account to access
// this URL.
//
...
// MessageText:
//
// You have been redirected to an address that does not exist. Check that the
// source address is redirecting to an accessible address.
//
...
// MessageText:
//
// The item will not be crawled because the Access Control List allows no one to
// read the item. Check that this is intended.
//
...
// MessageText:
//
// Search cannot crawl the item, because its Access Control List exceeded 64 KB.
// Check that the item has a valid Access Control List.
//
...
// MessageText:
//
// The relevant specified documents were not found, because a clause of the
// query contained only words that are ignored.
//
...
// MessageText:
//
// No information was found in the relevant documents with the specified
// properties, because a clause of the query contained only words that are
// ignored.
//
...
// MessageText:
//
// No relevant information is found in the relevant documents with the specified
...
// MessageText:
//
// The server is running an older version of software that cannot handle this
// query.
//
...
// MessageText:
//
// Coalesce is either not used properly or this form is not currently supported.
// Check for possible mixing of grouping with coalesce
//
...
// MessageText:
//
// Queries are disabled because the index is being updated. Try your query again
// in a few minutes.
//
...
// MessageText:
//
// The top level group does not support ORDER IN GROUP since there is no parent
// group.
//
...
//
// Codes 0x1650-0x167f are reserved for QUERYLIB (see
// querylib\include\qutilerr.mc)
//
//
...
// MessageText:
//
// The content index cannot be read. A content scan will be scheduled after
// chkdsk or autochk is run.
//
...
// MessageText:
//
// The document was too large to filter completely.  Portions of the document
// were not emitted.
//
...
// MessageText:
//
// The index does not exist or is currently unavailable. Try again later. If
// this problem persists, contact the system administrator.
//
...
// MessageText:
//
// Page size is not an integral multiple of the sector size of the volume where
...
// MessageText:
//
// Administrative connections from client were not associated with a document
// store.
//
...
// MessageText:
//
// The system only stores index entries in a protected catalog, but no protected
...
// MessageText:
//
// Content indexer only currently supports having a single protected user per
// device, but multiple protected users were present.
//
...
// MessageText:
//
// The system detected a SID owner for a protected item that did not match the
// SID owner of the protected catalog for the item.
//
...
// MessageText:
//
// On a system with a protected catalog, indexer only supports queries from the
...
// MessageText:
//
// Failed to encrypt an existing database file during first encryption upgrade
// for the device.
//
...
// The merge will be rescheduled later.  Please free some disk space for
// indexing to continue.
//
...
//
// %1Catalog: %2.
// A master merge was started because the expected number of documents in the
// catalog (%3) were indexed.
//
#define MSG_CI_MASTER_MERGE_REASON_EXPECTED_DOCS ((HRESULT)0x40001046L)

//
// This message is provided for future master merge reasons so that if some
// other reason appears and the change is made in a hotfix/service pack no
...
// MessageText:
//
// %1Catalog: %2. The master merge was started because of internal reason number
// %3.
//
...
// MessageText:
//
// %1 Unable to create the query engine's first request item due to error %2.
// It's possible that the MSFTESQL service account is invalid or the password
// has expired.
//
...
// MessageText:
//
// The property name cannot be set because it is already being used in a VIEW.
// SQLSTATE=42000
//
...
// MessageText:
//
// View '%1' already exists in the index '%2' and cannot be redefined.
// SQLSTATE=42S01
//
