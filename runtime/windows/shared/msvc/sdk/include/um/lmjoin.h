...
#define NETSETUP_ACCT_CREATE                                                   \
  0x00000002 // Do the server side account creation/rename
#define NETSETUP_ACCT_DELETE                                                   \
  0x00000004 // Delete the account when a domain is left
#define NETSETUP_WIN9X_UPGRADE                                                 \
  0x00000010 // Invoked during upgrade of Windows 9x to
             // Windows NT
#define NETSETUP_DOMAIN_JOIN_IF_JOINED                                         \
  0x00000020 // Allow the client to join a new domain
...
#define NETSETUP_JOIN_DC_ACCOUNT                                               \
  0x00000200 // Allow join if existing account is a DC
#define NETSETUP_JOIN_WITH_NEW_NAME 0x00000400 // Check for computer name change
#define NETSETUP_JOIN_READONLY                                                 \
  0x00000800 // Perform join using a pre-created account w/o requiring a
             // writable DC
#define NETSETUP_DNS_NAME_CHANGES_ONLY                                         \
  0x00001000 // When performing machine rename only update DNS based names

#define NETSETUP_INSTALL_INVOCATION                                            \
  0x00040000 // The APIs were invoked during install

#define NETSETUP_AMBIGUOUS_DC                                                  \
  0x00001000 // When joining the domain don't try to set the
             // preferred DC in the registry.
#define NETSETUP_NO_NETLOGON_CACHE 0x00002000 // Don't create the netlogon cache
#define NETSETUP_DONT_CONTROL_SERVICES                                         \
  0x00004000 // Don't force netlogon to start
#define NETSETUP_SET_MACHINE_NAME                                              \
  0x00008000 // For offline join only, set target machine hostname and NB name.
#define NETSETUP_FORCE_SPN_SET                                                 \
  0x00010000 // Override other settings during domain join
             // and attempt to set the SPN.
#define NETSETUP_NO_ACCT_REUSE 0x00020000 // Do not reuse an existing account
#define NETSETUP_ALT_SAMACCOUNTNAME                                            \
  0x00020000 // do not use OEM code page to derive the netbios\samAccountName

#define NETSETUP_IGNORE_UNSUPPORTED_FLAGS                                      \
  0x10000000 // If this bit is set, unrecognized flags
...
// The following flags are used when the system is processing information left
// from a prior offline join.  We want to force DC discovery and we don't want
// to create the netlogon cache.  We also don't want to immediately force
// netlogon to start, let it start on its own.
...
NET_API_STATUS
NET_API_FUNCTION
NetJoinDomain(_In_opt_ LPCWSTR lpServer, _In_ LPCWSTR lpDomain,
              _In_opt_ LPCWSTR lpMachineAccountOU, _In_opt_ LPCWSTR lpAccount,
              _In_opt_ LPCWSTR lpPassword, _In_ DWORD fJoinOptions);

NET_API_STATUS
NET_API_FUNCTION
NetUnjoinDomain(_In_opt_ LPCWSTR lpServer, _In_opt_ LPCWSTR lpAccount,
                _In_opt_ LPCWSTR lpPassword, _In_ DWORD fUnjoinOptions);

NET_API_STATUS
NET_API_FUNCTION
NetRenameMachineInDomain(_In_opt_ LPCWSTR lpServer,
...
NET_API_STATUS
NET_API_FUNCTION
NetValidateName(_In_opt_ LPCWSTR lpServer, _In_ LPCWSTR lpName,
                _In_opt_ LPCWSTR lpAccount, _In_opt_ LPCWSTR lpPassword,
...
NET_API_STATUS
NET_API_FUNCTION
NetGetJoinableOUs(_In_ LPCWSTR lpServer, _In_ LPCWSTR lpDomain,
                  _In_opt_ LPCWSTR lpAccount, _In_opt_ LPCWSTR lpPassword,
                  _Out_ DWORD *OUCount,
...
NET_API_STATUS
NET_API_FUNCTION
NetAddAlternateComputerName(_In_opt_ LPCWSTR Server, _In_ LPCWSTR AlternateName,
...
NET_API_STATUS
NET_API_FUNCTION
NetRemoveAlternateComputerName(_In_opt_ LPCWSTR Server,
...
NET_API_STATUS
NET_API_FUNCTION
NetSetPrimaryComputerName(_In_opt_ LPCWSTR Server, _In_ LPCWSTR PrimaryName,
                          _In_opt_ LPCWSTR DomainAccount,
                          _In_opt_ LPCWSTR DomainAccountPassword,
                          _In_ ULONG Reserved);
...
NET_API_STATUS
NET_API_FUNCTION
NetEnumerateComputerNames(_In_opt_ LPCWSTR Server,
                          _In_ NET_COMPUTER_NAME_TYPE NameType,
                          _In_ ULONG Reserved, _Out_ PDWORD EntryCount,
                          _Outptr_result_buffer_(*EntryCount)
...
// The caller requires account creation by privilege, this option will cause a
// retry on failure using down level account creation APIs.
//
#define NETSETUP_PROVISION_DOWNLEVEL_PRIV_SUPPORT 0x00000001

// If the named account already exists an attempt will be made to reuse.
// Requires sufficient credentials i.e. Domain Administrator or the object
// owner.
//
#define NETSETUP_PROVISION_REUSE_ACCOUNT 0x00000002

// Use the default machine account password which is the machine name in
// lowercase.
//
...
// When the pre-requisites are met, this option allows for must faster
...
NetProvisionComputerAccount(
    _In_z_ LPCWSTR lpDomain, _In_z_ LPCWSTR lpMachineName,
    _In_opt_z_ LPCWSTR lpMachineAccountOU, _In_opt_z_ LPCWSTR lpDcName,
    ...
    _Outptr_opt_result_maybenull_z_ LPWSTR *pProvisionTextData);

NET_API_STATUS
NET_API_FUNCTION
NetRequestOfflineDomainJoin(_In_reads_bytes_(cbProvisionBinDataSize)
                                BYTE *pProvisionBinData,
                            _In_ DWORD cbProvisionBinDataSize,
                            _In_ DWORD dwOptions, _In_z_ LPCWSTR lpWindowsPath);
...
NET_API_STATUS
NET_API_FUNCTION
NetRequestProvisioningPackageInstall(_In_reads_bytes_(dwPackageBinDataSize)
...
NetFreeAadJoinInformation(_In_opt_ PDSREG_JOIN_INFO pJoinInfo);
...
NET_API_STATUS
NET_API_FUNCTION
NetGetJoinInformation(_In_opt_ LPCWSTR lpServer, _Outptr_ LPWSTR *lpNameBuffer,
