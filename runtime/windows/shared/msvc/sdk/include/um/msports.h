...
Return Value:
  ...
  --*/
LONG SerialDisplayAdvancedSettings(_In_ HWND ParentHwnd,
...
typedef BOOL (*PPORT_ADVANCED_DIALOG)(_In_ HWND ParentHwnd,
...
LONG WINAPI ComDBOpen(_Out_ PHCOMDB PHComDB);
/*++
...
LONG WINAPI ComDBClose(_In_ HCOMDB HComDB);
/*++
...
LONG WINAPI ComDBGetCurrentPortUsage(_In_ HCOMDB HComDB,
...
Routine Description:
    ...
    if Buffer is NULL, than MaxPortsReported will contain the max number of
ports the DB will report (this value is NOT the number of bytes need for
Buffer). ReportType is ignored in this case.
    ...
    if ReportType == CDB_REPORT_BYTES
        returns a byte array indicating if a comX name is claimed.  Zero unused,
non zero used, ie, byte 0 is com1, byte 1 is com2, etc
...
Arguments:
    ...
    MaxPortsReported    Pointer to DWORD that holds the number of bytes in
buffer filled in
...
LONG WINAPI ComDBClaimNextFreePort(_In_ HCOMDB HComDB, _Out_ LPDWORD ComNumber);
/*++
...
Return Value:
    ...
    if successful, then ComNumber will be that next free com value and claims it
in the database


--*/

LONG WINAPI ComDBClaimPort(_In_ HCOMDB HComDB, _In_ DWORD ComNumber,
                           _In_ BOOL ForceClaim, _Out_opt_ PBOOL Forced);
/*++
...
Arguments:
    ...
    Force          - If TRUE, will force the port to be claimed even if in use
already
...
Return Value:
    ...
    returns ERROR_SUCCESS if port name was not already claimed, or if it was
claimed and Force was TRUE.
...
LONG WINAPI ComDBReleasePort(_In_ HCOMDB HComDB, _In_ DWORD ComNumber);
/*++
...
LONG WINAPI ComDBResizeDatabase(_In_ HCOMDB HComDB, _In_ DWORD NewSize);
/*++
