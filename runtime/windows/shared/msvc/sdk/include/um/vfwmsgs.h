...
// MessageText:
//
// Version number of DirectDraw not suitable. Make sure to install dx5 or higher
// version.%0
//
...
// MessageText:
//
//  The current display device does not support Content Output Protection
//  Protocol (COPP) H/W.%0
//
...
// MessageText:
//
//  Nonblocking APIs are enabled and the nav cannot perform the operation at
//  this time (IDVDControl) or partially incomplete information may be returned
//  (DVDInfo).%0
//
...
// MessageText:
//
// The DVD Graph Builder cannot build a DVD playback filter graph if the EVR and
// another renderer are in the graph builder's filter graph.
//
...
// MessageText:
//
// AM_DVD_EVR_ONLY cannot be passed to RenderDvdVideoVolume if the graph
// builder's filter graph contains a renderer which is not the EVR.
//
