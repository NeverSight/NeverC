...
DEFINE_GUID(CLSID_DsFindWriteableDomainController, 0x7cbef079, 0xaa84, 0x444b,
            0xbc, 0x70, 0x68, 0xe4, 0x12, 0x83, 0xea, 0xbc);
...
#define DSQPF_SAVELOCATION                                                     \
  0x00000002 // = 1 => pSaveLocation contains directory to save queries into
#define DSQPF_SHOWHIDDENOBJECTS                                                \
  0x00000004 // = 1 => show objects marked as "hidden" in results
#define DSQPF_ENABLEADMINFEATURES                                              \
  0x00000008 // = 1 => show admin verbs, property pages etc
#define DSQPF_ENABLEADVANCEDFEATURES                                           \
  0x00000010 // = 1 => set the advanced flag for the property pages
#define DSQPF_HASCREDENTIALS                                                   \
  0x00000020 // = 1 => pServer, pUserName & pPassword are valid
#define DSQPF_NOCHOOSECOLUMNS                                                  \
  0x00000040 // = 1 => remove choose columns from view

typedef struct {
  DWORD cbStruct;
  DWORD dwFlags;
  LPWSTR pDefaultScope; // -> Active Directory path to use as scope / == NULL
                        // for none
  LPWSTR pDefaultSaveLocation; // -> Directory to save queries into / == NULL
                               // default location
...
#define DSQPM_GETCLASSLIST                                                     \
  (CQPM_HANDLERSPECIFIC + 0) // wParam == flags, lParam = LPLPDSQUERYCLASSLIST
...
#define DSQPM_HELPTOPICS                                                       \
  (CQPM_HANDLERSPECIFIC + 1) // wParam = 0, lParam = hWnd parent
