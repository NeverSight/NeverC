...
/*
 * The interface confugration in invalid. There is already another interface
 * that is connected to the same inteface on the remote router.%0
 */
 ...
 * Remote Access Clients only.%0
 */

#define ERROR_CLIENT_INTERFACE_ALREADY_EXISTS (ROUTEBASE + 15)
/*
 * The client interface with this name already exists and is currently
 * connected.%0
 */
...
/*
 * The connection could not be established because the authentication protocol
 * used by the RAS/VPN server to verify your username and password could not be
 * matched with the settings in your connection profile.%0
 */
 ...
 * invalid on the domain.%0
 */

#define ERROR_INTERFACE_HAS_NO_DEVICES (ROUTEBASE + 25)
/*
 * There are no routing enabled ports available for use by this demand dial
 * interface.%0
 */
...
/*
 * The maximum limit on the number of LAN interfaces supported has been
 * reached.%0
 */
 ...
 * cannot accept your connection request.%0
 */
 ...
 * Make sure that the case sensitive secret pasword for the RADIUS server is set
 * correctly.%0
 */
...
/*
 * The connection could not be established because the authentication method
 * used by your connection profile is not permitted for use by an access policy
 * configured on the RAS/VPN server. Specifically, this could be due to
 * configuration differences between the authentication method selected on the
 * RAS/VPN server and the access policy configured for it.%0
 */
