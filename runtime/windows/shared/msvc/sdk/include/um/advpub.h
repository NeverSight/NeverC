...
//@[contract("advpub"), comment("MVI_tracked -
...
//      E_INVALIDARG                         NULL specified in szCmdName or
//      szDir HRESULT_FROM_WIN32(ERROR_OLD_WIN_VERSION) INF's not supported on
//      this OS version E_UNEXPECTED                         Catastrophic
//      failure(should never happen). HRESULT_FROM_WIN32(GetLastError())
//      Anything else
/////////////////////////////////////////////////////////////////////////////
...
//      pszSec  INF section.  NULL is translated as DefaultInstall or
//      DefaultInstall.NT. dwReserved Not used.
// RETURN:
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI RebootCheckOnInstallA(HWND hwnd, LPCSTR pszINF, LPCSTR pszSec,
                                     DWORD dwReserved);
HRESULT WINAPI RebootCheckOnInstallW(HWND hwnd, LPCWSTR pszINF, LPCWSTR pszSec,
                                     DWORD dwReserved);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI RebootCheckOnInstall(HWND hwnd, LPCSTR pszINF, LPCSTR pszSec,
                                    DWORD dwReserved);
#endif // (_WIN32_IE >= 0x0605)
...
//                                      translated string.  Required size is in
//                                      *pdwRequiredSize.
//      E_INVALIDARG                         NULL specified in pszInfFilename,
//      pszTranslateSection,
//                                      pszTranslateKey, pdwRequiredSize.
//      HRESULT_FROM_WIN32(ERROR_OLD_WIN_VERSION)
//                                      OS not supported.
//      E_UNEXPECTED                         Catastrophic failure -- should
//      never happen. HRESULT_FROM_WIN32(ERROR_INVALID_PARAMETER)
//                                      The section or key specified does not
//                                      exist.
...
HRESULT WINAPI TranslateInfStringA(
    _In_ LPCSTR pszInfFilename, _In_ LPCSTR pszInstallSection,
    _In_ LPCSTR pszTranslateSection, _In_ LPCSTR pszTranslateKey,
    _Out_writes_opt_(cchBuffer) LPSTR pszBuffer, _In_ DWORD cchBuffer,
    _Out_ PDWORD pdwRequiredSize, _Reserved_ PVOID pvReserved);
HRESULT WINAPI TranslateInfStringW(
    _In_ LPCWSTR pszInfFilename, _In_ LPCWSTR pszInstallSection,
    _In_ LPCWSTR pszTranslateSection, _In_ LPCWSTR pszTranslateKey,
    _Out_writes_opt_(cchBuffer) LPWSTR pszBuffer, _In_ DWORD cchBuffer,
    _Out_ PDWORD pdwRequiredSize, _Reserved_ PVOID pvReserved);
#ifdef UNICODE
...
HRESULT WINAPI RegInstallA(HMODULE hmod, LPCSTR pszSection,
                           const STRTABLEA *pstTable);
HRESULT WINAPI RegInstallW(HMODULE hmod, LPCWSTR pszSection,
...
HRESULT WINAPI RegInstall(HMODULE hmod, LPCSTR pszSection,
                          const STRTABLEA *pstTable);
#endif // (_WIN32_IE >= 0x0605)

//
// For people whos minds are too addled to use DELAYLOAD (or UNICODE for that
// matter)
//
#define REGINSTALL REGINSTALLA
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI LaunchINFSectionExA(_In_opt_ HWND hwnd,
                                   _In_opt_ HINSTANCE hInstance,
                                   _In_ LPSTR pszParms, _In_ INT nShow);
HRESULT WINAPI LaunchINFSectionExW(_In_opt_ HWND hwnd,
                                   _In_opt_ HINSTANCE hInstance,
                                   _In_ LPWSTR pszParms, _In_ INT nShow);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI LaunchINFSectionEx(_In_opt_ HWND hwnd,
                                  _In_opt_ HINSTANCE hInstance,
                                  _In_ LPSTR pszParms, _In_ INT nShow);
#endif // (_WIN32_IE >= 0x0605)

// FLAGS: - need to stay this way is for compatibility. Don't change them (but
// ok to add new ones).
//
...
//      #define COPYFLG_WARN_IF_SKIP            0x00000001   // warn if user
//      tries to skip file #define COPYFLG_NOSKIP                  0x00000002 //
//      disallow skipping this file #define COPYFLG_NOVERSIONCHECK 0x00000004 //
//      ignore versions and overwrite target #define COPYFLG_FORCE_FILE_IN_USE
//      0x00000008   // force file-in-use behavior #define COPYFLG_NO_OVERWRITE
//      0x00000010   // do not copy if file exists on target #define
//      COPYFLG_NO_VERSION_DIALOG       0x00000020   // do not copy if target is
//      newer #define COPYFLG_REPLACEONLY             0x00000400   // copy only
//      if file exists on target

#define AIF_WARNIFSKIP                                                         \
  0x00000001 // system critical file: warn if user tries to skip
#define AIF_NOSKIP 0x00000002 // Skip is disallowed for this file
#define AIF_NOVERSIONCHECK                                                     \
  0x00000004 // don't check the version number of the file overwrite
...
HRESULT WINAPI AdvInstallFileW(HWND hwnd, LPCWSTR lpszSourceDir,
                               LPCWSTR lpszSourceFile, LPCWSTR lpszDestDir,
                               LPCWSTR lpszDestFile, DWORD dwFlags,
                               DWORD dwReserved);
#ifdef UNICODE
...
#define IE4_BACKNEW                                                            \
  0x00000002 // backup all files which are not backed up before
#define IE4_NODELETENEW                                                        \
  0x00000004 // don't delete files we don't backed up before
...
#define IE4_NOENUMKEY                                                          \
  0x00000020 // this bit on: Don't Enum sub key even there is no given valuename
...
#define IE4_UPDREFCNT                                                          \
  0x00000200 // Update the ref count in .ini backup file list
#define IE4_USEREFCNT                                                          \
  0x00000400 // use ref count to determin if the backup file should be put back
#define IE4_EXTRAINCREFCNT                                                     \
  0x00000800 // if increase the ref cnt if it has been updated before

#define IE4_REMOVREGBKDATA                                                     \
  0x00001000 // This bit should be used with restore bit

/////////////////////////////////////////////////////////////////////////////
// ENTRY POINT: RegSaveRestore
//
// SYNOPSIS:    Save or Restore the given register value or given INF reg
// section.
//
// RETURNS:     E_FAIL on failure, S_OK on success.
/////////////////////////////////////////////////////////////////////////////
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI RegSaveRestoreA(HWND hWnd, LPCSTR pszTitleString,
                               HKEY hkBckupKey, LPCSTR pcszRootKey,
                               LPCSTR pcszSubKey, LPCSTR pcszValueName,
                               DWORD dwFlags);
HRESULT WINAPI RegSaveRestoreW(HWND hWnd, LPCWSTR pszTitleString,
                               HKEY hkBckupKey, LPCWSTR pcszRootKey,
                               LPCWSTR pcszSubKey, LPCWSTR pcszValueName,
                               DWORD dwFlags);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI RegSaveRestore(HWND hWnd, LPCSTR pszTitleString, HKEY hkBckupKey,
...
//
// SYNOPSIS:    Save or Restore the given INF Reg Section. At restore, if INF
// and Section pointers are NULL,
...
#endif // !UNICODE
#else
HRESULT WINAPI RegSaveRestoreOnINF(HWND hWnd, LPCSTR pszTitle, LPCSTR pszINF,
                                   LPCSTR pszSection, HKEY hHKLMBackKey,
                                   HKEY hHKCUBackKey, DWORD dwFlags);
#endif // (_WIN32_IE >= 0x0605)
...
// FLAGS:
#define ARSR_RESTORE                                                           \
  IE4_RESTORE // if this bit is off, means Save. Otherwise, restore.
#define ARSR_NOMESSAGES IE4_NOMESSAGES // Quiet no messages in any event.
#define ARSR_REGSECTION                                                        \
  IE4_REGSECTION // if this bit is off, the given section is GenInstall Section
#define ARSR_REMOVREGBKDATA                                                    \
  IE4_REMOVREGBKDATA // if both this bit and restore bit on, remove the backup
                     // reg data without restore it
...
#endif // !UNICODE
#else
HRESULT WINAPI RegRestoreAll(_In_opt_ HWND hWnd, _In_opt_ LPCSTR pszTitleString,
                             _In_ HKEY hkBckupKey);
#endif // (_WIN32_IE >= 0x0605)
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI FileSaveRestoreA(HWND hDlg, _In_opt_ LPSTR lpFileList,
                                LPCSTR lpDir, LPCSTR lpBaseName, DWORD dwFlags);
HRESULT WINAPI FileSaveRestoreW(HWND hDlg, _In_opt_ LPWSTR lpFileList,
                                LPCWSTR lpDir, LPCWSTR lpBaseName,
                                DWORD dwFlags);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI FileSaveRestore(HWND hDlg, _In_opt_ LPSTR lpFileList,
                               LPCSTR lpDir, LPCSTR lpBaseName, DWORD dwFlags);
#endif // (_WIN32_IE >= 0x0605)
...
HRESULT WINAPI FileSaveRestoreOnINFW(HWND hWnd, LPCWSTR pszTitle,
                                     LPCWSTR pszINF, LPCWSTR pszSection,
...
#endif // !UNICODE
#else
HRESULT WINAPI FileSaveRestoreOnINF(HWND hWnd, LPCSTR pszTitle, LPCSTR pszINF,
                                    LPCSTR pszSection, LPCSTR pszBackupDir,
                                    LPCSTR pszBaseBackupFile, DWORD dwFlags);
#endif // (_WIN32_IE >= 0x0605)
...
#define AFSR_BACKNEW                                                           \
  IE4_BACKNEW // backup all files which are not backed up before
#define AFSR_NODELETENEW                                                       \
  IE4_NODELETENEW // don't delete files we don't backed up before
#define AFSR_NOMESSAGES IE4_NOMESSAGES // No message display in any events.
#define AFSR_NOPROGRESS                                                        \
  IE4_NOPROGRESS                     // this bit on: No file backup progressbar
#define AFSR_UPDREFCNT IE4_UPDREFCNT // update the reference count for the files
#define AFSR_USEREFCNT                                                         \
  IE4_USEREFCNT // use the ref count to guide the restore file
#define AFSR_EXTRAINCREFCNT IE4_EXTRAINCREFCNT

/////////////////////////////////////////////////////////////////////////////
// ENTRY POINT: AddDelBackupEntry
//
// SYNOPSIS:    If AADBE_ADD_ENTRY is specified, mark the file in the File list
// as not existing
//              during file save in the INI file.  This can be used to mark
//              additional files that they did not exist during backup to avoid
//              having them backup the next time the FileSaveRestore is called
//              to save files. If AADBE_DEL_ENTRY is specified, delete the entry
//              from the INI.  This mechanism can be used to leave files
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI AddDelBackupEntryA(_In_opt_ LPCSTR lpcszFileList,
                                  _In_opt_ LPCSTR lpcszBackupDir,
                                  _In_opt_ LPCSTR lpcszBaseName, DWORD dwFlags);
HRESULT WINAPI AddDelBackupEntryW(_In_opt_ LPCWSTR lpcszFileList,
                                  _In_opt_ LPCWSTR lpcszBackupDir,
                                  _In_opt_ LPCWSTR lpcszBaseName,
                                  DWORD dwFlags);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI AddDelBackupEntry(_In_opt_ LPCSTR lpcszFileList,
                                 _In_opt_ LPCSTR lpcszBackupDir,
                                 _In_opt_ LPCSTR lpcszBaseName, DWORD dwFlags);
#endif // (_WIN32_IE >= 0x0605)
...
//
// SYNOPSIS:    Mark the file in the File list as not existing during file save
// in the INI file
//              This can be used to mark additional files that they did not
//              exist during backup to avoid having them backup the next time
//              the FileSaveRestore is called to save files
//
// RETURNS:
//      S_OK    success
//      E_FAIL  failure
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI FileSaveMarkNotExistA(_In_opt_ LPCSTR lpFileList,
                                     _In_opt_ LPCSTR lpDir,
                                     _In_opt_ LPCSTR lpBaseName);
HRESULT WINAPI FileSaveMarkNotExistW(_In_opt_ LPCWSTR lpFileList,
...
#endif // !UNICODE
#else
HRESULT WINAPI FileSaveMarkNotExist(_In_opt_ LPCSTR lpFileList,
                                    _In_opt_ LPCSTR lpDir,
                                    _In_opt_ LPCSTR lpBaseName);
#endif // (_WIN32_IE >= 0x0605)
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI GetVersionFromFileA(LPCSTR lpszFilename, _Out_ LPDWORD pdwMSVer,
                                   _Out_ LPDWORD pdwLSVer, BOOL bVersion);
HRESULT WINAPI GetVersionFromFileW(LPCWSTR lpszFilename, _Out_ LPDWORD pdwMSVer,
                                   _Out_ LPDWORD pdwLSVer, BOOL bVersion);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI GetVersionFromFile(LPCSTR lpszFilename, _Out_ LPDWORD pdwMSVer,
                                  _Out_ LPDWORD pdwLSVer, BOOL bVersion);
#endif // (_WIN32_IE >= 0x0605)
...
#endif // !UNICODE
#else
HRESULT WINAPI GetVersionFromFileEx(LPCSTR lpszFilename, _Out_ LPDWORD pdwMSVer,
                                    _Out_ LPDWORD pdwLSVer, BOOL bVersion);
#endif // (_WIN32_IE >= 0x0605)
...
#define ADN_DONT_DEL_SUBDIRS                                                   \
  0x00000002 // don't delete any sub-dirs; delete only the files
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI DelNodeRunDLL32A(HWND hwnd, HINSTANCE hInstance,
                                _Inout_ LPSTR pszParms, INT nShow);
HRESULT WINAPI DelNodeRunDLL32W(HWND hwnd, HINSTANCE hInstance,
                                _Inout_ LPWSTR pszParms, INT nShow);
#ifdef UNICODE
...
#endif // !UNICODE
#else
HRESULT WINAPI DelNodeRunDLL32(HWND hwnd, HINSTANCE hInstance,
                               _Inout_ LPSTR pszParms, INT nShow);
#endif // (_WIN32_IE >= 0x0605)
...
//
// SYNOPSIS:    Three APIs give the caller the option to be more efficient when
// need
...
#endif // !UNICODE
#else
HRESULT WINAPI OpenINFEngine(LPCSTR pszInfFilename, LPCSTR pszInstallSection,
                             DWORD dwFlags, HINF *phInf, PVOID pvReserved);
#endif // (_WIN32_IE >= 0x0605)
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI TranslateInfStringExA(
    HINF hInf, LPCSTR pszInfFilename, LPCSTR pszTranslateSection,
    LPCSTR pszTranslateKey,
    _Inout_updates_to_(dwBufferSize, *pdwRequiredSize) LPSTR pszBuffer,
    DWORD dwBufferSize, _Out_ PDWORD pdwRequiredSize,
    _Reserved_ PVOID pvReserved);
HRESULT WINAPI TranslateInfStringExW(
    HINF hInf, LPCWSTR pszInfFilename, LPCWSTR pszTranslateSection,
    LPCWSTR pszTranslateKey,
    _Inout_updates_to_(dwBufferSize, *pdwRequiredSize) LPWSTR pszBuffer,
    DWORD dwBufferSize, _Out_ PDWORD pdwRequiredSize,
    _Reserved_ PVOID pvReserved);
#ifdef UNICODE
...
#if (_WIN32_IE >= 0x0605)
HRESULT WINAPI ExtractFilesA(LPCSTR pszCabName, LPCSTR pszExpandDir,
                             DWORD dwFlags, LPCSTR pszFileList,
                             LPVOID lpReserved, DWORD dwReserved);
HRESULT WINAPI ExtractFilesW(LPCWSTR pszCabName, LPCWSTR pszExpandDir,
                             DWORD dwFlags, LPCWSTR pszFileList,
                             LPVOID lpReserved, DWORD dwReserved);
#ifdef UNICODE
...
#if (_WIN32_IE >= 0x0605)
INT WINAPI LaunchINFSectionA(HWND hwndOwner, HINSTANCE hInstance,
                             _Inout_ LPSTR pszParams, INT nShow);
INT WINAPI LaunchINFSectionW(HWND hwndOwner, HINSTANCE hInstance,
                             _Inout_ LPWSTR pszParams, INT nShow);
#ifdef UNICODE
...
// Flags in Advanced INF RunPreSetupCommands and RunPostSetupCommands of the
// Install section Those flags can tell advpack how to run those commands, quiet
// or not quiet, wait or not wait. The Default for runing those commands are:
// Not Quiet and Wait for finish before return the caller. I.E>
// RunPostSetupCommands = MyCmdsSecA:1, MyCmdsSecB:2, MyCmdsSecC
...
//              do some generic/intelligent function on behalf of every
//              component.
//
// RETURNS:     E_FAIL on failure, S_OK on success.
/////////////////////////////////////////////////////////////////////////////
...
//
// SYNOPSIS:    The function set the per-user stub reg values under
// IsInstalled\{GUID}
