...
#define WINHTTP_FLAG_SECURE_DEFAULTS                                           \
  0x30000000 // note that this flag also forces async
...
#define WINHTTP_FLAG_AUTOMATIC_CHUNKING                                        \
  0x00000200 // Send request without content-length header or chunked TE
...
#define SECURITY_FLAG_IGNORE_ALL_CERT_ERRORS                                   \
...
// see WinHttpGetProxyForUrlEx and WinHttpGetProxyResult, use
// WinHttpFreeProxyResult to free its members.
//
...
#define WINHTTP_MATCH_CONNECTION_GUID_FLAGS_MASK                               \
...
typedef struct _WINHTTP_RESOLVER_CACHE_CONFIG {
  ...
  //
  // ulMaxCacheEntryAge is the maximum allowed age of a cache entry specified in
  // minutes.
  //
  ...
  //
  // If WINHTTP_RESOLVER_CACHE_CONFIG_FLAG_CONN_USE_TTL is set, then
  // ullCOnnResolutionWaitTime can be used to control how frequently a
  // re-resolution attempt is made for any connection. It is specified in 100
  // nanosecond units, and the default is 600000000 (one minute).
  //
...
//
// Values for WINHTTP_OPTION_ENABLE_HTTP_PROTOCOL /
...
#define WINHTTP_AUTOLOGON_SECURITY_LEVEL_MAX                                   \
...
typedef VOID(CALLBACK *WINHTTP_STATUS_CALLBACK)(
    IN HINTERNET hInternet, IN DWORD_PTR dwContext, IN DWORD dwInternetStatus,
...
#define WINHTTP_CALLBACK_FLAG_GETPROXYSETTINGS_COMPLETE                        \
...
// WinHttpQueryHeaders(), then the response trailers will be queried, if they
// exist
//
...
//
// WINHTTP_QUERY_FLAG_WIRE_ENCODING - if this bit is set in the dwInfoLevel
// parameter of WinHttpQueryHeaders(), then the value of the header will be
// returned with as it gets encoded when sent over the wire.
//
...
#define ICU_ENCODE_PERCENT                                                     \
  0x00001000 // Encode any percent (ASCII25) signs encountered, default is to
             // not encode percent.
...
#define ICU_INCLUDE_DEFAULT_PORT                                               \
  0x00008000 // Include default port numbers in URLs
#define ICU_REJECT_USERPWD                                                     \
  0x00004000 // Rejects URLs whick have username/pwd sections
#define ICU_ESCAPE_AUTHORITY                                                   \
  0x00002000 // Escape chars in authority components (user, pwd, host)
...
//
// WINHTTP_EXTENDED_HEADER_FLAG_UNICODE - indicates the value of the request
// header is unicode.
//
...
BOOL WINAPI WinHttpTimeFromSystemTime(
    ...
    _Out_writes_bytes_(WINHTTP_TIME_FORMAT_BUFSIZE)
...
WINHTTPAPI
BOOL WINAPI WinHttpCrackUrl(_In_reads_(dwUrlLength) LPCWSTR pwszUrl,
                            _In_ DWORD dwUrlLength, _In_ DWORD dwFlags,
                            _Inout_ LPURL_COMPONENTS lpUrlComponents);

_Success_(return != FALSE) WINHTTPAPI BOOL WINAPI
    WinHttpCreateUrl(_In_ LPURL_COMPONENTS lpUrlComponents, _In_ DWORD dwFlags,
                     _Out_writes_to_opt_(*pdwUrlLength, *pdwUrlLength)
                         LPWSTR pwszUrl,
...
HINTERNET
WINAPI
WinHttpOpen(_In_opt_z_ LPCWSTR pszAgentW, _In_ DWORD dwAccessType,
...
HINTERNET
WINAPI
WinHttpConnect(IN HINTERNET hSession, IN LPCWSTR pswzServerName,
               IN INTERNET_PORT nServerPort, IN DWORD dwReserved);

WINHTTPAPI
BOOL WINAPI WinHttpReadData(IN HINTERNET hRequest,
                            _Out_writes_bytes_to_(dwNumberOfBytesToRead,
...
DWORD
WINAPI
WinHttpReadDataEx(IN HINTERNET hRequest,
                  _Out_writes_bytes_to_(dwNumberOfBytesToRead,
                                        *lpdwNumberOfBytesRead)
                      __out_data_source(NETWORK) LPVOID lpBuffer,
                  IN DWORD dwNumberOfBytesToRead,
                  OUT LPDWORD lpdwNumberOfBytesRead, IN ULONGLONG ullFlags,
                  IN DWORD cbProperty,
                  _In_reads_bytes_opt_(cbProperty) PVOID pvProperty);

WINHTTPAPI
BOOL WINAPI WinHttpWriteData(IN HINTERNET hRequest,
                             _In_reads_bytes_opt_(dwNumberOfBytesToWrite)
                                 LPCVOID lpBuffer,
                             IN DWORD dwNumberOfBytesToWrite,
                             OUT LPDWORD lpdwNumberOfBytesWritten);

WINHTTPAPI
BOOL WINAPI WinHttpQueryDataAvailable(IN HINTERNET hRequest,
                                      __out_data_source(NETWORK)
                                          LPDWORD lpdwNumberOfBytesAvailable);

_Success_(return != FALSE) WINHTTPAPI BOOL WINAPI
    WinHttpQueryOption(IN HINTERNET hInternet, IN DWORD dwOption,
...
BOOL WINAPI WinHttpSetOption(
    _In_opt_ HINTERNET hInternet, _In_ DWORD dwOption,
...
WinHttpSetTimeouts(IN HINTERNET hInternet, // Session/Request handle.
                   IN int nResolveTimeout, IN int nConnectTimeout,
                   IN int nSendTimeout, IN int nReceiveTimeout);
...
DWORD
WINAPI
WinHttpIsHostInProxyBypassList(_In_ const WINHTTP_PROXY_INFO *pProxyInfo,
...
HINTERNET
WINAPI
WinHttpOpenRequest(IN HINTERNET hConnect, IN LPCWSTR pwszVerb,
...
DWORD
WINAPI
WinHttpAddRequestHeadersEx(IN HINTERNET hRequest, IN DWORD dwModifiers,
                           IN ULONGLONG ullFlags, IN ULONGLONG ullExtra,
                           IN DWORD cHeaders,
                           _In_reads_(cHeaders)
...
BOOL WINAPI WinHttpSendRequest(
    IN HINTERNET hRequest, _In_reads_opt_(dwHeadersLength) LPCWSTR lpszHeaders,
    IN DWORD dwHeadersLength,
    _In_reads_bytes_opt_(dwOptionalLength) LPVOID lpOptional,
    IN DWORD dwOptionalLength, IN DWORD dwTotalLength, IN DWORD_PTR dwContext);
...
BOOL WINAPI WinHttpQueryAuthSchemes(
    ...
    OUT LPDWORD
        lpdwFirstScheme, // returns the first auth scheme returned by the server
...
WINHTTPAPI
BOOL WINAPI WinHttpQueryAuthParams(
    IN HINTERNET hRequest, // HINTERNET handle returned by WinHttpOpenRequest
    IN DWORD AuthScheme,
...
_Success_(return != FALSE) WINHTTPAPI BOOL WINAPI WinHttpQueryHeaders(
    IN HINTERNET hRequest, IN DWORD dwInfoLevel, IN LPCWSTR pwszName OPTIONAL,
...
WinHttpQueryHeadersEx(
    _In_ HINTERNET hRequest, _In_ DWORD dwInfoLevel, _In_ ULONGLONG ullFlags,
...
WinHttpQueryConnectionGroup(
    _In_ HINTERNET hInternet, _In_opt_ const GUID *pGuidConnection,
    _In_ ULONGLONG ullFlags,
    _Inout_ PWINHTTP_QUERY_CONNECTION_GROUP_RESULT *ppResult);

WINHTTPAPI
VOID WINAPI WinHttpFreeQueryConnectionGroupResult(
...
WINHTTPAPI
BOOL WINAPI
WinHttpGetProxyForUrl(IN HINTERNET hSession, IN LPCWSTR lpcwszUrl,
...
DWORD
WINAPI
WinHttpCreateProxyResolver(_In_ HINTERNET hSession,
                           _Out_ HINTERNET *phResolver);

WINHTTPAPI
DWORD
WINAPI
WinHttpGetProxyForUrlEx(_In_ HINTERNET hResolver, _In_ PCWSTR pcwszUrl,
...
DWORD
WINAPI
WinHttpGetProxyForUrlEx2(_In_ HINTERNET hResolver, _In_ PCWSTR pcwszUrl,
                         _In_ WINHTTP_AUTOPROXY_OPTIONS *pAutoProxyOptions,
                         _In_ DWORD cbInterfaceSelectionContext,
                         _In_reads_bytes_opt_(cbInterfaceSelectionContext)
...
DWORD
WINAPI
WinHttpGetProxyResult(_In_ HINTERNET hResolver,
                      _Out_ WINHTTP_PROXY_RESULT *pProxyResult);

WINHTTPAPI
DWORD
WINAPI
WinHttpGetProxyResultEx(_In_ HINTERNET hResolver,
...
WINHTTPAPI
VOID WINAPI
WinHttpFreeProxyResultEx(_Inout_ WINHTTP_PROXY_RESULT_EX *pProxyResultEx);

WINHTTPAPI
DWORD
WINAPI
WinHttpResetAutoProxy(_In_ HINTERNET hSession, _In_ DWORD dwFlags);
...
DWORD
WINAPI
WinHttpWriteProxySettings(_In_ HINTERNET hSession, _In_ BOOL fForceUpdate,
                          _In_ WINHTTP_PROXY_SETTINGS *pWinHttpProxySettings);

WINHTTPAPI
DWORD
WINAPI
WinHttpReadProxySettings(_In_ HINTERNET hSession,
...
WINHTTPAPI
VOID WINAPI
WinHttpFreeProxySettings(_In_ WINHTTP_PROXY_SETTINGS *pWinHttpProxySettings);

WINHTTPAPI
DWORD
WINAPI
WinHttpGetProxySettingsVersion(_In_ HINTERNET hSession,
                               _Out_ DWORD *pdwProxySettingsVersion);

WINHTTPAPI
DWORD
WINAPI
WinHttpSetProxySettingsPerUser(_In_ BOOL fProxySettingsPerUser);
...
HINTERNET
WINAPI
WinHttpWebSocketCompleteUpgrade(_In_ HINTERNET hRequest,
                                _In_opt_ DWORD_PTR pContext);

WINHTTPAPI
DWORD
WINAPI
WinHttpWebSocketSend(_In_ HINTERNET hWebSocket,
...
DWORD
WINAPI
WinHttpWebSocketReceive(_In_ HINTERNET hWebSocket,
                        _Out_writes_bytes_to_(dwBufferLength, *pdwBytesRead)
                            PVOID pvBuffer,
                        _In_ DWORD dwBufferLength,
...
DWORD
WINAPI
WinHttpWebSocketShutdown(_In_ HINTERNET hWebSocket, _In_ USHORT usStatus,
...
DWORD
WINAPI
WinHttpWebSocketClose(_In_ HINTERNET hWebSocket, _In_ USHORT usStatus,
                      _In_reads_bytes_opt_(dwReasonLength) PVOID pvReason,
...
HINTERNET
WINAPI
WinHttpProtocolCompleteUpgrade(_In_ HINTERNET hRequest,
                               _In_opt_ DWORD_PTR dwContext);

WINHTTPAPI
DWORD
WINAPI
WinHttpProtocolSend(_In_ HINTERNET ProtocolHandle, _In_ ULONGLONG Flags,
                    _In_reads_opt_(dwBufferLength) PVOID pvBuffer,
                    _In_ DWORD dwBufferLength);
...
DWORD
WINAPI
WinHttpProtocolReceive(_In_ HINTERNET ProtocolHandle, _In_ ULONGLONG Flags,
                       _Out_writes_bytes_to_(dwBufferLength, *pdwBytesRead)
                           PVOID pvBuffer,
                       _In_ DWORD dwBufferLength,
                       _Out_range_(0, dwBufferLength) DWORD *pdwBytesRead);
...
typedef VOID(CALLBACK *WINHTTP_PROXY_CHANGE_CALLBACK)(_In_ ULONGLONG ullFlags,
...
WinHttpRegisterProxyChangeNotification(
    _In_ ULONGLONG ullFlags, _In_ WINHTTP_PROXY_CHANGE_CALLBACK pfnCallback,
    _In_ PVOID pvContext,
...
DWORD
WINAPI
WinHttpGetProxySettingsEx(_In_ HINTERNET hResolver,
                          _In_ WINHTTP_PROXY_SETTINGS_TYPE ProxySettingsType,
                          _In_opt_ PWINHTTP_PROXY_SETTINGS_PARAM
                              pProxySettingsParam,
                          _In_opt_ DWORD_PTR pContext);
...
DWORD
WINAPI
WinHttpGetProxySettingsResultEx(_In_ HINTERNET hResolver,
                                _Out_ PVOID pProxySettingsEx);

WINHTTPAPI
DWORD
WINAPI
WinHttpFreeProxySettingsEx(_In_ WINHTTP_PROXY_SETTINGS_TYPE ProxySettingsType,
...
} WINHTTP_FAST_FORWARDING_STATE,
    *PWINHTTP_FAST_FORWARDING_STATE;
