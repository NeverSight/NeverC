...
#if _MSC_VER >= 1200
#pragma warning(push)
#pragma warning(disable : 4263) /* member function does not override any base  \
                                   class virtual member function */
#pragma warning(disable                                                        \
                : 4264) /* no override available for virtual member function   \
...
public:
  ...
  Status GetData(_Out_writes_bytes_to_(bufferSize, *sizeFilled) BYTE *buffer,
                 _In_ UINT bufferSize, _Out_opt_ UINT *sizeFilled = NULL) const;
...
public:
  ...
  Status GetFamilyName(_Out_writes_(LF_FACESIZE) LPWSTR name,
...
public:
  ...
  Status GetFamilies(_In_ INT numSought,
                     _Out_writes_to_(numSought, *numFound)
                         FontFamily *gpfamilies,
                     _Out_ INT *numFound) const;
...
public:
  ...
  Bitmap(_In_ INT width, _In_ INT height, _In_ INT stride,
  ...
  static Status ApplyEffect(IN Bitmap **inputs, IN INT numInputs,
                            IN Effect *effect,
                            IN RECT *ROI,         // optional parameter.
                            OUT RECT *outputRect, // optional parameter.
                            OUT Bitmap **output);

  Status GetHistogram(IN HistogramFormat format, IN UINT NumberOfEntries,
  ...
  static Status GetHistogramSize(IN HistogramFormat format,
...
public:
  ...
  static Status
  GetMetafileHeader(IN HMETAFILE hWmf,
                    IN const WmfPlaceableFileHeader *wmfPlaceableFileHeader,
  ...
  static UINT EmfToWmfBits(_In_ HENHMETAFILE hemf, _In_ UINT cbData16,
...
#if (GDIPVER >= 0x0110)
  Status ConvertToEmfPlus(IN const Graphics *refGraphics,
  ...
  Status ConvertToEmfPlus(IN const Graphics *refGraphics,
                          ...
                          IN EmfType emfType = EmfTypeEmfPlusOnly,
                          IN const WCHAR *description = NULL);
  Status ConvertToEmfPlus(IN const Graphics *refGraphics, IN IStream *stream,
                          IN OUT INT *conversionFailureFlag = NULL,
                          IN EmfType emfType = EmfTypeEmfPlusOnly,
                          IN const WCHAR *description = NULL);
#endif
