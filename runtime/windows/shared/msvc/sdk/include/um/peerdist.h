...
DWORD WINAPI PeerDistStartup(_In_ DWORD dwVersionRequested,
                             _Out_ PPEERDIST_INSTANCE_HANDLE phPeerDist,
                             _Out_opt_ PDWORD pdwSupportedVersion);

DWORD WINAPI PeerDistShutdown(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist);

DWORD WINAPI PeerDistGetStatus(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
                               _Out_ PEERDIST_STATUS *pPeerDistStatus);

DWORD WINAPI PeerDistRegisterForStatusChangeNotification(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist, _In_opt_ HANDLE hCompletionPort,
    _In_opt_ ULONG_PTR ulCompletionKey, _In_ LPOVERLAPPED lpOverlapped,
    _Out_ PEERDIST_STATUS *pPeerDistStatus);
...
DWORD WINAPI PeerDistServerPublishStream(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist, DWORD cbContentIdentifier,
    ...
    _In_opt_ HANDLE hCompletionPort, _In_opt_ ULONG_PTR ulCompletionKey,
    _Out_ PPEERDIST_STREAM_HANDLE phStream);

DWORD WINAPI PeerDistServerPublishAddToStream(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
    _In_ PEERDIST_STREAM_HANDLE hStream, DWORD cbNumberOfBytes,
    _In_reads_bytes_(cbNumberOfBytes) PBYTE pBuffer,
    _In_ LPOVERLAPPED lpOverlapped);

DWORD WINAPI PeerDistServerPublishCompleteStream(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
    _In_ PEERDIST_STREAM_HANDLE hStream, _In_ LPOVERLAPPED lpOverlapped);

DWORD WINAPI
PeerDistServerCloseStreamHandle(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
                                _In_ PEERDIST_STREAM_HANDLE hStream);

DWORD WINAPI PeerDistServerUnpublish(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
                                     DWORD cbContentIdentifier,
                                     _In_reads_bytes_(cbContentIdentifier)
                                         PBYTE pContentIdentifier);

DWORD WINAPI PeerDistServerOpenContentInformation(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist, DWORD cbContentIdentifier,
    _In_reads_bytes_(cbContentIdentifier) PBYTE pContentIdentifier,
    ULONGLONG ullContentOffset, ULONGLONG cbContentLength,
    _In_opt_ HANDLE hCompletionPort, _In_opt_ ULONG_PTR ulCompletionKey,
    _Out_ PPEERDIST_CONTENTINFO_HANDLE phContentInfo);

DWORD WINAPI PeerDistServerRetrieveContentInformation(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
    _In_ PEERDIST_CONTENTINFO_HANDLE hContentInfo, DWORD cbMaxNumberOfBytes,
    _Out_writes_bytes_(cbMaxNumberOfBytes) PBYTE pBuffer,
    _In_ LPOVERLAPPED
        lpOverlapped // Offset and OffsetHigh are offset into content info
);
...
DWORD WINAPI PeerDistClientOpenContent(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
    _In_ PCPEERDIST_CONTENT_TAG pContentTag, _In_opt_ HANDLE hCompletionPort,
    _In_opt_ ULONG_PTR ulCompletionKey,
    _Out_ PPEERDIST_CONTENT_HANDLE phContentHandle);

DWORD WINAPI
PeerDistClientCloseContent(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
                           _In_ PEERDIST_CONTENT_HANDLE hContentHandle);

DWORD WINAPI PeerDistClientAddContentInformation(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
    _In_ PEERDIST_CONTENT_HANDLE hContentHandle, DWORD cbNumberOfBytes,
    _In_reads_bytes_(cbNumberOfBytes) PBYTE pBuffer,
    _In_ LPOVERLAPPED lpOverlapped);
...
DWORD WINAPI PeerDistClientAddData(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
                                   _In_ PEERDIST_CONTENT_HANDLE hContentHandle,
                                   DWORD cbNumberOfBytes,
                                   _In_reads_bytes_(cbNumberOfBytes)
                                       PBYTE pBuffer,
                                   _In_ LPOVERLAPPED lpOverlapped);

DWORD WINAPI PeerDistClientBlockRead(
    _In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
    _In_ PEERDIST_CONTENT_HANDLE hContentHandle, DWORD cbMaxNumberOfBytes,
    _Out_writes_bytes_opt_(cbMaxNumberOfBytes) PBYTE pBuffer,
    DWORD dwTimeoutInMilliseconds, _In_ LPOVERLAPPED lpOverlapped);
...
DWORD WINAPI PeerDistClientFlushContent(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
...
DWORD WINAPI
PeerDistClientCancelAsyncOperation(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
                                   _In_ PEERDIST_CONTENT_HANDLE hContentHandle,
...
DWORD WINAPI PeerDistGetStatusEx(_In_ PEERDIST_INSTANCE_HANDLE hPeerDist,
...
BOOL WINAPI PeerDistGetOverlappedResult(
    _In_ LPOVERLAPPED lpOverlapped, _Out_ LPDWORD lpNumberOfBytesTransferred,
    _In_ BOOL bWait);
...
DWORD WINAPI PeerDistClientGetInformationByHandle(
    ...
    DWORD dwBufferSize, _Out_writes_bytes_(dwBufferSize) LPVOID lpInformation);
