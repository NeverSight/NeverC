...
Abstract:
    ...
    This file contains macros/typedef/function prototypes that are common for
ELS.
...
typedef void(CALLBACK *PFN_MAPPINGCALLBACKPROC)(
    struct _MAPPING_PROPERTY_BAG *pBag, LPVOID data, DWORD dwDataSize,
    HRESULT Result);
...
HRESULT
WINAPI
MappingGetServices(_In_opt_ PMAPPING_ENUM_OPTIONS pOptions,
                   _Out_ PMAPPING_SERVICE_INFO *prgServices,
                   _Out_ DWORD *pdwServicesCount);

HRESULT
WINAPI
MappingFreeServices(_In_ PMAPPING_SERVICE_INFO pServiceInfo);

HRESULT
WINAPI
MappingRecognizeText(_In_ PMAPPING_SERVICE_INFO pServiceInfo,
                     _In_reads_(dwLength) LPCWSTR pszText, _In_ DWORD dwLength,
                     _In_ DWORD dwIndex, _In_opt_ PMAPPING_OPTIONS pOptions,
...
HRESULT
WINAPI
MappingDoAction(_Inout_ PMAPPING_PROPERTY_BAG pBag, _In_ DWORD dwRangeIndex,
...
HRESULT
WINAPI
MappingFreePropertyBag(_In_ PMAPPING_PROPERTY_BAG pBag);
