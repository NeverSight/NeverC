...
public:
  ...
  //    This function wraps the necessary calls and provides the correct
  //    CoSetProxyBlanket parameters in a single function.
  //
  // Arguments:
  ...
  static HRESULT CoSetProxyBlanket(IUnknown *pIUnk) {
    ...
    // Try to see if it's a COM proxy.  If so, see if it's local to the process
    // or a remote COM proxy. If it's remote, set the proxy blanket
      ...
      if (S_OK == hr) {
        ...
        if (S_OK == hr) {
          hr = ::CoSetProxyBlanket(
              pRealUnknown, RPC_C_AUTHN_DEFAULT, RPC_C_AUTHZ_NONE,
              COLE_DEFAULT_PRINCIPAL, RPC_C_AUTHN_LEVEL_DEFAULT,
              RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_STATIC_CLOAKING);
          if (S_OK == hr) {
            hr = ::CoSetProxyBlanket(
                pIUnk, RPC_C_AUTHN_DEFAULT, RPC_C_AUTHZ_NONE,
                COLE_DEFAULT_PRINCIPAL, RPC_C_AUTHN_LEVEL_DEFAULT,
                RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_STATIC_CLOAKING);
          }
          if (E_NOINTERFACE == hr)
            hr = S_OK; // Provider can end up being inproc if we are in the
                       // fdPHost process.  Ignore error and try to proceed.
  ...
  //    provider thread is from a thread pool instead of a thread called
  //    directly by FD. If you want to use this method you must call Initialize
  //    on the class instance. This is best done in
  //    IFunctionDiscoveryProvider::Initialize. Initialize must be called from
  //    one of the methods called by the client process.
  //
  // Arguments:
  //    pIUnk: Interface proxy to enable callbacks on
  //
  ...
  //    Before using that function you must call Initialize on the class
  //    instance. This is best done in IFunctionDiscoveryProvider::Initialize.
  //    Initialize must be called from one of the methods called by the client
  //    process.
  //
  ...
  //          Code=0x80010117 (2147549463): RPC_E_CALL_COMPLETE: "Call context
  //          cannot be accessed after call completed." Severity=FAILURE;
  //          Code=279 (0x117); Facility=1 (0x1) (FACILITY_RPC)
  //       Handle this case gracefully and effectively noop on the
  //       CoSetProxyBlanketWithThreadToken as well.
  //    3. If we get an inproc proxy to the provider we detect that and
  //    subsequent
  //       calls to CoSetProxyBlanketWithThreadToken will effectively noop.
  //
  // Returns: S_OK on success, appropriate HRESULT otherwise
  HRESULT Initialize(IUnknown *pIUnk) {
    ...
    const SID LOCAL_SYSTEM_SID = {
        SID_REVISION, 1, {0, 0, 0, 0, 0, 5}, SECURITY_LOCAL_SYSTEM_RID};
    ...
    if (S_OK == hr) {
      ...
      BOOL isLocalCall =
          (GetRpcOptions(pIUnk) == SERVER_LOCALITY_PROCESS_LOCAL);

      if (!isLocalCall) {
        if (!CheckTokenMembership(NULL, (PSID)&LOCAL_SYSTEM_SID,
                                  &isLocalSystem)) {
          hr = HRESULT_FROM_WIN32(GetLastError());
        }
      }
      ...
      if ((S_OK == hr) && (!isLocalCall) && (!isLocalSystem)) {
        ...
        if (!OpenThreadToken(GetCurrentThread(),
                             TOKEN_IMPERSONATE | TOKEN_QUERY, FALSE, &m_hToken))
          hr = HRESULT_FROM_WIN32(GetLastError());
        if (S_OK == hr) {
          DWORD dwSizeReturned;
          if (!GetTokenInformation(m_hToken, TokenSessionId, &m_dwSessionId,
                                   sizeof(m_dwSessionId), &dwSizeReturned))
            hr = HRESULT_FROM_WIN32(GetLastError());
        }
...
protected:
  ...
  //
  // Returns: ULONG_PTR rpc options on success, assume it's inproc otherwise
  ...
  static ULONG_PTR GetRpcOptions(IUnknown *pIUnk) {
    ...
    HRESULT hr = pIUnk->QueryInterface(
        __uuidof(IRpcOptions), reinterpret_cast<LPVOID *>(&pRpcOptions));
    if (S_OK == hr) {
