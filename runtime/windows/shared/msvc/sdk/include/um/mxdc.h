...
typedef enum tagMxdcImageTypeEnums {
  MXDC_IMAGETYPE_JPEGHIGH_COMPRESSION =
      1, // Lowest Quality, and smallest file size
  MXDC_IMAGETYPE_JPEGMEDIUM_COMPRESSION =
      2, // Medium Quality, and medium file size
...
//
// MxdcGetPDEVAdjustment is exported by XPSDrv configuration module and called
// by the Microsoft XPS Document Converter (MXDC) to supply printer
// configuration data in the form of a property bag.
//
HRESULT WINAPI MxdcGetPDEVAdjustment(
    _In_ HANDLE hPrinter, ULONG cbDevMode,
    _In_reads_bytes_(cbDevMode) const DEVMODE *pDevMode, ULONG cbIn,
    _In_reads_bytes_opt_(cbIn) const VOID *pvIn,
    ULONG cbPrintPropertiesCollection,
    _Inout_updates_bytes_(cbPrintPropertiesCollection)
