...
typedef void(CALLBACK *NotificationHandler)(_In_ GUID connectionId,
...
//   the connection. When re-authentication is needed due to system health
//   change, the callback function func will be called and this GUID pointer
//   will be passed as an _In_ parameter to indicate which connection the
//   callback is for.
//
// - func:
//   callback function pointer used for re-authentication. This function
//   will be called when system health changes and a re-authentication is
//   needed. if the function handler is NULL, pContextData will be ignored, and
//   it means the caller is not interested in system health change notification
//
// - pSessionId:
//   Return session ID in it. This is used to identify the authentication
//   session.
DWORD APIENTRY EapHostPeerBeginSession(
    _In_ DWORD dwFlags, _In_ EAP_METHOD_TYPE eapType,
    ...
    _In_opt_ void *pContextData, _Out_ EAP_SESSIONID *pSessionId,
...
//
// - pEapOutput:
//   It will tell the supplicant what's the next EAPHost API that it should
//   call.
DWORD APIENTRY EapHostPeerProcessReceivedPacket(
    _In_ EAP_SESSIONID sessionHandle, _In_ DWORD cbReceivePacket,
...
DWORD APIENTRY EapHostPeerGetSendPacket(_In_ EAP_SESSIONID sessionHandle,
                                        _Out_ DWORD *pcbSendPacket,
                                        _Outptr_result_buffer_(*pcbSendPacket)
                                            BYTE **ppSendPacket,
                                        _Outptr_ EAP_ERROR **ppEapError);
...
DWORD APIENTRY EapHostPeerGetResult(_In_ EAP_SESSIONID sessionHandle,
...
DWORD APIENTRY EapHostPeerGetUIContext(
    _In_ EAP_SESSIONID sessionHandle, _Out_ DWORD *pdwSizeOfUIContextData,
    _Outptr_result_buffer_(*pdwSizeOfUIContextData) BYTE **ppUIContextData,
    _Outptr_ EAP_ERROR **ppEapError);

// It will be called after UI has been raised. Refer to
// EapHostPeerGetUIContext() for more information.
//
// Parameters:
// - sessionHandle:
//   the value returned through *pSessionId from EapHostPeerBeginSession()
...
DWORD APIENTRY EapHostPeerSetUIContext(
    _In_ EAP_SESSIONID sessionHandle, _In_ DWORD dwSizeOfUIContextData,
...
DWORD APIENTRY EapHostPeerGetResponseAttributes(
    _In_ EAP_SESSIONID sessionHandle, _Out_ EapAttributes *pAttribs,
    _Outptr_ EAP_ERROR **ppEapError);
...
DWORD APIENTRY EapHostPeerSetResponseAttributes(
    _In_ EAP_SESSIONID sessionHandle, _In_ const EapAttributes *const pAttribs,
    _Out_ EapHostPeerResponseAction *pEapOutput,
    _Outptr_ EAP_ERROR **ppEapError);
...
DWORD APIENTRY EapHostPeerGetAuthStatus(_In_ EAP_SESSIONID sessionHandle,
                                        _In_ EapHostPeerAuthParams authParam,
                                        _Out_ DWORD *pcbAuthData,
                                        _Outptr_result_buffer_(*pcbAuthData)
                                            BYTE **ppAuthData,
                                        _Outptr_ EAP_ERROR **ppEapError);
...
// - sessionHandle:
//   the value returned through *pSessionId from EapHostPeerBeginSession()
DWORD APIENTRY EapHostPeerEndSession(_In_ EAP_SESSIONID sessionHandle,
                                     _Outptr_ EAP_ERROR **ppEapError);
...
DWORD APIENTRY EapHostPeerGetDataToUnplumbCredentials(
    ...
    _In_ EAP_SESSIONID sessionHandle, _Outptr_ EAP_ERROR **ppEapError,
...
DWORD APIENTRY EapHostPeerClearConnection(_In_ GUID *pConnectionId,
                                          _Outptr_ EAP_ERROR **ppEapError);
...
// EAPHost will call this function, and it would return user data and user
// identity.
...
DWORD APIENTRY EapHostPeerGetIdentity(
    ...
    // impersonation token for logged on user, to obtain user-related
    // information
    _In_ HANDLE hTokenImpersonateUser,
    // should return true if the user identity and user data blob can not be
    // returned successfully and in needs to be collected through UI
    _Out_ BOOL *pfInvokeUI,
    // size of the buffer pUserDataOut. caller should free this using
    ...
    // identity returned by method. caller should free this using
    // EapHostPeerFreeRuntimeMemory
...
DWORD APIENTRY EapHostPeerGetEncryptedPassword(
    _In_ DWORD dwSizeofPassword,
    _In_reads_bytes_(dwSizeofPassword) LPWSTR szPassword,
    _Outptr_ LPWSTR *ppszEncPassword);
