...
//
//****************************************************************************
typedef LPVOID(WINAPI *PFN_NCRYPT_ALLOC)(_In_ SIZE_T cbSize);

typedef VOID(WINAPI *PFN_NCRYPT_FREE)(_In_ LPVOID pv);
...
#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)
//
// This algorithm is not supported by any BCrypt provider. This identifier is
// for creating persistent stored HMAC keys in the TPM KSP.
//
...
#define NCRYPTBUFFER_PROTECTION_DESCRIPTOR_STRING                              \
  3 // The buffer contains a null-terminated Unicode string that contains the
    // Protection Descriptor.
#define NCRYPTBUFFER_PROTECTION_FLAGS                                          \
  4    // DWORD flags to be passed to NCryptCreateProtectionDescriptor function.
#endif // (NTDDI_VERSION >= NTDDI_WIN8)
...
#define NCRYPTBUFFER_ATTESTATION_STATEMENT_SIGNATURE_PADDING_SALT              \
  NCRYPTBUFFER_ATTESTATION_STATEMENT_SIGNATURE_PADDING_SALT_SIZE // Added to
                                                                 // avoid
                                                                 // failure for
                                                                 // existing
                                                                 // users
#define NCRYPTBUFFER_ATTESTATION_STATEMENT_NONCE                               \
...
typedef _Struct_size_bytes_(cbSize + cbIV +
                            cbOtherInfo) struct _NCRYPT_CIPHER_PADDING_INFO {
  // size of this struct
  ULONG cbSize;
  ...
  // [in, out, optional]
  // The address of a buffer that contains the initialization vector (IV) to use
  // during encryption. The cbIV parameter contains the size of this buffer.
  // This function will modify the contents of this buffer. If you need to reuse
  // the IV later, make sure you make a copy of this buffer before calling this
  // function.
  _Field_size_bytes_(cbIV) PUCHAR pbIV;
  ULONG cbIV;

  // [in, out, optional]
  // The address of a buffer that contains the algorithm specific info to use
  // during encryption. The cbOtherInfo parameter contains the size of this
  // buffer. This function will modify the contents of this buffer. If you need
  // to reuse the buffer later, make sure you make a copy of this buffer before
  ...
  // pbOtherInfo buffer.
  //
  _Field_size_bytes_(cbOtherInfo) PUCHAR pbOtherInfo;
  ULONG cbOtherInfo;
...
//
// The NCRYPT_CLAIM_VBS_KEY_ATTESTATION_STATEMENT claim type is automatically
// transformed to NCRYPT_CLAIM_VBS_ROOT in NCryptCreateClaim and
// NCryptVerifyClaim.
...
#define NCRYPT_ISOLATED_KEY_FLAG_CREATED_IN_ISOLATION                          \
  0x00000001 // if set, this key was generated in isolation, not imported
#define NCRYPT_ISOLATED_KEY_FLAG_IMPORT_ONLY                                   \
  0x00000002 // if set, this key can only be used for importing other keys

#if (NTDDI_VERSION >= NTDDI_WIN11_SV3)
#define NCRYPT_ISOLATED_KEY_FLAG_PER_BOOT_KEY                                  \
  0x00000004 // Indicates renewed keys per boot
#define NCRYPT_VBS_KEY_FLAG_CREATED_IN_ISOLATION                               \
...
// Indicates that the key may be used to import ephemeral keys. This is useful
// in pkcs11 import
...
#define NCRYPT_ISOLATED_KEY_ATTESTED_ATTRIBUTES_CURRENT_VERSION                \
...
#define NCRYPT_VBS_KEY_ATTESTED_ATTRIBUTES                                     \
  NCRYPT_ISOLATED_KEY_ATTESTED_ATTRIBUTES
#define PNCRYPT_VBS_KEY_ATTESTED_ATTRIBUTES                                    \
  PNCRYPT_ISOLATED_KEY_ATTESTED_ATTRIBUTES
#define NCRYPT_VBS_KEY_ATTESTED_ATTRIBUTES_CURRENT_VERSION                     \
...
#define NCRYPT_EXPORTED_ISOLATED_KEY_HEADER_CURRENT_VERSION                    \
...
typedef struct _NCRYPT_EXPORTED_ISOLATED_KEY_HEADER {
  ...
  ULONG PerBootKey : 1; // Set to TRUE if the key is to be valid in the current
                        // boot cycle only
  ULONG Reserved : 31;  // Leave as 0
  ULONG cbAlgName; // Number of bytes in Unicode algorithm name following header
                   // + terminating NULL
  ULONG
  cbNonce; // Number of bytes in the nonce used to encrypt the isolated key
  ULONG cbAuthTag;     // Number of bytes in authentication tag resulting from
                       // encrypting the isolated key
...
typedef struct _NCRYPT_EXPORTED_ISOLATED_KEY_ENVELOPE {
  ...
  // UCHAR AlgorithmName[Header.cbAlgName]       -- Unicode algorithm name
  // including terminating NULL UCHAR Nonce[Header.cbNonce]                 --
  // Nonce buffer used when encrypting isolated key
  // ---- data after this point is not integrity protected in transit
  // UCHAR AesGcmAuthTag[Header.cbAuthTag]
  // UCHAR WrappingKeyBlob[Header.cbWrappingKey] -- RSA-OAEP encrypted AES
  // wrapping key UCHAR IsolatedKeyBlob[Header.cbIsolatedKey] -- AES-GCM
  // encrypted key to import
...
#if (NTDDI_VERSION >=                                                          \
     NTDDI_WIN10_RS3) // && (NTDDI_VERSION < NTDDI_WIN11_SV3) // Keep on
                      // backward compitability for VSM_KEY_ATTESTATION
...
#define NCRYPT_VBS_ROOT_ATTESTATION_HEADER_CURRENT_VERSION                     \
  NCRYPT_VBS_ROOT_ATTESTATION_HEADER_V0
#define VBS_ROOT_ATTESTATION_HEADER_MAGIC                                      \
  0x48435256 // 'VRCH' = 0x56, 0x52, 0x43, 0x48.
...
typedef struct _NCRYPT_VBS_ROOT_ATTESTATION_HEADER {
  ...
  ULONG cbAttributes; // Number of bytes in attributes of the isolated key
                      // (including public key blob )
  ULONG cbNonce;     // Number of bytes in the provided nonce, can be 0 if nonce
                     // doesn't exist
  ULONG cbReport;    // Number of bytes in key isolation report from the secure
                     // kernel
  ULONG cbSignature; // Number of bytes in the secure kernel signature over the
                     // isolation report
  // UCHAR Attributes[cbAttributes]  -- Trustlet-reported attributes of the key
  // UCHAR Nonce[cbNonce]            -- Nonce value to be used when hashing
  // Attributes UCHAR Report[cbReport]          -- Secure kernel report
  // including hash of Attributes, dwFlags and nonce (if available) UCHAR
  // Signature[cbSignature]    -- Secure kernel signature of the report
...
#define NCRYPT_VBS_IDENTITY_ATTESTATION_PADDING_CURRENT_VERSION                \
  NCRYPT_VBS_IDENTITY_ATTESTATION_PADDING_V0

typedef struct _NCRYPT_VBS_IDENTITY_ATTESTATION_PADDING {
  ULONG
  Version; // Set to NCRYPT_VBS_IDENTITY_ATTESTATION_PADDING_CURRENT_VERSION
  ULONG ulPaddingScheme; // Padding scheme of the signing algorithm that was
                         // used through claim creation.
  ULONG cbHashAlg;       // Number of bytes in Unicode name that identifies the
                   // cryptographic padding algorithm. This algorithm must be a
                   // hashing algorithm.
  ULONG ulSalt; // Number of bytes in of the random salt to use for the padding.
  // UCHAR HashAlg[cbHashAlg]  -- Unicode name of hash algorithm used through
  // attributes signing to create padding
...
#define NCRYPT_VBS_IDENTITY_ATTESTATION_HEADER_CURRENT_VERSION                 \
  NCRYPT_VBS_IDENTITY_ATTESTATION_HEADER_V0
#define VBS_IDENTITY_ATTESTATION_HEADER_MAGIC                                  \
  0x48434956 // 'VICH' = 0x56, 0x49, 0x43, 0x48.

typedef struct _NCRYPT_VBS_IDENTITY_ATTESTATION_HEADER {
  ULONG Magic; // {'H', 'C', 'I', 'V'} - 'VICH' for VBS Identity Claim Header
  ULONG
  Version; // Set to NCRYPT_VBS_IDENTITY_ATTESTATION_HEADER_CURRENT_VERSION
  ULONG cbAttributes; // Number of bytes in attributes of the isolated key
                      // (including public key blob )
  ULONG cbNonce;   // Number of bytes in the provided nonce, can be 0 if nonce
                   // doesn't exist
  ULONG cbHashAlg; // Number of bytes in Unicode name of hash algorithm used
  ULONG cbPadding; // Padding info (scheme and more) of the signing algorithm
                   // that was used through claim creation
  ULONG cbSignatureAlg; // Number of bytes in Unicode name of the signing hash
                        // algorithm used
  ULONG cbSignature;    // Number of bytes in the signature over the hash of
                        // Attributes, dwFlags and nonce (if available)
  // UCHAR Attributes[cbAttributes]     -- Trustlet-reported attributes of the
  // subject key blob UCHAR Nonce[cbNonce]               -- Nonce value to be
  // used when hashing Attributes UCHAR HashAlg[cbHashAlg]           -- Unicode
  // name of hash algorithm used through attributes signing UCHAR
  // Padding[cbPadding]           -- Padding information that is set through
  // identity hash signing UCHAR SignatureAlg[cbSignatureAlg] -- Unicode name of
  // signing algorithm used UCHAR Signature[cbSignature]       -- Signature over
  // the hash of Attributes and nonce (if available)
...
#define NCRYPT_VBS_KEY_ATTESTATION_STATEMENT_CURRENT_VERSION                   \
  NCRYPT_VBS_KEY_ATTESTATION_STATEMENT_V2
#define VBS_KEY_ATTESTATION_STATEMENT_MAGIC                                    \
  0x53414B56 // 'VKAS' = 0x56, 0x4B, 0x41, 0x53.

typedef struct _NCRYPT_VBS_KEY_ATTESTATION_STATEMENT {
  ULONG
  Magic; // {'S', 'A', 'K', 'V'} - 'VKAS' for VBS Key Attestation Statement
  ...
  // name of hash algorithm used through attributes signing UCHAR
  // Padding[cbPadding]            -- Padding information that is set through
  // identity hash signing UCHAR SignatureAlg[cbSignatureAlg]  -- Unicode name
  // of signing algorithm used UCHAR Signature[cbSignature]        -- Signature
  // over the hash of Attributes and nonce (if available)
...
typedef struct _NCRYPT_VBS_IDENTITY_KEY_ATTESTATION_CLAIM_DETAILS {
  ULONG ulKeyFlags;            // NCRYPT_ISOLATED_KEY_ flags
  LPCWSTR pszSignatureHashAlg; // A pointer to a Unicode string of the claim
                               // signature hash algorithm
  ULONG ulPaddingScheme;     // Padding scheme of the signing algorithm that was
                             // used through claim creation
  LPCWSTR pszPaddingHashAlg; // A pointer to a Unicode string of the claim
                             // padding hash algorithm
  ULONG ulPaddingSalt; // Padding salt of the signing algorithm that was used
...
#define NCRYPT_TPM_PLATFORM_ATTESTATION_STATEMENT_CURRENT_VERSION              \
...
#define NCRYPT_PAD_PKCS1_FLAG                                                  \
  0x00000002 // NCryptEncrypt/Decrypt NCryptSignHash/VerifySignature
...
#endif                                    // (NTDDI_VERSION >= NTDDI_WIN8)
#if (NTDDI_VERSION >= NTDDI_WIN11_GE)
#define NCRYPT_PAD_PQDSA_FLAG                                                  \
  BCRYPT_PAD_PQDSA // BCryptSignHash/VerifySignature
                   // NCryptSignHash/VerifySignature
#endif
...
#if (NTDDI_VERSION >= NTDDI_WIN10_RS2)
#define NCRYPT_PREFER_VIRTUAL_ISOLATION_FLAG                                   \
  0x00010000 // NCryptCreatePersistedKey NCryptImportKey
#define NCRYPT_USE_VIRTUAL_ISOLATION_FLAG                                      \
  0x00020000 // NCryptCreatePersistedKey NCryptImportKey
#define NCRYPT_USE_PER_BOOT_KEY_FLAG                                           \
  0x00040000 // NCryptCreatePersistedKey NCryptImportKey
#endif       // (NTDDI_VERSION >= NTDDI_WIN10_RS2)

#if (NTDDI_VERSION >= NTDDI_WIN11_SV3)
#define NCRYPT_PREFER_VBS_FLAG                                                 \
  NCRYPT_PREFER_VIRTUAL_ISOLATION_FLAG // NCryptCreatePersistedKey
                                       // NCryptImportKey
#define NCRYPT_REQUIRE_VBS_FLAG                                                \
  NCRYPT_USE_VIRTUAL_ISOLATION_FLAG // NCryptCreatePersistedKey NCryptImportKey
#define NCRYPT_USE_VBS_PER_BOOT_KEY_FLAG                                       \
  NCRYPT_USE_PER_BOOT_KEY_FLAG // NCryptCreatePersistedKey NCryptImportKey
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptOpenStorageProvider(_Out_ NCRYPT_PROV_HANDLE *phProvider,
                          _In_opt_ LPCWSTR pszProviderName, _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI NCryptEnumAlgorithms(
    _In_ NCRYPT_PROV_HANDLE hProvider, _In_ DWORD dwAlgOperations,
    _Out_ DWORD *pdwAlgCount,
    _Outptr_result_buffer_(*pdwAlgCount) NCryptAlgorithmName **ppAlgList,
    _In_ DWORD dwFlags);

_Check_return_ SECURITY_STATUS WINAPI
NCryptIsAlgSupported(_In_ NCRYPT_PROV_HANDLE hProvider, _In_ LPCWSTR pszAlgId,
                     _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptEnumKeys(_In_ NCRYPT_PROV_HANDLE hProvider, _In_opt_ LPCWSTR pszScope,
               _Outptr_ NCryptKeyName **ppKeyName, _Inout_ PVOID *ppEnumState,
               _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptEnumStorageProviders(_Out_ DWORD *pdwProviderCount,
                           _Outptr_result_buffer_(*pdwProviderCount)
...
SECURITY_STATUS
WINAPI
NCryptFreeBuffer(_Pre_notnull_ PVOID pvInput);
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptOpenKey(_In_ NCRYPT_PROV_HANDLE hProvider, _Out_ NCRYPT_KEY_HANDLE *phKey,
              _In_ LPCWSTR pszKeyName, _In_opt_ DWORD dwLegacyKeySpec,
              _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI NCryptCreatePersistedKey(
    _In_ NCRYPT_PROV_HANDLE hProvider, _Out_ NCRYPT_KEY_HANDLE *phKey,
    _In_ LPCWSTR pszAlgId, _In_opt_ LPCWSTR pszKeyName,
    _In_ DWORD dwLegacyKeySpec, _In_ DWORD dwFlags);
...
#if (NTDDI_VERSION >= NTDDI_WIN10_RS5)
#define NCRYPT_DISMISS_UI_TIMEOUT_SEC_PROPERTY                                 \
  L"SmartCardDismissUITimeoutSeconds"
#endif                                 // (NTDDI_VERSION >= NTDDI_WIN10_RS5)
#if (NTDDI_VERSION >= NTDDI_WIN11_SV3) // NTDDI_WIN11_SV3 is not final
...
#define NCRYPT_PARAMETER_SET_NAME_PROPERTY                                     \
  BCRYPT_PARAMETER_SET_NAME // For PQC keys
...
#define NCRYPT_PCP_TPM12_IDBINDING_DYNAMIC_PROPERTY                            \
  L"PCP_TPM12_IDBINDING_DYNAMIC"
...
#define NCRYPT_PCP_PLATFORM_BINDING_PCRALGID_PROPERTY                          \
  L"PCP_PLATFORM_BINDING_PCRALGID"
...
#define NCRYPT_PCP_TPM_IFX_RSA_KEYGEN_PROHIBITED_PROPERTY                      \
...
#define NCRYPT_TPM_PSS_SALT_SIZE_MAXIMUM                                       \
  0x00000001 // Pre-TPM Spec-1.16: Max allowed salt size
#define NCRYPT_TPM_PSS_SALT_SIZE_HASHSIZE                                      \
  0x00000002 // Post-1.16: PSS salt = hashLen
#endif       // (NTDDI_VERSION >= NTDDI_WIN10_RS5)
...
#define NCRYPT_PLUTON_KDF_PARENT_KEY_UNIQUE_NAME_PROPERTY                      \
  L"PlutonKdfParentKeyUniqueName"
#define NCRYPT_PLUTON_KDF_PARAMS_BUFFER_DESC_PROPERTY                          \
  L"PlutonKdfParamsBufferDesc"
...
#define NCRYPT_ALTERNATE_KEY_STORAGE_LOCATION_PROPERTY                         \
...
#define NCRYPT_PIN_CACHE_FREE_APPLICATION_TICKET_PROPERTY                      \
  L"PinCacheFreeApplicationTicket"

#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)

#define NCRYPT_PIN_CACHE_FLAGS_PROPERTY L"PinCacheFlags"

// The NCRYPT_PIN_CACHE_FLAGS_PROPERTY property is a DWORD value that can be set
// from a trusted process. The following flags can be set.
...
#define NCRYPT_PIN_CACHE_APPLICATION_TICKET_PROPERTY                           \
  L"PinCacheApplicationTicket"
#define NCRYPT_PIN_CACHE_APPLICATION_IMAGE_PROPERTY L"PinCacheApplicationImage"
#define NCRYPT_PIN_CACHE_APPLICATION_STATUS_PROPERTY                           \
  L"PinCacheApplicationStatus"
#define NCRYPT_PIN_CACHE_PIN_PROPERTY L"PinCachePin"
#define NCRYPT_PIN_CACHE_IS_GESTURE_REQUIRED_PROPERTY                          \
  L"PinCacheIsGestureRequired"

#define NCRYPT_PIN_CACHE_REQUIRE_GESTURE_FLAG 0x00000001

// The NCRYPT_PIN_CACHE_PIN_PROPERTY and
// NCRYPT_PIN_CACHE_APPLICATION_TICKET_PROPERTY properties return a 32 byte
// random unique ID encoded as a null terminated base64 Unicode string. The
// string length is 32 * 4/3 + 1 characters = 45 characters, 90 bytes
...
// The NCRYPT_PIN_CACHE_CLEAR_PROPERTY property is a DWORD value. The following
// option can be set:
...
_Check_return_ _Success_(return == 0) SECURITY_STATUS WINAPI
    NCryptGetProperty(_In_ NCRYPT_HANDLE hObject, _In_ LPCWSTR pszProperty,
                      _Out_writes_bytes_to_opt_(cbOutput, *pcbResult)
                          PBYTE pbOutput,
                      _In_ DWORD cbOutput, _Out_ DWORD *pcbResult,
                      _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptSetProperty(_In_ NCRYPT_HANDLE hObject, _In_ LPCWSTR pszProperty,
                  _In_reads_bytes_(cbInput) PBYTE pbInput, _In_ DWORD cbInput,
                  _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptFinalizeKey(_In_ NCRYPT_KEY_HANDLE hKey, _In_ DWORD dwFlags);

_Check_return_ SECURITY_STATUS WINAPI NCryptEncrypt(
    _In_ NCRYPT_KEY_HANDLE hKey, _In_reads_bytes_opt_(cbInput) PBYTE pbInput,
    _In_ DWORD cbInput, _In_opt_ VOID *pPaddingInfo,
    _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PBYTE pbOutput,
    _In_ DWORD cbOutput, _Out_ DWORD *pcbResult, _In_ DWORD dwFlags);

_Check_return_ SECURITY_STATUS WINAPI NCryptDecrypt(
    _In_ NCRYPT_KEY_HANDLE hKey, _In_reads_bytes_opt_(cbInput) PBYTE pbInput,
    _In_ DWORD cbInput, _In_opt_ VOID *pPaddingInfo,
    _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PBYTE pbOutput,
    _In_ DWORD cbOutput, _Out_ DWORD *pcbResult, _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI NCryptEncapsulate(
    _In_ NCRYPT_KEY_HANDLE hKey,
    _Out_writes_bytes_to_opt_(cbSecretKey, *pcbSecretKey) PBYTE pbSecretKey,
    _In_ ULONG cbSecretKey, _Out_ ULONG *pcbSecretKey,
    _Out_writes_bytes_to_opt_(cbCipherText, *pcbCipherText) PBYTE pbCipherText,
    _In_ ULONG cbCipherText, _Out_ ULONG *pcbCipherText, _In_ ULONG dwFlags);

_Check_return_ SECURITY_STATUS WINAPI NCryptDecapsulate(
    _In_ NCRYPT_KEY_HANDLE hKey,
    _In_reads_bytes_(cbCipherText) PBYTE pbCipherText, _In_ ULONG cbCipherText,
    _Out_writes_bytes_to_opt_(cbSecretKey, *pcbSecretKey) PBYTE pbSecretKey,
    _In_ ULONG cbSecretKey, _Out_ ULONG *pcbSecretKey, _In_ ULONG dwFlags);
...
#define NCRYPT_TPM_LOADABLE_KEY_BLOB_MIN_SIZE                                  \
  sizeof(NCRYPT_TPM_LOADABLE_KEY_BLOB_HEADER)
...
#define NCRYPT_TPM_PERSISTENT_KEY_BLOB_MIN_SIZE                                \
  sizeof(NCRYPT_TPM_PERSISTENT_KEY_BLOB_HEADER)
...
typedef struct _NCRYPT_PQ_BLOB {
  ...
  // UCHAR BCryptType[cbBCryptType]  -- The BCrypt blob type including '\0'. For
  // example, BCRYPT_PQDSA_PRIVATE_BLOB or BCRYPT_PQDSA_PUBLIC_BLOB . UCHAR
  // BCryptBlob[cbBCryptBlob]  -- BCrypt blob data. Currently it's
...
_Check_return_ SECURITY_STATUS WINAPI NCryptImportKey(
    _In_ NCRYPT_PROV_HANDLE hProvider, _In_opt_ NCRYPT_KEY_HANDLE hImportKey,
    _In_ LPCWSTR pszBlobType, _In_opt_ NCryptBufferDesc *pParameterList,
    _Out_ NCRYPT_KEY_HANDLE *phKey, _In_reads_bytes_(cbData) PBYTE pbData,
    _In_ DWORD cbData, _In_ DWORD dwFlags);

_Check_return_ SECURITY_STATUS WINAPI NCryptExportKey(
    _In_ NCRYPT_KEY_HANDLE hKey, _In_opt_ NCRYPT_KEY_HANDLE hExportKey,
    _In_ LPCWSTR pszBlobType, _In_opt_ NCryptBufferDesc *pParameterList,
    _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PBYTE pbOutput,
    _In_ DWORD cbOutput, _Out_ DWORD *pcbResult, _In_ DWORD dwFlags);

_Check_return_ SECURITY_STATUS WINAPI NCryptSignHash(
    _In_ NCRYPT_KEY_HANDLE hKey, _In_opt_ VOID *pPaddingInfo,
    _In_reads_bytes_(cbHashValue) PBYTE pbHashValue, _In_ DWORD cbHashValue,
...
_Check_return_ SECURITY_STATUS WINAPI NCryptVerifySignature(
    _In_ NCRYPT_KEY_HANDLE hKey, _In_opt_ VOID *pPaddingInfo,
    _In_reads_bytes_(cbHashValue) PBYTE pbHashValue, _In_ DWORD cbHashValue,
    _In_reads_bytes_(cbSignature) PBYTE pbSignature, _In_ DWORD cbSignature,
    _In_ DWORD dwFlags);

SECURITY_STATUS
WINAPI
NCryptDeleteKey(_In_ NCRYPT_KEY_HANDLE hKey, _In_ DWORD dwFlags);

SECURITY_STATUS
WINAPI
NCryptFreeObject(_In_ NCRYPT_HANDLE hObject);

#pragma region Desktop Family or OneCore Family
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_SYSTEM)
BOOL WINAPI NCryptIsKeyHandle(_In_ NCRYPT_KEY_HANDLE hKey);

_Check_return_ SECURITY_STATUS WINAPI NCryptTranslateHandle(
    _Out_opt_ NCRYPT_PROV_HANDLE *phProvider, _Out_ NCRYPT_KEY_HANDLE *phKey,
    _In_ HCRYPTPROV hLegacyProv, _In_opt_ HCRYPTKEY hLegacyKey,
    _In_opt_ DWORD dwLegacyKeySpec, _In_ DWORD dwFlags);
...
_Check_return_ SECURITY_STATUS WINAPI
NCryptNotifyChangeKey(_In_ NCRYPT_PROV_HANDLE hProvider,
                      _Inout_ HANDLE *phEvent, _In_ DWORD dwFlags);

#endif // WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP |
       // WINAPI_PARTITION_SYSTEM)
#pragma endregion

_Check_return_ SECURITY_STATUS WINAPI NCryptSecretAgreement(
    _In_ NCRYPT_KEY_HANDLE hPrivKey, _In_ NCRYPT_KEY_HANDLE hPubKey,
    _Out_ NCRYPT_SECRET_HANDLE *phAgreedSecret, _In_ DWORD dwFlags);

_Check_return_ SECURITY_STATUS WINAPI NCryptDeriveKey(
    _In_ NCRYPT_SECRET_HANDLE hSharedSecret, _In_ LPCWSTR pwszKDF,
...
_Check_return_ SECURITY_STATUS WINAPI NCryptKeyDerivation(
    _In_ NCRYPT_KEY_HANDLE hKey, _In_opt_ NCryptBufferDesc *pParameterList,
...
_Check_return_ SECURITY_STATUS WINAPI NCryptCreateClaim(
    _In_opt_ NCRYPT_KEY_HANDLE hSubjectKey,
    _In_opt_ NCRYPT_KEY_HANDLE hAuthorityKey, _In_ DWORD dwClaimType,
    _In_opt_ NCryptBufferDesc *pParameterList,
    _Out_writes_bytes_to_opt_(cbClaimBlob, *pcbResult) PBYTE pbClaimBlob,
    _In_ DWORD cbClaimBlob, _Out_ DWORD *pcbResult, _In_ DWORD dwFlags);

#endif // (NTDDI_VERSION >= NTDDI_WINTHRESHOLD)

#if (NTDDI_VERSION >= NTDDI_WINTHRESHOLD)

_Check_return_ SECURITY_STATUS WINAPI NCryptVerifyClaim(
    _In_ NCRYPT_KEY_HANDLE hSubjectKey,
    _In_opt_ NCRYPT_KEY_HANDLE hAuthorityKey, _In_ DWORD dwClaimType,
    _In_opt_ NCryptBufferDesc *pParameterList,
    _In_reads_bytes_(cbClaimBlob) PBYTE pbClaimBlob, _In_ DWORD cbClaimBlob,
    _Out_ NCryptBufferDesc *pOutput, _In_ DWORD dwFlags);
...
#define NCRYPT_KEY_STORAGE_INTERFACE_VERSION_3                                 \
  BCRYPT_MAKE_INTERFACE_VERSION(3, 0)
#define NCRYPT_KEY_STORAGE_INTERFACE_VERSION_4                                 \
  BCRYPT_MAKE_INTERFACE_VERSION(4, 0)
