...
#define WTS_SECURITY_CURRENT_USER_ACCESS                                       \
  (WTS_SECURITY_SET_INFORMATION |                                              \
   WTS_SECURITY_RESET WTS_SECURITY_VIRTUAL_CHANNELS |                          \
...
typedef enum _WTS_INFO_CLASS {
  ...
  WTSSessionInfoEx,
  WTSConfigInfo,
  WTSValidationInfo, // Info Class value used to fetch Validation Information
...
/*=====================================================================
     ...
     This structure will be returned from WTSQuerySessionInformation when the
user queries for license validation information.
=====================================================================*/
...
BOOL WINAPI WTSStopRemoteControlSession(IN ULONG LogonId);

BOOL WINAPI WTSStartRemoteControlSessionW(_In_ LPWSTR pTargetServerName,
                                          IN ULONG TargetLogonId,
                                          IN BYTE HotkeyVk,
                                          IN USHORT HotkeyModifiers);

BOOL WINAPI WTSStartRemoteControlSessionA(_In_ LPSTR pTargetServerName,
                                          IN ULONG TargetLogonId,
                                          IN BYTE HotkeyVk,
                                          IN USHORT HotkeyModifiers);
...
BOOL WINAPI WTSConnectSessionA(IN ULONG LogonId, IN ULONG TargetLogonId,
                               _In_ PSTR pPassword, IN BOOL bWait);

BOOL WINAPI WTSConnectSessionW(IN ULONG LogonId, IN ULONG TargetLogonId,
                               _In_ PWSTR pPassword, IN BOOL bWait);
...
BOOL WINAPI WTSEnumerateServersW(_In_ LPWSTR pDomainName, IN DWORD Reserved,
                                 IN DWORD Version,
                                 _Outptr_result_buffer_(*pCount)
                                     PWTS_SERVER_INFOW *ppServerInfo,
                                 _Out_ DWORD *pCount);

BOOL WINAPI WTSEnumerateServersA(_In_ LPSTR pDomainName, IN DWORD Reserved,
                                 IN DWORD Version,
                                 _Outptr_result_buffer_(*pCount)
...
HANDLE
WINAPI
WTSOpenServerW(_In_ LPWSTR pServerName);

HANDLE
WINAPI
WTSOpenServerA(_In_ LPSTR pServerName);

HANDLE
WINAPI
WTSOpenServerExW(_In_ LPWSTR pServerName);

HANDLE
WINAPI
WTSOpenServerExA(_In_ LPSTR pServerName);
...
BOOL WINAPI WTSEnumerateSessionsW(IN HANDLE hServer, IN DWORD Reserved,
                                  IN DWORD Version,
                                  _Outptr_result_buffer_(*pCount)
                                      PWTS_SESSION_INFOW *ppSessionInfo,
                                  _Out_ DWORD *pCount);

BOOL WINAPI WTSEnumerateSessionsA(IN HANDLE hServer, IN DWORD Reserved,
                                  IN DWORD Version,
                                  _Outptr_result_buffer_(*pCount)
                                      PWTS_SESSION_INFOA *ppSessionInfo,
                                  _Out_ DWORD *pCount);

BOOL WINAPI WTSEnumerateSessionsExW(_In_ HANDLE hServer, _Inout_ DWORD *pLevel,
                                    _In_ DWORD Filter,
                                    _Outptr_result_buffer_(*pCount)
                                        PWTS_SESSION_INFO_1W *ppSessionInfo,
                                    _Out_ DWORD *pCount);

BOOL WINAPI WTSEnumerateSessionsExA(_In_ HANDLE hServer, _Inout_ DWORD *pLevel,
                                    _In_ DWORD Filter,
                                    _Outptr_result_buffer_(*pCount)
...
BOOL WINAPI WTSEnumerateProcessesW(IN HANDLE hServer, IN DWORD Reserved,
                                   IN DWORD Version,
                                   OUT PWTS_PROCESS_INFOW *ppProcessInfo,
                                   OUT DWORD *pCount);

BOOL WINAPI WTSEnumerateProcessesA(IN HANDLE hServer, IN DWORD Reserved,
                                   IN DWORD Version,
...
BOOL WINAPI WTSTerminateProcess(IN HANDLE hServer, IN DWORD ProcessId,
                                IN DWORD ExitCode);

/*------------------------------------------------*/

BOOL WINAPI WTSQuerySessionInformationW(
    IN HANDLE hServer, IN DWORD SessionId, IN WTS_INFO_CLASS WTSInfoClass,
...
BOOL WINAPI WTSQueryUserConfigW(_In_ LPWSTR pServerName, _In_ LPWSTR pUserName,
                                IN WTS_CONFIG_CLASS WTSConfigClass,
                                _Outptr_result_bytebuffer_(*pBytesReturned)
                                    LPWSTR *ppBuffer,
                                _Out_ DWORD *pBytesReturned);

BOOL WINAPI WTSQueryUserConfigA(_In_ LPSTR pServerName, _In_ LPSTR pUserName,
                                IN WTS_CONFIG_CLASS WTSConfigClass,
                                _Outptr_result_bytebuffer_(*pBytesReturned)
                                    LPSTR *ppBuffer,
                                _Out_ DWORD *pBytesReturned);
...
BOOL WINAPI WTSSetUserConfigW(_In_ LPWSTR pServerName, _In_ LPWSTR pUserName,
                              IN WTS_CONFIG_CLASS WTSConfigClass,
                              _In_reads_bytes_(DataLength) LPWSTR pBuffer,
                              IN DWORD DataLength);

BOOL WINAPI WTSSetUserConfigA(_In_ LPSTR pServerName, _In_ LPSTR pUserName,
                              IN WTS_CONFIG_CLASS WTSConfigClass,
...
BOOL WINAPI WTSSendMessageW(IN HANDLE hServer, IN DWORD SessionId,
                            _In_reads_bytes_(TitleLength) LPWSTR pTitle,
                            IN DWORD TitleLength,
                            _In_reads_bytes_(MessageLength) LPWSTR pMessage,
                            IN DWORD MessageLength, IN DWORD Style,
                            IN DWORD Timeout, _Out_ DWORD *pResponse,
                            IN BOOL bWait);

BOOL WINAPI WTSSendMessageA(IN HANDLE hServer, IN DWORD SessionId,
...
BOOL WINAPI WTSDisconnectSession(IN HANDLE hServer, IN DWORD SessionId,
                                 IN BOOL bWait);

/*------------------------------------------------*/

BOOL WINAPI WTSLogoffSession(IN HANDLE hServer, IN DWORD SessionId,
                             IN BOOL bWait);
...
BOOL WINAPI WTSWaitSystemEvent(IN HANDLE hServer, IN DWORD EventMask,
...
HANDLE
WINAPI
WTSVirtualChannelOpen(IN HANDLE hServer, IN DWORD SessionId,
...
HANDLE
WINAPI
WTSVirtualChannelOpenEx(IN DWORD SessionId,
                        _In_ LPSTR pVirtualName, /* ascii name */
                        IN DWORD flags);

BOOL WINAPI WTSVirtualChannelClose(IN HANDLE hChannelHandle);

BOOL WINAPI WTSVirtualChannelRead(IN HANDLE hChannelHandle, IN ULONG TimeOut,
                                  _Out_writes_bytes_to_(BufferSize, *pBytesRead)
                                      PCHAR Buffer,
                                  IN ULONG BufferSize, _Out_ PULONG pBytesRead);

BOOL WINAPI WTSVirtualChannelWrite(IN HANDLE hChannelHandle,
                                   _In_reads_bytes_(Length) PCHAR Buffer,
                                   IN ULONG Length, _Out_ PULONG pBytesWritten);
...
BOOL WINAPI WTSVirtualChannelQuery(IN HANDLE hChannelHandle,
                                   IN WTS_VIRTUAL_CLASS, OUT PVOID *ppBuffer,
...
BOOL WINAPI WTSRegisterSessionNotificationEx(IN HANDLE hServer, IN HWND hWnd,
                                             IN DWORD dwFlags);

BOOL WINAPI WTSUnRegisterSessionNotificationEx(IN HANDLE hServer, IN HWND hWnd);
...
BOOL WINAPI WTSFreeMemoryExW(_In_ WTS_TYPE_CLASS WTSTypeClass,
                             _In_ PVOID pMemory, _In_ ULONG NumberOfEntries);

BOOL WINAPI WTSFreeMemoryExA(_In_ WTS_TYPE_CLASS WTSTypeClass,
                             _In_ PVOID pMemory, _In_ ULONG NumberOfEntries);
...
BOOL WINAPI WTSEnumerateProcessesExW(_In_ HANDLE hServer, _Inout_ DWORD *pLevel,
                                     _In_ DWORD SessionId,
                                     _Out_ LPWSTR *ppProcessInfo,
                                     _Out_ DWORD *pCount);

BOOL WINAPI WTSEnumerateProcessesExA(_In_ HANDLE hServer, _Inout_ DWORD *pLevel,
                                     _In_ DWORD SessionId,
...
BOOL WINAPI WTSEnumerateListenersW(_In_ HANDLE hServer, _In_ PVOID pReserved,
                                   _In_ DWORD Reserved,
                                   _Out_writes_opt_(*pCount)
                                       PWTSLISTENERNAMEW pListeners,
                                   _Inout_ DWORD *pCount);

BOOL WINAPI WTSEnumerateListenersA(_In_ HANDLE hServer, _In_ PVOID pReserved,
                                   _In_ DWORD Reserved,
                                   _Out_writes_opt_(*pCount)
...
BOOL WINAPI WTSQueryListenerConfigW(_In_ HANDLE hServer, _In_ PVOID pReserved,
                                    _In_ DWORD Reserved,
                                    _In_ LPWSTR pListenerName,
                                    _Out_ PWTSLISTENERCONFIGW pBuffer);

BOOL WINAPI WTSQueryListenerConfigA(_In_ HANDLE hServer, _In_ PVOID pReserved,
                                    _In_ DWORD Reserved,
                                    _In_ LPSTR pListenerName,
                                    _Out_ PWTSLISTENERCONFIGA pBuffer);

BOOL WINAPI WTSCreateListenerW(_In_ HANDLE hServer, _In_ PVOID pReserved,
                               _In_ DWORD Reserved, _In_ LPWSTR pListenerName,
                               _In_ PWTSLISTENERCONFIGW pBuffer,
                               _In_ DWORD flag);

BOOL WINAPI WTSCreateListenerA(_In_ HANDLE hServer, _In_ PVOID pReserved,
                               _In_ DWORD Reserved, _In_ LPSTR pListenerName,
                               _In_ PWTSLISTENERCONFIGA pBuffer,
                               _In_ DWORD flag);

BOOL WINAPI WTSSetListenerSecurityW(
    _In_ HANDLE hServer, _In_ PVOID pReserved, _In_ DWORD Reserved,
    _In_ LPWSTR pListenerName, _In_ SECURITY_INFORMATION SecurityInformation,
    _In_ PSECURITY_DESCRIPTOR pSecurityDescriptor);

BOOL WINAPI WTSSetListenerSecurityA(
    _In_ HANDLE hServer, _In_ PVOID pReserved, _In_ DWORD Reserved,
    _In_ LPSTR pListenerName, _In_ SECURITY_INFORMATION SecurityInformation,
    _In_ PSECURITY_DESCRIPTOR pSecurityDescriptor);

BOOL WINAPI WTSGetListenerSecurityW(
    _In_ HANDLE hServer, _In_ PVOID pReserved, _In_ DWORD Reserved,
    _In_ LPWSTR pListenerName, _In_ SECURITY_INFORMATION SecurityInformation,
    _Out_opt_ PSECURITY_DESCRIPTOR pSecurityDescriptor, _In_ DWORD nLength,
...
WTS_CLOUD_AUTH_HANDLE WINAPI WTSCloudAuthOpen(_In_ const GUID *activityId);

void WINAPI WTSCloudAuthClose(_In_ WTS_CLOUD_AUTH_HANDLE cloudAuthHandle);

BOOL WINAPI
WTSCloudAuthGetServerNonce(_In_ WTS_CLOUD_AUTH_HANDLE cloudAuthHandle,
...
BOOL WINAPI WTSCloudAuthConvertAssertionToSerializedUserCredential(
    ...
    _In_ ULONG assertionLength, _In_z_ PCWSTR resourceId,
    _Out_ WTS_SERIALIZED_USER_CREDENTIAL **userCredential);

BOOL WINAPI WTSCloudAuthNetworkLogonWithSerializedCredential(
    _In_ WTS_CLOUD_AUTH_HANDLE cloudAuthHandle,
    _In_ WTS_SERIALIZED_USER_CREDENTIAL *userCredential, _Out_ HANDLE *token);
...
BOOL WTSIsChildSessionsEnabled(_Out_ PBOOL pbEnabled);

BOOL WTSGetChildSessionId(_Out_ PULONG pSessionId);

BOOL WINAPI WTSActiveSessionExists(_Out_ PBOOL pbActiveSessionExists);
