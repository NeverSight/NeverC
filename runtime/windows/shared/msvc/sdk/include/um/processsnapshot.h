...
// Win32 APIs.
_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssCaptureSnapshot(_In_ HANDLE ProcessHandle,
                       _In_ PSS_CAPTURE_FLAGS CaptureFlags,
                       _In_opt_ DWORD ThreadContextFlags,
                       _Out_ HPSS *SnapshotHandle);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssFreeSnapshot(_In_ HANDLE ProcessHandle, _In_ HPSS SnapshotHandle);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssQuerySnapshot(_In_ HPSS SnapshotHandle,
                     _In_ PSS_QUERY_INFORMATION_CLASS InformationClass,
                     _Out_writes_bytes_(BufferLength) void *Buffer,
                     _In_ DWORD BufferLength);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssWalkSnapshot(_In_ HPSS SnapshotHandle,
...
_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssDuplicateSnapshot(_In_ HANDLE SourceProcessHandle,
...
_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssWalkMarkerCreate(_In_opt_ PSS_ALLOCATOR const *Allocator,
                        _Out_ HPSSWALK *WalkMarkerHandle);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssWalkMarkerFree(_In_ HPSSWALK WalkMarkerHandle);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssWalkMarkerGetPosition(_In_ HPSSWALK WalkMarkerHandle,
                             _Out_ ULONG_PTR *Position);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssWalkMarkerSetPosition(_In_ HPSSWALK WalkMarkerHandle,
                             _In_ ULONG_PTR Position);

_Success_(return == ERROR_SUCCESS) STDAPI_(DWORD)
    PssWalkMarkerSeekToBeginning(_In_ HPSSWALK WalkMarkerHandle);
