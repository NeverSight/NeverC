...
#define DNS_RCLASS_UNICAST_RESPONSE                                            \
  0x0080 // Set the top-bit of the field to one, in net order!
#define DNS_RCLASS_MDNS_CACHE_FLUSH                                            \
  0x0080 // mDNS cache flush bit set on record announcement in net order!
...
DNS_STATUS
WINAPI
DnsQueryConfig(_In_ DNS_CONFIG_TYPE Config, _In_ DWORD Flag,
               _In_opt_ PCWSTR pwsAdapterName, _In_opt_ PVOID pReserved,
...
typedef VOID(WINAPI *DNS_PROXY_COMPLETION_ROUTINE)(
    _In_opt_ void *completionContext, _In_ DNS_STATUS status);
...
PDNS_RECORD
WINAPI
DnsRecordCopyEx(_In_ PDNS_RECORD pRecord, _In_ DNS_CHARSET CharSetIn,
                _In_ DNS_CHARSET CharSetOut);

PDNS_RECORD
WINAPI
DnsRecordSetCopyEx(_In_ PDNS_RECORD pRecordSet, _In_ DNS_CHARSET CharSetIn,
                   _In_ DNS_CHARSET CharSetOut);
...
BOOL WINAPI DnsRecordCompare(_In_ PDNS_RECORD pRecord1,
                             _In_ PDNS_RECORD pRecord2);

BOOL WINAPI
DnsRecordSetCompare(_Inout_ PDNS_RECORD pRR1, _Inout_ PDNS_RECORD pRR2,
...
PDNS_RECORD
DnsRecordSetDetach(_Inout_ PDNS_RECORD pRecordList);
...
VOID WINAPI DnsFree(_Pre_opt_valid_ _Frees_ptr_opt_ PVOID pData,
...
#else
VOID WINAPI DnsRecordListFree(_Inout_opt_ PDNS_RECORD pRecordList,
                              _In_ DNS_FREE_TYPE FreeType);
...
//
//  Sets pfFlat to FALSE if a record has been parsed into its corresponding
//  struct format and TRUE if it has been flat read (i.e. just a data buffer),
//  ullFlags is currently unused and exists for forwards compatibility
//

DNS_STATUS
WINAPI
DnsIsFlatRecord(_In_ PDNS_RECORD pRecord, _In_ ULONG64 ullFlags,
                _Out_ BOOL *pfFlat);
...
#define DNS_QUERY_DNSSEC_CHECKING_DISABLED                                     \
  0x02000000 // Sets DNSSEC checking disabled (CD) bit in query
#define DNS_QUERY_DNSSEC_REQUIRED                                              \
  0x04000000 // Sets DNSSEC OK (DO) bit in query AND requires that response
             // contains authenticated data (AD) bit set
...
//
//  When DNS_QUERY_PARSE_ALL_RECORDS is NOT set: the following records will be
//  parsed: DNS_TYPE_A DNS_TYPE_NS DNS_TYPE_MD DNS_TYPE_MF DNS_TYPE_CNAME
//  DNS_TYPE_SOA
//  DNS_TYPE_MB
//  DNS_TYPE_MG
//  DNS_TYPE_MR
...
//  DNS_TYPE_WINS
//  DNS_TYPE_WINSR
//  All other record types will be returned in flat format (so long as they are
//  flat read compatible) and it is the caller's responsibility to parse them if
//  needed NOTE: to get any other record types back in a parsed format (where
//  available) one must set DNS_QUERY_PARSE_ALL_RECORDS
//

//
//  When DNS_QUERY_PARSE_ALL_RECORDS is set: ONLY parsed records will be
//  returned (unknown records which cannot be parsed will not be returned) NOTE:
//  for backwards compatiblity one MUST set this flag to parse
//  DNS_TYPE_SVCB/DNS_TYPE_HTTPS, or any new record types defined in the future
//
...
DNS_STATUS
WINAPI
DnsQuery_A(_In_ PCSTR pszName, _In_ WORD wType, _In_ DWORD Options,
...
DNS_STATUS
WINAPI
DnsQuery_UTF8(_In_ PCSTR pszName, _In_ WORD wType, _In_ DWORD Options,
              _Inout_opt_ PVOID pExtra,
              _Outptr_result_maybenull_ PDNS_RECORD *ppQueryResults,
              _Outptr_opt_result_maybenull_ PVOID *pReserved);
...
DNS_STATUS
WINAPI
DnsQuery_W(_In_ PCWSTR pszName, _In_ WORD wType, _In_ DWORD Options,
           _Inout_opt_ PVOID pExtra,
           _Outptr_result_maybenull_ PDNS_RECORD *ppQueryResults,
           _Outptr_opt_result_maybenull_ PVOID *pReserved);
...
typedef VOID WINAPI DNS_QUERY_COMPLETION_ROUTINE(
    _In_ PVOID pQueryContext, _Inout_ PDNS_QUERY_RESULT pQueryResults);
...
VOID DnsFreeCustomServers(_Inout_ DWORD *pcServers,
                          _Inout_ DNS_CUSTOM_SERVER **ppServers);

DWORD
DnsGetApplicationSettings(_Out_ DWORD *pcServers,
                          _Outptr_result_buffer_(*pcServers)
                              DNS_CUSTOM_SERVER **ppDefaultServers,
                          _Out_opt_ DNS_APPLICATION_SETTINGS *pSettings);

DWORD
DnsSetApplicationSettings(_In_ DWORD cServers,
                          _In_reads_(cServers)
...
DNS_STATUS
WINAPI
DnsQueryEx(_In_ PDNS_QUERY_REQUEST pQueryRequest,
           _Inout_ PDNS_QUERY_RESULT pQueryResults,
           _Inout_opt_ PDNS_QUERY_CANCEL pCancelHandle);

DNS_STATUS
WINAPI
DnsCancelQuery(_In_ PDNS_QUERY_CANCEL pCancelHandle);
...
#endif
      _Field_size_(cCustomServers) DNS_CUSTOM_SERVER *pCustomServers;
...
typedef VOID(CALLBACK *DNS_QUERY_RAW_COMPLETION_ROUTINE)(
    _In_ VOID *queryContext, _In_ DNS_QUERY_RAW_RESULT *queryResults);
...
DNS_STATUS
WINAPI
DnsQueryRaw(_In_ DNS_QUERY_RAW_REQUEST *queryRequest,
            _Inout_ DNS_QUERY_RAW_CANCEL *cancelHandle);

DNS_STATUS
WINAPI
DnsCancelQueryRaw(_In_ DNS_QUERY_RAW_CANCEL *cancelHandle);
...
DNS_STATUS
WINAPI
DnsAcquireContextHandle_W(_In_ DWORD CredentialFlags,
                          _In_opt_ PVOID Credentials,
                          _Outptr_ PHANDLE pContext);

DNS_STATUS
WINAPI
DnsAcquireContextHandle_A(_In_ DWORD CredentialFlags,
                          _In_opt_ PVOID Credentials,
                          _Outptr_ PHANDLE pContext);
...
VOID WINAPI DnsReleaseContextHandle(_In_ HANDLE hContext);
...
DNS_STATUS
WINAPI
DnsModifyRecordsInSet_W(_In_opt_ PDNS_RECORD pAddRecords,
                        _In_opt_ PDNS_RECORD pDeleteRecords, _In_ DWORD Options,
...
DNS_STATUS
WINAPI
DnsModifyRecordsInSet_A(_In_opt_ PDNS_RECORD pAddRecords,
                        _In_opt_ PDNS_RECORD pDeleteRecords, _In_ DWORD Options,
                        _In_opt_ HANDLE hCredentials,
                        _Inout_opt_ PVOID pExtraList,
                        _Inout_opt_ PVOID pReserved);
...
DNS_STATUS
WINAPI
DnsModifyRecordsInSet_UTF8(_In_opt_ PDNS_RECORD pAddRecords,
                           _In_opt_ PDNS_RECORD pDeleteRecords,
                           _In_ DWORD Options, _In_opt_ HANDLE hCredentials,
                           _Inout_opt_ PVOID pExtraList,
                           _Inout_opt_ PVOID pReserved);
...
DNS_STATUS
WINAPI
DnsReplaceRecordSetW(_In_ PDNS_RECORD pReplaceSet, _In_ DWORD Options,
                     _In_opt_ HANDLE hContext, _Inout_opt_ PVOID pExtraInfo,
                     _Inout_opt_ PVOID pReserved);

DNS_STATUS
WINAPI
DnsReplaceRecordSetA(_In_ PDNS_RECORD pReplaceSet, _In_ DWORD Options,
                     _In_opt_ HANDLE hContext, _Inout_opt_ PVOID pExtraInfo,
                     _Inout_opt_ PVOID pReserved);

DNS_STATUS
WINAPI
DnsReplaceRecordSetUTF8(_In_ PDNS_RECORD pReplaceSet, _In_ DWORD Options,
                        _In_opt_ HANDLE hContext, _Inout_opt_ PVOID pExtraInfo,
                        _Inout_opt_ PVOID pReserved);
...
DNS_STATUS
DnsValidateName_W(_In_ PCWSTR pszName, _In_ DNS_NAME_FORMAT Format);

DNS_STATUS
DnsValidateName_A(_In_ PCSTR pszName, _In_ DNS_NAME_FORMAT Format);

DNS_STATUS
DnsValidateName_UTF8(_In_ PCSTR pszName, _In_ DNS_NAME_FORMAT Format);
...
// Winsock2.h must be included before windns.h to use the validate server
// function
#ifdef _WS2DEF_

DNS_STATUS
DnsValidateServerStatus(_In_ PSOCKADDR server, _In_opt_ PCWSTR queryName,
...
BOOL WINAPI DnsNameCompare_A(_In_ PCSTR pName1, _In_ PCSTR pName2);

BOOL WINAPI DnsNameCompare_W(_In_ PCWSTR pName1, _In_ PCWSTR pName2);
...
BOOL WINAPI DnsWriteQuestionToBuffer_W(_Inout_ PDNS_MESSAGE_BUFFER pDnsBuffer,
                                       ...
                                       _In_ PCWSTR pszName, _In_ WORD wType,
                                       _In_ WORD Xid,
                                       _In_ BOOL fRecursionDesired);

BOOL WINAPI DnsWriteQuestionToBuffer_UTF8(
    _Inout_ PDNS_MESSAGE_BUFFER pDnsBuffer, _Inout_ PDWORD pdwBufferSize,
    _In_ PCSTR pszName, _In_ WORD wType, _In_ WORD Xid,
    _In_ BOOL fRecursionDesired);

DNS_STATUS
WINAPI
DnsExtractRecordsFromMessage_W(_In_ PDNS_MESSAGE_BUFFER pDnsBuffer,
                               _In_ WORD wMessageLength,
                               _Outptr_ PDNS_RECORD *ppRecord);

DNS_STATUS
WINAPI
DnsExtractRecordsFromMessage_UTF8(_In_ PDNS_MESSAGE_BUFFER pDnsBuffer,
                                  _In_ WORD wMessageLength,
                                  _Outptr_ PDNS_RECORD *ppRecord);
...
DnsGetProxyInformation(
    _In_ PCWSTR hostName, _Inout_ DNS_PROXY_INFORMATION *proxyInformation,
...
VOID WINAPI DnsFreeProxyName(_Frees_ptr_opt_ PWSTR proxyName);
...
//
// These flags can be ORed to form the 'Flags' field of
...
DnsConnectionGetProxyInfoForHostUrl(
    ...
    _In_ DWORD dwSelectionContextLength, _In_ DWORD dwExplicitInterfaceIndex,
...
DWORD
DnsConnectionGetProxyInfo(_In_z_ PCWSTR pwszConnectionName,
                          _In_ DNS_CONNECTION_PROXY_TYPE Type,
                          _Out_ DNS_CONNECTION_PROXY_INFO *pProxyInfo);

VOID DnsConnectionFreeProxyInfo(_Inout_ DNS_CONNECTION_PROXY_INFO *pProxyInfo);

DWORD
DnsConnectionSetProxyInfo(_In_z_ PCWSTR pwszConnectionName,
                          _In_ DNS_CONNECTION_PROXY_TYPE Type,
                          _In_ const DNS_CONNECTION_PROXY_INFO *pProxyInfo);

DWORD
DnsConnectionDeleteProxyInfo(_In_z_ PCWSTR pwszConnectionName,
                             _In_ DNS_CONNECTION_PROXY_TYPE Type);

DWORD
DnsConnectionGetProxyList(_In_z_ PCWSTR pwszConnectionName,
                          _Out_ DNS_CONNECTION_PROXY_LIST *pProxyList);

VOID DnsConnectionFreeProxyList(_Inout_ DNS_CONNECTION_PROXY_LIST *pProxyList);

DWORD
DnsConnectionGetNameList(_Out_ DNS_CONNECTION_NAME_LIST *pNameList);

VOID DnsConnectionFreeNameList(_Inout_ DNS_CONNECTION_NAME_LIST *pNameList);
...
DWORD
DnsConnectionDeletePolicyEntries(_In_ DNS_CONNECTION_POLICY_TAG PolicyEntryTag);
...
WINAPI
DnsServiceConstructInstance(_In_ PCWSTR pServiceName, _In_ PCWSTR pHostName,
                            _In_opt_ PIP4_ADDRESS pIp4,
                            _In_opt_ PIP6_ADDRESS pIp6, _In_ WORD wPort,
                            _In_ WORD wPriority, _In_ WORD wWeight,
...
PDNS_SERVICE_INSTANCE
WINAPI
DnsServiceCopyInstance(_In_ PDNS_SERVICE_INSTANCE pOrig);

VOID WINAPI DnsServiceFreeInstance(_In_ PDNS_SERVICE_INSTANCE pInstance);
...
typedef VOID WINAPI DNS_SERVICE_BROWSE_CALLBACK(_In_ DWORD Status,
                                                _In_ PVOID pQueryContext,
...
DNS_STATUS
WINAPI
DnsServiceBrowse(_In_ PDNS_SERVICE_BROWSE_REQUEST pRequest,
                 _Inout_ PDNS_SERVICE_CANCEL pCancel);

DNS_STATUS
WINAPI
DnsServiceBrowseCancel(_In_ PDNS_SERVICE_CANCEL pCancelHandle);
...
typedef VOID WINAPI
DNS_SERVICE_RESOLVE_COMPLETE(_In_ DWORD Status, _In_ PVOID pQueryContext,
...
DNS_STATUS
WINAPI
DnsServiceResolve(_In_ PDNS_SERVICE_RESOLVE_REQUEST pRequest,
                  _Inout_ PDNS_SERVICE_CANCEL pCancel);

DNS_STATUS
WINAPI
DnsServiceResolveCancel(_In_ PDNS_SERVICE_CANCEL pCancelHandle);
...
typedef VOID WINAPI
DNS_SERVICE_REGISTER_COMPLETE(_In_ DWORD Status, _In_ PVOID pQueryContext,
                              _In_ PDNS_SERVICE_INSTANCE pInstance);
...
DWORD
WINAPI
DnsServiceRegister(_In_ PDNS_SERVICE_REGISTER_REQUEST pRequest,
                   _Inout_opt_ PDNS_SERVICE_CANCEL pCancel);

DWORD
WINAPI
DnsServiceDeRegister(_In_ PDNS_SERVICE_REGISTER_REQUEST pRequest,
                     _Inout_opt_ PDNS_SERVICE_CANCEL pCancel);

DWORD
WINAPI
DnsServiceRegisterCancel(_In_ PDNS_SERVICE_CANCEL pCancelHandle);
...
typedef VOID WINAPI MDNS_QUERY_CALLBACK(
    _In_ PVOID pQueryContext, _Inout_ PMDNS_QUERY_HANDLE pQueryHandle,
    _Inout_ PDNS_QUERY_RESULT pQueryResults);
...
DNS_STATUS WINAPI DnsStartMulticastQuery(_In_ PMDNS_QUERY_REQUEST pQueryRequest,
                                         _Inout_ PMDNS_QUERY_HANDLE pHandle);

DNS_STATUS WINAPI DnsStopMulticastQuery(_Inout_ PMDNS_QUERY_HANDLE pHandle);
...
//
// Note: Below API is not yet library linkable and hence need to be loaded using
// GetProcAddress.
//

#if (NTDDI_VERSION >= NTDDI_WIN11_GE)
BOOL WINAPI DnsIsZtEnabled(VOID);
