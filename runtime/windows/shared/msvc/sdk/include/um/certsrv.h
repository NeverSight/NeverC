...
#define DBFLAGS_ENABLEVOLATILEREQUESTS                                         \
  0X00000800 // enables the use of CCertDBMem

#define DBFLAGS_DEFAULT                                                        \
  (DBFLAGS_MAXCACHESIZEX100 | DBFLAGS_CHECKPOINTDEPTH60MB |                    \
...
// This contains a list of UNC or local directories, each containing a selection
// of EK public keys that will be explicitly trusted for TPM attestation
...
#define wszREGEKUOIDSFORPUBLISHEXPIREDCERTINCRL                                \
  TEXT("EKUOIDsForPublishExpiredCertInCRL")

#define wszzDEFAULTEKUOIDSFORPUBLISHEXPIREDCERTINCRL                           \
  TEXT(szOID_PKIX_KP_CODE_SIGNING)                                             \
  L"\0" TEXT(szOID_KP_KERNEL_MODE_CODE_SIGNING) L"\0"
...
#define IF_DEFAULT                                                             \
  (IF_NOREMOTEICERTADMINBACKUP | IF_LOCKICERTREQUEST |                         \
...
// * If a value is present, a basic ASN1 compliance check must be performed on
//   policy module start-up. If the compliance of OIDs fail validation, the
//   policy module must fail to initialize.
#define wszREGEKUOIDSFORVOLATILEREQUESTS TEXT("EKUOIDsforVolatileRequests")

// Following values would be used to configure the ldap session during setup of
// connection to DC from policy module
...
// #define wszINFKEY_ALTERNATESIGNATUREALGORITHM
...
#define wszREGKEYGROUPPOLICYENROLLMENT                                         \
...
#define wszREGKEYDISALLOWEDSCEPALGS                                            \
  wszREGKEYENROLLMENT L"\\DisallowedSCEPAlgorithms"
...
// Set low bit to 0 (or if reg value is missing) to use default behavior: user
// context GP lookup Only applies to InitializeFromTemplateName methods using
// default policy & only from user context. REG_DWORD <Internal template Name>
...
//==========================================================================================
// (Un)Installation of each ADCS role create a sub key with names
// (CertificateAuthority, WebEnrollment, CEP, CES, NDES) and then adds the
// "ConfigurationStatus" reg value to indicate whether a particular role has
// been configured (2), not configured (1) or failed (0). If customers require a
// different EP setting than default value, they should create
// "EPTokenCheckValue" reg value directly under the ""..\\ADCS" parent key
// before starting configuration of the given role. This should be created on
// the machine where the given role is being configured and not on the machine
// where CA role is configured.
...
#define wszPROPREQUESTERNAMEFROMOLDCERTIFICATE                                 \
  TEXT("RequesterNameFromOldCertificate")
