...
#define MARSHALINTERFACE_MIN                                                   \
  500 // minimum number of bytes for interface marshal
...
WINOLEAPI CoCreateStandardMalloc(_In_ DWORD memctx,
                                 _Outptr_ IMalloc FAR * FAR * ppMalloc);

#if (_WIN32_WINNT >= 0x0501)

_Check_return_
    WINOLEAPI CoRegisterInitializeSpy(_In_ IInitializeSpy *pSpy,
                                      _Out_ ULARGE_INTEGER *puliCookie);
...
CoLoadLibrary(_In_ LPOLESTR lpszLibName, _In_ BOOL bAutoFree);
...
_Check_return_ WINOLEAPI CoGetInstanceFromFile(
    _In_opt_ COSERVERINFO *pServerInfo, _In_opt_ CLSID *pClsid,
    _In_opt_ IUnknown *punkOuter, // only relevant locally
    _In_ DWORD dwClsCtx, _In_ DWORD grfMode,
    _In_ _Null_terminated_ OLECHAR *pwszName, _In_ DWORD dwCount,
    _Inout_updates_(dwCount) MULTI_QI *pResults);

_Check_return_ WINOLEAPI CoGetInstanceFromIStorage(
    _In_opt_ COSERVERINFO *pServerInfo, _In_opt_ CLSID *pClsid,
    _In_opt_ IUnknown *punkOuter, // only relevant locally
    _In_ DWORD dwClsCtx, _In_ struct IStorage *pstg, _In_ DWORD dwCount,
    _Inout_updates_(dwCount) MULTI_QI *pResults);
...
WINOLEAPI CoAllowSetForegroundWindow(_In_ IUnknown *pUnk,
                                     _In_opt_ LPVOID lpvReserved);

WINOLEAPI DcomChannelSetHResult(_In_opt_ LPVOID pvReserved,
...
WINOLEAPI_(BOOL)
CoFileTimeToDosDateTime(_In_ FILETIME FAR *lpFileTime, _Out_ LPWORD lpDosDate,
                        _Out_ LPWORD lpDosTime);
WINOLEAPI_(BOOL)
CoDosDateTimeToFileTime(_In_ WORD nDosDate, _In_ WORD nDosTime,
                        _Out_ FILETIME FAR *lpFileTime);
WINOLEAPI CoFileTimeNow(_Out_ FILETIME FAR *lpFileTime);

_Check_return_ WINOLEAPI CoRegisterMessageFilter(
    _In_opt_ LPMESSAGEFILTER lpMessageFilter,
    _Outptr_opt_result_maybenull_ LPMESSAGEFILTER FAR *lplpMessageFilter);

#if (_WIN32_WINNT >= 0x0400) || defined(_WIN32_DCOM) // DCOM

WINOLEAPI CoRegisterChannelHook(_In_ REFGUID ExtensionUuid,
                                _In_ IChannelHook *pChannelHook);
#endif // DCOM

/* TreatAs APIS */

_Check_return_ WINOLEAPI CoTreatAsClass(_In_ REFCLSID clsidOld,
...
_Check_return_ WINOLEAPI StgOpenAsyncDocfileOnIFillLockBytes(
    _In_ IFillLockBytes *pflb, _In_ DWORD grfMode, _In_ DWORD asyncFlags,
    _Outptr_ IStorage **ppstgOpen);

_Check_return_ WINOLEAPI StgGetIFillLockBytesOnILockBytes(
    _In_ ILockBytes *pilb, _Outptr_ IFillLockBytes **ppflb);

_Check_return_ WINOLEAPI
StgGetIFillLockBytesOnFile(_In_ _Null_terminated_ OLECHAR const *pwcsName,
                           _Outptr_ IFillLockBytes **ppflb);

_Check_return_ WINOLEAPI StgOpenLayoutDocfile(
    _In_ _Null_terminated_ OLECHAR const *pwcsDfName, _In_ DWORD grfMode,
    _In_ DWORD reserved, _Outptr_ IStorage **ppstgOpen);
...
WINOLEAPI CoInstall(_In_ IBindCtx *pbc, _In_ DWORD dwFlags,
                    _In_ uCLSSPEC *pClassSpec, _In_ QUERYCONTEXT *pQuery,
...
_Check_return_ WINOLEAPI BindMoniker(_In_ LPMONIKER pmk, _In_ DWORD grfOpt,
                                     _In_ REFIID iidResult,
                                     _Outptr_ LPVOID FAR *ppvResult);

_Check_return_ WINOLEAPI CoGetObject(_In_ LPCWSTR pszName,
                                     _In_opt_ BIND_OPTS *pBindOptions,
                                     _In_ REFIID riid, _Outptr_ void **ppv);
_Check_return_ WINOLEAPI MkParseDisplayName(_In_ LPBC pbc,
                                            _In_ LPCOLESTR szUserName,
                                            _Out_ ULONG FAR *pchEaten,
                                            _Outptr_ LPMONIKER FAR *ppmk);
_Check_return_ WINOLEAPI MonikerRelativePathTo(
    _In_ LPMONIKER pmkSrc, _In_ LPMONIKER pmkDest,
    _Outptr_ LPMONIKER FAR *ppmkRelPath, _In_ BOOL dwReserved);
_Check_return_ WINOLEAPI
MonikerCommonPrefixWith(_In_ LPMONIKER pmkThis, _In_ LPMONIKER pmkOther,
                        _Outptr_ LPMONIKER FAR *ppmkCommon);
_Check_return_ WINOLEAPI CreateBindCtx(_In_ DWORD reserved,
                                       _Outptr_ LPBC FAR *ppbc);
_Check_return_ WINOLEAPI
CreateGenericComposite(_In_opt_ LPMONIKER pmkFirst, _In_opt_ LPMONIKER pmkRest,
                       _Outptr_ LPMONIKER FAR *ppmkComposite);
_Check_return_ WINOLEAPI GetClassFile(_In_ LPCOLESTR szFilename,
                                      _Out_ CLSID FAR *pclsid);

_Check_return_ WINOLEAPI CreateClassMoniker(_In_ REFCLSID rclsid,
                                            _Outptr_ LPMONIKER FAR *ppmk);

_Check_return_ WINOLEAPI CreateFileMoniker(_In_ LPCOLESTR lpszPathName,
                                           _Outptr_ LPMONIKER FAR *ppmk);

_Check_return_ WINOLEAPI CreateItemMoniker(_In_ LPCOLESTR lpszDelim,
                                           _In_ LPCOLESTR lpszItem,
                                           _Outptr_ LPMONIKER FAR *ppmk);
_Check_return_ WINOLEAPI CreateAntiMoniker(_Outptr_ LPMONIKER FAR *ppmk);
_Check_return_ WINOLEAPI CreatePointerMoniker(_In_opt_ LPUNKNOWN punk,
                                              _Outptr_ LPMONIKER FAR *ppmk);
_Check_return_ WINOLEAPI CreateObjrefMoniker(_In_opt_ LPUNKNOWN punk,
                                             _Outptr_ LPMONIKER FAR *ppmk);
...
_Check_return_
    WINOLEAPI GetRunningObjectTable(_In_ DWORD reserved,
                                    _Outptr_ LPRUNNINGOBJECTTABLE FAR *pprot);
