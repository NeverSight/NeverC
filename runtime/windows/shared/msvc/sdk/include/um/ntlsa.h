...
// present in NTIFS prior to Windows Server 2003 SP1. All of the definitions
// however exist down to windows 2000 (except for the few exceptions noted in
// the code).
//
...
typedef enum _SE_ADT_PARAMETER_TYPE {
  ...
  SeAdtParmTypeLogonIdAsSid, // Produces 4 parameters.
                             ...
                             // Results in:
                             //
                             //  param 1: Sid string (based on SID and not
                             //  derived from the LUID) param 2: - param 3: -
                             //  param 4: -
  ...
  SeAdtParmTypeMultiSzString, // Produces 1 parameter
                              ...
                              // Results in:
                              //
                              //  Unicode string with each null replaced with
                              //  /r/n
...
typedef struct _SE_ADT_ACCESS_REASON {
  ...
  ULONG AccessGranted;
  PSECURITY_DESCRIPTOR SecurityDescriptor; // multple SDs may be stored here in
                                           // self-relative way.
...
typedef struct _SE_ADT_CLAIMS {
  ...
  ULONG Length;
  PCLAIMS_BLOB
  Claims; // one claim blob will be stored here in self-relative way
...
// a) Add new sub-category at the end of a category in
// POLICY_AUDIT_EVENT_TYPE_EX b) Define new GUID (next guid ID in sequence) and
// add it to adtapi.w c) Add new sub-category to LsapAdtGuidToIdMapping in
// adtapi.c. Make sure GUID is added in
//        sequential order
// d) Update per-category count in ntrmlsa.w
// e) Add textual description for the new sub-category to msobjs.mc
// f) Add a task for this subcategory in MsAuditEvtLog.man. The task ID should
// be the same as the
//    SubCategory ID defined in step a).
// g) Update LSAP_ADT_START_CATEGORY_ID_IN_MSOBJS and/or
// LSAP_ADT_START_SUB_CATEGORY_ID_IN_MSOBJS
//        if necessary.
// h) Update the subcategory count in base\published\ntseapi_x.w
// (POLICY_AUDIT_SUBCATEGORY_COUNT). This
//    is necessary because base\wow64 compilation fails if we ask the compiler
//    to compute the count via (((ULONG) AuditSubCategoryMaxType -
//    AuditSubCategoryMinType + 1))
...
} POLICY_AUDIT_EVENT_TYPE_EX,
    *PPOLICY_AUDIT_EVENT_TYPE_EX;
...
_IRQL_requires_same_ _IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS NTAPI
...
_IRQL_requires_same_ _IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS NTAPI
    LsaLogonUser(_In_ HANDLE LsaHandle, _In_ PLSA_STRING OriginName,
                 ...
                 _Out_ PULONG ProfileBufferLength, _Inout_ PLUID LogonId,
...
__drv_sameIRQL NTSTATUS LsaInsertProtectedProcessAddress(
    __in PVOID BufferAddress, __in ULONG BufferSize);

__drv_sameIRQL NTSTATUS LsaRemoveProtectedProcessAddress(
    __in PVOID BufferAddress, __in ULONG BufferSize);
...
typedef NTSTATUS(NTAPI *PFN_LSA_CALL_AUTH_PKG)(
    _In_ HANDLE LsaHandle, _In_ ULONG AuthenticationPackage,
    _In_reads_bytes_(SubmitBufferLength) PVOID ProtocolSubmitBuffer,
    _In_ ULONG SubmitBufferLength, _Out_opt_ PVOID *ProtocolReturnBuffer,
    _Out_opt_ PULONG ReturnBufferLength, _Out_opt_ PNTSTATUS ProtocolStatus);

typedef NTSTATUS(NTAPI *PFN_LSA_DEREGISTER_PROC)(_In_ HANDLE LsaHandle);

typedef NTSTATUS(NTAPI *PFN_LSA_FREE_BUFFER)(_In_ PVOID Buffer);
...
typedef NTSTATUS(NTAPI *PFN_LSA_LOGON_USER)(
    ...
    _Out_ PLUID LogonId, _Out_ PHANDLE Token, _Out_ PQUOTA_LIMITS Quotas,
    _Out_ PNTSTATUS SubStatus);
...
typedef NTSTATUS(NTAPI *PFN_LSA_REGISTER_PROC)(
    _In_ PLSA_STRING LogonProcessName, _Out_ PHANDLE LsaHandle,
    _Out_ PLSA_OPERATIONAL_MODE SecurityMode);

typedef NTSTATUS(NTAPI *PFN_LSA_LOGON_SMA)(_In_ LUID LogonId,
                                           _Out_ PHANDLE Token);
...
#define POLICY_MODE_ALL                                                        \
  (POLICY_MODE_INTERACTIVE | POLICY_MODE_NETWORK | POLICY_MODE_BATCH |         \
   POLICY_MODE_SERVICE | POLICY_MODE_PROXY | POLICY_MODE_DENY_INTERACTIVE |    \
...
#define POLICY_MODE_ALL_NT4                                                    \
  (POLICY_MODE_INTERACTIVE | POLICY_MODE_NETWORK | POLICY_MODE_BATCH |         \
   POLICY_MODE_SERVICE | POLICY_MODE_PROXY)
...
// This flag returns the internet names. Otherwise the NT4 style name eg.
// domain\username will be returned. The exception is if the MSA internet SID is
// specified then the internet name will be returned unless
...
//
// The following structure type corresponds to the
...
#if (_WIN32_WINNT < 0x0502)
//
// QualityOfService information.  Corresponds to
...
// where the members have the following usage:
//
//  QualityOfService - Determines what specific QOS actions a machine should
//  take
//
#endif
...
// PolicyDomainKerberosTicketInformation information class
//
...
//
// The following data type defines the classes of Policy Information / Policy
// Domain Information that may be used to request notification
//
...
#define ACCOUNT_ALL_ACCESS                                                     \
  (STANDARD_RIGHTS_REQUIRED | ACCOUNT_VIEW | ACCOUNT_ADJUST_PRIVILEGES |       \
...
#define ACCOUNT_WRITE                                                          \
  (STANDARD_RIGHTS_WRITE | ACCOUNT_ADJUST_PRIVILEGES | ACCOUNT_ADJUST_QUOTAS | \
...
#define TRUSTED_ALL_ACCESS                                                     \
  (STANDARD_RIGHTS_REQUIRED | TRUSTED_QUERY_DOMAIN_NAME |                      \
   TRUSTED_QUERY_CONTROLLERS | TRUSTED_SET_CONTROLLERS | TRUSTED_QUERY_POSIX | \
   TRUSTED_SET_POSIX | TRUSTED_SET_AUTH | TRUSTED_QUERY_AUTH)

#define TRUSTED_READ (STANDARD_RIGHTS_READ | TRUSTED_QUERY_DOMAIN_NAME)

#define TRUSTED_WRITE                                                          \
  (STANDARD_RIGHTS_WRITE | TRUSTED_SET_CONTROLLERS | TRUSTED_SET_POSIX |       \
   TRUSTED_SET_AUTH)
...
#define TRUST_ATTRIBUTE_UPLEVEL_ONLY                                           \
  0x00000002 // Trust link only valid for uplevel client
#if (_WIN32_WINNT == 0x0500)
...
#else
#define TRUST_ATTRIBUTE_QUARANTINED_DOMAIN                                     \
  0x00000004 // Used to quarantine domains
#endif

#if (_WIN32_WINNT >= 0x0501)
#define TRUST_ATTRIBUTE_FOREST_TRANSITIVE                                      \
  0x00000008 // This link may contain forest trust information
#if (_WIN32_WINNT >= 0x0502)
#define TRUST_ATTRIBUTE_CROSS_ORGANIZATION                                     \
  0x00000010 // This trust is to a domain/forest which is not part of this
             // enterprise
#define TRUST_ATTRIBUTE_WITHIN_FOREST                                          \
  0x00000020 // Trust is internal to this forest
#define TRUST_ATTRIBUTE_TREAT_AS_EXTERNAL                                      \
  0x00000040 // Trust is to be treated as external for trust boundary purposes
#if (_WIN32_WINNT >= 0x0600)
#define TRUST_ATTRIBUTE_TRUST_USES_RC4_ENCRYPTION                              \
  0x00000080 // MIT trust with RC4
#define TRUST_ATTRIBUTE_TRUST_USES_AES_KEYS                                    \
  0x00000100 // Use AES keys to encrypte KRB TGTs
#endif
#if (_WIN32_WINNT >= 0x0602)
#define TRUST_ATTRIBUTE_CROSS_ORGANIZATION_NO_TGT_DELEGATION                   \
  0x00000200 // do not forward TGT to the other side of the trust which is not
             // part of this enterprise
#define TRUST_ATTRIBUTE_PIM_TRUST 0x00000400 // Outgoing trust to a PIM forest.
#endif

#if (_WIN32_WINNT >= 0x0603)
// Forward the TGT to the other side of the trust which is not part of this
// enterprise This flag has the opposite meaning of
// TRUST_ATTRIBUTE_CROSS_ORGANIZATION_NO_TGT_DELEGATION which is now deprecated.
// Note: setting TRUST_ATTRIBUTE_CROSS_ORGANIZATION_ENABLE_TGT_DELEGATION is not
// recommended from a security standpoint.
#define TRUST_ATTRIBUTE_CROSS_ORGANIZATION_ENABLE_TGT_DELEGATION 0x00000800
#endif

// Disables authentication target validation for all NTLM pass-through
// authentication requests using this trust.
...
#endif
// Trust attributes 0x00400000 through 0x00800000 were used previously (up to
// W2K) and should not be re-used Trust attributes 0x01000000 through 0x80000000
// are reserved for user
...
#if (_WIN32_WINNT >= 0x0502)
//
//  To prevent huge data to be passed in, we should put a limit on
//  LSA_FOREST_TRUST_BINARY_DATA.
//      128K is large enough that can't be reached in the near future, and small
//      enough not to cause memory problems.
...
NTSTATUS
NTAPI
LsaDelete(_In_ LSA_HANDLE ObjectHandle);

NTSTATUS
NTAPI
LsaQuerySecurityObject(_In_ LSA_HANDLE ObjectHandle,
                       _In_ SECURITY_INFORMATION SecurityInformation,
                       _Out_ PSECURITY_DESCRIPTOR *SecurityDescriptor);

NTSTATUS
NTAPI
LsaSetSecurityObject(_In_ LSA_HANDLE ObjectHandle,
                     _In_ SECURITY_INFORMATION SecurityInformation,
                     _In_ PSECURITY_DESCRIPTOR SecurityDescriptor);

NTSTATUS
NTAPI
LsaChangePassword(_In_ PLSA_UNICODE_STRING ServerName,
...
NTSTATUS
NTAPI
LsaOpenPolicy(_In_opt_ PLSA_UNICODE_STRING SystemName,
...
NTSTATUS
NTAPI
LsaOpenPolicySce(_In_opt_ PLSA_UNICODE_STRING SystemName,
                 _In_ PLSA_OBJECT_ATTRIBUTES ObjectAttributes,
...
//
// WARNING !
// Keep the fields of the following struct in sync with
// LSAPR_CENTRAL_ACCESS_POLICY_ENTRY struct in lsarpc.idl
//
//
typedef struct _CENTRAL_ACCESS_POLICY_ENTRY {
  ...
  _Field_size_bytes_(
      LengthAppliesTo) PUCHAR AppliesTo; // Must satisfy to enforce CAPE
  ULONG LengthSD;
...
// Central Access Policies are associated with a CAP by a CAP ID placed in the
// SACL of the Resource. Applicable CAPEs of a CAP are enforced during access
// check.
//
// WARNING !
// Keep the fields of the following struct in sync with
// LSAPR_CENTRAL_ACCESS_POLICY struct in lsarpc.idl
//
//
...
NTSTATUS
NTAPI
LsaSetCAPs(_In_reads_opt_(CAPDNCount) PLSA_UNICODE_STRING CAPDNs,
           _In_ ULONG CAPDNCount, _In_ ULONG Flags);

NTSTATUS
NTAPI
LsaGetAppliedCAPIDs(_In_opt_ PLSA_UNICODE_STRING SystemName,
                    _Outptr_result_buffer_(*CAPIDCount) PSID **CAPIDs,
                    _Out_ PULONG CAPIDCount);

NTSTATUS
NTAPI
LsaQueryCAPs(_In_reads_opt_(CAPIDCount) PSID *CAPIDs, _In_ ULONG CAPIDCount,
...
NTSTATUS
NTAPI
LsaClearAuditLog(_In_ LSA_HANDLE PolicyHandle);

NTSTATUS
NTAPI
LsaCreateAccount(_In_ LSA_HANDLE PolicyHandle, _In_ PSID AccountSid,
                 _In_ ACCESS_MASK DesiredAccess,
                 _Out_ PLSA_HANDLE AccountHandle);

NTSTATUS
NTAPI
LsaEnumerateAccounts(_In_ LSA_HANDLE PolicyHandle,
                     _Inout_ PLSA_ENUMERATION_HANDLE EnumerationContext,
                     _Out_ PVOID *Buffer, _In_ ULONG PreferedMaximumLength,
                     _Out_ PULONG CountReturned);

NTSTATUS
NTAPI
LsaCreateTrustedDomain(_In_ LSA_HANDLE PolicyHandle,
...
NTSTATUS
NTAPI
LsaEnumerateTrustedDomains(_In_ LSA_HANDLE PolicyHandle,
                           _Inout_ PLSA_ENUMERATION_HANDLE EnumerationContext,
                           _Out_ PVOID *Buffer,
...
NTSTATUS
NTAPI
LsaEnumeratePrivileges(_In_ LSA_HANDLE PolicyHandle,
                       _Inout_ PLSA_ENUMERATION_HANDLE EnumerationContext,
                       _Out_ PVOID *Buffer, _In_ ULONG PreferedMaximumLength,
                       _Out_ PULONG CountReturned);
...
NTSTATUS
NTAPI
LsaLookupNames2(_In_ LSA_HANDLE PolicyHandle,
...
NTSTATUS
NTAPI
LsaLookupSids2(_In_ LSA_HANDLE PolicyHandle, _In_ ULONG LookupOptions,
               _In_ ULONG Count, _In_ PSID *Sids,
               _Out_ PLSA_REFERENCED_DOMAIN_LIST *ReferencedDomains,
               _Out_ PLSA_TRANSLATED_NAME *Names);
...
NTSTATUS
NTAPI
LsaCreateSecret(_In_ LSA_HANDLE PolicyHandle,
                _In_ PLSA_UNICODE_STRING SecretName,
                _In_ ACCESS_MASK DesiredAccess, _Out_ PLSA_HANDLE SecretHandle);
...
NTSTATUS
NTAPI
LsaIsCredentialGuardRunning(_Out_ PBOOLEAN IsCredentialGuardRunning);
...
NTSTATUS
NTAPI
LsaOpenAccount(_In_ LSA_HANDLE PolicyHandle, _In_ PSID AccountSid,
               _In_ ACCESS_MASK DesiredAccess, _Out_ PLSA_HANDLE AccountHandle);

NTSTATUS
NTAPI
LsaEnumeratePrivilegesOfAccount(_In_ LSA_HANDLE AccountHandle,
                                _Out_ PPRIVILEGE_SET *Privileges);

NTSTATUS
NTAPI
LsaAddPrivilegesToAccount(_In_ LSA_HANDLE AccountHandle,
                          _In_ PPRIVILEGE_SET Privileges);

NTSTATUS
NTAPI
LsaRemovePrivilegesFromAccount(_In_ LSA_HANDLE AccountHandle,
                               _In_ BOOLEAN AllPrivileges,
                               _In_opt_ PPRIVILEGE_SET Privileges);

NTSTATUS
NTAPI
LsaGetQuotasForAccount(_In_ LSA_HANDLE AccountHandle,
                       _Out_ PQUOTA_LIMITS QuotaLimits);

NTSTATUS
NTAPI
LsaSetQuotasForAccount(_In_ LSA_HANDLE AccountHandle,
                       _In_ PQUOTA_LIMITS QuotaLimits);

NTSTATUS
NTAPI
LsaGetSystemAccessAccount(_In_ LSA_HANDLE AccountHandle,
                          _Out_ PULONG SystemAccess);

NTSTATUS
NTAPI
LsaSetSystemAccessAccount(_In_ LSA_HANDLE AccountHandle,
...
NTSTATUS
NTAPI
LsaQueryLocalSystemAccess(_In_ PSID AccountSid, _Out_ PULONG SystemAccessLocal);

NTSTATUS
NTAPI
LsaQueryLocalSystemAccessAll(
    _Outptr_result_nullonfailure_ PLSA_LOCAL_ACCESSRIGHT_ASSIGNMENTS
        *ppLocalAccessRightAssignments);
...
NTSTATUS
NTAPI
LsaOpenTrustedDomain(_In_ LSA_HANDLE PolicyHandle, _In_ PSID TrustedDomainSid,
                     _In_ ACCESS_MASK DesiredAccess,
                     _Out_ PLSA_HANDLE TrustedDomainHandle);

NTSTATUS
NTAPI
LsaQueryInfoTrustedDomain(_In_ LSA_HANDLE TrustedDomainHandle,
                          _In_ TRUSTED_INFORMATION_CLASS InformationClass,
                          _Out_ PVOID *Buffer);

NTSTATUS
NTAPI
LsaSetInformationTrustedDomain(_In_ LSA_HANDLE TrustedDomainHandle,
                               _In_ TRUSTED_INFORMATION_CLASS InformationClass,
                               _In_ PVOID Buffer);
...
NTSTATUS
NTAPI
LsaOpenSecret(_In_ LSA_HANDLE PolicyHandle, _In_ PLSA_UNICODE_STRING SecretName,
              _In_ ACCESS_MASK DesiredAccess, _Out_ PLSA_HANDLE SecretHandle);

NTSTATUS
NTAPI
LsaSetSecret(_In_ LSA_HANDLE SecretHandle,
             _In_opt_ PLSA_UNICODE_STRING CurrentValue,
             _In_opt_ PLSA_UNICODE_STRING OldValue);

NTSTATUS
NTAPI
LsaQuerySecret(_In_ LSA_HANDLE SecretHandle,
...
NTSTATUS
NTAPI
LsaLookupPrivilegeValue(_In_ LSA_HANDLE PolicyHandle,
                        _In_ PLSA_UNICODE_STRING Name, _Out_ PLUID Value);

NTSTATUS
NTAPI
LsaLookupPrivilegeName(_In_ LSA_HANDLE PolicyHandle, _In_ PLUID Value,
                       _Out_ PLSA_UNICODE_STRING *Name);

NTSTATUS
NTAPI
LsaLookupPrivilegeDisplayName(_In_ LSA_HANDLE PolicyHandle,
...
NTSTATUS
LsaGetUserName(_Outptr_ PLSA_UNICODE_STRING *UserName,
               _Outptr_opt_ PLSA_UNICODE_STRING *DomainName);

NTSTATUS
LsaGetRemoteUserName(_In_opt_ PLSA_UNICODE_STRING SystemName,
...
LsaEnumerateAccountRights(_In_ LSA_HANDLE PolicyHandle, _In_ PSID AccountSid,
                          _Outptr_result_buffer_(*CountOfRights)
...
NTSTATUS
NTAPI
LsaEnumerateTrustedDomainsEx(_In_ LSA_HANDLE PolicyHandle,
                             _Inout_ PLSA_ENUMERATION_HANDLE EnumerationContext,
                             _Out_ PVOID *Buffer,
                             _In_ ULONG PreferedMaximumLength,
                             _Out_ PULONG CountReturned);
...
LsaSetForestTrustInformation2(
    _In_ LSA_HANDLE PolicyHandle, _In_ PLSA_UNICODE_STRING TrustedDomainName,
    _In_ LSA_FOREST_TRUST_RECORD_TYPE HighestRecordType,
    _In_ PLSA_FOREST_TRUST_INFORMATION2 ForestTrustInfo, _In_ BOOLEAN CheckOnly,
    _Out_ PLSA_FOREST_TRUST_COLLISION_INFORMATION *CollisionInfo);

// end_ntsecapi

NTSTATUS
LsaInvokeTrustScanner(_In_ LSA_HANDLE PolicyHandle, _In_opt_ LPWSTR DomainName,
                      _In_ ULONG Flags, _In_opt_ LPWSTR CompletionEvent);
...
NTSTATUS
NTAPI
LsaSetPolicyReplicationHandle(_Inout_ PLSA_HANDLE PolicyHandle);
...
NTSTATUS
NTAPI
LsaGetDeviceRegistrationInfo(_Out_ PLSA_REGISTRATION_INFO *RegistrationInfo);
...
} LSA_CREDENTIAL_KEY_SOURCE_TYPE,
    *PLSA_CREDENTIAL_KEY_SOURCE_TYPE;
...
NTSTATUS
SeciIsProtectedUser(__out PBOOLEAN ProtectedUser);
...
#define LSAD_AES256_ENC_KEY_STRING                                             \
  "Microsoft LSAD encryption key AEAD-AES-256-CBC-HMAC-SHA512 16"
#define LSAD_AES256_MAC_KEY_STRING                                             \
