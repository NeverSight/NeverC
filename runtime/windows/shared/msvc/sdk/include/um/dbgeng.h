...
DEFINE_GUID(IID_IDebugOutputCallbacks2, 0x67721fe9, 0x56d2, 0x4a44, 0xa3, 0x25,
            0x2b, 0x65, 0x51, 0x3c, 0xe6, 0xeb);
...
STDAPI
DebugConnect(_In_ PCSTR RemoteOptions, _In_ REFIID InterfaceId,
             _Out_ PVOID *Interface);

STDAPI
DebugConnectWide(_In_ PCWSTR RemoteOptions, _In_ REFIID InterfaceId,
                 _Out_ PVOID *Interface);

STDAPI
DebugCreate(_In_ REFIID InterfaceId, _Out_ PVOID *Interface);

STDAPI
DebugCreateEx(_In_ REFIID InterfaceId, _In_ DWORD DbgEngOptions,
              _Out_ PVOID *Interface);
...
DECLARE_INTERFACE_(IDebugAdvanced, IUnknown) {
  // IUnknown.
  STDMETHOD(QueryInterface)(THIS_ _In_ REFIID InterfaceId,
                            _Out_ PVOID * Interface) PURE;
  ...
  STDMETHOD(GetThreadContext)(
      THIS_ _Out_writes_bytes_(ContextSize) /* align_is(16) */ PVOID Context,
      _In_ ULONG ContextSize) PURE;
  STDMETHOD(SetThreadContext)(
      THIS_ _In_reads_bytes_(ContextSize) /* align_is(16) */ PVOID Context,
      _In_ ULONG ContextSize) PURE;
};
...
typedef struct _PROCESS_NAME_ENTRY {
  ...
  ULONG NameSize; // ProcessName will always be NULL terminated, NameSize is for
                  // struct align and safeguard.
...
// request code OutBuffer - NtQueryInformationToken() return
...
// InBuffer - Unused
// OutBuffer - Unused
// return - S_OK if non-invasive user-mode attach, S_FALSE if not (but still
// live user-mode), E_FAIL otherwise.
...
// InBuffer - LONG32 - 0:query current state; >0:enable inline queries; <0:
// disable inline queries OutBuffer - Unused return - S_OK: inline queries are
// enabled; S_FALSE: inline queries are disabled; others: errors.
...
DECLARE_INTERFACE_(IDebugAdvanced2, IUnknown) {
  ...
  STDMETHOD(Request)(THIS_ _In_ ULONG Request,
                     _In_reads_bytes_opt_(InBufferSize) PVOID InBuffer,
                     _In_ ULONG InBufferSize,
                     _Out_writes_bytes_opt_(OutBufferSize) PVOID OutBuffer,
                     _In_ ULONG OutBufferSize, _Out_opt_ PULONG OutSize) PURE;

  STDMETHOD(GetSourceFileInformation)(
      THIS_ _In_ ULONG Which, _In_ PSTR SourceFile, _In_ ULONG64 Arg64,
      _In_ ULONG Arg32, _Out_writes_bytes_opt_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG InfoSize) PURE;
  STDMETHOD(FindSourceFileAndToken)(
      THIS_ _In_ ULONG StartElement, _In_ ULONG64 ModAddr, _In_ PCSTR File,
      _In_ ULONG Flags, _In_reads_bytes_opt_(FileTokenSize) PVOID FileToken,
      _In_ ULONG FileTokenSize, _Out_opt_ PULONG FoundElement,
      _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG FoundSize) PURE;

  STDMETHOD(GetSymbolInformation)(
      THIS_ _In_ ULONG Which, _In_ ULONG64 Arg64, _In_ ULONG Arg32,
      ...
      _In_ ULONG StringBufferSize, _Out_opt_ PULONG StringSize) PURE;

  STDMETHOD(GetSystemObjectInformation)(
      THIS_ _In_ ULONG Which, _In_ ULONG64 Arg64, _In_ ULONG Arg32,
      _Out_writes_bytes_opt_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG InfoSize) PURE;
};
...
DECLARE_INTERFACE_(IDebugAdvanced3, IUnknown) {
  ...
  STDMETHOD(GetSourceFileInformationWide)(
      THIS_ _In_ ULONG Which, _In_ PWSTR SourceFile, _In_ ULONG64 Arg64,
      _In_ ULONG Arg32, _Out_writes_bytes_opt_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG InfoSize) PURE;
  STDMETHOD(FindSourceFileAndTokenWide)(
      THIS_ _In_ ULONG StartElement, _In_ ULONG64 ModAddr, _In_ PCWSTR File,
      _In_ ULONG Flags, _In_reads_bytes_opt_(FileTokenSize) PVOID FileToken,
      _In_ ULONG FileTokenSize, _Out_opt_ PULONG FoundElement,
      _Out_writes_opt_(BufferSize) PWSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG FoundSize) PURE;
...
typedef struct _SYMBOL_INFO_EX {
  ...
  ULONG TypeOfInfo;   // Type of the symbol information stored (Ex.
                      // DEBUG_SYMINFO_BREAKPOINT_SOURCE_LINE)
  ULONG64 Offset;     // Address of the line.
  ULONG Line; // First line number that does not correspond to a compiler added
              // glue lines.
  ULONG Displacement; // Line displacement: Offset between given address and the
...
DECLARE_INTERFACE_(IDebugAdvanced4, IUnknown) {
  ...
  STDMETHOD(GetSymbolInformationWideEx)(
      ...
      _Out_opt_ PULONG InfoSize,
      _Out_writes_opt_(StringBufferSize) PWSTR StringBuffer,
      _In_ ULONG StringBufferSize, _Out_opt_ PULONG StringSize,
      _Out_opt_ PSYMBOL_INFO_EX pInfoEx) PURE;
};
...
DECLARE_INTERFACE_(IDebugBreakpoint, IUnknown) {
  ...
  STDMETHOD(GetId)(THIS_ _Out_ PULONG Id) PURE;
  // Retrieves the type of break and
  // processor type for the breakpoint.
  STDMETHOD(GetType)(THIS_ _Out_ PULONG BreakType, _Out_ PULONG ProcType) PURE;
  // Returns the client that called AddBreakpoint.
  STDMETHOD(GetAdder)(THIS_ _Out_ PDEBUG_CLIENT * Adder) PURE;

  STDMETHOD(GetFlags)(THIS_ _Out_ PULONG Flags) PURE;
  // Only certain flags can be changed.  Flags
  // are: GO_ONLY, ENABLE.
  // Sets the given flags.
  STDMETHOD(AddFlags)(THIS_ _In_ ULONG Flags) PURE;
  // Clears the given flags.
  STDMETHOD(RemoveFlags)(THIS_ _In_ ULONG Flags) PURE;
  // Sets the flags.
  STDMETHOD(SetFlags)(THIS_ _In_ ULONG Flags) PURE;
  ...
  // I/O port, etc.
  STDMETHOD(GetOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  STDMETHOD(SetOffset)(THIS_ _In_ ULONG64 Offset) PURE;
  ...
  STDMETHOD(GetDataParameters)(THIS_ _Out_ PULONG Size, _Out_ PULONG AccessType)
      PURE;
  STDMETHOD(SetDataParameters)(THIS_ _In_ ULONG Size, _In_ ULONG AccessType)
      PURE;

  // Pass count defaults to one.
  STDMETHOD(GetPassCount)(THIS_ _Out_ PULONG Count) PURE;
  STDMETHOD(SetPassCount)(THIS_ _In_ ULONG Count) PURE;
  // Gets the current number of times
  // the breakpoint has been hit since
  // it was last triggered.
  STDMETHOD(GetCurrentPassCount)(THIS_ _Out_ PULONG Count) PURE;
  ...
  // in kernel mode.
  STDMETHOD(GetMatchThreadId)(THIS_ _Out_ PULONG Id) PURE;
  STDMETHOD(SetMatchThreadId)(THIS_ _In_ ULONG Thread) PURE;
  ...
  STDMETHOD(GetCommand)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                        _In_ ULONG BufferSize, _Out_opt_ PULONG CommandSize)
      PURE;
  STDMETHOD(SetCommand)(THIS_ _In_ PCSTR Command) PURE;
  ...
  STDMETHOD(GetOffsetExpression)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                                 _In_ ULONG BufferSize,
                                 _Out_opt_ PULONG ExpressionSize) PURE;
  STDMETHOD(SetOffsetExpression)(THIS_ _In_ PCSTR Expression) PURE;

  STDMETHOD(GetParameters)(THIS_ _Out_ PDEBUG_BREAKPOINT_PARAMETERS Params)
      PURE;
};
...
DECLARE_INTERFACE_(IDebugBreakpoint2, IUnknown) {
  ...
  STDMETHOD(GetCommandWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                            _In_ ULONG BufferSize, _Out_opt_ PULONG CommandSize)
      PURE;
  STDMETHOD(SetCommandWide)(THIS_ _In_ PCWSTR Command) PURE;

  STDMETHOD(GetOffsetExpressionWide)(
      THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG ExpressionSize) PURE;
  STDMETHOD(SetOffsetExpressionWide)(THIS_ _In_ PCWSTR Expression) PURE;
};
...
DECLARE_INTERFACE_(IDebugBreakpoint3, IUnknown) {
  ...
  STDMETHOD(GetGuid)(THIS_ _Out_ LPGUID Guid) PURE;
};
...
DECLARE_INTERFACE_(IDebugClient, IUnknown) {
  ...
  // itself.
  STDMETHOD(AttachKernel)(THIS_ _In_ ULONG Flags, _In_opt_ PCSTR ConnectOptions)
      PURE;
  STDMETHOD(GetKernelConnectionOptions)(
      THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG OptionsSize) PURE;
  ...
  STDMETHOD(SetKernelConnectionOptions)(THIS_ _In_ PCSTR Options) PURE;

  // Starts a process server for remote
  // user-mode process control.
  // The local process server is server zero.
  STDMETHOD(StartProcessServer)(THIS_ _In_ ULONG Flags, _In_ PCSTR Options,
                                _In_opt_ _Reserved_ PVOID Reserved) PURE;
  STDMETHOD(ConnectProcessServer)(THIS_ _In_ PCSTR RemoteOptions,
                                  _Out_ PULONG64 Server) PURE;
  STDMETHOD(DisconnectProcessServer)(THIS_ _In_ ULONG64 Server) PURE;

  // Enumerates and describes processes
  // accessible through the given process server.
  STDMETHOD(GetRunningProcessSystemIds)(
      THIS_ _In_ ULONG64 Server, _Out_writes_opt_(Count) PULONG Ids,
      _In_ ULONG Count, _Out_opt_ PULONG ActualCount) PURE;
  STDMETHOD(GetRunningProcessSystemIdByExecutableName)(
      THIS_ _In_ ULONG64 Server, _In_ PCSTR ExeName, _In_ ULONG Flags,
      _Out_ PULONG Id) PURE;
  STDMETHOD(GetRunningProcessDescription)(
      THIS_ _In_ ULONG64 Server, _In_ ULONG SystemId, _In_ ULONG Flags,
      _Out_writes_opt_(ExeNameSize) PSTR ExeName, _In_ ULONG ExeNameSize,
      _Out_opt_ PULONG ActualExeNameSize,
      _Out_writes_opt_(DescriptionSize) PSTR Description,
      _In_ ULONG DescriptionSize, _Out_opt_ PULONG ActualDescriptionSize) PURE;

  // Attaches to a running user-mode process.
  STDMETHOD(AttachProcess)(THIS_ _In_ ULONG64 Server, _In_ ULONG ProcessId,
                           _In_ ULONG AttachFlags) PURE;
  ...
  // must be specified.
  STDMETHOD(CreateProcess)(THIS_ _In_ ULONG64 Server, _In_ PSTR CommandLine,
                           _In_ ULONG CreateFlags) PURE;
  ...
  STDMETHOD(CreateProcessAndAttach)(
      THIS_ _In_ ULONG64 Server, _In_opt_ PSTR CommandLine,
      _In_ ULONG CreateFlags, _In_ ULONG ProcessId, _In_ ULONG AttachFlags)
      PURE;
  // Gets and sets process control flags.
  STDMETHOD(GetProcessOptions)(THIS_ _Out_ PULONG Options) PURE;
  STDMETHOD(AddProcessOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(RemoveProcessOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(SetProcessOptions)(THIS_ _In_ ULONG Options) PURE;

  // Opens any kind of user- or kernel-mode dump file
  // and begins a debug session with the information
  // contained within it.
  STDMETHOD(OpenDumpFile)(THIS_ _In_ PCSTR DumpFile) PURE;
  ...
  STDMETHOD(WriteDumpFile)(THIS_ _In_ PCSTR DumpFile, _In_ ULONG Qualifier)
      PURE;
  ...
  STDMETHOD(ConnectSession)(THIS_ _In_ ULONG Flags, _In_ ULONG HistoryLimit)
      PURE;
  ...
  //    tcp:Port=<IP port>
  STDMETHOD(StartServer)(THIS_ _In_ PCSTR Options) PURE;
  // List the servers running on the given machine.
  // Uses the line prefix.
  STDMETHOD(OutputServers)(THIS_ _In_ ULONG OutputControl, _In_ PCSTR Machine,
                           _In_ ULONG Flags) PURE;
  ...
  STDMETHOD(EndSession)(THIS_ _In_ ULONG Flags) PURE;
  // If a process was started and ran to completion
  // this method can be used to retrieve its exit code.
  STDMETHOD(GetExitCode)(THIS_ _Out_ PULONG Code) PURE;
  ...
  // timeout expires.
  STDMETHOD(DispatchCallbacks)(THIS_ _In_ ULONG Timeout) PURE;
  ...
  STDMETHOD(ExitDispatch)(THIS_ _In_ PDEBUG_CLIENT Client) PURE;
  ...
  STDMETHOD(CreateClient)(THIS_ _Out_ PDEBUG_CLIENT * Client) PURE;

  STDMETHOD(GetInputCallbacks)(THIS_ _Out_ PDEBUG_INPUT_CALLBACKS * Callbacks)
      PURE;
  STDMETHOD(SetInputCallbacks)(THIS_ _In_opt_ PDEBUG_INPUT_CALLBACKS Callbacks)
      PURE;

  // Output callback interfaces are described separately.
  STDMETHOD(GetOutputCallbacks)(THIS_ _Out_ PDEBUG_OUTPUT_CALLBACKS * Callbacks)
      PURE;
  STDMETHOD(SetOutputCallbacks)(
      THIS_ _In_opt_ PDEBUG_OUTPUT_CALLBACKS Callbacks) PURE;
  ...
  STDMETHOD(GetOutputMask)(THIS_ _Out_ PULONG Mask) PURE;
  STDMETHOD(SetOutputMask)(THIS_ _In_ ULONG Mask) PURE;
  ...
  STDMETHOD(GetOtherOutputMask)(THIS_ _In_ PDEBUG_CLIENT Client,
                                _Out_ PULONG Mask) PURE;
  STDMETHOD(SetOtherOutputMask)(THIS_ _In_ PDEBUG_CLIENT Client,
                                _In_ ULONG Mask) PURE;
  // Control the width of an output line for
  // commands which produce formatted output.
  // This setting is just a suggestion.
  STDMETHOD(GetOutputWidth)(THIS_ _Out_ PULONG Columns) PURE;
  STDMETHOD(SetOutputWidth)(THIS_ _In_ ULONG Columns) PURE;
  ...
  STDMETHOD(GetOutputLinePrefix)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                                 _In_ ULONG BufferSize,
                                 _Out_opt_ PULONG PrefixSize) PURE;
  STDMETHOD(SetOutputLinePrefix)(THIS_ _In_opt_ PCSTR Prefix) PURE;
  ...
  // may also be present.
  STDMETHOD(GetIdentity)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                         _In_ ULONG BufferSize, _Out_opt_ PULONG IdentitySize)
      PURE;
  // Format is a printf-like format string
  // with one %s where the identity string should go.
  STDMETHOD(OutputIdentity)(THIS_ _In_ ULONG OutputControl, _In_ ULONG Flags,
                            _In_ PCSTR Format) PURE;

  // Event callbacks allow a client to
  // receive notification about changes
  // during the debug session.
  STDMETHOD(GetEventCallbacks)(THIS_ _Out_ PDEBUG_EVENT_CALLBACKS * Callbacks)
      PURE;
  STDMETHOD(SetEventCallbacks)(THIS_ _In_opt_ PDEBUG_EVENT_CALLBACKS Callbacks)
      PURE;
...
DECLARE_INTERFACE_(IDebugClient2, IUnknown) {
  ...
  STDMETHOD(WriteDumpFile2)(THIS_ _In_ PCSTR DumpFile, _In_ ULONG Qualifier,
                            _In_ ULONG FormatFlags, _In_opt_ PCSTR Comment)
      PURE;
  ...
  STDMETHOD(AddDumpInformationFile)(THIS_ _In_ PCSTR InfoFile, _In_ ULONG Type)
      PURE;

  // Requests that the remote process server shut down.
  STDMETHOD(EndProcessServer)(THIS_ _In_ ULONG64 Server) PURE;
  ...
  STDMETHOD(WaitForProcessServerEnd)(THIS_ _In_ ULONG Timeout) PURE;
...
DECLARE_INTERFACE_(IDebugClient3, IUnknown) {
  ...
  STDMETHOD(GetRunningProcessSystemIdByExecutableNameWide)(
      THIS_ _In_ ULONG64 Server, _In_ PCWSTR ExeName, _In_ ULONG Flags,
      _Out_ PULONG Id) PURE;
  STDMETHOD(GetRunningProcessDescriptionWide)(
      THIS_ _In_ ULONG64 Server, _In_ ULONG SystemId, _In_ ULONG Flags,
      _Out_writes_opt_(ExeNameSize) PWSTR ExeName, _In_ ULONG ExeNameSize,
      _Out_opt_ PULONG ActualExeNameSize,
      _Out_writes_opt_(DescriptionSize) PWSTR Description,
      _In_ ULONG DescriptionSize, _Out_opt_ PULONG ActualDescriptionSize) PURE;

  STDMETHOD(CreateProcessWide)(THIS_ _In_ ULONG64 Server,
                               _In_ PWSTR CommandLine, _In_ ULONG CreateFlags)
      PURE;
  STDMETHOD(CreateProcessAndAttachWide)(
      THIS_ _In_ ULONG64 Server, _In_opt_ PWSTR CommandLine,
      _In_ ULONG CreateFlags, _In_ ULONG ProcessId, _In_ ULONG AttachFlags)
      PURE;
};
...
DECLARE_INTERFACE_(IDebugClient4, IUnknown) {
  ...
  STDMETHOD(OpenDumpFileWide)(THIS_ _In_opt_ PCWSTR FileName,
                              _In_ ULONG64 FileHandle) PURE;
  STDMETHOD(WriteDumpFileWide)(THIS_ _In_opt_ PCWSTR FileName,
                               _In_ ULONG64 FileHandle, _In_ ULONG Qualifier,
                               _In_ ULONG FormatFlags, _In_opt_ PCWSTR Comment)
      PURE;
  STDMETHOD(AddDumpInformationFileWide)(THIS_ _In_opt_ PCWSTR FileName,
                                        _In_ ULONG64 FileHandle,
                                        _In_ ULONG Type) PURE;
  // These methods can be used to retrieve
  // file information for all targets that
  // involve files.
  STDMETHOD(GetNumberDumpFiles)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetDumpFile)(THIS_ _In_ ULONG Index,
                         _Out_writes_opt_(BufferSize) PSTR Buffer,
                         _In_ ULONG BufferSize, _Out_opt_ PULONG NameSize,
                         _Out_opt_ PULONG64 Handle, _Out_ PULONG Type) PURE;
  STDMETHOD(GetDumpFileWide)(THIS_ _In_ ULONG Index,
...
DECLARE_INTERFACE_(IDebugClient5, IUnknown) {
  ...
  STDMETHOD(AttachKernelWide)(THIS_ _In_ ULONG Flags,
                              _In_opt_ PCWSTR ConnectOptions) PURE;
  STDMETHOD(GetKernelConnectionOptionsWide)(
      THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG OptionsSize) PURE;
  STDMETHOD(SetKernelConnectionOptionsWide)(THIS_ _In_ PCWSTR Options) PURE;

  STDMETHOD(StartProcessServerWide)(THIS_ _In_ ULONG Flags, _In_ PCWSTR Options,
                                    _In_opt_ _Reserved_ PVOID Reserved) PURE;
  STDMETHOD(ConnectProcessServerWide)(THIS_ _In_ PCWSTR RemoteOptions,
                                      _Out_ PULONG64 Server) PURE;

  STDMETHOD(StartServerWide)(THIS_ _In_ PCWSTR Options) PURE;
  STDMETHOD(OutputServersWide)(THIS_ _In_ ULONG OutputControl,
                               _In_ PCWSTR Machine, _In_ ULONG Flags) PURE;

  STDMETHOD(GetOutputCallbacksWide)(THIS_ _Out_ PDEBUG_OUTPUT_CALLBACKS_WIDE *
                                    Callbacks) PURE;
  STDMETHOD(SetOutputCallbacksWide)(
      THIS_ _In_ PDEBUG_OUTPUT_CALLBACKS_WIDE Callbacks) PURE;
  STDMETHOD(GetOutputLinePrefixWide)(
      THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG PrefixSize) PURE;
  STDMETHOD(SetOutputLinePrefixWide)(THIS_ _In_opt_ PCWSTR Prefix) PURE;

  STDMETHOD(GetIdentityWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                             _In_ ULONG BufferSize,
                             _Out_opt_ PULONG IdentitySize) PURE;
  STDMETHOD(OutputIdentityWide)(THIS_ _In_ ULONG OutputControl,
                                _In_ ULONG Flags, _In_ PCWSTR Format) PURE;

  STDMETHOD(GetEventCallbacksWide)(THIS_ _Out_ PDEBUG_EVENT_CALLBACKS_WIDE *
                                   Callbacks) PURE;
  STDMETHOD(SetEventCallbacksWide)(
      THIS_ _In_ PDEBUG_EVENT_CALLBACKS_WIDE Callbacks) PURE;

  STDMETHOD(CreateProcess2)(
      THIS_ _In_ ULONG64 Server, _In_ PSTR CommandLine,
      _In_reads_bytes_(OptionsBufferSize) PVOID OptionsBuffer,
      _In_ ULONG OptionsBufferSize, _In_opt_ PCSTR InitialDirectory,
      _In_opt_ PCSTR Environment) PURE;
  STDMETHOD(CreateProcess2Wide)(
      THIS_ _In_ ULONG64 Server, _In_ PWSTR CommandLine,
      _In_reads_bytes_(OptionsBufferSize) PVOID OptionsBuffer,
      _In_ ULONG OptionsBufferSize, _In_opt_ PCWSTR InitialDirectory,
      _In_opt_ PCWSTR Environment) PURE;
  STDMETHOD(CreateProcessAndAttach2)(
      THIS_ _In_ ULONG64 Server, _In_opt_ PSTR CommandLine,
      _In_reads_bytes_(OptionsBufferSize) PVOID OptionsBuffer,
      _In_ ULONG OptionsBufferSize, _In_opt_ PCSTR InitialDirectory,
      _In_opt_ PCSTR Environment, _In_ ULONG ProcessId, _In_ ULONG AttachFlags)
      PURE;
  STDMETHOD(CreateProcessAndAttach2Wide)(
      THIS_ _In_ ULONG64 Server, _In_opt_ PWSTR CommandLine,
      _In_reads_bytes_(OptionsBufferSize) PVOID OptionsBuffer,
      _In_ ULONG OptionsBufferSize, _In_opt_ PCWSTR InitialDirectory,
      _In_opt_ PCWSTR Environment, _In_ ULONG ProcessId, _In_ ULONG AttachFlags)
      PURE;

  // Helpers for saving and restoring the
  // current output line prefix.
  STDMETHOD(PushOutputLinePrefix)(THIS_ _In_opt_ PCSTR NewPrefix,
                                  _Out_ PULONG64 Handle) PURE;
  STDMETHOD(PushOutputLinePrefixWide)(THIS_ _In_opt_ PCWSTR NewPrefix,
                                      _Out_ PULONG64 Handle) PURE;
  STDMETHOD(PopOutputLinePrefix)(THIS_ _In_ ULONG64 Handle) PURE;

  // Queries to determine if any clients
  // could potentially respond to the given callback.
  STDMETHOD(GetNumberInputCallbacks)(THIS_ _Out_ PULONG Count) PURE;
  STDMETHOD(GetNumberOutputCallbacks)(THIS_ _Out_ PULONG Count) PURE;
  STDMETHOD(GetNumberEventCallbacks)(THIS_ _In_ ULONG EventFlags,
                                     _Out_ PULONG Count) PURE;

  // Control over locking the session against
  // undesired quits.  The quit lock string
  // cannot be retrieved from a secure session.
  STDMETHOD(GetQuitLockString)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                               _In_ ULONG BufferSize,
                               _Out_opt_ PULONG StringSize) PURE;
  STDMETHOD(SetQuitLockString)(THIS_ _In_ PCSTR String) PURE;
  STDMETHOD(GetQuitLockStringWide)(
      THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG StringSize) PURE;
  STDMETHOD(SetQuitLockStringWide)(THIS_ _In_ PCWSTR String) PURE;
};
...
DECLARE_INTERFACE_(IDebugClient6, IUnknown) {
  ...
  STDMETHOD(SetEventContextCallbacks)(
      THIS_ _In_opt_ PDEBUG_EVENT_CONTEXT_CALLBACKS Callbacks) PURE;
};
...
DECLARE_INTERFACE_(IDebugClient7, IUnknown) {
  ...
  STDMETHOD(SetClientContext)(THIS_ _In_reads_bytes_(ContextSize) PVOID Context,
                              _In_ ULONG ContextSize) PURE;
};
...
DECLARE_INTERFACE_(IDebugClient8, IUnknown) {
  ...
  STDMETHOD(OpenDumpFileWide2)(THIS_ _In_opt_ PCWSTR FileName,
                               _In_ ULONG64 FileHandle,
                               _In_ ULONG AlternateArch) PURE;
};
...
DECLARE_INTERFACE_(IDebugClient9, IUnknown) {
  ...
  STDMETHOD(OpenDumpDirectoryWide)(THIS_ _In_opt_ PCWSTR DirName,
                                   _In_ ULONG AlternateArch) PURE;
  STDMETHOD(OpenDumpDirectory)(THIS_ _In_ PCSTR DumpDir,
                               _In_ ULONG AlternativeArch) PURE;
};
...
DECLARE_INTERFACE_(IDebugPlmClient, IUnknown) {
  ...
  STDMETHOD(LaunchPlmPackageForDebugWide)(
      THIS_ _In_ ULONG64 Server, _In_ ULONG Timeout,
      _In_ PCWSTR PackageFullName, _In_ PCWSTR AppName,
      _In_opt_ PCWSTR Arguments, _Out_ PULONG ProcessId, _Out_ PULONG ThreadId)
      PURE;
};
...
DECLARE_INTERFACE_(IDebugPlmClient2, IUnknown) {
  ...
  STDMETHOD(LaunchPlmBgTaskForDebugWide)(
      THIS_ _In_ ULONG64 Server, _In_ ULONG Timeout,
      _In_ PCWSTR PackageFullName, _In_ PCWSTR BackgroundTaskId,
      _Out_ PULONG ProcessId, _Out_ PULONG ThreadId) PURE;
};
...
DECLARE_INTERFACE_(IDebugPlmClient3, IUnknown) {
  ...
  STDMETHOD(QueryPlmPackageWide)(THIS_ _In_ ULONG64 Server,
                                 _In_ PCWSTR PackageFullName,
                                 _In_ PDEBUG_OUTPUT_STREAM Stream) PURE;

  STDMETHOD(QueryPlmPackageList)(THIS_ _In_ ULONG64 Server,
                                 _In_ PDEBUG_OUTPUT_STREAM Stream) PURE;

  STDMETHOD(EnablePlmPackageDebugWide)(THIS_ _In_ ULONG64 Server,
                                       _In_ PCWSTR PackageFullName) PURE;

  STDMETHOD(DisablePlmPackageDebugWide)(THIS_ _In_ ULONG64 Server,
                                        _In_ PCWSTR PackageFullName) PURE;

  STDMETHOD(SuspendPlmPackageWide)(THIS_ _In_ ULONG64 Server,
                                   _In_ PCWSTR PackageFullName) PURE;

  STDMETHOD(ResumePlmPackageWide)(THIS_ _In_ ULONG64 Server,
                                  _In_ PCWSTR PackageFullName) PURE;

  STDMETHOD(TerminatePlmPackageWide)(THIS_ _In_ ULONG64 Server,
                                     _In_ PCWSTR PackageFullName) PURE;

  // Launches and attaches to Plm Application. Starts debugger session
  // if it is not already started
  STDMETHOD(LaunchAndDebugPlmAppWide)(
      THIS_ _In_ ULONG64 Server, _In_ PCWSTR PackageFullName,
      _In_ PCWSTR AppName, _In_ PCWSTR Arguments) PURE;

  // Launches and attaches to Plm Bg Task. Starts debugger session
  // if it is not already started
  STDMETHOD(ActivateAndDebugPlmBgTaskWide)(THIS_ _In_ ULONG64 Server,
                                           _In_ PCWSTR PackageFullName,
                                           _In_ PCWSTR BackgroundTaskId) PURE;
};
...
DECLARE_INTERFACE_(IDebugOutputStream, IUnknown) {
  // IUnknown.
  STDMETHOD(QueryInterface)(THIS_ __in REFIID InterfaceId,
                            __out PVOID * Interface) PURE;
  STDMETHOD_(ULONG, AddRef)(THIS) PURE;
  STDMETHOD_(ULONG, Release)(THIS) PURE;

  // IDebugOutputStream.
  STDMETHOD(Write)(THIS_ _In_ PCWSTR psz) PURE;
};
...
// This is used to disable the source stepping (step over/step in) into CFG
// code.
...
// Use suffixes of the form @n (n is a non-negative integer) to disambiguate
// shadowed variables.
...
#define DBG_FRAME_DEFAULT                                                      \
  0 // the same as INLINE_FRAME_CONTEXT_INIT in dbghelp.h
#define DBG_FRAME_IGNORE_INLINE                                                \
  0xFFFFFFFF // the same as INLINE_FRAME_CONTEXT_IGNORE in dbghelp.h
...
#define STACK_FRAME_TYPE_RA                                                    \
  0x80 // Whether the instruction pointer is the current IP or a RA from callee
       // frame.
...
typedef struct _DEBUG_LAST_EVENT_INFO_SERVICE_EXCEPTION {
  ...
  //
  // (Kind) Specific Data... (e.g.: an EXCEPTION_RECORD64 or another definition
  // given by
  //                                a specific platform service)
  //
} DEBUG_LAST_EVENT_INFO_SERVICE_EXCEPTION,
    *PDEBUG_LAST_EVENT_INFO_SERVICE_EXCEPTION;
...
DECLARE_INTERFACE_(IDebugControl, IUnknown) {
  ...
  // This method is reentrant.
  STDMETHOD(SetInterrupt)(THIS_ _In_ ULONG Flags) PURE;
  ...
  STDMETHOD(GetInterruptTimeout)(THIS_ _Out_ PULONG Seconds) PURE;
  STDMETHOD(SetInterruptTimeout)(THIS_ _In_ ULONG Seconds) PURE;

  STDMETHOD(GetLogFile)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                        _In_ ULONG BufferSize, _Out_opt_ PULONG FileSize,
                        _Out_ PBOOL Append) PURE;
  ...
  // already open.
  STDMETHOD(OpenLogFile)(THIS_ _In_ PCSTR File, _In_ BOOL Append) PURE;
  STDMETHOD(CloseLogFile)(THIS) PURE;
  // Controls what output is logged.
  STDMETHOD(GetLogMask)(THIS_ _Out_ PULONG Mask) PURE;
  STDMETHOD(SetLogMask)(THIS_ _In_ ULONG Mask) PURE;

  // Input requests input from all clients.
  // The first input that is returned is used
  // to satisfy the call.  Other returned
  // input is discarded.
  STDMETHOD(Input)(THIS_ _Out_writes_(BufferSize) PSTR Buffer,
                   _In_ ULONG BufferSize, _Out_opt_ PULONG InputSize) PURE;
  ...
  // This method is reentrant.
  STDMETHOD(ReturnInput)(THIS_ _In_ PCSTR Buffer) PURE;
  ...
  // settings.
  STDMETHODV(Output)(THIS_ _In_ ULONG Mask, _In_ PCSTR Format, ...) PURE;
  STDMETHOD(OutputVaList)(THIS_ _In_ ULONG Mask, _In_ PCSTR Format,
                          _In_ va_list Args) PURE;
  ...
  STDMETHODV(ControlledOutput)(THIS_ _In_ ULONG OutputControl, _In_ ULONG Mask,
                               _In_ PCSTR Format, ...) PURE;
  STDMETHOD(ControlledOutputVaList)(THIS_ _In_ ULONG OutputControl,
                                    _In_ ULONG Mask, _In_ PCSTR Format,
                                    _In_ va_list Args) PURE;
  ...
  // does not get input.
  STDMETHODV(OutputPrompt)(THIS_ _In_ ULONG OutputControl,
                           _In_opt_ PCSTR Format, ...) PURE;
  STDMETHOD(OutputPromptVaList)(THIS_ _In_ ULONG OutputControl,
                                _In_opt_ PCSTR Format, _In_ va_list Args) PURE;
  // Gets the text that would be displayed by OutputPrompt.
  STDMETHOD(GetPromptText)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                           _In_ ULONG BufferSize, _Out_opt_ PULONG TextSize)
      PURE;
  ...
  // Uses the line prefix.
  STDMETHOD(OutputCurrentState)(THIS_ _In_ ULONG OutputControl,
                                _In_ ULONG Flags) PURE;

  // Outputs the debugger and extension version
  // information.  This method is reentrant.
  // Uses the line prefix.
  STDMETHOD(OutputVersionInformation)(THIS_ _In_ ULONG OutputControl) PURE;
  ...
  STDMETHOD(GetNotifyEventHandle)(THIS_ _Out_ PULONG64 Handle) PURE;
  STDMETHOD(SetNotifyEventHandle)(THIS_ _In_ ULONG64 Handle) PURE;

  STDMETHOD(Assemble)(THIS_ _In_ ULONG64 Offset, _In_ PCSTR Instr,
                      _Out_ PULONG64 EndOffset) PURE;
  STDMETHOD(Disassemble)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG Flags,
      _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG DisassemblySize, _Out_ PULONG64 EndOffset) PURE;
  // Returns the value of the effective address
  // computed for the last Disassemble, if there
  // was one.
  STDMETHOD(GetDisassembleEffectiveOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  // Uses the line prefix if necessary.
  STDMETHOD(OutputDisassembly)(THIS_ _In_ ULONG OutputControl,
                               _In_ ULONG64 Offset, _In_ ULONG Flags,
                               _Out_ PULONG64 EndOffset) PURE;
  ...
  STDMETHOD(OutputDisassemblyLines)(
      THIS_ _In_ ULONG OutputControl, _In_ ULONG PreviousLines,
      _In_ ULONG TotalLines, _In_ ULONG64 Offset, _In_ ULONG Flags,
      _Out_opt_ PULONG OffsetLine, _Out_opt_ PULONG64 StartOffset,
      _Out_opt_ PULONG64 EndOffset,
      _Out_writes_opt_(TotalLines) PULONG64 LineOffsets) PURE;
  ...
  STDMETHOD(GetNearInstruction)(THIS_ _In_ ULONG64 Offset, _In_ LONG Delta,
                                _Out_ PULONG64 NearOffset) PURE;

  // Offsets can be passed in as zero to use the current
  // thread state.
  STDMETHOD(GetStackTrace)(
      THIS_ _In_ ULONG64 FrameOffset, _In_ ULONG64 StackOffset,
      _In_ ULONG64 InstructionOffset,
      _Out_writes_(FramesSize) PDEBUG_STACK_FRAME Frames, _In_ ULONG FramesSize,
      _Out_opt_ PULONG FramesFilled) PURE;
  // Does a simple stack trace to determine
  // what the current return address is.
  STDMETHOD(GetReturnOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  ...
  // are used.
  // Uses the line prefix.
  STDMETHOD(OutputStackTrace)(THIS_ _In_ ULONG OutputControl,
                              _In_reads_opt_(FramesSize)
                                  PDEBUG_STACK_FRAME Frames,
                              _In_ ULONG FramesSize, _In_ ULONG Flags) PURE;

  // Returns information about the debuggee such
  // as user vs. kernel, dump vs. live, etc.
  STDMETHOD(GetDebuggeeType)(THIS_ _Out_ PULONG Class, _Out_ PULONG Qualifier)
      PURE;
  // Returns the type of physical processors in
  // the machine.
  // Returns one of the IMAGE_FILE_MACHINE values.
  STDMETHOD(GetActualProcessorType)(THIS_ _Out_ PULONG Type) PURE;
  // Returns the type of processor used in the
  // current processor context.
  STDMETHOD(GetExecutingProcessorType)(THIS_ _Out_ PULONG Type) PURE;
  // Query all the possible processor types that
  // may be encountered during this debug session.
  STDMETHOD(GetNumberPossibleExecutingProcessorTypes)(THIS_ _Out_ PULONG Number)
      PURE;
  STDMETHOD(GetPossibleExecutingProcessorTypes)(
      THIS_ _In_ ULONG Start, _In_ ULONG Count,
      _Out_writes_(Count) PULONG Types) PURE;
  // Get the number of actual processors in
  // the machine.
  STDMETHOD(GetNumberProcessors)(THIS_ _Out_ PULONG Number) PURE;
  ...
  STDMETHOD(GetSystemVersion)(
      THIS_ _Out_ PULONG PlatformId, _Out_ PULONG Major, _Out_ PULONG Minor,
      _Out_writes_opt_(ServicePackStringSize) PSTR ServicePackString,
      _In_ ULONG ServicePackStringSize, _Out_opt_ PULONG ServicePackStringUsed,
      _Out_ PULONG ServicePackNumber,
      _Out_writes_opt_(BuildStringSize) PSTR BuildString,
      _In_ ULONG BuildStringSize, _Out_opt_ PULONG BuildStringUsed) PURE;
  // Returns the page size for the currently executing
  // processor context.  The page size may vary between
  // processor types.
  STDMETHOD(GetPageSize)(THIS_ _Out_ PULONG Size) PURE;
  ...
  STDMETHOD(ReadBugCheckData)(THIS_ _Out_ PULONG Code, _Out_ PULONG64 Arg1,
                              _Out_ PULONG64 Arg2, _Out_ PULONG64 Arg3,
                              _Out_ PULONG64 Arg4) PURE;
  ...
  // or the debuggee.
  STDMETHOD(GetNumberSupportedProcessorTypes)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetSupportedProcessorTypes)(THIS_ _In_ ULONG Start,
                                        _In_ ULONG Count,
                                        _Out_writes_(Count) PULONG Types) PURE;
  // Returns a full, descriptive name and an
  // abbreviated name for a processor type.
  STDMETHOD(GetProcessorTypeNames)(
      THIS_ _In_ ULONG Type,
      _Out_writes_opt_(FullNameBufferSize) PSTR FullNameBuffer,
      _In_ ULONG FullNameBufferSize, _Out_opt_ PULONG FullNameSize,
      _Out_writes_opt_(AbbrevNameBufferSize) PSTR AbbrevNameBuffer,
      _In_ ULONG AbbrevNameBufferSize, _Out_opt_ PULONG AbbrevNameSize) PURE;
  ...
  STDMETHOD(GetEffectiveProcessorType)(THIS_ _Out_ PULONG Type) PURE;
  STDMETHOD(SetEffectiveProcessorType)(THIS_ _In_ ULONG Type) PURE;
  ...
  // NO_DEBUGGEE.
  // This method is reentrant.
  STDMETHOD(GetExecutionStatus)(THIS_ _Out_ PULONG Status) PURE;
  // Changes the execution status of the
  // engine from stopped to running.
  // Status must be one of the go or step
  // status values.
  STDMETHOD(SetExecutionStatus)(THIS_ _In_ ULONG Status) PURE;
  ...
  STDMETHOD(GetCodeLevel)(THIS_ _Out_ PULONG Level) PURE;
  STDMETHOD(SetCodeLevel)(THIS_ _In_ ULONG Level) PURE;

  // Gets and sets engine control flags.
  // These methods are reentrant.
  STDMETHOD(GetEngineOptions)(THIS_ _Out_ PULONG Options) PURE;
  STDMETHOD(AddEngineOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(RemoveEngineOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(SetEngineOptions)(THIS_ _In_ ULONG Options) PURE;
  ...
  // may be set.
  STDMETHOD(GetSystemErrorControl)(THIS_ _Out_ PULONG OutputLevel,
                                   _Out_ PULONG BreakLevel) PURE;
  STDMETHOD(SetSystemErrorControl)(THIS_ _In_ ULONG OutputLevel,
                                   _In_ ULONG BreakLevel) PURE;
  ...
  STDMETHOD(GetTextMacro)(
      THIS_ _In_ ULONG Slot, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG MacroSize) PURE;
  STDMETHOD(SetTextMacro)(THIS_ _In_ ULONG Slot, _In_ PCSTR Macro) PURE;

  // Controls the default number radix used
  // in expressions and commands.
  STDMETHOD(GetRadix)(THIS_ _Out_ PULONG Radix) PURE;
  STDMETHOD(SetRadix)(THIS_ _In_ ULONG Radix) PURE;
  ...
  STDMETHOD(Evaluate)(THIS_ _In_ PCSTR Expression, _In_ ULONG DesiredType,
                      _Out_ PDEBUG_VALUE Value, _Out_opt_ PULONG RemainderIndex)
      PURE;
  ...
  STDMETHOD(CoerceValue)(THIS_ _In_ PDEBUG_VALUE In, _In_ ULONG OutType,
                         _Out_ PDEBUG_VALUE Out) PURE;
  STDMETHOD(CoerceValues)(THIS_ _In_ ULONG Count,
                          _In_reads_(Count) PDEBUG_VALUE In,
                          _In_reads_(Count) PULONG OutTypes,
                          _Out_writes_(Count) PDEBUG_VALUE Out) PURE;
  ...
  // of time.
  STDMETHOD(Execute)(THIS_ _In_ ULONG OutputControl, _In_ PCSTR Command,
                     _In_ ULONG Flags) PURE;
  // Executes the given command file by
  // reading a line at a time and processing
  // it with Execute.
  STDMETHOD(ExecuteCommandFile)(THIS_ _In_ ULONG OutputControl,
                                _In_ PCSTR CommandFile, _In_ ULONG Flags) PURE;

  // Breakpoint interfaces are described
  // elsewhere in this section.
  STDMETHOD(GetNumberBreakpoints)(THIS_ _Out_ PULONG Number) PURE;
  ...
  STDMETHOD(GetBreakpointByIndex)(THIS_ _In_ ULONG Index,
                                  _Out_ PDEBUG_BREAKPOINT * Bp) PURE;
  STDMETHOD(GetBreakpointById)(THIS_ _In_ ULONG Id,
                               _Out_ PDEBUG_BREAKPOINT * Bp) PURE;
  ...
  STDMETHOD(GetBreakpointParameters)(
      THIS_ _In_ ULONG Count, _In_reads_opt_(Count) PULONG Ids,
      _In_ ULONG Start, _Out_writes_(Count) PDEBUG_BREAKPOINT_PARAMETERS Params)
      PURE;
  ...
  // the call will fail.
  STDMETHOD(AddBreakpoint)(THIS_ _In_ ULONG Type, _In_ ULONG DesiredId,
                           _Out_ PDEBUG_BREAKPOINT * Bp) PURE;
  // Breakpoint interface is invalid after this call.
  STDMETHOD(RemoveBreakpoint)(THIS_ _In_ PDEBUG_BREAKPOINT Bp) PURE;

  // Control and use extension DLLs.
  STDMETHOD(AddExtension)(THIS_ _In_ PCSTR Path, _In_ ULONG Flags,
                          _Out_ PULONG64 Handle) PURE;
  STDMETHOD(RemoveExtension)(THIS_ _In_ ULONG64 Handle) PURE;
  STDMETHOD(GetExtensionByPath)(THIS_ _In_ PCSTR Path, _Out_ PULONG64 Handle)
      PURE;
  // If Handle is zero the extension
  // chain is walked searching for the
  // function.
  STDMETHOD(CallExtension)(THIS_ _In_ ULONG64 Handle, _In_ PCSTR Function,
                           _In_opt_ PCSTR Arguments) PURE;
  ...
  STDMETHOD(GetExtensionFunction)(THIS_ _In_ ULONG64 Handle,
                                  _In_ PCSTR FuncName, _Out_ FARPROC * Function)
      PURE;
  ...
  STDMETHOD(GetWindbgExtensionApis32)(
      THIS_ _Inout_ PWINDBG_EXTENSION_APIS32 Api) PURE;
  STDMETHOD(GetWindbgExtensionApis64)(
      THIS_ _Inout_ PWINDBG_EXTENSION_APIS64 Api) PURE;
  ...
  STDMETHOD(GetNumberEventFilters)(THIS_ _Out_ PULONG SpecificEvents,
                                   _Out_ PULONG SpecificExceptions,
                                   _Out_ PULONG ArbitraryExceptions) PURE;
  // Some filters have descriptive text associated with them.
  STDMETHOD(GetEventFilterText)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG TextSize) PURE;
  // All filters support executing a command when the
  // event occurs.
  STDMETHOD(GetEventFilterCommand)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG CommandSize) PURE;
  STDMETHOD(SetEventFilterCommand)(THIS_ _In_ ULONG Index, _In_ PCSTR Command)
      PURE;
  STDMETHOD(GetSpecificFilterParameters)(
      THIS_ _In_ ULONG Start, _In_ ULONG Count,
      _Out_writes_(Count) PDEBUG_SPECIFIC_FILTER_PARAMETERS Params) PURE;
  STDMETHOD(SetSpecificFilterParameters)(
      THIS_ _In_ ULONG Start, _In_ ULONG Count,
      _In_reads_(Count) PDEBUG_SPECIFIC_FILTER_PARAMETERS Params) PURE;
  // Some specific filters have arguments to further
  // qualify their operation.
  STDMETHOD(GetSpecificFilterArgument)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG ArgumentSize) PURE;
  STDMETHOD(SetSpecificFilterArgument)(THIS_ _In_ ULONG Index,
                                       _In_ PCSTR Argument) PURE;
  // If Codes is non-NULL Start is ignored.
  STDMETHOD(GetExceptionFilterParameters)(
      THIS_ _In_ ULONG Count, _In_reads_opt_(Count) PULONG Codes,
      _In_ ULONG Start,
      _Out_writes_(Count) PDEBUG_EXCEPTION_FILTER_PARAMETERS Params) PURE;
  ...
  STDMETHOD(SetExceptionFilterParameters)(
      THIS_ _In_ ULONG Count,
      _In_reads_(Count) PDEBUG_EXCEPTION_FILTER_PARAMETERS Params) PURE;
  // Exception filters support an additional command for
  // second-chance events.
  STDMETHOD(GetExceptionFilterSecondCommand)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG CommandSize) PURE;
  STDMETHOD(SetExceptionFilterSecondCommand)(THIS_ _In_ ULONG Index,
                                             _In_ PCSTR Command) PURE;
  ...
  // kernel debugging.
  STDMETHOD(WaitForEvent)(THIS_ _In_ ULONG Flags, _In_ ULONG Timeout) PURE;
  ...
  STDMETHOD(GetLastEventInformation)(
      THIS_ _Out_ PULONG Type, _Out_ PULONG ProcessId, _Out_ PULONG ThreadId,
      _Out_writes_bytes_opt_(ExtraInformationSize) PVOID ExtraInformation,
      _In_ ULONG ExtraInformationSize, _Out_opt_ PULONG ExtraInformationUsed,
      _Out_writes_opt_(DescriptionSize) PSTR Description,
      _In_ ULONG DescriptionSize, _Out_opt_ PULONG DescriptionUsed) PURE;
};
...
DECLARE_INTERFACE_(IDebugControl2, IUnknown) {
  ...
  STDMETHOD(GetCurrentTimeDate)(THIS_ _Out_ PULONG TimeDate) PURE;
  // Retrieves the number of seconds since the
  // machine started running.
  STDMETHOD(GetCurrentSystemUpTime)(THIS_ _Out_ PULONG UpTime) PURE;

  // If the current session is a dump session,
  // retrieves any extended format information.
  STDMETHOD(GetDumpFormatFlags)(THIS_ _Out_ PULONG FormatFlags) PURE;
  ...
  // string can exist.
  STDMETHOD(GetNumberTextReplacements)(THIS_ _Out_ PULONG NumRepl) PURE;
  ...
  STDMETHOD(GetTextReplacement)(
      THIS_ _In_opt_ PCSTR SrcText, _In_ ULONG Index,
      _Out_writes_opt_(SrcBufferSize) PSTR SrcBuffer, _In_ ULONG SrcBufferSize,
      _Out_opt_ PULONG SrcSize, _Out_writes_opt_(DstBufferSize) PSTR DstBuffer,
      _In_ ULONG DstBufferSize, _Out_opt_ PULONG DstSize) PURE;
  // Setting the destination text to
  // NULL removes the alias.
  STDMETHOD(SetTextReplacement)(THIS_ _In_ PCSTR SrcText,
                                _In_opt_ PCSTR DstText) PURE;
  STDMETHOD(RemoveTextReplacements)(THIS) PURE;
  // Outputs the complete list of current
  // replacements.
  STDMETHOD(OutputTextReplacements)(THIS_ _In_ ULONG OutputControl,
                                    _In_ ULONG Flags) PURE;
};
...
DECLARE_INTERFACE_(IDebugControl3, IUnknown) {
  ...
  STDMETHOD(GetAssemblyOptions)(THIS_ _Out_ PULONG Options) PURE;
  STDMETHOD(AddAssemblyOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(RemoveAssemblyOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(SetAssemblyOptions)(THIS_ _In_ ULONG Options) PURE;

  // Control the expression syntax.
  STDMETHOD(GetExpressionSyntax)(THIS_ _Out_ PULONG Flags) PURE;
  STDMETHOD(SetExpressionSyntax)(THIS_ _In_ ULONG Flags) PURE;
  // Look up a syntax by its abbreviated
  // name and set it.
  STDMETHOD(SetExpressionSyntaxByName)(THIS_ _In_ PCSTR AbbrevName) PURE;
  STDMETHOD(GetNumberExpressionSyntaxes)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetExpressionSyntaxNames)(
      THIS_ _In_ ULONG Index,
      _Out_writes_opt_(FullNameBufferSize) PSTR FullNameBuffer,
      _In_ ULONG FullNameBufferSize, _Out_opt_ PULONG FullNameSize,
      _Out_writes_opt_(AbbrevNameBufferSize) PSTR AbbrevNameBuffer,
      _In_ ULONG AbbrevNameBufferSize, _Out_opt_ PULONG AbbrevNameSize) PURE;
  ...
  STDMETHOD(GetNumberEvents)(THIS_ _Out_ PULONG Events) PURE;
  ...
  STDMETHOD(GetEventIndexDescription)(
      THIS_ _In_ ULONG Index, _In_ ULONG Which, _In_opt_ PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG DescSize) PURE;
  STDMETHOD(GetCurrentEventIndex)(THIS_ _Out_ PULONG Index) PURE;
  ...
  // is called.
  STDMETHOD(SetNextEventIndex)(THIS_ _In_ ULONG Relation, _In_ ULONG Value,
                               _Out_ PULONG NextIndex) PURE;
};
...
DECLARE_INTERFACE_(IDebugControl4, IUnknown) {
  ...
  STDMETHOD(GetLogFileWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                            _In_ ULONG BufferSize, _Out_opt_ PULONG FileSize,
                            _Out_ PBOOL Append) PURE;
  STDMETHOD(OpenLogFileWide)(THIS_ _In_ PCWSTR File, _In_ BOOL Append) PURE;

  STDMETHOD(InputWide)(THIS_ _Out_writes_(BufferSize) PWSTR Buffer,
                       _In_ ULONG BufferSize, _Out_opt_ PULONG InputSize) PURE;
  STDMETHOD(ReturnInputWide)(THIS_ _In_ PCWSTR Buffer) PURE;

  STDMETHODV(OutputWide)(THIS_ _In_ ULONG Mask, _In_ PCWSTR Format, ...) PURE;
  STDMETHOD(OutputVaListWide)(THIS_ _In_ ULONG Mask, _In_ PCWSTR Format,
                              _In_ va_list Args) PURE;
  STDMETHODV(ControlledOutputWide)(THIS_ _In_ ULONG OutputControl,
                                   _In_ ULONG Mask, _In_ PCWSTR Format, ...)
      PURE;
  STDMETHOD(ControlledOutputVaListWide)(THIS_ _In_ ULONG OutputControl,
                                        _In_ ULONG Mask, _In_ PCWSTR Format,
                                        _In_ va_list Args) PURE;

  STDMETHODV(OutputPromptWide)(THIS_ _In_ ULONG OutputControl,
                               _In_opt_ PCWSTR Format, ...) PURE;
  STDMETHOD(OutputPromptVaListWide)(THIS_ _In_ ULONG OutputControl,
                                    _In_opt_ PCWSTR Format, _In_ va_list Args)
      PURE;
  STDMETHOD(GetPromptTextWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                               _In_ ULONG BufferSize, _Out_opt_ PULONG TextSize)
      PURE;

  STDMETHOD(AssembleWide)(THIS_ _In_ ULONG64 Offset, _In_ PCWSTR Instr,
                          _Out_ PULONG64 EndOffset) PURE;
  ...
  STDMETHOD(GetTextMacroWide)(
      THIS_ _In_ ULONG Slot, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG MacroSize) PURE;
  STDMETHOD(SetTextMacroWide)(THIS_ _In_ ULONG Slot, _In_ PCWSTR Macro) PURE;

  STDMETHOD(EvaluateWide)(THIS_ _In_ PCWSTR Expression, _In_ ULONG DesiredType,
                          _Out_ PDEBUG_VALUE Value,
                          _Out_opt_ PULONG RemainderIndex) PURE;

  STDMETHOD(ExecuteWide)(THIS_ _In_ ULONG OutputControl, _In_ PCWSTR Command,
                         _In_ ULONG Flags) PURE;
  STDMETHOD(ExecuteCommandFileWide)(THIS_ _In_ ULONG OutputControl,
                                    _In_ PCWSTR CommandFile, _In_ ULONG Flags)
      PURE;

  STDMETHOD(GetBreakpointByIndex2)(THIS_ _In_ ULONG Index,
                                   _Out_ PDEBUG_BREAKPOINT2 * Bp) PURE;
  STDMETHOD(GetBreakpointById2)(THIS_ _In_ ULONG Id,
                                _Out_ PDEBUG_BREAKPOINT2 * Bp) PURE;
  STDMETHOD(AddBreakpoint2)(THIS_ _In_ ULONG Type, _In_ ULONG DesiredId,
                            _Out_ PDEBUG_BREAKPOINT2 * Bp) PURE;
  STDMETHOD(RemoveBreakpoint2)(THIS_ _In_ PDEBUG_BREAKPOINT2 Bp) PURE;

  STDMETHOD(AddExtensionWide)(THIS_ _In_ PCWSTR Path, _In_ ULONG Flags,
                              _Out_ PULONG64 Handle) PURE;
  STDMETHOD(GetExtensionByPathWide)(THIS_ _In_ PCWSTR Path,
                                    _Out_ PULONG64 Handle) PURE;
  STDMETHOD(CallExtensionWide)(THIS_ _In_ ULONG64 Handle, _In_ PCWSTR Function,
                               _In_opt_ PCWSTR Arguments) PURE;
  STDMETHOD(GetExtensionFunctionWide)(THIS_ _In_ ULONG64 Handle,
                                      _In_ PCWSTR FuncName,
                                      _Out_ FARPROC * Function) PURE;

  STDMETHOD(GetEventFilterTextWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG TextSize) PURE;
  STDMETHOD(GetEventFilterCommandWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG CommandSize) PURE;
  STDMETHOD(SetEventFilterCommandWide)(THIS_ _In_ ULONG Index,
                                       _In_ PCWSTR Command) PURE;
  STDMETHOD(GetSpecificFilterArgumentWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG ArgumentSize) PURE;
  STDMETHOD(SetSpecificFilterArgumentWide)(THIS_ _In_ ULONG Index,
                                           _In_ PCWSTR Argument) PURE;
  STDMETHOD(GetExceptionFilterSecondCommandWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG CommandSize) PURE;
  STDMETHOD(SetExceptionFilterSecondCommandWide)(THIS_ _In_ ULONG Index,
                                                 _In_ PCWSTR Command) PURE;
  ...
  STDMETHOD(GetTextReplacementWide)(
      THIS_ _In_opt_ PCWSTR SrcText, _In_ ULONG Index,
      _Out_writes_opt_(SrcBufferSize) PWSTR SrcBuffer, _In_ ULONG SrcBufferSize,
      _Out_opt_ PULONG SrcSize, _Out_writes_opt_(DstBufferSize) PWSTR DstBuffer,
      _In_ ULONG DstBufferSize, _Out_opt_ PULONG DstSize) PURE;
  STDMETHOD(SetTextReplacementWide)(THIS_ _In_ PCWSTR SrcText,
                                    _In_opt_ PCWSTR DstText) PURE;

  STDMETHOD(SetExpressionSyntaxByNameWide)(THIS_ _In_ PCWSTR AbbrevName) PURE;
  ...
  STDMETHOD(GetEventIndexDescriptionWide)(
      THIS_ _In_ ULONG Index, _In_ ULONG Which, _In_opt_ PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG DescSize) PURE;

  STDMETHOD(GetLogFile2)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                         _In_ ULONG BufferSize, _Out_opt_ PULONG FileSize,
                         _Out_ PULONG Flags) PURE;
  STDMETHOD(OpenLogFile2)(THIS_ _In_ PCSTR File, _In_ ULONG Flags) PURE;
  STDMETHOD(GetLogFile2Wide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                             _In_ ULONG BufferSize, _Out_opt_ PULONG FileSize,
                             _Out_ PULONG Flags) PURE;
  STDMETHOD(OpenLogFile2Wide)(THIS_ _In_ PCWSTR File, _In_ ULONG Flags) PURE;
  ...
  STDMETHOD(GetSystemVersionValues)(
      THIS_ _Out_ PULONG PlatformId, _Out_ PULONG Win32Major,
      _Out_ PULONG Win32Minor, _Out_opt_ PULONG KdMajor,
      _Out_opt_ PULONG KdMinor) PURE;
  // Strings are selected with DEBUG_SYSVERSTR_*.
  STDMETHOD(GetSystemVersionString)(
      THIS_ _In_ ULONG Which, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG StringSize) PURE;
  STDMETHOD(GetSystemVersionStringWide)(
      THIS_ _In_ ULONG Which, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG StringSize) PURE;
  ...
  STDMETHOD(GetContextStackTrace)(
      THIS_ _In_reads_bytes_opt_(StartContextSize) PVOID StartContext,
      ...
      _In_ ULONG FrameContextsSize, _In_ ULONG FrameContextsEntrySize,
      _Out_opt_ PULONG FramesFilled) PURE;
  STDMETHOD(OutputContextStackTrace)(
      THIS_ _In_ ULONG OutputControl,
      _In_reads_(FramesSize) PDEBUG_STACK_FRAME Frames, _In_ ULONG FramesSize,
  ...
  STDMETHOD(GetStoredEventInformation)(
      THIS_ _Out_ PULONG Type, _Out_ PULONG ProcessId, _Out_ PULONG ThreadId,
      _Out_writes_bytes_opt_(ContextSize) PVOID Context, _In_ ULONG ContextSize,
  ...
  STDMETHOD(GetManagedStatus)(
      THIS_ _Out_opt_ PULONG Flags, _In_ ULONG WhichString,
      _Out_writes_opt_(StringSize) PSTR String, _In_ ULONG StringSize,
      _Out_opt_ PULONG StringNeeded) PURE;
  STDMETHOD(GetManagedStatusWide)(
      THIS_ _Out_opt_ PULONG Flags, _In_ ULONG WhichString,
      _Out_writes_opt_(StringSize) PWSTR String, _In_ ULONG StringSize,
      _Out_opt_ PULONG StringNeeded) PURE;
  // Clears and reinitializes the engine's
  // managed code debugging support.
  STDMETHOD(ResetManagedStatus)(THIS_ _In_ ULONG Flags) PURE;
};
...
DECLARE_INTERFACE_(IDebugControl5, IUnknown) {
  ...
  STDMETHOD(GetStackTrace)(
      ...
      _In_ ULONG FramesSize, _Out_opt_ PULONG FramesFilled) PURE;
  // Does a simple stack trace to determine
  // what the current return address is.
  STDMETHOD(GetReturnOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  // If Frames is NULL OutputStackTrace will
  ...
  STDMETHOD(OutputStackTraceEx)(THIS_ _In_ ULONG OutputControl,
                                _In_reads_opt_(FramesSize)
  ...
  STDMETHOD(GetContextStackTraceEx)(
      THIS_ _In_reads_bytes_opt_(StartContextSize) PVOID StartContext,
      _In_ ULONG StartContextSize,
      _Out_writes_to_opt_(FramesSize, *FramesFilled)
          PDEBUG_STACK_FRAME_EX Frames,
      _In_ ULONG FramesSize,
      _Out_writes_bytes_opt_(FrameContextsSize) PVOID FrameContexts,
      _In_ ULONG FrameContextsSize, _In_ ULONG FrameContextsEntrySize,
  ...
  STDMETHOD(GetBreakpointByGuid)(THIS_ _In_ LPGUID Guid,
                                 _Out_ PDEBUG_BREAKPOINT3 * Bp) PURE;
};
...
DECLARE_INTERFACE_(IDebugControl6, IUnknown) {
  ...
  STDMETHOD(GetExecutionStatusEx)(THIS_ _Out_ PULONG Status) PURE;

  STDMETHOD(GetSynchronizationStatus)(THIS_ _Out_ PULONG SendsAttempted,
...
DECLARE_INTERFACE_(IDebugControl7, IUnknown) {
  ...
  STDMETHOD(GetDebuggeeType2)(THIS_ _In_ ULONG Flags, _Out_ PULONG Class,
                              _Out_ PULONG Qualifier) PURE;
};
...
DECLARE_INTERFACE_(IDebugDataSpaces, IUnknown) {
  ...
  STDMETHOD(ReadVirtual)(
      THIS_ _In_ ULONG64 Offset, _Out_writes_bytes_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG BytesRead) PURE;
  STDMETHOD(WriteVirtual)(
      THIS_ _In_ ULONG64 Offset, _In_reads_bytes_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG BytesWritten) PURE;
  ...
  STDMETHOD(SearchVirtual)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG64 Length,
      _In_reads_bytes_(PatternSize) PVOID Pattern, _In_ ULONG PatternSize,
      _In_ ULONG PatternGranularity, _Out_ PULONG64 MatchOffset) PURE;
  ...
  STDMETHOD(ReadPointersVirtual)(THIS_ _In_ ULONG Count, _In_ ULONG64 Offset,
                                 _Out_writes_(Count) PULONG64 Ptrs) PURE;
  STDMETHOD(WritePointersVirtual)(THIS_ _In_ ULONG Count, _In_ ULONG64 Offset,
                                  _In_reads_(Count) PULONG64 Ptrs) PURE;
  ...
  STDMETHOD(WritePhysical)(
      THIS_ _In_ ULONG64 Offset, _In_reads_bytes_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG BytesWritten) PURE;
  STDMETHOD(ReadControl)(THIS_ _In_ ULONG Processor, _In_ ULONG64 Offset,
                         _Out_writes_bytes_(BufferSize) PVOID Buffer,
                         _In_ ULONG BufferSize, _Out_opt_ PULONG BytesRead)
      PURE;
  STDMETHOD(WriteControl)(THIS_ _In_ ULONG Processor, _In_ ULONG64 Offset,
                          _In_reads_bytes_(BufferSize) PVOID Buffer,
                          _In_ ULONG BufferSize, _Out_opt_ PULONG BytesWritten)
      PURE;
  STDMETHOD(ReadIo)(THIS_ _In_ ULONG InterfaceType, _In_ ULONG BusNumber,
                    _In_ ULONG AddressSpace, _In_ ULONG64 Offset,
                    _Out_writes_bytes_(BufferSize) PVOID Buffer,
                    _In_ ULONG BufferSize, _Out_opt_ PULONG BytesRead) PURE;
  STDMETHOD(WriteIo)(THIS_ _In_ ULONG InterfaceType, _In_ ULONG BusNumber,
                     _In_ ULONG AddressSpace, _In_ ULONG64 Offset,
                     _In_reads_bytes_(BufferSize) PVOID Buffer,
                     _In_ ULONG BufferSize, _Out_opt_ PULONG BytesWritten) PURE;
  STDMETHOD(ReadMsr)(THIS_ _In_ ULONG Msr, _Out_ PULONG64 Value) PURE;
  STDMETHOD(WriteMsr)(THIS_ _In_ ULONG Msr, _In_ ULONG64 Value) PURE;
  STDMETHOD(ReadBusData)(
      THIS_ _In_ ULONG BusDataType, _In_ ULONG BusNumber, _In_ ULONG SlotNumber,
      _In_ ULONG Offset, _Out_writes_bytes_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG BytesRead) PURE;
  STDMETHOD(WriteBusData)(
      THIS_ _In_ ULONG BusDataType, _In_ ULONG BusNumber, _In_ ULONG SlotNumber,
      _In_ ULONG Offset, _In_reads_bytes_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG BytesWritten) PURE;
  STDMETHOD(CheckLowMemory)(THIS) PURE;
  STDMETHOD(ReadDebuggerData)(
      THIS_ _In_ ULONG Index, _Out_writes_bytes_(BufferSize) PVOID Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG DataSize) PURE;
  STDMETHOD(ReadProcessorSystemData)(
      THIS_ _In_ ULONG Processor, _In_ ULONG Index,
      _Out_writes_bytes_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG DataSize) PURE;
};
...
DECLARE_INTERFACE_(IDebugDataSpaces2, IUnknown) {
  ...
  STDMETHOD(VirtualToPhysical)(THIS_ _In_ ULONG64 Virtual,
  ...
  STDMETHOD(GetVirtualTranslationPhysicalOffsets)(
      THIS_ _In_ ULONG64 Virtual,
      _Out_writes_opt_(OffsetsSize) PULONG64 Offsets, _In_ ULONG OffsetsSize,
      _Out_opt_ PULONG Levels) PURE;
  ...
  // to platform.
  STDMETHOD(ReadHandleData)(THIS_ _In_ ULONG64 Handle, _In_ ULONG DataType,
                            _Out_writes_bytes_opt_(BufferSize) PVOID Buffer,
                            _In_ ULONG BufferSize, _Out_opt_ PULONG DataSize)
      PURE;

  // Fills memory with the given pattern.
  // The fill stops at the first non-writable byte.
  STDMETHOD(FillVirtual)(THIS_ _In_ ULONG64 Start, _In_ ULONG Size,
                         _In_reads_bytes_(PatternSize) PVOID Pattern,
                         _In_ ULONG PatternSize, _Out_opt_ PULONG Filled) PURE;
  STDMETHOD(FillPhysical)(THIS_ _In_ ULONG64 Start, _In_ ULONG Size,
                          _In_reads_bytes_(PatternSize) PVOID Pattern,
                          _In_ ULONG PatternSize, _Out_opt_ PULONG Filled) PURE;
  ...
  STDMETHOD(QueryVirtual)(THIS_ _In_ ULONG64 Offset,
                          _Out_ PMEMORY_BASIC_INFORMATION64 Info) PURE;
};
...
DECLARE_INTERFACE_(IDebugDataSpaces3, IUnknown) {
  ...
  STDMETHOD(ReadImageNtHeaders)(THIS_ _In_ ULONG64 ImageBase,
                                _Out_ PIMAGE_NT_HEADERS64 Headers) PURE;
  ...
  // absent.
  STDMETHOD(ReadTagged)(THIS_ _In_ LPGUID Tag, _In_ ULONG Offset,
                        _Out_writes_bytes_opt_(BufferSize) PVOID Buffer,
                        _In_ ULONG BufferSize, _Out_opt_ PULONG TotalSize) PURE;
  STDMETHOD(StartEnumTagged)(THIS_ _Out_ PULONG64 Handle) PURE;
  STDMETHOD(GetNextTagged)(THIS_ _In_ ULONG64 Handle, _Out_ LPGUID Tag,
                           _Out_ PULONG Size) PURE;
  STDMETHOD(EndEnumTagged)(THIS_ _In_ ULONG64 Handle) PURE;
};
...
DECLARE_INTERFACE_(IDebugDataSpaces4, IUnknown) {
  ...
  STDMETHOD(GetOffsetInformation)(
      THIS_ _In_ ULONG Space, _In_ ULONG Which, _In_ ULONG64 Offset,
      _Out_writes_bytes_opt_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG InfoSize) PURE;
  ...
  STDMETHOD(GetNextDifferentlyValidOffsetVirtual)(
      THIS_ _In_ ULONG64 Offset, _Out_ PULONG64 NextOffset) PURE;

  // Given a particular range of virtual addresses,
  // find the first region which is valid memory.
  STDMETHOD(GetValidRegionVirtual)(THIS_ _In_ ULONG64 Base, _In_ ULONG Size,
                                   _Out_ PULONG64 ValidBase,
                                   _Out_ PULONG ValidSize) PURE;

  STDMETHOD(SearchVirtual2)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG64 Length, _In_ ULONG Flags,
      _In_reads_bytes_(PatternSize) PVOID Pattern, _In_ ULONG PatternSize,
      _In_ ULONG PatternGranularity, _Out_ PULONG64 MatchOffset) PURE;
  ...
  STDMETHOD(ReadMultiByteStringVirtual)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG MaxBytes,
      _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG StringBytes) PURE;
  // Reads a multi-byte string and converts
  // it to Unicode using the given code page.
  STDMETHOD(ReadMultiByteStringVirtualWide)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG MaxBytes, _In_ ULONG CodePage,
      _Out_writes_opt_(BufferSize) PWSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG StringBytes) PURE;
  ...
  STDMETHOD(ReadPhysical2)(THIS_ _In_ ULONG64 Offset, _In_ ULONG Flags,
                           _Out_writes_bytes_(BufferSize) PVOID Buffer,
                           _In_ ULONG BufferSize, _Out_opt_ PULONG BytesRead)
      PURE;
  STDMETHOD(WritePhysical2)(THIS_ _In_ ULONG64 Offset, _In_ ULONG Flags,
                            _In_reads_bytes_(BufferSize) PVOID Buffer,
                            _In_ ULONG BufferSize,
                            _Out_opt_ PULONG BytesWritten) PURE;
};
...
DECLARE_INTERFACE_(IDebugEventCallbacks, IUnknown) {
  ...
  STDMETHOD(GetInterestMask)(THIS_ _Out_ PULONG Mask) PURE;
  ...
  // it is done with it.
  STDMETHOD(Breakpoint)(THIS_ _In_ PDEBUG_BREAKPOINT Bp) PURE;

  // Exceptions include breaks which cannot
  // be mapped to an existing breakpoint
  // instance.
  STDMETHOD(Exception)(THIS_ _In_ PEXCEPTION_RECORD64 Exception,
                       _In_ ULONG FirstChance) PURE;
  ...
  STDMETHOD(CreateThread)(THIS_ _In_ ULONG64 Handle, _In_ ULONG64 DataOffset,
                          _In_ ULONG64 StartOffset) PURE;
  STDMETHOD(ExitThread)(THIS_ _In_ ULONG ExitCode) PURE;

  // Any of these values can be zero if they
  // cannot be provided by the engine.
  STDMETHOD(CreateProcess)(
      THIS_ _In_ ULONG64 ImageFileHandle, _In_ ULONG64 Handle,
      _In_ ULONG64 BaseOffset, _In_ ULONG ModuleSize, _In_opt_ PCSTR ModuleName,
      _In_opt_ PCSTR ImageName, _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp,
      _In_ ULONG64 InitialThreadHandle, _In_ ULONG64 ThreadDataOffset,
      _In_ ULONG64 StartOffset) PURE;
  _Analysis_noreturn_ STDMETHOD(ExitProcess)(THIS_ _In_ ULONG ExitCode) PURE;

  // Any of these values may be zero.
  STDMETHOD(LoadModule)(THIS_ _In_ ULONG64 ImageFileHandle,
                        _In_ ULONG64 BaseOffset, _In_ ULONG ModuleSize,
                        _In_opt_ PCSTR ModuleName, _In_opt_ PCSTR ImageName,
                        _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp) PURE;
  STDMETHOD(UnloadModule)(THIS_ _In_opt_ PCSTR ImageBaseName,
                          _In_ ULONG64 BaseOffset) PURE;

  STDMETHOD(SystemError)(THIS_ _In_ ULONG Error, _In_ ULONG Level) PURE;
  ...
  // is generated.
  STDMETHOD(SessionStatus)(THIS_ _In_ ULONG Status) PURE;
  ...
  // has changed.
  STDMETHOD(ChangeDebuggeeState)(THIS_ _In_ ULONG Flags, _In_ ULONG64 Argument)
      PURE;
  // Engine state has changed.
  STDMETHOD(ChangeEngineState)(THIS_ _In_ ULONG Flags, _In_ ULONG64 Argument)
      PURE;
  // Symbol state has changed.
  STDMETHOD(ChangeSymbolState)(THIS_ _In_ ULONG Flags, _In_ ULONG64 Argument)
      PURE;
};
...
DECLARE_INTERFACE_(IDebugEventCallbacksWide, IUnknown) {
  ...
  // to the breakpoint and should release it when
  // it is done with it.
  STDMETHOD(Breakpoint)(THIS_ _In_ PDEBUG_BREAKPOINT2 Bp) PURE;
  ...
  STDMETHOD(CreateThread)(THIS_ _In_ ULONG64 Handle, _In_ ULONG64 DataOffset,
                          _In_ ULONG64 StartOffset) PURE;
  _Analysis_noreturn_ STDMETHOD(ExitThread)(THIS_ _In_ ULONG ExitCode) PURE;
  ...
  STDMETHOD(CreateProcess)(
      THIS_ _In_ ULONG64 ImageFileHandle, _In_ ULONG64 Handle,
      _In_ ULONG64 BaseOffset, _In_ ULONG ModuleSize,
      _In_opt_ PCWSTR ModuleName, _In_opt_ PCWSTR ImageName,
      _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp,
      _In_ ULONG64 InitialThreadHandle, _In_ ULONG64 ThreadDataOffset,
      _In_ ULONG64 StartOffset) PURE;
  STDMETHOD(ExitProcess)(THIS_ _In_ ULONG ExitCode) PURE;
  ...
  STDMETHOD(LoadModule)(THIS_ _In_ ULONG64 ImageFileHandle,
                        ...
                        _In_opt_ PCWSTR ModuleName, _In_opt_ PCWSTR ImageName,
                        _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp) PURE;
  STDMETHOD(UnloadModule)(THIS_ _In_opt_ PCWSTR ImageBaseName,
                          _In_ ULONG64 BaseOffset) PURE;
...
DECLARE_INTERFACE_(IDebugEventContextCallbacks, IUnknown) {
  ...
  // to the breakpoint and should release it when
  // it is done with it.
  STDMETHOD(Breakpoint)(THIS_ _In_ PDEBUG_BREAKPOINT2 Bp,
  ...
  // instance.
  STDMETHOD(Exception)(
      THIS_ _In_ PEXCEPTION_RECORD64 Exception, _In_ ULONG FirstChance,
      _In_reads_bytes_(ContextSize) PVOID Context, _In_ ULONG ContextSize) PURE;
  ...
  STDMETHOD(CreateThread)(THIS_ _In_ ULONG64 Handle, _In_ ULONG64 DataOffset,
                          ...
                          _In_reads_bytes_(ContextSize) PVOID Context,
                          _In_ ULONG ContextSize) PURE;
  STDMETHOD(ExitThread)(THIS_ _In_ ULONG ExitCode,
                        _In_reads_bytes_(ContextSize) PVOID Context,
                        _In_ ULONG ContextSize) PURE;
  ...
  STDMETHOD(CreateProcess)(
      ...
      _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp,
      _In_ ULONG64 InitialThreadHandle, _In_ ULONG64 ThreadDataOffset,
      _In_ ULONG64 StartOffset, _In_reads_bytes_(ContextSize) PVOID Context,
      _In_ ULONG ContextSize) PURE;
  STDMETHOD(ExitProcess)(THIS_ _In_ ULONG ExitCode,
                         _In_reads_bytes_(ContextSize) PVOID Context,
                         _In_ ULONG ContextSize) PURE;

  // Any of these values may be zero.
  STDMETHOD(LoadModule)(
      THIS_ _In_ ULONG64 ImageFileHandle, _In_ ULONG64 BaseOffset,
      _In_ ULONG ModuleSize, _In_opt_ PCWSTR ModuleName,
      _In_opt_ PCWSTR ImageName, _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp,
      _In_reads_bytes_(ContextSize) PVOID Context, _In_ ULONG ContextSize) PURE;
  STDMETHOD(UnloadModule)(
      THIS_ _In_opt_ PCWSTR ImageBaseName, _In_ ULONG64 BaseOffset,
      _In_reads_bytes_(ContextSize) PVOID Context, _In_ ULONG ContextSize) PURE;

  STDMETHOD(SystemError)(THIS_ _In_ ULONG Error, _In_ ULONG Level,
                         _In_reads_bytes_(ContextSize) PVOID Context,
                         _In_ ULONG ContextSize) PURE;
...
DECLARE_INTERFACE_(IDebugInputCallbacks, IUnknown) {
  ...
  STDMETHOD(StartInput)(THIS_ _In_ ULONG BufferSize) PURE;
  // The return value is ignored.
...
DECLARE_INTERFACE_(IDebugOutputCallbacks, IUnknown) {
  ...
  // The return value is ignored.
  STDMETHOD(Output)(THIS_ _In_ ULONG Mask, _In_ PCSTR Text) PURE;
};
...
DECLARE_INTERFACE_(IDebugOutputCallbacksWide, IUnknown) {
  ...
  // is allowed by the clients output control.
  // The return value is ignored.
  STDMETHOD(Output)(THIS_ _In_ ULONG Mask, _In_ PCWSTR Text) PURE;
};
...
DECLARE_INTERFACE_(IDebugOutputCallbacks2, IUnknown) {
  ...
  STDMETHOD(Output2)(THIS_ _In_ ULONG Which, _In_ ULONG Flags, _In_ ULONG64 Arg,
                     _In_opt_ PCWSTR Text) PURE;
};
...
DECLARE_INTERFACE_(IDebugRegisters, IUnknown) {
  ...
  // IDebugRegisters.
  STDMETHOD(GetNumberRegisters)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetDescription)(THIS_ _In_ ULONG Register,
                            _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
                            _In_ ULONG NameBufferSize,
                            _Out_opt_ PULONG NameSize,
                            _Out_opt_ PDEBUG_REGISTER_DESCRIPTION Desc) PURE;
  STDMETHOD(GetIndexByName)(THIS_ _In_ PCSTR Name, _Out_ PULONG Index) PURE;

  STDMETHOD(GetValue)(THIS_ _In_ ULONG Register, _Out_ PDEBUG_VALUE Value) PURE;
  ...
  STDMETHOD(SetValue)(THIS_ _In_ ULONG Register, _In_ PDEBUG_VALUE Value) PURE;
  ...
  STDMETHOD(GetValues)(THIS_ _In_ ULONG Count,
                       _In_reads_opt_(Count) PULONG Indices, _In_ ULONG Start,
                       _Out_writes_(Count) PDEBUG_VALUE Values) PURE;
  STDMETHOD(SetValues)(THIS_ _In_ ULONG Count,
                       _In_reads_opt_(Count) PULONG Indices, _In_ ULONG Start,
                       _In_reads_(Count) PDEBUG_VALUE Values) PURE;

  // Outputs a group of registers in a well-formatted
  // way thats specific to the platforms register set.
  // Uses the line prefix.
  STDMETHOD(OutputRegisters)(THIS_ _In_ ULONG OutputControl, _In_ ULONG Flags)
      PURE;
  ...
  // stack walking.
  STDMETHOD(GetInstructionOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  STDMETHOD(GetStackOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  STDMETHOD(GetFrameOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
};
...
DECLARE_INTERFACE_(IDebugRegisters2, IUnknown) {
  ...
  STDMETHOD(GetDescriptionWide)(
      THIS_ _In_ ULONG Register,
      _Out_writes_opt_(NameBufferSize) PWSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize,
      _Out_opt_ PDEBUG_REGISTER_DESCRIPTION Desc) PURE;
  STDMETHOD(GetIndexByNameWide)(THIS_ _In_ PCWSTR Name, _Out_ PULONG Index)
      PURE;
  ...
  STDMETHOD(GetNumberPseudoRegisters)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetPseudoDescription)(
      THIS_ _In_ ULONG Register,
      _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize,
      _Out_opt_ PULONG64 TypeModule, _Out_opt_ PULONG TypeId) PURE;
  STDMETHOD(GetPseudoDescriptionWide)(
      THIS_ _In_ ULONG Register,
      _Out_writes_opt_(NameBufferSize) PWSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize,
      _Out_opt_ PULONG64 TypeModule, _Out_opt_ PULONG TypeId) PURE;
  STDMETHOD(GetPseudoIndexByName)(THIS_ _In_ PCSTR Name, _Out_ PULONG Index)
      PURE;
  STDMETHOD(GetPseudoIndexByNameWide)(THIS_ _In_ PCWSTR Name,
                                      _Out_ PULONG Index) PURE;
  // Some pseudo-register values are affected
  // by the register source, others are not.
  STDMETHOD(GetPseudoValues)(THIS_ _In_ ULONG Source, _In_ ULONG Count,
                             _In_reads_opt_(Count) PULONG Indices,
                             _In_ ULONG Start,
                             _Out_writes_(Count) PDEBUG_VALUE Values) PURE;
  // Many pseudo-registers are read-only and cannot be set.
  STDMETHOD(SetPseudoValues)(THIS_ _In_ ULONG Source, _In_ ULONG Count,
                             _In_reads_opt_(Count) PULONG Indices,
                             _In_ ULONG Start,
                             _In_reads_(Count) PDEBUG_VALUE Values) PURE;
  ...
  STDMETHOD(GetValues2)(THIS_ _In_ ULONG Source, _In_ ULONG Count,
                        _In_reads_opt_(Count) PULONG Indices, _In_ ULONG Start,
                        _Out_writes_(Count) PDEBUG_VALUE Values) PURE;
  STDMETHOD(SetValues2)(THIS_ _In_ ULONG Source, _In_ ULONG Count,
                        _In_reads_opt_(Count) PULONG Indices, _In_ ULONG Start,
                        _In_reads_(Count) PDEBUG_VALUE Values) PURE;
  STDMETHOD(OutputRegisters2)(THIS_ _In_ ULONG OutputControl, _In_ ULONG Source,
                              _In_ ULONG Flags) PURE;
  STDMETHOD(GetInstructionOffset2)(THIS_ _In_ ULONG Source,
                                   _Out_ PULONG64 Offset) PURE;
  STDMETHOD(GetStackOffset2)(THIS_ _In_ ULONG Source, _Out_ PULONG64 Offset)
      PURE;
  STDMETHOD(GetFrameOffset2)(THIS_ _In_ ULONG Source, _Out_ PULONG64 Offset)
      PURE;
};
...
DECLARE_INTERFACE_(IDebugSymbolGroup, IUnknown) {
  ...
  // IDebugSymbolGroup.
  STDMETHOD(GetNumberSymbols)(THIS_ _Out_ PULONG Number) PURE;
  ...
  STDMETHOD(AddSymbol)(THIS_ _In_ PCSTR Name, _Inout_ PULONG Index) PURE;
  STDMETHOD(RemoveSymbolByName)(THIS_ _In_ PCSTR Name) PURE;
  STDMETHOD(RemoveSymbolByIndex)(THIS_ _In_ ULONG Index) PURE;
  STDMETHOD(GetSymbolName)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG NameSize) PURE;
  STDMETHOD(GetSymbolParameters)(THIS_ _In_ ULONG Start, _In_ ULONG Count,
                                 _Out_writes_(Count)
                                     PDEBUG_SYMBOL_PARAMETERS Params) PURE;
  STDMETHOD(ExpandSymbol)(THIS_ _In_ ULONG Index, _In_ BOOL Expand) PURE;
  // Uses the line prefix.
  STDMETHOD(OutputSymbols)(THIS_ _In_ ULONG OutputControl, _In_ ULONG Flags,
                           _In_ ULONG Start, _In_ ULONG Count) PURE;
  STDMETHOD(WriteSymbol)(THIS_ _In_ ULONG Index, _In_ PCSTR Value) PURE;
  STDMETHOD(OutputAsType)(THIS_ _In_ ULONG Index, _In_ PCSTR Type) PURE;
};
...
DECLARE_INTERFACE_(IDebugSymbolGroup2, IUnknown) {
  ...
  STDMETHOD(AddSymbolWide)(THIS_ _In_ PCWSTR Name, _Inout_ PULONG Index) PURE;
  STDMETHOD(RemoveSymbolByNameWide)(THIS_ _In_ PCWSTR Name) PURE;
  STDMETHOD(GetSymbolNameWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG NameSize) PURE;
  STDMETHOD(WriteSymbolWide)(THIS_ _In_ ULONG Index, _In_ PCWSTR Value) PURE;
  STDMETHOD(OutputAsTypeWide)(THIS_ _In_ ULONG Index, _In_ PCWSTR Type) PURE;
  ...
  STDMETHOD(GetSymbolTypeNameWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG NameSize) PURE;
  STDMETHOD(GetSymbolSize)(THIS_ _In_ ULONG Index, _Out_ PULONG Size) PURE;
  // If the symbol has an absolute address
  // this method will retrieve it.
  STDMETHOD(GetSymbolOffset)(THIS_ _In_ ULONG Index, _Out_ PULONG64 Offset)
      PURE;
  // If the symbol is enregistered this
  // method will return the register index.
  STDMETHOD(GetSymbolRegister)(THIS_ _In_ ULONG Index, _Out_ PULONG Register)
      PURE;
  ...
  STDMETHOD(GetSymbolValueTextWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG NameSize) PURE;
  STDMETHOD(GetSymbolEntryInformation)(THIS_ _In_ ULONG Index,
                                       _Out_ PDEBUG_SYMBOL_ENTRY Entry) PURE;
};
...
#define DEBUG_SCOPE_GROUP_VALID_FLAGS                                          \
...
// a source file from various source code repositories. This means that the
// downloaded file would have a checksum which depends on file content and the
// updated line endings. The engine will try to match the checksum of the file
// to the requested checksum. If they don't match, the engine will calculate an
// alternative checksum of the file with modified line endings and will try to
// match it to the requested checksum. In summary:
// DEBUG_FIND_SOURCE_WITH_CHECKSUM_STRICT will return a file if its checksum
// matches to the requested checksum, or if the file's checksum calculated based
// on updated line endings matches the requested checksum.
...
DECLARE_INTERFACE_(IDebugSymbols, IUnknown) {
  ...
  STDMETHOD(GetSymbolOptions)(THIS_ _Out_ PULONG Options) PURE;
  STDMETHOD(AddSymbolOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(RemoveSymbolOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(SetSymbolOptions)(THIS_ _In_ ULONG Options) PURE;

  STDMETHOD(GetNameByOffset)(THIS_ _In_ ULONG64 Offset,
                             _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
                             _In_ ULONG NameBufferSize,
                             _Out_opt_ PULONG NameSize,
                             _Out_opt_ PULONG64 Displacement) PURE;
  ...
  STDMETHOD(GetOffsetByName)(THIS_ _In_ PCSTR Symbol, _Out_ PULONG64 Offset)
      PURE;
  ...
  STDMETHOD(GetNearNameByOffset)(
      THIS_ _In_ ULONG64 Offset, _In_ LONG Delta,
      _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize,
      _Out_opt_ PULONG64 Displacement) PURE;

  STDMETHOD(GetLineByOffset)(THIS_ _In_ ULONG64 Offset, _Out_opt_ PULONG Line,
                             _Out_writes_opt_(FileBufferSize) PSTR FileBuffer,
                             _In_ ULONG FileBufferSize,
                             _Out_opt_ PULONG FileSize,
                             _Out_opt_ PULONG64 Displacement) PURE;
  STDMETHOD(GetOffsetByLine)(THIS_ _In_ ULONG Line, _In_ PCSTR File,
                             _Out_ PULONG64 Offset) PURE;
  ...
  // modules.
  STDMETHOD(GetNumberModules)(THIS_ _Out_ PULONG Loaded, _Out_ PULONG Unloaded)
      PURE;
  STDMETHOD(GetModuleByIndex)(THIS_ _In_ ULONG Index, _Out_ PULONG64 Base) PURE;
  // The module name may not be unique.
  // This method returns the first match.
  STDMETHOD(GetModuleByModuleName)(THIS_ _In_ PCSTR Name, _In_ ULONG StartIndex,
                                   _Out_opt_ PULONG Index,
                                   _Out_opt_ PULONG64 Base) PURE;
  ...
  // first match.
  STDMETHOD(GetModuleByOffset)(THIS_ _In_ ULONG64 Offset, _In_ ULONG StartIndex,
                               _Out_opt_ PULONG Index, _Out_opt_ PULONG64 Base)
      PURE;
  // If Index is DEBUG_ANY_ID the base address
  // is used to look up the module instead.
  STDMETHOD(GetModuleNames)(
      THIS_ _In_ ULONG Index, _In_ ULONG64 Base,
      _Out_writes_opt_(ImageNameBufferSize) PSTR ImageNameBuffer,
      _In_ ULONG ImageNameBufferSize, _Out_opt_ PULONG ImageNameSize,
      _Out_writes_opt_(ModuleNameBufferSize) PSTR ModuleNameBuffer,
      _In_ ULONG ModuleNameBufferSize, _Out_opt_ PULONG ModuleNameSize,
      _Out_writes_opt_(LoadedImageNameBufferSize) PSTR LoadedImageNameBuffer,
      _In_ ULONG LoadedImageNameBufferSize,
      _Out_opt_ PULONG LoadedImageNameSize) PURE;
  STDMETHOD(GetModuleParameters)(
      THIS_ _In_ ULONG Count, _In_reads_opt_(Count) PULONG64 Bases,
      _In_ ULONG Start, _Out_writes_(Count) PDEBUG_MODULE_PARAMETERS Params)
      PURE;
  // Looks up the module from a <Module>!<Symbol>
  // string.
  STDMETHOD(GetSymbolModule)(THIS_ _In_ PCSTR Symbol, _Out_ PULONG64 Base) PURE;

  // Returns the string name of a type.
  STDMETHOD(GetTypeName)(THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId,
                         _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
                         _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize)
      PURE;
  // Returns the ID for a type name.
  STDMETHOD(GetTypeId)(THIS_ _In_ ULONG64 Module, _In_ PCSTR Name,
                       _Out_ PULONG TypeId) PURE;
  STDMETHOD(GetTypeSize)(THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId,
                         _Out_ PULONG Size) PURE;
  ...
  STDMETHOD(GetFieldOffset)(THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId,
                            _In_ PCSTR Field, _Out_ PULONG Offset) PURE;

  STDMETHOD(GetSymbolTypeId)(THIS_ _In_ PCSTR Symbol, _Out_ PULONG TypeId,
                             _Out_opt_ PULONG64 Module) PURE;
  ...
  // to avoid ambiguity.
  STDMETHOD(GetOffsetTypeId)(THIS_ _In_ ULONG64 Offset, _Out_ PULONG TypeId,
                             _Out_opt_ PULONG64 Module) PURE;
  ...
  STDMETHOD(ReadTypedDataVirtual)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG64 Module, _In_ ULONG TypeId,
      _Out_writes_bytes_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG BytesRead) PURE;
  STDMETHOD(WriteTypedDataVirtual)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG64 Module, _In_ ULONG TypeId,
      _In_reads_bytes_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG BytesWritten) PURE;
  STDMETHOD(OutputTypedDataVirtual)(THIS_ _In_ ULONG OutputControl,
                                    _In_ ULONG64 Offset, _In_ ULONG64 Module,
                                    _In_ ULONG TypeId, _In_ ULONG Flags) PURE;
  ...
  STDMETHOD(WriteTypedDataPhysical)(
      ...
      _In_reads_bytes_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG BytesWritten) PURE;
  STDMETHOD(OutputTypedDataPhysical)(THIS_ _In_ ULONG OutputControl,
                                     _In_ ULONG64 Offset, _In_ ULONG64 Module,
                                     _In_ ULONG TypeId, _In_ ULONG Flags) PURE;
  ...
  STDMETHOD(GetScope)(THIS_ _Out_opt_ PULONG64 InstructionOffset,
                      _Out_opt_ PDEBUG_STACK_FRAME ScopeFrame,
                      _Out_writes_bytes_opt_(ScopeContextSize)
                          PVOID ScopeContext,
                      _In_ ULONG ScopeContextSize) PURE;
  ...
  STDMETHOD(SetScope)(THIS_ _In_ ULONG64 InstructionOffset,
  ...
  STDMETHOD(GetScopeSymbolGroup)(THIS_ _In_ ULONG Flags,
                                 _In_opt_ PDEBUG_SYMBOL_GROUP Update,
                                 _Out_ PDEBUG_SYMBOL_GROUP * Symbols) PURE;

  // Create a new symbol group.
  STDMETHOD(CreateSymbolGroup)(THIS_ _Out_ PDEBUG_SYMBOL_GROUP * Group) PURE;
  ...
  STDMETHOD(StartSymbolMatch)(THIS_ _In_ PCSTR Pattern, _Out_ PULONG64 Handle)
      PURE;
  // If Buffer is NULL the match does not
  // advance.
  STDMETHOD(GetNextSymbolMatch)(
      THIS_ _In_ ULONG64 Handle, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG MatchSize,
      _Out_opt_ PULONG64 Offset) PURE;
  STDMETHOD(EndSymbolMatch)(THIS_ _In_ ULONG64 Handle) PURE;

  STDMETHOD(Reload)(THIS_ _In_ PCSTR Module) PURE;

  STDMETHOD(GetSymbolPath)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                           _In_ ULONG BufferSize, _Out_opt_ PULONG PathSize)
      PURE;
  STDMETHOD(SetSymbolPath)(THIS_ _In_ PCSTR Path) PURE;
  STDMETHOD(AppendSymbolPath)(THIS_ _In_ PCSTR Addition) PURE;
  ...
  // images.
  STDMETHOD(GetImagePath)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                          _In_ ULONG BufferSize, _Out_opt_ PULONG PathSize)
      PURE;
  STDMETHOD(SetImagePath)(THIS_ _In_ PCSTR Path) PURE;
  STDMETHOD(AppendImagePath)(THIS_ _In_ PCSTR Addition) PURE;

  // Path routines for source file location
  // methods.
  STDMETHOD(GetSourcePath)(THIS_ _Out_writes_opt_(BufferSize) PSTR Buffer,
                           _In_ ULONG BufferSize, _Out_opt_ PULONG PathSize)
      PURE;
  // Gets the nth part of the source path.
  STDMETHOD(GetSourcePathElement)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG ElementSize) PURE;
  STDMETHOD(SetSourcePath)(THIS_ _In_ PCSTR Path) PURE;
  STDMETHOD(AppendSourcePath)(THIS_ _In_ PCSTR Addition) PURE;
  ...
  STDMETHOD(FindSourceFile)(
      THIS_ _In_ ULONG StartElement, _In_ PCSTR File, _In_ ULONG Flags,
      _Out_opt_ PULONG FoundElement, _Out_writes_opt_(BufferSize) PSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG FoundSize) PURE;
  ...
  STDMETHOD(GetSourceFileLineOffsets)(
      THIS_ _In_ PCSTR File, _Out_writes_opt_(BufferLines) PULONG64 Buffer,
      _In_ ULONG BufferLines, _Out_opt_ PULONG FileLines) PURE;
};
...
DECLARE_INTERFACE_(IDebugSymbols2, IUnknown) {
  ...
  STDMETHOD(GetModuleVersionInformation)(
      THIS_ _In_ ULONG Index, _In_ ULONG64 Base, _In_ PCSTR Item,
      _Out_writes_bytes_opt_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG VerInfoSize) PURE;
  ...
  STDMETHOD(GetModuleNameString)(
      THIS_ _In_ ULONG Which, _In_ ULONG Index, _In_ ULONG64 Base,
      _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG NameSize) PURE;

  // Returns the string name of a constant type.
  STDMETHOD(GetConstantName)(
      THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId, _In_ ULONG64 Value,
      _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize) PURE;

  // Gets name of a field in a struct
  // FieldNumber is 0 based index of field in a struct
  STDMETHOD(GetFieldName)(
      THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId, _In_ ULONG FieldIndex,
      _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize) PURE;

  // Control options for typed values.
  STDMETHOD(GetTypeOptions)(THIS_ _Out_ PULONG Options) PURE;
  STDMETHOD(AddTypeOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(RemoveTypeOptions)(THIS_ _In_ ULONG Options) PURE;
  STDMETHOD(SetTypeOptions)(THIS_ _In_ ULONG Options) PURE;
};
...
DECLARE_INTERFACE_(IDebugSymbols3, IUnknown) {
  ...
  STDMETHOD(GetNameByOffsetWide)(
      THIS_ _In_ ULONG64 Offset,
      _Out_writes_opt_(NameBufferSize) PWSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize,
      _Out_opt_ PULONG64 Displacement) PURE;
  STDMETHOD(GetOffsetByNameWide)(THIS_ _In_ PCWSTR Symbol,
                                 _Out_ PULONG64 Offset) PURE;
  ...
  STDMETHOD(GetLineByOffsetWide)(
      THIS_ _In_ ULONG64 Offset, _Out_opt_ PULONG Line,
      _Out_writes_opt_(FileBufferSize) PWSTR FileBuffer,
      _In_ ULONG FileBufferSize, _Out_opt_ PULONG FileSize,
      _Out_opt_ PULONG64 Displacement) PURE;
  STDMETHOD(GetOffsetByLineWide)(THIS_ _In_ ULONG Line, _In_ PCWSTR File,
                                 _Out_ PULONG64 Offset) PURE;

  STDMETHOD(GetModuleByModuleNameWide)(
      THIS_ _In_ PCWSTR Name, _In_ ULONG StartIndex, _Out_opt_ PULONG Index,
      _Out_opt_ PULONG64 Base) PURE;
  STDMETHOD(GetSymbolModuleWide)(THIS_ _In_ PCWSTR Symbol, _Out_ PULONG64 Base)
      PURE;

  STDMETHOD(GetTypeNameWide)(THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId,
                             _Out_writes_opt_(NameBufferSize) PWSTR NameBuffer,
                             _In_ ULONG NameBufferSize,
                             _Out_opt_ PULONG NameSize) PURE;
  // Returns the ID for a type name.
  STDMETHOD(GetTypeIdWide)(THIS_ _In_ ULONG64 Module, _In_ PCWSTR Name,
                           _Out_ PULONG TypeId) PURE;
  STDMETHOD(GetFieldOffsetWide)(THIS_ _In_ ULONG64 Module, _In_ ULONG TypeId,
                                _In_ PCWSTR Field, _Out_ PULONG Offset) PURE;

  STDMETHOD(GetSymbolTypeIdWide)(THIS_ _In_ PCWSTR Symbol, _Out_ PULONG TypeId,
                                 _Out_opt_ PULONG64 Module) PURE;

  STDMETHOD(GetScopeSymbolGroup2)(THIS_ _In_ ULONG Flags,
                                  _In_opt_ PDEBUG_SYMBOL_GROUP2 Update,
                                  _Out_ PDEBUG_SYMBOL_GROUP2 * Symbols) PURE;

  STDMETHOD(CreateSymbolGroup2)(THIS_ _Out_ PDEBUG_SYMBOL_GROUP2 * Group) PURE;

  STDMETHOD(StartSymbolMatchWide)(THIS_ _In_ PCWSTR Pattern,
                                  _Out_ PULONG64 Handle) PURE;
  STDMETHOD(GetNextSymbolMatchWide)(
      THIS_ _In_ ULONG64 Handle, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG MatchSize,
      _Out_opt_ PULONG64 Offset) PURE;

  STDMETHOD(ReloadWide)(THIS_ _In_ PCWSTR Module) PURE;

  STDMETHOD(GetSymbolPathWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                               _In_ ULONG BufferSize, _Out_opt_ PULONG PathSize)
      PURE;
  STDMETHOD(SetSymbolPathWide)(THIS_ _In_ PCWSTR Path) PURE;
  STDMETHOD(AppendSymbolPathWide)(THIS_ _In_ PCWSTR Addition) PURE;

  STDMETHOD(GetImagePathWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                              _In_ ULONG BufferSize, _Out_opt_ PULONG PathSize)
      PURE;
  STDMETHOD(SetImagePathWide)(THIS_ _In_ PCWSTR Path) PURE;
  STDMETHOD(AppendImagePathWide)(THIS_ _In_ PCWSTR Addition) PURE;

  STDMETHOD(GetSourcePathWide)(THIS_ _Out_writes_opt_(BufferSize) PWSTR Buffer,
                               _In_ ULONG BufferSize, _Out_opt_ PULONG PathSize)
      PURE;
  STDMETHOD(GetSourcePathElementWide)(
      THIS_ _In_ ULONG Index, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG ElementSize) PURE;
  STDMETHOD(SetSourcePathWide)(THIS_ _In_ PCWSTR Path) PURE;
  STDMETHOD(AppendSourcePathWide)(THIS_ _In_ PCWSTR Addition) PURE;
  STDMETHOD(FindSourceFileWide)(
      THIS_ _In_ ULONG StartElement, _In_ PCWSTR File, _In_ ULONG Flags,
      _Out_opt_ PULONG FoundElement, _Out_writes_opt_(BufferSize) PWSTR Buffer,
      _In_ ULONG BufferSize, _Out_opt_ PULONG FoundSize) PURE;
  STDMETHOD(GetSourceFileLineOffsetsWide)(
      THIS_ _In_ PCWSTR File, _Out_writes_opt_(BufferLines) PULONG64 Buffer,
      _In_ ULONG BufferLines, _Out_opt_ PULONG FileLines) PURE;

  STDMETHOD(GetModuleVersionInformationWide)(
      THIS_ _In_ ULONG Index, _In_ ULONG64 Base, _In_ PCWSTR Item,
      _Out_writes_bytes_opt_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG VerInfoSize) PURE;
  ...
  // to check.
  STDMETHOD(IsManagedModule)(THIS_ _In_ ULONG Index, _In_ ULONG64 Base) PURE;

  // The module name may not be unique.
  // This method returns the first match.
  STDMETHOD(GetModuleByModuleName2)(
      THIS_ _In_ PCSTR Name, _In_ ULONG StartIndex, _In_ ULONG Flags,
      _Out_opt_ PULONG Index, _Out_opt_ PULONG64 Base) PURE;
  STDMETHOD(GetModuleByModuleName2Wide)(
      THIS_ _In_ PCWSTR Name, _In_ ULONG StartIndex, _In_ ULONG Flags,
      _Out_opt_ PULONG Index, _Out_opt_ PULONG64 Base) PURE;
  // Offset can be any offset within
  // the module extent.  Extents may
  // not be unique when including unloaded
  ...
  STDMETHOD(GetModuleByOffset2)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG StartIndex, _In_ ULONG Flags,
      _Out_opt_ PULONG Index, _Out_opt_ PULONG64 Base) PURE;
  ...
  STDMETHOD(AddSyntheticModule)(THIS_ _In_ ULONG64 Base, _In_ ULONG Size,
                                _In_ PCSTR ImagePath, _In_ PCSTR ModuleName,
                                _In_ ULONG Flags) PURE;
  STDMETHOD(AddSyntheticModuleWide)(
      THIS_ _In_ ULONG64 Base, _In_ ULONG Size, _In_ PCWSTR ImagePath,
      _In_ PCWSTR ModuleName, _In_ ULONG Flags) PURE;
  STDMETHOD(RemoveSyntheticModule)(THIS_ _In_ ULONG64 Base) PURE;

  // Modify the current frame used for scoping.
  // This is equivalent to the '.frame' command.
  STDMETHOD(GetCurrentScopeFrameIndex)(THIS_ _Out_ PULONG Index) PURE;
  STDMETHOD(SetScopeFrameByIndex)(THIS_ _In_ ULONG Index) PURE;
  ...
  STDMETHOD(SetScopeFromJitDebugInfo)(THIS_ _In_ ULONG OutputControl,
                                      _In_ ULONG64 InfoOffset) PURE;
  ...
  STDMETHOD(OutputSymbolByOffset)(THIS_ _In_ ULONG OutputControl,
                                  _In_ ULONG Flags, _In_ ULONG64 Offset) PURE;
  ...
  STDMETHOD(GetFunctionEntryByOffset)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG Flags,
      _Out_writes_bytes_opt_(BufferSize) PVOID Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG BufferNeeded) PURE;
  ...
  STDMETHOD(GetFieldTypeAndOffset)(
      THIS_ _In_ ULONG64 Module, _In_ ULONG ContainerTypeId, _In_ PCSTR Field,
      _Out_opt_ PULONG FieldTypeId, _Out_opt_ PULONG Offset) PURE;
  STDMETHOD(GetFieldTypeAndOffsetWide)(
      THIS_ _In_ ULONG64 Module, _In_ ULONG ContainerTypeId, _In_ PCWSTR Field,
      _Out_opt_ PULONG FieldTypeId, _Out_opt_ PULONG Offset) PURE;
  ...
  STDMETHOD(AddSyntheticSymbol)(THIS_ _In_ ULONG64 Offset, _In_ ULONG Size,
                                _In_ PCSTR Name, _In_ ULONG Flags,
                                _Out_opt_ PDEBUG_MODULE_AND_ID Id) PURE;
  STDMETHOD(AddSyntheticSymbolWide)(THIS_ _In_ ULONG64 Offset, _In_ ULONG Size,
                                    _In_ PCWSTR Name, _In_ ULONG Flags,
                                    _Out_opt_ PDEBUG_MODULE_AND_ID Id) PURE;
  STDMETHOD(RemoveSyntheticSymbol)(THIS_ _In_ PDEBUG_MODULE_AND_ID Id) PURE;
  ...
  STDMETHOD(GetSymbolEntriesByOffset)(
      ...
      _Out_writes_opt_(IdsCount) PULONG64 Displacements, _In_ ULONG IdsCount,
      _Out_opt_ PULONG Entries) PURE;
  STDMETHOD(GetSymbolEntriesByName)(
      THIS_ _In_ PCSTR Symbol, _In_ ULONG Flags,
      _Out_writes_opt_(IdsCount) PDEBUG_MODULE_AND_ID Ids, _In_ ULONG IdsCount,
      _Out_opt_ PULONG Entries) PURE;
  STDMETHOD(GetSymbolEntriesByNameWide)(
      THIS_ _In_ PCWSTR Symbol, _In_ ULONG Flags,
      _Out_writes_opt_(IdsCount) PDEBUG_MODULE_AND_ID Ids, _In_ ULONG IdsCount,
      _Out_opt_ PULONG Entries) PURE;
  // Symbol lookup by managed metadata token.
  STDMETHOD(GetSymbolEntryByToken)(THIS_ _In_ ULONG64 ModuleBase,
                                   _In_ ULONG Token,
                                   _Out_ PDEBUG_MODULE_AND_ID Id) PURE;

  // Retrieves full symbol entry information from an ID.
  STDMETHOD(GetSymbolEntryInformation)(THIS_ _In_ PDEBUG_MODULE_AND_ID Id,
                                       _Out_ PDEBUG_SYMBOL_ENTRY Info) PURE;
  STDMETHOD(GetSymbolEntryString)(
      THIS_ _In_ PDEBUG_MODULE_AND_ID Id, _In_ ULONG Which,
      _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG StringSize) PURE;
  ...
  STDMETHOD(GetSymbolEntryOffsetRegions)(
      THIS_ _In_ PDEBUG_MODULE_AND_ID Id, _In_ ULONG Flags,
      _Out_writes_opt_(RegionsCount) PDEBUG_OFFSET_REGION Regions,
      _In_ ULONG RegionsCount, _Out_opt_ PULONG RegionsAvail) PURE;

  // This method allows navigating within the
  // symbol entry hierarchy.
  STDMETHOD(GetSymbolEntryBySymbolEntry)(THIS_ _In_ PDEBUG_MODULE_AND_ID FromId,
                                         _In_ ULONG Flags,
                                         _Out_ PDEBUG_MODULE_AND_ID ToId) PURE;
  ...
  STDMETHOD(GetSourceEntriesByOffset)(
      ...
      _In_ ULONG EntriesCount, _Out_opt_ PULONG EntriesAvail) PURE;
  STDMETHOD(GetSourceEntriesByLine)(
      THIS_ _In_ ULONG Line, _In_ PCSTR File, _In_ ULONG Flags,
      _Out_writes_opt_(EntriesCount) PDEBUG_SYMBOL_SOURCE_ENTRY Entries,
      _In_ ULONG EntriesCount, _Out_opt_ PULONG EntriesAvail) PURE;
  STDMETHOD(GetSourceEntriesByLineWide)(
      THIS_ _In_ ULONG Line, _In_ PCWSTR File, _In_ ULONG Flags,
      _Out_writes_opt_(EntriesCount) PDEBUG_SYMBOL_SOURCE_ENTRY Entries,
      _In_ ULONG EntriesCount, _Out_opt_ PULONG EntriesAvail) PURE;

  STDMETHOD(GetSourceEntryString)(
      THIS_ _In_ PDEBUG_SYMBOL_SOURCE_ENTRY Entry, _In_ ULONG Which,
      _Out_writes_opt_(BufferSize) PSTR Buffer, _In_ ULONG BufferSize,
      _Out_opt_ PULONG StringSize) PURE;
  ...
  STDMETHOD(GetSourceEntryOffsetRegions)(
      THIS_ _In_ PDEBUG_SYMBOL_SOURCE_ENTRY Entry, _In_ ULONG Flags,
      _Out_writes_opt_(RegionsCount) PDEBUG_OFFSET_REGION Regions,
      _In_ ULONG RegionsCount, _Out_opt_ PULONG RegionsAvail) PURE;
  ...
  STDMETHOD(GetSourceEntryBySourceEntry)(
      THIS_ _In_ PDEBUG_SYMBOL_SOURCE_ENTRY FromEntry, _In_ ULONG Flags,
      _Out_ PDEBUG_SYMBOL_SOURCE_ENTRY ToEntry) PURE;
};
...
DECLARE_INTERFACE_(IDebugSymbols4, IUnknown) {
  ...
  // IDebugSymbols4
  STDMETHOD(GetScopeEx)(THIS_ _Out_opt_ PULONG64 InstructionOffset,
                        _Out_opt_ PDEBUG_STACK_FRAME_EX ScopeFrame,
                        _Out_writes_bytes_opt_(ScopeContextSize)
                            PVOID ScopeContext,
                        _In_ ULONG ScopeContextSize) PURE;

  STDMETHOD(SetScopeEx)(THIS_ _In_ ULONG64 InstructionOffset,
                        _In_opt_ PDEBUG_STACK_FRAME_EX ScopeFrame,
                        _In_reads_bytes_opt_(ScopeContextSize)
                            PVOID ScopeContext,
                        _In_ ULONG ScopeContextSize) PURE;

  STDMETHOD(GetNameByInlineContext)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG InlineContext,
      _Out_writes_opt_(NameBufferSize) PSTR NameBuffer,
      _In_ ULONG NameBufferSize, _Out_opt_ PULONG NameSize,
      _Out_opt_ PULONG64 Displacement) PURE;
  ...
  STDMETHOD(GetLineByInlineContext)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG InlineContext,
      _Out_opt_ PULONG Line, _Out_writes_opt_(FileBufferSize) PSTR FileBuffer,
      _In_ ULONG FileBufferSize, _Out_opt_ PULONG FileSize,
      _Out_opt_ PULONG64 Displacement) PURE;

  STDMETHOD(GetLineByInlineContextWide)(
      THIS_ _In_ ULONG64 Offset, _In_ ULONG InlineContext,
      _Out_opt_ PULONG Line, _Out_writes_opt_(FileBufferSize) PWSTR FileBuffer,
      _In_ ULONG FileBufferSize, _Out_opt_ PULONG FileSize,
      _Out_opt_ PULONG64 Displacement) PURE;

  STDMETHOD(OutputSymbolByInlineContext)(THIS_ _In_ ULONG OutputControl,
                                         _In_ ULONG Flags, _In_ ULONG64 Offset,
                                         _In_ ULONG InlineContext) PURE;
};
...
DECLARE_INTERFACE_(IDebugSymbols5, IUnknown) {
  ...
  // IDebugSymbols5
  STDMETHOD(GetCurrentScopeFrameIndexEx)(THIS_ _In_ ULONG Flags,
                                         _Out_ PULONG Index) PURE;
  STDMETHOD(SetScopeFrameByIndexEx)(THIS_ _In_ ULONG Flags, _In_ ULONG Index)
      PURE;
};
...
DECLARE_INTERFACE_(IDebugSystemObjects, IUnknown) {
  ...
  STDMETHOD(GetEventThread)(THIS_ _Out_ PULONG Id) PURE;
  STDMETHOD(GetEventProcess)(THIS_ _Out_ PULONG Id) PURE;
  ...
  STDMETHOD(GetCurrentThreadId)(THIS_ _Out_ PULONG Id) PURE;
  STDMETHOD(SetCurrentThreadId)(THIS_ _In_ ULONG Id) PURE;
  // The current process is the process
  // that owns the current thread.
  STDMETHOD(GetCurrentProcessId)(THIS_ _Out_ PULONG Id) PURE;
  // Setting the current process automatically
  // sets the current thread to the thread that
  // was last current in that process.
  STDMETHOD(SetCurrentProcessId)(THIS_ _In_ ULONG Id) PURE;

  // Gets the number of threads in the current process.
  STDMETHOD(GetNumberThreads)(THIS_ _Out_ PULONG Number) PURE;
  // Gets thread count information for all processes
  // and the largest number of threads in a single process.
  STDMETHOD(GetTotalNumberThreads)(THIS_ _Out_ PULONG Total,
                                   _Out_ PULONG LargestProcess) PURE;
  STDMETHOD(GetThreadIdsByIndex)(THIS_ _In_ ULONG Start, _In_ ULONG Count,
                                 _Out_writes_opt_(Count) PULONG Ids,
                                 _Out_writes_opt_(Count) PULONG SysIds) PURE;
  // Gets the debugger ID for the thread
  // currently running on the given
  // processor.  Only works in kernel
  // debugging.
  STDMETHOD(GetThreadIdByProcessor)(THIS_ _In_ ULONG Processor, _Out_ PULONG Id)
      PURE;
  ...
  // of the current TEB.
  STDMETHOD(GetCurrentThreadDataOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  ...
  STDMETHOD(GetThreadIdByDataOffset)(THIS_ _In_ ULONG64 Offset, _Out_ PULONG Id)
      PURE;
  // Returns the offset of the current threads
  // TEB.  In user mode this is equivalent to
  // the threads data offset.
  STDMETHOD(GetCurrentThreadTeb)(THIS_ _Out_ PULONG64 Offset) PURE;
  // Looks up a debugger thread ID for the given TEB.
  // Currently when kernel debugging this will fail
  // if the thread is not executing on a processor.
  STDMETHOD(GetThreadIdByTeb)(THIS_ _In_ ULONG64 Offset, _Out_ PULONG Id) PURE;
  // Returns the system unique ID for the current thread.
  // Not currently supported when kernel debugging.
  STDMETHOD(GetCurrentThreadSystemId)(THIS_ _Out_ PULONG SysId) PURE;
  // Looks up a debugger thread ID for the given
  // system thread ID.
  // Currently when kernel debugging this will fail
  // if the thread is not executing on a processor.
  STDMETHOD(GetThreadIdBySystemId)(THIS_ _In_ ULONG SysId, _Out_ PULONG Id)
      PURE;
  // Returns the handle of the current thread.
  // In kernel mode the value returned is the
  // index of the processor the thread is
  // executing on plus one.
  STDMETHOD(GetCurrentThreadHandle)(THIS_ _Out_ PULONG64 Handle) PURE;
  // Looks up a debugger thread ID for the given handle.
  // Currently when kernel debugging this will fail
  // if the thread is not executing on a processor.
  STDMETHOD(GetThreadIdByHandle)(THIS_ _In_ ULONG64 Handle, _Out_ PULONG Id)
      PURE;

  // Currently kernel mode sessions will only have
  // a single process representing kernel space.
  STDMETHOD(GetNumberProcesses)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetProcessIdsByIndex)(THIS_ _In_ ULONG Start, _In_ ULONG Count,
                                  _Out_writes_opt_(Count) PULONG Ids,
                                  _Out_writes_opt_(Count) PULONG SysIds) PURE;
  ...
  // When user debugging it is the offset
  // of the current PEB.
  STDMETHOD(GetCurrentProcessDataOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  // Looks up a debugger process ID for the given
  // system process data structure.
  // Not currently supported when kernel debugging.
  STDMETHOD(GetProcessIdByDataOffset)(THIS_ _In_ ULONG64 Offset,
                                      _Out_ PULONG Id) PURE;
  // Returns the offset of the current processs
  // PEB.  In user mode this is equivalent to
  // the processs data offset.
  STDMETHOD(GetCurrentProcessPeb)(THIS_ _Out_ PULONG64 Offset) PURE;
  // Looks up a debugger process ID for the given PEB.
  // Not currently supported when kernel debugging.
  STDMETHOD(GetProcessIdByPeb)(THIS_ _In_ ULONG64 Offset, _Out_ PULONG Id) PURE;
  // Returns the system unique ID for the current process.
  // Not currently supported when kernel debugging.
  STDMETHOD(GetCurrentProcessSystemId)(THIS_ _Out_ PULONG SysId) PURE;
  // Looks up a debugger process ID for the given
  // system process ID.
  // Not currently supported when kernel debugging.
  STDMETHOD(GetProcessIdBySystemId)(THIS_ _In_ ULONG SysId, _Out_ PULONG Id)
      PURE;
  ...
  STDMETHOD(GetCurrentProcessHandle)(THIS_ _Out_ PULONG64 Handle) PURE;
  // Looks up a debugger process ID for the given handle.
  STDMETHOD(GetProcessIdByHandle)(THIS_ _In_ ULONG64 Handle, _Out_ PULONG Id)
      PURE;
...
DECLARE_INTERFACE_(IDebugSystemObjects2, IUnknown) {
  ...
  STDMETHOD(GetCurrentProcessUpTime)(THIS_ _Out_ PULONG UpTime) PURE;
  ...
  // default value.
  STDMETHOD(GetImplicitThreadDataOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  STDMETHOD(SetImplicitThreadDataOffset)(THIS_ _In_ ULONG64 Offset) PURE;
  STDMETHOD(GetImplicitProcessDataOffset)(THIS_ _Out_ PULONG64 Offset) PURE;
  STDMETHOD(SetImplicitProcessDataOffset)(THIS_ _In_ ULONG64 Offset) PURE;
};
...
DECLARE_INTERFACE_(IDebugSystemObjects3, IUnknown) {
  ...
  STDMETHOD(GetEventSystem)(THIS_ _Out_ PULONG Id) PURE;

  STDMETHOD(GetCurrentSystemId)(THIS_ _Out_ PULONG Id) PURE;
  STDMETHOD(SetCurrentSystemId)(THIS_ _In_ ULONG Id) PURE;

  STDMETHOD(GetNumberSystems)(THIS_ _Out_ PULONG Number) PURE;
  STDMETHOD(GetSystemIdsByIndex)(THIS_ _In_ ULONG Start, _In_ ULONG Count,
                                 _Out_writes_(Count) PULONG Ids) PURE;
  STDMETHOD(GetTotalNumberThreadsAndProcesses)(
      THIS_ _Out_ PULONG TotalThreads, _Out_ PULONG TotalProcesses,
      _Out_ PULONG LargestProcessThreads, _Out_ PULONG LargestSystemThreads,
      _Out_ PULONG LargestSystemProcesses) PURE;
  STDMETHOD(GetCurrentSystemServer)(THIS_ _Out_ PULONG64 Server) PURE;
  STDMETHOD(GetSystemByServer)(THIS_ _In_ ULONG64 Server, _Out_ PULONG Id) PURE;
...
FORCEINLINE void DebugCommandException(ULONG Command, ULONG ArgSize,
                                       PVOID Arg) {
  ULONG_PTR ExArgs[4];
...
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_INITIALIZE)(_Out_ PULONG Version,
                                                       _Out_ PULONG Flags);
...
// model.
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_CANUNLOAD)(void);
...
typedef void(CALLBACK *PDEBUG_EXTENSION_UNLOAD)(void);
...
typedef void(CALLBACK *PDEBUG_EXTENSION_NOTIFY)(_In_ ULONG Notify,
...
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_CALL)(_In_ PDEBUG_CLIENT Client,
                                                 _In_opt_ PCSTR Args);
...
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_KNOWN_STRUCT)(
    _In_ ULONG Flags, _In_ ULONG64 Offset, _In_opt_ PSTR TypeName,
    _Out_writes_opt_(*BufferChars) PSTR Buffer, _Inout_opt_ PULONG BufferChars);
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_KNOWN_STRUCT_EX)(
    _In_ PDEBUG_CLIENT Client, _In_ ULONG Flags, _In_ ULONG64 Offset,
    _In_opt_ PCSTR TypeName, _Out_writes_opt_(*BufferChars) PSTR Buffer,
...
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_QUERY_VALUE_NAMES)(
    _In_ PDEBUG_CLIENT Client, _In_ ULONG Flags,
    _Out_writes_(BufferChars) PWSTR Buffer, _In_ ULONG BufferChars,
...
typedef HRESULT(CALLBACK *PDEBUG_EXTENSION_PROVIDE_VALUE)(
    _In_ PDEBUG_CLIENT Client, _In_ ULONG Flags, _In_ PCWSTR Name,
    _Out_ PULONG64 Value, _Out_ PULONG64 TypeModBase, _Out_ PULONG TypeId,
...
typedef HRESULT(CALLBACK *PDEBUG_STACK_PROVIDER_BEGINTHREADSTACKRECONSTRUCTION)(
    _In_ ULONG StreamType, _In_reads_(BufferSize) PVOID MiniDumpStreamBuffer,
    _In_ ULONG BufferSize);
...
typedef HRESULT(CALLBACK *PDEBUG_STACK_PROVIDER_RECONSTRUCTSTACK)(
    ...
    _Outptr_result_buffer_(*StackSymFramesFilled)
...
public:
  ...
  STDMETHOD(Breakpoint)(THIS_ _In_ PDEBUG_BREAKPOINT Bp) {
  ...
  STDMETHOD(CreateProcess)(THIS_ _In_ ULONG64 ImageFileHandle,
                           _In_ ULONG64 Handle, _In_ ULONG64 BaseOffset,
                           _In_ ULONG ModuleSize, _In_ PCSTR ModuleName,
                           _In_ PCSTR ImageName, _In_ ULONG CheckSum,
  ...
  STDMETHOD(LoadModule)(THIS_ _In_ ULONG64 ImageFileHandle,
                        _In_ ULONG64 BaseOffset, _In_ ULONG ModuleSize,
                        _In_ PCSTR ModuleName, _In_ PCSTR ImageName,
                        _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp) {
    UNREFERENCED_PARAMETER(ImageFileHandle);
    UNREFERENCED_PARAMETER(BaseOffset);
    UNREFERENCED_PARAMETER(ModuleSize);
  ...
  STDMETHOD(UnloadModule)(THIS_ _In_ PCSTR ImageBaseName,
  ...
  STDMETHOD(SessionStatus)(THIS_ _In_ ULONG Status) {
    UNREFERENCED_PARAMETER(Status);
    return DEBUG_STATUS_NO_CHANGE;
  }
  STDMETHOD(ChangeDebuggeeState)(THIS_ _In_ ULONG Flags,
                                 _In_ ULONG64 Argument) {
...
public:
  ...
  STDMETHOD(CreateProcess)(THIS_ _In_ ULONG64 ImageFileHandle,
                           _In_ ULONG64 Handle, _In_ ULONG64 BaseOffset,
                           _In_ ULONG ModuleSize, _In_ PCWSTR ModuleName,
                           _In_ PCWSTR ImageName, _In_ ULONG CheckSum,
                           _In_ ULONG TimeDateStamp,
                           _In_ ULONG64 InitialThreadHandle,
                           _In_ ULONG64 ThreadDataOffset,
                           _In_ ULONG64 StartOffset) {
  ...
  STDMETHOD(LoadModule)(THIS_ _In_ ULONG64 ImageFileHandle,
                        _In_ ULONG64 BaseOffset, _In_ ULONG ModuleSize,
                        _In_ PCWSTR ModuleName, _In_ PCWSTR ImageName,
                        _In_ ULONG CheckSum, _In_ ULONG TimeDateStamp) {
    UNREFERENCED_PARAMETER(ImageFileHandle);
    UNREFERENCED_PARAMETER(BaseOffset);
    UNREFERENCED_PARAMETER(ModuleSize);
  ...
  STDMETHOD(UnloadModule)(THIS_ _In_ PCWSTR ImageBaseName,
                          _In_ ULONG64 BaseOffset) {
    UNREFERENCED_PARAMETER(ImageBaseName);
    UNREFERENCED_PARAMETER(BaseOffset);
    return DEBUG_STATUS_NO_CHANGE;
...
#define GetRunningProcessSystemIdByExecutableNameT                             \
