...
//
// Each Trace event will need to be described with the HTTP_TRACE_EVENT
// structure. It is designed to contain all the information relevant to ETW, but
// ETW is not the only consumer
//
//
...
struct HTTP_TRACE_EVENT {
  ...
  //
  // Trace Area Flag ( a bit flag that may represent area such as
  // "Authentication")
  //
  DWORD dwArea;
  //
  ...
  //
  // Verbosity of the event ( General, FatalError, Error, Warning, Info,
  // Verbose) In the ETW terminology this is the "event level"
  DWORD dwVerbosity;
  //
  ...
  // the event
  //
  _Field_size_full_(cEventItems) struct HTTP_TRACE_EVENT_ITEM *pEventItems;
};

// Each HTTP_TRACE_EVENT can have multiple items or elements that describe the
// event (for example the "GENERAL_REQUEST_START" event contains an item with
// the Request URL)
...
struct HTTP_TRACE_EVENT_ITEM {
  ...
  // For example the HTTP_TRACE_EVENT_ITEM describing HTTP request URL would
  // have the pbData pointing to the actual URL string
  //
  _Field_size_bytes_full_(cbData) PBYTE pbData;
  //
  // # of bytes of the actual data (it should matter only for string types)
  //
  DWORD cbData;
  //
  // Additional description of the data - enumerations take advantage of this
  // field (always set to NULL if not used)
  //
...
DEFINE_GUID(GUID_IIS_ALL_TRACE_PROVIDERS, 0x00000000, 0x0000, 0x0000, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);

DEFINE_GUID(GUID_IIS_WWW_SERVER_TRACE_PROVIDER, 0x3a2a4e84, 0x4c21, 0x4981,
            0xae, 0x10, 0x3f, 0xda, 0x0d, 0x9b, 0x0f, 0x83);

// IIS V2(Crimson/Unified) trace provider
DEFINE_GUID(GUID_IIS_WWW_SERVER_V2_TRACE_PROVIDER, 0xde4649c9, 0x15e8, 0x4fea,
            0x9d, 0x85, 0x1c, 0xdd, 0xa5, 0x20, 0xc3, 0x34);

// ETW name for the provider: "ASP.NET Events
DEFINE_GUID(GUID_IIS_ASPNET_TRACE_PROVIDER, 0xAFF081FE, 0x0247, 0x4275, 0x9C,
            0x4E, 0x02, 0x1F, 0x3D, 0xC1, 0xDA, 0x35);

// ETW name for the provider: "IIS: Active Server Pages (ASP)"
DEFINE_GUID(GUID_IIS_ASP_TRACE_TRACE_PROVIDER, 0x06b94d9a, 0xb15e, 0x456e, 0xa4,
            0xef, 0x37, 0xc9, 0x84, 0xa2, 0xcb, 0x4b);

DEFINE_GUID(GUID_IIS_WWW_GLOBAL_TRACE_PROVIDER, 0xd55d3bc9, 0xcba9, 0x44df,
            0x82, 0x7e, 0x13, 0x2d, 0x3a, 0x45, 0x96, 0xc2);

// ETW name for the provider: "IIS: WWW Isapi Extension"
DEFINE_GUID(GUID_IIS_ISAPI_TRACE_PROVIDER, 0xa1c2040e, 0x8840, 0x4c31, 0xba,
            0x11, 0x98, 0x71, 0x03, 0x1a, 0x19, 0xea);
