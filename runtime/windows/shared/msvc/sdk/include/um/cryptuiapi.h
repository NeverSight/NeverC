...
//-----------------------------------------------------------------------------
_Success_(return == TRUE) BOOL WINAPI CryptUIDlgViewContext(
    _In_ DWORD dwContextType, _In_ const void *pvContext,
    _In_opt_ HWND hwnd,         // Defaults to the desktop window
    _In_opt_ LPCWSTR pwszTitle, // Defaults to the context type title
    _In_ DWORD dwFlags, _In_ void *pvReserved);
...
CryptUIDlgSelectCertificateFromStore(
    ...
    _In_opt_ HWND hwnd, // Defaults to the desktop window
    _In_opt_ LPCWSTR pwszTitle, _In_opt_ LPCWSTR pwszDisplayString,
    _In_ DWORD dwDontUseColumn, _In_ DWORD dwFlags, _In_ void *pvReserved);
...
HRESULT
WINAPI
CertSelectionGetSerializedBlob(_In_ PCERT_SELECTUI_INPUT pcsi,
...
_Success_(return == TRUE) BOOL WINAPI
    CryptUIDlgCertMgr(_In_ PCCRYPTUI_CERT_MGR_STRUCT pCryptUICertMgr);
...
//
//  pPvkFileInfo            IN Required: If dwPvkChoice ==
//  CRYPTUI_WIZ_DIGITAL_SIGN_PVK_FILE
//
//  pPvkProvInfo            IN Required: If dwPvkContainer ==
//  CRYPTUI_WIZ_DIGITAL_SIGN_PVK_PROV
//
//-----------------------------------------------------------------------------
...
//
// pwszFileName             IN Required: If dwSubjectChoice ==
// CRYPTUI_WIZ_DIGITAL_SIGN_SUBJECT_FILE
//
// pSignBlobInfo            IN Required: If dwSubhectChoice ==
// CRYPTUI_WIZ_DIGITAL_SIGN_SUBJECT_BLOB
//
...
//                                       CRYPTUI_WIZ_DIGITAL_SIGN_PVK
//
// pSigningCertContext      IN Required: If dwSigningCertChoice ==
// CRYPTUI_WIZ_DIGITAL_SIGN_CERT
//
// pSigningCertStore        IN Required: If dwSigningCertChoice ==
// CRYPTUI_WIZ_DIGITAL_SIGN_STORE
//
// pSigningCertPvkInfo      IN Required: If dwSigningCertChoise ==
// CRYPTUI_WIZ_DIGITAL_SIGN_PVK
//
...
//
//-----------------------------------------------------------------------------
_Success_(return == TRUE) BOOL WINAPI CryptUIWizDigitalSign(
    _In_ DWORD dwFlags, _In_opt_ HWND hwndParent,
...
_Success_(return == TRUE) BOOL WINAPI CryptUIWizFreeDigitalSignContext(
    _In_ PCCRYPTUI_WIZ_DIGITAL_SIGN_CONTEXT pSignContext);

/////////////////////////////////////////////////////////////////////////////////////////////////////
//
// dwSize                          size of this struct
// hwndParent                      parent of this dialog (OPTIONAL) dwFlags
// flags, may a combination of any of the flags below       (OPTIONAL) szTitle
// title for the window                                     (OPTIONAL)
// pCertContext                    the cert context that is to be displayed
// rgszPurposes                    array of purposes that this cert is to be
// validated for  (OPTIONAL) cPurposes                       number of purposes
// (OPTIONAL) pCryptProviderData/hWVTStateData if WinVerifyTrust has already
// been called for the cert  (OPTIONAL)
//                                 then pass in a pointer to the state struct
//                                 that was acquired through a call to
//                                 WTHelperProvDataFromStateData(), or pass in
//                                 the hWVTStateData of the WINTRUST_DATA struct
//                                 if WTHelperProvDataFromStateData() was not
//                                 called. if pCryptProviderData/hWVTStateData
//                                 is used then fpCryptProviderDataTrustedUsage,
//                                 idxSigner, idxCert, and fCounterSignature
//                                 must be set
// fpCryptProviderDataTrustedUsage if WinVerifyTrust was called this is the
// result of whether (OPTIONAL)
//                                 the cert was trusted
// idxSigner                       the index of the signer to view (OPTIONAL)
// idxCert                         the index of the cert that is being viewed
// within the    (OPTIONAL)
//                                 signer chain.  the cert context of this cert
//                                 MUST match pCertContext
// fCounterSigner                  set to TRUE if a counter signature is being
// viewed.  if  (OPTIONAL)
//                                 this is TRUE then idxCounterSigner must be
//                                 valid
// idxCounterSigner                the index of the counter signer to view
// (OPTIONAL) cStores                         Count of other stores to search
// when building and        (OPTIONAL)
//                                 validating chain
// rghStores                       Array of other stores to search when buliding
// and        (OPTIONAL)
//                                 validating chain
// cPropSheetPages                 number of extra pages to add to the dialog.
// (OPTIONAL) rgPropSheetPages                extra pages to add to the dialog.
// (OPTIONAL)
//                                 each page in this array will NOT recieve the
//                                 lParam in the PROPSHEET structure as the
//                                 lParam in the WM_INITDIALOG, instead it will
//                                 receive a pointer to a
//                                 CRYPTUI_INITDIALOG_STRUCT (defined below)
//                                 which contains the lParam in the PROPSSHEET
//                                 structure AND the PCCERT_CONTEXT for which
//                                 the page is being displayed.
// nStartPage                      this is the index of the initial page that
// will be
//                                 displayed.  if the upper most bit (0x8000) is
//                                 set then the index is assumed to index
//                                 rgPropSheetPages (after the upper most bit
//                                 has been stripped off.  eg. 0x8000 will
//                                 indicate the first page in rgPropSheetPages),
//                                 if the upper most bit is 0 then nStartPage
//                                 will be the starting index of the default
//                                 certificate dialog pages.
//
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
#define CRYPTUI_DISABLE_EXPORT                                                 \
  0x00002000 // If this flag is set, then the "Copy to file" button will be
             // disabled on the Detail page.

// Revocation flags is only valid if pCryptProviderData/hWVTStateData is not
// passed in.
...
#define CRYPTUI_ENABLE_REVOCATION_CHECK_CHAIN_EXCLUDE_ROOT                     \
  CRYPTUI_ENABLE_REVOCATION_CHECKING // Changed the default behavior

// to not check root.
#define CRYPTUI_DISABLE_HTMLLINK                                               \
  0x00010000 // to disable helplink in viewing certificate
#define CRYPTUI_DISABLE_ISSUERSTATEMENT                                        \
  0x00020000 // to disable issuer statement button

#define CRYPTUI_CACHE_ONLY_URL_RETRIEVAL                                       \
  0x00040000 // to disable online revocation checking
//
...
//
// pfPropertiesChanged             this will be set by the dialog proc to inform
// the caller
//                                 if any properties have been changed on certs
//                                 in the chain while the dialog was open
//
_Success_(return == TRUE) BOOL WINAPI CryptUIDlgViewCertificateW(
    _In_ PCCRYPTUI_VIEWCERTIFICATE_STRUCTW pCertViewInfo,
    _Out_ BOOL *pfPropertiesChanged // OPTIONAL
);

_Success_(return == TRUE) BOOL WINAPI CryptUIDlgViewCertificateA(
...
typedef struct _CRYPTUI_WIZ_EXPORT_INFO {
  ...
  LPCWSTR pwszExportFileName; // Required if the CRYPTUI_WIZ_NO_UI flag is set,
                              // Optional otherwise. The fully qualified file
                              // name to export to, if this is non-NULL and the
                              // CRYPTUI_WIZ_NO_UI flag is NOT set, then it is
  ...
  DWORD
  cStores; // Optional: count of extra stores to search for the certs in the
           //           trust chain if the chain is being exported with a
           //           cert. this is ignored if dwSubjectChoice is anything
           //           other than CRYPTUI_WIZ_EXPORT_CERT_CONTEXT
  HCERTSTORE *rghStores; // Optional: array of extra stores to search for the
                         // certs in the
                         //           trust chain if the chain is being exported
                         //           with a cert. this is ignored if
                         //           dwSubjectChoice is anything other than
                         //           CRYPTUI_WIZ_EXPORT_CERT_CONTEXT
...
#define CRYPTUI_WIZ_EXPORT_FORMAT_SERIALIZED_CERT_STORE                        \
  5 // NOTE: not currently supported!!
...
typedef struct _CRYPTUI_WIZ_EXPORT_CERTCONTEXT_INFO {
  DWORD
  dwSize; // Required: should be set to sizeof(CRYPTUI_WIZ_EXPORT_NOUI_INFO)
  DWORD
  dwExportFormat; // Required:
                  //           It can be one of the following:
  ...
  LPCWSTR pwszPassword;   // Required if the fExportPrivateKeys boolean is TRUE,
                          // otherwise, it is ignored
  BOOL fStrongEncryption; // Required if dwExportFormat is
                          // CRYPTUI_WIZ_EXPORT_FORMAT_PFX Note that if this
                          // flag is TRUE then the PFX blob produced is NOT
                          // compatible with IE4.
...
//      pvoid:              IN Required:    Contains information about how to do
//      the export based on what
//                                          is being exported
//
...
//      pwszWizardTitle:    IN Optional:    The title of the wizard
//                                          If NULL, the default will be
//                                          IDS_EXPORT_WIZARD_TITLE
//      pExportInfo:        IN Required:    The subject to export.
//      pvoid:              IN Optional:    Contains information about how to do
//      the export based on what
//                                          is being exported.  See above table
//                                          for values, if this is non-NULL the
//                                          values are displayed to the user as
//                                          the default choices.
//------------------------------------------------------------------------
_Success_(return == TRUE) BOOL WINAPI
    CryptUIWizExport(_In_ DWORD dwFlags, _In_opt_ HWND hwndParent,
                     _In_opt_ LPCWSTR pwszWizardTitle,
...
//  CRYPTUI_WIZ_NO_UI                           No UI will be shown.  Otherwise,
//  User will be
//                                              prompted by a wizard.
//  CRYPTUI_WIZ_IMPORT_ALLOW_CERT               Allow importing certificate
//  CRYPTUI_WIZ_IMPORT_ALLOW_CRL                Allow importing CRL(certificate
//  revocation list) CRYPTUI_WIZ_IMPORT_ALLOW_CTL                Allow importing
//  CTL(certificate trust list) CRYPTUI_WIZ_IMPORT_NO_CHANGE_DEST_STORE     user
//  will not be allowed to change
//                                              the hDesCertStore in the wizard
//                                              page
//  CRYPTUI_WIZ_IMPORT_TO_LOCALMACHINE          the contents should be imported
//  to local machine
//                                              (currently only applicable for
//                                              PFX imports)
//  CRYPTUI_WIZ_IMPORT_TO_CURRENTUSER           the contents should be imported
//  to current user
//                                              (currently only applicable for
//                                              PFX imports)
//
//  Please notice that if neither of following three flags is in dwFlags,
//  default to is allow everything. CRYPTUI_WIZ_IMPORT_ALLOW_CERT
//  CRYPTUI_WIZ_IMPORT_ALLOW_CRL
//  CRYPTUI_WIZ_IMPORT_ALLOW_CTL
//
//  Also, note that the CRYPTUI_WIZ_IMPORT_TO_LOCALMACHINE and
//  CRYPTUI_WIZ_IMPORT_TO_CURRENTUSER flags are used force the content of a pfx
//  blob into either local machine or current user. If neither of these flags
//  are used and hDesCertStore is NULL then: 1) The private key in the pfx blob
//  will be forced to be imported into current user. 2) If CRYPTUI_WIZ_NO_UI is
//  NOT set, the wizard will prompt the user to select a certificate
...
//                                          If NULL, the wizard will prompt user
//                                          to enter the file name
//      hDesCertStore:      IN Optional:    The destination certificate store
//      where the file wil be
//                                          imported to.  The store should be
//                                          opened with flag
//                                          CERT_STORE_SET_LOCALIZED_NAME_FLAG.
//                                          If NULL, the wizard will prompt user
//                                          to select a certificate store.
//------------------------------------------------------------------------
_Success_(return == TRUE) BOOL WINAPI
    CryptUIWizImport(_In_ DWORD dwFlags, _In_opt_ HWND hwndParent,
                     _In_opt_ LPCWSTR pwszWizardTitle,
