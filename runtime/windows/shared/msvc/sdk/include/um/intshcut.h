...
typedef enum iurl_seturl_flags {
  ...
  IURL_SETURL_FL_USE_DEFAULT_PROTOCOL =
      0x0002, // Use default protocol if missing
} IURL_SETURL_FLAGS;
...
typedef struct urlinvokecommandinfoA {
  ...
  HWND hwndParent; // Parent window.  Valid only if
                   // IURL_INVOKECOMMAND_FL_ALLOW_UI is set.
  LPCSTR pcszVerb; // Verb to invoke.  Ignored if
                   // IURL_INVOKECOMMAND_FL_USE_DEFAULT_VERB is set.
...
typedef struct urlinvokecommandinfoW {
  ...
  HWND hwndParent;  // Parent window.  Valid only if
                    // IURL_INVOKECOMMAND_FL_ALLOW_UI is set.
  LPCWSTR pcszVerb; // Verb to invoke.  Ignored if
                    // IURL_INVOKECOMMAND_FL_USE_DEFAULT_VERB is set.
...
DECLARE_INTERFACE_IID_(IUniformResourceLocatorA, IUnknown,
  ...
  STDMETHOD(GetURL)(THIS_ _Outptr_ LPSTR * ppszURL) PURE;
...
DECLARE_INTERFACE_IID_(IUniformResourceLocatorW, IUnknown,
  ...
  STDMETHOD(GetURL)(THIS_ _Outptr_ LPWSTR * ppszURL) PURE;
...
INTSHCUTAPI HRESULT WINAPI TranslateURLW(
    PCWSTR pcszURL, DWORD dwInFlags, _Out_ PWSTR UNALIGNED *ppszTranslatedURL);
#ifdef UNICODE
...
INTSHCUTAPI HRESULT WINAPI URLAssociationDialogA(
    HWND hwndParent, DWORD dwInFlags, PCSTR pcszFile, PCSTR pcszURL,
    _Out_writes_(ucAppBufLen) PSTR pszAppBuf, UINT ucAppBufLen);
INTSHCUTAPI HRESULT WINAPI URLAssociationDialogW(
    HWND hwndParent, DWORD dwInFlags, PCWSTR pcszFile, PCWSTR pcszURL,
    _Out_writes_(ucAppBufLen) PWSTR pszAppBuf, UINT ucAppBufLen);
#ifdef UNICODE
...
INTSHCUTAPI HRESULT WINAPI MIMEAssociationDialogW(
    ...
    PCWSTR pcszMIMEContentType, _Out_writes_(ucAppBufLen) PWSTR pszAppBuf,
    UINT ucAppBufLen);
#ifdef UNICODE
