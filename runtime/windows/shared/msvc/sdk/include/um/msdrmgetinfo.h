...
The following codes are used to indicate where the various query strings may be
used:

for example, GI(*) means that all DRMHANDLES may be asked the indicated question
using DRMGetInfo & GI(hEnv) means on environment handle only
...
const PWSTR g_wszTAG_ASCII = (PWSTR)L"ASCII Tag",
            ...
            g_wszPLUG_STANDARDENABLINGPRINCIPAL =
                (PWSTR)L"UDStdPlg Enabling Principal",
            g_wszPLUG_STANDARDRIGHTSINTERPRETER = (PWSTR)L"XrMLv2a",
            g_wszPLUG_STANDARDEBDECRYPTOR =
                (PWSTR)L"UDStdPlg Enabling Bits Decryptor",
            g_wszPLUG_STANDARDEBENCRYPTOR =
                (PWSTR)L"UDStdPlg Enabling Bits Encryptor",
            g_wszPLUG_STANDARDEBCRYPTOPROVIDER =
                (PWSTR)L"UDStdPlg Enabling Bits Crypto Provider",
    ...
    g_wszQUERY_OBJECTIDTYPE = (PWSTR)L"object-id-type",
            ...
            // GBLA(on a bound right object), GULA(on a principal object, rights
            // group, right, & work)
...
const PWSTR g_wszQUERY_ACCESSCONDITION = (PWSTR)L"access-condition",
    ...
    g_wszQUERY_ENABLINGPRINCIPALIDTYPE = (PWSTR)L"enabling-principal-id-type",
            g_wszQUERY_ENABLINGPRINCIPALIDVALUE =
                (PWSTR)L"enabling-principal-id-value",
