...
typedef LONG(APIENTRY *APPLET_PROC)(HWND hwndCpl, UINT msg, LPARAM lParam1,
                                    LPARAM lParam2);
...
typedef struct tagCPLINFO {
  ...
  int idInfo; /* description/tooltip/status bar string resource id, provided by
                 CPlApplet() */
...
typedef struct tagNEWCPLINFOA {
  ...
  LONG_PTR lData;      /* user defined data */
  HICON hIcon; /* icon to use, this is owned by the Control Panel window (may be
                  deleted) */
 ...
 * description */
/*  are not dynamic then CPL_DYNAMIC_RES should not be used and the
 * CPL_NEWINQURE message */
/*  should be ignored */
 ...
 */
/*  is used in CPL_INQUIRE.  CPLs which respond to CPL_NEWINQUIRE cannot be
 * cached */
