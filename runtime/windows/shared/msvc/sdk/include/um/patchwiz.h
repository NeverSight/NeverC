...
//
// Synopsis:    Main entry point to PatchWiz.  This method is used for backwards
// compatibility with the previous
//				version of PatchWiz.  Its purpose is to create a
// valid patch.  This method calls the updated version of
// UiCreatePatchPackageEx.
//
// Arguments:
//			[in]	szPcpPath - full absolute path to Windows
// Installer database 					(PCP file) that contains
// appropriate tables of input-data for
...
// from the PCP file. 			[in]	szLogPath - path to store the
// log file. 			[in] hwndStatus - optional, handle to a window
// that displays text. 			[in] szTempFolder - optional location to
// use for temp files. 					Default is
//%TEMP%\~pcw_tmp.tmp\, if specified, directory must exist.
...
//
// Synopsis:    Updated main entry point to PatchWiz.  This method provides a
// few new parameter requirements as
//				listed below.  This method calls each of the
// five phases in order to create a valid patch as output.
//
// Arguments:
//			[in]	szPcpPath - full absolute path to Windows
// Installer database 					(PCP file) that contains
...
//[in] fRemoveTempFolderIfPresent - remove temp folder (and all its
// contents) if present.  If FALSE and folder is present, api
// will fail. 			*NEW PARAMETERS* 			[in]
// uiLogLevel - Determines what entries are written to the log file.
//							- 0x00000000 - No
// logging.
//							- 0x00000001 - INFO
//							- 0x00000002 - WARNING
//							- 0x00000004 - ERROR
//			[in]	UILevel - If 0, no UI is shown, else if set to 1
// then UI can occur. 			[in]	szSummaryOutputPath - optional,
// path to store any summary output of the patch
// functionality and description.  If null, no output is written.
//
// Returns:
//			ERROR_SUCCESS, plus ERROR_PCW_* that are listed in
// constants.h.
//
// Notes:
//
...
UINT WINAPI UiCreatePatchPackageExA(LPCSTR szPcpPath, LPCSTR szPatchPath,
                                    ...
                                    LPCSTR szTempFolder,
                                    BOOL fRemoveTempFolderIfPresent,
                                    IN DWORD dwFlags, IN DWORD dwReserved);
...
const UINT DEFAULT_FILE_SEQUENCE_START =
    2; // Default sequence start for adding files to cab.
...
// -- obsolete #define ERROR_PCW_FILERANGE_MISSING_FILE_TABLE_KEY
// (ERROR_PCW_BASE + 0x31) -- obsolete #define
// ERROR_PCW_FILERANGE_LONG_PATH_TO_FILE         (ERROR_PCW_BASE + 0x32) --
// obsolete #define ERROR_PCW_FILERANGE_MISSING_FILE (ERROR_PCW_BASE + 0x33) --
// obsolete #define ERROR_PCW_FILERANGE_INVALID_OFFSET (ERROR_PCW_BASE + 0x34)
// -- obsolete #define ERROR_PCW_FILERANGE_INVALID_SIZE (ERROR_PCW_BASE + 0x35)
// -- obsolete #define ERROR_PCW_FILERANGE_INVALID_RETAIN (ERROR_PCW_BASE +
// 0x36) -- obsolete #define ERROR_PCW_BAD_MEDIA_SRC_PROP_NAME (ERROR_PCW_BASE +
// 0x37) -- obsolete #define ERROR_PCW_BAD_MEDIA_DISK_ID (ERROR_PCW_BASE + 0x38)
// -- obsolete
...
// -- obsolete #define ERROR_PCW_CANNOT_CREATE_STREAM (ERROR_PCW_BASE + 0x47) --
// obsolete #define ERROR_PCW_CANNOT_WRITE_STREAM (ERROR_PCW_BASE + 0x48) --
// obsolete #define ERROR_PCW_CANNOT_READ_CABINET (ERROR_PCW_BASE + 0x49) --
// obsolete
