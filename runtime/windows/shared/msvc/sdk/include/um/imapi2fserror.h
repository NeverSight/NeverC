...
#pragma once
// Since the message codes are also included in the header file produced from
// the .idl, don't include them here
#ifndef _SKIP_FSI_ERROR_MESSAGE_CODES
// -----  Catch-all error(s) -- should not actually occur, indicative of
...
//  NOTE:  the quote characters delimiting the parameter name are used by the
//  Managed Shim
//         to parse the parameter name from the message text for exception
//         handling Don't remove or change these quote-delimiters w/o also
//         changing the exception mapping in the managed shim.
//
...
// MessageText:
//
// The specified Volume Identifier is either too long or contains one or more
// invalid characters.
//
...
// MessageText:
//
// '%1!ls!' is not part of the file system.  It must be added to complete this
// operation.
//
...
// MessageText:
//
// The name '%1!ls!' specified is not legal:  Name of file or directory object
// created while the UseRestrictedCharacterSet property is set may only contain
// ANSI characters.
//
...
// MessageText:
//
// Attempt to add '%1!ls!' failed:  cannot create a file-system-specific unique
// name for the %2!ls! file system.
//
...
// MessageText:
//
// Adding '%1!ls!' would result in a result image having a size larger than the
...
// MessageText:
//
// Value specified for FreeMediaBlocks property is too small for estimated image
// size based on current data.
//
...
// MessageText:
//
// Data stream supplied for file '%1!ls!' is inconsistent:  expected %2!I64d!
// bytes, found %3!I64d!.
//
...
// MessageText:
//
// The following error was encountered when trying to create data stream for
// '%1!ls!':
//
...
// MessageText:
//
// The following error was encountered when trying to enumerate files in
// directory '%1!ls!':
//
...
// MessageText:
//
// Operation failed because of incompatible layout of the previous session
// imported from the medium.
//
...
// MessageText:
//
// Cannot set working directory to '%1!ls!'.  Space available is %2!I64d! bytes,
// approximately %3!I64d! bytes required.
//
...
// MessageText:
//
// The '%1!ls!'file system on the selected disc contains a feature not supported
// for import.
//
...
// MessageText:
//
// Could not import %2!ls! file system from disc.  The file '%1!ls!' already
// exists within the image hierarchy as a directory.
//
#define IMAPI_E_IMPORT_TYPE_COLLISION_FILE_EXISTS_AS_DIRECTORY                 \
  ((HRESULT)0xC0AAB155L)
...
// MessageText:
//
// Import from previous session failed due to an error reading a block on the
// media (most likely block %1!u!).
//
...
// MessageText:
//
// IMAPI can not do multi-session with the current media because it does not
// support a compatible UDF revision for write.
//
...
// MessageText:
//
// IMAPI supports none of the multisession type(s) provided on the current
// media.
//
...
// MessageText:
//
// Could not import %2!ls! file system from disc.  The directory '%1!ls!'
// already exists within the image hierarchy as a file.
//
#define IMAPI_E_IMPORT_TYPE_COLLISION_DIRECTORY_EXISTS_AS_FILE                 \
  ((HRESULT)0xC0AAB15EL)
...
// MessageText:
//
// Feature is not supported for the current file system revision, image will be
// created without this feature.
//
...
// MessageText:
//
// You cannot change the file system to be created, because the file system in
// the imported session and the one in the new session must match.
//
...
// MessageText:
//
// No valid file system Volume Descriptor was found in the iso image. This image
// format is not supported and the resulting disc might not be readable.
//
...
#define IMAPIError(hr)                                                         \
  ((HRESULT_FACILITY(hr) == FACILITY_IMAPI2) &&                                \
   (HRESULT_CODE(hr) <= IMAPI_ERROR_LAST) &&                                   \
   (HRESULT_CODE(hr) >= IMAPI_ERROR_FIRST))
#define IMAPI_FSI_ERROR_FIRST (unsigned int)0xB100
#define IMAPI_FSI_ERROR_LAST (unsigned int)0xB2FF
#define IMAPIFsiError(hr)                                                      \
  ((HRESULT_FACILITY(hr) == FACILITY_IMAPI2) &&                                \
   (HRESULT_CODE(hr) <= IMAPI_FSI_ERROR_LAST) &&                               \
   (HRESULT_CODE(hr) >= IMAPI_FSI_ERROR_FIRST))
