...
DEFINE_GUID(IID_IStiUSD, 0x0C9BB460L, 0x51AC, 0x11D0, 0x90, 0xEA, 0x00, 0xAA,
            0x00, 0x60, 0xF8, 0x6C);

// {128A9860-52DC-11D0-9EDF-444553540000}
DEFINE_GUID(IID_IStiDeviceControl, 0x128A9860L, 0x52DC, 0x11D0, 0x9E, 0xDF,
...
// USD receives this  bit only when associated device instance is created by
// monitor process
//
...
DECLARE_INTERFACE_(IStiDeviceControl, IUnknown) {
  ...
  STDMETHOD(Initialize)(THIS_ DWORD dwDeviceType, DWORD dwMode,
                        LPCWSTR pwszPortName, DWORD dwFlags) PURE;
  ...
  STDMETHOD(RawWriteCommand)(THIS_ LPVOID lpBuffer, DWORD nNumberOfBytes,
                             LPOVERLAPPED lpOverlapped) PURE;
  STDMETHOD(RawDeviceControl)(THIS_ USD_CONTROL_CODE EscapeFunction,
                              LPVOID lpInData, DWORD cbInDataSize,
                              LPVOID pOutData, DWORD dwOutDataSize,
                              LPDWORD pdwActualData) PURE;
  STDMETHOD(GetLastError)(THIS_ LPDWORD lpdwLastError) PURE;
  STDMETHOD(GetMyDevicePortName)(THIS_ _Out_writes_(cwDevicePathSize)
                                     LPWSTR lpszDevicePath,
                                 DWORD cwDevicePathSize) PURE;
  ...
  STDMETHOD(WriteToErrorLog)(THIS_ DWORD dwMessageType, LPCWSTR pszMessage,
                             DWORD dwErrorCode) PURE;
};

#if !defined(__cplusplus) || defined(CINTERFACE)

#define IStiDeviceControl_QueryInterface(p, a, b)                              \
  (p)->lpVtbl->QueryInterface(p, a, b)
#define IStiDeviceControl_AddRef(p) (p)->lpVtbl->AddRef(p)
#define IStiDeviceControl_Release(p) (p)->lpVtbl->Release(p)
#define IStiDeviceControl_Initialize(p, a, b, c, d)                            \
  (p)->lpVtbl->Initialize(p, a, b, c, d)

#define IStiDeviceControl_RawReadData(p, a, b, c)                              \
  (p)->lpVtbl->RawReadData(p, a, b, c)
#define IStiDeviceControl_RawWriteData(p, a, b, c)                             \
  (p)->lpVtbl->RawWriteData(p, a, b, c)
#define IStiDeviceControl_RawReadCommand(p, a, b, c)                           \
  (p)->lpVtbl->RawReadCommand(p, a, b, c)
#define IStiDeviceControl_RawWriteCommand(p, a, b, c)                          \
  (p)->lpVtbl->RawWriteCommand(p, a, b, c)
#define IStiDeviceControl_RawDeviceControl(p, a, b, c, d, e, f)                \
  (p)->lpVtbl->RawDeviceControl(p, a, b, c, d, e, f)
#define IStiDeviceControl_GetLastError(p, a) (p)->lpVtbl->GetLastError(p, a)
#define IStiDeviceControl_GetMyDevicePortName(p, a, b)                         \
  (p)->lpVtbl->GetMyDevicePortName(p, a, b)
#define IStiDeviceControl_GetMyDeviceHandle(p, a)                              \
  (p)->lpVtbl->GetMyDeviceHandle(p, a)
#define IStiDeviceControl_GetMyDeviceOpenMode(p, a)                            \
  (p)->lpVtbl->GetMyDeviceOpenMode(p, a)
#define IStiDeviceControl_WriteToErrorLog(p, a, b, c)                          \
  (p)->lpVtbl->WriteToErrorLog(p, a, b, c)
...
DECLARE_INTERFACE_(IStiUSD, IUnknown) {
  ...
  /*** IStiUSD methods ***/
  STDMETHOD(Initialize)(THIS_ _In_ PSTIDEVICECONTROL pHelDcb,
                        DWORD dwStiVersion, _In_ HKEY hParametersKey) PURE;
  ...
  STDMETHOD(Escape)(THIS_ STI_RAW_CONTROL_CODE EscapeFunction,
                    _In_reads_bytes_(cbInDataSize) LPVOID lpInData,
                    DWORD cbInDataSize,
                    _Out_writes_bytes_(cbOutDataSize) LPVOID pOutData,
                    DWORD cbOutDataSize, _Out_ LPDWORD pdwActualData) PURE;
  ...
  STDMETHOD(RawReadData)(THIS_ _Out_writes_bytes_(*lpdwNumberOfBytes)
                             LPVOID lpBuffer,
  ...
  STDMETHOD(RawReadCommand)(THIS_ _Out_writes_bytes_(*lpdwNumberOfBytes)
                                LPVOID lpBuffer,
                            _Inout_ LPDWORD lpdwNumberOfBytes,
                            _In_opt_ LPOVERLAPPED lpOverlapped) PURE;
...
#define IStiUSD_Escape(p, a, b, c, d, e, f)                                    \
...
#define IStiUSD_RawReadCommand(p, a, b, c)                                     \
  (p)->lpVtbl->RawReadCommand(p, a, b, c)
#define IStiUSD_RawWriteCommand(p, a, b, c)                                    \
  (p)->lpVtbl->RawWriteCommand(p, a, b, c)
#define IStiUSD_SetNotificationHandle(p, a)                                    \
  (p)->lpVtbl->SetNotificationHandle(p, a)
