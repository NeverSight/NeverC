 ...
 */
typedef void(STDAPICALLTYPE MSGCALLRELEASE)(ULONG ulCallerData,
                                            LPMESSAGE lpMessage);
...
STDAPI_(void)
CloseIMsgSession(LPMSGSESS lpMsgSess); /* -> message session object	*/
 ...
 *	IMessage will call the routine when the last release on (the toplevel
 * only) message is called.  It is intended to allow the callee to free the
 * IStorage that contains the message.  IMessage will not use the IStorage
 * object after making this call.
 *
 *	NOTE 4:  Behavior of multiple opens of sub-objects (Attachments,
 * Streams, Storages, Messages, etc.) within a message is deliberately undefined
 * in MAPI.  This implementation allows them, but will do it by AddRef'ing the
 ...
 *	specific Attachment or Property had is what all others will get
 * regardless of what the subsequent opens asked for.
 *
 ...
 *	IMessage object. Some clients of IMessage may wish to commit the
 * IStorage themselves after writing additional data to the storage (beyond what
...
OpenIMsgOnIStg(
    ...
    WIN_NOEXCEPT; /* <- open message object			 */
 ...
 *
 *	This call is provided because there is no method of IMAPIPropSet to
 * allow getting attributes.
 */
_Check_return_ STDAPI
GetAttribIMsgOnIStg(LPVOID lpObject, LPSPropTagArray lpPropTagArray,
 ...
 */
_Check_return_ STDAPI SetAttribIMsgOnIStg(
    LPVOID lpObject, LPSPropTagArray lpPropTags, LPSPropAttrArray lpPropAttrs,
 ...
 *
 *	WARNING:	There is no guarantee that this entry point will exist
 * in shipped versions of mapiu.dll.
 */
