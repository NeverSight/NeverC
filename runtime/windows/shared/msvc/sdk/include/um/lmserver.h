...
NET_API_STATUS NET_API_FUNCTION NetServerEnum(
    _In_opt_ IN LMCSTR servername OPTIONAL, _In_ IN DWORD level,
    _Outptr_result_buffer_(_Inexpressible_(entriesread)) OUT LPBYTE *bufptr,
    _In_ IN DWORD prefmaxlen, _Out_ OUT LPDWORD entriesread,
    _Out_ OUT LPDWORD totalentries, _In_ IN DWORD servertype,
...
_Check_return_ _Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetServerGetInfo(
        ...
        _When_(level == 100,
               _Outptr_result_bytebuffer_(sizeof(SERVER_INFO_100)))
            _When_(level == 101,
                   _Outptr_result_bytebuffer_(sizeof(SERVER_INFO_101)))
                _When_(level == 102,
                       _Outptr_result_bytebuffer_(sizeof(SERVER_INFO_102)))
                    LPBYTE *bufptr);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerSetInfo(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _When_(level == 100, _In_reads_bytes_(sizeof(SERVER_INFO_100))) _When_(
        level == 101, _In_reads_bytes_(sizeof(SERVER_INFO_101)))
        _When_(level == 102, _In_reads_bytes_(sizeof(SERVER_INFO_102))) _When_(
            level == 402, _In_reads_bytes_(sizeof(SERVER_INFO_402)))
            _When_(level == 403, _In_reads_bytes_(sizeof(SERVER_INFO_403)))
                _When_(level == 1005,
                       _In_reads_bytes_(sizeof(SERVER_INFO_1005)))
                    _When_(level == 1010,
                           _In_reads_bytes_(sizeof(SERVER_INFO_1010)))
                        _When_(level == 1016,
                               _In_reads_bytes_(sizeof(SERVER_INFO_1016)))
                            _When_(level == 1017,
                                   _In_reads_bytes_(sizeof(SERVER_INFO_1017)))
                                _When_(level == 1018, _In_reads_bytes_(sizeof(
                                                          SERVER_INFO_1018)))
                                    LPBYTE buf,
...
NET_API_STATUS
NetServerSetInfoCommandLine(_In_ WORD argc, _In_reads_(argc) LMSTR argv[]);
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetServerDiskEnum(
        _In_opt_ LMSTR servername, _In_ DWORD level,
        _Outptr_result_bytebuffer_((sizeof(TCHAR) * 3 * (*entriesread)) +
                                   sizeof(TCHAR)) LPBYTE *bufptr,
        _In_ DWORD prefmaxlen, _Out_ LPDWORD entriesread,
        _Out_ LPDWORD totalentries, _Inout_opt_ LPDWORD resume_handle);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerComputerNameAdd(
    _In_opt_ LMSTR ServerName, _In_opt_ LMSTR EmulatedDomainName,
    _In_ LMSTR EmulatedServerName);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerComputerNameDel(
    _In_opt_ LMSTR ServerName, _In_ LMSTR EmulatedServerName);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerTransportAdd(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_0)) LPBYTE bufptr);

_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerTransportAddEx(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _When_(level == 0, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_0)))
        _When_(level == 1, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_1)))
            _When_(level == 2,
                   _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_2)))
                _When_(level == 3,
                       _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_3)))
                    LPBYTE bufptr);
...
_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerTransportAddForInstance(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _When_(level == 0, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_0)))
        _When_(level == 1, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_1)))
            _When_(level == 2,
...
_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerTransportDel(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _When_(level == 0, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_0)))
        _When_(level == 1, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_1)))
            LPBYTE bufptr);
...
_Check_return_ NET_API_STATUS NET_API_FUNCTION NetServerTransportDelForInstance(
    _In_opt_ LMSTR servername, _In_ DWORD level,
    _When_(level == 0, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_0)))
        _When_(level == 1, _In_reads_bytes_(sizeof(SERVER_TRANSPORT_INFO_1)))
            LPBYTE bufptr,
...
_Check_return_ _Success_(return == 0 || return == ERROR_MORE_DATA)
    NET_API_STATUS NET_API_FUNCTION NetServerTransportEnum(
        ...
        _When_(level == 0,
            ...
            _When_(level == 1,
                   _Outptr_result_bytebuffer_(*entriesread *
                                              sizeof(SERVER_TRANSPORT_INFO_1)))
                _When_(level == 2,
                       _Outptr_result_bytebuffer_(
                           *entriesread * sizeof(SERVER_TRANSPORT_INFO_2)))
                    LPBYTE *bufptr,
        _In_ DWORD prefmaxlen, _Out_ LPDWORD entriesread,
        _Out_ LPDWORD totalentries, _Inout_opt_ LPDWORD resume_handle);
...
//
BOOL NET_API_FUNCTION SetServiceBits(IN SERVICE_STATUS_HANDLE hServiceStatus,
                                     IN DWORD dwServiceBits, IN BOOL bSetBitsOn,
...
#define SV_TYPE_DCE                                                            \
  0x10000000 /* IBM DSS (Directory and Security Services) or equivalent */
#define SV_TYPE_ALTERNATE_XPORT                                                \
  0x20000000 /* return list for alternate transport */
...
#define SVTI2_VALID_FLAGS                                                      \
  (SVTI2_REMAP_PIPE_NAMES | SVTI2_SCOPED_NAME | SVTI2_CLUSTER_NAME |           \
   SVTI2_CLUSTER_DNN_NAME | SVTI2_UNICODE_TRANSPORT_ADDRESS)
//
