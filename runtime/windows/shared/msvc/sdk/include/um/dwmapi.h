...
#define DWM_BB_TRANSITIONONMAXIMIZED                                           \
  0x00000004 // fTransitionOnMaximized has been specified
...
enum DWMWINDOWATTRIBUTE {
  DWMWA_NCRENDERING_ENABLED =
      1,                    // [get] Is non-client rendering enabled/disabled
  DWMWA_NCRENDERING_POLICY, // [set] DWMNCRENDERINGPOLICY - Non-client rendering
                            // policy
  DWMWA_TRANSITIONS_FORCEDISABLED, // [set] Potentially enable/forcibly disable
                                   // transitions
  DWMWA_ALLOW_NCPAINT, // [set] Allow contents rendered in the non-client area
                       // to be visible on the DWM-drawn frame.
  DWMWA_CAPTION_BUTTON_BOUNDS, // [get] Bounds of the caption button area in
                               // window-relative space.
  DWMWA_NONCLIENT_RTL_LAYOUT,  // [set] Is non-client content RTL mirrored
  DWMWA_FORCE_ICONIC_REPRESENTATION, // [set] Force this window to display
                                     // iconic thumbnails.
  DWMWA_FLIP3D_POLICY, // [set] Designates how Flip3D will treat the window.
  DWMWA_EXTENDED_FRAME_BOUNDS, // [get] Gets the extended frame bounds rectangle
                               // in screen space
  DWMWA_HAS_ICONIC_BITMAP,  // [set] Indicates an available bitmap when there is
                            // no better thumbnail representation.
  DWMWA_DISALLOW_PEEK,      // [set] Don't invoke Peek on the window.
  DWMWA_EXCLUDED_FROM_PEEK, // [set] LivePreview exclusion information
  DWMWA_CLOAK,              // [set] Cloak or uncloak the window
  DWMWA_CLOAKED,            // [get] Gets the cloaked state of the window
  DWMWA_FREEZE_REPRESENTATION, // [set] BOOL, Force this window to freeze the
                               // thumbnail without live update
  DWMWA_PASSIVE_UPDATE_MODE, // [set] BOOL, Updates the window only when desktop
                             // composition runs for other reasons
  DWMWA_USE_HOSTBACKDROPBRUSH, // [set] BOOL, Allows the use of host backdrop
                               // brushes for the window.
  DWMWA_USE_IMMERSIVE_DARK_MODE =
      20, // [set] BOOL, Allows a window to either use the accent color, or
          // dark, according to the user Color Mode preferences.
  DWMWA_WINDOW_CORNER_PREFERENCE =
      33, // [set] WINDOW_CORNER_PREFERENCE, Controls the policy that rounds
          // top-level window corners
  DWMWA_BORDER_COLOR,  // [set] COLORREF, The color of the thin border around a
                       // top-level window
  DWMWA_CAPTION_COLOR, // [set] COLORREF, The color of the caption
  DWMWA_TEXT_COLOR,    // [set] COLORREF, The color of the caption text
  DWMWA_VISIBLE_FRAME_BORDER_THICKNESS, // [get] UINT, width of the visible
                                        // border around a thick frame window
  DWMWA_SYSTEMBACKDROP_TYPE,     // [get, set] SYSTEMBACKDROP_TYPE, Controls the
                                 // system-drawn backdrop material of a window,
                                 // including behind the non-client area.
  DWMWA_REDIRECTIONBITMAP_ALPHA, // [set] BOOL, GDI redirection bitmap contains
                                 // premultiplied alpha
  DWMWA_BORDER_MARGINS, // [set] FRAME_MARGIN, Override location of window
                        // border (distance from each edge)
  DWMWA_LAST
};
...
// Use this constant to reset any window part colors to the system default
// behavior
...
enum DWM_SYSTEMBACKDROP_TYPE {
  DWMSBT_AUTO,       // [Default] Let DWM automatically decide the system-drawn
                     // backdrop for this window.
  DWMSBT_NONE,       // Do not draw any system backdrop.
  DWMSBT_MAINWINDOW, // Draw the backdrop material effect corresponding to a
                     // long-lived window.
  DWMSBT_TRANSIENTWINDOW, // Draw the backdrop material effect corresponding to
                          // a transient window.
  DWMSBT_TABBEDWINDOW, // Draw the backdrop material effect corresponding to a
                       // window with a tabbed title bar.
};

// Non-client rendering policy attribute values
enum DWMNCRENDERINGPOLICY {
  DWMNCRP_USEWINDOWSTYLE, // Enable/disable non-client rendering based on window
                          // style
...
enum DWMFLIP3DWINDOWPOLICY {
  DWMFLIP3D_DEFAULT, // Hide or include the window in Flip3D based on window
...
#define DWM_TNP_RECTDESTINATION                                                \
  0x00000001 // A value for the "rcDestination" member has been specified.
#define DWM_TNP_RECTSOURCE                                                     \
  0x00000002 // A value for the "rcSource" member has been specified.
#define DWM_TNP_OPACITY                                                        \
  0x00000004 // A value for the "opacity" member has been specified.
#define DWM_TNP_VISIBLE                                                        \
  0x00000008 // A value for the "fVisible" member has been specified.
#define DWM_TNP_SOURCECLIENTAREAONLY                                           \
  0x00000010 // A value for the "fSourceClientAreaOnly" member has been
             // specified.
#pragma endregion

typedef struct _DWM_THUMBNAIL_PROPERTIES {
  DWORD dwFlags; // Specifies which members of this struct have been specified
  RECT rcDestination; // The area in the destination window where the thumbnail
                      // will be rendered
  RECT rcSource; // The region of the source window to use as the thumbnail.  By
                 // default, the entire window is used as the thumbnail
  BYTE opacity;  // The opacity with which to render the thumbnail.  0 is fully
                // transparent, while 255 is fully opaque.  The default value is
                // 255
  BOOL fVisible; // Whether the thumbnail should be visible.  The default is
                 // FALSE
  BOOL fSourceClientAreaOnly; // Whether only the client area of the source
                              // window should be included in the thumbnail. The
                              // default is FALSE
...
DWMAPI_(BOOL)
DwmDefWindowProc(_In_ HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
                 _Out_ LRESULT *plResult);

DWMAPI
DwmEnableBlurBehindWindow(HWND hWnd, _In_ const DWM_BLURBEHIND *pBlurBehind);
...
DWMAPI
DwmExtendFrameIntoClientArea(HWND hWnd, _In_ const MARGINS *pMarInset);

DWMAPI
DwmGetColorizationColor(_Out_ DWORD *pcrColorization,
                        _Out_ BOOL *pfOpaqueBlend);

DWMAPI
DwmGetCompositionTimingInfo(HWND hwnd, _Out_ DWM_TIMING_INFO *pTimingInfo);
...
DWMAPI
DwmIsCompositionEnabled(_Out_ BOOL *pfEnabled);

DWMAPI
DwmModifyPreviousDxFrameDuration(HWND hwnd, INT cRefreshes, BOOL fRelative);

DWMAPI
DwmQueryThumbnailSourceSize(HTHUMBNAIL hThumbnail, _Out_ PSIZE pSize);

DWMAPI
DwmRegisterThumbnail(HWND hwndDestination, HWND hwndSource,
...
#define DWM_SIT_DISPLAYFRAME                                                   \
  0x00000001 // Display a window frame around the provided bitmap

DWMAPI DwmSetIconicThumbnail(HWND hwnd, HBITMAP hbmp, DWORD dwSITFlags);

DWMAPI DwmSetIconicLivePreviewBitmap(HWND hwnd, HBITMAP hbmp,
...
DWMAPI
DwmAttachMilContent(_In_ HWND hwnd);

DWMAPI
DwmDetachMilContent(_In_ HWND hwnd);
...
DWMAPI
DwmGetGraphicsStreamTransformHint(UINT uIndex, _Out_ MilMatrix3x2D *pTransform);

DWMAPI
DwmGetGraphicsStreamClient(UINT uIndex, _Out_ UUID *pClientUuid);
#endif // MILCORE_KERNEL_COMPONENT

DWMAPI
DwmGetTransportAttributes(_Out_writes_(1) BOOL *pfIsRemoting,
...
DWMAPI
DwmTransitionOwnedWindow(HWND hwnd,
...
DWMAPI
DwmRenderGesture(_In_ enum GESTURE_TYPE gt, _In_ UINT cContacts,
                 _In_reads_(cContacts) const DWORD *pdwPointerID,
                 _In_reads_(cContacts) const POINT *pPoints);

DWMAPI
DwmTetherContact(DWORD dwPointerID, BOOL fEnable, POINT ptTether);
...
DWMAPI
DwmShowContact(DWORD dwPointerID, enum DWM_SHOWCONTACT eShowContact);
#endif // NTDDI_WIN8
...
enum DWM_TAB_WINDOW_REQUIREMENTS {
  ...
  // In some configurations, admin/user setting or mode of the system means that
  // windows won't be tabbed This requirement says that the system/mode must
  // implement tabbing and if it does not nothing can be done to change this.
  ...
  // It must not extended its client area into the title bar using
  // DwmExtendFrameIntoClientArea It must not have DWMWA_NCRENDERING_POLICY set
  // to DWMNCRP_ENABLED
  DWMTWR_WINDOW_DWM_ATTRIBUTES = 0x0010,

  // The window is ineligible due to it's margins, most likely due to custom
  // handling in WM_NCCALCSIZE. The window must use the default window margins
  // for the non-client area.
  DWMTWR_WINDOW_MARGINS = 0x0020,

  // The window has been explicitly opted out by setting DWMWA_TABBING_ENABLED
  // to FALSE.
  ...
  // The group policy has configured this application to not participate in
  // tabbing.
  DWMTWR_GROUP_POLICY = 0x0100,

  // This is set if app compat has blocked tabs for this window. Can be
  // overridden per window by setting DWMWA_TABBING_ENABLED to TRUE. That does
  // not override any other tabbing requirements.
...
DWMAPI
DwmGetUnmetTabRequirements(_In_opt_ HWND appWindow,
                           _Out_ enum DWM_TAB_WINDOW_REQUIREMENTS *value);
