...
// struct WLAN_PROFILE_INFO defines the basic information of an 802.11 network
// profile
...
// "Mixed cell" suspected (AP not beaconing privacy, we have privacy enabled
// profile)
...
#define WLAN_AVAILABLE_NETWORK_CONNECTED                                       \
  0x00000001 // This network is currently connected
#define WLAN_AVAILABLE_NETWORK_HAS_PROFILE                                     \
  0x00000002 // There is a profile for this network
#define WLAN_AVAILABLE_NETWORK_CONSOLE_USER_PROFILE                            \
  0x00000004 // The profile is the active console user's per user profile
#define WLAN_AVAILABLE_NETWORK_INTERWORKING_SUPPORTED                          \
  0x00000008 // Interworking is supported
#define WLAN_AVAILABLE_NETWORK_HOTSPOT2_ENABLED                                \
  0x00000010                                              // Hotspot2 is enabled
...
#define WLAN_AVAILABLE_NETWORK_AUTO_CONNECT_FAILED                             \
  0x00000100 // This network failed to connect
...
typedef struct _WLAN_RATE_SET {
  ...
  _Field_size_part_(DOT11_RATE_SET_MAX_LENGTH,
                    uRateSetLength) USHORT usRateSet[DOT11_RATE_SET_MAX_LENGTH];
} WLAN_RATE_SET, *PWLAN_RATE_SET;

#define WLAN_MAX_PHY_TYPE_NUMBER 8
//
// struct WLAN_AVAILABLE_NETWORK defines information needed for an available
// network
...
typedef struct _WLAN_QOS_INFO {
  ...
  // bConnected indicates whether or not there is an established connection and
  // therefore whether the connection QoS info is present in connectionQoSInfo.
  BOOL bConnected;
  // QoS info of the current connection. Meaningful only if bConnected is true;
  // otherwise, if bConnected is false, connectionQoSInfo will be zeroed and
  // should be ignored.
  WLAN_CONNECTION_QOS_INFO connectionQoSInfo;
} WLAN_QOS_INFO, *PWLAN_QOS_INFO;

// structure WLAN_CONNECTION_ATTRIBUTES defines attributes of a wireless
// connection
...
#ifdef __midl
  [ unique,
    size_is(dwNumberOfItems) ] DOT11_AUTH_CIPHER_PAIR pAuthCipherPairList[*];
#else
...
#ifdef __midl
  [
    unique, size_is(dwNumberOfItems)
...
// Ignore the privacy bit for the association. This is used to support easy
// config. This flag is valid only for wlan_connection_mode_temporary_profile
...
// user profile, with the name generated from the SSID using
// WlanUtf8SsidToDisplayName. If there is already a profile with the same name,
// a number will be appended to the end of the profile name. The profile will be
// saved with manual connection mode, unless
// WLAN_CONNECTION_PERSIST_DISCOVERY_PROFILE_CONNECTION_MODE_AUTO is also
// specified.
...
#define WLAN_CONNECTION_PERSIST_DISCOVERY_PROFILE_CONNECTION_MODE_AUTO         \
  0x00000020
// To be used in conjunction with WLAN_CONNECTION_PERSIST_DISCOVERY_PROFILE. The
// discovery profile will be persisted and attempt to overwrite an existing
...
#define WLAN_CONNECTION_NOTIFICATION_ADHOC_NETWORK_FORMED                      \
  0x00000001 // Formed ad hoc network
#define WLAN_CONNECTION_NOTIFICATION_CONSOLE_USER_PROFILE                      \
  0x00000004 // The profile is the active console user's per user profile
...
typedef VOID(WINAPI *WLAN_NOTIFICATION_CALLBACK)(PWLAN_NOTIFICATION_DATA,
                                                 PVOID);
...
// public APIs
DWORD WINAPI WlanOpenHandle(_In_ DWORD dwClientVersion,
                            _Reserved_ PVOID pReserved,
                            _Out_ PDWORD pdwNegotiatedVersion,
                            _Out_ PHANDLE phClientHandle);

DWORD WINAPI WlanCloseHandle(_In_ HANDLE hClientHandle,
                             _Reserved_ PVOID pReserved);

DWORD WINAPI
WlanEnumInterfaces(_In_ HANDLE hClientHandle, _Reserved_ PVOID pReserved,
                   _Outptr_ PWLAN_INTERFACE_INFO_LIST *ppInterfaceList);

DWORD WINAPI WlanSetAutoConfigParameter(_In_ HANDLE hClientHandle,
                                        _In_ WLAN_AUTOCONF_OPCODE OpCode,
                                        _In_ DWORD dwDataSize,
                                        _In_reads_bytes_(dwDataSize)
                                            CONST PVOID pData,
                                        _Reserved_ PVOID pReserved);

DWORD WINAPI WlanQueryAutoConfigParameter(
    _In_ HANDLE hClientHandle, _In_ WLAN_AUTOCONF_OPCODE OpCode,
    _Reserved_ PVOID pReserved, _Out_ PDWORD pdwDataSize,
...
DWORD WINAPI WlanSetInterface(_In_ HANDLE hClientHandle,
...
DWORD WINAPI
WlanQueryInterface(_In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
                   _In_ WLAN_INTF_OPCODE OpCode, _Reserved_ PVOID pReserved,
                   _Out_ PDWORD pdwDataSize,
                   _Outptr_result_bytebuffer_(*pdwDataSize) PVOID *ppData,
                   _Out_opt_ PWLAN_OPCODE_VALUE_TYPE pWlanOpcodeValueType);

DWORD WINAPI WlanIhvControl(_In_ HANDLE hClientHandle,
                            _In_ CONST GUID *pInterfaceGuid,
                            _In_ WLAN_IHV_CONTROL_TYPE Type,
                            _In_ DWORD dwInBufferSize,
                            _In_reads_bytes_(dwInBufferSize) PVOID pInBuffer,
                            _In_ DWORD dwOutBufferSize,
                            _Inout_updates_bytes_opt_(dwOutBufferSize)
                                PVOID pOutBuffer,
                            _Out_ PDWORD pdwBytesReturned);

DWORD WINAPI WlanScan(_In_ HANDLE hClientHandle,
                      _In_ CONST GUID *pInterfaceGuid,
...
DWORD WINAPI WlanGetAvailableNetworkList(
    _In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
    _In_ DWORD dwFlags, _Reserved_ PVOID pReserved,
...
DWORD WINAPI WlanGetNetworkBssList(_In_ HANDLE hClientHandle,
                                   _In_ CONST GUID *pInterfaceGuid,
                                   _In_opt_ CONST PDOT11_SSID pDot11Ssid,
...
DWORD WINAPI
WlanConnect(_In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
            _In_ CONST PWLAN_CONNECTION_PARAMETERS pConnectionParameters,
            _Reserved_ PVOID pReserved);

DWORD WINAPI
WlanConnect2(_In_ HANDLE hClientHandle, _In_ const GUID *pInterfaceGuid,
             _In_ const PWLAN_CONNECTION_PARAMETERS_V2 pConnectionParameters,
             _Reserved_ PVOID pReserved);

DWORD WINAPI WlanDisconnect(_In_ HANDLE hClientHandle,
                            _In_ CONST GUID *pInterfaceGuid,
                            _Reserved_ PVOID pReserved);

DWORD WINAPI WlanRegisterNotification(
    _In_ HANDLE hClientHandle, _In_ DWORD dwNotifSource,
    _In_ BOOL bIgnoreDuplicate,
    _In_opt_ WLAN_NOTIFICATION_CALLBACK funcCallback,
    _In_opt_ PVOID pCallbackContext, _Reserved_ PVOID pReserved,
    _Out_opt_ PDWORD pdwPrevNotifSource);

DWORD WINAPI WlanGetProfile(_In_ HANDLE hClientHandle,
                            _In_ CONST GUID *pInterfaceGuid,
...
DWORD WINAPI WlanSetProfileEapUserData(
    _In_ HANDLE hClientHandle, _In_ const GUID *pInterfaceGuid,
    _In_ LPCWSTR strProfileName, _In_ EAP_METHOD_TYPE eapType,
    _In_ DWORD dwFlags, _In_ DWORD dwEapUserDataSize,
    _In_reads_bytes_opt_(dwEapUserDataSize) const LPBYTE pbEapUserData,
    _Reserved_ PVOID pReserved);

DWORD WINAPI WlanSetProfileEapXmlUserData(_In_ HANDLE hClientHandle,
                                          _In_ const GUID *pInterfaceGuid,
                                          _In_ LPCWSTR strProfileName,
                                          _In_ DWORD dwFlags,
...
DWORD WINAPI WlanSetProfile(_In_ HANDLE hClientHandle,
                            _In_ CONST GUID *pInterfaceGuid, _In_ DWORD dwFlags,
                            _In_ LPCWSTR strProfileXml,
                            _In_opt_ LPCWSTR strAllUserProfileSecurity,
                            _In_ BOOL bOverwrite, _Reserved_ PVOID pReserved,
                            _Out_ DWORD *pdwReasonCode);

DWORD WINAPI WlanDeleteProfile(_In_ HANDLE hClientHandle,
                               _In_ CONST GUID *pInterfaceGuid,
                               _In_ LPCWSTR strProfileName,
                               _Reserved_ PVOID pReserved);

DWORD WINAPI WlanRenameProfile(_In_ HANDLE hClientHandle,
                               _In_ CONST GUID *pInterfaceGuid,
...
DWORD WINAPI
WlanGetProfileList(_In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
                   _Reserved_ PVOID pReserved,
                   _Outptr_ PWLAN_PROFILE_INFO_LIST *ppProfileList);

DWORD WINAPI WlanSetProfileList(_In_ HANDLE hClientHandle,
                                _In_ CONST GUID *pInterfaceGuid,
                                _In_ DWORD dwItems,
                                _In_reads_(dwItems) LPCWSTR *strProfileNames,
                                _Reserved_ PVOID pReserved);

DWORD WINAPI WlanSetProfilePosition(_In_ HANDLE hClientHandle,
                                    _In_ CONST GUID *pInterfaceGuid,
                                    _In_ LPCWSTR strProfileName,
...
DWORD WINAPI WlanSetProfileCustomUserData(
    _In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
    _In_ LPCWSTR strProfileName, _In_ DWORD dwDataSize,
    _In_reads_bytes_(dwDataSize) CONST PBYTE pData, _Reserved_ PVOID pReserved);

DWORD WINAPI WlanGetProfileCustomUserData(
    _In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
    _In_ LPCWSTR strProfileName, _Reserved_ PVOID pReserved,
    _Out_ DWORD *pdwDataSize,
    _Outptr_result_bytebuffer_(*pdwDataSize) PBYTE *ppData);

DWORD WINAPI WlanSetFilterList(_In_ HANDLE hClientHandle,
                               _In_ WLAN_FILTER_LIST_TYPE wlanFilterListType,
                               _In_opt_ CONST PDOT11_NETWORK_LIST pNetworkList,
                               _Reserved_ PVOID pReserved);

DWORD WINAPI WlanGetFilterList(_In_ HANDLE hClientHandle,
                               _In_ WLAN_FILTER_LIST_TYPE wlanFilterListType,
                               _Reserved_ PVOID pReserved,
                               _Outptr_ PDOT11_NETWORK_LIST *ppNetworkList);

DWORD WINAPI
WlanSetPsdIEDataList(_In_ HANDLE hClientHandle, _In_opt_ LPCWSTR strFormat,
                     _In_opt_ CONST PWLAN_RAW_DATA_LIST pPsdIEDataList,
                     _Reserved_ PVOID pReserved);

DWORD WINAPI WlanSaveTemporaryProfile(
    _In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
    _In_ LPCWSTR strProfileName, _In_opt_ LPCWSTR strAllUserProfileSecurity,
    _In_ DWORD dwFlags, _In_ BOOL bOverWrite, _Reserved_ PVOID pReserved);

DWORD WINAPI WlanDeviceServiceCommand(
    _In_ HANDLE hClientHandle, _In_ CONST GUID *pInterfaceGuid,
    _In_ LPGUID pDeviceServiceGuid, _In_ DWORD dwOpCode,
    _In_ DWORD dwInBufferSize,
...
// client side APIs
DWORD WINAPI
WlanExtractPsdIEDataList(_In_ HANDLE hClientHandle, _In_ DWORD dwIeDataSize,
                         _In_reads_bytes_(dwIeDataSize) CONST PBYTE pRawIeData,
                         _In_ LPCWSTR strFormat, _Reserved_ PVOID pReserved,
                         _Outptr_ PWLAN_RAW_DATA_LIST *ppPsdIEDataList);

DWORD WINAPI WlanReasonCodeToString(_In_ DWORD dwReasonCode,
                                    _In_ DWORD dwBufferSize,
                                    _In_reads_(dwBufferSize)
...
DWORD WINAPI WlanSetSecuritySettings(_In_ HANDLE hClientHandle,
                                     _In_ WLAN_SECURABLE_OBJECT SecurableObject,
                                     _In_ LPCWSTR strModifiedSDDL);

DWORD WINAPI WlanGetSecuritySettings(
    _In_ HANDLE hClientHandle, _In_ WLAN_SECURABLE_OBJECT SecurableObject,
    _Out_opt_ PWLAN_OPCODE_VALUE_TYPE pValueType,
    _Outptr_ LPWSTR *pstrCurrentSDDL, _Out_ PDWORD pdwGrantedAccess);
...
DWORD WINAPI WlanUIEditProfile(_In_ DWORD dwClientVersion,
                               _In_ LPCWSTR wstrProfileName,
                               _In_ GUID *pInterfaceGuid, _In_ HWND hWnd,
...
DWORD
WINAPI
WlanHostedNetworkStartUsing(_In_ HANDLE hClientHandle,
                            _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
                            _Reserved_ PVOID pvReserved);

DWORD
WINAPI
WlanHostedNetworkStopUsing(_In_ HANDLE hClientHandle,
                           _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
                           _Reserved_ PVOID pvReserved);

DWORD
WINAPI
WlanHostedNetworkForceStart(_In_ HANDLE hClientHandle,
                            _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
                            _Reserved_ PVOID pvReserved);

DWORD
WINAPI
WlanHostedNetworkForceStop(_In_ HANDLE hClientHandle,
                           _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
                           _Reserved_ PVOID pvReserved);
...
// Definitions required for calling WlanHostedNetworkQueryProperty and
...
WlanHostedNetworkQueryProperty(
    _In_ HANDLE hClientHandle, _In_ WLAN_HOSTED_NETWORK_OPCODE OpCode,
    _Out_ PDWORD pdwDataSize,
...
DWORD
WINAPI
WlanHostedNetworkSetProperty(_In_ HANDLE hClientHandle,
...
DWORD
WINAPI
WlanHostedNetworkInitSettings(_In_ HANDLE hClientHandle,
                              _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
                              _Reserved_ PVOID pvReserved);

DWORD
WINAPI
WlanHostedNetworkRefreshSecuritySettings(_In_ HANDLE hClientHandle,
                                         _Out_opt_ PWLAN_HOSTED_NETWORK_REASON
                                             pFailReason,
                                         _Reserved_ PVOID pvReserved);
...
#ifdef __midl
  [ unique,
    size_is(dwNumberOfPeers) ] WLAN_HOSTED_NETWORK_PEER_STATE PeerList[*];
#else
...
WlanHostedNetworkSetSecondaryKey(
    _In_ HANDLE hClientHandle, _In_ DWORD dwKeyLength,
    _In_reads_bytes_(dwKeyLength) PUCHAR pucKeyData, _In_ BOOL bIsPassPhrase,
    _In_ BOOL bPersistent, _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
    _Reserved_ PVOID pvReserved);
...
WlanHostedNetworkQuerySecondaryKey(
    _In_ HANDLE hClientHandle, _Out_ PDWORD pdwKeyLength,
    _Outptr_result_buffer_(*pdwKeyLength) PUCHAR *ppucKeyData,
    _Out_ PBOOL pbIsPassPhrase, _Out_ PBOOL pbPersistent,
    _Out_opt_ PWLAN_HOSTED_NETWORK_REASON pFailReason,
    _Reserved_ PVOID pvReserved);

//
// This function is used to register and unregister notifications on virtual
// station.
//
DWORD
WINAPI
WlanRegisterVirtualStationNotification(_In_ HANDLE hClientHandle,
                                       _In_ BOOL bRegister,
                                       _Reserved_ PVOID pReserved);
...
//
DEFINE_GUID(GUID_DEVINTERFACE_WIFIDIRECT_DEVICE, 0x439b20af, 0x8955, 0x405b,
            0x99, 0xf0, 0xa6, 0x2a, 0xf0, 0xc6, 0x8d, 0x43);
...
//
DEFINE_GUID(GUID_AEPSERVICE_WIFIDIRECT_DEVICE, 0xcc29827c, 0x9caf, 0x4928, 0x99,
            0xa9, 0x18, 0xf7, 0xc2, 0x38, 0x13, 0x89);
...
// interface identified by this GUID.
//
DEFINE_GUID(GUID_DEVINTERFACE_ASP_INFRA_DEVICE, 0xff823995, 0x7a72, 0x4c80,
            0x87, 0x57, 0xc6, 0x7e, 0xe1, 0x3d, 0x1a, 0x49);
...
// Availability: Always.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_DeviceAddress, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x01);
...
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_InterfaceAddress, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x02);
...
// Availability: When DEVPKEY_WiFiDirect_IsConnected == DEVPROP_TRUE.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_InterfaceGuid, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x03);
...
// Availability: Always.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_GroupId, 0x1506935d, 0xe3e7, 0x450f, 0x86,
                  0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x04);
...
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_IsConnected, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x05);
...
//               or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_IsVisible, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x06);
...
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_IsLegacyDevice, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x07);

//
// Property: DEVPKEY_WiFiDirect_MiracastVersion
// Description: A value indicating version of Miracast protocol if WFD Device is
// Miracast capable. Type: DEVPROP_TYPE_UINT32 Availability: If Miracast
// capable, set to the highest version of protocol supported or empty
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_MiracastVersion, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x08);

//
// Property: DEVPKEY_WiFiDirect_IsMiracastLCPSupported
// Description: A value indicating if link content protection is supported by
// the device if WFD Device is Miracast capable. Type: DEVPROP_TYPE_BOOLEAN
// Availability: If link content protection is supported, set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE,
//               or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_IsMiracastLCPSupported, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x09);

//
// Property: DEVPKEY_WiFiDirect_Services
// Description: A value indicating services supported by the Wi Fi Direct
// device. Type: DEVPROP_TYPE_STRING_LIST Availability: If Miracast capable or
// advertising well-known service such as WSB, else empty
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_Services, 0x1506935d, 0xe3e7, 0x450f, 0x86,
                  0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x0A);
...
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_SupportedChannelList, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x0B);
...
// Availability: Always.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_InformationElements, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x0C);
...
// Availability: Always.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_DeviceAddressCopy, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x0D);
...
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_IsRecentlyAssociated, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x0E);

//
// Property: DEVPKEY_WiFiDirect_Service_Aeps
// Description: A value indicating AEP Service IDs of services supported by the
// Wi-Fi Direct device. Type: DEVPROP_TYPE_STRING_LIST Availability: If
// advertising well-known service such as WSB, else empty
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_Service_Aeps, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x0F);

//
// Property: DEVPKEY_WiFiDirect_NoAutoProject
// Description: A value indicating if WFDSConMgr initiated the association so
// hotplug auto-project should not be used.
//              When true, WFDSConMgr is responsible for calling
//              StartMiracastDisplayDevice
// Type: DEVPROP_TYPE_BOOLEAN
// Availability: If WFDSConMgr initiated the association, set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE,
//               or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_NoMiracastAutoProject, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x10);

//
// Property: DEVPKEY_InfraCast_Supported
// Description: A value indicating if the remote Miracast Sink supports
// infrastructure connections Type: DEVPROP_TYPE_BOOLEAN Availability: If remote
// device is a Miracast sink and supports infrastructure connections, then this
// is set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE, or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_Supported, 0x1506935d, 0xe3e7, 0x450f, 0x86,
                  0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x11);

//
// Property: DEVPKEY_InfraCast_StreamSecuritySupported
// Description: A value indicating if the remote Miracast Sink supports stream
// security Type: DEVPROP_TYPE_BOOLEAN Availability: If remote device is a
// Miracast sink and supports stream security, then this is set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE, or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_StreamSecuritySupported, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x12);

//
// Property: DEVPKEY_InfraCast_AccessPointBssid
// Description: A value indicating the BSSID of the Access Point the Miracast
// Sink is connected to, if the network is secure. Type: DEVPROP_TYPE_BINARY
// Binary Data: DOT11_MAC_ADDRESS (UCHAR[6])
// Availability: If the Miracast Sink connection to an Access Point is secure,
// this value is set else is empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_AccessPointBssid, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x13);

//
// Property: DEVPKEY_InfraCast_SinkHostName
// Description: A value indicating the DNS hostname of the Miracast Sink.
// Type: DEVPROP_TYPE_STRING
// Availability: If the Miracast Sink supports connection over infrastructure,
// this value is set else is empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_SinkHostName, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x14);

//
// Parameter: DEVPKEY_InfraCast_ChallengeAep
// Description: Tells InfraCast DAF provider which challenge is supposed to be
// used next Type: DEVPROP_TYPE_STRING
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_ChallengeAep, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x15);

//
// Property: DEVPKEY_WiFiDirect_IsDMGCapable
// Description: Indicates that the device was discovered over a Directional
// Multi-Gigabit (802.11ad) interface Type: DEVPROP_TYPE_BOOLEAN Availability:
// Always
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_IsDMGCapable, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x16);

//
// Parameter: DEVPKEY_InfraCast_DevnodeAep
// Description: Tells InfraCast DAF provider which AEP it needs to take offline
// when the association is closed Type: DEVPROP_TYPE_STRING
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_DevnodeAep, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x17);

//
// Property: DEVPKEY_WiFiDirect_FoundWsbService
// Description: A value indicating if this device was found with the WSB
// service. Persisted in the AEP store
//              so that challenges can add the WSB hash to the query if the
//              device previously had the WSB service.
// Type: DEVPROP_TYPE_BOOLEAN
// Availability: If WSB service was found during discovery, set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE,
//               or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_FoundWsbService, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x18);

//
// Property: DEVPKEY_InfraCast_HostName_ResolutionMode
// Description: String representation of InfraCast Sink HostName resolution mode
// (Valid Values: Unknown, mDNS, DNS) Type: DEVPROP_TYPE_STRING Availability:
// Always.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_HostName_ResolutionMode, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x19);

//
// Property: DEVPKEY_InfraCast_SinkIpAddress
// Description: A value containing an IPv4 or IPV6 IP address of the Miracast
// Sink. Type: DEVPROP_TYPE_STRING Availability: If the Miracast Sink supports
// connection over infrastructure, this value maybe set, else is empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_SinkIpAddress, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x1A);

//
// Property: DEVPKEY_WiFiDirect_TransientAssociation
// Description: Set when the association should not be persisted. Tells the DAF
// provider to remove the association
//              when the devnode goes offline
// Type: DEVPROP_TYPE_BOOLEAN
// Availability: If the pairing result indicated that the persistent group is
// not supported, set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE,
//               or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_TransientAssociation, 0x1506935d, 0xe3e7,
                  0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x1B);

//
// Property: DEVPKEY_WiFiDirect_LinkQuality
// Description: The signal quality value ranging from 0 through 100. A value of
// 100 specifies the highest link quality. Type: DEVPROP_TYPE_UINT32
// Availability: Always
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_LinkQuality, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x1C);

//
// Property: DEVPKEY_InfraCast_PinSupported
// Description: A value indicating if the remote Miracast over Infrastructure
// Sink supports PIN entry Type: DEVPROP_TYPE_BOOLEAN Availability: If remote
// device is a Miracast sink and supports Infrastructure and PIN entry, then
// this is set to DEVPROP_TRUE,
//               otherwise set to DEVPROP_FALSE, or empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_PinSupported, 0x1506935d, 0xe3e7, 0x450f,
                  0x86, 0x37, 0x82, 0x23, 0x3e, 0xbe, 0x5f, 0x6E, 0x1D);

//
// Property: DEVPKEY_InfraCast_RtspTcpConnectionParametersSupported
// Description: A value indicating if the remote Miracast over Infrastructure
// Sink supports changing the RTSP TCP connection direction. Type:
// DEVPROP_TYPE_BOOLEAN Availability: If remote device is a Miracast sink and
// supports Infrastructure and changing the RTSP TCP connection direction, then
// this
//               is set to DEVPROP_TRUE, otherwise set to DEVPROP_FALSE, or
//               empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_InfraCast_RtspTcpConnectionParametersSupported,
                  0x1506935d, 0xe3e7, 0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e,
                  0xbe, 0x5f, 0x6E, 0x1E);
...
// Availability: If Miracast capable, set to the value of the Session Management
// Control Port in the WFD Device Information sublement.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_Miracast_SessionMgmtControlPort,
                  0x1506935d, 0xe3e7, 0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e,
                  0xbe, 0x5f, 0x6E, 0x1F);

//
// Property: DEVPKEY_WiFiDirect_RtspTcpConnectionParametersSupported
// Description: A value indicating if the remote Miracast Sink supports changing
// the RTSP TCP connection direction. Type: DEVPROP_TYPE_BOOLEAN Availability:
// If remote device is a Miracast sink and supports changing the RTSP TCP
// connection direction, then this
//               is set to DEVPROP_TRUE, otherwise set to DEVPROP_FALSE, or
//               empty.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirect_RtspTcpConnectionParametersSupported,
                  0x1506935d, 0xe3e7, 0x450f, 0x86, 0x37, 0x82, 0x23, 0x3e,
                  0xbe, 0x5f, 0x6E, 0x20);
...
// Availability: Always.
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirectServices_ServiceAddress, 0x31b37743, 0x7c5e,
                  0x4005, 0x93, 0xe6, 0xe9, 0x53, 0xf9, 0x2b, 0x82, 0xe9, 0x02);
...
// Availability: Always
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirectServices_ServiceName, 0x31b37743, 0x7c5e,
                  0x4005, 0x93, 0xe6, 0xe9, 0x53, 0xf9, 0x2b, 0x82, 0xe9, 0x03);
...
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirectServices_ServiceInformation, 0x31b37743,
...
// Availability: Always
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirectServices_AdvertisementId, 0x31b37743,
                  0x7c5e, 0x4005, 0x93, 0xe6, 0xe9, 0x53, 0xf9, 0x2b, 0x82,
                  0xe9, 0x05);
...
// Availability: Always
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFiDirectServices_ServiceConfigMethods, 0x31b37743,
                  0x7c5e, 0x4005, 0x93, 0xe6, 0xe9, 0x53, 0xf9, 0x2b, 0x82,
                  0xe9, 0x06);
...
//              This property is only allowed as a filter in the query and is
//              never returned for the Aep Service object.
// Type: DEVPROP_TYPE_STRING
...
DWORD
WINAPI
WFDOpenHandle(_In_ DWORD dwClientVersion, _Out_ PDWORD pdwNegotiatedVersion,
              _Out_ PHANDLE phClientHandle);

DWORD
WINAPI
WFDCloseHandle(_In_ HANDLE hClientHandle);

typedef VOID(WINAPI *WFD_OPEN_SESSION_COMPLETE_CALLBACK)(
    _In_ HANDLE hSessionHandle, _In_ PVOID pvContext,
    _In_ GUID guidSessionInterface, _In_ DWORD dwError,
    _In_ DWORD dwReasonCode);

DWORD
WINAPI
WFDStartOpenSession(_In_ HANDLE hClientHandle,
...
DWORD
WINAPI
WFDCancelOpenSession(_In_ HANDLE hSessionHandle);

DWORD
WINAPI
WFDOpenLegacySession(_In_ HANDLE hClientHandle,
...
DWORD
WINAPI
WFDCloseSession(_In_ HANDLE hSessionHandle);

DWORD
WINAPI
WFDUpdateDeviceVisibility(_In_ PDOT11_MAC_ADDRESS pDeviceAddress);
...
//     Guid by WlanSvc
// Type: DEVPROP_TYPE_GUID
// Availability: This will be set after first time WlanSvc brings up its
// interfaces
//
DEFINE_DEVPROPKEY(DEVPKEY_WiFi_InterfaceGuid, 0xef1167eb, 0xcbfc, 0x4341, 0xa5,
                  0x68, 0xa7, 0xc9, 0x1a, 0x68, 0x98, 0x2c, 0x02);
