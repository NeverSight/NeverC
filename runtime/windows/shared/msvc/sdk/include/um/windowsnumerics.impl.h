...
// Define conversion operators between these C++ structs and a set of WinRT ABI
// types with matching layouts.
#define _DEFINE_WINDOWS_NUMERICS_INTEROP_(CppName, WinRTName)                  \
  ...
  operator _WINDOWS_NUMERICS_INTEROP_NAMESPACE_::WinRTName() const {           \
    return *reinterpret_cast<                                                  \
        _WINDOWS_NUMERICS_INTEROP_NAMESPACE_::WinRTName const *>(this);        \
  }
...
// Mark the numerics constructors as constexpr, except when using the C++/CX
// projection.
...
_WINDOWS_NUMERICS_BEGIN_NAMESPACE_ {
  ...
  struct float3x2 {
    ...
    // Constructors.
    float3x2() = default;
    _WINDOWS_NUMERICS_CONSTEXPR_ float3x2(float m11, float m12, float m21,
  ...
  struct float4x4 {
    ...
    // Constructors.
    float4x4() = default;
    _WINDOWS_NUMERICS_CONSTEXPR_
    float4x4(float m11, float m12, float m13, float m14, float m21, float m22,
  ...
  // Factory functions.
  float4x4 make_float4x4_billboard(
      float3 const &objectPosition, float3 const &cameraPosition,
      float3 const &cameraUpVector, float3 const &cameraForwardVector);
  float4x4 make_float4x4_constrained_billboard(
      float3 const &objectPosition, float3 const &cameraPosition,
      float3 const &rotateAxis, float3 const &cameraForwardVector,
      float3 const &objectForwardVector);
  float4x4 make_float4x4_translation(float3 const &position);
  float4x4 make_float4x4_translation(float xPosition, float yPosition,
                                     float zPosition);
  float4x4 make_float4x4_scale(float xScale, float yScale, float zScale);
  float4x4 make_float4x4_scale(float xScale, float yScale, float zScale,
                               float3 const &centerPoint);
  ...
  float4x4 make_float4x4_perspective_field_of_view(
      float fieldOfView, float aspectRatio, float nearPlaneDistance,
      float farPlaneDistance);
  float4x4 make_float4x4_perspective(float width, float height,
                                     float nearPlaneDistance,
                                     float farPlaneDistance);
  float4x4 make_float4x4_perspective_off_center(
      float left, float right, float bottom, float top, float nearPlaneDistance,
      float farPlaneDistance);
  float4x4 make_float4x4_orthographic(float width, float height,
                                      float zNearPlane, float zFarPlane);
  float4x4 make_float4x4_orthographic_off_center(
      float left, float right, float bottom, float top, float zNearPlane,
      float zFarPlane);
  float4x4 make_float4x4_look_at(float3 const &cameraPosition,
                                 float3 const &cameraTarget,
                                 float3 const &cameraUpVector);
  float4x4 make_float4x4_world(float3 const &position, float3 const &forward,
                               float3 const &up);
  float4x4 make_float4x4_from_quaternion(quaternion const &quaternion);
  float4x4 make_float4x4_from_yaw_pitch_roll(float yaw, float pitch,
                                             float roll);
  float4x4 make_float4x4_shadow(float3 const &lightDirection,
                                plane const &plane);
  ...
  bool decompose(float4x4 const &matrix, _Out_ float3 *scale,
                 _Out_ quaternion *rotation, _Out_ float3 *translation);
  ...
  // Factory functions.
  plane make_plane_from_vertices(float3 const &point1, float3 const &point2,
                                 float3 const &point3);
  ...
  struct quaternion {
    ...
    _WINDOWS_NUMERICS_CONSTEXPR_ quaternion(float3 vectorPart,
                                            float scalarPart);
  ...
  quaternion make_quaternion_from_yaw_pitch_roll(float yaw, float pitch,
                                                 float roll);
  ...
  quaternion slerp(quaternion const &quaternion1, quaternion const &quaternion2,
                   float amount);
  quaternion lerp(quaternion const &quaternion1, quaternion const &quaternion2,
                  float amount);
...
XMLoadFloat2(_In_ _WINDOWS_NUMERICS_NAMESPACE_::float2 const *pSource);
XMVECTOR XM_CALLCONV
XMLoadFloat3(_In_ _WINDOWS_NUMERICS_NAMESPACE_::float3 const *pSource);
XMVECTOR XM_CALLCONV
XMLoadFloat4(_In_ _WINDOWS_NUMERICS_NAMESPACE_::float4 const *pSource);
XMMATRIX XM_CALLCONV
XMLoadFloat3x2(_In_ _WINDOWS_NUMERICS_NAMESPACE_::float3x2 const *pSource);
XMMATRIX XM_CALLCONV
XMLoadFloat4x4(_In_ _WINDOWS_NUMERICS_NAMESPACE_::float4x4 const *pSource);
XMVECTOR XM_CALLCONV
XMLoadPlane(_In_ _WINDOWS_NUMERICS_NAMESPACE_::plane const *pSource);
XMVECTOR XM_CALLCONV
XMLoadQuaternion(_In_ _WINDOWS_NUMERICS_NAMESPACE_::quaternion const *pSource);

void XM_CALLCONV
XMStoreFloat2(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::float2 *pDestination,
              _In_ FXMVECTOR value);
void XM_CALLCONV
XMStoreFloat3(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::float3 *pDestination,
              _In_ FXMVECTOR value);
void XM_CALLCONV
XMStoreFloat4(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::float4 *pDestination,
              _In_ FXMVECTOR value);
void XM_CALLCONV
XMStoreFloat3x2(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::float3x2 *pDestination,
                _In_ FXMMATRIX value);
void XM_CALLCONV
XMStoreFloat4x4(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::float4x4 *pDestination,
                _In_ FXMMATRIX value);
void XM_CALLCONV
XMStorePlane(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::plane *pDestination,
             _In_ FXMVECTOR value);
void XM_CALLCONV
XMStoreQuaternion(_Out_ _WINDOWS_NUMERICS_NAMESPACE_::quaternion *pDestination,
                  _In_ FXMVECTOR value);
} // namespace DirectX
