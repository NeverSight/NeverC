...
_Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetApiBufferAllocate(_In_ DWORD ByteCount,
                         _Outptr_result_bytebuffer_(ByteCount) LPVOID *Buffer);

_Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetApiBufferFree(_Frees_ptr_opt_ LPVOID Buffer);

_Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetApiBufferReallocate(_Frees_ptr_opt_ LPVOID OldBuffer,
                           _In_ DWORD NewByteCount,
                           _Outptr_result_bytebuffer_(NewByteCount)
                               LPVOID *NewBuffer);

_Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetApiBufferSize(_In_ LPVOID Buffer, _Out_ LPDWORD ByteCount);
