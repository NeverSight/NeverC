...
#define PERF_METADATA_MULTIPLE_INSTANCES                                       \
  (-2)                                  // metadata-only, 0 or more instances
...
// Indicates the counter is not a  counter but rather Unicode text Display as
// text.
...
// no data.
//
typedef DWORD(APIENTRY PM_OPEN_PROC)(_In_opt_ LPWSTR pContext);
...
typedef DWORD(APIENTRY PM_COLLECT_PROC)(
    ...
    _Inout_ _At_(*ppData, _Pre_writable_byte_size_(*pcbTotalBytes)
                              _Post_writable_byte_size_(*pcbTotalBytes))
        _At_(_Old_(*ppData),
             _Post_writable_byte_size_(*pcbTotalBytes)) void **ppData,
    _Inout_ DWORD *pcbTotalBytes, _Out_ DWORD *pNumObjectTypes);
