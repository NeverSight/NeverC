...
_Check_return_ SHSTDAPI
SHGetMalloc(_Outptr_ IMalloc **ppMalloc);  // CoGetMalloc(MEMCTX_TASK,ppMalloc)
...
//   cchMax     [in]  -- Specifies the size of szIconFile (almost always
//   MAX_PATH) piIndex    [out] -- Sepcifies the address of UINT for the index.
...
#define GIL_DEFAULTICON                                                        \
  0x0040 // get the default icon location if the final one takes too long to get
#define GIL_FORSHORTCUT 0x0080 // the icon is for a shortcut to the object
#define GIL_CHECKSHIELD                                                        \
  0x0200 // return GIL_SHIELD or GIL_FORCENOSHIELD, don't block if GIL_ASYNC is
         // set

// GetIconLocation() return flags

#define GIL_SIMULATEDOC 0x0001 // simulate this document icon for this
#define GIL_PERINSTANCE                                                        \
  0x0002 // icons from this class are per instance (each file has its own)
#define GIL_PERCLASS                                                           \
  0x0004 // icons from this class per class (shared for all files of this type)
#define GIL_NOTFILENAME                                                        \
  0x0008 // location is not a filename, must call ::ExtractIcon
#define GIL_DONTCACHE 0x0010 // this icon should not be cached
#define GIL_SHIELD 0x0200    // icon should be "stamped" with the LUA shield
#define GIL_FORCENOSHIELD                                                      \
  0x0400 // icon must *not* be "stamped" with the LUA shield
...
DECLARE_INTERFACE_IID_(IExtractIconA, IUnknown,
  ...
  STDMETHOD(GetIconLocation)(THIS_ UINT uFlags,
                             _Out_writes_(cchMax) PSTR pszIconFile, UINT cchMax,
                             _Out_ int *piIndex, _Out_ UINT *pwFlags) PURE;

  STDMETHOD(Extract)(THIS_ _In_ PCSTR pszFile, UINT nIconIndex,
                     _Out_opt_ HICON * phiconLarge,
                     _Out_opt_ HICON * phiconSmall, UINT nIconSize) PURE;
};
...
DECLARE_INTERFACE_IID_(IExtractIconW, IUnknown,
  ...
  STDMETHOD(GetIconLocation)(
      THIS_ UINT uFlags, _Out_writes_(cchMax) PWSTR pszIconFile, UINT cchMax,
      _Out_ int *piIndex, _Out_ UINT *pwFlags) PURE;

  STDMETHOD(Extract)(THIS_ _In_ PCWSTR pszFile, UINT nIconIndex,
                     _Out_opt_ HICON * phiconLarge,
                     _Out_opt_ HICON * phiconSmall, UINT nIconSize) PURE;
};
...
//
// Used to return the icon overlay information including OverlayIndex, Image
// Index or Priority for an IShellFolder object.
//
// IShellIconOverlayManager:GetFileOverlayInfo(LPCWSTR pwszPath, DWORD dwAttrib,
// int * pIndex, DWORD dwflags)
...
// IShellIconOverlayManager:GetReservedOverlayInfo(LPCWSTR pwszPath, DWORD
// dwAttrib, int * pIndex, DWORD dwflags, int iReservedID)
...
// IShellIconOverlayManager::OverlayIndexFromImageIndex(int iImage, int
// *piIndex, BOOL fAdd)
...
DECLARE_INTERFACE_IID_(IShellIconOverlayManager, IUnknown,
  ...
  STDMETHOD(GetFileOverlayInfo)(THIS_ _In_ PCWSTR pwszPath, DWORD dwAttrib,
                                _Out_ int *pIndex, DWORD dwflags) PURE;
  STDMETHOD(GetReservedOverlayInfo)(THIS_ _In_opt_ PCWSTR pwszPath,
                                    DWORD dwAttrib, _Out_ int *pIndex,
                                    DWORD dwflags, int iReservedID) PURE;
  STDMETHOD(RefreshOverlayImages)(THIS_ DWORD dwFlags) PURE;
  STDMETHOD(LoadNonloadedOverlayIdentifiers)(THIS) PURE;
  STDMETHOD(OverlayIndexFromImageIndex)(THIS_ int iImage, _Out_ int *piIndex,
                                        BOOL fAdd) PURE;
};
...
// IShellIconOverlay
//
// Used to return the icon overlay index or its icon index for an IShellFolder
// object, this is always implemented with IShellFolder
//
...
DECLARE_INTERFACE_IID_(IShellIconOverlay, IUnknown,
  ...
  STDMETHOD(GetOverlayIndex)(THIS_ _In_ PCUITEMID_CHILD pidl,
                             _Inout_ int *pIndex) PURE;
  STDMETHOD(GetOverlayIconIndex)(THIS_ _In_ PCUITEMID_CHILD pidl,
                                 _Inout_ int *pIconIndex) PURE;
};
...
// Note: there are totally only 15 slots for system image overlays, some of
// which was reserved by the system, or taken by the overlayidentifiers, so it's
// possible that this function would fail and return -1;
//
// To get the default overlays in the system, such as the share hand, link
// shortcut and slow files, pass NULL as the file name, then the IDO_SHGIOI_*
// flags as the icon index
//-------------------------------------------------------------------------
...
SHSTDAPI_(int)
SHGetIconOverlayIndexA(_In_opt_ LPCSTR pszIconPath, int iIconIndex);
SHSTDAPI_(int)
SHGetIconOverlayIndexW(_In_opt_ LPCWSTR pszIconPath, int iIconIndex);
#ifdef UNICODE
...
typedef enum {
  ...
  SLDF_FORCE_NO_LINKINFO =
      0x00000100, // disable LINKINFO tracking information (used to track
                  // network drives and compute UNC paths if one exists)
  SLDF_HAS_EXP_SZ = 0x00000200, // the link contains expandable env strings
  SLDF_RUN_IN_SEPARATE =
      0x00000400, // Run the 16-bit target exe in a separate VDM/WOW
#if (NTDDI_VERSION < NTDDI_VISTA)
  SLDF_HAS_LOGO3ID = 0x00000800, // not used anymore
#endif
  SLDF_HAS_DARWINID =
      0x00001000, // MSI (Darwin) link that can be installed on demand
  SLDF_RUNAS_USER = 0x00002000, // Run target as a different user
  SLDF_HAS_EXP_ICON_SZ =
      0x00004000, // contains expandable env string for icon path
#if (NTDDI_VERSION >= NTDDI_WINXP)
  SLDF_NO_PIDL_ALIAS = 0x00008000, // disable IDList alias mapping when parsing
                                   // the IDList from the path
  SLDF_FORCE_UNCNAME =
      0x00010000, // make GetPath() prefer the UNC name to the local name
  SLDF_RUN_WITH_SHIMLAYER =
      0x00020000, // activate target of this link with shim layer active
#if (NTDDI_VERSION >= NTDDI_VISTA)
  SLDF_FORCE_NO_LINKTRACK = 0x00040000, // disable ObjectID tracking information
  SLDF_ENABLE_TARGET_METADATA =
      0x00080000, // enable caching of target metadata into link
  SLDF_DISABLE_LINK_PATH_TRACKING =
      0x00100000, // disable EXP_SZ_LINK_SIG tracking
  SLDF_DISABLE_KNOWNFOLDER_RELATIVE_TRACKING =
      0x00200000, // disable KnownFolder tracking information (EXP_KNOWN_FOLDER)
#if (NTDDI_VERSION >= NTDDI_WIN7)
  SLDF_NO_KF_ALIAS = 0x00400000, // disable Known Folder alias mapping when
                                 // loading the IDList during deserialization
  SLDF_ALLOW_LINK_TO_LINK =
      0x00800000, // allows this link to point to another shell link - must only
                  // be used when it is not possible to create cycles
  SLDF_UNALIAS_ON_SAVE = 0x01000000, // unalias the IDList when saving
  SLDF_PREFER_ENVIRONMENT_PATH =
      0x02000000, // the IDList is not persisted, instead it is recalculated
                  // from the path with environmental variables at load time we
                  // don't hit the disk to recalculate the IDList (the result is
                  // a simple IDList).  also Resolve does nothing if SetPath is
                  // called and the path does not have environmental variable in
                  // it, SLDF_PREFER_ENVIRONMENT_PATH is removed
  SLDF_KEEP_LOCAL_IDLIST_FOR_UNC_TARGET =
      0x04000000, // if target is a UNC location on a local machine, keep the
                  // local target in addition to the remote one
#if (NTDDI_VERSION >= NTDDI_WIN8)
  SLDF_PERSIST_VOLUME_ID_RELATIVE =
      0x08000000, // persist target idlist in its volume ID-relative form to
                  // avoid dependency on drive letters
...
#endif
#endif
  SLDF_RESERVED = (int)0x80000000, // Reserved-- so we can use the low word as
                                   // an index value in the future
#endif
...
DECLARE_INTERFACE_IID_(IURLSearchHook, IUnknown,
  ...
  STDMETHOD(Translate)(THIS_ _Out_writes_(cchBufferSize) PWSTR pwszSearchURL,
...
DECLARE_INTERFACE_IID_(IURLSearchHook2, IURLSearchHook,
  ...
  STDMETHOD(TranslateWithSearchContext)(
      THIS_ _Out_writes_(cchBufferSize) PWSTR pwszSearchURL,
      DWORD cchBufferSize, _In_opt_ ISearchContext * pSearchContext) PURE;
};
...
#define FCIDM_MENU_TOOLS_SEP_GOTO                                              \
  (FCIDM_GLOBALFIRST + 0x00c1) // for Win9x compat
...
ILCombine(_In_opt_ PCIDLIST_ABSOLUTE pidl1, _In_opt_ PCUIDLIST_RELATIVE pidl2);
...
ILFindChild(_In_ PIDLIST_ABSOLUTE pidlParent, _In_ PCIDLIST_ABSOLUTE pidlChild);
...
SHSTDAPI_(BOOL)
ILIsEqual(_In_ PCIDLIST_ABSOLUTE pidl1, _In_ PCIDLIST_ABSOLUTE pidl2);
SHSTDAPI_(BOOL)
ILIsParent(_In_ PCIDLIST_ABSOLUTE pidl1, _In_ PCIDLIST_ABSOLUTE pidl2,
           BOOL fImmediate);
SHSTDAPI ILSaveToStream(_In_ IStream *pstm, _In_ PCUIDLIST_RELATIVE pidl);
EXTERN_C DECLSPEC_DEPRECATED HRESULT STDAPICALLTYPE ILLoadFromStream(
    _In_ IStream *pstm,
    _Inout_ PIDLIST_RELATIVE *pidl); // use ILLoadFromStreamEx instead
#if (NTDDI_VERSION >= NTDDI_VISTA)
SHSTDAPI ILLoadFromStreamEx(_In_ IStream *pstm,
                            _Outptr_ PIDLIST_RELATIVE *pidl);
#endif // NTDDI_VISTA
...
SHSTDAPI SHILCreateFromPath(_In_ PCWSTR pszPath,
                            _Outptr_ PIDLIST_ABSOLUTE *ppidl,
                            _Inout_opt_ DWORD *rgfInOut);
...
inline PIDLIST_ABSOLUTE ILCloneFull(_In_ PCUIDLIST_ABSOLUTE pidl) {
  return reinterpret_cast<PIDLIST_ABSOLUTE>(ILClone(pidl));
}
inline PITEMID_CHILD ILCloneChild(_In_ PCUITEMID_CHILD pidl) {
  return ILCloneFirst(pidl);
}

#if (NTDDI_VERSION >= NTDDI_VISTA)
inline HRESULT ILLoadFromStreamEx(_In_ IStream *pstm,
                                  _Outptr_ PIDLIST_ABSOLUTE *ppidl) {
  return ILLoadFromStreamEx(pstm, reinterpret_cast<PIDLIST_RELATIVE *>(ppidl));
}
inline HRESULT ILLoadFromStreamEx(_In_ IStream *pstm,
                                  _Outptr_ PITEMID_CHILD *ppidl) {
  return ILLoadFromStreamEx(pstm, reinterpret_cast<PIDLIST_RELATIVE *>(ppidl));
}
#endif // NTDDI_VISTA

inline PCUIDLIST_RELATIVE ILSkip(_In_ PCUIDLIST_RELATIVE pidl, UINT cb) {
  return reinterpret_cast<PCUIDLIST_RELATIVE>(VOID_OFFSET(pidl, cb));
}
inline PUIDLIST_RELATIVE ILSkip(_In_ PUIDLIST_RELATIVE pidl, UINT cb) {
  return const_cast<PUIDLIST_RELATIVE>(
      ILSkip(const_cast<PCUIDLIST_RELATIVE>(pidl), cb));
}

inline PCUIDLIST_RELATIVE ILNext(_In_ PCUIDLIST_RELATIVE pidl) {
  return ILSkip(pidl, pidl->mkid.cb);
}
inline PUIDLIST_RELATIVE ILNext(_In_ PUIDLIST_RELATIVE pidl) {
  return const_cast<PUIDLIST_RELATIVE>(
      ILNext(const_cast<PCUIDLIST_RELATIVE>(pidl)));
}

inline BOOL ILIsAligned(_In_ PCUIDLIST_RELATIVE pidl) {
  return (((DWORD_PTR)(pidl) & (sizeof(void *) - 1)) == 0);
}

inline BOOL ILIsEmpty(_In_opt_ PCUIDLIST_RELATIVE pidl) {
  return ((pidl == NULL) || (pidl->mkid.cb == 0));
}

// ILIsChild does not guarantee that pidl is non-null or non-empty.
inline BOOL ILIsChild(_In_opt_ PCUIDLIST_RELATIVE pidl) {
  return (ILIsEmpty(pidl) || ILIsEmpty(ILNext(pidl)));
}

#if defined(STRICT_TYPED_ITEMIDS) && defined(__cplusplus)
inline PCUIDLIST_RELATIVE ILFindChild(_In_ PCIDLIST_ABSOLUTE pidlParent,
                                      _In_ PCIDLIST_ABSOLUTE pidlChild) {
  return const_cast<PCUIDLIST_RELATIVE>(
      ILFindChild(const_cast<PIDLIST_ABSOLUTE>(pidlParent), pidlChild));
}
...
ILAppendID(_In_opt_ PIDLIST_RELATIVE pidl, _In_ LPCSHITEMID pmkid,
           BOOL fAppend);

#if (NTDDI_VERSION >= NTDDI_VISTA)
// SHGetPathFromIDListEx returns a win32 file system path for the item in the
// name space.
//  and has a few special cases that include returning UNC printer names too!
enum {
  GPFIDL_DEFAULT =
      0x0000, // normal Win32 file name, servers and drive roots included
  GPFIDL_ALTNAME = 0x0001, // short file name
  GPFIDL_UNCPRINTER =
      0x0002, // include UNC printer names too (non file system item)
};

typedef int GPFIDL_FLAGS;
SHSTDAPI_(BOOL)
SHGetPathFromIDListEx(_In_ PCIDLIST_ABSOLUTE pidl,
                      _Out_writes_(cchPath) PWSTR pszPath, DWORD cchPath,
                      GPFIDL_FLAGS uOpts);
#endif // NTDDI_VISTA
//
// SHGetPathFromIDListW is the old version of SHGetPathFromIDListEx that assumes
// the size of the buffer (MAX_PATH). The pidl should point to a file system
// object.
_Success_(return != 0) SHSTDAPI_(BOOL)
    SHGetPathFromIDListA(_In_ PCIDLIST_ABSOLUTE pidl,
...
// object.
_Success_(return != 0) SHSTDAPI_(BOOL)
    SHGetPathFromIDListW(_In_ PCIDLIST_ABSOLUTE pidl,
...
SHSTDAPI_(int)
SHCreateDirectoryExA(_In_opt_ HWND hwnd, _In_ LPCSTR pszPath,
                     _In_opt_ const SECURITY_ATTRIBUTES *psa);
SHSTDAPI_(int)
SHCreateDirectoryExW(_In_opt_ HWND hwnd, _In_ LPCWSTR pszPath,
                     _In_opt_ const SECURITY_ATTRIBUTES *psa);
#ifdef UNICODE
...
SHSTDAPI SHOpenFolderAndSelectItems(_In_ PCIDLIST_ABSOLUTE pidlFolder,
                                    UINT cidl,
                                    _In_reads_opt_(cidl)
...
SHSTDAPI SHCreateShellItem(_In_opt_ PCIDLIST_ABSOLUTE pidlParent,
...
#define CSIDL_MYDOCUMENTS                                                      \
  CSIDL_PERSONAL             //  Personal was just a silly name for My Documents
...
#define CSIDL_LOCAL_APPDATA                                                    \
  0x001c // <user name>\Local Settings\Applicaiton Data (non roaming)
...
#define CSIDL_COMMON_ADMINTOOLS                                                \
  0x002f // All Users\Start Menu\Programs\Administrative Tools
#define CSIDL_ADMINTOOLS                                                       \
  0x0030 // <user name>\Start Menu\Programs\Administrative Tools
#endif   // _SHFOLDER_H_
...
#define CSIDL_CDBURN_AREA                                                      \
  0x003b // USERPROFILE\Local Settings\Application Data\Microsoft\CD Burning
// unused                               0x003c
#define CSIDL_COMPUTERSNEARME                                                  \
  0x003d // Computers Near Me (computered from Workgroup membership)
...
#define CSIDL_FLAG_DONT_VERIFY                                                 \
  0x4000 // combine with CSIDL_ value to return an unverified folder path
#define CSIDL_FLAG_DONT_UNEXPAND                                               \
  0x2000 // combine with CSIDL_ value to avoid unexpanding environment variables
#if (NTDDI_VERSION >= NTDDI_WINXP)
#define CSIDL_FLAG_NO_ALIAS                                                    \
  0x1000 // combine with CSIDL_ value to insure non-alias versions of the pidl
#define CSIDL_FLAG_PER_USER_INIT                                               \
  0x0800 // combine with CSIDL_ value to indicate per-user init (eg. upgrade)
#endif   // NTDDI_WINXP
#define CSIDL_FLAG_MASK 0xFF00 // mask for all possible flag values

_Check_return_
    STDAPI SHGetSpecialFolderLocation(_Reserved_ HWND hwnd, _In_ int csidl,
                                      _Outptr_ PIDLIST_ABSOLUTE *ppidl);

SHSTDAPI_(PIDLIST_ABSOLUTE)
SHCloneSpecialIDList(_Reserved_ HWND hwnd, _In_ int csidl, _In_ BOOL fCreate);
_Success_(return != 0) SHSTDAPI_(BOOL)
    SHGetSpecialFolderPathA(_Reserved_ HWND hwnd,
                            _Out_writes_(MAX_PATH) LPSTR pszPath,
                            _In_ int csidl, _In_ BOOL fCreate);
_Success_(return != 0) SHSTDAPI_(BOOL)
    SHGetSpecialFolderPathW(_Reserved_ HWND hwnd,
                            _Out_writes_(MAX_PATH) LPWSTR pszPath,
                            _In_ int csidl, _In_ BOOL fCreate);
#ifdef UNICODE
...
SHFOLDERAPI SHGetFolderPathA(_Reserved_ HWND hwnd, _In_ int csidl,
                             _In_opt_ HANDLE hToken, _In_ DWORD dwFlags,
                             _Out_writes_(MAX_PATH) LPSTR pszPath);
SHFOLDERAPI SHGetFolderPathW(_Reserved_ HWND hwnd, _In_ int csidl,
                             _In_opt_ HANDLE hToken, _In_ DWORD dwFlags,
                             _Out_writes_(MAX_PATH) LPWSTR pszPath);
#ifdef UNICODE
...
#endif // !UNICODE
SHSTDAPI SHGetFolderLocation(_Reserved_ HWND hwnd, _In_ int csidl,
                             _In_opt_ HANDLE hToken, _In_ DWORD dwFlags,
                             _Outptr_ PIDLIST_ABSOLUTE *ppidl);
#endif // NTDDI_WIN2K

#if (NTDDI_VERSION >= NTDDI_WINXP)
STDAPI SHSetFolderPathA(_In_ int csidl, _In_opt_ HANDLE hToken,
                        _In_ DWORD dwFlags, _In_ LPCSTR pszPath);
STDAPI SHSetFolderPathW(_In_ int csidl, _In_opt_ HANDLE hToken,
                        _In_ DWORD dwFlags, _In_ LPCWSTR pszPath);
#ifdef UNICODE
#define SHSetFolderPath SHSetFolderPathW
#else
#define SHSetFolderPath SHSetFolderPathA
#endif // !UNICODE
STDAPI SHGetFolderPathAndSubDirA(_Reserved_ HWND hwnd, _In_ int csidl,
                                 _In_opt_ HANDLE hToken, _In_ DWORD dwFlags,
                                 _In_opt_ LPCSTR pszSubDir,
                                 _Out_writes_(MAX_PATH) LPSTR pszPath);
STDAPI SHGetFolderPathAndSubDirW(_Reserved_ HWND hwnd, _In_ int csidl,
                                 _In_opt_ HANDLE hToken, _In_ DWORD dwFlags,
                                 _In_opt_ LPCWSTR pszSubDir,
                                 _Out_writes_(MAX_PATH) LPWSTR pszPath);
#ifdef UNICODE
...
typedef enum {
  ...
  // When called from packaged app, LocalAppData/RoamingAppData folders are
  // redirected to app private locations that match the paths returned from
  // WinRT API:
  ...
  // When running in a Centennial process, some file system locations are
  // redirected to package-specific locations by the file system. Specifying
  // this flag will cause the target of the redirection to be returned for these
  // locations. This is useful in cases where the real location in the file
  // system needs to be known.
  KF_FLAG_RETURN_FILTER_REDIRECTION_TARGET = 0x00040000,

  // New form for KF_FLAG_FORCE_APPCONTAINER_REDIRECTION and
  // KF_FLAG_NO_APPCONTAINER_REDIRECTION The new forms should be used and will
  // exhibit the same behavior as the old flags.

  // When running inside an AppContainer process, or when providing an
  // AppContainer token, some folders are redirected to AppContainer-specific
  // locations within the package's location. Specifying this flag will force
  // this redirection for folders that are not normally redirected for
  // centennial processes. Useful for sharing files between between UWA and
  // Centennial apps that are within the same package.
  KF_FLAG_FORCE_PACKAGE_REDIRECTION = 0x00020000,

  // When running inside a packaged process (Centennial,  AppContainer, etc.),
  // or when providing a packaged process token, some folders are redirected to
  // package-specific locations. Specifying this flag will disable redirection
  // on locations where it applied, and instead return the non-redirected
  // location.
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
  // When running inside an AppContainer, or when poviding an AppContainer
  // token, specifying this flag will prevent redirection to AppContainer
  // folders and instead return the path that would be returned when not running
  // inside an AppContainer
  KF_FLAG_NO_APPCONTAINER_REDIRECTION = 0x00010000,
#endif // NTDDI_WIN7

  // Make sure that the folder already exists or create it and apply security
  // specified in folder definition If folder can not be created then function
  // will return failure and no folder path (IDList) will be returned If folder
  // is located on the network the function may take long time to execute
  KF_FLAG_CREATE = 0x00008000,

  // If this flag is specified then the folder path is returned and no
  // verification is performed Use this flag is you want to get folder's path
  // (IDList) and do not need to verify folder's existence
  //
  // If this flag is NOT specified then Known Folder API will try to verify that
  // the folder exists
  //     If folder does not exist or can not be accessed then function will
  //     return failure and no folder path (IDList) will be returned If folder
  //     is located on the network the function may take long time to execute
  KF_FLAG_DONT_VERIFY = 0x00004000,

  // Set folder path as is and do not try to substitute parts of the path with
  // environments variables. If flag is not specified then Known Folder will try
  // to replace parts of the path with some known environment variables
  // (%USERPROFILE%, %APPDATA% etc.)
  KF_FLAG_DONT_UNEXPAND = 0x00002000,

  // Get file system based IDList if available. If the flag is not specified the
  // Known Folder API will try to return aliased IDList by default. Example for
  // FOLDERID_Documents - Aliased - [desktop]\[user]\[Documents] - exact
  // location is determined by shell namespace layout and might change Non
  // aliased - [desktop]\[computer]\[disk_c]\[users]\[user]\[Documents] -
  // location is determined by folder location in the file system
  KF_FLAG_NO_ALIAS = 0x00001000,

  // Initialize the folder with desktop.ini settings
  // If folder can not be initialized then function will return failure and no
  // folder path will be returned If folder is located on the network the
  // function may take long time to execute
  KF_FLAG_INIT = 0x00000800,

  // Get the default path, will also verify folder existence unless
  // KF_FLAG_DONT_VERIFY is also specified
  KF_FLAG_DEFAULT_PATH = 0x00000400,

  // Get the not-parent-relative default path. Only valid with
...
STDAPI SHSetKnownFolderPath(_In_ REFKNOWNFOLDERID rfid,
                            _In_ DWORD /* KNOWN_FOLDER_FLAG */ dwFlags,
                            _In_opt_ HANDLE hToken, _In_ PCWSTR pszPath);

STDAPI SHGetKnownFolderPath(
    _In_ REFKNOWNFOLDERID rfid, _In_ DWORD /* KNOWN_FOLDER_FLAG */ dwFlags,
    _In_opt_ HANDLE hToken,
...
STDAPI SHGetKnownFolderItem(_In_ REFKNOWNFOLDERID rfid,
                            ...
                            _In_opt_ HANDLE hToken, _In_ REFIID riid,
                            _Outptr_ void **ppv);
#endif // NTDDI_WIN7
...
typedef struct {
  ...
  DWORD cchWebViewTemplate;  // IN - Specifies the size of the buffer pointed to
                             // by pszWebViewTemplate Ignored if dwReadWrite is
                             // FCS_READ
  ...
  DWORD cchInfoTip;  // IN - Specifies the size of the buffer pointed to by
                     // pszInfoTip Ignored if dwReadWrite is FCS_READ
  ...
  DWORD cchIconFile;  // IN - Specifies the size of the buffer pointed to by
                      // pszIconFile Ignored if dwReadWrite is FCS_READ

  int iIconIndex; // OUT - if dwReadWrite is FCS_READ, IN - otherwise

  LPWSTR pszLogo; // OUT - if dwReadWrite is FCS_READ, IN - otherwise
  DWORD cchLogo;  // IN - Specifies the size of the buffer pointed to by
                  // pszIconFile Ignored if dwReadWrite is FCS_READ
...
SHSTDAPI SHGetSetFolderCustomSettings(_Inout_ LPSHFOLDERCUSTOMSETTINGS pfcs,
                                      _In_ PCWSTR pszPath, DWORD dwReadWrite);
...
#define BIF_RETURNONLYFSDIRS                                                   \
  0x00000001 // For finding a folder to start document searching
#define BIF_DONTGOBELOWDOMAIN 0x00000002 // For starting the Find Computer
#define BIF_STATUSTEXT                                                         \
  0x00000004 // Top of the dialog has 2 lines of text for BROWSEINFO.lpszTitle
             // and one line if
// this flag is set.  Passing the message BFFM_SETSTATUSTEXTA to the hwnd can
// set the rest of the text.  This is not used with BIF_USENEWUI and
// BROWSEINFO.lpszTitle gets all three lines of text.
...
#define BIF_NEWDIALOGSTYLE                                                     \
  0x00000040 // Use the new dialog layout with the ability to resize
             // Caller needs to call OleInitialize() before using this API

#define BIF_USENEWUI (BIF_NEWDIALOGSTYLE | BIF_EDITBOX)

#define BIF_BROWSEINCLUDEURLS                                                  \
  0x00000080 // Allow URLs to be displayed or entered. (Requires BIF_USENEWUI)
#define BIF_UAHINT                                                             \
  0x00000100 // Add a UA hint to the dialog, in place of the edit box. May not
             // be combined with BIF_EDITBOX
#define BIF_NONEWFOLDERBUTTON                                                  \
  0x00000200 // Do not add the "New Folder" button to the dialog.  Only
             // applicable with BIF_NEWDIALOGSTYLE.
...
#define BIF_SHAREABLE                                                          \
  0x00008000 // sharable resources displayed (remote shares, requires
             // BIF_USENEWUI)
#define BIF_BROWSEFILEJUNCTIONS                                                \
  0x00010000 // allow folder junctions like zip files and libraries to be
             // browsed
...
_Check_return_ SHSTDAPI SHLoadInProc(_In_ REFCLSID rclsid);
...
// extra data, besides the name, necessary for the parse. the object in the bind
// context implements IPropertyStore and provides a fixed set of properties
#define STR_PARSE_WITH_PROPERTIES L"ParseWithProperties"

// Bindctx key, passed to IShellFolder::ParseDisplayName(). used to pass the
// original item that is being re-parsed. that item is stored as an IShellItem
// that supports IParentAndItem, and should be the un-aliased form of the item.
...
//
_Check_return_ SHSTDAPI SHGetDesktopFolder(_Outptr_ IShellFolder **ppshf);
...
DECLARE_INTERFACE_IID_(IShellDetails, IUnknown,
  ...
  STDMETHOD(GetDetailsOf)(THIS_ _In_opt_ PCUITEMID_CHILD pidl, UINT iColumn,
                          _Out_ SHELLDETAILS * pDetails) PURE;
...
typedef enum _tagAUTOCOMPLETELISTOPTIONS {
  ...
  ACLO_FILESYSDIRS =
      32, // enumerate only the file system dirs, UNC shares, and UNC servers.
#endif
#if (_WIN32_IE >= _WIN32_IE_IE70)
...
IID_IProgressDialog, (void **)&ppd); ppd->SetTitle(L"My Slow Operation"); // Set
the title of the dialog. ppd->SetAnimation(hInstApp, IDA_OPERATION_ANIMATION);
// Set the animation to play. ppd->StartProgressDialog(hwndParent, punk,
PROGDLG_AUTOTIME, NULL); // Display and enable automatic estimated time
remaining. ppd->SetCancelMsg(L"Please wait while the current operation is
cleaned up", NULL);   // Will only be displayed if Cancel button is pressed.

        dwComplete = 0;
        dwTotal = CalcTotalUnitsToDo();

        // Reset because CalcTotalUnitsToDo() took a long time and the estimated
time
...
#define PROGDLG_MODAL                                                          \
  0x00000001 // the dialog is modal to its hwndParent (default is modeless)
#define PROGDLG_AUTOTIME                                                       \
  0x00000002 // automatically updates the "Line3" text with the "time remaining"
             // (you cant call SetLine3 if you passs this!)
#define PROGDLG_NOTIME                                                         \
  0x00000004 // we dont show the "time remaining" if this is set. We need this
             // if dwTotal < dwCompleted for sparse files
#define PROGDLG_NOMINIMIZE                                                     \
  0x00000008 // Do not have a minimize button in the caption bar.
#define PROGDLG_NOPROGRESSBAR 0x00000010 // Don't display the progress bar
#if (_WIN32_IE >= _WIN32_IE_IE70)
#define PROGDLG_MARQUEEPROGRESS                                                \
  0x00000020 // Use marquee progress (comctl32 v6 required)
#define PROGDLG_NOCANCEL                                                       \
  0x00000040 // No cancel button (operation cannot be canceled) (use sparingly)
#endif

// Time Actions (dwTimerAction)
#define PDTIMER_RESET                                                          \
  0x00000001 // Reset the timer so the progress will be calculated from now
             // until the first ::SetProgress() is called so those this time
             // will correspond to the values passed to ::SetProgress().  Only
             // do this before ::SetProgress() is called.
#if (_WIN32_IE >= _WIN32_IE_IE70)
...
DECLARE_INTERFACE_IID_(IProgressDialog, IUnknown,
  ...
  STDMETHOD(StartProgressDialog)(
      THIS_ _In_opt_ HWND hwndParent, _In_opt_ IUnknown * punkEnableModless,
      DWORD dwFlags, _Reserved_ LPCVOID pvResevered) PURE;
  STDMETHOD(StopProgressDialog)(THIS) PURE;
  STDMETHOD(SetTitle)(THIS_ _In_ PCWSTR pwzTitle) PURE;
  STDMETHOD(SetAnimation)(THIS_ _In_opt_ HINSTANCE hInstAnimation,
                          UINT idAnimation) PURE;
  ...
  STDMETHOD(SetProgress64)(THIS_ ULONGLONG ullCompleted, ULONGLONG ullTotal)
      PURE;
  STDMETHOD(SetLine)(THIS_ DWORD dwLineNum, _In_ PCWSTR pwzString,
                     BOOL fCompactPath, _Reserved_ LPCVOID pvResevered) PURE;
  STDMETHOD(SetCancelMsg)(THIS_ _In_ PCWSTR pwzCancelMsg,
                          _Reserved_ LPCVOID pvResevered) PURE;
  STDMETHOD(Timer)(THIS_ DWORD dwTimerAction, _Reserved_ LPCVOID pvResevered)
      PURE;
};
...
DECLARE_INTERFACE_IID_(IDockingWindowSite, IOleWindow,
  ...
  STDMETHOD(GetBorderDW)(THIS_ _In_ IUnknown * punkObj, _Out_ RECT * prcBorder)
      PURE;
  STDMETHOD(RequestBorderSpaceDW)(THIS_ _In_ IUnknown * punkObj,
                                  _In_ LPCBORDERWIDTHS pbw) PURE;
  STDMETHOD(SetBorderSpaceDW)(THIS_ _In_ IUnknown * punkObj,
                              _In_ LPCBORDERWIDTHS pbw) PURE;
};
...
typedef struct _tagCOMPPOS {
  ...
  int iPreferredLeftPercent; // Left of top-left corner as percent of screen
                             // width
  int iPreferredTopPercent;  // Top of top-left corner as percent of screen
                             // height
} COMPPOS;
...
typedef struct _tagCOMPSTATEINFO {
  ...
  DWORD dwWidth;     // Width in pixels.
  DWORD dwHeight;    // Height in pixels.
  DWORD dwItemState; // State of the component (full-screen mode or split-screen
                     // or normal state.
} COMPSTATEINFO;
...
#define COMPONENT_TOP                                                          \
  (0x3fffffff) // izOrder value meaning component is at the top
...
//
// The following is the new NT5 component structure. Note that the initial
// portion of this component exactly matches the IE4COMPONENT structure. All new
// fields are added at the bottom and the dwSize field is used to distinguish
// between IE4COMPONENT and the new COMPONENT structures.
//
typedef struct _tagCOMPONENT {
  ...
  // New fields are added below. Everything above here must exactly match the
...
#define IS_VALIDSIZESTATEBITS                                                  \
  (IS_NORMAL | IS_SPLIT |                                                      \
   IS_FULLSCREEN) // The set of IS_* state bits which define the "size" of the
                  // component - these bits are mutually exclusive.
#define IS_VALIDSTATEBITS                                                      \
  (IS_NORMAL | IS_SPLIT | IS_FULLSCREEN | 0x80000000 |                         \
   0x40000000) // All of the currently defined IS_* bits.
...
DECLARE_INTERFACE_IID_(IActiveDesktop, IUnknown,
  ...
  STDMETHOD(GetWallpaper)(THIS_ _Out_writes_(cchWallpaper) PWSTR pwszWallpaper,
                          UINT cchWallpaper, DWORD dwFlags) PURE;
  STDMETHOD(SetWallpaper)(THIS_ _In_ PCWSTR pwszWallpaper, DWORD dwReserved)
      PURE;
  STDMETHOD(GetWallpaperOptions)(THIS_ _Inout_ LPWALLPAPEROPT pwpo,
                                 DWORD dwReserved) PURE;
  STDMETHOD(SetWallpaperOptions)(THIS_ _In_ LPCWALLPAPEROPT pwpo,
                                 DWORD dwReserved) PURE;
  STDMETHOD(GetPattern)(THIS_ _Out_writes_(cchPattern) PWSTR pwszPattern,
                        UINT cchPattern, DWORD dwReserved) PURE;
  STDMETHOD(SetPattern)(THIS_ _In_ PCWSTR pwszPattern, DWORD dwReserved) PURE;
  STDMETHOD(GetDesktopItemOptions)(THIS_ _Inout_ LPCOMPONENTSOPT pco,
                                   DWORD dwReserved) PURE;
  STDMETHOD(SetDesktopItemOptions)(THIS_ _In_ LPCCOMPONENTSOPT pco,
                                   DWORD dwReserved) PURE;
  STDMETHOD(AddDesktopItem)(THIS_ _In_ LPCCOMPONENT pcomp, DWORD dwReserved)
      PURE;
  STDMETHOD(AddDesktopItemWithUI)(
      THIS_ _In_opt_ HWND hwnd, _In_ LPCOMPONENT pcomp, DWORD dwReserved) PURE;
  STDMETHOD(ModifyDesktopItem)(THIS_ _Inout_ LPCCOMPONENT pcomp, DWORD dwFlags)
      PURE;
  STDMETHOD(RemoveDesktopItem)(THIS_ _In_ LPCCOMPONENT pcomp, DWORD dwReserved)
      PURE;
  STDMETHOD(GetDesktopItemCount)(THIS_ _Out_ int *pcItems, DWORD dwReserved)
      PURE;
  STDMETHOD(GetDesktopItem)(THIS_ int nComponent, _Inout_ LPCOMPONENT pcomp,
                            DWORD dwReserved) PURE;
  STDMETHOD(GetDesktopItemByID)(THIS_ ULONG_PTR dwID, _Inout_ LPCOMPONENT pcomp,
                                DWORD dwReserved) PURE;
  STDMETHOD(GenerateDesktopItemHtml)(THIS_ _In_ PCWSTR pwszFileName,
                                     _In_ LPCOMPONENT pcomp, DWORD dwReserved)
      PURE;
  STDMETHOD(AddUrl)(THIS_ _In_opt_ HWND hwnd, _In_ PCWSTR pszSource,
                    _In_ LPCOMPONENT pcomp, DWORD dwFlags) PURE;
  STDMETHOD(GetDesktopItemBySource)(THIS_ _In_ PCWSTR pwszSource,
                                    _Inout_ LPCOMPONENT pcomp, DWORD dwReserved)
      PURE;
};
...
#define CFSTR_SHELLIDLISTOFFSET                                                \
  TEXT("Shell Object Offsets")                  // CF_OBJECTPOSITIONS
#define CFSTR_NETRESOURCES TEXT("Net Resource") // CF_NETRESOURCE
#define CFSTR_FILEDESCRIPTORA                                                  \
  TEXT("FileGroupDescriptor") // CF_FILEGROUPDESCRIPTORA
#define CFSTR_FILEDESCRIPTORW                                                  \
  TEXT("FileGroupDescriptorW")                  // CF_FILEGROUPDESCRIPTORW
...
#define CFSTR_TARGETCLSID                                                      \
  TEXT("TargetCLSID") // HGLOBAL with a CLSID of the drop target
#define CFSTR_LOGICALPERFORMEDDROPEFFECT TEXT("Logical Performed DropEffect")
#define CFSTR_AUTOPLAY_SHELLIDLISTS                                            \
  TEXT("Autoplay Enumerated IDList Array") // (HGLOBAL with LPIDA)
#define CFSTR_UNTRUSTEDDRAGDROP TEXT("UntrustedDragDrop") //  DWORD
#define CFSTR_FILE_ATTRIBUTES_ARRAY                                            \
  TEXT("File Attributes Array") // (FILE_ATTRIBUTES_ARRAY format on HGLOBAL)
#define CFSTR_INVOKECOMMAND_DROPPARAM                                          \
  TEXT("InvokeCommand DropParam") // (HGLOBAL with LPWSTR)
#define CFSTR_SHELLDROPHANDLER                                                 \
  TEXT("DropHandlerCLSID") // (HGLOBAL with CLSID of drop handler)
#define CFSTR_DROPDESCRIPTION                                                  \
  TEXT("DropDescription") // (HGLOBAL with DROPDESCRIPTION)
#define CFSTR_ZONEIDENTIFIER                                                   \
  TEXT("ZoneIdentifier") //  DWORD, to be used with CFSTR_FILECONTENTS data
                         //  transfers
...
#define DVASPECT_SHORTNAME                                                     \
  2 // use for CF_HDROP to get short name version of file paths
#define DVASPECT_COPY                                                          \
  3 // use to indicate format is a "Copy" of the data (FILECONTENTS,
    // FILEDESCRIPTOR, etc)
#define DVASPECT_LINK                                                          \
  4 // use to indicate format is a "Shortcut" to the data (FILECONTENTS,
    // FILEDESCRIPTOR, etc)
...
//
// FILEDESCRIPTOR.dwFlags field indicate which fields are valid in the
...
} FILE_ATTRIBUTES_ARRAY;         // clipboard format definition for
...
typedef enum {
  ...
  DROPIMAGE_WARNING =
      7, // yellow exclamation, something is amiss with the operation
...
typedef struct {
  ...
  // some UI coloring is applied to the text in szInsert, if used by specifying
  // %1 in szMessage.
...
#define SHCNE_INTERRUPT                                                        \
  0x80000000L // The presence of this flag indicates
...
//
SHSTDAPI_(void)
SHChangeNotify(LONG wEventId, UINT uFlags, _In_opt_ LPCVOID dwItem1,
...
DECLARE_INTERFACE_IID_(IShellChangeNotify, IUnknown,
  ...
  STDMETHOD(OnChange)(THIS_ LONG lEvent, _In_opt_ PCIDLIST_ABSOLUTE pidl1,
                      _In_opt_ PCIDLIST_ABSOLUTE pidl2) PURE;
};
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
#define QITIPF_USESLOWTIP                                                      \
  0x00000008 // Flag says it's OK to take a long time generating tip
#endif
#if (NTDDI_VERSION >= NTDDI_VISTA)
...
typedef enum {
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
  SHARD_APPIDINFO = 0x00000004L, // indicates the data type is a pointer to a
  ...
  SHARD_LINK = 0x00000006L, // indicates the data type is a pointer to an
                            // IShellLink instance
  SHARD_APPIDINFOLINK = 0x00000007L, // indicates the data type is a pointer to
                                     // a SHARDAPPIDINFOLINK structure
  SHARD_SHELLITEM = 0x00000008L, // indicates the data type is a pointer to an
                                 // IShellItem instance
#endif
} SHARD;
...
typedef struct SHARDAPPIDINFO {
  IShellItem *psi; // The namespace location of the the item that should be
                   // added to the recent docs folder.
  PCWSTR pszAppID; // The id of the application that should be associated with
                   // this recent doc.
} SHARDAPPIDINFO;

typedef struct SHARDAPPIDINFOIDLIST {
  PCIDLIST_ABSOLUTE pidl; // The idlist for the shell item that should be added
                          // to the recent docs folder.
  PCWSTR pszAppID; // The id of the application that should be associated with
                   // this recent doc.
} SHARDAPPIDINFOIDLIST;

typedef struct SHARDAPPIDINFOLINK {
  IShellLink *psl; // An IShellLink instance that when launched opens a recently
                   // used item in the specified application. This link is not
                   // added to the recent docs folder, but will be added to the
...
// SHARD_XXX values.  PV can be a PCIDLIST_ABSOLUTE, PCWSTR, PCSTR,
...
SHSTDAPI_(void)
SHUpdateImageA(_In_ LPCSTR pszHashItem, int iIndex, UINT uFlags,
               int iImageIndex);
SHSTDAPI_(void)
SHUpdateImageW(_In_ LPCWSTR pszHashItem, int iIndex, UINT uFlags,
               int iImageIndex);
#ifdef UNICODE
...
SHSTDAPI_(ULONG)
SHChangeNotifyRegister(_In_ HWND hwnd, int fSources, LONG fEvents, UINT wMsg,
                       int cEntries, _In_ const SHChangeNotifyEntry *pshcne);
...
SHSTDAPI_(HANDLE)
SHChangeNotification_Lock(_In_ HANDLE hChange, DWORD dwProcId,
                          _Outptr_opt_result_buffer_(2)
                              PIDLIST_ABSOLUTE **pppidl,
                          _Out_opt_ LONG *plEvent);
SHSTDAPI_(BOOL) SHChangeNotification_Unlock(_In_ HANDLE hLock);
// The pidls that are given to the view via the ChangeNotifyEvents are simple
// Pidls, SHGetRealIDL() will convert them to true PIDLs.
SHSTDAPI SHGetRealIDL(_In_ IShellFolder *psf, _In_ PCUITEMID_CHILD pidlSimple,
                      _Outptr_ PITEMID_CHILD *ppidlReal);

_Check_return_ SHSTDAPI SHGetInstanceExplorer(_Outptr_ IUnknown **ppunk);
...
SHSTDAPI SHGetDataFromIDListA(_In_ IShellFolder *psf, _In_ PCUITEMID_CHILD pidl,
                              int nFormat, _Out_writes_bytes_(cb) void *pv,
                              int cb);
SHSTDAPI SHGetDataFromIDListW(_In_ IShellFolder *psf, _In_ PCUITEMID_CHILD pidl,
                              int nFormat, _Out_writes_bytes_(cb) void *pv,
                              int cb);
#ifdef UNICODE
...
SHSTDAPI_(int)
RestartDialog(_In_opt_ HWND hwnd, _In_opt_ PCWSTR pszPrompt, DWORD dwReturn);
#if (NTDDI_VERSION >= NTDDI_WINXP)
SHSTDAPI_(int)
RestartDialogEx(_In_opt_ HWND hwnd, _In_opt_ PCWSTR pszPrompt, DWORD dwReturn,
                DWORD dwReasonCode);
#endif

SHSTDAPI SHCoCreateInstance(_In_opt_ PCWSTR pszCLSID,
                            _In_opt_ const CLSID *pclsid,
                            _In_opt_ IUnknown *pUnkOuter, _In_ REFIID riid,
                            _Outptr_ void **ppv);

#if (NTDDI_VERSION >= NTDDI_VISTA)
SHSTDAPI SHCreateDataObject(_In_opt_ PCIDLIST_ABSOLUTE pidlFolder,
                            _In_ UINT cidl,
                            _In_reads_opt_(cidl) PCUITEMID_CHILD_ARRAY apidl,
                            _In_opt_ IDataObject *pdtInner, _In_ REFIID riid,
                            _Outptr_ void **ppv);
#endif
SHSTDAPI CIDLData_CreateFromIDArray(_In_ PCIDLIST_ABSOLUTE pidlFolder,
                                    _In_ UINT cidl,
                                    _In_reads_opt_(cidl)
                                        PCUIDLIST_RELATIVE_ARRAY apidl,
                                    _Outptr_ IDataObject **ppdtobj);
SHSTDAPI SHCreateStdEnumFmtEtc(_In_ UINT cfmt,
                               _In_reads_(cfmt) const FORMATETC afmt[],
                               _Outptr_ IEnumFORMATETC **ppenumFormatEtc);
SHSTDAPI SHDoDragDrop(_In_opt_ HWND hwnd, _In_ IDataObject *pdata,
                      _In_opt_ IDropSource *pdsrc, _In_ DWORD dwEffect,
...
SHSTDAPI_(BOOL)
DAD_DragEnterEx2(_In_ HWND hwndTarget, const POINT ptStart,
...
SHSTDAPI_(BOOL)
ReadCabinetState(_Out_writes_bytes_(cLength) CABINETSTATE *pcs, int cLength);
SHSTDAPI_(BOOL) WriteCabinetState(_In_ CABINETSTATE *pcs);
SHSTDAPI_(BOOL)
PathMakeUniqueName(_Out_writes_(cchMax) PWSTR pszUniqueName, UINT cchMax,
                   _In_ PCWSTR pszTemplate, _In_opt_ PCWSTR pszLongPlate,
...
SHSTDAPI_(int)
PathResolve(_Inout_updates_(MAX_PATH) PWSTR pszPath, _In_opt_ PZPCWSTR dirs,
            UINT fFlags);
SHSTDAPI_(BOOL)
GetFileNameFromBrowse(_In_opt_ HWND hwnd,
                      _Inout_updates_(cchFilePath) PWSTR pszFilePath,
                      UINT cchFilePath, _In_opt_ PCWSTR pszWorkingDir,
                      _In_ PCWSTR pszDefExt, _In_opt_ PCWSTR pszFilters,
...
SHSTDAPI_(UINT)
Shell_MergeMenus(_In_ HMENU hmDst, _In_ HMENU hmSrc, UINT uInsert,
 ...
 *   PARAMETERS
 *
 *     hwnd              The window handle of the window which will own the
 * dialog shopObjectType    A SHOP_ value as defined below pszObjectName Name of
 * the object, see SHOP_ values below pszPropertyPage   The name of the property
 * sheet page to open to or NULL.
 *
 *   RETURN
 *
 ...
 */
SHSTDAPI_(BOOL)
SHObjectProperties(_In_opt_ HWND hwnd, _In_ DWORD shopObjectType,
                   _In_ PCWSTR pszObjectName, _In_opt_ PCWSTR pszPropertyPage);

#define SHOP_PRINTERNAME                                                       \
  0x00000001 // pszObjectName points to a printer friendly name
#define SHOP_FILEPATH                                                          \
  0x00000002 // pszObjectName points to a fully qualified path+file name
 ...
 */
SHSTDAPI_(DWORD)
SHFormatDrive(_In_ HWND hwnd, UINT drive, UINT fmtID, UINT options);
...
WINSHELLAPI UINT WINAPI SHAddFromPropSheetExtArray(
    _In_ HPSXA hpsxa, _In_ LPFNADDPROPSHEETPAGE lpfnAddPage, LPARAM lParam);
WINSHELLAPI UINT WINAPI SHReplaceFromPropSheetExtArray(
    _In_ HPSXA hpsxa, UINT uPageID, _In_ LPFNADDPROPSHEETPAGE lpfnReplaceWith,
    LPARAM lParam);
...
typedef enum RESTRICTIONS {
  ...
  REST_NOCLOSE_DRAGDROPBAND =
      0x40000016,                 // Disable Close and Drag & Drop on ALL Bands
  ...
  REST_NOCHANGESTARMENU =
      0x40000020, // No Context menu or Drag and Drop on Start menu
  ...
  REST_NOCONTROLPANEL =
      0x40000027, // Remove the Control Panel only from SM|Settings
  ...
  REST_NOCHANGEMAPPEDDRIVELABEL =
      0x4000002E, // Don't enable the UI which allows users to rename mapped
                  // drive labels
  REST_NOCHANGEMAPPEDDRIVECOMMENT =
      0x4000002F, // Don't enable the UI which allows users to change mapped
                  // drive comments
  REST_MaxRecentDocs = 0x40000030,
  REST_NONETWORKCONNECTIONS =
      0x40000031, // No Start Menu | Settings |Network Connections
  ...
  REST_NOHARDWARETAB =
      0x40000039, // No Hardware tab on Drives or in control panel
  REST_NORUNASINSTALLPROMPT =
      0x4000003A, // Don't bring up "Run As" prompt for install programs
  REST_PROMPTRUNASINSTALLNETPATH =
      0x4000003B, // Force the  "Run As" prompt for install programs on
                  // unc/network shares
  ...
  REST_NOWELCOMESCREEN =
      0x4000003F, // don't allow the welcome screen to be displayed.
  ...
  REST_NOENCRYPTONMOVE = 0x40000045, // Don't automatically try to encrypt files
                                     // that are moved to encryped directories
  REST_NOLOCALMACHINERUN =
      0x40000046, // ignore HKLM\sw\ms\win\cv\Run and all of it's sub keys
  REST_NOCURRENTUSERRUN =
      0x40000047, // ignore HKCU\sw\ms\win\cv\Run and all of it's sub keys
  REST_NOLOCALMACHINERUNONCE =
      0x40000048, // ignore HKLM\sw\ms\win\cv\RunOnce and all of it's sub keys
  REST_NOCURRENTUSERRUNONCE =
      0x40000049, // ignore HKCU\sw\ms\win\cv\RunOnce and all of it's sub keys
  REST_FORCEACTIVEDESKTOPON =
      0x4000004A, // Force ActiveDesktop to be turned ON all the time.
                  //  Do NOT use me                     0x4000004B,
  REST_NOVIEWONDRIVE = 0x4000004C, // disallows CreateViewObject() on specified
                                   // drives (CFSFolder only)
  ...
  REST_NOSHAREDDOCUMENTS =
      0x4000004E, // don't auto share the Shared Documents/create link
#endif            // NTDDI_WINXP
  REST_NOSMMYDOCS =
      0x4000004F, // Don't show the My Documents item on the Start Menu.
#if (NTDDI_VERSION >= NTDDI_WINXP)
  REST_NOSMMYPICS =
      0x40000050, // Don't show the My Pictures item on the Start Menu
  REST_ALLOWBITBUCKDRIVES = 0x40000051, // Bit mask indicating which which
                                        // drives have bit bucket support
  REST_NONLEGACYSHELLMODE = 0x40000052, // new consumer shell modes
  REST_NOCONTROLPANELBARRICADE =
      0x40000053,                     // The webview barricade in Control Panel
  ...
  REST_MYCOMPNOPROP =
      0x40000058, // disables Properties on My Computer's context menu
  REST_MYDOCSNOPROP =
      0x40000059, // disables Properties on My Documents' context menu
#endif            // NTDDI_WIN2KSP3
#if (NTDDI_VERSION >= NTDDI_WINXP)
  REST_NOSTARTPANEL =
      0x4000005A, // Windows start panel (New start menu) for Whistler.
  REST_NODISPLAYAPPEARANCEPAGE =
      0x4000005B, // disable Themes and Appearance tabs in the Display Control
                  // Panel.
  REST_NOTHEMESTAB =
      0x4000005C, // disable the Themes tab in the Display Control Panel.
  REST_NOVISUALSTYLECHOICE =
      0x4000005D, // disable the visual style drop down in the Appearance tab of
                  // the Display Control Panel.
  REST_NOSIZECHOICE =
      0x4000005E, // disable the size drop down in the Appearance tab of the
                  // Display Control Panel.
  REST_NOCOLORCHOICE =
      0x4000005F, // disable the color drop down in the Appearance tab of the
                  // Display Control Panel.
  REST_SETVISUALSTYLE =
      0x40000060, // Load the specified file as the visual style.
#endif            // NTDDI_WINXP
#if (NTDDI_VERSION >= NTDDI_WIN2KSP3)
  REST_STARTRUNNOHOMEPATH =
      0x40000061, // dont use the %HOMEPATH% env var for the Start-Run dialog
#endif            // NTDDI_WIN2KSP3
#if (NTDDI_VERSION >= NTDDI_WINXP)
  REST_NOUSERNAMEINSTARTPANEL =
      0x40000062, // don't show the username is the startpanel.
  REST_NOMYCOMPUTERICON =
      0x40000063, // don't show my computer anywhere, hide its contents
  REST_NOSMNETWORKPLACES =
      0x40000064,                   // don't show network places in startpanel.
  ...
  REST_NOSMMOREPROGRAMS =
      0x40000068, // don't show "More Programs" button in StartPanel.
  REST_NOSMMFUPROGRAMS =
      0x40000069, // don't show the MFU programs list in StartPanel.
  REST_NOTRAYITEMSDISPLAY =
      0x4000006A, // disables the display of the system tray
  REST_NOTOOLBARSONTASKBAR =
      0x4000006B, // disables toolbar display on the taskbar
#endif            // NTDDI_WINXP
#if (NTDDI_VERSION >= NTDDI_WIN2KSP3)
  REST_NOSMCONFIGUREPROGRAMS =
      0x4000006F, // No Configure Programs on Settings Menu
#endif            // NTDDI_WIN2KSP3
#if (NTDDI_VERSION >= NTDDI_WINXP)
  ...
  REST_NOENTIRENETWORK = 0x40000072, // removes the "Entire Network" link (i.e.
  ...
  REST_BITBUCKCONFIRMDELETE = 0x40000075, // always show the delete confirmation
                                          // dialog when deleting files
  REST_BITBUCKNOPROP =
      0x40000076, // disables Properties on Recycle Bin's context menu
  REST_NODISPBACKGROUND =
      0x40000077, // disables the Desktop tab in the Display CPL
  REST_NODISPSCREENSAVEPG =
      0x40000078, // disables the Screen Saver tab in the Display CPL
  REST_NODISPSETTINGSPG =
      0x40000079, // disables the Settings tab in the Display CPL
  REST_NODISPSCREENSAVEPREVIEW =
      0x4000007A, // disables the screen saver on the Screen Saver tab in the
                  // Display CPL
  ...
  REST_NOSTRCMPLOGICAL = 0x4000007E, // dont use StrCmpLogical() instead use
  ...
  REST_NOWEBSERVICES =
      0x40000081, // disables the web specified services for both OPW and WPW
#endif            // NTDDI_WINXPSP1
#if (NTDDI_VERSION >= NTDDI_WIN2KSP3)
  REST_ALLOWUNHASHEDWEBVIEW =
      0x40000082, // allow the user to be promted to accept web view templates
                  // that don't already have an md5 hash in the registry
#endif            // NTDDI_WIN2KSP3
  REST_ALLOWLEGACYWEBVIEW =
      0x40000083, // allow legacy webview template to be shown.
#if (NTDDI_VERSION >= NTDDI_WIN2KSP3)
  REST_REVERTWEBVIEWSECURITY =
      0x40000084, // disable added webview security measures (revert to w2k
                  // functionality).
#endif            // NTDDI_WIN2KSP3
#if (NTDDI_VERSION >= NTDDI_WIN2KSP4)
  REST_INHERITCONSOLEHANDLES =
      0x40000086, // ShellExec() will check for the current process and target
                  // process being console processes to inherit handles
#endif            // NTDDI_WIN2KSP4
#if (NTDDI_VERSION >= NTDDI_WINXPSP2 && NTDDI_VERSION < NTDDI_VISTA)
  REST_SORTMAXITEMCOUNT =
      0x40000087, // Do not sort views with more items than this key. Useful for
                  // viewing big amount of files in one folder.
#endif
#if (NTDDI_VERSION >= NTDDI_WINXPSP2)
  REST_NOREMOTERECURSIVEEVENTS =
      0x40000089, // Dont register network change events recursively to avoid
                  // network traffic
#endif            // NTDDI_WINXPSP2
#if (NTDDI_VERSION >= NTDDI_WINXPSP2)
  REST_NOREMOTECHANGENOTIFY =
      0x40000091, // Do not notify for remote changy notifies
#if (NTDDI_VERSION < NTDDI_VISTA)
  REST_NOSIMPLENETIDLIST = 0x40000092, // No simple network IDLists
#endif
  REST_NOENUMENTIRENETWORK =
      0x40000093, // Don't enumerate entire network if we happen to get to it
                  // (in conjunction with REST_NOENTIRENETWORK)
#if (NTDDI_VERSION < NTDDI_VISTA)
  REST_NODETAILSTHUMBNAILONNETWORK =
      0x40000094, // Disable Thumbnail for Network files in DUI Details pane
#endif
  REST_NOINTERNETOPENWITH =
      0x40000095, // dont allow looking on the internet for file associations
#endif            // NTDDI_WINXPSP2
#if (NTDDI_VERSION >= NTDDI_WINXPSP2)
  REST_DONTRETRYBADNETNAME = 0x4000009B, // In Network Places: if provider
                                         // returns ERROR_BAD_NET_NAME, give up
  REST_ALLOWFILECLSIDJUNCTIONS =
      0x4000009C, // re-enable legacy support for file.{guid} junctions in
                  // FileSystem Folder
  REST_NOUPNPINSTALL =
      0x4000009D, // disable "install UPnP" task in My Net Places
#endif            // NTDDI_WINXPSP2
  REST_ARP_DONTGROUPPATCHES =
      0x400000AC, // List individual patches in Add/Remove Programs
  ...
  REST_NOFILEASSOCIATE =
      0x41000003, // Do not allow user to change file association
#if (NTDDI_VERSION >= NTDDI_WINXPSP2)
  REST_ALLOWCOMMENTTOGGLE =
      0x41000004, // Allow the user to toggle the positions of the Comment and
                  // the Computer Name
#if (NTDDI_VERSION < NTDDI_VISTA)
  REST_USEDESKTOPINICACHE =
      0x41000005, // Cache desktop.ini entries from network folders
#endif            // NTDDI_VISTA
#endif            // NTDDI_WINXPSP2
} RESTRICTIONS;
SHSTDAPI_(IStream *)
OpenRegStream(_In_ HKEY hkey, _In_opt_ PCWSTR pszSubkey,
              _In_opt_ PCWSTR pszValue, DWORD grfMode);
SHSTDAPI_(BOOL)
SHFindFiles(_In_opt_ PCIDLIST_ABSOLUTE pidlFolder,
            _In_opt_ PCIDLIST_ABSOLUTE pidlSaveFile);
SHSTDAPI_(void) PathGetShortPath(_Inout_updates_(MAX_PATH) PWSTR pszLongPath);
_Success_(return != 0) SHSTDAPI_(BOOL)
    PathYetAnotherMakeUniqueName(_Out_writes_(MAX_PATH) PWSTR pszUniqueName,
                                 _In_ PCWSTR pszPath, _In_opt_ PCWSTR pszShort,
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
SHSTDAPI AssocGetDetailsOfPropKey(_In_ IShellFolder *psf,
                                  _In_ PCUITEMID_CHILD pidl,
                                  _In_ const PROPERTYKEY *pkey,
                                  _Out_ VARIANT *pv,
                                  _Out_opt_ BOOL *pfFoundPropKey);
#endif

// both ANSI and UNICODE
SHSTDAPI SHStartNetConnectionDialogA(_In_opt_ HWND hwnd,
                                     _In_opt_ LPCSTR pszRemoteName,
                                     DWORD dwType);
// both ANSI and UNICODE
SHSTDAPI SHStartNetConnectionDialogW(_In_opt_ HWND hwnd,
                                     _In_opt_ LPCWSTR pszRemoteName,
                                     DWORD dwType);
#ifdef UNICODE
...
enum tagOPEN_AS_INFO_FLAGS {
  OAIF_ALLOW_REGISTRATION =
      0x00000001, // enable the "always use this file" checkbox (NOTE if you
                  // don't pass this, it will be disabled)
  OAIF_REGISTER_EXT =
      0x00000002,         // do the registration after the user hits "ok"
  OAIF_EXEC = 0x00000004, // execute file after registering
  OAIF_FORCE_REGISTRATION =
      0x00000008, // force the "always use this file" checkbox to be checked
                  // (normally, you won't use the OAIF_ALLOW_REGISTRATION when
                  // you pass this)
#if (NTDDI_VERSION >= NTDDI_VISTA)
  OAIF_HIDE_REGISTRATION =
      0x00000020, // hide the "always use this file" checkbox
  OAIF_URL_PROTOCOL =
      0x00000040, // the "extension" passed is actually a protocol (uri scheme),
                  // and open with should show apps registered as capable of
...
typedef struct _openasinfo {
  ...
  OPEN_AS_INFO_FLAGS
  oaifInFlags; // [in] input flags from OPEN_AS_INFO_FLAGS enumeration
} OPENASINFO, *POPENASINFO;

#include <poppack.h> /* Return to byte packing */

#if (NTDDI_VERSION >= NTDDI_VISTA)
SHSTDAPI SHOpenWithDialog(_In_opt_ HWND hwndParent,
                          _In_ const OPENASINFO *poainfo);
#endif // NTDDI_VISTA

SHSTDAPI_(BOOL)
Shell_GetImageLists(_Out_opt_ HIMAGELIST *phiml,
                    _Out_opt_ HIMAGELIST *phimlSmall);
SHSTDAPI_(int)
Shell_GetCachedImageIndex(_In_ PCWSTR pwszIconPath, int iIconIndex,
                          UINT uIconFlags);
#if (NTDDI_VERSION >= NTDDI_VISTA)
SHSTDAPI_(int)
Shell_GetCachedImageIndexA(_In_ LPCSTR pszIconPath, int iIconIndex,
                           UINT uIconFlags);
SHSTDAPI_(int)
Shell_GetCachedImageIndexW(_In_ LPCWSTR pszIconPath, int iIconIndex,
                           UINT uIconFlags);
#ifdef UNICODE
...
#define VALIDATEUNC_PRINT                                                      \
  0x0004 // validate as print share instead of disk share
#if (NTDDI_VERSION >= NTDDI_VISTA)
...
SHSTDAPI_(BOOL)
SHValidateUNC(_In_opt_ HWND hwndOwner, _Inout_ PWSTR pszFile, UINT fConnect);
...
typedef struct PROPPRG {      /* prg */
  ...
  CHAR achCmdLine[PIFSTARTLOCSIZE + PIFPARAMSSIZE +
                  1];                // startfile[63] + params[64]
...
SHSTDAPI_(HANDLE)
PifMgr_OpenProperties(_In_ PCWSTR pszApp, _In_opt_ PCWSTR pszPIF, UINT hInf,
                      UINT flOpt);
SHSTDAPI_(int)
PifMgr_GetProperties(_In_opt_ HANDLE hProps, _In_opt_ PCSTR pszGroup,
                     _Out_writes_bytes_opt_(cbProps) void *lpProps, int cbProps,
                     UINT flOpt);
SHSTDAPI_(int)
PifMgr_SetProperties(_In_opt_ HANDLE hProps, _In_opt_ PCSTR pszGroup,
                     _In_reads_bytes_(cbProps) const void *lpProps, int cbProps,
                     UINT flOpt);
...
SHSTDAPI_(LRESULT)
SHShellFolderView_Message(_In_ HWND hwndMain, UINT uMsg, LPARAM lParam);

//
// Callback interface for the IShellView object returned from
...
//
// IShellFolderView is supported by the IShellView object returned from
// SHCreateShellFolderView.
//
// Warnings:
...
//  incorrectly.
//  - Few of these methods have parameter or range validation, so callers can
//  cause the IShellView to fault.
...
#define SFVSOC_INVALIDATE_ALL                                                  \
  0x00000001 // Assumed to reset only what is neccessary...
...
DECLARE_INTERFACE_IID_(IShellFolderView, IUnknown,
  ...
  STDMETHOD(Rearrange)(THIS_ LPARAM lParamSort)
      PURE; // use IFolderView2::SetSortColumns
  STDMETHOD(GetArrangeParam)(THIS_ _Out_ LPARAM * plParamSort)
      PURE;                             // use IFolderView2::GetSortColumns
  ...
  STDMETHOD(AddObject)(THIS_ _In_ PUITEMID_CHILD pidl, _Out_ UINT * puItem)
      PURE; // items added here may disappear (the data source is the final
            // arbiter of which items are available to the view)
  STDMETHOD(GetObject)(THIS_ _Outptr_ PITEMID_CHILD * ppidl, UINT uItem)
      PURE; // use IFolderView::Item
  STDMETHOD(RemoveObject)(THIS_ _In_opt_ PUITEMID_CHILD pidl,
                          _Out_ UINT * puItem)
      PURE; // items removed here may reappear (the data source is the final
            // arbiter of which items are available to the view)
  STDMETHOD(GetObjectCount)(THIS_ _Out_ UINT * puCount)
      PURE; // use IFolderView::ItemCount
  STDMETHOD(SetObjectCount)(THIS_ UINT uCount, UINT dwFlags)
      PURE; // not implemented on Vista.  Sends LVM_SETITEMCOUNT with
            // WPARAM=uCount and LPARAM=dwFlags to listview on XP.
  STDMETHOD(UpdateObject)(THIS_ _In_ PUITEMID_CHILD pidlOld,
                          _In_ PUITEMID_CHILD pidlNew, _Out_ UINT * puItem)
      PURE; // swaps ITEMID_CHILDs, returning new index.  Changes may be
            // discarded (the data source is the final arbiter of which items
            // are available to the view)
  STDMETHOD(RefreshObject)(THIS_ _In_ PUITEMID_CHILD pidl, _Out_ UINT * puItem)
      PURE; // tickles the listview to re-draw the item
  STDMETHOD(SetRedraw)(THIS_ BOOL bRedraw)
      PURE; // sends WM_SETREDRAW to the listview
  STDMETHOD(GetSelectedCount)(THIS_ _Out_ UINT * puSelected)
      PURE; // use IFolderView2::GetSelection
  // NOTE: GetSelectedObjects hands out const pointers to internal ITEMID_CHILD
  // structures. The caller is expected to act on them immediately (and not
  // cache them).  LocalFree the array, but not the items it contains.
  STDMETHOD(GetSelectedObjects)(
      THIS_ _Outptr_result_buffer_(*puItems) PCUITEMID_CHILD * *pppidl,
      _Out_ UINT * puItems) PURE; // use IFolderView2::GetSelection.
  STDMETHOD(IsDropOnSource)(THIS_ _In_opt_ IDropTarget * pDropTarget)
      PURE; // use IFolderView2::IsMoveInSameFolder
  STDMETHOD(GetDragPoint)(THIS_ _Out_ POINT * ppt)
      PURE; // returns point corresponding to drag-and-drop operation
  STDMETHOD(GetDropPoint)(THIS_ _Out_ POINT * ppt)
      PURE; // returns point corresponding to drag-and-drop operation
  STDMETHOD(MoveIcons)(THIS_ _In_ IDataObject * pDataObject)
      PURE; // not implemented
  STDMETHOD(SetItemPos)(THIS_ _In_ PCUITEMID_CHILD pidl, _In_ POINT * ppt)
      PURE; // use IFolderView::SelectAndPositionItems
  STDMETHOD(IsBkDropTarget)(THIS_ _In_opt_ IDropTarget * pDropTarget)
      PURE; // returns S_OK if drag-and-drop is on the background, S_FALSE
            // otherwise
  STDMETHOD(SetClipboard)(THIS_ BOOL bMove)
      PURE; // if bMove is TRUE, this attempts to cut (edit.cut, ctrl-x) the
            // current selection.  bMove of FALSE is not supported.
  STDMETHOD(SetPoints)(THIS_ _In_ IDataObject * pDataObject)
      PURE; // copies points of current selection in to data object.  Call is
            // not needed if drag operation was originated by the IShellView.
  STDMETHOD(GetItemSpacing)(THIS_ _Out_ ITEMSPACING * pSpacing)
      PURE; // use IFolderView::GetSpacing instead.  GetItemSpacing returns the
            // spacing for small and large view modes only, returning S_OK if
            // the current view mode is is positionable, S_FALSE otherwise.
  STDMETHOD(SetCallback)(THIS_ _In_opt_ IShellFolderViewCB * pNewCB,
                         _Outptr_result_maybenull_ IShellFolderViewCB *
                             *ppOldCB)
      PURE; // replace the IShellFolderViewCB that the IShellView uses
  STDMETHOD(Select)(THIS_ UINT dwFlags)
      PURE; // SFVS_ select flags: select all, select none, invert selection
  STDMETHOD(QuerySupport)(THIS_ _Inout_ UINT * pdwSupport)
      PURE; // does nothing, returns S_OK.
  STDMETHOD(SetAutomationObject)(THIS_ _In_opt_ IDispatch * pdisp)
      PURE; // replaces the IShellView's internal automation object.
};
...
SHSTDAPI SHCreateShellFolderView(_In_ const SFV_CREATE *pcsfv,
                                 _Outptr_ IShellView **ppsv);
typedef HRESULT(CALLBACK *LPFNDFMCALLBACK)(_In_opt_ IShellFolder *psf,
                                           _In_opt_ HWND hwnd,
...
SHSTDAPI CDefFolderMenu_Create2(
    _In_opt_ PCIDLIST_ABSOLUTE pidlFolder, _In_opt_ HWND hwnd, UINT cidl,
    _In_reads_opt_(cidl) PCUITEMID_CHILD_ARRAY apidl,
    _In_opt_ IShellFolder *psf, _In_opt_ LPFNDFMCALLBACK pfn, UINT nKeys,
    _In_reads_opt_(nKeys) const HKEY *ahkeys, _Outptr_ IContextMenu **ppcm);
typedef struct {
  HWND hwnd;
  ...
  PCUITEMID_CHILD_ARRAY apidl;  // items operating on, used to get IDataObject
                                // and IAssociationArray
  IUnknown
      *punkAssociationInfo; // optional: IQueryAssociations, specifies where to
                            // load extensions from, computed from apidl if NULL
  UINT cKeys;               // # of items in aKeys, may be zero
  const HKEY *aKeys;        // optional: specifies where to load extensions from
} DEFCONTEXTMENU;

// creates object that implements IContextMenu/IContextMenu2/IContextMenu3,
// typically used in the implemetnation of ::GetUIObjectOf()

#if (NTDDI_VERSION >= NTDDI_VISTA)
SHSTDAPI SHCreateDefaultContextMenu(_In_ const DEFCONTEXTMENU *pdcm,
...
//  Standard Items // DFM_MERGECONTEXTMENU, context menu extensions,
//  DFM_MERGECONTEXTMENU_TOP Separator View Items   // context menu extensions
//  can get here Separator (defcm S_FALSE "default" items, if applicable)
//  Separator
...
#define DFM_MODIFYQCMFLAGS                                                     \
  18 // uFlags       UINT *puNewFlags;   modify the CFM_XXX values passed to
...
SHFind_InitMenuPopup(_In_ HMENU hmenu, _In_opt_ HWND hwndOwner, UINT idCmdFirst,
                     UINT idCmdLast);
SHSTDAPI SHCreateShellFolderViewEx(_In_ CSFV *pcsfv,
                                   _Outptr_ IShellView **ppsv);
...
//      icon index. PID_INTSITE_RAWURL       [VT_LPWSTR]   The raw, un-encoded,
//      unicode url. PID_INTSITE_ROAMED       [VT_UI4]      Indicates that this
//      entry was roamed from a different machine
...
#define PIDISR_NEEDS_UPDATE                                                    \
  2 // The entry was roamed and contains updated information
...
typedef struct {
  ...
  // new for Whistler.
  BOOL fStartPanelOn
      : 1; // Indicates if the Whistler StartPanel mode is ON or OFF.
  BOOL fShowStartPage
      : 1; // Indicates if the Whistler StartPage on desktop is ON or OFF.
...
SHSTDAPI_(void)
SHGetSetSettings(_Inout_opt_ LPSHELLSTATE lpss, DWORD dwMask, BOOL bSet);
...
typedef struct {
  ...
  BOOL fAutoCheckSelect : 1;
  BOOL fIconsOnly : 1;
  UINT fRestFlags : 1; // when adding additional flags keep SHELLSTATE and
                       // SHGetSettings in sync.
#else
  UINT fRestFlags : 3; // when adding additional flags keep SHELLSTATE and
                       // SHGetSettings in sync.
#endif
...
// pidl's parent folder (in ppv) If ppidlLast is non-NULL, you can also get the
// pidl of the last item.
//
SHSTDAPI SHBindToParent(_In_ PCIDLIST_ABSOLUTE pidl, _In_ REFIID riid,
                        _Outptr_ void **ppv,
...
SHSTDAPI SHBindToFolderIDListParent(_In_opt_ IShellFolder *psfRoot,
                                    _In_ PCUIDLIST_RELATIVE pidl,
                                    _In_ REFIID riid, _Outptr_ void **ppv,
                                    _Outptr_opt_ PCUITEMID_CHILD *ppidlLast);

// same as SHBindToFolderIDListParent but with a IBindCtx *pbc;
SHSTDAPI SHBindToFolderIDListParentEx(_In_opt_ IShellFolder *psfRoot,
                                      _In_ PCUIDLIST_RELATIVE pidl,
                                      _In_opt_ IBindCtx *ppbc, _In_ REFIID riid,
                                      _Outptr_ void **ppv,
                                      _Outptr_opt_ PCUITEMID_CHILD *ppidlLast);

// helper function that gets the desktop object, then calls BindToObject on that
SHSTDAPI SHBindToObject(_In_opt_ IShellFolder *psf,
                        _In_ PCUIDLIST_RELATIVE pidl, _In_opt_ IBindCtx *pbc,
                        _In_ REFIID riid, _Outptr_ void **ppv);

#endif // NTDDI_VISTA

// This function is used to validate that the container structure of an IDList
// is valid. This should be used by all code that reads an IDList from a
// persistence format to ensure that invalid forms do not lead to a security
// exploit in the code that interprets the IDList. Shell data sources are
// responsible for validating the private parts of the ITEMIDs. Hidden data is
// validated by the functions that interpret that data.
__inline BOOL IDListContainerIsConsistent(_In_reads_bytes_(cbAlloc)
...
//  a pidl if no pbc specified, it uses the preferred options for parsing. this
//  includes mapping file system paths to their appropriate aliased location
//  (RegisterObjectParam(STR_PARSE_TRANSLATE_ALIASES)) psfgaoOut is optional for
//  SFGAO attributes
//
#if (NTDDI_VERSION >= NTDDI_WINXP)
SHSTDAPI SHParseDisplayName(_In_ PCWSTR pszName, _In_opt_ IBindCtx *pbc,
                            _Outptr_ PIDLIST_ABSOLUTE *ppidl,
                            _In_ SFGAOF sfgaoIn, _Out_opt_ SFGAOF *psfgaoOut);
#endif // NTDDI_WINXP
...
// 3. Remount mapped drives if the connection was lost. (\\unc\share mapped to
// N: becomes disconnected)
// 4. If the path doesn't exist, create it.  (SHPPFW_DIRCREATE and
// SHPPFW_ASKDIRCREATE)
...
//      punkEnableModless: Parent that will be set to modal during UI using
//      IOleInPlaceActiveObject::EnableModeless(). OPTIONAL pszPath: Path to
//      verify is valid for writting.  This can be a UNC or file drive path. The
//      path
//               should only contain directories.  Pass SHPPFW_IGNOREFILENAME if
//               the last path segment is always filename to ignore.
...
#define SHPPFW_DIRCREATE                                                       \
  0x00000001 // Create the directory if it doesn't exist without asking the
             // user.
#define SHPPFW_ASKDIRCREATE                                                    \
  0x00000002 // Create the directory if it doesn't exist after asking the user.
#define SHPPFW_IGNOREFILENAME                                                  \
  0x00000004 // Ignore the last item in pszPath because it's a file.  Example:
             // pszPath="C:\DirA\DirB", only use "C:\DirA".
#define SHPPFW_NOWRITECHECK                                                    \
  0x00000008 // Caller only needs to read from the drive, so don't check if it's
             // READ ONLY.
#if (NTDDI_VERSION >= NTDDI_WINXP)
#define SHPPFW_MEDIACHECKONLY                                                  \
  0x00000010 // do the retrys on the media (or net path), return errors if the
             // file can't be found
#endif       // NTDDI_WINXP

SHSTDAPI SHPathPrepareForWriteA(_In_opt_ HWND hwnd,
                                _In_opt_ IUnknown *punkEnableModless,
                                _In_ LPCSTR pszPath, DWORD dwFlags);
SHSTDAPI SHPathPrepareForWriteW(_In_opt_ HWND hwnd,
                                _In_opt_ IUnknown *punkEnableModless,
                                _In_ LPCWSTR pszPath, DWORD dwFlags);
#ifdef UNICODE
...
DECLARE_INTERFACE_IID_(INamedPropertyBag, IUnknown,
  ...
  STDMETHOD(WritePropertyNPB)(THIS_ _In_ PCWSTR pszBagname,
                              _In_ PCWSTR pszPropName, _In_ PROPVARIANT * pVar)
      PURE;
...
//  be created with PROPSETFLAG_ANSI, otherwise PROPSETFLAG_DEFAULT.
...
//  puCodePage      //  Optional address to receive the code page ID for the
//  set.
//
SHSTDAPI SHPropStgCreate(_In_ IPropertySetStorage *psstg, _In_ REFFMTID fmtid,
                         _In_opt_ const CLSID *pclsid, DWORD grfFlags,
                         DWORD grfMode, DWORD dwDisposition,
                         _Outptr_ IPropertyStorage **ppstg,
...
//
SHSTDAPI SHPropStgReadMultiple(_In_ IPropertyStorage *pps, UINT uCodePage,
                               ULONG cpspec,
                               _In_reads_(cpspec) PROPSPEC const rgpspec[],
                               _Out_writes_all_(cpspec) PROPVARIANT rgvar[]);
...
//  be >= PID_FIRST_USABLE
//
SHSTDAPI SHPropStgWriteMultiple(_In_ IPropertyStorage *pps,
                                _Inout_opt_ UINT *puCodePage, ULONG cpspec,
                                _In_reads_(cpspec) PROPSPEC const rgpspec[],
                                _Inout_updates_(cpspec) PROPVARIANT rgvar[],
                                PROPID propidNameFirst);

#if (NTDDI_VERSION >= NTDDI_WINXP)
SHSTDAPI SHCreateFileExtractIconA(_In_ LPCSTR pszFile,
                                  _In_ DWORD dwFileAttributes, _In_ REFIID riid,
                                  _Outptr_ void **ppv);
SHSTDAPI SHCreateFileExtractIconW(_In_ LPCWSTR pszFile,
                                  _In_ DWORD dwFileAttributes, _In_ REFIID riid,
                                  _Outptr_ void **ppv);
#ifdef UNICODE
...
STDAPI SHGetAttributesFromDataObject(_In_opt_ IDataObject *pdo,
...
// icon.
SHSTDAPI_(int)
SHMapPIDLToSystemImageListIndex(_In_ IShellFolder *pshf,
                                _In_ PCUITEMID_CHILD pidl,
                                _Out_opt_ int *piIndexSel);

SHSTDAPI SHCLSIDFromString(_In_ PCWSTR psz, _Out_ CLSID *pclsid);
SHSTDAPI_(int)
PickIconDlg(_In_opt_ HWND hwnd, _Inout_updates_(cchIconPath) PWSTR pszIconPath,
            UINT cchIconPath, _Inout_opt_ int *piIconIndex);

#if (NTDDI_VERSION >= NTDDI_WIN7)
// returns an IStream or IStorage via riid/ppv
STDAPI StgMakeUniqueName(_In_ IStorage *pstgParent, _In_ PCWSTR pszFileSpec,
                         _In_ DWORD grfMode, _In_ REFIID riid,
                         _Outptr_ void **ppv);
#endif // NTDDI_WIN7
