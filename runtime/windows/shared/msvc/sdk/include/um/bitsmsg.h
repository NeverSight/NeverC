...
// MessageText:
//
// The requested action is not allowed in the current job state. The job might
// have been canceled or completed transferring. It is in a read-only state now.
//
...
// MessageText:
//
// There are no files attached to this job. Attach files to the job, and then
// try again.
//
...
// MessageText:
//
// The error occurred in the Background Intelligent Transfer Service (BITS)
// queue manager.
//
...
// MessageText:
//
// The error occurred while the local file was being processed. Verify that the
// file is not in use, and then try again.
//
...
// MessageText:
//
// The error occurred in the transport layer. The client could not connect to
// the server.
//
...
// MessageText:
//
// The error occurred while the notification callback was being processed.
...
// program, such as CheckDisk, is not running, which would lock the volume. When
// the volume is available, Background Intelligent Transfer Service (BITS) will
// try again.
//
...
// MessageText:
//
// There are currently no active network connections. Background Intelligent
// Transfer Service (BITS) will try again when an adapter is connected.
//
...
// MessageText:
//
// The server did not return the file size. The URL might point to dynamic
// content. The Content-Length header is not available in the server's HTTP
// reply.
//
...
// Intelligent Transfer Service (BITS) requires that the server support the
// Range protocol header.
//
...
// MessageText:
//
// The drive mapping for the job is different for the current owner than for the
// previous owner. Use a UNC path instead.
//
...
// MessageText:
//
// The new owner has insufficient access to the local files for the job. The new
// owner might not have permissions to access the job files. Verify that the new
// owner has sufficient permissions, and then try again.
//
...
// MessageText:
//
// The HTTP proxy list cannot be longer than 32,000 characters. Try again with a
// shorter proxy list.
//
...
// MessageText:
//
// The HTTP proxy bypass list cannot be longer than 32,000 characters. Try again
// with a shorter bypass proxy list.
//
...
// MessageText:
//
// Some of the temporary files could not be deleted. Check the system event log
// for the complete list of files that could not be deleted.
//
...
// MessageText:
//
// The server's response was not valid. The server was not following the defined
// protocol. Resume the job, and then Background Intelligent Transfer Service
// (BITS) will try again.
//
...
// MessageText:
//
// The local file was changed during the transfer. Recreate the job, and then
// try to transfer it again.
//
...
// MessageText:
//
// The specified session could not be found on the server. Background
...
// MessageText:
//
// The job is too large for the server to accept. This job might exceed a job
// size limit set by the server administrator. Reduce the size of the job, and
// then try again.
//
...
// MessageText:
//
// The client and server versions of Background Intelligent Transfer Service
// (BITS) are incompatible.
//
...
// MessageText:
//
// Scripting OR execute permissions are enabled on the IIS virtual directory
// associated with the job. To upload files to the virtual directory, disable
// the scripting and execute permissions on the virtual directory.
//
...
// MessageText:
//
// The job is not making headway.  The server may be misconfigured.  Background
// Intelligent Transfer Service (BITS) will try again later.
//
...
// MessageText:
//
// The user name cannot be longer than 300 characters. Try again with a shorter
// name.
//
...
// MessageText:
//
// The password cannot be longer than 65536 characters. Try again with a shorter
// password.
//
...
// MessageText:
//
// The requested byte range extends beyond the end of the web page.  Use byte
// ranges that are wholly within the page.
//
...
// MessageText:
//
// The list of byte ranges contains some overlapping ranges, which are not
// supported.
//
...
// MessageText:
//
// The chosen cache record has been deleted.  The attempt to update it has been
// abandoned.
//
...
// MessageText:
//
// The system is already searching for neighbors.  Retry the operation after a
// few seconds.
//
...
// MessageText:
//
// A Universal Plug and Play (UPnP) error has occured.  Please check your
...
// MessageText:
//
// The cache record is in use and can not be changed or deleted at this time.
...
// MessageText:
//
// The application requested data from a web site, but the response was invalid.
// Using Event Viewer, check the log 'Application Logs \ Microsoft \ Windows
// \ Bits-client \ Operational' for more details.
//
...
// MessageText:
//
// BITS timed out downloading the job. The download did not complete within the
// maximum download time set on the job.
//
...
// MessageText:
//
// The configuration preferences have been saved successfully, but one or more
// of them are overridden by Group Policy.
//
...
// MessageText:
//
// The job is configured to use a different security token for some operations,
// but the token is not currently available.  The application must provide a
// token and then resume the job.
//
...
// MessageText:
//
// The job's cost transfer policy settings prevent the job from transferring at
// this time.
//
...
// MessageText:
//
// The value provided for BITS_JOB_PROPERTY_MAX_DOWNLOAD_SIZE property is
// invalid. Please provide value between 1 and UINT64_MAX.
//
...
// MessageText:
//
// BITS cannot continue downloading the job. The download reached the maximum
// download size limit set on the job.
//
...
// MessageText:
//
// The transfer was paused because the computer is in power-saving mode. The
// transfer will resume when the computer wakes up.
//
...
// MessageText:
//
// The value provided for BITS_JOB_PROPERTY_USE_STORED_CREDENTIALS property is
// invalid. The property is only supported for Proxy (BG_AUTH_TARGET_PROXY)
// targets.
//
...
// MessageText:
//
// The transfer was paused because the computer is running on battery power. The
// transfer will resume when the system is reconnected to A/C power.
//
...
// MessageText:
//
// The transfer was paused because the computer is in Battery Saver mode. The
// transfer will resume when the system is reconnected to A/C power or no longer
...
// MessageText:
//
// BITS usage by app packages cannot be supported on this device because one or
...
// MessageText:
//
// BITS persistent state corruption was detected. The necessary steps will be
// taken to fix this issue once BITS is restarted. BITS will now shutdown.
//
...
// MessageText:
//
// BITS IBackgroundCopyFile6 methods are not supported in SMB, dynamic content,
// multirange, upload, or upload-reply transfers.
//
...
// MessageText:
//
// The background access settings of the job's owner app prevent the job from
...
// MessageText:
//
// The job was paused because there is an active Game Mode recipient that isn't
// the job's owner. The job will resume once the Game Mode recipient loses input
// focus or is closed.
//
...
// MessageText:
//
// The job was paused due to system resource constraints. The job will
// automatically resume once system conditions change.
//
...
// MessageText:
//
// The requested action is not supported by jobs configured with a custom HTTP
// method.
//
...
// MessageText:
//
// The remote file was changed during the transfer. If this is expected, call
// Resume on the job to initiate a retry.
//
...
// MessageText:
//
// The job is configured to raise a certificate validation callback, but the
// callback object is not currently available. The job owner must provide a new
// callback object and then resume the job.
//
...
// MessageText:
//
// This method should only be called when the job is in a paused state
// (SUSPENDED/ERROR/TRANSFERRED).
//
...
// MessageText:
//
// The error occurred while the user-provided server certificate validation
// callback was being processed.
//
...
// MessageText:
//
// HTTP status 200: The server's response was not valid. The server was not
// following the defined protocol. Resume the job, and then Background
// Intelligent Transfer Service (BITS) will try again.
//
...
// MessageText:
//
// HTTP status 201: The request was fulfilled and resulted in the creation of a
// new resource.
//
...
// MessageText:
//
// HTTP status 202: The request was accepted for processing, but the processing
...
// MessageText:
//
// HTTP status 203: The returned metadata in the entity-header is not the
// definitive set available from the server of origin.
//
...
// MessageText:
//
// HTTP status 204: The server has fulfilled the request, but there is no new
...
// MessageText:
//
// HTTP status 205: The server's response was not valid. The server was not
// following the defined protocol. Resume the job, and then Background
// Intelligent Transfer Service (BITS) will try again.
//
...
// MessageText:
//
// HTTP status 206: The server fulfilled the partial GET request for the
// resource.
//
...
// MessageText:
//
// HTTP status 301: The requested resource was assigned to a new permanent
// Uniform Resource Identifier (URI), and any future references to this resource
...
// MessageText:
//
// HTTP status 302: The requested resource was assigned a different Uniform
// Resource Identifier (URI). This change is temporary.
//
...
// MessageText:
//
// HTTP status 303: The response to the request is under a different Uniform
// Resource Identifier (URI) and must be retrieved using a GET method on that
// resource.
//
...
// MessageText:
//
// HTTP status 304: The server's response was not valid. The server was not
// following the defined protocol. Resume the job, and then Background
// Intelligent Transfer Service (BITS) will try again.
//
...
// MessageText:
//
// HTTP status 305: The requested resource must be accessed through the proxy
...
// MessageText:
//
// HTTP status 400: The server cannot process the request because the syntax is
// not valid.
//
...
// MessageText:
//
// HTTP status 402: The server's response was not valid. The server was not
// following the defined protocol. Resume the job, and then Background
// Intelligent Transfer Service (BITS) will try again.
//
...
// MessageText:
//
// HTTP status 403: The client does not have sufficient access rights to the
...
// MessageText:
//
// HTTP status 409: The request could not be completed because of a conflict
// with the current state of the resource. The user should resubmit the request
...
// MessageText:
//
// HTTP status 410: The requested resource is not currently available at the
...
// MessageText:
//
// HTTP status 411: The server cannot accept the request without a defined
// content length.
//
...
// MessageText:
//
// HTTP status 412: The precondition given in one or more of the request header
...
// MessageText:
//
// HTTP status 413: The server cannot process the request because the request
// entity is too large.
//
...
// MessageText:
//
// HTTP status 414: The server cannot process the request because the request
...
// MessageText:
//
// HTTP status 415: The server's response was not valid. The server was not
// following the defined protocol. Resume the job, and then Background
// Intelligent Transfer Service (BITS) will try again.
//
...
// MessageText:
//
// HTTP status 417: The server could not meet the expectation given in an Expect
...
// MessageText:
//
// HTTP status 449: The server's response was not valid. The server was not
// following the defined protocol. Resume the job, and then Background
// Intelligent Transfer Service (BITS) will try again.
//
...
// MessageText:
//
// HTTP status 500: An unexpected condition prevented the server from fulfilling
// the request.
//
...
// MessageText:
//
// HTTP status 501: The server does not support the functionality required to
// fulfill the request.
//
...
// MessageText:
//
// HTTP status 502: The server, while acting as a gateway or proxy to fulfill
// the request, received an invalid response from the upstream server it
// accessed.
//
...
// MessageText:
//
// HTTP status 505: The server does not support the HTTP protocol version that
...
// MessageText:
//
// While canceling job "%2", BITS was not able to remove the temporary files
// listed below. If you can delete them, then you will regain some disk space.
// The job ID was %1.
//
// %3
//
...
// %3
//
// Due to space limitations, not all files are listed here.  Check for
// additional files of the form BITxxx.TMP in the same directory.
//
...
// MessageText:
//
// The administrator %3 modified the %4 property of job "%2".  The job ID was
// %1.
//
...
// MessageText:
//
// Job "%2" owned by %3 was canceled after being inactive for more than %4 days.
// The job ID was %1.
//
...
// MessageText:
//
// Job "%2" owned by %3 failed to notify its associated application.  BITS will
// retry in %4 minutes.  The job ID was %1.
//
...
// MessageText:
//
// The BITS job list is not in a recognized format.  It may have been created by
// a different version of BITS.  The job list has been cleared.
//
...
// MessageText:
//
// BITS has hit a fatal error communicating with an Internet Gateway Device.
// Please check that the device is functioning properly. BITS will not attempt
// to use this device until the next system reboot.
//
