...
#define DS_BEHAVIOR_WIN2025                                                    \
  10 // Note: WS2019 and WS2022 did not have native functional levels
...
//
#define DS_INSTANCETYPE_IS_NC_HEAD                                             \
  0x00000001 // This if what to specify on an object to indicate it's an NC
             // Head.
#define DS_INSTANCETYPE_NC_IS_WRITEABLE                                        \
  0x00000004 // This is to indicate that the NC Head is writeable.
#define DS_INSTANCETYPE_NC_COMING                                              \
  0x00000010 // This is to indicate that this NC is still replicating in objects
             // to this DC, and may not be a complete NC.
#define DS_INSTANCETYPE_NC_GOING                                               \
  0x00000020 // This is to indicate that this NC is in the process of being
             // removed from this DC, and may not be a complete NC.
...
#define NTDSDSA_OPT_DISABLE_INBOUND_REPL                                       \
  (1 << 1) /* disable inbound replication */
#define NTDSDSA_OPT_DISABLE_OUTBOUND_REPL                                      \
  (1 << 2) /* disable outbound replication */
#define NTDSDSA_OPT_DISABLE_NTDSCONN_XLATE                                     \
  (1 << 3) /* disable logical conn xlation */
#define NTDSDSA_OPT_DISABLE_SPN_REGISTRATION                                   \
  (1 << 4) /* disable SPN registration for ADAM */
#define NTDSDSA_OPT_GENERATE_OWN_TOPO (1 << 5) /* create own site topology */
#define NTDSDSA_OPT_BLOCK_RPC                                                  \
  (1 << 6) /* do not connect to this server via any RPC calls */

// Bit flags for options attribute on NTDS-Connection objects.
//
// The reasons that two bits are required to control notification are as
// follows. We must support existing connections with the old behavior and the
// UI does not create manual connections with the new bit set. The default for
// existing and manually created connections with bits 2 and 3 clear must be the
// standard prior behavior: notification for intra-site and no notification for
// inter-site. We need a way to distinguish a old connection which desires the
// default notification rules, and a new connection for which we desire to
// explicitly control the notification state as passed down from a site link.
// Thus we have a new bit to say we are overriding the default, and a new bit to
// indicate what the overridden default shall be.
//
#define NTDSCONN_OPT_IS_GENERATED                                              \
  (1 << 0) /* object generated by DS, not admin */
#define NTDSCONN_OPT_TWOWAY_SYNC                                               \
  (1 << 1) /* force sync in opposite direction at end of sync */
#define NTDSCONN_OPT_OVERRIDE_NOTIFY_DEFAULT                                   \
  (1 << 2) // Do not use defaults to determine notification
...
// for inbound connections. Bit 31 is used to force FRS to ignore schedule
// during the initial sync. Bits 30 - 28 are used to specify a priority between
// 0-7.
//
...
//
#define NTDSSETTINGS_OPT_IS_AUTO_TOPOLOGY_DISABLED                             \
  (1 << 0) /* automatic topology gen disabled */
#define NTDSSETTINGS_OPT_IS_TOPL_CLEANUP_DISABLED                              \
  (1 << 1) /* automatic topology cleanup disabled */
#define NTDSSETTINGS_OPT_IS_TOPL_MIN_HOPS_DISABLED                             \
  (1 << 2) /* automatic minimum hops topology disabled */
#define NTDSSETTINGS_OPT_IS_TOPL_DETECT_STALE_DISABLED                         \
  (1 << 3) /* automatic stale server detection disabled */
#define NTDSSETTINGS_OPT_IS_INTER_SITE_AUTO_TOPOLOGY_DISABLED                  \
  (1 << 4) /* automatic inter-site topology gen disabled */
#define NTDSSETTINGS_OPT_IS_GROUP_CACHING_ENABLED                              \
  (1 << 5) /* group memberships for users enabled */
#define NTDSSETTINGS_OPT_FORCE_KCC_WHISTLER_BEHAVIOR                           \
  (1 << 6) /* force KCC to operate in Whistler behavior mode */
#define NTDSSETTINGS_OPT_FORCE_KCC_W2K_ELECTION                                \
  (1 << 7) /* force KCC to use the Windows 2000 ISTG election algorithm */
#define NTDSSETTINGS_OPT_IS_RAND_BH_SELECTION_DISABLED                         \
  (1 << 8) /* prevent the KCC from randomly picking a bridgehead when creating \
              a connection */
#define NTDSSETTINGS_OPT_IS_SCHEDULE_HASHING_ENABLED                           \
  (1 << 9) /* allow the KCC to use hashing when creating a replication         \
              schedule */
#define NTDSSETTINGS_OPT_IS_REDUNDANT_SERVER_TOPOLOGY_ENABLED                  \
  (1 << 10) /* create static failover connections */

// The following two options allow the new kcc spanning tree algorithm, which
// replaces the ISM, to override the ISM policy settings. These two flags are
// analogous to those found on the transport object.
...
// default without flag: links transitive (bridges not required) auto site link
// bridging enabled This flag causes the KCC in W2K3 mode to require bridges
#define NTDSSETTINGS_OPT_W2K3_BRIDGES_REQUIRED                                 \
  (1 << 12) // siteLink bridges are required
...
#define NTDSTRANSPORT_OPT_BRIDGES_REQUIRED                                     \
  (1 << 1) // siteLink bridges are required
...
#define NTDSSITECONN_OPT_TWOWAY_SYNC                                           \
  (1 << 1) /* force sync in opposite direction at end of sync */
...
#define NTDSSITELINK_OPT_TWOWAY_SYNC                                           \
  (1 << 1) /* force sync in opposite direction at end of sync */
...
#define GUID_MANAGED_SERVICE_ACCOUNTS_CONTAINER_W                              \
  L"1EB93889E40C45DF9F0C64D23BBB6237"
...
_Check_return_ NTDSAPI DWORD WINAPI
DsBindW(_In_opt_ LPCWSTR DomainControllerName, // in, optional
        _In_opt_ LPCWSTR DnsDomainName,        // in, optional
        _Out_ HANDLE *phDS);

_Check_return_ NTDSAPI DWORD WINAPI
DsBindA(_In_opt_ LPCSTR DomainControllerName, // in, optional
...
_Check_return_ NTDSAPI DWORD WINAPI
DsBindWithCredW(_In_opt_ LPCWSTR DomainControllerName,          // in, optional
                _In_opt_ LPCWSTR DnsDomainName,                 // in, optional
                _In_opt_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity, // in, optional
                _Out_ HANDLE *phDS);

_Check_return_ NTDSAPI DWORD WINAPI
DsBindWithCredA(_In_opt_ LPCSTR DomainControllerName,           // in, optional
                _In_opt_ LPCSTR DnsDomainName,                  // in, optional
                _In_opt_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity, // in, optional
                _Out_ HANDLE *phDS);
...
_Check_return_ NTDSAPI DWORD WINAPI
DsBindWithSpnW(_In_opt_ LPCWSTR DomainControllerName,          // in, optional
               _In_opt_ LPCWSTR DnsDomainName,                 // in, optional
               _In_opt_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity, // in, optional
...
_Check_return_ NTDSAPI DWORD WINAPI
DsBindWithSpnA(_In_opt_ LPCSTR DomainControllerName,           // in, optional
               _In_opt_ LPCSTR DnsDomainName,                  // in, optional
               _In_opt_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity, // in, optional
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI
DsBindWithSpnExW(_In_opt_ LPCWSTR DomainControllerName,          // in, optional
                 _In_opt_ LPCWSTR DnsDomainName,                 // in, optional
                 _In_opt_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity, // in, optional
                 _In_opt_ LPCWSTR ServicePrincipalName,          // in, optional
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI
DsBindWithSpnExA(_In_opt_ LPCSTR DomainControllerName,           // in, optional
                 _In_opt_ LPCSTR DnsDomainName,                  // in, optional
                 _In_opt_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity, // in, optional
                 _In_opt_ LPCSTR ServicePrincipalName,           // in, optional
                 _In_opt_ DWORD BindFlags,                       // in, optional
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI DsBindByInstanceW(
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI DsBindByInstanceA(
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI
DsBindToISTGW(_In_opt_ LPCWSTR SiteName, // in, optional
              _Out_ HANDLE *phDS);

_Check_return_ NTDSAPI_POSTXP DWORD WINAPI
DsBindToISTGA(_In_opt_ LPCSTR SiteName, // in, optional
              _Out_ HANDLE *phDS);
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI DsBindingSetTimeout(
    _In_ HANDLE hDS,        // in
...
DWORD
WINAPI
DsUnBindW(_In_ HANDLE *phDS); // in

NTDSAPI
DWORD
WINAPI
DsUnBindA(_In_ HANDLE *phDS); // in
...
_Check_return_ NTDSAPI DWORD WINAPI DsMakePasswordCredentialsW(
    _In_opt_ LPCWSTR User, _In_opt_ LPCWSTR Domain, _In_opt_ LPCWSTR Password,
    _Out_ RPC_AUTH_IDENTITY_HANDLE *pAuthIdentity);

_Check_return_ NTDSAPI DWORD WINAPI DsMakePasswordCredentialsA(
    _In_opt_ LPCSTR User, _In_opt_ LPCSTR Domain, _In_opt_ LPCSTR Password,
    _Out_ RPC_AUTH_IDENTITY_HANDLE *pAuthIdentity);
...
NTDSAPI
VOID WINAPI
DsFreePasswordCredentials(_In_ RPC_AUTH_IDENTITY_HANDLE AuthIdentity);
...
_Check_return_ NTDSAPI DWORD WINAPI
DsCrackNamesW(_In_opt_ HANDLE hDS,                       // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsCrackNamesA(_In_opt_ HANDLE hDS,                      // in
              _In_ DS_NAME_FLAGS flags,                 // in
              _In_ DS_NAME_FORMAT formatOffered,        // in
              _In_ DS_NAME_FORMAT formatDesired,        // in
              _In_ DWORD cNames,                        // in
...
NTDSAPI
void WINAPI DsFreeNameResultW(_In_ DS_NAME_RESULTW *pResult); // in

NTDSAPI
void WINAPI DsFreeNameResultA(_In_ DS_NAME_RESULTA *pResult); // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsGetSpnA(_In_ DS_SPN_NAME_TYPE ServiceType, _In_ LPCSTR ServiceClass,
          _In_opt_ LPCSTR ServiceName, _In_ USHORT InstancePort,
          ...
          _Out_ DWORD *pcSpn, _Outptr_result_buffer_(*pcSpn) LPSTR **prpszSpn);

_Check_return_ NTDSAPI DWORD WINAPI
DsGetSpnW(_In_ DS_SPN_NAME_TYPE ServiceType, _In_ LPCWSTR ServiceClass,
          _In_opt_ LPCWSTR ServiceName, _In_ USHORT InstancePort,
          _In_ USHORT cInstanceNames,
          _In_reads_opt_(cInstanceNames) LPCWSTR *pInstanceNames,
          _In_reads_opt_(cInstanceNames) const USHORT *pInstancePorts,
          _Out_ DWORD *pcSpn, _Outptr_result_buffer_(*pcSpn) LPWSTR **prpszSpn);
...
NTDSAPI
void WINAPI DsFreeSpnArrayA(_In_ DWORD cSpn,
                            _Inout_updates_to_(cSpn, 0) LPSTR *rpszSpn);

NTDSAPI
void WINAPI DsFreeSpnArrayW(_In_ DWORD cSpn,
...
_Check_return_ NTDSAPI DWORD WINAPI DsWriteAccountSpnA(
    _In_ HANDLE hDS, _In_ DS_SPN_WRITE_OP Operation, _In_ LPCSTR pszAccount,
    _In_ DWORD cSpn, _In_reads_(cSpn) LPCSTR *rpszSpn);

_Check_return_ NTDSAPI DWORD WINAPI DsWriteAccountSpnW(
    _In_ HANDLE hDS, _In_ DS_SPN_WRITE_OP Operation, _In_ LPCWSTR pszAccount,
    _In_ DWORD cSpn, _In_reads_(cSpn) LPCWSTR *rpszSpn);
...
_Check_return_ NTDSAPI DWORD WINAPI DsClientMakeSpnForTargetServerW(
    _In_ LPCWSTR ServiceClass, _In_ LPCWSTR ServiceName,
    _Inout_ DWORD *pcSpnLength,
    _Out_writes_to_(*pcSpnLength, *pcSpnLength) LPWSTR pszSpn);

_Check_return_ NTDSAPI DWORD WINAPI DsClientMakeSpnForTargetServerA(
    _In_ LPCSTR ServiceClass, _In_ LPCSTR ServiceName,
    _Inout_ DWORD *pcSpnLength,
...
_Check_return_ NTDSAPI DWORD WINAPI
DsServerRegisterSpnA(DS_SPN_WRITE_OP Operation, _In_ LPCSTR ServiceClass,
                     _In_opt_ LPCSTR UserObjectDN);

_Check_return_ NTDSAPI DWORD WINAPI
DsServerRegisterSpnW(DS_SPN_WRITE_OP Operation, _In_ LPCWSTR ServiceClass,
...
_Check_return_ NTDSAPI DWORD WINAPI DsReplicaSyncA(_In_ HANDLE hDS,
                                                   _In_ LPCSTR NameContext,
                                                   _In_ const UUID *pUuidDsaSrc,
                                                   ULONG Options);

_Check_return_ NTDSAPI DWORD WINAPI DsReplicaSyncW(_In_ HANDLE hDS,
...
_Check_return_ NTDSAPI DWORD WINAPI
DsReplicaAddA(_In_ HANDLE hDS, _In_ LPCSTR NameContext, _In_ LPCSTR SourceDsaDn,
              _In_ LPCSTR TransportDn, _In_ LPCSTR SourceDsaAddress,
              _In_opt_ const PSCHEDULE pSchedule, DWORD Options);

_Check_return_ NTDSAPI DWORD WINAPI DsReplicaAddW(
    _In_ HANDLE hDS, _In_ LPCWSTR NameContext, _In_ LPCWSTR SourceDsaDn,
    _In_ LPCWSTR TransportDn, _In_ LPCWSTR SourceDsaAddress,
    _In_opt_ const PSCHEDULE pSchedule, DWORD Options);
...
_Check_return_ NTDSAPI DWORD WINAPI DsReplicaDelA(_In_ HANDLE hDS,
                                                  _In_ LPCSTR NameContext,
                                                  _In_ LPCSTR DsaSrc,
                                                  ULONG Options);

_Check_return_ NTDSAPI DWORD WINAPI DsReplicaDelW(_In_ HANDLE hDS,
                                                  _In_ LPCWSTR NameContext,
                                                  _In_ LPCWSTR DsaSrc,
                                                  ULONG Options);
...
_Check_return_ NTDSAPI DWORD WINAPI DsReplicaModifyA(
    _In_ HANDLE hDS, _In_ LPCSTR NameContext,
    _In_opt_ const UUID *pUuidSourceDsa, _Reserved_ LPCSTR TransportDn,
    _In_ LPCSTR SourceDsaAddress, _In_opt_ const PSCHEDULE pSchedule,
    _Reserved_ DWORD ReplicaFlags, DWORD ModifyFields, DWORD Options);

_Check_return_ NTDSAPI DWORD WINAPI DsReplicaModifyW(
    _In_ HANDLE hDS, _In_ LPCWSTR NameContext,
    _In_opt_ const UUID *pUuidSourceDsa, _Reserved_ LPCWSTR TransportDn,
    _In_ LPCWSTR SourceDsaAddress, _In_opt_ const PSCHEDULE pSchedule,
...
_Check_return_ NTDSAPI DWORD WINAPI DsReplicaUpdateRefsA(
    _In_ HANDLE hDS, _In_ LPCSTR NameContext, _In_ LPCSTR DsaDest,
    _In_ const UUID *pUuidDsaDest, ULONG Options);

_Check_return_ NTDSAPI DWORD WINAPI DsReplicaUpdateRefsW(
    _In_ HANDLE hDS, _In_ LPCWSTR NameContext, _In_ LPCWSTR DsaDest,
    _In_ const UUID *pUuidDsaDest, ULONG Options);
...
//*) 				object that will hold an array of error
// structures.

_Check_return_ NTDSAPI DWORD WINAPI DsReplicaSyncAllA(
    _In_ HANDLE hDS, _In_ LPCSTR pszNameContext, ULONG ulFlags,
...
_Check_return_ NTDSAPI DWORD WINAPI DsReplicaSyncAllW(
    _In_ HANDLE hDS, _In_ LPCWSTR pszNameContext, ULONG ulFlags,
...
_Check_return_ NTDSAPI DWORD WINAPI DsRemoveDsServerW(
    _In_ HANDLE hDs,                 // in
...
_Check_return_ NTDSAPI DWORD WINAPI DsRemoveDsServerA(
    _In_ HANDLE hDs,                 // in
...
_Check_return_ NTDSAPI DWORD WINAPI DsRemoveDsDomainW(_In_ HANDLE hDs,     // in
                                                      _In_ LPWSTR DomainDN // in
);

_Check_return_ NTDSAPI DWORD WINAPI DsRemoveDsDomainA(_In_ HANDLE hDs,    // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsListSitesA(_In_ HANDLE hDs,                     // in
             _Outptr_ PDS_NAME_RESULTA *ppSites); // out

_Check_return_ NTDSAPI DWORD WINAPI
DsListSitesW(_In_ HANDLE hDs,                     // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsListServersInSiteA(_In_ HANDLE hDs,                       // in
                     _In_ LPCSTR site,                      // in
                     _Outptr_ PDS_NAME_RESULTA *ppServers); // out

_Check_return_ NTDSAPI DWORD WINAPI
DsListServersInSiteW(_In_ HANDLE hDs,                       // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsListDomainsInSiteA(_In_ HANDLE hDs,                       // in
                     _In_ LPCSTR site,                      // in
                     _Outptr_ PDS_NAME_RESULTA *ppDomains); // out

_Check_return_ NTDSAPI DWORD WINAPI
DsListDomainsInSiteW(_In_ HANDLE hDs,                       // in
                     _In_ LPCWSTR site,                     // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsListServersForDomainInSiteA(_In_ HANDLE hDs,                       // in
                              _In_ LPCSTR domain,                    // in
                              _In_ LPCSTR site,                      // in
                              _Outptr_ PDS_NAME_RESULTA *ppServers); // out

_Check_return_ NTDSAPI DWORD WINAPI
DsListServersForDomainInSiteW(_In_ HANDLE hDs,                       // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsListInfoForServerA(_In_ HANDLE hDs,                    // in
                     _In_ LPCSTR server,                 // in
                     _Outptr_ PDS_NAME_RESULTA *ppInfo); // out

_Check_return_ NTDSAPI DWORD WINAPI
DsListInfoForServerW(_In_ HANDLE hDs,                    // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsListRolesA(_In_ HANDLE hDs,                     // in
             _Outptr_ PDS_NAME_RESULTA *ppRoles); // out

_Check_return_ NTDSAPI DWORD WINAPI
DsListRolesW(_In_ HANDLE hDs,                     // in
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI DsQuerySitesByCostW(
    _In_ HANDLE hDS,                           // in
...
_Check_return_ NTDSAPI_POSTXP DWORD WINAPI DsQuerySitesByCostA(
    _In_ HANDLE hDS,                         // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsMapSchemaGuidsA(_In_ HANDLE hDs,                           // in
                  DWORD cGuids,                              // in
                  _In_reads_(cGuids) GUID *rGuids,           // in
                  _Outptr_ DS_SCHEMA_GUID_MAPA **ppGuidMap); // out

NTDSAPI
VOID WINAPI DsFreeSchemaGuidMapA(_In_ PDS_SCHEMA_GUID_MAPA pGuidMap); // in

_Check_return_ NTDSAPI DWORD WINAPI
DsMapSchemaGuidsW(_In_ HANDLE hDs,                           // in
                  DWORD cGuids,                              // in
                  _In_reads_(cGuids) GUID *rGuids,           // in
                  _Outptr_ DS_SCHEMA_GUID_MAPW **ppGuidMap); // out

NTDSAPI
VOID WINAPI DsFreeSchemaGuidMapW(_In_ PDS_SCHEMA_GUID_MAPW pGuidMap); // in
...
_Check_return_ _When_(InfoLevel == 1,
                      _At_(ppInfo,
                           _Outptr_result_bytebuffer_(
                               *pcOut * sizeof(DS_DOMAIN_CONTROLLER_INFO_1A))))
    _When_(InfoLevel == 2,
           _At_(ppInfo, _Outptr_result_bytebuffer_(
                            *pcOut * sizeof(DS_DOMAIN_CONTROLLER_INFO_2A))))
        _When_(InfoLevel == 3,
               _At_(ppInfo, _Outptr_result_bytebuffer_(
                                *pcOut * sizeof(DS_DOMAIN_CONTROLLER_INFO_3A))))
            NTDSAPI DWORD WINAPI
    DsGetDomainControllerInfoA(_In_ HANDLE hDs,         // in
...
_Check_return_ _When_(InfoLevel == 1,
                      _At_(ppInfo,
                           _Outptr_result_bytebuffer_(
                               *pcOut * sizeof(DS_DOMAIN_CONTROLLER_INFO_1W))))
    _When_(InfoLevel == 2,
           _At_(ppInfo, _Outptr_result_bytebuffer_(
                            *pcOut * sizeof(DS_DOMAIN_CONTROLLER_INFO_2W))))
        _When_(InfoLevel == 3,
               _At_(ppInfo, _Outptr_result_bytebuffer_(
                                *pcOut * sizeof(DS_DOMAIN_CONTROLLER_INFO_3W))))
            NTDSAPI DWORD WINAPI
    DsGetDomainControllerInfoW(_In_ HANDLE hDs,         // in
...
_Check_return_
    NTDSAPI DWORD WINAPI DsReplicaConsistencyCheck(_In_ HANDLE hDS,      // in
                                                   DS_KCC_TASKID TaskID, // in
                                                   DWORD dwFlags);       // in

_Check_return_ NTDSAPI DWORD WINAPI
DsReplicaVerifyObjectsW(_In_ HANDLE hDS,              // in
                        _In_ LPCWSTR NameContext,     // in
                        _In_ const UUID *pUuidDsaSrc, // in
                        ULONG ulOptions);             // in

_Check_return_ NTDSAPI DWORD WINAPI
DsReplicaVerifyObjectsA(_In_ HANDLE hDS,              // in
...
typedef enum _DS_REPL_INFO_TYPE {
  ...
  DS_REPL_INFO_METADATA_FOR_ATTR_VALUE =
      6,                               // returns DS_REPL_ATTR_VALUE_META_DATA *
  ...
  DS_REPL_INFO_METADATA_EXT_FOR_ATTR_VALUE =
      11, // returns DS_REPL_ATTR_VALUE_META_DATA_EXT *
...
_Check_return_ NTDSAPI DWORD WINAPI
DsReplicaGetInfoW(_In_ HANDLE hDS,                         // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsReplicaGetInfo2W(_In_ HANDLE hDS,                         // in
                   DS_REPL_INFO_TYPE InfoType,              // in
                   _In_opt_ LPCWSTR pszObject,              // in
                   _In_opt_ UUID *puuidForSourceDsaObjGuid, // in
...
_Check_return_ NTDSAPI DWORD WINAPI
DsAddSidHistoryW(_In_ HANDLE hDS,                      // in
                 ...
                 _In_opt_ RPC_AUTH_IDENTITY_HANDLE
                     SrcDomainCreds,         // in - creds for src domain
                 _In_ LPCWSTR DstDomain,     // in - DNS or NetBIOS
                 _In_ LPCWSTR DstPrincipal); // in - SAM account name

_Check_return_ NTDSAPI DWORD WINAPI
DsAddSidHistoryA(_In_ HANDLE hDS,                     // in
                 _Reserved_ DWORD Flags,              // in - sbz for now
...
_Check_return_ NTDSAPI DWORD WINAPI DsInheritSecurityIdentityW(
    _In_ HANDLE hDS,            // in
    _Reserved_ DWORD Flags,     // in - sbz for now
...
_Check_return_ NTDSAPI DWORD WINAPI
DsInheritSecurityIdentityA(_In_ HANDLE hDS,           // in
                           _Reserved_ DWORD Flags,    // in - sbz for now
...
// object.
#define FLAG_FOREST_OPTIONAL_FEATURE                                           \
  (0x00000001) // The optional feature is a forest level feature.
#define FLAG_DOMAIN_OPTIONAL_FEATURE                                           \
  (0x00000002) // The optional feature is a domain level feature.
#define FLAG_DISABLABLE_OPTIONAL_FEATURE                                       \
  (0x00000004) // The optional feature may be turned off.
#define FLAG_SERVER_OPTIONAL_FEATURE                                           \
  (0x00000008) // The optional feature is a server level feature..
...
#define GUID_RECYCLE_BIN_OPTIONAL_FEATURE_BYTE                                 \
  "\xd8\xdc\x6d\x76\xd0\xac\x5e\x44\xf3\xb9\xa7\xf9\xb6\x74\x4f\x2a"

#define GUID_PRIVILEGED_ACCESS_MANAGEMENT_OPTIONAL_FEATURE_A                   \
  "73e843ece8cc4046b4ab07ffe4ab5bcd"
#define GUID_PRIVILEGED_ACCESS_MANAGEMENT_OPTIONAL_FEATURE_W                   \
  L"73e843ece8cc4046b4ab07ffe4ab5bcd"
#define GUID_PRIVILEGED_ACCESS_MANAGEMENT_OPTIONAL_FEATURE_BYTE                \
  "\x73\xe8\x43\xec\xe8\xcc\x40\x46\xb4\xab\x07\xff\xe4\xab\x5b\xcd"

// GUID for Database 32k Pages Feature {52982AC6-1E73-754F-AE24-73AE2775AAB8}
#define GUID_DATABASE_32K_PAGES_OPTIONAL_FEATURE_A                             \
  "c62a9852731e4f75ae2473ae2775aab8"
#define GUID_DATABASE_32K_PAGES_OPTIONAL_FEATURE_W                             \
  L"c62a9852731e4f75ae2473ae2775aab8"
#define GUID_DATABASE_32K_PAGES_OPTIONAL_FEATURE_BYTE                          \
  "\xc6\x2a\x98\x52\x73\x1e\x4f\x75\xae\x24\x73\xae\x27\x75\xaa\xb8"
