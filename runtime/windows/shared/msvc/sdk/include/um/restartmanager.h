...
typedef struct _RM_FILTER_INFO {
  ...
  union {
    ...
    RM_UNIQUE_PROCESS
    Process; // When RmFilterTrigger == RmFilterTriggerProcess,
             // this contains the process this filter applies to
    LPWSTR strServiceShortName; // When RmFilterTrigger ==
                                // RmFilterTriggerService, this contians the
                                // service name this filter applies to
  };
...
DWORD WINAPI RmStartSession(_Out_ DWORD *pSessionHandle,
                            _Reserved_ DWORD dwSessionFlags,
                            _Out_writes_(CCH_RM_SESSION_KEY + 1)
                                WCHAR strSessionKey[]);
...
//        ERROR_OUTOFMEMORY
//            Could not allocate memory or instantiate class object.
DWORD WINAPI RmJoinSession(_Out_ DWORD *pSessionHandle,
                           _In_reads_(CCH_RM_SESSION_KEY + 1)
...
DWORD WINAPI RmEndSession(_In_ DWORD dwSessionHandle);
...
DWORD WINAPI RmRegisterResources(
    _In_ DWORD dwSessionHandle, _In_ UINT nFiles,
    _In_reads_opt_(nFiles) LPCWSTR rgsFileNames[], _In_ UINT nApplications,
    _In_reads_opt_(nApplications) RM_UNIQUE_PROCESS rgApplications[],
    _In_ UINT nServices, _In_reads_opt_(nServices) LPCWSTR rgsServiceNames[]);
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmGetList(_In_ DWORD dwSessionHandle, _Out_ UINT *pnProcInfoNeeded,
                       _Inout_ UINT *pnProcInfo,
                       _Inout_updates_opt_(*pnProcInfo)
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmShutdown(_In_ DWORD dwSessionHandle, _In_ ULONG lActionFlags,
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmRestart(_In_ DWORD dwSessionHandle,
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmCancelCurrentTask(_In_ DWORD dwSessionHandle);
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmAddFilter(_In_ DWORD dwSessionHandle,
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmRemoveFilter(_In_ DWORD dwSessionHandle,
                            _In_opt_ LPCWSTR strModuleName,
                            _In_opt_ RM_UNIQUE_PROCESS *pProcess,
                            _In_opt_ LPCWSTR strServiceShortName);
...
//        ERROR_INVALID_HANDLE
//            No session exists for the supplied handle.
DWORD WINAPI RmGetFilterList(_In_ DWORD dwSessionHandle,
                             _Out_writes_bytes_opt_(cbFilterBuf)
                                 PBYTE pbFilterBuf,
