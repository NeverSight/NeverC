...
/*
 * A connection to the remote computer could not be established, so the port
 * used for this connection was closed.%0
 */
...
/*
 * The connection was terminated by the remote computer before it could be
 * completed.%0
 */
...
/*
 * The modem (or other connecting device) was disconnected due to hardware
 * failure.%0
 */
...
/*
 * The modem (or other connecting device) is already in use or is not configured
 * properly.%0
 */
...
//
// One of your computer's NetBIOS names is already registered on the remote
// network.
//
// This error was obsoleted in the Windows Vista  release.
//
...
/*
 * There was an unrecognized response from the modem (or other connecting
 * device).%0
 */

#define ERROR_MACRO_NOT_FOUND (RASBASE + 53)
/*
 * A macro required by the modem (or other connecting device) was not found in
 * the device.INF file.%0
 */
...
/*
 * The <defaultoff> macro in the device.INF file section contains an undefined
 * macro.%0
 */
...
#if (WINVER < 0x600)
//
// The system was unable to read the maximum connection BPS rate from the
// media.INI file.
//
// This error was obsoleted in the Windows Vista  release.
//
...
//
// The system was unable to read the maximum carrier connection speed from the
// media.INI file.
//
// This error was obsoleted in the Windows Vista  release.
//
...
/*
 * The remote computer did not respond. To make sure that the server can be
 * reached, ping the remote computer.%0
 */
 ...
 * combination you provided is not recognized, or the selected authentication
 * protocol is not permitted on the remote access server.%0
 */
...
/*
 * The connection needs information from you, but the application does not allow
 * user interaction.%0
 */
...
/*
 * There was an error changing the password on the domain.  The password might
 * have been too short or might have matched a previously used password.%0
 */
...
/*
 * The operation could not finish because it could not start the Remote Access
 * Connection Manager service in time. Please try the operation again.%0
 */
...
/*
 * No IP addresses are available in the static pool of Remote Access Service IP
 * addresses.%0
 */

#define ERROR_PPP_TIMEOUT (RASBASE + 118)
/*
 * The connection was terminated because the remote computer did not respond in
 * a timely manner.%0
 */
...
/*
 * A connection to the remote computer could not be established.  You might need
 * to change the network settings for this connection.%0
 */

#define ERROR_PPP_NO_RESPONSE (RASBASE + 121)
/*
 * A connection to the remote access server was not made because the remote
 * access server did not respond.%0
 */
...
#if (WINVER < 0x600)
//
// The IPX protocol cannot dial out on the modem (or other connecting device)
// because this computer is not configured for dialing out (it is an IPX
// router).
//
// This error was obsoleted in the Windows Vista  release.
//
...
//
// The IPX protocol cannot dial in on the modem (or other connecting device)
// because this computer is not configured for dialing in (the IPX router is not
// installed).
//
// This error was obsoleted in the Windows Vista  release.
//
...
/*
 * The IPX protocol cannot be used for dialing out on more than one modem (or
 * other connecting device) at a time.%0
 */
...
/*
 * A connection to the remote computer could not be completed.  You might need
 * to adjust the protocols on this computer.%0
 */
...
/*
 * The remote connection was not made because the remote access server did not
 * assign an IP address.%0
 */

#define ERROR_CANNOT_USE_LOGON_CREDENTIALS (RASBASE + 139)
/*
 * The authentication protocol required by the remote server cannot use the
 * stored password.  Redial, entering the password explicitly.%0
 */
...
/*
 * The connection could not be disconnected because the user does not have the
 * required permission to disconnect.%0
 */
...
/*
 * Remote Access Services could not be started automatically.  Additional
 * information is provided in the event log.%0
 */
...
//
// An error occurred while the existing Internet Connection Sharing settings
// were being changed.
//
// This error was obsoleted in the Windows Vista  release.
//
 ...
 * over IPSec require the installation of a machine certificate, also known as a
 * computer certificate.%0
 */
 ...
 * as the private network has more than one IP address configured.  Please
...
/*
 * The call could not be connected because the remote computer has invoked the
 * Do Not Disturb feature.%0
 */

#define ERROR_OUTOFORDER (RASBASE + 177)
/*
 * The connection attempt failed because the modem (or other connecting device)
 * on the remote computer is out of order.%0
 */
...
// For further assistance, click More Info or search Help and Support Center for
// this error number.
//
// This error was obsoleted in the Windows Vista  release.
//
 ...
 * cannot be enabled because Routing and Remote Access has been enabled on this
 * computer.  To enable ICS or ICF, first disable Routing and Remote Access. For
 * more information about Routing and Remote Access, ICS, or ICF, see Help and
 * Support.%0
 */
...
/*
 * Internet Connection Sharing cannot be enabled.  The LAN connection selected
 * as the private network is either not present, or is disconnected from the
...
/*
 * You cannot dial using this connection at logon time, because it is configured
 * to use a user name different than the one on the smart card.  If you want to
 * use it at logon time, you must configure it to use the user name on the smart
 * card.%0
 */

#define ERROR_NO_REG_CERT_AT_LOGON (RASBASE + 185)
/*
 * You cannot dial using this connection at logon time, because it is not
 * configured to use a smart card.  If you want to use it at logon time, you
 * must edit the properties of this connection so that it uses a smart card.%0
 */

#define ERROR_OAKLEY_NO_CERT (RASBASE + 186)
/*
 * The connection attempt failed because there is no valid machine certificate
 * on your computer for security authentication.%0
 */

#define ERROR_OAKLEY_AUTH_FAIL (RASBASE + 187)
/*
 * The L2TP connection attempt failed because the security layer could not
 * authenticate the remote computer.%0
 */

#define ERROR_OAKLEY_ATTRIB_FAIL (RASBASE + 188)
/*
 * The L2TP connection attempt failed because the security layer could not
 * negotiate compatible parameters with the remote computer.%0
 */
 ...
 * processing error during initial negotiations with the remote computer.%0
 */

#define ERROR_OAKLEY_NO_PEER_CERT (RASBASE + 190)
/*
 * The L2TP connection attempt failed because certificate validation of the
 * remote computer failed.%0
 */

#define ERROR_OAKLEY_NO_POLICY (RASBASE + 191)
/*
 * The L2TP connection attempt failed because security policy for the connection
 * was not found.%0
 */
...
/*
 * The L2TP connection attempt failed because an error occurred while
 * negotiating security.%0
 */
...
/*
 * The remote connection request was denied because the VPN tunnel type being
 * used is not allowed.%0
 */

#define ERROR_UNKNOWN_SERVICE_TYPE (RASBASE + 196)
/*
 * The Service Type RADIUS attribute for this user is neither Framed nor
 * Callback Framed.%0
 */

#define ERROR_CONNECTING_DEVICE_NOT_FOUND (RASBASE + 197)
/*
 * A connection to the remote access server was not made because the modem was
 * not found.%0
 */
...
/*
 * Internet Connection Sharing (ICS) cannot be enabled due to an IP address
 * conflict on the network.  ICS requires the host be configured to use
 * 192.168.137.1.  Please ensure that no other client on the network is
 * configured to use 192.168.137.1.%0
 */

#define ERROR_AUTOMATIC_VPN_FAILED (RASBASE + 200)
/*
 * The remote connection was not made because the attempted VPN tunnels failed.
 * The VPN server might be unreachable. If this connection is attempting to use
 * an L2TP/IPsec tunnel, the security parameters required for IPsec negotiation
 * might not be configured properly.%0
 */
 ...
 * but Windows cannot verify the digital certificate sent by the server.%0
 */

#define ERROR_READING_SCARD (RASBASE + 202)
/*
 * The card supplied was not recognized.  Please check that the card is inserted
 * correctly, and fits tightly.%0
 */

#define ERROR_INVALID_PEAP_COOKIE_CONFIG (RASBASE + 203)
/*
 * The PEAP configuration stored in the session cookie does not match the
 * current session configuration.%0
 */

#define ERROR_INVALID_PEAP_COOKIE_USER (RASBASE + 204)
/*
 * The PEAP identity stored in the session cookie does not match the current
 * identity.%0
 */
...
/*
 * You cannot dial using this connection at logon time, because it is configured
 * to use logged on user's credentials.%0
 */
 ...
 * administrator or Internet Service Provider.%0
 */

#define ERROR_VPN_DISCONNECT (RASBASE + 207)
/*
 * The network connection between your computer and the VPN server was
 * interrupted.  This can be caused by a problem in the VPN transmission and is
 * commonly the result of internet latency or simply that your VPN server has
 * reached capacity.  Please try to reconnect to the VPN server.  If this
 * problem persists, contact the VPN administrator and analyze quality of
 ...
 * established because the remote server refused the connection. This is
 * typically caused by a mismatch between the server's configuration and your
 * connection settings. Please contact the remote server's Administrator to
 * verify the server configuration and your connection settings.%0
 */

#define ERROR_VPN_TIMEOUT (RASBASE + 209)
/*
 * The network connection between your computer and the VPN server could not be
 * established because the remote server is not responding. This could be
 * because one of the network devices (e.g, firewalls, NAT, routers, etc)
 * between your computer and the remote server is not configured to allow VPN
 * connections. Please contact your Administrator or your service provider to
 * determine which device may be causing the problem.%0
 */

#define ERROR_VPN_BAD_CERT (RASBASE + 210)
/*
 * A network connection between your computer and the VPN server was started,
 * but the VPN connection was not completed. This is typically caused by the use
 * of an incorrect or expired certificate for authentication between the client
 * and the server. Please contact your Administrator to ensure that the
 * certificate being used for authentication is valid.%0
 */

#define ERROR_VPN_BAD_PSK (RASBASE + 211)
/*
 * The network conection between your computer and the VPN server could not be
 * established because the remote server is not responding.  This is typically
 * caused by a pre-shared key problem between the client and server.  A
 * pre-shared key is used to guarantee you are who you say you are in an IP
 * Security (IPSec) communication cycle.  Please get the assistance of your
 * administrator to determine where the pre-shared key problem is originating.%0
 */

#define ERROR_SERVER_POLICY (RASBASE + 212)
/*
 *The connection was prevented because of a policy configured on your RAS/VPN
 * server. Specifically, the authentication method used by the server to verify
 * your username and password may not match the authentication method configured
 * in your connection profile. Please contact the Administrator of the RAS
 * server and notify them of this error.%0
 */

#define ERROR_BROADBAND_ACTIVE (RASBASE + 213)
/*
 * You have attempted to establish a second broadband connection while a
 * previous broadband connection is already established using the same device or
 * port. Please disconnect the earlier connection and then reestablish the
 * connection.%0
 */

#define ERROR_BROADBAND_NO_NIC (RASBASE + 214)
/*
 * The underlying Ethernet connectivity required for the broadband connection
 * was not found. Please install and enable the Ethernet adapter on your
 * computer via the Network Connections folder before attempting this
 * connection.%0
 */

#define ERROR_BROADBAND_TIMEOUT (RASBASE + 215)
/*
 * The broadband network conection could not be established on your computer
 * because the remote server is not responding. This could be caused by an
 * invalid value for the 'Service Name' field for this connection. Please
 * contact your Internet Service Provider and inquire about the correct value
 * for this field and update it in the Connection Properties.%0
 */

#define ERROR_FEATURE_DEPRECATED (RASBASE + 216)
/*
 * A feature or setting you have tried to enable is no longer supported by the
...
/*
 * The Network Access Protection (NAP) enforcement client could not create
 * system resources for remote access connections. Some network services or
 * resources might not be available. If the problem persists, disconnect and
 * retry the remote access connection or contact the administrator for the
 * remote access server.
 */

#define ERROR_RASQEC_NAPAGENT_NOT_ENABLED (RASBASE + 219)
/*
 * The Network Access Protection Agent (NAPAgent) service has been disabled or
 * is not installed on this computer. Some network services or resources might
 * not be available. If the problem persists, disconnect and retry the remote
 * access connection or contact the administrator for the remote access server.
 */

#define ERROR_RASQEC_NAPAGENT_NOT_CONNECTED (RASBASE + 220)
/*
 * The Network Access Protection (NAP) enforcement client failed to register
 * with the Network Access Protection Agent (NAPAgent) service. Some network
 * services or resources might not be available. If the problem persists,
 * disconnect and retry the remote access connection or contact the
 * administrator for the remote access server.
 */

#define ERROR_RASQEC_CONN_DOESNOTEXIST (RASBASE + 221)
/*
 * The Network Access Protection (NAP) enforcement client was unable to process
 * the request because the remote access connection does not exist. Retry the
 * remote access connection. If the problem persists, make sure that you can
 * connect to the Internet, and then contact the administrator for the remote
 * access server.
 */

#define ERROR_RASQEC_TIMEOUT (RASBASE + 222)
/*
 * The Network Access Protection (NAP) enforcement client did not respond. Some
 * network services or resources might not be available. If the problem
 * persists, disconnect and retry the remote access connection or contact the
 * administrator for the remote access server.
 */
...
/*
 * The remote connection was not made because Point-to-Point Tunneling Protocol
 * (PPTP) is incompatible with IPv6. Use any other tunneling protocol. %0
 */
...
/*
 * The VPN connection cannot be completed because the 'IKE and AuthIP IPSec
 * Keying Modules' service and/or the 'Base Filtering Engine' service is not
 * running. These services are required to establish the connection. Please
 * ensure that these services have been started before dialing the connection.
 */
...
/*
 * The modem (or other connecting device) was disconnected due to link
 * failure.%0
 */
...
/*
 * The connection was terminated because Remote Access Connection manager
 * stopped.%0
 */
...
/*
 * The L2TP connection attempt failed because the security layer could not
 * authenticate the remote computer. This could be because one or more fields of
 * the certificate presented by the remote server could not be validated as
 * belonging to the target destination.
 */

#define ERROR_NOT_NAP_CAPABLE (RASBASE + 236)
/*
 * The Network Access Protection (NAP) health state of the computer cannot be
 * determined. Contact your administrator to verify that the NAP enforcement
 * client is enabled, the NAP Agent service is running, and NAP is enforced in
 * the Protected Extensible Authentication Protocol (PEAP) properties of the
...
/*
 * Another Update connection request is in progress. RAS allows only one Update
 * Connection request at a time.%0
 */

#define ERROR_PROTOCOL_ENGINE_DISABLED (RASBASE + 239)
/*
 * Negotiating using configured protocol is disable. Edit connection properties
 * and select different protocol for negotiation and try again.%0
 */
...
/*
 * The VPN Connection is still connecting or reauthenticating because of
 * Quarantine state change. Initiate mobike update only when connection state is
 * 'Connected'.
 */

#define ERROR_PEAP_SERVER_REJECTED_CLIENT_TLV (RASBASE + 245)
/*
 * Server rejected client authentication, due unexpected TLV or value mismatch
 * for a TLV.
 */

#define ERROR_INVALID_PREFERENCES (RASBASE + 246)
/*
 * Either VPN Destination preference is not selected by the user or it is no
 * longer valid.
 */
...
/*
 * VPN connection attempt failed due to internal error occurred while adding
 * cookies to the Secure Socket Tunneling Protocol (SSTP). Please see the System
 * Event Log for the detailed information.
 */
...
/*
 * The Extensible Authentication Protocol type required for authentication of
 * the remote access connection is not installed on your computer.
 */

#define ERROR_EAP_METHOD_DOES_NOT_SUPPORT_SSO (RASBASE + 251)
/*
 * The Extensible Authentication Protocol type configured on the remote access
 * connection does not support single sign-on.
 */

#define ERROR_EAP_METHOD_OPERATION_NOT_SUPPORTED (RASBASE + 252)
/*
 * The Extensible Authentication Protocol type configured on the remote access
 * connection does not support the requested operation.
 */

#define ERROR_EAP_USER_CERT_INVALID (RASBASE + 253)
/*
 * The remote access connection completed, but authentication failed because the
 * certificate that authenticates the client to the server is not valid. Ensure
...
/*
 * The remote access connection completed, but authentication failed because the
 * certificate that authenticates the client to the server is expired. Renew the
 * certificate.
 */
...
/*
 * The remote access connection completed, but authentication failed because the
 * certificate that authenticates the client to the server is revoked.
 */

#define ERROR_EAP_USER_CERT_OTHER_ERROR (RASBASE + 256)
/*
 * The remote access connection completed, but authentication failed because of
 * an error in the certificate that authenticates the client to the server.
 */

#define ERROR_EAP_SERVER_CERT_INVALID (RASBASE + 257)
/*
 * The remote access connection completed, but authentication failed because the
 * certificate that the client uses to authenticate the server is not valid.
 */

#define ERROR_EAP_SERVER_CERT_EXPIRED (RASBASE + 258)
/*
 * The remote access connection completed, but authentication failed because the
 * certificate that the client uses to authenticate the server is expired.
 */

#define ERROR_EAP_SERVER_CERT_REVOKED (RASBASE + 259)
/*
 * The remote access connection completed, but authentication failed because the
 * certificate that the client uses to authenticate the server is revoked.
 */

#define ERROR_EAP_SERVER_CERT_OTHER_ERROR (RASBASE + 260)
/*
 * The remote access connection completed, but authentication failed because of
 * an error in the certificate that the client uses to authenticate the server.
 */

#define ERROR_EAP_USER_ROOT_CERT_NOT_FOUND (RASBASE + 261)
/*
 * The remote access connection completed, but authentication failed because a
 * trusted root certificate that validates the user certificate was not found in
...
/*
 * The remote access connection completed, but authentication failed because the
 * trusted root certificate that is used to validate the user certificate is not
 * valid.
 */
...
/*
 * The remote access connection completed, but authentication failed because the
 * certificate in the Trusted Root Certification Authorities certificate store
 * that authenticates the user certificate is expired. Renew the certificate.
 */

#define ERROR_EAP_SERVER_ROOT_CERT_NOT_FOUND (RASBASE + 264)
/*
 * The remote access connection completed, but authentication failed because a
 * certificate that validates the server certificate was not found in the
 * Trusted Root Certification Authorities certificate store.
 */

#define ERROR_EAP_SERVER_ROOT_CERT_INVALID (RASBASE + 265)
/*
 * The remote zccess connection completed, but authentication failed because the
 * certificate in the Trusted Root Certification Authorities certificate store
 * that validates the server certificate is not valid.
 */

#define ERROR_EAP_SERVER_ROOT_CERT_NAME_REQUIRED (RASBASE + 266)
/*
 * The remote access connection completed, but authentication failed because the
 * certificate on the server computer does not have a server name specified.
 */

#define ERROR_PEAP_IDENTITY_MISMATCH (RASBASE + 267)
/*
 *    The PEAP outer identity is not same as the inner identity when identity
 * privacy is turned OFF.
 */

#define ERROR_DNSNAME_NOT_RESOLVABLE (RASBASE + 268)
/*
 *    The remote connection was not made because the name of the remote access
...
/*
 * The Interface  could not be enabled as more than one interface with same
 * destination has been created with authentication method as Pre Shared Key.
 * Change the destination/auth method and enable the interface.
 */

#define ERROR_INVALID_DESTINATION_IP (RASBASE + 271)
/*
 * The Interface could not be enabled as no valid destination is found for it.
 * Ensure that a valid/resolvable destination IP is configured and try again.
 */
...
/*
 * Couldn't connect because we couldn't find a certificate for single sign-on.
...
/*
 * The app %1!ws! is required for this VPN connection. Please download it in the
 * Microsoft Store or contact your IT administrators.
 */
...
/*
 * Couldn't connect because the call to the WinHttp Auto Proxy Service failed.
 * This is usually the result of Web Proxy Auto-Discovery (WPAD) being disabled.
 * Please enable it to allow the VPN connection to proceed, or contact your
