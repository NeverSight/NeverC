...
#pragma warning(                                                               \
    disable                                                                    \
    : 4200) // warning about zero-sized arrays being non-stadard C extension
...
DEFINE_GUID(CLSID_Sti, 0xB323F8E0L, 0x2E68, 0x11D0, 0x90, 0xEA, 0x00, 0xAA,
            0x00, 0x60, 0xF8, 0x6C);
...
DEFINE_GUID(IID_IStillImageW, 0x641BD880L, 0x2DC8, 0x11D0, 0x90, 0xEA, 0x00,
            0xAA, 0x00, 0x60, 0xF8, 0x6C);

// {6CFA5A80-2DC8-11D0-90EA-00AA0060F86C}
DEFINE_GUID(IID_IStiDevice, 0x6CFA5A80L, 0x2DC8, 0x11D0, 0x90, 0xEA, 0x00, 0xAA,
            0x00, 0x60, 0xF8, 0x6C);
...
//
//  Following information is used for enumerating still image devices ,
//  currently configured in the system. Presence of the device in the enumerated
//  list does not mean availability
...
//
// Generic capabilities mask contain 16 bits , common for all devices,
// maintained by MS and 16 bits , which USD can use for proprietary capbailities
// reporting.
//
...
// If this capability is set, still image service will generate event when
// device instance is successfully initialized ( typically in response to PnP
// arrival)
...
//
// Device information structure, this is not configurable. This data is returned
// from device enumeration API and is used for populating UI or selecting which
// device should be used in current session
//
typedef struct _STI_DEVICE_INFORMATIONW {
  ...
  // Local specific ("friendly") name of the device, mainly used for showing in
  // the UI
  LPWSTR pszLocalName;
...
// ------------------------
//
// Following types  are used to inquire state characteristics of the device
...
//
// Flags passed to WriteToErrorLog call in a first parameter, indicating type of
// the message which needs to be logged
//
...
// ------------------------------
//
// Those are used to inform last subscribed caller of the changes in device
// state, initiated by device.
//
// The only supported discipline of notification is stack. The last caller to
// subscribe will be notified and will receive notification data. After caller
// unsubscribes , the previously subscribed caller will become active.
//

// Notifications are sent to subscriber via window message. Window handle is
// passed as parameter
...
typedef struct _STISUBSCRIBE {
  ...
  // Not used . Will be used for subscriber to set bit mask filtering different
  // events
  DWORD dwFilter;
  ...
  // Handle of Win32 auto-reset event , which will be signalled whenever device
  // has notification pending
  HANDLE hEvent;
...
//
// When STI Device is being added or removed, PnP broadacst is being sent , but
// it is not obvious for application code to recognize if it is STI device and
// if so, what is the name of the device. STI subsystem will analyze PnP
// broadcasts and rebroadcast another message via BroadcastSystemMessage /
// WM_DEVICECHANGE / DBT_USERDEFINED .

// String passed as user defined message contains STI prefix, action and device
// name
...
//
// Bit mask for legitimate mode bits, which can be used when calling
// CreateDevice
//
...
// DLLEXP STDMETHODIMP StiCreateInstance(HINSTANCE hinst, DWORD dwVer, PSTI
// *ppSti, LPUNKNOWN punkOuter);
...
DECLARE_INTERFACE_(IStillImageW, IUnknown) {
  /*** IUnknown methods ***/
  STDMETHOD(QueryInterface)(THIS_ _In_ REFIID riid, _Outptr_ LPVOID * ppvObj)
      PURE;
  ...
  STDMETHOD(GetDeviceList)(THIS_ DWORD dwType, DWORD dwFlags,
                           _Out_ DWORD * pdwItemsReturned,
                           _Out_ LPVOID * ppBuffer) PURE;
  STDMETHOD(GetDeviceInfo)(THIS_ _In_ LPWSTR pwszDeviceName,
                           _Out_ LPVOID * ppBuffer) PURE;

  STDMETHOD(CreateDevice)(THIS_ _In_ LPWSTR pwszDeviceName, DWORD dwMode,
                          _Out_ PSTIDEVICE * pDevice,
                          _In_opt_ LPUNKNOWN punkOuter) PURE;

  //
  // Device instance values. Used to associate various data with device.
  //
  STDMETHOD(GetDeviceValue)(THIS_ _In_ LPWSTR pwszDeviceName,
                            _In_ LPWSTR pValueName, _Out_ LPDWORD pType,
                            _Out_writes_bytes_(*cbData) LPBYTE pData,
                            _Inout_ LPDWORD cbData);
  STDMETHOD(SetDeviceValue)(
      THIS_ _In_ LPWSTR pwszDeviceName, _In_ LPWSTR pValueName, DWORD Type,
      _In_reads_bytes_(cbData) LPBYTE pData, DWORD cbData);

  //
  // For appllication started through push model launch, returns associated
  // information
  //
  STDMETHOD(GetSTILaunchInformation)(
      THIS_ _Out_writes_(STI_MAX_INTERNAL_NAME_LENGTH) LPWSTR pwszDeviceName,
      _Out_opt_ DWORD * pdwEventCode,
      _Out_writes_(STI_MAX_INTERNAL_NAME_LENGTH) LPWSTR pwszEventName) PURE;
  STDMETHOD(RegisterLaunchApplication)(THIS_ _In_ LPWSTR pwszAppName,
                                       _In_ LPWSTR pwszCommandLine) PURE;
  ...
  // state of monitor polling, for true notification devices means
  // enabling/disabling notification flow from monitor to registered
  // applications
  //
  STDMETHOD(EnableHwNotifications)(THIS_ _In_ LPCWSTR pwszDeviceName,
                                   BOOL bNewState) PURE;
  STDMETHOD(GetHwNotificationState)(THIS_ _In_ LPCWSTR pwszDeviceName,
                                    _Out_ BOOL * pbCurrentState) PURE;

  //
  // When device is installed but not accessible, application may request bus
  // refresh which in some cases will make device known. This is mainly used for
  // nonPnP buses like SCSI, when device was powered on after PnP enumeration
  //
  //
  ...
  // style components, which intercept device event , analyze and later force
  // launch based on certain criteria.
  //
  STDMETHOD(LaunchApplicationForDevice)(THIS_ _In_ LPWSTR pwszDeviceName,
                                        _In_ LPWSTR pwszAppName,
                                        _In_ LPSTINOTIFY pStiNotify);

  //
  // For non-PnP devices with non-known bus type connection, setup extension,
  // associated with the device can set it's parameters
  //
  ...
  //
  STDMETHOD(WriteToErrorLog)(THIS_ DWORD dwMessageType, _In_ LPCWSTR pszMessage)
      PURE;
...
DECLARE_INTERFACE_(IStiDevice, IUnknown) {
  ...
  /*** IStiDevice methods ***/
  STDMETHOD(Initialize)(THIS_ _In_ HINSTANCE hinst, _In_ LPCWSTR pwszDeviceName,
  ...
  STDMETHOD(Escape)(
      THIS_ STI_RAW_CONTROL_CODE EscapeFunction,
      _In_reads_bytes_(cbInDataSize) LPVOID lpInData, DWORD cbInDataSize,
      _Inout_updates_bytes_to_(dwOutDataSize, *pdwActualData) LPVOID pOutData,
      DWORD dwOutDataSize, _Out_ LPDWORD pdwActualData) PURE;
  ...
  STDMETHOD(RawReadData)(THIS_ _Inout_updates_bytes_(*lpdwNumberOfBytes)
                             LPVOID lpBuffer,
                         _Inout_ LPDWORD lpdwNumberOfBytes,
                         _In_opt_ LPOVERLAPPED lpOverlapped) PURE;
  STDMETHOD(RawWriteData)(
      THIS_ _In_reads_bytes_(nNumberOfBytes) LPVOID lpBuffer,
      DWORD nNumberOfBytes, _In_opt_ LPOVERLAPPED lpOverlapped) PURE;

  STDMETHOD(RawReadCommand)(THIS_ _Inout_updates_bytes_(*lpdwNumberOfBytes)
                                LPVOID lpBuffer,
                            _Inout_ LPDWORD lpdwNumberOfBytes,
                            _In_opt_ LPOVERLAPPED lpOverlapped) PURE;
  ...
  //
  // Subscription is used to enable "control center" style applications , where
  // flow of notifications should be redirected from monitor itself to another
  // "launcher"
  //
