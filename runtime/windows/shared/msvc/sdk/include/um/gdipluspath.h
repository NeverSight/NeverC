...
public:
  ...
  INT NextSubpath(OUT const GraphicsPath *path, OUT BOOL *isClosed) {
    GpPath *nativePath = NULL;
  ...
  INT NextPathType(_Out_ BYTE *pathType, _Out_ INT *startIndex,
                   _Out_ INT *endIndex) {
    INT resultCount;
  ...
  INT Enumerate(_Out_writes_to_(count, return) PointF *points,
                _Out_writes_to_(count, return) BYTE *types, _In_ INT count) {
    INT resultCount;
  ...
  INT CopyData(_Out_writes_to_(endIndex - startIndex + 1, return)
                   PointF *points,
               _Out_writes_to_(endIndex - startIndex + 1, return) BYTE *types,
               _In_ INT startIndex, _In_ INT endIndex) {
    INT resultCount;
...
#pragma warning(suppress                                                       \
                : 6385) // Tool bug, doesn't see that *numFound > numSought
