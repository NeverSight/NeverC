...
#define IPF_SAL_RECORD_REFERENCE_SECTION_GUID                                  \
...
typedef struct _WHEA_ERROR_RECORD_HEADER {
  ...
  _Field_range_(>=,
                (sizeof(WHEA_ERROR_RECORD_HEADER) +
                 (SectionCount *
                  sizeof(WHEA_ERROR_RECORD_SECTION_DESCRIPTOR)))) ULONG Length;
...
CPER_FIELD_CHECK(WHEA_PROCESSOR_GENERIC_ERROR_SECTION, InstructionPointer, 184,
                 8);
...
typedef struct WHEA_XPF_PROCESSOR_ERROR_SECTION WHEA_XPF_PROCESSOR_ERROR,
    *PWHEA_XPF_PROCESSOR_ERROR;
...
CPER_FIELD_CHECK(WHEA_XPF_PROCESSOR_ERROR_SECTION, VariableInfo, 64,
                 ANYSIZE_ARRAY);
...
#define WHEA_PMEM_IS_PFN_ALREADY_MARKED_BAD(PageRange, TargetPfn)              \
  (((TargetPfn) - ((PageRange)->StartingPfn) < sizeof(ULONG64) * 8) &&         \
   ((((PageRange)->MarkedBadBitmap) &                                          \
     (1ull << ((TargetPfn) - ((PageRange)->StartingPfn)))) != 0))

#define WHEA_PMEM_IS_PAGE_RANGE_ALREADY_MARKED_BAD(PageRange)                  \
  (((PageRange)->PageCount <= sizeof(ULONG64) * 8) &&                          \
   (((PageRange)->MarkedBadBitmap) ==                                          \
    (ULONG64_MAX >> (sizeof(ULONG64) * 8 - (PageRange)->PageCount))))
...
typedef WHEA_PCIEXPRESS_ERROR_SECTION_VALIDBITS WHEA_PCIEXPRESS_ERROR_VALIDBITS,
...
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_SECTION, ErrorInformationStructures,
                 4, 2);
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_SECTION, ContextInformationStructures,
                 6, 2);
...
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_INFORMATION, VirtualFaultAddress, 16,
                 8);
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_INFORMATION, PhysicalFaultAddress, 24,
                 8);
...
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_CONTEXT_INFORMATION_HEADER, Version,
                 0, 2);
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_CONTEXT_INFORMATION_HEADER,
                 RegisterContextType, 2, 2);
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_CONTEXT_INFORMATION_HEADER,
                 RegisterArraySize, 4, 4);
CPER_FIELD_CHECK(WHEA_ARM_PROCESSOR_ERROR_CONTEXT_INFORMATION_HEADER,
                 RegisterArray, 8, 1);
...
//
// To make ensure backwards compatability in the future the number of node
// field's is recorded. This allows expanding the section on new machines
// without breaking parsing for older generations.
//
