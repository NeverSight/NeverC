 ...
 *   The object is already associated with an ink object and cannot be
 * reassociated.
 */
 ...
 *   The interface pointer points to an object that is incompatible with the Ink
 * API
 */
 ...
 *   "The window input rectangle overlaps with an enabled InkCollector's window
 * input rectangle."
 */
...
// IErrorInfo helper for objects that support error info (CLSID_IFoo &&
// IID_IFoo)
#define MAKE_OBJ_ERROR_INFO(ID, hr, helpid, helpfile)                          \
  AtlReportError(CLSID_##ID, IDS_##hr, helpid, helpfile, IID_I##ID, hr,        \
                 _Module.GetModuleInstance())

// IErrorInfo helper for interfaces that support error info, but are not
// cocreatable
//      (e.g. IID_IFoo, but NOT CLSID_IFoo)
#define MAKE_INT_ERROR_INFO(ID, hr, helpid, helpfile)                          \
  AtlReportError(GUID_NULL, IDS_##hr, helpid, helpfile, IID_I##ID, hr,         \
                 _Module.GetModuleInstance())

#define TPC_E_INSUFFICIENT_BUFFER                                              \
