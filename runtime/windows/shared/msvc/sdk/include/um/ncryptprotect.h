...
// SID=%SidString%
//
// %SidString% is a SID string that identifies the object's group or principal
// identity
//
...
//                    - user will not be able to unprotect after logoff or
//                    reboot;
// Local=user         : protects to the user on local machine,
//                    - only this caller on the local machine will be able to
//                    unprotect;
// Local=machine      : protects to Local Machine,
//                    - all users on the local machine will be able to
//                    unprotect;
//
...
SECURITY_STATUS
WINAPI
NCryptRegisterProtectionDescriptorName(_In_ LPCWSTR pwszName,
...
/****************************************************************************
 ...
 pwszDescriptorString
    [out] Specifies a buffer that receive a Unicode string of Protection
Descriptor.
...
SECURITY_STATUS
WINAPI
NCryptQueryProtectionDescriptorName(_In_ LPCWSTR pwszName,
                                    _Out_writes_to_opt_(*pcDescriptorString,
                                                        *pcDescriptorString)
...
SECURITY_STATUS
WINAPI
NCryptCreateProtectionDescriptor(_In_ LPCWSTR pwszDescriptorString,
                                 DWORD dwFlags,
...
SECURITY_STATUS
WINAPI
NCryptCloseProtectionDescriptor(_In_ NCRYPT_DESCRIPTOR_HANDLE hDescriptor);
...
/****************************************************************************
 ...
 dwInfoType
    ...
    The type of data to be retrieved determines the type of structure to use for
*ppvInfo.

 ppvInfo
    [out] A pointer to a buffer that receives the data retrieved.
    The form of this data will vary depending on the value of the dwInfoType
parameter.
...
_Success_(return == 0)
    _When_(dwInfoType == NCRYPT_PROTECTION_INFO_TYPE_DESCRIPTOR_STRING,
           _At_((LPWSTR *)ppvInfo,
                _Outptr_result_nullonfailure_)) SECURITY_STATUS WINAPI
    NCryptGetProtectionDescriptorInfo(
        _In_ NCRYPT_DESCRIPTOR_HANDLE hDescriptor,
        _In_opt_ const NCRYPT_ALLOC_PARA *pMemPara, DWORD dwInfoType,
...
/****************************************************************************
 NCryptProtectSecret
  ...
  For large data protection, applications should use NCryptProtectMessage
function.
...
SECURITY_STATUS
WINAPI
NCryptProtectSecret(_In_ NCRYPT_DESCRIPTOR_HANDLE hDescriptor, DWORD dwFlags,
                    _In_reads_bytes_(cbData) const BYTE *pbData, ULONG cbData,
...
WINAPI
NCryptUnprotectSecret(
    _Out_opt_ NCRYPT_DESCRIPTOR_HANDLE *phDescriptor, _In_ DWORD dwFlags,
    _In_reads_bytes_(cbProtectedBlob) const BYTE *pbProtectedBlob,
    ULONG cbProtectedBlob, _In_opt_ const NCRYPT_ALLOC_PARA *pMemPara,
    _In_opt_ HWND hWnd, _Outptr_result_bytebuffer_(*pcbData) BYTE **ppbData,
    _Out_ ULONG *pcbData);
...
/****************************************************************************
  PFNCryptStreamOutputCallback
    ...
    pbData
        A pointer to a block of processed data that is available to the
application.
...
typedef SECURITY_STATUS(WINAPI *PFNCryptStreamOutputCallback)(
    _In_ void *pvCallbackCtxt, _In_reads_bytes_(cbData) const BYTE *pbData,
...
SECURITY_STATUS
WINAPI
NCryptStreamOpenToProtect(_In_ NCRYPT_DESCRIPTOR_HANDLE hDescriptor,
                          DWORD dwFlags, _In_opt_ HWND hWnd,
...
SECURITY_STATUS
WINAPI
NCryptStreamOpenToUnprotect(_In_ NCRYPT_PROTECT_STREAM_INFO *pStreamInfo,
                            DWORD dwFlags, _In_opt_ HWND hWnd,
                            _Out_ NCRYPT_STREAM_HANDLE *phStream);
...
If data is not available yet, but the descriptor is, this will be NULL.
...
****************************************************************************/
typedef SECURITY_STATUS(WINAPI *PFNCryptStreamOutputCallbackEx)(
    _In_ void *pvCallbackCtxt, _In_reads_bytes_opt_(cbData) const BYTE *pbData,
    SIZE_T cbData, _In_ NCRYPT_DESCRIPTOR_HANDLE hDescriptor, BOOL fFinal);

/****************************************************************************
 NCRYPT_PROTECT_STREAM_INFO_EX

    The NCRYPT_PROTECT_STREAM_INFO_EX structure is used to enable stream
processing of data rather than single block processing. This structure is passed
to the NCryptStreamOpenToUnprotectEx function.  There is not equivalent
NCryptStreamOpenToProtectEx function, thus you need to use the
PFNCryptStreamOutputCallback, NCRYPT_PROTECT_STREAM_INFO and
...
/****************************************************************************
 NCryptStreamOpenToUnprotectEx
    ...
    hWnd
        [in, optional] A window handle (HWND) to be used as the parent of any
user interface that is displayed.
...
function. Possible return codes include, but are not limited to, the following.
...
SECURITY_STATUS
WINAPI
NCryptStreamOpenToUnprotectEx(_In_ NCRYPT_PROTECT_STREAM_INFO_EX *pStreamInfo,
                              DWORD dwFlags, _In_opt_ HWND hWnd,
                              _Out_ NCRYPT_STREAM_HANDLE *phStream);
...
SECURITY_STATUS
WINAPI
NCryptStreamUpdate(_In_ NCRYPT_STREAM_HANDLE hStream,
                   _In_reads_bytes_(cbData) const BYTE *pbData, SIZE_T cbData,
                   BOOL fFinal);
...
SECURITY_STATUS
WINAPI
NCryptStreamClose(_In_ NCRYPT_STREAM_HANDLE hStream);
