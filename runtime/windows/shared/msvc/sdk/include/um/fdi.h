...
typedef INT_PTR(FAR DIAMONDAPI *PFNOPEN)(_In_ LPSTR pszFile, int oflag,
                                         int pmode);
typedef UINT(FAR DIAMONDAPI *PFNREAD)(_In_ INT_PTR hf,
                                      _Out_writes_bytes_(cb) void FAR *pv,
                                      UINT cb);
typedef UINT(FAR DIAMONDAPI *PFNWRITE)(_In_ INT_PTR hf,
                                       _In_reads_bytes_(cb) void FAR *pv,
                                       UINT cb);
...
#define FNREAD(fn)                                                             \
  UINT FAR DIAMONDAPI fn(_In_ INT_PTR hf, _Out_writes_bytes_(cb) void FAR *pv, \
                         UINT cb)
#define FNWRITE(fn)                                                            \
  UINT FAR DIAMONDAPI fn(_In_ INT_PTR hf, _In_reads_bytes_(cb) void FAR *pv,   \
                         UINT cb)
#define FNCLOSE(fn) int FAR DIAMONDAPI fn(_In_ INT_PTR hf)
#define FNSEEK(fn)                                                             \
  long FAR DIAMONDAPI fn(_In_ INT_PTR hf, long dist, int seektype)
...
typedef INT_PTR(FAR DIAMONDAPI *PFNFDINOTIFY)(
    FDINOTIFICATIONTYPE fdint, PFDINOTIFICATION pfdin); /* pfnfdin */

#define FNFDINOTIFY(fn)                                                        \
  INT_PTR FAR DIAMONDAPI fn(FDINOTIFICATIONTYPE fdint, PFDINOTIFICATION pfdin)
 ...
 */
HFDI FAR DIAMONDAPI
FDICreate(__callback PFNALLOC pfnalloc, __callback PFNFREE pfnfree,
          __callback PFNOPEN pfnopen, __callback PFNREAD pfnread,
          __callback PFNWRITE pfnwrite, __callback PFNCLOSE pfnclose,
          __callback PFNSEEK pfnseek, int cpuType, _Inout_ PERF perf);
 ...
 */
BOOL FAR DIAMONDAPI FDIIsCabinet(_In_ HFDI hfdi, _In_ INT_PTR hf,
 ...
 *      writing it out).
 */
BOOL FAR DIAMONDAPI FDICopy(_In_ HFDI hfdi, _In_ LPSTR pszCabinet,
                            _In_ LPSTR pszCabPath, int flags,
