...
#if defined(SCHEMA_VERIFY_VSSYM32) // Verify that all the symbols defined in the
                                   // thschema.h exist in the vssym32.h
//-----------------------------------------------------------------

#define BEGIN_TM_SCHEMA(name)                                                  \
  inline void __Verify__VSSYM32__Function__##name() {

#define TM_ENUM(val, prefix, name) COMPILETIME_ASSERT(prefix##_##name == (val));
#define TM_PROP(val, prefix, name, primval)                                    \
  COMPILETIME_ASSERT(prefix##_##name == (val));
