...
// The following headers are unused by the resource complier (RC). Since the RC
// supports only subset of statements it causes dependencies of these headers to
// trigger W4 warnings. So we are removing them in the RC case.
#ifndef RC_INVOKED
#include <objbase.h>
#include <tchar.h>
#include <winbase.h>
...
//
// The following macros let you enable tracing on per-file and per-function
// level. To use these macros in a file, here is what you should do:
//
...
#define IS_DBCSCHARSET(j)                                                      \
  (((j) == SHIFTJIS_CHARSET) || ((j) == HANGEUL_CHARSET) ||                    \
   ((j) == JOHAB_CHARSET) || ((j) == GB2312_CHARSET) ||                        \
