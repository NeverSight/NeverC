...
#define PATCH_OPTION_USE_LZX_BEST                                              \
  0x00000003 // auto choose best of LZX A/B (but not large)
#define PATCH_OPTION_USE_LZX_A 0x00000001 // normal
#define PATCH_OPTION_USE_LZX_B 0x00000002 // better on some x86 binaries
#define PATCH_OPTION_USE_LZX_LARGE                                             \
  0x00000004 // better support for large files (requires 5.1 or higher applyer)
...
#define PATCH_OPTION_FAIL_IF_BIGGER                                            \
  0x00100000 // fail if patch is larger than simply compressing new file
             // (slower)
...
#define PATCH_OPTION_NO_TIMESTAMP                                              \
  0x00800000 // don't store new file timestamp in patch
#define PATCH_OPTION_SIGNATURE_MD5                                             \
  0x01000000 // use MD5 instead of CRC (reserved for future support)
#define PATCH_OPTION_INTERLEAVE_FILES                                          \
  0x40000000 // better support for large files (requires 5.2 or higher applyer)
...
#define PATCH_SYMBOL_NO_FAILURES                                               \
  0x00000002 // don't fail patch due to imagehlp failures
#define PATCH_SYMBOL_UNDECORATED_TOO                                           \
  0x00000004 // after matching decorated symbols, try to match remaining by
             // undecorated names
...
#define PATCH_TRANSFORM_PE_RESOURCE_2                                          \
  0x00000100 // better handling of PE resources (requires 5.2 or higher applyer)
#define PATCH_TRANSFORM_PE_IRELOC_2                                            \
  0x00000200 // better handling of PE stripped relocs (requires 5.2 or higher
             // applyer)
...
typedef BOOL(CALLBACK PATCH_PROGRESS_CALLBACK)(PVOID CallbackContext,
...
typedef struct _PATCH_OPTION_DATA {
  ...
  PPATCH_INTERLEAVE_MAP *InterleaveMapArray; // array[ OldFileCount ]
                                             // (requires 5.2 or higher applyer)
  ULONG MaxLzxWindowSize; // limit memory requirements (requires 5.2 or higher
                          // applyer)
...
BOOL PATCHAPI CreatePatchFileA(
    ...
    _In_ ULONG OptionFlags, _In_opt_ PPATCH_OPTION_DATA OptionData);
...
BOOL PATCHAPI TestApplyPatchToFileA(_In_ LPCSTR PatchFileName,
                                    _In_ LPCSTR OldFileName,
                                    _In_ ULONG ApplyOptionFlags);

BOOL PATCHAPI TestApplyPatchToFileW(_In_ LPCWSTR PatchFileName,
...
BOOL PATCHAPI
ApplyPatchToFileA(_In_ LPCSTR PatchFileName,   // input file (required)
                  _In_opt_ LPCSTR OldFileName, // input file (optional)
                  _In_ LPCSTR NewFileName,     // output file (required)
                  _In_ ULONG ApplyOptionFlags);

BOOL PATCHAPI
ApplyPatchToFileW(_In_ LPCWSTR PatchFileName,   // input file (required)
                  _In_opt_ LPCWSTR OldFileName, // input file (optional)
...
BOOL PATCHAPI
ApplyPatchToFileExA(_In_ LPCSTR PatchFileName,   // input file (required)
                    _In_opt_ LPCSTR OldFileName, // input file (optional)
                    _In_ LPCSTR NewFileName,     // output file (required)
                    _In_ ULONG ApplyOptionFlags,
                    _In_opt_ PPATCH_PROGRESS_CALLBACK ProgressCallback,
...
BOOL PATCHAPI
ApplyPatchToFileExW(_In_ LPCWSTR PatchFileName,   // input file (required)
                    _In_opt_ LPCWSTR OldFileName, // input file (optional)
                    _In_ LPCWSTR NewFileName,     // output file (required)
                    _In_ ULONG ApplyOptionFlags,
                    _In_opt_ PPATCH_PROGRESS_CALLBACK ProgressCallback,
...
_Success_(return != FALSE) BOOL PATCHAPI ApplyPatchToFileByBuffers(
    ...
    _In_ ULONG OldFileSize,
    _Inout_ _At_(*NewFileBuffer,
                 _When_(*NewFileBuffer != NULL,
                        _Pre_readable_byte_size_(NewFileBufferSize))
                     _Post_readable_byte_size_(*NewFileActualSize))
...
BOOL PATCHAPI GetFilePatchSignatureA(
    _In_ LPCSTR FileName, _In_ ULONG OptionFlags, _In_opt_ PVOID OptionData,
...
BOOL PATCHAPI GetFilePatchSignatureW(
    _In_ LPCWSTR FileName, _In_ ULONG OptionFlags, _In_opt_ PVOID OptionData,
    _In_ ULONG IgnoreRangeCount,
    _In_reads_opt_(IgnoreRangeCount) PPATCH_IGNORE_RANGE IgnoreRangeArray,
    _In_ ULONG RetainRangeCount,
    _In_reads_opt_(RetainRangeCount) PPATCH_RETAIN_RANGE RetainRangeArray,
...
BOOL PATCHAPI GetFilePatchSignatureByHandle(
    _In_ HANDLE FileHandle, _In_ ULONG OptionFlags, _In_opt_ PVOID OptionData,
    _In_ ULONG IgnoreRangeCount,
    _In_reads_opt_(IgnoreRangeCount) PPATCH_IGNORE_RANGE IgnoreRangeArray,
    _In_ ULONG RetainRangeCount,
    _In_reads_opt_(RetainRangeCount) PPATCH_RETAIN_RANGE RetainRangeArray,
...
BOOL PATCHAPI GetFilePatchSignatureByBuffer(
    ...
    _In_ ULONG FileSize, _In_ ULONG OptionFlags, _In_opt_ PVOID OptionData,
    _In_ ULONG IgnoreRangeCount,
    _In_reads_opt_(IgnoreRangeCount) PPATCH_IGNORE_RANGE IgnoreRangeArray,
    _In_ ULONG RetainRangeCount,
    _In_reads_opt_(RetainRangeCount) PPATCH_RETAIN_RANGE RetainRangeArray,
...
INT WINAPI NormalizeFileForPatchSignature(
    ...
    _In_ ULONG FileSize, _In_ ULONG OptionFlags,
    _In_opt_ PATCH_OPTION_DATA *OptionData, _In_ ULONG NewFileCoffBase,
    _In_ ULONG NewFileCoffTime, _In_ ULONG IgnoreRangeCount,
    _In_reads_opt_(IgnoreRangeCount) PPATCH_IGNORE_RANGE IgnoreRangeArray,
    _In_ ULONG RetainRangeCount,
    _In_reads_opt_(RetainRangeCount) PPATCH_RETAIN_RANGE RetainRangeArray);
