...
#define ICM_GETFORMATNAME                                                      \
  (ICM_RESERVED + 20)                       // get a name for a format ;Internal
#define ICM_ENUMFORMATS (ICM_RESERVED + 21) // cycle through formats ;Internal

#define ICM_GETDEFAULTQUALITY                                                  \
  (ICM_RESERVED + 30)                      // get the default value for quality
...
#define ICM_DECOMPRESS_SET_PALETTE                                             \
  (ICM_USER + 29) // fill in the DIB color table
#define ICM_DECOMPRESS_GET_PALETTE                                             \
  (ICM_USER + 30) // fill in the DIB color table
...
#define ICM_DRAW_GET_PALETTE                                                   \
  (ICM_USER + 16) // get the palette needed for drawing
#define ICM_DRAW_UPDATE                                                        \
  (ICM_USER + 17) // update screen with current frame ;Internal
...
#define ICM_DRAW_SETTIME                                                       \
  (ICM_USER + 35) // set correct value for decompress clock
...
#define ICM_GETDEFAULTKEYFRAMERATE                                             \
  (ICM_USER + 42) // get the default value for key frames

#define ICM_DECOMPRESSEX_BEGIN                                                 \
  (ICM_USER + 60) // start a series of decompress calls
#define ICM_DECOMPRESSEX_QUERY                                                 \
  (ICM_USER + 61)                        // start a series of decompress calls
...
#define ICM_COMPRESS_FRAMES                                                    \
  (ICM_USER + 71) // compress a bunch of frames ;Internal
...
typedef struct {
  ...
  _Field_size_bytes_(
      lpbiInput->biSizeImage) LPVOID lpInput; // frame data to compress
...
typedef struct {
  ...
  LONG(CALLBACK *GetData)(_In_ LPARAM lInput, _In_ LONG lFrame,
                          _Out_writes_bytes_(len) LPVOID lpBits, _In_ LONG len);
  LONG(CALLBACK *PutData)(_In_ LPARAM lOutput, _In_ LONG lFrame,
                          _In_reads_bytes_(len) LPVOID lpBits, _In_ LONG len);
} ICCOMPRESSFRAMES;
...
#define ICDRAW_BUFFER                                                          \
  0x00000100L // please buffer this data offscreen, we will need to update it
...
typedef struct {
  ...
  LPBITMAPINFOHEADER
  lpbiSuggest; // location for suggested format (or NULL to get size)
...
BOOL VFWAPI ICInfo(_In_ DWORD fccType, _In_ DWORD fccHandler,
                   _Out_ ICINFO FAR *lpicinfo);

BOOL VFWAPI ICInstall(_In_ DWORD fccType, _In_ DWORD fccHandler,
                      _In_ LPARAM lParam, _In_ LPSTR szDesc, _In_ UINT wFlags);

BOOL VFWAPI ICRemove(_In_ DWORD fccType, _In_ DWORD fccHandler,
                     _In_ UINT wFlags);

_Success_(return != 0) LRESULT VFWAPI
    ICGetInfo(_In_ HIC hic,
...
HIC VFWAPI ICOpenFunction(_In_ DWORD fccType, _In_ DWORD fccHandler,
                          _In_ UINT wMode, _In_ FARPROC lpfnHandler);

LRESULT
VFWAPI
ICClose(_In_ HIC hic);

LRESULT
VFWAPI
ICSendMessage(_In_ HIC hic, _In_ UINT msg, _In_ DWORD_PTR dw1,
              _In_ DWORD_PTR dw2);
#ifndef _WIN32
...
ICCompress(_In_ HIC hic,
           ...
           _In_reads_bytes_opt_(lpbiPrev->biSizeImage)
...
DWORD
VFWAPIV
ICDecompress(_In_ HIC hic,
             _In_ DWORD dwFlags, // flags (from AVI index...)
             _In_ LPBITMAPINFOHEADER
                 lpbiFormat, // BITMAPINFO of compressed data
                             // biSizeImage has the chunk size
 ...
 *
 */
static __inline LRESULT VFWAPI_INLINE ICDecompressEx(
    _In_ HIC hic, _In_ DWORD dwFlags, _In_ LPBITMAPINFOHEADER lpbiSrc,
    _In_reads_bytes_(lpbiSrc->biSizeImage) LPVOID lpSrc, _In_ int xSrc,
    _In_ int ySrc, _In_ int dxSrc, _In_ int dySrc,
    _In_ LPBITMAPINFOHEADER lpbiDst,
    _Out_writes_bytes_(lpbiDst->biSizeImage) LPVOID lpDst, _In_ int xDst,
    _In_ int yDst, _In_ int dxDst, _In_ int dyDst) {
  ICDECOMPRESSEX ic;
 ...
 *
 */
static __inline LRESULT VFWAPI_INLINE ICDecompressExBegin(
    _In_ HIC hic, _In_ DWORD dwFlags, _In_ LPBITMAPINFOHEADER lpbiSrc,
    _In_opt_ LPVOID lpSrc, _In_ int xSrc, _In_ int ySrc, _In_ int dxSrc,
    _In_ int dySrc, _In_ LPBITMAPINFOHEADER lpbiDst, _Out_opt_ LPVOID lpDst,
    _In_ int xDst, _In_ int yDst, _In_ int dxDst, _In_ int dyDst) {
  ICDECOMPRESSEX ic;
 ...
 *
 */
static __inline LRESULT VFWAPI_INLINE ICDecompressExQuery(
    _In_ HIC hic, _In_ DWORD dwFlags, _In_ LPBITMAPINFOHEADER lpbiSrc,
    _Reserved_ LPVOID lpSrc, _In_ int xSrc, _In_ int ySrc, _In_ int dxSrc,
    _In_ int dySrc, _In_opt_ LPBITMAPINFOHEADER lpbiDst, _Out_opt_ LPVOID lpDst,
    _In_ int xDst, _In_ int yDst, _In_ int dxDst, _In_ int dyDst) {
  ICDECOMPRESSEX ic;
...
// to do the same job
static __inline LRESULT VFWAPI_INLINE ICDrawSuggestFormat(
    _In_ HIC hic, _In_ LPBITMAPINFOHEADER lpbiIn,
    _Out_ LPBITMAPINFOHEADER lpbiOut, _In_ int dxSrc, _In_ int dySrc,
    _In_ int dxDst, _In_ int dyDst, _In_ HIC hicDecomp) {
  ICDRAWSUGGEST ic;
...
static __inline LRESULT VFWAPI_INLINE ICSetStatusProc(
    _In_ HIC hic, _In_ DWORD dwFlags, _In_ LRESULT lParam,
...
HIC VFWAPI ICLocate(_In_ DWORD fccType, _In_ DWORD fccHandler,
                    _In_ LPBITMAPINFOHEADER lpbiIn,
                    _In_opt_ LPBITMAPINFOHEADER lpbiOut, _In_ WORD wFlags);

HIC VFWAPI ICGetDisplayFormat(_In_opt_ HIC hic, _In_ LPBITMAPINFOHEADER lpbiIn,
                              _Out_ LPBITMAPINFOHEADER lpbiOut,
                              _In_ int BitDepth, _In_ int dx, _In_ int dy);
...
#define ICMF_CHOOSE_ALLCOMPRESSORS                                             \
  0x0008 // don't only show those that
...
**
*/
extern BOOL VFWAPI DrawDibClose(_In_ HDRAWDIB hdd);

/*
** DrawDibGetBuffer()
**
*/
extern LPVOID VFWAPI DrawDibGetBuffer(_In_ HDRAWDIB hdd,
                                      _Out_ LPBITMAPINFOHEADER lpbi,
                                      _In_ DWORD dwSize, _In_ DWORD dwFlags);
...
**
*/
extern HPALETTE VFWAPI DrawDibGetPalette(_In_ HDRAWDIB hdd);
...
**
*/
extern BOOL VFWAPI DrawDibSetPalette(_In_ HDRAWDIB hdd, _In_opt_ HPALETTE hpal);

/*
**  DrawDibChangePalette()
*/
extern BOOL VFWAPI DrawDibChangePalette(_In_ HDRAWDIB hdd, _In_ int iStart,
                                        _In_ int iLen,
...
**
*/
extern UINT VFWAPI DrawDibRealize(_In_ HDRAWDIB hdd, _In_ HDC hdc,
...
**
*/
extern BOOL VFWAPI DrawDibStart(_In_ HDRAWDIB hdd, _In_ DWORD rate);
...
**
*/
extern BOOL VFWAPI DrawDibStop(_In_ HDRAWDIB hdd);
...
**
*/
extern BOOL VFWAPI DrawDibBegin(_In_ HDRAWDIB hdd, _In_opt_ HDC hdc,
                                _In_ int dxDst, _In_ int dyDst,
                                _In_ LPBITMAPINFOHEADER lpbi, _In_ int dxSrc,
                                _In_ int dySrc, _In_ UINT wFlags);
...
**
*/
extern BOOL VFWAPI DrawDibDraw(_In_ HDRAWDIB hdd, _In_ HDC hdc, _In_ int xDst,
                               _In_ int yDst, _In_ int dxDst, _In_ int dyDst,
                               _In_opt_ LPBITMAPINFOHEADER lpbi,
                               _In_opt_ LPVOID lpBits, _In_ int xSrc,
                               _In_ int ySrc, _In_ int dxSrc, _In_ int dySrc,
                               _In_ UINT wFlags);
...
**  DrawDibEnd()
*/
extern BOOL VFWAPI DrawDibEnd(_In_ HDRAWDIB hdd);
...
LRESULT
VFWAPI
DrawDibProfileDisplay(_In_ LPBITMAPINFOHEADER lpbi);

#ifdef DRAWDIB_INCLUDE_STRETCHDIB
void WINAPI StretchDIB(_In_ LPBITMAPINFOHEADER biDst, _Out_ LPVOID lpDst,
                       _In_ int DstX, _In_ int DstY, _In_ int DstXE,
                       _In_ int DstYE, _In_ LPBITMAPINFOHEADER biSrc,
                       _In_ LPVOID lpSrc, _In_ int SrcX, _In_ int SrcY,
                       _In_ int SrcXE, _In_ int SrcYE);
#endif
...
#define AVIIF_FIRSTPART                                                        \
  0x00000020L // this frame is the start of a partial frame.
...
DECLARE_INTERFACE_(IAVIStream, IUnknown) {
  // *** IUnknown methods ***
  STDMETHOD(QueryInterface)(THIS_ REFIID riid, _Outptr_ LPVOID FAR * ppvObj)
      PURE;
  ...
  STDMETHOD(Info)(THIS_ _Out_writes_bytes_(lSize) AVISTREAMINFOW FAR * psi,
  ...
  STDMETHOD(ReadFormat)(THIS_ _In_ LONG lPos,
                        _Out_writes_bytes_to_opt_(*lpcbFormat, *lpcbFormat)
                            LPVOID lpFormat,
                        _Inout_ LONG FAR * lpcbFormat) PURE;
  STDMETHOD(SetFormat)(THIS_ _In_ LONG lPos,
                       _In_reads_bytes_(cbFormat) LPVOID lpFormat,
                       _In_ LONG cbFormat) PURE;
  STDMETHOD(Read)(THIS_ _In_ LONG lStart, _In_ LONG lSamples,
                  _Out_writes_bytes_opt_(cbBuffer) LPVOID lpBuffer,
                  _In_ LONG cbBuffer, _Out_opt_ LONG FAR * plBytes,
                  _Out_opt_ LONG FAR * plSamples) PURE;
  STDMETHOD(Write)(THIS_ _In_ LONG lStart, _In_ LONG lSamples,
                   _In_reads_bytes_(cbBuffer) LPVOID lpBuffer,
                   _In_ LONG cbBuffer, _In_ DWORD dwFlags,
  ...
  STDMETHOD(ReadData)(THIS_ _In_ DWORD fcc,
                      _Out_writes_bytes_to_opt_(*lpcb, *lpcb) LPVOID lp,
                      _Inout_ LONG FAR * lpcb) PURE;
  STDMETHOD(WriteData)(THIS_ _In_ DWORD fcc, _In_reads_bytes_(cb) LPVOID lp,
                       _In_ LONG cb) PURE;
#ifdef _WIN32
...
DECLARE_INTERFACE_(IAVIStreaming, IUnknown) {
  ...
  STDMETHOD(Begin)(THIS_ _In_ LONG lStart, // start of what we expect
                                           // to play
...
DECLARE_INTERFACE_(IAVIEditStream, IUnknown) {
  ...
  STDMETHOD(Cut)(THIS_ _Inout_ LONG FAR * plStart, _Inout_ LONG FAR * plLength,
                 _Outptr_ PAVISTREAM FAR * ppResult) PURE;
  STDMETHOD(Copy)(THIS_ _Inout_ LONG FAR * plStart, _Inout_ LONG FAR * plLength,
                  _Outptr_ PAVISTREAM FAR * ppResult) PURE;
  STDMETHOD(Paste)(THIS_ _Inout_ LONG FAR * plPos, _Inout_ LONG FAR * plLength,
                   _Inout_ PAVISTREAM pstream, _In_ LONG lStart, _In_ LONG lEnd)
      PURE;
...
DECLARE_INTERFACE_(IAVIFile, IUnknown) {
  ...
  // *** IAVIFile methods ***
  STDMETHOD(Info)(THIS_ _Out_writes_bytes_(lSize) AVIFILEINFOW FAR * pfi,
                  _In_ LONG lSize) PURE;
  STDMETHOD(GetStream)(THIS_ _Outptr_ PAVISTREAM FAR * ppStream,
                       _In_ DWORD fccType, _In_ LONG lParam) PURE;
  STDMETHOD(CreateStream)(THIS_ _Outptr_ PAVISTREAM FAR * ppStream,
                          _In_ AVISTREAMINFOW FAR * psi) PURE;
  STDMETHOD(WriteData)(THIS_ _In_ DWORD ckid,
                       _In_reads_bytes_(cbData) LPVOID lpData, _In_ LONG cbData)
      PURE;
  STDMETHOD(ReadData)(THIS_ _In_ DWORD ckid,
...
DECLARE_INTERFACE_(IGetFrame, IUnknown) {
  ...
  STDMETHOD(Begin)(THIS_ _In_ LONG lStart, _In_ LONG lEnd, _In_ LONG lRate)
      PURE;
  STDMETHOD(End)(THIS) PURE;

  STDMETHOD(SetFormat)(THIS_ _In_ LPBITMAPINFOHEADER lpbi,
                       _In_opt_ LPVOID lpBits, _In_ int x, _In_ int y,
                       _In_ int dx, _In_ int dy) PURE;

  //  STDMETHOD(DrawFrameStart) (THIS) PURE;
  //  STDMETHOD(DrawFrame) (THIS_ _In_ LONG lPos, HDC hdc, _In_ int x, _In_ int
  //  y, _In_ int dx, _In_ int dy) PURE; STDMETHOD(DrawFrameEnd) (THIS) PURE;
};
...
#endif
#else                            // win16
STDAPI AVIFileOpen(PAVIFILE FAR *ppfile, LPCSTR szFile, UINT uMode,
                   LPCLSID lpHandler);
#define AVIFileOpenW AVIFileOpen /* ;Internal */
#endif

#ifdef _WIN32
STDAPI AVIFileInfoW(_In_ PAVIFILE pfile,
                    _Out_writes_bytes_(lSize) LPAVIFILEINFOW pfi,
                    _In_ LONG lSize);
STDAPI AVIFileInfoA(_In_ PAVIFILE pfile,
                    _Out_writes_bytes_(lSize) LPAVIFILEINFOA pfi,
                    _In_ LONG lSize);
#ifdef UNICODE
#define AVIFileInfo AVIFileInfoW
#else
...
STDAPI AVIFileGetStream(_In_ PAVIFILE pfile, _Outptr_ PAVISTREAM FAR *ppavi,
                        _In_ DWORD fccType, _In_ LONG lParam);

#ifdef _WIN32
STDAPI AVIFileCreateStreamW(_In_ PAVIFILE pfile, _Outptr_ PAVISTREAM FAR *ppavi,
                            _In_ AVISTREAMINFOW FAR *psi);
STDAPI AVIFileCreateStreamA(_In_ PAVIFILE pfile, _Outptr_ PAVISTREAM FAR *ppavi,
                            _In_ AVISTREAMINFOA FAR *psi);
#ifdef UNICODE
...
STDAPI AVIFileWriteData(_In_ PAVIFILE pfile, _In_ DWORD ckid,
                        _In_reads_bytes_(cbData) LPVOID lpData,
                        _In_ LONG cbData);
STDAPI AVIFileReadData(_In_ PAVIFILE pfile, _In_ DWORD ckid,
                       _Out_writes_bytes_to_(*lpcbData, *lpcbData)
                           LPVOID lpData,
...
STDAPI AVIStreamInfoW(_In_ PAVISTREAM pavi,
                      _Out_writes_bytes_(lSize) LPAVISTREAMINFOW psi,
                      _In_ LONG lSize);
STDAPI AVIStreamInfoA(_In_ PAVISTREAM pavi,
                      _Out_writes_bytes_(lSize) LPAVISTREAMINFOA psi,
                      _In_ LONG lSize);
#ifdef UNICODE
#define AVIStreamInfo AVIStreamInfoW
#else
...
STDAPI_(LONG)
AVIStreamFindSample(_In_ PAVISTREAM pavi, _In_ LONG lPos, _In_ LONG lFlags);
STDAPI AVIStreamReadFormat(_In_ PAVISTREAM pavi, _In_ LONG lPos,
                           _Out_writes_bytes_to_opt_(*lpcbFormat, *lpcbFormat)
                               LPVOID lpFormat,
                           _Inout_ LONG FAR *lpcbFormat);
STDAPI AVIStreamSetFormat(_In_ PAVISTREAM pavi, _In_ LONG lPos,
                          _In_reads_bytes_(cbFormat) LPVOID lpFormat,
                          _In_ LONG cbFormat);
STDAPI AVIStreamReadData(_In_ PAVISTREAM pavi, _In_ DWORD fcc,
                         _Out_writes_bytes_to_opt_(*lpcb, *lpcb) LPVOID lp,
                         _Inout_ LONG FAR *lpcb);
STDAPI AVIStreamWriteData(_In_ PAVISTREAM pavi, _In_ DWORD fcc,
                          _In_reads_bytes_(cb) LPVOID lp, _In_ LONG cb);

STDAPI AVIStreamRead(_In_ PAVISTREAM pavi, _In_ LONG lStart, _In_ LONG lSamples,
                     _Out_writes_bytes_opt_(cbBuffer) LPVOID lpBuffer,
                     _In_ LONG cbBuffer, _Out_opt_ LONG FAR *plBytes,
                     _Out_opt_ LONG FAR *plSamples);
#define AVISTREAMREAD_CONVENIENT (-1L)

STDAPI AVIStreamWrite(_In_ PAVISTREAM pavi, _In_ LONG lStart,
                      _In_ LONG lSamples,
                      _In_reads_bytes_(cbBuffer) LPVOID lpBuffer,
                      _In_ LONG cbBuffer, _In_ DWORD dwFlags,
                      _Out_opt_ LONG FAR *plSampWritten,
...
STDAPI AVIStreamBeginStreaming(_In_ PAVISTREAM pavi, _In_ LONG lStart,
                               _In_ LONG lEnd, _In_ LONG lRate);
...
//
STDAPI_(PGETFRAME)
AVIStreamGetFrameOpen(_In_ PAVISTREAM pavi,
...
// Shortcut function
STDAPI AVIStreamOpenFromFileA(_Outptr_ PAVISTREAM FAR *ppavi,
                              _In_ LPCSTR szFile, _In_ DWORD fccType,
                              _In_ LONG lParam, _In_ UINT mode,
...
STDAPI AVIStreamCreate(_Outptr_ PAVISTREAM FAR *ppavi, _In_ LONG lParam1,
                       _In_ LONG lParam2, _In_opt_ CLSID FAR *pclsidHandler);

// PHANDLER    AVIAPI AVIGetHandler         (PAVISTREAM pavi, PAVISTREAMHANDLER
// psh); PAVISTREAM  AVIAPI AVIGetStream          (PHANDLER p);
...
STDAPI AVIMakeCompressedStream(_Outptr_ PAVISTREAM FAR *ppsCompressed,
...
EXTERN_C HRESULT CDECL AVISaveA(LPCSTR szFile,
                                ...
                                _In_ int nStreams, _In_ PAVISTREAM pfile,
                                _In_ LPAVICOMPRESSOPTIONS lpOptions, ...);

STDAPI AVISaveVA(LPCSTR szFile, _In_opt_ CLSID FAR *pclsidHandler,
                 _In_ AVISAVECALLBACK lpfnCallback, _In_ int nStreams,
...
STDAPI AVISaveVW(LPCWSTR szFile, _In_opt_ CLSID FAR *pclsidHandler,
                 _In_ AVISAVECALLBACK lpfnCallback, _In_ int nStreams,
                 _In_reads_(nStreams) PAVISTREAM FAR *ppavi,
                 _In_reads_(nStreams) LPAVICOMPRESSOPTIONS FAR *plpOptions);
#ifdef UNICODE
...
STDAPI_(INT_PTR)
AVISaveOptions(_In_ HWND hwnd, _In_ UINT uiFlags, _In_ int nStreams,
               _In_reads_(nStreams) PAVISTREAM FAR *ppavi,
...
STDAPI AVIBuildFilterW(_Out_writes_(cbFilter) LPWSTR lpszFilter,
                       _In_ LONG cbFilter, _In_ BOOL fSaving);
STDAPI AVIBuildFilterA(_Out_writes_(cbFilter) LPSTR lpszFilter,
                       _In_ LONG cbFilter, _In_ BOOL fSaving);
#ifdef UNICODE
...
#endif
STDAPI AVIMakeFileFromStreams(_Outptr_ PAVIFILE FAR *ppfile, _In_ int nStreams,
                              _In_reads_(nStreams) PAVISTREAM FAR *papStreams);

STDAPI AVIMakeStreamFromClipboard(UINT cfFormat, _In_ HANDLE hGlobal,
 ...
 *
 ***************************************************************************/
STDAPI CreateEditableStream(_Outptr_ PAVISTREAM FAR *ppsEditable,
                            _In_ PAVISTREAM psSource);

STDAPI EditStreamCut(_In_ PAVISTREAM pavi, _Inout_ LONG FAR *plStart,
                     _Inout_ LONG FAR *plLength,
                     _Outptr_ PAVISTREAM FAR *ppResult);

STDAPI EditStreamCopy(_In_ PAVISTREAM pavi, _Inout_ LONG FAR *plStart,
                      _Inout_ LONG FAR *plLength,
                      _Outptr_ PAVISTREAM FAR *ppResult);

STDAPI EditStreamPaste(_In_ PAVISTREAM pavi, _Inout_ LONG FAR *plPos,
                       _Inout_ LONG FAR *plLength, _In_ PAVISTREAM pstream,
                       _In_ LONG lStart, _In_ LONG lEnd);
...
STDAPI EditStreamSetInfoW(_In_ PAVISTREAM pavi,
                          _In_reads_bytes_(cbInfo) LPAVISTREAMINFOW lpInfo,
                          _In_ LONG cbInfo);
STDAPI EditStreamSetInfoA(_In_ PAVISTREAM pavi,
                          _In_reads_bytes_(cbInfo) LPAVISTREAMINFOA lpInfo,
                          _In_ LONG cbInfo);
#ifdef UNICODE
...
HWND VFWAPIV MCIWndCreateA(_In_opt_ HWND hwndParent,
                           _In_opt_ HINSTANCE hInstance, _In_ DWORD dwStyle,
                           _In_opt_ LPCSTR szFile);

HWND VFWAPIV MCIWndCreateW(_In_opt_ HWND hwndParent,
                           _In_opt_ HINSTANCE hInstance, _In_ DWORD dwStyle,
...
#define MCIWndSetActiveTimer(hwnd, active)                                     \
  (VOID) MCIWndSM(hwnd, MCIWNDM_SETACTIVETIMER, (WPARAM)(UINT)(active), 0L)
...
// #define MCIWndActivate(hwnd, f)     (void)MCIWndSM(hwnd, WM_ACTIVATE,
// (WPARAM)(BOOL)(f), 0)
...
#define DV_ERR_INT_CONFLICT                                                    \
  (DV_ERR_BASE + 26) /* Interrupt conflict detected                            \
                      */
#define DV_ERR_PROTECT_ONLY                                                    \
  (DV_ERR_BASE + 27) /* Can not run in standard mode */
...
#define capSetCallbackOnStatus(hwnd, fpProc)                                   \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_STATUS, 0,                         \
...
#define capSetCallbackOnVideoStream(hwnd, fpProc)                              \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_VIDEOSTREAM, 0,                    \
                  (LPARAM)(LPVOID)(fpProc)))
#define capSetCallbackOnWaveStream(hwnd, fpProc)                               \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_WAVESTREAM, 0,                     \
                  (LPARAM)(LPVOID)(fpProc)))
#define capSetCallbackOnCapControl(hwnd, fpProc)                               \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_CAPCONTROL, 0,                     \
                  (LPARAM)(LPVOID)(fpProc)))

#define capSetUserData(hwnd, lUser)                                            \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_USER_DATA, 0, (LPARAM)lUser))
...
#define capFileSetCaptureFile(hwnd, szName)                                    \
  ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_SET_CAPTURE_FILE, 0,                       \
                  (LPARAM)(LPVOID)(LPTSTR)(szName)))
#define capFileGetCaptureFile(hwnd, szName, wSize)                             \
  ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_GET_CAPTURE_FILE, (WPARAM)(wSize),         \
                  (LPARAM)(LPVOID)(LPTSTR)(szName)))
...
#define capFileSaveAs(hwnd, szName)                                            \
  ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_SAVEAS, 0,                                 \
                  (LPARAM)(LPVOID)(LPTSTR)(szName)))
...
#define capFileSaveDIB(hwnd, szName)                                           \
  ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_SAVEDIB, 0,                                \
                  (LPARAM)(LPVOID)(LPTSTR)(szName)))
...
#define capGetAudioFormatSize(hwnd)                                            \
  ((DWORD)AVICapSM(hwnd, WM_CAP_GET_AUDIOFORMAT, (WPARAM)0, (LPARAM)0L))
...
#define capCaptureSingleFrameOpen(hwnd)                                        \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SINGLE_FRAME_OPEN, (WPARAM)0, (LPARAM)0L))
#define capCaptureSingleFrameClose(hwnd)                                       \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SINGLE_FRAME_CLOSE, (WPARAM)0, (LPARAM)0L))
...
#define capSetMCIDeviceName(hwnd, szName)                                      \
  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_MCI_DEVICE, 0,                              \
                  (LPARAM)(LPVOID)(LPTSTR)(szName)))
...
#define capPaletteOpen(hwnd, szName)                                           \
  ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_OPEN, 0, (LPARAM)(LPVOID)(LPTSTR)(szName)))
#define capPaletteSave(hwnd, szName)                                           \
  ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_SAVE, 0, (LPARAM)(LPVOID)(LPTSTR)(szName)))
...
typedef LRESULT(CALLBACK *CAPSTATUSCALLBACKW)(_In_ HWND hWnd, _In_ int nID,
                                              LPCWSTR lpsz);
typedef LRESULT(CALLBACK *CAPERRORCALLBACKW)(_In_ HWND hWnd, _In_ int nID,
                                             LPCWSTR lpsz);
typedef LRESULT(CALLBACK *CAPSTATUSCALLBACKA)(_In_ HWND hWnd, _In_ int nID,
                                              LPCSTR lpsz);
typedef LRESULT(CALLBACK *CAPERRORCALLBACKA)(_In_ HWND hWnd, _In_ int nID,
                                             LPCSTR lpsz);
#ifdef UNICODE
...
#endif
typedef LRESULT(CALLBACK *CAPVIDEOCALLBACK)(_In_ HWND hWnd,
                                            _In_ LPVIDEOHDR lpVHdr);
typedef LRESULT(CALLBACK *CAPWAVECALLBACK)(_In_ HWND hWnd,
...
HWND VFWAPI capCreateCaptureWindowA(LPCSTR lpszWindowName, _In_ DWORD dwStyle,
...
BOOL VFWAPI capGetDriverDescriptionA(UINT wDriverIndex,
                                     _Out_writes_(cbName) LPSTR lpszName,
                                     _In_ int cbName,
                                     _Out_writes_(cbVer) LPSTR lpszVer,
                                     _In_ int cbVer);

HWND VFWAPI capCreateCaptureWindowW(LPCWSTR lpszWindowName, DWORD dwStyle,
                                    _In_ int x, _In_ int y, _In_ int nWidth,
                                    _In_ int nHeight, _In_opt_ HWND hwndParent,
                                    _In_ int nID);

BOOL VFWAPI capGetDriverDescriptionW(UINT wDriverIndex,
                                     _Out_writes_(cbName) LPWSTR lpszName,
                                     _In_ int cbName,
                                     _Out_writes_(cbVer) LPWSTR lpszVer,
                                     _In_ int cbVer);
#ifdef UNICODE
...
#define IDS_CAP_SEQ_MSGSTART                                                   \
  410 /* "Select OK to start capture\nof video sequence\nto %s." */
#define IDS_CAP_SEQ_MSGSTOP 411 /* "Hit ESCAPE or click to end capture" */

#define IDS_CAP_VIDEDITERR                                                     \
  412 /* "An error occurred while trying to run VidEdit." */
#define IDS_CAP_READONLYFILE 413 /* "The file '%s' is a read-only file." */
#define IDS_CAP_WRITEERROR                                                     \
  414 /* "Unable to write to file '%s'.\nDisk may be full." */
#define IDS_CAP_NODISKSPACE                                                    \
  415 /* "There is no space to create a capture file on the specified device." \
       */
...
#define IDS_CAP_WAVE_ALLOC_ERROR                                               \
  420 /* "Error: Out of memory for wave buffers." */
#define IDS_CAP_WAVE_PREPARE_ERROR                                             \
  421                               /* "Error: Cannot prepare wave buffers." */
#define IDS_CAP_WAVE_ADD_ERROR 422  /* "Error: Cannot add wave buffers." */
#define IDS_CAP_WAVE_SIZE_ERROR 423 /* "Error: Bad wave size." */

#define IDS_CAP_VIDEO_OPEN_ERROR                                               \
  424 /* "Error: Cannot open the video input device." */
#define IDS_CAP_VIDEO_ALLOC_ERROR                                              \
  425 /* "Error: Out of memory for video buffers." */
#define IDS_CAP_VIDEO_PREPARE_ERROR                                            \
  426                               /* "Error: Cannot prepare video buffers." */
...
#define IDS_CAP_FILE_WRITE_ERROR                                               \
  430 /* "Error: Cannot write to capture file.  Disk may be full." */
#define IDS_CAP_RECORDING_ERROR                                                \
  431 /* "Error: Cannot write to capture file.  Data rate too high or disk     \
         full." */
#define IDS_CAP_RECORDING_ERROR2 432 /* "Error while recording" */
#define IDS_CAP_AVI_INIT_ERROR                                                 \
  433 /* "Error: Unable to initialize for capture." */
#define IDS_CAP_NO_FRAME_CAP_ERROR                                             \
  434 /* "Warning: No frames captured.\nConfirm that vertical sync             \
         interrupts\nare configured and enabled." */
#define IDS_CAP_NO_PALETTE_WARN 435 /* "Warning: Using default palette." */
#define IDS_CAP_MCI_CONTROL_ERROR                                              \
  436 /* "Error: Unable to access MCI device." */
#define IDS_CAP_MCI_CANT_STEP_ERROR                                            \
  437 /* "Error: Unable to step MCI device." */
#define IDS_CAP_NO_AUDIO_CAP_ERROR                                             \
  438 /* "Error: No audio data captured.\nCheck audio card settings." */
#define IDS_CAP_AVI_DRAWDIB_ERROR                                              \
  439 /* "Error: Unable to draw this data format." */
#define IDS_CAP_COMPRESSOR_ERROR                                               \
  440 /* "Error: Unable to initialize compressor." */
#define IDS_CAP_AUDIO_DROP_ERROR                                               \
  441 /* "Error: Audio data was lost during capture, reduce capture rate." */
#define IDS_CAP_AUDIO_DROP_COMPERROR                                           \
  442 /* "Error: Audio data was lost during capture.  Try capturing without    \
         compressing." */
...
#define IDS_CAP_STAT_CAP_FINI                                                  \
  503 /* "Finished capture, now writing frame %ld" */
...
#define IDS_CAP_STAT_VIDEOCURRENT                                              \
  510 /* "Captured %ld frames (%ld dropped) %d.%03d sec." */
#define IDS_CAP_STAT_VIDEOAUDIO                                                \
  511 /* "Captured %d.%03d sec.  %ld frames (%ld dropped) (%d.%03d fps).  %ld  \
         audio bytes (%d,%03d sps)" */
#define IDS_CAP_STAT_VIDEOONLY                                                 \
  512 /* "Captured %d.%03d sec.  %ld frames (%ld dropped) (%d.%03d fps)" */
#define IDS_CAP_STAT_FRAMESDROPPED                                             \
  513  /* "Dropped %ld of %ld frames (%d.%02d%%) during capture." */
#endif /* NOAVIFILE */
...
BOOL VFWAPI GetOpenFileNamePreviewA(_Inout_ LPOPENFILENAMEA lpofn);

BOOL VFWAPI GetSaveFileNamePreviewA(_Inout_ LPOPENFILENAMEA lpofn);

BOOL VFWAPI GetOpenFileNamePreviewW(_Inout_ LPOPENFILENAMEW lpofn);

BOOL VFWAPI GetSaveFileNamePreviewW(_Inout_ LPOPENFILENAMEW lpofn);
