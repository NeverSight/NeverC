...
//@[contract("wdbgexts"), comment("MVI_tracked -
...
typedef VOID(WDBGAPI *PWINDBG_GET_SYMBOL)(PVOID offset, PCHAR pchBuffer,
                                          ULONG_PTR *pDisplacement);

typedef VOID(WDBGAPI *PWINDBG_GET_SYMBOL32)(ULONG offset, PCHAR pchBuffer,
                                            PULONG pDisplacement);

typedef VOID(WDBGAPI *PWINDBG_GET_SYMBOL64)(ULONG64 offset, PCHAR pchBuffer,
                                            PULONG64 pDisplacement);

typedef ULONG(WDBGAPI *PWINDBG_DISASM)(ULONG_PTR *lpOffset, PCSTR lpBuffer,
                                       ULONG fShowEffectiveAddress);

typedef ULONG(WDBGAPI *PWINDBG_DISASM32)(ULONG *lpOffset, PCSTR lpBuffer,
                                         ULONG fShowEffectiveAddress);

typedef ULONG(WDBGAPI *PWINDBG_DISASM64)(ULONG64 *lpOffset, PCSTR lpBuffer,
                                         ULONG fShowEffectiveAddress);

typedef ULONG(WDBGAPI *PWINDBG_CHECK_CONTROL_C)(VOID);

typedef ULONG(WDBGAPI *PWINDBG_READ_PROCESS_MEMORY_ROUTINE)(
    ULONG_PTR offset, PVOID lpBuffer, ULONG cb, PULONG lpcbBytesRead);

typedef ULONG(WDBGAPI *PWINDBG_READ_PROCESS_MEMORY_ROUTINE32)(
    ULONG offset, PVOID lpBuffer, ULONG cb, PULONG lpcbBytesRead);

typedef ULONG(WDBGAPI *PWINDBG_READ_PROCESS_MEMORY_ROUTINE64)(
    ULONG64 offset, PVOID lpBuffer, ULONG cb, PULONG lpcbBytesRead);

typedef ULONG(WDBGAPI *PWINDBG_WRITE_PROCESS_MEMORY_ROUTINE)(
    ULONG_PTR offset, LPCVOID lpBuffer, ULONG cb, PULONG lpcbBytesWritten);

typedef ULONG(WDBGAPI *PWINDBG_WRITE_PROCESS_MEMORY_ROUTINE32)(
    ULONG offset, LPCVOID lpBuffer, ULONG cb, PULONG lpcbBytesWritten);

typedef ULONG(WDBGAPI *PWINDBG_WRITE_PROCESS_MEMORY_ROUTINE64)(
    ULONG64 offset, LPCVOID lpBuffer, ULONG cb, PULONG lpcbBytesWritten);

typedef ULONG(WDBGAPI *PWINDBG_GET_THREAD_CONTEXT_ROUTINE)(
    ULONG Processor, PCONTEXT lpContext, ULONG cbSizeOfContext);

typedef ULONG(WDBGAPI *PWINDBG_SET_THREAD_CONTEXT_ROUTINE)(
    ULONG Processor, PCONTEXT lpContext, ULONG cbSizeOfContext);

typedef ULONG(WDBGAPI *PWINDBG_IOCTL_ROUTINE)(USHORT IoctlType, PVOID lpvData,
                                              ULONG cbSize);

typedef ULONG(WDBGAPI *PWINDBG_OLDKD_READ_PHYSICAL_MEMORY)(ULONGLONG address,
                                                           PVOID buffer,
                                                           ULONG count,
                                                           PULONG bytesread);

typedef ULONG(WDBGAPI *PWINDBG_OLDKD_WRITE_PHYSICAL_MEMORY)(
    ULONGLONG address, PVOID buffer, ULONG length, PULONG byteswritten);
...
typedef ULONG(WDBGAPI *PWINDBG_STACKTRACE_ROUTINE)(ULONG FramePointer,
                                                   ULONG StackPointer,
                                                   ULONG ProgramCounter,
                                                   PEXTSTACKTRACE StackFrames,
                                                   ULONG Frames);

typedef ULONG(WDBGAPI *PWINDBG_STACKTRACE_ROUTINE32)(
    ULONG FramePointer, ULONG StackPointer, ULONG ProgramCounter,
    PEXTSTACKTRACE32 StackFrames, ULONG Frames);

typedef ULONG(WDBGAPI *PWINDBG_STACKTRACE_ROUTINE64)(
    ULONG64 FramePointer, ULONG64 StackPointer, ULONG64 ProgramCounter,
...
typedef VOID(WDBGAPI *PWINDBG_OLD_EXTENSION_ROUTINE)(
    ULONG dwCurrentPc, PWINDBG_EXTENSION_APIS lpExtensionApis,
    PCSTR lpArgumentString);

typedef VOID(WDBGAPI *PWINDBG_EXTENSION_ROUTINE)(HANDLE hCurrentProcess,
                                                 HANDLE hCurrentThread,
                                                 ULONG dwCurrentPc,
                                                 ULONG dwProcessor,
                                                 PCSTR lpArgumentString);

typedef VOID(WDBGAPI *PWINDBG_EXTENSION_ROUTINE32)(HANDLE hCurrentProcess,
                                                   HANDLE hCurrentThread,
                                                   ULONG dwCurrentPc,
                                                   ULONG dwProcessor,
                                                   PCSTR lpArgumentString);

typedef VOID(WDBGAPI *PWINDBG_EXTENSION_ROUTINE64)(HANDLE hCurrentProcess,
                                                   HANDLE hCurrentThread,
                                                   ULONG64 dwCurrentPc,
                                                   ULONG dwProcessor,
                                                   PCSTR lpArgumentString);

typedef VOID(WDBGAPI *PWINDBG_OLDKD_EXTENSION_ROUTINE)(
    ULONG dwCurrentPc, PWINDBG_OLDKD_EXTENSION_APIS lpExtensionApis,
    PCSTR lpArgumentString);

typedef VOID(WDBGAPI *PWINDBG_EXTENSION_DLL_INIT)(
    PWINDBG_EXTENSION_APIS lpExtensionApis, USHORT MajorVersion,
    USHORT MinorVersion);

typedef VOID(WDBGAPI *PWINDBG_EXTENSION_DLL_INIT32)(
    PWINDBG_EXTENSION_APIS32 lpExtensionApis, USHORT MajorVersion,
    USHORT MinorVersion);

typedef VOID(WDBGAPI *PWINDBG_EXTENSION_DLL_INIT64)(
    PWINDBG_EXTENSION_APIS64 lpExtensionApis, USHORT MajorVersion,
    USHORT MinorVersion);
...
//
// A list of type is dumped, listLink should have info about next element
// pointer
//
...
//
// Ioctl returns the size as usual, but will not do field prints/callbacks if
// this is set
//
...
//
// An array of type is dumped, number of elements can be specified in
// listLink->size
//
...
//
// Match the type size to resolve ambiguity in case multiple matches with same
// name are available
//
...
//
// In callback or when Ioctl returns, the FIELD_INFO.address has the address of
// field.
//  If no address is supplied for the type, it contains total offset of the
//  field.
//
...
//
// Nothing is printed  for field if this is set (only callbacks and data copies
// done)
//
#define DBG_DUMP_FIELD_NO_PRINT 0x00004000
//
// If the field is a pointer, it is dumped as a string, ANSI, WCHAR, MULTI or
// GUID depending on following options
//
...
typedef ULONG(WDBGAPI *PSYM_DUMP_FIELD_CALLBACK)(struct _FIELD_INFO *pField,
                                                 PVOID UserContext);
...
__inline VOID ReadPhysical(ULONG64 address,
                           _Out_writes_bytes_to_(size, *sizer) PVOID buf,
                           ULONG size, _Out_ PULONG sizer) {
  PPHYSICAL phy = NULL;
  *sizer = 0;
...
__inline VOID WritePhysical(ULONG64 address, _In_reads_bytes_(size) PVOID buf,
                            ULONG size, _Out_ PULONG sizew) {
  PPHYSICAL phy = NULL;
  *sizew = 0;
  if (size <= WDBGEXTS_MAXSIZE_T - sizeof(*phy)) {
    phy = (PPHYSICAL)LocalAlloc(LPTR, sizeof(*phy) + size);
...
__inline VOID ReadPhysicalWithFlags(ULONG64 address,
                                    _Out_writes_bytes_to_(size, *sizer)
                                        PVOID buf,
...
__inline VOID WritePhysicalWithFlags(ULONG64 address,
...
__inline VOID ReadMsr(ULONG MsrReg, _Out_ ULONGLONG *MsrValue) {
  READ_WRITE_MSR msr;
...
__inline VOID ReadControlSpace(USHORT processor, ULONG address,
...
__inline VOID ReadControlSpace32(USHORT processor, ULONG address,
                                 _Out_writes_bytes_to_(size, 0) PVOID buf,
                                 ULONG size) {
...
#define ReadTypedControlSpace32(_Proc, _Addr, _Buf)                            \
  ReadControlSpace64((USHORT)(_Proc), (ULONG)(_Addr), (PVOID) & (_Buf),        \
                     (ULONG)sizeof(_Buf))

__inline VOID ReadControlSpace64(USHORT processor, ULONG64 address,
                                 _Out_writes_bytes_to_(size, 0) PVOID buf,
                                 ULONG size) {
...
#define ReadTypedControlSpace64(_Proc, _Addr, _Buf)                            \
  ReadControlSpace64((USHORT)(_Proc), (ULONG64)(_Addr), (PVOID) & (_Buf),      \
                     (ULONG)sizeof(_Buf))

__inline VOID WriteControlSpace(USHORT processor, ULONG address,
                                _In_reads_bytes_(size) PVOID buf, ULONG size) {
  PREADCONTROLSPACE64 prc = NULL;
  if (size <= WDBGEXTS_MAXSIZE_T - sizeof(*prc)) {
    prc = (PREADCONTROLSPACE64)LocalAlloc(LPTR, sizeof(*prc) + size);
  }
...
__inline VOID ReadIoSpace(ULONG address, _Out_writes_bytes_(*size) PULONG data,
                          _Inout_ PULONG size) {
  IOSPACE is;
  is.Address = address;
  is.Length = *size;
...
__inline VOID ReadIoSpace32(ULONG address,
...
__inline VOID ReadIoSpace64(ULONG64 address,
                            _Out_writes_bytes_(*size) PULONG data,
                            _Inout_ PULONG size) {
  IOSPACE64 is;
  is.Address = address;
...
__inline VOID WriteIoSpace(ULONG address, ULONG data, _Inout_ PULONG size) {
  IOSPACE is;
...
__inline VOID WriteIoSpace32(ULONG address, ULONG data, _Inout_ PULONG size) {
  IOSPACE32 is;
  is.Address = address;
  is.Length = *size;
  is.Data = data;
...
__inline VOID WriteIoSpace64(ULONG64 address, ULONG data, _Inout_ PULONG size) {
  IOSPACE64 is;
  is.Address = address;
  is.Length = *size;
  is.Data = data;
...
__inline VOID ReadIoSpaceEx(ULONG address, _Out_ PULONG data,
                            _Inout_ PULONG size, ULONG interfacetype,
...
__inline VOID ReadIoSpaceEx32(ULONG address, _Out_ PULONG data,
                              _Inout_ PULONG size, ULONG interfacetype,
                              ULONG busnumber, ULONG addressspace) {
  IOSPACE_EX32 is;
  is.Address = address;
...
__inline VOID ReadIoSpaceEx64(ULONG64 address, _Out_ PULONG data,
                              _Inout_ PULONG size, ULONG interfacetype,
                              ULONG busnumber, ULONG addressspace) {
  IOSPACE_EX64 is;
  is.Address = address;
...
__inline VOID WriteIoSpaceEx(ULONG address, ULONG data, _Inout_ PULONG size,
...
__inline VOID WriteIoSpaceEx32(ULONG address, ULONG data, _Inout_ PULONG size,
                               ULONG interfacetype, ULONG busnumber,
                               ULONG addressspace) {
  IOSPACE_EX32 is;
  is.Address = address;
...
__inline VOID WriteIoSpaceEx64(ULONG64 address, ULONG data, _Inout_ PULONG size,
                               ULONG interfacetype, ULONG busnumber,
                               ULONG addressspace) {
  IOSPACE_EX64 is;
  is.Address = address;
...
__inline VOID ReloadSymbols(_In_ PSTR Arg OPTIONAL)
/*++
...
__inline VOID GetSetSympath(_In_ PSTR Arg,
...
/**
   ...
   If the Field is NULL and the size of Type is <= 8 Whole type value is read
 into pOutValue. This is to allow to read in primitive types suchas ULONG, PVOID
 etc.
 ...
 **/
__inline ULONG GetFieldData(_In_ ULONG64 TypeAddress, _In_ LPCSTR Type,
                            _In_ LPCSTR Field, _In_ ULONG OutSize,
                            _Out_writes_bytes_(OutSize) PVOID pOutValue) {
  FIELD_INFO flds = {(PUCHAR)Field,
                     NULL,
                     0,
                     DBG_DUMP_FIELD_FULL_NAME | DBG_DUMP_FIELD_COPY_FIELD_DATA |
...
//
__inline ULONG64 GetShortField(IN ULONG64 TypeAddress, IN LPCSTR Name,
  ...
  SYM_DUMP_PARAM Sym = {
      (ULONG)sizeof(SYM_DUMP_PARAM),
      SavedName,
      DBG_DUMP_NO_PRINT |
          ((StoreAddress & 2) ? DBG_DUMP_READ_PHYSICAL : (ULONG)0),
      SavedAddress,
      NULL,
      NULL,
      NULL,
  ...
  if (StoreAddress) {
    ...
    return SavedAddress ? Ioctl(IG_DUMP_SYMBOL_INFO, &Sym, Sym.size)
                        : MEMORY_READ_ERROR; // zero on success
  } else {
 ...
 *            parameter of callback contains the address of next Type element in
 * list.
 *
 *   Address, ListByFieldAddress :
 *      if ListByFieldAddress is 0, Adress is the address of first element of
 * Type List.
 *
 *   Lists by LIST_ENTRY are also handled implicitly (by Ioctl). If the
 * NextPointer is a pointer to LIST_ENTRY type, the type address is properly
 * calculated by subtracting the offsets.
 *
 *      If ListByFieldAddress is 1, the Address is considered to be the address
 * of field "NextPointer" of the first Type element and first element address is
 * derived from it.
 *
 */

__inline ULONG ListType(IN LPCSTR Type, IN ULONG64 Address,
                        IN USHORT ListByFieldAddress, IN LPCSTR NextPointer,
                        IN PVOID Context,
...
__inline ULONG GetFieldOffset(IN LPCSTR Type, IN LPCSTR Field,
                              OUT PULONG pOffset) {
  FIELD_INFO flds = {(PUCHAR)Field,
                     (PUCHAR) "",
                     0,
...
__inline ULONG DecodeNtPoolHeaderSizes(IN ULONG64 Address, IN ULONG Sizes,
                                       IN ULONG PoolKey) {
  return Sizes ^ (ULONG)Address ^ PoolKey;
}

__inline ULONG GetPoolHeaderSizeFieldValue(IN ULONG64 pHeader,
  ...
  *FieldValue = 0;
  ...
  PlainPoolHeader = GetFieldValue(StateLoc, "nt!_EX_POOL_HEAP_MANAGER_STATE",
                                  "PoolKey", PoolKey);
  ...
  if (PlainPoolHeader) {
    ...
    Status =
        GetFieldValue(pHeader, "nt!_POOL_HEADER", FieldNameStr, FieldValueTmp);
    ...
    *FieldValue = FieldValueTmp;
    ...
    Status = GetFieldValue(pHeader, "nt!_POOL_HEADER", "Sizes", PoolSizes);
    ...
    PoolSizes.Ulong1 =
        DecodeNtPoolHeaderSizes(pHeader, PoolSizes.Ulong1, PoolKey);
...
__inline VOID GetCurrentProcessAddr(DWORD Processor, ULONG64 CurrentThread,
                                    PULONG64 Address) {
...
__inline VOID SearchMemory(ULONG64 SearchAddress, ULONG64 SearchLength,
                           ULONG PatternLength, PVOID Pattern,
...
__inline ULONG GetInputLine(PCSTR Prompt,
...
__inline BOOL GetExpressionEx(PCSTR Expression, ULONG64 *Value,
                              PCSTR *Remainder) {
...
__inline BOOL ExtMatchPatternA(IN PCSTR Str, IN PCSTR Pattern,
