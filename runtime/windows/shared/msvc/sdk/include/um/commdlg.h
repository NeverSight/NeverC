...
WINCOMMDLGAPI short APIENTRY GetFileTitleA(LPCSTR,
                                           _Out_writes_(cchSize) LPSTR Buf,
                                           _In_ WORD cchSize);
WINCOMMDLGAPI short APIENTRY GetFileTitleW(LPCWSTR,
                                           _Out_writes_(cchSize) LPWSTR Buf,
                                           _In_ WORD cchSize);
#ifdef UNICODE
...
#define OFN_FORCESHOWHIDDEN                                                    \
  0x10000000 // Show All files including System and hidden files
#endif       // (_WIN32_WINNT >= 0x0500)
...
#define CommDlg_OpenSave_GetFolderPathW(_hdlg, _psz, _cbmax)                   \
  (int)SNDMSG(_hdlg, CDM_GETFOLDERPATH, (WPARAM)(_cbmax),                      \
              (LPARAM)(LPWSTR)(_psz))
#ifdef UNICODE
...
#define CommDlg_OpenSave_GetFolderIDList(_hdlg, _pidl, _cbmax)                 \
  (int)SNDMSG(_hdlg, CDM_GETFOLDERIDLIST, (WPARAM)(_cbmax),                    \
              (LPARAM)(LPVOID)(_pidl))
...
// Messages to notify the Find or Replace dialogs of the result of a find or
// replace operation for accessibility purposes

// lParam = pointer to the FINDREPLACE structure that was sent to the client to
// begin the operation wParam = FR_* flags indicating which operations were
// completed return = not used
...
// lParam = pointer to a string to be used to communicate the result of an
// operation to a user wParam = not used return = not used
...
#define CF_WYSIWYG                                                             \
  0x00008000L // must also have CF_SCREENFONTS & CF_PRINTERFONTS
...
WINCOMMDLGAPI _Success_(return != FALSE) BOOL APIENTRY
    PrintDlgA(_Inout_ LPPRINTDLGA pPD);
WINCOMMDLGAPI _Success_(return != FALSE) BOOL APIENTRY
    PrintDlgW(_Inout_ LPPRINTDLGW pPD);
#ifdef UNICODE
...
DECLARE_INTERFACE_IID_(IPrintDialogCallback, IUnknown,
                       "5852A2C3-6530-11D1-B6A3-0000F8757BF9") {
  // *** IUnknown methods ***
...
DECLARE_INTERFACE_IID_(IPrintDialogServices, IUnknown,
                       "509AAEDA-5639-11D1-B6A1-0000F8757BF9") {
  // *** IUnknown methods ***
  STDMETHOD(QueryInterface)(THIS_ _In_ REFIID riid, _Outptr_ void **ppvObj)
      PURE;
  ...
  STDMETHOD(GetCurrentDevMode)(THIS_ _Inout_ LPDEVMODE pDevMode,
                               _Inout_ UINT * pcbSize) PURE;
  STDMETHOD(GetCurrentPrinterName)(THIS_ _Out_writes_opt_(*pcchSize)
                                       LPWSTR pPrinterName,
                                   _Inout_ UINT * pcchSize) PURE;
  STDMETHOD(GetCurrentPortName)(THIS_ _Out_writes_opt_(*pcchSize)
                                    LPWSTR pPortName,
                                _Inout_ UINT * pcchSize) PURE;
};
