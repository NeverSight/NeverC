...
typedef void(STDAPICALLTYPE CALLERRELEASE)(ULONG ulCallerData,
...
#define MAPI_ITABLEDATA_METHODS(IPURE)                                         \
  ...
  (THIS_ LPSSortOrderSet lpSSortOrderSet,                                      \
   CALLERRELEASE FAR * lpfCallerRelease, ULONG ulCallerData,                   \
  ...
  (THIS_ LPSPropValue lpsPropValue, LPSRow FAR * lppSRow,                      \
  ...
  (THIS_ ULONG ulFlags, LPSRowSet lprowsetToDelete, ULONG FAR * cRowsDeleted)  \
      IPURE;
...
STDAPI_(SCODE)
CreateTable(LPCIID lpInterface, ALLOCATEBUFFER FAR *lpAllocateBuffer,
            ALLOCATEMORE FAR *lpAllocateMore, FREEBUFFER FAR *lpFreeBuffer,
 ...
 *		NULL.
 *	ulCallerData
 *		arbitrary data the caller wants saved with this view and
 * returned in the Release callback.
 */
 ...
 *		in the table.  One of the properties must be the index column.
 * Any row in the table with the same value for its index column is replaced, or
 * if there is no current row with that value the row is added. Each row in
 * LPSRowSet MUST have a unique Index column! If any views are open, the view is
 * updated as well. The properties do not have to be in the same order as the
 ...
 *		Each row in the row set contains all the properties for one row
 *		in the table.  One of the properties must be the index column.
 * Any row in the table with the same value for its index column is deleted.
 ...
 *	uliRow
 *		The row number before which this row will be inserted into the
 * table. Row numbers can be from 0 to n where o to n-1 result in row insertion
...
#ifndef CreateIProp
STDAPI_(SCODE)
CreateIProp(LPCIID lpInterface, ALLOCATEBUFFER FAR *lpAllocateBuffer,
            ALLOCATEMORE FAR *lpAllocateMore, FREEBUFFER FAR *lpFreeBuffer,
            LPVOID lpvReserved, LPPROPDATA FAR *lppPropData);
#endif

STDAPI_(SCODE)
WABCreateIProp(LPCIID lpInterface, ALLOCATEBUFFER FAR *lpAllocateBuffer,
               ALLOCATEMORE FAR *lpAllocateMore, FREEBUFFER FAR *lpFreeBuffer,
               LPVOID lpvReserved, LPPROPDATA FAR *lppPropData);
 ...
 *
 *		FIROWAIT		- time given is minimum idle time before
 * calling for the first time in the block of idle time, afterwhich call as
 * often as possible. FIROINTERVAL	- time given is minimum interval between
 * each successive call FIROPERBLOCK	- called only once per contiguous block
 * of idle time FIRODISABLED	- initially disabled when registered, the
 *						  default is to enable the
 * function when registered. FIROONCEONLY	- called only one time by the
 * scheduler and then deregistered automatically.
 */
 ...
 *
 *		Registers the function pfn of type PFNIDLE, i.e., (BOOL
 * (*)(LPVOID)) as an idle function.
 *
...
STDMETHODIMP OpenStreamOnFile(_In_ LPALLOCATEBUFFER lpAllocateBuffer,
                              _In_ LPFREEBUFFER lpFreeBuffer, ULONG ulFlags,
...
typedef HRESULT(STDMETHODCALLTYPE FAR *LPOPENSTREAMONFILE)(
    _In_ LPALLOCATEBUFFER lpAllocateBuffer, _In_ LPFREEBUFFER lpFreeBuffer,
    ULONG ulFlags, _In_ LPTSTR lpszFileName, _In_opt_ LPTSTR lpszPrefix,
    _Outptr_ LPSTREAM FAR *lppStream);
...
STDAPI
HrAllocAdviseSink(LPNOTIFCALLBACK lpfnCallback, LPVOID lpvContext,
...
BuildDisplayTable(LPALLOCATEBUFFER lpAllocateBuffer,
                  ...
                  LPMALLOC lpMalloc, HINSTANCE hInstance, UINT cPages,
                  LPDTPAGE lpPage, ULONG ulFlags, LPMAPITABLE *lppTable,
...
ScRelocProps(int cValues, LPSPropValue lpPropArray, LPVOID lpvBaseOld,
             LPVOID lpvBaseNew, ULONG FAR *lpcb);
...
STDAPI_(SCODE)
ScUNCFromLocalPath(_In_ LPSTR lpszLocal, _In_reads_(cchUNC) LPSTR lpszUNC,
                   UINT cchUNC);
STDAPI_(SCODE)
ScLocalPathFromUNC(_In_ LPSTR lpszUNC, _In_reads_(cchLocal) LPSTR lpszLocal,
                   UINT cchLocal);
...
ScCreateConversationIndex(ULONG cbParent, LPBYTE lpbParent,
                          ULONG FAR *lpcbConvIndex, LPBYTE FAR *lppbConvIndex);

/* Store support */

STDAPI WrapStoreEntryID(ULONG ulFlags, _In_ LPTSTR lpszDLLName,
                        ULONG cbOrigEntry,
                        _In_reads_bytes_(cbOrigEntry) LPENTRYID lpOrigEntry,
                        _Out_ ULONG *lpcbWrappedEntry,
                        _Out_writes_bytes_(*lpcbWrappedEntry)
...
typedef SCODE(STDAPICALLTYPE CREATECONVERSATIONINDEX)(
    ULONG cbParent, LPBYTE lpbParent, ULONG FAR *lpcbConvIndex,
