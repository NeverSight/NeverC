...
//
typedef UINT(WINAPI *PFNCMHOOKPROC)(IN HWND hwndDialog, IN UINT message,
...
DWORD
WINAPI
GetFriendlyNameOfCertA(_In_ PCCERT_CONTEXT pccert,
                       _Out_writes_to_opt_(cch, return) LPSTR pch,
                       _In_ DWORD cch);
CRYPTDLGAPI
DWORD
WINAPI
GetFriendlyNameOfCertW(_In_ PCCERT_CONTEXT pccert,
...
typedef HRESULT(WINAPI *PFNTRUSTHELPER)(IN PCCERT_CONTEXT pCertContext,
                                        IN LPARAM lCustData,
...
//      be freed by the caller.  The data in the TrustInfo array are
//      individually allocated and must be freed.  The data in rgChain must be
//      freed by calling CertFreeCertificateContext.
//
//  Defaults:
//      pszUsageOid == NULL     indicates that no trust validation should be
//      done cRootStores == 0        Will default to User's Root store cStores
//      == 0            Will default to User's CA and system's SPC stores
...
//
//  CertModifyCertificatesToTrust can be used to do modifications to the set of
//  certificates
//      on trust lists for a given purpose.
//      if hcertstoreTrust is NULL, the System Store TRUST in Current User will
//      be used if pccertSigner is specified, it will be used to sign the
//      resulting trust lists,
...
CertModifyCertificatesToTrust(
    ...
    PCCERT_CONTEXT
        pccertSigner); // Certificate to be used in signing trust list
...
BOOL WINAPI
FormatVerisignExtension(DWORD /*dwCertEncodingType*/, DWORD /*dwFormatType*/,
