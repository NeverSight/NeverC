...
typedef union _DUMP_FILE_ATTRIBUTES {
  struct {
    ...
    //
    // If below flag is set, it means Dump data (i.e. non-secureheader data) is
    // encrypted, and Secure header is in use
    //

    ULONG EncryptedDumpData : 1;

    //
    // Below flag would be set by dump decryption software to indicate that the
    // dump was originally encrypted and current dump is obtained after
