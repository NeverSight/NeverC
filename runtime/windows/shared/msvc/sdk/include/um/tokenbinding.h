...
TokenBindingGenerateBinding(
    _In_ TOKENBINDING_KEY_PARAMETERS_TYPE keyType, _In_ PCWSTR targetURL,
    _In_ TOKENBINDING_TYPE bindingType,
    _In_reads_bytes_(tlsEKMSize) const void *tlsEKM, _In_ DWORD tlsEKMSize,
...
STDAPI_(SECURITY_STATUS)
TokenBindingVerifyMessage(_In_reads_bytes_(tokenBindingMessageSize)
                              const void *tokenBindingMessage,
...
STDAPI_(SECURITY_STATUS)
TokenBindingGetKeyTypesClient(_Outptr_ TOKENBINDING_KEY_TYPES **keyTypes);

STDAPI_(SECURITY_STATUS)
TokenBindingGetKeyTypesServer(_Outptr_ TOKENBINDING_KEY_TYPES **keyTypes);

STDAPI_(SECURITY_STATUS)
TokenBindingDeleteBinding(_In_ PCWSTR targetURL);

STDAPI_(SECURITY_STATUS)
TokenBindingDeleteAllBindings();

STDAPI_(SECURITY_STATUS)
TokenBindingGenerateID(_In_ TOKENBINDING_KEY_PARAMETERS_TYPE keyType,
...
STDAPI_(SECURITY_STATUS)
TokenBindingGenerateIDForUri(_In_ TOKENBINDING_KEY_PARAMETERS_TYPE keyType,
                             _In_ PCWSTR targetUri,
                             _Outptr_ TOKENBINDING_RESULT_DATA **resultData);
#endif

STDAPI_(SECURITY_STATUS)
TokenBindingGetHighestSupportedVersion(_Out_ BYTE *majorVersion,
