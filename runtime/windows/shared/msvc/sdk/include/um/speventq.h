...
// This class wraps the SPEVENTEX structure providing a next pointer to allow it
// to be used in queues; and helper methods to serialize and deserialize. This
// has some similar code to CSpEvent in sphelper.h, but that method is public
// and not based on SPEVENTEX.
class CSpEventExNode : public SPEVENTEX {
public:
  CSpEventExNode() { memset(this, 0, sizeof(SPEVENTEX)); }
  ~CSpEventExNode() { Clear(); }
  ...
  static LONG Compare(const CSpEventExNode *p1, const CSpEventExNode *p2) {
    ...
    if (p1->eEventId == SPEI_END_INPUT_STREAM ||
        p2->eEventId == SPEI_END_INPUT_STREAM) {
      // ensure end stream order is not changed because the offset is invalid
      // for it
      return 0;
    } else if (p1->eEventId == SPEI_START_INPUT_STREAM ||
               p2->eEventId == SPEI_START_INPUT_STREAM) {
      // ensure start stream order is not changed because the offset is invalid
      // for it
      return 0;
  ...
  // Copy data from this class to an SPEVENTEX, making new copies of associated
  // memory
  ...
  // Initialize this object from an existing SPEVENTEX, copying associated
  // memory
  ...
  // Finds the additional size of data needed to be appended to the end of the
  // event when serializing
  ...
  ULONG SerializeSizeEx() const {
    ...
    return (ulSize == (ULONG)-1)
               ? 0
               : sizeof(SPSERIALIZEDEVENT64) + ulSize + sizeof(ULONGLONG);
  }
  ...
  // Serialize into an SPSERIALIZEDEVENT64 structure. The ullAudioTimeOffset
  // field is appended at the end of the other serialized data. This is to make
  // this structure binary compatible with SAPI 5.1 code trying to deserialize
  // it.
  HRESULT SerializeEx(SPSERIALIZEDEVENT64 *pSerEvent) const {
    ...
    *(UNALIGNED ULONGLONG *)((BYTE *)pSerEvent + ulSize - sizeof(ULONGLONG)) =
        ullAudioTimeOffset;
  ...
  HRESULT DeserializeEx(const SPSERIALIZEDEVENT64 *pSerEvent, ULONG cbSize) {
    ...
    if (SUCCEEDED(hr)) {
      ullAudioTimeOffset =
          *(UNALIGNED ULONGLONG *)((BYTE *)pTemp + cbSize - sizeof(ULONGLONG));
...
#define DECLARE_SPNOTIFYSOURCE_METHODS(T)                                      \
  ...
  STDMETHODIMP SetNotifyWindowMessage(HWND hWnd, UINT Msg, WPARAM wParam,      \
                                      LPARAM lParam) {                         \
    return T._SetNotifyWindowMessage(hWnd, Msg, wParam, lParam);               \
  }                                                                            \
  STDMETHODIMP SetNotifyCallbackFunction(SPNOTIFYCALLBACK *pfnCallback,        \
                                         WPARAM wParam, LPARAM lParam) {       \
    return T._SetNotifyCallbackFunction(pfnCallback, wParam, lParam);          \
  }                                                                            \
  STDMETHODIMP SetNotifyCallbackInterface(ISpNotifyCallback *pSpCallback,      \
                                          WPARAM wParam, LPARAM lParam) {      \
    return T._SetNotifyCallbackInterface(pSpCallback, wParam, lParam);         \
  }                                                                            \
  STDMETHODIMP SetNotifyWin32Event() { return T._SetNotifyWin32Event(); }      \
...
#define DECLARE_SPEVENTSOURCE_METHODS(T)                                       \
  ...
  STDMETHODIMP SetInterest(ULONGLONG ullEventInterest,                         \
                           ULONGLONG ullQueuedInterest) {                      \
    return T._SetInterest(ullEventInterest, ullQueuedInterest);                \
  }                                                                            \
  STDMETHODIMP GetEvents(ULONG ulCount, SPEVENT *pEventArray,                  \
                         ULONG *pulFetched) {                                  \
    return T._GetEvents(ulCount, pEventArray, pulFetched);                     \
  }                                                                            \
  STDMETHODIMP GetInfo(SPEVENTSOURCEINFO *pInfo) { return T._GetInfo(pInfo); }

class CSpEventSource {
public:
  CSpEventSource(CComObjectRootEx<CComMultiThreadModel> *pParent,
                 bool fSortEvents = true)
      : m_pParent(pParent), m_fSortEvents(fSortEvents) {
    m_ullEventInterest = 0;
    m_ullQueuedInterest = 0;
    m_ulStreamNum = 0;
  }
  HRESULT _SetNotifySink(ISpNotifySink *pNotifySink);
  HRESULT _SetNotifyWindowMessage(HWND hWnd, UINT Msg, WPARAM wParam,
                                  LPARAM lParam);
  HRESULT _SetNotifyCallbackFunction(SPNOTIFYCALLBACK *pfnCallback,
                                     WPARAM wParam, LPARAM lParam);
  HRESULT _SetNotifyCallbackInterface(ISpNotifyCallback *pSpCallback,
                                      WPARAM wParam, LPARAM lParam);
...
#ifdef ETW_TRACE_HEADER
#ifdef SOFTWARE_TRACING
        if (pCur->eEventId == SPEI_RECOGNITION ||
            pCur->eEventId == SPEI_FALSE_RECOGNITION ||
            pCur->eEventId == SPEI_RECOGNITION_ALL) {
          DoTraceMessage(Trace_Verbose,
                         "CpEventSource::Removed event with offset %I64u and "
                         "Id %u from queue",
                         pCur->ullAudioStreamOffset, pCur->eEventId);
        }
#endif
#endif
  ...
  inline HRESULT _GetStreamNumber(const ULONGLONG ullAudioOffset,
                                  ULONG *pulStreamNum);
  //=== Data members ==============================
public:
  ...
  CComPtr<ISpNotifyTranslator>
      m_cpEventTranslator; // If non-NULL then Win32 events being used
  CComObjectRootEx<CComMultiThreadModel> *m_pParent;
  CComAutoCriticalSection
      m_NotifyObjChangeCrit; // Critical section used to make sure that
...
//
//  WARNING:  If this logic changes, you will need to change the logic in
//  SetNotifyWin32Event also.
//
...
inline HRESULT CSpEventSource::_SetNotifyWindowMessage(HWND hWnd, UINT Msg,
                                                       WPARAM wParam,
                                                       LPARAM lParam) {
...
inline HRESULT
CSpEventSource::_SetNotifyCallbackFunction(SPNOTIFYCALLBACK *pfnCallback,
                                           WPARAM wParam, LPARAM lParam) {
...
inline HRESULT
CSpEventSource::_SetNotifyCallbackInterface(ISpNotifyCallback *pSpCallback,
                                            WPARAM wParam, LPARAM lParam) {
...
inline HRESULT CSpEventSource::_SetNotifyWin32Event(void) {
  ...
  if (SUCCEEDED(hr)) {
    //
    //  In this case we do NOT call _SetNotify sink since we want to set the
    //  cpEventTranslator
    //
    m_pParent->Lock();
    m_NotifyObjChangeCrit.Lock();
...
inline HRESULT CSpEventSource::_SetInterest(ULONGLONG ullEventInterest,
                                            ULONGLONG ullQueuedInterest) {
  HRESULT hr = S_OK;
  m_pParent->Lock();
...
//
//  Same as AddEvents except:  No param validation, and caller must take the
//  critical section prior to calling.
//
inline HRESULT CSpEventSource::_AddEvents(const SPEVENT *pEventArray,
                                          ULONG ulCount) {
  HRESULT hr = S_OK;
  for (ULONG i = 0; i < ulCount && SUCCEEDED(hr); ++i) {
    SPEVENTEX EventEx;
    memcpy(&EventEx, pEventArray + i, sizeof(SPEVENT));
    EventEx.ullAudioTimeOffset =
        0; // We set the audio time to zero if no information available
    hr = _AddEvent(EventEx);
  }
  return hr;
}

inline HRESULT CSpEventSource::_AddEvents(const SPEVENTEX *pEventArray,
                                          ULONG ulCount) {
  HRESULT hr = S_OK;
  for (ULONG i = 0; i < ulCount && SUCCEEDED(hr = _AddEvent(pEventArray[i]));
       ++i) {
  }
  return hr;
}
...
#ifdef ETW_TRACE_HEADER
#ifdef SOFTWARE_TRACING
          if (pNode->eEventId == SPEI_RECOGNITION ||
              pNode->eEventId == SPEI_FALSE_RECOGNITION ||
              pNode->eEventId == SPEI_RECOGNITION_ALL) {
            DoTraceMessage(Trace_Verbose,
                           "CpEventSource::Added reco event with offset %I64u "
                           "and event id %u to queue",
                           pNode->ullAudioStreamOffset, pNode->eEventId);
          }
#endif
#endif
        } else {
...
inline HRESULT CSpEventSource::_GetStreamNumber(const ULONGLONG ullAudioOffset,
                                                ULONG *pulStreamNum) {
  CSpEventExNode *pNode = m_PendingList.m_pHead;
  *pulStreamNum = m_ulStreamNum;
  for (; pNode && pNode->ullAudioStreamOffset <= ullAudioOffset;
       pNode = pNode->m_pNext) {
