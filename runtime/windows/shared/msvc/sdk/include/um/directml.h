...
#define DML_DECLARE_INTERFACE(iid) DECLSPEC_UUID(iid) DECLSPEC_NOVTABLE
    ...
    defined(DML_TARGET_VERSION_USE_LATEST) // Use the latest if using redist or
                                           // no Windows target set.
...
#define DML_TARGET_VERSION 0x3000
    ...
    NTDDI_VERSION >= NTDDI_WIN10_VB // Windows 10 2004 Update
#define DML_TARGET_VERSION 0x2000
#else // defined(NTDDI_WIN10_19H1) && NTDDI_VERSION >= NTDDI_WIN10_19H1 //
...
enum DML_OPERATOR_TYPE {
  ...
  DML_OPERATOR_SCATTER =
      DML_OPERATOR_SCATTER_ELEMENTS, // Alias name for backwards compatibility.
...
struct DML_ELEMENT_WISE_QUANTIZED_LINEAR_ADD_OPERATOR_DESC {
  ...
  _Maybenull_ const DML_TENSOR_DESC
      *OutputZeroPointTensor; // This is an input tensor
  const DML_TENSOR_DESC *OutputTensor;
};
...
struct DML_QUANTIZE_OPERATOR_DESC {
  ...
  _Field_size_(QuantizationTensorCount) const
      DML_TENSOR_DESC *QuantizationTensors;
  const DML_TENSOR_DESC *OutputTensor;
};
...
struct DML_FOLD_OPERATOR_DESC {
  ...
  UINT DimensionCount;
  _Field_size_(
      DimensionCount) const UINT *WindowSizes; // Size of the extracted patch
  _Field_size_(
      DimensionCount) const UINT *Strides; // Step size of the extracted patches
  _Field_size_(DimensionCount)
      const UINT *Dilations; // Dialations of the extracted patch
  _Field_size_(DimensionCount)
      const UINT *StartPadding; // Start padding of the "source tensor"
  _Field_size_(DimensionCount)
      const UINT *EndPadding; // End padding of the "source tensor"
};
...
struct DML_FEATURE_QUERY_FEATURE_LEVELS {
  ...
  _Field_size_(RequestedFeatureLevelCount) const
...
STDAPI DMLCreateDevice(ID3D12Device *d3d12Device, DML_CREATE_DEVICE_FLAGS flags,
...
interface DML_DECLARE_INTERFACE("c8263aac-9e0c-4a2d-9b8e-007521a3317c")
    IDMLObject : IUnknown {
  IFACEMETHOD(GetPrivateData)(REFGUID guid, _Inout_ UINT * dataSize,
                              _Out_writes_bytes_opt_(*dataSize) void *data) = 0;

  IFACEMETHOD(SetPrivateData)(REFGUID guid, UINT dataSize,
                              _In_reads_bytes_opt_(dataSize) const void *data) =
      0;

  IFACEMETHOD(SetPrivateDataInterface)(REFGUID guid, _In_opt_ IUnknown * data) =
      0;
...
interface DML_DECLARE_INTERFACE("6dbd6437-96fd-423f-a98c-ae5e7c2a573f")
    IDMLDevice : IDMLObject {
  IFACEMETHOD(CheckFeatureSupport)(
      DML_FEATURE feature, UINT featureQueryDataSize,
      _In_reads_bytes_opt_(featureQueryDataSize) const void *featureQueryData,
      UINT featureSupportDataSize,
      _Out_writes_bytes_(featureSupportDataSize) void *featureSupportData) = 0;

  IFACEMETHOD(CreateOperator)(const DML_OPERATOR_DESC *desc,
                              REFIID riid, // expected: IDMLOperator
                              _COM_Outptr_opt_ void **ppv) = 0;

  IFACEMETHOD(CompileOperator)(IDMLOperator * op, DML_EXECUTION_FLAGS flags,
  ...
  IFACEMETHOD(CreateBindingTable)(_In_opt_ const DML_BINDING_TABLE_DESC *desc,
                                  REFIID riid, // expected: IDMLBindingTable
                                  _COM_Outptr_ void **ppv) = 0;

  IFACEMETHOD(Evict)(UINT count,
                     _In_reads_(count) IDMLPageable *const *ppObjects) = 0;

  IFACEMETHOD(MakeResident)(UINT count, _In_reads_(count)
                                            IDMLPageable *const *ppObjects) = 0;
...
interface DML_DECLARE_INTERFACE("27e83142-8165-49e3-974e-2fd66e4cb69d")
    IDMLDeviceChild : IDMLObject {
  IFACEMETHOD(GetDevice)(REFIID riid, // expected: IDMLDevice
                         _COM_Outptr_ void **ppv) = 0;
};

interface DML_DECLARE_INTERFACE("b1ab0825-4542-4a4b-8617-6dde6e8f6201")
    IDMLPageable : IDMLDeviceChild{};

// ===================================================================================================================
//   DML operator
// ===================================================================================================================

interface DML_DECLARE_INTERFACE("26caae7a-3081-4633-9581-226fbe57695d")
    IDMLOperator : IDMLDeviceChild{};
...
interface DML_DECLARE_INTERFACE("dcb821a8-1039-441e-9f1c-b1759c2f3cec")
    IDMLDispatchable : IDMLPageable {
...
interface DML_DECLARE_INTERFACE("6b15e56a-bf5c-4902-92d8-da3a650afea4")
    IDMLCompiledOperator : IDMLDispatchable{};

// ===================================================================================================================
//   DML operator initializer
// ===================================================================================================================

interface DML_DECLARE_INTERFACE("427c1113-435c-469c-8676-4d5dd072f813")
    IDMLOperatorInitializer : IDMLDispatchable {
  IFACEMETHOD(Reset)(UINT operatorCount,
                     _In_reads_opt_(operatorCount)
                         IDMLCompiledOperator *const *operators) = 0;
};
...
struct DML_BINDING_DESC {
  ...
  _Field_size_opt_(_Inexpressible_("Dependent on binding type")) const
      void *Desc;
};
...
interface DML_DECLARE_INTERFACE("29c687dc-de74-4e3b-ab00-1168f2fc3cfc")
    IDMLBindingTable : IDMLDeviceChild {
  IFACEMETHOD_(void, BindInputs)(UINT bindingCount,
                                 _In_reads_opt_(bindingCount)
                                     const DML_BINDING_DESC *bindings) = 0;

  IFACEMETHOD_(void, BindOutputs)(UINT bindingCount,
                                  _In_reads_opt_(bindingCount)
                                      const DML_BINDING_DESC *bindings) = 0;

  IFACEMETHOD_(void, BindTemporaryResource)(
      _In_opt_ const DML_BINDING_DESC *binding) = 0;
...
interface DML_DECLARE_INTERFACE("e6857a76-2e3e-4fdd-bff4-5d2ba10fb453")
    IDMLCommandRecorder : IDMLDeviceChild {
  IFACEMETHOD_(void, RecordDispatch)(ID3D12CommandList * commandList,
                                     IDMLDispatchable * dispatchable,
                                     IDMLBindingTable * bindings) = 0;
};
...
interface DML_DECLARE_INTERFACE("7d6f3ac9-394a-4ac3-92a7-390cc57a8217")
    IDMLDebugDevice : IUnknown {
...
struct DML_GRAPH_DESC {
  ...
  _Field_size_opt_(IntermediateEdgeCount) const
      DML_GRAPH_EDGE_DESC *IntermediateEdges;
};

interface DML_DECLARE_INTERFACE("a0884f9a-d2be-4355-aa5d-5901281ad1d2")
    IDMLDevice1 : IDMLDevice {
  IFACEMETHOD(CompileGraph)(const DML_GRAPH_DESC *desc,
