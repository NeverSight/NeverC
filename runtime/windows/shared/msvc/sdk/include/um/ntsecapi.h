...
/*
The Audit_ObjectAccess_Handle sub-category behaves different from the other
sub-categories. For handle based audits to be generated (Open handle AuditId:
0x1230, Close handle AuditId: 0x1232), the corresponding object sub-category AND
Audit_ObjectAccess_Handle must be enabled. For eg, to generate handle based
audits for Reg keys, both Audit_ObjectAccess_Registry and
Audit_ObjectAccess_Handle must be enabled
*/
...
DEFINE_GUID(Audit_Logon_AccessRights, 0x0cce924b, 0x69ae, 0x11d9, 0xbe, 0xd3,
            0x50, 0x50, 0x54, 0x50, 0x30, 0x30);
#ifdef INITGUID
...
typedef BOOLEAN (*PSAM_PASSWORD_FILTER_ROUTINE)(
    _In_ PUNICODE_STRING AccountName, _In_ PUNICODE_STRING FullName,
    _In_ PUNICODE_STRING Password, _In_ BOOLEAN SetOperation);
...
// #if (_WIN32_WINNT >= 0x0501) -- Disabled until IIS fixes their target
// version.
...
#define MSV1_0_CHECK_LOGONHOURS_FOR_S4U                                        \
  0x00040000 // check logon hours for S4U logon
#endif
...
#define LOGON_NTLM_V2                                                          \
  0x2000 // LM response was used to authenticate but NT response was used to
...
#define LOGON_PKINIT                                                           \
  0x10000 // Kerberos PKINIT extension was used to authenticate the user
#define LOGON_NO_OPTIMIZED                                                     \
  0x20000 // optimized logon has been disabled for this account
...
#ifdef MIDL_PASS
  [size_is(EncryptedCredsSize)] UCHAR EncryptedCreds[*];
#else
  _Field_size_(EncryptedCredsSize) UCHAR EncryptedCreds[ANYSIZE_ARRAY];
#endif
...
#if (_WIN32_WINNT >= 0x0600)
#define MSV1_0_AV_FLAG_MIC_HANDSHAKE_MESSAGES                                  \
  0x00000002 // the client supports
...
// this is an MSV1_0 private data structure, defining the layout of an NTLM3
// response, as sent by a
//  client in the NtChallengeResponse field of the NETLOGON_NETWORK_INFO
//  structure. If can be differentiated from an old style NT response by its
//  length. This is crude, but it needs to pass through servers and the servers'
//  DCs that do not understand NTLM3 but that are willing to pass longer
//  responses.
typedef struct _MSV1_0_NTLM3_RESPONSE {
  UCHAR Response[MSV1_0_NTLM3_RESPONSE_LENGTH]; // hash of OWF of password with
  ...
  ULONG MsgWord;       // 32 bit message from client to server (for use by auth
                       // protocol)
  ULONGLONG TimeStamp; // time stamp when client generated response -- NT system
                       // time, quad part
  UCHAR ChallengeFromClient[MSV1_0_CHALLENGE_LENGTH];
  ULONG AvPairsOff; // offset to start of AvPairs (to allow future expansion)
  UCHAR Buffer[1];  // start of buffer with AV pairs (or future stuff -- so use
                    // the offset)
...
#if (_WIN32_WINNT >= 0x0501)
BOOLEAN
__stdcall RtlGenRandom(_Out_writes_bytes_(RandomBufferLength)
                           PVOID RandomBuffer,
                       _In_ ULONG RandomBufferLength);
#endif

/*
 * #if(_WIN32_WINNT >= 0x0500) -- Disabled until WinHTTP fixes their target
 * version.
 */
...
// eg: encrypted buffer passed across LPC to another process which calls
// RtlDecryptMemory.
//
...
// eg: encrypted buffer passed across LPC to another process which calls
// RtlDecryptMemory whilst impersonating.
//
...
NTSTATUS
__stdcall RtlEncryptMemory(_Inout_updates_bytes_(MemorySize) PVOID Memory,
                           _In_ ULONG MemorySize, _In_ ULONG OptionFlags);

NTSTATUS
__stdcall RtlDecryptMemory(_Inout_updates_bytes_(MemorySize) PVOID Memory,
                           _In_ ULONG MemorySize, _In_ ULONG OptionFlags);
// #endif
...
#define KRB_NT_PRINCIPAL                                                       \
  1 // Just the name of the principal as in DCE, or for users
...
#define KRB_NT_SRV_HST                                                         \
  3 // Service with host name as instance (telnet, rcommands)
...
typedef struct _KERB_CERTIFICATE_LOGON {
  ...
  UNICODE_STRING
  DomainName; // OPTIONAL, if supplied, used to locate the account forest
...
#define KERB_CERTIFICATE_S4U_LOGON_FLAG_IDENTIFY                               \
  0x8 // request for identify token, must have the same value as
...
typedef struct _KERB_CERTIFICATE_S4U_LOGON {
  ...
  UNICODE_STRING DomainName; // used to locate the forest
                             // OPTIONAL, certificate mapping hints: if missing,
                             // using the local machine's domain
...
typedef struct _KERB_TICKET_LOGON {
  ...
  PUCHAR
  TicketGrantingTicket; // OPTIONAL: User's encdoded in a KERB_CRED message,
                        // encrypted with session key from service ticket
...
//
// If set, use the primary key from the current logon session of the one
// provided in the LogonId field. Otherwise, use the Key in the
// KERB_DECRYPT_MESSAGE.
...
typedef struct _KERB_CERTIFICATE_HASHINFO {
  USHORT StoreNameLength; // length in bytes, if not 0, must inclue L'\0', if 0,
                          // default to L"MY"
...
BOOLEAN
NTAPI
AuditSetSystemPolicy(_In_reads_(dwPolicyCount)
                         PCAUDIT_POLICY_INFORMATION pAuditPolicy,
                     _In_ ULONG dwPolicyCount);

BOOLEAN
NTAPI
AuditSetPerUserPolicy(_In_ const PSID pSid,
                      _In_reads_(dwPolicyCount)
                          PCAUDIT_POLICY_INFORMATION pAuditPolicy,
                      _In_ ULONG dwPolicyCount);

_Check_return_ BOOLEAN NTAPI
AuditQuerySystemPolicy(_In_reads_(dwPolicyCount) const GUID *pSubCategoryGuids,
                       _In_ ULONG dwPolicyCount,
...
_Check_return_ BOOLEAN NTAPI
AuditQueryPerUserPolicy(_In_ const PSID pSid,
...
_Check_return_ BOOLEAN NTAPI AuditEnumeratePerUserPolicy(
    _Out_ _When_(return != 0, __drv_allocatesMem(Mem))
        PPOLICY_AUDIT_SID_ARRAY *ppAuditSidArray);

_Check_return_ BOOLEAN NTAPI AuditComputeEffectivePolicyBySid(
...
_Check_return_ BOOLEAN NTAPI AuditComputeEffectivePolicyByToken(
...
_Check_return_ BOOLEAN NTAPI
AuditEnumerateCategories(_Outptr_result_buffer_(*pdwCountReturned)
                             _When_(return != 0, __drv_allocatesMem(Mem))
                                 GUID **ppAuditCategoriesArray,
                         _Out_ PULONG pdwCountReturned);

_Check_return_ BOOLEAN NTAPI
AuditEnumerateSubCategories(_In_opt_ const GUID *pAuditCategoryGuid,
...
_Check_return_ BOOLEAN NTAPI
AuditLookupCategoryNameW(_In_ const GUID *pAuditCategoryGuid,
                         _Outptr_ _When_(return != 0, __drv_allocatesMem(Mem))
                             PWSTR *ppszCategoryName);

_Check_return_ BOOLEAN NTAPI
AuditLookupCategoryNameA(_In_ const GUID *pAuditCategoryGuid,
                         _Outptr_ _When_(return != 0, __drv_allocatesMem(Mem))
...
_Check_return_ BOOLEAN NTAPI AuditLookupSubCategoryNameW(
    _In_ const GUID *pAuditSubCategoryGuid,
    _Outptr_ _When_(return != 0, __drv_allocatesMem(Mem))
        PWSTR *ppszSubCategoryName);

_Check_return_ BOOLEAN NTAPI AuditLookupSubCategoryNameA(
    _In_ const GUID *pAuditSubCategoryGuid,
    _Outptr_ _When_(return != 0, __drv_allocatesMem(Mem))
...
BOOLEAN
NTAPI
AuditLookupCategoryIdFromCategoryGuid(_In_ const GUID *pAuditCategoryGuid,
                                      _Out_ PPOLICY_AUDIT_EVENT_TYPE
                                          pAuditCategoryId);

BOOLEAN
NTAPI
AuditLookupCategoryGuidFromCategoryId(_In_ POLICY_AUDIT_EVENT_TYPE
                                          AuditCategoryId,
                                      _Out_ GUID *pAuditCategoryGuid);

BOOLEAN
NTAPI
AuditSetSecurity(_In_ SECURITY_INFORMATION SecurityInformation,
                 _In_ PSECURITY_DESCRIPTOR pSecurityDescriptor);

_Check_return_ BOOLEAN NTAPI
AuditQuerySecurity(_In_ SECURITY_INFORMATION SecurityInformation,
                   _Outptr_ _When_(return != 0, __drv_allocatesMem(Mem))
                       PSECURITY_DESCRIPTOR *ppSecurityDescriptor);

BOOLEAN
NTAPI
AuditSetGlobalSaclW(_In_ PCWSTR ObjectTypeName, _In_opt_ PACL Acl);

BOOLEAN
NTAPI
AuditSetGlobalSaclA(_In_ PCSTR ObjectTypeName, _In_opt_ PACL Acl);
...
_Check_return_ BOOLEAN NTAPI AuditQueryGlobalSaclW(
    _In_ PCWSTR ObjectTypeName,
    _Out_ _When_(return != 0, __drv_allocatesMem(Mem)) PACL *Acl);

_Check_return_ BOOLEAN NTAPI AuditQueryGlobalSaclA(
...
VOID NTAPI AuditFree(_In_ __drv_freesMem(Mem) _Post_invalid_ PVOID Buffer);
...
typedef struct _PKU2U_CERTIFICATE_S4U_LOGON {
  ...
  ULONG CertificateLength; // for the client certificate
  _Field_size_bytes_(CertificateLength) PUCHAR
      Certificate; // for the client certificate, BER encoded
