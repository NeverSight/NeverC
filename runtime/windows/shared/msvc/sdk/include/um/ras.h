...
#define RASCF_IsOwner(rascFlags)                                               \
  (((rascFlags) & (RASCF_OwnerKnown | RASCF_OwnerMatch)) ==                    \
   (RASCF_OwnerKnown | RASCF_OwnerMatch))
#define RASCF_IsNotOwner(rascFlags)                                            \
  (((rascFlags) & (RASCF_OwnerKnown | RASCF_OwnerMatch)) == (RASCF_OwnerKnown))
#endif
...
RASPROJECTION{RASP_Amb = 0x10000,    RASP_PppNbf = 0x803F,
              RASP_PppIpx = 0x802B,  RASP_PppIp = 0x8021,
#if (WINVER >= 0x500)
              RASP_PppCcp = 0x80FD,
#endif
              RASP_PppLcp = 0xC021,
...
typedef enum _IKEV2_ID_PAYLOAD_TYPE {
  ...
  IKEV2_ID_PAYLOAD_TYPE_FQDN =
      2, // A fully-qualified domain name string, e.g., "example.com"
  IKEV2_ID_PAYLOAD_TYPE_RFC822_ADDR =
      3, // A fully-qualified RFC 822 email address string, e.g.,
         // "jsmith@example.com".
  IKEV2_ID_PAYLOAD_TYPE_RESERVED1 = 4, // Reserved-Not used
  IKEV2_ID_PAYLOAD_TYPE_ID_IPV6_ADDR =
      5, // A single sixteen (16) octet IPv6 address.
  ...
  IKEV2_ID_PAYLOAD_TYPE_DER_ASN1_DN =
      9, // The binary Distinguished Encoding Rules (DER) encoding of an ASN.1
         // X.500 Distinguished Name [PKIX].
  IKEV2_ID_PAYLOAD_TYPE_DER_ASN1_GN =
      10, // The binary DER encoding of an ASN.1 X.509 GeneralName [PKIX
 ...
 */
DWORD APIENTRY RasDialA(_In_opt_ LPRASDIALEXTENSIONS, _In_opt_ LPCSTR,
                        _In_ LPRASDIALPARAMSA, _In_ DWORD, _In_opt_ LPVOID,
                        _Out_ LPHRASCONN);

DWORD APIENTRY RasDialW(_In_opt_ LPRASDIALEXTENSIONS, _In_opt_ LPCWSTR,
                        _In_ LPRASDIALPARAMSW, _In_ DWORD, _In_opt_ LPVOID,
                        _Out_ LPHRASCONN);

DWORD APIENTRY RasEnumConnectionsA(_Inout_opt_ LPRASCONNA, _Inout_ LPDWORD,
                                   _Out_ LPDWORD);

DWORD APIENTRY RasEnumConnectionsW(_Inout_opt_ LPRASCONNW, _Inout_ LPDWORD,
                                   _Out_ LPDWORD);

DWORD APIENTRY RasEnumEntriesA(_In_opt_ LPCSTR, _In_opt_ LPCSTR,
                               _Inout_opt_ LPRASENTRYNAMEA, _Inout_ LPDWORD,
                               _Out_ LPDWORD);

DWORD APIENTRY RasEnumEntriesW(_In_opt_ LPCWSTR, _In_opt_ LPCWSTR,
                               _Inout_opt_ LPRASENTRYNAMEW, _Inout_ LPDWORD,
                               _Out_ LPDWORD);
...
DWORD APIENTRY RasGetErrorStringA(_In_ UINT ResourceId,
                                  _Out_writes_(InBufSize) LPSTR lpszString,
                                  _In_ DWORD InBufSize);

DWORD APIENTRY RasGetErrorStringW(_In_ UINT ResourceId,
                                  _Out_writes_(InBufSize) LPWSTR lpszString,
                                  _In_ DWORD InBufSize);
...
DWORD APIENTRY RasGetProjectionInfoA(_In_ HRASCONN, _In_ RASPROJECTION,
                                     _Out_ LPVOID, _Inout_ LPDWORD);

DWORD APIENTRY RasGetProjectionInfoW(_In_ HRASCONN, _In_ RASPROJECTION,
                                     _Out_ LPVOID, _Inout_ LPDWORD);
...
DWORD APIENTRY RasEditPhonebookEntryW(_In_ HWND, _In_opt_ LPCWSTR,
                                      _In_ LPCWSTR);

DWORD APIENTRY RasSetEntryDialParamsA(_In_opt_ LPCSTR, _In_ LPRASDIALPARAMSA,
                                      _In_ BOOL);

DWORD APIENTRY RasSetEntryDialParamsW(_In_opt_ LPCWSTR, _In_ LPRASDIALPARAMSW,
                                      _In_ BOOL);

DWORD APIENTRY RasGetEntryDialParamsA(_In_opt_ LPCSTR, _Inout_ LPRASDIALPARAMSA,
                                      _Out_ LPBOOL);

DWORD APIENTRY RasGetEntryDialParamsW(_In_opt_ LPCWSTR,
                                      _Inout_ LPRASDIALPARAMSW, _Out_ LPBOOL);

DWORD APIENTRY RasEnumDevicesA(_Inout_opt_ LPRASDEVINFOA, _Inout_ LPDWORD,
                               _Out_ LPDWORD);

DWORD APIENTRY RasEnumDevicesW(_Inout_opt_ LPRASDEVINFOW, _Inout_ LPDWORD,
                               _Out_ LPDWORD);
...
DWORD APIENTRY RasGetEntryPropertiesA(_In_opt_ LPCSTR, _In_ LPCSTR,
                                      _Inout_opt_ LPRASENTRYA, _Inout_ LPDWORD,
                                      _Out_opt_ LPBYTE, _Inout_opt_ LPDWORD);

DWORD APIENTRY RasGetEntryPropertiesW(_In_opt_ LPCWSTR, _In_ LPCWSTR,
                                      _Inout_opt_ LPRASENTRYW, _Inout_ LPDWORD,
                                      _Out_opt_ LPBYTE, _Inout_opt_ LPDWORD);

DWORD APIENTRY RasSetEntryPropertiesA(_In_opt_ LPCSTR, _In_ LPCSTR,
                                      _In_ LPRASENTRYA, _In_ DWORD,
                                      _In_opt_ LPBYTE, _In_ DWORD);

DWORD APIENTRY RasSetEntryPropertiesW(_In_opt_ LPCWSTR, _In_ LPCWSTR,
                                      _In_ LPRASENTRYW, _In_ DWORD,
                                      _In_opt_ LPBYTE, _In_ DWORD);
...
DWORD APIENTRY RasConnectionNotificationA(_In_ HRASCONN, _In_ HANDLE,
                                          _In_ DWORD);

DWORD APIENTRY RasConnectionNotificationW(_In_ HRASCONN, _In_ HANDLE,
                                          _In_ DWORD);

#if (WINVER >= 0x401)
DWORD APIENTRY RasGetSubEntryHandleA(_In_ HRASCONN, _In_ DWORD,
                                     _Out_ LPHRASCONN);

DWORD APIENTRY RasGetSubEntryHandleW(_In_ HRASCONN, _In_ DWORD,
                                     _Out_ LPHRASCONN);

DWORD APIENTRY RasGetCredentialsA(_In_opt_ LPCSTR, _In_ LPCSTR,
                                  _Inout_ LPRASCREDENTIALSA);

DWORD APIENTRY RasGetCredentialsW(_In_opt_ LPCWSTR, _In_ LPCWSTR,
                                  _Inout_ LPRASCREDENTIALSW);

DWORD APIENTRY RasSetCredentialsA(_In_opt_ LPCSTR, _In_ LPCSTR,
                                  _In_ LPRASCREDENTIALSA, _In_ BOOL);

DWORD APIENTRY RasSetCredentialsW(_In_opt_ LPCWSTR, _In_ LPCWSTR,
                                  _In_ LPRASCREDENTIALSW, _In_ BOOL);

DWORD APIENTRY RasGetSubEntryPropertiesA(_In_opt_ LPCSTR, _In_ LPCSTR,
                                         _In_ DWORD, _Inout_opt_ LPRASSUBENTRYA,
                                         _Inout_opt_ LPDWORD, _Out_opt_ LPBYTE,
                                         _Inout_opt_ LPDWORD);

DWORD APIENTRY RasGetSubEntryPropertiesW(_In_opt_ LPCWSTR, _In_ LPCWSTR,
                                         _In_ DWORD, _Inout_opt_ LPRASSUBENTRYW,
                                         _Inout_opt_ LPDWORD, _Out_opt_ LPBYTE,
                                         _Inout_opt_ LPDWORD);

DWORD APIENTRY RasSetSubEntryPropertiesA(_In_opt_ LPCSTR, _In_ LPCSTR,
                                         _In_ DWORD, _In_ LPRASSUBENTRYA,
                                         _In_ DWORD, _In_opt_ LPBYTE,
                                         _In_ DWORD);

DWORD APIENTRY RasSetSubEntryPropertiesW(_In_opt_ LPCWSTR, _In_ LPCWSTR,
                                         _In_ DWORD, _In_ LPRASSUBENTRYW,
                                         _In_ DWORD, _In_opt_ LPBYTE,
                                         _In_ DWORD);

DWORD APIENTRY RasGetAutodialAddressA(_In_opt_ LPCSTR, _In_opt_ LPDWORD,
                                      _Inout_opt_ LPRASAUTODIALENTRYA,
                                      _Inout_ LPDWORD, _Out_ LPDWORD);

DWORD APIENTRY RasGetAutodialAddressW(_In_opt_ LPCWSTR, _In_opt_ LPDWORD,
                                      _Inout_opt_ LPRASAUTODIALENTRYW,
                                      _Inout_ LPDWORD, _Out_ LPDWORD);

DWORD APIENTRY RasSetAutodialAddressA(_In_opt_ LPCSTR, _In_ DWORD,
                                      _In_opt_ LPRASAUTODIALENTRYA, _In_ DWORD,
                                      _In_ DWORD);

DWORD APIENTRY RasSetAutodialAddressW(_In_opt_ LPCWSTR, _In_ DWORD,
                                      _In_opt_ LPRASAUTODIALENTRYW, _In_ DWORD,
                                      _In_ DWORD);

DWORD APIENTRY
RasEnumAutodialAddressesA(_Inout_updates_bytes_opt_(*lpdwcbRasAutodialAddresses)
                              LPSTR *lppRasAutodialAddresses,
                          _Inout_ LPDWORD lpdwcbRasAutodialAddresses,
                          _Out_ LPDWORD lpdwcRasAutodialAddresses);

DWORD APIENTRY
RasEnumAutodialAddressesW(_Inout_updates_bytes_opt_(*lpdwcbRasAutodialAddresses)
...
DWORD APIENTRY RasInvokeEapUI(_In_ HRASCONN, _In_ DWORD,
                              _In_ LPRASDIALEXTENSIONS, _In_ HWND);
...
DWORD APIENTRY RasGetEapUserDataA(_In_opt_ HANDLE hToken,
                                  _In_opt_ LPCSTR pszPhonebook,
                                  _In_ LPCSTR pszEntry,
                                  _Out_opt_ BYTE *pbEapData,
                                  _Inout_ DWORD *pdwSizeofEapData);

DWORD APIENTRY RasGetEapUserDataW(_In_opt_ HANDLE hToken,
                                  _In_opt_ LPCWSTR pszPhonebook,
                                  _In_ LPCWSTR pszEntry,
                                  _Out_opt_ BYTE *pbEapData,
                                  _Inout_ DWORD *pdwSizeofEapData);

DWORD APIENTRY RasSetEapUserDataA(_In_opt_ HANDLE hToken,
                                  _In_opt_ LPCSTR pszPhonebook,
                                  _In_ LPCSTR pszEntry, _In_ BYTE *pbEapData,
                                  _In_ DWORD dwSizeofEapData);

DWORD APIENTRY RasSetEapUserDataW(_In_opt_ HANDLE hToken,
                                  _In_opt_ LPCWSTR pszPhonebook,
                                  _In_ LPCWSTR pszEntry, _In_ BYTE *pbEapData,
                                  _In_ DWORD dwSizeofEapData);

DWORD APIENTRY RasGetCustomAuthDataA(
    _In_opt_ LPCSTR pszPhonebook, _In_ LPCSTR pszEntry,
...
DWORD APIENTRY RasGetCustomAuthDataW(
    _In_opt_ LPCWSTR pszPhonebook, _In_ LPCWSTR pszEntry,
    _Out_writes_bytes_opt_(*pdwSizeofCustomAuthData) BYTE *pbCustomAuthData,
    _Inout_ DWORD *pdwSizeofCustomAuthData);

DWORD APIENTRY RasSetCustomAuthDataA(_In_opt_ LPCSTR pszPhonebook,
                                     _In_ LPCSTR pszEntry,
                                     _In_reads_bytes_(dwSizeofCustomAuthData)
                                         BYTE *pbCustomAuthData,
                                     _In_ DWORD dwSizeofCustomAuthData);

DWORD APIENTRY RasSetCustomAuthDataW(_In_opt_ LPCWSTR pszPhonebook,
                                     _In_ LPCWSTR pszEntry,
                                     _In_reads_bytes_(dwSizeofCustomAuthData)
                                         BYTE *pbCustomAuthData,
                                     _In_ DWORD dwSizeofCustomAuthData);
DWORD APIENTRY RasGetEapUserIdentityW(
    _In_opt_ LPCWSTR pszPhonebook, _In_ LPCWSTR pszEntry, _In_ DWORD dwFlags,
    _In_ HWND hwnd, _Out_ LPRASEAPUSERIDENTITYW *ppRasEapUserIdentity);
DWORD APIENTRY RasGetEapUserIdentityA(
    _In_opt_ LPCSTR pszPhonebook, _In_ LPCSTR pszEntry, _In_ DWORD dwFlags,
    _In_ HWND hwnd, _Out_ LPRASEAPUSERIDENTITYA *ppRasEapUserIdentity);
VOID APIENTRY
RasFreeEapUserIdentityW(_In_ LPRASEAPUSERIDENTITYW pRasEapUserIdentity);
VOID APIENTRY
RasFreeEapUserIdentityA(_In_ LPRASEAPUSERIDENTITYA pRasEapUserIdentity);
#endif

#if (WINVER >= 0x501)

DWORD APIENTRY RasDeleteSubEntryA(_In_opt_ LPCSTR pszPhonebook,
                                  _In_ LPCSTR pszEntry,
                                  _In_ DWORD dwSubentryId);

DWORD APIENTRY RasDeleteSubEntryW(_In_opt_ LPCWSTR pszPhonebook,
                                  _In_ LPCWSTR pszEntry,
                                  _In_ DWORD dwSubEntryId);
...
DWORD APIENTRY RasGetNapStatus(_In_ HRASCONN hRasconn,
...
/* Update an active RAS connection.Localendpoint if specified is used for
performing /  mobike update.If not specified then dwIfIndex value is used for
performing mobike. /  (See RasUpdateConnection)
*/
...
DWORD APIENTRY RasUpdateConnection(_In_ HRASCONN hrasconn,
                                   _In_ LPRASUPDATECONN lprasupdateconn);

/* Ras Projection information for PPP or IKEv2
 */
DWORD APIENTRY RasGetProjectionInfoEx(
    _In_ HRASCONN hrasconn, _Inout_opt_ PRAS_PROJECTION_INFO pRasProjection,
