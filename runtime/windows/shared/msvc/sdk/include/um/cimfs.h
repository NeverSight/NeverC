...
typedef struct _CIMFS_FILE_METADATA {
  ...
  _Field_size_bytes_(SecurityDescriptorSize) const
      void *SecurityDescriptorBuffer;
...
CimCreateImage(_In_ PCWSTR imageContainingPath,
               _In_opt_ PCWSTR existingImageName, _In_opt_ PCWSTR newImageName,
...
CimCreateAlternateStream(_In_ CIMFS_IMAGE_HANDLE cimImageHandle,
                         _In_ PCWSTR imageRelativePath, _In_ UINT64 streamSize,
                         _Out_ CIMFS_STREAM_HANDLE *cimStreamHandle);
...
STDAPI
CimMountImage(_In_ PCWSTR imageContainingPath, _In_ PCWSTR imageName,
...
//
STDAPI
CimReadFile(_In_ PCWSTR imagePath, _In_ PCWSTR filePath, _In_ UINT64 offset,
            _Out_ void *buffer, _In_ UINT64 bufferSize, _Out_ UINT64 *bytesRead,
...
//
STDAPI
CimGetFileStatBasicInformation(_In_ PCWSTR imagePath, _In_ PCWSTR filePath,
