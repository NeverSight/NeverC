...
// all other bits are reserved for future use and should be returned as 0 (zero)
// for now
...
// Windows Media Player sends and retrieves these structures using code similar
// to this:
//   IWMDMDevice3 *pDevice3;
//   WMP_WMDM_METADATA_ROUND_TRIP_PC2DEVICE PC2Device = {0,0};
//   DWORD dwSizeDevice2PC = sizeof(WMP_WMDM_METADATA_ROUND_TRIP_DEVICE2PC) +
//   0x50000; WMP_WMDM_METADATA_ROUND_TRIP_DEVICE2PC *pDevice2PC =
//   (WMP_WMDM_METADATA_ROUND_TRIP_DEVICE2PC *) new BYTE[dwSizeDevice2PC];
//   HRESULT hr = pDevice3->DeviceIoControl(IOCTL_WMP_METADATA_ROUND_TRIP,
//                                  &PC2Device,
//                                  sizeof(WMP_WMDM_METADATA_ROUND_TRIP_PC2DEVICE),
...
typedef struct _WMP_WMDM_METADATA_ROUND_TRIP_PC2DEVICE {
  DWORD
  dwChangesSinceTransactionID; // 0 on first call ever to device (return all
                               // changes),
                               //   value of
                               //   WMP_WMDM_METADATA_ROUND_TRIP_DEVICE2PC::dwCurrentTransactionID
                               //   from last sync session
  DWORD dwResultSetStartingIndex; // 0 based starting index into result set that
                                  // should
                                  //   be returned, always zero on first call

  // WMDM SP should check the passed buffer size is at least
  // sizeof(WMP_WMDM_METADATA_ROUND_TRIP_PC2DEVICE). Future versions of WMP may
  // pass larger structures which enable new functionality but these
...
typedef struct _WMP_WMDM_METADATA_ROUND_TRIP_DEVICE2PC {
  DWORD
  dwCurrentTransactionID; // Device's current transaction ID (WMP will pass
                          // this
                          //   value as
                          //   WMP_WMDM_METADATA_ROUND_TRIP_PC2DEVICE::dwChangesSinceTransactionID
                          //   during the next sync session)
  DWORD dwReturnedObjectCount; // how many object pathnames are in
                               // wsObjectPathnameList
  DWORD
  dwUnretrievedObjectCount; // how many objects have changes that were not
                            // returned in
                            //   this response (non-zero value implies at
                            //   least one more request must be made by WMP)
  DWORD
  dwDeletedObjectStartingOffset; // wsObjectPathnameList[dwDeletedObjectStartingOffset]
                                 // is
                                 //   first character of first deleted
                                 //   object pathname
                                 // pass 0 if wsObjectPathnameList only
                                 // contains deleted
                                 //   objects (contains no updated or
                                 //   added objects)
                                 // pass the character offset of the last
                                 // nul in
                                 //   wsObjectPathnameList if the list
                                 //   only contains updated or added
                                 //   objects (no deleted objects)
  ...
  WCHAR wsObjectPathnameList
      [1]; // list of null terminated unicode pathname strings, one after
...
//
// This IOCTL will pass no parameters and expects a DWORD return value (1 for
// able to sync, 0 for not able to sync)
//
...
__inline BOOL WMPNotifyDeviceArrival() {
  return (::PostMessage(HWND_BROADCAST,
                        ::RegisterWindowMessageA("WMPlayer_PluginAddRemove"), 2,
                        0));
}

__inline BOOL WMPNotifyDeviceRemoval() {
  return (::PostMessage(HWND_BROADCAST,
                        ::RegisterWindowMessageA("WMPlayer_PluginAddRemove"), 3,
                        0));
}
