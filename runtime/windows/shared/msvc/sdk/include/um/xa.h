...
struct xa_switch_t {
  ...
  int(__cdecl *xa_close_entry)(char *, int,
                               long); /* xa_close function pointer*/
  int(__cdecl *xa_start_entry)(XID *, int,
                               long);           /* xa_start function pointer */
  int(__cdecl *xa_end_entry)(XID *, int, long); /* xa_end function pointer */
  int(__cdecl *xa_rollback_entry)(XID *, int,
                                  long); /* xa_rollback function pointer */
  int(__cdecl *xa_prepare_entry)(XID *, int,
                                 long); /* xa_prepare function pointer */
  int(__cdecl *xa_commit_entry)(XID *, int,
                                long); /* xa_commit function pointer */
  int(__cdecl *xa_recover_entry)(XID *, long, int, long);
  /* xa_recover function pointer*/
  int(__cdecl *xa_forget_entry)(XID *, int,
                                long); /* xa_forget function pointer */
...
#define TMNOMIGRATE                                                            \
  0x00000002L /* resource manager does not support association migration */
#define TMUSEASYNC                                                             \
...
#define TMONEPHASE                                                             \
  0x40000000L /* caller is using one-phase commit optimisation */
#define TMFAIL                                                                 \
  0x20000000L /* dissociates caller and marks transaction branch rollback-only \
               */
#define TMNOWAIT 0x10000000L /* return if blocking condition exists */
#define TMRESUME                                                               \
  0x08000000L /* caller is resuming association with suspended transaction     \
                 branch */
#define TMSUCCESS 0x04000000L /* dissociate caller from transaction branch */
#define TMSUSPEND                                                              \
  0x02000000L /* caller is suspending, not ending, association */
...
#define XA_RBEND                                                               \
  XA_RBTRANSIENT /* The inclusive upper bound of the rollback codes */

#define XA_NOMIGRATE 9 /* resumption must occur where suspension occurred */
#define XA_HEURHAZ                                                             \
  8 /* the transaction branch may have been heuristically completed */
#define XA_HEURCOM                                                             \
  7 /* the transaction branch has been heuristically committed */
#define XA_HEURRB                                                              \
  6 /* the transaction branch has been heuristically rolled back */
#define XA_HEURMIX                                                             \
  5 /* the transaction branch has been heuristically committed and rolled back \
     */
#define XA_RETRY 4 /* routine returned with no effect and may be re-issued */
#define XA_RDONLY                                                              \
  3 /* the transaction branch was read-only and has been committed */
#define XA_OK 0         /* normal execution */
#define XAER_ASYNC (-2) /* asynchronous operation already outstanding */
#define XAER_RMERR                                                             \
  (-3) /* a resource manager error occurred in the transaction branch */
...
typedef int(__cdecl *XA_PREPARE_EPT)(XID *, int,
                                     long); /* xa_prepare entry point */
typedef int(__cdecl *XA_COMMIT_EPT)(XID *, int,
                                    long); /* xa_commit entry point */
typedef int(__cdecl *XA_RECOVER_EPT)(XID *, long, int, long);
/* xa_recover entry point*/
typedef int(__cdecl *XA_FORGET_EPT)(XID *, int,
                                    long); /* xa_forget entry point */
