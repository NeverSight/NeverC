...
typedef VOID(CALLBACK *PFN_WdsTransportClientSessionStart)(
    _In_ HANDLE hSessionKey, _In_ PVOID pCallerData,
...
typedef VOID(CALLBACK *PFN_WdsTransportClientReceiveMetadata)(
    _In_ HANDLE hSessionKey, _In_ PVOID pCallerData,
    _In_reads_bytes_(ulSize) PVOID pMetadata, _In_ ULONG ulSize);

typedef VOID(CALLBACK *PFN_WdsTransportClientReceiveContents)(
    _In_ HANDLE hSessionKey, _In_ PVOID pCallerData,
    _In_reads_bytes_(ulSize) PVOID pContents, _In_ ULONG ulSize,
    _In_ PULARGE_INTEGER pullContentOffset);

typedef VOID(CALLBACK *PFN_WdsTransportClientSessionComplete)(
    _In_ HANDLE hSessionKey, _In_ PVOID pCallerData, _In_ DWORD dwError);

typedef VOID(CALLBACK *PFN_WdsTransportClientSessionNegotiate)(
    _In_ HANDLE hSessionKey, _In_ PVOID pCallerData,
    _In_ PTRANSPORTCLIENT_SESSION_INFO pInfo, _In_ HANDLE hNegotiateKey);
...
DWORD
WDSTCIAPI
WdsTransportClientInitialize(VOID);

DWORD
WDSTCIAPI
WdsTransportClientInitializeSession(
    _In_ PWDS_TRANSPORTCLIENT_REQUEST pSessionRequest, _In_ PVOID pCallerData,
    _Out_ PHANDLE hSessionKey);

DWORD
WDSTCIAPI
WdsTransportClientRegisterCallback(_In_ HANDLE hSessionKey,
                                   _In_ TRANSPORTCLIENT_CALLBACK_ID CallbackId,
                                   _In_ PVOID pfnCallback);

DWORD
WDSTCIAPI
WdsTransportClientStartSession(_In_ HANDLE hSessionKey);

DWORD
WDSTCIAPI
WdsTransportClientCompleteReceive(_In_ HANDLE hSessionKey, _In_ ULONG ulSize,
                                  _In_ PULARGE_INTEGER pullOffset);

DWORD
WDSTCIAPI
WdsTransportClientCancelSession(_In_ HANDLE hSessionKey);

DWORD
WDSTCIAPI
WdsTransportClientCancelSessionEx(_In_ HANDLE hSessionKey,
                                  _In_ DWORD dwErrorCode);

DWORD
WDSTCIAPI
WdsTransportClientWaitForCompletion(_In_ HANDLE hSessionKey,
                                    _In_ ULONG uTimeout);

DWORD
WDSTCIAPI
WdsTransportClientQueryStatus(_In_ HANDLE hSessionKey, _Out_ PULONG puStatus,
                              _Out_ PULONG puErrorCode);

DWORD
WDSTCIAPI
WdsTransportClientCloseSession(_In_ HANDLE hSessionKey);

DWORD
WDSTCIAPI
WdsTransportClientAddRefBuffer(_In_ PVOID pvBuffer);

DWORD
WDSTCIAPI
WdsTransportClientReleaseBuffer(_In_ PVOID pvBuffer);

DWORD
WDSTCIAPI
WdsTransportClientShutdown(VOID);

DWORD
WDSTCIAPI
WdsTransportClientNegotiateRequiredDataRange(
    _In_ HANDLE hSessionKey, _In_ HANDLE hNegotiateKey,
