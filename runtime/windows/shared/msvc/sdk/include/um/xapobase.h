...
protected:
  ...
  ////
  virtual HRESULT ValidateFormatDefault(_Inout_ WAVEFORMATEX *pFormat,
                                        BOOL fOverwrite);
  ...
  HRESULT ValidateFormatPair(const WAVEFORMATEX *pSupportedFormat,
                             _Inout_ WAVEFORMATEX *pRequestedFormat,
                             BOOL fOverwrite);
  ...
  //  void
  ////
  void ProcessThru(const void *pInputBuffer,
                   _Inout_updates_(FrameCount *OutputChannelCount)
                       FLOAT32 *pOutputBuffer,
                   UINT32 FrameCount, UINT32 InputChannelCount,
                   UINT32 OutputChannelCount, BOOL MixWithOutput);
...
public:
  ...
  STDMETHOD(IsInputFormatSupported)(
      ...
      _Outptr_opt_ WAVEFORMATEX **ppSupportedInputFormat);
  ...
  STDMETHOD(IsOutputFormatSupported)(
      ...
      _Outptr_opt_ WAVEFORMATEX **ppSupportedOutputFormat);

  // Performs any effect-specific initialization.
  // This default implementation is a no-op and only returns S_OK.
  STDMETHOD(Initialize)(_In_reads_bytes_opt_(DataByteSize) const void *,
                        UINT32 DataByteSize) {
  ...
  STDMETHOD(LockForProcess)(
      _Pre_equal_to_(OutputLockedParameterCount)
          UINT32 InputLockedParameterCount,
      _In_reads_opt_(InputLockedParameterCount)
          const XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS *pInputLockedParameters,
      _Pre_equal_to_(InputLockedParameterCount)
          UINT32 OutputLockedParameterCount,
      _In_reads_opt_(OutputLockedParameterCount)
...
////
class DECLSPEC_NOVTABLE CXAPOParametersBase : public CXAPOBase,
...
private:
  ...
  BOOL m_fNewerResultsReady; // TRUE if there exists new processing results not
                             ...
                             // aligned for atomic operations
  BOOL m_fProducer; // TRUE if IXAPO::Process produces data to be returned by
                    // GetParameters(), SetParameters() and ParametersChanged()
                    // disallowed
...
public:
  ...
  //  parameter blocks, must be > 0, should be > 4 fProducer               -
  //  [in] TRUE if IXAPO::Process produces data to be returned by
  //  GetParameters() (SetParameters() and ParametersChanged() disallowed)
  ////
  CXAPOParametersBase(
      const XAPO_REGISTRATION_PROPERTIES *pRegistrationProperties,
      _In_reads_bytes_opt_(3 * uParameterBlockByteSize) BYTE *pParameterBlocks,
      UINT32 uParameterBlockByteSize, BOOL fProducer);
  virtual ~CXAPOParametersBase();

  // IUnknown methods:
  // retrieves the requested interface pointer if supported
  STDMETHOD(QueryInterface)(REFIID riid,
                            _Outptr_result_maybenull_ void **ppInterface) {
    XAPOASSERT(ppInterface != NULL);
    HRESULT hr = S_OK;
  ...
  STDMETHOD_(void, SetParameters)(_In_reads_bytes_(ParameterByteSize)
  ...
  STDMETHOD_(void, GetParameters)(
      _Out_writes_bytes_(ParameterByteSize) void *pParameters,
      UINT32 ParameterByteSize);
  ...
  virtual void OnSetParameters(_In_reads_bytes_(ParameterByteSize)
                                   const void *pParameters,
                               UINT32 ParameterByteSize) {
