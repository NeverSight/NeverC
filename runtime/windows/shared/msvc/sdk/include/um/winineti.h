...
#define INTERNET_REQFLAG_FROM_APP_CACHE                                        \
  0x00000100 // Response came from AppCache

//
// These two flags should really be included in a new mask called
// INTERNET_FLAGS_INTERNAL_MASK. Since BGUPDATE is included in
// INTERNET_FLAGS_MASK already it will cause compile errors when used without
// winineti.h.  FTP_FOLDER_VIEW is not being included so as to not compound the
// problem.
//

#define INTERNET_FLAG_BGUPDATE 0x00000008
#define INTERNET_FLAG_FTP_FOLDER_VIEW 0x00000004
#define INTERNET_FLAGS_MASK_INTERNAL                                           \
  (INTERNET_FLAGS_MASK | INTERNET_FLAG_FTP_FOLDER_VIEW)
...
BOOLAPI InternetAlgIdToStringA(_In_ ALG_ID ai,
                               _Out_writes_(*lpdwstrLength) LPSTR lpstr,
                               _Inout_ LPDWORD lpdwstrLength,
                               _Reserved_ DWORD dwReserved);
BOOLAPI InternetAlgIdToStringW(_In_ ALG_ID ai,
...
InternetSecurityProtocolToStringA(
    ...
    _Inout_ LPDWORD lpdwstrLength, _Reserved_ DWORD dwReserved);
BOOLAPI
...
BOOLAPI InternetGetSecurityInfoByURLA(_In_ LPSTR lpszURL,
                                      _Out_ PCCERT_CHAIN_CONTEXT *ppCertChain,
                                      _Out_ DWORD *pdwSecureFlags);

BOOLAPI InternetGetSecurityInfoByURLW(_In_ LPCWSTR lpszURL,
                                      _Out_ PCCERT_CHAIN_CONTEXT *ppCertChain,
                                      _Out_ DWORD *pdwSecureFlags);
...
#else
BOOLAPI InternetGetSecurityInfoByURL(_In_ LPSTR lpszURL,
                                     _Out_ PCCERT_CHAIN_CONTEXT *ppCertChain,
                                     _Out_ DWORD *pdwSecureFlags);
#endif // _WINX32_
#endif // !UNICODE
...
INTERNETAPI_(DWORD)
ShowSecurityInfo(_In_ HWND hWndParent,
                 _In_ LPINTERNET_SECURITY_INFO pSecurityInfo);
#endif // INTERNET_SECURITY_INFO_DEFINED

INTERNETAPI_(DWORD)
ShowX509EncodedCertificate(_In_ HWND hWndParent,
                           _In_reads_bytes_(cbCert) LPBYTE lpCert,
                           _In_ DWORD cbCert);

INTERNETAPI_(DWORD) ShowClientAuthCerts(_In_ HWND hWndParent);

INTERNETAPI_(DWORD)
ParseX509EncodedCertificateForListBoxEntry(_In_reads_bytes_(cbCert)
                                               LPBYTE lpCert,
                                           _In_ DWORD cbCert,
                                           _Out_writes_opt_(*lpdwListBoxEntry)
...
BOOLAPI InternetShowSecurityInfoByURLA(_In_ LPSTR lpszURL,
                                       _In_ HWND hwndParent);

BOOLAPI InternetShowSecurityInfoByURLW(_In_ LPCWSTR lpszURL,
                                       _In_ HWND hwndParent);
...
#else
BOOLAPI InternetShowSecurityInfoByURL(_In_ LPSTR lpszURL, _In_ HWND hwndParent);
#endif // _WINX32_
#endif // !UNICODE
...
BOOLAPI InternetFortezzaCommand(_In_ DWORD dwCommand, _In_ HWND hwnd,
...
BOOLAPI InternetQueryFortezzaStatus(_Out_ DWORD *pdwStatus,
                                    _Reserved_ DWORD_PTR dwReserved);

BOOLAPI InternetDebugGetLocalTime(_Out_ SYSTEMTIME *pstLocalTime,
                                  _Out_opt_ DWORD *pdwReserved);

// causes InternetCreateUrlA to escape chars in authority components (user, pwd,
// host)
...
#define InternetConnectUrl(hInternet, lpszUrl, dwFlags, dwContext)             \
  InternetConnect(hInternet, lpszUrl, INTERNET_INVALID_PORT_NUMBER, NULL,      \
                  NULL, INTERNET_SERVICE_URL, dwFlags, dwContext)

BOOLAPI InternetWriteFileExA(_In_ HINTERNET hFile,
                             _In_ LPINTERNET_BUFFERSA lpBuffersIn,
                             _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
BOOLAPI InternetWriteFileExW(_In_ HINTERNET hFile,
...
#define INTERNET_LINE_STATE_MASK                                               \
  (INTERNET_STATE_ONLINE | INTERNET_STATE_OFFLINE)
#define INTERNET_BUSY_STATE_MASK (INTERNET_STATE_IDLE | INTERNET_STATE_BUSY)

//
// the following are used with InternetSetOption(...,
// INTERNET_OPTION_CALLBACK_FILTER, ...) to filter out unrequired callbacks.
// INTERNET_STATUS_REQUEST_COMPLETE cannot be filtered out
//
...
// headers allowed to be modified is restricted.
//
...
INTERNETAPI_(DWORD)
HttpGetServerCredentials(_In_ PWSTR pwszUrl,
...
INTERNETAPI_(DWORD)
HttpPushEnable(_In_ HINTERNET hRequest,
               _In_ HTTP_PUSH_TRANSPORT_SETTING *pTransportSetting,
               _Out_ HTTP_PUSH_WAIT_HANDLE *phWait);

INTERNETAPI_(DWORD)
HttpPushWait(_In_ HTTP_PUSH_WAIT_HANDLE hWait, _In_ HTTP_PUSH_WAIT_TYPE eType,
             _Out_opt_ HTTP_PUSH_NOTIFICATION_STATUS *pNotificationStatus);

INTERNETAPI_(VOID)
HttpPushClose(_In_ HTTP_PUSH_WAIT_HANDLE hWait);
...
#define COOKIE_ALLOWED_SET_FLAGS                                               \
  (INTERNET_COOKIE_THIRD_PARTY | INTERNET_COOKIE_PROMPT_REQUIRED |             \
   INTERNET_COOKIE_EVALUATE_P3P | INTERNET_COOKIE_NON_SCRIPT |                 \
   INTERNET_COOKIE_RESTRICTED_ZONE | INTERNET_COOKIE_NO_CALLBACK |             \
   INTERNET_COOKIE_APPLY_HOST_ONLY | INTERNET_COOKIE_EDGE_COOKIES)

#define COOKIE_ALLOWED_COOKIE_FLAGS                                            \
  (INTERNET_COOKIE_IS_SECURE | INTERNET_COOKIE_IS_SESSION |                    \
   INTERNET_COOKIE_IS_RESTRICTED | INTERNET_COOKIE_HTTPONLY |                  \
   INTERNET_COOKIE_HOST_ONLY | INTERNET_COOKIE_HOST_ONLY_APPLIED |             \
   INTERNET_COOKIE_ECTX_3RDPARTY | INTERNET_COOKIE_SAME_SITE_STRICT |          \
   INTERNET_COOKIE_SAME_SITE_LAX)

#define COOKIE_ALLOWED_GET_FLAGS                                               \
  (INTERNET_COOKIE_NON_SCRIPT | INTERNET_COOKIE_THIRD_PARTY |                  \
   INTERNET_COOKIE_SAME_SITE_LEVEL_CROSS_SITE |                                \
   INTERNET_FLAG_RESTRICTED_ZONE | INTERNET_COOKIE_ALL_COOKIES |               \
   INTERNET_COOKIE_EDGE_COOKIES)
//
// DAV Detection
//
BOOLAPI HttpCheckDavComplianceA(_In_ LPCSTR lpszUrl,
                                _In_ LPCSTR lpszComplianceToken,
                                _Inout_ LPBOOL lpfFound, _In_ HWND hWnd,
                                _In_ LPVOID lpvReserved);
//
// DAV Detection
//
BOOLAPI HttpCheckDavComplianceW(_In_ LPCWSTR lpszUrl,
...
#define ERROR_HTTP_COOKIE_NEEDS_CONFIRMATION_EX                                \
  (INTERNET_INTERNAL_ERROR_BASE + 7)
...
#define INCLUDE_BY_DEFAULT_CACHE_ENTRY                                         \
  (HTTP_1_1_CACHE_ENTRY | STATIC_CACHE_ENTRY | MUST_REVALIDATE_CACHE_ENTRY |   \
   PRIVACY_IMPACTED_CACHE_ENTRY | POST_CHECK_CACHE_ENTRY |                     \
   COOKIE_ACCEPTED_CACHE_ENTRY | COOKIE_LEASHED_CACHE_ENTRY |                  \
   COOKIE_DOWNGRADED_CACHE_ENTRY | COOKIE_REJECTED_CACHE_ENTRY |               \
   SHORTPATH_CACHE_ENTRY | IMMUTABLE_CACHE_ENTRY)
...
BOOLAPI IsUrlCacheEntryExpiredA(_In_ LPCSTR lpszUrlName, _In_ DWORD dwFlags,
                                _Inout_ FILETIME *pftLastModified);
BOOLAPI IsUrlCacheEntryExpiredW(_In_ LPCWSTR lpszUrlName, _In_ DWORD dwFlags,
                                _Inout_ FILETIME *pftLastModified);
#ifdef UNICODE
...
BOOLAPI CreateUrlCacheEntryExW(_In_ LPCWSTR lpszUrlName,
...
URLCACHEAPI_(DWORD)
GetUrlCacheEntryBinaryBlob(_In_ PCWSTR pwszUrlName, _Out_ DWORD *dwType,
                           _Out_ FILETIME *pftExpireTime,
                           _Out_ FILETIME *pftAccessTime,
                           _Out_ FILETIME *pftModifiedTime,
                           _Outptr_result_buffer_all_maybenull_(*pcbBlob)
                               BYTE **ppbBlob,
                           _Out_ DWORD *pcbBlob);

URLCACHEAPI_(DWORD)
CommitUrlCacheEntryBinaryBlob(_In_ PCWSTR pwszUrlName, _In_ DWORD dwType,
...
BOOLAPI CreateUrlCacheContainerA(
    _In_ LPCSTR Name, _In_ LPCSTR lpCachePrefix, _In_opt_ LPCSTR lpszCachePath,
    _In_ DWORD KBCacheLimit, _In_ DWORD dwContainerType, _In_ DWORD dwOptions,
    _Reserved_ LPVOID pvBuffer, _Reserved_ LPDWORD cbBuffer);
BOOLAPI CreateUrlCacheContainerW(_In_ LPCWSTR Name, _In_ LPCWSTR lpCachePrefix,
...
BOOLAPI DeleteUrlCacheContainerA(_In_ LPCSTR Name, _In_ DWORD dwOptions);
BOOLAPI DeleteUrlCacheContainerW(_In_ LPCWSTR Name, _In_ DWORD dwOptions);
#ifdef UNICODE
...
typedef struct _INTERNET_CACHE_CONTAINER_INFOA {
  ...
  LPSTR
  lpszVolumeLabel; // embedded pointer to the container volume label if any.
  LPSTR
  lpszVolumeTitle; // embedded pointer to the container volume title if any.
...
FindFirstUrlCacheContainerA(
    ...
    _Out_writes_bytes_(*lpcbContainerInfo)
        LPINTERNET_CACHE_CONTAINER_INFOA lpContainerInfo,
    _Inout_ LPDWORD lpcbContainerInfo, _In_ DWORD dwOptions);
INTERNETAPI_(HANDLE)
...
BOOLAPI
FindNextUrlCacheContainerA(_In_ HANDLE hEnumHandle,
                           _Out_writes_bytes_(*lpcbContainerInfo)
                               LPINTERNET_CACHE_CONTAINER_INFOA lpContainerInfo,
                           _Inout_ LPDWORD lpcbContainerInfo);
BOOLAPI
FindNextUrlCacheContainerW(_In_ HANDLE hEnumHandle,
                           _Out_writes_bytes_(*lpcbContainerInfo)
                               LPINTERNET_CACHE_CONTAINER_INFOW lpContainerInfo,
                           _Inout_ LPDWORD lpcbContainerInfo);
#ifdef UNICODE
...
BOOLAPI FreeUrlCacheSpaceA(_In_opt_ LPCSTR lpszCachePath, _In_ DWORD dwSize,
                           _In_ DWORD dwFilter);
BOOLAPI FreeUrlCacheSpaceW(_In_opt_ LPCWSTR lpszCachePath, _In_ DWORD dwSize,
                           _In_ DWORD dwFilter);
#ifdef UNICODE
...
DWORD UrlCacheFreeGlobalSpace(_In_ ULONGLONG ullTargetSize,
                              _In_ DWORD dwFilter);

DWORD UrlCacheGetGlobalCacheSize(_In_ DWORD dwFilter, _Out_ PULONGLONG pullSize,
...
#define CACHE_CONFIG_DISK_SPACE_VERYLOW_APPCONTAINER_TOTAL                     \
  36700160ULL                                                       //   35 MB
#define CACHE_CONFIG_DISK_SPACE_LOW_APPCONTAINER_TOTAL 104857600ULL //  100 MB
#define CACHE_CONFIG_DISK_SPACE_BELOWNORMAL_APPCONTAINER_TOTAL                 \
  262144000ULL //  250 MB
#define CACHE_CONFIG_DISK_SPACE_NORMAL_APPCONTAINER_TOTAL                      \
  1048576000ULL // 1000 MB
...
BOOLAPI
GetUrlCacheConfigInfoA(_Inout_ LPINTERNET_CACHE_CONFIG_INFOA lpCacheConfigInfo,
                       _Reserved_ LPDWORD lpcbCacheConfigInfo,
                       _In_ DWORD dwFieldControl);
BOOLAPI
GetUrlCacheConfigInfoW(_Inout_ LPINTERNET_CACHE_CONFIG_INFOW lpCacheConfigInfo,
                       _Reserved_ LPDWORD lpcbCacheConfigInfo,
                       _In_ DWORD dwFieldControl);
#ifdef UNICODE
...
BOOLAPI
SetUrlCacheConfigInfoA(_In_ LPINTERNET_CACHE_CONFIG_INFOA lpCacheConfigInfo,
                       _In_ DWORD dwFieldControl);
BOOLAPI
SetUrlCacheConfigInfoW(_In_ LPINTERNET_CACHE_CONFIG_INFOW lpCacheConfigInfo,
                       _In_ DWORD dwFieldControl);
#ifdef UNICODE
...
INTERNETAPI_(DWORD)
RunOnceUrlCache(_In_ HWND hwnd, _In_ HINSTANCE hinst, _In_ LPSTR lpszCmd,
                _In_ int nCmdShow);

INTERNETAPI_(DWORD)
DeleteIE3Cache(_In_ HWND hwnd, _In_ HINSTANCE hinst, _In_ LPSTR lpszCmd,
               _In_ int nCmdShow);
...
BOOLAPI
RegisterUrlCacheNotification(_In_opt_ HWND hWnd, _In_ UINT uMsg,
                             _In_ GROUPID gid, _In_ DWORD dwOpsFilter,
                             _In_ DWORD dwReserved);
...
BOOLAPI
GetUrlCacheContainerInfoA(_In_ LPSTR lpszUrlName,
                          _Out_writes_bytes_(*lpcbContainerInfo)
                              LPINTERNET_CACHE_CONTAINER_INFOA lpContainerInfo,
                          _Inout_ LPDWORD lpcbContainerInfo,
                          _In_ DWORD dwOptions);
BOOLAPI
GetUrlCacheContainerInfoW(_In_ LPWSTR lpszUrlName,
                          _Out_writes_bytes_(*lpcbContainerInfo)
                              LPINTERNET_CACHE_CONTAINER_INFOW lpContainerInfo,
                          _Inout_ LPDWORD lpcbContainerInfo,
                          _In_ DWORD dwOptions);
...
#ifdef _WINSOCK2API_
INTERNETAPI_(DWORD)
HttpGetTunnelSocket(_In_ HINTERNET hRequest, _Out_ SOCKET *pSocket,
                    _Outptr_result_buffer_all_maybenull_(*pdwDataLength)
                        PBYTE *ppbData,
...
INTERNETAPI_(DWORD)
AppCacheLookup(_In_ PCWSTR pwszUrl, _In_ DWORD dwFlags,
...
INTERNETAPI_(DWORD)
AppCacheCheckManifest(_In_opt_ PCWSTR pwszMasterUrl,
                      _In_ PCWSTR pwszManifestUrl,
                      _In_reads_bytes_(dwManifestDataSize)
                          const BYTE *pbManifestData,
                      _In_ DWORD dwManifestDataSize,
                      _In_reads_bytes_(dwManifestResponseHeadersSize)
...
INTERNETAPI_(DWORD)
AppCacheGetDownloadList(_In_ APP_CACHE_HANDLE hAppCache,
                        _Out_ APP_CACHE_DOWNLOAD_LIST *pDownloadList);

INTERNETAPI_(VOID)
AppCacheFreeDownloadList(_Inout_ APP_CACHE_DOWNLOAD_LIST *pDownloadList);
...
INTERNETAPI_(DWORD)
AppCacheFinalize(_In_ APP_CACHE_HANDLE hAppCache,
                 _In_reads_bytes_(dwManifestDataSize)
                     const BYTE *pbManifestData,
                 _In_ DWORD dwManifestDataSize,
...
INTERNETAPI_(DWORD)
AppCacheGetFallbackUrl(_In_ APP_CACHE_HANDLE hAppCache, _In_ PCWSTR pwszUrl,
                       _Outptr_result_z_ PWSTR *ppwszFallbackUrl);

INTERNETAPI_(DWORD)
AppCacheGetManifestUrl(_In_ APP_CACHE_HANDLE hAppCache,
                       _Outptr_result_z_ PWSTR *ppwszManifestUrl);

INTERNETAPI_(DWORD)
AppCacheDuplicateHandle(_In_ APP_CACHE_HANDLE hAppCache,
                        _Outptr_ APP_CACHE_HANDLE *phDuplicatedAppCache);

INTERNETAPI_(VOID)
AppCacheCloseHandle(_In_ APP_CACHE_HANDLE hAppCache);
...
INTERNETAPI_(VOID)
AppCacheFreeGroupList(_Inout_ APP_CACHE_GROUP_LIST *pAppCacheGroupList);

INTERNETAPI_(DWORD)
AppCacheGetGroupList(_Out_ APP_CACHE_GROUP_LIST *pAppCacheGroupList);

INTERNETAPI_(DWORD)
AppCacheGetInfo(_In_ APP_CACHE_HANDLE hAppCache,
                _Out_ APP_CACHE_GROUP_INFO *pAppCacheInfo);

INTERNETAPI_(DWORD)
AppCacheDeleteGroup(_In_ PCWSTR pwszManifestUrl);

INTERNETAPI_(DWORD)
AppCacheFreeSpace(_In_ FILETIME ftCutOff);

INTERNETAPI_(DWORD)
AppCacheGetIEGroupList(_Out_ APP_CACHE_GROUP_LIST *pAppCacheGroupList);

INTERNETAPI_(DWORD)
AppCacheDeleteIEGroup(_In_ PCWSTR pwszManifestUrl);

INTERNETAPI_(DWORD)
AppCacheFreeIESpace(_In_ FILETIME ftCutOff);

INTERNETAPI_(DWORD)
AppCacheCreateAndCommitFile(_In_ APP_CACHE_HANDLE hAppCache,
                            _In_ PCWSTR pwszSourceFilePath, _In_ PCWSTR pwszUrl,
                            _In_reads_bytes_(dwResponseHeadersSize)
...
INTERNETAPI_(DWORD)
HttpOpenDependencyHandle(_In_ HINTERNET hRequestHandle, _In_ BOOL fBackground,
                         _Outptr_ HTTP_DEPENDENCY_HANDLE *phDependencyHandle);

INTERNETAPI_(VOID)
HttpCloseDependencyHandle(_In_ HTTP_DEPENDENCY_HANDLE hDependencyHandle);
...
INTERNETAPI_(DWORD)
HttpIndicatePageLoadComplete(_In_ HTTP_DEPENDENCY_HANDLE hDependencyHandle);
...
URLCACHEAPI_(VOID)
UrlCacheFreeEntryInfo(_Inout_ PURLCACHE_ENTRY_INFO pCacheEntryInfo);

URLCACHEAPI
UrlCacheGetEntryInfo(_In_opt_ APP_CACHE_HANDLE hAppCache, _In_ PCWSTR pcwszUrl,
                     _Out_opt_ PURLCACHE_ENTRY_INFO pCacheEntryInfo);

URLCACHEAPI_(VOID)
UrlCacheCloseEntryHandle(_In_ URLCACHE_HANDLE hEntryFile);

URLCACHEAPI
UrlCacheRetrieveEntryFile(_In_opt_ APP_CACHE_HANDLE hAppCache,
                          _In_ PCWSTR pcwszUrl,
                          _Out_ PURLCACHE_ENTRY_INFO pCacheEntryInfo,
                          _Out_ URLCACHE_HANDLE *phEntryFile);

URLCACHEAPI
UrlCacheReadEntryStream(_In_ URLCACHE_HANDLE hUrlCacheStream,
                        _In_ ULONGLONG ullLocation, _Inout_ PVOID pBuffer,
                        _In_ DWORD dwBufferLen, _Out_ PDWORD pdwBufferLen);

URLCACHEAPI
UrlCacheRetrieveEntryStream(_In_opt_ APP_CACHE_HANDLE hAppCache,
                            _In_ PCWSTR pcwszUrl, _In_ BOOL fRandomRead,
                            _Out_ PURLCACHE_ENTRY_INFO pCacheEntryInfo,
                            _Out_ URLCACHE_HANDLE *phEntryStream);

URLCACHEAPI
UrlCacheUpdateEntryExtraData(_In_opt_ APP_CACHE_HANDLE hAppCache,
                             _In_ PCWSTR pcwszUrl,
                             _In_reads_bytes_(cbExtraData)
                                 const BYTE *pbExtraData,
                             _In_ DWORD cbExtraData);

URLCACHEAPI
UrlCacheCreateContainer(_In_z_ const WCHAR *pwszName,
                        _In_z_ const WCHAR *pwszPrefix,
                        _In_z_ const WCHAR *pwszDirectory,
                        _In_ ULONGLONG ullLimit, _In_ DWORD dwOptions);

URLCACHEAPI
UrlCacheCheckEntriesExist(_In_reads_(cEntries) PCWSTR *rgpwszUrls,
                          _In_ DWORD cEntries,
                          _Out_writes_(cEntries) BOOL *rgfExist);

URLCACHEAPI
UrlCacheGetContentPaths(_Outptr_result_buffer_(*pcDirectories)
...
URLCACHEAPI
UrlCacheGetGlobalLimit(_In_ URL_CACHE_LIMIT_TYPE limitType,
                       _Out_ ULONGLONG *pullLimit);

URLCACHEAPI
UrlCacheSetGlobalLimit(_In_ URL_CACHE_LIMIT_TYPE limitType,
...
URLCACHEAPI
UrlCacheContainerSetEntryMaximumAge(_In_z_ const WCHAR *pwszPrefix,
                                    _In_ DWORD dwEntryMaxAge);

URLCACHEAPI
UrlCacheFindFirstEntry(_In_opt_z_ const WCHAR *pwszPrefix, _In_ DWORD dwFlags,
                       _In_ DWORD dwFilter, _In_ GROUPID GroupId,
                       _Out_ PURLCACHE_ENTRY_INFO pCacheEntryInfo,
                       _Out_ HANDLE *phFind);

URLCACHEAPI
UrlCacheFindNextEntry(_In_ HANDLE hFind,
                      _Out_ PURLCACHE_ENTRY_INFO pCacheEntryInfo);
...
INTERNETAPI
InternetGetDialEngineW(_In_ LPWSTR pwzConnectoid, _In_ IDialEventSink *pdes,
                       _Out_ IDialEngine **ppde);

INTERNETAPI
InternetGetDialBrandingW(_In_ LPWSTR pwzConnectoid, _Out_ IDialBranding **ppdb);

BOOLAPI
ReadGuidsForConnectedNetworks(_Out_opt_ DWORD *pcNetworks,
...
#define INTERNET_AUTOPROXY_INIT_MASK                                           \
  (INTERNET_AUTOPROXY_INIT_DEFAULT | INTERNET_AUTOPROXY_INIT_DOWNLOADSYNC |    \
   INTERNET_AUTOPROXY_INIT_QUERYSTATE | INTERNET_AUTOPROXY_INIT_ONLYQUERY)
...
BOOLAPI IsHostInProxyBypassList(_In_ INTERNET_SCHEME tScheme,
                                _In_reads_(cchHost) LPCSTR lpszHost,
                                _In_ DWORD cchHost);

typedef struct _WININET_PROXY_INFO {
  BOOL fProxy;  // Is this a proxy or DIRECT?
  BOOL fBypass; // If DIRECT, is it bypassing a proxy (intranet) or is all
                // traffic DIRECT (internet)
  INTERNET_SCHEME ProxyScheme; // The scheme of the proxy, SOCKS, HTTP (CERN
                               // Proxy), HTTPS (SSL through Proxy)
...
INTERNETAPI_(VOID)
InternetFreeProxyInfoList(_Inout_ WININET_PROXY_INFO_LIST *pProxyInfoList);

INTERNETAPI_(DWORD)
InternetGetProxyForUrl(_In_ HINTERNET hInternet, _In_ PCWSTR pcwszUrl,
...
BOOL DoConnectoidsExist(
    void); // Returns TRUE if any RAS connectoids exist and FALSE otherwise

BOOLAPI GetDiskInfoA(_In_ PCSTR pszPath, _Out_opt_ PDWORD pdwClusterSize,
                     _Out_opt_ PDWORDLONG pdlAvail,
                     _Out_opt_ PDWORDLONG pdlTotal);

typedef BOOL (*CACHE_OPERATOR)(INTERNET_CACHE_ENTRY_INFO *pcei, PDWORD pcbcei,
                               PVOID pOpData);

BOOL PerformOperationOverUrlCacheA(
    _In_opt_ PCSTR pszUrlSearchPattern, _In_ DWORD dwFlags, _In_ DWORD dwFilter,
    _In_ GROUPID GroupId, _Reserved_ PVOID pReserved1,
    _Reserved_ PDWORD pdwReserved2, _Reserved_ PVOID pReserved3,
    _In_ CACHE_OPERATOR op, _Inout_ PVOID pOperatorData);

BOOL IsProfilesEnabled();

INTERNETAPI_(DWORD)
_GetFileExtensionFromUrl(_In_ LPSTR lpszUrl, _In_ DWORD dwFlags,
                         _Inout_updates_bytes_(*pcchExt) LPSTR lpszExt,
                         _Inout_ DWORD *pcchExt);

INTERNETAPI_(DWORD)
InternalInternetGetCookie(_In_ LPCSTR lpszUrl,
                          _Out_writes_(*lpdwDataSize) LPSTR lpszCookieData,
                          _Inout_ DWORD *lpdwDataSize);

//  in cookimp.cxx and cookexp.cxx
BOOLAPI ImportCookieFileA(_In_ LPCSTR szFilename);
//  in cookimp.cxx and cookexp.cxx
BOOLAPI ImportCookieFileW(_In_ LPCWSTR szFilename);
#ifdef UNICODE
#define ImportCookieFile ImportCookieFileW
#else
#define ImportCookieFile ImportCookieFileA
#endif // !UNICODE
BOOLAPI ExportCookieFileA(_In_ LPCSTR szFilename, _In_ BOOL fAppend);
BOOLAPI ExportCookieFileW(_In_ LPCWSTR szFilename, _In_ BOOL fAppend);
#ifdef UNICODE
...
BOOLAPI IsDomainLegalCookieDomainA( // in "wininet\http\cookie.cxx"
    _In_ LPCSTR pchDomain, _In_ LPCSTR pchFullDomain);
BOOLAPI IsDomainLegalCookieDomainW( // in "wininet\http\cookie.cxx"
    _In_ LPCWSTR pchDomain, _In_ LPCWSTR pchFullDomain);
#ifdef UNICODE
...
HttpWebSocketCompleteUpgrade(_In_ HINTERNET hRequest, _In_ DWORD_PTR dwContext);

BOOLAPI HttpWebSocketSend(_In_ HINTERNET hWebSocket,
                          _In_ HTTP_WEB_SOCKET_BUFFER_TYPE BufferType,
                          _In_reads_bytes_opt_(dwBufferLength) PVOID pvBuffer,
                          _In_ DWORD dwBufferLength);

BOOLAPI HttpWebSocketReceive(_In_ HINTERNET hWebSocket,
                             _Out_writes_bytes_to_(dwBufferLength,
                                                   *pdwBytesRead)
                                 PVOID pvBuffer,
                             _In_ DWORD dwBufferLength,
...
BOOLAPI HttpWebSocketClose(_In_ HINTERNET hWebSocket, _In_ USHORT usStatus,
...
BOOLAPI
HttpWebSocketShutdown(_In_ HINTERNET hWebSocket, _In_ USHORT usStatus,
                      _In_reads_bytes_opt_(dwReasonLength) PVOID pvReason,
                      _In_range_(0, HTTP_WEB_SOCKET_MAX_CLOSE_REASON_LENGTH)
                          DWORD dwReasonLength);

BOOLAPI HttpWebSocketQueryCloseStatus(
    _In_ HINTERNET hWebSocket, _Out_ USHORT *pusStatus,
    ...
    _Out_range_(0, HTTP_WEB_SOCKET_MAX_CLOSE_REASON_LENGTH)
...
typedef DWORD(CALLBACK *INTERNET_GLOBAL_NOTIFICATION_CALLBACK)(
    _In_ HINTERNET hInternet, _In_opt_ PVOID pvContext, _In_ PCSTR pcszUrl,
    _In_ PCSTR pcszHost, _In_ PSOCKADDR_STORAGE pRemoteAddress,
    _In_ DWORD dwNotification, _In_opt_ PVOID pvEventData,
...
InternetConvertUrlFromWireToWideChar(
    _In_reads_(cchUrl) PCSTR pcszUrl, _In_ DWORD cchUrl,
    _In_ PCWSTR pcwszBaseUrl, _In_ DWORD dwCodePageHost,
    _In_ DWORD dwCodePagePath, _In_ BOOL fEncodePathExtra,
    _In_ DWORD dwCodePageExtra, _Outptr_result_z_ PWSTR *ppwszConvertedUrl);

STDAPI_(DWORD)
HttpPreConnect(_In_ HANDLE hRequest, _In_ PCWSTR pwszUrl,
...
typedef enum { POLICY_EXTENSION_VERSION1 = 1 } HTTP_POLICY_EXTENSION_VERSION;

typedef DWORD(WINAPI *HTTP_POLICY_EXTENSION_INIT)(
    _In_ HTTP_POLICY_EXTENSION_VERSION Version,
    _In_ HTTP_POLICY_EXTENSION_TYPE Type, _In_ VOID *pvData, _In_ ULONG cbData);

typedef DWORD(WINAPI *HTTP_POLICY_EXTENSION_SHUTDOWN)(
