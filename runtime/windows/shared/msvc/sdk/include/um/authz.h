...
typedef struct _AUTHZ_ACCESS_REPLY {
  ...
  //
  // Array of SACL evaluation results.  This memory is allocated by the RM, if
  // SACL evaluation results are desired. Access check routines just fill in the
  // values. Sacl evaluation will only be performed if auditing is requested.
  //
...
typedef BOOL(CALLBACK *PFN_AUTHZ_DYNAMIC_ACCESS_CHECK)(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext, _In_ PACE_HEADER pAce,
    _In_opt_ PVOID pArgs, _Inout_ PBOOL pbAceApplicable);
...
//     pRestrictedSidAttrArray - To allocate and return an array of (sids,
//     attribute)
...
typedef BOOL(CALLBACK *PFN_AUTHZ_COMPUTE_DYNAMIC_GROUPS)(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext, _In_ PVOID Args,
    _Out_ PSID_AND_ATTRIBUTES *pSidAttrArray, _Out_ PDWORD pSidCount,
...
//         computation of access evaluation. If NULL, the default CAP is
//         applied.
//
// Returns
//     TRUE if the API succeeded.
...
typedef BOOL(CALLBACK *PFN_AUTHZ_GET_CENTRAL_ACCESS_POLICY)(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext, _In_ PSID capid,
    _In_opt_ PVOID pArgs, _Out_ PBOOL pCentralAccessPolicyApplicable,
...
BOOL WINAPI AuthzAccessCheck(
    _In_ DWORD Flags, _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
...
AUTHZAPI
BOOL WINAPI AuthzOpenObjectAudit(
    _In_ DWORD Flags, _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
    _In_ PAUTHZ_ACCESS_REQUEST pRequest,
...
AUTHZAPI
BOOL WINAPI
AuthzFreeHandle(_Inout_ AUTHZ_ACCESS_CHECK_RESULTS_HANDLE hAccessCheckResults);
...
//
// pfnDynamicAccessCheck - Pointer to the RM supplied access check function to
// be called when a callback ACE is encountered by the access check algorithm.
//
...
//
// pfnFreeCentralAccessPolicy - Pointer to the function to free the memory
// allocated by the pfnGetCentralAccessPolicy function.
//
...
AUTHZAPI
BOOL WINAPI AuthzInitializeResourceManagerEx(
    _In_opt_ DWORD Flags, _In_opt_ PAUTHZ_INIT_INFO pAuthzInitInfo,
    _Out_ PAUTHZ_RESOURCE_MANAGER_HANDLE phAuthzResourceManager);

AUTHZAPI
BOOL WINAPI AuthzInitializeRemoteResourceManager(
...
AUTHZAPI
BOOL WINAPI AuthzFreeResourceManager(
    _In_ AUTHZ_RESOURCE_MANAGER_HANDLE hAuthzResourceManager);

AUTHZAPI
BOOL WINAPI AuthzInitializeContextFromToken(
    _In_ DWORD Flags, _In_ HANDLE TokenHandle,
    _In_ AUTHZ_RESOURCE_MANAGER_HANDLE hAuthzResourceManager,
    _In_opt_ PLARGE_INTEGER pExpirationTime, _In_ LUID Identifier,
...
BOOL WINAPI AuthzInitializeContextFromSid(
    _In_ DWORD Flags, _In_ PSID UserSid,
    _In_ AUTHZ_RESOURCE_MANAGER_HANDLE hAuthzResourceManager,
    _In_opt_ PLARGE_INTEGER pExpirationTime, _In_ LUID Identifier,
    _In_opt_ PVOID DynamicGroupArgs,
    _Out_ PAUTHZ_CLIENT_CONTEXT_HANDLE phAuthzClientContext);
...
AUTHZAPI
BOOL WINAPI AuthzInitializeCompoundContext(
...
BOOL WINAPI AuthzAddSidsToContext(
    ...
    _In_opt_ PSID_AND_ATTRIBUTES Sids, _In_ DWORD SidCount,
    _In_opt_ PSID_AND_ATTRIBUTES RestrictedSids, _In_ DWORD RestrictedSidCount,
    _Out_ PAUTHZ_CLIENT_CONTEXT_HANDLE phNewAuthzClientContext);
...
AUTHZAPI
BOOL WINAPI AuthzModifySecurityAttributes(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
    _In_ _When_(pAttributes != NULL &&
                    *pOperations !=
                        AUTHZ_SECURITY_ATTRIBUTE_OPERATION_REPLACE_ALL,
                _In_reads_(pAttributes->AttributeCount))
...
#if (_WIN32_WINNT >= _WIN32_WINNT_WIN8)
AUTHZAPI
BOOL WINAPI AuthzModifyClaims(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
    _In_ AUTHZ_CONTEXT_INFORMATION_CLASS ClaimClass,
    _In_ _When_(pClaims != NULL &&
                    *pClaimOperations !=
                        AUTHZ_SECURITY_ATTRIBUTE_OPERATION_REPLACE_ALL,
                _In_reads_(pClaims->AttributeCount))
        PAUTHZ_SECURITY_ATTRIBUTE_OPERATION pClaimOperations,
    _In_opt_ PAUTHZ_SECURITY_ATTRIBUTES_INFORMATION pClaims);

AUTHZAPI
BOOL WINAPI AuthzModifySids(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
    _In_ AUTHZ_CONTEXT_INFORMATION_CLASS SidClass,
    _In_ _When_(pSids != NULL &&
                    *pSidOperations != AUTHZ_SID_OPERATION_REPLACE_ALL,
                _In_reads_(pSids->GroupCount))
        PAUTHZ_SID_OPERATION pSidOperations,
    _In_opt_ PTOKEN_GROUPS pSids);

AUTHZAPI
BOOL WINAPI AuthzSetAppContainerInformation(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
    _In_ PSID pAppContainerSid, _In_ DWORD CapabilityCount,
    _In_reads_opt_(CapabilityCount) PSID_AND_ATTRIBUTES pCapabilitySids);

#endif // (_WIN32_WINNT >= _WIN32_WINNT_WIN8)

AUTHZAPI
BOOL WINAPI AuthzGetInformationFromContext(
    _In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext,
    _In_ AUTHZ_CONTEXT_INFORMATION_CLASS InfoClass, _In_ DWORD BufferSize,
    _Out_ PDWORD pSizeRequired, _Out_ PVOID Buffer);

AUTHZAPI
BOOL WINAPI
AuthzFreeContext(_In_ AUTHZ_CLIENT_CONTEXT_HANDLE hAuthzClientContext);
...
AUTHZAPI
BOOL WINAPI AuthzInitializeObjectAccessAuditEvent(
    _In_ DWORD Flags, _In_ AUTHZ_AUDIT_EVENT_TYPE_HANDLE hAuditEventType,
    _In_ PWSTR szOperationType, _In_ PWSTR szObjectType,
    _In_ PWSTR szObjectName, _In_ PWSTR szAdditionalInfo,
    _Out_ PAUTHZ_AUDIT_EVENT_HANDLE phAuditEvent,
    _In_ DWORD dwAdditionalParameterCount, ...);

AUTHZAPI
BOOL WINAPI AuthzInitializeObjectAccessAuditEvent2(
    _In_ DWORD Flags, _In_ AUTHZ_AUDIT_EVENT_TYPE_HANDLE hAuditEventType,
    _In_ PWSTR szOperationType, _In_ PWSTR szObjectType,
    _In_ PWSTR szObjectName, _In_ PWSTR szAdditionalInfo,
    _In_ PWSTR szAdditionalInfo2, _Out_ PAUTHZ_AUDIT_EVENT_HANDLE phAuditEvent,
    _In_ DWORD dwAdditionalParameterCount, ...);
...
AUTHZAPI
BOOL WINAPI AuthzFreeAuditEvent(_In_ AUTHZ_AUDIT_EVENT_HANDLE hAuditEvent);
...
AUTHZAPI
BOOL WINAPI
AuthzEvaluateSacl(_In_ AUTHZ_CLIENT_CONTEXT_HANDLE AuthzClientContext,
                  _In_ PAUTHZ_ACCESS_REQUEST pRequest, _In_ PACL Sacl,
                  _In_ ACCESS_MASK GrantedAccess, _In_ BOOL AccessGranted,
...
BOOL WINAPI AuthzInstallSecurityEventSource(
    _In_ DWORD dwFlags, _In_ PAUTHZ_SOURCE_SCHEMA_REGISTRATION pRegistration);

AUTHZAPI
BOOL WINAPI AuthzUninstallSecurityEventSource(_In_ DWORD dwFlags,
                                              _In_ PCWSTR szEventSourceName);

AUTHZAPI
BOOL WINAPI AuthzEnumerateSecurityEventSources(
    _In_ DWORD dwFlags, _Out_ PAUTHZ_SOURCE_SCHEMA_REGISTRATION Buffer,
    _Out_ PDWORD pdwCount, _Inout_ PDWORD pdwLength);

AUTHZAPI
BOOL WINAPI AuthzRegisterSecurityEventSource(
    _In_ DWORD dwFlags, _In_ PCWSTR szEventSourceName,
...
BOOL WINAPI AuthzReportSecurityEvent(
    ...
    _In_ DWORD dwAuditId, _In_opt_ PSID pUserSid, _In_ DWORD dwCount, ...);

AUTHZAPI
BOOL WINAPI AuthzReportSecurityEventFromParams(
    _In_ DWORD dwFlags,
    _Inout_ AUTHZ_SECURITY_EVENT_PROVIDER_HANDLE hEventProvider,
    _In_ DWORD dwAuditId, _In_opt_ PSID pUserSid, _In_ PAUDIT_PARAMS pParams);

#if (_WIN32_WINNT >= _WIN32_WINNT_WIN8)
_Success_(return != FALSE) AUTHZAPI BOOL WINAPI
...
AUTHZAPI
BOOL WINAPI AuthzUnregisterCapChangeNotification(
