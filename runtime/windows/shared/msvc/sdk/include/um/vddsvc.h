...
#define ICA_MASTER 0
#define ICA_SLAVE                                                              \
  1 // Can't PC that one since it has been published forever, but use next
    // define
#define ICA_SUBORDINATE 1

#define VDDSimulateInterrupt(ms, line, count)                                  \
  (call_ica_hw_interrupt)(ms, line, 1)
...
VOID call_ica_hw_interrupt(int ms, BYTE line, int count);
...
PBYTE
MGetVdmPointer(ULONG Address, ULONG Size, UCHAR ProtectedMode);

#else

PBYTE
Sim32GetVDMPointer(ULONG Address, ULONG Size, UCHAR ProtectedMode);

BOOLEAN
Sim32FlushVDMPointer(ULONG Address, USHORT Size, PBYTE Buffer,
