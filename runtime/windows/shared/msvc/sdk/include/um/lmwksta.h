...
NET_API_STATUS NET_API_FUNCTION NetWkstaGetInfo(
    _In_opt_ LMSTR servername OPTIONAL, _In_ DWORD level,
    _Outptr_opt_result_buffer_(_Inexpressible_("size varies with level"))
        OUT LPBYTE *bufptr);

NET_API_STATUS NET_API_FUNCTION
NetWkstaSetInfo(_In_opt_ LMSTR servername OPTIONAL, _In_ DWORD level,
...
NET_API_STATUS NET_API_FUNCTION NetWkstaUserGetInfo(
    _In_opt_ LMSTR reserved, _In_ DWORD level,
    _Outptr_result_buffer_(_Inexpressible_("varies")) OUT LPBYTE *bufptr);

NET_API_STATUS NET_API_FUNCTION
NetWkstaUserSetInfo(_In_opt_ LMSTR reserved, _In_ DWORD level,
                    _In_reads_(_Inexpressible_("varies")) LPBYTE buf,
                    _Out_opt_ LPDWORD parm_err OPTIONAL);

NET_API_STATUS NET_API_FUNCTION
NetWkstaUserEnum(_In_opt_ LMSTR servername OPTIONAL, IN DWORD level,
                 _Out_opt_ LPBYTE *bufptr, IN DWORD prefmaxlen,
                 _Out_opt_ LPDWORD entriesread, _Out_ LPDWORD totalentries,
                 _Inout_opt_ LPDWORD resumehandle OPTIONAL);

NET_API_STATUS NET_API_FUNCTION NetWkstaTransportAdd(
    _In_opt_ LPTSTR servername, _In_ DWORD level,
    _When_(level == 0, _In_reads_bytes_(sizeof(WKSTA_TRANSPORT_INFO_0)))
        LPBYTE buf,
    _Out_opt_ LPDWORD parm_err);

NET_API_STATUS NET_API_FUNCTION
NetWkstaTransportDel(_In_opt_ IN LMSTR servername OPTIONAL,
                     _In_opt_ IN LMSTR transportname, IN DWORD ucond);

_Check_return_ _Success_(return == NERR_Success || return == ERROR_MORE_DATA)
