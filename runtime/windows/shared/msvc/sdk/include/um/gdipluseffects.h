...
static const GUID HueSaturationLightnessEffectGuid = {
    ...
    0xeb07,
    0x4d87,
    {0xa5, 0xf0, 0x71, 0x8, 0xe2, 0x6a, 0x9c, 0x5f}};
...
static const GUID LevelsEffectGuid = {
    ...
    0x2a31,
    0x4f3a,
    {0x8c, 0x34, 0x17, 0xa8, 0x3, 0xb3, 0x3a, 0x25}};
...
Status __stdcall GdipSetEffectParameters(CGpEffect *effect, const VOID *params,
                                         const UINT size);

Status __stdcall GdipGetEffectParameters(CGpEffect *effect, UINT *size,
                                         VOID *params);
}
...
public:
  ...
  INT GetAuxDataSize() const { return auxDataSize; }

  VOID *GetAuxData() const { return auxData; }

  VOID UseAuxData(const BOOL useAuxDataFlag) { useAuxData = useAuxDataFlag; }
...
public:
  ...
  Blur() { GdipCreateEffect(BlurEffectGuid, &nativeEffect); }
...
public:
  Sharpen() { GdipCreateEffect(SharpenEffectGuid, &nativeEffect); }
...
public:
  Levels() { GdipCreateEffect(LevelsEffectGuid, &nativeEffect); }
...
public:
  Tint() { GdipCreateEffect(TintEffectGuid, &nativeEffect); }
...
public:
  ColorBalance() { GdipCreateEffect(ColorBalanceEffectGuid, &nativeEffect); }
...
public:
  ...
  ColorLUT() { GdipCreateEffect(ColorLUTEffectGuid, &nativeEffect); }
...
public:
  ColorCurve() { GdipCreateEffect(ColorCurveEffectGuid, &nativeEffect); }
