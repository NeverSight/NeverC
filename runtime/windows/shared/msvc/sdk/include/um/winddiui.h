...
LONG WINAPI DrvDevicePropertySheets(_In_opt_ PPROPSHEETUI_INFO pPSUIInfo,
                                    LPARAM lParam);
...
LONG WINAPI DrvDocumentPropertySheets(_In_opt_ PPROPSHEETUI_INFO pPSUIInfo,
                                      LPARAM lParam);
...
_Success_(return == 1) BOOL WINAPI
    DrvConvertDevMode(_In_ LPTSTR pPrinterName, _In_ PDEVMODE pdmIn,
                      _Out_writes_bytes_(*pcbNeeded) PDEVMODE pdmOut,
                      _Inout_ PLONG pcbNeeded, _In_ DWORD fMode);
...
BOOL WINAPI DevQueryPrintEx(_Inout_ PDEVQUERYPRINT_INFO pDQPInfo);
...
#define DOCUMENTEVENT_CREATEDCPRE                                              \
  1 // in-pszDriver, pszDevice, pdm, bIC, out-ppdm
...
int WINAPI DrvDocumentEvent(HANDLE hPrinter, HDC hdc, int iEsc, ULONG cbIn,
                            _In_reads_bytes_(cbIn) PVOID pvIn, ULONG cbOut,
...
BOOL WINAPI DrvPrinterEvent(_In_ LPWSTR pPrinterName, int DriverEvent,
...
BOOL WINAPI DrvDriverEvent(
    DWORD dwDriverEvent, _In_range_(1, 3) DWORD dwLevel,
...
//
// DrvQueryJobAttributes is called by the spooler(print processor) to get
// information about the printing options used with the job. These options
// include N-up and reverse order printing.
//
BOOL WINAPI DrvQueryJobAttributes(
    _In_ HANDLE hPrinter, _In_ PDEVMODE pDevMode,
    ...
    _When_(dwLevel == 1, _Out_writes_bytes_(sizeof(ATTRIBUTE_INFO_1)))
        _When_(dwLevel == 2, _Out_writes_bytes_(sizeof(ATTRIBUTE_INFO_2)))
            _When_(dwLevel == 3, _Out_writes_bytes_(sizeof(ATTRIBUTE_INFO_3)))
                _When_(dwLevel == 4,
                       _Out_writes_bytes_(sizeof(ATTRIBUTE_INFO_4)))
                    LPBYTE lpAttributeInfo);

//
// DrvQueryColorProfile is called by the GDI (graphics device interface) to get
// information about the default color profile for the given DEVMODE, used with
// ICM (image color management).
//
BOOL WINAPI DrvQueryColorProfile(_In_ HANDLE hPrinter, _In_ PDEVMODEW pdevmode,
                                 _In_ ULONG ulQueryMode,
                                 _Out_writes_(*pcbProfileData)
                                     VOID *pvProfileData,
...
#define QCP_PROFILEMEMORY                                                      \
  0x0001 // The pvProfileData points the color profile data itself.
#define QCP_PROFILEDISK                                                        \
  0x0002 // The pvProfileData points the color profile file name in Unicode.
...
__drv_preferredFunction("DrvDocumentEvent", "Obsolete") HANDLE WINAPI
    DrvSplStartDoc(HANDLE hPrinter, DWORD JobId);

__drv_preferredFunction("DrvDocumentEvent", "Obsolete") BOOL WINAPI
    DrvSplWritePrinter(HANDLE hDriver, LPVOID pBuf, DWORD cbBuf,
                       LPDWORD pcWritten);

__drv_preferredFunction("DrvDocumentEvent", "Obsolete") VOID WINAPI
...
__drv_preferredFunction("DrvDocumentEvent", "Obsolete") BOOL WINAPI
    DrvSplStartPage(HANDLE hDriver);

__drv_preferredFunction("DrvDocumentEvent", "Obsolete") BOOL WINAPI
    DrvSplEndPage(HANDLE hDriver);
...
DWORD
DrvSplDeviceCaps(HANDLE hPrinter, _In_ PWSTR pszDeviceName, WORD Capability,
                 _At_((TCHAR *)pOutput, _Out_writes_opt_(cchBufSize))
                     PVOID pOutput,
                 DWORD cchBufSize, _In_opt_ PDEVMODE pDevmode);

DWORD
WINAPI
DrvDeviceCapabilities(
    HANDLE hPrinter, _In_ PWSTR pszDeviceName, WORD Capability,
    _Out_writes_to_opt_(
        _Inexpressible_("Size of the buffer is dependent on Capability"),
        _Inexpressible_("Amount filled is dependent on Capability"))
        PVOID pOutput,
