...
#define MAKE_BDA_S_ITF_HRESULT(x)                                              \
  MAKE_HRESULT(SEVERITY_SUCCESS, FACILITY_NULL, (x))
#define MAKE_BDA_E_ITF_HRESULT(x)                                              \
  ((MAKE_HRESULT(SEVERITY_ERROR, FACILITY_ITF, (x))) |                         \
   0x40000000) // setting bit 30 to 1
...
const HRESULT BDA_E_IPNETWORK_ADDRESS_NOT_FOUND =
    MAKE_BDA_E_ITF_HRESULT(0x5002);
...
#define IS_BDAERROR(Status)                                                    \
  ((((unsigned long)(Status)) >> 31 == SEVERITY_ERROR) &&                      \
   ((((Status) >> 16) & 0x1fff) == FACILITY_ITF) &&                            \
   (((Status) & 0x40000000) == 0x40000000)) // bit 30 has to be 1

#define IS_BDASUCCESS(Status)                                                  \
  ((((unsigned long)(Status)) >> 31 == SEVERITY_SUCCESS) &&                    \
   ((((Status) >> 16) & 0x1fff) == FACILITY_NULL))
