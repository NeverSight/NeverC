...
#define MAPI_ITNEF_METHODS(IPURE)                                              \
  MAPIMETHOD(AddProps)                                                         \
  (THIS_ ULONG ulFlags, ULONG ulElemID, LPVOID lpvData,                        \
   LPSPropTagArray lpPropList) IPURE;                                          \
  MAPIMETHOD(ExtractProps)                                                     \
  (THIS_ ULONG ulFlags, LPSPropTagArray lpPropList,                            \
   LPSTnefProblemArray FAR * lpProblems) IPURE;                                \
  MAPIMETHOD(Finish)                                                           \
  (THIS_ ULONG ulFlags, WORD FAR * lpKey,                                      \
   LPSTnefProblemArray FAR * lpProblems) IPURE;                                \
  MAPIMETHOD(OpenTaggedBody)                                                   \
  (THIS_ LPMESSAGE lpMessage, ULONG ulFlags, LPSTREAM FAR * lppStream) IPURE;  \
  MAPIMETHOD(SetProps)                                                         \
  (THIS_ ULONG ulFlags, ULONG ulElemID, ULONG cValues, LPSPropValue lpProps)   \
      IPURE;                                                                   \
  MAPIMETHOD(EncodeRecips)                                                     \
  (THIS_ ULONG ulFlags, LPMAPITABLE lpRecipientTable) IPURE;                   \
  MAPIMETHOD(FinishComponent)                                                  \
  (THIS_ ULONG ulFlags, ULONG ulComponentID, LPSPropTagArray lpCustomPropList, \
   LPSPropValue lpCustomProps, LPSPropTagArray lpPropList,                     \
   LPSTnefProblemArray FAR * lpProblems) IPURE;
...
_Check_return_ STDMETHODIMP OpenTnefStream(LPVOID lpvSupport, LPSTREAM lpStream,
                                           _In_ LPTSTR lpszStreamName,
                                           ULONG ulFlags, LPMESSAGE lpMessage,
                                           WORD wKeyVal,
                                           LPITNEF FAR *lppTNEF) WIN_NOEXCEPT;

typedef _Check_return_ HRESULT(STDMETHODCALLTYPE FAR *LPOPENTNEFSTREAM)(
    LPVOID lpvSupport, LPSTREAM lpStream, _In_ LPTSTR lpszStreamName,
    ULONG ulFlags, LPMESSAGE lpMessage, WORD wKeyVal, LPITNEF FAR *lppTNEF);

_Check_return_ STDMETHODIMP OpenTnefStreamEx(
    LPVOID lpvSupport, LPSTREAM lpStream, _In_ LPTSTR lpszStreamName,
    ULONG ulFlags, LPMESSAGE lpMessage, WORD wKeyVal, LPADRBOOK lpAdressBook,
    LPITNEF FAR *lppTNEF) WIN_NOEXCEPT;
...
STDMETHODIMP GetTnefStreamCodepage(LPSTREAM lpStream, ULONG FAR *lpulCodepage,
                                   ULONG FAR *lpulSubCodepage) WIN_NOEXCEPT;

typedef HRESULT(STDMETHODCALLTYPE FAR *LPGETTNEFSTREAMCODEPAGE)(
    LPSTREAM lpStream, ULONG FAR *lpulCodepage, ULONG FAR *lpulSubCodepage);
...
#define attAttachModifyDate                                                    \
  ATT(atpDate, 0x8013)                       /* PR_LAST_MODIFICATION_TIME      \
                                              */
#define attDateModified ATT(atpDate, 0x8020) /* PR_LAST_MODIFICATION_TIME */
#define attAttachTransportFilename                                             \
  ATT(atpByte, 0x9001) /* PR_ATTACH_TRANSPORT_NAME */
...
#define attOriginalMessageClass                                                \
  ATT(atpWord, 0x0006) /* PR_ORIG_MESSAGE_CLASS                                \
                        */

#define attOwner                                                               \
  ATT(atpByte,                                                                 \
      0x0000)                               /* PR_RCVD_REPRESENTING_xxx  or    \
