...
//
// NT4 version of setupapi   (_WIN32_WINNT_NT4) is earliest, and installed onto
// Win95 by IE. Win2k version of setupapi (_WIN32_WINNT_WIN2K) also shipped in
// WinME we'll use "0x0410" to indicate version of setupapi shipped with Win98
//
#ifndef _SETUPAPI_VER
#if defined(_WIN32_WINNT) &&                                                   \
    (!defined(_WIN32_WINDOWS) || (_WIN32_WINNT < _WIN32_WINDOWS))
#define _SETUPAPI_VER                                                          \
  _WIN32_WINNT // SetupAPI version follows Windows NT version
#elif defined(_WIN32_WINDOWS)
#if _WIN32_WINDOWS >= 0x0490
#define _SETUPAPI_VER                                                          \
  _WIN32_WINNT_WIN2K // WinME uses same version of SetupAPI as Win2k
...
#define SP_ALTPLATFORM_FLAGS_VERSION_RANGE                                     \
  (0x0001) // FirstValidatedMajor/MinorVersion
#endif
...
//
//
#define INF_STYLE_CACHE_ENABLE                                                 \
  0x00000010 // always cache INF, even outside of %windir%\Inf
...
//
typedef UINT(CALLBACK *PSP_FILE_CALLBACK_A)(_In_ PVOID Context,
                                            _In_ UINT Notification,
                                            _In_ UINT_PTR Param1,
                                            _In_ UINT_PTR Param2);

typedef UINT(CALLBACK *PSP_FILE_CALLBACK_W)(_In_ PVOID Context,
                                            _In_ UINT Notification,
                                            _In_ UINT_PTR Param1,
                                            _In_ UINT_PTR Param2);
...
#define COPYFLG_NOVERSIONCHECK                                                 \
  0x00000004 // ignore versions and overwrite target
...
#define COPYFLG_OVERWRITE_OLDER_ONLY                                           \
  0x00000040 // leave target alone if version same as source
#define COPYFLG_PROTECTED_WINDOWS_DRIVER_FILE                                  \
  0x00000100 // a Windows driver file to be
...
#define COPYFLG_IN_USE_TRY_RENAME                                              \
  0x00004000 // If file in use, try to rename the target first
...
// Flag to indicate the setting properties for this Device (or class) caused a
// change so the Dev Mgr UI probably needs to be updatd.
...
// SP_DEVINSTALL_PARAMS.DriverPath should be searched.
#define DI_ENUMSINGLEINF 0x00010000L

// Flag that prevents ConfigMgr from removing/re-enumerating devices during
// device registration, installation, and deletion.
#define DI_DONOTCALLCONFIGMG 0x00020000L

// The following flag can be used to install a device disabled
#define DI_INSTALLDISABLED 0x00040000L

// Flag that causes SetupDiBuildDriverInfoList to build a device's compatible
// driver list from its existing class driver list, instead of the normal INF
// search.
...
// The setupx flag, DI_NOSYNCPROCESSING (0x00400000L) is not support in the
// Setup APIs.
...
#define DI_USECI_SELECTSTRINGS                                                 \
  0x08000000L // Use Class Installer Provided strings in the Select Device Dlg
...
#define DI_FLAGSEX_RESERVED2                                                   \
  0x00000001L // DI_FLAGSEX_USEOLDINFSEARCH is obsolete
#define DI_FLAGSEX_RESERVED3                                                   \
  0x00000002L // DI_FLAGSEX_AUTOSELECTRANK0 is obsolete
#define DI_FLAGSEX_CI_FAILED 0x00000004L // Failed to Load/Call class installer

#if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN //

#define DI_FLAGSEX_FINISHINSTALL_ACTION                                        \
  0x00000008L // Class/co-installer wants to get a DIF_FINISH_INSTALL action in
              // client context.
...
#define DI_FLAGSEX_RESERVED4                                                   \
  0x00004000L // DI_FLAGSEX_OLDINF_IN_CLASSLIST is obsolete

#define DI_FLAGSEX_NO_DRVREG_MODIFY                                            \
  0x00008000L // Don't run AddReg and DelReg for device's software (driver) key.
#define DI_FLAGSEX_IN_SYSTEM_SETUP                                             \
...
#define DI_FLAGSEX_FILTERSIMILARDRIVERS                                        \
  0x02000000L // only include similar drivers in class list
...
#define DI_FLAGSEX_NO_CLASSLIST_NODE_MERGE                                     \
  0x08000000L // Don't remove identical driver nodes from the class list
...
#define DI_FLAGSEX_RECURSIVESEARCH                                             \
  0x40000000L // Tell SetupDiBuildDriverInfoList to do a recursive search
#define DI_FLAGSEX_SEARCH_PUBLISHED_INFS                                       \
  0x80000000L // Tell SetupDiBuildDriverInfoList to do a "published INF" search
...
//
typedef BOOL(CALLBACK *PDETECT_PROGRESS_NOTIFY)(_In_ PVOID ProgressNotifyParam,
...
//
// Driver information structure (member of a driver info list that may be
...
typedef PSP_DRVINFO_DATA_V2_A PSP_DRVINFO_DATA_V2;
    ...
    (_SETUPAPI_VER <                                                           \
     _WIN32_WINNT_WIN2K) // use version 1 driver info data structure
...
#define DNF_NODRIVER                                                           \
  0x00000008 // if we want to install no driver
             // (e.g no mouse drv)
#define DNF_LEGACYINF                                                          \
  0x00000010 // Driver node came from an old-style INF (obsolete)
#define DNF_CLASS_DRIVER 0x00000020 // Driver node represents a class driver
#define DNF_COMPATIBLE_DRIVER                                                  \
  0x00000040                       // Driver node represents a compatible driver
...
#define DNF_OEM_F6_INF                                                         \
  0x00004000 // INF specified from F6 during textmode setup.
#define DNF_DUPDRIVERVER                                                       \
  0x00008000 // Multipe drivers have the same desc, provider, and DriverVer
             // values
...
#if _SETUPAPI_VER >= _WIN32_WINNT_WS03
#define DNF_AUTHENTICODE_SIGNED                                                \
  0x00020000 // Inf file is signed by an Authenticode(tm) catalog.
#endif       // _SETUPAPI_VER >= _WIN32_WINNT_WS03

#if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN
#define DNF_INSTALLEDDRIVER                                                    \
  0x00040000 // This driver node is currently installed on the device.
#define DNF_ALWAYSEXCLUDEFROMLIST                                              \
  0x00080000 // If set, this driver is not even displayed in
             // alternative platform either.
#define DNF_INBOX_DRIVER                                                       \
  0x00100000 // This driver node came from an INF that shipped with Windows.
#endif       // _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

#if _SETUPAPI_VER >= _WIN32_WINNT_WIN7
#define DNF_REQUESTADDITIONALSOFTWARE                                          \
  0x00200000 // This driver is only part of a software solution needed
             // by this device
...
typedef DWORD(CALLBACK *PSP_DETSIG_CMPPROC)(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA NewDeviceData,
    _In_ PSP_DEVINFO_DATA ExistingDeviceData, _In_opt_ PVOID CompareContext);
...
// ExtensionPropSheetPageProc entry point in setupapi.dll or to
// "EnumPropPages32" or "BasicProperties32" entry points provided by
// class/device property page providers.  Used to retrieve a handle (or,
// potentially, multiple handles) to property pages for a specified property
// page type.
//
...
#define SPPSR_ENUM_BASIC_DEVICE_PROPERTIES                                     \
  2 // supplied by device's BasicProperties32 provider
#define SPPSR_ENUM_ADV_DEVICE_PROPERTIES                                       \
  3 // supplied by class and/or device's EnumPropPages32 provider
...
// compatibility with Windows 2000/Windows 95/98/ME SETUPAPI.DLL
//
...
typedef PSP_BACKUP_QUEUE_PARAMS_V2_A PSP_BACKUP_QUEUE_PARAMS_V2;
    ...
    (_SETUPAPI_VER <                                                           \
     _WIN32_WINNT_WINXP) // use version 1 driver info data structure
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupGetInfInformationA(_In_ LPCVOID InfSpec, _In_ DWORD SearchControl,
                            _Out_writes_bytes_to_opt_(ReturnBufferSize,
                                                      *RequiredSize)
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupGetInfInformationW(_In_ LPCVOID InfSpec, _In_ DWORD SearchControl,
                            _Out_writes_bytes_to_opt_(ReturnBufferSize,
                                                      *RequiredSize)
                                PSP_INF_INFORMATION ReturnBuffer,
                            _In_ DWORD ReturnBufferSize,
...
WINSETUPAPI
BOOL WINAPI SetupQueryInfFileInformationA(
    _In_ PSP_INF_INFORMATION InfInformation, _In_ UINT InfIndex,
    _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
    _In_ DWORD ReturnBufferSize, _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupQueryInfFileInformationW(
    _In_ PSP_INF_INFORMATION InfInformation, _In_ UINT InfIndex,
...
BOOL WINAPI SetupQueryInfVersionInformationA(
    _In_ PSP_INF_INFORMATION InfInformation, _In_ UINT InfIndex,
    _In_opt_ PCSTR Key, _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
    _In_ DWORD ReturnBufferSize, _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupQueryInfVersionInformationW(
    _In_ PSP_INF_INFORMATION InfInformation, _In_ UINT InfIndex,
    _In_opt_ PCWSTR Key, _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
    _In_ DWORD ReturnBufferSize, _Out_opt_ PDWORD RequiredSize);
...
BOOL WINAPI SetupGetInfDriverStoreLocationA(
    _In_ PCSTR FileName, _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
    _In_opt_ PCSTR LocaleName, _Out_writes_(ReturnBufferSize) PSTR ReturnBuffer,
    _In_ DWORD ReturnBufferSize, _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupGetInfDriverStoreLocationW(
    _In_ PCWSTR FileName, _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
...
WINSETUPAPI
BOOL WINAPI SetupGetInfPublishedNameA(_In_ PCSTR DriverStoreLocation,
                                      _Out_writes_(ReturnBufferSize)
                                          PSTR ReturnBuffer,
                                      _In_ DWORD ReturnBufferSize,
                                      _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupGetInfPublishedNameW(_In_ PCWSTR DriverStoreLocation,
                                      _Out_writes_(ReturnBufferSize)
                                          PWSTR ReturnBuffer,
                                      _In_ DWORD ReturnBufferSize,
                                      _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupGetInfFileListA(_In_opt_ PCSTR DirectoryPath,
                                 _In_ DWORD InfStyle,
...
WINSETUPAPI
BOOL WINAPI SetupGetInfFileListW(_In_opt_ PCWSTR DirectoryPath,
                                 _In_ DWORD InfStyle,
                                 _Out_writes_(ReturnBufferSize)
                                     PWSTR ReturnBuffer,
                                 _In_ DWORD ReturnBufferSize,
...
WINSETUPAPI
HINF WINAPI SetupOpenInfFileW(_In_ PCWSTR FileName, _In_opt_ PCWSTR InfClass,
                              _In_ DWORD InfStyle, _Out_opt_ PUINT ErrorLine);

WINSETUPAPI
HINF WINAPI SetupOpenInfFileA(_In_ PCSTR FileName, _In_opt_ PCSTR InfClass,
                              _In_ DWORD InfStyle, _Out_opt_ PUINT ErrorLine);
...
WINSETUPAPI
BOOL WINAPI SetupOpenAppendInfFileW(_In_opt_ PCWSTR FileName,
                                    _In_ HINF InfHandle,
                                    _Out_opt_ PUINT ErrorLine);

WINSETUPAPI
BOOL WINAPI SetupOpenAppendInfFileA(_In_opt_ PCSTR FileName,
                                    _In_ HINF InfHandle,
                                    _Out_opt_ PUINT ErrorLine);
...
WINSETUPAPI
VOID WINAPI SetupCloseInfFile(_In_ HINF InfHandle);

WINSETUPAPI
BOOL WINAPI SetupFindFirstLineA(_In_ HINF InfHandle, _In_ PCSTR Section,
                                _In_opt_ PCSTR Key, _Out_ PINFCONTEXT Context);

WINSETUPAPI
BOOL WINAPI SetupFindFirstLineW(_In_ HINF InfHandle, _In_ PCWSTR Section,
                                _In_opt_ PCWSTR Key, _Out_ PINFCONTEXT Context);
...
WINSETUPAPI
BOOL WINAPI SetupFindNextLine(_In_ PINFCONTEXT ContextIn,
                              _Out_ PINFCONTEXT ContextOut);

WINSETUPAPI
BOOL WINAPI SetupFindNextMatchLineA(_In_ PINFCONTEXT ContextIn,
                                    _In_opt_ PCSTR Key,
                                    _Out_ PINFCONTEXT ContextOut);

WINSETUPAPI
BOOL WINAPI SetupFindNextMatchLineW(_In_ PINFCONTEXT ContextIn,
                                    _In_opt_ PCWSTR Key,
                                    _Out_ PINFCONTEXT ContextOut);
...
WINSETUPAPI
BOOL WINAPI SetupGetLineByIndexA(_In_ HINF InfHandle, _In_ PCSTR Section,
                                 _In_ DWORD Index, _Out_ PINFCONTEXT Context);

WINSETUPAPI
BOOL WINAPI SetupGetLineByIndexW(_In_ HINF InfHandle, _In_ PCWSTR Section,
                                 _In_ DWORD Index, _Out_ PINFCONTEXT Context);
...
WINSETUPAPI
LONG WINAPI SetupGetLineCountA(_In_ HINF InfHandle, _In_ PCSTR Section);

WINSETUPAPI
LONG WINAPI SetupGetLineCountW(_In_ HINF InfHandle, _In_ PCWSTR Section);
...
WINSETUPAPI
BOOL WINAPI SetupGetLineTextA(_In_opt_ PINFCONTEXT Context,
                              _In_opt_ HINF InfHandle, _In_opt_ PCSTR Section,
                              _In_opt_ PCSTR Key,
                              _Out_writes_opt_(ReturnBufferSize)
                                  PSTR ReturnBuffer,
                              _In_ DWORD ReturnBufferSize,
...
WINSETUPAPI
BOOL WINAPI SetupGetLineTextW(_In_opt_ PINFCONTEXT Context,
                              _In_opt_ HINF InfHandle, _In_opt_ PCWSTR Section,
                              _In_opt_ PCWSTR Key,
                              _Out_writes_opt_(ReturnBufferSize)
                                  PWSTR ReturnBuffer,
                              _In_ DWORD ReturnBufferSize,
...
DWORD
WINAPI
SetupGetFieldCount(_In_ PINFCONTEXT Context);

WINSETUPAPI
BOOL WINAPI SetupGetStringFieldA(_In_ PINFCONTEXT Context,
...
WINSETUPAPI
BOOL WINAPI SetupGetStringFieldW(_In_ PINFCONTEXT Context,
                                 _In_ DWORD FieldIndex,
                                 _Out_writes_opt_(ReturnBufferSize)
                                     PWSTR ReturnBuffer,
                                 _In_ DWORD ReturnBufferSize,
...
WINSETUPAPI
BOOL WINAPI SetupGetIntField(_In_ PINFCONTEXT Context, _In_ DWORD FieldIndex,
                             _Out_ PINT IntegerValue);

WINSETUPAPI
BOOL WINAPI SetupGetMultiSzFieldA(_In_ PINFCONTEXT Context,
                                  _In_ DWORD FieldIndex,
                                  _Out_writes_opt_(ReturnBufferSize)
                                      PSTR ReturnBuffer,
                                  _In_ DWORD ReturnBufferSize,
...
WINSETUPAPI
BOOL WINAPI SetupGetMultiSzFieldW(_In_ PINFCONTEXT Context,
                                  _In_ DWORD FieldIndex,
                                  _Out_writes_opt_(ReturnBufferSize)
                                      PWSTR ReturnBuffer,
                                  _In_ DWORD ReturnBufferSize,
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupGetBinaryField(_In_ PINFCONTEXT Context, _In_ DWORD FieldIndex,
                        _Out_writes_bytes_to_opt_(ReturnBufferSize,
                                                  *RequiredSize)
                            PBYTE ReturnBuffer,
                        _In_ DWORD ReturnBufferSize,
...
DWORD
WINAPI
SetupGetFileCompressionInfoA(_In_ PCSTR SourceFileName,
...
DWORD
WINAPI
SetupGetFileCompressionInfoW(_In_ PCWSTR SourceFileName,
...
WINSETUPAPI
BOOL WINAPI SetupGetFileCompressionInfoExA(
    _In_ PCSTR SourceFileName,
    _In_reads_opt_(ActualSourceFileNameBufferLen)
        PSTR ActualSourceFileNameBuffer,
    _In_ DWORD ActualSourceFileNameBufferLen,
    _Out_opt_ PDWORD RequiredBufferLen, _Out_ PDWORD SourceFileSize,
    _Out_ PDWORD TargetFileSize, _Out_ PUINT CompressionType);

WINSETUPAPI
BOOL WINAPI SetupGetFileCompressionInfoExW(
...
DWORD
WINAPI
SetupDecompressOrCopyFileA(_In_ PCSTR SourceFileName, _In_ PCSTR TargetFileName,
                           _In_opt_ PUINT CompressionType);

WINSETUPAPI
DWORD
WINAPI
SetupDecompressOrCopyFileW(_In_ PCWSTR SourceFileName,
...
BOOL WINAPI SetupGetSourceFileLocationA(
    _In_ HINF InfHandle, _In_opt_ PINFCONTEXT InfContext,
    _In_opt_ PCSTR FileName, _Out_ PUINT SourceId,
    _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
    _In_ DWORD ReturnBufferSize, _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupGetSourceFileLocationW(
    _In_ HINF InfHandle, _In_opt_ PINFCONTEXT InfContext,
    _In_opt_ PCWSTR FileName, _Out_ PUINT SourceId,
    _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
    _In_ DWORD ReturnBufferSize, _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupGetSourceFileSizeA(_In_ HINF InfHandle,
...
WINSETUPAPI
BOOL WINAPI SetupGetSourceFileSizeW(_In_ HINF InfHandle,
                                    _In_opt_ PINFCONTEXT InfContext,
...
WINSETUPAPI
BOOL WINAPI SetupGetTargetPathA(_In_ HINF InfHandle,
                                _In_opt_ PINFCONTEXT InfContext,
                                _In_opt_ PCSTR Section,
                                _Out_writes_opt_(ReturnBufferSize)
                                    PSTR ReturnBuffer,
...
WINSETUPAPI
BOOL WINAPI SetupGetTargetPathW(_In_ HINF InfHandle,
                                _In_opt_ PINFCONTEXT InfContext,
                                _In_opt_ PCWSTR Section,
                                _Out_writes_opt_(ReturnBufferSize)
                                    PWSTR ReturnBuffer,
...
WINSETUPAPI
BOOL WINAPI SetupSetSourceListA(_In_ DWORD Flags,
                                _In_reads_(SourceCount) PCSTR *SourceList,
                                _In_ UINT SourceCount);

WINSETUPAPI
BOOL WINAPI SetupSetSourceListW(_In_ DWORD Flags,
...
WINSETUPAPI
BOOL WINAPI SetupAddToSourceListA(_In_ DWORD Flags, _In_ PCSTR Source);

WINSETUPAPI
BOOL WINAPI SetupAddToSourceListW(_In_ DWORD Flags, _In_ PCWSTR Source);
...
WINSETUPAPI
BOOL WINAPI SetupRemoveFromSourceListA(_In_ DWORD Flags, _In_ PCSTR Source);

WINSETUPAPI
BOOL WINAPI SetupRemoveFromSourceListW(_In_ DWORD Flags, _In_ PCWSTR Source);
...
WINSETUPAPI
BOOL WINAPI SetupQuerySourceListA(_In_ DWORD Flags,
                                  _Outptr_result_buffer_(*Count) PCSTR **List,
                                  _Out_ PUINT Count);

WINSETUPAPI
BOOL WINAPI SetupQuerySourceListW(_In_ DWORD Flags,
...
WINSETUPAPI
BOOL WINAPI SetupFreeSourceListA(_Inout_ _At_(*List, _Pre_readable_size_(Count)
                                                         _Post_null_)
                                     PCSTR **List,
                                 _In_ UINT Count);

WINSETUPAPI
BOOL WINAPI SetupFreeSourceListW(_Inout_ _At_(*List, _Pre_readable_size_(Count)
                                                         _Post_null_)
                                     PCWSTR **List,
                                 _In_ UINT Count);
...
WINSETUPAPI
UINT WINAPI SetupPromptForDiskA(
    _In_ HWND hwndParent, _In_opt_ PCSTR DialogTitle, _In_opt_ PCSTR DiskName,
    _In_opt_ PCSTR PathToSource, _In_ PCSTR FileSought, _In_opt_ PCSTR TagFile,
    _In_ DWORD DiskPromptStyle,
    _Out_writes_opt_(PathBufferSize) PSTR PathBuffer, _In_ DWORD PathBufferSize,
    _Out_opt_ PDWORD PathRequiredSize);

WINSETUPAPI
UINT WINAPI SetupPromptForDiskW(
    _In_ HWND hwndParent, _In_opt_ PCWSTR DialogTitle, _In_opt_ PCWSTR DiskName,
    _In_opt_ PCWSTR PathToSource, _In_ PCWSTR FileSought,
    _In_opt_ PCWSTR TagFile, _In_ DWORD DiskPromptStyle,
    _Out_writes_opt_(PathBufferSize) PWSTR PathBuffer,
    _In_ DWORD PathBufferSize, _Out_opt_ PDWORD PathRequiredSize);
...
WINSETUPAPI
UINT WINAPI SetupCopyErrorA(_In_ HWND hwndParent, _In_opt_ PCSTR DialogTitle,
                            _In_opt_ PCSTR DiskName, _In_ PCSTR PathToSource,
                            _In_ PCSTR SourceFile,
                            _In_opt_ PCSTR TargetPathFile,
                            _In_ UINT Win32ErrorCode, _In_ DWORD Style,
                            _Out_writes_opt_(PathBufferSize) PSTR PathBuffer,
                            _In_ DWORD PathBufferSize,
                            _Out_opt_ PDWORD PathRequiredSize);

WINSETUPAPI
UINT WINAPI SetupCopyErrorW(_In_ HWND hwndParent, _In_opt_ PCWSTR DialogTitle,
                            _In_opt_ PCWSTR DiskName, _In_ PCWSTR PathToSource,
...
WINSETUPAPI
UINT WINAPI SetupRenameErrorA(_In_ HWND hwndParent, _In_opt_ PCSTR DialogTitle,
                              _In_ PCSTR SourceFile, _In_ PCSTR TargetFile,
                              _In_ UINT Win32ErrorCode, _In_ DWORD Style);

WINSETUPAPI
UINT WINAPI SetupRenameErrorW(_In_ HWND hwndParent, _In_opt_ PCWSTR DialogTitle,
                              _In_ PCWSTR SourceFile, _In_ PCWSTR TargetFile,
                              _In_ UINT Win32ErrorCode, _In_ DWORD Style);
...
WINSETUPAPI
UINT WINAPI SetupDeleteErrorA(_In_ HWND hwndParent, _In_opt_ PCSTR DialogTitle,
                              _In_ PCSTR File, _In_ UINT Win32ErrorCode,
                              _In_ DWORD Style);

WINSETUPAPI
UINT WINAPI SetupDeleteErrorW(_In_ HWND hwndParent, _In_opt_ PCWSTR DialogTitle,
                              _In_ PCWSTR File, _In_ UINT Win32ErrorCode,
                              _In_ DWORD Style);
...
WINSETUPAPI
UINT WINAPI SetupBackupErrorA(_In_ HWND hwndParent, _In_opt_ PCSTR DialogTitle,
                              _In_ PCSTR SourceFile, _In_opt_ PCSTR TargetFile,
                              _In_ UINT Win32ErrorCode, _In_ DWORD Style);

WINSETUPAPI
UINT WINAPI SetupBackupErrorW(_In_ HWND hwndParent, _In_opt_ PCWSTR DialogTitle,
                              _In_ PCWSTR SourceFile,
...
WINSETUPAPI
BOOL WINAPI SetupSetDirectoryIdA(_In_ HINF InfHandle, _In_ DWORD Id,
                                 _In_opt_ PCSTR Directory);

WINSETUPAPI
BOOL WINAPI SetupSetDirectoryIdW(_In_ HINF InfHandle, _In_ DWORD Id,
...
WINSETUPAPI
BOOL WINAPI SetupSetDirectoryIdExA(_In_ HINF InfHandle, _In_ DWORD Id,
                                   _In_opt_ PCSTR Directory, _In_ DWORD Flags,
                                   _Reserved_ DWORD Reserved1,
                                   _Reserved_ PVOID Reserved2);

WINSETUPAPI
BOOL WINAPI SetupSetDirectoryIdExW(_In_ HINF InfHandle, _In_ DWORD Id,
                                   _In_opt_ PCWSTR Directory, _In_ DWORD Flags,
                                   _Reserved_ DWORD Reserved1,
                                   _Reserved_ PVOID Reserved2);
...
WINSETUPAPI
BOOL WINAPI SetupGetSourceInfoA(_In_ HINF InfHandle, _In_ UINT SourceId,
...
WINSETUPAPI
BOOL WINAPI SetupGetSourceInfoW(_In_ HINF InfHandle, _In_ UINT SourceId,
                                _In_ UINT InfoDesired,
                                _Out_writes_opt_(ReturnBufferSize)
                                    PWSTR ReturnBuffer,
                                _In_ DWORD ReturnBufferSize,
...
//
#define SRCINFO_TAGFILE2                                                       \
  5 // alternate tagfile, when SRCINFO_TAGFILE is a cabfile

#define SRC_FLAGS_CABFILE                                                      \
  (0x0010) // if set, treat SRCINFO_TAGFILE as a cabfile and specify alternate
           // tagfile

#endif // _SETUPAPI_VER >= _WIN32_WINNT_WINXP

WINSETUPAPI
BOOL WINAPI SetupInstallFileA(
    _In_opt_ HINF InfHandle, _In_opt_ PINFCONTEXT InfContext,
    _In_opt_ PCSTR SourceFile, _In_opt_ PCSTR SourcePathRoot,
    _In_opt_ PCSTR DestinationName, _In_ DWORD CopyStyle,
    _In_opt_ PSP_FILE_CALLBACK_A CopyMsgHandler, _In_opt_ PVOID Context);

WINSETUPAPI
BOOL WINAPI SetupInstallFileW(
    _In_opt_ HINF InfHandle, _In_opt_ PINFCONTEXT InfContext,
    _In_opt_ PCWSTR SourceFile, _In_opt_ PCWSTR SourcePathRoot,
    _In_opt_ PCWSTR DestinationName, _In_ DWORD CopyStyle,
    _In_opt_ PSP_FILE_CALLBACK_W CopyMsgHandler, _In_opt_ PVOID Context);
...
WINSETUPAPI
BOOL WINAPI
SetupInstallFileExA(_In_opt_ HINF InfHandle, _In_opt_ PINFCONTEXT InfContext,
                    _In_opt_ PCSTR SourceFile, _In_opt_ PCSTR SourcePathRoot,
                    _In_opt_ PCSTR DestinationName, _In_ DWORD CopyStyle,
                    _In_opt_ PSP_FILE_CALLBACK_A CopyMsgHandler,
                    _In_opt_ PVOID Context, _Out_ PBOOL FileWasInUse);

WINSETUPAPI
BOOL WINAPI
SetupInstallFileExW(_In_opt_ HINF InfHandle, _In_opt_ PINFCONTEXT InfContext,
                    _In_opt_ PCWSTR SourceFile, _In_opt_ PCWSTR SourcePathRoot,
                    _In_opt_ PCWSTR DestinationName, _In_ DWORD CopyStyle,
...
#define SP_COPY_REPLACEONLY                                                    \
  0x0000002 // copy only if target file already present
#define SP_COPY_NEWER                                                          \
  0x0000004 // copy only if source newer than or same as target
...
#define SP_COPY_LANGUAGEAWARE                                                  \
  0x0000020 // don't overwrite file of different language
...
#define SP_COPY_IN_USE_NEEDS_REBOOT                                            \
  0x0000100                            // System needs reboot if file in use
...
#define SP_FLAG_CABINETCONTINUATION                                            \
  0x0000800 // Used with need media notification
#define SP_COPY_FORCE_NOOVERWRITE                                              \
  0x0001000 // like NOOVERWRITE but no callback nofitication
#define SP_COPY_FORCE_NEWER 0x0002000 // like NEWER but no callback nofitication
#define SP_COPY_WARNIFSKIP                                                     \
  0x0004000 // system critical file: warn if user tries to skip
#define SP_COPY_NOBROWSE                                                       \
  0x0008000 // Browsing is disallowed for this file or section
#define SP_COPY_NEWER_ONLY                                                     \
  0x0010000 // copy only if source file newer than target
#define SP_COPY_RESERVED                                                       \
  0x0020000 // was: SP_COPY_SOURCE_SIS_MASTER (deprecated)
...
#define SP_COPY_IN_USE_TRY_RENAME                                              \
  0x4000000 // If file in use, try to rename the target first
...
#define SP_BACKUP_BOOTFILE                                                     \
  0x00000008 // file marked with COPYFLG_REPLACE_BOOT_FILE
...
WINSETUPAPI
BOOL WINAPI SetupCloseFileQueue(_In_ HSPFILEQ QueueHandle);

WINSETUPAPI
BOOL WINAPI SetupSetFileQueueAlternatePlatformA(
...
WINSETUPAPI
BOOL WINAPI SetupSetPlatformPathOverrideA(_In_opt_ PCSTR Override);

WINSETUPAPI
BOOL WINAPI SetupSetPlatformPathOverrideW(_In_opt_ PCWSTR Override);
...
WINSETUPAPI
BOOL WINAPI
SetupQueueCopyA(_In_ HSPFILEQ QueueHandle, _In_opt_ PCSTR SourceRootPath,
                _In_opt_ PCSTR SourcePath, _In_ PCSTR SourceFilename,
                _In_opt_ PCSTR SourceDescription, _In_opt_ PCSTR SourceTagfile,
                _In_ PCSTR TargetDirectory, _In_opt_ PCSTR TargetFilename,
                _In_ DWORD CopyStyle);

WINSETUPAPI
BOOL WINAPI
SetupQueueCopyW(_In_ HSPFILEQ QueueHandle, _In_opt_ PCWSTR SourceRootPath,
                _In_opt_ PCWSTR SourcePath, _In_ PCWSTR SourceFilename,
                _In_opt_ PCWSTR SourceDescription,
                _In_opt_ PCWSTR SourceTagfile, _In_ PCWSTR TargetDirectory,
                _In_opt_ PCWSTR TargetFilename, _In_ DWORD CopyStyle);
...
WINSETUPAPI
BOOL WINAPI SetupQueueCopyIndirectA(_In_ PSP_FILE_COPY_PARAMS_A CopyParams);

WINSETUPAPI
BOOL WINAPI SetupQueueCopyIndirectW(_In_ PSP_FILE_COPY_PARAMS_W CopyParams);
...
WINSETUPAPI
BOOL WINAPI SetupQueueDefaultCopyA(_In_ HSPFILEQ QueueHandle,
                                   _In_ HINF InfHandle,
...
WINSETUPAPI
BOOL WINAPI SetupQueueDefaultCopyW(_In_ HSPFILEQ QueueHandle,
                                   _In_ HINF InfHandle,
...
WINSETUPAPI
BOOL WINAPI SetupQueueCopySectionA(_In_ HSPFILEQ QueueHandle,
                                   _In_opt_ PCSTR SourceRootPath,
                                   _In_ HINF InfHandle,
                                   _In_opt_ HINF ListInfHandle,
                                   _In_ PCSTR Section, _In_ DWORD CopyStyle);

WINSETUPAPI
BOOL WINAPI SetupQueueCopySectionW(_In_ HSPFILEQ QueueHandle,
                                   _In_opt_ PCWSTR SourceRootPath,
                                   _In_ HINF InfHandle,
                                   _In_opt_ HINF ListInfHandle,
                                   _In_ PCWSTR Section, _In_ DWORD CopyStyle);
...
WINSETUPAPI
BOOL WINAPI SetupQueueDeleteA(_In_ HSPFILEQ QueueHandle, _In_ PCSTR PathPart1,
                              _In_opt_ PCSTR PathPart2);

WINSETUPAPI
BOOL WINAPI SetupQueueDeleteW(_In_ HSPFILEQ QueueHandle, _In_ PCWSTR PathPart1,
...
WINSETUPAPI
BOOL WINAPI SetupQueueDeleteSectionA(_In_ HSPFILEQ QueueHandle,
                                     _In_ HINF InfHandle,
                                     _In_opt_ HINF ListInfHandle,
                                     _In_ PCSTR Section);

WINSETUPAPI
BOOL WINAPI SetupQueueDeleteSectionW(_In_ HSPFILEQ QueueHandle,
                                     _In_ HINF InfHandle,
                                     _In_opt_ HINF ListInfHandle,
                                     _In_ PCWSTR Section);
...
WINSETUPAPI
BOOL WINAPI SetupQueueRenameA(_In_ HSPFILEQ QueueHandle, _In_ PCSTR SourcePath,
                              _In_opt_ PCSTR SourceFilename,
                              _In_opt_ PCSTR TargetPath,
                              _In_ PCSTR TargetFilename);

WINSETUPAPI
BOOL WINAPI SetupQueueRenameW(_In_ HSPFILEQ QueueHandle, _In_ PCWSTR SourcePath,
...
WINSETUPAPI
BOOL WINAPI SetupQueueRenameSectionA(_In_ HSPFILEQ QueueHandle,
                                     _In_ HINF InfHandle,
                                     _In_opt_ HINF ListInfHandle,
                                     _In_ PCSTR Section);

WINSETUPAPI
BOOL WINAPI SetupQueueRenameSectionW(_In_ HSPFILEQ QueueHandle,
                                     _In_ HINF InfHandle,
                                     _In_opt_ HINF ListInfHandle,
                                     _In_ PCWSTR Section);
...
WINSETUPAPI
BOOL WINAPI SetupCommitFileQueueA(_In_opt_ HWND Owner,
                                  _In_ HSPFILEQ QueueHandle,
                                  _In_ PSP_FILE_CALLBACK_A MsgHandler,
                                  _In_ PVOID Context);

WINSETUPAPI
BOOL WINAPI SetupCommitFileQueueW(_In_opt_ HWND Owner,
                                  _In_ HSPFILEQ QueueHandle,
...
WINSETUPAPI
BOOL WINAPI SetupScanFileQueueA(_In_ HSPFILEQ FileQueue, _In_ DWORD Flags,
...
WINSETUPAPI
BOOL WINAPI SetupScanFileQueueW(_In_ HSPFILEQ FileQueue, _In_ DWORD Flags,
                                _In_opt_ HWND Window,
...
//
#define SPQ_DELAYED_COPY                                                       \
  0x00000001 // file was in use; registered for delayed copy

#if _SETUPAPI_VER >= _WIN32_WINNT_WINXP

WINSETUPAPI
BOOL WINAPI SetupGetFileQueueCount(_In_ HSPFILEQ FileQueue,
                                   _In_ UINT SubQueueFileOp,
                                   _Out_ PUINT NumOperations);

WINSETUPAPI
BOOL WINAPI SetupGetFileQueueFlags(_In_ HSPFILEQ FileQueue, _Out_ PDWORD Flags);

WINSETUPAPI
BOOL WINAPI SetupSetFileQueueFlags(_In_ HSPFILEQ FileQueue, _In_ DWORD FlagMask,
                                   _In_ DWORD Flags);

//
// Flags/FlagMask for use with SetupSetFileQueueFlags and returned by
...
#define SPQ_FLAG_DO_SHUFFLEMOVE                                                \
  0x00000008 // If set then always do a shuffle move. A shuffle
...
#define SPQ_FLAG_VALID                                                         \
  0x0000000F // mask of valid flags (can be passed as FlagMask)
...
BOOL WINAPI SetupCopyOEMInfA(
    _In_ PCSTR SourceInfFileName, _In_opt_ PCSTR OEMSourceMediaLocation,
    _In_ DWORD OEMSourceMediaType, _In_ DWORD CopyStyle,
    _Out_writes_opt_(DestinationInfFileNameSize) PSTR DestinationInfFileName,
    _In_ DWORD DestinationInfFileNameSize, _Out_opt_ PDWORD RequiredSize,
    _Out_opt_ PSTR *DestinationInfFileNameComponent);

WINSETUPAPI
BOOL WINAPI SetupCopyOEMInfW(
    _In_ PCWSTR SourceInfFileName, _In_opt_ PCWSTR OEMSourceMediaLocation,
    _In_ DWORD OEMSourceMediaType, _In_ DWORD CopyStyle,
...
#if _SETUPAPI_VER >= _WIN32_WINNT_WINXP
WINSETUPAPI
BOOL WINAPI SetupUninstallOEMInfA(_In_ PCSTR InfFileName, _In_ DWORD Flags,
                                  _Reserved_ PVOID Reserved);

WINSETUPAPI
BOOL WINAPI SetupUninstallOEMInfW(_In_ PCWSTR InfFileName, _In_ DWORD Flags,
                                  _Reserved_ PVOID Reserved);
...
WINSETUPAPI
BOOL WINAPI SetupUninstallNewlyCopiedInfs(_In_ HSPFILEQ FileQueue,
                                          _In_ DWORD Flags,
                                          _Reserved_ PVOID Reserved);
...
HDSKSPC
WINAPI
SetupCreateDiskSpaceListA(_Reserved_ PVOID Reserved1,
                          _Reserved_ DWORD Reserved2, _In_ UINT Flags);

WINSETUPAPI
HDSKSPC
WINAPI
SetupCreateDiskSpaceListW(_Reserved_ PVOID Reserved1,
                          _Reserved_ DWORD Reserved2, _In_ UINT Flags);
...
#define SPDSL_IGNORE_DISK                                                      \
  0x00000001 // ignore deletes and on-disk files in copies
#define SPDSL_DISALLOW_NEGATIVE_ADJUST 0x00000002

WINSETUPAPI
HDSKSPC
WINAPI
SetupDuplicateDiskSpaceListA(_In_ HDSKSPC DiskSpace, _Reserved_ PVOID Reserved1,
                             _Reserved_ DWORD Reserved2, _In_ UINT Flags);

WINSETUPAPI
HDSKSPC
WINAPI
SetupDuplicateDiskSpaceListW(_In_ HDSKSPC DiskSpace, _Reserved_ PVOID Reserved1,
                             _Reserved_ DWORD Reserved2, _In_ UINT Flags);
...
WINSETUPAPI
BOOL WINAPI SetupDestroyDiskSpaceList(_Inout_ HDSKSPC DiskSpace);

WINSETUPAPI
BOOL WINAPI SetupQueryDrivesInDiskSpaceListA(_In_ HDSKSPC DiskSpace,
                                             _Out_writes_opt_(ReturnBufferSize)
                                                 PSTR ReturnBuffer,
                                             _In_ DWORD ReturnBufferSize,
                                             _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupQueryDrivesInDiskSpaceListW(_In_ HDSKSPC DiskSpace,
                                             _Out_writes_opt_(ReturnBufferSize)
                                                 PWSTR ReturnBuffer,
                                             _In_ DWORD ReturnBufferSize,
                                             _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupQuerySpaceRequiredOnDriveA(_In_ HDSKSPC DiskSpace,
...
WINSETUPAPI
BOOL WINAPI SetupQuerySpaceRequiredOnDriveW(_In_ HDSKSPC DiskSpace,
...
WINSETUPAPI
BOOL WINAPI SetupAdjustDiskSpaceListA(_In_ HDSKSPC DiskSpace,
...
WINSETUPAPI
BOOL WINAPI SetupAdjustDiskSpaceListW(_In_ HDSKSPC DiskSpace,
...
WINSETUPAPI
BOOL WINAPI SetupAddToDiskSpaceListA(
    _In_ HDSKSPC DiskSpace, _In_ PCSTR TargetFilespec, _In_ LONGLONG FileSize,
    _In_ UINT Operation, _Reserved_ PVOID Reserved1, _Reserved_ UINT Reserved2);

WINSETUPAPI
BOOL WINAPI SetupAddToDiskSpaceListW(
    _In_ HDSKSPC DiskSpace, _In_ PCWSTR TargetFilespec, _In_ LONGLONG FileSize,
    _In_ UINT Operation, _Reserved_ PVOID Reserved1, _Reserved_ UINT Reserved2);
...
BOOL WINAPI SetupAddSectionToDiskSpaceListA(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF ListInfHandle,
    _In_ PCSTR SectionName, _In_ UINT Operation, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);

WINSETUPAPI
BOOL WINAPI SetupAddSectionToDiskSpaceListW(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF ListInfHandle,
    _In_ PCWSTR SectionName, _In_ UINT Operation, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);
...
BOOL WINAPI SetupAddInstallSectionToDiskSpaceListA(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF LayoutInfHandle,
    _In_ PCSTR SectionName, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);

WINSETUPAPI
BOOL WINAPI SetupAddInstallSectionToDiskSpaceListW(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF LayoutInfHandle,
    _In_ PCWSTR SectionName, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);
...
WINSETUPAPI
BOOL WINAPI SetupRemoveFromDiskSpaceListA(_In_ HDSKSPC DiskSpace,
...
WINSETUPAPI
BOOL WINAPI SetupRemoveFromDiskSpaceListW(_In_ HDSKSPC DiskSpace,
...
WINSETUPAPI
BOOL WINAPI SetupRemoveSectionFromDiskSpaceListA(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF ListInfHandle,
    _In_ PCSTR SectionName, _In_ UINT Operation, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);

WINSETUPAPI
BOOL WINAPI SetupRemoveSectionFromDiskSpaceListW(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF ListInfHandle,
    _In_ PCWSTR SectionName, _In_ UINT Operation, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);
...
WINSETUPAPI
BOOL WINAPI SetupRemoveInstallSectionFromDiskSpaceListA(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF LayoutInfHandle,
    _In_ PCSTR SectionName, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);

WINSETUPAPI
BOOL WINAPI SetupRemoveInstallSectionFromDiskSpaceListW(
    _In_ HDSKSPC DiskSpace, _In_ HINF InfHandle, _In_opt_ HINF LayoutInfHandle,
    _In_ PCWSTR SectionName, _Reserved_ PVOID Reserved1,
    _Reserved_ UINT Reserved2);
...
WINSETUPAPI
BOOL WINAPI SetupIterateCabinetA(_In_ PCSTR CabinetFile,
                                 _Reserved_ DWORD Reserved,
                                 _In_ PSP_FILE_CALLBACK_A MsgHandler,
                                 _In_ PVOID Context);

WINSETUPAPI
BOOL WINAPI SetupIterateCabinetW(_In_ PCWSTR CabinetFile,
                                 _Reserved_ DWORD Reserved,
                                 _In_ PSP_FILE_CALLBACK_W MsgHandler,
                                 _In_ PVOID Context);
...
WINSETUPAPI
INT WINAPI SetupPromptReboot(_In_opt_ HSPFILEQ FileQueue, _In_opt_ HWND Owner,
                             _In_ BOOL ScanOnly);
...
PVOID
WINAPI
SetupInitDefaultQueueCallback(_In_opt_ HWND OwnerWindow);

WINSETUPAPI
PVOID
WINAPI
SetupInitDefaultQueueCallbackEx(_In_opt_ HWND OwnerWindow,
...
WINSETUPAPI
VOID WINAPI SetupTermDefaultQueueCallback(_In_ PVOID Context);

WINSETUPAPI
UINT WINAPI SetupDefaultQueueCallbackA(_In_ PVOID Context,
                                       _In_ UINT Notification,
                                       _In_ UINT_PTR Param1,
                                       _In_ UINT_PTR Param2);

WINSETUPAPI
UINT WINAPI SetupDefaultQueueCallbackW(_In_ PVOID Context,
                                       _In_ UINT Notification,
                                       _In_ UINT_PTR Param1,
                                       _In_ UINT_PTR Param2);
...
#define FLG_ADDREG_DELREG_BIT                                                  \
  (0x00008000) // if set, interpret as DELREG, see below
...
#define FLG_ADDREG_64BITKEY                                                    \
  (0x00001000) // make this change in the 64 bit registry.
#define FLG_ADDREG_KEYONLY_COMMON                                              \
  (0x00002000) // same as FLG_ADDREG_KEYONLY but also works for DELREG
#define FLG_ADDREG_32BITKEY                                                    \
  (0x00004000) // make this change in the 32 bit registry.
...
//
// In SetupX and some versions of SetupAPI, <Operation> will be ignored and
// <ValueName> will be deleted. Use with care.
//
// The bits determined by mask FLG_DELREG_TYPE_MASK indicates type of data
// expected. <Operation> must have FLG_ADDREG_DELREG_BIT set, otherwise it is
// ignored and specified value will be deleted (allowing an AddReg section to
// also be used as a DelReg section) if <Operation> is not specified,
// <ValueName> will be deleted (if specified) otherwise <SubKey> will be
// deleted.
//
...
//               removes all entries matching <String> (case ignored) from
//               multi-sz registry value
//

#define FLG_DELREG_OPERATION_MASK (0x000000FE)
#define FLG_DELREG_MULTI_SZ_DELSTRING                                          \
  (FLG_DELREG_TYPE_MULTI_SZ | FLG_ADDREG_DELREG_BIT | 0x00000002) // 0x00018002
...
WINSETUPAPI
BOOL WINAPI SetupInstallFromInfSectionA(
    _In_opt_ HWND Owner, _In_ HINF InfHandle, _In_ PCSTR SectionName,
    _In_ UINT Flags, _In_opt_ HKEY RelativeKeyRoot,
    _In_opt_ PCSTR SourceRootPath, _In_ UINT CopyFlags,
    _In_opt_ PSP_FILE_CALLBACK_A MsgHandler, _In_opt_ PVOID Context,
    _In_opt_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData);

WINSETUPAPI
BOOL WINAPI SetupInstallFromInfSectionW(
    _In_opt_ HWND Owner, _In_ HINF InfHandle, _In_ PCWSTR SectionName,
    _In_ UINT Flags, _In_opt_ HKEY RelativeKeyRoot,
    _In_opt_ PCWSTR SourceRootPath, _In_ UINT CopyFlags,
    _In_opt_ PSP_FILE_CALLBACK_W MsgHandler, _In_opt_ PVOID Context,
    _In_opt_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData);
...
WINSETUPAPI
BOOL WINAPI SetupInstallFilesFromInfSectionA(
    _In_ HINF InfHandle, _In_opt_ HINF LayoutInfHandle, _In_ HSPFILEQ FileQueue,
    _In_ PCSTR SectionName, _In_opt_ PCSTR SourceRootPath, _In_ UINT CopyFlags);

WINSETUPAPI
BOOL WINAPI SetupInstallFilesFromInfSectionW(_In_ HINF InfHandle,
...
WINSETUPAPI
BOOL WINAPI SetupInstallServicesFromInfSectionExA(
    _In_ HINF InfHandle, _In_ PCSTR SectionName, _In_ DWORD Flags,
    _In_opt_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _Reserved_ PVOID Reserved1, _Reserved_ PVOID Reserved2);

WINSETUPAPI
BOOL WINAPI SetupInstallServicesFromInfSectionExW(
    _In_ HINF InfHandle, _In_ PCWSTR SectionName, _In_ DWORD Flags,
    _In_opt_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _Reserved_ PVOID Reserved1, _Reserved_ PVOID Reserved2);
...
//
VOID WINAPI InstallHinfSectionA(_In_ HWND Window, _In_ HINSTANCE ModuleHandle,
                                _In_ PCSTR CommandLine, _In_ INT ShowCommand);

VOID WINAPI InstallHinfSectionW(_In_ HWND Window, _In_ HINSTANCE ModuleHandle,
                                _In_ PCWSTR CommandLine, _In_ INT ShowCommand);
...
HSPFILELOG
WINAPI
SetupInitializeFileLogA(_In_opt_ PCSTR LogFileName, _In_ DWORD Flags);

WINSETUPAPI
HSPFILELOG
WINAPI
SetupInitializeFileLogW(_In_opt_ PCWSTR LogFileName, _In_ DWORD Flags);
...
#define SPFILELOG_SYSTEMLOG                                                    \
  0x00000001                          // use system log -- must be Administrator
#define SPFILELOG_FORCENEW 0x00000002 // not valid with SPFILELOG_SYSTEMLOG
#define SPFILELOG_QUERYONLY                                                    \
  0x00000004 // allows non-administrators to read system log

WINSETUPAPI
BOOL WINAPI SetupTerminateFileLog(_In_ HSPFILELOG FileLogHandle);

WINSETUPAPI
BOOL WINAPI SetupLogFileA(_In_ HSPFILELOG FileLogHandle,
                          _In_opt_ PCSTR LogSectionName,
                          _In_ PCSTR SourceFilename, _In_ PCSTR TargetFilename,
                          _In_ DWORD Checksum, _In_opt_ PCSTR DiskTagfile,
                          _In_opt_ PCSTR DiskDescription,
                          _In_opt_ PCSTR OtherInfo, _In_ DWORD Flags);

WINSETUPAPI
BOOL WINAPI SetupLogFileW(_In_ HSPFILELOG FileLogHandle,
                          _In_opt_ PCWSTR LogSectionName,
                          _In_ PCWSTR SourceFilename,
                          _In_ PCWSTR TargetFilename, _In_ DWORD Checksum,
                          _In_opt_ PCWSTR DiskTagfile,
                          _In_opt_ PCWSTR DiskDescription,
                          _In_opt_ PCWSTR OtherInfo, _In_ DWORD Flags);
...
WINSETUPAPI
BOOL WINAPI SetupRemoveFileLogEntryA(_In_ HSPFILELOG FileLogHandle,
                                     _In_opt_ PCSTR LogSectionName,
                                     _In_opt_ PCSTR TargetFilename);

WINSETUPAPI
BOOL WINAPI SetupRemoveFileLogEntryW(_In_ HSPFILELOG FileLogHandle,
                                     _In_opt_ PCWSTR LogSectionName,
                                     _In_opt_ PCWSTR TargetFilename);
...
WINSETUPAPI
BOOL WINAPI SetupQueryFileLogA(_In_ HSPFILELOG FileLogHandle,
                               _In_opt_ PCSTR LogSectionName,
                               _In_ PCSTR TargetFilename,
...
WINSETUPAPI
BOOL WINAPI SetupQueryFileLogW(_In_ HSPFILELOG FileLogHandle,
                               _In_opt_ PCWSTR LogSectionName,
                               _In_ PCWSTR TargetFilename,
                               _In_ SetupFileLogInfo DesiredInfo,
...
WINSETUPAPI
BOOL WINAPI SetupOpenLog(_In_ BOOL Erase);

WINSETUPAPI
BOOL WINAPI SetupLogErrorA(_In_ LPCSTR MessageString,
                           _In_ LogSeverity Severity);

WINSETUPAPI
BOOL WINAPI SetupLogErrorW(_In_ LPCWSTR MessageString,
                           _In_ LogSeverity Severity);
...
WINSETUPAPI
VOID WINAPI SetupSetThreadLogToken(_In_ SP_LOG_TOKEN LogToken);

WINSETUPAPI
VOID WINAPI SetupWriteTextLog(_In_ SP_LOG_TOKEN LogToken, _In_ DWORD Category,
                              _In_ DWORD Flags, _In_ PCSTR MessageStr, ...);

WINSETUPAPI
VOID WINAPI SetupWriteTextLogError(_In_ SP_LOG_TOKEN LogToken,
                                   _In_ DWORD Category, _In_ DWORD LogFlags,
                                   _In_ DWORD Error, _In_ PCSTR MessageStr,
                                   ...);

WINSETUPAPI
VOID WINAPI SetupWriteTextLogInfLine(_In_ SP_LOG_TOKEN LogToken,
                                     _In_ DWORD Flags, _In_ HINF InfHandle,
...
WINSETUPAPI
BOOL WINAPI SetupGetBackupInformationA(
    _In_ HSPFILEQ QueueHandle, _Inout_ PSP_BACKUP_QUEUE_PARAMS_A BackupParams);

WINSETUPAPI
BOOL WINAPI SetupGetBackupInformationW(
    _In_ HSPFILEQ QueueHandle, _Inout_ PSP_BACKUP_QUEUE_PARAMS_W BackupParams);
...
WINSETUPAPI
BOOL WINAPI SetupPrepareQueueForRestoreA(_In_ HSPFILEQ QueueHandle,
                                         _In_ PCSTR BackupPath,
                                         _In_ DWORD RestoreFlags);

WINSETUPAPI
BOOL WINAPI SetupPrepareQueueForRestoreW(_In_ HSPFILEQ QueueHandle,
...
WINSETUPAPI
BOOL WINAPI SetupSetNonInteractiveMode(_In_ BOOL NonInteractiveFlag);
...
_Check_return_ WINSETUPAPI HDEVINFO WINAPI SetupDiCreateDeviceInfoList(
    _In_opt_ CONST GUID *ClassGuid, _In_opt_ HWND hwndParent);

_Check_return_ WINSETUPAPI HDEVINFO WINAPI SetupDiCreateDeviceInfoListExA(
    _In_opt_ CONST GUID *ClassGuid, _In_opt_ HWND hwndParent,
    _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);

_Check_return_ WINSETUPAPI HDEVINFO WINAPI SetupDiCreateDeviceInfoListExW(
    _In_opt_ CONST GUID *ClassGuid, _In_opt_ HWND hwndParent,
    _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInfoListClass(_In_ HDEVINFO DeviceInfoSet,
                                          _Out_ LPGUID ClassGuid);

WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInfoListDetailA(_In_ HDEVINFO DeviceInfoSet,
                                            _Out_ PSP_DEVINFO_LIST_DETAIL_DATA_A
                                                DeviceInfoSetDetailData);

WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInfoListDetailW(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiCreateDeviceInfoA(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiCreateDeviceInfoW(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiOpenDeviceInfoA(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiOpenDeviceInfoW(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInstanceIdA(_In_ HDEVINFO DeviceInfoSet,
                                        _In_ PSP_DEVINFO_DATA DeviceInfoData,
                                        _Out_writes_opt_(DeviceInstanceIdSize)
                                            PSTR DeviceInstanceId,
                                        _In_ DWORD DeviceInstanceIdSize,
                                        _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInstanceIdW(_In_ HDEVINFO DeviceInfoSet,
                                        _In_ PSP_DEVINFO_DATA DeviceInfoData,
                                        _Out_writes_opt_(DeviceInstanceIdSize)
...
WINSETUPAPI
BOOL WINAPI SetupDiEnumDeviceInfo(_In_ HDEVINFO DeviceInfoSet,
                                  _In_ DWORD MemberIndex,
                                  _Out_ PSP_DEVINFO_DATA DeviceInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiDestroyDeviceInfoList(_In_ HDEVINFO DeviceInfoSet);

WINSETUPAPI
BOOL WINAPI SetupDiEnumDeviceInterfaces(
    _In_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ CONST GUID *InterfaceClassGuid, _In_ DWORD MemberIndex,
...
BOOL WINAPI SetupDiCreateDeviceInterfaceA(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ CONST GUID *InterfaceClassGuid, _In_opt_ PCSTR ReferenceString,
    _In_ DWORD CreationFlags,
...
BOOL WINAPI SetupDiCreateDeviceInterfaceW(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ CONST GUID *InterfaceClassGuid, _In_opt_ PCWSTR ReferenceString,
    _In_ DWORD CreationFlags,
    _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData);
...
BOOL WINAPI SetupDiOpenDeviceInterfaceA(
    _In_ HDEVINFO DeviceInfoSet, _In_ PCSTR DevicePath, _In_ DWORD OpenFlags,
    _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData);

WINSETUPAPI
BOOL WINAPI SetupDiOpenDeviceInterfaceW(
    _In_ HDEVINFO DeviceInfoSet, _In_ PCWSTR DevicePath, _In_ DWORD OpenFlags,
    _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInterfaceAlias(
...
WINSETUPAPI
BOOL WINAPI SetupDiDeleteDeviceInterfaceData(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiRemoveDeviceInterface(_In_ HDEVINFO DeviceInfoSet,
...
_Success_(return != FALSE) _At_((LPSTR)DeviceInterfaceDetailData->DevicePath,
                                _Post_z_) WINSETUPAPI BOOL WINAPI
    SetupDiGetDeviceInterfaceDetailA(
        _In_ HDEVINFO DeviceInfoSet,
        _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        _Inout_updates_bytes_to_opt_(DeviceInterfaceDetailDataSize,
                                     *RequiredSize)
...
_Success_(return != FALSE) _At_((LPWSTR)DeviceInterfaceDetailData->DevicePath,
                                _Post_z_) WINSETUPAPI BOOL WINAPI
...
//
WINSETUPAPI
BOOL WINAPI SetupDiInstallDeviceInterfaces(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData);
...
WINSETUPAPI
BOOL WINAPI SetupDiSetDeviceInterfaceDefault(
    _In_ HDEVINFO DeviceInfoSet,
    _Inout_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData, _In_ DWORD Flags,
    _Reserved_ PVOID Reserved);
...
BOOL WINAPI SetupDiRegisterDeviceInfo(
    _In_ HDEVINFO DeviceInfoSet, _Inout_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ DWORD Flags, _In_opt_ PSP_DETSIG_CMPPROC CompareProc,
...
WINSETUPAPI
BOOL WINAPI SetupDiBuildDriverInfoList(
    _In_ HDEVINFO DeviceInfoSet, _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ DWORD DriverType);

WINSETUPAPI
BOOL WINAPI SetupDiCancelDriverInfoSearch(_In_ HDEVINFO DeviceInfoSet);

WINSETUPAPI
BOOL WINAPI SetupDiEnumDriverInfoA(_In_ HDEVINFO DeviceInfoSet,
...
WINSETUPAPI
BOOL WINAPI SetupDiEnumDriverInfoW(_In_ HDEVINFO DeviceInfoSet,
                                   _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
                                   _In_ DWORD DriverType,
                                   _In_ DWORD MemberIndex,
...
WINSETUPAPI
BOOL WINAPI SetupDiGetSelectedDriverA(_In_ HDEVINFO DeviceInfoSet,
                                      _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
                                      _Out_ PSP_DRVINFO_DATA_A DriverInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiGetSelectedDriverW(_In_ HDEVINFO DeviceInfoSet,
                                      _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
                                      _Out_ PSP_DRVINFO_DATA_W DriverInfoData);
...
WINSETUPAPI
BOOL WINAPI SetupDiSetSelectedDriverA(
    _In_ HDEVINFO DeviceInfoSet, _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _Inout_opt_ PSP_DRVINFO_DATA_A DriverInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiSetSelectedDriverW(
    _In_ HDEVINFO DeviceInfoSet, _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData,
...
_Check_return_ WINSETUPAPI HDEVINFO WINAPI
SetupDiGetClassDevsA(_In_opt_ CONST GUID *ClassGuid, _In_opt_ PCSTR Enumerator,
                     _In_opt_ HWND hwndParent, _In_ DWORD Flags);

_Check_return_ WINSETUPAPI HDEVINFO WINAPI
SetupDiGetClassDevsW(_In_opt_ CONST GUID *ClassGuid, _In_opt_ PCWSTR Enumerator,
                     _In_opt_ HWND hwndParent, _In_ DWORD Flags);
...
_Check_return_ WINSETUPAPI HDEVINFO WINAPI SetupDiGetClassDevsExA(
    _In_opt_ CONST GUID *ClassGuid, _In_opt_ PCSTR Enumerator,
    _In_opt_ HWND hwndParent, _In_ DWORD Flags, _In_opt_ HDEVINFO DeviceInfoSet,
    _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);

_Check_return_ WINSETUPAPI HDEVINFO WINAPI SetupDiGetClassDevsExW(
    _In_opt_ CONST GUID *ClassGuid, _In_opt_ PCWSTR Enumerator,
    _In_opt_ HWND hwndParent, _In_ DWORD Flags, _In_opt_ HDEVINFO DeviceInfoSet,
    _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetINFClassA(_In_ PCSTR InfName, _Out_ LPGUID ClassGuid,
                                _Out_writes_(ClassNameSize) PSTR ClassName,
                                _In_ DWORD ClassNameSize,
                                _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupDiGetINFClassW(_In_ PCWSTR InfName, _Out_ LPGUID ClassGuid,
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI SetupDiBuildClassInfoList(
    _In_ DWORD Flags,
    _Out_writes_to_opt_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
    _In_ DWORD ClassGuidListSize, _Out_ PDWORD RequiredSize);
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI SetupDiBuildClassInfoListExW(
    _In_ DWORD Flags,
    _Out_writes_to_opt_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
    _In_ DWORD ClassGuidListSize, _Out_ PDWORD RequiredSize,
    _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetClassDescriptionA(_In_ CONST GUID *ClassGuid,
                                        _Out_writes_(ClassDescriptionSize)
                                            PSTR ClassDescription,
                                        _In_ DWORD ClassDescriptionSize,
                                        _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupDiGetClassDescriptionW(_In_ CONST GUID *ClassGuid,
                                        _Out_writes_(ClassDescriptionSize)
...
WINSETUPAPI
BOOL WINAPI SetupDiGetClassDescriptionExA(_In_ CONST GUID *ClassGuid,
                                          _Out_writes_(ClassDescriptionSize)
                                              PSTR ClassDescription,
                                          _In_ DWORD ClassDescriptionSize,
                                          _Out_opt_ PDWORD RequiredSize,
...
WINSETUPAPI
BOOL WINAPI SetupDiGetClassDescriptionExW(_In_ CONST GUID *ClassGuid,
                                          _Out_writes_(ClassDescriptionSize)
                                              PWSTR ClassDescription,
                                          _In_ DWORD ClassDescriptionSize,
                                          _Out_opt_ PDWORD RequiredSize,
...
WINSETUPAPI
BOOL WINAPI SetupDiCallClassInstaller(_In_ DI_FUNCTION InstallFunction,
                                      _In_ HDEVINFO DeviceInfoSet,
                                      _In_opt_ PSP_DEVINFO_DATA DeviceInfoData);
...
//
WINSETUPAPI
BOOL WINAPI SetupDiSelectDevice(_In_ HDEVINFO DeviceInfoSet,
...
//
WINSETUPAPI
BOOL WINAPI SetupDiSelectBestCompatDrv(
    _In_ HDEVINFO DeviceInfoSet, _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData);
...
//
WINSETUPAPI
BOOL WINAPI SetupDiInstallDevice(_In_ HDEVINFO DeviceInfoSet,
...
//
WINSETUPAPI
BOOL WINAPI SetupDiInstallDriverFiles(_In_ HDEVINFO DeviceInfoSet,
                                      _In_ PSP_DEVINFO_DATA DeviceInfoData);
...
//
WINSETUPAPI
BOOL WINAPI SetupDiRegisterCoDeviceInstallers(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData);
...
//
WINSETUPAPI
BOOL WINAPI SetupDiRemoveDevice(_In_ HDEVINFO DeviceInfoSet,
                                _Inout_ PSP_DEVINFO_DATA DeviceInfoData);
...
//
WINSETUPAPI
BOOL WINAPI SetupDiUnremoveDevice(_In_ HDEVINFO DeviceInfoSet,
                                  _Inout_ PSP_DEVINFO_DATA DeviceInfoData);

#if _SETUPAPI_VER >= _WIN32_WINNT_WS03

WINSETUPAPI
BOOL WINAPI SetupDiRestartDevices(_In_ HDEVINFO DeviceInfoSet,
                                  _Inout_ PSP_DEVINFO_DATA DeviceInfoData);
...
//
WINSETUPAPI
BOOL WINAPI SetupDiChangeState(_In_ HDEVINFO DeviceInfoSet,
                               _Inout_ PSP_DEVINFO_DATA DeviceInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiInstallClassA(_In_opt_ HWND hwndParent,
                                 _In_ PCSTR InfFileName, _In_ DWORD Flags,
                                 _In_opt_ HSPFILEQ FileQueue);

WINSETUPAPI
BOOL WINAPI SetupDiInstallClassW(_In_opt_ HWND hwndParent,
                                 _In_ PCWSTR InfFileName, _In_ DWORD Flags,
                                 _In_opt_ HSPFILEQ FileQueue);
...
WINSETUPAPI
BOOL WINAPI SetupDiInstallClassExA(_In_opt_ HWND hwndParent,
                                   _In_opt_ PCSTR InfFileName, _In_ DWORD Flags,
                                   _In_opt_ HSPFILEQ FileQueue,
...
BOOL WINAPI SetupDiInstallClassExW(
    _In_opt_ HWND hwndParent, _In_opt_ PCWSTR InfFileName, _In_ DWORD Flags,
    _In_opt_ HSPFILEQ FileQueue, _In_opt_ CONST GUID *InterfaceClassGuid,
    _Reserved_ PVOID Reserved1, _Reserved_ PVOID Reserved2);
...
_Check_return_ WINSETUPAPI HKEY WINAPI
SetupDiOpenClassRegKey(_In_opt_ CONST GUID *ClassGuid, _In_ REGSAM samDesired);
...
_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiOpenClassRegKeyExA(
    _In_opt_ CONST GUID *ClassGuid, _In_ REGSAM samDesired, _In_ DWORD Flags,
    _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);

_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiOpenClassRegKeyExW(
    _In_opt_ CONST GUID *ClassGuid, _In_ REGSAM samDesired, _In_ DWORD Flags,
    _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
...
_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiCreateDeviceInterfaceRegKeyA(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
    _Reserved_ DWORD Reserved, _In_ REGSAM samDesired, _In_opt_ HINF InfHandle,
    _In_opt_ PCSTR InfSectionName);

_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiCreateDeviceInterfaceRegKeyW(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
    _Reserved_ DWORD Reserved, _In_ REGSAM samDesired, _In_opt_ HINF InfHandle,
...
_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiOpenDeviceInterfaceRegKey(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
    _Reserved_ DWORD Reserved, _In_ REGSAM samDesired);
...
WINSETUPAPI
BOOL WINAPI SetupDiDeleteDeviceInterfaceRegKey(_In_ HDEVINFO DeviceInfoSet,
                                               _In_ PSP_DEVICE_INTERFACE_DATA
                                                   DeviceInterfaceData,
                                               _Reserved_ DWORD Reserved);
...
_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiCreateDevRegKeyA(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ DWORD Scope, _In_ DWORD HwProfile, _In_ DWORD KeyType,
    _In_opt_ HINF InfHandle, _In_opt_ PCSTR InfSectionName);

_Check_return_ WINSETUPAPI HKEY WINAPI SetupDiCreateDevRegKeyW(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ DWORD Scope, _In_ DWORD HwProfile, _In_ DWORD KeyType,
    _In_opt_ HINF InfHandle, _In_opt_ PCWSTR InfSectionName);
...
WINSETUPAPI
BOOL WINAPI SetupDiDeleteDevRegKey(_In_ HDEVINFO DeviceInfoSet,
                                   _In_ PSP_DEVINFO_DATA DeviceInfoData,
                                   _In_ DWORD Scope, _In_ DWORD HwProfile,
                                   _In_ DWORD KeyType);

_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiGetHwProfileList(_Out_writes_to_(HwProfileListSize, *RequiredSize)
                                PDWORD HwProfileList,
                            _In_ DWORD HwProfileListSize,
                            _Out_ PDWORD RequiredSize,
                            _Out_opt_ PDWORD CurrentlyActiveIndex);

_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI SetupDiGetHwProfileListExA(
    _Out_writes_to_(HwProfileListSize, *RequiredSize) PDWORD HwProfileList,
    _In_ DWORD HwProfileListSize, _Out_ PDWORD RequiredSize,
    _Out_opt_ PDWORD CurrentlyActiveIndex, _In_opt_ PCSTR MachineName,
    _Reserved_ PVOID Reserved);

_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI SetupDiGetHwProfileListExW(
    _Out_writes_to_(HwProfileListSize, *RequiredSize) PDWORD HwProfileList,
    _In_ DWORD HwProfileListSize, _Out_ PDWORD RequiredSize,
    _Out_opt_ PDWORD CurrentlyActiveIndex, _In_opt_ PCWSTR MachineName,
    _Reserved_ PVOID Reserved);
...
BOOL WINAPI SetupDiGetDevicePropertyKeys(
    ...
    _In_ DWORD PropertyKeyCount, _Out_opt_ PDWORD RequiredPropertyKeyCount,
    _In_ DWORD Flags);

_Success_(return != FALSE)
    _When_(*PropertyType == DEVPROP_TYPE_STRING,
           _At_((PWSTR)PropertyBuffer,
                _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
        _When_(*PropertyType == DEVPROP_TYPE_STRING_INDIRECT,
               _At_((PWSTR)PropertyBuffer,
                    _Out_writes_bytes_to_opt_(PropertyBufferSize,
                                              *RequiredSize)))
            _When_(*PropertyType == DEVPROP_TYPE_STRING_LIST,
                   _At_((PZZWSTR)PropertyBuffer,
                        _Out_writes_bytes_to_opt_(PropertyBufferSize,
                                                  *RequiredSize)))
                WINSETUPAPI BOOL WINAPI
    SetupDiGetDevicePropertyW(
        _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
        _In_ CONST DEVPROPKEY *PropertyKey, _Out_ DEVPROPTYPE *PropertyType,
        _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)
            PBYTE PropertyBuffer,
        _In_ DWORD PropertyBufferSize, _Out_opt_ PDWORD RequiredSize,
        _In_ DWORD Flags);
...
WINSETUPAPI
BOOL WINAPI SetupDiSetDevicePropertyW(
    _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ CONST DEVPROPKEY *PropertyKey, _In_ DEVPROPTYPE PropertyType,
    _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
    _In_ DWORD PropertyBufferSize, _In_ DWORD Flags);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetDeviceInterfacePropertyKeys(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
    _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
    _In_ DWORD PropertyKeyCount, _Out_opt_ PDWORD RequiredPropertyKeyCount,
...
WINSETUPAPI
BOOL WINAPI SetupDiSetDeviceInterfacePropertyW(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
    _In_ CONST DEVPROPKEY *PropertyKey, _In_ DEVPROPTYPE PropertyType,
    _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
...
WINSETUPAPI
BOOL WINAPI SetupDiGetClassPropertyKeys(
    _In_ CONST GUID *ClassGuid,
    _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
    _In_ DWORD PropertyKeyCount, _Out_opt_ PDWORD RequiredPropertyKeyCount,
    _In_ DWORD Flags);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetClassPropertyKeysExW(
    _In_ CONST GUID *ClassGuid,
    _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
    _In_ DWORD PropertyKeyCount, _Out_opt_ PDWORD RequiredPropertyKeyCount,
    _In_ DWORD Flags, _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
            ...
            _When_(*PropertyType == DEVPROP_TYPE_STRING_LIST,
                   _At_((PZZWSTR)PropertyBuffer,
                        _Out_writes_bytes_to_opt_(PropertyBufferSize,
                                                  *RequiredSize)))
                WINSETUPAPI BOOL WINAPI
    SetupDiGetClassPropertyW(_In_ CONST GUID *ClassGuid,
                             ...
                             _Out_opt_ PDWORD RequiredSize, _In_ DWORD Flags);
...
_Success_(return != FALSE)
    ...
    SetupDiGetClassPropertyExW(
        _In_ CONST GUID *ClassGuid, _In_ CONST DEVPROPKEY *PropertyKey,
        _Out_ DEVPROPTYPE *PropertyType,
        _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)
            PBYTE PropertyBuffer,
        _In_ DWORD PropertyBufferSize, _Out_opt_ PDWORD RequiredSize,
        _In_ DWORD Flags, _In_opt_ PCWSTR MachineName,
        _Reserved_ PVOID Reserved);
...
WINSETUPAPI
BOOL WINAPI SetupDiSetClassPropertyW(_In_ CONST GUID *ClassGuid,
                                     _In_ CONST DEVPROPKEY *PropertyKey,
...
WINSETUPAPI
BOOL WINAPI SetupDiSetClassPropertyExW(
    _In_ CONST GUID *ClassGuid, _In_ CONST DEVPROPKEY *PropertyKey,
    _In_ DEVPROPTYPE PropertyType,
    _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
    _In_ DWORD PropertyBufferSize, _In_ DWORD Flags,
...
#define SPDRP_PHYSICAL_DEVICE_OBJECT_NAME                                      \
  (0x0000000E)                             // PhysicalDeviceObjectName (R)
...
#define SPDRP_REMOVAL_POLICY_HW_DEFAULT                                        \
  (0x00000020) // Hardware Removal Policy (R)
#define SPDRP_REMOVAL_POLICY_OVERRIDE                                          \
  (0x00000021)                              // Removal Policy Override (RW)
...
// they should also have a 1:1 correspondence with Device registers, where
// applicable but no overlap otherwise
//
...
_Success_(return != FALSE) _When_((*PropertyRegDataType == REG_SZ),
                                  _At_((PSTR)PropertyBuffer, _Post_valid_))
    _When_((*PropertyRegDataType == REG_MULTI_SZ),
           _At_((PZZSTR)PropertyBuffer, _Post_valid_)) WINSETUPAPI BOOL WINAPI
    SetupDiGetDeviceRegistryPropertyA(
        _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
        _In_ DWORD Property, _Out_opt_ PDWORD PropertyRegDataType,
        _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)
            PBYTE PropertyBuffer,
        _In_ DWORD PropertyBufferSize, _Out_opt_ PDWORD RequiredSize);

_Success_(return != FALSE) _When_((*PropertyRegDataType == REG_SZ),
                                  _At_((PWSTR)PropertyBuffer, _Post_valid_))
    _When_((*PropertyRegDataType == REG_MULTI_SZ),
           _At_((PZZWSTR)PropertyBuffer, _Post_valid_)) WINSETUPAPI BOOL WINAPI
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiGetClassRegistryPropertyA(
        _In_ CONST GUID *ClassGuid, _In_ DWORD Property,
        _Out_opt_ PDWORD PropertyRegDataType,
        _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize)
            PBYTE PropertyBuffer,
        _In_ DWORD PropertyBufferSize, _Out_opt_ PDWORD RequiredSize,
        _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);
...
WINSETUPAPI
BOOL WINAPI SetupDiSetClassRegistryPropertyA(
    _In_ CONST GUID *ClassGuid, _In_ DWORD Property,
    _In_reads_bytes_opt_(PropertyBufferSize) CONST BYTE *PropertyBuffer,
    _In_ DWORD PropertyBufferSize, _In_opt_ PCSTR MachineName,
    _Reserved_ PVOID Reserved);

WINSETUPAPI
BOOL WINAPI SetupDiSetClassRegistryPropertyW(
    _In_ CONST GUID *ClassGuid, _In_ DWORD Property,
    _In_reads_bytes_opt_(PropertyBufferSize) CONST BYTE *PropertyBuffer,
    _In_ DWORD PropertyBufferSize, _In_opt_ PCWSTR MachineName,
    _Reserved_ PVOID Reserved);
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiGetClassInstallParamsA(
        _In_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        _Out_writes_bytes_to_opt_(ClassInstallParamsSize, *RequiredSize)
            PSP_CLASSINSTALL_HEADER ClassInstallParams,
        _In_ DWORD ClassInstallParamsSize, _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetDriverInstallParamsA(
    _In_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ PSP_DRVINFO_DATA_A DriverInfoData,
    _Out_ PSP_DRVINSTALL_PARAMS DriverInstallParams);

WINSETUPAPI
BOOL WINAPI SetupDiGetDriverInstallParamsW(
    _In_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ PSP_DRVINFO_DATA_W DriverInfoData,
    _Out_ PSP_DRVINSTALL_PARAMS DriverInstallParams);
...
WINSETUPAPI
BOOL WINAPI SetupDiSetDriverInstallParamsA(
    _In_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ PSP_DRVINFO_DATA_A DriverInfoData,
    _In_ PSP_DRVINSTALL_PARAMS DriverInstallParams);

WINSETUPAPI
BOOL WINAPI SetupDiSetDriverInstallParamsW(
    _In_ HDEVINFO DeviceInfoSet, _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ PSP_DRVINFO_DATA_W DriverInfoData,
    _In_ PSP_DRVINSTALL_PARAMS DriverInstallParams);
...
WINSETUPAPI
BOOL WINAPI SetupDiLoadClassIcon(_In_ CONST GUID *ClassGuid,
...
WINSETUPAPI
BOOL WINAPI SetupDiLoadDeviceIcon(_In_ HDEVINFO DeviceInfoSet,
                                  _In_ PSP_DEVINFO_DATA DeviceInfoData,
                                  _In_ UINT cxIcon, _In_ UINT cyIcon,
                                  _In_ DWORD Flags, _Out_ HICON *hIcon);
...
WINSETUPAPI
INT WINAPI SetupDiDrawMiniIcon(_In_ HDC hdc, _In_ RECT rc,
                               _In_ INT MiniIconIndex, _In_ DWORD Flags);

WINSETUPAPI
BOOL WINAPI SetupDiGetClassBitmapIndex(_In_opt_ CONST GUID *ClassGuid,
                                       _Out_ PINT MiniIconIndex);

WINSETUPAPI
BOOL WINAPI
SetupDiGetClassImageList(_Out_ PSP_CLASSIMAGELIST_DATA ClassImageListData);
...
WINSETUPAPI
BOOL WINAPI
SetupDiGetClassImageIndex(_In_ PSP_CLASSIMAGELIST_DATA ClassImageListData,
                          _In_ CONST GUID *ClassGuid, _Out_ PINT ImageIndex);

WINSETUPAPI
BOOL WINAPI
SetupDiDestroyClassImageList(_In_ PSP_CLASSIMAGELIST_DATA ClassImageListData);
...
BOOL WINAPI SetupDiGetClassDevPropertySheetsA(
    ...
    _In_ DWORD PropertySheetHeaderPageListSize, _Out_opt_ PDWORD RequiredSize,
...
WINSETUPAPI
BOOL WINAPI SetupDiAskForOEMDisk(_In_ HDEVINFO DeviceInfoSet,
                                 _In_opt_ PSP_DEVINFO_DATA DeviceInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiSelectOEMDrv(_In_opt_ HWND hwndParent,
                                _In_ HDEVINFO DeviceInfoSet,
                                _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiClassNameFromGuidA(_In_ CONST GUID *ClassGuid,
                                      _Out_writes_(ClassNameSize)
                                          PSTR ClassName,
                                      _In_ DWORD ClassNameSize,
                                      _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupDiClassNameFromGuidW(_In_ CONST GUID *ClassGuid,
                                      _Out_writes_(ClassNameSize)
                                          PWSTR ClassName,
                                      _In_ DWORD ClassNameSize,
                                      _Out_opt_ PDWORD RequiredSize);
...
BOOL WINAPI SetupDiClassNameFromGuidExA(
    _In_ CONST GUID *ClassGuid, _Out_writes_(ClassNameSize) PSTR ClassName,
    _In_ DWORD ClassNameSize, _Out_opt_ PDWORD RequiredSize,
    _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);

WINSETUPAPI
BOOL WINAPI SetupDiClassNameFromGuidExW(
    _In_ CONST GUID *ClassGuid, _Out_writes_(ClassNameSize) PWSTR ClassName,
    _In_ DWORD ClassNameSize, _Out_opt_ PDWORD RequiredSize,
    _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiClassGuidsFromNameA(_In_ PCSTR ClassName,
                               _Out_writes_to_(ClassGuidListSize, *RequiredSize)
                                   LPGUID ClassGuidList,
                               _In_ DWORD ClassGuidListSize,
                               _Out_ PDWORD RequiredSize);

_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiClassGuidsFromNameW(_In_ PCWSTR ClassName,
                               _Out_writes_to_(ClassGuidListSize, *RequiredSize)
                                   LPGUID ClassGuidList,
                               _In_ DWORD ClassGuidListSize,
                               _Out_ PDWORD RequiredSize);
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI SetupDiClassGuidsFromNameExA(
    _In_ PCSTR ClassName,
    _Out_writes_to_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
    _In_ DWORD ClassGuidListSize, _Out_ PDWORD RequiredSize,
    _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);

_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI SetupDiClassGuidsFromNameExW(
...
WINSETUPAPI
BOOL WINAPI SetupDiGetHwProfileFriendlyNameA(_In_ DWORD HwProfile,
                                             _Out_writes_(FriendlyNameSize)
                                                 PSTR FriendlyName,
                                             _In_ DWORD FriendlyNameSize,
                                             _Out_opt_ PDWORD RequiredSize);

WINSETUPAPI
BOOL WINAPI SetupDiGetHwProfileFriendlyNameW(_In_ DWORD HwProfile,
                                             _Out_writes_(FriendlyNameSize)
                                                 PWSTR FriendlyName,
                                             _In_ DWORD FriendlyNameSize,
                                             _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupDiGetHwProfileFriendlyNameExA(
    _In_ DWORD HwProfile, _Out_writes_(FriendlyNameSize) PSTR FriendlyName,
    _In_ DWORD FriendlyNameSize, _Out_opt_ PDWORD RequiredSize,
    _In_opt_ PCSTR MachineName, _Reserved_ PVOID Reserved);

WINSETUPAPI
BOOL WINAPI SetupDiGetHwProfileFriendlyNameExW(
    _In_ DWORD HwProfile, _Out_writes_(FriendlyNameSize) PWSTR FriendlyName,
    _In_ DWORD FriendlyNameSize, _Out_opt_ PDWORD RequiredSize,
    _In_opt_ PCWSTR MachineName, _Reserved_ PVOID Reserved);
...
HPROPSHEETPAGE
WINAPI
SetupDiGetWizardPage(_In_ HDEVINFO DeviceInfoSet,
                     _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
                     _In_ PSP_INSTALLWIZARD_DATA InstallWizardData,
                     _In_ DWORD PageType, _In_ DWORD Flags);

WINSETUPAPI
BOOL WINAPI SetupDiGetSelectedDevice(_In_ HDEVINFO DeviceInfoSet,
                                     _Out_ PSP_DEVINFO_DATA DeviceInfoData);

WINSETUPAPI
BOOL WINAPI SetupDiSetSelectedDevice(_In_ HDEVINFO DeviceInfoSet,
                                     _In_ PSP_DEVINFO_DATA DeviceInfoData);

#if _SETUPAPI_VER >= _WIN32_WINNT_WS03

WINSETUPAPI
BOOL WINAPI SetupDiGetActualModelsSectionA(
    _In_ PINFCONTEXT Context,
    _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
...
WINSETUPAPI
BOOL WINAPI SetupDiGetActualModelsSectionW(
    _In_ PINFCONTEXT Context,
    _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
...
BOOL WINAPI SetupDiGetActualSectionToInstallA(
    _In_ HINF InfHandle, _In_ PCSTR InfSectionName,
    _Out_writes_opt_(InfSectionWithExtSize) PSTR InfSectionWithExt,
    _In_ DWORD InfSectionWithExtSize, _Out_opt_ PDWORD RequiredSize,
...
BOOL WINAPI SetupDiGetActualSectionToInstallW(
    _In_ HINF InfHandle, _In_ PCWSTR InfSectionName,
    _Out_writes_opt_(InfSectionWithExtSize) PWSTR InfSectionWithExt,
    _In_ DWORD InfSectionWithExtSize, _Out_opt_ PDWORD RequiredSize,
...
BOOL WINAPI SetupDiGetActualSectionToInstallExA(
    ...
    _Out_writes_opt_(InfSectionWithExtSize) PSTR InfSectionWithExt,
    _In_ DWORD InfSectionWithExtSize, _Out_opt_ PDWORD RequiredSize,
    _Out_opt_ PSTR *Extension, _Reserved_ PVOID Reserved);
...
BOOL WINAPI SetupDiGetActualSectionToInstallExW(
    ...
    _Out_writes_opt_(InfSectionWithExtSize) PWSTR InfSectionWithExt,
    _In_ DWORD InfSectionWithExtSize, _Out_opt_ PDWORD RequiredSize,
    _Out_opt_ PWSTR *Extension, _Reserved_ PVOID Reserved);
...
//
WINSETUPAPI
BOOL WINAPI SetupEnumInfSectionsA(_In_ HINF InfHandle, _In_ UINT Index,
                                  _Out_writes_opt_(Size) PSTR Buffer,
                                  _In_ UINT Size, _Out_opt_ UINT *SizeNeeded);

WINSETUPAPI
BOOL WINAPI SetupEnumInfSectionsW(_In_ HINF InfHandle, _In_ UINT Index,
...
#define SIGNERSCORE_UNCLASSIFIED                                               \
  0x0D000004 // UNCLASSIFIED == INBOX == STANDARD == PREMIUM when the
             // SIGNERSCORE_MASK
...
#define SIGNERSCORE_MASK                                                       \
  0xFF000000 // Mask out all but the upper BYTE which contains the ranking
             // signer information
#define SIGNERSCORE_SIGNED_MASK                                                \
  0xF0000000 // Mask out only the upper nibble, which tells us if the package is
             // signed or not.

#endif // _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

#if USE_SP_INF_SIGNER_INFO_V1 ||                                               \
    (_SETUPAPI_VER <                                                           \
     _WIN32_WINNT_LONGHORN) // use version 1 signer info structure
...
WINSETUPAPI
BOOL WINAPI SetupVerifyInfFileA(_In_ PCSTR InfName,
                                _In_opt_ PSP_ALTPLATFORM_INFO AltPlatformInfo,
                                _Inout_ PSP_INF_SIGNER_INFO_A InfSignerInfo);

WINSETUPAPI
BOOL WINAPI SetupVerifyInfFileW(_In_ PCWSTR InfName,
                                _In_opt_ PSP_ALTPLATFORM_INFO AltPlatformInfo,
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiGetCustomDevicePropertyA(
        _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
        _In_ PCSTR CustomPropertyName, _In_ DWORD Flags,
        _Out_opt_ PDWORD PropertyRegDataType,
        _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize)
            PBYTE PropertyBuffer,
        _In_ DWORD PropertyBufferSize, _Out_opt_ PDWORD RequiredSize);
...
_Success_(return != FALSE) WINSETUPAPI BOOL WINAPI
    SetupDiGetCustomDevicePropertyW(
        _In_ HDEVINFO DeviceInfoSet, _In_ PSP_DEVINFO_DATA DeviceInfoData,
        _In_ PCWSTR CustomPropertyName, _In_ DWORD Flags,
        _Out_opt_ PDWORD PropertyRegDataType,
        _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize)
            PBYTE PropertyBuffer,
        _In_ DWORD PropertyBufferSize, _Out_opt_ PDWORD RequiredSize);
...
WINSETUPAPI
BOOL WINAPI SetupConfigureWmiFromInfSectionA(_In_ HINF InfHandle,
                                             _In_ PCSTR SectionName,
                                             _In_ DWORD Flags);

WINSETUPAPI
BOOL WINAPI SetupConfigureWmiFromInfSectionW(_In_ HINF InfHandle,
                                             _In_ PCWSTR SectionName,
                                             _In_ DWORD Flags);
