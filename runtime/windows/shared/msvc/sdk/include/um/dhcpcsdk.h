...
#define DHCPCAPI_REQUEST_ASYNCHRONOUS                                          \
  0x04 // request and return, set event on completion
...
DWORD                                  // win32 status
    APIENTRY DhcpRequestParams(        // request parameters of client
        ...
        IN OUT DHCPCAPI_PARAMS_ARRAY
            RecdParams, // parameters that are to be requested..
        _Out_writes_bytes_to_(*pSize, *pSize)
            LPBYTE Buffer, // a buffer to hold data for RecdParams
        _Inout_ LPDWORD
            pSize, // i/p: size of above in BYTES, o/p required bytes..
        _In_ LPWSTR RequestIdStr // needed for persistent requests
    ); // returns ERROR_MORE_DATA if o/p buffer is of insufficient size, and
       // fills in reqd size in # of bytes
...
DWORD                               // win32 status
    APIENTRY DhcpUndoRequestParams( // undo the effect of a persistent request
                                    // -- currently undo from registry
...
DWORD                                       // win32 status
    APIENTRY DhcpRegisterParamChange(       // notify if a parameter has changed
        ...
        IN OUT LPVOID Handle // handle to event that will be SetEvent'ed in case
                             // of param change
    );

#define DHCPCAPI_DEREGISTER_HANDLE_EVENT                                       \
  0x01 // de-register handle that is an event
DWORD
APIENTRY
...
DWORD
APIENTRY
DhcpGetOriginalSubnetMask(IN LPCWSTR sAdapterName, OUT DWORD *dwSubnetMask);
