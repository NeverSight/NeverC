...
#define MUI_LANGUAGE_EXACT                                                     \
  0x10 // Don't fall back if language module is not found
...
Arguments:
    ...
    convention was used to setup the files.
        ...
        Options:
            MUI_LANGUAGE_EXACT - When wLangID is present and the associated
library is not found, return NULL rather than falling back to the module at
pwszModuleName
...
// ANSI version
HINSTANCE __stdcall LoadMUILibraryA(_In_ PCSTR pszFullModuleName,
                                    _In_ DWORD dwLangConvention,
                                    _In_ LANGID LangID);

// Wide char version
HINSTANCE __stdcall LoadMUILibraryW(_In_ PCWSTR pszFullModuleName,
                                    _In_ DWORD dwLangConvention,
                                    _In_ LANGID LangID);
...
--*/
BOOL __stdcall FreeMUILibrary(_In_ HMODULE hResModule);
...
Arguments:
    ...
    pFallbackList   - On successful return, contains a NULL-delimited list
                      of UI Language names e.g. 'tk-TM'.  This list is
...
--*/
BOOL __stdcall GetUILanguageFallbackList(_Out_writes_opt_(cchFallbackList)
                                             PWSTR pFallbackList,
