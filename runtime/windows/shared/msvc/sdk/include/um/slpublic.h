...
//
// Active directory activation (header.type ==
...
#define SL_INFO_KEY_KMS_REQUIRED_CLIENT_COUNT                                  \
  L"KeyManagementServiceRequiredClientCount"
#define SL_INFO_KEY_KMS_UNLICENSED_REQUESTS                                    \
  L"KeyManagementServiceUnlicensedRequests"
#define SL_INFO_KEY_KMS_LICENSED_REQUESTS                                      \
  L"KeyManagementServiceLicensedRequests"
#define SL_INFO_KEY_KMS_OOB_GRACE_REQUESTS                                     \
  L"KeyManagementServiceOOBGraceRequests"
#define SL_INFO_KEY_KMS_OOT_GRACE_REQUESTS                                     \
  L"KeyManagementServiceOOTGraceRequests"
#define SL_INFO_KEY_KMS_NON_GENUINE_GRACE_REQUESTS                             \
  L"KeyManagementServiceNonGenuineGraceRequests"
#define SL_INFO_KEY_KMS_NOTIFICATION_REQUESTS                                  \
...
// SL Rearm Flags
//
#define SL_REARM_REBOOT_REQUIRED                                               \
  0x00000001 // require a reboot after performing rearm
...
#define SL_PROP_LAST_ACT_ATTEMPT_SERVER_FLAGS                                  \
  L"SL_LAST_ACT_ATTEMPT_SERVER_FLAGS"
#define SL_PROP_ACTIVATION_VALIDATION_IN_PROGRESS                              \
  L"SL_ACTIVATION_VALIDATION_IN_PROGRESS"

#define SL_POLICY_EVALUATION_MODE_ENABLED L"Security-SPP-EvaluationModeEnabled"

#define SL_DEFAULT_MIGRATION_ENCRYPTOR_URI                                     \
  L"msft:spp/migrationencryptor/tokenact/1.0"

__control_entrypoint(DllExport) HRESULT WINAPI SLOpen(_Out_ HSLC *phSLC);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI SLClose(_In_ HSLC hSLC);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLInstallProofOfPurchase(_In_ HSLC hSLC, _In_ PCWSTR pwszPKeyAlgorithm,
                             _In_ PCWSTR pwszPKeyString,
                             _In_ UINT cbPKeySpecificData,
                             _In_reads_bytes_opt_(cbPKeySpecificData)
...
__control_entrypoint(DllExport) HRESULT WINAPI SLInstallProofOfPurchaseEx(
    _In_ HSLC hSLC, _In_ CONST SLID *pApplicationId,
    _In_opt_ CONST SLID *pProductSkuId, _In_ PCWSTR pwszPKeyAlgorithm,
    _In_ PCWSTR pwszPKeyString, _In_ UINT cbPKeySpecificData,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLUninstallProofOfPurchase(_In_ HSLC hSLC, _In_ CONST SLID *pPKeyId);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLInstallLicense(_In_ HSLC hSLC, _In_ UINT cbLicenseBlob,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLUninstallLicense(_In_ HSLC hSLC, _In_ CONST SLID *pLicenseFileId);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLConsumeRight(_In_ HSLC hSLC, _In_ CONST SLID *pAppId,
                   _In_opt_ CONST SLID *pProductSkuId,
                   _In_opt_ PCWSTR pwszRightName, _Reserved_ PVOID pvReserved);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI SLGetProductSkuInformation(
    _In_ HSLC hSLC, _In_ CONST SLID *pProductSkuId, _In_ PCWSTR pwszValueName,
    _Out_opt_ SLDATATYPE *peDataType, _Out_ UINT *pcbValue,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetPKeyInformation(_In_ HSLC hSLC, _In_ CONST SLID *pPKeyId,
...
__control_entrypoint(DllExport) HRESULT WINAPI SLGetLicenseInformation(
    _In_ HSLC hSLC, _In_ CONST SLID *pSLLicenseId, _In_ PCWSTR pwszValueName,
    _Out_opt_ SLDATATYPE *peDataType, _Out_ UINT *pcbValue,
    _Outptr_result_bytebuffer_(*pcbValue) PBYTE *ppbValue);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI SLGetLicensingStatusInformation(
    _In_ HSLC hSLC, _In_opt_ CONST SLID *pAppID,
    _In_opt_ CONST SLID *pProductSkuId, _In_opt_ PCWSTR pwszRightName,
    _Out_ UINT *pnStatusCount,
    _Outptr_result_buffer_(*pnStatusCount)
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetPolicyInformation(_In_ HSLC hSLC, _In_ PCWSTR pwszValueName,
                           _Out_opt_ SLDATATYPE *peDataType,
                           _Out_ UINT *pcbValue,
                           _Outptr_result_bytebuffer_(*pcbValue)
                               PBYTE *ppbValue);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetPolicyInformationDWORD(_In_ HSLC hSLC, _In_ PCWSTR pwszValueName,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetServiceInformation(_In_ HSLC hSLC, _In_ PCWSTR pwszValueName,
                            _Out_opt_ SLDATATYPE *peDataType,
                            _Out_ UINT *pcbValue,
                            _Outptr_result_bytebuffer_(*pcbValue)
                                PBYTE *ppbValue);
...
__control_entrypoint(DllExport) HRESULT WINAPI SLGetApplicationInformation(
    _In_ HSLC hSLC, _In_ const SLID *pApplicationId, _In_ PCWSTR pwszValueName,
    _Out_opt_ SLDATATYPE *peDataType, _Out_ UINT *pcbValue,
    _Outptr_result_bytebuffer_(*pcbValue) PBYTE *ppbValue);
/*++
Routine Description:
...
Arguments:
    ...
    pwszValueName
        SL_INFO_KEY_IS_KMS
            Return Type = SL_DATA_DWORD
            Return Data = Indicates whether the machine has a key management
service enabled SL_INFO_KEY_KMS_CURRENT_COUNT Return Type = SL_DATA_DWORD Return
Data = Count of currently active volume clients on KMS host
        SL_INFO_KEY_KMS_REQUIRED_CLIENT_COUNT
            Return Type = SL_DATA_DWORD
            Return Data = Min number of VL clients required to connect to a KMS
host for enabling activation SL_INFO_KEY_KMS_UNLICENSED_REQUESTS Return Type =
SL_DATA_DWORD Return Data = Count of KMS requests from VL clients with License
Status=Unlicensed SL_INFO_KEY_KMS_LICENSED_REQUESTS Return Type = SL_DATA_DWORD
            Return Data = Count of KMS requests from VL clients with License
Status=Licensed SL_INFO_KEY_KMS_OOB_GRACE_REQUESTS Return Type = SL_DATA_DWORD
            Return Data = Count of KMS requests from VL clients with License
Status=OOB Grace SL_INFO_KEY_KMS_OOT_GRACE_REQUESTS Return Type = SL_DATA_DWORD
            Return Data = Count of KMS requests from VL clients with License
Status=OOT Grace SL_INFO_KEY_KMS_NON_GENUINE_GRACE_REQUESTS Return Type =
SL_DATA_DWORD Return Data = Count of KMS requests from VL clients with License
Status=Non-Genuine Grace SL_INFO_KEY_KMS_NOTIFICATION_REQUESTS Return Type =
SL_DATA_DWORD Return Data = Count of KMS requests from VL clients with License
Status=Notification SL_INFO_KEY_KMS_TOTAL_REQUESTS Return Type = SL_DATA_DWORD
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLActivateProduct(_In_ HSLC hSLC, _In_ CONST SLID *pProductSkuId,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetServerStatus(_In_ PCWSTR pwszServerURL,
                      _In_ PCWSTR pwszAcquisitionType,
                      _In_opt_ PCWSTR pwszProxyServer, _In_opt_ WORD wProxyPort,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetActiveLicenseInfo(_In_ HSLC hSLC, _Reserved_ CONST SLID *pReserved,
                           _Out_ UINT *pcbValue,
                           _Outptr_result_bytebuffer_(*pcbValue)
                               PBYTE *ppbValue);
/*++
Routine Description:
    ...
    This function returns active license data so the caller can decide on
applicability.
...
Arguments: none
    ...
    ppbValue
        receives the allocated byte buffer that contains the active license
info. The caller should call LocalFree to free the buffer.
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLDepositStoreToken(_In_ HSLC hSLC, _In_ UINT cbValue,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLReArm(_In_ HSLC hSLC, _In_ CONST SLID *pApplicationId,
            _In_opt_ CONST SLID *pProductSkuId, _In_ DWORD dwFlags);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGenerateOfflineInstallationId(_In_ HSLC hSLC,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGenerateOfflineInstallationIdEx(
        _In_ HSLC hSLC, _In_opt_ CONST SLID *pProductSkuId,
        _In_opt_ CONST SL_ACTIVATION_INFO_HEADER *pActivationInfo,
        _Outptr_ PWSTR *ppwszInstallationId);
/*++
Routine Description:
...
Remark:
    ...
    Active Directory activation:
        ...
        AdaRequestInfo->header.type              =
SL_ACTIVATION_TYPE_ACTIVE_DIRECTORY; AdaRequestInfo->pwszProductKey           =
L"<5x5 Product Key>"; AdaRequestInfo->pwszActivationObjectName = NULL;
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLDepositOfflineConfirmationId(_In_ HSLC hSLC,
                                   _In_ CONST SLID *pProductSkuId,
...
Routine Description:
    ...
    Deposit Installation Id (IID) and Confirmation Id (CID) for off-line
activation.
...
__control_entrypoint(DllExport) HRESULT WINAPI SLDepositOfflineConfirmationIdEx(
    _In_ HSLC hSLC, _In_opt_ CONST SLID *pProductSkuId,
    _In_opt_ CONST SL_ACTIVATION_INFO_HEADER *pActivationInfo,
    _In_ PCWSTR pwszInstallationId, _In_ PCWSTR pwszConfirmationId);
/*++
Routine Description:
...
Remark:
    ...
    Active Directory activation:
        ...
        AdaDepositInfo->header.type              =
SL_ACTIVATION_TYPE_ACTIVE_DIRECTORY; AdaDepositInfo->pwszProductKey           =
L"<5x5 Product Key>"; AdaDepositInfo->pwszActivationObjectName = L"<string>"; //
or NULL
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetPKeyId(_In_ HSLC hSLC, _In_ PCWSTR pwszPKeyAlgorithm,
                _In_ PCWSTR pwszPKeyString, _In_ UINT cbPKeySpecificData,
                _In_reads_bytes_opt_(cbPKeySpecificData)
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetInstalledProductKeyIds(_In_ HSLC hSLC, _In_ CONST SLID *pProductSkuId,
                                _Out_ UINT *pnProductKeyIds,
                                _Outptr_result_buffer_(*pnProductKeyIds)
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLSetCurrentProductKey(_In_ HSLC hSLC, _In_ CONST SLID *pProductSkuId,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetSLIDList(_In_ HSLC hSLC, _In_ SLIDTYPE eQueryIdType,
                  _In_opt_ CONST SLID *pQueryId, _In_ SLIDTYPE eReturnIdType,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetLicenseFileId(_In_ HSLC hSLC, _In_ UINT cbLicenseBlob,
                       _In_reads_bytes_(cbLicenseBlob)
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetLicense(_In_ HSLC hSLC, _In_ CONST SLID *pLicenseFileId,
                 _Out_ UINT *pcbLicenseFile,
                 _Outptr_result_bytebuffer_(*pcbLicenseFile)
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLFireEvent(_In_ HSLC hSLC, _In_ PCWSTR pwszEventId,
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLRegisterEvent(_In_opt_ HSLC hSLC, _In_ PCWSTR pwszEventId,
                    _In_ CONST SLID *pApplicationId, _In_ HANDLE hEvent);
/*++
Routine Description:
...
Arguments:
    ...
    hSLC
        ...
        - Service is not running (hSLC is not used in that case):
            Service won't be started, event will be correctly registered on
service start.
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLUnregisterEvent(_In_opt_ HSLC hSLC, _In_ PCWSTR pwszEventId,
                      _In_ CONST SLID *pApplicationId, _In_ HANDLE hEvent);
/*++
Routine Description:
...
HRESULT
WINAPI
SLGetWindowsInformation(_In_ PCWSTR pwszValueName,
                        _Out_opt_ SLDATATYPE *peDataType, _Out_ UINT *pcbValue,
                        _Outptr_result_bytebuffer_(*pcbValue) PBYTE *ppbValue);
/*++
Routine Description:
...
HRESULT
WINAPI
SLGetWindowsInformationDWORD(_In_ PCWSTR pwszValueName, _Out_ DWORD *pdwValue);
/*++
Routine Description:
...
HRESULT
WINAPI
SLIsGenuineLocal(_In_ CONST SLID *pAppId, _Out_ SL_GENUINE_STATE *pGenuineState,
...
HRESULT
WINAPI
SLIsGenuineLocalEx(_In_ CONST SLID *pAppId, _In_opt_ CONST SLID *pSkuId,
                   _Out_ SL_GENUINE_STATE *pGenuineState);
/*++
Routine Description:

    Determines if an installation is a Genuine installation.
    If the SkuId is provided, it is used for the primary check.  If the Sku
license contains a ProductUniquenessGroupId value, that is also used.  Finally,
the AppId is used. If the AppId, ProductUniquenessGroupId, or SkuId contains a
"Tampered" flag or if the license state is invalid or "Tampered", the
installation is assumed to be invalid.
...
HRESULT
WINAPI
SLAcquireGenuineTicket(_Outptr_result_bytebuffer_(*pcbTicketBlob)
                           VOID **ppTicketBlob,
                       _Out_ UINT *pcbTicketBlob, _In_ PCWSTR pwszTemplateId,
...
HRESULT
WINAPI
SLSetGenuineInformation(_In_ CONST SLID *pQueryId, _In_ PCWSTR pwszValueName,
                        _In_ SLDATATYPE eDataType, _In_opt_ UINT cbValue,
...
HRESULT
WINAPI
SLGetReferralInformation(_In_ HSLC hSLC, _In_ SLREFERRALTYPE eReferralType,
                         _In_ CONST SLID *pSkuOrAppId,
                         _In_ PCWSTR pwszValueName, _Outptr_ PWSTR *ppwszValue);
/*++
      ...
      pwszValueName
          ...
          Allowable value pairs:
                ...
                SL_INSTALLATIONPARAMETERS
                    Any parameters that are used when running the associated
application's installer SL_MERCHANTSUPPORTPHONENUMBER The merchant support phone
number(s) SL_MERCHANTSUPPORTEMAIL The merchant support email address
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLPersistRTSPayloadOverride(_In_ HSLC hSLC, _In_ CONST SLID *pApplicationId,
                                _In_opt_ CONST SLID *pProductSkuId,
...
Routine Description:
      ...
      Associates information with the specified product for online and phone
activation
...
first three bytes: Byte[0] is the LSB of the HIWORD, Byte[1] is the HSB of the
LOWORD, and Byte[2] is the LSB of the LOWORD. Any value composed of these three
bytes that exceeds 20 bits will be rejected with E_INVALIDARG
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLPersistApplicationPolicies(_In_ CONST SLID *pApplicationId,
                                 _In_opt_ CONST SLID *pProductSkuId,
                                 _In_ DWORD dwFlags);
/*++
Routine Description:
...
Arguments:
    ...
    pProductSkuId
        optional - Identifier of the ACID to be used for the fast policy
queries.
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetApplicationPolicy(_In_ HSLP hPolicyContext, _In_ PCWSTR pwszValueName,
                           _Out_opt_ SLDATATYPE *peDataType,
                           _Out_ UINT *pcbValue,
                           _Outptr_result_bytebuffer_(*pcbValue)
                               PBYTE *ppbValue);
...
Routine Description:
    ...
    Queries a policy from the set loaded set stored with
SLPersistApplicationPolicies and loaded using SLLoadApplicationPolicies
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLLoadApplicationPolicies(_In_ CONST SLID *pApplicationId,
                              _In_opt_ CONST SLID *pProductSkuId,
                              _In_ DWORD dwFlags, _Out_ HSLP *phPolicyContext);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLUnloadApplicationPolicies(_In_ HSLP hPolicyContext, _In_ DWORD dwFlags);
/*++
Routine Description:
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLGetAuthenticationResult(_In_ HSLC hSLC, _Out_ UINT *pcbValue,
                              _Outptr_result_bytebuffer_(*pcbValue)
                                  PBYTE *ppbValue);
/*++
Routine Description:
...
done or failed. Or this feature is not enabled in licenses.
...
__control_entrypoint(DllExport) HRESULT WINAPI
    SLSetAuthenticationData(_In_ HSLC hSLC, _In_opt_ UINT cbValue,
                            _In_reads_bytes_opt_(cbValue) CONST BYTE *pbValue);
/*++
Routine Description:
...
HRESULT
WINAPI
SLGetGenuineInformation(_In_ CONST SLID *pQueryId, _In_ PCWSTR pwszValueName,
                        _Out_opt_ SLDATATYPE *peDataType, _Out_ UINT *pcbValue,
...
HRESULT
WINAPI
SLGetGenuineInformationEx(_In_ CONST SLID *pAppId, _In_ PCWSTR pwszValueName,
                          _Out_opt_ SLDATATYPE *peDataType,
                          _Out_ UINT *pcbValue,
                          _Outptr_result_bytebuffer_(*pcbValue)
                              BYTE **ppbValue);
...
HRESULT
WINAPI
SLGatherMigrationBlob(_In_ BOOL bMigratableOnly,
                      _In_opt_ LPCWSTR pwszEncryptorUri, _In_ HANDLE hFile);
/*+
Routine Description:

    Gathers licensing information into provided file handle. This licensing
information can later be applied/deposited using SLDepositMigrationBlob.
...
HRESULT
WINAPI
SLGatherMigrationBlobEx(__in DWORD dwFlags, __in_opt LPCWSTR pwszEncryptorUri,
                        __in HANDLE hFile);
/*+
Routine Description:
...
Arguments:
    ...
    dwFlags
        ...
        SPP_MIGRATION_GATHER_MIGRATABLE_APPS
            Any license and corresponding product key data that is marked as
migratable
...
HRESULT
WINAPI
SLDepositMigrationBlob(_In_ HANDLE hFile);
/*+
Routine Description:

    Deposits licensing information previously collected/gathered using
...
--*/
STDAPI
SLQueryLicenseValueFromApp(_In_ PCWSTR valueName, _Out_opt_ ULONG *valueType,
                           _Out_writes_bytes_to_opt_(dataSize, *resultDataSize)
                               PVOID dataBuffer,
                           _In_ ULONG dataSize, _Out_ ULONG *resultDataSize);
