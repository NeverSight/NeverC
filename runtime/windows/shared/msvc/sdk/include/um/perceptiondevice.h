...
HRESULT
WINAPI
PerceptionDeviceCreateFactory(_In_ REFIID riid, _COM_Outptr_ LPVOID *ppFactory);

#if NTDDI_VERSION >= NTDDI_WIN10_VB

// Inject an alternate PerceptionDeviceCreateFactory implementation.
// The original implementation is restored when
// PerceptionDeviceSetCreateFactoryOverride is called with createFactoryOverride
// = null.
EXTERN_C
HRESULT
WINAPI
...
IPerceptionDevicePropertyChangedHandler : public IUnknown {
  STDMETHOD(Invoke)(_In_ IPerceptionDevicePropertyListener * sender,
                    _In_ IPerceptionDevicePropertyChangedEventArgs *
                        eventArgs) = 0;
};
...
IPerceptionDevicePropertyListenerAbortedEventArgs : public IUnknown {
  STDMETHOD_(void, GetErrorCode)(_Out_ HRESULT * errorCode) = 0;
};

MIDL_INTERFACE("{A5082165-2B1A-4C4E-A0D0-3D3D6C4091C5}")
IPerceptionDevicePropertyListenerAbortedHandler : public IUnknown {
  STDMETHOD(Invoke)(_In_ IPerceptionDevicePropertyListener2 * sender,
                    _In_ IPerceptionDevicePropertyListenerAbortedEventArgs *
                        eventArgs) = 0;
};
...
IPerceptionDevicePropertyListener : public IUnknown {
  ...
  //! The callback will be invoked with the initial value of the property as
  //! soon as possible after Start() is called. Note that the callback may be
  //! invoked even before Start() returns.
  STDMETHOD(Start)() = 0;
  ...
  STDMETHOD_(__success(true) void,
             GetDevice)(_COM_Outptr_ IPerceptionDevice * *device) = 0;

  //! Returns the id of the object that the watched property belongs to.
  //! Returned reference is only valid as long as a strong reference is held to
  //! the listener.
...
IPerceptionDevicePropertyListener2 : public IPerceptionDevicePropertyListener {
  ...
  //! The aborted handler is called if an internal property request fails. When
  //! an internal io request fails after the listener has been started, the
  //! error is surfaced via the aborted handler and the listener is stopped.
...
IPerceptionDevice : public IUnknown {
  ...
  //! Returns HRESULT_FROM_WIN32(ERROR_INVALID_DATA) if driver doesn't return
  //! the expected amount of data.
  STDMETHOD(ReadProperty)(REFGUID objectId, REFGUID propertyId, UINT valueSize,
                          _Out_writes_bytes_all_(valueSize) void *value,
                          _Out_opt_ LUID *changeId) = 0;

  //! Get a property of variable size.
  //! The returned buffer is is CoTaskMemAlloc'd, and the caller must free it
  //! with CoTaskMemFree.
  STDMETHOD(ReadVariableSizeProperty)(
      REFGUID objectId, REFGUID propertyId, _Out_ UINT * valueSize,
      _Outptr_result_bytebuffer_all_maybenull_(*valueSize) void **value,
      _Out_opt_ LUID *changeId) = 0;

  //! Create an instance of IPropertyListener that listens for changes to the
  //! specified property on the specified object. objectId may be GUID_NULL to
  //! listen to a top-level property on the device. This method will fail if the
  //! property is not supported. This method will fail if the device returns a
  //! failure when querying the property. Will invoke the callback with initial
  //! value as soon as possible after creation.
  STDMETHOD(CreatePropertyListener)(
      REFGUID objectId, REFGUID propertyId,
      _COM_Outptr_ IPerceptionDevicePropertyListener * *listener) = 0;

  //! Synchronously send a custom command to the object, with a known /
  //! fixed-size output,
  //!
  //! Returns HRESULT_FROM_WIN32(ERROR_INVALID_DATA) if the driver doesn't
  //! return the expected amount of data. The inBuffer may be nullptr if
  //! inBufferSize is zero, and the command does not require input.
  //! outBufferSize and outBuffer may be 0 and nullptr if the command does not
  //! generate output.
  STDMETHOD(SendCommand)(
      REFGUID objectId, REFGUID commandId, UINT inBufferSize,
  ...
  //! The inBuffer may be nullptr if inBufferSize is zero, and the command does
  //! not require input. outBufferSize and outBuffer may be nullptr if the
  //! command does not generate output. Resulting memory is CoTaskMemAlloc'd.
  //! The caller must free the buffer with CoTaskMemFree.
  STDMETHOD(SendCommandWithVariableSizeOutput)(
      REFGUID objectId, REFGUID commandId, UINT inBufferSize,
      _In_reads_bytes_opt_(inBufferSize) const void *inBuffer,
      _Out_opt_ UINT *outBufferSize,
      _Outptr_opt_result_bytebuffer_all_maybenull_(
          *outBufferSize) void **outBuffer) = 0;

  //! Returns the device interface path for this device.
  //! The returned pointer is only valid while the caller holds a reference to
  //! this device object.
  ...
  //! All IPerceptionDevice instances that are communicating with the same
  //! device and driver session will have the same LifetimeId. This can be used
  //! to disambiguate unplug / replug / driver crash situations.
  STDMETHOD_(REFGUID, GetLifetimeId)() = 0;

  //! Create an IPerceptionDeviceStateStream from this device for the specified
  //! state stream object id.
  STDMETHOD(CreateStateStream)(REFGUID stateStreamObjectId,
                               _COM_Outptr_ IPerceptionDeviceStateStream *
                                   *stateStream) = 0;
  ...
  STDMETHOD(CreateObjectSubscription)(
      REFGUID objectId, UINT subscriptionFlags,
...
IPerceptionDeviceRootObjectAddedHandler : public IUnknown {
  STDMETHOD(Invoke)(_In_ IPerceptionDeviceRootObjectWatcher * sender,
...
IPerceptionDeviceRootObjectRemovedHandler : public IUnknown {
  STDMETHOD(Invoke)(_In_ IPerceptionDeviceRootObjectWatcher * sender,
                    _In_ IPerceptionDeviceRootObjectRemovedEventArgs * args) =
      0;
};
...
IPerceptionDeviceRootObjectWatcherEnumerationCompletedHandler
    : public IUnknown {
  STDMETHOD(Invoke)(_In_ IPerceptionDeviceRootObjectWatcher2 * sender,
...
IPerceptionDeviceRootObjectWatcher : public IUnknown {
  ...
  //! The callback must be set before calling Start().
  STDMETHOD(SetAddedHandler)(_In_ IPerceptionDeviceRootObjectAddedHandler *
                             handler) = 0;

  //! Sets the removed callback.
  //! The callback must be set before calling Start().
  STDMETHOD(SetRemovedHandler)(_In_ IPerceptionDeviceRootObjectRemovedHandler *
                               handler) = 0;
...
IPerceptionDeviceRootObjectWatcher2
    : public IPerceptionDeviceRootObjectWatcher {
  ...
  //! Object removals may occur after the enumeration completed callback has
  //! been called.
  STDMETHOD(SetEnumerationCompletedHandler)(
      _In_ IPerceptionDeviceRootObjectWatcherEnumerationCompletedHandler *
      handler) = 0;
};
...
IPerceptionDevicePayloadDescriptor : public IUnknown {
  ...
  STDMETHOD_(_Ret_maybenull_ const PERCEPTION_PAYLOAD_FIELD *,
             FindFieldById)(REFGUID fieldId) = 0;
};

MIDL_INTERFACE("{C2FB5866-1C71-4B74-89E1-EA13F2709B11}")
IPerceptionDeviceStateStream : public IUnknown {
  //! Gets the payload descriptor. Does not fail, because the descriptor was
  //! read and validated during creation of the IPerceptionDeviceStateStream
  //! object.
  ...
  STDMETHOD(CreateReader)(UINT subscriptionFlags,
                          _COM_Outptr_ IPerceptionDeviceStateStreamReader *
                              *reader) = 0;
};

MIDL_INTERFACE("{3B84AAB1-D2BE-4D26-B285-E21CDE1DE058}")
IPerceptionDeviceStateStreamReader : public IUnknown {
  //! Reads the next one or more states AFTER the specified timestamp into
  //! caller-supplied memory Reads into caller-allocated memory. bufferSize
  //! should be a multiple of the payload size. On output, *numBytesRead will be
  //! divisible by the payload size.
  STDMETHOD(ReadStatesAfterTime)(
      LONGLONG timestampInQpcTicks, UINT bufferSize,
...
IPerceptionDeviceRootObjectCollection : public IUnknown {
  ...
  //! Pointer to the first element in an array of pointers to
  //! IPerceptionDeviceRootObject.
  STDMETHOD_(_Ret_opt_count_x_(GetCount()) IPerceptionDeviceRootObject *const *,
             GetData)() = 0;
};
...
IPerceptionDeviceFactory : public IUnknown {
  ...
  //! and options.
  //!
  STDMETHOD(OpenDevice)(_In_z_ LPCWSTR deviceInterfacePath,
                        PerceptionDeviceOptions options,
                        _COM_Outptr_ IPerceptionDevice * *device) = 0;

  //!
  //! Create an IPayloadDescriptor from a buffer.  For use when you need a
  //! payload descriptor without a state stream.  Makes a copy of the input
  //! buffer, so the caller does not need to keep the buffer alive after the
  //! method returns.
  //!
  STDMETHOD(CreatePayloadDescriptor)(
      UINT size, _In_bytecount_(size) const void *data,
      _COM_Outptr_ IPerceptionDevicePayloadDescriptor **descriptor) = 0;
  ...
  STDMETHOD(CreateRootObjectWatcher)(
      UINT propertyIdCount, _In_reads_(propertyIdCount) const GUID *propertyIds,
...
IPerceptionDeviceFactory2 : public IPerceptionDeviceFactory {
  //! Check if any devices exist that support any of the given root object
  //! property ids. IsRootObjectSupported will return true if a root object is
  //! found, even if it is not accessible.
  STDMETHOD(IsRootObjectSupported)(
      UINT propertyIdCount, _In_reads_(propertyIdCount) const GUID *propertyIds,
      _Out_ bool *isSupported) = 0;

  //! Enumerate all existing objects on devices that support any of the given
  //! set of property ids. Only one object will be returned per each device.
  //! Like the RootObjectWatcher, objects that correspond with the front-most
  //! property id are prioritized. Inaccessible objects are silently excluded
  //! from the set of root objects that is returned.
  STDMETHOD(FindAllRootObjects)(
      UINT propertyIdCount, _In_reads_(propertyIdCount) const GUID *propertyIds,
      PerceptionDeviceOptions deviceOptions,
      _COM_Outptr_ IPerceptionDeviceRootObjectCollection *
          *ppRootObjectCollection) = 0;
};
