...
#define KBDNLS_CONV_OR_NONCONV                                                 \
  15 // VK_CONVERT and VK_NONCONVERT [Fujitsu FMV oyayubi Only]
...
#define FUJITSU_KBD_CONSOLE(Id)                                                \
  (JAPANESE_KEYBOARD(Id) &&                                                    \
   (Id).Subtype == ((NLSKBD_OEM_FUJITSU << 4) | FMV_KBD_OASYS_TYPE))
// This number 0x00020002 is registered in registry key as
// HKLM\System\CurrentControlSet\Control\Terminal Server\KeyboardType
// Mapping\JPN
#define FUJITSU_KBD_REMOTE(Id)                                                 \
  (JAPANESE_KEYBOARD(Id) && (Id).SubType == 0x00020002)
...
#define T56                                                                    \
  _NE(OEM_102, HELP, OEM_102, OEM_102, _none_, OEM_PA2) // KB3270 => LEFT
...
*     | Scan | |   kbd    |   kbd    |   kbd    |   kbd    |   kbd    |   kbd |
kbd    |
*     | code | |  type 7  |  type 8  |  type 16 |  type 10 |  type 11 |  type 12
|  type 13 |
\****+-------+-+----------+----------+----------+----------+----------+----------+----------+*/
#define T00 _EQ(_none_)
#define T01 _EQ(ESCAPE)
...
#define T39 _EQ(' ')
#define T3A                                                                    \
  _NE(DBE_ALPHANUMERIC, DBE_ALPHANUMERIC, CAPITAL, CAPITAL, CAPITAL, CAPITAL,  \
      CAPITAL)
#define T3B _EQ(F1)
#define T3C _EQ(F2)
#define T3D _EQ(F3)
...
#define T59 _EQ(CLEAR)
#define T5A                                                                    \
  _NE(NONAME, NONAME, NONCONVERT, OEM_WSCTRL, OEM_WSCTRL, OEM_WSCTRL,          \
      OEM_WSCTRL)
#define T5B                                                                    \
  _NE(NONAME, NONAME, CONVERT, OEM_FINISH, OEM_FINISH, OEM_FINISH, OEM_FINISH)
#define T5C _NE(NONAME, NONAME, OEM_AX, OEM_JUMP, OEM_JUMP, OEM_JUMP, OEM_JUMP)
#define T5D _EQ(EREOF)
#define T5E                                                                    \
  _NE(_none_, _none_, _none_, OEM_BACKTAB, OEM_BACKTAB, OEM_BACKTAB,           \
      OEM_BACKTAB)
...
#define T70                                                                    \
  _NE(DBE_KATAKANA, DBE_HIRAGANA, _none_, _none_, _none_, _none_, _none_)
#define T71                                                                    \
  _NE(_none_, _none_, _none_, OEM_RESET, OEM_RESET, OEM_RESET, OEM_RESET)
#define T72 _EQ(_none_)
...
#define T7B                                                                    \
  _NE(NONCONVERT, NONCONVERT, _none_, OEM_PA1, OEM_PA1, OEM_PA1, OEM_PA1)
#define T7C _EQ(TAB)
...
#define X1C _EQ(RETURN)
#define X1D                                                                    \
  _NE(RCONTROL, RCONTROL, DBE_KATAKANA, HANJA, HANGEUL, RCONTROL, RCONTROL)
#define X20 _EQ(VOLUME_MUTE)      // SpeedRacer
...
#define T29 _NE(DBE_SBCSCHAR, DBE_SBCSCHAR, OEM_3)
#define T2A _EQ(LSHIFT)
...
#define T74 _NE(_none_, OEM_NEC_EQUAL, _none_)
