...
//
STDAPI_(DWORD)
OfflineFilesEnable(_In_ BOOL bEnable, _Out_ BOOL *pbRebootRequired);
...
//
// Query the active/inactive and enabled/disabled state of the Offline Files
// feature.
//
//  - active/inactive describes the current running state.
//
//  - enabled/disabled describes the configured state as last successfully set
//  by
//    OfflineFilesEnable().
//
STDAPI_(DWORD)
OfflineFilesQueryStatus(_Out_opt_ BOOL *pbActive, _Out_opt_ BOOL *pbEnabled);

#if (NTDDI_VERSION >= NTDDI_WIN8)
//
// Query the active/inactive, enabled/disabled and available/unavailable state
...
//    If the status is inactive/enabled/available then the OfflineFilesStart
//    function can be used to transition the Offline Files feature to active
//    state (no reboot required).
//
STDAPI_(DWORD)
OfflineFilesQueryStatusEx(_Out_opt_ BOOL *pbActive, _Out_opt_ BOOL *pbEnabled,
