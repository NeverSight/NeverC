...
#define DFS_VALID_PROPERTY_FLAGS                                               \
  (DFS_PROPERTY_FLAG_INSITE_REFERRALS | DFS_PROPERTY_FLAG_ROOT_SCALABILITY |   \
   DFS_PROPERTY_FLAG_SITE_COSTING | DFS_PROPERTY_FLAG_TARGET_FAILBACK |        \
   DFS_PROPERTY_FLAG_CLUSTER_ENABLED | DFS_PROPERTY_FLAG_ABDE)
...
#define DFS_RESTORE_VOLUME                                                     \
  2 // Volume/Replica is being restored - do not verify share etc.
...
//
// Structures used for site reporting.  Last used in Windows 2000, maintained
// for the obsolete SRVSVC RPC API NetrDfsManagerReportSiteInfo.
//
...
//
_When_(Level == 1, _At_(Buffer, _Outptr_result_bytebuffer_(*EntriesRead *
                                                           sizeof(DFS_INFO_1))))
    _When_(Level == 2,
           _At_(Buffer,
                _Outptr_result_bytebuffer_(*EntriesRead * sizeof(DFS_INFO_2))))
        _When_(Level == 3,
                        ...
                        _When_(Level == 8,
                               _At_(Buffer,
                                    _Outptr_result_bytebuffer_(
                                        *EntriesRead * sizeof(DFS_INFO_8))))
                            _When_(Level == 9,
                                   _At_(Buffer,
                                        _Outptr_result_bytebuffer_(
                                            *EntriesRead * sizeof(DFS_INFO_9))))
                                _When_(Level == 200,
                                       _At_(Buffer, _Outptr_result_bytebuffer_(
                                                        *EntriesRead *
                                                        sizeof(DFS_INFO_200))))
                                    _When_(Level == 300,
                                           _At_(Buffer,
    ...
    NetDfsEnum(
        ...
        _Out_ LPBYTE
            *Buffer, // API allocates and returns buffer with requested info
        _Out_ LPDWORD EntriesRead, // Number of entries returned
        _Inout_ LPDWORD
            ResumeHandle // Must be 0 on first call, reused on subsequent calls
    );
...
_When_(Level == 1, _At_(Buffer, _Outptr_result_bytebuffer_(sizeof(DFS_INFO_1))))
    _When_(Level == 2,
           _At_(Buffer, _Outptr_result_bytebuffer_(sizeof(DFS_INFO_2))))
        _When_(Level == 3,
               _At_(Buffer, _Outptr_result_bytebuffer_(sizeof(DFS_INFO_3))))
            _When_(Level == 4,
                   _At_(Buffer, _Outptr_result_bytebuffer_(sizeof(DFS_INFO_4))))
                _When_(Level == 5,
                       _At_(Buffer,
                            _Outptr_result_bytebuffer_(sizeof(DFS_INFO_5))))
                    _When_(Level == 6,
                           _At_(Buffer,
                                _Outptr_result_bytebuffer_(sizeof(DFS_INFO_6))))
                        _When_(Level == 7,
                               _At_(Buffer, _Outptr_result_bytebuffer_(
                                                sizeof(DFS_INFO_7))))
                            _When_(Level == 8,
...
NET_API_STATUS NET_API_FUNCTION NetDfsAddRootTarget(_In_ LPWSTR pDfsPath,
...
NET_API_STATUS NET_API_FUNCTION NetDfsRemoveRootTarget(
    _In_ LPWSTR pDfsPath, _In_opt_ LPWSTR pTargetPath, _In_ ULONG Flags);

NET_API_STATUS NET_API_FUNCTION NetDfsGetSecurity(
    _In_ LPWSTR DfsEntryPath, _In_ SECURITY_INFORMATION SecurityInformation,
    _Outptr_result_bytebuffer_(*lpcbSecurityDescriptor)
...
typedef enum {
  DFS_NAMESPACE_VERSION_ORIGIN_COMBINED =
      0,                               //  Max version {server, AD DS domain}
                                       //      can support.
...
NET_API_STATUS NET_API_FUNCTION NetDfsGetSupportedNamespaceVersion(
    _In_ DFS_NAMESPACE_VERSION_ORIGIN Origin, _In_opt_ PWSTR pName,
    _Outptr_ PDFS_SUPPORTED_NAMESPACE_VERSION_INFO *ppVersionInfo);

#ifndef _DFSFSCTL_
#define FSCTL_DFS_GET_PKT_ENTRY_STATE                                          \
  CTL_CODE(FSCTL_DFS_BASE, 2031, METHOD_BUFFERED, FILE_ANY_ACCESS)

//  FSCTL_DFS_GET_PKT_ENTRY_STATE Input Buffer
//  All the strings appear in Buffer in the same order as the length fields. The
//  strings are not NULL terminated. The length values are in bytes.
typedef struct {
