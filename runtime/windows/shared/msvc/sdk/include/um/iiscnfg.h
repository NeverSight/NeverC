...
//      the files and directories - such as Access permissions (Read, Write
//      etc), Extension mapping, IP Security etc.
//
...
#define IIS_MD_UT_END_RESERVED                                                 \
  2000 // All user types below this are
...
#define ASP_MD_ID_BEGIN_RESERVED                                               \
  0x00007000 // ASP reserved range, subrange of IIS.
#define ASP_MD_ID_END_RESERVED 0x000074ff
#define WAM_MD_ID_BEGIN_RESERVED                                               \
  0x00007500 // ASP reserved range, subrange of IIS.
...
#define MD_LOGCUSTOM_SERVICES_STRING                                           \
  (IIS_MD_LOGCUSTOM_BASE +                                                     \
   6) // MultiSZ List of services that the property is applicable to.
...
#define MD_ASP_ALLOWOUTOFPROCCMPNTS                                            \
  (MD_ASP_ALLOWOUTOFPROCCOMPONENTS) // Deprecated.  Use
...
/*
  ...
  This object is composed of 4 lists : 2 lists ( deny & grant ) of network
addresses, the only allowed family is AF_INET. Each of this list is composed of
sublists, one for each ( network address family, significant subnet mask )
combination. The significant subnet mask is stored as ( number of bytes all 1 (
0xff ), bitmask in last byte ). This is followed by 2 lists ( deny & grant ) of
DNS names. Each of these lists is composed of sublists, based on then number of
components in the DNS name e.g. "microsoft.com" has 2 components, "www.msft.com"
has 3.
