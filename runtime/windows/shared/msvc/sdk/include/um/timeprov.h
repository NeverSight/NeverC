...
//          optional MetadataProvider parameter are set to false, this provider
//          is as an output provider.
#define wszW32TimeRegValueInputProvider L"InputProvider"

// Path:    ...\TimeProviders\<PrividerName>\MetaDataProvider
// Type:    REG_DWORD (cast to BOOL)
// Meaning: Optional parameter. If InputProvider parameter is true and this
// parameter is set to true,
//          this provider is a metadata provider, and will be called upon to
//          return time metadata. If both InputProvider and this parameter are
//          set to false, this provider is as an output provider. Additional
//          checks are applied before loading a metadata provider. See
...
typedef enum TimeSysInfo {
  ...
  TSI_CurrentTime, // (unsigned __int64 *)pvInfo, UTC-compatible NtTimeEpoch, in
                   // (10^-7)s. This removes the leap seconds, if any, from the
                   // system time.
  TSI_PhaseOffset, // (  signed __int64 *)pvInfo, opaque
  TSI_TickCount,   // (unsigned __int64 *)pvInfo, opaque
  TSI_LeapFlags,   // (            BYTE *)pvInfo, a warning of an impending leap
                   // second or loss of synchronization
  TSI_Stratum, // (            BYTE *)pvInfo, how far away the computer is from
               // a reference source
  ...
  TSI_CurrentUtcOffset, // (  signed __int64 *)pvInfo, Number of leap seconds
                        // elapsed since 12:00 AM June 1st 2018. This can be a
                        // +ve or -ve number. Add this number to the time value
                        // after June 1st 2018 obtained using TSI_CurrentTime to
                        // determine the approximate invariant time. This can be
                        // used with loose consistency for time computations.
                        // The consistency around leap second roll over is
                        // undefined.
} TimeSysInfo;
...
typedef void(__stdcall SetProviderStatusInfoFreeFunc)(
...
typedef struct SetProviderStatusInfo {
  ...
  LPWSTR wszProvName; // IN   the name of the provider who's status should be
                      // adjusted
  HANDLE hWaitEvent; // IN   the event to signal when the operation is complete,
                     // NULL if notification is not needed
  SetProviderStatusInfoFreeFunc
      *pfnFree; // IN   function used to free the struct on completion
...
typedef HRESULT(__stdcall GetTimeSysInfoFunc)(IN TimeSysInfo eInfo,
                                              OUT void *pvInfo);

// Time Service provided callback to log an event on behalf of the Time
// Provider.
typedef HRESULT(__stdcall LogTimeProvEventFunc)(IN WORD wType,
                                                IN WCHAR *wszProvName,
                                                IN WCHAR *wszMessage);

// Time Service provided callback to inform the system of newly available
// samples
...
typedef struct TimeSample {
  ...
  signed __int64 toOffset; // NtTimeOffset, in (10^-7)s - difference between
                           // local and remote clocks
  signed __int64 toDelay;  // NtTimeOffset, in (10^-7)s - round trip delay; time
                           // packets spent in flight, INCLUDING root delay
  unsigned __int64 tpDispersion; // NtTimePeriod, in (10^-7)s - measurement
                                 // error, INCLUDING root dispersion
  unsigned __int64
      nSysTickCount; // opaque, must be GetTimeSysInfo(TSI_TickCount)
  signed __int64
      nSysPhaseOffset; // opaque, must be GetTimeSysInfo(TSI_PhaseOffset)
  BYTE nLeapFlags;     // a warning of an impending leap second or loss of
                       // synchronization
  BYTE nStratum;       // how far away the computer is from a reference source
  DWORD dwTSFlags;     // time source flags
  WCHAR wszUniqueName[256]; // Admin readable name that uniquely identifies this
                            // peer
} TimeSample;
...
HRESULT __stdcall
TimeProvOpen(_In_ IN PWSTR wszName,
             _In_ IN TimeProvSysCallbacks
                 *pSysCallbacks, // copy this data, do not free it!
             _Out_ OUT TimeProvHandle *phTimeProv);

HRESULT __stdcall TimeProvCommand(_In_ IN TimeProvHandle hTimeProv,
                                  _In_ IN TimeProvCmd eCmd,
                                  _In_ IN TimeProvArgs pvArgs);

HRESULT __stdcall TimeProvClose(_In_ IN TimeProvHandle hTimeProv);
