...
//      Probable causes:
//          - Broker m/c account does not have required permission on the
//          requested OU
//          - AD DC unreachable
//      Event log gives more detailed info about the failure
//
#define E_SB_PROV_DELETE_VM_AD_ACCOUNT_FAILED                                  \
  MAKE_SBTSV_ERR(SB_PROV_ERR_BASE + 1)

//
// Broker failed to find the specified VMHost name in the list of currently
// connected VMHosts
//      Probable causes:
//          - Incorrect hyperV host name
//          - specified hyperV host is not joined to VM host
//          - specified hyperV host is not running/unreachable/VMHA service not
//          running
//      Event log gives more detailed info about the failure
//
...
//
// The VM name specified in the CREATE pool request is longer then supported
// Netbios name
//
...
//      - provisioning op. for some other VM in the Job request failed and
//      StopOnError was specified.
...
//
// the VM specified in the selectedVMs list in the provisioning job is not
// present in broker Target database
//
#define E_SB_PROV_SELECTED_VM_NOT_FOUND MAKE_SBTSV_ERR(SB_PROV_ERR_BASE + 8)

// TODO: added error code Operation aborted to distinguish between user
// cancelled action and
//  provisioning operation aborted when stop on error was specified.

//
// The Wmi object (win32_RdPublishedFarm) does not exist for the pool name in
...
//
#define E_SB_PROV_UNATTEND_XML_NOT_SUPPORTED                                   \
  MAKE_SBTSV_ERR(SB_PROV_ERR_BASE + 10)
...
//
#define E_SB_PROV_VM_AD_ACCOUNT_ALREADY_EXISTS                                 \
  MAKE_SBTSV_ERR(SB_PROV_ERR_BASE + 12)
...
//      Event log gives more detailed info about the failure
//
#define E_SB_PROV_CREATE_VM_AD_ACCOUNT_FAILED                                  \
  MAKE_SBTSV_ERR(SB_PROV_ERR_BASE + 13)
...
//          - No read/write permission to VMHA service/VMHA Machine account on
//          Base VM location
//          - if BaseVmLocation is on SMB, then there could N/W connectivity
//          issues
//      Event log gives more detailed info about the failure
//
...
//      Event log gives more detailed info about the failure
//
#define E_SB_PROV_INVALID_VM_CREATION_PATH                                     \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 2)
...
//      Event log gives more detailed info about the failure
//
#define E_SB_PROV_INVALID_SMB_SHARE_PATH                                       \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 3)

//
// VMHost Agent failed to cache the Gold VM from Base VM location to
// LocalGOldCache
//      Probable causes:
...
//      Event log gives more detailed info about the failure
//
#define E_SB_PROV_MASTER_VHD_CACHING_FAILED                                    \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 4)
...
//
#define E_SB_PROV_CREATE_DIFF_VHD_FAILED                                       \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 5)

//
// TODO: Applicable when we move to V2 namespace
//
#define E_SB_PROV_CREATE_CLONE_VHD_FAILED                                      \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 6)
...
//          - hyperv specific failure
//      VMHA and hyperV Event Event log gives more detailed info about the
//      failure
//
#define E_SB_PROV_MIGRATE_STORAGE_FAILED                                       \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 11)

//
// VMHost Agent failed to import from BASE VM Location
//      Probable causes:
//          - Invalid import files in BaseVmLocation
//          - hyperV configuration does not match the host from where gold was
//          exported (e.g. External Virtual N/W switch name)
...
//      VMHA and hyperV Event log gives more detailed info about the failure
//
#define E_SB_PROV_DISCONNECT_VHD_FAILED                                        \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 14)
...
//      VMHA and hyperV Event log gives more detailed info about the failure
//
#define E_SB_PROV_GEN_QUERY_OFFVM_FAILURE                                      \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 16)
...
//      VMHA and hyperV Event log gives more detailed info about the failure
//
#define E_SB_PROV_MULTIPLE_WINDOWS_FOLDERS                                     \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 20)
...
//          -
//
#define E_SB_PROV_GOLDVM_NOT_GENERALIZED                                       \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 21)
...
//          -
//
#define E_SB_PROV_GOLDVM_IC_UPGRADE_REQUIRED                                   \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 22)

//
// VMHost Agent failed to verify gold VM export share
//      Probable causes:
//          - Import operation failed on gold cache during initial validation
//          - VmHost there gold exited and the Vmhost which returned this error
//          are not configured identically
//          - The export share is corrupted
//
#define E_SB_PROV_GOLDVM_INVALID_EXPORT_SHARE                                  \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 23)

//
// VMHost Agent did not find any network adapter attached to the VM
//
#define E_SB_PROV_NO_NETWORK_ADAPTER_ATTACHED                                  \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 24)

//
// VMHost Agent did not find the Network connected to the network adapter of the
// VM
//
#define E_SB_PROV_NETWORK_NOT_CONNECTED                                        \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 25)

//
// VMHost Agent failed to add a VM to a Farm, because the VM does not have a
// SCSI controller. Please configure a SCSI controller in the gold image of the
// Farm, re-create the VM and re-try adding it to the Farm.
//
#define E_VM_NEEDS_SCSI_CONTROLLER MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 26)

//
// VMHost Agent failed to provision VM because there is not enough RAM assigned
// to Gold VM If Dynamic Memory is enabled, then the Max RAM must be set to at
// least 1024MB, otherwise, Startup RAM must be at least 1024GB
//
#define E_SB_PROV_GOLDVM_NOT_ENOUGH_RAM                                        \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 27)

//
// ServerSKUs are not supported as the GuestOS.
//
#define E_SB_PROV_SERVER_SKU_UNSUPPORTED                                       \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 28)

//
// Timed out waiting for SysPrep specialize to be complete
//
#define E_SB_PROV_VM_SYSPREP_SPECIALIZE_INCOMPLETE                             \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 29)

//
// Sysprep specialize is complete, not an error in most cases
//
#define E_SB_PROV_VM_SYSPREP_SPECIALIZE_COMPLETE                               \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 30)

//
// Failed to successfully detect VM's domain-joinedness
//
#define E_SB_PROV_VM_FAILED_DOMAIN_CHECK                                       \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 31)

//
// Failed to communicate with the guest
//
#define E_SB_PROV_VM_FAILED_GUEST_COMMUNICATION                                \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 32)

//
// VM is not in fully deployable state
//
#define E_SB_PROV_VM_NOT_DEPLOYABLE_STATE                                      \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 33)
...
// VM is in an invalid state for provisioning operation
//
#define E_SB_PROV_VM_GENERATION_NOT_SUPPORTED                                  \
  MAKE_SBTSV_ERR(SB_PROV_VMHA_ERR_BASE + 35)
