...
#define INSTALLFLAG_FORCE                                                      \
  0x00000001 // Force the installation of the specified driver
#define INSTALLFLAG_READONLY 0x00000002 // Do a read-only install (no file copy)
#define INSTALLFLAG_NONINTERACTIVE                                             \
  0x00000004 // No UI shown at all. API will fail if any UI must be shown.
#define INSTALLFLAG_BITS 0x00000007

#if (WINVER >= _WIN32_WINNT_WIN2K)

BOOL WINAPI UpdateDriverForPlugAndPlayDevicesA(_In_opt_ HWND hwndParent,
                                               _In_ LPCSTR HardwareId,
                                               _In_ LPCSTR FullInfPath,
                                               _In_ DWORD InstallFlags,
                                               _Out_opt_ PBOOL bRebootRequired);

BOOL WINAPI UpdateDriverForPlugAndPlayDevicesW(_In_opt_ HWND hwndParent,
...
//
#define DIIDFLAG_SHOWSEARCHUI                                                  \
  0x00000001 // Show search UI if no drivers can be found.
#define DIIDFLAG_NOFINISHINSTALLUI                                             \
  0x00000002 // Do NOT show the finish install UI.
#define DIIDFLAG_INSTALLNULLDRIVER                                             \
  0x00000004 // Install the NULL driver on this device.
#define DIIDFLAG_INSTALLCOPYINFDRIVERS                                         \
  0x00000008 // Install any extra INFs specified via CopyInf directive.
#define DIIDFLAG_BITS 0x0000000F

BOOL WINAPI DiInstallDevice(_In_opt_ HWND hwndParent,
                            _In_ HDEVINFO DeviceInfoSet,
                            _In_ PSP_DEVINFO_DATA DeviceInfoData,
                            _In_opt_ PSP_DRVINFO_DATA DriverInfoData,
                            _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);

//
// Flags for DiInstallDriver
//
#define DIIRFLAG_INF_ALREADY_COPIED                                            \
  0x00000001                          // Don't copy inf, it has been published
#define DIIRFLAG_FORCE_INF 0x00000002 // use the inf as if users picked it.
#define DIIRFLAG_HW_USING_THE_INF                                              \
  0x00000004                         // limit installs on hw using the inf
...
#define DIIRFLAG_BITS                                                          \
  (DIIRFLAG_FORCE_INF | DIIRFLAG_HOTPATCH | DIIRFLAG_PRE_CONFIGURE_INF |       \
   DIIRFLAG_INSTALL_AS_SET)
#define DIIRFLAG_SYSTEM_BITS                                                   \
  (DIIRFLAG_INF_ALREADY_COPIED | DIIRFLAG_FORCE_INF |                          \
   DIIRFLAG_HW_USING_THE_INF | DIIRFLAG_HOTPATCH | DIIRFLAG_NOBACKUP |         \
   DIIRFLAG_PRE_CONFIGURE_INF | DIIRFLAG_INSTALL_AS_SET)

#endif // (WINVER >= _WIN32_WINNT_WINBLUE)

BOOL WINAPI DiInstallDriverW(_In_opt_ HWND hwndParent, _In_ LPCWSTR InfPath,
                             _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);

BOOL WINAPI DiInstallDriverA(_In_opt_ HWND hwndParent, _In_ LPCSTR InfPath,
                             _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);
...
BOOL WINAPI DiUninstallDevice(_In_ HWND hwndParent, _In_ HDEVINFO DeviceInfoSet,
                              _In_ PSP_DEVINFO_DATA DeviceInfoData,
                              _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);
...
#define DIURFLAG_VALID (DIURFLAG_NO_REMOVE_INF | DIURFLAG_RESERVED)

BOOL WINAPI DiUninstallDriverW(_In_opt_ HWND hwndParent, _In_ LPCWSTR InfPath,
                               _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);

BOOL WINAPI DiUninstallDriverA(_In_opt_ HWND hwndParent, _In_ LPCSTR InfPath,
                               _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);
...
#if (WINVER >= _WIN32_WINNT_LONGHORN)
BOOL WINAPI DiShowUpdateDevice(_In_opt_ HWND hwndParent,
                               _In_ HDEVINFO DeviceInfoSet,
                               _In_ PSP_DEVINFO_DATA DeviceInfoData,
                               _In_ DWORD Flags, _Out_opt_ PBOOL NeedReboot);
...
//
#define ROLLBACK_FLAG_NO_UI                                                    \
  0x00000001 // don't show any UI (this could cause failures if UI must be
             // displayed)
#define ROLLBACK_BITS 0x00000001

BOOL WINAPI DiRollbackDriver(_In_ HDEVINFO DeviceInfoSet,
                             _In_ PSP_DEVINFO_DATA DeviceInfoData,
...
BOOL WINAPI DiShowUpdateDriver(__in_opt HWND hwndParent,
                               __in_opt PCWSTR FilePath, __in DWORD Flags,
