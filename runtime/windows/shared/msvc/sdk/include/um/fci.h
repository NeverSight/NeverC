...
typedef enum {
  ...
  FCIERR_CAB_FORMAT_LIMIT // Data-size or file-count exceeded CAB format limits
                          // i.e. Total-bytes (uncompressed) in a CAB-folder
                          // exceeded 0x7FFF8000 (~ 2GB)
...
typedef INT_PTR(FAR DIAMONDAPI *PFNFCIOPEN)(_In_ LPSTR pszFile, int oflag,
                                            int pmode, int FAR *err,
                                            void FAR *pv);
...
typedef UINT(FAR DIAMONDAPI *PFNFCIWRITE)(INT_PTR hf, void FAR *memory, UINT cb,
                                          int FAR *err, void FAR *pv);
typedef int(FAR DIAMONDAPI *PFNFCICLOSE)(INT_PTR hf, int FAR *err,
                                         void FAR *pv);
typedef long(FAR DIAMONDAPI *PFNFCISEEK)(INT_PTR hf, long dist, int seektype,
                                         int FAR *err, void FAR *pv);
typedef int(FAR DIAMONDAPI *PFNFCIDELETE)(_In_ LPSTR pszFile, int FAR *err,
                                          void FAR *pv);

#define FNFCIOPEN(fn)                                                          \
  INT_PTR FAR DIAMONDAPI fn(_In_ LPSTR pszFile, int oflag, int pmode,          \
                            int FAR *err, void FAR *pv)
#define FNFCIREAD(fn)                                                          \
  UINT FAR DIAMONDAPI fn(INT_PTR hf, void FAR *memory, UINT cb, int FAR *err,  \
                         void FAR *pv)
#define FNFCIWRITE(fn)                                                         \
  UINT FAR DIAMONDAPI fn(INT_PTR hf, void FAR *memory, UINT cb, int FAR *err,  \
                         void FAR *pv)
#define FNFCICLOSE(fn)                                                         \
  int FAR DIAMONDAPI fn(INT_PTR hf, int FAR *err, void FAR *pv)
#define FNFCISEEK(fn)                                                          \
  long FAR DIAMONDAPI fn(INT_PTR hf, long dist, int seektype, int FAR *err,    \
                         void FAR *pv)
#define FNFCIDELETE(fn)                                                        \
  int FAR DIAMONDAPI fn(_In_ LPSTR pszFile, int FAR *err, void FAR *pv)
 ...
 *      returns FALSE;
 */
typedef BOOL(DIAMONDAPI *PFNFCIGETNEXTCABINET)(PCCAB pccab, ULONG cbPrevCab,
                                               void FAR *pv); /* pfnfcignc */

#define FNFCIGETNEXTCABINET(fn)                                                \
  BOOL DIAMONDAPI fn(PCCAB pccab, ULONG cbPrevCab, void FAR *pv)
 ...
 */
typedef int(DIAMONDAPI *PFNFCIFILEPLACED)(PCCAB pccab, _In_ LPSTR pszFile,
                                          long cbFile, BOOL fContinuation,
                                          void FAR *pv); /* pfnfcifp */

#define FNFCIFILEPLACED(fn)                                                    \
  int DIAMONDAPI fn(PCCAB pccab, _In_ LPSTR pszFile, long cbFile,              \
                    BOOL fContinuation, void FAR *pv)
...
typedef INT_PTR(DIAMONDAPI *PFNFCIGETOPENINFO)(_In_ LPSTR pszName,
                                               USHORT *pdate, USHORT *ptime,
                                               USHORT *pattribs, int FAR *err,
                                               void FAR *pv); /* pfnfcigoi */

#define FNFCIGETOPENINFO(fn)                                                   \
  INT_PTR DIAMONDAPI fn(_In_ LPSTR pszName, USHORT *pdate, USHORT *ptime,      \
                        USHORT *pattribs, int FAR *err, void FAR *pv)
...
typedef long(DIAMONDAPI *PFNFCISTATUS)(UINT typeStatus, ULONG cb1, ULONG cb2,
                                       void FAR *pv); /* pfnfcis */

#define FNFCISTATUS(fn)                                                        \
  long DIAMONDAPI fn(UINT typeStatus, ULONG cb1, ULONG cb2, void FAR *pv)
...
typedef BOOL(DIAMONDAPI *PFNFCIGETTEMPFILE)(
    _Out_writes_bytes_(cbTempName) char *pszTempName,
    _In_range_(<=, 260) int cbTempName, void FAR *pv); /* pfnfcigtf */

#define FNFCIGETTEMPFILE(fn)                                                   \
  BOOL DIAMONDAPI fn(_Out_writes_bytes_(cbTempName) char *pszTempName,         \
                     _In_range_(<=, 260) int cbTempName, void FAR *pv)
 ...
 */
HFCI DIAMONDAPI
FCICreate(_In_ PERF perf, __callback PFNFCIFILEPLACED pfnfcifp,
          __callback PFNFCIALLOC pfna, __callback PFNFCIFREE pfnf,
          __callback PFNFCIOPEN pfnopen, __callback PFNFCIREAD pfnread,
          __callback PFNFCIWRITE pfnwrite, __callback PFNFCICLOSE pfnclose,
          __callback PFNFCISEEK pfnseek, __callback PFNFCIDELETE pfndelete,
          __callback PFNFCIGETTEMPFILE pfnfcigtf, _In_ PCCAB pccab,
 ...
 */
BOOL DIAMONDAPI FCIAddFile(_In_ HFCI hfci, _In_ LPSTR pszSourceFile,
                           _In_ LPSTR pszFileName, BOOL fExecute,
 ...
 */
BOOL DIAMONDAPI FCIFlushCabinet(_In_ HFCI hfci, BOOL fGetNextCab,
                                __callback PFNFCIGETNEXTCABINET pfnfcignc,
                                __callback PFNFCISTATUS pfnfcis);
