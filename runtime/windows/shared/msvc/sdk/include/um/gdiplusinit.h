...
struct GdiplusStartupInputEx : GdiplusStartupInput {
  enum class Version : UINT32 {
    V2 = 2,
    V3 = 3 // Enables Heif and Avif image codecs.  Unlike other functionalities
           // in Gdiplus, these two codecs require COM to be initiailized.
  };

  INT StartupParameters; // Do we not set the FPU rounding mode

  GdiplusStartupInputEx(INT startupParameters = 0,
  ...
  GdiplusStartupInputEx(Version gdiplusVersion, INT startupParameters = 0,
                        DebugEventProc debugEventCallback = NULL,
                        BOOL suppressBackgroundThread = FALSE,
                        BOOL suppressExternalCodecs = FALSE) {
