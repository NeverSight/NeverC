...
HRESULT WINAPI WSDSetConfigurationOption(DWORD dwOption,
                                         _In_reads_bytes_(cbInBuffer)
                                             LPVOID pVoid,
                                         DWORD cbInBuffer);

HRESULT WINAPI WSDGetConfigurationOption(DWORD dwOption,
                                         _Out_writes_bytes_(cbOutBuffer)
                                             LPVOID pVoid,
                                         DWORD cbOutBuffer);
...
HRESULT WINAPI WSDXMLBuildAnyForSingleElement(WSDXML_NAME *pElementName,
                                              _In_opt_ LPCWSTR pszText,
                                              _Outptr_ WSDXML_ELEMENT **ppAny);

HRESULT WINAPI WSDXMLGetValueFromAny(_In_ LPCWSTR pszNamespace,
                                     _In_ LPCWSTR pszName, WSDXML_ELEMENT *pAny,
                                     _Outptr_ LPCWSTR *ppszValue);

HRESULT WINAPI WSDXMLAddSibling(WSDXML_ELEMENT *pFirst,
...
HRESULT WINAPI WSDGenerateFault(_In_ LPCWSTR pszCode,
...
HRESULT WINAPI WSDGenerateFaultEx(_In_ WSDXML_NAME *pCode,
...
#if (WINVER >= _WIN32_WINNT_WIN7)
HRESULT WINAPI WSDUriEncode(_In_reads_(cchSource) LPCWSTR source,
                            _In_ DWORD cchSource,
                            _Out_writes_(*cchDestOut) LPWSTR *destOut,
                            _Out_opt_ DWORD *cchDestOut);

HRESULT WINAPI WSDUriDecode(_In_reads_(cchSource) LPCWSTR source,
                            _In_ DWORD cchSource,
                            _Out_writes_(*cchDestOut) LPWSTR *destOut,
                            _Out_opt_ DWORD *cchDestOut);
#endif
