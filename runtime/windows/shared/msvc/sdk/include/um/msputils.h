...
//
// return TRUE if the (possibly aggregated) media type that was passed in is
// valid.
//
...
inline BOOL IsValidAggregatedMediaType(DWORD dwAggregatedMediaType) {
  ...
  if ((0 == (dwAggregatedMediaType &
             dwAllPossibleMediaTypes)) || // any valid bits set
      (0 != (dwAggregatedMediaType &
             (~dwAllPossibleMediaTypes)))) // no invalid bits are set
  {
