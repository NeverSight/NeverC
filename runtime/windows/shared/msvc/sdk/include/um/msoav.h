...
typedef struct _msoavinfo {
  ...
  struct {
    ...
    ULONG fHttpDownload
        : 1; // the file at pwzFullPath is a temp file downloaded from http/ftp
  };
...
MsoFDoSecurityLevelDlg(HMSOINST hmsoinst, DWORD msorid, int *pSecurityLevel,
                       BOOL *pfTrustInstalled, HWND hwndParent,
                       BOOL fShowVirusCheckers, WCHAR *wzHelpFile,
                       DWORD dwHelpId);
...
// msodsvPassedTrusted is very unfortunately named because it has nothing to do
// with trust - it just means that the doc is signed and the signature matched.
// Too late to change the name now so I'm adding a msodsvPassedTrustedCert to
// mean the doc was signed and cert was trusted.
