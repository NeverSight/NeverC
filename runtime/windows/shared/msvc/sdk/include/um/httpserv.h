...
IHttpStoredContext {
public:
  virtual VOID CleanupStoredContext(VOID) = 0;
};
...
public:
  virtual IHttpStoredContext *
  GetModuleContext(_In_ HTTP_MODULE_ID moduleId) = 0;

  virtual HRESULT SetModuleContext(_In_ IHttpStoredContext *ppStoredContext,
                                   _In_ HTTP_MODULE_ID moduleId) = 0;
};
...
public:
  virtual IHttpStoredContext *GetNamedContext(_In_ LPCWSTR szName) = 0;

  virtual HRESULT SetNamedContext(_In_ IHttpStoredContext *ppStoredContext,
...
public:
  virtual VOID ReleaseContainer(VOID) = 0;
};
...
public:
  virtual VOID IncrementCounter(DWORD dwCounterIndex, DWORD dwValue = 1) = 0;
...
IHttpApplication {
public:
  virtual PCWSTR GetApplicationPhysicalPath(VOID) const = 0;
...
class __declspec(uuid("311a3b7a-b673-4a1c-8349-2e98a23cadcb")) IHttpApplication2
    : public IHttpApplication {
public:
  virtual VOID BeginApplicationWarmup(VOID) = 0;

  virtual VOID EndApplicationWarmup(VOID) = 0;

  virtual BOOL QueryIsWarmingUp(VOID) = 0;
};
...
public:
  virtual PCWSTR GetPath(VOID) const = 0;

  virtual PCSTR GetAllowedVerbs(VOID) const = 0;

  virtual _Ret_writes_(*pcchModules) PCWSTR
      GetModules(_Out_ DWORD *pcchModules = NULL) const = 0;

  virtual _Ret_writes_(*pcchScriptProcessor) PCWSTR
      GetScriptProcessor(_Out_ DWORD *pcchScriptProcessor = NULL) const = 0;

  virtual _Ret_writes_(*pcchManagedType) PCWSTR
      GetManagedType(_Out_ DWORD *pcchManagedType = NULL) const = 0;
...
public:
  virtual PCWSTR GetMetaPath(VOID) const = 0;
...
public:
  virtual VOID ReferenceMetadata(VOID) = 0;
...
public:
  ...
  virtual _Ret_writes_bytes_(*pcchHeaderValue) PCSTR
      GetHeader(_In_ PCSTR pszHeaderName,
                _Out_ USHORT *pcchHeaderValue = NULL) const = 0;

  virtual _Ret_writes_bytes_(*pcchHeaderValue) PCSTR
      GetHeader(_In_ HTTP_HEADER_ID ulHeaderIndex,
                _Out_ USHORT *pcchHeaderValue = NULL) const = 0;

  virtual HRESULT SetHeader(_In_ PCSTR pszHeaderName,
                            _In_reads_bytes_(cchHeaderValue)
                                PCSTR pszHeaderValue,
                            _In_ USHORT cchHeaderValue, _In_ BOOL fReplace) = 0;

  virtual HRESULT SetHeader(_In_ HTTP_HEADER_ID ulHeaderIndex,
                            _In_ PCSTR pszHeaderValue,
                            _In_ USHORT cchHeaderValue, _In_ BOOL fReplace) = 0;

  virtual HRESULT DeleteHeader(_In_ PCSTR pszHeaderName) = 0;

  virtual HRESULT DeleteHeader(_In_ HTTP_HEADER_ID ulHeaderIndex) = 0;

  virtual PCSTR GetHttpMethod(VOID) const = 0;

  virtual HRESULT SetHttpMethod(_In_ PCSTR pszHttpMethod) = 0;

  virtual HRESULT SetUrl(_In_reads_(cchUrl) PCWSTR pszUrl, _In_ DWORD cchUrl,
                         _In_ BOOL fResetQueryString) = 0;

  virtual HRESULT SetUrl(_In_reads_bytes_(cchUrl) PCSTR pszUrl,
                         _In_ DWORD cchUrl, _In_ BOOL fResetQueryString) = 0;
  ...
  virtual HRESULT ReadEntityBody(_Out_writes_bytes_all_(cbBuffer)
                                     VOID *pvBuffer,
                                 _In_ DWORD cbBuffer, _In_ BOOL fAsync,
                                 _Out_ DWORD *pcbBytesReceived,
                                 _Out_ BOOL *pfCompletionPending = NULL) = 0;

  virtual HRESULT InsertEntityBody(_In_reads_bytes_(cbBuffer) VOID *pvBuffer,
                                   _In_ DWORD cbBuffer) = 0;

  virtual DWORD GetRemainingEntityBytes(VOID) = 0;

  virtual VOID GetHttpVersion(_Out_ USHORT *pMajorVersion,
                              _Out_ USHORT *pMinorVersion) const = 0;

  virtual HRESULT
  GetClientCertificate(_Outptr_ HTTP_SSL_CLIENT_CERT_INFO **ppClientCertInfo,
                       _Out_ BOOL *pfClientCertNegotiated) = 0;

  virtual HRESULT
  NegotiateClientCertificate(_In_ BOOL fAsync,
                             _Out_ BOOL *pfCompletionPending = NULL) = 0;
  ...
  virtual HRESULT GetHeaderChanges(
      ...
      _Out_ DWORD **ppDiffedUnknownHeaderIndices) = 0;
};

class __declspec(uuid("d9244ae1-51f8-4aa1-a66d-19277c33e610")) IHttpRequest2
    : public IHttpRequest {
public:
  virtual HRESULT
  GetChannelBindingToken(_Outptr_result_bytebuffer_to_(*pTokenSize, *pTokenSize)
                             PBYTE *ppToken,
                         _Out_ DWORD *pTokenSize) = 0;
};
...
typedef REQUEST_NOTIFICATION_STATUS(WINAPI *PFN_ASYNC_COMPLETION)(
    IHttpContext3 *pHttpContext, IHttpCompletionInfo2 *pCompletionInfo,
    VOID *pvCompletionContext);

class __declspec(uuid("b32e4e0f-4057-4feb-aeab-7b69c03c6314")) IHttpRequest3
    : public IHttpRequest2 {
public:
  virtual HRESULT
  ReadEntityBody(_Out_writes_bytes_all_(cbBuffer) VOID *pvBuffer,
                 _In_ DWORD cbBuffer, _In_ BOOL fAsync,
                 _In_ PFN_ASYNC_COMPLETION pfnCompletionCallback,
                 _In_ VOID *pvCompletionContext, _Out_ DWORD *pcbBytesReceived,
                 _Out_ BOOL *pfCompletionPending = NULL) = 0;
};

class __declspec(uuid("3816f517-f631-49f0-8b51-16d6f0c6ffb2")) IHttpRequest4
    : public IHttpRequest3 {
public:
  virtual HRESULT ReadEntityBody(_Out_writes_bytes_all_(cbBuffer)
                                     VOID *pvBuffer,
                                 _In_ DWORD cbBuffer, _In_ BOOL fAsync,
...
IHttpCachePolicy {
public:
  virtual HTTP_CACHE_POLICY *GetKernelCachePolicy(VOID) = 0;

  virtual VOID SetKernelCacheInvalidatorSet(VOID) = 0;

  virtual HTTP_CACHE_POLICY *GetUserCachePolicy(VOID) = 0;

  virtual HRESULT AppendVaryByHeader(_In_ PCSTR pszHeader) = 0;

  virtual PCSTR GetVaryByHeaders(VOID) const = 0;

  virtual HRESULT AppendVaryByQueryString(_In_ PCSTR pszParam) = 0;

  virtual PCSTR GetVaryByQueryStrings(VOID) const = 0;

  virtual HRESULT SetVaryByValue(_In_ PCSTR pszValue) = 0;
...
class __declspec(uuid("9f4ba807-050e-4495-ae55-8870f7e9194a")) IHttpCachePolicy2
    : public IHttpCachePolicy {
public:
  virtual BOOL IsForceUpdateSet(VOID) const = 0;
...
public:
  virtual HTTP_RESPONSE *GetRawHttpResponse(VOID) = 0;

  virtual const HTTP_RESPONSE *GetRawHttpResponse(VOID) const = 0;

  virtual IHttpCachePolicy *GetCachePolicy(VOID) = 0;

  virtual HRESULT SetStatus(_In_ USHORT statusCode, _In_ PCSTR pszReason,
                            _In_ USHORT uSubStatus = 0,
                            _In_ HRESULT hrErrorToReport = S_OK,
                            _In_opt_ IAppHostConfigException *pException = NULL,
                            _In_ BOOL fTrySkipCustomErrors = FALSE) = 0;

  virtual HRESULT SetHeader(_In_ PCSTR pszHeaderName, _In_ PCSTR pszHeaderValue,
                            _In_ USHORT cchHeaderValue, _In_ BOOL fReplace) = 0;
  ...
  virtual HRESULT Flush(_In_ BOOL fAsync, _In_ BOOL fMoreData,
                        _Out_ DWORD *pcbSent,
                        _Out_ BOOL *pfCompletionExpected = NULL) = 0;

  virtual HRESULT Redirect(_In_ PCSTR pszUrl, _In_ BOOL fResetStatusCode = TRUE,
                           _In_ BOOL fIncludeParameters = FALSE) = 0;

  virtual HRESULT
  WriteEntityChunkByReference(_In_ HTTP_DATA_CHUNK *pDataChunk,
                              _In_ LONG lInsertPosition = -1) = 0;

  virtual HRESULT
  WriteEntityChunks(_In_reads_(nChunks) HTTP_DATA_CHUNK *pDataChunks,
  ...
  virtual VOID GetStatus(
      _Out_ USHORT *pStatusCode, _Out_ USHORT *pSubStatus = NULL,
      _Outptr_opt_result_bytebuffer_(*pcchReason) PCSTR *ppszReason = NULL,
      _Out_ USHORT *pcchReason = NULL, _Out_ HRESULT *phrErrorToReport = NULL,
      _Outptr_opt_ PCWSTR *ppszModule = NULL,
      _Out_ DWORD *pdwNotification = NULL,
      _Outptr_opt_ IAppHostConfigException **ppException = NULL,
      _Out_ BOOL *pfTrySkipCustomErrors = NULL) = 0;

  virtual HRESULT SetErrorDescription(_In_reads_(cchDescription)
                                          PCWSTR pszDescription,
                                      _In_ DWORD cchDescription,
                                      _In_ BOOL fHtmlEncode = TRUE) = 0;

  virtual _Ret_writes_(*pcchDescription) PCWSTR
      GetErrorDescription(_Out_ DWORD *pcchDescription = NULL) = 0;
...
class __declspec(uuid("0822c871-f14e-4974-a1e6-5b3c1f09b76a")) IHttpResponse2
...
public:
  ...
  WriteEntityChunks(_In_reads_(nChunks) HTTP_DATA_CHUNK *pDataChunks,
                    _In_ DWORD nChunks, _In_ BOOL fAsync, _In_ BOOL fMoreData,
                    _In_ PFN_ASYNC_COMPLETION pfnCompletionCallback,
                    _In_ VOID *pvCompletionContext, _Out_ DWORD *pcbSent,
                    _Out_ BOOL *pfCompletionExpected = NULL) = 0;
...
class __declspec(uuid("19578e49-d080-45da-b673-96cb475d3c5d")) IHttpResponse3
    : public IHttpResponse2 {
public:
  virtual HRESULT PushPromise(_In_ PCSTR pszVerb, _In_ PCWSTR pwszPath,
                              _In_opt_ PCWSTR pwszQuery, _In_ DWORD cHeaders,
                              _In_opt_count_(cHeaders) PCSTR *rgHeaderNames,
                              _In_opt_count_(cHeaders)
                                  PCSTR *rgHeaderValues) = 0;
};
...
class __declspec(uuid("1a2acc57-cae2-4f28-b4ab-00c8f96b12ec")) IHttpResponse4
    : public IHttpResponse3 {
public:
  virtual HRESULT DeleteTrailer(_In_ PCSTR pszHeaderName) = 0;

  virtual PCSTR GetTrailer(_In_ PCSTR pszHeaderName,
                           _Out_ USHORT *pcchHeaderValue = NULL) const = 0;

  virtual VOID ResetStream(_In_ ULONG errorCode) = 0;

  virtual VOID SetNeedGoAway(VOID) = 0;

  virtual HRESULT SetTrailer(_In_ PCSTR pszHeaderName,
                             _In_ PCSTR pszHeaderValue,
                             _In_ USHORT cchHeaderValue,
                             _In_ BOOL fReplace) = 0;
};
...
public:
  ...
  virtual HRESULT IsInRole(_In_ PCWSTR pszRoleName, _Out_ BOOL *pfInRole) = 0;

  virtual PVOID GetUserVariable(_In_ PCSTR pszVariableName) = 0;
};
...
public:
  virtual VOID NotifyDisconnect(VOID) = 0;
};
...
public:
  virtual IHttpConnectionStoredContext *
  GetConnectionModuleContext(_In_ HTTP_MODULE_ID moduleId) = 0;

  virtual HRESULT
  SetConnectionModuleContext(_In_ IHttpConnectionStoredContext *ppStoredContext,
                             _In_ HTTP_MODULE_ID moduleId) = 0;
};
...
public:
  ...
  virtual
      ...
      _Ret_opt_ _Post_writable_byte_size_(cbAllocation) VOID *AllocateMemory(
          DWORD cbAllocation) = 0;

  virtual IHttpConnectionModuleContextContainer *
      GetModuleContextContainer(VOID) = 0;
};
...
class __declspec(uuid(
    "671e6d34-9380-4df4-b453-91129df02b24")) ICustomNotificationProvider;

class __declspec(uuid(
    "6f3f657d-2fb8-43c6-a096-5064b41f0580")) IHttpEventProvider;
...
public:
  virtual IHttpSite *GetSite(VOID) = 0;
  ...
  virtual VOID
  IndicateCompletion(_In_ REQUEST_NOTIFICATION_STATUS notificationStatus) = 0;

  virtual HRESULT PostCompletion(_In_ DWORD cbBytes) = 0;

  virtual VOID DisableNotifications(_In_ DWORD dwNotifications,
                                    _In_ DWORD dwPostNotifications) = 0;

  virtual BOOL GetNextNotification(
      _In_ REQUEST_NOTIFICATION_STATUS status, _Out_ DWORD *pdwNotification,
      _Out_ BOOL *pfIsPostNotification, _Outptr_ CHttpModule **ppModuleInfo,
      _Outptr_ IHttpEventProvider **ppRequestOutput) = 0;

  virtual BOOL
  GetIsLastNotification(_In_ REQUEST_NOTIFICATION_STATUS status) = 0;

  virtual HRESULT ExecuteRequest(_In_ BOOL fAsync,
  ...
  virtual HRESULT GetServerVariable(_In_ PCSTR pszVariableName,
                                    _Outptr_result_buffer_(*pcchValueLength)
                                        PCWSTR *ppszValue,
                                    _Out_ DWORD *pcchValueLength) = 0;
  ...
  virtual HRESULT SetServerVariable(PCSTR pszVariableName,
                                    PCWSTR pszVariableValue) = 0;

  virtual

      _Ret_opt_ _Post_writable_byte_size_(cbAllocation)
          VOID *AllocateRequestMemory(_In_ DWORD cbAllocation) = 0;

  virtual IHttpUrlInfo *GetUrlInfo(VOID) = 0;

  virtual IMetadataInfo *GetMetadata(VOID) = 0;

  virtual _Ret_writes_(*pcchPhysicalPath) PCWSTR
      GetPhysicalPath(_Out_ DWORD *pcchPhysicalPath = NULL) = 0;

  virtual _Ret_writes_(*pcchScriptName) PCWSTR
      GetScriptName(_Out_ DWORD *pcchScriptName = NULL) const = 0;

  virtual _Ret_writes_(*pcchScriptTranslated) PCWSTR
      GetScriptTranslated(_Out_ DWORD *pcchScriptTranslated = NULL) = 0;
  ...
  virtual HRESULT MapPath(_In_ PCWSTR pszUrl,
                          _Inout_updates_(*pcbPhysicalPath)
                              PWSTR pszPhysicalPath,
                          _Inout_ DWORD *pcbPhysicalPath) = 0;

  virtual HRESULT
  NotifyCustomNotification(_In_ ICustomNotificationProvider *pCustomOutput,
                           _Out_ BOOL *pfCompletionExpected) = 0;

  virtual IHttpContext *GetParentContext(VOID) const = 0;

  virtual IHttpContext *GetRootContext(VOID) const = 0;

  virtual HRESULT CloneContext(_In_ DWORD dwCloneFlags,
                               _Outptr_ IHttpContext **ppHttpContext) = 0;
  ...
  virtual HRESULT GetCurrentExecutionStats(
      ...
      _Out_ DWORD *pdwAsyncNotificationStartTickCount = NULL) const = 0;
  ...
  virtual HRESULT GetServerVarChanges(
      ...
      _Out_ DWORD **ppDiffedVariableIndices) = 0;

  virtual HRESULT CancelIo(VOID) = 0;

  virtual HRESULT MapHandler(_In_ DWORD dwSiteId, _In_ PCWSTR pszSiteName,
                             _In_ PCWSTR pszUrl, _In_ PCSTR pszVerb,
                             _Outptr_ IScriptMapInfo **ppScriptMap,
                             _In_ BOOL fIgnoreWildcardMappings = FALSE) = 0;

  __declspec(deprecated(
      "This method is deprecated. Use the HttpGetExtendedInterface helper "
      "function instead.")) virtual HRESULT
  GetExtendedInterface(_In_ HTTP_CONTEXT_INTERFACE_VERSION version,
                       _Outptr_ PVOID *ppInterface) = 0;
};
...
//
class __declspec(uuid("c986182c-cf4a-4482-8205-0dbbc1fd6cee")) IHttpContext2
    : public IHttpContext {
public:
  virtual IHttpUser *GetOriginalUser(VOID) const = 0;
};
...
class __declspec(uuid("bf53c022-ca4b-4349-a38e-a4b78ad897fb")) IHttpContext3
    : public IHttpContext2 {
public:
  virtual INamedContextContainer *GetNamedContextContainer(VOID) = 0;

  virtual VOID EnableFullDuplex(VOID) = 0;

  virtual HRESULT InheritServerVariable(_In_ PCSTR szName,
                                        _In_ BOOL fFailIfNotExist = FALSE) = 0;

  virtual HRESULT
  PostCompletion(DWORD cbBytes,
                 LPOVERLAPPED_COMPLETION_ROUTINE pfnCompletionRoutine,
                 VOID *pvCompletionContext) = 0;
};
...
class __declspec(uuid("2c9463d8-4ec4-4622-8917-d486321a9443")) IHttpContext4
    : public IHttpContext3 {
public:
  //
  // Gets the request start time as returned by GetTickCount64()
  //
  virtual ULONGLONG GetRequestStartTime(VOID) = 0;

  //
  // Gets the elapsed time in milliseconds
  //
  virtual ULONGLONG GetRequestElapsedTime(VOID) = 0;
};
...
public:
  virtual HRESULT GetTraceConfiguration(
      _Inout_ HTTP_TRACE_CONFIGURATION *pHttpTraceConfiguration) = 0;

  virtual HRESULT
  SetTraceConfiguration(_In_ HTTP_MODULE_ID moduleId,
                        _In_ HTTP_TRACE_CONFIGURATION *pHttpTraceConfiguration,
                        _In_ DWORD cHttpTraceConfiguration = 1) = 0;

  virtual HRESULT RaiseTraceEvent(_In_ HTTP_TRACE_EVENT *pTraceEvent) = 0;

  virtual LPCGUID GetTraceActivityId() = 0;

  virtual HRESULT QuickTrace(_In_ PCWSTR pszData1,
                             _In_opt_ PCWSTR pszData2 = NULL,
                             _In_ HRESULT hrLastError = S_OK,
                             //
                             // 4 == TRACE_LEVEL_INFORMATION
                             //
                             _In_ UCHAR Level = 4) = 0;
};

class __declspec(uuid(
    "37776aff-852e-4eec-93a5-b85a285a95b8")) IHttpCacheSpecificData;
...
public:
  virtual DWORD GetHash(VOID) const = 0;

  virtual PCWSTR GetCacheName(VOID) const = 0;

  virtual bool GetIsEqual(_In_ IHttpCacheKey *pCacheCompareKey) const = 0;

  virtual bool GetIsPrefix(_In_ IHttpCacheKey *pCacheCompareKey) const = 0;

  virtual VOID Enum(_In_ IHttpCacheSpecificData *pCacheData) = 0;
};
...
public:
  ...
  virtual VOID DecrementTTL(_Out_ BOOL *pfTTLExpired) = 0;
...
//
class __declspec(uuid("71e95595-8c74-44d9-88a9-f5112d5f5900")) IHttpFileInfo
    : public IHttpCacheSpecificData {
public:
  virtual DWORD GetAttributes(VOID) const = 0;

  virtual VOID GetSize(_Out_ ULARGE_INTEGER *pliSize) const = 0;
  ...
  virtual PCSTR GetETag(_Out_ USHORT *pcchETag = NULL) const = 0;

  virtual VOID GetLastModifiedTime(_Out_ FILETIME *pFileTime) const = 0;

  virtual PCSTR GetLastModifiedString(VOID) const = 0;

  virtual BOOL GetHttpCacheAllowed(_Out_ DWORD *pSecondsToLive) const = 0;

  virtual HRESULT AccessCheck(_In_ HANDLE hUserToken, _In_ PSID pUserSid) = 0;
  ...
  virtual BOOL CheckIfFileHasChanged(_In_ HANDLE hUserToken) = 0;
};
...
public:
  virtual BOOL IsCommandLineLaunch(VOID) const = 0;

  virtual PCWSTR GetAppPoolName(VOID) const = 0;

  virtual HRESULT AssociateWithThreadPool(_In_ HANDLE hHandle,
                                          _In_ LPOVERLAPPED_COMPLETION_ROUTINE
                                              completionRoutine) = 0;
  ...
  virtual VOID ReportUnhealthy(_In_ PCWSTR pszReasonString,
                               _In_ HRESULT hrReason) = 0;

  virtual VOID RecycleProcess(_In_ PCWSTR pszReason) = 0;

  virtual IAppHostAdminManager *GetAdminManager(VOID) const = 0;

  virtual HRESULT
  GetFileInfo(_In_ PCWSTR pszPhysicalPath, _In_ HANDLE hUserToken,
              ...
              _In_opt_ IHttpTraceContext *pHttpTraceContext = NULL) = 0;

  virtual HRESULT FlushKernelCache(_In_ PCWSTR pszUrl) = 0;

  virtual HRESULT
  DoCacheOperation(_In_ CACHE_OPERATION cacheOperation,
                   _In_ IHttpCacheKey *pCacheKey,
                   _Outptr_ IHttpCacheSpecificData **ppCacheSpecificData,
                   _In_opt_ IHttpTraceContext *pHttpTraceContext = NULL) = 0;

  virtual GLOBAL_NOTIFICATION_STATUS
  NotifyCustomNotification(_In_ ICustomNotificationProvider *pCustomOutput) = 0;

  virtual IHttpPerfCounterInfo *GetPerfCounterInfo(VOID) = 0;

  virtual VOID RecycleApplication(_In_ PCWSTR pszAppConfigPath) = 0;

  virtual VOID NotifyConfigurationChange(_In_ PCWSTR pszPath) = 0;

  virtual VOID NotifyFileChange(_In_ PCWSTR pszFileName) = 0;

  virtual IDispensedHttpModuleContextContainer *DispenseContainer(VOID) = 0;

  virtual HRESULT AddFragmentToCache(_In_ HTTP_DATA_CHUNK *pDataChunk,
                                     _In_ PCWSTR pszFragmentName) = 0;

  virtual HRESULT ReadFragmentFromCache(_In_ PCWSTR pszFragmentName,
                                        _Out_writes_bytes_all_(cbSize)
                                            BYTE *pvBuffer,
                                        _In_ DWORD cbSize,
                                        _Out_ DWORD *pcbCopied) = 0;

  virtual HRESULT RemoveFragmentFromCache(_In_ PCWSTR pszFragmentName) = 0;

  virtual HRESULT
  GetWorkerProcessSettings(_Outptr_ IWpfSettings **ppWorkerProcessSettings) = 0;

  virtual HRESULT
  GetProtocolManagerCustomInterface(_In_ PCWSTR pProtocolManagerDll,
                                    _In_ PCWSTR pProtocolManagerDllInitFunction,
                                    _In_ DWORD dwCustomInterfaceId,
                                    _Outptr_ PVOID *ppCustomInterface) = 0;

  virtual BOOL
  SatisfiesPrecondition(_In_ PCWSTR pszPrecondition,
                        _Out_ BOOL *pfUnknownPrecondition = NULL) const = 0;

  virtual IHttpTraceContext *GetTraceContext(VOID) const = 0;

  virtual HRESULT
  RegisterFileChangeMonitor(_In_ PCWSTR pszPath, _In_ HANDLE hToken,
                            _Outptr_ IHttpFileMonitor **ppFileMonitor) = 0;

  virtual HRESULT
  GetExtendedInterface(_In_ HTTP_SERVER_INTERFACE_VERSION version,
                       _Outptr_ PVOID *ppInterface) = 0;
};

class __declspec(uuid("34af637e-afe8-4556-bcc1-767f8e0b4a4e")) IHttpServer2
    : public IHttpServer {
public:
  virtual HRESULT
  GetToken(_In_ PCWSTR pszUserName, _In_ PCWSTR pszPassword,
  ...
  virtual _Ret_writes_(*pcchConfigFilePath) PCWSTR
      GetAppPoolConfigFile(_Out_ DWORD *pcchConfigFilePath = NULL) const = 0;

  virtual HRESULT GetExtendedInterface(_In_ const GUID &Version1,
                                       _In_ PVOID pInput,
                                       _In_ const GUID &Version2,
                                       _Outptr_ PVOID *ppOutput) = 0;

  virtual HRESULT
  GetMetadata(_In_ PCWSTR pszSiteName, _In_ PCWSTR pszUrl,
              _Outptr_ IReferencedMetadataInfo **ppMetadataInfo) = 0;

  virtual HRESULT GetWorkerProcessFramework(
      _Outptr_ IWorkerProcessFramework **ppWorkerProcessFramework) = 0;
};

class __declspec(uuid("f7a66d59-c0a2-48b5-a88f-be5975c960fb")) IHttpServer3
    : public IHttpServer2 {
public:
  virtual HRESULT QueryRequestProperty(
      _In_ HTTP_OPAQUE_ID id, _In_ HTTP_REQUEST_PROPERTY propertyId,
      _In_reads_bytes_opt_(qualifierSize) PVOID pQualifier,
      _In_ ULONG qualifierSize,
      _Out_writes_bytes_to_opt_(outputBufferSize, *pcbBytesReturned)
          PVOID pOutput,
      _In_ ULONG outputBufferSize, _Out_opt_ PULONG pcbBytesReturned,
      _In_ LPOVERLAPPED pOverlapped) = 0;
};
...
//   HRESULT hr = HttpGetExtendedInterface(g_pHttpServer, pHttpRequest,
//   &pHttpRequest2); if( SUCCEEDED(hr) )
//   {
//      // Use pHttpRequest2.
//   }
//
...
HRESULT HttpGetExtendedInterface(_In_ IHttpServer *pHttpServer,
                                 _In_ HttpType1 *pInput,
                                 _Outptr_ HttpType2 **ppOutput) {
  HRESULT hr;
  IHttpServer2 *pHttpServer2;
  hr = pHttpServer->GetExtendedInterface(
      HTTP_SERVER_INTERFACE_V2, reinterpret_cast<void **>(&pHttpServer2));
  if (SUCCEEDED(hr)) {
    hr = pHttpServer2->GetExtendedInterface(
        __uuidof(HttpType1), pInput, __uuidof(HttpType2),
...
IHttpEventProvider {
public:
  virtual VOID SetErrorStatus(_In_ HRESULT hrError) = 0;
};
...
IHttpCompletionInfo {
public:
  virtual DWORD GetCompletionBytes(VOID) const = 0;
...
public:
  virtual DWORD GetCompletedOperation(VOID) const = 0;
};
...
public:
  virtual PCWSTR QueryNotificationType(VOID) = 0;
};
...
public:
  virtual VOID SetUser(_In_ IHttpUser *pUser) = 0;
};
...
public:
  virtual HRESULT SetScriptName(_In_reads_(cchScriptName) PCWSTR pszScriptName,
                                _In_ DWORD cchScriptName) = 0;

  virtual VOID SetScriptMap(_In_ IScriptMapInfo *pScriptMap) = 0;

  virtual VOID SetFileInfo(_In_ IHttpFileInfo *pFileInfo) = 0;
};

//
// RQ_MAP_PATH
//
class __declspec(uuid("8efdf557-a8f1-4bc9-b462-6df3b038a59a")) IMapPathProvider
    : public IHttpEventProvider {
public:
  virtual PCWSTR GetUrl(VOID) const = 0;

  virtual PCWSTR GetPhysicalPath(VOID) const = 0;

  virtual HRESULT SetPhysicalPath(_In_reads_(cchPhysicalPath)
                                      PCWSTR pszPhysicalPath,
                                  _In_ DWORD cchPhysicalPath) = 0;
};
...
public:
  virtual BOOL GetHeadersBeingSent(VOID) const = 0;

  virtual DWORD GetFlags(VOID) const = 0;

  virtual VOID SetFlags(_In_ DWORD dwFlags) = 0;

  virtual HTTP_LOG_DATA *GetLogData(VOID) const = 0;

  virtual HRESULT SetLogData(_In_ HTTP_LOG_DATA *pLogData) = 0;
...
public:
  virtual VOID GetEntity(_Outptr_result_bytebuffer_to_(*pcbBuffer, *pcbData)
                             PVOID *ppBuffer,
                         _Out_ DWORD *pcbData, _Out_ DWORD *pcbBuffer) = 0;

  virtual VOID SetEntity(_In_reads_(cbBuffer) PVOID pBuffer, _In_ DWORD cbData,
                         _In_ DWORD cbBuffer) = 0;
};
...
public:
  virtual IHttpContext *GetHttpContext(VOID) = 0;
};
...
class __declspec(uuid(
    "ba32d330-9ea8-4b9e-89f1-8c76a323277f")) IHttpModuleFactory;
...
public:
  virtual HRESULT RegisterModule(_In_ HTTP_MODULE_ID parentModuleId,
                                 ...
                                 _In_ DWORD dwPostRequestNotifications) = 0;

  virtual HRESULT
  SetPriorityForRequestNotification(_In_ PCWSTR pszModuleName,
                                    _In_ DWORD dwRequestNotification,
                                    _In_ PCWSTR pszPriorityAlias) = 0;
};
...
public:
  virtual PCWSTR GetFunctionName(VOID) const = 0;
  ...
  virtual
      ...
      HRESULT
      GetOutputBuffer(_In_ DWORD cbBuffer,
                      _Outptr_result_bytebuffer_to_(cbBuffer, 0)
                          BYTE **ppbBuffer) = 0;

  virtual HRESULT ResizeOutputBuffer(
      _In_ DWORD cbNewBuffer,
      _In_ _In_range_(<=, cbNewBuffer) DWORD cbBytesToCopy,
      _Inout_ _At_(*ppbBuffer, _Pre_readable_size_(cbBytesToCopy)
                                   _Post_readable_size_(cbBytesToCopy)
                                       _Post_writable_size_(cbNewBuffer))
          BYTE **ppbBuffer) = 0;

  virtual VOID SetResult(_In_ DWORD cbData, _In_ HRESULT hr) = 0;
};
...
public:
  virtual BOOL DrainRequestsGracefully(VOID) const = 0;
};

//
// GL_CACHE_OPERATION
//
class __declspec(uuid("58925fb9-7c5e-4684-833b-4a04e1286690")) ICacheProvider
    : public IHttpEventProvider {
public:
  virtual CACHE_OPERATION GetCacheOperation(VOID) const = 0;

  virtual IHttpCacheKey *GetCacheKey(VOID) const = 0;

  virtual IHttpCacheSpecificData *GetCacheRecord(VOID) const = 0;

  virtual VOID SetCacheRecord(_In_ IHttpCacheSpecificData *pCacheRecord) = 0;
...
public:
  virtual PCWSTR GetChangePath(VOID) const = 0;
};
...
public:
  virtual PCWSTR GetFileName(VOID) const = 0;
...
public:
  virtual HRESULT GetTraceEvent(_Outptr_ HTTP_TRACE_EVENT **ppTraceEvent) = 0;

  virtual BOOL CheckSubscription(_In_ HTTP_MODULE_ID ModuleId) = 0;

  virtual HRESULT
  GetCurrentHttpRequestContext(_Outptr_ IHttpContext **ppHttpContext) = 0;
};
...
public:
  virtual HRESULT CreateContext(_Outptr_ IHttpContext **ppHttpContext) = 0;

  virtual HRESULT ExecuteRequest(_In_ IHttpContext *pHttpContext,
                                 _In_ IHttpUser *pHttpUser) = 0;
};
...
public:
  virtual BOOL IsProcessRecycled(VOID) const = 0;
};
...
public:
  ...
  OnBeginRequest(_In_ IHttpContext *pHttpContext,
    ...
    OutputDebugStringA(
        ...
        "  Please check the method signature to make sure it matches the "
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostBeginRequest(_In_ IHttpContext *pHttpContext,
                     _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnAuthenticateRequest(_In_ IHttpContext *pHttpContext,
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostAuthenticateRequest(_In_ IHttpContext *pHttpContext,
                            _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnAuthorizeRequest(_In_ IHttpContext *pHttpContext,
                     _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostAuthorizeRequest(_In_ IHttpContext *pHttpContext,
                         _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnResolveRequestCache(_In_ IHttpContext *pHttpContext,
                        _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostResolveRequestCache(_In_ IHttpContext *pHttpContext,
                            _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnMapRequestHandler(_In_ IHttpContext *pHttpContext,
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostMapRequestHandler(_In_ IHttpContext *pHttpContext,
                          _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnAcquireRequestState(_In_ IHttpContext *pHttpContext,
                        _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostAcquireRequestState(_In_ IHttpContext *pHttpContext,
                            _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPreExecuteRequestHandler(_In_ IHttpContext *pHttpContext,
                             _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostPreExecuteRequestHandler(_In_ IHttpContext *pHttpContext,
                                 _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnExecuteRequestHandler(_In_ IHttpContext *pHttpContext,
                          _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostExecuteRequestHandler(_In_ IHttpContext *pHttpContext,
                              _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnReleaseRequestState(_In_ IHttpContext *pHttpContext,
                        _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostReleaseRequestState(_In_ IHttpContext *pHttpContext,
                            _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnUpdateRequestCache(_In_ IHttpContext *pHttpContext,
                       _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostUpdateRequestCache(_In_ IHttpContext *pHttpContext,
                           _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnLogRequest(_In_ IHttpContext *pHttpContext,
               _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostLogRequest(_In_ IHttpContext *pHttpContext,
                   _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnEndRequest(_In_ IHttpContext *pHttpContext,
               _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnPostEndRequest(_In_ IHttpContext *pHttpContext,
                   _In_ IHttpEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnSendResponse(_In_ IHttpContext *pHttpContext,
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnMapPath(_In_ IHttpContext *pHttpContext, _In_ IMapPathProvider *pProvider) {
    UNREFERENCED_PARAMETER(pHttpContext);
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnReadEntity(_In_ IHttpContext *pHttpContext,
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnCustomRequestNotification(_In_ IHttpContext *pHttpContext,
  ...
  virtual REQUEST_NOTIFICATION_STATUS
  OnAsyncCompletion(_In_ IHttpContext *pHttpContext, _In_ DWORD dwNotification,
...
public:
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalStopListening(_In_ IGlobalStopListeningProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalCacheOperation(_In_ ICacheProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalFileChange(_In_ IGlobalFileChangeProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalPreBeginRequest(_In_ IPreBeginRequestProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalApplicationStart(_In_ IHttpApplicationStartProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalApplicationStop(_In_ IHttpApplicationStopProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalRSCAQuery(_In_ IGlobalRSCAQueryProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalTraceEvent(_In_ IGlobalTraceEventProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalCustomNotification(_In_ ICustomNotificationProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  virtual GLOBAL_NOTIFICATION_STATUS
  OnGlobalThreadCleanup(_In_ IGlobalThreadCleanupProvider *pProvider) {
    UNREFERENCED_PARAMETER(pProvider);
    OutputDebugStringA("This module subscribed to event ");
    OutputDebugStringA(__FUNCTION__);
    OutputDebugStringA(
  ...
  // GL_SUSPEND_PROCESS
  virtual GLOBAL_NOTIFICATION_STATUS
  OnSuspendProcess(_Outptr_ IGlobalSuspendProcessCallback **pCallback) {
...
public:
  virtual
      ...
      _Ret_opt_ _Post_writable_byte_size_(cbAllocation) VOID *AllocateMemory(
          _In_ DWORD cbAllocation) = 0;
};

class __declspec(uuid("ba32d330-9ea8-4b9e-89f1-8c76a323277f"))
IHttpModuleFactory {
public:
  virtual HRESULT GetHttpModule(_Outptr_ CHttpModule **ppModule,
                                _In_ IModuleAllocator *pAllocator) = 0;
...
public:
  ...
  virtual HRESULT
  SetRequestNotifications(_In_ IHttpModuleFactory *pModuleFactory,
                          _In_ DWORD dwRequestNotifications,
                          _In_ DWORD dwPostRequestNotifications) = 0;

  virtual HRESULT SetGlobalNotifications(_In_ CGlobalModule *pGlobalModule,
                                         _In_ DWORD dwGlobalNotifications) = 0;

  virtual HRESULT
  SetPriorityForRequestNotification(_In_ DWORD dwRequestNotification,
                                    _In_ PCWSTR pszPriority) = 0;

  virtual HRESULT
  SetPriorityForGlobalNotification(_In_ DWORD dwGlobalNotification,
                                   _In_ PCWSTR pszPriority) = 0;
};
...
public:
  virtual HRESULT
  SetPriorityForPostRequestNotification(_In_ DWORD dwPostRequestNotification,
                                        _In_ PCWSTR pszPriority) = 0;
};
//
...
typedef HRESULT(WINAPI *PFN_REGISTERMODULE)(
    DWORD dwServerVersion, IHttpModuleRegistrationInfo *pModuleInfo,
