...
#define ADMXCOMMENTS_EXTENSION_GUID                                            \
...
DECLARE_INTERFACE_(IGPEInformation, IUnknown) {
  ...
  STDMETHOD(GetName)(THIS_ _Out_writes_(cchMaxLength) LPOLESTR pszName,
  ...
  STDMETHOD(GetDisplayName)(THIS_ _Out_writes_(cchMaxLength) LPOLESTR pszName,
                            int cchMaxLength) PURE;
  ...
  STDMETHOD(GetDSPath)(THIS_ DWORD dwSection,
                       _Out_writes_(cchMaxPath) LPOLESTR pszPath,
                       int cchMaxPath) PURE;
  ...
  STDMETHOD(GetFileSysPath)(THIS_ DWORD dwSection,
                            _Out_writes_(cchMaxPath) LPOLESTR pszPath,
                            int cchMaxPath) PURE;
  ...
  //
  // The three types are:  a GPO in the Active Directory, the GPO on the local
  // machine, and the GPO on a remote machine.
  //
  // Machine GPOs only have file system storage (no Active Directory storage
  // available). If GetDSPath is called for a machine GPO, the function will
  // succeed and the returned buffer will be the empty string ""
  //
  // Active Directory GPOs have both file system and Active Directory storage
  // available to them.
  //
  ...
  //
  // Returns the type of Active Directory object (or machine) that could be
  // linked to this GPO
  //
  // This is a hint api only.  The GPE does not know which Active Directory
  // objects are linked to a particular GPO, but it can offer a hint based upon
  // how the user started the GPE.
  //
  ...
  STDMETHOD(PolicyChanged)(THIS_ BOOL bMachine, BOOL bAdd,
...
#define GPO_OPTION_DISABLE_USER                                                \
  0x00000001 // The user portion of this GPO is disabled
#define GPO_OPTION_DISABLE_MACHINE                                             \
  0x00000002 // The machine portion of this GPO is disabled
...
DECLARE_INTERFACE_(IGroupPolicyObject, IUnknown) {
  ...
  STDMETHOD(New)(THIS_ _In_ LPOLESTR pszDomainName,
                 _In_opt_ LPOLESTR pszDisplayName, DWORD dwFlags) PURE;
  ...
  STDMETHOD(OpenRemoteMachineGPO)(THIS_ _In_ LPOLESTR pszComputerName,
                                  DWORD dwFlags) PURE;
  ...
  STDMETHOD(Save)(THIS_ BOOL bMachine, BOOL bAdd, GUID * pGuidExtension,
                  GUID * pGuid) PURE;
  ...
  STDMETHOD(GetPath)(THIS_ _Out_writes_(cchMaxLength) LPOLESTR pszPath,
                     int cchMaxLength) PURE;
  ...
  STDMETHOD(GetMachineName)(THIS_ _Out_writes_(cchMaxLength) LPOLESTR pszName,
                            int cchMaxLength) PURE;
...
#define RSOP_INFO_FLAG_DIAGNOSTIC_MODE                                         \
  0x00000001 // Running in diagnostic mode vs planning mode
...
DECLARE_INTERFACE_(IRSOPInformation, IUnknown) {
  ...
  STDMETHOD(GetNamespace)(THIS_ DWORD dwSection,
  ...
  STDMETHOD(GetEventLogEntryText)(THIS_ _In_ LPOLESTR pszEventSource,
                                  _In_ LPOLESTR pszEventLogName,
                                  _In_ LPOLESTR pszEventTime, DWORD dwEventID,
                                  _Outptr_ LPOLESTR * ppszText) PURE;
};
...
// CreateGPOLink
//
// Creates a link to a GPO for the specified Site, Domain, or Organizational
// Unit
//
...
HRESULT
WINAPI
CreateGPOLink(_In_ LPOLESTR lpGPO, _In_ LPOLESTR lpContainer,
              BOOL fHighPriority);
...
// DeleteGPOLink
//
// Deletes a link to a GPO for the specified Site, Domain, or Organizational
// Unit
//
// lpGPO         - ADSI path to the GPO
// lpContainer   - ADSI path to the Site, Domain, or Organizational Unit
...
HRESULT
WINAPI
DeleteGPOLink(_In_ LPOLESTR lpGPO, _In_ LPOLESTR lpContainer);
...
HRESULT
WINAPI
DeleteAllGPOLinks(_In_ LPOLESTR lpContainer);
...
#define GPO_BROWSE_DISABLENEW                                                  \
  0x00000001 // Disables the New GPO functionality on all pages except "All"
...
#define GPO_BROWSE_INITTOALL                                                   \
  0x00000010 // Initialize the dialog focused on the All pane
#define GPO_BROWSE_NOUSERGPOS 0x00000020 // Removes the Users tab
#define GPO_BROWSE_SENDAPPLYONEDIT                                             \
  0x00000040 // Send the apply event if edit menu is selected
...
typedef struct tag_GPOBROWSEINFO {
  ...
  LPOLESTR lpTitle; // [in] Title bar text.  If NULL, "Browse for a Group Policy
                    // Object" will be the default text
  LPOLESTR
  lpInitialOU; // [in] Initial Domain/Organizational Unit to open focus on
  _Field_size_(
      dwDSPathSize) LPOLESTR lpDSPath; // [in/out] Pointer to the buffer that
                                       // receives the Active Directory GPO path
  DWORD dwDSPathSize; // [in] Size in characters of buffer given in lpDSPath
  _Field_size_bytes_(dwNameSize)
      LPOLESTR lpName; // [in/out] Pointer to a buffer that receives either the
                       // computer name or
  ...
  GROUP_POLICY_HINT_TYPE
  gpoHint; // [out] Specifies a hint of the GPO association
...
HRESULT
WINAPI
ImportRSoPData(_In_ LPOLESTR lpNameSpace, _In_ LPOLESTR lpFileName);
...
HRESULT
WINAPI
ExportRSoPData(_In_ LPOLESTR lpNameSpace, _In_ LPOLESTR lpFileName);
