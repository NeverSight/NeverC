...
#define TIME_CALLBACK_EVENT_PULSE                                              \
  0x0020 /* callback is event - use PulseEvent */
#endif

#if WINVER >= 0x0501
#define TIME_KILL_SYNCHRONOUS                                                  \
  0x0100 /* This flag prevents the event from occurring */
...
MMRESULT
WINAPI
timeSetEvent(_In_ UINT uDelay, _In_ UINT uResolution, _In_ LPTIMECALLBACK fptc,
             _In_ DWORD_PTR dwUser, _In_ UINT fuEvent);

WINMMAPI
MMRESULT
WINAPI
timeKillEvent(_In_ UINT uTimerID);
