...
#define SP_IS_BAD_OPTIONAL_WRITE_PTR(p)                                        \
  ((p) && SPIsBadWritePtr(p, sizeof(*(p))))
...
#define SP_IS_BAD_READ_ARRAY(p, cElements)                                     \
  (SPIsBadReadArray(p, cElements, sizeof(*p)))
#define SP_IS_BAD_WRITE_ARRAY(p, cElements)                                    \
  (SPIsBadWriteArray(p, cElements, sizeof(*p)))

#define SP_DOES_PTR_ADDITION_WRAP(p) (SPDoesPtrAdditionWrap(p, sizeof(*p)))
#define SP_IS_PTR_OUT_OF_BOUNDS(p, pBound)                                     \
  (SPIsPtrOutOfBounds(p, sizeof(*p), pBound))
...
inline BOOL SPIsBadReadPtr(_In_reads_bytes_(Size) const void *pMem,
                           size_t Size) {
...
inline BOOL SPIsBadVARIANTPtr(const VARIANT *pVar) { return (pVar == NULL); }

inline BOOL SPDoesPtrAdditionWrap(_In_reads_bytes_(Size) const void *pMem,
                                  size_t Size) {
  return ((const BYTE *)pMem + Size) < (const BYTE *)pMem;
}

inline BOOL SPIsBadReadArray(const void *pMem, ULONG cElementCount,
                             size_t cBytesPerElement) {
...
inline BOOL SPIsBadWriteArray(void *pMem, ULONG cElementCount,
                              size_t cBytesPerElement) {
  size_t cByteCount = cBytesPerElement * cElementCount;
...
inline BOOL SPIsPtrOutOfBounds(_In_reads_bytes_(n_cBytes) const void *n_pStart,
                               size_t n_cBytes, const void *n_pBound) {
  ...
  if ((n_pStart <= pEnd) && (pEnd <= n_pBound)) {
    return FALSE;
  } else {
    return TRUE;
  }
...
inline BOOL SPIsArrayOutOfBounds(const void *n_pStart, ULONG n_cElements,
...
template <class _C>
inline HRESULT SpSafePtrAdd(_C **n_ppBase, size_t n_cBytes,
                            const void *n_pBound = NULL) {
  HRESULT hr = S_OK;
  ...
  // Check for wrapping
  // Check for a bound having been specified, and make sure it doesn't exceed
  // it.
  if ((pByteAdded < pByteCast) || (n_pBound != NULL && pByteAdded > n_pBound)) {
    hr = E_INVALIDARG;
  } else {
...
template <class _C>
inline HRESULT SpSafePtrMultiplyAdd(_C **n_ppBase, ULONG n_cElements,
                                    size_t n_cBytesPerElement,
                                    const void *n_pBound = NULL) {
  HRESULT hr = S_OK;
...
//
//  Helper functions can be used to implement GetObjectToken/SetObjectToken for
//  objects that support ISpObjectWithToken
//
inline HRESULT SpGenericSetObjectToken(ISpObjectToken *pCallersToken,
...
inline HRESULT SpGenericGetObjectToken(ISpObjectToken **ppCallersToken,
                                       CComPtr<ISpObjectToken> &cpObjToken) {
  HRESULT hr = S_OK;
...
//
//  Helper class for SPSTATEINFO sturcture automatically initializes and cleans
//  up the structure + provides a few helper functions.
//
...
public:
  ...
  ~CSpStateInfo() { ::CoTaskMemFree(pTransitions); }
  SPTRANSITIONENTRY *FirstEpsilon() { return pTransitions; }
  SPTRANSITIONENTRY *FirstRule() { return pTransitions + cEpsilons; }
  SPTRANSITIONENTRY *FirstWord() { return pTransitions + cEpsilons + cRules; }
...
//
//  This basic queue implementation can be used to maintin linked lists of
//  classes.  The class T must contain the member m_pNext which is used by this
//  template to point to the next element. If the bPurgeWhenDeleted is TRUE then
//  all of the elements in the queue will be deleted when the queue is deleted,
//  otherwise they will not. If bMaintainCount is TRUE then a running count will
//  be maintained, and GetCount() will be efficent.  If it is FALSE then a
//  running count will not be maintained, and GetCount() will be an order N
//  operation.  If you do not require a count, then
//
...
public:
  T *m_pHead;
  T *m_pTail;
  ULONG m_cElements; // Warning!  Use GetCount() -- Not maintained if
                     // bMaintainCount is FALSE.
  ...
  T *GetNext(const T *pNode) { return pNode->m_pNext; }
  ...
  //
  //  The following functions require the class T to implement a static
  //  function:
  //
  //      LONG Compare(const T * pElem1, const T * pElem2)
  //
  //  which returns < 0 if pElem1 is less than pElem2, 0 if they are equal, and
  ...
  void InsertSorted(T *pNode) {
    if (m_pHead) {
      if (T::Compare(pNode, m_pTail) >= 0) {
        ...
        //
        //  We don't have to worry about walking off of the end of the list here
        //  since we have already checked the tail.
        //
        T **ppNext = &m_pHead;
  ...
  //
  template <class TFIND> T *Find(TFIND &FindVal) const {
    T *pNode = m_pHead;
  ...
  template <class TFIND> T *FindNext(const T *pCurNode, TFIND &FindVal) const {
  ...
  //
  template <class TFIND> T *FindAndRemove(TFIND &FindVal) {
    T *pNode = m_pHead;
    if (pNode) {
      if (*pNode == FindVal) {
        m_pHead = pNode->m_pNext;
  ...
  //
  template <class TFIND> void FindAndDeleteAll(TFIND &FindVal) {
    T *pNode = m_pHead;
...
template <class T, BOOL bPurgeWhenDeleted = TRUE> class CSpBasicList {
public:
  T *m_pFirst;
...
#define STACK_ALLOC_AND_ZERO(TYPE, COUNT)                                      \
  (TYPE *)memset(_alloca(sizeof(TYPE) * (COUNT)), 0, (sizeof(TYPE) * (COUNT)))
#define STACK_ALLOC_AND_COPY(TYPE, COUNT, SOURCE)                              \
  (TYPE *)memcpy(_alloca(sizeof(TYPE) * (COUNT)), (SOURCE),                    \
                 (sizeof(TYPE) * (COUNT)))

inline HRESULT SpGetSubTokenFromToken(ISpObjectToken *pToken,
  ...
  if (SP_IS_BAD_INTERFACE_PTR(pToken) || SP_IS_BAD_STRING_PTR(pszSubKeyName) ||
  ...
  if (SUCCEEDED(hr)) {
    hr = cpTokenInit->InitFromDataKey(dstrCategoryId, dstrTokenId,
                                      cpDataKeyForNewToken);
  }
...
template <class T>
HRESULT SpCreateObjectFromSubToken(ISpObjectToken *pToken,
                                   const WCHAR *pszSubKeyName, T **ppObject,
...
// Return a token enumerator containing all tokens that match the primary
// language of a particular language id. pszRequiredAttributes can be used to
// specify additional attributes all tokens must have.
inline HRESULT SpEnumTokensMatchingPrimaryLangID(const LPCWSTR pszCategoryId,
                                                 LANGID priLangID,
                                                 LPCWSTR pszRequiredAtts,
  ...
  for (ULONG ul = 0; SUCCEEDED(hr) && ul < ulTokens; ul++) {
    ...
    if (SUCCEEDED(hr) &&
        PRIMARYLANGID(tokenLangID) == PRIMARYLANGID(priLangID)) {
      // Add to builder
