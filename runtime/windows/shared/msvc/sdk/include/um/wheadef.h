...
typedef NTSTATUS(_WHEA_ERROR_SOURCE_INITIALIZE_DEVICE_DRIVER)(
    _Inout_opt_ PVOID Context, _In_ ULONG ErrorSourceId);

typedef _WHEA_ERROR_SOURCE_INITIALIZE_DEVICE_DRIVER
    *WHEA_ERROR_SOURCE_INITIALIZE_DEVICE_DRIVER;

typedef VOID(_WHEA_ERROR_SOURCE_UNINITIALIZE_DEVICE_DRIVER)(
    _Inout_opt_ PVOID Context);

typedef _WHEA_ERROR_SOURCE_UNINITIALIZE_DEVICE_DRIVER
    *WHEA_ERROR_SOURCE_UNINITIALIZE_DEVICE_DRIVER;

typedef NTSTATUS(_WHEA_ERROR_SOURCE_CORRECT_DEVICE_DRIVER)(
    _Inout_ PVOID ErrorSourceDesc, _Out_ PULONG MaximumSectionLength);
...
typedef struct _WHEA_DRIVER_BUFFER_SET {
  ULONG Version;
  _Field_size_full_(DataSize) PUCHAR Data;
  ULONG DataSize;
  LPGUID SectionTypeGuid;
  _Field_size_full_(20) PUCHAR SectionFriendlyName;
  PUCHAR Flags;
...
__inline BOOLEAN WheaIsGhesAssistSrc(_In_ PWHEA_ERROR_SOURCE_DESCRIPTOR ErrSrc)
...
#define IOCTL_IPMI_INTERNAL_RECORD_SEL_EVENT                                   \
  CTL_CODE(FILE_DEVICE_UNKNOWN, IPMI_IOCTL_INDEX + 0, METHOD_BUFFERED,         \
           FILE_ANY_ACCESS)
