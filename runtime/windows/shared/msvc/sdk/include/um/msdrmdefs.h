...
typedef struct _DRMID {
  ...
  _DRMID() : uVersion(DRMIDVERSION), wszIDType(NULL), wszID(NULL) {}

  _DRMID(_In_opt_ PWSTR wszTypein, _In_opt_ PWSTR wszIDin)
      : uVersion(DRMIDVERSION), wszIDType(wszTypein), wszID(wszIDin) {}
...
typedef struct _DRMBOUNDLICENSEPARAMS {
  ...
  _DRMBOUNDLICENSEPARAMS()
      : uVersion(DRMBOUNDLICENSEPARAMSVERSION), hEnablingPrincipal(NULL),
        hSecureStore(NULL), wszRightsRequested(NULL), wszRightsGroup(NULL),
        cAuthenticatorCount(0), rghAuthenticators(NULL),
        wszDefaultEnablingPrincipalCredentials(NULL), idResource(NULL, NULL),
        dwFlags(0) {}
...
typedef struct _DRM_LICENSE_ACQ_DATA {
  ...
  _DRM_LICENSE_ACQ_DATA()
      : uVersion(DRMLICENSEACQDATAVERSION), wszURL(NULL),
        wszLocalFilename(NULL), pbPostData(NULL), dwPostDataSize(NULL),
...
#ifdef __cplusplus
  _DRM_ACTSERV_INFO()
      : uVersion(DRMACTSERVINFOVERSION), wszPubKey(NULL), wszURL(NULL) {}
#endif // #ifdef __cplusplus
} DRM_ACTSERV_INFO;
...
#define DRM_ACTIVATE_SHARED_GROUPIDENTITY                                      \
  0x20                            // Shared Group Identity certificate
...
typedef HRESULT(__stdcall *DRMCALLBACK)(DRM_STATUS_MSG, HRESULT, void *,
                                        void *);
