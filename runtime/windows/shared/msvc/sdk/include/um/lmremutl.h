...
_Check_return_ _Success_(return == 0) NET_API_STATUS NET_API_FUNCTION
    NetRemoteTOD(_In_opt_ LPCWSTR UncServerName,
                 _Outptr_result_bytebuffer_(sizeof(TIME_OF_DAY_INFO))
                     LPBYTE *BufferPtr);
...
__cdecl RxRemoteApi(IN DWORD ApiNumber,
                    ...
                    _In_opt_ IN LPDESC AuxDescSmb OPTIONAL, IN DWORD Flags,
...
#define ALLOCATE_RESPONSE                                                      \
  0x00000002 // set if RxRemoteApi allocates response buffer
