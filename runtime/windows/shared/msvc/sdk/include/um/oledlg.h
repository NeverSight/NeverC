...
STDAPI_(BOOL)
OleUIAddVerbMenuW(_In_opt_ LPOLEOBJECT lpOleObj, _In_opt_ LPCWSTR lpszShortType,
                  HMENU hMenu, UINT uPos, UINT uIDVerbMin, UINT uIDVerbMax,
                  BOOL bAddConvert, UINT idConvert, _Out_ HMENU *lphMenu);
STDAPI_(BOOL)
OleUIAddVerbMenuA(_In_opt_ LPOLEOBJECT lpOleObj, _In_opt_ LPCSTR lpszShortType,
                  HMENU hMenu, UINT uPos, UINT uIDVerbMin, UINT uIDVerbMax,
                  BOOL bAddConvert, UINT idConvert, _Out_ HMENU *lphMenu);
#ifdef UNICODE
...
typedef struct tagOLEUIPASTESPECIALW {
  ...
  LPDATAOBJECT lpSrcDataObj; // IN-OUT: Source IDataObject* on the clipboard
                             // If lpSrcDataObj is NULL when OleUIPasteSpecial
                             // is called, then OleUIPasteSpecial will attempt
                             // to retrieve a pointer to an IDataObject from the
                             // clipboard.  If OleUIPasteSpecial succeeds then
                             // it is the caller's responsibility to free the
                             // IDataObject returned in lpSrcDataObj.
...
DECLARE_INTERFACE_(IOleUILinkContainerW, IUnknown) {
  // *** IUnknown methods *** //
  STDMETHOD(QueryInterface)(THIS_ _In_ REFIID riid,
                            _Outptr_ LPVOID FAR * ppvObj) PURE;
  ...
  STDMETHOD(SetLinkSource)(THIS_ DWORD dwLink, _In_ LPWSTR lpszDisplayName,
                           ULONG lenFileName, _Out_ ULONG FAR * pchEaten,
                           BOOL fValidateSource) PURE;
  STDMETHOD(GetLinkSource)(
      THIS_ DWORD dwLink,
      _Outptr_opt_result_maybenull_ LPWSTR FAR * lplpszDisplayName,
      _Out_ ULONG FAR * lplenFileName,
      _Outptr_opt_result_maybenull_ LPWSTR FAR * lplpszFullLinkType,
      _Outptr_opt_result_maybenull_ LPWSTR FAR * lplpszShortLinkType,
...
DECLARE_INTERFACE_(IOleUILinkContainerA, IUnknown) {
  ...
  STDMETHOD(GetLinkSource)(
      THIS_ DWORD dwLink,
      _Outptr_opt_result_maybenull_ LPSTR FAR * lplpszDisplayName,
      _Out_ ULONG FAR * lplenFileName,
      _Outptr_opt_result_maybenull_ LPSTR FAR * lplpszFullLinkType,
      _Outptr_opt_result_maybenull_ LPSTR FAR * lplpszShortLinkType,
      _Out_ BOOL FAR * lpfSourceAvailable, _Out_ BOOL FAR * lpfIsSelected) PURE;
  STDMETHOD(OpenLinkSource)(THIS_ DWORD dwLink) PURE;
  STDMETHOD(UpdateLink)(THIS_ DWORD dwLink, BOOL fErrorMessage, BOOL fReserved)
      PURE;
...
// the given clsid.
STDAPI_(BOOL)
OleUICanConvertOrActivateAs(_In_ REFCLSID rClsid, BOOL fIsLinkedObject,
                            WORD wFormat);
...
typedef struct tagOLEUICHANGESOURCEW {
  ...
  lpOleUILinkContainer;   // IN: used to validate link sources
...
#define CSF_EXPLORER                                                           \
  0x00000008L // IN: use new OFN_EXPLORER custom template behavior
...
DECLARE_INTERFACE_(IOleUIObjInfoW, IUnknown) {
  ...
  STDMETHOD(GetObjectInfo)(
      THIS_ DWORD dwObject, _Out_ DWORD FAR * lpdwObjSize,
  ...
  STDMETHOD(GetConvertInfo)(
      THIS_ DWORD dwObject, _Out_ CLSID FAR * lpClassID,
      _Out_ WORD FAR * lpwFormat, _Out_ CLSID FAR * lpConvertDefaultClassID,
  ...
  STDMETHOD(GetViewInfo)(
      THIS_ DWORD dwObject, _In_opt_ HGLOBAL FAR * phMetaPict,
...
DECLARE_INTERFACE_(IOleUIObjInfoA, IUnknown) {
  ...
  STDMETHOD(GetObjectInfo)(
      THIS_ DWORD dwObject, _Out_ DWORD FAR * lpdwObjSize,
      _Outptr_opt_result_maybenull_ LPSTR FAR * lplpszLabel,
      _Outptr_opt_result_maybenull_ LPSTR FAR * lplpszType,
      _Outptr_opt_result_maybenull_ LPSTR FAR * lplpszShortType,
