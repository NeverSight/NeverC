...
typedef D3DNTHAL_VALIDATETEXTURESTAGESTATEDATA FAR
...
typedef struct _D3DNTHAL_DP2CLEAR {
  // dwFlags can contain D3DCLEAR_TARGET, D3DCLEAR_ZBUFFER, and/or
  // D3DCLEAR_STENCIL
  DWORD dwFlags;        // in:  surfaces to clear
  DWORD dwFillColor;    // in:  Color value for rtarget
...
typedef struct _D3DNTHAL_DRAWPRIMITIVES2DATA {
  ...
  LPDWORD
  lpdwRStates; // in: Pointer to the array where render states are updated
  union {
...
// Indicates that the command buffer and vertex buffer are a system memory
// execute buffer resulting from the use of the Execute buffer API.
#define D3DNTHALDP2_EXECUTEBUFFER 0x00000002L

// The swap flags indicate if it is OK for the driver to swap the submitted
// buffers with new buffers and asyncronously work on the submitted buffers.
#define D3DNTHALDP2_SWAPVERTEXBUFFER 0x00000004L
#define D3DNTHALDP2_SWAPCOMMANDBUFFER 0x00000008L
// The requested flags are present if the new buffers which the driver can
// allocate need to be of atleast a given size. If any of these flags are set,
// the corresponding dwReq* field in D3DNTHAL_DRAWPRIMITIVES2DATA will also be
// set with the requested size in bytes.
#define D3DNTHALDP2_REQVERTEXBUFSIZE 0x00000010L
#define D3DNTHALDP2_REQCOMMANDBUFSIZE 0x00000020L
// These flags are set by the driver upon return from DrawPrimitives2 indicating
// if the new buffers are not in system memory.
...
typedef DWORD(APIENTRY *LPD3DNTHAL_VALIDATETEXTURESTAGESTATECB)(
    LPD3DNTHAL_VALIDATETEXTURESTAGESTATEDATA);
typedef DWORD(APIENTRY *LPD3DNTHAL_DRAWPRIMITIVES2CB)(
...
typedef HRESULT(CALLBACK *PFND3DNTPARSEUNKNOWNCOMMAND)(
    LPVOID lpvCommands, LPVOID *lplpvReturnedCommand);
...
#define D3DRS_MAXVERTEXSHADERINST                                              \
  196 // DDI only: vs_3_0+ num instructions to execute.
#endif

#ifndef D3DRS_MAXPIXELSHADERINST
#define D3DRS_MAXPIXELSHADERINST                                               \
  197 // DDI only: ps_3_0+ num instructions to execute.
#endif
...
#define D3DRENDERSTATE_EVICTMANAGEDTEXTURES                                    \
  61 // DDI render state only to Evict textures
...
typedef struct _D3DNTHAL_DP2SETVERTEXSHADERCONST {
  ...
  DWORD
  dwCount; // Number of 4-float vectors to copy for
           // D3DDP2OP_SETVERTEXSHADERCONST Number of 4-integer vectors to
           // copy for D3DDP2OP_SETVERTEXSHADERCONSTI Number of BOOL values
           // to copy for D3DDP2OP_SETVERTEXSHADERCONSTB Data follows
...
typedef struct _D3DNTHAL_DP2SETPIXELSHADERCONST {
  DWORD dwRegister; // Const register to start copying
  DWORD dwCount;    // Number of 4-float vectors to copy for
                 // D3DDP2OP_SETPIXELSHADERCONST Number of 4-integer vectors to
                 // copy for D3DDP2OP_SETPIXELSHADERCONSTI Number of BOOL values
                 // to copy for D3DDP2OP_SETPIXELSHADERCONSTB
  // Data follows
...
typedef D3DNTHAL_DP2CREATEVERTEXSHADERDECL
    *LPD3DNTHAL_DP2CREATEVERTEXSHADERDECL;
...
typedef D3DNTHAL_DP2CREATEVERTEXSHADERFUNC
    *LPD3DNTHAL_DP2CREATEVERTEXSHADERFUNC;
...
typedef struct _D3DNTHAL_DP2SETCONVOLUTIONKERNELMONO {
  ...
  DWORD dwFlags;
  // If dwFlags & D3DNTHAL_ROW_WEIGHTS, then width floats follow. Otherwise row
  // weights are 1.0. If dwFlags & D3DNTHAL_COL_WEIGHTS, then height floats
  // follow. Otherwise column weights are 1.0.
} D3DNTHAL_DP2SETCONVOLUTIONKERNELMONO;
typedef D3DNTHAL_DP2SETCONVOLUTIONKERNELMONO
    *LPD3DNTHAL_DP2SETCONVOLUTIONKERNELMONO;
...
#define DDBLT_EXTENDED_PRESENTATION_STRETCHFACTOR                              \
...
typedef struct _D3DNTHAL_DP2RESPONSE {
  ...
  DWORD dwTotalSize; // total size of response (including the DP2REPONSE struct)
...
#define _NT_D3DSI_GETREGTYPE(token)                                            \
  ((D3DSHADER_PARAM_REGISTER_TYPE)(token & D3DSP_REGTYPE_MASK))
...
#define _NT_D3DSI_GETREGTYPE(token)                                            \
  ((D3DSHADER_PARAM_REGISTER_TYPE)(((token & D3DSP_REGTYPE_MASK) >>            \
                                    D3DSP_REGTYPE_SHIFT) |                     \
                                   ((token & D3DSP_REGTYPE_MASK2) >>           \
                                    D3DSP_REGTYPE_SHIFT2)))
#define _NT_D3DSI_GETUSAGE(token)                                              \
  ((token & D3DSP_DCL_USAGE_MASK) >> D3DSP_DCL_USAGE_SHIFT)
#define _NT_D3DSI_GETUSAGEINDEX(token)                                         \
  ((token & D3DSP_DCL_USAGEINDEX_MASK) >> D3DSP_DCL_USAGEINDEX_SHIFT)
#define _NT_D3DSI_GETINSTLENGTH(token)                                         \
  ((token & D3DSI_INSTLENGTH_MASK) >> D3DSI_INSTLENGTH_SHIFT)
#define _NT_D3DSI_GETCOMPARISON(token)                                         \
  ((D3DSHADER_COMPARISON)((token & D3DSHADER_COMPARISON_MASK) >>               \
                          D3DSHADER_COMPARISON_SHIFT))
#define _NT_D3DSI_GETREGISTERPROPERTIES(token)                                 \
  (token & D3DSP_REGISTERPROPERTIES_MASK)
...
#define _NT_D3DSI_GETSWIZZLECOMP(source, component)                            \
  (source >> ((component << 1) + 16) & 0x3)
...
#ifdef __cplusplus
// This gets regtype, and also maps D3DSPR_CONSTn to D3DSPR_CONST (for easier
// parsing)
inline D3DSHADER_PARAM_REGISTER_TYPE
_NT_D3DSI_GETREGTYPE_RESOLVING_CONSTANTS(DWORD token) {
...
inline UINT _NT_D3DSI_GETREGNUM_RESOLVING_CONSTANTS(DWORD token) {
  D3DSHADER_PARAM_REGISTER_TYPE RegType = _NT_D3DSI_GETREGTYPE(token);
...
//                     size_t copySize = min(sizeof(myD3DCaps8),
//                     pgdi2->dwExpectedSize); memcpy(lpData->lpvData,
//                     &myD3DCaps8, copySize); lpData->dwActualSize = copySize;
//                     lpData->ddRVal       = DD_OK;
...
#define _NT_D3DGDI2_TYPE_GETFORMATCOUNT                                        \
...
#define _NT_D3DGDI2_TYPE_DXVERSION                                             \
  (0x00000004ul) // Notify driver of current DX Version
#define _NT_D3DGDI2_TYPE_DEFERRED_AGP_AWARE                                    \
  (0x00000018ul) // Runtime is aware of deferred AGP frees, and will send
                 // following
#define _NT_D3DGDI2_TYPE_FREE_DEFERRED_AGP                                     \
  (0x00000019ul) // Free any deferred-freed AGP allocations for this process
#define _NT_D3DGDI2_TYPE_DEFER_AGP_FREES                                       \
  (0x00000020ul) // Start Deferring AGP frees for this process
#if (DIRECT3D_VERSION >= 0x0900)
#define _NT_D3DGDI2_TYPE_GETD3DCAPS9 (0x00000010ul) // Return the D3DCAPS9 data
#define _NT_D3DGDI2_TYPE_GETEXTENDEDMODECOUNT                                  \
  (0x00000011ul) // Return the number of supported extended mode
#define _NT_D3DGDI2_TYPE_GETEXTENDEDMODE                                       \
  (0x00000012ul) // Return a particular extended mode
#define _NT_D3DGDI2_TYPE_GETADAPTERGROUP                                       \
  (0x00000013ul) // Return a adapter group information

#define _NT_D3DGDI2_TYPE_GETMULTISAMPLEQUALITYLEVELS                           \
  (0x00000016ul) // Return the number of multisample quality levels
#define _NT_D3DGDI2_TYPE_GETD3DQUERYCOUNT                                      \
...
// Informs driver that runtime will send a notification after last outstanding
// AGP lock has been released.
typedef struct _DDNT_DEFERRED_AGP_AWARE_DATA {
  DDNT_GETDRIVERINFO2DATA gdi2; // [in/out] GetDriverInfo2 data
} DDNT_DEFERRED_AGP_AWARE_DATA;

// Notification that the last AGP lock has been released. Driver can free all
// deferred AGP allocations for this process.
...
//
// This data structure is returned by the driver in response to a
// GetDriverInfo2 query with the type D3DGDI2_TYPE_GETEXTENDEDMODECOUNT. It
// simply gives the number of extended video modes supported by the driver.
// Currently this structure consists of a single member giving the number of
...
//
// This data structure is used to request a specific extended video mode from
// the driver. It is guaranteed that the requested format will be greater than
// or equal to zero and less that the format count reported by the driver from
...
//
// This data structure is used to request a adapter group information from the
// driver. A adapter group is a set of adapters which share video hardware (like
// video memory, 3D accelerator). Thus it is mainly for DualView video adapter.
// Direct3D runtime will share surface resources (like texture, vertex buffers)
// across adapters within a adapter group upon application's request.
//
typedef struct _DDNT_GETADAPTERGROUPDATA {
  DDNT_GETDRIVERINFO2DATA gdi2;     // [in/out] GetDriverInfo2 data
  ULONG_PTR ulUniqueAdapterGroupId; // [out] The unique id of adapter group that
                                    // this adapter belonging to
  DWORD dwReserved1;                // Reserved, must be 0
  DWORD dwReserved2;                // Reserved, must be 0
} DDNT_GETADAPTERGROUPDATA;

// This data structure used to request the supported quality levels for a given
// sample count, presentation type, and pixel format.
...
// GetDriverState IDs - D3DDEVINFO structures used for query mechanism in public
// headers

// This was eliminated in DX9 but was exposed in DX8.1 so the drivers still need
// it
#define D3DFMT_W11V11U10 (D3DFORMAT)65
#endif // (DIRECT3D_VERSION >= 0x0900 )

// New Caps that are not API visible that the driver exposes.
#define _NT_D3DDEVCAPS_HWVERTEXBUFFER                                          \
  0x02000000L // Device supports Driver Allocated Vertex Buffers
#define _NT_D3DDEVCAPS_HWINDEXBUFFER                                           \
  0x04000000L // Device supports Driver Allocated Index Buffers
#define _NT_D3DDEVCAPS_SUBVOLUMELOCK                                           \
  0x08000000L // Device supports locking a part of volume texture
#define _NT_D3DPMISCCAPS_FOGINFVF                                              \
  0x00002000L // Device supports separate fog value in the FVF

// New FVF flags that are not API visible but accessed by the driver
// Note, that D3DFVF_RESERVED2 includes this flag and should not be used for
// validation

#define _NT_D3DFVF_FOG                                                         \
  0x00002000L // There is a separate fog value in the FVF vertex

// Flags that drivers need to expose for DX8 but were removed from the DX9
// headers
#if (DIRECT3D_VERSION >= 0x0900)
...
// The number of INTEGER constants for software is limited only by the binary
// opcode specification: ;internal
...
// The number of BOOL constants for software is limited only by the binary
// opcode specification: ;internal
...
// The number of FLOAT constants for software is limited only by the binary
// opcode specification: ;internal
