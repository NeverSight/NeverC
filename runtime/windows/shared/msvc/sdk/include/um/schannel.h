...
typedef struct _SecPkgContext_SupportedSignatures {
  ...
  _Field_size_(cSignatureAndHashAlgorithms) WORD *pSignatureAndHashAlgorithms;
...
typedef struct _SecPkgContext_KeyingMaterialInfo {
  ...
  LPSTR pszLabel; // Disambiguating ASCII label, NUL terminator will be removed
                  // by schannel.
  WORD cbContextValue; // Application context value length, in bytes, can be 0.
  PBYTE
  pbContextValue; // Application context value, NULL if cbContextValue == 0.
  DWORD cbKeyingMaterial; // Requested keying material length, in bytes, greater
                          // than 0.
...
typedef struct _SecPkgContext_KeyingMaterial {
  ...
  _Field_size_bytes_(cbKeyingMaterial) PBYTE
      pbKeyingMaterial; // Exported keying material.
...
typedef struct _SecPkgContext_SrtpParameters {
  WORD ProtectionProfile; // Negotiated SRTP protection profile (0x0000 means no
                          // profile negotiated).
  BYTE MasterKeyIdentifierSize; // Size in bytes of the SRTP master key
                                // identifier.
  _Field_size_bytes_(MasterKeyIdentifierSize) PBYTE
      MasterKeyIdentifier; // Negotiated SRTP master key identifier.
...
typedef struct _SecPkgContext_TokenBinding {
  ...
  _Field_size_bytes_(KeyParametersSize) PBYTE
      KeyParameters; // IDs of the negotiated Token Binding key parameters.
} SecPkgContext_TokenBinding, *PSecPkgContext_TokenBinding;

typedef struct _SecPkgContext_CertificateValidationResult {
  DWORD dwChainErrorStatus;    // Contains chain build error flags set by
                               // CertGetCertificateChain.
  HRESULT hrVerifyChainStatus; // Certificate validation policy error returned
                               // by CertVerifyCertificateChainPolicy.
} SecPkgContext_CertificateValidationResult,
    *PSecPkgContext_CertificateValidationResult;
...
typedef enum _eTlsAlgorithmUsage {
  TlsParametersCngAlgUsageKeyExchange, // Key exchange algorithm. RSA, ECHDE,
                                       // DHE, etc.
  TlsParametersCngAlgUsageSignature,   // Signature algorithm. RSA, DSA, ECDSA,
                                       // etc.
  TlsParametersCngAlgUsageCipher, // Encryption algorithm. AES, DES, RC4, etc.
  TlsParametersCngAlgUsageDigest, // Digest of cipher suite. SHA1, SHA256,
                                  // SHA384, etc.
  TlsParametersCngAlgUsageCertSig // Signature, hash and/or padding mode
                                  // components of a TLS signature suite. RSA,
                                  // DSA, ECDSA, SHA1, SHA256, PSS, etc.
} eTlsAlgorithmUsage;
...
typedef struct _CRYPTO_SETTINGS {
  ...
  DWORD cChainingModes; // Set to 0 if CNG algorithm does not have a chaining
                        // mode.
  PUNICODE_STRING rgstrChainingModes; // Set to NULL if CNG algorithm does not
                                      // have a chaining mode.
  DWORD dwMinBitLength; // Blacklist key sizes less than this. Set to 0 if not
                        // defined or CNG algorithm implies bit length.
  DWORD dwMaxBitLength; // Blacklist key sizes greater than this. Set to 0 if
                        // not defined or CNG algorithm implies bit length.
} CRYPTO_SETTINGS, *PCRYPTO_SETTINGS;

typedef struct _TLS_PARAMETERS {
  DWORD cAlpnIds; // Valid for server applications only. Must be zero otherwise.
                  // Number of ALPN IDs in rgstrAlpnIds; set to 0 if applies to
                  // all.
  PUNICODE_STRING
  rgstrAlpnIds; // Valid for server applications only. Must be NULL
                // otherwise. Array of ALPN IDs that the following settings
                // apply to; set to NULL if applies to all.
  DWORD grbitDisabledProtocols; // List protocols you DO NOT want negotiated.
  DWORD cDisabledCrypto; // Number of CRYPTO_SETTINGS structures; set to 0 if
                         // there are none.
  PCRYPTO_SETTINGS pDisabledCrypto; // Array of CRYPTO_SETTINGS structures; set
                                    // to NULL if there are none;
  DWORD dwFlags; // Optional flags to pass; set to 0 if there are none.
} TLS_PARAMETERS, *PTLS_PARAMETERS;

#define TLS_PARAMS_OPTIONAL                                                    \
  0x00000001 // Valid for server applications only. Must be zero otherwise.
             // TLS_PARAMETERS that will only be honored if they do not cause
             // this server to terminate the handshake.
...
typedef struct _SUBSCRIBE_GENERIC_TLS_EXTENSION {
  ...
  Subscriptions[ANYSIZE_ARRAY]; // Array of TLS_EXTENSION_SUBSCRIPTION
                                // structures.
...
typedef BOOL(WINAPI *SSL_EMPTY_CACHE_FN_A)(LPSTR pszTargetName, DWORD dwFlags);

BOOL WINAPI SslEmptyCacheA(_In_ LPSTR pszTargetName, _In_ DWORD dwFlags);

typedef BOOL(WINAPI *SSL_EMPTY_CACHE_FN_W)(LPWSTR pszTargetName, DWORD dwFlags);

BOOL WINAPI SslEmptyCacheW(_In_ LPWSTR pszTargetName, _In_ DWORD dwFlags);
...
typedef struct _SCH_CRED {
  ...
  struct _HMAPPER *
      *aphMappers; // pointer to an array of pointers to credential mappers
} SCH_CRED, *PSCH_CRED;
...
BOOL WINAPI SslGenerateKeyPair(PSSL_CREDENTIAL_CERTIFICATE pCerts,
                               _In_ PSTR pszDN, _In_ PSTR pszPassword,
                               DWORD Bits);
...
// Pre NT4.0 SP2 calls.  Call CAPI1 or CAPI2
// to get the same functionality instead.
BOOL WINAPI SslCrackCertificate(PUCHAR pbCertificate, DWORD cbCertificate,
...
// Pre NT4.0 SP2 calls.  Call CAPI1 or CAPI2
// to get the same functionality instead.
typedef BOOL(WINAPI *SSL_CRACK_CERTIFICATE_FN)(PUCHAR pbCertificate,
                                               DWORD cbCertificate,
...
typedef SECURITY_STATUS(WINAPI *SslGetServerIdentityFn)(
    ...
    _Out_ PDWORD ServerIdentitySize, _In_ DWORD Flags);

EXTERN_C
SECURITY_STATUS
WINAPI
SslGetServerIdentity(_In_reads_bytes_(ClientHelloSize) PBYTE ClientHello,
                     _In_ DWORD ClientHelloSize,
                     _Outptr_result_bytebuffer_(*ServerIdentitySize)
...
typedef enum _SchGetExtensionsOptions {
  ...
  SCH_NO_RECORD_HEADER = 0x1 // Specifies that the ClientHello message does not
                             // contain the record header.
...
typedef SECURITY_STATUS(WINAPI *SslGetExtensionsFn)(
    ...
    _Inout_updates_(genericExtensionsCount)
        SCH_EXTENSION_DATA *genericExtensions,
    _In_ BYTE genericExtensionsCount, _Out_ DWORD *bytesToRead,
    _In_ SchGetExtensionsOptions flags);

EXTERN_C
SECURITY_STATUS
WINAPI
SslGetExtensions(_In_reads_(clientHelloByteSize) const BYTE *clientHello,
                 _In_ DWORD clientHelloByteSize,
                 _Inout_updates_(genericExtensionsCount)
                     SCH_EXTENSION_DATA *genericExtensions,
                 _In_ BYTE genericExtensionsCount, _Out_ DWORD *bytesToRead,
...
// SECPKG_ATTR_SERIALIZED_REMOTE_CERT_CONTEXT[_INPROC]. The certificate context
// of the remote peer's leaf certificate is returned. The caller must free
// *ppCertContext when done by calling CertFreeCertificateContext.
EXTERN_C
SECURITY_STATUS
WINAPI
