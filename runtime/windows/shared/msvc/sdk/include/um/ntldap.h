...
#define SERVER_SEARCH_FLAG_PHANTOM_ROOT                                        \
  0x2 // search all NCs subordinate
      // to search base
...
#define LDAP_CAP_ACTIVE_DIRECTORY_PARTIAL_SECRETS_OID_W                        \
  L"1.2.840.113556.1.4.1920"
...
// Without this flag, the caller must have the replicate changes right. With
// this flag, the caller needs no rights, but is only allowed to see objects and
// attributes which are accessible to them.
...
// Without this flag, all the values (up to a limit) in a multi-valued attribute
// are returned when any value changes. With this flag, only the changed values
// are returned.
