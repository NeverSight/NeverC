...
typedef enum IORING_CREATE_ADVISORY_FLAGS {
  ...
  // Requests the IORING implementation to skip parameter checks in the builder
  // APIs. Ordinarily the builder APIs perform checks to catch programming
  // errors as early as possible. This flag is used to disable that if the
  // implementation understands it (as an advisory flag it has no effect on an
  // implementation that doesn't understand it so it is safe to use on all
  // versions). Normally, this is used in RELEASE builds to eliminate the
  // redundant checks. Errors from invalid parameters are still checked in the
  // kernel and any errors appear in the completion queue entries for the
  // operation.
...
STDAPI
CreateIoRing(IORING_VERSION ioringVersion, IORING_CREATE_FLAGS flags,
             UINT32 submissionQueueSize, UINT32 completionQueueSize,
             _Out_ HIORING *h);

STDAPI GetIoRingInfo(_In_ HIORING ioRing, _Out_ IORING_INFO *info);

STDAPI SubmitIoRing(_In_ HIORING ioRing, UINT32 waitOperations,
                    UINT32 milliseconds, _Out_opt_ UINT32 *submittedEntries);
...
STDAPI BuildIoRingCancelRequest(_In_ HIORING ioRing,
                                _In_ IORING_HANDLE_REF file,
                                UINT_PTR opToCancel, UINT_PTR userData);

// Builds a submission queue entry for IORING_OP_READ
STDAPI
BuildIoRingReadFile(_In_ HIORING ioRing, IORING_HANDLE_REF fileRef,
                    IORING_BUFFER_REF dataRef, UINT32 numberOfBytesToRead,
                    UINT64 fileOffset, UINT_PTR userData,
                    IORING_SQE_FLAGS sqeFlags);

// Builds a submission queue entry for IORING_OP_REGISTER_FILES
STDAPI
BuildIoRingRegisterFileHandles(_In_ HIORING ioRing, UINT32 count,
                               _In_reads_(count) HANDLE const handles[],
                               UINT_PTR userData);

// Builds a submission queue entry for IORING_OP_REGISTER_BUFFERS
STDAPI
BuildIoRingRegisterBuffers(_In_ HIORING ioRing, UINT32 count,
                           _In_reads_(count) IORING_BUFFER_INFO const buffers[],
                           UINT_PTR userData);

#pragma region api-ms-win-core-ioring-l1-1-1
STDAPI
BuildIoRingWriteFile(_In_ HIORING ioRing, IORING_HANDLE_REF fileRef,
                     IORING_BUFFER_REF bufferRef, UINT32 numberOfBytesToWrite,
                     UINT64 fileOffset, FILE_WRITE_FLAGS writeFlags,
                     UINT_PTR userData, IORING_SQE_FLAGS sqeFlags);

STDAPI
BuildIoRingFlushFile(_In_ HIORING ioRing, IORING_HANDLE_REF fileRef,
...
STDAPI
BuildIoRingReadFileScatter(_In_ HIORING ioRing, IORING_HANDLE_REF fileRef,
                           UINT32 segmentCount,
                           _In_reads_(segmentCount)
                               FILE_SEGMENT_ELEMENT segmentArray[],
                           UINT32 numberOfBytesToRead, UINT64 fileOffset,
                           UINT_PTR userData, IORING_SQE_FLAGS sqeFlags);

STDAPI
BuildIoRingWriteFileGather(_In_ HIORING ioRing, IORING_HANDLE_REF fileRef,
                           UINT32 segmentCount,
                           _In_reads_(segmentCount)
                               FILE_SEGMENT_ELEMENT segmentArray[],
