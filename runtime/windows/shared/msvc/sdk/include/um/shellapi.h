...
SHSTDAPI_(LPWSTR *)
CommandLineToArgvW(_In_ LPCWSTR lpCmdLine, _Out_ int *pNumArgs);
...
_Success_(return != 0) SHSTDAPI_(UINT)
    DragQueryFileA(_In_ HDROP hDrop, _In_ UINT iFile,
                   _Out_writes_opt_(cch) LPSTR lpszFile, _In_ UINT cch);
_Success_(return != 0) SHSTDAPI_(UINT)
    DragQueryFileW(_In_ HDROP hDrop, _In_ UINT iFile,
                   _Out_writes_opt_(cch) LPWSTR lpszFile, _In_ UINT cch);
#ifdef UNICODE
...
SHSTDAPI_(HINSTANCE)
ShellExecuteA(_In_opt_ HWND hwnd, _In_opt_ LPCSTR lpOperation,
              _In_ LPCSTR lpFile, _In_opt_ LPCSTR lpParameters,
              _In_opt_ LPCSTR lpDirectory, _In_ INT nShowCmd);
SHSTDAPI_(HINSTANCE)
ShellExecuteW(_In_opt_ HWND hwnd, _In_opt_ LPCWSTR lpOperation,
              _In_ LPCWSTR lpFile, _In_opt_ LPCWSTR lpParameters,
...
_Success_(return > 32) // SE_ERR_DLLNOTFOUND
    SHSTDAPI_(HINSTANCE)
        FindExecutableA(_In_ LPCSTR lpFile, _In_opt_ LPCSTR lpDirectory,
                        _Out_writes_(MAX_PATH) LPSTR lpResult);
_Success_(return > 32) // SE_ERR_DLLNOTFOUND
    SHSTDAPI_(HINSTANCE)
        FindExecutableW(_In_ LPCWSTR lpFile, _In_opt_ LPCWSTR lpDirectory,
...
SHSTDAPI_(INT)
ShellAboutA(_In_opt_ HWND hWnd, _In_ LPCSTR szApp, _In_opt_ LPCSTR szOtherStuff,
            _In_opt_ HICON hIcon);
SHSTDAPI_(INT)
ShellAboutW(_In_opt_ HWND hWnd, _In_ LPCWSTR szApp,
            _In_opt_ LPCWSTR szOtherStuff, _In_opt_ HICON hIcon);
#ifdef UNICODE
...
SHSTDAPI_(HICON)
ExtractAssociatedIconA(_Reserved_ HINSTANCE hInst,
                       _Inout_updates_(128) LPSTR pszIconPath,
                       _Inout_ WORD *piIcon);
SHSTDAPI_(HICON)
ExtractAssociatedIconW(_Reserved_ HINSTANCE hInst,
...
#endif // !UNICODE
SHSTDAPI_(HICON)
ExtractAssociatedIconExA(_Reserved_ HINSTANCE hInst,
                         _Inout_updates_(128) LPSTR pszIconPath,
                         _Inout_ WORD *piIconIndex, _Inout_ WORD *piIconId);
SHSTDAPI_(HICON)
ExtractAssociatedIconExW(_Reserved_ HINSTANCE hInst,
                         _Inout_updates_(128) LPWSTR pszIconPath,
                         _Inout_ WORD *piIconIndex, _Inout_ WORD *piIconId);
#ifdef UNICODE
...
#endif // !UNICODE
SHSTDAPI_(HICON)
ExtractIconA(_Reserved_ HINSTANCE hInst, _In_ LPCSTR pszExeFileName,
             UINT nIconIndex);
SHSTDAPI_(HICON)
ExtractIconW(_Reserved_ HINSTANCE hInst, _In_ LPCWSTR pszExeFileName,
             UINT nIconIndex);
#ifdef UNICODE
...
SHSTDAPI_(UINT_PTR)
SHAppBarMessage(_In_ DWORD dwMessage, _Inout_ PAPPBARDATA pData);
...
SHSTDAPI_(DWORD)
DoEnvironmentSubstA(_Inout_updates_(cchSrc) LPSTR pszSrc, UINT cchSrc);
SHSTDAPI_(DWORD)
DoEnvironmentSubstW(_Inout_updates_(cchSrc) LPWSTR pszSrc, UINT cchSrc);
#ifdef UNICODE
...
SHSTDAPI_(UINT)
ExtractIconExA(_In_ LPCSTR lpszFile, int nIconIndex,
               _Out_writes_opt_(nIcons) HICON *phiconLarge,
               _Out_writes_opt_(nIcons) HICON *phiconSmall, UINT nIcons);
SHSTDAPI_(UINT)
ExtractIconExW(_In_ LPCWSTR lpszFile, int nIconIndex,
               _Out_writes_opt_(nIcons) HICON *phiconLarge,
               _Out_writes_opt_(nIcons) HICON *phiconSmall, UINT nIcons);
#ifdef UNICODE
...
#define FOF_SILENT                                                             \
  0x0004 // don't display progress UI (confirm prompts may be displayed still)
#define FOF_RENAMEONCOLLISION                                                  \
  0x0008 // automatically rename the source files to avoid the collisions
#define FOF_NOCONFIRMATION                                                     \
  0x0010 // don't display confirmation UI, assume "yes" for cases that can be
         // bypassed, "no" for those that can not
#define FOF_WANTMAPPINGHANDLE                                                  \
  0x0020 // Fill in SHFILEOPSTRUCT.hNameMappings
         // Must be freed using SHFreeNameMappings
#define FOF_ALLOWUNDO                                                          \
  0x0040 // enable undo including Recycle behavior for IFileOperation::Delete()
#define FOF_FILESONLY                                                          \
  0x0080 // only operate on the files (non folders), both files and folders are
         // assumed without this
#define FOF_SIMPLEPROGRESS 0x0100 // means don't show names of files
#define FOF_NOCONFIRMMKDIR                                                     \
  0x0200 // don't dispplay confirmatino UI before making any needed directories,
         // assume "Yes" in these cases
#define FOF_NOERRORUI                                                          \
  0x0400 // don't put up error UI, other UI may be displayed, progress,
         // confirmations
#define FOF_NOCOPYSECURITYATTRIBS                                              \
  0x0800 // dont copy file security attributes (ACLs)
#define FOF_NORECURSION                                                        \
  0x1000 // don't recurse into directories for operations that would recurse
#define FOF_NO_CONNECTED_ELEMENTS                                              \
  0x2000 // don't operate on connected elements ("xxx_files" folders that go
         // with .htm files)
#define FOF_WANTNUKEWARNING                                                    \
  0x4000 // during delete operation, warn if object is being permanently
         // destroyed instead of recycling (partially overrides
         // FOF_NOCONFIRMATION)
#if (NTDDI_VERSION >= NTDDI_WINXP)
#define FOF_NORECURSEREPARSE                                                   \
  0x8000 // deprecated; the operations engine always does the right thing on
         // FolderLink objects (symlinks, reparse points, folder shortcuts)
#endif   // (NTDDI_VERSION >= NTDDI_WINXP)
#define FOF_NO_UI                                                              \
  (FOF_SILENT | FOF_NOCONFIRMATION | FOF_NOERRORUI |                           \
   FOF_NOCONFIRMMKDIR) // don't display any UI at all
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
// When SEE_MASK_FLAG_HINST_IS_SITE is specified SHELLEXECUTEINFO.hInstApp is
// used as an _In_ parameter and specifies a IUnknown* to be used as a site
// pointer. The site pointer is used to provide services to shell execute, the
// handler binding process and the verb handlers once they are invoked.
...
SHSTDAPI SHEvaluateSystemCommandTemplate(_In_ PCWSTR pszCmdTemplate,
                                         _Outptr_ PWSTR *ppszApplication,
                                         _Outptr_opt_ PWSTR *ppszCommandLine,
                                         _Outptr_opt_ PWSTR *ppszParameters);
//
//  SHEvaluateSystemCommandTemplate()
//      *   enforces stricter validation before calling CreateProcess().  may
//      also be
//              used before calling ShellExecute().
//      *   should be used when caller wants the deterministic behavior from a
//      command template
//              regardless of execution context.  it ignores the current process
//              state, such as the %PATH%, GetCurrentDirectory(), and parent
//              process directory.
...
//      *   is not designed for processing user input, and may generate
//      unexpected failures.
//
//  INPUT:
//      pszCmdTemplate =    command line, this may or may not include
//      parameters.
//                          if the parameters are substitution parameters then
//                          this API should be called before parameters have
//                          been replaced. (check the examples below to see
//                          sample supported inputs.)
//
//  OUTPUT on return: S_OK
//      ppszApplication =   verified path to the Application.  this should be
//      passed as the lpApplication
//                          parameter to CreateProcess() or the lpFile parameter
//                          to ShellExecute(). (allocated using
//                          CoTaskMemAlloc(), free with CoTaskMemFree())
//
//      ppszCommandLine =   OPTIONAL - use if planning to call CreateProcess().
//                          resulting command line template.  parameters should
//                          be replaced based on this template, and then passed
//                          as the lpCommandLine parameter to CreateProcess().
//                          it is guaranteed to be of a form that PathGetArgs()
//                          will always succeed correctly. (allocated using
//                          CoTaskMemAlloc(), free with CoTaskMemFree())
//
//      ppszParameters  =   OPTIONAL - use if planning to call ShellExecute().
//                          resulting parameter list template.  parameters
//                          should be replaced based on this template, and then
//                          passed as the lpParameters parameter to
//                          ShellExecute(). NOTE: identical to
//                          PathGetArgs(*ppszCommandLine). (allocated using
//                          CoTaskMemAlloc(), free with CoTaskMemFree())
//
//  OUTPUT on return: FAILED()
//      all outputs will be NULL'ed on failure
//
//  NOTES:  the parsing logic to determine a valid Application path is
//  non-trivial, although
//              the extension is not required and if missing will be completed
//              in the following standard order:  { .PIF, .COM, .EXE, .BAT, .CMD
//              }
//
//      Relative Paths are System Paths - if the first token has no path
//      qualifiers
//              then the token is first checked to see if a key of the same name
//              has been installed under
//              HKLM\Software\Microsoft\Windows\CurrentVersion\App Paths. if the
//              key or default value does not exist, it is assumed to be a child
//              of the system directories.  the following directories will be
//              searched in order for the relative token: { CSIDL_SYSTEM,
//              CSIDL_WINDOWS }
//
//      Prefer Quoted Paths - if the first token in pszCmdTemplate is quoted and
//      appears
//              to be an absolute path then the token is the only possible
//              result.
//
//      Limit Forms of Unquoted Paths - if the first token is unquoted and
//      appears
//              to be an absolute path, then it is subject to more stringent
//              limitations. if the token is a substring of CSIDL_PROGRAM_FILES
//              or does not exist on the file system, then
//              SHEvaluateSystemCommandTemplate() will attempt to complete using
//              a token delimited by the first space of the last valid path
//              segment (usually the file name).  if this token also doesnt
//              exist, then the next space will be used, etc.
//
//  USAGE:      used before calling into CreateProcess() or ShellExecute(),
//  callers
...
//  EXAMPLE:   Each example will show an input parameter and the results
//  returned by
//              SHEvaluateSystemCommandTemplate().  Also included is the
//              alternate result of what CreateProcess() would have created if
//              pszCmdTemplate were passed directly as lpCommandLine and
//              lpApplication were NULL. (results marked with an asterisk (*)
...
//          PATH                    =
//          c:\windows\system32;C:\windows;c:\;C:\Program Files\Compilers\
//
...
//      pszCmdTemplate      =   "C:\Program Files\Internet
//      Explorer\iexplore.exe" -nohome
//          SHEvaluateSystemCommandTemplate() returns: S_OK
//              pszApplication  =   C:\Program Files\Internet
//              Explorer\iexplore.exe pszCommandLine  =   "C:\Program
...
//              pszApplication  =   C:\Program Files\Internet
//              Explorer\iexplore.exe pszCommandLine  =   "C:\Program
//              Files\Internet Explorer\iexplore.exe" -url fool.htm
//          * CreateProcess() would return TRUE
//              new process =   C:\Program.exe
//
//  Unquoted Example #3
...
//              pszApplication  =   C:\Program Files\Internet
//              Explorer\iexplore.exe pszCommandLine  =   "C:\Program
//              Files\Internet Explorer\iexplore.exe" -url C:\fool.htm
//          * CreateProcess() would return TRUE
//              new process =   C:\Program.exe
//
//  Unquoted Example #4
...
//          SHEvaluateSystemCommandTemplate() returns: S_OK
//              pszApplication  =   C:\Other Programs\prog.exe
//              pszCommandLine  =   "C:\Other Programs\prog.exe" -go "\fool"
//              "%1"
//          * CreateProcess() would return TRUE
//              new process =   C:\Other Programs\prog.exe
//
...
//  SHCreateDefaultContextMenu() and others use IQueryAssociations to build up
//  data sets

typedef enum ASSOCCLASS {   //  which other members are used
  ...
  ASSOCCLASS_APP_STR,       //  pszClass
                            //  (HKCR\Applications\PathFindFileName(pszClass))
  ASSOCCLASS_SYSTEM_STR,    //  pszClass
  ASSOCCLASS_FOLDER,        //  none
  ASSOCCLASS_STAR,          //  none
#if (NTDDI_VERSION >= NTDDI_WIN8)
  ASSOCCLASS_FIXED_PROGID_STR, //  pszClass (HKCR\pszClass), do not apply
                               //  mapping of pszClass based on user defaults
  ASSOCCLASS_PROTOCOL_STR, //  pszClass is a protocol, apply mapping of pszClass
...
SHSTDAPI AssocCreateForClasses(_In_reads_(cClasses)
                                   const ASSOCIATIONELEMENT *rgClasses,
                               ULONG cClasses, _In_ REFIID riid,
                               _COM_Outptr_ void **ppv);

/* #if 0 // SAMPLE CODE
HRESULT CCustomFolder::_AssocCreate(_In_ PCUITEMID_CHILD pidl, _In_ REFIID riid,
_Outptr_ void **ppv)
{
    *ppv = nullptr;
...
SHSTDAPI SHQueryRecycleBinA(_In_opt_ LPCSTR pszRootPath,
                            _Inout_ LPSHQUERYRBINFO pSHQueryRBInfo);
SHSTDAPI SHQueryRecycleBinW(_In_opt_ LPCWSTR pszRootPath,
                            _Inout_ LPSHQUERYRBINFO pSHQueryRBInfo);
#ifdef UNICODE
...
#endif // !UNICODE
SHSTDAPI SHEmptyRecycleBinA(_In_opt_ HWND hwnd, _In_opt_ LPCSTR pszRootPath,
                            DWORD dwFlags);
SHSTDAPI SHEmptyRecycleBinW(_In_opt_ HWND hwnd, _In_opt_ LPCWSTR pszRootPath,
                            DWORD dwFlags);
#ifdef UNICODE
...
typedef enum {
  QUNS_NOT_PRESENT =
      1, // The user is not present.  Heuristic check for modes like: screen
         // saver, locked machine, non-active FUS session
  QUNS_BUSY =
      2, // The user is busy.  Heuristic check for modes like: full-screen app
  QUNS_RUNNING_D3D_FULL_SCREEN = 3, // full-screen (exlusive-mode) D3D app
  QUNS_PRESENTATION_MODE =
      4, // Windows presentation mode (laptop feature) is turned on
...
SHSTDAPI
SHQueryUserNotificationState(_Out_ QUERY_USER_NOTIFICATION_STATE *pquns);
#endif // (NTDDI_VERSION >= NTDDI_VISTA)

#if (NTDDI_VERSION >= NTDDI_WIN7)
// This api retrieves an IPropertyStore that stores the window's properties.
SHSTDAPI SHGetPropertyStoreForWindow(_In_ HWND hwnd, _In_ REFIID riid,
                                     _Outptr_ void **ppv);
#endif
...
SHSTDAPI_(BOOL)
Shell_NotifyIconA(DWORD dwMessage, _In_ PNOTIFYICONDATAA lpData);
SHSTDAPI_(BOOL)
Shell_NotifyIconW(DWORD dwMessage, _In_ PNOTIFYICONDATAW lpData);
#ifdef UNICODE
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
SHSTDAPI Shell_NotifyIconGetRect(_In_ const NOTIFYICONIDENTIFIER *identifier,
...
SHSTDAPI_(DWORD_PTR)
SHGetFileInfoA(_In_ LPCSTR pszPath, DWORD dwFileAttributes,
               _Inout_updates_bytes_opt_(cbFileInfo) SHFILEINFOA *psfi,
               UINT cbFileInfo, UINT uFlags);
SHSTDAPI_(DWORD_PTR)
SHGetFileInfoW(_In_ LPCWSTR pszPath, DWORD dwFileAttributes,
...
SHSTDAPI_(BOOL)
SHGetDiskFreeSpaceExA(_In_ LPCSTR pszDirectoryName,
                      _Out_opt_ ULARGE_INTEGER *pulFreeBytesAvailableToCaller,
                      _Out_opt_ ULARGE_INTEGER *pulTotalNumberOfBytes,
                      _Out_opt_ ULARGE_INTEGER *pulTotalNumberOfFreeBytes);
SHSTDAPI_(BOOL)
SHGetDiskFreeSpaceExW(_In_ LPCWSTR pszDirectoryName,
                      _Out_opt_ ULARGE_INTEGER *pulFreeBytesAvailableToCaller,
                      _Out_opt_ ULARGE_INTEGER *pulTotalNumberOfBytes,
                      _Out_opt_ ULARGE_INTEGER *pulTotalNumberOfFreeBytes);
#ifdef UNICODE
...
_Success_(return != 0) SHSTDAPI_(BOOL)
    SHGetNewLinkInfoA(_In_ LPCSTR pszLinkTo, _In_ LPCSTR pszDir,
                      _Out_writes_(MAX_PATH) LPSTR pszName,
                      _Out_ BOOL *pfMustCopy, _In_ UINT uFlags);
_Success_(return != 0) SHSTDAPI_(BOOL)
    SHGetNewLinkInfoW(_In_ LPCWSTR pszLinkTo, _In_ LPCWSTR pszDir,
...
#define SHGNLI_NOLOCNAME                                                       \
  0x000000010 // use non localized (parsing) name from the target
#endif
#if (NTDDI_VERSION >= NTDDI_WIN7)
...
#define PRINTACTION_PROPERTIES                                                 \
  1 // pszBuf1:<PrinterName>, pszBuf2:optional <PageName>
#define PRINTACTION_NETINSTALL 2 // pszBuf1:<NetPrinterName>
#define PRINTACTION_NETINSTALLLINK                                             \
  3 // pszBuf1:<NetPrinterName>, pszBuf2:<path to store link>
...
// deprecated, instead invoke verbs on printers/netprinters using IContextMenu
// or ShellExecute()

SHSTDAPI_(BOOL)
SHInvokePrinterCommandA(_In_opt_ HWND hwnd, UINT uAction, _In_ LPCSTR lpBuf1,
                        _In_opt_ LPCSTR lpBuf2, BOOL fModal);
SHSTDAPI_(BOOL)
SHInvokePrinterCommandW(_In_opt_ HWND hwnd, UINT uAction, _In_ LPCWSTR lpBuf1,
                        _In_opt_ LPCWSTR lpBuf2, BOOL fModal);
#ifdef UNICODE
...
//     indicating "open in both places." This is common when the server is
//     online.
//
SHSTDAPI SHIsFileAvailableOffline(_In_ PCWSTR pwszPath,
                                  _Out_opt_ DWORD *pdwStatus);
...
SHSTDAPI SHSetLocalizedName(_In_ PCWSTR pszPath, _In_ PCWSTR pszResModule,
                            int idsRes);
#endif // (NTDDI_VERSION >= NTDDI_WINXP)
#if (NTDDI_VERSION >= NTDDI_VISTA)
//  sets the specified path to use the string resource
...
SHSTDAPI SHGetLocalizedName(_In_ PCWSTR pszPath,
                            _Out_writes_(cch) PWSTR pszResModule, UINT cch,
                            _Out_ int *pidsRes);
#endif // (NTDDI_VERSION >= NTDDI_VISTA)
...
LWSTDAPIV_(int)
ShellMessageBoxA(_In_opt_ HINSTANCE hAppInst, _In_opt_ HWND hWnd,
                 _In_ LPCSTR lpcText, _In_opt_ LPCSTR lpcTitle,
                 _In_ UINT fuStyle, ...);
LWSTDAPIV_(int)
ShellMessageBoxW(_In_opt_ HINSTANCE hAppInst, _In_opt_ HWND hWnd,
                 _In_ LPCWSTR lpcText, _In_opt_ LPCWSTR lpcTitle,
                 _In_ UINT fuStyle, ...);
#ifdef UNICODE
...
STDAPI SHEnumerateUnreadMailAccountsA(_In_opt_ HKEY hKeyUser, DWORD dwIndex,
                                      _Out_writes_(cchMailAddress)
                                          LPSTR pszMailAddress,
                                      int cchMailAddress);
STDAPI SHEnumerateUnreadMailAccountsW(_In_opt_ HKEY hKeyUser, DWORD dwIndex,
                                      _Out_writes_(cchMailAddress)
...
#endif // !UNICODE
STDAPI SHGetUnreadMailCountA(_In_opt_ HKEY hKeyUser,
                             _In_opt_ LPCSTR pszMailAddress,
                             _Out_opt_ DWORD *pdwCount,
                             _Out_opt_ FILETIME *pFileTime,
                             _Out_writes_opt_(cchShellExecuteCommand)
                                 LPSTR pszShellExecuteCommand,
                             int cchShellExecuteCommand);
STDAPI SHGetUnreadMailCountW(_In_opt_ HKEY hKeyUser,
...
#endif // !UNICODE
STDAPI SHSetUnreadMailCountA(_In_ LPCSTR pszMailAddress, DWORD dwCount,
                             _In_ LPCSTR pszShellExecuteCommand);
STDAPI SHSetUnreadMailCountW(_In_ LPCWSTR pszMailAddress, DWORD dwCount,
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
SHSTDAPI SHGetImageList(_In_ int iImageList, _In_ REFIID riid,
...
#define SHIL_SYSSMALL                                                          \
  3 // like SHIL_SMALL, but tracks system small icon metric correctly
...
typedef HRESULT(STDMETHODCALLTYPE *PFNSHOWSHAREFOLDERUIW)(
    _In_opt_ HWND hwndParent, _In_ PCWSTR pszPath);
