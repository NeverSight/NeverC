...
//  4.  Add 0x10000
//
//  Result: U+10000. This is correct, since the first character in the
//  Supplementary character range immediately follows the last code point in the
//  16-bit UTF-16 range (U+FFFF)
//
//  B) A UTF-32 code point such as U+2040A (this a CJK character in CJK
//  Extension B), and wish to convert it in UTF-16:
//
//  1.  Subtract 0x10000 - Result: 0x1040A
//  2.  Split into two ten-bit pieces: 0001000001 0000001010
//  3.  Add 1101100000000000 (0xD800) to the high 10 bits piece (0001000001) -
//  Result: 1101100001000001 (0xD841)
//  4.  Add 1101110000000000 (0xDC00) to the low 10 bits piece (0000001010) -
//  Result: 1101110000001010 (0xDC0A)
//
...
#define MB_PRECOMPOSED                                                         \
  0x00000001 // DEPRECATED: use single precomposed characters when possible.
#define MB_COMPOSITE                                                           \
  0x00000002 // DEPRECATED: use multiple discrete characters when possible.
#define MB_USEGLYPHCHARS                                                       \
  0x00000004 // DEPRECATED: use glyph chars, not ctrl chars
#define MB_ERR_INVALID_CHARS 0x00000008 // error for invalid chars

// WC_COMPOSITECHECK, WC_DISCARDNS and WC_SEPCHARS are deprecated, not
// recommended, and provide out-of-date behavior. Windows typically uses Unicode
// Normalization Form C type sequences, If explicit normalization forms are
// required, please use NormalizeString.
#define WC_COMPOSITECHECK 0x00000200 // convert composite to precomposed
#define WC_DISCARDNS                                                           \
  0x00000010 // discard non-spacing chars          // Used with
             // WC_COMPOSITECHECK
#define WC_SEPCHARS                                                            \
  0x00000020 // generate separate chars            // Used with
             // WC_COMPOSITECHECK
#define WC_DEFAULTCHAR                                                         \
  0x00000040 // replace w/ default char            // Used with
             // WC_COMPOSITECHECK
#if (WINVER >= 0x0600)
...
#define LINGUISTIC_IGNOREDIACRITIC                                             \
  0x00000020 // linguistically appropriate 'ignore nonspace'
...
#define LCMAP_SIMPLIFIED_CHINESE                                               \
  0x02000000 // map traditional chinese to simplified chinese
#define LCMAP_TRADITIONAL_CHINESE                                              \
  0x04000000 // map simplified chinese to traditional chinese
...
#define FIND_FROMSTART                                                         \
  0x00400000 // look for value in source, starting at the beginning
...
// inaccurate, no longer maintained, and no longer supported.
//
// ** DEPRECATED ** DEPRECATED ** DEPRECATED ** DEPRECATED ** DEPRECATED **
//
...
#define LOCALE_WINDOWS                                                         \
  0x00000001 // shipped locales and/or replacements for them
...
#define LOCALE_REPLACEMENT                                                     \
  0x00000008 // locales that replace shipped locales (callback flag only)
#endif       // (WINVER >= _WIN32_WINNT_VISTA)
#if (WINVER >= _WIN32_WINNT_WIN7)
#define LOCALE_NEUTRALDATA                                                     \
  0x00000010 // Locales that are "neutral" (language only, region data is
             // default)
#define LOCALE_SPECIFICDATA                                                    \
  0x00000020 // Locales that contain language and region data
#endif       // (WINVER >= _WIN32_WINNT_WIN7)
...
//
//    DEPRECATED: LOCALE_USE_CP_ACP is used in many of the A (Ansi) apis that
//    need
//                to do string translation.  Callers are encouraged to use the W
//                (WCHAR/Unicode) apis instead.
//
#define LOCALE_NOUSEROVERRIDE                                                  \
  0x80000000 // Not Recommended - do not use user overrides
#define LOCALE_USE_CP_ACP                                                      \
  0x40000000 // DEPRECATED, call Unicode APIs instead: use the system ACP
...
#define LOCALE_RETURN_GENITIVE_NAMES                                           \
  0x10000000 // Flag to return the Genitive forms of month names
#define LOCALE_ALLOW_NEUTRAL_NAMES                                             \
  0x08000000 // Flag to allow returning neutral names/lcids for name conversion
#endif       //(WINVER >= _WIN32_WINNT_WIN7)
...
#define LOCALE_SLOCALIZEDDISPLAYNAME                                           \
  0x00000002 // localized name of locale, eg "German (Germany)" in UI language
#if (WINVER >= _WIN32_WINNT_WIN7)
#define LOCALE_SENGLISHDISPLAYNAME                                             \
  0x00000072 // Display name (language + country/region usually) in English, eg
             // "German (Germany)"
#define LOCALE_SNATIVEDISPLAYNAME                                              \
  0x00000073 // Display name in native locale language, eg "Deutsch
             // (Deutschland)
#endif       //(WINVER >= _WIN32_WINNT_WIN7)

#if (WINVER >= _WIN32_WINNT_VISTA)
#define LOCALE_SLOCALIZEDLANGUAGENAME                                          \
  0x0000006f // Language Display Name for a language, eg "German" in UI language
#endif       //(WINVER >= _WIN32_WINNT_VISTA)
#define LOCALE_SENGLISHLANGUAGENAME                                            \
  0x00001001 // English name of language, eg "German"
#define LOCALE_SNATIVELANGUAGENAME                                             \
  0x00000004 // native name of language, eg "Deutsch"

#define LOCALE_SLOCALIZEDCOUNTRYNAME                                           \
  0x00000006 // localized name of country/region, eg "Germany" in UI language
#define LOCALE_SENGLISHCOUNTRYNAME                                             \
  0x00001002 // English name of country/region, eg "Germany"
#define LOCALE_SNATIVECOUNTRYNAME                                              \
  0x00000008 // native name of country/region, eg "Deutschland"

// Additional LCTypes
#define LOCALE_IDIALINGCODE                                                    \
  0x00000005 // country/region dialing code, example: en-US and en-CA return 1.
...
#define LOCALE_ILZERO                                                          \
  0x00000012 // leading zeros for decimal, 0 for .97, 1 for 0.97
#define LOCALE_INEGNUMBER                                                      \
  0x00001010 // negative number mode, 0-4, see documentation
#define LOCALE_SNATIVEDIGITS                                                   \
  0x00000013 // native digits for 0-9, eg "0123456789"
...
#define LOCALE_SMONDECIMALSEP                                                  \
  0x00000016 // monetary decimal separator, eg "." for $1,234.00
#define LOCALE_SMONTHOUSANDSEP                                                 \
  0x00000017 // monetary thousand separator, eg "," for $1,234.00
#define LOCALE_SMONGROUPING                                                    \
  0x00000018 // monetary grouping, eg "3;0" for $1,000,000.00
#define LOCALE_ICURRDIGITS 0x00000019 // # local monetary digits, eg 2 for $1.00
#define LOCALE_ICURRENCY                                                       \
  0x0000001B // positive currency mode, 0-3, see documentation
#define LOCALE_INEGCURR                                                        \
  0x0000001C // negative currency mode, 0-15, see documentation

#define LOCALE_SSHORTDATE                                                      \
  0x0000001F // short date format string, eg "MM/dd/yyyy"
#define LOCALE_SLONGDATE                                                       \
  0x00000020 // long date format string, eg "dddd, MMMM dd, yyyy"
...
#define LOCALE_ICALENDARTYPE                                                   \
  0x00001009 // type of calendar specifier, eg CAL_GREGORIAN
#define LOCALE_IOPTIONALCALENDAR                                               \
  0x0000100B // additional calendar types specifier, eg CAL_GREGORIAN_US
#define LOCALE_IFIRSTDAYOFWEEK                                                 \
  0x0000100C // first day of week specifier, 0-6, 0=Monday, 6=Sunday
#define LOCALE_IFIRSTWEEKOFYEAR                                                \
  0x0000100D // first week of year specifier, 0-2, see documentation
...
#define LOCALE_SABBREVMONTHNAME13                                              \
  0x0000100F // abbreviated name for 13th month (if exists)
...
#define LOCALE_IPOSSIGNPOSN                                                    \
  0x00000052 // positive sign position (derived from INEGCURR)
#define LOCALE_INEGSIGNPOSN                                                    \
  0x00000053 // negative sign position (derived from INEGCURR)
#define LOCALE_IPOSSYMPRECEDES                                                 \
  0x00000054 // mon sym precedes pos amt (derived from ICURRENCY)
#define LOCALE_IPOSSEPBYSPACE                                                  \
  0x00000055 // mon sym sep by space from pos amt (derived from ICURRENCY)
#define LOCALE_INEGSYMPRECEDES                                                 \
  0x00000056 // mon sym precedes neg amt (derived from INEGCURR)
#define LOCALE_INEGSEPBYSPACE                                                  \
  0x00000057 // mon sym sep by space from neg amt (derived from INEGCURR)

#if (WINVER >= 0x0400)
#define LOCALE_FONTSIGNATURE 0x00000058 // font signature
#define LOCALE_SISO639LANGNAME                                                 \
  0x00000059 // ISO abbreviated language name, eg "en"
#define LOCALE_SISO3166CTRYNAME                                                \
  0x0000005A // ISO abbreviated country/region name, eg "US"
#endif       /* WINVER >= 0x0400 */
...
#define LOCALE_SSORTNAME                                                       \
  0x00001013 // sort name, usually "", eg "Dictionary" in UI Language
#define LOCALE_IDIGITSUBSTITUTION                                              \
  0x00001014 // 0 = context, 1 = none, 2 = national
...
#define LOCALE_SISO639LANGNAME2                                                \
  0x00000067 // 3 character ISO abbreviated language name, eg "eng"
#define LOCALE_SISO3166CTRYNAME2                                               \
  0x00000068                   // 3 character ISO country/region name, eg "USA"
...
#define LOCALE_SSCRIPTS                                                        \
  0x0000006c // Typical scripts in the locale: ; delimited script codes, eg
             // "Latn;"
#define LOCALE_SPARENT                                                         \
  0x0000006d // Fallback name for resources, eg "en" for "en-US"
#define LOCALE_SCONSOLEFALLBACKNAME                                            \
  0x0000006e // Fallback name for within the console for Unicode Only locales,
             // eg "en" for bn-IN
#endif       //(WINVER >= 0x0600)

#if (WINVER >= _WIN32_WINNT_WIN7)
#define LOCALE_IREADINGLAYOUT                                                  \
  0x00000070 // Returns one of the following 4 reading layout values:
             // 0 - Left to right (eg en-US)
             // 1 - Right to left (eg arabic locales)
             // 2 - Vertical top to bottom with columns to the left and also
             // left to right (ja-JP locales) 3 - Vertical top to bottom with
             // columns proceeding to the right
#define LOCALE_INEUTRAL                                                        \
  0x00000071 // Returns 0 for specific cultures, 1 for neutral cultures.
#define LOCALE_INEGATIVEPERCENT                                                \
  0x00000074 // Returns 0-11 for the negative percent format
#define LOCALE_IPOSITIVEPERCENT                                                \
  0x00000075 // Returns 0-3 for the positive percent formatIPOSITIVEPERCENT
...
#define LOCALE_SSHORTTIME                                                      \
  0x00000079 // Returns the preferred short time format (ie: no seconds, just
             // h:mm)
#define LOCALE_SOPENTYPELANGUAGETAG                                            \
  0x0000007a // Open type language tag, eg: "latn" or "dflt"
#define LOCALE_SSORTLOCALE                                                     \
  0x0000007b // Name of locale to use for sorting/collation/casing behavior.
#endif       //(WINVER >= _WIN32_WINNT_WIN7)

#if (WINVER >= _WIN32_WINNT_WIN8)
#define LOCALE_SRELATIVELONGDATE                                               \
  0x0000007c // Long date without year, day of week, month, date, eg: for lock
             // screen
#endif
#if (WINVER >= _WIN32_WINNT_WINBLUE)
#define LOCALE_ICONSTRUCTEDLOCALE                                              \
  0x0000007d // Flags if this locale is constructed.  Avoid using.
#endif
...
#define LOCALE_IUSEUTF8LEGACYACP                                               \
  0x00000666 // default ansi code page (use of Unicode is recommended instead)
#define LOCALE_IUSEUTF8LEGACYOEMCP                                             \
  0x00000999 // default oem code page (use of Unicode is recommended instead)
#endif
...
// Applications are strongly encouraged to Use Unicode, such as UTF-16 (WCHAR
// type) or the CP_UTF8 Code Page.  Legacy encodings are unable to represent the
// full set of scripts/language and characters (& emoji!) available on modern
// computers. Use of legacy code pages (encodings) is a leading cause of data
// loss and corruption.
#define LOCALE_IDEFAULTCODEPAGE                                                \
  0x0000000B // default oem code page for locale (user may configure as UTF-8,
             // use of Unicode is recommended instead)
#define LOCALE_IDEFAULTANSICODEPAGE                                            \
  0x00001004 // default ansi code page for locale (user may configure as UTF-8,
             // use of Unicode is recommended instead)
#define LOCALE_IDEFAULTMACCODEPAGE                                             \
  0x00001011 // default mac code page for locale (user may configure as UTF-8,
             // use of Unicode is recommended instead)
#if (WINVER >= 0x0500)
#define LOCALE_IDEFAULTEBCDICCODEPAGE                                          \
  0x00001012 // default ebcdic code page for a locale (use of Unicode is
             // recommended instead)
#endif       /* WINVER >= 0x0500 */

// LCTYPEs using out-of-date concepts
#define LOCALE_ILANGUAGE                                                       \
  0x00000001 // DEPRECATED language id (LCID), LOCALE_SNAME preferred
#define LOCALE_SABBREVLANGNAME                                                 \
  0x00000003 // DEPRECATED arbitrary abbreviated language name,
             // LOCALE_SISO639LANGNAME instead.
#define LOCALE_SABBREVCTRYNAME                                                 \
  0x00000007 // DEPRECATED arbitrary abbreviated country/region name,
             // LOCALE_SISO3166CTRYNAME instead.
#define LOCALE_IGEOID                                                          \
  0x0000005B // DEPRECATED geographical location id, use LOCALE_SISO3166CTRYNAME
             // instead.
#define LOCALE_IDEFAULTLANGUAGE                                                \
  0x00000009 // DEPRECATED default language id, deprecated
#define LOCALE_IDEFAULTCOUNTRY                                                 \
  0x0000000A // DEPRECATED default country/region code, deprecated
#define LOCALE_IINTLCURRDIGITS                                                 \
  0x0000001A // DEPRECATED, use LOCALE_ICURRDIGITS # intl monetary digits, eg 2
             // for $1.00

// Derived legacy date & time values for compatibility only.
// Please use the appropriate date or time pattern instead.
// These can be misleading, for example a locale configured as 12h24m52s could
// have a time separator of "h".
#define LOCALE_SDATE                                                           \
  0x0000001D // DEPRECATED date separator (derived from LOCALE_SSHORTDATE, use
             // that instead)
#define LOCALE_STIME                                                           \
  0x0000001E // DEPRECATED time separator (derived from LOCALE_STIMEFORMAT, use
             // that instead)
#define LOCALE_IDATE                                                           \
  0x00000021 // DEPRECATED short date format ordering (derived from
             // LOCALE_SSHORTDATE, use that instead)
#define LOCALE_ILDATE                                                          \
  0x00000022 // DEPRECATED long date format ordering (derived from
             // LOCALE_SLONGDATE, use that instead)
#define LOCALE_ITIME                                                           \
  0x00000023 // DEPRECATED time format specifier (derived from
             // LOCALE_STIMEFORMAT, use that instead)
#define LOCALE_ITIMEMARKPOSN                                                   \
  0x00001005 // DEPRECATED time marker position (derived from
             // LOCALE_STIMEFORMAT, use that instead)
#define LOCALE_ICENTURY                                                        \
  0x00000024 // DEPRECATED century format specifier (short date,
             // LOCALE_SSHORTDATE is preferred)
#define LOCALE_ITLZERO                                                         \
  0x00000025 // DEPRECATED leading zeros in time field (derived from
             // LOCALE_STIMEFORMAT, use that instead)
#define LOCALE_IDAYLZERO                                                       \
  0x00000026 // DEPRECATED leading zeros in day field (short date,
             // LOCALE_SSHORTDATE is preferred)
#define LOCALE_IMONLZERO                                                       \
  0x00000027 // DEPRECATED leading zeros in month field (short date,
             // LOCALE_SSHORTDATE is preferred)

#if (WINVER >= 0x0600)
#define LOCALE_SKEYBOARDSTOINSTALL                                             \
  0x0000005e // Used internally, see GetKeyboardLayoutName() function
#endif       /* WINVER >= 0x0600 */

// LCTYPEs which have been renamed to enable more understandable source code.
#define LOCALE_SLANGUAGE                                                       \
  LOCALE_SLOCALIZEDDISPLAYNAME // DEPRECATED as new name is more readable.
#if (WINVER >= _WIN32_WINNT_VISTA)
#define LOCALE_SLANGDISPLAYNAME                                                \
  LOCALE_SLOCALIZEDLANGUAGENAME // DEPRECATED as new name is more readable.
#endif                          //(WINVER >= _WIN32_WINNT_VISTA)
#define LOCALE_SENGLANGUAGE                                                    \
  LOCALE_SENGLISHLANGUAGENAME // DEPRECATED as new name is more readable.
#define LOCALE_SNATIVELANGNAME                                                 \
  LOCALE_SNATIVELANGUAGENAME // DEPRECATED as new name is more readable.
#define LOCALE_SCOUNTRY                                                        \
  LOCALE_SLOCALIZEDCOUNTRYNAME // DEPRECATED as new name is more readable.
#define LOCALE_SENGCOUNTRY                                                     \
  LOCALE_SENGLISHCOUNTRYNAME // DEPRECATED as new name is more readable.
#define LOCALE_SNATIVECTRYNAME                                                 \
  LOCALE_SNATIVECOUNTRYNAME // DEPRECATED as new name is more readable.
// DEPRECATED: Use LOCALE_SISO3166CTRYNAME to query for a region identifier,
// LOCALE_ICOUNTRY is not a region identifier.
#define LOCALE_ICOUNTRY                                                        \
  LOCALE_IDIALINGCODE // Deprecated synonym for LOCALE_IDIALINGCODE
#define LOCALE_S1159                                                           \
  LOCALE_SAM // DEPRECATED: Please use LOCALE_SAM, which is more readable.
#define LOCALE_S2359                                                           \
  LOCALE_SPM // DEPRECATED: Please use LOCALE_SPM, which is more readable.
...
#define DATE_LTRREADING                                                        \
  0x00000010 // add marks for left to right reading order layout
#define DATE_RTLREADING                                                        \
  0x00000020 // add marks for right to left reading order layout
#endif       /* WINVER >= 0x0500 */

#if (WINVER >= _WIN32_WINNT_WIN7)
#define DATE_AUTOLAYOUT                                                        \
  0x00000040 // add appropriate marks for left-to-right or right-to-left reading
             // order layout
#endif       //(WINVER >= _WIN32_WINNT_WIN7)
...
#define CAL_NOUSEROVERRIDE                                                     \
  LOCALE_NOUSEROVERRIDE // Not Recommended - do not use user overrides
#define CAL_USE_CP_ACP                                                         \
  LOCALE_USE_CP_ACP // DEPRECATED, call Unicode APIs instead: use the system ACP
#define CAL_RETURN_NUMBER                                                      \
  LOCALE_RETURN_NUMBER // return number instead of string
#endif                 /* WINVER >= 0x0500 */

#if (WINVER >= _WIN32_WINNT_WIN7)
#define CAL_RETURN_GENITIVE_NAMES                                              \
  LOCALE_RETURN_GENITIVE_NAMES // return genitive forms of month names
...
// Note that in the hebrew calendar the leap month name is always returned as
// the 7th month
...
#define CAL_SABBREVMONTHNAME13                                                 \
  0x0000002e // abbreviated name for 13th month (if any)
...
#define CAL_SRELATIVELONGDATE                                                  \
  0x0000003a // Long date without year, day of week, month, date, eg: for lock
             // screen
#endif

#if (NTDDI_VERSION >= NTDDI_WIN10_RS2)
#define CAL_SENGLISHERANAME                                                    \
  0x0000003b // Japanese calendar only: return the English era names for .Net
             // compatibility
#define CAL_SENGLISHABBREVERANAME                                              \
  0x0000003c // Japanese calendar only: return the English Abbreviated era names
             // for .Net compatibility
#endif

// CAL_SJAPANESEERAFIRSTYEAR is only supported on machines with updates to
// support the "gannen" era first year behavior Machines without that update
// will return 0 and ERROR_INVALID_FLAGS, in which case ichinen is presumed.
#define CAL_SJAPANESEERAFIRSTYEAR                                              \
  0x0000003d // Japanese calendar only: return ichinen or gannen first year
...
#define MUI_MERGE_SYSTEM_FALLBACK                                              \
  0x10 // GetThreadPreferredUILanguages merges in parent and base languages
#define MUI_MERGE_USER_FALLBACK                                                \
  0x20 // GetThreadPreferredUILanguages merges in user preferred languages
#define MUI_UI_FALLBACK MUI_MERGE_SYSTEM_FALLBACK | MUI_MERGE_USER_FALLBACK
#define MUI_THREAD_LANGUAGES                                                   \
  0x40 // GetThreadPreferredUILanguages merges in user preferred languages
#define MUI_CONSOLE_FILTER                                                     \
  0x100 // SetThreadPreferredUILanguages takes on console specific behavior
#define MUI_COMPLEX_SCRIPT_FILTER                                              \
  0x200 // SetThreadPreferredUILanguages takes on complex script specific
        // behavior
#define MUI_RESET_FILTERS                                                      \
  0x001 // Reset MUI_CONSOLE_FILTER and MUI_COMPLEX_SCRIPT_FILTER
#define MUI_USER_PREFERRED_UI_LANGUAGES                                        \
  0x10 // GetFileMUIPath returns the MUI files for the languages in the fallback
       // list
#define MUI_USE_INSTALLED_LANGUAGES                                            \
  0x20 // GetFileMUIPath returns all the MUI files installed in the machine
#define MUI_USE_SEARCH_ALL_LANGUAGES                                           \
  0x40 // GetFileMUIPath returns all the MUI files irrespective of whether
       // language is installed
#define MUI_LANG_NEUTRAL_PE_FILE                                               \
  0x100 // GetFileMUIPath returns target file with .mui extension
#define MUI_NON_LANG_NEUTRAL_FILE                                              \
  0x200 // GetFileMUIPath returns target file with same name as source
#define MUI_MACHINE_LANGUAGE_SETTINGS 0x400
#define MUI_FILETYPE_NOT_LANGUAGE_NEUTRAL                                      \
  0x001 // GetFileMUIInfo found a non-split resource file
#define MUI_FILETYPE_LANGUAGE_NEUTRAL_MAIN                                     \
  0x002 // GetFileMUIInfo found a LN main module resource file
#define MUI_FILETYPE_LANGUAGE_NEUTRAL_MUI                                      \
  0x004 // GetFileMUIInfo found a LN MUI module resource file
#define MUI_QUERY_TYPE                                                         \
  0x001 // GetFileMUIInfo will look for the type of the resource file
#define MUI_QUERY_CHECKSUM                                                     \
  0x002 // GetFileMUIInfo will look for the checksum of the resource file
#define MUI_QUERY_LANGUAGE_NAME                                                \
  0x004 // GetFileMUIInfo will look for the culture of the resource file
#define MUI_QUERY_RESOURCE_TYPES                                               \
  0x008 // GetFileMUIInfo will look for the resource types of the resource file
#define MUI_FILEINFO_VERSION                                                   \
  0x001 // Version of FILEMUIINFO structure used with GetFileMUIInfo
...
#define MUI_CALLBACK_ALL_FLAGS                                                 \
  MUI_CALLBACK_FLAG_UPGRADED_INSTALLATION // OR all other flags when defined.
...
DEPRECATED("Use Unicode. The information in this structure cannot represent "
           "all encodings accurately and may be unreliable on many machines. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
...
//
// Please use GetGeoInfoEx and query by country/region name instead of GEOID
// (eg: "US" instead of 244)
enum SYSGEOTYPE {
...
//
DEPRECATED(
    "The Geo Class concept is obsolete and no longer supported.  GetGeoInfoEx "
    "is preferred. Set DISABLE_NLS_DEPRECATION to disable this warning.")
enum SYSGEOCLASS {
  GEOCLASS_NATION = 16,
...
typedef BOOL(CALLBACK *LOCALE_ENUMPROCA)(
    LPSTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *LOCALE_ENUMPROCW)(
    LPWSTR); // DEPRECATED: please use LOCALE_ENUMPROCEX
...
typedef enum _NORM_FORM {
  ...
  NormalizationC = 0x1, // Each base plus combining characters to the canonical
                        // precomposed equivalent.
  NormalizationD =
      0x2, // Each precomposed character to its canonical decomposed equivalent.
  NormalizationKC =
      0x5, // Each base plus combining characters to the canonical precomposed
           //   equivalents and all compatibility characters to their
           //   equivalents.
  NormalizationKD =
      0x6 // Each precomposed character to its canonical decomposed equivalent
...
//
#define IDN_ALLOW_UNASSIGNED                                                   \
  0x01 // Allow unassigned "query" behavior per RFC 3454
#define IDN_USE_STD3_ASCII_RULES                                               \
  0x02 // Enforce STD3 ASCII restrictions for legal characters
#define IDN_EMAIL_ADDRESS                                                      \
  0x04 // Enable EAI algorithmic fallback for email local parts behavior
#define IDN_RAW_PUNYCODE 0x08 // Disable validation and mapping of punycode.

#define VS_ALLOW_LATIN                                                         \
  0x0001 // Allow Latin in test script even if not present in locale script

#define GSS_ALLOW_INHERITED_COMMON                                             \
  0x0001 // Output script ids for inherited and common character types if
         // present
#endif   //(WINVER >= 0x0600)
...
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
typedef BOOL(CALLBACK *LANGUAGEGROUP_ENUMPROCA)(
    LGRPID, LPSTR, LPSTR, DWORD, LONG_PTR); // Deprecated, please use Unicode
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
typedef BOOL(CALLBACK *LANGGROUPLOCALE_ENUMPROCA)(
    LGRPID, LCID, LPSTR, LONG_PTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *UILANGUAGE_ENUMPROCA)(
    LPSTR, LONG_PTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *CODEPAGE_ENUMPROCA)(
    LPSTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *DATEFMT_ENUMPROCA)(
    LPSTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *DATEFMT_ENUMPROCEXA)(
    LPSTR, CALID); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *TIMEFMT_ENUMPROCA)(
    LPSTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *CALINFO_ENUMPROCA)(
    LPSTR); // Deprecated, please use Unicode
typedef BOOL(CALLBACK *CALINFO_ENUMPROCEXA)(
    LPSTR, CALID); // Deprecated, please use Unicode

DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
typedef BOOL(CALLBACK *LANGUAGEGROUP_ENUMPROCW)(
    LGRPID, LPWSTR, LPWSTR, DWORD,
    LONG_PTR); // DEPRECATED: Language groups are no longer supported
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
typedef BOOL(CALLBACK *LANGGROUPLOCALE_ENUMPROCW)(
    LGRPID, LCID, LPWSTR,
    LONG_PTR); // DEPRECATED: Language groups are no longer supported
typedef BOOL(CALLBACK *UILANGUAGE_ENUMPROCW)(LPWSTR, LONG_PTR);
typedef BOOL(CALLBACK *CODEPAGE_ENUMPROCW)(
    LPWSTR); // Please use Unicode / UTF-8
typedef BOOL(CALLBACK *DATEFMT_ENUMPROCW)(LPWSTR);
typedef BOOL(CALLBACK *DATEFMT_ENUMPROCEXW)(LPWSTR, CALID);
typedef BOOL(CALLBACK *TIMEFMT_ENUMPROCW)(LPWSTR);
typedef BOOL(CALLBACK *CALINFO_ENUMPROCW)(LPWSTR);
typedef BOOL(CALLBACK *CALINFO_ENUMPROCEXW)(LPWSTR, CALID);
typedef BOOL(CALLBACK *GEO_ENUMPROC)(
    GEOID); // DEPRECATED, use GEO_ENUMNAMEPROC instead
#if (NTDDI_VERSION >= NTDDI_WIN10_RS3)
...
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
typedef FARPROC LANGUAGEGROUP_ENUMPROCW; // DEPRECATED: Language groups are no
                                         // longer supported
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
typedef FARPROC LANGGROUPLOCALE_ENUMPROCW; // DEPRECATED: Language groups are no
                                           // longer supported
...
typedef struct _FILEMUIINFO {
  ...
  DWORD dwTypeNameMainOffset; // Multistring array of TypeNames in main module
                              // [out]
  ...
  DWORD
  dwTypeNameMUIOffset; // Multistring array of TypeNames in MUI module [out]
...
#define FILEMUIINFO_GET_MAIN_TYPEIDS(pInfo)                                    \
  ((DWORD *)((pInfo->dwTypeIDMainOffset > 0)                                   \
                 ...
                 : NULL))
//
// Gets the main module type array element iType (DWORD), the array is not
// initialized or index is out of bounds
#define FILEMUIINFO_GET_MAIN_TYPEID(pInfo, iType)                              \
  (((iType < pInfo->dwTypeIDMainSize) && (pInfo->dwTypeIDMainOffset > 0))      \
       ...
       : 0)
//
// Gets the main module names multistring array (LPWSTR), NULL if not
// initialized
...
#define FILEMUIINFO_GET_MUI_TYPEIDS(pInfo)                                     \
  ((DWORD *)((pInfo->dwTypeIDMUIOffset > 0)                                    \
                 ...
                 : NULL))
//
// Gets the mui module type array element iType (DWORD), the array is not
// initialized or index is out of bounds
...
WINBASEAPI
BOOL WINAPI IsValidCodePage(_In_ UINT CodePage);
...
DEPRECATED("Use Unicode. The information in this structure cannot represent "
           "all encodings accuratedly and may be unreliable on many machines. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI GetCPInfo(_In_ UINT CodePage, _Out_ LPCPINFO lpCPInfo);

#endif /* WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP                     \
          |WINAPI_PARTITION_PC_APP | WINAPI_PARTITION_SYSTEM |                 \
          WINAPI_PARTITION_GAMES) */
#pragma endregion
...
DEPRECATED("Use Unicode. The information in this structure cannot represent "
           ...
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI GetCPInfoExA(_In_ UINT CodePage, _In_ DWORD dwFlags,
                         _Out_ LPCPINFOEXA lpCPInfoEx);
DEPRECATED("Use Unicode. The information in this structure cannot represent "
           "all encodings accurately and may be unreliable on many machines. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI GetCPInfoExW(_In_ UINT CodePage, _In_ DWORD dwFlags,
...
DEPRECATED("CompareStringEx is preferred. Set DISABLE_NLS_DEPRECATION to "
           "disable this warning.")
WINBASEAPI
int WINAPI CompareStringA(_In_ LCID Locale, _In_ DWORD dwCmpFlags,
...
DEPRECATED("CompareStringEx is preferred. Set DISABLE_NLS_DEPRECATION to "
           "disable this warning.")
__inline int CompareString(LCID Locale, DWORD dwCmpFlags, LPCTSTR lpString1,
...
WINBASEAPI
int WINAPI FindNLSString(_In_ LCID Locale, _In_ DWORD dwFindNLSStringFlags,
                         _In_reads_(cchSource) LPCWSTR lpStringSource,
                         _In_ int cchSource,
                         _In_reads_(cchValue) LPCWSTR lpStringValue,
                         _In_ int cchValue, _Out_opt_ LPINT pcchFound);
...
WINBASEAPI
int WINAPI LCMapStringW(_In_ LCID Locale, _In_ DWORD dwMapFlags,
                        _In_reads_(cchSrc) LPCWSTR lpSrcStr, _In_ int cchSrc,
                        _Out_writes_opt_(_Inexpressible_(cchDest))
                            LPWSTR lpDestStr,
                        _In_ int cchDest);
#ifdef UNICODE
...
WINBASEAPI
int WINAPI LCMapStringA(_In_ LCID Locale, _In_ DWORD dwMapFlags,
                        _In_reads_(cchSrc) LPCSTR lpSrcStr, _In_ int cchSrc,
                        _Out_writes_opt_(_Inexpressible_(cchDest))
                            LPSTR lpDestStr,
                        _In_ int cchDest);
#ifndef UNICODE
...
WINBASEAPI
int WINAPI GetLocaleInfoW(_In_ LCID Locale, _In_ LCTYPE LCType,
...
WINBASEAPI
int WINAPI GetLocaleInfoA(_In_ LCID Locale, _In_ LCTYPE LCType,
...
WINBASEAPI
BOOL WINAPI SetLocaleInfoA(_In_ LCID Locale, _In_ LCTYPE LCType,
                           _In_ LPCSTR lpLCData);
WINBASEAPI
BOOL WINAPI SetLocaleInfoW(_In_ LCID Locale, _In_ LCTYPE LCType,
...
WINBASEAPI
int WINAPI GetCalendarInfoA(_In_ LCID Locale, _In_ CALID Calendar,
                            _In_ CALTYPE CalType,
                            _Out_writes_opt_(cchData) LPSTR lpCalData,
                            _In_ int cchData, _Out_opt_ LPDWORD lpValue);
// DEPRECATED: GetCalendarInfoEx is preferred
WINBASEAPI
int WINAPI GetCalendarInfoW(_In_ LCID Locale, _In_ CALID Calendar,
                            _In_ CALTYPE CalType,
...
WINBASEAPI
BOOL WINAPI SetCalendarInfoA(_In_ LCID Locale, _In_ CALID Calendar,
                             _In_ CALTYPE CalType, _In_ LPCSTR lpCalData);
WINBASEAPI
BOOL WINAPI SetCalendarInfoW(_In_ LCID Locale, _In_ CALID Calendar,
                             _In_ CALTYPE CalType, _In_ LPCWSTR lpCalData);
#ifdef UNICODE
...
WINBASEAPI
BOOL WINAPI LoadStringByReference(_In_ DWORD Flags, _In_opt_ PCWSTR Language,
...
DEPRECATED(
    "Use Unicode. The information provided by this structure is inaccurate for "
    "some encodings and may be unreliable on many machines.")
WINBASEAPI
BOOL WINAPI IsDBCSLeadByte(_In_ BYTE TestChar);

DEPRECATED(
    "Use Unicode. The information provided by this structure is inaccurate for "
    "some encodings and may be unreliable on many machines.")
WINBASEAPI
BOOL WINAPI IsDBCSLeadByteEx(_In_ UINT CodePage, _In_ BYTE TestChar);
...
int WINAPI LCIDToLocaleName(_In_ LCID Locale,
                            ...
                            _In_ int cchName, _In_ DWORD dwFlags);
...
#if (WINVER >= 0x0600)
WINBASEAPI
int WINAPI GetDurationFormat(_In_ LCID Locale, _In_ DWORD dwFlags,
...
WINBASEAPI
int WINAPI GetNumberFormatA(_In_ LCID Locale, _In_ DWORD dwFlags,
                            _In_ LPCSTR lpValue,
...
// DEPRECATED: GetNumberFormatEx is preferred
WINBASEAPI
int WINAPI GetNumberFormatW(_In_ LCID Locale, _In_ DWORD dwFlags,
...
WINBASEAPI
int WINAPI GetCurrencyFormatA(_In_ LCID Locale, _In_ DWORD dwFlags,
                              _In_ LPCSTR lpValue,
...
// DEPRECATED: GetCurrencyFormatEx is preferred
WINBASEAPI
int WINAPI GetCurrencyFormatW(_In_ LCID Locale, _In_ DWORD dwFlags,
                              _In_ LPCWSTR lpValue,
                              _In_opt_ CONST CURRENCYFMTW *lpFormat,
                              _Out_writes_opt_(cchCurrency)
...
WINBASEAPI
BOOL WINAPI EnumCalendarInfoA(_In_ CALINFO_ENUMPROCA lpCalInfoEnumProc,
                              _In_ LCID Locale, _In_ CALID Calendar,
                              _In_ CALTYPE CalType);
// DEPRECATED: EnumCalendarInfoExEx is preferred
WINBASEAPI
BOOL WINAPI EnumCalendarInfoW(_In_ CALINFO_ENUMPROCW lpCalInfoEnumProc,
                              _In_ LCID Locale, _In_ CALID Calendar,
                              _In_ CALTYPE CalType);
#ifdef UNICODE
...
// DEPRECATED: EnumCalendarInfoExEx is preferred
WINBASEAPI
BOOL WINAPI EnumCalendarInfoExA(_In_ CALINFO_ENUMPROCEXA lpCalInfoEnumProcEx,
                                _In_ LCID Locale, _In_ CALID Calendar,
                                _In_ CALTYPE CalType);
// DEPRECATED: EnumCalendarInfoExEx is preferred
WINBASEAPI
BOOL WINAPI EnumCalendarInfoExW(_In_ CALINFO_ENUMPROCEXW lpCalInfoEnumProcEx,
                                _In_ LCID Locale, _In_ CALID Calendar,
                                _In_ CALTYPE CalType);
#ifdef UNICODE
...
WINBASEAPI
BOOL WINAPI EnumTimeFormatsA(_In_ TIMEFMT_ENUMPROCA lpTimeFmtEnumProc,
                             _In_ LCID Locale, _In_ DWORD dwFlags);
// DEPRECATED: EnumTimeFormatsEx is preferred
WINBASEAPI
BOOL WINAPI EnumTimeFormatsW(_In_ TIMEFMT_ENUMPROCW lpTimeFmtEnumProc,
                             _In_ LCID Locale, _In_ DWORD dwFlags);
#ifdef UNICODE
...
WINBASEAPI
BOOL WINAPI EnumDateFormatsA(_In_ DATEFMT_ENUMPROCA lpDateFmtEnumProc,
                             _In_ LCID Locale, _In_ DWORD dwFlags);
// DEPRECATED: EnumDateFormatsExEx is preferred
WINBASEAPI
BOOL WINAPI EnumDateFormatsW(_In_ DATEFMT_ENUMPROCW lpDateFmtEnumProc,
                             _In_ LCID Locale, _In_ DWORD dwFlags);
#ifdef UNICODE
...
// DEPRECATED: EnumDateFormatsExEx is preferred
WINBASEAPI
BOOL WINAPI EnumDateFormatsExA(_In_ DATEFMT_ENUMPROCEXA lpDateFmtEnumProcEx,
                               _In_ LCID Locale, _In_ DWORD dwFlags);
// DEPRECATED: EnumDateFormatsExEx is preferred
WINBASEAPI
BOOL WINAPI EnumDateFormatsExW(_In_ DATEFMT_ENUMPROCEXW lpDateFmtEnumProcEx,
                               _In_ LCID Locale, _In_ DWORD dwFlags);
#ifdef UNICODE
...
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI IsValidLanguageGroup(_In_ LGRPID LanguageGroup, _In_ DWORD dwFlags);
#endif /* WINVER >= 0x0500 */

// DEPRECATED: GetNLSVersionEx is preferred
WINBASEAPI
BOOL WINAPI GetNLSVersion(_In_ NLS_FUNCTION Function, _In_ LCID Locale,
...
WINBASEAPI
BOOL WINAPI IsValidLocale(_In_ LCID Locale, _In_ DWORD dwFlags);
...
DEPRECATED("The GeoID concept is obsolete. Use GetGeoInfoEx instead. Set "
           "DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
int WINAPI GetGeoInfoA(_In_ GEOID Location, _In_ GEOTYPE GeoType,
                       _Out_writes_opt_(cchData) LPSTR lpGeoData,
                       _In_ int cchData, _In_ LANGID LangId);
// GetGeoInfoEx is preferred where available
DEPRECATED("The GeoID concept is obsolete. Use GetGeoInfoEx instead. Set "
           "DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
int WINAPI GetGeoInfoW(_In_ GEOID Location, _In_ GEOTYPE GeoType,
...
#if (NTDDI_VERSION >= NTDDI_WIN10_RS3)
WINBASEAPI
int WINAPI GetGeoInfoEx(_In_ PWSTR location, _In_ GEOTYPE geoType,
...
DEPRECATED("The GeoID concept is obsolete. Use EnumSystemGoNames instead. Set "
           "DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI EnumSystemGeoID(_In_ GEOCLASS GeoClass, _In_ GEOID ParentGeoId,
                            _In_ GEO_ENUMPROC lpGeoEnumProc);

#if (NTDDI_VERSION >= NTDDI_WIN10_RS3)
WINBASEAPI
BOOL WINAPI EnumSystemGeoNames(_In_ GEOCLASS geoClass,
...
DEPRECATED("The GeoID concept is obsolete. Use GetUserDefaultGeoName instead. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
GEOID
WINAPI
GetUserGeoID(_In_ GEOCLASS GeoClass);
 ...
 */
WINBASEAPI
int WINAPI GetUserDefaultGeoName(_Out_writes_z_(geoNameCount) LPWSTR geoName,
...
DEPRECATED("The GeoID concept is obsolete. Use SetUserGeoName instead. Set "
           "DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI SetUserGeoID(_In_ GEOID GeoId);

#if (NTDDI_VERSION >= NTDDI_WIN10_RS3)
// Applications are recommended to not change user settings themselves.
WINBASEAPI
BOOL WINAPI SetUserGeoName(_In_ PWSTR geoName);
#endif

// DEPRECATED: Please use ResolveLocaleName
WINBASEAPI
LCID WINAPI ConvertDefaultLocale(_In_ LCID Locale);
...
// DEPRECATED: Please use GetUserDefaultLocaleName or the user's Language
// Profile
WINBASEAPI
LANGID
WINAPI
...
BOOL WINAPI GetUserPreferredUILanguages(_In_ DWORD dwFlags,
                                        ...
                                        _Out_writes_opt_(*pcchLanguagesBuffer)
...
WINBASEAPI
BOOL WINAPI SetThreadPreferredUILanguages(_In_ DWORD dwFlags,
                                          _In_opt_ PCZZWSTR pwszLanguagesBuffer,
                                          _Out_opt_ PULONG pulNumLanguages);

WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    GetFileMUIInfo(DWORD dwFlags, _In_ PCWSTR pcwszFilePath,
                   _Inout_updates_bytes_to_opt_(*pcbFileMUIInfo,
                                                *pcbFileMUIInfo)
                       PFILEMUIINFO pFileMUIInfo,
                   _Inout_ DWORD *pcbFileMUIInfo);

WINBASEAPI
BOOL WINAPI GetFileMUIPath(_In_ DWORD dwFlags, _In_ PCWSTR pcwszFilePath,
                           _Inout_updates_opt_(*pcchLanguage)
                               PWSTR pwszLanguage,
                           _Inout_ PULONG pcchLanguage,
                           _Out_writes_opt_(*pcchFileMUIPath)
                               PWSTR pwszFileMUIPath,
                           _Inout_ PULONG pcchFileMUIPath,
                           _Inout_ PULONGLONG pululEnumerator);

WINBASEAPI
BOOL WINAPI GetUILanguageInfo(_In_ DWORD dwFlags, _In_ PCZZWSTR pwmszLanguage,
                              _Out_writes_opt_(*pcchFallbackLanguages)
...
WINBASEAPI
BOOL WINAPI SetThreadPreferredUILanguages2(
    _In_ ULONG flags, _In_opt_ PCZZWSTR languages,
    _Out_opt_ PULONG numLanguagesSet, _Out_opt_ HSAVEDUILANGUAGES *snapshot);
...
WINBASEAPI
BOOL WINAPI NotifyUILanguageChange(_In_ DWORD dwFlags,
...
WINBASEAPI
BOOL WINAPI GetStringTypeExA(_In_ LCID Locale, _In_ DWORD dwInfoType,
...
//
WINBASEAPI
BOOL WINAPI GetStringTypeA(_In_ LCID Locale, _In_ DWORD dwInfoType,
                           _In_reads_(cchSrc) LPCSTR lpSrcStr, _In_ int cchSrc,
...
WINBASEAPI
BOOL WINAPI EnumSystemLocalesA(_In_ LOCALE_ENUMPROCA lpLocaleEnumProc,
                               _In_ DWORD dwFlags);
// DEPRECATED, please use Locale Names and call EnumSystemLocalesEx
WINBASEAPI
BOOL WINAPI EnumSystemLocalesW(_In_ LOCALE_ENUMPROCW lpLocaleEnumProc,
                               _In_ DWORD dwFlags);
#ifdef UNICODE
...
WINBASEAPI
BOOL WINAPI
EnumSystemLanguageGroupsA(_In_ LANGUAGEGROUP_ENUMPROCA lpLanguageGroupEnumProc,
                          _In_ DWORD dwFlags, _In_ LONG_PTR lParam);
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
BOOL WINAPI
EnumSystemLanguageGroupsW(_In_ LANGUAGEGROUP_ENUMPROCW lpLanguageGroupEnumProc,
                          _In_ DWORD dwFlags, _In_ LONG_PTR lParam);
#ifdef UNICODE
...
BOOL WINAPI EnumLanguageGroupLocalesA(
    ...
    _In_ LGRPID LanguageGroup, _In_ DWORD dwFlags, _In_ LONG_PTR lParam);
DEPRECATED("The Language Group concept is obsolete and no longer supported. "
           "Set DISABLE_NLS_DEPRECATION to disable this warning.")
WINBASEAPI
...
WINBASEAPI
BOOL WINAPI EnumUILanguagesA(_In_ UILANGUAGE_ENUMPROCA lpUILanguageEnumProc,
                             _In_ DWORD dwFlags, _In_ LONG_PTR lParam);
// DEPRECATED: use the user language profile instead.
WINBASEAPI
BOOL WINAPI EnumUILanguagesW(_In_ UILANGUAGE_ENUMPROCW lpUILanguageEnumProc,
                             _In_ DWORD dwFlags, _In_ LONG_PTR lParam);
#ifdef UNICODE
...
// discouraged.
WINBASEAPI
BOOL WINAPI EnumSystemCodePagesA(_In_ CODEPAGE_ENUMPROCA lpCodePageEnumProc,
                                 _In_ DWORD dwFlags);
// Please use Unicode instead.  Use of other code pages/encodings is
// discouraged.
WINBASEAPI
BOOL WINAPI EnumSystemCodePagesW(_In_ CODEPAGE_ENUMPROCW lpCodePageEnumProc,
                                 _In_ DWORD dwFlags);
#ifdef UNICODE
...
int WINAPI IdnToUnicode(_In_ DWORD dwFlags,
                        ...
                        _In_ int cchASCIIChar,
                        _Out_writes_opt_(cchUnicodeChar)
                            LPWSTR lpUnicodeCharStr,
                        _In_ int cchUnicodeChar);

WINNORMALIZEAPI
int WINAPI IdnToNameprepUnicode(_In_ DWORD dwFlags,
                                _In_reads_(cchUnicodeChar)
                                    LPCWSTR lpUnicodeCharStr,
                                _In_ int cchUnicodeChar,
                                _Out_writes_opt_(cchNameprepChar)
...
WINBASEAPI
BOOL WINAPI
VerifyScripts(_In_ DWORD dwFlags,           // optional behavior flags
...
WINBASEAPI
int WINAPI GetCalendarInfoEx(_In_opt_ LPCWSTR lpLocaleName, _In_ CALID Calendar,
                             _In_opt_ LPCWSTR lpReserved, _In_ CALTYPE CalType,
                             _Out_writes_opt_(cchData) LPWSTR lpCalData,
                             _In_ int cchData, _Out_opt_ LPDWORD lpValue);
...
WINBASEAPI
int WINAPI GetDurationFormatEx(
    _In_opt_ LPCWSTR lpLocaleName, _In_ DWORD dwFlags,
    _In_opt_ CONST SYSTEMTIME *lpDuration, _In_ ULONGLONG ullDuration,
    _In_opt_ LPCWSTR lpFormat,
    _Out_writes_opt_(cchDuration) LPWSTR lpDurationStr, _In_ int cchDuration);
#endif
...
WINBASEAPI
int WINAPI GetNumberFormatEx(_In_opt_ LPCWSTR lpLocaleName, _In_ DWORD dwFlags,
                             _In_ LPCWSTR lpValue,
                             _In_opt_ CONST NUMBERFMTW *lpFormat,
                             _Out_writes_opt_(cchNumber) LPWSTR lpNumberStr,
                             _In_ int cchNumber);
...
WINBASEAPI
int WINAPI GetCurrencyFormatEx(_In_opt_ LPCWSTR lpLocaleName,
                               _In_ DWORD dwFlags, _In_ LPCWSTR lpValue,
                               _In_opt_ CONST CURRENCYFMTW *lpFormat,
                               _Out_writes_opt_(cchCurrency)
                                   LPWSTR lpCurrencyStr,
                               _In_ int cchCurrency);
...
WINBASEAPI
int WINAPI GetUserDefaultLocaleName(_Out_writes_(cchLocaleName)
                                        LPWSTR lpLocaleName,
...
WINBASEAPI
int WINAPI GetSystemDefaultLocaleName(_Out_writes_(cchLocaleName)
                                          LPWSTR lpLocaleName,
                                      _In_ int cchLocaleName);

WINBASEAPI
BOOL WINAPI IsNLSDefinedString(_In_ NLS_FUNCTION Function, _In_ DWORD dwFlags,
...
DWORD
WINAPI
IsValidNLSVersion(_In_ NLS_FUNCTION function, _In_opt_ LPCWSTR lpLocaleName,
...
int WINAPI FindNLSStringEx(
    ...
    _In_reads_(cchSource) LPCWSTR lpStringSource, _In_ int cchSource,
    _In_reads_(cchValue) LPCWSTR lpStringValue, _In_ int cchValue,
    _Out_opt_ LPINT pcchFound, _In_opt_ LPNLSVERSIONINFO lpVersionInformation,
    _In_opt_ LPVOID lpReserved, _In_opt_ LPARAM sortHandle);

#if (WINVER >= _WIN32_WINNT_WIN8)
_When_((dwMapFlags &
        (LCMAP_SORTKEY | LCMAP_BYTEREV | LCMAP_HASH | LCMAP_SORTHANDLE)) != 0,
       _At_((LPBYTE)lpDestStr, _Out_writes_bytes_opt_(cchDest)))
#else
_When_((dwMapFlags & (LCMAP_SORTKEY | LCMAP_BYTEREV)) != 0,
       _At_((LPBYTE)lpDestStr, _Out_writes_bytes_opt_(cchDest)))
#endif
    _When_(cchSrc != -1, _At_((WCHAR *)lpSrcStr, _Out_writes_opt_(cchSrc)))
        _When_(cchDest != -1,
               _At_((WCHAR *)lpDestStr, _Out_writes_opt_(cchDest))) WINBASEAPI
    int WINAPI
    LCMapStringEx(_In_opt_ LPCWSTR lpLocaleName, _In_ DWORD dwMapFlags,
                  _In_reads_(cchSrc) LPCWSTR lpSrcStr, _In_ int cchSrc,
                  _Out_writes_opt_(cchDest) LPWSTR lpDestStr, _In_ int cchDest,
...
WINBASEAPI
BOOL WINAPI EnumCalendarInfoExEx(_In_ CALINFO_ENUMPROCEXEX pCalInfoEnumProcExEx,
                                 _In_opt_ LPCWSTR lpLocaleName,
                                 _In_ CALID Calendar,
                                 _In_opt_ LPCWSTR lpReserved,
                                 _In_ CALTYPE CalType, _In_ LPARAM lParam);

typedef BOOL(CALLBACK *DATEFMT_ENUMPROCEXEX)(LPWSTR, CALID, LPARAM);

WINBASEAPI
BOOL WINAPI EnumDateFormatsExEx(_In_ DATEFMT_ENUMPROCEXEX lpDateFmtEnumProcExEx,
                                _In_opt_ LPCWSTR lpLocaleName,
...
WINBASEAPI
BOOL WINAPI EnumTimeFormatsEx(_In_ TIMEFMT_ENUMPROCEX lpTimeFmtEnumProcEx,
                              _In_opt_ LPCWSTR lpLocaleName, _In_ DWORD dwFlags,
                              _In_ LPARAM lParam);
...
WINBASEAPI
BOOL WINAPI EnumSystemLocalesEx(_In_ LOCALE_ENUMPROCEX lpLocaleEnumProcEx,
                                _In_ DWORD dwFlags, _In_ LPARAM lParam,
...
WINBASEAPI
int WINAPI ResolveLocaleName(_In_opt_ LPCWSTR lpNameToResolve,
                             _Out_writes_opt_(cchLocaleName)
                                 LPWSTR lpLocaleName,
                             _In_ int cchLocaleName);
