...
/// <summary>
/// Specifies the policy used by GetRecommendedRenderingMode to determine
/// whether to render glyphs in outline mode. Glyphs are rendered in outline
/// mode by default at large sizes for performance reasons, but how large (i.e.,
/// the outline threshold) depends on the quality of outline rendering. If the
/// graphics system renders anti- aliased outlines then a relatively low
/// threshold is used, but if the graphics system renders aliased outlines then
/// a much higher threshold is used.
/// </summary>
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("30572f99-dac6-41db-a16e-0486307e606a")
  ...
  /// </remarks>
  STDMETHOD(GetEudcFontCollection)(_COM_Outptr_ IDWriteFontCollection *
                                       *fontCollection,
  ...
  /// greater.</param> <param name="enhancedContrastGrayscale">The amount of
  ...
  STDMETHOD(CreateCustomRenderingParams)(
      FLOAT gamma, FLOAT enhancedContrast, FLOAT enhancedContrastGrayscale,
      FLOAT clearTypeLevel, DWRITE_PIXEL_GEOMETRY pixelGeometry,
      DWRITE_RENDERING_MODE renderingMode,
      _COM_Outptr_ IDWriteRenderingParams1 * *renderingParams) PURE;
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("a71efdb4-9fdb-4838-ad90-cfc3be8c3daf")
  ...
  STDMETHOD_(void, GetMetrics)(_Out_ DWRITE_FONT_METRICS1 * fontMetrics) PURE;
  ...
  STDMETHOD(GetGdiCompatibleMetrics)(
      ...
      _Out_ DWRITE_FONT_METRICS1 *fontMetrics) PURE;
  ...
  STDMETHOD_(void, GetCaretMetrics)(_Out_ DWRITE_CARET_METRICS * caretMetrics)
      PURE;
  ...
  STDMETHOD(GetUnicodeRanges)(
      ...
      _Out_writes_to_opt_(maxRangeCount, *actualRangeCount)
              DWRITE_UNICODE_RANGE *
          unicodeRanges,
      _Out_ UINT32 * actualRangeCount) PURE;
  ...
  /// </summary>
  /// <param name="glyphCount">Number of glyphs to retrieve advances
  /// for.</param> <param name="glyphIndices">Array of glyph id's to retrieve
  /// advances for.</param> <param name="glyphAdvances">Returned advances in
  /// font design units for
  ///     each glyph.</param>
  ...
  STDMETHOD(GetDesignGlyphAdvances)(
      ...
      _Out_writes_(glyphCount) INT32 *glyphAdvances, BOOL isSideways = FALSE)
      PURE;
  ...
  ///     (DWRITE_MEASURING_MODE_GDI_NATURAL).</param>
  ...
  /// <param name="glyphIndices">Array of glyph id's to retrieve
  /// advances.</param> <param name="glyphAdvances">Returned advances in font
  /// design units for
  ///     each glyph.</param>
  /// <remarks>
  ...
  STDMETHOD(GetGdiCompatibleGlyphAdvances)(
      ...
      _Out_writes_(glyphCount) INT32 *glyphAdvances) PURE;

  /// <summary>
  /// Retrieves the kerning pair adjustments from the font's kern table.
  /// </summary>
  /// <param name="glyphCount">Number of glyphs to retrieve adjustments
  /// for.</param> <param name="glyphIndices">Array of glyph id's to retrieve
  /// adjustments
  ///     for.</param>
  /// <param name="glyphAdvanceAdjustments">Returned advances in font design
  /// units for
  ...
  STDMETHOD(GetKerningPairAdjustments)(
      UINT32 glyphCount, _In_reads_(glyphCount) UINT16 const *glyphIndices,
      _Out_writes_(glyphCount) INT32 *glyphAdvanceAdjustments) PURE;
  ...
  /// name="renderingMode">Receives the recommended rendering mode.</param>
  /// <remarks>
  /// This method should be used to determine the actual rendering mode in cases
  /// where the rendering mode of the rendering params object is
  ...
  /// <param name="nominalGlyphIndices">Original glyph indices from
  /// cmap.</param> <param name="verticalGlyphIndices">The vertical form of
  /// glyph indices.</param> <remarks> Call GetGlyphIndices to get the nominal
  /// glyph indices, followed by calling this to remap the to the substituted
  /// forms, when the run is sideways, and the font has vertical glyph variants.
  /// </remarks>
  /// <returns>
  /// Standard HRESULT error code.
  /// </returns>
  STDMETHOD(GetVerticalGlyphVariants)(
      ...
      _Out_writes_(glyphCount) UINT16 *verticalGlyphIndices) PURE;
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("acd16696-8c14-4f5d-877e-fe3fc1d32738")
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("94413cf4-a6fc-4248-8b50-6674348fcad3")
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("80DAD800-E21F-4E83-96CE-BFCCE500DB7C")
    IDWriteTextAnalyzer1 : public IDWriteTextAnalyzer {
  /// <summary>
  /// Applies spacing between characters, properly adjusting glyph clusters
  /// and diacritics.
  /// </summary>
  /// <param name="leadingSpacing">The spacing before each character, in reading
  /// order.</param> <param name="trailingSpacing">The spacing after each
  /// character, in reading order.</param> <param name="minimumAdvanceWidth">The
  /// minimum advance of each character,
  ///     to prevent characters from becoming too thin or zero-width. This
  ///     must be zero or greater.</param>
  /// <param name="textLength">The length of the clustermap and original
  /// text.</param> <param name="glyphCount">The number of glyphs.</param>
  /// <param name="clusterMap">Mapping from character ranges to glyph
  /// ranges.</param> <param name="glyphAdvances">The advance width of each
  /// glyph.</param> <param name="glyphOffsets">The offset of the origin of each
  /// glyph.</param> <param name="glyphProperties">Properties of each glyph,
  /// from GetGlyphs.</param> <param name="modifiedGlyphAdvances">The new
  /// advance width of each glyph.</param> <param
  /// name="modifiedGlyphOffsets">The new offset of the origin of each
  /// glyph.</param> <remarks> The input and output advances/offsets are allowed
  /// to alias the same array.
  /// </remarks>
  /// <returns>
  /// Standard HRESULT error code.
  /// </returns>
  STDMETHOD(ApplyCharacterSpacing)(
      FLOAT leadingSpacing, FLOAT trailingSpacing, FLOAT minimumAdvanceWidth,
      UINT32 textLength, UINT32 glyphCount,
  ...
  /// <param name="isVertical">Whether the baseline is vertical or
  /// horizontal.</param> <param name="isSimulationAllowed">Simulate the
  /// baseline if it is missing in the font.</param> <param
  /// name="scriptAnalysis">Script analysis result from AnalyzeScript.</param>
  /// <param name="localeName">The language of the run.</param>
  /// <param name="baselineCoordinate">The baseline coordinate value in design
  /// units.</param> <param name="exists">Whether the returned baseline exists
  /// in the font.</param> <remarks> If the baseline does not exist in the font,
  /// it is not considered an error, but the function will return exists =
  /// false. You may then use heuristics to calculate the missing base, or, if
  /// the flag simulationAllowed is true, the function will compute a reasonable
  ...
  STDMETHOD(GetBaseline)(
      _In_ IDWriteFontFace * fontFace, DWRITE_BASELINE baseline,
      BOOL isVertical, BOOL isSimulationAllowed,
      DWRITE_SCRIPT_ANALYSIS scriptAnalysis, _In_opt_z_ WCHAR const *localeName,
      _Out_ INT32 *baselineCoordinate, _Out_ BOOL *exists) PURE;
  ...
  STDMETHOD(AnalyzeVerticalGlyphOrientation)(
      _In_ IDWriteTextAnalysisSource1 * analysisSource, UINT32 textPosition,
      UINT32 textLength, _In_ IDWriteTextAnalysisSink1 * analysisSink) PURE;
  ...
  /// <param name="isSideways">Whether the run's glyphs are sideways or
  /// not.</param> <param name="transform">Returned transform.</param>
  /// </remarks>
  /// <returns>
  /// Standard HRESULT error code.
  /// </returns>
  ...
  STDMETHOD(GetGlyphOrientationTransform)(
      DWRITE_GLYPH_ORIENTATION_ANGLE glyphOrientationAngle, BOOL isSideways,
  ...
  /// </returns>
  STDMETHOD(GetScriptProperties)(DWRITE_SCRIPT_ANALYSIS scriptAnalysis,
                                 _Out_ DWRITE_SCRIPT_PROPERTIES *
                                     scriptProperties) PURE;
  ...
  STDMETHOD(GetTextComplexity)(
      ...
      _In_ IDWriteFontFace *fontFace, _Out_ BOOL *isTextSimple,
  ...
  /// <param name="glyphProperties">Glyph properties produced from
  /// shaping.</param> <param name="justificationOpportunities">Receives
  /// information for the
  ...
  STDMETHOD(GetJustificationOpportunities)(
      _In_opt_ IDWriteFontFace * fontFace, FLOAT fontEmSize,
      DWRITE_SCRIPT_ANALYSIS scriptAnalysis, UINT32 textLength,
      UINT32 glyphCount, _In_reads_(textLength) WCHAR const *textString,
      _In_reads_(textLength) UINT16 const *clusterMap,
      _In_reads_(glyphCount)
          DWRITE_SHAPING_GLYPH_PROPERTIES const *glyphProperties,
      _Out_writes_(glyphCount)
          DWRITE_JUSTIFICATION_OPPORTUNITY *justificationOpportunities) PURE;
  ...
  STDMETHOD(JustifyGlyphAdvances)(
      FLOAT lineWidth, UINT32 glyphCount,
      _In_reads_(glyphCount)
  ...
  /// <param name="glyphAdvances">Original glyph advances produced from
  /// shaping.</param> <param name="justifiedGlyphAdvances">Justified glyph
  /// advances from
  ///     JustifyGlyphAdvances().</param>
  /// <param name="justifiedGlyphOffsets">Justified glyph offsets from
  ///     JustifyGlyphAdvances().</param>
  /// <param name="glyphProperties">Properties of each glyph, from
  /// GetGlyphs.</param> <param name="actualGlyphCount">The new glyph count
  /// written to the
  ...
  STDMETHOD(GetJustifiedGlyphs)(
      _In_opt_ IDWriteFontFace * fontFace, FLOAT fontEmSize,
      DWRITE_SCRIPT_ANALYSIS scriptAnalysis, UINT32 textLength,
      UINT32 glyphCount, UINT32 maxGlyphCount,
      ...
      _Out_writes_to_(maxGlyphCount, *actualGlyphCount)
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("639CFAD8-0FB4-4B21-A58A-067920120009")
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("B0D941A0-85E7-4D8B-9FD3-5CED9934482A")
  ...
  /// <param name="textLength">Number of UTF-16 units of the reported
  /// range.</param> <param name="glyphOrientationAngle">Angle of the glyphs
  /// within the text
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("9064D822-80A7-465C-A986-DF65F78B8FEB")
    IDWriteTextLayout1 : public IDWriteTextLayout {
  /// <summary>
  /// Enables/disables pair-kerning on the given range.
  /// </summary>
  /// <param name="isPairKerningEnabled">The Boolean flag indicates whether text
  /// is pair-kerned.</param> <param name="textRange">Text range to which this
  /// change applies.</param> <returns> Standard HRESULT error code.
  /// </returns>
  STDMETHOD(SetPairKerning)(BOOL isPairKerningEnabled,
  ...
  /// <param name="isPairKerningEnabled">The Boolean flag indicates whether text
  /// is pair-kerned.</param> <param name="textRange">The position range of the
  /// current format.</param> <returns> Standard HRESULT error code.
  /// </returns>
  STDMETHOD(GetPairKerning)(UINT32 currentPosition,
  ...
  /// Standard HRESULT error code.
  /// </returns>
  STDMETHOD(SetCharacterSpacing)(FLOAT leadingSpacing, FLOAT trailingSpacing,
  ...
  STDMETHOD(GetCharacterSpacing)(
      UINT32 currentPosition, _Out_ FLOAT * leadingSpacing,
      _Out_ FLOAT * trailingSpacing, _Out_ FLOAT * minimumAdvanceWidth,
      _Out_opt_ DWRITE_TEXT_RANGE *textRange = NULL) PURE;
};

/// <summary>
/// Represents the type of antialiasing to use for text when the rendering mode
/// calls for antialiasing.
/// </summary>
enum DWRITE_TEXT_ANTIALIAS_MODE {
  /// <summary>
  /// ClearType antialiasing computes coverage independently for the red, green,
  /// and blue color elements of each pixel. This allows for more detail than
  /// conventional antialiasing. However, because there is no one alpha value
  /// for each pixel, ClearType is not suitable rendering text onto a
  /// transparent intermediate bitmap.
  /// </summary>
  DWRITE_TEXT_ANTIALIAS_MODE_CLEARTYPE,

  /// <summary>
  /// Grayscale antialiasing computes one coverage value for each pixel. Because
  /// the alpha value of each pixel is well-defined, text can be rendered onto a
  /// transparent bitmap, which can then be composited with other content. Note
  /// that grayscale rendering with IDWriteBitmapRenderTarget1 uses
  /// premultiplied alpha.
  /// </summary>
...
/// </summary>
interface DWRITE_DECLARE_INTERFACE("791e8298-3ef3-4230-9880-c9bdecc42064")
  ...
  /// DWRITE_TEXT_ANTIALIAS_MODE_CLEARTYPE. An application can change the
  /// antialiasing mode by calling SetTextAntialiasMode. For example, an
  /// application might specify grayscale antialiasing when rendering text onto
  /// a transparent bitmap.
  /// </remarks>
  STDMETHOD(SetTextAntialiasMode)(DWRITE_TEXT_ANTIALIAS_MODE antialiasMode)
      PURE;
};
