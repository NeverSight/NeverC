...
const IID
    IID_IVssExamineWriterMetadata = // 902fcf7f-b7fd-42f8-81f1-b2e400b1e5bd
    {0x902fcf7f,
     0xb7fd,
     0x42f8,
...
const IID
    IID_IVssExamineWriterMetadataEx = // 0c0e5ec0-ca44-472b-b702-e652db1c0451
    {0x0c0e5ec0,
     0xca44,
     0x472b,
...
const IID
    IID_IVssExamineWriterMetadataEx2 = // ce115780-a611-431b-b57f-c38303ab6aee
    {0xce115780,
     0xa611,
     0x431b,
...
typedef struct _VSS_COMPONENTINFO {
  ...
  bool bNotifyOnBackupComplete; // whether component needs to be informed if
...
public:
  ...
  (_Out_ PVSSCOMPONENTINFO *ppInfo) = 0;

  // free component information
  STDMETHOD(FreeComponentInfo)
  (_In_ PVSSCOMPONENTINFO pInfo) = 0;

  // obtain a specific file in a file group
  STDMETHOD(GetFile)
  (_In_ UINT iFile, _Out_ IVssWMFiledesc **ppFiledesc) = 0;

  // obtain a specific physical database file for a database
  STDMETHOD(GetDatabaseFile)
  (_In_ UINT iDBFile, _Out_ IVssWMFiledesc **ppFiledesc) = 0;

  // obtain a specific physical log file for a database
  STDMETHOD(GetDatabaseLogFile)
  (_In_ UINT iDbLogFile, _Out_ IVssWMFiledesc **ppFiledesc) = 0;

  STDMETHOD(GetDependency)
  (_In_ UINT iDependency, _Out_ IVssWMDependency **ppDependency) = 0;
};

// interface to examine writer metadata
class __declspec(uuid("902fcf7f-b7fd-42f8-81f1-b2e400b1e5bd"))
IVssExamineWriterMetadata : public IUnknown {
public:
  // obtain identity of the writer
  STDMETHOD(GetIdentity)
  (_Out_ VSS_ID *pidInstance, _Out_ VSS_ID *pidWriter,
   _Out_ BSTR *pbstrWriterName, _Out_ VSS_USAGE_TYPE *pUsage,
   _Out_ VSS_SOURCE_TYPE *pSource) = 0;

  // obtain number of include files, exclude files, and components
  STDMETHOD(GetFileCounts)
  (_Out_ UINT *pcIncludeFiles, _Out_ UINT *pcExcludeFiles,
   _Out_ UINT *pcComponents) = 0;
  ...
  (_In_ UINT iComponent, _Out_ IVssWMComponent **ppComponent) = 0;

  // obtain restoration method
  STDMETHOD(GetRestoreMethod)
  (_Out_ VSS_RESTOREMETHOD_ENUM *pMethod, _Out_ BSTR *pbstrService,
   _Out_ BSTR *pbstrUserProcedure, _Out_ VSS_WRITERRESTORE_ENUM *pwriterRestore,
   _Out_ bool *pbRebootRequired, _Out_ UINT *pcMappings) = 0;

  // obtain a specific alternative location mapping
  STDMETHOD(GetAlternateLocationMapping)
  (_In_ UINT iMapping, _Out_ IVssWMFiledesc **ppFiledesc) = 0;

  // get the backup schema
  STDMETHOD(GetBackupSchema)
  (_Out_ DWORD *pdwSchemaMask) = 0;

  // obtain reference to actual XML document
  STDMETHOD(GetDocument)
  (_Out_ IXMLDOMDocument **pDoc) = 0;

  // convert document to a XML string
  STDMETHOD(SaveAsXML)
  (_In_ BSTR *pbstrXML) = 0;
...
class __declspec(uuid("0c0e5ec0-ca44-472b-b702-e652db1c0451"))
IVssExamineWriterMetadataEx : public IVssExamineWriterMetadata {
public:
  // obtain identity of the writer
  STDMETHOD(GetIdentityEx)
  (_Out_ VSS_ID *pidInstance, _Out_ VSS_ID *pidWriter,
   _Out_ BSTR *pbstrWriterName, _Out_ BSTR *pbstrInstanceName,
   _Out_ VSS_USAGE_TYPE *pUsage, _Out_ VSS_SOURCE_TYPE *pSource) = 0;
};

class __declspec(uuid("ce115780-a611-431b-b57f-c38303ab6aee"))
IVssExamineWriterMetadataEx2 : public IVssExamineWriterMetadataEx {
public:
  STDMETHOD(GetVersion)
  (_Out_ DWORD *pdwMajorVersion, _Out_ DWORD *pdwMinorVersion) = 0;

  STDMETHOD(GetExcludeFromSnapshotCount)
  (_Out_ UINT *pcExcludedFromSnapshot) = 0;
...
class __declspec(uuid("665c1d5f-c218-414d-a05d-7fef5f9d5c86"))
...
public:
  ...
  (_In_ UINT iWriter, _Out_ IVssWriterComponentsExt **ppWriter) = 0;

  // initialize and create BACKUP_COMPONENTS document
  STDMETHOD(InitializeForBackup)
  (_In_opt_ BSTR bstrXML = NULL) = 0;

  // set state describing backup
  STDMETHOD(SetBackupState)
  (_In_ bool bSelectComponents, _In_ bool bBackupBootableSystemState,
   _In_ VSS_BACKUP_TYPE backupType, _In_ bool bPartialFileSupport = false) = 0;
  ...
  (_In_ VSS_ID instanceId, _In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName) = 0;
  ...
  (_In_ UINT iWriter, _Out_ VSS_ID *pidInstance, _Out_ VSS_ID *pidWriter,
   _Out_ BSTR *pbstrWriter, _Out_ VSS_WRITER_STATE *pnStatus,
   _Out_ HRESULT *phResultFailure) = 0;

  // indicate whether backup succeeded on a component
  STDMETHOD(SetBackupSucceeded)
  (_In_ VSS_ID instanceId, _In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ bool bSucceded) = 0;

  // set backup options for the writer
  STDMETHOD(SetBackupOptions)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszBackupOptions) = 0;

  // indicate that a given component is selected to be restored
  STDMETHOD(SetSelectedForRestore)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ bool bSelectedForRestore) = 0;

  // set restore options for the writer
  STDMETHOD(SetRestoreOptions)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszRestoreOptions) = 0;

  // indicate that additional restores will follow
  STDMETHOD(SetAdditionalRestores)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ bool bAdditionalRestores) = 0;
  ...
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszPreviousBackupStamp) = 0;
  ...
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE componentType,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszPath, _In_ LPCWSTR wszFilespec, _In_ bool bRecursive,
   _In_ LPCWSTR wszDestination) = 0;

  // add a subcomponent to be restored
  STDMETHOD(AddRestoreSubcomponent)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE componentType,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszSubComponentLogicalPath, _In_ LPCWSTR wszSubComponentName,
   _In_ bool bRepair) = 0;
  ...
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ VSS_FILE_RESTORE_STATUS status) = 0;

  // add a new location target for a file to be restored
  STDMETHOD(AddNewTarget)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszPath, _In_ LPCWSTR wszFileName, _In_ bool bRecursive,
   _In_ LPCWSTR wszAlternatePath) = 0;
  ...
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ UINT iPartialFile, _In_ LPCWSTR wszRangesFile) = 0;
  ...
  (_Out_ VSS_ID *pSnapshotSetId) = 0;

  // add a volume to a snapshot set
  STDMETHOD(AddToSnapshotSet)
  (_In_z_ VSS_PWSZ pwszVolumeName, _In_ VSS_ID ProviderId,
   _Out_ VSS_ID *pidSnapshot) = 0;
  ...
  (_In_ VSS_ID SourceObjectId, _In_ VSS_OBJECT_TYPE eSourceObjectType,
   _In_ BOOL bForceDelete, _In_ LONG *plDeletedSnapshots,
   _In_ VSS_ID *pNondeletedSnapshotID) = 0;
  ...
  (_In_ VSS_ID SnapshotId, _Out_ VSS_SNAPSHOT_PROP *pProp) = 0;

  STDMETHOD(Query)
  (_In_ VSS_ID QueriedObjectId, _In_ VSS_OBJECT_TYPE eQueriedObjectType,
   _In_ VSS_OBJECT_TYPE eReturnedObjectsType, _In_ IVssEnumObject **ppEnum) = 0;

  STDMETHOD(IsVolumeSupported)
  (_In_ VSS_ID ProviderId, _In_z_ VSS_PWSZ pwszVolumeName,
   _In_ BOOL *pbSupportedByThisProvider) = 0;

  STDMETHOD(DisableWriterClasses)
  (_In_ const VSS_ID *rgWriterClassId, _In_ UINT cClassId) = 0;

  STDMETHOD(EnableWriterClasses)
  (_In_ const VSS_ID *rgWriterClassId, _In_ UINT cClassId) = 0;

  STDMETHOD(DisableWriterInstances)
  (_In_ const VSS_ID *rgWriterInstanceId, _In_ UINT cInstanceId) = 0;

  // called to expose a snapshot
  STDMETHOD(ExposeSnapshot)
  (_In_ VSS_ID SnapshotId, _In_z_ VSS_PWSZ wszPathFromRoot,
   _In_ LONG lAttributes, _In_z_ VSS_PWSZ wszExpose,
   _Outptr_result_z_ VSS_PWSZ *pwszExposed) = 0;

  STDMETHOD(RevertToSnapshot)
  (_In_ VSS_ID SnapshotId, _In_ BOOL bForceDismount) = 0;

  STDMETHOD(QueryRevertStatus)
  (_In_z_ VSS_PWSZ pwszVolume, _Out_ IVssAsync **ppAsync) = 0;
};

class __declspec(uuid("963f03ad-9e4c-4a34-ac15-e4b6174e5036"))
IVssBackupComponentsEx : public IVssBackupComponents {
public:
  // get writer metadata for a specific writer
  STDMETHOD(GetWriterMetadataEx)
  (_In_ UINT iWriter, _Out_ VSS_ID *pidInstance,
   _Out_ IVssExamineWriterMetadataEx **ppMetadata) = 0;

  // indicate that a given component is selected to be restored
  STDMETHOD(SetSelectedForRestoreEx)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ bool bSelectedForRestore, _In_ VSS_ID instanceId = GUID_NULL) = 0;
};

class __declspec(uuid("acfe2b3a-22c9-4ef8-bd03-2f9ca230084e"))
...
public:
  ...
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ VSS_ROLLFORWARD_TYPE rollType, _In_ LPCWSTR wszRollForwardPoint) = 0;

  STDMETHOD(SetRestoreName)
  (_In_ VSS_ID writerId, _In_ VSS_COMPONENT_TYPE ct,
   _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszComponentName,
   _In_ LPCWSTR wszRestoreName) = 0;

  STDMETHOD(BreakSnapshotSetEx)
  (_In_ VSS_ID SnapshotSetID, _In_ DWORD dwBreakFlags,
   _Out_ IVssAsync **ppAsync) = 0;

  STDMETHOD(PreFastRecovery)
  (_In_ VSS_ID SnapshotSetID, _In_ DWORD dwPreFastRecoveryFlags,
   _Out_ IVssAsync **ppAsync) = 0;

  STDMETHOD(FastRecovery)
  (_In_ VSS_ID SnapshotSetID, _In_ DWORD dwFastRecoveryFlags,
   _Out_ IVssAsync **ppAsync) = 0;
};

class __declspec(uuid("c191bfbc-b602-4675-8bd1-67d642f529d5"))
IVssBackupComponentsEx3 : public IVssBackupComponentsEx2 {
public:
  STDMETHOD(GetWriterStatusEx)
  (_In_ UINT iWriter, _Out_ VSS_ID *pidInstance, _Out_ VSS_ID *pidWriter,
   _Out_ BSTR *pbstrWriter, _Out_ VSS_WRITER_STATE *pnStatus,
   _Out_ HRESULT *phrFailureWriter, _Out_opt_ HRESULT *phrApplication = NULL,
   _Out_opt_ BSTR *pbstrApplicationMessage = NULL) = 0;

  STDMETHOD(AddSnapshotToRecoverySet)
  (_In_ VSS_ID snapshotId, _In_ DWORD dwFlags,
   _In_opt_ VSS_PWSZ pwszDestinationVolume = NULL) = 0;

  STDMETHOD(RecoverSet)
  (_In_ DWORD dwFlags, _Outptr_ IVssAsync **ppAsync) = 0;

  STDMETHOD(GetSessionId)
  (_Out_ VSS_ID *idSession) = 0;
};

class __declspec(uuid("f434c2fd-b553-4961-a9f9-a8e90b673e53"))
IVssBackupComponentsEx4 : public IVssBackupComponentsEx3 {
public:
  STDMETHOD(GetRootAndLogicalPrefixPaths)
  (_In_ VSS_PWSZ pwszFilePath, _Outptr_ VSS_PWSZ *ppwszRootPath,
   _Outptr_ VSS_PWSZ *ppwszLogicalPrefix,
   _In_opt_ BOOL bNormalizeFQDNforRootPath = FALSE) = 0;
};
...
CreateVssBackupComponentsInternal(_Out_ IVssBackupComponents **ppBackup);

HRESULT STDAPICALLTYPE CreateVssExamineWriterMetadataInternal(
    _In_ BSTR bstrXML, _Out_ IVssExamineWriterMetadata **ppMetadata);

HRESULT APIENTRY IsVolumeSnapshottedInternal(_In_z_ VSS_PWSZ pwszVolumeName,
...
void APIENTRY VssFreeSnapshotPropertiesInternal(_In_ VSS_SNAPSHOT_PROP *pProp);

HRESULT APIENTRY GetProviderMgmtInterfaceInternal(_In_ VSS_ID ProviderId,
                                                  _In_ IID InterfaceId,
                                                  _Out_ IUnknown **ppItf);

HRESULT APIENTRY ShouldBlockRevertInternal(_In_ LPCWSTR wszVolumeName,
                                           _Out_ bool *pbBlock);
}

inline HRESULT
CreateVssBackupComponents(_Out_ IVssBackupComponents **ppBackup) {
  return CreateVssBackupComponentsInternal(ppBackup);
}

inline HRESULT
CreateVssExamineWriterMetadata(_In_ BSTR bstrXML,
                               _Out_ IVssExamineWriterMetadata **ppMetadata) {
  return CreateVssExamineWriterMetadataInternal(bstrXML, ppMetadata);
}

inline HRESULT IsVolumeSnapshotted(_In_z_ VSS_PWSZ pwszVolumeName,
                                   _Out_ BOOL *pbSnapshotsPresent,
                                   _Out_ LONG *plSnapshotCapability) {
...
inline void VssFreeSnapshotProperties(_In_ VSS_SNAPSHOT_PROP *pProp) {
  return VssFreeSnapshotPropertiesInternal(pProp);
}

inline HRESULT GetProviderMgmtInterface(_In_ VSS_ID ProviderId,
                                        _In_ IID InterfaceId,
                                        _Out_ IUnknown **ppItf) {
  return GetProviderMgmtInterfaceInternal(ProviderId, InterfaceId, ppItf);
}

inline HRESULT ShouldBlockRevert(_In_ LPCWSTR wszVolumeName,
                                 _Out_ bool *pbBlock) {
  return ShouldBlockRevertInternal(wszVolumeName, pbBlock);
}

#endif /* WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP |                   \
          WINAPI_PARTITION_PKG_VSS) */
#pragma endregion
