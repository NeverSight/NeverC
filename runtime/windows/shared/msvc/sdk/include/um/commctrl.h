...
#define ICC_DATE_CLASSES                                                       \
  0x00000100 // month picker, date picker, time picker, updown
...
WINCOMMCTRLAPI
BOOL WINAPI InitCommonControlsEx(_In_ const INITCOMMONCONTROLSEX *picce);
...
#define NM_TVSTATEIMAGECHANGING                                                \
  (NM_FIRST - 24) // uses NMTVSTATEIMAGECHANGING struct, defined after HTREEITEM
#endif
...
typedef struct tagNMMOUSE {
  ...
  POINT pt;
  LPARAM dwHitInfo; // any specifics about where on the item or control the
                    // mouse is
} NMMOUSE, *LPNMMOUSE;
...
#define CDRF_NOTIFYSUBITEMDRAW                                                 \
  0x00000020 // flags are the same, we can distinguish by context
...
typedef struct tagNMCUSTOMDRAWINFO {
  ...
  HDC hdc;
  RECT rc;
  DWORD_PTR dwItemSpec; // this is control specific, but it's how to specify an
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
#define ILC_MIRROR                                                             \
  0x00002000 // Mirror the icons contained, if the process is mirrored
#define ILC_PERITEMMIRROR                                                      \
  0x00008000 // Causes the mirroring code to mirror each item when inserting a
             // set of images, verses the whole strip
#endif
#if (NTDDI_VERSION >= NTDDI_VISTA)
#define ILC_ORIGINALSIZE                                                       \
  0x00010000 // Imagelist should accept smaller than set images and apply
             // OriginalSize based on image added
#define ILC_HIGHQUALITYSCALE                                                   \
  0x00020000 // Imagelist should enable use of the high quality scaler.
#endif
...
#define ILD_SCALE                                                              \
  0x00002000 // Causes the image to be scaled to cx, cy instead of clipped
...
WINCOMMCTRLAPI BOOL WINAPI ImageList_Draw(_In_ HIMAGELIST himl, _In_ int i,
                                          _In_ HDC hdcDst, _In_ int x,
                                          _In_ int y, _In_ UINT fStyle);
...
WINCOMMCTRLAPI BOOL WINAPI ImageList_Replace(_In_ HIMAGELIST himl, _In_ int i,
                                             _In_ HBITMAP hbmImage,
                                             _In_opt_ HBITMAP hbmMask);
...
WINCOMMCTRLAPI BOOL WINAPI ImageList_DrawEx(_In_ HIMAGELIST himl, _In_ int i,
                                            _In_ HDC hdcDst, _In_ int x,
                                            _In_ int y, _In_ int dx,
                                            _In_ int dy, _In_ COLORREF rgbBk,
                                            _In_ COLORREF rgbFg,
                                            _In_ UINT fStyle);
WINCOMMCTRLAPI BOOL WINAPI
ImageList_DrawIndirect(_In_ IMAGELISTDRAWPARAMS *pimldp);
WINCOMMCTRLAPI BOOL WINAPI ImageList_Remove(_In_ HIMAGELIST himl, _In_ int i);
WINCOMMCTRLAPI HICON WINAPI ImageList_GetIcon(_In_ HIMAGELIST himl, _In_ int i,
                                              _In_ UINT flags);
...
WINCOMMCTRLAPI BOOL WINAPI ImageList_Copy(_In_ HIMAGELIST himlDst,
                                          ...
                                          _In_ int iSrc, _In_ UINT uFlags);
...
WINCOMMCTRLAPI BOOL WINAPI
ImageList_SetDragCursorImage(_In_ HIMAGELIST himlDrag, _In_ int iDrag,
                             _In_ int dxHotspot, _In_ int dyHotspot);

WINCOMMCTRLAPI BOOL WINAPI ImageList_DragShowNolock(BOOL fShow);
_Success_(return != NULL) WINCOMMCTRLAPI HIMAGELIST WINAPI
    ImageList_GetDragImage(_Out_opt_ POINT *ppt, _Out_opt_ POINT *pptHotspot);
...
WINCOMMCTRLAPI BOOL WINAPI ImageList_Write(_In_ HIMAGELIST himl,
                                           _In_ struct IStream *pstm);

#if (NTDDI_VERSION >= NTDDI_WINXP)
#define ILP_NORMAL                                                             \
  0 // Writes or reads the stream using new sematics for this version of
    // comctl32
...
_Success_(return) WINCOMMCTRLAPI BOOL WINAPI
    ImageList_GetIconSize(_In_ HIMAGELIST himl, _Out_opt_ int *cx,
                          _Out_opt_ int *cy);
_Success_(return) WINCOMMCTRLAPI BOOL WINAPI
    ImageList_SetIconSize(_In_ HIMAGELIST himl, _In_ int cx, _In_ int cy);
_Success_(return) WINCOMMCTRLAPI BOOL WINAPI
    ImageList_GetImageInfo(_In_ HIMAGELIST himl, _In_ int i,
                           _Out_ IMAGEINFO *pImageInfo);
WINCOMMCTRLAPI HIMAGELIST WINAPI ImageList_Merge(_In_ HIMAGELIST himl1,
                                                 _In_ int i1,
                                                 _In_ HIMAGELIST himl2,
                                                 _In_ int i2, _In_ int dx,
                                                 _In_ int dy);
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
WINCOMMCTRLAPI HRESULT WINAPI HIMAGELIST_QueryInterface(_In_ HIMAGELIST himl,
                                                        _In_ REFIID riid,
                                                        _Outptr_ void **ppv);
...
#define HDF_FIXEDWIDTH                                                         \
  0x0100 // Can't resize the column; same as LVCFMT_FIXED_WIDTH
#define HDF_SPLITBUTTON                                                        \
  0x1000000 // Column is a split button; same as LVCFMT_SPLITBUTTON
#endif
...
#define Header_GetItem(hwndHD, i, phdi)                                        \
  (BOOL) SNDMSG((hwndHD), HDM_GETITEM, (WPARAM)(int)(i),                       \
                (LPARAM)(HD_ITEM *)(phdi))
...
#define HDM_GETITEMDROPDOWNRECT                                                \
  (HDM_FIRST + 25) // rect of item's drop down button
...
WINCOMMCTRLAPI HBITMAP WINAPI CreateMappedBitmap(HINSTANCE hInstance,
                                                 ...
                                                 _In_opt_ LPCOLORMAP lpColorMap,
                                                 int iNumMaps);
...
#define TBSTYLE_CHECKGROUP                                                     \
  (TBSTYLE_GROUP | TBSTYLE_CHECK) // obsolete; use BTNS_CHECKGROUP instead
...
#define BTNS_NOPREFIX                                                          \
  TBSTYLE_NOPREFIX           // 0x0020; this button should not have accel prefix
...
#define TBCDRF_HILITEHOTTRACK                                                  \
  0x00020000                       // Use color of the button bk when hottracked
#define TBCDRF_NOOFFSET 0x00040000 // Don't offset button if pressed
#define TBCDRF_NOMARK                                                          \
  0x00080000 // Don't draw default highlight of image/text for TBSTATE_MARKED
#define TBCDRF_NOETCHEDEFFECT                                                  \
  0x00100000 // Don't draw etched effect for disabled items
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
#define TBCDRF_USECDCOLORS                                                     \
  0x00800000 // Use CustomDrawColors to RenderText regardless of VisualStyle
#endif
...
#define TB_INSERTMARKHITTEST                                                   \
  (WM_USER + 81) // wParam == LPPOINT lParam == LPTBINSERTMARK
...
#define TB_GETCOLORSCHEME                                                      \
  CCM_GETCOLORSCHEME // fills in COLORSCHEME pointed to by lParam
...
#define TBIF_BYINDEX                                                           \
  0x80000000 // this specifies that the wparam in Get/SetButtonInfo is an index,
             // not id
...
#define TB_SETDRAWTEXTFLAGS                                                    \
  (WM_USER + 70) // wParam == mask lParam == bit values
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
#define TB_GETITEMDROPDOWNRECT                                                 \
  (WM_USER + 103) // Rect of item's drop down button
...
#define TBN_DRAGOUT                                                            \
  (TBN_FIRST - 14) // this is sent when the user clicks down on a button then
                   // drags off the button
#define TBN_DELETINGBUTTON (TBN_FIRST - 15) // uses TBNOTIFY
#define TBN_GETDISPINFOA                                                       \
  (TBN_FIRST -                                                                 \
   16) // This is sent when the  toolbar needs  some display information
#define TBN_GETDISPINFOW                                                       \
  (TBN_FIRST -                                                                 \
   17) // This is sent when the  toolbar needs  some display information
...
typedef struct {
  NMHDR hdr;
  DWORD dwMask;     // [in] Specifies the values requested .[out] Client ask the
                    // data to be set for future use
...
typedef struct {
  NMHDR hdr;
  DWORD dwMask; //[in] Specifies the values requested .[out] Client ask the data
                // to be set for future use
  int idCommand;    // [in] id of button we're requesting info for
  DWORD_PTR lParam; // [in] lParam of button
  int iImage;       // [out] image index
...
#define RBS_VERTICALGRIPPER                                                    \
  0x00004000 // this always has the vertical gripper (default for horizontal
             // mode)
...
#define RBBS_VARIABLEHEIGHT                                                    \
  0x00000040 // allow autosizing of this child vertically
...
#define RBBS_USECHEVRON                                                        \
  0x00000200 // display drop-down button for this band if it's sized smaller
             // than ideal width
...
#define RB_SIZETORECT                                                          \
  (WM_USER + 23) // resize the rebar/break bands and such to this rect (lparam)
...
#define RB_GETBANDBORDERS                                                      \
  (WM_USER +                                                                   \
   34) // returns in lparam = lprc the amount of edges added to band wparam
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
#define RBAB_AUTOSIZE                                                          \
  0x0001 // These are not flags and are all mutually exclusive
...
#define TTM_RELAYEVENT                                                         \
  (WM_USER +                                                                   \
   7) // Win7: wParam = GetMessageExtraInfo() when relaying WM_MOUSEMOVE
...
#define TTM_TRACKACTIVATE                                                      \
  (WM_USER + 17) // wParam = TRUE/FALSE start end  lparam = LPTOOLINFO
...
WINCOMMCTRLAPI void
    WINAPI MenuHelp(UINT uMsg, WPARAM wParam, LPARAM lParam, HMENU hMainMenu,
                    HINSTANCE hInst, HWND hwndStatus,
                    _In_reads_(_Inexpressible_(2 + 2n && n >= 1)) UINT *lpwIDs);
WINCOMMCTRLAPI BOOL WINAPI ShowHideMenuCtl(_In_ HWND hWnd, _In_ UINT_PTR uFlags,
                                           _In_z_ LPINT lpInfo);
...
#define TBS_REVERSED                                                           \
  0x0200 // Accessibility hint: the smaller number (usually the min value) means
         // "high" and the larger number (usually the max value) means "low"

#define TBS_DOWNISLEFT                                                         \
  0x0400 // Down=Left and Up=Right (default is Down=Right and Up=Left)

#if (_WIN32_IE >= 0x0600)
#define TBS_NOTIFYBEFOREMOVE                                                   \
  0x0800 // Trackbar should notify parent before repositioning the slider due to
         // user action (enables snapping)
#endif

#if (NTDDI_VERSION >= NTDDI_VISTA)
#define TBS_TRANSPARENTBKGND                                                   \
  0x1000 // Background is painted by the parent via WM_PRINTCLIENT
#endif
...
#define PBM_GETRANGE                                                           \
  (WM_USER +                                                                   \
   7) // wParam = return (TRUE ? low : high). lParam = PPBRANGE or NULL
...
#define PBM_SETSTATE                                                           \
  (WM_USER + 16) // wParam = PBST_[State] (NORMAL, ERROR, PAUSED)
...
//  LinkWindow messages
#define LM_HITTEST                                                             \
  (WM_USER + 0x300) // wParam: n/a, lparam: PLHITTESTINFO, ret: BOOL
#define LM_GETIDEALHEIGHT                                                      \
  (WM_USER + 0x301)                  // wParam: cxMaxWidth, lparam: n/a, ret: cy
...
#define LM_GETIDEALSIZE                                                        \
  (LM_GETIDEALHEIGHT) // wParam: cxMaxWidth, lparam: SIZE*, ret: cy
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
#define LVIF_COLFMT                                                            \
  0x00010000 // The piColFmt member is valid in addition to puColumns
#endif
...
#if (NTDDI_VERSION >=                                                          \
     NTDDI_VISTA) // Will be unused downlevel, but sizeof(LVITEMA) must be equal
                  // to sizeof(LVITEMW)
...
#define ListView_HitTestEx(hwndLV, pinfo)                                      \
  (int)SNDMSG((hwndLV), LVM_HITTEST, (WPARAM) - 1,                             \
              (LPARAM)(LV_HITTESTINFO *)(pinfo))
...
typedef struct tagLVCOLUMNA {
  ...
  int cxIdeal;   // read only. ideal may not eqaul current width if auto sized
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
#define LVCFMT_FIXED_WIDTH                                                     \
  0x00100 // Can't resize the column; same as HDF_FIXEDWIDTH
#define LVCFMT_NO_DPI_SCALE                                                    \
  0x40000 // If not set, CCM_DPISCALE will govern scaling up fixed width
#define LVCFMT_FIXED_RATIO 0x80000 // Width will augment with the row height

// The following flags
#define LVCFMT_LINE_BREAK                                                      \
  0x100000 // Move to the top of the next list of columns
#define LVCFMT_FILL                                                            \
  0x200000 // Fill the remainder of the tile area. Might have a title.
...
#define LVCFMT_SPLITBUTTON                                                     \
  0x1000000 // Column is a split button; same as HDF_SPLITBUTTON
#endif
...
#define ListView_SetColumn(hwnd, iCol, pcol)                                   \
  (BOOL) SNDMSG((hwnd), LVM_SETCOLUMN, (WPARAM)(int)(iCol),                    \
...
#define LVS_EX_LABELTIP                                                        \
  0x00004000 // listview unfolds partly hidden labels if it does not have
             // infotip text
#define LVS_EX_BORDERSELECT                                                    \
  0x00008000 // border selection style instead of highlight
#if (NTDDI_VERSION >= NTDDI_WINXP)
...
#define LVS_EX_SIMPLESELECT                                                    \
  0x00100000 // Also changes overlay rendering to top right for icon mode.
#endif
#if (NTDDI_VERSION >= NTDDI_VISTA)
#define LVS_EX_JUSTIFYCOLUMNS                                                  \
  0x00200000 // Icons are lined up in columns that use up the whole view area.
#define LVS_EX_TRANSPARENTBKGND                                                \
  0x00400000 // Background is painted by the parent via WM_PRINTCLIENT
#define LVS_EX_TRANSPARENTSHADOWTEXT                                           \
  0x00800000 // Enable shadow text on transparent backgrounds only (useful with
             // bitmaps)
#define LVS_EX_AUTOAUTOARRANGE                                                 \
  0x01000000 // Icons automatically arrange if no icon positions have been set
#define LVS_EX_HEADERINALLVIEWS                                                \
  0x02000000 // Display column header in all view modes
...
#define ListView_ApproximateViewRect(hwnd, iWidth, iHeight, iCount)            \
  (DWORD) SNDMSG((hwnd), LVM_APPROXIMATEVIEWRECT, (WPARAM)(iCount),            \
...
#define LVGF_SUBSETITEMS                                                       \
  0x00010000 // readonly, cItems holds count of items in visible subset,
             // iFirstItem is valid
#endif
...
// repositioned within listview. For example: Many developers use the lParam to
// store an identifier that is unique. Unfortunatly, in order to find this item,
// they have to iterate through all of the items in the listview. Listview will
// maintain a unique identifier.  The upper bound is the size of a DWORD.
...
#define ListView_GetNextItemIndex(hwnd, plvii, flags)                          \
  (BOOL) SNDMSG((hwnd), LVM_GETNEXTITEMINDEX, (WPARAM)(LVITEMINDEX *)(plvii),  \
                MAKELPARAM((flags), 0))
...
typedef struct tagNMLVCUSTOMDRAW {
  ...
  // Group Custom Draw
  RECT rcText;
  UINT uAlign; // Alignment. Use LVGA_HEADER_CENTER, LVGA_HEADER_RIGHT,
               // LVGA_HEADER_LEFT
#endif
...
#define TreeView_GetItemRect(hwnd, hitem, prc, code)                           \
  (*(HTREEITEM *)(prc) = (hitem),                                              \
   (BOOL)SNDMSG((hwnd), TVM_GETITEMRECT, (WPARAM)(code),                       \
                (LPARAM)(RECT *)(prc)))
...
// treeview's customdraw return meaning don't draw images.  valid on
// CDRF_NOTIFYITEMPREPAINT
...
#define CBEM_SETEXTENDEDSTYLE                                                  \
  (WM_USER + 14) // lparam == new style, wParam (optional) == mask
...
// iNewSelection gives what would be the new selection unless the notify is
// failed
...
#define TabCtrl_GetItem(hwnd, iItem, pitem)                                    \
  (BOOL) SNDMSG((hwnd), TCM_GETITEM, (WPARAM)(int)(iItem),                     \
...
#define TabCtrl_SetItem(hwnd, iItem, pitem)                                    \
  (BOOL) SNDMSG((hwnd), TCM_SETITEM, (WPARAM)(int)(iItem),                     \
                (LPARAM)(TC_ITEM *)(pitem))
...
#define TabCtrl_AdjustRect(hwnd, bLarger, prc)                                 \
  (int)SNDMSG(hwnd, TCM_ADJUSTRECT, (WPARAM)(BOOL)(bLarger),                   \
              (LPARAM)(RECT *)(prc))
...
#define TabCtrl_HighlightItem(hwnd, i, fHighlight)                             \
  (BOOL) SNDMSG((hwnd), TCM_HIGHLIGHTITEM, (WPARAM)(i),                        \
...
//   To display two months, undo the AdjustWindowRect calculation already done
//   to this rect, double the width, and redo the AdjustWindowRect calculation
//   -- the monthcal control will display two calendars in this window (if you
//   also double the vertical size, you will get 4 calendars) NOTE: if you want
//   to gurantee that the "Today" string is not clipped, get the
//   MCM_GETMAXTODAYWIDTH and use the max of that width and this width
...
//   modifies rgst[0] to be the minimum ALLOWABLE systemtime (or 0 if no
//   minimum) modifies rgst[1] to be the maximum ALLOWABLE systemtime (or 0 if
//   no maximum) returns GDTR_MIN|GDTR_MAX if there is a minimum|maximum limit
...
//   if GDTR_MIN, sets the minimum ALLOWABLE systemtime to rgst[0], otherwise
//   removes minimum if GDTR_MAX, sets the maximum ALLOWABLE systemtime to
//   rgst[1], otherwise removes maximum returns TRUE on success, FALSE on error
...
// Note: iRow of -1 refers to the row header and iCol of -1 refers to the col
// header.
...
#define MonthCal_GetCalendarGridInfo(hmc, pmcGridInfo)                         \
  (BOOL) SNDMSG(hmc, MCM_GETCALENDARGRIDINFO, 0,                               \
...
//   GetTimeFormat for valid formatting chars) NOTE: 'X' is a valid formatting
//   character which indicates that the application will determine how to
//   display information. Such apps must support DTN_WMKEYDOWN, DTN_FORMAT, and
//   DTN_FORMATQUERY.
...
#define DTS_SHORTDATEFORMAT                                                    \
  0x0000 // use the short date format (app must forward WM_WININICHANGE
         // messages)
#define DTS_LONGDATEFORMAT                                                     \
  0x0004 // use the long date format (app must forward WM_WININICHANGE messages)
#define DTS_SHORTDATECENTURYFORMAT                                             \
  0x000C // short date format with century (app must forward WM_WININICHANGE
         // messages)
#define DTS_TIMEFORMAT                                                         \
  0x0009 // use the time format (app must forward WM_WININICHANGE messages)
#define DTS_APPCANPARSE                                                        \
  0x0010 // allow user entered strings (app MUST respond to DTN_USERSTRING)
...
#define DTN_WMKEYDOWNA                                                         \
  (DTN_FIRST2 - 4) // modify keydown on app format field (X), , -757
...
#define DTN_FORMATA                                                            \
  (DTN_FIRST2 - 3) // query display for app format field (X), -756
...
#define DTN_FORMATQUERYA                                                       \
  (DTN_FIRST2 - 2) // query formatting info for app format field (X), -755
...
#define IPM_GETADDRESS                                                         \
  (WM_USER + 102) // lresult = # of non black fields.  lparam = LPDWORD for
                  // TCP/IP address
...
typedef struct {
  HIMAGELIST himl; // Images: Normal, Hot, Pushed, Disabled. If count is less
                   // than 4, we use index 1
...
#if (NTDDI_VERSION >= NTDDI_WIN10_RS5)
// Edit Control Extended Styles to use with
// EM_SETEXTENDEDSTYLE/EM_GETEXTENDEDSTYLE
...
#define EM_SETCUEBANNER                                                        \
  (ECM_FIRST + 1) // Set the cue banner with the lParm = LPCWSTR
...
#define EM_GETCUEBANNER                                                        \
  (ECM_FIRST + 2) // Set the cue banner with the lParm = LPCWSTR
...
#define EM_SHOWBALLOONTIP                                                      \
  (ECM_FIRST + 3) // Show a balloon tip associated to the edit control
#define Edit_ShowBalloonTip(hwnd, peditballoontip)                             \
  (BOOL) SNDMSG((hwnd), EM_SHOWBALLOONTIP, 0, (LPARAM)(peditballoontip))
#define EM_HIDEBALLOONTIP                                                      \
  (ECM_FIRST + 4) // Hide any balloon tip associated with the edit control
...
#define Edit_NoSetFocus(hwndCtl)                                               \
  ((DWORD)SNDMSG((hwndCtl), EM_NOSETFOCUS, 0L, 0L))
...
#define Edit_SearchWeb(hwndCtl) (BOOL) SNDMSG((hwndCtl), EM_SEARCHWEB, 0, 0)
...
#define Edit_GetZoom(hwndCtl, numerator, denominator)                          \
  (BOOL) SNDMSG((hwndCtl), EM_GETZOOM, (WPARAM)(numerator),                    \
                (LPARAM)(denominator))
#define EM_SETZOOM (WM_USER + 225)
#define Edit_SetZoom(hwndCtl, numerator, denominator)                          \
  (BOOL) SNDMSG((hwndCtl), EM_SETZOOM, (WPARAM)(numerator),                    \
                (LPARAM)(denominator))
...
#define Edit_GetFileLine(hwndCtl, lineNumber, textBuffer)                      \
  (DWORD) SNDMSG((hwndCtl), EM_GETFILELINE, (WPARAM)(lineNumber),              \
                 (LPARAM)(textBuffer))
...
typedef HRESULT(CALLBACK *PFTASKDIALOGCALLBACK)(_In_ HWND hwnd, _In_ UINT msg,
                                                _In_ WPARAM wParam,
                                                _In_ LPARAM lParam,
...
enum _TASKDIALOG_FLAGS {
  ...
  TDF_NO_SET_FOREGROUND =
      0x00010000, // Don't call SetForegroundWindow() when activating the dialog
#endif            // (NTDDI_VERSION >= NTDDI_WIN8)
  TDF_SIZE_TO_CONTENT = 0x01000000 // used by ShellMessageBox to emulate
                                   // MessageBox sizing behavior
};
...
typedef enum _TASKDIALOG_MESSAGES {
  ...
  TDM_SET_MARQUEE_PROGRESS_BAR =
      WM_USER + 103, // wParam = 0 (nonMarque) wParam != 0 (Marquee)
  TDM_SET_PROGRESS_BAR_STATE = WM_USER + 104, // wParam = new progress state
  TDM_SET_PROGRESS_BAR_RANGE =
      WM_USER + 105, // lParam = MAKELPARAM(nMinRange, nMaxRange)
  TDM_SET_PROGRESS_BAR_POS = WM_USER + 106, // wParam = new position
  TDM_SET_PROGRESS_BAR_MARQUEE =
      WM_USER + 107, // wParam = 0 (stop marquee), wParam != 0 (start marquee),
                     // lparam = speed (milliseconds between repaints)
  TDM_SET_ELEMENT_TEXT =
      WM_USER + 108, // wParam = element (TASKDIALOG_ELEMENTS), lParam = new
                     // element text (LPCWSTR)
  TDM_CLICK_RADIO_BUTTON = WM_USER + 110, // wParam = Radio Button ID
  TDM_ENABLE_BUTTON =
      WM_USER +
      111, // lParam = 0 (disable), lParam != 0 (enable), wParam = Button ID
  TDM_ENABLE_RADIO_BUTTON = WM_USER + 112, // lParam = 0 (disable), lParam != 0
                                           // (enable), wParam = Radio Button ID
  TDM_CLICK_VERIFICATION =
      WM_USER +
      113, // wParam = 0 (unchecked), 1 (checked), lParam = 1 (set key focus)
  TDM_UPDATE_ELEMENT_TEXT =
      WM_USER + 114, // wParam = element (TASKDIALOG_ELEMENTS), lParam = new
                     // element text (LPCWSTR)
  TDM_SET_BUTTON_ELEVATION_REQUIRED_STATE =
      WM_USER + 115, // wParam = Button ID, lParam = 0 (elevation not required),
                     // lParam != 0 (elevation required)
  TDM_UPDATE_ICON =
      WM_USER +
      116 // wParam = icon element (TASKDIALOG_ICON_ELEMENTS), lParam = new icon
...
typedef enum _TASKDIALOG_NOTIFICATIONS {
  ...
  TDN_VERIFICATION_CLICKED = 8, // wParam = 1 if checkbox checked, 0 if not,
                                // lParam is unused and always 0
  TDN_HELP = 9,
  TDN_EXPANDO_BUTTON_CLICKED = 10 // wParam = 0 (dialog is now collapsed),
                                  // wParam != 0 (dialog is now expanded)
...
typedef int
    TASKDIALOG_COMMON_BUTTON_FLAGS; // Note: _TASKDIALOG_COMMON_BUTTON_FLAGS is
                                    // an int
...
typedef struct _TASKDIALOGCONFIG {
  ...
  dwCommonButtons;       // TASKDIALOG_COMMON_BUTTON (TDCBF_XXX) flags
  ...
  UINT cxWidth; // width of the Task Dialog's client area in DLU's. If 0, Task
                // Dialog will calculate the ideal width.
} TASKDIALOGCONFIG;

WINCOMMCTRLAPI HRESULT WINAPI TaskDialogIndirect(
    _In_ const TASKDIALOGCONFIG *pTaskConfig, _Out_opt_ int *pnButton,
    _Out_opt_ int *pnRadioButton, _Out_opt_ BOOL *pfVerificationFlagChecked);
WINCOMMCTRLAPI HRESULT WINAPI TaskDialog(
    _In_opt_ HWND hwndOwner, _In_opt_ HINSTANCE hInstance,
    _In_opt_ PCWSTR pszWindowTitle, _In_opt_ PCWSTR pszMainInstruction,
    _In_opt_ PCWSTR pszContent, TASKDIALOG_COMMON_BUTTON_FLAGS dwCommonButtons,
    _In_opt_ PCWSTR pszIcon, _Out_opt_ int *pnButton);
...
#ifdef _WIN64
WINCOMMCTRLAPI BOOL WINAPI FlatSB_GetScrollPropPtr(HWND, int propIndex,
                                                   PINT_PTR);
#else
...
WINCOMMCTRLAPI int WINAPI FlatSB_SetScrollInfo(HWND, int code, LPSCROLLINFO psi,
                                               BOOL fRedraw);

WINCOMMCTRLAPI int WINAPI FlatSB_SetScrollRange(HWND, int code, int min,
                                                int max, BOOL fRedraw);
WINCOMMCTRLAPI BOOL WINAPI FlatSB_SetScrollProp(HWND, UINT index,
                                                INT_PTR newValue, BOOL);
...
_Success_(return) BOOL WINAPI
    SetWindowSubclass(_In_ HWND hWnd, _In_ SUBCLASSPROC pfnSubclass,
                      _In_ UINT_PTR uIdSubclass, _In_ DWORD_PTR dwRefData);
_Success_(return) BOOL WINAPI
    GetWindowSubclass(_In_ HWND hWnd, _In_ SUBCLASSPROC pfnSubclass,
                      _In_ UINT_PTR uIdSubclass,
                      _Out_opt_ DWORD_PTR *pdwRefData);
_Success_(return) BOOL WINAPI
    RemoveWindowSubclass(_In_ HWND hWnd, _In_ SUBCLASSPROC pfnSubclass,
                         _In_ UINT_PTR uIdSubclass);
...
WINCOMMCTRLAPI HRESULT WINAPI LoadIconMetric(HINSTANCE hinst, PCWSTR pszName,
                                             int lims, _Out_ HICON *phico);
WINCOMMCTRLAPI HRESULT WINAPI LoadIconWithScaleDown(HINSTANCE hinst,
                                                    PCWSTR pszName, int cx,
                                                    int cy, _Out_ HICON *phico);

#endif // NTDDI_VISTA

#if (NTDDI_VERSION >= NTDDI_WINXP)

int WINAPI DrawShadowText(_In_ HDC hdc, _In_reads_(cch) LPCWSTR pszText,
                          _In_ UINT cch, _In_ RECT *prc, _In_ DWORD dwFlags,
                          _In_ COLORREF crText, _In_ COLORREF crShadow,
