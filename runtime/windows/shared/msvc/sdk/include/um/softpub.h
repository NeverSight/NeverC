...
typedef HRESULT(WINAPI *PFN_WTD_GENERIC_CHAIN_POLICY_CALLBACK)(
    IN PCRYPT_PROVIDER_DATA pProvData, IN DWORD dwStepError,
    IN DWORD dwRegPolicySettings, IN DWORD cSigner,
    IN PWTD_GENERIC_CHAIN_POLICY_SIGNER_INFO *rgpSigner, IN void *pvPolicyArg);
...
typedef struct DRIVER_VER_INFO_ {
  ...
  DRIVER_VER_MAJORMINOR
  sOSVersionHigh; // IN - OPTIONAL: highest compatible version
  ...
  //
  // NOTES:
  // 1. dwPlatform _must_ be set to a non-zero value in order for proper version
  // checking to be done.
  // 2. dwVersion is no longer used, sOSVersionLow and sOsVersionhigh have taken
  // its place
  // 3. If dwBuildNumberLow and dwBuildNumberHigh are 0, they are unused.
  // Otherwise, they are considered
  //    to be extensions of sOSVersionLow and sOSVersionHigh respectively.  Make
  //    special note of this when reading note 4.
  // 4. If you are validating against a single OS version, then set both
  // sOSVersionLow and sOSVersion high,
  //    to the version you are validating against.  If sOSVersionLow and
  //    sOSVersionHigh are different, then the validation is done for the whole
  //    version range, from sOSVersionLow to sOSVersionHigh.
  //
...
typedef _Struct_size_bytes_(cbSize) struct CONFIG_CI_PROV_INFO_RESULT2_ {
  DWORD cbSize; // IN
...
typedef _Struct_size_bytes_(cbSize) struct CONFIG_CI_PROV_INFO_ {
  DWORD cbSize;
  DWORD dwPolicies;
