...
// this is needed because some windows headers are still using deprecated string
// functions.
...
//
_Success_(return == S_OK) STDAPI
    AERT_Allocate(_In_ size_t size,
                  _Outptr_result_bytebuffer_(size) void **pMemory);
...
//  Base class for APOs
class __declspec(novtable) CBaseAudioProcessingObject
...
public:
  ...
  //
  STDMETHOD_(void,
             APOProcess)(_In_ UINT32 u32NumInputConnections,
                         _In_reads_(u32NumInputConnections)
                             APO_CONNECTION_PROPERTY **ppInputConnections,
                         _In_ UINT32 u32NumOutputConnections,
                         _Inout_updates_(u32NumOutputConnections)
  ...
  STDMETHOD(LockForProcess)(
      ...
      _In_ UINT32 u32NumOutputConnections,
      _In_reads_(u32NumOutputConnections)
          APO_CONNECTION_DESCRIPTOR **ppOutputConnections);
  STDMETHOD(UnlockForProcess)(void);
  STDMETHOD(Reset)(void);
  STDMETHOD(GetLatency)(_Out_ HNSTIME *pTime);
  STDMETHOD(GetRegistrationProperties)(
      _Outptr_opt_ APO_REG_PROPERTIES **ppRegProps);
  STDMETHOD(Initialize)(_In_ UINT32 cbDataSize,
                        _In_reads_bytes_(cbDataSize) BYTE *pbyData);
  STDMETHOD(IsInputFormatSupported)(
      _In_opt_ IAudioMediaType *pOutputFormat,
      _In_opt_ IAudioMediaType *pRequestedInputFormat,
      _Outptr_opt_ IAudioMediaType **ppSupportedInputFormat);
  STDMETHOD(IsOutputFormatSupported)(
      _In_opt_ IAudioMediaType *pInputFormat,
      _In_opt_ IAudioMediaType *pRequestedOutputFormat,
      _Outptr_opt_ IAudioMediaType **ppSupportedOutputFormat);
  STDMETHOD(GetInputChannelCount)(_Out_ UINT32 *pu32ChannelCount);

protected:
  HRESULT IsFormatTypeSupported(_In_ IAudioMediaType *pOppositeFormat,
                                _In_ IAudioMediaType *pRequestedFormat,
                                _Outptr_ IAudioMediaType **ppSupportedFormat,
                                _In_ bool bIsInput);
  HRESULT __fastcall ValidateConnection(
      _In_ const UNCOMPRESSEDAUDIOFORMAT &pUncompressedAudioFormat);
  ...
  HRESULT __fastcall ValidateInitializeParameters(_In_ UINT32 cbDataSize,
                                                  _In_reads_bytes_(cbDataSize)
                                                      BYTE *pbyData,
                                                  _In_ REFCLSID clsid,
                                                  _In_ UINT32 cbStructSize);
  virtual HRESULT __fastcall
  ValidateDefaultAPOFormat(_In_ UNCOMPRESSEDAUDIOFORMAT &audioFormat,
                           _In_ bool bIsInput);
...
protected:
  ...
  // Number of samples per frame.  Only valid if the
  // APO_FLAG_SAMPLESPERFRAME_MUST_MATCH registry property is set.
  UINT32 m_u32SamplesPerFrame;

private:
  // The following variables are stored for convenience and for validation
  // purposes.  They are all set to zero initially and any time there are no
  // valid connections attached to the APO.

  // The size of the container for each audio sample.  Only valid if the
  // APO_FLAG_BITSPERSAMPLE_MUST_MATCH registry property is set.
  UINT32 m_u32BytesPerSampleContainer;

  // Valid number of bits per sample.   Only valid if the
  // APO_FLAG_BITSPERSAMPLE_MUST_MATCH registry property is set.
  UINT32 m_u32ValidBitsPerSample;

  // Frame rate, in frames per second (hertz).  Only valid if the
  // APO_FLAG_FRAMESPERSECOND_MUST_MATCH registry property is set.
...
// Parameters:
//
//  NumAPOInterfaces - [in] The number of APO interfaces in this properties
//  class.
//
template <int NumAPOInterfaces> class CRegAPOProperties {
public:
  CRegAPOProperties(
      _In_ REFCLSID clsid, _In_z_ LPCWSTR pszFriendlyName,
      _In_z_ LPCWSTR pszCopyrightInfo, _In_ UINT32 u32MajorVersion,
      _In_ UINT32 u32MinorVersion, _In_ REFIID iidAPOInterface1,
...
#ifdef _DEBUG
    HRESULT hResult =
        StringCbCopyW(m_Properties.szFriendlyName,
                      sizeof(m_Properties.szFriendlyName), pszFriendlyName);
    _ASSERTE(S_OK == hResult); // nothing to do here except assert
    hResult =
        StringCbCopyW(m_Properties.szCopyrightInfo,
                      sizeof(m_Properties.szCopyrightInfo), pszCopyrightInfo);
    _ASSERTE(S_OK == hResult); // nothing to do here except assert
#else
    StringCbCopyW(m_Properties.szFriendlyName,
                  sizeof(m_Properties.szFriendlyName), pszFriendlyName);
  ...
  operator const APO_REG_PROPERTIES &() const { return m_Properties; }
...
#pragma warning(disable : 4200) // zero length array
  IID m_aAdditionalAPOIIDs[NumAPOInterfaces -
                           1]; // Set up the array of interface IDs this APO
                               // supports
#pragma warning(pop)
};
