...
typedef struct _WINTRUST_DATA {
  ...
  LPVOID pPolicyCallbackData; // optional: used to pass data between the app and
                              // policy
...
#define WTD_CACHE_ONLY_URL_RETRIEVAL                                           \
  0x00001000 // affects CRL retrieval and AIA retrieval
...
// Puts count of secondary signatures in
// WINTRUST_SIGNATURE_SETTINGS.cSecondarySigs
...
//----------------------------------------------------------------------------
//  Used when calling WinVerifyTrust against a PKCS7 detached signature and its
//  associated content
//
//
...
typedef struct WINTRUST_CATALOG_INFO_ {
  ...
  _Field_size_(cbCalculatedFileHash)
      BYTE *pbCalculatedFileHash; // optional: pass in the calculated hash
  DWORD cbCalculatedFileHash; // optional: pass in the count bytes of the calc
                              // hash

  PCCTL_CONTEXT
  pcCatalogContext; // optional: pass in to use instead of CatalogFilePath.

#if (NTDDI_VERSION >= NTDDI_WIN8)
  HCATADMIN hCatAdmin; // optional for SHA-1 hashes, required for all other hash
                       // types.
#endif                 // #if (NTDDI_VERSION >= NTDDI_WIN8)
...
typedef struct WINTRUST_CERT_INFO_ {
  ...
  DWORD dwFlags;
#define WTCI_DONT_OPEN_STORES                                                  \
  0x00000001 // only open dummy "root" all other are in pahStores.
#define WTCI_OPEN_ONLY_ROOT 0x00000002
#define WTCI_USE_LOCAL_MACHINE 0x00000004 // Local machine context

  FILETIME *psftVerifyAsOf; // if not null, each cert will be validated as of
                            // this time.
...
// This API returns a Win32 error code, previously it was declared to return
// HRESULT. Do not test the result with SUCCEEDED() or FAILED(), any non-zero
// value represents verification failure. To avoid this problem use
// WinVerifyTrust() instead.
extern long WINAPI WinVerifyTrustEx(HWND hwnd, GUID *pgActionID,
...
#define WTPF_OFFLINEOKNBU_IND                                                  \
  0x00001000 // off-line is ok individual certs, no bad ui
#define WTPF_OFFLINEOKNBU_COM                                                  \
  0x00002000 // off-line is ok commercial certs, no bad ui
#define WTPF_VERIFY_V1_OFF 0x00010000 // turn verify of v1 certs off
#define WTPF_IGNOREREVOCATIONONTS                                              \
  0x00020000 // ignore TimeStamp revocation checks
#define WTPF_ALLOWONLYPERTRUST                                                 \
  0x00040000 // allow only items in personal trust db.
...
typedef BOOL (*PFN_CPD_ADD_PRIVDATA)(
    _In_ struct _CRYPT_PROVIDER_DATA *pProvData,
...
typedef HRESULT (*PFN_PROVIDER_INIT_CALL)(
    _Inout_ struct _CRYPT_PROVIDER_DATA *pProvData);
...
//
//  entry point for the Policy Provider's clean up routine for any PRIVDATA
//  allocated
//
...
typedef BOOL (*PFN_PROVIDER_CERTCHKPOLICY_CALL)(
    _In_ struct _CRYPT_PROVIDER_DATA *pProvData, _In_ DWORD idxSigner,
    _In_ BOOL fCounterSignerChain, _In_opt_ DWORD idxCounterSigner);
...
#define WVT_ISINSTRUCT(structtypedef, structpassedsize, member)                \
  ((WVT_OFFSETOF(structtypedef, member) +                                      \
        sizeof(((structtypedef *)0)->member) <=                                \
    structpassedsize)                                                          \
       ? TRUE                                                                  \
       : FALSE)
...
typedef struct _CRYPT_PROVIDER_DATA {
  ...
  DWORD
  *padwTrustStepErrors; // allocated in WVT.  filled in WVT & Trust Provider

  DWORD chStores; // number of stores in pahStores (root set in WVT)
  HCERTSTORE *
      pahStores; // array of known stores (root set in WVT) root is ALWAYS #0!!!
  ...
  struct _CRYPT_PROVIDER_PRIVDATA
      *pasProvPrivData; // use Add2 and Get functions!
...
typedef struct _CRYPT_PROVIDER_SIGSTATE {
  DWORD cbStruct;
  _Field_size_(cSecondarySigs)
      HCRYPTMSG *rhSecondarySigs; // Array of handles for secondary signatures
  HCRYPTMSG hPrimarySig;          // Handle of the primary signature
  BOOL fFirstAttemptMade; // Has the first attempt to verify a signature been
                          // made
  ...
  DWORD dwCryptoPolicySupport; // Indicates what porition of a policy provider
                               // supports crypto policy
#if (NTDDI_VERSION >= NTDDI_WINBLUE)
  DWORD iAttemptCount;  // Indicates how many times through WVT loop we've gone
  BOOL fCheckedSealing; // Indicates if the first signature attempted was for a
                        // sealing signature
  struct _SEALING_SIGNATURE_ATTRIBUTE
      *pSealingSignature; // Contains the sealing signature attribute from the
                          // primary signature
#endif                    // (NTDDI_VERSION >= NTDDI_WINBLUE)
...
typedef struct _CRYPT_PROVIDER_FUNCTIONS {
  ...
  PFN_CPD_ADD_SGNR
  pfnAddSgnr2Chain; // call to add a sgnr struct to a msg struct sgnr chain
  PFN_CPD_ADD_CERT
  pfnAddCert2Chain; // call to add a cert struct to a sgnr struct cert chain
  PFN_CPD_ADD_PRIVDATA
  pfnAddPrivData2Chain; // call to add provider private data to struct.
  ...
  pfnSignatureTrust; // build info to the signing cert
  ...
  pfnCertCheckPolicy; // check each cert will building chain
  PFN_PROVIDER_TESTFINALPOLICY_CALL
  pfnTestFinalPolicy; // dump structures to a file (or whatever the policy
                      // chooses)
...
typedef BOOL (*PFN_PROVUI_CALL)(_In_ HWND hWndSecurityDialog,
                                _In_ struct _CRYPT_PROVIDER_DATA *pProvData);
...
typedef struct _CRYPT_PROVUI_DATA {
  ...
  // "Do you want to install and run ""%1"" signed on an unknown date/time and
  // distributed by:"
...
typedef struct _PROVDATA_SIP {
  ...
  struct SIP_SUBJECTINFO_
      *psSipSubjectInfo; // set in Sig Prov - defined in sipbase.h
  struct SIP_SUBJECTINFO_
      *psSipCATSubjectInfo; // set in Sig Prov - defined in sipbase.h
  struct SIP_INDIRECT_DATA_
      *psIndirectData; // set in Sig Prov - defined in sipbase.h
...
//
extern BOOL WINAPI
WintrustAddActionID(_In_ GUID *pgActionID, _In_ DWORD fdwFlags,
...
extern BOOL WINAPI WintrustAddDefaultForUsage(
    _In_ const char *pszUsageOID, _In_ CRYPT_PROVIDER_REGDEFUSAGE *psDefUsage);
...
extern BOOL
    WINAPI WintrustGetDefaultForUsage(_In_ DWORD dwAction,
                                      _In_ const char *pszUsageOID,
...
#define WIN_CERT_TYPE_X509                                                     \
  (0x0001) // bCertificate contains an X.509 Certificate
#define WIN_CERT_TYPE_PKCS_SIGNED_DATA                                         \
  (0x0002) // bCertificate contains a PKCS SignedData structure
...
//
extern BOOL WINAPI OpenPersonalTrustDBDialogEx(_In_opt_ HWND hwndParent,
                                               _In_ DWORD dwFlags,
...
extern BOOL WINAPI OpenPersonalTrustDBDialog(_In_opt_ HWND hwndParent);
