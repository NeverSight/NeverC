...
typedef VOID (*PFN_IO_COMPLETION)(IN struct FIO_CONTEXT *pContext,
                                  IN struct FH_OVERLAPPED *lpo, IN DWORD cb,
...
//
FILEHC_EXPORT
BOOL __stdcall FIOInitialize(IN DWORD dwFlags);

//
//	Terminate the DLL's support for Async IO !
//
FILEHC_EXPORT
BOOL __stdcall FIOTerminate(VOID);

//
//	Do an async read against the File !
//
FILEHC_EXPORT
BOOL __stdcall FIOReadFile(IN PFIO_CONTEXT pContext,
...
BOOL __stdcall FIOReadFileEx(
    IN PFIO_CONTEXT pContext, _In_reads_bytes_(BytesToRead) IN LPVOID lpBuffer,
    IN DWORD BytesToRead,
    IN DWORD BytesAvailable, // must be >= BytesToWrite - number of bytes I can
                             // mess with.
    _Inout_ IN struct FH_OVERLAPPED *lpo,
    IN BOOL fFinalWrite,       // Is this the final write ?
    IN BOOL fIncludeTerminator // if TRUE contains CRLF.CRLF terminator which
                               // shouldn't be stuffed
);
...
//
FILEHC_EXPORT
BOOL __stdcall FIOWriteFile(IN PFIO_CONTEXT pContext,
                            _Inout_updates_bytes_(BytesToWrite)
                                IN LPCVOID lpBuffer,
...
//
typedef HANDLE(__stdcall *FCACHE_CREATE_CALLBACK)(IN LPSTR lpstrName,
                                                  IN LPVOID lpvData,
...
//	cbFileSize - The function should return the size of the file through
// this 	cbFileSizeHigh - place to return the High DWORD of the file size
//	pfDidWeScanIt - if THIS is true then at some point the created file has
// been 		scanned for DOTs appearing at the beginning of lines
...
// pfStoredWithDots - If this is TRUE then it indicates that any DOTs that
// appear 		at the beginning of lines are stored with an extra dot
// as required in NNTP, 		SMTP and POP3 protocols.  if this is
// FALSE then the message is stored without 		DOT stuffing.
//
typedef HANDLE(__stdcall *FCACHE_RICHCREATE_CALLBACK)(
    IN LPSTR lpstrName, IN LPVOID lpvData, OUT DWORD *cbFileSize,
    OUT DWORD *cbFileSizeHigh, OUT BOOL *pfDidWeScanIt, OUT BOOL *pfIsStuffed,
    OUT BOOL *pfStoredWithDots, OUT BOOL *pfStoredWithTerminatingDot);
...
//
FILEHC_EXPORT
PFIO_CONTEXT __stdcall AssociateFile(_In_ HANDLE hFile);

//
//	This allows the user to specify whether file stores content with extra
// DOTS 	added for RFC 822 protocols (i.e. NNTP and SMTP DATA commands).
//
//	NOTE: AssociateFile() is the same as AssociateFileEx( hFile, FALSE ) ;
//
//	hFile - The file that contains message content, or in which we will
// write message content 	fStoreWithDots - if TRUE then each period or DOT
// in the file which starts a line 		but is NOT part of the
// terminating CRLF.CRLF will be stored with an extra dot 		adjacent
// to it.  This is the on the wire format for NNTP for instance.
//
FILEHC_EXPORT
PFIO_CONTEXT __stdcall AssociateFileEx(_In_ HANDLE hFile, BOOL fStoreWithDots,
...
//	also add a single reference which must be matched by a call to
// ReleaseContext().
//
FILEHC_EXPORT
...
CacheCreateFile(_In_ IN LPSTR lpstrName, IN FCACHE_CREATE_CALLBACK pfnCallBack,
                _In_ IN LPVOID lpv, IN BOOL fAsyncContext);
...
CacheRichCreateFile(_In_ IN LPSTR lpstrName,
                    IN FCACHE_RICHCREATE_CALLBACK pfnCallBack, IN LPVOID lpv,
...
//
FILEHC_EXPORT
void __stdcall CacheRemoveFiles(_In_ IN LPSTR lpstrName, IN BOOL fAllPrefixes);
//
...
//
FILEHC_EXPORT
BOOL __stdcall InsertFile(_In_ IN LPSTR lpstrName,
...
//
FILEHC_EXPORT
DWORD __stdcall GetFileSizeFromContext(IN struct FIO_CONTEXT *pContext,
...
//
typedef int(__stdcall *CACHE_KEY_COMPARE)(IN DWORD cbKey1, IN LPBYTE lpbKey1,
                                          IN DWORD cbKey2, IN LPBYTE lpbKey2);
...
//
//
typedef BOOL(__stdcall *CACHE_READ_CALLBACK)(IN DWORD cb, IN LPBYTE lpb,
...
//
//	This is a callback this is called whenever we evaluate a security
// descriptor. 	If it is not provided we will call the standard NT AccessCheck()
// call !
//
//	The function has the same signature as AccessCheck, however there are
// arguments 	we don't use - PrivilegeSet will always be NULL and
// PrivilegeSetLength will always be 0 !
//
typedef BOOL(WINAPI *CACHE_ACCESS_CHECK)(
    IN PSECURITY_DESCRIPTOR pSecurityDescriptor, IN HANDLE hClientToken,
    IN DWORD dwDesiredAccess, IN PGENERIC_MAPPING GenericMapping,
    IN PRIVILEGE_SET *PrivilegeSet, IN LPDWORD PrivilegeSetLength,
    IN LPDWORD GrantedAccess, IN LPBOOL AccessStatus);
...
//
//	The caller must guarantee the thread safety of this call - This function
// must not 	be called if any other thread is simultanesouly executing within
//	CacheFindContectFromName(), AssociateContextWithName(),
// AssociateDataWithName(), or InvalidateName()
//
FILEHC_EXPORT
...
BOOL __stdcall FindContextFromName(
    ...
    //
    //	User provides arbitrary bytes for Key to the cache item -
    // pfnKeyCompare() used 	to compare keys !
    //
    _In_reads_bytes_(cbName) IN LPBYTE lpbName, _In_ IN DWORD cbName,
    //
    //	User provides function which is called with the key once the key
    // comparison 	matches the key.  This lets the user do some extra
    // checking that they're getting 	what they want.
    //
...
BOOL __stdcall FindSyncContextFromName(
    ...
    // pfnKeyCompare() used 	to compare keys !
    //
    _In_reads_bytes_(cbName) IN LPBYTE lpbName, IN DWORD cbName,
    //
    //	User provides function which is called with the key once the key
    // comparison 	matches the key.  This lets the user do some extra
    // checking that they're getting 	what they want.
...
//	This insert a Name into the Name cache, that will find the specified
// FIO_CONTEXT !
//
//	If the name is already present in the cache, this will fail with
// GetLastError()==ERROR_DUP_NAME !
//
FILEHC_EXPORT
BOOL __stdcall AssociateContextWithName(
    ...
    //
    _In_ IN LPBYTE lpbData, IN DWORD cbData,
    //
    ...
    //
    //	User specifies whether they wish to keep their reference on the
    // FIO_CONTEXT
    //
    BOOL fKeepReference);

//
//	This function allows the user to remove a single name and all associated
// data 	from the name cache.
//
FILEHC_EXPORT
BOOL __stdcall InvalidateName(
    //
    //	The name cache the client wishes to use !
    //
    PNAME_CACHE_CONTEXT pNameCache,
...
//  NOTE: if we have to do work, the user has the only reference to the
//  resulting FIO_CONTEXT which will go away when they call ReleaseContext() !
//
//	pContext - the original FIO_CONTEXT
//	lpstrName - the file name associated with pContext
//	fWantItDotStuffed - if TRUE the resulting FIO_CONTEXT should be dot
// stuffed ! 	fTerminatorIncluded - if this is TRUE the source FIO_CONTEXT
// contains a terminating 	dot that we should be carefull not to stuff !
//
//	NOTE: We may return the same FIO_CONTEXT as the caller provided - in
// which case 	an extra reference has been added that needs to be dropped with
// ReleaseContext() !
//
//
FILEHC_EXPORT
struct FIO_CONTEXT *__stdcall ProduceDotStuffedContext(
    IN struct FIO_CONTEXT *pContext, _In_ IN LPSTR lpstrName,
...
//
//	The user specifies whether the Destination FIO_CONTEXT should be dot
// stuffed 	with fWantItDotStuffed, and whether the source FIO_CONTEXT
// includes the 	terminating CRLF.CRLF
//
...
FILEHC_EXPORT
BOOL __stdcall
ProduceDotStuffedContextInContext(IN struct FIO_CONTEXT *pContextSource,
...
//
FILEHC_EXPORT
BOOL __stdcall GetIsFileDotTerminated(IN struct FIO_CONTEXT *pContext);

//
//	Set whether the file has a terminating 'CRLF.CRLF' sequence !
//
FILEHC_EXPORT
void __stdcall SetIsFileDotTerminated(IN struct FIO_CONTEXT *pContext,
...
//
FILEHC_EXPORT
BOOL __stdcall SetDotStuffingOnWrites(
    IN struct FIO_CONTEXT *pContext,
    //
    ...
    //	fStripDots == FALSE means that we add dots to make the message dot
    // stuffed
    //
    IN BOOL fStripDots);
...
//
FILEHC_EXPORT
BOOL __stdcall SetDotScanningOnWrites(IN struct FIO_CONTEXT *pContext,
                                      IN BOOL fEnable);

//
//
//	This function should be called when we have finished doing all writes to
// an FIO_CONTEXT 	This function should be paired with
// SetDotStuffingOnWrites() and the fStripDots 	parameter should be the same as
// when SetDotStuffingOnWrites() was called.
//
...
//
//	If this function call is paired with a call to SetDotScanningOnWrites()
// fStripDots should be TRUE !
//
FILEHC_EXPORT
void __stdcall CompleteDotStuffingOnWrites(IN struct FIO_CONTEXT *pContext,
                                           IN BOOL fStripDots);
...
//	if the user tries to pend any synchronous reads while we are in this
// state !
//
FILEHC_EXPORT
BOOL __stdcall SetDotScanningOnReads(IN struct FIO_CONTEXT *pContext,
                                     IN BOOL fEnable);
...
//
FILEHC_EXPORT
BOOL __stdcall GetDotStuffState(IN struct FIO_CONTEXT *pContext, IN BOOL fReads,
                                _Inout_ OUT BOOL *pfStuffed,
                                _Out_ OUT BOOL *pfStoredWithDots);

//
//	In this case we always assume that the FIO_CONTEXT is not going to be
// dot stuffed. 	fRequiresStuffing == TRUE indicates that it SHOULD BE
// stuffed. 	fRequiresStuffing == FALSE indicates that the message does not
// need
// dot stuffing.
//
FILEHC_EXPORT
void __stdcall SetDotStuffState(
    IN struct FIO_CONTEXT *pContext,
    //
