...
//      lParam  = Ignored
//
//  * It is possible that only one or two of the three states are available. In
//  this case,
//    only the available items will be shown. If there is only one item
//    available, all three radio buttons will be hided, the description static
//    text will be shown to show the text.
//
// TVOT_UDARROW:
//      Count       = 2
...
//
//  Sel             - Current selection for this item, range from 0 to cListItem
//  - 1
//
...
#define IS_DMPUB_HIDDEN(dw, DMPub)                                             \
  (BOOL)((DWORD)(dw) & MAKE_DMPUB_HIDEBIT(DMPub))
...
//
// COMPROPSHEETUI data structure is used when calling common UI to display
// dialog box of properties pages.
//
//
...
//  Return Value:
//
//      the return value is less or equal to zero if it encounter an error
//      and/or something failed (look up the last error for details).
//
...
//
//                  This value is new to Window 8 and provided only to the
//                  original PFNPROPSHEETUI parameter passed to
//                  CommonPropertySheetUI.
//
//                  This is used between common UI and the system provided
//                  config module for v4 printer drivers and should be ignored
//                  by v3 drivers.
//
...
LONG APIENTRY CommonPropertySheetUIA(HWND hWndOwner,
                                     ...
                                     LPARAM lParam, _Out_opt_ LPDWORD pResult);
