...
#define IS_ENHANCED_STORAGE_GENERAL_ERROR(x)                                   \
  ((x) >= ES_GENERAL_ERROR_START && (x) <= ES_GENERAL_ERROR_END)

#define IS_ENHANCED_STORAGE_AUTHN_ERROR(x)                                     \
  ((x) >= ES_AUTHN_ERROR_START && (x) <= ES_AUTHN_ERROR_END)

#define IS_ENHANCED_STORAGE_PW_SILO_ERROR(x)                                   \
  ((x) >= ES_PW_SILO_ERROR_START && (x) <= ES_PW_SILO_ERROR_END)

#define IS_ENHANCED_STORAGE_VENDOR_ERROR(x)                                    \
  ((x) >= ES_VENDOR_ERROR_START && (x) <= ES_VENDOR_ERROR_END)

#define IS_ENHANCED_STORAGE_RESERVED_ERROR(x)                                  \
  (((x) >= ES_RESERVED_SILO_ERROR_START &&                                     \
...
// MessageText:
//
// Parameter Byte Length specified in the payload is not consistent with the
...
// MessageText:
//
// The challenge provided by the device does not match the one provided in an
// earlier step in the authentication sequence.
//
...
// MessageText:
//
// The size of the challenge specified by the device does not match the choice
...
// MessageText:
//
// Cannot enable IEEE 1667 password security since another security protocol is
// still active on the device.
//
