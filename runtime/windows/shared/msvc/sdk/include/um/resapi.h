...
#define CLRES_V3_FUNCTION_TABLE_SET(                                           \
    _Name, _Version, _Prefix, _Arbitrate, _Release, _BeginResourceControl,     \
    _BeginResourceTypeControl, _LooksAlive, _IsAlive, _Cancel)                 \
...
#define CLRES_V4_FUNCTION_TABLE_SET(                                           \
    _Name, _Version, _Prefix, _Arbitrate, _Release, _BeginResCtrl,             \
    _BeginResTypeCtrl, _LooksAlive, _IsAlive, _Cancel, _BeginResCtrlAsUser,    \
...
// To be used with Control Code
// CLUSCTL_RESOURCE_TYPE_GET_LOCAL_NODE_UTILIZATION_INFO Only: Return with an
// array of NodeIntelligenceInfoElement(s)
...
typedef DWORD(_stdcall *PSET_RESOURCE_STATUS_ROUTINE_EX)(
    IN RESOURCE_HANDLE ResourceHandle, IN PRESOURCE_STATUS_EX ResourceStatus);
...
typedef DWORD(_stdcall *PSET_RESOURCE_STATUS_ROUTINE)(
    IN RESOURCE_HANDLE ResourceHandle, IN PRESOURCE_STATUS ResourceStatus);

//
// Define Resource DLL callback method for notifying that a quorum
// resource DLL failed to hold the quorum resource.
//
typedef VOID(_stdcall *PQUORUM_RESOURCE_LOST)(IN RESOURCE_HANDLE Resource);
...
typedef VOID(_stdcall *PLOG_EVENT_ROUTINE)(IN RESOURCE_HANDLE ResourceHandle,
                                           IN LOG_LEVEL LogLevel,
                                           IN LPCWSTR FormatString, ...);

typedef RESID(_stdcall *POPEN_ROUTINE)(IN LPCWSTR ResourceName,
                                       IN HKEY ResourceKey,
                                       IN RESOURCE_HANDLE ResourceHandle);

typedef VOID(_stdcall *PCLOSE_ROUTINE)(IN RESID Resource);

typedef DWORD(_stdcall *PONLINE_ROUTINE)(IN RESID Resource,
                                         IN OUT LPHANDLE EventHandle);

typedef DWORD(_stdcall *POFFLINE_ROUTINE)(IN RESID Resource);

typedef VOID(_stdcall *PTERMINATE_ROUTINE)(IN RESID Resource);

typedef BOOL(_stdcall *PIS_ALIVE_ROUTINE)(IN RESID Resource);

typedef BOOL(_stdcall *PLOOKS_ALIVE_ROUTINE)(IN RESID Resource);

typedef DWORD(_stdcall *PARBITRATE_ROUTINE)(
    IN RESID Resource, IN PQUORUM_RESOURCE_LOST LostQuorumResource);

typedef DWORD(_stdcall *PRELEASE_ROUTINE)(IN RESID Resource);

typedef DWORD(_stdcall *PRESOURCE_CONTROL_ROUTINE)(
    IN RESID Resource, IN DWORD ControlCode, IN PVOID InBuffer,
    IN DWORD InBufferSize, OUT PVOID OutBuffer, IN DWORD OutBufferSize,
    OUT LPDWORD BytesReturned);

typedef DWORD(_stdcall *PRESOURCE_TYPE_CONTROL_ROUTINE)(
    IN LPCWSTR ResourceTypeName, IN DWORD ControlCode, IN PVOID InBuffer,
    IN DWORD InBufferSize, OUT PVOID OutBuffer, IN DWORD OutBufferSize,
    OUT LPDWORD BytesReturned);
...
typedef RESID(_stdcall *POPEN_V2_ROUTINE)(_In_ LPCWSTR ResourceName,
...
typedef DWORD(_stdcall *PONLINE_V2_ROUTINE)(
    _In_ RESID Resource, _Out_ LPHANDLE EventHandle, _In_ DWORD OnlineFlags,
    _In_reads_bytes_opt_(InBufferSize) PBYTE InBuffer, _In_ DWORD InBufferSize,
...
typedef DWORD(_stdcall *POFFLINE_V2_ROUTINE)(
    _In_ RESID Resource, _In_opt_ LPCWSTR DestinationNodeName,
    _In_ DWORD OfflineFlags, _In_reads_bytes_opt_(InBufferSize) PBYTE InBuffer,
    _In_ DWORD InBufferSize, _In_ DWORD Reserved /*Reserved*/
);

typedef DWORD(_stdcall *PCANCEL_ROUTINE)(
    IN RESID Resource, IN DWORD CancelFlags_RESERVED /*RESERVED*/
);
...
typedef DWORD(_stdcall *PBEGIN_RESCALL_ROUTINE)(
    IN RESID Resource, IN DWORD ControlCode, IN PVOID InBuffer,
    IN DWORD InBufferSize, OUT PVOID OutBuffer, IN DWORD OutBufferSize,
    OUT LPDWORD BytesReturned, IN PRES_CTL_CTX context,
    OUT PBOOL ReturnedAsynchronously);

typedef DWORD(_stdcall *PBEGIN_RESTYPECALL_ROUTINE)(
    IN LPCWSTR ResourceTypeName, IN DWORD ControlCode, IN PVOID InBuffer,
    IN DWORD InBufferSize, OUT PVOID OutBuffer, IN DWORD OutBufferSize,
    OUT LPDWORD BytesReturned, IN PRESTYPE_CTL_CTX context,
    OUT PBOOL ReturnedAsynchronously);
#endif
...
typedef DWORD(_stdcall *PBEGIN_RESCALL_AS_USER_ROUTINE)(
    IN RESID Resource, IN HANDLE TokenHandle, IN DWORD ControlCode,
    IN PVOID InBuffer, IN DWORD InBufferSize, OUT PVOID OutBuffer,
    IN DWORD OutBufferSize, OUT LPDWORD BytesReturned, IN PRES_CTL_CTX context,
    OUT PBOOL ReturnedAsynchronously);

typedef DWORD(_stdcall *PBEGIN_RESTYPECALL_AS_USER_ROUTINE)(
    IN LPCWSTR ResourceTypeName, IN HANDLE TokenHandle, IN DWORD ControlCode,
    IN PVOID InBuffer, IN DWORD InBufferSize, OUT PVOID OutBuffer,
...
#define RESUTIL_PROPITEM_SIGNED                                                \
  0x00000004 // Numeric property is signed (defaults to unsigned)
#define RESUTIL_PROPITEM_IN_MEMORY                                             \
  0x00000008 // Node-Local In-Memory Properties are not stored in Cluster
             // Database
...
typedef DWORD(_stdcall *PSTARTUP_ROUTINE)(
    IN LPCWSTR ResourceType, IN DWORD MinVersionSupported,
    IN DWORD MaxVersionSupported,
    IN PSET_RESOURCE_STATUS_ROUTINE SetResourceStatus,
    IN PLOG_EVENT_ROUTINE LogEvent, OUT PCLRES_FUNCTION_TABLE *FunctionTable);
...
typedef DWORD(_stdcall *PSET_RESOURCE_LOCKED_MODE_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ BOOL LockedModeEnabled,
    _In_ DWORD LockedModeReason);

typedef DWORD(_stdcall *PSIGNAL_FAILURE_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ FAILURE_TYPE FailureType,
    _In_ DWORD ApplicationSpecificErrorCode);

typedef DWORD(_stdcall *PSET_RESOURCE_INMEMORY_NODELOCAL_PROPERTIES_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ unsigned char *propertyListBuffer,
    _In_ DWORD propertyListBufferSize);

typedef DWORD(_stdcall *PEND_CONTROL_CALL)(_In_ const PRES_CTL_CTX context,
                                           _In_ DWORD status);

typedef DWORD(_stdcall *PEND_TYPE_CONTROL_CALL)(
    _In_ const PRESTYPE_CTL_CTX context, _In_ DWORD status);

typedef DWORD(_stdcall *PEXTEND_RES_CONTROL_CALL)(
    _In_ const PRES_CTL_CTX context, _In_ DWORD newTimeoutInMs);

typedef DWORD(_stdcall *PEXTEND_RES_TYPE_CONTROL_CALL)(
    _In_ const PRESTYPE_CTL_CTX context, _In_ DWORD newTimeoutInMs);

typedef DWORD(_stdcall *PRAISE_RES_TYPE_NOTIFICATION)(
    _In_ LPCWSTR ResourceType,
    _In_reads_bytes_(payloadSize) const PBYTE pPayload, _In_ DWORD payloadSize);

typedef DWORD(_stdcall *PCHANGE_RESOURCE_PROCESS_FOR_DUMPS)(
    _In_ RESOURCE_HANDLE resource, _In_ LPCWSTR processName,
    _In_ DWORD processId, _In_ BOOL isAdd);
typedef DWORD(_stdcall *PCHANGE_RES_TYPE_PROCESS_FOR_DUMPS)(
    _In_ LPCWSTR resourceTypeName, _In_ LPCWSTR processName,
    _In_ DWORD processId, _In_ BOOL isAdd);

typedef DWORD(_stdcall *PSET_INTERNAL_STATE)(
    _In_ RESOURCE_HANDLE, _In_ CLUSTER_RESOURCE_APPLICATION_STATE stateType,
    _In_ BOOL active);
...
typedef DWORD(_stdcall *PSET_RESOURCE_LOCKED_MODE_EX_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ BOOL LockedModeEnabled,
    _In_ DWORD LockedModeReason, _In_ DWORD LockedModeFlags);

typedef DWORD(_stdcall *PREQUEST_DUMP_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ BOOL DumpDueToCallInProgress,
...
typedef DWORD(_stdcall *PSET_RESOURCE_WPR_POLICY_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ DWORD WprPolicyFlags);
...
typedef DWORD(_stdcall *PARM_WPR_WATCHDOG_FOR_CURRENT_RESOURCE_CALL_ROUTINE)(
    _In_ RESOURCE_HANDLE ResourceHandle, _In_ ULONGLONG TimeoutInMs);
...
//
DWORD
InitializeClusterHealthFault(_Inout_ CLUSTER_HEALTH_FAULT *clusterHealthFault);
...
DWORD
FreeClusterHealthFault(_Inout_ CLUSTER_HEALTH_FAULT *clusterHealthFault);
...
DWORD
WINAPI
ClusGetClusterHealthFaults(_In_ HCLUSTER hCluster,
                           _Out_ CLUSTER_HEALTH_FAULT_ARRAY *objects,
                           _In_ DWORD flags);

DWORD
WINAPI
ClusRemoveClusterHealthFault(_In_ HCLUSTER hCluster, _In_ LPCWSTR id,
                             _In_ DWORD flags);

DWORD
WINAPI
ClusAddClusterHealthFault(_In_ HCLUSTER hCluster,
...
DWORD
WINAPI
ResUtilStartResourceService(IN LPCWSTR pszServiceName,
                            OUT LPSC_HANDLE phServiceHandle);

typedef DWORD(WINAPI *PRESUTIL_START_RESOURCE_SERVICE)(
    IN LPCWSTR pszServiceName, OUT LPSC_HANDLE phServiceHandle);
...
typedef DWORD(WINAPI *PRESUTIL_VERIFY_SERVICE)(IN SC_HANDLE hServiceHandle);

DWORD
WINAPI
ResUtilStopService(IN SC_HANDLE hServiceHandle);

typedef DWORD(WINAPI *PRESUTIL_STOP_SERVICE)(IN SC_HANDLE hServiceHandle);

DWORD
WINAPI
ResUtilCreateDirectoryTree(IN LPCWSTR pszPath);

typedef DWORD(WINAPI *PRESUTIL_CREATE_DIRECTORY_TREE)(IN LPCWSTR pszPath);

BOOL WINAPI ResUtilIsPathValid(IN LPCWSTR pszPath);

typedef BOOL(WINAPI *PRESUTIL_IS_PATH_VALID)(IN LPCWSTR pszPath);

DWORD
WINAPI
ResUtilEnumProperties(_In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                      _Out_writes_bytes_to_(cbOutPropertiesSize,
                                            *pcbBytesReturned)
...
typedef DWORD(WINAPI *PRESUTIL_ENUM_PROPERTIES)(
    ...
    _Out_writes_bytes_to_(cbOutPropertiesSize, *pcbBytesReturned)
        LPWSTR pszOutProperties,
    _In_ DWORD cbOutPropertiesSize, _Out_ LPDWORD pcbBytesReturned,
    _Out_ LPDWORD pcbRequired);

DWORD
WINAPI
ResUtilEnumPrivateProperties(_In_ HKEY hkeyClusterKey,
                             _Out_writes_bytes_to_(cbOutPropertiesSize,
                                                   *pcbBytesReturned)
                                 LPWSTR pszOutProperties,
                             _In_ DWORD cbOutPropertiesSize,
...
DWORD
WINAPI
ResUtilGetProperties(_In_ HKEY hkeyClusterKey,
                     _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                     _Out_writes_bytes_to_(cbOutPropertyListSize,
                                           *pcbBytesReturned)
                         PVOID pOutPropertyList,
                     _In_ DWORD cbOutPropertyListSize,
                     _Out_ LPDWORD pcbBytesReturned, _Out_ LPDWORD pcbRequired);

typedef DWORD(WINAPI *PRESUTIL_GET_PROPERTIES)(
    _In_ HKEY hkeyClusterKey, _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    _Out_writes_bytes_to_(cbOutPropertyListSize, *pcbBytesReturned)
        PVOID pOutPropertyList,
    _In_ DWORD cbOutPropertyListSize, _Out_ LPDWORD pcbBytesReturned,
    _Out_ LPDWORD pcbRequired);
...
DWORD
WINAPI
ResUtilGetPrivateProperties(_In_ HKEY hkeyClusterKey,
                            _Out_writes_bytes_to_(cbOutPropertyListSize,
                                                  *pcbBytesReturned)
                                PVOID pOutPropertyList,
                            _In_ DWORD cbOutPropertyListSize,
...
DWORD
WINAPI
ResUtilGetPropertySize(IN HKEY hkeyClusterKey,
...
typedef DWORD(WINAPI *PRESUTIL_GET_PROPERTY_SIZE)(
    IN HKEY hkeyClusterKey, IN const PRESUTIL_PROPERTY_ITEM pPropertyTableItem,
    IN OUT LPDWORD pcbOutPropertyListSize, IN OUT LPDWORD pnPropertyCount);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI ResUtilGetProperty(
    _In_ HKEY hkeyClusterKey,
    _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTableItem,
    _Inout_ _At_(*pOutPropertyItem,
                 _Pre_readable_byte_size_(*pcbOutPropertyItemSize)
                     _Post_readable_byte_size_(*pcbOutPropertyItemSize))
        PVOID *pOutPropertyItem,
    _Inout_ LPDWORD pcbOutPropertyItemSize);

typedef _Success_(return == ERROR_SUCCESS) DWORD(WINAPI *PRESUTIL_GET_PROPERTY)(
    _In_ HKEY hkeyClusterKey,
    _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTableItem,
    _Inout_ _At_(*pOutPropertyItem,
                 _Pre_readable_byte_size_(*pcbOutPropertyItemSize)
...
DWORD
WINAPI
ResUtilVerifyPropertyTable(_In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                           _Reserved_ PVOID Reserved,
                           _In_ BOOL bAllowUnknownProperties,
                           _In_reads_bytes_(cbInPropertyListSize)
...
typedef DWORD(WINAPI *PRESUTIL_VERIFY_PROPERTY_TABLE)(
    _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable, _Reserved_ PVOID Reserved,
    _In_ BOOL bAllowUnknownProperties,
    _In_reads_bytes_(cbInPropertyListSize) const PVOID pInPropertyList,
    _In_ DWORD cbInPropertyListSize, _Out_opt_ LPBYTE pOutParams);

DWORD
WINAPI
ResUtilSetPropertyTable(
    _In_ HKEY hkeyClusterKey, _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    _Reserved_ PVOID Reserved, _In_ BOOL bAllowUnknownProperties,
    _In_reads_bytes_(cbInPropertyListSize) const PVOID pInPropertyList,
    _In_ DWORD cbInPropertyListSize, _Out_opt_ LPBYTE pOutParams);
...
DWORD
WINAPI
ResUtilSetPropertyTableEx(IN HKEY hkeyClusterKey,
                          IN const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                          IN PVOID Reserved, IN BOOL bAllowUnknownProperties,
                          IN const PVOID pInPropertyList,
                          IN DWORD cbInPropertyListSize, IN BOOL bForceWrite,
                          OUT OPTIONAL LPBYTE pOutParams);

typedef DWORD(WINAPI *PRESUTIL_SET_PROPERTY_TABLE_EX)(
    IN HKEY hkeyClusterKey, IN const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    IN PVOID Reserved, IN BOOL bAllowUnknownProperties,
    IN const PVOID pInPropertyList, IN DWORD cbInPropertyListSize,
    IN BOOL bForceWrite, OUT OPTIONAL LPBYTE pOutParams);

DWORD
WINAPI
ResUtilSetPropertyParameterBlock(IN HKEY hkeyClusterKey,
                                 IN const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                                 IN PVOID Reserved, IN const LPBYTE pInParams,
                                 IN const PVOID pInPropertyList,
...
DWORD
WINAPI
ResUtilSetUnknownProperties(_In_ HKEY hkeyClusterKey,
                            _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                            _In_reads_bytes_(cbInPropertyListSize)
                                const PVOID pInPropertyList,
                            _In_ DWORD cbInPropertyListSize);
...
ResUtilGetPropertiesToParameterBlock(
    _In_ HKEY hkeyClusterKey, _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    _Inout_ LPBYTE pOutParams, _In_ BOOL bCheckForRequiredProperties,
    _Outptr_result_maybenull_ LPWSTR *pszNameOfPropInError);

typedef DWORD(WINAPI *PRESUTIL_GET_PROPERTIES_TO_PARAMETER_BLOCK)(
    IN HKEY hkeyClusterKey, IN const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    OUT LPBYTE pOutParams, IN BOOL bCheckForRequiredProperties,
...
ResUtilPropertyListFromParameterBlock(
    _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    _Out_writes_bytes_to_opt_(*pcbOutPropertyListSize, *pcbBytesReturned)
        PVOID pOutPropertyList,
    _Inout_ LPDWORD pcbOutPropertyListSize, _In_ const LPBYTE pInParams,
    _Out_ LPDWORD pcbBytesReturned, _Out_ LPDWORD pcbRequired);
...
DWORD
WINAPI
ResUtilDupParameterBlock(OUT LPBYTE pOutParams, IN const LPBYTE pInParams,
                         IN const PRESUTIL_PROPERTY_ITEM pPropertyTable);

typedef DWORD(WINAPI *PRESUTIL_DUP_PARAMETER_BLOCK)(
    OUT LPBYTE pOutParams, IN const LPBYTE pInParams,
    IN const PRESUTIL_PROPERTY_ITEM pPropertyTable);

void WINAPI
ResUtilFreeParameterBlock(IN OUT LPBYTE pOutParams, IN const LPBYTE pInParams,
                          IN const PRESUTIL_PROPERTY_ITEM pPropertyTable);

typedef void(WINAPI *PRESUTIL_FREE_PARAMETER_BLOCK)(
    IN OUT LPBYTE pOutParams, IN const LPBYTE pInParams,
    IN const PRESUTIL_PROPERTY_ITEM pPropertyTable);

DWORD
WINAPI
ResUtilAddUnknownProperties(IN HKEY hkeyClusterKey,
                            IN const PRESUTIL_PROPERTY_ITEM pPropertyTable,
...
typedef DWORD(WINAPI *PRESUTIL_ADD_UNKNOWN_PROPERTIES)(
    IN HKEY hkeyClusterKey, IN const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    IN OUT PVOID pOutPropertyList, IN DWORD pcbOutPropertyListSize,
    IN OUT LPDWORD pcbBytesReturned, IN OUT LPDWORD pcbRequired);

DWORD
WINAPI
ResUtilSetPrivatePropertyList(_In_ HKEY hkeyClusterKey,
                              _In_reads_bytes_(cbInPropertyListSize)
                                  const PVOID pInPropertyList,
                              _In_ DWORD cbInPropertyListSize);
...
DWORD
WINAPI
ResUtilVerifyPrivatePropertyList(_In_reads_bytes_(cbInPropertyListSize)
                                     const PVOID pInPropertyList,
                                 _In_ DWORD cbInPropertyListSize);
...
typedef PWSTR(WINAPI *PRESUTIL_DUP_STRING)(IN LPCWSTR pszInString);

DWORD
WINAPI
ResUtilGetBinaryValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                      _Outptr_result_bytebuffer_to_maybenull_(*pcbOutValueSize,
                                                              *pcbOutValueSize)
                          LPBYTE *ppbOutValue,
                      _Out_ LPDWORD pcbOutValueSize);

typedef DWORD(WINAPI *PRESUTIL_GET_BINARY_VALUE)(
    _In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
    _Outptr_result_bytebuffer_to_maybenull_(*pcbOutValueSize, *pcbOutValueSize)
        LPBYTE *ppbOutValue,
    _Out_ LPDWORD pcbOutValueSize);
...
typedef LPWSTR(WINAPI *PRESUTIL_GET_SZ_VALUE)(IN HKEY hkeyClusterKey,
                                              IN LPCWSTR pszValueName);

LPWSTR
WINAPI
ResUtilGetExpandSzValue(IN HKEY hkeyClusterKey, IN LPCWSTR pszValueName,
                        IN BOOL bExpand);

typedef LPWSTR(WINAPI *PRESUTIL_GET_EXPAND_SZ_VALUE)(IN HKEY hkeyClusterKey,
                                                     IN LPCWSTR pszValueName,
                                                     IN BOOL bExpand);
...
DWORD
WINAPI_INLINE
ResUtilGetMultiSzValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                       _Outptr_result_bytebuffer_to_maybenull_(*pcbOutValueSize,
                                                               *pcbOutValueSize)
                           LPWSTR *ppszOutValue,
                       _Out_ LPDWORD pcbOutValueSize) {
...
DWORD
WINAPI
ResUtilGetDwordValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                     _Out_ LPDWORD pdwOutValue, _In_ DWORD dwDefaultValue);

typedef DWORD(WINAPI *PRESUTIL_GET_DWORD_VALUE)(_In_ HKEY hkeyClusterKey,
...
DWORD
WINAPI
ResUtilGetQwordValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                     _Out_ PULONGLONG pqwOutValue,
                     _In_ ULONGLONG qwDefaultValue);

typedef DWORD(WINAPI *PRESUTIL_GET_QWORD_VALUE)(_In_ HKEY hkeyClusterKey,
                                                _In_ LPCWSTR pszValueName,
                                                _Out_ PULONGLONG pqwOutValue,
                                                _In_ ULONGLONG qwDefaultValue);
...
ResUtilSetBinaryValue(
    ...
    _Inout_opt_ _At_(*ppbOutValue,
                     _When_(*ppbOutValue != NULL,
                            _Pre_readable_byte_size_(*pcbOutValueSize))
                         _Post_readable_byte_size_(*pcbOutValueSize))
        LPBYTE *ppbOutValue,
...
DWORD
WINAPI
ResUtilSetSzValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                  _In_ LPCWSTR pszNewValue, _Inout_opt_ LPWSTR *ppszOutString);

typedef DWORD(WINAPI *PRESUTIL_SET_SZ_VALUE)(_In_ HKEY hkeyClusterKey,
                                             _In_ LPCWSTR pszValueName,
...
DWORD
WINAPI
ResUtilSetExpandSzValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                        _In_ LPCWSTR pszNewValue,
                        _Inout_opt_ LPWSTR *ppszOutString);
...
DWORD
WINAPI
ResUtilSetMultiSzValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                       _In_reads_bytes_(cbNewValueSize) LPCWSTR pszNewValue,
                       _In_ DWORD cbNewValueSize,
                       _Outptr_opt_result_bytebuffer_to_(*pcbOutValueSize,
                                                         *pcbOutValueSize)
                           LPWSTR *ppszOutValue,
...
typedef DWORD(WINAPI *PRESUTIL_SET_MULTI_SZ_VALUE)(
    ...
    _In_reads_bytes_(cbNewValueSize) LPCWSTR pszNewValue,
    _In_ DWORD cbNewValueSize,
    _Outptr_opt_result_bytebuffer_to_(*pcbOutValueSize, *pcbOutValueSize)
        LPWSTR *ppszOutValue,
    _Inout_opt_ LPDWORD pcbOutValueSize);

DWORD
WINAPI
ResUtilSetDwordValue(IN HKEY hkeyClusterKey, IN LPCWSTR pszValueName,
                     IN DWORD dwNewValue, IN OUT LPDWORD pdwOutValue);

typedef DWORD(WINAPI *PRESUTIL_SET_DWORD_VALUE)(IN HKEY hkeyClusterKey,
                                                IN LPCWSTR pszValueName,
                                                IN DWORD dwNewValue,
...
DWORD
WINAPI
ResUtilSetQwordValue(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
                     _In_ ULONGLONG qwNewValue,
                     _Inout_opt_ PULONGLONG pqwOutValue);

typedef DWORD(WINAPI *PRESUTIL_SET_QWORD_VALUE)(
    _In_ HKEY hkeyClusterKey, _In_ LPCWSTR pszValueName,
    _In_ ULONGLONG qwNewValue, _Inout_opt_ PULONGLONG pqwOutValue);

DWORD
WINAPI
ResUtilSetValueEx(_In_ HKEY hkeyClusterKey, _In_ LPCWSTR valueName,
                  _In_ DWORD valueType,
                  _In_reads_bytes_(valueSize) const LPBYTE valueData,
                  _In_ DWORD valueSize, _In_ DWORD flags);

DWORD
WINAPI
ResUtilGetBinaryProperty(
    _Outptr_result_bytebuffer_(*pcbOutValueSize) LPBYTE *ppbOutValue,
    _Out_ LPDWORD pcbOutValueSize, _In_ const PCLUSPROP_BINARY pValueStruct,
    _In_reads_bytes_opt_(cbOldValueSize) const LPBYTE pbOldValue,
    _In_ DWORD cbOldValueSize,
    _Inout_ _At_(*ppPropertyList,
                 _Pre_readable_byte_size_(*pcbPropertyListSize)
                     _Post_readable_byte_size_(*pcbPropertyListSize))
...
ResUtilGetSzProperty(
    _Outptr_ LPWSTR *ppszOutValue, _In_ const PCLUSPROP_SZ pValueStruct,
...
ResUtilGetMultiSzProperty(
    ...
    _Out_ LPDWORD pcbOutValueSize, _In_ const PCLUSPROP_SZ pValueStruct,
...
DWORD
WINAPI
ResUtilGetDwordProperty(OUT LPDWORD pdwOutValue,
                        IN const PCLUSPROP_DWORD pValueStruct,
                        IN DWORD dwOldValue, IN DWORD dwMinimum,
                        IN DWORD dwMaximum, OUT LPBYTE *ppPropertyList,
                        OUT LPDWORD pcbPropertyListSize);

typedef DWORD(WINAPI *PRESUTIL_GET_DWORD_PROPERTY)(
    OUT LPDWORD pdwOutValue, IN const PCLUSPROP_DWORD pValueStruct,
    IN DWORD dwOldValue, IN DWORD dwMinimum, IN DWORD dwMaximum,
    OUT LPBYTE *ppPropertyList, OUT LPDWORD pcbPropertyListSize);

DWORD
WINAPI
ResUtilGetLongProperty(OUT LPLONG plOutValue,
                       IN const PCLUSPROP_LONG pValueStruct, IN LONG lOldValue,
                       IN LONG lMinimum, IN LONG lMaximum,
                       OUT LPBYTE *ppPropertyList,
                       OUT LPDWORD pcbPropertyListSize);

typedef DWORD(WINAPI *PRESUTIL_GET_LONG_PROPERTY)(
    OUT LPLONG plOutValue, IN const PCLUSPROP_LONG pValueStruct,
    IN LONG lOldValue, IN LONG lMinimum, IN LONG lMaximum,
    OUT LPBYTE *ppPropertyList, OUT LPDWORD pcbPropertyListSize);

DWORD
WINAPI
ResUtilGetFileTimeProperty(OUT LPFILETIME pftOutValue,
                           IN const PCLUSPROP_FILETIME pValueStruct,
                           IN FILETIME ftOldValue, IN FILETIME ftMinimum,
                           IN FILETIME ftMaximum, OUT LPBYTE *ppPropertyList,
                           OUT LPDWORD pcbPropertyListSize);

typedef DWORD(WINAPI *PRESUTIL_GET_FILETIME_PROPERTY)(
    OUT LPFILETIME pftOutValue, IN const PCLUSPROP_FILETIME pValueStruct,
    IN FILETIME ftOldValue, IN FILETIME ftMinimum, IN FILETIME ftMaximum,
    OUT LPBYTE *ppPropertyList, OUT LPDWORD pcbPropertyListSize);

LPVOID
WINAPI
ResUtilGetEnvironmentWithNetName(_In_ HRESOURCE hResource);
...
typedef DWORD(WINAPI *PRESUTIL_FREE_ENVIRONMENT)(IN LPVOID lpEnvironment);

LPWSTR
WINAPI
ResUtilExpandEnvironmentStrings(IN LPCWSTR pszSrc);

typedef LPWSTR(WINAPI *PRESUTIL_EXPAND_ENVIRONMENT_STRINGS)(IN LPCWSTR pszSrc);

DWORD
WINAPI
ResUtilSetResourceServiceEnvironment(IN LPCWSTR pszServiceName,
                                     IN HRESOURCE hResource,
                                     IN PLOG_EVENT_ROUTINE pfnLogEvent,
                                     IN RESOURCE_HANDLE hResourceHandle);

typedef DWORD(WINAPI *PRESUTIL_SET_RESOURCE_SERVICE_ENVIRONMENT)(
    IN LPCWSTR pszServiceName, IN HRESOURCE hResource,
    IN PLOG_EVENT_ROUTINE pfnLogEvent, IN RESOURCE_HANDLE hResourceHandle);

DWORD
WINAPI
ResUtilRemoveResourceServiceEnvironment(IN LPCWSTR pszServiceName,
                                        IN PLOG_EVENT_ROUTINE pfnLogEvent,
                                        IN RESOURCE_HANDLE hResourceHandle);

typedef DWORD(WINAPI *PRESUTIL_REMOVE_RESOURCE_SERVICE_ENVIRONMENT)(
    IN LPCWSTR pszServiceName, IN PLOG_EVENT_ROUTINE pfnLogEvent,
    IN RESOURCE_HANDLE hResourceHandle);

DWORD
WINAPI
ResUtilSetResourceServiceStartParameters(IN LPCWSTR pszServiceName,
...
typedef DWORD(WINAPI *PRESUTIL_SET_RESOURCE_SERVICE_START_PARAMETERS)(
    IN LPCWSTR pszServiceName, IN SC_HANDLE schSCMHandle,
    IN OUT LPSC_HANDLE phService, IN PLOG_EVENT_ROUTINE pfnLogEvent,
    IN RESOURCE_HANDLE hResourceHandle);

DWORD
WINAPI
ResUtilFindSzProperty(_In_reads_bytes_(cbPropertyListSize)
...
typedef DWORD(WINAPI *PRESUTIL_FIND_SZ_PROPERTY)(
    ...
    _In_ DWORD cbPropertyListSize, _In_ LPCWSTR pszPropertyName,
    _Outptr_opt_ LPWSTR *pszPropertyValue);

DWORD
WINAPI
ResUtilFindExpandSzProperty(_In_reads_bytes_(cbPropertyListSize)
                                const PVOID pPropertyList,
                            _In_ DWORD cbPropertyListSize,
                            _In_ LPCWSTR pszPropertyName,
                            _Outptr_opt_ LPWSTR *pszPropertyValue);
...
DWORD
WINAPI
ResUtilFindExpandedSzProperty(_In_reads_bytes_(cbPropertyListSize)
                                  const PVOID pPropertyList,
                              _In_ DWORD cbPropertyListSize,
                              _In_ LPCWSTR pszPropertyName,
                              _Outptr_opt_ LPWSTR *pszPropertyValue);
...
DWORD
WINAPI
ResUtilFindDwordProperty(_In_reads_bytes_(cbPropertyListSize)
                             const PVOID pPropertyList,
                         _In_ DWORD cbPropertyListSize,
                         _In_ LPCWSTR pszPropertyName,
...
ResUtilFindBinaryProperty(
    _In_reads_bytes_(cbPropertyListSize) const PVOID pPropertyList,
    _In_ DWORD cbPropertyListSize, _In_ LPCWSTR pszPropertyName,
    _Outptr_opt_result_bytebuffer_to_(*pcbPropertyValueSize,
...
ResUtilFindMultiSzProperty(
    _In_reads_bytes_(cbPropertyListSize) const PVOID pPropertyList,
    _In_ DWORD cbPropertyListSize, _In_ LPCWSTR pszPropertyName,
    _Outptr_result_bytebuffer_to_(*pcbPropertyValueSize, *pcbPropertyValueSize)
...
DWORD
WINAPI
ResUtilFindLongProperty(_In_reads_bytes_(cbPropertyListSize)
                            const PVOID pPropertyList,
                        _In_ DWORD cbPropertyListSize,
                        _In_ LPCWSTR pszPropertyName,
...
DWORD
WINAPI
ResUtilFindULargeIntegerProperty(_In_reads_bytes_(cbPropertyListSize)
                                     const PVOID pPropertyList,
                                 _In_ DWORD cbPropertyListSize,
                                 _In_ LPCWSTR pszPropertyName,
...
DWORD
WINAPI
ResUtilFindFileTimeProperty(_In_reads_bytes_(cbPropertyListSize)
                                const PVOID pPropertyList,
                            _In_ DWORD cbPropertyListSize,
                            _In_ LPCWSTR pszPropertyName,
...
DWORD
WINAPI
ClusWorkerCreate(OUT PCLUS_WORKER lpWorker,
                 IN PWORKER_START_ROUTINE lpStartAddress, IN PVOID lpParameter);

typedef DWORD(WINAPI *PCLUSAPI_CLUS_WORKER_CREATE)(
    OUT PCLUS_WORKER lpWorker, IN PWORKER_START_ROUTINE lpStartAddress,
    IN PVOID lpParameter);

BOOL WINAPI ClusWorkerCheckTerminate(IN PCLUS_WORKER lpWorker);

typedef BOOL(WINAPI *PCLUSAPIClusWorkerCheckTerminate)(
    IN PCLUS_WORKER lpWorker);

VOID WINAPI ClusWorkerTerminate(_In_ PCLUS_WORKER lpWorker);
...
DWORD
WINAPI
ClusWorkerTerminateEx(_Inout_ PCLUS_WORKER ClusWorker,
                      _In_ DWORD TimeoutInMilliseconds, _In_ BOOL WaitOnly);

DWORD
ClusWorkersTerminate(_Inout_updates_(ClusWorkersCount)
                         PCLUS_WORKER *ClusWorkers,
                     _In_ size_t const ClusWorkersCount,
                     _In_ DWORD TimeoutInMilliseconds, _In_ BOOL WaitOnly);

// Define enumerate resource callback function. This gets called for each
// resource enumerated by ResUtilEnumResources
...
typedef BOOL(WINAPI *PRESUTIL_RESOURCES_EQUAL)(IN HRESOURCE hSelf,
                                               IN HRESOURCE hResource);

BOOL WINAPI ResUtilResourceTypesEqual(IN LPCWSTR lpszResourceTypeName,
                                      IN HRESOURCE hResource);

typedef BOOL(WINAPI *PRESUTIL_RESOURCE_TYPES_EQUAL)(
    IN LPCWSTR lpszResourceTypeName, IN HRESOURCE hResource);

BOOL WINAPI ResUtilIsResourceClassEqual(IN PCLUS_RESOURCE_CLASS_INFO prci,
                                        IN HRESOURCE hResource);

typedef BOOL(WINAPI *PRESUTIL_IS_RESOURCE_CLASS_EQUAL)(
    IN PCLUS_RESOURCE_CLASS_INFO prci, IN HRESOURCE hResource);

DWORD
WINAPI
ResUtilEnumResources(IN HRESOURCE hSelf, IN LPCWSTR lpszResTypeName,
                     IN LPRESOURCE_CALLBACK pResCallBack, IN PVOID pParameter);
...
DWORD
WINAPI
ResUtilEnumResourcesEx(IN HCLUSTER hCluster, IN HRESOURCE hSelf,
                       IN LPCWSTR lpszResTypeName,
                       IN LPRESOURCE_CALLBACK_EX pResCallBack,
                       IN PVOID pParameter);

typedef DWORD(WINAPI *PRESUTIL_ENUM_RESOURCES_EX)(
    IN HCLUSTER hCluster, IN HRESOURCE hSelf, IN LPCWSTR lpszResTypeName,
    IN LPRESOURCE_CALLBACK_EX pResCallBack, IN PVOID pParameter);
...
typedef HRESOURCE(WINAPI *PRESUTIL_GET_RESOURCE_DEPENDENCY)(
    IN HANDLE hSelf, IN LPCWSTR lpszResourceType);

HRESOURCE
WINAPI
ResUtilGetResourceDependencyByName(IN HCLUSTER hCluster, IN HANDLE hSelf,
                                   IN LPCWSTR lpszResourceType,
                                   IN BOOL bRecurse);

typedef HRESOURCE(WINAPI *PRESUTIL_GET_RESOURCE_DEPENDENCY_BY_NAME)(
    IN HCLUSTER hCluster, IN HANDLE hSelf, IN LPCWSTR lpszResourceType,
    IN BOOL bRecurse);

HRESOURCE
WINAPI
ResUtilGetResourceDependencyByClass(IN HCLUSTER hCluster, IN HANDLE hSelf,
                                    IN PCLUS_RESOURCE_CLASS_INFO prci,
                                    IN BOOL bRecurse);

typedef HRESOURCE(WINAPI *PRESUTIL_GET_RESOURCE_DEPENDENCY_BY_CLASS)(
    IN HCLUSTER hCluster, IN HANDLE hSelf, IN PCLUS_RESOURCE_CLASS_INFO prci,
    IN BOOL bRecurse);

HRESOURCE
WINAPI
ResUtilGetResourceNameDependency(IN LPCWSTR lpszResourceName,
                                 IN LPCWSTR lpszResourceType);

typedef HRESOURCE(WINAPI *PRESUTIL_GET_RESOURCE_NAME_DEPENDENCY)(
    IN LPCWSTR lpszResourceName, IN LPCWSTR lpszResourceType);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
...
typedef _Success_(return == ERROR_SUCCESS)
    DWORD(WINAPI *PRESUTIL_GET_RESOURCE_DEPENDENTIP_ADDRESS_PROPS)(
        _In_ HRESOURCE hResource,
        _Out_writes_to_(*pcchAddress, *pcchAddress) LPWSTR pszAddress,
        _Inout_ DWORD *pcchAddress,
        _Out_writes_to_(*pcchSubnetMask, *pcchSubnetMask) LPWSTR pszSubnetMask,
...
_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    ResUtilFindDependentDiskResourceDriveLetter(
        ...
        _In_ HRESOURCE
            hResource, // handle to resource to query for dependencies
        _Out_writes_to_(*pcchDriveLetter, *pcchDriveLetter)
            LPWSTR pszDriveLetter, // buffer to store drive letter (ex. "X:\0")
        _Inout_ DWORD *pcchDriveLetter // IN size of the pszDriveLetter buffer,
                                       // OUT size of buffer required
    );

typedef _Success_(return == ERROR_SUCCESS) DWORD(
    ...
    _Out_writes_to_(*pcchDriveLetter, *pcchDriveLetter)
        LPWSTR pszDriveLetter,     // buffer to store drive letter (ex. "X:\0")
    _Inout_ DWORD *pcchDriveLetter // IN size of the pszDriveLetter buffer, OUT
                                   // size of buffer required
);

DWORD
WINAPI
ResUtilTerminateServiceProcessFromResDll(_In_ DWORD dwServicePid,
                                         _In_ BOOL bOffline,
...
typedef DWORD(WINAPI *PRESUTIL_TERMINATE_SERVICE_PROCESS_FROM_RES_DLL)(
    _In_ DWORD dwServicePid, _In_ BOOL bOffline, _Out_ PDWORD pdwResourceState,
    _In_ PLOG_EVENT_ROUTINE pfnLogEvent, _In_ RESOURCE_HANDLE hResourceHandle);

DWORD
WINAPI
ResUtilGetPropertyFormats(_In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
                          _Out_writes_bytes_to_(cbPropertyFormatListSize,
                                                *pcbBytesReturned)
...
typedef DWORD(WINAPI *PRESUTIL_GET_PROPERTY_FORMATS)(
    _In_ const PRESUTIL_PROPERTY_ITEM pPropertyTable,
    _Out_writes_bytes_to_(cbPropertyFormatListSize, *pcbBytesReturned)
        PVOID pOutPropertyFormatList,
    _In_ DWORD cbPropertyFormatListSize, _Out_ LPDWORD pcbBytesReturned,
    _Out_ LPDWORD pcbRequired);

DWORD
WINAPI
ResUtilGetCoreClusterResources(_In_ HCLUSTER hCluster,
...
typedef DWORD(WINAPI *PRESUTIL_GET_CORE_CLUSTER_RESOURCES)(
    _In_ HCLUSTER hCluster, _Out_ HRESOURCE *phClusterNameResource,
    _Out_ HRESOURCE *phClusterIPAddressResource,
    _Out_ HRESOURCE *phClusterQuorumResource);

DWORD
WINAPI
ResUtilGetResourceName(_In_ HRESOURCE hResource,
                       _Out_writes_to_(*pcchResourceNameInOut,
...
typedef DWORD(WINAPI *PRESUTIL_GET_RESOURCE_NAME)(
    _In_ HRESOURCE hResource,
    _Out_writes_to_(*pcchResourceNameInOut, *pcchResourceNameInOut)
        PWSTR pszResourceName,
    _Inout_ DWORD *pcchResourceNameInOut);
...
_Success_(return >= 0) // != ClusterRoleUnknown
    ...
    ResUtilGetClusterRoleState(_In_ HCLUSTER hCluster,
                               _In_ CLUSTER_ROLE eClusterRole);

// TODO: should we implement versioning here like ClusApi - maybe overkill for
// now.
BOOL WINAPI ClusterIsPathOnSharedVolume(_In_ LPCWSTR lpszPathName);

typedef BOOL(WINAPI *PCLUSTER_IS_PATH_ON_SHARED_VOLUME)(
    _In_ LPCWSTR lpszPathName);

BOOL WINAPI ClusterGetVolumePathName(_In_ LPCWSTR lpszFileName,
                                     _Out_ LPWSTR lpszVolumePathName,
                                     _In_ DWORD cchBufferLength);

typedef BOOL(WINAPI *PCLUSTER_GET_VOLUME_PATH_NAME)(
    _In_ LPCWSTR lpszFileName, _Out_ LPWSTR lpszVolumePathName,
    _In_ DWORD cchBufferLength);

BOOL WINAPI ClusterGetVolumeNameForVolumeMountPoint(
    _In_ LPCWSTR lpszVolumeMountPoint, _Out_ LPWSTR lpszVolumeName,
    _In_ DWORD cchBufferLength);
...
DWORD
WINAPI
ClusterPrepareSharedVolumeForBackup(_In_ LPCWSTR lpszFileName,
                                    _Out_ LPWSTR lpszVolumePathName,
...
typedef DWORD(WINAPI *PCLUSTER_PREPARE_SHARED_VOLUME_FOR_BACKUP)(
    _In_ LPCWSTR lpszFileName, _Out_ LPWSTR lpszVolumePathName,
    _Inout_ LPDWORD lpcchVolumePathName, _Out_ LPWSTR lpszVolumeName,
    _Inout_ LPDWORD lpcchVolumeName);

DWORD
WINAPI
ClusterClearBackupStateForSharedVolume(_In_ LPCWSTR lpszVolumePathName);
...
DWORD
WINAPI
ResUtilSetResourceServiceStartParametersEx(IN LPCWSTR pszServiceName,
                                           IN SC_HANDLE schSCMHandle,
                                           IN OUT LPSC_HANDLE phService,
...
typedef DWORD(WINAPI *PRESUTIL_SET_RESOURCE_SERVICE_START_PARAMETERS_EX)(
    IN LPCWSTR pszServiceName, IN SC_HANDLE schSCMHandle,
    IN OUT LPSC_HANDLE phService, IN DWORD dwDesiredAccess,
    IN PLOG_EVENT_ROUTINE pfnLogEvent, IN RESOURCE_HANDLE hResourceHandle);

DWORD
WINAPI
ResUtilEnumResourcesEx2(IN HCLUSTER hCluster, IN HRESOURCE hSelf,
                        IN LPCWSTR lpszResTypeName,
                        IN LPRESOURCE_CALLBACK_EX pResCallBack,
                        IN PVOID pParameter, IN DWORD dwDesiredAccess);
...
HRESOURCE
WINAPI
ResUtilGetResourceDependencyEx(IN HANDLE hSelf, IN LPCWSTR lpszResourceType,
                               IN DWORD dwDesiredAccess);

typedef HRESOURCE(WINAPI *PRESUTIL_GET_RESOURCE_DEPENDENCY_EX)(
    IN HANDLE hSelf, IN LPCWSTR lpszResourceType, IN DWORD dwDesiredAccess);

HRESOURCE
WINAPI
ResUtilGetResourceDependencyByNameEx(IN HCLUSTER hCluster, IN HANDLE hSelf,
                                     IN LPCWSTR lpszResourceType,
                                     IN BOOL bRecurse,
                                     IN DWORD dwDesiredAccess);
...
typedef HRESOURCE(WINAPI *PRESUTIL_GET_RESOURCE_DEPENDENCY_BY_NAME_EX)(
    IN HCLUSTER hCluster, IN HANDLE hSelf, IN LPCWSTR lpszResourceType,
    IN BOOL bRecurse, IN DWORD dwDesiredAccess);

HRESOURCE
WINAPI
ResUtilGetResourceDependencyByClassEx(IN HCLUSTER hCluster, IN HANDLE hSelf,
                                      IN PCLUS_RESOURCE_CLASS_INFO prci,
                                      IN BOOL bRecurse,
                                      IN DWORD dwDesiredAccess);
...
HRESOURCE
WINAPI
ResUtilGetResourceNameDependencyEx(IN LPCWSTR lpszResourceName,
                                   IN LPCWSTR lpszResourceType,
                                   IN DWORD dwDesiredAccess);
...
ResUtilGetCoreClusterResourcesEx(
    _In_ HCLUSTER hClusterIn, _Out_opt_ HRESOURCE *phClusterNameResourceOut,
    _Out_opt_ HRESOURCE *phClusterQuorumResourceOut,
    _In_ DWORD dwDesiredAccess);

typedef DWORD(WINAPI *PRESUTIL_GET_CORE_CLUSTER_RESOURCES_EX)(
    _In_ HCLUSTER hClusterIn, _Out_ HRESOURCE *phClusterNameResourceOut,
    _Out_ HRESOURCE *phClusterIPAddressResourceOut,
    _Out_ HRESOURCE *phClusterQuorumResourceOut, _In_ DWORD dwDesiredAccess);
...
HCLUSCRYPTPROVIDER
WINAPI
OpenClusterCryptProvider(_In_ LPCWSTR lpszResource, _In_ LPCTSTR lpszProvider,
                         _In_ DWORD dwType, _In_ DWORD dwFlags);

typedef HCLUSCRYPTPROVIDER(WINAPI *POPEN_CLUSTER_CRYPT_PROVIDER)(
    _In_ LPCWSTR lpszResource, _In_ LPCTSTR lpszProvider, _In_ DWORD dwType,
    _In_ DWORD dwFlags);

HCLUSCRYPTPROVIDER
WINAPI
OpenClusterCryptProviderEx(_In_ LPCWSTR lpszResource, _In_ LPCWSTR lpszKeyname,
                           _In_ LPCTSTR lpszProvider, _In_ DWORD dwType,
                           _In_ DWORD dwFlags);

typedef HCLUSCRYPTPROVIDER(WINAPI *POPEN_CLUSTER_CRYPT_PROVIDEREX)(
    _In_ LPCWSTR lpszResource, _In_ LPCWSTR lpszKeyname,
    _In_ LPCTSTR lpszProvider, _In_ DWORD dwType, _In_ DWORD dwFlags);

DWORD
WINAPI
CloseClusterCryptProvider(_In_ HCLUSCRYPTPROVIDER hClusCryptProvider);
...
DWORD
WINAPI
ClusterEncrypt(_In_ HCLUSCRYPTPROVIDER hClusCryptProvider,
               _In_reads_(cbData) PBYTE pData, _In_ DWORD cbData,
               _Outptr_result_bytebuffer_(*pcbData) PBYTE *ppData,
               _Out_ PDWORD pcbData);

typedef DWORD(WINAPI *PCLUSTER_ENCRYPT)(
    _In_ HCLUSCRYPTPROVIDER hClusCryptProvider, _In_reads_(cbData) PBYTE pData,
    _In_ DWORD cbData, _Outptr_result_bytebuffer_(*pcbData) PBYTE *ppData,
    _Out_ PDWORD pcbData);

DWORD
WINAPI
ClusterDecrypt(_In_ HCLUSCRYPTPROVIDER hClusCryptProvider,
               _In_reads_(cbData) PBYTE pCryptInput, _In_ DWORD cbCryptInput,
...
DWORD
WINAPI
FreeClusterCrypt(_In_ PVOID pCryptInfo);

typedef DWORD(WINAPI *PFREE_CLUSTER_CRYPT)(_In_ PVOID pCryptInfo);

DWORD
WINAPI
ResUtilVerifyShutdownSafe(_In_ DWORD flags, _In_ DWORD reason,
                          _Out_ PDWORD pResult);

typedef DWORD(WINAPI *PRES_UTIL_VERIFY_SHUTDOWN_SAFE)(_In_ DWORD flags,
                                                      _In_ DWORD reason,
                                                      _Out_ PDWORD pResult);
...
BOOL ResUtilPaxosComparer(const PaxosTagCStruct *const left,
                          const PaxosTagCStruct *const right);

BOOL ResUtilLeftPaxosIsLessThanRight(const PaxosTagCStruct *const left,
                                     const PaxosTagCStruct *const right);
...
DWORD
WINAPI
ResUtilGroupsEqual(IN HGROUP hSelf, IN HGROUP hGroup, OUT BOOL *pEqual);

DWORD
WINAPI
ResUtilEnumGroups(IN HCLUSTER hCluster, IN HGROUP hSelf,
                  IN LPGROUP_CALLBACK_EX pResCallBack, IN PVOID pParameter);

DWORD
WINAPI
ResUtilEnumGroupsEx(IN HCLUSTER hCluster, IN HGROUP hSelf,
...
DWORD
WINAPI
ResUtilResourceDepEnum(IN HRESOURCE hSelf, IN DWORD enumType,
                       IN LPRESOURCE_CALLBACK_EX pResCallBack,
                       IN PVOID pParameter);
DWORD
WINAPI
...
DWORD
WINAPI
ResUtilNodeEnum(IN HCLUSTER hCluster, IN LPNODE_CALLBACK pNodeCallBack,
                IN PVOID pParameter);
