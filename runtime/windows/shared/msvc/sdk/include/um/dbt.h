 ...
 *
 *      Messages issued by IFSMGR for volume locking purposes on
 * WM_DEVICECHANGE. All these messages pass a pointer to a struct which has no
 * pointers.
 */
...
typedef struct _DEV_BROADCAST_HANDLE {
  ...
  //
  GUID dbch_eventguid;
  LONG dbch_nameoffset; // offset (bytes) of variable-length string buffer (-1
                        // if none)
  BYTE dbch_data[1];    // variable-sized buffer, potentially containing binary
                        // and/or text data
