...
EXTERN_C
FEATURE_ENABLED_STATE
GetFeatureEnabledState(UINT32 featureId, FEATURE_CHANGE_TIME changeTime);

EXTERN_C
void RecordFeatureUsage(UINT32 featureId, UINT32 kind, UINT32 addend,
                        _In_ PCSTR originName);

EXTERN_C
void RecordFeatureError(UINT32 featureId, _In_ const FEATURE_ERROR *error);

EXTERN_C
void SubscribeFeatureStateChangeNotification(
    _Outptr_ FEATURE_STATE_CHANGE_SUBSCRIPTION *subscription,
    _In_ PFEATURE_STATE_CHANGE_CALLBACK callback, _In_opt_ void *context);
...
EXTERN_C
UINT32
GetFeatureVariant(UINT32 featureId, FEATURE_CHANGE_TIME changeTime,
                  _Out_ UINT32 *payloadId, _Out_ BOOL *hasNotification);
