...
// during IHV DLL initialization.
typedef struct _DOT11EXT_IHV_HANDLERS DOT11EXT_IHV_HANDLERS,
    *PDOT11EXT_IHV_HANDLERS;

// Forward declaration for the
// API set for controlling virtual station.
typedef struct _DOT11EXT_VIRTUAL_STATION_APIS DOT11EXT_VIRTUAL_STATION_APIS,
...
typedef DWORD(WINAPI *DOT11EXT_ALLOCATE_BUFFER)(
    DWORD dwByteCount,
    _Outptr_result_bytebuffer_(dwByteCount) LPVOID *ppvBuffer);

typedef VOID(WINAPI *DOT11EXT_FREE_BUFFER)(_In_opt_ LPVOID pvMemory);

typedef DWORD(WINAPI *DOT11EXT_SET_PROFILE_CUSTOM_USER_DATA)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ HANDLE hConnectSession,
    DWORD dwSessionID, DWORD dwDataSize,
    _In_reads_bytes_(dwDataSize) LPVOID pvData);

typedef DWORD(WINAPI *DOT11EXT_GET_PROFILE_CUSTOM_USER_DATA)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ HANDLE hConnectSession,
    DWORD dwSessionID, _Out_ DWORD *pdwDataSize,
    _Outptr_result_bytebuffer_(*pdwDataSize) LPVOID *ppvData);

typedef DWORD(WINAPI *DOT11EXT_SET_CURRENT_PROFILE)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ HANDLE hConnectSession,
    _In_opt_ PDOT11EXT_IHV_CONNECTIVITY_PROFILE pIhvConnProfile,
    _In_opt_ PDOT11EXT_IHV_SECURITY_PROFILE pIhvSecProfile);

typedef DWORD(WINAPI *DOT11EXT_SEND_UI_REQUEST)(_In_opt_ HANDLE hDot11SvcHandle,
                                                _In_ PDOT11EXT_IHV_UI_REQUEST
                                                    pIhvUIRequest);

typedef DWORD(WINAPI *DOT11EXT_PRE_ASSOCIATE_COMPLETION)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ HANDLE hConnectSession,
    DWORD dwReasonCode, DWORD dwWin32Error);

typedef DWORD(WINAPI *DOT11EXT_POST_ASSOCIATE_COMPLETION)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ HANDLE hSecuritySessionID,
    _In_opt_ PDOT11_MAC_ADDRESS pPeer, DWORD dwReasonCode, DWORD dwWin32Error);

typedef DWORD(WINAPI *DOT11EXT_SEND_NOTIFICATION)(
    _In_opt_ HANDLE hDot11SvcHandle,
    _In_ PL2_NOTIFICATION_DATA pNotificationData);

typedef DWORD(WINAPI *DOT11EXT_SEND_PACKET)(_In_opt_ HANDLE hDot11SvcHandle,
                                            ULONG uPacketLen,
                                            _In_reads_bytes_(uPacketLen)
                                                LPVOID pvPacket,
                                            _In_opt_ HANDLE hSendCompletion);

typedef DWORD(WINAPI *DOT11EXT_SET_ETHERTYPE_HANDLING)(
    _In_opt_ HANDLE hDot11SvcHandle, ULONG uMaxBackLog, ULONG uNumOfExemption,
...
typedef DWORD(WINAPI *DOT11EXT_SET_AUTH_ALGORITHM)(
    _In_opt_ HANDLE hDot11SvcHandle, DWORD dwAuthAlgo);

typedef DWORD(WINAPI *DOT11EXT_SET_UNICAST_CIPHER_ALGORITHM)(
    _In_opt_ HANDLE hDot11SvcHandle, DWORD dwUnicastCipherAlgo);

typedef DWORD(WINAPI *DOT11EXT_SET_MULTICAST_CIPHER_ALGORITHM)(
    _In_opt_ HANDLE hDot11SvcHandle, DWORD dwMulticastCipherAlgo);

typedef DWORD(WINAPI *DOT11EXT_SET_DEFAULT_KEY)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_ PDOT11_CIPHER_DEFAULT_KEY_VALUE pKey,
    DOT11_DIRECTION dot11Direction);

typedef DWORD(WINAPI *DOT11EXT_SET_KEY_MAPPING_KEY)(
    _In_opt_ HANDLE hDot11SvcHandle,
    _In_ PDOT11_CIPHER_KEY_MAPPING_KEY_VALUE pKey);

typedef DWORD(WINAPI *DOT11EXT_SET_DEFAULT_KEY_ID)(
    _In_opt_ HANDLE hDot11SvcHandle, ULONG uDefaultKeyId);

typedef DWORD(WINAPI *DOT11EXT_SET_EXCLUDE_UNENCRYPTED)(
    _In_opt_ HANDLE hDot11SvcHandle, BOOL bExcludeUnencrypted);

typedef DWORD(WINAPI *DOT11EXT_NIC_SPECIFIC_EXTENSION)(
    _In_opt_ HANDLE hDot11SvcHandle, DWORD dwInBufferSize,
    _In_reads_bytes_(dwInBufferSize) LPVOID pvInBuffer,
    _Inout_ DWORD *pdwOutBufferSize,
    _Out_writes_bytes_to_opt_(*pdwOutBufferSize, *pdwOutBufferSize)
        LPVOID pvOutBuffer);
...
typedef DWORD(WINAPI *DOT11EXT_ONEX_START)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ EAP_ATTRIBUTES *pEapAttributes);

typedef DWORD(WINAPI *DOT11EXT_ONEX_STOP)(_In_opt_ HANDLE hDot11SvcHandle);

typedef DWORD(WINAPI *DOT11EXT_PROCESS_ONEX_PACKET)(
    _In_opt_ HANDLE hDot11SvcHandle, DWORD dwInPacketSize,
...
typedef DWORD(WINAPI *DOT11EXT_REQUEST_VIRTUAL_STATION)(
    _In_opt_ HANDLE hDot11PrimaryHandle, _Reserved_ LPVOID pvReserved);

typedef DWORD(WINAPI *DOT11EXT_RELEASE_VIRTUAL_STATION)(
    _In_opt_ HANDLE hDot11PrimaryHandle, _Reserved_ LPVOID pvReserved);

typedef DWORD(WINAPI *DOT11EXT_QUERY_VIRTUAL_STATION_PROPERTIES)(
    _In_opt_ HANDLE hDot11SvcHandle, _Out_ BOOL *pbIsVirtualStation,
    _Out_ GUID *pgPrimary, _Reserved_ LPVOID pvReserved);

typedef DWORD(WINAPI *DOT11EXT_SET_VIRTUAL_STATION_AP_PROPERTIES)(
    _In_opt_ HANDLE hDot11SvcHandle, _In_opt_ HANDLE hConnectSession,
    DWORD dwNumProperties,
    _In_reads_(dwNumProperties)
...
typedef DWORD(WINAPI *DOT11EXTIHV_GET_VERSION_INFO)(
    _Out_ PDOT11_IHV_VERSION_INFO pDot11IHVVersionInfo);

typedef DWORD(WINAPI *DOT11EXTIHV_INIT_SERVICE)(
    DWORD dwVerNumUsed, _In_ PDOT11EXT_APIS pDot11ExtAPI,
    _Reserved_ LPVOID pvReserved,
    _Out_ PDOT11EXT_IHV_HANDLERS pDot11IHVHandlers);

typedef DWORD(WINAPI *DOT11EXTIHV_INIT_VIRTUAL_STATION)(
    _In_ PDOT11EXT_VIRTUAL_STATION_APIS pDot11ExtVSAPI,
    _Reserved_ LPVOID pvReserved);

typedef VOID(WINAPI *DOT11EXTIHV_DEINIT_SERVICE)(VOID);

typedef DWORD(WINAPI *DOT11EXTIHV_INIT_ADAPTER)(
    _In_ PDOT11_ADAPTER pDot11Adapter, _In_opt_ HANDLE hDot11SvcHandle,
    _Out_ PHANDLE phIhvExtAdapter);

typedef VOID(WINAPI *DOT11EXTIHV_DEINIT_ADAPTER)(
    _In_opt_ HANDLE hIhvExtAdapter);

typedef DWORD(WINAPI *DOT11EXTIHV_PERFORM_PRE_ASSOCIATE)(
    _In_opt_ HANDLE hIhvExtAdapter, _In_opt_ HANDLE hConnectSession,
    ...
    _In_ PDOT11_BSS_LIST pConnectableBssid, _Out_ PDWORD pdwReasonCode);

typedef DWORD(WINAPI *DOT11EXTIHV_ADAPTER_RESET)(
    _In_opt_ HANDLE hIhvExtAdapter);

typedef DWORD(WINAPI *DOT11EXTIHV_PERFORM_POST_ASSOCIATE)(
    _In_opt_ HANDLE hIhvExtAdapter, _In_opt_ HANDLE hSecuritySessionID,
    _In_ PDOT11_PORT_STATE pPortState, ULONG uDot11AssocParamsBytes,
    _In_reads_bytes_(uDot11AssocParamsBytes)
        PDOT11_ASSOCIATION_COMPLETION_PARAMETERS pDot11AssocParams);

typedef DWORD(WINAPI *DOT11EXTIHV_STOP_POST_ASSOCIATE)(
    _In_opt_ HANDLE hIhvExtAdapter, _In_ PDOT11_MAC_ADDRESS pPeer,
    DOT11_ASSOC_STATUS dot11AssocStatus);

typedef DWORD(WINAPI *DOT11EXTIHV_VALIDATE_PROFILE)(
    _In_opt_ HANDLE hIhvExtAdapter,
    _In_opt_ PDOT11EXT_IHV_PROFILE_PARAMS pIhvProfileParams,
    _In_ PDOT11EXT_IHV_CONNECTIVITY_PROFILE pIhvConnProfile,
    _In_ PDOT11EXT_IHV_SECURITY_PROFILE pIhvSecProfile,
...
typedef DWORD(WINAPI *DOT11EXTIHV_PERFORM_CAPABILITY_MATCH)(
    _In_opt_ HANDLE hIhvExtAdapter,
    _In_opt_ PDOT11EXT_IHV_PROFILE_PARAMS pIhvProfileParams,
    _In_ PDOT11EXT_IHV_CONNECTIVITY_PROFILE pIhvConnProfile,
    _In_ PDOT11EXT_IHV_SECURITY_PROFILE pIhvSecProfile,
    _In_opt_ PDOT11_BSS_LIST pConnectableBssid, _Out_ PDWORD pdwReasonCode);

typedef DWORD(WINAPI *DOT11EXTIHV_CREATE_DISCOVERY_PROFILES)(
    _In_opt_ HANDLE hIhvExtAdapter, BOOL bInsecure,
    _In_opt_ PDOT11EXT_IHV_PROFILE_PARAMS pIhvProfileParams,
    _In_opt_ PDOT11_BSS_LIST pConnectableBssid,
    _Out_ PDOT11EXT_IHV_DISCOVERY_PROFILE_LIST pIhvDiscoveryProfileList,
    _Out_ PDWORD pdwReasonCode);

typedef DWORD(WINAPI *DOT11EXTIHV_PROCESS_SESSION_CHANGE)(
    ULONG uEventType, _In_ PWTSSESSION_NOTIFICATION pSessionNotification);

typedef DWORD(WINAPI *DOT11EXTIHV_RECEIVE_INDICATION)(
    _In_opt_ HANDLE hIhvExtAdapter, DOT11EXT_IHV_INDICATION_TYPE indicationType,
    ULONG uBufferLength, _In_reads_bytes_opt_(uBufferLength) LPVOID pvBuffer);

typedef DWORD(WINAPI *DOT11EXTIHV_RECEIVE_PACKET)(
    _In_opt_ HANDLE hIhvExtAdapter, DWORD dwInBufferSize,
    _In_reads_bytes_(dwInBufferSize) LPVOID pvInBuffer);

typedef DWORD(WINAPI *DOT11EXTIHV_SEND_PACKET_COMPLETION)(
    _In_opt_ HANDLE hSendCompletion);

typedef DWORD(WINAPI *DOT11EXTIHV_IS_UI_REQUEST_PENDING)(
    GUID guidUIRequest, _Out_ PBOOL pbIsRequestPending);

typedef DWORD(WINAPI *DOT11EXTIHV_PROCESS_UI_RESPONSE)(
    GUID guidUIRequest, DWORD dwByteCount,
    _In_reads_bytes_opt_(dwByteCount) LPVOID pvResponseBuffer);

typedef DWORD(WINAPI *DOT11EXTIHV_QUERY_UI_REQUEST)(
    _In_opt_ HANDLE hIhvExtAdapter,
    DOT11EXT_IHV_CONNECTION_PHASE connectionPhase,
    _Outptr_ PDOT11EXT_IHV_UI_REQUEST *ppIhvUIRequest);

typedef DWORD(WINAPI *DOT11EXTIHV_ONEX_INDICATE_RESULT)(
    _In_opt_ HANDLE hIhvExtAdapter, DOT11_MSONEX_RESULT msOneXResult,
    _In_opt_ PDOT11_MSONEX_RESULT_PARAMS pDot11MsOneXResultParams);

typedef DWORD(WINAPI *DOT11EXTIHV_CONTROL)(
    _In_opt_ HANDLE hIhvExtAdapter, DWORD dwInBufferSize,
    _In_reads_bytes_opt_(dwInBufferSize) PBYTE pInBuffer, DWORD dwOutBufferSize,
    _Out_writes_bytes_to_opt_(dwOutBufferSize, *pdwBytesReturned)
        PBYTE pOutBuffer,
