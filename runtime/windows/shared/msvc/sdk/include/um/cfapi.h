...
STDAPI
CfGetPlatformInfo(_Out_ CF_PLATFORM_INFO *PlatformVersion);
...
STDAPI
CfConnectSyncRoot(_In_ LPCWSTR SyncRootPath,
...
STDAPI
CfDisconnectSyncRoot(_In_ CF_CONNECTION_KEY ConnectionKey);

STDAPI
CfGetTransferKey(_In_ HANDLE FileHandle, _Out_ CF_TRANSFER_KEY *TransferKey);

STDAPI_(VOID)
CfReleaseTransferKey(_In_ HANDLE FileHandle,
...
STDAPI
CfExecute(_In_ CONST CF_OPERATION_INFO *OpInfo,
          _Inout_ CF_OPERATION_PARAMETERS *OpParams);

STDAPI
CfUpdateSyncProviderStatus(_In_ CF_CONNECTION_KEY ConnectionKey,
                           _In_ CF_SYNC_PROVIDER_STATUS ProviderStatus);

STDAPI
CfQuerySyncProviderStatus(_In_ CF_CONNECTION_KEY ConnectionKey,
                          _Out_ CF_SYNC_PROVIDER_STATUS *ProviderStatus);

#pragma endregion

STDAPI
CfReportSyncStatus(_In_ LPCWSTR SyncRootPath,
...
STDAPI
CfCreatePlaceholders(_In_ LPCWSTR BaseDirectoryPath,
                     _Inout_updates_(PlaceholderCount)
                         CF_PLACEHOLDER_CREATE_INFO *PlaceholderArray,
...
STDAPI
CfOpenFileWithOplock(_In_ LPCWSTR FilePath, _In_ CF_OPEN_FILE_FLAGS Flags,
                     _Out_ PHANDLE ProtectedHandle);

STDAPI_(BOOLEAN)
CfReferenceProtectedHandle(_In_ HANDLE ProtectedHandle);

STDAPI_(HANDLE)
CfGetWin32HandleFromProtectedHandle(_In_ HANDLE ProtectedHandle);

STDAPI_(VOID)
CfReleaseProtectedHandle(_In_ HANDLE ProtectedHandle);

STDAPI_(VOID)
CfCloseHandle(_In_ HANDLE FileHandle);
...
STDAPI
CfConvertToPlaceholder(_In_ HANDLE FileHandle,
                       _In_reads_bytes_opt_(FileIdentityLength)
                           LPCVOID FileIdentity,
...
STDAPI
CfUpdatePlaceholder(
    _In_ HANDLE FileHandle, _In_opt_ CONST CF_FS_METADATA *FsMetadata,
    _In_reads_bytes_opt_(FileIdentityLength) LPCVOID FileIdentity,
    _In_ DWORD FileIdentityLength,
    _In_reads_opt_(DehydrateRangeCount)
        CONST CF_FILE_RANGE *DehydrateRangeArray,
    _In_ DWORD DehydrateRangeCount, _In_ CF_UPDATE_FLAGS UpdateFlags,
    _Inout_opt_ USN *UpdateUsn, _Inout_opt_ LPOVERLAPPED Overlapped);
...
STDAPI
CfRevertPlaceholder(_In_ HANDLE FileHandle, _In_ CF_REVERT_FLAGS RevertFlags,
                    _Inout_opt_ LPOVERLAPPED Overlapped);
...
STDAPI
CfHydratePlaceholder(_In_ HANDLE FileHandle, _In_ LARGE_INTEGER StartingOffset,
...
STDAPI
CfDehydratePlaceholder(_In_ HANDLE FileHandle,
...
STDAPI
CfSetPinState(_In_ HANDLE FileHandle, _In_ CF_PIN_STATE PinState,
...
STDAPI
CfSetInSyncState(_In_ HANDLE FileHandle, _In_ CF_IN_SYNC_STATE InSyncState,
                 _In_ CF_SET_IN_SYNC_FLAGS InSyncFlags,
                 _Inout_opt_ USN *InSyncUsn);

STDAPI
CfSetCorrelationVector(_In_ HANDLE FileHandle,
                       _In_ const PCORRELATION_VECTOR CorrelationVector);

STDAPI
CfGetCorrelationVector(_In_ HANDLE FileHandle,
...
CfGetPlaceholderStateFromAttributeTag(_In_ DWORD FileAttributes,
                                      _In_ DWORD ReparseTag);

STDAPI_(CF_PLACEHOLDER_STATE)
CfGetPlaceholderStateFromFileInfo(_In_ LPCVOID InfoBuffer,
                                  _In_ FILE_INFO_BY_HANDLE_CLASS InfoClass);

STDAPI_(CF_PLACEHOLDER_STATE)
CfGetPlaceholderStateFromFindData(_In_ CONST WIN32_FIND_DATA *FindData);
...
STDAPI
CfGetPlaceholderInfo(_In_ HANDLE FileHandle,
...
STDAPI
CfGetSyncRootInfoByPath(_In_ LPCWSTR FilePath,
                        _In_ CF_SYNC_ROOT_INFO_CLASS InfoClass,
                        _Out_ PVOID InfoBuffer, _In_ DWORD InfoBufferLength,
                        _Out_opt_ DWORD *ReturnedLength);

STDAPI
CfGetSyncRootInfoByHandle(_In_ HANDLE FileHandle,
                          _In_ CF_SYNC_ROOT_INFO_CLASS InfoClass,
                          _Out_ PVOID InfoBuffer, _In_ DWORD InfoBufferLength,
                          _Out_opt_ DWORD *ReturnedLength);
...
STDAPI
CfGetPlaceholderRangeInfo(_In_ HANDLE FileHandle,
...
CfGetPlaceholderRangeInfoForHydration(
    ...
    _In_ LARGE_INTEGER FileId, _In_ CF_PLACEHOLDER_RANGE_INFO_CLASS InfoClass,
    _In_ LARGE_INTEGER StartingOffset, _In_ LARGE_INTEGER RangeLength,
    _Out_writes_bytes_to_(InfoBufferSize, *InfoBufferWritten) PVOID InfoBuffer,
    _In_ DWORD InfoBufferSize, _Out_opt_ PDWORD InfoBufferWritten);
...
STDAPI
CfReportProviderProgress(_In_ CF_CONNECTION_KEY ConnectionKey,
...
STDAPI
CfReportProviderProgress2(_In_ CF_CONNECTION_KEY ConnectionKey,
                          _In_ CF_TRANSFER_KEY TransferKey,
