...
DECLARE_INTERFACE_IID_(INewShortcutHookA, IUnknown,
  ...
  STDMETHOD(SetReferent)(THIS_ _In_ PCSTR pcszReferent, _In_opt_ HWND hwnd)
      PURE;
  STDMETHOD(GetReferent)(THIS_ _Out_writes_(cchReferent) PSTR pszReferent,
                         int cchReferent) PURE;
  STDMETHOD(SetFolder)(THIS_ _In_ PCSTR pcszFolder) PURE;
  STDMETHOD(GetFolder)(THIS_ _Out_writes_(cchFolder) PSTR pszFolder,
                       int cchFolder) PURE;
  STDMETHOD(GetName)(THIS_ _Out_writes_(cchName) PSTR pszName, int cchName)
      PURE;
  STDMETHOD(GetExtension)(THIS_ _Out_writes_(cchExtension) PSTR pszExtension,
...
DECLARE_INTERFACE_IID_(INewShortcutHookW, IUnknown,
  ...
  STDMETHOD_(ULONG, AddRef)(THIS) PURE;
  STDMETHOD_(ULONG, Release)(THIS) PURE;
  STDMETHOD(SetReferent)(THIS_ _In_ PCWSTR pcszReferent, _In_opt_ HWND hwnd)
      PURE;
  STDMETHOD(GetReferent)(THIS_ _Out_writes_(cchReferent) PWSTR pszReferent,
                         int cchReferent) PURE;
  STDMETHOD(SetFolder)(THIS_ _In_ PCWSTR pcszFolder) PURE;
  STDMETHOD(GetFolder)(THIS_ _Out_writes_(cchFolder) PWSTR pszFolder,
                       int cchFolder) PURE;
  STDMETHOD(GetName)(THIS_ _Out_writes_(cchName) PWSTR pszName, int cchName)
      PURE;
  STDMETHOD(GetExtension)(THIS_ _Out_writes_(cchExtension) PWSTR pszExtension,
                          int cchExtension) PURE;
};
...
DECLARE_INTERFACE_IID_(ICopyHookA, IUnknown,
  ...
  STDMETHOD_(UINT, CopyCallback)(
      THIS_ _In_opt_ HWND hwnd, UINT wFunc, UINT wFlags, _In_ PCSTR pszSrcFile,
      DWORD dwSrcAttribs, _In_opt_ PCSTR pszDestFile, DWORD dwDestAttribs) PURE;
};
...
DECLARE_INTERFACE_IID_(ICopyHookW, IUnknown,
  ...
  STDMETHOD_(UINT, CopyCallback)(
      THIS_ _In_opt_ HWND hwnd, UINT wFunc, UINT wFlags, _In_ PCWSTR pszSrcFile,
      DWORD dwSrcAttribs, _In_opt_ PCWSTR pszDestFile, DWORD dwDestAttribs)
      PURE;
};
...
DECLARE_INTERFACE_IID_(ICurrentWorkingDirectory, IUnknown,
  ...
  STDMETHOD(GetDirectory)(THIS_ _Out_writes_(cchSize) PWSTR pwzPath,
                          DWORD cchSize) PURE;
...
#define DWFAF_AUTOHIDE                                                         \
  0x0010 // The toolbar will be subject to AutoHide in Full Screen mode
...
DECLARE_INTERFACE_IID_(IDockingWindowFrame, IOleWindow,
  ...
  STDMETHOD(AddToolbar)(THIS_ _In_ IUnknown * punkSrc, _In_ PCWSTR pwszItem,
                        DWORD dwAddFlags) PURE;
  STDMETHOD(RemoveToolbar)(THIS_ _In_ IUnknown * punkSrc, DWORD dwRemoveFlags)
      PURE;
  STDMETHOD(FindToolbar)(THIS_ _In_ PCWSTR pwszItem, _In_ REFIID riid,
                         _Outptr_ void **ppv) PURE;
};

/* ***************** IThumbnailCapture
 * CaptureThumbnail : takes an IHTMLDocument2 and returns a thumbnail of
 * specified size as an hbitmap
 */
...
DECLARE_INTERFACE_IID_(IEnumShellImageStore, IUnknown,
  ...
  STDMETHOD(Next)(THIS_ ULONG celt,
                  _Out_writes_(*pceltFetched) PENUMSHELLIMAGESTOREDATA * prgElt,
                  _Out_opt_ ULONG * pceltFetched) PURE;
...
#define SHIMSTCAPFLAG_PURGEABLE                                                \
  0x0002 // does the store require dead items purging externally ?

#undef INTERFACE
#define INTERFACE IShellImageStore

// this interface is used to manipulate the Image cache. It can potentially be
// used in a free threaded manner in conjunction with the Lock parameter to Open
// and close
DECLARE_INTERFACE_IID_(IShellImageStore, IUnknown,
  ...
  STDMETHOD(AddEntry)(THIS_ _In_ PCWSTR pszName,
                      _In_ const FILETIME *pftTimeStamp, DWORD dwMode,
                      _In_ HBITMAP hImage) PURE;
  STDMETHOD(GetEntry)(THIS_ _In_ PCWSTR pszName, DWORD dwMode,
                      _Out_ HBITMAP * phImage) PURE;
  STDMETHOD(DeleteEntry)(THIS_ _In_ PCWSTR pszName) PURE;
  STDMETHOD(IsEntryInStore)(THIS_ _In_ PCWSTR pszName,
                            _Out_ FILETIME * pftTimeStamp) PURE;
...
#define ISFB_MASK_COLORS                                                       \
  0x00000020 // TRUE if crXXXX fields are valid (except bkgnd)
...
#define ISFB_STATE_CHANNELBAR                                                  \
  0x00000010 // TRUE if we want NavigateTarget support
#define ISFB_STATE_QLINKSMODE                                                  \
  0x00000020 // TRUE if we want to turn off drag & drop onto content items
#define ISFB_STATE_FULLOPEN                                                    \
  0x00000040 // TRUE if band should maximize when opened
#define ISFB_STATE_NONAMESORT                                                  \
  0x00000080 // TRUE if band should _not_ sort icons by name
#define ISFB_STATE_BTNMINSIZE                                                  \
  0x00000100 // TRUE if band should report min thickness of button
...
DECLARE_INTERFACE_IID_(IShellFolderBand, IUnknown,
  ...
  STDMETHOD(InitializeSFB)(THIS_ _In_opt_ IShellFolder * psf,
                           _In_opt_ PCIDLIST_ABSOLUTE pidl) PURE;
...
#define DBC_GS_SIZEDOWN                                                        \
  1 // clip the height of a rect to a multiple of the rebar's integral size
...
enum {
  ...
  DBCID_UPDATESIZE =
      5, // resize from non-user source (search activation failure)
};
...
#define SCHEME_DONOTUSE                                                        \
  0x0040 // used to be SCHEME_ENUMERATE; no longer supported
...
DECLARE_INTERFACE_IID_(IActiveDesktopP, IUnknown,
  ...
  STDMETHOD(GetScheme)(THIS_ _Out_writes_(*pdwcchBuffer) PWSTR pwszSchemeName,
                       _Inout_ DWORD * pdwcchBuffer, DWORD dwFlags) PURE;
};
...
typedef struct {
  ...
  WCHAR wszDescription[MAX_COLUMN_DESC_LEN]; // OUT full description of this
                                             // column
...
typedef struct {
  ...
  WCHAR wszFolder[MAX_PATH]; // fully qualified folder path (or empty if
                             // multiple folders)
} SHCOLUMNINIT, *LPSHCOLUMNINIT;
typedef const SHCOLUMNINIT *LPCSHCOLUMNINIT;

#define SHCDF_UPDATEITEM                                                       \
  0x00000001 // this flag is a hint that the file has changed since the last
             // call to GetItemData
...
DECLARE_INTERFACE_IID_(IColumnProvider, IUnknown,
  ...
  STDMETHOD(GetItemData)(THIS_ _In_ LPCSHCOLUMNID pscid,
                         _In_ LPCSHCOLUMNDATA pscd, _Out_ VARIANT * pvarData)
      PURE;
};
...
//  end the call with SCNRT_DISABLE can block while it synchronizes with the
//  main ChangeNotify thread
...
WINSHELLAPI HPSXA WINAPI SHCreatePropSheetExtArray(_In_ HKEY hKey,
...
#define PPCF_ADDARGUMENTS                                                      \
  0x00000003 // appends arguments (and wraps in quotes if required)
...
SHSTDAPI_(LONG)
PathProcessCommand(_In_ PCWSTR pszSrc, _Out_writes_(cchDest) PWSTR pszDest,
                   int cchDest, DWORD dwFlags);
#endif
...
SHSTDAPI_(BOOL)
SHOpenPropSheetA(_In_opt_ LPCSTR pszCaption,
                 _In_reads_opt_(ckeys) HKEY ahkeys[], UINT ckeys,
                 _In_opt_ const CLSID *pclsidDefault, _In_ IDataObject *pdtobj,
                 _In_opt_ IShellBrowser *psb, _In_opt_ LPCSTR pStartPage);
SHSTDAPI_(BOOL)
SHOpenPropSheetW(_In_opt_ LPCWSTR pszCaption,
                 _In_reads_opt_(ckeys) HKEY ahkeys[], UINT ckeys,
                 _In_opt_ const CLSID *pclsidDefault, _In_ IDataObject *pdtobj,
...
//    szDistUnit          - [in] Unique identifier string for a code
//    distribution unit. For
//                               ActiveX controls and Active Setup installed
//                               components, this is typically a GUID string.
//    dwFlags             - [in] Must be 0.
//    psdi                - [in,out] Pointer to SOFTDISTINFO ( see URLMon.h ).
//    May be NULL.
//                                cbSize should be initialized
//                                by the caller to sizeof(SOFTDISTINFO),
//                                dwReserved should be set to 0.
//
//    RETURNS:
//
//    IDNO     - The user chose cancel. If *pbRemind is FALSE, the caller should
//    save the
//               update version from the SOFTDISTINFO and pass it in as the
//               Advertised version in future calls.
//
//    IDYES    - The user has selected Update Now/About Update. The caller
//    should navigate to
//               the SOFTDISTINFO's pszHREF to initiate the install or learn
//               about it. The caller should save the update version from the
//               SOFTDISTINFO and pass it in as the Advertised version in future
//               calls.
//
//    IDIGNORE - There is no pending software update. Note: There is
//               no Ignore button in the standard UI. This occurs if the
//               available version is less than the installed version or is not
//               present or if the Advertised version is greater than or equal
//               to the update version.
//
//    IDABORT  - An error occured. Call GetSoftwareUpdateInfo() for a more
//    specific HRESULT.
//               Note: There is no Abort button in the standard UI.

SHDOCAPI_(DWORD)
SoftwareUpdateMessageBox(_In_opt_ HWND hWnd, _In_ PCWSTR pszDistUnit,
                         DWORD dwFlags, _Out_opt_ LPSOFTDISTINFO psdi);
...
SHSTDAPI SHCreateQueryCancelAutoPlayMoniker(
    _Outptr_ IMoniker *
        *ppmoniker); // deprecated: use CreateClassMoniker(CLSID_YourOwnClsid,
                     // ...)
STDAPI_(void) PerUserInit(void);
SHSTDAPI_(BOOL)
SHRunControlPanel(_In_ PCWSTR lpcszCmdLine, _In_opt_ HWND hwndMsgParent);
...
SHDOCAPI_(BOOL)
ImportPrivacySettings(_In_ PCWSTR pszFilename,
...
#endif
SHDOCAPI DoPrivacyDlg(_In_opt_ HWND hwndOwner, _In_ PCWSTR pszUrl,
