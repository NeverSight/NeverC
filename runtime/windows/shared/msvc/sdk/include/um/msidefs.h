...
#define IPROPNAME_PRODUCTCODE                                                  \
  TEXT("ProductCode") // unique string GUID for product
#define IPROPNAME_PRODUCTVERSION                                               \
  TEXT("ProductVersion") // string product version
#define IPROPNAME_INSTALLLANGUAGE                                              \
  TEXT("ProductLanguage") // install language of product, use to load resources
#define IPROPNAME_MANUFACTURER TEXT("Manufacturer") // name of manufacturer

// Customization properties: set in Property table
#define IPROPNAME_UPGRADECODE                                                  \
  TEXT("UpgradeCode") // unique string GUID for product family
#define IPROPNAME_PIDTEMPLATE                                                  \
  TEXT("PIDTemplate")                           // drives Product ID processing
#define IPROPNAME_DISKPROMPT TEXT("DiskPrompt") // prompt for CD
#define IPROPNAME_LEFTUNIT                                                     \
  TEXT("LeftUnit") // name of unit placed to left of number instead of right
#define IPROPNAME_ADMIN_PROPERTIES                                             \
  TEXT("AdminProperties") // properties to stuff in admin package
#define IPROPNAME_DEFAULTUIFONT                                                \
  TEXT("DefaultUIFont") // the font used in the UI if no other font is specified
#define IPROPNAME_ALLOWEDPROPERTIES TEXT("SecureCustomProperties")
#define IPROPNAME_ENABLEUSERCONTROL                                            \
  TEXT("EnableUserControl") // allows user to specify any public property
#define IPROPNAME_HIDDEN_PROPERTIES                                            \
  TEXT("MsiHiddenProperties") // properties that should not be dumped into the
                              // log file
...
#define IPROPNAME_PIDKEY                                                       \
  TEXT("PIDKEY") // used with PIDTemplate to form ProductID
#define IPROPNAME_PATCH                                                        \
  TEXT("PATCH") // patch package to apply - SET BY INSTALLER
#define IPROPNAME_MSIPATCHREMOVE                                               \
  TEXT("MSIPATCHREMOVE") // patch package to remove - SET BY INSTALLER
#define IPROPNAME_TARGETDIR                                                    \
  TEXT("TARGETDIR") // target location - defaults to ROOTDRIVE
#define IPROPNAME_ACTION                                                       \
  TEXT("ACTION") // top-level action to perform - default to INSTALL
...
#if (_WIN32_MSI >= 110)
#define IPROPNAME_REBOOTPROMPT                                                 \
  TEXT("REBOOTPROMPT") // allow or suppress reboot prompt
...
#define IPROPVALUE_EXECUTEMODE_SCRIPT                                          \
  TEXT("SCRIPT") // default - run script to update system
#define IPROPNAME_EXECUTEACTION                                                \
  TEXT("EXECUTEACTION") // run action on server side
#define IPROPNAME_SOURCELIST TEXT("SOURCELIST")
#define IPROPNAME_ROOTDRIVE                                                    \
  TEXT("ROOTDRIVE") // default drive to install - SET BY INSTALLER
#define IPROPNAME_TRANSFORMS TEXT("TRANSFORMS") // transforms to apply
#define IPROPNAME_TRANSFORMSATSOURCE                                           \
  TEXT("TRANSFORMSATSOURCE") // transforms can be found at source
#define IPROPNAME_TRANSFORMSSECURE                                             \
  TEXT("TRANSFORMSSECURE") // file transforms are secured
#define IPROPNAME_SEQUENCE                                                     \
  TEXT("SEQUENCE") // sequence table to run with SEQUENCE action
#define IPROPNAME_SHORTFILENAMES                                               \
  TEXT("SHORTFILENAMES") // force short file names
#define IPROPNAME_PRIMARYFOLDER                                                \
  TEXT("PRIMARYFOLDER") // Folder on the volume the author wants costing info
                        // for
#define IPROPNAME_AFTERREBOOT                                                  \
  TEXT("AFTERREBOOT") // install is after a ForceReboot triggered reboot
#define IPROPNAME_NOCOMPANYNAME TEXT("NOCOMPANYNAME")
#define IPROPNAME_NOUSERNAME TEXT("NOUSERNAME")
#define IPROPNAME_DISABLEROLLBACK                                              \
  TEXT("DISABLEROLLBACK") // disable rollback for this install
#define IPROPNAME_AVAILABLEFREEREG                                             \
  TEXT("AVAILABLEFREEREG") // set up the free space in the registry before
                           // commencing the install
#define IPROPNAME_DISABLEADVTSHORTCUTS                                         \
  TEXT("DISABLEADVTSHORTCUTS") // disable creating darwin shortcuts even if
                               // supported
#define IPROPNAME_PATCHNEWPACKAGECODE                                          \
  TEXT("PATCHNEWPACKAGECODE") // added to property table by patch transforms -
                              // used to update PackageCode of admin packages
                              // when patching admin installs
#define IPROPNAME_PATCHNEWSUMMARYSUBJECT                                       \
  TEXT("PATCHNEWSUMMARYSUBJECT") // added to property table by patch transforms
                                 // - used to update Subject summary info
                                 // property of admin packages when patching
                                 // admin installs
#define IPROPNAME_PATCHNEWSUMMARYCOMMENTS                                      \
  TEXT("PATCHNEWSUMMARYCOMMENTS") // added to property table by patch transforms
                                  // - used to update Comments summary info
                                  // property of admin packages when patching
                                  // admin installs
#define IPROPNAME_PRODUCTLANGUAGE                                              \
  TEXT("PRODUCTLANGUAGE") // requested language, must be one in summary
                          // information list, selects language transform

#if (_WIN32_MSI >= 150)
#define IPROPNAME_CHECKCRCS                                                    \
  TEXT("MSICHECKCRCS") // requests Darwin to check CRCs after copying, moving,
                       // patching & duplicating files.
#define IPROPNAME_MSINODISABLEMEDIA                                            \
  TEXT("MSINODISABLEMEDIA") // if set, DISABLEMEDIA won't be set in the
                            // AdminProperties stream during an admin install of
                            // a package with compressed source

// property used for URT bootstrapping
#define IPROPNAME_CARRYINGNDP TEXT("CARRYINGNDP")
#define IPROPVALUE__CARRYINGNDP_URTREINSTALL                                   \
  TEXT("URTREINSTALL") // reinstalling/ uninstalling core URT files
#define IPROPVALUE__CARRYINGNDP_URTUPGRADE                                     \
  TEXT("URTUPGRADE") // upgrading core URT files
#define IPROPNAME_ENFORCE_UPGRADE_COMPONENT_RULES                              \
  TEXT(                                                                        \
      "MSIENFORCEUPGRADECOMPONENTRULES") // if set, doesn't allow upgrade
                                         // packages to break component rules.
...
#if (_WIN32_MSI >= 300)
// properties for limited-user account (LUA) patching support (per-machine
// applications only)
...
#if (_WIN32_MSI >= 450)
#define IPROPNAME_MSI_UNINSTALL_SUPERSEDED_COMPONENTS                          \
  TEXT("MSIUNINSTALLSUPERSEDEDCOMPONENTS")
...
#define IPROPNAME_PRODUCTSTATE                                                 \
  TEXT("ProductState") // state of product (installed,advertised,etc...)
#define IPROPNAME_PRESELECTED                                                  \
  TEXT("Preselected")                   // selections made on command line
#define IPROPNAME_RESUME TEXT("RESUME") // resuming suspended install
#define IPROPNAME_UPDATESTARTED                                                \
  TEXT("UpdateStarted") // have begun to update system
#define IPROPNAME_PRODUCTID                                                    \
  TEXT("ProductID") // the complete validated Product ID
...
#define IPROPNAME_SOURCEDIR                                                    \
  TEXT("SourceDir") // source location - SET BY INSTALLER
#define IPROPNAME_REPLACEDINUSEFILES                                           \
  TEXT("ReplacedInUseFiles") // need reboot to completely install one or more
                             // files
...
#if (_WIN32_MSI >= 300)
#define IPROPNAME_SOURCERESONLY                                                \
  TEXT("MsiUISourceResOnly") // INSTALLUILEVEL_SOURCERESONLY provided as
                             // internal UI level
#define IPROPNAME_HIDECANCEL                                                   \
  TEXT("MsiUIHideCancel") // UI configured with a hidden cancel button
                          // (INSTALLUILEVEL_HIDECANCEL)
#define IPROPNAME_PROGRESSONLY                                                 \
  TEXT("MsiUIProgressOnly") // UI configured to only show progress, no modal or
                            // error dialogs displayed
                            // (INSTALLUILEVEL_PROGRESSONLY)
#endif                      // (_WIN32_MSI >= 300)
#if (_WIN32_MSI >= 500)
#define IPROPNAME_UACONLY                                                      \
  TEXT("MsiUIUACOnly") // INSTALLUILEVEL_UACONLY provided as internal UI level
#endif                 // (_WIN32_MSI >= 500)
...
#if (_WIN32_MSI >= 150)
#define IPROPNAME_TEMPLATE_AMD64                                               \
  TEXT("AMD64") // Oct. 2004: this is to be retired once grace period expires,
                // use the x64 below instead
#define IPROPNAME_TEMPLATE_X64 TEXT("x64")
#define IPROPNAME_MSIAMD64                                                     \
  TEXT("MsiAMD64") // Oct. 2004: this is to be retired once grace period
                   // expires, use the x64 below instead
...
#undef PID_SECURITY // defined as ( 0x80000002 ) in objidl.h, need to redefine
                    // here
...
#define PID_MSIVERSION                                                         \
  PID_PAGECOUNT // integer, Installer version number (major*100+minor)
#define PID_MSISOURCE                                                          \
  PID_WORDCOUNT // integer, type of file image, short/long, media/tree
...
enum msidbComponentAttributes {
  ...
  msidbComponentAttributesRegistryKeyPath =
      0x00000004, // KeyPath is key to Registry table
  msidbComponentAttributesSharedDllRefCount =
      0x00000008,                                 // increment SharedDll count
  msidbComponentAttributesPermanent = 0x00000010, // never uninstall component
  msidbComponentAttributesODBCDataSource =
      0x00000020, // KeyFile is key to ODBCDataSource table
  msidbComponentAttributesTransitive =
      0x00000040, // Can transition to/from installed/uninstalled based on
                  // changing conditional
  msidbComponentAttributesNeverOverwrite =
      0x00000080, // dont stomp over existing component if key path exists
                  // (file/ regkey)
#if (_WIN32_MSI >= 150)
  msidbComponentAttributes64bit =
      0x00000100, // designates a 64-bit component; 32-bit if missing.
#endif            // (_WIN32_MSI >= 150)
#if (_WIN32_MSI >= 400)
  msidbComponentAttributesDisableRegistryReflection =
      0x00000200, // Disables registry reflection for this component.
#endif            // (_WIN32_MSI >= 400)
#if (_WIN32_MSI >= 450)
...
enum msidbCustomActionType {
  ...
  msidbCustomActionTypeTextData =
      0x00000003, // Target = text string to be formatted and set into property
  msidbCustomActionTypeJScript =
      0x00000005, // Target = entry point name, null if none to call
  msidbCustomActionTypeVBScript =
      0x00000006, // Target = entry point name, null if none to call
  msidbCustomActionTypeInstall =
      0x00000007, // Target = property list for nested engine initialization

  // source of code
  msidbCustomActionTypeBinaryData =
      0x00000000, // Source = Binary.Name, data stored in stream
  msidbCustomActionTypeSourceFile =
      0x00000010, // Source = File.File, file part of installation
  msidbCustomActionTypeDirectory =
      0x00000020, // Source = Directory.Directory, folder containing existing
                  // file
  msidbCustomActionTypeProperty =
      0x00000030, // Source = Property.Property, full path to executable

  // return processing                  // default is syncronous execution,
  // process return code
  msidbCustomActionTypeContinue =
      0x00000040, // ignore action return status, continue running
  msidbCustomActionTypeAsync = 0x00000080, // run asynchronously

  // execution scheduling flags               // default is execute whenever
  // sequenced
  msidbCustomActionTypeFirstSequence =
      0x00000100, // skip if UI sequence already run
  msidbCustomActionTypeOncePerProcess =
      0x00000200, // skip if UI sequence already run in same process
  msidbCustomActionTypeClientRepeat =
      0x00000300, // run on client only if UI already run on client
  msidbCustomActionTypeInScript =
      0x00000400, // queue for execution within script
  msidbCustomActionTypeRollback =
      0x00000100, // in conjunction with InScript: queue in Rollback script
  msidbCustomActionTypeCommit = 0x00000200, // in conjunction with InScript: run
                                            // Commit ops from script on success

  // security context flag, default to impersonate as user, valid only if
  // InScript
  msidbCustomActionTypeNoImpersonate =
      0x00000800, // no impersonation, run in system context
#if (_WIN32_MSI >= 150)
  msidbCustomActionTypeTSAware =
      0x00004000, // impersonate for per-machine installs on TS machines
#endif            // (_WIN32_MSI >= 150)

#if (_WIN32_MSI >= 150)
  // script requires 64bit process
  msidbCustomActionType64BitScript =
      0x00001000, // script should run in 64bit process
  ...
  msidbCustomActionTypePatchUninstall =
      0x00008000, // custom action to be run only during a patch uninstall
#endif            // (_WIN32_MSI >= 450)
...
enum msidbFileAttributes {
  ...
  msidbFileAttributesPatchAdded =
      0x00001000, // Internal use only - set by patches
...
enum msidbSumInfoSourceType {
  ...
  msidbSumInfoSourceTypeLUAPackage =
      0x00000008, // package can be installed by LUA user (elevated privileges
                  // are not required for install)
#endif            //(_WIN32_MSI >= 400)
};
...
enum msifiFastInstallBits {
  msifiFastInstallNoSR =
      0x00000001, // no system restore point for per-user install
  msifiFastInstallQuickCosting =
      0x00000002, // only perform file related costing (baseline costing not
                  // included though)
