...
WINBASEAPI
BOOL WINAPI GlobalMemoryStatusEx(_Out_ LPMEMORYSTATUSEX lpBuffer);
...
WINBASEAPI
BOOL WINAPI GetSystemLeapSecondInformation(_Out_ PBOOL Enabled,
                                           _Out_ PDWORD Flags);
...
WINBASEAPI
__drv_preferredFunction("IsWindows*",
                        "Deprecated. Use VerifyVersionInfo* or IsWindows* "
                        "macros from VersionHelpers.") DWORD WINAPI
    GetVersion(VOID);
...
__drv_preferredFunction(
    "GetTickCount64",
    "GetTickCount overflows roughly every 49 days.  Code that does not take "
    "that into account can loop indefinitely.  GetTickCount64 operates on 64 "
    "bit values and does not have that problem") WINBASEAPI DWORD WINAPI
    GetTickCount(VOID);
...
_Success_(return != FALSE) BOOL WINAPI
    GetSystemTimeAdjustment(_Out_ PDWORD lpTimeAdjustment,
...
WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    GetSystemTimeAdjustmentPrecise(_Out_ PDWORD64 lpTimeAdjustment,
...
WINBASEAPI
_Success_(return != 0 && return < uSize) UINT WINAPI
    GetSystemDirectoryA(_Out_writes_to_opt_(uSize, return +1) LPSTR lpBuffer,
                        _In_ UINT uSize);

WINBASEAPI
_Success_(return != 0 && return < uSize) UINT WINAPI
    GetSystemDirectoryW(_Out_writes_to_opt_(uSize, return +1) LPWSTR lpBuffer,
                        _In_ UINT uSize);
...
WINBASEAPI
_Must_inspect_result_ _Success_(return != 0 && return < uSize) UINT WINAPI
    GetWindowsDirectoryA(_Out_writes_to_opt_(uSize, return +1) LPSTR lpBuffer,
                         _In_ UINT uSize);

WINBASEAPI
_Must_inspect_result_ _Success_(return != 0 && return < uSize) UINT WINAPI
    GetWindowsDirectoryW(_Out_writes_to_opt_(uSize, return +1) LPWSTR lpBuffer,
                         _In_ UINT uSize);
...
WINBASEAPI
_Success_(return != 0 && return < uSize) UINT WINAPI
    GetSystemWindowsDirectoryA(_Out_writes_to_opt_(uSize, return +1)
                                   LPSTR lpBuffer,
                               _In_ UINT uSize);

WINBASEAPI
_Success_(return != 0 && return < uSize) UINT WINAPI
    GetSystemWindowsDirectoryW(_Out_writes_to_opt_(uSize, return +1)
                                   LPWSTR lpBuffer,
                               _In_ UINT uSize);
...
WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    GetComputerNameExA(_In_ COMPUTER_NAME_FORMAT NameType,
                       _Out_writes_to_opt_(*nSize, *nSize + 1) LPSTR lpBuffer,
                       _Inout_ LPDWORD nSize);

WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    GetComputerNameExW(_In_ COMPUTER_NAME_FORMAT NameType,
...
WINBASEAPI
BOOL WINAPI SetComputerNameExW(_In_ COMPUTER_NAME_FORMAT NameType,
...
__drv_preferredFunction("IsWindows*",
                        "Deprecated. Use VerifyVersionInfo* or IsWindows* "
                        "macros from VersionHelpers.") BOOL WINAPI
    GetVersionExA(_Inout_ LPOSVERSIONINFOA lpVersionInformation);
...
__drv_preferredFunction("IsWindows*",
                        "Deprecated. Use VerifyVersionInfo* or IsWindows* "
                        "macros from VersionHelpers.") BOOL WINAPI
    GetVersionExW(_Inout_ LPOSVERSIONINFOW lpVersionInformation);
...
WINBASEAPI
BOOL WINAPI GetProductInfo(_In_ DWORD dwOSMajorVersion,
...
WINBASEAPI
BOOL WINAPI GetOsSafeBootMode(_Out_ PDWORD Flags);
...
UINT WINAPI GetSystemFirmwareTable(_In_ DWORD FirmwareTableProviderSignature,
                                   ...
                                   _Out_writes_bytes_to_opt_(BufferSize, return)
...
WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    DnsHostnameToComputerNameExW(_In_ LPCWSTR Hostname,
...
WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    GetPhysicallyInstalledSystemMemory(_Out_ PULONGLONG TotalMemoryInKilobytes);

#define SCEX2_ALT_NETBIOS_NAME 0x00000001

WINBASEAPI
BOOL WINAPI SetComputerNameEx2W(_In_ COMPUTER_NAME_FORMAT NameType,
                                _In_ DWORD Flags, _In_ LPCWSTR lpBuffer);
...
WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    SetSystemTimeAdjustment(_In_ DWORD dwTimeAdjustment,
                            _In_ BOOL bTimeAdjustmentDisabled);

WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI
    SetSystemTimeAdjustmentPrecise(_In_ DWORD64 dwTimeAdjustment,
                                   _In_ BOOL bTimeAdjustmentDisabled);
...
WINBASEAPI
BOOL WINAPI GetProcessorSystemCycleTime(
    _In_ USHORT Group,
    _Out_writes_bytes_to_opt_(*ReturnedLength, *ReturnedLength)
...
WINBASEAPI
BOOL WINAPI GetOsManufacturingMode(_Out_ PBOOL pbEnabled);
...
HRESULT
WINAPI
GetIntegratedDisplaySize(_Out_ double *sizeInInches);
...
WINBASEAPI
BOOL WINAPI SetComputerNameA(_In_ LPCSTR lpComputerName);

WINBASEAPI
BOOL WINAPI SetComputerNameW(_In_ LPCWSTR lpComputerName);
...
WINBASEAPI
BOOL WINAPI SetComputerNameExA(_In_ COMPUTER_NAME_FORMAT NameType,
...
#pragma region Application Family or OneCore Family or Games Family
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM |   \
                            WINAPI_PARTITION_GAMES | WINAPI_PARTITION_DESKTOP)

#if defined(NTDDI_WIN11_GE) && (NTDDI_VERSION >= NTDDI_WIN11_GE)

WINBASEAPI
BOOL WINAPI GetRuntimeAttestationReport(
    _In_reads_bytes_opt_(RUNTIME_REPORT_NONCE_SIZE) UCHAR *Nonce,
    _In_ UINT16 PackageVersion, _In_ UINT64 ReportTypesBitmap,
    _Out_writes_bytes_to_opt_(*ReportBufferSize, *ReportBufferSize)
        PVOID ReportBuffer,
