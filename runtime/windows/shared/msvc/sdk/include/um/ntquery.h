...
STDAPI LoadIFilter(PCWSTR pwcsPath, _In_ IUnknown *pUnkOuter,
...
STDAPI LoadIFilterEx(PCWSTR pwcsPath, DWORD dwFlags, REFIID riid,
                     _Outptr_ void **ppIUnk);

STDAPI BindIFilterFromStorage(_In_ IStorage *pStg, _In_ IUnknown *pUnkOuter,
                              _Outptr_ void **ppIUnk);

STDAPI BindIFilterFromStream(_In_ IStream *pStm, _In_ IUnknown *pUnkOuter,
                             _Outptr_ void **ppIUnk);
...
#define CI_STATE_CONTENT_SCAN_REQUIRED                                         \
  0x0004 // Index is likely corrupt, and a rescan is required
#define CI_STATE_ANNEALING_MERGE                                               \
  0x0008 // Index is performing an annealing (optimizing) merge
...
#define CI_STATE_LOW_MEMORY                                                    \
  0x0080 // Indexing is paused due to low memory availability
...
#define CI_STATE_READ_ONLY                                                     \
  0x0400 // Indexing has been manually paused (read-only)
#define CI_STATE_BATTERY_POWER                                                 \
  0x0800 // Indexing is paused to conserve battery life
#define CI_STATE_USER_ACTIVE                                                   \
  0x1000 // Indexing is paused due to high user activity (keyboard/mouse)
...
#define CI_STATE_LOW_DISK                                                      \
  0x10000 // Index is paused due to low disk availability
#define CI_STATE_HIGH_CPU 0x20000 // Index is paused due to high CPU
#define CI_STATE_BATTERY_POLICY                                                \
  0x40000 // Indexing is paused due to backoff on battery policy
