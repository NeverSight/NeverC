...
#define NGC_TICKET_PROPERTY_STRING_LENGTH                                      \
  (NCRYPT_PIN_CACHE_PIN_BYTE_LENGTH / sizeof(WCHAR))
...
// This structure is used in EapPeerGetResult API to convey the authentication
// result and pass additional information to the EapHost and Supplicant.
...
typedef struct _EAP_PEER_METHOD_ROUTINES {
  ...
  DWORD(APIENTRY *EapPeerGetIdentity)(
      _In_ DWORD flags, _In_ DWORD dwSizeofConnectionData,
      ...
      _In_ HANDLE hTokenImpersonateUser, _Out_ BOOL *pfInvokeUI,
      _Out_ DWORD *pdwSizeOfUserDataOut,
      _Outptr_result_buffer_(*pdwSizeOfUserDataOut) BYTE **ppUserDataOut,
      _Outptr_ LPWSTR *ppwszIdentity, _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerBeginSession)(
      _In_ DWORD dwFlags, _In_ const EapAttributes *const pAttributeArray,
      _In_ HANDLE hTokenImpersonateUser, _In_ DWORD dwSizeofConnectionData,
      _In_reads_(dwSizeofConnectionData) BYTE *pConnectionData,
      _In_ DWORD dwSizeofUserData, _In_reads_(dwSizeofUserData) BYTE *pUserData,
      _In_ DWORD dwMaxSendPacketSize, _Out_ EAP_SESSION_HANDLE *pSessionHandle,
      _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerSetCredentials)(_In_ EAP_SESSION_HANDLE sessionHandle,
                                         _In_ WCHAR *pwszIdentity,
                                         _In_ WCHAR *pwszPassword,
                                         _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerProcessRequestPacket)(
      _In_ EAP_SESSION_HANDLE sessionHandle, _In_ DWORD cbReceivePacket,
      _In_reads_bytes_(cbReceivePacket) EapPacket *pReceivePacket,
      _Out_ EapPeerMethodOutput *pEapOutput, _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerGetResponsePacket)(
      _In_ EAP_SESSION_HANDLE sessionHandle,
      _Inout_ _Pre_satisfies_(*pcbSendPacket >= sizeof(EapPacket))
          DWORD *pcbSendPacket,
      _Inout_ _Pre_writable_byte_size_(*pcbSendPacket)
          _Pre_readable_byte_size_(*pcbSendPacket) EapPacket *pSendPacket,
      _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerGetResult)(_In_ EAP_SESSION_HANDLE sessionHandle,
  ...
  DWORD(APIENTRY *EapPeerSetUIContext)(_In_ EAP_SESSION_HANDLE sessionHandle,
                                       _In_ DWORD dwSizeOfUIContextData,
                                       _In_reads_(dwSizeOfUIContextData)
                                           const BYTE *pUIContextData,
                                       _Out_ EapPeerMethodOutput *pEapOutput,
                                       _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerGetResponseAttributes)(
      _In_ EAP_SESSION_HANDLE sessionHandle, _Out_ EapAttributes *pAttribs,
      _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerSetResponseAttributes)(
      _In_ EAP_SESSION_HANDLE sessionHandle, _In_ EapAttributes *pAttribs,
      _Out_ EapPeerMethodOutput *pEapOutput, _Outptr_ EAP_ERROR **ppEapError);

  DWORD(APIENTRY *EapPeerEndSession)(_In_ EAP_SESSION_HANDLE sessionHandle,
                                     _Outptr_ EAP_ERROR **ppEapError);
...
DWORD WINAPI EapPeerGetInfo(_In_ EAP_TYPE *pEapType,
...
DWORD WINAPI EapPeerGetIdentity(
    _In_ DWORD dwFlags, _In_ DWORD dwSizeofConnectionData,
    _In_reads_(dwSizeofConnectionData) const BYTE *pConnectionData,
    _In_ DWORD dwSizeofUserData,
    _In_reads_(dwSizeofUserData) const BYTE *pUserData,
    _In_ HANDLE hTokenImpersonateUser, _Out_ BOOL *pfInvokeUI,
    _Out_ DWORD *pdwSizeOfUserDataOut,
    _Outptr_result_buffer_(*pdwSizeOfUserDataOut) BYTE **ppUserDataOut,
    _Out_ LPWSTR *ppwszIdentity, _Outptr_ EAP_ERROR **ppEapError);
...
//   send a message larger than this size, the method has to handle
//   fragmentation and resembling.
//
// - pSessionHandle:
...
DWORD WINAPI EapPeerProcessRequestPacket(_In_ EAP_SESSION_HANDLE sessionHandle,
                                         _In_ DWORD cbReceivedPacket,
                                         _In_reads_bytes_(cbReceivedPacket)
...
DWORD WINAPI EapPeerGetResult(_In_ EAP_SESSION_HANDLE sessionHandle,
                              _In_ EapPeerMethodResultReason reason,
                              _Out_ EapPeerMethodResult *pResult,
                              _Outptr_ EAP_ERROR **ppEapError);
...
//   pUIContextData
DWORD WINAPI EapPeerGetUIContext(_In_ EAP_SESSION_HANDLE sessionHandle,
                                 _Out_ DWORD *pdwSizeOfUIContextData,
                                 _Outptr_result_buffer_(*pdwSizeOfUIContextData)
...
// - pEapOutput:
//   the method should fill this struct to tell the supplicant what to do.
DWORD WINAPI EapPeerSetUIContext(_In_ EAP_SESSION_HANDLE sessionHandle,
                                 _In_ DWORD dwSizeOfUIContextData,
                                 _In_reads_(dwSizeOfUIContextData)
                                     const BYTE *pUIContextData,
                                 _Out_ EapPeerMethodOutput *pEapOutput,
...
DWORD WINAPI EapPeerGetResponseAttributes(_In_ EAP_SESSION_HANDLE sessionHandle,
...
// - pEapOutput:
//   the method should fill this struct to tell the supplicant what to do.
DWORD WINAPI EapPeerSetResponseAttributes(_In_ EAP_SESSION_HANDLE sessionHandle,
...
//   the value returned through *pSessionHandle from EapPeerBeginSession()
//
DWORD WINAPI EapPeerEndSession(_In_ EAP_SESSION_HANDLE sessionHandle,
                               _Outptr_ EAP_ERROR **ppEapError);
...
DWORD WINAPI EapPeerInvokeConfigUI(
    _In_ EAP_METHOD_TYPE *pEapType, _In_ HWND hwndParent, _In_ DWORD dwFlags,
    ...
    _Outptr_result_buffer_(*dwSizeOfConnectionDataOut)
        BYTE **ppConnectionDataOut,
    _Outptr_ EAP_ERROR **ppEapError);

// This function should raise interactive UI for the Method. What UI to be
// raised and how to raise is Method specific information that is embedded in
// pUIContextData.
//
// Parameters:
// - pEapType:
...
// - pUIContextData:
//   A blob that conveys the state of the authentication and describes what and
//   how to show the UI to the user.
...
// - pEapError
//   Pointer to pointer to EAP_ERROR that will be filled by the Method in the
//   case of error. In some cases
//  the structure might not be filled.
DWORD WINAPI EapPeerInvokeInteractiveUI(
    _In_ EAP_METHOD_TYPE *pEapType, _In_ HWND hwndParent,
    _In_ DWORD dwSizeofUIContextData,
    _In_reads_(dwSizeofUIContextData) BYTE *pUIContextData,
    _Out_ DWORD *pdwSizeOfDataFromInteractiveUI,
    _Outptr_result_buffer_(*pdwSizeOfDataFromInteractiveUI)
        BYTE **ppDataFromInteractiveUI,
    _Outptr_ EAP_ERROR **ppEapError);

// This function should raise credential UI for the Method. Method can use the
// configuration passed in to decide what kind of UI should be shown to the
// user.
//
// Parameters:
// - pEapType:
...
//
// - pUserData
//   Cached credential blob. This would be NULL if this is the first time
//   successful authentication is being done.
//
// - ppwszIdentity
//   Pointer to unicode string that receives the identity that EapHost should
//   use as part of EAP Identity reponse.
//
// - pEapError
//   Pointer to pointer to EAP_ERROR that will be filled by the Method in the
//   case of error. In some cases
...
DWORD WINAPI EapPeerInvokeIdentityUI(
    _In_ EAP_METHOD_TYPE *pEapType, _In_ DWORD dwFlags, _In_ HWND hwndParent,
...
// This function returns credential fields to be shown to the user for the
// authentication. As part of this function Method should not bring up UI
// instead it should describe what credentials are to be obtained. Based on the
// return values, Supplicant draws the UI to the user and obtains the requested
// credentials.
//
// Parameters:
...
//   Pointer to EAP_CONFIG_INPUT_FIELD_ARRAY that receives array of credential
//   fields.
//
// - pEapError
//   Pointer to pointer to EAP_ERROR that will be filled by the Method in the
...
DWORD WINAPI EapPeerQueryCredentialInputFields(
    _In_ HANDLE hUserImpersonationToken, _In_ EAP_METHOD_TYPE eapMethodType,
    _In_ DWORD dwFlags, _In_ DWORD dwEapConnDataSize,
    _In_reads_(dwEapConnDataSize) BYTE *pbEapConnData,
    _Out_ EAP_CONFIG_INPUT_FIELD_ARRAY *pEapConfigFieldsArray,
    _Outptr_ EAP_ERROR **ppEapError);

// This function returns credential blob based on the credentials passed in the
// credential array structure. Method should not bring up UI. It should convert
// the credentials in to a blob that could be passed to EapPeerBeginSession.
//
// Parameters:
// - hUserImpersonationToken:
//   Impersonation token of the user/machine who is being authenticated.
...
//
// - pEapConfigFieldsArray
//   Pointer to EAP_CONFIG_INPUT_FIELD_ARRAY that contains array of credential
//   fields filled by supplicant.
//
// - pdwUserBlobSize:
...
DWORD WINAPI EapPeerQueryUserBlobFromCredentialInputFields(
    ...
    // pointer to DWORD that receives size of credential blob, if EAPHost passes
    // in non-zero size and non-NULL data below, EAP-method will just attempt to
    // Update the blob with passed in values (if supported) instead of creating
    // a new one. EAP-method is not expected to release the passed in buffer,
    // EAPHost will release it after API completion..
    _Inout_ _Pre_satisfies_(*pdwUserBlobSize >= sizeof(UINT_PTR))
        DWORD *pdwUserBlobSize,
    _Inout_ _At_(*ppUserBlob, _Pre_writable_size_(*pdwUserBlobSize)
                                  _Post_readable_size_(*pdwUserBlobSize))
        BYTE **ppUserBlob,
    _Outptr_ EAP_ERROR **ppEapError);

// This function returns fields to be displayed during interactive time when
// alternative UI is used. Method should not bring up UI in this function.
//
DWORD WINAPI EapPeerQueryInteractiveUIInputFields(
    ...
    // structure that gets filled with fields and how they should be shown to
    // the user.
    ...
    // ppEapError using EapPeerFreeErrorMemory
    _Outptr_ EAP_ERROR **ppEapError,
    // Reserved for future usage. It must be NULL.
    _Inout_ LPVOID *ppvReserved);

// This function returns interactive UI blob from the UI fields filled by the
// user
//
DWORD WINAPI EapPeerQueryUIBlobFromInteractiveUIInputFields(
    ...
    // authentication. Caller (EapHost) should free the memory using
    // EapPeerFreeMemory.
    _Outptr_result_buffer_(*pdwSizeOfDataFromInteractiveUI)
        BYTE **ppDataFromInteractiveUI,
    // In case of error, API fills ppEapError if possible.  Caller should free
...
// This function converts XML document containing configuration information into
// a blob that could be passed to other APIs that expect configuration blob.
//
// Parameters:
// - dwFlags:
//   EAP_FLAG_xxx defined in eaptypes.w
...
//
// - pConfigDoc:
//   Pointer to XMLDOMDocument2 that refers to the XML document containg the
//   configuration XML
//
// - ppConfigOut
...
//   case of error. In some cases
//  the structure might not be filled.
DWORD WINAPI EapPeerConfigXml2Blob(_In_ DWORD dwFlags,
                                   _In_ EAP_METHOD_TYPE eapMethodType,
                                   _In_ IXMLDOMDocument2 *pConfigDoc,
                                   _Outptr_result_buffer_(*pdwSizeOfConfigOut)
                                       BYTE **ppConfigOut,
                                   _Out_ DWORD *pdwSizeOfConfigOut,
                                   _Outptr_ EAP_ERROR **ppEapError);

// This function converts XML document containing credential information into a
// blob that could be passed to other APIs that expect credential blob.
//
// Parameters:
// - dwFlags:
//   EAP_FLAG_xxx defined in eaptypes.w
...
DWORD WINAPI EapPeerCredentialsXml2Blob(
    _In_ DWORD dwFlags, _In_ EAP_METHOD_TYPE eapMethodType,
    ...
    _Out_ DWORD *pdwSizeOfCredentialsOut, _Outptr_ EAP_ERROR **ppEapError);

// This function converts configuration blob into configuration XML document.
// Methods exporting this API will be supporting exporting of and importing of
// configuration with out problem. It also helps administrators modify the
// configuration.
//
// Parameters:
// - dwFlags:
...
//
// - ppConfigDoc
//  Pointer to IXMLDOMDocument2 that receives configuration XML for the given
//  configuration blob.
//
// - pEapError
//   Pointer to pointer to EAP_ERROR that will be filled by the Method in the
//   case of error. In some cases
//  the structure might not be filled.
DWORD WINAPI EapPeerConfigBlob2Xml(_In_ DWORD dwFlags,
                                   _In_ EAP_METHOD_TYPE eapMethodType,
...
// This function retrieves the method properties for a specific configuration
// data. The method properties returned by the API will be a subset of the
// properties value in the registry.
//
// Parameters:
// - dwVersion:
...
//
// - hUserImpersonationToken
//   An impersonation token for the user whose credentials are to be requested
//   and obtained.
//
// - eapMethodType
...
//
// - ppEapError
//   A pointer to a pointer to an EAP_ERROR structure that contains any errors
//   raised by EAPHost during the execution of this function call.
DWORD WINAPI EapPeerGetMethodProperties(
    _In_ DWORD dwVersion, _In_ DWORD dwFlags,
    _In_ EAP_METHOD_TYPE eapMethodType, _In_ HANDLE hUserImpersonationToken,
    _In_ DWORD dwSizeOfConnectionDataIn,
...
DWORD WINAPI EapPeerGetConfigBlobAndUserBlob(
    _In_ DWORD dwFlags, _In_ EAP_METHOD_TYPE eapMethodType,
    _In_ EapCredential eapCredential, _Out_ DWORD *pdwConfigBlobSize,
...
// This function frees memory returned by the configuration APIs. This function
// is called by EapHost when it is done with the memory returned by the Method.
//
// Parameters:
// - pUIContextData:
