...
class CBaseTerminal
    : virtual public CComObjectRootEx<
          CComMultiThreadModelNoCS>, // we have our own CS implementation
      public IDispatchImpl<ITTerminalVtblBase<CBaseTerminal>, &IID_ITTerminal,
                           &LIBID_TAPI3Lib>,
...
public:
  ...
  virtual HRESULT Initialize(IN IID iidTerminalClass, IN DWORD dwMediaType,
...
public:
  ...
  STDMETHOD(get_AddressHandle)(OUT MSP_HANDLE *phtAddress);
  ...
  STDMETHOD(ConnectTerminal)(IN IGraphBuilder *pGraph,
                             IN DWORD dwTerminalDirection,
                             IN OUT DWORD *pdwNumPins, OUT IPin **ppPins);
  ...
  STDMETHOD(DisconnectTerminal)(IN IGraphBuilder *pGraph, IN DWORD dwReserved);
...
public:
  ...
  virtual HRESULT GetNumExposedPins(IN IGraphBuilder *pGraph,
                                    OUT DWORD *pdwNumPins) = 0;

  // Returns an array of pins that the stream can connect to.
  // Arguments are checked by the caller.

  virtual HRESULT GetExposedPins(OUT IPin **ppPins) = 0;
...
public:
  ...
  virtual HRESULT GetExposedPins(OUT IPin **ppPins);
...
public:
  // public because CreateTerminal and CMSPAddress::UpdateTerminalListForPnp
  // accesses it
  CComPtr<IMoniker> m_pMoniker;

  // this flag allows CMSPAddress::UpdateTerminalListForPnp to perform a mark
  // and sweep on the terminal list
  BOOL m_bMark;

  //
  // Compares this terminal's moniker to pMoniker, returns S_OK if they match,
  // S_FALSE if they don't
  //
