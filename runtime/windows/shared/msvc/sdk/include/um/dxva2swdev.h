...
typedef HRESULT(CALLBACK *PDXVA2SW_GETVIDEOPROCESSORRENDERTARGETCOUNT)(
    _In_ const DXVA2_VideoDesc *pVideoDesc, _Out_ UINT *pCount);

typedef HRESULT(CALLBACK *PDXVA2SW_GETVIDEOPROCESSORRENDERTARGETS)(
    _In_ const DXVA2_VideoDesc *pVideoDesc, _In_ UINT Count,
    _Out_writes_(Count) D3DFORMAT *pFormats);

typedef HRESULT(CALLBACK *PDXVA2SW_GETVIDEOPROCESSORCAPS)(
    _In_ const DXVA2_VideoDesc *pVideoDesc, _In_ D3DFORMAT RenderTargetFormat,
...
typedef HRESULT(CALLBACK *PDXVA2SW_GETVIDEOPROCESSORSUBSTREAMFORMATS)(
    _In_ const DXVA2_VideoDesc *pVideoDesc, _In_ D3DFORMAT RenderTargetFormat,
    _In_ UINT Count, _Out_writes_(Count) D3DFORMAT *pFormats);

typedef HRESULT(CALLBACK *PDXVA2SW_GETPROCAMPRANGE)(
    _In_ const DXVA2_VideoDesc *pVideoDesc, _In_ D3DFORMAT RenderTargetFormat,
    _In_ UINT ProcAmpCap, _Out_ DXVA2_ValueRange *pRange);

typedef HRESULT(CALLBACK *PDXVA2SW_GETFILTERPROPERTYRANGE)(
    _In_ const DXVA2_VideoDesc *pVideoDesc, _In_ D3DFORMAT RenderTargetFormat,
    _In_ UINT FilterSetting, _Out_ DXVA2_ValueRange *pRange);

typedef HRESULT(CALLBACK *PDXVA2SW_CREATEVIDEOPROCESSDEVICE)(
    _In_ IDirect3DDevice9 *pD3DD9, _In_ const DXVA2_VideoDesc *pVideoDesc,
    _In_ D3DFORMAT RenderTargetFormat, _In_ UINT MaxSubStreams,
    _Out_ HANDLE *phDevice);

typedef HRESULT(CALLBACK *PDXVA2SW_DESTROYVIDEOPROCESSDEVICE)(
    _In_ HANDLE hDevice);

typedef HRESULT(CALLBACK *PDXVA2SW_VIDEOPROCESSBEGINFRAME)(_In_ HANDLE hDevice);

typedef HRESULT(CALLBACK *PDXVA2SW_VIDEOPROCESSENDFRAME)(
    _In_ HANDLE hDevice, _Inout_opt_ HANDLE *pHandleComplete);

typedef HRESULT(CALLBACK *PDXVA2SW_VIDEOPROCESSSETRENDERTARGET)(
    _In_ HANDLE hDevice, _In_ IDirect3DSurface9 *pRenderTarget);

typedef HRESULT(CALLBACK *PDXVA2SW_VIDEOPROCESSBLT)(
    _In_ HANDLE hDevice, _In_ const DXVA2_VIDEOPROCESSBLT *pBlt);
