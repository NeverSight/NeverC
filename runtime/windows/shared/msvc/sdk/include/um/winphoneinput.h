...
// \src\uxplat\Splash\Dev\Libraries\InputReader\Library\InputStateManager.cpp
//                  and in \src\uxplat\Input\Dev\Drivers\TchHID\TchHID.c
//

DEFINE_GUID(GUID_WP_DEVINTERFACE_TCH, 0x2c05ce1a, 0x75e8, 0x4ea7, 0xa7, 0xa,
            0xe3, 0x23, 0xb0, 0x72, 0x25, 0x8c);
...
// Hover touch sessions do not overlap with contact touch sessions.  Input
// reader will synthesize begin/end events to ensure the separation.
//
...
enum ModifierKeyStateFlag {
  ...
  ModifierKeyStateFlag_RefreshModifier =
      0x10000 // Only SIP send this flag to refresh the latest Modifier status
              // on SIP
};
...
//
// TouchContact represents information about a Single Touch Contact that should
// only exists as part of a Touch Session update (See TouchInfo).
//
...
//   (Windows Phone UxPlat
//   depot)\CoreUI\Dev\CoreUIComponents\RemoteInterfaces\Input\IRemoteInputInjection.cs
// * Limited version of WinPhoneInput.h sent out to OEMs while hover is still
// confidential.
...
//   (Windows ShellTest
//   depot)\Personality\Touch\Manipulation\ConvergedTests\DataCollector\Inc\Phone.h
//
...
struct TouchContact {
  ...
  SHORT xTilt; // Angle in degrees along X-axis. 0=straight up, Positive angle =
               // tilt right.
  SHORT yTilt; // Angle in degrees along Y-axis. 0=straight up, Positive angle =
               // tilt up.
...
typedef struct _TouchContact {
  ...
  INT16 TiltX; // Angle in degrees along X-axis. 0=straight up, Positive angle =
               // tilt right.
  INT16 TiltY; // Angle in degrees along Y-axis. 0=straight up, Positive angle =
               // tilt up.
  ULONG64 InputSink;       // For CoreInput, window handle for inputSink.
  float InputTransform[6]; // For CoreInput, 2D affine transform for inputSink
} TouchContact;
...
//
// TouchInfo represents information about an in-the-moment view of Touch Events
// within a Touch Session.
//
// Any changes here require a corresponding change in:
// * XNA's C# declaration
//   (Windows Phone Runtimes depot)\XNA\Runtime\Framework\Touch\NativeMethods.cs
...
typedef struct _TouchInfo {
  ...
  UINT8 InContactContactCount; // The count of contacts that are marked Down or
                               // Move
...
static const DWORD c_cbTouchInfoMaximum =
    (c_cbTouchInfoMinimum +
     (sizeof(TouchContact) * (c_cTouchContactsMaximum - 1)));
#else // __cplusplus
#define c_cbTouchInfoMinimum                                                   \
  (sizeof(TouchInfo) - (sizeof(TouchContact) * (c_cTouchContactsMaximum - 1)))
...
enum InputDeviceType {
  ...
  InputDeviceType_AllDevicesMask =
      InputDeviceType_Mouse | InputDeviceType_Touch | InputDeviceType_Keyboard
};
...
// This struct must keep the same as the struct Input::PointerDeviceProperty
// that is defined in \mincore\CoreUI\Dev\MinUser\Input\InputInterop.cs on
// windows side, so that the device usage information can be propagated
// correctly between MinUser GetPointerDeviceProperties, MinUserHost, and
// TchHID.
//
...
//
// Device IO codes for retrieving the supported device usages from TchHID. There
// is no FILE_DEVICE_* defined for touch, and FILE_DEVICE_UNKNOWN is used based
// on a recommendation from Windows.
//

#define IOCTL_TCHHID_GET_DEVICE_USAGE_COUNT                                    \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 0x8001, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_TCHHID_GET_DEVICE_USAGES                                         \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 0x8002, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_TCHHID_GET_DEVICE_MAX_INPUTS                                     \
  CTL_CODE(FILE_DEVICE_UNKNOWN, 0x8003, METHOD_BUFFERED, FILE_ANY_ACCESS)

//
// Device name and the symbolic link for the virtual driver device that
// user-mode applications can retrieve touch device usages from.
//
...
//
// The height of the 0D capacitive buttons in number of pixels. Any arbitrary
// number can be used because the buttons are outside fo the regular touch panel
// on 0D devices. It works as long as Chrome and InputStateManager agree on the
// same button height.
//
...
//
// These constants define the default maximum distance from the display surface
// edge in physical pixels of the first event of a gesture for the gesture to be
// recognized as a system edge gesture. Gestures that begin farther away than
// this distance are never recognized as edge gestures. Additionally, edge
// gestures are only recognized when they begin over a visual that has
// registered to receive edge gestures.
//
// Values for the constants assume a 480x800 display size.  At runtime the
// values are scaled up or down based on the actual screen size.
//
// The default value may be overwritten by an oem in the registry based on
// specific hardware characteristcs.  More sensitive hardware could use a lower
// value, which reduces the latency added into any gesture that is a candidate
// for edge gestures and reduces the likelihood of a false positive in the case
// that user did not intend an edge gesture.  The registry keys examined for
// overrides are dword values at
//
...
//      Right
//
// Note: these are used in
// \src\tools\Product\DH\Test\libs\HealthUtils\Native\HealthUtil.cpp and
// \src\uxplat\Splash\Dev\Render\Engine\Components\Desktop\Mobile\EdgeGestureRedirector.cpp
//
...
//
// Calculate enough space (in bytes) to hold a TouchInfo structure plus any
// additional needed for contact data beyond the mandatory one TouchContact.
//
//------------------------------------------------------------------------------

#ifdef __cplusplus
inline DWORD
CalculateTouchInfoSize(DWORD cContacts) // the number of contacts that the
                                        // TouchInfo structure will require
#else                                   // __cplusplus
__inline ULONG
CalculateTouchInfoSize(ULONG cContacts) // the number of contacts that the
                                        // TouchInfo structure will require
#endif                                  // __cplusplus
{
