...
const IID
    IID_IVssCreateWriterMetadataEx = // 9f21981d-d469-4349-b807-39e64e4674e1
    {0x9f21981d,
     0xd469,
     0x4349,
...
const IID
    IID_IVssCreateExpressWriterMetadata = // 9c772e77-b26e-427f-92dd-c996f41ea5e3
    {0x9c772e77,
     0xb26e,
     0x427f,
...
public:
  ...
  STDMETHOD(GetPath)(_Out_ BSTR *pbstrPath) = 0;

  // get filespec (may include wildcards)
  STDMETHOD(GetFilespec)(_Out_ BSTR *pbstrFilespec) = 0;

  // is path a directory or root of a tree
  STDMETHOD(GetRecursive)(_Out_ bool *pbRecursive) = 0;

  // alternate location for files
  STDMETHOD(GetAlternateLocation)(_Out_ BSTR *pbstrAlternateLocation) = 0;

  // backup type
  STDMETHOD(GetBackupTypeMask)(_Out_ DWORD *pdwTypeMask) = 0;
};
...
public:
  ...
  (OUT VSS_ID *pidInstance, OUT VSS_ID *pidWriter) = 0;

  // obtain a specific component
  STDMETHOD(GetComponent)
  (IN UINT iComponent, OUT IVssComponent **ppComponent) = 0;
};
...
public:
  ...
  (_Outptr_result_maybenull_ BSTR *pbstrFailureMsg) = 0;
  ...
  (_Out_ VSS_ROLLFORWARD_TYPE *pRollType,
   _Outptr_result_maybenull_ BSTR *pbstrPoint) = 0;

  // get the restore name
  STDMETHOD(GetRestoreName)
  (_Outptr_result_maybenull_ BSTR *pbstrName) = 0;
};
...
public:
  ...
  (_In_ HRESULT hr, _In_ HRESULT hrApplication,
   _In_opt_ LPCWSTR wszApplicationMessage, _In_ DWORD dwReserved) = 0;

  STDMETHOD(GetFailure)
  (_Out_ HRESULT *phr, _Out_ HRESULT *phrApplication,
   _Out_ BSTR *pbstrApplicationMessage, _Out_ DWORD *pdwReserved) = 0;
};
...
public:
  ...
  (IN LPCWSTR wszPath, IN LPCWSTR wszFilespec, IN bool bRecursive,
   IN LPCWSTR wszAlternateLocation) = 0;

  // add files to exclude to metadata document
  STDMETHOD(AddExcludeFiles)
  (IN LPCWSTR wszPath, IN LPCWSTR wszFilespec, IN bool bRecursive) = 0;

  // add component to metadata document
  STDMETHOD(AddComponent)
  (IN VSS_COMPONENT_TYPE ct, IN LPCWSTR wszLogicalPath,
   IN LPCWSTR wszComponentName, IN LPCWSTR wszCaption, IN const BYTE *pbIcon,
   IN UINT cbIcon, IN bool bRestoreMetadata, IN bool bNotifyOnBackupComplete,
   IN bool bSelectable, IN bool bSelectableForRestore = false,
   IN DWORD dwComponentFlags = 0) = 0;

  // add physical database files to a database component
  STDMETHOD(AddDatabaseFiles)
  (IN LPCWSTR wszLogicalPath, IN LPCWSTR wszDatabaseName, IN LPCWSTR wszPath,
  ...
  (IN LPCWSTR wszLogicalPath, IN LPCWSTR wszGroupName, IN LPCWSTR wszPath,
   IN LPCWSTR wszFilespec, IN bool bRecursive, IN LPCWSTR wszAlternateLocation,
   IN DWORD dwBackupTypeMask =
       (VSS_FSBT_ALL_BACKUP_REQUIRED | VSS_FSBT_ALL_SNAPSHOT_REQUIRED)) = 0;

  // create a restore method
  STDMETHOD(SetRestoreMethod)
  (IN VSS_RESTOREMETHOD_ENUM method, IN LPCWSTR wszService,
   IN LPCWSTR wszUserProcedure, IN VSS_WRITERRESTORE_ENUM writerRestore,
   IN bool bRebootRequired) = 0;

  // add alternative location mappings to the restore method
  STDMETHOD(AddAlternateLocationMapping)
  (IN LPCWSTR wszSourcePath, IN LPCWSTR wszSourceFilespec, IN bool bRecursive,
   IN LPCWSTR wszDestination) = 0;

  // add a dependency to another writer's component
  STDMETHOD(AddComponentDependency)
  (IN LPCWSTR wszForLogicalPath, IN LPCWSTR wszForComponentName,
   IN VSS_ID onWriterId, IN LPCWSTR wszOnLogicalPath,
   IN LPCWSTR wszOnComponentName) = 0;

  // Set the schema used during backup
  STDMETHOD(SetBackupSchema)
  (IN DWORD dwSchemaMask) = 0;
...
class __declspec(uuid("9f21981d-d469-4349-b807-39e64e4674e1"))
IVssCreateWriterMetadataEx : public IVssCreateWriterMetadata,
                             public IUnknown {
public:
  // add files to exclude to metadata document
  STDMETHOD(AddExcludeFilesFromSnapshot)
  (_In_ LPCWSTR wszPath, _In_ LPCWSTR wszFilespec, _In_ bool bRecursive) = 0;
};
...
public:
  // initialize writer
  virtual HRESULT
  Initialize(_In_ VSS_ID writerId, _In_ LPCWSTR wszWriterName,
             _In_opt_ LPCWSTR wszWriterInstanceName, _In_ DWORD dwMajorVersion,
             _In_ DWORD dwMinorVersion, _In_ VSS_USAGE_TYPE ut,
             _In_ VSS_SOURCE_TYPE st, _In_ VSS_APPLICATION_LEVEL nLevel,
             _In_ DWORD dwTimeout, _In_ VSS_ALTERNATE_WRITER_STATE aws,
             _In_ bool bIOThrottlingOnly) = 0;

  // subscribe to events
  virtual HRESULT Subscribe(_In_ DWORD dwSubscribeTimeout,
                            _In_ DWORD dwEventFlags) = 0;
  ...
  virtual HRESULT GetSnapshotDeviceName(
      ...
      _Outptr_result_maybenull_ LPCWSTR *ppwszSnapshotDevice) const = 0;
  ...
  virtual HRESULT SetWriterFailure(_In_ HRESULT hr) = 0;

  // determine if requestor support partial file backups
  virtual bool IsPartialFileSupportEnabled() const = 0;

  virtual HRESULT InstallAlternateWriter(_In_ VSS_ID idWriter,
                                         _In_ CLSID clsid) = 0;
  ...
  // let writer pass back indication of reason for failure
  virtual HRESULT SetWriterFailureEx(_In_ HRESULT hr,
                                     _In_ HRESULT hrApplication,
                                     _In_ LPCWSTR wszApplicationMessage) = 0;

  virtual HRESULT GetSessionId(_Out_ VSS_ID *idSession) = 0;

  virtual bool IsWriterShuttingDown() = 0;
};

class __declspec(uuid("9c772e77-b26e-427f-92dd-c996f41ea5e3"))
IVssCreateExpressWriterMetadata : public IUnknown {
public:
  // add files to exclude to metadata document
  STDMETHOD(AddExcludeFiles)(_In_ LPCWSTR wszPath, _In_ LPCWSTR wszFilespec,
                             _In_ bool bRecursive) = 0;

  // add component to metadata document
  STDMETHOD(AddComponent)(_In_ VSS_COMPONENT_TYPE ct,
                          _In_ LPCWSTR wszLogicalPath,
                          _In_ LPCWSTR wszComponentName,
                          _In_ LPCWSTR wszCaption, _In_ const BYTE *pbIcon,
                          _In_ UINT cbIcon, _In_ bool bRestoreMetadata,
                          _In_ bool bNotifyOnBackupComplete,
                          _In_ bool bSelectable,
                          _In_ bool bSelectableForRestore = false,
                          _In_ DWORD dwComponentFlags = 0) = 0;

  // add files to a FILE_GROUP component
  STDMETHOD(AddFilesToFileGroup)(
      _In_ LPCWSTR wszLogicalPath, _In_ LPCWSTR wszGroupName,
      _In_ LPCWSTR wszPath, _In_ LPCWSTR wszFilespec, _In_ bool bRecursive,
      _In_ LPCWSTR wszAlternateLocation,
      _In_ DWORD dwBackupTypeMask = (VSS_FSBT_ALL_BACKUP_REQUIRED |
                                     VSS_FSBT_ALL_SNAPSHOT_REQUIRED)) = 0;

  // create a restore method
  STDMETHOD(SetRestoreMethod)(_In_ VSS_RESTOREMETHOD_ENUM method,
                              _In_ LPCWSTR wszService,
                              _In_ LPCWSTR wszUserProcedure,
                              _In_ VSS_WRITERRESTORE_ENUM writerRestore,
                              _In_ bool bRebootRequired) = 0;

  // add a dependency to another writer's component
  STDMETHOD(AddComponentDependency)(_In_ LPCWSTR wszForLogicalPath,
                                    _In_ LPCWSTR wszForComponentName,
                                    _In_ VSS_ID onWriterId,
                                    _In_ LPCWSTR wszOnLogicalPath,
                                    _In_ LPCWSTR wszOnComponentName) = 0;

  // Set the schema used during backup
  STDMETHOD(SetBackupSchema)(_In_ DWORD dwSchemaMask) = 0;

  // save document as an XML string
  STDMETHOD(SaveAsXML)(_Out_ BSTR *pbstrXML) = 0;
};
...
public:
  STDMETHOD(CreateMetadata)(
      _In_ VSS_ID writerId, _In_ LPCWSTR writerName,
      _In_ VSS_USAGE_TYPE usageType, _In_ DWORD versionMajor,
      _In_ DWORD versionMinor, _In_ DWORD reserved,
      _Out_ IVssCreateExpressWriterMetadata **ppMetadata) = 0;

  STDMETHOD(LoadMetadata)(_In_ LPCWSTR metadata, _In_ DWORD reserved) = 0;

  STDMETHOD(Register)() = 0;

  STDMETHOD(Unregister)(_In_ VSS_ID writerId) = 0;
};
...
extern "C" {
HRESULT STDAPICALLTYPE CreateWriter(_In_ CVssWriter *pWriter,
...
public:
  ...
  Initialize(_In_ VSS_ID WriterId, _In_ LPCWSTR wszWriterName,
             _In_ VSS_USAGE_TYPE ut, _In_ VSS_SOURCE_TYPE st,
             _In_ VSS_APPLICATION_LEVEL nLevel = VSS_APP_FRONT_END,
             _In_ DWORD dwTimeoutFreeze =
                 60000, // Maximum milliseconds between Freeze/Thaw
    ...
    hr = m_pWriterImpl->Initialize(WriterId, wszWriterName,
                                   wszWriterInstanceName, 0, 0, ut, st, nLevel,
                                   dwTimeoutFreeze, aws, bIOThrottlingOnly);
    if (FAILED(hr)) {
      m_pWriterImpl->Release();
      m_pWriterImpl = NULL;
    }
  ...
  HRESULT STDMETHODCALLTYPE
  Subscribe(_In_ DWORD dwEventFlags = VSS_SM_BACKUP_EVENTS_FLAG |
  ...
  HRESULT STDMETHODCALLTYPE InstallAlternateWriter(
      _In_ VSS_ID writerId, _In_ CLSID persistentWriterClassId) {
    if (m_pWriterImpl == NULL)
      return E_FAIL;

    return m_pWriterImpl->InstallAlternateWriter(writerId,
                                                 persistentWriterClassId);
  }
...
protected:
  ...
  HRESULT STDMETHODCALLTYPE GetSnapshotDeviceName(
      _In_ LPCWSTR wszOriginalVolume,
      _Outptr_result_maybenull_ LPCWSTR *ppwszSnapshotDevice) const {
    if (m_pWriterImpl == NULL)
      return E_FAIL;

    return m_pWriterImpl->GetSnapshotDeviceName(wszOriginalVolume,
                                                ppwszSnapshotDevice);
  }
  ...
  bool STDMETHODCALLTYPE IsPathAffected(_In_ LPCWSTR wszPath) const {
    if (m_pWriterImpl == NULL)
      return false;
...
public:
  ...
  OnIdentify(IN IVssCreateWriterMetadata *pMetadata) {
  ...
  virtual bool STDMETHODCALLTYPE
  OnPrepareBackup(_In_ IVssWriterComponents *pComponent) {
  ...
  virtual bool STDMETHODCALLTYPE
  OnBackupComplete(_In_ IVssWriterComponents *pComponent) {
    UNREFERENCED_PARAMETER(pComponent);
    return true;
  }

  // callback indicating that the backup process has either completed or has
  // shut down
  virtual bool STDMETHODCALLTYPE OnBackupShutdown(_In_ VSS_ID SnapshotSetId) {
  ...
  virtual bool STDMETHODCALLTYPE
  OnPreRestore(_In_ IVssWriterComponents *pComponent) {
    UNREFERENCED_PARAMETER(pComponent);
    return true;
  }
  ...
  virtual bool STDMETHODCALLTYPE
  OnPostRestore(_In_ IVssWriterComponents *pComponent) {
    UNREFERENCED_PARAMETER(pComponent);
    return true;
  }
  ...
  virtual bool STDMETHODCALLTYPE
  OnPostSnapshot(_In_ IVssWriterComponents *pComponent) {
    UNREFERENCED_PARAMETER(pComponent);
    return true;
  }
  ...
  virtual bool STDMETHODCALLTYPE
  OnBackOffIOOnVolume(_In_ VSS_PWSZ wszVolumeName, _In_ VSS_ID snapshotId,
  ...
  virtual bool STDMETHODCALLTYPE
  OnContinueIOOnVolume(_In_ VSS_PWSZ wszVolumeName, _In_ VSS_ID snapshotId,
                       _In_ VSS_ID providerId) {
    UNREFERENCED_PARAMETER(wszVolumeName);
    UNREFERENCED_PARAMETER(snapshotId);
    UNREFERENCED_PARAMETER(providerId);
  ...
  virtual bool STDMETHODCALLTYPE OnVSSShutdown() { return true; }

  // callback to an alternative writer when the application writer subscribes.
  // Used to signal the alternative writer to shutdown.
  virtual bool STDMETHODCALLTYPE OnVSSApplicationStartup() { return true; }
...
class CVssWriterEx;
extern "C" {
HRESULT STDMETHODCALLTYPE CreateWriterEx(_In_ CVssWriterEx *pWriter,
                                         _Out_ IVssWriterImpl **pWriterImpl);
}
...
public:
  ...
  HRESULT STDMETHODCALLTYPE InitializeEx(
      _In_ VSS_ID WriterId, _In_ LPCWSTR wszWriterName,
      _In_ DWORD dwMajorVersion, _In_ DWORD dwMinorVersion,
      _In_ VSS_USAGE_TYPE ut, _In_ VSS_SOURCE_TYPE st,
      _In_ VSS_APPLICATION_LEVEL nLevel = VSS_APP_FRONT_END,
      _In_ DWORD dwTimeoutFreeze =
          60000, // Maximum milliseconds between Freeze/Thaw
    ...
    hr = m_pWriterImpl->Initialize(WriterId, wszWriterName,
                                   wszWriterInstanceName, dwMajorVersion,
                                   dwMinorVersion, ut, st, nLevel,
                                   dwTimeoutFreeze, aws, bIOThrottlingOnly);
    if (FAILED(hr)) {
      m_pWriterImpl->Release();
      m_pWriterImpl = NULL;
  ...
  virtual bool STDMETHODCALLTYPE
  OnIdentifyEx(_In_ IVssCreateWriterMetadataEx *pMetadata) {
    UNREFERENCED_PARAMETER(pMetadata);
    return false;
  }

protected:
  HRESULT GetIdentifyInformation(_Out_ IVssExamineWriterMetadata **ppMetadata) {
  ...
  // cause the writer to subscribe to events.
  HRESULT STDMETHODCALLTYPE
  SubscribeEx(_In_ DWORD dwUnsubscribeTimeout,
              _In_ DWORD dwEventFlags = VSS_SM_BACKUP_EVENTS_FLAG |
                                        VSS_SM_RESTORE_EVENTS_FLAG) {
    if (m_pWriterImpl == NULL)
      return E_FAIL;
...
public:
  HRESULT STDMETHODCALLTYPE SetWriterFailureEx(
      _In_ HRESULT hrWriter, _In_ HRESULT hrApplication = S_FALSE,
      _In_ LPCWSTR wszApplicationMessage = NULL) {
    if (m_pWriterImpl == NULL)
      return E_FAIL;

    return m_pWriterImpl->SetWriterFailureEx(hrWriter, hrApplication,
                                             wszApplicationMessage);
  }

  HRESULT STDMETHODCALLTYPE GetSessionId(_Out_ VSS_ID *idSession) {
    if (m_pWriterImpl == NULL)
      return E_FAIL;
...
CreateVssExpressWriterInternal(_Out_ IVssExpressWriter **ppWriter);
}

inline HRESULT CreateVssExpressWriter(_Out_ IVssExpressWriter **ppWriter) {
