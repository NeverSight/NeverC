...
typedef struct _DAV_CALLBACK_AUTH_BLOB {
  ...
  ULONG ulType; // Type of Cred sent in ppBuffer. Currently the only value it
                // supports is 1 for PCCERT_CONTEXT.
...
typedef struct _DAV_CALLBACK_CRED {
  ...
  BOOL bAuthBlobValid; // Indicates if AuthBlob contents is valid. If false
                       // UNPBlob is valid
  BOOL bSave; // Indicates the value of bsave flag. True indicates that the
              // creds were written to credman false indicates that creds were
              // not written to credman
...
//  Function signature for the callback to delete the memory allocated by the
//  creds
...
typedef DWORD(CALLBACK *PFNDAVAUTHCALLBACK)(
    ...
    _Out_ PFNDAVAUTHCALLBACK_FREECRED
        *pFreeCred // Callback to free the memory allocated for creds by the
                   // callback
);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavAddConnection(_Inout_ HANDLE *ConnectionHandle, _In_ LPCWSTR RemoteName,
                     _In_opt_ LPCWSTR UserName, _In_opt_ LPCWSTR Password,
                     _In_reads_bytes_(CertSize) PBYTE ClientCert,
                     _In_ DWORD CertSize);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavDeleteConnection(_In_ HANDLE ConnectionHandle);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavGetUNCFromHTTPPath(_In_ LPCWSTR Url,
                          _Out_writes_to_opt_(*lpSize, *lpSize) LPWSTR UncPath,
                          _Inout_ LPDWORD lpSize);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavGetHTTPFromUNCPath(_In_ LPCWSTR UncPath,
                          _Out_writes_to_opt_(*lpSize, *lpSize) LPWSTR Url,
                          _Inout_ LPDWORD lpSize);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI DavGetTheLockOwnerOfTheFile(
    _In_ LPCWSTR FileName,
    _Out_writes_bytes_opt_(*LockOwnerNameLengthInBytes) PWSTR LockOwnerName,
    _Inout_ PULONG LockOwnerNameLengthInBytes);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavGetExtendedError(_In_ HANDLE hFile, _Out_ DWORD *ExtError,
                        _Out_writes_(*cChSize) LPWSTR ExtErrorString,
                        _Inout_ DWORD *cChSize);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI DavFlushFile(_In_ HANDLE hFile);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavInvalidateCache(_In_ LPCWSTR URLName);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    DavCancelConnectionsToServer(_In_ LPWSTR lpName, _In_ BOOL fForce);

OPAQUE_HANDLE
WINAPI
DavRegisterAuthCallback(_In_ PFNDAVAUTHCALLBACK CallBack, _In_ ULONG Version);

VOID WINAPI DavUnregisterAuthCallback(_In_ OPAQUE_HANDLE hCallback);
