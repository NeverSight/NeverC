...
DWORD
PXEAPI
PxeProviderRegister(_In_ LPCWSTR pszProviderName, _In_ LPCWSTR pszModulePath,
                    _In_ PXE_REG_INDEX Index, _In_ BOOL bIsCritical,
                    _Out_opt_ PHKEY phProviderKey);

DWORD
PXEAPI
PxeProviderUnRegister(_In_ LPCWSTR pszProviderName);

DWORD
PXEAPI
PxeProviderQueryIndex(_In_ LPCWSTR pszProviderName, _Out_ PULONG puIndex);

DWORD
PXEAPI
PxeProviderEnumFirst(_Out_ HANDLE *phEnum);

DWORD
PXEAPI
PxeProviderEnumNext(_In_ HANDLE hEnum, _Out_ PPXE_PROVIDER *ppProvider);

DWORD
PXEAPI
PxeProviderEnumClose(_In_ HANDLE hEnum);

DWORD
PxeProviderFreeInfo(_In_ PPXE_PROVIDER pProvider);
...
DWORD
PXEAPI
PxeRegisterCallback(_In_ HANDLE hProvider, _In_ PXE_CALLBACK_TYPE CallbackType,
                    _In_ PVOID pCallbackFunction, _In_opt_ PVOID pContext);
...
DWORD
PXEAPI
PxeSendReply(_In_ HANDLE hClientRequest,
             _In_reads_bytes_(uPacketLen) PVOID pPacket, _In_ ULONG uPacketLen,
...
DWORD
PXEAPI
PxeAsyncRecvDone(_In_ HANDLE hClientRequest, _In_ PXE_BOOT_ACTION Action);
...
DWORD
PXEAPI
PxeTrace(_In_ HANDLE hProvider, _In_ PXE_SEVERITY Severity,
         _In_ LPCWSTR pszFormat, ...);

DWORD
PXEAPI
PxeTraceV(_In_ HANDLE hProvider, _In_ PXE_SEVERITY Severity,
          _In_ LPCWSTR pszFormat, _In_ va_list Params);
...
PVOID
PXEAPI
PxePacketAllocate(_In_ HANDLE hProvider, _In_ HANDLE hClientRequest,
                  _In_ ULONG uSize);

DWORD
PXEAPI
PxePacketFree(_In_ HANDLE hProvider, _In_ HANDLE hClientRequest,
              _In_ PVOID pPacket);
...
DWORD
PXEAPI
PxeProviderSetAttribute(_In_ HANDLE hProvider,
...
DWORD
PXEAPI
PxeDhcpInitialize(_In_reads_bytes_(uRecvPacketLen) PVOID pRecvPacket,
                  _In_ ULONG uRecvPacketLen,
                  _Out_writes_bytes_to_(uMaxReplyPacketLen, *puReplyPacketLen)
                      PVOID pReplyPacket,
                  _In_ ULONG uMaxReplyPacketLen, _Out_ PULONG puReplyPacketLen);

DWORD
PXEAPI
PxeDhcpv6Initialize(_In_reads_bytes_(cbRequest) PVOID pRequest,
                    _In_ ULONG cbRequest,
                    _Out_writes_bytes_to_(cbReply, *pcbReplyUsed) PVOID pReply,
                    _In_ ULONG cbReply, _Out_ PULONG pcbReplyUsed);

DWORD
PXEAPI
PxeDhcpAppendOption(_Inout_updates_bytes_(uMaxReplyPacketLen)
                        PVOID pReplyPacket,
                    _In_ ULONG uMaxReplyPacketLen,
                    _Inout_ PULONG puReplyPacketLen, _In_ BYTE bOption,
                    _In_ BYTE bOptionLen,
                    _In_reads_bytes_opt_(bOptionLen) PVOID pValue);

_Success_(return == ERROR_SUCCESS) DWORD PXEAPI
    PxeDhcpv6AppendOption(_Inout_updates_bytes_to_(cbReply, *pcbReplyUsed)
                              PVOID pReply,
                          _In_ ULONG cbReply, _Inout_ PULONG pcbReplyUsed,
                          _In_ WORD wOptionType, _In_ WORD cbOption,
                          _In_reads_bytes_(cbOption) PVOID pOption);

DWORD
PXEAPI
PxeDhcpAppendOptionRaw(_Inout_updates_bytes_(uMaxReplyPacketLen)
                           PVOID pReplyPacket,
                       _In_ ULONG uMaxReplyPacketLen,
                       _Inout_ PULONG puReplyPacketLen, _In_ USHORT uBufferLen,
                       _In_reads_bytes_(uBufferLen) PVOID pBuffer);

_Success_(return == ERROR_SUCCESS) DWORD PXEAPI
    PxeDhcpv6AppendOptionRaw(_Inout_updates_bytes_to_(cbReply, *pcbReplyUsed)
                                 PVOID pReply,
                             _In_ ULONG cbReply, _Inout_ PULONG pcbReplyUsed,
...
DWORD
PXEAPI
PxeDhcpIsValid(_In_reads_bytes_(uPacketLen) PVOID pPacket,
               _In_ ULONG uPacketLen, _In_ BOOL bRequestPacket,
               _Out_opt_ PBOOL pbPxeOptionPresent);

DWORD
PXEAPI
PxeDhcpv6IsValid(_In_reads_bytes_(uPacketLen) PVOID pPacket,
                 _In_ ULONG uPacketLen, _In_ BOOL bRequestPacket,
                 _Out_opt_ PBOOL pbPxeOptionPresent);

DWORD
PXEAPI
PxeDhcpGetOptionValue(_In_reads_bytes_(uPacketLen) PVOID pPacket,
                      _In_ ULONG uPacketLen, _In_ ULONG uInstance,
                      _In_ BYTE bOption, _Out_opt_ PBYTE pbOptionLen,
                      _Outptr_opt_result_bytebuffer_(*pbOptionLen)
                          PVOID *ppOptionValue);

DWORD
PXEAPI
PxeDhcpv6GetOptionValue(_In_reads_bytes_(uPacketLen) PVOID pPacket,
                        _In_ ULONG uPacketLen, _In_ ULONG uInstance,
                        _In_ WORD wOption, _Out_opt_ PWORD pwOptionLen,
                        _Outptr_opt_result_bytebuffer_(*pwOptionLen)
                            PVOID *ppOptionValue);

DWORD
PXEAPI
PxeDhcpGetVendorOptionValue(_In_reads_bytes_(uPacketLen) PVOID pPacket,
                            _In_ ULONG uPacketLen, _In_ BYTE bOption,
                            _In_ ULONG uInstance, _Out_opt_ PBYTE pbOptionLen,
                            _Outptr_opt_result_bytebuffer_(*pbOptionLen)
                                PVOID *ppOptionValue);

DWORD
PXEAPI
PxeDhcpv6GetVendorOptionValue(_In_reads_bytes_(uPacketLen) PVOID pPacket,
                              _In_ ULONG uPacketLen,
                              _In_ DWORD dwEnterpriseNumber, _In_ WORD wOption,
                              _In_ ULONG uInstance, _Out_opt_ PWORD pwOptionLen,
                              _Outptr_opt_result_bytebuffer_(*pwOptionLen)
                                  PVOID *ppOptionValue);

DWORD
PXEAPI
PxeDhcpv6ParseRelayForw(_In_reads_bytes_(uRelayForwPacketLen)
                            PVOID pRelayForwPacket,
                        _In_ ULONG uRelayForwPacketLen,
                        _Out_writes_to_(nRelayMessages, *pnRelayMessages)
                            PPXE_DHCPV6_NESTED_RELAY_MESSAGE pRelayMessages,
                        _In_ ULONG nRelayMessages, _Out_ PULONG pnRelayMessages,
                        _Outptr_result_bytebuffer_(*pcbInnerPacket)
                            PBYTE *ppInnerPacket,
                        _Out_ PULONG pcbInnerPacket);

DWORD
PXEAPI
PxeDhcpv6CreateRelayRepl(_In_reads_(nRelayMessages)
                             PPXE_DHCPV6_NESTED_RELAY_MESSAGE pRelayMessages,
                         _In_ ULONG nRelayMessages,
                         _In_reads_bytes_(cbInnerPacket) PBYTE pInnerPacket,
                         _In_ ULONG cbInnerPacket,
                         _Out_writes_bytes_to_(cbReplyBuffer, *pcbReplyBuffer)
                             PVOID pReplyBuffer,
                         _In_ ULONG cbReplyBuffer, _Out_ PULONG pcbReplyBuffer);
...
DWORD
PXEAPI
PxeGetServerInfo(_In_ PXE_GSI_TYPE uInfoType,
                 _Out_writes_bytes_(uBufferLen) PVOID pBuffer,
                 _In_ ULONG uBufferLen);

DWORD
PXEAPI
PxeGetServerInfoEx(_In_ PXE_GSI_TYPE uInfoType,
                   _Out_writes_bytes_to_(uBufferLen, *puBufferUsed)
                       PVOID pBuffer,
                   _In_ ULONG uBufferLen, _Out_ PULONG puBufferUsed);
