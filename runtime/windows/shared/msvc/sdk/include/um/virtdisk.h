...
DWORD
WINAPI
OpenVirtualDisk(_In_ PVIRTUAL_STORAGE_TYPE VirtualStorageType, _In_ PCWSTR Path,
...
typedef enum _CREATE_VIRTUAL_DISK_FLAG {
  ...
  // If set, the SourceLimitPath is an change tracking ID, and all data that has
  // changed since that change tracking ID will be copied from the source. If
  // clear, the SourceLimitPath is a VHD file path in the source VHD's chain,
  // and all data that is present in the children of that VHD in the chain will
  ...
  // backing store, but intsead use the file as is. If this flag is not
  // specified and a source file is passed to CreateVirtualDisk for a VHDSet
  // file, the data in the source file is copied. If this flag is set the data
  // is moved. The name of the file may change.
  CREATE_VIRTUAL_DISK_FLAG_VHD_SET_USE_ORIGINAL_BACKING_STORAGE = 0x40,

  //
  // When creating a fixed virtual disk, take advantage of an underlying sparse
  // file. Only supported on file systems that support sparse VDLs.
  //
  CREATE_VIRTUAL_DISK_FLAG_SPARSE_FILE = 0x80,

  //
  // Creates a VHD suitable as the backing store for a virtual persistent memory
  // device.
  //
  ...
  //
  // Allow a VHD to be created when it may be marked as a sparse file. This flag
  // is a companion to CREATE_VIRTUAL_DISK_FLAG_SPARSE_FILE, and overrides the
  // behavior that only allows sparse files on file systems that support sparse
  // VDLs.
  //
...
#define CREATE_VIRTUAL_DISK_FLAG_USE_RCT_SOURCE_LIMIT                          \
...
DWORD
WINAPI
CreateVirtualDisk(_In_ PVIRTUAL_STORAGE_TYPE VirtualStorageType,
                  _In_ PCWSTR Path,
                  _In_ VIRTUAL_DISK_ACCESS_MASK VirtualDiskAccessMask,
                  ...
                  _In_opt_ LPOVERLAPPED Overlapped, _Out_ PHANDLE Handle);
...
DWORD
WINAPI
AttachVirtualDisk(_In_ HANDLE VirtualDiskHandle,
                  _In_opt_ PSECURITY_DESCRIPTOR SecurityDescriptor,
...
DWORD
WINAPI
DetachVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
GetVirtualDiskPhysicalPath(_In_ HANDLE VirtualDiskHandle,
                           _Inout_ PULONG DiskPathSizeInBytes,
                           _Out_writes_bytes_(*DiskPathSizeInBytes)
                               PWSTR DiskPath);
...
#define GET_STORAGE_DEPENDENCY_FLAG_PARENTS                                    \
...
WINAPI
GetStorageDependencyInformation(
    _In_ HANDLE ObjectHandle, _In_ GET_STORAGE_DEPENDENCY_FLAG Flags,
...
_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    GetVirtualDiskInformation(_In_ HANDLE VirtualDiskHandle,
                              _Inout_ PULONG VirtualDiskInfoSize,
                              _Inout_updates_bytes_to_(*VirtualDiskInfoSize,
                                                       *VirtualDiskInfoSize)
...
DWORD
WINAPI
SetVirtualDiskInformation(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
EnumerateVirtualDiskMetadata(_In_ HANDLE VirtualDiskHandle,
                             _Inout_ PULONG NumberOfItems,
                             _Out_writes_(*NumberOfItems) GUID *Items);

DWORD
WINAPI
GetVirtualDiskMetadata(_In_ HANDLE VirtualDiskHandle, _In_ const GUID *Item,
                       _Inout_ PULONG MetaDataSize,
                       _Out_writes_bytes_(*MetaDataSize) PVOID MetaData);

DWORD
WINAPI
SetVirtualDiskMetadata(_In_ HANDLE VirtualDiskHandle, _In_ const GUID *Item,
                       _In_ ULONG MetaDataSize,
                       _In_reads_bytes_(MetaDataSize) const void *MetaData);

DWORD
WINAPI
DeleteVirtualDiskMetadata(_In_ HANDLE VirtualDiskHandle, _In_ const GUID *Item);
...
DWORD WINAPI GetVirtualDiskOperationProgress(
    _In_ HANDLE VirtualDiskHandle, _In_ LPOVERLAPPED Overlapped,
...
DWORD
WINAPI
CompactVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
MergeVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
ExpandVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
ResizeVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
MirrorVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
BreakMirrorVirtualDisk(_In_ HANDLE VirtualDiskHandle);
...
DWORD
WINAPI
AddVirtualDiskParent(_In_ HANDLE VirtualDiskHandle, _In_ PCWSTR ParentPath);
...
typedef struct _RAW_SCSI_VIRTUAL_DISK_PARAMETERS {
  ...
  union {
    struct {
      ...
      _Field_size_bytes_full_(DataTransferLength) PVOID DataBuffer;

      _Field_size_bytes_full_(SenseInfoLength) UCHAR *SenseInfo;

      _Field_size_bytes_full_(CdbLength) UCHAR *Cdb;
    } Version1;
  };
...
typedef struct _RAW_SCSI_VIRTUAL_DISK_RESPONSE {
  ...
  union {
    struct {
      UCHAR ScsiStatus;
      UCHAR SenseInfoLength; // bytes transferred to SenseInfo pointed to by the
                             // parameters.
      ULONG DataTransferLength; // bytes transferred to DataBuffer pointed to by
                                // the parameters.
    } Version1;
  };
...
DWORD
WINAPI
QueryChangesVirtualDisk(_In_ HANDLE VirtualDiskHandle,
                        _In_ PCWSTR ChangeTrackingId, _In_ ULONG64 ByteOffset,
                        _In_ ULONG64 ByteLength,
                        _In_ QUERY_CHANGES_VIRTUAL_DISK_FLAG Flags,
                        _Out_writes_to_(*RangeCount, *RangeCount)
...
DWORD
WINAPI
ForkVirtualDisk(_In_ HANDLE VirtualDiskHandle,
...
DWORD
WINAPI
CompleteForkVirtualDisk(_In_ HANDLE VirtualDiskHandle);
...
//
//  The Surface and Unsurface API names are deprecated.  Use Attach/Detach
//  versions instead.  If you use any of these defines below your code will
//  break post Win7.
//
...
#define SURFACE_VIRTUAL_DISK_VERSION_UNSPECIFIED                               \
...
#define SURFACE_VIRTUAL_DISK_FLAG_NO_DRIVE_LETTER                              \
  ATTACH_VIRTUAL_DISK_FLAG_NO_DRIVE_LETTER
#define SURFACE_VIRTUAL_DISK_FLAG_PERMANENT_LIFETIME                           \
  ATTACH_VIRTUAL_DISK_FLAG_PERMANENT_LIFETIME
#define SURFACE_VIRTUAL_DISK_FLAG_NO_LOCAL_HOST                                \
  ATTACH_VIRTUAL_DISK_FLAG_NO_LOCAL_HOST
#define SURFACE_VIRTUAL_DISK_FLAG_NO_SECURITY_DESCRIPTOR                       \
  ATTACH_VIRTUAL_DISK_FLAG_NO_SECURITY_DESCRIPTOR
#define SURFACE_VIRTUAL_DISK_FLAG_BYPASS_DEFAULT_ENCRYPTION_POLICY             \
