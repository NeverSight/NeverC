...
//
//  Synopsis:
//      Deduces the class and arguments and then calls a member-function
//      property setter callback for a value-type property.
//
//      This should not be called directly.
//
//--------------------------------------------------------------------------------
template <class C, typename P, typename I>
HRESULT DeducingValueSetter(_In_ HRESULT (C::*callback)(P), _In_ I *effect,
  ...
  return (static_cast<C *>(effect)->*callback)(
...
//
//  Synopsis:
//      Calls a member-function property setter callback for a value-type
//      property.
//
//--------------------------------------------------------------------------------
...
//  Synopsis:
//      Deduces the class and arguments and then calls a member-function
//      property getter callback for a value-type property.
//
//      This should not be called directly.
//
//--------------------------------------------------------------------------------
template <class C, typename P, typename I>
HRESULT DeducingValueGetter(_In_ P (C::*callback)() const, _In_ const I *effect,
  ...
  if (dataSize > 0 && data) {
    ...
    *reinterpret_cast<P *>(data) =
        (static_cast<const C *>(effect)->*callback)();
  }
...
//  Synopsis:
//      Deduces the class and arguments and then calls a member-function
//      property setter callback for a blob-type property.
//
//      This should not be called directly.
//
//--------------------------------------------------------------------------------
template <class C, typename I>
HRESULT DeducingBlobSetter(_In_ HRESULT (C::*callback)(const BYTE *, UINT32),
                           _In_ I *effect,
                           _In_reads_(dataSize) const BYTE *data,
                           UINT32 dataSize) {
...
//
//  Synopsis:
//      Calls a member-function property setter callback for a blob-type
//      property.
//
//--------------------------------------------------------------------------------
template <typename T, T P, typename I>
...
//  Synopsis:
//      Deduces the class and arguments and then calls a member-function
//      property getter callback for a blob-type property.
//
//      This should not be called directly.
//
//--------------------------------------------------------------------------------
template <class C, typename I>
HRESULT
DeducingBlobGetter(_In_ HRESULT (C::*callback)(BYTE *, UINT32, UINT32 *) const,
                   _In_ const I *effect, _Out_writes_opt_(dataSize) BYTE *data,
                   UINT32 dataSize, _Out_opt_ UINT32 *actualSize) {
  return (static_cast<const C *>(effect)->*callback)(data, dataSize,
                                                     actualSize);
}
...
//
//  Synopsis:
//      Calls a member-function property getter callback for a blob-type
//      property.
//
//--------------------------------------------------------------------------------
template <typename T, T P, typename I>
HRESULT CALLBACK BlobGetter(_In_ const IUnknown *effect,
                            ...
                            UINT32 dataSize, _Out_opt_ UINT32 *actualSize) {
  // Cast through I to resolve multiple-inheritance ambiguities.
  return DeducingBlobGetter(P, static_cast<const I *>(effect), data, dataSize,
                            actualSize);
}
...
//  Synopsis:
//      Deduces the class and arguments and then calls a member-function
//      property setter callback for a string-type property.
//
//      This should not be called directly.
//
//--------------------------------------------------------------------------------
template <class C, typename I>
HRESULT
DeducingStringSetter(_In_ HRESULT (C::*callback)(PCWSTR string), _In_ I *effect,
                     _In_reads_(dataSize) const BYTE *data, UINT32 dataSize) {
  (void)dataSize;
...
//
//  Synopsis:
//      Calls a member-function property setter callback for a string-type
//      property.
//
//--------------------------------------------------------------------------------
template <typename T, T P, typename I>
...
//  Synopsis:
//      Deduces the class and arguments and then calls a member-function
//      property getter callback for a string-type property.
//
//      This should not be called directly.
//
//--------------------------------------------------------------------------------
template <class C, typename I>
HRESULT DeducingStringGetter(_In_ HRESULT (C::*callback)(PWSTR, UINT32,
                                                         UINT32 *) const,
                             _In_ const I *effect,
                             _Out_writes_opt_(dataSize) BYTE *data,
                             UINT32 dataSize, _Out_opt_ UINT32 *actualSize) {
  ...
  HRESULT hr = (static_cast<const C *>(effect)->*callback)(
      reinterpret_cast<PWSTR>(data), dataSize / sizeof(WCHAR), &cchString);
...
//
//  Synopsis:
//      Calls a member-function property getter callback for a string-type
//      property.
//
//--------------------------------------------------------------------------------
template <typename T, T P, typename I>
HRESULT CALLBACK StringGetter(_In_ const IUnknown *effect,
                              ...
                              UINT32 dataSize, _Out_opt_ UINT32 *actualSize) {
  // Cast through I to resolve multiple-inheritance ambiguities.
  return DeducingStringGetter(P, static_cast<const I *>(effect), data, dataSize,
                              actualSize);
}
...
//
// Helper macros for declaring a D2D1_PROPERTY_BINDING for value, blob, or
// string callbacks.
//
...
#define D2D1_BLOB_TYPE_BINDING(NAME, SETTER, GETTER)                           \
  {NAME, &BlobSetter<decltype(GetType(SETTER)), SETTER, ID2D1EffectImpl>,      \
   &BlobGetter<decltype(GetType(GETTER)), GETTER, ID2D1EffectImpl>}

#define D2D1_STRING_TYPE_BINDING(NAME, SETTER, GETTER)                         \
  {NAME, &StringSetter<decltype(GetType(SETTER)), SETTER, ID2D1EffectImpl>,    \
   &StringGetter<decltype(GetType(GETTER)), GETTER, ID2D1EffectImpl>}
