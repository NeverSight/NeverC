...
//
// Implemented as a macro so that the caller can delay load credui.dll only if
// this
//      macro returns TRUE.
//
// Include only status codes that imply the username/password are wrong or that
// the
//      password is expired.  In the former case, asking for a another username
//      or password is appropriate.  In the later case, we put up a different
//      dialog asking the user to change the password on the server.
//
// Don't include status codes such as ERROR_ACCOUNT_DISABLED,
// ERROR_ACCOUNT_RESTRICTION,
//      ERROR_ACCOUNT_LOCKED_OUT, ERROR_ACCOUNT_EXPIRED,
//      ERROR_LOGON_TYPE_NOT_GRANTED. For those, the user isn't going to have
//      another account so prompting him won't help.
//
// STATUS_DOWNGRADE_DETECTED is included to handle the case where a corporate
// laptop
...
//      other LAN.
//
// STATUS_NO_SUCH_LOGON_SESSION and related error codes come from error mappings
// of
//      SEC_E_NO_CREDENTIALS and generally indicates that there are not
//      credentials available for SSO and some need to be provided.
//
// STATUS_NO_SUCH_USER comes when a username is mistyped or a certificate
// credential is
//      used where a username/password credential is expected.
//
// Don't use the CREDUIP_* macros directly.  Their definition is private to
// credui.dll.
//
...
#define CREDUIP_IS_USER_PASSWORD_ERROR(_Status)                                \
  ((_Status) == STATUS_SMB_GUEST_LOGON_BLOCKED ||                              \
   ...
   (_Status) ==                                                                \
       HRESULT_FROM_NT(STATUS_SMB_GUEST_LOGON_BLOCKED_SIGNING_REQUIRED) ||     \
   (_Status) == ERROR_SMB_GUEST_LOGON_BLOCKED_SIGNING_REQUIRED ||              \
   (_Status) ==                                                                \
       __HRESULT_FROM_WIN32(ERROR_SMB_GUEST_LOGON_BLOCKED_SIGNING_REQUIRED) || \
   (_Status) == ERROR_SMB_GUEST_ENCRYPTION_NOT_SUPPORTED ||                    \
   (_Status) ==                                                                \
       __HRESULT_FROM_WIN32(ERROR_SMB_GUEST_ENCRYPTION_NOT_SUPPORTED) ||       \
   ...
   (_Status) == SEC_E_NO_CONTEXT || (_Status) == STATUS_NO_SECURITY_CONTEXT)
...
#define CRED_TARGETNAME_DOMAIN_NAMESPACE_LENGTH                                \
  (sizeof(CRED_TARGETNAME_DOMAIN_NAMESPACE_A) - 1)
#define CRED_UNIVERSAL_WILDCARD_W L'*'
#define CRED_UNIVERSAL_WILDCARD_A '*'
...
#define CRED_TARGETNAME_LEGACYGENERIC_NAMESPACE_LENGTH                         \
  (sizeof(CRED_TARGETNAME_LEGACYGENERIC_NAMESPACE_A) - 1)
...
#define CRED_TARGETNAME_ATTRIBUTE_TARGET_LENGTH                                \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_TARGET_A) - 1)
#define CRED_TARGETNAME_ATTRIBUTE_NAME_W L"name"
#define CRED_TARGETNAME_ATTRIBUTE_NAME_A "name"
#define CRED_TARGETNAME_ATTRIBUTE_NAME_LENGTH                                  \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_NAME_A) - 1)
#define CRED_TARGETNAME_ATTRIBUTE_BATCH_W L"batch"
#define CRED_TARGETNAME_ATTRIBUTE_BATCH_A "batch"
#define CRED_TARGETNAME_ATTRIBUTE_BATCH_LENGTH                                 \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_BATCH_A) - 1)
#define CRED_TARGETNAME_ATTRIBUTE_INTERACTIVE_W L"interactive"
#define CRED_TARGETNAME_ATTRIBUTE_INTERACTIVE_A "interactive"
#define CRED_TARGETNAME_ATTRIBUTE_INTERACTIVE_LENGTH                           \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_INTERACTIVE_A) - 1)
#define CRED_TARGETNAME_ATTRIBUTE_SERVICE_W L"service"
#define CRED_TARGETNAME_ATTRIBUTE_SERVICE_A "service"
#define CRED_TARGETNAME_ATTRIBUTE_SERVICE_LENGTH                               \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_SERVICE_A) - 1)
#define CRED_TARGETNAME_ATTRIBUTE_NETWORK_W L"network"
#define CRED_TARGETNAME_ATTRIBUTE_NETWORK_A "network"
#define CRED_TARGETNAME_ATTRIBUTE_NETWORK_LENGTH                               \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_NETWORK_A) - 1)
...
#define CRED_TARGETNAME_ATTRIBUTE_NETWORKCLEARTEXT_LENGTH                      \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_NETWORKCLEARTEXT_A) - 1)
...
#define CRED_TARGETNAME_ATTRIBUTE_REMOTEINTERACTIVE_LENGTH                     \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_REMOTEINTERACTIVE_A) - 1)
...
#define CRED_TARGETNAME_ATTRIBUTE_CACHEDINTERACTIVE_LENGTH                     \
  (sizeof(CRED_TARGETNAME_ATTRIBUTE_CACHEDINTERACTIVE_A) - 1)
...
#define CRED_TARGETNAME_NAMESPACE_SEPERATOR                                    \
  = CRED_TARGETNAME_NAMESPACE_SEPERATOR_W
#define CRED_TARGETNAME_ATTRIBUTE_SEPERATOR                                    \
...
#define CRED_TARGETNAME_ATTRIBUTE_INTERACTIVE                                  \
...
#define CRED_TARGETNAME_ATTRIBUTE_NETWORKCLEARTEXT                             \
  CRED_TARGETNAME_ATTRIBUTE_NETWORKCLEARTEXT_W
#define CRED_TARGETNAME_ATTRIBUTE_REMOTEINTERACTIVE                            \
  CRED_TARGETNAME_ATTRIBUTE_REMOTEINTERACTIVE_W
#define CRED_TARGETNAME_ATTRIBUTE_CACHEDINTERACTIVE                            \
...
#define CredAppendLogonTypeToFlags(Flags, LogonType)                           \
  (Flags) |= ((LogonType) << 12)
#define CredGetLogonTypeFromFlags(Flags)                                       \
  ((SECURITY_LOGON_TYPE)(((Flags) & CRED_LOGON_TYPES_MASK) >> 12))
...
#define CRED_TYPE_MAXIMUM_EX                                                   \
  (CRED_TYPE_MAXIMUM + 1000) // Allow new applications to run on old OSes
...
#ifdef MIDL_PASS
  [ string,
    max_is(CRED_MAX_GENERIC_TARGET_NAME_LENGTH - 1) ] wchar_t *TargetName;
#else  // MIDL_PASS
  LPWSTR TargetName;
#endif // MIDL_PASS
#ifdef MIDL_PASS
...
#define CRED_TI_SERVER_FORMAT_UNKNOWN                                          \
  0x0001 // Don't know if server name is DNS or netbios format
#define CRED_TI_DOMAIN_FORMAT_UNKNOWN                                          \
  0x0002 // Don't know if domain name is DNS or netbios format
#define CRED_TI_ONLY_PASSWORD_REQUIRED                                         \
  0x0004 // Server only requires a password and not a username
#define CRED_TI_USERNAME_TARGET 0x0008 // TargetName is username
#define CRED_TI_CREATE_EXPLICIT_CRED                                           \
  0x0010 // When creating a cred, create one named TargetInfo->TargetName
#define CRED_TI_WORKGROUP_MEMBER                                               \
  0x0020 // Indicates the machine is a member of a workgroup
#define CRED_TI_DNSTREE_IS_DFS_SERVER                                          \
  0x0040 // used to tell credman that the DNSTreeName could be DFS server
...
#ifdef MIDL_PASS
  [ string,
    max_is(CRED_MAX_DOMAIN_TARGET_NAME_LENGTH - 1) ] wchar_t *TargetName;
  [ string,
    max_is(CRED_MAX_DOMAIN_TARGET_NAME_LENGTH - 1) ] wchar_t *NetbiosServerName;
  [ string,
    max_is(CRED_MAX_DOMAIN_TARGET_NAME_LENGTH - 1) ] wchar_t *DnsServerName;
  [ string,
    max_is(CRED_MAX_DOMAIN_TARGET_NAME_LENGTH - 1) ] wchar_t *NetbiosDomainName;
  [ string,
    max_is(CRED_MAX_DOMAIN_TARGET_NAME_LENGTH - 1) ] wchar_t *DnsDomainName;
  [ string,
    max_is(CRED_MAX_DOMAIN_TARGET_NAME_LENGTH - 1) ] wchar_t *DnsTreeName;
...
//
// This credential can be pass to LsaLogonUser to ask it to find a credential
// with a TargetName of UserName.
//
...
#define CREDUI_FLAGS_INCORRECT_PASSWORD                                        \
  0x00001 // indicates the username is valid, but password is not
#define CREDUI_FLAGS_DO_NOT_PERSIST                                            \
  0x00002 // Do not show "Save" checkbox, and do not persist credentials
#define CREDUI_FLAGS_REQUEST_ADMINISTRATOR                                     \
  0x00004 // Populate list box with admin accounts
#define CREDUI_FLAGS_EXCLUDE_CERTIFICATES                                      \
  0x00008 // do not include certificates in the drop list
...
#define CREDUI_FLAGS_PERSIST                                                   \
  0x01000 // Do not show "Save" checkbox, but persist credentials anyway
#define CREDUI_FLAGS_SERVER_CREDENTIAL 0x04000
#define CREDUI_FLAGS_EXPECT_CONFIRMATION                                       \
  0x20000 // do not persist unless caller later confirms credential via
          // CredUIConfirmCredential() api
#define CREDUI_FLAGS_GENERIC_CREDENTIALS                                       \
  0x40000 // Credential is a generic credential
#define CREDUI_FLAGS_USERNAME_TARGET_CREDENTIALS                               \
  0x80000 // Credential has a username as the target
#define CREDUI_FLAGS_KEEP_USERNAME                                             \
  0x100000 // don't allow the user to change the supplied username
...
#define CREDUIWIN_GENERIC                                                      \
  0x00000001 // Plain text username/password is being requested
#define CREDUIWIN_CHECKBOX 0x00000002 // Show the Save Credential checkbox
#define CREDUIWIN_AUTHPACKAGE_ONLY                                             \
  0x00000010 // Only Cred Providers that support the input auth package should
             // enumerate
#define CREDUIWIN_IN_CRED_ONLY                                                 \
  0x00000020 // Only the incoming cred for the specific auth package should be
             // enumerated
#define CREDUIWIN_ENUMERATE_ADMINS                                             \
  0x00000100 // Cred Providers should enumerate administrators only
#define CREDUIWIN_ENUMERATE_CURRENT_USER                                       \
  0x00000200 // Only the incoming cred for the specific auth package should be
             // enumerated
#define CREDUIWIN_SECURE_PROMPT                                                \
  0x00001000 // The Credui prompt should be displayed on the secure desktop
#define CREDUIWIN_PREPROMPTING                                                 \
  0X00002000 // CredUI is invoked by SspiPromptForCredentials and the client is
             // prompting before a prior handshake
#define CREDUIWIN_PACK_32_WOW                                                  \
  0x10000000 // Tell the credential provider it should be packing its Auth Blob
             // 32 bit even though it is running 64 native
#define CREDUIWIN_USE_V2 0x00000040 // For the rejuvenated CREDUI experience
#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)
#define CREDUIWIN_IGNORE_CLOUDAUTHORITY_NAME                                   \
  0x00040000 // Tell the credential provider it should not pack AAD authority
             // name
#endif
#if (NTDDI_VERSION >= NTDDI_WIN10_RS5)
#define CREDUIWIN_DOWNLEVEL_HELLO_AS_SMART_CARD                                \
  0x80000000 // Force collected Hello credentials to be packed in a smart card
             // auth buffer.
#endif
...
#define CREDUIWIN_VALID_FLAGS                                                  \
  (CREDUIWIN_GENERIC | CREDUIWIN_CHECKBOX | CREDUIWIN_AUTHPACKAGE_ONLY |       \
   CREDUIWIN_IN_CRED_ONLY | CREDUIWIN_ENUMERATE_ADMINS |                       \
   CREDUIWIN_ENUMERATE_CURRENT_USER | CREDUIWIN_SECURE_PROMPT |                \
   CREDUIWIN_PREPROMPTING | CREDUIWIN_PACK_32_WOW |                            \
...
WINADVAPI
BOOL WINAPI CredWriteW(_In_ PCREDENTIALW Credential, _In_ DWORD Flags);

WINADVAPI
BOOL WINAPI CredWriteA(_In_ PCREDENTIALA Credential, _In_ DWORD Flags);
...
WINADVAPI
BOOL WINAPI CredReadW(_In_ LPCWSTR TargetName, _In_ DWORD Type,
                      _Reserved_ DWORD Flags, _Out_ PCREDENTIALW *Credential);

WINADVAPI
BOOL WINAPI CredReadA(_In_ LPCSTR TargetName, _In_ DWORD Type,
                      _Reserved_ DWORD Flags, _Out_ PCREDENTIALA *Credential);
...
WINADVAPI
BOOL WINAPI CredEnumerateW(_In_opt_ LPCWSTR Filter, _Reserved_ DWORD Flags,
                           _Out_ DWORD *Count,
                           _Outptr_result_buffer_(*Count)
                               PCREDENTIALW **Credential);

WINADVAPI
BOOL WINAPI CredEnumerateA(_In_opt_ LPCSTR Filter, _Reserved_ DWORD Flags,
                           _Out_ DWORD *Count,
                           _Outptr_result_buffer_(*Count)
...
WINADVAPI
BOOL WINAPI
CredWriteDomainCredentialsW(_In_ PCREDENTIAL_TARGET_INFORMATIONW TargetInfo,
                            _In_ PCREDENTIALW Credential, _In_ DWORD Flags);

WINADVAPI
BOOL WINAPI
CredWriteDomainCredentialsA(_In_ PCREDENTIAL_TARGET_INFORMATIONA TargetInfo,
                            _In_ PCREDENTIALA Credential, _In_ DWORD Flags);
...
WINADVAPI
BOOL WINAPI CredReadDomainCredentialsW(
    _In_ PCREDENTIAL_TARGET_INFORMATIONW TargetInfo, _In_ DWORD Flags,
    _Out_ DWORD *Count,
    _Outptr_result_buffer_(*Count) PCREDENTIALW **Credential);

WINADVAPI
BOOL WINAPI CredReadDomainCredentialsA(
    _In_ PCREDENTIAL_TARGET_INFORMATIONA TargetInfo, _In_ DWORD Flags,
    _Out_ DWORD *Count,
...
WINADVAPI
BOOL WINAPI CredDeleteW(_In_ LPCWSTR TargetName, _In_ DWORD Type,
                        _Reserved_ DWORD Flags);

WINADVAPI
BOOL WINAPI CredDeleteA(_In_ LPCSTR TargetName, _In_ DWORD Type,
                        _Reserved_ DWORD Flags);
...
WINADVAPI
BOOL WINAPI CredRenameW(_In_ LPCWSTR OldTargetName, _In_ LPCWSTR NewTargetName,
                        _In_ DWORD Type, _Reserved_ DWORD Flags);

WINADVAPI
BOOL WINAPI CredRenameA(_In_ LPCSTR OldTargetName, _In_ LPCSTR NewTargetName,
                        _In_ DWORD Type, _Reserved_ DWORD Flags);
...
WINADVAPI
BOOL WINAPI
CredGetTargetInfoW(_In_ LPCWSTR TargetName, _In_ DWORD Flags,
                   _Out_ PCREDENTIAL_TARGET_INFORMATIONW *TargetInfo);

WINADVAPI
BOOL WINAPI
CredGetTargetInfoA(_In_ LPCSTR TargetName, _In_ DWORD Flags,
...
WINADVAPI
BOOL WINAPI CredMarshalCredentialW(_In_ CRED_MARSHAL_TYPE CredType,
                                   _In_ PVOID Credential,
                                   _Out_ LPWSTR *MarshaledCredential);

WINADVAPI
BOOL WINAPI CredMarshalCredentialA(_In_ CRED_MARSHAL_TYPE CredType,
                                   _In_ PVOID Credential,
...
WINADVAPI
BOOL WINAPI CredUnmarshalCredentialW(_In_ LPCWSTR MarshaledCredential,
                                     _Out_ PCRED_MARSHAL_TYPE CredType,
                                     _Out_ PVOID *Credential);

WINADVAPI
BOOL WINAPI CredUnmarshalCredentialA(_In_ LPCSTR MarshaledCredential,
                                     _Out_ PCRED_MARSHAL_TYPE CredType,
                                     _Out_ PVOID *Credential);
...
WINADVAPI
BOOL WINAPI CredIsMarshaledCredentialW(_In_ LPCWSTR MarshaledCredential);
...
WINADVAPI
BOOL WINAPI CredIsMarshaledCredentialA(_In_ LPCSTR MarshaledCredential);
...
CREDUIAPI
BOOL WINAPI CredUnPackAuthenticationBufferW(
    _In_ DWORD dwFlags, _In_reads_bytes_(cbAuthBuffer) PVOID pAuthBuffer,
...
CREDUIAPI
BOOL WINAPI CredUnPackAuthenticationBufferA(
    _In_ DWORD dwFlags, _In_reads_bytes_(cbAuthBuffer) PVOID pAuthBuffer,
    _In_ DWORD cbAuthBuffer,
...
CREDUIAPI
BOOL WINAPI CredPackAuthenticationBufferW(
    _In_ DWORD dwFlags, _In_ LPWSTR pszUserName, _In_ LPWSTR pszPassword,
    _Out_writes_bytes_opt_(*pcbPackedCredentials) PBYTE pPackedCredentials,
    _Inout_ DWORD *pcbPackedCredentials);

CREDUIAPI
BOOL WINAPI CredPackAuthenticationBufferA(
    _In_ DWORD dwFlags, _In_ LPSTR pszUserName, _In_ LPSTR pszPassword,
    _Out_writes_bytes_opt_(*pcbPackedCredentials) PBYTE pPackedCredentials,
    _Inout_ DWORD *pcbPackedCredentials);
...
_Success_(return) WINADVAPI BOOL WINAPI
    CredProtectW(_In_ BOOL fAsSelf,
                 _In_reads_(cchCredentials) LPWSTR pszCredentials,
                 _In_ DWORD cchCredentials,
                 _Out_writes_to_(*pcchMaxChars, *pcchMaxChars)
...
_Success_(return) WINADVAPI BOOL WINAPI
    CredProtectEx(_In_ ULONG Flags,
                  _In_reads_(cchCredentials) LPWSTR pszCredentials,
                  _In_ DWORD cchCredentials,
                  _Out_writes_to_(*pcchMaxChars, *pcchMaxChars)
                      LPWSTR pszProtectedCredentials,
...
WINADVAPI
BOOL WINAPI CredProtectA(_In_ BOOL fAsSelf,
                         _In_reads_(cchCredentials) LPSTR pszCredentials,
                         _In_ DWORD cchCredentials,
                         _Out_writes_(*pcchMaxChars)
...
_Success_(return) WINADVAPI BOOL WINAPI
    CredUnprotectW(_In_ BOOL fAsSelf,
                   _In_reads_(cchProtectedCredentials)
                       LPWSTR pszProtectedCredentials,
                   _In_ DWORD cchProtectedCredentials,
                   _Out_writes_to_opt_(*pcchMaxChars, *pcchMaxChars)
                       LPWSTR pszCredentials,
                   _Inout_ DWORD *pcchMaxChars);

#define CRED_UNPROTECT_AS_SELF 0x1
#define CRED_UNPROTECT_ALLOW_TO_SYSTEM 0x2
#define CRED_UNPROTECT_VALID_FLAGS                                             \
  (CRED_UNPROTECT_AS_SELF | CRED_UNPROTECT_ALLOW_TO_SYSTEM)

_Success_(return) WINADVAPI BOOL WINAPI
    CredUnprotectEx(_In_ ULONG Flags,
                    _In_reads_(cchProtectedCredentials)
                        LPWSTR pszProtectedCredentials,
                    _In_ DWORD cchProtectedCredentials,
                    _Out_writes_to_opt_(*pcchMaxChars, *pcchMaxChars)
...
WINADVAPI
BOOL WINAPI CredUnprotectA(_In_ BOOL fAsSelf,
                           _In_reads_(cchProtectedCredentials)
                               LPSTR pszProtectedCredentials,
                           _In_ DWORD cchProtectedCredentials,
...
WINADVAPI
BOOL WINAPI CredIsProtectedW(_In_ LPWSTR pszProtectedCredentials,
                             _Out_ CRED_PROTECTION_TYPE *pProtectionType);

WINADVAPI
BOOL WINAPI CredIsProtectedA(_In_ LPSTR pszProtectedCredentials,
                             _Out_ CRED_PROTECTION_TYPE *pProtectionType);
...
WINADVAPI
BOOL WINAPI CredFindBestCredentialW(_In_ LPCWSTR TargetName, _In_ DWORD Type,
                                    _In_ DWORD Flags,
                                    _Out_ PCREDENTIALW *Credential);

WINADVAPI
BOOL WINAPI CredFindBestCredentialA(_In_ LPCSTR TargetName, _In_ DWORD Type,
                                    _In_ DWORD Flags,
...
WINADVAPI
BOOL WINAPI CredGetSessionTypes(_In_ DWORD MaximumPersistCount,
                                _Out_writes_(MaximumPersistCount)
                                    LPDWORD MaximumPersist);

WINADVAPI
VOID WINAPI CredFree(_In_ PVOID Buffer);
...
CredUIPromptForCredentialsW(
    _In_opt_ PCREDUI_INFOW pUiInfo, _In_opt_ PCWSTR pszTargetName,
    _Reserved_ PCtxtHandle pContext, _In_ DWORD dwAuthError,
    ...
    _In_ ULONG ulPasswordBufferSize, _Inout_opt_ BOOL *save,
    _In_ DWORD dwFlags);
...
CredUIPromptForCredentialsA(
    _In_opt_ PCREDUI_INFOA pUiInfo, _In_opt_ PCSTR pszTargetName,
    _Reserved_ PCtxtHandle pContext, _In_ DWORD dwAuthError,
...
CredUIPromptForWindowsCredentialsW(
    _In_opt_ PCREDUI_INFOW pUiInfo, _In_ DWORD dwAuthError,
    _Inout_ ULONG *pulAuthPackage,
    _In_reads_bytes_opt_(ulInAuthBufferSize) LPCVOID pvInAuthBuffer,
    _In_ ULONG ulInAuthBufferSize,
    _Outptr_result_bytebuffer_to_(*pulOutAuthBufferSize, *pulOutAuthBufferSize)
        LPVOID *ppvOutAuthBuffer,
    _Out_ ULONG *pulOutAuthBufferSize, _Inout_opt_ BOOL *pfSave,
    _In_ DWORD dwFlags);
...
CredUIPromptForWindowsCredentialsA(
    _In_opt_ PCREDUI_INFOA pUiInfo, _In_ DWORD dwAuthError,
    _Inout_ ULONG *pulAuthPackage,
    _In_reads_bytes_opt_(ulInAuthBufferSize) LPCVOID pvInAuthBuffer,
    _In_ ULONG ulInAuthBufferSize,
    _Outptr_result_bytebuffer_to_(*pulOutAuthBufferSize, *pulOutAuthBufferSize)
...
DWORD
WINAPI
CredUIParseUserNameW(_In_ PCWSTR UserName,
...
DWORD
WINAPI
CredUIParseUserNameA(_In_ PCSTR userName,
...
CredUICmdLinePromptForCredentialsW(
    _In_opt_ PCWSTR pszTargetName, _Reserved_ PCtxtHandle pContext,
    _In_ DWORD dwAuthError, _Inout_updates_(ulUserBufferSize) PWSTR UserName,
    _In_ ULONG ulUserBufferSize,
    _Inout_updates_(ulPasswordBufferSize) PWSTR pszPassword,
    _In_ ULONG ulPasswordBufferSize, _Inout_opt_ PBOOL pfSave,
    _In_ DWORD dwFlags);
...
CredUICmdLinePromptForCredentialsA(
    _In_opt_ PCSTR pszTargetName, _Reserved_ PCtxtHandle pContext,
    _In_ DWORD dwAuthError, _Inout_updates_(ulUserBufferSize) PSTR UserName,
    _In_ ULONG ulUserBufferSize,
    _Inout_updates_(ulPasswordBufferSize) PSTR pszPassword,
    _In_ ULONG ulPasswordBufferSize, _Inout_opt_ PBOOL pfSave,
    _In_ DWORD dwFlags);
...
//
// Call this API with bConfirm set to TRUE to confirm that the credential
// (previously created via CredUIGetCredentials or CredUIPromptForCredentials
// worked, or with bConfirm set to FALSE to indicate it didn't

CREDUIAPI
DWORD
WINAPI
CredUIConfirmCredentialsW(_In_ PCWSTR pszTargetName, _In_ BOOL bConfirm);

CREDUIAPI
DWORD
WINAPI
CredUIConfirmCredentialsA(_In_ PCSTR pszTargetName, _In_ BOOL bConfirm);
...
DWORD
WINAPI
CredUIStoreSSOCredW(_In_opt_ PCWSTR pszRealm, _In_ PCWSTR pszUsername,
                    _In_ PCWSTR pszPassword, _In_ BOOL bPersist);

CREDUIAPI
DWORD
WINAPI
CredUIReadSSOCredW(_In_opt_ PCWSTR pszRealm, _Outptr_ PWSTR *ppszUsername);
