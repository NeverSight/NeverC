...
// File:     drt.h
//
// Contents: Win32 APIs and structures for the Microsoft Distributed Routing
// Table.
//
...
#define DRT_E_INVALID_MAX_ENDPOINTS                                            \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x1011)
#define DRT_E_INVALID_SEARCH_RANGE                                             \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x1012)

#define DRT_E_INVALID_PORT MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2000)
#define DRT_E_INVALID_TRANSPORT_PROVIDER                                       \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2001)
#define DRT_E_INVALID_SECURITY_PROVIDER                                        \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2002)
#define DRT_E_STILL_IN_USE MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2003)
#define DRT_E_INVALID_BOOTSTRAP_PROVIDER                                       \
...
#define DRT_E_TRANSPORT_SHUTTING_DOWN                                          \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2007)
#define DRT_E_NO_ADDRESSES_AVAILABLE                                           \
...
#define DRT_E_TRANSPORTPROVIDER_IN_USE                                         \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x200a)
#define DRT_E_TRANSPORTPROVIDER_NOT_ATTACHED                                   \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x200b)
#define DRT_E_SECURITYPROVIDER_IN_USE                                          \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x200c)
#define DRT_E_SECURITYPROVIDER_NOT_ATTACHED                                    \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x200d)
#define DRT_E_BOOTSTRAPPROVIDER_IN_USE                                         \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x200e)
#define DRT_E_BOOTSTRAPPROVIDER_NOT_ATTACHED                                   \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x200f)
...
#define DRT_E_INVALID_SETTINGS                                                 \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2108)
#define DRT_E_INVALID_SEARCH_INFO                                              \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x2109)
#define DRT_E_FAULTED MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x210a)
#define DRT_E_TRANSPORT_STILL_BOUND                                            \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x210b)
#define DRT_E_INSUFFICIENT_BUFFER                                              \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x210c)
#define DRT_E_INVALID_INSTANCE_PREFIX                                          \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x210d)
#define DRT_E_INVALID_SECURITY_MODE                                            \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x210e)
#define DRT_E_CAPABILITY_MISMATCH                                              \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_DRT, 0x210f)
...
typedef struct drt_security_provider_tag {
  ...
  HRESULT (*Attach)(_In_ const PVOID pvContext);

  VOID (*Detach)(_In_ const PVOID pvContext);

  HRESULT (*RegisterKey)(_In_ const PVOID pvContext,
                         _In_ const DRT_REGISTRATION *pRegistration,
                         _In_opt_ PVOID pvKeyContext);

  HRESULT (*UnregisterKey)(_In_ const PVOID pvContext,
                           _In_ const DRT_DATA *pKey,
                           _In_opt_ PVOID pvKeyContext);

  HRESULT (*ValidateAndUnpackPayload)(
      _In_ const PVOID pvContext, _In_ DRT_DATA *pSecuredAddressPayload,
      _In_opt_ DRT_DATA *pCertChain, _In_opt_ DRT_DATA *pClassifier,
      _In_opt_ DRT_DATA *pNonce, _In_opt_ DRT_DATA *pSecuredPayload,

      _Out_ BYTE *pbProtocolMajor, _Out_ BYTE *pbProtocolMinor,
      _Out_ DRT_DATA *pKey, _Out_opt_ DRT_DATA *pPayload,
      _Out_ CERT_PUBLIC_KEY_INFO **ppPublicKey,
      _Out_opt_ SOCKET_ADDRESS_LIST **ppAddressList, _Out_ DWORD *pdwFlags);

  HRESULT (*SecureAndPackPayload)(
      _In_ const PVOID pvContext, _In_opt_ PVOID pvKeyContext,
      BYTE bProtocolMajor, BYTE bProtocolMinor, DWORD dwFlags,
      _In_ const DRT_DATA *pKey, _In_opt_ const DRT_DATA *pPayload,
      _In_opt_ const SOCKET_ADDRESS_LIST *pAddressList,
      _In_ const DRT_DATA *pNonce,

      _Out_ DRT_DATA *pSecuredAddressPayload, _Out_opt_ DRT_DATA *pClassifier,
      _Out_opt_ DRT_DATA *pSecuredPayload, _Out_opt_ DRT_DATA *pCertChain);

  void (*FreeData)(_In_ const PVOID pvContext, _In_opt_ PVOID pv);

  HRESULT (*EncryptData)(_In_ const PVOID pvContext,
  ...
  HRESULT (*DecryptData)(_In_ const PVOID pvContext, _In_ DRT_DATA *pKeyToken,
                         _In_ const PVOID pvKeyContext, _In_ DWORD dwBuffers,
                         _Inout_updates_(dwBuffers) DRT_DATA *pData);

  HRESULT (*GetSerializedCredential)(_In_ const PVOID pvContext,
                                     _Out_ DRT_DATA *pSelfCredential);

  HRESULT (*ValidateRemoteCredential)(_In_ const PVOID pvContext,
                                      _In_ DRT_DATA *pRemoteCredential);

  HRESULT (*SignData)(_In_ const PVOID pvContext, _In_ DWORD dwBuffers,
                      _In_reads_(dwBuffers) DRT_DATA *pDataBuffers,
                      _Out_ DRT_DATA *pKeyIdentifier,
                      _Out_ DRT_DATA *pSignature);

  HRESULT (*VerifyData)(_In_ const PVOID pvContext, _In_ DWORD dwBuffers,
                        _In_reads_(dwBuffers) DRT_DATA *pDataBuffers,
...
typedef void(CALLBACK *DRT_BOOTSTRAP_RESOLVE_CALLBACK)(
    HRESULT hr, PVOID pvContext, SOCKET_ADDRESS_LIST *pAddresses,
    BOOL fFatalError);
...
typedef struct drt_bootstrap_provider_tag {
  ...
  HRESULT (*InitResolve)(_In_ const PVOID pvContext, _In_ BOOL fSplitDetect,
                         _In_ ULONG timeout, _In_ ULONG cMaxResults,
                         _Out_ DRT_BOOTSTRAP_RESOLVE_CONTEXT *ResolveContext,
                         _Out_ BOOL *fFatalError);
  HRESULT (*IssueResolve)(_In_ const PVOID pvContext,
  ...
  VOID (*EndResolve)(_In_ const PVOID pvContext,
                     _In_ DRT_BOOTSTRAP_RESOLVE_CONTEXT ResolveContext);

  HRESULT (*Register)(_In_ const PVOID pvContext,
                      _In_ const SOCKET_ADDRESS_LIST *pAddressList);

  VOID (*Unregister)(_In_ const PVOID pvContext);
...
HRESULT WINAPI DrtCreatePnrpBootstrapResolver(
    _In_ BOOL fPublish, _In_ PCWSTR pwzPeerName, _In_opt_ PCWSTR pwzCloudName,
    _In_opt_ PCWSTR pwzPublishingIdentity,
    _Out_ DRT_BOOTSTRAP_PROVIDER **ppResolver);

void WINAPI
DrtDeletePnrpBootstrapResolver(_In_ DRT_BOOTSTRAP_PROVIDER *pResolver);

HRESULT WINAPI
DrtCreateDnsBootstrapResolver(_In_ USHORT port, _In_ PCWSTR pwszAddress,
                              _Out_ DRT_BOOTSTRAP_PROVIDER **ppModule);

void WINAPI
DrtDeleteDnsBootstrapResolver(_In_ DRT_BOOTSTRAP_PROVIDER *pResolver);

HRESULT WINAPI DrtCreateIpv6UdpTransport(DRT_SCOPE scope, ULONG dwScopeId,
                                         ULONG dwLocalityThreshold,
                                         _Inout_ USHORT *pwPort,
                                         _Out_ HDRT_TRANSPORT *phTransport);

HRESULT WINAPI DrtDeleteIpv6UdpTransport(_In_ HDRT_TRANSPORT hTransport);

HRESULT WINAPI DrtCreateDerivedKeySecurityProvider(
    _In_ PCCERT_CONTEXT pRootCert, _In_opt_ PCCERT_CONTEXT pLocalCert,
    _Out_ DRT_SECURITY_PROVIDER **ppSecurityProvider);

HRESULT WINAPI DrtCreateDerivedKey(_In_ PCCERT_CONTEXT pLocalCert,
                                   _Out_ DRT_DATA *pKey);
...
HRESULT WINAPI
DrtCreateNullSecurityProvider(_Out_ DRT_SECURITY_PROVIDER **ppSecurityProvider);

void WINAPI
DrtDeleteNullSecurityProvider(_In_ DRT_SECURITY_PROVIDER *pSecurityProvider);

//////////////////////////////////////////////////////////////////////////////
// DRT Lifetime Management APIs

HRESULT WINAPI DrtOpen(_In_ const DRT_SETTINGS *pSettings,
                       _In_opt_ HANDLE hEvent, _In_opt_ const PVOID pvContext,
                       _Out_ HDRT *phDrt);

VOID WINAPI DrtClose(_In_ HDRT hDrt);

HRESULT WINAPI DrtGetEventDataSize(_In_ HDRT hDrt,
                                   _Out_ ULONG *pulEventDataLen);

HRESULT WINAPI DrtGetEventData(_In_ HDRT hDrt, _In_ ULONG ulEventDataLen,
                               _Out_writes_bytes_all_(ulEventDataLen)
                                   DRT_EVENT_DATA *pEventData);

//////////////////////////////////////////////////////////////////////////////
// DRT Key Registration APIs

HRESULT WINAPI
DrtRegisterKey(_In_ HDRT hDrt, _In_ DRT_REGISTRATION *pRegistration,
               _In_opt_ PVOID pvKeyContext,
               _Out_ HDRT_REGISTRATION_CONTEXT *phKeyRegistration);

HRESULT WINAPI DrtUpdateKey(_In_ HDRT_REGISTRATION_CONTEXT hKeyRegistration,
                            _In_ DRT_DATA *pAppData);

VOID WINAPI DrtUnregisterKey(_In_ HDRT_REGISTRATION_CONTEXT hKeyRegistration);

//////////////////////////////////////////////////////////////////////////////
// DRT Search APIs

HRESULT WINAPI DrtStartSearch(_In_ HDRT hDrt, _In_ DRT_DATA *pKey,
                              _In_opt_ const DRT_SEARCH_INFO *pInfo,
                              ULONG timeout, _In_ HANDLE hEvent,
                              _In_opt_ const PVOID pvContext,
                              _Out_ HDRT_SEARCH_CONTEXT *hSearchContext);

HRESULT WINAPI DrtContinueSearch(_In_ HDRT_SEARCH_CONTEXT hSearchContext);

HRESULT WINAPI DrtGetSearchResultSize(_In_ HDRT_SEARCH_CONTEXT hSearchContext,
                                      _Out_ ULONG *pulSearchResultSize);

HRESULT WINAPI DrtGetSearchResult(_In_ HDRT_SEARCH_CONTEXT hSearchContext,
                                  _Inout_ ULONG ulSearchResultSize,
                                  _Out_writes_bytes_all_(ulSearchResultSize)
                                      DRT_SEARCH_RESULT *pSearchResult);

HRESULT WINAPI DrtGetSearchPathSize(_In_ HDRT_SEARCH_CONTEXT hSearchContext,
                                    _Out_ ULONG *pulSearchPathSize);

HRESULT WINAPI DrtGetSearchPath(_In_ HDRT_SEARCH_CONTEXT hSearchContext,
                                _Inout_ ULONG ulSearchPathSize,
                                _Out_writes_bytes_all_(ulSearchPathSize)
                                    DRT_ADDRESS_LIST *pSearchPath);

HRESULT WINAPI DrtEndSearch(_In_ HDRT_SEARCH_CONTEXT hSearchContext);

//////////////////////////////////////////////////////////////////////////////
// DRT Instance name related APIs

HRESULT WINAPI DrtGetInstanceName(_In_ HDRT hDrt,
                                  _In_ ULONG ulcbInstanceNameSize,
                                  _Out_writes_bytes_all_(ulcbInstanceNameSize)
                                      PWSTR pwzDrtInstanceName);

HRESULT WINAPI DrtGetInstanceNameSize(_In_ HDRT hDrt,
