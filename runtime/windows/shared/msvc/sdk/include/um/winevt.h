...
EVT_HANDLE
WINAPI
EvtOpenSession(EVT_LOGIN_CLASS LoginClass,
               _When_((LoginClass == EvtRpcLogin),
                      _In_reads_bytes_(sizeof(EVT_RPC_LOGIN))) PVOID Login,
               _Reserved_ DWORD Timeout, _Reserved_ DWORD Flags);
...
_Success_(return != 0) BOOL WINAPI
    EvtClose(_In_ _Post_invalid_ EVT_HANDLE Object);

BOOL WINAPI EvtCancel(_In_opt_ EVT_HANDLE Object);

_Success_(return == ERROR_SUCCESS) DWORD WINAPI
    EvtGetExtendedStatus(DWORD BufferSize,
                         _Out_writes_to_opt_(BufferSize, *BufferUsed)
                             LPWSTR Buffer,
...
EVT_HANDLE
WINAPI
EvtQuery(_In_opt_ EVT_HANDLE Session, _In_opt_z_ LPCWSTR Path,
         _In_opt_z_ LPCWSTR Query, DWORD Flags);

_Success_(return != 0) BOOL WINAPI
    EvtNext(_In_ EVT_HANDLE ResultSet, DWORD EventsSize,
...
BOOL WINAPI EvtSeek(_In_ EVT_HANDLE ResultSet, LONGLONG Position,
                    _In_opt_ EVT_HANDLE Bookmark, _Reserved_ DWORD Timeout,
                    DWORD Flags);
...
typedef DWORD(WINAPI *EVT_SUBSCRIBE_CALLBACK)(
    EVT_SUBSCRIBE_NOTIFY_ACTION Action, _Maybenull_ PVOID UserContext,
    _In_ EVT_HANDLE Event);

EVT_HANDLE
WINAPI
EvtSubscribe(_In_opt_ EVT_HANDLE Session, _In_opt_ HANDLE SignalEvent,
             _In_opt_z_ LPCWSTR ChannelPath, _In_opt_z_ LPCWSTR Query,
             _In_opt_ EVT_HANDLE Bookmark, _Maybenull_ PVOID Context,
             _Maybenull_ EVT_SUBSCRIBE_CALLBACK Callback, DWORD Flags);
...
EVT_HANDLE
WINAPI
EvtCreateRenderContext(DWORD ValuePathsCount,
                       _In_reads_opt_(ValuePathsCount) LPCWSTR *ValuePaths,
                       DWORD Flags);

_Success_(return != 0) BOOL WINAPI
    EvtRender(_In_opt_ EVT_HANDLE Context, _In_ EVT_HANDLE Fragment,
              DWORD Flags, DWORD BufferSize,
              _Out_writes_bytes_to_opt_(BufferSize, *BufferUsed) PVOID Buffer,
              _Out_ PDWORD BufferUsed, _Out_ PDWORD PropertyCount);
...
_Success_(return != 0) BOOL WINAPI
    EvtFormatMessage(_In_opt_ EVT_HANDLE PublisherMetadata,
                     _In_opt_ EVT_HANDLE Event, DWORD MessageId,
                     DWORD ValueCount,
...
EVT_HANDLE
WINAPI
EvtOpenLog(_In_opt_ EVT_HANDLE Session, _In_z_ LPCWSTR Path, DWORD Flags);

_Success_(return != 0) BOOL WINAPI
    EvtGetLogInfo(_In_ EVT_HANDLE Log, EVT_LOG_PROPERTY_ID PropertyId,
                  DWORD PropertyValueBufferSize,
                  _Out_writes_bytes_to_opt_(PropertyValueBufferSize,
                                            *PropertyValueBufferUsed)
                      PEVT_VARIANT PropertyValueBuffer,
                  _Out_ PDWORD PropertyValueBufferUsed);

BOOL WINAPI EvtClearLog(_In_opt_ EVT_HANDLE Session, _In_z_ LPCWSTR ChannelPath,
                        _In_opt_z_ LPCWSTR TargetFilePath, DWORD Flags);
...
BOOL WINAPI EvtExportLog(_In_opt_ EVT_HANDLE Session, _In_opt_z_ LPCWSTR Path,
                         _In_opt_z_ LPCWSTR Query,
                         _In_z_ LPCWSTR TargetFilePath, DWORD Flags);

BOOL WINAPI EvtArchiveExportedLog(_In_opt_ EVT_HANDLE Session,
                                  _In_z_ LPCWSTR LogFilePath, LCID Locale,
                                  DWORD Flags);
...
typedef enum _EVT_CHANNEL_CONFIG_PROPERTY_ID {
  ...
  EvtChannelPublishingConfigClockType,   // EvtVarTypeUInt32,
...
EVT_HANDLE
WINAPI
EvtOpenChannelEnum(_In_opt_ EVT_HANDLE Session, DWORD Flags);

_Success_(return != 0) BOOL WINAPI
    EvtNextChannelPath(_In_ EVT_HANDLE ChannelEnum, DWORD ChannelPathBufferSize,
                       _Out_writes_to_opt_(ChannelPathBufferSize,
...
EVT_HANDLE
WINAPI
EvtOpenChannelConfig(_In_opt_ EVT_HANDLE Session, _In_z_ LPCWSTR ChannelPath,
                     DWORD Flags);

BOOL WINAPI EvtSaveChannelConfig(_In_ EVT_HANDLE ChannelConfig, DWORD Flags);

BOOL WINAPI EvtSetChannelConfigProperty(
    _In_ EVT_HANDLE ChannelConfig, EVT_CHANNEL_CONFIG_PROPERTY_ID PropertyId,
    DWORD Flags, _In_ PEVT_VARIANT PropertyValue);

_Success_(return != 0) BOOL WINAPI EvtGetChannelConfigProperty(
    _In_ EVT_HANDLE ChannelConfig, EVT_CHANNEL_CONFIG_PROPERTY_ID PropertyId,
    DWORD Flags, DWORD PropertyValueBufferSize,
    _Out_writes_bytes_to_opt_(PropertyValueBufferSize, *PropertyValueBufferUsed)
        PEVT_VARIANT PropertyValueBuffer,
    _Out_ PDWORD PropertyValueBufferUsed);
...
EVT_HANDLE
WINAPI
EvtOpenPublisherEnum(_In_opt_ EVT_HANDLE Session, DWORD Flags);

_Success_(return != 0) BOOL WINAPI
    EvtNextPublisherId(_In_ EVT_HANDLE PublisherEnum,
                       DWORD PublisherIdBufferSize,
                       _Out_writes_to_opt_(PublisherIdBufferSize,
...
EVT_HANDLE
WINAPI
EvtOpenPublisherMetadata(_In_opt_ EVT_HANDLE Session,
                         _In_z_ LPCWSTR PublisherId,
                         _In_opt_z_ LPCWSTR LogFilePath, LCID Locale,
                         DWORD Flags);

_Success_(return != 0) BOOL WINAPI EvtGetPublisherMetadataProperty(
    _In_ EVT_HANDLE PublisherMetadata,
    EVT_PUBLISHER_METADATA_PROPERTY_ID PropertyId, DWORD Flags,
    DWORD PublisherMetadataPropertyBufferSize,
    _Out_writes_bytes_to_opt_(PublisherMetadataPropertyBufferSize,
...
EVT_HANDLE
WINAPI
EvtOpenEventMetadataEnum(_In_ EVT_HANDLE PublisherMetadata, DWORD Flags);

EVT_HANDLE
WINAPI
EvtNextEventMetadata(_In_ EVT_HANDLE EventMetadataEnum, DWORD Flags);

_Success_(return != 0) BOOL WINAPI EvtGetEventMetadataProperty(
    _In_ EVT_HANDLE EventMetadata, EVT_EVENT_METADATA_PROPERTY_ID PropertyId,
    DWORD Flags, DWORD EventMetadataPropertyBufferSize,
    _Out_writes_bytes_to_opt_(EventMetadataPropertyBufferSize,
...
_Success_(return != 0) BOOL WINAPI
    EvtGetObjectArraySize(_In_ EVT_OBJECT_ARRAY_PROPERTY_HANDLE ObjectArray,
                          _Out_ PDWORD ObjectArraySize);

_Success_(return != 0) BOOL WINAPI EvtGetObjectArrayProperty(
    _In_ EVT_OBJECT_ARRAY_PROPERTY_HANDLE ObjectArray, DWORD PropertyId,
...
_Success_(return != 0) BOOL WINAPI
    EvtGetQueryInfo(_In_ EVT_HANDLE QueryOrSubscription,
...
EVT_HANDLE
WINAPI
EvtCreateBookmark(_In_opt_z_ LPCWSTR BookmarkXml);

BOOL WINAPI EvtUpdateBookmark(_In_ EVT_HANDLE Bookmark, _In_ EVT_HANDLE Event);

_Success_(return != 0) BOOL WINAPI
    EvtGetEventInfo(_In_ EVT_HANDLE Event, EVT_EVENT_PROPERTY_ID PropertyId,
                    DWORD PropertyValueBufferSize,
                    _Out_writes_bytes_to_opt_(PropertyValueBufferSize,
                                              *PropertyValueBufferUsed)
                        PEVT_VARIANT PropertyValueBuffer,
