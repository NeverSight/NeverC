...
BOOL _Success_(return) WINAPI
    WofShouldCompressBinaries(__in LPCWSTR Volume, __out PULONG Algorithm);

HRESULT
WINAPI
WofGetDriverVersion(__in HANDLE FileOrVolumeHandle, __in ULONG Provider,
...
HRESULT
WINAPI
WofSetFileDataLocation(__in HANDLE FileHandle, __in ULONG Provider,
                       __in PVOID ExternalFileInfo, __in ULONG Length);
...
HRESULT
WINAPI
WofIsExternalFile(__in LPCWSTR FilePath, __out_opt PBOOL IsExternalFile,
                  __out_opt PULONG Provider, __out_opt PVOID ExternalFileInfo,
...
typedef BOOL(CALLBACK *WofEnumEntryProc)(__in const PVOID EntryInfo,
...
typedef BOOL(CALLBACK *WofEnumFilesProc)(__in PCWSTR FilePath,
                                         __in PVOID ExternalFileInfo,
                                         __in_opt PVOID UserData);

HRESULT
WINAPI
WofEnumEntries(__in PCWSTR VolumeName, __in ULONG Provider,
               __in WofEnumEntryProc EnumProc, __in_opt PVOID UserData);
...
HRESULT
WINAPI
WofWimAddEntry(__in PCWSTR VolumeName, __in PCWSTR WimPath, __in DWORD WimType,
               __in DWORD WimIndex, __out PLARGE_INTEGER DataSourceId);

HRESULT
WINAPI
WofWimEnumFiles(__in PCWSTR VolumeName, __in LARGE_INTEGER DataSourceId,
                __in WofEnumFilesProc EnumProc, __in_opt PVOID UserData);

HRESULT
WINAPI
WofWimSuspendEntry(__in PCWSTR VolumeName, __in LARGE_INTEGER DataSourceId);

HRESULT
WINAPI
WofWimRemoveEntry(__in PCWSTR VolumeName, __in LARGE_INTEGER DataSourceId);

HRESULT
WINAPI
WofWimUpdateEntry(__in PCWSTR VolumeName, __in LARGE_INTEGER DataSourceId,
...
HRESULT
WINAPI
WofFileEnumFiles(__in PCWSTR VolumeName, __in ULONG Algorithm,
                 __in WofEnumFilesProc EnumProc, __in_opt PVOID UserData);
