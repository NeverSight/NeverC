...
BOOL WslIsDistributionRegistered(_In_ PCWSTR distributionName);

/* Registers a new distribution given the information provided. */
HRESULT
WslRegisterDistribution(_In_ PCWSTR distributionName,
                        _In_ PCWSTR tarGzFilename);

/* Unregisters the specified distribution */
HRESULT
WslUnregisterDistribution(_In_ PCWSTR distributionName);
...
HRESULT
WslConfigureDistribution(_In_ PCWSTR distributionName, _In_ ULONG defaultUID,
...
WslGetDistributionConfiguration(
    _In_ PCWSTR distributionName, _Out_ ULONG *distributionVersion,
    _Out_ ULONG *defaultUID, _Out_ WSL_DISTRIBUTION_FLAGS *wslDistributionFlags,
    _Outptr_result_buffer_(*defaultEnvironmentVariableCount)
        PSTR **defaultEnvironmentVariables,
    _Out_ ULONG *defaultEnvironmentVariableCount);

HRESULT
WslLaunchInteractive(_In_ PCWSTR distributionName, _In_opt_ PCWSTR command,
                     _In_ BOOL useCurrentWorkingDirectory,
                     _Out_ DWORD *exitCode);

HRESULT
WslLaunch(_In_ PCWSTR distributionName, _In_opt_ PCWSTR command,
          _In_ BOOL useCurrentWorkingDirectory, _In_ HANDLE stdIn,
          _In_ HANDLE stdOut, _In_ HANDLE stdErr, _Out_ HANDLE *process);
