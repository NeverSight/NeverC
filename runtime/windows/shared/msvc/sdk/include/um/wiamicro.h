...
#define MCRO_ERROR_USER_INTERVENTION                                           \
  6 // User needs to interact with the physical device
...
typedef struct _SCANINFO {
  ...
  LONG SupportedCompressionType; // (mask of supported compression types, 0 -
                                 // None)
  LONG SupportedDataTypes;       // (mask of supported types, (ie.
                                 // SUPPORT_COLOR|SUPPORT_BW...))
  // Current Image Info
  LONG
      WidthPixels; // (width of image, using current scanner settings in pixels)
  ...
  LONG DelayBetweenRead;   // delay between WIA Scan() calls requesting data
                           // (milliseconds)
  LONG MaxBufferSize;      // maximum buffer size in scanner
  HANDLE DeviceIOHandles[MAX_IO_HANDLES]; // Device IO handles needed for device
                                          // communication
  LONG lReserved[MAX_RESERVED];           // (silly reserved bits)
  VOID *pMicroDriverContext; // private data for Micro driver's only.
                             // The Micro Driver is responsible for allocating
                             // and freeing. CMD_INITIALIZE - allocate,
                             // CMD_UNINITIALIZE - free
} SCANINFO, *PSCANINFO;
...
WIAMICRO_API HRESULT Scan(_Inout_ PSCANINFO pScanInfo, LONG lPhase,
                          _Out_writes_bytes_(lLength) PBYTE pBuffer,
                          LONG lLength, _Out_ LONG *plReceived);
WIAMICRO_API HRESULT SetPixelWindow(_Inout_ PSCANINFO pScanInfo, LONG x, LONG y,
