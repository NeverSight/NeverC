...
#define IMF_IMECANCELCOMPLETE                                                  \
  0x0004 // High completes comp string when aborting, low cancels
...
// application only after OleLoad has been called.  With these messages it is
// possible to set the rich edit control to a mode where it will query the
// client application before OleLoad is called
...
#if (_RICHEDIT_VER >= 0x0800)
#define EPR_SE                                                                 \
  5 // Text flows top to bottom and left to right (Mongolian text layout)
#endif
...
typedef struct _tableRowParms { // EM_INSERTTABLE wparam is a (TABLEROWPARMS *)
  ...
  DWORD nAlignment
      : 3; // Row alignment (like PARAFORMAT::bAlignment, \trql, trqr, \trqc)
  DWORD fRTL : 1;  // Display cells in RTL order (\rtlrow)
  DWORD fKeep : 1; // Keep row together (\trkeep}
  DWORD fKeepFollow
      : 1;         // Keep row on same page as following row (\trkeepfollow)
...
typedef struct _tableCellParms { // EM_INSERTTABLE lparam is a (TABLECELLPARMS
                                 // *)
  ...
  WORD wShading;        // Shading in .01%		(\clshdng) e.g., 10000 flips
                        // fore/back
...
#define SES_EX_USEMOUSEWPARAM                                                  \
  0x20000000 // Use wParam when handling WM_MOUSEMOVE message and do not call
             // GetAsyncKeyState
...
#define SCF_NOKBUPDATE                                                         \
  0x0020                          // Do not update KB layout for this change
...
#define PFE_TABLEROWDELIMITER                                                  \
  (PFM_TABLEROWDELIMITER >> 16) // Table row start. RE 4.0
...
#define GCMF_SPELLING                                                          \
  0x00000002 // pSpellingSuggestions is valid
...
#define BOM_DEFPARADIR                                                         \
  0x0001 // Default paragraph direction (implies alignment) (obsolete)
...
void WINAPI HyphenateProc(_In_ WCHAR *pszWord, LANGID langid, long ichExceed,
...
// This constant is used for a discontinued lparam value in EM_AUTOURLDETECT
// handling
