...
Compiler switches:
    ...
    -DNT35_STRICT
        Specifies that the target platform is Windows NT 3.5. This switch
disables the new functions added after the Windows NT 3.5 release.
...
// By default, this header cannot be compiled as C++ without defining CINTERFACE
// (which uses a C-style view of COM interfaces for C++ in MIDL-generated
// headers), because type definitions here depend on *Vtbl types not present in
// this configuration. Define RPCPROXY_ENABLE_CPP_NO_CINTERFACE to enable this
// when needed.
#if defined(RPCPROXY_ENABLE_CPP_NO_CINTERFACE) && defined(__cplusplus) &&      \
    !defined(CINTERFACE)
...
#if _MSC_VER >= 1200
#pragma warning(push)
#pragma warning(disable : 4610) // struct can never be instantiated - user
...
#define IInspectableNdrStubCall2CommonStubListTag                              \
  ((const PRPC_STUB_FUNCTION *)-1)
#define IInspectableNdrStubCall3CommonStubListTag                              \
  ((const PRPC_STUB_FUNCTION *)-2)
...
#define RPCPROXY_GET_STUB_HEADER(StubVtblListEntry)                            \
  (&(StubVtblListEntry)->header)
#else
...
// a stub vtable list.
#define RPCPROXY_GET_STUB_HEADER(StubVtblListEntry)                            \
  (reinterpret_cast<const CInterfaceStubHeader *>(StubVtblListEntry))
...
typedef struct tagCStdStubBuffer {
  const struct IRpcStubBufferVtbl
      *lpVtbl; // Points to Vtbl field in CInterfaceStubVtbl.
  LONG RefCount;
...
RPCRTAPI
void RPC_ENTRY NdrProxyInitialize(void *This, PRPC_MESSAGE pRpcMsg,
...
RPCRTAPI
void RPC_ENTRY NdrStubInitializePartial(PRPC_MESSAGE pRpcMsg,
                                        PMIDL_STUB_MESSAGE pStubMsg,
                                        PMIDL_STUB_DESC pStubDescriptor,
                                        IRpcChannelBuffer *pRpcChannelBuffer,
                                        ULONG RequestedBufferSize);

void __RPC_STUB NdrStubForwardingFunction(IN IRpcStubBuffer *This,
...
ULONG STDMETHODCALLTYPE NdrCStdStubBuffer_Release(IRpcStubBuffer *This,
...
CStdStubBuffer_Invoke(IRpcStubBuffer *This, RPCOLEMESSAGE *pRpcMsg,
                      IRpcChannelBuffer *pRpcChannelBuffer);
...
HRESULT STDMETHODCALLTYPE CStdStubBuffer2_QueryInterface(IRpcStubBuffer *This,
                                                         REFIID riid,
                                                         void **ppvObject);

ULONG STDMETHODCALLTYPE CStdStubBuffer2_Release(IRpcStubBuffer *This);

HRESULT STDMETHODCALLTYPE CStdStubBuffer2_Connect(IRpcStubBuffer *pthis,
                                                  IUnknown *pUnkServer);
...
HRESULT STDMETHODCALLTYPE CStdAsyncStubBuffer_QueryInterface(
    IRpcStubBuffer *pthis, REFIID riid, void **ppvObject);
...
HRESULT STDMETHODCALLTYPE CStdAsyncStubBuffer_Connect(IRpcStubBuffer *pthis,
                                                      IUnknown *pUnkServer);

void STDMETHODCALLTYPE CStdAsyncStubBuffer_Disconnect(IRpcStubBuffer *pthis);

HRESULT STDMETHODCALLTYPE
CStdAsyncStubBuffer_Invoke(IRpcStubBuffer *pthis, RPCOLEMESSAGE *_prpcmsg,
                           IRpcChannelBuffer *_pRpcChannelBuffer);

ULONG STDMETHODCALLTYPE CStdAsyncStubBuffer2_Release(IRpcStubBuffer *pthis);

HRESULT STDMETHODCALLTYPE CStdAsyncStubBuffer2_Connect(IRpcStubBuffer *pthis,
                                                       IUnknown *pUnkServer);
...
#define CStdStubBuffer_DELEGATING_METHODS_OPT                                  \
  CStdStubBuffer2_QueryInterface, CStdStubBuffer_AddRef,                       \
      CStdStubBuffer2_Release, CStdStubBuffer2_Connect,                        \
      CStdStubBuffer2_Disconnect, CStdStubBuffer_Invoke,                       \
      CStdStubBuffer_IsIIDSupported, CStdStubBuffer2_CountRefs,                \
      CStdStubBuffer_DebugServerQueryInterface,                                \
      CStdStubBuffer_DebugServerRelease

#define CStdAsyncStubBuffer_METHODS_OPT                                        \
  CStdAsyncStubBuffer_QueryInterface, CStdAsyncStubBuffer_AddRef,              \
      CStdAsyncStubBuffer_Release, CStdAsyncStubBuffer_Connect,                \
...
#define CStdAsyncStubBuffer_DELEGATING_METHODS_OPT                             \
  CStdAsyncStubBuffer_QueryInterface, CStdAsyncStubBuffer_AddRef,              \
      CStdAsyncStubBuffer2_Release, CStdAsyncStubBuffer2_Connect,              \
      CStdAsyncStubBuffer2_Disconnect, CStdAsyncStubBuffer_Invoke,             \
      CStdStubBuffer_IsIIDSupported, CStdStubBuffer2_CountRefs,                \
      CStdStubBuffer_DebugServerQueryInterface,                                \
      CStdStubBuffer_DebugServerRelease
...
HRESULT
RPC_ENTRY
NdrDllGetClassObject(IN REFCLSID rclsid, IN REFIID riid, OUT void **ppv,
...
HRESULT
RPC_ENTRY
NdrDllRegisterProxy(IN HMODULE hDll, IN const ProxyFileInfo **pProxyFileList,
                    IN const CLSID *pclsid);

RPCRTAPI
HRESULT
RPC_ENTRY
NdrDllUnregisterProxy(IN HMODULE hDll, IN const ProxyFileInfo **pProxyFileList,
                      IN const CLSID *pclsid);
...
#define REGISTER_PROXY_DLL_ROUTINES(pProxyFileList, pClsID)                    \
  ...
  HRESULT STDAPICALLTYPE DLLREGISTERSERVER_ENTRY() {                           \
    return NdrDllRegisterProxy(                                                \
        hProxyDll, (const ProxyFileInfo **)pProxyFileList, pClsID);            \
  }                                                                            \
                                                                               \
  /* DllUnregisterServer unregisters the interfaces contained in the proxy     \
   * DLL. */                                                                   \
...
#define DLLGETCLASSOBJECTROUTINE(pPFlist, pClsid, pFactory)                    \
  _Check_return_ HRESULT STDAPICALLTYPE DLLGETCLASSOBJECT_ENTRY(               \
    ...
    return NdrDllGetClassObject(                                               \
        rclsid, riid, ppv, (const ProxyFileInfo **)pPFlist, pClsid, pFactory); \
  }
