...
template <class T, DWORD Magic> class CItemIDFactory : public IDelegateFolder {
protected:
...
public:
  ...
  BOOL IsDelegateFolder() { return (_pmalloc != NULL); }

  // get a read only pointer to the client provided structure in the first
  // ItemID in the IDList. returns NULL if the IDList isn't valid.

  static const UNALIGNED T *
  GetDataFromIDList(_In_opt_ PCUIDLIST_RELATIVE pidl) {
    PCITEM pitem = _IsValid(pidl);
    return pitem ? &pitem->innerData : NULL;
  }

  static HRESULT GetDataFromIDList(_In_ PCUIDLIST_RELATIVE pidl,
                                   _Outptr_ const UNALIGNED T **ppData) {
    *ppData = GetDataFromIDList(pidl);
    return (*ppData) ? S_OK : E_INVALIDARG;
  }

  // return a read only pointer to the serialized property storage that we use
  // for storing metadata

  static PCUSERIALIZEDPROPSTORAGE
  GetPropertyStorage(_In_opt_ PCUIDLIST_RELATIVE pidl, _Out_ DWORD *pcb) {
    PCITEM pitem = _IsValid(pidl);
  ...
  // return a property from the IPropertyStore within the IDList, convert the
  // resulting PROPVARIANT to a variant (useful when implementing
  // IShellFolder2::GetDetailsEx) return vt == VT_EMPTY if not found

  static HRESULT GetPropertyFromIDList(_In_ PCUIDLIST_RELATIVE pidl,
                                       _In_ REFPROPERTYKEY rkey,
                                       _Out_ VARIANT *pvar) {
  ...
  // read a PROPVARIANT from the IPropertyStore within the first ItemID in the
  // IDList.
  ...
  // read a PROPVARIANT from the IPropertyStore within the first ItemID in the
  // IDList using the named property
  ...
  // create an instance of the IPropertyStore based on the serialized property
  // storage associated with the first ItemID.

  static HRESULT GetPropertyStorageFromIDList(_In_ PCUIDLIST_RELATIVE pidl,
                                              _In_ REFIID riid,
                                              _Outptr_ void **ppv) {
    HRESULT hr = E_INVALIDARG;
    if (pidl && ppv) {
  ...
  // package up the user supplied data into an ItemID.  pinner points to the
  // clients structure that should be copied, and the IPropertyStore is
  // serialized into the ItemID.   If the client has called
  // IDelegateFolder::SetItemAlloc then we will allocate with that allocator,
  // otherwise the COM allocator is used.  the size of the user supplied data
  // must equal sizeof(T). don't use structs with variably allocated
  // array/string members. the struct must also follow standard SHITEMID for
  // persistance and portability. see shtypes.idl for details.
  HRESULT CreateItemID(_In_opt_ const UNALIGNED T *pinner,
                       _Inout_opt_ IPropertyStore *pps,
                       _Outptr_ PITEMID_CHILD *ppidl) {
    return s_CreateItemID(pinner, pps, ppidl, _pmalloc);
  }

  static HRESULT s_CreateItemID(_In_opt_ const UNALIGNED T *pinner,
                                _Inout_opt_ IPropertyStore *pps,
                                _Outptr_ PITEMID_CHILD *ppidl,
                                _In_opt_ IMalloc *pMalloc = NULL) {
    *ppidl = NULL;
    ...
    // either we succeeded at getting the serialized data, or an IPropertyStore
    // wasn't passed to us.

    if (SUCCEEDED(hr)) {
      UINT cbInner =
          sizeof(CHILDITEMID) - (sizeof(DELEGATEITEMID) - 1) + cbPropStore;
      CHILDITEMID *pitem = s_Alloc(cbInner + sizeof(USHORT), pMalloc);
      if (pitem) {
        pitem->dwMagic = Magic;
        pitem->cbPropStore = (WORD)cbPropStore; // size of the property store
        pitem->cbInnerData =
            sizeof(pitem->innerData); // size of the "innerData" structure
...
private:
  // ItemID form is derived from the delegate folder verison, this allows us to
  // easily switch allocators without having to recompute the ItemID structure
  // dynamically.
...
#include <pshpack1.h>
  ...
  {              // {
    ...
    WORD wOuter;  //     WORD wOuter;         // Private data owned by the outer
                  //     folder
...
#include <poppack.h>
  ...
  static CHILDITEMID *s_Alloc(SIZE_T cbInner, _In_opt_ IMalloc *pMalloc) {
    ...
    if (pMalloc) {
      ...
      SIZE_T cbAlloc = sizeof(DELEGATEITEMID) - sizeof(pidl->rgb[0]) + // header
                       cbInner +                                       // inner
  ...
  static PCITEM _IsValid(_In_opt_ PCUIDLIST_RELATIVE pidl) {
    ...
    if ((pitem->dwMagic != Magic) ||
        (pitem->cbSize < (sizeof(CHILDITEMID) + pitem->cbPropStore)))
      return NULL;
