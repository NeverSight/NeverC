...
NET_API_STATUS NET_API_FUNCTION NetServiceControl(_In_opt_ LPCWSTR servername,
...
NET_API_STATUS NET_API_FUNCTION NetServiceEnum(
    _In_opt_ LPCWSTR servername, _In_ DWORD level,
...
NET_API_STATUS NET_API_FUNCTION NetServiceGetInfo(_In_opt_ LPCWSTR servername,
                                                  _In_ LPCWSTR service,
                                                  _In_ DWORD level,
                                                  _Outptr_ LPBYTE *bufptr);

NET_API_STATUS NET_API_FUNCTION NetServiceInstall(
    _In_opt_ LPCWSTR servername, _In_ LPCWSTR service, _In_ DWORD argc,
    _In_reads_(argc) LPCWSTR argv[], _Outptr_ LPBYTE *bufptr);
 ...
 * domain. However, the computer %1 is currently claiming to be the primary
...
#define SERVICE_CCP_CODE(tt, nn)                                               \
  ((long)SERVICE_CCP_QUERY_HINT |                                              \
   (long)(nn | (tt << SERVICE_IP_WAITTIME_SHIFT)))
...
#define SERVICE_NT_CCP_CODE(tt, nn)                                            \
  (((long)SERVICE_CCP_QUERY_HINT) | ((long)(nn)) |                             \
