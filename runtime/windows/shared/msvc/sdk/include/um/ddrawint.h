...
DEFINE_GUID(GUID_VideoPortCaps, 0xefd60cc3, 0x49e7, 0x11d0, 0x88, 0x9d, 0x0,
            0xaa, 0x0, 0xbb, 0xb7, 0x6a);
DEFINE_GUID(GUID_D3DCaps, 0x7bf06991, 0x8794, 0x11d0, 0x91, 0x39, 0x08, 0x00,
            0x36, 0xd2, 0xef, 0x02);
DEFINE_GUID(GUID_D3DExtendedCaps, 0x7de41f80, 0x9d93, 0x11d0, 0x89, 0xab, 0x00,
            0xa0, 0xc9, 0x05, 0x41, 0x29);
DEFINE_GUID(GUID_D3DCallbacks, 0x7bf06990, 0x8794, 0x11d0, 0x91, 0x39, 0x08,
            0x00, 0x36, 0xd2, 0xef, 0x02);
DEFINE_GUID(GUID_D3DCallbacks2, 0xba584e1, 0x70b6, 0x11d0, 0x88, 0x9d, 0x0,
            0xaa, 0x0, 0xbb, 0xb7, 0x6a);
...
DEFINE_GUID(GUID_DDMoreCaps, 0x880baf30, 0xb030, 0x11d0, 0x8e, 0xa7, 0x00, 0x60,
            0x97, 0x97, 0xea, 0x5b);
...
DEFINE_GUID(GUID_GetHeapAlignment, 0x42e02f16, 0x7b41, 0x11d2, 0x8b, 0xff, 0x0,
            0xa0, 0xc9, 0x83, 0xea, 0xf6);
DEFINE_GUID(GUID_UpdateNonLocalHeap, 0x42e02f17, 0x7b41, 0x11d2, 0x8b, 0xff,
            0x0, 0xa0, 0xc9, 0x83, 0xea, 0xf6);
DEFINE_GUID(GUID_NTPrivateDriverCaps, 0xfad16a23, 0x7b66, 0x11d2, 0x83, 0xd7,
            0x0, 0xc0, 0x4f, 0x7c, 0xe5, 0x8c);
DEFINE_GUID(GUID_DDStereoMode, 0xf828169c, 0xa8e8, 0x11d2, 0xa1, 0xf2, 0x0,
            0xa0, 0xc9, 0x83, 0xea, 0xf6);
DEFINE_GUID(GUID_VPE2Callbacks, 0x52882147, 0x2d47, 0x469a, 0xa0, 0xd1, 0x3,
            0x45, 0x58, 0x90, 0xf6, 0xc8);
...
typedef DWORD(APIENTRY *PDD_SURFCB_SETOVERLAYPOSITION)(
...
typedef DWORD(APIENTRY *PDD_SURFCB_ADDATTACHEDSURFACE)(
...
typedef DWORD(APIENTRY *PDD_VPORTCB_GETAUTOFLIPSURF)(
    PDD_GETVPORTAUTOFLIPSURFACEDATA);
...
typedef DWORD(APIENTRY *PDD_MOCOMPCB_GETCOMPBUFFINFO)(
    PDD_GETMOCOMPCOMPBUFFDATA);
typedef DWORD(APIENTRY *PDD_MOCOMPCB_GETINTERNALINFO)(
...
typedef struct _DD_SURFACE_GLOBAL {
  ...
  union {
    ...
    DWORD dwUserMemSize; // user-mode memory size that display driver requested
                         // (return)
  };
...
typedef struct _DD_SURFACE_MORE {
  ...
  PDD_VIDEOPORT_LOCAL
  lpVideoPort;           // video port currently writing data to this surface
...
typedef struct _DD_MORECAPS {
  ...
  DWORD
  dwAlphaCaps; // driver-specific alpha caps for overlays & Vmem->Vmem blts
  DWORD dwSVBAlphaCaps; // driver-specific alpha capabilities for System->Vmem
                        // blts
  DWORD dwVSBAlphaCaps; // driver-specific alpha capabilities for Vmem->System
                        // blts
  DWORD dwSSBAlphaCaps; // driver-specific alpha capabilities for System->System
                        // blts
  DWORD dwFilterCaps;   // driver-specific filter caps for overlays & Vmem->Vmem
                        // blts
  DWORD dwSVBFilterCaps; // driver-specific filter capabilities for System->Vmem
                         // blts
  DWORD dwVSBFilterCaps; // driver-specific filter capabilities for Vmem->System
                         // blts
  DWORD dwSSBFilterCaps; // driver-specific filter capabilities for
                         // System->System blts
} DD_MORECAPS;
...
typedef struct _DD_DIRECTDRAW_GLOBAL {
  ...
  LPDDVIDEOPORTCAPS lpDDVideoPortCaps; // Info returned by the HAL (an array if
                                       // more than one video port)
 ...
 * Drivers which have a unified Blt/AlphaBlt DDI can use this flag to
 * distinguish between the two API calls.
 */
...
typedef struct _DD_UPDATEVPORTDATA {
  ...
  DWORD
  dwNumAutoflip; // # of autoflip surfaces. If > 1, lpDDSurface is an array.
  DWORD dwNumVBIAutoflip; // # of autoflip surfaces. If > 1, lpDDVBISurface is
                          // an array.
  HRESULT ddRVal;         // return value
...
typedef struct _DD_GETMOCOMPCOMPBUFFDATA {
  ...
  DWORD dwNumTypesCompBuffs;   // [in/out] number of memory types required for
                               // comp buffers
  LPDDCOMPBUFFERINFO lpCompBuffInfo; // [in]   driver supplied info regarding
...
typedef struct _DD_RENDERMOCOMPDATA {
  ...
  lpBufferInfo;     // [in]  Surfaces containing macro block info
