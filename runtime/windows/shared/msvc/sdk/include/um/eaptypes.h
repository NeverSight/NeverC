...
// EAPHost populates this array with the information for all the eap-methods
// installed on machine and passes it to a supplicant
...
// EAPHost populates this error structs with appropriate information and passes
// it to supplicant, which can use this information for error reporting and
// troubleshooting
...
// Show the Credentails UI before authenticating even if cached creds are
// present
...
// credentials, even if config says use certain way of cred retrieval. If this
// fails, fallback to Method specific credentail retrieval
...
#define EAP_UI_INPUT_FIELD_PROPS_NON_DISPLAYABLE                               \
...
#ifdef __midl
typedef[switch_type(EAP_INTERACTIVE_UI_DATA_TYPE)] union {
  [case (EapCredReq, EapCredResp)] EAP_CRED_REQ *credData;
  [case (EapCredExpiryReq,
         EapCredExpiryResp)] EAP_CRED_EXPIRY_REQ *credExpiryData;
...
#ifdef __midl
typedef[switch_type(EAP_METHOD_PROPERTY_VALUE_TYPE)] union {
  [case (empvtBool)] EAP_METHOD_PROPERTY_VALUE_BOOL
      empvBool; // value is boolean type

  [case (empvtDword)] EAP_METHOD_PROPERTY_VALUE_DWORD
      empvDword; // value is dword type

  [case (empvtString)] EAP_METHOD_PROPERTY_VALUE_STRING
      empvString; // value is string type
...
#ifdef __midl
  [switch_is(eapMethodPropertyValueType)] EAP_METHOD_PROPERTY_VALUE
...
#ifdef __midl
typedef[switch_type(EapCredentialType)] union {
  [case (EAP_USERNAME_PASSWORD_CREDENTIAL)] EapUsernamePasswordCredential
      username_password;
