...
typedef enum _WEB_SOCKET_ACTION_QUEUE {
  ...
  WEB_SOCKET_ALL_ACTION_QUEUE =
      WEB_SOCKET_SEND_ACTION_QUEUE | WEB_SOCKET_RECEIVE_ACTION_QUEUE
...
HRESULT WINAPI WebSocketCreateClientHandle(
    ...
    _In_ ULONG ulPropertyCount, _Out_ WEB_SOCKET_HANDLE *phWebSocket);
...
HRESULT WINAPI WebSocketBeginServerHandshake(
    _In_ WEB_SOCKET_HANDLE hWebSocket, _In_opt_ PCSTR pszSubprotocolSelected,
    _In_reads_opt_(ulExtensionSelectedCount) PCSTR *pszExtensionSelected,
    _In_ ULONG ulExtensionSelectedCount,
    _In_reads_(ulRequestHeaderCount)
        const PWEB_SOCKET_HTTP_HEADER pRequestHeaders,
    _In_ ULONG ulRequestHeaderCount,
    _Out_writes_(*pulResponseHeaderCount)
        PWEB_SOCKET_HTTP_HEADER *pResponseHeaders,
    _Out_ ULONG *pulResponseHeaderCount);

HRESULT WINAPI WebSocketEndServerHandshake(_In_ WEB_SOCKET_HANDLE hWebSocket);

HRESULT WINAPI WebSocketSend(_In_ WEB_SOCKET_HANDLE hWebSocket,
                             _In_ WEB_SOCKET_BUFFER_TYPE BufferType,
                             _In_opt_ WEB_SOCKET_BUFFER *pBuffer,
                             _In_opt_ PVOID Context);

HRESULT WINAPI WebSocketReceive(_In_ WEB_SOCKET_HANDLE hWebSocket,
                                _In_opt_ WEB_SOCKET_BUFFER *pBuffer,
...
HRESULT WINAPI WebSocketGetAction(
    ...
    _Inout_updates_to_(*pulDataBufferCount, *pulDataBufferCount)
        WEB_SOCKET_BUFFER *pDataBuffers,
    _Inout_ ULONG *pulDataBufferCount, _Out_ WEB_SOCKET_ACTION *pAction,
    _Out_ WEB_SOCKET_BUFFER_TYPE *pBufferType,
    _Out_opt_ PVOID *pvApplicationContext, _Out_ PVOID *pvActionContext);

VOID WINAPI WebSocketCompleteAction(_In_ WEB_SOCKET_HANDLE hWebSocket,
                                    _In_ PVOID pvActionContext,
                                    _In_ ULONG ulBytesTransferred);

VOID WINAPI WebSocketAbortHandle(_In_ WEB_SOCKET_HANDLE hWebSocket);

VOID WINAPI WebSocketDeleteHandle(_In_ WEB_SOCKET_HANDLE hWebSocket);

HRESULT WINAPI WebSocketGetGlobalProperty(_In_ WEB_SOCKET_PROPERTY_TYPE eType,
                                          _Inout_updates_to_(*ulSize, *ulSize)
                                              PVOID pvValue,
