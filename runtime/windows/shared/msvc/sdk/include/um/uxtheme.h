...
//---------------------------------------------------------------------------
THEMEAPI_(BOOL)
BeginPanningFeedback(_In_ HWND hwnd);
//---------------------------------------------------------------------------
...
// feedback
//                             For the method to succeed this must be the same
//                             hwnd as provided in BeginPanningFeedback
//
// LONG lTotalOverpanOffsetX - The Total displacement that the window has moved
// in the horizontal direction
//                             since the end of scrollable region was reached.
//                             The API would move the window by the distance
//                             specified A maximum displacement of 30 pixels is
//                             allowed
//
...
//                             by the distance specified A maximum displacement
//                             of 30 pixels is allowed
//
// BOOL fInInertia           - Flag dictating whether the Application is
// handling a WM_GESTURE message with the
//                             GF_INERTIA FLAG set
//
//   Incremental calls to UpdatePanningFeedback should make sure they always
//   pass the sum of the increments and not just the increment themselves Eg :
//   If the initial displacement is 10 pixels and the next displacement 10
//   pixels
...
//
THEMEAPI_(BOOL)
UpdatePanningFeedback(_In_ HWND hwnd, _In_ LONG lTotalOverpanOffsetX,
                      _In_ LONG lTotalOverpanOffsetY, _In_ BOOL fInInertia);
...
//   BeginPanningFeedback and UpdatePanningFeedback The EndPanningFeedBack needs
//   to be called Prior to calling any BeginPanningFeedBack if we have already
//   called a BeginPanningFeedBack followed by one/ more UpdatePanningFeedback
//   calls
//
//  HWND hwnd         - The handle to the Target window that will receive
//  feedback
//
//  BOOL fAnimateBack - Flag to indicate whether you wish the displaced window
//  to move back
//                      to the original position via animation or a direct jump.
//                      Either ways the method will try to restore the moved
//                      window. The latter case exists for compatibility with
//                      legacy apps.
//
THEMEAPI_(BOOL)
EndPanningFeedback(_In_ HWND hwnd, _In_ BOOL fAnimateBack);
#endif
...
//
//                                Storyboard id and target id is defined in
//                                vsanimation.h
//
...
//
//  eProperty                   - Property associated with the animation
//  storyboard and target
//
//  pvProperty                  - Buffer to receive returned property value
//
//  cbSize                      - Byte size of a buffer pointed by pvProperty
//
//  pcbSizeOut                  - pointer to a DWORD buffer to receive the
//  returned
//                                byte size
//
THEMEAPI
GetThemeAnimationProperty(_In_ HTHEME hTheme, _In_ int iStoryboardId,
                          _In_ int iTargetId, _In_ TA_PROPERTY eProperty,
...
//                                    based on storyboard id, target id and
//                                    transform index
//
//                                    Transform index is in the range of (0,
//                                    dwTransformCount - 1). dwTransformCount
//                                    can be retrieved by calling
//                                    GetThemeAnimationProperty with
//                                    TAP_TRANSFORMCOUNT
//
...
//
//  pTransform                      - Pointer buffer to receive a transform
//  structure
//
//  pcbSizeOut                      - pointer to a DWORD buffer to receive
//  returned
//                                    byte-size of transform operation struct
//
THEMEAPI
GetThemeAnimationTransform(_In_ HTHEME hTheme, _In_ int iStoryboardId,
                           _In_ int iTargetId, _In_ DWORD dwTransformIndex,
                           _Out_writes_bytes_to_opt_(cbSize, *pcbSizeOut)
...
//---------------------------------------------------------------------------
//  GetThemeTimingFunction          - Retrieve a predefined timing function
//  based on
...
//
//  ppThemeTimingFunction           - Buffer to receive a pre-defined timing
//  function pointer
//
//  pcbSizeOut                      - pointer to a DWORD buffer to receive
//  byte-size of
...
//
THEMEAPI GetThemeTimingFunction(_In_ HTHEME hTheme, _In_ int iTimingFunctionId,
                                _Out_writes_bytes_to_opt_(cbSize, *pcbSizeOut)
...
//---------------------------------------------------------------------------
THEMEAPI_(HTHEME)
OpenThemeData(_In_opt_ HWND hwnd, _In_ LPCWSTR pszClassList);
...
THEMEAPI_(HTHEME)
OpenThemeDataForDpi(_In_opt_ HWND hwnd, _In_ LPCWSTR pszClassList,
                    _In_ UINT dpi);
...
//---------------------------------------------------------------------------
THEMEAPI_(HTHEME)
OpenThemeDataEx(_In_opt_ HWND hwnd, _In_ LPCWSTR pszClassList,
                _In_ DWORD dwFlags);
...
//---------------------------------------------------------------------------
THEMEAPI
CloseThemeData(_In_ HTHEME hTheme);
...
//------------------------------------------------------------------------
THEMEAPI
DrawThemeBackground(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
...
#define DTBG_DRAWSOLID                                                         \
  0x00000002 // DEPRECATED: draw transparent/alpha images as solid
...
#define DTBG_NOMIRROR                                                          \
  0x00000040 // don't mirror the output, overrides everything else
...
//------------------------------------------------------------------------
THEMEAPI
DrawThemeBackgroundEx(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
                      _In_ int iStateId, _In_ LPCRECT pRect,
...
//-------------------------------------------------------------------------
THEMEAPI
DrawThemeText(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
              _In_ int iStateId, _In_reads_(cchText) LPCWSTR pszText,
              _In_ int cchText, _In_ DWORD dwTextFlags, _In_ DWORD dwTextFlags2,
              _In_ LPCRECT pRect);
...
//-------------------------------------------------------------------------
THEMEAPI
GetThemeBackgroundContentRect(_In_ HTHEME hTheme, _In_opt_ HDC hdc,
...
//-------------------------------------------------------------------------
THEMEAPI
GetThemeBackgroundExtent(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
...
//-------------------------------------------------------------------------
THEMEAPI
GetThemeBackgroundRegion(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
                         _In_ int iStateId, _In_ LPCRECT pRect,
                         _Out_ HRGN *pRegion);
...
//-------------------------------------------------------------------------
THEMEAPI
GetThemePartSize(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
                 _In_ int iStateId, _In_opt_ LPCRECT prc,
...
//-------------------------------------------------------------------------
THEMEAPI
GetThemeTextExtent(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
                   _In_ int iStateId, _In_reads_(cchCharCount) LPCWSTR pszText,
...
//-------------------------------------------------------------------------
THEMEAPI
GetThemeTextMetrics(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
                    _In_ int iStateId, _Out_ TEXTMETRICW *ptm);

//-------------------------------------------------------------------------
//----- HitTestThemeBackground, HitTestThemeBackgroundRegion flags ----

//  Theme background segment hit test flag (default). possible return values
//  are: HTCLIENT: hit test succeeded in the middle background segment HTTOP,
//  HTLEFT, HTTOPLEFT, etc:  // hit test succeeded in the the respective theme
//  background segment.
...
#define HTTB_FIXEDBORDER                                                       \
  0x00000002 // Return code may be either HTCLIENT or HTBORDER.
//  Caption hit test option.  Possible return values are:
//  HTCAPTION: hit test succeeded in the top, top left, or top right background
//  segments HTNOWHERE or another return code, depending on absence or presence
//  of accompanying flags, resp.
...
//  HTTOP, HTTOPLEFT, HTLEFT, HTRIGHT, etc:    hit test succeeded in the
//  respective system resizing zone HTBORDER: hit test failed in middle segment
//  and resizing zones, but succeeded in a background border segment
...
// This option is mutually exclusive with HTTB_SYSTEMSIZINGWIDTH;
// HTTB_SIZINGTEMPLATE takes precedence
#define HTTB_SIZINGTEMPLATE 0x00000100
// Use system resizing border width rather than theme content margins.
// This option is mutually exclusive with HTTB_SIZINGTEMPLATE, which takes
// precedence.
...
//-------------------------------------------------------------------------
THEMEAPI
HitTestThemeBackground(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
                       _In_ int iStateId, _In_ DWORD dwOptions,
                       _In_ LPCRECT pRect, _In_opt_ HRGN hrgn,
                       _In_ POINT ptTest, _Out_ WORD *pwHitTestCode);
...
//------------------------------------------------------------------------
THEMEAPI
DrawThemeEdge(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
              _In_ int iStateId, _In_ LPCRECT pDestRect, _In_ UINT uEdge,
...
//------------------------------------------------------------------------
THEMEAPI
DrawThemeIcon(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
              _In_ int iStateId, _In_ LPCRECT pRect, _In_ HIMAGELIST himl,
...
//---------------------------------------------------------------------------
THEMEAPI_(BOOL)
IsThemePartDefined(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId);
...
//---------------------------------------------------------------------------
THEMEAPI_(BOOL)
IsThemeBackgroundPartiallyTransparent(_In_ HTHEME hTheme, _In_ int iPartId,
                                      _In_ int iStateId);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeColor(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
              _In_ int iPropId, _Out_ COLORREF *pColor);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeMetric(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeString(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
               _In_ int iPropId, _Out_writes_(cchMaxBuffChars) LPWSTR pszBuff,
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeBool(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
             _In_ int iPropId, _Out_ BOOL *pfVal);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeInt(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeEnumValue(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
                  _In_ int iPropId, _Out_ int *piVal);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemePosition(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
                 _In_ int iPropId, _Out_ POINT *pPoint);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeFont(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
             _In_ int iStateId, _In_ int iPropId, _Out_ LOGFONTW *pFont);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeRect(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
             _In_ int iPropId, _Out_ LPRECT pRect);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeMargins(_In_ HTHEME hTheme, _In_opt_ HDC hdc, _In_ int iPartId,
                _In_ int iStateId, _In_ int iPropId, _In_opt_ LPCRECT prc,
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeIntList(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
                _In_ int iPropId, _Out_ INTLIST *pIntList);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemePropertyOrigin(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
                       _In_ int iPropId, _Out_ enum PROPERTYORIGIN *pOrigin);
...
//---------------------------------------------------------------------------
THEMEAPI
SetWindowTheme(_In_ HWND hwnd, _In_opt_ LPCWSTR pszSubAppName,
...
//---------------------------------------------------------------------------
THEMEAPI
GetThemeFilename(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
                 _In_ int iPropId,
...
//---------------------------------------------------------------------------
THEMEAPI_(COLORREF)
GetThemeSysColor(_In_opt_ HTHEME hTheme, _In_ int iColorId);
...
//---------------------------------------------------------------------------
THEMEAPI_(HBRUSH)
GetThemeSysColorBrush(_In_opt_ HTHEME hTheme, _In_ int iColorId);
...
//---------------------------------------------------------------------------
THEMEAPI_(BOOL)
GetThemeSysBool(_In_opt_ HTHEME hTheme, _In_ int iBoolId);
...
//---------------------------------------------------------------------------
THEMEAPI_(int)
GetThemeSysSize(_In_opt_ HTHEME hTheme, _In_ int iSizeId);
...
//---------------------------------------------------------------------------
THEMEAPI
GetThemeSysFont(_In_opt_ HTHEME hTheme, _In_ int iFontId, _Out_ LOGFONTW *plf);
...
//---------------------------------------------------------------------------
THEMEAPI
GetThemeSysString(_In_ HTHEME hTheme, _In_ int iStringId,
...
//---------------------------------------------------------------------------
THEMEAPI
GetThemeSysInt(_In_ HTHEME hTheme, _In_ int iIntId, _Out_ int *piValue);
...
//---------------------------------------------------------------------------
THEMEAPI
EnableThemeDialogTexture(_In_ HWND hwnd, _In_ DWORD dwFlags);
...
//---------------------------------------------------------------------------
THEMEAPI_(BOOL)
IsThemeDialogTextureEnabled(_In_ HWND hwnd);
...
//---------------------------------------------------------------------------
THEMEAPI_(void)
SetThemeAppProperties(_In_ DWORD dwFlags);
...
//---------------------------------------------------------------------------
THEMEAPI
GetCurrentThemeName(_Out_writes_(cchMaxNameChars) LPWSTR pszThemeFileName,
...
THEMEAPI
GetThemeDocumentationProperty(_In_ LPCWSTR pszThemeName,
...
//---------------------------------------------------------------------------
THEMEAPI
DrawThemeParentBackground(_In_ HWND hwnd, _In_ HDC hdc,
...
//---------------------------------------------------------------------------
THEMEAPI
EnableTheming(_In_ BOOL fEnable);
...
//                      S_OK
//                        without sending a WM_CTLCOLORSTATIC message if the
//                        parent actually painted on WM_ERASEBKGND.
...
//---------------------------------------------------------------------------
THEMEAPI
DrawThemeParentBackgroundEx(_In_ HWND hwnd, _In_ HDC hdc, _In_ DWORD dwFlags,
                            _In_opt_ const RECT *prc);
...
typedef struct _WTA_OPTIONS {
  ...
  DWORD dwMask;  // bitmask for flags that are changing
                 // valid options are: WTNCA_NODRAWCAPTION, WTNCA_NODRAWICON,
                 // WTNCA_NOSYSMENU
...
#define WTNCA_NOSYSMENU                                                        \
  0x00000004 // don't expose the system menu icon functionality
#define WTNCA_NOMIRRORHELP                                                     \
  0x00000008 // don't mirror the question mark, even in RTL layout
#define WTNCA_VALIDBITS                                                        \
  (WTNCA_NODRAWCAPTION | WTNCA_NODRAWICON | WTNCA_NOSYSMENU |                  \
   WTNCA_NOMIRRORHELP)

THEMEAPI
SetWindowThemeAttribute(_In_ HWND hwnd,
                        _In_ enum WINDOWTHEMEATTRIBUTETYPE eAttribute,
                        _In_reads_bytes_(cbAttribute) PVOID pvAttribute,
                        _In_ DWORD cbAttribute);

__inline HRESULT SetWindowThemeNonClientAttributes(HWND hwnd, DWORD dwMask,
                                                   DWORD dwAttributes) {
  WTA_OPTIONS wta;
  wta.dwFlags = dwAttributes;
  wta.dwMask = dwMask;
  return SetWindowThemeAttribute(hwnd, WTA_NONCLIENT, (void *)&(wta),
                                 sizeof(wta));
}
...
typedef int(WINAPI *DTT_CALLBACK_PROC)(_In_ HDC hdc,
                                       _Inout_updates_(cchText) LPWSTR pszText,
                                       _In_ int cchText, _Inout_ LPRECT prc,
                                       _In_ UINT dwFlags, _In_ LPARAM lParam);
...
#define DTT_COMPOSITED                                                         \
  (1UL << 13) // Draws text with antialiased alpha (needs a DIB section)
...
typedef struct _DTTOPTS {
  ...
  int iColorPropId;     // Color property to use for the text instead of
                        // TMT_TEXTCOLOR
...
THEMEAPI
DrawThemeTextEx(_In_ HTHEME hTheme, _In_ HDC hdc, _In_ int iPartId,
                _In_ int iStateId, _In_reads_(cchText) LPCWSTR pszText,
                _In_ int cchText, _In_ DWORD dwTextFlags, _Inout_ LPRECT pRect,
...
//      to be freed) pcbStream   - if non-null receives the size of the STREAM
//      property hInst       - NULL when iPropId==TMT_STREAM, HINSTANCE of a
//      loaded msstyles
//                    file when iPropId==TMT_DISKSTREAM (use GetCurrentThemeName
//                    and LoadLibraryEx(LOAD_LIBRARY_AS_DATAFILE)
//-----------------------------------------------------------------------
THEMEAPI
GetThemeBitmap(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
               _In_ int iPropId, _In_ ULONG dwFlags, _Out_ HBITMAP *phBitmap);
...
//-----------------------------------------------------------------------
THEMEAPI
GetThemeStream(_In_ HTHEME hTheme, _In_ int iPartId, _In_ int iStateId,
               _In_ int iPropId, _Out_ VOID **ppvStream,
               _Out_opt_ DWORD *pcbStream, _In_opt_ HINSTANCE hInst);
...
//                          BufferedPaintInit, when calls to BeginBufferedPaint
...
//    rcTarget           - Rectangle specifying the area of the target DC to
//    paint to dwFormat           - Format of the buffer (see BP_BUFFERFORMAT)
//    pPaintParams       - Paint operation parameters (see BP_PAINTPARAMS)
//    phBufferedPaint    - Pointer to receive handle to new buffered paint
//    context
//------------------------------------------------------------------------
...
#define BPPF_NOCLIP                                                            \
  0x0002 // Don't apply the target DC's clip region to the double buffer
...
BeginBufferedPaint(_In_ HDC hdcTarget, _In_ const RECT *prcTarget,
                   _In_ BP_BUFFERFORMAT dwFormat,
                   _In_opt_ BP_PAINTPARAMS *pPaintParams, _Out_ HDC *phdc);
...
//------------------------------------------------------------------------
THEMEAPI
EndBufferedPaint(_In_ HPAINTBUFFER hBufferedPaint, _In_ BOOL fUpdateTarget);
...
//------------------------------------------------------------------------
THEMEAPI
GetBufferedPaintTargetRect(_In_ HPAINTBUFFER hBufferedPaint, _Out_ RECT *prc);
...
//------------------------------------------------------------------------
THEMEAPI_(HDC)
GetBufferedPaintTargetDC(_In_ HPAINTBUFFER hBufferedPaint);
...
//------------------------------------------------------------------------
THEMEAPI_(HDC)
GetBufferedPaintDC(_In_ HPAINTBUFFER hBufferedPaint);
...
//------------------------------------------------------------------------
THEMEAPI
GetBufferedPaintBits(_In_ HPAINTBUFFER hBufferedPaint,
                     _Out_ RGBQUAD **ppbBuffer, _Out_ int *pcxRow);
...
//------------------------------------------------------------------------
THEMEAPI
BufferedPaintClear(_In_ HPAINTBUFFER hBufferedPaint, _In_opt_ const RECT *prc);

//------------------------------------------------------------------------
//  BufferedPaintSetAlpha() - Set alpha to given value in given rectangle
//
//    hBufferedPaint        - handle to buffered paint context
//    prc                   - rectangle to set alpha in; NULL specifies entire
//    buffer alpha                 - alpha value to set in the given rectangle
//------------------------------------------------------------------------
THEMEAPI
BufferedPaintSetAlpha(_In_ HPAINTBUFFER hBufferedPaint,
                      _In_opt_ const RECT *prc, _In_ BYTE alpha);
...
//------------------------------------------------------------------------
THEMEAPI
BufferedPaintStopAllAnimations(_In_ HWND hwnd);

typedef HANDLE HANIMATIONBUFFER; // handle to a buffered paint animation

THEMEAPI_(HANIMATIONBUFFER)
BeginBufferedAnimation(_In_ HWND hwnd, _In_ HDC hdcTarget,
                       _In_ const RECT *prcTarget,
                       _In_ BP_BUFFERFORMAT dwFormat,
                       _In_opt_ BP_PAINTPARAMS *pPaintParams,
                       _In_ BP_ANIMATIONPARAMS *pAnimationParams,
                       _Out_ HDC *phdcFrom, _Out_ HDC *phdcTo);

THEMEAPI
EndBufferedAnimation(_In_ HANIMATIONBUFFER hbpAnimation,
                     _In_ BOOL fUpdateTarget);

THEMEAPI_(BOOL)
BufferedPaintRenderAnimation(_In_ HWND hwnd, _In_ HDC hdcTarget);
...
//------------------------------------------------------------------------
THEMEAPI
GetThemeTransitionDuration(_In_ HTHEME hTheme, _In_ int iPartId,
                           _In_ int iStateIdFrom, _In_ int iStateIdTo,
                           _In_ int iPropId, _Out_ DWORD *pdwDuration);
