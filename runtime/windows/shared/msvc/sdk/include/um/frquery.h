...
public:
  ...
  // Select * from win32_directory where drive = "C:" GetValuesForProp(L"Drive")
  // -> C: Where Drive = "C:" or Drive = "D:" GetValuesForProp(L"Drive") -> C:,
  // D: Where Path = "\DOS" GetValuesForProp(L"Drive") -> (empty) Where Drive <>
  // "C:" GetValuesForProp(L"Drive") -> (empty) Where Drive = "C:" or (Drive =
  // "D:" and Mounted = true) GetValuesForProp(L"Drive") -> C:, D:
  HRESULT GetValuesForProp(LPCWSTR wszPropName, CHStringArray &achNames);

  // Here's an overloaded version in case client wants to pass in a vector of
  // _bstr_t's
  ...
  // all the the properties from the Where clauses.  If the returned array is
  // empty, all properties are required.
  ...
  bool AllPropertiesAreRequired(void) {
    return (m_csaPropertiesRequired.GetSize() == 0);
  }
  ...
  // Moves the values into the member variables.  Should never be called by
  // users.
  HRESULT Init(

      const BSTR bstrQueryFormat, const BSTR bstrQuery, long lFlags,
      CHString &sNamespace);
  ...
  HRESULT Init(
      ...
      ParsedObjectPath *pParsedObjectPath, IWbemContext *pCtx,
      LPCWSTR lpwszClassName, CHString &sNamespace);
