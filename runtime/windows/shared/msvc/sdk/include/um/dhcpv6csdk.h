...
typedef enum {
  ...
  STATUS_UNSPECIFIED_FAILURE, // Some unspecified failure occurred  while trying
                              // to process the prefix.
  STATUS_NO_BINDING = 3,      // The server doesn't have binding for the prefix.
  STATUS_NOPREFIX_AVAIL =
      6 // The server doesn't have prefix to offer to the requesting client.
} StatusCode;

typedef struct _DHCPV6Prefix {
  UCHAR prefix[16];        // 128 bit prefix
  DWORD prefixLength;      //(48-64 bits)
  DWORD preferredLifeTime; // The Preferred Lifetime of the Prefix returned or
                           // requested in seconds
  DWORD validLifeTime; // The Valid Lifetime of the Prefix returned or requested
                       // in seconds
...
typedef struct _DHCPV6PrefixLeaseInformation {
  ...
  DWORD
  iaid; // The 32 bit Identity Association identifier for the prefix option.
  ...
  time_t MaxLeaseExpirationTime; // The absolute maximum lease expiration time
                                 // of all the prefix leases in this structure.
  time_t LastRenewalTime; // The absolute time at which the last renewal for the
                          // prefixes happened.
  StatusCode status;      // The status code returned by the server for the IAPD
  _Field_size_bytes_(ServerIdLen)
      LPBYTE ServerId; // The server DUID from which the prefix is received.
                       // This is used in subsequent Renews.
  DWORD ServerIdLen;   // The length of the above DUID data.
} DHCPV6PrefixLeaseInformation, *PDHCPV6PrefixLeaseInformation,
    *LPDHCPV6PrefixLeaseInformation;
...
VOID APIENTRY Dhcpv6CApiInitialize(OUT LPDWORD Version);

VOID APIENTRY Dhcpv6CApiCleanup(VOID);

DWORD                             // win32 status
    APIENTRY Dhcpv6RequestParams( // request parameters of client
        IN BOOL forceNewInform, IN LPVOID reserved, _In_ IN LPWSTR adapterName,
        IN LPDHCPV6CAPI_CLASSID classId,
        IN OUT DHCPV6CAPI_PARAMS_ARRAY recdParams, IN LPBYTE buffer,
...
DWORD
APIENTRY
Dhcpv6RequestPrefix(IN _In_ LPWSTR adapterName,
...
DWORD
APIENTRY
Dhcpv6RenewPrefix(IN _In_ LPWSTR adapterName, IN LPDHCPV6CAPI_CLASSID pclassId,
                  IN OUT LPDHCPV6PrefixLeaseInformation prefixleaseInfo,
                  _Out_ DWORD *pdwTimeToWait, IN DWORD bValidatePrefix);

DWORD
APIENTRY
Dhcpv6ReleasePrefix(_In_ IN LPWSTR adapterName, IN LPDHCPV6CAPI_CLASSID classId,
