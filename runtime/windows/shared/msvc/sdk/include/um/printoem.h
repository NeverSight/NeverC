...
NOTE:
    ...
    This file contains hsplit tags that publish structures and constants to
public\internal headers. So please take care before making any changes to this
file. Please build usermode\test\PublishingTests before checking in any changes
to this file.
...
//
// PScript5's private DEVMODE structure to allow its plugins to determine its
// size.
//
...
#define GET_PSCRIPT5_PRIVATE_DEVMODE_SIZE(pdm)                                 \
  (((pdm)->dmDriverExtra >                                                     \
    (FIELD_OFFSET(PSCRIPT5_PRIVATE_DEVMODE, wSize) + sizeof(WORD)))            \
       ? ((PPSCRIPT5_PRIVATE_DEVMODE)((PBYTE)(pdm) + (pdm)->dmSize))->wSize    \
       : 0)

//
// Unidrv's private DEVMODE structure to allow its plugins to determine its
// size.
//
...
#define GET_UNIDRV_PRIVATE_DEVMODE_SIZE(pdm)                                   \
  (((pdm)->dmDriverExtra >                                                     \
    (FIELD_OFFSET(UNIDRV_PRIVATE_DEVMODE, wSize) + sizeof(WORD)))              \
       ? ((PUNIDRV_PRIVATE_DEVMODE)((PBYTE)(pdm) + (pdm)->dmSize))->wSize      \
       : 0)
...
// OEMGetInfo
//
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMGetInfo(DWORD dwMode, _Out_writes_bytes_(cbSize) PVOID pBuffer,
               DWORD cbSize, _Out_ PDWORD pcbNeeded);

__drv_preferredFunction("IPrintOemUni", "Obsolete") BOOL APIENTRY
    OEMDriverDMS(PVOID pDevObj, _Out_writes_bytes_(cbSize) PVOID pBuffer,
                 DWORD cbSize, _Out_ PDWORD pcbNeeded);
...
typedef BOOL(APIENTRY *PFN_DrvGetDriverSetting)(
    PVOID pdriverobj, PCSTR Feature,
    _Out_writes_bytes_opt_(cbSize) PVOID pOutput, DWORD cbSize,
    _Out_opt_ PDWORD pcbNeeded, _Out_opt_ PDWORD pdwOptionsReturned);
...
typedef BOOL(APIENTRY *PFN_DrvUpgradeRegistrySetting)(HANDLE hPrinter,
                                                      PCSTR pFeature,
                                                      PCSTR pOption);
...
typedef BOOL(APIENTRY *PFN_DrvUpdateUISetting)(PVOID pdriverobj, PVOID pOptItem,
...
__drv_preferredFunction("IPrintOemPS or IPrintOemUni", "Obsolete") BOOL APIENTRY
    OEMEnableDriver(DWORD dwOemIntfVersion, DWORD cbSize,
                    _Out_writes_bytes_(cbSize) PDRVENABLEDATA pded);

typedef struct _DEVOBJ *PDEVOBJ;
typedef PVOID PDEVOEM;

typedef DWORD(APIENTRY *PFN_DrvWriteSpoolBuf)(PDEVOBJ pdevobj,
                                              _In_reads_bytes_(cbSize)
                                                  PVOID pBuffer,
                                              DWORD cbSize);

typedef DWORD(APIENTRY *PFN_DrvWriteAbortBuf)(PDEVOBJ pdevobj,
                                              _In_reads_bytes_(cbSize)
                                                  PVOID pBuffer,
                                              DWORD cbSize, DWORD dwWait);

typedef INT(APIENTRY *PFN_DrvXMoveTo)(PDEVOBJ pdevobj, INT x, DWORD dwFlags);

typedef INT(APIENTRY *PFN_DrvYMoveTo)(PDEVOBJ pdevobj, INT y, DWORD dwFlags);

typedef BOOL(APIENTRY *PFN_DrvGetStandardVariable)(
    PDEVOBJ pdevobj, DWORD dwIndex,
    _Out_writes_bytes_opt_(cbSize) PVOID pBuffer, DWORD cbSize,
...
typedef enum _STDVARIABLEINDEX {
  ...
  SVI_CURSORORIGINY,         //  "CursorOriginY"
                     //  this is in MasterUnits and in the coordinates of the
                     //  currently selected orientation. this value is defined
                     //  as ImageableOrigin - CursorOrigin
  SVI_PAGENUMBER, //  "PageNumber"
                  //  this value tracks number of times DrvStartBand has been
                  //  called since StartDoc.

  SVI_MAX //  Just a placeholder do not use.
} STDVARIABLEINDEX;

typedef BOOL(APIENTRY *PFN_DrvUnidriverTextOut)(
    SURFOBJ *pso, STROBJ *pstro, FONTOBJ *pfo, CLIPOBJ *pco, RECTL *prclExtra,
    RECTL *prclOpaque, BRUSHOBJ *pboFore, BRUSHOBJ *pboOpaque,
...
typedef struct _DRVPROCS {
  ...
  PFN_DrvGetDriverSetting
      DrvGetDriverSetting; // common to both pscript and unidrv
...
__drv_preferredFunction("IPrintOemPS or IPrintOemUni", "Obsolete") VOID APIENTRY
    OEMDisableDriver(VOID);

//
// OEMEnablePDEV
//

__drv_preferredFunction("IPrintOemPS or IPrintOemUni",
                        "Obsolete") PDEVOEM APIENTRY
    OEMEnablePDEV(PDEVOBJ pdevobj, _In_ PWSTR pPrinterName, ULONG cPatterns,
...
__drv_preferredFunction("IPrintOemPS or IPrintOemUni", "Obsolete") VOID APIENTRY
    OEMDisablePDEV(PDEVOBJ pdevobj);
...
__drv_preferredFunction("IPrintOemPS or IPrintOemUni", "Obsolete") BOOL APIENTRY
    OEMResetPDEV(PDEVOBJ pdevobjOld, PDEVOBJ pdevobjNew);

//
// OEMCommand - PSCRIPT only
//

__drv_preferredFunction("IPrintOemPS", "Obsolete") DWORD APIENTRY
    OEMCommand(PDEVOBJ pdevobj, DWORD dwIndex,
               _In_reads_bytes_opt_(cbSize) PVOID pData, DWORD cbSize);

__drv_preferredFunction("IPrintOemUni", "Obsolete") INT APIENTRY
    OEMCommandCallback(PDEVOBJ pdevobj, DWORD dwCallbackID, DWORD dwCount,
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") PBYTE APIENTRY
    OEMImageProcessing(PDEVOBJ pdevobj, PBYTE pSrcBitmap,
                       PBITMAPINFOHEADER pBitmapInfoHeader, PBYTE pColorTable,
                       DWORD dwCallbackID, PIPPARAMS pIPParams);
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") BOOL APIENTRY
    OEMFilterGraphics(PDEVOBJ pdevobj, _In_reads_bytes_(dwLen) PBYTE pBuf,
                      DWORD dwLen);
...
//
__drv_preferredFunction("IPrintOemUni", "Obsolete") INT APIENTRY
    OEMCompression(PDEVOBJ pdevobj, _In_reads_bytes_(dwInLen) PBYTE pInBuf,
                   _Out_writes_bytes_(dwOutLen) PBYTE pOutBuf, DWORD dwInLen,
                   DWORD dwOutLen);
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") BOOL APIENTRY
    OEMHalftonePattern(PDEVOBJ pdevobj, PBYTE pHTPattern, DWORD dwHTPatternX,
                       DWORD dwHTPatternY, DWORD dwHTNumPatterns,
                       DWORD dwCallbackID,
...
VOID APIENTRY OEMMemoryUsage(PDEVOBJ pdevobj,
...
//
__drv_preferredFunction("IPrintOemUni", "Obsolete") INT APIENTRY
    OEMTTYGetInfo(PDEVOBJ pdevobj, DWORD dwInfoIndex,
                  _Out_writes_bytes_(dwSize) PVOID pOutputBuf, DWORD dwSize,
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") DWORD APIENTRY
    OEMDownloadFontHeader(PDEVOBJ pdevobj, PUNIFONTOBJ pUFObj);
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") DWORD APIENTRY
    OEMDownloadCharGlyph(PDEVOBJ pdevobj, PUNIFONTOBJ pUFObj, HGLYPH hGlyph,
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") DWORD APIENTRY
    OEMTTDownloadMethod(PDEVOBJ pdevobj, PUNIFONTOBJ pUFObj);
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") VOID APIENTRY
    OEMOutputCharStr(PDEVOBJ pdevobj, PUNIFONTOBJ pUFObj, DWORD dwType,
                     DWORD dwCount, _In_reads_(dwCount) PVOID pGlyph);
...
__drv_preferredFunction("IPrintOemUni", "Obsolete") VOID APIENTRY
    OEMSendFontCmd(PDEVOBJ pdevobj, PUNIFONTOBJ pUFObj, PFINVOCATION pFInv);

//
// OEMTextOutAsBitmap - UNIDRV only
//
__drv_preferredFunction("IPrintOemUni", "Obsolete") BOOL APIENTRY
    OEMTextOutAsBitmap(SURFOBJ *pso, STROBJ *pstro, FONTOBJ *pfo, CLIPOBJ *pco,
...
BOOL APIENTRY OEMBitBlt(SURFOBJ *psoTrg, SURFOBJ *psoSrc, SURFOBJ *psoMask,
...
BOOL APIENTRY OEMStretchBlt(SURFOBJ *psoDest, SURFOBJ *psoSrc, SURFOBJ *psoMask,
...
BOOL APIENTRY OEMCopyBits(SURFOBJ *psoDest, SURFOBJ *psoSrc, CLIPOBJ *pco,
...
BOOL APIENTRY OEMTextOut(SURFOBJ *pso, STROBJ *pstro, FONTOBJ *pfo,
...
BOOL APIENTRY OEMStrokePath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco,
...
BOOL APIENTRY OEMFillPath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco,
...
BOOL APIENTRY OEMStrokeAndFillPath(SURFOBJ *pso, PATHOBJ *ppo, CLIPOBJ *pco,
                                   XFORMOBJ *pxo, BRUSHOBJ *pboStroke,
                                   LINEATTRS *plineattrs, BRUSHOBJ *pboFill,
                                   POINTL *pptlBrushOrg, MIX mixFill,
                                   FLONG flOptions);
...
BOOL APIENTRY OEMRealizeBrush(BRUSHOBJ *pbo, SURFOBJ *psoTarget,
...
ULONG APIENTRY OEMEscape(SURFOBJ *pso, ULONG iEsc, ULONG cjIn,
                         _In_reads_bytes_(cjIn) PVOID pvIn, ULONG cjOut,
...
BOOL APIENTRY OEMStartDoc(SURFOBJ *pso, _In_ PWSTR pwszDocName, DWORD dwJobId);
...
PIFIMETRICS APIENTRY OEMQueryFont(DHPDEV dhpdev, ULONG_PTR iFile, ULONG iFace,
                                  ULONG_PTR *pid);
...
PVOID APIENTRY OEMQueryFontTree(DHPDEV dhpdev, ULONG_PTR iFile, ULONG iFace,
                                ULONG iMode, ULONG_PTR *pid);

//
// OEMQueryFontData
//

LONG APIENTRY OEMQueryFontData(DHPDEV dhpdev, FONTOBJ *pfo, ULONG iMode,
...
BOOL APIENTRY OEMQueryAdvanceWidths(DHPDEV dhpdev, FONTOBJ *pfo, ULONG iMode,
...
ULONG APIENTRY OEMFontManagement(SURFOBJ *pso, FONTOBJ *pfo, ULONG iMode,
                                 ULONG cjIn, _In_reads_bytes_(cjIn) PVOID pvIn,
                                 ULONG cjOut,
                                 _Out_writes_bytes_(cjOut) PVOID pvOut);
...
ULONG APIENTRY OEMGetGlyphMode(DHPDEV dhpdev, FONTOBJ *pfo);

BOOL APIENTRY OEMNextBand(SURFOBJ *pso, POINTL *pptl);

BOOL APIENTRY OEMStartBanding(SURFOBJ *pso, POINTL *pptl);

ULONG APIENTRY OEMDitherColor(DHPDEV dhpdev, ULONG iMode, ULONG rgbColor,
                              ULONG *pulDither);

BOOL APIENTRY OEMPaint(SURFOBJ *pso, CLIPOBJ *pco, BRUSHOBJ *pbo,
                       POINTL *pptlBrushOrg, MIX mix);

BOOL APIENTRY OEMLineTo(SURFOBJ *pso, CLIPOBJ *pco, BRUSHOBJ *pbo, LONG x1,
...
BOOL APIENTRY OEMPlgBlt(SURFOBJ *psoDst, SURFOBJ *psoSrc, SURFOBJ *psoMask,
                        CLIPOBJ *pco, XLATEOBJ *pxlo, COLORADJUSTMENT *pca,
                        POINTL *pptlBrushOrg, POINTFIX *pptfixDest,
...
BOOL APIENTRY OEMAlphaBlend(SURFOBJ *psoDest, SURFOBJ *psoSrc, CLIPOBJ *pco,
...
BOOL APIENTRY OEMGradientFill(SURFOBJ *psoDest, CLIPOBJ *pco, XLATEOBJ *pxlo,
...
HANDLE APIENTRY OEMIcmCreateColorTransform(
    DHPDEV dhpdev, LPLOGCOLORSPACEW pLogColorSpace,
    _In_reads_bytes_opt_(cjSourceProfile) PVOID pvSourceProfile,
    ULONG cjSourceProfile, _In_reads_bytes_(cjDestProfile) PVOID pvDestProfile,
    ULONG cjDestProfile,
    _In_reads_bytes_opt_(cjTargetProfile) PVOID pvTargetProfile,
    ULONG cjTargetProfile, DWORD dwReserved);

//
// OEMIcmDeleteTransform
//

BOOL APIENTRY OEMIcmDeleteColorTransform(DHPDEV dhpdev, HANDLE hcmXform);

//
// OEMQueryDeviceSupport
//

BOOL APIENTRY OEMQueryDeviceSupport(SURFOBJ *pso, XLATEOBJ *pxlo, XFORMOBJ *pxo,
...
BOOL APIENTRY OEMTransparentBlt(SURFOBJ *psoDst, SURFOBJ *psoSrc, CLIPOBJ *pco,
...
typedef struct _SIMULATE_CAPS_1 {
  ...
  DWORD dwNupOptions;     // The (1-base) bit set represents the N-up option
                      // available. 0x0001 means 1-up 0x0002 means 2-up 0x0008
                      // means 4-up 0x812B means (1,2,4,6,9,16)
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMCommonUIProp(DWORD dwMode, POEMCUIPPARAM pOemCUIPParam);
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") LRESULT APIENTRY
    OEMDocumentPropertySheets(_In_ PPROPSHEETUI_INFO pPSUIInfo, LPARAM lParam);
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") LRESULT APIENTRY
    OEMDevicePropertySheets(_In_ PPROPSHEETUI_INFO pPSUIInfo, LPARAM lParam);
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMDevQueryPrintEx(POEMUIOBJ poemuiobj, PDEVQUERYPRINT_INFO pDQPInfo,
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") DWORD APIENTRY
    OEMDeviceCapabilities(POEMUIOBJ poemuiobj, HANDLE hPrinter,
                          PWSTR pDeviceName, WORD wCapability, PVOID pOutput,
                          PDEVMODE pPublicDM, PVOID pOEMDM, DWORD dwLastResult);
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMUpgradePrinter(DWORD dwLevel, PBYTE pDriverUpgradeInfo);
...
__drv_preferredFunction("(see documentation)", "Obsolete") BOOL APIENTRY
    OEMUpgradeRegistry(DWORD dwLevel, PBYTE pDriverUpgradeInfo,
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMPrinterEvent(PWSTR pPrinterName, INT iDriverEvent, DWORD dwFlags,
                    LPARAM lParam);
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMPDriverEvent(DWORD dwDriverEvent, DWORD dwLevel, LPBYTE pDriverInfo,
                    LPARAM lParam);
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL APIENTRY
    OEMQueryColorProfile(HANDLE hPrinter, POEMUIOBJ poemuiobj,
                         PDEVMODE pPublicDM, PVOID pOEMDM, ULONG ulQueryMode,
                         _Out_writes_bytes_(*pcbProfileData)
                             VOID *pvProfileData,
...
__drv_preferredFunction("IPrintOemUI", "Obsolete") INT_PTR CALLBACK
    OEMFontInstallerDlgProc(HWND hWnd, UINT usMsg, WPARAM wParam,
                            LPARAM lParam);

__drv_preferredFunction("IPrintOemUI", "Obsolete") BOOL CALLBACK
    OEMUpdateExternalFonts(HANDLE hPrinter, HANDLE hHeap,
...
//
// Data structure for storing information about PPD's *ParamCustomPageSize
// entries
//
