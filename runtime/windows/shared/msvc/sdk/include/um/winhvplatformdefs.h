...
typedef enum WHV_REGISTER_NAME {
  ...
  // APIC state (also accessible via
  // WHv(Get/Set)VirtualProcessorInterruptControllerState)
  WHvX64RegisterApicId = 0x00003002,
  WHvX64RegisterApicVersion = 0x00003003,
  // X2APIC state (also accessible via
  // WHv(Get/Set)VirtualProcessorInterruptControllerState)
...
typedef enum WHV_REGISTER_NAME {
  ...
  //
  // ID registers are exposed as partition wide registers. A partition wide
  // register can be set by the parent using WHV_ANY_VP to change the value read
  // by a guest. ID registers have the following behavior: -Parent write with
  // WHV_ANY_VP - sets an override to replace the default value read by a guest.
  // -Parent read with WHV_ANY_VP - returns the override if one has been set,
  // else the default value. -Guest write\read with WHV_ANY_VP - not allowed.
  ...
  // Hypervisor synthetic CPUID leaves are exposed as partition wide registers.
  // A partition wide register can be set by the parent using WHV_ANY_VP to
  // change the value read by a guest. Hypervisor synthetic CPUID leaf registers
  // have the following behavior: -Parent write with WHV_ANY_VP - sets an
  // override to replace the default value read by a guest. -Parent read with
  // WHV_ANY_VP - returns the override if one has been set, else the default
  // value. -Guest write\read with WHV_ANY_VP - not allowed. -Parent\guest write
  // with VP index - not allowed. -Parent\guest read with VP index - returns the
  // override if one has been set, else the default value.
  //
  // Version
  WHvRegisterHypervisorVersion =
      0x00000100, // 128-bit result same as CPUID 0x40000002

  // Feature Access
  WHvRegisterPrivilegesAndFeaturesInfo =
      0x00000200, // 128-bit result same as CPUID 0x40000003
  WHvRegisterFeaturesInfo =
      0x00000201, // 128-bit result same as CPUID 0x40000004
  WHvRegisterImplementationLimitsInfo =
      0x00000202, // 128-bit result same as CPUID 0x40000005
  WHvRegisterHardwareFeaturesInfo =
      0x00000203, // 128-bit result same as CPUID 0x40000006
  WHvRegisterCpuManagementFeaturesInfo =
      0x00000204, // 128-bit result same as CPUID 0x40000007
...
#if defined(_AMD64_)
    UINT64 X64ApicInitSipiExitTrap
        : 1; // WHvRunVpExitReasonX64ApicInitSipiTrap supported
    UINT64 X64ApicWriteLint0ExitTrap
        : 1; // WHvRunVpExitReasonX64ApicWriteTrap supported
    UINT64 X64ApicWriteLint1ExitTrap
        : 1; // WHvRunVpExitReasonX64ApicWriteTrap supported
    UINT64 X64ApicWriteSvrExitTrap
        : 1; // WHvRunVpExitReasonX64ApicWriteTrap supported
#else
    UINT64 ReservedArm1 : 4;
#endif
    UINT64 UnknownSynicConnection
        : 1; // WHvRunVpExitReasonHypercall supported for unknown synic
             // connection to HvSignalEvent
    UINT64 RetargetUnknownVpciDevice
        : 1; // WHvRunVpExitReasonHypercall supported for unknown device to
             // HvRetargetDeviceInterrupt
#if defined(_AMD64_)
    UINT64 X64ApicWriteLdrExitTrap
        : 1; // WHvRunVpExitReasonX64ApicWriteTrap supported
    UINT64 X64ApicWriteDfrExitTrap
        : 1; // WHvRunVpExitReasonX64ApicWriteTrap supported
#else
    UINT64 ReservedArm2 : 2;
#endif
    UINT64 GpaAccessFaultExit : 1; // WHvRunVpExitReasonMemoryAccess supported
                                   // for second-level page faults
    UINT64 Reserved : 49;
  };
...
// for WHvPartitionPropertyCodeProcessorFeaturesBanks. BanksCount must be
// populated before calling WHP APIs.
//
...
typedef union WHV_ARM64_PROCESSOR_FEATURES {
  struct {
    ...
    UINT64 TGran16
        : 1; // Indicated support for 16KB memory translation granule.
    UINT64 TGran64
        : 1; // Indicated support for 64KB memory translation granule.
    ...
    UINT64 GicV3V4 : 1; // System register interface to versions 3 and 4 of the
                        // GIC CPU interface is implemented.
    UINT64 GicV41 : 1;  // System register interface to version 4.1 of the GIC
                        // CPU interface is implemented.
    UINT64 Ras : 1;     // Ras
    ...
    UINT64 DcCvadp
        : 1; // Clean data cache by address to point of deep persistence.
    ...
    UINT64 ApaEp2 : 1;   // HaveEnhancedPAC -> false, HaveEnhancedPAC2 -> true,
                         // HaveFPAC -> false, HaveFPACCombined -> false.
    UINT64 ApaEp2Fp : 1; // HaveEnhancedPAC -> false, HaveEnahancedPAC2 -> true,
                         // HaveFPAC -> true, HaveFPACCombined -> false.
    UINT64 ApaEp2Fpc : 1; // HaveEnhancedPAC -> false, HaveEnahancedPAC2 ->
                          // true, HaveFPAC -> true, HaveFPACCombined -> true.

    UINT64 Jscvt : 1;   // FJCVTZS instruction implemented. Support for JS
                        // conversion from double to integer.
    UINT64 Fcma : 1;    // Complex number instructions(FCMLA and FCADD)
    ...
    UINT64 Ssbs : 1;        // FEAT_SSBS (Speculative Store Bypass Safe):
                            // ID_AA64PFR1_EL1.SSBS >= 0b0001
    UINT64 SsbsRw : 1;      // FEAT_SSBS2 (Speculative Store Bypass
                            // Safe):ID_AA64PFR1_EL1.SSBS >= 0b0010
    UINT64 Reserved49 : 1;  // Unused
    UINT64 Reserved50 : 1;  // Unused
    UINT64 Reserved51 : 1;  // Unused
    UINT64 Reserved52 : 1;  // Unused
    UINT64 Csv2 : 1;        // FEAT_CSV2 (Cache Speculation Variant 2):
                            // ID_AA64PFR0_EL1.CSV2 >= 0b0001
    UINT64 Csv3 : 1;        // FEAT_CSV3 (Cache Speculation Variant 3):
                            // ID_AA64PFR0_EL1.CSV3 >= 0b0001
    UINT64 Sb
        : 1; // FEAT_SB (Speculation Barrier): ID_AA64ISAR1_EL1.SB >= 0b0001
    UINT64 Idc : 1;         // CTR_EL0.IDC == 0b1
    UINT64 Dic : 1;         // CTR_EL0.DIC == 0b1
    UINT64 TlbiOs : 1;      // FEAT_TLBIOS (TLB invalidate instructions in Outer
                            // Shareable domain): ID_AA64ISAR0_EL1.TLB >= 0b0001
    UINT64 TlbiOsRange : 1; // FEAT_TLBIRANGE (TLB invalidate range
                            // instructions): ID_AA64ISAR0_EL1.TLB >= 0b0010
    UINT64 FlagsM : 1;      // ID_AA64ISAR0_EL1.TS >= 0b0001
    UINT64 FlagsM2 : 1;     // ID_AA64ISAR0_EL1.TS >= 0b0010
    UINT64 Bf16 : 1;        // FEAT_BF16 (AArch64 BFloat16 instructions):
                            // ID_AA64ISAR1_EL1.BF16 >= 0b0001
    UINT64 Ebf16 : 1; // FEAT_EBF16 (AArch64 Extended BFloat16 instructions):
...
typedef union WHV_ARM64_PROCESSOR_FEATURES1 {
  struct {
    UINT64 SveBf16
        : 1; // ID_AA64ZFR0_EL1.BF16 >= 0b0001 (Second bank starts here)
    ...
    UINT64 Sve : 1;   // ID_AA64PFR0_EL1.SVE >= 0b0001 && ID_AA64ZFR0_EL1.SVEver
                      // >= 0b0000
    UINT64 SveV2 : 1; // ID_AA64PFR0_EL1.SVE >= 0b0001 && ID_AA64ZFR0_EL1.SVEver
                      // >= 0b0001
    UINT64 SveV2P1 : 1;   // ID_AA64PFR0_EL1.SVE >= 0b0001 &&
...
C_ASSERT(sizeof(WHV_PROCESSOR_FEATURES_BANKS) ==
         8 * (WHV_PROCESSOR_FEATURES_BANKS_COUNT + 1));
...
typedef union WHV_SYNTHETIC_PROCESSOR_FEATURES {
  struct {
    ...
    // On AMD64, HvCallFlushVirtualAddressSpace / HvCallFlushVirtualAddressList
    // are supported, on ARM64 HvCallFlushVirtualAddressSpace / HvCallFlushTlb
    // are supported.
...
//
// Context data for an exit caused by an exception generated by the virtual
// processor (WHvRunVpExitReasonException)
//
...
//
// Context data for an exit caused by the use of an unsupported processor
// feature (WHvRunVpExitReasonUnsupportedFeature)
//
...
typedef enum WHV_RUN_VP_CANCEL_REASON {
  WHvRunVpCancelReasonUser =
      0 // Execution canceled by WHvCancelRunVirtualProcessor
...
//
// Context data for an exit caused by an interrupt delivery window cancellation
// from the host (WHvRunVpExitReasonX64InterruptWindow)
//
...
//
// Context data for an exit caused by an APIC SMI
...
//
// Context data for an exit caused by an APIC INIT SIPI
...
//
// Context data for an exit caused by an APIC write
...
typedef enum WHV_VIRTUAL_PROCESSOR_STATE_TYPE {
  ...
  WHvVirtualProcessorStateTypeSynicMessagePage =
      0x00000002 | WHV_VIRTUAL_PROCESSOR_STATE_TYPE_PFN,
  WHvVirtualProcessorStateTypeSynicEventFlagPage =
      0x00000003 | WHV_VIRTUAL_PROCESSOR_STATE_TYPE_PFN,
  WHvVirtualProcessorStateTypeSynicTimerState = 0x00000004,

  WHvVirtualProcessorStateTypeGlobalInterruptState =
      0x00000006 | WHV_VIRTUAL_PROCESSOR_STATE_TYPE_PFN |
      WHV_VIRTUAL_PROCESSOR_STATE_TYPE_ANY_VP,
  WHvVirtualProcessorStateTypeSveState =
      0x00000007 | WHV_VIRTUAL_PROCESSOR_STATE_TYPE_PFN,
} WHV_VIRTUAL_PROCESSOR_STATE_TYPE;
...
typedef struct WHV_X64_SVM_NESTED_STATE {
  ...
  //
  // Specifices the current VMCB if the VP is currently in guest mode. Format is
  // as specified by AMD.
  //
