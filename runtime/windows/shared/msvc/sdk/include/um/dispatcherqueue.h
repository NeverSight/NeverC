...
struct DispatcherQueueOptions {
  ...
  threadType; // Thread affinity on which DispatcherQueueController is
              // created.
  DISPATCHERQUEUE_THREAD_APARTMENTTYPE
  apartmentType; // Initialize COM apartment on the new thread as ASTA or
                 // STA. Only relevant if threadType is
...
//     DispatcherQueueController will be created on the dedicated thread. As
//     part of the API we will also create the thread and create this object on
//     it. It will also run the DispatcherQueue event loop on the new thread.
//
...
//
//     Event loop will be run on the background thread asynchronously to
//     dispatch the pending task items on the new thread.
//
//
...
//
// Parameters:
//     options - struct to configure the DispatcherQueueController
//     initialization.
//
...
extern "C" HRESULT __declspec(dllexport) WINAPI CreateDispatcherQueueController(
