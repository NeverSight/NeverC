...
USERENVAPI
BOOL WINAPI LoadUserProfileA(_In_ HANDLE hToken,
                             _Inout_ LPPROFILEINFOA lpProfileInfo);
USERENVAPI
BOOL WINAPI LoadUserProfileW(_In_ HANDLE hToken,
...
//           FALSE if not.  Call GetLastError() for more details
//
// Note:     The caller of this function must have admin privileges on the
// machine.
//
//=============================================================================
...
//           FALSE if not.  Call GetLastError() for more details
//
// Note:     If lpProfilesDir is not large enough or NULL, the function will
// fail,
...
USERENVAPI
BOOL WINAPI GetProfilesDirectoryA(_Out_writes_opt_(*lpcchSize)
                                      LPSTR lpProfileDir,
                                  _Inout_ LPDWORD lpcchSize);
USERENVAPI
BOOL WINAPI GetProfilesDirectoryW(_Out_writes_opt_(*lpcchSize)
                                      LPWSTR lpProfileDir,
                                  _Inout_ LPDWORD lpcchSize);
#ifdef UNICODE
...
//
//  Comments:   if profile is not already loaded the function will return an
//  error.
...
#define PT_TEMPORARY                                                           \
  0x00000001 // A profile has been allocated that will be deleted at logoff.
...
#define PT_ROAMING_PREEXISTING                                                 \
  0x00000008 // The loaded roaming profile is not a brand new one - it was
             // obtained from roamed location

USERENVAPI
BOOL WINAPI GetProfileType(_Out_ DWORD *dwFlags);
...
HRESULT
WINAPI
CreateProfile(_In_ LPCWSTR pszUserSid, _In_ LPCWSTR pszUserName,
...
//           FALSE if not.  Call GetLastError() for more details
//
// Note:     If lpProfileDir is not large enough or NULL, the function will
// fail,
//           and lpcchSize will contain the necessary buffer size.
//
...
USERENVAPI
BOOL WINAPI GetDefaultUserProfileDirectoryA(_Out_writes_opt_(*lpcchSize)
                                                LPSTR lpProfileDir,
                                            _Inout_ LPDWORD lpcchSize);
USERENVAPI
BOOL WINAPI GetDefaultUserProfileDirectoryW(_Out_writes_opt_(*lpcchSize)
                                                LPWSTR lpProfileDir,
                                            _Inout_ LPDWORD lpcchSize);
#ifdef UNICODE
...
USERENVAPI
BOOL WINAPI GetAllUsersProfileDirectoryA(_Out_writes_opt_(*lpcchSize)
                                             LPSTR lpProfileDir,
                                         _Inout_ LPDWORD lpcchSize);
USERENVAPI
BOOL WINAPI GetAllUsersProfileDirectoryW(_Out_writes_opt_(*lpcchSize)
                                             LPWSTR lpProfileDir,
                                         _Inout_ LPDWORD lpcchSize);
#ifdef UNICODE
...
// hToken         -  User's token returned from LogonUser() (optional, can be
// NULL) bInherit       -  Inherit from the current process's environment block
...
BOOL WINAPI CreateEnvironmentBlock(_At_((PZZWSTR *)lpEnvironment, _Outptr_)
                                       LPVOID *lpEnvironment,
                                   _In_opt_ HANDLE hToken, _In_ BOOL bInherit);
...
//
// hToken         -  User's token returned from LogonUser() (optional, can be
// NULL) lpSrc          -  Pointer to the string with environment variables
...
USERENVAPI
BOOL WINAPI ExpandEnvironmentStringsForUserA(_In_opt_ HANDLE hToken,
                                             _In_ LPCSTR lpSrc,
                                             _Out_writes_(dwSize) LPSTR lpDest,
                                             _In_ DWORD dwSize);
USERENVAPI
BOOL WINAPI ExpandEnvironmentStringsForUserW(_In_opt_ HANDLE hToken,
                                             _In_ LPCWSTR lpSrc,
...
USERENVAPI
BOOL WINAPI RefreshPolicy(_In_ BOOL bMachine);
...
#define RP_SYNC                                                                \
  2 // Refresh Policy sync; the call does not return till the time policy
    // processing is completed

USERENVAPI
BOOL WINAPI RefreshPolicyEx(_In_ BOOL bMachine, _In_ DWORD dwOptions);
...
HANDLE
WINAPI
EnterCriticalPolicySection(_In_ BOOL bMachine);
...
USERENVAPI
BOOL WINAPI LeaveCriticalPolicySection(_In_ HANDLE hSection);
...
//                wParam - 1 if machine policy was applied, 0 if user policy was
//                applied. lParam - Points to the string "Policy"
//
//=============================================================================
...
USERENVAPI
BOOL WINAPI RegisterGPNotification(_In_ HANDLE hEvent, _In_ BOOL bMachine);
...
USERENVAPI
BOOL WINAPI UnregisterGPNotification(_In_ HANDLE hEvent);
...
//
// The GUID is the GPO name, and the number following the LDAP path are the
// options for that link from this DS object.  Note, there can be multiple GPOs
...
//
// hToken           - User or machine token, if NULL, lpName and lpHostName must
// be supplied lpName           - User or machine name in DN format, if hToken
// is supplied, this must be NULL lpHostName       - Domain DN name or domain
// controller name. If hToken is supplied, this must be NULL lpComputerName   -
// Computer name to use to determine site location.  If NULL,
//                    the local computer is used as the reference. Format:
//                    \\machinename
...
typedef struct _GROUP_POLICY_OBJECTA {
  ...
  LPSTR lpLink; // Path to the Active Directory site, domain, or organizational
                // unit this GPO is linked to If this is the local GPO, this
                // points to the word "Local"
...
typedef struct _GROUP_POLICY_OBJECTW {
  ...
  LPARAM lParam2; // Free space for the caller to store GPO specific information
  LPWSTR lpLink; // Path to the Active Directory site, domain, or organizational
                 // unit this GPO is linked to If this is the local GPO, this
                 // points to the word "Local"
...
#define GPO_LIST_FLAG_NO_WMIFILTERS                                            \
  0x00000004 // Ignore WMI filters when filtering GPO's
#define GPO_LIST_FLAG_NO_SECURITYFILTERS 0x00000008 // Ignore security filters

USERENVAPI
BOOL WINAPI GetGPOListA(_In_opt_ HANDLE hToken, _In_opt_ LPCSTR lpName,
                        _In_opt_ LPCSTR lpHostName,
                        _In_opt_ LPCSTR lpComputerName, _In_ DWORD dwFlags,
                        _Outptr_ PGROUP_POLICY_OBJECTA *pGPOList);
USERENVAPI
BOOL WINAPI GetGPOListW(_In_opt_ HANDLE hToken, _In_opt_ LPCWSTR lpName,
                        _In_opt_ LPCWSTR lpHostName,
                        _In_opt_ LPCWSTR lpComputerName, _In_ DWORD dwFlags,
...
USERENVAPI
BOOL WINAPI FreeGPOListA(_In_ PGROUP_POLICY_OBJECTA pGPOList);
USERENVAPI
BOOL WINAPI FreeGPOListW(_In_ PGROUP_POLICY_OBJECTW pGPOList);
#ifdef UNICODE
...
// user or machine and specified client side extension. This function will
// return a linked list of Group Policy Objects.  Call FreeGPOList to free the
// list.
//
// dwFlags          - User or machine policy, if it is GPO_LIST_FLAG_MACHINE
// then
//                    return machine policy information
// pMachineName     - Name of remote computer in the form \\computername. If
// null
//                    then local computer is used.
// pSidUser         - Security id of user (relevant for user policy). If
// pMachineName is
//                    null and pSidUser is null then it means current logged on
//                    user. If pMachine is null and pSidUser is non-null then it
//                    means user represented by pSidUser on local machine. If
//                    pMachineName is non-null then and if dwFlags specifies
//                    user policy, then pSidUser must be non-null.
// pGuidExtension   - Guid of the specified extension
// ppGPOList        - Address of a pointer which receives the link list of GPOs
//
// The return value is a Win32 error code. ERROR_SUCCESS means the
// GetAppliedGPOList function completed successfully. Otherwise it indicates
...
DWORD
WINAPI
GetAppliedGPOListA(_In_ DWORD dwFlags, _In_opt_ LPCSTR pMachineName,
                   _In_opt_ PSID pSidUser, _In_ GUID *pGuidExtension,
                   _Outptr_ PGROUP_POLICY_OBJECTA *ppGPOList);
USERENVAPI
DWORD
WINAPI
GetAppliedGPOListW(_In_ DWORD dwFlags, _In_opt_ LPCWSTR pMachineName,
                   _In_opt_ PSID pSidUser, _In_ GUID *pGuidExtension,
...
//
// The subkey needs to be a guid so that it is unique. The noname value of the
// subkey can be the friendly name of the extension. Then add these values:
//
//     DllName                      REG_EXPAND_SZ  Path to your DLL
//     ProcessGroupPolicy           REG_SZ       Function name (see
//     PFNPROCESSGROUPPOLICY prototype). This
//                                                 is obsolete, it has been
//                                                 superseded by
//                                                 ProcessGroupPolicyEx. It's
//                                                 here for backward
//                                                 compatibility reasons only.
//     ProcessGroupPolicyEx         REG_SZ       Function name (see
//     PFNPROCESSGROUPPOLICYEX prototype) GenerateGroupPolicy          REG_SZ
//     Function name for Rsop (see PFNGENERATEGROUPPOLICY prototype)
//     NoMachinePolicy              REG_DWORD    True, if extension does not
//     have to be called when
//                                                 machine policies are being
//                                                 processed.
//     NoUserPolicy                 REG_DWORD    True, if extension does not
//     have to be called when
//                                                 user policies are being
//                                                 processed.
//     NoSlowLink                   REG_DWORD    True, if extension does not
//     have to be called on a slow link NoBackgroundPolicy           REG_DWORD
//     True, if extension does not have to be called
//                                                 for background policy
//                                                 processing.
//     NoGPOListChanges             REG_DWORD    True, if extension does not
//     have to be called when
//                                                 there are no changes between
//                                                 cached and current GPO lists.
//     PerUserLocalSettings         REG_DWORD    True, if user policies have to
//     be cached on a per user and
//                                                 per machine basis.
//     RequiresSuccessfulRegistry   REG_DWORD    True, if extension should be
//     called only if registry extension
//                                                 was successfully processed.
//     EnableAsynchronousProcessing REG_DWORD    True, if registry extension
//     will complete its processing
//                                                 asynchronously.
//     NotifyLinkTransition         REG_DWORD    True, if extension should be
//     called when a change in link
//                                                 speed is detected between
//                                                 previous policy application
//                                                 and current policy
//                                                 application.
//
// The return value is a Win32 error code. ERROR_SUCCESS means the
// ProcessGroupPolicy function completed successfully. If return value is
// ERROR_OVERRIDE_NOCHANGES then it means that the extension will be called the
// next time even if NoGPOListChanges is set and there are no changes to the GPO
// list. Any other return value indicates that the ProcessGroupPolicy or
// ProcessGroupPolicyEx function failed.
//
//=============================================================================
...
#define GPO_INFO_FLAG_NOCHANGES                                                \
...
#define GPO_INFO_FLAG_FORCED_REFRESH                                           \
...
#define GPO_INFO_FLAG_ASYNC_FOREGROUND                                         \
  0x00001000 // Asynchronous foreground refresh of policy

typedef UINT_PTR ASYNCCOMPLETIONHANDLE;
typedef DWORD (*PFNSTATUSMESSAGECALLBACK)(_In_ BOOL bVerbose,
...
typedef DWORD (*PFNPROCESSGROUPPOLICY)(
    ...
    _In_opt_ PFNSTATUSMESSAGECALLBACK
...
typedef DWORD (*PFNPROCESSGROUPPOLICYEX)(
    ...
    _In_opt_ PFNSTATUSMESSAGECALLBACK
        pStatusCallback, // Callback function for displaying status messages
                         // Note, this can be NULL
    _In_opt_ IWbemServices
        *pWbemServices, // Pointer to namespace to log diagnostic mode data
...
typedef struct _RSOP_TARGET {
  ...
  IWbemServices
      *pWbemServices; // Pointer to namespace to log planning mode data
...
//
// The return value is a Win32 error code. ERROR_SUCCESS means the
// ProcessGroupPolicyCompleted function completed successfully. Otherwise it
...
DWORD
WINAPI
ProcessGroupPolicyCompleted(_In_ REFGPEXTENSIONID extensionId,
...
DWORD
WINAPI
ProcessGroupPolicyCompletedEx(_In_ REFGPEXTENSIONID extensionId,
                              _In_ ASYNCCOMPLETIONHANDLE pAsyncHandle,
                              _In_ DWORD dwStatus, _In_ HRESULT RsopStatus);
...
//
// Description: Determines whether the security descriptor pointed to by
// pSecurityDescriptor
//                              grants the set of access rights specified in
//                              dwDesiredAccessMask to the client identified by
//                              the RSOPTOKEN pointed to by pRsopToken.
//
...
// needs to be checked dwDesiredAccessMask  - Mask of requested generic and/or
// standard and or specific access rights pObjectTypeList      - Object Type
// List ObjectTypeListLength - Object Type List Length pGenericMapping      -
// Generic Mapping pPrivilegeSet        - privilege set pdwPrivilegeSetLength-
// privilege set length pdwGrantedAccessMask - On success, if pbAccessStatus is
// true, it contains
//                                         the mask of standard and specific
//                                         rights granted. If pbAccessStatus is
//                                         false, it is set to 0. On failure, it
//                                         is not modified.
...
RsopAccessCheckByType(
    ...
    _In_opt_ PSID pPrincipalSelfSid, _In_ PRSOPTOKEN pRsopToken,
    _In_ DWORD dwDesiredAccessMask,
    _In_reads_opt_(ObjectTypeListLength) POBJECT_TYPE_LIST pObjectTypeList,
    _In_ DWORD ObjectTypeListLength, _In_ PGENERIC_MAPPING pGenericMapping,
    _In_reads_bytes_opt_(*pdwPrivilegeSetLength) PPRIVILEGE_SET pPrivilegeSet,
    _In_opt_ LPDWORD pdwPrivilegeSetLength, _Out_ LPDWORD pdwGrantedAccessMask,
...
//
// Description: Determines whether the security descriptor on the file grants
// the set of file access
//                              rights specified in dwDesiredAccessMask
//                              to the client identified by the RSOPTOKEN
//                              pointed to by pRsopToken.
//
// pszFileName          - Name of an existing filename
// pRsopToken           - Pointer to a valid RSOPTOKEN against which access
// needs to be checked dwDesiredAccessMask  - Mask of requested generic and/or
// standard and or specific access rights pdwGrantedAccessMask - On success, if
// pbAccessStatus is true, it contains
//                                         the mask of standard and specific
//                                         rights granted. If pbAccessStatus is
//                                         false, it is set to 0. On failure, it
//                                         is not modified.
...
HRESULT
WINAPI
RsopFileAccessCheck(_In_ LPWSTR pszFileName, _In_ PRSOPTOKEN pRsopToken,
                    _In_ DWORD dwDesiredAccessMask,
...
//  szEventLogName      - name of the event log database where the messages are
//  logged dwEventID           - event log message ID status              -
//  status of the policy setting timeLogged          - time at which the event
//  log message was logged
//
//=============================================================================
...
HRESULT
WINAPI
RsopSetPolicySettingStatus(_In_ DWORD dwFlags, _In_ IWbemServices *pServices,
...
HRESULT
WINAPI
RsopResetPolicySettingStatus(_In_ DWORD dwFlags, _In_ IWbemServices *pServices,
                             _In_ IWbemClassObject *pSettingInstance);
...
#define FLAG_NO_GPO_FILTER                                                     \
  0x80000000 // GPOs are not filtered, implies FLAG_NO_CSE_INVOKE
#define FLAG_NO_CSE_INVOKE                                                     \
  0x40000000 // only GP processing done for planning mode
#define FLAG_ASSUME_SLOW_LINK 0x20000000 // planning mode RSoP assumes slow link
#define FLAG_LOOPBACK_MERGE                                                    \
  0x10000000 // planning mode RSoP assumes merge loop back
#define FLAG_LOOPBACK_REPLACE                                                  \
  0x08000000 // planning mode RSoP assumes replace loop back

#define FLAG_ASSUME_USER_WQLFILTER_TRUE                                        \
  0x04000000 // planning mode RSoP assumes all comp filters to be true
#define FLAG_ASSUME_COMP_WQLFILTER_TRUE                                        \
  0x02000000 // planning mode RSoP assumes all user filters to be true

#define FLAG_PLANNING_MODE                                                     \
  0x01000000 // flag that indicates that a given namespace was created
...
// This user is an interactive non admin user, the temp snapshot namespace
// already exists and the FLAG_FORCE_CREATENAMESPACE was not passed in
...
DWORD
WINAPI
GenerateGPNotification(_In_ BOOL bMachine, _In_ LPCWSTR lpwszMgmtProduct,
...
//  ppSidAppContainerSid   - SID of the AppContainer. Must be freed using
//  FreeSid.
//
//  Return:     S_OK : Successfully created the profile
//              E_ACCESSDENIED : The caller does not have enough permission to
//                      create the profile.
//              HRESULT_FROM_WIN32(ERROR_ALREADY_EXISTS) : A profile already
//              exists
...
HRESULT
WINAPI
DeleteAppContainerProfile(_In_ PCWSTR pszAppContainerName);
...
HRESULT
WINAPI
GetAppContainerRegistryLocation(_In_ REGSAM desiredAccess,
...
//  If a thread token is set, that user is used to determine the AppContainer.
//  Otherwise, the process identity is used.
//
...
HRESULT
WINAPI
GetAppContainerFolderPath(_In_ PCWSTR pszAppContainerSid,
...
WINAPI
DeriveRestrictedAppContainerSidFromAppContainerSidAndRestrictedName(
    _In_ PSID psidAppContainerSid, _In_ PCWSTR pszRestrictedAppContainerName,
...
HRESULT
WINAPI
DeriveAppContainerSidFromAppContainerName(_In_ PCWSTR pszAppContainerName,
                                          _Outptr_ PSID *ppsidAppContainerSid);
