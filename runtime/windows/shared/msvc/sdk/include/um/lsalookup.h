...
NTSTATUS
LsaLookupOpenLocalPolicy(_In_ PLSA_OBJECT_ATTRIBUTES ObjectAttributes,
                         _In_ ACCESS_MASK AccessMask,
                         _Inout_ PLSA_LOOKUP_HANDLE PolicyHandle);

NTSTATUS
LsaLookupClose(_In_ LSA_LOOKUP_HANDLE ObjectHandle);

NTSTATUS
LsaLookupTranslateSids(_In_ LSA_LOOKUP_HANDLE PolicyHandle, _In_ ULONG Count,
                       _In_ PSID *Sids,
...
#if (_WIN32_WINNT >= 0x0501)
NTSTATUS
LsaLookupTranslateNames(_In_ LSA_LOOKUP_HANDLE PolicyHandle, _In_ ULONG Flags,
...
NTSTATUS
LsaLookupGetDomainInfo(_In_ LSA_LOOKUP_HANDLE PolicyHandle,
                       _In_ LSA_LOOKUP_DOMAIN_INFO_CLASS DomainInfoClass,
                       _Out_ PVOID *DomainInfo);

NTSTATUS
LsaLookupFreeMemory(_In_ PVOID Buffer);
