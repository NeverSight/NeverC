...
enum TXTHITRESULT {
  ...
  TXTHITRESULT_TRANSPARENT = 1, //@emem point is within the text's rectangle,
                                // but in a transparent region
...
#define TXTBIT_NOTHREADREFCOUNT                                                \
  0x400000 // don't reference TLS data on behalf of this instance
#define TXTBIT_SHOWPASSWORD 0x800000 // Show password string
#define TXTBIT_D2DDWRITE                                                       \
  0x1000000 // Use D2D/DWrite for this instance (and not GDI/Uniscribe).
#define TXTBIT_D2DSIMPLETYPOGRAPHY                                             \
  0x2000000 // Don't glyph all. Only valid if D2DDWRITE is set.
#define TXTBIT_D2DPIXELSNAPPED                                                 \
  0x4000000 // Don't do subpixel. Only valid if D2DDWRITE is set.
...
#define TXTBIT_FLASHLASTPASSWORDCHAR                                           \
  0x10000000                            // Show last password char momentarily
 ...
 *			extra functionality for windowless operation.  In
 * conjunction with ITextHost, ITextServices provides the means by which the the
 * RichEdit control can be used *without* creating a window.
 *
...
public:
  ...
  virtual HRESULT TxDraw(DWORD dwDrawAspect, LONG lindex, void *pvAspect,
                         ...
                         LPRECT lprcUpdate, BOOL(CALLBACK *pfnContinue)(DWORD),
...
// These definition are used with SES_LOGOCALCARET to delegate caret creation to
// a host through TxCreateCaret With SES_LOGICALCARET, the bitmap passed to the
// host is actually a set of flags The host can use these definitions to convert
// the bitmap handle passed to TxCreateCaret into caret information. Only
// logical carets are provided to D2D hosts
enum CARET_FLAGS {
 ...
 *	ITextHost
 *
 * 	@class	Interface to be used by text services to obtain text host
 * services
 *
 *	@base	public | IUnknown
 */
...
public:
  ...
  virtual BOOL TxSetScrollRange(INT fnBar, LONG nMinPos, INT nMaxPos,
                                BOOL fRedraw) = 0;
 ...
 *
 * 	@class	Interface to be used by hosts to get the UIA object for Text
 * Services
 *
 *	@base	public | IUnknown
 */
interface DECLSPEC_UUID("983E572D-20CD-460B-9104-83111592DD10")
    IRicheditWindowlessAccessibility : public IUnknown {
 ...
 *	@base	public | IUnknown
 */
interface DECLSPEC_UUID("23969A9D-8546-4032-A1BB-73750CBF3333")
    IRichEditUiaInformation : public IUnknown {
  virtual HRESULT STDMETHODCALLTYPE GetBoundaryRectangle(UiaRect *
                                                         pUiaRect) = 0;
 ...
 *
 * 	@class	Interface to be used by containers of windowless RichEdit
 * controls to allow the container to override the RichEdit's UIA properties.
 *
 *	@base	public | IUnknown
 */
interface IRicheditUiaOverrides : public IUnknown {
  virtual HRESULT STDMETHODCALLTYPE GetPropertyOverrideValue(
      PROPERTYID propertyId, VARIANT * pRetValue) = 0;
};
...
typedef HRESULT(STDAPICALLTYPE *PCreateTextServices)(IUnknown *punkOuter,
 ...
 *	ITextServices2
 *
 * 	@class	Extends TxGetNaturalSize to return ascent of single-line
 * controls
 *
...
public:
  ...
  virtual HRESULT TxGetNaturalSize2(DWORD dwAspect, HDC hdcDraw,
                                    HDC hicTargetDev, DVTARGETDEVICE *ptd,
                                    DWORD dwMode, const SIZEL *psizelExtent,
                                    LONG *pwidth, LONG *pheight,
                                    LONG *pascent) = 0;

  virtual HRESULT TxDrawD2D(ID2D1RenderTarget *pRenderTarget,
                            LPCRECTL lprcBounds, LPRECT lprcUpdate,
                            LONG lViewId) = 0;
};
