...
#define USP_E_SCRIPT_NOT_IN_FONT                                               \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_ITF,                                   \
               0x200) // Script doesn't exist in font
...
_Check_return_ HRESULT WINAPI ScriptFreeCache(_Inout_updates_(1) _At_(
    *psc, _Post_null_) SCRIPT_CACHE *psc); // InOut  Cache handle
...
typedef struct tag_SCRIPT_CONTROL {
  ...
  DWORD fContextDigits
      : 1; // Means use previous script instead of uDefaultLanguage

  // The following flags provide legacy support for GetCharacterPlacement
  // features
  DWORD fInvertPreBoundDir
      : 1; // Reading order of virtual item immediately prior to string
  DWORD fInvertPostBoundDir
      : 1; // Reading order of virtual item immediately following string
  ...
  DWORD fNeutralOverride : 1; // Causes all neutrals to be strong in the current
                              // embedding direction
  DWORD fNumericOverride : 1; // Causes all numerals to be strong in the current
                              // embedding direction
  DWORD fLegacyBidiClass : 1; // Causes plus and minus to be reated as neutrals,
                              // slash as a common separator
  DWORD fMergeNeutralItems
      : 1; // Causes merging neutral characters into strong items, when possible
...
typedef struct tag_SCRIPT_STATE {
  ...
  WORD fDisplayZWG
      : 1; // Equiv GCP_DisplayZWG, no Unicode control characters yet
  WORD fArabicNumContext : 1; // For EN->AN Unicode rule
  WORD fGcpClusters
      : 1; // For Generating Backward Compatible GCP Clusters (legacy Apps)
  WORD fReserved : 1;
...
typedef struct tag_SCRIPT_ANALYSIS {
  ...
  WORD fNoGlyphIndex : 1; // Generated by ScriptShape/Place - this item does not
                          // use glyph indices
  SCRIPT_STATE s;
} SCRIPT_ANALYSIS;
//
...
//              Set by ScriptItemize, may be overriden before calling
//              ScriptShape.
//
...
//              TRUE for hdcs containing symbolic, unrecognised and device
//              fonts. Disabling glyphing disables complex script shaping. When
//              set, shaping and placing for this item is implemented directly
//              by calls to GetTextExtentExPoint and ExtTextOut.
/////   SCRIPT_ITEM
//
...
_Check_return_ HRESULT WINAPI ScriptItemize(
    ...
    _In_reads_opt_(1) const SCRIPT_STATE
        *psState, // In   Initial bidi algorithm state (optional)
    _Out_writes_to_(cMaxItems, *pcItems)
...
//      (pItems) such that the length of an item at pItem is always available
//      as:
//
...
//      digits as follows:
//
// t     fDigitSubstitute | FContextDigits | Digit shapes displayed for Unicode
// U+0030 through U+0039 t     ---------------- | -------------- |
// ------------------------------------ t     False            | Any | Western
// (European / American) digits t     True             | False          | As
// specified in SCRIPT_CONTROL.uDefaultLanguage t     True             | True |
...
//
//      However, also in a right-to-left string, if
//      SCRIPT_CONTROL.uDefaultLanguage is 0x1e (LANG_THAI), then no
//      substitution occurs on leading Western digits because the Thai language
...
//      characters whose language depends on the
//      SCRIPT_CONTROL.uDefaultLangauge.
//
...
//      logical run should be displayed - the leftmost display position being
//      zero.
//
...
_Check_return_ HRESULT WINAPI ScriptLayout(
    ...
    _Out_writes_all_opt_(cRuns) int
        *piVisualToLogical, // Out  List of run indices in visual order
    _Out_writes_all_opt_(
        cRuns) int *piLogicalToVisual); // Out  List of visual run positions
...
typedef enum tag_SCRIPT_JUSTIFY {
  ...
  SCRIPT_JUSTIFY_ARABIC_SEEN =
      14, // Highest priority: Initial shape of Seen(U+633) (end)
...
_Check_return_ HRESULT WINAPI ScriptShape(
    ...
    _Inout_updates_(1) SCRIPT_ANALYSIS
        *psa, // InOut Result of ScriptItemize (may have fNoGlyphIndex set)
    _Out_writes_to_(cMaxGlyphs, *pcGlyphs)
...
//      appropriately. If the last parameter is not null, GDI's ExtTextOutW
//      calls are routed to this function.
//
...
_Check_return_ HRESULT WINAPI ScriptJustify(
    _In_reads_(cGlyphs) const SCRIPT_VISATTR
        *psva, // In   Collected visual attributes for entire line
    _In_reads_(cGlyphs)
        const int *piAdvance, // In   Advance widths from ScriptPlace
    int cGlyphs,              // In   Size of all arrays
    int iDx,         // In   Desired width change, either increase or descrease
    int iMinKashida, // In   Minimum length of continuous kashida glyph to
                     // generate
    _Out_writes_all_(cGlyphs) int
        *piJustify); // Out  Updated advance widths to pass to ScriptTextOut
...
// t     -------------------------------- |
// -------------------------------------------------------- t     Caret move by
// character cluster  | LogClust or VISATTR.fClusterStart or LOGATTR.fCharStop
// t     Line breaking between characters | LogClust or VISATTR.fClusterStart or
// LOGATTR.fCharStop t     Caret move by word               | LOGATTR.fWordStop
...
//
// c     const SCRIPT_PROPERTIES **g_ppScriptProperties; // Array of pointers to
...
// c         // Use ScriptBreak to restrict the caret from entering clusters
// (for example). c     }
//
//
...
//      will generally need to apply futher processing to determine which
//      charset to use when requesting a font suitable for this run. For example
//      it determine that the run consists of multiple languages and split it up
...
_Check_return_ HRESULT WINAPI ScriptApplyLogicalWidth(
    ...
    _In_reads_(cChars) const WORD *pwLogClust, // In     Logical clusters
    _In_reads_(cGlyphs) const SCRIPT_VISATTR
        *psva, // In     Visual attributes from ScriptShape/Place
    _In_reads_(cGlyphs)
        const int *piAdvance, // In     Glyph advance widths from ScriptPlace
    _In_reads_(1) const SCRIPT_ANALYSIS
        *psa, // In     Script analysis from item attributes
    _Inout_updates_opt_(1)
        ABC *pABC, // InOut  Updated item ABC width (optional)
    _Out_writes_all_(cGlyphs) int
        *piJustify); // Out    Resulting glyph advance widths for ScriptTextOut

/////
// p     piDx: Pointer to an array of dx widths in logical order, one per
// codepoint.
//
...
//
// p     piAdvance: Pointer to an array of glyph advance widths from
// ScriptPlace.
//
...
//           This is suitable for passing to the piJustify parameter of
//           ScriptTextOut.
/////   ScriptGetCMap
//
...
#define SGCM_RTL                                                               \
  0x00000001 // Return mirrored glyph for mirrorable Unicode codepoints
...
//          glyph E_HANDLE - font or system does not support glyph indices
...
typedef struct {
  ...
  DWORD fNeedsWordBreaking
      : 1; // Requires ScriptBreak for word breaking information
  ...
  DWORD fPrivateUseArea
      : 1; // This item is from the Unicode range U+E000 through U+F8FF
  DWORD fNeedsCharacterJustify : 1; // Requires inter-character justification
  DWORD fInvalidGlyph
      : 1; // Invalid combinations generate glyph wgInvalid in the glyph buffer
  DWORD fInvalidLogAttr : 1;   // Invalid combinations are marked by fInvalid in
  ...
  DWORD fClusterSizeVaries
      : 1; // Measured cluster width depends on adjacent clusters
  DWORD fRejectInvalid : 1; // Invalid combinations should be rejected
} SCRIPT_PROPERTIES;
//
// p     langid: Language associated with this script. When a script is used for
// many languages,
//          langid id represents a default language. For example, Western script
//          is represented by LANG_ENGLISH although it is also used for French,
//          German, Spanish etc.
//
// p     fNumeric: Script contains numerics and characters used in conjunction
// with numerics
//          by the rules of the Unicode bidirectional algorithm. For example
//          dollar sign and period are classified as numeric when adjacent to or
//          in between digits.
//
// p     fComplex: Indicates a script that requires complex script handling. If
// fComplex is false
//          the script contains no combining characters and requires no
//          contextual shaping or reordering.
//
// p     fNeedsWordBreaking: A script, such as Thai, which requires algorithmic
// wordbreaking.
//          Use ScriptBreak to obtain a wordbreak points using the standard
//          system wordbreaker.
//
// p     fNeedsCaretInfo: A script, such as Thai and Indian, where the caret may
// not be placed
//          inside a cluster. To determine valid caret positions inspect the
//          fCharStop flag in the logical attributes returned by ScriptBreak, or
//          compare adjacent values in the pwLogClust array returned by
//          ScriptShape.
//
// p     bCharSet: Nominal charset associated with script. May be used in a
// logfont when creating
//          a font suitable for displaying this script. Note that for new
//          scripts where there is no charset defined, bCharSet may be
//          innapropriate and DEFAULT_CHARSET should be used instead - see the
//          description of fAmbiguousCharSet below.
//
// p     fControl: contains control characters.
//
// p     fPrivateUseArea: The Unicode range U+E000 through U+F8FF.
//
// p     fNeedsCharacterJustify: A script, such as Thai, where justification is
// conventionally
//          achieved by increasing the space between all letters, not just
//          between words.
//
// p     fInvalidGlyph: A script for which ScriptShape generates an invalid
// glyph
//          to represent invalid sequences. The glyph index of the invalid glyph
//          for a particular font may be obtained by calling
//          ScriptGetFontProperties.
//
...
//
// p     fRejectInvalid: A script, such as Thai, where invalid sequences
// conventionally
...
_Check_return_ HRESULT WINAPI ScriptGetProperties(
    _Outptr_result_buffer_(*piNumScripts) const SCRIPT_PROPERTIES **
        *ppSp, // Out  Receives pointer to table of pointers to properties
               // indexed by script
    _Out_ int *piNumScripts); // Out  Receives number of scripts (valid values
                              // are 0 through NumScripts-1)
...
typedef struct {
  ...
  WORD wgInvalid; // Glyph used for invalid character combinations (especially
                  // in Thai)
  WORD wgKashida; // Shortest continuous kashida glyph in the font, -1 if
                  // doesn't exist
...
//
#define SSA_PASSWORD                                                           \
  0x00000001 // Input string contains a single character to be duplicated
             // iLength times
...
#define SSA_DZWG                                                               \
  0x00000010 // Provide representation glyphs for control characters
...
#define SSA_GCP                                                                \
  0x00000200 // Return missing glyphs and LogCLust with GetCharacterPlacement
             // conventions
#define SSA_HOTKEY                                                             \
  0x00000400 // Replace '&' with underline on subsequent codepoint
#define SSA_METAFILE                                                           \
  0x00000800 // Write items with ExtTextOutW Unicode calls, not glyphs
#define SSA_LINK                                                               \
  0x00001000 // Apply FE font linking/association to non-complex text
...
#define SSA_FULLMEASURE                                                        \
  0x04000000 // Internal - calculate full width and out the number of chars can
             // fit in iReqWidth.
#define SSA_LPKANSIFALLBACK                                                    \
  0x08000000 // Internal - enable FallBack for all LPK Ansi calls Except BiDi
             // hDC calls
#define SSA_PIDX 0x10000000      // Internal
#define SSA_LAYOUTRTL 0x20000000 // Internal - Used when DC is mirrored
#define SSA_DONTGLYPH                                                          \
  0x40000000 // Internal - Used only by GDI during metafiling - Use ExtTextOutA
             // for positioning
#define SSA_NOKASHIDA                                                          \
  0x80000000 // Internal - Used by GCP to justify the non Arabic glyphs only.
//
//
...
/////   SCRIPT_TABDEF
//
//      Defines tabstop positions for ScriptStringAnalyse (ignored unless
//      SSA_TAB passed)
//
...
//      font the character string is treated as a single neutral
//      SCRIPT_UNDEFINED item.
//
...
_Check_return_ HRESULT WINAPI ScriptStringAnalyse(
    ...
    const BYTE *pbInClass, // In  Legacy GetCharacterPlacement character
                           // classifications (deprecated)
    _Outptr_result_buffer_(1)
...
_Check_return_ HRESULT WINAPI ScriptStringFree(_Inout_updates_(
    1) SCRIPT_STRING_ANALYSIS *pssa); // InOut Address of pointer to analysis
...
//
//      returns pointer to length of string after clipping (requires SSA_CLIP
//      set)
//
//      Note that the int pointer remains valid only until the
//      SCRIPT_STRING_ANALYSIS is passed to ScriptStringFree.

const int *WINAPI ScriptString_pcOutChars(_In_reads_(1)
...
const SCRIPT_LOGATTR *WINAPI
ScriptString_pLogAttr(_In_reads_(1) SCRIPT_STRING_ANALYSIS ssa);
...
ScriptStringGetOrder(_In_reads_(1) SCRIPT_STRING_ANALYSIS ssa, UINT *puOrder);
...
_Check_return_ HRESULT WINAPI
ScriptStringXtoCP(_In_reads_(1) SCRIPT_STRING_ANALYSIS ssa, // In
                  int iX,                                   // In
...
_Check_return_ HRESULT WINAPI ScriptStringGetLogicalWidths(
    _In_reads_(1) SCRIPT_STRING_ANALYSIS ssa, int *piDx);
...
_Check_return_ HRESULT WINAPI
ScriptStringValidate(_In_reads_(1) SCRIPT_STRING_ANALYSIS ssa);
...
#define SCRIPT_DIGITSUBSTITUTE_CONTEXT                                         \
  0                                   // Substitute to match preceeding letters
#define SCRIPT_DIGITSUBSTITUTE_NONE 1 // No substitution
#define SCRIPT_DIGITSUBSTITUTE_NATIONAL                                        \
  2 // Substitute with official national digits
#define SCRIPT_DIGITSUBSTITUTE_TRADITIONAL                                     \
  3 // Substitute with traditional digits of the locale
//
//
...
// (4 bytes now) pfCanGlyphAlone      - flag per character, indicate that char
...
_Check_return_ HRESULT WINAPI ScriptShapeOpenType(
    ...
    _In_reads_opt_(cRanges) int
        *rcRangeChars, // In    Array of number of characters per range
    _In_reads_opt_(cRanges) TEXTRANGE_PROPERTIES *
        *rpRangeProperties, // In    Array of range properties (for each range)
    ...
    _Out_writes_all_(cChars) WORD *pwLogClust, // Out   Logical clusters
    _Out_writes_all_(cChars) SCRIPT_CHARPROP
        *pCharProps, // Out   Output buffer for character properties
...
// New output parameters:
//
// Character information - pwcChars, pwLogClust, pCharProps are now passed to
// ScriptPlace pGlyphProps           - array of glyph properties, replaces
// visual attributes (4 bytes now)
//
_Check_return_ HRESULT WINAPI ScriptPlaceOpenType(
    ...
    _In_reads_(cChars) SCRIPT_CHARPROP
        *pCharProps, // In    Output buffer for character properties
    _In_ int cChars, // In    Length of unicode run
...
_Check_return_ HRESULT WINAPI ScriptItemizeOpenType(
    ...
    _In_opt_ const SCRIPT_CONTROL
        *psControl, // In   Analysis control (optional)
    _In_opt_ const SCRIPT_STATE
        *psState, // In   Initial bidi algorithm state (optional)
    _Out_writes_to_(cMaxItems, *pcItems)
        SCRIPT_ITEM *pItems, // Out  Array to receive itemization
    _Out_writes_to_(cMaxItems, *pcItems) OPENTYPE_TAG
        *pScriptTags,    // Out  Array of script tags - parallel to items
...
// psa==NULL). Usually complex scripts return single tag and neutral (like
// digits) return all tags from the font
//
...
_Check_return_ HRESULT WINAPI ScriptGetFontScriptTags(
    _In_opt_ HDC hdc,          // In    Optional (see under caching)
    _Inout_ SCRIPT_CACHE *psc, // InOut Cache handle
    _In_opt_ SCRIPT_ANALYSIS
        *psa,          // In    Result of ScriptItemize (can be NULL)
    _In_ int cMaxTags, // In    Length of pScriptTags array
    _Out_writes_to_(cMaxTags, *pcTags)
...
//
// Returns list of language system tags supported by layout tables for
// particular script.
//
...
//
// Returns list of feature tags supported by layout tables for particular
// language system.
//
// This method will hide features that are language specific, because they are
// controlled by shaping engines and can not be switched on or off by the
// client. Example of such features are initial, medial and final forms for
// Arabic script
//
...
_Check_return_ HRESULT WINAPI ScriptGetFontAlternateGlyphs(
    ...
    _Out_writes_to_(cMaxAlternates, *pcAlternates)
