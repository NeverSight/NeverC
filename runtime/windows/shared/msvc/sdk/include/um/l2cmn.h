...
// If a notification needs to be private that is only consumed by internal
// components then it should be part of l2cmnpriv.h

// the types of notification
// This signifies what kind of notification is to be passed on , the
// 0X0000FFFF  signifies all public notifications , so all notfications that
// need to be public should be in this range and have a single bit set as these
...
// This structure is the notification structure which needs to be filled in by
// each component The interface guid is filled in only by the AC The
// NotificationSource signifies the type above and NotificationCode is one of
// the enum values
