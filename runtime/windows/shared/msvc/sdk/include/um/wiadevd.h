...
typedef struct tagDEVICEDIALOGDATA2 {
  ...
  LONG
      lNumFiles; // Number of items in ppbstrFilePaths array.  Filled on return.
  BSTR *pbstrFilePaths; // file names created after successful transfers.
  IWiaItem2 *pWiaItem;  // IWiaItem2 interface pointer.  This is the IWiaItem2
                        // used for transfer.
} DEVICEDIALOGDATA2, *LPDEVICEDIALOGDATA2, *PDEVICEDIALOGDATA2;

DECLARE_INTERFACE_IID_(IWiaUIExtension2, IUnknown,
  ...
  STDMETHOD(GetDeviceIcon)(_In_ THIS_ BSTR bstrDeviceId, _Out_ HICON * phIcon,
                           ULONG nSize) PURE;
};

// {305600d7-5088-46d7-9a15-b77b09cdba7a}
DEFINE_GUID(IID_IWiaUIExtension2, 0x305600D7, 0x5088, 0x46D7, 0x9A, 0x15, 0xB7,
            0x7B, 0x09, 0xCD, 0xBA, 0x7A);
...
typedef struct tagDEVICEDIALOGDATA {
  ...
  IWiaItem **ppWiaItems; // Array of IWiaItem interface pointers.  Array must
                         // be allocated using CoTaskMemAlloc, and all interface
                         // pointers must be AddRef'ed
...
// IWiaUIExtension provides a means to replace a device's image acquisition
// dialog and to provide custom icons and logo bitmaps to appear on the standard
// dialog
#undef INTERFACE
#define INTERFACE IWiaUIExtension
DECLARE_INTERFACE_IID_(IWiaUIExtension, IUnknown,
  ...
  STDMETHOD(GetDeviceIcon)(_In_ THIS_ BSTR bstrDeviceId, _Out_ HICON * phIcon,
                           ULONG nSize) PURE;
  STDMETHOD(GetDeviceBitmapLogo)(_In_ THIS_ BSTR bstrDeviceId,
                                 _Out_ HBITMAP * phBitmap, ULONG nMaxWidth,
