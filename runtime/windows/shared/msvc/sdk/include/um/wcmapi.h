...
typedef enum _WCM_PROPERTY {
  ...
  wcm_intf_property_connection_cost, // used to set/get cost level and flags for
                                     // the connection
  wcm_intf_property_dataplan_status, // used by MNO to indicate plan data
                                     // associated with new cost
  wcm_intf_property_hotspot_profile, // used to store hotspot profile (WISPr
                                     // credentials)
} WCM_PROPERTY,
    *PWCM_PROPERTY;
...
DWORD WINAPI
WcmQueryProperty(_In_opt_ const GUID *pInterface, // NULL for global properties
                 _In_opt_ LPCWSTR strProfileName, _In_ WCM_PROPERTY Property,
                 _Reserved_ void *pReserved, _Out_ PDWORD pdwDataSize,
                 _Outptr_result_buffer_maybenull_(*pdwDataSize) PBYTE *ppData);

DWORD WINAPI
WcmSetProperty(_In_opt_ const GUID *pInterface, // NULL for global properties
               _In_opt_ LPCWSTR strProfileName, _In_ WCM_PROPERTY Property,
               _Reserved_ void *pReserved, _In_ DWORD dwDataSize,
               _In_reads_opt_(dwDataSize) const BYTE *pbData);

DWORD WINAPI WcmGetProfileList(_Reserved_ void *pReserved,
                               _Outptr_ WCM_PROFILE_INFO_LIST **ppProfileList);

// Ethernet pseudo-profiles should not be included in pProfileList
DWORD WINAPI WcmSetProfileList(_In_ WCM_PROFILE_INFO_LIST *pProfileList,
