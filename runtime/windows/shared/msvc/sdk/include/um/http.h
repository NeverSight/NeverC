...
typedef enum _HTTP_SERVICE_CONFIG_SETTING_KEY {
  ...
  HttpTlsThrottle
    ...
    *PHTTP_SERVICE_CONFIG_SETTING_KEY;
...
#define HTTP_AUTH_ENABLE_ALL                                                   \
  (HTTP_AUTH_ENABLE_BASIC | HTTP_AUTH_ENABLE_DIGEST | HTTP_AUTH_ENABLE_NTLM |  \
   HTTP_AUTH_ENABLE_NEGOTIATE | HTTP_AUTH_ENABLE_KERBEROS)
...
//
// Used with HttpServerRequestInfoProperty. These flags will identify optional
// request info fields not normally supplied; these fields are only populated
// on-demand as they are less commonly requested.
//
...
typedef enum _HTTP_CREATE_REQUEST_QUEUE_PROPERTY_ID {
  ...
  CreateRequestQueueMax
    ...
    *PHTTP_CREATE_REQUEST_QUEUE_PROPERTY_ID;
...
typedef struct _HTTP_CREATE_REQUEST_QUEUE_PROPERTY_INFO {
  ...
  PVOID PropertyInfo;
    ...
    *PHTTP_CREATE_REQUEST_QUEUE_PROPERTY_INFO;
...
//
// List of possible sizes for which information will be retured in
...
} HTTP_REQUEST_TRANSPORT_IDLE_CONNECTION_TIMEOUT_INFO,
    *PHTTP_REQUEST_TRANSPORT_IDLE_CONNECTION_TIMEOUT_INFO;
...
//                                             the old flag.
//                                             It has been used first, but the
//                                             flag was split into request and
//                                             response side. This old flag has
...
#define HTTP_REQUEST_FLAG_FAST_FORWARDING_RESPONSE_ALLOWED                     \
  HTTP_REQUEST_FLAG_FAST_FORWARDING_ALLOWED
...
typedef enum _HTTP_SSL_SERVICE_CONFIG_EX_PARAM_TYPE {
  ...
  ExParamTypeMax
    ...
    *PHTTP_SSL_SERVICE_CONFIG_EX_PARAM_TYPE;
...
//
// Input types for HttpQueryRequestProperty. Only types are public and not the
// API so that IIS need not have their own types for public usage.
//
...
// - HTTP_REQUEST_PROPERTY_SNI_FLAG_SNI_USED: Indicates that SNI was used for
// succesful
//   endpoint lookup during handshake. If client sent the SNI but Http.sys still
//   decided to use IP endpoint binding then this flag will not be set.
// - HTTP_REQUEST_PROPERTY_SNI_FLAG_NO_SNI: Indicates that client did not send
// the SNI.
//   If this flag is set, HTTP_REQUEST_PROPERTY_SNI_FLAG_SNI_USED can not be
//   set.
//
...
ULONG
WINAPI
HttpInitialize(IN HTTPAPI_VERSION Version, IN ULONG Flags,
...
ULONG
WINAPI
HttpCreateRequestQueue(IN HTTPAPI_VERSION Version, IN PCWSTR Name OPTIONAL,
                       IN PSECURITY_ATTRIBUTES SecurityAttributes OPTIONAL,
                       IN ULONG Flags OPTIONAL, OUT PHANDLE RequestQueueHandle);
...
HttpSetRequestQueueProperty(_In_ HANDLE RequestQueueHandle,
                            ...
                            _In_reads_bytes_(PropertyInformationLength)
...
HttpQueryRequestQueueProperty(
    _In_ HANDLE RequestQueueHandle, _In_ HTTP_SERVER_PROPERTY Property,
    _Out_writes_bytes_to_opt_(PropertyInformationLength, *ReturnLength)
        PVOID PropertyInformation,
    _In_ ULONG PropertyInformationLength, _Reserved_ ULONG Reserved1,
    _Out_opt_ PULONG ReturnLength, _Reserved_ PVOID Reserved2);

HTTPAPI_LINKAGE
ULONG
WINAPI
HttpSetRequestProperty(_In_ HANDLE RequestQueueHandle, _In_ HTTP_OPAQUE_ID Id,
...
HttpQueryRequestProperty(
    _In_ HANDLE RequestQueueHandle, _In_ HTTP_OPAQUE_ID Id,
    _In_ HTTP_REQUEST_PROPERTY PropertyId,
    ...
    _In_ ULONG OutputBufferSize, _Out_opt_ PULONG BytesReturned,
...
HttpReceiveClientCertificate(IN HANDLE RequestQueueHandle,
                             IN HTTP_CONNECTION_ID ConnectionId, IN ULONG Flags,
                             _Out_writes_bytes_to_(SslClientCertInfoSize,
                                                   *BytesReceived)
...
HTTPAPI_LINKAGE
_Success_(return == NO_ERROR) ULONG WINAPI HttpQueryServerSessionProperty(
    IN HTTP_SERVER_SESSION_ID ServerSessionId, IN HTTP_SERVER_PROPERTY Property,
    _Out_writes_bytes_to_opt_(PropertyInformationLength, *ReturnLength)
        PVOID PropertyInformation,
    IN ULONG PropertyInformationLength, _Out_opt_ PULONG ReturnLength);
...
ULONG
WINAPI
HttpAddUrl(IN HANDLE RequestQueueHandle, IN PCWSTR FullyQualifiedUrl,
           _Reserved_ PVOID Reserved);

HTTPAPI_LINKAGE
ULONG
WINAPI
HttpRemoveUrl(IN HANDLE RequestQueueHandle, IN PCWSTR FullyQualifiedUrl);
...
HttpRemoveUrlFromUrlGroup(IN HTTP_URL_GROUP_ID UrlGroupId,
                          IN PCWSTR pFullyQualifiedUrl, IN ULONG Flags);
...
_Success_(return == NO_ERROR) ULONG WINAPI HttpQueryUrlGroupProperty(
    IN HTTP_URL_GROUP_ID UrlGroupId, IN HTTP_SERVER_PROPERTY Property,
    _Out_writes_bytes_to_opt_(PropertyInformationLength, *ReturnLength)
        PVOID PropertyInformation,
    IN ULONG PropertyInformationLength, _Out_opt_ PULONG ReturnLength);
...
ULONG
WINAPI
HttpPrepareUrl(_Reserved_ PVOID Reserved, _Reserved_ ULONG Flags,
               _In_ PCWSTR Url, _Outptr_ PWSTR *PreparedUrl);
...
HttpReceiveHttpRequest(
    IN HANDLE RequestQueueHandle, IN HTTP_REQUEST_ID RequestId, IN ULONG Flags,
    _Out_writes_bytes_to_(RequestBufferLength, *BytesReturned)
        PHTTP_REQUEST RequestBuffer,
    IN ULONG RequestBufferLength, _Out_opt_ PULONG BytesReturned,
    IN LPOVERLAPPED Overlapped OPTIONAL);
...
HttpReceiveRequestEntityBody(
    IN HANDLE RequestQueueHandle, IN HTTP_REQUEST_ID RequestId, IN ULONG Flags,
    _Out_writes_bytes_to_(EntityBufferLength, *BytesReturned)
        PVOID EntityBuffer,
    IN ULONG EntityBufferLength, _Out_opt_ PULONG BytesReturned,
    IN LPOVERLAPPED Overlapped OPTIONAL);
...
ULONG
WINAPI
HttpSendHttpResponse(IN HANDLE RequestQueueHandle, IN HTTP_REQUEST_ID RequestId,
                     IN ULONG Flags, IN PHTTP_RESPONSE HttpResponse,
                     IN PHTTP_CACHE_POLICY CachePolicy OPTIONAL,
                     OUT PULONG BytesSent OPTIONAL, _Reserved_ PVOID Reserved1,
...
ULONG
WINAPI
HttpSendHttpResponse(IN HANDLE ReqQueueHandle, IN HTTP_REQUEST_ID RequestId,
                     IN ULONG Flags, IN PHTTP_RESPONSE pHttpResponse,
...
HttpSendResponseEntityBody(
    ...
    OUT PULONG BytesSent OPTIONAL, _Reserved_ PVOID Reserved1 OPTIONAL,
    _Reserved_ ULONG Reserved2 OPTIONAL, IN LPOVERLAPPED Overlapped OPTIONAL,
    IN PHTTP_LOG_DATA LogData OPTIONAL);
...
HttpSendResponseEntityBody(IN HANDLE ReqQueueHandle,
                           IN HTTP_REQUEST_ID RequestId, IN ULONG Flags,
                           IN USHORT EntityChunkCount OPTIONAL,
...
ULONG
WINAPI
HttpDeclarePush(_In_ HANDLE RequestQueueHandle, _In_ HTTP_REQUEST_ID RequestId,
                _In_ HTTP_VERB Verb, _In_ PCWSTR Path, _In_opt_ PCSTR Query,
...
BOOL WINAPI HttpIsFeatureSupported(_In_ HTTP_FEATURE_ID FeatureId);

HTTPAPI_LINKAGE
ULONG
WINAPI
HttpDelegateRequestEx(
    _In_ HANDLE RequestQueueHandle, _In_ HANDLE DelegateQueueHandle,
    _In_ HTTP_REQUEST_ID RequestId, _In_ HTTP_URL_GROUP_ID DelegateUrlGroupId,
...
ULONG
WINAPI
HttpFindUrlGroupId(_In_ PCWSTR FullyQualifiedUrl,
...
ULONG
WINAPI
HttpFlushResponseCache(IN HANDLE RequestQueueHandle, IN PCWSTR UrlPrefix,
                       IN ULONG Flags, IN LPOVERLAPPED Overlapped OPTIONAL);

HTTPAPI_LINKAGE
ULONG
WINAPI
HttpAddFragmentToCache(IN HANDLE RequestQueueHandle, IN PCWSTR UrlPrefix,
...
ULONG
WINAPI
HttpReadFragmentFromCache(IN HANDLE RequestQueueHandle, IN PCWSTR UrlPrefix,
                          IN PHTTP_BYTE_RANGE ByteRange OPTIONAL,
                          _Out_writes_bytes_to_(BufferLength, *BytesRead)
                              PVOID Buffer,
                          IN ULONG BufferLength, _Out_opt_ PULONG BytesRead,
                          IN LPOVERLAPPED Overlapped OPTIONAL);
...
ULONG
WINAPI
HttpSetServiceConfiguration(_Reserved_ HANDLE ServiceHandle,
                            _In_ HTTP_SERVICE_CONFIG_ID ConfigId,
                            _In_reads_bytes_(ConfigInformationLength)
...
ULONG
WINAPI
HttpUpdateServiceConfiguration(_Reserved_ HANDLE Handle,
                               _In_ HTTP_SERVICE_CONFIG_ID ConfigId,
                               _In_reads_bytes_(ConfigInfoLength)
                                   PVOID ConfigInfo,
...
ULONG
WINAPI
HttpDeleteServiceConfiguration(_Reserved_ HANDLE ServiceHandle,
                               _In_ HTTP_SERVICE_CONFIG_ID ConfigId,
                               _In_reads_bytes_(ConfigInformationLength)
                                   PVOID pConfigInformation,
                               _In_ ULONG ConfigInformationLength,
...
HttpQueryServiceConfiguration(
    _Reserved_ HANDLE ServiceHandle, _In_ HTTP_SERVICE_CONFIG_ID ConfigId,
    _In_reads_bytes_opt_(InputLength) PVOID pInput, _In_ ULONG InputLength,
    _Out_writes_bytes_to_opt_(OutputLength, *pReturnLength) PVOID pOutput,
    _In_ ULONG OutputLength, _Out_opt_ PULONG pReturnLength,
    _Reserved_ LPOVERLAPPED pOverlapped);

ULONG
WINAPI
HttpGetExtension(__in HTTPAPI_VERSION Version, __in ULONG Extension,
