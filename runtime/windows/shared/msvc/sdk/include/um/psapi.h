...
#define LIST_MODULES_DEFAULT                                                   \
  0x0 // This is the default one app would get without any flag.
#define LIST_MODULES_32BIT 0x01 // list 32bit modules in the target process.
#define LIST_MODULES_64BIT                                                     \
  0x02 // list all 64bit modules. 32bit exe will be stripped off.

// list all the modules
#define LIST_MODULES_ALL (LIST_MODULES_32BIT | LIST_MODULES_64BIT)

//
// Give teams a choice of using a downlevel version of psapi.h for an OS
// versions. Teams can set C_DEFINES=$(C_DEFINES) -DPSAPI_VERSION=1 for
// downlevel psapi on windows 7 and higher.  We found that test code needs this
// capability.
//
...
BOOL WINAPI EnumProcessModules(_In_ HANDLE hProcess,
                               _Out_writes_bytes_(cb) HMODULE *lphModule,
                               _In_ DWORD cb, _Out_ LPDWORD lpcbNeeded);

BOOL WINAPI EnumProcessModulesEx(_In_ HANDLE hProcess,
                                 _Out_writes_bytes_(cb) HMODULE *lphModule,
                                 _In_ DWORD cb, _Out_ LPDWORD lpcbNeeded,
...
DWORD
WINAPI
GetModuleBaseNameA(_In_ HANDLE hProcess, _In_opt_ HMODULE hModule,
                   _Out_writes_(nSize) LPSTR lpBaseName, _In_ DWORD nSize);

DWORD
WINAPI
GetModuleBaseNameW(_In_ HANDLE hProcess, _In_opt_ HMODULE hModule,
                   _Out_writes_(nSize) LPWSTR lpBaseName, _In_ DWORD nSize);
...
_Success_(return != 0) _Ret_range_(1, nSize) DWORD WINAPI GetModuleFileNameExA(
    _In_opt_ HANDLE hProcess, _In_opt_ HMODULE hModule,
    _When_(return < nSize, _Out_writes_to_(nSize, return +1))
        _When_(return == nSize, _Out_writes_all_(nSize)) LPSTR lpFilename,
    _In_ DWORD nSize);

_Success_(return != 0) _Ret_range_(1, nSize) DWORD WINAPI GetModuleFileNameExW(
    _In_opt_ HANDLE hProcess, _In_opt_ HMODULE hModule,
    _When_(return < nSize, _Out_writes_to_(nSize, return +1))
...
BOOL WINAPI GetModuleInformation(_In_ HANDLE hProcess, _In_ HMODULE hModule,
                                 _Out_ LPMODULEINFO lpmodinfo, _In_ DWORD cb);
...
BOOL WINAPI GetWsChanges(_In_ HANDLE hProcess,
                         _Out_writes_bytes_(cb)
                             PPSAPI_WS_WATCH_INFORMATION lpWatchInfo,
                         _In_ DWORD cb);

BOOL WINAPI GetWsChangesEx(_In_ HANDLE hProcess,
...
DWORD
WINAPI
GetMappedFileNameW(_In_ HANDLE hProcess, _In_ LPVOID lpv,
                   _Out_writes_(nSize) LPWSTR lpFilename, _In_ DWORD nSize);

DWORD
WINAPI
GetMappedFileNameA(_In_ HANDLE hProcess, _In_ LPVOID lpv,
                   _Out_writes_(nSize) LPSTR lpFilename, _In_ DWORD nSize);
...
BOOL WINAPI EnumDeviceDrivers(_Out_writes_bytes_(cb) LPVOID *lpImageBase,
                              _In_ DWORD cb, _Out_ LPDWORD lpcbNeeded);

DWORD
WINAPI
GetDeviceDriverBaseNameA(_In_ LPVOID ImageBase,
                         _Out_writes_(nSize) LPSTR lpFilename,
                         _In_ DWORD nSize);

DWORD
WINAPI
GetDeviceDriverBaseNameW(_In_ LPVOID ImageBase,
...
DWORD
WINAPI
GetDeviceDriverFileNameA(_In_ LPVOID ImageBase,
                         _Out_writes_(nSize) LPSTR lpFilename,
                         _In_ DWORD nSize);

DWORD
WINAPI
GetDeviceDriverFileNameW(_In_ LPVOID ImageBase,
...
BOOL WINAPI QueryWorkingSet(_In_ HANDLE hProcess,
                            _Out_writes_bytes_(cb) PVOID pv, _In_ DWORD cb);

BOOL WINAPI QueryWorkingSetEx(_In_ HANDLE hProcess,
                              _Out_writes_bytes_(cb) PVOID pv, _In_ DWORD cb);
...
BOOL WINAPI GetProcessMemoryInfo(_In_ HANDLE Process,
                                 _Out_writes_bytes_(cb)
...
DWORD
WINAPI
GetProcessImageFileNameA(_In_ HANDLE hProcess,
                         _Out_writes_(nSize) LPSTR lpImageFileName,
                         _In_ DWORD nSize);

DWORD
WINAPI
GetProcessImageFileNameW(_In_ HANDLE hProcess,
