...
#endif                                    // ifdef NEC_98
#define REGSTR_KEY_LOGCONFIG                                                   \
  TEXT("LogConfig") // child of enum\<enumerator>\<deviceid>\<instanceid>
...
#define REG_KEY_INSTDEV                                                        \
  TEXT("Installed") // child of hklm\class\classname (Win98-only)
...
#define REGSTR_VAL_DEVICEPATH                                                  \
  TEXT("DevicePath") // default search path for
                     // .INFs
#define REGSTR_VAL_SRCPATH                                                     \
  TEXT("SourcePath") // last source files path during setup.
#define REGSTR_VAL_DRIVERCACHEPATH                                             \
  TEXT("DriverCachePath") // location of driver cache

#define REGSTR_VAL_OLDWINDIR TEXT("OldWinDir") // old windows location
#define REGSTR_VAL_SETUPFLAGS                                                  \
  TEXT("SetupFlags") // flags that setup passes on after install.
...
#define REGSTR_VAL_OLDMSDOSVER                                                 \
  TEXT("OldMSDOSVer") // will be DOS ver < 7 (when Setup run)
#define REGSTR_VAL_FIRSTINSTALLDATETIME                                        \
  TEXT("FirstInstallDateTime") // will Win 95 install date-time
...
// Values under REGSTR_PATH_DRIVERSIGN_POLICY and
...
#define VPDF_DISABLERINGRESUME                                                 \
  0x00000010 // Don't let the modem wake the machine (APM 1.2 only)
#define VPDF_SHOWMULTIBATT                                                     \
  0x00000020 // Show all batteries checkbox in power control panel

//
// Entries under REGSTR_PATH_BUSINFORMATION
//
#define BIF_SHOWSIMILARDRIVERS                                                 \
  0x00000001 // Show similar drivers instead of all class drivers in UI.
#define BIF_RAWDEVICENEEDSDRIVER                                               \
  0x00000002 // RAW device needs a driver installed.
...
//
//      A sub-key under which a variable number of variable length structures
//      are stored.
//
...
//      set of patterns matches a pattern already in the boot record, VFAT will
//      not write a volume tracking serial number in the OEM_SerialNum field of
//      the boot record on the volume being mounted.
//
...
#define REGSTR_VAL_CDPREFETCH                                                  \
  TEXT("Prefetch") // Number of 2K cache sectors for prefetching
#define REGSTR_VAL_CDPREFETCHTAIL                                              \
  TEXT("PrefetchTail") // Number of LRU1 prefetch sectors
#define REGSTR_VAL_CDRAWCACHE                                                  \
  TEXT("RawCache") // Number of 2352-byte cache sectors
#define REGSTR_VAL_CDEXTERRORS                                                 \
  TEXT("ExtendedErrors")                       // Return extended error codes
#define REGSTR_VAL_CDSVDSENSE TEXT("SVDSense") // 0=PVD, 1=Kanji, 2=Unicode
#define REGSTR_VAL_CDSHOWVERSIONS                                              \
  TEXT("ShowVersions") // Show file version numbers
#define REGSTR_VAL_CDCOMPATNAMES                                               \
  TEXT("MSCDEXCompatNames") // Disable Numeric Tails on long file names
#define REGSTR_VAL_CDNOREADAHEAD                                               \
  TEXT("NoReadAhead") // Disable Read Ahead if set to 1
...
#define REGSTR_VAL_NOUSECLASS                                                  \
  TEXT("NoUseClass") // Don't include this class in PnP functions
#define REGSTR_VAL_NOINSTALLCLASS                                              \
  TEXT("NoInstallClass") // Don't include this class in New Device Wizard
#define REGSTR_VAL_NODISPLAYCLASS                                              \
  TEXT("NoDisplayClass") // Don't include this class in Device Manager
#define REGSTR_VAL_SILENTINSTALL                                               \
  TEXT("SilentInstall") // Always Silent Install devices of this class.
#define REGSTR_VAL_FSFILTERCLASS                                               \
  TEXT("FSFilterClass") // File System Filter class
...
#define REGSTR_VAL_PCMCIA_ALLOC                                                \
...
#define REGSTR_VAL_ISAPNP_RDP_OVERRIDE                                         \
  TEXT("RDPOverRide") // ReadDataPort OverRide
...
#define REGSTR_VAL_CARDSPECIFIC                                                \
  TEXT("CardSpecific")                         // Netcard specific info (WORD)
...
#define REGSTR_VAL_SCAN_ONLY_FIRST                                             \
  TEXT("ScanOnlyFirstDrive")                         // used with IDE driver
#define REGSTR_VAL_SHARE_IRQ TEXT("ForceIRQSharing") // used with IDE driver
#define REGSTR_VAL_NONSTANDARD_ATAPI                                           \
  TEXT("NonStandardATAPI") // used with IDE driver
#define REGSTR_VAL_IDE_FORCE_SERIALIZE                                         \
  TEXT("ForceSerialization")         // used with IDE driver
...
#define CONFIGFLAG_REMOVED                                                     \
  0x00000002 // Set if a present hardware enum device deleted
#define CONFIGFLAG_MANUAL_INSTALL                                              \
  0x00000004 // Set if the devnode was manually installed
...
#define CONFIGFLAG_FINISH_INSTALL                                              \
  0x00000400 // Complete install for devnode running 'raw'
#define CONFIGFLAG_NEEDS_FORCED_CONFIG                                         \
  0x00000800 // This devnode requires a forced config
#if defined(REMOTE_BOOT)
#define CONFIGFLAG_NETBOOT_CARD                                                \
  0x00001000 // This is the remote boot network card
#endif       // defined(REMOTE_BOOT)
#define CONFIGFLAG_PARTIAL_LOG_CONF                                            \
  0x00002000 // This device has a partial logconfig
#define CONFIGFLAG_SUPPRESS_SURPRISE                                           \
  0x00004000 // Set if unsafe removals should be ignored
#define CONFIGFLAG_VERIFY_HARDWARE                                             \
  0x00008000 // Set if hardware should be tested for logo failures
#define CONFIGFLAG_FINISHINSTALL_UI                                            \
  0x00010000 // Show the finish install wizard pages for the installed device.
#define CONFIGFLAG_FINISHINSTALL_ACTION                                        \
  0x00020000 // Call installer with DIF_FINISHINSTALL_ACTION in client context.
#define CONFIGFLAG_BOOT_DEVICE                                                 \
  0x00040000 // Configured devnode during boot phase
#define CONFIGFLAG_NEEDS_CLASS_CONFIG                                          \
  0x00080000 // Device needs additional class configuration to start
...
#define DMSTATEFLAG_APPLYTOALL                                                 \
  0x00000001 // Set if Apply To All check box is checked
...
#define REGSTR_VAL_PORTNAME                                                    \
  TEXT("PortName") // VCOMM uses this for it's port names
...
#define REGSTR_VAL_DONTLOADIFCONFLICT                                          \
  TEXT("DontLoadIfConflict") // SCSI specific
#define REGSTR_VAL_PORTSUBCLASS TEXT("PortSubClass")
#define REGSTR_VAL_NETCLEAN                                                    \
  TEXT("NetClean") // Driver required for NetClean boot
...
#define REGSTR_VAL_LOCATION_INFORMATION_OVERRIDE                               \
  TEXT("LocationInformationOverride") // value of DRV
...
#define REGSTR_VAL_SYSDMFUNC                                                   \
  TEXT("SysDMFunc")                        // The device installer DLL function
#define REGSTR_VAL_PRIVATE TEXT("Private") // The private library
#define REGSTR_VAL_PRIVATEFUNC                                                 \
  TEXT("PrivateFunc")                    // The private library function
#define REGSTR_VAL_DETECT TEXT("Detect") // The detection library
#define REGSTR_VAL_DETECTFUNC                                                  \
  TEXT("DetectFunc") // The detection library function
#define REGSTR_VAL_ASKFORCONFIG TEXT("AskForConfig") // The AskForConfig library
#define REGSTR_VAL_ASKFORCONFIGFUNC                                            \
  TEXT("AskForConfigFunc") // The AskForConfig library function
#define REGSTR_VAL_WAITFORUNDOCK                                               \
  TEXT("WaitForUndock") // The WaitForUndock library
#define REGSTR_VAL_WAITFORUNDOCKFUNC                                           \
  TEXT("WaitForUndockFunc") // The WaitForUndock library function
#define REGSTR_VAL_REMOVEROMOKAY                                               \
  TEXT("RemoveRomOkay") // The RemoveRomOkay library
#define REGSTR_VAL_REMOVEROMOKAYFUNC                                           \
  TEXT("RemoveRomOkayFunc") // The RemoveRomOkay library function
...
#define REGSTR_VAL_PROFILEFLAGS                                                \
  TEXT("ProfileFlags") // value of HKEY_CURRENT_CONFIG
...
#define REGSTR_VAL_INSTALLER                                                   \
  TEXT("Installer") // 16-bit class installer module/entry point
#define REGSTR_VAL_INSTALLER_32                                                \
  TEXT("Installer32") // 32-bit class installer module/entry point
#define REGSTR_VAL_INSICON TEXT("Icon") // value of class\name
#define REGSTR_VAL_ENUMPROPPAGES                                               \
  TEXT("EnumPropPages") // For Class/Device Properties (16-bit)
#define REGSTR_VAL_ENUMPROPPAGES_32                                            \
  TEXT("EnumPropPages32") // For Class/Device Properties (32-bit)
#define REGSTR_VAL_BASICPROPERTIES                                             \
  TEXT("BasicProperties") // For CPL basic Properties (16-bit)
#define REGSTR_VAL_BASICPROPERTIES_32                                          \
  TEXT("BasicProperties32") // For CPL basic Properties (32-bit)
#define REGSTR_VAL_COINSTALLERS_32                                             \
  TEXT("CoInstallers32") // Device-specific co-installer multi-sz list (32-bit)
#define REGSTR_VAL_PRIVATEPROBLEM                                              \
  TEXT("PrivateProblem") // For Handling Private Problems
...
//
#define REGSTR_VAL_PCICOPTIONS                                                 \
  TEXT("PCICOptions") // Binary DWORD.  IRQ mask in
...
// section where password providers are installed (each provider has subkey
// under this key)
...
// (following are values REG_DWORD, legal values 0 or 1, treat as TEXT("0") if
// value not present) policies under NETWORK key
#define REGSTR_VAL_NOFILESHARING                                               \
  TEXT("NoFileSharing") // TEXT("1") prevents server from loading
#define REGSTR_VAL_NOPRINTSHARING TEXT("NoPrintSharing")
#define REGSTR_VAL_NOFILESHARINGCTRL                                           \
  TEXT("NoFileSharingControl") // TEXT("1") removes sharing ui
#define REGSTR_VAL_NOPRINTSHARINGCTRL TEXT("NoPrintSharingControl")
#define REGSTR_VAL_HIDESHAREPWDS                                               \
  TEXT("HideSharePwds") // TEXT("1") hides share passwords with asterisks
#define REGSTR_VAL_DISABLEPWDCACHING                                           \
  TEXT("DisablePwdCaching") // TEXT("1") disables caching
#define REGSTR_VAL_ALPHANUMPWDS                                                \
  TEXT("AlphanumPwds") // TEXT("1") forces alphanumeric passwords
...
// (following are values REG_DWORD, legal values 0 or 1, treat as TEXT("1") if
// value not present) policies under Policies\SYSTEM key
...
//      Entries here for system dlls we need to version check in case
//      overwritten
...
//      Keys here for bad applications we want to warn the user about before
//      running
...
//
// Entries for values in uninstaller keys under REGSTR_PATH_REINSTALL
// \ instanceid
//
...
#define REGSTR_PATH_PCIIR                                                      \
  L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\Pnp\\PciIrqRouti" \
  L"ng"
...
// each value is an indication of post-processing to be done after files have
// been recovered LASTGOOD_OPERATION bits indicate the primary post-processing
// operation remaining bits may be used as flags (allocate flags from highest
// bits first) a value of 0 is the same as the value being omitted, ie, no post
// processing.
//
