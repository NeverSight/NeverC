...
// volume. It is also not allowed on any volume on a basic MBR disk that
// contains the boot, system or pagefile volume.
//
...
// MessageText:
//
// An active partition was detected on the selected disk, and it is not the
// active partition used to boot the currently running OS.
//
...
// 'PARTITION_BASIC_DATA_GUID', and is both preceeded and followed by a
// partition of type 'PARTITION_BASIC_DATA_GUID'.
//
...
// MessageText:
//
// The volume's driver information is corrupt. Operations on the volume will
// fail.
//
...
// MessageText:
//
// The operation cannot be completed because there is only one valid disk in the
// pack.
//
...
// MessageText:
//
// The Logical Disk Manangement database is full, no more volumes or disks may
// be configured.
//
...
// MessageText:
//
// The disk layout contains partitions which less than the minimum required
// size.
//
...
// configuration. Dynamic disks are not supported on clusters.
//
...
// MessageText:
//
// The operation is not allowed on a disk that is in the process of being
// cleaned.
//
...
// MessageText:
//
// Cannot delete a protected partition without the force protected parameter
// set.
//
...
//
#define VDS_S_ISCSI_SESSION_NOT_FOUND_PERSISTENT_LOGIN_REMOVED                 \
  ((HRESULT)0x00042800L)
...
// MessageText:
//
// The operation is not allowed on a disk that contains a hibernation file
// volume.
//
...
// MessageText:
//
// The target portal was not found. For calls to GetPathInfo(), the target
// portal is associated with the path end point.
//
...
// MessageText:
//
// The path returned for the port is invalid. Either it has an incorrect port
// type specified, or, the HBA port properties structure is NULL.
//
...
// MessageText:
//
// The specified disk or volume is managed by the Microsoft Failover Clustering
// component. The disk must be in cluster maintenance mode and the cluster
// resource status must be online to perform this operation.
//
...
// MessageText:
//
// The specified disk or volume is managed by Microsoft Failover Clustering. The
// disk must be removed from the cluster to perform this operation.
//
...
// MessageText:
//
// The requested operation cannot be performed on the object, because it is not
// in a state that permits it. Call Refresh and retry the operation.
//
...
//
//
// Clean is not allowed on the disk containing volume that backs your Windows
// boot volume.
//
...
//
//
// The specified shrink or extend size does not meet alignment requirements for
// the device.
//
