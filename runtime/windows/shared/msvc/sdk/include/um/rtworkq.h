...
typedef enum {
  ...
  RTWQ_WINDOW_WORKQUEUE =
      1, // Message loop that calls PeekMessage() / DispatchMessage()..
  RTWQ_MULTITHREADED_WORKQUEUE = 2, // multithreaded MTA
} RTWQ_WORKQUEUE_TYPE;

interface DECLSPEC_UUID("ac6b7889-0740-4d51-8619-905994a55cc6")
    DECLSPEC_NOVTABLE IRtwqAsyncResult : public IUnknown {
...
interface DECLSPEC_UUID("a27003cf-2354-4f2a-8d6a-ab7cff15437e")
    DECLSPEC_NOVTABLE IRtwqAsyncCallback : public IUnknown {
...
STDAPI RtwqLockSharedWorkQueue(_In_ PCWSTR usageClass, _In_ LONG basePriority,
                               _Inout_ DWORD *taskId, _Out_ DWORD *id);
...
STDAPI RtwqCreateAsyncResult(_In_ IUnknown *appObject,
                             _In_ IRtwqAsyncCallback *callback,
                             _In_ IUnknown *appState,
                             _Out_ IRtwqAsyncResult **asyncResult);
STDAPI RtwqInvokeCallback(IRtwqAsyncResult *result);

STDAPI RtwqLockPlatform();
STDAPI RtwqUnlockPlatform();

STDAPI RtwqRegisterPlatformWithMMCSS(_In_ PCWSTR usageClass,
                                     _Inout_ DWORD *taskId,
                                     _In_ LONG lPriority);
STDAPI RtwqUnregisterPlatformFromMMCSS();

STDAPI RtwqPutWorkItem(DWORD dwQueue, LONG lPriority,
                       _In_ IRtwqAsyncResult *result);

STDAPI RtwqPutWaitingWorkItem(HANDLE hEvent, LONG lPriority,
                              _In_ IRtwqAsyncResult *result,
                              _Out_opt_ RTWQWORKITEM_KEY *key);
STDAPI RtwqAllocateSerialWorkQueue(_In_ DWORD workQueueIdIn,
                                   _Out_ DWORD *workQueueIdOut);

STDAPI RtwqScheduleWorkItem(IRtwqAsyncResult *result, INT64 Timeout,
                            _Out_opt_ RTWQWORKITEM_KEY *key);

STDAPI RtwqAddPeriodicCallback(RTWQPERIODICCALLBACK Callback, IUnknown *context,
                               _Out_opt_ DWORD *key);
STDAPI RtwqRemovePeriodicCallback(DWORD dwKey);

STDAPI RtwqCancelWorkItem(RTWQWORKITEM_KEY Key);

STDAPI RtwqAllocateWorkQueue(_In_ RTWQ_WORKQUEUE_TYPE WorkQueueType,
                             _Out_ DWORD *workQueueId);

STDAPI RtwqBeginRegisterWorkQueueWithMMCSS(
    DWORD workQueueId, _In_ PCWSTR usageClass, DWORD dwTaskId, LONG lPriority,
    _In_ IRtwqAsyncCallback *doneCallback, _In_ IUnknown *doneState);
STDAPI
RtwqBeginUnregisterWorkQueueWithMMCSS(DWORD workQueueId,
                                      _In_ IRtwqAsyncCallback *doneCallback,
                                      _In_ IUnknown *doneState);
STDAPI RtwqEndRegisterWorkQueueWithMMCSS(_In_ IRtwqAsyncResult *result,
                                         _Out_ DWORD *taskId);

STDAPI RtwqGetWorkQueueMMCSSClass(DWORD workQueueId,
                                  _Out_writes_to_opt_(*usageClassLength,
                                                      *usageClassLength)
                                      PWSTR usageClass,
                                  _Inout_ DWORD *usageClassLength);
...
interface DECLSPEC_UUID("63d9255a-7ff1-4b61-8faf-ed6460dacf2b")
    DECLSPEC_NOVTABLE IRtwqPlatformEvents : public IUnknown {
...
STDAPI RtwqSetDeadline(DWORD workQueueId, LONGLONG deadlineInHNS,
                       _Out_ HANDLE *pRequest);
// TODO: switch to RS define once it exists (see: 5312604)
#if (WINVER >= _WIN32_WINNT_WIN10)
STDAPI RtwqSetDeadline2(DWORD workQueueId, LONGLONG deadlineInHNS,
                        LONGLONG preDeadlineInHNS, _Out_ HANDLE *pRequest);
