...
typedef HRESULT(WINAPI *PFN_Direct3DCreate9On12Ex)(
    UINT SDKVersion, D3D9ON12_ARGS *pOverrideList, UINT NumOverrideEntries,
    IDirect3D9Ex **ppOutputInterface);
HRESULT WINAPI Direct3DCreate9On12Ex(UINT SDKVersion,
...
typedef IDirect3D9 *(WINAPI *PFN_Direct3DCreate9On12)(
    UINT SDKVersion, D3D9ON12_ARGS *pOverrideList, UINT NumOverrideEntries);
IDirect3D9 *WINAPI Direct3DCreate9On12(UINT SDKVersion,
                                       D3D9ON12_ARGS *pOverrideList,
                                       UINT NumOverrideEntries);

/* IID_IDirect3DDevice9On12 */
/* {e7fda234-b589-4049-940d-8878977531c8} */
DEFINE_GUID(IID_IDirect3DDevice9On12, 0xe7fda234, 0xb589, 0x4049, 0x94, 0x0d,
            0x88, 0x78, 0x97, 0x75, 0x31, 0xc8);

interface DECLSPEC_UUID("e7fda234-b589-4049-940d-8878977531c8")
...
DECLARE_INTERFACE_(IDirect3DDevice9On12, IUnknown) {
  ...
  STDMETHOD(UnwrapUnderlyingResource)(THIS_ IDirect3DResource9 * pResource,
                                      ID3D12CommandQueue * pCommandQueue,
                                      REFIID riid, void **ppvResource12) PURE;
  STDMETHOD(ReturnUnderlyingResource)(THIS_ IDirect3DResource9 * pResource,
                                      UINT NumSync, UINT64 * pSignalValues,
                                      ID3D12Fence * *ppFences) PURE;
};

typedef struct IDirect3DDevice9On12 *LPDIRECT3DDEVICE9ON12,
    *PDIRECT3DDEVICE9ON12;

#if !defined(__cplusplus) || defined(CINTERFACE)
#define IDirect3DDevice9On12_QueryInterface(p, a, b)                           \
  (p)->lpVtbl->QueryInterface(p, a, b)
#define IDirect3DDevice9On12_AddRef(p) (p)->lpVtbl->AddRef(p)
#define IDirect3DDevice9On12_Release(p) (p)->lpVtbl->Release(p)
#define IDirect3DDevice9On12_GetD3D12Device(p, a, b)                           \
  (p)->lpVtbl->GetD3D12Device(p, a, b)
#define IDirect3DDevice9On12_UnwrapUnderlyingResource(p, a, b, c, d)           \
  (p)->lpVtbl->UnwrapUnderlyingResource(p, a, b, c, d)
#define IDirect3DDevice9On12_ReturnUnderlyingResource(p, a, b, c, d)           \
...
#define IDirect3DDevice9On12_ReturnUnderlyingResource(p, a, b, c, d)           \
  (p)->ReturnUnderlyingResource(a, b, c, d)
#endif
