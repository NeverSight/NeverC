...
#define EAP_E_CERT_STORE_INACCESSIBLE                                          \
  0x80420010 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 0)

// Requested EAP-method is not installed
#define EAP_E_EAPHOST_METHOD_NOT_INSTALLED                                     \
  0x80420011 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 1)

//
#define EAP_E_EAPHOST_THIRDPARTY_METHOD_HOST_RESET                             \
  0x80420012 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 2)

// EAPHost not able to communicate with EAPQEC on a NAP enabled client
#define EAP_E_EAPHOST_EAPQEC_INACCESSIBLE                                      \
  0x80420013 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 3)

// EAPHost returns this error, if the authenticator fails the authentication
// after peer sent its identity
#define EAP_E_EAPHOST_IDENTITY_UNKNOWN                                         \
  0x80420014 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 4)

// EAPHost returns this error on authentication failure.
#define EAP_E_AUTHENTICATION_FAILED                                            \
  0x80420015 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 5)

// EAPHost returns this error, when the client & the server aren't configured
// with compatible EAP-types.
#define EAP_I_EAPHOST_EAP_NEGOTIATION_FAILED                                   \
  0x40420016 // (EAP_I_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 6)

// EAPMethod received an EAP packet that can not be processed
#define EAP_E_EAPHOST_METHOD_INVALID_PACKET                                    \
  0x80420017 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 7)

// EAPHost received a packet that can not be processed.
#define EAP_E_EAPHOST_REMOTE_INVALID_PACKET                                    \
  0x80420018 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 8)

// EAPHost ConfigSchema validation failed
#define EAP_E_EAPHOST_XML_MALFORMED                                            \
  0x80420019 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 9)

// Method doesn't support SSO for the provided config
#define EAP_E_METHOD_CONFIG_DOES_NOT_SUPPORT_SSO                               \
  0x8042001A // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + A)

// EAPHost returns this error, when a configured EAP-method does not support a
// requested operation (procedure call)
#define EAP_E_EAPHOST_METHOD_OPERATION_NOT_SUPPORTED                           \
  0x80420020 // (EAP_E_EAPHOST_FIRST + _EAP_GENERAL_FIRST + 10)
...
#define EAP_E_USER_CERT_NOT_FOUND                                              \
  0x80420100 // (EAP_E_USER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_NOT_FOUND)

// user-cert being user for authentication does not have proper usage (EKU) set
#define EAP_E_USER_CERT_INVALID                                                \
  0x80420101 // (EAP_E_USER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_INVALID)

// EAPhost found user-cert which has expired already
#define EAP_E_USER_CERT_EXPIRED                                                \
  0x80420102 // (EAP_E_USER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_EXPIRED)

// user-cert being used for authentication has been revoked
#define EAP_E_USER_CERT_REVOKED                                                \
  0x80420103 // (EAP_E_USER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_REVOKED)

// unknown error occured with user-cert being used for authentication
#define EAP_E_USER_CERT_OTHER_ERROR                                            \
  0x80420104 // (EAP_E_USER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_OTHER_ERROR)

// Authenticator rejected user-cert for authentication
#define EAP_E_USER_CERT_REJECTED                                               \
  0x80420105 // (EAP_E_USER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_REJECTED)

// Received EAP-Failure after Identity exchange:  There is likely a problem with
// the authenticating user's account.
#define EAP_I_USER_ACCOUNT_OTHER_ERROR                                         \
  0x40420110 // (EAP_I_USER_FIRST + _EAP_GENERAL_FIRST + 0

// Authenticator rejected user credentials for authentication
#define EAP_E_USER_CREDENTIALS_REJECTED                                        \
  0x80420111 // (EAP_E_USER_FIRST + _EAP_GENERAL_FIRST + 1)

// Authenticator rejected user credentials for authentication
#define EAP_E_USER_NAME_PASSWORD_REJECTED                                      \
  0x80420112 // (EAP_E_USER_FIRST + _EAP_GENERAL_FIRST + 2)

// No Smart Card Reader Present
#define EAP_E_NO_SMART_CARD_READER                                             \
  0x80420113 // (EAP_E_USER_FIRST + _EAP_GENERAL_FIRST + 3)
...
#define EAP_E_SERVER_CERT_NOT_FOUND                                            \
  0x80420200 // (EAP_E_SERVER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_NOT_FOUND)

// server-cert being user for authentication does not have proper usage (EKU)
// set
#define EAP_E_SERVER_CERT_INVALID                                              \
  0x80420201 // (EAP_E_SERVER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_INVALID)

// EAPhost found server-cert which has expired already
#define EAP_E_SERVER_CERT_EXPIRED                                              \
  0x80420202 // (EAP_E_SERVER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_EXPIRED)

// server-cert being used for authentication has been revoked
#define EAP_E_SERVER_CERT_REVOKED                                              \
  0x80420203 // (EAP_E_SERVER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_REVOKED)

// unknown error occured with server-cert being used for authentication
#define EAP_E_SERVER_CERT_OTHER_ERROR                                          \
  0x80420204 // (EAP_E_SERVER_FIRST + _EAP_CERT_FIRST + _EAP_CERT_OTHER_ERROR)

#define EAP_E_USER_ROOT_CERT_FIRST 0x80420300L
#define EAP_E_USER_ROOT_CERT_LAST 0x804203FFL

// EAPHost could not find a certificate in trusted-root cert-store for user cert
// velidation
#define EAP_E_USER_ROOT_CERT_NOT_FOUND                                         \
  0x80420300 // (EAP_E_USER_ROOT_CERT_FIRST + _EAP_CERT_FIRST +
             // _EAP_CERT_NOT_FOUND)

// The authentication failed because the root certificate used for this network
// is invalid
#define EAP_E_USER_ROOT_CERT_INVALID                                           \
  0x80420301 // (EAP_E_USER_ROOT_CERT_FIRST + _EAP_CERT_FIRST +
             // _EAP_CERT_INVALID)

// Trusted root certificate needed for user-cert validation has been expired.
#define EAP_E_USER_ROOT_CERT_EXPIRED                                           \
  0x80420302 // (EAP_E_USER_ROOT_CERT_FIRST + _EAP_CERT_FIRST +
             // _EAP_CERT_EXPIRED)

#define EAP_E_SERVER_ROOT_CERT_FIRST 0x80420400L
#define EAP_E_SERVER_ROOT_CERT_LAST 0x804204FFL

// EAPHost could not find a Root certificate in trusted-root cert-store for
// server cert velidation
#define EAP_E_SERVER_ROOT_CERT_NOT_FOUND                                       \
  0x80420400 // (EAP_E_SERVER_ROOT_CERT_FIRST + _EAP_CERT_FIRST +
             // _EAP_CERT_NOT_FOUND)

// The authentication failed because the server certificate required for this
// network on the server computer is invalid
#define EAP_E_SERVER_ROOT_CERT_INVALID                                         \
  0x80420401 // (EAP_E_SERVER_ROOT_CERT_FIRST + _EAP_CERT_FIRST +
             // _EAP_CERT_INVALID)

// The authentication failed because the certificate on the server computer does
// not have a server name specified
#define EAP_E_SERVER_ROOT_CERT_NAME_REQUIRED                                   \
...
EXTERN_C const CLSID DECLSPEC_SELECTANY
...
EXTERN_C const CLSID DECLSPEC_SELECTANY GUID_EapHost_Help_ObtainingCerts = {
    ...
    0x1bdd,
    0x46ca,
    {0xa2, 0xfc, 0xa6, 0x65, 0x59, 0x39, 0xb7, 0xe8}};

// This currently is a generic networking troubleshooting help page, not EAP
// specific.
EXTERN_C const CLSID DECLSPEC_SELECTANY GUID_EapHost_Help_Troubleshooting = {
    0x33307acf,
    0x0698,
    0x41ba,
    {0xb0, 0x14, 0xea, 0x0a, 0x2e, 0xb8, 0xd0, 0xa8}};
...
EXTERN_C const CLSID DECLSPEC_SELECTANY
    GUID_EapHost_Cause_Method_Config_Does_Not_Support_Sso = {
        ...
        0x004F,
        0x41fa,
        {0xae, 0x08, 0x0b, 0xc8, 0x5e, 0x58, 0x45, 0xac}};
