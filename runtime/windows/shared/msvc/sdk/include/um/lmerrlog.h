...
NET_API_STATUS NET_API_FUNCTION NetErrorLogClear(_In_opt_ LPCWSTR UncServerName,
                                                 _In_opt_ LPCWSTR BackupFile,
                                                 _In_opt_ LPBYTE Reserved);

NET_API_STATUS NET_API_FUNCTION NetErrorLogRead(
    _In_opt_ LPCWSTR UncServerName, _In_opt_ LPWSTR Reserved1,
    _In_ LPHLOG ErrorLogHandle, _In_ DWORD Offset, _In_opt_ LPDWORD Reserved2,
    _In_ DWORD Reserved3, _In_ DWORD OffsetFlag, _Outptr_ LPBYTE *BufPtr,
    _In_ DWORD PrefMaxSize, _Out_ LPDWORD BytesRead,
    _Out_ LPDWORD TotalAvailable);

NET_API_STATUS NET_API_FUNCTION NetErrorLogWrite(
    _In_opt_ LPBYTE Reserved1, _In_ DWORD Code, _In_ LPCWSTR Component,
    _In_ LPBYTE Buffer, _In_ DWORD NumBytes, _In_ LPBYTE MsgBuf,
    _In_ DWORD StrCount, _In_opt_ LPBYTE Reserved2);
...
/* The network %1 ran out of network control blocks (NCBs).  You may need to
 * increase NCBs for this network.  The following information includes the
 ...
 *  occurrences of a network control block (NCB) error.  The last bad NCB
 * follows in raw data.
 */
...
/*
 * The Windows NT or Windows 2000 %1 logon request for the user %2\%3 from %4
 * (via %5) failed.
 */
...
/*
 * The session setup to the Windows Domain Controller %1 for the domain %2
 * failed because the computer %3 does not have a local security database
 * account.
 */
 ...
 *
 * %n%nADDITIONAL DATA
 * %nIf this computer is a member of or a Domain Controller in the specified
 * domain, the aforementioned account is a computer account for this computer in
 * the specified domain. Otherwise, the account is an interdomain trust account
 ...
 * location if appropriate or otherwise ensure connectivity to a domain
 * controller capable of servicing the request (for example a writable domain
 * controller).
 *
 ...
 *
 * %n%nIf '%1' is a Domain Controller, then the trust associated with '%2'
 * should be deleted.
 *
 * %n%nIf '%1' is not a Domain Controller, it should be disjoined from the
 * domain.
 */
...
/*
 * Replication of the %1 Trusted Domain Object "%2" from primary domain
 ...
 * Use the Active Directory Users and Computers tool or Server Manager to remove
 * the BDC account for %2.
 */
 ...
 * %n%nUSER ACTION
 *
 * %nConfigure the DNS server to allow dynamic DNS updates or manually add the
...
/*
 * The dynamic registration of the DNS record '%1' failed on the following DNS
 * server:
 *
 ...
 *
 * %n%nFor computers and users to locate this domain controller, this record
 ...
 * the command prompt on the domain controller or restart Net Logon service. %n
...
/*
 * The dynamic deletion of the DNS record '%1' failed on the following DNS
 * server:
 *
 * %n%nDNS server IP address: %3
 * %nReturned Response Code (RCODE): %4
 ...
 * %n%nUSER ACTION
 *
 * %nTo prevent remote computers from connecting unnecessarily to the domain
 ...
 * DNS domain '%1' failed.  These records are used by other computers to locate
 ...
 *
 * %n- TCP/IP properties of the network connections of this computer contain
 * wrong IP address(es) of the preferred and alternate DNS servers %n- Specified
 * preferred and alternate DNS servers are not running %n- DNS server(s) primary
 * for the records to be registered is not running %n- Preferred or alternate
 * DNS servers are configured with wrong root hints %n- Parent DNS zone contains
 * incorrect delegation to the child zone authoritative for the DNS records that
 * failed registration
 *
 * %n%nUSER ACTION
 *
 * %nFix possible misconfiguration(s) specified above and initiate registration
 * or deletion of the DNS records by running 'nltest.exe /dsregdns' from the
 * command prompt on the domain controller or by restarting Net Logon service on
 * the domain controller.
 */

#define NELOG_NetlogonDynamicDnsFailure (ERRLOG2_BASE + 82)
/*
 * Dynamic registration or deregistration of one or more DNS records failed with
 * the following error: %n%1
 */

#define NELOG_NetlogonRpcCallCancelled (ERRLOG2_BASE + 83)
/*
 * The session setup to the Windows Domain Controller %1 for the domain %2
 * is not responsive.  The current RPC call from Netlogon on \\%3 to %1 has been
 * cancelled.
 */
...
/*
 * This Domain Controller no longer automatically covers site '%1' for domain
 * '%2'.
 */
...
/*
 * This Global Catalog server no longer automatically covers site '%1' for
 * forest '%2'.
 */
...
/*
 * This LDAP server no longer automatically covers site '%1' for non-domain NC
 * '%2'.
 */
...
/*
 * This Domain Controller no longer automatically covers site '%1' for domain
 * '%2'. However, site '%1' is still (manually) covered by this Domain
 * Controller for domain '%2' since this site has been manually configured in
 * the registry.
 */
...
/*
 * This Global Catalog server no longer automatically covers site '%1' for
 * forest '%2'. However, site '%1' is still (manually) covered by this Global
 * catalog for forest '%2' since this site has been manually configured in the
 * registry.
 */
...
/*
 * This LDAP server no longer automatically covers site '%1' for non-domain NC
 * '%2'. However, site '%1' is still (manually) covered by this LDAP server for
 * non-domain NC '%2' since this site has been manually configured in the
 * registry.
 */

#define NELOG_NetlogonSpnMultipleSamAccountNames (ERRLOG2_BASE + 100)
/*
 * Attempt to update DnsHostName and HOST Service Principal Name (SPN)
 * attributes of the computer object in Active Directory failed because the
 * Domain Controller
 * '%1' had more than one account with the name '%2' corresponding to this
 * computer. Not having SPNs registered may result in authentication failures
 * for this computer. Contact your domain administrator who may need to manually
 * resolve the account name collision.
 */

#define NELOG_NetlogonSpnCrackNamesFailure (ERRLOG2_BASE + 101)
/*
 * Attempt to update DnsHostName and HOST Service Principal Name (SPN)
 * attributes of the computer object in Active Directory failed because this
 * computer account name, '%2' could not be mapped to the computer object on
 * Domain Controller '%1'. Not having SPNs registered may result in
 * authentication failures for this computer. Contact your domain administrator.
 * The following technical information may be useful for the resolution of this
 * failure:%n DsCrackNames status = 0x%3, crack error = 0x%4.
 */

#define NELOG_NetlogonNoAddressToSiteMapping (ERRLOG2_BASE + 102)
/*
 * None of the IP addresses (%2) of this Domain Controller map to the configured
 * site '%1'. While this may be a temporary situation due to IP address changes,
 * it is generally recommended that the IP address of the Domain Controller
 * (accessible to machines in its domain) maps to the Site which it services. If
 * the above list of IP addresses is stable, consider moving this server to a
 * site (or create one if it does not already exist) such that the above IP
 * address maps to the selected site. This may require the creation of a new
 * subnet object (whose range includes the above IP address) which maps to the
...
/*
 * The Netlogon %1 registry key contains an invalid value 0x%2 for parameter
 * '%3'. The minimum and maximum values allowed for this parameter are 0x%4 and
 * 0x%5, respectively. The value of 0x%6 has been assigned to this parameter.
 */
 ...
 * %n%nUSER ACTION
 *
 * %nReconfigure this domain controller to use dynamic DNS updates or manually
 * add the DNS records from the file '%SystemRoot%\System32\Config\Netlogon.dns'
 * to the DNS database.
 */
 ...
 * at the time of this domain controller demotion because the DNS
 * deregistrations took too long.
 *
 * %n%nUSER ACTION
 *
...
/*
 * The NetLogon service on this domain controller has been configured to use
 * port %1 for incoming RPC connections over TCP/IP from remote machines.
 * However, the following error occurred when Netlogon attempted to register
 * this port with the RPC endpoint mapper service: %n%2 %nThis will prevent the
 * NetLogon service on remote machines from connecting to this domain controller
 * over TCP/IP that may result in authentication problems.
 *
 * %n%nUSER ACTION
 *
 * %nThe specified port is configured via the Group Policy or via a registry
 * value 'DcTcpipPort' under the
 * 'HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Netlogon\Parameters'
 * registry key; the value configured through the Group Policy takes precedence.
 * If the port specified is in error, reset it to a correct value. You can also
 * remove this configuration for the port in which case the port will be
 * assigned dynamically by the endpoint mapper at the time the NetLogon service
 * on remote machines makes RPC connections to this domain controller. After the
 * misconfiguration is corrected, restart the NetLogon service on this machine
 * and verify that this event log no longer appears.
 */
 ...
 * correct behavior for applications running on member computers and servers
 * that rely on subnet-site mappings, dual-stack IPv4/IPv6 clients must have
 * both IPv4 and global IPv6 addresses mapped to the same site. If a partially
 * mapped client attempts to connect to this domain controller using its
 * unmapped IP address, its mapped address is used for the client's site
 * mapping.
 *
 * %n%nThe log files %SystemRoot%\debug\netlogon.log or
 * %SystemRoot%\debug\netlogon.bak contain the name, unmapped IP address and
 * mapped IP address for each partially mapped client. The log files may also
 * contain unrelated debugging information. To locate the information pertaining
 * to partial-subnet mappings, search for lines that contain the text
 * 'PARTIAL_CLIENT_SITE_MAPPING:'. The first word after this text is the client
 * name. Following the client name is the client's unmapped IP address (the IP
 * address that does not have a subnet-site mapping) and the client's mapped IP
 * address, which was used to return site information.
 *
 * %n%nUSER ACTION
 *
 * %nUse the Active Directory Sites and Services management console (MMC)
 * snap-in to add the subnet mapping for the unmapped IP addresses to the same
 * site being used by the mapped IP addresses. When adding site mappings for
 * IPv6 addresses, you should use global IPv6 addresses and not for instance
 * temporary, link-local or site-local IPv6 addresses.
 *
 * %n%nThe default maximum size of the log files is %3 bytes. The current
 * maximum size is %4 bytes. To set a different maximum size, create the
 * following registry DWORD value to specify the maximum size in bytes:
 *
...
/*
 * The dynamic registration of the DNS record '%1' for the remote domain
 * controller '%3' failed on the following DNS server:
 *
 * %n%nDNS server IP address: %4
 * %nReturned Response Code (RCODE): %5
 * %nReturned Status Code: %6
 *
 * %n%nFor computers and users to locate the domain controller '%3', this record
 * must be registered in DNS.
 *
 * %n%nUSER ACTION
 *
 * %nDetermine what might have caused this failure, resolve the problem, and
 * initiate registration of the DNS records by the domain controller '%3'. For
 * help with determining and resolving the problem, see Help and Support for
 * information about troubleshooting DNS. To initiate registration of the DNS
 * records by the domain controller '%3', run 'nltest.exe /dsregdns' from the
 * command prompt on the domain controller '%3' or restart the Net Logon service
 * on the domain controller '%3'. Nltest.exe is a command line tool that is
 * built into Windows Server. %n As a workaround, you can manually add this
 * record to DNS, but it is not recommended because you then must manually
 ...
 * %n%nUSER ACTION
 *
 * %nTo prevent remote computers from attempting to connect to the domain
 * controller '%3' using an invalid record, delete the record '%1' manually or
 * troubleshoot the root cause behind the dynamic deregistration failure. To
 * learn more about troubleshooting DNS, see Help and Support.
 *
 * %n%nADDITIONAL DATA
 * %nError Value: %2
 */
...
/*
 * The dynamic registration request for the DNS record '%1' has been rejected by
 * the remote domain controller '%2'. Error: '%3'
 *
 * %n%nFor computers and users to locate this domain controller, this record
 * must be registered in DNS. If the problem persists, please contact your
 * domain administrator.
 */

#define NELOG_NetlogonRejectedRemoteDynamicDnsDeregister (ERRLOG2_BASE + 114)
/*
 * The dynamic deregistration request of the DNS record '%1' has been rejected
 * by the remote domain controller '%2'. Error: '%3'
 *
 * %n%nTo prevent remote computers from connecting unnecessarily to this domain
 * controller, an administrator with sufficient privileges must manually delete
 * the record on the DNS server that hosts it.
 */

#define NELOG_NetlogonRemoteDynamicDnsUpdateRequestFailure (ERRLOG2_BASE + 115)
/*
 * The remoting of the dynamic update request for the local domain controller's
 * DNS records through a secure session has failed with error '%1'.
 *
 * %n%nFor other computers and member servers to locate this domain controller,
 * the appropriate records must be registered in DNS. On this domain controller,
 * look for events related to failure to set up a secure session to determine
 * why the request is failing. If the problem persists, please contact your
 * domain administrator.
 */

#define NELOG_NetlogonUserValidationReqInitialTimeOut (ERRLOG2_BASE + 116)
/*
 * Netlogon has failed an authentication request of account %1 in domain %2. The
 * request timed out before it could be sent to domain controller %3 in domain
 * %4. This is the first failure. If the problem continues, consolidated events
 * will be logged about every %5 minutes. Please see
...
/*
 * Netlogon has failed an additional %1 authentication requests in the last %2
 * minutes. The requests timed out before they could be sent to domain
 * controller %3 in domain %4. Please see
 * http://support.microsoft.com/kb/2654097 for more information.
 */

#define NELOG_NetlogonUserValidationReqWaitInitialWarning (ERRLOG2_BASE + 118)
/*
 * Netlogon took more than %1 seconds for an authentication request of account
 * %2 in domain %3, through domain controller %4 in domain %5. This is the first
 * warning. If the problem persists, a recurring event will be logged every %6
...
/*
 * Netlogon took more than %1 seconds for %2 authentication requests through
 * domain controller %3 in domain %4 in the last %5 minutes. Please see
 * http://support.microsoft.com/kb/2654097 for more information.
 */
...
/*
 * The Netlogon service denied a vulnerable Netlogon secure channel connection
 ...
 *
 * %n%nFor more information about why this was denied, please visit
...
#define NETLOG_NetlogonUnsecuredRpcMachineTemporarilyAllowed                   \
  (ERRLOG2_BASE + 129)
/*
 * The Netlogon service allowed a vulnerable Netlogon secure channel connection.
 *
 * %n%nWarning: This connection will be denied once the enforcement phase is
 * released. To better understand the enforcement phase, please visit
 * https://go.microsoft.com/fwlink/?linkid=2133485.
 *
 * %n%n Machine SamAccountName: %1
 * %n Domain: %2
...
/*
 * The Netlogon service allowed a vulnerable Netlogon secure channel connection
 * because the machine account is allowed in the "Domain controller: Allow
 * vulnerable Netlogon secure channel connections" group policy.
 *
 * %n%nWarning: Using vulnerable Netlogon secure channels will expose the
 * domain-joined devices to attack. To protect your device from attack, remove a
 * machine account from "Domain controller: Allow vulnerable Netlogon secure
 * channel connections" group policy after the third-party Netlogon client has
 * been updated. To better understand the risk of configuring machine accounts
 * to be allowed to use vulnerable Netlogon secure channel connections, please
 * visit  https://go.microsoft.com/fwlink/?linkid=2133485.
 *
 * %n%n Machine SamAccountName: %1
 * %n Domain: %2
 * %n Account Type: %3
...
/*
 * The Netlogon service allowed a vulnerable Netlogon secure channel connection
 * because the trust account is allowed in the "Domain controller: Allow
 * vulnerable Netlogon secure channel connections" group policy.
 *
 * %n%nWarning: Using vulnerable Netlogon secure channels will expose Active
 * Directory forests to attack. To protect your Active Directory forests from
 * attack, all trusts must use secure RPC with Netlogon secure channel. Remove a
 * trust account from "Domain controller: Allow vulnerable Netlogon secure
 * channel connections" group policy after the third-party Netlogon client on
 * the domain controllers have been updated. To better understand the risk of
 * configuring trust accounts to be allowed to use vulnerable Netlogon secure
 * channel connections, please visit
 * https://go.microsoft.com/fwlink/?linkid=2133485.
 *
 * %n%n Account Type: %1
 * %n Trust Name: %2
...
/*
 * The Netlogon service allowed one or more unsecure pass-through NTLM
 * authentication requests from trusted domains and/or forests during the most
 * recent event throttling window. These unsecure requests would normally be
 * blocked but were allowed to proceed due to the current trust configuration.%n
 *
 * %n
 *
 * Warning: Allowing unsecure pass-through authentication requests will expose
 * your Active Directory forest to attack. For more information about this issue
 * please visit https://go.microsoft.com/fwlink/?linkid=276811&.%n
 *
 * %n
 *
...
/*
 * The Netlogon service blocked one or more unsecure pass-through NTLM
 * authentication requests from trusted clients, domains, and/or forests during
 * the most recent event throttling window. For more information about this
 * issue, including how to enable more verbose logging, please visit
 * https://go.microsoft.com/fwlink/?linkid=276811&.%n
 *
 * %n
 *
...
/*
 * The Netlogon service allowed an unsecure pass-through NTLM authentication
 * request from a trusted client, domain, or forest. This unsecure request would
 * normally be blocked but was allowed to proceed due to the current trust
 * configuration.%n
 *
 * %n
 *
...
/*
 * The Netlogon service blocked an unsecure pass-through NTLM authentication
 * requests from a trusted client, domain, or forest. For more information,
 * please visit https://go.microsoft.com/fwlink/?linkid=276811&.
 *
 * %n%n
 *
 * Account name: %1%n
...
/*
 * The Netlogon service was able to bind to a TCP/IP port with the configured
 * backlog size of %1.
 */
 ...
 * backlog size of %1 but failed. %n%n
 *
 * More information can be found in the following log file
 * '%SystemRoot%\debug\netlogon.log' and, potentially, in the log file
 * '%SystemRoot%\debug\netlogon.bak' created if the former log becomes full. For
 * steps in enabling the log, please visit
