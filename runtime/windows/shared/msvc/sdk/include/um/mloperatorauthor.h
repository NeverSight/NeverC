...
enum class MLOperatorTensorDataType : uint32_t {
  ...
  //! 8 bit boolean. Values other than zero and one result in undefined
  //! behavior.
  Bool = 9,
...
struct MLOperatorEdgeDescription {
  ...
  union {
    ...
    //! The data type of each tensor in a sequence of tensors.  Used when
    //! edgeType is set to Sequence.
    MLOperatorTensorDataType tensorDataType;
  };
};

//! \interface IMLOperatorAttributes
//! \brief Represents the values of an operator's attributes, as determined by a
//! model using the operator. This interface is called by implementations of
//! custom operator kernels, and by implementations of shape and type inferrers.
interface DECLSPEC_UUID("4B1B1759-EC40-466C-AAB4-BEB5347FD24C")
    DECLSPEC_NOVTABLE IMLOperatorAttributes : IUnknown {
  ...
  STDMETHOD(GetAttributeElementCount)(
      _In_z_ const char *name, MLOperatorAttributeType type,
      _Out_ uint32_t *elementCount) const noexcept PURE;
  ...
  STDMETHOD(GetAttribute)(
      _In_z_ const char *name, MLOperatorAttributeType type,
      uint32_t elementCount, size_t elementByteSize,
  ...
  //! specified index.
  //! The string is in UTF-8 format.  The size includes the null termination
  //! character.
  STDMETHOD(GetStringAttributeElementLength)(
      _In_z_ const char *name, uint32_t elementIndex,
      _Out_ uint32_t *attributeElementByteSize) const noexcept PURE;
...
//! This interface is called by the factory objects registered to create
//! kernels. It is available to these factory objects unless corresponding
//! kernels are registered using the
//! MLOperatorKernelOptions::AllowDynamicInputShapes flag.
interface DECLSPEC_UUID("F20E8CBE-3B28-4248-BE95-F96FBC6E4643")
    DECLSPEC_NOVTABLE IMLOperatorTensorShapeDescription : IUnknown {
  ...
  STDMETHOD(GetInputTensorDimensionCount)(
      uint32_t inputIndex, _Out_ uint32_t *dimensionCount) const noexcept PURE;

  //! Gets the sizes of dimensions of an input tensor of the operator.
  //! Returns an error if the input at the specified index is not a tensor.
  STDMETHOD(GetInputTensorShape)(uint32_t inputIndex, uint32_t dimensionCount,
                                 _Out_writes_(dimensionCount)
                                     uint32_t *dimensions) const noexcept PURE;

  //! Returns true if output shapes may be queried using
  //! GetOutputTensorDimensionCount and GetOutputTensorShape. This is true if
  //! the kernel was registered with a shape inferrer.
  ...
  STDMETHOD(GetOutputTensorDimensionCount)(
      uint32_t outputIndex, _Out_ uint32_t *dimensionCount) const noexcept PURE;

  //! Gets the sizes of dimensions of a tensor output of the operator.
  //! Returns an error if the output at the specified index is not a tensor.
  STDMETHOD(GetOutputTensorShape)(uint32_t outputIndex, uint32_t dimensionCount,
                                  _Out_writes_(dimensionCount)
                                      uint32_t *dimensions) const noexcept PURE;
};

//! \interface IMLOperatorKernelCreationContext
//! \brief Provides information about an operator's usage while kernels are
//! being created.
interface DECLSPEC_UUID("5459B53D-A0FC-4665-ADDD-70171EF7E631")
    DECLSPEC_NOVTABLE IMLOperatorKernelCreationContext
    : public IMLOperatorAttributes {
  ...
  //! This always returns true if within GetInputCount except for optional
  //! inputs.
  STDMETHOD_(bool, IsInputValid)(uint32_t inputIndex) const noexcept PURE;

  //! Returns true if an output to the operator is valid.
  //! This always returns true if within GetOutputCount except for optional
  //! outputs.
  ...
  STDMETHOD(GetInputEdgeDescription)(
      uint32_t inputIndex, _Out_ MLOperatorEdgeDescription *edgeDescription)
      const noexcept PURE;

  //! Gets the description of the specified output edge of the operator.
  STDMETHOD(GetOutputEdgeDescription)(
      uint32_t outputIndex, _Out_ MLOperatorEdgeDescription *edgeDescription)
      const noexcept PURE;
  ...
  //! Returns an object whose supported interfaces vary based on the kernel
  //! type. For kernels registered with MLOperatorExecutionType::Cpu,
  //! executionObject will be set to nullptr. For kernels registered with
  //! MLOperatorExecutionType::D3D12, executionObject will support the
  //! ID3D12GraphicsCommandList interface.
  STDMETHOD_(void, GetExecutionInterface)(_Outptr_result_maybenull_ IUnknown *
                                          *executionObject) const noexcept PURE;
};

//! \interface IMLOperatorTensor
//! \brief Representation of a tensor used during computation of custom operator
//! kernels.
interface DECLSPEC_UUID("7FE41F41-F430-440E-AECE-54416DC8B9DB")
    DECLSPEC_NOVTABLE IMLOperatorTensor : IUnknown {
  ...
  STDMETHOD(GetShape)(uint32_t dimensionCount,
  ...
  //! This is true when kernels are registered using
  ...
  STDMETHOD_(void, GetDataInterface)(_Outptr_result_maybenull_ IUnknown *
                                     *dataInterface) noexcept PURE;
};

//! \interface IMLOperatorKernelContext
//! \brief Provides information about an operator's usage while kernels are
//! being computed.
interface DECLSPEC_UUID("82536A28-F022-4769-9D3F-8B278F84C0C3")
    DECLSPEC_NOVTABLE IMLOperatorKernelContext : IUnknown {
  ...
  //! then the overload of GetOutputTensor which consumes the tensor's shape
  //! must be called instead. Returns an error if the output at the specified
  //! index is not a tensor.
  STDMETHOD(GetOutputTensor)(
      uint32_t outputIndex,
      _COM_Outptr_result_maybenull_ IMLOperatorTensor **tensor) noexcept PURE;

  //! Gets the output tensor of the operator at the specified index, while
  //! declaring its shape. This returns nullptr for optional outputs which do
  //! not exist. If the operator kernel was registered with a shape inference
  //! method, then the overload of GetOutputTensor which doesn't consume a shape
  //! may also be called. Returns an error if the output at the specified index
  //! is not a tensor.
  STDMETHOD(GetOutputTensor)(
      uint32_t outputIndex, uint32_t dimensionCount,
      _In_reads_(dimensionCount) const uint32_t *dimensionSizes,
      _COM_Outptr_result_maybenull_ IMLOperatorTensor **tensor) noexcept PURE;

  //! Allocates temporary data which will be usable as intermediate memory for
  //! the duration of a call to IMLOperatorKernel::Compute.  This may be used by
  //! kernels registered using MLOperatorExecutionType::D3D12.  The data object
  //! supports the ID3D12Resource interface, and is a GPU buffer.
  STDMETHOD(AllocateTemporaryData)(size_t size, _COM_Outptr_ IUnknown **data)
      const = 0;
  ...
  //! executionObject will be set to nullptr. For kernels registered with
  //! MLOperatorExecutionType::D3D12, executionObject will support the
  //! ID3D12GraphicsCommandList interface. This may be a different object than
  //! was provided to IMLOperatorKernelCreationContext::GetExecutionInterface
...
//! registering custom operator kernels using
//! IMLOperatorKernelFactory::RegisterOperatorKernel.
interface DECLSPEC_UUID("11C4B4A0-B467-4EAA-A1A6-B961D8D0ED79")
    DECLSPEC_NOVTABLE IMLOperatorKernel : IUnknown {
...
//! \brief Specifies the manner in which types of input and output edges are
//! described. This is used within MLOperatorSchemaEdgeDescription while
//! defining custom operator schema.
enum class MLOperatorSchemaEdgeTypeFormat {
  //! The type is defined using MLOperatorEdgeDescription.
  EdgeDescription = 0,

  //! The type is defined by a type string constructed as in ONNX operator
  //! schema.
  Label = 1,
};
...
struct MLOperatorSchemaEdgeDescription {
  ...
  union {
    ...
    //! A type label string constructed as in ONNX operator schema. For example,
    //! "T". This is used when typeFormat is
    //! MLOperatorSchemaEdgeTypeFormat::Label.
    _Field_z_ const char *typeLabel;

    //! A structure describing type support.
    //! This is used when typeFormat is
    //! MLOperatorSchemaEdgeTypeFormat::EdgeDescription.
    MLOperatorEdgeDescription edgeDescription;
  };
};

//! \struct MLOperatorEdgeTypeConstraint
//! \brief Specifies constraints upon the types of edges supported in custom
//! operator kernels and schema. The provided type label string corresponds to
//! type labels in the ONNX specification for the same operator. For custom
//! schema, it corresponds to type labels specified within
//! MLOperatorSchemaEdgeDescription when registering the operator's schema.
struct MLOperatorEdgeTypeConstraint {
  ...
  _Field_size_opt_(allowedTypeCount) const
      MLOperatorEdgeDescription *allowedTypes;
...
//! \brief Provides information about an operator's usage while shape inferrers
//! are being invoked.
interface DECLSPEC_UUID("105B6B29-5408-4A68-9959-09B5955A3492")
    DECLSPEC_NOVTABLE IMLOperatorShapeInferenceContext
    : public IMLOperatorAttributes {
  //! Gets the number of inputs to the operator.
  STDMETHOD_(uint32_t, GetInputCount)() const noexcept PURE;
  ...
  //! Returns an error if the output at the specified index is not a tensor.
  STDMETHOD(SetOutputTensorShape)(uint32_t outputIndex, uint32_t dimensionCount,
                                  const uint32_t *dimensions) noexcept PURE;
};

//! \interface IMLOperatorTypeInferenceContext
//! \brief Provides information about an operator's usage while type inferrers
//! are being invoked.
interface DECLSPEC_UUID("EC893BB1-F938-427B-8488-C8DCF775F138")
    DECLSPEC_NOVTABLE IMLOperatorTypeInferenceContext
    : public IMLOperatorAttributes {
  //! Gets the number of inputs to the operator.
  STDMETHOD_(uint32_t, GetInputCount)() const noexcept PURE;
  ...
  STDMETHOD(SetOutputEdgeDescription)(
      uint32_t outputIndex, const MLOperatorEdgeDescription *edgeDescription)
      const noexcept PURE;
};

//! \interface IMLOperatorTypeInferrer
//! \brief Implemented by type inferrers to infer types of an operator's output
//! edges. Type inferrers must be provided when registering schema of custom
//! operators if the MLOperatorSchemaDescription structure cannot express how
//! output types are determined.  For example, such as when an attribute of the
//! operator determines the data type of one of that operator's outputs.
interface DECLSPEC_UUID("781AEB48-9BCB-4797-BF77-8BF455217BEB")
    DECLSPEC_NOVTABLE IMLOperatorTypeInferrer : IUnknown {
  //! Called to infer types of an operator's output edges
  STDMETHOD(InferOutputTypes)(IMLOperatorTypeInferenceContext *
                              context) noexcept PURE;
};
...
interface DECLSPEC_UUID("540BE5BE-A6C9-40EE-83F6-D2B8B40A7798")
    DECLSPEC_NOVTABLE IMLOperatorShapeInferrer : IUnknown {
...
//! \brief Specifies the name and properties of an attribute of a custom
//! operator. This is used when registering custom operator kernels and custom
//! operator schema.
struct MLOperatorAttribute {
  ...
  //! Whether the attribute is required in any model using the associated
  //! operator type.
  bool required;
};
...
//! This is used when registering custom operator kernels and custom operator
//! schema.
struct MLOperatorAttributeNameValue {
  //! NULL-terminated UTF-8 string representing the name of the attribute in the
  //! associated operator type.
  ...
  //! The number of elements in the attribute value.  This must be one, except
  //! for attributes which are of array types.
  uint32_t valueCount;
...
struct MLOperatorSchemaDescription {
  ...
  //! The operator set version at which this operator was introduced or last
  //! changed.
  ...
  _Field_size_opt_(typeConstraintCount) const
  ...
  //! The default values of attributes.  These will be applied when the
  //! attributes are missing in a model containing the operator type.
  _Field_size_opt_(defaultAttributeCount) const
...
enum class MLOperatorKernelOptions : uint32_t {
  ...
  //! Specifies whether the shapes of input tensors are allowed to vary among
  //! invocations of an operator kernel instance.  If this is not set, kernel
  //! instances may query input tensor shapes during creation, and front-load
  //! initialization work which depends on those shapes.  Setting this may
  //! improve performance if shapes vary dynamically between inference
  //! operations, and the kernel implementation handles this efficiently.
...
struct MLOperatorKernelDescription {
  //! NULL-terminated UTF-8 string representing the name of the operator's
  //! domain.
  _Field_z_ const char *domain;
  ...
  //! The maximum version is inferred based on registrations of operator set
  //! schema for subsequent versions of the same domain.
...
//! \brief Implemented by the author of a custom operator kernel to create
//! instances of that kernel.
interface DECLSPEC_UUID("EF15AD6F-0DC9-4908-AB35-A575A30DFBF8")
    DECLSPEC_NOVTABLE IMLOperatorKernelFactory : IUnknown {
  //! Creates an instance of the associated operator kernel, given information
  //! about the operator's usage within a model described in the provided
  //! context object.
  STDMETHOD(CreateKernel)(IMLOperatorKernelCreationContext * context,
                          _COM_Outptr_ IMLOperatorKernel *
                              *kernel) noexcept PURE;
};

//! \interface IMLOperatorRegistry
//! \brief Represents an instance of a registry for custom operator kernel and
//! schema. Custom operators may be used with WinML APIs by returning instances
//! of IMLOperatorRegistry through ILearningModelOperatorProviderNative.
interface DECLSPEC_UUID("2AF9DD2D-B516-4672-9AB5-530C208493AD")
    DECLSPEC_NOVTABLE IMLOperatorRegistry : IUnknown {
  //! Registers a set of custom operator schema comprising an operator set.
  //! Operator sets follow the ONNX versioning design.  Callers should provide
  //! schema for all operators that have changed between the specified baseline
  //! version and the version specified within operatorSetId.  This prevents
  //! older versions of kernels from being used in models which import the newer
  //! operator set version. A type inferrer must be provided if the
  //! MLOperatorSchemaDescription structure cannot express how output types are
  //! determined.  A shape inferrer may optionally be provided to enable model
  //! validation.
  STDMETHOD(RegisterOperatorSetSchema)(
      const MLOperatorSetId *operatorSetId, int32_t baselineVersion,
      _In_reads_opt_(schemaCount)
          const MLOperatorSchemaDescription *const *schema,
      uint32_t schemaCount, _In_opt_ IMLOperatorTypeInferrer *typeInferrer,
      _In_opt_ IMLOperatorShapeInferrer *shapeInferrer) const noexcept PURE;

  //! Registers a custom operator kernel.
  //! A shape inferrer may optionally be provided.  This may improve performance
  //! and enables the kernel to query the shape of its output tensors when it is
  //! created and computed.
...
//! Creates an instance of IMLOperatorRegistry which may be used to register
//! custom operator kernel and custom operator schema.
HRESULT WINAPI
MLCreateOperatorRegistry(_COM_Outptr_ IMLOperatorRegistry **registry);
}
