...
#define RF_ARBITRARY_ANGLE                                                     \
  1024L                  // can read text written at arbitrary angles (mimio)
...
#define RF_STROKEREORDER                                                       \
  8192L // indicates that stroke order - spatial and temporal is handled
#define RF_PERSONALIZABLE 16384L // Supports personalization
//                               32768L is reserved.
#define RF_PERFORMSLINEBREAKING                                                \
  65536L // Recognizer prefers to do the line breaking
#define RF_REQUIRESSEGMENTATIONBREAKING                                        \
  131072L // Recognizer wants only segments of ink
...
typedef struct tagRECO_LATTICE_PROPERTY {
  ...
  [
    unique, size_is(cbPropertyValue)
  ] BYTE *pPropertyValue; // points to data, depending on guid, it may begin
                          // with size
} RECO_LATTICE_PROPERTY;
typedef struct tagRECO_LATTICE_PROPERTIES {
  ULONG cProperties; // count of properties
  [
    unique, size_is(, cProperties)
  ] RECO_LATTICE_PROPERTY **apProps; // array of property structures
} RECO_LATTICE_PROPERTIES;
typedef int RECO_SCORE;
typedef struct tagRECO_LATTICE_ELEMENT {
