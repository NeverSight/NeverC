...
typedef struct {
  CLSID clsidNamespace; // namespace identifier (indicates which namespace
                        // selection from)
...
// -----------------------
//  When invoking an object referenced by a display specifier (context menu,
//  property page, etc) we call the IShellExtInit interface passing a
//  IDataObject.  This data object supports the CF_DSDISPLAYSPECOPTIONS format
//  to give out configuration informaiton about admin/shell invocation.
//
...
#define DSDSOF_HASUSERANDSERVERINFO                                            \
  0x00000001 // = 1 => user name/password are valid
#define DSDSOF_SIMPLEAUTHENTICATE                                              \
  0x00000002 // = 1 => don't use secure authentication to DS
#define DSDSOF_DONTSIGNSEAL                                                    \
  0x00000004 // = 1 => don't sign+seal when opening DS objects
...
#define DBDTF_RETURNMIXEDDOMAINS                                               \
  0x00000002 // set it if you want downlevel trust domains too
#define DBDTF_RETURNEXTERNAL                                                   \
  0x00000004 // set it if you want external trust domains too
#define DBDTF_RETURNINBOUND 0x00000008 // set it if you want trusting domains
#define DBDTF_RETURNINOUTBOUND                                                 \
  0x00000010 // set it if you want both trusted and trusting domains
...
DECLARE_INTERFACE_IID_(IDsBrowseDomainTree, IUnknown,
  ...
  STDMETHOD(BrowseTo)(THIS_ HWND hwndParent, _Out_ LPWSTR * ppszTargetPath,
...
//
#define DSSSF_SIMPLEAUTHENTICATE                                               \
  0x00000001 // = 1 => don't use secure authentication to DS
#define DSSSF_DONTSIGNSEAL                                                     \
  0x00000002 // = 1 => don't use sign+seal when opening objects in the DS
...
#define DSGIF_GETDEFAULTICON                                                   \
  0x0000010 // = 1 => if no icon then get default (from shell32.dll)
#define DSGIF_DEFAULTISCONTAINER                                               \
  0x0000020 // = 1 => if returning default icon, return it as a container

//
// Flags for IDsDisplaySpecifier::IsClassContainer
//
#define DSICCF_IGNORETREATASLEAF                                               \
  0x00000001 // = 1 => igore the "treatAsLeaf" and use only schema information

//
// Callback function used for IDsDisplaySpecifier::EnumClassAttributes
//

#define DSECAF_NOTLISTED                                                       \
  0x00000001 // = 1 => hide from the field drop down in the query UI
...
#define DSCCIF_HASWIZARDDIALOG                                                 \
  0x00000001 // = 1 => return the wizard dialog CLSID
#define DSCCIF_HASWIZARDPRIMARYPAGE                                            \
  0x00000002 // = 1 => returning a primary wizard dlg CLSID
...
DECLARE_INTERFACE_IID_(IDsDisplaySpecifier, IUnknown,
  ...
  STDMETHOD(GetDisplaySpecifier)(THIS_ LPCWSTR pszObjectClass, REFIID riid,
                                 void **ppv) PURE;
  STDMETHOD(GetIconLocation)(THIS_ LPCWSTR pszObjectClass, _In_ DWORD dwFlags,
                             _Out_writes_(cchBuffer) LPWSTR pszBuffer,
                             _In_ INT cchBuffer, _Out_ INT * presid) PURE;
  ...
  STDMETHOD(GetFriendlyClassName)(THIS_ LPCWSTR pszObjectClass,
                                  _Out_writes_(cchBuffer) LPWSTR pszBuffer,
                                  _In_ INT cchBuffer) PURE;
  STDMETHOD(GetFriendlyAttributeName)(
      THIS_ LPCWSTR pszObjectClass, _In_ LPCWSTR pszAttributeName,
      _Out_writes_(cchBuffer) LPWSTR pszBuffer, _In_ UINT cchBuffer) PURE;
  STDMETHOD_(BOOL, IsClassContainer)(THIS_ LPCWSTR pszObjectClass,
...
#define DSBI_NOROOT                                                            \
  0x00010000 // don't include pszRoot in tree (its children become top level
             // nodes)
#define DSBI_INCLUDEHIDDEN 0x00020000 // display hidden objects
#define DSBI_EXPANDONOPEN                                                      \
  0x00040000 // expand to the path specified in pszPath when opening the dialog
#define DSBI_ENTIREDIRECTORY                                                   \
  0x00090000 // browse the entire directory (defaults to having DSBI_NOROOT set)
...
#define DSBI_IGNORETREATASLEAF                                                 \
  0x00400000 // ignore the treat as leaf flag when calling IsClassContainer
#define DSBI_SIMPLEAUTHENTICATE                                                \
  0x00800000 // don't use secure authentication to DS
#define DSBI_RETURNOBJECTCLASS                                                 \
  0x01000000                         // return object class of selected object
...
#define DSBM_QUERYINSERTW                                                      \
  100 // lParam = PDSBITEMW (state, icon & name may be modified). Return TRUE if
      // handled.
#define DSBM_QUERYINSERTA                                                      \
  101 // lParam = PDSBITEMA (state, icon & name may be modified). Return TRUE if
      // handled.
...
#define DSBM_CHANGEIMAGESTATE                                                  \
  102 // lParam = adspath.  Return TRUE/FALSE top allow/disallow
...
//
// These are the control IDs for the controls in the dialog.   The callback can
// use these to modify the contents of the dialog as required.
//
...
STDAPI_(HICON)
DsGetIcon(DWORD dwFlags, LPCWSTR pszObjectClass, INT cxImage, INT cyImage);
STDAPI DsGetFriendlyClassName(LPCWSTR pszObjectClass,
                              _Out_writes_(cchBuffer) LPWSTR pszBuffer,
                              UINT cchBuffer);

#ifdef __cplusplus
struct __declspec(uuid(
    "1ab4a8c0-6a0b-11d2-ad49-00c04fa31a86")) IDsDisplaySpecifier;
#endif
