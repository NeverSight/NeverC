...
class __declspec(uuid(
    "C4137916-2BE1-46FD-8599-441536F49856")) FXMasteringLimiter;
EXTERN_C const GUID DECLSPEC_SELECTANY CLSID_FXMasteringLimiter =
    __uuidof(FXMasteringLimiter);
...
typedef struct FXECHO_INITDATA {
  float MaxDelay; // maximum delay (all channels) in milliseconds, must be
                  // within [FXECHO_MIN_DELAY, FXECHO_MAX_DELAY]
} FXECHO_INITDATA;
...
typedef struct FXECHO_PARAMETERS {
  ...
  float Delay;     // delay (all channels) in milliseconds, must be within
                   // [FXECHO_MIN_DELAY, FXECHO_PARAMETERS.MaxDelay]
} FXECHO_PARAMETERS;
...
//  pInitData        - [in] effect-specific initialization parameters, may be
//  NULL if InitDataByteSize == 0 InitDataByteSize - [in] size of pInitData in
//  bytes, may be 0 if pInitData is NULL
FX_API_(HRESULT)
CreateFX(REFCLSID clsid, _Outptr_ IUnknown **pEffect,
         _In_reads_bytes_opt_(InitDataByteSize)
             const void *pInitDat DEFAULT(NULL),
         UINT32 InitDataByteSize DEFAULT(0));
