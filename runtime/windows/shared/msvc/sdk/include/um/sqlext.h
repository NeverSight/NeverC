...
#define SQL_C_USHORT                                                           \
  (SQL_C_SHORT + SQL_UNSIGNED_OFFSET) /* UNSIGNED                              \
                                         SMALLINT*/
#define SQL_C_UTINYINT                                                         \
  (SQL_TINYINT + SQL_UNSIGNED_OFFSET) /* UNSIGNED                              \
                                         TINYINT*/
...
// base value of driver-specific fields/attributes (max are 0x7fff [16-bit] or
// 0x00007fff [32-bit]) define driver-specific SQL-Type, named as
// SQL_DRIVER_SQL_TYPE_BASE, SQL_DRIVER_SQL_TYPE_BASE+1,
// SQL_DRIVER_SQL_TYPE_BASE+2, etc.
//
...
SQLRETURN SQL_API SQLDriverConnect(
    SQLHDBC hdbc, SQLHWND hwnd, _In_reads_(cchConnStrIn) SQLCHAR *szConnStrIn,
    SQLSMALLINT cchConnStrIn,
    _Out_writes_opt_(cchConnStrOutMax) SQLCHAR *szConnStrOut,
...
SQLRETURN SQL_API SQLBrowseConnect(
    SQLHDBC hdbc, _In_reads_(cchConnStrIn) SQLCHAR *szConnStrIn,
    SQLSMALLINT cchConnStrIn,
    _Out_writes_opt_(cchConnStrOutMax) SQLCHAR *szConnStrOut,
    SQLSMALLINT cchConnStrOutMax, _Out_opt_ SQLSMALLINT *pcchConnStrOut);
...
SQLRETURN SQL_API SQLColumnPrivileges(
    SQLHSTMT hstmt, _In_reads_opt_(cchCatalogName) SQLCHAR *szCatalogName,
    SQLSMALLINT cchCatalogName,
    _In_reads_opt_(cchSchemaName) SQLCHAR *szSchemaName,
    SQLSMALLINT cchSchemaName,
    _In_reads_opt_(cchTableName) SQLCHAR *szTableName, SQLSMALLINT cchTableName,
    _In_reads_opt_(cchColumnName) SQLCHAR *szColumnName,
    SQLSMALLINT cchColumnName);

SQLRETURN SQL_API SQLDescribeParam(SQLHSTMT hstmt, SQLUSMALLINT ipar,
                                   _Out_opt_ SQLSMALLINT *pfSqlType,
                                   _Out_opt_ SQLULEN *pcbParamDef,
...
SQLRETURN SQL_API SQLExtendedFetch(SQLHSTMT hstmt, SQLUSMALLINT fFetchType,
                                   SQLLEN irow, _Out_opt_ SQLULEN *pcrow,
                                   _Out_opt_ SQLUSMALLINT *rgfRowStatus);

SQLRETURN SQL_API SQLForeignKeys(
    SQLHSTMT hstmt, _In_reads_opt_(cchPkCatalogName) SQLCHAR *szPkCatalogName,
    SQLSMALLINT cchPkCatalogName,
    _In_reads_opt_(cchPkSchemaName) SQLCHAR *szPkSchemaName,
    SQLSMALLINT cchPkSchemaName,
    _In_reads_opt_(cchPkTableName) SQLCHAR *szPkTableName,
    SQLSMALLINT cchPkTableName,
    _In_reads_opt_(cchFkCatalogName) SQLCHAR *szFkCatalogName,
    SQLSMALLINT cchFkCatalogName,
    _In_reads_opt_(cchFkSchemaName) SQLCHAR *szFkSchemaName,
    SQLSMALLINT cchFkSchemaName,
    _In_reads_opt_(cchFkTableName) SQLCHAR *szFkTableName,
...
SQLRETURN SQL_API SQLNativeSql(SQLHDBC hdbc,
                               ...
                               SQLINTEGER cchSqlStrMax, SQLINTEGER *pcbSqlStr);
...
SQLRETURN SQL_API SQLPrimaryKeys(
    ...
    _In_reads_opt_(cchSchemaName) SQLCHAR *szSchemaName,
    SQLSMALLINT cchSchemaName,
    _In_reads_opt_(cchTableName) SQLCHAR *szTableName,
...
SQLRETURN SQL_API SQLProcedureColumns(
    ...
    SQLSMALLINT cchCatalogName,
    _In_reads_opt_(cchSchemaName) SQLCHAR *szSchemaName,
    SQLSMALLINT cchSchemaName, _In_reads_opt_(cchProcName) SQLCHAR *szProcName,
...
SQLRETURN SQL_API
SQLDrivers(SQLHENV henv, SQLUSMALLINT fDirection,
           _Out_writes_opt_(cchDriverDescMax) SQLCHAR *szDriverDesc,
           SQLSMALLINT cchDriverDescMax, _Out_opt_ SQLSMALLINT *pcchDriverDesc,
           _Out_writes_opt_(cchDrvrAttrMax) SQLCHAR *szDriverAttributes,
...
RETCODE SQL_API TraceOpenLogFile(_In_opt_ LPWSTR szFileName,
                                 _Out_writes_bytes_opt_(cbOutputMsg)
...
VOID SQL_API TraceReturn(RETCODE,
                         RETCODE); // Processes trace after FN is called
DWORD SQL_API TraceVersion();      // Returns trace API version

/* Functions for Visual Studio Analyzer*/
/* to turn on/off tracing or VS events, call TraceVSControl by setting or
 * clearing the following bits  */
...
//------------- Visual Studio Analyzer Support is removed from ODBC (Windows 8
// and onwards) ---------
// From Windows 8 and onwards, Visual Studio Analyzer Support for ODBC is
// removed. Third-party trace library does not need to define the function
// FireVSDebugEvent any more, which would not be called from ODBC DM in
// Windows 8. If your trace library is going to work on downlevel platforms, you
// may still want to define the function FireVSDebugEvent. But the trace library
// can still be loaded on downlevel platforms even if this function is not
// defined.
//
#pragma deprecated(ODBC_VS_FLAG_UNICODE_ARG, ODBC_VS_FLAG_UNICODE_COR,         \
                   ODBC_VS_FLAG_RETCODE, ODBC_VS_FLAG_STOP, tagODBC_VS_ARGS,   \
