...
#error                                                                         \
    "This version of XAudio2 is available only in Windows 8 or later. Use the XAudio2 headers and libraries from XAudio2Redist with applications that target Windows 7. See https://aka.ms/xaudio2redist."
...
interface __declspec(uuid(
    "84ac29bb-d619-44d2-b197-e4acf7df3ed6")) IXAudio2Extension;
EXTERN_C const GUID DECLSPEC_SELECTANY IID_IXAudio2Extension =
    __uuidof(IXAudio2Extension);
...
#else  // #if (NTDDI_VERSION >= NTDDI_WIN10)
// Compiling with C for Windows 8 or 8.1
DEFINE_GUID(IID_IXAudio2, 0x60d8dac8, 0x5aa1, 0x4e8e, 0xb5, 0x97, 0x2f, 0x5e,
            0x28, 0x83, 0xd4, 0x84);
#endif // #if (NTDDI_VERSION >= NTDDI_WIN10)
...
#define XAUDIO2_VOICE_NOSAMPLESPLAYED                                          \
  0x0100 // Used in IXAudio2SourceVoice::GetState
#define XAUDIO2_STOP_ENGINE_WHEN_IDLE                                          \
  0x2000 // Used in XAudio2Create to force the engine to Stop when no source
         // voices are Started, and Start when a voice is Started
#define XAUDIO2_1024_QUANTUM                                                   \
  0x8000 // Used in XAudio2Create to specify nondefault processing quantum
         // of 21.33 ms (1024 samples at 48KHz)
#define XAUDIO2_NO_VIRTUAL_AUDIO_CLIENT                                        \
...
#define XAUDIO2_E_INVALID_CALL                                                 \
  ((HRESULT)0x88960001) // An API call or one of its arguments was illegal
#define XAUDIO2_E_XMA_DECODER_ERROR                                            \
  ((HRESULT)0x88960002) // The XMA hardware suffered an unrecoverable error
#define XAUDIO2_E_XAPO_CREATION_FAILED                                         \
  ((HRESULT)0x88960003) // XAudio2 failed to initialize an XAPO effect
#define XAUDIO2_E_DEVICE_INVALIDATED                                           \
  ((HRESULT)0x88960004) // An audio device became unusable (unplugged, etc)
...
// This value indicates that XAudio2 will choose the default processor by
// itself. The actual value chosen may vary depending on the hardware platform.
#define XAUDIO2_USE_DEFAULT_PROCESSOR 0x00000000
#endif

// This definition is included for backwards compatibilty. Implementations
// targeting Games and WIN10_19H1 and later, should use
...
typedef enum XAUDIO2_FILTER_TYPE {
  ...
  HighPassFilter, // Attenuates frequencies below the cutoff frequency
                  // (state-variable filter).
  NotchFilter,    // Attenuates frequencies inside a given range (state-variable
                  // filter).
  LowPassOnePoleFilter, // Attenuates frequencies above the cutoff frequency
                        // (one-pole filter, XAUDIO2_FILTER_PARAMETERS.OneOverQ
                        // has no effect)
  HighPassOnePoleFilter // Attenuates frequencies below the cutoff frequency
                        // (one-pole filter, XAUDIO2_FILTER_PARAMETERS.OneOverQ
                        // has no effect)
} XAUDIO2_FILTER_TYPE;
...
typedef struct XAUDIO2_FILTER_PARAMETERS {
  ...
  //  See XAudio2CutoffFrequencyToRadians() for state-variable
  //  filter types and
  //  XAudio2CutoffFrequencyToOnePoleCoefficient() for one-pole
  //  filter types.
...
DECLARE_INTERFACE_(IXAudio2, IUnknown) {
  ...
  //              Only IID_IUnknown, IID_IXAudio2 and IID_IXaudio2Extension are
  //              supported.
  //
  // ARGUMENTS:
  ...
  //
  STDMETHOD(QueryInterface)(THIS_ REFIID riid, _COM_Outptr_ void **ppvInterface)
      PURE;
  ...
  //
  STDMETHOD(RegisterForCallbacks)(THIS_ _In_ IXAudio2EngineCallback * pCallback)
      PURE;
  ...
  //
  STDMETHOD_(void, UnregisterForCallbacks)(THIS_ _In_ IXAudio2EngineCallback *
                                           pCallback) PURE;
  ...
  STDMETHOD(CreateSourceVoice)(
      ...
      _In_ const WAVEFORMATEX *pSourceFormat, UINT32 Flags X2DEFAULT(0),
  ...
  STDMETHOD(CreateSubmixVoice)(
      THIS_ _Outptr_ IXAudio2SubmixVoice * *ppSubmixVoice, UINT32 InputChannels,
  ...
  STDMETHOD(CreateMasteringVoice)(
      ...
      _In_ AUDIO_STREAM_CATEGORY StreamCategory X2DEFAULT(
          AudioCategory_GameEffects)) PURE;
  ...
  //
  STDMETHOD_(void, GetPerformanceData)(THIS_ _Out_ XAUDIO2_PERFORMANCE_DATA *
                                       pPerfData) PURE;
...
DECLARE_INTERFACE_(IXAudio2Extension, IUnknown) {
  ...
  //              quantumMilliseconds = (1000.0f * quantumNumerator /
  //              quantumDenominator)
  //
  // ARGUMENTS:
  ...
  //
  STDMETHOD_(void, GetProcessingQuantum)(THIS_ _Out_ UINT32 * quantumNumerator,
                                         _Out_range_(!=, 0) UINT32 *
                                             quantumDenominator);
  ...
  //
  STDMETHOD_(void, GetProcessor)(THIS_ _Out_range_(!=, 0) XAUDIO2_PROCESSOR *
                                 processor);
};
...
#define Declare_IXAudio2Voice_Methods()                                        \
  ...
  */                                                                           \
  STDMETHOD_(void, GetVoiceDetails)(THIS_ _Out_ XAUDIO2_VOICE_DETAILS *        \
                                    pVoiceDetails) PURE;                       \
                                                                               \
  ...
  */                                                                           \
  STDMETHOD_(void, GetEffectState)(THIS_ UINT32 EffectIndex,                   \
                                   _Out_ BOOL * pEnabled) PURE;                \
                                                                               \
  ...
  */                                                                           \
  STDMETHOD(SetOutputMatrix)(                                                  \
      THIS_ _In_opt_ IXAudio2Voice * pDestinationVoice, UINT32 SourceChannels, \
      UINT32 DestinationChannels,                                              \
      _In_reads_(SourceChannels * DestinationChannels)                         \
  ...
  */                                                                           \
  STDMETHOD_(void, GetOutputMatrix)(                                           \
      THIS_ _In_opt_ IXAudio2Voice * pDestinationVoice, UINT32 SourceChannels, \
      UINT32 DestinationChannels,                                              \
      _Out_writes_(SourceChannels * DestinationChannels) float *pLevelMatrix)  \
      PURE;                                                                    \
                                                                               \
...
DECLARE_INTERFACE_(IXAudio2SourceVoice, IXAudio2Voice) {
  ...
  //  OperationSet - Used to identify this call as part of a deferred batch.
  //
  STDMETHOD(Start)(THIS_ UINT32 Flags X2DEFAULT(0),
                   UINT32 OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;
  ...
  //  OperationSet - Used to identify this call as part of a deferred batch.
  //
  STDMETHOD(Stop)(THIS_ UINT32 Flags X2DEFAULT(0),
                  UINT32 OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;
  ...
  STDMETHOD(SubmitSourceBuffer)(
      THIS_ _In_ const XAUDIO2_BUFFER *pBuffer,
      _In_opt_ const XAUDIO2_BUFFER_WMA *pBufferWMA X2DEFAULT(NULL)) PURE;
  ...
  //
  STDMETHOD_(void, GetState)(THIS_ _Out_ XAUDIO2_VOICE_STATE * pVoiceState,
                             UINT32 Flags X2DEFAULT(0)) PURE;
  ...
  //  batch.
  //
  STDMETHOD(SetFrequencyRatio)(THIS_ float Ratio, UINT32 OperationSet X2DEFAULT(
                                                      XAUDIO2_COMMIT_NOW)) PURE;
...
// IXAudio2Extension
#define IXAudio2Extension_GetProcessingQuantum(This, quantumNumerator,         \
                                               quantumDenominator)             \
  ((This)->lpVtbl->GetProcessingQuantum(This, quantumNumerator,                \
                                        quantumDenominator))
...
// filter types only.  Use XAudio2CutoffFrequencyToOnePoleCoefficient() for
// one-pole filter types. Note that the highest CutoffFrequency supported is
// SampleRate/6. Higher values of CutoffFrequency will return
...
__inline float XAudio2CutoffFrequencyToOnePoleCoefficient(float CutoffFrequency,
                                                          UINT32 SampleRate) {
...
XAUDIO2_STDAPI XAudio2CreateWithVersionInfo(
    _Outptr_ IXAudio2 **ppXAudio2, UINT32 Flags X2DEFAULT(0),
...
inline HRESULT XAudio2Create(
  ...
  typedef HRESULT(__stdcall * XAudio2CreateWithVersionInfoFunc)(
      _Outptr_ IXAudio2 **, UINT32, XAUDIO2_PROCESSOR, DWORD);
  typedef HRESULT(__stdcall * XAudio2CreateInfoFunc)(_Outptr_ IXAudio2 **,
  ...
  if (s_dllInstance == NULL) {
    s_dllInstance =
        LoadLibraryEx(XAUDIO2_DLL, NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (s_dllInstance == NULL) {
      return HRESULT_FROM_WIN32(GetLastError());
    }

    s_pfnAudio2CreateWithVersion =
        (XAudio2CreateWithVersionInfoFunc)(void *)GetProcAddress(
            s_dllInstance, "XAudio2CreateWithVersionInfo");
    if (s_pfnAudio2CreateWithVersion == NULL) {
      s_pfnAudio2Create = (XAudio2CreateInfoFunc)(void *)GetProcAddress(
          s_dllInstance, "XAudio2Create");
  ...
  if (s_pfnAudio2CreateWithVersion != NULL) {
    return (*s_pfnAudio2CreateWithVersion)(ppXAudio2, Flags, XAudio2Processor,
                                           NTDDI_VERSION);
  }
  return (*s_pfnAudio2Create)(ppXAudio2, Flags, XAudio2Processor);
}
#elif (NTDDI_VERSION <= NTDDI_WIN10_RS5)
// Definition of XAudio2Create for Non-Desktop apps on RS5 and older, and
// Desktop apps on RS4 and older
XAUDIO2_STDAPI XAudio2Create(
    _Outptr_ IXAudio2 **ppXAudio2, UINT32 Flags X2DEFAULT(0),
    XAUDIO2_PROCESSOR XAudio2Processor X2DEFAULT(XAUDIO2_DEFAULT_PROCESSOR));
#else
// Definition of XAudio2Create for Non-Desktop apps targeting an OS release
// greater than RS5
XAUDIO2_STDAPI XAudio2Create(_Outptr_ IXAudio2 **ppXAudio2,
                             UINT32 Flags X2DEFAULT(0),
                             XAUDIO2_PROCESSOR XAudio2Processor
                                 X2DEFAULT(XAUDIO2_USE_DEFAULT_PROCESSOR));
#endif
...
#pragma pack(pop)
          ...
          WINAPI_PARTITION_TV_APP | WINAPI_PARTITION_TV_TITLE |                \
          WINAPI_PARTITION_GAMES) */
#pragma endregion
