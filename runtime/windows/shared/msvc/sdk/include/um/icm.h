...
typedef enum { CPT_ICC, CPT_DMP, CPT_CAMP, CPT_GMMP } COLORPROFILETYPE;
...
typedef enum {
  ...
  COLOR_10b_R10G10B10A2_XR, // Packed extended range WORD per channel.  data
                            // range [-1, 3] using 4.0 scale and -1.0 bias.
...
//
// Profile flag bits for vendor extensions (bytes 44..45 of the ICC profile
// header).
//
...
#define CMS_DISABLEINTENT                                                      \
  0x400 // Disable intent selection (render & proofing) always
...
typedef struct _tagCOLORMATCHSETUPW {
  ...
  PCMSCALLBACKW
  lpfnApplyCallback; //  Callback Procedure address when apply is pushed
...
HPROFILE WINAPI OpenColorProfileA(_In_ PPROFILE pProfile, DWORD dwDesiredAccess,
                                  DWORD dwShareMode, DWORD dwCreationMode);
HPROFILE WINAPI OpenColorProfileW(_In_ PPROFILE pProfile, DWORD dwDesiredAccess,
                                  DWORD dwShareMode, DWORD dwCreationMode);
BOOL WINAPI CloseColorProfile(_In_opt_ HPROFILE hProfile);
BOOL WINAPI GetColorProfileFromHandle(_In_ HPROFILE hProfile,
                                      _Out_writes_bytes_opt_(*pcbProfile)
                                          PBYTE pProfile,
                                      _Inout_ PDWORD pcbProfile);
...
BOOL WINAPI CreateProfileFromLogColorSpaceA(
    ...
    _Outptr_result_buffer_(_Inexpressible_(GlobalSize(*pProfile)))
        PBYTE *pProfile);
BOOL WINAPI CreateProfileFromLogColorSpaceW(
    _In_ LPLOGCOLORSPACEW pLogColorSpace,
    _Outptr_result_buffer_(_Inexpressible_(GlobalSize(*pProfile)))
        PBYTE *pProfile);
BOOL WINAPI GetCountColorProfileElements(_In_ HPROFILE hProfile,
                                         _Out_ PDWORD pnElementCount);
BOOL WINAPI GetColorProfileHeader(_In_ HPROFILE hProfile,
                                  _Out_ PPROFILEHEADER pHeader);
BOOL WINAPI GetColorProfileElementTag(_In_ HPROFILE hProfile, DWORD dwIndex,
                                      _Out_ PTAGTYPE pTag);
BOOL WINAPI IsColorProfileTagPresent(_In_ HPROFILE hProfile, TAGTYPE tag,
                                     _Out_ PBOOL pbPresent);
BOOL WINAPI GetColorProfileElement(_In_ HPROFILE hProfile, TAGTYPE tag,
                                   DWORD dwOffset, _Inout_ PDWORD pcbElement,
                                   _Out_writes_bytes_opt_(*pcbElement)
                                       PVOID pElement,
                                   _Out_ PBOOL pbReference);
BOOL WINAPI SetColorProfileHeader(_In_ HPROFILE hProfile,
                                  _In_reads_bytes_(sizeof(PROFILEHEADER))
                                      PPROFILEHEADER pHeader);
BOOL WINAPI SetColorProfileElementSize(_In_ HPROFILE hProfile, TAGTYPE tagType,
                                       DWORD pcbElement);
BOOL WINAPI SetColorProfileElement(_In_ HPROFILE hProfile, TAGTYPE tag,
                                   DWORD dwOffset, _In_ PDWORD pcbElement,
                                   _In_reads_(*pcbElement) PVOID pElement);
BOOL WINAPI SetColorProfileElementReference(_In_ HPROFILE hProfile,
                                            TAGTYPE newTag, TAGTYPE refTag);
BOOL WINAPI GetPS2ColorSpaceArray(_In_ HPROFILE hProfile, DWORD dwIntent,
                                  DWORD dwCSAType,
                                  _Out_writes_bytes_opt_(*pcbPS2ColorSpaceArray)
                                      PBYTE pPS2ColorSpaceArray,
                                  _Inout_ PDWORD pcbPS2ColorSpaceArray,
                                  _Out_ PBOOL pbBinary);
BOOL WINAPI GetPS2ColorRenderingIntent(
    _In_ HPROFILE hProfile, DWORD dwIntent,
    _Out_writes_bytes_opt_(*pcbPS2ColorRenderingIntent) PBYTE pBuffer,
    _Inout_ PDWORD pcbPS2ColorRenderingIntent);
BOOL WINAPI GetPS2ColorRenderingDictionary(
    ...
    _Inout_ PDWORD pcbPS2ColorRenderingDictionary, _Inout_ PBOOL pbBinary);
BOOL WINAPI GetNamedProfileInfo(
    _In_ HPROFILE hProfile, _Inout_updates_bytes_(sizeof(NAMED_PROFILE_INFO))
                                PNAMED_PROFILE_INFO pNamedProfileInfo);
BOOL WINAPI ConvertColorNameToIndex(_In_ HPROFILE hProfile,
                                    _In_reads_(dwCount) PCOLOR_NAME paColorName,
                                    _Out_writes_(dwCount) PDWORD paIndex,
                                    DWORD dwCount);
BOOL WINAPI ConvertIndexToColorName(_In_ HPROFILE hProfile,
                                    _In_reads_(dwCount) PDWORD paIndex,
                                    _Out_writes_(dwCount)
                                        PCOLOR_NAME paColorName,
                                    DWORD dwCount);
BOOL WINAPI CreateDeviceLinkProfile(
    _In_reads_(nProfiles) PHPROFILE hProfile, DWORD nProfiles,
    _In_reads_(nIntents) PDWORD padwIntent, DWORD nIntents, DWORD dwFlags,
    _Outptr_result_buffer_(_Inexpressible_(GlobalSize(*pProfileData)))
        PBYTE *pProfileData,
    DWORD indexPreferredCMM);
HTRANSFORM WINAPI CreateColorTransformA(_In_ LPLOGCOLORSPACEA pLogColorSpace,
                                        _In_ HPROFILE hDestProfile,
                                        _In_ HPROFILE hTargetProfile,
                                        DWORD dwFlags);
HTRANSFORM WINAPI CreateColorTransformW(_In_ LPLOGCOLORSPACEW pLogColorSpace,
                                        _In_ HPROFILE hDestProfile,
                                        _In_ HPROFILE hTargetProfile,
                                        DWORD dwFlags);
HTRANSFORM WINAPI CreateMultiProfileTransform(
    _In_reads_(nProfiles) PHPROFILE pahProfiles, DWORD nProfiles,
    _In_reads_(nIntents) PDWORD padwIntent, DWORD nIntents, DWORD dwFlags,
    DWORD indexPreferredCMM);
BOOL WINAPI DeleteColorTransform(_Inout_ HTRANSFORM hxform);
BOOL WINAPI TranslateBitmapBits(_In_ HTRANSFORM hColorTransform,
                                _In_ PVOID pSrcBits, BMFORMAT bmInput,
                                DWORD dwWidth, DWORD dwHeight,
                                DWORD dwInputStride, _Out_ PVOID pDestBits,
                                BMFORMAT bmOutput, DWORD dwOutputStride,
                                _In_opt_ __callback PBMCALLBACKFN pfnCallBack,
                                _In_opt_ LPARAM ulCallbackData);
BOOL WINAPI CheckBitmapBits(_In_ HTRANSFORM hColorTransform,
                            _In_ PVOID pSrcBits, BMFORMAT bmInput,
                            DWORD dwWidth, DWORD dwHeight, DWORD dwStride,
                            _Out_writes_(dwWidth *dwHeight) PBYTE paResult,
                            _In_opt_ __callback PBMCALLBACKFN pfnCallback,
                            _In_opt_ LPARAM lpCallbackData);
BOOL WINAPI TranslateColors(_In_ HTRANSFORM hColorTransform,
                            _In_reads_(nColors) PCOLOR paInputColors,
                            DWORD nColors, COLORTYPE ctInput,
                            _Out_writes_(nColors) PCOLOR paOutputColors,
                            COLORTYPE ctOutput);
BOOL WINAPI CheckColors(_In_ HTRANSFORM hColorTransform,
                        _In_reads_(nColors) PCOLOR paInputColors, DWORD nColors,
                        COLORTYPE ctInput,
                        _Out_writes_(nColors) PBYTE paResult);
DWORD WINAPI GetCMMInfo(_In_ HTRANSFORM hColorTransform, _In_ DWORD);
BOOL WINAPI RegisterCMMA(_In_opt_ PCSTR pMachineName, _In_ DWORD cmmID,
                         _In_ PCSTR pCMMdll);
BOOL WINAPI RegisterCMMW(_In_opt_ PCWSTR pMachineName, _In_ DWORD cmmID,
                         _In_ PCWSTR pCMMdll);
...
__declspec(deprecated("GetColorDirectoryW is deprecated and might not work on "
                      "all platforms. For more info, see MSDN."))
#endif
BOOL WINAPI GetColorDirectoryA(_In_opt_ PCSTR pMachineName,
                               _Out_writes_bytes_to_opt_(*pdwSize, *pdwSize)
                                   PSTR pBuffer,
                               _Inout_ PDWORD pdwSize);

#ifndef BUILD_WINDOWS
__declspec(deprecated("GetColorDirectoryA is deprecated and might not work on "
                      "all platforms. For more info, see MSDN."))
#endif
BOOL WINAPI GetColorDirectoryW(_In_opt_ PCWSTR pMachineName,
                               _Out_writes_bytes_to_opt_(*pdwSize, *pdwSize)
                                   PWSTR pBuffer,
                               _Inout_ PDWORD pdwSize);
...
BOOL WINAPI InstallColorProfileA(_In_opt_ PCSTR pMachineName,
                                 _In_ PCSTR pProfileName);
BOOL WINAPI InstallColorProfileW(_In_opt_ PCWSTR pMachineName,
...
BOOL WINAPI UninstallColorProfileA(_In_opt_ PCSTR pMachineName,
                                   _In_ PCSTR pProfileName, BOOL bDelete);
BOOL WINAPI UninstallColorProfileW(_In_opt_ PCWSTR pMachineName,
                                   _In_ PCWSTR pProfileName, BOOL bDelete);
BOOL WINAPI EnumColorProfilesA(
    _In_opt_ PCSTR pMachineName, _In_ PENUMTYPEA pEnumRecord,
    _Out_writes_bytes_opt_(*pdwSizeOfEnumerationBuffer)
        PBYTE pEnumerationBuffer,
    _Inout_ PDWORD pdwSizeOfEnumerationBuffer, _Out_opt_ PDWORD pnProfiles);
BOOL WINAPI EnumColorProfilesW(
    _In_opt_ PCWSTR pMachineName, _In_ PENUMTYPEW pEnumRecord,
    _Out_writes_bytes_opt_(*pdwSizeOfEnumerationBuffer)
        PBYTE pEnumerationBuffer,
    _Inout_ PDWORD pdwSizeOfEnumerationBuffer, _Out_opt_ PDWORD pnProfiles);
BOOL WINAPI SetStandardColorSpaceProfileA(_In_opt_ PCSTR pMachineName,
                                          DWORD dwProfileID,
                                          _In_ PCSTR pProfilename);
BOOL WINAPI SetStandardColorSpaceProfileW(_In_opt_ PCWSTR pMachineName,
                                          DWORD dwProfileID,
                                          _In_ PCWSTR pProfileName);

_Success_(return != FALSE) BOOL WINAPI
    GetStandardColorSpaceProfileA(_In_opt_ PCSTR pMachineName, _In_ DWORD dwSCS,
                                  _Out_writes_bytes_to_opt_(*pcbSize, *pcbSize)
                                      PSTR pBuffer,
                                  _Inout_ PDWORD pcbSize);

_Success_(return != FALSE) BOOL WINAPI
    GetStandardColorSpaceProfileW(_In_opt_ PCWSTR pMachineName,
                                  _In_ DWORD dwSCS,
                                  _Out_writes_bytes_to_opt_(*pcbSize, *pcbSize)
                                      PWSTR pBuffer,
                                  _Inout_ PDWORD pcbSize);

BOOL WINAPI AssociateColorProfileWithDeviceA(_In_opt_ PCSTR pMachineName,
                                             _In_ PCSTR pProfileName,
                                             _In_ PCSTR pDeviceName);
BOOL WINAPI AssociateColorProfileWithDeviceW(_In_opt_ PCWSTR pMachineName,
                                             _In_ PCWSTR pProfileName,
                                             _In_ PCWSTR pDeviceName);
BOOL WINAPI DisassociateColorProfileFromDeviceA(_In_opt_ PCSTR pMachineName,
                                                _In_ PCSTR pProfileName,
                                                _In_ PCSTR pDeviceName);
BOOL WINAPI DisassociateColorProfileFromDeviceW(_In_opt_ PCWSTR pMachineName,
                                                _In_ PCWSTR pProfileName,
                                                _In_ PCWSTR pDeviceName);
BOOL WINAPI SetupColorMatchingW(_Inout_updates_bytes_(sizeof(COLORMATCHSETUPW))
                                    PCOLORMATCHSETUPW pcms);
BOOL WINAPI SetupColorMatchingA(_Inout_updates_bytes_(sizeof(COLORMATCHSETUPA))
                                    PCOLORMATCHSETUPA pcms);

#if NTDDI_VERSION >= NTDDI_VISTA

BOOL WINAPI WcsAssociateColorProfileWithDevice(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ PCWSTR pProfileName,
    _In_ PCWSTR pDeviceName);
...
BOOL WINAPI WcsEnumColorProfilesSize(_In_ WCS_PROFILE_MANAGEMENT_SCOPE scope,
                                     _In_ PENUMTYPEW pEnumRecord,
                                     _Out_ PDWORD pdwSize);

BOOL WINAPI WcsEnumColorProfiles(_In_ WCS_PROFILE_MANAGEMENT_SCOPE scope,
                                 _In_ PENUMTYPEW pEnumRecord,
...
BOOL WINAPI WcsGetDefaultColorProfileSize(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_opt_ PCWSTR pDeviceName,
    _In_ COLORPROFILETYPE cptColorProfileType,
    _In_ COLORPROFILESUBTYPE cpstColorProfileSubType, _In_ DWORD dwProfileID,
...
BOOL WINAPI WcsSetDefaultRenderingIntent(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ DWORD dwRenderingIntent);

BOOL WINAPI WcsGetDefaultRenderingIntent(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _Out_ PDWORD pdwRenderingIntent);

BOOL WINAPI WcsGetUsePerUserProfiles(_In_ LPCWSTR pDeviceName,
                                     _In_ DWORD dwDeviceClass,
                                     _Out_ PBOOL pUsePerUserProfiles);

BOOL WINAPI WcsSetUsePerUserProfiles(_In_ LPCWSTR pDeviceName,
                                     _In_ DWORD dwDeviceClass,
                                     _In_ BOOL usePerUserProfiles);

BOOL WINAPI WcsTranslateColors(_In_ HTRANSFORM hColorTransform,
                               _In_ DWORD nColors, _In_ DWORD nInputChannels,
                               _In_ COLORDATATYPE cdtInput, _In_ DWORD cbInput,
...
BOOL WINAPI WcsCheckColors(_In_ HTRANSFORM hColorTransform, _In_ DWORD nColors,
...
BOOL WINAPI CMConvertColorNameToIndex(_In_ HPROFILE hProfile,
                                      _In_reads_(dwCount)
                                          PCOLOR_NAME paColorName,
                                      _Out_writes_(dwCount) PDWORD paIndex,
                                      DWORD dwCount);

BOOL WINAPI CMConvertIndexToColorName(_In_ HPROFILE hProfile,
                                      _In_reads_(dwCount) PDWORD paIndex,
                                      _Out_writes_(dwCount)
                                          PCOLOR_NAME paColorName,
                                      DWORD dwCount);
...
BOOL WINAPI CMCreateDeviceLinkProfile(
    ...
    _Outptr_result_buffer_(_Inexpressible_(GlobalSize(*lpProfileData)))
...
// are deprecated as of Windows Vista, and CMM implementors are no longer
// required to implement them. The implementations in the default CMM
// (icm32.dll) in Windows Vista now SetLastError to ERROR_NOT_SUPPORTED, and
// return an invalid transform handle (a value < 256).
//
...
//
// The CMM APIs CMCheckColorsInGamut, CMCreateProfile, CMTranslateRGB and
// CMTranslateRGBs are deprecated as of Windows 7, and CMM implementors are no
// longer required to implement them. The implementations in the default CMM
// (icm32.dll) in Windows 7 now SetLastError to ERROR_NOT_SUPPORTED, and return
// FALSE.
//
...
BOOL WINAPI CMTranslateRGB(_In_ HCMTRANSFORM hcmTransform, COLORREF ColorRef,
                           _Out_ LPCOLORREF lpColorRef, DWORD dwFlags);

BOOL WINAPI CMTranslateRGBs(_In_ HCMTRANSFORM hcmTransform,
                            _In_ LPVOID lpSrcBits, BMFORMAT bmInput,
                            DWORD dwWidth, DWORD dwHeight, DWORD dwStride,
                            _Out_ LPVOID lpDestBits, BMFORMAT bmOutput,
...
BOOL WINAPI CMGetNamedProfileInfo(
    ...
    _Inout_ PNAMED_PROFILE_INFO
        pNamedProfileInfo // pointer to named profile info
);

BOOL WINAPI CMGetPS2ColorRenderingDictionary(_In_ HPROFILE hProfile,
                                             DWORD dwIntent,
                                             _Out_writes_bytes_opt_(*lpcbSize)
                                                 LPBYTE lpBuffer,
                                             _Inout_ LPDWORD lpcbSize,
                                             _Inout_ LPBOOL lpbBinary);

BOOL WINAPI CMGetPS2ColorRenderingIntent(_In_ HPROFILE hProfile, DWORD dwIntent,
                                         _Out_writes_bytes_opt_(*lpcbSize)
                                             LPBYTE lpBuffer,
                                         _Inout_ LPDWORD lpcbSize);

BOOL WINAPI CMGetPS2ColorSpaceArray(_In_ HPROFILE hProfile, DWORD dwIntent,
                                    DWORD dwCSAType,
                                    _Out_writes_bytes_opt_(*lpcbSize)
                                        LPBYTE lpBuffer,
                                    _Inout_ LPDWORD lpcbSize,
...
BOOL WINAPI CMIsProfileValid(_In_ HPROFILE hProfile, // proflle handle
...
BOOL WINAPI CMTranslateRGBsExt(_In_ HCMTRANSFORM hcmTransform,
                               _In_ LPVOID lpSrcBits, BMFORMAT bmInput,
                               DWORD dwWidth, DWORD dwHeight,
                               DWORD dwInputStride, _Out_ LPVOID lpDestBits,
                               BMFORMAT bmOutput, DWORD dwOutputStride,
...
HPROFILE WINAPI WcsOpenColorProfileA(
    _In_ PPROFILE pCDMPProfile, _In_opt_ PPROFILE pCAMPProfile,
    _In_opt_ PPROFILE pGMMPProfile, _In_ DWORD dwDesireAccess,
    _In_ DWORD dwShareMode, _In_ DWORD dwCreationMode, _In_ DWORD dwFlags);
...
// Thus, they must use different bits from the INTENT_ flags. Currrently, the
// INTENT_ flags use the values 0-3, or the low order two bits. Hence we use the
// third lowest bit.
//
...
HPROFILE WINAPI WcsCreateIccProfile(_In_ HPROFILE hWcsProfile,
...
BOOL WINAPI WcsGetCalibrationManagementState(_Out_ BOOL *pbIsEnabled);

BOOL WINAPI WcsSetCalibrationManagementState(_In_ BOOL bIsEnabled);
...
/*
 * Associates a color profile (already installed) with a specified display in
 * the given scope.
 *
 * Parameters:
 *    profileManagementScope   specifies the association as system-wide or
 * user-specific profileName              identifies the installed profile to
 * associate targetAdapterID          the target adapter of the display sourceID
 * the source identifier of the display setAsDefault             whether or not
 * to set the newly associated profile as the default associateAsAdvancedColor
 * specifies to which association list the new profile is added
 */
HRESULT WINAPI ColorProfileAddDisplayAssociation(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ PCWSTR profileName,
    _In_ LUID targetAdapterID, _In_ UINT32 sourceID, _In_ BOOL setAsDefault,
    _In_ BOOL associateAsAdvancedColor);

/*
 * Disassociates a color profile (already installed) from a specified display in
 * the given scope.
 *
 * Parameters:
 *    profileManagementScope    specifies the association as system-wide or
 * user-specific profileName               identifies the installed profile to
 * associate targetAdapterID           the target adapter of the display
 *    sourceID                  the source identifier of the display
 *    disassociateAdvancedColor specifies if the association to be removed is
 * Advanced Color
 */
HRESULT WINAPI ColorProfileRemoveDisplayAssociation(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ PCWSTR profileName,
    _In_ LUID targetAdapterID, _In_ UINT32 sourceID,
    _In_ BOOL dissociateAdvancedColor);

/*
 * Sets a given profile (already installed) as the default profile for a
 * specified display in the given scope.
 *
 * Parameters:
 *    profileManagementScope specifies the association as system-wide or
 * user-specific profileName            identifies the installed profile to
 * associate profileType            the color profile type (right now will only
 * accept CPT_ICC) profileSubtype         the color profile subtype to return
 *    targetAdapterID        the target adapter of the display
 *    sourceID               the source identifier of the display
 */
HRESULT WINAPI ColorProfileSetDisplayDefaultAssociation(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ PCWSTR profileName,
    _In_ COLORPROFILETYPE profileType, _In_ COLORPROFILESUBTYPE profileSubType,
    _In_ LUID targetAdapterID, _In_ UINT32 sourceID);

/*
 * Retrieves list of profiles associated with a given display in the specified
 * scope
 *
 * Parameters:
 ...
 *    targetAdapterID the target adapter of the display
 *    sourceID        the source identifier of the display
 *    profileList     pointer to a buffer where the profile names are placed,
 * must be freed with LocalFree profileCount    receives the number of profiles
 * names copied into profileList
 */
HRESULT WINAPI ColorProfileGetDisplayList(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ LUID targetAdapterID,
    _In_ UINT32 sourceID, _Outptr_ LPWSTR **profileList,
 ...
 *    targetAdapterID the target adapter of the display
 *    sourceID        the source identifier of the display
 *    profileType     the color profile type to return (currently only CPT_ICC
 * is supported) profileSubType  the color profile subtype to return profileName
 * on output will have the default color profile name, must be freed with
 *                    LocalFree
 */
HRESULT WINAPI ColorProfileGetDisplayDefault(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ LUID targetAdapterID,
    _In_ UINT32 sourceID, _In_ COLORPROFILETYPE profileType,
    _In_ COLORPROFILESUBTYPE profileSubType, _Outptr_ LPWSTR *profileName);

/*
 * Gets the currently selected color profile scope of the provided display -
 * either user or system
 *
 * Parameters:
 *    targetAdapterID     the target adapter of the display
 *    sourceID            the source identifier of the display
 *    scope               on output will specify if the display's scope is user
 * or system
 */
HRESULT WINAPI
ColorProfileGetDisplayUserScope(_In_ LUID targetAdapterID, _In_ UINT32 sourceID,
 ...
 *
 * Parameters:
 *    scope                 specifies the association as system-wide or
 * user-specific targetAdapterID       the target adapter of the display
 *    sourceID              the source identifier of the display
...
HRESULT WINAPI ColorProfileGetDeviceCapabilities(
    _In_ WCS_PROFILE_MANAGEMENT_SCOPE scope, _In_ LUID targetAdapterID,
    _In_ UINT32 sourceID, _In_ WCS_DEVICE_CAPABILITIES_TYPE capsType,
