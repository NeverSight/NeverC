...
#define CRYPTCAT_OPEN_EXCLUDE_PAGE_HASHES                                      \
  0x00010000 // Exclude page hashes in SPC_INDIRECT_DATA

// The above CRYPTCAT_OPEN_EXCLUDE_PAGE_HASHES takes precedence if also set.
#define CRYPTCAT_OPEN_INCLUDE_PAGE_HASHES                                      \
  0x00020000 // Include page hashes in SPC_INDIRECT_DATA
#define CRYPTCAT_OPEN_VERIFYSIGHASH                                            \
  0x10000000 // verifies the signature (not the certs!)

#define CRYPTCAT_OPEN_NO_CONTENT_HCRYPTMSG                                     \
  0x20000000 // does CryptMsgDecode without content
#define CRYPTCAT_OPEN_SORTED                                                   \
  0x40000000 // does CertCreateContext(CERT_CREATE_CONTEXT_SORTED_FLAG)
...
#define CRYPTCAT_ATTR_DATAREPLACE                                              \
  0x00040000 // this data is a replacment for an existing attr
...
typedef struct CRYPTCATMEMBER_ {
  ...
  struct SIP_INDIRECT_DATA_
      *pIndirectData;  // may be null -- see sEncodedIndirectData
...
//      ERROR_INVALID_PARAMETER:        an input parameter is incorrect
//
extern void WINAPI CryptCATSetCreatePageHashesFlag(
    IN HANDLE hCatalog, IN BOOL newCreatePageHashesFlag);
...
//      ERROR_INVALID_PARAMETER:        an input parameter is incorrect
//
extern CRYPTCATATTRIBUTE *WINAPI
CryptCATGetCatAttrInfo(IN HANDLE hCatalog, _In_ LPWSTR pwszReferenceTag);
...
//      ERROR_INVALID_PARAMETER:        an input parameter is incorrect
//
extern CRYPTCATATTRIBUTE *WINAPI CryptCATPutCatAttrInfo(
    IN HANDLE hCatalog, _In_ LPWSTR pwszReferenceTag,
    IN DWORD dwAttrTypeAndAction, IN DWORD cbData, IN BYTE *pbData);
...
//      ERROR_INVALID_PARAMETER:        an input parameter is incorrect
//
extern CRYPTCATMEMBER *WINAPI
CryptCATGetMemberInfo(IN HANDLE hCatalog, _In_ LPWSTR pwszReferenceTag);
...
//
extern CRYPTCATMEMBER *WINAPI
CryptCATAllocSortedMemberInfo(IN HANDLE hCatalog, _In_ LPWSTR pwszReferenceTag);
...
//
extern CRYPTCATATTRIBUTE *WINAPI
CryptCATGetAttrInfo(IN HANDLE hCatalog, IN CRYPTCATMEMBER *pCatMember,
...
//
extern CRYPTCATMEMBER *WINAPI
CryptCATPutMemberInfo(IN HANDLE hCatalog, _In_opt_ LPWSTR pwszFileName,
                      _In_ LPWSTR pwszReferenceTag, IN GUID *pgSubjectType,
...
//
extern CRYPTCATATTRIBUTE *WINAPI
CryptCATPutAttrInfo(IN HANDLE hCatalog, IN CRYPTCATMEMBER *pCatMember,
                    _In_ LPWSTR pwszReferenceTag, IN DWORD dwAttrTypeAndAction,
...
//
extern CRYPTCATATTRIBUTE *WINAPI
CryptCATEnumerateAttr(IN HANDLE hCatalog, IN CRYPTCATMEMBER *pCatMember,
...
//
extern CRYPTCATCDF *WINAPI
CryptCATCDFOpen(_In_ LPWSTR pwszFilePath,
...
//
extern CRYPTCATATTRIBUTE *WINAPI
CryptCATCDFEnumCatAttributes(CRYPTCATCDF *pCDF, CRYPTCATATTRIBUTE *pPrevAttr,
...
//
extern CRYPTCATMEMBER *WINAPI
CryptCATCDFEnumMembers(IN CRYPTCATCDF *pCDF, IN CRYPTCATMEMBER *pPrevMember,
...
extern CRYPTCATATTRIBUTE *WINAPI CryptCATCDFEnumAttributes(
    IN CRYPTCATCDF *pCDF, IN CRYPTCATMEMBER *pMember,
    IN CRYPTCATATTRIBUTE *pPrevAttr,
    IN OPTIONAL PFN_CDF_PARSE_ERROR_CALLBACK pfnParseError);
...
//
//
_Success_(return != FALSE) extern BOOL WINAPI
    CryptCATAdminAcquireContext(_Out_ HCATADMIN *phCatAdmin,
...
_Success_(return != FALSE) extern BOOL WINAPI CryptCATAdminAcquireContext2(
    _Out_ HCATADMIN *phCatAdmin, _In_opt_ const GUID *pgSubsystem,
...
//      This function uses this parameter to determine the last catalog
//      returned.
//
...
extern HCATINFO WINAPI CryptCATAdminEnumCatalogFromHash(
    _In_ HCATADMIN hCatAdmin, _In_reads_bytes_(cbHash) BYTE *pbHash,
...
//      be TRUE and a call to GetLastError() will equal
...
_Success_(return != FALSE) extern BOOL WINAPI
    CryptCATAdminCalcHashFromFileHandle(
        _In_ HANDLE hFile, _Inout_ DWORD *pcbHash,
...
_Success_(return != FALSE) extern BOOL WINAPI
    CryptCATAdminCalcHashFromFileHandle2(
        _In_ HCATADMIN hCatAdmin, _In_ HANDLE hFile, _Inout_ DWORD *pcbHash,
        _Out_writes_bytes_to_opt_(*pcbHash, *pcbHash) BYTE *pbHash,
        _Reserved_ DWORD dwFlags);
#endif // #if (NTDDI_VERSION >= NTDDI_WIN8)
...
extern HCATINFO WINAPI CryptCATAdminAddCatalog(
    _In_ HCATADMIN hCatAdmin, _In_ PWSTR pwszCatalogFile,
    _In_opt_ PWSTR pwszSelectBaseName, _Reserved_ DWORD dwFlags);
#else
extern HCATINFO WINAPI
CryptCATAdminAddCatalog(_In_ HCATADMIN hCatAdmin, _In_ PWSTR pwszCatalogFile,
                        _In_opt_ PWSTR pwszSelectBaseName, _In_ DWORD dwFlags);
#endif // #if (NTDDI_VERSION < NTDDI_WINBLUE)
...
//
_Success_(return != FALSE) extern BOOL WINAPI
    CryptCATCatalogInfoFromContext(_In_ HCATINFO hCatInfo,
...
//
_Success_(return != FALSE) extern BOOL WINAPI
    CryptCATAdminResolveCatalogPath(_In_ HCATADMIN hCatAdmin,
