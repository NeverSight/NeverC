...
WINBASEAPI
int WINAPI CompareStringEx(_In_opt_ LPCWSTR lpLocaleName, _In_ DWORD dwCmpFlags,
...
WINBASEAPI
int WINAPI CompareStringOrdinal(_In_NLS_string_(cchCount1) LPCWCH lpString1,
                                _In_ int cchCount1,
                                _In_NLS_string_(cchCount2) LPCWCH lpString2,
                                _In_ int cchCount2, _In_ BOOL bIgnoreCase);

#endif //(WINVER >= 0x0600)

WINBASEAPI
int WINAPI CompareStringW(_In_ LCID Locale, _In_ DWORD dwCmpFlags,
...
int WINAPI FoldStringW(_In_ DWORD dwMapFlags,
                       _In_NLS_string_(cchSrc) LPCWCH lpSrcStr, _In_ int cchSrc,
...
WINBASEAPI
BOOL WINAPI GetStringTypeExW(_In_ LCID Locale, _In_ DWORD dwInfoType,
...
WINBASEAPI
_Success_(return != 0)
    _When_((cbMultiByte == -1) && (cchWideChar != 0),
           _Post_equal_to_(_String_length_(lpWideCharStr) + 1)) int WINAPI
    MultiByteToWideChar(_In_ UINT CodePage, _In_ DWORD dwFlags,
...
_Success_(return != 0)
    _When_((cchWideChar == -1) && (cbMultiByte != 0),
           _Post_equal_to_(_String_length_(lpMultiByteStr) + 1)) int WINAPI
    WideCharToMultiByte(_In_ UINT CodePage, _In_ DWORD dwFlags,
                        _In_NLS_string_(cchWideChar) LPCWCH lpWideCharStr,
                        _In_ int cchWideChar,
                        _Out_writes_bytes_to_opt_(cbMultiByte, return)
