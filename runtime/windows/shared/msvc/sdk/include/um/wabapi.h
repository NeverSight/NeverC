...
#define WAB_IWABOBJECT_METHODS(IPURE)                                          \
  ...
  (THIS_ HRESULT hResult, ULONG ulFlags, LPMAPIERROR * lppMAPIError) IPURE;    \
  ...
  (THIS_ _In_ ULONG cbSize, _In_ LPVOID lpObject,                              \
   _Outptr_result_bytebuffer_(cbSize) LPVOID * lppBuffer) IPURE;               \
  ...
  MAPIMETHOD(Find)                                                             \
  (THIS_ _In_ LPADRBOOK lpIAB, _In_opt_ HWND hWnd) IPURE;                      \
  MAPIMETHOD(VCardDisplay)                                                     \
  (THIS_ _In_ LPADRBOOK lpIAB, _In_opt_ HWND hWnd, _In_ LPSTR lpszFileName)    \
      IPURE;                                                                   \
  MAPIMETHOD(LDAPUrl)                                                          \
  (THIS_ _In_ LPADRBOOK lpIAB, _In_opt_ HWND hWnd, ULONG ulFlags,              \
   _In_ LPSTR lpszURL, _Outptr_result_maybenull_ LPMAILUSER * lppMailUser)     \
      IPURE;                                                                   \
  MAPIMETHOD(VCardCreate)                                                      \
  (THIS_ _In_ LPADRBOOK lpIAB, ULONG ulFlags, _In_ LPSTR lpszVCard,            \
   _In_ LPMAILUSER lpMailUser) IPURE;                                          \
  MAPIMETHOD(VCardRetrieve)                                                    \
  (THIS_ _In_ LPADRBOOK lpIAB, ULONG ulFlags, _In_ LPSTR lpszVCard,            \
   _Outptr_ LPMAILUSER * lppMailUser) IPURE;                                   \
  MAPIMETHOD(GetMe)                                                            \
  (THIS_ _In_ LPADRBOOK lpIAB, ULONG ulFlags, _Out_ DWORD * lpdwAction,        \
   _Out_ SBinary * lpsbEID, _In_opt_ HWND hwnd) IPURE;                         \
  MAPIMETHOD(SetMe)                                                            \
  (THIS_ _In_ LPADRBOOK lpIAB, ULONG ulFlags, SBinary sbEID,                   \
...
typedef struct _tagWAB_PARAM {
  ...
  GUID guidPSExt;   // A GUID that identifies the calling application's Property
                  // Sheet extensions The GUID can be used to determine whether
                  // the extension prop sheets are displayed or not.
} WAB_PARAM, *LPWAB_PARAM;

// flags for WAB_PARAM
#define WAB_USE_OE_SENDMAIL                                                    \
  0x00000001 // Tells WAB to use Outlook Express for e-mail before checking for
             // a default Simple MAPI client. Default behaviour is to check for
             // the Simple MAPI client first

#define WAB_ENABLE_PROFILES                                                    \
  0x00400000 // Invokes WAB in a Identity-aware session using Identity-Manager
             // based profiles
             // DEPRECATED IN VISTA.
...
//
STDMETHODIMP WABOpen(_Outptr_ LPADRBOOK *lppAdrBook,
                     _Outptr_ LPWABOBJECT *lppWABObject,
                     _In_opt_ LPWAB_PARAM lpWP, DWORD Reserved2);

typedef HRESULT(STDMETHODCALLTYPE WABOPEN)(LPADRBOOK FAR *lppAdrBook,
...
STDMETHODIMP WABOpenEx(_Outptr_ LPADRBOOK *lppAdrBook,
                       _Outptr_ LPWABOBJECT *lppWABObject,
                       _In_opt_ LPWAB_PARAM lpWP, DWORD Reserved,
                       ALLOCATEBUFFER *fnAllocateBuffer,
                       ALLOCATEMORE *fnAllocateMore, FREEBUFFER *fnFreeBuffer);

typedef HRESULT(STDMETHODCALLTYPE WABOPENEX)(LPADRBOOK FAR *lppAdrBook,
                                             LPWABOBJECT FAR *lppWABObject,
...
typedef struct _WABIMPORTPARAM {
  ...
  LPSTR lpszFileName;  // FileName to import or NULL .. if NULL will show
                       // FileOpen dialog
...
// Flags
#define WAB_DISPLAY_LDAPURL                                                    \
  0x00000001 // The object being displayed is an LDAP URL
             // The URL can be found in the lpsz struct member

#define WAB_CONTEXT_ADRLIST                                                    \
  0x00000002 // THe lpv parameter contains a pointer to an
             // AdrList structure corresponding to selected items
             // on which to display a context menu

#define WAB_DISPLAY_ISNTDS                                                     \
  0x00000004 // Identifies that the entry being displayed originated
             // on the NT Directory Service, for clients that use ADSI and
             // retrieve additional information from the service.

//      MAPI_UNICODE        0x80000000  // Indicates that the WED.lpsz string is
//      actually a UNICODE
