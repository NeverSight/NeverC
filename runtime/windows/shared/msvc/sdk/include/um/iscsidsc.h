...
#define SECURITY_FLAG_QUALIFIERS                                               \
  description("Security flags")                                                \
      : amended, BitMap{ISCSI_SECURITY_FLAG_TUNNEL_MODE_PREFERRED,             \
        ...
        BitValues{"Tunnel mode preferred",                                     \
                  "Transport mode preferred",                                  \
                  "PFS Enabled",                                               \
                  "Aggressive mode enabled",                                   \
                  "Main Mode Enabled",                                         \
                  "IKE/IPSec Enabled",                                         \
                  "ISCSI_SECURITY_FLAGS are valid"} : amended
...
#define ISCSI_LOGIN_FLAGS_QUALIFIERS                                           \
  BitValues{"Require IPSEC", "Multipath Enabled", "Reserved1",                 \
            "Allow Portal Hopping"},                                           \
      BitMap{"0x00000001", "0x00000002", "0x00000004", "0x00000008"}

#define ISCSI_LOGIN_FLAGS uint32

#define ISCSI_AUTH_TYPES_QUALIFIERS                                            \
  Values{"No Authentication", "CHAP", "Mutual CHAP"} : amended,                \
                                                       ValueMap{"0", "1", "2"}
...
#define ISCSI_LOGIN_OPTIONS_INFO_QUALIFIERS                                    \
  BitMap{ISCSI_LOGIN_OPTIONS_HEADER_DIGEST, ISCSI_LOGIN_OPTIONS_DATA_DIGEST,   \
...
#define IKE_IDENTIFICATION_TYPE_QUALIFIERS                                     \
  ValueMap{ID_IPV4_ADDR, ID_FQDN, ID_USER_FQDN, ID_IPV6_ADDR},                 \
      Values{"ID_IPV4_ADDR", "ID_FQDN", "ID_USER_FQDN", "ID_IPV6_ADDR"}
...
typedef enum {
  ...
  // Requires DiscoveryMechansim parameter
  PortalGroups, // Portal Groups - array of portal groups, preceeded by a ULONG
                // count. Each Portal Group must be aliagned on a 4 byte boundry
...
ISDSC_STATUS
ISDSC_API GetIScsiVersionInformation(PISCSI_VERSION_INFO VersionInfo);

ISDSC_STATUS ISDSC_API GetIScsiTargetInformationW(
    IN _In_ PWSTR TargetName, IN _In_opt_ PWSTR DiscoveryMechanism,
    IN TARGET_INFORMATION_CLASS InfoClass, IN OUT PULONG BufferSize,
    OUT PVOID Buffer);

ISDSC_STATUS ISDSC_API GetIScsiTargetInformationA(
    IN _In_ PSTR TargetName, IN _In_opt_ PSTR DiscoveryMechanism,
    IN TARGET_INFORMATION_CLASS InfoClass, IN OUT PULONG BufferSize,
    OUT PVOID Buffer);
...
ISDSC_STATUS ISDSC_API AddIScsiConnectionW(
    IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId, IN PVOID Reserved,
    IN ULONG InitiatorPortNumber, IN PISCSI_TARGET_PORTALW TargetPortal,
    IN OPTIONAL ISCSI_SECURITY_FLAGS SecurityFlags,
    IN OPTIONAL PISCSI_LOGIN_OPTIONS LoginOptions, IN OPTIONAL ULONG KeySize,
...
ISDSC_STATUS ISDSC_API AddIScsiConnectionA(
    IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId, IN PVOID Reserved,
    IN ULONG InitiatorPortNumber, IN PISCSI_TARGET_PORTALA TargetPortal,
    IN OPTIONAL ISCSI_SECURITY_FLAGS SecurityFlags,
    IN OPTIONAL PISCSI_LOGIN_OPTIONS LoginOptions, IN OPTIONAL ULONG KeySize,
    IN OPTIONAL _In_reads_opt_(KeySize) PCHAR Key,
    OUT OPTIONAL PISCSI_UNIQUE_CONNECTION_ID ConnectionId);
...
ISDSC_STATUS ISDSC_API
RemoveIScsiConnection(IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId,
                      IN PISCSI_UNIQUE_CONNECTION_ID ConnectionId);

ISDSC_STATUS ISDSC_API ReportIScsiTargetsW(IN BOOLEAN ForceUpdate,
                                           IN OUT PULONG BufferSize,
                                           OUT _Inout_updates_opt_(*BufferSize)
                                               PWCHAR Buffer);

ISDSC_STATUS ISDSC_API ReportIScsiTargetsA(IN BOOLEAN ForceUpdate,
                                           IN OUT PULONG BufferSize,
                                           OUT _Inout_updates_opt_(*BufferSize)
                                               PCHAR Buffer);
...
ISDSC_STATUS ISDSC_API AddIScsiStaticTargetW(
    IN _In_ PWSTR TargetName, IN OPTIONAL _In_opt_ PWSTR TargetAlias,
    IN ISCSI_TARGET_FLAGS TargetFlags, IN BOOLEAN Persist,
...
ISDSC_STATUS ISDSC_API AddIScsiStaticTargetA(
    IN _In_ PSTR TargetName, IN OPTIONAL _In_opt_ PSTR TargetAlias,
    IN ISCSI_TARGET_FLAGS TargetFlags, IN BOOLEAN Persist,
...
ISDSC_STATUS ISDSC_API RemoveIScsiStaticTargetW(IN _In_ PWSTR TargetName);

ISDSC_STATUS ISDSC_API RemoveIScsiStaticTargetA(IN _In_ PSTR TargetName);
...
ISDSC_STATUS ISDSC_API
AddIScsiSendTargetPortalW(IN OPTIONAL _In_opt_ PWSTR InitiatorInstance,
...
ISDSC_STATUS ISDSC_API
AddIScsiSendTargetPortalA(IN OPTIONAL _In_opt_ PSTR InitiatorInstance,
                          IN OPTIONAL ULONG InitiatorPortNumber,
                          IN OPTIONAL PISCSI_LOGIN_OPTIONS LoginOptions,
                          IN OPTIONAL ISCSI_SECURITY_FLAGS SecurityFlags,
...
ISDSC_STATUS ISDSC_API RemoveIScsiSendTargetPortalW(
    ...
    IN OPTIONAL ULONG InitiatorPortNumber, IN PISCSI_TARGET_PORTALW Portal);

ISDSC_STATUS ISDSC_API RemoveIScsiSendTargetPortalA(
    IN OPTIONAL PSTR _In_opt_ InitiatorInstance,
    IN OPTIONAL ULONG InitiatorPortNumber, IN PISCSI_TARGET_PORTALA Portal);
...
ISDSC_STATUS ISDSC_API ReportIScsiSendTargetPortalsW(
    IN OUT PULONG PortalCount, IN OUT PISCSI_TARGET_PORTAL_INFOW PortalInfo);

ISDSC_STATUS ISDSC_API ReportIScsiSendTargetPortalsA(
    IN OUT PULONG PortalCount, IN OUT PISCSI_TARGET_PORTAL_INFOA PortalInfo);
#ifdef UNICODE
...
ISDSC_STATUS ISDSC_API ReportIScsiSendTargetPortalsExW(
    OUT PULONG PortalCount, IN OUT PULONG PortalInfoSize,
...
ISDSC_STATUS ISDSC_API LoginIScsiTargetW(
    IN _In_ PWSTR TargetName, IN BOOLEAN IsInformationalSession,
    ...
    IN OPTIONAL PISCSI_TARGET_MAPPINGW Mappings,
    IN OPTIONAL PISCSI_LOGIN_OPTIONS LoginOptions, IN OPTIONAL ULONG KeySize,
    IN OPTIONAL _In_reads_opt_(KeySize) PCHAR Key, IN BOOLEAN IsPersistent,
...
ISDSC_STATUS ISDSC_API LoginIScsiTargetA(
    IN PSTR _In_ TargetName, IN BOOLEAN IsInformationalSession,
...
ISDSC_STATUS ISDSC_API ReportIScsiPersistentLoginsW(
    OUT ULONG *Count, IN OUT PPERSISTENT_ISCSI_LOGIN_INFOW PersistentLoginInfo,
    IN OUT PULONG BufferSizeInBytes);

ISDSC_STATUS ISDSC_API ReportIScsiPersistentLoginsA(
    OUT ULONG *Count, IN OUT PPERSISTENT_ISCSI_LOGIN_INFOA PersistentLoginInfo,
    IN OUT PULONG BufferSizeInBytes);
...
ISDSC_STATUS
ISDSC_API LogoutIScsiTarget(IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId);

ISDSC_STATUS ISDSC_API RemoveIScsiPersistentTargetW(
    IN PWSTR _In_ InitiatorInstance, IN OPTIONAL ULONG InitiatorPortNumber,
    IN PWSTR _In_ TargetName, IN PISCSI_TARGET_PORTALW Portal);

ISDSC_STATUS ISDSC_API RemoveIScsiPersistentTargetA(
    IN PSTR _In_ InitiatorInstance, IN OPTIONAL ULONG InitiatorPortNumber,
    IN PSTR _In_ TargetName, IN PISCSI_TARGET_PORTALA Portal);
...
ISDSC_STATUS ISDSC_API
SendScsiInquiry(IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId, IN ULONGLONG Lun,
                IN UCHAR EvpdCmddt, IN UCHAR PageCode, OUT PUCHAR ScsiStatus,
                IN OUT PULONG ResponseSize, OUT PUCHAR ResponseBuffer,
                IN OUT PULONG SenseSize, OUT PUCHAR SenseBuffer);

ISDSC_STATUS ISDSC_API SendScsiReadCapacity(
    IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId, IN ULONGLONG Lun,
    OUT PUCHAR ScsiStatus, IN OUT PULONG ResponseSize,
    OUT PUCHAR ResponseBuffer, IN OUT PULONG SenseSize, OUT PUCHAR SenseBuffer);

ISDSC_STATUS ISDSC_API SendScsiReportLuns(
    IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId, OUT PUCHAR ScsiStatus,
    IN OUT PULONG ResponseSize, OUT PUCHAR ResponseBuffer,
    IN OUT PULONG SenseSize, OUT PUCHAR SenseBuffer);

ISDSC_STATUS ISDSC_API
ReportIScsiInitiatorListW(IN OUT ULONG *BufferSize,
                          OUT _Inout_updates_opt_(*BufferSize) PWCHAR Buffer);

ISDSC_STATUS ISDSC_API
ReportIScsiInitiatorListA(IN OUT PULONG BufferSize,
...
ISDSC_STATUS ISDSC_API ReportActiveIScsiTargetMappingsW(
    IN OUT PULONG BufferSize, OUT PULONG MappingCount,
...
ISDSC_STATUS ISDSC_API SetIScsiTunnelModeOuterAddressW(
    IN OPTIONAL _In_opt_ PWSTR InitiatorName, IN ULONG InitiatorPortNumber,
    IN PWSTR _In_opt_ DestinationAddress, IN PWSTR _In_opt_ OuterModeAddress,
    IN BOOLEAN Persist);

ISDSC_STATUS ISDSC_API SetIScsiTunnelModeOuterAddressA(
    IN OPTIONAL _In_opt_ PSTR InitiatorName, IN ULONG InitiatorPortNumber,
    IN PSTR _In_opt_ DestinationAddress, IN PSTR _In_opt_ OuterModeAddress,
    IN BOOLEAN Persist);
...
ISDSC_STATUS ISDSC_API SetIScsiIKEInfoW(
    IN OPTIONAL _In_opt_ PWSTR InitiatorName, IN ULONG InitiatorPortNumber,
    IN PIKE_AUTHENTICATION_INFORMATION AuthInfo, IN BOOLEAN Persist);
...
ISDSC_STATUS
ISDSC_API GetIScsiIKEInfoW(IN OPTIONAL _In_opt_ PWSTR InitiatorName,
                           IN ULONG InitiatorPortNumber, IN PULONG Reserved,
                           IN OUT PIKE_AUTHENTICATION_INFORMATION AuthInfo);

ISDSC_STATUS ISDSC_API GetIScsiIKEInfoA(
    IN OPTIONAL _In_opt_ PSTR InitiatorName, IN ULONG InitiatorPortNumber,
    IN PULONG Reserved, IN OUT PIKE_AUTHENTICATION_INFORMATION AuthInfo);
...
ISDSC_STATUS ISDSC_API SetIScsiGroupPresharedKey(IN ULONG KeyLength,
                                                 IN PUCHAR Key,
                                                 BOOLEAN Persist);

ISDSC_STATUS ISDSC_API SetIScsiInitiatorCHAPSharedSecret(
    IN ULONG SharedSecretLength, IN PUCHAR SharedSecret);

ISDSC_STATUS ISDSC_API SetIScsiInitiatorRADIUSSharedSecret(
    IN ULONG SharedSecretLength, IN PUCHAR SharedSecret);

ISDSC_STATUS ISDSC_API
SetIScsiInitiatorNodeNameW(IN OPTIONAL _In_opt_ PWSTR InitiatorNodeName);

ISDSC_STATUS ISDSC_API
SetIScsiInitiatorNodeNameA(IN OPTIONAL _In_opt_ PSTR InitiatorNodeName);
...
ISDSC_STATUS
ISDSC_API GetIScsiInitiatorNodeNameW(_Out_writes_(MAX_ISCSI_NAME_LEN + 1)
...
ISDSC_STATUS ISDSC_API AddISNSServerW(IN _In_ PWSTR Address);

ISDSC_STATUS ISDSC_API AddISNSServerA(IN _In_ PSTR Address);
...
ISDSC_STATUS ISDSC_API RemoveISNSServerW(IN _In_ PWSTR Address);

ISDSC_STATUS ISDSC_API RemoveISNSServerA(IN _In_ PSTR Address);
#ifdef UNICODE
...
ISDSC_STATUS ISDSC_API RefreshISNSServerW(IN _In_ PWSTR Address);

ISDSC_STATUS ISDSC_API RefreshISNSServerA(IN _In_ PSTR Address);
#ifdef UNICODE
...
ISDSC_STATUS
ISDSC_API ReportISNSServerListW(IN OUT PULONG BufferSizeInChar,
                                OUT _Out_writes_opt_(*BufferSizeInChar)
                                    PWCHAR Buffer);

ISDSC_STATUS ISDSC_API
ReportISNSServerListA(IN OUT PULONG BufferSizeInChar,
...
ISDSC_STATUS ISDSC_API
GetIScsiSessionListW(IN OUT ULONG *BufferSize, OUT ULONG *SessionCount,
                     OUT OPTIONAL PISCSI_SESSION_INFOW SessionInfo);

ISDSC_STATUS ISDSC_API
GetIScsiSessionListA(IN OUT ULONG *BufferSize, OUT ULONG *SessionCount,
...
ISDSC_STATUS ISDSC_API
GetIScsiSessionListEx(IN OUT ULONG *BufferSize, OUT ULONG *SessionCountPtr,
                      OUT OPTIONAL PISCSI_SESSION_INFO_EX SessionInfo);

ISDSC_STATUS ISDSC_API GetDevicesForIScsiSessionW(
    IN PISCSI_UNIQUE_SESSION_ID UniqueSessionId, IN OUT ULONG *DeviceCount,
...
ISDSC_STATUS ISDSC_API AddPersistentIScsiDeviceW(IN _In_ PWSTR DevicePath);

ISDSC_STATUS ISDSC_API AddPersistentIScsiDeviceA(IN _In_ PSTR DevicePath);
...
ISDSC_STATUS ISDSC_API RemovePersistentIScsiDeviceW(IN _In_ PWSTR DevicePath);

ISDSC_STATUS ISDSC_API RemovePersistentIScsiDeviceA(IN _In_ PSTR DevicePath);
...
ISDSC_STATUS ISDSC_API ReportIScsiTargetPortalsW(
    IN OPTIONAL _In_opt_ PWSTR InitiatorName, IN PWSTR _In_ TargetName,
    IN OPTIONAL PUSHORT TargetPortalTag, IN OUT PULONG ElementCount,
    OUT PISCSI_TARGET_PORTALW Portals);

ISDSC_STATUS ISDSC_API ReportIScsiTargetPortalsA(
    IN OPTIONAL _In_opt_ PSTR InitiatorName, IN PSTR _In_ TargetName,
    IN OPTIONAL PUSHORT TargetPortalTag, IN OUT PULONG ElementCount,
...
ISDSC_STATUS ISDSC_API AddRadiusServerW(IN _In_ PWSTR Address);

ISDSC_STATUS ISDSC_API AddRadiusServerA(IN _In_ PSTR Address);
...
ISDSC_STATUS ISDSC_API RemoveRadiusServerW(IN _In_ PWSTR Address);

ISDSC_STATUS ISDSC_API RemoveRadiusServerA(IN _In_ PSTR Address);
#ifdef UNICODE
...
ISDSC_STATUS
ISDSC_API ReportRadiusServerListW(IN OUT PULONG BufferSizeInChar,
                                  OUT _Out_writes_opt_(*BufferSizeInChar)
                                      PWCHAR Buffer);

ISDSC_STATUS ISDSC_API
ReportRadiusServerListA(IN OUT PULONG BufferSizeInChar,
                        OUT _Out_writes_opt_(*BufferSizeInChar) PCHAR Buffer);
#ifdef UNICODE
