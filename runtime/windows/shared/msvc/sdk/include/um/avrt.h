...
_Success_(return != NULL) AVRTAPI HANDLE WINAPI
    AvSetMmThreadCharacteristicsA(_In_ LPCSTR TaskName,
                                  _Inout_ LPDWORD TaskIndex);
_Success_(return != NULL) AVRTAPI HANDLE WINAPI
    AvSetMmThreadCharacteristicsW(_In_ LPCWSTR TaskName,
                                  _Inout_ LPDWORD TaskIndex);
#ifdef UNICODE
...
_Success_(return != NULL) AVRTAPI HANDLE WINAPI
    AvSetMmMaxThreadCharacteristicsA(_In_ LPCSTR FirstTask,
                                     _In_ LPCSTR SecondTask,
                                     _Inout_ LPDWORD TaskIndex);
_Success_(return != NULL) AVRTAPI HANDLE WINAPI
    AvSetMmMaxThreadCharacteristicsW(_In_ LPCWSTR FirstTask,
...
_Success_(return != FALSE) AVRTAPI BOOL WINAPI
    AvRevertMmThreadCharacteristics(_In_ HANDLE AvrtHandle);

_Success_(return != FALSE) AVRTAPI BOOL WINAPI
    AvSetMmThreadPriority(_In_ HANDLE AvrtHandle, _In_ AVRT_PRIORITY Priority);
...
_Success_(return != FALSE) AVRTAPI BOOL WINAPI
    AvRtWaitOnThreadOrderingGroup(_In_ HANDLE Context);

_Success_(return != FALSE) AVRTAPI BOOL WINAPI
    AvRtLeaveThreadOrderingGroup(_In_ HANDLE Context);

_Success_(return != FALSE) AVRTAPI BOOL WINAPI
    AvRtDeleteThreadOrderingGroup(_In_ HANDLE Context);

_Success_(return != FALSE) AVRTAPI BOOL WINAPI
    AvQuerySystemResponsiveness(_In_ HANDLE AvrtHandle,
