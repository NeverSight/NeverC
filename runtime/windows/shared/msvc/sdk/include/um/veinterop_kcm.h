...
InitializeUserBoundKeySession(
    ...
    _Outptr_result_buffer_(*reportSize) void **report, _Out_ UINT32 *reportSize,
...
CreateUserBoundKeyRequestForRetrieveAuthorizationContext(
    _Inout_ USER_BOUND_KEY_SESSION_HANDLE sessionHandle, _In_ PCWSTR keyName,
    _Out_ UINT64 *nonce,
...
GetUserBoundKeyAuthContext(
    ...
    _In_ UINT32 authContextBlobSize, _In_ UINT64 nonce,
 ...
 * @param authContextHandle Handle to the auth context from
 * GetUserBoundKeyAuthContext
 * @param count Number of properties to validate
 * @param values Array of property validation structures
 *
 * @return S_OK on success, E_INVALIDARG for validation failures, or
 * E_ACCESSDENIED for authorization failures
 */
STDAPI
 ...
 */
STDAPI
ProtectUserBoundKey(_In_ USER_BOUND_KEY_AUTH_CONTEXT_HANDLE authContext,
 ...
 */
STDAPI
CloseUserBoundKeyAuthContext(_In_ USER_BOUND_KEY_AUTH_CONTEXT_HANDLE handle);
 ...
 */
STDAPI
CloseUserBoundKeySession(_In_ USER_BOUND_KEY_SESSION_HANDLE sessionHandle);
...
CreateUserBoundKeyRequestForDeriveSharedSecret(
    ...
    _In_ UINT32 publicKeyBytesSize, _Out_ UINT64 *nonce,
    _Outptr_result_buffer_(*encryptedRequestSize) void **encryptedRequest,
    _Out_ UINT32 *encryptedRequestSize);
 ...
 */
STDAPI
UnprotectUserBoundKey(_In_ USER_BOUND_KEY_SESSION_HANDLE sessionHandle,
                      _In_ USER_BOUND_KEY_AUTH_CONTEXT_HANDLE authContext,
                      _In_reads_bytes_(sessionEncryptedDerivedSecretSize)
                          const void *sessionEncryptedDerivedSecret,
                      _In_ UINT32 sessionEncryptedDerivedSecretSize,
                      _In_reads_bytes_(encryptedUserBoundKeySize)
                          const void *encryptedUserBoundKey,
                      _In_ UINT32 encryptedUserBoundKeySize, _In_ UINT64 nonce,
