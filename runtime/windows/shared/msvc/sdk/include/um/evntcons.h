...
EVNTCONS_INLINE
ULONG
GetEventProcessorIndex(_In_ PCEVENT_RECORD EventRecord) {
...
typedef enum {
  ...
  EtwProviderTraitDecodeGuid =
      2, // Decode GUID (when different from control GUID)
  EtwProviderTraitTypeMax
} ETW_PROVIDER_TRAIT_TYPE;

EVNTCONS_INLINE
VOID EtwGetTraitFromProviderTraits(_In_ PVOID ProviderTraits,
                                   _In_ UCHAR TraitType, _Out_ PVOID *Trait,
                                   _Out_ PUSHORT Size) {
...
ULONG
EVNTAPI
EventAccessControl(_In_ LPGUID Guid, _In_ ULONG Operation, _In_ PSID Sid,
                   _In_ ULONG Rights, _In_ BOOLEAN AllowOrDeny);
#endif

#if (WINVER >= _WIN32_WINNT_LONGHORN)
ULONG
EVNTAPI
EventAccessQuery(_In_ LPGUID Guid,
...
ULONG
EVNTAPI
EventAccessRemove(_In_ LPGUID Guid);
#endif
