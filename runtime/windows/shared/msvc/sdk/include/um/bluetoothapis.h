...
typedef struct _BLUETOOTH_LOCAL_SERVICE_INFO {
  ...
  BLUETOOTH_ADDRESS
  btAddr; //  If service is to be advertised for a particular remote device

  WCHAR szName[BLUETOOTH_MAX_SERVICE_NAME_SIZE];    //  SDP Service Name to be
                                                    //  advertised.
  WCHAR szDeviceString[BLUETOOTH_DEVICE_NAME_SIZE]; //  Local device name (if
                                                    //  any) like COM4 or LPT1
...
//          handle.
//
_Must_inspect_result_ _Success_(return != NULL) HBLUETOOTH_RADIO_FIND WINAPI
    BluetoothFindFirstRadio(_In_ const BLUETOOTH_FIND_RADIO_PARAMS *pbtfrp,
...
//          other Win32 errors
//
_Must_inspect_result_ _Success_(return) BOOL WINAPI
    BluetoothFindNextRadio(_In_ HBLUETOOTH_RADIO_FIND hFind,
                           _Out_ HANDLE *phRadio);
...
//              The handle is NULL.
//
BOOL WINAPI BluetoothFindRadioClose(_In_ HBLUETOOTH_RADIO_FIND hFind);
...
typedef struct _BLUETOOTH_RADIO_INFO {
  ...
  USHORT
  manufacturer; // Manufacturer of the radio, BTH_MFG_Xxx value.  For the
                // most up to date list, goto the Bluetooth specification
                // website and get the Bluetooth assigned numbers document.
...
//
_Must_inspect_result_ DWORD WINAPI BluetoothGetRadioInfo(
    _In_ HANDLE hRadio, _Inout_ PBLUETOOTH_RADIO_INFO pRadioInfo);
...
typedef struct _BLUETOOTH_DEVICE_INFO {
  ...
  SYSTEMTIME stLastUsed; //  Last time the device was used for other than RNR,
                         //  inquiry, or SDP
...
typedef struct _BLUETOOTH_DEVICE_SEARCH_PARAMS {
  ...
  HANDLE hRadio; //  IN  handle to radio to enumerate - NULL == all radios will
                 //  be searched
...
//
_Must_inspect_result_ _Success_(return != NULL) HBLUETOOTH_DEVICE_FIND WINAPI
    BluetoothFindFirstDevice(_In_ const BLUETOOTH_DEVICE_SEARCH_PARAMS *pbtsp,
...
//          other Win32 errors
//
_Must_inspect_result_ BOOL WINAPI BluetoothFindNextDevice(
    _In_ HBLUETOOTH_DEVICE_FIND hFind, _Inout_ BLUETOOTH_DEVICE_INFO *pbtdi);
...
//              The handle is NULL.
//
BOOL WINAPI BluetoothFindDeviceClose(_In_ HBLUETOOTH_DEVICE_FIND hFind);
...
//      other error codes
//
_Must_inspect_result_ DWORD WINAPI BluetoothGetDeviceInfo(
    _In_opt_ HANDLE hRadio, _Inout_ BLUETOOTH_DEVICE_INFO *pbtdi);
...
BluetoothUpdateDeviceRecord(_In_ const BLUETOOTH_DEVICE_INFO *pbtdi);
...
DWORD
WINAPI
BluetoothRemoveDevice(_In_ const BLUETOOTH_ADDRESS *pAddress);
...
//              }
//
//              pbtdi = (BLUETOOTH_DEVICE_INFO *) ((LPBYTE)pbtdi +
//              pbtdi->dwSize);
//          }
//
...
typedef BOOL(WINAPI *PFN_DEVICE_CALLBACK)(LPVOID pvParam,
                                          const BLUETOOTH_DEVICE_INFO *pDevice);

typedef struct _BLUETOOTH_SELECT_DEVICE_PARAMS {
  DWORD dwSize; //  IN  sizeof this structure

  ULONG cNumOfClasses; //  IN  Number in prgClassOfDevice - if ZERO search for
                       //  all devices
  ...
  BOOL fForceAuthentication; //  IN  If TRUE, authenication will be forced
                             //  before returning
  BOOL fShowAuthenticated; //  IN  If TRUE, authenticated devices will be shown
                           //  in the picker
  BOOL fShowRemembered; //  IN  If TRUE, remembered devices will be shown in the
                        //  picker
  BOOL fShowUnknown; //  IN  If TRUE, unknown devices that are not authenticated
                     //  or "remember" will be shown.

  BOOL fAddNewDeviceWizard; //  IN  If TRUE, invokes the add new device wizard.
  BOOL fSkipServicesPage;   //  IN  If TRUE, skips the "Services" page in the
                            //  wizard.

  PFN_DEVICE_CALLBACK
  pfnDeviceCallback; //  IN  If non-NULL, a callback that will be called for
                     //  each device. If the
                     //      the callback returns TRUE, the item will be
                     //      added. If the callback is is FALSE, the item
                     //      will not be shown.
  LPVOID pvParam;    //  IN  Parameter to be passed to pfnDeviceCallback as the
                     //  pvParam.

  DWORD cNumDevices; //  IN  number calles wants - ZERO == no limit.
                     //  OUT the number of devices returned.

  _Field_size_opt_(cNumDevices) PBLUETOOTH_DEVICE_INFO
      pDevices; //  OUT pointer to an array for BLUETOOTH_DEVICE_INFOs.
...
BluetoothSelectDevices(_Inout_ BLUETOOTH_SELECT_DEVICE_PARAMS *pbtsdp);
...
//
BOOL WINAPI
BluetoothSelectDevicesFree(_Inout_ BLUETOOTH_SELECT_DEVICE_PARAMS *pbtsdp);
...
//          for more details.
//
BOOL WINAPI BluetoothDisplayDeviceProperties(
    _In_opt_ HWND hwndParent, _Inout_ BLUETOOTH_DEVICE_INFO *pbtdi);
...
//
_Must_inspect_result_ DWORD WINAPI
BluetoothAuthenticateDevice(_In_opt_ HWND hwndParent, _In_opt_ HANDLE hRadio,
...
//      after which the authentication request will be sent. The user will see
//      the success or failure of the authentication attempt. The user will also
//      be given the oppurtunity to try to fix a failed authentication.
//
...
//          then UI is displayed and the user steps through the authentication
//          process. If not NULL, no UI is shown.
//
...
//          },
//
_Must_inspect_result_ DWORD WINAPI BluetoothAuthenticateDeviceEx(
    _In_opt_ HWND hwndParentIn, _In_opt_ HANDLE hRadioIn,
...
//          Failure. Return value is the error code.
//
_Must_inspect_result_ DWORD WINAPI BluetoothAuthenticateMultipleDevices(
    _In_opt_ HWND hwndParent, _In_opt_ HANDLE hRadio, _In_ DWORD cDevices,
...
//          The call failed.
//
_Must_inspect_result_ DWORD WINAPI BluetoothSetServiceState(
    _In_opt_ HANDLE hRadio, _In_ const BLUETOOTH_DEVICE_INFO *pbtdi,
    _In_ const GUID *pGuidService, _In_ DWORD dwServiceFlags);
...
//          The call failed.
//
_Must_inspect_result_ _Success_(return == ERROR_SUCCESS) DWORD WINAPI
    BluetoothEnumerateInstalledServices(_In_opt_ HANDLE hRadio,
                                        _In_ const BLUETOOTH_DEVICE_INFO *pbtdi,
                                        _Inout_ DWORD *pcServiceInout,
                                        _Out_writes_to_opt_(*pcServiceInout,
                                                            *pcServiceInout)
                                            GUID *pGuidServices);
...
//
BOOL WINAPI BluetoothEnableDiscovery(_In_opt_ HANDLE hRadio,
                                     _In_ BOOL fEnabled);
...
//
_Must_inspect_result_ BOOL WINAPI
BluetoothIsDiscoverable(_In_opt_ HANDLE hRadio);
...
//
_Must_inspect_result_ BOOL WINAPI
BluetoothEnableIncomingConnections(_In_opt_ HANDLE hRadio, _In_ BOOL fEnabled);
...
//
_Must_inspect_result_ BOOL WINAPI
BluetoothIsConnectable(_In_opt_ HANDLE hRadio);
...
//
_Must_inspect_result_ DWORD WINAPI BluetoothRegisterForAuthentication(
    _In_opt_ const BLUETOOTH_DEVICE_INFO *pbtdi,
    _Out_ HBLUETOOTH_AUTHENTICATION_REGISTRATION *phRegHandle,
    _In_opt_ PFN_AUTHENTICATION_CALLBACK pfnCallback, _In_opt_ PVOID pvParam);
...
typedef BOOL(CALLBACK *PFN_AUTHENTICATION_CALLBACK_EX)(
    ...
    _In_ PBLUETOOTH_AUTHENTICATION_CALLBACK_PARAMS pAuthCallbackParams);
...
//          Failure. The registration handle is invalid.
//
_Must_inspect_result_ DWORD WINAPI BluetoothRegisterForAuthenticationEx(
...
//          other Win32 errors.
//
BOOL WINAPI BluetoothUnregisterAuthentication(
...
//          there is a communications problem with the local radio.
//
//      E_FAIL
...
//      other Win32 error codes
//
_Must_inspect_result_ DWORD WINAPI BluetoothSendAuthenticationResponse(
    _In_opt_ HANDLE hRadio, _In_ const BLUETOOTH_DEVICE_INFO *pbtdi,
...
//      other Win32 error codes
//
_Must_inspect_result_ DWORD WINAPI BluetoothSendAuthenticationResponseEx(
...
//
_Must_inspect_result_ _Success_(return == ERROR_SUCCESS) DWORD WINAPI
    BluetoothSdpGetElementData(_In_reads_bytes_(cbSdpStreamLength)
                                   LPBYTE pSdpStream,
...
//
_Must_inspect_result_ _Success_(return == ERROR_SUCCESS) DWORD WINAPI
    BluetoothSdpGetAttributeValue(_In_reads_bytes_(cbRecordLength)
...
//
_Must_inspect_result_ _Success_(return == 0) DWORD WINAPI
    BluetoothSdpGetString(_In_reads_bytes_(cbRecordLength) LPBYTE pRecordStream,
                          _In_ ULONG cbRecordLength,
                          _In_opt_ const PSDP_STRING_TYPE_DATA pStringData,
                          _In_ USHORT usStringOffset,
                          _Out_writes_to_(*pcchStringLength, *pcchStringLength)
                              PWSTR pszString,
...
typedef BOOL(CALLBACK *PFN_BLUETOOTH_ENUM_ATTRIBUTES_CALLBACK)(
    _In_ ULONG uAttribId, _In_reads_bytes_(cbStreamSize) LPBYTE pValueStream,
    _In_ ULONG cbStreamSize, _In_opt_ LPVOID pvParam);
...
_Must_inspect_result_ BOOL WINAPI BluetoothSdpEnumAttributes(
    _In_reads_bytes_(cbStreamSize) LPBYTE pSDPStream, _In_ ULONG cbStreamSize,
...
_Must_inspect_result_ DWORD WINAPI BluetoothSetLocalServiceInfo(
    _In_opt_ HANDLE hRadioIn, _In_ const GUID *pClassGuid,
...
//
_Must_inspect_result_ BOOL WINAPI
BluetoothIsVersionAvailable(_In_ UCHAR MajorVersion, _In_ UCHAR MinorVersion);
