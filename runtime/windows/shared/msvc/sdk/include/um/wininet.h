...
#define INTERNETAPIX(type, salannotation)                                      \
  EXTERN_C DECLSPEC_IMPORT salannotation type STDAPICALLTYPE
...
#define INTERNETAPIX(type, salannotation)                                      \
  EXTERN_C salannotation type STDAPICALLTYPE
...
#define INTERNET_REQFLAG_NO_HEADERS                                            \
  0x00000008 // orginal response contained no headers
#define INTERNET_REQFLAG_PASSIVE 0x00000010 // FTP: passive-mode connection
#define INTERNET_REQFLAG_CACHE_WRITE_DISABLED                                  \
  0x00000040 // HTTPS: this request not cacheable
#define INTERNET_REQFLAG_NET_TIMEOUT                                           \
  0x00000080 // w/ _FROM_CACHE: net request timed out

//
// flags for IDN enable/disable via INTERNET_OPTION_IDN
//

#define INTERNET_FLAG_IDN_DIRECT                                               \
  0x00000001                               // IDN enabled for direct connections
...
#define INTERNET_FLAG_RAW_DATA                                                 \
  0x40000000 // FTP/gopher find: receive the item as raw (structured) data
#define INTERNET_FLAG_EXISTING_CONNECT                                         \
  0x20000000 // FTP: use existing InternetConnect handle for server if possible
...
#define INTERNET_FLAG_ASYNC                                                    \
  0x10000000 // this request is asynchronous (where supported)
...
#define INTERNET_FLAG_NO_CACHE_WRITE                                           \
  0x04000000 // don't write this item to the cache
#define INTERNET_FLAG_DONT_CACHE INTERNET_FLAG_NO_CACHE_WRITE
#define INTERNET_FLAG_MAKE_PERSISTENT                                          \
  0x02000000 // make this item persistent in cache
...
#define INTERNET_FLAG_NO_AUTO_REDIRECT                                         \
  0x00200000 // don't handle redirections automatically
...
#define INTERNET_FLAG_RESTRICTED_ZONE                                          \
  0x00020000 // apply restricted zone policies for cookies, auth
#define INTERNET_FLAG_CACHE_IF_NET_FAIL                                        \
  0x00010000 // return cache file if net request fails
...
#define INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTP                                  \
  0x00008000 // ex: https:// to http://
#define INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTPS                                 \
  0x00004000 // ex: http:// to https://
...
#define INTERNET_FLAG_RESYNCHRONIZE                                            \
  0x00000800 // asking wininet to update an item if it is newer
#define INTERNET_FLAG_HYPERLINK                                                \
  0x00000400 // asking wininet to do hyperlinking semantic which works right for
             // scripts
#define INTERNET_FLAG_NO_UI 0x00000200 // no cookie popup
#define INTERNET_FLAG_PRAGMA_NOCACHE                                           \
  0x00000100 // asking wininet to add "pragma: no-cache"
...
#define WININET_API_FLAG_USE_CONTEXT                                           \
  0x00000008 // use value supplied in dwContext (even if 0)
...
#define AUTO_PROXY_FLAG_ALWAYS_DETECT                                          \
  0x00000002 // force detection even when its not needed
...
#define AUTO_PROXY_FLAG_DONT_CACHE_PROXY_RESULT                                \
  0x00000010 // don't cache result of host=proxy name
#define AUTO_PROXY_FLAG_CACHE_INIT_RUN                                         \
  0x00000020 // don't initalize and run unless URL expired
#define AUTO_PROXY_FLAG_DETECTION_SUSPECT                                      \
  0x00000040 // results of detection are suspect
...
typedef struct _INTERNET_BUFFERSA {
  DWORD
  dwStructSize; // used for API versioning. Set to sizeof(INTERNET_BUFFERS)
...
BOOLAPI
InternetTimeFromSystemTimeA(_In_ CONST SYSTEMTIME *pst, // input GMT time
                            _In_ DWORD dwRFC,           // RFC format
                            _Out_writes_bytes_(cbTime)
                                LPSTR lpszTime, // output string buffer
                            _In_ DWORD cbTime   // output buffer size
);

BOOLAPI
InternetTimeFromSystemTimeW(_In_ CONST SYSTEMTIME *pst, // input GMT time
                            _In_ DWORD dwRFC,           // RFC format
                            _Out_writes_bytes_(cbTime)
...
#else
BOOLAPI InternetTimeFromSystemTime(_In_ CONST SYSTEMTIME *pst, // input GMT time
                                   _In_ DWORD dwRFC,           // RFC format
                                   _Out_writes_bytes_(cbTime)
                                       LPSTR lpszTime, // output string buffer
                                   _In_ DWORD cbTime   // output buffer size
...
BOOLAPI
InternetTimeToSystemTimeA(_In_ LPCSTR lpszTime,  // NULL terminated string
                          _Out_ SYSTEMTIME *pst, // output in GMT time
                          _Reserved_ DWORD dwReserved);

BOOLAPI
InternetTimeToSystemTimeW(_In_ LPCWSTR lpszTime, // NULL terminated string
                          _Out_ SYSTEMTIME *pst, // output in GMT time
                          _Reserved_ DWORD dwReserved);
...
#else
BOOLAPI InternetTimeToSystemTime(_In_ LPCSTR lpszTime, // NULL terminated string
                                 _Out_ SYSTEMTIME *pst, // output in GMT time
                                 _Reserved_ DWORD dwReserved);
#endif // _WINX32_
#endif // !UNICODE
...
BOOLAPI
InternetCrackUrlA(_In_reads_(dwUrlLength) LPCSTR lpszUrl,
                  _In_ DWORD dwUrlLength, _In_ DWORD dwFlags,
                  _Inout_ LPURL_COMPONENTSA lpUrlComponents);
BOOLAPI
InternetCrackUrlW(_In_reads_(dwUrlLength) LPCWSTR lpszUrl,
                  _In_ DWORD dwUrlLength, _In_ DWORD dwFlags,
...
BOOLAPI
InternetCreateUrlA(_In_ LPURL_COMPONENTSA lpUrlComponents, _In_ DWORD dwFlags,
                   _Out_writes_opt_(*lpdwUrlLength) LPSTR lpszUrl,
                   _Inout_ LPDWORD lpdwUrlLength);
BOOLAPI
InternetCreateUrlW(_In_ LPURL_COMPONENTSW lpUrlComponents, _In_ DWORD dwFlags,
...
BOOLAPI
InternetCanonicalizeUrlA(_In_ LPCSTR lpszUrl,
                         _Out_writes_(*lpdwBufferLength) LPSTR lpszBuffer,
                         _Inout_ LPDWORD lpdwBufferLength, _In_ DWORD dwFlags);
BOOLAPI
InternetCanonicalizeUrlW(_In_ LPCWSTR lpszUrl,
...
BOOLAPI
InternetCombineUrlA(_In_ LPCSTR lpszBaseUrl, _In_ LPCSTR lpszRelativeUrl,
                    _Out_writes_(*lpdwBufferLength) LPSTR lpszBuffer,
                    _Inout_ LPDWORD lpdwBufferLength, _In_ DWORD dwFlags);
BOOLAPI
InternetCombineUrlW(_In_ LPCWSTR lpszBaseUrl, _In_ LPCWSTR lpszRelativeUrl,
                    _Out_writes_(*lpdwBufferLength) LPWSTR lpszBuffer,
                    _Inout_ LPDWORD lpdwBufferLength, _In_ DWORD dwFlags);
#ifdef UNICODE
...
InternetOpenA(_In_opt_ LPCSTR lpszAgent, _In_ DWORD dwAccessType,
              _In_opt_ LPCSTR lpszProxy, _In_opt_ LPCSTR lpszProxyBypass,
              _In_ DWORD dwFlags);
INTERNETAPI_(HINTERNET)
InternetOpenW(_In_opt_ LPCWSTR lpszAgent, _In_ DWORD dwAccessType,
              _In_opt_ LPCWSTR lpszProxy, _In_opt_ LPCWSTR lpszProxyBypass,
              _In_ DWORD dwFlags);
#ifdef UNICODE
...
INTERNETAPI_(HINTERNET)
InternetConnectA(_In_ HINTERNET hInternet, _In_ LPCSTR lpszServerName,
                 _In_ INTERNET_PORT nServerPort, _In_opt_ LPCSTR lpszUserName,
                 _In_opt_ LPCSTR lpszPassword, _In_ DWORD dwService,
                 _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
InternetConnectW(_In_ HINTERNET hInternet, _In_ LPCWSTR lpszServerName,
                 _In_ INTERNET_PORT nServerPort, _In_opt_ LPCWSTR lpszUserName,
                 _In_opt_ LPCWSTR lpszPassword, _In_ DWORD dwService,
                 _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
INTERNETAPI_(HINTERNET)
InternetOpenUrlA(_In_ HINTERNET hInternet, _In_ LPCSTR lpszUrl,
                 _In_reads_opt_(dwHeadersLength) LPCSTR lpszHeaders,
                 _In_ DWORD dwHeadersLength, _In_ DWORD dwFlags,
                 _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
InternetOpenUrlW(_In_ HINTERNET hInternet, _In_ LPCWSTR lpszUrl,
...
BOOLAPI InternetReadFile(_In_ HINTERNET hFile,
                         _Out_writes_bytes_(dwNumberOfBytesToRead)
                             __out_data_source(NETWORK) LPVOID lpBuffer,
                         _In_ DWORD dwNumberOfBytesToRead,
                         _Out_ LPDWORD lpdwNumberOfBytesRead);

BOOLAPI InternetReadFileExA(_In_ HINTERNET hFile,
                            _Out_ __out_data_source(NETWORK)
                                LPINTERNET_BUFFERSA lpBuffersOut,
                            _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
BOOLAPI InternetReadFileExW(_In_ HINTERNET hFile,
                            _Out_ __out_data_source(NETWORK)
...
INTERNETAPI_(DWORD)
InternetSetFilePointer(_In_ HINTERNET hFile, _In_ LONG lDistanceToMove,
                       _Inout_opt_ PLONG lpDistanceToMoveHigh,
                       _In_ DWORD dwMoveMethod, _Reserved_ DWORD_PTR dwContext);

BOOLAPI InternetWriteFile(_In_ HINTERNET hFile,
                          _In_reads_bytes_(dwNumberOfBytesToWrite)
                              LPCVOID lpBuffer,
                          _In_ DWORD dwNumberOfBytesToWrite,
                          _Out_ LPDWORD lpdwNumberOfBytesWritten);

BOOLAPI InternetQueryDataAvailable(_In_ HINTERNET hFile,
                                   _Out_opt_ __out_data_source(NETWORK)
                                       LPDWORD lpdwNumberOfBytesAvailable,
                                   _In_ DWORD dwFlags,
                                   _In_opt_ DWORD_PTR dwContext);

BOOLAPI InternetFindNextFileA(_In_ HINTERNET hFind, _Out_ LPVOID lpvFindData);
BOOLAPI InternetFindNextFileW(_In_ HINTERNET hFind, _Out_ LPVOID lpvFindData);
#ifdef UNICODE
...
BOOLAPI InternetQueryOptionA(_In_opt_ HINTERNET hInternet, _In_ DWORD dwOption,
...
BOOLAPI InternetSetOptionA(_In_opt_ HINTERNET hInternet, _In_ DWORD dwOption,
                           _In_opt_ LPVOID lpBuffer, _In_ DWORD dwBufferLength);
BOOLAPI InternetSetOptionW(_In_opt_ HINTERNET hInternet, _In_ DWORD dwOption,
                           _In_opt_ LPVOID lpBuffer, _In_ DWORD dwBufferLength);
#ifdef UNICODE
...
BOOLAPI InternetSetOptionExA(_In_opt_ HINTERNET hInternet, _In_ DWORD dwOption,
                             _In_opt_ LPVOID lpBuffer,
                             _In_ DWORD dwBufferLength, _In_ DWORD dwFlags);
BOOLAPI InternetSetOptionExW(_In_opt_ HINTERNET hInternet, _In_ DWORD dwOption,
                             _In_opt_ LPVOID lpBuffer,
                             _In_ DWORD dwBufferLength, _In_ DWORD dwFlags);
#ifdef UNICODE
...
BOOLAPI InternetLockRequestFile(_In_ HINTERNET hInternet,
                                _Out_ HANDLE *lphLockRequestInfo);

BOOLAPI InternetUnlockRequestFile(_Inout_ HANDLE hLockRequestInfo);
...
//
// Values for INTERNET_OPTION_ENABLE_HTTP_PROTOCOL /
...
BOOLAPI
InternetGetLastResponseInfoA(_Out_ LPDWORD lpdwError,
                             _Out_writes_opt_(*lpdwBufferLength)
                                 LPSTR lpszBuffer,
                             _Inout_ LPDWORD lpdwBufferLength);
BOOLAPI
InternetGetLastResponseInfoW(_Out_ LPDWORD lpdwError,
                             _Out_writes_opt_(*lpdwBufferLength)
                                 LPWSTR lpszBuffer,
                             _Inout_ LPDWORD lpdwBufferLength);
#ifdef UNICODE
...
typedef VOID(CALLBACK *INTERNET_STATUS_CALLBACK)(
    _In_ HINTERNET hInternet, _In_opt_ DWORD_PTR dwContext,
    _In_ DWORD dwInternetStatus, _In_opt_ LPVOID lpvStatusInformation,
    _In_ DWORD dwStatusInformationLength);

typedef INTERNET_STATUS_CALLBACK *LPINTERNET_STATUS_CALLBACK;

INTERNETAPI_(INTERNET_STATUS_CALLBACK)
InternetSetStatusCallbackA(_In_ HINTERNET hInternet,
                           _In_opt_ INTERNET_STATUS_CALLBACK
                               lpfnInternetCallback);

INTERNETAPI_(INTERNET_STATUS_CALLBACK)
InternetSetStatusCallbackW(_In_ HINTERNET hInternet,
                           _In_opt_ INTERNET_STATUS_CALLBACK
                               lpfnInternetCallback);
...
#else
INTERNETAPI_(INTERNET_STATUS_CALLBACK)
InternetSetStatusCallback(_In_ HINTERNET hInternet,
                          _In_opt_ INTERNET_STATUS_CALLBACK
                              lpfnInternetCallback);
#endif // _WINX32_
#endif // !UNICODE
...
#define INTERNET_STATE_CONNECTED                                               \
  0x00000001 // connected state (mutually exclusive with disconnected)
#define INTERNET_STATE_DISCONNECTED 0x00000002 // disconnected from network
#define INTERNET_STATE_DISCONNECTED_BY_USER                                    \
  0x00000010 // disconnected by user request
#define INTERNET_STATE_IDLE                                                    \
  0x00000100 // no network requests being made (by Wininet)
#define INTERNET_STATE_BUSY                                                    \
  0x00000200 // network requests being made (by Wininet)
...
typedef struct {
  ...
  const char *pszLocation; // Optional: URL associated with reported cookie
                           // events This can be used to override request URL
} IncomingCookieState;
...
INTERNETAPI_(HINTERNET)
FtpFindFirstFileA(_In_ HINTERNET hConnect, _In_opt_ LPCSTR lpszSearchFile,
                  _Out_opt_ LPWIN32_FIND_DATAA lpFindFileData,
                  _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
FtpFindFirstFileW(_In_ HINTERNET hConnect, _In_opt_ LPCWSTR lpszSearchFile,
...
BOOLAPI FtpGetFileA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszRemoteFile,
                    _In_ LPCSTR lpszNewFile, _In_ BOOL fFailIfExists,
                    _In_ DWORD dwFlagsAndAttributes, _In_ DWORD dwFlags,
                    _In_opt_ DWORD_PTR dwContext);
BOOLAPI FtpGetFileW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszRemoteFile,
                    _In_ LPCWSTR lpszNewFile, _In_ BOOL fFailIfExists,
                    _In_ DWORD dwFlagsAndAttributes, _In_ DWORD dwFlags,
                    _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
BOOLAPI FtpPutFileA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszLocalFile,
                    _In_ LPCSTR lpszNewRemoteFile, _In_ DWORD dwFlags,
                    _In_opt_ DWORD_PTR dwContext);
BOOLAPI FtpPutFileW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszLocalFile,
                    _In_ LPCWSTR lpszNewRemoteFile, _In_ DWORD dwFlags,
                    _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
BOOLAPI FtpGetFileEx(_In_ HINTERNET hFtpSession, _In_ LPCSTR lpszRemoteFile,
                     _In_ LPCWSTR lpszNewFile, _In_ BOOL fFailIfExists,
                     _In_ DWORD dwFlagsAndAttributes, _In_ DWORD dwFlags,
                     _In_opt_ DWORD_PTR dwContext);

BOOLAPI FtpPutFileEx(_In_ HINTERNET hFtpSession, _In_ LPCWSTR lpszLocalFile,
                     _In_ LPCSTR lpszNewRemoteFile, _In_ DWORD dwFlags,
                     _In_opt_ DWORD_PTR dwContext);

BOOLAPI FtpDeleteFileA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszFileName);
BOOLAPI FtpDeleteFileW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszFileName);
#ifdef UNICODE
...
BOOLAPI FtpRenameFileA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszExisting,
                       _In_ LPCSTR lpszNew);
BOOLAPI FtpRenameFileW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszExisting,
...
INTERNETAPI_(HINTERNET)
FtpOpenFileA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszFileName,
             _In_ DWORD dwAccess, _In_ DWORD dwFlags,
             _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
FtpOpenFileW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszFileName,
             _In_ DWORD dwAccess, _In_ DWORD dwFlags,
             _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
BOOLAPI FtpCreateDirectoryA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszDirectory);
BOOLAPI FtpCreateDirectoryW(_In_ HINTERNET hConnect,
...
BOOLAPI FtpRemoveDirectoryA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszDirectory);
BOOLAPI FtpRemoveDirectoryW(_In_ HINTERNET hConnect,
                            _In_ LPCWSTR lpszDirectory);
#ifdef UNICODE
...
BOOLAPI FtpSetCurrentDirectoryA(_In_ HINTERNET hConnect,
                                _In_ LPCSTR lpszDirectory);
BOOLAPI FtpSetCurrentDirectoryW(_In_ HINTERNET hConnect,
                                _In_ LPCWSTR lpszDirectory);
#ifdef UNICODE
...
BOOLAPI FtpGetCurrentDirectoryA(_In_ HINTERNET hConnect,
                                _Out_writes_(*lpdwCurrentDirectory)
                                    LPSTR lpszCurrentDirectory,
                                _Inout_ LPDWORD lpdwCurrentDirectory);
BOOLAPI FtpGetCurrentDirectoryW(_In_ HINTERNET hConnect,
                                _Out_writes_(*lpdwCurrentDirectory)
...
BOOLAPI FtpCommandA(_In_ HINTERNET hConnect, _In_ BOOL fExpectResponse,
                    _In_ DWORD dwFlags, _In_ LPCSTR lpszCommand,
                    _In_opt_ DWORD_PTR dwContext,
                    _Out_opt_ HINTERNET *phFtpCommand);
BOOLAPI FtpCommandW(_In_ HINTERNET hConnect, _In_ BOOL fExpectResponse,
                    _In_ DWORD dwFlags, _In_ LPCWSTR lpszCommand,
                    _In_opt_ DWORD_PTR dwContext,
                    _Out_opt_ HINTERNET *phFtpCommand);
#ifdef UNICODE
...
INTERNETAPI_(DWORD)
FtpGetFileSize(_In_ HINTERNET hFile, _Out_opt_ LPDWORD lpdwFileSizeHigh);
...
#define IS_GOPHER_FILE(type)                                                   \
  (BOOL)(((type) & GOPHER_TYPE_FILE_MASK) ? TRUE : FALSE)
#define IS_GOPHER_DIRECTORY(type)                                              \
  (BOOL)(((type) & GOPHER_TYPE_DIRECTORY) ? TRUE : FALSE)
#define IS_GOPHER_PHONE_SERVER(type)                                           \
  (BOOL)(((type) & GOPHER_TYPE_CSO) ? TRUE : FALSE)
#define IS_GOPHER_ERROR(type)                                                  \
  (BOOL)(((type) & GOPHER_TYPE_ERROR) ? TRUE : FALSE)
#define IS_GOPHER_INDEX_SERVER(type)                                           \
  (BOOL)(((type) & GOPHER_TYPE_INDEX_SERVER) ? TRUE : FALSE)
#define IS_GOPHER_TELNET_SESSION(type)                                         \
  (BOOL)(((type) & GOPHER_TYPE_TELNET) ? TRUE : FALSE)
#define IS_GOPHER_BACKUP_SERVER(type)                                          \
  (BOOL)(((type) & GOPHER_TYPE_REDUNDANT) ? TRUE : FALSE)
#define IS_GOPHER_TN3270_SESSION(type)                                         \
  (BOOL)(((type) & GOPHER_TYPE_TN3270) ? TRUE : FALSE)
#define IS_GOPHER_ASK(type) (BOOL)(((type) & GOPHER_TYPE_ASK) ? TRUE : FALSE)
#define IS_GOPHER_PLUS(type)                                                   \
  (BOOL)(((type) & GOPHER_TYPE_GOPHER_PLUS) ? TRUE : FALSE)

#define IS_GOPHER_TYPE_KNOWN(type)                                             \
  (BOOL)(((type) & GOPHER_TYPE_UNKNOWN) ? FALSE : TRUE)
...
BOOLAPI GopherCreateLocatorA(_In_ LPCSTR lpszHost,
                             ...
                             _Out_writes_opt_(*lpdwBufferLength)
                                 LPSTR lpszLocator,
                             _Inout_ LPDWORD lpdwBufferLength);
BOOLAPI GopherCreateLocatorW(_In_ LPCWSTR lpszHost,
                             _In_ INTERNET_PORT nServerPort,
...
BOOLAPI GopherGetLocatorTypeA(_In_ LPCSTR lpszLocator,
                              _Out_ LPDWORD lpdwGopherType);
BOOLAPI GopherGetLocatorTypeW(_In_ LPCWSTR lpszLocator,
                              _Out_ LPDWORD lpdwGopherType);
#ifdef UNICODE
...
INTERNETAPI_(HINTERNET)
GopherFindFirstFileA(_In_ HINTERNET hConnect, _In_opt_ LPCSTR lpszLocator,
                     _In_opt_ LPCSTR lpszSearchString,
                     _Out_opt_ LPGOPHER_FIND_DATAA lpFindData,
                     _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
GopherFindFirstFileW(_In_ HINTERNET hConnect, _In_opt_ LPCWSTR lpszLocator,
...
INTERNETAPI_(HINTERNET)
GopherOpenFileA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszLocator,
                _In_opt_ LPCSTR lpszView, _In_ DWORD dwFlags,
                _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
GopherOpenFileW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszLocator,
                _In_opt_ LPCWSTR lpszView, _In_ DWORD dwFlags,
                _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
typedef BOOL(CALLBACK *GOPHER_ATTRIBUTE_ENUMERATOR)(
    _In_ LPGOPHER_ATTRIBUTE_TYPE lpAttributeInfo, _In_ DWORD dwError);

BOOLAPI GopherGetAttributeA(_In_ HINTERNET hConnect, _In_ LPCSTR lpszLocator,
                            _In_opt_ LPCSTR lpszAttributeName,
                            _At_((LPSTR)lpBuffer, _Out_writes_(dwBufferLength))
                                LPBYTE lpBuffer,
...
BOOLAPI GopherGetAttributeW(_In_ HINTERNET hConnect, _In_ LPCWSTR lpszLocator,
                            ...
                            _At_((LPWSTR)lpBuffer, _Out_writes_(dwBufferLength))
                                LPBYTE lpBuffer,
                            _In_ DWORD dwBufferLength,
                            _Out_ LPDWORD lpdwCharactersReturned,
                            _In_opt_ GOPHER_ATTRIBUTE_ENUMERATOR lpfnEnumerator,
...
INTERNETAPI_(HINTERNET)
HttpOpenRequestA(_In_ HINTERNET hConnect, _In_opt_ LPCSTR lpszVerb,
                 _In_opt_ LPCSTR lpszObjectName, _In_opt_ LPCSTR lpszVersion,
                 _In_opt_ LPCSTR lpszReferrer,
                 _In_opt_z_ LPCSTR FAR *lplpszAcceptTypes, _In_ DWORD dwFlags,
                 _In_opt_ DWORD_PTR dwContext);
INTERNETAPI_(HINTERNET)
HttpOpenRequestW(_In_ HINTERNET hConnect, _In_opt_ LPCWSTR lpszVerb,
                 _In_opt_ LPCWSTR lpszObjectName, _In_opt_ LPCWSTR lpszVersion,
                 _In_opt_ LPCWSTR lpszReferrer,
                 _In_opt_z_ LPCWSTR FAR *lplpszAcceptTypes, _In_ DWORD dwFlags,
                 _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
BOOLAPI HttpAddRequestHeadersA(_In_ HINTERNET hRequest,
                               _When_(dwHeadersLength == (DWORD)-1, _In_z_)
                                   _When_(dwHeadersLength != (DWORD)-1,
                                          _In_reads_(dwHeadersLength))
                                       LPCSTR lpszHeaders,
                               _In_ DWORD dwHeadersLength,
                               _In_ DWORD dwModifiers);
BOOLAPI HttpAddRequestHeadersW(_In_ HINTERNET hRequest,
                               _When_(dwHeadersLength == (DWORD)-1, _In_z_)
                                   _When_(dwHeadersLength != (DWORD)-1,
                                          _In_reads_(dwHeadersLength))
                                       LPCWSTR lpszHeaders,
...
BOOLAPI HttpSendRequestA(_In_ HINTERNET hRequest,
                         _In_reads_opt_(dwHeadersLength) LPCSTR lpszHeaders,
                         _In_ DWORD dwHeadersLength,
                         _In_reads_bytes_opt_(dwOptionalLength)
                             LPVOID lpOptional,
                         _In_ DWORD dwOptionalLength);
BOOLAPI HttpSendRequestW(_In_ HINTERNET hRequest,
                         _In_reads_opt_(dwHeadersLength) LPCWSTR lpszHeaders,
                         _In_ DWORD dwHeadersLength,
                         _In_reads_bytes_opt_(dwOptionalLength)
                             LPVOID lpOptional,
...
BOOLAPI HttpSendRequestExA(_In_ HINTERNET hRequest,
                           _In_opt_ LPINTERNET_BUFFERSA lpBuffersIn,
                           _Out_opt_ LPINTERNET_BUFFERSA lpBuffersOut,
                           _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
BOOLAPI HttpSendRequestExW(_In_ HINTERNET hRequest,
...
#define HSR_INITIATE                                                           \
  0x00000008 // iterative operation (completed by HttpEndRequest)
#define HSR_DOWNLOAD 0x00000010 // download to file
#define HSR_CHUNKED 0x00000020  // operation is send of chunked data

BOOLAPI HttpEndRequestA(_In_ HINTERNET hRequest,
                        _Out_opt_ LPINTERNET_BUFFERSA lpBuffersOut,
                        _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
BOOLAPI HttpEndRequestW(_In_ HINTERNET hRequest,
                        _Out_opt_ LPINTERNET_BUFFERSW lpBuffersOut,
                        _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext);
#ifdef UNICODE
...
BOOLAPI
HttpQueryInfoA(_In_ HINTERNET hRequest, _In_ DWORD dwInfoLevel,
               _Inout_updates_bytes_to_opt_(*lpdwBufferLength,
                                            *lpdwBufferLength)
                   __out_data_source(NETWORK) LPVOID lpBuffer,
               _Inout_ LPDWORD lpdwBufferLength, _Inout_opt_ LPDWORD lpdwIndex);
BOOLAPI
HttpQueryInfoW(_In_ HINTERNET hRequest, _In_ DWORD dwInfoLevel,
               _Inout_updates_bytes_to_opt_(*lpdwBufferLength,
                                            *lpdwBufferLength)
                   __out_data_source(NETWORK) LPVOID lpBuffer,
               _Inout_ LPDWORD lpdwBufferLength, _Inout_opt_ LPDWORD lpdwIndex);
...
BOOLAPI
InternetSetCookieA(_In_ LPCSTR lpszUrl, _In_opt_ LPCSTR lpszCookieName,
                   _In_ LPCSTR lpszCookieData);
BOOLAPI
InternetSetCookieW(_In_ LPCWSTR lpszUrl, _In_opt_ LPCWSTR lpszCookieName,
...
BOOLAPI
InternetGetCookieA(_In_ LPCSTR lpszUrl, _In_opt_ LPCSTR lpszCookieName,
                   _Out_writes_opt_(*lpdwSize) LPSTR lpszCookieData,
                   _Inout_ LPDWORD lpdwSize);
BOOLAPI
InternetGetCookieW(_In_ LPCWSTR lpszUrl, _In_opt_ LPCWSTR lpszCookieName,
...
INTERNETAPI_(DWORD)
InternetSetCookieExA(_In_ LPCSTR lpszUrl, _In_opt_ LPCSTR lpszCookieName,
                     _In_ LPCSTR lpszCookieData, _In_ DWORD dwFlags,
                     _In_opt_ DWORD_PTR dwReserved);
INTERNETAPI_(DWORD)
InternetSetCookieExW(_In_ LPCWSTR lpszUrl, _In_opt_ LPCWSTR lpszCookieName,
                     _In_ LPCWSTR lpszCookieData, _In_ DWORD dwFlags,
                     _In_opt_ DWORD_PTR dwReserved);
#ifdef UNICODE
...
BOOLAPI
InternetGetCookieExA(_In_ LPCSTR lpszUrl, _In_opt_ LPCSTR lpszCookieName,
                     _In_reads_opt_(*lpdwSize) LPSTR lpszCookieData,
                     _Inout_ LPDWORD lpdwSize, _In_ DWORD dwFlags,
                     _Reserved_ LPVOID lpReserved);
BOOLAPI
InternetGetCookieExW(_In_ LPCWSTR lpszUrl, _In_opt_ LPCWSTR lpszCookieName,
...
INTERNETAPI_(VOID)
InternetFreeCookies(_Inout_opt_ INTERNET_COOKIE2 *pCookies,
                    _In_ DWORD dwCookieCount);

INTERNETAPI_(DWORD)
InternetGetCookieEx2(_In_ PCWSTR pcwszUrl, _In_opt_ PCWSTR pcwszCookieName,
                     _In_ DWORD dwFlags, _Outptr_ INTERNET_COOKIE2 **ppCookies,
                     _Out_ PDWORD pdwCookieCount);

INTERNETAPI_(DWORD)
InternetSetCookieEx2(_In_ PCWSTR pcwszUrl, _In_ const INTERNET_COOKIE2 *pCookie,
                     _In_opt_ PCWSTR pcwszP3PPolicy, _In_ DWORD dwFlags,
...
INTERNETAPI_(DWORD)
InternetAttemptConnect(_In_ DWORD dwReserved);

BOOLAPI
InternetCheckConnectionA(_In_ LPCSTR lpszUrl, _In_ DWORD dwFlags,
                         _In_ DWORD dwReserved);
BOOLAPI
InternetCheckConnectionW(_In_ LPCWSTR lpszUrl, _In_ DWORD dwFlags,
                         _In_ DWORD dwReserved);
#ifdef UNICODE
...
BOOLAPI ResumeSuspendedDownload(_In_ HINTERNET hRequest,
                                _In_ DWORD dwResultCode);

INTERNETAPI_(DWORD)
InternetErrorDlg(_In_ HWND hWnd, _Inout_opt_ HINTERNET hRequest,
                 _In_ DWORD dwError, _In_ DWORD dwFlags,
                 _Inout_opt_ LPVOID *lppvData);

INTERNETAPI_(DWORD)
InternetConfirmZoneCrossingA(_In_ HWND hWnd, _In_ LPSTR szUrlPrev,
                             _In_ LPSTR szUrlNew, _In_ BOOL bPost);

INTERNETAPI_(DWORD)
InternetConfirmZoneCrossingW(_In_ HWND hWnd, _In_ LPWSTR szUrlPrev,
                             _In_ LPWSTR szUrlNew, _In_ BOOL bPost);
...
#else
INTERNETAPI_(DWORD)
InternetConfirmZoneCrossing(_In_ HWND hWnd, _In_ LPSTR szUrlPrev,
                            _In_ LPSTR szUrlNew, _In_ BOOL bPost);
#endif // _WINX32_
#endif // !UNICODE
...
#define URLCACHE_FIND_DEFAULT_FILTER                                           \
  NORMAL_CACHE_ENTRY                                                           \
  | COOKIE_CACHE_ENTRY | URLHISTORY_CACHE_ENTRY | TRACK_OFFLINE_CACHE_ENTRY |  \
      TRACK_ONLINE_CACHE_ENTRY | STICKY_CACHE_ENTRY
...
typedef struct _INTERNET_CACHE_ENTRY_INFOA {
  ...
  LPSTR lpszFileExtension;   // File extension used to retrive the urldata as a
                             // file.
...
typedef struct _INTERNET_CACHE_ENTRY_INFOW {
  ...
  DWORD dwHeaderInfoSize;    // size of the above header.
  LPWSTR lpszFileExtension;  // File extension used to retrive the urldata as a
                             // file.
  union {                    // Exemption delta from last access time.
    DWORD dwReserved;
    DWORD dwExemptDelta;
...
#define CACHEGROUP_READWRITE_MASK                                              \
  ...
  | CACHEGROUP_ATTRIBUTE_QUOTA | CACHEGROUP_ATTRIBUTE_GROUPNAME |              \
...
BOOLAPI CreateUrlCacheEntryA(_In_ LPCSTR lpszUrlName,
                             _In_ DWORD dwExpectedFileSize,
                             _In_opt_ LPCSTR lpszFileExtension,
                             _Inout_updates_(MAX_PATH) LPSTR lpszFileName,
                             _In_ DWORD dwReserved);
BOOLAPI CreateUrlCacheEntryW(_In_ LPCWSTR lpszUrlName,
                             _In_ DWORD dwExpectedFileSize,
...
BOOLAPI
CommitUrlCacheEntryA(_In_ LPCSTR lpszUrlName, _In_opt_ LPCSTR lpszLocalFileName,
                     _In_ FILETIME ExpireTime, _In_ FILETIME LastModifiedTime,
...
BOOLAPI
CommitUrlCacheEntryW(_In_ LPCWSTR lpszUrlName,
...
BOOLAPI
RetrieveUrlCacheEntryFileA(_In_ LPCSTR lpszUrlName,
                           _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                               LPINTERNET_CACHE_ENTRY_INFOA lpCacheEntryInfo,
                           _Inout_ LPDWORD lpcbCacheEntryInfo,
                           _Reserved_ DWORD dwReserved);
BOOLAPI
RetrieveUrlCacheEntryFileW(_In_ LPCWSTR lpszUrlName,
                           _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
...
BOOLAPI UnlockUrlCacheEntryFileA(_In_ LPCSTR lpszUrlName,
                                 _Reserved_ DWORD dwReserved);

BOOLAPI UnlockUrlCacheEntryFileW(_In_ LPCWSTR lpszUrlName,
                                 _Reserved_ DWORD dwReserved);
...
#else
BOOLAPI UnlockUrlCacheEntryFile(_In_ LPCSTR lpszUrlName,
                                _Reserved_ DWORD dwReserved);
#endif // _WINX32_
#endif // !UNICODE

INTERNETAPI_(HANDLE)
RetrieveUrlCacheEntryStreamA(_In_ LPCSTR lpszUrlName,
                             _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                                 LPINTERNET_CACHE_ENTRY_INFOA lpCacheEntryInfo,
                             _Inout_ LPDWORD lpcbCacheEntryInfo,
                             ...
                             _Reserved_ DWORD dwReserved);
INTERNETAPI_(HANDLE)
RetrieveUrlCacheEntryStreamW(_In_ LPCWSTR lpszUrlName,
                             _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                                 LPINTERNET_CACHE_ENTRY_INFOW lpCacheEntryInfo,
                             _Inout_ LPDWORD lpcbCacheEntryInfo,
                             _In_ BOOL fRandomRead,
...
BOOLAPI
ReadUrlCacheEntryStream(_In_ HANDLE hUrlCacheStream, _In_ DWORD dwLocation,
                        _Out_writes_bytes_(*lpdwLen) __out_data_source(NETWORK)
                            LPVOID lpBuffer,
                        _Inout_ LPDWORD lpdwLen, _Reserved_ DWORD Reserved);

BOOLAPI
ReadUrlCacheEntryStreamEx(_In_ HANDLE hUrlCacheStream,
                          _In_ DWORDLONG qwLocation,
                          _Out_writes_bytes_(*lpdwLen)
                              __out_data_source(NETWORK) LPVOID lpBuffer,
                          _Inout_ LPDWORD lpdwLen);

BOOLAPI
UnlockUrlCacheEntryStream(_In_ HANDLE hUrlCacheStream,
                          _Reserved_ DWORD Reserved);

BOOLAPI
GetUrlCacheEntryInfoA(_In_ LPCSTR lpszUrlName,
                      _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                          LPINTERNET_CACHE_ENTRY_INFOA lpCacheEntryInfo,
                      _Inout_opt_ LPDWORD lpcbCacheEntryInfo);
BOOLAPI
GetUrlCacheEntryInfoW(_In_ LPCWSTR lpszUrlName,
                      _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                          LPINTERNET_CACHE_ENTRY_INFOW lpCacheEntryInfo,
                      _Inout_opt_ LPDWORD lpcbCacheEntryInfo);
#ifdef UNICODE
...
URLCACHEAPI_(HANDLE)
FindFirstUrlCacheGroup(_In_ DWORD dwFlags, _In_ DWORD dwFilter,
                       _Reserved_ LPVOID lpSearchCondition,
                       _Reserved_ DWORD dwSearchCondition,
                       _Out_ GROUPID *lpGroupId, _Reserved_ LPVOID lpReserved);

BOOLAPI FindNextUrlCacheGroup(_In_ HANDLE hFind, _Out_ GROUPID *lpGroupId,
                              _Reserved_ LPVOID lpReserved);

BOOLAPI GetUrlCacheGroupAttributeA(_In_ GROUPID gid, _Reserved_ DWORD dwFlags,
                                   _In_ DWORD dwAttributes,
                                   _Out_writes_bytes_(*lpcbGroupInfo)
                                       LPINTERNET_CACHE_GROUP_INFOA lpGroupInfo,
                                   _Inout_ LPDWORD lpcbGroupInfo,
                                   _Reserved_ LPVOID lpReserved);
BOOLAPI GetUrlCacheGroupAttributeW(_In_ GROUPID gid, _Reserved_ DWORD dwFlags,
                                   _In_ DWORD dwAttributes,
                                   _Out_writes_bytes_(*lpcbGroupInfo)
...
BOOLAPI
SetUrlCacheGroupAttributeA(_In_ GROUPID gid, _Reserved_ DWORD dwFlags,
                           _In_ DWORD dwAttributes,
                           _In_ LPINTERNET_CACHE_GROUP_INFOA lpGroupInfo,
                           _Reserved_ LPVOID lpReserved);
BOOLAPI
SetUrlCacheGroupAttributeW(_In_ GROUPID gid, _Reserved_ DWORD dwFlags,
                           _In_ DWORD dwAttributes,
...
GetUrlCacheEntryInfoExA(_In_ LPCSTR lpszUrl,
                        ...
                        _Reserved_ LPVOID lpReserved, _In_ DWORD dwFlags);
BOOLAPI
GetUrlCacheEntryInfoExW(_In_ LPCWSTR lpszUrl,
                        _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                            LPINTERNET_CACHE_ENTRY_INFOW lpCacheEntryInfo,
                        _Inout_opt_ LPDWORD lpcbCacheEntryInfo,
...
BOOLAPI
SetUrlCacheEntryInfoA(_In_ LPCSTR lpszUrlName,
                      _In_ LPINTERNET_CACHE_ENTRY_INFOA lpCacheEntryInfo,
                      _In_ DWORD dwFieldControl);
BOOLAPI
SetUrlCacheEntryInfoW(_In_ LPCWSTR lpszUrlName,
...
CreateUrlCacheGroup(_In_ DWORD dwFlags, _Reserved_ LPVOID lpReserved);

BOOLAPI DeleteUrlCacheGroup(_In_ GROUPID GroupId,
...
BOOLAPI SetUrlCacheEntryGroupA(_In_ LPCSTR lpszUrlName, _In_ DWORD dwFlags,
                               _In_ GROUPID GroupId,
                               _Reserved_ LPBYTE pbGroupAttributes,
                               _Reserved_ DWORD cbGroupAttributes,
                               _Reserved_ LPVOID lpReserved);

BOOLAPI SetUrlCacheEntryGroupW(_In_ LPCWSTR lpszUrlName, _In_ DWORD dwFlags,
                               _In_ GROUPID GroupId,
                               _Reserved_ LPBYTE pbGroupAttributes,
                               _Reserved_ DWORD cbGroupAttributes,
                               _Reserved_ LPVOID lpReserved);
...
#else
BOOLAPI SetUrlCacheEntryGroup(_In_ LPCSTR lpszUrlName, _In_ DWORD dwFlags,
                              _In_ GROUPID GroupId,
                              _Reserved_ LPBYTE pbGroupAttributes,
                              _Reserved_ DWORD cbGroupAttributes,
                              _Reserved_ LPVOID lpReserved);
...
FindFirstUrlCacheEntryExA(
    _In_opt_ LPCSTR lpszUrlSearchPattern, _In_ DWORD dwFlags,
    _In_ DWORD dwFilter, _In_ GROUPID GroupId,
    _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
        LPINTERNET_CACHE_ENTRY_INFOA lpFirstCacheEntryInfo,
    _Inout_ LPDWORD lpcbCacheEntryInfo, _Reserved_ LPVOID lpGroupAttributes,
    _Reserved_ LPDWORD lpcbGroupAttributes, _Reserved_ LPVOID lpReserved);
INTERNETAPI_(HANDLE)
FindFirstUrlCacheEntryExW(
    _In_opt_ LPCWSTR lpszUrlSearchPattern, _In_ DWORD dwFlags,
    _In_ DWORD dwFilter, _In_ GROUPID GroupId,
    _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
...
BOOLAPI
FindNextUrlCacheEntryExA(_In_ HANDLE hEnumHandle,
                         _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                         ...
                         _Reserved_ LPVOID lpReserved);
BOOLAPI
FindNextUrlCacheEntryExW(_In_ HANDLE hEnumHandle,
                         _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
...
INTERNETAPI_(HANDLE)
FindFirstUrlCacheEntryA(_In_opt_ LPCSTR lpszUrlSearchPattern,
                        _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                            LPINTERNET_CACHE_ENTRY_INFOA lpFirstCacheEntryInfo,
                        _Inout_ LPDWORD lpcbCacheEntryInfo);
INTERNETAPI_(HANDLE)
FindFirstUrlCacheEntryW(_In_opt_ LPCWSTR lpszUrlSearchPattern,
                        _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                            LPINTERNET_CACHE_ENTRY_INFOW lpFirstCacheEntryInfo,
                        _Inout_ LPDWORD lpcbCacheEntryInfo);
#ifdef UNICODE
...
BOOLAPI
FindNextUrlCacheEntryA(_In_ HANDLE hEnumHandle,
                       _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                           LPINTERNET_CACHE_ENTRY_INFOA lpNextCacheEntryInfo,
                       _Inout_ LPDWORD lpcbCacheEntryInfo);
BOOLAPI
FindNextUrlCacheEntryW(_In_ HANDLE hEnumHandle,
                       _Inout_updates_bytes_opt_(*lpcbCacheEntryInfo)
                           LPINTERNET_CACHE_ENTRY_INFOW lpNextCacheEntryInfo,
                       _Inout_ LPDWORD lpcbCacheEntryInfo);
#ifdef UNICODE
...
BOOLAPI FindCloseUrlCache(_In_ HANDLE hEnumHandle);

BOOLAPI DeleteUrlCacheEntryA(_In_ LPCSTR lpszUrlName);

BOOLAPI DeleteUrlCacheEntryW(_In_ LPCWSTR lpszUrlName);
...
#else
BOOLAPI DeleteUrlCacheEntry(_In_ LPCSTR lpszUrlName);
#endif // _WINX32_
#endif // !UNICODE
...
INTERNETAPI_(DWORD)
InternetDialA(_In_ HWND hwndParent, _In_opt_ LPSTR lpszConnectoid,
              _In_ DWORD dwFlags, _Out_ DWORD_PTR *lpdwConnection,
              _Reserved_ DWORD dwReserved);

INTERNETAPI_(DWORD)
InternetDialW(_In_ HWND hwndParent, _In_opt_ LPWSTR lpszConnectoid,
              _In_ DWORD dwFlags, _Out_ DWORD_PTR *lpdwConnection,
              _Reserved_ DWORD dwReserved);
...
#else
INTERNETAPI_(DWORD)
InternetDial(_In_ HWND hwndParent, _In_opt_ LPSTR lpszConnectoid,
             _In_ DWORD dwFlags, _Out_ LPDWORD lpdwConnection,
             _In_ DWORD dwReserved);
#endif // _WINX32_
#endif // !UNICODE
...
INTERNETAPI_(DWORD)
InternetHangUp(_In_ DWORD_PTR dwConnection, _Reserved_ DWORD dwReserved);
...
BOOLAPI InternetGoOnlineA(_In_opt_ LPCSTR lpszURL, _In_ HWND hwndParent,
                          _In_ DWORD dwFlags);

BOOLAPI InternetGoOnlineW(_In_opt_ LPCWSTR lpszURL, _In_ HWND hwndParent,
                          _In_ DWORD dwFlags);
...
#else
BOOLAPI InternetGoOnline(_In_opt_ LPSTR lpszURL, _In_ HWND hwndParent,
                         _In_ DWORD dwFlags);
#endif // _WINX32_
#endif // !UNICODE

BOOLAPI InternetAutodial(_In_ DWORD dwFlags, _In_opt_ HWND hwndParent);
...
BOOLAPI InternetAutodialHangup(_Reserved_ DWORD dwReserved);

BOOLAPI InternetGetConnectedState(_Out_ LPDWORD lpdwFlags,
                                  _Reserved_ DWORD dwReserved);

BOOLAPI
InternetGetConnectedStateExA(_Out_opt_ LPDWORD lpdwFlags,
                             _Out_writes_opt_(cchNameLen)
                                 LPSTR lpszConnectionName,
                             _In_ DWORD cchNameLen,
                             _Reserved_ DWORD dwReserved);

BOOLAPI
InternetGetConnectedStateExW(_Out_opt_ LPDWORD lpdwFlags,
                             _Out_writes_opt_(cchNameLen)
...
typedef struct AutoProxyHelperVtbl {
  ...
  DWORD(__stdcall *ResolveHostName)(LPSTR lpszHostName, LPSTR lpszIPAddress,
                                    LPDWORD lpdwIPAddressSize);
  ...
  DWORD(__stdcall *ResolveHostNameEx)(LPSTR lpszHostName, LPSTR lpszIPAddress,
                                      LPDWORD lpdwIPAddressSize);

  BOOL(__stdcall *IsInNetEx)(LPSTR lpszIPAddress, LPSTR lpszIPPrefix);

  DWORD(__stdcall *SortIpList)(LPSTR lpszIPAddressList, LPSTR lpszIPSortedList,
...
BOOLAPI DeleteWpadCacheForNetworks(_In_ WPAD_CACHE_DELETE);

BOOLAPI InternetInitializeAutoProxyDll(_In_ DWORD dwReserved);

BOOLAPI DetectAutoProxyUrl(_Out_writes_(cchAutoProxyUrl) PSTR pszAutoProxyUrl,
                           _In_ DWORD cchAutoProxyUrl,
                           _In_ DWORD dwDetectFlags);

BOOLAPI CreateMD5SSOHash(_In_ PWSTR pszChallengeInfo, _In_ PWSTR pwszRealm,
                         _In_ PWSTR pwszTarget, _Out_ PBYTE pbHexHash);
...
#else
BOOLAPI InternetGetConnectedStateEx(_Out_ LPDWORD lpdwFlags,
                                    _Out_writes_opt_(dwNameLen)
                                        LPSTR lpszConnectionName,
...
BOOLAPI InternetSetDialStateA(_In_opt_ LPCSTR lpszConnectoid,
                              _In_ DWORD dwState, _Reserved_ DWORD dwReserved);

BOOLAPI InternetSetDialStateW(_In_opt_ LPCWSTR lpszConnectoid,
                              _In_ DWORD dwState, _Reserved_ DWORD dwReserved);
...
#else
BOOLAPI InternetSetDialState(_In_opt_ LPCSTR lpszConnectoid, _In_ DWORD dwState,
                             _In_ DWORD dwReserved);
#endif // _WINX32_
#endif // !UNICODE
...
BOOLAPI InternetSetPerSiteCookieDecisionA(_In_ LPCSTR pchHostName,
                                          _In_ DWORD dwDecision);
BOOLAPI InternetSetPerSiteCookieDecisionW(_In_ LPCWSTR pchHostName,
                                          _In_ DWORD dwDecision);
#ifdef UNICODE
...
BOOLAPI InternetGetPerSiteCookieDecisionA(_In_ LPCSTR pchHostName,
                                          _Out_ unsigned long *pResult);
BOOLAPI InternetGetPerSiteCookieDecisionW(_In_ LPCWSTR pchHostName,
                                          _Out_ unsigned long *pResult);
#ifdef UNICODE
...
BOOLAPI InternetEnumPerSiteCookieDecisionA(
    ...
    _Inout_ unsigned long *pcSiteNameSize, _Out_ unsigned long *pdwDecision,
...
INTERNETAPI_(DWORD)
PrivacySetZonePreferenceW(_In_ DWORD dwZone, _In_ DWORD dwType,
                          _In_ DWORD dwTemplate,
                          _In_opt_ LPCWSTR pszPreference);

INTERNETAPI_(DWORD)
PrivacyGetZonePreferenceW(_In_ DWORD dwZone, _In_ DWORD dwType,
                          _Out_opt_ LPDWORD pdwTemplate,
                          _Out_writes_opt_(*pdwBufferLength) LPWSTR pszBuffer,
                          _Inout_opt_ LPDWORD pdwBufferLength);

INTERNETAPI_(DWORD)
HttpIsHostHstsEnabled(_In_z_ PCWSTR pcwszUrl, _Out_ PBOOL pfIsHsts);
