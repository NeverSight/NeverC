...
typedef struct _D3DDeviceDesc_V3 {
  ...
  DWORD dwStencilCaps;
  DWORD
  dwFVFCaps; /* low 4 bits: 0 implies TLVERTEX only, 1..8 imply FVF aware */
...
typedef DWORD(__stdcall *LPD3DHAL_CONTEXTDESTROYCB)(
    LPD3DHAL_CONTEXTDESTROYDATA);
typedef DWORD(__stdcall *LPD3DHAL_CONTEXTDESTROYALLCB)(
...
typedef D3DHAL_DRAWONEINDEXEDPRIMITIVEDATA
...
typedef DWORD(CALLBACK *LPD3DHAL_SETRENDERTARGETCB)(
    LPD3DHAL_SETRENDERTARGETDATA);
typedef DWORD(CALLBACK *LPD3DHAL_CLEARCB)(LPD3DHAL_CLEARDATA);
typedef DWORD(CALLBACK *LPD3DHAL_DRAWONEPRIMITIVECB)(
    LPD3DHAL_DRAWONEPRIMITIVEDATA);
typedef DWORD(CALLBACK *LPD3DHAL_DRAWONEINDEXEDPRIMITIVECB)(
    LPD3DHAL_DRAWONEINDEXEDPRIMITIVEDATA);
...
typedef D3DHAL_VALIDATETEXTURESTAGESTATEDATA
...
typedef struct _D3DHAL_DP2SETVERTEXSHADERCONST {
  ...
  DWORD dwCount;    // Number of 4-float vectors to copy for
                 ...
                 // copy for D3DDP2OP_SETVERTEXSHADERCONSTI Number of BOOL
                 // values to copy for D3DDP2OP_SETVERTEXSHADERCONSTB
  // Data follows
...
typedef struct _D3DHAL_DP2SETCONVOLUTIONKERNELMONO {
  ...
  DWORD dwFlags;
  // If dwFlags & D3DHAL_ROW_WEIGHTS, then width floats follow. Otherwise row
  // weights are 1.0. If dwFlags & D3DHAL_COL_WEIGHTS, then height floats
  // follow. Otherwise column weights are 1.0.
} D3DHAL_DP2SETCONVOLUTIONKERNELMONO;
typedef D3DHAL_DP2SETCONVOLUTIONKERNELMONO
    *LPD3DHAL_DP2SETCONVOLUTIONKERNELMONO;
...
typedef struct _D3DHAL_DRAWPRIMITIVES2DATA {
  ...
  union {                // based on D3DHALDP2_USERMEMVERTICES flag
  ...
  DWORD dwReqVertexBufSize;  // in: number of bytes required for the next vertex
                             // buffer
  DWORD dwReqCommandBufSize; // in: number of bytes required for the next
                             // commnand buffer
  LPDWORD
...
#define D3DVS_GETSWIZZLECOMP(source, component)                                \
...
#define D3DSI_GETSWIZZLECOMP(source, component)                                \
  (source >> ((component << 1) + 16) & 0x3)
...
// the corresponding dwReq* field in D3DHAL_DRAWPRIMITIVES2DATA will also be set
// with the requested size in bytes.
...
typedef DWORD(CALLBACK *LPD3DHAL_VALIDATETEXTURESTAGESTATECB)(
    LPD3DHAL_VALIDATETEXTURESTAGESTATEDATA);
typedef DWORD(CALLBACK *LPD3DHAL_DRAWPRIMITIVES2CB)(
...
typedef HRESULT(CALLBACK *PFND3DPARSEUNKNOWNCOMMAND)(
...
#define D3DRS_MAXVERTEXSHADERINST                                              \
  ((D3DRENDERSTATETYPE)196) // DDI only: vs_3_0+ num instructions to execute.
#define D3DRS_MAXPIXELSHADERINST                                               \
  ((D3DRENDERSTATETYPE)197) // DDI only: ps_3_0+ num instructions to execute.
...
#define D3DGDI2_TYPE_GETEXTENDEDMODECOUNT                                      \
  (0x00000011ul) // Return the number of supported extended mode
#define D3DGDI2_TYPE_GETEXTENDEDMODE                                           \
  (0x00000012ul) // Return a particular extended mode
#define D3DGDI2_TYPE_GETADAPTERGROUP                                           \
  (0x00000013ul) // Return a adapter group information
#define D3DGDI2_TYPE_GETMULTISAMPLEQUALITYLEVELS                               \
  (0x00000016ul) // Return the number of multisample quality levels
#define D3DGDI2_TYPE_DEFERRED_AGP_AWARE                                        \
  (0x00000018ul) // Runtime is aware of deferred AGP frees, and will send
                 // following (NT only)
#define D3DGDI2_TYPE_FREE_DEFERRED_AGP                                         \
  (0x00000019ul) // Free any deferred-freed AGP allocations for this process (NT
                 // only)
#define D3DGDI2_TYPE_DEFER_AGP_FREES                                           \
  (0x00000020ul) // Start defering AGP frees for this process
#define D3DGDI2_TYPE_GETD3DQUERYCOUNT                                          \
  (0x00000021ul) // Return the number of supported queries
