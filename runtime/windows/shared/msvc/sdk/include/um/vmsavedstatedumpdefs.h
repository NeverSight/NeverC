...
Abstract:
    ...
    This module contains the VmSavedSate Dump Provider definitions used by the
APIs.
...
///
typedef void(CALLBACK *GUEST_SYMBOLS_PROVIDER_DEBUG_INFO_CALLBACK)(
    _In_ LPCSTR InfoMessage);

/// Function type for the dos image scan callback.
///
/// \param ImageInfo  Supplies a found valid DOS image in the virtual address
/// space.
///                   Do not assume the PdbName string's pointer will be valid
///                   after the callback has returned.
///
/// \return FALSE if the caller wants the scan to continue.
///         TRUE if the caller has found an image it was looking for and wishes
///         to stop the scan.
///
typedef BOOL(CALLBACK *FOUND_IMAGE_CALLBACK)(_In_ PVOID Context,
                                             _In_ PDOS_IMAGE_INFO ImageInfo);
