...
inline Font::Font(IN const WCHAR *familyName, IN REAL emSize, IN INT style,
  ...
  lastResult = DllExports::GdipCreateFont(nativeFamily, emSize, style, unit,
                                          &nativeFont);
...
inline REAL Font::GetHeight(IN REAL dpi) const {
  REAL height;
...
inline _Post_equal_to_(status) Status Font::SetStatus(IN Status status) const {
  if (status != Ok)
