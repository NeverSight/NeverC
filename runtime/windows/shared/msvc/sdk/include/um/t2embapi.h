...
// Embed EUDC font. If there is typeface EUDC embed it otherwise embed system
// EUDC.
...
// Bit set if font embedding permissions indicate no subset and subset requested
// by client.
...
typedef unsigned long(WINAPIV *READEMBEDPROC)(void *, void *,
                                              const unsigned long);
typedef unsigned long(WINAPIV *WRITEEMBEDPROC)(void *, const void *,
                                               const unsigned long);

typedef struct {
  unsigned short usStructSize; // size in bytes of structure client should set
                               // to sizeof(TTLOADINFO)
  unsigned short usRefStrSize; // size in wide characters of pusRefStr including
                               // NULL terminator
...
typedef struct {
  unsigned short usStructSize;  // size in bytes of structure client should set
                                // to sizeof(TTEMBEDINFO)
  unsigned short usRootStrSize; // size in wide chars of pusSubStr including
                                // NULL terminator(s)
  unsigned short *pusRootStr; // substring(s) of strings given at load time. can
                              // have multiple strings separated
...
LONG WINAPI TTEmbedFont(
    ...
    _Out_ ULONG
        *pulPrivStatus, // upon completion contains embedding priv of font
    _Out_ ULONG
        *pulStatus, // on completion may contain status flags for request
    __callback WRITEEMBEDPROC
        lpfnWriteToStream,      // callback function for doc/disk writes
    _In_ LPVOID lpvWriteStream, // the output stream token
    _In_reads_(usCharCodeCount)
...
LONG WINAPI TTEmbedFontFromFileA(
    ...
    _In_ USHORT
        usTTCIndex,     // If file image is a TTC, this must be zero based index
    _In_ ULONG ulFlags, // flags specifying the request
    _In_ ULONG ulCharSet, // flags specifying char set
    _Out_ ULONG
        *pulPrivStatus, // upon completion contains embedding priv of font
...
LONG WINAPI TTLoadEmbeddedFont(
    _Out_ HANDLE *phFontReference, // on completion, contains handle to identify
                                   // embedded font installed on system
    _In_ ULONG ulFlags,            // flags specifying the request
    ...
    _Out_ ULONG
        *pulStatus, // on completion, may contain status flags for request
    _In_ READEMBEDPROC
        lpfnReadFromStream,    // callback function for doc/disk reads
    _In_ LPVOID lpvReadStream, // the input stream token
    _In_opt_ LPWSTR
        szWinFamilyName, // the new 16 bit windows family name can be NULL
...
LONG WINAPI TTDeleteEmbeddedFont(
    _In_ HANDLE
        hFontReference, // Reference to font value provided by load functions
    _In_ ULONG ulFlags, _Out_ ULONG *pulStatus);
...
LONG WINAPI
TTEnableEmbeddingForFacename( // If fEnable != 0, it removes the indicated
...
LONG WINAPI TTEmbedFontEx(
    ...
    _In_ LPVOID lpvWriteStream, // the output stream token
    _In_reads_(usCharCodeCount)
        ULONG *pulCharCodeSet,   // address of buffer containing optional
                                 // character codes for subsetting
    _In_ USHORT usCharCodeCount, // number of characters in the
                                 // lpvCharCodeSet buffer
    _In_ USHORT usLanguage, // specifies the language in the name table to keep
...
LONG WINAPI TTRunValidationTestsEx(_In_ HDC hDC, // device context handle
                                   _In_ TTVALIDATIONTESTSPARAMSEX *pTestParam //
);

LONG WINAPI TTGetNewFontName(
    _In_ HANDLE *phFontReference, // contains handle to identify embedded font
                                  // installed on system
    _Out_writes_(cchMaxWinName)
        LPWSTR wzWinFamilyName, // the new 16 bit windows family name. Must be
                                // at least LF_FACESIZE long.
    _In_ LONG cchMaxWinName,    // actual length of the windows name.
    _Out_writes_(cchMaxMacName)
        LPSTR szMacFamilyName, // the new 8 bit mac family name. Must be at
                               // least LF_FACESIZE long.
