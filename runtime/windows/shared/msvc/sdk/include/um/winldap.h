...
#define LDAP_FILTER_EQUALITY                                                   \
  0xa3 // context specific + constructed - AttributeValueAssertion.
#define LDAP_FILTER_SUBSTRINGS                                                 \
  0xa4 // context specific + constructed - SubstringFilter
#define LDAP_FILTER_GE                                                         \
  0xa5 // context specific + constructed - AttributeValueAssertion.
#define LDAP_FILTER_LE                                                         \
  0xa6 // context specific + constructed - AttributeValueAssertion.
#define LDAP_FILTER_PRESENT                                                    \
  0x87 // context specific + primitive   - AttributeType.
#define LDAP_FILTER_APPROX                                                     \
  0xa8 // context specific + constructed - AttributeValueAssertion.
#define LDAP_FILTER_EXTENSIBLE                                                 \
  0xa9 // context specific + constructed - MatchingRuleAssertion.
...
WINLDAPAPI LDAP *LDAPAPI ldap_openW(_In_ const PWSTR HostName,
                                    ULONG PortNumber);
WINLDAPAPI LDAP *LDAPAPI ldap_openA(_In_ const PSTR HostName, ULONG PortNumber);

WINLDAPAPI LDAP *LDAPAPI ldap_initW(_In_ const PWSTR HostName,
                                    ULONG PortNumber);
WINLDAPAPI LDAP *LDAPAPI ldap_initA(_In_ const PSTR HostName, ULONG PortNumber);

WINLDAPAPI LDAP *LDAPAPI ldap_sslinitW(_In_ PWSTR HostName, ULONG PortNumber,
                                       int secure);
WINLDAPAPI LDAP *LDAPAPI ldap_sslinitA(_In_ PSTR HostName, ULONG PortNumber,
                                       int secure);
...
WINLDAPAPI LDAP *LDAPAPI ldap_sslinit(_In_ PSTR HostName, ULONG PortNumber,
                                      int secure);
...
//
// The user can set a preferred SASL method prior to binding using
// LDAP_AUTH_NEGOTIATE We will try to use this mechanism while binding. One
// example is "GSSAPI".
//
...
//
// Setting this option to LDAP_OPT_ON will instruct the library to only perform
// an A-Record DNS lookup on the supplied host string. This option is OFF by
// default.
//
...
WINLDAPAPI ULONG LDAPAPI ldap_simple_bindW(LDAP *ld, _In_opt_ PWSTR dn,
                                           _In_opt_ PWSTR passwd);
WINLDAPAPI ULONG LDAPAPI ldap_simple_bindA(LDAP *ld, _In_opt_ PSTR dn,
                                           _In_opt_ PSTR passwd);
WINLDAPAPI ULONG LDAPAPI ldap_simple_bind_sW(LDAP *ld, _In_opt_ PWSTR dn,
                                             _In_opt_ PWSTR passwd);
WINLDAPAPI ULONG LDAPAPI ldap_simple_bind_sA(LDAP *ld, _In_opt_ PSTR dn,
                                             _In_opt_ PSTR passwd);

WINLDAPAPI ULONG LDAPAPI ldap_bindW(LDAP *ld, _In_opt_ PWSTR dn,
                                    _In_opt_ PWCHAR cred, ULONG method);
WINLDAPAPI ULONG LDAPAPI ldap_bindA(LDAP *ld, _In_opt_ PSTR dn,
                                    _In_opt_ PCHAR cred, ULONG method);
WINLDAPAPI ULONG LDAPAPI ldap_bind_sW(LDAP *ld, _In_opt_ PWSTR dn,
                                      _In_opt_ PWCHAR cred, ULONG method);
WINLDAPAPI ULONG LDAPAPI ldap_bind_sA(LDAP *ld, _In_opt_ PSTR dn,
                                      _In_opt_ PCHAR cred, ULONG method);
...
WINLDAPAPI INT LDAPAPI ldap_sasl_bindA(
    LDAP *ExternalHandle, _In_ const PSTR DistName,
    _In_ const PSTR AuthMechanism, const BERVAL *cred,
    PLDAPControlA *ServerCtrls, PLDAPControlA *ClientCtrls, int *MessageNumber);

WINLDAPAPI INT LDAPAPI ldap_sasl_bindW(
    LDAP *ExternalHandle, _In_ const PWSTR DistName,
    _In_ const PWSTR AuthMechanism, const BERVAL *cred,
    PLDAPControlW *ServerCtrls, PLDAPControlW *ClientCtrls, int *MessageNumber);

WINLDAPAPI INT LDAPAPI ldap_sasl_bind_sA(LDAP *ExternalHandle,
...
WINLDAPAPI INT LDAPAPI ldap_sasl_bind_sW(LDAP *ExternalHandle,
...
WINLDAPAPI ULONG LDAPAPI ldap_simple_bind(LDAP *ld, _In_opt_ const PSTR dn,
                                          _In_opt_ const PSTR passwd);
WINLDAPAPI ULONG LDAPAPI ldap_simple_bind_s(LDAP *ld, _In_opt_ const PSTR dn,
                                            _In_opt_ const PSTR passwd);

WINLDAPAPI ULONG LDAPAPI ldap_bind(LDAP *ld, _In_opt_ const PSTR dn,
                                   _In_opt_ const PCHAR cred, ULONG method);
WINLDAPAPI ULONG LDAPAPI ldap_bind_s(LDAP *ld, _In_opt_ const PSTR dn,
                                     _In_opt_ const PCHAR cred, ULONG method);
...
ldap_search_sW(LDAP *ld, _In_opt_ const PWSTR base, ULONG scope,
               _In_ const PWSTR filter, _In_ PZPWSTR attrs, ULONG attrsonly,
               _Outptr_result_maybenull_ LDAPMessage **res);
WINLDAPAPI ULONG LDAPAPI
ldap_search_sA(LDAP *ld, _In_opt_ const PSTR base, ULONG scope,
               _In_ const PSTR filter, _In_ PZPSTR attrs, ULONG attrsonly,
               _Outptr_result_maybenull_ LDAPMessage **res);

WINLDAPAPI ULONG LDAPAPI ldap_search_stW(
    LDAP *ld, _In_opt_ const PWSTR base, ULONG scope, _In_ const PWSTR filter,
    _In_ PZPWSTR attrs, ULONG attrsonly, struct l_timeval *timeout,
    _Outptr_result_maybenull_ PLDAPMessage *res);
WINLDAPAPI ULONG LDAPAPI ldap_search_stA(
    LDAP *ld, _In_opt_ const PSTR base, ULONG scope, _In_ const PSTR filter,
    _In_ PZPSTR attrs, ULONG attrsonly, struct l_timeval *timeout,
    _Outptr_result_maybenull_ PLDAPMessage *res);

WINLDAPAPI ULONG LDAPAPI ldap_search_extW(
    LDAP *ld, _In_opt_ const PWSTR base, ULONG scope, _In_ const PWSTR filter,
    _In_ PZPWSTR attrs, ULONG attrsonly, _In_opt_ PLDAPControlW *ServerControls,
    _In_opt_ PLDAPControlW *ClientControls, ULONG TimeLimit, ULONG SizeLimit,
    ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_search_extA(
    LDAP *ld, _In_opt_ const PSTR base, ULONG scope, _In_ const PSTR filter,
    _In_ PZPSTR attrs, ULONG attrsonly, _In_opt_ PLDAPControlA *ServerControls,
    _In_opt_ PLDAPControlA *ClientControls, ULONG TimeLimit, ULONG SizeLimit,
    ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_search_ext_sW(
    LDAP *ld, _In_opt_ const PWSTR base, ULONG scope, _In_ const PWSTR filter,
    _In_ PZPWSTR attrs, ULONG attrsonly, _In_opt_ PLDAPControlW *ServerControls,
    _In_opt_ PLDAPControlW *ClientControls, struct l_timeval *timeout,
    ULONG SizeLimit, _Outptr_result_maybenull_ PLDAPMessage *res);

WINLDAPAPI ULONG LDAPAPI ldap_search_ext_sA(
    LDAP *ld, _In_opt_ const PSTR base, ULONG scope, _In_ const PSTR filter,
    _In_ PZPSTR attrs, ULONG attrsonly, _In_opt_ PLDAPControlA *ServerControls,
    _In_opt_ PLDAPControlA *ClientControls, struct l_timeval *timeout,
    ULONG SizeLimit, _Outptr_result_maybenull_ PLDAPMessage *res);
...
WINLDAPAPI ULONG LDAPAPI
ldap_search_s(LDAP *ld, _In_opt_ PSTR base, ULONG scope, _In_ PSTR filter,
              _In_ PZPSTR attrs, ULONG attrsonly,
              _Outptr_result_maybenull_ PLDAPMessage *res);

WINLDAPAPI ULONG LDAPAPI
ldap_search_st(LDAP *ld, _In_opt_ PSTR base, ULONG scope, _In_ PSTR filter,
               _In_ PZPSTR attrs, ULONG attrsonly, struct l_timeval *timeout,
               _Outptr_result_maybenull_ PLDAPMessage *res);

WINLDAPAPI ULONG LDAPAPI ldap_search_ext(LDAP *ld, _In_opt_ PSTR base,
                                         ULONG scope, _In_ PSTR filter,
                                         _In_ PZPSTR attrs, ULONG attrsonly,
...
WINLDAPAPI ULONG LDAPAPI ldap_search_ext_s(
    LDAP *ld, _In_opt_ PSTR base, ULONG scope, _In_ PSTR filter,
    _In_ PZPSTR attrs, ULONG attrsonly, _In_opt_ PLDAPControlA *ServerControls,
    _In_opt_ PLDAPControlA *ClientControls, struct l_timeval *timeout,
    ULONG SizeLimit, _Outptr_result_maybenull_ PLDAPMessage *res);
...
WINLDAPAPI ULONG LDAPAPI ldap_check_filterW(LDAP *ld, _In_ PWSTR SearchFilter);

WINLDAPAPI ULONG LDAPAPI ldap_check_filterA(LDAP *ld, _In_ PSTR SearchFilter);
...
WINLDAPAPI ULONG LDAPAPI ldap_modifyW(LDAP *ld, _In_ PWSTR dn,
                                      LDAPModW *mods[]);
WINLDAPAPI ULONG LDAPAPI ldap_modifyA(LDAP *ld, _In_ PSTR dn, LDAPModA *mods[]);

WINLDAPAPI ULONG LDAPAPI ldap_modify_sW(LDAP *ld, _In_ PWSTR dn,
                                        LDAPModW *mods[]);
WINLDAPAPI ULONG LDAPAPI ldap_modify_sA(LDAP *ld, _In_ PSTR dn,
                                        LDAPModA *mods[]);

WINLDAPAPI ULONG LDAPAPI ldap_modify_extW(LDAP *ld, _In_ const PWSTR dn,
                                          LDAPModW *mods[],
                                          PLDAPControlW *ServerControls,
                                          PLDAPControlW *ClientControls,
                                          ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_modify_extA(LDAP *ld, _In_ const PSTR dn,
                                          LDAPModA *mods[],
                                          PLDAPControlA *ServerControls,
                                          PLDAPControlA *ClientControls,
                                          ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_modify_ext_sW(LDAP *ld, _In_ const PWSTR dn,
                                            LDAPModW *mods[],
                                            PLDAPControlW *ServerControls,
                                            PLDAPControlW *ClientControls);

WINLDAPAPI ULONG LDAPAPI ldap_modify_ext_sA(LDAP *ld, _In_ const PSTR dn,
                                            LDAPModA *mods[],
                                            PLDAPControlA *ServerControls,
                                            PLDAPControlA *ClientControls);
...
WINLDAPAPI ULONG LDAPAPI ldap_modify_s(LDAP *ld, _In_ PSTR dn,
                                       LDAPModA *mods[]);

WINLDAPAPI ULONG LDAPAPI ldap_modify_ext(LDAP *ld, const PSTR dn,
                                         LDAPModA *mods[],
                                         PLDAPControlA *ServerControls,
                                         PLDAPControlA *ClientControls,
                                         ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_modify_ext_s(LDAP *ld, const PSTR dn,
                                           LDAPModA *mods[],
                                           PLDAPControlA *ServerControls,
                                           PLDAPControlA *ClientControls);
#endif
...
WINLDAPAPI ULONG LDAPAPI ldap_rename_extW(LDAP *ld, _In_ const PWSTR dn,
...
WINLDAPAPI ULONG LDAPAPI ldap_rename_extA(LDAP *ld, _In_ const PSTR dn,
...
WINLDAPAPI ULONG LDAPAPI ldap_rename_ext_sW(LDAP *ld, _In_ const PWSTR dn,
                                            _In_ const PWSTR NewRDN,
                                            _In_opt_ const PWSTR NewParent,
                                            INT DeleteOldRdn,
                                            PLDAPControlW *ServerControls,
...
WINLDAPAPI ULONG LDAPAPI ldap_rename_ext_sA(LDAP *ld, _In_ const PSTR dn,
                                            _In_ const PSTR NewRDN,
                                            _In_opt_ const PSTR NewParent,
                                            INT DeleteOldRdn,
                                            PLDAPControlA *ServerControls,
...
WINLDAPAPI ULONG LDAPAPI ldap_rename_ext(LDAP *ld, const PSTR dn,
                                         const PSTR NewRDN,
                                         const PSTR NewParent, INT DeleteOldRdn,
                                         PLDAPControlA *ServerControls,
                                         PLDAPControlA *ClientControls,
                                         ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_rename_ext_s(LDAP *ld, const PSTR dn,
                                           const PSTR NewRDN,
...
WINLDAPAPI ULONG LDAPAPI ldap_add_sW(LDAP *ld, _In_ PWSTR dn,
                                     LDAPModW *attrs[]);
WINLDAPAPI ULONG LDAPAPI ldap_add_sA(LDAP *ld, _In_ PSTR dn, LDAPModA *attrs[]);

WINLDAPAPI ULONG LDAPAPI ldap_add_extW(LDAP *ld, _In_ const PWSTR dn,
                                       LDAPModW *attrs[],
                                       PLDAPControlW *ServerControls,
                                       PLDAPControlW *ClientControls,
                                       ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_add_extA(LDAP *ld, _In_ const PSTR dn,
                                       LDAPModA *attrs[],
                                       PLDAPControlA *ServerControls,
                                       PLDAPControlA *ClientControls,
                                       ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_add_ext_sW(LDAP *ld, _In_ const PWSTR dn,
                                         LDAPModW *attrs[],
                                         PLDAPControlW *ServerControls,
                                         PLDAPControlW *ClientControls);

WINLDAPAPI ULONG LDAPAPI ldap_add_ext_sA(LDAP *ld, _In_ const PSTR dn,
                                         LDAPModA *attrs[],
                                         PLDAPControlA *ServerControls,
                                         PLDAPControlA *ClientControls);
...
WINLDAPAPI ULONG LDAPAPI ldap_add_ext(LDAP *ld, const PSTR dn,
                                      LDAPModA *attrs[],
                                      PLDAPControlA *ServerControls,
                                      PLDAPControlA *ClientControls,
                                      ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_add_ext_s(LDAP *ld, const PSTR dn,
                                        LDAPModA *attrs[],
                                        PLDAPControlA *ServerControls,
                                        PLDAPControlA *ClientControls);
...
WINLDAPAPI ULONG LDAPAPI ldap_compareW(LDAP *ld, _In_ const PWSTR dn,
                                       _In_ const PWSTR attr, _In_ PWSTR value);
WINLDAPAPI ULONG LDAPAPI ldap_compareA(LDAP *ld, _In_ const PSTR dn,
                                       _In_ const PSTR attr, _In_ PSTR value);

WINLDAPAPI ULONG LDAPAPI ldap_compare_sW(LDAP *ld, _In_ const PWSTR dn,
                                         _In_ const PWSTR attr,
                                         _In_ PWSTR value);
WINLDAPAPI ULONG LDAPAPI ldap_compare_sA(LDAP *ld, _In_ const PSTR dn,
                                         _In_ const PSTR attr, _In_ PSTR value);
...
WINLDAPAPI ULONG LDAPAPI ldap_compare(LDAP *ld, _In_ const PSTR dn,
                                      _In_ const PSTR attr, _In_ PSTR value);
WINLDAPAPI ULONG LDAPAPI ldap_compare_s(LDAP *ld, _In_ const PSTR dn,
                                        _In_ const PSTR attr, _In_ PSTR value);
...
WINLDAPAPI ULONG LDAPAPI
ldap_compare_ext(LDAP *ld, const PSTR dn, const PSTR Attr,
...
WINLDAPAPI ULONG LDAPAPI ldap_compare_ext_s(
    LDAP *ld, const PSTR dn, const PSTR Attr,
    const PSTR Value, // either value or Data is not null, not both
    struct berval *Data, PLDAPControlA *ServerControls,
    PLDAPControlA *ClientControls);
#endif
...
WINLDAPAPI ULONG LDAPAPI ldap_delete_extW(LDAP *ld, _In_ const PWSTR dn,
                                          PLDAPControlW *ServerControls,
                                          PLDAPControlW *ClientControls,
                                          ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_delete_extA(LDAP *ld, _In_ const PSTR dn,
                                          PLDAPControlA *ServerControls,
                                          PLDAPControlA *ClientControls,
                                          ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_delete_ext_sW(LDAP *ld, _In_ const PWSTR dn,
                                            PLDAPControlW *ServerControls,
                                            PLDAPControlW *ClientControls);

WINLDAPAPI ULONG LDAPAPI ldap_delete_ext_sA(LDAP *ld, _In_ const PSTR dn,
                                            PLDAPControlA *ServerControls,
                                            PLDAPControlA *ClientControls);
...
WINLDAPAPI ULONG LDAPAPI ldap_delete_ext(LDAP *ld, const PSTR dn,
                                         PLDAPControlA *ServerControls,
                                         PLDAPControlA *ClientControls,
                                         ULONG *MessageNumber);

WINLDAPAPI ULONG LDAPAPI ldap_delete_ext_s(LDAP *ld, const PSTR dn,
                                           PLDAPControlA *ServerControls,
                                           PLDAPControlA *ClientControls);
#endif
...
WINLDAPAPI ULONG LDAPAPI
ldap_result(LDAP *ld, ULONG msgid, ULONG all, _In_opt_ PLDAP_TIMEVAL timeout,
            _Outptr_result_maybenull_ PLDAPMessage *res);
...
WINLDAPAPI ULONG LDAPAPI ldap_parse_resultW(
    ...
    _Outptr_opt_result_maybenull_ PZPWSTR
        *Referrals,                 // free with ldap_value_freeW
...
WINLDAPAPI ULONG LDAPAPI ldap_parse_resultA(
    ...
    _Outptr_opt_result_maybenull_ PZPSTR
        *Referrals,                 // free with ldap_value_freeA
...
WINLDAPAPI ULONG LDAPAPI ldap_parse_result(
    ...
    _Outptr_opt_result_maybenull_ PSTR *MatchedDNs
        OPTIONAL, // free with ldap_memfree
    _Outptr_opt_result_maybenull_ PSTR *ErrorMessage
        OPTIONAL, // free with ldap_memfree
    _Outptr_opt_result_maybenull_ PSTR **Referrals
        OPTIONAL,                            // free with ldap_value_free
...
WINLDAPAPI _Ret_maybenull_ PWCHAR *LDAPAPI
ldap_get_valuesW(LDAP *ld, LDAPMessage *entry, _In_ const PWSTR attr);
WINLDAPAPI _Ret_maybenull_ PCHAR *LDAPAPI
ldap_get_valuesA(LDAP *ld, LDAPMessage *entry, _In_ const PSTR attr);
...
WINLDAPAPI _Ret_maybenull_ PCHAR *
    LDAPAPI ldap_get_values(LDAP *ld, LDAPMessage *entry, _In_ const PSTR attr);
#endif
...
WINLDAPAPI ULONG LDAPAPI ldap_ufn2dnW(_In_ const PWSTR ufn,
                                      _Outptr_ PWSTR *pDn);
WINLDAPAPI ULONG LDAPAPI ldap_ufn2dnA(_In_ const PSTR ufn, _Outptr_ PSTR *pDn);
...
WINLDAPAPI ULONG LDAPAPI ldap_ufn2dn(_In_ const PSTR ufn, _Outptr_ PSTR *pDn);
...
//
// Calls to retrieve basic information about the API and specific
// implementations being used. The caller has to pass the LDAP_OPT_API_INFO
// option along with a pointer to the following structure to retrieve
// information about this library. It is the caller's responsibility to free the
// individual strings and string arrays in the structure using ldap_memfree()
// and ldap_value_free() respectively.
//
...
WINLDAPAPI int LDAPAPI LdapUTF8ToUnicode(_In_reads_(cchSrc) LPCSTR lpSrcStr,
                                         int cchSrc,
                                         _Out_writes_(cchDest) LPWSTR lpDestStr,
                                         int cchDest);

WINLDAPAPI
int LDAPAPI LdapUnicodeToUTF8(_In_reads_(cchSrc) LPCWSTR lpSrcStr, int cchSrc,
...
WINLDAPAPI ULONG LDAPAPI ldap_parse_sort_controlA(
    PLDAP ExternalHandle, PLDAPControlA *Control, ULONG *Result,
    _Outptr_opt_result_maybenull_ PCHAR *Attribute);

WINLDAPAPI ULONG LDAPAPI ldap_parse_sort_controlW(
    PLDAP ExternalHandle, PLDAPControlW *Control, ULONG *Result,
...
WINLDAPAPI ULONG LDAPAPI ldap_parse_page_controlW(
    PLDAP ExternalHandle, PLDAPControlW *ServerControls, ULONG *TotalCount,
    struct berval **Cookie // Use ber_bvfree to free
);

WINLDAPAPI ULONG LDAPAPI ldap_parse_page_controlA(
    PLDAP ExternalHandle, PLDAPControlA *ServerControls, ULONG *TotalCount,
    struct berval **Cookie // Use ber_bvfree to free
);
...
WINLDAPAPI PLDAPSearch LDAPAPI ldap_search_init_pageW(
    PLDAP ExternalHandle, _In_ const PWSTR DistinguishedName,
    ULONG ScopeOfSearch, _In_ const PWSTR SearchFilter,
    _In_ PZPWSTR AttributeList, ULONG AttributesOnly,
...
WINLDAPAPI PLDAPSearch LDAPAPI ldap_search_init_pageA(
    PLDAP ExternalHandle, _In_ const PSTR DistinguishedName,
    ULONG ScopeOfSearch, _In_ const PSTR SearchFilter,
    _In_ PZPSTR AttributeList, ULONG AttributesOnly,
...
WINLDAPAPI PLDAPSearch LDAPAPI ldap_search_init_page(
    PLDAP ExternalHandle, const PSTR DistinguishedName, ULONG ScopeOfSearch,
    const PSTR SearchFilter, PZPSTR AttributeList, ULONG AttributesOnly,
...
//
// If the server fails the request for whatever reason, the API returns
// LDAP_OTHER and the ServerReturnValue will contain the error code from the
// server.
//
...
// reasons, we have decided to NOT chase referrals by default. In the former
// case the referral message is returned as an LDAPMessage to the user.
//
// The operation has a default timeout of about 30 seconds.
//

WINLDAPAPI ULONG LDAPAPI ldap_start_tls_sW(IN PLDAP ExternalHandle,
                                           OUT PULONG ServerReturnValue,
                                           OUT LDAPMessage **result,
                                           IN PLDAPControlW *ServerControls,
                                           IN PLDAPControlW *ClientControls);

WINLDAPAPI ULONG LDAPAPI ldap_start_tls_sA(IN PLDAP ExternalHandle,
                                           OUT PULONG ServerReturnValue,
                                           OUT LDAPMessage **result,
...
WINLDAPAPI ULONG LDAPAPI ldap_extended_operationW(LDAP *ld,
...
WINLDAPAPI ULONG LDAPAPI ldap_extended_operationA(LDAP *ld, _In_ const PSTR Oid,
                                                  struct berval *Data,
                                                  PLDAPControlA *ServerControls,
                                                  PLDAPControlA *ClientControls,
                                                  ULONG *MessageNumber);
...
WINLDAPAPI ULONG LDAPAPI ldap_extended_operation_sA(
    LDAP *ExternalHandle, _In_ PSTR Oid, struct berval *Data,
    PLDAPControlA *ServerControls, PLDAPControlA *ClientControls,
    _Outptr_ PCHAR *ReturnedOid, struct berval **ReturnedData);

WINLDAPAPI ULONG LDAPAPI ldap_extended_operation_sW(
    LDAP *ExternalHandle, _In_ PWSTR Oid, struct berval *Data,
    PLDAPControlW *ServerControls, PLDAPControlW *ClientControls,
    _Outptr_ PWCHAR *ReturnedOid, struct berval **ReturnedData);
...
WINLDAPAPI ULONG LDAPAPI ldap_extended_operation(LDAP *ld, const PSTR Oid,
                                                 struct berval *Data,
                                                 PLDAPControlA *ServerControls,
                                                 PLDAPControlA *ClientControls,
                                                 ULONG *MessageNumber);
...
// authorization. The application should examine the list of Certificate
// Authorities the server trusts and supply an appropriate client certificate.
// wldap32.dll subsequently passes these credentials to the SSL server as part
// of the handshake. If the application desires that anonymous credentials be
// used, it must return FALSE instead of a certificate. Any certificate must be
// freed by the application after the connection has been completed. Note that
// the application MUST perform an EXTERNAL bind subsequent to connection
// establishment for these credentials to be used by the server.
//

typedef BOOLEAN(_cdecl QUERYCLIENTCERT)(
    IN PLDAP Connection, IN PSecPkgContext_IssuerListInfoEx trusted_CAs,
...
// callback, simply call ldap_set_option( conn, LDAP_OPT_SERVER_CERTIFICATE,
// &CertRoutine )
//
