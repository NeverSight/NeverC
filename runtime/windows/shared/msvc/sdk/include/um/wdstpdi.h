...
HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderCreateInstance(_In_ PCWSTR pwszConfigString,
                                   _Out_ PHANDLE phInstance);

HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderCompareContent(_In_ HANDLE hInstance,
...
HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderOpenContent(_In_ HANDLE hInstance,
                                _In_ PCWSTR pwszContentName,
                                _Out_ PHANDLE phContent);

HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderGetContentMetadata(_In_ HANDLE hContent,
                                       _Out_writes_bytes_(*pulLength)
                                           PVOID *pvData,
                                       _Out_ PULONG pulLength);

HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderUserAccessCheck(_In_ HANDLE hContent,
                                    _In_ HANDLE hUserToken,
                                    _Out_ PBOOL pbAccessAllowed);

HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderGetContentSize(_In_ HANDLE hContent,
                                   _Out_ PULARGE_INTEGER pContentSize);

HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderReadContent(_In_ HANDLE hContent,
...
HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderCloseContent(_In_ HANDLE hContent);

HRESULT
WDSTRANSPORTPROVIDERAPI
WdsTransportProviderCloseInstance(_In_ HANDLE hInstance);
...
WdsTransportServerRegisterCallback(
    _In_ HANDLE hProvider, _In_ TRANSPORTPROVIDER_CALLBACK_ID CallbackId,
    _In_ PVOID pfnCallback);

HRESULT
WDSMCSAPI
WdsTransportServerCompleteRead(_In_ HANDLE hProvider, _In_ ULONG ulBytesRead,
                               _In_ PVOID pvUserData, _In_ HRESULT hReadResult);

HRESULT
WDSMCSAPI
WdsTransportServerTrace(_In_ HANDLE hProvider, _In_ WDS_MC_SEVERITY Severity,
                        _In_ LPCWSTR pwszFormat, ...);

HRESULT
WDSMCSAPI
WdsTransportServerTraceV(_In_ HANDLE hProvider, _In_ WDS_MC_SEVERITY Severity,
                         _In_ LPCWSTR pwszFormat, _In_ va_list Params);

PVOID
WDSMCSAPI
WdsTransportServerAllocateBuffer(_In_ HANDLE hProvider,
                                 _In_ ULONG ulBufferSize);

HRESULT
WDSMCSAPI
WdsTransportServerFreeBuffer(_In_ HANDLE hProvider, _In_ PVOID pvBuffer);
