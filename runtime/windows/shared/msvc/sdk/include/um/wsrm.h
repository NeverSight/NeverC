...
**
**	This file contains PGM specific information for use by WinSock2
* compatible
...
// Set/Query rate (Kb/Sec) + window size (Kb and/or MSec) -- described by
// RM_SEND_WINDOW below
...
// Set or Query the window advance method on the sender -- methods enumerated in
...
// Set/Query the Window's Advance rate (has to be less that
...
typedef struct _RM_SENDER_STATS {
  ...
  ULONGLONG RateKBitsPerSecOverall; // Internally calculated send-rate from the
                                    // beginning
...
typedef struct _RM_RECEIVER_STATS {
  ...
  ULONGLONG RateKBitsPerSecOverall; // Internally calculated Receive-rate from
                                    // the beginning
  ...
  ULONGLONG NumOutstandingNaks;     // # Sequences for which Ncfs have been
                                    // received, but no data
  ULONGLONG
  NumDataPacketsBuffered; // # Data packets currently buffered by transport
