...
#define DSGETDC_VALID_FLAGS                                                    \
  (DS_FORCE_REDISCOVERY | DS_DIRECTORY_SERVICE_REQUIRED |                      \
   ...
   DS_TRY_NEXTCLOSEST_SITE | DS_DIRECTORY_SERVICE_6_REQUIRED |                 \
   DS_DIRECTORY_SERVICE_8_REQUIRED | DS_DIRECTORY_SERVICE_9_REQUIRED |         \
   DS_DIRECTORY_SERVICE_10_REQUIRED | DS_KEY_LIST_SUPPORT_REQUIRED |           \
...
#define DS_DIRECTORY_SERVICE_ALL_VERSIONS                                      \
  (DS_DIRECTORY_SERVICE_REQUIRED | DS_DIRECTORY_SERVICE_6_REQUIRED |           \
   DS_DIRECTORY_SERVICE_8_REQUIRED | DS_DIRECTORY_SERVICE_9_REQUIRED |         \
   DS_DIRECTORY_SERVICE_10_REQUIRED | DS_DIRECTORY_SERVICE_13_REQUIRED)
...
#define DS_GOOD_TIMESERV_FLAG                                                  \
  0x00000200 // DC is running time service (and has clock hardware)
#define DS_NDNC_FLAG                                                           \
  0x00000400 // DomainName is non-domain NC serviced by the LDAP server
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetDcNameA(_In_opt_ LPCSTR ComputerName, _In_opt_ LPCSTR DomainName,
                 _In_opt_ GUID *DomainGuid, _In_opt_ LPCSTR SiteName,
                 _In_ ULONG Flags,
                 _Outptr_result_nullonfailure_ PDOMAIN_CONTROLLER_INFOA
                     *DomainControllerInfo);

_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetDcNameW(_In_opt_ LPCWSTR ComputerName, _In_opt_ LPCWSTR DomainName,
                 _In_opt_ GUID *DomainGuid, _In_opt_ LPCWSTR SiteName,
                 _In_ ULONG Flags,
                 _Outptr_result_nullonfailure_ PDOMAIN_CONTROLLER_INFOW
                     *DomainControllerInfo);
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetSiteNameA(_In_opt_ LPCSTR ComputerName,
                   _Outptr_result_nullonfailure_ LPSTR *SiteName);

_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetSiteNameW(_In_opt_ LPCWSTR ComputerName,
...
DWORD
WINAPI
DsValidateSubnetNameW(_In_ LPCWSTR SubnetName);

DSGETDCAPI
DWORD
WINAPI
DsValidateSubnetNameA(_In_ LPCSTR SubnetName);
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsAddressToSiteNamesW(
        _In_opt_ LPCWSTR ComputerName, _In_ DWORD EntryCount,
        _In_reads_(EntryCount) PSOCKET_ADDRESS SocketAddresses,
        _Outptr_result_nullonfailure_ _Outptr_result_buffer_(EntryCount)
            LPWSTR **SiteNames);

_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsAddressToSiteNamesA(
        _In_opt_ LPCSTR ComputerName, _In_ DWORD EntryCount,
        _In_reads_(EntryCount) PSOCKET_ADDRESS SocketAddresses,
        _Outptr_result_nullonfailure_ _Outptr_result_buffer_(EntryCount)
            LPSTR **SiteNames);
...
#define DS_DOMAIN_PRIMARY                                                      \
  0x0008 // Domain is the primary domain of queried server
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsEnumerateDomainTrustsW(
        _In_opt_ LPWSTR ServerName, _In_ ULONG Flags,
        _Outptr_result_nullonfailure_ _Outptr_result_buffer_(*DomainCount)
            PDS_DOMAIN_TRUSTSW *Domains,
        _Out_ PULONG DomainCount);

_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsEnumerateDomainTrustsA(
        _In_opt_ LPSTR ServerName, _In_ ULONG Flags,
        _Outptr_result_nullonfailure_ _Outptr_result_buffer_(*DomainCount)
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetForestTrustInformationW(
        _In_opt_ LPCWSTR ServerName, _In_opt_ LPCWSTR TrustedDomainName,
        _In_ DWORD Flags,
        _Outptr_result_nullonfailure_ PLSA_FOREST_TRUST_INFORMATION
            *ForestTrustInfo);

#define DS_GFTI_UPDATE_TDO 0x1 // Update TDO with information returned
#define DS_GFTI_VALID_FLAGS                                                    \
  0x1 // All valid flags to DsGetForestTrustInformation
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI DsGetDcSiteCoverageW(
    _In_opt_ LPCWSTR ServerName, _Out_ PULONG EntryCount,
    _Outptr_result_nullonfailure_ _Outptr_result_buffer_(*EntryCount)
        LPWSTR **SiteNames);

_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI DsGetDcSiteCoverageA(
    _In_opt_ LPCSTR ServerName, _Out_ PULONG EntryCount,
    _Outptr_result_nullonfailure_ _Outptr_result_buffer_(*EntryCount)
        LPSTR **SiteNames);
...
DWORD
WINAPI
DsDeregisterDnsHostRecordsW(_In_opt_ LPWSTR ServerName,
                            _In_opt_ LPWSTR DnsDomainName,
                            _In_opt_ GUID *DomainGuid, _In_opt_ GUID *DsaGuid,
                            _In_ LPWSTR DnsHostName);

DSGETDCAPI
DWORD
WINAPI
DsDeregisterDnsHostRecordsA(_In_opt_ LPSTR ServerName,
...
DWORD
WINAPI
DsGetDcOpenW(_In_ LPCWSTR DnsName, _In_ ULONG OptionFlags,
             _In_opt_ LPCWSTR SiteName, _In_opt_ GUID *DomainGuid,
             _In_opt_ LPCWSTR DnsForestName, _In_ ULONG DcFlags,
             _Out_ PHANDLE RetGetDcContext);

DSGETDCAPI
DWORD
WINAPI
DsGetDcOpenA(_In_ LPCSTR DnsName, _In_ ULONG OptionFlags,
             _In_opt_ LPCSTR SiteName, _In_opt_ GUID *DomainGuid,
             _In_opt_ LPCSTR DnsForestName, _In_ ULONG DcFlags,
             _Out_ PHANDLE RetGetDcContext);
...
_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetDcNextW(_In_ HANDLE GetDcContextHandle,
                 _Out_opt_ PULONG SockAddressCount,
                 _Outptr_opt_result_buffer_(*SockAddressCount)
                     LPSOCKET_ADDRESS *SockAddresses,
                 _Outptr_opt_result_nullonfailure_ LPWSTR *DnsHostName);

_Success_(return == ERROR_SUCCESS) DSGETDCAPI DWORD WINAPI
    DsGetDcNextA(_In_ HANDLE GetDcContextHandle,
                 _Out_opt_ PULONG SockAddressCount,
                 _Outptr_opt_result_buffer_(*SockAddressCount)
                     LPSOCKET_ADDRESS *SockAddresses,
...
DSGETDCAPI
VOID WINAPI DsGetDcCloseW(_In_ HANDLE GetDcContextHandle);
