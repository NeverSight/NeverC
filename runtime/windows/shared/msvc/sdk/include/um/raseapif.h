...
#define RAS_EAP_FLAG_CONFG_READONLY                                            \
  0x00080000 // connection settings are read-only
...
#define RAS_EAP_FLAG_SERVER_VALIDATION_REQUIRED                                \
  0x02000000 // Certificate server validation must be performed
...
typedef struct _PPP_EAP_INPUT {
  ...
  _Field_size_bytes_opt_(dwSizeOfDataFromInteractiveUI) PBYTE
...
typedef struct _PPP_EAP_INFO {
  ...
  DWORD(APIENTRY *RasEapBegin)(_Outptr_ VOID **ppWorkBuffer,
  ...
  DWORD(APIENTRY *RasEapMakeMessage)(_In_ VOID *pWorkBuf,
                                     _In_ PPP_EAP_PACKET *pReceivePacket,
                                     _Inout_updates_(cbSendPacket)
...
DWORD APIENTRY RasEapGetInfo(_In_ DWORD dwEapTypeId,
                             _Out_ PPP_EAP_INFO *pEapInfo);
...
DWORD APIENTRY RasEapInvokeInteractiveUI(
    _In_ DWORD dwEapTypeId, _In_ HWND hwndParent,
...
DWORD APIENTRY RasEapInvokeConfigUI(
    _In_ DWORD dwEapTypeId, _In_ HWND hwndParent, _In_ DWORD dwFlags,
    _In_reads_(dwSizeOfConnectionDataIn) BYTE *pConnectionDataIn,
    _In_ DWORD dwSizeOfConnectionDataIn,
    _Outptr_result_buffer_(*pdwSizeOfConnectionDataOut)
...
DWORD APIENTRY RasEapGetIdentity(
    _In_ DWORD dwEapTypeId, _In_ HWND hwndParent, _In_ DWORD dwFlags,
    _In_ LPCWSTR pwszPhonebook, _In_ LPCWSTR pwszEntry,
    _In_reads_(dwSizeOfConnectionDataIn) BYTE *pConnectionDataIn,
    _In_ DWORD dwSizeOfConnectionDataIn,
    ...
    _Out_ DWORD *pdwSizeOfUserDataOut, _Outptr_ LPWSTR *ppwszIdentityOut);
