...
typedef PVOID(__cdecl *PFN_COMPRESS_ALLOCATE)(_In_ PVOID UserContext,
                                              _In_ SIZE_T Size);

typedef VOID(__cdecl *PFN_COMPRESS_FREE)(_In_ PVOID UserContext,
                                         _In_ PVOID Memory);
...
BOOL WINAPI
CreateCompressor(_In_ DWORD Algorithm,
...
BOOL WINAPI Compress(_In_ COMPRESSOR_HANDLE CompressorHandle,
                     _In_reads_bytes_opt_(UncompressedDataSize)
                         LPCVOID UncompressedData,
                     _In_ SIZE_T UncompressedDataSize,
                     _Out_writes_bytes_opt_(CompressedBufferSize)
                         PVOID CompressedBuffer,
                     _In_ SIZE_T CompressedBufferSize,
                     _Out_ PSIZE_T CompressedDataSize);

BOOL WINAPI ResetCompressor(_In_ COMPRESSOR_HANDLE CompressorHandle);

BOOL WINAPI CloseCompressor(_In_ COMPRESSOR_HANDLE CompressorHandle);

// Decompression routines

BOOL WINAPI
CreateDecompressor(_In_ DWORD Algorithm,
                   _In_opt_ PCOMPRESS_ALLOCATION_ROUTINES AllocationRoutines,
...
BOOL WINAPI Decompress(_In_ DECOMPRESSOR_HANDLE DecompressorHandle,
                       _In_reads_bytes_opt_(CompressedDataSize)
                           LPCVOID CompressedData,
                       _In_ SIZE_T CompressedDataSize,
                       _Out_writes_bytes_opt_(UncompressedBufferSize)
                           PVOID UncompressedBuffer,
                       _In_ SIZE_T UncompressedBufferSize,
                       _Out_opt_ PSIZE_T UncompressedDataSize);

BOOL WINAPI ResetDecompressor(_In_ DECOMPRESSOR_HANDLE DecompressorHandle);

BOOL WINAPI CloseDecompressor(_In_ DECOMPRESSOR_HANDLE DecompressorHandle);
