...
Routine Description:
    ...
    other routine in the library may be called. It serves to initialize any
global state that may be required by other routines in the file.
...
Routine Description:
    ...
    This routine is used to set the current file system cache working set size.
It requires that the caller has enabled the SE_INCREASE_QUOTA_PRIVILEGE in the
currently active token prior to invoking this routine.
...
Arguments:
    ...
    Flags - Flags relevant to the file cache size adjustment. Currently this
must be zero.

Return Value:

    Boolean status. Error code available via GetLastError (). If the routine is
        invoked prior to invoking the initialization routine, the returned error
code will be ERROR_INVALID_FUNCTION.

--*/

BOOL WINAPI AuxUlibSetSystemFileCacheSize(_In_ SIZE_T MinimumFileCacheSize,
...
BOOL WINAPI AuxUlibIsDLLSynchronizationHeld(_Out_ PBOOL SynchronizationHeld);
