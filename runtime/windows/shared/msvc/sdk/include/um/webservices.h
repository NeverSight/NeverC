...
typedef struct _WS_XML_CANONICALIZATION_PROPERTY
...
typedef struct _WS_HTTP_HEADER_AUTH_BINDING_TEMPLATE
...
//  XML Canonicalization enum
//
//   Each XML canonicalization property is identified by an ID and has an
//   associated
//  value.
//
typedef enum {
...
//  XML Writer enum
//
//   Each xml writer property is identified by an ID and has an associated
//   value.
//
typedef enum {
...
//  value.
//
typedef enum {} WS_XML_BUFFER_PROPERTY_ID;
...
//  Async Model enum
//
//   Used to specify the threading behavior of a callback (for example, a
//   WS_ASYNC_CALLBACK).
//
typedef enum {
  WS_SHORT_CALLBACK = 0,
...
//  Channel enum
//
//   Specifies whether a message is required or not when receiving from a
//   channel.
//
typedef enum {
...
//  Channel enum
//
//   The channel type indicates the basic characteristics of the channel, such
//   as whether it is
...
typedef enum {
  ...
  WS_CHANNEL_TYPE_DUPLEX_SESSION =
      (WS_CHANNEL_TYPE_INPUT | WS_CHANNEL_TYPE_OUTPUT |
...
//  Channel enum
//
//   The transfer mode indicates whether messages that are sent or received are
...
//  Channel enum
//
//   Proxy setting indicates HTTP proxy setting for the channel with binding
//   WS_HTTP_CHANNEL_BINDING.
//  This is specified as part of WS_CHANNEL_PROPERTY_HTTP_PROXY_SETTING_MODE
//  channel property.
//
typedef enum {
...
//  Channel enum
//
//   A set of flags that control how mapped headers appear in an HTTP request or
//   response.
//
enum {
...
//  Context enum
//
//   Each property represents a property available on the Context. Not all
//   properties may be available
//  at a given point on a context. All context properties are available through
...
//  Errors enum
//
//   An enumeration of structured exception codes thrown by this component.
//   These
...
//  is not specified when the listener is created, then its default value is
//  used.
//
typedef enum {
...
//  Message enum
//
//   A set of flags representing the SOAP mustUnderstand and relay attributes of
//   a header.
//
enum {
...
//  Security Binding Settings enum
//
//   Defines which set of actions to use when negotiating security tokens using
//   WS-Trust.
//
typedef enum {
...
//  Security Channel Settings enum
//
//   Defines the WS-SecureCoversation specification version to be used with
//   message
//  security and mixed-mode security.
//
typedef enum {
...
//  Security Channel Settings enum
//
//   Defines secure protocol that can be used by
//   WS_SSL_TRANSPORT_SECURITY_BINDING binding.
//
typedef enum {
...
//
typedef enum {} WS_SECURITY_ALGORITHM_PROPERTY_ID;
...
//  Security Binding Settings enum
//
//   A suite of security algorithms used for tasks such as signing and
//   encryting.
//  The values in this enumeration correspond to the suites defined in
//   (http://schemas.xmlsoap.org/ws/2005/07/securitypolicy/)
//   WS-SecurityPolicy 1.1 section 7.1.
//
typedef enum {
...
//  Security Binding Settings enum
//
//   Defines failures related to certificate validation. Can be used with
//   WS_SECURITY_BINDING_PROPERTY_CERT_FAILURES_TO_IGNORE to
...
//
//   Defines if Extended Protection data should be validated. This property is
//   only available on the server,
//  and can only be set when WS_HTTP_CHANNEL_BINDING with
//  WS_SSL_TRANSPORT_SECURITY_BINDING and either
//  WS_KERBEROS_APREQ_MESSAGE_SECURITY_BINDING
...
//  Extended Protection enum
//
//   Defines how Extended Protection is validated. For most configurations, the
//   runtime can automatically determine what needs to
//  be validated based on the presence of the WS_SSL_TRANSPORT_SECURITY_BINDING.
//  However, if the SSL connection is terminated at an intermediary such as a
//  proxy prior to reaching the server then the validation method must change,
//  and this scenario cannot be automatically detected.
//
//   Only available on the server.
//
typedef enum {
...
//   that are used, in turn, in a (WS_SECURITY_DESCRIPTION) security
//   description.
//
...
//  Specified using the (WS_REQUEST_SECURITY_TOKEN_PROPERTY_ID)
...
//  Serialization enum
//
//   Specifies how a field of a structure is represented in XML.  This is used
//   within
...
//  Serialization enum
//
//   Specifies whether a value is required, and how the value should be
//   allocated.
//
typedef enum {
...
//  Serialization enum
//
//   Specifies whether a storage specified contains the value, or a pointer to
//   the value,
...
//  Service Host enum
//
//   Each property represents optional parameters for configuring the service
//   host.
//
typedef enum {
...
//  Service Proxy enum
//
//   Optional parameters for configuring a call on a client side service
//   operation.
//
typedef enum {
...
//  Metadata Import enum
//
//   The values in this enumeration are used to identify the sub-types of
...
typedef HRESULT(CALLBACK *WS_READ_CALLBACK)(
    ...
    _Out_writes_bytes_to_(maxSize, *actualSize) void *bytes, _In_ ULONG maxSize,
    _Out_ ULONG *actualSize, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
...
typedef HRESULT(CALLBACK *WS_WRITE_CALLBACK)(
    _In_opt_ void *callbackState, _In_reads_(count) const WS_BYTES *buffers,
    _In_ ULONG count, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);

//  XML Writer callback
//
//   A user-defined callback used by WsPushBytes to request that data be
//   written.
//
typedef HRESULT(CALLBACK *WS_PUSH_BYTES_CALLBACK)(
    _In_opt_ void *callbackState, _In_ WS_WRITE_CALLBACK writeCallback,
...
typedef HRESULT(CALLBACK *WS_DYNAMIC_STRING_CALLBACK)(
    _In_opt_ void *callbackState, _In_ const WS_XML_STRING *string,
    _Out_ BOOL *found, _Out_ ULONG *id, _In_opt_ WS_ERROR *error);

//  Async Model callback
//
//   The type of a callback used in the Async Model.
//
typedef void(CALLBACK *WS_ASYNC_CALLBACK)(_In_ HRESULT errorCode,
...
typedef HRESULT(CALLBACK *WS_ASYNC_FUNCTION)(
    _In_ HRESULT hr, _In_ WS_CALLBACK_MODEL callbackModel,
    _In_opt_ void *callbackState, _Inout_ WS_ASYNC_OPERATION *next,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_CREATE_CHANNEL_CALLBACK)(
    ...
    _In_ ULONG channelParametersSize, _Outptr_ void **channelInstance,
    _In_opt_ WS_ERROR *error);
...
//  for a WS_CUSTOM_CHANNEL_BINDING.
//
typedef HRESULT(CALLBACK *WS_RESET_CHANNEL_CALLBACK)(_In_ void *channelInstance,
                                                     _In_opt_ WS_ERROR *error);
...
//  for a WS_CUSTOM_CHANNEL_BINDING.
//
typedef HRESULT(CALLBACK *WS_ABORT_CHANNEL_CALLBACK)(_In_ void *channelInstance,
                                                     _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_OPEN_CHANNEL_CALLBACK)(
    _In_ void *channelInstance, _In_ const WS_ENDPOINT_ADDRESS *endpointAddress,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_CLOSE_CHANNEL_CALLBACK)(
    _In_ void *channelInstance, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_SET_CHANNEL_PROPERTY_CALLBACK)(
    _In_ void *channelInstance, _In_ WS_CHANNEL_PROPERTY_ID id,
    _In_reads_bytes_(valueSize) const void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_GET_CHANNEL_PROPERTY_CALLBACK)(
    _In_ void *channelInstance, _In_ WS_CHANNEL_PROPERTY_ID id,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_READ_MESSAGE_START_CALLBACK)(
    _In_ void *channelInstance, _In_ WS_MESSAGE *message,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_CREATE_ENCODER_CALLBACK)(
    _In_ void *createContext, _In_ WS_WRITE_CALLBACK writeCallback,
    _In_ void *writeContext, _Outptr_ void **encoderContext,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_ENCODER_GET_CONTENT_TYPE_CALLBACK)(
    _In_ void *encoderContext, _In_ const WS_STRING *contentType,
    _Out_ WS_STRING *newContentType, _Out_ WS_STRING *contentEncoding,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_ENCODER_START_CALLBACK)(
    _In_ void *encoderContext, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_ENCODER_ENCODE_CALLBACK)(
    _In_ void *encoderContext, _In_reads_(count) const WS_BYTES *buffers,
    _In_ ULONG count, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_CREATE_DECODER_CALLBACK)(
    _In_ void *createContext, _In_ WS_READ_CALLBACK readCallback,
    _In_ void *readContext, _Outptr_ void **decoderContext,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_DECODER_GET_CONTENT_TYPE_CALLBACK)(
    _In_ void *decoderContext, _In_ const WS_STRING *contentType,
    _In_opt_ const WS_STRING *contentEncoding, _Out_ WS_STRING *newContentType,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_DECODER_DECODE_CALLBACK)(
    ...
    _In_ ULONG maxLength, _Out_ ULONG *length,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_HTTP_REDIRECT_CALLBACK)(
    _In_opt_ void *state, _In_ const WS_STRING *originalUrl,
...
typedef HRESULT(CALLBACK *WS_CREATE_LISTENER_CALLBACK)(
    ...
    _In_ ULONG listenerParametersSize, _Outptr_ void **listenerInstance,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_RESET_LISTENER_CALLBACK)(
    _In_ void *listenerInstance, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_OPEN_LISTENER_CALLBACK)(
    _In_ void *listenerInstance, _In_ const WS_STRING *url,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_CLOSE_LISTENER_CALLBACK)(
    _In_ void *listenerInstance, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_GET_LISTENER_PROPERTY_CALLBACK)(
    _In_ void *listenerInstance, _In_ WS_LISTENER_PROPERTY_ID id,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_ACCEPT_CHANNEL_CALLBACK)(
    _In_ void *listenerInstance, _In_ void *channelInstance,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
//
//   This callback is invoked when a connection to an HTTP server was
//   established and
//  headers were sent.
//
typedef HRESULT(CALLBACK *WS_CERTIFICATE_VALIDATION_CALLBACK)(
    _In_ PCCERT_CONTEXT certContext, _In_opt_ void *state);
...
//  certificate credential. The runtime will invoke this callback when the
//  channel (client-side) or the listener (server-side) is opened.
//
...
typedef HRESULT(CALLBACK *WS_GET_CERT_CALLBACK)(
    ...
    _Outptr_ const struct _CERT_CONTEXT **cert, _In_opt_ WS_ERROR *error);
...
//  containing this callback is included in the security description, this
//  callback is invoked for each received message at the server.  This callback
//  is expected to return S_OKif the username/password pair was successfully
//  validated, S_FALSE when the pair could not be validated and an error value
//  if an unexpected error occurred. Returning any result other than S_OK from
//  this callback will result in the associated receive message failing with a
//  security error.
//
...
typedef HRESULT(CALLBACK *WS_VALIDATE_PASSWORD_CALLBACK)(
    ...
    _In_ const WS_STRING *username, _In_ const WS_STRING *password,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_VALIDATE_SAML_CALLBACK)(
    ...
    _In_ WS_XML_BUFFER *samlAssertion, _In_opt_ WS_ERROR *error);
...
typedef HRESULT(CALLBACK *WS_DURATION_COMPARISON_CALLBACK)(
    _In_ const WS_DURATION *duration1, _In_ const WS_DURATION *duration2,
    _Out_ int *result, _In_opt_ WS_ERROR *error);

//  Serialization callback
//
//   This callback is invoked to read an value when WS_CUSTOM_TYPE
//   has been specified.  This allows reading of XML constructs which do not
//   easily
//  map to the core serialization model.
//
typedef HRESULT(CALLBACK *WS_READ_TYPE_CALLBACK)(
    _In_ WS_XML_READER *reader, _In_ WS_TYPE_MAPPING typeMapping,
    _In_ const void *descriptionData, _In_opt_ WS_HEAP *heap,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
//   has been specified.  This allows writing of XML constructs which do not
//   easily
//  map to the core serialization model.
//
typedef HRESULT(CALLBACK *WS_WRITE_TYPE_CALLBACK)(
    _In_ WS_XML_WRITER *writer, _In_ WS_TYPE_MAPPING typeMapping,
...
typedef HRESULT(CALLBACK *WS_IS_DEFAULT_VALUE_CALLBACK)(
    ...
    _In_ ULONG valueSize, _Out_ BOOL *isDefault, _In_opt_ WS_ERROR *error);
...
//  async service operation call as a result of an aborted shutdown of service
//  host.
//
typedef void(CALLBACK *WS_OPERATION_CANCEL_CALLBACK)(
    _In_ const WS_SERVICE_CANCEL_REASON reason, _In_opt_ void *state);
...
//  operation call. This callback is generated by (Web Service Compiler Tool)
//  wsutil.exe
//   for every service operation. It is defined on the WS_OPERATION_DESCRIPTION
//   for each
//  service operation.
//
typedef HRESULT(CALLBACK *WS_SERVICE_STUB_CALLBACK)(
    _In_ const WS_OPERATION_CONTEXT *context, _In_ void *frame,
    _In_ const void *callback, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
//
//   For (Contract) session based service contract, this notification signifies
//   session initiation.
//  Thus an application state scoped for the session can be created within this
//  callback.
//
typedef HRESULT(CALLBACK *WS_SERVICE_ACCEPT_CHANNEL_CALLBACK)(
    _In_ const WS_OPERATION_CONTEXT *context, _Outptr_ void **channelState,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);

//  Service Host callback
//
//   This callback is invoked when a channel is closed/or aborted on an
//   endpoint.
...
//  signifies session tear down. Thus an application state scoped for the
//  session can be destroyed within this callback.
//
...
typedef HRESULT(CALLBACK *WS_SERVICE_SECURITY_CALLBACK)(
    _In_ const WS_OPERATION_CONTEXT *context, _Out_ BOOL *authorized,
    _In_opt_ WS_ERROR *error);
...
//  about to be sent through, or when an output message headers are just
//  received.
//
typedef HRESULT(CALLBACK *WS_PROXY_MESSAGE_CALLBACK)(_In_ WS_MESSAGE *message,
                                                     _In_ WS_HEAP *heap,
                                                     _In_ void *state,
                                                     _In_opt_ WS_ERROR *error);
...
//   information associated with it.  The xml APIs use WS_XML_STRINGs to
//   identify prefixes,
...
//  position will cause subsequent use of the position to fail.  The position
//  itself remains valid, but operations that depend on that position will fail.
//
//   Positions may be used as long as the containing XML buffer is valid.  Using
//   a position
//  after its corresponding buffer has been deleted will exhibit undefined
//  behavior.
//
...
//  XML Node structure
//
//   Represents base64 encoded data. (e.g. The three bytes { 0, 0, 0 } represent
//   the text "AAAA".)
//
...
//  XML Node structure
//
//   Represents a signed 32 bit integer.  (e.g. The value 255 represents the
//   text "255")
//
...
//  XML Node structure
//
//   Represents a signed 64 bit integer.  (e.g. The value 255 represents the
//   text "255")
//
...
//  XML Node structure
//
//   Represents an unsigned 64 bit integer.  (e.g. The value 255 represents the
//   text "255")
//
...
//  XML Node structure
//
//   Represents a 4 byte floating point value.  (e.g. The value 0.0 represents
//   the text "0")
//
...
//  XML Node structure
//
//   Represents an 8 byte floating point value.  (e.g. The value 0.0 represents
//   the text "0")
//
...
//  XML Node structure
//
//   Represents a 12 byte fixed point value.  (e.g. The value 1.23 represents
//   the text "1.23")
//
...
//  XML Node structure
//
//   Represents a guid formatted as the text
...
//
//  .mm is a two digit number representing the minute of the hour, from to 0
//  to 59.
//
//  .ss is a two digit number representing the second of the minute, from to 0
//  to 59.
//
...
//  XML Node structure
//
//   Represents a list of text values separated by a single whitespace
//   character.
//
...
//  XML Node structure
//
//   Represents an attribute (e.g. <a:purchaseOrder xmlns:a="http://tempuri.org"
//   id="5">)
//
...
//  XML Reader structure
//
//   Specifies where the reader should obtain the bytes that comprise the xml
//   document.
//
...
//  XML Reader structure
//
//   This structure is the base type for all the different kinds of reader
//   encodings.
//
...
//  XML Reader structure
//
//   Used to indicate that the reader should interpret the bytes it reads as
//   textual xml.
//
...
//  XML Reader structure
//
//   Used to indicate that the reader should interpret the bytes it reads as in
//   MTOM format.
//
...
//  XML Reader structure
//
//   Used to indicate that the reader should surface the bytes of the document
//   as base64 encoded characters.
//
...
//  XML Writer structure
//
//   This structure is the base type for all the different kinds of writer
//   encodings.
//
...
//  XML Writer structure
//
//   Used to indicate that the writer should emit bytes from decoded base64
//   characters.
//
...
//  XML Writer structure
//
//   Specifies where the writer should emit the bytes that comprise the xml
//   document.
//
...
//  Channel structure
//
//   Specifies an individual header that is mapped as part of
...
struct _WS_HTTP_MESSAGE_MAPPING {
  ...
  _Field_size_(requestHeaderMappingCount)
      WS_HTTP_HEADER_MAPPING **requestHeaderMappings;
  ULONG requestHeaderMappingCount;
  _Field_size_(responseHeaderMappingCount)
...
//  Channel structure
//
//   Specifies the callback function and state for controlling the HTTP auto
...
//
//
//   Type for specifying an endpoint identity represented by a UPN (user
//   principal name).
//
//
...
//  Endpoint Identity structure
//
//   Type for specifying an endpoint identity represented by an SPN (service
//   principal name).
//
...
//
//   A list of Server Principal Names (SPNs) that are used to validate Extended
//   Protection.
//
//   Only available on the server.
//
...
//  Security Channel Settings structure
//
//   Specifies the callback function and state for validating the HTTP
//   certificate.
//
//   See also
...
//  Security Credentials structure
//
//   Type for supplying a Windows credential as username, password, domain
//   strings.
//
...
//  Security Credentials structure
//
//   Type for supplying a Windows Integrated Authentication credential based on
//   the current Windows identity.
//  If this credential subtype is used for a security binding, the current
//  thread token on the thread that calls
//   WsOpenChannel or WsOpenServiceProxy is used as the Windows
//  identity when sending messages or making service calls. WsAcceptChannel and
//  WsOpenServiceHost do not support this credential type when called from an
...
//  Security Credentials structure
//
//  Type for supplying a Windows Integrated Authentication credential as an
//  opaque handle created by SspiPromptForCredentials and the related family of
//  APIs.  This feature is available only on Windows 7 and later.
//
...
//  They are best suitable for application-level username/password pairs, such
//  as those used for online customer accounts.  The usernames and passwords
//  specified are not interpreted by the security runtime, and are merely
//  carried client-to-server for authentication by the specified server-side
...
//  Windows Vista and later, please use
//  WS_NCRYPT_ASYMMETRIC_SECURITY_KEY_HANDLE.
//
...
//   WS_HTTP_CHANNEL_BINDING.
//
//   With this security binding, the following security binding property may be
//   specified:
//
//  . WS_SECURITY_BINDING_PROPERTY_CERT_FAILURES_TO_IGNORE (client side only)
//
//  . WS_SECURITY_BINDING_PROPERTY_DISABLE_CERT_REVOCATION_CHECK (client side
//  only)
//
...
//  allowed, the security binding property
...
//   protocol and the (http://msdn.microsoft.com/en-us/library/cc236723.aspx)
//   .Net Message Framing specification.
//
...
//  that string identity value is used directly with the SSP.  If the identity
//  is a
...
//
//   With this security binding, the following security binding properties may
//   be specified:
//
//  . WS_SECURITY_BINDING_PROPERTY_WINDOWS_INTEGRATED_AUTH_PACKAGE
...
//
//  . WS_SECURITY_BINDING_PROPERTY_ALLOWED_IMPERSONATION_LEVEL (client side
//  only)
//
...
//  the security binding property
//  WS_SECURITY_BINDING_PROPERTY_WINDOWS_INTEGRATED_AUTH_PACKAGE; if that
//  property is not specified, SPNEGO is used by default.
//
//   This security binding operates at the transport security level and is
...
//
//   The (WS_NAMEDPIPE_CHANNEL_BINDING) named pipe binding supports only this
//   one transport security binding and does not support any message security
//   bindings.
//
//   With this security binding, the following security binding properties may
//   be specified:
...
//  Security Bindings structure
//
//   The security binding subtype for specifying the use of HTTP header
//   authentication against a target service or a HTTP proxy server
//  based on the basic, digest ( (http://tools.ietf.org/html/rfc2617) RFC 2617)
//  and the SPNEGO ( (http://tools.ietf.org/html/rfc4559) RFC4559) protocols.
//  Since this security binding operates at the HTTP header level, it is
//  supported only with the WS_HTTP_CHANNEL_BINDING. By default, this security
//  binding is used for the target service. However
//  WS_SECURITY_BINDING_PROPERTY_HTTP_HEADER_AUTH_TARGET
//   security binding property can be specified to use it for a HTTP proxy
//   server. This binding provides client authentication, but not message
//   protection
//  since the HTTP body is unaffected by this binding. While this security
//  binding can be used alone, such usage is not recommended; more typically,
//  HTTP header authentication is done in conjunction with transport level
//  security provided by a security binding such as the
//   WS_SSL_TRANSPORT_SECURITY_BINDING. To use this binding without SSL, the the
//   security description property
//   WS_SECURITY_PROPERTY_TRANSPORT_PROTECTION_LEVEL must be explicitly set to
...
//
//  . WS_SECURITY_BINDING_PROPERTY_HTTP_HEADER_AUTH_BASIC_REALM (server side
//  only)
//
//  . WS_SECURITY_BINDING_PROPERTY_HTTP_HEADER_AUTH_DIGEST_REALM (server side
//  only)
//
//  . WS_SECURITY_BINDING_PROPERTY_HTTP_HEADER_AUTH_DIGEST_DOMAIN (server side
//  only)
//
...
//
//   Only one instance of this binding may be present in a
//   (WS_SECURITY_DESCRIPTION) security description.
//  This security binding is not supported with the
//  (WS_NAMEDPIPE_CHANNEL_BINDING) named pipe binding.
//
//   With this security binding, the following security binding properties may
//   be specified:
//
...
//  . WS_SECURITY_BINDING_PROPERTY_ALLOW_ANONYMOUS_CLIENTS (server side only)
//
//   Client side on Vista and above, using this binding with HTTP will result in
//   the message being sent using chunked transfer.
//
...
//
//   Only one instance of this binding may be present in a
//   (WS_SECURITY_DESCRIPTION) security description and
//  this security binding is not supported with the
//  (WS_NAMEDPIPE_CHANNEL_BINDING) named pipe binding.
//
//   With this security binding, no security binding properties may be
//   specified.
//
...
//  (WS_NAMEDPIPE_CHANNEL_BINDING) named pipe binding.
//
//   When this binding is used, the channel must complete the receive of at
//   least one
...
struct _WS_CERT_SIGNED_SAML_AUTHENTICATOR {
  ...
  _Field_size_(trustedIssuerCertCount) const
      struct _CERT_CONTEXT **trustedIssuerCerts;
...
//
//   The index of the parameters in the incoming/outgoing messages field
//   descriptions.
//
...
//  Service Host structure
//
//   Specifies the callback which is called when a channel is successfully
//   accepted.
//
...
//  Service Host structure
//
//   Specifies the callback which is called when a channel is about to be
//   closed.
...
//
//   Note, the port type will only be generated into the WSDL document if the
//   service
...
//  Service Host structure
//
//   Represents an individual endpoint on a service host. The properties on the
//   endpoint
...
//  to associate or inspect headers in an input or an output message
//  respectively.
//
...
//  Url structure
//
//   The abstract base type for all URL schemes used with WsDecodeUrl and
//   WsEncodeUrl APIs.
//
struct _WS_URL {
...
struct _WS_SECURITY_BINDING_CONSTRAINT {
  ...
  _Field_size_opt_(propertyConstraintCount)
...
//  are assertions that are directly handled by applications such as custom
//  assertions.
//
...
struct _WS_POLICY_CONSTRAINTS {
  WS_CHANNEL_BINDING channelBinding;
  _Field_size_opt_(channelPropertyConstraintCount)
...
//  Policy Support structure
//
//   Describes the policy specifying http channel binding with SSL transport
//   security and
...
//
//   This type description is used with template APIs to describe
//  the security context related templates generated accordingly to input policy
//  setting.
//
...
//  Policy Support structure
//
//   Describes the policy specifying security context message binding using TCP
//   channel binding
//  with windows SSPI transport security. The bootstrap channel uses TCP channel
//  binding with windows SSPI transport security and kerberos message security.
//
...
//
//   Describes the policy specifying security context message binding using TCP
//   channel binding with windows SSPI transport
//  security. The bootstrap channel uses TCP channel binding with windows SSPI
//  transport security and username/password message security.
//
...
//  Policy Support structure
//
//   Describes the policy specifying security context message binding over http
//   channel binding, with SSL
//  transport security. The bootstrap channel uses http channel binding with SSL
//  transport security and username/password message security.
//
...
//   channel binding, with SSL
//  transport security. The bootstrap channel uses http channel binding with SSL
//  transport security and KERBEROS AP_REQ message security.
//
...
//  Policy Support structure
//
//   The security binding template for specifying the use of HTP header
//   authentication
...
//  Policy Support structure
//
//   HTTP header authentication security template information to be filled in by
//   application.
...
//  Policy Support structure
//
//   Username/password security template information to be filled in by
//   application.
...
//  be used only with message security. So, it is used in conjunction with
//  another transport security or message security binding that provides message
//  protection.
//
...
//  supplied security context security binding.  This security binding may
//  be used only with message security. So, it is used in conjunction with
//  another transport security binding that provides message protection. The
//  security properties are used to establish the secure conversation.
//
//   See also WS_SECURITY_CONTEXT_MESSAGE_SECURITY_BINDING
...
//  XML Node macro
//
//   Provides an initializer for a WS_XML_STRING structure when there is an
//   associated dictionary ID.
//
#define WS_XML_STRING_DICTIONARY_VALUE(S, D, I)                                \
  {sizeof(S) - 1, (BYTE *)RTL_CONST_CAST(char *)(S),                           \
   RTL_CONST_CAST(WS_XML_DICTIONARY *)(D), I}

//  XML Node macro
//
//   Provides an initializer for a WS_XML_STRING structure when there is no
//   associated dictionary ID.
//
#define WS_XML_STRING_VALUE(S)                                                 \
  {sizeof(S) - 1, (BYTE *)RTL_CONST_CAST(char *)(S), NULL, 0}
...
//  Utilities macro
//
//   Returns the offset, in bytes, of a field within a contained structure,
//   given the name
...
HRESULT WINAPI WsStartReaderCanonicalization(
    _In_ WS_XML_READER *reader, _In_ WS_WRITE_CALLBACK writeCallback,
    _In_opt_ void *writeCallbackState,
    _In_reads_opt_(propertyCount)
        const WS_XML_CANONICALIZATION_PROPERTY *properties,
    _In_ ULONG propertyCount, _In_opt_ WS_ERROR *error);

//  XML Canonicalization function
//
//   Stops XML canonicalization started by the preceding
//   WsStartReaderCanonicalization call.
//
HRESULT WINAPI WsEndReaderCanonicalization(_In_ WS_XML_READER *reader,
                                           _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsStartWriterCanonicalization(
    _In_ WS_XML_WRITER *writer, _In_ WS_WRITE_CALLBACK writeCallback,
    _In_opt_ void *writeCallbackState,
    _In_reads_opt_(propertyCount)
        const WS_XML_CANONICALIZATION_PROPERTY *properties,
    _In_ ULONG propertyCount, _In_opt_ WS_ERROR *error);
...
//
//   Stops XML canonicalization started by the preceding
//   WsStartWriterCanonicalization call.
//
HRESULT WINAPI WsEndWriterCanonicalization(_In_ WS_XML_WRITER *writer,
                                           _In_opt_ WS_ERROR *error);

//  XML Buffer function
//
//   Creates an XML Buffer which can be used to process xml data in-memory.  It
//   can be
//  navigated through, written to, and read from.
//
HRESULT WINAPI WsCreateXmlBuffer(_In_ WS_HEAP *heap,
                                 _In_reads_opt_(propertyCount)
...
//  XML Buffer function
//
//   Removes the node at the specified position from the xml buffer.  If
//   positioned
//  on an element it will remove the element including all of its children and
//  its corresponding end element, otherwise it will remove a single node.
//
//   The use of any API with a WS_XML_READER or WS_XML_WRITER that
//  currently depends on this position or a child of this position will fail.
//  The
...
//
//   Calling WsSetReaderPosition or WsSetWriterPosition after calling
//   WsRemoveNode will fail.
//
HRESULT WINAPI WsRemoveNode(_In_ const WS_XML_NODE_POSITION *nodePosition,
                            _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Creates an instance of a WS_XML_READER.  Use WsSetInput or
//   WsSetInputToBuffer to choose the encoding of the reader and to indicate the
//   source of the input.
//
HRESULT WINAPI WsCreateReader(_In_reads_opt_(propertyCount)
...
//
HRESULT WINAPI WsSetInput(_In_ WS_XML_READER *reader,
...
//
HRESULT WINAPI WsSetInputToBuffer(_In_ WS_XML_READER *reader,
...
HRESULT WINAPI WsGetReaderProperty(_In_ WS_XML_READER *reader,
...
//  XML Reader function
//
//   Returns the (WS_XML_NODE) node that the (WS_XML_READER) reader is currently
//   positioned on.
//
HRESULT WINAPI WsGetReaderNode(_In_ WS_XML_READER *xmlReader,
                               _Outptr_ const WS_XML_NODE **node,
                               _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Ensures that the reader has at least a specified amount of data available
//   to it for reading.
//
HRESULT WINAPI WsFillReader(_In_ WS_XML_READER *reader, _In_ ULONG minSize,
                            _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                            _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Advances the reader past a start element skipping any whitespace in front
//   of it.
//
HRESULT WINAPI WsReadStartElement(_In_ WS_XML_READER *reader,
                                  _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Advances the reader to the next start element skipping whitespace and
//   comments if necessary.  Optionally,
//  it may also verify the localName and namespace of the element.
//
HRESULT WINAPI WsReadToStartElement(_In_ WS_XML_READER *reader,
...
//  XML Reader function
//
//   Moves the reader to the specified attribute so that its content may be read
//   using ReadValue, ReadChars, or ReadBytes.
//
HRESULT WINAPI WsReadStartAttribute(_In_ WS_XML_READER *reader,
                                    _In_ ULONG attributeIndex,
                                    _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Moves the reader back to the element node containing the attribute that was
//   read.
//
HRESULT WINAPI WsReadEndAttribute(_In_ WS_XML_READER *reader,
                                  _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsReadNode(_In_ WS_XML_READER *reader, _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Advances the reader in the input stream.  If the current node is an
//   element,
//  all of the children of that element are skipped, and the reader is
//  positioned on the node following its end element.  Otherwise, the reader is
//  positioned on the next node in the same manner as WsReadNode.
//
HRESULT WINAPI WsSkipNode(_In_ WS_XML_READER *reader, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsReadEndElement(_In_ WS_XML_READER *reader,
                                _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsFindAttribute(_In_ WS_XML_READER *reader,
                               _In_ const WS_XML_STRING *localName,
                               _In_ const WS_XML_STRING *ns, _In_ BOOL required,
                               _Out_ ULONG *attributeIndex,
                               _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Reads text from the reader and parses it according to the specified value
//   type.
//
HRESULT WINAPI WsReadValue(_In_ WS_XML_READER *reader,
                           _In_ WS_VALUE_TYPE valueType,
                           _Out_writes_bytes_(valueSize) void *value,
                           _In_ ULONG valueSize, _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Reads a specified number of text characters from the reader.
//
HRESULT WINAPI WsReadChars(_In_ WS_XML_READER *reader,
                           _Out_writes_to_(maxCharCount, *actualCharCount)
                               WCHAR *chars,
...
//  XML Reader function
//
//   Reads a specified number of text characters from the reader and returns
//   them encoded in UTF-8.
//
HRESULT WINAPI WsReadCharsUtf8(_In_ WS_XML_READER *reader,
                               _Out_writes_to_(maxByteCount, *actualByteCount)
                                   BYTE *bytes,
...
//  XML Reader function
//
//   Reads text from the reader and decodes the characters as bytes according to
//   the base64 specification.
//
HRESULT WINAPI WsReadBytes(_In_ WS_XML_READER *reader,
                           _Out_writes_bytes_to_(maxByteCount,
                                                 *actualByteCount) void *bytes,
                           _In_ ULONG maxByteCount,
                           _Out_ ULONG *actualByteCount,
                           _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsReadArray(_In_ WS_XML_READER *reader,
                           _In_ const WS_XML_STRING *localName,
                           _In_ const WS_XML_STRING *ns,
                           _In_ WS_VALUE_TYPE valueType,
                           _Out_writes_bytes_opt_(arraySize) void *array,
                           _In_ ULONG arraySize, _In_ ULONG itemOffset,
                           _In_ ULONG itemCount, _Out_ ULONG *actualItemCount,
                           _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Returns the current position of the reader.  This can only be used on a
//   reader
//  that is set to an XmlBuffer.
//
HRESULT WINAPI WsGetReaderPosition(_In_ WS_XML_READER *reader,
                                   _Out_ WS_XML_NODE_POSITION *nodePosition,
                                   _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Sets the current position of the reader.  The position must have been
//   obtained by a call to WsGetReaderPosition or WsGetWriterPosition.
//
HRESULT WINAPI WsSetReaderPosition(
    _In_ WS_XML_READER *reader, _In_ const WS_XML_NODE_POSITION *nodePosition,
    _In_opt_ WS_ERROR *error);

//  XML Reader function
//
//   Moves the current position of the reader as specified by the moveTo
//   parameter.
//
HRESULT WINAPI WsMoveReader(_In_ WS_XML_READER *reader, _In_ WS_MOVE_TO moveTo,
                            _Out_opt_ BOOL *found, _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Creates an instance of a WS_XML_WRITER.  Use WsSetOutput or
//   WsSetOutputToBuffer to choose the encoding of the writer and to indicate
//   where to direct the output.
//
HRESULT WINAPI WsCreateWriter(_In_reads_opt_(propertyCount)
...
//  XML Writer function
//
//   Sets the encoding and output callbacks for the writer.  The callbacks are
//   used to
//  provides buffers to the writer and to perform asynchronous i/o.
//
HRESULT WINAPI WsSetOutput(_In_ WS_XML_WRITER *writer,
...
//
HRESULT WINAPI WsSetOutputToBuffer(_In_ WS_XML_WRITER *writer,
                                   _In_ WS_XML_BUFFER *buffer,
                                   _In_reads_opt_(propertyCount)
                                       const WS_XML_WRITER_PROPERTY *properties,
                                   _In_ ULONG propertyCount,
...
HRESULT WINAPI WsGetWriterProperty(_In_ WS_XML_WRITER *writer,
...
//   specified in WS_XML_WRITER_STREAM_OUTPUT if sufficient data has been
//   buffered.
//
HRESULT WINAPI WsFlushWriter(_In_ WS_XML_WRITER *writer, _In_ ULONG minSize,
                             _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                             _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteStartElement(_In_ WS_XML_WRITER *writer,
...
//  XML Writer function
//
//   Forces the writer to commit the current element and prevent further
//   attributes
//  from being written to the element.
//
HRESULT WINAPI WsWriteEndStartElement(_In_ WS_XML_WRITER *writer,
                                      _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteXmlnsAttribute(_In_ WS_XML_WRITER *writer,
                                     _In_opt_ const WS_XML_STRING *prefix,
                                     _In_ const WS_XML_STRING *ns,
...
//
HRESULT WINAPI WsWriteStartAttribute(_In_ WS_XML_WRITER *writer,
                                     _In_opt_ const WS_XML_STRING *prefix,
                                     _In_ const WS_XML_STRING *localName,
                                     _In_ const WS_XML_STRING *ns,
                                     _In_ BOOL singleQuote,
...
//
HRESULT WINAPI WsWriteEndAttribute(_In_ WS_XML_WRITER *writer,
                                   _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteValue(_In_ WS_XML_WRITER *writer,
                            _In_ WS_VALUE_TYPE valueType,
                            _In_reads_bytes_(valueSize) const void *value,
                            _In_ ULONG valueSize, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteXmlBuffer(_In_ WS_XML_WRITER *writer,
...
//
HRESULT WINAPI WsReadXmlBuffer(_In_ WS_XML_READER *reader, _In_ WS_HEAP *heap,
...
HRESULT WINAPI WsWriteXmlBufferToBytes(
    _In_ WS_XML_WRITER *writer, _In_ WS_XML_BUFFER *xmlBuffer,
    _In_opt_ const WS_XML_WRITER_ENCODING *encoding,
    _In_reads_opt_(propertyCount) const WS_XML_WRITER_PROPERTY *properties,
    _In_ ULONG propertyCount, _In_ WS_HEAP *heap,
    _Outptr_result_bytebuffer_(*byteCount) void **bytes, _Out_ ULONG *byteCount,
    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsReadXmlBufferFromBytes(
    _In_ WS_XML_READER *reader, _In_opt_ const WS_XML_READER_ENCODING *encoding,
    _In_reads_opt_(propertyCount) const WS_XML_READER_PROPERTY *properties,
    _In_ ULONG propertyCount, _In_reads_bytes_(byteCount) const void *bytes,
    _In_ ULONG byteCount, _In_ WS_HEAP *heap,
    _Outptr_ WS_XML_BUFFER **xmlBuffer, _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Writes a series of elements to the writer.
//
HRESULT WINAPI WsWriteArray(_In_ WS_XML_WRITER *writer,
                            _In_ const WS_XML_STRING *localName,
                            _In_ const WS_XML_STRING *ns,
                            _In_ WS_VALUE_TYPE valueType,
                            _In_reads_bytes_opt_(arraySize) const void *array,
                            _In_ ULONG arraySize, _In_ ULONG itemOffset,
                            _In_ ULONG itemCount, _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Writes an xml qualified name to the writer.
//
HRESULT WINAPI WsWriteQualifiedName(_In_ WS_XML_WRITER *writer,
                                    _In_opt_ const WS_XML_STRING *prefix,
                                    _In_ const WS_XML_STRING *localName,
                                    _In_opt_ const WS_XML_STRING *ns,
                                    _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteChars(_In_ WS_XML_WRITER *writer,
                            _In_reads_(charCount) const WCHAR *chars,
                            _In_ ULONG charCount, _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Writes a series of characters encoded as UTF-8 to an element or attribute.
//
HRESULT WINAPI WsWriteCharsUtf8(_In_ WS_XML_WRITER *writer,
                                _In_reads_(byteCount) const BYTE *bytes,
                                _In_ ULONG byteCount, _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Writes bytes to the writer in a format optimized for the encoding.  When
//   writing
//  in a text encoding, it will emit the bytes encoded in base64.  When writing
//  to a binary format, it will emit the bytes directly.
//
HRESULT WINAPI WsWriteBytes(_In_ WS_XML_WRITER *writer,
                            _In_reads_bytes_(byteCount) const void *bytes,
                            _In_ ULONG byteCount, _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Sets up a callback to be invoked to write bytes within an element.  In some
//   encodings this can
//  be more efficient by eliminating a copy of the data.
//
HRESULT WINAPI WsPushBytes(_In_ WS_XML_WRITER *writer,
...
//  XML Writer function
//
//   Sets up a callback to be invoked to obtain the bytes to be written within
//   an element.
//  In some encodings this can be more efficient by eliminating a copy of the
//  data.
//
HRESULT WINAPI WsPullBytes(_In_ WS_XML_WRITER *writer,
...
//
HRESULT WINAPI WsWriteEndElement(_In_ WS_XML_WRITER *writer,
                                 _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteText(_In_ WS_XML_WRITER *writer,
...
//
HRESULT WINAPI WsWriteStartCData(_In_ WS_XML_WRITER *writer,
                                 _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteEndCData(_In_ WS_XML_WRITER *writer,
                               _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteNode(_In_ WS_XML_WRITER *writer,
...
//
HRESULT WINAPI WsGetPrefixFromNamespace(_In_ WS_XML_WRITER *writer,
                                        _In_ const WS_XML_STRING *ns,
                                        _In_ BOOL required,
...
//
HRESULT WINAPI WsGetWriterPosition(_In_ WS_XML_WRITER *writer,
                                   _Out_ WS_XML_NODE_POSITION *nodePosition,
                                   _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Sets the current position of the writer.  The position must have been
//   obtained by a
//  call to WsGetReaderPosition or WsGetWriterPosition.
//
HRESULT WINAPI WsSetWriterPosition(
    _In_ WS_XML_WRITER *writer, _In_ const WS_XML_NODE_POSITION *nodePosition,
    _In_opt_ WS_ERROR *error);

//  XML Writer function
//
//   Moves the current position of the writer as specified by the moveTo
//   parameter.
//
HRESULT WINAPI WsMoveWriter(_In_ WS_XML_WRITER *writer, _In_ WS_MOVE_TO moveTo,
                            _Out_opt_ BOOL *found, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsTrimXmlWhitespace(_In_reads_(charCount) WCHAR *chars,
                                   _In_ ULONG charCount,
                                   _Outptr_result_buffer_(*trimmedCount)
                                       WCHAR **trimmedChars,
...
//
HRESULT WINAPI WsVerifyXmlNCName(_In_reads_(ncNameCharCount)
...
//  XML Node function
//
//   Compares two WS_XML_STRING's for equality by performing an ordinal
//   comparison
//  of the character values.
//
HRESULT WINAPI WsXmlStringEquals(_In_ const WS_XML_STRING *string1,
...
//
HRESULT WINAPI WsGetNamespaceFromPrefix(_In_ WS_XML_READER *reader,
...
//
HRESULT WINAPI WsReadQualifiedName(_In_ WS_XML_READER *reader,
                                   _In_ WS_HEAP *heap,
...
//  XML Reader function
//
//   Finds the nearest xml attribute in scope with the specified localName and
//   returns its value.
//  The returned value is placed on the specified heap.
//
HRESULT WINAPI WsGetXmlAttribute(_In_ WS_XML_READER *reader,
                                 _In_ const WS_XML_STRING *localName,
                                 _In_ WS_HEAP *heap,
                                 _Outptr_opt_result_buffer_(*valueCharCount)
                                     WCHAR **valueChars,
...
//
HRESULT WINAPI WsCopyNode(_In_ WS_XML_WRITER *writer,
                          _In_ WS_XML_READER *reader, _In_opt_ WS_ERROR *error);

//  Async Model function
//
//   WsAsyncExecute is a helper that can be used to implement an asynchronous
//   operation.
//
HRESULT WINAPI WsAsyncExecute(_In_ WS_ASYNC_STATE *asyncState,
...
HRESULT WINAPI WsCreateChannel(
    _In_ WS_CHANNEL_TYPE channelType, _In_ WS_CHANNEL_BINDING channelBinding,
    ...
    _Outptr_ WS_CHANNEL **channel, _In_opt_ WS_ERROR *error);

//  Channel function
//
//   Open a channel to an endpoint.
//
HRESULT WINAPI WsOpenChannel(_In_ WS_CHANNEL *channel,
...
HRESULT WINAPI WsSendMessage(
    _In_ WS_CHANNEL *channel, _In_ WS_MESSAGE *message,
    ...
    _In_ ULONG bodyValueSize, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsReceiveMessage(
    _In_ WS_CHANNEL *channel, _In_ WS_MESSAGE *message,
    _In_reads_(messageDescriptionCount)
        const WS_MESSAGE_DESCRIPTION **messageDescriptions,
    _In_ ULONG messageDescriptionCount, _In_ WS_RECEIVE_OPTION receiveOption,
    _In_ WS_READ_OPTION readBodyOption, _In_opt_ WS_HEAP *heap,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _Out_opt_ ULONG *index, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsRequestReply(
    _In_ WS_CHANNEL *channel, _In_ WS_MESSAGE *requestMessage,
    ...
    _In_ ULONG requestBodyValueSize, _In_ WS_MESSAGE *replyMessage,
    _In_ const WS_MESSAGE_DESCRIPTION *replyMessageDescription,
    _In_ WS_READ_OPTION readOption, _In_opt_ WS_HEAP *heap,
    _Out_writes_bytes_opt_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsSendReplyMessage(
    _In_ WS_CHANNEL *channel, _In_ WS_MESSAGE *replyMessage,
    _In_ const WS_MESSAGE_DESCRIPTION *replyMessageDescription,
    _In_ WS_WRITE_OPTION writeOption,
    _In_reads_bytes_opt_(replyBodyValueSize) const void *replyBodyValue,
    _In_ ULONG replyBodyValueSize, _In_ WS_MESSAGE *requestMessage,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsSendFaultMessageForError(
    _In_ WS_CHANNEL *channel, _In_ WS_MESSAGE *replyMessage,
    _In_ WS_ERROR *faultError, _In_ HRESULT faultErrorCode,
    _In_ WS_FAULT_DISCLOSURE faultDisclosure, _In_ WS_MESSAGE *requestMessage,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsGetChannelProperty(_In_ WS_CHANNEL *channel,
...
//
HRESULT WINAPI WsSetChannelProperty(_In_ WS_CHANNEL *channel,
                                    _In_ WS_CHANNEL_PROPERTY_ID id,
                                    _In_reads_bytes_(valueSize)
                                        const void *value,
                                    _In_ ULONG valueSize,
                                    _In_opt_ WS_ERROR *error);
...
//  Channel function
//
//   Write out all the headers of the message to the channel, and prepare to
//   write the body elements.
//
...
//
HRESULT WINAPI WsWriteMessageEnd(_In_ WS_CHANNEL *channel,
...
//  Channel function
//
//   Read the headers of the next message from the channel, and prepare to read
//   the body elements.
//
HRESULT WINAPI WsReadMessageStart(_In_ WS_CHANNEL *channel,
                                  _In_ WS_MESSAGE *message,
                                  _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                                  _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsReadMessageEnd(_In_ WS_CHANNEL *channel,
                                _In_ WS_MESSAGE *message,
                                _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                                _In_opt_ WS_ERROR *error);
...
//   Close a channel.
//
HRESULT WINAPI WsCloseChannel(_In_ WS_CHANNEL *channel,
                              _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                              _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsAbortChannel(_In_ WS_CHANNEL *channel,
                              _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsResetChannel(_In_ WS_CHANNEL *channel,
                              _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsAbandonMessage(_In_ WS_CHANNEL *channel,
                                _In_ WS_MESSAGE *message,
                                _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsShutdownSessionChannel(
    _In_ WS_CHANNEL *channel, _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
    _In_opt_ WS_ERROR *error);
...
//  Context function
//
//   Returns a property of the specified operation context. It should be noted
//   that the
//  validity of these property is limited to the lifetime of the operation
//  context itself. See Operation Context Lifetime and Threading for details
//  around accessing these properties and the lifetime of the operation context.
//
HRESULT WINAPI
WsGetOperationContextProperty(_In_ const WS_OPERATION_CONTEXT *context,
...
HRESULT WINAPI
WsGetDictionary(_In_ WS_ENCODING encoding,
...
HRESULT WINAPI WsReadEndpointAddressExtension(
    _In_ WS_XML_READER *reader, _In_ WS_ENDPOINT_ADDRESS *endpointAddress,
    _In_ WS_ENDPOINT_ADDRESS_EXTENSION_TYPE extensionType,
    _In_ WS_READ_OPTION readOption, _In_ WS_HEAP *heap,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsCreateError(_In_reads_opt_(propertyCount)
...
//
HRESULT WINAPI WsAddErrorString(_In_ WS_ERROR *error,
...
//
HRESULT WINAPI WsGetErrorString(_In_ WS_ERROR *error, _In_ ULONG index,
...
//
HRESULT WINAPI WsGetErrorProperty(_In_ WS_ERROR *error,
...
//
HRESULT WINAPI WsSetErrorProperty(_In_ WS_ERROR *error,
                                  _In_ WS_ERROR_PROPERTY_ID id,
                                  _In_reads_bytes_(valueSize) const void *value,
                                  _In_ ULONG valueSize);
...
//  Errors function
//
//   Frees the contents of the error object, but does not free the error object
//   itself.
//
...
HRESULT WINAPI WsGetFaultErrorProperty(
    _In_ WS_ERROR *error, _In_ WS_FAULT_ERROR_PROPERTY_ID id,
    _Out_writes_bytes_(bufferSize) void *buffer, _In_ ULONG bufferSize);

//  Faults function
//
//   Set a fault-related property of the error object.
//
HRESULT WINAPI WsSetFaultErrorProperty(_In_ WS_ERROR *error,
...
//
HRESULT WINAPI WsCreateFaultFromError(_In_ WS_ERROR *error,
...
HRESULT WINAPI WsSetFaultErrorDetail(
    ...
    _In_ WS_WRITE_OPTION writeOption,
    _In_reads_bytes_opt_(valueSize) const void *value, _In_ ULONG valueSize);
...
//
HRESULT WINAPI WsCreateHeap(_In_ SIZE_T maxSize, _In_ SIZE_T trimSize,
                            _In_opt_ const WS_HEAP_PROPERTY *properties,
                            _In_ ULONG propertyCount, _Outptr_ WS_HEAP **heap,
                            _In_opt_ WS_ERROR *error);

//  Heap function
//  Allocate a chunk of data from the heap.
HRESULT WINAPI WsAlloc(_In_ WS_HEAP *heap, _In_ SIZE_T size,
                       _Outptr_result_bytebuffer_(size) void **ptr,
                       _In_opt_ WS_ERROR *error);

//  Heap function
//  Returns a property of the specified heap.
HRESULT WINAPI WsGetHeapProperty(_In_ WS_HEAP *heap,
...
//  Heap function
//
//   This resets the heap to have no allocations.  All allocations made on the
//   heap
//  using WsAlloc are no longer valid.  However, this call does not actually
//  free the heap object.
//
HRESULT WINAPI WsResetHeap(_In_ WS_HEAP *heap, _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsCreateListener(
    ...
    _In_ ULONG propertyCount,
    _In_opt_ const WS_SECURITY_DESCRIPTION *securityDescription,
    _Outptr_ WS_LISTENER **listener, _In_opt_ WS_ERROR *error);

//  Listener function
//
//   Start listening.
//
HRESULT WINAPI WsOpenListener(_In_ WS_LISTENER *listener,
...
//
HRESULT WINAPI WsAcceptChannel(_In_ WS_LISTENER *listener,
...
//   Stop listening.
//
HRESULT WINAPI WsCloseListener(_In_ WS_LISTENER *listener,
                               _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                               _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsAbortListener(_In_ WS_LISTENER *listener,
                               _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsResetListener(_In_ WS_LISTENER *listener,
                               _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsGetListenerProperty(_In_ WS_LISTENER *listener,
...
//
HRESULT WINAPI WsSetListenerProperty(_In_ WS_LISTENER *listener,
                                     _In_ WS_LISTENER_PROPERTY_ID id,
                                     _In_reads_bytes_(valueSize)
                                         const void *value,
                                     _In_ ULONG valueSize,
...
//  Listener function
//
//   Create a channel that is used to accept incoming message exchanges from a
//   listener.
//
HRESULT WINAPI WsCreateChannelForListener(
    _In_ WS_LISTENER *listener,
    _In_reads_opt_(propertyCount) const WS_CHANNEL_PROPERTY *properties,
    _In_ ULONG propertyCount, _Outptr_ WS_CHANNEL **channel,
    _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsCreateMessage(_In_ WS_ENVELOPE_VERSION envelopeVersion,
...
HRESULT WINAPI WsCreateMessageForChannel(
    ...
    _In_ ULONG propertyCount, _Outptr_ WS_MESSAGE **message,
    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsInitializeMessage(
    _In_ WS_MESSAGE *message, _In_ WS_MESSAGE_INITIALIZATION initialization,
    _In_opt_ WS_MESSAGE *sourceMessage, _In_opt_ WS_ERROR *error);

//  Message function
//
//   Prepare the message for reuse.
//
HRESULT WINAPI WsResetMessage(_In_ WS_MESSAGE *message,
                              _In_opt_ WS_ERROR *error);
...
//  Message function
//
//   Returns the WS_HEADER_ATTRIBUTES for the header element the reader is
//   positioned on.  The
//  envelope version of the message is used to determine which attributes to
//  return.
//
HRESULT WINAPI WsGetHeaderAttributes(_In_ WS_MESSAGE *message,
...
//
HRESULT WINAPI WsGetHeader(_In_ WS_MESSAGE *message,
...
HRESULT WINAPI WsGetCustomHeader(
    ...
    _In_ WS_REPEATING_HEADER_OPTION repeatingOption, _In_ ULONG headerIndex,
    _In_ WS_READ_OPTION readOption, _In_opt_ WS_HEAP *heap,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _Out_opt_ ULONG *headerAttributes, _In_opt_ WS_ERROR *error);

//  Message function
//
//   Removes all instances of the specified standard header from the message.
//
HRESULT WINAPI WsRemoveHeader(_In_ WS_MESSAGE *message,
                              _In_ WS_HEADER_TYPE headerType,
                              _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsSetHeader(_In_ WS_MESSAGE *message,
                           _In_ WS_HEADER_TYPE headerType,
                           _In_ WS_TYPE valueType,
                           _In_ WS_WRITE_OPTION writeOption,
                           _In_reads_bytes_(valueSize) const void *value,
...
//
HRESULT WINAPI WsRemoveCustomHeader(_In_ WS_MESSAGE *message,
...
HRESULT WINAPI WsAddCustomHeader(
    ...
    _In_ WS_WRITE_OPTION writeOption,
    _In_reads_bytes_(valueSize) const void *value, _In_ ULONG valueSize,
    _In_ ULONG headerAttributes, _In_opt_ WS_ERROR *error);

//  Message function
//
//   Adds a specified mapped header to the message.
//
HRESULT WINAPI WsAddMappedHeader(_In_ WS_MESSAGE *message,
                                 _In_ const WS_XML_STRING *headerName,
                                 _In_ WS_TYPE valueType,
                                 _In_ WS_WRITE_OPTION writeOption,
                                 _In_reads_bytes_(valueSize) const void *value,
...
//
HRESULT WINAPI WsRemoveMappedHeader(_In_ WS_MESSAGE *message,
                                    _In_ const WS_XML_STRING *headerName,
                                    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsGetMappedHeader(
    _In_ WS_MESSAGE *message, _In_ const WS_XML_STRING *headerName,
    _In_ WS_REPEATING_HEADER_OPTION repeatingOption, _In_ ULONG headerIndex,
    _In_ WS_TYPE valueType, _In_ WS_READ_OPTION readOption,
    _In_opt_ WS_HEAP *heap, _Out_writes_bytes_(valueSize) void *value,
    _In_ ULONG valueSize, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteBody(_In_ WS_MESSAGE *message,
...
//
HRESULT WINAPI WsReadBody(_In_ WS_MESSAGE *message,
                          _In_ const WS_ELEMENT_DESCRIPTION *bodyDescription,
                          _In_ WS_READ_OPTION readOption,
                          _In_opt_ WS_HEAP *heap,
                          _Out_writes_bytes_(valueSize) void *value,
...
//  Message function
//
//   Write out all the headers of the message and prepare to write the body
//   elements.
//
HRESULT WINAPI WsWriteEnvelopeStart(
    _In_ WS_MESSAGE *message, _In_ WS_XML_WRITER *writer,
    _In_opt_ WS_MESSAGE_DONE_CALLBACK doneCallback,
    _In_opt_ void *doneCallbackState, _In_opt_ WS_ERROR *error);

//  Message function
//
//   Write the closing elements of the message.
//
HRESULT WINAPI WsWriteEnvelopeEnd(_In_ WS_MESSAGE *message,
                                  _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI
WsReadEnvelopeStart(_In_ WS_MESSAGE *message, _In_ WS_XML_READER *reader,
                    _In_opt_ WS_MESSAGE_DONE_CALLBACK doneCallback,
                    _In_opt_ void *doneCallbackState, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsReadEnvelopeEnd(_In_ WS_MESSAGE *message,
                                 _In_opt_ WS_ERROR *error);

//  Message function
//  Returns a property of the specified message.
HRESULT WINAPI WsGetMessageProperty(_In_ WS_MESSAGE *message,
...
//
HRESULT WINAPI WsSetMessageProperty(_In_ WS_MESSAGE *message,
                                    _In_ WS_MESSAGE_PROPERTY_ID id,
                                    _In_reads_bytes_(valueSize)
                                        const void *value,
                                    _In_ ULONG valueSize,
...
//
HRESULT WINAPI WsAddressMessage(_In_ WS_MESSAGE *message,
...
//
HRESULT WINAPI WsCheckMustUnderstandHeaders(_In_ WS_MESSAGE *message,
                                            _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsMarkHeaderAsUnderstood(
    _In_ WS_MESSAGE *message, _In_ const WS_XML_NODE_POSITION *headerPosition,
    _In_opt_ WS_ERROR *error);

//  Message function
//
//   Ensures that the message has at least a specified amount of body data
//   available to it for reading.
//
HRESULT WINAPI WsFillBody(_In_ WS_MESSAGE *message, _In_ ULONG minSize,
                          _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                          _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsFlushBody(_In_ WS_MESSAGE *message, _In_ ULONG minSize,
                           _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                           _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsRequestSecurityToken(
    ...
    _In_ ULONG propertyCount, _Outptr_ WS_SECURITY_TOKEN **token,
    _In_opt_ const WS_ASYNC_CONTEXT *asyncContext, _In_opt_ WS_ERROR *error);

//  Security Processing Results function
//
//   Extract a field or a property from a security token. If the queried
//   property does not use the 'heap' parameter, the returned
//  data is owned by the security token and remains valid as long as the
//  security token itself remains valid. Specifically, for security tokens
//  extracted from a received message, the security token and fields extracted
//  from it are valid only as long as the message is not reset or freed.
//   If the 'heap' parameter is required by the property, then the returned data
//   is stored on the heap, with its lifetime
...
HRESULT WINAPI WsGetSecurityTokenProperty(
    ...
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_HEAP *heap, _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsCreateXmlSecurityToken(
    _In_opt_ WS_XML_BUFFER *tokenXml, _In_opt_ WS_SECURITY_KEY_HANDLE *tokenKey,
    _In_reads_opt_(propertyCount)
...
HRESULT WINAPI WsRevokeSecurityContext(
    _In_ WS_SECURITY_CONTEXT *securityContext, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI
WsGetSecurityContextProperty(_In_ WS_SECURITY_CONTEXT *securityContext,
...
//
HRESULT WINAPI
WsReadElement(_In_ WS_XML_READER *reader,
...
//
HRESULT WINAPI
WsReadAttribute(_In_ WS_XML_READER *reader,
...
//
HRESULT WINAPI WsReadType(_In_ WS_XML_READER *reader,
                          _In_ WS_TYPE_MAPPING typeMapping, _In_ WS_TYPE type,
...
//
HRESULT WINAPI
WsWriteElement(_In_ WS_XML_WRITER *writer,
               _In_ const WS_ELEMENT_DESCRIPTION *elementDescription,
               _In_ WS_WRITE_OPTION writeOption,
...
//
HRESULT WINAPI
WsWriteAttribute(_In_ WS_XML_WRITER *writer,
                 _In_ const WS_ATTRIBUTE_DESCRIPTION *attributeDescription,
                 _In_ WS_WRITE_OPTION writeOption,
                 _In_reads_bytes_opt_(valueSize) const void *value,
                 _In_ ULONG valueSize, _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsWriteType(_In_ WS_XML_WRITER *writer,
                           _In_ WS_TYPE_MAPPING typeMapping, _In_ WS_TYPE type,
                           _In_opt_ const void *typeDescription,
                           _In_ WS_WRITE_OPTION writeOption,
                           _In_reads_bytes_opt_(valueSize) const void *value,
...
//
//   A service operation can use this function to register for a cancel
//   notification.
//  It is only valid to call this API when the service operation is executing.
//  The behavior for calling it after the completion of Service Operation is not
//  supported.
//
//   While this API is being called and the runtime has determined that the
//   cancellation of the
//  service operation is necessary, it can call the callback during the call to
//  this API by the application.
//
//   The caller should therefore assume that the runtime may call on the
//   callback WS_OPERATION_CANCEL_CALLBACK as soon as the
...
HRESULT WINAPI WsRegisterOperationForCancel(
    ...
    _In_opt_ void *userState, _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsGetServiceHostProperty(
    _In_ WS_SERVICE_HOST *serviceHost, _In_ const WS_SERVICE_PROPERTY_ID id,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsCreateServiceHost(
    ...
    _In_reads_opt_(servicePropertyCount)
        const WS_SERVICE_PROPERTY *serviceProperties,
    _In_ ULONG servicePropertyCount, _Outptr_ WS_SERVICE_HOST **serviceHost,
    _In_opt_ WS_ERROR *error);

//  Service Host function
//
//   Open a service host for communication. Starts the listeners on all the
//   endpoints.
//
HRESULT WINAPI WsOpenServiceHost(_In_ WS_SERVICE_HOST *serviceHost,
                                 _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                                 _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsCloseServiceHost(_In_ WS_SERVICE_HOST *serviceHost,
                                  _In_opt_ const WS_ASYNC_CONTEXT *asyncContext,
                                  _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsAbortServiceHost(_In_ WS_SERVICE_HOST *serviceHost,
                                  _In_opt_ WS_ERROR *error);
...
//  this allows for an efficient way for reusing the same service host. It
//  resets the underlying channel and listeners for reuse.
//
HRESULT WINAPI WsResetServiceHost(_In_ WS_SERVICE_HOST *serviceHost,
                                  _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsGetServiceProxyProperty(
    _In_ WS_SERVICE_PROXY *serviceProxy, _In_ const WS_PROXY_PROPERTY_ID id,
    _Out_writes_bytes_(valueSize) void *value, _In_ ULONG valueSize,
    _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsCreateServiceProxy(
    ...
    _In_reads_opt_(channelPropertyCount)
        const WS_CHANNEL_PROPERTY *channelProperties,
    _In_ const ULONG channelPropertyCount,
    _Outptr_ WS_SERVICE_PROXY **serviceProxy, _In_opt_ WS_ERROR *error);

//  Service Proxy function
//
//   Opens the service proxy to a service endpoint.
//
//   Once the operation succeeds, application can now go ahead and make calls on
//   the service proxy.
//
HRESULT WINAPI WsOpenServiceProxy(_In_ WS_SERVICE_PROXY *serviceProxy,
...
//
//  .If a service operation call is pending on the service proxy, close will
//  wait for each call to complete.
//
//  .If the proxy was created with session based channel binding this will close
...
//   WsAbortServiceProxy can be used to abort the open operation. The service
//   proxy will
//  cancel all pending I/O and transition back to WS_SERVICE_PROXY_STATE_CREATED
//  state. WsAbortServiceProxy: While making calls or
//  WS_SERVICE_PROXY_STATE_OPEN state
//   WsAbortServiceProxy will abort all the underlying channels. The service
//   proxy is transitioned to
//  to WS_SERVICE_PROXY_STATE_FAULTED. Once abort is initiated service proxy
//  will not accept any new calls. The application can call WsCloseServiceProxy
//  to close it. WsAbortServiceProxy: During Close or
//  WS_SERVICE_PROXY_STATE_CLOSING state
//   During close, all underlying channels are aborted. The service proxy is
//   transitioned to the WS_SERVICE_PROXY_STATE_CLOSED state.
//
HRESULT WINAPI WsAbortServiceProxy(_In_ WS_SERVICE_PROXY *serviceProxy,
                                   _In_opt_ WS_ERROR *error);
...
//
//   Reusing the service proxy is helpful in scenarios where an application
//   connects
//  to the same service time and time again. The cost of initialization is only
//  paid once during the initial creation of the service proxy.
//
HRESULT WINAPI WsResetServiceProxy(_In_ WS_SERVICE_PROXY *serviceProxy,
                                   _In_opt_ WS_ERROR *error);

//  Service Proxy function
//
//   Abandons a call identified by the callId on the given (Service Proxy)
//   service proxy.
//
//   An applications must be careful with the usage of this operation. Since the
//   actual I/O specific
//  to the call is not canceled. The service proxy will keep the resources for
//  the abandoned call around to complete call.
//
//   This consumption of resources can be aggravated if the application
//   continues to abandon more
//  and more calls. This aggravation usually results if the server is slow to
//  respond back to the client. Mean while if the application continues to make
//  more calls on the service proxy that can further aggravate the situation.
//
HRESULT WINAPI WsAbandonCall(_In_ WS_SERVICE_PROXY *serviceProxy,
                             _In_ ULONG callId, _In_opt_ WS_ERROR *error);
...
//
//   This function is internally used by proxy service operation. The
//   application should
//  never call it directly.
//
HRESULT WINAPI WsCall(_In_ WS_SERVICE_PROXY *serviceProxy,
                      _In_ const WS_OPERATION_DESCRIPTION *operation,
                      _In_opt_ const void **arguments, _In_ WS_HEAP *heap,
                      _In_reads_opt_(callPropertyCount)
...
HRESULT WINAPI WsDecodeUrl(_In_ const WS_STRING *url, _In_ ULONG flags,
                           _In_ WS_HEAP *heap, _Outptr_ WS_URL **outUrl,
                           _In_opt_ WS_ERROR *error);

//  Url function
//  Encodes an URL into a string given its component parts.
HRESULT WINAPI WsEncodeUrl(_In_ const WS_URL *url, _In_ ULONG flags,
                           _In_ WS_HEAP *heap, _Out_ WS_STRING *outUrl,
                           _In_opt_ WS_ERROR *error);

//  Url function
//
//   Produces an absolute URL given a URL reference (absolute or relative URL)
//   and an absolute base URL.
//
HRESULT WINAPI WsCombineUrl(_In_ const WS_STRING *baseUrl,
                            _In_ const WS_STRING *referenceUrl,
                            _In_ ULONG flags, _In_ WS_HEAP *heap,
...
//
HRESULT WINAPI WsDateTimeToFileTime(_In_ const WS_DATETIME *dateTime,
...
//
HRESULT WINAPI WsFileTimeToDateTime(_In_ const FILETIME *fileTime,
                                    _Out_ WS_DATETIME *dateTime,
                                    _In_opt_ WS_ERROR *error);

//  Metadata Import function
//
//   Create a metadata object which is used to collect and process metadata
//   documents.
//
HRESULT WINAPI WsCreateMetadata(_In_reads_opt_(propertyCount)
...
//
HRESULT WINAPI WsReadMetadata(_In_ WS_METADATA *metadata,
                              _In_ WS_XML_READER *reader,
                              _In_ const WS_STRING *url,
                              _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsResetMetadata(_In_ WS_METADATA *metadata,
                               _In_opt_ WS_ERROR *error);
...
//
HRESULT WINAPI WsGetMetadataProperty(_In_ WS_METADATA *metadata,
...
//  Metadata Import function
//
//   Returns the address of a missing document that is referenced by the
//   metadata object.
//
...
//
HRESULT WINAPI WsGetMetadataEndpoints(_In_ WS_METADATA *metadata,
...
//  with the specified policy constraint structures.  If the alternative is
//  compatible, then the "out" fields within the constraint structures are
//  filled with information from the policy.
//
HRESULT WINAPI WsMatchPolicyAlternative(
    _In_ WS_POLICY *policy, _In_ ULONG alternativeIndex,
    _In_ WS_POLICY_CONSTRAINTS *policyConstraints, _In_ BOOL matchRequired,
    _In_ WS_HEAP *heap, _In_opt_ WS_ERROR *error);

//  Metadata Import function
//
//   Retrieve a property of the policy object.
//
HRESULT WINAPI WsGetPolicyProperty(_In_ WS_POLICY *policy,
...
//
HRESULT WINAPI WsGetPolicyAlternativeCount(_In_ WS_POLICY *policy,
                                           _Out_ ULONG *count,
                                           _In_opt_ WS_ERROR *error);
...
HRESULT WINAPI WsCreateServiceProxyFromTemplate(
    _In_ WS_CHANNEL_TYPE channelType,
    _In_reads_opt_(propertyCount) const WS_PROXY_PROPERTY *properties,
    _In_ const ULONG propertyCount, _In_ WS_BINDING_TEMPLATE_TYPE templateType,
    _In_reads_bytes_opt_(templateSize) void *templateValue,
    _In_ ULONG templateSize, _In_ const void *templateDescription,
...
HRESULT WINAPI WsCreateServiceEndpointFromTemplate(
    ...
    _In_ ULONG propertyCount, _In_opt_ const WS_STRING *addressUrl,
    _In_ const WS_SERVICE_CONTRACT *contract,
    _In_opt_ WS_SERVICE_SECURITY_CALLBACK authorizationCallback,
    _In_ WS_HEAP *heap, _In_ WS_BINDING_TEMPLATE_TYPE templateType,
    _In_reads_bytes_opt_(templateSize) void *templateValue,
    _In_ ULONG templateSize, _In_ const void *templateDescription,
    _In_ ULONG templateDescriptionSize,
    _Outptr_ WS_SERVICE_ENDPOINT **serviceEndpoint, _In_opt_ WS_ERROR *error);
