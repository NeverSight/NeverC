// HyperVDeviceVirtualization.h: ApiSet Contract for
// ext-ms-win-hyperv-devicevirtualization-l1-1 Copyright (c) Microsoft
...
HRESULT
WINAPI
HdvInitializeDeviceHost(_In_ HCS_SYSTEM computeSystem,
                        _Out_ HDV_HOST *deviceHostHandle);

HRESULT
WINAPI
HdvInitializeDeviceHostEx(_In_ HCS_SYSTEM computeSystem,
                          _In_ HDV_DEVICE_HOST_FLAGS flags,
                          _Out_ HDV_HOST *deviceHostHandle);

HRESULT
WINAPI
HdvTeardownDeviceHost(_In_ HDV_HOST deviceHostHandle);

HRESULT
WINAPI
HdvCreateDeviceInstance(_In_ HDV_HOST deviceHostHandle,
...
HRESULT
WINAPI
HdvReadGuestMemory(_In_ HDV_DEVICE requestor, _In_ UINT64 guestPhysicalAddress,
                   _In_ UINT32 byteCount, _Out_writes_(byteCount) BYTE *buffer);

HRESULT
WINAPI
HdvWriteGuestMemory(_In_ HDV_DEVICE requestor, _In_ UINT64 guestPhysicalAddress,
                    _In_ UINT32 byteCount,
                    _In_reads_(byteCount) const BYTE *buffer);

HRESULT
WINAPI
HdvCreateGuestMemoryAperture(_In_ HDV_DEVICE requestor,
                             _In_ UINT64 guestPhysicalAddress,
                             _In_ UINT32 byteCount, _In_ BOOL writeProtected,
                             _Out_ PVOID *mappedAddress);

HRESULT
WINAPI
HdvDestroyGuestMemoryAperture(_In_ HDV_DEVICE requestor,
                              _In_ PVOID mappedAddress);

HRESULT
WINAPI
HdvDeliverGuestInterrupt(_In_ HDV_DEVICE requestor, _In_ UINT64 msiAddress,
                         _In_ UINT32 msiData);

HRESULT
WINAPI
HdvRegisterDoorbell(_In_ HDV_DEVICE requestor,
                    _In_ HDV_PCI_BAR_SELECTOR BarIndex, _In_ UINT64 BarOffset,
                    _In_ UINT64 TriggerValue, _In_ UINT64 Flags,
                    _In_ HANDLE DoorbellEvent);

HRESULT
WINAPI
HdvUnregisterDoorbell(_In_ HDV_DEVICE requestor,
                      _In_ HDV_PCI_BAR_SELECTOR BarIndex, _In_ UINT64 BarOffset,
                      _In_ UINT64 TriggerValue, _In_ UINT64 Flags);

HRESULT
WINAPI
HdvCreateSectionBackedMmioRange(_In_ HDV_DEVICE requestor,
...
HRESULT
WINAPI
HdvDestroySectionBackedMmioRange(_In_ HDV_DEVICE requestor,
                                 _In_ HDV_PCI_BAR_SELECTOR barIndex,
                                 _In_ UINT64 offsetInPages);
...
typedef void(CALLBACK *HDV_PCI_DEVICE_TEARDOWN)(_In_opt_ PVOID deviceContext);

typedef HRESULT(CALLBACK *HDV_PCI_DEVICE_SET_CONFIGURATION)(
    _In_opt_ PVOID deviceContext, _In_ UINT32 configurationValueCount,
    _In_reads_(configurationValueCount) const LPCWSTR *configurationValues);

typedef HRESULT(CALLBACK *HDV_PCI_DEVICE_GET_DETAILS)(
    _In_opt_ PVOID deviceContext, _Out_ PHDV_PCI_PNP_ID pnpId,
    _In_ UINT32 probedBarsCount,
    _Out_writes_(probedBarsCount) UINT32 *probedBars);

typedef HRESULT(CALLBACK *HDV_PCI_DEVICE_START)(_In_opt_ PVOID deviceContext);

typedef void(CALLBACK *HDV_PCI_DEVICE_STOP)(_In_opt_ PVOID deviceContext);

typedef HRESULT(CALLBACK *HDV_PCI_READ_CONFIG_SPACE)(
    _In_opt_ PVOID deviceContext, _In_ UINT32 offset, _Out_ UINT32 *value);

typedef HRESULT(CALLBACK *HDV_PCI_WRITE_CONFIG_SPACE)(
    _In_opt_ PVOID deviceContext, _In_ UINT32 offset, _In_ UINT32 value);

typedef HRESULT(CALLBACK *HDV_PCI_READ_INTERCEPTED_MEMORY)(
    _In_opt_ PVOID deviceContext, _In_ HDV_PCI_BAR_SELECTOR barIndex,
    _In_ UINT64 offset, _In_ UINT64 length, _Out_writes_(length) BYTE *value);
...
BOOLEAN
__stdcall IsHdvInitializeDeviceHostPresent(VOID);

BOOLEAN
__stdcall IsHdvInitializeDeviceHostExPresent(VOID);

BOOLEAN
__stdcall IsHdvTeardownDeviceHostPresent(VOID);

BOOLEAN
__stdcall IsHdvCreateDeviceInstancePresent(VOID);

BOOLEAN
__stdcall IsHdvReadGuestMemoryPresent(VOID);

BOOLEAN
__stdcall IsHdvWriteGuestMemoryPresent(VOID);

BOOLEAN
__stdcall IsHdvCreateGuestMemoryAperturePresent(VOID);

BOOLEAN
__stdcall IsHdvDestroyGuestMemoryAperturePresent(VOID);

BOOLEAN
__stdcall IsHdvDeliverGuestInterruptPresent(VOID);

BOOLEAN
__stdcall IsHdvRegisterDoorbellPresent(VOID);

BOOLEAN
__stdcall IsHdvUnregisterDoorbellPresent(VOID);

BOOLEAN
__stdcall IsHdvCreateSectionBackedMmioRangePresent(VOID);

BOOLEAN
__stdcall IsHdvDestroySectionBackedMmioRangePresent(VOID);
