...
// DLL-interface classkey 'identifier' 	C4800: forcing bool variable to one or
// zero, possible performance loss 	C4786: 'identifier' : identifier was
// truncated to 'number' characters in the debug information 	C4251:
// 'identifier' : class 'type' needs to have dll-interface to export class
//'type2'
...
// If USE_POLARITY not defined, some Framedyn.dll symbols may not resolve
// correctly It's a declspec(dllimport/dllexport) issue.
#ifndef USE_POLARITY
...
// Required since TCHAR.H defines _ttoi64 -> atoi64, which requires
// oldnames.lib, which we don't use.
...
public:
  CFramework_Exception(LPCWSTR lpwszErrorText, HRESULT hr)
      : m_hr(hr), m_sErrorText(lpwszErrorText) {}
  CFramework_Exception(LPCWSTR lpwszErrorText)
      : m_hr(WBEM_S_NO_ERROR), m_sErrorText(lpwszErrorText) {}
