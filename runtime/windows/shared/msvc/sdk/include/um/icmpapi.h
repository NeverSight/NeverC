...
DWORD
WINAPI
IcmpSendEcho(_In_ HANDLE IcmpHandle, _In_ IPAddr DestinationAddress,
...
//                           comes in.
//
//    ApcRoutine           - This routine would be called when the calling
//    thread
//                           is in an alertable thread and an ICMP reply comes
//                           in.
//
...
//    comes in.
//    ICMP Response data is copied to the ReplyBuffer provided, and the caller
//    of this function has to parse it asynchronously.  The function
//    IcmpParseReply is provided for this purpose.
//
//    On non-NT platforms,
...
DWORD
WINAPI
IcmpSendEcho2(_In_ HANDLE IcmpHandle, _In_opt_ HANDLE Event,
#ifdef PIO_APC_ROUTINE_DEFINED
              _In_opt_ PIO_APC_ROUTINE ApcRoutine,
#else
              _In_opt_ FARPROC ApcRoutine,
#endif
              _In_opt_ PVOID ApcContext, _In_ IPAddr DestinationAddress,
              _In_reads_bytes_(RequestSize) LPVOID RequestData,
              _In_ WORD RequestSize,
              _In_opt_ PIP_OPTION_INFORMATION RequestOptions,
              _Out_writes_bytes_(ReplySize) LPVOID ReplyBuffer,
...
DWORD
WINAPI
IcmpSendEcho2Ex(_In_ HANDLE IcmpHandle, _In_opt_ HANDLE Event,
#ifdef PIO_APC_ROUTINE_DEFINED
                _In_opt_ PIO_APC_ROUTINE ApcRoutine,
#else
                _In_opt_ FARPROC ApcRoutine,
#endif
                _In_opt_ PVOID ApcContext, _In_ IPAddr SourceAddress,
...
DWORD
WINAPI
Icmp6SendEcho2(_In_ HANDLE IcmpHandle, _In_opt_ HANDLE Event,
#ifdef PIO_APC_ROUTINE_DEFINED
               _In_opt_ PIO_APC_ROUTINE ApcRoutine,
#else
               _In_opt_ FARPROC ApcRoutine,
...
// Routine Description:
//
//    Parses the reply buffer provided and returns the number of ICMP responses
//    found.
//
// Arguments:
//
...
//    IcmpSendEcho2
//                             This is rewritten to hold an array of
//                             ICMP_ECHO_REPLY structures. (i.e. the type is
//                             PICMP_ECHO_REPLY).
//
//    ReplySize              - This must be the size of the above buffer.
//
// Return Value:
//    Returns the number of ICMP responses found.  If there is an errors, return
//    value is zero.  The error can be determined by a call to GetLastError.
//
// Remarks:
//    This function SHOULD NOT BE USED on a reply buffer that was passed to
//    SendIcmpEcho. SendIcmpEcho actually parses the buffer before returning
//    back to the user.  This function is meant to be used only with
//    SendIcmpEcho2.
//--
...
DWORD
WINAPI
IcmpParseReplies(_Out_writes_bytes_(ReplySize) LPVOID ReplyBuffer,
...
DWORD
WINAPI
Icmp6ParseReplies(_Out_writes_bytes_(ReplySize) LPVOID ReplyBuffer,
