...
_Check_return_ _ACRTIMP int __cdecl _isctype_l(_In_ int _C, _In_ int _Type,
...
_When_(_Param_(1) == 0,
       _Post_equal_to_(0)) _Check_return_ _CRT_JIT_INTRINSIC _ACRTIMP
    int __cdecl
    isdigit(_In_ int _C);
...
// directly.
_Post_satisfies_(return > 0 && return < 5) _ACRTIMP int __cdecl
...
#define __ascii_tolower(c)                                                     \
  ((((c) >= 'A') && ((c) <= 'Z')) ? ((c) - 'A' + 'a') : (c))
#define __ascii_toupper(c)                                                     \
  ((((c) >= 'a') && ((c) <= 'z')) ? ((c) - 'a' + 'A') : (c))
#define __ascii_iswalpha(c)                                                    \
  (('A' <= (c) && (c) <= 'Z') || ('a' <= (c) && (c) <= 'z'))
#define __ascii_iswdigit(c) ('0' <= (c) && (c) <= '9')
#define __ascii_towlower(c)                                                    \
  ((((c) >= L'A') && ((c) <= L'Z')) ? ((c) - L'A' + L'a') : (c))
#define __ascii_towupper(c)                                                    \
  ((((c) >= L'a') && ((c) <= L'z')) ? ((c) - L'a' + L'A') : (c))

#if defined _CRT_DISABLE_PERFCRIT_LOCKS && !defined _DLL && !defined __cplusplus
#define isalpha(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _ALPHA) : __chvalidchk(c, _ALPHA))
#define isupper(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _UPPER) : __chvalidchk(c, _UPPER))
#define islower(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _LOWER) : __chvalidchk(c, _LOWER))
#define isdigit(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _DIGIT) : __chvalidchk(c, _DIGIT))
#define isxdigit(c) (MB_CUR_MAX > 1 ? _isctype(c, _HEX) : __chvalidchk(c, _HEX))
#define isspace(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _SPACE) : __chvalidchk(c, _SPACE))
#define ispunct(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _PUNCT) : __chvalidchk(c, _PUNCT))
#define isblank(c)                                                             \
  (MB_CUR_MAX > 1 ? (((c) == '\t') ? _BLANK : _isctype(c, _BLANK))             \
                  : (((c) == '\t') ? _BLANK : __chvalidchk(c, _BLANK)))
#define isalnum(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _ALPHA | _DIGIT)                               \
                  : __chvalidchk(c, (_ALPHA | _DIGIT)))
#define isprint(c)                                                             \
...
#define isgraph(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _PUNCT | _ALPHA | _DIGIT)                      \
                  : __chvalidchk(c, (_PUNCT | _ALPHA | _DIGIT)))
#define iscntrl(c)                                                             \
  (MB_CUR_MAX > 1 ? _isctype(c, _CONTROL) : __chvalidchk(c, _CONTROL))
#endif

__inline __crt_locale_data_public *__CRTDECL
__acrt_get_locale_data_prefix(void const volatile *const _LocalePointers) {
  _locale_t const _TypedLocalePointers = (_locale_t)_LocalePointers;
  return (__crt_locale_data_public *)_TypedLocalePointers->locinfo;
}

#ifdef _DEBUG
_ACRTIMP int __cdecl _chvalidator_l(_In_opt_ _locale_t, _In_ int _Ch,
                                    _In_ int _Mask);
#endif

__inline int __CRTDECL _chvalidchk_l(_In_ int const _C, _In_ int const _Mask,
...
__inline int __CRTDECL _ischartype_l(_In_ int const _C, _In_ int const _Mask,
                                     _In_opt_ _locale_t const _Locale) {
  if (_Locale &&
      __acrt_get_locale_data_prefix(_Locale)->_locale_mb_cur_max > 1) {
