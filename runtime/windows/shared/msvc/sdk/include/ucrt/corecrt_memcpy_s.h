...
_Success_(return == 0) _Check_return_opt_ _CRT_MEMCPY_S_INLINE errno_t __CRTDECL
    memcpy_s(_Out_writes_bytes_to_opt_(_DestinationSize,
  ...
  if (_Source == NULL || _DestinationSize < _SourceSize) {
    ...
    _CRT_MEMCPY_S_VALIDATE_RETURN_ERRCODE(_DestinationSize >= _SourceSize,
                                          ERANGE);
...
memmove_s(_Out_writes_bytes_to_opt_(_DestinationSize,
                                    _SourceSize) void *const _Destination,
          _In_ rsize_t const _DestinationSize,
          _In_reads_bytes_opt_(_SourceSize) void const *const _Source,
          _In_ rsize_t const _SourceSize) {
