...
_memicmp(_In_reads_bytes_opt_(_Size) void const *_Buf1,
         _In_reads_bytes_opt_(_Size) void const *_Buf2, _In_ size_t _Size);

_Check_return_ _ACRTIMP int __cdecl
_memicmp_l(_In_reads_bytes_opt_(_Size) void const *_Buf1,
           _In_reads_bytes_opt_(_Size) void const *_Buf2, _In_ size_t _Size,
...
inline typename _CrtEnableIf<(_Size > 1), void *>::_Type __cdecl
memcpy(_DstType (&_Dst)[_Size], _In_reads_bytes_opt_(_SrcSize) void const *_Src,
       _In_ size_t _SrcSize) _CRT_SECURE_CPP_NOTHROW {
  return memcpy_s(_Dst, _Size * sizeof(_DstType), _Src, _SrcSize) == 0 ? _Dst
                                                                       : 0;
}
}
#endif
...
inline errno_t __CRTDECL memcpy_s(
    _DstType (&_Dst)[_Size], _In_reads_bytes_opt_(_SrcSize) void const *_Src,
...
_ACRTIMP void *__cdecl memccpy(_Out_writes_bytes_opt_(_Size) void *_Dst,
...
memchr(_In_reads_bytes_opt_(_N) void *_Pv, _In_ int _C, _In_ size_t _N) {
