 ...
 *
 *       The * operators take precedence, get overloaded, return a char, and
 * then you have:
 *
 *       char i = (char)64 + (char)64; //overflow!
 *
 *       This situation would mean that safety would depend on usage, which
 * isn't acceptable.
 *
 *       One key operator that is missing is an implicit cast to type T. The
 * reason for this is that if there is an implicit cast operator, then we end up
 * with an ambiguous compile-time precedence. Because of this amiguity, there
 ...
 * Performance Notes below for additional information.
 *
 *       Many of the conditionals will optimize out or be inlined for a release
 *       build (especially with /Ox), but it does have significantly more
 * overhead, especially for signed numbers. If you do not _require_ negative
 * numbers, use unsigned integer types - certain types of problems cannot occur,
 * and this class performs most efficiently.
 *
 ...
 *       We provide two error policy along with SafeInt:
 * SafeIntErrorPolicy_SafeIntException, which throws SafeIntException in case of
 * error, and SafeIntErrorPolicy_InvalidParameter, which calls
 * _invalid_parameter to terminate the program.
 *
 *       You can replace the error policy class with any class you like. This is
 * accomplished by: 1) Create a class that has the following interface:
 *
 ...
 * exceptions, or do anything you like, just don't return from the call back
 * into the code.
 *
 ...
 *       Performance:
 *
 *       Due to the highly nested nature of this class, you can expect
 * relatively poor performance in unoptimized code. In tests of optimized code
 * vs. correct inline checks in native code, this class has been found to take
 * approximately 8% more CPU time (this varies), most of which is due to
 * exception handling.
 *
 *       Binary Operators:
 *
 *       All of the binary operators have certain assumptions built into the
 * class design. This is to ensure correctness. Notes on each class of operator
 * follow:
 *
 ...
 *       U op SafeInt< T, E >
 *
 *       The SafeInt< T, E > op SafeInt< U, E > variant is explicitly not
 * supported, and if you try to do this the compiler with throw the following
 * error:
 *
 *       error C2593: 'operator *' is ambiguous
 *
 *       This is because the arithmetic operators are required to return a
 * SafeInt of some type. The compiler cannot know whether you'd prefer to get a
 * type T or a type U returned. If you need to do this, you need to extract the
 * value contained within one of the two using the casting operator. For
 * example:
 *
 ...
 * differing types is allowed.
 *
 *       Shift Operators:
 *
 *       Shift operators always return the type on the left hand side of the
 * operator. Mixed type operations are allowed because the return type is always
 * known.
 *
 *       Boolean Operators:
 *
 *       Like comparison operators, these overloads always return type bool, and
 * mixed-type SafeInts are allowed. Additionally, specific overloads exist for
 * type bool on both sides of the operator.
 *
 *       Binary Operators:
 *
 *       Mixed-type operations are discouraged, however some provision has been
 * made in order to enable things like:
 *
 *       SafeInt<char> c = 2;
 *
 *       if(c & 0x02)
 ...
 *       In the case of binary operations on integers smaller than 32-bit, or of
 * mixed type, corner cases do exist where you could get unexpected results. In
 * any case where SafeInt returns a different result than the underlying
 * operator, it will call _ASSERTE(). You should examine your code and cast
 * things properly so that you are not programming with side effects.
 *
 *       Comparison Operators and ANSI Conversions:
 *
 *       The comparison operator behavior in this class varies from the ANSI
 * definition. As an example, consider the following:
 *
 ...
 * 0xffffffff, and then gets cast again to an unsigned int, losing the true
 * value. This behavior is despite the fact that an __int64 exists, and the
 * following code will yield a different (and intuitively correct) answer:
 *
 ...
 *
 *       Note that combinations with smaller integers won't display the problem
 * - if you changed "unsigned int" above to "unsigned short", you'd get the
 * right answer.
 *
 *       If you prefer to retain the ANSI standard behavior insert, before
 * including safeint.h:
 *
...
// Free-standing functions that can be used where you only need to check one
...
inline bool SafeCast(const T From, U &To) throw() {
  return (
      details::SafeCastHelper<U, T, details::SafeIntErrorPolicy_NoThrow>::Cast(
          From, To) == SafeIntNoError);
}
...
inline bool SafeModulus(const T &t, const U &u, T &result) throw() {
  return (
      details::ModulusHelper<
          T, U, details::SafeIntErrorPolicy_NoThrow>::Modulus(t, u, result) ==
      SafeIntNoError);
}
...
inline bool SafeMultiply(T t, U u, T &result) throw() {
  return (
      details::MultiplicationHelper<
          T, U, details::SafeIntErrorPolicy_NoThrow>::Multiply(t, u, result) ==
      SafeIntNoError);
}
...
inline bool SafeDivide(T t, U u, T &result) throw() {
  return (
      details::DivisionHelper<
          T, U, details::SafeIntErrorPolicy_NoThrow>::Divide(t, u, result) ==
      SafeIntNoError);
}
...
inline bool SafeAdd(T t, U u, T &result) throw() {
  return (
      details::AdditionHelper<
          T, U, details::SafeIntErrorPolicy_NoThrow>::Addition(t, u, result) ==
      SafeIntNoError);
}
...
inline bool SafeSubtract(T t, U u, T &result) throw() {
  return (
      details::SubtractionHelper<
          T, U, details::SafeIntErrorPolicy_NoThrow>::Subtract(t, u, result) ==
      SafeIntNoError);
}
...
public:
  SafeInt() throw() {
    static_assert(details::NumericType<T>::isInt,
                  "SafeInt<T>: T needs to be an integer type");
    m_int = 0;
  }
  ...
  template <typename U> SafeInt(const SafeInt<U, E> &u) {
    static_assert(details::NumericType<T>::isInt,
                  "SafeInt<T>: T needs to be an integer type");
    *this = SafeInt<T, E>((U)u);
  }

  template <typename U> SafeInt(const U &i) {
    static_assert(details::NumericType<T>::isInt,
                  "SafeInt<T>: T needs to be an integer type");
  ...
  operator bool() const throw() { return !!m_int; }
  ...
  SafeInt<T, E> operator-() const {
    ...
    T tmp;
    details::NegationHelper<T, E, details::IntTraits<T>::isSigned>::Negative(
        m_int, tmp);
  ...
  // Left shift
  // Also, shifting > bitcount is undefined - trap in debug (check
  // _SAFEINT_SHIFT_ASSERT)
  ...
  Min(SafeInt<T, E> test,
      SafeInt<T, E> floor = SafeInt<T, E>(details::IntTraits<T>::minInt)) const
      throw() {
  ...
  Max(SafeInt<T, E> test,
      SafeInt<T, E> upper = SafeInt<T, E>(details::IntTraits<T>::maxInt)) const
      throw() {
    T tmp = test > m_int ? test : m_int;
  ...
  template <int bits> const SafeInt<T, E> &Align() {
    ...
    _SAFEINT_SHIFT_ASSERT(((details::IntTraits<T>::isSigned &&
                            bits < (int)details::IntTraits<T>::bitCount - 1) ||
                           (!details::IntTraits<T>::isSigned &&
                            bits < (int)details::IntTraits<T>::bitCount)) &&
                          bits >= 0 &&
                          (!details::IntTraits<T>::isSigned || m_int > 0));
...
#pragma warning(suppress : 4127 6326)
  if (sizeof(T) == sizeof(U) &&
      details::IntTraits<T>::isSigned == details::IntTraits<U>::isSigned) {
    if (rhs != 0) {
...
#pragma warning(disable : 4127 4146 4307 4310 6326)
  ...
  if (details::DivisionMethod<U, T>::method ==
      details::DivisionState_UnsignedSigned) {
    if ((T)rhs > 0)
...
SafeInt<T, E> operator-(U lhs, SafeInt<T, E> rhs) {
  T ret(0);
  details::SubtractionHelper<
      U, T, E, details::SubtractionMethod2<U, T>::method>::Subtract(lhs,
                                                                    rhs.Ref(),
                                                                    ret);
...
T *&operator+=(T *&lhs, SafeInt<U, E> rhs) {
  ...
  lhs = reinterpret_cast<T *>(
      (uintptr_t)(ptr_val +
                  (ptrdiff_t)(SafeInt<ptrdiff_t, E>(rhs) * sizeof(T))));
  return lhs;
}
...
T *&operator*=(T *lhs, SafeInt<U, E> rhs) {
  static_assert(details::DependentFalse<T>::value,
                "SafeInt<T>: This operator explicitly not supported");
  return lhs;
}
