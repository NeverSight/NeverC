...
//   #include <ntddscsi.h> in that source file a second time to instantiate the
//   GUIDs
//
#ifdef DEFINE_GUID
//
...
#define IOCTL_ATA_MINIPORT                                                     \
  CTL_CODE(IOCTL_SCSI_BASE, 0x040d, METHOD_BUFFERED,                           \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
#define IOCTL_MINIPORT_PROCESS_SERVICE_IRP                                     \
  CTL_CODE(IOCTL_SCSI_BASE, 0x040e, METHOD_BUFFERED,                           \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#define IOCTL_MPIO_PASS_THROUGH_PATH_DIRECT                                    \
  CTL_CODE(IOCTL_SCSI_BASE, 0x0410, METHOD_BUFFERED,                           \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#define IOCTL_MPIO_PASS_THROUGH_PATH_EX                                        \
  CTL_CODE(IOCTL_SCSI_BASE, 0x0413, METHOD_BUFFERED,                           \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
#define IOCTL_MPIO_PASS_THROUGH_PATH_DIRECT_EX                                 \
  CTL_CODE(IOCTL_SCSI_BASE, 0x0414, METHOD_BUFFERED,                           \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
typedef struct _SCSI_PASS_THROUGH_EX {
  ...
  ULONG TimeOutValue;
  ULONG StorAddressOffset; // a value bigger than (structure size + CdbLength)
...
typedef struct _SCSI_PASS_THROUGH_DIRECT32_EX {
  ...
  _Field_size_bytes_full_opt_(DataInTransferLength) VOID *POINTER_32
      DataInBuffer;
  _Field_size_bytes_full_(CdbLength) UCHAR Cdb[ANYSIZE_ARRAY];
...
typedef struct _MPIO_PASS_THROUGH_PATH_DIRECT_EX {
  ULONG
  PassThroughOffset; // Offset to a PSCSI_PASS_THROUGH_DIRECT_EX structure.
  ULONG Version;
  USHORT Length;
  UCHAR Flags;
  UCHAR PortNumber;
...
//
// Define the IOCTL_MPIO_PASS_THROUGH_PATH_DIRECT structure for Win64
// (thunking).
//
...
//
// Define the IOCTL_MPIO_PASS_THROUGH_PATH_DIRECT_EX structure for Win64
// (thunking).
//
...
//
// Acceptable signatures for SCSI IOCTL MINIPORT calls. Must be equal in byte
// size to sizeof(SrbIoctl->Signature)
//
...
//
// Function set to control write back caching policy separated non volatile
// cache in the miniport
//
...
//
// NOTE that data structure NVCACHE_HINT_PAYLOAD is deprecated along with NRB
// function - NRB_FUNCTION_PASS_HINT_PAYLOAD.
//
// Parameter structure for NRB_FUNCTION_PASS_HINT_PAYLOAD request
// This is the corresponding data structure for FIS 0x27 defined in SATA-IO
// spec. Caller that prepares this data structure shall set '0' to any non-used
// data field.
//
...
//
// Describes parameters of non volatile cache, supported by the miniport (aka
// separated nv cache).
//
// Caching is associated with the logical unit, it is up to the miniport to
// implement the exact scope (adapter or unit). MP caching  is relative to what
// miniport considers as primary non volatile media for the unit. Miniport
// caching layer  is separate from the caching layer integrated on the device
// and managed by the specific NRB functions. Miniport may have to support both
// separated cache and on device cache as well as volatile caches. Volatile
// cache policy is queried through the StorageProperty interface.
//
// Note that owner of the caching  policy may be miniport or the HBA firmware,
// in either case miniport is the handler of the policy setting.
//
...
typedef struct _NV_SEP_CACHE_PARAMETER {
  ...
  union {
    ...
    struct {
      ...
      //
      UCHAR WriteCacheEnabled : 1; // Is write caching enabled by the miniport
                                   // as a persistent policy
      UCHAR WriteCacheChangeable
          : 1; // Does the miniport respect change in write caching policy
      UCHAR WriteThroughIOSupported
          : 1; // Does the miniport support WriteThrough semantics for the NV
               // cache on individual Writes .
      UCHAR FlushCacheSupported
          : 1; // Does the miniport support flushing of the NV cache
  ...
  //
  UCHAR WriteCacheType; // Type of the NV cache (policy) supported by the
                        // miniport - persistent setting.
  UCHAR WriteCacheTypeEffective; // Effective type of the NV cache (policy) used
                                 // by the miniport at the time of query.
...
//
// There are 2 distinct settings fields, one describing policy as used by the
// miniport after boot. The other field describes current effective policy
// setting, which may be impacted by NRB_FUNCTION_NVSEPARATED_WB_DISABLE
//
// Caller of NRB_FUNCTION_NVSEPARATED_INFO should expect only the effective
// policy setting to be modified if the query is made after call to
// NRB_FUNCTION_NVSEPARATED_WB_DISABLE (or other control codes in the future).
//
// Control of the persistent policy setting (WriteCacheType) is performed only
// through miniport proprietary interfaces
//

typedef enum _NV_SEP_WRITE_CACHE_TYPE {
  NVSEPWriteCacheTypeUnknown =
      0, // Miniport can't report the type of the write cache
  NVSEPWriteCacheTypeNone =
      1, // Miniport doesn't support non volatile write cache
...
//
// Diagnostic level allow caller to control what kinds of data the provider
// should return.
//
// Currently there is only default level defined, provider makes the call to
// return anything it assumes helpful for diagnostic purpose.
//
...
// Input buffer should contain SRB_IO_CONTROL, STORAGE_DIAGNOSTIC_MP_REQUEST
// structures.
//
...
//     As input:
//       "BufferSize" should be set to number of bytes allocated for the
//       DataBuffer.
//     As output:
//       If the request is failed because of buffer too short, "BufferSize"
//       should be set to the length required for DataBuffer by the diagnostic
//       data provider; If the request is successful, it should be filled with
//       returned data size of DataBuffer; For other cases, it should be cleared
//       to 0.
// - Output: DataBuffer
//
...
//
// Structure used to describe the list of ranges to process, should match the
// one in ntddstor.h
//

typedef struct _MP_DEVICE_DATA_SET_RANGE {
  LONGLONG
  StartingOffset; // Measured in bytes,  must align to a sector boundary
...
typedef struct _DSM_NOTIFICATION_REQUEST_BLOCK {
  ...
  ULONG DataSetProfile; // Expected profile of IO access (based on OS file type
                        // : eg, hibernation, crashdump)
...
//
// Define access profiles, which are passed to the miniport, as describing
// expected access pattern to the dataset
//
...
//
#define MINIPORT_DSM_NOTIFY_FLAG_BEGIN                                         \
  0x00000001 // The given LBA range is being used as defined by the profileID
#define MINIPORT_DSM_NOTIFY_FLAG_END                                           \
  0x00000002 // The given LBA range is no longer being used as defined by the
             // proileID
...
// e.g. it increases this value every time when it receives a
// HYBRID_FUNCTION_ENABLE_CACHING_MEDIUM call.
//
// When driver receives HYBRID_FUNCTION_DISABLE_CACHING_MEDIUM call, it
// decreases "RefCount" when it's bigger than 0, and then only send command to
// disable caching medium when the new "RefCount" value is 0.
//
// In case of HYBRID_FUNCTION_DISABLE_CACHING_MEDIUM is received but no command
// is sent to device because of decreased "RefCount" is not 0, the driver
// returns following status to inform caller.
//
#define HYBRID_STATUS_ENABLE_REFCOUNT_HOLD 0x10

//
// General input data structure.
// Any data structures in input buffer after SRB_IO_CONTROL should contain
...
// Input buffer should contain SRB_IO_CONTROL and HYBRID_REQUEST_BLOCK data
// structures. Field "DataBufferOffset" of HYBRID_REQUEST_BLOCK points to output
// data buffer and HYBRID_INFORMATION should be returned.
//
...
typedef enum _NVCACHE_STATUS {
  ...
  NvCacheStatusDisabling =
      1, // non-volatile cache is in process of being disabled.
...
typedef struct _HYBRID_INFORMATION {
  ...
  NVCACHE_STATUS
  Status; // for hybrid disk, expect values can be: NvCacheStatusDisabling,
          // NvCacheStatusDisabled or NvCacheStatusEnabled
  NVCACHE_TYPE CacheTypeEffective; // for hybrid disk, expect value will be:
                                   // NvCacheTypeWriteBack
  NVCACHE_TYPE CacheTypeDefault;   // for hybrid disk, expect values can be:
                                   // NvCacheTypeWriteBack

  ULONG FractionBase; // Base value of all fraction type of fields in the data
                      // structure. For hybrid disk, value of this field will be
                      // 255.

  ULONGLONG CacheSize; // total size of NVCache. unit: LBA count

  struct {
    ULONG WriteCacheChangeable
        : 1; // Does the device respect change in write caching policy
    ULONG WriteThroughIoSupported
        : 1; // Does the device support WriteThrough semantics for the NVCache
             // on individual Writes.
    ULONG FlushCacheSupported
        : 1;             // Does the device support flushing of the NVCache
    ULONG Removable : 1; // Does the nvcache can be removed.
    ULONG ReservedBits : 28;
  } Attributes;

  struct {
    UCHAR PriorityLevelCount;    // A non-zero value indicates the non-volatile
                                 // cache supports priority levels.
    BOOLEAN MaxPriorityBehavior; // If set to TRUE, the disk may fail IO sent
                                 // with max priority level when it cannot find
                                 // space for the IO in caching medium.
    UCHAR OptimalWriteGranularity; // In LBAs. Value is the power value (of 2).
                                   // Value 0xFF means that Optimal Write
                                   // Granularity is not indicated.
                                   //          For example: value 0 indicates
                                   //          2^0 = 1 logical sector, 1
                                   //          indicates 2^1 = 2 logical sectors
    UCHAR Reserved;

    ULONG
    DirtyThresholdLow; // fraction type of value, with base "FractionBase".
    ULONG
    DirtyThresholdHigh; // fraction type of value, with base "FractionBase".

    struct {
      ULONG CacheDisable : 1;      // support of disabling the caching medium
      ULONG SetDirtyThreshold : 1; // support of Setting dirty threshold for the
                                   // entire caching medium
      ...
      ULONG MaxEvictCommands; // Max outstanding Evict commands concurrently.
                              // Only value when "Evict" value is 1.

      ULONG MaxLbaRangeCountForEvict; // Count of LBA ranges can be associated
                                      // with evict command. Only value when
                                      // "Evict" value is 1.
      ULONG MaxLbaRangeCountForChangeLba; // Count of LBA ranges associated with
                                          // PriorityChangeByLbaRange command.
                                          // Only value when
                                          // "PriorityChangeByLbaRange" value
                                          // is 1.
    } SupportedCommands;
...
//
// Parameter for HYBRID_FUNCTION_DISABLE_CACHING_MEDIUM and
// HYBRID_FUNCTION_ENABLE_CACHING_MEDIUM Input buffer should contain
// SRB_IO_CONTROL and HYBRID_REQUEST_BLOCK data structures. Field
// "DataBufferOffset" of HYBRID_REQUEST_BLOCK should be set to "0" indicating
// there is no other parameter associated. NOTE that these functions don't have
// output parameter.
//

//
// Parameter for HYBRID_FUNCTION_SET_DIRTY_THRESHOLD
// Input buffer should contain SRB_IO_CONTROL, HYBRID_REQUEST_BLOCK and
// HYBRID_DIRTY_THRESHOLDS data structures. Field "DataBufferOffset" of
// HYBRID_REQUEST_BLOCK should be set to the starting offset of
// HYBRID_DIRTY_THRESHOLDS from beginning of buffer. NOTE that these functions
// don't have output parameter.
//
...
// Input buffer should contain SRB_IO_CONTROL, HYBRID_REQUEST_BLOCK and
// HYBRID_DEMOTE_BY_SIZE data structures. Field "DataBufferOffset" of
// HYBRID_REQUEST_BLOCK should be set to the starting offset of
// HYBRID_DEMOTE_BY_SIZE from beginning of buffer. NOTE that these functions
// don't have output parameter.
//
...
//
// For IOCTL_SCSI_MINIPORT_FIRMWARE, the data buffer should contain following
// structures/fields:
// 1. SRB_IO_CONTROL
//    This is the header data structure indicating which IOCTL is sent. In this
//    case, it's IOCTL_SCSI_MINIPORT_FIRMWARE.
...
//    b: the data buffer location (in field - DataBufferOffset) and length (in
//    field - DataBufferLength) for the firmware function.
//       DataBufferOffset: should have value: ALIGN_UP(sizeof(SRB_IO_CONTROL) +
//       sizeof(FIRMWARE_REQUEST_BLOCK), PVOID). This is to make sure the buffer
//       is pointer aligned.
// 3. Padding. In case of (sizeof(SRB_IO_CONTROL) +
// sizeof(FIRMWARE_REQUEST_BLOCK)) is not multiple of pointer size, there will
// be padding space.
// 4. STORAGE_FIRMWARE_INFO or STORAGE_FIRMWARE_DOWNLOAD or
// STORAGE_FIRMWARE_ACTIVATE depending on the function code of firmware request.
//
...
// downloaded image, and activated with controller reset.
//
...
// Input buffer should contain SRB_IO_CONTROL and FIRMWARE_REQUEST_BLOCK data
// structures. Field "DataBufferOffset" of FIRMWARE_REQUEST_BLOCK points to
// output data buffer and STORAGE_FIRMWARE_INFO should be returned.
//

//
// NOTE: In case of (NTDDI_VERSION >= NTDDI_WINTHRESHOLD), applications should
// use
...
//
//      Storage port driver uses FIRMWARE_FUNCTION_GET_INFO as corresponding
//      interface to communicate to miniport. Version 1 data structures below
//      are kept for backwards compatibility only. It's recommended that
//      miniport drivers support Version 2 data structures.
//

//
// Output parameter for FIRMWARE_FUNCTION_GET_INFO
// The total size of returned data is for Firmware Info is:
//   sizeof(STORAGE_FIRMWARE_INFO) + sizeof(STORAGE_FIRMWARE_SLOT_INFO) *
//   SlotCount.
// If the buffer is not big enough, callee should set
// "FIRMWARE_STATUS_OUTPUT_BUFFER_TOO_SMALL" into "ReturnCode" field of
// SRB_IO_CONTROL, and the required size in "DataBufferLength" field of
...
// Input buffer should contain SRB_IO_CONTROL and FIRMWARE_REQUEST_BLOCK data
// structures. Field "DataBufferOffset" of FIRMWARE_REQUEST_BLOCK points to
// input data buffer that contains STORAGE_FIRMWARE_DOWNLOAD.
//
...
// Input buffer should contain SRB_IO_CONTROL and FIRMWARE_REQUEST_BLOCK data
// structures. Field "DataBufferOffset" of FIRMWARE_REQUEST_BLOCK points to
// input data buffer that contains STORAGE_FIRMWARE_ACTIVATE.
//
...
typedef LONG DUMP_DEVICE_POWERON_ROUTINE(_In_ PVOID Context);
...
typedef struct _DUMP_POINTERS_EX {
  ...
  //
  ULONG dwPortFlags; // Bit mask of various flags, returned by the port driver
                     // to crashdump driver

  //
  // Configuration values for device telemetry collection - obtained from either
  // live device or from the registry.
  //
  ULONG MaxDeviceDumpSectionSize; // Size of the buffer to hold complete
                                  // telemetry section from the storage stack
                                  // (header, public and private subsections)
  ULONG MaxDeviceDumpLevel; // Max level of device dump to request from the
                            // device at the crash time
...
#define DUMP_POINTERS_EX_V2_SIZE                                               \
  ((ULONG)FIELD_OFFSET(DUMP_POINTERS_EX, dwPortFlags))
...
#define DUMP_POINTERS_EX_V2_SIZE                                               \
  ((ULONG)FIELD_OFFSET(DUMP_POINTERS_EX, dwPortFlags))
#define DUMP_POINTERS_EX_V3_SIZE                                               \
  ((ULONG)FIELD_OFFSET(DUMP_POINTERS_EX, DumpDevicePowerOn))
...
//
// Driver stack is capable of producing device and driver telemetry data
// (individual device may not be able to return data)
//
...
//
// Driver stack is capable to provide driver full path(DUMP_DRIVER_EX is
// allocated/used by lower stack)
//
...
//      The storage port driver uses these structures when communicating with
//      the miniport.
//
