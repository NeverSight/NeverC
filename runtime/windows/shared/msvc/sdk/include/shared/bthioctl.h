...
#define IOCTL_INTERNAL_BTHENUM_GET_ENUMINFO                                    \
...
// Output:  BTH_SDP_STREAM_RESPONSE or bigger
#define IOCTL_BTH_SDP_SERVICE_ATTRIBUTE_SEARCH BTH_CTL(BTH_IOCTL_BASE + 0x84)
...
typedef struct _BTH_VENDOR_SPECIFIC_COMMAND {
  ...
  // then if a single pattern matches the command, we decide that we have a
  // match.
