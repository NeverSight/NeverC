...
#define IOCTL_VOLUME_ALLOCATE_BC_STREAM                                        \
  CTL_CODE(IOCTL_VOLUME_BASE, 16, METHOD_BUFFERED,                             \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
#define IOCTL_VOLUME_FREE_BC_STREAM                                            \
  CTL_CODE(IOCTL_VOLUME_BASE, 17, METHOD_BUFFERED,                             \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#define IOCTL_VOLUME_PREPARE_FOR_CRITICAL_IO                                   \
  CTL_CODE(IOCTL_VOLUME_BASE, 19, METHOD_BUFFERED,                             \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#define IOCTL_VOLUME_PREPARE_FOR_SHRINK                                        \
  CTL_CODE(IOCTL_VOLUME_BASE, 23, METHOD_BUFFERED,                             \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#define IOCTL_VOLUME_IS_CSV                                                    \
  CTL_CODE(IOCTL_VOLUME_BASE, 24, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_VOLUME_GET_CSVBLOCKCACHE_CALLBACK                                \
  CTL_CODE(IOCTL_VOLUME_BASE, 26, METHOD_BUFFERED,                             \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
typedef _Function_class_(CSV_BLOCKCACHE_PURGE_CACHE)
    _IRQL_requires_max_(PASSIVE_LEVEL) void (*CSV_BLOCKCACHE_PURGE_CACHE)(
        _Inout_ PVOID CallbackContext, _In_ ULONGLONG Offset,
        _In_ ULONGLONG Length);

typedef _Function_class_(CSV_BLOCKCACHE_DISABLE_CACHE)
    _IRQL_requires_max_(PASSIVE_LEVEL) void (*CSV_BLOCKCACHE_DISABLE_CACHE)(
        _Inout_ PVOID CallbackContext);

typedef _Function_class_(CSV_BLOCKCACHE_ENABLE_CACHE)
    _IRQL_requires_max_(PASSIVE_LEVEL)
        LONG (*CSV_BLOCKCACHE_ENABLE_CACHE)(_Inout_ PVOID CallbackContext);

// V2 Callbacks

typedef _Function_class_(CSV_FILECACHE_PURGE_RDCACHE)
    _IRQL_requires_max_(PASSIVE_LEVEL) void (*CSV_FILECACHE_PURGE_RDCACHE)(
        _Inout_ PVOID CallbackContext, _In_ PCSV_FILECACHE_CONTEXT CSVContext);

typedef _Function_class_(CSV_FILECACHE_HANDLE_CACHE_IO)
    _IRQL_requires_max_(PASSIVE_LEVEL) LONG (*CSV_FILECACHE_HANDLE_CACHE_IO)(
        _Inout_ PVOID CallbackContext,
        _Inout_ PCSV_FILECACHE_CONTEXT CSVContext);

typedef _Function_class_(CSV_FILECACHE_COMPLETE_CACHE_MISS)
    _IRQL_requires_max_(DISPATCH_LEVEL) void (
        *CSV_FILECACHE_COMPLETE_CACHE_MISS)(_Inout_ PVOID CallbackContext,
                                            _Inout_ PCSV_FILECACHE_CONTEXT
                                                CSVContext);

// Callback Provided by Csvfs

typedef _Function_class_(CSV_FILECACHE_HANDLE_CACHE_MISS)
    _IRQL_requires_max_(PASSIVE_LEVEL) LONG (*CSV_FILECACHE_HANDLE_CACHE_MISS)(
        _Inout_ PCSV_FILECACHE_CONTEXT CSVContext);

typedef _Function_class_(CSV_FILECACHE_COMPLETE_CACHE_IO)
    _IRQL_requires_max_(DISPATCH_LEVEL) void (*CSV_FILECACHE_COMPLETE_CACHE_IO)(
        _Inout_ PCSV_FILECACHE_CONTEXT CSVContext);
...
#pragma warning(                                                               \
    disable                                                                    \
    : 4201) // error C4201: nonstandard extension used : nameless struct/union
...
typedef _Function_class_(CSV_FILECACHE_HANDLE_CACHE_MISS_V2)
    _IRQL_requires_max_(PASSIVE_LEVEL) void (
        *CSV_FILECACHE_HANDLE_CACHE_MISS_V2)(
        _Inout_ PCSV_FILECACHE_IO_CONTEXT pContext, ULONG cRanges,
        CSV_FILECACHE_IO_RANGE const *pNotRequiredRanges);

typedef _Function_class_(CSV_FILECACHE_COMPLETE_CACHE_IO_V2)
    _IRQL_requires_max_(DISPATCH_LEVEL) void (
        *CSV_FILECACHE_COMPLETE_CACHE_IO_V2)(
        _Inout_ PCSV_FILECACHE_IO_CONTEXT pContext);
...
typedef struct _CSV_FILECACHE_FILE_CONTEXT_EXTENSION
    CSV_FILECACHE_FILE_CONTEXT_EXTENSION,
    *PCSV_FILECACHE_FILE_CONTEXT_EXTENSION;

typedef GUID FILE_SIG_T;
typedef GUID VOLUME_SIG_T;

typedef NTSTATUS (*CSV_FILECACHE_FILE_CREATE)(PVOID pRegContext,
                                              VOLUME_SIG_T const *volSig,
                                              FILE_SIG_T const *fileSig,
                                              PVOID *ppContext);
typedef void (*CSV_FILECACHE_FILE_CLOSE)(PVOID pRegContext, PVOID pContext);
typedef void (*CSV_FILECACHE_HANDLE_CACHE_IO_V2)(
    PVOID pRegContext, PCSV_FILECACHE_IO_CONTEXT pContext);
typedef void (*CSV_FILECACHE_COMPLETE_CACHE_MISS_V2)(
    PVOID pRegContext, PCSV_FILECACHE_IO_CONTEXT pContext);

typedef PCSV_FILECACHE_FILE_CONTEXT_EXTENSION (
    *CSV_FILECACHE_FILE_SET_EXTENSION)(
    PVOID pRegContext, PCSV_FILECACHE_IO_CONTEXT pContext,
    PCSV_FILECACHE_FILE_CONTEXT_EXTENSION pExtension);
typedef PCSV_FILECACHE_FILE_CONTEXT_EXTENSION (
    *CSV_FILECACHE_FILE_GET_EXTENSION)(PVOID pRegContext,
                                       PCSV_FILECACHE_IO_CONTEXT pContext);

typedef _Function_class_(CSV_FILECACHE_CLEANUP_CACHE)
    _IRQL_requires_max_(PASSIVE_LEVEL) void (*CSV_FILECACHE_CLEANUP_CACHE)(
        _Inout_ PVOID pRegContext);
