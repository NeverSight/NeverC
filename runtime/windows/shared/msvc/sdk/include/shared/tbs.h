...
TBS_RESULT WINAPI Tbsi_Context_Create(_In_ PCTBS_CONTEXT_PARAMS pContextParams,
                                      _Out_ PTBS_HCONTEXT phContext);

TBS_RESULT WINAPI Tbsi_Tpm_Vendor_Maintenance_Mode(
    _In_ PCTBS_CONTEXT_PARAMS pContextParams, _Out_ PTBS_HCONTEXT phContext);

TBS_RESULT WINAPI Tbsip_Context_Close(_In_ TBS_HCONTEXT hContext);

TBS_RESULT WINAPI Tbsip_Submit_Command(
    _In_ TBS_HCONTEXT hContext, _In_ TBS_COMMAND_LOCALITY Locality,
    _In_ TBS_COMMAND_PRIORITY Priority,
    _In_reads_bytes_(cbCommand) PCBYTE pabCommand, _In_ UINT32 cbCommand,
    _Out_writes_bytes_(*pcbResult) PBYTE pabResult, _Inout_ PUINT32 pcbResult);

TBS_RESULT WINAPI Tbsip_Cancel_Commands(_In_ TBS_HCONTEXT hContext);

TBS_RESULT WINAPI Tbsi_Physical_Presence_Command(
    _In_ TBS_HCONTEXT hContext, _In_reads_bytes_(cbInput) PCBYTE pabInput,
    _In_ UINT32 cbInput, _Out_writes_bytes_(*pcbOutput) PBYTE pabOutput,
...
_Success_(return == TBS_SUCCESS) TBS_RESULT WINAPI
    Tbsi_Get_TCG_Log(_In_ TBS_HCONTEXT hContext,
                     _Out_writes_bytes_to_opt_(*pOutputBufLen, *pOutputBufLen)
                         PBYTE pOutputBuf,
...
TBS_RESULT WINAPI Tbsi_GetDeviceInfo(_In_ UINT32 Size,
                                     _Out_writes_bytes_(Size) PVOID Info);

_Success_(return == TBS_SUCCESS) TBS_RESULT WINAPI
    Tbsi_Get_OwnerAuth(_In_ TBS_HCONTEXT hContext,
...
HRESULT
GetDeviceID(_Out_writes_bytes_to_opt_(cbWindowsAIK, *pcbResult)
                PBYTE pbWindowsAIK,
            _In_ UINT32 cbWindowsAIK, _Out_ PUINT32 pcbResult,
            _Out_opt_ BOOL *pfProtectedByTPM);

HRESULT
GetDeviceIDString(_Out_writes_to_opt_(cchWindowsAIK, *pcchResult)
                      PWSTR pszWindowsAIK,
                  _In_ UINT32 cchWindowsAIK, _Out_ PUINT32 pcchResult,
                  _Out_opt_ BOOL *pfProtectedByTPM);
...
TBS_RESULT WINAPI Tbsi_Create_Windows_Key(__in TBS_HANDLE keyHandle);
...
// (User mode only) Returns the SRTM log associated with the most recent clean
// boot
#define TBS_TCGLOG_SRTM_RESUME 3
// (User mode only) Returns the SRTM log associated with the current resume from
// hibernation
#define TBS_TCGLOG_DRTM_BOOT 4
// (User mode only) Returns the DRTM log associated with the most recent clean
// boot
#define TBS_TCGLOG_DRTM_RESUME 5
// (User mode only) Returns the DRTM log associated with the current resume from
// hibernation

_Success_(return == TBS_SUCCESS) TBS_RESULT WINAPI
    Tbsi_Get_TCG_Log_Ex(_In_ UINT32 logType,
...
inline BOOL WINAPI Tbsi_Is_Tpm_Present();
