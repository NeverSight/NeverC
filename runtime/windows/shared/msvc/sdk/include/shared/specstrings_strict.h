 ...
 *
 *  __in_awcount(expr, size) T* p : Pointer p is a buffer whose size may be
 * given in either bytes or elements. If <expr> is true, this acts like
...
#define __analysis_assume_nullterminated(x) __allowed(as_statement_with_arg(x))
...
#define __post_nullnullterminated __allowed(on_parameter_or_return)
/***************************************************************************
 * Expert Macros
 ***************************************************************************/
...
#define __$allowed_as_statement_with_arg(x)                                    \
  __pragma(warning(push)) __pragma(warning(disable : 4548)) do { __noop(x); }  \
...
#define DECL_SA(name, loc)                                                     \
  [repeatable][source_annotation_attribute(loc)] struct name##Attribute {      \
    name##Attribute();                                                         \
    const char *ignored;                                                       \
  };
#else
#define SA(x) SA_##x
#define DECL_SA(name, loc)                                                     \
  [source_annotation_attribute(loc)] struct name { const char *ignored; };     \
