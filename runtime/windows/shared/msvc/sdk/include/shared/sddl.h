...
#define SDDL_CALLBACK_OBJECT_ACCESS_ALLOWED                                    \
  TEXT("ZA")                          // Callback object access allowed
...
#define SDDL_ENTERPRISE_DOMAIN_CONTROLLERS                                     \
  TEXT("ED") // Enterprise domain controllers
...
#define SDDL_BUILTIN_ADMINISTRATORS                                            \
  TEXT("BA")                                  // Builtin (local ) administrators
...
#define SDDL_CERT_SERV_ADMINISTRATORS                                          \
  TEXT("CA")                                // Certificate Server Administrators
...
#define SDDL_ALIAS_PREW2KCOMPACC                                               \
  TEXT("RU")                          // alias to allow previous windows 2000
#define SDDL_LOCAL_SERVICE TEXT("LS") // Local service account (for services)
#define SDDL_NETWORK_SERVICE                                                   \
  TEXT("NS") // Network service account (for services)
#define SDDL_REMOTE_DESKTOP                                                    \
  TEXT("RD") // Remote desktop users (for terminal server)
#define SDDL_NETWORK_CONFIGURATION_OPS                                         \
  TEXT("NO") // Network configuration operators ( to manage configuration of
             // networking features)
...
#define SDDL_ENTERPRISE_RO_DCs                                                 \
  TEXT("RO") // Enterprise Read-only domain controllers
#define SDDL_CERTSVC_DCOM_ACCESS                                               \
  TEXT("CD") // Users who can connect to certification authorities using DCOM
#define SDDL_ALL_APP_PACKAGES                                                  \
  TEXT("AC") // All applications running in an app package context
#define SDDL_RDS_REMOTE_ACCESS_SERVERS                                         \
  TEXT("RA") // Servers in this group enable users of RemoteApp programs and
             // personal virtual desktops access to these resources.
#define SDDL_RDS_ENDPOINT_SERVERS                                              \
  TEXT("ES") // Servers in this group run virtual machines and host sessions
             // where users RemoteApp programs and personal virtual desktops
             // run.
#define SDDL_RDS_MANAGEMENT_SERVERS                                            \
  TEXT("MS") // Servers in this group can perform routine administrative actions
             // on servers running Remote Desktop Services.
#define SDDL_USER_MODE_DRIVERS TEXT("UD") // UserMode driver
#define SDDL_HYPER_V_ADMINS                                                    \
  TEXT("HA") // Members of this group have complete and unrestricted access to
             // all features of Hyper-V.
#define SDDL_CLONEABLE_CONTROLLERS                                             \
  TEXT("CN") // Members of this group that are domain controllers may be cloned.
#define SDDL_ACCESS_CONTROL_ASSISTANCE_OPS                                     \
  TEXT("AA") // Members of this group can remotely query authorization
             // attributes and permissions for resources on this computer.
#define SDDL_REMOTE_MANAGEMENT_USERS                                           \
  TEXT("RM") // Members of this group can access WMI resources over management
             // protocols (such as WS-Management via the Windows Remote
             // Management service). This applies only to WMI namespaces that
...
#define SDDL_PROTECTED_USERS                                                   \
  TEXT("AP") // Members of this group are afforded additional protections
             // against authentication security threats.
#define SDDL_KEY_ADMINS                                                        \
  TEXT("KA") // Members of this group have full control over all key credential
             // objects in the domain
#define SDDL_ENTERPRISE_KEY_ADMINS                                             \
  TEXT("EK") // Members of this group have full control over all key credential
             // objects in the forest
#define SDDL_USER_MODE_HARDWARE_OPERATORS                                      \
  TEXT("HO") // Members of this group may operate hardware from user mode
#define SDDL_OPENSSH_USERS                                                     \
  TEXT("SH") // Members of this group may connect to this computer using SSH
...
_Success_(return != FALSE) BOOL WINAPI
    ConvertSidToStringSidA(_In_ PSID Sid, _Outptr_ LPSTR *StringSid);

_Success_(return != FALSE) BOOL WINAPI
    ConvertSidToStringSidW(_In_ PSID Sid, _Outptr_ LPWSTR *StringSid);
...
_Success_(return != FALSE) BOOL WINAPI
    ConvertStringSidToSidA(_In_ LPCSTR StringSid, _Outptr_ PSID *Sid);

_Success_(return != FALSE) BOOL WINAPI
    ConvertStringSidToSidW(_In_ LPCWSTR StringSid, _Outptr_ PSID *Sid);
...
_Success_(return != FALSE) BOOL WINAPI
    ConvertStringSecurityDescriptorToSecurityDescriptorA(
        _In_ LPCSTR StringSecurityDescriptor, _In_ DWORD StringSDRevision,
        _Outptr_ PSECURITY_DESCRIPTOR *SecurityDescriptor,
        _Out_opt_ PULONG SecurityDescriptorSize);

_Success_(return != FALSE) BOOL WINAPI
    ConvertStringSecurityDescriptorToSecurityDescriptorW(
        _In_ LPCWSTR StringSecurityDescriptor, _In_ DWORD StringSDRevision,
        _Outptr_ PSECURITY_DESCRIPTOR *SecurityDescriptor,
        _Out_opt_ PULONG SecurityDescriptorSize);
