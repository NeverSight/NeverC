...
#pragma warning(                                                               \
    error : 4703) // Local pointer potentially used used w/o being initialized
#pragma warning(                                                               \
    error : 4754) // Bad integer overflow checks warning (ineffective check)
...
#pragma warning(disable : 4786) // identifier was truncated to 255 chararcers in
                                // debug information.
#pragma warning(disable                                                        \
                : 4503) // decorated name length exceeded, name was truncated.
#pragma warning(disable                                                        \
                : 4263) // Derived override doesn't match base - who cares...
...
#pragma warning(                                                               \
    error : 4552) // <<, >> ops used to no effect (probably missing an = sign)
...
// expected operator with side-effect #pragma warning(3:4548)   // expression
// before comma has no effect; expected expression with side-effect

#pragma warning(disable : 4306) // conversion from smaller to greater size

#if _MSC_VER > 1300
#pragma warning(                                                               \
    disable : 4356) // static member cannot be initialized via derived class
#endif
...
#pragma warning(error                                                          \
                : 4028) // C4028: formal parameter different from declaration
#pragma warning(                                                               \
    error                                                                      \
    : 4029) // C4029: declared formal parameter list different from definition
...
// DONT_REDEFINE_TRYEXCEPT_IN_WARNING_H allows C files to opt-out of getting
// these symbols redefined so that they can be used as regular C identifiers for
// variables
#undef try
#undef except
#undef finally
...
#pragma warning(disable                                                        \
                : 4068) // turn off unknown pragma warning so prefast pragmas
                        // won't show show up in build.wrn/build.err
...
#pragma warning(disable : 4603) // macro is not defined or definition is
                                // different after precompiled header use
#pragma warning(disable                                                        \
                : 4627) // skipped when looking for precompiled header use
#endif
#endif

#ifdef _PREFAST_
#pragma warning(disable                                                        \
                : 4091) // (If prefast only): annotations followed by ; are
                        // ignored (actual text is not that clear).
#endif
