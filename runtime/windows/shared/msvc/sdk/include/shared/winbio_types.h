...
typedef WINBIO_SESSION_HANDLE WINBIO_FRAMEWORK_HANDLE,
    *PWINBIO_FRAMEWORK_HANDLE;
...
const WINBIO_IDENTITY_TYPE WINBIO_ID_TYPE_NULL =
    (WINBIO_IDENTITY_TYPE)0; // The Identity structure is empty.
const WINBIO_IDENTITY_TYPE WINBIO_ID_TYPE_WILDCARD =
    (WINBIO_IDENTITY_TYPE)1; // The Identity matches "all identities".
const WINBIO_IDENTITY_TYPE WINBIO_ID_TYPE_GUID =
    (WINBIO_IDENTITY_TYPE)2; // A GUID identifies the template.
const WINBIO_IDENTITY_TYPE WINBIO_ID_TYPE_SID =
    (WINBIO_IDENTITY_TYPE)3; // An account SID identifies the template.
const WINBIO_IDENTITY_TYPE WINBIO_ID_TYPE_SECURE_ID =
    (WINBIO_IDENTITY_TYPE)4; // A secure ID identifies the template.
...
#define WINBIO_ID_TYPE_NULL                                                    \
  ((WINBIO_IDENTITY_TYPE)0) // The Identity structure is empty.
#define WINBIO_ID_TYPE_WILDCARD                                                \
  ((WINBIO_IDENTITY_TYPE)1) // The Identity matches "all identities"
#define WINBIO_ID_TYPE_GUID                                                    \
  ((WINBIO_IDENTITY_TYPE)2) // A GUID identifies the template.
#define WINBIO_ID_TYPE_SID                                                     \
  ((WINBIO_IDENTITY_TYPE)3) // An account SID identifies the template.
#define WINBIO_ID_TYPE_SECURE_ID                                               \
  ((WINBIO_IDENTITY_TYPE)4) // A secure ID identifies the template.
...
const WINBIO_BIOMETRIC_TYPE WINBIO_STANDARD_TYPE_MASK =
    (WINBIO_BIOMETRIC_TYPE)0x00FFFFFF;

const WINBIO_BIOMETRIC_TYPE WINBIO_NO_TYPE_AVAILABLE =
    (WINBIO_BIOMETRIC_TYPE)0x00000000;

//
// Standard biometric types (from NISTIR 6529-A)
//
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_MULTIPLE =
    (WINBIO_BIOMETRIC_TYPE)0x00000001;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_FACIAL_FEATURES =
    (WINBIO_BIOMETRIC_TYPE)0x00000002;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_VOICE =
    (WINBIO_BIOMETRIC_TYPE)0x00000004;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_FINGERPRINT =
    (WINBIO_BIOMETRIC_TYPE)0x00000008;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_IRIS =
    (WINBIO_BIOMETRIC_TYPE)0x00000010;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_RETINA =
    (WINBIO_BIOMETRIC_TYPE)0x00000020;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_HAND_GEOMETRY =
    (WINBIO_BIOMETRIC_TYPE)0x00000040;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_SIGNATURE_DYNAMICS =
    (WINBIO_BIOMETRIC_TYPE)0x00000080;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_KEYSTROKE_DYNAMICS =
    (WINBIO_BIOMETRIC_TYPE)0x00000100;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_LIP_MOVEMENT =
    (WINBIO_BIOMETRIC_TYPE)0x00000200;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_THERMAL_FACE_IMAGE =
    (WINBIO_BIOMETRIC_TYPE)0x00000400;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_THERMAL_HAND_IMAGE =
    (WINBIO_BIOMETRIC_TYPE)0x00000800;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_GAIT =
    (WINBIO_BIOMETRIC_TYPE)0x00001000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_SCENT =
    (WINBIO_BIOMETRIC_TYPE)0x00002000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_DNA = (WINBIO_BIOMETRIC_TYPE)0x00004000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_EAR_SHAPE =
    (WINBIO_BIOMETRIC_TYPE)0x00008000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_FINGER_GEOMETRY =
    (WINBIO_BIOMETRIC_TYPE)0x00010000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_PALM_PRINT =
    (WINBIO_BIOMETRIC_TYPE)0x00020000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_VEIN_PATTERN =
    (WINBIO_BIOMETRIC_TYPE)0x00040000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_FOOT_PRINT =
    (WINBIO_BIOMETRIC_TYPE)0x00080000;
//
// WinBio extended types
//
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_OTHER =
    (WINBIO_BIOMETRIC_TYPE)0x40000000;
const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_PASSWORD =
    (WINBIO_BIOMETRIC_TYPE)0x80000000;

const WINBIO_BIOMETRIC_TYPE WINBIO_TYPE_ANY =
    (WINBIO_BIOMETRIC_TYPE)(WINBIO_STANDARD_TYPE_MASK | WINBIO_TYPE_OTHER |
...
//
typedef ULONG WINBIO_BIOMETRIC_SENSOR_SUBTYPE,
    *PWINBIO_BIOMETRIC_SENSOR_SUBTYPE;

#define WINBIO_SENSOR_SUBTYPE_UNKNOWN                                          \
  ((WINBIO_BIOMETRIC_SENSOR_SUBTYPE)0x00000000)

#define WINBIO_FP_SENSOR_SUBTYPE_SWIPE                                         \
  ((WINBIO_BIOMETRIC_SENSOR_SUBTYPE)0x00000001)
#define WINBIO_FP_SENSOR_SUBTYPE_TOUCH                                         \
  ((WINBIO_BIOMETRIC_SENSOR_SUBTYPE)0x00000002)
...
//  - A (single) enumerated value explaining the reason for the rejection.
//  [0x0000FFFF]
//
...
// WINBIO_FACE_OCCLUDED indicates that the user's eyes, mouth or nose are
// occluded. This is create to replace WINBIO_FACE_EYES_OCCLUDED
...
#define WINBIO_BIR_FIELD_NEVER_VALID                                           \
  (WINBIO_BIR_FIELD_SUBHEAD_COUNT | WINBIO_BIR_FIELD_PATRON_ID |               \
   WINBIO_BIR_FIELD_INDEX | WINBIO_BIR_FIELD_CHALLENGE |                       \
...
//
#define WINBIO_ANSI_381_FORMAT_OWNER                                           \
  ((USHORT)0x001B) // INCITS Technical Committee M1
...
typedef struct _WINBIO_BDB_ANSI_381_HEADER {
  ...
  USHORT Reserved;
  // 38 bytes (because of unused 2 bytes in ULONG64) vs. 36 bytes defined in
  // ANSI INCITS spec
...
// can be any value between WINBIO_VOICE_MIN_UTTERANCE and
// WINBIO_VOICE_MAX_UTTERANCE. After enrollment, WinBioIdentify will return the
// utterance ID through the SubFactor return value.
//
...
#define WINBIO_POOL_UNASSIGNED                                                 \
  ((WINBIO_POOL_TYPE)3) // Reserved for Microsoft - do not use.
...
#define WINBIO_FLAG_BASIC                                                      \
  ((WINBIO_SESSION_FLAGS)((WINBIO_SENSOR_BASIC_MODE & 0xFFFF) << 16))
#define WINBIO_FLAG_ADVANCED                                                   \
  ((WINBIO_SESSION_FLAGS)((WINBIO_SENSOR_ADVANCED_MODE & 0xFFFF) << 16))
//
...
// Fingerprint events.
//
const WINBIO_EVENT_TYPE WINBIO_EVENT_FP_UNCLAIMED =
    (WINBIO_EVENT_TYPE)0x00000001;
const WINBIO_EVENT_TYPE WINBIO_EVENT_FP_UNCLAIMED_IDENTIFY =
    (WINBIO_EVENT_TYPE)0x00000002;
...
#define WINBIO_OPAQUE_ENGINE_DATA_ITEM_COUNT                                   \
  ((ULONG)78) // Number of ULONG slots in the array
...
typedef[switch_type(WINBIO_BIOMETRIC_TYPE)] union _WINBIO_PRESENCE_PROPERTIES {
  ...
  [case (WINBIO_NO_TYPE_AVAILABLE)] ULONG32 Null;

  [case (WINBIO_TYPE_FACIAL_FEATURES)] struct {
    RECT BoundingBox;
    LONG Distance;
    struct {
...
#define WINBIO_FRAMEWORK_CHANGE_UNIT_STATUS                                    \
  ((WINBIO_FRAMEWORK_CHANGE_TYPE)0x00000002)
...
//      PropertyBufferSize - points to a SIZE_T variable containing
//      sizeof(ULONG)
//
...
//
// Outputs:
//      PropertyBuffer - points to a buffer containing a
//      WINBIO_EXTENDED_SENSOR_INFO structure PropertyBufferSize - points to a
//      SIZE_T variable containing sizeof(WINBIO_EXTENDED_SENSOR_INFO)
//
...
typedef struct _WINBIO_EXTENDED_SENSOR_INFO {
  ...
  [switch_is(Factor)] union {
    ...
    [case (WINBIO_TYPE_FACIAL_FEATURES)] struct {
      RECT FrameSize; // Camera frame size, in pixels; (top, left) fields always
                      // (0,0)
      POINT FrameOffset; // Offset of face camera frame from video camera, in
                         // pixels. (0, 0) indicates complete overlap
    ...
    [case (WINBIO_TYPE_FINGERPRINT)] struct {
      ULONG32 Reserved; // Reserved for future expansion
    } Fingerprint;

    [case (WINBIO_TYPE_IRIS)] struct {
      RECT FrameSize; // Iris camera frame size, in pixels; (top, left) fields
                      // always (0,0)
      POINT FrameOffset; // Offset of iris camera frame from video camera, in
                         // pixels. (0, 0) indicates complete overlap
      WINBIO_ORIENTATION MandatoryOrientation;
    } Iris;

    [case (WINBIO_TYPE_VOICE)] struct {
      ULONG32 Reserved; // Reserved for future expansion
    } Voice;
  } Specific;
...
// Outputs:
//      PropertyBuffer - points to a buffer containing a
//      WINBIO_EXTENDED_ENGINE_INFO structure PropertyBufferSize - points to a
//      SIZE_T variable containing sizeof(WINBIO_EXTENDED_ENGINE_INFO)
//
...
// Outputs:
//      PropertyBuffer - points to a buffer containing a
//      WINBIO_EXTENDED_STORAGE_INFO structure PropertyBufferSize - points to a
//      SIZE_T variable containing sizeof(WINBIO_EXTENDED_STORAGE_INFO)
//
...
// Outputs:
//      PropertyBuffer - points to a buffer containing a
//      WINBIO_EXTENDED_ENROLLMENT_STATUS structure PropertyBufferSize - points
//      to a SIZE_T variable containing
//      sizeof(WINBIO_EXTENDED_ENROLLMENT_STATUS)
//
...
// Outputs:
//      PropertyBuffer - points to a buffer containing a
//      WINBIO_EXTENDED_UNIT_STATUS structure PropertyBufferSize - points to a
//      SIZE_T variable containing sizeof(WINBIO_EXTENDED_UNIT_STATUS)
//
...
//
// Outputs:
//      PropertyBuffer - points to a WINBIO_UNIT_SECURITY_LEVEL buffer
//      containing the level PropertyBufferSize - points to a SIZE_T variable
//      containing sizeof(WINBIO_UNIT_SECURITY_LEVEL)
//
...
//
// Outputs:
//      PropertyBuffer - points to buffer containing a WINBIO_FP_BU_STATE
//      structure PropertyBufferSize - points to a SIZE_T variable containing
//      sizeof(WINBIO_FP_BU_STATE)
//
...
// Outputs:
//      PropertyBuffer - points to a buffer containing a
//      WINBIO_ANTI_SPOOF_POLICY structure PropertyBufferSize - points to a
//      SIZE_T variable containing sizeof(WINBIO_ANTI_SPOOF_POLICY)
//
...
typedef enum _WINBIO_ANTI_SPOOF_POLICY_ACTION {
  ...
  WINBIO_ANTI_SPOOF_REMOVE = 0x00000002,
    ...
    *PWINBIO_ANTI_SPOOF_POLICY_ACTION;
...
typedef struct _WINBIO_EXTENDED_ENROLLMENT_PARAMETERS {
  ...
  WINBIO_BIOMETRIC_SUBTYPE SubFactor;
    ...
    *PWINBIO_EXTENDED_ENROLLMENT_PARAMETERS;
...
#define WINBIO_PROTECTION_TRUSTED_EXECUTION_ENVIRONMENT                        \
  ((WINBIO_PROTECTION_TYPE)2)
...
enum WINBIO_ESS_STATE_FLAGS {
  ...
  // On this PC, ESS is disabled either using a "Enhanced Sign-in Security"
  // toggle in Accounts -> Sign-in options under "Additional settings" or using
  // a policy. By default following flag will not be set.
  WINBIO_ESS_REQUIRES_ENABLEMENT = 0x00000040,

  // On this PC, ESS is managed using a policy available through group policy or
  ...
  // This PC contains enrollments performed using biometric sensors that are not
  // ESS-capable.
  WINBIO_ESS_REQUIRES_NON_VBS_BIOMETRIC_ENROLLMENT_ABSENCE = 0x00000100,

  // This PC does not have enrollments performed using ESS capable biometric
  // sensors.
