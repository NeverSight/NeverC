...
Abstract:
    ...
    This module contains definitions related to the management of API
namespaces.
...
#ifdef _API_SET_HOST
// This section is intended for hosting contracts. Any legacy host definitions
// are ignored.
...
#define API_SET_LEGACY_BY_ORDINAL_DIR(X, L, O, PO, DIR)                        \
  X @##O NONAME DIR PRIVATE
#define API_SET_LEGACY_WITH_OPTIONAL_ORDINAL(X, L, O, PO) X @##O PRIVATE
#define API_SET_LEGACY_WITH_OPTIONAL_ORDINAL_DIR(X, L, O, PO, DIR)             \
  X @##O DIR PRIVATE

#else

#if defined(_API_SET_LEGACY_TARGET)
// This section is intended for "classic apiset forwarders" that are forwarding
// back to a legacy host. In this section, the definition for
// _API_SET_LEGACY_TARGET is taken as the default legacy host for the entire
// contract. But any APIs which specify LegacyHost directly will override this
// definition.

#define API_SET(X) X = _API_SET_LEGACY_TARGET##.##X
#define API_SET_DIR(X, DIR) X = _API_SET_LEGACY_TARGET##.##X DIR
#define API_SET_BY_ORDINAL(X, O, PO)                                           \
  X = _API_SET_LEGACY_TARGET##.##PO @##O NONAME
#define API_SET_BY_ORDINAL_DIR(X, O, PO, DIR)                                  \
  X = _API_SET_LEGACY_TARGET##.##PO @##O NONAME DIR
#define API_SET_WITH_OPTIONAL_ORDINAL(X, O, PO)                                \
  X = _API_SET_LEGACY_TARGET##.##PO @##O
#define API_SET_WITH_OPTIONAL_ORDINAL_DIR(X, O, PO, DIR)                       \
  X = _API_SET_LEGACY_TARGET##.##PO @##O DIR
...
// This section is intended for "reverse forwarders" that are forwarding to an
// explicit apiset name. Any legacy targets are ignored.
#define API_SET(X) X = _API_SET_FORWARDER_TARGET##.##X
#define API_SET_DIR(X, DIR) X = _API_SET_FORWARDER_TARGET##.##X DIR
#define API_SET_BY_ORDINAL(X, O, PO)                                           \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O NONAME
#define API_SET_BY_ORDINAL_DIR(X, O, PO, DIR)                                  \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O NONAME DIR
#define API_SET_WITH_OPTIONAL_ORDINAL(X, O, PO)                                \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O
#define API_SET_WITH_OPTIONAL_ORDINAL_DIR(X, O, PO, DIR)                       \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O DIR
...
#define API_SET_LEGACY_BY_ORDINAL(X, L, O, PO)                                 \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O NONAME
#define API_SET_LEGACY_BY_ORDINAL_DIR(X, L, O, PO, DIR)                        \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O NONAME DIR
#define API_SET_LEGACY_WITH_OPTIONAL_ORDINAL(X, L, O, PO)                      \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O
#define API_SET_LEGACY_WITH_OPTIONAL_ORDINAL_DIR(X, L, O, PO, DIR)             \
  X = _API_SET_FORWARDER_TARGET##.##PO @##O DIR

#else
// This section is intended for internal contract generation to build artifacts
// that model the contract without forwarding.
