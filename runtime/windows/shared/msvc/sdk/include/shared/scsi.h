...
//
// For user-mode application development, make sure to add the line "#define
// _NTSCSI_USER_MODE_" prior to "#include <scsi.h>"
//
// For example,
//
...
//
// Also see the SPTI sample (located in src\storage\tools\spti directory under
// the Windows Kits root directory)
//
#error                                                                         \
    "For user-mode application development, make sure to #define _NTSCSI_USER_MODE_ prior to #include <scsi.h>"
...
//
//      if (RTL_CONTAINS_FIELD(pBlock, pBlock->cbSize, dwMumble)) { // safe to
//      use pBlock->dwMumble
//
...
typedef union _CDB {
  ...
  struct _REPORT_TIMESTAMP {
    ...
    UCHAR ServiceAction
        : 5;             // Byte  1, bit 0-4 : SERVICE_ACTION_REPORT_TIMESTAMP
  ...
  struct _REPORT_SUPPORTED_OPERATION_CODES {
    ...
    UCHAR ServiceAction
        : 5; // 0x0C SERVICE_ACTION_REPORT_SUPPORTED_OPERATION_CODES
    UCHAR Reserved0 : 3;
  ...
  struct _GET_PHYSICAL_ELEMENT_STATUS {
    ...
    UCHAR ServiceAction
        : 5; // 0x17 - SERVICE_ACTION_GET_PHYSICAL_ELEMENT_STATUS
    UCHAR Reserved1 : 3;
    UCHAR Reserved2[4];
  ...
  struct _REMOVE_ELEMENT_AND_TRUNCATE {
    ...
    UCHAR ServiceAction
        : 5; // 0x18 - SERVICE_ACTION_REMOVE_ELEMENT_AND_TRUNCATE
    UCHAR Reserved1 : 3;
...
#define NOTIFICATION_EXTERNAL_EVENT_EXTERNAL                                   \
  0x3 // respond with GET_CONFIGURATION?
...
// #pragma deprecated(_DISK_INFORMATION)  // Use DISC_INFORMATION, note size
// change #pragma deprecated( DISK_INFORMATION)  // Use DISC_INFORMATION, note
// size change #pragma deprecated(PDISK_INFORMATION)  // Use DISC_INFORMATION,
// note size change #endif
...
#define SCSIOP_ZBC_OUT                                                         \
  0x94 // Close Zone, Finish Zone, Open Zone, Reset Write Pointer, etc.
...
typedef struct _VPD_THIRD_PARTY_COPY_PAGE {
  ...
  UCHAR DeviceTypeQualifier : 3;
  UCHAR PageCode;      // 0x8F
  UCHAR PageLength[2]; // at least 0x24 if target supports Windows Offload Data
                       // Transfers
#if !defined(__midl)
...
//
// Optional VERSION DESCRIPTOR fields provide the opportunity for SCSI targets
// to claim conformance with a number of standards. The INQUIRY response can
// contain any number of version descriptors between one and eight. Below values
// are excerpted from table 136 of SPC-4 specification available at
// http://www.t10.org (backup copies maintained at http://www.incits.org and //
// http://www.ansi.org).
//
...
typedef struct {
  ...
  // we use PRI_FULL_STATUS_DESCRIPTOR_HEADER rather than
  // PRI_FULL_STATUS_DESCRIPTOR as a place holder here.
  //
...
typedef enum _TRANSFER_COUNT_UNITS {  // Multiplier to convert a Transfer Count
                                      // field to bytes
  ...
  TRANSFER_COUNT_UNITS_NUMBER_BLOCKS =
      0xF1 // Logical Block Length In Bytes (from ReadCapacity)
...
//
#define SCSI_WRITE_BUFFER_MODE_DOWNLOAD_MICROCODE_WITH_OFFSETS_SAVE_DEFER_ACTIVATE \
  0x0E
...
#define MAX_ADDITIONAL_SENSE_BYTES_EX                                          \
...
//
// Data set management IOCTL to match DSM notifications. Lba Ranges carried by
// this IOCTL belong to the same file. This IOCTL carries SRB_IO_CONTROL and
// DSM_NOTIFICATION_REQUEST_BLOCK as part of input parameters.
//
#define IOCTL_SCSI_MINIPORT_DSM ((FILE_DEVICE_SCSI << 16) + 0x0720)

//
// Data set management IOCTL sent to miniport driver. Lba Ranges carried by this
// IOCTL may cross different files or do not belong to file. This IOCTL carries
// SRB_IO_CONTROL and DEVICE_MANAGE_DATA_SET_ATTRIBUTES as part of input
// parameters. NOTE that when construct input buffer, padding place may be
// needed between SRB_IO_CONTROL and DEVICE_MANAGE_DATA_SET_ATTRIBUTES to make
// sure DEVICE_MANAGE_DATA_SET_ATTRIBUTES is pointer safe. e.g. input buffer
// layout should be: ALIGN_UP(sizeof(SRB_IO_CONTROL), PVOID) +
// DEVICE_MANAGE_DATA_SET_ATTRIBUTES.
//      (Parameter Block and DataSet Ranges will be indicated by fields in
//      DEVICE_MANAGE_DATA_SET_ATTRIBUTES)
//
...
C_ASSERT(sizeof(MODE_COMMAND_DURATION_LIMIT_PAGE_T2A_SUBPAGE) ==
         (0xE4 + sizeof(MODE_PAGE_SUBPAGE_HEADER)));
...
C_ASSERT(sizeof(MODE_COMMAND_DURATION_LIMIT_PAGE_T2B_SUBPAGE) ==
         (0xE4 + sizeof(MODE_PAGE_SUBPAGE_HEADER)));
...
// so the pointer fields within the variable length components are 8 bytes
// align.
//
...
// dependencies on the legacy SCSI infrastructure.  If there are any future
// updates to scsi.h the following values should be treated as reserved.
//
...
} SES_ADDITIONAL_ELEMENT_STATUS_DESCRIPTOR,
    *PSES_ADDITIONAL_ELEMENT_STATUS_DESCRIPTOR;
...
} SES_ADDITIONAL_ELEMENT_STATUS_DIAGNOSTIC_PAGE,
    *PSES_ADDITIONAL_ELEMENT_STATUS_DIAGNOSTIC_PAGE;
...
//
// Definitions related to 0x9B - SCSIOP_READ_DATA_BUFF16(Mode 0x1C: Error
// History)
//
...
//
#define BUFFER_ID_RETURN_ERROR_HISTORY_DIRECTORY_CREATE_NEW_ERROR_HISTORY_SNAPSHOT \
  0x1
...
//
#define BUFFER_ID_RETURN_ERROR_HISTORY_DIRECTORY_ESTABLISH_NEW_NEXUS_AND_SNAPSHOT \
  0x3
...
//
// The error history I_T nexus has requested buffer ID FEh (i.e., clear error
// history I_T nexus) or buffer ID FFh (i.e., clear error history I_T nexus and
// release snapshot) for the current error history snapshot.
//
#define ERROR_HISTORY_RETRIEVED_BUFFER_ID_FE_OR_FF 0x1

//
// An error history I_T nexus has not requested buffer ID FEh (i.e., clear error
// history I_T nexus) or buffer ID FFh (i.e., clear error history I_T nexus and
// release snapshot) for the current error history snapshot.
//
...
//
// Determine if sense data format indicated in sense data payload is a valid
// value
//
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN
    ScsiGetTotalSenseByteCountIndicated(_In_reads_bytes_(SenseInfoBufferLength)
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN
    ScsiGetFixedSenseKeyAndCodes(_In_reads_bytes_(SenseInfoBufferLength)
                                     PVOID SenseInfoBuffer,
                                 _In_ UCHAR SenseInfoBufferLength,
...
Arguments:
    ...
    AdditionalSenseCode
      - On output, buffer contains the additional sense code.
        If null is specified, the function will not retrieve the additional
sense code.
...
--*/
{
  ...
  if (RTL_CONTAINS_FIELD(fixedSenseData, SenseInfoBufferLength,
    ...
    dataLength =
        fixedSenseData->AdditionalSenseLength +
        RTL_SIZEOF_THROUGH_FIELD(FIXED_SENSE_DATA, AdditionalSenseLength);
    ...
    if (AdditionalSenseCode != NULL) {
      *AdditionalSenseCode =
          RTL_CONTAINS_FIELD(fixedSenseData, dataLength, AdditionalSenseCode)
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN
    ScsiGetDescriptorSenseKeyAndCodes(
        ...
        _In_ UCHAR SenseInfoBufferLength, _Out_opt_ PUCHAR SenseKey,
        _Out_opt_ PUCHAR AdditionalSenseCode,
        _Out_opt_ PUCHAR AdditionalSenseCodeQualifier)
/*++
...
--*/
{
  PDESCRIPTOR_SENSE_DATA descriptorSenseData =
      (PDESCRIPTOR_SENSE_DATA)SenseInfoBuffer;
  BOOLEAN succeed = FALSE;
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN
    ScsiGetSenseKeyAndCodes(_In_reads_bytes_(SenseInfoBufferLength)
                                PVOID SenseInfoBuffer,
                            _In_ UCHAR SenseInfoBufferLength,
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN
    ScsiGetSenseDescriptor(_In_reads_bytes_(SenseInfoBufferLength)
                               PVOID SenseInfoBuffer,
                           _In_ UCHAR SenseInfoBufferLength,
                           _Outptr_result_bytebuffer_(*DescriptorBufferLength)
  ...
  *DescriptorBufferLength = 0;
  ...
  if (RTL_CONTAINS_FIELD(descriptorSenseData, SenseInfoBufferLength,
    ...
    if (descriptorSenseData->AdditionalSenseLength <=
      ...
      *DescriptorBufferLength =
          dataLength - RTL_SIZEOF_THROUGH_FIELD(DESCRIPTOR_SENSE_DATA,
                                                AdditionalSenseLength);
...
Description:
    ...
    This function validates if buffer contains a valid payload for descriptor of
Information type
...
Returns:
    ...
    TRUE if DescriptorBuffer contains valid payload for Information type
descriptor.
...
--*/
{
  ...
  if (descriptor->Header.DescriptorType !=
      SCSI_SENSE_DESCRIPTOR_TYPE_INFORMATION) {
    return FALSE;
  }

  additionalLength =
      sizeof(SCSI_SENSE_DESCRIPTOR_INFORMATION) -
...
Description:
    ...
    This function validates if buffer contains a valid payload for Block Command
type descriptor
...
Returns:
    ...
    TRUE if DescriptorBuffer contains a valid payload for descriptor of Block
Command type
...
--*/
{
  ...
  additionalLength =
      sizeof(SCSI_SENSE_DESCRIPTOR_BLOCK_COMMAND) -
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN
    ScsiConvertToFixedSenseFormat(_In_reads_bytes_(SenseInfoBufferLength)
                                      PVOID SenseInfoBuffer,
                                  _In_ UCHAR SenseInfoBufferLength,
...
Description:
    ...
    This routine converts descriptor format sense data to fixed format sense
data.

    Due to differences between two formats, the conversion is only based on
Sense Key, Additional Sense Code, and Additional Sense Code Qualififer.
...
Arguments:
    ...
    OutBuffer
      - On output, OutBuffer contains the fixed sense data as result of
conversion.
...
--*/
{
  ...
  if (SenseInfoBuffer == NULL || SenseInfoBufferLength == 0 ||
      OutBuffer == NULL || OutBufferLength < sizeof(FIXED_SENSE_DATA)) {
    return FALSE;
  }
...
_Success_(return != FALSE) FORCEINLINE BOOLEAN ScsiGetNextSenseDescriptorByType(
    _In_reads_bytes_(BufferLength) PVOID Buffer, _In_ UCHAR BufferLength,
    _In_reads_(TypeListCount) PUCHAR TypeList, _In_ ULONG TypeListCount,
...
--*/
{
  ...
  if (Buffer == NULL || BufferLength == 0 || TypeList == NULL ||
      TypeListCount == 0 || OutType == NULL || OutBuffer == NULL ||
