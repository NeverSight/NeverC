...
#if defined(i286) || defined(_X86_) || defined(_AMD64_) || defined(_IA64_) ||  \
...
__inline DL_ADDRESS_TYPE EthernetAddressType(_In_reads_(sizeof(DL_EUI48))
                                                 CONST UCHAR *Address) {
...
#pragma warning(default                                                        \
                : 4001) /* nonstandard extension : single line comment */
#pragma warning(default : 4200) /* nonstandard extension used : zero-sized     \
                                   array in struct/union */
#pragma warning(                                                               \
...
#define IPV4_DEFAULT_VERHLEN                                                   \
  ((IPV4_VERSION_BYTE) | (sizeof(IPV4_HEADER) / sizeof(UINT32)))
...
//
__inline UCHAR
Ip4HeaderLengthInBytes(_In_ CONST UNALIGNED IPV4_HEADER *Header) {
...
__inline UINT16 Ip4FragmentOffset(_In_ CONST UNALIGNED IPV4_HEADER *Header) {
...
typedef enum { ARP_REQUEST = 1, ARP_RESPONSE = 2 } ARP_OPCODE;
...
typedef enum { ARP_HW_ENET = 1, ARP_HW_802 = 6 } ARP_HARDWARE_TYPE;
...
__inline UINT16
Ip6FragmentOffset(_In_ CONST UNALIGNED IPV6_FRAGMENT_HEADER *Header) {
...
typedef _Struct_size_bytes_(
    _Inexpressible_(Length)) struct _IPV6_ROUTING_HEADER {
  UCHAR NextHeader;
...
__inline UINT32 Ipv6pGetVersionClassEcnFlow(IN UINT8 Class, IN UINT8 EcnField,
                                            IN UINT32 Flow) {
  //
...
__inline UINT32 In6IsEmbeddedV4AddrPrefixLengthValid(_In_ ULONG PrefixLength) {
  //
...
_Success_(return) __inline UINT32
    In6ExtractEmbeddedV4AddrFromV6(_In_ const IN6_ADDR *Ipv6Address,
  ...
  if (PrefixLength < 96) {
    ...
    RtlMoveMemory((((PUCHAR)&TmpAddress) + IN6_EMBEDDEDV4_UOCTET_POSITION),
...
_Success_(return) __inline UINT32
    In6SetAddrV4Embedded(_In_ const IN6_ADDR *Prefix, _In_ ULONG PrefixLength,
  ...
  RtlCopyMemory((((PUCHAR)Ipv6Address) + PrefixByteLength), Ipv4Address,
                sizeof(IN_ADDR));

  if (PrefixLength < 96) {
    RtlZeroMemory((((PUCHAR)Ipv6Address) + PrefixByteLength + sizeof(IN_ADDR)),
                  sizeof(IN6_ADDR) - (PrefixByteLength + sizeof(IN_ADDR)));
    RtlMoveMemory((((PUCHAR)Ipv6Address) + IN6_EMBEDDEDV4_UOCTET_POSITION + 1),
...
__inline BOOLEAN NmrIsEqualNpiModuleId(_In_ PNPI_MODULEID ModuleId1,
  ...
  if (ModuleId1->Type == ModuleId2->Type) {
    if (ModuleId1->Type == MIT_GUID) {
      ...
      return (BOOLEAN)RtlEqualMemory(&ModuleId1->IfLuid, &ModuleId2->IfLuid,
                                     sizeof(LUID));
    }
  }
  return FALSE;
...
typedef enum { FallbackIndexTcpFastopen, FallbackIndexMax } FALLBACK_INDEX;
