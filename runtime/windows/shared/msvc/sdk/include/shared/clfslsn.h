...
// CLFS_RECORD_INDEX
//
// Log record offset on container file.  The log record offset consists of a
// block offset in the container and a bucket identifier indexing the records in
// the block. Declared up here because including clfs_x.h will try to define the
// LSN, which needs this.
//
...
// CLS_LSN
//
// The log sequence number (LSN) is a valid log file address.  The LSN consists
// of three (3) parts: (a) a log identifier to identify which physical log the
// log record belongs to, (b) a container index identifying the log container
// where the log record lies, and (c) a record offset identified by the offset
// of the block in the container and an ordinal number for the record within the
// container.
//
//
// The structure of the LSN poses some inherent limitations of the number of
// logs, the number of containers, the size of a container, and the number of
// log records in a log block.
//
