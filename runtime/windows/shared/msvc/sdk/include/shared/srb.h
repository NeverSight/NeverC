...
//@[contract("ntoskrnl-srb"), comment("MVI_tracked -
...
// The Controller field will contain the StorMQ controller extension known to
// the miniport.
//

//
// N.B. The other legacy STOR_ADDRESS_TYPE_xxx values and structs are defined in
// scsi.h. Any updates to STOR_ADDRESS should be put here to not introduce new
// dependencies on legacy SCSI infrastructure.
//
...
typedef struct STOR_ADDRESS_ALIGN _STOR_ADDR_NVME {
  _Field_range_(STOR_ADDRESS_TYPE_NVME, STOR_ADDRESS_TYPE_NVME) USHORT Type;
  USHORT Port;
  _Field_range_(STOR_ADDR_NVME_ADDRESS_LENGTH,
                STOR_ADDR_NVME_ADDRESS_LENGTH) ULONG AddressLength;
  PVOID Controller;
  ULONG NamespaceId;
  ULONG Reserved;
...
//
// SRB extended data for bi-directional commands. For these SRBs, it will have
// both a SRB extended data (e.g. SRBEX_DATA_SCSI_CDB*) and a
...
typedef struct SRB_ALIGN _SRBEX_DATA_BIDIRECTIONAL {
  ...
  ULONG Reserved1;
  _Field_size_bytes_full_(DataInTransferLength) PVOID POINTER_ALIGN
      DataInBuffer;
...
typedef struct SRB_ALIGN _SRBEX_DATA_SCSI_CDB16 {
  _Field_range_(SrbExDataTypeScsiCdb16,
                SrbExDataTypeScsiCdb16) SRBEXDATATYPE Type;
  ...
  UCHAR Reserved;
  ULONG Reserved1;
  _Field_size_bytes_full_(SenseInfoBufferLength) PVOID POINTER_ALIGN
      SenseInfoBuffer;
...
typedef struct SRB_ALIGN _SRBEX_DATA_SCSI_CDB32 {
  _Field_range_(SrbExDataTypeScsiCdb32,
                SrbExDataTypeScsiCdb32) SRBEXDATATYPE Type;
...
typedef struct SRB_ALIGN _SRBEX_DATA_SCSI_CDB_VAR {
  _Field_range_(SrbExDataTypeScsiCdbVar,
                SrbExDataTypeScsiCdbVar) SRBEXDATATYPE Type;
...
#define SRBEX_DATA_WMI_LENGTH                                                  \
  ((4 * sizeof(UCHAR)) + sizeof(ULONG) + sizeof(PVOID))
...
#define SRBEX_DATA_POWER_LENGTH                                                \
  ((4 * sizeof(UCHAR)) + sizeof(STOR_DEVICE_POWER_STATE) +                     \
   sizeof(STOR_POWER_ACTION))
...
typedef struct SRB_ALIGN _SRBEX_DATA_MINIPORT_PASSTHROUGH {
  _Field_range_(SrbExDataTypeMiniportPassthrough,
                SrbExDataTypeMiniportPassthrough) SRBEXDATATYPE Type;
  _Field_range_(SRBEX_DATA_MINIPORT_PASSTHROUGH_LENGTH,
                SRBEX_DATA_MINIPORT_PASSTHROUGH_LENGTH) ULONG Length;
...
typedef struct SRB_ALIGN _SRBEX_DATA_IO_INFO {
  ...
  _Field_range_(SRBEX_DATA_IO_INFO_LENGTH,
                SRBEX_DATA_IO_INFO_LENGTH) ULONG Length;
  ULONG Flags;
  ULONG Key;
  ULONG RWLength;
...
#define SRBEX_DATA_NVME_COMMAND_LENGTH                                         \
  ((4 * sizeof(ULONGLONG)) + (14 * sizeof(ULONG)) + (5 * sizeof(USHORT)) +     \
   (2 * sizeof(UCHAR)))
...
typedef enum {
  SRBEX_DATA_NVME_COMMAND_FLAG_REQUIRE_DATA_TRANSFER_IN =
      0x1, // Data is being read in from the device.
  SRBEX_DATA_NVME_COMMAND_FLAG_REQUIRE_DATA_TRANSFER_OUT =
      0x2, // Data is being written out to the device.
  ...
  SRBEX_DATA_NVME_COMMAND_FLAG_NO_POLLING =
      0x10 // Indicate to send the command with interrupt mode.
...
typedef struct SRB_ALIGN _SRBEX_DATA_NVME_COMMAND {
  ...
  _Field_range_(SrbExDataTypeNvmeCommand,
                SrbExDataTypeNvmeCommand) SRBEXDATATYPE Type;

  _Field_range_(SRBEX_DATA_NVME_COMMAND_LENGTH,
                SRBEX_DATA_NVME_COMMAND_LENGTH) ULONG Length;
...
#define SRBEX_DATA_NVMEOF_OPERATION_LENGTH                                     \
  ((2 * sizeof(USHORT)) + (2 * sizeof(ULONG)))

typedef struct SRB_ALIGN _SRBEX_DATA_NVMEOF_OPERATION {

  _Field_range_(SrbExDataTypeNvmeofOperation,
                SrbExDataTypeNvmeofOperation) SRBEXDATATYPE Type;

  _Field_range_(SRBEX_DATA_NVMEOF_OPERATION_LENGTH,
                SRBEX_DATA_NVMEOF_OPERATION_LENGTH) ULONG Length;

  //
  // Version of this structure
  //
  _Field_range_(STOR_NVMEOF_OPERATION_V1,
                STOR_NVMEOF_OPERATION_V1) USHORT Version;
...
typedef _Struct_size_bytes_(SrbLength) struct SRB_ALIGN _STORAGE_REQUEST_BLOCK {
  ...
  union {
    ...
    //
    // Used to store high 4 bytes of unique tag if unique tag feature is
    // enabled.
    //
  ...
  // storage filter drivers and to have defined behavior for drivers that
  // misinterpret new SRB format for old on 32-bit platforms.
  //
  ULONG ZeroGuard1;
  ...
  // Guard page that should always be zero. Use to guard against misbehaving
  // storage filter drivers and to have defined behavior for drivers that
  // misinterpret new SRB format for old on 64-bit platforms.
  //
...
typedef _Must_inspect_result_
    BOOLEAN (*PHW_INITIALIZE)(_In_ PVOID DeviceExtension);

typedef _Must_inspect_result_
    BOOLEAN (*PHW_STARTIO)(_In_ PVOID DeviceExtension,
                           _In_ PSCSI_REQUEST_BLOCK Srb);

typedef _Must_inspect_result_
    BOOLEAN (*PHW_INTERRUPT)(_In_ PVOID DeviceExtension);

typedef VOID (*PHW_TIMER)(_In_ PVOID DeviceExtension);

typedef VOID (*PHW_DMA_STARTED)(_In_ PVOID DeviceExtension);

typedef _Must_inspect_result_ ULONG (*PHW_FIND_ADAPTER)(
    _In_ PVOID DeviceExtension, _In_ PVOID HwContext, _In_ PVOID BusInformation,
    _In_ PCHAR ArgumentString,
    _Inout_ PPORT_CONFIGURATION_INFORMATION ConfigInfo, _Out_ PBOOLEAN Again);

typedef _Must_inspect_result_
    BOOLEAN (*PHW_RESET_BUS)(_In_ PVOID DeviceExtension, _In_ ULONG PathId);

typedef _Must_inspect_result_
    BOOLEAN (*PHW_ADAPTER_STATE)(_In_ PVOID DeviceExtension, _In_ PVOID Context,
                                 _In_ BOOLEAN SaveState);

typedef _Must_inspect_result_ SCSI_ADAPTER_CONTROL_STATUS (
    *PHW_ADAPTER_CONTROL)(_In_ PVOID DeviceExtension,
...
SCSIPORT_API
ULONG
ScsiPortInitialize(_In_ PVOID Argument1, _In_ PVOID Argument2,
                   _In_ struct _HW_INITIALIZATION_DATA *HwInitializationData,
                   _In_ PVOID HwContext);

SCSIPORT_API
VOID ScsiPortFreeDeviceBase(_In_ PVOID HwDeviceExtension,
                            _In_ PVOID MappedAddress);

_Must_inspect_result_ SCSIPORT_API ULONG
ScsiPortGetBusData(_In_ PVOID DeviceExtension, _In_ ULONG BusDataType,
                   _In_ ULONG SystemIoBusNumber, _In_ ULONG SlotNumber,
                   _In_reads_bytes_(Length) PVOID Buffer, _In_ ULONG Length);

_Must_inspect_result_ SCSIPORT_API ULONG
ScsiPortSetBusDataByOffset(_In_ PVOID DeviceExtension, _In_ ULONG BusDataType,
                           _In_ ULONG SystemIoBusNumber, _In_ ULONG SlotNumber,
                           _In_reads_bytes_(Length) PVOID Buffer,
                           _In_ ULONG Offset, _In_ ULONG Length);

_Must_inspect_result_ SCSIPORT_API PVOID ScsiPortGetDeviceBase(
    _In_ PVOID HwDeviceExtension, _In_ INTERFACE_TYPE BusType,
    _In_ ULONG SystemIoBusNumber, _In_ SCSI_PHYSICAL_ADDRESS IoAddress,
    _In_ ULONG NumberOfBytes, _In_ BOOLEAN InIoSpace);

_Must_inspect_result_ SCSIPORT_API PVOID
ScsiPortGetLogicalUnit(_In_ PVOID HwDeviceExtension, _In_ UCHAR PathId,
                       _In_ UCHAR TargetId, _In_ UCHAR Lun);

_Must_inspect_result_ SCSIPORT_API PSCSI_REQUEST_BLOCK
ScsiPortGetSrb(_In_ PVOID DeviceExtension, _In_ UCHAR PathId,
               _In_ UCHAR TargetId, _In_ UCHAR Lun, _In_ LONG QueueTag);

_Must_inspect_result_ SCSIPORT_API SCSI_PHYSICAL_ADDRESS
ScsiPortGetPhysicalAddress(_In_ PVOID HwDeviceExtension,
                           _In_ PSCSI_REQUEST_BLOCK Srb,
                           _In_ PVOID VirtualAddress, _Out_ ULONG *Length);

_Must_inspect_result_ SCSIPORT_API PVOID ScsiPortGetVirtualAddress(
    _In_ PVOID HwDeviceExtension, _In_ SCSI_PHYSICAL_ADDRESS PhysicalAddress);

_Must_inspect_result_ SCSIPORT_API PVOID ScsiPortGetUncachedExtension(
    _In_ PVOID HwDeviceExtension,
    _In_ PPORT_CONFIGURATION_INFORMATION ConfigInfo, _In_ ULONG NumberOfBytes);

SCSIPORT_API
VOID ScsiPortFlushDma(_In_ PVOID DeviceExtension);

SCSIPORT_API
VOID ScsiPortIoMapTransfer(_In_ PVOID HwDeviceExtension,
                           _In_ PSCSI_REQUEST_BLOCK Srb,
                           _In_ PVOID LogicalAddress, _In_ ULONG Length);

SCSIPORT_API
VOID ScsiPortNotification(_In_ SCSI_NOTIFICATION_TYPE NotificationType,
                          _In_ PVOID HwDeviceExtension, ...);

SCSIPORT_API
VOID ScsiPortLogError(_In_ PVOID HwDeviceExtension,
                      _In_ PSCSI_REQUEST_BLOCK Srb OPTIONAL, _In_ UCHAR PathId,
                      _In_ UCHAR TargetId, _In_ UCHAR Lun, _In_ ULONG ErrorCode,
                      _In_ ULONG UniqueId);

SCSIPORT_API
VOID ScsiPortCompleteRequest(_In_ PVOID HwDeviceExtension, _In_ UCHAR PathId,
                             _In_ UCHAR TargetId, _In_ UCHAR Lun,
                             _In_ UCHAR SrbStatus);

SCSIPORT_API
VOID ScsiPortStallExecution(_In_ ULONG Delay);
...
_Must_inspect_result_
    SCSIPORT_API UCHAR ScsiPortReadPortUchar(_In_ PUCHAR Port);

_Must_inspect_result_ SCSIPORT_API USHORT
ScsiPortReadPortUshort(_In_ PUSHORT Port);

_Must_inspect_result_ SCSIPORT_API ULONG
ScsiPortReadPortUlong(_In_ PULONG Port);

SCSIPORT_API
VOID ScsiPortReadPortBufferUchar(_In_ PUCHAR Port, _In_ PUCHAR Buffer,
                                 _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortReadPortBufferUshort(_In_ PUSHORT Port, _In_ PUSHORT Buffer,
                                  _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortReadPortBufferUlong(_In_ PULONG Port, _In_ PULONG Buffer,
                                 _In_ ULONG Count);

_Must_inspect_result_ SCSIPORT_API UCHAR
ScsiPortReadRegisterUchar(_In_ PUCHAR Register);

_Must_inspect_result_ SCSIPORT_API USHORT
ScsiPortReadRegisterUshort(_In_ PUSHORT Register);

_Must_inspect_result_ SCSIPORT_API ULONG
ScsiPortReadRegisterUlong(_In_ PULONG Register);

SCSIPORT_API
VOID ScsiPortReadRegisterBufferUchar(_In_ PUCHAR Register, _In_ PUCHAR Buffer,
                                     _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortReadRegisterBufferUshort(_In_ PUSHORT Register,
                                      _In_ PUSHORT Buffer, _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortReadRegisterBufferUlong(_In_ PULONG Register, _In_ PULONG Buffer,
                                     _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortWritePortUchar(_In_ PUCHAR Port, _In_ UCHAR Value);

SCSIPORT_API
VOID ScsiPortWritePortUshort(_In_ PUSHORT Port, _In_ USHORT Value);

SCSIPORT_API
VOID ScsiPortWritePortUlong(_In_ PULONG Port, _In_ ULONG Value);

SCSIPORT_API
VOID ScsiPortWritePortBufferUchar(_In_ PUCHAR Port, _In_ PUCHAR Buffer,
                                  _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortWritePortBufferUshort(_In_ PUSHORT Port, _In_ PUSHORT Buffer,
                                   _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortWritePortBufferUlong(_In_ PULONG Port, _In_ PULONG Buffer,
                                  _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortWriteRegisterUchar(_In_ PUCHAR Register, _In_ UCHAR Value);

SCSIPORT_API
VOID ScsiPortWriteRegisterUshort(_In_ PUSHORT Register, _In_ USHORT Value);

SCSIPORT_API
VOID ScsiPortWriteRegisterUlong(_In_ PULONG Register, _In_ ULONG Value);

SCSIPORT_API
VOID ScsiPortWriteRegisterBufferUchar(_In_ PUCHAR Register, _In_ PUCHAR Buffer,
                                      _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortWriteRegisterBufferUshort(_In_ PUSHORT Register,
                                       _In_ PUSHORT Buffer, _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortWriteRegisterBufferUlong(_In_ PULONG Register, _In_ PULONG Buffer,
                                      _In_ ULONG Count);

SCSIPORT_API
VOID ScsiPortMoveMemory(_In_ PVOID WriteBuffer, _In_ PVOID ReadBuffer,
                        _In_ ULONG Length);

#endif

_Must_inspect_result_ SCSIPORT_API SCSI_PHYSICAL_ADDRESS
ScsiPortConvertUlongToPhysicalAddress(_In_ ULONG_PTR UlongAddress);

_Must_inspect_result_ SCSIPORT_API ULONG
ScsiPortConvertPhysicalAddressToUlong(_In_ SCSI_PHYSICAL_ADDRESS Address);

SCSIPORT_API
VOID ScsiPortQuerySystemTime(_Out_ PLARGE_INTEGER CurrentTime);
...
_Must_inspect_result_ SCSIPORT_API BOOLEAN ScsiPortValidateRange(
    _In_ PVOID HwDeviceExtension, _In_ INTERFACE_TYPE BusType,
    _In_ ULONG SystemIoBusNumber, _In_ SCSI_PHYSICAL_ADDRESS IoAddress,
    _In_ ULONG NumberOfBytes, _In_ BOOLEAN InIoSpace);
