...
int main(int argc, char* argv[])
{
    ...
    TraceLoggingWriteStart(
        parentActivity,
        "ParentActivity", // The name of the start event is used as the activity
name. TraceLoggingValue(argc, "SampleField1"));

    {
        // Declare a local (nested) activity variable, with explicit keyword and
level: TraceLoggingActivity<g_hProvider, MyKeywordValue, WINEVENT_LEVEL_ERROR>
nestedActivity;
        ...
        // TraceLoggingWriteTagged writes an event that is tagged with an
activity
        // ID. Note that TraceLoggingWriteTagged is just like TraceLoggingWrite
        // except that the first parameter is an activity (not a provider) and
the
        // resulting event is explicitly tagged with the activity ID. In
particular,
        // TraceLoggingWriteTagged does NOT use the activity's level and keyword
--
        // they are specified just as they would be for a TraceLoggingWrite
event. TraceLoggingWriteTagged( nestedActivity, "NestedActivityIdTaggedEvent",
        ...
        // Note that if the activity is destroyed without being stopped, a
default
        // "ActivityStoppedAutomatically" event will be written by the
destructor.
    }
...
- TraceLoggingWriteTagged: Writes an event explicitly tagged with an activity
ID.
...
Activities can be nested by calling
childActivity.SetRelatedActivity(parentActivity) before calling
TraceLoggingWriteStart(childActivity, ...).

Use TraceLoggingWriteStart to generate a unique activity ID and write the START
event for the activity.

Use TraceLoggingWriteTagged to write events that are associated with the
activity.
...
template <TraceLoggingHProvider const &provider,
          ...
          typename TlgReflectorTag =
              _TlgReflectorTag_Param0IsHProvider> // helps TlgReflector
...
CurrentActivityId. This automatic behavior can be convenient in some cases, but
it only works correctly when the activities nest precisely and do not cross
thread boundaries. In addition, TraceLoggingThreadActivity is not available on
the Windows Runtime (i.e. this class cannot be used for Windows Store or Windows
Phone apps).

PLEASE NOTE: use TraceLoggingThreadActivity only when you can guarantee that
lifetimes of all activities of this type on the thread fully nest in all cases,
i.e. a parent activity will not end before a child activity, even in error or
exceptional cases. Activities defined in function-local scopes typically meet
this criteria.
...
Use TraceLoggingWriteTagged to write events that are explicitly associated with
the activity.
...
template <
    ...
    UINT8 level = 5,    // defaults to 5 = WINEVENT_LEVEL_VERBOSE
    typename TlgReflectorTag =
        _TlgReflectorTag_Param0IsHProvider> // helps TlgReflector understand
                                            // that this is a wrapper type
class TraceLoggingThreadActivity;

/*
TraceLoggingThreadActivityIdSetter is a helper class that sets the thread-local
CurrentActivity variable during construction and saves the original value. It
then restores the original value during destruction. This class is not available
on the Windows Runtime (i.e. it cannot be used for Windows Store or Windows
Phone apps).
...
#define TraceLoggingWriteStart(activity, name, ...)                            \
  __pragma(warning(push)) __pragma(warning(disable : 4127)) do {               \
    ...
    static_assert(_tlgActivity_Keyword == (_tlgActivity_Keyword _tlg_FOREACH(  \
                                              _tlgKeywordVal, __VA_ARGS__)),   \
                  "Do not use TraceLoggingKeyword in TraceLoggingWriteStart. " \
                  "Keywords for START events are "                             \
    ...
    static_assert(_tlgActivity_Level == (_tlgActivity_Level _tlg_FOREACH(      \
                  ...
                  "Do not use TraceLoggingLevel in TraceLoggingWriteStart. "   \
                  "The Level for START events is "                             \
                  "specified in the activity type, e.g. "                      \
                  "TraceLoggingActivity<Provider,Keyword,Level>.");            \
    _tlgActivityRef(activity).zInternalStart();                                \
    _tlgActivityWriteStartStop(                                                \
        _tlg_IS_EMPTY(__VA_ARGS__),                                            \
        (activity, (name), _tlgActivityRef(activity).Id(),                     \
         _tlgActivityRef(activity).zInternalRelatedId(),                       \
         1 /* WINEVENT_OPCODE_START */, _tlgActivity_Keyword,                  \
         _tlgActivity_Level, __VA_ARGS__));                                    \
  }                                                                            \
  while (0)                                                                    \
...
#define TraceLoggingWriteStop(activity, name, ...)                             \
  __pragma(warning(push)) __pragma(warning(disable : 4127)) do {               \
    ...
    static_assert(_tlgActivity_Keyword == (_tlgActivity_Keyword _tlg_FOREACH(  \
                                              _tlgKeywordVal, __VA_ARGS__)),   \
                  "Do not use TraceLoggingKeyword in TraceLoggingWriteStop. "  \
                  "Keywords for STOP events are "                              \
                  "specified in the activity type, e.g. "                      \
                  "TraceLoggingActivity<Provider,Keyword,Level>.");            \
    static_assert(_tlgActivity_Level == (_tlgActivity_Level _tlg_FOREACH(      \
                                            _tlgLevelVal, __VA_ARGS__)),       \
                  "Do not use TraceLoggingLevel in TraceLoggingWriteStop. "    \
                  "The Level for STOP events is "                              \
                  "specified in the activity type, e.g. "                      \
                  "TraceLoggingActivity<Provider,Keyword,Level>.");            \
    _tlgActivityRef(activity).zInternalStop();                                 \
    _tlgActivityWriteStartStop(                                                \
        _tlg_IS_EMPTY(__VA_ARGS__),                                            \
        (activity, (name), _tlgActivityRef(activity).Id(), NULL,               \
         2 /* WINEVENT_OPCODE_STOP */, _tlgActivity_Keyword,                   \
         _tlgActivity_Level, __VA_ARGS__));                                    \
  }                                                                            \
  while (0)                                                                    \
  __pragma(warning(pop))
...
level. If the level is not specified via TraceLoggingLevel, the level will
default to verbose (5). If the keyword is not specified via TraceLoggingKeyword,
the keyword will default to none (0).
*/
#define TraceLoggingWriteTagged(activity, name, ...)                           \
  __pragma(warning(push)) __pragma(warning(disable : 4127)) do {               \
    _tlgActivityDecl(activity) TraceLoggingWriteActivity(                      \
        _tlgActivityRef(activity).Provider(), (name),                          \
...
#define TraceLoggingWriteTaggedIfStarted(activity, name, ...)                  \
  __pragma(warning(push)) __pragma(warning(disable : 4127)) do {               \
    _tlgActivityDecl(activity) if (_tlgActivityRef(activity).IsStarted()) {    \
      TraceLoggingWriteActivity(_tlgActivityRef(activity).Provider(), (name),  \
...
#define TraceLoggingFunction(providerHandle, ...)                              \
  TraceLoggingThreadActivity<providerHandle,                                   \
...
#define _tlgActivityWriteStartStop_imp0(                                       \
    activity, eventName, pActivityId, pRelatedId, opcode, keyword, level, ...) \
  TraceLoggingWriteActivity(                                                   \
      _tlgActivityRef(activity).Provider(), eventName, pActivityId,            \
      pRelatedId, TraceLoggingOpcode(opcode), TraceLoggingKeyword(keyword),    \
      TraceLoggingLevel(level),                                                \
      TraceLoggingDescription("~^" _tlg_LSTRINGIZE(activity) L"^~"),           \
      __VA_ARGS__)
#define _tlgActivityWriteStartStop_imp1(                                       \
    activity, eventName, pActivityId, pRelatedId, opcode, keyword, level, ...) \
  TraceLoggingWriteActivity(                                                   \
      _tlgActivityRef(activity).Provider(), eventName, pActivityId,            \
      pRelatedId, TraceLoggingOpcode(opcode), TraceLoggingKeyword(keyword),    \
...
void _tlgWriteActivityAutoStop(TraceLoggingHProvider provider,
                               _In_ GUID const *pActivityId) {
  TraceLoggingWriteActivity(
      provider, "ActivityStoppedAutomatically", pActivityId, NULL,
      TraceLoggingOpcode(2 /* WINEVENT_OPCODE_STOP */),
      TraceLoggingKeyword(keyword), TraceLoggingLevel(level));
}
...
*/
template <typename DerivedTy, // DerivedTy must define Provider(), OnStarted(),
                              // OnStopped().
          UINT64 keyword, UINT8 level>
...
protected:
  ~_TlgActivityBase() {
    if (m_State == TlgStateStarted) {
      ...
      _tlgWriteActivityAutoStop<keyword, level>(
          static_cast<DerivedTy *>(this)->Provider(), &m_Id);
    }

    m_State = TlgStateDestroyed;
  }

  _TlgActivityBase() { Reset(); }
  ...
  _TlgActivityBase &operator=(_TlgActivityBase &&rhs) {
    _tlg_ASSERT(m_State == TlgStateCreated,
                "Move-assign to newly created activities only");
    m_State = rhs.m_State;
  ...
  */
  void SetRelatedId(const GUID &relatedActivityId) {
    _tlg_ASSERT(m_State == TlgStateCreated,
                "_TlgActivityBase::SetRelatedId called from invalid state.");
    _tlg_ASSERT(!m_HasRelatedId,
                "_TlgActivityBase::RelatedActivity was already set.");
    m_CapturedRelatedId = relatedActivityId;
    m_HasRelatedId = true;
  }

  /*
  Should be called at an appropriate point in activity setup, after
  SetRelatedId() is called. Returns the related activity Id, if there is one.
  Returns NULL if the related activity Id has not been set or if it is
  GUID_NULL.
  */
  _Ret_opt_ const GUID *GetRelatedId() const {
    return m_HasRelatedId && !_tlgGuidIsZero(m_CapturedRelatedId)
               ? &m_CapturedRelatedId
               : NULL;
  }
  ...
  */
  void PushThreadActivityId() {
    _tlg_ASSERT(
        m_State == TlgStateCreated,
        "_TlgActivityBase::PushThreadActivityId called from invalid state.");
    _tlg_ASSERT(!m_HasRelatedId,
                "_TlgActivityBase::RelatedActivity was already set.");
    m_CapturedRelatedId = m_Id;
    ::EventActivityIdControl(EVENT_ACTIVITY_CTRL_GET_SET_ID,
                             &m_CapturedRelatedId);
    m_HasRelatedId = true;
  }
  ...
  */
  void PopThreadActivityId() {
    _tlg_ASSERT(
        ...
        "_TlgActivityBase::PopThreadActivityId called from invalid state.");
    if (m_HasRelatedId) {
      ::EventActivityIdControl(EVENT_ACTIVITY_CTRL_GET_SET_ID,
                               &m_CapturedRelatedId);

      /*
      If this assertion fails, it means that your thread activity's ID was
      corrupted. For example:

      TraceLoggingWriteStart(activity1, ...); // Saves activity1.SavedId =
      thread.ActivityId, sets thread.ActivityId = activity1.Id(). DoWork();
      TraceLoggingWrite(...); // Uses thread.ActivityId.
      TraceLoggingWriteStop(activity1, ...); // Verifies that thread.ActivityId
      == activity1.Id(), restores thread.ActivityId = activity1.SavedId.
      ...
      will work as expected. If DoWork changes thread.ActivityId to its own
      value and does not restore it before returning, the TraceLoggingWrite call
      will use the activity ID set by DoWork instead of using activity1.Id().
      The assertion fires to warn you about this situation.

      The following are possible solutions for this issue:

      1. If you can change the DoWork() code: Fix the DoWork() code to correctly
         restore the activity ID before returning. Note that the activity ID
      should be restored even in cases where DoWork() exits via exception.
      2. If you cannot change the DoWork() code: Save the activity ID before
         calling DoWork(), then restore the activity ID after DoWork() returns.
      You can use the EventActivityIdControl function to save and restore the
         activity ID.
      3. Use TraceLoggingActivity instead of TraceLoggingThreadActivity so that
      you are not impacted by code that incorrectly manages thread.ActivityId.
      In this case, you will also need to switch from TraceLoggingWrite to
         TraceLoggingWriteTagged.
      ...
      */
      _tlg_ASSERT(_tlgGuidEqual(m_Id, m_CapturedRelatedId),
                  "_TlgActivityBase: current id does not match set id!");
    }
  }
...
public:
  /*
  The activity's Keyword, which is used as the keyword for the START and STOP
  events.
  */
  static const UINT64 Keyword = keyword;

  /*
  The activity's Level, which is used as the level for the START and STOP
  events.
  */
  ...
  */
  _Ret_ const GUID *Id() const {
    _tlg_ASSERT(m_State >= TlgStateStarted,
                "TraceLoggingActivity::Id() called from invalid state");
    _tlg_ASSERT(m_State != TlgStateDestroyed,
                "TraceLoggingActivity::Id() called after destruction");
    return &m_Id;
  }
  ...
  */
  bool IsStarted() const { return m_State == TlgStateStarted; }
  ...
  */
  _Ret_opt_ const GUID *zInternalRelatedId() const {
    _tlg_ASSERT(m_State == TlgStateStarted,
                "TraceLoggingWriteStart race condition");
    return GetRelatedId();
  }
  ...
  */
  void zInternalStart() {
    _tlg_ASSERT(m_State == TlgStateCreated,
                "TraceLoggingWriteStart called from invalid state.");
  ...
  */
  void zInternalStop() {
    _tlg_ASSERT(m_State == TlgStateStarted,
                "TraceLoggingWriteStop called from invalid state");
...
private:
  ...
  static auto CreateActivityId(int, _In_ _DerivedTy *Derived,
                               _Out_ GUID &ChildActivityId)
      -> decltype(Derived->CreateActivityId(ChildActivityId), void(0)) {
    Derived->CreateActivityId(ChildActivityId);
  }

  template <typename _DerivedTy>
  static auto CreateActivityId(long, _In_ _DerivedTy *Derived,
                               _Out_ GUID &ChildActivityId) -> void {
    (void)Derived;
    ::EventActivityIdControl(EVENT_ACTIVITY_CTRL_CREATE_ID, &ChildActivityId);
  }
};

template <TraceLoggingHProvider const &provider, UINT64 keyword, UINT8 level,
          typename TlgReflectorTag>
class TraceLoggingActivity
    : public _TlgActivityBase<
          TraceLoggingActivity<provider, keyword, level, TlgReflectorTag>,
          keyword, level> {
  typedef _TlgActivityBase<
      TraceLoggingActivity<provider, keyword, level, TlgReflectorTag>, keyword,
      level>
      BaseTy;
  friend BaseTy;
...
public:
  ...
  */
  TraceLoggingHProvider Provider() const { return provider; }
  ...
  */
  void SetRelatedActivityId(_In_ const GUID *relatedActivityId) {
    _tlg_ASSERT(relatedActivityId != NULL,
                "TraceLoggingActivity SetRelatedActivity called with NULL id.");
    BaseTy::SetRelatedId(*relatedActivityId);
  }
};
...
class TraceLoggingThreadActivity
    : public _TlgActivityBase<
          TraceLoggingThreadActivity<provider, keyword, level, TlgReflectorTag>,
          keyword, level> {
  typedef _TlgActivityBase<
      TraceLoggingThreadActivity<provider, keyword, level, TlgReflectorTag>,
      keyword, level>
  ...
  void OnStarted() { BaseTy::PushThreadActivityId(); }

  void OnStopped() { BaseTy::PopThreadActivityId(); }
...
class TraceLoggingThreadActivityIdSetter {
  ...
  TraceLoggingThreadActivityIdSetter(
      const TraceLoggingThreadActivityIdSetter &); // = delete
  const TraceLoggingThreadActivityIdSetter &
  operator=(const TraceLoggingThreadActivityIdSetter &); // = delete

public:
  explicit TraceLoggingThreadActivityIdSetter(_In_ const GUID &activityId)
      : m_ActivityId(activityId), m_SavedActivityId(activityId) {
    ::EventActivityIdControl(EVENT_ACTIVITY_CTRL_GET_SET_ID,
                             &m_SavedActivityId);
  }
  ...
  ~TraceLoggingThreadActivityIdSetter() {
    ::EventActivityIdControl(EVENT_ACTIVITY_CTRL_GET_SET_ID,
                             &m_SavedActivityId);
    _tlg_ASSERT(
        _tlgGuidEqual(m_ActivityId, m_SavedActivityId),
        "TraceLoggingThreadActivityIdSetter current id does not match set id!");
  }
};
