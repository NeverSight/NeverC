...
typedef enum _NVME_PROPERTY_OFFSET {
  ...
  NvmePropINTMC =
      0x0010, // Interrupt Mask Clear  # Note: This is incorrect in the spec.
  ...
  NvmePropCMBSWTP =
      0x0060,            // Controller Memory Buffer Sustained Write Throughput
  ...
  NvmePropPMRSWTP =
      0x0E10, // Persistent Memory Region Sustained Write Throughput
  NvmePropPMRMSCL =
      0x0E14, // Persistent Memory Region Controller Memory Space Control Lower
  NvmePropPMRMSCU =
      0x0E18 // Persistent Memory Region Controller Memory Space Control Upper
...
typedef union {
  ...
  struct {
    ...
    ULONGLONG MQES
        : 16;          // RO - Bit 00-15: Maximum Queue Entries Supported (MQES)
    ULONGLONG CQR : 1; // RO - Bit 16: Contiguous Queues Required (CQR)

    // Bit 17, 18 - AMS; RO - Arbitration Mechanism Supported (AMS)
    ULONGLONG AMS_WeightedRoundRobinWithUrgent
        : 1; // Bit 17: Weighted Round Robin with Urgent;
    ...
    ULONGLONG CSS_AdminOnly
        : 1; // Bit 44: Only Admin command set (no IO command set)
    ...
    ULONGLONG PMRS
        : 1; // RO - Bit 56: Persistent Memory Region Supported (PMRS)
    ULONGLONG CMBS
        : 1; // RO - Bit 57: Controller Memory Buffer Supported (CMBS)
    ULONGLONG NSSS : 1; // RO - Bit 58: NVM Subsystem Shutdown Supported (NSSS)
    ULONGLONG CRWMS
        : 1; // RO - Bit 59: Controller Ready With Media Support (CRWMS)
    ULONGLONG CRIMS : 1; // RO - Bit 60: Controller Ready Independent of Media
                         // Support (CRIMS)
...
typedef union {
  ...
  struct {
    ...
    ULONG CRIME
        : 1; // RO/RW - Controller Ready Independent of Media Enable (CRIME).
...
//
// Offset (1000h + ((2y) * (4 << CAP.DSTRD))): SQyTDBL (Submission Queue y Tail
// Doorbell)
//
typedef union {
...
//
// Offset  (1000h + ((2y + 1) * (4 << CAP.DSTRD))): CQyHDBL (Completion Queue y
// Head Doorbell)
//
typedef union {
...
typedef struct {
  ...
  NVME_ADMIN_SUBMISSION_QUEUE_BASE_ADDRESS
  ASQ; // Admin Submission Queue Base Address; 8 bytes
  NVME_ADMIN_COMPLETION_QUEUE_BASE_ADDRESS
  ACQ; // Admin Completion Queue Base Address; 8 bytes

  NVME_CONTROLLER_MEMORY_BUFFER_LOCATION
  CMBLOC; // Controller Memory Buffer Location (Optional)
  NVME_CONTROLLER_MEMORY_BUFFER_SIZE
  CMBSZ; // Controller Memory Buffer Size (Optional)
  ...
  ULONG Doorbells[0]; // Start of the first Doorbell register. (Admin SQ Tail
                      // Doorbell)
...
typedef enum {
  ...
  NVME_STATUS_INVALID_QUEUE_IDENTIFIER =
      0x01, // Create I/O Submission Queue, Create I/O Completion Queue, Delete
            // I/O Completion Queue, Delete I/O Submission Queue
  NVME_STATUS_MAX_QUEUE_SIZE_EXCEEDED =
      0x02, // Create I/O Submission Queue, Create I/O Completion Queue
  NVME_STATUS_ABORT_COMMAND_LIMIT_EXCEEDED = 0x03, // Abort
  NVME_STATUS_ASYNC_EVENT_REQUEST_LIMIT_EXCEEDED =
      0x05,                                    // Asynchronous Event Request
  ...
  NVME_STATUS_INVALID_SECONDARY_CONTROLLER_STATE =
      0x20, // Virtualization Management
  NVME_STATUS_INVALID_NUMBER_OF_CONTROLLER_RESOURCES =
      0x21,                                       // Virtualization Management
  ...
  NVME_STATUS_NAMESPACE_ATTACHMENT_LIMIT_EXCEEDED =
      0x27,                                     // Namespace Attachment
  NVME_STATUS_PROHIBITION_NOT_SUPPORTED = 0x28, // Lockdown

  NVME_IO_COMMAND_SET_NOT_SUPPORTED =
      0x29, // Namespace Attachment/Management, Get Log Page
  ...
  NVME_STATUS_INVALID_DISCOVERY_INFORMATION =
      0x2F, // Discovery Information Management
  NVME_STATUS_INSUFFICIENT_DISCOVERY_RESOURCES =
      0x32, // Discovery Information Management
  ...
  NVME_STATUS_NVM_CONFLICTING_ATTRIBUTES =
      0x80, // Dataset Management, Read, Write
  NVME_STATUS_NVM_INVALID_PROTECTION_INFORMATION =
      0x81, // Compare, Read, Write, Write Zeroes, Verify
  ...
  NVME_STATUS_ZONE_BOUNDARY_ERROR =
      0xB8, // Compare, Read, Verify, Write, Write Uncorrectable, Write Zeroes,
            // Copy, Zone Append
  NVME_STATUS_ZONE_FULL =
      0xB9, // Write, Write Uncorrectable, Write Zeroes, Copy, Zone Append
  NVME_STATUS_ZONE_READ_ONLY =
      0xBA, // Write, Write Uncorrectable, Write Zeroes, Copy, Zone Append
  NVME_STATUS_ZONE_OFFLINE =
      0xBB, // Compare, Read, Verify, Write, Write Uncorrectable, Write Zeroes,
            // Copy, Zone Append
  NVME_STATUS_ZONE_INVALID_WRITE =
      0xBC, // Write, Write Uncorrectable, Write Zeroes, Copy
  NVME_STATUS_ZONE_TOO_MANY_ACTIVE =
      0xBD, // Write, Write Uncorrectable, Write Zeroes, Copy, Zone Append, Zone
            // Management Send
  NVME_STATUS_ZONE_TOO_MANY_OPEN =
      0xBE, // Write, Write Uncorrectable, Write Zeroes, Copy, Zone Append, Zone
            // Management Send
...
typedef enum {
  ...
  NVME_STATUS_AUTHENTICATION_REQUIRED =
      0x91, // Queue has not yet been authenticated, in-band authentication is
            // required
...
typedef enum {
  ...
  NVME_ADMIN_COMMAND_FIRMWARE_COMMIT =
      0x10, // "Firmware Activate" command has been renamed to "Firmware Commit"
            // command in spec v1.2
...
typedef enum {
  ...
  NVME_FEATURE_CLEAR_FW_UPDATE_HISTORY =
      0xC1, // This is from OCP NVMe Cloud SSD spec.
  NVME_FEATURE_READONLY_WRITETHROUGH_MODE =
      0xC2, // This is from OCP NVMe Cloud SSD spec.
  NVME_FEATURE_CLEAR_PCIE_CORRECTABLE_ERROR_COUNTERS =
      0xC3, // This is from OCP NVMe Cloud SSD spec.
  NVME_FEATURE_ENABLE_IEEE1667_SILO =
      0xC4,                            // This is from OCP NVMe Cloud SSD spec.
  NVME_FEATURE_LATENCY_MONITOR = 0xC5, // This is from OCP NVMe Cloud SSD spec.
  NVME_FEATURE_PLP_HEALTH_CHECK_INTERVAL =
      0xC6,                             // This is from OCP NVMe Cloud SSD spec.
...
typedef enum {
  ...
  NVME_IDENTIFY_CNS_ACTIVE_NAMESPACES =
      0x2, // A list of up to 1024 active namespace IDs is returned to the host
           // containing active namespaces with a namespace identifier greater
           // than the value specified in the Namespace Identifier (CDW1.NSID)
           // field.
...
typedef union {
  ...
  struct {
    ULONG CNS : 8; // Controller or Namespace Structure (CNS, Defined in
                   // NVME_IDENTIFY_CNS_CODES)
    ULONG Reserved : 8;
...
typedef union {
  ...
  struct {
    ULONG CNSID : 16; // CNS Specific Identifier (NVM Set ID/Domain ID/Endurance
                      // Group ID)
    ULONG Reserved2 : 8;
    ULONG CSI : 8; // Command Set Identifier (CSI, Defined in
                   // NVME_COMMAND_SET_IDENTIFIERS)
  } DUMMYSTRUCTNAME2;
...
typedef struct {
  ...
  struct {
    UCHAR ProtectionInfoTypeEnabled
        : 3; // 0 - not enabled; 1 ~ 3: enabled type; 4 ~ 7: reserved
    UCHAR InfoAtBeginningOfMetadata : 1;
    UCHAR Reserved : 4;
  ...
  struct {
    ...
    UCHAR Reserved : 7;
  } NMIC; // byte 30      O - Namespace Multi-path I/O and Namespace Sharing
          // Capabilities (NMIC)
  ...
  struct {
    UCHAR PercentageRemained : 7; // Bits 6:0: indicate the percentage of the
                                  // namespace that remains to be formatted
    UCHAR Supported : 1; // Bit 7: if set to 1 indicates that the namespace
                         // supports the Format Progress Indicator.
  } FPI;                 // byte 32      O - Format Progress Indicator (FPI)

  struct {
    UCHAR ReadBehavior : 3; // Bits 2:0: indicate deallocated logical block read
                            // behavior (NVME_DEALLOCATE_READ_BEHAVIOR)
    UCHAR WriteZeroes : 1; // Bit 3: indicate controller supports the deallocate
                           // bit in Write Zeroes
    UCHAR GuardFieldWithCRC : 1; // Bit 4: indicate guard field for deallocated
                                 // logical blocks is set to CRC
    UCHAR Reserved : 3;
  } DLFEAT; // byte 33

  USHORT NAWUN; // byte 34:35 O - Namespace Atomic Write Unit Normal (NAWUN)
  USHORT
  NAWUPF; // byte 36:37 O - Namespace Atomic Write Unit Power Fail (NAWUPF)
  ...
  USHORT NABSPF; // byte 44:45 O - Namespace Atomic Boundary Size Power Fail
                 // (NABSPF)
  ...
  USHORT
  NPDG; // byte 68:69 O - Namespace Preferred Deallocate Granularity (NPDG)
  ...
  USHORT NVMSETID; // byte 100:101 O - Associated NVM Set Identifier: 1-based
                   // value specifying NVM Set of this namespace.

  USHORT ENDGID; // byte 102:103 O - Associated Endurance Group Identier

  UCHAR NGUID[16]; // byte 104:119 O - Namespace Globally Unique Identifier
                   // (NGUID)
  ...
  UCHAR VS[3712]; // byte 384:4095 O - Vendor Specific (VS): This range of bytes
                  // is allocated for vendor specific usage.
...
typedef struct {
  ...
  UCHAR IEEE[3]; // byte 73:75.  M - IEEE OUI Identifier (IEEE). Controller
                 // Vendor code.
  ...
  struct {
    ...
    UCHAR ANAR : 1;
    UCHAR Reserved : 4;
  } CMIC; // byte 76.     O - Controller Multi-Path I/O and Namespace Sharing
          // Capabilities (CMIC)
  ...
  UCHAR
  ReservedForManagement[13]; // byte 240:252.  Refer to the NVMe Management
  ...
  struct {
    ...
    UCHAR Reserved : 7;
  } AVSCC; // byte 264.    M - Admin Vendor Specific Command Configuration
           // (AVSCC)

  struct {
    UCHAR Supported : 1;
    UCHAR Reserved : 7;
  } APSTA; // byte 265.     O - Autonomous Power State Transition Attributes
           // (APSTA)

  USHORT WCTEMP; // byte 266:267. M - Warning Composite Temperature Threshold
                 // (WCTEMP)
  USHORT CCTEMP; // byte 268:269. M - Critical Composite Temperature Threshold
                 // (CCTEMP)
  ...
  struct {
    USHORT Supported : 1;
    USHORT Reserved : 15;
  } HCTMA; // byte 322:323  O - Host Controlled Thermal Management Attributes
           // (HCTMA)

  USHORT
  MNTMT; // byte 324:325  O - Minimum Thermal Management Temperature (MNTMT)
  USHORT
  MXTMT; // byte 326:327  O - Maximum Thermal Management Temperature (MXTMT)
  ...
  ULONG HMMINDS; // byte 332:335  O - Host Memory Buffer Minimum Descriptor
                 // Entry Size (HMMINDS)
  USHORT HMMAXD; // byte 336:337  O - Host Memory Maxiumum Descriptors Entries
                 // (HMMAXD)

  USHORT NSETIDMAX; // byte 338:339  O - NVM Set Identifier Maximum: 1-based
                    // maximum value of a valid NVM Set Identifier.
  USHORT ENDGIDMAX; // byte 340:341  O - Endurance Group Identifier Maximum
                    // (ENDGIDMAX)
  ...
  struct {
    ...
    UCHAR Reserved : 1;
    UCHAR StaticANAGRPID
        : 1; // If set, ANAGRPID in Identify Namespace doesn't change
    UCHAR SupportNonZeroANAGRPID
        : 1; // If set, Controller supports a non-zero value in ANAGRPID field
             // of Namespace Mgmt Command
  ...
  ULONG NANAGRPID; // byte 348:351  O - Number of ANA Group Identifiers
                   // (NANAGRPID)
  ...
  UCHAR
  TMPTHHA; // byte 384      O - Temperature Threshold Hysteresis Attributes
  ...
  //
  struct {
    UCHAR RequiredEntrySize
        : 4; // The value is in bytes and is reported as a power of two (2^n).
    UCHAR MaxEntrySize : 4; // This value is larger than or equal to the
                            // required SQ entry size.  The value is in bytes
  ...
  struct {
    UCHAR RequiredEntrySize
        : 4; // The value is in bytes and is reported as a power of two (2^n).
    UCHAR MaxEntrySize : 4; // This value is larger than or equal to the
                            // required CQ entry size. The value is in bytes and
  ...
  struct {
    UCHAR CommandFormatInSpec : 1;
    UCHAR Reserved : 7;
  } NVSCC; // byte 530.     M - NVM Vendor Specific Command Configuration
           // (NVSCC)
  ...
  UCHAR MAXDNA[16]; // byte 544:559. O - Maximum Domain Namespace Attachments
                    // (MAXDNA)

  ULONG MAXCNA; // byte 560:563. O - Maximum I/O Controller Namespace
                // Attachments (MAXCNA)

  UCHAR Reserved6[204]; // byte 564:767.

  UCHAR SUBNQN[256]; // byte 768:1023. M - NVM Subsystem NVMe Qualified Name
                     // (SUBNQN)
  ...
  //
  ULONG IOCCSZ; // byte 1792:1795. M - I/O Queue Command Capsule Supported Size
                // (IOCCSZ)

  ULONG IORCSZ; // byte 1796:1799. M - I/O Queue Response Capsule Supported Size
                // (IORCSZ)
  ...
  //
  NVME_POWER_STATE_DESC PDS
      [32]; // byte 2048:2079. M - Power State 0 Descriptor (PSD0):  This field
            // indicates the characteristics of power state 0 byte 2080:2111. O
            // - Power State 1 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 1 byte 2112:2143. O - Power State
            // 2 Descriptor (PSD1):  This field indicates the characteristics of
            // power state 2 byte 2144:2175. O - Power State 3 Descriptor
            // (PSD1):  This field indicates the characteristics of power state
            // 3 byte 2176:2207. O - Power State 4 Descriptor (PSD1):  This
            // field indicates the characteristics of power state 4 byte
            // 2208:2239. O - Power State 5 Descriptor (PSD1):  This field
            // indicates the characteristics of power state 5 byte 2240:2271. O
            // - Power State 6 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 6 byte 2272:2303. O - Power State
            // 7 Descriptor (PSD1):  This field indicates the characteristics of
            // power state 7 byte 2304:2335. O - Power State 8 Descriptor
            // (PSD1):  This field indicates the characteristics of power state
            // 8 byte 2336:2367. O - Power State 9 Descriptor (PSD1):  This
            // field indicates the characteristics of power state 9 byte
            // 2368:2399. O - Power State 10 Descriptor (PSD1):  This field
            // indicates the characteristics of power state 10 byte 2400:2431. O
            // - Power State 11 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 11 byte 2432:2463. O - Power State
            // 12 Descriptor (PSD1):  This field indicates the characteristics
            // of power state 12 byte 2464:2495. O - Power State 13 Descriptor
            // (PSD1):  This field indicates the characteristics of power state
            // 13 byte 2496:2527. O - Power State 14 Descriptor (PSD1):  This
            // field indicates the characteristics of power state 14 byte
            // 2528:2559. O - Power State 15 Descriptor (PSD1):  This field
            // indicates the characteristics of power state 15 byte 2560:2591. O
            // - Power State 16 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 16 byte 2592:2623. O - Power State
            // 17 Descriptor (PSD1):  This field indicates the characteristics
            // of power state 17 byte 2624:2655. O - Power State 18 Descriptor
            // (PSD1):  This field indicates the characteristics of power state
            // 18 byte 2656:2687. O - Power State 19 Descriptor (PSD1):  This
            // field indicates the characteristics of power state 19 byte
            // 2688:2719. O - Power State 20 Descriptor (PSD1):  This field
            // indicates the characteristics of power state 20 byte 2720:2751. O
            // - Power State 21 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 21 byte 2752:2783. O - Power State
            // 22 Descriptor (PSD1):  This field indicates the characteristics
            // of power state 22 byte 2784:2815. O - Power State 23 Descriptor
            // (PSD1):  This field indicates the characteristics of power state
            // 23 byte 2816:2847. O - Power State 24 Descriptor (PSD1):  This
            // field indicates the characteristics of power state 24 byte
            // 2848:2879. O - Power State 25 Descriptor (PSD1):  This field
            // indicates the characteristics of power state 25 byte 2880:2911. O
            // - Power State 26 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 26 byte 2912:2943. O - Power State
            // 27 Descriptor (PSD1):  This field indicates the characteristics
            // of power state 27 byte 2944:2975. O - Power State 28 Descriptor
            // (PSD1):  This field indicates the characteristics of power state
            // 28 byte 2976:3007. O - Power State 29 Descriptor (PSD1):  This
            // field indicates the characteristics of power state 29 byte
            // 3008:3039. O - Power State 30 Descriptor (PSD1):  This field
            // indicates the characteristics of power state 30 byte 3040:3071. O
            // - Power State 31 Descriptor (PSD1):  This field indicates the
            // characteristics of power state 31
...
//
// Namespace Identfier Length (NIDL) for a given type defined by
// NVME_IDENTIFIER_TYPE
//
typedef enum {
...
typedef struct {
  ...
  UCHAR Reserved[2];
  UCHAR NID[ANYSIZE_ARRAY]; // Namespace Identifier (Based on
                            // NVME_IDENTIFIER_TYPE)
...
typedef struct {
  ...
  LBAEF[16]; // byte 2816:2831 M - LBA Format 0 Extension (LBAFE0)
  ...
  UCHAR VS[256]; // byte 3840:4095 O - Vendor Specific (VS): This range of bytes
                 // is allocated for vendor specific usage.
...
//
// Output of NVME_IDENTIFY_CNS_SPECIFIC_CONTROLLER_IO_COMMAND_SET (0x06) with
// Command Set Identifier (0x00)
//
typedef struct {
...
//
// Output of NVME_IDENTIFY_CNS_CONTROLLER_LIST_OF_NSID
// (0x12)/NVME_IDENTIFY_CNS_CONTROLLER_LIST_OF_NVM_SUBSYSTEM (0x13)
//
typedef struct {
...
#define NVME_UUID_ASSOCIATION_PCI_VID                                          \
  1 // UUID is associated with the vendor reported in the PCI Vendor ID field of
    // Identify Controller data
#define NVME_UUID_ASSOCIATION_PCI_SUBSYSTEM_VID                                \
  2 // UUID is associated with vendor reported in the PCI subsystem vendor ID
    // field of Identify Controller data
#define NVME_UUID_ASSOCIATION_RESERVED 3

typedef struct {

  UCHAR IdentifierAssociation
      : 2; // Byte 0, indicates whether the UUID is associated with a vendor.
  UCHAR Reserved : 6;
...
#define NVME_MAX_UUID_INDEX                                                    \
  127 // Index for last entry in a UUID List.  Note, NVM Express
      // requires UUID List Entry at index 127 shall be cleared to 0h.

typedef struct {

  NVME_UUID_LIST_ENTRY
  UUID[NVME_NUM_UUID_LIST_ENTRIES]; // Note, UUID[0] is reserved.
...
typedef struct {
  ...
  ULONGLONG NVMCommandSet
      : 1; // Bit 0, indicates whether this vector supports NVM command set.
  ULONGLONG KVCommandSet : 1; // Bit 1, indicates whether this vector supports
                              // Key Value command set.
  ULONGLONG ZNCommandSet : 1; // Bit 2, indicates whether this vector supports
...
typedef struct {
  ...
  struct {
    ...
    UCHAR Hidden : 1;
    UCHAR Reserved : 6;
  } Attributes; // Attributes: Specifies attributes of the LBA range. Each bit
                // defines an attribute.

  UCHAR Reserved0[14];
  ULONGLONG
  SLBA; // Starting LBA (SLBA): This field specifies the 64-bit address of
        // the first logical block that is part of this LBA range.
  ULONGLONG NLB;  // Number of Logical Blocks (NLB): This field specifies the
                  // number of logical blocks that are part of this LBA range.
                  // This is a 0s based value.
  UCHAR GUID[16]; // Unique Identifier (GUID): This field is a global unique
                  // identifier that uniquely specifies the type of this LBA
                  // range. Well known Types may be defined and are published on
...
typedef enum {
  NVME_LOG_PAGE_OCP_DEVICE_SMART_INFORMATION =
      0xC0, // OCP device SMART Information log page
  NVME_LOG_PAGE_OCP_DEVICE_ERROR_RECOVERY =
      0xC1, // OCP device Error Recovery log page
  NVME_LOG_PAGE_OCP_FIRMWARE_ACTIVATION_HISTORY =
      0xC2, // OCP device Firmware Activation History log page
  NVME_LOG_PAGE_OCP_LATENCY_MONITOR =
      0xC3, // OCP device Latency Monitor log page
  NVME_LOG_PAGE_OCP_DEVICE_CAPABILITIES =
      0xC4, // OCP device Device Capabilities log page
  NVME_LOG_PAGE_OCP_UNSUPPORTED_REQUIREMENTS =
      0xC5, // OCP device Unsupported Requirements log page
  NVME_LOG_PAGE_OCP_TCG_CONFIGURATION =
      0xC8,                            // OCP device TCG Configuration log page
...
#define NVME_LOG_PAGE_WCS_DEVICE_SMART_ATTRIBUTES                              \
  NVME_LOG_PAGE_OCP_DEVICE_SMART_INFORMATION // WCS device SMART Attributes log
                                             // page
#define NVME_LOG_PAGE_WCS_DEVICE_ERROR_RECOVERY                                \
  NVME_LOG_PAGE_OCP_DEVICE_ERROR_RECOVERY // WCS device Error Recovery log page

//
// SMART Attributes Log Page GUID is defined in spec as byte stream:
// 0xAFD514C97C6F4F9CA4F2BFEA2810AFC5 which is converted to GUID format as:
// {2810AFC5-BFEA-A4F2-9C4F-6F7CC914D5AF}
//
#define GUID_OCP_DEVICE_SMART_INFORMATIONGuid                                  \
  {0x2810AFC5, 0xBFEA, 0xA4F2, {0x9C, 0x4F, 0x6F, 0x7C, 0xC9, 0x14, 0xD5, 0xAF}}
DEFINE_GUID(GUID_OCP_DEVICE_SMART_INFORMATION, 0x2810AFC5, 0xBFEA, 0xA4F2, 0x9C,
...
//
// Error Recovery Log Page GUID is defined in spec as byte stream:
// 0x5A1983BA3DFD4DABAE3430FE2131D944 which is converted to GUID format as:
// {2131D944-30FE-AE34-AB4D-FD3DBA83195A}
//
#define GUID_OCP_DEVICE_ERROR_RECOVERYGuid                                     \
  {0x2131D944, 0x30FE, 0xAE34, {0xAB, 0x4D, 0xFD, 0x3D, 0xBA, 0x83, 0x19, 0x5A}}
DEFINE_GUID(GUID_OCP_DEVICE_ERROR_RECOVERY, 0x2131D944, 0x30FE, 0xAE34, 0xAB,
...
//
// Firmware Activation History Log Page GUID is defined in spec as byte stream:
// 0xD11CF3AC8AB24DE2A3F6DAB4769A796D which is converted to GUID format as:
// {769A796D-DAB4-A3F6-E24D-B28AACF31CD1}
//
#define GUID_OCP_DEVICE_FIRMWARE_ACTIVATION_HISTORYGuid                        \
  {0x769A796D, 0xDAB4, 0xA3F6, {0xE2, 0x4D, 0xB2, 0x8A, 0xAC, 0xF3, 0x1C, 0xD1}}
DEFINE_GUID(GUID_OCP_DEVICE_FIRMWARE_ACTIVATION_HISTORY, 0x769A796D, 0xDAB4,
            0xA3F6, 0xE2, 0x4D, 0xB2, 0x8A, 0xAC, 0xF3, 0x1C, 0xD1);

//
// Latency Monitor Log Page GUID is defined in spec as byte stream:
// 0x85D45E58D4E643709C6C84D08CC07A92 which is converted to GUID format as:
// {8CC07A92-84D0-9C6C-7043-E6D4585ED485}
//
#define GUID_OCP_DEVICE_LATENCY_MONITORGuid                                    \
  {0x8CC07A92, 0x84D0, 0x9C6C, {0x70, 0x43, 0xE6, 0xD4, 0x58, 0x5E, 0xD4, 0x85}}
DEFINE_GUID(GUID_OCP_DEVICE_LATENCY_MONITOR, 0x8CC07A92, 0x84D0, 0x9C6C, 0x70,
            0x43, 0xE6, 0xD4, 0x58, 0x5E, 0xD4, 0x85);

//
// Device Capabilities Log Page GUID is defined in spec as byte stream:
// 0xB7053C914B58495D98C9E1D10D054297 which is converted to GUID format as:
// {0D054297-E1D1-98C9-5D49-584B913C05B7}
//
#define GUID_OCP_DEVICE_DEVICE_CAPABILITIESGuid                                \
  {0x0D054297, 0xE1D1, 0x98C9, {0x5D, 0x49, 0x58, 0x4B, 0x91, 0x3C, 0x05, 0xB7}}
DEFINE_GUID(GUID_OCP_DEVICE_DEVICE_CAPABILITIES, 0x0D054297, 0xE1D1, 0x98C9,
            0x5D, 0x49, 0x58, 0x4B, 0x91, 0x3C, 0x05, 0xB7);

//
// Unsupported Requirements Log Page GUID is defined in spec as byte stream:
// 0xC7BB98B7D0324863BB2C23990E9C722F which is converted to GUID format as:
// {0E9C722F-2399-BB2C-6348-32D0B798BBC7}
//
#define GUID_OCP_DEVICE_UNSUPPORTED_REQUIREMENTSGuid                           \
  {0x0E9C722F, 0x2399, 0xBB2C, {0x63, 0x48, 0x32, 0xD0, 0xB7, 0x98, 0xBB, 0xC7}}
DEFINE_GUID(GUID_OCP_DEVICE_UNSUPPORTED_REQUIREMENTS, 0x0E9C722F, 0x2399,
            0xBB2C, 0x63, 0x48, 0x32, 0xD0, 0xB7, 0x98, 0xBB, 0xC7);

//
// TCG Configuration Log Page GUID is defined in spec as byte stream:
// 0x54E02A9DFA5447C083E6E07EBD244006 which is converted to GUID format as:
// {BD244006-E07E-83E6-C047-54FA9D2AE054}
//
#define GUID_OCP_DEVICE_TCG_CONFIGURATIONGuid                                  \
  {0xBD244006, 0xE07E, 0x83E6, {0xC0, 0x47, 0x54, 0xFA, 0x9D, 0x2A, 0xE0, 0x54}}
DEFINE_GUID(GUID_OCP_DEVICE_TCG_CONFIGURATION, 0xBD244006, 0xE07E, 0x83E6, 0xC0,
            0x47, 0x54, 0xFA, 0x9D, 0x2A, 0xE0, 0x54);

//
// TCG History Log Page GUID is defined in spec as byte stream:
// 0x88D7909696D04E27949009C6704b513E which is converted to GUID format as:
// {704B513E-09C6-9490-274E-D0969690D788}
//
#define GUID_OCP_DEVICE_TCG_HISTORYGuid                                        \
  {0x704B513E, 0x09C6, 0x9490, {0x27, 0x4E, 0xD0, 0x96, 0x96, 0x90, 0xD7, 0x88}}
DEFINE_GUID(GUID_OCP_DEVICE_TCG_HISTORY, 0x704B513E, 0x09C6, 0x9490, 0x27, 0x4E,
            0xD0, 0x96, 0x96, 0x90, 0xD7, 0x88);

//
// MFND child controller event Log Page GUID is defined in spec as byte stream:
// 0x9C669D257FD944A5BF35A5F098BCCE18 which is converted to GUID format as:
// {98BCCE18-A5F0-BF35-A544-D97F259D669C}
//
#define GUID_MFND_CHILD_CONTROLLER_EVENT_LOG_PAGEGuid                          \
  {0x98BCCE18, 0xA5F0, 0xBF35, {0xA5, 0x44, 0xD9, 0x7F, 0x25, 0x9D, 0x66, 0x9C}}
DEFINE_GUID(GUID_MFND_CHILD_CONTROLLER_EVENT_LOG_PAGE, 0x98BCCE18, 0xA5F0,
            0xBF35, 0xA5, 0x44, 0xD9, 0x7F, 0x25, 0x9D, 0x66, 0x9C);

//
// MFND child controller QoS statistics Log Page GUID is defined in spec as byte
// stream: 0x0F5F578400403E87464406529CB5FA26 which is converted to GUID format
// as: {9CB5FA26-0652-4644-873E-400084575F0F}
//
#define GUID_MFND_CHILD_CONTROLLER_QOS_STAT_LOG_PAGEGuid                       \
  {0x9CB5FA26, 0x0652, 0x4644, {0x87, 0x3E, 0x40, 0x00, 0x84, 0x57, 0x5F, 0x0F}}
DEFINE_GUID(GUID_MFND_CHILD_CONTROLLER_QOS_STAT_LOG_PAGE, 0x9CB5FA26, 0x0652,
            0x4644, 0x87, 0x3E, 0x40, 0x00, 0x84, 0x57, 0x5F, 0x0F);
...
//
// Device recommended reset action on firmware assert for Windows Cloud Server
// Devices
//
...
typedef enum _NVME_WCS_DEVICE_RECOVERY_ACTION1 {
  ...
  NVMeDeviceRecovery1Max = 15 // Not an actual action, denotes max action.
    ...
    *PNVME_WCS_DEVICE_RECOVERY_ACTION1;
...
typedef enum _NVME_WCS_DEVICE_RECOVERY_ACTION2 {
  ...
  NVMeDeviceRecovery2Max = 15 // Not an actual action, denotes max action.
    ...
    *PNVME_WCS_DEVICE_RECOVERY_ACTION2;

#pragma pack(push, 1)

//
// Log page definition of
// NVME_LOG_PAGE_WCS_DEVICE_SMART_ATTRIBUTES/NVME_LOG_PAGE_OCP_DEVICE_SMART_INFORMATION.
// Size 512 bytes
//
...
typedef struct _NVME_WCS_DEVICE_SMART_ATTRIBUTES_LOG_V2 {
  ...
  USHORT LogPageVersionNumber; // Shall be set to
...
typedef struct _NVME_OCP_DEVICE_SMART_INFORMATION_LOG_V3 {
  ...
  GUID LogPageGUID; // Shall be set to GUID_OCP_DEVICE_SMART_INFORMATION
    ...
    *PNVME_OCP_DEVICE_SMART_INFORMATION_LOG_V3;

C_ASSERT(sizeof(NVME_OCP_DEVICE_SMART_INFORMATION_LOG_V3) ==
         sizeof(NVME_WCS_DEVICE_SMART_ATTRIBUTES_LOG));

//
// Log page definition of NVME_LOG_PAGE_WCS_DEVICE_ERROR_RECOVERY. Size 512
// bytes
//
...
typedef struct _NVME_WCS_DEVICE_ERROR_RECOVERY_LOG {
  ...
  // Amount of time to wait for device panic workflow to complete in msec. Delay
  // the reset accordingly
  ...
  // CDW12 value for the Vendor Specific command to recover device from panic
  // condition
  ULONG VendorSpecificCommandCDW12;

  // CDW13 value for the Vendor Specific command to recover device from panic
  // condition
...
typedef struct _NVME_WCS_DEVICE_ERROR_RECOVERY_LOG
    NVME_WCS_DEVICE_ERROR_RECOVERY_LOG_V1,
    *PNVME_WCS_DEVICE_ERROR_RECOVERY_LOG_V1;

C_ASSERT(sizeof(NVME_WCS_DEVICE_ERROR_RECOVERY_LOG_V1) ==
         sizeof(NVME_WCS_DEVICE_ERROR_RECOVERY_LOG));
...
typedef struct _NVME_OCP_DEVICE_ERROR_RECOVERY_LOG_V2 {
  ...
  GUID LogPageGUID; // Shall be set to GUID_OCP_DEVICE_ERROR_RECOVERY
    ...
    *PNVME_OCP_DEVICE_ERROR_RECOVERY_LOG_V2;

C_ASSERT(sizeof(NVME_OCP_DEVICE_ERROR_RECOVERY_LOG_V2) ==
         sizeof(NVME_WCS_DEVICE_ERROR_RECOVERY_LOG));

//
// Log page definition of NVME_LOG_PAGE_OCP_FIRMWARE_ACTIVATION_HISTORY. Size
// 4096 bytes
//
...
//
// Log page definition of NVME_LOG_PAGE_OCP_UNSUPPORTED_REQUIREMENTS. Size 4096
// bytes
//
...
typedef struct _UNSUPPORTED_REQUIREMENT {
  ...
  UCHAR
  ReqId[16]; // Zero padded ASCII string of the requirement id not supported
...
typedef struct _NVME_OCP_DEVICE_UNSUPPORTED_REQUIREMENTS_LOG {
  ...
  GUID LogPageGUID; // Shall be set to GUID_OCP_DEVICE_UNSUPPORTED_REQUIREMENTS
    ...
    *PNVME_OCP_DEVICE_UNSUPPORTED_REQUIREMENTS_LOG;
...
typedef struct _NVME_OCP_DEVICE_TCG_CONFIGURATION_LOG {
  ...
  GUID LogPageGUID; // Shall be set to GUID_OCP_DEVICE_TCG_CONFIGURATION
    ...
    *PNVME_OCP_DEVICE_TCG_CONFIGURATION_LOG;
...
//
// Parameters for NVME_ADMIN_COMMAND_DELETE_IO_CQ and
...
//
// Output and Parameters for NVME_ADMIN_COMMAND_GET_FEATURES or
...
typedef union {
  ...
  struct {
    ...
    ULONG SEL : 3; // Select (SEL): This field specifies which value of the
...
typedef struct {
  ...
  struct {
    ...
    ULONGLONG Synch : 1;      // 0 = controller counted time continuously, 1 =
                              // controller may have stopped
    ULONGLONG Origin : 3; // 000 = controller set timestamp to 0 at reset, 001 =
                          // timestamp set via Set Features
...
typedef union {
  ...
  struct {
    ULONG NOPPME
        : 1; // Non-Operational Power State Permissive Mode Enable (NOPPME)
    ULONG Reserved0 : 31;
  } DUMMYSTRUCTNAME;
...
typedef union {
  ...
  struct {
    ...
    ULONG PredictableLogChangeNotices
        : 1; // Predictable Latency Event Aggregate Log Change Notices
    ULONG LBAStatusNotices : 1; // LBA Status Information Notices
    ULONG EnduranceEventNotices
        : 1; // Endurance Group Event Aggregate Log Change Notices
...
//
// Parameter for NVME_FEATURE_AUTONOMOUS_POWER_STATE_TRANSITION
// There is an array of 32 of these (one for each power state) in the data
// buffer.
//
typedef struct {
  ULONG Reserved0 : 3; // Bits 0-2 are reserved.
  ULONG IdleTransitionPowerState
      : 5; // Bits 3-7 - (ITPS) The non-operational power state for the
           // controller to autonomously transition to after there is a
           // continuous period of idle time in the current power state that
           // exceeds time specified in the ITPT field.
  ULONG IdleTimePriorToTransition
      : 24; // Bits 8-31 - (ITPT) The amount of idle time (in ms) that occurs in
            // this power state prior to transitioning to the Idle Transition
            // Power State.  A value of 0 disables APST for this power state.
...
typedef union {
  ...
  struct {
    ULONG TMPTH : 16; // Temperature Threshold (TMPTH):  Indicates the threshold
                      // for the temperature of the overall device (controller
                      // and NVM included) in units of Kelvin.
...
typedef union {
  struct {
    ...
    ULONG DULBE
        : 1; // Deallocated or unwritten logical block error enable (DULBE)
    ULONG Reserved0 : 15;
  } DUMMYSTRUCTNAME;
  ULONG AsUlong;
...
typedef union {
  struct {
    ...
    ULONG MR : 1;  // Memory Return (MR) - Indicates if the host is returning
                   // previously allocated memory to the controller.
    ULONG Reserved : 30;
  } DUMMYSTRUCTNAME;
...
typedef union {
  struct {
    ULONG HSIZE; // Host Memory Buffer Size (HSIZE) - The size of the host
                 // memory buffer in memory page size (CC.MPS) units.
  } DUMMYSTRUCTNAME;
...
typedef union {
  struct {
    ULONG Reserved : 4;
    ULONG HMDLLA : 28; // Host Memory Descriptor List Lower Address (HMDLLA) -
                       // 16-byte aligned, lower 32 bits of the physical
                       // location of the Host Memory Descriptor List.
  } DUMMYSTRUCTNAME;
...
typedef union {
  struct {
    ULONG HMDLUA; // Host Memory Descriptor List Upper Address (HMDLLA) - Upper
                  // 32 bits of the physical location of the Host Memory
                  // Descriptor List.
  } DUMMYSTRUCTNAME;
...
typedef union {
  struct {
    ULONG HMDLEC; // Host Memory Descriptor List Entry Count (HMDLEC) - Number
                  // of entries in the Host Memory Descriptor List.
  } DUMMYSTRUCTNAME;
...
//
typedef struct {
  ULONGLONG BADD; // Buffer Address (BADD) - Physical host memory address
                  // aligned to the memory page size (CC.MPS)
  ULONG BSIZE;    // Buffer Size (BSIZE) - The number of contiguous memory page
                  // size (CC.MPS) units for this entry.
  ULONG Reserved;
...
//
// Parameters for NVME_FEATURE_ENHANDED_CONTROLLER_METADATA,
// NVME_FEATURE_CONTROLLER_METADATA, NVME_FEATURE_NAMESPACE_METADATA
//
typedef union {
...
typedef union {
  ...
  struct {
    ...
    ULONG EA : 2; // Element Action (EA), value defined in enum
                  // NVME_HOST_METADATA_ELEMENT_ACTIONS
...
typedef struct {
  ...
  ULONG ET : 6; // Element Type (ET), value defined in enum
                // NVME_CONTROLLER_METADATA_ELEMENT_TYPES,
                // NVME_NAMESPACE_METADATA_ELEMENT_TYPES
...
typedef struct {
  ...
  UCHAR MetadataElementDescriptors
      [4094]; // Use NVME_HOST_METADATA_ELEMENT_DESCRIPTOR to access this list.
...
//
// DWORD 0 for get feature command (Error Injection) shares the same format with
// DWORD 11 for set feature command (Error Injection).
//
...
typedef struct {
  ...
  union {
    ...
    struct {
      ...
      UCHAR Enable
          : 1; // A value of 0 indicates error injection is not enabled. A value
               // of 1 indicates error injection is enabled.

      UCHAR SingleInstance
          : 1; // A value of 0 indicates error injection is enabled until
               // disable. A value of 1 indicates a single instance error
               // injection where a single error is injected. After a single
               // instance error has been created, the value of the Enable field
               // shall be 0 in the results from Get Features command.
      UCHAR Reserved0 : 6;
  ...
  UCHAR ErrorInjectionTypeSpecific[28]; // Error Injection Type specific
                                        // definition.
...
//
// NVM Express Log Page Identifier values for NVME_ADMIN_COMMAND_GET_LOG_PAGE
// Command
//
typedef enum {
  ...
  NVME_LOG_PAGE_CHANGED_ZONE_LIST =
      0xBF, // NVM Express Zoned Namespace Command Set
...
//
// Get Log Page CDW10 format for NVM Express specification revisions 1.3a
// thru 1.4, inclusive.
//
typedef union {
...
//
// Get Log Page CDW10 format for NVM Express Base Specification revision 2.0a
// and above.
//
typedef union {
...
//
// Defined values for bits 09:08 of Log Specific Field (LSP) in CDW10 for Get
// Log Page, Persistent Event Log (Log Identifier 0Dh) NVM Express Base
...
typedef union {
  ...
  struct {
    ...
    ULONG LogSpecificIdentifier : 16; // Log Specific Identifier  (Reserved
                                      // prior to NVM Express 1.4 specification)
  } DUMMYSTRUCTNAME;
...
//
// LID Supported and Effects Data Structure - LID Specific Field for Log Page
// Identifier 0Dh (NVME_LOG_PAGE_PERSISTENT_EVENT_LOG)
//
...
} NVME_LID_SPECIFIC_PERSISTENT_EVENT_LOG,
    *PNVME_LID_SPECIFIC_PERSISTENT_EVENT_LOG;
...
#define NVME_MAX_LOG_PAGE_IDENTIFIER                                           \
  0xFF // Highest legal Log Page Identifier value.

//
// Get Log Pages - Supported Log Pages - Log (Log Identifier 00h). Size: 1024
// bytes.
//

typedef struct {

  NVME_LID_SUPPORTED_AND_EFFECTS
  LogPageIdentifierSupported[NVME_NUM_LOG_PAGE_IDENTIFIERS];
...
typedef struct {
  ...
  NVME_COMMAND_STATUS
  Status; // Status Field: This field indicates the Status Field for the
          // command  that completed.  The Status Field is located in bits
          // 15:01, bit 00 corresponds to the Phase Tag posted for the
          // command.
  ...
  ULONGLONG Lba; // LBA: This field indicates the first LBA that experienced the
                 // error condition, if applicable.
  ULONG NameSpace; // Namespace: This field indicates the namespace that the
                   // error is associated with, if applicable.
  ...
  ULONGLONG CommandSpecificInfo; // This field contains command specific
                                 // information. If used, the command definition
                                 // specifies the information returned.

  USHORT TransportTypeSpecificInfo; // This field contains the transport type
...
typedef struct {
  ...
  union {
    ...
    struct {
      UCHAR AvailableSpaceLow : 1; // If set to 1, then the available spare
                                   // space has fallen below the threshold.
      UCHAR TemperatureThreshold
          : 1; // If set to 1, then a temperature is above an over temperature
               // threshold or below an under temperature threshold.
      UCHAR ReliabilityDegraded
          : 1; // If set to 1, then the device reliability has been degraded due
               // to significant media related  errors or any internal error
               // that degrades device reliability.
      UCHAR ReadOnly
          : 1; // If set to 1, then the media has been placed in read only mode
      UCHAR VolatileMemoryBackupDeviceFailed
          : 1; // If set to 1, then the volatile memory backup device has
               // failed. This field is only valid if the controller has a
               // volatile memory backup solution.
      UCHAR Reserved : 3;
    } DUMMYSTRUCTNAME;

    UCHAR AsUchar;

  } CriticalWarning; // This field indicates critical warnings for the state of
                     // the  controller. Each bit corresponds to a critical
                     // warning type; multiple bits may be set.

  UCHAR Temperature[2]; // Temperature: Contains the temperature of the overall
                        // device (controller and NVM included) in units of
                        // Kelvin. If the temperature exceeds the temperature
                        // threshold, refer to section 5.12.1.4, then an
                        // asynchronous event completion may occur
  UCHAR AvailableSpare; // Available Spare:  Contains a normalized percentage (0
                        // to 100%) of the remaining spare capacity available
  UCHAR
  AvailableSpareThreshold; // Available Spare Threshold:  When the Available
                           // Spare falls below the threshold indicated in
                           // this field, an asynchronous event  completion
                           // may occur. The value is indicated as a
                           // normalized percentage (0 to 100%).
  UCHAR PercentageUsed;    // Percentage Used
  UCHAR Reserved0[26];

  UCHAR DataUnitRead
      [16]; // Data Units Read:  Contains the number of 512 byte data units the
            // host has read from the controller; this value does not include
            // metadata. This value is reported in thousands (i.e., a value of 1
            // corresponds to 1000 units of 512 bytes read)  and is rounded up.
            // When the LBA size is a value other than 512 bytes, the controller
            // shall convert the amount of data read to 512 byte units. For the
            // NVM command set, logical blocks read as part of Compare and Read
            // operations shall be included in this value
  UCHAR DataUnitWritten
      [16]; // Data Units Written: Contains the number of 512 byte data units
            // the host has written to the controller; this value does not
            // include metadata. This value is reported in thousands (i.e., a
            // value of 1 corresponds to 1000 units of 512 bytes written)  and
            // is rounded up.  When the LBA size is a value other than 512
            // bytes, the controller shall convert the amount of data written to
            // 512 byte units. For the NVM command set, logical blocks written
            // as part of Write operations shall be included in this value.
            // Write Uncorrectable commands shall not impact this value.
  UCHAR HostReadCommands[16]; // Host Read Commands:  Contains the number of
                              // read commands  completed by  the controller.
                              // For the NVM command set, this is the number of
                              // Compare and Read commands.
  UCHAR HostWrittenCommands[16]; // Host Write Commands:  Contains the number of
                                 // write commands  completed by  the
                                 // controller. For the NVM command set, this is
                                 // the number of Write commands.
  UCHAR ControllerBusyTime
      [16]; // Controller Busy Time:  Contains the amount of time the controller
            // is busy with I/O commands. The controller is busy when there is a
            // command outstanding to an I/O Queue (specifically, a command was
            // issued via an I/O Submission Queue Tail doorbell write and the
            // corresponding  completion queue entry  has not been posted yet to
            // the associated I/O Completion Queue). This value is reported in
            // minutes.
  UCHAR PowerCycle[16]; // Power Cycles: Contains the number of power cycles.
  UCHAR
  PowerOnHours[16]; // Power On Hours: Contains the number of power-on
                    // hours. This does not include time that the controller
                    // was powered and in a low power state condition.
  UCHAR UnsafeShutdowns[16]; // Unsafe Shutdowns: Contains the number of unsafe
                             // shutdowns. This count is incremented when a
                             // shutdown notification (CC.SHN) is not received
                             // prior to loss of power.
  UCHAR MediaErrors[16]; // Media Errors:  Contains the number of occurrences
                         // where the controller detected an unrecovered data
                         // integrity error. Errors such as uncorrectable ECC,
                         // CRC checksum failure, or LBA tag mismatch are
                         // included in this field.
  UCHAR
  ErrorInfoLogEntryCount[16]; // Number of Error Information Log Entries:
  ...
  WarningCompositeTemperatureTime; // Warning Composite Temperature Time:
                                   // Contains the amount of time in minutes
                                   // that the controller is operational and
                                   // the Composite Temperature is greater
                                   // than or equal to the Warning Composite
                                   // Temperature Threshold (WCTEMP) field
                                   // and less than the Critical Composite
                                   // Temperature Threshold (CCTEMP) field
                                   // in the Identify Controller data
                                   // structure
  ULONG CriticalCompositeTemperatureTime; // Critical Composite Temperature
                                          // Time: Contains the amount of time
                                          // in minutes that the controller is
                                          // operational and the Composite
                                          // Temperature is greater the Critical
                                          // Composite Temperature Threshold
                                          // (CCTEMP) field in the Identify
                                          // Controller data structure
  USHORT TemperatureSensor1; // Contains the current temperature reported by
                             // temperature sensor 1.
  USHORT TemperatureSensor2; // Contains the current temperature reported by
                             // temperature sensor 2.
  USHORT TemperatureSensor3; // Contains the current temperature reported by
                             // temperature sensor 3.
  USHORT TemperatureSensor4; // Contains the current temperature reported by
                             // temperature sensor 4.
  USHORT TemperatureSensor5; // Contains the current temperature reported by
                             // temperature sensor 5.
  USHORT TemperatureSensor6; // Contains the current temperature reported by
                             // temperature sensor 6.
  USHORT TemperatureSensor7; // Contains the current temperature reported by
                             // temperature sensor 7.
  USHORT TemperatureSensor8; // Contains the current temperature reported by
...
#define NVME_TELEMETRY_DATA_BLOCK_SIZE                                         \
  0x200 // All NVMe Telemetry Data Blocks are 512 bytes in size.
...
typedef struct {
  ...
  struct {
    UCHAR ActiveSlot : 3; // Bits 2:0 indicates the firmware slot that contains
                          // the actively running firmware revision.
    UCHAR Reserved0 : 1;
    UCHAR PendingActivateSlot
        : 3; // Bits 6:4 indicates the firmware slot that is going to be
             // activated at the next controller reset.
    UCHAR Reserved1 : 1;
  ...
  ULONGLONG
  FRS[7]; // Firmware Revision for Slot 1 - 7(FRS1 - FRS7):  Contains the
          // revision of the firmware downloaded to firmware slot 1 - 7.
...
typedef struct {
  ...
  ULONGLONG ZoneIdentifier[511]; // List of Zone Identifiers upto 511 entries.
                                 // Identifier contains Zone Start Logical Block
                                 // Address(ZSLBA)
...
typedef struct {
  ...
  struct {
    UCHAR Result : 4; // Result of Device Self-Test operation of this particular
                      // result data
  ...
  struct {
    UCHAR NSIDValid
        : 1; // If set to 1, the contents of Namespace Identifier field is valid
    UCHAR FLBAValid
        : 1; // If set to 1, the contents of Failing LBA field is valid
    UCHAR SCTValid
        : 1; // If set to 1, the contents of Status Code Type field is valid
    UCHAR SCValid
        : 1; // If set to 1, the contents of Status Code field is valid
    UCHAR Reserved : 4;
  } ValidDiagnostics;
  ...
  ULONGLONG FailingLBA; // Failed LBA which caused test to fail. Only valid if
                        // FLBAValid is set

  struct {
    UCHAR AdditionalInfo
        : 3; // Additional information related to errors/conditions. Only valid
             // if SCTValid is set
    UCHAR Reserved : 5;
  } StatusCodeType;

  UCHAR StatusCode; // Additional information related to errors/conditons. Only
                    // valid if SCValid is set
...
typedef struct {
  ...
  struct {
    UCHAR CompletePercent : 7; // Percentage of completion of Device Self-Test
                               // operation. Valid if Status field is non-zero.
    UCHAR Reserved : 1;
  } CurrentCompletion;
  ...
  ResultData[20]; // Last 20 Self-Test Result Data, latest to oldest
                  // available in sorted order

} NVME_DEVICE_SELF_TEST_LOG, *PNVME_DEVICE_SELF_TEST_LOG;

//
// Information of log: NVME_LOG_PAGE_ENDURANCE_GROUP_INFORMATION. Size: 512
// bytes
//
typedef struct {

  ULONG Reserved0;
  UCHAR AvailableSpareThreshold; // Available spare indicated as normalized
                                 // percentage (0-100)
  UCHAR PercentageUsed; // Vendor specific estimate of percentage of life used
                        // for the NVM set(s) of Endurance Group (Billion Unit)

  UCHAR Reserved1[26];
  UCHAR
  EnduranceEstimate[16];   // Estimate of total number of data bytes written
                           // to NVM set(s) of Endurance Group (Billion Unit)
  UCHAR DataUnitsRead[16]; // Total number of data bytes read from NVM set(s) of
                           // Endurance Group (Billion Unit)
  UCHAR DataUnitsWritten[16]; // Total number of data bytes written to NVM
                              // sets(s) of Endurance Group (Billion Unit)
                              // Includes only host writes

  UCHAR MediaUnitsWritten[16]; // Total number of data bytes written to NVM
                               // sets(s) of Endurance Group (Billion Unit)
...
//
// Information of log: NVME_LOG_PAGE_PERSISTENT_EVENT_LOG. Header Size: 512
// bytes
//
typedef struct {
  ...
  ULONG TotalNumberOfEvents; // Bytes 4-7   - Contains the number of event
                             // entries in the log.
  ULONGLONG TotalLogLength;  // Bytes 8-15  - Contains the total number of bytes
                             // of persistent event log page data available,
                             // including the header.
  UCHAR LogRevision; // Bytes 16    - Contains a number indicating the revision
                     // of the Get Log Page data structure that this log page
                     // data complies with.
  UCHAR Reserved1;   // Bytes 17
  USHORT LogHeaderLength; // Bytes 18-19 - Contains the length in bytes of the
                          // log header information that follows. The total
                          // length of the log header in bytes is the value in
                          // this field plus 20.
  ULONGLONG Timestamp;    // Bytes 20-27
  UCHAR
  PowerOnHours[16]; // Bytes 28-43 - Indicates the number of power-on hours
                    // at the time the Persistent Event log was retrieved.
  ULONGLONG PowerCycleCount; // Bytes 44-51 - Contains the number of power
                             // cycles for the controller.
  USHORT PciVendorId; // Bytes 52-53 - Same value as reported in the Identify
                      // Controller data PCI Vendor ID field.
  USHORT PciSubsystemVendorId; // Bytes 54-55 - Same value as reported in the
                               // Identify Controller data PCI Subsystem Vendor
                               // ID field.
  UCHAR SerialNumber[20];      // Bytes 56-75 - Same value as reported in the
                               // Identify Controller data Serial Number field.
  UCHAR ModelNumber[40];       // Bytes 76-115 - Same value as reported in the
                               // Identify Controller data Model Number field.
  UCHAR NVMSubsystemNVMeQualifiedName[256]; // Bytes 116-371 - Same value as
                                            // reported in the Identify
                                            // Controller data.
  UCHAR Reserved[108];                      // Bytes 372-479
  UCHAR
  SupportedEventsBitmap[32]; // Bytes 480-511 - Contains a bitmap indicating
                             // support for the persistent event log events.
...
typedef struct {
  ...
  UCHAR EventTypeRevision; // Byte 1      - Contains a number indicating the
                           // revision of the event data.
  UCHAR EventHeaderLength; // Byte 2      - Contains the length in bytes of the
                           // event header information that follows.
  UCHAR Reserved0;         // Byte 3
  USHORT ControllerIdentifier; // Bytes 4-5   - Contains the NVM subsystem
                               // unique controller identifier for the
                               // controller that created this event.
  ULONGLONG EventTimestamp;    // Bytes 6-13
  UCHAR Reserved1[6];          // Bytes 14-19
  USHORT VendorSpecificInformationLength; // Bytes 20-21 - Indicates the length
                                          // in bytes of the Vendor Specific
                                          // Information.
  USHORT EventLength; // Bytes 22-23 - Indicates the length in bytes of the
...
//
// Information of log: NVME_LOG_PAGE_FEATURE_IDENTIFIERS_SUPPORTED_AND_EFFECTS
// (Log Identifier: 12h). Size: 1024 bytes Note: This log page describes the
// FIDs that are supported on the interface to which the Get Log Page command
// was submitted and the effects of those features on the state of the NVM
// subsystem.
//
...
typedef struct {
  ...
  FeatureIdentifierSupported[NVME_NUM_FID_SUPPORTED];
...
//
// Log Page: NVME_LOG_PAGE_NVME_MI_COMMANDS_SUPPORTED_AND_EFFECTS (Log
// Identifier: 13h). Size: 4096 bytes
//
...
} NVME_NVME_MI_COMMANDS_SUPPORTED_AND_EFFECTS,
    *PNVME_NVME_MI_COMMANDS_SUPPORTED_AND_EFFECTS;
...
typedef struct {
  ...
  NVME_NVME_MI_COMMANDS_SUPPORTED_AND_EFFECTS
  ManagementInterfaceCommandSupported[NVME_NUM_NVME_MI_COMMANDS_SUPPORTED];
  UCHAR Reserved[3 * 1024];

} NVME_NVME_MI_COMMANDS_SUPPORTED_AND_EFFECTS_LOG,
    *PNVME_NVME_MI_COMMANDS_SUPPORTED_AND_EFFECTS_LOG;
...
typedef enum {
  ...
  //
  // The most recent sanitize operation for which No-Deallocate After Sanitize
  // was Requested
...
typedef struct {
  ...
  //
  // This contains the number of completed passes if the most recent sanitize
  // operation was an Overwrite.
  //
  USHORT NumberCompletedPassesOfOverwrite : 4;

  //
  // If set to 1, then no namespace logical block in the NVM subsystem has been
  // written to and no Persistent Memory Region in the NVM subsystem has been
  // enabled since manufactured or most recent successfully sanitized operation.
  //
  // If set to 0, then a namespace logical block in the NVM subsystem has been
  // written to or a Persistent Memory Region in the NVM subsystem has been
  // enabled since manufactured or most recent successfully sanitized operation.
  //
...
typedef struct {
  ...
  // This field indicates the fraction complete of the sanitize operation. The
  // value is a numerator of the fraction complete that has 65536 (10000h) as
  // its denominator. This value shall be set to FFFFh if bits 2:0 of the SSTAT
  // field are not set to 10b.
  //
  USHORT SPROG;

  //
  // Sanitize Status (SSTAT)
  // This field indicates the status associated with the most recent sanitize
  // operation.
  //
  ...
  // This field contains the value of the Command Dword 10 field of the Sanitize
  // command that started the sanitize operation whose status is reported in the
  // SSTAT field.
  //
  ULONG SCDW10;

  //
  // These fields below indicates the number of seconds required to complete the
  // sanitize operation of Overwrite/Block Erase/Crypto Erase methods when the
  // No-Deallocate Modifies Media After Sanitize field is not set to 10b. A
  // value of 0 indicates that the sanitize operation is expected to be
  ...
  //
  // These fields below indicates the number of seconds required to complete the
  // sanitize operation of Overwrite/Block Erase/CryptoErase methods and the
  // associated additional media modification after the sanitize operation. A
  // value of 0 indicates that the sanitize operation is expected to be
  // completed in the background when the Sanitize command that started that
  // operation is completed. A value of FFFFFFFFh indicates that no time period
  // is reported.
...
typedef enum {
  ...
  //
  // The existing image in the specified Firmware Slot is activated at the next
  // Controller
  // Level Reset
  //
  ...
  // Downloaded image replaces the existing image, if any, in the specified
  // Firmware Slot
  // and is then activated immediately. If there is not a newly downloaded
  // image, then the
...
typedef enum {
  ...
  //
  // Media is additionally modified after sanitize completes sucessfully. The
  // Sanitize Operation Completed event
  // does not occur until the additional media modification associated with this
  // field has completed.
  //
...
typedef union {
  ...
  struct {
    ...
    // This field specifies the number of overwrite passes using the data from
    // Overwrite Pattern. A value of 0h specified 16 overwrite passes. This is
    // ignored unless Sanitize Action is Overwrite (011b).
    //
    ULONG OWPASS : 4; // Overwrite Pass Count (OWPASS)

    //
    // Overwrite Invert Pattern Between Passes (OIPBP)
    // This field indicates if Overwrite Pattern shall be inverted between
    // passes. This is ignored unless Sanitize Action is Overwrite (011b).
    //
    ...
    //     controller shall not deallocate any logical blocks after sanitize
    //     completed successfully.
    // If set to 1 and No-Deallocate Inhibited bit is 1, or if set to 0,
    //     controller shall deallocate logical blocks after sanitize completed
    //     successfully.
...
typedef union {
  ...
  struct {
    ...
    //
    // Overwrite Pattern
    // This field is ignored unless the Sanitize Action field in Command Dword
    // 10 is set to 011b (Overwrite). This field specifies a 32-bit pattern that
...
typedef struct {
  ...
  //
  // If the Reseravation Acquire Action is set to 001b (Preempt) or 010b
  // (Preempt and Abort), then this field specifies the reservation key to be
  // unregistered from the namespace. For all other Reservation Acquire Action
  // values, this field is reserved.
  //
...
typedef enum {
  ...
  NVME_RESERVATION_REGISTER_PTPL_STATE_SET_TO_1 =
      3, // Reservations and registrants persist across a power loss.
...
typedef struct {
  ...
  //      a. a Reservation Register command completes successfully on any
  //      controller associated with the namespace; b. a Reservation Release
  //      command with Reservation Release Action set to 001b (Clear) completes
  //      successfully
  //         on any controller associated with the name space;
  //      c. a Reservation Acquire command with Reservation Acquire Action set
  //      to 001b (Preempt) or 010b (Preempt and Abort)
  //         completes successfully on any controller associated with the
  //         namespace.
  // If the value of this field is FFFFFFFFh, then the field shall be cleared to
  // 0b when incremented.
  //
...
typedef struct {
  ...
  USHORT StreamIdentifiers[NVME_STREAMS_GET_STATUS_MAX_IDS]; // Array of stream
                                                             // IDs that are
                                                             // currently open.
...
typedef enum {
  ...
  NVME_ACCESS_FREQUENCY_TYPICAL =
      1, // Typical number of reads and writes expected for this LBA range.
  ...
  NVME_ACCESS_FREQUENCY_ONE_TIME_READ =
      6, // One time read. E.g. command is due to virus scan, backup, file copy,
         // or archive.
  NVME_ACCESS_FREQUENCY_SPECULATIVE_READ =
      7, // Speculative read. The command is part of a prefetch operation.
...
typedef struct {
  ...
  Attributes; // The use of this information is optional and the controller
...
typedef enum {
  ...
  NVME_ZONE_SEND_SET_ZONE_DESCRIPTOR =
      0x10, // Attach Zone Descriptor Extension data to a zone in the Empty
            // state and transition the zone to the Closed state
...
//
// Data structure of CDW13 for Zone Management Receive command and related
// defines
//
typedef enum {
  NVME_ZONE_RECEIVE_REPORT_ZONES = 0, // Returns report zone Descriptors
  NVME_ZONE_RECEIVE_EXTENDED_REPORT_ZONES =
      1, // Returns report zone descriptors with extended report zone
         // information
...
typedef enum {
  ...
  NVME_ZRA_IO_STATE_ZONES =
      2, // List zones with state Zone State Implicitly Opened
  NVME_ZRA_EO_STATE_ZONES =
      3, // List zones with state Zone State Explicitly Opened
...
typedef union {
  ...
  struct {
    ULONG ZRA
        : 8; // Zone Receive Action, as defined in NVME_ZONE_RECEIVE_ACTION
    ULONG ZRASpecific : 8; // Zone Receive Action Specific field, as defined in
                           // NVME_ZONE_RECEIVE_ACTION_SPECIFIC
...
#define NVME_PSDT_XFER_SGL_BYTE                                                \
  1 // SGLs are used for this transfer. If used, Metadata Pointer (MPTR)
    // contains an address
    //  of a single contiguous physical buffer that is byte aligned. Required
    //  for all Admin and I/O commands for NVMe over Fabrics implementations.
#define NVME_PSDT_XFER_SGL_QWORD                                               \
  2 // SGLs are used for this transfer. If used, Metadata Pointer (MPTR)
    // contains an address
    //  of an SGL segment containing exactly one SGL Descriptor that is qword
    //  aligned.
...
//
// If the namespace is not used for the command, then 'NSID' field shall be
// cleared to 0h. If a command shall be applied to all namespaces on the device,
// then 'NSID' field shall be set to FFFFFFFFh.
//
...
C_ASSERT(sizeof(NVME_COMMAND) ==
         64); // NVMe commands are always 64 bytes
              // (defined by constant STORAGE_PROTOCOL_COMMAND_LENGTH_NVME)

//
// The SCSI name string identifier used for the page 83 descriptor in NVMe to
// SCSI translation For NVMe devices compliant with revision 1.0.
//
typedef struct {
...
typedef struct _NVME_SGL_DATABLOCK_DESC {
  ...
  union {
    struct {
      ...
      _Field_range_(NvmeSglDescTypeDataBlock,
                    NvmeSglDescTypeDataBlock) UCHAR Type : 4;
    };
...
typedef struct _NVME_SGL_BITBUCKET_DESC {
  ...
  union {
    struct {
      ...
      _Field_range_(NvmeSglDescTypeBitBucket,
                    NvmeSglDescTypeBitBucket) UCHAR Type : 4;
    };
...
typedef struct _NVME_SGL_SEGMENT_DESC {
  ...
  union {
    struct {
      ...
      _Field_range_(NvmeSglDescTypeSegment, NvmeSglDescTypeSegment) UCHAR Type
          : 4;
    };
...
typedef struct _NVME_SGL_LASTSEG_DESC {
  ...
  union {
    struct {
      ...
      _Field_range_(NvmeSglDescTypeLastSegment,
                    NvmeSglDescTypeLastSegment) UCHAR Type : 4;
    };
...
typedef struct _NVME_SGL_KEYDATABLOCK_DESC {
  ...
  union {
    struct {
      ...
      _Field_range_(NvmeSglDescTypeKeyedDataBlock,
                    NvmeSglDescTypeKeyedDataBlock) UCHAR Type : 4;
    };
...
typedef struct _NVME_SGL_TRANSPORTDATA_DESC {
  ...
  union {
    struct {
      ...
      _Field_range_(NvmeSglDescTypeTransportDataBlock,
                    NvmeSglDescTypeTransportDataBlock) UCHAR Type : 4;
    };
...
typedef struct _NVMEOF_FABRICS_COMMAND {
  ...
  UCHAR PSDT;   // PRP or SGL for data transfer (only Bits 7:6 are to be used,
                // rest are reserved)
...
typedef struct _NVMEOF_CONNECT_COMMAND {
  ...
  UCHAR Reserved2;
  ULONG KATO; // Keep Alive Timeout, valid only for Admin Queue, reserved for IO
              // Queue
  UCHAR Reserved3[12];
...
typedef struct _NVMEOF_CONNECT_RESPONSE {
  ...
  ULONG Reserved0;
  USHORT SQHD; // Current Submission Queue Head pointer if SQ flow control is
               // enabled
  USHORT Reserved1;
  USHORT CID; // Command Identifier
  USHORT STS; // Status
...
typedef struct _NVMEOF_DISCONNECT_RESPONSE {
  ...
  ULONGLONG Reserved0;
  USHORT SQHD; // Current Submission Queue Head pointer for the associated
               // Submission Queue
  USHORT Reserved1;
  USHORT CID; // Command Identifier
  USHORT STS; // Status
...
typedef struct _NVMEOF_AUTH_RECEIVE_COMMAND {
  ...
  NVME_SGL_DESC SGL1;  // SGL descriptor that describes the entire data transfer
  UCHAR Reserved2;
  UCHAR SPSP0; // Bits 07:00 of Security Protocol Specific field as defined in
               // SPC-5
  UCHAR SPSP1; // Bits 15:08 of Security Protocol Specific field as defined in
               // SPC-5
  UCHAR SECP;  // Security protocol as defined in SPC-5
  ULONG AL; // Allocation Length, specific to the Security Protocol as defined
            // in SPC-5 where INC_512 is cleared to '0'
...
typedef struct _NVMEOF_AUTH_SEND_COMMAND {
  ...
  UCHAR SPSP1; // Bits 15:08 of Security Protocol Specific field as defined in
               // SPC-5
  UCHAR SECP;  // Security protocol as defined in SPC-5
  ULONG TL; // Transfer Length, specific to the Security Protocol as defined in
            // SPC-5 where INC_512 is cleared to '0'
...
typedef struct _NVME_DISCOVERY_ENTRY {
  ...
  CNTLID; // Controller Id
          // If subsystem supports dynamic controller model the value will
          // be FFFFh. If subsystem supports static controller model and
          // value is FFFEh, the
          //   host should remember the controller Id returned by Connect
          //   command.
          // If subsystem supports static controller model and value is
          // between 0h and FFEFh, then a specific controller is specified.
  USHORT
  ASQSZ; // Maximum size of an Admin Submission Queue, minimum value of 32
  ...
  UCHAR Reserved0[20];
  UCHAR TRSVCID[NVMEOF_TRANSPORT_SERVID_MAX_LEN]; // NVMe Transport service
                                                  // identifier
  UCHAR Reserved1[192];
  UCHAR NQN[NVME_NQN_MAX_LEN]; // NQN that uniquely identifies the NVM entity
                               // (Subsystem/Host)

  UCHAR TRADDR[NVMEOF_TRANSPORT_ADDR_MAX_LEN]; // Address of the fabric
                                               // interface on the NVM entity
                                               // (Subsystem/Host)
  UCHAR TSAS[NVMEOF_TRANSPORT_SAS_MAX_LEN]; // Transport specific information of
                                            // the address
...
typedef struct _NVME_EXTENDED_DISCOVERY_ENTRY {
  ...
  ULONG
  TEL; // Length in bytes of the entire extended discovery information entry
...
typedef struct _NVME_EXTENDED_ATTR {
  ...
  _Field_size_(EXATLEN) UCHAR EXATVAL[0]; // Extended Attribute Value
...
typedef struct _NVME_DISCOVERY_HEADER {
  ...
  ULONGLONG GENCTR; // Version of the discovery information starting at 0h and
                    // incrementing
...
#define NVME_DISCOVERY_INFO_MGMT_EKTYPE_PORTID                                 \
  0x003F // EKTYPE - Port ID Based (For Subsystem)
#define NVME_DISCOVERY_INFO_MGMT_EKTYPE_TRADDR                                 \
  0x005F // EKTYPE - TRADDR Based (For Subsystem and Host)
...
UCHAR
FORCEINLINE
NVMEOF_AUTH_GET_HASH_LENGTH(_In_ UCHAR HashId) {
  UCHAR Length = 0;
...
typedef struct _NVMEOF_AUTH_DHCHAP_DESCRIPTOR {
  ...
  UCHAR
  AuthId; // Authentication protocol identifier (NVMEOF_DHCHAP_PROTOCOL_ID)
  UCHAR Reserved0;

  UCHAR
  HALEN; // HashIDList Length: Number of hash function identifiers (1 to 30)
  UCHAR DHLEN; // DHgIDList Length: Number of Diffie-Hellman group identifiers
               // (1 to 30)
