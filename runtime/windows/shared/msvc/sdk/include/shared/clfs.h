...
#define CLFS_FLAG_FORCE_APPEND                                                 \
  0x00000001                             // Flag to force an append to log queue
#define CLFS_FLAG_FORCE_FLUSH 0x00000002 // Flag to force a log flush
#define CLFS_FLAG_USE_RESERVATION                                              \
  0x00000004 // Flag to charge a data append to reservation
#define CLFS_FLAG_REENTRANT_FILE_SYSTEM                                        \
  0x00000008 // Kernel mode create flag indicating a re-entrant file system.
#define CLFS_FLAG_NON_REENTRANT_FILTER                                         \
  0x00000010 // Kernel mode create flag indicating non-reentrant filter.
#define CLFS_FLAG_REENTRANT_FILTER                                             \
  0x00000020 // Kernel mode create flag indicating reentrant filter.
#define CLFS_FLAG_IGNORE_SHARE_ACCESS                                          \
  0x00000040 // Kernel mode create flag indicating IO_IGNORE_SHARE_ACCESS_CHECK
             // semantics.
#define CLFS_FLAG_READ_IN_PROGRESS                                             \
  0x00000080 // Flag indicating read in progress and not completed.
#define CLFS_FLAG_MINIFILTER_LEVEL                                             \
  0x00000100 // Kernel mode create flag indicating mini-filter target.
#define CLFS_FLAG_HIDDEN_SYSTEM_LOG                                            \
  0x00000200 // Kernel mode create flag indicating the log and containers should
             // be marked hidden & system.
//      CLFS_FLAG_MAX_VALUE             0x00800000      // Highest value allowed
//      for public flags.
/* NOTE that the uppper 8 bits of CLFS_FLAG values are reserved for internal
 * use. */
//
// Marshalling Context Flag
//
#define CLFS_MARSHALLING_FLAG_NONE 0x00000000 // No flags
#define CLFS_MARSHALLING_FLAG_DISABLE_BUFF_INIT                                \
  0x00000001 // Flag to disable mashalling buffer intialization

//
// Flag indicating all CLFS I/O will be targeted to an intermediate level of the
// I/O stack
//
...
//
// Flag indicating all CLFS I/O will be targeted to the top level of the I/O
// stack
//
...
// [CLFS_LSN_NULL, CLFS_LSN_INVALID) define the only valid LSN range.  LSN
// values are strictly monotonic increasing.
//
...
#define CLFS_CONTAINER_RELATIVE_PREFIX L"%BLF%\\"
    ...
    typedef UCHAR CLS_RECORD_TYPE,
              *PCLS_RECORD_TYPE, **PPCLS_RECORD_TYPE;
...
// CLFS_INFORMATION
//
// Logical log file information structure describing either virtual or physical
// log file data, depending on the type of information queried.
//

typedef struct _CLS_INFORMATION {
  LONGLONG TotalAvailable;   // Total log data space available.
  LONGLONG CurrentAvailable; // Usable space in the log file.
  LONGLONG TotalReservation; // Space reserved for UNDO's (aggregate for
                             // physical log)
...
{
    CLS_INFORMATION ClfsInfo;                           // Contains base log
file information. ULONG ClientAttributes;                             // Virtual
log file attributes. LONGLONG ClientUndoCommitment;                      // Max.
undo commitment for client. CLS_LSN ClientArchiveTailLsn; // Marks the client
archive tail. CLS_LSN ClientBaseLsn;                              // Min. client
LSN in active log region. CLS_LSN ClientLastLsn;                              //
Max. client LSN in active log region. CLS_LSN ClientRestartLsn; // Location of
restart record.
...
typedef enum _CLS_LOG_INFORMATION_CLASS {
  ...
  ClfsLogBasicInformation = 0x00, // For virtual or physical logs, indicates the
                                  // respective basic information.
  ClfsLogBasicInformationPhysical, // Always indicates physical log basic
                                   // information.
  ...
  ClfsLogPhysicalLsnInformation    // Maps virtual LSNs to physical LSNs; only
...
// CLS_IO_STATISTICS
//
// This structure defines I/O performance counters particular to a log file.  It
// consists of a header followed by the I/O statistics counters.  The header is
...
CLFSUSER_API BOOLEAN NTAPI ClfsLsnEqual(_In_ const CLFS_LSN *plsn1,
...
CLFSUSER_API BOOLEAN NTAPI ClfsLsnLess(_In_ const CLFS_LSN *plsn1,
                                       _In_ const CLFS_LSN *plsn2);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API BOOLEAN NTAPI ClfsLsnGreater(_In_ const CLFS_LSN *plsn1,
                                          _In_ const CLFS_LSN *plsn2);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API BOOLEAN NTAPI ClfsLsnNull(_In_ const CLFS_LSN *plsn);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
ClfsLsnContainer(_In_ const CLFS_LSN *plsn);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API CLFS_LSN NTAPI ClfsLsnCreate(_In_ CLFS_CONTAINER_ID cidContainer,
                                          _In_ ULONG offBlock,
                                          _In_ ULONG cRecord);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API ULONG NTAPI ClfsLsnBlockOffset(_In_ const CLFS_LSN *plsn);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API ULONG NTAPI ClfsLsnRecordSequence(_In_ const CLFS_LSN *plsn);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API BOOLEAN NTAPI ClfsLsnInvalid(_In_ const CLFS_LSN *plsn);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
CLFSUSER_API CLFS_LSN NTAPI ClfsLsnIncrement(_In_ PCLFS_LSN plsn);
#endif /* NTDDI_VERSION || _WIN32_WINNT */
...
inline CLFS_LSN operator++(_Inout_ CLFS_LSN &refLsn) {
  //
...
inline BOOLEAN operator<(_In_ const CLFS_LSN &refLsn1,
