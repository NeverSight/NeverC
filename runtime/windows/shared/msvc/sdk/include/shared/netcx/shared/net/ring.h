...
typedef struct DECLSPEC_CACHEALIGN _NET_RING {
  ...
  // Number of elements in the ring buffer.  Is always a power of 2, greater
  // than 1.
  UINT32 NumberOfElements;

  // A mask that can be used to efficiently clamp an index to [0,
  // NumberOfElements). Use the identity:  (x % NumberOfElements) == (x &
  // ElementIndexMask)
  ...
  // The NIC driver may optionally use this to track which elements have been
  // consumed.
  UINT32 NextIndex;
  ...
  DECLSPEC_CACHEALIGN
  _Field_size_(
      NumberOfElements *ElementStride) unsigned char Buffer[ANYSIZE_ARRAY];
...
Arguments:
    ...
    Index - The element index.  Must be in the range [0,
Ring->NumberOfElements).
...
inline void *NetRingGetElementAtIndex(_In_ NET_RING const *Ring,
                                      _In_ UINT32 Index) {
  return (void *)(Ring->Buffer + (SIZE_T)Index * Ring->ElementStride);
}

inline UINT32 NetRingAdvanceIndex(_In_ NET_RING const *Ring, _In_ UINT32 Index,
                                  _In_ INT32 Distance) {
...
--*/
inline UINT32 NetRingIncrementIndex(_In_ NET_RING const *Ring,
                                    _In_ UINT32 Index) {
  return NetRingAdvanceIndex(Ring, Index, 1);
}

inline UINT32 NetRingGetRangeCount(_In_ NET_RING const *Ring,
                                   _In_ UINT32 StartIndex, _In_ UINT32 EndIndex)
...
inline NET_PACKET *NetRingGetPacketAtIndex(NET_RING const *Ring, UINT32 Index) {
  return (NET_PACKET *)NetRingGetElementAtIndex(Ring, Index);
}

inline NET_FRAGMENT *NetRingGetFragmentAtIndex(NET_RING const *Ring,
                                               UINT32 Index) {
  return (NET_FRAGMENT *)NetRingGetElementAtIndex(Ring, Index);
}

inline NET_FRAGMENT_RETURN_CONTEXT *
NetRingGetFragmentReturnContextAtIndex(NET_RING const *Ring, UINT32 Index) {
