...
inline NET_RING *
NetRingCollectionGetPacketRing(NET_RING_COLLECTION const *Rings) {
  return Rings->Rings[NetRingTypePacket];
}

inline NET_RING *
NetRingCollectionGetFragmentRing(NET_RING_COLLECTION const *Rings) {
  return Rings->Rings[NetRingTypeFragment];
}

inline NET_RING *NetRingCollectionGetFragmentReturnContextRing(
