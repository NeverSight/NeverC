...
Abstract:
    ...
    This is the Virtual HID Framework (VHF) interface for both User mode and
Kernel mode.
...
typedef _Function_class_(EVT_VHF_READY_FOR_NEXT_READ_REPORT)
    _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_requires_same_ VOID
    EVT_VHF_READY_FOR_NEXT_READ_REPORT(_In_ PVOID VhfClientContext);

typedef EVT_VHF_READY_FOR_NEXT_READ_REPORT *PEVT_VHF_READY_FOR_NEXT_READ_REPORT;

typedef _Function_class_(EVT_VHF_CLEANUP)
    _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_requires_same_ VOID
    EVT_VHF_CLEANUP(_In_ PVOID VhfClientContext);

typedef EVT_VHF_CLEANUP *PEVT_VHF_CLEANUP;

typedef _Function_class_(EVT_VHF_ASYNC_OPERATION)
    _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_requires_same_ VOID
    EVT_VHF_ASYNC_OPERATION(_In_ PVOID VhfClientContext,
...
typedef struct _VHF_CONFIG {
  ...
  _Field_size_bytes_full_(InstanceIDLength) PWSTR InstanceID;
  ...
  _Field_size_bytes_full_(HardwareIDsLength) PWSTR HardwareIDs;

} VHF_CONFIG, *PVHF_CONFIG;

FORCEINLINE
VOID VHF_CONFIG_INIT(_Out_ PVHF_CONFIG Config,
#ifdef _KERNEL_MODE
...
_IRQL_requires_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
    VhfCreate(_In_ PVHF_CONFIG VhfConfig, _Out_ VHFHANDLE *VhfHandle);

_IRQL_requires_max_(DISPATCH_LEVEL) _Must_inspect_result_ NTSTATUS
    VhfStart(_In_ VHFHANDLE VhfHandle);

_When_(Wait == __true, _IRQL_requires_(PASSIVE_LEVEL))
    _When_(Wait == __false, _IRQL_requires_max_(DISPATCH_LEVEL)) VOID
    VhfDelete(_In_ VHFHANDLE VhfHandle, _In_ BOOLEAN Wait);

_IRQL_requires_max_(DISPATCH_LEVEL) _Must_inspect_result_ NTSTATUS
    VhfReadReportSubmit(_In_ VHFHANDLE VhfHandle,
                        _In_ PHID_XFER_PACKET HidTransferPacket);

_IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS
    VhfAsyncOperationComplete(_In_ VHFOPERATIONHANDLE VhfOperationHandle,
