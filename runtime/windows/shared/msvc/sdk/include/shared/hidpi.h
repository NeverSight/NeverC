...
// pointers is similarly determined.  When a collection B is defined, it becomes
// the FirstChild of it's parent collection.  The previously defined FirstChild
// of the parent collection becomes the NextSibling of the new collection.  As
// new collections with the same parent are discovered, the chain of sibling is
// built.
//
...
_Must_inspect_result_ _IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS __stdcall
HidP_GetCaps(_In_ PHIDP_PREPARSED_DATA PreparsedData,
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_GetLinkCollectionNodes(_Out_writes_to_(*LinkCollectionNodesLength,
...
HidP_GetSpecificButtonCaps(
    ...
    _Out_writes_to_(*ButtonCapsLength, *ButtonCapsLength)
        PHIDP_BUTTON_CAPS ButtonCaps,
    _Inout_ PUSHORT ButtonCapsLength, _In_ PHIDP_PREPARSED_DATA PreparsedData);
/*++
Description:
...
Parameters:
   ...
   ButtonCapsLength   As input, this parameter specifies the length of the
                  ...
                  array elements were filled in by the function.  The maximum
number of button caps that can be returned is found in the HIDP_CAPS structure.
If HIDP_STATUS_BUFFER_TOO_SMALL is returned, this value contains the number of
array elements needed to successfully complete the request.
...
--*/
_Must_inspect_result_ _IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS __stdcall
HidP_GetButtonCaps(_In_ HIDP_REPORT_TYPE ReportType,
                   _Out_writes_to_(*ButtonCapsLength, *ButtonCapsLength)
                       PHIDP_BUTTON_CAPS ButtonCaps,
...
HidP_GetSpecificValueCaps(
    ...
    _In_opt_ USHORT LinkCollection, // Optional (0 => ignore)
    _In_opt_ USAGE Usage,           // Optional (0 => ignore)
    _Out_writes_to_(*ValueCapsLength, *ValueCapsLength)
        PHIDP_VALUE_CAPS ValueCaps,
    _Inout_ PUSHORT ValueCapsLength, _In_ PHIDP_PREPARSED_DATA PreparsedData);
/*++
Description:
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_GetValueCaps(_In_ HIDP_REPORT_TYPE ReportType,
                  _Out_writes_to_(*ValueCapsLength, *ValueCapsLength)
                      PHIDP_VALUE_CAPS ValueCaps,
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_GetExtendedAttributes(_In_ HIDP_REPORT_TYPE ReportType,
                           _In_ USHORT DataIndex,
                           _In_ PHIDP_PREPARSED_DATA PreparsedData,
                           _Out_writes_to_(*LengthAttributes, *LengthAttributes)
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_InitializeReportForID(_In_ HIDP_REPORT_TYPE ReportType,
                           _In_ UCHAR ReportID,
                           _In_ PHIDP_PREPARSED_DATA PreparsedData,
...
equal to the length specified in HIDP_CAPS structure for the given ReportType
  HIDP_STATUS_REPORT_DOES_NOT_EXIST  -- if there are no reports on this device
                                        for the given ReportType

--*/

_Must_inspect_result_ NTSTATUS __stdcall
HidP_SetData(_In_ HIDP_REPORT_TYPE ReportType,
             _Inout_updates_to_(*DataLength, *DataLength) PHIDP_DATA DataList,
             _Inout_ PULONG DataLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_GetData(_In_ HIDP_REPORT_TYPE ReportType,
...
_IRQL_requires_max_(DISPATCH_LEVEL) ULONG __stdcall
HidP_MaxDataListLength(_In_ HIDP_REPORT_TYPE ReportType,
                       _In_ PHIDP_PREPARSED_DATA PreparsedData);
/*++
Routine Description:
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_SetUsages(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection,
    _Inout_updates_to_(*UsageLength, *UsageLength) PUSAGE UsageList,
    _Inout_ PULONG UsageLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _In_reads_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
/*++
...
Parameters:
    ...
    UsageList   A usage array containing the usages that HidP_SetUsages will set
...
Return Value
    HidP_SetUsages returns the following error codes.  On error, the report
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_UnsetUsages(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection,
    _Inout_updates_to_(*UsageLength, *UsageLength) PUSAGE UsageList,
    _Inout_ PULONG UsageLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
...
Routine Description:
    This function unsets (turns off) binary values (buttons) in the report.
Given an initialized packet of correct length, it modifies the report packet so
...
Parameters:
    ...
    UsagePage   All of the usages in the usage array, which HidP_UnsetUsages
will unset in the report, refer to this same usage page. If a client wishes to
unset usages in a report for multiple usage pages then that client needs to make
multiple calls to HidP_UnsetUsages for each of the usage pages.
...
Return Value
  ...
  HIDP_STATUS_SUCCESS                -- upon successful "unsetting" of all
usages in the report packet. HIDP_STATUS_INVALID_REPORT_TYPE    -- if ReportType
is not valid. HIDP_STATUS_INVALID_PREPARSED_DATA -- if PreparsedData is not
valid HIDP_STATUS_INVALID_REPORT_LENGTH  -- the length of the report packet is
not equal to the length specified in the HIDP_CAPS structure for the given
                                        ReportType
  HIDP_STATUS_REPORT_DOES_NOT_EXIST  -- if there are no reports on this device
                                        for the given ReportType
  HIDP_STATUS_INCOMPATIBLE_REPORT_ID -- if a usage was found that exists in a
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_GetUsages(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection,
    _Out_writes_to_(*UsageLength, *UsageLength) PUSAGE UsageList,
    _Inout_ PULONG UsageLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _Out_writes_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
/*++
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_GetUsagesEx(_In_ HIDP_REPORT_TYPE ReportType,
                 _In_opt_ USHORT LinkCollection, // Optional
                 _Inout_updates_to_(*UsageLength, *UsageLength)
...
Return Value
  ...
  HIDP_STATUS_INCOMPATIBLE_REPORT_ID -- if no usages were found but usages
                                        ...
                                        ID.
  HIDP_STATUS_USAGE_NOT_FOUND        -- if there are no usages in any reports
that match the LinkCollection parameter
--*/

_IRQL_requires_max_(PASSIVE_LEVEL) ULONG __stdcall
HidP_MaxUsageListLength(_In_ HIDP_REPORT_TYPE ReportType,
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_SetUsageValue(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage, _In_ ULONG UsageValue,
    _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _Inout_updates_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
/*++
Description:
...
Parameters:
    ...
    UsageValue  The raw value to set in the report buffer.  This value must be
within the logical range or if a NULL value this value should be the most
negative value that can be represented by the number of bits for this field.
...
--*/
_Must_inspect_result_ NTSTATUS __stdcall HidP_SetScaledUsageValue(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage, _In_ LONG UsageValue,
    _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _Inout_updates_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
...
Return Value:
  ...
  HIDP_STATUS_USAGE_NOT_FOUND        -- if the usage page, usage, and link
                                        ...
                                        in any reports for this ReportType
--*/
_Must_inspect_result_ NTSTATUS __stdcall HidP_SetUsageValueArray(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage,
    _In_reads_bytes_(UsageValueByteLength) PCHAR UsageValue,
    _In_ USHORT UsageValueByteLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _Inout_updates_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);

/*++
Routine Descripton:
    A usage value array occurs when the last usage in the list of usages
    describing a main item must be repeated because there are less usages
defined than there are report counts declared for the given main item.  In this
case a single value cap is allocated for that usage and the report count of that
...
Parameters:
    ...
    UsageValue  The buffer with the values to set into the value array.
                ...
                or not the field is byte alligned or if the BitSize is a
...
in the array HIDP_STATUS_NOT_IMPLEMENTED        -- if the usage value array has
field sizes that are not multiples of 8 bits, this error code is returned since
the function currently does not handle setting into such arrays.
  HIDP_STATUS_INCOMPATIBLE_REPORT_ID -- the specified usage page, usage and
                                        link collection exist but exists in
                                        a report with a different report ID
                                        than the report being passed in.  To
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_GetUsageValue(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage, _Out_ PULONG UsageValue,
    _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _In_reads_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_GetScaledUsageValue(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage, _Out_ PLONG UsageValue,
    _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _In_reads_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
...
Return Value:
  ...
  HIDP_STATUS_USAGE_NOT_FOUND        -- if the usage page, usage, and link
                                        ...
                                        in any reports for this ReportType
--*/
_Must_inspect_result_ NTSTATUS __stdcall HidP_GetUsageValueArray(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage,
...
Return Value:
  ...
  HIDP_STATUS_NOT_IMPLEMENTED        -- if the usage value array has field sizes
                                        ...
                                        error code is returned since the
function currently does not handle getting values from such arrays.
  HIDP_STATUS_INCOMPATIBLE_REPORT_ID -- the specified usage page, usage and
                                        link collection exist but exists in
                                        a report with a different report ID
                                        than the report being passed in.  To
...
_Must_inspect_result_ _IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS __stdcall
HidP_UsageListDifference(_In_reads_(UsageListLength) PUSAGE PreviousUsageList,
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_GetButtonArray(
    _In_ HIDP_REPORT_TYPE ReportType, _In_ USAGE UsagePage,
    _In_opt_ USHORT LinkCollection, _In_ USAGE Usage,
    _Out_writes_to_(*ButtonDataLength, *ButtonDataLength)
        PHIDP_BUTTON_ARRAY_DATA ButtonData,
    _Inout_ PUSHORT ButtonDataLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _In_reads_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
...
Routine Descripton:
    ...
    describing a main item, must be repeated because there are less usages
defined than the ReportCount declared for the given main item.  In this case, a
single HIDP_BUTTON_CAPS is allocated for that usage and the ReportCount of the
    HIDP_BUTTON_CAPS is set to reflect the number of fields the usage refers.

    A HIDP_BUTTON_CAPS that describes a button array, will always have
ReportCount > 1. If ReportCount == 1, then it is not a button array and cannot
be used with HidP_GetButtonArray. (see instead HidP_GetUsages)

    HidP_GetButtonArray returns (via _Out_ parameter) an array of
HIDP_BUTTON_ARRAY_DATAs, one for each button (in the first button array found
(and within the specified LinkCollection) with the supplied Usage) that is set
to ON, for the supplied Report.
...
Parameters:
    ...
    LinkCollection  (Optional)  This value can be used to differentiate between
two fields that may have the same UsagePage and Usage but exist in different
collections. If the LinkCollection value is HIDP_LINK_COLLECTION_UNSPECIFIED,
the first found button array matching the UsagePage and Usage will be returned
(regardless of location). If the LinkCollection value is
HIDP_LINK_COLLECTION_ROOT, the first found button array (in the root collection)
matching the UsagePage and Usage will be returned.

    Usage    The usage whose buttons HidP_GetButtonArray will retrieve.

    ButtonData    A HIDP_BUTTON_ARRAY_DATAs array where the data of buttons set
to ON will be placed.  The number of elements required is the ReportCount field
                  of the HIDP_BUTTON_CAPS for this control.  This buffer is
provided by the caller.

    ButtonDataLength   As input, this parameter specifies the length of the
                  ButtonData parameter (array) in number of array elements (NOT
number of bytes). As output, if HIDP_STATUS_SUCCESS is returned, this value is
set to indicate how many of those array elements were filled in by the function.
The maximum number of HIDP_BUTTON_ARRAY_DATA that can be returned is determined
by HIDP_BUTTON_CAPS.ReportCount If HIDP_STATUS_BUFFER_TOO_SMALL is returned,
this value contains the number of array elements needed to successfully complete
the request.

    PreparsedData The preparsed data returned by the HIDCLASS

    Report      The report packet.  (Note: The first byte MUST be the ReportId;
this will be correctly set if the report is read from the system)

    ReportLength   Length of the given report packet (in bytes).

Return Value:

    HIDP_STATUS_SUCCESS                -- upon successfully retrieving the
buttons from the report packet HIDP_STATUS_INVALID_REPORT_TYPE    -- if
ReportType is not valid. HIDP_STATUS_INVALID_PREPARSED_DATA -- if PreparsedData
is not valid HIDP_STATUS_INVALID_REPORT_LENGTH  -- the length of the report
packet is not equal to the length specified in the HIDP_CAPS structure for the
given ReportType HIDP_STATUS_NOT_BUTTON_ARRAY       -- if the control specified
is not a button array -- a button array will have a ReportCount field in the
                                          HIDP_BUTTON_CAPS structure that is
> 1. If ReportCount == 1, Use HidP_GetUsages instead.
    HIDP_STATUS_BUFFER_TOO_SMALL       -- if the size of the passed in buffer in
                                          which to return the array is too small
                                          (i.e. has fewer values than the number
of fields in the array).  ButtonDataLength will be set to the required size.
    HIDP_STATUS_INCOMPATIBLE_REPORT_ID -- the specified usage page, usage and
                                          link collection exist but exists in
                                          a report with a different report ID
                                          than the report being passed in.  To
...
_Must_inspect_result_ NTSTATUS __stdcall HidP_SetButtonArray(
    ...
    _In_ USHORT ButtonDataLength, _In_ PHIDP_PREPARSED_DATA PreparsedData,
    _Inout_updates_bytes_(ReportLength) PCHAR Report, _In_ ULONG ReportLength);
...
single HIDP_BUTTON_CAPS is allocated for that usage and the ReportCount of the
    ...
    A HIDP_BUTTON_CAPS that describes a button array, will always have
ReportCount > 1. If ReportCount == 1, then it is not a button array and cannot
be used with HidP_SetButtonArray. (see instead HidP_SetUsages)

    HidP_SetButtonArray sets the state of buttons via an array of
HIDP_BUTTON_ARRAY_DATAs.

    HidP_SetButtonArray sets the state of buttons via an array of
HIDP_BUTTON_ARRAY_DATAs (for the first button array found (and within the
specified LinkCollection) with the supplied Usage) for the supplied Report.
...
Return Value:
    ...
    HIDP_STATUS_NOT_BUTTON_ARRAY       -- if the control specified is not a
                                          button array -- a button array will
have a ReportCount field in the HIDP_BUTTON_CAPS structure that is > 1. If
ReportCount == 1, Use HidP_SetUsages instead. HIDP_STATUS_INCOMPATIBLE_REPORT_ID
-- the specified usage page, usage and link collection exist but exists in a
report with a different report ID than the report being passed in.  To set this
value, call HidP_SetButtonArray again with a zero-initialized report packet
    HIDP_STATUS_USAGE_NOT_FOUND        -- if the usage page, usage, and link
                                          collection combination does not exist
                                          in any reports for this ReportType
    HIDP_STATUS_DATA_INDEX_OUT_OF_RANGE -- if the ArrayIndex for one of the
supplied HIDP_BUTTON_ARRAY_DATA is outside the valid range for this button array
(i.e. >= HIDP_BUTTON_CAPS.ReportCount)
--*/
...
HidP_TranslateUsageAndPagesToI8042ScanCodes(
    ...
    _In_ ULONG UsageListLength, _In_ HIDP_KEYBOARD_DIRECTION KeyAction,
...
Parameters:
--*/
_Must_inspect_result_ NTSTATUS __stdcall HidP_TranslateUsagesToI8042ScanCodes(
...
typedef NTSTATUS (*PFN_HidP_GetVersionInternal)(_Out_ ULONG *Version);

_Must_inspect_result_ inline NTSTATUS __stdcall
HidP_GetVersion(_Out_ ULONG *Version)
/*++
Routine Description:
  Header-only implementation of HidP versioning, to separate API versions that
support HidP_GetButtonArray && HidP_SetButtonArray APIs and those that don't.
--*/
{
  ...
  *Version = 1;
  ...
  PFN_HidP_GetVersionInternal fnVersionInternal =
      (PFN_HidP_GetVersionInternal)GetProcAddress(module,
                                                  "HidP_GetVersionInternal");
  if (fnVersionInternal != NULL) {
    status = fnVersionInternal(Version);
  } else {
    // This DLL build doesn't support HidP_GetVersionInternal, so the version
    // is 1.
  }
