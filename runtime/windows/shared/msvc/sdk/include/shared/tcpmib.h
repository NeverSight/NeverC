...
#define SIZEOF_TCPTABLE2(X)                                                    \
  (FIELD_OFFSET(MIB_TCPTABLE2, table[0]) + ((X) * sizeof(MIB_TCPROW2)) +       \
   ALIGN_SIZE)
...
#define SIZEOF_TCP6TABLE(X)                                                    \
  (FIELD_OFFSET(MIB_TCP6TABLE, table[0]) + ((X) * sizeof(MIB_TCP6ROW)) +       \
   ALIGN_SIZE)
...
#define SIZEOF_TCP6TABLE2(X)                                                   \
  (FIELD_OFFSET(MIB_TCP6TABLE2, table[0]) + ((X) * sizeof(MIB_TCP6ROW2)) +     \
   ALIGN_SIZE)
...
typedef struct _MIB_TCP6TABLE_OWNER_PID {
  DWORD dwNumEntries;
  _Field_size_(dwNumEntries) MIB_TCP6ROW_OWNER_PID table[ANY_SIZE];
...
typedef struct _MIB_TCP6TABLE_OWNER_MODULE {
  DWORD dwNumEntries;
  _Field_size_(dwNumEntries) MIB_TCP6ROW_OWNER_MODULE table[ANY_SIZE];
