...
DEFINE_GUID(SOCKET_DEFAULT2_QM_POLICY, 0xaec2ef9c, 0x3a4d, 0x4d3e, 0x88, 0x42,
            0x23, 0x99, 0x42, 0xe3, 0x9a, 0x47);
// GUID definition for use with Real Time Notification setting API.
// 6b59819a-5cae-492d-a901-2a3c2c50164f
DEFINE_GUID(REAL_TIME_NOTIFICATION_CAPABILITY, 0x6b59819a, 0x5cae, 0x492d, 0xa9,
            0x01, 0x2a, 0x3c, 0x2c, 0x50, 0x16, 0x4f);
// GUID definition for use with Real Time Notification setting API.
// 6843da03-154a-4616-a508-44371295f96b
DEFINE_GUID(REAL_TIME_NOTIFICATION_CAPABILITY_EX, 0x6843da03, 0x154a, 0x4616,
...
DEFINE_GUID(ASSOCIATE_NAMERES_CONTEXT, 0x59a38b67, 0xd4fe, 0x46e1, 0xba, 0x3c,
            0x87, 0xea, 0x74, 0xca, 0x30, 0x49);
...
//
// Flags                - Enable/disable timestamp reception for rx/tx
// direction. TxTimestampsBuffered - Determines how many tx timestamps may be
// buffered.
...
#define IN4ADDR_LINKLOCALPREFIX_INIT                                           \
  {                                                                            \
      0xa9,                                                                    \
      0xfe,                                                                    \
  }
#define IN4ADDR_MULTICASTPREFIX_INIT                                           \
  {                                                                            \
      0xe0,                                                                    \
  }
...
MSTCPIP_INLINE
BOOLEAN
IN4_UNALIGNED_ADDR_EQUAL(_In_ CONST IN_ADDR UNALIGNED *a,
...
MSTCPIP_INLINE
VOID IN4ADDR_SETSOCKADDR(_Out_ PSOCKADDR_IN a, _In_ CONST IN_ADDR *addr,
                         _In_ USHORT port) {
...
MSTCPIP_INLINE
VOID IN4ADDR_SETANY(_Out_ PSOCKADDR_IN a) {
  a->sin_family = AF_INET;
  a->sin_port = 0;
...
MSTCPIP_INLINE
VOID IN4ADDR_SETLOOPBACK(_Out_ PSOCKADDR_IN a) {
  a->sin_family = AF_INET;
  a->sin_port = 0;
...
MSTCPIP_INLINE
SCOPE_LEVEL
Ipv4UnicastAddressScope(_In_reads_(_Inexpressible_("Varies"))
                            CONST UCHAR *Address)
/*++
...
MSTCPIP_INLINE
SCOPE_LEVEL
Ipv4MulticastAddressScope(_In_reads_(_Inexpressible_("Varies"))
                              CONST UCHAR *Address)
/*++
...
MSTCPIP_INLINE
SCOPE_LEVEL
Ipv4AddressScope(_In_reads_(_Inexpressible_("Varies")) CONST UCHAR *Address)
/*++
...
MSTCPIP_INLINE
NL_ADDRESS_TYPE
Ipv4AddressType(_In_reads_(_Inexpressible_("Varies")) CONST UCHAR *Address) {
...
MSTCPIP_INLINE
VOID IN4_UNCANONICALIZE_SCOPE_ID(_In_ CONST IN_ADDR *Address,
...
IN4_IS_ADDR_6TO4ELIGIBLE(_In_ CONST IN_ADDR *a) {
  return (BOOLEAN)((Ipv4AddressType((CONST UCHAR *)a) == NlatUnicast) &&
                   !(IN4_IS_ADDR_LOOPBACK(a) || IN4_IS_ADDR_LINKLOCAL(a) ||
                     IN4_IS_ADDR_SITELOCAL(a) || IN4_IS_ADDR_RFC1918(a)));
}
...
#ifdef _PREFAST_
#pragma prefast(push)
#pragma prefast(disable : 6385,                                                \
                "apparent one byte buffer overread in IN6_PREFIX_EQUAL")
#endif

MSTCPIP_INLINE
BOOLEAN
IN6_PREFIX_EQUAL(_In_ CONST IN6_ADDR *a, _In_ CONST IN6_ADDR *b,
                 _In_ _In_range_(0, (sizeof(IN6_ADDR) * 8)) UINT8 len) {
  UINT8 Bytes = len / 8;
  UINT8 Bits = len % 8;
  UINT8 Mask = 0xff << (8 - Bits);

  return (BOOLEAN)(((memcmp(a, b, Bytes)) == 0) &&
                   ((Bits == 0) || ((a->s6_bytes[Bytes] & Mask) ==
                                    (b->s6_bytes[Bytes] & Mask))));
}
...
MSTCPIP_INLINE
CONST UCHAR *IN6_GET_ADDR_V4MAPPED(_In_ CONST IN6_ADDR *Ipv6Address) {
  return (CONST UCHAR *)(Ipv6Address->s6_words + 6);
}

MSTCPIP_INLINE
CONST UCHAR *IN6_GET_ADDR_V4COMPAT(_In_ CONST IN6_ADDR *Ipv6Address) {
  return (CONST UCHAR *)(Ipv6Address->s6_words + 6);
}

MSTCPIP_INLINE
CONST UCHAR *IN6_EXTRACT_V4ADDR_FROM_ISATAP(_In_ CONST IN6_ADDR *Ipv6Address) {
  return (CONST UCHAR *)(Ipv6Address->s6_words + 6);
}

MSTCPIP_INLINE
CONST UCHAR *IN6_EXTRACT_V4ADDR_FROM_6TO4(_In_ CONST IN6_ADDR *Ipv6Address) {
  return (CONST UCHAR *)(Ipv6Address->s6_words + 1);
}

MSTCPIP_INLINE
VOID IN6_SET_ADDR_V4MAPPED(_Out_ PIN6_ADDR a6, _In_ CONST IN_ADDR *a4) {
...
MSTCPIP_INLINE
VOID IN6_SET_ADDR_V4COMPAT(_Out_ PIN6_ADDR a6, _In_ CONST IN_ADDR *a4) {
  *a6 = in6addr_any;
  a6->s6_bytes[12] = ((CONST UCHAR *)a4)[0];
  a6->s6_bytes[13] = ((CONST UCHAR *)a4)[1];
  a6->s6_bytes[14] = ((CONST UCHAR *)a4)[2];
...
MSTCPIP_INLINE
VOID IN6_SET_ADDR_SOLICITEDNODE(_Out_ PIN6_ADDR Multicast,
...
MSTCPIP_INLINE
VOID IN6_SET_ISATAP_IDENTIFIER(_Inout_ IN6_ADDR *Ipv6Address,
...
MSTCPIP_INLINE
VOID IN6_SET_6TO4_PREFIX(_Inout_ IN6_ADDR *Ipv6Address,
                         _In_ CONST IN_ADDR *Ipv4Address) {
...
MSTCPIP_INLINE
SCOPE_LEVEL
Ipv6UnicastAddressScope(_In_reads_(_Inexpressible_("Varies"))
                            CONST UCHAR *Address)
/*++
...
MSTCPIP_INLINE
SCOPE_LEVEL
IN6_MULTICAST_SCOPE(_In_reads_(_Inexpressible_("Varies"))
                        CONST UCHAR *Address) {
...
MSTCPIP_INLINE
SCOPE_LEVEL
Ipv6AddressScope(_In_reads_(_Inexpressible_("Varies")) CONST UCHAR *Address)
/*++
...
MSTCPIP_INLINE
NL_ADDRESS_TYPE
Ipv6AddressType(_In_reads_(_Inexpressible_("Varies")) CONST UCHAR *Address) {
...
MSTCPIP_INLINE
VOID IN6_UNCANONICALIZE_SCOPE_ID(_In_ CONST IN6_ADDR *Address,
                                 _Inout_ SCOPE_ID *ScopeId) {
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
MSTCPIP_INLINE
VOID IN6ADDR_SETSOCKADDR(_Out_ PSOCKADDR_IN6 a, _In_ CONST IN6_ADDR *addr,
...
MSTCPIP_INLINE
VOID IN6ADDR_SETV4MAPPED(_Out_ PSOCKADDR_IN6 a6, _In_ CONST IN_ADDR *a4,
                         _In_ SCOPE_ID scope, _In_ USHORT port) {
...
MSTCPIP_INLINE
BOOLEAN
INET_ADDR_EQUAL(_In_ ADDRESS_FAMILY af, _In_ CONST VOID *a,
                _In_ CONST VOID *b) {
  if (af == AF_INET6) {
...
MSTCPIP_INLINE
BOOLEAN
INET_UNALIGNED_ADDR_EQUAL(_In_ ADDRESS_FAMILY af, _In_ CONST VOID *a,
                          _In_ CONST VOID *b) {
  if (af == AF_INET6) {
    return IN6_ADDR_EQUAL((CONST IN6_ADDR *)a, (CONST IN6_ADDR *)b);
  } else {
...
MSTCPIP_INLINE
CONST UCHAR *INET_ADDR_UNSPECIFIED(_In_ ADDRESS_FAMILY af) {
  if (af == AF_INET6) {
...
MSTCPIP_INLINE
VOID INET_SET_ADDRESS(_In_ ADDRESS_FAMILY Family,
...
_Ret_range_(sizeof(IN_ADDR), sizeof(IN6_ADDR)) MSTCPIP_INLINE SIZE_T
...
_Ret_range_(sizeof(SOCKADDR_IN), sizeof(SOCKADDR_IN6)) MSTCPIP_INLINE SIZE_T
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
_At_((PSOCKADDR_IN6)a, _When_(af == AF_INET6, _Out_)) MSTCPIP_INLINE VOID
    INETADDR_SETSOCKADDR(_In_ ADDRESS_FAMILY af, _Out_ PSOCKADDR a,
                         _In_ CONST VOID *addr, _In_ SCOPE_ID scope,
                         _In_ USHORT port) {
  if (af == AF_INET6) {
...
MSTCPIP_INLINE
VOID INETADDR_SETANY(_Inout_ PSOCKADDR a) {
...
MSTCPIP_INLINE
VOID INETADDR_SETLOOPBACK(_Inout_ PSOCKADDR a) {
  if (a->sa_family == AF_INET6) {
...
MSTCPIP_INLINE
BOOLEAN
NL_ADDR_EQUAL(_In_ ADDRESS_FAMILY af, _In_ SCOPE_ID sa, _In_ CONST UCHAR *aa,
              _In_ SCOPE_ID sb, _In_ CONST UCHAR *ab) {
  return (BOOLEAN)((sa.Value == sb.Value) && INET_ADDR_EQUAL(af, aa, ab));
}

MSTCPIP_INLINE
BOOLEAN
NL_IS_ADDR_UNSPECIFIED(_In_ ADDRESS_FAMILY af, _In_ SCOPE_ID s,
                       _In_ CONST UCHAR *a) {
...
INETADDR_ISEQUAL(_In_ CONST SOCKADDR *a, _In_ CONST SOCKADDR *b) {
  if (a->sa_family == AF_INET6) {
    return (BOOLEAN)(b->sa_family == AF_INET6 &&
                     IN6ADDR_ISEQUAL((CONST SOCKADDR_IN6 *)a,
                                     (CONST SOCKADDR_IN6 *)b));
  } else {
    ASSERT(a->sa_family == AF_INET);
    return (BOOLEAN)(b->sa_family == AF_INET &&
                     IN4ADDR_ISEQUAL((CONST SOCKADDR_IN *)a,
                                     (CONST SOCKADDR_IN *)b));
  }
}
...
MSTCPIP_INLINE
VOID INETADDR_SET_PORT(_Inout_ PSOCKADDR a, _In_ USHORT Port) {
  SS_PORT(a) = Port;
}

MSTCPIP_INLINE
VOID INETADDR_SET_ADDRESS(_Inout_ PSOCKADDR a,
                          _In_reads_(_Inexpressible_("Varies"))
                              CONST UCHAR *Address) {
  if (a->sa_family == AF_INET6) {
...
MSTCPIP_INLINE
VOID INET_UNCANONICALIZE_SCOPE_ID(_In_ ADDRESS_FAMILY AddressFamily,
                                  _In_reads_(_Inexpressible_("Varies"))
                                      CONST UCHAR *Address,
                                  _Inout_ SCOPE_ID *ScopeId) {
...
NTSYSAPI
LONG NTAPI RtlIpv4AddressToStringExA(_In_ const struct in_addr *Address,
                                     _In_ USHORT Port,
...
PWSTR
NTAPI
RtlIpv4AddressToStringW(_In_ const struct in_addr *Addr,
                        _Out_writes_(16) PWSTR S);

NTSYSAPI
LONG NTAPI RtlIpv4AddressToStringExW(_In_ const struct in_addr *Address,
                                     _In_ USHORT Port,
                                     _Out_writes_to_(*AddressStringLength,
                                                     *AddressStringLength)
                                         PWSTR AddressString,
...
NTSYSAPI
LONG NTAPI RtlIpv4StringToAddressA(_In_ PCSTR S, _In_ BOOLEAN Strict,
                                   _Out_ PCSTR *Terminator,
                                   _Out_ struct in_addr *Addr);

NTSYSAPI
LONG NTAPI RtlIpv4StringToAddressExA(_In_ PCSTR AddressString,
                                     _In_ BOOLEAN Strict,
                                     _Out_ struct in_addr *Address,
                                     _Out_ PUSHORT Port);

NTSYSAPI
LONG NTAPI RtlIpv4StringToAddressW(_In_ PCWSTR S, _In_ BOOLEAN Strict,
                                   _Out_ LPCWSTR *Terminator,
                                   _Out_ struct in_addr *Addr);

NTSYSAPI
LONG NTAPI RtlIpv4StringToAddressExW(_In_ PCWSTR AddressString,
                                     _In_ BOOLEAN Strict,
                                     _Out_ struct in_addr *Address,
                                     _Out_ PUSHORT Port);
...
#endif /* WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP |                   \
          WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM) */
...
NTSYSAPI
LONG NTAPI RtlIpv6AddressToStringExA(_In_ const struct in6_addr *Address,
...
PWSTR
NTAPI
RtlIpv6AddressToStringW(_In_ const struct in6_addr *Addr,
                        _Out_writes_(46) PWSTR S);

NTSYSAPI
LONG NTAPI RtlIpv6AddressToStringExW(_In_ const struct in6_addr *Address,
                                     _In_ ULONG ScopeId, _In_ USHORT Port,
                                     _Out_writes_to_(*AddressStringLength,
                                                     *AddressStringLength)
                                         PWSTR AddressString,
...
NTSYSAPI
LONG NTAPI RtlIpv6StringToAddressA(_In_ PCSTR S, _Out_ PCSTR *Terminator,
                                   _Out_ struct in6_addr *Addr);

NTSYSAPI
LONG NTAPI RtlIpv6StringToAddressExA(_In_ PCSTR AddressString,
                                     _Out_ struct in6_addr *Address,
                                     _Out_ PULONG ScopeId, _Out_ PUSHORT Port);

NTSYSAPI
LONG NTAPI RtlIpv6StringToAddressW(_In_ PCWSTR S, _Out_ PCWSTR *Terminator,
                                   _Out_ struct in6_addr *Addr);

NTSYSAPI
LONG NTAPI RtlIpv6StringToAddressExW(_In_ PCWSTR AddressString,
                                     _Out_ struct in6_addr *Address,
                                     _Out_ PULONG ScopeId, _Out_ PUSHORT Port);
...
NTSYSAPI
PSTR NTAPI RtlEthernetAddressToStringA(_In_ const DL_EUI48 *Addr,
                                       _Out_writes_(18) PSTR S);

NTSYSAPI
PWSTR
NTAPI
RtlEthernetAddressToStringW(_In_ const DL_EUI48 *Addr,
                            _Out_writes_(18) PWSTR S);

NTSYSAPI
LONG NTAPI RtlEthernetStringToAddressA(_In_ PCSTR S, _Out_ PCSTR *Terminator,
                                       _Out_ DL_EUI48 *Addr);

NTSYSAPI
LONG NTAPI RtlEthernetStringToAddressW(_In_ PCWSTR S, _Out_ LPCWSTR *Terminator,
                                       _Out_ DL_EUI48 *Addr);
