...
// !!!! do not #pragma once, we use this file twice(once for native and once for
// mgd) in managed interop
...
//  In-band Event IDs
//
ENUM BDA_EVENT_ID{BDA_EVENT_SIGNAL_LOSS = 0,
                  ...
                  BDA_EVENT_SMART_CARD_REMOVED} BDA_EVENT_ID,
    *PBDA_EVENT_ID;
...
ENUM BDA_MULTICAST_MODE{BDA_PROMISCUOUS_MULTICAST = 0, BDA_FILTERED_MULTICAST,
                        BDA_NO_MULTICAST} BDA_MULTICAST_MODE,
    *PBDA_MULTICAST_MODE;
...
//
ENUM BDA_SIGNAL_STATE{BDA_SIGNAL_UNAVAILABLE = 0, BDA_SIGNAL_INACTIVE,
                      BDA_SIGNAL_ACTIVE} BDA_SIGNAL_STATE,
    *PBDA_SIGNAL_STATE;
...
//
ENUM BDA_CHANGE_STATE{BDA_CHANGES_COMPLETE = 0,
                      BDA_CHANGES_PENDING} BDA_CHANGE_STATE,
    *PBDA_CHANGE_STATE;
...
ENUM MEDIA_SAMPLE_CONTENT{
    ...
    MEDIA_TRANSPORT_PAYLOAD  //  gathered TS packet payloads (PES packets,
                             //  etc...)
...
typedef struct _BDA_WMDRM_RENEWLICENSE {
  ...
  BYTE argbXmrLicenceOutputBuffer[MIN_DIMENSION]; // License and Entitlement
                                                  // Token Buffer
...
ENUM MUX_PID_TYPE{
    ...
    PID_MPEG2_SECTION_PSI_SI, //  ISO 13818_1 Sections PAT, PMT, CAT, Private.
                              //  Service Information Sections e.g SDT, NIT,
                              //  EIT, BAT
} MUX_PID_TYPE;
...
typedef struct _BDA_RATING_PINRESET {
  ...
  BYTE argbNewPin[MIN_DIMENSION]; // Null terminated UTF8. Use empty string if
                                  // disable pin
...
ENUM DVBSystemType{
    DVB_Cable, DVB_Terrestrial, DVB_Satellite, ISDB_Terrestrial, ISDB_Satellite,
} DVBSystemType;
...
//        accordingly if the order/value changes. Also make sure
//        ehiProxy.asmmeta, ehiVidCtl.asmmeta and bdatunepia.asmmeta are
//        properly updated.
//
ENUM ComponentCategory{
    CategoryNotSet = -1, CategoryOther = 0,   CategoryVideo,
    CategoryAudio,       CategoryText,        CategorySubtitles,
    CategoryCaptions,    CategorySuperimpose, CategoryData,
    CATEGORY_COUNT,
} ComponentCategory;
...
ENUM MPEG2StreamType{
    BDA_UNITIALIZED_MPEG2STREAMTYPE = -1, Reserved1 = 0x00,
    ISO_IEC_11172_2_VIDEO = 0x01, ISO_IEC_13818_2_VIDEO = 0x02,
    ISO_IEC_11172_3_AUDIO = 0x03, ISO_IEC_13818_3_AUDIO = 0x04,
    ISO_IEC_13818_1_PRIVATE_SECTION = 0x05, ISO_IEC_13818_1_PES = 0x06,
    ISO_IEC_13522_MHEG = 0x07, ANNEX_A_DSM_CC = 0x08, ITU_T_REC_H_222_1 = 0x09,
    ISO_IEC_13818_6_TYPE_A = 0x0A, ISO_IEC_13818_6_TYPE_B = 0x0B,
    ISO_IEC_13818_6_TYPE_C = 0x0C, ISO_IEC_13818_6_TYPE_D = 0x0D,
    ISO_IEC_13818_1_AUXILIARY = 0x0E, ISO_IEC_13818_7_AUDIO = 0x0F,
    ISO_IEC_14496_2_VISUAL = 0x10, ISO_IEC_14496_3_AUDIO = 0x11,
    ISO_IEC_14496_1_IN_PES = 0x12, ISO_IEC_14496_1_IN_SECTION = 0x13,
    ISO_IEC_13818_6_DOWNLOAD = 0x14, METADATA_IN_PES = 0x15,
    METADATA_IN_SECTION = 0x16, METADATA_IN_DATA_CAROUSEL = 0x17,
    METADATA_IN_OBJECT_CAROUSEL = 0x18, METADATA_IN_DOWNLOAD_PROTOCOL = 0x19,
    IRPM_STREAMM = 0x1A, ITU_T_H264 = 0x1B,
    ISO_IEC_13818_1_RESERVED = 0x1C, // continues until 0x7F
    USER_PRIVATE =
        0x10, // standard says 0x80, retaining for backwards compatibility
    HEVC_VIDEO_OR_TEMPORAL_VIDEO = 0x24, HEVC_TEMPORAL_VIDEO_SUBSET = 0x25,
    MPEG_H_AUDIO = 0x2D,    // used for single-stream delivery or for the main
                            // stream in case of multi-stream delivery.
    MPEG_H_AUDIO_MS = 0x2E, // used for additional (auxiliary) stream in case of
                            // MPEG-H multi-stream delivery.
    ISO_IEC_USER_PRIVATE = 0x80, DOLBY_AC3_AUDIO = 0x81,
...
ENUM FECMethod{
    ...
    BDA_FEC_BCH, // Bose-Chaudhuri-Hocquenghem multiple error correction binary
                 // block code
    BDA_FEC_RS_147_130, // The FEC is Reed-Solomon 147/130 (outer FEC)
                        // DirecTV-DSS
    BDA_FEC_MAX,
} FECMethod;
...
ENUM ModulationType{
    ...
    BDA_MOD_896QAM,
    BDA_MOD_1024QAM,
    BDA_MOD_QPSK,  // Quadrature Phase Shift Keying (including backwards
                   // compatible mode)
    ...
    BDA_MOD_RF,   // analog TV (Video standards such as NTSC/PAL/SECAM specified
                  // in IAnalogLocator VideoStandard property)
    ...
    BDA_MOD_ISDB_T_TMCC, // Automatic demodulation by Transmission and
                         // Multiplexing Configuration Control signal for ISDB-T
    BDA_MOD_ISDB_S_TMCC, // Automatic demodulation by Transmission and
                         // Multiplexing Configuration Control signal for ISDB-S
    BDA_MOD_MAX,
} ModulationType;
...
ENUM SpectralInversion{BDA_SPECTRAL_INVERSION_NOT_SET = -1,
...
ENUM LNB_Source{
    BDA_LNB_SOURCE_NOT_SET = -1, BDA_LNB_SOURCE_NOT_DEFINED = 0,
    BDA_LNB_SOURCE_A = 1, //
    BDA_LNB_SOURCE_B = 2, //
    BDA_LNB_SOURCE_C = 3, //
    BDA_LNB_SOURCE_D = 4, //
...
ENUM GuardInterval{
    BDA_GUARD_NOT_SET = -1, BDA_GUARD_NOT_DEFINED = 0,
...
ENUM RollOff{
    BDA_ROLL_OFF_NOT_SET = -1, BDA_ROLL_OFF_NOT_DEFINED = 0,
...
ENUM Pilot{
    BDA_PILOT_NOT_SET = -1, BDA_PILOT_NOT_DEFINED = 0,
...
//
ENUM BDA_Frequency{BDA_FREQUENCY_NOT_SET = -1,
                   BDA_FREQUENCY_NOT_DEFINED = 0} BDA_Frequency;
...
//
ENUM BDA_Range{BDA_RANGE_NOT_SET = -1, BDA_RANGE_NOT_DEFINED = 0} BDA_Range;

//  Settings for Tuner Channel Bandwidth
//
ENUM BDA_Channel_Bandwidth{BDA_CHAN_BANDWITH_NOT_SET = -1,
                           BDA_CHAN_BANDWITH_NOT_DEFINED =
                               0} BDA_Channel_Bandwidth;

//  Settings for Tuner Frequency Multiplier
//
ENUM BDA_Frequency_Multiplier{BDA_FREQUENCY_MULTIPLIER_NOT_SET = -1,
                              BDA_FREQUENCY_MULTIPLIER_NOT_DEFINED =
                                  0} BDA_Frequency_Multiplier;
...
FLAGS BDA_Comp_Flags{
    ...
    BDACOMP_EXCLUDE_TS_FROM_TR =
        0x00000001, // never put TS in TR equiv comparison
    BDACOMP_INCLUDE_LOCATOR_IN_TR =
        0x00000002, // always include loc in TR equiv comparison
    BDACOMP_INCLUDE_COMPONENTS_IN_TR =
        0x00000004, // always include components in TR equiv comparison
} BDA_Comp_Flags;
...
ENUM BDA_CONDITIONALACCESS_REQUESTTYPE{
    ...
    CONDITIONALACCESS_ACCESS_NOT_POSSIBLE, CONDITIONALACCESS_ACCESS_POSSIBLE,
    CONDITIONALACCESS_ACCESS_POSSIBLE_NO_STREAMING_DISRUPTION} BDA_CONDITIONALACCESS_REQUESTTYPE;
...
ENUM BDA_CONDITIONALACCESS_MMICLOSEREASON{
    ...
    CONDITIONALACCESS_DIALOG_USER_NOT_AVAILABLE} BDA_CONDITIONALACCESS_MMICLOSEREASON;

ENUM BDA_CONDITIONALACCESS_SESSION_RESULT{
    CONDITIONALACCESS_SUCCESSFULL = 0, CONDITIONALACCESS_ENDED_NOCHANGE,
    CONDITIONALACCESS_ABORTED} BDA_CONDITIONALACCESS_SESSION_RESULT;

ENUM BDA_DISCOVERY_STATE{BDA_DISCOVERY_UNSPECIFIED = 0, BDA_DISCOVERY_REQUIRED,
                         BDA_DISCOVERY_COMPLETE} BDA_DISCOVERY_STATE;
