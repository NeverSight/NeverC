...
SPB_TRANSFER_LIST_ENTRY_INIT_SIMPLE(_In_ SPB_TRANSFER_DIRECTION Direction,
                                    _In_ ULONG DelayInUs,
                                    _Pre_writable_byte_size_(BufferCb)
                                        PVOID Buffer,
                                    _In_ ULONG BufferCb) {
...
FORCEINLINE
SPB_TRANSFER_LIST_ENTRY
SPB_TRANSFER_LIST_ENTRY_INIT_NON_PAGED(_In_ SPB_TRANSFER_DIRECTION Direction,
                                       _In_ ULONG DelayInUs,
                                       _Pre_writable_byte_size_(BufferCb)
                                           PVOID Buffer,
                                       _In_ ULONG BufferCb) {
...
FORCEINLINE
SPB_TRANSFER_LIST_ENTRY
SPB_TRANSFER_LIST_ENTRY_INIT_MDL(_In_ SPB_TRANSFER_DIRECTION Direction,
                                 _In_ ULONG DelayInUs, _In_ PMDL Mdl) {
  SPB_TRANSFER_LIST_ENTRY entry;
...
SPB_TRANSFER_LIST_ENTRY_INIT_BUFFER_LIST(
    _In_ SPB_TRANSFER_DIRECTION Direction, _In_ ULONG DelayInUs,
    _In_ SPB_TRANSFER_BUFFER_LIST_ENTRY BufferList[], _In_ ULONG BufferListCe) {
  SPB_TRANSFER_LIST_ENTRY entry;
...
typedef struct SPB_TRANSFER_LIST {
  ...
  _Field_range_(==, sizeof(SPB_TRANSFER_LIST)) ULONG Size;
...
VOID FORCEINLINE SPB_TRANSFER_LIST_INIT(_Out_ SPB_TRANSFER_LIST *TransferList,
                                        _In_ ULONG TransferCount) {
  memset(TransferList, 0,
...
typedef struct SPB_MULTI_SPI_TRANSFER_HEADER {
  ...
  _Field_range_(==, sizeof(struct SPB_MULTI_SPI_TRANSFER)) ULONG Size;
...
VOID FORCEINLINE SPB_MULTI_SPI_TRANSFER_INIT(
    _Out_ SPB_MULTI_SPI_TRANSFER *SpiTransfer,
    _In_ SPB_MULTI_SPI_TRANSFER_MODE Mode, _In_ ULONG TransferPhaseCount,
    _In_ ULONG WritePhaseSingleSpiByteCount, _In_ ULONG WaitCycleByteCount) {
  memset(SpiTransfer, 0,
  ...
  SpiTransfer->Header.WritePhaseSingleSpiByteCount =
      WritePhaseSingleSpiByteCount;
  SpiTransfer->Header.WaitCycleByteCount = WaitCycleByteCount;
  SpiTransfer->TransferPhaseCount = TransferPhaseCount;
}

VOID FORCEINLINE SPB_MULTI_SPI_WRITE_TRANSFER_INIT(
    _Out_ SPB_MULTI_SPI_WRITE_TRANSFER *SpiTransfer,
    _In_ SPB_MULTI_SPI_TRANSFER_MODE Mode,
    _In_ ULONG WritePhaseSingleSpiByteCount, _In_ ULONG WaitCycleByteCount) {
  SPB_MULTI_SPI_TRANSFER_INIT(&SpiTransfer->SpiTransfer, Mode, 1,
                              WritePhaseSingleSpiByteCount, WaitCycleByteCount);
}

VOID FORCEINLINE SPB_MULTI_SPI_READ_TRANSFER_INIT(
    _Out_ SPB_MULTI_SPI_READ_TRANSFER *SpiTransfer,
    _In_ SPB_MULTI_SPI_TRANSFER_MODE Mode,
    _In_ ULONG WritePhaseSingleSpiByteCount, _In_ ULONG WaitCycleByteCount) {
  SPB_MULTI_SPI_TRANSFER_INIT(&SpiTransfer->SpiTransfer, Mode, 2,
                              WritePhaseSingleSpiByteCount, WaitCycleByteCount);
}

// begin_wpp config
// CUSTOM_TYPE(SPB_TRANSFER_DIRECTION, ItemEnum(_SPB_TRANSFER_DIRECTION));
// CUSTOM_TYPE(SPB_TRANSFER_BUFFER_FORMAT,
// ItemEnum(_SPB_TRANSFER_BUFFER_FORMAT)); end_wpp
