...
DEFINE_GUID(GUID_DEVINTERFACE_LAMP, 0x6c11e9e3, 0x8238, 0x4f0a, 0x0a, 0x19,
            0xaa, 0xec, 0x26, 0xca, 0x5e, 0x98);

// The string literal for constructing the reference string of the lamp device
// interface.
...
DEFINE_GUID(GUID_LAMP_RESOURCES_LOST, 0xf770e98c, 0x4403, 0x48c9, 0xb1, 0xd2,
            0x4e, 0xec, 0x33, 0x02, 0xe4, 0x1f);

// The PnP event which indicates the hardware resources previously lost have
// become available to the lamp device again.
DEFINE_GUID(GUID_LAMP_RESOURCES_AVAILABLE, 0x185fe7ce, 0x2616, 0x481b, 0x90,
            0x94, 0x20, 0xbb, 0x89, 0x3a, 0xcd, 0x81);
...
// Gets the operating mode (i.e. white-vs-color light) with which the device is
// currently configured. I/O parameter type: LAMP_MODE
...
// Gets the current light intensity if the device is configured to emit white
// light. I/O parameter type: LAMP_INTENSITY_WHITE
...
// Gets the current light intensity if the device is configured to emit color
// light. I/O parameter type: LAMP_INTENSITY_COLOR
...
typedef struct LAMP_CAPABILITIES_COLOR {
  BOOLEAN
  IsSupported; // True if the device can emit color light; false otherwise.
  BOOLEAN IsLightIntensityAdjustable;
