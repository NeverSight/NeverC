...
typedef struct _D3DKMT_CREATEDEVICEFLAGS {
  ...
  UINT TestDevice : 1; // 0x00000008 The device is created by a test.
                       // Testsigning must be enabled.
...
typedef struct _D3DKMT_CREATEDEVICE {
  ...
  D3DKMT_PTR(D3DDDI_ALLOCATIONLIST *,
             pAllocationList); // out: D3D10 compatibility.
  UINT AllocationListSize;     // out: D3D10 compatibility.
  D3DKMT_PTR(D3DDDI_PATCHLOCATIONLIST *,
             pPatchLocationList); // out: D3D10 compatibility.
...
typedef struct _D3DKMT_CREATECONTEXT {
  ...
  D3DKMT_PTR(VOID *,
             pCommandBuffer); // out: Pointer to the first command buffer.
  UINT CommandBufferSize;     // out: Command buffer size (bytes).
  D3DKMT_PTR(D3DDDI_ALLOCATIONLIST *,
             pAllocationList); // out: Pointer to the first allocation list.
  UINT AllocationListSize;     // out: Allocation list size (elements).
  D3DKMT_PTR(
      D3DDDI_PATCHLOCATIONLIST *,
      pPatchLocationList);    // out: Pointer to the first patch location list.
  UINT PatchLocationListSize; // out: Patch location list size (elements).
  D3DGPU_VIRTUAL_ADDRESS
  CommandBuffer; // out: GPU virtual address of the command buffer. _ADVSCH_
...
typedef struct _D3DKMT_CREATESYNCHRONIZATIONOBJECT {
  ...
  Info; // in:  Attributes of the synchronization object.
  D3DKMT_HANDLE
  hSyncObject; // out: Handle to the synchronization object created.
} D3DKMT_CREATESYNCHRONIZATIONOBJECT;

typedef struct _D3DKMT_CREATESYNCHRONIZATIONOBJECT2 {
  D3DKMT_HANDLE hDevice; // in:  Handle to the device.
  D3DDDI_SYNCHRONIZATIONOBJECTINFO2
  Info; // in/out: Attributes of the synchronization object.
  D3DKMT_HANDLE
  hSyncObject; // out: Handle to the synchronization object created.
...
typedef struct _D3DKMT_CREATENATIVEFENCE {
  ...
  BYTE PrivateDriverData
      [D3DDDI_NATIVE_FENCE_PDD_SIZE]; // in/out: Private driver data to pass to
                                      // KMD CreateNativeFence call,
...
typedef struct _D3DKMT_GETNATIVEFENCELOGDETAIL {
  D3DKMT_HANDLE
  hHwQueue; // in: HWQueue for which log buffer details are requested
...
typedef struct _D3DKMT_DESTROYSYNCHRONIZATIONOBJECT {
  D3DKMT_HANDLE hSyncObject; // in:  Identifies the synchronization objects
                             // being destroyed.
} D3DKMT_DESTROYSYNCHRONIZATIONOBJECT;

typedef struct _D3DKMT_OPENSYNCHRONIZATIONOBJECT {
  D3DKMT_HANDLE hSharedHandle; // in: shared handle to synchronization object to
                               // be opened.
  D3DKMT_HANDLE hSyncObject;   // out: Handle to sync object in this process.
...
typedef struct _D3DKMT_WAITFORSYNCHRONIZATIONOBJECT {
  ...
  D3DKMT_HANDLE
  ObjectHandleArray[D3DDDI_MAX_OBJECT_WAITED_ON]; // in: Specifies the
                                                  // object to wait on.
...
typedef struct _D3DKMT_SIGNALSYNCHRONIZATIONOBJECT {
  ...
  D3DKMT_HANDLE
  ObjectHandleArray[D3DDDI_MAX_OBJECT_SIGNALED]; // in: Specifies the object
                                                 // to be signaled.
...
typedef struct _D3DKMT_SIGNALSYNCHRONIZATIONOBJECT2 {
  ...
  D3DKMT_HANDLE
  BroadcastContext[D3DDDI_MAX_BROADCAST_CONTEXT]; // in: Specifies the
...
typedef struct _D3DKMT_LOCK {
  ...
  D3DGPU_VIRTUAL_ADDRESS GpuVirtualAddress; // out: GPU's Virtual Address of
                                            // locked allocation. _ADVSCH_
} D3DKMT_LOCK;

typedef struct _D3DKMT_UNLOCK {
  D3DKMT_HANDLE hDevice; // in: Identifies the device
  UINT NumAllocations;   // in: Number of allocations in the array
  D3DKMT_PTR(CONST D3DKMT_HANDLE *,
             phAllocations); // in: array of allocations to unlock
} D3DKMT_UNLOCK;

typedef enum _D3DKMDT_MODE_PRUNING_REASON {
  D3DKMDT_MPR_UNINITIALIZED = 0, // mode was pruned or is supported because of:
  D3DKMDT_MPR_ALLCAPS =
      1, //   all of the monitor caps (only used to imply lack of support - for
         //   support, specific reason is always indicated)
  ...
  D3DKMDT_MPR_DESCRIPTOR_MONITOR_FREQUENCY_RANGE =
      3, //   monitor frequency range in the monitor descriptor
  D3DKMDT_MPR_DESCRIPTOR_OVERRIDE_MONITOR_SOURCE_MODE =
      4, //   monitor source mode in the monitor descriptor override
  D3DKMDT_MPR_DESCRIPTOR_OVERRIDE_MONITOR_FREQUENCY_RANGE =
      5, //   monitor frequency range in the monitor descriptor override
  D3DKMDT_MPR_DEFAULT_PROFILE_MONITOR_SOURCE_MODE =
      6, //   monitor source mode in the default monitor profile
  D3DKMDT_MPR_DRIVER_RECOMMENDED_MONITOR_SOURCE_MODE =
      7, //   monitor source mode recommended by the driver
  D3DKMDT_MPR_MONITOR_FREQUENCY_RANGE_OVERRIDE =
      8, //   monitor frequency range override
  D3DKMDT_MPR_CLONE_PATH_PRUNED =
      9, //   Mode is pruned because other path(s) in clone cluster has(have) no
         //   mode supported by monitor
...
typedef struct _D3DKMT_SETDISPLAYMODE {
  ...
  UINT
      PrivateDriverFormatAttribute; // out: Private Format Attribute of the
                                    // current primary surface if
                                    // DxgkSetDisplayMode failed with
...
typedef struct _D3DKMT_PRESENTFLAGS {
  union {
    struct {
...
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
      UINT FlipStereo : 1; // 0x00020000   // This is a flip from a stereo
                           // alloc. Used in addition to Flip.
      UINT FlipStereoTemporaryMono
          : 1; // 0x00040000   // This is a flip from a stereo alloc. The left
               // image should used to produce both images. Used in addition to
               // Flip.
      UINT FlipStereoPreferRight
          : 1; // 0x00080000   // This is a flip from a stereo alloc. Use the
               // right image when cloning to a mono monitor. Used in addition
               // to Flip.
      UINT BltStereoUseRight
          : 1; // 0x00100000   // This is a Blt from a stereo alloc to a mono
               // alloc. The right image should be used.
      UINT PresentHistoryTokenOnly
          : 1; // 0x00200000   // Submit Present History Token only.
      UINT PresentRegionsValid
          : 1;             // 0x00400000   // Ptr to present regions is valid
      ...
      UINT PresentIndirect
          : 1; // 0x10000000   // Present to an indirect-display adapter
...
typedef struct _D3DKMT_FLIPMANAGER_AUXILIARYPRESENTINFO {
  ...
  // out: An IFlip submitted token was subsequently cancelled and should be
  // resubmitted as non-IFlip token.
...
// User mode timeout is in milliseconds, kernel mode timeout is in 100
// nanoseconds
...
// The size of D3DKMT_PRESENTHISTORYTOKEN is fixed since it is part of
// D3DKMT_PRESENT
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
...
typedef struct _D3DKMT_PRESENT {
  ...
  VidPnSourceId; // in: VidPn source ID if RestrictVidPnSource is flagged
...
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM1_3)
  union {
    D3DKMT_HANDLE hAdapter; // in: iGpu adapter for PHT redirection. Valid only
                            // when the CrossAdapter flag is set.
    D3DKMT_HANDLE
    hIndirectContext; // in: indirect adapter context for redirecting
                      // through the DoD present path. Only
                      //     valid if PresentIndirect flag is set.
  };
  UINT Duration; // in: Per-present duration. Valid only when the DurationValid
                 // flag is set.
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_0)
  D3DKMT_PTR(_Field_size_(BroadcastContextCount) D3DKMT_HANDLE *,
             BroadcastSrcAllocation); // in: LDA
  D3DKMT_PTR(_Field_size_opt_(BroadcastContextCount) D3DKMT_HANDLE *,
             BroadcastDstAllocation); // in: LDA
  UINT PrivateDriverDataSize;         // in:
  D3DKMT_PTR(_Field_size_bytes_(PrivateDriverDataSize) PVOID,
             pPrivateDriverData);  // in: Private driver data to pass to
                                   // DdiPresent and DdiSetVidPnSourceAddress
...
typedef struct _D3DKMT_PRESENT_REDIRECTED {
  ...
  D3DKMT_ALIGN64 ULONGLONG
      WaitedFenceValue; // in: Fence value of hSyncObj that PHT waits on
  D3DKMT_PRESENTHISTORYTOKEN PresentHistoryToken;
...
typedef struct _D3DKMT_CREATE_DOORBELL {
  ...
  D3DKMT_HANDLE
  hRingBufferControl; // in opt:  ring buffer control area allocation handle
  D3DKMT_CREATE_DOORBELL_FLAGS Flags; // in opt:  flags
  _Field_range_(0, D3DDDI_DOORBELL_PRIVATEDATA_MAX_BYTES_WDDM3_1) UINT
      PrivateDriverDataSize; // in:      Size of private driver data
  D3DKMT_PTR(_Field_size_(PrivateDriverDataSize) VOID *,
             PrivateDriverData); // in/out:  Private driver data
  D3DKMT_PTR(
      VOID *,
      DoorbellCPUVirtualAddress); // out:     CPU VA of the created doorbell
  D3DKMT_PTR(VOID *,
             DoorbellSecondaryCPUVirtualAddress); // out opt: Secondary CPU VA
                                                  // of the created doorbell
  D3DKMT_PTR(VOID *,
             DoorbellStatusCPUVirtualAddress); // out:     CPU VA of the status
                                               // page of doorbell
  D3DKMT_PTR(
      VOID *,
      HwQueueProgressFenceLastQueuedValueCPUVirtualAddress); // out:     CPU VA
                                                             // of the location
                                                             // where the UMD
                                                             // will write
                                                             //          the
                                                             ...
                                                             //          buffer.
  D3DKMT_HANDLE
  hDoorbell; // out:     Runtime handle to the newly created doorbell object
...
typedef struct _D3DKMT_CONNECT_DOORBELL {
  D3DKMT_HANDLE
  hDoorbell; // in: UM handle of the doorbell which must be connected
  D3DKMT_CONNECT_DOORBELL_FLAGS Flags;
} D3DKMT_CONNECT_DOORBELL;

typedef struct _D3DKMT_DESTROY_DOORBELL {
  D3DKMT_HANDLE
  hDoorbell; // in: UM handle of the doorbell which must be destroyed
...
typedef enum D3DKMT_MULTIPLANE_OVERLAY_YCbCr_FLAGS {
  ...
  D3DKMT_MULTIPLANE_OVERLAY_YCbCr_FLAG_xvYCC =
      0x4, // xvYCC vs. conventional YCbCr
...
typedef struct D3DKMT_CHECK_MULTIPLANE_OVERLAY_SUPPORT_RETURN_INFO {
  union {
    struct {
      UINT FailingPlane : 4; // The 0 based index of the first plane that could
                             // not be supported
      UINT TryAgain : 1;     // The configuration is not supported due to a
                         // transition condition, which should shortly go away
      UINT Reserved : 27;
    };
    UINT Value;
  };
...
typedef struct _D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT {
  ...
  D3DKMT_PTR(D3DKMT_CHECK_MULTIPLANE_OVERLAY_PLANE *,
             pOverlayPlanes); // in : Array of resource handles to pin
  BOOL Supported;
...
typedef struct _D3DKMT_MULTIPLANE_OVERLAY_ATTRIBUTES2 {
  UINT Flags;   // D3DKMT_MULTIPLANE_OVERLAY_FLAGS
  RECT SrcRect; // Specifies the source rectangle, of type RECT, relative to the
                // source resource.
  RECT DstRect; // Specifies the destination rectangle, of type RECT, relative
                // to the monitor resolution.
  RECT ClipRect; // Specifies any additional clipping, of type RECT, relative to
                 // the DstRect rectangle, after the data has been stretched
                 // according to the values of SrcRect and DstRect.

  // The driver and hardware can use the ClipRect member to apply a common
  // stretch factor as the clipping changes when an app occludes part of the
  // DstRect destination rectangle.
  D3DDDI_ROTATION
  Rotation; // Specifies the clockwise rotation of the overlay plane, given
            // as a value from the D3DDDI_ROTATION enumeration.
  D3DKMT_MULTIPLANE_OVERLAY_BLEND
  Blend; // Specifies the blend mode that applies to this overlay plane and
         // the plane beneath it, given as a value from the
         // DXGK_MULTIPLANE_OVERLAY_BLEND enumeration.
  UINT DirtyRectCount;
  D3DKMT_PTR(RECT *, pDirtyRects);
  D3DKMT_MULTIPLANE_OVERLAY_VIDEO_FRAME_FORMAT
  VideoFrameFormat; // DXGK_MULTIPLANE_OVERLAY_VIDEO_FRAME_FORMAT
  D3DDDI_COLOR_SPACE_TYPE ColorSpace;
  D3DKMT_MULTIPLANE_OVERLAY_STEREO_FORMAT
  StereoFormat;              // DXGK_MULTIPLANE_OVERLAY_STEREO_FORMAT
  ...
  DXGKMT_MULTIPLANE_OVERLAY_STEREO_FLIP_MODE
  StereoFlipMode; // DXGK_MULTIPLANE_OVERLAY_STEREO_FLIP_MODE
  DXGKMT_MULTIPLANE_OVERLAY_STRETCH_QUALITY
  StretchQuality; // DXGK_MULTIPLANE_OVERLAY_STRETCH_QUALITY
  UINT Reserved1;
...
typedef struct _D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT2 {
  ...
  D3DKMT_HANDLE hDevice;  // in : Indentifies the device
  UINT PlaneCount;        // in : Number of resources to pin
  D3DKMT_PTR(D3DKMT_CHECK_MULTIPLANE_OVERLAY_PLANE2 *,
             pOverlayPlanes); // in : Array of resource handles to pin
  BOOL Supported;
  D3DKMT_CHECK_MULTIPLANE_OVERLAY_SUPPORT_RETURN_INFO ReturnInfo;
...
typedef struct _D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT3 {
  ...
  _Field_size_(PlaneCount) D3DKMT_CHECK_MULTIPLANE_OVERLAY_PLANE3 *
      *ppOverlayPlanes;      // in : Array of pointers to overlay planes
  UINT PostCompositionCount; // in : Number of resources to pin
  _Field_size_(PostCompositionCount)
      D3DKMT_MULTIPLANE_OVERLAY_POST_COMPOSITION_WITH_SOURCE *
          *ppPostComposition; // in : Array of pointers to overlay planes
  BOOL Supported;
  D3DKMT_CHECK_MULTIPLANE_OVERLAY_SUPPORT_RETURN_INFO ReturnInfo;
...
typedef struct _D3DKMT_MULTIPLANE_OVERLAY3 {
  ...
  D3DKMT_PTR(_Field_size_(AllocationCount) D3DKMT_HANDLE *, pAllocationList);
  UINT DriverPrivateDataSize;
  D3DKMT_PTR(_Field_size_bytes_(DriverPrivateDataSize) VOID *,
             pDriverPrivateData);
...
typedef struct _D3DKMT_PRESENT_MULTIPLANE_OVERLAY_FLAGS {
  union {
    struct {
      UINT FlipStereo
          : 1; // 0x00000001 This is a flip from a stereo alloc. Used in
               // addition to FlipImmediate or FlipOnNextVSync.
      UINT FlipStereoTemporaryMono
          : 1; // 0x00000002 This is a flip from a stereo alloc. The left image
               // should used. Used in addition to FlipImmediate or
               // FlipOnNextVSync.
      UINT FlipStereoPreferRight
          : 1; // 0x00000004 This is a flip from a stereo alloc. The right image
               // should used when cloning to a mono monitor. Used in addition
      ...
      UINT TrueImmediate
          : 1; // 0x00000200 If a present interval is 0, allow tearing rather
               // than override a previously queued flip
...
typedef struct _D3DKMT_PRESENT_MULTIPLANE_OVERLAY3 {
  D3DKMT_HANDLE hAdapter; // in: adapter handle
  UINT ContextCount;
  D3DKMT_PTR(_Field_size_(ContextCount) D3DKMT_HANDLE *, pContextList);
  ...
  UINT PresentPlaneCount;
  // Note: Array-of-pointers don't work in 32bit WSL
  _Field_size_(PresentPlaneCount) D3DKMT_MULTIPLANE_OVERLAY3 **ppPresentPlanes;
  D3DKMT_PTR(D3DKMT_MULTIPLANE_OVERLAY_POST_COMPOSITION *, pPostComposition);
  UINT Duration;
  D3DDDI_HDR_METADATA_TYPE HDRMetaDataType;
  UINT HDRMetaDataSize;
  D3DKMT_PTR(_Field_size_bytes_(HDRMetaDataSize) const VOID *, pHDRMetaData);
...
typedef struct _D3DKMT_MULTIPLANE_OVERLAY_CAPS {
  union {
    struct {
      ...
      UINT RotationWithoutIndependentFlip
          : 1; // Rotation, but without simultaneous IndependentFlip support
      ...
      UINT Plane0ForVirtualModeOnly
          : 1; // Stretching plane 0 will also stretch the HW cursor and should
               // only be used for virtual mode support
...
typedef struct _D3DKMT_OUTPUTDUPLPRESENTTOHWQUEUE {
  ...
  D3DKMT_PTR(_Field_size_(BroadcastHwQueueCount) D3DKMT_HANDLE *, hHwQueues);
  D3DKMT_PRESENT_RGNS PresentRegions; // in: Dirty and move regions
  D3DKMT_OUTPUTDUPLPRESENTFLAGS Flags;
...
typedef struct _D3DKMT_RENDER {
  ...
  UINT PatchLocationCount; // in: Number of patch locations in patch allocation
                           // list.
  D3DKMT_PTR(VOID *,
             pNewCommandBuffer); // out: Pointer to the next command buffer to
                                 // use. in: When RenderKm flag is set, it
                                 // points to a command buffer.
  UINT NewCommandBufferSize; // in: Size requested for the next command buffer.
                             // out: Size of the next command buffer to use.
  D3DKMT_PTR(D3DDDI_ALLOCATIONLIST *,
             pNewAllocationList); // out: Pointer to the next allocation list to
                                  // use. in: When RenderKm flag is set, it
                                  // points to an allocation list.
  UINT NewAllocationListSize;     // in: Size requested for the next allocation
                                  // list. out: Size of the new allocation list.
  D3DKMT_PTR(
      D3DDDI_PATCHLOCATIONLIST *,
      pNewPatchLocationList); // out: Pointer to the next patch location list.
  UINT NewPatchLocationListSize; // in: Size requested for the next patch
                                 // location list. out: Size of the new patch
                                 // location list.
  D3DKMT_RENDERFLAGS Flags;      // in:
  D3DKMT_ALIGN64 ULONGLONG PresentHistoryToken; // in: Present history token for
                                                // redirected present calls
  ULONG BroadcastContextCount; // in: Specifies the number of context
                               //     to broadcast this command buffer to.
  D3DKMT_HANDLE
  BroadcastContext[D3DDDI_MAX_BROADCAST_CONTEXT]; // in: Specifies the
                                                  ...
                                                  // to
                                                  //     broadcast to.
  ULONG QueuedBufferCount; // out: Number of DMA buffer queued to this context
                           // after this submission.
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      NewCommandBuffer; // out: GPU virtual address of next command buffer to
                        // use. _ADVSCH_
  D3DKMT_PTR(
      VOID *,
      pPrivateDriverData);    // in: pointer to private driver data. _ADVSCH_
...
typedef struct _D3DKMT_CREATESTANDARDALLOCATION {
  //
  // update onecoreuap/windows/core/ntuser/inc/whwin32.tpl when adding new
  // memeber to this struct
  //
...
typedef struct _D3DKMT_CREATEALLOCATIONFLAGS {
  ...
  UINT CreateProtected : 1;      // 0x00000008 Cannot be used when allocation is
                                 // created from the user mode.
  UINT RestrictSharedAccess : 1; // 0x00000010
  UINT ExistingSysMem : 1;       // 0x00000020 Cannot be used when allocation is
                                 // created from the user mode.
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
  UINT NtSecuritySharing : 1;   // 0x00000040
  UINT ReadOnly : 1;            // 0x00000080
  UINT CreateWriteCombined : 1; // 0x00000100 Cannot be used when allocation is
                                // created from the user mode.
  UINT CreateCached : 1; // 0x00000200 Cannot be used when allocation is created
                         // from the user mode.
  UINT SwapChainBackBuffer : 1; // 0x00000400 Specifies whether an allocation
                                // corresponds to a swap chain back buffer.
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM1_3)
  UINT CrossAdapter : 1;     // 0x00000800
  UINT OpenCrossAdapter : 1; // 0x00001000 Cannot be used when allocation is
                             // created from the user mode.
  UINT PartialSharedCreation : 1; // 0x00002000
  UINT Zeroed
      : 1; // 0x00004000  // out: set when allocation fulfilled by zero pages
  UINT WriteWatch : 1; // 0x00008000  // in: request Mm to track writes to pages
                       // of this allocation
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_3)
  UINT StandardAllocation : 1; // 0x00010000  // in: use pStandardAllocation
                               // instead of pPrivateDriverData
  UINT ExistingSection : 1; // 0x00020000  // in: Use Section Handle instead of
                            // SysMem in D3DDI_ALLOCATIONINFO2
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_6)
  UINT AllowNotZeroed
      : 1; // 0x00040000  // in: indicate zeroed pages are not required
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_7)
  UINT PhysicallyContiguous : 1; // 0x00080000  // in: indicate allocation must
                                 // be physically contguous
  UINT NoKmdAccess
      : 1; // 0x00100000  // in: KMD is not notified about the allocation
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_0)
  UINT SharedDisplayable : 1; // 0x00200000
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_2)
  UINT NoImplicitSynchronization
      : 1; // 0x00400000  // in: request that kernel shouldn't do any implicit
           // synchronization for this allocation
...
typedef struct _D3DKMT_CREATEALLOCATION {
  ...
  D3DKMT_HANDLE hGlobalShare; // out:Shared handle if CreateShared and not
                              // NtSecuritySharing
  D3DKMT_PTR(_Field_size_bytes_(PrivateRuntimeDataSize) CONST VOID *,
             pPrivateRuntimeData);
  UINT PrivateRuntimeDataSize;
  union {
    //
    // update onecoreuap/windows/core/ntuser/inc/whwin32.tpl when adding new
    // memeber to this union
    //
  ...
  union {
    ...
    _Field_size_(NumAllocations)
        D3DDDI_ALLOCATIONINFO2 *pAllocationInfo2; // _ADVSCH_
#endif
  ...
  D3DKMT_PTR(
      HANDLE,
      hPrivateRuntimeResourceHandle); // opaque handle used for event tracing
...
typedef struct _D3DKMT_OPENRESOURCE {
  ...
  UINT
      NumAllocations; // in : Number of allocations associated with the resource
  union {
    ...
    _Field_size_(NumAllocations) D3DDDI_OPENALLOCATIONINFO2
        *pOpenAllocationInfo2; // in : Array of open allocation structs //
                               // _ADVSCH_
#endif
    D3DKMT_PTR_HELPER(AlignUnionTo64)
  };
  D3DKMT_PTR(_Field_size_bytes_(PrivateRuntimeDataSize) VOID *,
             pPrivateRuntimeData); // in : Caller supplied buffer where the
                                   // runtime private data associated with this
                                   // resource will be copied
  UINT PrivateRuntimeDataSize; // in : Size in bytes of the pPrivateRuntimeData
                               // buffer
  D3DKMT_PTR(_Field_size_bytes_(ResourcePrivateDriverDataSize) VOID *,
             pResourcePrivateDriverData); // in : Caller supplied buffer where
                                          // the driver private data associated
                                          // with the resource will be copied
  UINT ResourcePrivateDriverDataSize;     // in : Size in bytes of the
                                          // pResourcePrivateDriverData buffer
  D3DKMT_PTR(
      _Field_size_bytes_(TotalPrivateDriverDataBufferSize) VOID *,
      pTotalPrivateDriverDataBuffer); // in : Caller supplied buffer where the
                                      // Driver private data will be stored
  UINT TotalPrivateDriverDataBufferSize; // in/out : Size in bytes of
                                         // pTotalPrivateDriverDataBuffer / Size
                                         // in bytes of data written to
...
typedef struct _D3DKMT_OPENRESOURCEFROMNTHANDLE {
  ...
  D3DKMT_PTR(
      _In_reads_bytes_opt_(PrivateRuntimeDataSize) VOID *,
      pKeyedMutexPrivateRuntimeData); // in:  Buffer containing initial private
                                      // data.
                                      //      If NULL then
                                      //      PrivateRuntimeDataSize must be 0.
                                      //      It will only be copied if the
                                      //      keyed mutex does not already have
                                      //      private data.
  UINT KeyedMutexPrivateRuntimeDataSize; // in:  Size in bytes of
                                         // pPrivateRuntimeData.
  D3DKMT_HANDLE hSyncObject; // out: Handle to sync object in this process.
...
typedef struct _D3DKMT_OPENSYNCOBJECTFROMNTHANDLE2 {
  ...
  Flags; // in : specifies sync object behavior for this device.
  D3DKMT_HANDLE hSyncObject; // out: Handle to sync object in this process.
  ...
  union {
    ...
    struct {
      D3DKMT_PTR(VOID *,
                 FenceValueCPUVirtualAddress); // out: Read-only mapping of the
                                               // fence value for the CPU
      D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
          FenceValueGPUVirtualAddress; // out: Read/write mapping of the fence
                                       // value for the GPU
      UINT EngineAffinity; // in: Defines physical adapters where the GPU VA
                           // should be mapped
    } MonitoredFence;
...
typedef struct _D3DKMT_OPENNATIVEFENCEFROMNTHANDLE {
  ...
  UINT EngineAffinity;   // in: Defines physical adapters where the GPU VA is
                         // mapped
  ...
  NativeFenceMapping; // out: process mapping information for the fence
                      // object
  BYTE PrivateDriverData
      [D3DDDI_NATIVE_FENCE_PDD_SIZE]; // in/out: Private driver data to pass to
                                      // KMD DdiOpenNativeFence call and copy
                                      // back to UMD
  BYTE Reserved[32];
...
#define SHARED_ALLOCATION_ALL_ACCESS                                           \
  (STANDARD_RIGHTS_REQUIRED | SHARED_ALLOCATION_WRITE)

typedef struct _D3DKMT_QUERYRESOURCEINFOFROMNTHANDLE {
  D3DKMT_HANDLE hDevice;         // in : Indentifies the device
  D3DKMT_PTR(HANDLE, hNtHandle); // in : Global resource handle to open
  D3DKMT_PTR(VOID *,
             pPrivateRuntimeData); // in : Ptr to buffer that will receive
                                   // runtime private data for the resource
  UINT PrivateRuntimeDataSize; // in/out : Size in bytes of buffer passed in for
                               // runtime private data / If pPrivateRuntimeData
                               // was NULL then size in bytes of buffer required
                               // for the runtime private data otherwise size in
                               // bytes of runtime private data copied into the
                               // buffer
  UINT TotalPrivateDriverDataSize; // out : Size in bytes of buffer required to
                                   // hold all the DriverPrivate data for all of
                                   // the allocations associated withe the
                                   // resource
  UINT ResourcePrivateDriverDataSize; // out : Size in bytes of the driver's
                                      // resource private data
  UINT NumAllocations; // out : Number of allocations associated with this
                       // resource
...
typedef struct _D3DKMT_DESTROYALLOCATION {
  ...
  D3DKMT_HANDLE hResource;
  D3DKMT_PTR(CONST D3DKMT_HANDLE *,
             phAllocationList); // in: pointer to an array allocation handles to
                                // destroy
...
typedef struct _D3DKMT_DESTROYALLOCATION2 {
  ...
  Flags; // in: Bit field defined by D3DDDICB_DESTROYALLOCATION2FLAGS
...
typedef struct _D3DKMT_SETALLOCATIONPRIORITY {
  ...
  D3DKMT_PTR(
      CONST UINT *,
      pPriorities); // in: New priority for each of the allocation in the array.
...
typedef struct _D3DKMT_QUERYALLOCATIONRESIDENCY {
  D3DKMT_HANDLE hDevice; // in: Indentifies the device
  D3DKMT_HANDLE
  hResource; // in: pointer to resource owning the list of allocation.
  D3DKMT_PTR(CONST D3DKMT_HANDLE *,
             phAllocationList); // in: pointer to an array allocation to get
                                // residency status.
  UINT AllocationCount;         // in: Number of allocations in phAllocationList
  D3DKMT_PTR(D3DKMT_ALLOCATIONRESIDENCYSTATUS *,
             pResidencyStatus); // out: Residency status of each allocation in
                                // the array.
...
typedef struct _D3DKMT_FLIPINFOFLAGS {
  UINT FlipInterval
      : 1; // 0x00000001 // Set when kmd driver support FlipInterval natively
  UINT Reserved : 31; // 0xFFFFFFFE
} D3DKMT_FLIPINFOFLAGS;

typedef struct _D3DKMT_FLIPQUEUEINFO {
  UINT MaxHardwareFlipQueueLength; // Max flip can be queued for hardware flip
                                   // queue.
  UINT MaxSoftwareFlipQueueLength; // Max flip can be queued for software flip
                                   // queue for non-legacy device.
...
typedef struct _D3DKMT_GET_DEVICE_VIDPN_OWNERSHIP_INFO {
  D3DKMT_HANDLE hDevice;          // in : Indentifies the device
  BOOLEAN bFailedDwmAcquireVidPn; // out : True if Dwm Acquire VidPn failed due
                                  // to another Dwm device having ownership
...
typedef struct _D3DKMT_NODE_PERFDATA {
  ...
  D3DKMT_ALIGN64 ULONGLONG
      Frequency; // out: Clock frequency of the engine in hertz
  D3DKMT_ALIGN64 ULONGLONG MaxFrequency; // out: Max engine clock frequency
  D3DKMT_ALIGN64 ULONGLONG
      MaxFrequencyOC; // out: Max engine over clock frequency
  ULONG Voltage;      // out: Voltage of the engine in milli volts mV
  ULONG VoltageMax;   // out: Max voltage levels in milli volts.
  ULONG
  VoltageMaxOC; // out: Max voltage level while overclocked in milli volts.
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_5)
  D3DKMT_ALIGN64 ULONGLONG
      MaxTransitionLatency; // out: Max transition latency to change the
                            // frequency in 100 nanoseconds
#else
...
typedef struct _D3DKMT_ADAPTER_PERFDATA {
  ...
  PhysicalAdapterIndex; // in: The physical adapter index, in an LDA chain
  D3DKMT_ALIGN64 ULONGLONG
      MemoryFrequency; // out: Clock frequency of the memory in hertz
  D3DKMT_ALIGN64 ULONGLONG
      MaxMemoryFrequency; // out: Max memory clock frequency
  D3DKMT_ALIGN64 ULONGLONG
      MaxMemoryFrequencyOC; // out: Clock frequency of the memory while
                            // overclocked in hertz.
  D3DKMT_ALIGN64 ULONGLONG
      MemoryBandwidth; // out: Amount of memory transferred in bytes
  D3DKMT_ALIGN64 ULONGLONG
      PCIEBandwidth; // out: Amount of memory transferred over PCI-E in bytes
  ...
  UCHAR PowerStateOverride; // out: Overrides dxgkrnls power view of linked
                            // adapters.
...
typedef struct _D3DKMT_ADAPTER_PERFDATACAPS {
  ...
  PhysicalAdapterIndex; // in: The physical adapter index, in an LDA chain
  D3DKMT_ALIGN64 ULONGLONG
      MaxMemoryBandwidth; // out: Max memory bandwidth in bytes for 1 second
  D3DKMT_ALIGN64 ULONGLONG
      MaxPCIEBandwidth; // out: Max pcie bandwidth in bytes for 1 second
  ULONG MaxFanRPM;      // out: Max fan rpm
  ULONG TemperatureMax; // out: Max temperature before damage levels
  ULONG
  TemperatureWarning; // out: The temperature level where throttling begins.
...
typedef struct _D3DKMT_GPUVERSION {
  ...
  PhysicalAdapterIndex; // in: The physical adapter index, in an LDA chain
  WCHAR
  BiosVersion[DXGK_MAX_GPUVERSION_NAME_LENGTH]; // out: The gpu bios version
...
typedef struct _D3DKMT_PARAVIRTUALIZATION {
  ...
  // replicated on adapters. It precludes extended device functions (i.e.
  // Escapes) for paravirtualized devices which not known at all, and therefore
  // assumed not to be secure enough for demanding server scenarios.
...
#endif // DXGKDDI_INTERFACE_VERSION_WDDM3_2
  // If a new enum will be used by DXGI or D3D11 software driver code, update
  // the test content in the area. Search for
  // KMTQAITYPE_PARAVIRTUALIZATION_RENDER in directx\dxg\dxgi\unittests for
  // references.
...
typedef struct _D3DKMT_OPENADAPTERFROMHDC {
  ...
  D3DDDI_VIDEO_PRESENT_SOURCE_ID
  VidPnSourceId; // out: VidPN source ID for that particular display
} D3DKMT_OPENADAPTERFROMHDC;

typedef struct _D3DKMT_OPENADAPTERFROMGDIDISPLAYNAME {
  WCHAR DeviceName[32]; // in:  Name of GDI device from which to open an adapter
                        // instance
  D3DKMT_HANDLE hAdapter; // out: adapter handle
  LUID AdapterLuid;       // out: adapter LUID
  D3DDDI_VIDEO_PRESENT_SOURCE_ID
...
typedef struct _D3DKMT_OPENADAPTERFROMDEVICENAME {
  D3DKMT_PTR(PCWSTR, pDeviceName); // in:  NULL terminated string containing the
                                   // device name to open
  D3DKMT_HANDLE hAdapter;          // out: adapter handle
  LUID AdapterLuid;                // out: adapter LUID
...
typedef struct _D3DKMT_ENUMADAPTERS2 {
  ULONG NumAdapters; // in/out: On input, the count of the pAdapters array
                     // buffer.  On output, the number of adapters enumerated.
  D3DKMT_PTR(D3DKMT_ADAPTERINFO *,
             pAdapters); // out: Array of enumerated adapters containing
                         // NumAdapters elements
...
typedef union _D3DKMT_ENUMADAPTERS_FILTER {
  ...
  // breaking applications. DisplayOnly adapters are also left out of the
  // default enumeration.
  struct {
...
typedef struct _D3DKMT_GETSHAREDPRIMARYHANDLE {
  D3DKMT_HANDLE hAdapter;                       // in: adapter handle
  D3DDDI_VIDEO_PRESENT_SOURCE_ID VidPnSourceId; // in: adapter's VidPN source ID
  D3DKMT_HANDLE hSharedPrimary; // out: global shared primary handle (if one
                                // exists currently)
...
typedef struct _D3DKMT_SHAREDPRIMARYLOCKNOTIFICATION {
  LUID AdapterLuid;
  D3DDDI_VIDEO_PRESENT_SOURCE_ID VidPnSourceId;
  RECTL LockRect; // in: If zero rect then we are locking the whole primary else
                  // the lock sub-rect
...
typedef struct _D3DKMT_PINDIRECTFLIPRESOURCES {
  ...
  D3DKMT_PTR(_Field_size_(ResourceCount) D3DKMT_HANDLE *,
             pResourceList); // in : Array of resource handles to pin
...
typedef struct _D3DKMT_UNPINDIRECTFLIPRESOURCES {
  ...
  D3DKMT_PTR(_Field_size_(ResourceCount) D3DKMT_HANDLE *,
             pResourceList); // in : Array of resource handles to unpin
...
// unused (19 was previously D3DKMT_ESCAPE_GENERIC_ADAPTER_DIAG_INFO)
  ...
  // 22 can be reused for future needs as it was never exposed for external
  // purposes
...
typedef struct _D3DKMT_DOD_SET_DIRTYRECT_MODE {
  BOOL bForceFullScreenDirty; // in: indicates if this adapter should always
                              // give full screen dirty for every Dod present
...
typedef enum _D3DKMT_TDRDBGCTRLTYPE {
  ...
  D3DKMT_TDRDBGCTRLTYPE_UNCONDITIONAL =
      3, // Disables all safety conditions (e.g. check for consecutive
         // recoveries)
  ...
  D3DKMT_TDRDBGCTRLTYPE_FORCEDODVSYNCTDR =
      7,                               // Simulate a Display Only Vsync TDR
...
typedef enum _D3DKMT_VIDSCHESCAPETYPE {
  ...
  D3DKMT_VIDSCHESCAPETYPE_SUSPENDSCHEDULER =
      1,                                  // Suspend/Resume scheduler (obsolate)
...
typedef struct _D3DKMT_GET_PTE {
  ...
  BOOLEAN b64KBPte; // In - Valid only when dual PTEs are supported. Out - PT is
                    // 64KB.
...
typedef struct _D3DKMT_VIDMM_ESCAPE {
  ...
  union {
    ...
    struct {
      ...
      D3DKMT_PTR(
          HANDLE,
          hProcess); // 0 to evict memory for the current process, otherwise it
                     // is a process handle from OpenProcess(PROCESS_ALL_ACCESS,
                     // FALSE, ProcessId).
    } Evict;
    struct {
      D3DKMT_ALIGN64 UINT64
          NtHandle; // Used by D3DKMT_VIDMMESCAPETYPE_EVICT_BY_NT_HANDLE
    } EvictByNtHandle;
    struct {
      union {
        struct {
...
typedef struct _D3DKMT_VIDSCH_ESCAPE {
  ...
  union {
    ...
    struct {
      ...
      union {
        ULONG NodeOrdinal; // valid if TdrControl is set to
    ...
    D3DKMT_ESCAPE_PFN_CONTROL_COMMAND
    PfnControl; // periodic frame notification control
  };
...
enum { D3DKMT_MAX_DMM_ESCAPE_DATASIZE = 100 * 1024 };

// NOTE: If (ProvidedBufferSize >= MinRequiredBufferSize), then
// MinRequiredBufferSize = size of the actual complete data set in the Data[]
// array.
typedef struct _D3DKMT_DMM_ESCAPE {
  _In_ D3DKMT_DMMESCAPETYPE Type;
  _In_ D3DKMT_ALIGN64 D3DKMT_SIZE_T
      ProvidedBufferSize; // actual size of Data[] array, in bytes.
  _Out_ D3DKMT_ALIGN64 D3DKMT_SIZE_T
      MinRequiredBufferSize; // minimum required size of Data[] array to contain
                             // requested data.
...
typedef struct _D3DKMT_DEVICE_ESCAPE {
  ...
  union {
    struct {
      ...
      D3DDDI_VIDEO_PRESENT_SOURCE_ID
      VidPnSourceId; // out: VidPnSoureId of primary allocation
    } VidPnFromAllocation;
  };
...
#define GET_OUTPUT_DUPL_DEBUG_INFO_FROM_SNAPSHOT(pSnapshot, VidPnSource,       \
  ...
  (pSnapshot->OutputDuplDebugInfos[(VidPnSource *                              \
                                    pSnapshot->NumOutputDuplContexts) +        \
                                   OutputDuplContextIndex])

typedef struct _D3DKMT_OUTPUTDUPL_SNAPSHOT {
  UINT Size; // _In_/out: Size of entire structure

  UINT SessionProcessCount; // _Out_: Number of processes currently duplicating
                            // output in this session (max possible will be
                            // equal to NumOutputDuplContexts)
  UINT SessionActiveConnectionsCount; // _Out_: Total number of active contexts
                                      // in this session, may be more than
                                      // number active in 2D array because that
                                      // is per adapter
...
typedef struct _D3DKMT_ACTIVATE_SPECIFIC_DIAG_ESCAPE {
  D3DKMT_ACTIVATE_SPECIFIC_DIAG_TYPE
  Type;          // The escape type that needs to be (de)activated
...
C_ASSERT(D3DKMT_QUERYSTATISTICS_QUEUE_PACKET_TYPE_MAX ==
...
typedef struct _D3DKMT_QUERYSTATISTICS_PACKET_INFORMATION {
  D3DKMT_QUERYSTATISTICS_QUEUE_PACKET_TYPE_INFORMATION QueuePacket
      [D3DKMT_QUERYSTATISTICS_QUEUE_PACKET_TYPE_MAX]; // Size =
                                                      // D3DKMT_QueuePacketTypeMax
  D3DKMT_QUERYSTATISTICS_DMA_PACKET_TYPE_INFORMATION DmaPacket
      [D3DKMT_QUERYSTATISTICS_DMA_PACKET_TYPE_MAX]; // Size =
...
typedef struct _D3DKMT_QUERYSTATISTICS_NODE_INFORMATION {
  D3DKMT_QUERYSTATISTICS_PROCESS_NODE_INFORMATION
  GlobalInformation; // Global statistics
  D3DKMT_QUERYSTATISTICS_PROCESS_NODE_INFORMATION
  SystemInformation; // Statistics for system thread
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_4)
...
typedef struct _D3DKMT_QUERYSTATSTICS_TERMINATIONS {
  //
  // We separate shared / nonshared because for nonshared we know that every
  // alloc terminated will lead cause a global alloc destroyed, but not for
  // nonshared.
  //
...
typedef struct _D3DKMT_QUERYSTATISTICS_POLICY {
  D3DKMT_ALIGN64 ULONGLONG PreferApertureForRead
...
// Process interference counters indicate how much this process GPU workload
// interferes with packets attempting to preempt it. 9 buckets will be exposed
// based on how long preemption took: [0] 100 microseconds <= preemption time <
// 250 microseconds [1] 250 microseconds <= preemption time < 500 microseconds
...
typedef struct _D3DKMT_QUERYSTATISTICS_PROCESS_INTERFERENCE_COUNTERS {
  D3DKMT_ALIGN64 UINT64 InterferenceCount
...
typedef struct _D3DKMT_QUERYSTATISTICS_SEGMENT_INFORMATION {
  ...
  //
  D3DKMT_ALIGN64 ULONGLONG TotalBytesEvictedByPriority
      [D3DKMT_QUERYSTATISTICS_ALLOCATION_PRIORITY_CLASS_MAX]; // Size =
                                                              // D3DKMT_MaxAllocationPriorityClass
...
typedef struct _D3DKMT_QUERYSTATISTICS_PROCESS_SEGMENT_GROUP_INFORMATION {
  ...
  D3DKMT_ALIGN64 UINT64
      Demoted[D3DKMT_QUERYSTATISTICS_ALLOCATION_PRIORITY_CLASS_MAX];
...
typedef union _D3DKMT_QUERYSTATISTICS_RESULT {
  D3DKMT_QUERYSTATISTICS_ADAPTER_INFORMATION
  AdapterInformation; // out: result of D3DKMT_QUERYSTATISTICS_ADAPTER(2)
                      // query
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_4)
  D3DKMT_QUERYSTATISTICS_PHYSICAL_ADAPTER_INFORMATION
  PhysAdapterInformation; // out: result of
                          // D3DKMT_QUERYSTATISTICS_PHYSICAL_ADAPTER query
#endif
  D3DKMT_QUERYSTATISTICS_SEGMENT_INFORMATION
  SegmentInformation; // out: result of D3DKMT_QUERYSTATISTICS_SEGMENT(2)
                      // query
  D3DKMT_QUERYSTATISTICS_NODE_INFORMATION
  NodeInformation; // out: result of D3DKMT_QUERYSTATISTICS_NODE query
  D3DKMT_QUERYSTATISTICS_VIDPNSOURCE_INFORMATION
  VidPnSourceInformation; // out: result of
                          // D3DKMT_QUERYSTATISTICS_VIDPNSOURCE query
  D3DKMT_QUERYSTATISTICS_PROCESS_INFORMATION
  ProcessInformation; // out: result of D3DKMT_QUERYSTATISTICS_PROCESS query
  D3DKMT_QUERYSTATISTICS_PROCESS_ADAPTER_INFORMATION
  ProcessAdapterInformation; // out: result of
                             // D3DKMT_QUERYSTATISTICS_PROCESS_ADAPTER(2)
                             // query
  D3DKMT_QUERYSTATISTICS_PROCESS_SEGMENT_INFORMATION
  ProcessSegmentInformation; // out: result of
                             // D3DKMT_QUERYSTATISTICS_PROCESS_SEGMENT(2)
                             // query
  D3DKMT_QUERYSTATISTICS_PROCESS_NODE_INFORMATION
  ProcessNodeInformation; // out: result of
                          // D3DKMT_QUERYSTATISTICS_PROCESS_NODE query
  D3DKMT_QUERYSTATISTICS_PROCESS_VIDPNSOURCE_INFORMATION
  ProcessVidPnSourceInformation; // out: result of
                                 // D3DKMT_QUERYSTATISTICS_PROCESS_VIDPNSOURCE
                                 // query
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_1)
  D3DKMT_QUERYSTATISTICS_PROCESS_SEGMENT_GROUP_INFORMATION
  ProcessSegmentGroupInformation; // out: result of
                                  // D3DKMT_QUERYSTATISTICS_PROCESS_SEGMENT_GROUP(2)
                                  // query
#endif
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_1)
  D3DKMT_QUERYSTATISTICS_MEMORY_USAGE
  SegmentUsageInformation; // out: result of
                           // D3DKMT_QUERYSTATISTICS_SEGMENT_USAGE query
  D3DKMT_QUERYSTATISTICS_MEMORY_USAGE
  SegmentGroupUsageInformation; // out: result of
...
typedef struct _D3DKMT_QUERYSTATISTICS {
  ...
  union {
    D3DKMT_QUERYSTATISTICS_QUERY_SEGMENT
    QuerySegment; // in: id of segment to get statistics for
    D3DKMT_QUERYSTATISTICS_QUERY_SEGMENT
    QueryProcessSegment; // in: id of segment to get statistics for
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_1)
    D3DKMT_MEMORY_SEGMENT_GROUP
    QueryProcessSegmentGroup; // in: id of segment group to get statistics
                              // for
#endif
    D3DKMT_QUERYSTATISTICS_QUERY_NODE
    QueryNode; // in: id of node to get statistics for
    D3DKMT_QUERYSTATISTICS_QUERY_NODE
    QueryProcessNode; // in: id of node to get statistics for
    D3DKMT_QUERYSTATISTICS_QUERY_VIDPNSOURCE
    QueryVidPnSource; // in: id of vidpnsource to get statistics for
    D3DKMT_QUERYSTATISTICS_QUERY_VIDPNSOURCE
    QueryProcessVidPnSource; // in: id of vidpnsource to get statistics for
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_4)
    D3DKMT_QUERYSTATISTICS_QUERY_PHYSICAL_ADAPTER
    QueryPhysAdapter; // in: id of physical adapter to get statistics for
#endif
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_1)
    ...
    D3DKMT_QUERYSTATISTICS_QUERY_NODE2
    QueryNode2; // in: id of node to get statistics for
    D3DKMT_QUERYSTATISTICS_QUERY_NODE2
    QueryProcessNode2; // in: id of node to get statistics for
#endif
  };
...
typedef enum _D3DKMT_VIDPNSOURCEOWNER_TYPE {
  ...
  D3DKMT_VIDPNSOURCEOWNER_SHARED =
      1, // Has shared owner, that is owner can yield to any exclusive owner,
         // not available to legacy devices
  D3DKMT_VIDPNSOURCEOWNER_EXCLUSIVE =
      2, // Has exclusive owner without shared gdi primary,
  D3DKMT_VIDPNSOURCEOWNER_EXCLUSIVEGDI =
      3, // Has exclusive owner with shared gdi primary and must be exclusive
         // owner of all VidPn sources, only available to legacy devices
  D3DKMT_VIDPNSOURCEOWNER_EMULATED =
      4, // Does not have real primary ownership, but allows the device to set
         // gamma on its owned sources
...
typedef struct _D3DKMT_SETVIDPNSOURCEOWNER {
  ...
  D3DKMT_PTR(CONST D3DKMT_VIDPNSOURCEOWNER_TYPE *,
             pType); // in: OwnerType array
  D3DKMT_PTR(CONST D3DDDI_VIDEO_PRESENT_SOURCE_ID *,
             pVidPnSourceId); // in: VidPn source ID array
...
typedef struct _D3DKMT_SETVIDPNSOURCEOWNER2 {
  ...
  D3DKMT_PTR(
      CONST D3DKMT_PTR_TYPE *,
      pVidPnSourceNtHandles); // in: VidPn source owner DispMgr NT handles
...
typedef struct _D3DKMT_GETPRESENTHISTORY {
  ...
  D3DKMT_PTR(_Field_size_bytes_(ProvidedSize) D3DKMT_PRESENTHISTORYTOKEN *,
             pTokens); // in: Pointer to buffer.
...
typedef struct _D3DKMT_WAITFORVERTICALBLANKEVENT2 {
  ...
  D3DDDI_VIDEO_PRESENT_SOURCE_ID VidPnSourceId; // in: adapter's VidPN Source ID
  UINT NumObjects;
  D3DKMT_PTR_TYPE ObjectHandleArray
      [D3DKMT_MAX_WAITFORVERTICALBLANK_OBJECTS]; // in: Specifies the objects to
                                                 // wait on.
...
typedef struct _D3DKMT_DEVICEPAGEFAULT_STATE {
  D3DKMT_ALIGN64 UINT64
      FaultedPrimitiveAPISequenceNumber; // when per draw fence write is
                                         // enabled, identifies the draw that
                                         // caused the page fault, or
                                         // DXGK_PRIMITIVE_API_SEQUENCE_NUMBER_UNKNOWN
                                         // if such information is not
                                         // available.
  DXGK_RENDER_PIPELINE_STAGE
  FaultedPipelineStage; // render pipeline stage during which the fault was
                        // generated, or DXGK_RENDER_PIPELINE_STAGE_UNKNOWN
                        // if such information is not available.
  UINT FaultedBindTableEntry; // a bind table index of a resource being accessed
                              // at the time of the fault, or
                              // DXGK_BIND_TABLE_ENTRY_UNKNOWN if such
                              // information is not available.
  DXGK_PAGE_FAULT_FLAGS
  PageFaultFlags; // flags specifying the nature of the fault
  DXGK_FAULT_ERROR_CODE FaultErrorCode; // Structure that contains error code
                                        // describing the fault.
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      FaultedVirtualAddress; // virtual address of faulting resource, or
                             // D3DGPU_NULL if such information is not
                             // available.
...
typedef struct _D3DKMT_DEVICEPRESENT_QUEUE_STATE {
  D3DDDI_VIDEO_PRESENT_SOURCE_ID VidPnSourceId; // in: present source id
  BOOLEAN bQueuedPresentLimitReached; // out: whether the queued present limit
                                      // has been reached
...
typedef struct _D3DKMT_GETDEVICESTATE {
  ...
  union {
    ...
    D3DKMT_DEVICEPRESENT_QUEUE_STATE
    PresentQueueState; // in/out: present queue state
...
typedef struct _D3DKMT_SETQUEUEDLIMIT {
  ...
  union {
    ...
    struct {
      D3DDDI_VIDEO_PRESENT_SOURCE_ID
      VidPnSourceId;               // in: adapter's VidPN source ID
...
typedef struct _D3DKMT_POLLDISPLAYCHILDREN {
  ...
  UINT DisableModeReset
      : 1; // in: 0x00000004 Disable DMM mode reset on monitor event
  UINT PollAllAdapters : 1; // in: 0x00000008 Poll all adapters
  UINT PollInterruptible
      : 1;            // in: 0x00000010 Poll interruptible targets as well.
...
typedef struct _D3DKMT_SETDISPLAYPRIVATEDRIVERFORMAT {
  D3DKMT_HANDLE hDevice; // in: Identifies the device
  D3DDDI_VIDEO_PRESENT_SOURCE_ID
  VidPnSourceId; // in: Identifies which VidPn we are changing the private
                 // driver format attribute for
  UINT PrivateDriverFormatAttribute; // In: Requested private format attribute
                                     // for VidPn specified
...
typedef struct _D3DKMT_ACQUIREKEYEDMUTEX {
  ...
  D3DKMT_ALIGN64 UINT64
      FenceValue; // out: Current fence value of the GPU sync object
...
typedef struct _D3DKMT_RELEASEKEYEDMUTEX {
  ...
  D3DKMT_ALIGN64 UINT64
      FenceValue; // in: New fence value to use for GPU sync object
...
typedef struct _D3DKMT_CREATEKEYEDMUTEX2_FLAGS {
  union {
    struct {
      UINT NtSecuritySharing
          : 1; // If set, the keyed mutex will be shared using DxgkShareObjects
               // instead of D3DKMT_CREATEKEYEDMUTEX2::hSharedHandle
      UINT Reserved : 31;
    };
    UINT Value;
  };
...
typedef struct _D3DKMT_CREATEKEYEDMUTEX2 {
  D3DKMT_ALIGN64 UINT64 InitialValue; // in:  Initial value to release to
  D3DKMT_HANDLE hSharedHandle; // out: Global handle to keyed mutex, NULL if
                               // NtSecuritySharing is set.
  D3DKMT_HANDLE hKeyedMutex;   // out: Handle to the keyed mutex in this process
  D3DKMT_PTR(
      _In_reads_bytes_opt_(PrivateRuntimeDataSize) VOID *,
      pPrivateRuntimeData); // in:  Buffer containing initial private data.
                            //      If NULL then PrivateRuntimeDataSize must be
                            //      0.
...
typedef struct _D3DKMT_OPENKEYEDMUTEX2 {
  ...
  D3DKMT_PTR(
      ...
      pPrivateRuntimeData); // in:  Buffer containing initial private data.
                            //      If NULL then PrivateRuntimeDataSize must be
                            //      0. It will only be copied if the keyed mutex
                            //      does not already have private data.
  UINT PrivateRuntimeDataSize; // in:  Size in bytes of pPrivateRuntimeData.
...
typedef struct _D3DKMT_ACQUIREKEYEDMUTEX2 {
  ...
  D3DKMT_ALIGN64 UINT64
      FenceValue; // out: Current fence value of the GPU sync object
  D3DKMT_PTR(_Out_writes_bytes_all_opt_(PrivateRuntimeDataSize) VOID *,
             pPrivateRuntimeData); // out: Buffer to copy private data to.
                                   //      If NULL then PrivateRuntimeDataSize
                                   //      must be 0.
  UINT PrivateRuntimeDataSize;     // in:  Size in bytes of pPrivateRuntimeData.
...
typedef struct _D3DKMT_RELEASEKEYEDMUTEX2 {
  ...
  D3DKMT_PTR(_In_reads_bytes_opt_(PrivateRuntimeDataSize) VOID *,
             pPrivateRuntimeData); // in: Buffer containing new private data.
                                   //     If NULL then PrivateRuntimeDataSize
                                   //     must be 0.
  UINT PrivateRuntimeDataSize;     // in: Size in bytes of pPrivateRuntimeData.
...
typedef enum _D3DKMT_OFFER_PRIORITY {
  ...
  D3DKMT_OFFER_PRIORITY_AUTO, // Let VidMm decide offer priority based on
                              // eviction priority
...
typedef struct _D3DKMT_OFFERALLOCATIONS {
  ...
  D3DKMT_PTR(D3DKMT_HANDLE *,
             pResources); // in: array of D3D runtime resource handles.
  D3DKMT_PTR(CONST D3DKMT_HANDLE *,
             HandleList); // in: array of allocation handles. If non-NULL,
                          // pResources must be NULL.
  UINT NumAllocations;    // in: number of items in whichever of pResources or
                          // HandleList is non-NULL.
  D3DKMT_OFFER_PRIORITY
  Priority; // in: priority with which to offer the allocations
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_0)
...
typedef struct _D3DKMT_RECLAIMALLOCATIONS {
  ...
  D3DKMT_PTR(CONST D3DKMT_HANDLE *,
             HandleList); // in: array of allocation handles. If non-NULL,
                          // pResources must be NULL.
  D3DKMT_PTR(BOOL *,
             pDiscarded); // out: optional array of booleans specifying whether
                          // each resource or allocation was discarded.
  UINT NumAllocations;    // in:  number of items in pDiscarded and whichever of
...
typedef struct _D3DKMT_RECLAIMALLOCATIONS2 {
  ...
  union {
    BOOL *pDiscarded; // out: optional array of booleans specifying whether each
                      // resource or allocation was discarded.
    D3DDDI_RECLAIM_RESULT
    *pResults; // out: array of results specifying whether each resource or
               // allocation is OK, discarded, or has no commitment.
    D3DKMT_PTR_HELPER(AlignUnionTo64)
  };
#else
  D3DKMT_PTR(BOOL *,
  ...
  D3DKMT_ALIGN64 UINT64
      PagingFenceValue; // out: The paging fence to synchronize against before
                        // submitting work to the GPU which
                        //      references any of the resources or allocations
...
typedef struct _D3DKMT_OUTPUTDUPL_METADATA {
  ...
  D3DKMT_PTR(_Field_size_bytes_part_(BufferSizeSupplied, BufferSizeRequired)
                 PVOID,
             pBuffer);
...
typedef struct _D3DKMT_HYBRID_LIST {
  ...
  LUID AdapterLuid; // in,opt: Adapter luid to per-adapter DList state. Optional
                    // if QueryType == D3DKMT_GPU_PREFERENCE_TYPE_IHV_DLIST
  BOOL bUserPreferenceQuery; // Whether referring to user gpu preference, or
                             // per-adapter DList query
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_6)
  D3DKMT_GPU_PREFERENCE_QUERY_TYPE
  QueryType; // Replaced bUserPreferenceQuery, for referring to which
             // D3DKMT_GPU_PREFERENCE_QUERY_TYPE
#endif
} D3DKMT_HYBRID_LIST;
...
typedef struct _D3DKMT_WAITFORSYNCHRONIZATIONOBJECTFROMCPU {
  ...
  D3DKMT_PTR(_Field_size_(ObjectCount) const D3DKMT_HANDLE *,
             ObjectHandleArray); // in: Handle to monitored fence
                                 // synchronization objects to wait on.

  D3DKMT_PTR(_Field_size_(ObjectCount) const UINT64 *,
             FenceValueArray); // in: Fence values to be waited on.

  D3DKMT_PTR(HANDLE,
             hAsyncEvent); // in: Event to be signaled when the wait condition
                           // is satisfied. When set to NULL, the call will not
                           // return until the wait condition is satisfied.

  D3DDDI_WAITFORSYNCHRONIZATIONOBJECTFROMCPU_FLAGS
  Flags; // in: Flags that specify the wait mode.
...
typedef struct _D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMCPU {
  ...
  D3DKMT_PTR(_Field_size_(ObjectCount) const UINT64 *,
             FenceValueArray); // in: Fence values to be signaled.
...
typedef struct _D3DKMT_WAITFORSYNCHRONIZATIONOBJECTFROMGPU {
  ...
  D3DKMT_PTR(
      _Field_size_(ObjectCount) const D3DKMT_HANDLE *,
      ObjectHandleArray); // in: Handles to synchronization objects to wait on.

  union {
    _Field_size_(ObjectCount) const UINT64
        *MonitoredFenceValueArray; // in: monitored fence values to be waited.
...
typedef struct _D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU {
  D3DKMT_HANDLE hContext; // in: Identifies the context that the signal is being
                          // submitted to.
  UINT ObjectCount;       // in: Specifies the number of objects to signal.

  D3DKMT_PTR(_Field_size_(ObjectCount) const D3DKMT_HANDLE *,
             ObjectHandleArray); // in: Specifies the objects to signal.

  union {
    _Field_size_(ObjectCount) const UINT64
        *MonitoredFenceValueArray; // in: monitored fence values to be signaled
...
typedef struct _D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU2 {
  ...
  ULONG BroadcastContextCount; // in: Specifies the number of contexts to
                               // broadcast this signal to.

  D3DKMT_PTR(
      _Field_size_(BroadcastContextCount) const D3DKMT_HANDLE *,
      BroadcastContextArray); // in: Specifies context handles to broadcast to.

  union {
    D3DKMT_ALIGN64 UINT64 FenceValue; // in: fence value to be signaled;

    HANDLE CpuEventHandle; // in: handle of a CPU event to be signaled if
                           // Flags.EnqueueCpuEvent flag is set.
...
typedef struct _D3DKMT_CREATEPAGINGQUEUE {
  D3DKMT_HANDLE hDevice;                // in: Handle to the device.
  D3DDDI_PAGINGQUEUE_PRIORITY Priority; // in: scheduling priority relative to
                                        // other paging queues on this device
  D3DKMT_HANDLE hPagingQueue; // out: handle to the paging queue used to
                              // synchronize paging operations for this device.
  D3DKMT_HANDLE
  hSyncObject; // out: handle to the monitored fence object used to
               // synchronize paging operations for this paging queue.
  D3DKMT_PTR(VOID *,
             FenceValueCPUVirtualAddress); // out: Read-only mapping of the
                                           // fence value for the CPU
...
typedef struct _D3DKMT_EVICT {
  ...
  D3DKMT_PTR(
      CONST D3DKMT_HANDLE *,
      AllocationList);      // in: an array of NumAllocations allocation handles
  D3DDDI_EVICT_FLAGS Flags; // in: eviction flags
  D3DKMT_ALIGN64 UINT64
      NumBytesToTrim; // out: This value indicates how much to trim in order to
...
typedef struct _D3DKMT_FREEGPUVIRTUALADDRESS {
  ...
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      BaseAddress; // in: Start of a virtual address range in bytes
  D3DKMT_ALIGN64 D3DGPU_SIZE_T
      Size; // in: Size of the virtual address range in bytes
...
typedef struct _D3DKMT_CREATECONTEXTVIRTUAL {
  ...
  UINT PrivateDriverDataSize;             // in:
  D3DKMT_CLIENTHINT
  ClientHint;             // in:  Hints which client is creating the context
...
typedef struct _D3DKMT_SUBMITCOMMANDTOHWQUEUE {
  ...
  D3DKMT_ALIGN64 UINT64
      HwQueueProgressFenceId; // in: Hardware queue progress fence value that
                              // will be signaled once the command is finished.

  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      CommandBuffer;  // in: GPU VA of the command buffer to be executed on the
                      // GPU.
  ...
  D3DKMT_PTR(_Field_size_bytes_(PrivateDriverDataSize) VOID *,
             pPrivateDriverData); // in: Pointer to the private driver data.

  UINT NumPrimaries; // in: The number of primaries written by this command
                     // buffer.

  D3DKMT_PTR(_Field_size_(NumPrimaries) D3DKMT_HANDLE CONST *,
             WrittenPrimaries); // in: The array of primaries written by this
                                // command buffer.
...
typedef struct _D3DKMT_SUBMITWAITFORSYNCOBJECTSTOHWQUEUE {
  ...
  D3DKMT_PTR(_Field_size_(ObjectCount) const D3DKMT_HANDLE *,
             ObjectHandleArray); // in: Handles to monitored fence
                                 // synchronization objects to wait on.

  D3DKMT_PTR(_Field_size_(ObjectCount) const UINT64 *,
             FenceValueArray); // in: monitored fence values to be waited.
} D3DKMT_SUBMITWAITFORSYNCOBJECTSTOHWQUEUE;

typedef struct _D3DKMT_SUBMITSIGNALSYNCOBJECTSTOHWQUEUE {
  D3DDDICB_SIGNALFLAGS Flags; // in: Specifies signal behavior.

  ULONG BroadcastHwQueueCount; // in: Specifies the number of hardware queues to
                               // broadcast this signal to.

  D3DKMT_PTR(_Field_size_(BroadcastHwQueueCount) const D3DKMT_HANDLE *,
             BroadcastHwQueueArray); // in: Specifies hardware queue handles to
                                     // broadcast to.
  ...
  D3DKMT_PTR(_Field_size_(ObjectCount) const UINT64 *,
             FenceValueArray); // in: monitored fence values to signal.
...
typedef struct _D3DKMT_QUERYVIDEOMEMORYINFO {
  D3DKMT_PTR(HANDLE,
             hProcess);   // in,opt: A handle to a process. If NULL, the current
                          // process is used.
                          //         The process handle must be opened with
                          //         PROCESS_QUERY_INFORMATION privileges
  ...
  D3DKMT_ALIGN64 UINT64
      CurrentReservation; // out: Current reservation of the device
  D3DKMT_ALIGN64 UINT64
      AvailableForReservation; // out: Total that the device may reserve
  UINT PhysicalAdapterIndex;   // in : Zero based physical adapter index in the
                               // LDA configuration.
...
typedef struct _D3DKMT_CHANGEVIDEOMMEMORYRESERVATION {
  D3DKMT_PTR(HANDLE,
             hProcess);   // in,opt: A handle to a process. If NULL, the current
                          // process is used.
                          //         The process handle must be opened with
                          //         PROCESS_SET_INFORMATION privileges
  D3DKMT_HANDLE hAdapter; // in : The adapter to change reservation for.
  D3DKMT_MEMORY_SEGMENT_GROUP
  MemorySegmentGroup; // in : The memory segment group to change reservation
                      // for.
  D3DKMT_ALIGN64 UINT64
      Reservation; // in : Desired reservation in the range between 0 and
                   // AvailableForReservation returned by QueryVideoMemoryInfo.
  UINT PhysicalAdapterIndex; // in : Zero based physical adapter index in the
                             // LDA configuration.
...
typedef struct _D3DKMT_SETSTABLEPOWERSTATE {
  D3DKMT_HANDLE
  hAdapter;     // in: The adapter to enable or disable stable power for
...
// A sync_file is a wrapper around the given monitored fence and the fence
// value. When a sync_file is created, a wait for sync object on CPU is issued
// and a file descriptor (FD) it returned to the app. The app can wait on the
// FD, which will be unblocked when the sync object with this fence value is
// signaled.
//
typedef struct _D3DKMT_CREATESYNCFILE {
  ...
  D3DKMT_ALIGN64 UINT64 hSyncFile;  // out: File descriptor on Android or a NT
                                    // handle on Windows (when implemented)
} D3DKMT_CREATESYNCFILE;

typedef struct _D3DKMT_WAITSYNCFILE {
  D3DKMT_ALIGN64 UINT64 hSyncFile; // in:  Sync file descriptor on Android or a
                                   // NT handle on Windows (when implemented)
...
typedef struct _D3DKMT_OPENSYNCOBJECTFROMSYNCFILE {
  ...
  D3DKMT_ALIGN64 UINT64
      FenceValue; // out: Fence value, associated with the sync file
  D3DKMT_PTR(VOID *,
             FenceValueCPUVirtualAddress); // out: Read-only mapping of the
                                           // fence value for the CPU
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      FenceValueGPUVirtualAddress; // out: Read/write mapping of the fence value
                                   // for the GPU
...
typedef struct _D3DKMT_TRIMNOTIFICATION {
  ...
  D3DKMT_ALIGN64 UINT64
      NumBytesToTrim; // In: When TrimToBudget flag is set, this value indicates
                      // how much VidMm requests the app to trim to fit in the
                      // new budget.
} D3DKMT_TRIMNOTIFICATION;

typedef VOID(APIENTRY *PFND3DKMT_TRIMNOTIFICATIONCALLBACK)(
...
typedef struct _D3DKMT_UNREGISTERTRIMNOTIFICATION {
  D3DKMT_PTR(VOID *,
             Handle); // In: Handle returned from RegisterTrimNotification,
                      // or NULL to unregister all Callback instances.
  D3DKMT_PTR(
      PFND3DKMT_TRIMNOTIFICATIONCALLBACK,
      Callback); // In: When Handle is NULL, this parameter specifies that all
                 // registered instances of Callback should be unregistered.
                 // This unregistration method should only be used in DLL unload
                 // scenarios when the DLL being unloaded cannot guarantee that
...
typedef VOID(APIENTRY *PFND3DKMT_BUDGETCHANGENOTIFICATIONCALLBACK)(
...
typedef struct _D3DKMT_CREATEHWCONTEXT {
  ...
  UINT PrivateDriverDataSize;        // in:  Size of private driver data
  D3DKMT_PTR(_Inout_ _Field_size_bytes_(PrivateDriverDataSize) VOID *,
             pPrivateDriverData); // in/out:  Private driver data
...
typedef struct _D3DKMT_CREATEHWQUEUE {
  D3DKMT_HANDLE hHwContext; // in:  Handle to the hardware context the queue is
                            // associated with.
  ...
  D3DKMT_PTR(_Inout_ _Field_size_bytes_(PrivateDriverDataSize) VOID *,
             pPrivateDriverData); // in/out:  Private driver data
  D3DKMT_HANDLE
  hHwQueue; // out: handle to the hardware queue object to submit work to.
  D3DKMT_HANDLE
  hHwQueueProgressFence; // out: handle to the monitored fence object used
                         // to monitor the queue progress.
  D3DKMT_PTR(VOID *,
             HwQueueProgressFenceCPUVirtualAddress); // out: Read-only mapping
                                                     // of the queue progress
                                                     // fence value for the CPU
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      HwQueueProgressFenceGPUVirtualAddress; // out: Read/write mapping of the
                                             // queue progress fence value for
                                             // the GPU
...
typedef struct _D3DKMT_CREATEPROTECTEDSESSION {
  D3DKMT_HANDLE hDevice;     // in: device handle
  D3DKMT_HANDLE hSyncObject; // in: monitored fence handle associated to this
                             // session (kernel handle)
  D3DKMT_PTR(_Field_size_bytes_(PrivateDriverDataSize) CONST VOID *,
             pPrivateDriverData); // in: Private driver data
  UINT PrivateDriverDataSize;     // in: Size of private runtime data
  D3DKMT_PTR(_Field_size_bytes_(PrivateRuntimeDataSize) CONST VOID *,
             pPrivateRuntimeData); // in: Private runtime data
...
typedef struct _D3DKMT_GETPROCESSDEVICEREMOVALSUPPORT {
  ...
  BOOLEAN Support;  // out: Whether or not the process using the adapter can
                    // recover from graphics device removal
...
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEALLOCATION)(
    _Inout_ D3DKMT_CREATEALLOCATION *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEALLOCATION2)(
    _Inout_ D3DKMT_CREATEALLOCATION *); // _ADVSCH_
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYRESOURCEINFO)(
    _Inout_ D3DKMT_QUERYRESOURCEINFO *);
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_QUERYRESOURCEINFOFROMNTHANDLE)(
...
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SHAREOBJECTS)(
    ...
    _In_ POBJECT_ATTRIBUTES pObjectAttributes, _In_ DWORD dwDesiredAccess,
    _Out_ HANDLE *phSharedNtHandle);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENNTHANDLEFROMNAME)(
    _Inout_ D3DKMT_OPENNTHANDLEFROMNAME *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENRESOURCEFROMNTHANDLE)(
    _Inout_ D3DKMT_OPENRESOURCEFROMNTHANDLE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENSYNCOBJECTFROMNTHANDLE)(
    _Inout_ D3DKMT_OPENSYNCOBJECTFROMNTHANDLE *);
#endif
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENRESOURCE)(_Inout_ D3DKMT_OPENRESOURCE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENRESOURCE2)(
    _Inout_ D3DKMT_OPENRESOURCE *); // _ADVSCH_
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYALLOCATION)(
    _In_ CONST D3DKMT_DESTROYALLOCATION *);

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_0)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYALLOCATION2)(
    _In_ CONST D3DKMT_DESTROYALLOCATION2 *);
#endif // DXGKDDI_INTERFACE_VERSION_WDDM2_0

typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETALLOCATIONPRIORITY)(
    _In_ CONST D3DKMT_SETALLOCATIONPRIORITY *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYALLOCATIONRESIDENCY)(
    _In_ CONST D3DKMT_QUERYALLOCATIONRESIDENCY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATEDEVICE)(_Inout_ D3DKMT_CREATEDEVICE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_DESTROYDEVICE)(_In_ CONST D3DKMT_DESTROYDEVICE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATECONTEXT)(_Inout_ D3DKMT_CREATECONTEXT *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYCONTEXT)(
    _In_ CONST D3DKMT_DESTROYCONTEXT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATESYNCHRONIZATIONOBJECT)(
    _Inout_ D3DKMT_CREATESYNCHRONIZATIONOBJECT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATESYNCHRONIZATIONOBJECT2)(
    _Inout_ D3DKMT_CREATESYNCHRONIZATIONOBJECT2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENSYNCHRONIZATIONOBJECT)(
    _Inout_ D3DKMT_OPENSYNCHRONIZATIONOBJECT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_DESTROYSYNCHRONIZATIONOBJECT)(
    _In_ CONST D3DKMT_DESTROYSYNCHRONIZATIONOBJECT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_WAITFORSYNCHRONIZATIONOBJECT)(
    _In_ CONST D3DKMT_WAITFORSYNCHRONIZATIONOBJECT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_WAITFORSYNCHRONIZATIONOBJECT2)(
    _In_ CONST D3DKMT_WAITFORSYNCHRONIZATIONOBJECT2 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SIGNALSYNCHRONIZATIONOBJECT)(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SIGNALSYNCHRONIZATIONOBJECT2)(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECT2 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_LOCK)(_Inout_ D3DKMT_LOCK *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_UNLOCK)(_In_ CONST D3DKMT_UNLOCK *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETDISPLAYMODELIST)(
    _Inout_ D3DKMT_GETDISPLAYMODELIST *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETDISPLAYMODE)(
    _Inout_ CONST D3DKMT_SETDISPLAYMODE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETMULTISAMPLEMETHODLIST)(
    _Inout_ D3DKMT_GETMULTISAMPLEMETHODLIST *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_PRESENT)(_Inout_ D3DKMT_PRESENT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_RENDER)(_Inout_ D3DKMT_RENDER *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETRUNTIMEDATA)(
    _Inout_ CONST D3DKMT_GETRUNTIMEDATA *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYADAPTERINFO)(
    _Inout_ CONST D3DKMT_QUERYADAPTERINFO *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENADAPTERFROMHDC)(
    _Inout_ D3DKMT_OPENADAPTERFROMHDC *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENADAPTERFROMGDIDISPLAYNAME)(
    _Inout_ D3DKMT_OPENADAPTERFROMGDIDISPLAYNAME *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENADAPTERFROMDEVICENAME)(
    _Inout_ D3DKMT_OPENADAPTERFROMDEVICENAME *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CLOSEADAPTER)(_In_ CONST D3DKMT_CLOSEADAPTER *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETSHAREDPRIMARYHANDLE)(
    _Inout_ D3DKMT_GETSHAREDPRIMARYHANDLE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_ESCAPE)(_In_ CONST D3DKMT_ESCAPE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYSTATISTICS)(
    _In_ CONST D3DKMT_QUERYSTATISTICS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETVIDPNSOURCEOWNER)(
    _In_ CONST D3DKMT_SETVIDPNSOURCEOWNER *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETPRESENTHISTORY)(
    _Inout_ D3DKMT_GETPRESENTHISTORY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATEOVERLAY)(_Inout_ D3DKMT_CREATEOVERLAY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_UPDATEOVERLAY)(_In_ CONST D3DKMT_UPDATEOVERLAY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_FLIPOVERLAY)(_In_ CONST D3DKMT_FLIPOVERLAY *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYOVERLAY)(
    _In_ CONST D3DKMT_DESTROYOVERLAY *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_WAITFORVERTICALBLANKEVENT)(
    _In_ CONST D3DKMT_WAITFORVERTICALBLANKEVENT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETGAMMARAMP)(_In_ CONST D3DKMT_SETGAMMARAMP *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETDEVICESTATE)(_Inout_ D3DKMT_GETDEVICESTATE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEDCFROMMEMORY)(
    _Inout_ D3DKMT_CREATEDCFROMMEMORY *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYDCFROMMEMORY)(
    _In_ CONST D3DKMT_DESTROYDCFROMMEMORY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETCONTEXTSCHEDULINGPRIORITY)(
    _In_ CONST D3DKMT_SETCONTEXTSCHEDULINGPRIORITY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETCONTEXTSCHEDULINGPRIORITY)(
    _Inout_ D3DKMT_GETCONTEXTSCHEDULINGPRIORITY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETPROCESSSCHEDULINGPRIORITYCLASS)(
    _In_ HANDLE, _In_ D3DKMT_SCHEDULINGPRIORITYCLASS);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETPROCESSSCHEDULINGPRIORITYCLASS)(
    _In_ HANDLE, _Out_ D3DKMT_SCHEDULINGPRIORITYCLASS *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_RELEASEPROCESSVIDPNSOURCEOWNERS)(_In_ HANDLE);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETSCANLINE)(_Inout_ D3DKMT_GETSCANLINE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CHANGESURFACEPOINTER)(
    _In_ CONST D3DKMT_CHANGESURFACEPOINTER *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETQUEUEDLIMIT)(
    _In_ CONST D3DKMT_SETQUEUEDLIMIT *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_POLLDISPLAYCHILDREN)(
    _In_ CONST D3DKMT_POLLDISPLAYCHILDREN *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_INVALIDATEACTIVEVIDPN)(
    _In_ CONST D3DKMT_INVALIDATEACTIVEVIDPN *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CHECKOCCLUSION)(
    _In_ CONST D3DKMT_CHECKOCCLUSION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_WAITFORIDLE)(_In_ CONST D3DKMT_WAITFORIDLE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CHECKMONITORPOWERSTATE)(
    _In_ CONST D3DKMT_CHECKMONITORPOWERSTATE *);
typedef _Check_return_ BOOLEAN(APIENTRY *PFND3DKMT_CHECKEXCLUSIVEOWNERSHIP)();
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CHECKVIDPNEXCLUSIVEOWNERSHIP)(
    _In_ CONST D3DKMT_CHECKVIDPNEXCLUSIVEOWNERSHIP *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETDISPLAYPRIVATEDRIVERFORMAT)(
    _In_ CONST D3DKMT_SETDISPLAYPRIVATEDRIVERFORMAT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SHAREDPRIMARYLOCKNOTIFICATION)(
    _In_ CONST D3DKMT_SHAREDPRIMARYLOCKNOTIFICATION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SHAREDPRIMARYUNLOCKNOTIFICATION)(
    _In_ CONST D3DKMT_SHAREDPRIMARYUNLOCKNOTIFICATION *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEKEYEDMUTEX)(
    _Inout_ D3DKMT_CREATEKEYEDMUTEX *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENKEYEDMUTEX)(_Inout_ D3DKMT_OPENKEYEDMUTEX *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYKEYEDMUTEX)(
    _In_ CONST D3DKMT_DESTROYKEYEDMUTEX *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_ACQUIREKEYEDMUTEX)(
    _Inout_ D3DKMT_ACQUIREKEYEDMUTEX *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_RELEASEKEYEDMUTEX)(
    _Inout_ D3DKMT_RELEASEKEYEDMUTEX *);
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEKEYEDMUTEX2)(
    _Inout_ D3DKMT_CREATEKEYEDMUTEX2 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENKEYEDMUTEX2)(_Inout_ D3DKMT_OPENKEYEDMUTEX2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_ACQUIREKEYEDMUTEX2)(
    _Inout_ D3DKMT_ACQUIREKEYEDMUTEX2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_RELEASEKEYEDMUTEX2)(
    _Inout_ D3DKMT_RELEASEKEYEDMUTEX2 *);
#endif
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CONFIGURESHAREDRESOURCE)(
    _In_ CONST D3DKMT_CONFIGURESHAREDRESOURCE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETOVERLAYSTATE)(_Inout_ D3DKMT_GETOVERLAYSTATE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CHECKSHAREDRESOURCEACCESS)(
    _In_ CONST D3DKMT_CHECKSHAREDRESOURCEACCESS *);
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OFFERALLOCATIONS)(
    _In_ CONST D3DKMT_OFFERALLOCATIONS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_RECLAIMALLOCATIONS)(
    _Inout_ CONST D3DKMT_RECLAIMALLOCATIONS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEOUTPUTDUPL)(
    _In_ CONST D3DKMT_CREATE_OUTPUTDUPL *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYOUTPUTDUPL)(
    _In_ CONST D3DKMT_DESTROY_OUTPUTDUPL *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OUTPUTDUPLGETFRAMEINFO)(
...
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OUTPUTDUPLGETPOINTERSHAPEDATA)(
    _Inout_ D3DKMT_OUTPUTDUPL_GET_POINTER_SHAPE_DATA *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OUTPUTDUPLRELEASEFRAME)(
    _In_ D3DKMT_OUTPUTDUPL_RELEASE_FRAME *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OUTPUTDUPLPRESENT)(
    _In_ CONST D3DKMT_OUTPUTDUPLPRESENT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_ENUMADAPTERS)(_Inout_ CONST D3DKMT_ENUMADAPTERS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_ENUMADAPTERS2)(
    _Inout_ CONST D3DKMT_ENUMADAPTERS2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENADAPTERFROMLUID)(
    _Inout_ D3DKMT_OPENADAPTERFROMLUID *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_QUERYREMOTEVIDPNSOURCEFROMGDIDISPLAYNAME)(
    _Inout_ D3DKMT_QUERYREMOTEVIDPNSOURCEFROMGDIDISPLAYNAME *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETVIDPNSOURCEOWNER1)(
    _In_ CONST D3DKMT_SETVIDPNSOURCEOWNER1 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_PINDIRECTFLIPRESOURCES)(
    _In_ CONST D3DKMT_PINDIRECTFLIPRESOURCES *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_UNPINDIRECTFLIPRESOURCES)(
    _In_ CONST D3DKMT_UNPINDIRECTFLIPRESOURCES *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_WAITFORVERTICALBLANKEVENT2)(
    _In_ CONST D3DKMT_WAITFORVERTICALBLANKEVENT2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETDWMVERTICALBLANKEVENT)(
    _In_ CONST D3DKMT_GETVERTICALBLANKEVENT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETSYNCREFRESHCOUNTWAITTARGET)(
    _In_ CONST D3DKMT_SETSYNCREFRESHCOUNTWAITTARGET *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETCONTEXTINPROCESSSCHEDULINGPRIORITY)(
    _In_ CONST D3DKMT_SETCONTEXTINPROCESSSCHEDULINGPRIORITY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETCONTEXTINPROCESSSCHEDULINGPRIORITY)(
    _Inout_ D3DKMT_GETCONTEXTINPROCESSSCHEDULINGPRIORITY *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_PRESENTMULTIPLANEOVERLAY)(
    _In_ CONST D3DKMT_PRESENT_MULTIPLANE_OVERLAY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETSHAREDRESOURCEADAPTERLUID)(
    _Inout_ D3DKMT_GETSHAREDRESOURCEADAPTERLUID *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETSTEREOENABLED)(_In_ BOOL);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYHYBRIDLISTVALUE)(
    _Inout_ D3DKMT_HYBRID_LIST *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETHYBRIDLISTVVALUE)(_Inout_ D3DKMT_HYBRID_LIST *);
#endif
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM1_3)
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT)(
    _Inout_ D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_0)
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_MAKERESIDENT)(_Inout_ D3DDDI_MAKERESIDENT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_EVICT)(_Inout_ D3DKMT_EVICT *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_WAITFORSYNCHRONIZATIONOBJECTFROMCPU)(
    _In_ CONST D3DKMT_WAITFORSYNCHRONIZATIONOBJECTFROMCPU *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMCPU)(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMCPU *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_WAITFORSYNCHRONIZATIONOBJECTFROMGPU)(
    _In_ CONST D3DKMT_WAITFORSYNCHRONIZATIONOBJECTFROMGPU *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU)(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU2)(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEPAGINGQUEUE)(
    _Inout_ D3DKMT_CREATEPAGINGQUEUE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYPAGINGQUEUE)(
    _Inout_ D3DDDI_DESTROYPAGINGQUEUE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_LOCK2)(_Inout_ D3DKMT_LOCK2 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_UNLOCK2)(_In_ CONST D3DKMT_UNLOCK2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_INVALIDATECACHE)(
    _In_ const D3DKMT_INVALIDATECACHE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_RESERVEGPUVIRTUALADDRESS)(
    _Inout_ D3DDDI_RESERVEGPUVIRTUALADDRESS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_MAPGPUVIRTUALADDRESS)(
    _Inout_ D3DDDI_MAPGPUVIRTUALADDRESS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_FREEGPUVIRTUALADDRESS)(
    _In_ CONST D3DKMT_FREEGPUVIRTUALADDRESS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_UPDATEGPUVIRTUALADDRESS)(
    _In_ CONST D3DKMT_UPDATEGPUVIRTUALADDRESS *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETRESOURCEPRESENTPRIVATEDRIVERDATA)(
    _Inout_ D3DDDI_GETRESOURCEPRESENTPRIVATEDRIVERDATA *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATECONTEXTVIRTUAL)(
    _Inout_ D3DKMT_CREATECONTEXTVIRTUAL *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SUBMITCOMMAND)(_In_ CONST D3DKMT_SUBMITCOMMAND *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENSYNCOBJECTFROMNTHANDLE2)(
    _Inout_ D3DKMT_OPENSYNCOBJECTFROMNTHANDLE2 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENSYNCOBJECTNTHANDLEFROMNAME)(
    _Inout_ D3DKMT_OPENSYNCOBJECTNTHANDLEFROMNAME *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYVIDEOMEMORYINFO)(
    _Inout_ D3DKMT_QUERYVIDEOMEMORYINFO *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CHANGEVIDEOMEMORYRESERVATION)(
    _In_ CONST D3DKMT_CHANGEVIDEOMEMORYRESERVATION *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_REGISTERTRIMNOTIFICATION)(
    _Inout_ D3DKMT_REGISTERTRIMNOTIFICATION *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_UNREGISTERTRIMNOTIFICATION)(
    _Inout_ D3DKMT_UNREGISTERTRIMNOTIFICATION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_REGISTERBUDGETCHANGENOTIFICATION)(
    _Inout_ D3DKMT_REGISTERBUDGETCHANGENOTIFICATION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_UNREGISTERBUDGETCHANGENOTIFICATION)(
    _Inout_ D3DKMT_UNREGISTERBUDGETCHANGENOTIFICATION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT2)(
    _Inout_ D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_PRESENTMULTIPLANEOVERLAY2)(
    _In_ CONST D3DKMT_PRESENT_MULTIPLANE_OVERLAY2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_RECLAIMALLOCATIONS2)(
    _Inout_ D3DKMT_RECLAIMALLOCATIONS2 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETSTABLEPOWERSTATE)(
    _In_ CONST D3DKMT_SETSTABLEPOWERSTATE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYCLOCKCALIBRATION)(
    _Inout_ D3DKMT_QUERYCLOCKCALIBRATION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_QUERYVIDPNEXCLUSIVEOWNERSHIP)(
    _Inout_ D3DKMT_QUERYVIDPNEXCLUSIVEOWNERSHIP *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_ADJUSTFULLSCREENGAMMA)(
    _In_ D3DKMT_ADJUSTFULLSCREENGAMMA *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETVIDPNSOURCEHWPROTECTION)(
    _In_ D3DKMT_SETVIDPNSOURCEHWPROTECTION *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_MARKDEVICEASERROR)(
    _In_ D3DKMT_MARKDEVICEASERROR *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_FLUSHHEAPTRANSITIONS)(
    _In_ D3DKMT_FLUSHHEAPTRANSITIONS *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETHWPROTECTIONTEARDOWNRECOVERY)(
    _In_ D3DKMT_SETHWPROTECTIONTEARDOWNRECOVERY *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_1)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_QUERYPROCESSOFFERINFO)(
    _Inout_ D3DKMT_QUERYPROCESSOFFERINFO *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_TRIMPROCESSCOMMITMENT)(
    _Inout_ D3DKMT_TRIMPROCESSCOMMITMENT *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_UPDATEALLOCATIONPROPERTY)(
    _Inout_ D3DDDI_UPDATEALLOCPROPERTY *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT3)(
    _Inout_ D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT3 *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_PRESENTMULTIPLANEOVERLAY3)(
    _In_ CONST D3DKMT_PRESENT_MULTIPLANE_OVERLAY3 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SETFSEBLOCK)(_In_ CONST D3DKMT_SETFSEBLOCK *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_QUERYFSEBLOCK)(_Inout_ D3DKMT_QUERYFSEBLOCK *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_2)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETALLOCATIONPRIORITY)(
    _In_ CONST D3DKMT_GETALLOCATIONPRIORITY *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETMULTIPLANEOVERLAYCAPS)(
    _Inout_ D3DKMT_GET_MULTIPLANE_OVERLAY_CAPS *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETPOSTCOMPOSITIONCAPS)(
    _Inout_ D3DKMT_GET_POST_COMPOSITION_CAPS *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_3)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SETVIDPNSOURCEOWNER2)(
    _In_ CONST D3DKMT_SETVIDPNSOURCEOWNER2 *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_GETPROCESSDEVICEREMOVALSUPPORT)(
    _Inout_ D3DKMT_GETPROCESSDEVICEREMOVALSUPPORT *);

typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATEPROTECTEDSESSION)(
    _Inout_ D3DKMT_CREATEPROTECTEDSESSION *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYPROTECTEDSESSION)(
    _Inout_ D3DKMT_DESTROYPROTECTEDSESSION *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_QUERYPROTECTEDSESSIONSTATUS)(
    _Inout_ D3DKMT_QUERYPROTECTEDSESSIONSTATUS *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_QUERYPROTECTEDSESSIONINFOFROMNTHANDLE)(
    _Inout_ D3DKMT_QUERYPROTECTEDSESSIONINFOFROMNTHANDLE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENPROTECTEDSESSIONFROMNTHANDLE)(
    _Inout_ D3DKMT_OPENPROTECTEDSESSIONFROMNTHANDLE *);

typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OPENKEYEDMUTEXFROMNTHANDLE)(
    _Inout_ D3DKMT_OPENKEYEDMUTEXFROMNTHANDLE *);

#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_4)
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATEHWQUEUE)(_Inout_ D3DKMT_CREATEHWQUEUE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_DESTROYHWQUEUE)(
    _In_ CONST D3DKMT_DESTROYHWQUEUE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SUBMITCOMMANDTOHWQUEUE)(
    _In_ CONST D3DKMT_SUBMITCOMMANDTOHWQUEUE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SUBMITWAITFORSYNCOBJECTSTOHWQUEUE)(
    _In_ CONST D3DKMT_SUBMITWAITFORSYNCOBJECTSTOHWQUEUE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_SUBMITSIGNALSYNCOBJECTSTOHWQUEUE)(
    _In_ CONST D3DKMT_SUBMITSIGNALSYNCOBJECTSTOHWQUEUE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SUBMITPRESENTBLTTOHWQUEUE)(
    _In_ CONST D3DKMT_SUBMITPRESENTBLTTOHWQUEUE *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_5)
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_SUBMITPRESENTTOHWQUEUE)(
    _Inout_ D3DKMT_SUBMITPRESENTTOHWQUEUE *);
#endif // DXGKDDI_INTERFACE_VERSION_WDDM2_5

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_6)

typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_OUTPUTDUPLPRESENTTOHWQUEUE)(
    _In_ CONST D3DKMT_OUTPUTDUPLPRESENTTOHWQUEUE *);
...
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_ENUMADAPTERS3)(_Inout_ D3DKMT_ENUMADAPTERS3 *);
...
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CANCELPRESENTS)(_In_ D3DKMT_CANCEL_PRESENTS *);

#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_1)

typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_CREATENATIVEFENCE)(
    _Inout_ D3DKMT_CREATENATIVEFENCE *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_OPENNATIVEFENCEFROMNTHANDLE)(
    _Inout_ D3DKMT_OPENNATIVEFENCEFROMNTHANDLE *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_GETNATIVEFENCELOGDETAIL)(
    _Inout_ D3DKMT_GETNATIVEFENCELOGDETAIL *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CREATEDOORBELL)(_In_ D3DKMT_CREATE_DOORBELL *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_CONNECTDOORBELL)(_In_ D3DKMT_CONNECT_DOORBELL *);
typedef _Check_return_
NTSTATUS(APIENTRY *PFND3DKMT_DESTROYDOORBELL)(_In_ D3DKMT_DESTROY_DOORBELL *);
typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_NOTIFYWORKSUBMISSION)(
    _In_ D3DKMT_NOTIFY_WORK_SUBMISSION *);

#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_2)

typedef _Check_return_ NTSTATUS(APIENTRY *PFND3DKMT_ISFEATUREENABLED)(
    _Inout_ D3DKMT_ISFEATUREENABLED *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateAllocation(_Inout_ D3DKMT_CREATEALLOCATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateAllocation2(_Inout_ D3DKMT_CREATEALLOCATION *); // _ADVSCH_
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryResourceInfo(_Inout_ D3DKMT_QUERYRESOURCEINFO *);
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTQueryResourceInfoFromNtHandle(
    _Inout_ D3DKMT_QUERYRESOURCEINFOFROMNTHANDLE *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTShareObjects(_In_range_(1, D3DKMT_MAX_OBJECTS_PER_HANDLE) UINT cObjects,
                   _In_reads_(cObjects) CONST D3DKMT_HANDLE *hObjects,
                   _In_ POBJECT_ATTRIBUTES pObjectAttributes,
                   _In_ DWORD dwDesiredAccess, _Out_ HANDLE *phSharedNtHandle);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenNtHandleFromName(_Inout_ D3DKMT_OPENNTHANDLEFROMNAME *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenResourceFromNtHandle(_Inout_ D3DKMT_OPENRESOURCEFROMNTHANDLE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenSyncObjectFromNtHandle(_Inout_ D3DKMT_OPENSYNCOBJECTFROMNTHANDLE *);
#endif
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenResource(_Inout_ D3DKMT_OPENRESOURCE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenResource2(_Inout_ D3DKMT_OPENRESOURCE *); // _ADVSCH_
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyAllocation(_In_ CONST D3DKMT_DESTROYALLOCATION *);

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_0)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyAllocation2(_In_ CONST D3DKMT_DESTROYALLOCATION2 *);
#endif // DXGKDDI_INTERFACE_VERSION_WDDM2_0

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetAllocationPriority(_In_ CONST D3DKMT_SETALLOCATIONPRIORITY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryAllocationResidency(_In_ CONST D3DKMT_QUERYALLOCATIONRESIDENCY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateDevice(_Inout_ D3DKMT_CREATEDEVICE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyDevice(_In_ CONST D3DKMT_DESTROYDEVICE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateContext(_Inout_ D3DKMT_CREATECONTEXT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyContext(_In_ CONST D3DKMT_DESTROYCONTEXT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateSynchronizationObject(_Inout_ D3DKMT_CREATESYNCHRONIZATIONOBJECT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTCreateSynchronizationObject2(
    _Inout_ D3DKMT_CREATESYNCHRONIZATIONOBJECT2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenSynchronizationObject(_Inout_ D3DKMT_OPENSYNCHRONIZATIONOBJECT *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTWaitForSynchronizationObject(
    _In_ CONST D3DKMT_WAITFORSYNCHRONIZATIONOBJECT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTWaitForSynchronizationObject2(
    _In_ CONST D3DKMT_WAITFORSYNCHRONIZATIONOBJECT2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSignalSynchronizationObject(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSignalSynchronizationObject2(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECT2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTLock(_Inout_ D3DKMT_LOCK *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTUnlock(_In_ CONST D3DKMT_UNLOCK *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetDisplayModeList(_Inout_ D3DKMT_GETDISPLAYMODELIST *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetDisplayMode(_Inout_ CONST D3DKMT_SETDISPLAYMODE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetMultisampleMethodList(_Inout_ D3DKMT_GETMULTISAMPLEMETHODLIST *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTPresent(_Inout_ D3DKMT_PRESENT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTRender(_Inout_ D3DKMT_RENDER *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetRuntimeData(_Inout_ CONST D3DKMT_GETRUNTIMEDATA *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryAdapterInfo(_Inout_ CONST D3DKMT_QUERYADAPTERINFO *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenAdapterFromHdc(_Inout_ D3DKMT_OPENADAPTERFROMHDC *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTOpenAdapterFromGdiDisplayName(
    _Inout_ D3DKMT_OPENADAPTERFROMGDIDISPLAYNAME *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenAdapterFromDeviceName(_Inout_ D3DKMT_OPENADAPTERFROMDEVICENAME *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCloseAdapter(_In_ CONST D3DKMT_CLOSEADAPTER *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetSharedPrimaryHandle(_Inout_ D3DKMT_GETSHAREDPRIMARYHANDLE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTEscape(_In_ CONST D3DKMT_ESCAPE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryStatistics(_In_ CONST D3DKMT_QUERYSTATISTICS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetVidPnSourceOwner(_In_ CONST D3DKMT_SETVIDPNSOURCEOWNER *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetPresentHistory(_Inout_ D3DKMT_GETPRESENTHISTORY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetPresentQueueEvent(_In_ D3DKMT_HANDLE hAdapter, _Inout_ HANDLE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateOverlay(_Inout_ D3DKMT_CREATEOVERLAY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTUpdateOverlay(_In_ CONST D3DKMT_UPDATEOVERLAY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTFlipOverlay(_In_ CONST D3DKMT_FLIPOVERLAY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyOverlay(_In_ CONST D3DKMT_DESTROYOVERLAY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTWaitForVerticalBlankEvent(_In_ CONST D3DKMT_WAITFORVERTICALBLANKEVENT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetGammaRamp(_In_ CONST D3DKMT_SETGAMMARAMP *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetDeviceState(_Inout_ D3DKMT_GETDEVICESTATE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateDCFromMemory(_Inout_ D3DKMT_CREATEDCFROMMEMORY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyDCFromMemory(_In_ CONST D3DKMT_DESTROYDCFROMMEMORY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSetContextSchedulingPriority(
    _In_ CONST D3DKMT_SETCONTEXTSCHEDULINGPRIORITY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTGetContextSchedulingPriority(
    _Inout_ D3DKMT_GETCONTEXTSCHEDULINGPRIORITY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetProcessSchedulingPriorityClass(_In_ HANDLE,
                                        _In_ D3DKMT_SCHEDULINGPRIORITYCLASS);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetProcessSchedulingPriorityClass(_In_ HANDLE,
                                        _Out_ D3DKMT_SCHEDULINGPRIORITYCLASS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTReleaseProcessVidPnSourceOwners(_In_ HANDLE);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetScanLine(_Inout_ D3DKMT_GETSCANLINE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTChangeSurfacePointer(_In_ CONST D3DKMT_CHANGESURFACEPOINTER *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetQueuedLimit(_In_ CONST D3DKMT_SETQUEUEDLIMIT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTPollDisplayChildren(_In_ CONST D3DKMT_POLLDISPLAYCHILDREN *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTInvalidateActiveVidPn(_In_ CONST D3DKMT_INVALIDATEACTIVEVIDPN *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCheckOcclusion(_In_ CONST D3DKMT_CHECKOCCLUSION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTWaitForIdle(IN CONST D3DKMT_WAITFORIDLE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCheckMonitorPowerState(_In_ CONST D3DKMT_CHECKMONITORPOWERSTATE *);
EXTERN_C _Check_return_ BOOLEAN APIENTRY D3DKMTCheckExclusiveOwnership(VOID);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTCheckVidPnExclusiveOwnership(
    _In_ CONST D3DKMT_CHECKVIDPNEXCLUSIVEOWNERSHIP *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSetDisplayPrivateDriverFormat(
    _In_ CONST D3DKMT_SETDISPLAYPRIVATEDRIVERFORMAT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSharedPrimaryLockNotification(
    _In_ CONST D3DKMT_SHAREDPRIMARYLOCKNOTIFICATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSharedPrimaryUnLockNotification(
    _In_ CONST D3DKMT_SHAREDPRIMARYUNLOCKNOTIFICATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateKeyedMutex(_Inout_ D3DKMT_CREATEKEYEDMUTEX *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenKeyedMutex(_Inout_ D3DKMT_OPENKEYEDMUTEX *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyKeyedMutex(_In_ CONST D3DKMT_DESTROYKEYEDMUTEX *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTAcquireKeyedMutex(_Inout_ D3DKMT_ACQUIREKEYEDMUTEX *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTReleaseKeyedMutex(_Inout_ D3DKMT_RELEASEKEYEDMUTEX *);
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateKeyedMutex2(_Inout_ D3DKMT_CREATEKEYEDMUTEX2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenKeyedMutex2(_Inout_ D3DKMT_OPENKEYEDMUTEX2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTAcquireKeyedMutex2(_Inout_ D3DKMT_ACQUIREKEYEDMUTEX2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTReleaseKeyedMutex2(_Inout_ D3DKMT_RELEASEKEYEDMUTEX2 *);
#endif
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTConfigureSharedResource(_In_ CONST D3DKMT_CONFIGURESHAREDRESOURCE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetOverlayState(_Inout_ D3DKMT_GETOVERLAYSTATE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCheckSharedResourceAccess(_In_ CONST D3DKMT_CHECKSHAREDRESOURCEACCESS *);
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOfferAllocations(_In_ CONST D3DKMT_OFFERALLOCATIONS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTReclaimAllocations(_Inout_ CONST D3DKMT_RECLAIMALLOCATIONS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateOutputDupl(_In_ CONST D3DKMT_CREATE_OUTPUTDUPL *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyOutputDupl(_In_ CONST D3DKMT_DESTROY_OUTPUTDUPL *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOutputDuplGetFrameInfo(_Inout_ D3DKMT_OUTPUTDUPL_GET_FRAMEINFO *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOutputDuplGetMetaData(_Inout_ D3DKMT_OUTPUTDUPL_METADATA *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTOutputDuplGetPointerShapeData(
    _Inout_ D3DKMT_OUTPUTDUPL_GET_POINTER_SHAPE_DATA *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOutputDuplReleaseFrame(_Inout_ D3DKMT_OUTPUTDUPL_RELEASE_FRAME *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOutputDuplPresent(_In_ CONST D3DKMT_OUTPUTDUPLPRESENT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTEnumAdapters(_Inout_ CONST D3DKMT_ENUMADAPTERS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTEnumAdapters2(_Inout_ CONST D3DKMT_ENUMADAPTERS2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenAdapterFromLuid(_Inout_ CONST D3DKMT_OPENADAPTERFROMLUID *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryRemoteVidPnSourceFromGdiDisplayName(
    _Inout_ D3DKMT_QUERYREMOTEVIDPNSOURCEFROMGDIDISPLAYNAME *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetVidPnSourceOwner1(_In_ CONST D3DKMT_SETVIDPNSOURCEOWNER1 *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSetSyncRefreshCountWaitTarget(
    _In_ CONST D3DKMT_SETSYNCREFRESHCOUNTWAITTARGET *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetDWMVerticalBlankEvent(_In_ CONST D3DKMT_GETVERTICALBLANKEVENT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTPresentMultiPlaneOverlay(_In_ CONST D3DKMT_PRESENT_MULTIPLANE_OVERLAY *);
...
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_0)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTMakeResident(_Inout_ D3DDDI_MAKERESIDENT *);
...
D3DKMTSignalSynchronizationObjectFromGpu2(
    _In_ CONST D3DKMT_SIGNALSYNCHRONIZATIONOBJECTFROMGPU2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreatePagingQueue(_Inout_ D3DKMT_CREATEPAGINGQUEUE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyPagingQueue(_Inout_ D3DDDI_DESTROYPAGINGQUEUE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTLock2(_Inout_ D3DKMT_LOCK2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTUnlock2(_In_ CONST D3DKMT_UNLOCK2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTInvalidateCache(_In_ CONST D3DKMT_INVALIDATECACHE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTMapGpuVirtualAddress(_Inout_ D3DDDI_MAPGPUVIRTUALADDRESS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTReserveGpuVirtualAddress(_Inout_ D3DDDI_RESERVEGPUVIRTUALADDRESS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTFreeGpuVirtualAddress(_In_ CONST D3DKMT_FREEGPUVIRTUALADDRESS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTUpdateGpuVirtualAddress(_In_ CONST D3DKMT_UPDATEGPUVIRTUALADDRESS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetResourcePresentPrivateDriverData(
    _Inout_ D3DDDI_GETRESOURCEPRESENTPRIVATEDRIVERDATA *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateContextVirtual(_In_ D3DKMT_CREATECONTEXTVIRTUAL *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSubmitCommand(_In_ CONST D3DKMT_SUBMITCOMMAND *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenSyncObjectFromNtHandle2(_Inout_ D3DKMT_OPENSYNCOBJECTFROMNTHANDLE2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTOpenSyncObjectNtHandleFromName(
    _Inout_ D3DKMT_OPENSYNCOBJECTNTHANDLEFROMNAME *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryVideoMemoryInfo(_Inout_ D3DKMT_QUERYVIDEOMEMORYINFO *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTChangeVideoMemoryReservation(
    _In_ CONST D3DKMT_CHANGEVIDEOMEMORYRESERVATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTRegisterTrimNotification(_Inout_ D3DKMT_REGISTERTRIMNOTIFICATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTUnregisterTrimNotification(_Inout_ D3DKMT_UNREGISTERTRIMNOTIFICATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTCheckMultiPlaneOverlaySupport2(
    _Inout_ D3DKMT_CHECKMULTIPLANEOVERLAYSUPPORT2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTPresentMultiPlaneOverlay2(
    _In_ CONST D3DKMT_PRESENT_MULTIPLANE_OVERLAY2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTReclaimAllocations2(_Inout_ D3DKMT_RECLAIMALLOCATIONS2 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetStablePowerState(_In_ CONST D3DKMT_SETSTABLEPOWERSTATE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryClockCalibration(_Inout_ D3DKMT_QUERYCLOCKCALIBRATION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTQueryVidPnExclusiveOwnership(
    _Inout_ D3DKMT_QUERYVIDPNEXCLUSIVEOWNERSHIP *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTAdjustFullscreenGamma(_In_ D3DKMT_ADJUSTFULLSCREENGAMMA *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetVidPnSourceHwProtection(_In_ D3DKMT_SETVIDPNSOURCEHWPROTECTION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTMarkDeviceAsError(_In_ D3DKMT_MARKDEVICEASERROR *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTFlushHeapTransitions(_In_ D3DKMT_FLUSHHEAPTRANSITIONS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTSetHwProtectionTeardownRecovery(
    _In_ D3DKMT_SETHWPROTECTIONTEARDOWNRECOVERY *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_1)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryProcessOfferInfo(_Inout_ D3DKMT_QUERYPROCESSOFFERINFO *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTTrimProcessCommitment(_Inout_ D3DKMT_TRIMPROCESSCOMMITMENT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTUpdateAllocationProperty(_Inout_ D3DDDI_UPDATEALLOCPROPERTY *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTPresentMultiPlaneOverlay3(
    _In_ CONST D3DKMT_PRESENT_MULTIPLANE_OVERLAY3 *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetFSEBlock(_In_ CONST D3DKMT_SETFSEBLOCK *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryFSEBlock(_Inout_ D3DKMT_QUERYFSEBLOCK *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_2)

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateHwContext(_Inout_ D3DKMT_CREATEHWCONTEXT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyHwContext(_In_ CONST D3DKMT_DESTROYHWCONTEXT *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateHwQueue(_Inout_ D3DKMT_CREATEHWQUEUE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyHwQueue(_In_ CONST D3DKMT_DESTROYHWQUEUE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSubmitCommandToHwQueue(_In_ CONST D3DKMT_SUBMITCOMMANDTOHWQUEUE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
...
D3DKMTSubmitSignalSyncObjectsToHwQueue(
    _In_ CONST D3DKMT_SUBMITSIGNALSYNCOBJECTSTOHWQUEUE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetAllocationPriority(_In_ CONST D3DKMT_GETALLOCATIONPRIORITY *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetMultiPlaneOverlayCaps(_Inout_ D3DKMT_GET_MULTIPLANE_OVERLAY_CAPS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetPostCompositionCaps(_Inout_ D3DKMT_GET_POST_COMPOSITION_CAPS *);

#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_3)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTPresentRedirected(_In_ D3DKMT_PRESENT_REDIRECTED *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetVidPnSourceOwner2(_In_ CONST D3DKMT_SETVIDPNSOURCEOWNER2 *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSetMonitorColorSpaceTransform(_In_ D3DKMT_SET_COLORSPACE_TRANSFORM *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateProtectedSession(_Inout_ D3DKMT_CREATEPROTECTEDSESSION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyProtectedSession(_Inout_ D3DKMT_DESTROYPROTECTEDSESSION *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTQueryProtectedSessionStatus(_Inout_ D3DKMT_QUERYPROTECTEDSESSIONSTATUS *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTGetProcessDeviceRemovalSupport(
    _Inout_ D3DKMT_GETPROCESSDEVICEREMOVALSUPPORT *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenKeyedMutexFromNtHandle(_Inout_ D3DKMT_OPENKEYEDMUTEXFROMNTHANDLE *);
...
EXTERN_C
_Check_return_ NTSTATUS APIENTRY D3DKMTRegisterVailProcess(_In_ GUID *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSubmitPresentBltToHwQueue(_In_ CONST D3DKMT_SUBMITPRESENTBLTTOHWQUEUE *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTSubmitPresentToHwQueue(_Inout_ D3DKMT_SUBMITPRESENTTOHWQUEUE *);

#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_6)

EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTOutputDuplPresentToHwQueue(
    _In_ CONST D3DKMT_OUTPUTDUPLPRESENTTOHWQUEUE *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTEnumAdapters3(_Inout_ D3DKMT_ENUMADAPTERS3 *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCancelPresents(_In_ D3DKMT_CANCEL_PRESENTS *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTShareObjectWithHost(_Inout_ D3DKMT_SHAREOBJECTWITHHOST *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateSyncFile(_Inout_ D3DKMT_CREATESYNCFILE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTWaitSyncFile(_In_ CONST D3DKMT_WAITSYNCFILE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenSyncObjectFromSyncFile(_Inout_ D3DKMT_OPENSYNCOBJECTFROMSYNCFILE *);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateNativeFence(_Inout_ D3DKMT_CREATENATIVEFENCE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTOpenNativeFenceFromNtHandle(_Inout_ D3DKMT_OPENNATIVEFENCEFROMNTHANDLE *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTGetNativeFenceLogDetail(_Inout_ D3DKMT_GETNATIVEFENCELOGDETAIL *);

EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTCreateDoorbell(_In_ D3DKMT_CREATE_DOORBELL *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTConnectDoorbell(_In_ D3DKMT_CONNECT_DOORBELL *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTDestroyDoorbell(_In_ D3DKMT_DESTROY_DOORBELL *);
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTNotifyWorkSubmission(_In_ D3DKMT_NOTIFY_WORK_SUBMISSION *);
#endif

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM3_2)
EXTERN_C _Check_return_ NTSTATUS APIENTRY
D3DKMTIsFeatureEnabled(_Inout_ D3DKMT_ISFEATUREENABLED *);
...
DEFINE_GUID(GUID_DEVINTERFACE_GRAPHICSPOWER, 0xea5c6870, 0xe93c, 0x4588, 0xbe,
            0xf1, 0xfe, 0xc4, 0x2f, 0xc9, 0x42, 0x9a);
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    VOID (*PDXGK_POWER_NOTIFICATION)(PVOID GraphicsDeviceHandle,
                                     DEVICE_POWER_STATE NewGrfxPowerState,
                                     BOOLEAN PreNotification,
                                     PVOID PrivateHandle);

typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    VOID (*PDXGK_REMOVAL_NOTIFICATION)(PVOID GraphicsDeviceHandle,
                                       PVOID PrivateHandle);

typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    VOID (*PDXGK_FSTATE_NOTIFICATION)(PVOID GraphicsDeviceHandle,
                                      ULONG ComponentIndex, UINT NewFState,
                                      BOOLEAN PreNotification,
                                      PVOID PrivateHandle);

typedef _IRQL_requires_(DISPATCH_LEVEL) VOID (*PDXGK_INITIAL_COMPONENT_STATE)(
    PVOID GraphicsDeviceHandle, PVOID PrivateHandle, ULONG ComponentIndex,
    BOOLEAN IsBlockingType, UINT InitialFState, GUID ComponentGuid,
...
typedef DXGK_GRAPHICSPOWER_REGISTER_INPUT_V_1_2
    DXGK_GRAPHICSPOWER_REGISTER_INPUT;
typedef DXGK_GRAPHICSPOWER_REGISTER_INPUT *PDXGK_GRAPHICSPOWER_REGISTER_INPUT;

typedef _Check_return_ _IRQL_requires_max_(APC_LEVEL)
    NTSTATUS (*PDXGK_SET_SHARED_POWER_COMPONENT_STATE)(PVOID DeviceHandle,
                                                       PVOID PrivateHandle,
                                                       ULONG ComponentIndex,
                                                       BOOLEAN Active);

typedef _Check_return_ _IRQL_requires_max_(APC_LEVEL)
    NTSTATUS (*PDXGK_GRAPHICSPOWER_UNREGISTER)(PVOID DeviceHandle,
                                               PVOID PrivateHandle);
