...
#define SO_CONDITIONAL_ACCEPT                                                  \
  0x3002                         // enable true conditional accept:
...
#define SO_REUSE_UNICASTPORT                                                   \
  0x3007 // defer ephemeral port allocation for
         // outbound connections
#define SO_REUSE_MULTICASTPORT                                                 \
  0x3008 // enable port reuse and disable unicast
         // reception.
#define SO_ORIGINAL_DST                                                        \
  0x300F // Query the original destination address
...
#define SO_RECEIVED_PROCESSOR                                                  \
  0x3011 // Receive the processor number packets
...
//
// If you don't use the entire sdl_data field, then fill it starting with the
// low bytes...
//
...
#define AI_V4MAPPED                                                            \
  0x00000800 // On v6 failure, query v4 and convert to V4MAPPED format
...
#define AI_SECURE_WITH_FALLBACK                                                \
  0x00100000 // Forces clear text fallback if the secure DNS query fails
#define AI_EXCLUSIVE_CUSTOM_SERVERS                                            \
  0x00200000 // Use exclusively the custom DNS servers
#define AI_RETURN_RESPONSE_FLAGS                                               \
  0x10000000 // Requests extra information about the DNS results
#define AI_REQUIRE_SECURE                                                      \
  0x20000000 // Forces the DNS query to be done over seucre protocols
#define AI_RESOLUTION_HANDLE 0x40000000 // Request resolution handle
#define AI_EXTENDED                                                            \
  0x80000000 // Indicates this is extended ADDRINFOEX(2/..) struct
...
// These options will be set in the ai_servertype field in the
...
// These options will be set in the ai_flags field in the ADDRINFO_DNS_SERVER
// struct.
//
...
#define AI_DNS_RESPONSE_SECURE                                                 \
  0x1 // Present if the resolution was done through secure protocols
...
#define AI_EXTRA_DNSSEC_REQUIRED                                               \
  0x0000000000000001 // Set DO and AD bits in request, returns IP address info
                     // IF DNSSEC authenticated, NOTE: only the IP address will
                     // be returned not the DNSSEC specific records
