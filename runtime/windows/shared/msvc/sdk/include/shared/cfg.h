...
#define CM_PROB_DRIVER_FAILED_PRIOR_UNLOAD                                     \
  (0x00000026)                                  // Driver should have unloaded.
#define CM_PROB_DRIVER_FAILED_LOAD (0x00000027) // Driver load unsuccessful.
#define CM_PROB_DRIVER_SERVICE_KEY_INVALID                                     \
  (0x00000028) // Error accessing driver's service key
#define CM_PROB_LEGACY_SERVICE_NO_DEVICES                                      \
  (0x00000029) // Loaded legacy service created no devices
#define CM_PROB_DUPLICATE_DEVICE                                               \
  (0x0000002A) // Two devices were discovered with the same name
#define CM_PROB_FAILED_POST_START                                              \
  (0x0000002B) // The drivers set the device state to failed
#define CM_PROB_HALTED                                                         \
  (0x0000002C) // This device was failed post start via usermode
#define CM_PROB_PHANTOM                                                        \
  (0x0000002D) // The devinst currently exists only in the registry
#define CM_PROB_SYSTEM_SHUTDOWN (0x0000002E) // The system is shutting down
#define CM_PROB_HELD_FOR_EJECT                                                 \
  (0x0000002F) // The device is offline awaiting removal
#define CM_PROB_DRIVER_BLOCKED                                                 \
  (0x00000030) // One or more drivers is blocked from loading
#define CM_PROB_REGISTRY_TOO_LARGE                                             \
  (0x00000031) // System hive has grown too large
#define CM_PROB_SETPROPERTIES_FAILED                                           \
  (0x00000032) // Failed to apply one or more registry properties
#define CM_PROB_WAITING_ON_DEPENDENCY                                          \
  (0x00000033) // Device is stalled waiting on a dependency to start
#define CM_PROB_UNSIGNED_DRIVER                                                \
  (0x00000034) // Failed load driver due to unsigned image.
#define CM_PROB_USED_BY_DEBUGGER                                               \
  (0x00000035) // Device is being used by kernel debugger
#define CM_PROB_DEVICE_RESET (0x00000036) // Device is being reset
#define CM_PROB_CONSOLE_LOCKED                                                 \
  (0x00000037) // Device is blocked while console is locked
#define CM_PROB_NEED_CLASS_CONFIG                                              \
  (0x00000038) // Device needs extended class configuration to start
#define CM_PROB_GUEST_ASSIGNMENT_FAILED                                        \
  (0x00000039) // Assignment to guest partition failed
...
#define DN_BAD_PARTIAL                                                         \
  0x00400000 // S: This devnode's log_confs do not have same resources
...
#define DN_BOOT_LOG_PROB                                                       \
  0x80000000 // S: Had a problem during preassignment of boot log conf
...
#define DN_NEED_RESTART                                                        \
  DN_LIAR // System needs to be restarted for this Devnode to work properly
...
#define DN_DRIVER_BLOCKED                                                      \
  DN_NOT_FIRST_TIME // One or more drivers are blocked from loading for this
                    // Devnode
#define DN_LEGACY_DRIVER DN_MOVED // This device is using a legacy driver
#define DN_CHILD_WITH_INVALID_ID                                               \
  DN_HAS_MARK // One or more children have invalid ID(s)

#endif

#if (NTDDI_VERSION >= NTDDI_WIN8)

#define DN_DEVICE_DISCONNECTED                                                 \
  DN_NEEDS_LOCKING // The function driver for a device reported that the device
                   // is not connected.  Typically this means a wireless device
                   // is out of range.
...
#define DN_QUERY_REMOVE_PENDING                                                \
  DN_MF_PARENT // Device is part of a set of related devices collectively
               // pending query-removal
#define DN_QUERY_REMOVE_ACTIVE                                                 \
  DN_MF_CHILD // Device is actively engaged in a query-remove IRP
