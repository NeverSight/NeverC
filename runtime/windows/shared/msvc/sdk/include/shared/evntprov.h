...
#define EVENT_MIN_LEVEL                                                        \
  (0) /*                                                                       \
Lowest value for an event level is 0. */
#define EVENT_MAX_LEVEL                                                        \
  (0xff) /*                                                                    \
...
#define MAX_EVENT_DATA_DESCRIPTORS                                             \
  (128) /*                                                                     \
...
#define EVENT_FILTER_TYPE_PACKAGE_APP_ID                                       \
  (0x80000020) // Package Relative App Id (PRAID).
#define EVENT_FILTER_TYPE_PAYLOAD (0x80000100)  // TDH payload filter.
#define EVENT_FILTER_TYPE_EVENT_ID (0x80000200) // Event IDs.
#define EVENT_FILTER_TYPE_EVENT_NAME                                           \
  (0x80000400) // Event name (TraceLogging only).
#define EVENT_FILTER_TYPE_STACKWALK (0x80001000) // Event IDs for stack.
#define EVENT_FILTER_TYPE_STACKWALK_NAME                                       \
  (0x80002000) // Event name for stack (TraceLogging only).
#define EVENT_FILTER_TYPE_STACKWALK_LEVEL_KW                                   \
  (0x80004000) // Filter stack collection by level and keyword.
...
*/
#define EVENT_DATA_DESCRIPTOR_TYPE_NONE                                        \
  (0) /*                                                                       \
Event data (payload). */
#define EVENT_DATA_DESCRIPTOR_TYPE_EVENT_METADATA                              \
  (1) /*                                                                       \
TraceLogging event decoding information. */
#define EVENT_DATA_DESCRIPTOR_TYPE_PROVIDER_METADATA                           \
  (2) /*                                                                       \
...
EventSetInformation. */
#define EVENT_DATA_DESCRIPTOR_TYPE_TIMESTAMP_OVERRIDE                          \
  (3) /*                                                                       \
...
*/
#define EVENT_WRITE_FLAG_NO_FAULTING                                           \
  0x00000001 /*                                                                \
...
#define EVENT_WRITE_FLAG_INPRIVATE                                             \
  0x00000002 /*                                                                \
...
typedef struct _EVENT_DATA_DESCRIPTOR {
  ...
  union {
    ...
    struct {
      UCHAR Type; // Default is EVENT_DATA_DESCRIPTOR_TYPE_NONE, meaning normal
                  // payload data.
      UCHAR Reserved1;
      USHORT Reserved2;
    } DUMMYSTRUCTNAME;
...
typedef struct _EVENT_DESCRIPTOR {
  ...
  Opcode;            /*
           ...
           event and restore the previous activity ID. Note that there is no
           enforcement      of this pattern, and an application must be aware that
           other      code may      potentially overwrite the activity ID without
           restoring it. In      addition, the implicit activity ID does not work well
           with cross-thread      activities. For these reasons, it may be more
           appropriate to use      explicit activity IDs (explicitly pass a GUID to
           EventWriteTransfer)      instead of relying on the implicity activity ID. */
  USHORT Task;       /*
...
typedef VOID(NTAPI *PENABLECALLBACK)(
    _In_ LPCGUID SourceId, _In_ ULONG IsEnabled, _In_ UCHAR Level,
...
ULONG
EVNTAPI
EventRegister(_In_ LPCGUID ProviderId, _In_opt_ PENABLECALLBACK EnableCallback,
              _In_opt_ PVOID CallbackContext, _Out_ PREGHANDLE RegHandle);
#endif
...
ULONG
EVNTAPI
EventSetInformation(_In_ REGHANDLE RegHandle,
...
BOOLEAN
EVNTAPI
EventProviderEnabled(_In_ REGHANDLE RegHandle, _In_ UCHAR Level,
...
ULONG
EVNTAPI
EventWrite(_In_ REGHANDLE RegHandle, _In_ PCEVENT_DESCRIPTOR EventDescriptor,
...
EventWriteTransfer(regHandle, &eventDesc, pAI, pRAI, cData, pData) is equivalent
to EventWriteEx(regHandle, &eventDesc, 0, 0, pAI, pRAI, cData, pData).
*/
ULONG
EVNTAPI
EventWriteTransfer(
    _In_ REGHANDLE RegHandle, _In_ PCEVENT_DESCRIPTOR EventDescriptor,
    _In_opt_ LPCGUID ActivityId, _In_opt_ LPCGUID RelatedActivityId,
    _In_range_(0, MAX_EVENT_DATA_DESCRIPTORS) ULONG UserDataCount,
    _In_reads_opt_(UserDataCount) PEVENT_DATA_DESCRIPTOR UserData);
#endif
...
ULONG
EVNTAPI
EventWriteEx(_In_ REGHANDLE RegHandle, _In_ PCEVENT_DESCRIPTOR EventDescriptor,
             _In_ ULONG64 Filter, _In_ ULONG Flags, _In_opt_ LPCGUID ActivityId,
...
ULONG
EVNTAPI
EventWriteString(_In_ REGHANDLE RegHandle, _In_ UCHAR Level,
                 _In_ ULONGLONG Keyword, _In_ PCWSTR String);
#endif
...
VOID EventDataDescCreate(_Out_ PEVENT_DATA_DESCRIPTOR EventDataDescriptor,
...
*/
EVNTPROV_PFORCEINLINE
VOID EventDescCreate(_Out_ PEVENT_DESCRIPTOR EventDescriptor, _In_ USHORT Id,
                     _In_ UCHAR Version, _In_ UCHAR Channel, _In_ UCHAR Level,
                     _In_ USHORT Task, _In_ UCHAR Opcode,
                     _In_ ULONGLONG Keyword) {
...
*/
EVNTPROV_PFORCEINLINE
VOID EventDescZero(_Out_ PEVENT_DESCRIPTOR EventDescriptor) {
...
EVNTPROV_PFORCEINLINE
USHORT
EventDescGetId(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
  return (EventDescriptor->Id);
}

EVNTPROV_PFORCEINLINE
UCHAR
EventDescGetVersion(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
  return (EventDescriptor->Version);
}

EVNTPROV_PFORCEINLINE
USHORT
EventDescGetTask(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
  return (EventDescriptor->Task);
}

EVNTPROV_PFORCEINLINE
UCHAR
EventDescGetOpcode(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
  return (EventDescriptor->Opcode);
}

EVNTPROV_PFORCEINLINE
UCHAR
EventDescGetChannel(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
  return (EventDescriptor->Channel);
}

EVNTPROV_PFORCEINLINE
UCHAR
EventDescGetLevel(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
  return (EventDescriptor->Level);
}

EVNTPROV_PFORCEINLINE
ULONGLONG
EventDescGetKeyword(_In_ PCEVENT_DESCRIPTOR EventDescriptor) {
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetId(_In_ PEVENT_DESCRIPTOR EventDescriptor, _In_ USHORT Id) {
  EventDescriptor->Id = Id;
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetVersion(_In_ PEVENT_DESCRIPTOR EventDescriptor,
                    _In_ UCHAR Version) {
  EventDescriptor->Version = Version;
  return (EventDescriptor);
}
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetTask(_In_ PEVENT_DESCRIPTOR EventDescriptor, _In_ USHORT Task) {
  EventDescriptor->Task = Task;
  return (EventDescriptor);
}
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetOpcode(_In_ PEVENT_DESCRIPTOR EventDescriptor, _In_ UCHAR Opcode) {
  EventDescriptor->Opcode = Opcode;
  return (EventDescriptor);
}
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetLevel(_In_ PEVENT_DESCRIPTOR EventDescriptor, _In_ UCHAR Level) {
  EventDescriptor->Level = Level;
  return (EventDescriptor);
}
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetChannel(_In_ PEVENT_DESCRIPTOR EventDescriptor,
                    _In_ UCHAR Channel) {
  EventDescriptor->Channel = Channel;
  return (EventDescriptor);
}
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescSetKeyword(_In_ PEVENT_DESCRIPTOR EventDescriptor,
                    _In_ ULONGLONG Keyword) {
  EventDescriptor->Keyword = Keyword;
  return (EventDescriptor);
}
...
EVNTPROV_PFORCEINLINE
PEVENT_DESCRIPTOR
EventDescOrKeyword(_In_ PEVENT_DESCRIPTOR EventDescriptor,
                   _In_ ULONGLONG Keyword) {
  EventDescriptor->Keyword |= Keyword;
  return (EventDescriptor);
}
