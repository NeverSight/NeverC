...
#if !defined(_D3DKMDT_H) && !defined(_D3DKMTHK_H_) &&                          \
    !defined(_D3DUMDDI_H_) && !defined(__DXGKRNLETW_H__)
...
// Components which depend on D3D_UMD_INTERFACE_VERSION need to be updated,
// static assert validation present. Search for D3D_UMD_INTERFACE_VERSION across
// all depots to ensure all dependencies are updated.
...
//
// These macros are used to enable non-Windows 32-bit usermode to fill out a
// fixed-size structure for D3DKMT structures, so that they can be sent to a
// 64-bit kernel without a thunking layer for translation.
//
// Note that a thunking layer is still used for wchar_t translation, where
// Windows uses 16-bit characters and non-Windows uses 32-bit characters.
//
// If brace initialization is used (e.g. D3DKMT_FOO foo = { a, b, c }), be aware
// that for non-Windows, pointers will be unioned such that a 64-bit integer is
// the member that is initialized. It is not possible to achieve both type
// safety and proper zero-initialization of the high bits of pointers with brace
// initialization in this model. Use D3DKMT_PTR_INIT(ptr) to appropriately cast
// to a 64-bit integer for non-Windows, or to pass the pointer unchanged for
// Windows. To maintain type safety, manually assign the fields after
// zero-initializing the struct.
//
#ifdef _WIN32
...
//
#define IOCTL_GPUP_DRIVER_ESCAPE                                               \
  CTL_CODE(FILE_DEVICE_UNKNOWN, (8 + 0x910), METHOD_BUFFERED, FILE_READ_DATA)
...
//
//  Page Table Entry structure. Contains segment/physical address pointing to a
//  page
//
...
typedef struct _DXGK_PTE {
  ...
  union {
    ULONGLONG PageAddress; // High 52 bits of 64 bit physical address. Low 12
                           // bits are zero.
    ULONGLONG PageTableAddress; // High 52 bits of 64 bit physical address. Low
                                // 12 bits are zero.
  };
} DXGK_PTE;
...
//
// DDI level handle that represents a kernel mode object (allocation, device,
// etc)
//
...
//
// Remarks: Fractional value used to represent vertical and horizontal
// frequencies of a video mode
//          (i.e. VSync and HSync). Vertical frequencies are stored in Hz.
//          Horizontal frequencies are stored in Hz. The dynamic range of this
//          encoding format, given 10^-7 resolution is {0..(2^32 - 1) / 10^7},
//          which translates to {0..428.4967296} [Hz] for vertical frequencies
//          and {0..428.4967296} [Hz] for horizontal frequencies. This
...
//          10^7/(60*60*24) = 115.741 days.
//
//          If rational number with a finite fractional sequence, use
//          denominator of form 10^(length of fractional sequence). If rational
//          number without a finite fractional sequence, or a sequence exceeding
//          the precision allowed by the dynamic range of the denominator, or an
//          irrational number, use an appropriate ratio of integers which best
...
typedef struct _D3DDDI_ALLOCATIONINFO {
  ...
  D3DKMT_PTR(
      VOID *,
      pPrivateDriverData); // in(out optional): Private data for each allocation
  UINT PrivateDriverDataSize; // in: Size of the private data
  D3DDDI_VIDEO_PRESENT_SOURCE_ID
  VidPnSourceId; // in: VidPN source ID if this is a primary
  union {
    struct {
...
typedef struct _D3DDDI_ALLOCATIONINFO2 {
  ...
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      GpuVirtualAddress; // out: GPU Virtual address of the allocation created.
#if ((DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_2) ||       \
     (D3D_UMD_INTERFACE_VERSION >= D3D_UMD_INTERFACE_VERSION_WDDM2_2))
  union {
...
typedef struct _D3DDDI_OPENALLOCATIONINFO {
  ...
  D3DKMT_PTR(CONST VOID *, pPrivateDriverData); // in: Ptr to driver private
                                                // buffer for this allocations
  UINT PrivateDriverDataSize; // in: Size in bytes of driver private buffer for
                              // this allocations
...
typedef struct _D3DDDI_OPENALLOCATIONINFO2 {
  ...
  UINT PrivateDriverDataSize; // in: Size in bytes of driver private buffer for
                              // this allocations
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      GpuVirtualAddress; // out: GPU Virtual address of the allocation opened.
  D3DKMT_ALIGN64 ULONG_PTR Reserved[6]; // Reserved
...
typedef enum _D3DDDI_OFFER_PRIORITY {
  ...
  D3DDDI_OFFER_PRIORITY_AUTO, // Let VidMm decide offer priority based on
                              // eviction priority
  ...
  union {
    struct {
     ...
     (D3D_UMD_INTERFACE_VERSION >= D3D_UMD_INTERFACE_VERSION_WDDM1_3))
      ...
      UINT DriverKnownEscape : 1;  // 0x00000040       // Driver private data
                                   // points to a well known structure
      UINT DriverCommonEscape : 1; // 0x00000080       // Private data points
...
// Used as a value for D3DDDI_VIDEO_PRESENT_SOURCE_ID and
// D3DDDI_VIDEO_PRESENT_TARGET_ID types to specify that the respective video
// present source/target ID hasn't been initialized.
#define D3DDDI_ID_UNINITIALIZED (UINT)(~0)

// TODO:[mmilirud] Define this as (UINT)(~1) to avoid collision with valid
// source ID equal to 0.
//
// Used as a value for D3DDDI_VIDEO_PRESENT_SOURCE_ID and
// D3DDDI_VIDEO_PRESENT_TARGET_ID types to specify that the respective video
// present source/target ID isn't applicable for the given execution context.
#define D3DDDI_ID_NOTAPPLICABLE (UINT)(0)

// Indicates that a resource can be associated with "any" VidPn source, even
// none at all.
#define D3DDDI_ID_ANY (UINT)(~1)

// Used as a value for D3DDDI_VIDEO_PRESENT_SOURCE_ID and
// D3DDDI_VIDEO_PRESENT_TARGET_ID types to specify that the respective video
// present source/target ID describes every VidPN source/target in question.
...
//
// Remarks: Driver is free to partition its quality levels for a given
// multi-sampling method into as many
//          increments as it likes, with the condition that each incremental
//          step does noticably improve quality of the presented image.
//
typedef struct _D3DDDI_MULTISAMPLINGMETHOD {
  // Number of sub-pixels employed in this multi-sampling method (e.g. 2 for 2x
  // and 8 for 8x multi-sampling)
  UINT NumSamples;

  // Upper bound on the quality range supported for this multi-sampling method.
  // The range starts from 0 and goes upto and including the reported maximum
  // quality setting.
...
//
// Remarks: Scan-line ordering of the video mode, specifies whether each field
// contains the entire
...
//          this characteristic explicitly with an enum both frees up the client
//          from having to maintain mode-based look-up tables and is extensible
//          for future standard modes not listed in the
//          D3DKMDT_VIDEO_SIGNAL_STANDARD enum.
//
...
typedef enum D3DDDI_FLIPINTERVAL_TYPE {
  ...
  // This value is only valid for the D3D9 runtime PresentCb
  // SyncIntervalOverride field. For this field, IMMEDIATE means the API
  // semantic of sync interval 0, where IMMEDIATE_ALLOW_TEARING is equivalent to
  // the addition of the DXGI ALLOW_TEARING API flags.
...
typedef struct _D3DDDIARG_CREATERESOURCE {
  ...
  CONST D3DDDI_SURFACEINFO
      *pSurfList; // in: List of sub resource objects to create
  UINT SurfCount; // in: Number of sub resource objects
  UINT MipLevels;
  UINT Fvf; // in: FVF format for vertex buffers
  D3DDDI_VIDEO_PRESENT_SOURCE_ID
  VidPnSourceId; // in: VidPnSourceId on which the primary surface is
                 // created
  D3DDDI_RATIONAL RefreshRate; // in: RefreshRate that this primary surface is
                               // to be used with
  HANDLE hResource;            // in/out: D3D runtime handle/UM driver handle
  D3DDDI_RESOURCEFLAGS Flags;
  D3DDDI_ROTATION
  Rotation; // in: The orientation of the resource. (0, 90, 180, 270)
...
#define D3DDDI_SYNC_OBJECT_ALL_ACCESS                                          \
  (STANDARD_RIGHTS_REQUIRED | SYNCHRONIZE | D3DDDI_SYNC_OBJECT_WAIT |          \
...
typedef struct {
  ...
  UINT EncodeRate; // Encode bitrate driver reported for the chunk, kilobits per
                   // second
...
typedef struct D3DDDI_MAKERESIDENT_FLAGS {
  union {
    struct {
      UINT CantTrimFurther
          : 1; // When set, MakeResidentCb will succeed even if the request puts
               // the application over the current budget. MakeResidentCb will
               // still fail if the request puts the application over the
               // maximum budget.
      UINT MustSucceed
          : 1; // When set, instructs MakeResidentCb to put the device in error
               // if the resource cannot be made resident.
      UINT Reserved : 30;
    };
    UINT Value;
  };
...
typedef struct D3DDDI_MAKERESIDENT {
  D3DKMT_HANDLE hPagingQueue; // [in] Handle to the paging queue used to
                              // synchronize paging operations for this call.
  UINT NumAllocations; // [in/out] On input, the number of allocation handles om
                       // the AllocationList array. On output,
                       //          the number of allocations successfully made
                       //          resident.
  D3DKMT_PTR(
      _Field_size_(NumAllocations) CONST D3DKMT_HANDLE *,
      AllocationList); // [in] An array of NumAllocations allocation handles
  D3DKMT_PTR(CONST UINT *,
             PriorityList); // [in] Residency priority array for each of the
                            // allocations in the resource or allocation list
  D3DDDI_MAKERESIDENT_FLAGS Flags; // [in] Residency flags
  D3DKMT_ALIGN64 UINT64
      PagingFenceValue; // [out] Paging fence value to synchronize on before
                        // submitting the command
                        //      that uses above resources to the GPU. This value
                        //      applies to the monitored fence synchronization
                        //      object associated with hPagingQueue.
  D3DKMT_ALIGN64 UINT64
      NumBytesToTrim; // [out] When MakeResident fails due to being over budget,
                      // this value
                      //      indicates how much to trim in order for the call
...
typedef struct D3DDDI_TRIMRESIDENCYSET_FLAGS {
  union {
    struct {
      UINT PeriodicTrim
          : 1; // When PeriodicTrim flag is set, the driver is required to
               // performed the following operations: a) trim all allocations
               // that were not referenced since the previous periodic trim
               // request by comparing the allocation last referenced fence with
               // the last periodic trim context fence b) Refresh the last
               // periodic trim context fence with the last completed context
               // fence.
      UINT RestartPeriodicTrim
          : 1; // May not be set together with PeriodicTrim flag.
               // Reset the last periodic trim context fence to the last
               // completed context fence.
      UINT TrimToBudget : 1; // Indicates that the application usage is over the
                             // memory budget, and NumBytesToTrim bytes should
                             // be trimmed to fit in the new memory budget.
      UINT Reserved : 29;
    };
    UINT Value;
  };
...
typedef struct D3DDDI_MAPGPUVIRTUALADDRESS {
  D3DKMT_HANDLE
  hPagingQueue; // in: Paging queue to synchronize the operation on.
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      BaseAddress; // in_opt: Base virtual address to map
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      MinimumAddress; // in_opt: Minimum virtual address
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      MaximumAddress;                         // in_opt: Maximum virtual address
  D3DKMT_HANDLE hAllocation;                  // in: Allocation handle to map
  D3DKMT_ALIGN64 D3DGPU_SIZE_T OffsetInPages; // in: Offset in 4 KB pages from
                                              // the start of the allocation
  D3DKMT_ALIGN64 D3DGPU_SIZE_T SizeInPages;   // in: Size in 4 KB pages to map
  D3DDDIGPUVIRTUALADDRESS_PROTECTION_TYPE
  Protection;                             // in: Virtual address protection
  ...
  D3DKMT_ALIGN64 UINT64
      PagingFenceValue; // out: Paging fence Id for synchronization
...
typedef struct D3DDDI_RESERVEGPUVIRTUALADDRESS {
  ...
  union {
    D3DDDIGPUVIRTUALADDRESS_RESERVATION_TYPE
    ReservationType; // in: Reservation type
    UINT Reserved0;  // M2
  };
  union {
    D3DKMT_ALIGN64 UINT64 DriverProtection; // in: Driver specific protection
...
typedef struct D3DDDI_UPDATEALLOCPROPERTY_FLAGS {
  union {
    struct {
      UINT AccessedPhysically
          : 1; // The new value for AccessedPhysically on an allocation
      UINT Unmoveable : 1; // Indicates an allocation cannot be moved while
                           // pinned in a memory segment
      UINT Reserved : 30;
    };
    UINT Value;
  };
...
typedef struct D3DDDI_UPDATEALLOCPROPERTY {
  ...
  UINT SupportedSegmentSet;   // [in] New supported segment set, ignored if the
                              // same.
  D3DDDI_SEGMENTPREFERENCE
  PreferredSegment; // [in] New preferred segment set, ignored if the same.
  D3DDDI_UPDATEALLOCPROPERTY_FLAGS
  Flags; // [in] Flags to set on the allocation, ignored if the same.
  D3DKMT_ALIGN64 UINT64
      PagingFenceValue; // [out] Paging fence value to synchronize on before
                        // using the above allocation.
                        //       This value applies to the monitored fence
                        //       synchronization object associated with
                        //       hPagingQueue.
  union {
    struct {
      UINT SetAccessedPhysically
          : 1; // [in] When set to 1, will set AccessedPhysically to new value
      UINT SetSupportedSegmentSet
          : 1; // [in] When set to 1, will set SupportedSegmentSet to new value
      UINT SetPreferredSegment
          : 1; // [in] When set to 1, will set PreferredSegment to new value
      UINT SetUnmoveable
          : 1; // [in] When set to 1, will set Unmoveable to new value
      UINT Reserved : 28;
    };
...
typedef struct _D3DDDI_SYNCHRONIZATIONOBJECTINFO {
  D3DDDI_SYNCHRONIZATIONOBJECT_TYPE
  Type; // in: Type of synchronization object to create.
  union {
    struct {
  ...
  union {
    struct {
     ...
     (D3D_UMD_INTERFACE_VERSION >= D3D_UMD_INTERFACE_VERSION_WDDM2_0))
      ...
      // When set, the sync object is signaled as soon as the contents of
      // command buffers preceding it is entirely copied to the GPU pipeline,
      // but not necessarily completed execution. This flag can be set in order
      // to start reusing command buffers as soon as possible.
      ...
      // When set, instructs the GPU scheduler to bypass signaling of the
      // monitored fence to the maximum value when the device is affected by the
      // GPU reset.
      UINT NoSignalMaxValueOnTdr : 1;

      // When set, the fence will not be mapped into the GPU virtual address
      // space. Only packet-based signal/wait operations are supported When this
      // is set, the fence is always stored as a 64-bit value (regardless of
      // adapter caps)
      UINT NoGPUAccess : 1;
      ...
      // waiters are unblocked when a local sync object is destroyed, but the
      // main shared sync object is still opened by another local sync object.
...
typedef struct _D3DDDI_SYNCHRONIZATIONOBJECTINFO2 {
  ...
  union {
    ...
    struct {
      D3DKMT_HANDLE
      hAdapter; // in: A handle to the adapter associated with VidPnTargetId
      D3DDDI_VIDEO_PRESENT_TARGET_ID
      VidPnTargetId; // in: The output that the compositor wishes to receive
                     // notifications for
      D3DKMT_ALIGN64 UINT64 Time; // in: Represents an offset before the VSync.
                                  // The Time value may not be longer than a
                                  // VSync interval. In units of 100ns.
      D3DKMT_PTR(VOID *,
                 FenceValueCPUVirtualAddress); // out: Read-only mapping of the
                                               // fence value for the CPU
      D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
          FenceValueGPUVirtualAddress; // out: Read-only mapping of the fence
                                       // value for the GPU
      UINT EngineAffinity; // in: Defines physical adapters where the GPU VA
                           // will be mapped
      UINT Padding;
  ...
  D3DKMT_HANDLE
  SharedHandle; // out: global shared handle (when requested to be shared)
...
typedef enum _D3DDDI_NATIVEFENCE_TYPE {
  ...
  D3DDDI_NATIVEFENCE_TYPE_INTRA_GPU =
      1, // Special fence type for engine-engine synchronization, which
...
typedef struct _D3DDDI_NATIVEFENCEMAPPING {
  D3DKMT_PTR(
      VOID *,
      CurrentValueCpuVa); // Read-only mapping of the current value for the CPU
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      CurrentValueGpuVa; // Read/write mapping of the current value for the GPU
                         // in the current process address space
  D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS
      MonitoredValueGpuVa; // Read/write mapping of the monitored value for the
                           // GPU in the current process address space
...
typedef struct _D3DDDI_NATIVEFENCEINFO {
  ...
  D3DDDI_SYNCHRONIZATIONOBJECT_FLAGS Flags; // in: Flags.
  D3DDDI_NATIVEFENCEMAPPING
  NativeFenceMapping; // out: process mapping information for the native
                      // fence
...
typedef struct _DXGK_NATIVE_FENCE_LOG_HEADER {
  union {
    struct {
      UINT32 FirstFreeEntryIndex; // same as LowPart of
                                  // AtomicWraparoundAndEntryIndex
      UINT32
      WraparoundCount; // same as HighPart of AtomicWraparoundAndEntryIndex
    };
...
typedef struct _DXGK_NATIVE_FENCE_LOG_ENTRY {
  ...
  D3DKMT_HANDLE hNativeFence; // UMD payload: user mode D3DKMT_HANDLE of native
                              // fence to which this operation belongs
  UINT OperationType;         // UMD payload: DXGK_FENCE_LOG_OPERATION type
  UINT64 Reserved0;           // Reserved for alignment
  UINT64
  FenceObservedGpuTimestamp;   // GPU Payload: OPERATION_WAIT_UNBLOCKED only:
                               // GPU Time at which an unresolved wait command
                               // was seen by engine and stalled the HWQueue
  UINT64 Reserved1;            // Reserved for alignment
  UINT64 FenceEndGpuTimestamp; // GPU Payload: GPU Time at which the fence
                               // operation completed on GPU
} DXGK_NATIVE_FENCE_LOG_ENTRY;

typedef struct _DXGK_NATIVE_FENCE_LOG_BUFFER {
  DXGK_NATIVE_FENCE_LOG_HEADER Header;

  _Field_size_(Header.NumberOfEntries) DXGK_NATIVE_FENCE_LOG_ENTRY Entries[1];
...
typedef struct _D3DDDI_WAITFORSYNCHRONIZATIONOBJECTFROMCPU_FLAGS {
  union {
    struct {
      UINT WaitAny : 1; // when waiting for multiple objects, signal the wait
                        // event if any of the wait array conditions is
                        // satisfied as opposed to all conditions.
      UINT Reserved : 31;
    };
    UINT Value;
  };
...
// PrivateDriverSize must be sizeof(D3DDDI_QUERYREGISTRY_INFO) + (size of the
// the key value in bytes)
//
typedef struct _D3DDDI_QUERYREGISTRY_INFO {
  ...
  ULONG OutputValueSize; // Out. Number of bytes written to the output value or
                         // required in case of
...
typedef enum _DXGK_DRIVER_FEATURE {
  ...
  DXGK_DRIVER_FEATURE_KMD_SIGNAL_CPU_EVENT =
      3, // Support for signaling CPU event by KMD
  ...
  DXGK_DRIVER_FEATURE_SYNC_PRESENT_RENDER_HWQ_ONLY =
      41, // Feature to avoid synchronizing present with other render queue
...
// Drivers that don't support the feature don't have to call the OS to query its
// status.
//
typedef enum _DXGK_FEATURE_ID {
  ...
  // Driver features
  //
  DXGK_FEATURE_HWSCH = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER,
                                              DXGK_DRIVER_FEATURE_HWSCH),
  DXGK_FEATURE_HWFLIPQUEUE = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_HWFLIPQUEUE),
  DXGK_FEATURE_LDA_GPUPV = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_LDA_GPUPV),
  DXGK_FEATURE_KMD_SIGNAL_CPU_EVENT = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_KMD_SIGNAL_CPU_EVENT),
  DXGK_FEATURE_USER_MODE_SUBMISSION = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_USER_MODE_SUBMISSION),
  ...
  DXGK_FEATURE_SAMPLE = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER,
                                               DXGK_DRIVER_FEATURE_SAMPLE),
  DXGK_FEATURE_PAGE_BASED_MEMORY_MANAGER =
      DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER,
                             DXGK_DRIVER_FEATURE_PAGE_BASED_MEMORY_MANAGER),
  DXGK_FEATURE_KERNEL_MODE_TESTING = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_KERNEL_MODE_TESTING),
  DXGK_FEATURE_64K_PT_DEMOTION_FIX = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_64K_PT_DEMOTION_FIX),
  DXGK_FEATURE_GPUPV_PRESENT_HWQUEUE = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_GPUPV_PRESENT_HWQUEUE),
  DXGK_FEATURE_GPUVAIOMMU = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_GPUVAIOMMU),
  DXGK_FEATURE_NATIVE_FENCE = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_NATIVE_FENCE),
  ...
  // DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER,
  // DXGK_DRIVER_FEATURE_NOTIFY_RESIDENCY2), DXGK_FEATURE_P2P_FENCE_SIGNAL =
  // DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER,
  // DXGK_DRIVER_FEATURE_P2P_FENCE_SIGNAL),
  // DXGK_FEATURE_PROCESS_DEBUG_BLOB_COLLECTION      =
  // DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER,
  // DXGK_DRIVER_FEATURE_PROCESS_DEBUG_BLOB_COLLECTION),
  DXGK_FEATURE_PANEL_BUFFER_CONTROL = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_PANEL_BUFFER_CONTROL),

  //
  // OS features
  //
  DXGK_FEATURE_QUERYSTATISTICS_EXTENSIONS = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_QUERYSTATISTICS_EXTENSIONS),
  DXGK_FEATURE_RESERVE_GPUVA_ZERO_BASE_ADDRESS =
      DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_OS,
                             DXGK_OS_FEATURE_RESERVE_GPUVA_ZERO_BASE_ADDRESS),
  DXGK_FEATURE_PER_PTE_PAGE_SIZE = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_PER_PTE_PAGE_SIZE),
  DXGK_FEATURE_FENCE_SIGNAL_FROM_SWS_NODE = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_FENCE_SIGNAL_FROM_SWS_NODE),
  DXGK_FEATURE_SUPPRESSVSYNC_INTERRUPTS = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_SUPPRESSVSYNC_INTERRUPTS),
  DXGK_FEATURE_FEATURE_INTERFACE_EXTENSIONS = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_FEATURE_INTERFACE_EXTENSIONS),
  DXGK_FEATURE_READONLY_EXISTINGSYSMEM = DXGK_DEFINE_FEATURE_ID(
      DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_READONLY_EXISTINGSYSMEM),
...
static_assert(sizeof(D3DDDI_TESTCOMMANDBUFFER) == 72,
              "D3DDDI_TESTCOMMANDBUFFER must be 72 bytes");
#endif
...
typedef struct _D3DDDI_DRIVERESCAPE_BUILDTESTCOMMANDBUFFER {
  ...
  D3DKMT_PTR(_Field_size_bytes_(DmaBufferSize) PVOID, pDmaBuffer);
  D3DKMT_PTR(_Field_size_bytes_(DmaBufferPrivateDataSize) PVOID,
