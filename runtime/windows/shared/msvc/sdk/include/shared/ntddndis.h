...
typedef struct _NDIS_STATISTICS_VALUE {
  NDIS_OID Oid;
  ULONG DataLength;
  _Field_size_bytes_(DataLength) UCHAR Data[1]; // variable length
...
#define NDIS_OBJECT_TYPE_DEFAULT                                               \
  0x80 // used when object type is implicit in the API call
#define NDIS_OBJECT_TYPE_MINIPORT_INIT_PARAMETERS                              \
  0x81 // used by NDIS in NDIS_MINIPORT_INIT_PARAMETERS
#define NDIS_OBJECT_TYPE_SG_DMA_DESCRIPTION                                    \
  0x83 // used by miniport drivers in NDIS_SG_DMA_DESCRIPTION
#define NDIS_OBJECT_TYPE_MINIPORT_INTERRUPT                                    \
  0x84 // used by miniport drivers in NDIS_MINIPORT_INTERRUPT_EX
#define NDIS_OBJECT_TYPE_DEVICE_OBJECT_ATTRIBUTES                              \
  0x85 // used by miniport or filter drivers in NDIS_DEVICE_OBJECT_ATTRIBUTES
#define NDIS_OBJECT_TYPE_BIND_PARAMETERS                                       \
  0x86 // used by NDIS in NDIS_BIND_PARAMETERS
#define NDIS_OBJECT_TYPE_OPEN_PARAMETERS                                       \
  0x87 // used by protocols in NDIS_OPEN_PARAMETERS
#define NDIS_OBJECT_TYPE_RSS_CAPABILITIES                                      \
  0x88 // used by miniport in NDIS_RECEIVE_SCALE_CAPABILITIES
#define NDIS_OBJECT_TYPE_RSS_PARAMETERS                                        \
  0x89 // used by miniport and protocol in NDIS_RECEIVE_SCALE_PARAMETERS
...
#define NDIS_OBJECT_TYPE_RSS_PARAMETERS_V2                                     \
  0xC8 // used by miniport and protocol in NDIS_RECEIVE_SCALE_PARAMETERS_V2
#define NDIS_OBJECT_TYPE_RSS_SET_INDIRECTION_ENTRIES                           \
  0xC9 // used by miniport and protocol in NDIS_RSS_SET_INDIRECTION_ENTRIES
...
typedef struct _NDIS_RSC_STATISTICS_INFO {
  ...
  ULONG64 CoalesceEvents;  // Total number of coalescing events (i.e. packets
                           // formed from coalescing)
...
//
// Bits used in Flags parameter of NDIS_INTERRUPT_MODERATION_PARAMETERS
// structure:
//
...
typedef struct _NDIS_PCI_DEVICE_CUSTOM_PROPERTIES {
  ...
  UINT32 CurrentSpeedAndMode; // PCI_DEVICE_CONVENTIONAL_xx or PCIX_MODExxx.
                              // valid only for conventional and PCI-X devices
  UINT32 CurrentPayloadSize;  // PCI_EXPRESS_MAX_PAYLOAD_SIZE, valid only for
                              // PCI-E devices
  UINT32 MaxPayloadSize; // PCI_EXPRESS_MAX_PAYLOAD_SIZE, valid only for PCI-E
                         // devices
  UINT32 MaxReadRequestSize; // PCI_EXPRESS_MAX_PAYLOAD_SIZE, valid only for
                             // PCI-E devices
  UINT32 CurrentLinkSpeed;   // PCI_EXPRESS_LINK_SPEED_xxx. valid only for PCI-E
                             // devices
  UINT32 CurrentLinkWidth;   // PCI_EXPRESS_LINK_WIDTH_xx. valid only for PCI-E
                             // devices
  UINT32
  MaxLinkSpeed; // PCI_EXPRESS_LINK_SPEED_xxx. valid only for PCI-E devices
  UINT32
  MaxLinkWidth; // PCI_EXPRESS_LINK_WIDTH_xx. valid only for PCI-E devices
#if ((NTDDI_VERSION >= NTDDI_WIN7) || NDIS_SUPPORT_NDIS620)
  UINT32 PciExpressVersion; // DevProp_PciExpressDevice_Spec_Version_xx, valid
                            // for PCI -E devices
...
#define OID_GEN_MEDIA_CONNECT_STATUS_EX                                        \
  0x0001028A                                  // used in querying interfaces
...
//
// more WWAN specific oids (may need to restrict to higher NDIS version such
// as 7.0)
//
...
// if it supports sending multiple packets, etc. refer to NDIS_MINIPORT_XXX
// flags
//
...
typedef struct _NDIS_802_11_KEY {
  ...
  _Field_size_bytes_(KeyLength) UCHAR
      KeyMaterial[1]; // variable length depending on above field
...
typedef struct NDIS_CO_DEVICE_PROFILE {
  ...
  ULONG ulMaxNumActiveCalls; // (This * ulMinRate) = total bandwidth (which may
                             // equal ulMaxRate)
...
//
//  Protocol types supported by ndis. These values need to be consistent with
//  ADDRESS_TYPE_XXX defined in TDI.H
//
...
//
// The following is used with OID_GEN_TRANSPORT_HEADER_OFFSET to indicate the
// length of the layer-2 header for packets sent by a particular protocol.
//
typedef struct _TRANSPORT_HEADER_OFFSET {
  USHORT ProtocolType; // The protocol that is sending this OID
                       // (NDIS_PROTOCOL_ID_XXX above)
  USHORT HeaderOffset; // The header offset
} TRANSPORT_HEADER_OFFSET, *PTRANSPORT_HEADER_OFFSET;

//
// The structures below need to be consistent with TRANSPORT_ADDRESS structures
// in TDI.H
//
...
//
// The following is set on a per-packet basis as OOB data with
// NdisClassIrdaPacketInfo This is the per-packet info specified on a per-packet
// basis
//
...
#if (NDIS_SUPPORT_NDIS650)
#define NDIS_SIZEOF_OFFLOAD_PARAMETERS_REVISION_4                              \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD_PARAMETERS,                            \
                           EncapsulationProtocolParameters)
#endif // (NDIS_SUPPORT_NDIS650)

#if (NDIS_SUPPORT_NDIS683)
#define NDIS_SIZEOF_OFFLOAD_PARAMETERS_REVISION_5                              \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD_PARAMETERS, UdpSegmentation)
#endif // (NDIS_SUPPORT_NDIS683)

#if (NDIS_SUPPORT_NDIS689)
#define NDIS_SIZEOF_OFFLOAD_PARAMETERS_REVISION_6                              \
...
typedef struct _NDIS_IPSEC_OFFLOAD_V2 {
  ...
  BOOLEAN IPv6NonIPsecExtensionHeaders; // Supports offload of packets with non
...
typedef enum _NDIS_RFC6877_464XLAT_OFFLOAD_OPTIONS {
  NDIS_RFC6877_464XLAT_OFFLOAD_NOT_SUPPORTED =
      0, // incapable of 464XLAT hardware offload
...
#if (NDIS_SUPPORT_NDIS650)
#define NDIS_SIZEOF_NDIS_OFFLOAD_REVISION_4                                    \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD, EncapsulationTypes)
#endif // (NDIS_SUPPORT_NDIS650)

#if (NDIS_SUPPORT_NDIS670)
#define NDIS_SIZEOF_NDIS_OFFLOAD_REVISION_5                                    \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD, Rfc6877Xlat)
#endif // (NDIS_SUPPORT_NDIS670)

#if (NDIS_SUPPORT_NDIS683)
#define NDIS_SIZEOF_NDIS_OFFLOAD_REVISION_6                                    \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD, UdpSegmentation)
#endif // (NDIS_SUPPORT_NDIS683)

#if (NDIS_SUPPORT_NDIS685)
#define NDIS_SIZEOF_NDIS_OFFLOAD_REVISION_7                                    \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_OFFLOAD, UdpSegmentation)
#endif // (NDIS_SUPPORT_NDIS685)

#if (NDIS_SUPPORT_NDIS689)
#define NDIS_SIZEOF_NDIS_OFFLOAD_REVISION_8                                    \
...
#if (NDIS_SUPPORT_NDIS683)
#define NDIS_SIZEOF_NDIS_WMI_OFFLOAD_REVISION_4                                \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_WMI_OFFLOAD, UdpSegmentation)
#endif // (NDIS_SUPPORT_NDIS683)

#if (NDIS_SUPPORT_NDIS689)
#define NDIS_SIZEOF_NDIS_WMI_OFFLOAD_REVISION_5                                \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_WMI_OFFLOAD, UdpRsc)
#endif // (NDIS_SUPPORT_NDIS689)
...
//
// NDIS_PM_CAPABILITIES structure is used in PowerManagementCapabilitiesEx field
// of NDIS_MINIPORT_ADAPTER_GENERAL_ATTRIBUTES and NDIS_BIND_PARAMETERS
//
...
typedef struct _NDIS_PM_CAPABILITIES {
  ...
  ULONG
  MaxWoLPatternSize; // maximum bytes that can be compared against a pattern
  ULONG MaxWoLPatternOffset; // strting from MAC header, how many bytes in the
                             // packet can be examined
  ULONG MaxWoLPacketSaveBuffer; // how many bytes of WOL packet can be saved to
                                // a buffer and indicated up
  ULONG
  SupportedProtocolOffloads; // NDIS_PM_PROTOCOL_OFFLOAD_XXX_SUPPORTED flags
  ...
  ULONG
  MediaSpecificWakeUpEvents; // NDIS_{WLAN|WWAN}_WAKE_ON_XXX_SUPPORTED flags
...
typedef struct _NDIS_PM_WOL_PATTERN {
  ...
  union _WOL_PATTERN {
    ...
    struct _WOL_BITMAP_PATTERN {
      ULONG Flags;
      ULONG MaskOffset;    // offset for mask buffer from the beginning of
                           // NDIS_PM_WOL_PATTERN structure
      ULONG MaskSize;      // Mask size
      ULONG PatternOffset; // offset for pattern buffer from the beginning of
                           // NDIS_PM_WOL_PATTERN structure
...
typedef struct _NDIS_PM_PROTOCOL_OFFLOAD {
  ...
  union _PROTOCOL_OFFLOAD_PARAMETERS {
    ...
    // flags determine link local or global address. Discovery, reachability or
    // dup address detection multicat MAC address: 3333WXYZ where WXYZ is the
    // least significant 4 bytes from the solicited node IPv6 address
    //
...
#if (NDIS_SUPPORT_NDIS684)
    //
    // KCK and KEK keys for drivers that support GTK offload- V2 is for WDI
    // drivers only
    //
...
//
// The size of the NDIS_PM_PROTOCOL_OFFLOAD struct for Rev1 was struct-aligned
// based on the union ProtocolOffloadParameters. However any macro that tries to
// recalculate this size from the new struct based on the size of the largest
// sub-struct in the older union (IPv6NSParameters) does not match the size of
// the overall struct, so its size is different from the original value. Hence
// we have to hard-code the size based on the value from the original struct.
//
...
#define NDIS_SIZEOF_NDIS_PM_PROTOCOL_OFFLOAD_REVISION_2                        \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_PM_PROTOCOL_OFFLOAD,                           \
                           ProtocolOffloadParameters) // 256 bytes
#endif                                                // (NDIS_SUPPORT_NDIS684)
...
#define NDIS_RECEIVE_FILTER_DYNAMIC_PROCESSOR_AFFINITY_CHANGE_FOR_DEFAULT_QUEUE_SUPPORTED \
  0x00000040
...
// OID_RECEIVE_FILTER_HARDWARE_CAPABILITIES  and
...
#define NDIS_NIC_SWITCH_CAPS_RSS_PER_PF_VPORT_INDIRECTION_TABLE_SUPPORTED      \
  0x00000100
...
#define NDIS_NIC_SWITCH_CAPS_RSS_PER_PF_VPORT_INDIRECTION_TABLE_SIZE_RESTRICTED \
  0x00001000
...
// Also used in NicSwitchCapabilities field of NDIS_BIND_PARAMETERS and
// NDIS_FILTER_ATTACH_PARAMETERS structure Also used in
// NDIS_STATUS_NIC_SWITCH_CURRENT_CAPABILITIES and
...
typedef _When_(FieldParametersArrayNumElements == 0,
               _Struct_size_bytes_(sizeof(NDIS_RECEIVE_FILTER_PARAMETERS)))
    _When_(FieldParametersArrayNumElements > 0,
           _Struct_size_bytes_(FieldParametersArrayOffset +
                               FieldParametersArrayNumElements *
                                   FieldParametersArrayElementSize)) struct
...
// that enumerates receive filters on a queue or a VPort. Each element in the
// array is an NDIS_RECEIVE_FILTER_INFO structure
//
...
typedef _When_(NumElements >= 1, _Struct_size_bytes_(FirstElementOffset +
                                                     NumElements * ElementSize))
    _When_(NumElements == 0,
           _Struct_size_bytes_(
               sizeof(NDIS_RECEIVE_QUEUE_ALLOCATION_COMPLETE_ARRAY))) struct
    _NDIS_RECEIVE_QUEUE_ALLOCATION_COMPLETE_ARRAY {
  NDIS_OBJECT_HEADER Header;
  ULONG Flags;
  ULONG FirstElementOffset; // offset to an array of
                            // NDIS_RECEIVE_QUEUE_ALLOCATION_COMPLETE_PARAMETERS
  ULONG NumElements;
  ULONG ElementSize;
...
//
// What kind the receive scale capabilities the miniport can support, miniport
// drivers return some of these flags as CapabilitiesFlags in a structure
// _NDIS_RECEIVE_SCALE_CAPABILITIES when they get a query about theire receive
// side scale capabilities.
//
...
#endif // (NDIS_SUPPORT_NDIS680)
//
// Typedef to use as flags holder to correlate to the NDIS_RSS_CAPS_ prefixed
// flags above.
//
typedef ULONG NDIS_RSS_CAPS_FLAGS;

//
// the following structure defines the Receive scale capabilities of the
// miniport
//
...
typedef struct _NDIS_RECEIVE_SCALE_PARAMETERS {
  ...
  ULONG NumberOfProcessorMasks;  // Array of type GROUP_AFFINITY representing
                                 // procs used in the indirection table
...
//
#define NDIS_RSS_INDIRECTION_TABLE_MAX_SIZE_REVISION_3                         \
  (128 * sizeof(PROCESSOR_NUMBER))
...
C_ASSERT(sizeof(NDIS_SYSTEM_PROCESSOR_INFO_EX) %
             __alignof(NDIS_PROCESSOR_INFO_EX) ==
         0);
...
#define NDIS_NDK_PERFORMANCE_COUNTER_MASK(CounterFieldName)                    \
  (1ui64 << (FIELD_OFFSET(NDIS_NDK_PERFORMANCE_COUNTERS, CounterFieldName) /   \
             sizeof(ULONG64)))
...
typedef struct _NDIS_NDK_CAPABILITIES {
  ...
  // system-wide management/partitioning of resouces available on a given NDK
  // adapter.
  //
  ...
  // Maximum number of incoming outstanding read requests supported by the
  // adapter If this field is 0, there's no adapter-wide limit. There is still a
  // limit per queue pair, which is indicated by the NdkInfo.MaxInboundReadLimit
  // field.
  ULONG MaxInboundReadLimit;

  // Maximum number of outgoing outstanding read requests supported by the
  // adapter If this field is 0, there's no adapter-wide limit. There is still a
  // limit per queue pair, which is indicated by the
  ...
  // a provider cannot support a counter due to extreme implementation
  // difficulties, it must indicate so via the following field by OR'ing the
  // mask value for the counter which it cannot support.
  ULONG64 MissingCounterMask;

  //
  // Following field points to a structure that denotes NDK adapter capabilities
  // and limits that are relevant for an individual NDKPI client (in contrast to
  // system-wide limits).
  //
...
typedef struct _NDIS_NDK_CONNECTION_ENTRY {
  ...
  BOOLEAN UserModeOwner; // TRUE if NDSPI (user-mode) connection, FALSE if NDKPI
                         // (kernel-mode)
...
// This structure is variable-sized based on the actual number of connections
// being returned. The actual size of the connection array (as element count) is
// indicated by the Count field. If the RDMA technology for the NDK provider
// requires the provider to map ND connections to TCP connections, then the NDK
// provider must also report the underlying TCP connection 4-tuple for each ND
// connection as follows: set the NDConnectionsMappedtoTCPConnections field to
// TRUE, and use two consecutive entries for each ND connection where the first
// entry contains the ND addressing information (local & remote IP address/ND
// port number) and the immediate next entry contains the corresponding TCP
// connection's addressing information (local & remote IP address/TCP port
// number). Fields other than addressing information (UserModeOwner, OwnerPid)
// must be set only in the first entry, and left untouched in the next entry.
// So, entries at index 0, 2, 4, ... will contain ND addressing info (plus
// properly filled-in other fields) and entries at index 1, 3, 5, ... will
// contain TCP addressing info (where other fields are left untouched) when
...
typedef struct _NDIS_NDK_CONNECTIONS {
  ...
  // Header.Size = (USHORT)min(MAXUSHORT,
  // NDIS_SIZEOF_NDK_CONNECTIONS_REVISION_1(n));
  //
  NDIS_OBJECT_HEADER Header;
  ULONG Flags; // reserved, must be set to 0
  ULONG Count;
...
typedef struct _NDIS_NDK_LOCAL_ENDPOINT_ENTRY {
  SOCKADDR_INET Local;   // Local IP address and port
  BOOLEAN UserModeOwner; // TRUE if NDSPI (user-mode) listener, FALSE if NDKPI
                         // (kernel-mode)
  BOOLEAN Listener; // TRUE if a listener, FALSE if a SharedEndpoint (used for
                    // connections)
...
// This structure is variable-sized based on the actual number of local
// endpoints being returned. The actual size of the local endpoint array (as
// element count) is indicated by the Count field. If the RDMA technology for
// the NDK provider requires the provider to map ND local endpoints to TCP local
// endpoints, then the NDK provider must also report the underlying TCP local
// endpoint for each ND local endpoint as follows: set the
// NDLocalEndpointsMappedtoTCPLocalEndpoints field to TRUE, and use two
// consecutive entries for each ND local endpoint where the first entry contains
// the ND addressing information (local IP address/ND port number) and the
// immediate next entry contains the corresponding TCP local endpoint's
// addressing information (local IP address/TCP port number). Fields other than
// addressing information (UserModeOwner, Listener, OwnerPid) must be set only
// in the first entry, and left untouched in the next entry. So, entries at
// index 0, 2, 4, ... will contain ND addressing info (plus properly filled-in
// other fields) and entries at index 1, 3, 5, ... will contain TCP addressing
// info (where other fields are left untouched) when
...
typedef struct _NDIS_NDK_LOCAL_ENDPOINTS {
  ...
  // Header.Size = (USHORT)min(MAXUSHORT,
  // NDIS_SIZEOF_NDK_LOCAL_ENDPOINTS_REVISION_1(n));
  //
  NDIS_OBJECT_HEADER Header;
  ULONG Flags; // reserved, must be set to 0
  ULONG Count;
...
//
// NDIS_QOS_CAPABILITIES is used as the return result of the
// OID_QOS_CURRENT_CAPABILITIES and OID_QOS_HARDWARE_CAPABILITIES OID queries.
// It is also specified as the HardwareQosCapabilities and
// CurrentQosCapabilities fields in the
// NDIS_MINIPORT_ADAPTER_HARDWARE_ASSIST_ATTRIBUTES structure.
//
typedef _Struct_size_bytes_(Header.Size) struct _NDIS_QOS_CAPABILITIES {
  ...
  _In_ _Field_range_(<=,
                     MaxNumTrafficClasses) ULONG MaxNumEtsCapableTrafficClasses;
  _In_ _Field_range_(<=,
                     MaxNumTrafficClasses) ULONG MaxNumPfcEnabledTrafficClasses;
...
//
#define NDIS_QOS_CLASSIFICATION_SET_BY_MINIPORT_MASK                           \
  0xFF000000 // Mask of flags set by miniport only
#define NDIS_QOS_CLASSIFICATION_ENFORCED_BY_MINIPORT                           \
  0x01000000 // Set by miniport
...
typedef _Struct_size_bytes_(
    Header.Size) struct _NDIS_QOS_CLASSIFICATION_ELEMENT {
  //
  ...
  _In_ _Field_range_(<, NDIS_QOS_CONDITION_MAXIMUM) USHORT ConditionSelector;
  _In_ USHORT ConditionField;

  _In_ _Field_range_(<, NDIS_QOS_ACTION_MAXIMUM) USHORT ActionSelector;
...
//
#define NDIS_QOS_PARAMETERS_ETS_CHANGED                                        \
  0x00000001 // Used in OID_QOS_PARAMETERS,
             // optional in NDIS_STATUS_QOS_XXX_PARAMETERS_CHANGE
#define NDIS_QOS_PARAMETERS_ETS_CONFIGURED 0x00000002
#define NDIS_QOS_PARAMETERS_PFC_CHANGED                                        \
  0x00000100 // Used in OID_QOS_PARAMETERS,
             // optional in NDIS_STATUS_QOS_XXX_PARAMETERS_CHANGE
#define NDIS_QOS_PARAMETERS_PFC_CONFIGURED 0x00000200
#define NDIS_QOS_PARAMETERS_CLASSIFICATION_CHANGED                             \
  0x00010000 // Used in OID_QOS_PARAMETERS,
             // optional in NDIS_STATUS_QOS_XXX_PARAMETERS_CHANGE
...
//
// The following values are used in NDIS_QOS_PARAMETERS.TsaAssignmentTable
// field.
//
...
typedef _Struct_size_bytes_(Header.Size) struct _NDIS_QOS_PARAMETERS {
  ...
  // Header.Size = NDIS_SIZEOF_QOS_PARAMETERS_REVISION_1 +
  // sizeof_all_classification_entries
  //
  _In_ NDIS_OBJECT_HEADER Header;
  _In_ ULONG Flags;
  ...
  //
  _In_ _Field_range_(0,
                     NDIS_QOS_MAXIMUM_TRAFFIC_CLASSES) ULONG NumTrafficClasses;
  ...
  //
  _In_ _Field_range_(0,
                     (1ul << NDIS_QOS_MAXIMUM_PRIORITIES) - 1) ULONG PfcEnable;
...
//
#define NDIS_NIC_SWITCH_PARAMETERS_DEFAULT_NUMBER_OF_QUEUE_PAIRS_FOR_DEFAULT_VPORT \
  1

//
// This structure is used in OID_NIC_SWITCH_CREATE_SWITCH and
// OID_NIC_SWITCH_PARAMETERS OIDs
//
...
// NicSwitches field of NDIS_BIND_PARAMETERS and NDIS_FILTER_ATTACH_PARAMETERS
// structures. Each element in the array is of type NDIS_NIC_SWITCH_INFO
// structure
//
...
//
// This structure is used in OID_SRIOV_CURRENT_CAPABILITIES and
// OID_SRIOV_HARDWARE_CAPABILITIES. It is also used in the
// CurrentSriovCapabilities and HardwareSriovCapabilities fields of
// NDIS_MINIPORT_ADAPTER_HARDWARE_ASSIST_ATTRIBUTES structure .
// It is also used in SriovCapabilities field of NDIS_BIND_PARAMETERS and
...
typedef NDIS_ISOLATION_NAME NDIS_ROUTING_DOMAIN_NAME,
    *PNDIS_ROUTING_DOMAIN_NAME;
...
//
// Retrieves the first NDIS_ROUTING_DOMAIN_ISOLATION_ENTRY object in the
// Isolation info.
//
#define NDIS_ROUTING_DOMAIN_ENTRY_GET_FIRST_ISOLATION_ENTRY(                   \
...
//
#define NDIS_ISOLATION_PARAMETERS_GET_FIRST_ROUTING_DOMAIN_ENTRY(              \
    _MultiTenancyInfo_)                                                        \
...
// Isolation info.
//
#define NDIS_SWITCH_PORT_PROPERTY_ROUTING_DOMAIN_GET_FIRST_ISOLATION_ENTRY(    \
...
#define NDIS_SWITCH_PORT_PROPERTY_CUSTOM_GET_BUFFER(_PortPropertyCustom_)      \
  ((PVOID)((PUCHAR)(_PortPropertyCustom_) +                                    \
           (_PortPropertyCustom_)->PropertyBufferOffset))
...
//
// Returns the property described by the NDIS_SWITCH_PORT_PROPERTY_PARAMETERS
// structure.
//
#define NDIS_SWITCH_PORT_PROPERTY_PARAMETERS_GET_PROPERTY(_PortParameters_)    \
  ((PVOID)((PUCHAR)(_PortParameters_) +                                        \
           (_PortParameters_)->PropertyBufferOffset))

//
// Converts the version defined from the property MOF files (e.g. "1.0") to the
// serialized value passed to vSwitch extensions (e.g. 0x100).
//
...
//
// Retrieves the first NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO object in the
// enumeration.
//
#define NDIS_SWITCH_PORT_PROPERTY_ENUM_PARAMETERS_GET_FIRST_INFO(              \
    _PortEnumParams_)                                                          \
...
// to be 8 byte aligned.
//
C_ASSERT((sizeof(NDIS_SWITCH_PORT_PROPERTY_ENUM_PARAMETERS) % sizeof(UINT64)) ==
         0);
...
//
// Retrieves the next NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO object in the
// enumeration.
//
#define NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO_GET_NEXT(_PortEnumInfo_)               \
  ((PNDIS_SWITCH_PORT_PROPERTY_ENUM_INFO)((ULONG_PTR)(_PortEnumInfo_) +            \
                                          (_PortEnumInfo_)                         \
...
//
// Returns the property described by the NDIS_SWITCH_PORT_PROPERTY_ENUM_INFO
// entry.
//
...
// to be 8 byte aligned.
//
C_ASSERT((sizeof(NDIS_SWITCH_PORT_FEATURE_STATUS_PARAMETERS) %
          sizeof(UINT64)) == 0);
...
// serialized value passed to vSwitch extensions (e.g. 0x100).
//
#define NDIS_SWITCH_CREATE_FEATURE_STATUS_VERSION(_VersionMajor_,              \
                                                  _VersionMinor_)              \
  (((_VersionMajor_) << 8) + (_VersionMinor_))
...
#define NDIS_SWITCH_PROPERTY_CUSTOM_GET_BUFFER(_SwitchPropertyCustom_)         \
  ((PVOID)((PUCHAR)(_SwitchPropertyCustom_) +                                  \
           (_SwitchPropertyCustom_)->PropertyBufferOffset))
...
//
// Returns the property described by the NNDIS_SWITCH_PROPERTY_PARAMETERS
// structure.
//
#define NDIS_SWITCH_PROPERTY_PARAMETERS_GET_PROPERTY(_SwitchParameters_)       \
  ((PVOID)((PUCHAR)(_SwitchParameters_) +                                      \
           (_SwitchParameters_)->PropertyBufferOffset))
...
#define NDIS_SWITCH_PROPERTY_ENUM_INFO_GET_NEXT(_SwitchEnumInfo_)              \
  ((PNDIS_SWITCH_PROPERTY_ENUM_INFO)((ULONG_PTR)(_SwitchEnumInfo_) +           \
                                     (_SwitchEnumInfo_)                        \
                                         ->QwordAlignedPropertyBufferLength +  \
...
#define NDIS_SWITCH_PROPERTY_ENUM_INFO_GET_PROPERTY(_SwitchEnumInfo_)          \
  ((PVOID)((PUCHAR)(_SwitchEnumInfo_) +                                        \
           (_SwitchEnumInfo_)->PropertyBufferOffset))
...
//
#define NDIS_SWITCH_PROPERTY_ENUM_PARAMETERS_GET_FIRST_INFO(                   \
    _SwitchEnumParams_)                                                        \
...
//
// Since the feature status buffer may need 8 byte alignment, we require this
...
typedef struct _NDIS_SWITCH_NIC_PARAMETERS {
  ...
  // to the device friendly name. For Nics of type
  // SwitchNicTypeExternalInternal, this corresponds to the device display name.
  // For other Nics the value is set by the management layer.
  //
...
typedef struct _NDIS_SWITCH_NIC_SAVE_STATE {
  ...
  NDIS_SWITCH_NIC_INDEX NicIndex;
  //
  // Identifier for extension, used by extension on restore to recognize its
  // data.
  //
  GUID ExtensionId;
  //
...
//
// flags used to specify fields in the Ethernet, IP, Transport and encapsulation
// headers
//
...
// in SupportedEncapsulationTypes field of NDIS_GFT_OFFLOAD_CAPABILITIES
// structure
//
...
typedef struct DECLSPEC_ALIGN(8) _NDIS_GFP_HEADER_GROUP_EXACT_MATCH_PROFILE {
  ...
  ULONG64 MatchFields;
    ...
    *PNDIS_GFP_HEADER_GROUP_EXACT_MATCH_PROFILE;
...
//
// This structure is used to fully define a match profile that can span multiple
// header groups
//
...
typedef struct DECLSPEC_ALIGN(8) _NDIS_GFP_EXACT_MATCH_PROFILE {
  ...
  // NDIS_GFP_HEADER_GROUP_EXACT_MATCH_PROFILE structures
  //
  _In_ ULONG HeaderGroupExactMatchProfileArrayOffset; //  from the beginning of
                                                      //  this structure
...
} NDIS_GFP_WILDCARD_MATCH_TYPE,
    *PNDIS_GFP_WILDCARD_MATCH_TYPE;
...
typedef struct DECLSPEC_ALIGN(8) _NDIS_GFP_HEADER_GROUP_WILDCARD_MATCH_PROFILE {
  ...
  UINT8 TcpFlags;
    ...
    *PNDIS_GFP_HEADER_GROUP_WILDCARD_MATCH_PROFILE;
...
// OID_GFT_WILDCARD_MATCH_PROFILE and NDIS_GFT_WILDCARD_MATCH_FLOW_ENTRY.
//
// It uses an array of NDIS_GFP_HEADER_GROUP_WILDCARD_MATCH_PROFILEs in order to
// describe a full wildcard match profile
//
...
typedef struct DECLSPEC_ALIGN(8) _NDIS_GFP_WILDCARD_MATCH_PROFILE {
  ...
  //
  // The following fields are used to describe an array of
  // NDIS_GFP_HEADER_GROUP_WILDCARD_MATCH_PROFILE structures
  //
  _In_ ULONG HeaderGroupWildcardMatchProfileArrayOffset; // from the beginning
                                                         // of this structure
...
typedef struct _NDIS_GFP_IPV4_SRC_AND_DEST_ADDRESS_WILDCARD_MATCH {
  ...
  NDIS_GFP_IPV4_ADDRESS_WILDCARD_MATCH DestinationIPv4Address;
    ...
    *PNDIS_GFP_IPV4_SRC_AND_DEST_ADDRESS_WILDCARD_MATCH;
...
typedef struct _NDIS_GFP_IPV6_SRC_AND_DEST_ADDRESS_WILDCARD_MATCH {
  ...
  NDIS_GFP_IPV6_ADDRESS_WILDCARD_MATCH DestinationIPv6Address;
    ...
    *PNDIS_GFP_IPV6_SRC_AND_DEST_ADDRESS_WILDCARD_MATCH;
...
} NDIS_GFP_IP_SRC_AND_DEST_ADDRESS_WILDCARD_MATCH,
    *PNDIS_GFP_IP_SRC_AND_DEST_ADDRESS_WILDCARD_MATCH;

typedef struct _NDIS_GFP_TRANSPORT_PORT_WILDCARD_MATCH {
  NDIS_GFP_WILDCARD_MATCH_TYPE MatchType;
  USHORT Port;
  USHORT Range;
} NDIS_GFP_TRANSPORT_PORT_WILDCARD_MATCH,
    *PNDIS_GFP_TRANSPORT_PORT_WILDCARD_MATCH;
...
} NDIS_GFP_TRANSPORT_SRC_AND_DEST_PORT_WILDCARD_MATCH,
    *PNDIS_GFP_TRANSPORT_SRC_AND_DEST_PORT_WILDCARD_MATCH;
...
// NDIS_PD_CAPS_NOTIFICATION_MODERATION_INTERVAL_SUPPORTED is set (i.e., there's
// no moderation support based on purely count).
//
...
#define NDIS_SIZEOF_PD_CAPABILITIES_REVISION_1                                 \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_PD_CAPABILITIES, MaxTxQueueSize)
...
typedef struct _NDIS_GFT_TABLE_INFO_ARRAY {
  ...
  //
  // The following fields are used to describe an array of NDIS_GFT_TABLE_INFO
  // structures
  //
...
// NDIS_GFT_EXACT_MATCH_FLOW_ENTRY and NDIS_GFT_WILDCARD_MATCH_FLOW_ENTRY
// structures used in OID_GFT_ADD_FLOW_ENTRIES
//
...
} NDIS_GFT_COUNTER_UPDATE_FREQUENCY,
    *PNDIS_GFT_COUNTER_UPDATE_FREQUENCY;
...
} NDIS_GFT_COUNTER_TYPE,
    *PNDIS_GFT_COUNTER_TYPE;

typedef volatile struct _NDIS_GFT_PACKET_COUNTER_VALUE {
  ULONG64 Packets;
  LARGE_INTEGER
  LastUpdate; // Absolute system time returned from KeQuerySystemTime
...
} NDIS_GFT_PACKET_BYTE_COUNTER_VALUE_AND_STATE,
    *PNDIS_GFT_PACKET_BYTE_COUNTER_VALUE_AND_STATE;
...
typedef struct _NDIS_GFT_COUNTER_PARAMETERS {
  ...
  union {
    ...
    _Inout_ PNDIS_GFT_PACKET_BYTE_COUNTER_VALUE_AND_STATE
        PacketByteCountersAndState;
...
typedef struct _NDIS_GFT_FREE_COUNTER_PARAMETERS {
  ...
  // Last counter values
  // The following fields are used to describe an array of
  // NDIS_GFT_COUNTER_VALUE
  //
  _Out_ ULONG LastCounterValueAndStateArrayOffset; //  from the beginning of
                                                   //  this structure
...
#define NDIS_SIZEOF_GFT_FREE_COUNTER_PARAMETERS_REVISION_1                     \
  RTL_SIZEOF_THROUGH_FIELD(NDIS_GFT_FREE_COUNTER_PARAMETERS,                   \
                           LastCounterValueAndStateArrayElementSize)
...
} NDIS_GFT_COUNTER_VALUE_QUERY_METHOD,
    *PNDIS_GFT_COUNTER_VALUE_QUERY_METHOD;
...
} NDIS_GFT_HEADER_GROUP_TRANSPOSITION_ACTION,
    *PNDIS_GFT_HEADER_GROUP_TRANSPOSITION_ACTION;

//
// flags used in Flags fields of NDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE
//
#define NDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE_DECREMENT_TTL_IF_NOT_ONE   \
  0x00000001

//
// NDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE defines a header transposition
// profile for a single header group
//
...
typedef struct _NDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE {
  ...
  ULONG HeaderFields;
    ...
    *PNDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE;
...
// NDIS_GFT_HEADER_TRANSPOSITION_PROFILE structure in order to describe a custom
// action profile. Custom actions profiles follow a TLV (Type Length Value)
// format to allow linking multiple custom action profiles.
//
...
typedef struct _NDIS_GFT_CUSTOM_ACTION_PROFILE {
  ...
  union {
    ULONG_PTR Alignment;
    _Field_size_bytes_(Length) UCHAR ActionProfileData[1];
  };
...
typedef struct _NDIS_GFT_HEADER_TRANSPOSITION_PROFILE {
  ...
  //
  // The following fields are used to describe an array of
  // NDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE structures
  //
  ULONG HeaderGroupTranspositionProfileArrayOffset; //  from the beginning of
                                                    //  this structure
  ...
  ULONG CustomActionProfileOffset;
    ...
    *PNDIS_GFT_HEADER_TRANSPOSITION_PROFILE;
...
//
// NDIS_GFT_FLOW_ENTRY_CACHE_HINT is used to provide a hint to the GFT offload
// provider so it can use its resources more efficiently
//
...
} NDIS_GFT_FLOW_ENTRY_CACHE_HINT,
    *PNDIS_GFT_FLOW_ENTRY_CACHE_HINT;
...
} NDIS_GFT_FLOW_ENTRY_STATE,
    *PNDIS_GFT_FLOW_ENTRY_STATE;
...
typedef struct _NDIS_GFT_CUSTOM_ACTION {
  ...
  union {
    ULONG_PTR Alignment;
    _Field_size_bytes_(Length) UCHAR ActionData[1];
  };
...
#define NDIS_GFT_EMFE_REDIRECT_TO_INGRESS_QUEUE_OF_VPORT_IF_TTL_IS_ONE         \
  0x00004000
...
typedef struct _NDIS_GFT_EXACT_MATCH_FLOW_ENTRY {
  ...
  union {
    ...
    _Inout_ PNDIS_GFT_PACKET_BYTE_COUNTER_VALUE_AND_STATE
        PacketByteCounterAndStateAddress;
  } CounterValueBuffer;
  ...
  _In_ NDIS_GFT_COUNTER_ID
      CounterIdArray[NDIS_GFT_MAX_COUNTER_OBJECTS_PER_FLOW_ENTRY];
  ...
  //
  // The following fields are used to describe an array of
  // NDIS_GFP_HEADER_GROUP_EXACT_MATCH structures if a match profile ID is
  // specified in MatchProfileId, then these fields must be consistent with the
  // specified profile ID
  //
  _In_ ULONG
      HeaderGroupExactMatchArrayOffset; //  from the beginning of this structure
  _In_ ULONG HeaderGroupExactMatchArrayNumElements;
  _In_ ULONG HeaderGroupExactMatchArrayElementSize;

  //
  // The following fields are used to describe an array of
  // NDIS_GFT_HEADER_GROUP_TRANSPOSITION structures if a header transposition
  // profile ID is specified in HeaderTranspositionProfileId, then these fields
  // must be consistent with the specified profile ID
  //
  _In_ ULONG HeaderGroupTranspositionArrayOffset; //  from the beginning of this
                                                  //  structure
...
} NDIS_GFT_WILDCARD_ACTION,
    *PNDIS_GFT_WILDCARD_ACTION;
...
#define NDIS_GFT_WCFE_REDIRECT_TO_INGRESS_QUEUE_OF_VPORT_IF_TTL_IS_ONE         \
  0x00000008
...
typedef struct _NDIS_GFT_WILDCARD_MATCH_FLOW_ENTRY {
  ...
  //
  // The following fields are used to describe an array of
  // NDIS_GFP_HEADER_GROUP_WILDCARD_MATCH structures if a match profile ID is
  // specified in MatchProfileId, then these fields must be consistent with the
  // specified profile ID
  //
  _In_ ULONG HeaderGroupWildcardMatchArrayOffset; //  from the beginning of this
                                                  //  structure
...
} NDIS_GFT_PROFILE_TYPE,
    *PNDIS_GFT_PROFILE_TYPE;
...
typedef struct _NDIS_GFT_PROFILE_INFO {
  ...
  // The following fields are used to describe an array of
  // NDIS_GFP_HEADER_GROUP_EXACT_MATCH_PROFILE,
  // NDIS_GFT_HEADER_GROUP_TRANSPOSITION_PROFILE, or
  // NDIS_GFP_HEADER_GROUP_WILDCARD_MATCH_PROFILE structures
  //
  // The value of ProfileType field above determines the type of profile the
  // structure describes
  //
...
} NDIS_GFT_FLOW_ENTRY_TYPE,
    *PNDIS_GFT_FLOW_ENTRY_TYPE;
...
typedef struct _NDIS_GFT_FLOW_ENTRY_ARRAY {
  ...
  //
  // The following fields are used to describe an array of
  // NDIS_GFT_EXACT_MATCH_FLOW_ENTRY or NDIS_GFT_WILDCARD_MATCH_FLOW_ENTRY
  // structures. The table type of the table identified by TableId field above
  // determines the type of the flow entries that are added.
  //
...
typedef struct _NDIS_GFT_FLOW_ENTRY_INFO_ARRAY {
  ...
  // NDIS_GFT_EXACT_MATCH_FLOW_ENTRY or NDIS_GFT_WILDCARD_MATCH_FLOW_ENTRY
  // structures. The table type of the table identified by TableId field above
  // determines the type of the flows that are enumerated.
  //
...
//
// NDIS_GFT_FLOW_ENTRY_ID_ARRAY is used in OID_GFT_DELETE_FLOW_ENTRIES,
// OID_GFT_ACTIVATE_FLOW_ENTRIES and OID_GFT_DEACTIVATE_FLOW_ENTRIES
//
...
typedef struct _NDIS_GFT_FLOW_ENTRY_ID_ARRAY {
  ...
  _In_ union {
    struct {
      //
      // The following fields are used to describe an array of
      // NDIS_GFT_FLOW_ENTRY_IDs
      //
      ULONG ProviderFlowEntryIdArrayOffset; //  from the beginning of this
                                            //  structure
    ...
    struct {
      //
      // The following fields are used to describe an array of
      // NDIS_GFT_FLOW_ENTRY_ID_AND_COUNTERs
      //
      ULONG ProviderFlowEntryIdAndCounterArrayOffset; //  from the beginning of
                                                      //  this structure
...
#define NDIS_GFT_OFFLOAD_CAPS_REDIRECT_TO_INGRESS_QUEUE_OF_VPORT_IF_TTL_IS_ONE \
  0x00000040
#define NDIS_GFT_OFFLOAD_CAPS_REDIRECT_TO_EGRESS_QUEUE_OF_VPORT_IF_TTL_IS_ONE  \
  0x00000080
...
//
// NDIS_GFT_OFFLOAD_CAPABILITIES structure advertises the GFT offload
// capabilities of the miniport adapter. This structure is used in
...
typedef struct _NDIS_GFT_OFFLOAD_CAPABILITIES {
  ...
  //
  // Number of discrete counter objects. does not include per flow entry
  // counters that the provider might support.
  //
...
typedef struct _NDIS_GFT_VPORT_PARAMETERS {
  ...
  //
  // on query, sampling rate is an out parameter and returns the current
  // sampling rate on set, sampling rate is an "in" parameter 0 means no
  // sampling. Any value "n" other than 0 means one sample for every "n" packet
  //
  ...
  _In_ NDIS_GFT_COUNTER_ID
      DscpMaskCounterIdArray[NDIS_GFT_VPORT_MAX_DSCP_MASK_COUNTER_OBJECTS];

  _In_ ULONG64 PriorityMask;
  _In_ ULONG NumPriorityMaskCounterObjects;
  _In_ NDIS_GFT_COUNTER_ID PriorityMaskCounterIdArray
...
#if (NDIS_SUPPORT_NDIS684)
typedef struct _NDIS_QOS_SQ_PARAMETERS_ENUM_ARRAY NDIS_QOS_SQ_ARRAY,
    *PNDIS_QOS_SQ_ARRAY;
