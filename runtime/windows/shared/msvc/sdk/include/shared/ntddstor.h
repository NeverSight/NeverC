...
//   #include <ntddstor.h> in that source file a second time to instantiate the
//   GUIDs
//
#ifdef DEFINE_GUID
//
...
//
// GUID, identifying crash dump section, containing device information or driver
// information. These GUIDs are used both to identify secondary dump section in
// the crashdump and in IOCTL to identify why section is requested by the user
// mode application
//
...
// end_winioctl
#define IOCTL_STORAGE_MANAGE_BYPASS_IO                                         \
  CTL_CODE(IOCTL_STORAGE_BASE, 0x0230, METHOD_BUFFERED, FILE_ANY_ACCESS)
// begin_winioctl
...
typedef struct _DEVICE_LB_PROVISIONING_DESCRIPTOR {
  ...
  ULONG MaxUnmapLbaCount; // Max LBAs that can be unmapped in a single UNMAP
                          // command, in logical blocks.

  ULONG MaxUnmapBlockDescriptorCount; // Max number of descriptors allowed in a
...
typedef struct _DEVICE_POWER_DESCRIPTOR {
  ...
  BOOLEAN NoVerifyDuringIdlePower; // Device require no verification during idle
                                   // power transitions.
  UCHAR Reserved[2];
  ULONG IdleTimeoutInMS; // The idle timeout value in milliseconds. Only valid
...
typedef struct _STORAGE_CRYPTO_DESCRIPTOR_V2 {
  ...
  //       curCryptoCapability =
  //          (STORAGE_CRYPTO_CAPABILITY_V2*)((PUCHAR)curCryptoCapability +
...
typedef struct _STORAGE_HW_CRYPTO_DESCRIPTOR {
  ...
  _Field_range_(sizeof(struct _STORAGE_HW_CRYPTO_DESCRIPTOR),
                Header.Size) ULONG OffsetToCryptoCapabilities;
...
const STORAGE_HW_CRYPTO_CAPABILITY *GetStorageHwCryptoCapability(
    const STORAGE_HW_CRYPTO_DESCRIPTOR *CryptoDescriptor, ULONG Index) {
...
typedef struct _STORAGE_PROTOCOL_SPECIFIC_DATA {
  ...
  ULONG DataType; // The value will be protocol specific, as defined in
  ...
  ULONG ProtocolDataOffset; // The offset of data buffer is from beginning of
                            // this data structure.
...
typedef struct _STORAGE_TEMPERATURE_DATA_DESCRIPTOR {
  ...
  USHORT InfoCount; // Some devices may report more than one temperature
...
typedef struct _STORAGE_PHYSICAL_NODE_DATA {
  ...
  ULONG AdapterDataOffset; // Offset from beginning of this data structure. The
                           // buffer contains an array of
                           // STORAGE_PHYSICAL_ADAPTER_DATA.

  ULONG DeviceCount; // >= 1
  ULONG DeviceDataLength;
  ULONG DeviceDataOffset; // Offset from beginning of this data structure. The
                          // buffer contains an array of
...
typedef struct _STORAGE_PHYSICAL_TOPOLOGY_DESCRIPTOR {
  ...
  ULONG Size;    // Total size of the data. Should be >=
...
typedef struct _STORAGE_HW_ENDURANCE_INFO {
  ULONG ValidFields; // ValidFields represents bit mapping of valid fields of
...
DeviceDsmParameterBlock(_In_ PDEVICE_DSM_INPUT Input) {
  return (PVOID)((ULONG_PTR)Input + Input->ParameterBlockOffset);
}

FORCEINLINE
PDEVICE_DSM_RANGE
DeviceDsmDataSetRanges(_In_ PDEVICE_DSM_INPUT Input) {
  return (PDEVICE_DSM_RANGE)((ULONG_PTR)Input + Input->DataSetRangesOffset);
}
...
DeviceDsmAddDataSetRange(_Inout_updates_bytes_(InputLength)
                         ...
                         _In_ _In_range_(>=, sizeof(DEVICE_DSM_INPUT))
                             ULONG InputLength,
                         _In_ LONGLONG Offset, _In_ ULONGLONG Length) {
  ULONG Bytes = 0;
  ULONG Index = 0;
...
DeviceDsmOutputBlock(_In_ PDEVICE_DSM_OUTPUT Output) {
  return (PVOID)((ULONG_PTR)Output + Output->OutputBlockOffset);
}
...
DeviceDsmValidateOutputLength(_In_ PDEVICE_DSM_DEFINITION Definition,
  ...
  ULONG Bytes = DeviceDsmGetOutputLength(Definition, 0);
...
#define MAX_FW_BUCKET_ID_LENGTH                                                \
  132 // 128 (ACS specification + 1 for zero termination + 3 to align on ULONG)
...
typedef struct _DEVICEDUMP_SUBSECTION_POINTER {
  ...
  ULONG dwOffset; // Offset (in bytes) of the subsection block from the start of
                  // the buffer
...
typedef struct _STORAGE_HW_FIRMWARE_INFO_QUERY {
  ...
  ULONG Size;    // Whole size of the buffer (in case this data structure being
...
typedef struct _STORAGE_HW_FIRMWARE_DOWNLOAD {
  ...
  ULONGLONG Offset; // Image file offset, should be aligned to
                    // "ImagePayloadAlignment" value from STORAGE_FIRMWARE_INFO.
  ULONGLONG BufferSize; // should be multiple of "ImagePayloadAlignment" value
...
typedef struct _STORAGE_PROTOCOL_COMMAND {
  ...
  ULONG DataToDeviceTransferLength; // optional, can be zero. Used by WRITE type
                                    // of request.
  ULONG DataFromDeviceTransferLength; // optional, can be zero. Used by READ
                                      // type of request.
  ...
  ULONG FixedProtocolReturnData; // return data, optional. Some protocol, such
...
// The supported features is defined in "StorageSupportedFeatures" as a DWORD
// value under "Parameters" registry key of the driver
//

//
// Bypass IO feature. If set, the driver understands bypass IO and supports
// bypass IO related IOCTL(s).
//
#define STORAGE_SUPPORTED_FEATURES_BYPASS_IO 0x00000001

//
// Supported Features Mask. This is "OR" of all defined bits of supported
// features.
//
...
typedef enum _BPIO_OPERATIONS {
  ...
  //  longer support BypassIO, they can call <TBD> to inform the stack below
  //  them
  ...
  //
  //  This is a request to disable BypassIO for the given volume/disk.  It
  //  allows
  ...
  //  the given volume/disk.  This should be processed the same as an ENABLE
  //  operation
...
typedef struct _BPIO_RESULTS {
  ...
  //  ENABLE/QUERY requests.  The name should match the actual name of the
  //  driver used by the system with extension.  Ex: "ntfs.sys".
  //
...
//
//  Defines the variable sized OUTPUT structure for
//  IOCTL_STORAGE_MANAGE_BYPASS_IO
//
...
#define BPIO_OUTPUT_VOLUME_DISABLE_SIZE                                        \
  ((size_t)FIELD_OFFSET(BPIO_OUTPUT, Enable))
