...
Abstract:
    ...
    This is the include file that defines common constants, types and control
codes related to persistent memory, also known as storage-class memory (SCM).

    There are three types of persistent memory device objects.

    Physical persistent memory devices are the actual hardware that enable
persistent memory to work. In Windows, their corresponding device objects can be
used for health monitoring, firmware upgrade, secure erase etc., but they can't
be used to read or write data. IOCTLs sent to physical persistent memory
devices, or types that represent information about them, have the abbreviation
"SCM_PD".

    Logical persistent memory devices are formed by combining one or more
physical devices into a logical object that can perform reads and writes. A
logical persistent memory device corresponds to a disk, and the usual disk
interfaces should be used to query its health, do I/O and so on. The
abbreviation that identifies IOCTLs and structures directed to logical
persistent memory devices is "SCM_LD".

    Finally, the SCM bus driver is responsible for enumerating all physical and
logical persistent memory devices on the system. The IOCTLs and structures
related to it have "SCM_BUS" in their names.
...
//   #include <ntddscm.h> in that source file a second time to instantiate the
//   GUIDs
//
#ifdef DEFINE_GUID
...
typedef struct _SCM_PD_DEVICE_INFO {
  ...
  ULONG SerialNumber4Byte; // 4-byte serial number as defined in the JEDEC SPD
