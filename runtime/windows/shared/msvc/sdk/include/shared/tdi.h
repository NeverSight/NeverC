...
#define TDI_ADDRESS_TYPE_NETBIOS_UNICODE_EX                                    \
  ((USHORT)24) // WCHAR Netbios address
...
typedef struct _TDI_CONNECTION_INFORMATION {
  ...
  _Field_size_bytes_(
      UserDataLength) PVOID UserData; // pointer to user data buffer
  LONG OptionsLength;                 // length of follwoing buffer
  _Field_size_bytes_(
      OptionsLength) PVOID Options; // pointer to buffer containing options
  LONG RemoteAddressLength;         // length of following buffer
  _Field_size_bytes_(RemoteAddressLength) PVOID
      RemoteAddress; // buffer containing the remote address
...
#define TDI_EVENT_RECEIVE_DATAGRAM                                             \
  ((USHORT)4) // TDI_IND_RECEIVE_DATAGRAM event handler.
#define TDI_EVENT_RECEIVE_EXPEDITED                                            \
  ((USHORT)5) // TDI_IND_RECEIVE_EXPEDITED event handler.
#define TDI_EVENT_SEND_POSSIBLE                                                \
  ((USHORT)6) // TDI_IND_SEND_POSSIBLE event handler
...
// some providers, Disconnect allows a "graceful" disconnect, causing new
// activity to be rejected but allowing old activity to run down to completion.
//
...
#define TDI_RECEIVE_ENTIRE_MESSAGE                                             \
  0x00000400 // opposite of RECEIVE_PARTIAL
             //  (for kernel-mode indications)
#define TDI_RECEIVE_AT_DISPATCH_LEVEL                                          \
  0x00000800                                // receive indication called
                                            //  at dispatch level
...
#define TDI_SEND_NO_RESPONSE_EXPECTED                                          \
  ((USHORT)0x0080) // HINT: no back traffic expected.
#define TDI_SEND_NON_BLOCKING                                                  \
  ((USHORT)0x0100) // don't block if no buffer space in protocol
...
#define TDI_DISCONNECT_WAIT                                                    \
  ((USHORT)0x0001) // used for disconnect
                   //   notification
#define TDI_DISCONNECT_ABORT                                                   \
  ((USHORT)0x0002) // immediately terminate
                   //   connection
#define TDI_DISCONNECT_RELEASE                                                 \
  ((USHORT)0x0004) // initiate graceful
                   //   disconnect
...
#define TDI_SERVICE_CONNECTIONLESS_MODE                                        \
  0x00000004 // connectionless mode supported.
#define TDI_SERVICE_ERROR_FREE_DELIVERY                                        \
  0x00000008                                  // error free delivery supported.
#define TDI_SERVICE_SECURITY_LEVEL 0x00000010 // security wrapper supported.
#define TDI_SERVICE_BROADCAST_SUPPORTED                                        \
  0x00000020 // broadcast datagrams supported.
#define TDI_SERVICE_MULTICAST_SUPPORTED                                        \
  0x00000040 // multicast datagrams supported.
#define TDI_SERVICE_DELAYED_ACCEPTANCE                                         \
  0x00000080 // use of TDI_ACCEPT_OR_REJECT is supported.
#define TDI_SERVICE_EXPEDITED_DATA 0x00000100 // expedited data supported.
#define TDI_SERVICE_INTERNAL_BUFFERING                                         \
  0x00000200 // protocol does internal buffering
#define TDI_SERVICE_ROUTE_DIRECTED                                             \
  0x00000400 // directed packets may go further than MC.
#define TDI_SERVICE_NO_ZERO_LENGTH 0x00000800 // zero-length sends NOT supported
#define TDI_SERVICE_POINT_TO_POINT                                             \
  0x00001000 // transport is functioning as a RAS gateway
#define TDI_SERVICE_MESSAGE_MODE 0x00002000 // message-mode send supported
#define TDI_SERVICE_HALF_DUPLEX                                                \
  0x00004000 // data can be received after local disc
#define TDI_SERVICE_DGRAM_CONNECTION                                           \
  0x00008000 // Pseudo connection for datagrams to handle
             // GPC, QOS etc.,
#define TDI_SERVICE_FORCE_ACCESS_CHECK                                         \
  0x00010000 // kernel mode caller should force access
             // check when opening trasnport objects
             // if it passes the handle to user mode
#define TDI_SERVICE_SEND_AND_DISCONNECT                                        \
  0x00020000 // combines send and disconnect processing
#define TDI_SERVICE_DIRECT_ACCEPT                                              \
  0x00040000 // (deprecated) completes accept-requests directly
#define TDI_SERVICE_ACCEPT_LOCAL_ADDR                                          \
  0x00080000 // supplies local address
             // with accept-completion
#define TDI_SERVICE_ADDRESS_SECURITY                                           \
  0x00100000 // supports security descriptor assignment to
...
#define TDI_SERVICE_NO_PUSH                                                    \
  0x00400000 // can complete receives after
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Success_(return == STATUS_SUCCESS)
    _At_(Buffer, _Out_writes_bytes_(sizeof(FILE_FULL_EA_INFORMATION) +
                                    TDI_TRANSPORT_ADDRESS_LENGTH +
                                    sizeof(TA_NETBIOS_ADDRESS))) NTSTATUS
    TdiOpenNetbiosAddress(_Out_ PHANDLE FileHandle, PUCHAR Buffer,
                          _In_ PVOID DeviceName, _In_opt_ PVOID Name);
...
// NdisAllocatePacketPoolEx api. Ndis uses this as pooltag for allocating packet
// pools for that transport.
