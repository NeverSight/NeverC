...
typedef VOID(CALLBACK *TCI_NOTIFY_HANDLER)(_In_ HANDLE ClRegCtx,
                                           _In_ HANDLE ClIfcCtx,
                                           _In_ ULONG Event, // See list below
                                           _In_ HANDLE SubCode,
                                           _In_ ULONG BufSize,
                                           _In_reads_bytes_(BufSize)
                                               PVOID Buffer);

typedef VOID(CALLBACK *TCI_ADD_FLOW_COMPLETE_HANDLER)(_In_ HANDLE ClFlowCtx,
                                                      _In_ ULONG Status);

typedef VOID(CALLBACK *TCI_MOD_FLOW_COMPLETE_HANDLER)(_In_ HANDLE ClFlowCtx,
                                                      _In_ ULONG Status);

typedef VOID(CALLBACK *TCI_DEL_FLOW_COMPLETE_HANDLER)(_In_ HANDLE ClFlowCtx,
                                                      _In_ ULONG Status);
...
typedef struct _TC_GEN_FILTER {
  ...
  _Field_size_bytes_(
      PatternSize) PVOID Pattern; // specific format, e.g. IP_PATTERN
...
ULONG
APIENTRY
TcRegisterClient(_In_ ULONG TciVersion, _In_ HANDLE ClRegCtx,
                 _In_ PTCI_CLIENT_FUNC_LIST ClientHandlerList,
                 _Out_ PHANDLE pClientHandle);

ULONG
APIENTRY
TcEnumerateInterfaces(_In_ HANDLE ClientHandle, _Inout_ PULONG pBufferSize,
                      _Out_ PTC_IFC_DESCRIPTOR InterfaceBuffer);

ULONG
APIENTRY
TcOpenInterfaceA(_In_ LPSTR pInterfaceName, _In_ HANDLE ClientHandle,
                 _In_ HANDLE ClIfcCtx, _Out_ PHANDLE pIfcHandle);

ULONG
APIENTRY
TcOpenInterfaceW(_In_ LPWSTR pInterfaceName, _In_ HANDLE ClientHandle,
                 _In_ HANDLE ClIfcCtx, _Out_ PHANDLE pIfcHandle);

ULONG
APIENTRY
TcCloseInterface(_In_ HANDLE IfcHandle);

ULONG
APIENTRY
TcQueryInterface(_In_ HANDLE IfcHandle, _In_ LPGUID pGuidParam,
                 _In_ BOOLEAN NotifyChange, _Inout_ PULONG pBufferSize,
                 _Out_writes_bytes_(*pBufferSize) PVOID Buffer);

ULONG
APIENTRY
TcSetInterface(_In_ HANDLE IfcHandle, _In_ LPGUID pGuidParam,
               _In_ ULONG BufferSize,
               _In_reads_bytes_(BufferSize) PVOID Buffer);

ULONG
APIENTRY
TcQueryFlowA(_In_ LPSTR pFlowName, _In_ LPGUID pGuidParam,
             _Inout_ PULONG pBufferSize,
             _Out_writes_bytes_(*pBufferSize) PVOID Buffer);

ULONG
APIENTRY
TcQueryFlowW(_In_ LPWSTR pFlowName, _In_ LPGUID pGuidParam,
             _Inout_ PULONG pBufferSize,
             _Out_writes_bytes_(*pBufferSize) PVOID Buffer);

ULONG
APIENTRY
TcSetFlowA(_In_ LPSTR pFlowName, _In_ LPGUID pGuidParam, _In_ ULONG BufferSize,
           _In_reads_bytes_(BufferSize) PVOID Buffer);

ULONG
APIENTRY
TcSetFlowW(_In_ LPWSTR pFlowName, _In_ LPGUID pGuidParam, _In_ ULONG BufferSize,
           _In_reads_bytes_(BufferSize) PVOID Buffer);

ULONG
APIENTRY
TcAddFlow(_In_ HANDLE IfcHandle, _In_ HANDLE ClFlowCtx, _In_ ULONG Flags,
          _In_ PTC_GEN_FLOW pGenericFlow, _Out_ PHANDLE pFlowHandle);

ULONG
APIENTRY
TcGetFlowNameA(_In_ HANDLE FlowHandle, _In_ ULONG StrSize,
               _Out_writes_(StrSize) LPSTR pFlowName);

ULONG
APIENTRY
TcGetFlowNameW(_In_ HANDLE FlowHandle, _In_ ULONG StrSize,
               _Out_writes_(StrSize) LPWSTR pFlowName);

ULONG
APIENTRY
TcModifyFlow(_In_ HANDLE FlowHandle, _In_ PTC_GEN_FLOW pGenericFlow);

ULONG
APIENTRY
TcAddFilter(_In_ HANDLE FlowHandle, _In_ PTC_GEN_FILTER pGenericFilter,
            _Out_ PHANDLE pFilterHandle);

ULONG
APIENTRY
TcDeregisterClient(_In_ HANDLE ClientHandle);

ULONG
APIENTRY
TcDeleteFlow(_In_ HANDLE FlowHandle);

ULONG
APIENTRY
TcDeleteFilter(_In_ HANDLE FilterHandle);

ULONG
APIENTRY
TcEnumerateFlows(_In_ HANDLE IfcHandle, _Inout_ PHANDLE pEnumHandle,
                 _Inout_ PULONG pFlowCount, _Inout_ PULONG pBufSize,
