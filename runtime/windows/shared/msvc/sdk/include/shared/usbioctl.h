...
/*
   ...
   All previously defined USB Ioctls use FILE_DEVICE_USB (aka
   FILE_DEVICE_UNKNOWN).
*/
...
#define IOCTL_INTERNAL_USB_FAIL_GET_STATUS_FROM_DEVICE                         \
  CTL_CODE(FILE_DEVICE_USB, USB_FAIL_GET_STATUS, METHOD_NEITHER,               \
           FILE_ANY_ACCESS)
...
#define IOCTL_USB_GET_TRANSPORT_CHARACTERISTICS                                \
  CTL_CODE(FILE_DEVICE_USB, USB_GET_TRANSPORT_CHARACTERISTICS,                 \
           METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_USB_NOTIFY_ON_TRANSPORT_CHARACTERISTICS_CHANGE                   \
  CTL_CODE(FILE_DEVICE_USB, USB_NOTIFY_ON_TRANSPORT_CHARACTERISTICS_CHANGE,    \
           METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_USB_GET_FRAME_NUMBER_AND_QPC_FOR_TIME_SYNC                       \
  CTL_CODE(FILE_DEVICE_USB, USB_GET_FRAME_NUMBER_AND_QPC_FOR_TIME_SYNC,        \
           METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_USB_GET_DEVICE_CHARACTERISTICS                                   \
  CTL_CODE(FILE_DEVICE_USB, USB_GET_DEVICE_CHARACTERISTICS, METHOD_BUFFERED,   \
           FILE_ANY_ACCESS)
...
typedef struct _USB_DEVICE_PERFORMANCE_INFO {
  ...
  // Interrupt BW is reported by endpoint period in AllocedInterrupt[n] where
  // period = 2^n The total interrupt BW allocated for this device is the sum of
  // the BW for all periods
  //
  ...
  // Number of ISO packets that were not scheduled or processed by the
  // controller
  ...
  // Async (EHCI) Cache stats -- incremented each time we flush the async cache
  // (doorbell).
  ...
  // Periodic (EHCI) Cache stats -- incremented each time we flush the periodic
  // prefetch cache.
...
typedef struct _USB_PORT_CONNECTOR_PROPERTIES {
  ...
  // The number of bytes required to hold the entire
  // USB_PORT_CONNECTOR_PROPERTIES structure, including the full
...
typedef struct _USB_NODE_CONNECTION_INFORMATION_EX_V2 {
  ...
  // On input a bitmask that indicates which USB protocols are understood by the
  // caller On output a bitmask that indicates which USB signaling protocols are
