...
RpcAsyncRegisterInfo(_In_ PRPC_ASYNC_STATE pAsync);
#endif

RPCRTAPI
_Must_inspect_result_ RPC_STATUS RPC_ENTRY RpcAsyncInitializeHandle(
    _Out_writes_bytes_(Size) PRPC_ASYNC_STATE pAsync, _In_ unsigned int Size);

RPCRTAPI
_Must_inspect_result_ RPC_STATUS RPC_ENTRY
RpcAsyncGetCallStatus(_In_ PRPC_ASYNC_STATE pAsync);

RPCRTAPI
_Must_inspect_result_ RPC_STATUS RPC_ENTRY
RpcAsyncCompleteCall(_Inout_ PRPC_ASYNC_STATE pAsync, _Out_opt_ void *Reply);

RPCRTAPI
_Must_inspect_result_ RPC_STATUS RPC_ENTRY RpcAsyncAbortCall(
    _Inout_ PRPC_ASYNC_STATE pAsync, _In_ unsigned long ExceptionCode);

RPCRTAPI
_Must_inspect_result_ RPC_STATUS RPC_ENTRY
RpcAsyncCancelCall(_Inout_ PRPC_ASYNC_STATE pAsync, _In_ BOOL fAbort);
...
RPC_STATUS
RPC_ENTRY
RpcErrorStartEnumeration(_Out_ RPC_ERROR_ENUM_HANDLE *EnumHandle);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcErrorGetNextRecord(_In_ RPC_ERROR_ENUM_HANDLE *EnumHandle,
...
RPC_STATUS
RPC_ENTRY
RpcErrorEndEnumeration(_Inout_ RPC_ERROR_ENUM_HANDLE *EnumHandle);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcErrorResetEnumeration(_Inout_ RPC_ERROR_ENUM_HANDLE *EnumHandle);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcErrorGetNumberOfRecords(_In_ RPC_ERROR_ENUM_HANDLE *EnumHandle,
                           _Out_ int *Records);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcErrorSaveErrorInfo(_In_ RPC_ERROR_ENUM_HANDLE *EnumHandle,
...
RPC_STATUS
RPC_ENTRY
RpcErrorLoadErrorInfo(_In_reads_bytes_(BlobSize) PVOID ErrorBlob,
...
RPC_STATUS
RPC_ENTRY
RpcErrorAddRecord(_In_ RPC_EXTENDED_ERROR_INFO *ErrorInfo);
...
RPC_STATUS
RPC_ENTRY
RpcAsyncCleanupThread(_In_ DWORD dwTimeout);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcGetAuthorizationContextForClient(_In_opt_ RPC_BINDING_HANDLE ClientBinding,
                                    _In_ BOOL ImpersonateOnReturn,
                                    _In_opt_ PVOID Reserved1,
                                    _In_opt_ PLARGE_INTEGER pExpirationTime,
                                    _In_ LUID Reserved2, _In_ DWORD Reserved3,
...
RPC_STATUS
RPC_ENTRY
RpcFreeAuthorizationContext(_Inout_ PVOID *pAuthzClientContext);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcSsContextLockExclusive(_In_opt_ RPC_BINDING_HANDLE ServerBindingHandle,
                          _In_ PVOID UserContext);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcSsContextLockShared(_In_ RPC_BINDING_HANDLE ServerBindingHandle,
                       _In_ PVOID UserContext);
...
typedef struct tagRPC_CALL_ATTRIBUTES_V2_W {
  ...
  *CallLocalAddress; // one of the CallLocalAddress flavors
...
RPC_STATUS
RPC_ENTRY
RpcServerInqCallAttributesW(_In_opt_ RPC_BINDING_HANDLE ClientBinding,
                            _Inout_ void *RpcCallAttributes);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcServerInqCallAttributesA(_In_opt_ RPC_BINDING_HANDLE ClientBinding,
                            _Inout_ void *RpcCallAttributes);
...
RpcServerSubscribeForNotification(
    _In_opt_ RPC_BINDING_HANDLE Binding, _In_ RPC_NOTIFICATIONS Notification,
...
RPC_STATUS
RPC_ENTRY
RpcServerUnsubscribeForNotification(_In_opt_ RPC_BINDING_HANDLE Binding,
...
RPC_STATUS
RPC_ENTRY
RpcBindingBind(_In_opt_ PRPC_ASYNC_STATE pAsync,
               _In_ RPC_BINDING_HANDLE Binding, _In_ RPC_IF_HANDLE IfSpec);

RPCRTAPI
RPC_STATUS
RPC_ENTRY
RpcBindingUnbind(_In_ RPC_BINDING_HANDLE Binding);
...
//
_Must_inspect_result_ RPC_STATUS RPC_ENTRY
I_RpcAsyncSetHandle(_In_ PRPC_MESSAGE Message, _In_ PRPC_ASYNC_STATE pAsync);

_Must_inspect_result_ RPC_STATUS RPC_ENTRY I_RpcAsyncAbortCall(
    _In_ PRPC_ASYNC_STATE pAsync, _In_ unsigned long ExceptionCode);

#if (NTDDI_VERSION >= NTDDI_WINXP)
int RPC_ENTRY I_RpcExceptionFilter(_In_ unsigned long ExceptionCode);
#endif // (NTDDI_VERSION >= NTDDI_WINXP)
...
RPC_STATUS
RPC_ENTRY
I_RpcBindingInqClientTokenAttributes(_In_ RPC_BINDING_HANDLE Binding,
