...
// It used to align declspec(align(4)) on x86 and declspec(align(8)) on x64/ia64
// but all structures that used it contained a pointer so they were already 4/8
// aligned.
//
...
//      KDF_GENERIC_PARAMETER = KDF_ALGORITHMID || KDF_PARTYUINFO ||
...
//      required KDF_SUPPPUBINFO, KDF_SUPPPRIVINFO are optional
...
#define BCRYPT_PKCS11_RSA_AES_WRAP_BLOB_MAGIC                                  \
  0x57504152 // 'RAPW' for RSA-AES-PAD-WRAP (PKCS11-RSA-AES-WRAP)
typedef struct _BCRYPT_PKCS11_RSA_AES_WRAP_BLOB {
  ...
  ULONG cbPaddingAlgId; // Number of bytes in OAEP Padding algorithm per
                        // OAEPParams in PKCS#11 specification
  ULONG cbPaddingLabel; // Number of bytes in OAEP Padding label per OAEPParams
                        // in PKCS#11 specification
  // UCHAR Key[cbKey];                   -- PKCS#11 binary blob
  // UCHAR PaddingAlgId[cbPaddingAlgId]; -- OAEP Padding information for PKCS#11
  // unwrapping UCHAR PaddingLabel[cbPaddingLabel]; -- OAEP Padding information
  // for PKCS#11 unwrapping
...
#define BCRYPT_GENERATE_IV                                                     \
  0x00000020 // BCryptGenerateSymmetricKey BCryptEncrypt
#endif

// RSA padding schemes
#define BCRYPT_PAD_NONE 0x00000001
#define BCRYPT_PAD_PKCS1                                                       \
  0x00000002 // BCryptEncrypt/Decrypt BCryptSignHash/VerifySignature
...
typedef struct _BCRYPT_ECCFULLKEY_BLOB {
  ...
  dwCurveGenerationAlgId; // For X.592 verification purposes, if we include
                          // Seed we will need to include the algorithm ID.
  ...
  // A[cbFieldLength]              Coefficient A of the equation y^2 = x^3 + A*x
  // + B mod p B[cbFieldLength]              Coefficient B of the equation y^2 =
  // x^3 + A*x + B mod p Gx[cbFieldLength]             X-coordinate of the base
  // point. Gy[cbFieldLength]             Y-coordinate of the base point.
...
typedef struct BCRYPT_PQDSA_KEY_BLOB {
  ...
  // WCHAR parameterSet[cbParameterSet / sizeof(WCHAR)];  // For ML-DSA and
  // SLH-DSA, \0-terminated BYTE key[cbKey]; // Key material
...
typedef struct BCRYPT_MLKEM_KEY_BLOB {
  ...
  ULONG cbParameterSet; // Byte size of parameterSet[]
  ULONG cbKey;          // Byte size of key[]
  // WCHAR parameterSet[cbParameterSet / sizeof(WCHAR)];  // For ML-KEM,
  // \0-terminated BYTE key[cbKey];                                     // Key
  // material
...
typedef enum {
  BCRYPT_OPERATION_TYPE_HASH =
      1, // structure type is BCRYPT_MULTI_HASH_OPERATION
} BCRYPT_MULTI_OPERATION_TYPE;

typedef struct _BCRYPT_MULTI_OBJECT_LENGTH_STRUCT {
  ULONG cbPerObject;
  ULONG cbPerElement; // required size for N elements is (cbPerObject + N *
                      // cbPerElement)
...
//
// Extendable-output functions (XOFs) allow generating output multiple times
// from their state. BCRYPT_HASH_DONT_RESET_FLAG allows to override the default
// behavior of BCryptFinishHash, which is to reset the hash state. If this flag
// is set, the hash state is not reset and users may invoke BCryptFinishHash to
// generate more data out of the hash/XOF state, until after a BCryptFinishHash
// call where this flag is unset.
//
#if (NTDDI_VERSION >= NTDDI_WIN11_ZN)
...
//
// The TLS_CBC_HMAC_VERIFY flag provides a side-channel safe way of verifying
// TLS data records from the CBC-HMAC cipher suites. See RFC 5246
// section 6.2.3.2. This flag is used in BCryptOpenAlgorithmProvider and in
// BCryptHashData. For BCryptOpenAlgorithmProvider it ensures that you get a
// provider that supports this feature. For BCryptHashData is changes the
// functionality. The Input buffer now contains the whole TLS data record,
// consisting of the plaintext, followed by the MAC value, followed by the
// padding, followed by the padding_length. The function will compute the HMAC
// over the data already hashed plus the plaintext, compare it to the MAC value,
// and verify that the padding is correct. If all works out, it returns a
// success value; if anything fails it returns an error. What makes this special
// is that the code path or the memory access pattern used to do this
// verification does not depend on padding_length to stop attacks on the CBC
// encryption that was used to decrypt this data. This flag is only useful for
// TLS implementations, other callers should not use it. This flag is only valid
// for HMAC-SHA1, HMAC-SHA256, and HMAC-SHA384. This flag implies the
// BCRYPT_HASH_REUSABLE_FLAG.
//
// This flag is available staring in Windows 10 19H1, but we define it for all
// NTDDI values to allow applications to dynamically test wethere the OS
// supports the feature, and adjust accordingly.
//
...
// for that algorithm. Some callers depend on this feature and pass in large
// buffers. This makes it impossible to silently extend the supported key size
// without breaking backward compatibility. This flag indicates that the
// extended key size support is requested. It has the following consequences:
...
//
// ENABLE_INCOMPATIBLE_FIPS_CHECKS flag enables some FIPS 140-2-mandated checks
// that are incompatible with the original algorithm.
//
...
// - BCryptGenerateSymmetricKey when generating an XTS-AES key with this flag
// specified and FIPS mode enabled
//      will verify that the two halves of the key are not identical. If they
//      are, an error is returned. This is actually incompatible with the NIST
//      SP 800-38E and IEEE Std 1619-2007 definitions of XTS-AES. Rather than
//      change the standard, NIST added this requirement in the FIPS 140-2
//      implementation guidance.
//      This check breaks existing usage of the algorithm, which is why we only
//      perform the check when the caller explicitly asks for it. Use of this
//      flag  for any algorithm other than XTS-AES generates an error.
...
_Must_inspect_result_ NTSTATUS WINAPI BCryptOpenAlgorithmProvider(
    _Out_ BCRYPT_ALG_HANDLE *phAlgorithm, _In_z_ LPCWSTR pszAlgId,
    _In_opt_z_ LPCWSTR pszImplementation, _In_ ULONG dwFlags);
...
_Must_inspect_result_ NTSTATUS WINAPI BCryptEnumAlgorithms(
    _In_ ULONG dwAlgOperations, _Out_ ULONG *pAlgCount,
    _Out_ BCRYPT_ALGORITHM_IDENTIFIER **ppAlgList, _In_ ULONG dwFlags);

typedef struct _BCRYPT_PROVIDER_NAME {
  LPWSTR pszProviderName;
} BCRYPT_PROVIDER_NAME;

_Must_inspect_result_ NTSTATUS WINAPI BCryptEnumProviders(
    _In_z_ LPCWSTR pszAlgId, _Out_ ULONG *pImplCount,
    _Out_ BCRYPT_PROVIDER_NAME **ppImplList, _In_ ULONG dwFlags);
...
_Must_inspect_result_ NTSTATUS WINAPI BCryptGetProperty(
    _In_ BCRYPT_HANDLE hObject, _In_z_ LPCWSTR pszProperty,
    _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PUCHAR pbOutput,
    _In_ ULONG cbOutput, _Out_ ULONG *pcbResult, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptSetProperty(_Inout_ BCRYPT_HANDLE hObject, _In_z_ LPCWSTR pszProperty,
                  _In_reads_bytes_(cbInput) PUCHAR pbInput, _In_ ULONG cbInput,
                  _In_ ULONG dwFlags);

NTSTATUS
WINAPI
BCryptCloseAlgorithmProvider(_Inout_ BCRYPT_ALG_HANDLE hAlgorithm,
                             _In_ ULONG dwFlags);

VOID WINAPI BCryptFreeBuffer(_In_ PVOID pvBuffer);

//
// Primitive encryption functions.
//
_Must_inspect_result_ NTSTATUS WINAPI BCryptGenerateSymmetricKey(
    _Inout_ BCRYPT_ALG_HANDLE hAlgorithm, _Out_ BCRYPT_KEY_HANDLE *phKey,
    _Out_writes_bytes_all_opt_(cbKeyObject) PUCHAR pbKeyObject,
    _In_ ULONG cbKeyObject, _In_reads_bytes_(cbSecret) PUCHAR pbSecret,
    _In_ ULONG cbSecret, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI BCryptGenerateKeyPair(
    _Inout_ BCRYPT_ALG_HANDLE hAlgorithm, _Out_ BCRYPT_KEY_HANDLE *phKey,
    _In_ ULONG dwLength, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptEncrypt(_Inout_ BCRYPT_KEY_HANDLE hKey,
              _In_reads_bytes_opt_(cbInput) PUCHAR pbInput, _In_ ULONG cbInput,
              _In_opt_ VOID *pPaddingInfo,
              _Inout_updates_bytes_opt_(cbIV) PUCHAR pbIV, _In_ ULONG cbIV,
              _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PUCHAR pbOutput,
              _In_ ULONG cbOutput, _Out_ ULONG *pcbResult, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptDecrypt(_Inout_ BCRYPT_KEY_HANDLE hKey,
              _In_reads_bytes_opt_(cbInput) PUCHAR pbInput, _In_ ULONG cbInput,
              _In_opt_ VOID *pPaddingInfo,
              _Inout_updates_bytes_opt_(cbIV) PUCHAR pbIV, _In_ ULONG cbIV,
              _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PUCHAR pbOutput,
...
_Must_inspect_result_ NTSTATUS WINAPI BCryptExportKey(
    _In_ BCRYPT_KEY_HANDLE hKey, _In_opt_ BCRYPT_KEY_HANDLE hExportKey,
    _In_z_ LPCWSTR pszBlobType,
    _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PUCHAR pbOutput,
    _In_ ULONG cbOutput, _Out_ ULONG *pcbResult, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI BCryptImportKey(
    _In_ BCRYPT_ALG_HANDLE hAlgorithm, _In_opt_ BCRYPT_KEY_HANDLE hImportKey,
    _In_z_ LPCWSTR pszBlobType, _Out_ BCRYPT_KEY_HANDLE *phKey,
    _Out_writes_bytes_all_opt_(cbKeyObject) PUCHAR pbKeyObject,
    _In_ ULONG cbKeyObject, _In_reads_bytes_(cbInput) PUCHAR pbInput,
    _In_ ULONG cbInput, _In_ ULONG dwFlags);

#define BCRYPT_NO_KEY_VALIDATION 0x00000008

#if (NTDDI_VERSION >= NTDDI_WIN10_CO)
#define BCRYPT_KEY_VALIDATION_RANGE 0x00000010 // BCryptImportKeyPair
#define BCRYPT_KEY_VALIDATION_RANGE_AND_ORDER                                  \
  0x00000018 // BCryptImportKeyPair & BCryptFinalizeKeyPair
#define BCRYPT_KEY_VALIDATION_REGENERATE 0x00000020 // BCryptImportKeyPair
#endif

_Must_inspect_result_ NTSTATUS WINAPI BCryptImportKeyPair(
    _In_ BCRYPT_ALG_HANDLE hAlgorithm, _In_opt_ BCRYPT_KEY_HANDLE hImportKey,
    _In_z_ LPCWSTR pszBlobType, _Out_ BCRYPT_KEY_HANDLE *phKey,
    _In_reads_bytes_(cbInput) PUCHAR pbInput, _In_ ULONG cbInput,
    _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI BCryptDuplicateKey(
    _In_ BCRYPT_KEY_HANDLE hKey, _Out_ BCRYPT_KEY_HANDLE *phNewKey,
    _Out_writes_bytes_all_opt_(cbKeyObject) PUCHAR pbKeyObject,
    _In_ ULONG cbKeyObject, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptFinalizeKeyPair(_Inout_ BCRYPT_KEY_HANDLE hKey, _In_ ULONG dwFlags);

NTSTATUS
WINAPI
BCryptDestroyKey(_Inout_ BCRYPT_KEY_HANDLE hKey);

NTSTATUS
WINAPI
BCryptDestroySecret(_Inout_ BCRYPT_SECRET_HANDLE hSecret);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptSignHash(_In_ BCRYPT_KEY_HANDLE hKey, _In_opt_ VOID *pPaddingInfo,
               _In_reads_bytes_(cbInput) PUCHAR pbInput, _In_ ULONG cbInput,
               _Out_writes_bytes_to_opt_(cbOutput, *pcbResult) PUCHAR pbOutput,
               _In_ ULONG cbOutput, _Out_ ULONG *pcbResult, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptVerifySignature(_In_ BCRYPT_KEY_HANDLE hKey, _In_opt_ VOID *pPaddingInfo,
                      _In_reads_bytes_(cbHash) PUCHAR pbHash, _In_ ULONG cbHash,
                      _In_reads_bytes_(cbSignature) PUCHAR pbSignature,
                      _In_ ULONG cbSignature, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI BCryptSecretAgreement(
    _In_ BCRYPT_KEY_HANDLE hPrivKey, _In_ BCRYPT_KEY_HANDLE hPubKey,
    _Out_ BCRYPT_SECRET_HANDLE *phAgreedSecret, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI BCryptDeriveKey(
    _In_ BCRYPT_SECRET_HANDLE hSharedSecret, _In_z_ LPCWSTR pwszKDF,
    _In_opt_ BCryptBufferDesc *pParameterList,
    _Out_writes_bytes_to_opt_(cbDerivedKey, *pcbResult) PUCHAR pbDerivedKey,
    _In_ ULONG cbDerivedKey, _Out_ ULONG *pcbResult, _In_ ULONG dwFlags);

#if (NTDDI_VERSION >= NTDDI_WIN8)
_Must_inspect_result_ NTSTATUS WINAPI BCryptKeyDerivation(
    _In_ BCRYPT_KEY_HANDLE hKey, _In_opt_ BCryptBufferDesc *pParameterList,
...
//
_Must_inspect_result_ NTSTATUS WINAPI BCryptCreateHash(
    _Inout_ BCRYPT_ALG_HANDLE hAlgorithm, _Out_ BCRYPT_HASH_HANDLE *phHash,
...
_Must_inspect_result_ NTSTATUS WINAPI BCryptHashData(
    _Inout_ BCRYPT_HASH_HANDLE hHash, _In_reads_bytes_(cbInput) PUCHAR pbInput,
    _In_ ULONG cbInput, _In_ ULONG dwFlags);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptFinishHash(_Inout_ BCRYPT_HASH_HANDLE hHash,
                 _Out_writes_bytes_all_(cbOutput) PUCHAR pbOutput,
                 _In_ ULONG cbOutput, _In_ ULONG dwFlags);

#if (NTDDI_VERSION > NTDDI_WINBLUE ||                                          \
     (NTDDI_VERSION == NTDDI_WINBLUE && defined(WINBLUE_KBSPRING14)))
_Must_inspect_result_ NTSTATUS WINAPI BCryptCreateMultiHash(
    _Inout_ BCRYPT_ALG_HANDLE hAlgorithm, _Out_ BCRYPT_HASH_HANDLE *phHash,
    _In_ ULONG nHashes,
    _Out_writes_bytes_all_opt_(cbHashObject) PUCHAR pbHashObject,
    _In_ ULONG cbHashObject,
...
_Must_inspect_result_ NTSTATUS WINAPI
BCryptProcessMultiOperations(_Inout_ BCRYPT_HANDLE hObject,
                             _In_ BCRYPT_MULTI_OPERATION_TYPE operationType,
                             _In_reads_bytes_(cbOperations) PVOID pOperations,
                             _In_ ULONG cbOperations, _In_ ULONG dwFlags);
#endif

_Must_inspect_result_ NTSTATUS WINAPI BCryptDuplicateHash(
    _In_ BCRYPT_HASH_HANDLE hHash, _Out_ BCRYPT_HASH_HANDLE *phNewHash,
    _Out_writes_bytes_all_opt_(cbHashObject) PUCHAR pbHashObject,
    _In_ ULONG cbHashObject, _In_ ULONG dwFlags);

NTSTATUS
WINAPI
BCryptDestroyHash(_Inout_ BCRYPT_HASH_HANDLE hHash);

#if (NTDDI_VERSION >= NTDDI_WINTHRESHOLD)
NTSTATUS
WINAPI
BCryptHash(_Inout_ BCRYPT_ALG_HANDLE hAlgorithm,
...
_Must_inspect_result_
    NTSTATUS WINAPI BCryptGenRandom(_In_opt_ BCRYPT_ALG_HANDLE hAlgorithm,
                                    _Out_writes_bytes_(cbBuffer)
                                        PUCHAR pbBuffer,
                                    _In_ ULONG cbBuffer, _In_ ULONG dwFlags);

#if (NTDDI_VERSION >= NTDDI_WIN7)
//
// Primitive key derivation functions.
//
_Must_inspect_result_ NTSTATUS WINAPI BCryptDeriveKeyCapi(
    _In_ BCRYPT_HASH_HANDLE hHash, _In_opt_ BCRYPT_ALG_HANDLE hTargetAlg,
    _Out_writes_bytes_(cbDerivedKey) PUCHAR pbDerivedKey,
    _In_ ULONG cbDerivedKey, _In_ ULONG dwFlags);
#endif

#if (NTDDI_VERSION >= NTDDI_WIN7)
_Must_inspect_result_ NTSTATUS WINAPI BCryptDeriveKeyPBKDF2(
    _In_ BCRYPT_ALG_HANDLE hPrf,
    _In_reads_bytes_opt_(cbPassword) PUCHAR pbPassword, _In_ ULONG cbPassword,
    _In_reads_bytes_opt_(cbSalt) PUCHAR pbSalt, _In_ ULONG cbSalt,
...
#if (NTDDI_VERSION >= NTDDI_WIN11_GE)
_Must_inspect_result_ NTSTATUS WINAPI BCryptEncapsulate(
...
#if (NTDDI_VERSION >= NTDDI_WIN11_GE)
_Must_inspect_result_ NTSTATUS WINAPI BCryptDecapsulate(
    _In_ BCRYPT_KEY_HANDLE hKey,
    _In_reads_bytes_(cbCipherText) PUCHAR pbCipherText, _In_ ULONG cbCipherText,
    _Out_writes_bytes_to_opt_(cbSecretKey, *pcbSecretKey) PUCHAR pbSecretKey,
...
#define BCRYPT_HASH_INTERFACE_VERSION_2                                        \
...
#define BCRYPT_KEY_ENCAPSULATION_INTERFACE_VERSION_1                           \
  BCRYPT_MAKE_INTERFACE_VERSION(1, 0)
...
_Must_inspect_result_ NTSTATUS WINAPI BCryptQueryProviderRegistration(
    _In_z_ LPCWSTR pszProvider, _In_ ULONG dwMode, _In_ ULONG dwInterface,
    _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
        PCRYPT_PROVIDER_REG *ppBuffer);

_Must_inspect_result_ NTSTATUS WINAPI BCryptEnumRegisteredProviders(
    _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
...
//
_Must_inspect_result_ NTSTATUS WINAPI
BCryptCreateContext(_In_ ULONG dwTable, _In_z_ LPCWSTR pszContext,
                    _In_opt_ PCRYPT_CONTEXT_CONFIG pConfig); // Optional

_Must_inspect_result_ NTSTATUS WINAPI
BCryptDeleteContext(_In_ ULONG dwTable, _In_z_ LPCWSTR pszContext);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptEnumContexts(_In_ ULONG dwTable, _Inout_ ULONG *pcbBuffer,
                   _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
                       PCRYPT_CONTEXTS *ppBuffer);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptConfigureContext(_In_ ULONG dwTable, _In_z_ LPCWSTR pszContext,
                       _In_ PCRYPT_CONTEXT_CONFIG pConfig);

_Must_inspect_result_ NTSTATUS WINAPI BCryptQueryContextConfiguration(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
        PCRYPT_CONTEXT_CONFIG *ppBuffer);

_Must_inspect_result_ NTSTATUS WINAPI BCryptAddContextFunction(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _In_z_ LPCWSTR pszFunction, _In_ ULONG dwPosition);

_Must_inspect_result_ NTSTATUS WINAPI
BCryptRemoveContextFunction(_In_ ULONG dwTable, _In_z_ LPCWSTR pszContext,
                            _In_ ULONG dwInterface, _In_z_ LPCWSTR pszFunction);

_Must_inspect_result_ NTSTATUS WINAPI BCryptEnumContextFunctions(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
        PCRYPT_CONTEXT_FUNCTIONS *ppBuffer);

_Must_inspect_result_ NTSTATUS WINAPI BCryptConfigureContextFunction(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _In_z_ LPCWSTR pszFunction, _In_ PCRYPT_CONTEXT_FUNCTION_CONFIG pConfig);

_Must_inspect_result_ NTSTATUS WINAPI BCryptQueryContextFunctionConfiguration(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _In_z_ LPCWSTR pszFunction, _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
        PCRYPT_CONTEXT_FUNCTION_CONFIG *ppBuffer);

_Must_inspect_result_ NTSTATUS WINAPI BCryptEnumContextFunctionProviders(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _In_z_ LPCWSTR pszFunction, _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
        PCRYPT_CONTEXT_FUNCTION_PROVIDERS *ppBuffer);

_Must_inspect_result_ NTSTATUS WINAPI BCryptSetContextFunctionProperty(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _In_z_ LPCWSTR pszFunction, _In_z_ LPCWSTR pszProperty, _In_ ULONG cbValue,
    _In_reads_bytes_opt_(cbValue) PUCHAR pbValue);

_Must_inspect_result_ NTSTATUS WINAPI BCryptQueryContextFunctionProperty(
    _In_ ULONG dwTable, _In_z_ LPCWSTR pszContext, _In_ ULONG dwInterface,
    _In_z_ LPCWSTR pszFunction, _In_z_ LPCWSTR pszProperty,
...
_Must_inspect_result_
    NTSTATUS WINAPI BCryptRegisterConfigChangeNotify(_In_ PRKEVENT pEvent);

#else
_Must_inspect_result_
    NTSTATUS WINAPI BCryptRegisterConfigChangeNotify(_Out_ HANDLE *phEvent);
#endif

#ifdef KERNEL_MODE_CNG
NTSTATUS
WINAPI
BCryptUnregisterConfigChangeNotify(_In_ PRKEVENT pEvent);

#else
NTSTATUS
WINAPI
BCryptUnregisterConfigChangeNotify(_In_ HANDLE hEvent);
#endif

//
// Provider Resolution Functions
//
_Must_inspect_result_ NTSTATUS WINAPI BCryptResolveProviders(
    _In_opt_z_ LPCWSTR pszContext, _In_opt_ ULONG dwInterface,
    _In_opt_z_ LPCWSTR pszFunction, _In_opt_z_ LPCWSTR pszProvider,
    _In_ ULONG dwMode, _In_ ULONG dwFlags, _Inout_ ULONG *pcbBuffer,
    _Outptr_opt_result_bytebuffer_all_maybenull_(*pcbBuffer)
...
NTSTATUS
WINAPI
BCryptGetFipsAlgorithmMode(_Out_ BOOLEAN *pfEnabled);
