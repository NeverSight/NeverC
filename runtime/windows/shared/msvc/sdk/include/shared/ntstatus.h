...
// This allows, for example, all privileges to be disabled without having to
// know exactly which privileges are assigned.
//
...
// transport, but the connection failed. The computer WAS able to connect on a
// secondary transport.
//
...
// MessageText:
//
// The directory service evaluated group memberships locally, as it was unable
// to contact a global catalog server.
//
...
// MessageText:
//
// The specified ring buffer was empty before the packet was successfully
// inserted.
//
...
// MessageText:
//
// The specified ring buffer was full before the packet was successfully
// removed.
//
...
// MessageText:
//
// The specified ring buffer has dropped below its quota of outstanding
// transactions.
//
...
// MessageText:
//
// The specified ring buffer has, with the removal of the current packet, now
// become empty.
//
...
// MessageText:
//
// The specified ring buffer was either previously empty or previously full
// which implies that the caller should signal the opposite endpoint.
//
...
// MessageText:
//
// A reparse should be performed by the Object Manager from the global root to
// escape the container name space.
//
...
//
// {Thread Suspended}
// A thread termination occurred while the thread was suspended. The thread was
...
// An attempt was made to set the working set minimum or maximum to values which
...
// Win16 program segment image. An exception is raised so a debugger can load,
...
// read the requested data from a redundant copy. This was done because the file
// system encountered a failure on a member of the fault-tolerant volume, but
...
// wrote a redundant copy of the information. This was done because the file
// system encountered a failure on a member of the fault-tolerant volume, but
// was not able to reassign the failing area of the device.
//
...
// The Windows password is too complex to be converted to a LAN Manager
...
// The image file %hs is valid, but is for a machine type other than the current
...
// The network transport returned partial data to its client. The remaining data
// will be sent later.
//
...
// The network transport returned partial data to its client and this data was
// marked as expedited by the remote system. The remaining data will be sent
// later.
//
...
//
// {Page Unlocked}
// The page protection of a locked page was changed to 'No Access' and the page
// was unlocked from memory and from the process.
//
...
// this system is not configured to allow the CardBus controller to be run in
// CardBus mode. The operating system will currently accept only 16-bit (R2)
// pc-cards on this controller.
//
...
// MessageText:
//
// The ALPC message being canceled has already been retrieved from the queue on
// the other side.
//
...
// MessageText:
//
// The receive operation was successful. Check the ALPC completion list for the
// received message.
//
...
// MessageText:
//
// A system patch was successfully loaded but is not applicable to any currently
// loaded images.
//
...
// A page of memory that marks the end of a data structure, such as a stack or
// an array, has been accessed.
//
...
//
// {GUID Substitution}
// During the translation of a global identifier (GUID) to a Windows security ID
// (SID), no administratively-defined GUID prefix was found. A substitute prefix
// was used, which will not compromise system security. However, this may
...
//
// {Verifying Disk}
// The media has changed and a verify operation is in progress so no reads or
// writes may be performed to the device, except those used in the verify
// operation.
//
...
// MessageText:
//
// The action requested resulted in no work being done. Error-style clean-up has
// been performed.
//
...
// MessageText:
//
// A return address hijack is being attempted. This is supported by the
// operating system when user-mode shadow stacks are enabled.
//
...
// MessageText:
//
// The secure page table update could not be completed due to a temporary
// address exclusion.
//
...
// MessageText:
//
// BitLocker encryption keys were ignored because the volume was in a transient
// state.
//
...
/*++
 ...
 MessageId's 0xcf00 - 0xcfff (inclusive) are for Cloud Files specific warning
messages.
...
// MessageText:
//
// The cloud file property is possibly corrupt. The on-disk checksum does not
// match the computed checksum.
//
#define STATUS_CLOUD_FILE_PROPERTY_BLOB_CHECKSUM_MISMATCH                      \
  ((NTSTATUS)0x8000CF00L)
...
// MessageText:
//
// The specified information record length does not match the length required
...
// MessageText:
//
// The instruction at 0x%p referenced memory at 0x%p. The memory could not be
// %s.
//
...
// MessageText:
//
// The instruction at 0x%p referenced memory at 0x%p. The required data was not
// placed into memory because of an I/O error status of 0x%x.
//
...
// MessageText:
//
// The end-of-file marker has been reached. There is no valid data in the file
// beyond this marker.
//
...
//
// {Still Busy}
// The specified I/O request packet (IRP) cannot be disposed of because the I/O
...
//
// {Invalid Mapping}
// An attempt was made to create a view for a section which is bigger than the
// section.
//
...
//
// {Bad File}
// The attributes of the specified mapping file for a section of memory cannot
// be read.
//
...
// MessageText:
//
// A port with the 'do not disturb' flag set attempted to send a message to a
// port in a suspended process. The process was not woken, and the message was
// not delivered.
//
...
// MessageText:
//
// An attempt was made to release a semaphore such that its maximum count would
// have been exceeded.
//
...
// MessageText:
//
// An attempt to set a process's DebugPort or ExceptionPort was made, but a port
// already exists in the process or an attempt to set a file's CompletionPort
// made, but a port was already set in the file or an attempt to set an ALPC
...
// MessageText:
//
// An attempt was made to query image information on a section which does not
// map an image.
//
...
// MessageText:
//
// An attempt was made to set the working set limit to an invalid value (minimum
// greater than maximum, etc).
//
...
// MessageText:
//
// A section was created to map a file which is not compatible to an already
// existing section which maps the same file.
//
...
// MessageText:
//
// A view to a section specifies a protection which is incompatible with the
// initial view's protection.
//
...
// MessageText:
//
// An operation involving EAs failed because the file system does not support
// EAs.
//
...
// MessageText:
//
// A non close operation has been requested of a file object with a delete
// pending.
//
...
// MessageText:
//
// An attempt was made to set the control attribute on a file. This attribute is
// not supported in the target file system.
//
...
// service. It may be a more recent revision than the service is aware of.
//
...
// MessageText:
//
// Indicates a particular Security ID may not be assigned as the owner of an
// object.
//
...
// MessageText:
//
// Indicates a particular Security ID may not be assigned as the primary group
// of an object.
//
...
// available. Make sure your device is connected to your organization's network
// and try again. If you previously signed in on this device with another
...
// MessageText:
//
// Indicates the requested operation would disable, delete or could prevent
// logon for an administration account. This is not allowed to prevent creating
...
// MessageText:
//
// When trying to update a password, this status indicates that the password
// does not meet the length, complexity, or history requirements of the machine
// or domain. Try increasing the length of your password, along with including
// upper and lowercase characters, numbers, and symbols.
//
...
// MessageText:
//
// The user account has time restrictions and may not be logged onto at this
// time.
//
...
// MessageText:
//
// The user account is restricted such that it may not be used to log on from
...
//
// {Bad Image}
// %hs is either not designed to run on Windows or it contains an error. Try
// installing the program again using the original installation media or contact
// your system administrator or the software vendor for support. Error status
// 0x%08lx.
//
...
// This is typically done by referencing the token associated with a thread when
// the thread is not impersonating a client.
//
...
// probable causes is the replacement of a CreatorId with an SID that didn't fit
// into the ACE or ACL.
//
...
// If this is a thinly provisioned volume the physical storage backing this
...
// MessageText:
//
// There are no more authority agent values available for the given identifier
// authority value.
//
...
// successive updates may exceed the amount of memory originally allotted. Since
// quota may already have been charged to several processes which have handles
// to the object, it is not reasonable to alter the size of the allocated
// memory. Instead, a request that requires more memory than has been allotted
// must fail and the STATUS_ALLOTED_SPACE_EXCEEDED error returned.
//
...
// MessageText:
//
// Virtual memory cannot be freed as base address is not the base of the region
// and a region size of zero was specified.
//
...
// the write protection from the volume %hs in drive %hs.
//
...
//
// {Drive Not Ready}
// The drive is not ready for use; its door may be open. Please check drive %hs
...
// operations queued so it cannot be changed to queue operations.
//
...
// The specified device type (LPT, for example) conflicts with the actual device
// type on the remote resource.
//
...
//
// {Incorrect Volume}
// The target file of a rename request is located on a different device than the
...
// MessageText:
//
// Indicates an attempt was made to operate on the security of an object that
...
// MessageText:
//
// Error status returned when an invalid oplock acknowledgment is received by a
// file system.
//
...
// MessageText:
//
// This error indicates that the requested operation cannot be completed due to
// a catastrophic media failure or on-disk data structure corruption.
//
...
// MessageText:
//
// Indicates generic access types were contained in an access mask which should
// already be mapped to non-generic access types.
//
...
// MessageText:
//
// Indicates a security descriptor is not in the necessary format (absolute or
// self-relative).
//
...
// is defined since the caller does not want to accept STATUS_ACCESS_VIOLATION
// in its filter.
//
...
// MessageText:
//
// If an I/O error is returned which is not defined in the standard FsRtl
// filter, it is converted to the following error which is guaranteed to be in
...
// MessageText:
//
// An attempt was made to access a network file, but the network software was
// not yet started.
//
...
// MessageText:
//
// An attempt was made to start the redirector, but the redirector has already
// been started.
//
...
// MessageText:
//
// An internal LSA error has occurred. An authentication package has requested
// the creation of a Logon Session but the ID of an already existing Logon
...
// MessageText:
//
// The user attempted to force close the files on a redirected drive, but there
// were opened files on the drive, and the user did not specify a sufficient
// level of force.
//
...
//
// The user attempted to force close the files on a redirected drive, but there
// were opened directories on the drive, and the user did not specify a
...
// MessageText:
//
// RtlFindMessage could not locate the requested message ID in the message table
// resource.
//
...
// MessageText:
//
// Indicates that an attempt has been made to impersonate via a named pipe that
...
// MessageText:
//
// The specified image file did not have the correct format. It appears to be NE
// format.
//
...
// MessageText:
//
// Indicates that the transaction state of a registry sub-tree is incompatible
// with the requested operation. For example, a request has been made to start a
// new transaction with one already in progress, or a request has been made to
// apply a transaction when one is not currently in progress.
//
...
// database has been left in an unknown, but probably inconsistent, state. The
// state of the registry transaction is left as COMMITTING.
//
...
// This error should only be returned by the Windows redirector on a remote
// drive.
//
...
// MessageText:
//
// An I/O request other than close was performed on a file after it has been
// deleted, which can only happen to a request which did not complete before the
// last handle was closed via NtClose.
//
...
// MessageText:
//
// Indicates an operation has been attempted on a built-in (special) SAM account
// which is incompatible with built-in accounts. For example, built-in accounts
// cannot be deleted.
//
...
// MessageText:
//
// The operation requested may not be performed on the specified group because
// it is a built-in special group.
//
...
// MessageText:
//
// The operation requested may not be performed on the specified user because it
// is a built-in special user.
//
...
// MessageText:
//
// An attempt was made to establish a token for use as a primary token but the
// token is already in use. A token can only be the primary token of one process
// at a time.
//
...
// MessageText:
//
// The specified image file did not have the correct format, it appears to be LE
// format.
//
...
// MessageText:
//
// The specified image file did not have the correct format, it did not have a
// proper e_lfarlc in the MZ header.
//
...
// MessageText:
//
// The specified image file did not have the correct format, it appears to be a
// 16-bit Windows image.
//
...
// MessageText:
//
// The time at the Primary Domain Controller is different than the time at the
...
// MessageText:
//
// The code execution cannot proceed because %hs was not found. Reinstalling the
...
// The procedure entry point %hs could not be located in the dynamic link
// library %hs.
//
...
// The network transport on your computer has closed a network connection. There
...
//
// {Virtual Circuit Closed}
// The network transport on a remote computer has closed a network connection.
// There may or may not be I/O requests outstanding.
//
...
// request. For instance, there may not be enough memory available on the remote
// computer to carry out the request at this time.
//
...
//
// {Virtual Circuit Closed}
// An existing connection (virtual circuit) has been broken at the remote
// computer. There is probably something wrong with the network software
...
// because it had to wait too long for a response from the remote computer.
//
...
//
// {Application Error}
// The application was unable to start correctly (0x%lx). Click OK to close the
// application.
//
...
// the image of the file in memory is corrupt, or the file could not be
// recovered because the alternate copy or log was absent or corrupt.
//
...
// granted by the target system's local security policy. Please ask the system
// administrator to grant the necessary form of logon.
//
...
// number of secrets is limited to satisfy United States State Department export
// restrictions.
//
...
// MessageText:
//
// A user has requested a type of logon (e.g., interactive or network) that has
// not been granted. An administrator has control over who may logon
...
// without providing the necessary Windows cross-encrypted password.
//
...
// via a device control to the Fault Tolerance driver and the secondary copy is
...
// MessageText:
//
// A configuration registry node representing a driver service entry was
// ill-formed and did not contain required value entries.
//
...
//
// {Floppy Disk Error}
// While accessing a floppy disk, the track address from the sector ID field was
// found to be different than the track address maintained by the controller.
//
...
// MessageText:
//
// An attempt was made to open a device that was sharing an IRQ with other
// devices. At least one other device that uses that IRQ was already opened. Two
// concurrent opens of devices that share an IRQ and only work via interrupts is
// not supported for the particular bus type that the devices use.
//
...
// MessageText:
//
// The I/O device is configured incorrectly or the configuration parameters to
// the driver are incorrect.
//
...
// MessageText:
//
// The workstation does not have a trust secret for the primary domain in the
// local LSA database.
//
...
// MessageText:
//
// The SAM database on the Windows Server does not have a computer account for
...
// MessageText:
//
// The logon request failed because the trust relationship between the primary
...
// MessageText:
//
// The operation failed because the application is not part of an application
// package.
//
...
// MessageText:
//
// The Security Account Manager blocked the use of a weak Windows Hello for
// Business key.
//
...
// MessageText:
//
// Multiple mappings to shared resource(s) on a server, using more than one
// transport, are not allowed. Use a single transport for all mappings to a
...
// MessageText:
//
// Multiple mappings to shared resource(s) on a server, using different
// certificate validation preferences, are not allowed. Use the same preference
// for all mappings to a server and try again.
//
#define STATUS_CERTIFICATE_VALIDATION_PREFERENCE_CONFLICT                      \
  ((NTSTATUS)0xC00001B5L)
...
// MessageText:
//
// The transport rejected the network address specified due to an invalid use of
// a wildcard.
//
...
// MessageText:
//
// The transport address could not be opened because all the available addresses
// are in use.
//
...
// MessageText:
//
// The transport does not recognized the transaction request identifier
// specified.
//
...
// MessageText:
//
// The transport can only process the specified request on the server side of a
// session.
//
...
// MessageText:
//
// The transport can only process the specified request on the client side of a
// session.
//
...
// request. You may choose OK to terminate the process, or Cancel to ignore the
// error.
//
...
// An attempt was made to reply to an LPC message, but the thread specified by
...
// address or the offset into the file were not aligned on the proper allocation
// granularity.
//
...
// MessageText:
//
// The attempt to insert the ID in the index failed because the ID is already in
// the index.
//
...
// MessageText:
//
// The attempt to set the object's ID failed because the object already has an
// ID.
//
...
// The image %hs has been modified for use on a uniprocessor system, but you are
// running it on a multiprocessor machine. Please reinstall the image file.
//
...
// The dynamic link library %hs is not written correctly. The stack pointer has
// been left in an inconsistent state. The entrypoint should be declared as
// WINAPI or STDCALL. Select YES to fail the DLL load. Select NO to continue
...
// The %hs service is not written correctly. The stack pointer has been left in
// an inconsistent state. The callback entrypoint should be declared as WINAPI
// or STDCALL. Selecting OK will cause the service to continue operation.
...
// compromised then please contact your administrator immediately to have a new
// one assigned.
//
...
// MessageText:
//
// You have attempted to load a legacy device driver while its device instance
// had been disabled.
//
...
//
// {Application Error}
// The application attempted to release a resource it did not own. Click OK to
...
// Windows, please upgrade this installation using a licensed distribution of
// this product.
//
...
// properly. The relocation occurred because the DLL %hs occupied an address
// range reserved for Windows system DLLs. The vendor supplying the DLL should
...
//
// {License Violation}
// The system has detected tampering with your registered product type. This is
// a violation of your software license. Tampering with product type is not
// permitted.
//
...
// MessageText:
//
// The Windows I/O reparse tag does not match the one present in the reparse
// point.
//
...
// MessageText:
//
// The symbolic link could not be resolved even though the initial file name is
// valid.
//
...
// MessageText:
//
// The specified element is contained within a magazine that is no longer
// present.
//
...
// MessageText:
//
// The device has indicated that its door is open. Further operations require it
// closed and secured.
//
...
// MessageText:
//
// The reparse attribute cannot be set as it is incompatible with an existing
// attribute.
//
...
// been exhausted. Please see http://go.microsoft.com/fwlink/?LinkId=228610 for
// recommended diagnostic steps and the procedure to re-enable account creation.
//
...
// The driver %hs does not support standby mode. Updating this driver may allow
...
// MessageText:
//
// The medium changer's transport element contains media, which is causing the
// operation to fail.
//
...
// error: %hs Error Status: 0x%x. Please shutdown this system and reboot into
...
//
// {Fatal System Error}
// The system image %s is not properly signed. The file has been replaced with
// the signed file. The system has been shut down.
//
...
// MessageText:
//
// In mixed domain, cannot nest local groups with other local groups, if the
...
// MessageText:
//
// Cannot change to security disabled group because of having primary members in
// this group.
//
...
//
// Security Accounts Manager initialization failed because of the following
// error: %hs Error Status: 0x%x. Please click OK to shutdown this system and
// reboot into Safe Mode, check the event log for more detailed information.
//
...
//
// Security Accounts Manager initialization failed because of the following
// error: %hs Error Status: 0x%x. Please click OK to shutdown the system. You
...
// MessageText:
//
// Unable to accomplish the requested task because the local machine does not
...
// MessageText:
//
// The crypto system or checksum function is invalid because a required function
// is unavailable.
//
...
// MessageText:
//
// The other end of the security negotiation is requires strong crypto but it is
...
// MessageText:
//
// The client certificate does not contain a valid UPN, or does not match the
// client name in the logon request. Please contact your administrator.
//
...
// MessageText:
//
// This operation is not supported on a computer running Windows Server 2003 for
...
// MessageText:
//
// An administrator has restricted sign in. To sign in, make sure your device is
// connected to the Internet, and have your administrator sign in first.
//
...
// certificate during logon. There is more information in the system event log.
//
...
// than the one used on the destination file.
//
...
// MessageText:
//
// An attempt to remove a process's DebugPort was made, but a port was not
...
// MessageText:
//
// Debugger Inactive: Windows may have been started without kernel debugging
// enabled.
//
...
// MessageText:
//
// The client's session has expired, so the client must reauthenticate to
...
// MessageText:
//
// Access to %1 has been restricted by your Administrator by software publisher
// policy.
//
...
// point which resolves to a volume to which the specified device object is not
// attached.
//
...
// MessageText:
//
// The smartcard certificate used for authentication has been revoked. Please
// contact your system administrator. There may be additional information in the
// event log.
//
...
// MessageText:
//
// The revocation status of the certificate used for authentication could not be
// determined.
//
...
// MessageText:
//
// The smartcard provider could not perform the action since the context was
// acquired as silent.
//
...
// request for a target outside of the server's realm. This is not supported,
// and indicates a misconfiguration on this server's allowed to delegate to
...
// authentication could not be determined. There is additional information in
...
// MessageText:
//
// An untrusted certificate authority was detected while processing the domain
// controller certificate used for authentication. There is additional
...
// MessageText:
//
// The domain controller certificate used for logon has expired. There is
// additional information in the system event log.
//
...
// MessageText:
//
// The domain controller certificate used for logon has been revoked. There is
// additional information in the system event log.
//
...
// authentication firewall. The specified account is not allowed to authenticate
// to the machine.
//
...
// MessageText:
//
// %hs is a 16-bit application. You do not have permissions to execute 16-bit
...
// reboot the machine a dialog will be displayed giving you a chance to report
// this failure to Microsoft.
//
...
// MessageText:
//
// An exception has occurred in a user mode callback and the kernel callback
...
// SymbolicName=STATUS_VERSION_PARSE_ERROR Language=English A version number
// could not be parsed.
// .
//
...
// MessageText:
//
// The page fault mappings changed in the middle of processing a fault so the
...
// MessageText:
//
// The attempt to purge this file from memory failed to purge some or all the
// data from memory.
//
...
// MessageText:
//
// One or more copies of data on this device may be out of sync. No writes may
// be performed until a data integrity scan is completed.
//
...
// The target device does not support read returning zeros from trimmed/unmapped
// blocks.
//
...
// The specified I/O operation failed to complete within the expected time
// period.
//
...
// MessageText:
//
// The deployment operation failed because the specified application needs to be
// registered first.
//
...
// MessageText:
//
// A file system block being referenced has already reached the maximum
...
// MessageText:
//
// The requested operation failed because the file stream is marked to disallow
// writes.
//
...
// MessageText:
//
// Windows Information Protection policy does not allow access to this network
// resource.
//
...
// MessageText:
//
// An externally encrypted syskey has been configured, but the system no longer
// supports this feature.  Please see
...
// MessageText:
//
// An attempt was made to access protected memory in violation of its secure
// access policy.
//
...
// MessageText:
//
// The read or write operation to an encrypted file could not be completed
// because the file can only be accessed when the device is unlocked.
//
...
// MessageText:
//
// A read or write of raw encrypted data cannot be performed because the file is
// not encrypted.
//
...
// MessageText:
//
// An invalid file offset in the encrypted data info block was passed for read
// or write operation of file's raw encrypted data.
//
#define STATUS_RWRAW_ENCRYPTED_INVALID_EDATAINFO_FILEOFFSET                    \
  ((NTSTATUS)0xC00004A8L)
...
// MessageText:
//
// An invalid offset and length combination in the encrypted data info was
// passed for read or write operation of file's raw encrypted data.
//
#define STATUS_RWRAW_ENCRYPTED_INVALID_EDATAINFO_FILERANGE                     \
  ((NTSTATUS)0xC00004A9L)
...
// MessageText:
//
// An invalid parameter in the encrypted data info was passed for read or write
// operation of file's raw encrypted data.
//
#define STATUS_RWRAW_ENCRYPTED_INVALID_EDATAINFO_PARAMETER                     \
  ((NTSTATUS)0xC00004AAL)
...
// MessageText:
//
// A system patch could not be applied due to conflicting accesses to the system
// image.
//
...
// MessageText:
//
// The file cannot be safely opened because it is not supported by this version
// of Windows.
//
...
// MessageText:
//
// Encryption cannot be cleared on this file/directory because it still has an
// encrypted attribute.
//
...
// MessageText:
//
// The operation could not be completed due to one or more unsatisfied
// dependencies.
//
...
// MessageText:
//
// This image cannot be loaded because it has already been loaded as a hot patch
// image.
//
...
// MessageText:
//
// The specified operation is not supported while encryption is enabled on the
// target object.
//
...
// MessageText:
//
// The specified operation is not supported while compression is enabled on the
// target object.
//
...
// MessageText:
//
// The specified operation is not supported while replication is enabled on the
// target object.
//
...
// MessageText:
//
// The specified operation is not supported while deduplication is enabled on
// the target object.
//
...
// MessageText:
//
// The specified operation is not supported while auditing is enabled on the
// target object.
//
...
// MessageText:
//
// The specified operation is not supported while monitoring is enabled on the
// target object.
//
...
// MessageText:
//
// The specified operation is not supported while snapshot is enabled on the
// target object.
//
...
// MessageText:
//
// The specified operation is not supported while virtualization is enabled on
// the target object.
//
...
// MessageText:
//
// The volume is opened for exclusive write access, preventing files from being
...
// MessageText:
//
// Personal Data Encryption keys to encrypt data are currently unavailable due
// to a failure.
//
...
// MessageText:
//
// Personal Data Encryption keys to decrypt data are currently unavailable due
// to a failure.
//
...
//
#define STATUS_VOLUME_UPGRADE_DISABLED_TILL_OS_DOWNGRADE_EXPIRED               \
  ((NTSTATUS)0xC00004DCL)
...
// MessageText:
//
// The data stored in compressed memory falied to be decompressed due to
// encountering an uncorrectable hardware memory error.
//
...
// MessageText:
//
// The operation was aborted because the observed volume identity or current
// state was not expected.
//
...
// MessageText:
//
// Unable to finish the requested operation because the specified process is not
// a GUI process.
//
...
// MessageText:
//
// The requested operation could not be completed because the specified job has
// children.
//
...
// system to attack. Your system requires SMB2 or higher. For more info on
// resolving this issue, see: https://go.microsoft.com/fwlink/?linkid=852747
//
...
//
// Windows cannot verify the digital signature for this file. The signing
// certificate for this file has expired.
//
...
// MessageText:
//
// The specified image file was blocked from loading because it does not enable
// a feature required by the process: Control Flow Guard.
//
...
// MessageText:
//
// The thread context could not be updated because this has been restricted for
// the process.
//
...
// MessageText:
//
// After a callback to 0x%p(0x%p), a completion call to SetEvent(0x%p) failed
// with status 0x%08x.
//
...
// MessageText:
//
// After a callback to 0x%p(0x%p), a completion call to ReleaseSemaphore(0x%p,
// %d) failed with status 0x%08x.
//
...
// MessageText:
//
// After a callback to 0x%p(0x%p), a completion call to ReleaseMutex(%p) failed
// with status 0x%08x.
//
...
// MessageText:
//
// After a callback to 0x%p(0x%p), an completion call to FreeLibrary(%p) failed
// with status 0x%08x.
//
...
// MessageText:
//
// The threadpool 0x%p was released while a thread was posting a callback to
// 0x%p(0x%p) to it.
//
...
// MessageText:
//
// A threadpool worker thread is impersonating a client, after a callback to
// 0x%p(0x%p). This is unexpected, indicating that the callback is missing a
...
// This is unexpected, indicating that the APC is missing a call to revert the
// impersonation.
//
...
// MessageText:
//
// A threadpool worker thread enter a callback at thread base priority 0x%x and
// exited at priority 0x%x. This is unexpected, indicating that the callback
...
// MessageText:
//
// An invalid thread, handle %p, is specified for this operation. Possibly, a
// threadpool worker thread was specified.
//
...
// MessageText:
//
// A threadpool worker thread enter a callback, which left with preferred
// languages set. This is unexpected, indicating that the callback missed
// clearing them.
//
...
// MessageText:
//
// A threadpool worker thread enter a callback, which left with background
// priorities set. This is unexpected, indicating that the callback missed
// restoring the original priorities.
//
...
// MessageText:
//
// A threadpool worker thread enter a callback at thread affinity %p and exited
// at affinity %p. This is unexpected, indicating that the callback missed
...
// MessageText:
//
// The caller has exceeded the maximum number of handles that may be transmitted
// in a single local procedure call.
//
...
// MessageText:
//
// A write to executable memory occurred for a process that is managing such
// operations.
//
...
// MessageText:
//
// A write to executable memory occurred from kernel mode for a process that is
...
// MessageText:
//
// A write to executable memory occurred from kernel mode while attached to a
...
// MessageText:
//
// A write to executable memory was triggered cross-process to a process that is
// managing such operations.
//
...
// MessageText:
//
// The Directory Service cannot perform the requested operation because a domain
...
// records in them. The volume contains detected corruptions and must be
// scanned.
//
...
// MessageText:
//
// One of the volume corruption logs was cleared by chkdsk and no longer
// contains real corruptions.
//
...
// MessageText:
//
// The operation could not be completed because an instance of Proactive Scanner
...
//
// The read or write operation to an encrypted file could not be completed
// because the file has not been opened for data access.
//
...
// MessageText:
//
// One of the volume corruption logs comes from a newer version of Windows and
// contains corruption records. The log will be emptied and reset to the current
// version, and the volume health state will be updated accordingly.
//
...
// MessageText:
//
// Access Denied. Before opening files in this location, you must first browse
...
// MessageText:
//
// The specified section is supported by direct map technology but the caller
// specifically does not want that.
//
...
// MessageText:
//
// The source file system block has been modified after containing a weak
// reference.
//
...
// MessageText:
//
// The target file system block has been modified after containing a weak
// reference.
//
...
// MessageText:
//
// Enabling driver verification from volatile command is currently not supported
// when both CFG and IO are enabled.
//
...
// MessageText:
//
// Removal of current driver verification is not supported from volatile
// command.
//
...
// MessageText:
//
// Enabling driver verification is not supported from volatile mode in current
// system.
//
...
// MessageText:
//
// The specified driver is protected and volatile verification is currently not
// supported.
//
...
// MessageText:
//
// Enabling driver verification is not supported for a driver with  NMI
// callback(s) registered.
//
...
// MessageText:
//
// Volatile verification settings cannot be changed when verification is enabled
// from boot or DIF volatile verification is enabled.
//
...
// MessageText:
//
// The specified driver is not associated with driver object or driver
// extension.
//
...
// MessageText:
//
// Verification cannot start because an attempt to lock code or data section
// failed.
//
...
// MessageText:
//
// Filesystem needs to release all the file resources and callback into Cc to
// drain pending section deletions, before retrying the operation.
//
...
// MessageText:
//
// Indicates that the caller made the connection request in the wrong routing
// compartment.
//
...
// MessageText:
//
// Indicates that there was an AuthIP failure when attempting to connect to the
// remote host.
//
...
// MessageText:
//
// The local account policy modification request was rejected because the policy
// is controlled by a regional authority.
//
...
// MessageText:
//
// The Local Administrator Password Solution password update operation failed
// because the legacy LAPS schema needs to be added to Active Directory.
//
...
//
// The Local Administrator Password Solution password update operation failed
// because the Windows LAPS schema needs to be added to Active Directory.
//
...
// MessageText:
//
// The Local Administrator Password Solution encrypted password update operation
// failed because Active Directory is not yet running at the minimum required
...
// MessageText:
//
// The process was terminated by Windows Local Administrator Password Solution
// per the configured post-authentication-action policy.
//
...
// MessageText:
//
// The Active Directory JET database page size limit for this request was
// exceeded.
//
...
// MessageText:
//
// The Active Directory JET database page size does not match on all Domain
// Controllers in this forest.
//
...
/*++
 ...
 MessageId's 0xa200 - 0xa280 (inclusive) are reserved for app container specific
messages.
...
// MessageText:
//
// Access to the specified resource has been denied for a less privileged app
// container.
//
...
// MessageText:
//
// The WOF driver encountered a corruption in the compressed file's Resource
// Table.
//
...
//
#define STATUS_FILE_SYSTEM_VIRTUALIZATION_METADATA_CORRUPT                     \
  ((NTSTATUS)0xC000CE02L)
...
// MessageText:
//
// The provider that supports file system virtualization is too busy to complete
// this operation.
//
...
//
#define STATUS_FILE_SYSTEM_VIRTUALIZATION_PROVIDER_UNKNOWN                     \
  ((NTSTATUS)0xC000CE04L)
...
//
#define STATUS_FILE_SYSTEM_VIRTUALIZATION_INVALID_OPERATION                    \
  ((NTSTATUS)0xC000CE05L)
...
// MessageText:
//
// The cloud sync provider failed to perform the operation due to low system
// resources.
//
...
// MessageText:
//
// The cloud sync provider failed to perform the operation due to network being
// unavailable.
//
...
// MessageText:
//
// The cloud provider failed to acknowledge a message before the time-out
// expired.
//
...
// MessageText:
//
// Cloud file cannot be retrieved from your cloud provider. Try again after
// responding to your cloud provider's request for action.
//
...
// MessageText:
//
// A snapshot of the file cannot be taken because a user-mapped section is
// present.
//
...
// MessageText:
//
// The file snapshot operation was terminated because one of the files was
// modified in a way incompatible with a snapshot operation.  Please try again.
//
...
// MessageText:
//
// An attempt was made to create a new union root as an ancestor or descendant
// of an existing union root, but the new union is not configured to allow
// nesting.
//
...
// Services is currently busy. Please try again in a few minutes. Other users
// should still be able to log on.
//
...
// MessageText:
//
// A process has requested access to a session, but has not been granted those
// access rights.
//
...
// session, a session that has no user logged on, nor control other sessions
// from the console.
//
...
// of the Terminal Client. Please call your system administrator for help in
...
// and has disconnected the client. Client IP: %2.
//
...
#define FILTER_FLT_NTSTATUS_FROM_HRESULT(x)                                    \
  ((NTSTATUS)(((x) & 0xC0007FFF) | (FACILITY_FILTER_MANAGER << 16) |           \
              0x40000000))
...
// MessageText:
//
// Internal error code used by the filter manager to determine if a fastio
// operation should be forced down the IRP path. Mini-filters should never
// return this value.
//
...
// MessageText:
//
// The filesystem database resource is in use. Registration cannot complete at
// this time.
//
...
// MessageText:
//
// The filter is not allowed to attach because it has not declared compability
// with WCOS.
//
...
// MessageText:
//
// The operation has succeeded on the down level file system, but CSV is failing
// it because it is in draining state.
//
#define STATUS_CLUSTER_CSV_VOLUME_DRAINING_SUCCEEDED_DOWNLEVEL                 \
  ((NTSTATUS)0xC0130026L)
...
// MessageText:
//
// Volsnap on the coordinating node returned an error indicating that there is
// no snapshots on this volume.
//
...
// MessageText:
//
// The operation has failed because CSV volume was not able to recover in time
// specified on this file object.
//
...
// MessageText:
//
// The buffer passed in to NtPushTransaction or NtPullTransaction is not in a
// valid format.
//
...
// MessageText:
//
// The transactional metadata attribute on the file or directory %hs is corrupt
// and unreadable.
//
...
// MessageText:
//
// The transactional resource manager had too many transactions outstanding that
...
// MessageText:
//
// The specified ResourceManager was unable to be opened, because it was not
// found.
//
...
// MessageText:
//
// The specified TransactionManager was unable to be opened, because it was not
// found.
//
...
// MessageText:
//
// Log service encountered an attempt to erroneously allocate or dispose
// reservation space.
//
...
// MessageText:
//
// Log service has attempted to read or write backwards past the start of the
// log.
//
...
// The %hs display driver has stopped working normally. Save your work and
// reboot the system to restore full display functionality. The next time you
// reboot the machine a dialog will be displayed giving you a chance to upload
...
// The %hs display driver has stopped working normally. The recovery had been
// performed.
//
#define STATUS_VIDEO_HUNG_DISPLAY_DRIVER_THREAD_RECOVERED                      \
  ((NTSTATUS)0x801B00EBL)
...
// The %hs display driver has detected and recovered from a failure. Some
// graphical operations may have failed. The next time you reboot the machine a
// dialog will be displayed giving you a chance to upload data about this
...
// MessageText:
//
// The kernel driver detected a version mismatch between it and the user mode
// driver.
//
...
// Partial data populated up to the size of the buffer. Caller needs to provide
// buffer of size as specified in the partially populated buffer's content
// (interface specific).
//
...
// MessageText:
//
// Present redirection is disabled (desktop windowing management subsystem is
// off).
//
...
// MessageText:
//
// Windowless present is disabled (desktop windowing management subsystem is
// off).
//
...
//
#define STATUS_GRAPHICS_INDIRECT_DISPLAY_ABANDON_SWAPCHAIN                     \
  ((NTSTATUS)0xC01E0012L)
...
// MessageText:
//
// Notifying indirect display UMDF class driver that indirect display device has
// been stopped.
//
...
// and can't be referenced any further.
//
...
// MessageText:
//
// A problem couldn't be solved due to some currently existing condition. The
// problem should be tried again later.
//
...
//
// A problem couldn't be solved due to some currently existing condition. The
// problem should be tried again immediately.
//
...
// MessageText:
//
// The allocation can't be used from its current segment location for the
// specified operation.
//
...
// MessageText:
//
// GPU exception is detected on the given device. The device is not able to be
// scheduled.
//
...
// MessageText:
//
// Specified VidPN modality is not supported (e.g. at least two of the pinned
...
// MessageText:
//
// Number of video present targets must be greater than or equal to the number
// of video present sources.
//
#define STATUS_GRAPHICS_VIDEO_PRESENT_TARGETS_LESS_THAN_SOURCES                \
  ((NTSTATUS)0xC01E0326L)
...
// MessageText:
//
// Specified VidPN source is already owned by a DMM client and cannot be used
// until that client releases it.
//
...
// MessageText:
//
// Miniport requested that augmentation be cancelled for the specified source of
// the specified VidPN's topology.
//
#define STATUS_GRAPHICS_CANCEL_VIDPN_TOPOLOGY_AUGMENTATION                     \
  ((NTSTATUS)0xC01E035AL)
...
// MessageText:
//
// An attempt was made to power up a lead link display adapter when the chain
// links were powered down.
//
...
// MessageText:
//
// The adapter link was found to be in an inconsistent state. Not all adapters
// are in an expected PNP/Power state.
//
...
// MessageText:
//
// The display adapter is being polled for children too frequently at the same
// polling level.
//
...
// MessageText:
//
// An operation is being attempted that requires the display adapter to be in a
// quiescent state.
//
...
// MessageText:
//
// The DxgkDdiOpmCreateProtectedOutput function could not create a protected
// output because the Video Present Target is in spanning mode.
//
...
//
// The DxgkDdiOpmCreateProtectedOutput function could not create a protected
// output because the Video Present Target is in theater mode.
//
...
// MessageText:
//
// The function failed because the display adapter's Hardware Functionality Scan
// failed to validate the graphics hardware.
//
...
// MessageText:
//
// The HDCP System Renewability Message passed to this function did not comply
...
// MessageText:
//
// The protected output cannot enable the High-bandwidth Digital Content
...
// MessageText:
//
// The protected output cannot enable Analogue Copy Protection (ACP) because it
...
// MessageText:
//
// The protected output cannot enable the Content Generation Management System
// Analogue (CGMS-A) protection technology because it does not support CGMS-A.
//
#define STATUS_GRAPHICS_OPM_OUTPUT_DOES_NOT_SUPPORT_CGMSA                      \
  ((NTSTATUS)0xC01E0515L)
...
// MessageText:
//
// The DxgkDdiOPMGetInformation function cannot return the version of the SRM
// being used because the application never successfully passed an SRM to the
// protected output.
//
...
// MessageText:
//
// The DxgkDdiOPMConfigureProtectedOutput function cannot enable the specified
// output protection technology because the output's screen resolution is too
// high.
//
...
// MessageText:
//
// The DxgkDdiOPMConfigureProtectedOutput function cannot enable HDCP because
// the display adapter's HDCP hardware is already being used by other physical
// outputs.
//
#define STATUS_GRAPHICS_OPM_ALL_HDCP_HARDWARE_ALREADY_IN_USE                   \
  ((NTSTATUS)0xC01E0518L)
...
// functions return this error code if the passed in sequence number is not the
...
// MessageText:
//
// The function failed because an unexpected error occurred inside of a display
// driver.
//
...
// driver does not support the DXGKMDT_OPM_GET_ACP_AND_CGMSA_SIGNALING and
...
// MessageText:
//
// The DxgkDdiOPMConfigureProtectedOutput function returns this error code if
...
// MessageText:
//
// The function failed because a monitor returned an invalid Timing Status byte
// when the operating system used the DDC/CI Get Timing Report & Timing Message
// command to get a timing report from a monitor.
//
#define STATUS_GRAPHICS_DDCCI_MONITOR_RETURNED_INVALID_TIMING_STATUS_BYTE      \
  ((NTSTATUS)0xC01E0586L)
...
// MessageText:
//
// An error occurred because the field length of a DDC/CI message contained an
// invalid value.
//
...
// MessageText:
//
// The operating system asynchronously destroyed the monitor which corresponds
// to this handle because the operating system's state changed. This error
// typically occurs because the monitor PDO associated with this handle was
// removed, the monitor PDO associated with this handle was stopped, or a
// display mode change occurred. A display mode change occurs when windows sends
// a WM_DISPLAYCHANGE windows message to applications.
//
...
// MessageText:
//
// This function can only be used if a program is running in the local console
// session. It cannot be used if a program is running on a remote desktop
// session or on a terminal server session.
//
...
// MessageText:
//
// This function cannot find an actual GDI display device which corresponds to
// the specified GDI display device name.
//
#define STATUS_GRAPHICS_NO_DISPLAY_DEVICE_CORRESPONDS_TO_NAME                  \
  ((NTSTATUS)0xC01E05E1L)
...
// MessageText:
//
// The function failed because the specified GDI display device was not attached
// to the Windows desktop.
//
#define STATUS_GRAPHICS_DISPLAY_DEVICE_NOT_ATTACHED_TO_DESKTOP                 \
  ((NTSTATUS)0xC01E05E2L)
...
// mirroring display devices do not have any physical monitors associated with
// them.
//
...
// MessageText:
//
// An array passed to the function cannot hold all of the data that the function
// must copy into the array.
//
...
// MessageText:
//
// The function failed because the current session is changing its type. This
// function cannot be called when the current session is changing its type.
// There are currently three types of sessions: console, disconnected and
// remote.
//
...
// MessageText:
//
// The Basic Display Driver cannot start because there is no frame buffer found,
// and the system cannot determine if the system is BIOS-compatible.
//
#define STATUS_GRAPHICS_UNKNOWN_BIOS_FRAME_BUFFER_NOT_FOUND                    \
  ((NTSTATUS)0xC01E0600L)
...
//
// The Basic Display Driver cannot start because there is no frame buffer found
// from UEFI or from a previously running graphics driver.
//
...
//
// The remaining NTSTATUS in camera category may not have mapping with System
// Error code.
//
...
// MessageText:
//
// The file system size is larger than the partition size in the partition
// table.
//
...
// MessageText:
//
// The BitLocker encryption key could not be obtained from the startup key or
// recovery password.
//
...
// MessageText:
//
// The authorization data for the Storage Root Key (SRK) of the Trusted Platform
// Module (TPM) is not zero.
//
...
// MessageText:
//
// The system boot information changed or the Trusted Platform Module (TPM)
// locked out access to BitLocker encryption keys until the computer is
// restarted.
//
...
// MessageText:
//
// The BitLocker encryption key could not be obtained from the Trusted Platform
// Module (TPM).
//
...
// MessageText:
//
// A boot application hash does not match the hash computed when BitLocker was
// turned on.
//
...
// MessageText:
//
// The Boot Configuration Data (BCD) settings are not supported or have changed
// since BitLocker was enabled.
//
...
// MessageText:
//
// The auto-unlock master key was not available from the operating system
...
// MessageText:
//
// This feature of BitLocker Drive Encryption is not included with this version
// of Windows.
//
...
// MessageText:
//
// Bitlocker Drive Encryption failed to recover from aborted conversion. This
// could be due to either all conversion logs being corrupted or the media being
// write-protected.
//
...
// MessageText:
//
// The management information stored on the drive contained an unknown type. If
// you are using an old version of Windows, try accessing the drive from the
// latest version.
//
...
//
// The BitLocker encryption key could not be obtained from the Trusted Platform
// Module (TPM) and enhanced PIN. Try using a PIN containing only numerals.
//
...
// MessageText:
//
// BitLocker Drive Encryption only supports Used Space Only encryption on thin
// provisioned storage.
//
#define STATUS_FVE_FULL_ENCRYPTION_NOT_ALLOWED_ON_TP_STORAGE                   \
  ((NTSTATUS)0xC0210032L)
...
// MessageText:
//
// BitLocker Drive Encryption does not support wiping free space on thin
// provisioned storage.
//
...
// MessageText:
//
// This command can only be performed from the coordinator node for the
// specified CSV volume.
//
...
//
#define STATUS_FVE_NOT_ALLOWED_TO_UPGRADE_WHILE_CONVERTING                     \
  ((NTSTATUS)0xC0210036L)
...
// MessageText:
//
// Your computer doesn't support BitLocker hardware-based encryption. Check with
// your computer manufacturer for firmware updates.
//
...
// MessageText:
//
// Secure Boot has been disabled. Either Secure Boot must be re-enabled, or
// BitLocker must be suspended for Windows to start normally.
//
...
// MessageText:
//
// Device encryption removal is blocked due to volume being extended beyond its
// original size.
//
...
// MessageText:
//
// This action isn't supported because this drive isn't automatically managed
...
// MessageText:
//
// BitLocker Drive Encryption has been suspended on this drive. All BitLocker
// key protectors configured for this drive are effectively disabled, and the
// drive will be automatically unlocked using an unencrypted (clear) key.
//
...
// MessageText:
//
// BitLocker Drive Encryption policy does not allow KSR operation with protected
// OS volume.
//
...
// MessageText:
//
// BitLocker can't enable encryption on this hardware encrypting drive volume
// because the drive bands couldn't be enumerated.
//
...
// MessageText:
//
// This removable data drive has been excluded from device encryption by
...
// MessageText:
//
// This operation cannot be completed because BitLocker Drive Encryption
// metadata area is full. Consider removing unnecessary key protectors for this
// drive.
//
...
// MessageText:
//
// The operation failed because BitLocker Drive Encryption cannot suspend
// protection due to policy.
//
...
// MessageText:
//
// The datum is invalid in its current state, but may be fixed so that it is
// valid.
//
...
// MessageText:
//
// The TPM protector contains conflicting information about whether the binding
// is a Secure Boot binding.
//
...
// MessageText:
//
// The dataset contains conflicting information about whether the TPM protector
// binding is a Secure Boot binding.
//
...
// MessageText:
//
// The dataset contains a TPM binding without associated binding information.
// Try removing and re-adding the TPM protector.
//
...
// MessageText:
//
// The dataset contains TPM binding information incorrectly associated to
// multiple TPM bindings. Try re-enabling BitLocker.
//
...
// MessageText:
//
// BitLocker failed to obtain the key from the TPM due to an unforseen TPM
// failure.
//
...
// MessageText:
//
// BitLocker prevented an attempt to create a TPM binding for a PCR that
// contains events extended into the TPM after the BitLocker boot lock event.
//
...
// MessageText:
//
// The firmware update was not applied because BitLocker TPM bindings do not
// include the latest measurements. Restart your computer to retry.
//
...
// MessageText:
//
// The dataset contains binding information without an associated TPM binding.
// Try re-enabling BitLocker.
//
...
// MessageText:
//
// BitLocker failed to find any TPM bindings that can be used to unlock the
// drive.
//
...
// MessageText:
//
// The firmware update was not applied. To finish installation of the firmware,
// please suspend BitLocker and restart your device.
//
...
// MessageText:
//
// The firmware update was not applied because TPM PCRs affected by the firmware
// installation were not excluded by BitLocker's TPM protector. Restart your
// computer to retry.
//
...
// MessageText:
//
// FIPS compliance is required, but the hardware crypto accelerator does not
// report compliance with this standard.
//
#define STATUS_FVE_HARDWARE_CRYPTO_ACCELERATOR_NOT_FIPS_COMPLIANT              \
  ((NTSTATUS)0xC021005FL)
...
// MessageText:
//
// FIPS compliance is required, but the hardware crypto key manager does not
// report compliance with this standard.
//
#define STATUS_FVE_HARDWARE_CRYPTO_KEY_MANAGER_NOT_FIPS_COMPLIANT              \
  ((NTSTATUS)0xC0210060L)
...
// MessageText:
//
// The IKEEXT service is not running.  This service only runs when there is
// IPsec policy applied to the machine.
//
...
// MessageText:
//
// An attempt was made to remove a token ring group address that is in use by
// other components.
//
...
// MessageText:
//
// The I/O operation failed because network media is disconnected or wireless
...
// MessageText:
//
// The current state of the specified port on this network interface does not
...
// MessageText:
//
// The wireless local area network interface is in auto configuration mode and
// doesn't support the requested parameter change operation.
//
...
// MessageText:
//
// The wireless local area network interface is powered down and doesn't support
...
// MessageText:
//
// The wireless local area network interface cannot start an AP on the specified
// channel right now.
//
#define STATUS_NDIS_DOT11_AP_CHANNEL_CURRENTLY_NOT_AVAILABLE                   \
  ((NTSTATUS)0xC0232005L)
...
// band right now.
//
#define STATUS_NDIS_DOT11_AP_BAND_CURRENTLY_NOT_AVAILABLE                      \
  ((NTSTATUS)0xC0232006L)
...
// MessageText:
//
// The wireless local area network interface cannot start an AP on this channel
...
// MessageText:
//
// The wireless local area network interface cannot start an AP on this band due
...
// MessageText:
//
// The wireless local area network interface cannot start an AP because the
// minimum radio hardware required for this operation is not present.
//
...
// MessageText:
//
// TPM 1.2: An operation completed successfully but the auditing of that
// operation failed.
//
...
// MessageText:
//
// TPM 1.2: The clear disable flag is set and all clear operations now require
// physical access.
//
...
// MessageText:
//
// TPM 1.2: The TPM has insufficient internal resources to perform the requested
// action.
//
...
// MessageText:
//
// TPM 1.2: The calculation is unable to proceed because the existing SHA-1
...
// MessageText:
//
// TPM 1.2: The TPM hardware device reported a failure during its internal self
// test. Try restarting the computer to resolve the problem. If the problem
...
// MessageText:
//
// TPM 1.2: The authorization for the second key in a 2 key function failed
// authorization.
//
...
// MessageText:
//
// TPM 1.2: The command was received in the wrong sequence relative to TPM_Init
...
// MessageText:
//
// TPM 1.2: The signature or encryption scheme for this key is incorrect or not
...
// MessageText:
//
// TPM 1.2: The size of the data (or blob) parameter is bad or inconsistent with
// the referenced key.
//
...
// MessageText:
//
// TPM 1.2: A mode parameter is bad, such as capArea or subCapArea for
...
// MessageText:
//
// TPM 1.2: Either the physicalPresence or physicalPresenceLock bits have the
// wrong value.
//
...
// MessageText:
//
// TPM 1.2: TPM audit construction failed and the underlying command was
...
// MessageText:
//
// TPM 1.2: Attempt to reset a PCR register that does not have the resettable
// attribute.
//
...
// MessageText:
//
// TPM 1.2: Attempt to reset a PCR register that requires locality and locality
...
// MessageText:
//
// TPM 1.2: When saving context identified resource type does not match actual
// resource.
//
...
// MessageText:
//
// TPM 1.2: The TPM is attempting to execute a command that is not allowed when
// in FIPS mode.
//
...
// MessageText:
//
// TPM 1.2: The key is under control of the TPM Owner and can only be evicted by
// the TPM Owner.
//
...
// MessageText:
//
// TPM 1.2: There was a command executed outside of an exclusive transport
// session.
//
...
// MessageText:
//
// TPM 1.2: The atomic process indicated by the submitted DAA command is not the
// expected process.
//
...
// MessageText:
//
// TPM 2.0: Public and sensitive portions of an object are not cryptographically
// bound.
//
...
// MessageText:
//
// TPM 2.0: A _TPM_Init and Startup(CLEAR) is required before the TPM can resume
// operation.
//
...
// balanced. The digest size of the hash must be larger than the key size of the
// symmetric algorithm.
//
...
// MessageText:
//
// TPM 2.0: The TPM command's commandSize value is inconsistent with contents of
// the command buffer; either the size is not the same as the bytes loaded by
...
// MessageText:
//
// A new TPM context could not be created because there are too many open
// contexts.
//
...
// MessageText:
//
// The caller is trying to request an operation while the TPM driver has an
// exclusive context open.
//
...
// MessageText:
//
// The TPM driver was previously in exclusive mode and now requires a reboot to
...
// MessageText:
//
// This is an error mask to convert Platform Crypto Provider errors to win
// errors.
//
...
// MessageText:
//
// The Platform Crypto Device is currently not ready. It needs to be fully
// provisioned to be operational.
//
...
// MessageText:
//
// The buffer is too small to contain all data. No information has been written
// to the buffer.
//
...
// MessageText:
//
// The Platform Crypto Device has ignored the authorization for the provider
// object, to mitigate against a dictionary attack.
//
...
// MessageText:
//
// A Platform Crypto Device was not found.  Operations that require a Platform
// Crypto Device will not be submitted.
//
...
// MessageText:
//
// An attempt was made to import or load a key under an incorrect storage
// parent.
//
...
// MessageText:
//
// The RSA key creation is blocked on this TPM due to known security
// vulnerabilities.
//
...
// MessageText:
//
// The TPM key's handle was unexpectedly invalidated due to a hardware or
// firmware issue.
//
...
// MessageText:
//
// The requested salt size for signing with RSAPSS does not match what the TPM
// uses.
//
...
// MessageText:
//
// The remote TPM context exchange is not complete. The context should be
// transported to the target and continued.
//
...
// MessageText:
//
// The TPM returned incomplete PCR results. This maybe due to an unsupported
// selection set. Attempt the read again with a different selection set.
//
...
// MessageText:
//
// The rTPM target does not support remote processing of the specified TPM
// command.
//
...
// MessageText:
//
// TPM related network operations are blocked as Zero Exhaust mode is enabled on
// client.
//
...
// MessageText:
//
// There are no authorizations for the current running MLE. The current state is
// not authorized.
//
...
// MessageText:
//
// The physical connection being used for debugging has not recorded any receive
// activity since the last operation.
//
...
// MessageText:
//
// The hypervisor could not perform the operation because it is not allowed with
...
// MessageText:
//
// There is not enough contiguous memory in the partition's pool to complete the
// operation.
//
...
// MessageText:
//
// There is not enough memory in the partition's pool to complete the mirroring
// operation.
//
...
// mirroring operation.
//
#define STATUS_HV_INSUFFICIENT_CONTIGUOUS_MEMORY_MIRRORING                     \
  ((NTSTATUS)0xC0350082L)
...
// MessageText:
//
// There is not enough contiguous memory in the root partition's pool to
...
//
#define STATUS_HV_INSUFFICIENT_CONTIGUOUS_ROOT_MEMORY_MIRRORING                \
  ((NTSTATUS)0xC0350085L)
...
// Infrastructure Driver (VID) and other components
...
// MessageText:
//
// The handler for the virtualization infrastructure driver is already
// registered. Restarting the virtual machine may fix the problem. If the
...
// driver exceeded the maximum. Restarting the virtual machine may fix the
...
// MessageText:
//
// The message queue for the virtualization infrastructure driver is full and
// cannot accept new messages. Restarting the virtual machine may fix the
// problem. If the problem persists, try restarting the physical computer.
//
...
// driver. Restarting the virtual machine may fix the problem. If the problem
...
// MessageText:
//
// The name of the partition or message queue for the virtualization
// infrastructure driver is invalid. Restarting the virtual machine may fix the
// problem. If the problem persists, try restarting the physical computer.
//
...
// MessageText:
//
// The partition name of the virtualization infrastructure driver exceeds the
// maximum.
//
...
// MessageText:
//
// The message queue name of the virtualization infrastructure driver exceeds
// the maximum.
//
...
// because another partition with the same name already exists.
//
...
// MessageText:
//
// The virtualization infrastructure driver has encountered an error. The
// requested partition does not exist. Restarting the virtual machine may fix
...
// MessageText:
//
// The virtualization infrastructure driver has encountered an error. Could not
// find the requested partition. Restarting the virtual machine may fix the
// problem. If the problem persists, try restarting the physical computer.
//
...
// MessageText:
//
// A message queue with the same name already exists for the virtualization
...
// MessageText:
//
// The memory block page for the virtualization infrastructure driver cannot be
// mapped because the page map limit has been reached. Restarting the virtual
// machine may fix the problem. If the problem persists, try restarting the
// physical computer.
//
...
// MessageText:
//
// The memory block for the virtualization infrastructure driver is still being
// used and cannot be destroyed.
//
...
// MessageText:
//
// Cannot unlock the page array for the guest operating system memory address
// because it does not match a previous lock request. Restarting the virtual
// machine may fix the problem. If the problem persists, try restarting the
// physical computer.
//
...
// MessageText:
//
// The non-uniform memory access (NUMA) node settings do not match the system
// NUMA topology. In order to start the virtual machine, you will need to modify
...
// MessageText:
//
// The non-uniform memory access (NUMA) node index does not match a valid index
...
// MessageText:
//
// The memory block for the virtualization infrastructure driver is already
...
// MessageText:
//
// The handle is not a valid memory block handle for the virtualization
// infrastructure driver.
//
...
// MessageText:
//
// The request exceeded the memory block page limit for the virtualization
// infrastructure driver. Restarting the virtual machine may fix the problem. If
...
// MessageText:
//
// The handle is not a valid message queue handle for the virtualization
// infrastructure driver.
//
...
// MessageText:
//
// The handle is not a valid page range handle for the virtualization
// infrastructure driver.
//
...
// MessageText:
//
// Cannot install client notifications because no message queue for the
// virtualization infrastructure driver is associated with the memory block.
//
...
// MessageText:
//
// The request to lock or map a memory block page failed because the
// virtualization infrastructure driver memory block limit has been reached.
// Restarting the virtual machine may fix the problem. If the problem persists,
...
// MessageText:
//
// The handle is not a valid parent partition mapping handle for the
...
// MessageText:
//
// The message queue for the virtualization infrastructure driver has been
// closed. Restarting the virtual machine may fix the problem. If the problem
// persists, try restarting the physical computer.
//
...
// MessageText:
//
// Cannot add a virtual processor to the partition because the maximum has been
// reached.
//
...
// MessageText:
//
// Invalid state for the virtual processor. Restarting the virtual machine may
// fix the problem. If the problem persists, try restarting the physical
// computer.
//
...
// MessageText:
//
// The maximum number of kernel mode clients for the virtualization
// infrastructure driver has been reached. Restarting the virtual machine may
// fix the problem. If the problem persists, try restarting the physical
// computer.
//
...
// MessageText:
//
// This kernel mode interface for the virtualization infrastructure driver has
// already been initialized. Restarting the virtual machine may fix the problem.
...
// MessageText:
//
// Cannot set or reset the memory block property more than once for the
// virtualization infrastructure driver. Restarting the virtual machine may fix
// the problem. If the problem persists, try restarting the physical computer.
//
...
// MessageText:
//
// The memory mapped I/O for this page range no longer exists. Restarting the
// virtual machine may fix the problem. If the problem persists, try restarting
...
// address. Restarting the virtual machine may fix the problem. If the problem
// persists, try restarting the physical computer.
//
...
// MessageText:
//
// Cannot destroy or reuse the reserve page set for the virtualization
// infrastructure driver because it is in use. Restarting the virtual machine
// may fix the problem. If the problem persists, try restarting the physical
// computer.
//
...
// MessageText:
//
// The reserve page set for the virtualization infrastructure driver is too
// small to use in the lock request. Restarting the virtual machine may fix the
// problem. If the problem persists, try restarting the physical computer.
//
...
// MessageText:
//
// Cannot lock or map the memory block page for the virtualization
// infrastructure driver because it has already been locked using a reserve page
// set page. Restarting the virtual machine may fix the problem. If the problem
// persists, try restarting the physical computer.
//
#define STATUS_VID_MBP_ALREADY_LOCKED_USING_RESERVED_PAGE                      \
...
// MessageText:
//
// Cannot create the memory block for the virtualization infrastructure driver
// because the requested number of pages exceeded the limit. Restarting the
// virtual machine may fix the problem. If the problem persists, try restarting
// the physical computer.
//
...
// MessageText:
//
// Cannot restore this virtual machine because the saved state data cannot be
// read. Delete the saved state data and then try to start the virtual machine.
//
...
// MessageText:
//
// Cannot restore this virtual machine because an item read from the saved state
// data is not recognized. Delete the saved state data and then try to start the
// virtual machine.
//
...
// MessageText:
//
// Cannot restore this virtual machine to the saved state because of hypervisor
// incompatibility. Delete the saved state data and then try to start the
// virtual machine.
//
...
// MessageText:
//
// Failed to allocate memory for the physical buffer used to back certain
// internal structures.
//
#define STATUS_VID_INSUFFICIENT_RESOURCES_PHYSICAL_BUFFER                      \
  ((NTSTATUS)0xC037002CL)
...
// MessageText:
//
// A virtual machine is running with its memory allocated across multiple NUMA
// nodes. This does not indicate a problem unless the performance of your
// virtual machine is unusually slow. If you are experiencing performance
// problems, you may need to modify the NUMA configuration.
//
...
// MessageText:
//
// The regeneration operation was not able to copy all data from the active
// plexes due to bad sectors.
//
...
// MessageText:
//
// One or more disks were not fully migrated to the target pack. They may or may
// not require reimport after fixing the hardware problems.
//
...
// MessageText:
//
// The specified disk id is invalid. There are no disks with the specified disk
// id.
//
...
// MessageText:
//
// The specified disk is an invalid disk. Operation cannot complete on an
// invalid disk.
//
...
// MessageText:
//
// The disk layout contains non-basic partitions which appear after basic
// partitions. This is an invalid disk layout.
//
#define STATUS_VOLMGR_DISK_LAYOUT_NON_BASIC_BETWEEN_BASIC_PARTITIONS           \
  ((NTSTATUS)0xC038000CL)
...
// MessageText:
//
// The disk layout contains primary partitions in between logical drives. This
// is an invalid disk layout.
//
#define STATUS_VOLMGR_DISK_LAYOUT_PRIMARY_BETWEEN_LOGICAL_PARTITIONS           \
  ((NTSTATUS)0xC038000FL)
...
// MessageText:
//
// The disk layout contains more than the maximum number of supported
// partitions.
//
...
// MessageText:
//
// The specified disk is missing. The operation cannot complete on a missing
// disk.
//
...
// MessageText:
//
// The specified disk set contains volumes which exist on disks outside of the
// set.
//
...
// MessageText:
//
// A disk in the volume layout provides extents to more than one member of a
// plex.
//
...
// MessageText:
//
// The specified volume is retained and can only be extended into a contiguous
// extent. The specified extent to grow the volume is not contiguous with the
// specified volume.
//
...
// MessageText:
//
// The specified partition overlaps an EBR (the first track of an extended
// partition on a MBR disks).
//
...
// MessageText:
//
// The specified extent lengths cannot be used to construct a volume with
// specified length.
//
...
// MessageText:
//
// The specified member is already in-sync with the other active members. It
...
// MessageText:
//
// The specified member index is greater or equal than the number of members in
// the volume plex.
//
...
// MessageText:
//
// The specified member is not detached. Cannot replace a member which is not
// detached.
//
...
// MessageText:
//
// There are currently no registered users for notifications. The task number is
// irrelevant unless there are registered users.
//
...
// MessageText:
//
// The specified notification user does not exist. Failed to unregister user for
// notifications.
//
...
// MessageText:
//
// The notifications have been reset. Notifications for the current user are
// invalid. Unregister and re-register for notifications.
//
...
// MessageText:
//
// The specified pack id is invalid. There are no packs with the specified pack
// id.
//
...
// MessageText:
//
// The specified pack is the invalid pack. The operation cannot complete with
// the invalid pack.
//
...
// MessageText:
//
// The specified disk has an unsupported partition style. Only MBR and GPT
// partition styles are supported.
//
...
// MessageText:
//
// The specified plex is already in-sync with the other active plexes. It does
...
// MessageText:
//
// The specified plex index is greater or equal than the number of plexes in the
// volume.
//
...
// MessageText:
//
// The specified plex is the last active plex in the volume. The plex cannot be
// removed or else the volume will go offline.
//
...
// MessageText:
//
// There is already a pending request for notifications. Wait for the existing
// request to return before requesting for more notifications.
//
...
// MessageText:
//
// The specified volume id is invalid. There are no volumes with the specified
// volume id.
//
...
//
#define STATUS_VOLMGR_VOLUME_LENGTH_NOT_SECTOR_SIZE_MULTIPLE                   \
  ((NTSTATUS)0xC0380048L)
...
// MessageText:
//
// The virtual hard disk is corrupted. The virtual hard disk drive footer is
// missing.
//
...
// MessageText:
//
// The virtual hard disk is corrupted. The virtual hard disk drive footer in the
// virtual hard disk is corrupted.
//
...
// MessageText:
//
// The virtual hard disk is corrupted. The sparse header checksum does not match
...
// MessageText:
//
// The system does not support this version of the virtual hard disk.This
// version of the sparse header is not supported.
//
...
// MessageText:
//
// The virtual hard disk is corrupted. The sparse header in the virtual hard
// disk is corrupt.
//
...
// MessageText:
//
// Failed to write to the virtual hard disk failed because the system failed to
// allocate a new block in the virtual hard disk.
//
...
// MessageText:
//
// The virtual hard disk is corrupted. The block allocation table in the virtual
...
// MessageText:
//
// The system does not support this version of the virtual hard disk. The block
// size is invalid.
//
...
// MessageText:
//
// The virtual hard disk is corrupted. The block bitmap does not match with the
// block data present in the virtual hard disk.
//
...
// MessageText:
//
// The chain of virtual hard disks is broken. The system cannot locate the
// parent virtual hard disk for the differencing disk.
//
...
// identifiers of the parent virtual hard disk and differencing disk.
//
...
// MessageText:
//
// The chain of virtual hard disks is corrupted. The time stamp of the parent
// virtual hard disk does not match the time stamp of the differencing disk.
//
...
// granted access rights to the parent virtual hard disk for the differencing
// disk.
//
...
// MessageText:
//
// The chain of virtual hard disks is inaccessible. There was an error opening a
...
// MessageText:
//
// The requested operation cannot be performed on the virtual disk in its
// current state.
//
...
// MessageText:
//
// The sector size of the physical disk on which the virtual disk resides is not
// supported.
//
...
// MessageText:
//
// Change Tracking for the virtual disk is not in a valid state to perform this
// request.  Change tracking could be discontinued or already in the requested
// state.
//
...
// MessageText:
//
// There is not enough space in the virtual disk file for the provided metadata
// item.
//
...
// MessageText:
//
// Change tracking is disabled for the specified virtual hard disk, so no change
...
// MessageText:
//
// There is no change tracking data available associated with the specified
...
// MessageText:
//
// The requested resize operation might truncate user data residing on the
// virtual disk.
//
...
// MessageText:
//
// The minimum safe size of the virtual disk could not be determined. This may
// be due to a missing or corrupt partition table.
//
#define STATUS_VHD_COULD_NOT_COMPUTE_MINIMUM_VIRTUAL_SIZE                      \
  ((NTSTATUS)0xC03A0032L)
...
//
#define STATUS_VHD_ALREADY_AT_OR_BELOW_MINIMUM_VIRTUAL_SIZE                    \
  ((NTSTATUS)0xC03A0033L)
...
// MessageText:
//
// The Resume Key Filter could not find the resume key supplied for the
// operation.
//
...
// MessageText:
//
// The Resume Key Filter found an existing resume key that matches the one
// supplied for the handle.
//
...
// MessageText:
//
// The Resume Key Filter data blob attached to the handle is full. No more space
// is available.
//
...
// MessageText:
//
// The Resume Key Filter handle store is full. No more resume handles can be
// accepted.
//
...
// MessageText:
//
// The Resume Key Filter failed the operation because the file is temporarily
// blocked pending the resume of existing handles on the file.
//
...
//
// The Resume Key Filter found an existing resume key that matches the one
// supplied on a handle that's active/open. The operation requires an
// inactive/closed handle.
//
...
// MessageText:
//
// A new Secure Boot policy did not contain the current publisher on its update
// list.
//
...
// MessageText:
//
// The Secure Boot policy is either not signed or is signed by a non-trusted
// signer.
//
...
// MessageText:
//
// Secure Boot requires that certain files and drivers are not replaced by other
// files or drivers.
//
...
//
#define STATUS_SECUREBOOT_POLICY_MISSING_ANTIROLLBACKVERSION                   \
  ((NTSTATUS)0xC043000AL)
...
// MessageText:
//
// The Platform ID specified in the Secure Boot policy does not match the
// Platform ID on this device.
//
...
// MessageText:
//
// The Secure Boot policy file has an older Antirollback Version than this
// device.
//
...
// MessageText:
//
// Supplemental Secure Boot policy file can not be loaded as a base Secure Boot
// policy.
//
...
// MessageText:
//
// Base Secure Boot policy file can not be loaded as a Supplemental Secure Boot
// policy.
//
...
// MessageText:
//
// The file is not authorized on this platform because an entry was not found in
...
// MessageText:
//
// The catalog is not authorized on this platform because an entry was not found
...
// MessageText:
//
// The file is not authorized on this platform because a Binary ID was not found
...
// MessageText:
//
// The Application Control policy is either not signed or is signed by a
// non-trusted signer.
//
...
// MessageText:
//
// The Application Control supplemental policy is not authorized by an
// Application Control base policy.
//
#define STATUS_SYSTEM_INTEGRITY_SUPPLEMENTAL_POLICY_NOT_AUTHORIZED             \
  ((NTSTATUS)0xC0E90006L)
...
// MessageText:
//
// An Application Control policy has blocked this file.  Potentially unwanted
// application.
//
...
// MessageText:
//
// An Application Control policy has blocked this file.  Unable to contact
// reputation service for unknown file.
//
...
//
#define STATUS_SYSTEM_INTEGRITY_REPUTATION_UNFRIENDLY_FILE                     \
  ((NTSTATUS)0xC0E9000BL)
...
// MessageText:
//
// An Application Control policy has blocked this file.  Failed to obtain file
// reputation because an infrastructure issue occurred. Try again later.
//
...
//
#define STATUS_SYSTEM_INTEGRITY_REPUTATION_EXPLICIT_DENY_FILE                  \
  ((NTSTATUS)0xC0E9000DL)
...
// MessageText:
//
// An Application Control policy has blocked this file.  File did not pass
// WHQL-only requirements.
//
...
// MessageText:
//
// CLiP license device ID does not match the device ID in the bound device
// license.
//
...
// MessageText:
//
// PortCls could not find an audio engine node exposed by a miniport driver
// claiming support for IMiniportAudioEngineNode.
//
...
// MessageText:
//
// The specified fault domain type or combination of minimum / maximum fault
// domain type is not valid.
//
...
// MessageText:
//
// The number of data copies requested is outside of the supported range of
// values.
//
...
// MessageText:
//
// Activation of persistent snapshots on this volume took longer than was
// allowed.
//
...
// MessageText:
//
// The operation was preempted by a higher priority operation. It must be
// resumed later.
//
...
// MessageText:
//
// The data capacity of the device has changed, resulting in a Unit Attention
// condition.
//
#define STATUS_SVHDX_UNIT_ATTENTION_CAPACITY_DATA_CHANGED                      \
  ((NTSTATUS)0xC05CFF02L)
...
// MessageText:
//
// A previous operation resulted in this initiator's reservations being
// preempted, resulting in a Unit Attention condition.
//
#define STATUS_SVHDX_UNIT_ATTENTION_RESERVATIONS_PREEMPTED                     \
  ((NTSTATUS)0xC05CFF03L)
...
//
// A previous operation resulted in this initiator's reservations being
// released, resulting in a Unit Attention condition.
//
#define STATUS_SVHDX_UNIT_ATTENTION_RESERVATIONS_RELEASED                      \
  ((NTSTATUS)0xC05CFF04L)
...
// MessageText:
//
// A previous operation resulted in this initiator's registrations being
// preempted, resulting in a Unit Attention condition.
//
#define STATUS_SVHDX_UNIT_ATTENTION_REGISTRATIONS_PREEMPTED                    \
  ((NTSTATUS)0xC05CFF05L)
...
// MessageText:
//
// The data storage format of the device has changed, resulting in a Unit
// Attention condition.
//
#define STATUS_SVHDX_UNIT_ATTENTION_OPERATING_DEFINITION_CHANGED               \
  ((NTSTATUS)0xC05CFF06L)
...
// MessageText:
//
// The current initiator is not allowed to perform the SCSI command because of a
...
// MessageText:
//
// Multiple virtual machines sharing a virtual hard disk is supported only on
// Fixed or Dynamic VHDX format virtual hard disks.
//
...
// MessageText:
//
// The requested operation cannot be performed on the virtual disk as it is
// currently used in shared mode.
//
...
// MessageText:
//
// Invalid Shared VHDX open due to lack of initiator ID. Check for related
// Continuous Availability failures.
//
...
// MessageText:
//
// The requested operation failed. The Remote Mailslot Protocol has been
// disabled.
//
...
// MessageText:
//
// You can't access this shared folder because your computer is configured to
// require SMB signing. These policies help protect your PC from unsafe or
...
// MessageText:
//
// The SMB client was denied access to the SMB server during mutual
// authentication.
//
...
// MessageText:
//
// Virtual Secure Mode (VSM) is not initialized. The hypervisor or VSM may not
// be present or enabled.
//
...
// MessageText:
//
// The hypervisor is not protecting DMA because an IOMMU is not present or not
// enabled in the BIOS.
//
...
// MessageText:
//
// The VSM encryption key could not be released because the minimum Code
// Integrity policy version was not met.
//
...
// MessageText:
//
// The condition supplied for the app execution request was not satisfied, so
...
// MessageText:
//
// The supplied handle has been invalidated and may not be used for the
// requested operation.
//
...
// MessageText:
//
// The supplied host generation has been invalidated and may not be used for the
// requested operation.
//
...
// MessageText:
//
// An attempt to register a process failed because the target host was not in a
// valid state to receive process registrations.
//
...
// MessageText:
//
// The operation was not completed because a required resource donor was not
// found for the host.
//
...
// MessageText:
//
// The operation was not completed because the specified user was not known to
// the service.
//
...
// MessageText:
//
// The caller specified wait timed out before the operation completed because a
// host termination is in queued.
//
...
//
// The caller specified wait timed out before the operation completed because a
// licensing operation is being performed.
//
...
// MessageText:
//
// The caller specified wait timed out before the operation completed because
...
// MessageText:
//
// The QUIC connection encountered a certificate with an unkown certificate
// authority during TLS.
//
...
// MessageText:
//
// The completion queue does not have enough free space, to post completions,
// for all entries being submitted.
//
...
// MessageText:
//
// The PRM update service provided module version does not match that present in
// the actual update module header.
//
...
//
#define STATUS_AAD_CLOUDAP_E_BAD_DEVICE_ACCESS_TOKEN_FORMAT                    \
  ((NTSTATUS)0xC0048551L)
...
// MessageText:
//
// The Entra ID tenant specified in the server nonce or the assertion is
// invalid.
//
...
// MessageText:
//
// The binding key specified in the assertion does not match the binding key
...
// MessageText:
//
// The root certificate associated with the access token's signature cannot be
// found.
//
...
// MessageText:
//
// The resource URL in the assertion does not match the audience specified in
// the access token.
//
...
// MessageText:
//
// The specified resource ID does not match the audience specified in the access
// token.
//
