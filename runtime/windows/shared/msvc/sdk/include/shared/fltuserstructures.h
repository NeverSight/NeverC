...
typedef enum _FLT_FILESYSTEM_TYPE {
  ...
  FLT_FSTYPE_ROXIO_UDF1, // Roxio's UDF writeable file system
                         // (\FileSystem\cdudf_xp)
  FLT_FSTYPE_ROXIO_UDF2, // Roxio's UDF readable file system
                         // (\FileSystem\UdfReadr_xp)
  ...
  FLT_FSTYPE_CSVFS,    // Microsoft's Cluster Shared Volume file system
                       // (\FileSystem\csvfs)
  FLT_FSTYPE_REFS,     // Microsoft's ReFS file system      (\FileSystem\Refs or
                       // \FileSystem\Refsv1)
...
//
//  The structures for the information returned from the query of the
//  information on the Instance.  All strings are UNICODE strings.  All Lengths
//  are in bytes
//

typedef _Struct_size_bytes_(
    sizeof(INSTANCE_BASIC_INFORMATION) *
...
typedef _Struct_size_bytes_(
    sizeof(INSTANCE_PARTIAL_INFORMATION) + InstanceNameLength +
    AltitudeLength) struct _INSTANCE_PARTIAL_INFORMATION {
...
typedef _Struct_size_bytes_(
    sizeof(INSTANCE_FULL_INFORMATION) + InstanceNameLength + AltitudeLength +
    VolumeNameLength + FilterNameLength) struct _INSTANCE_FULL_INFORMATION {
...
#define FLTFL_IASIM_DETACHED_VOLUME 0x00000001
      ...
      //
      //  The name of the minifilter associated with this instace (unicode
      //  string)
      //
...
#define FLTFL_IASIL_DETACHED_VOLUME 0x00000001
      ...
      //
      //  The name of the filter associated with this attachment (unicode
      //  string)
      //
