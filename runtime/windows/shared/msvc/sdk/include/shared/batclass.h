...
_Function_class_(BCLASS_QUERY_TAG_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_QUERY_TAG_CALLBACK)(
    _In_ PVOID Context, _Out_ PULONG BatteryTag);

typedef BCLASS_QUERY_TAG_CALLBACK *PBCLASS_QUERY_TAG_CALLBACK;

_Function_class_(BCLASS_QUERY_INFORMATION_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_QUERY_INFORMATION_CALLBACK)(
    _In_ PVOID Context, _In_ ULONG BatteryTag,
    _In_ BATTERY_QUERY_INFORMATION_LEVEL Level, _In_ LONG AtRate,
    _Out_writes_bytes_to_(BufferLength, *ReturnedLength) PVOID Buffer,
    _In_ ULONG BufferLength, _Out_ PULONG ReturnedLength);

typedef BCLASS_QUERY_INFORMATION_CALLBACK *PBCLASS_QUERY_INFORMATION_CALLBACK;

_Function_class_(BCLASS_QUERY_STATUS_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_QUERY_STATUS_CALLBACK)(
    _In_ PVOID Context, _In_ ULONG BatteryTag,
    _Out_ PBATTERY_STATUS BatteryStatus);

typedef BCLASS_QUERY_STATUS_CALLBACK *PBCLASS_QUERY_STATUS_CALLBACK;

_Function_class_(ACLASS_QUERY_STATUS_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(ACLASS_QUERY_STATUS_CALLBACK)(
    _In_ PVOID Context, _Out_ PPOWER_ADAPTER_STATUS AdapterStatus);
...
_Function_class_(ACLASS_SET_STATUS_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(ACLASS_SET_STATUS_CALLBACK)(
    _In_ PVOID Context, _In_ PPOWER_ADAPTER_SET_STATUS_BUFFER AdapterStatus);
...
_Function_class_(ACLASS_SET_STATUS_NOTIFY_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(ACLASS_SET_STATUS_NOTIFY_CALLBACK)(
    _In_ PVOID Context, _In_ PADAPTER_NOTIFY AdapterNotify);
...
_Function_class_(ACLASS_DISABLE_STATUS_NOTIFY_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(ACLASS_DISABLE_STATUS_NOTIFY_CALLBACK)(
    _In_ PVOID Context);

typedef ACLASS_DISABLE_STATUS_NOTIFY_CALLBACK
    *PACLASS_DISABLE_STATUS_NOTIFY_CALLBACK;
...
_Function_class_(BCLASS_SET_STATUS_NOTIFY_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_SET_STATUS_NOTIFY_CALLBACK)(
    _In_ PVOID Context, _In_ ULONG BatteryTag,
...
_Function_class_(BCLASS_SET_INFORMATION_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_SET_INFORMATION_CALLBACK)(
    _In_ PVOID Context, _In_ ULONG BatteryTag,
    _In_ BATTERY_SET_INFORMATION_LEVEL Level, _In_opt_ PVOID Buffer);

typedef BCLASS_SET_INFORMATION_CALLBACK *PBCLASS_SET_INFORMATION_CALLBACK;

_Function_class_(BCLASS_DISABLE_STATUS_NOTIFY_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_DISABLE_STATUS_NOTIFY_CALLBACK)(
    _In_ PVOID Context);
...
_Function_class_(BCLASS_QUERY_PEAK_POWER_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_QUERY_PEAK_POWER_CALLBACK)(
    _In_ PVOID Context, _In_ ULONG BatteryTag,
...
_Function_class_(BCLASS_SET_PEAK_POWER_NOTIFY_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_SET_PEAK_POWER_NOTIFY_CALLBACK)(
    _In_ PVOID Context, _In_ ULONG BatteryTag,
...
_Function_class_(BCLASS_DISABLE_PEAK_POWER_NOTIFY_CALLBACK) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL)
_Check_return_ typedef NTSTATUS(BCLASS_DISABLE_PEAK_POWER_NOTIFY_CALLBACK)(
    _In_ PVOID Context);
...
typedef PBCLASS_DISABLE_PEAK_POWER_NOTIFY_CALLBACK
    BCLASS_DISABLE_PEAK_POWER_NOTIFY;
...
typedef struct {
  ...
  //
  // This field is set by the class driver to expose its capabilities, this
  // helps newer miniport driver to be compatiable with older class drivers. For
  // V1.0 and V1.1 class driver, this field won't be updated.
  //
...
typedef struct _ADAPTER_INFORMATION {
  ...
  POWER_ADAPTER_CHARGE_REQUIREMENT
  ChargeRequirement[CHARGE_REQUIREMENT_MAX_POWER_SOURCE_TYPES];
...
#define BATTERY_MINIPORT_UPDATE_DATA_VER_1_SIZE                                \
  sizeof(BATTERY_MINIPORT_UPDATE_DATA)
#define BATTERY_MINIPORT_UPDATE_DATA_VER_2_SIZE                                \
  sizeof(BATTERY_MINIPORT_UPDATE_DATA_V2)
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ NTSTATUS BATTERYCLASSAPI
    BatteryClassInitializeDevice(_In_ PBATTERY_MINIPORT_INFO MiniportInfo,
                                 _Out_ PVOID *ClassData);

_IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS BATTERYCLASSAPI
    BatteryClassUnload(_In_ PVOID ClassData);

_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ NTSTATUS BATTERYCLASSAPI
    BatteryClassIoctl(_In_ PVOID ClassData, _Inout_ PIRP Irp);

#if (NTDDI_VERSION >= NTDDI_WINXP)

_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ NTSTATUS BATTERYCLASSAPI
    BatteryClassSystemControl(_In_ PVOID ClassData,
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ NTSTATUS BATTERYCLASSAPI
    BatteryClassQueryWmiDataBlock(_In_ PVOID ClassData,
                                  _Inout_ PDEVICE_OBJECT DeviceObject,
                                  _Inout_ PIRP Irp, _In_ ULONG GuidIndex,
...
_IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS BATTERYCLASSAPI
    BatteryClassStatusNotify(_In_ PVOID ClassData);

_IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS BATTERYCLASSAPI
    BatteryClassPeakPowerNotify(_In_ PVOID ClassData);

_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ NTSTATUS BATTERYCLASSAPI
    BatteryClassUpdateData(_In_ PVOID ClassData,
                           _In_ PBATTERY_MINIPORT_UPDATE_DATA UpdateData);

_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ NTSTATUS BATTERYCLASSAPI
    AdapterClassInitializeDevice(_In_ PADAPTER_MINIPORT_INFO MiniportInfo,
                                 _Out_ PVOID *ClassData);

_IRQL_requires_max_(DISPATCH_LEVEL) NTSTATUS BATTERYCLASSAPI
    AdapterClassStatusNotify(_In_ PVOID ClassData);

_IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS BATTERYCLASSAPI
    AdapterClassUnload(_In_ PVOID ClassData);
