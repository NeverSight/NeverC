...
public:
  ...
  inline bool operator!=(const CVEC3D &_V) const { return !(this == &_V); };
  ...
  inline CVEC3D AngularDiff(const CVEC3D &_V) const {
    ...
    fSine = Out.Magnitude();
    if (fSine >=
        0.001f) // if Sine is very small, Angle=Sin(Angle) - so skip this
    {
      float fCosine = (*this).DotProduct(_V);
      float fAngle =
          (float)(atan2(fSine, fCosine)); // Angle is in ]-PI..PI] range
  ...
  inline CVEC3D RotateAngleByAxis(const float AngleInRad,
                                  const VEC3D ReferenceAxis) {
    //
    //  Rotate 3D vector pvToRotate by angle AngleRad (in radian), along axis
    //  pvAxis, using right handed space (positive rotation is
    //  counter-clockwize) This is using Rodrigues' rotation formula which is
    //  more efficient than converting the axis and angle into a rotation
    //  matrix, and using the rotation matrix to compute the rotated vector.
    //
  ...
  inline bool HasNAN(void) { return !(X == X && Y == Y && Z == Z); };
...
public:
  ...
  inline CMATRIX3X3(float _A11, float _A12, float _A13, float _A21, float _A22,
                    float _A23, float _A31, float _A32, float _A33) {
    A11 = _A11;
    A12 = _A12;
    A13 = _A13;
    A21 = _A21;
  ...
  inline CMATRIX3X3 operator*(const CMATRIX3X3 &_M) const {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        m33Ret.M[i][j] = (M[i][0] * _M.M[0][j]) + (M[i][1] * _M.M[1][j]) +
                         (M[i][2] * _M.M[2][j]);
      }
    }
    return m33Ret;
  ...
  inline bool operator!=(const CMATRIX3X3 &_M) const { return !(this == &_M); };
  ...
  inline QUATERNION ToQuaternion(void) {
    ...
    // This method is safer and removes the imprecisions around PI rotation than
    // using the classical: W = Sqrt(1+M11+M22+M33); X = (M32-M23) /4W; Y =
    // (M13-M31)/4W; Z = (M21-M12)/4W
    ...
    // Get signs
    if ((A32 - A23) < 0.0f) { // negate QX:
      quarternion.X = -quarternion.X;
    }
    if ((A13 - A31) < 0.0f) { // negate QY:
      quarternion.Y = -quarternion.Y;
    }
    if ((A21 - A12) < 0.0f) { // negate QZ:
...
public:
  ...
  inline CQUATERNION RotateAngleByAxis(const float AngleInRad,
                                       const VEC3D Axis) {
