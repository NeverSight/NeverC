...
//
// There are numerous challenges with the LCID concept which makes it impossible
// for them to support all of the languages and dialects that may be encountered
// by a modern application.  Please use Locale Names.
//
typedef ULONG LCID;    // DEPRECATED, use Locale Names.  See documentation for
                       // GetLocaleInfoEx. // winnt
typedef PULONG PLCID;  // DEPRECATED, use Locale Names.  See documentation for
                       // GetLocaleInfoEx. // winnt
typedef USHORT LANGID; // DEPRECATED, use Locale Names.  See documentation for
                       // GetLocaleInfoEx. // winnt
...
ULONGLONG
NTAPI
Int64ShllMod32(_In_ ULONGLONG Value, _In_ ULONG ShiftCount);

LONGLONG
NTAPI
Int64ShraMod32(_In_ LONGLONG Value, _In_ ULONG ShiftCount);

ULONGLONG
NTAPI
Int64ShrlMod32(_In_ ULONGLONG Value, _In_ ULONG ShiftCount);
...
#define DECLARE_GLOBAL_CONST_UNICODE_STRING(_var, _str)                        \
  extern const __declspec(selectany) UNICODE_STRING _var =                     \
      RTL_CONSTANT_STRING(_str)
...
#if !defined(MIDL_PASS)
__inline void ListEntry32To64(_In_ PLIST_ENTRY32 l32, _Out_ PLIST_ENTRY64 l64) {
  l64->Flink = (ULONGLONG)(ULONG)l32->Flink;
  l64->Blink = (ULONGLONG)(ULONG)l32->Blink;
}

__inline void ListEntry64To32(_In_ PLIST_ENTRY64 l64, _Out_ PLIST_ENTRY32 l32) {
...
//
// This is being moved from ntexapi_h.h -- so for now it is defined in both
// places.
//
...
#define _RTL_CONSTANT_STRING_remove_const_macro(s)                             \
  (const_cast<                                                                 \
      _RTL_CONSTANT_STRING_remove_const_template_class<sizeof((s)[0])>::T *>(  \
      s))
}
#else
...
typedef _IRQL_requires_same_ _Function_class_(EXCEPTION_ROUTINE)
