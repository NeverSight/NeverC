...
typedef struct _RSVP_RESERVE_INFO {
  ...
  LPRSVP_POLICY_INFO
  PolicyElementList;             /* Points to the set of policy elements */
...
#define ALLOWED_TO_SEND_DATA                                                   \
  (QOSSPBASE + 1) // query the SBM/BEST_EFFORT limit
...
#define ABLE_TO_RECV_RSVP                                                      \
  (QOSSPBASE + 2) // query the SBM/BEST_EFFORT limit
                  //   -- result is based on a comparison between
                  //      the user-specified token rate and the
                  //      SBM/BEST_EFFORT bandwidth parameters in
                  //      the system
...
#define LINE_RATE                                                              \
  (QOSSPBASE + 3) // query the interface capacity
                  //   -- result returned in the output buffer
                  //      in kilo-bits per second

#define LOCAL_TRAFFIC_CONTROL                                                  \
  (QOSSPBASE + 4) // check if Kernel Traffic Control is available or not
                  //   -- 0 if not available
                  //   -- 1 if avaiable
                  //   -- INFO_NOT_AVAILABLE if there is no way to check

#define LOCAL_QOSABILITY                                                       \
  (QOSSPBASE + 5) // The followings are for
                  // the Discovery of Local QOSability
                  //   -- 0 if no local QOS support
                  //   -- 1 if local QOS support available
                  //   -- INFO_NOT_AVAILABLE if there is no way to check

#define END_TO_END_QOSABILITY                                                  \
  (QOSSPBASE + 6) // The followings are for
...
// The followings are for Guaranteed_Service+Latency -> ISSLOW_Traffic_Class
// Mapping
...
#define GQOS_API_BADSEND                                                       \
  (QOSSP_ERR_BASE + 400 + 1) /* Sender addr not my interface */
#define GQOS_API_BADRECV                                                       \
  (QOSSP_ERR_BASE + 400 + 2) /* Recv addr not my interface */
#define GQOS_API_BADSPORT                                                      \
  (QOSSP_ERR_BASE + 400 + 3) /* Sport !=0 but Dport == 0 */
