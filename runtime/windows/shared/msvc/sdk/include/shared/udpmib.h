...
#define SIZEOF_UDPTABLE2(X)                                                    \
  (FIELD_OFFSET(MIB_UDPTABLE2, table[0]) + ((X) * sizeof(MIB_UDPROW2)) +       \
   ALIGN_SIZE)
...
#define SIZEOF_UDP6TABLE(X)                                                    \
  (FIELD_OFFSET(MIB_UDP6TABLE, table[0]) + ((X) * sizeof(MIB_UDP6ROW)) +       \
   ALIGN_SIZE)
...
typedef struct _MIB_UDP6TABLE_OWNER_PID {
  DWORD dwNumEntries;
  _Field_size_(dwNumEntries) MIB_UDP6ROW_OWNER_PID table[ANY_SIZE];
...
#define SIZEOF_UDP6TABLE2(X)                                                   \
  (FIELD_OFFSET(MIB_UDP6TABLE2, table[0]) + ((X) * sizeof(MIB_UDP6ROW2)) +     \
   ALIGN_SIZE)
