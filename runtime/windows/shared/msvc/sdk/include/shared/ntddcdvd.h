...
#define IOCTL_AACS_GENERATE_BINDING_NONCE                                      \
  CTL_CODE(IOCTL_DVD_BASE, 0x43C, METHOD_BUFFERED,                             \
           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
typedef _At_((LONG)_Curr_, _Field_range_(-1, 3)) ULONG DVD_SESSION_ID,
    *PDVD_SESSION_ID;
...
typedef struct _DVD_LAYER_DESCRIPTOR {
  ...
  UCHAR Reserved5 : 7;
  UCHAR BCAFlag : 1;
  // The large Media Specific field is not declared here to enable stack
  // allocation
...
// format 0x1A (HD DVD-R - Last recorded RMD in the latest R) is unstructured in
// public spec formats 0x1B through 0x1F are not yet defined
...
typedef struct _DVD_DUAL_LAYER_MIDDLE_ZONE_START_ADDRESS {
  ...
  UCHAR ShiftedMiddleAreaStartAddress[4];
    ...
    *PDVD_DUAL_LAYER_MIDDLE_ZONE_START_ADDRESS;
...
// format 0x81 (Pre-Recorded AACS media serial number) is unstructured in public
// spec format 0x82 (AACS media identifier) is unstructured in public spec
...
typedef struct _DVD_LIST_OF_RECOGNIZED_FORMAT_LAYERS {
  ...
  // DVD_LIST_OF_RECOGNIZED_FORMAT_LAYERS_TYPE_CODE TypeCodes[0];
    ...
    *PDVD_LIST_OF_RECOGNIZED_FORMAT_LAYERS_TYPE_CODE;
...
// The prerecorded Serial Number is one of the unique identifiers on AACS
// protected media
