...
typedef LARGE_INTEGER _SECURITY_INTEGER, SECURITY_INTEGER,
    *PSECURITY_INTEGER; // ntifs
...
#define SECPKG_FLAG_MULTI_REQUIRED                                             \
  0x00000020 // Full 3-leg required for re-auth.
#define SECPKG_FLAG_CLIENT_ONLY                                                \
  0x00000040 // Server side functionality not available
...
#define SECPKG_FLAG_NEGOTIABLE                                                 \
  0x00000800 // Can be used by the negotiate package
...
#define SECPKG_FLAG_MUTUAL_AUTH                                                \
  0x00010000 // Package can perform mutual authentication
...
#define SECPKG_FLAG_RESTRICTED_TOKENS                                          \
  0x00080000 // Package supports restricted callers
#define SECPKG_FLAG_NEGO_EXTENDER                                              \
  0x00100000 // this package extends SPNEGO, there is at most one
#define SECPKG_FLAG_NEGOTIABLE2                                                \
  0x00200000 // this package is negotiated under the NegoExtender
#define SECPKG_FLAG_APPCONTAINER_PASSTHROUGH                                   \
  0x00400000 // this package receives all calls from appcontainer apps
#define SECPKG_FLAG_APPCONTAINER_CHECKS                                        \
  0x00800000 // this package receives calls from appcontainer apps
...
#define SECPKG_FLAG_CREDENTIAL_ISOLATION_ENABLED                               \
  0x01000000 // this package is running with Credential Guard enabled
#define SECPKG_FLAG_APPLY_LOOPBACK                                             \
  0x02000000 // this package supports reliable detection of loopback
...
#define SECBUFFER_APPLICATION_PROTOCOLS                                        \
  18 // Lists of application protocol IDs, one per negotiation extension
#define SECBUFFER_SRTP_PROTECTION_PROFILES                                     \
  19 // List of SRTP protection profiles, in descending order of preference
#define SECBUFFER_SRTP_MASTER_KEY_IDENTIFIER 20 // SRTP master key identifier
#define SECBUFFER_TOKEN_BINDING                                                \
  21 // Supported Token Binding protocol version and key parameters
...
#define SECBUFFER_SEND_GENERIC_TLS_EXTENSION                                   \
  25 // Buffer for sending generic TLS extensions.
#define SECBUFFER_SUBSCRIBE_GENERIC_TLS_EXTENSION                              \
  26                       // Buffer for subscribing to generic TLS extensions.
#define SECBUFFER_FLAGS 27 // ISC/ASC REQ Flags
#define SECBUFFER_TRAFFIC_SECRETS                                              \
  28 // Message sequence lengths and corresponding traffic secrets.
#define SECBUFFER_CERTIFICATE_REQUEST_CONTEXT                                  \
  29 // TLS 1.3 certificate request context.
#define SECBUFFER_CHANNEL_BINDINGS_RESULT                                      \
  30 // Output buffer for Channel Bindings Audit
#define SECBUFFER_APP_SESSION_STATE                                            \
  31 // Application state associated with the TLS 1.3+ session ticket. Server
     // only.
...
#define SECBUFFER_READONLY_WITH_CHECKSUM                                       \
  0x10000000                          // Buffer is read-only, and checksummed
...
typedef struct _SEC_CHANNEL_BINDINGS_EX {
  unsigned long
      magicNumber; // contains SEC_CHANNEL_BINDINGS_VERSION_2.  distinguish ex
                   // buffer from normal channel bindings buffer. Shouldn't
                   // collide with any of assigned dwInitiatorAddrType values
...
#define SEC_CHANNEL_BINDINGS_RESULT_CLIENT_SUPPORT                             \
  0x1 // Auth package indicates client versions should support bindings
#define SEC_CHANNEL_BINDINGS_RESULT_ABSENT                                     \
  0x2 // The bindings are omitted or all zeroes
#define SEC_CHANNEL_BINDINGS_RESULT_NOTVALID_MISMATCH                          \
  0x4 // The channel binding hash was incorrect
#define SEC_CHANNEL_BINDINGS_RESULT_NOTVALID_MISSING                           \
  0x8 // Missing channel bindings are not allowed for this client
#define SEC_CHANNEL_BINDINGS_RESULT_VALID_MATCHED                              \
  0x10 // The client and server bindings match
#define SEC_CHANNEL_BINDINGS_RESULT_VALID_PROXY                                \
  0x20 // ASC_REQ_PROXY_BINDINGS required the client to provide a binding
#define SEC_CHANNEL_BINDINGS_RESULT_VALID_MISSING                              \
  0x40 // Client permitted by ASC_REQ_ALLOW_MISSING_BINDINGS

#define SEC_CHANNEL_BINDINGS_RESULT_VALID                                      \
...
#define SEC_CHANNEL_BINDINGS_RESULT_NOTVALID                                   \
...
typedef struct _SEC_APPLICATION_PROTOCOL_LIST {
  SEC_APPLICATION_PROTOCOL_NEGOTIATION_EXT
  ProtoNegoExt; // Protocol negotiation extension type to use with this list
                // of protocols
  unsigned short ProtocolListSize; // Size in bytes of the protocol ID list
  unsigned char
      ProtocolList[ANYSIZE_ARRAY]; // 8-bit length-prefixed application protocol
                                   // IDs, most preferred first
} SEC_APPLICATION_PROTOCOL_LIST, *PSEC_APPLICATION_PROTOCOL_LIST;

typedef struct _SEC_APPLICATION_PROTOCOLS {
  unsigned long
      ProtocolListsSize; // Size in bytes of the protocol ID lists array
  SEC_APPLICATION_PROTOCOL_LIST
  ProtocolLists[ANYSIZE_ARRAY]; // Array of protocol ID lists
} SEC_APPLICATION_PROTOCOLS, *PSEC_APPLICATION_PROTOCOLS;

typedef struct _SEC_SRTP_PROTECTION_PROFILES {
  unsigned short
      ProfilesSize; // Size in bytes of the SRTP protection profiles array
  unsigned short
      ProfilesList[ANYSIZE_ARRAY]; // Array of SRTP protection profiles
} SEC_SRTP_PROTECTION_PROFILES, *PSEC_SRTP_PROTECTION_PROFILES;

typedef struct _SEC_SRTP_MASTER_KEY_IDENTIFIER {
  unsigned char MasterKeyIdentifierSize; // Size in bytes of the SRTP master key
                                         // identifier
  unsigned char
      MasterKeyIdentifier[ANYSIZE_ARRAY]; // SRTP master key identifier
} SEC_SRTP_MASTER_KEY_IDENTIFIER, *PSEC_SRTP_MASTER_KEY_IDENTIFIER;

typedef struct _SEC_TOKEN_BINDING {
  unsigned char
      MajorVersion; // Supported major version of the Token Binding protocol
  unsigned char
      MinorVersion; // Supported minor version of the Token Binding protocol
  unsigned short KeyParametersSize; // Size in bytes of the Token Binding key
                                    // parameter IDs array
  unsigned char KeyParameters[ANYSIZE_ARRAY]; // Token Binding key parameter
                                              // IDs, most preferred first
...
typedef struct _SEC_FLAGS {
  unsigned long long Flags; // The caller sets ISC/ASC REQ flags; the lower 32
                            // bits are reserved, must be set to 0.
} SEC_FLAGS, *PSEC_FLAGS;

typedef struct _SEC_CERTIFICATE_REQUEST_CONTEXT {
  unsigned char
      cbCertificateRequestContext; // Size in bytes of the
                                   // rgCertificateRequestContext array.
  unsigned char
      rgCertificateRequestContext[ANYSIZE_ARRAY]; // The TLS 1.3 certificate
                                                  // request context.
} SEC_CERTIFICATE_REQUEST_CONTEXT, *PSEC_CERTIFICATE_REQUEST_CONTEXT;

typedef struct _SEC_APP_SESSION_STATE {
  unsigned short AppSessionStateSize; // Size in bytes of the application state,
                                      // up to 2048 bytes.
  unsigned char
      AppSessionState[ANYSIZE_ARRAY]; // Application state to be associated with
                                      // the session ticket.
...
typedef struct _SEC_TRAFFIC_SECRETS {
  wchar_t
      SymmetricAlgId[SZ_ALG_MAX_SIZE]; // Negotiated symmetric key algorithm.
                                       // e.g. BCRYPT_AES_ALGORITHM.
  wchar_t ChainingMode
      [SZ_ALG_MAX_SIZE]; // Negotiated symmetric key algorithm chaining mode.
                         // e.g. BCRYPT_CHAIN_MODE_GCM or BCRYPT_CHAIN_MODE_CCM.
  wchar_t HashAlgId[SZ_ALG_MAX_SIZE]; // Negotiated hash algorithm. e.g.
                                      // BCRYPT_SHA256_ALGORITHM or
                                      // BCRYPT_SHA384_ALGORITHM.
  unsigned short KeySize; // Size in bytes of the symmetric key to derive from
                          // this traffic secret.
  unsigned short
      IvSize; // Size in bytes of the IV to derive from this traffic secret.
  unsigned short MsgSequenceStart; // Offset of the first byte of the TLS
                                   // message sequence to be protected with a
                                   // key derived from TrafficSecret. Zero to
                                   // indicate the first byte of the buffer.
  unsigned short
      MsgSequenceEnd; // Offset of the last byte of the TLS message sequence to
                      // be protected with a key derived from TrafficSecret.
                      // Zero if the secret is for the encryption of application
                      // data or decryption of incoming records.
  SEC_TRAFFIC_SECRET_TYPE TrafficSecretType; // Type of traffic secret from the
                                             // TRAFFIC_SECRET_TYPE enumeration.
  unsigned short TrafficSecretSize; // Size in bytes of the traffic secret.
  unsigned char
      TrafficSecret[ANYSIZE_ARRAY]; // Traffic secret of type TrafficSecretType,
                                    // TrafficSecretSize bytes long, used to
                                    // derive write key and IV for message
                                    // protection.
...
// Enables configuring Kerberos to only request tickets from a specific DC
// version
...
#define ISC_REQ_MESSAGES                                                       \
  0x0000000100000000 // Disables the TLS 1.3+ record layer and causes the
                     // security context to consume and produce cleartext TLS
                     // messages, rather than records.
// Request that schannel perform server cert chain validation without failing
// the handshake on errors (deferred), same as SCH_CRED_DEFERRED_CRED_VALIDATION
// except applies to context not credential handle.
#define ISC_REQ_DEFERRED_CRED_VALIDATION 0x0000000200000000
// Prevents the client sending the post_handshake_auth extension in the TLS 1.3
// Client Hello.
#define ISC_REQ_NO_POST_HANDSHAKE_AUTH 0x0000000400000000
// Request TLS 1.3+ session ticket reuse. Passive observers may be able to track
// the TLS client across networks.
#define ISC_REQ_REUSE_SESSION_TICKETS 0x0000000800000000
// Request explicit TLS 1.3+ session management. Received TLS 1.3+ session
// tickets will be returned to the SSPI caller. The SSPI caller will specify the
// session ticket to send in each resumption attempt.
...
#define ISC_RET_MESSAGES                                                       \
  0x0000000100000000 // Indicates that the TLS 1.3+ record layer is disabled,
                     // and the security context consumes and produces cleartext
                     // TLS messages, rather than records.
#define ISC_RET_DEFERRED_CRED_VALIDATION                                       \
  0x0000000200000000 // Indicates that
                     // SCH_CRED_DEFERRED_CRED_VALIDATION/ISC_REQ_DEFERRED_CRED_VALIDATION
                     // request will be honored.
#define ISC_RET_NO_POST_HANDSHAKE_AUTH                                         \
  0x0000000400000000 // Indicates that the TLS 1.3 Client Hello will not contain
                     // the post_handshake_auth extension.
#define ISC_RET_REUSE_SESSION_TICKETS                                          \
  0x0000000800000000 // Indicates that the TLS 1.3+ client may reuse session
                     // tickets.
#define ISC_RET_EXPLICIT_SESSION                                               \
  0x0000001000000000 // Indicates that explicit TLS 1.3+ session management is
                     // enabled.
...
#define ASC_REQ_MESSAGES                                                       \
  0x0000000100000000 // Disables the TLS 1.3+ record layer and causes the
                     // security context to consume and produce cleartext TLS
                     // messages, rather than records.
// Request explicit TLS 1.3+ session management. TLS 1.3+ session ticket will
// only be generated when requested by the SSPI caller.
...
#define ASC_RET_ALLOW_CONTEXT_REPLAY                                           \
  0x00400000 // deprecated - don't use this flag!!!
#define ASC_RET_FRAGMENT_ONLY 0x00800000
#define ASC_RET_NO_TOKEN 0x01000000
#define ASC_RET_NO_ADDITIONAL_TOKEN 0x02000000 // *INTERNAL*
//      SSP_RET_REAUTHENTICATION        0x08000000  // *INTERNAL*
#define ASC_RET_MESSAGES                                                       \
  0x0000000100000000 // Indicates that the TLS 1.3+ record layer is disabled,
                     // and the security context consumes and produces cleartext
                     // TLS messages, rather than records.
#define ASC_RET_REUSE_SESSION_TICKETS                                          \
  0x0000000800000000 // Indicates that the TLS 1.3+ server will allow session
                     // ticket reuse.
#define ASC_RET_EXPLICIT_SESSION                                               \
  0x0000001000000000 // Indicates that explicit TLS 1.3+ session management is
                     // enabled.
...
#define SECPKG_CRED_ATTR_KDC_PROXY_SETTINGS                                    \
  3 // aliases SECPKG_CRED_ATTR_KDC_NETWORK_SETTINGS
#define SECPKG_CRED_ATTR_KDC_NETWORK_SETTINGS                                  \
  3 // aliases SECPKG_CRED_ATTR_KDC_PROXY_SETTINGS
...
typedef struct _SecPkgContext_SubjectAttributes {
  void *AttributeInfo; // contains a PAUTHZ_SECURITY_ATTRIBUTES_INFORMATION
                       // structure
...
typedef struct _SecPkgContext_ApplicationProtocol {
  SEC_APPLICATION_PROTOCOL_NEGOTIATION_STATUS
  ProtoNegoStatus; // Application  protocol negotiation status
  SEC_APPLICATION_PROTOCOL_NEGOTIATION_EXT
  ProtoNegoExt; // Protocol negotiation extension type corresponding to this
                // protocol ID
  unsigned char ProtocolIdSize; // Size in bytes of the application protocol ID
  unsigned char
      ProtocolId[MAX_PROTOCOL_ID_SIZE]; // Byte string representing the
                                        // negotiated application protocol ID
...
typedef struct _SecPkgContext_NegotiatedTlsExtensions {
  ...
  _Field_size_(ExtensionsCount) unsigned short
      *Extensions; // Pointer to array of 2-byte TLS extension IDs (allocated by
                   // IANA).
...
#define SECPKG_CONTEXT_EXPORT_RESET_NEW                                        \
  0x00000001 // New context is reset to initial state
#define SECPKG_CONTEXT_EXPORT_DELETE_OLD                                       \
  0x00000002 // Old context is deleted during export
// This is only valid in W2K3SP1 and greater
#define SECPKG_CONTEXT_EXPORT_TO_KERNEL                                        \
  0x00000004 // Context is to be transferred to the kernel

KSECDDDECLSPEC
SECURITY_STATUS SEC_ENTRY AcquireCredentialsHandleW(
#if ISSP_MODE == 0 // For Kernel mode
    _In_opt_ PSECURITY_STRING pPrincipal, _In_ PSECURITY_STRING pPackage,
#else
...
typedef SECURITY_STATUS(SEC_ENTRY *ACQUIRE_CREDENTIALS_HANDLE_FN_W)(
    ...
    SEC_WCHAR *, SEC_WCHAR *,
#endif
    unsigned long, void *, void *, SEC_GET_KEY_FN, void *, PCredHandle,
    PTimeStamp);
...
KSECDDDECLSPEC
SECURITY_STATUS SEC_ENTRY AddCredentialsW(
...
//
typedef void (*SspiAsyncNotifyCallback)(_In_ SspiAsyncContext *Handle,
...
// Free up a context.
//
void SspiFreeAsyncContext(_In_opt_ SspiAsyncContext *Handle);
...
//
NTSTATUS SspiReinitAsyncContext(_Inout_ SspiAsyncContext *Handle);

//
// Installs a callback which will be notified on async call completion.
//
SECURITY_STATUS
SspiSetAsyncNotifyCallback(_In_ SspiAsyncContext *Context,
...
// of the call.
//
BOOLEAN SspiAsyncContextRequiresNotify(_In_ SspiAsyncContext *AsyncContext);

//
// Gets the current status of an async call. Until the call is completed,
// status will be SEC_I_ASYNC_CALL_PENDING.
//
SECURITY_STATUS SspiGetAsyncCallStatus(_In_ SspiAsyncContext *Handle);
...
SECURITY_STATUS
SspiFreeCredentialsHandleAsync(_Inout_ SspiAsyncContext *AsyncContext,
                               _In_ PCredHandle phCredential // Handle to free
);

SECURITY_STATUS
SspiDeleteSecurityContextAsync(_Inout_ SspiAsyncContext *AsyncContext,
...
SECURITY_STATUS SEC_ENTRY ChangeAccountPasswordW(
    _In_ SEC_WCHAR *pszPackageName, _In_ SEC_WCHAR *pszDomainName,
    _In_ SEC_WCHAR *pszAccountName, _In_ SEC_WCHAR *pszOldPassword,
    _In_ SEC_WCHAR *pszNewPassword, _In_ BOOLEAN bImpersonating,
    _In_ unsigned long dwReserved, _Inout_ PSecBufferDesc pOutput);

typedef SECURITY_STATUS(SEC_ENTRY *CHANGE_PASSWORD_FN_W)(
    SEC_WCHAR *, SEC_WCHAR *, SEC_WCHAR *, SEC_WCHAR *, SEC_WCHAR *, BOOLEAN,
    unsigned long, PSecBufferDesc);

SECURITY_STATUS SEC_ENTRY ChangeAccountPasswordA(
    _In_ SEC_CHAR *pszPackageName, _In_ SEC_CHAR *pszDomainName,
    _In_ SEC_CHAR *pszAccountName, _In_ SEC_CHAR *pszOldPassword,
    _In_ SEC_CHAR *pszNewPassword, _In_ BOOLEAN bImpersonating,
    _In_ unsigned long dwReserved, _Inout_ PSecBufferDesc pOutput);
...
typedef SECURITY_STATUS(SEC_ENTRY *ACCEPT_SECURITY_CONTEXT_FN)(
    PCredHandle, PCtxtHandle, PSecBufferDesc, unsigned long, unsigned long,
    PCtxtHandle, PSecBufferDesc, unsigned long *, PTimeStamp);
...
KSECDDDECLSPEC
_Check_return_ SECURITY_STATUS SEC_ENTRY ImpersonateSecurityContext(
...
typedef SECURITY_STATUS(SEC_ENTRY *QUERY_SECURITY_CONTEXT_TOKEN_FN)(PCtxtHandle,
                                                                    void **);
...
SecAllocateAndSetIPAddress(
    _In_reads_bytes_(cchIpAddress) PUCHAR lpIpAddress, _In_ ULONG cchIpAddress,
    _Out_ int *FreeCallContext // Avoid creating a dependence on minwindef.h by
                               // replacing PBOOL with its definition int*
);
...
SecAllocateAndSetCallTarget(
    ...
    _In_ ULONG cchIpAddress, _In_opt_ LPWSTR TargetName,
    _Out_ int *FreeCallContext // Avoid creating a dependence on minwindef.h by
                               // replacing PBOOL with its definition int*
);

VOID SEC_ENTRY SecFreeCallContext(VOID);
...
typedef SECURITY_STATUS(SEC_ENTRY *ENCRYPT_MESSAGE_FN)(PCtxtHandle,
                                                       unsigned long,
                                                       PSecBufferDesc,
                                                       unsigned long);
...
SecMakeSPN(IN PUNICODE_STRING ServiceClass, IN PUNICODE_STRING ServiceName,
           ...
           IN PUNICODE_STRING Referrer OPTIONAL, IN OUT PUNICODE_STRING Spn,
           OUT PULONG Length OPTIONAL, IN BOOLEAN Allocate);
...
SecMakeSPNEx(IN PUNICODE_STRING ServiceClass, IN PUNICODE_STRING ServiceName,
             ...
             IN PUNICODE_STRING TargetInfo OPTIONAL, IN OUT PUNICODE_STRING Spn,
             OUT PULONG Length OPTIONAL, IN BOOLEAN Allocate);
...
NTSTATUS
NTAPI
SecMakeSPNEx2(IN PUNICODE_STRING ServiceClass, IN PUNICODE_STRING ServiceName,
              IN PUNICODE_STRING InstanceName OPTIONAL,
              IN USHORT InstancePort OPTIONAL,
              IN PUNICODE_STRING Referrer OPTIONAL,
              IN PUNICODE_STRING InTargetInfo OPTIONAL,
              IN OUT PUNICODE_STRING Spn, OUT PULONG TotalSize OPTIONAL,
              IN BOOLEAN Allocate, IN BOOLEAN IsTargetInfoMarshaled);
...
NTSTATUS
SEC_ENTRY
SecLookupAccountSid(_In_ PSID Sid, _Out_ PULONG NameSize,
...
NTSTATUS
SEC_ENTRY
SecLookupAccountName(_In_ PUNICODE_STRING Name, _Inout_ PULONG SidSize,
                     _Out_ PSID Sid, _Out_ PSID_NAME_USE NameUse,
                     _Out_ PULONG DomainSize OPTIONAL,
...
NTSTATUS
SEC_ENTRY
SecLookupWellKnownSid(_In_ WELL_KNOWN_SID_TYPE SidType, _Out_ PSID Sid,
...
SECURITY_STATUS
SEC_ENTRY
SaslEnumerateProfilesA(_Outptr_ LPSTR *ProfileList, _Out_ ULONG *ProfileCount);

SECURITY_STATUS
SEC_ENTRY
SaslEnumerateProfilesW(_Outptr_ LPWSTR *ProfileList, _Out_ ULONG *ProfileCount);
...
SECURITY_STATUS
SEC_ENTRY
SaslGetProfilePackageA(_In_ LPSTR ProfileName,
                       _Outptr_ PSecPkgInfoA *PackageInfo);

SECURITY_STATUS
SEC_ENTRY
SaslGetProfilePackageW(_In_ LPWSTR ProfileName,
...
SECURITY_STATUS
SEC_ENTRY
SaslIdentifyPackageA(_In_ PSecBufferDesc pInput,
                     _Outptr_ PSecPkgInfoA *PackageInfo);

SECURITY_STATUS
SEC_ENTRY
SaslIdentifyPackageW(_In_ PSecBufferDesc pInput,
                     _Outptr_ PSecPkgInfoW *PackageInfo);
...
typedef enum _SASL_AUTHZID_STATE {
  Sasl_AuthZIDForbidden, // allow no AuthZID strings to be specified - error out
                         // (default)
...
SECURITY_STATUS
SEC_ENTRY
SaslSetContextOption(_In_ PCtxtHandle ContextHandle, _In_ ULONG Option,
                     _In_ PVOID Value, _In_ ULONG Size);

SECURITY_STATUS
SEC_ENTRY
SaslGetContextOption(_In_ PCtxtHandle ContextHandle, _In_ ULONG Option,
                     _Out_ PVOID Value, _In_ ULONG Size,
...
typedef struct _SEC_WINNT_AUTH_IDENTITY_EX2 {
  ...
  unsigned long
      PackedCredentialsOffset; // Non-NULL terminated string, unicode only
  unsigned short
      PackedCredentialsLength; // # of bytes (NOT WCHARs), not including NULL.
  unsigned long Flags;
...
typedef struct _SEC_WINNT_AUTH_IDENTITY_W {
  ...
  unsigned long
      DomainLength;         //  # of characters (NOT bytes), not including NULL.
  unsigned short *Password; //  Non-NULL terminated string.
  unsigned long
      PasswordLength; //  # of characters (NOT bytes), not including NULL.
  unsigned long Flags;
...
//   DWORD is 0x200, it is either an AuthIdExw or AuthIdExA, otherwise if the
//   first DWORD is 0x201, the structure is an AuthIdEx2 structure. Otherwise
//   the structure is either an AuthId_a or an AuthId_w.
//
// 2) Secondly check the flags for SEC_WINNT_AUTH_IDENTITY_ANSI or
//   SEC_WINNT_AUTH_IDENTITY_UNICODE, the presence of the former means the
//   structure is an ANSI structure. Otherwise, the structure is the wide
//   version.  Note that AuthIdEx2 does not have an ANSI version so this check
...
typedef PVOID
    PSEC_WINNT_AUTH_IDENTITY_OPAQUE; // the credential structure is opaque
...
unsigned long SEC_ENTRY SspiPromptForCredentialsA(
    ...
    _In_opt_ PVOID pUiInfo,
#endif // _CREDUI_INFO_DEFINED
    _In_ unsigned long dwAuthError, _In_ PCSTR pszPackage,
    _In_opt_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE pInputAuthIdentity,
    _Outptr_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE *ppAuthIdentity,
    _Inout_opt_ int *pfSave, _In_ unsigned long dwFlags);
#endif // _SSPIPFC_NONE_
...
#define NGC_DATA_FLAG_KERB_CERTIFICATE_LOGON_FLAG_CHECK_DUPLICATES             \
  (0x1) // corresponds to KERB_CERTIFICATE_LOGON_FLAG_CHECK_DUPLICATES
#define NGC_DATA_FLAG_KERB_CERTIFICATE_LOGON_FLAG_USE_CERTIFICATE_INFO         \
  (0x2) // corresponds to  KERB_CERTIFICATE_LOGON_FLAG_USE_CERTIFICATE_INFO
#define NGC_DATA_FLAG_IS_SMARTCARD_DATA (0x4)
#define NGC_DATA_FLAG_IS_CLOUD_TRUST_CRED                                      \
  (0x8) // credential should be treated as "cloud trust" - use Cloud TGTs
        // instead of on-prem PKINIT
...
typedef struct _SEC_WINNT_AUTH_FIDO_DATA {
  ...
  Secret; // offsets are from the beginning of this structure
  SEC_WINNT_AUTH_BYTE_VECTOR NewSecret;
  SEC_WINNT_AUTH_BYTE_VECTOR EncryptedNewSecret; // For storage by cloud AP
  SEC_WINNT_AUTH_BYTE_VECTOR
  NetworkLogonBuffer;       // Opaque data, understood by plugin, may contain
                            // signed Nonce and other data to perform a network
                            // logon
  ULONG64 ulSignatureCount; // signature count to be stored in public cached
                            // info, required for CredProv
...
SspiGetCredUIContext(
    _In_ HANDLE ContextHandle, _In_ GUID *CredType,
    _In_opt_ LUID *LogonId, // use this LogonId, the caller must be localsystem
                            // to supply a logon id
    _Outptr_ PSEC_WINNT_CREDUI_CONTEXT_VECTOR *CredUIContexts,
    _Out_opt_ HANDLE *TokenHandle);

SECURITY_STATUS
SEC_ENTRY
SspiUpdateCredentials(_In_ HANDLE ContextHandle, _In_ GUID *CredType,
                      _In_ ULONG FlatCredUIContextLength,
                      _In_reads_bytes_(FlatCredUIContextLength)
...
SECURITY_STATUS
SEC_ENTRY
SspiUnmarshalCredUIContext(_In_reads_bytes_(MarshaledCredUIContextLength)
...
SECURITY_STATUS
SEC_ENTRY
SspiPrepareForCredRead(_In_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthIdentity,
                       _In_ PCWSTR pszTargetName,
...
SspiPrepareForCredWrite(
    ...
    _In_opt_ PCWSTR
        pszTargetName, // supply NULL for username-target credentials
    _Out_ PULONG pCredmanCredentialType, _Outptr_ PCWSTR *ppszCredmanTargetName,
...
SECURITY_STATUS
SEC_ENTRY
SspiEncryptAuthIdentity(_Inout_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthData);
...
SspiDecryptAuthIdentityEx(_In_ ULONG Options,
                          _Inout_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE
                              EncryptedAuthData);
...
SECURITY_STATUS
SEC_ENTRY
SspiValidateAuthIdentity(_In_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthData);
...
SECURITY_STATUS
SEC_ENTRY
SspiCopyAuthIdentity(_In_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthData,
                     _Outptr_ _When_(return != 0, __drv_allocatesMem(Mem))
...
VOID SEC_ENTRY
SspiZeroAuthIdentity(_In_opt_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthData);
...
SspiEncodeStringsAsAuthIdentity(
    _In_opt_ PCWSTR pszUserName, _In_opt_ PCWSTR pszDomainName,
...
SECURITY_STATUS
SEC_ENTRY
SspiMarshalAuthIdentity(_In_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthIdentity,
                        _Out_ unsigned long *AuthIdentityLength,
                        _Outptr_result_bytebuffer_(
                            *AuthIdentityLength) char **AuthIdentityByteArray);
...
BOOLEAN
SEC_ENTRY
SspiIsPromptingNeeded(_In_ unsigned long ErrorOrNtStatus);
...
SECURITY_STATUS
SEC_ENTRY
SspiGetTargetHostName(_In_ PCWSTR pszTargetName, _Outptr_ PWSTR *pszHostName);

SECURITY_STATUS
SEC_ENTRY
SspiExcludePackage(_In_opt_ PSEC_WINNT_AUTH_IDENTITY_OPAQUE AuthIdentity,
...
#define SEC_WINNT_AUTH_IDENTITY_ONLY                                           \
  0x8 // these credentials are for identity only - no PAC needed

// end_ntifs

// Set the requested flags in the channel bindings.  pBindings->Bindings may
// change if the structure gets larger, but the caller's obligation to call
// FreeContextBuffer is unchanged.
SECURITY_STATUS
SEC_ENTRY
SspiSetChannelBindingFlags(_Inout_ SecPkgContext_Bindings *pBindings,
                           unsigned long flags);
...
SECURITY_STATUS
SEC_ENTRY
AddSecurityPackageA(_In_ LPSTR pszPackageName,
                    _In_opt_ PSECURITY_PACKAGE_OPTIONS pOptions);

SECURITY_STATUS
SEC_ENTRY
AddSecurityPackageW(_In_ LPWSTR pszPackageName,
                    _In_opt_ PSECURITY_PACKAGE_OPTIONS pOptions);
...
SECURITY_STATUS
SEC_ENTRY
DeleteSecurityPackageA(_In_ LPSTR pszPackageName);

SECURITY_STATUS
SEC_ENTRY
DeleteSecurityPackageW(_In_ LPWSTR pszPackageName);
