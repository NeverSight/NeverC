...
#if !defined(_D3DKMDDI_H_) && !defined(_D3DKMDDI_PEXT_H_) &&                   \
    !defined(_DXGDMM_H_) && !defined(_VIDPRIV_H_) && !defined(_DISPMPRT_H_) && \
...
// Alias VOID* to make LDDM kernel mode interface prototypes using adapter
// handles self-explanatory.
typedef VOID *D3DKMDT_ADAPTER;

//////////////////// VidPN management DDI constants
////////////////////////////////////////////////////////////

// Special values representing that given variable has not been initialized to a
// valid value intended to catch development time errors. A valid parameter
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Purpose: Video mode standard descriptor type, listing standards that are
// explicitly supported by Windows.
//
// Remarks: This enum specifies based on which standard the video signal timing
// parameters should be computed.
//          Note that NTSC, PAL, and SECAM mode variants are treated as
//          individual groups to avoid complicating the DDI with the notion of
//          "sub-standard" (because they don't differ from each other in the
//          parameters used to describe the video signal in the DDI and the
//          parameters that they do differ in are of no interest to us in DMM).
//
...
typedef struct _D3DKMDT_VIDEO_PRESENT_SOURCE {
  // Unique ID used to reference the respective video present source by the
  // miniport and the OS.
...
//
// Remarks: This type is used to specify whether a VidPN source mode is a
// graphics or a text mode
...
//
// Remarks: Use Direct to represent VidPN source modes with colors stored
// directly in the primary surface.
//          Use PresetPalette to represent VidPN source modes with colors'
//          indices stored in the primary surface and actual color values stored
//          in a palette specific to the video card, that must be queried from
//          the video miniport. Use SettablePalette to represent VidPN source
//          modes with colors' indices stored in the primary surface and actual
//          color values stored in a settable palette that can be dynamically
//          set on the video card, by specifying it to the video miniport.
//
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Purpose: Descriptor type of the color basis with respect to which the pixels'
// colors are expanded,
//          or conversely, based on which the color values are synthesized.
//
// Remarks: The commonly used color bases in graphics industry are RGB, which
// has the basis (red, green, blue),
//          as well as YPbPr and YCbCr, which have scaled variants of basis:
//          (1, blue-1, red-1)*intensity(red,green,blue).
//          Tri-stimulus linear RGB is well suited for real-time rendering,
//          since most filtering algorithms use tri-stimulus values to
//          approximate light's spectral transformations caused by its
//          interaction with the environment, primarily due to the fact that
//          there is a linear relationship between the perceived light level and
//          the light's spectral intensity. Ideally, all processing of video
...
//          x^0.45).  This allows more precision to be stored in darker
//          intensities where the human eye is more sensitive. sRGB (more
//          accurately, sR'G'B') stores light intensities relative to a gamma
//          curve. scRGB stores linear values and requires much higher precision
//          to represent the same perceptually similar signal. The
//          light-intensity based YPbPr and YCbCr is better suited for
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Purpose: Descriptor type of the color coefficients dynamic range, whose
// linear combination with the
//          respective color basis produces final pixel values.
//
// Remarks: Examples include (5,6,5,0) for R5G6B5, (8,8,8,8) for R8G8B8A8, and
// (24, 0, 0, 0) for 24-bit
//          grayscale pixel encoding format.
//          NOTE: Currently this is only used for target modes, none of which
//          has the 4th channel (e.g. alpha).
//                We are keeping the 4th field for extensibility purpose to
//                avoid miniport interface revision if 4-channel video
//                interfaces became available between display adapter and
//                monitor.
//
...
//
// Remarks: We define our own rather than reusing SIZE type to avoid dependency
// on SDK headers.
//
...
//
// Remarks: Graphics video present source mode is the dominantly used subtype of
// the video present source
//          modes (other being the text video present source mode).
//          Note that whenever video present source mode's visible size,
//          VIDEO_PRESENT_SOURCE_MODE.GRAPHICS_RENDERING_FORMAT.sizeVisible is
//          not equal to the respective video mode's visible size,
//          VIDEO_PRESENT_TARGET_MODE.sizeVisible, h/w scaling is undertaken by
//          the video output codec.
//
//          Miniport is free to support any D3D pixel format for its graphics
//          modes that is meaningful as a primary surface pixel format.
//
typedef struct _D3DKMDT_GRAPHICS_RENDERING_FORMAT {
  ...
  // Size of the visible part of the primary surface, used for panned modes
  // including zoom modes. The visible region size is required to be the same as
  // the primary surface
...
//
// Remarks: VidPN source (rendering) mode is the mode of operation of a given
// video present source and determines
//          the format of this source's primary surface to which the graphics
//          subsystem is rendering the visual image to be presented to the user,
//          and from which the video output codec is reading the visual image
//          content to be converted into a respective video mode signal.
//
typedef struct _D3DKMDT_VIDPN_SOURCE_MODE {
  ...
  union {
    // Descriptor of the graphics rendering mode (valid only if
    // Type==D3DKMDT_RMT_GRAPHICS).
    D3DKMDT_GRAPHICS_RENDERING_FORMAT Graphics;

    // Descriptor of the text rendering mode (valid only if
    // Type==D3DKMDT_RMT_TEXT).
...
// NOTE: Child device (e.g. video output) HPD awareness is used to represent the
// level of external
//       device (e.g. monitor) connectivity sensed by a display adapter. Child
//       device can either be always connected to an external device (e.g.
//       integrated LCD in a mobile system) or have:
//         1. No HPD-awareness iff miniport is *not* aware of external device
//         arrivals/departures
//            whether through interrupts or polling,
//         2. Polled HPD-awareness iff miniport can not asynchronously notify
//         the OS about external
//            device arrivals/departures, but OS can sporadically poll for its
//            presence, and of a monitor, causing visual artifacts on each poll.
//         3. Interruptible HPD-awareness iff miniport can asynchronously notify
//         the OS about
...
//
// Remarks: Video output technology is used to determine the hard-coded list of
...
typedef struct _D3DKMDT_VIDEO_PRESENT_TARGET {
  // Unique ID used to reference the respective video present target by the
  // miniport and the OS.
  D3DDDI_VIDEO_PRESENT_TARGET_ID Id;

  // Type of the video output technology (see D3DKMDT_VIDEO_OUTPUT_TECHNOLOGY
  // for more details).
  D3DKMDT_VIDEO_OUTPUT_TECHNOLOGY VideoOutputTechnology;

  // Type of the video output's HPD awareness (see
  // D3DKMDT_VIDEO_OUTPUT_HPD_AWARENESS for more details).
  DXGK_CHILD_DEVICE_HPD_AWARENESS VideoOutputHpdAwareness;

  D3DKMDT_MONITOR_ORIENTATION_AWARENESS MonitorOrientationAwareness;

  // NOTE: On monitor arrival, OS will leave monitor mode set empty for monitors
  // connected to video outputs
  //       supporting SDTV modes, expecting miniport to populate modes it wants
  //       to expose for that monitor.
  //
  // NOTE: This predicate is also used when selecting video output for
  // presentation on a TV via
  //       APIs that don't support explicit specification of monitors/video
  //       outputs (e.g. VIDEOPARAMETERS).
  //
  // Predicate specifying whether corresponding video output supports SDTV
  // modes.
...
//
// Remarks: Video signal info is the mode of operation of a given video output
// that's driving a connected
//          moitor and is driven by an internal video output codec.
//          Note that this descriptor supersedes subset of the XDDM's
//          VIDEO_MODE_INFORMATION structure related to video mode. In XDDM,
//          both video and rendering modes were described in this struct. LDDM
//          separates these two notions, and hence their descriptors. The video
//          standard field, vidStandard, should be used for video mode
//          comparisons, when it's set to a well-defined video standard. Note
//          that most of the standard modes do not comply with the VESA GTF
//          frequency constraints.
//
//          If color basis of the target mode does not correspond to that of the
//          source mode, appropriate color conversion is performed by the
//          respective video output codec.
//
...
//  In the D3DKMDT_VIDPN_TARGET_MODE structure all fields are valid and the wire
//  format fields indicate supported bit depts per color encoding format. In the
//  DXGK_SET_TIMING_PATH_INFO structure the Preference field is unused so should
//  be ignored by the driver and will be set to zero by the OS in case of future
//  usage.  The wire format fields are used to indicate which wire format has
//  been selected so only one bit across the five fields will be set.
//
...
typedef union _D3DKMDT_WIRE_FORMAT_AND_PREFERENCE {
  struct {
    ...
    // Flags indicating supported formats of color components transmitted
...
#else
  // Predicate specifying whether this mode is preferred by the adapter given
  // the mode pinned on the source of the respective present path.
...
// Enum specifying monitor's aspect ratio (see
// DISPLAYID_DETAILED_TIMING_TYPE_I.AspectRatio)
...
typedef struct _DISPLAYID_DETAILED_TIMING_TYPE_I {
  ...
  struct {
    USHORT HorizontalFrontPorch
        : 15; // Horizontal offset (front porch) pixel number
  ...
  struct {
    USHORT VerticalFrontPorch
        : 15; // Number of lines of vertical offset (front porch)
...
C_ASSERT(sizeof(DISPLAYID_DETAILED_TIMING_TYPE_I) ==
...
typedef enum _D3DKMDT_MONITOR_CAPABILITIES_ORIGIN {
  ...
  D3DKMDT_MCO_MONITORDESCRIPTOR_REGISTRYOVERRIDE =
      3, //  + registry override of the monitor descriptor.
  D3DKMDT_MCO_SPECIFICCAP_REGISTRYOVERRIDE =
      4,                  //  + registry override of a specific capability.
...
typedef struct _D3DKMDT_MONITOR_SOURCE_MODE {
  ...
  // Color basis with respect to which monitor is presenting the pixels sampled
  // from the video signal.
  D3DKMDT_COLOR_BASIS ColorBasis;

  // Supported dynamic range of each of the pixel color component coefficients
  // by the monitor's presentational technology - e.g. for a DFP LCD with 12-bit
  // bit-depth, this will be (4, 4, 4, 0). A TV supporting 10-bit per component
  // YCbCr this will be (10, 10, 10, 0) regardless of chroma compression.
  ...
  // Predicate specifying whether this mode is preferred by the monitor
...
// VidPN present path index used during enumeration of paths satisfying certain
// criteria (e.g. originating from the same source).
...
typedef enum _D3DKMDT_VIDPN_PRESENT_PATH_SCALING {
  ...
  // For the following transformation, spatial resolutions must be equal on both
  // the source and the target.

  // Source content is not modified in any way.
  D3DKMDT_VPPS_IDENTITY = 1,

  // For the following three transformations, spatial resolution of the source
  ...
  // OS does not specify the scaling mode, and miniport should decide based on
  // its own settings.
...
typedef enum _D3DKMDT_VIDPN_PRESENT_PATH_ROTATION {
  ...
  // Source content is not modified in any way. Driver should rotate this
  // content an extra 90 degrees.
  D3DKMDT_VPPR_IDENTITY_OFFSET90 = 5,

  // Source content is rotated 90 degrees. Driver should rotate this content an
  // extra 90 degrees.
  D3DKMDT_VPPR_ROTATE90_OFFSET90 = 6,

  // Source content is rotated 180 degrees. Driver should rotate this content an
  // extra 90 degrees.
  D3DKMDT_VPPR_ROTATE180_OFFSET90 = 7,

  // Source content is rotated 270 degrees. Driver should rotate this content an
  // extra 90 degrees.
  D3DKMDT_VPPR_ROTATE270_OFFSET90 = 8,

  // Source content is not modified in any way. Driver should rotate this
  // content an extra 180 degrees.
  ...
  // OS does not specify the rotation mode, and miniport should decide based on
  // its own settings.
...
__inline D3DKMDT_VIDPN_PRESENT_PATH_ROTATION
D3DKMDT_VPPR_GET_OFFSET_ROTATION(D3DKMDT_VIDPN_PRESENT_PATH_ROTATION Rotation) {
...
__inline D3DKMDT_VIDPN_PRESENT_PATH_ROTATION D3DKMDT_VPPR_GET_CONTENT_ROTATION(
    D3DKMDT_VIDPN_PRESENT_PATH_ROTATION Rotation) {
  if ((Rotation >= D3DKMDT_VPPR_IDENTITY_OFFSET90) &&
      (Rotation <= D3DKMDT_VPPR_ROTATE270_OFFSET270)) {
    ...
    Rotation = (D3DKMDT_VIDPN_PRESENT_PATH_ROTATION)((((ContentPart - 1) +
                                                       (OffsetPart - 1)) %
                                                      4) +
                                                     1);
  }
...
// Purpose: Specifies what scaling modes are supported given current path
// configuration.
...
// Purpose: Specifies what rotation modes are supported given current path
// configuration.
...
//
// Note: Higher order importance ordinals can be used (upto 255), but only the
// first 10 have been explicitly named.
//
...
typedef enum _D3DKMDT_VIDPN_PRESENT_PATH_CONTENT {
  ...
  // Miniport should optimize presentation of the present path for graphics
  // content.
  D3DKMDT_VPPC_GRAPHICS = 1,

  // Miniport should optimize presentation of the present path for video
  // content.
  D3DKMDT_VPPC_VIDEO = 2,

  // OS does not specify the content type, and miniport should decide based on
  // its own settings.
...
typedef struct _D3DKMDT_GAMMA_RAMP {
  ...
  // If (Type == D3DDDI_GAMMARAMP_DEFAULT), (DataSize == 0) and (Data.pRaw ==
  // NULL )
  union {
    D3DDDI_GAMMA_RAMP_RGB256x3x16
        *pRgb256x3x16;                // Type == D3DDDI_GAMMARAMP_RGB256x3x16.
    D3DDDI_GAMMA_RAMP_DXGI_1 *pDxgi1; // Type == D3DDDI_GAMMARAMP_DXGI_1.
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_3)
    D3DKMDT_3x4_COLORSPACE_TRANSFORM
        *p3x4; // Type == D3DDDI_GAMMARAMP_MATRIX_3x4.
...
//
// Remarks: This type is used to describe a mapping from a single video present
// target to a single
...
typedef struct _D3DKMDT_VIDPN_PRESENT_PATH {
  ...
  // Video present path importance ordinal (e.g. path presenting the primary
  // view may have higher importance set on it in order to guarantee that it
  // gets the best source/target mode sets).
  ...
  // Storing deltas for visible/active pixels mapping rather than visible
  // pixels' size & offset has the added benefit of ideal/default state being
  // zeros.
  ...
  // Video signal color basis with respect to which video output codec encodes
  // the pixels from the respective source's primary surface (e.g. on HDMI this
  // can be either RGB or YCbCr).
  D3DKMDT_COLOR_BASIS VidPnTargetColorBasis;

  // Supported dynamic range of each of the color bases' coefficients by the
  // video output codec's output E.g. With a DAC scaning out A2R10G10B10 primary
  // surface at R8G8B8 color resolution, this would be (8,8,8,0),
  D3DKMDT_COLOR_COEFF_DYNAMIC_RANGES VidPnTargetColorCoeffDynamicRanges;

  // Indicates the content being diplayed.  The driver can use this to determine
  // whether the flicker filter and overscan should be enabled or disabled.
  ...
  // A lookup table (LUT) used to compensate intensity of presented content for
  // the color response of the monitor connected to present path's target.
  D3DKMDT_GAMMA_RAMP GammaRamp;
} D3DKMDT_VIDPN_PRESENT_PATH;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Purpose: Specifies whether to ignore monitor connectivity checks or enforce
// them.
//
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Purpose: Type of the owner whose mode set does not change during an
// enumeration.
//
// Remarks: When OS pins/unpins a mode in one of VidPN's mode sets, the only
// mode set that is guaranteed to
//          stay the same is the mode set whose mode was pinned/unpinned.
//          Enumeration pivot lets the OS specify to the miniport which mode set
//          it should not update (because this mode set could not have been
//          invalidated as a result of the change the OS made to the VidPN.
//
...
typedef struct _D3DKMDT_SHADOWSURFACEDATA {
  ...
  UINT Height;
  D3DDDIFORMAT Format;
  UINT Pitch; // out: Driver must return Pitch as this allocation will be
              // lockabale
...
DEFINE_GUID(DXGKMDT_OPM_GET_CURRENT_HDCP_SRM_VERSION, 0x99c5ceff, 0x5f1d,
            0x4879, 0x81, 0xc1, 0xc5, 0x24, 0x43, 0xc9, 0x48, 0x2b);
DEFINE_GUID(DXGKMDT_OPM_GET_CONNECTED_HDCP_DEVICE_INFORMATION, 0x0db59d74,
            0xa992, 0x492e, 0xa0, 0xbd, 0xc2, 0x3f, 0xda, 0x56, 0x4e, 0x00);
DEFINE_GUID(DXGKMDT_OPM_GET_CONNECTOR_TYPE, 0x81d0bfd5, 0x6afe, 0x48c2, 0x99,
            0xc0, 0x95, 0xa0, 0x8f, 0x97, 0xc5, 0xda);
DEFINE_GUID(DXGKMDT_OPM_GET_SUPPORTED_PROTECTION_TYPES, 0x38f2a801, 0x9a6c,
            0x48bb, 0x91, 0x07, 0xb6, 0x69, 0x6e, 0x6f, 0x17, 0x97);
DEFINE_GUID(DXGKMDT_OPM_GET_VIRTUAL_PROTECTION_LEVEL, 0xb2075857, 0x3eda,
            0x4d5d, 0x88, 0xdb, 0x74, 0x8f, 0x8c, 0x1a, 0x05, 0x49);
DEFINE_GUID(DXGKMDT_OPM_GET_ACTUAL_PROTECTION_LEVEL, 0x1957210a, 0x7766, 0x452a,
            0xb9, 0x9a, 0xd2, 0x7a, 0xed, 0x54, 0xf0, 0x3a);
DEFINE_GUID(DXGKMDT_OPM_GET_ACTUAL_OUTPUT_FORMAT, 0xd7bf1ba3, 0xad13, 0x4f8e,
            0xaf, 0x98, 0x0d, 0xcb, 0x3c, 0xa2, 0x04, 0xcc);
DEFINE_GUID(DXGKMDT_OPM_GET_ADAPTER_BUS_TYPE, 0xc6f4d673, 0x6174, 0x4184, 0x8e,
            0x35, 0xf6, 0xdb, 0x52, 0x0, 0xbc, 0xba);
DEFINE_GUID(DXGKMDT_OPM_GET_ACP_AND_CGMSA_SIGNALING, 0x6629a591, 0x3b79, 0x4cf3,
            0x92, 0x4a, 0x11, 0xe8, 0xe7, 0x81, 0x16, 0x71);
DEFINE_GUID(DXGKMDT_OPM_GET_OUTPUT_ID, 0x72cb6df3, 0x244f, 0x40ce, 0xb0, 0x9e,
            0x20, 0x50, 0x6a, 0xf6, 0x30, 0x2f);
DEFINE_GUID(DXGKMDT_OPM_GET_DVI_CHARACTERISTICS, 0xa470b3bb, 0x5dd7, 0x4172,
            0x83, 0x9c, 0x3d, 0x37, 0x76, 0xe0, 0xeb, 0xf5);
DEFINE_GUID(DXGKMDT_OPM_GET_OUTPUT_HARDWARE_PROTECTION_SUPPORT, 0x3b129589,
            0x2af8, 0x4ef0, 0x96, 0xa2, 0x70, 0x4a, 0x84, 0x5a, 0x21, 0x8e);
DEFINE_GUID(DXGKMDT_OPM_GET_CODEC_INFO, 0x4f374491, 0x8f5f, 0x4445, 0x9d, 0xba,
            0x95, 0x58, 0x8f, 0x6b, 0x58, 0xb4);
DEFINE_GUID(DXGKMDT_OPM_SET_PROTECTION_LEVEL, 0x9bb9327c, 0x4eb5, 0x4727, 0x9f,
            0x00, 0xb4, 0x2b, 0x09, 0x19, 0xc0, 0xda);
DEFINE_GUID(DXGKMDT_OPM_SET_ACP_AND_CGMSA_SIGNALING, 0x09a631a5, 0xd684, 0x4c60,
            0x8e, 0x4d, 0xd3, 0xbb, 0x0f, 0x0b, 0xe3, 0xee);
DEFINE_GUID(DXGKMDT_OPM_SET_HDCP_SRM, 0x8b5ef5d1, 0xc30d, 0x44ff, 0x84, 0xa5,
            0xea, 0x71, 0xdc, 0xe7, 0x8f, 0x13);
DEFINE_GUID(DXGKMDT_OPM_SET_PROTECTION_LEVEL_ACCORDING_TO_CSS_DVD, 0x39ce333e,
            0x4cc0, 0x44ae, 0xbf, 0xcc, 0xda, 0x50, 0xb5, 0xf8, 0x2e, 0x72);
...
typedef enum _DXGKMDT_OPM_TYPE_ENFORCEMENT_HDCP_PROTECTION_LEVEL {
  ...
  DXGKMDT_OPM_TYPE_ENFORCEMENT_HDCP_ON_WITH_NO_TYPE_RESTRICTION =
      DXGKMDT_OPM_HDCP_ON, // No Type restriction on output topology nodes
  DXGKMDT_OPM_TYPE_ENFORCEMENT_HDCP_ON_WITH_TYPE1_RESTRICTION =
      DXGKMDT_OPM_HDCP_ON +
      1, // Restrict to Type 1 compliant output topology nodes
...
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_5)
  ULONGLONG MaxTransitionLatency; // out: Max transition latency to change the
...
typedef struct _DXGK_ADAPTER_PERFDATA {
  ...
  ULONGLONG MaxMemoryFrequencyOC; // out: Clock frequency of the memory while
...
typedef struct _D3DKMT_WDDM_2_7_CAPS {
  union {
    struct {
      UINT HwSchSupported
          : 1; // Specifies whether the GPU supports hardware scheduling
      UINT HwSchEnabled : 1; // Specifies whether the hardware scheduling is
                             // currently enabled for this GPU
      UINT HwSchEnabledByDefault : 1; // Set to 1 if the OS default policy is to
                                      // enable hardware scheduling for this GPU
...
// When a driver doesn't support a feature, it doesn't call into
// QueryFeatureSupport with that feature ID. This value is provided for
// implementation convenience of enumerating possible driver support states for
  ...
  union {
    struct {
      _Field_range_(DXGK_FEATURE_SUPPORT_ALWAYS_OFF,
                    DXGK_FEATURE_SUPPORT_ALWAYS_ON) UINT HwSchSupportState
          : 2; // DXGK_FEATURE_SUPPORT_* value that specifies driver support
               // state for GPU supports hardware scheduling
      UINT HwSchEnabled : 1; // Specifies whether the hardware scheduling is
                             // currently enabled for this GPU
      UINT SelfRefreshMemorySupported : 1; // Specifies whether Self Refresh
                                           // Memory is supported for this GPU
      UINT Reserved : 28;
    };
    UINT Value;
  };
...
typedef struct _D3DKMT_WDDM_3_0_CAPS {
  union {
    struct {
      _Field_range_(DXGK_FEATURE_SUPPORT_ALWAYS_OFF,
                    DXGK_FEATURE_SUPPORT_ALWAYS_ON) UINT HwFlipQueueSupportState
          : 2; // DXGK_FEATURE_SUPPORT_* value that specifies driver support
               // state for GPU supports hardware flip queue
      UINT HwFlipQueueEnabled : 1; // Specifies whether the hardware flip queue
                                   // is currently enabled for this GPU
      UINT DisplayableSupported
          : 1; // Specifies whether displayable feature is supported
      UINT Reserved : 28;
    };
    UINT Value;
  };
...
typedef struct _D3DKMT_WDDM_3_1_CAPS {
  union {
    struct {
      UINT NativeGpuFenceSupported
          : 1; // Specifies whether native GPU fence is supported by this GPU
      UINT Reserved : 31;
    };
    UINT Value;
  };
...
typedef struct _D3DKMT_NODEMETADATA {
  _In_ UINT NodeOrdinalAndAdapterIndex; // WDDMv2: High word is physical adapter
                                        // index, low word is node ordinal
...
typedef struct _D3DKMT_QUERYCLOCKCALIBRATION {
  ...
  DXGK_GPUCLOCKDATA ClockData; // out: Output frequency, Gpu clock, and Cpu
                               // clock on the specified engine
...
typedef enum _DXGK_PAGE_FAULT_FLAGS {
  DXGK_PAGE_FAULT_WRITE =
      0x1, // when set, indicates that a write to a GPU virtual address failed
  DXGK_PAGE_FAULT_FENCE_INVALID =
      0x2, // when set, indicates that a fence value for the packet that
           // generated the page fault could not be determined
  DXGK_PAGE_FAULT_ADAPTER_RESET_REQUIRED =
      0x4, // when set, indicates that the page fault put the GPU in a state
           // that requires a full adapter reset
  DXGK_PAGE_FAULT_ENGINE_RESET_REQUIRED =
      0x8, // when set, indicates that the page fault put the GPU in a state
           // that requires GPU engine reset
  DXGK_PAGE_FAULT_FATAL_HARDWARE_ERROR =
      0x10, // when set, indicates that the system hardware, not just the GPU,
            // cannot continue, and the OS should issue a bugcheck
  DXGK_PAGE_FAULT_IOMMU =
      0x20, // when set, indicates that the faulting GPU virtual address was
            // mapped using IoMmu, when not set, the faulting GPU virtual
            // address was mapped using GPU memory management unit.

#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WDDM2_2)

  DXGK_PAGE_FAULT_HW_CONTEXT_VALID =
      0x40, // when set, indicates that the faulting hardware queue was not
            // identified, but faulting HW context was.
  DXGK_PAGE_FAULT_PROCESS_HANDLE_VALID =
      0x80, // when set, indicates that the faulting hardware queue or context
            // was not identified, but process handle that submitted the faulted
            // buffer was.
...
typedef struct _DXGK_FAULT_ERROR_CODE {
  union {
    struct {
      UINT IsDeviceSpecificCode
          : 1; // when set to 1, indicates the low 31 bits correspond to a
               // device specific error code. when set to 0, indicates the low
               // 31 bits correspond to a code defined by
               // DXGK_GENERAL_ERROR_CODE.
      DXGK_GENERAL_ERROR_CODE GeneralErrorCode
          : 31; // General error code defined by DXGK_GENERAL_ERROR_CODE.
    };
    struct {
      UINT IsDeviceSpecificCodeReservedBit
          : 1; // Same as IsDeviceSpecificCode member above:
               // when set to 1, indicates the low 31 bits correspond to a
               // device specific error code. when set to 0, indicates the low
               // 31 bits correspond to a code defined by
               // DXGK_GENERAL_ERROR_CODE.
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Capabilities, preferences and other information reported by display only
// capable adapters.
