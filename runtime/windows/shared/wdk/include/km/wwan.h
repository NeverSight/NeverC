...
// For following bit in the field StatusFlags: 0 -- the indication came from the currently inactive slot; 1 -- the indication came
// from the currently active slot This bit may be set independent of WWAN_READY_STATUS_MASK_MASKS_IS_VALID bit
...
typedef struct _WWAN_PLMN
{
    ...
    UINT16 Mcc;
    // Mobile Network Code of either 3 or 2 decimal digits; The most significant bit indicates whether the MNC has 2 decimal
    // digits or 3 decimal digits. If this bit has 1, the MNC has 2 decimal digits and the least significant 8 bits contains them
    // in BCD-encoded form sequentially, with the last digit of the MNC in the least significant 4 bits. If the most significant
    // bit has 0, the MNC has 3 decimal digits and the least significant 12 bits contains them in BCD-encoded form sequentially,
    // with the last digit of the MNC in the least significant 4 bits. The unused bits in the UINT16 integer must be zeros. For
    // example, mnc 23 is 0x3 0x2 0x0 0x0 in in memory, and mnc 023 is 0x3 0x2 0x0 0x8
    UINT16 Mnc;
...
typedef struct _WWAN_NETWORK_PARAMS_INFO
{
    ...
    // The first 4 bytes are offset to the DataBuffer containing a list of WWAN_TAI_LIST, in bytes, calculated from the beginning
    // of WWAN_NETWORK_PARAMS_INFO. The second 4 bytes are size of a list of WWAN_TAI_LIST, in bytes.
