 ...
 *
 * Supports UUID v4 (random), UUID v7 (Unix-ms + monotonic counter), and parsing/formatting.
 */
...
/* Generate a UUID v7 (Unix milliseconds + monotonic counter). Nil on CSPRNG failure. */
neverc_uuid_t neverc_uuid_new_v7(void);

/* Generate a UUID v7 with explicit error reporting. Clears out on failure.
 * Consecutive successes from this process are strictly byte-ordered. */
