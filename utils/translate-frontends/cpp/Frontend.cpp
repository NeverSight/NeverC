    ...
    if (S.project()) {
      ...
      if (!Include.empty()) {
        ...
        if (llvm::sys::fs::real_path(Absolute, Real) ||
            ...
            (Real != S.Root && S.relativePath(Real).empty())) {
        ...
        S.Arguments.push_back(Real.str().str());
        continue;
      }
    }
...
static bool request(State &S, llvm::StringRef Path) {
  ...
  if (!Buffer || (*Buffer)->getBufferSize() > 1024 * 1024)
    return false;
  auto Data = json::parse((*Buffer)->getBuffer());
  if (!Data) {
  ...
  if (llvm::sys::fs::real_path(S.Root, RealRoot) ||
    ...
    return false;
  auto Prefix = RealRoot.str().str() + "/";
  if (!RealSource.str().starts_with(Prefix))
    return false;
  S.Relative = RealSource.str().drop_front(Prefix.size()).str();
  S.Root = RealRoot.str().str();
  S.Source = RealSource.str().str();
  ...
  if (S.project()) {
    ...
    if (llvm::sys::fs::real_path(*Working, RealWorking) ||
        !llvm::sys::fs::is_directory(RealWorking) ||
        (RealWorking != S.Root && S.relativePath(RealWorking).empty()))
      return false;
    S.WorkingDirectory = RealWorking.str().str();
  ...
  auto SourceBuffer = llvm::MemoryBuffer::getFile(S.Source);
  if (!SourceBuffer || (*SourceBuffer)->getBufferSize() > 8 * 1024 * 1024)
    return false;
  S.SourceText = (*SourceBuffer)->getBuffer().str();
  return true;
}
...
int main(int Argc, const char **Argv) {
  ...
  for (int I = 1; I < Argc; ++I) {
    ...
    if (I + 1 >= Argc || (Arg != "--request" && Arg != "--output")) {
      llvm::errs() << "usage: neverc-cpp-frontend --request request.json "
  ...
  if (Valid && S.project() && !isolateProjectEnvironment()) {
    S.diagnose(
        ...
        "Run the helper in an environment with removable compiler "
