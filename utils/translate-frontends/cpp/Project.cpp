...
std::string State::relativePath(llvm::StringRef Path) const {
  ...
  if (llvm::sys::fs::real_path(Path, Real))
    return {};
  auto Prefix = Root + "/";
  auto Relative = Real.str().starts_with(Prefix)
                      ? Real.str().drop_front(Prefix.size()).str()
                      : "";
...
bool isolateProjectEnvironment() {
  // This helper is a separate, single-request process. Environment changes are
  // local to it and happen before Clang creates its driver or worker threads.
