...
`remove_copy`, `replace`, `replace_copy`, `unique` and `unique_copy`; built-in
arithmetic elements admit default-order
...
and `fill_n` accepts integral or non-scoped enum counts through 64 bits after
