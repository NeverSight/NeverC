...
the nine unary algorithms below and addresses of standard algorithms remain rejected, as do calls outside a
documented direct lowering.

`find_if`, `find_if_not`, `none_of`, `all_of`, `any_of`, `count_if`,
`replace_if`, `replace_copy_if` and `remove_copy_if` additionally accept ordinary source-owned
...
`remove_copy_if` also accepts the same source predicate objects. Its exact
conditional copy loop and both iterator increments are authenticated; output
...
For these nine source-object overloads, `decltype`, `noexcept` and queries of
