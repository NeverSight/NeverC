...
Clang's exact locationless false literal for an unwritten inferred exception
specification instead retains its actual special-member/destructor owner as a
...
`<optional>`, `<iterator>`, `<algorithm>`, `<numeric>`, `<new>`, `<memory>`,
`<functional>` and `<ratio>`. A
...
```json
{
  ...
  "sdk_distribution_id": "neverc-embedded-clang20.1.8-libcxx200100-macos15.5-r11",
...
array source address before reading any scalar or recursively admitted
composite element, then uses existing member, array-decay, index and assign
nodes in concatenation order. The zero-argument and all-empty cases produce the
authenticated empty result without element operations.
