...
| Ordinary constructors in `cpp-core-v2` | User-provided default/converting/explicit constructors for standard-layout, trivially copyable records with trivial destruction. Fixtures check final destination identity, field declaration order, per-element array construction, materialization, hidden signatures and old-profile rejection. User copy/move/destruction, implicit nontrivial default construction, cleanup, exceptions and STL remain outside this increment; native results require the implementing revision. |
| Record call storage in `cpp-core-v2` | By-value record parameters use separate caller-prepared objects; record results use explicit destination pointers. Source-required copies and reference aliases remain intact. Protocol fixtures cover hidden signatures, direct forwarding, parameter separation and v1 compatibility; O0/O2 no-inline fixtures cover identity, mutation, recursion, receiver sequencing and nested initialization. This chosen convention for trivial records does not admit nontrivial copy/move/destruction or STL; native results require the implementing revision. |
