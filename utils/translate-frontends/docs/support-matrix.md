...
| Ordinary methods in `cpp-core-v2` | Named nonvirtual instance/const/lvalue-qualified and static methods of admitted trivial aggregates use verified ordinary calls with an explicit this pointer where required. Protocol checks cover signatures, identity and receiver copies; O0/O2 no-inline fixtures cover mutation, const overloads, reference returns and receiver/argument effects. Temporary nonstatic receivers, special members, templates and STL remain excluded; native results require the implementing revision. |
