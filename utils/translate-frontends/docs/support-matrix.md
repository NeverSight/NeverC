...
| Defined scalar static data members in `cpp-core-v2` | Defined integer/bool/enum members, including inline/constexpr and out-of-line definitions, share ordinary typed global storage. Constant/zero initialization, receiver effects, temporary cleanup and lasting static aliases are covered by fixtures. Every declared member currently needs a definition in this source unit, including non-inline const value-only uses; dynamic initialization and other static types remain restricted. Native validation requires the implementing revision. |
