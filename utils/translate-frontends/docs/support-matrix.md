...
| Ordinary constructors in `cpp-core-v2` | User-provided default/converting/explicit constructors for standard-layout records with admitted copying and destruction. Fixtures check final destination identity, field declaration order, per-element array construction, materialization, hidden signatures and old-profile rejection. Moves, implicit nontrivial default construction/copying, exceptions and STL remain outside this increment; native results require the implementing revision. |
...
| User copy operations in `cpp-core-v2` | Ordinary user-provided copy constructors and copy assignment with lvalue-reference sources execute through checked destination/receiver calls. O0/O2 fixtures cover selected source overloads, side effects, actual object identities, returned aliases, lifetime counts and operator-vs-member-call ordering. Protocol fixtures check selected functions, no extra prvalue copying, full signatures and relocation. Defaulted/implicit nontrivial special members, moves and general operators remain excluded; native results require the implementing revision. |
