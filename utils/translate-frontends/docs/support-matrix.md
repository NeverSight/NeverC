...
| Experimental `cpp-core-v2` | Adds checked type aliases, int/uint32 enums, static assertions, bounded object pointers, lvalue references, fixed local arrays/array fields, array decay and indexing to the single-source core contract. See the [core v2 contract](cpp-core-v2.md); earlier v1 CI evidence does not establish v2 execution support. |
