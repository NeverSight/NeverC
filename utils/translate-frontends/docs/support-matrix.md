...
| User copy operations in `cpp-core-v2` | Ordinary user-provided copy constructors and copy assignment with lvalue-reference sources execute through checked destination/receiver calls. O0/O2 fixtures cover selected source overloads, side effects, actual object identities, returned aliases, lifetime counts and operator-vs-member-call ordering. Protocol fixtures check selected functions, no extra prvalue copying, full signatures and relocation. General operators remain excluded; native results require the implementing revision. |
