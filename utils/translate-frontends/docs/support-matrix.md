...
| Resolved constexpr-if in `cpp-core-v2` | Non-template constant branch selection emits only the selected substatement, preserves runtime init/condition-variable storage and reverse cleanup, and avoids discarded automatic storage under outer switch registration. Both source branches remain checked and the stricter full-definition requirement remains in force; dependent templates and C++23 consteval branches stay excluded. Native validation requires the implementing revision; full C++/STL remains unfinished. |
