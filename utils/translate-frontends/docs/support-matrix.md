...
| Live rvalue references in `cpp-core-v2` | `T&&` aliases, parameters/results, conditional xvalues and ordinary `&&` methods retain the address of existing live scalar, pointer, record and array objects. Named rvalue references remain lvalues; resolved overloads and admitted source copies are preserved. Fixtures cover signatures, aliases, sequencing, arrays, cleanup counts and relocation. Lifetime extension and unsupported moves remain excluded; full-expression temporary calls follow their separate row; native results require the implementing revision. |
