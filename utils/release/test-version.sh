...
test_tag_matches_cmake_version() {
  ...
  if ! sh "$repo_root/utils/release/check-version.sh" \
    ...
    fail "v3389.1.2 did not match the CMake version"
  fi

  [ "$(cat "$output_file")" = 3389.1.2 ] \
...
test_mismatched_tag_is_rejected() {
  ...
  if sh "$repo_root/utils/release/check-version.sh" \
    "$repo_root/llvm/CMakeLists.txt" v3389.1.1 > "$output_file" 2>&1; then
    fail "checker accepted a tag that does not match the CMake version"
  fi

  grep -F "does not match CMake version 'v3389.1.2'" "$output_file" > /dev/null \
