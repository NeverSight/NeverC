...
public:
  SymbolTableWriter(ELFWriter &EWriter);
...
SymbolTableWriter::SymbolTableWriter(ELFWriter &EWriter)
...
void ELFWriter::createMemtagRelocs(MCAssembler &Asm) {
  ...
  for (const MCSymbol &Sym : Asm.symbols()) {
    ...
    if (MemtagRelocs == nullptr) {
      MemtagRelocs =
          OWriter.TargetObjectWriter->getMemtagRelocsSection(Asm.getContext());
      if (MemtagRelocs == nullptr)
        report_fatal_error(
            "Tagged globals are not available on this architecture.");
...
MCSectionELF *ELFWriter::createRelocationSection(MCContext &Ctx,
  ...
  unsigned EntrySize = Rela ? sizeof(ELF::Elf64_Rela) : sizeof(ELF::Elf64_Rel);
...
bool ELFWriter::maybeWriteCompression(
  ...
  uint64_t HdrSize = sizeof(ELF::Elf64_Chdr);
