...
void MachObjectWriter::writeHeader(MachO::HeaderFileType Type,
  ...
  W.write<uint32_t>(MachO::MH_MAGIC_64);
  ...
  assert(W.OS.tell() - Start == sizeof(MachO::mach_header_64));
}
...
void MachObjectWriter::writeSegmentLoadCommand(
  ...
  unsigned SegmentLoadCommandSize = sizeof(MachO::segment_command_64);
  W.write<uint32_t>(MachO::LC_SEGMENT_64);
...
void MachObjectWriter::writeSection(const MCAsmLayout &Layout,
  ...
  assert(W.OS.tell() - Start == sizeof(MachO::section_64));
}
...
void MachObjectWriter::writeLinkerOptionsLoadCommand(
  ...
  W.OS.write_zeros(offsetToAlignment(BytesWritten, Align(8)));
...
uint64_t MachObjectWriter::writeObject(MCAssembler &Asm,
  ...
  uint64_t LoadCommandsSize = sizeof(MachO::segment_command_64) +
                              NumSections * sizeof(MachO::section_64);
  ...
  uint64_t LOHSize = alignTo(LOHRawSize, 8);
  if (LOHSize) {
    ++NumLoadCommands;
    LoadCommandsSize += sizeof(MachO::linkedit_data_command);
  }
  ...
  for (const auto &Option : Asm.getLinkerOptions()) {
    ++NumLoadCommands;
    LoadCommandsSize += ComputeLinkerOptionsLoadCommandSize(Option);
  }

  // Compute the total size of the section data, as well as its file size and vm
  // size.
  uint64_t SectionDataStart = sizeof(MachO::mach_header_64) + LoadCommandsSize;
  ...
  unsigned SectionDataPadding =
      offsetToAlignment(SectionDataFileSize, Align(8));
  ...
  auto EmitDeploymentTargetVersion =
      [&](const MCAssembler::VersionInfoType &VersionInfo) {
        ...
        uint32_t SDKVersion = !VersionInfo.SDKVersion.empty()
                                  ...
                                  : 0;
        if (VersionInfo.EmitBuildVersion) {
          // Currently empty ntools field; LC_BUILD_VERSION requires it.
  ...
  if (NumSymbols) {
    ...
    uint64_t StringTableOffset =
        SymbolTableOffset + NumSymTabSymbols * sizeof(MachO::nlist_64);
  ...
  if (LOHSize) {
    ...
    // Pad to a multiple of the pointer size.
    W.OS.write_zeros(offsetToAlignment(LOHRawSize, Align(8)));
