...
static std::unique_ptr<Module> reportNonBitcode(const MemoryBufferRef &Buffer,
                                                SMDiagnostic &Err) {
  Err = SMDiagnostic(Buffer.getBufferIdentifier(), SourceMgr::DK_Error,
                     "neverc: textual LLVM IR (.ll) input is not supported; "
                     "only bitcode (.bc / embedded .o) is accepted");
  return nullptr;
}
...
llvm::getLazyIRModule(std::unique_ptr<MemoryBuffer> Buffer, SMDiagnostic &Err,
  ...
  return reportNonBitcode(Buffer->getMemBufferRef(), Err);
}
...
std::unique_ptr<Module> llvm::parseIR(MemoryBufferRef Buffer, SMDiagnostic &Err,
  ...
  return reportNonBitcode(Buffer, Err);
}
