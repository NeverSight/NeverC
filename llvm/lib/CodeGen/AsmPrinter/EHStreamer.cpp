...
void EHStreamer::computeCallSiteTable(
  ...
  for (const auto &MBB : *Asm->MF) {
    ...
    for (const auto &MI : MBB) {
      ...
      // between the try-ranges (SjLj handling doesn't use this).
...
MCSymbol *EHStreamer::emitExceptionTable() {
  ...
  unsigned CallSiteEncoding = Asm->getObjFileLowering().getCallSiteEncoding();
  ...
  //  * For SJLJ, they will be emitted only once in the LSDA header.
  auto EmitTypeTableRefAndCallSiteTableEndRef = [&]() {
    ...
    // label difference from here (start of the call-site table for SJLJ,
    // and start of a call-site range for Itanium) to the end of the
...

  {
    ...
    for (const CallSiteRange &CSRange : CallSiteRanges) {
      ...
      if (CallSiteRanges.size() == 1 || LandingPadRange == nullptr) {
        ...
        // Emit a PC-relative address for LPStart.
