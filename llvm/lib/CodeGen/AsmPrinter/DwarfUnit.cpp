...
void DwarfUnit::updateAcceleratorTables(const DIScope *Context,
  ...
  if (!Ty->getName().empty() && !Ty->isForwardDecl()) {
    ...
    if (auto *CT = dyn_cast<DICompositeType>(Ty)) {
      // A runtime language of 0 actually means C/C++.
...
std::string DwarfUnit::getParentContextString(const DIScope *) const {
  return "";
}
...
void DwarfUnit::constructTypeDIE(DIE &Buffer, const DICompositeType *CTy) {
  ...
  case dwarf::DW_TAG_namelist: {
    ...
    for (const auto *Element : Elements) {
      if (!Element)
        continue;
      if (auto *DDTy = dyn_cast<DIDerivedType>(Element)) {
        if (Tag == dwarf::DW_TAG_variant_part) {
...
void DwarfUnit::constructEnumTypeDIE(DIE &Buffer, const DICompositeType *CTy) {
  ...
  bool IndexEnumerators = !Context || isa<DICompileUnit>(Context) ||
                          isa<DIFile>(Context) || isa<DICommonBlock>(Context);
  DINodeArray Elements = CTy->getElements();
