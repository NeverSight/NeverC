...
void DwarfDebug::finishUnitAttributes(const DICompileUnit *DIUnit,
  ...
  if (DIUnit->getDWOId()) {
    // This CU is either a NeverC module DWO or a skeleton CU.
...
void DwarfDebug::finalizeModuleInfo() {
  ...
  // This handles LTO's situation where imported CUs may very easily be
  // duplicate with the same CU partially imported into another LTO unit.
  StringRef DWOName;
  if (CUMap.size() > 1)
  ...
  // Emit all frontend-produced Skeleton CUs, i.e., NeverC modules.
...
void DwarfDebug::emitDebugLocValue(const AsmPrinter &AP, const DIBasicType *BT,
  ...
  auto EmitValueLocEntry = [&DwarfExpr, &BT,
    ...
    if (Entry.isInt()) {
      ...
      if (AP.getDwarfVersion() >= 4 && !Cursor) {
