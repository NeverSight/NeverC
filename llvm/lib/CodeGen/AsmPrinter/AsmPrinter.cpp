...
static cl::opt<std::string> BasicBlockProfileDump(
    ...
    cl::desc("Basic block profile dump for external cost modelling. If "
             ...
             "flag during in-process LTO is not supported."));
...
MCSymbol *AsmPrinter::getSymbolPreferLocal(const GlobalValue &GV) const {
  ...
  if (TM.getTargetTriple().isOSBinFormatELF() &&
    ...
    if (M.getPIELevel() == PIELevel::Default && GV.isDSOLocal())
...
void AsmPrinter::emitGlobalVariable(const GlobalVariable *GV) {
  ...
  // sections and expected to be contiguous.
...
void AsmPrinter::emitFunctionBody() {
  ...
  bool EmitFunctionSize = MAI->hasDotTypeDotSizeDirective();
...
void AsmPrinter::emitGlobalAlias(Module &M, const GlobalAlias &GA) {
  ...
  // Treat bitcasts of functions as functions also.
  if (!IsFunction)
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  // Some targets' linkers expect aliases in this order.
...
void AsmPrinter::SetupMachineFunction(MachineFunction &MF) {
  ...
  if (F.hasFnAttribute("patchable-function-entry") || needFuncLabels(MF) ||
...
static void emitGlobalConstantFP(APFloat APF, Type *ET, AsmPrinter &AP) {
  ...
  // ppcf128 has different endianness conventions: p[0] goes first for
  // big-endian targets using this type.
