...
void RegPressureTracker::getMaxUpwardPressureDelta(
    ...
    ArrayRef<PressureChange> CriticalPSets,
    ArrayRef<unsigned> MaxPressureLimit) {
  // Snapshot pressure using stack-allocated storage (avoids heap alloc per call
  // in the hot scheduling loop; typical AArch64 has ~23 pressure sets).
  SmallVector<unsigned, 32> SavedPressure(CurrSetPressure.begin(),
                                          CurrSetPressure.end());
  SmallVector<unsigned, 32> SavedMaxPressure(P.MaxSetPressure.begin(),
                                             P.MaxSetPressure.end());
  ...
  P.MaxSetPressure.assign(SavedMaxPressure.begin(), SavedMaxPressure.end());
  CurrSetPressure.assign(SavedPressure.begin(), SavedPressure.end());
