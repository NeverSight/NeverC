...
// ISD::SDIV of type v2i64.  The type is legal (for example, addition on a
// v2i64 is legal), but ISD::SDIV isn't legal, so we have to unroll the
// operation, which may introduce types that must be expanded.  Similarly,
// unrolling an ISD::SRA of type v16i8 may introduce
// nodes with the illegal type i8 which must be promoted.
//
