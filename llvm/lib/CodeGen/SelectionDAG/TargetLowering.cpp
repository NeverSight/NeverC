...
unsigned TargetLowering::getJumpTableEncoding() const {
  // If the target supports a GPRel32 directive, use it.
...
bool TargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  (void)GA;
  // NeverC keeps PIC always enabled, so folding a global address offset is
  // never legal here because code generation always needs a PIC base.
  return false;
}
...
bool TargetLowering::SimplifyDemandedBits(
  ...
  case ISD::UMAX: {
    ...
    switch (Opc) {
    case ISD::SMIN: {
      int IsSLE = KnownBits::sle(Known0, Known1);
      if (IsSLE >= 0)
        return TLO.CombineTo(Op, IsSLE ? Op0 : Op1);
      int IsSLT = KnownBits::slt(Known0, Known1);
      if (IsSLT >= 0)
    ...
    case ISD::SMAX: {
      int IsSGE = KnownBits::sge(Known0, Known1);
      if (IsSGE >= 0)
        return TLO.CombineTo(Op, IsSGE ? Op0 : Op1);
      int IsSGT = KnownBits::sgt(Known0, Known1);
      if (IsSGT >= 0)
    ...
    case ISD::UMIN: {
      int IsULE = KnownBits::ule(Known0, Known1);
      if (IsULE >= 0)
        return TLO.CombineTo(Op, IsULE ? Op0 : Op1);
      int IsULT = KnownBits::ult(Known0, Known1);
      if (IsULT >= 0)
    ...
    case ISD::UMAX: {
      int IsUGE = KnownBits::uge(Known0, Known1);
      if (IsUGE >= 0)
        return TLO.CombineTo(Op, IsUGE ? Op0 : Op1);
      int IsUGT = KnownBits::ugt(Known0, Known1);
      if (IsUGT >= 0)
...
SDValue TargetLowering::foldSetCCWithAnd(EVT VT, SDValue N0, SDValue N1,
  ...
  if (isXAndYEqZeroPreferableToXAndYEqY(Cond, OpVT) &&
    ...
    // more efficient ways to deal with that case (for example, 'bt' on x86).
...
SDValue TargetLowering::BuildUDIV(SDNode *N, SelectionDAG &DAG,
  ...
  auto BuildUDIVPattern = [&](ConstantSDNode *C) {
    ...
    if (Divisor.isOne()) {
      ...
      // method on 64-bit targets: pre-shift c by (64-a) bits to eliminate
      // runtime shift sequence. Requires 64x64->128 bit high multiply.
      // Only for scalar types since SIMD lacks 64x64->128 high multiply.
      ...
      if (EltBits == 32 && !VT.isVector() && Has64BitMULHU && magics.IsAdd) {
        ...
        APInt RealMagic = APInt(65, 1).shl(32) + magics.Magic.zext(65);
        ...
        NPQFactor = DAG.getConstant(APInt::getZero(EltBits), dl, SVT);
        UseNPQ = false;
        UsePreShift = false;
        UsePostShift = false;
      } else {
  ...
  if (Use33BitOptimization) {
    ...
    if (isOperationLegalOrCustom(ISD::MULHU, WideVT, IsAfterLegalization)) {
      Result = DAG.getNode(ISD::MULHU, dl, WideVT, X64, MagicFactor64);
      Created.push_back(Result.getNode());
    } else if (isOperationLegalOrCustom(ISD::UMUL_LOHI, WideVT,
                                        IsAfterLegalization)) {
      SDValue LoHi =
          DAG.getNode(ISD::UMUL_LOHI, dl, DAG.getVTList(WideVT, WideVT), X64,
                      MagicFactor64);
...
SDValue TargetLowering::expandIS_FPCLASS(EVT ResultVT, SDValue Op,
  ...
  // ppcf128 is a pair of doubles, of which the higher part determines the
  // value class.
...
TargetLowering::scalarizeVectorLoad(LoadSDNode *LD, SelectionDAG &DAG) const {
  ...
  for (unsigned Idx = 0; Idx < NumElem; ++Idx) {
    uint64_t Offset = uint64_t(Idx) * Stride;
    SDValue ScalarLoad =
        DAG.getExtLoad(ExtType, SL, DstEltVT, Chain, BasePTR,
                       LD->getPointerInfo().getWithOffset(Offset), SrcEltVT,
...
SDValue TargetLowering::scalarizeVectorStore(StoreSDNode *ST,
  ...
  for (unsigned Idx = 0; Idx < NumElem; ++Idx) {
    ...
    SDValue Ptr =
        DAG.getObjectPtrOffset(SL, BasePtr, TypeSize::getFixed(Offset));

    // This scalar TruncStore may be illegal, but we legalize it later.
    SDValue Store = DAG.getTruncStore(
        Chain, SL, Elt, Ptr, ST->getPointerInfo().getWithOffset(Offset),
