...
SelectionDAGISel::SelectionDAGISel(char &ID, TargetMachine &tm,
    ...
    : MachineFunctionPass(ID), TM(tm), FuncInfo(new FunctionLoweringInfo()),
      ...
      SDB(std::make_unique<SelectionDAGBuilder>(*CurDAG, *FuncInfo, OL)),
      OptLevel(OL) {
...
bool SelectionDAGISel::runOnMachineFunction(MachineFunction &mf) {
  ...
  CurDAG->init(*MF, *ORE, this, LibInfo, PSI, BFI, FnVarLocs);
...
void SelectionDAGISel::SelectAllBasicBlocks(const Function &Fn) {
  ...
  for (const BasicBlock *LLVMBB : RPOT) {
    ...
    if (Begin != BI) {
      // Run SelectionDAG instruction selection on the entire block.
      bool HadTailCall;
