...
SDValue SelectionDAGBuilder::lowerEndEH(SDValue Chain, const InvokeInst *II,
  ...
  auto Pers = classifyEHPersonality(FuncInfo.Fn->getPersonalityFn());
  // Some personalities use funclet style IR but do not use outlined funclets.
...
TargetLowering::LowerCallTo(TargetLowering::CallLoweringInfo &CLI) const {
  ...
  for (unsigned i = 0, e = Args.size(); i != e; ++i) {
    ...
    for (unsigned Value = 0, NumValues = ValueVTs.size(); Value != NumValues;
      ...
      // Certain targets may have a different ABI alignment for a type
      // depending on the context. Give the target a chance to specify
      // the alignment it wants.
...
void SelectionDAGISel::LowerArguments(const Function &F) {
  ...
  for (const Argument &Arg : F.args()) {
    ...
    for (unsigned Val = 0; Val != NumValues; ++Val) {
      ...
      // Even for currently-unused arguments we still materialize values for
      // argument-related lowering.
      std::optional<ISD::NodeType> AssertOp;
      if (Arg.hasAttribute(Attribute::SExt))
        AssertOp = ISD::AssertSext;
