...
void FunctionLoweringInfo::set(const Function &fn, MachineFunction &mf,
  ...
  if (isFuncletEHPersonality(Personality)) {
    ...
    for (SEHUnwindMapEntry &UME : EHInfo.SEHUnwindMap) {
      ...
      // mapped to a MachineBasicBlock. However, for some SEH patterns
      // (especially after outlining/optimization) the handler may belong to a
      // different IR function. In that case we cannot map it through this
      // function's MBBMap; keep it as an IR BasicBlock and let the AsmPrinter
      // handle the fallback.
...
Register FunctionLoweringInfo::CreateRegs(const Value *V) {
  return CreateRegs(V->getType(), false);
}
