...
SDValue SelectionDAG::getConstant(const ConstantInt &Val, const SDLoc &DL,
  ...
  // In other cases the element type is illegal and needs to be expanded.
  // Find the nearest legal type, split the value into n parts and use a
  // vector type with n-times the elements.
...
bool SelectionDAG::isKnownNeverZero(SDValue Op, unsigned Depth) const {
  ...
  case ISD::SUB: {
    ...
    int ne = KnownBits::ne(computeKnownBits(Op.getOperand(0), Depth + 1),
    ...
    if (auto *C = dyn_cast<ConstantSDNode>(N1)) {
      ...
      case ISD::ZERO_EXTEND:
        return getConstant(Val.zextOrTrunc(VT.getSizeInBits()), DL, VT,
                           C->isTargetOpcode(), C->isOpaque());
      case ISD::ANY_EXTEND:
        // Some targets prefer to sign extend some types.
...
static SDValue getMemcpyLoadsAndStores(SelectionDAG &DAG, const SDLoc &dl,
  ...
  for (unsigned i = 0; i != NumMemOps; ++i) {
    ...
    if (!Store.getNode()) {
      ...
      // if the type is smaller than a legal type, so the right thing to do is
      // generate a LoadExt/StoreTrunc pair.  These simplify to Load/Store if
      // NVT==VT.
