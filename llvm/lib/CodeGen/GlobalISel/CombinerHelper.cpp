...
MachineInstr *CombinerHelper::buildUDivUsingMul(MachineInstr &MI) {
  ...
  auto BuildUDIVPattern = [&](const Constant *C) {
    ...
    if (!Divisor.isOne()) {
      ...
      if (EltBits == 32 && Ty.isScalar() && Has64BitUMulH && magics.IsAdd) {
        ...
        PreShifts.push_back(MIB.buildConstant(ScalarShiftAmtTy, 0).getReg(0));
        NPQFactors.push_back(
            MIB.buildConstant(ScalarTy, APInt::getZero(EltBits)).getReg(0));
        PostShifts.push_back(MIB.buildConstant(ScalarShiftAmtTy, 0).getReg(0));
        UseNPQ = false;
        return true;
      }
  ...
  if (Use33BitOptimization) {
    ...
    auto IsOne =
        MIB.buildICmp(CmpInst::Predicate::ICMP_EQ, LLT::scalar(1), RHS, One);
