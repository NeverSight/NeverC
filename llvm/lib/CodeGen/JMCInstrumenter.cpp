...
std::string getFlagName(DISubprogram &SP) {
...
bool runImpl(Module &M) {
  ...
  for (auto &F : M) {
    ...
    if (!Flag) {
      std::string FlagName = getFlagName(*SP);
    ...
    if (!CheckFunction) {
      Function *DefaultCheckFunc = createDefaultCheckFunction(M);
      if (IsELF) {
