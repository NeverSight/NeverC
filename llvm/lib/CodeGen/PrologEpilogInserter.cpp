  ...
  for (unsigned i = 0; CSRegs[i]; ++i) {
    ...
    if (SavedRegs.test(Reg)) {
      ...
      for (const MCPhysReg &SuperReg : RegInfo->superregs(Reg)) {
        // Some backends set all aliases for some registers as saved,
        // so they appear in SavedRegs but not CSRegs.
...
void PEI::spillCalleeSavedRegs(MachineFunction &MF) {
  ...
  // targets use virtual registers past this point, and the pass
