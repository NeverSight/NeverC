...
void GenericScheduler::initPolicy(MachineBasicBlock::iterator Begin,
  ...
  // At O2 and below, prefer bottom-up only scheduling over bidirectional.
  // Bidirectional scheduling is up to 13x slower with identical code quality
  // on typical workloads. Reserve it for O3 where maximum code quality matters.
  if (MF.getTarget().getOptLevel() < CodeGenOptLevel::Aggressive &&
      !RegionPolicy.OnlyTopDown && !RegionPolicy.OnlyBottomUp) {
    RegionPolicy.OnlyBottomUp = true;
  }
