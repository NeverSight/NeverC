...
Archive::Archive(MemoryBufferRef Source, Error &Err)
  ...
  // 64-bit ELF archives use a special symbol table format marked by
  // `ar_name` field equals to "/SYM64/".
  ...
  if (Name == "/<ECSYMBOLS>/") {
    // EC symbol map: an additional special member with an EC symbol map
    // after the string table. Format is similar to a regular symbol map,
    // except it doesn't contain member offsets. Its indexes refer to
    // member offsets from the regular symbol table instead.
    Expected<StringRef> BufOrErr = C->getBuffer();
    if (!BufOrErr) {
      Err = BufOrErr.takeError();
      return;
...
object::Archive::Kind Archive::getDefaultKindForHost() {
  ...
  return HostTriple.isOSDarwin() ? object::Archive::K_DARWIN
