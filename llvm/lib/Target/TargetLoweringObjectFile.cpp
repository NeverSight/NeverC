...
void TargetLoweringObjectFile::Initialize(MCContext &ctx,
  ...
  delete Mang;
  Mang = new Mangler();
  initMCObjectFileInfo(ctx, TM.getCodeModel() == CodeModel::Large);
...
TargetLoweringObjectFile::getKindForGlobal(const GlobalObject *GO,
  ...
  if (GVar->isConstant()) {
    // Handle global constants in windows driver
    if (GO->getParent() && GO->getParent()->getModuleFlag("ms-kernel"))
      return SectionKind::getText();
