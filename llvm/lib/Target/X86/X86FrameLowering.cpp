...
X86FrameLowering::X86FrameLowering(const X86Subtarget &STI,
    ...
    : TargetFrameLowering(StackGrowsDown, StackAlignOverride.valueOrOne(), -8),
...
static unsigned getSUBriOpcode() { return X86::SUB64ri32; }
static unsigned getADDriOpcode() { return X86::ADD64ri32; }
static unsigned getSUBrrOpcode() { return X86::SUB64rr; }
static unsigned getADDrrOpcode() { return X86::ADD64rr; }
static unsigned getANDriOpcode() { return X86::AND64ri32; }
static unsigned getLEArOpcode() { return X86::LEA64r; }

static unsigned getMOVriOpcode(int64_t Imm) {
  if (isUInt<32>(Imm))
...
static unsigned getPUSHOpcode(const X86Subtarget &ST) {
  return ST.hasPPX() ? X86::PUSHP64r : X86::PUSH64r;
}
static unsigned getPOPOpcode(const X86Subtarget &ST) {
  return ST.hasPPX() ? X86::POPP64r : X86::POP64r;
}
...
void X86FrameLowering::emitSPUpdate(MachineBasicBlock &MBB,
  ...
  if (EmitInlineStackProbe && !InEpilogue) {
    ...
    unsigned AddSubRROpc = isSub ? getSUBrrOpcode() : getADDrrOpcode();
    if (Reg) {
      BuildMI(MBB, MBBI, DL, TII.get(getMOVriOpcode(Offset)), Reg)
          .addImm(Offset)
          .setMIFlag(Flag);
      ...
      if (isSub)
        ...
        Offset = Offset + SlotSize;
      BuildMI(MBB, MBBI, DL, TII.get(getMOVriOpcode(Offset)), Rax)
          .addImm(Offset)
          .setMIFlag(Flag);
  ...
  while (Offset) {
    ...
    if (ThisVal == SlotSize) {
      ...
      unsigned Reg =
          isSub ? (unsigned)X86::RAX : TRI->findDeadCallerSavedReg(MBB, MBBI);
      if (Reg) {
        unsigned Opc = isSub ? X86::PUSH64r : X86::POP64r;
...
MachineInstrBuilder X86FrameLowering::BuildStackAdjustment(
  ...
  if (UseLEA) {
    MI =
        addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(getLEArOpcode()), StackPtr),
                     StackPtr, false, Offset);
  } else {
    bool IsSub = Offset < 0;
    uint64_t AbsOffset = IsSub ? -Offset : Offset;
    const unsigned Opc = IsSub ? getSUBriOpcode() : getADDriOpcode();
...
int X86FrameLowering::mergeSPUpdates(MachineBasicBlock &MBB,
  ...
  */
  ...
  if (Opc == X86::ADD64ri32 && PI->getOperand(0).getReg() == StackPtr) {
    assert(PI->getOperand(1).getReg() == StackPtr);
    Offset = PI->getOperand(2).getImm();
  } else if (Opc == X86::LEA64_32r && PI->getOperand(0).getReg() == StackPtr &&
  ...
  } else if (Opc == X86::SUB64ri32 && PI->getOperand(0).getReg() == StackPtr) {
    assert(PI->getOperand(1).getReg() == StackPtr);
    Offset = -PI->getOperand(2).getImm();
  } else
    return 0;
...
void X86FrameLowering::emitCalleeSavedFrameMovesFullCFA(
  ...
  unsigned DwarfReg = MRI->getDwarfRegNum(FramePtr, true);
  ...
  for (const CalleeSavedInfo &I : CSI) {
    ...
    if (IsPrologue) {
      if (X86FI->getStackPtrSaveMI()) {
        // +2*SlotSize because there is return address and RBP at the bottom
        // of the stack.
        // | retaddr |
        // | RBP     |
        // |         |<--RBP
        ...
        unsigned DwarfFramePtr = MRI->getDwarfRegNum(FramePtr, true);
...
bool X86FrameLowering::stackProbeFunctionModifiesSP() const { return false; }
...
void X86FrameLowering::emitStackProbeInlineGenericBlock(
  ...
  const X86Subtarget &STI = MF.getSubtarget<X86Subtarget>();
  const X86TargetLowering &TLI = *STI.getTargetLowering();
  const unsigned MovMIOpc = X86::MOV64mi32;
  const uint64_t StackProbeSize = TLI.getStackProbeSize(MF);
...
void X86FrameLowering::emitStackProbeInlineGenericLoop(
  ...
  Register FinalStackProbed = X86::R11;
  ...
  // save loop bound
  {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(getSUBriOpcode()), FinalStackProbed)
  ...
  BuildMI(testMBB, DL, TII.get(X86::CMP64rr))
      .addReg(StackPtr)
      .addReg(FinalStackProbed)
      .setMIFlag(MachineInstr::FrameSetup);
...
void X86FrameLowering::emitStackProbeCall(
  ...
  if (IsLargeCodeModel && STI.useIndirectThunkCalls())
  ...
  unsigned CallOp = IsLargeCodeModel ? X86::CALL64r : X86::CALL64pcrel32;
  ...
  // themselves; non-Windows stack probes also leave %rsp alone. Either way the
  // caller subtracts %rax from %rsp.
  MachineInstr *ModInst = BuildMI(MBB, MBBI, DL, TII.get(getSUBrrOpcode()), SP)
                              .addReg(SP)
                              .addReg(AX);

  if (InstrNum)
    MF.makeDebugValueSubstitution(*InstrNum, {ModInst->getDebugInstrNum(), 0});
  ...
  if (Reg == StackPtr && EmitInlineStackProbe && MaxAlign >= StackProbeSize) {
    {
      ...
      // Setup entry block
      {
        ...
        BuildMI(entryMBB, DL, TII.get(X86::CMP64rr))
            .addReg(FinalStackProbed)
            .addReg(StackPtr)
            .setMIFlag(MachineInstr::FrameSetup);
      ...
      {
        const unsigned SUBOpc = getSUBriOpcode();
        BuildMI(headMBB, DL, TII.get(SUBOpc), StackPtr)
            .addReg(StackPtr)
            .addImm(StackProbeSize)
            .setMIFlag(MachineInstr::FrameSetup);

        BuildMI(headMBB, DL, TII.get(X86::CMP64rr))
            .addReg(StackPtr)
            .addReg(FinalStackProbed)
            .setMIFlag(MachineInstr::FrameSetup);
      ...
      // setup loop body
      {
        ...
        // cmp with stack pointer bound
        BuildMI(bodyMBB, DL, TII.get(X86::CMP64rr))
            .addReg(FinalStackProbed)
            .addReg(StackPtr)
            .setMIFlag(MachineInstr::FrameSetup);
...
bool X86FrameLowering::has128ByteRedZone(const MachineFunction &MF) const {
  ...
  return !IsWin64CC && !Fn.hasFnAttribute(Attribute::NoRedZone);
}
...
void X86FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (auto *MI = X86FI->getStackPtrSaveMI()) {
    ...
    // andq    $-128, %rsp
    // pushq   -8(%basereg)
    BuildMI(MBB, MBBI, DL, TII.get(X86::LEA64r), ArgBaseReg)
        .addUse(StackPtr)
        .addImm(1)
  ...
  // stack alignment.
  if (Fn.getCallingConv() == CallingConv::X86_INTR && Fn.arg_size() == 2) {
    StackSize += 8;
  ...
  if (IsWin64Prologue && IsFunclet) {
    ...
    // MOV64mr %rdx, 16(%rsp)
    addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(X86::MOV64mr)), StackPtr, true,
                 16)
        .addReg(Establisher)
        .setMIFlag(MachineInstr::FrameSetup);
  ...
  if (HasFP) {
    assert(MF.getRegInfo().isReserved(FramePtr) && "FP reserved");
    ...
    // Save RBP into the appropriate stack slot.
    BuildMI(MBB, MBBI, DL,
    ...
    if (NeedsDwarfCFI && !ArgBaseReg.isValid()) {
      // Mark the place where RBP was saved.
    ...
    if (!IsFunclet) {
      if (!IsWin64Prologue && !IsFunclet) {
        // Update RBP with the new base value.
        ...
        if (NeedsDwarfCFI) {
          if (ArgBaseReg.isValid()) {
            ...
            CfaExpr.push_back(dwarf::DW_CFA_expression);
            uint8_t buffer[16];
            unsigned DwarfReg = TRI->getDwarfRegNum(FramePtr, true);
            CfaExpr.append(buffer, buffer + encodeULEB128(DwarfReg, buffer));
            CfaExpr.push_back(2);
            ...
            // Define the current CFA to use the RBP register.
            unsigned DwarfFramePtr = TRI->getDwarfRegNum(FramePtr, true);
            BuildCFI(
                MBB, MBBI, DL,
  ...
  // Update the offset adjustment, which is mainly used to translate
  // from RSP to VFRAME relative local variable offsets.
  if (!IsFunclet) {
  ...
  auto IsCSPush = [&](const MachineBasicBlock::iterator &MBBI) {
    ...
    return Opc == X86::PUSH64r || Opc == X86::PUSHP64r || Opc == X86::PUSH2 ||
           Opc == X86::PUSH2P;
  };
  ...
  // If there is a SUB of RSP immediately before this instruction, merge
  // the two. This can be the case when tail call elimination is enabled and
  // the callee has more arguments then the caller.
  NumBytes -= mergeSPUpdates(MBB, MBBI, true);

  // Adjust stack pointer: RSP -= numbytes.

  // Windows and cygwin/mingw require a prologue helper routine when allocating
  // more than 4K bytes on the stack.  Windows uses __chkstk and cygwin/mingw
  // uses __alloca.  __chkstk is only responsible for probing the stack.
  // The prologue is responsible for adjusting the stack pointer.  Touching the
  // stack at 4K
  ...
  if (AlignedNumBytes >= StackProbeSize && EmitStackProbeCall) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(getMOVriOpcode(Alloc)), X86::RAX)
        .addImm(Alloc)
        .setMIFlag(MachineInstr::FrameSetup);
  ...
  if (TRI->hasBasePointer(MF)) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(X86::MOV64rr), BasePtr)
        .addReg(SPOrEstablisher)
        .setMIFlag(MachineInstr::FrameSetup);
    if (X86FI->getRestoreBasePointer()) {
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(X86::MOV64mr)), FramePtr,
    ...
    if (X86FI->getHasSEHFramePtrSave() && !IsFunclet) {
      ...
      // SEH. SEH does the inverse of the above: it recovers the frame pointer
      // from the base pointer rather than the other way around.
      Register UsedReg;
      int Offset =
          getFrameIndexReference(MF, X86FI->getSEHFramePtrSaveIndex(), UsedReg)
              .getFixed();
      assert(UsedReg == BasePtr);
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(X86::MOV64mr)), UsedReg, true,
                   Offset)
          .addReg(FramePtr)
          .setMIFlag(MachineInstr::FrameSetup);
    }
  ...
  if (ArgBaseReg.isValid()) {
    ...
    int FI = MI->getOperand(1).getIndex();
    unsigned MOVmr = X86::MOV64mr;
    // movq    %basereg, offset(%rbp)
...
static bool isTailCallOpcode(unsigned Opc) {
  return Opc == X86::TCRETURNri64 || Opc == X86::TCRETURNdi64 ||
...
void X86FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (auto *MI = X86FI->getStackPtrSaveMI()) {
    ArgBaseReg = MI->getOperand(0).getReg();
    unsigned Opc = X86::LEA64r;
    Register StackReg = X86::RSP;
    // leaq    -8(%basereg), %rsp
    // .cfi_def_cfa %rsp, 8
  ...
  if (HasFP) {
    // Pop RBP.
    BuildMI(MBB, MBBI, DL,
            TII.get(getPOPOpcode(MF.getSubtarget<X86Subtarget>())), FramePtr)
        .setMIFlag(MachineInstr::FrameDestroy);

    if (NeedsDwarfCFI) {
      if (!ArgBaseReg.isValid()) {
        unsigned DwarfStackPtr = TRI->getDwarfRegNum(X86::RSP, true);
        BuildCFI(MBB, MBBI, DL,
                 MCCFIInstruction::cfiDefCfa(nullptr, DwarfStackPtr, SlotSize),
                 MachineInstr::FrameDestroy);
      }
  ...
  while (MBBI != MBB.begin()) {
    ...
    if (Opc != X86::DBG_VALUE && !PI->isTerminator()) {
      if (!PI->getFlag(MachineInstr::FrameDestroy) ||
          (Opc != X86::POP64r && Opc != X86::BTR64ri8 &&
  ...
  if (ArgBaseReg.isValid()) {
    ...
    auto *MI = X86FI->getStackPtrSaveMI();
    int FI = MI->getOperand(1).getIndex();
    unsigned MOVrm = X86::MOV64rm;
    // movq   offset(%rbp), %basereg
  ...
  if (MBBI != MBB.end())
    DL = MBBI->getDebugLoc();
  // If there is an ADD or SUB of RSP immediately before this
  // instruction, merge the two instructions.
  if (NumBytes || MFI.hasVarSizedObjects())
    NumBytes += mergeSPUpdates(MBB, MBBI, true);

  // If dynamic alloca is used, then reset RSP to point to the last callee-saved
  ...
  if (((TRI->hasStackRealignment(MF)) || MFI.hasVarSizedObjects()) &&
    ...
    if (LEAAmount != 0) {
      ...
      unsigned Opc = X86::MOV64rr;
      BuildMI(MBB, MBBI, DL, TII.get(Opc), StackPtr).addReg(FramePtr);
      --MBBI;
    }
  } else if (NumBytes) {
    // Adjust stack pointer back: RSP += numbytes.
  ...
  if (!HasFP && NeedsDwarfCFI) {
    ...
    while (MBBI != MBB.end()) {
      ...
      unsigned Opc = PI->getOpcode();
      ++MBBI;
      if (Opc == X86::POP64r || Opc == X86::POPP64r || Opc == X86::POP2 ||
          Opc == X86::POP2P) {
        Offset += SlotSize;
...
bool X86FrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (this->TRI->hasBasePointer(MF)) {
    // Allocate a spill slot for RBP if we have a base pointer and EH funclets.
    if (MF.hasEHFunclets()) {
...
/// segmented stack prologue. Depending on platform and the properties of the
/// function either one or two registers will be needed. Set primary to true for
/// the first register, false for the second.
static unsigned GetScratchRegister(bool Primary) {
...
void X86FrameLowering::adjustForSegmentedStacks(
  ...
  unsigned ScratchReg = GetScratchRegister(true);
  ...
  const unsigned RegAX = X86::RAX;
  const unsigned Reg10 = X86::R10;
  const unsigned Reg11 = X86::R11;
  const unsigned MOVrr = X86::MOV64rr;

  if (IsNested)
    BuildMI(allocMBB, DL, TII.get(MOVrr), RegAX).addReg(Reg10);

  BuildMI(allocMBB, DL, TII.get(getMOVriOpcode(StackSize)), Reg10)
      .addImm(StackSize);
  BuildMI(allocMBB, DL, TII.get(getMOVriOpcode(X86FI->getArgumentStackSize())),
          Reg11)
...
bool X86FrameLowering::adjustStackWithPops(MachineBasicBlock &MBB,
  ...
  auto &RegClass = X86::GR64_NOREX_NOSPRegClass;
  ...
  for (int i = 0; i < NumPops; ++i)
    BuildMI(MBB, MBBI, DL, TII.get(X86::POP64r), Regs[i]);
...
bool X86FrameLowering::enableShrinkWrapping(const MachineFunction &MF) const {
  ...
  return (MF.getFunction().hasFnAttribute(Attribute::NoUnwind) || hasFP(MF) ||
          !CompactUnwind) &&
         // The lowering of segmented stack only supports entry blocks as
         // prologue blocks: PR26107.
         !MF.shouldSplitStack();
}
...
// it (for example : 0(rsp) requires fewer bytes, symbols allocated at the
