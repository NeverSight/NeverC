  ...
  case X86::MOV64rr: {
    ...
    MachineInstr *NewMI =
        BuildMI(MBB, MBBI, MI.getDebugLoc(),
                TII->get(MI.getOpcode() == X86::MOV32rr ? X86::LEA64_32r
                                                        : X86::LEA64r))
            .add(Dest)
            .add(Src)
            .addImm(1)
...
static bool isLEA(unsigned Opcode) {
  return Opcode == X86::LEA64r || Opcode == X86::LEA64_32r;
}
...
static inline bool isInefficientLEAReg(unsigned Reg) {
  return Reg == X86::RBP || Reg == X86::R13;
}
...
bool FixupLEAPass::optTwoAddrLEA(MachineBasicBlock::iterator &I,
  ...
  if (UseLEAForSP && DestReg == X86::RSP)
    return false;
...
void FixupLEAPass::processInstruction(MachineBasicBlock::iterator &I,
  ...
  if (AddrOffset >= 0) {
    ...
    if (p.isReg() && p.getReg() != X86::RSP) {
      seekLEAFixup(p, I, MBB);
    }
    MachineOperand &q = MI.getOperand(AddrOffset + X86::AddrIndexReg);
    if (q.isReg() && q.getReg() != X86::RSP) {
