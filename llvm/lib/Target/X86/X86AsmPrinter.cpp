...
void X86AsmPrinter::PrintSymbolOperand(const MachineOperand &MO,
  ...
  case MachineOperand::MO_GlobalAddress: {
    ...
    MCSymbol *GVSym = getSymbolPreferLocal(*GV);
...
static bool isIndirectBranchOrTailCall(const MachineInstr &MI) {
  ...
  return MI.getDesc().isIndirectBranch() || Opc == X86::TAILJMPr64 ||
         Opc == X86::TAILJMPm64 || Opc == X86::TCRETURNri64 ||
...
static bool printAsmMRegister(const X86AsmPrinter &P, const MachineOperand &MO,
  ...
  case 'V':
    ...
    [[fallthrough]];
  case 'q':
    Reg = getX86SubSuperRegister(Reg, 64);
    break;
  }
...
void X86AsmPrinter::emitStartOfAsmFile(Module &M) {
  ...
  if (TT.isOSBinFormatELF()) {
    ...
    if (FeatureFlagsAnd) {
      ...
      assert(TT.isArch64Bit() && "CFProtection used on invalid architecture!");
      ...
      OutStreamer->emitIntValue(16, 4 /*size*/); // Elf_Prop size
...
void X86AsmPrinter::emitEndOfAsmFile(Module &M) {
  ...
  if (TT.isOSBinFormatMachO()) {
    ...
    if (MMI->usesMSVCFloatingPoint()) {
      ...
      // side-effects:
      //
      // It links in support routines for floating-point in scanf and printf.
      //
