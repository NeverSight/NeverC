...
enum class Style {
  RIPRel, // Used on x86-64.
  None    // Large code model or COFF (PIC via other mechanisms).
};
...
#include "X86GenSubtargetInfo.inc"
  ...
  bool canUseCMPXCHG16B() const { return hasCX16(); }
  ...
  /// supports it.
  bool hasCLFLUSH() const { return true; }

  /// Use mfence if we have SSE2 or we're on x86-64 (even if we asked for
  /// no-sse2). There isn't any reason to disable it if the target processor
  /// supports it.
  bool hasMFence() const { return true; }
  ...
  bool isTargetWin64() const { return isOSWindows(); }
