...
static unsigned getRetOpcode(const X86Subtarget &) { return X86::RET64; }
...
void X86MCInstLower::Lower(const MachineInstr *MI, MCInst &OutMI) const {
  ...
  case X86::CATCHRET: {
    ...
    unsigned ReturnReg = X86::RAX;
    OutMI = MCInst();
    OutMI.setOpcode(getRetOpcode(Subtarget));
    OutMI.addOperand(MCOperand::createReg(ReturnReg));
    break;
  }
  // TAILJMPd/d64, TAILJMPd64_CC - Lower to the correct jump instruction.
  case X86::TAILJMPr64:
  case X86::TAILJMPr64_REX:
  case X86::TAILJMPd:
  case X86::TAILJMPd64:
...
void X86AsmPrinter::LowerTlsAddr(X86MCInstLower &MCInstLowering,
  ...
  if (UseGot) {
    const MCExpr *Expr =
        MCSymbolRefExpr::create(TlsGetAddr, MCSymbolRefExpr::VK_GOTPCREL, Ctx);
    ...
    EmitAndCountInstruction(MCInstBuilder(X86::CALL64pcrel32)
                                .addExpr(MCSymbolRefExpr::create(
                                    TlsGetAddr, MCSymbolRefExpr::VK_PLT, Ctx)));
  }
}
...
void X86AsmPrinter::LowerFENTRY_CALL(const MachineInstr &MI,
  ...
  EmitAndCountInstruction(MCInstBuilder(X86::CALL64pcrel32).addExpr(Op));
}
...
#define CASE_512_MOV_RM() MOV_AVX512_CASE(Z)
...
#define CASE_ALL_MOV_RM()                                                      \
  ...
  case X86::VBROADCASTI64X4rm:
    ...
    if (auto *C = getConstantFromPool(*MI, MI->getOperand(1 + X86::AddrDisp))) {
      ...
      switch (MI->getOpcode()) {
        CASE_128_MOV_RM() NumLanes = 1;
        BitWidth = 128;
        break;
        CASE_256_MOV_RM() NumLanes = 1;
        BitWidth = 256;
        break;
        CASE_512_MOV_RM() NumLanes = 1;
        BitWidth = 512;
        break;
      case X86::VBROADCASTF128rm:
        NumLanes = 2;
