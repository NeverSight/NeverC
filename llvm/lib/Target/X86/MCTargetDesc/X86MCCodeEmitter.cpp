...
/// of the form _GLOBAL_OFFSET_TABLE_-symbol. We check only simple cases that
/// are known to be used: _GLOBAL_OFFSET_TABLE_ by itself or at the start of a
/// binary expression.
...
void X86MCCodeEmitter::emitMemModRMByte(
  ...
  if ( // The SIB byte must be used if there is an index register or the
      ...
      // present.
      BaseRegNo != N86::ESP &&
      // If there is no base register, we need a SIB byte to emit an addr that
      // is just 'disp32' (the non-RIP relative form).
      BaseReg != 0) {
    ...
    if (BaseRegNo != N86::EBP) {
      ...
      if (Disp.isExpr()) {
        ...
        if (Sym && Sym->getKind() == MCSymbolRefExpr::VK_TLSCALL) {
          ...
          // (R_X86_64_TLSCALL) applies to the beginning.
...
PrefixKind X86MCCodeEmitter::emitOpcodePrefix(int MemOperand, const MCInst &MI,
  ...
  if ((TSFlags & X86II::OpSizeMask) == X86II::OpSize16)
    emitByte(0x66, CB);
