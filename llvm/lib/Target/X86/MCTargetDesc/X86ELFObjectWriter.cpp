...
public:
  X86ELFObjectWriter(uint8_t OSABI, uint16_t EMachine);
...
X86ELFObjectWriter::X86ELFObjectWriter(uint8_t OSABI, uint16_t EMachine)
    : MCELFObjectTargetWriter(OSABI, EMachine,
...
unsigned X86ELFObjectWriter::getRelocType(MCContext &Ctx, const MCValue &Target,
  ...
  assert(getEMachine() == ELF::EM_X86_64);
  return getRelocType64(Ctx, Fixup.getLoc(), Modifier, Type, IsPCRel, Kind);
}

std::unique_ptr<MCObjectTargetWriter>
llvm::createX86ELFObjectWriter(uint8_t OSABI, uint16_t EMachine) {
  return std::make_unique<X86ELFObjectWriter>(OSABI, EMachine);
}
