...
static unsigned getRelaxedOpcode(const MCInst &MI) {
  ...
  return isRelaxableBranch(Opcode) ? getRelaxedOpcodeBranch(Opcode)
...
const MCFixupKindInfo &X86AsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  ...
  // Fixup kinds from .reloc directive are like R_X86_64_NONE. They
...
public:
  ...
  createObjectTargetWriter() const override {
    return createX86ELFObjectWriter(OSABI, ELF::EM_X86_64);
  }
};
...
public:
  ...
  std::unique_ptr<MCObjectTargetWriter>
  createObjectTargetWriter() const override {
    return createX86WinCOFFObjectWriter();
  }
};
...
class DarwinX86AsmBackend : public X86AsmBackend {
  ...
  static constexpr unsigned OffsetSize = 8;
  static constexpr unsigned MoveInstrSize = 3;
  static constexpr unsigned StackDivide = 8;
...
private:
  ...
  int getCompactUnwindRegNum(unsigned Reg) const {
    ...
    const MCPhysReg *CURegs = CU64BitRegs;
...
public:
  ...
  createObjectTargetWriter() const override {
    ...
    return createX86MachObjectWriter(CPUType, CPUSubType);
  }
  ...
  uint32_t generateCompactUnwindEncoding(const MCDwarfFrameInfo *FI,
    ...
    // Encode that we are using RBP as the frame pointer.
    uint32_t CompactUnwindEncoding = 0;

    unsigned SubtractInstrIdx = 3;
    ...
    for (const MCCFIInstruction &Inst : Instrs) {
      ...
      case MCCFIInstruction::OpDefCfaRegister: {
        ...
        // If the frame pointer is other than rsp, we do not have a way to
        // generate a compact unwinding representation, so bail out.
        if (*MRI.getLLVMRegNum(Inst.getRegister(), true) != X86::RBP)
          return CU::UNWIND_MODE_DWARF;
    ...
    if (HasFP) {
      ...
      if ((StackSize & 0xFF) == StackSize) {
        ...
        // Encode the offset to the nnnnnn value in the 'subq $nnnnnn, RSP'
        // instruction.
...
MCAsmBackend *llvm::createX86_64AsmBackend(const Target &T,
  ...
  if (TheTriple.isOSWindows() && TheTriple.isOSBinFormatCOFF())
    return new WindowsX86AsmBackend(T, STI);
