...
#include "MCTargetDesc/X86DarwinSdkConflictUndef.h"
...
X86InstrInfo::X86InstrInfo(X86Subtarget &STI)
    : X86GenInstrInfo(X86::ADJCALLSTACKDOWN64, X86::ADJCALLSTACKUP64,
                      X86::CATCHRET, X86::RET64),
...
void X86InstrInfo::reMaterialize(MachineBasicBlock &MBB,
  ...
  if (ClobbersEFLAGS && MBB.computeRegisterLiveness(&TRI, X86::EFLAGS, I) !=
    ...
    // effects.
    int Value;
    assert(Orig.getOpcode() == X86::MOV32r0);
    Value = 0;
...
bool X86InstrInfo::classifyLEAReg(MachineInstr &MI, const MachineOperand &Src,
  ...
  const TargetRegisterClass *RC =
      AllowSP ? &X86::GR64RegClass : &X86::GR64_NOSPRegClass;
  Register SrcReg = Src.getReg();
  isKill = MI.killsRegister(SrcReg);

  // For LEA64r the register already has the right 64-bit type and we may just
  // need to forbid SP.
...
MachineInstr *X86InstrInfo::convertToThreeAddress(MachineInstr &MI,
  ...
  case X86::INC32r: {
    ...
    unsigned Opc = MIOpc == X86::INC64r ? X86::LEA64r : (X86::LEA64_32r);
    bool isKill;
    MachineOperand ImplicitOp = MachineOperand::CreateReg(0, false);
    if (!classifyLEAReg(MI, Src, Opc, /*AllowSP=*/false, SrcReg, isKill,
                        ImplicitOp, LV, LIS))
  ...
  case X86::DEC32r: {
    ...
    unsigned Opc = MIOpc == X86::DEC64r ? X86::LEA64r : (X86::LEA64_32r);
...
bool X86InstrInfo::canMakeTailCallConditional(
  ...
  if (TailCall.getOpcode() != X86::TCRETURNdi64) {
...
void X86InstrInfo::replaceBranchWithTailCall(
  ...
  unsigned Opc = X86::TCRETURNdi64cc;
...
bool X86InstrInfo::analyzeBranchPredicate(MachineBasicBlock &MBB,
  ...
  //   je %label
  //
  const unsigned TestOpcode = X86::TEST64rr;
...
inline static bool isDefConvertible(const MachineInstr &MI, bool &NoSignFlag,
  ...
  // "ELF Handling for Thread-Local Storage" specifies that x86-64 GOTTPOFF
  // relocations can convert an ADD to a LEA during Initial Exec to Local Exec
  // relaxation. In these cases, we must not depend on the EFLAGS modification
  // of ADD actually happening in the final binary.
  if (MI.getOpcode() == X86::ADD64rm || MI.getOpcode() == X86::ADD32rm) {
    unsigned Flags = MI.getOperand(5).getTargetFlags();
    if (Flags == X86II::MO_GOTTPOFF)
      return false;
  }
...
static bool ExpandMOVImmSExti8(MachineInstrBuilder &MIB,
  ...
  if (X86FI->getUsesRedZone()) {
    MIB->setDesc(TII.get(
        MIB->getOpcode() == X86::MOV32ImmSExti8 ? X86::MOV32ri : X86::MOV64ri));
    return true;
  }
...
bool X86InstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  case X86::RDFLAGS64: {
    ...
    MachineInstr *NewMI =
        BuildMI(MBB, MI, MIB->getDebugLoc(), get(X86::PUSHF64)).getInstr();
  ...
  case X86::WRFLAGS64: {
    ...
    BuildMI(MBB, MI, MIB->getDebugLoc(), get(X86::PUSH64r))
        .addReg(MI.getOperand(0).getReg());
    BuildMI(MBB, MI, MIB->getDebugLoc(), get(X86::POPF64));
    MI.eraseFromParent();
    return true;
  }
...
MachineInstr *X86InstrInfo::foldMemoryOperandImpl(
  ...
  if (isSlowTwoMemOps && !MF.getFunction().hasMinSize() &&
      (MI.getOpcode() == X86::CALL64r || MI.getOpcode() == X86::PUSH64r))
    return nullptr;
...
unsigned X86InstrInfo::getGlobalBaseReg(MachineFunction *MF) const {
  ...
  MachineRegisterInfo &RegInfo = MF->getRegInfo();
  GlobalBaseReg = RegInfo.createVirtualRegister(&X86::GR64_NOSPRegClass);
...
struct LDTLSCleanup : public MachineFunctionPass {
  ...
  MachineInstr *SetRegister(MachineInstr &I, unsigned *TLSBaseAddrReg) {
    ...
    MachineRegisterInfo &RegInfo = MF->getRegInfo();
    *TLSBaseAddrReg = RegInfo.createVirtualRegister(&X86::GR64RegClass);
