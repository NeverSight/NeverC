...
static int64_t getDynAllocaAmount(MachineInstr *MI, MachineRegisterInfo *MRI) {
  assert(MI->getOpcode() == X86::DYN_ALLOCA_64);
...
void X86DynAllocaExpander::computeLowerings(MachineFunction &MF,
  ...
  for (MachineBasicBlock *MBB : RPO) {
    ...
    for (MachineInstr &MI : *MBB) {
      if (MI.getOpcode() == X86::DYN_ALLOCA_64) {
        ...
        Offset = 0;
      } else if (MI.getOpcode() == X86::ADJCALLSTACKUP64) {
        Offset -= MI.getOperand(0).getImm();
      } else if (MI.getOpcode() == X86::ADJCALLSTACKDOWN64) {
        Offset += MI.getOperand(0).getImm();
...
void X86DynAllocaExpander::lower(MachineInstr *MI, Lowering L) {
  ...
  case TouchAndSub: {
    ...
    BuildMI(*MBB, I, DL, TII->get(X86::PUSH64r))
  ...
  case Sub:
    ...
    if (Amount == SlotSize) {
      ...
      // Sub.
      BuildMI(*MBB, I, DL, TII->get(X86::SUB64ri32), StackPtr)
          .addReg(StackPtr)
          .addImm(Amount);
    }
    break;
  case Probe:
    if (!NoStackArgProbe) {
      // The probe lowering expects the amount in RAX.
      unsigned RegA = X86::RAX;
      ...
      // Sub
      BuildMI(*MBB, I, DL, TII->get(X86::SUB64rr), StackPtr)
          .addReg(StackPtr)
          .addReg(MI->getOperand(0).getReg());
    }
    break;
