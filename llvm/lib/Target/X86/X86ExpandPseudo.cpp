...
bool X86ExpandPseudo::ExpandMI(MachineBasicBlock &MBB,
  ...
  case X86::TCRETURNmi64: {
    bool isMem = Opcode == X86::TCRETURNmi64;
    ...
    if (Opcode == X86::TCRETURNdi64 || Opcode == X86::TCRETURNdi64cc) {
      unsigned Op;
      if (Opcode == X86::TCRETURNdi64cc) {
      ...
      if (Op == X86::TAILJMPd64_CC) {
        MIB.addImm(MBBI->getOperand(2).getImm());
      }

    } else if (Opcode == X86::TCRETURNmi64) {
      unsigned Op = IsWin64 ? X86::TAILJMPm64_REX : X86::TAILJMPm64;
      MachineInstrBuilder MIB = BuildMI(MBB, MBBI, DL, TII->get(Op));
      for (unsigned i = 0; i != X86::AddrNumOperands; ++i)
        MIB.add(MBBI->getOperand(i));
    } else {
      assert(Opcode == X86::TCRETURNri64);
  ...
  case X86::RET: {
    ...
    if (StackAdj == 0) {
      ...
      assert(isUInt<16>(StackAdj));
      MIB = BuildMI(MBB, MBBI, DL, TII->get(X86::RETI64)).addImm(StackAdj);
    }
