...
private:
  ...
  SDValue getSBBZero(SDNode *N) {
    ...
    SDValue Zero =
        SDValue(CurDAG->getMachineNode(X86::MOV32r0, dl, VTs, std::nullopt), 0);
    if (VT == MVT::i64) {
      Zero =
...
bool X86DAGToDAGISel::foldOffsetIntoAddress(uint64_t Offset,
  ...
  if (Val != 0 &&
      !X86::isOffsetSuitableForCodeModel(Val, M, AM.hasSymbolicDisplacement()))
    return true;
  if (AM.BaseType == X86ISelAddressMode::FrameIndexBase &&
...
bool X86DAGToDAGISel::matchLoadInAddress(LoadSDNode *N,
                                         X86ISelAddressMode &AM) {
  ...
  //
  // The GNU TLS model defines that fs:0 contains its own address.
...
bool X86DAGToDAGISel::matchWrapper(SDValue N, X86ISelAddressMode &AM) {
  ...
  CodeModel::Model M = TM.getCodeModel();
  if (M == CodeModel::Large && !IsRIPRelTLS)
    return true;
  ...
  if (!IsRIPRel && AM.GV && TM.isLargeGlobalValue(AM.GV)) {
    AM = Backup;
    return true;
  }
...
bool X86DAGToDAGISel::matchAddress(SDValue N, X86ISelAddressMode &AM) {
  ...
  if (TM.getCodeModel() != CodeModel::Large &&
      (!AM.GV || !TM.isLargeGlobalValue(AM.GV)) && AM.Scale == 1 &&
...
bool X86DAGToDAGISel::matchAddressRecursively(SDValue N, X86ISelAddressMode &AM,
  ...
  case ISD::FrameIndex:
    if (AM.BaseType == X86ISelAddressMode::RegBase &&
        AM.Base_Reg.getNode() == nullptr && isDispSafeForFrameIndex(AM.Disp)) {
...
bool X86DAGToDAGISel::selectLEA64_32Addr(SDValue N, SDValue &Base,
  ...
  if (RN && RN->getReg() == 0)
    ...
    // Base could already be %rip or a 64-bit register.
    SDValue ImplDef =
    ...
    case Intrinsic::x86_clzero: {
      ...
      case Intrinsic::x86_sse3_monitor:
        if (!Subtarget->hasSSE3())
          break;
        Opc = X86::MONITOR64rrr;
        break;
      case Intrinsic::x86_monitorx:
        if (!Subtarget->hasMWAITX())
          break;
        Opc = X86::MONITORX64rrr;
        break;
      case Intrinsic::x86_clzero:
