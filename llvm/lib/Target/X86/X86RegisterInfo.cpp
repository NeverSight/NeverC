...
X86RegisterInfo::X86RegisterInfo(const Triple &TT)
    : X86GenRegisterInfo(X86::RIP, X86_MC::getDwarfRegFlavour(TT, false),
                         X86_MC::getDwarfRegFlavour(TT, true), X86::RIP) {
  X86_MC::initLLVMToSEHAndCVRegMapping(this);

  IsWin64 = TT.isOSWindows();
...
BitVector X86RegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  if (!MF.getSubtarget<X86Subtarget>().hasAVX512()) {
...
// of the form 'lea (%rsp), %rbx' --> 'mov %rsp, %rbx'.
...
static bool tryOptimizeLEAtoMOV(MachineBasicBlock::iterator II) {
  ...
  // Check if this is a LEA of the form 'lea (%rsp), %rbx'
  if ((Opc != X86::LEA64r && Opc != X86::LEA64_32r) ||
  ...
  // For LEA64_32r, ensure the base-pointer is a 32-bit operand so the LEA
  // will be replaced with a 32-bit MOV that zero-extends.
...
bool X86RegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  } else if (MBB.isEHFuncletEntry() || IsEHFuncletEpilogue) {
  ...
  // simple FP case, and doesn't work with stack realignment. The offset is from
  // the SP at the end of the prologue, not the FP location. This matches the
  // behavior of llvm.frameaddress.
  unsigned Opc = MI.getOpcode();
  if (Opc == TargetOpcode::LOCAL_ESCAPE) {
    MachineOperand &FI = MI.getOperand(FIOperandNum);
    FI.ChangeToImmediate(FIOffset);
  ...
  // For LEA64_32r we can use full-size 64-bit register as source operand;
  // semantics are the same and it saves one byte (avoids 0x67 prefix).
