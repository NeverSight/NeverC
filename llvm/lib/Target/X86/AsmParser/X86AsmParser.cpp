...
private:
  ...
  bool is64BitMode() const { return getSTI().hasFeature(X86::Is64Bit); }
  unsigned getPointerWidth() { return 64; }
...
bool X86AsmParser::MatchRegisterByName(MCRegister &RegNo, StringRef RegName,
  ...
  assert(is64BitMode() && "expected 64-bit mode");
...
bool X86AsmParser::parseIntelOperand(OperandVector &Operands, StringRef Name) {
  ...
  if (isParsingMSInlineAsm())
    return CreateMemForMSInlineAsm(
        RegNo, Disp, BaseReg, IndexReg, Scale, IsUnconditionalBranch, Start,
        End, Size, SM.getSymName(), SM.getIdentifierInfo(), Operands);
...
bool X86AsmParser::ParseMemOperand(unsigned SegReg, const MCExpr *Disp,
  ...
  // gas requires isInt<32> and otherwise reports an error. For 32-bit
  // addressing gas reports a warning and allows a wider range.
  if (BaseReg || IndexReg) {
...
bool X86AsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name,
  ...
  while (true) {
    if (Name == "{") {
      ...
      auto Prefix = Parser.getTok().getString().lower();
  ...
  if ((Name == "mov" || Name == "movw" || Name == "movl") &&
    ...
    if (Op1.isReg() && Op2.isReg() &&
      ...
      if (Name != "mov" && Name[3] == 'w') {
        Name = "movl";
        Operands[0] = X86Operand::CreateToken(Name, NameLoc);
      }
      MCRegister Reg = getX86SubSuperRegister(Op1.getReg(), 32);
...
bool X86AsmParser::ParseDirectiveCode(StringRef IDVal, SMLoc L) {
  MCAsmParser &Parser = getParser();
  if (IDVal == ".code16" || IDVal == ".code16gcc" || IDVal == ".code32") {
    Parser.Lex();
    return Error(L, IDVal + " is not supported in 64-bit mode");
  }
  if (IDVal == ".code64") {
    Parser.Lex();
    return false;
