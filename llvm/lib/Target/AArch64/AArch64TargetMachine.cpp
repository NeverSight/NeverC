...
AArch64TargetMachine::AArch64TargetMachine(const Target &T, const Triple &TT,
                                           ...
                                           std::optional<CodeModel::Model> CM,
                                           CodeGenOptLevel OL)
    : LLVMTargetMachine(T, computeDataLayout(TT, Options.MCOptions), TT,
                        computeDefaultCPU(TT, CPU), FS, Options,
                        getEffectiveAArch64CodeModel(TT, CM), OL),
...
AArch64TargetMachine::getSubtargetImpl(const Function &F) const {
  ...
  if (!I) {
    ...
    I = std::make_unique<AArch64Subtarget>(
        TargetTriple, CPU, TuneCPU, FS, *this, MinSVEVectorSize,
        MaxSVEVectorSize, StreamingSVEMode, StreamingCompatibleSVEMode,
        HasMinSize);
  }

  assert((!StreamingSVEMode || I->hasSME()) && "Expected SME to be available");
...
AArch64leTargetMachine::AArch64leTargetMachine(
    ...
    const TargetOptions &Options, std::optional<CodeModel::Model> CM,
    CodeGenOptLevel OL)
    : AArch64TargetMachine(T, TT, CPU, FS, Options, CM, OL) {}
...
public:
  ...
  createMachineScheduler(MachineSchedContext *C) const override {
    ...
    bool IsO3 = C->MF->getTarget().getOptLevel() == CodeGenOptLevel::Aggressive;
    if (IsO3) {
...
void AArch64PassConfig::addIRPasses() {
  ...
  if (TM->getOptLevel() != CodeGenOptLevel::None && EnableAtomicTidy) {
    ...
    addPass(createCFGSimplificationPass(Opts));
  }
  ...
  // Match complex arithmetic patterns (complex multiply/divide).
  // Defer to O3: typical C code has no complex arithmetic, and the pass
  // requests expensive analyses (ScalarEvolution, TargetLibraryInfo).
  if (TM->getOptLevel() == CodeGenOptLevel::Aggressive)
