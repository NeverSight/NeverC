...
/// The homogeneous prolog/epilog size-optimization (driven by
/// `-homogeneous-prolog-epilog`) has been removed; always report that no
/// homogeneous frame helpers are injected. `producePairRegisters` still
/// pairs CSRs through the compact-unwind path when appropriate.
...
static void computeCalleeSaveRegisterPairs(
  ...
  assert((!produceCompactUnwindFrame(MF) || CC == CallingConv::PreserveMost ||
          CC == CallingConv::PreserveAll || CC == CallingConv::Win64 ||
          (Count & 1) == 0) &&
...
void TagStoreEdit::emitCode(MachineBasicBlock::iterator &InsertI,
  ...
  LLVM_DEBUG(
      ...
      for (const auto &Instr : TagStores) { dbgs() << "  " << *Instr.MI; });
