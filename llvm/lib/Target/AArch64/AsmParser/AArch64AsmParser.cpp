...
ParseStatus AArch64AsmParser::tryParseScalarRegister(MCRegister &RegNum) {
  ...
  auto lowerCase = Tok.getString().lower();
...
AArch64AsmParser::tryParseOptionalShiftExtend(OperandVector &Operands) {
  const AsmToken &Tok = getTok();
  auto LowerID = Tok.getString().lower();
...
bool AArch64AsmParser::parseSymbolicImmVal(const MCExpr *&ImmVal) {
  ...
  if (parseOptionalToken(AsmToken::Colon)) {
    ...
    auto LowerCase = getTok().getIdentifier().lower();
...
bool AArch64AsmParser::parseDirectiveCPU(SMLoc L) {
  ...
  const std::optional<llvm::AArch64::ArchInfo> CpuArch =
      llvm::AArch64::getArchForCpu(CPU);
  if (!CpuArch) {
