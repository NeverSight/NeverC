...
AArch64LegalizerInfo::AArch64LegalizerInfo(const AArch64Subtarget &ST)
  ...
  if (HasCSSC)
    MinMaxActions
        ...
        // Making clamping conditional on CSSC extension as without legal types
        // we lower to CMP which can fold one of the two sxtb's we'd otherwise
        // need if we detect a type smaller than 32-bit.
        .minScalar(0, s32);
  else
    MinMaxActions.legalFor({v8s8, v16s8, v4s16, v8s16, v2s32, v4s32});
  MinMaxActions.clampNumElements(0, v8s8, v16s8)
  ...
  if (HasCSSC)
    ABSActions.legalFor({s32, s64});
  ABSActions.legalFor(PackedVectorAllTypeList).lowerIf(isScalar(0));
  ...
  if (HasCSSC)
    CTPOPActions
        .legalFor({{s32, s32}, {s64, s64}, {v8s8, v8s8}, {v16s8, v16s8}})
        .customFor({{s128, s128},
                    {v2s64, v2s64},
                    {v2s32, v2s32},
                    {v4s32, v4s32},
    ...
    CTPOPActions.legalFor({{v8s8, v8s8}, {v16s8, v16s8}})
        .customFor({{s32, s32},
                    {s64, s64},
                    {s128, s128},
                    {v2s64, v2s64},
                    ...
                    {v4s16, v4s16},
                    {v8s16, v8s16}});
  CTPOPActions.clampScalar(0, s32, s128)
      .widenScalarToNextPow2(0)
...
bool AArch64LegalizerInfo::legalizeIntrinsic(LegalizerHelper &Helper,
  ...
  case Intrinsic::vacopy: {
    const unsigned PtrSize = 8;
    unsigned VaListSize =
        (ST->isTargetDarwin() || ST->isTargetWindows()) ? 8 : 32;
