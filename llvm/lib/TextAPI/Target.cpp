...
Target::operator std::string() const {
  auto Version =
      MinDeployment.empty() ? SmallString<64>() : MinDeployment.getAsString();
...
std::string getTargetTripleName(const Target &Targ) {
  auto Version = Targ.MinDeployment.empty() ? SmallString<64>()
                                            : Targ.MinDeployment.getAsString();

  return (getArchitectureName(Targ.Arch) + "-apple-" +
          getOSAndEnvironmentName(Targ.Platform, Version.str().str()))
      .str();
}
