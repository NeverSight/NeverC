...
Expected<StubT> getRequiredIntValue(
    TBDKey Key, const Object *Obj,
    std::function<std::optional<StubT>(int64_t)> Validate = nullptr) {
  int64_t Val;
  if (!Obj->getInteger(Keys[Key], Val))
...
template <typename StubT>
Expected<StubT>
getRequiredIntValue(TBDKey Key, const Object *Obj, StubT DefaultValue,
                    std::function<std::optional<StubT>(int64_t)> Validate) {
  int64_t Val;
  if (!Obj->getInteger(Keys[Key], Val))
    return DefaultValue;
...
inline Expected<StringRef> getRequiredStrValue(TBDKey Key, const Object *Obj) {
  StringRef Val = Obj->getString(Keys[Key]);
  if (!Val.data())
    return make_error<JSONStubError>(getParseErrorMsg(Key));
  return Val;
}
...
template <typename StubT>
Expected<StubT>
getRequiredStrValue(TBDKey Key, const Object *Obj, StubT DefaultValue,
                    std::function<std::optional<StubT>(StringRef)> Validate) {
  StringRef Val = Obj->getString(Keys[Key]);
  if (!Val.data())
    return DefaultValue;
...
Expected<FileType> getVersion(const Object *File) {
  auto VersionOrErr = getRequiredIntValue<FileType>(
      TBDKey::TBDVersion, File, [](int64_t Val) -> std::optional<FileType> {
        unsigned Result = Val;
        if (Result != 5)
          return std::nullopt;
...
Expected<TargetList> getTargetsSection(const Object *Section) {
  ...
  for (const Value &JSONTarget : *Targets) {
    ...
    if (!Obj)
      return make_error<JSONStubError>(getParseErrorMsg(TBDKey::Target));
    auto TargetStr = getRequiredStrValue(TBDKey::Target, Obj);
    if (!TargetStr)
      return make_error<JSONStubError>(getParseErrorMsg(TBDKey::Target));
    auto TargetOrErr = Target::create(*TargetStr);
    if (!TargetOrErr)
    ...
    if (VersionStr.data() && Version.tryParse(VersionStr))
...
Expected<StringRef> getNameSection(const Object *File) {
  ...
  return getRequiredStrValue(TBDKey::Name, Obj);
}
...
Expected<AttrToTargets> getUmbrellaSection(const Object *File,
  ...
  for (auto Val : *Umbrella) {
    ...
    auto UmbrellaOrErr = getRequiredStrValue(TBDKey::Umbrella, Obj);
    if (!UmbrellaOrErr)
...
Expected<PackedVersion> getPackedVersion(const Object *File, TBDKey Key) {
  ...
  for (const auto &Val : *Versions) {
    ...
    return getRequiredStrValue<PackedVersion>(
        TBDKey::Version, Obj, PackedVersion(1, 0, 0), ValidatePV);
  }
...
Array serializeSymbols(InterfaceFile::const_filtered_symbol_range Symbols,
  ...
  auto AssignForSymbolType = [](SymbolFields::SymbolTypes &Assignment,
                                const Symbol *Sym) {
    if (Sym->getKind() != SymbolKind::GlobalSymbol)
      return;
