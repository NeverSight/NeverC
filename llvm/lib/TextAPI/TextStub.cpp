...
archs: [ x86_64, arm64 ]  # the list of architecture slices that are
...
archs: [ x86_64, arm64 ]  # the list of architecture slices that are
                                 # supported by this file.
uuids: [ arm64:... ]              # Optional: List of architecture and UUID pairs.
platform: ios                    # Specifies the platform (macosx, ios, etc)
flags: []                        # Optional:
install-name: /u/l/libfoo.dylib  #
current-version: 1.2.3           # Optional: defaults to 1.0
...
targets: [ arm64-ios, x86_64-maccatalyst ]  # The list of applicable tapi supported target triples
...
template <> struct MappingTraits<const InterfaceFile *> {
  ...
  static void mapKeysToValues(FileType FileKind, IO &IO,
    ...
    IO.mapOptional("compatibility-version", Keys->CompatibilityVersion,
                   PackedVersion(1, 0, 0));
    if (FileKind != FileType::TBD_V3 && FileKind != FileType::TBD_V1)
...
TextAPIReader::get(MemoryBufferRef InputBuffer) {
  ...
  // Allow unknown TBD YAML keys we do not model; they were never
  // applied to InterfaceFile.
  ...
  if (YAMLIn.error())
    return make_error<StringError>(
        Ctx.ErrorMessage,
        std::error_code(YAMLIn.error(), std::generic_category()));
