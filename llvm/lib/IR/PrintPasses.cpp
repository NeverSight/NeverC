...
std::string llvm::doSystemDiff(StringRef Before, StringRef After,
  ...
  static ErrorOr<SmallString<256>> DiffExe = sys::findProgramByName(DiffBinary);
  if (!DiffExe)
  ...
  StringRef Redirects[] = {StringRef(), StringRef(FileName[2]), StringRef()};
  int Result = sys::ExecuteAndWait(*DiffExe, Args, {}, Redirects);
  if (Result < 0)
