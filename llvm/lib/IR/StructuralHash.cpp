...
// pass return status consistency with actual change.
...
public:
  ...
  // walks the BBs in depth first order and accumulates the opcode sequence of
  // each instruction within each block. Because this hash currently does not
  // look at the operands, it is
  ...
  // out of it (for example PM expensive checks for pass modification status).
  // When modifying this
  ...
  void update(const Function &F, bool DetailedHash) {
    ...
    // Walk the blocks in depth-first order, accumulating the hash of the
