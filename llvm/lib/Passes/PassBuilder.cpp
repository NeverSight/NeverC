...
static const Regex
    DefaultAliasRegex("^(default|lto-pre-link|lto)<(O[0123sz])>$");
...
static bool startsWithDefaultPipelineAliasPrefix(StringRef Name) {
  return Name.starts_with("default") || Name.starts_with("lto");
}
...
Error PassBuilder::parseModulePass(ModulePassManager &MPM,
  ...
  if (startsWithDefaultPipelineAliasPrefix(Name)) {
    ...
    // inconsistent with NeverC. NeverC doesn't enable loop vectorization
