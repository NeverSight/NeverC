...
void IRChangedTester::handleIR(const std::string &S, StringRef PassID) {
  ...
  static ErrorOr<SmallString<256>> Exe = sys::findProgramByName(TestChanged);
  if (!Exe) {
...
std::string DotCfgChangeReporter::genHTML(StringRef Text, StringRef DotFile,
  ...
  // Create the PDF file.
  static ErrorOr<SmallString<256>> DotExe = sys::findProgramByName(DotBinary);
  if (!DotExe)
