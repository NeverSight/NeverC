...
static bool getWindowsSDKDirViaCommandLine(
  ...
  if (WinSdkDir || WinSysRoot) {
    ...
    if (!SDKVersion.empty()) {
      ...
      Version = SDKVersion.getAsString().str();
...
static bool readFullStringValue(HKEY hkey, const char *valueName,
                                std::string &value) {
  llvm::SmallVector<wchar_t, 256> WideValueName;
  if (!llvm::ConvertUTF8toWide(valueName, WideValueName))
    return false;
  WideValueName.push_back(L'\0');

  DWORD result = 0;
  DWORD valueSize = 0;
  DWORD type = 0;
  result = RegQueryValueExW(hkey, WideValueName.data(), NULL, &type, NULL,
                            &valueSize);
  if (result != ERROR_SUCCESS || type != REG_SZ || !valueSize)
    return false;
  std::vector<BYTE> buffer(valueSize);
  result = RegQueryValueExW(hkey, WideValueName.data(), NULL, NULL, &buffer[0],
                            &valueSize);
  if (result == ERROR_SUCCESS) {
    const wchar_t *WideData = (const wchar_t *)(buffer.data());
    size_t WideLen = valueSize / sizeof(wchar_t);
    if (WideLen > 0 && WideData[WideLen - 1] == L'\0')
      WideLen--;
    value.clear();
    llvm::SmallString<256> Tmp;
    bool ok = llvm::convertWideToUTF8(WideData, WideLen, Tmp);
    if (ok)
      value.assign(Tmp.data(), Tmp.size());
    return ok;
  }
  return false;
}
...
bool findVCToolChainViaEnvironment(vfs::FileSystem &VFS, std::string &Path,
  ...
  if (auto VCToolsInstallDir = sys::Process::GetEnv("VCToolsInstallDir")) {
    // This is only set by newer Visual Studios, and it leads straight to
    // the toolchain directory.
    Path.assign(VCToolsInstallDir->begin(), VCToolsInstallDir->end());
    VSLayout = ToolsetLayout::VS2017OrNewer;
    return true;
  }
  if (auto VCInstallDir = sys::Process::GetEnv("VCINSTALLDIR")) {
    Path.assign(VCInstallDir->begin(), VCInstallDir->end());
  ...
  if (auto PathEnv = sys::Process::GetEnv("PATH")) {
    ...
    for (StringRef PathEntry : PathEntries) {
      ...
      // cl.exe existing isn't a conclusive test for a VC toolchain; NeverC also
...
bool findVCToolChainViaSetupConfig(vfs::FileSystem &VFS,
  ...
  SmallString<256> VCRootPath;
  {
    size_t wlen = wcslen(VCPathWide);
    convertWideToUTF8(VCPathWide, wlen, VCRootPath);
  }
