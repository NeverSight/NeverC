/*===- Unicode.c - Unicode character properties -----------------*- C -*-===*\
|*                                                                            *|
...
typedef struct {
  uint32_t lo;
  uint32_t hi;
} csupport_unicode_range_t;

static bool range_contains(const csupport_unicode_range_t *ranges,
                           size_t count, uint32_t c) {
...
static const csupport_unicode_range_t PrintableRanges[] = {
...
#define PRINTABLE_COUNT (sizeof(PrintableRanges) / sizeof(PrintableRanges[0]))

static const csupport_unicode_range_t CombiningRanges[] = {
...
#define COMBINING_COUNT (sizeof(CombiningRanges) / sizeof(CombiningRanges[0]))

static const csupport_unicode_range_t DoubleWidthRanges[] = {
...
#define DOUBLEWIDTH_COUNT (sizeof(DoubleWidthRanges) / sizeof(DoubleWidthRanges[0]))

bool csupport_unicode_is_printable(int ucs) {
  return ucs == 0x00AD ||
         range_contains(PrintableRanges, PRINTABLE_COUNT, (uint32_t)ucs);
}

bool csupport_unicode_is_formatting(int ucs) {
  static const csupport_unicode_range_t CfRanges[] = {
  ...
  return range_contains(CfRanges, sizeof(CfRanges) / sizeof(CfRanges[0]),
                        (uint32_t)ucs);
}

int csupport_unicode_char_width(int ucs) {
  if (!csupport_unicode_is_printable(ucs))
    return CSUPPORT_UNICODE_ERROR_NON_PRINTABLE;
  if (range_contains(CombiningRanges, COMBINING_COUNT, (uint32_t)ucs))
    return 0;
  if (range_contains(DoubleWidthRanges, DOUBLEWIDTH_COUNT, (uint32_t)ucs))
    return 2;
  return 1;
}

static bool is_printable_ascii(char c) { return c > 31 && c < 127; }

int csupport_unicode_column_width_utf8_raw(const char *data, size_t len) {
  csupport_string_ref_t t = {data, len};
  return csupport_unicode_column_width_utf8(t);
}

int csupport_unicode_column_width_utf8(csupport_string_ref_t text) {
  unsigned col_width = 0;
  for (size_t i = 0; i < text.length;) {
    unsigned nbytes = getNumBytesForUTF8((UTF8)text.data[i]);

    if (nbytes == 1) {
      if (!is_printable_ascii(text.data[i]))
        return CSUPPORT_UNICODE_ERROR_NON_PRINTABLE;
      col_width += 1;
      i += 1;
      continue;
    ...
    if (nbytes == 0 || i + nbytes > text.length)
