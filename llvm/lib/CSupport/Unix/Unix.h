...
static inline bool MakeErrMsg(llvm::SmallVectorImpl<char> *ErrMsg,
                              llvm::StringRef prefix, int errnum = -1) {
  if (!ErrMsg)
    return true;
  if (errnum == -1)
    errnum = errno;
  ...
  OS << prefix << ": " << llvm::sys::StrError(errnum);
...
[[noreturn]] static inline void ReportErrnumFatal(const char *Msg, int errnum) {
  ...
  llvm::report_fatal_error(llvm::Twine(llvm::StringRef(ErrMsg)));
}
