...
bool VectorCombine::scalarizeLoadExtract(Instruction &I) {
  ...
  InstructionCost OriginalCost = TTI.getMemoryOpCost(
      Instruction::Load, VecTy, LI->getAlign(), LI->getPointerAddressSpace());
...
static const char VectorCombineLrtID = 0;

PreservedAnalyses VectorCombinePass::run(Function &F,
                                         FunctionAnalysisManager &FAM) {
  auto &LRT = FAM.getResult<LastRunTrackingAnalysis>(F);
  if (LRT.shouldSkip(&VectorCombineLrtID))
  ...
  LRT.update(&VectorCombineLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
