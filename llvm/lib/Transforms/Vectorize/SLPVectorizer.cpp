...
public:
  ...
  BoUpSLP(Function *Func, ScalarEvolution *Se, TargetTransformInfo *Tti,
      ...
      : BatchAA(*Aa), F(Func), SE(Se), TTI(Tti), TLI(TLi), LI(Li), DT(Dt),
        AC(AC), DB(DB), DL(DL), ORE(ORE), Builder(Se->getContext()) {
...
static LoadsState canVectorizeLoads(ArrayRef<Value *> VL, const Value *VL0,
  ...
  if (IsSorted || all_of(PointerOps, [&](Value *P) {
    ...
    bool ProfitableGatherPointers =
        static_cast<unsigned>(count_if(PointerOps,
                                       [L](Value *V) {
...
Value *BoUpSLP::vectorizeTree(TreeEntry *E, bool PostponedPHIs) {
  ...
  switch (ShuffleOrOp) {
  case Instruction::PHI: {
    assert((E->ReorderIndices.empty() || E != VectorizableTree.front().get() ||
...
static const char SLPVectorizerLrtID = 0;
...
PreservedAnalyses SLPVectorizerPass::run(Function &F,
  ...
  if (LRT.shouldSkip(&SLPVectorizerLrtID))
    return PreservedAnalyses::all();
  ...
  LRT.update(&SLPVectorizerLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
...
public:
  ...
  Value *tryToReduce(BoUpSLP &V, TargetTransformInfo *TTI,
    ...
    if (NumReducedVals < ReductionLimit &&
        (!AllowHorRdxIdenityOptimization || all_of(
                                                ReducedVals,
                                                [](ArrayRef<Value *> RedV) {
                                                  return RedV.size() < 2 ||
                                                         !allConstant(RedV) ||
