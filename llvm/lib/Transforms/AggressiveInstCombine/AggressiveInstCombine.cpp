...
static const char AggressiveInstCombineLrtID = 0;

PreservedAnalyses AggressiveInstCombinePass::run(Function &F,
                                                 FunctionAnalysisManager &AM) {
  auto &LRT = AM.getResult<LastRunTrackingAnalysis>(F);
  if (LRT.shouldSkip(&AggressiveInstCombineLrtID))
  ...
  bool Changed = runImpl(F, AC, TTI, TLI, DT, AA);
  LRT.update(&AggressiveInstCombineLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
