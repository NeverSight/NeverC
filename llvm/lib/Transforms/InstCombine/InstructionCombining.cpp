...
static const char InstCombineLRTID = 0;

PreservedAnalyses InstCombinePass::run(Function &F,
                                       FunctionAnalysisManager &AM) {
  auto &LRT = AM.getResult<LastRunTrackingAnalysis>(F);
  if (LRT.shouldSkip(&InstCombineLRTID, Options))
  ...
  bool Changed = combineInstructionsOverFunction(
      F, Worklist, AA, AC, TLI, TTI, DT, ORE, BFI, PSI, LI, Options);
  LRT.update(&InstCombineLRTID, Changed, Options);
  if (!Changed)
    return PreservedAnalyses::all();
