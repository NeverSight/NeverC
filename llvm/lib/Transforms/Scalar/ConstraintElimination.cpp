...
static const char ConstraintElimLrtID = 0;

PreservedAnalyses ConstraintEliminationPass::run(Function &F,
                                                 FunctionAnalysisManager &AM) {
  auto &LRT = AM.getResult<LastRunTrackingAnalysis>(F);
  if (LRT.shouldSkip(&ConstraintElimLrtID))
  ...
  bool Changed = eliminateConstraints(F, DT, LI, SE, ORE);
  LRT.update(&ConstraintElimLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
