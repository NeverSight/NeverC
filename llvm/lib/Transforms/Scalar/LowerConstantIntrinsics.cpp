...
static const char LowerConstantIntrinsicsLrtID = 0;

PreservedAnalyses
LowerConstantIntrinsicsPass::run(Function &F, FunctionAnalysisManager &AM) {
  auto &LRT = AM.getResult<LastRunTrackingAnalysis>(F);
  if (LRT.shouldSkip(&LowerConstantIntrinsicsLrtID))
    return PreservedAnalyses::all();

  bool Changed =
      lowerConstantIntrinsics(F, AM.getResult<TargetLibraryAnalysis>(F),
                              AM.getCachedResult<DominatorTreeAnalysis>(F));
  LRT.update(&LowerConstantIntrinsicsLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
