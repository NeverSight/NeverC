...
static cl::opt<TTI::AddressingModeKind> PreferredAddresingMode(
    ...
    cl::values(clEnumValN(TTI::AMK_None, "none",
...
void LSRInstance::CollectFixupsAndInitialFormulae() {
  ...
  for (const IVStrideUse &U : IU) {
    ...
    if (ICmpInst *CI = dyn_cast<ICmpInst>(UserInst)) {
      // If CI can be saved in some target (e.g. replaced inside a hardware
      // loop), no need to generate initial formulae for it.
  ...
  if (EnableFormTerm) {
    if (auto Opt = canFoldTermCondOfLoop(L, SE, DT, LI)) {
      ...
      Value *TermValue = Expander.expandCodeFor(
          TermValueS, ToHelpFold->getType(), LoopPreheader->getTerminator());
