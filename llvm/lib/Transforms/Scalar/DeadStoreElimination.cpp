...
struct DSEState {
  ...
  bool eliminateDeadWritesAtEndOfFunction() {
    ...
    do {
      ...
      for (MemoryDef *Def : llvm::reverse(MemDefs)) {
        ...
        if (DefI->isVolatile() && !DefI->isPHINodeOrSelectInstOrSwitchInst())
          continue;
  ...
  bool removePartiallyOverlappedStores(InstOverlapIntervalsTy &IOL) {
    ...
    for (auto OI : IOL) {
      ...
      if (DeadI->isVolatile() && !DeadI->isPHINodeOrSelectInstOrSwitchInst())
        continue;
  ...
  bool eliminateRedundantStoresOfExistingValues() {
    ...
    for (auto *Def : MemDefs) {
      ...
      if (DefInst->isVolatile() &&
          !DefInst->isPHINodeOrSelectInstOrSwitchInst())
        continue;
...
static bool eliminateDeadStores(Function &F, AliasAnalysis &AA, MemorySSA &MSSA,
  ...
  for (unsigned I = 0; I < State.MemDefs.size(); I++) {
    ...
    if (KillingI->isVolatile() &&
        !KillingI->isPHINodeOrSelectInstOrSwitchInst())
      continue;
