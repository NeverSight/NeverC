...
private:
  ...
  bool visitIntrinsicInst(IntrinsicInst &II) {
    ...
    if (II.getParent() && II.getParent()->getParent() &&
        II.getParent()->getParent()->hasSEH()) {
...
private:
  ...
  bool foldGEPPhi(GetElementPtrInst &GEPI) {
    ...
    LLVM_DEBUG(for (Value *In : NewPN->incoming_values()) dbgs()
                   << "\n              " << *In;
               dbgs() << "\n              " << *NewPN << '\n');
