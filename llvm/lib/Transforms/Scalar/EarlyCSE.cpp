...
static unsigned hashCallInst(CallInst *CI) {
  ...
  return hash_combine(CI->getOpcode(), hash_combine_range(CI->value_op_begin(),
                                                          CI->value_op_end()));
}
...
static const char EarlyCSELrtID = 0;
...
PreservedAnalyses EarlyCSEPass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  if (LRT.shouldSkip(&EarlyCSELrtID, Opts))
  ...
  bool Changed = CSE.run();
  LRT.update(&EarlyCSELrtID, Changed, Opts);
  if (!Changed)
    return PreservedAnalyses::all();
