...
PreservedAnalyses CallSiteSplittingPass::run(Function &F,
  ...
  bool Changed = doCallSiteSplitting(F, TLI, TTI, DT);
  LRT.update(&CallSiteSplittingLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
