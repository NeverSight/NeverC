...
static cl::opt<bool>
    DisableLTOPropagation("disable-lto-funcattrs", cl::init(true), cl::Hidden,
                          cl::desc("Don't propagate function-attrs in LTO"));
...
static FunctionSummary *calculatePrevailingSummary(
  ...
  for (const auto &GVS : VI.getSummaryList()) {
    ...
    if (GlobalValue::isLocalLinkage(Linkage)) {
      if (Local) {
        LLVM_DEBUG(
            dbgs() << "LTO FunctionAttrs: Multiple Local Linkage, bailing on "
                      "function "
                   << VI.name() << " from " << FS->modulePath()
                   << ". Previous module " << Local->modulePath() << "\n");
        return nullptr;
      }
      Local = FS;
...
// unordered.  We conservatively consider monotonic operations potentially
// synchronizing since we generally don't do much with them; this is simply
// risk reduction.
