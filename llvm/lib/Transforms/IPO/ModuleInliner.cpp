...
// missing any calls in the module level. It doesn't need any information about
...
//
// NeverC: ported from upstream LLVM (release/21.x).  The upstream CtxProf and
// InlineOrder dependencies are trimmed in this tree, so this version uses a
// built-in smallest-callee-first lazy priority queue (equivalent to upstream's
// default SizePriority InlineOrder).
//
//===----------------------------------------------------------------------===//
...
// The CGSCC inliner curbs runaway inlining through call-graph cycles with
// IntraSCCCostMultiplier.  The module inliner has no SCC information, so the
// equivalent brake is based on the clone-chain depth recorded in the inline
// history: call sites materialized more than `module-inline-free-depth`
// clone levels deep get an exponentially growing cost multiplier.
static cl::opt<unsigned> ModuleInlineFreeDepth(
    "module-inline-free-depth", cl::init(4), cl::Hidden,
    cl::desc("Clone-chain depth up to which the module inliner applies no "
             "cost penalty to call sites produced by inlining"));

static cl::opt<unsigned> ModuleInlineDepthMultiplier(
    "module-inline-depth-multiplier", cl::init(2), cl::Hidden,
    cl::desc("Cost multiplier applied per clone-chain level beyond "
             "module-inline-free-depth"));

namespace {

/// Smallest-callee-first lazy priority worklist.  Priorities are captured at
/// push time; on pop, a stale priority (callee size changed by intervening
/// inlining) causes the entry to be re-pushed with the fresh size instead of
/// being processed.  This mirrors upstream's PriorityInlineOrder/SizePriority.
class SmallestCalleeFirstQueue {
  struct Entry {
    CallBase *CB;
    // Caller is recorded at push time: when a caller becomes trivially dead
    // its body (including CB) is deleted by dropAllReferences(), so CB must
    // not be dereferenced before checking the caller against the dead set.
    Function *Caller;
    int InlineHistoryID;
    unsigned Size;
  };
...
public:
  void push(CallBase *CB, int InlineHistoryID) {
    Function *Callee = CB->getCalledFunction();
    assert(Callee && !Callee->isDeclaration());
    Heap.push(
        {CB, CB->getCaller(), InlineHistoryID, Callee->getInstructionCount()});
  }

  bool empty() const { return Heap.empty(); }

  /// Pop the next up-to-date entry.  Entries whose caller is in \p
  /// DeadCallers (their CB pointers dangle) or whose callee got dropped are
  /// discarded.  Returns false when the queue is exhausted.
  bool pop(std::pair<CallBase *, int> &Out,
           const DenseSet<Function *> &DeadCallers) {
    while (!Heap.empty()) {
      Entry E = Heap.top();
      Heap.pop();
      if (DeadCallers.count(E.Caller))
        continue;
      Function *Callee = E.CB->getCalledFunction();
      if (!Callee || Callee->isDeclaration())
        continue;
      unsigned CurSize = Callee->getInstructionCount();
      if (CurSize != E.Size) {
        Heap.push({E.CB, E.Caller, E.InlineHistoryID, CurSize});
        continue;
      }
      Out = {E.CB, E.InlineHistoryID};
      return true;
    }
    return false;
  }
...
PreservedAnalyses ModuleInlinerPass::run(Module &M,
  ...
  // limited to bottom-up order.  More globally scoped inline order is enabled.
  // Also, the inline deferral logic becomes unnecessary in this module
  // inliner.
  SmallestCalleeFirstQueue Calls;
  ...
  // Clone-chain depth per history entry, used for the depth cost brake.
  SmallVector<unsigned, 16> InlineHistoryDepth;
  ...
  while (Calls.pop(P, DeadFunctionSet)) {
    ...
    if (!IFI.InlinedCallSites.empty()) {
      ...
      unsigned NewDepth =
          (InlineHistoryID == -1 ? 0 : InlineHistoryDepth[InlineHistoryID]) +
          1;
      InlineHistoryDepth.push_back(NewDepth);

      for (CallBase *ICB : reverse(IFI.InlinedCallSites)) {
        Function *NewCallee = ICB->getCalledFunction();
        if (NewCallee)
          if (!NewCallee->isDeclaration()) {
            // Brake runaway expansion through call-graph cycles: deep
            // clone chains get exponentially more expensive, mirroring the
            // CGSCC inliner's IntraSCCCostMultiplier behavior.
            if (NewDepth > ModuleInlineFreeDepth) {
              int CurMult =
                  getStringFnAttrAsInt(
                      *ICB,
                      InlineConstants::FunctionInlineCostMultiplierAttributeName)
                      .value_or(1);
              ICB->addFnAttr(Attribute::get(
                  M.getContext(),
                  InlineConstants::FunctionInlineCostMultiplierAttributeName,
                  itostr(CurMult * (int)ModuleInlineDepthMultiplier)));
            }
    ...
    if (Callee.hasLocalLinkage()) {
      ...
      if (Callee.use_empty() && !isKnownLibFunction(Callee, GetTLI(Callee))) {
        ...
        // finish inlining.  Calls inside the dead body are filtered out
        // lazily on pop via DeadFunctionSet.
