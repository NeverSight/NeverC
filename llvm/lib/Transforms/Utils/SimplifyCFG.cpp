...
/// For a switch whose case blocks all compute "CommonBase BinOp Constant" and
/// branch to a common merge block with a PHI, factor out CommonBase so that the
/// PHI only merges constants. This enables SwitchToLookupTable downstream.
///
/// Before:
///   case 0: %r0 = add %x, 100; br %merge
///   case 1: %r1 = add %x, 200; br %merge
///   merge: %phi = phi [%r0, c0], [%r1, c1]
///
/// After:
///   case 0: br %merge
///   case 1: br %merge
///   merge: %const_phi = phi [100, c0], [200, c1]
///          %result = add %x, %const_phi
static void factorCommonOperandFromSwitchPHIs(SwitchInst *SI) {
  if (SI->getNumCases() < 3)
    return;

  BasicBlock *SwitchBB = SI->getParent();

  // Collect case successor blocks. Each must have a single predecessor
  // (the switch block) and branch unconditionally to a common merge block.
  SmallPtrSet<BasicBlock *, 8> CaseBBSet;
  SmallVector<BasicBlock *, 8> CaseBBs;
  BasicBlock *MergeBB = nullptr;
  for (auto &Case : SI->cases()) {
    BasicBlock *CaseBB = Case.getCaseSuccessor();
    if (CaseBB == SwitchBB || CaseBB->getSinglePredecessor() != SwitchBB)
      return;
    ...
    if (!MergeBB)
      MergeBB = Succ;
    else if (Succ != MergeBB)
      return;
    if (CaseBBSet.insert(CaseBB).second)
      CaseBBs.push_back(CaseBB);
  }
  if (!MergeBB || CaseBBs.empty())
    return;

  // Find a single PHI candidate to factor.
  PHINode *TargetPHI = nullptr;
  Value *CommonBase = nullptr;
  unsigned CommonOpcode = 0;
  bool ConstIsRHS = true;

  for (PHINode &PHI : MergeBB->phis()) {
    if (!PHI.getType()->isIntegerTy())
      continue;

    // Require that EVERY incoming value of this PHI is either:
    // (a) from a case block and is BinOp(CommonBase, Const), or
    // (b) from the switch block directly (case 0 → merge, no intermediate BB).
    // We reject PHIs that have incoming values from unrelated blocks.
    Value *Base = nullptr;
    unsigned Opc = 0;
    bool RHS = true;
    bool Valid = true;
    ...
    for (unsigned i = 0, e = PHI.getNumIncomingValues(); i != e; ++i) {
      ...
      // Incoming from switch block directly (one case targets MergeBB).
      if (InBB == SwitchBB) {
        Valid = false;
        break;
      }

      if (!CaseBBSet.count(InBB)) {
        // Incoming from default block or unknown block.
        BasicBlock *DefaultBB = SI->getDefaultDest();
        if (InBB != DefaultBB) {
          Valid = false;
          break;
        }
        // Default must also match the pattern.
        auto *BO = dyn_cast<BinaryOperator>(V);
        if (!BO || BO->getParent() != InBB || !BO->hasOneUse()) {
          Valid = false;
          break;
        }
        // Will check pattern match below.
      }
      ...
      // The BinOp must be IN the incoming block (not hoisted).
      if (BO->getParent() != InBB) {
        Valid = false;
        break;
      }
      ...
      if (!Base) {
        ...
        Opc = BO->getOpcode();
        RHS = ThisRHS;
      } else if (ThisBase != Base || BO->getOpcode() != Opc) {
        Valid = false;
        break;
      }
    }
    ...
    // CommonBase must be defined before the switch.
    if (auto *BaseI = dyn_cast<Instruction>(Base)) {
      if (BaseI->getParent() == SwitchBB) {
        if (!BaseI->comesBefore(SI))
          continue;
      }
    }
  ...
  // Build the constant PHI.
  PHINode *ConstPHI =
      PHINode::Create(TargetPHI->getType(), TargetPHI->getNumIncomingValues(),
                      TargetPHI->getName() + ".const", TargetPHI);

  for (unsigned i = 0, e = TargetPHI->getNumIncomingValues(); i != e; ++i) {
    BasicBlock *InBB = TargetPHI->getIncomingBlock(i);
    auto *BO = cast<BinaryOperator>(TargetPHI->getIncomingValue(i));
    Constant *C = ConstIsRHS ? cast<Constant>(BO->getOperand(1))
                             : cast<Constant>(BO->getOperand(0));
    ConstPHI->addIncoming(C, InBB);
  }

  auto CombinedOpc = static_cast<Instruction::BinaryOps>(CommonOpcode);
  BinaryOperator *Combined =
      ConstIsRHS ? BinaryOperator::Create(CombinedOpc, CommonBase, ConstPHI,
                                          TargetPHI->getName() + ".combined")
                 : BinaryOperator::Create(CombinedOpc, ConstPHI, CommonBase,
                                          TargetPHI->getName() + ".combined");
  Combined->insertAfter(ConstPHI);
  TargetPHI->replaceAllUsesWith(Combined);

  // Collect dead BinOps, then erase the PHI, then erase dead BinOps.
  SmallVector<BinaryOperator *, 8> DeadBinOps;
  for (unsigned i = 0, e = TargetPHI->getNumIncomingValues(); i != e; ++i)
    if (auto *BO = dyn_cast<BinaryOperator>(TargetPHI->getIncomingValue(i)))
      DeadBinOps.push_back(BO);

  TargetPHI->eraseFromParent();

  for (auto *BO : DeadBinOps)
    if (BO->use_empty())
      BO->eraseFromParent();
}
...
static bool SwitchToLookupTable(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  if (NeedMask) {
    ...
    for (size_t I = 0, E = ResultList.size(); I != E; ++I) {
      uint64_t Idx =
          (ResultList[I].first->getValue() - TableIndexOffset->getValue())
              .getLimitedValue();
      MaskInt |= One << Idx;
    }
...
bool SimplifyCFGOpt::simplifySwitch(SwitchInst *SI, IRBuilder<> &Builder) {
  ...
  // Factor out common non-constant operands from switch PHI nodes so that
  // SwitchToLookupTable can handle patterns like:
  //   case 0: result = x + C0; case 1: result = x + C1; ...
  // Rewritten to: result = x + phi(C0, C1, ...)
  if (Options.ConvertSwitchToLookupTable)
    factorCommonOperandFromSwitchPHIs(SI);
