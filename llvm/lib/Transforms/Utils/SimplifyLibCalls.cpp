...
static bool insertSinCosCall(IRBuilderBase &B, Function *OrigCallee, Value *Arg,
  ...
  if (UseFloat) {
    ...
    assert(T.getArch() == Triple::x86_64 || T.getArch() == Triple::aarch64);
...
Value *FortifiedLibCallSimplifier::optimizeCall(CallInst *CI,
  ...
  // Some NeverC users checked for _chk libcall availability using:
  ...
  //
  // Until we change NeverC and/or teach external users to check for
  // availability differently, disregard the "nobuiltin" attribute and TLI::has.
  //
  // PR23093.
