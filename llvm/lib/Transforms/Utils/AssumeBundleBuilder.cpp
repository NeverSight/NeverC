...
static const char AssumeSimplifyLrtID = 0;
...
PreservedAnalyses AssumeSimplifyPass::run(Function &F,
  ...
  if (LRT.shouldSkip(&AssumeSimplifyLrtID))
    return PreservedAnalyses::all();
  bool Changed = simplifyAssumes(F, &AM.getResult<AssumptionAnalysis>(F),
                                 AM.getCachedResult<DominatorTreeAnalysis>(F));
  LRT.update(&AssumeSimplifyLrtID, Changed);
  if (!Changed)
    return PreservedAnalyses::all();
