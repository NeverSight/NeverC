...
SolveQuadraticAddRecExact(const SCEVAddRecExpr *AddRec, ScalarEvolution &SE) {
  ...
  APInt XVal = APIntOps::SolveQuadraticEquationWrap(A, B, C, BitWidth + 1);
  if (!XVal.getBitWidth())
    return std::nullopt;

  ConstantInt *CX = ConstantInt::get(SE.getContext(), XVal);
  ConstantInt *V = EvaluateConstantChrecAtConstant(AddRec, CX, SE);
  if (!V->isZero())
    return std::nullopt;

  return TruncIfPossible(std::optional<APInt>(XVal), BitWidth);
}
  ...
  auto SolveForBoundary =
    ...
    if (BitWidth > 1) {
      LLVM_DEBUG(dbgs() << "SolveQuadraticAddRecRange: solving for "
                           "signed overflow\n");
      APInt SOVal =
          APIntOps::SolveQuadraticEquationWrap(A, B, -Bound, BitWidth);
      if (SOVal.getBitWidth())
        SO = SOVal;
    }
    LLVM_DEBUG(dbgs() << "SolveQuadraticAddRecRange: solving for "
                         "unsigned overflow\n");
