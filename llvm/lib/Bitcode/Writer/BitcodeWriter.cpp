...
protected:
  ...
  /// Optional per-module index to write for LTO.
...
public:
  ...
  IndexBitcodeWriter(BitstreamWriter &Stream, StringTableBuilder &StrtabBuilder,
    ...
    forEachSummary([&](GVInfo I, bool IsAliasee) {
      ...
      // written, so that we can compact them in the case of distributed LTO
      // indexes.
...
void ModuleBitcodeWriterBase::writePerModuleGlobalValueSummary() {
  Stream.EnterSubblock(bitc::FULL_LTO_GLOBALVAL_SUMMARY_BLOCK_ID, 4);
  ...
  forEachSummary([&](GVInfo I, bool IsAliasee) {
    ...
    writeFunctionHeapProfileRecords(
        ...
        [&](const ValueInfo &VI) -> unsigned {
          ...
          // This can happen in shared index files for distributed LTO if
          ...
          // validation in the LTO backends.
          if (!ValueID)
            return 0;
          return *ValueID;
        },
...
static void emitDarwinBCHeaderAndTrailer(SmallVectorImpl<char> &Buffer,
  ...
  if (Arch == Triple::x86_64)
    ...
    CPUType = DARWIN_CPU_TYPE_ARM | DARWIN_CPU_ARCH_ABI64;
...
// writing the combined index file for LTO. When writing a subset of the
...
class ThinLinkBitcodeWriter : public ModuleBitcodeWriterBase {
  /// ModHash is for use in LTO incremental build, generated while writing
...
void BitcodeWriter::writeLinkBitcode(const Module &M,
                                     const ModuleSummaryIndex &Index,
                                     const ModuleHash &ModHash) {
  assert(!WroteStrtab);
...
// where it will be written in a new bitcode block. This is used when
// writing the per-module index file for LTO.
void llvm::writeLinkBitcodeToFile(const Module &M, raw_ostream &Out,
                                  const ModuleSummaryIndex &Index,
                                  const ModuleHash &ModHash) {
  SmallVector<char, 0> Buffer;
  Buffer.reserve(256 * 1024);
  ...
  BitcodeWriter Writer(Buffer);
  Writer.writeLinkBitcode(M, Index, ModHash);
  Writer.writeSymtab();
  Writer.writeStrtab();
