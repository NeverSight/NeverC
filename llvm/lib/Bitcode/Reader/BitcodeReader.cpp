...
std::error_code llvm::errorToErrorCodeAndEmitErrors(LLVMContext &Ctx,
  ...
  if (Err) {
    ...
    handleAllErrors(std::move(Err), [&](ErrorInfoBase &EIB) {
      int ec_val = EIB.convertToErrorCode();
      EC = std::error_code(ec_val, std::generic_category());
...
ModuleSummaryIndexBitcodeReader::getValueInfoFromValueId(unsigned ValueId) {
  ...
  // distributed LTO index files when the callee function summary is not
  ...
  while (true) {
    ...
    case BitstreamEntry::SubBlock:
      ...
      case bitc::FULL_LTO_GLOBALVAL_SUMMARY_BLOCK_ID:
        ...
        // performing LTO compiles so we don't later invoke
...
Error ModuleSummaryIndexBitcodeReader::parseEntireSummary(unsigned ID) {
  ...
  while (true) {
    ...
    // information used for LTO renaming and importing.
    Record.clear();
    Expected<unsigned> MaybeBitCode = Stream.readRecord(Entry.ID, Record);
    if (!MaybeBitCode)
      return MaybeBitCode.takeError();
...
Expected<BitcodeLTOInfo> BitcodeModule::getLTOInfo() {
  ...
  while (true) {
    ...
    case BitstreamEntry::Error:
      return error("Malformed block");
    case BitstreamEntry::EndBlock:
      return BitcodeLTOInfo{/*IsFullLTO=*/true, /*HasSummary=*/false,
...
llvm::getModuleSummaryIndexForFile(StringRef Path,
                                   bool IgnoreEmptyLTOIndexFile) {
  ErrorOr<std::unique_ptr<MemoryBuffer>> FileOrErr =
      MemoryBuffer::getFileOrSTDIN(Path);
  if (!FileOrErr)
    return errorCodeToError(FileOrErr.getError());
  if (IgnoreEmptyLTOIndexFile && !(*FileOrErr)->getBufferSize())
    return nullptr;
