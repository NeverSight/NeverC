...
/// Flag whether we need to import full type definitions for LTO.
...
static cl::opt<bool> ImportFullTypeDefinitions(
    ...
    cl::desc("Import full type definitions for LTO."));
...
class MetadataLoader::MetadataLoaderImpl {
  ...
  /// True if metadata is being parsed for a module being LTO imported.
...
MetadataLoader::MetadataLoaderImpl::lazyLoadModuleMetadataBlock() {
  ...
  while (true) {
    ...
    case BitstreamEntry::Record: {
      ...
      case bitc::METADATA_DISCARDED_DEBUG_RECORD_30:
  ...
  if (ModuleLevel && IsImporting && MetadataList.empty() &&
    ...
    if (SuccessOrErr.get()) {
      ...
      if (Error Err = Stream.SkipBlock()) {
        ...
        // LTO/X86/debuginfo-cu-import.ll relies on.
...
#define GET_OR_DISTINCT(CLASS, ARGS)                                           \
  ...
  case bitc::METADATA_COMPOSITE_TYPE: {
    ...
    // If this module is being parsed so that it can be LTO imported
  ...
  case bitc::METADATA_DISCARDED_DEBUG_RECORD_30: {
    MetadataList.assignValue(nullptr, NextMetadataNo);
    NextMetadataNo++;
    break;
  }
  case bitc::METADATA_IMPORTED_ENTITY: {
