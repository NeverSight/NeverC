...
struct Elf_Rel_Impl<ELFType<TargetEndianness, false>, false> {
  ...
  uint32_t getRInfo() const { return r_info; }
  void setRInfo(uint32_t R) { r_info = R; }

  // These accessors and mutators correspond to the ELF32_R_SYM, ELF32_R_TYPE,
  // and ELF32_R_INFO macros defined in the ELF specification:
  uint32_t getSymbol() const { return this->getRInfo() >> 8; }
  unsigned char getType() const {
    return (unsigned char)(this->getRInfo() & 0x0ff);
  }
...
struct Elf_Rel_Impl<ELFType<TargetEndianness, true>, false> {
  ...
  void setRInfo(uint64_t R) { r_info = R; }

  // These accessors and mutators correspond to the ELF64_R_SYM, ELF64_R_TYPE,
  // and ELF64_R_INFO macros defined in the ELF specification:
  uint32_t getSymbol() const { return (uint32_t)(this->getRInfo() >> 32); }
  uint32_t getType() const {
    return (uint32_t)(this->getRInfo() & 0xffffffffL);
  }
  void setSymbol(uint32_t s) { setSymbolAndType(s, getType()); }
