...
uint64_t ELFObjectFile<ELFT>::getRelocationType(DataRefImpl Rel) const {
  const Elf_Shdr *sec = getRelSection(Rel);
  if (sec->sh_type == ELF::SHT_REL)
    return getRel(Rel)->getType();
  else
    return getRela(Rel)->getType();
}
...
template <class ELFT> StringRef ELFObjectFile<ELFT>::getFileFormatName() const {
  ...
  case ELF::ELFCLASS64:
    ...
    case ELF::EM_X86_64:
      return "elf64-x86-64";
    case ELF::EM_AARCH64:
      return isLittleEndian() ? "elf64-littleaarch64" : "elf64-unknown";
    default:
...
template <class ELFT> Triple::ArchType ELFObjectFile<ELFT>::getArch() const {
  ...
  case ELF::EM_AARCH64:
    return IsLittleEndian ? Triple::aarch64 : Triple::UnknownArch;
