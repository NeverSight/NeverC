...
/// NeverC only uses the heuristic-based advisor. ML-based advisors (Release /
/// Development) have been removed; the only remaining mode is `Default`.
/// Dynamic plugin advisors can still override behaviour via
/// `PluginInlineAdvisorAnalysis`.
enum class InliningAdvisorMode : int { Default };
