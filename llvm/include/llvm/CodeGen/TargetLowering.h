  ...
  public:
    ...
    ArgListEntry()
        : IsSExt(false), IsZExt(false), IsInReg(false), IsSRet(false),
          ...
          IsPreallocated(false), IsReturned(false), IsCFGuardTarget(false) {}
...
public:
  ...
  /// condition bits, and/or when reducing cmp+br is a win.
  ...
  /// x86_64 the rep register class for i8, i16, i32, and i64 is GR64.
  ...
  /// Certain targets require that some types such as vectors are
...
#include "llvm/IR/ConstrainedOps.def"
  ...
  /// instance with i128 inline assembly operands.
  virtual unsigned
  ...
  /// For some targets all vector types must be passed through the integer
  /// register set.
  ...
  /// Certain targets require unusual breakdowns of certain types, e.g.
  /// when a vector type is used and vectors are passed through the integer
  /// register set.
...
public:
  ...
  virtual bool shouldFormOverflowOp(unsigned Opcode, EVT VT,
    ...
    // besides the overflow check. On some targets, it is not profitable
    // to form an overflow op if the math result has no concrete users.
    if (VT.isVector())
      return false;
...
public:
  ...
  ///
  /// Having a callback like this is necessary for targets
