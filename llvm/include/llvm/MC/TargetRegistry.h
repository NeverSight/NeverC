...
public:
  ...
  /// triple.
  ///
  MCObjectFileInfo *createMCObjectFileInfo(MCContext &Ctx,
                                           bool LargeCodeModel = false) const {
    if (!MCObjectFileInfoCtorFn) {
      MCObjectFileInfo *MOFI = new MCObjectFileInfo();
      MOFI->initMCObjectFileInfo(Ctx, LargeCodeModel);
      return MOFI;
    }
    return MCObjectFileInfoCtorFn(Ctx, LargeCodeModel);
  }
  ...
  createTargetMachine(StringRef TT, StringRef CPU, StringRef Features,
                      ...
                      CodeGenOptLevel OL = CodeGenOptLevel::Default) const {
    if (!TargetMachineCtorFn)
      return nullptr;
    return TargetMachineCtorFn(*this, Triple(TT), CPU, Features, Options, CM,
                               OL);
  }
...
private:
  static MCObjectFileInfo *Allocator(MCContext &Ctx,
                                     bool LargeCodeModel = false) {
    return new MCObjectFileInfoImpl(Ctx, LargeCodeModel);
  }
};
...
private:
  static TargetMachine *Allocator(const Target &T, const Triple &TT,
                                  ...
                                  std::optional<CodeModel::Model> CM,
                                  CodeGenOptLevel OL) {
    return new TargetMachineImpl(T, TT, CPU, FS, Options, CM, OL);
  }
};
