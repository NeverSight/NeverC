...
namespace detail {
bool scalableFixedErrorAsWarningEnabled();
}
void initTypeSizeOptions();
#else
inline void initTypeSizeOptions() {}
#endif
...
public:
  ...
  //   }
  inline operator ScalarTy() const;
...
inline void reportInvalidSizeRequest(const char *Msg) {
#ifndef STRICT_FIXED_SIZE_VECTORS
  if (detail::scalableFixedErrorAsWarningEnabled()) {
    errs() << "warning: Invalid size request on a scalable vector; " << Msg
           << "\n";
    return;
  }
#endif
...
inline TypeSize::operator TypeSize::ScalarTy() const {
  if (isScalable()) {
