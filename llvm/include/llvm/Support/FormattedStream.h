...
inline void formatted_raw_ostream::UpdatePosition(const char *Ptr,
                                                  size_t Size) {
  char partial_buf[4] = {};
  size_t partial_len = PartialUTF8Char.size();
  if (partial_len > 0)
    memcpy(partial_buf, PartialUTF8Char.data(), partial_len);
  csupport_update_column_position(&Position.first, &Position.second, Ptr, Size,
                                  partial_buf, &partial_len);
  PartialUTF8Char.clear();
  if (partial_len > 0)
    PartialUTF8Char = StringRef(partial_buf, partial_len);
}
inline void formatted_raw_ostream::ComputePosition(const char *Ptr,
                                                   size_t Size) {
...
inline formatted_raw_ostream &
formatted_raw_ostream::PadToColumn(unsigned NewCol) {
  ComputePosition(getBufferStart(), GetNumBytesInBuffer());
  indent(std::max(int(NewCol - getColumn()), 1));
  return *this;
}
inline void formatted_raw_ostream::write_impl(const char *Ptr, size_t Size) {
  ComputePosition(Ptr, Size);
  TheStream->write(Ptr, Size);
  Scanned = 0;
}
inline formatted_raw_ostream &fouts() {
  static formatted_raw_ostream S(outs());
  return S;
}
inline formatted_raw_ostream &ferrs() {
  static formatted_raw_ostream S(errs());
  return S;
}
inline formatted_raw_ostream &fdbgs() {
