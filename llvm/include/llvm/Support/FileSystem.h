...
// Include after llvm::sys::fs::{file_type,file_status,...} so that headers
// pulled in by Error.h (e.g. CommandLine -> VirtualFileSystem -> FileSystem)
// see a complete sys::fs when they reference those types during nested
// inclusion.
...
inline size_t mapped_file_region::size() const {
  assert(Mapping && "Mapping failed but used anyway!");
  return Size;
}

inline char *mapped_file_region::data() const {
  assert(Mapping && "Mapping failed but used anyway!");
  return (char *)(Mapping);
}

inline const char *mapped_file_region::const_data() const {
  assert(Mapping && "Mapping failed but used anyway!");
...
inline Expected<sys::fs::FileLocker> raw_fd_ostream::lock() {
  int ec = csupport_fd_lock(FD);
  if (!ec)
    return sys::fs::FileLocker(FD);
  return errorCodeToError(std::error_code(ec, std::generic_category()));
}

inline Expected<sys::fs::FileLocker>
raw_fd_ostream::tryLockFor(Duration const &Timeout) {
  auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                Timeout.getDuration())
                .count();
  int ec = csupport_fd_try_lock_for(FD, static_cast<int64_t>(ms));
  if (!ec)
    return sys::fs::FileLocker(FD);
  return errorCodeToError(std::error_code(ec, std::generic_category()));
}

/// Creates an output stream based on \p OutputFileName ("-" for stdout,
/// "/dev/null" for null, otherwise a temp file atomically renamed on success).
inline Error writeToOutput(StringRef OutputFileName,
                           function_ref<Error(raw_ostream &)> Write) {
  if (OutputFileName == "-")
    return Write(outs());
