...
#define LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(E) ((E) && *(E))

namespace llvm {

inline bool DataExtractor::prepareRead(uint64_t Offset, uint64_t Size,
                                       Error *E) const {
...
template <typename T>
inline T DataExtractor::getU(uint64_t *offset_ptr, Error *Err) const {
  ErrorAsOutParameter ErrAsOut(Err);
  T val = 0;
  if (LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(Err))
    return val;
...
template <typename T>
inline T *DataExtractor::getUs(uint64_t *offset_ptr, T *dst, uint32_t count,
                               Error *Err) const {
  ErrorAsOutParameter ErrAsOut(Err);
  if (LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(Err))
    return 0;
...
inline uint8_t DataExtractor::getU8(uint64_t *offset_ptr,
                                    llvm::Error *Err) const {
  return getU<uint8_t>(offset_ptr, Err);
}

inline uint8_t *DataExtractor::getU8(uint64_t *offset_ptr, uint8_t *dst,
                                     uint32_t count) const {
  return getUs<uint8_t>(offset_ptr, dst, count, 0);
}

inline uint8_t *DataExtractor::getU8(Cursor &C, uint8_t *Dst,
                                     uint32_t Count) const {
  return getUs<uint8_t>(&C.Offset, Dst, Count, &C.Err);
}

inline uint16_t DataExtractor::getU16(uint64_t *offset_ptr,
                                      llvm::Error *Err) const {
  return getU<uint16_t>(offset_ptr, Err);
}

inline uint16_t *DataExtractor::getU16(uint64_t *offset_ptr, uint16_t *dst,
                                       uint32_t count) const {
  return getUs<uint16_t>(offset_ptr, dst, count, 0);
}

inline uint32_t DataExtractor::getU24(uint64_t *OffsetPtr, Error *Err) const {
  uint24_t ExtractedVal = getU<uint24_t>(OffsetPtr, Err);
  return ExtractedVal.getAsUint32(sys::IsLittleEndianHost);
}

inline uint32_t DataExtractor::getU32(uint64_t *offset_ptr,
                                      llvm::Error *Err) const {
  return getU<uint32_t>(offset_ptr, Err);
}

inline uint32_t *DataExtractor::getU32(uint64_t *offset_ptr, uint32_t *dst,
                                       uint32_t count) const {
  return getUs<uint32_t>(offset_ptr, dst, count, 0);
}

inline uint64_t DataExtractor::getU64(uint64_t *offset_ptr,
                                      llvm::Error *Err) const {
  return getU<uint64_t>(offset_ptr, Err);
}

inline uint64_t *DataExtractor::getU64(uint64_t *offset_ptr, uint64_t *dst,
                                       uint32_t count) const {
  return getUs<uint64_t>(offset_ptr, dst, count, 0);
}

inline uint64_t DataExtractor::getUnsigned(uint64_t *offset_ptr,
                                           uint32_t byte_size,
                                           llvm::Error *Err) const {
  switch (byte_size) {
  case 1:
...
inline int64_t DataExtractor::getSigned(uint64_t *offset_ptr,
...
inline StringRef DataExtractor::getCStrRef(uint64_t *OffsetPtr,
                                           Error *Err) const {
  ErrorAsOutParameter ErrAsOut(Err);
  if (LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(Err))
    return StringRef();
  ...
  if (Err)
    *Err = createStringError(std::errc::illegal_byte_sequence,
...
inline StringRef
DataExtractor::getFixedLengthString(uint64_t *OffsetPtr, uint64_t Length,
                                    StringRef TrimChars) const {
  StringRef Bytes(getBytes(OffsetPtr, Length));
  return Bytes.trim(TrimChars);
}

inline StringRef DataExtractor::getBytes(uint64_t *OffsetPtr, uint64_t Length,
                                         Error *Err) const {
  ErrorAsOutParameter ErrAsOut(Err);
  if (LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(Err))
    return StringRef();
...
template <typename T>
inline static T
getLEB128(llvm::StringRef Data, uint64_t *OffsetPtr, llvm::Error *Err,
          T (&Decoder)(const uint8_t *p, unsigned *n, const uint8_t *end,
                       const char **error)) {
  llvm::ArrayRef<uint8_t> Bytes = llvm::arrayRefFromStringRef(Data);
  assert(*OffsetPtr <= Bytes.size());
  llvm::ErrorAsOutParameter ErrAsOut(Err);
  if (LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(Err))
    return T();
  ...
  if (error) {
    if (Err)
      *Err = llvm::createStringError(
          std::errc::illegal_byte_sequence,
          "unable to decode LEB128 at offset 0x%8.8" PRIx64 ": %s", *OffsetPtr,
          error);
    return T();
  }
...
inline uint64_t DataExtractor::getULEB128(uint64_t *offset_ptr,
                                          Error *Err) const {
  return getLEB128(Data, offset_ptr, Err, decodeULEB128);
}

inline int64_t DataExtractor::getSLEB128(uint64_t *offset_ptr,
                                         Error *Err) const {
  return getLEB128(Data, offset_ptr, Err, decodeSLEB128);
}

inline void DataExtractor::skip(Cursor &C, uint64_t Length) const {
  ErrorAsOutParameter ErrAsOut(&C.Err);
  if (LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET(&C.Err))
    return;
...
#undef LLVM_DATAEXTRACTOR_ERR_OUTPTR_IS_SET
