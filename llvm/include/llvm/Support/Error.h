...
public:
  ...
  virtual void log(::llvm::raw_ostream &OS) const = 0;
  ...
  virtual int convertToErrorCode() const = 0;
...
void logAllUnhandledErrors(Error E, ::llvm::raw_ostream &OS,
                           Twine ErrorBanner = {});

/// Write all error messages (if any) in E to a string. The newline character
/// is used to separate error messages.
SmallString<256> toString(Error E);
...
public:
  ...
  int convertToErrorCode() const override { return EC.value(); }
...
public:
  ...
  std::string message(int condition) const override {
    switch ((ErrorErrorCode)(condition)) {
...
inline ErrorErrorCategory &getErrorErrorCat() {
  static ErrorErrorCategory Cat;
  return Cat;
}
} // namespace detail

inline void ErrorInfoBase::anchor() {}
inline char ErrorInfoBase::ID = 0;
inline char ErrorList::ID = 0;
inline void ECError::anchor() {}
inline char ECError::ID = 0;
inline char StringError::ID = 0;
inline char FileError::ID = 0;

inline void logAllUnhandledErrors(Error E, ::llvm::raw_ostream &OS,
                                  Twine ErrorBanner) {
  if (!E)
    return;
  OS << ErrorBanner;
...
inline SmallString<256> toString(Error E) {
...
inline int ErrorList::convertToErrorCode() const {
  return (int)(detail::ErrorErrorCode::MultipleErrors);
}

inline std::error_code inconvertibleErrorCode() {
  return {(int)(detail::ErrorErrorCode::InconvertibleError),
          detail::getErrorErrorCat()};
}

inline int FileError::convertToErrorCode() const {
  int NestedEC = Err->convertToErrorCode();
  if (NestedEC == (int)(detail::ErrorErrorCode::InconvertibleError))
    return (int)(detail::ErrorErrorCode::FileError);
  return NestedEC;
}
...
inline Error errorCodeToError(std::error_code EC) {
  if (!EC)
    return Error::success();
  return Error(std::unique_ptr<ECError>(new ECError(ECError(EC))));
}

inline std::error_code errorToErrorCode(Error Err) {
  int ec_val = 0;
  handleAllErrors(std::move(Err), [&](const ErrorInfoBase &EI) {
    ec_val = EI.convertToErrorCode();
  });
  if (ec_val == (int)(detail::ErrorErrorCode::InconvertibleError))
    ::llvm::report_fatal_error("Inconvertible error value");
  return std::error_code(ec_val, std::generic_category());
}

#if LLVM_ENABLE_ABI_BREAKING_CHECKS
inline void Error::fatalUncheckedError() const {
...
inline StringError::StringError(std::error_code EC, const Twine &S)
    : Msg(S.str()), EC(EC) {}
inline StringError::StringError(const Twine &S, std::error_code EC)
    : Msg(S.str()), EC(EC), PrintMsgOnly(true) {}
inline void StringError::log(::llvm::raw_ostream &OS) const {
  if (PrintMsgOnly) {
    OS << Msg;
  } else {
    OS << EC.message();
...
inline int StringError::convertToErrorCode() const { return EC.value(); }

inline Error createStringError(std::error_code EC, char const *Msg) {
  return make_error<StringError>(Msg, EC);
}

inline void report_fatal_error(Error Err, bool GenCrashDiag) {
  assert(Err && "report_fatal_error called with success value");
  SmallString<256> ErrMsg;
  {
    raw_svector_ostream ErrStream(ErrMsg);
