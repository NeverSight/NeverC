...
public:
  ...
  virtual SmallVector<StringRef, 16> keys() = 0;
  ...
  virtual void scalarTag(SmallVectorImpl<char> &) = 0;
...
yamlize(IO &io, T &Val, bool, EmptyContext &Ctx) {
  if (io.outputting()) {
    ...
    TaggedScalarTraits<T>::output(Val, io.getContext(), ScalarBuffer,
                                  TagBuffer);
    SmallString<64> TagStr(TagBuffer.str());
    io.scalarTag(TagStr);
...
template <> struct ScalarTraits<std::string> {
  static void output(const std::string &Val, void *, raw_ostream &Out) {
    Out << Val;
  }
...
public:
  ...
  // Check if there was a syntax or semantic error during parsing.
  // Returns 0 on success, errno-style value on error.
  int error();
...
private:
  ...
  SmallVector<StringRef, 16> keys() override;
  ...
  void scalarTag(SmallVectorImpl<char> &) override;
...
//===----------------------------------------------------------------------===//
// Inline implementations - IO
//===----------------------------------------------------------------------===//

namespace llvm {
namespace yaml {

inline IO::IO(void *Context) : Ctxt(Context) {}

inline IO::~IO() = default;

inline void *IO::getContext() const { return Ctxt; }

inline void IO::setContext(void *Context) { Ctxt = Context; }

inline void IO::setAllowUnknownKeys(bool Allow) {
  llvm_unreachable("Only supported for Input");
}

//===----------------------------------------------------------------------===//
// Inline implementations - Input
//===----------------------------------------------------------------------===//

inline Input::Input(StringRef InputContent, void *Ctxt,
...
inline Input::Input(MemoryBufferRef Input, void *Ctxt,
                    SourceMgr::DiagHandlerTy DiagHandler, void *DiagHandlerCtxt)
...
inline Input::~Input() = default;

inline int Input::error() { return EC; }

inline bool Input::outputting() const { return false; }

inline bool Input::setCurrentDocument() {
...
inline bool Input::nextDocument() { return ++DocIterator != Strm->end(); }

inline const Node *Input::getCurrentNode() const {
  return CurrentNode ? CurrentNode->_node : 0;
}

inline bool Input::mapTag(StringRef Tag, bool Default) {
  if (!CurrentNode)
    return false;
  auto foundTag = CurrentNode->_node->getVerbatimTag();
  if (foundTag.empty())
    return Default;
  return Tag.equals(foundTag);
}

inline void Input::beginMapping() {
  if (EC)
    return;
...
inline SmallVector<StringRef, 16> Input::keys() {
  MapHNode *MN = dyn_cast<MapHNode>(CurrentNode);
  SmallVector<StringRef, 16> Ret;
  if (!MN) {
...
inline bool Input::preflightKey(const char *Key, bool Required, bool,
...
inline void Input::postflightKey(void *saveInfo) {
  CurrentNode = (HNode *)(saveInfo);
}

inline void Input::endMapping() {
  if (EC)
    return;
  MapHNode *MN = dyn_cast_or_null<MapHNode>(CurrentNode);
  if (!MN)
...
inline void Input::beginFlowMapping() { beginMapping(); }

inline void Input::endFlowMapping() { endMapping(); }

inline unsigned Input::beginSequence() {
...
inline void Input::endSequence() {}

inline bool Input::preflightElement(unsigned Index, void *&SaveInfo) {
  if (EC)
    return false;
  if (SequenceHNode *SQ = dyn_cast<SequenceHNode>(CurrentNode)) {
    SaveInfo = CurrentNode;
...
inline void Input::postflightElement(void *SaveInfo) {
  CurrentNode = (HNode *)(SaveInfo);
}

inline unsigned Input::beginFlowSequence() { return beginSequence(); }

inline bool Input::preflightFlowElement(unsigned index, void *&SaveInfo) {
  if (EC)
    return false;
  if (SequenceHNode *SQ = dyn_cast<SequenceHNode>(CurrentNode)) {
    SaveInfo = CurrentNode;
...
inline void Input::postflightFlowElement(void *SaveInfo) {
  CurrentNode = (HNode *)(SaveInfo);
}

inline void Input::endFlowSequence() {}

inline void Input::beginEnumScalar() { ScalarMatchFound = false; }

inline bool Input::matchEnumScalar(const char *Str, bool) {
  if (ScalarMatchFound)
    return false;
  if (ScalarHNode *SN = dyn_cast<ScalarHNode>(CurrentNode)) {
...
inline bool Input::matchEnumFallback() {
  if (ScalarMatchFound)
    return false;
  ScalarMatchFound = true;
  return true;
...
inline void Input::endEnumScalar() {
  if (!ScalarMatchFound) {
    setError(CurrentNode, "unknown enumerated scalar");
  }
}

inline bool Input::beginBitSetScalar(bool &DoClear) {
  BitValuesUsed.clear();
  if (SequenceHNode *SQ = dyn_cast<SequenceHNode>(CurrentNode)) {
...
inline bool Input::bitSetMatch(const char *Str, bool) {
  if (EC)
    return false;
  if (SequenceHNode *SQ = dyn_cast<SequenceHNode>(CurrentNode)) {
    unsigned Index = 0;
...
inline void Input::endBitSetScalar() {
  if (EC)
    return;
  if (SequenceHNode *SQ = dyn_cast<SequenceHNode>(CurrentNode)) {
...
inline void Input::scalarString(StringRef &S, QuotingType) {
  if (ScalarHNode *SN = dyn_cast<ScalarHNode>(CurrentNode)) {
    S = SN->value();
  } else {
...
inline void Input::blockScalarString(StringRef &S) {
  scalarString(S, QuotingType::None);
}

inline void Input::scalarTag(SmallVectorImpl<char> &Tag) {
  auto VTag = CurrentNode->_node->getVerbatimTag();
  Tag.assign(VTag.begin(), VTag.end());
}

inline void Input::setError(HNode *hnode, const Twine &message) {
  assert(hnode && "HNode must not be NULL");
  setError(hnode->_node, message);
}

inline NodeKind Input::getNodeKind() {
...
inline void Input::setError(Node *node, const Twine &message) {
  Strm->printError(node, message);
  EC = EINVAL;
}

inline void Input::setError(const SMRange &range, const Twine &message) {
  Strm->printError(range, message);
  EC = EINVAL;
}

inline void Input::reportWarning(HNode *hnode, const Twine &message) {
  assert(hnode && "HNode must not be NULL");
  Strm->printError(hnode->_node, message, SourceMgr::DK_Warning);
}

inline void Input::reportWarning(Node *node, const Twine &message) {
  Strm->printError(node, message, SourceMgr::DK_Warning);
}

inline void Input::reportWarning(const SMRange &range, const Twine &message) {
  Strm->printError(range, message, SourceMgr::DK_Warning);
}

inline void Input::releaseHNodeBuffers() {
...
inline Input::HNode *Input::createHNodes(Node *N) {
...
inline void Input::setError(const Twine &Message) {
  setError(CurrentNode, Message);
}

inline void Input::setAllowUnknownKeys(bool Allow) { AllowUnknownKeys = Allow; }

inline bool Input::canElideEmptySequence() { return false; }

//===----------------------------------------------------------------------===//
// Inline implementations - Output
//===----------------------------------------------------------------------===//

inline Output::Output(raw_ostream &yout, void *context, int WrapColumn)
    : IO(context), Out(yout), WrapColumn(WrapColumn) {}

inline Output::~Output() = default;

inline bool Output::outputting() const { return true; }

inline void Output::beginMapping() {
  StateStack.push_back(inMapFirstKey);
  PaddingBeforeContainer = Padding;
  Padding = "\n";
}

inline bool Output::mapTag(StringRef Tag, bool Use) {
  if (Use) {
...
inline SmallVector<StringRef, 16> Output::keys() {
  report_fatal_error("invalid call");
}

inline bool Output::preflightKey(const char *Key, bool Required,
...
inline void Output::postflightKey(void *) {
  if (StateStack.back() == inMapFirstKey) {
    StateStack.pop_back();
    StateStack.push_back(inMapOtherKey);
...
inline void Output::beginFlowMapping() {
...
inline void Output::endFlowMapping() {
  StateStack.pop_back();
  outputUpToEndOfLine(" }");
}

inline void Output::beginDocuments() { outputUpToEndOfLine("---"); }

inline bool Output::preflightDocument(unsigned index) {
  if (index > 0)
    outputUpToEndOfLine("\n---");
  return true;
}

inline void Output::postflightDocument() {}

inline void Output::endDocuments() { output("\n...\n"); }

inline unsigned Output::beginSequence() {
...
inline void Output::endSequence() {
...
inline bool Output::preflightElement(unsigned, void *&SaveInfo) {
  SaveInfo = 0;
  return true;
}

inline void Output::postflightElement(void *) {
  if (StateStack.back() == inSeqFirstElement) {
    StateStack.pop_back();
...
inline unsigned Output::beginFlowSequence() {
...
inline void Output::endFlowSequence() {
  StateStack.pop_back();
  outputUpToEndOfLine(" ]");
}

inline bool Output::preflightFlowElement(unsigned, void *&SaveInfo) {
...
inline void Output::postflightFlowElement(void *) {
  NeedFlowSequenceComma = true;
}

inline void Output::beginEnumScalar() { EnumerationMatchFound = false; }

inline bool Output::matchEnumScalar(const char *Str, bool Match) {
...
inline bool Output::matchEnumFallback() {
...
inline void Output::endEnumScalar() {
  if (!EnumerationMatchFound)
    llvm_unreachable("bad runtime enum value");
}

inline bool Output::beginBitSetScalar(bool &DoClear) {
  newLineCheck();
  output("[ ");
...
inline bool Output::bitSetMatch(const char *Str, bool Matches) {
  if (Matches) {
...
inline void Output::endBitSetScalar() { outputUpToEndOfLine(" ]"); }

inline void Output::scalarString(StringRef &S, QuotingType MustQuote) {
  newLineCheck();
  if (S.empty()) {
    outputUpToEndOfLine("''");
    return;
...
inline void Output::blockScalarString(StringRef &S) {
  if (!StateStack.empty())
    newLineCheck();
  output(" |");
  outputNewLine();
...
inline void Output::scalarTag(SmallVectorImpl<char> &Tag) {
  if (Tag.empty())
    return;
  newLineCheck();
  output(StringRef(Tag.data(), Tag.size()));
  output(" ");
}

inline void Output::setError(const Twine &message) {}

inline bool Output::canElideEmptySequence() {
...
inline void Output::output(StringRef s) {
  Column += s.size();
  Out << s;
}

inline void Output::outputUpToEndOfLine(StringRef s) {
  output(s);
...
inline void Output::outputNewLine() {
  Out << "\n";
  Column = 0;
}

inline void Output::newLineCheck(bool EmptySequence) {
  if (Padding != "\n") {
    output(Padding);
    Padding = {};
    return;
...
inline void Output::paddedKey(StringRef key) {
  output(key);
  output(":");
  const char *spaces = "                ";
...
inline void Output::flowKey(StringRef Key) {
...
inline NodeKind Output::getNodeKind() { report_fatal_error("invalid call"); }

inline bool Output::inSeqAnyElement(InState State) {
  return State == inSeqFirstElement || State == inSeqOtherElement;
}

inline bool Output::inFlowSeqAnyElement(InState State) {
  return State == inFlowSeqFirstElement || State == inFlowSeqOtherElement;
}

inline bool Output::inMapAnyKey(InState State) {
  return State == inMapFirstKey || State == inMapOtherKey;
}

inline bool Output::inFlowMapAnyKey(InState State) {
  return State == inFlowMapFirstKey || State == inFlowMapOtherKey;
}

//===----------------------------------------------------------------------===//
// Inline implementations - ScalarTraits for built-in types
//===----------------------------------------------------------------------===//

inline void ScalarTraits<bool>::output(const bool &Val, void *,
                                       raw_ostream &Out) {
  Out << (Val ? "true" : "false");
}

inline StringRef ScalarTraits<bool>::input(StringRef Scalar, void *,
                                           bool &Val) {
  int Parsed = parseBool(Scalar);
  if (Parsed >= 0) {
    Val = Parsed != 0;
    return StringRef();
  }
...
inline void ScalarTraits<StringRef>::output(const StringRef &Val, void *,
                                            raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<StringRef>::input(StringRef Scalar, void *,
                                                StringRef &Val) {
  Val = Scalar;
  return StringRef();
}

inline void ScalarTraits<uint8_t>::output(const uint8_t &Val, void *,
                                          raw_ostream &Out) {
  uint32_t Num = Val;
  Out << Num;
}

inline StringRef ScalarTraits<uint8_t>::input(StringRef Scalar, void *,
                                              uint8_t &Val) {
...
inline void ScalarTraits<uint16_t>::output(const uint16_t &Val, void *,
                                           raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<uint16_t>::input(StringRef Scalar, void *,
                                               uint16_t &Val) {
  unsigned long long n;
  if (getAsUnsignedInteger(Scalar, 0, n))
    return "invalid number";
...
inline void ScalarTraits<uint32_t>::output(const uint32_t &Val, void *,
                                           raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<uint32_t>::input(StringRef Scalar, void *,
                                               uint32_t &Val) {
  unsigned long long n;
  if (getAsUnsignedInteger(Scalar, 0, n))
    return "invalid number";
...
inline void ScalarTraits<uint64_t>::output(const uint64_t &Val, void *,
                                           raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<uint64_t>::input(StringRef Scalar, void *,
                                               uint64_t &Val) {
  unsigned long long N;
  if (getAsUnsignedInteger(Scalar, 0, N))
    return "invalid number";
...
inline void ScalarTraits<int8_t>::output(const int8_t &Val, void *,
                                         raw_ostream &Out) {
  int32_t Num = Val;
  Out << Num;
}

inline StringRef ScalarTraits<int8_t>::input(StringRef Scalar, void *,
                                             int8_t &Val) {
  long long N;
...
inline void ScalarTraits<int16_t>::output(const int16_t &Val, void *,
                                          raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<int16_t>::input(StringRef Scalar, void *,
                                              int16_t &Val) {
  long long N;
  if (getAsSignedInteger(Scalar, 0, N))
    return "invalid number";
...
inline void ScalarTraits<int32_t>::output(const int32_t &Val, void *,
                                          raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<int32_t>::input(StringRef Scalar, void *,
                                              int32_t &Val) {
  long long N;
  if (getAsSignedInteger(Scalar, 0, N))
    return "invalid number";
...
inline void ScalarTraits<int64_t>::output(const int64_t &Val, void *,
                                          raw_ostream &Out) {
  Out << Val;
}

inline StringRef ScalarTraits<int64_t>::input(StringRef Scalar, void *,
                                              int64_t &Val) {
  long long N;
  if (getAsSignedInteger(Scalar, 0, N))
    return "invalid number";
...
inline void ScalarTraits<double>::output(const double &Val, void *,
                                         raw_ostream &Out) {
  Out << format("%g", Val);
}

inline StringRef ScalarTraits<double>::input(StringRef Scalar, void *,
                                             double &Val) {
...
inline void ScalarTraits<float>::output(const float &Val, void *,
                                        raw_ostream &Out) {
  Out << format("%g", Val);
}

inline StringRef ScalarTraits<float>::input(StringRef Scalar, void *,
                                            float &Val) {
  if (to_float(Scalar, Val))
    return StringRef();
  return "invalid floating point number";
...
inline void ScalarTraits<Hex8>::output(const Hex8 &Val, void *,
                                       raw_ostream &Out) {
  Out << format("0x%" PRIX8, (uint8_t)Val);
}

inline StringRef ScalarTraits<Hex8>::input(StringRef Scalar, void *,
                                           Hex8 &Val) {
  unsigned long long n;
  if (getAsUnsignedInteger(Scalar, 0, n))
...
inline void ScalarTraits<Hex16>::output(const Hex16 &Val, void *,
                                        raw_ostream &Out) {
  Out << format("0x%" PRIX16, (uint16_t)Val);
}

inline StringRef ScalarTraits<Hex16>::input(StringRef Scalar, void *,
                                            Hex16 &Val) {
  unsigned long long n;
  if (getAsUnsignedInteger(Scalar, 0, n))
...
inline void ScalarTraits<Hex32>::output(const Hex32 &Val, void *,
                                        raw_ostream &Out) {
  Out << format("0x%" PRIX32, (uint32_t)Val);
}

inline StringRef ScalarTraits<Hex32>::input(StringRef Scalar, void *,
                                            Hex32 &Val) {
  unsigned long long n;
  if (getAsUnsignedInteger(Scalar, 0, n))
...
inline void ScalarTraits<Hex64>::output(const Hex64 &Val, void *,
                                        raw_ostream &Out) {
  Out << format("0x%" PRIX64, (uint64_t)Val);
}

inline StringRef ScalarTraits<Hex64>::input(StringRef Scalar, void *,
                                            Hex64 &Val) {
...
inline void ScalarTraits<VersionTuple>::output(const VersionTuple &Val, void *,
                                               llvm::raw_ostream &Out) {
  Out << Val.getAsString();
}

inline StringRef ScalarTraits<VersionTuple>::input(StringRef Scalar, void *,
                                                   VersionTuple &Val) {
