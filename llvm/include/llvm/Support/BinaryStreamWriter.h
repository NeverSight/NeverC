...
public:
  ...
  explicit inline BinaryStreamWriter(WritableBinaryStreamRef Ref)
      : Stream(Ref) {}
  explicit inline BinaryStreamWriter(WritableBinaryStream &Stream)
      : Stream(Stream) {}
  explicit inline BinaryStreamWriter(MutableArrayRef<uint8_t> Data,
  ...
  inline Error writeBytes(ArrayRef<uint8_t> Buffer) {
  ...
  /// \returns a success error code if the data was successfully written,
  /// otherwise returns an appropriate error code.
  inline Error writeULEB128(uint64_t Value) {
  ...
  /// \returns a success error code if the data was successfully written,
  /// otherwise returns an appropriate error code.
  inline Error writeSLEB128(int64_t Value) {
    uint8_t EncodedBytes[10] = {0};
  ...
  /// \returns a success error code if the data was successfully written,
  /// otherwise returns an appropriate error code.
  inline Error writeCString(StringRef Str) {
  ...
  /// \returns a success error code if the data was successfully written,
  /// otherwise returns an appropriate error code.
  inline Error writeFixedString(StringRef Str) {
  ...
  /// \returns a success error code if the data was successfully written,
  /// otherwise returns an appropriate error code.
  inline Error writeStreamRef(BinaryStreamRef Ref) {
  ...
  /// \returns a success error code if the data was successfully written,
  /// otherwise returns an appropriate error code.
  inline Error writeStreamRef(BinaryStreamRef Ref, uint64_t Size) {
    BinaryStreamReader SrcReader(Ref.slice(0, Size));
  ...
  inline void split(uint64_t Off, BinaryStreamWriter &First,
                    BinaryStreamWriter &Second) const {
    assert(getLength() >= Off);
    WritableBinaryStreamRef R = Stream.drop_front(Offset);
    WritableBinaryStreamRef S = R.drop_front(Off);
    R = R.keep_front(Off);
    First = BinaryStreamWriter{R};
    Second = BinaryStreamWriter{S};
  }
  ...
  inline Error padToAlignment(uint32_t Align) {
    uint64_t NewOffset = alignTo(Offset, Align);
    const uint64_t ZerosSize = 64;
    static const char Zeros[ZerosSize] = {};
