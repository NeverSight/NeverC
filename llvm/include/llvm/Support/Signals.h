...
//
// Do not include FileSystem.h here: FileSystem.h → Error.h → Debug.h →
// Signals.h while FileSystem's namespace fs is still open, so the include is
// skipped and fs appears empty. Symbolization helpers live in
// lib/CSupport/SignalsSymbolize.inc.
//
// Command-line registration for signal options lives in support_cpp.cpp so that
// this header never pulls in CommandLine.h (which includes lcommand/VFS and can
// recurse while SourceMgr.h is still parsing MemoryBuffer → FileSystem → …).
...
inline static const char DisableSymbolizationEnv[] =
    "LLVM_DISABLE_SYMBOLIZATION";
inline static const char LLVMSymbolizerPathEnv[] = "LLVM_SYMBOLIZER_PATH";
inline static const char EnableSymbolizerMarkupEnv[] =
    "LLVM_ENABLE_SYMBOLIZER_MARKUP";

/* CallbackAndCookie/RunSignalHandlers/insertSignalHandler moved to
 * CSupport/Signals.c →
 * csupport_run_signal_callbacks/csupport_insert_signal_callback */
inline void sys::RunSignalHandlers() { csupport_run_signal_callbacks(); }

inline static void insertSignalHandler(sys::SignalHandlerCallback FnPtr,
                                       void *Cookie) {
