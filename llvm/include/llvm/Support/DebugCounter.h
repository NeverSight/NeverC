...
inline void initDebugCounterOptions() { (void)DebugCounter::instance(); }

inline DebugCounter &DebugCounter::instance() {
  static detail::DebugCounterOwner O;
  return O.DC;
}

inline void DebugCounter::push_back(StringRef Val) {
  if (Val.empty())
    return;
  ...
  if (CounterPair.first.ends_with("-skip")) {
    ...
    unsigned CounterID = getCounterId(CounterName.str());
    if (!CounterID) {
...
inline void DebugCounter::print(raw_ostream &OS) const {
...
inline LLVM_DUMP_METHOD void DebugCounter::dump() const { print(dbgs()); }
