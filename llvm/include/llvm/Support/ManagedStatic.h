...
namespace detail {
inline const ManagedStaticBase *&getStaticList() {
...
inline void
ManagedStaticBase::RegisterManagedStatic(void *(*Creator)(),
...
inline void ManagedStaticBase::destroy() const {
  assert(DeleterFn && "ManagedStatic not initialized correctly!");
  assert(detail::getStaticList() == this &&
         "Not destroyed in reverse order of construction?");
  detail::getStaticList() = Next;
  Next = nullptr;
  DeleterFn(Ptr);
  Ptr = nullptr;
  DeleterFn = nullptr;
...
inline void llvm_shutdown() {
  while (detail::getStaticList())
    detail::getStaticList()->destroy();
}
