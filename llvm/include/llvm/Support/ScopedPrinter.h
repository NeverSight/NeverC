...
inline raw_ostream &operator<<(raw_ostream &OS, const HexNumber &Value) {
  OS << "0x" << utohexstr(Value.Value);
  return OS;
}

inline void ScopedPrinter::printBinaryImpl(StringRef Label, StringRef Str,
  ...
  if (Block) {
    ...
    if (!Str.empty())
      OS << " " << Str;
    OS << " (" << format_bytes(Data, {}, Data.size(), 1, 0, true) << ")\n";
  }
}

inline JSONScopedPrinter::JSONScopedPrinter(
