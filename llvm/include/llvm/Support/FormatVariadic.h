...
protected:
  ...
  struct ReplacementAndTail {
    ReplacementItem first;
    StringRef second;
  };
  static ReplacementAndTail splitLiteralAndReplacement(StringRef Fmt);
...
public:
  ...
  /// Returns ReplacementItem with Type==Empty on failure.
  static ReplacementItem parseReplacementItem(StringRef Spec);
...
/* twine_* bridge definitions live in llvm/lib/CSupport/support_cpp.cpp
 * (declared in llvm/ADT/TwineFormatv.h) so TUs that include Twine.h without
 * pulling this header still link. */
...
namespace detail {
inline void format_adapter::anchor() {}
} // namespace detail

namespace {
inline int translateLocCharFV(char C) {
  switch (C) {
  case '-':
    return (int)AlignStyle::Left;
  case '=':
    return (int)AlignStyle::Center;
  case '+':
    return (int)AlignStyle::Right;
  default:
    return -1;
  }
}
...
inline bool formatv_object_base::consumeFieldLayout(StringRef &Spec,
                                                    AlignStyle &Where,
                                                    size_t &Align, char &Pad) {
  ...
  if (Spec.empty())
    return true;
  if (Spec.size() > 1) {
    int Loc = translateLocCharFV(Spec[1]);
    if (Loc >= 0) {
      Pad = Spec[0];
      Where = (AlignStyle)Loc;
      Spec = Spec.drop_front(2);
    } else {
      Loc = translateLocCharFV(Spec[0]);
      if (Loc >= 0) {
        Where = (AlignStyle)Loc;
...
inline formatv_object_base::ReplacementAndTail
formatv_object_base::splitLiteralAndReplacement(StringRef Fmt) {
  while (!Fmt.empty()) {
    ...
    if (Braces.size() > 1) {
      ...
      return {ReplacementItem{Fmt.take_front(NumEscapedBraces)},
              Fmt.drop_front(NumEscapedBraces * 2)};
    }
    size_t BC = Fmt.find_first_of('}');
    if (BC == StringRef::npos) {
      assert(false && "Unterminated brace sequence.");
      return {ReplacementItem{Fmt}, StringRef()};
    }
...
inline SmallVector<ReplacementItem, 2>
formatv_object_base::parseFormatString(StringRef Fmt) {
  SmallVector<ReplacementItem, 2> Replacements;
  ReplacementItem I;
  while (!Fmt.empty()) {
    auto sr = splitLiteralAndReplacement(Fmt);
    I = sr.first;
    Fmt = sr.second;
...
namespace llvm {
inline raw_ostream &raw_ostream::operator<<(const formatv_object_base &Obj) {
  Obj.format(*this);
  return *this;
}
} // namespace llvm
