...
namespace llvm {
inline uint32_t crc32(uint32_t C, ArrayRef<uint8_t> D) {
  return csupport_crc32(C, D.data(), D.size());
}
inline uint32_t crc32(ArrayRef<uint8_t> D) {
  return csupport_crc32_initial(D.data(), D.size());
}
inline void JamCRC::update(ArrayRef<uint8_t> D) {
  CRC ^= 0xFFFFFFFFU;
  CRC = csupport_crc32(CRC, D.data(), D.size());
  CRC ^= 0xFFFFFFFFU;
}
} // end namespace llvm
