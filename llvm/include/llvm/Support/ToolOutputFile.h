...
class ToolOutputFile {
  ...
  public:
    ...
    explicit inline CleanupInstaller(StringRef Filename)
        : Filename(Filename), Keep(false) {
      if (Filename != "-")
        sys::RemoveFileOnSignal(Filename);
    }
    inline ~CleanupInstaller() {
      if (Filename == "-")
        return;
      if (!Keep)
...
public:
  ...
  /// constructor.
  inline ToolOutputFile(StringRef Filename, std::error_code &EC,
  ...
  inline ToolOutputFile(StringRef Filename, int FD) : Installer(Filename) {
  ...
  StringRef outputFilename() { return Installer.Filename; }
};
