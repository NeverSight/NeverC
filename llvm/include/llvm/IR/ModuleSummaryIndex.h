...
struct alignas(8) GlobalValueSummaryInfo {
  union NameOrGV {
    ...
    /// This is guaranteed to happen until runLTOBackend function is
    /// called, so it is safe to use this field during LTO link. This field
...
struct CallsiteInfo {
  ...
  // The LTO backend will need to create as many clones as there are entries
...
public:
  ...
  /// optimization. Will be applied in the LTO backends.
...
/// by means of internalizing such variables after LTO import.
...
private:
  ...
  /// module. Flag is suppose to be set by distributed LTO indexing
  ...
  // FIXME: Putting this in the distributed LTO index files breaks LTO
...
public:
  ...
  GlobalValueSummary *findSummaryInModule(ValueInfo VI,
    ...
    // NeverC: ValueInfo can be default-constructed / dangling on the
    // LTO path (observed during Redis -flto link).  Dereferencing
    // its internal pointer-int-pair to walk SummaryList would crash.
    // Treat an empty VI as "no summary" so the caller can fall through
    // to its conservative-default path.
    if (!VI)
      return nullptr;
  ...
  /// because it is possible in certain clients (not NeverC at the moment) for
  /// two rounds of LTO optimization and therefore promotion to occur.
