...
// machine/endian.h (via ADT/bit.h) pulls in mach/machine.h, whose macros
// collide with the LLVM Mach-O enum constant names below.
...
#if defined(__APPLE__)
// System <mach-o/loader.h> defines LC_*/MH_*/FAT_* macros that collide with the
// LLVM enumerator names in this header when Support headers pull in the system
// header first (e.g. MemoryBuffer.h -> FileSystem.h -> lpath.h).
//
// Order matters: include the SDK headers FIRST, then run the undef sweep.  An
// undef pass before the SDK include is a no-op because the SDK header re-
// defines every name afterwards.  macOS 15.5+ also routes <mach/machine.h>
// transitively through <mach-o/loader.h>, so even tooling that does not list
// loader.h directly still sees the macro pollution unless the undef sweep
// runs after the actual definitions land in the preprocessor's macro table.
#include <mach-o/fat.h>
#include <mach-o/loader.h>
#include "llvm/BinaryFormat/DarwinLoaderUndef.inc"
#endif
...
enum : uint32_t {
  ...
  /// S_ATTR_SELF_MODIFYING_CODE - Used with code stubs written on by dyld.
...
enum { TOOL_CLANG = 1, TOOL_LD = 3, TOOL_LLD = 4 };
...
struct dylib_module {
  ...
  uint32_t reserved_module_info_addr;
  uint32_t reserved_module_info_size;
};
...
struct dylib_module_64 {
  ...
  uint32_t ninit_nterm;
  uint32_t reserved_module_info_size;
  uint64_t reserved_module_info_addr;
};
...
enum CPUSubTypeX86 { CPU_SUBTYPE_X86_64_ALL = 3, CPU_SUBTYPE_X86_64_H = 8 };
...
enum ARMThreadFlavors { ARM_THREAD_STATE64 = 6, ARM_EXCEPTION_STATE64 = 7 };
