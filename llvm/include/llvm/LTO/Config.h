...
struct Config {
  ...
  /// When true, the LTO optimization pipeline only runs module simplification
  ...
  /// Disable entirely the optimizer.
  ...
  /// A task represents the entire pipeline for regular (non-parallel) LTO,
  /// but a parallel code generation task will be split into
