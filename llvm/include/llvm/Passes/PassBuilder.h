...
public:
  ...
  // NeverC: when true, drop a handful of marginal-cost IPO passes from the
  // module simplification pipeline:
  //   - CalledValuePropagation (~15% of LTO simplification time, used only
  //     for indirect-call metadata that benefits virtual-call-heavy code)
  //   - The second GlobalOpt run (~6%, mostly redundant with the first run
  //     plus the LTO post-link cleanup that the ParallelOpt path runs)
  // The LTO ParallelOpt hook sets this to true by default because it runs
  // its own GlobalDCE+GlobalOpt+ConstantMerge cleanup before partitioning,
  ...
  // skipped (loop passes, DSE, MemCpyOpt) are still executed afterwards
  // because the LTO ParallelOpt path runs the full
  // buildModuleOptimizationPipeline per partition in parallel.  Removing
...
public:
  ...
  ///
  /// This builds a pipeline that runs the LTO pre-link pipeline, and
  ...
  /// pipeline. This does not apply to 'backend' compiles (LTO
  /// link-time pipelines).
...
private:
  ...
  void addPGOInstrPasses(ModulePassManager &MPM, OptimizationLevel Level,
                         ...
                         bool AtomicCounterUpdate, StringRef ProfileFile,
                         IntrusiveRefCntPtr<vfs::FileSystem> FS);
