...
inline void Path::replaceRoot(void *Root, unsigned Size, IdxPair Offsets) {
...
inline NodeRef Path::getLeftSibling(unsigned Level) const {
  if (Level == 0)
    return NodeRef();
  unsigned l = Level - 1;
...
inline void Path::moveLeft(unsigned Level) {
...
inline NodeRef Path::getRightSibling(unsigned Level) const {
  if (Level == 0)
    return NodeRef();
  unsigned l = Level - 1;
...
inline void Path::moveRight(unsigned Level) {
  assert(Level != 0 && "Cannot move the root node");
  unsigned l = Level - 1;
  while (l && atLastEntry(l))
    --l;
...
inline IdxPair distribute(unsigned Nodes, unsigned Elements, unsigned Capacity,
                          const unsigned *CurSize, unsigned NewSize[],
                          unsigned Position, bool Grow) {
  ...
  if (Grow) {
    ...
    assert(NewSize[PosPair.first] && "Too few elements");
