...
bool getAsIntegerWrapper(StringRef Str, unsigned Radix,
                         unsigned long long &Result);
...
public:
  ...
  [[nodiscard]] SmallString<256> lower() const;

  /// Convert the given ASCII string to uppercase.
  [[nodiscard]] SmallString<256> upper() const;
...
#ifndef _MSC_VER
inline const size_t StringRef::npos;
#endif

inline int StringRef::compare_insensitive(StringRef RHS) const {
  return csupport_str_compare_insensitive(Data, Length, RHS.Data, RHS.Length);
}

inline bool StringRef::starts_with_insensitive(StringRef Prefix) const {
  return csupport_str_starts_with_insensitive(Data, Length, Prefix.Data,
                                              Prefix.Length);
}

inline bool StringRef::ends_with_insensitive(StringRef Suffix) const {
  return csupport_str_ends_with_insensitive(Data, Length, Suffix.Data,
                                            Suffix.Length);
}

inline size_t StringRef::find_insensitive(char C, size_t From) const {
  return csupport_str_find_char_insensitive(Data, Length, C, From);
}

/// compare_numeric - Compare strings, handle embedded numbers.
inline int StringRef::compare_numeric(StringRef RHS) const {
  return csupport_str_compare_numeric(Data, Length, RHS.Data, RHS.Length);
}

// Compute the edit distance between the two given strings.
inline unsigned StringRef::edit_distance(StringRef Other,
                                         bool AllowReplacements,
                                         unsigned MaxEditDistance) const {
  return csupport_cl_edit_distance(data(), size(), Other.data(), Other.size(),
                                   AllowReplacements ? 1 : 0, MaxEditDistance);
}

inline unsigned
StringRef::edit_distance_insensitive(StringRef Other, bool AllowReplacements,
                                     unsigned MaxEditDistance) const {
  return csupport_cl_edit_distance_insensitive(
      data(), size(), Other.data(), Other.size(), AllowReplacements ? 1 : 0,
      MaxEditDistance);
}
...
inline size_t StringRef::find(StringRef Str, size_t From) const {
  return csupport_str_find_substr(Data, Length, Str.data(), Str.size(), From);
}

inline size_t StringRef::find_insensitive(StringRef Str, size_t From) const {
  return csupport_str_find_insensitive(Data, Length, Str.data(), Str.size(),
                                       From);
}

inline size_t StringRef::rfind_insensitive(char C, size_t From) const {
  return csupport_str_rfind_char_insensitive(Data, Length, C, From);
}

inline size_t StringRef::rfind(StringRef Str) const {
  return csupport_str_rfind(Data, Length, Str.data(), Str.size());
}

inline size_t StringRef::rfind_insensitive(StringRef Str) const {
  return csupport_str_rfind_insensitive(Data, Length, Str.data(), Str.size());
}
...
inline StringRef::size_type StringRef::find_first_of(StringRef Chars,
                                                     size_t From) const {
  return csupport_str_find_first_of(Data, Length, Chars.data(), Chars.size(),
                                    From);
}

inline StringRef::size_type StringRef::find_first_not_of(char C,
                                                         size_t From) const {
  return csupport_str_find_first_not_of_char(Data, Length, C, From);
}
...
///
/// Note: O(size() + Chars.size())
inline StringRef::size_type StringRef::find_first_not_of(StringRef Chars,
                                                         size_t From) const {
  return csupport_str_find_first_not_of(Data, Length, Chars.data(),
                                        Chars.size(), From);
}

inline StringRef::size_type StringRef::find_last_of(StringRef Chars,
                                                    size_t From) const {
  return csupport_str_find_last_of(Data, Length, Chars.data(), Chars.size(),
                                   From);
}

inline StringRef::size_type StringRef::find_last_not_of(char C,
                                                        size_t From) const {
  return csupport_str_find_last_not_of_char(Data, Length, C, From);
}

inline StringRef::size_type StringRef::find_last_not_of(StringRef Chars,
                                                        size_t From) const {
  return csupport_str_find_last_not_of(Data, Length, Chars.data(), Chars.size(),
                                       From);
}

inline void StringRef::split(SmallVectorImpl<StringRef> &A, StringRef Separator,
...
inline void StringRef::split(SmallVectorImpl<StringRef> &A, char Separator,
                             int MaxSplit, bool KeepEmpty) const {
  StringRef S = *this;
...
/// the string.
inline size_t StringRef::count(StringRef Str) const {
  return csupport_str_count(Data, Length, Str.data(), Str.size());
}

inline bool consumeUnsignedInteger(StringRef &Str, unsigned Radix,
                                   unsigned long long &Result) {
  const char *s = Str.data();
  size_t len = Str.size();
  if (csupport_consume_unsigned(&s, &len, Radix, &Result))
    return true;
  Str = StringRef(s, len);
  return false;
}

inline bool consumeSignedInteger(StringRef &Str, unsigned Radix,
                                 long long &Result) {
  const char *s = Str.data();
  size_t len = Str.size();
  if (csupport_consume_signed(&s, &len, Radix, &Result))
    return true;
  Str = StringRef(s, len);
  return false;
}
...
inline bool getAsUnsignedInteger(StringRef Str, unsigned Radix,
                                 unsigned long long &Result) {
...
inline bool getAsSignedInteger(StringRef Str, unsigned Radix,
                               long long &Result) {
...
inline bool getAsIntegerWrapper(StringRef Str, unsigned Radix,
                                unsigned long long &Result) {
  if (Str[0] == '-') {
    long long SignedResult = 0;
    auto Ret = getAsSignedInteger(Str, Radix, SignedResult);
    Result = (uint64_t)(SignedResult);
    return Ret;
  } else {
