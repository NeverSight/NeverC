...
inline FPClassTest fneg(FPClassTest Mask) {
  return (FPClassTest)(csupport_fneg_fpclass((uint32_t)(Mask)));
}

/// Return the test mask which returns true after fabs is applied to the value.
inline FPClassTest inverse_fabs(FPClassTest Mask) {
  return (FPClassTest)(csupport_inverse_fabs_fpclass((uint32_t)(Mask)));
}

/// Return the test mask which returns true if the value could have the same set
/// of classes, but with a different sign.
inline FPClassTest unknown_sign(FPClassTest Mask) {
  return (FPClassTest)(csupport_unknown_sign_fpclass((uint32_t)(Mask)));
}

/// Write a human readable form of \p Mask to \p OS
inline raw_ostream &operator<<(raw_ostream &OS, FPClassTest Mask) {
  struct FPClassNameEntry {
    FPClassTest first;
    const char *second;
  };
  static const FPClassNameEntry Names[] = {
  ...
  for (const auto &Entry : Names) {
    if ((Mask & Entry.first) == Entry.first) {
      OS << Sep << Entry.second;
      Sep = " ";
      Mask &= ~Entry.first;
    }
