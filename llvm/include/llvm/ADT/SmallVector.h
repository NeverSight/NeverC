...
#include "csupport/lsmall_lvector.h"
...
inline void *
SmallVectorBase<Size_T>::replaceAllocation(void *NewElts, size_t TSize,
                                           size_t NewCapacity, size_t VSize) {
  return csupport_smallvec_replace_alloc(NewElts, TSize, NewCapacity, VSize);
}
template <class Size_T>
inline void *
SmallVectorBase<Size_T>::mallocForGrow(void *FirstEl, size_t MinSize,
                                       size_t TSize, size_t &NewCapacity) {
  if (sizeof(Size_T) <= 4) {
    return csupport_smallvec_malloc_for_grow_u32(
        this->BeginX, FirstEl, MinSize, TSize, (uint32_t)(this->Capacity),
        &NewCapacity);
  } else {
#if SIZE_MAX > UINT32_MAX
    return csupport_smallvec_malloc_for_grow_u64(
...
template <class Size_T>
inline void SmallVectorBase<Size_T>::grow_pod(void *FirstEl, size_t MinSize,
                                              size_t TSize) {
  if (sizeof(Size_T) <= 4) {
    uint32_t cap = (uint32_t)(this->Capacity);
    uint32_t sz = (uint32_t)(this->Size);
    csupport_smallvec_grow_pod_u32(&this->BeginX, &cap, sz, FirstEl, MinSize,
                                   TSize);
    this->Capacity = (Size_T)(cap);
  } else {
#if SIZE_MAX > UINT32_MAX
    uint64_t cap = (uint64_t)(this->Capacity);
    uint64_t sz = (uint64_t)(this->Size);
    csupport_smallvec_grow_pod_u64(&this->BeginX, &cap, sz, FirstEl, MinSize,
                                   TSize);
    this->Capacity = (Size_T)(cap);
#endif
  }
