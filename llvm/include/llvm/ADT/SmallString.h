...
// StringRef::{lower,upper} need SmallString<256> complete; StringRef.h only
// forward-declares SmallString.
inline SmallString<256> StringRef::lower() const {
  SmallString<256> result;
  result.resize(size());
  csupport_to_lower(result.data(), data(), size());
  return result;
}

inline SmallString<256> StringRef::upper() const {
  SmallString<256> result;
  result.resize(size());
  csupport_to_upper(result.data(), data(), size());
  return result;
}
