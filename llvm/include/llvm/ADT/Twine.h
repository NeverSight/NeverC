...
#include "llvm/ADT/TwineFormatv.h"
#include <assert.h>
#include <stdint.h>
#include <string>
#include <string_view>
...
public:
  ...
  StringRef getSingleStringRef() const {
    ...
    switch (getLHSKind()) {
    default:
      assert(false && "Out of sync with isSingleStringRef");
      return StringRef();
    case EmptyKind:
      return StringRef();
    case CStringKind:
...
inline std::string Twine::str() const {
  if (LHSKind == StdStringKind && RHSKind == EmptyKind)
    return *LHS.stdString;
  if (LHSKind == FormatvObjectKind && RHSKind == EmptyKind)
    return twine_str_from_formatv(LHS.formatvObject);
  SmallString<256> Vec;
...
Twine::toNullTerminatedStringRef(SmallVectorImpl<char> &Out) const {
  if (isUnary()) {
    ...
    case CStringKind:
      return StringRef(LHS.cString);
    case StdStringKind:
      return StringRef(LHS.stdString->c_str(), LHS.stdString->size());
    case StringLiteralKind:
      return StringRef(LHS.ptrAndLength.ptr, LHS.ptrAndLength.length);
    default:
      break;
