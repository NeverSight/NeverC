...
struct StringRefPair {
  StringRef first;
  StringRef second;
};
StringRefPair getToken(StringRef Source, StringRef Delimiters = " \t\n\v\f\r");
...
SmallString<256> convertToSnakeFromCamelCase(StringRef input);
...
/// letter)
SmallString<256> convertToCamelFromSnakeCase(StringRef input,
...
inline StringRefPair getToken(StringRef Source, StringRef Delimiters) {
  StringRef::size_type Start = Source.find_first_not_of(Delimiters);
  StringRef::size_type End = Source.find_first_of(Delimiters, Start);
  return {Source.slice(Start, End), Source.substr(End)};
}
inline void SplitString(StringRef Source,
                        SmallVectorImpl<StringRef> &OutFragments,
                        StringRef Delimiters) {
  auto S = getToken(Source, Delimiters);
  while (!S.first.empty()) {
    OutFragments.push_back(S.first);
    S = getToken(S.second, Delimiters);
  }
}
inline void printEscapedString(StringRef Name, raw_ostream &Out) {
...
inline void printHTMLEscaped(StringRef String, raw_ostream &Out) {
  for (char C : String) {
    if (C == '&')
      Out << "&amp;";
    else if (C == '<')
...
inline void printLowerCase(StringRef String, raw_ostream &Out) {
