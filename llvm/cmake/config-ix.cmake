...
set(ZLIB_FOUND 0)
if(LLVM_ENABLE_ZLIB)
  if(LLVM_USE_BUNDLED_ZLIB)
    if(NOT TARGET ZLIB::ZLIB)
      message(FATAL_ERROR "Bundled zlib target is unavailable")
    endif()
    set(LLVM_ZLIB_TARGET ZLIB::ZLIB)
    set(ZLIB_FOUND 1)
    set(HAVE_ZLIB 1)
  else()
    ...
    if(ZLIB_FOUND)
      # A package can be discoverable but unusable for the selected
      # architecture.
      cmake_push_check_state()
      list(APPEND CMAKE_REQUIRED_LIBRARIES ZLIB::ZLIB)
      # check_symbol_exists() treats an already-defined result as cached. Use a
      # private probe variable and clear both scopes so a stale HAVE_ZLIB cache
      # entry cannot turn discovery into either a false positive or a skipped
      # check.
      unset(LLVM_HAVE_ZLIB)
      unset(LLVM_HAVE_ZLIB CACHE)
      check_symbol_exists(compress2 zlib.h LLVM_HAVE_ZLIB)
      cmake_pop_check_state()
      set(HAVE_ZLIB "${LLVM_HAVE_ZLIB}")
      set(LLVM_ZLIB_TARGET ZLIB::ZLIB)
    endif()
  endif()
...
if(LLVM_ENABLE_ZSTD)
  if(LLVM_USE_BUNDLED_ZSTD)
    if(NOT TARGET zstd::libzstd_static)
      message(FATAL_ERROR "Bundled zstd target is unavailable")
    endif()
    set(zstd_FOUND 1)
    set(LLVM_ZSTD_TARGET zstd::libzstd_static)
  else()
    set(zstd_FOUND 0)
    ...
    if(LLVM_USE_STATIC_ZSTD)
      if(TARGET zstd::libzstd_static)
        set(LLVM_ZSTD_TARGET zstd::libzstd_static)
      elseif(LLVM_ENABLE_ZSTD STREQUAL FORCE_ON)
        message(FATAL_ERROR
                "Failed to find static zstd while LLVM_ENABLE_ZSTD is FORCE_ON")
      else()
        set(zstd_FOUND 0)
      endif()
    elseif(TARGET zstd::libzstd_shared)
      set(LLVM_ZSTD_TARGET zstd::libzstd_shared)
