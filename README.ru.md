...
Встроенный линкер · Конвейер shellcode · Встроенный тип `string`
...
[Документация](docs/README.ru.md) · [Руководство shellcode](docs/shellcode-compiler/README.ru.md) · [Встроенная строка](docs/builtin-string/README.ru.md)

</div>

---

> **Примечание:** GitHub всегда показывает на главной репозитория `README.md` (английский), без автоопределения языка браузера. Используйте ссылки языков выше; в [документации](docs/README.ru.md) и [руководстве shellcode](docs/shellcode-compiler/README.ru.md) сохраняйте ту же локаль через языковую панель и хлебные крошки.
...
- **[Компилятор shellcode](docs/shellcode-compiler/README.ru.md)** — многоступенчатый конвейер IR/MIR, кроссплатформенное извлечение, разрешение импортов/syscall, режим ядра, аудит запрещённых байт, архитектура плагинов
...
- **[Встроенный тип `string`](docs/builtin-string/README.ru.md)** — строковый тип с семантикой значения, методы через точку, автоматическое управление памятью и нативная UTF-8 поддержка
...
```bash
# Всегда указывайте -target: выбирает ABI целевой ОС/архитектуры, а не хоста.

# macOS arm64
neverc -fshellcode -target arm64-apple-macos -mshellcode-syscall hello.c -o hello.bin

# Linux x86_64 — тот же исходник
...
Подробности: **[индекс документации](docs/README.ru.md)** — дизайн, матрица платформ, справочник CLI, примеры.
...
После размещения splat SDK [xwin](https://github.com/Jake-Shadle/xwin) в `build-neverc/sdk/msvc/`:
...
Shellcode для Windows (`-fshellcode`, разрешение через PEB и т.д.): [документация компилятора shellcode](docs/shellcode-compiler/README.ru.md).
