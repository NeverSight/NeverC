...
// This is the base class of all sections that the linker handles. Some are
// sections in input files, some are sections in the produced output file and
// some exist just as a convenience for implementing special ways of combining
// some sections.
class SectionBase {
public:
...
public:
  ...
  // Used by --optimize-bb-jumps temporarily to indicate the number of bytes
  // which is not counted in the size. This should be reset to zero after uses.
...
// Returns the real InputSection used as the discarded-entry identity for the
// active ELF invocation. Some generic consumers inspect common section
