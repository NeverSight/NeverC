#ifndef LINKER_ELF_SYMBOLS_H
#define LINKER_ELF_SYMBOLS_H
...
public:
  ...
  //
  // NOTE: In the ELF image emitter this field is used to mark local defined
  // symbols which are referenced by relocations when -r or --emit-relocs is
  // given.
  uint8_t used : 1;
...
public:
  ...
  // AArch64 MemtagABI marks symbols that need special relocation and metadata
  // semantics.
...
// --end-lib options. The linker also handles traditional archives as if all the
// files in the archive are surrounded by --start-lib and --end-lib.
//
