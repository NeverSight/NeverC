...
using neverc::analyze_scanf::ScanfConversionSpecifier;
using neverc::analyze_scanf::ScanfSpecifier;
...
typedef neverc::analyze_format_string::SpecifierResult<ScanfSpecifier>
    ScanfSpecifierResult;

namespace {
bool parseScanList(FormatStringHandler &H, ScanfConversionSpecifier &CS,
...

  {
    ...
    if (LLVM_UNLIKELY(!End)) {
      H.HandleIncompleteScanList(start, E - 1);
      return true;
    }
    I = End;
  }
...
ScanfSpecifierResult parseScanfSpecifier(FormatStringHandler &H,
  ...
  using namespace neverc::analyze_scanf;
  const char *I = Beg;
  ...
  const OptionalAmount &Amt = neverc::analyze_format_string::ParseAmount(I, E);
...
ArgType ScanfSpecifier::getArgType(TreeContext &Ctx) const {
...
bool ScanfSpecifier::fixType(QualType QT, QualType RawQT,
                             const LangOptions &LangOpt, TreeContext &Ctx) {
...
bool neverc::analyze_format_string::ParseScanfString(FormatStringHandler &H,
                                                     const char *I,
                                                     const char *E,
                                                     const LangOptions &LO,
                                                     const TargetInfo &Target) {
