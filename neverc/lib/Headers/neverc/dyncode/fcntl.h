/*===---- fcntl.h - NeverC dyncode-oriented fcntl shim ------------------===*\
|*
|* Plain `fcntl.h` shim.  In normal compilation we forward to the system
|* fcntl.h.  In dyncode mode (`__NEVERC_DYNCODE__`) we expose the
...
#error                                                                         \
    "<fcntl.h> is a user-mode header. In -mdyncode-context=kernel mode include <neverc/dyncode/kernel.h> and route file-open semantics through a kernel-side helper (filp_open on Linux, ZwCreateFile on Windows, vnode_open on XNU)."
#endif
 ...
 * different bit assignments, and the dyncode pipeline passes them
