/*===---- neverc/dyncode/kernel.h - kernel-mode dyncode entry header ---===*\
 *
 * Pulled in by user source that wants to write ring-0 dyncode with
 * `-mdyncode-context=kernel`.  This header intentionally does NOT
 ...
 * prepends two hidden parameters to the dyncode entry function:
 *
 *     void dyncode_entry(void *resolver, void *cookie, <user params>...)
 *
 ...
 *     dyncode_entry((void *)my_resolver, (void *)my_cookie);
 *
 ...
 *     dyncode will crash (no error handling in the generated wrapper).
 *
 ...
 *
 * Using `#include <neverc/dyncode/kernel.h>` outside of kernel mode is an
...
#error "<neverc/dyncode/kernel.h> is only meaningful with -fdyncode."
#endif

#if !defined(__NEVERC_DYNCODE_KERNEL__)
#error                                                                         \
    "<neverc/dyncode/kernel.h> requires -mdyncode-context=kernel. For ring-3 payloads include <windows.h> / <unistd.h> / <string.h> instead."
#endif
