/*===---- sys/stat.h - NeverC dyncode-oriented stat shim ----------------===*\
|*
|* Forward to the system `<sys/stat.h>` in normal builds.  In dyncode
|* mode (`__NEVERC_DYNCODE__`) expose the syscall-backed primitives the
...
#error                                                                         \
    "<sys/stat.h> is a user-mode header. In -mdyncode-context=kernel mode include <neverc/dyncode/kernel.h> and call vfs_statx / ZwQueryInformationFile / vnode_getattr directly via the KernelImportPass resolver instead."
#endif
