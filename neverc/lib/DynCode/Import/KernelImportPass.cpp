...
#define NEVERC_KERN_IMPORT_RESERVED_PREFIX(prefix)                             \
  if (Name.starts_with(prefix))                                                \
    return true;
#include "neverc/DynCode/Tables/KernelImportReservedPrefixes.def"
#include "neverc/DynCode/Tables/UserExtra_KernelImportReservedPrefixes.def"
...
PreservedAnalyses KernelImportPass::run(Module &M, ModuleAnalysisManager &) {
  ...
  if (!Entry) {
    errs() << KernelResolverABI::DiagnosticPrefix
           ...
           << defaultEntryNameList() << " or -fdyncode-entry=<name>).\n";
    return PreservedAnalyses::all();
  }
