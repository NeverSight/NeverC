...
const char *machoRelocTypeName(const TargetDesc &Target, uint64_t T) {
  return Target.Arch == DynCodeArch::AArch64 ? arm64RelocTypeName(T)
...
int extractMachO(StringRef InputObj, StringRef OutputBin,
  ...
  if (Obj.getArch() != ExpectedArch) {
    errs() << "dyncode-extractor: Mach-O arch mismatch (got "
  ...
  if (!FoundText || TextData.empty()) {
    errs() << "dyncode-extractor: no __TEXT,__text section found in '"
           << InputObj << "'\n";
    return 1;
  }
  ...
  const bool IsArm64 = Target.Arch == DynCodeArch::AArch64;
  ...
  for (const auto &Reloc : TextSec.relocations()) {
    ...
    if (!IsArm64) {
      auto rejectUnsupported = [&](const char *Reason) {
        ExternalRelocCount++;
        errs() << "dyncode-extractor: unsupported x86_64 Mach-O reloc "
      ...
      case 8: { // X86_64_RELOC_SIGNED_4
        if (TargetAddr < TextAddr || TargetAddr >= TextEnd) {
          ExternalRelocCount++;
          errs() << "dyncode-extractor: pcrel32 target '" << SymName
                 << "' at 0x";
        ...
        if (RelocOffset + 4 > TextBytes.size()) {
          ExternalRelocCount++;
          errs() << "dyncode-extractor: pcrel32 reloc at offset 0x";
          errs().write_hex(RelocOffset);
        ...
        if (Expected < INT32_MIN || Expected > INT32_MAX) {
          ExternalRelocCount++;
          errs() << "dyncode-extractor: pcrel32 out of range at offset 0x";
          errs().write_hex(RelocOffset);
          errs() << " (delta=" << Expected << ")\n";
          continue;
        }
      ...
      case 0: // X86_64_RELOC_UNSIGNED -- absolute address, unsupported.
        rejectUnsupported("absolute 64/32-bit addresses require a loader; "
                          "dyncode must route globals through the stack "
                          "(Data2TextPass) or via resolver parameters");
        continue;
      case 3: // X86_64_RELOC_GOT_LOAD
      case 4: // X86_64_RELOC_GOT
        rejectUnsupported(
            "GOT indirection is not available in dyncode; keep "
      ...
      case 9: // X86_64_RELOC_TLV
        rejectUnsupported(
            ...
            "slot in dyncode; dyncode mode already downgrades "
    ...
    if (TargetAddr < TextAddr || TargetAddr >= TextEnd) {
      ExternalRelocCount++;
      errs() << "dyncode-extractor: relocation target '" << SymName
             << "' at 0x";
      errs().write_hex(TargetAddr);
      errs() << " is outside __text (expected [0x";
      errs().write_hex(TextAddr);
      ...
      if (SymName.starts_with("lCPI") || SymName.starts_with("LCPI"))
        errs() << "dyncode-extractor: hint: the backend spilled a "
    ...
    if (RelocOffset + 4 > TextBytes.size()) {
      ExternalRelocCount++;
      errs() << "dyncode-extractor: relocation at offset 0x";
      errs().write_hex(RelocOffset);
      errs() << " extends past __text end\n";
      continue;
    }
    ...
    case 2: {
      ...
      if (!patchArm64Branch26(TextViewAll, RelocOffset, PCRelOffset)) {
        ExternalRelocCount++;
        errs() << "dyncode-extractor: cannot patch BRANCH26 at offset 0x";
        errs().write_hex(RelocOffset);
        errs() << "\n";
        continue;
      }
    ...
    case 3:
      if (!patchArm64Page21(TextViewAll, RelocOffset,
        ...
        ExternalRelocCount++;
        errs() << "dyncode-extractor: cannot patch PAGE21 at offset 0x";
        errs().write_hex(RelocOffset);
        errs() << "\n";
        continue;
      }
    ...
    case 4: {
      ...
      if (!IsAdd && !IsIntLdSt && !IsFpLdSt) {
        ExternalRelocCount++;
        errs() << "dyncode-extractor: cannot patch PAGEOFF12 at offset 0x";
        errs().write_hex(RelocOffset);
    ...
    default: {
      ExternalRelocCount++;
      errs() << "dyncode-extractor: unsupported intra-section "
                "relocation type "
  ...
  if (ExternalRelocCount > 0) {
    errs() << "dyncode-extractor: " << ExternalRelocCount
  ...
  for (const auto &Sec : Obj.sections()) {
    ...
    if (DataOrErr)
      Size = DataOrErr->size();
    if (Size > 0) {
      errs() << "dyncode-extractor: unexpected data section '" << Name
             << "' (" << Size
  ...
  for (const auto &Sym : Obj.symbols()) {
    ...
    if (Offset != 0) {
      errs() << "dyncode-extractor: entry symbol '" << Name
  ...
  if (!FoundEntry) {
    if (!EntrySymbol.empty())
      errs() << "dyncode-extractor: entry symbol '" << EntrySymbol
  ...
  if (Verbose) {
    ...
    if (IsArm64)
      errs() << "dyncode-extractor: patched " << PatchedBranch26
             << " BRANCH26, " << PatchedPage21 << " PAGE21, "
             << PatchedPageOff12 << " PAGEOFF12 intra-section reloc(s)\n";
    else
      errs() << "dyncode-extractor: patched " << PatchedX86Pcrel32
