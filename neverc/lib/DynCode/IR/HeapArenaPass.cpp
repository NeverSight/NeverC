...
#include "neverc/DynCode/IR/MmapABI.h"
#include "neverc/DynCode/IR/DynCodeIRHelpers.h"
...
#include "neverc/DynCode/Tables/HeapArenaRewriteTargets.def"
#include "neverc/DynCode/Tables/UserExtra_HeapArenaRewriteTargets.def"
...
Value *emitMmapAlloc(IRBuilder<> &B, Module &M, Value *Size, DynCodeOS OS) {
  Type *SizeTy = getSizeType(M);
  PointerType *PtrTy = PointerType::getUnqual(M.getContext());
...
Value *emitFallbackAlloc(IRBuilder<> &B, Module &M, Value *Size,
                         HeapFallbackMode Mode, DynCodeOS OS) {
