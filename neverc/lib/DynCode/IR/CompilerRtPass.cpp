...
#include "neverc/DynCode/Tables/CompilerRtExternBinds.def"
...
bool rewriteExternDecls(
  ...
  for (const auto &Entry : CachedDecls) {
    ...
    if (!Helper)
      continue;
    Changed |= neverc::dyncode::rewriteExternCalls(M, *Decl, *Helper);
  }
  return Changed;
}
...
#include "neverc/DynCode/Tables/StackProbeNames.def"
#include "neverc/DynCode/Tables/UserExtra_StackProbeNames.def"
#undef NEVERC_NAME
  };
  ...
  for (Function &F : M) {
    if (F.isDeclaration())
      continue;
    if (isDynCodeInternalRuntimeName(F.getName()))
      continue;
