...
namespace {
class NeverCAsmParserCallback : public llvm::MCAsmParserSemaCallback {
  Parser &TheParser;
...
public:
  NeverCAsmParserCallback(Parser &P, SourceLocation Loc,
                          llvm::StringRef AsmString, llvm::ArrayRef<Token> Toks,
  ...
  llvm::StringRef LookupInlineAsmLabel(llvm::StringRef Identifier,
                                       llvm::SourceMgr &LSM,
                                       llvm::SMLoc Location,
  ...
  static void DiagHandlerCallback(const llvm::SMDiagnostic &D, void *Context) {
    ((NeverCAsmParserCallback *)Context)->handleDiagnostic(D);
  }

private:
  void findTokensForString(llvm::StringRef Str,
                           llvm::SmallVectorImpl<Token> &TempToks,
...
void NeverCAsmParserCallback::LookupInlineAsmIdentifier(
...
llvm::StringRef NeverCAsmParserCallback::LookupInlineAsmLabel(
    llvm::StringRef Identifier, llvm::SourceMgr &LSM, llvm::SMLoc Location,
    bool Create) {
...
void NeverCAsmParserCallback::findTokensForString(
...
SourceLocation
NeverCAsmParserCallback::translateLocation(const llvm::SourceMgr &LSM,
                                           llvm::SMLoc SMLoc) {
...
void NeverCAsmParserCallback::handleDiagnostic(const llvm::SMDiagnostic &D) {
...
ExprResult Parser::ParseMSAsmIdentifier(llvm::SmallVectorImpl<Token> &LineToks,
  ...
  PP.PushTokenStream(LineToks, /*DisableMacroExpansions*/ true,
                     /*IsReinject*/ true);
  ...
  UnqualifiedId Id;
  bool Invalid = ParseUnqualifiedId(Id);
  ExprResult Result =
      Actions.LookupInlineAsmIdentifier(Id, IsUnevaluatedContext);
...
namespace {
bool assembleMSAsmText(PrepEngine &PP, SourceLocation AsmLoc,
                       llvm::ArrayRef<Token> AsmToks,
...
StmtResult Parser::ParseMicrosoftAsmStatement(SourceLocation AsmLoc) {
  ...
  auto EmptyStmt = [&] {
    return Actions.OnMSAsmStmt(AsmLoc, LBraceLocs[0], AsmToks, AsmString,
  ...
  if (assembleMSAsmText(PP, AsmLoc, AsmToks, TokOffsets, AsmString))
    return StmtError();
  ...
  // Target MCTargetDesc may not be linked in NeverC-based tools.
  ...
  // Target AsmParser may not be linked in NeverC-based tools.
  if (!TargetParser) {
    Diag(AsmLoc, diag::err_msasm_unable_to_create_target)
  ...
  NeverCAsmParserCallback Callback(*this, AsmLoc, AsmString, AsmToks,
                                   TokOffsets);
  TargetParser->setSemaCallback(&Callback);
  TempSrcMgr.setDiagHandler(NeverCAsmParserCallback::DiagHandlerCallback,
                            &Callback);
  ...
  // constraints. NeverC always adds fpsr to the clobber list anyway.
  ...
  for (unsigned i = 0, e = NumExprs; i != e; ++i) {
    ...
    if (OpExprs[i].second)
      OpExpr =
          Actions.FormUnaryOp(getCurScope(), AsmLoc, UO_AddrOf, OpExpr).get();

    ConstraintRefs[i] = llvm::StringRef(Constraints[i]);
    Exprs[i] = OpExpr;
  }

  return Actions.OnMSAsmStmt(AsmLoc, LBraceLocs[0], AsmToks, AsmStringIR,
                             NumOutputs, NumInputs, ConstraintRefs, ClobberRefs,
                             Exprs, EndLoc);
}
...
StmtResult Parser::ParseAsmStatement(bool &msAsm) {
  ...
  if (Tok.is(tok::r_paren)) {
    ...
    T.consumeClose();
    return Actions.OnGCCAsmStmt(AsmLoc, /*isSimple*/ true, GAQ.isVolatile(),
                                /*NumOutputs*/ 0, /*NumInputs*/ 0, nullptr,
                                Constraints, Exprs, AsmString.get(), Clobbers,
                                /*NumLabels*/ 0, T.getCloseLocation());
  }

  // Parse Outputs, if present.
  bool AteExtraColon = false;
  if (Tok.is(tok::colon) || Tok.is(tok::coloncolon)) {
    // Operand-section separator: `:` or `::` (two colons).
  ...
  if (AteExtraColon || Tok.is(tok::colon) || Tok.is(tok::coloncolon)) {
    // Same as above for the inputs section.
    if (AteExtraColon)
      AteExtraColon = false;
    else {
      AteExtraColon = Tok.is(tok::coloncolon);
  ...
  if (AteExtraColon || Tok.is(tok::colon)) {
    ...
    while (true) {
      ...
      ExprResult Res =
          Actions.OnAddrLabel(Tok.getLocation(), Tok.getLocation(), LD);
  ...
  T.consumeClose();
  return Actions.OnGCCAsmStmt(AsmLoc, false, GAQ.isVolatile(), NumOutputs,
...
const char *Parser::GNUAsmQualifiers::getQualifierName(AQ Qualifier) {
  ...
  case AQ_inline:
    return tok::getKeywordSpelling(tok::kw_inline);
  case AQ_goto:
    return tok::getKeywordSpelling(tok::kw_goto);
  case AQ_unspecified:
    return "unspecified";
  }
}
