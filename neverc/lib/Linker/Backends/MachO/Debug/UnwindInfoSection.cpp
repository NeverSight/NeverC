...
// Internal representation of a compact unwind entry.
  ...
  for (const Defined *d : make_second_range(symbols))
    if (d->unwindEntry) {
      if (d->unwindEntry->getName() == section_names::compactUnwind) {
        ...
        // explicit GOT relocations, so the image emitter scan pass will add
        // those GOT entries. However, we still need to canonicalize the
        // personality pointers (like prepareRelocations() does for CU entries)
        // in order to avoid overflowing the 3-personality limit.
  ...
  for (size_t i = 0; i < isec->relocs.size(); ++i) {
    ...
    if (auto *s = r.referent.dyn_cast<Symbol *>()) {
      ...
      if (auto *defined = dyn_cast<Defined>(s)) {
        // Personality routines can show up as a local definition shadowed
        // by a global of the same name; deterministically take the global
        // so every reloc target lines up.
...
namespace {
bool canFoldEncoding(compact_unwind_encoding_t encoding) {
  ...
  if (target->cpuType == CPU_TYPE_X86_64 &&
...
void UnwindInfoSectionImpl::finalize() {
  ...
  for (auto foldBegin = cuIndices.begin(); foldBegin < cuIndices.end();) {
    ...
    // Common LSDA encodings (e.g. for C++) contain offsets from
