...
// * POINTER_TO_GOT only supports the 4-byte pc-relative form; the 8-byte
//   absolute form (writes the GOT slot value, not its address) is omitted
//   until a real-world need appears.
namespace {
constexpr std::array<RelocAttrs, 11> relocAttrsArray{{
...
namespace {
constexpr uint32_t stubHelperHeaderCode[] = {
...
namespace {
constexpr uint32_t stubHelperEntryCode[] = {
...
namespace {
bool parseAdrp(uint32_t insn, Adrp &adrp) {
...
bool parseAdd(uint32_t insn, Add &add) {
...
bool parseLdr(uint32_t insn, Ldr &ldr) {
...
bool isValidAdrOffset(int32_t delta) { return isInt<21>(delta); }

void writeAdr(void *loc, uint32_t dest, int32_t delta) {
...
void writeNop(void *loc) { write32le(loc, 0xd503201f); }

bool isLiteralLdrEligible(const Ldr &ldr) {
  return ldr.p2Size > 1 && isShiftedInt<19, 2>(ldr.offset);
}

void writeLiteralLdr(void *loc, const Ldr &ldr) {
...
bool isImmediateLdrEligible(const Ldr &ldr) {
  // The 12-bit immediate encodes (offset >> p2Size); only the unsigned
  // form is allowed here.
  uint32_t size = 1 << ldr.p2Size;
  return ldr.offset >= 0 && (ldr.offset % size) == 0 &&
         isUInt<12>(ldr.offset >> ldr.p2Size);
}

void writeImmediateLdr(void *loc, const Ldr &ldr) {
...
//   nop
namespace {
void applyAdrpAdd(uint8_t *buf, const ConcatInputSection *isec,
...
//   nop
namespace {
void applyAdrpAdrp(uint8_t *buf, const ConcatInputSection *isec,
                   uint64_t offset1, uint64_t offset2) {
  uint32_t ins1 = read32le(buf + offset1);
  uint32_t ins2 = read32le(buf + offset2);
  Adrp adrp1, adrp2;
...
//   ldr  xM, _foo
namespace {
void applyAdrpLdr(uint8_t *buf, const ConcatInputSection *isec,
                  uint64_t offset1, uint64_t offset2) {
  uint32_t ins1 = read32le(buf + offset1);
  uint32_t ins2 = read32le(buf + offset2);
  Adrp adrp;
...
namespace {
void applyAdrpLdrGot(uint8_t *buf, const ConcatInputSection *isec,
                     uint64_t offset1, uint64_t offset2) {
  uint32_t ins2 = read32le(buf + offset2);
  Add add;
  Ldr ldr;
...
//   ldr  x2, [x1, #off]
namespace {
void applyAdrpAddLdr(uint8_t *buf, const ConcatInputSection *isec,
                     uint64_t offset1, uint64_t offset2, uint64_t offset3) {
  uint32_t ins1 = read32le(buf + offset1);
  Adrp adrp;
  ...
  if (isValidAdrOffset(adrOffset)) {
    ...
    // Fold the offset into the adr; the add/ldr-got step becomes a nop.
    writeNop(buf + offset2);
    return;
  }
...
namespace {
void applyAdrpLdrGotLdr(uint8_t *buf, const ConcatInputSection *isec,
                        uint64_t offset1, uint64_t offset2, uint64_t offset3) {
  uint32_t ins2 = read32le(buf + offset2);
  Add add;
  Ldr ldr2;
...
uint64_t readValue(const uint8_t *&ptr, const uint8_t *end) {
  unsigned int n = 0;
...
void forEachHint(ArrayRef<uint8_t> data, Callback callback) {
