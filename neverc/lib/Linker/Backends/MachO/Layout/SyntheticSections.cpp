...
void MachHeaderSection::writeTo(uint8_t *buf) const {
  auto *hdr = reinterpret_cast<llvm_macho::mach_header *>(buf);
...
namespace {
void emitIncrement(uint64_t incr, raw_svector_ostream &os) {
  assert(incr != 0);
...
void flushRebase(const RebaseState &state, raw_svector_ostream &os) {
...
namespace {
void encodeRebases(const OutputSegment *seg,
...
namespace {
void writeChainedBind(uint8_t *buf, const Symbol *sym, int64_t addend) {
  assert(config->emitChainedFixups);
  assert(target->wordSize == 8 && "Only 64-bit platforms are supported");
...
// lastBinding.
namespace {
void encodeBinding(const OutputSection *osec, uint64_t outSecOff,
...
void optimizeOpcodes(std::vector<BindIR> &opcodes) {
  ...
  for (auto &p : opcodes) {
    // Match Apple-loader convention: only promote when the scaled addend
    // can't fit in the BIND_IMMEDIATE immediate field.
...
void flushOpcodes(const BindIR &op, raw_svector_ostream &os) {
...
int16_t ordinalForDylibSymbol(const DylibSymbol &dysym) {
...
int16_t ordinalForSymbol(const Symbol &sym) {
  if (const auto *dysym = dyn_cast<DylibSymbol>(&sym))
...
void encodeWeakOverride(const Defined *defined, raw_svector_ostream &os) {
...
namespace {
void addBindingsForStub(Symbol *sym) {
  assert(!config->emitChainedFixups);
...
namespace {
template <class LP>
std::vector<MachO::data_in_code_entry> collectDataInCodeEntries() {
  // Scatter: collect per-file entries in parallel.
  SmallVector<const ObjFile *> objFiles;
  for (const InputFile *f : inputFiles)
    if (auto *objFile = dyn_cast<ObjFile>(f))
      if (!objFile->getDataInCode().empty())
        objFiles.push_back(objFile);

  std::vector<SmallVector<MachO::data_in_code_entry, 0>> perFileEntries(
      objFiles.size());

  auto processFile = [&](size_t idx) {
    const ObjFile *objFile = objFiles[idx];
    auto &out = perFileEntries[idx];
    ArrayRef<MachO::data_in_code_entry> entries = objFile->getDataInCode();
    assert(is_sorted(entries, [](const auto &a, const auto &b) {
      return a.offset < b.offset;
    }));
    for (const Section *section : objFile->sections) {
      for (const Subsection &subsec : section->subsections) {
        ...
        auto it = llvm::lower_bound(entries, beginAddr,
                                    [](const MachO::data_in_code_entry &e,
                                       uint64_t a) { return e.offset < a; });
  ...
  parallelFor(0, objFiles.size(), processFile);

  // Gather and sort.
  size_t total = 0;
  for (const auto &v : perFileEntries)
    total += v.size();
  std::vector<MachO::data_in_code_entry> result;
  result.reserve(total);
  for (auto &v : perFileEntries)
...
void DataInCodeSection::finalizeContents() {
  entries = collectDataInCodeEntries<LP64>();
}
...
void FunctionStartsSection::finalizeContents() {
  ...
  // Scatter-gather: collect per-file function addresses in parallel.
  SmallVector<const ObjFile *> objFiles;
  for (const InputFile *file : inputFiles)
    if (auto *objFile = dyn_cast<ObjFile>(file))
      objFiles.push_back(objFile);

  std::vector<SmallVector<uint64_t, 0>> perFileAddrs(objFiles.size());
  auto collectAddrs = [&](size_t idx) {
    const ObjFile *objFile = objFiles[idx];
    auto &fileAddrs = perFileAddrs[idx];
    for (const Symbol *sym : objFile->symbols)
      if (const auto *defined = dyn_cast_or_null<Defined>(sym))
        if (defined->isec && isCodeSection(defined->isec) && defined->isLive())
          fileAddrs.push_back(defined->getVA());
  };

  parallelFor(0, objFiles.size(), collectAddrs);

  // Merge and sort.
  size_t total = 0;
  for (const auto &v : perFileAddrs)
    total += v.size();
...
void SymtabSection::finalizeContents() {
  ...
  // Scatter-gather: filter locals in parallel, assign string offsets serially.
  if (config->localSymbolsPresence != SymtabPresence::None) {
    ...
    std::vector<SmallVector<Defined *, 0>> perFileLocals(objFiles.size());

    auto filterLocals = [&](size_t idx) {
      const ObjFile *objFile = objFiles[idx];
      auto &locals = perFileLocals[idx];
      for (Symbol *sym : objFile->symbols)
        if (auto *defined = dyn_cast_or_null<Defined>(sym))
          if (!defined->isExternal() && defined->isLive() &&
              defined->includeInSymtab)
            locals.push_back(defined);
    };

    parallelFor(0, objFiles.size(), filterLocals);

    for (const auto &locals : perFileLocals)
      for (Defined *defined : locals)
...
namespace {
uint32_t indirectValue(const Symbol *sym) {
...
void macho::createSyntheticSymbols() {
  ...
  // The ABI allows __dso_handle to point anywhere in a loaded dylib's
  // segments; Apple loaders convention is the Mach-O header itself.
...
namespace {
bool needsWeakBind(const Symbol &sym) {
  if (auto *dysym = dyn_cast<DylibSymbol>(&sym))
...
namespace {
size_t writeImport(uint8_t *buf, int format, uint32_t libOrdinal, bool weakRef,
                   uint32_t nameOffset, int64_t addend) {
  switch (format) {
...
namespace {
size_t importEntrySize(int format) {
  switch (format) {
  case DYLD_CHAINED_IMPORT:
    return sizeof(dyld_chained_import);
  case DYLD_CHAINED_IMPORT_ADDEND:
