...
// Returns a symbol name for an error message. NeverC is C-only, so we do not
// demangle anything; we only strip the Mach-O leading `_` when `-demangle` was
// requested for CLI compatibility.
namespace {
std::string maybeDemangleSymbol(StringRef symName) {
  if (config->demangle)
...
std::string linker::toString(const Symbol &sym) {
  return maybeDemangleSymbol(sym.getName());
}

std::string linker::toMachOString(const object::Archive::Symbol &b) {
