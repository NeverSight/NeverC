...
// is \p skip.
namespace {
void transplantSymbolsAtOffset(InputSection *fromIsec, InputSection *toIsec,
                               Defined *skip, uint64_t fromOff,
                               uint64_t toOff) {
...
Defined *SymbolTable::addDefined(StringRef name, InputFile *file,
  ...
  if (!wasInserted) {
    if (auto *defined = dyn_cast<Defined>(s)) {
      // __attribute__((override)) is orthogonal to weak vs strong: a symbol
      // marked override forcibly replaces any other definition of the same
      // name, and a definition coming from an override source is itself
      // never replaced by a non-override definition. Handle this before the
      // default weak/strong logic so the policy is symmetric regardless of
      // which side is weak.
      if (auto overIt = config->overrideSymbols.find(name);
          overIt != config->overrideSymbols.end()) {
        const InputFile *overrideSource = overIt->second;
        bool incomingIsOverride =
            overrideSource == nullptr || overrideSource == file;
        bool existingIsOverride =
            overrideSource == nullptr || overrideSource == defined->getFile();
        if (incomingIsOverride) {
          if (auto concatIsec =
                  dyn_cast_or_null<ConcatInputSection>(defined->isec)) {
            concatIsec->wasCoalesced = true;
            if (isec)
              transplantSymbolsAtOffset(concatIsec, isec, defined,
                                        defined->value, value);
          }
          // Fall through to replaceSymbol below, replacing the existing
          // definition with the incoming override source.
        } else if (existingIsOverride) {
          // Existing definition is the override source: keep it and ignore
          // the incoming non-override definition.
          return defined;
        } else {
          // Neither side is the override source (the real source hasn't
          // been seen yet, or it's a different file entirely). Treat as a
          // duplicate but rely on reportPendingDuplicateSymbols to skip the
          // diagnostic since the symbol is in overrideSymbols.
...
namespace {
Defined *createBoundarySymbol(const Undefined &sym) {
...
namespace {
void handleSectionBoundarySymbol(const Undefined &sym, StringRef segSect,
                                 Boundary which) {
...
void handleSegmentBoundarySymbol(const Undefined &sym, StringRef segName,
                                 Boundary which) {
...
namespace {
bool recoverFromUndefinedSymbol(const Undefined &sym) {
...
// file of sym.
namespace {
const Symbol *getAlternativeSpelling(const Undefined &sym, std::string &preHint,
                                     std::string &postHint) {
...
namespace {
void reportUndefinedSymbol(const Undefined &sym, const UndefinedDiag &locations,
