...
#include "Linker/MachO/EhFrame.h"
...
namespace {
std::vector<PlatformInfo> getPlatformInfos(const InputFile *input) {
  ...
  for (auto *cmd : findCommands<llvm_macho::build_version_command>(
           hdr, llvm_macho::LC_BUILD_VERSION)) {
    PlatformInfo info;
    info.target.Platform = static_cast<llvm_macho::PlatformType>(cmd->platform);
    info.target.MinDeployment = decodeVersion(cmd->minos);
    platformInfos.emplace_back(std::move(info));
  }
  for (auto *cmd : findCommands<llvm_macho::version_min_command>(
           hdr, llvm_macho::LC_VERSION_MIN_MACOSX)) {
    PlatformInfo info;
    info.target.Platform = llvm_macho::PLATFORM_MACOS;
...
bool checkCompatibility(const InputFile *input) {
...
bool compatWithTargetArch(const InputFile *file, const Header *hdr) {
  uint32_t cpuType;
...
std::optional<MemoryBufferRef> macho::readFile(StringRef path,
  ...
  // and cachedReads is shared mutable state.
  std::lock_guard<std::mutex> lock(cachedReadsMu);
  ...
  const auto *hdr = reinterpret_cast<const llvm_macho::fat_header *>(buf);
  if (mbref.getBufferSize() < sizeof(uint32_t) ||
      read32be(&hdr->magic) != llvm_macho::FAT_MAGIC) {
    return cachedReads[key] = mbref;
  }

  llvm::BumpPtrAllocator &bAlloc = linker::bAlloc();
  ...
  const auto *arch =
      reinterpret_cast<const llvm_macho::fat_arch *>(buf + sizeof(*hdr));
...
namespace {
std::optional<size_t> getRecordSize(StringRef segname, StringRef name) {
...
namespace {
Error parseCallGraph(ArrayRef<uint8_t> data,
...
// ===----------------------------------------------------------------------===
// ObjFile: section & relocation parsing
// ===----------------------------------------------------------------------===
...
void ObjFile::parseSections(ArrayRef<SectionHeader> sectionHeaders) {
  ...
  for (const SectionHeader &sec : sectionHeaders) {
    ...
    if (sectionType(sec.flags) == S_CSTRING_LITERALS) {
      ...
      bool dedupLiterals = config->dedupStrings;
      auto *isec =
      ...
      // LTO re-emits the section into its native output, but the bitcode
      // source already populated overrideSymbols via marker symbols, so
      // skip re-parsing for LTO-built objects to avoid spurious diagnostics.
      if (!builtFromBitcode) {
        ...
        while (!payload.empty()) {
          ...
          if (!sym.empty()) {
            ...
            auto [it, inserted] =
                config->overrideSymbols.try_emplace(saved, this);
...
namespace {
template <class T>
Section *findContainingSection(const std::vector<Section *> &sections,
                               T *offset) {
...
namespace {
template <class T>
InputSection *findContainingSubsection(const Section &section, T *offset) {
  static_assert(std::is_same<uint64_t, T>::value ||
                    std::is_same<uint32_t, T>::value,
                "unexpected type for offset");
  auto it = std::prev(llvm::upper_bound(
...
Defined *findSymbolAtOffset(const ConcatInputSection *isec, uint64_t off) {
...
namespace {
template <class SectionHeader>
bool validateRelocationInfo(InputFile *file, const SectionHeader &sec,
                            llvm_macho::relocation_info rel) {
  ...
  if (rel.r_length < 2 || rel.r_length > 3 ||
    ...
    constexpr StringLiteral widths[]{"0", "4", "8", "4 or 8"};
...
void ObjFile::parseRelocations(ArrayRef<SectionHeader> sectionHeaders,
  ...
  auto *buf = reinterpret_cast<const uint8_t *>(mb.getBufferStart());
  ArrayRef<llvm_macho::relocation_info> relInfos(
      reinterpret_cast<const llvm_macho::relocation_info *>(buf + sec.reloff),
      sec.nreloc);
  ...
  if (relInfos.size() >= kRelocPreReserveThreshold && !subsections.empty()) {
    ...
    for (size_t i = 0; i < relInfos.size() && canReserve; ++i) {
      llvm_macho::relocation_info relInfo = relInfos[i];
  ...
  for (size_t i = 0; i < relInfos.size(); i++) {
    ...
    if (!validateRelocationInfo(this, sec, relInfo))
      continue;
    if (relInfo.r_address & R_SCATTERED)
      fatal("scattered relocations not supported");
    ...
    // Most producers emit per-section relocations in offset order; take
    // the linear-scan fast path, fall back to binary search if we hit an
    // out-of-order entry (rare, e.g. relocatable inputs).
    InputSection *subsec;
    ...
    if (isSubtrahend) {
      llvm_macho::relocation_info minuendInfo = relInfos[++i];
...
macho::Symbol *createDefined(const NList &sym, StringRef name,
  ...
  //                 table of the output.
  // N_PEXT: emitted only by relocatable links (`-r`); semantically the
  //         same as the 0 case for the symbol-table side.
  ...
  if (sym.n_type & N_EXT) {
    ...
    // Duplicate-symbol merge policy: keep the first definition seen on the
    // command line, just merging metadata (see SymbolTable::addDefined).
    // Mismatched -fvisibility-inlines-hidden across TUs is treated as ODR
    // violation; in well-formed inputs the result is observable-equivalent
    // because autohide and privateExtern both stay out of the dynamic symtab
  ...
  // external; for non-code symbols (string literals, constants externalized
  // by parallel codegen) strip them from the output symbol table.  Keep
  // code-section symbols so debugger backtraces show function names.
  if ((sym.n_type & MachO::N_PEXT) && isec && !isCodeSection(isec))
...
namespace {
template <class NList>
macho::Symbol *createAbsolute(const NList &sym, InputFile *file, StringRef name,
                              bool forceHidden) {
  if (sym.n_type & N_EXT) {
    bool isPrivateExtern = sym.n_type & N_PEXT || forceHidden;
...
macho::Symbol *ObjFile::parseNonSectionSymbol(const NList &sym,
  ...
  case N_INDR: {
    // Local indirect symbols are pointless: same-file relocs can name the
    // target directly. Only externally-visible aliases need a redirection.
    if (!(sym.n_type & N_EXT))
      return nullptr;
  ...
  case N_PBUD:
    error("symbols of type N_PBUD are not supported");
    return nullptr;
  case N_SECT:
    llvm_unreachable(
...
void ObjFile::parseLinkerOptions(SmallVectorImpl<StringRef> &LCLinkerOptions) {
  ...
  for (auto *cmd : findCommands<llvm_macho::linker_option_command>(
           hdr, llvm_macho::LC_LINKER_OPTION)) {
    StringRef data{reinterpret_cast<const char *>(cmd + 1),
                   cmd->cmdsize - sizeof(llvm_macho::linker_option_command)};
...
template <class LP> void ObjFile::parse() {
  ...
  if (const llvm_macho::load_command *cmd =
          findCommand(hdr, LP::segmentLCType)) {
  ...
  if (const llvm_macho::load_command *cmd =
          findCommand(hdr, llvm_macho::LC_SYMTAB)) {
    auto *c = reinterpret_cast<const llvm_macho::symtab_command *>(cmd);
    ArrayRef<NList> nList(reinterpret_cast<const NList *>(buf + c->symoff),
                          c->nsyms);
    const char *strtab = reinterpret_cast<const char *>(buf) + c->stroff;
    bool subsectionsViaSymbols =
        hdr->flags & llvm_macho::MH_SUBSECTIONS_VIA_SYMBOLS;
...
template <class LP> void ObjFile::parseLazy() {
  ...
  const llvm_macho::load_command *cmd = findCommand(hdr, llvm_macho::LC_SYMTAB);
  if (!cmd)
    return;
  auto *c = reinterpret_cast<const llvm_macho::symtab_command *>(cmd);
  ArrayRef<NList> nList(reinterpret_cast<const NList *>(buf + c->symoff),
...
// ===----------------------------------------------------------------------===
// ObjFile: debug info & compact unwind
// ===----------------------------------------------------------------------===
...
ArrayRef<llvm_macho::data_in_code_entry> ObjFile::getDataInCode() const {
  const auto *buf = reinterpret_cast<const uint8_t *>(mb.getBufferStart());
  const llvm_macho::load_command *cmd =
      findCommand(buf, llvm_macho::LC_DATA_IN_CODE);
  if (!cmd)
    return {};
  const auto *c =
      reinterpret_cast<const llvm_macho::linkedit_data_command *>(cmd);
  return {reinterpret_cast<const llvm_macho::data_in_code_entry *>(buf +
                                                                   c->dataoff),
          c->datasize / sizeof(llvm_macho::data_in_code_entry)};
}

ArrayRef<uint8_t> ObjFile::getOptimizationHints() const {
  const auto *buf = reinterpret_cast<const uint8_t *>(mb.getBufferStart());
  if (auto *cmd = findCommand<llvm_macho::linkedit_data_command>(
          buf, llvm_macho::LC_LINKER_OPTIMIZATION_HINT))
...
void ObjFile::registerCompactUnwind(Section &compactUnwindSection) {
  for (const Subsection &subsection : compactUnwindSection.subsections) {
    ...
    for (auto it = isec->relocs.begin(); it != isec->relocs.end();) {
      ...
      //
      // Given that NeverC emits relocations in reverse order of address, this
...
namespace {
uint8_t pointerEncodingToSize(uint8_t enc) {
  switch (enc & 0xf) {
...
namespace {
CIE parseCIE(const InputSection *isec, const EhReader &reader, size_t off) {
...
// EH-frame target addresses are encoded as (subtrahend, minuend) reloc
// pairs that materialise a pcrel offset. Recover the target Defined from
// that pair so subsequent passes can rewrite the reloc against an
// EH_Frame-local anchor and survive FDE dead-stripping.
//
// When `Invert` is set, the encoded value is (target - PC) instead of the
// usual (PC - target).
...
targetSymFromCanonicalSubtractor(const InputSection *isec,
  ...
  if (pcSym->isec == isec) {
    ...
    // We should really verify that the original relocation's semantics
...
linker::DWARFCache *ObjFile::getDwarf() {
...
namespace {
DylibFile *loadDylib(StringRef path, DylibFile *umbrella) {
...
// library.
namespace {
bool isImplicitlyLinked(StringRef path) {
...
// ===----------------------------------------------------------------------===
// DylibFile: dylib parsing & re-exports
// ===----------------------------------------------------------------------===
...
DylibFile::DylibFile(MemoryBufferRef mb, DylibFile *umbrella,
  ...
  auto *hdr =
      reinterpret_cast<const llvm_macho::mach_header *>(mb.getBufferStart());

  if (const llvm_macho::load_command *cmd =
          findCommand(hdr, llvm_macho::LC_ID_DYLIB)) {
    auto *c = reinterpret_cast<const llvm_macho::dylib_command *>(cmd);
  ...
  deadStrippable = hdr->flags & llvm_macho::MH_DEAD_STRIPPABLE_DYLIB;

  if (!checkCompatibility(this))
    return;

  checkAppExtensionSafety(hdr->flags & llvm_macho::MH_APP_EXTENSION_SAFE);

  for (auto *cmd :
       findCommands<llvm_macho::rpath_command>(hdr, llvm_macho::LC_RPATH)) {
  ...
  const auto *dyldInfo = findCommand<llvm_macho::dyld_info_command>(
      hdr, llvm_macho::LC_DYLD_INFO_ONLY);
  const auto *exportsTrie = findCommand<llvm_macho::linkedit_data_command>(
      hdr, llvm_macho::LC_DYLD_EXPORTS_TRIE);
...
void DylibFile::parseLoadCommands(MemoryBufferRef mb) {
  ...
  for (uint32_t i = 0, n = hdr->ncmds; i < n; ++i) {
    auto *cmd = reinterpret_cast<const llvm_macho::load_command *>(p);
    p += cmd->cmdsize;

    if (!(hdr->flags & llvm_macho::MH_NO_REEXPORTED_DYLIBS) &&
        cmd->cmd == llvm_macho::LC_REEXPORT_DYLIB) {
      const auto *c = reinterpret_cast<const llvm_macho::dylib_command *>(cmd);
    ...
    if (config->namespaceKind == NamespaceKind::flat &&
        cmd->cmd == llvm_macho::LC_LOAD_DYLIB) {
      const auto *c = reinterpret_cast<const llvm_macho::dylib_command *>(cmd);
      StringRef dylibPath =
          reinterpret_cast<const char *>(c) + read32le(&c->dylib.name);
...
namespace {
bool isArchABICompatible(ArchitectureSet archSet, Architecture targetArch) {
  uint32_t cpuType;
...
bool isTargetPlatformArchCompatible(
...
namespace {
Expected<InputFile *> loadArchiveMember(MemoryBufferRef mb, uint32_t modTime,
                                        StringRef archiveName,
                                        uint64_t offsetInArchive,
                                        bool forceHidden, bool compatArch) {
...
void ArchiveFile::fetch(const object::Archive::Symbol &sym) {
  ...
  // Keep the mangled name in the lookup message: demangling here would
  // confuse the user-visible "could not find member" error path.
...
namespace {
macho::Symbol *createBitcodeSymbol(const lto::InputFile::Symbol &objSym,
                                   BitcodeFile &file) {
...
BitcodeFile::BitcodeFile(MemoryBufferRef mb, StringRef archiveName,
  ...
  // collision and LTO can't proceed.
...
void BitcodeFile::parse() {
  // Convert LTO Symbols to linker Symbols in order to perform resolution. The
  ...
  for (const auto &objSym : obj->symbols()) {
    ...
    auto [it, inserted] = config->overrideSymbols.try_emplace(saved, this);
    if (!inserted && it->second != nullptr && it->second != this) {
      warn("symbol '" + saved +
           "' marked override in multiple files; last definition wins"
           "\n>>> " +
  ...
  // ObjFile<>::parseSymbols. Skip override markers entirely: their slots
  // stay nullptr so BitcodeCompiler::add can hand LTO a no-op resolution.
  auto isMarker = [&](const lto::InputFile::Symbol &irSym) {
    return irSym.getName().starts_with(overridePrefix);
  };
  for (auto it : llvm::enumerate(obj->symbols()))
    if (!it.value().isUndefined() && !isMarker(it.value()))
