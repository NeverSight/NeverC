...
// Returns a symbol for an error message. NeverC is C-only so there is no
// mangling to undo; the `config->demangle` flag is retained for CLI
// compatibility but now has no runtime effect.
namespace {
std::string maybeDemangleSymbol(StringRef symName) { return symName.str(); }
} // namespace

std::string linker::toString(const elf::Symbol &sym) {
...
uint64_t Symbol::getGotOffset() const {
  return uint64_t(getGotIdx()) * target->gotEntrySize;
}
...
uint64_t Symbol::getGotPltOffset() const {
  if (isInIplt)
    return uint64_t(getPltIdx()) * target->gotEntrySize;
  return (uint64_t(getPltIdx()) + target->gotPltHeaderEntriesNum) *
         target->gotEntrySize;
}

uint64_t Symbol::getPltVA() const {
  uint64_t outVA = isInIplt ? in.iplt->getVA() +
                                  uint64_t(getPltIdx()) * target->ipltEntrySize
                            : in.plt->getVA() + in.plt->headerSize +
                                  uint64_t(getPltIdx()) * target->pltEntrySize;
...
void elf::maybeWarnUnorderableSymbol(const Symbol *sym) {
  ...
  // UnresolvedPolicy::Ignore silences the unknown-symbol diagnostic for
  // symbols that didn't come from demoteSymbols() -- the caller has opted
  // in to ignoring them on the command line.
...
void Symbol::resolve(const Undefined &other) {
  ...
  if (isLazy()) {
    ...
    //
    // The linker's symbol resolution semantics is more relaxed than
    // traditional Unix linkers. For example,
    //
    //   neverc foo.a bar.o
    //
    ...
    //
    // That the linker accepts more relaxed form means that (besides it'd make
    // more sense) you can accidentally write a command line or a build file
    // that works only with this linker, even if you have a plan to distribute
    // it to wider
    ...
    // archives. Again, the linker accepts
    //
    //   neverc foo.a bar.a
    //
    ...
    //
    //   neverc A B --start-group C D --end-group E
    //
...
void Symbol::resolve(const Defined &other) {
  ...
  // __attribute__((override)) is intentionally orthogonal to STB_WEAK: a
