...
namespace {
bool isADRP(uint32_t instr) { return (instr & 0x9f000000) == 0x90000000; }
} // namespace
...
namespace {
bool isLoadStoreClass(uint32_t instr) {
...
namespace {
bool isST1MultipleOpcode(uint32_t instr) {
...
// L == 0 for Stores.
namespace {
bool isLoadStoreExclusive(uint32_t instr) {
  return (instr & 0x3f000000) == 0x08000000;
}

bool isLoadExclusive(uint32_t instr) {
...
namespace {
bool isLoadLiteral(uint32_t instr) {
...
namespace {
bool isSTNP(uint32_t instr) { return (instr & 0x3bc00000) == 0x28000000; }
} // namespace
...
// Writes to Rn.
namespace {
bool isSTPPost(uint32_t instr) { return (instr & 0x3bc00000) == 0x28800000; }
} // namespace

// (offset)
// | opc (2) 10 | 1 V 01 | 0 L | imm7 | Rt2 (5) | Rn (5) | Rt (5) |
namespace {
bool isSTPOffset(uint32_t instr) { return (instr & 0x3bc00000) == 0x29000000; }
} // namespace
...
// Writes to Rn.
namespace {
bool isSTPPre(uint32_t instr) { return (instr & 0x3bc00000) == 0x29800000; }

bool isSTP(uint32_t instr) {
...
namespace {
bool isLoadStoreUnscaled(uint32_t instr) {
...
namespace {
bool isLoadStoreImmediatePost(uint32_t instr) {
...
namespace {
bool isLoadStoreUnpriv(uint32_t instr) {
...
namespace {
bool isLoadStoreImmediatePre(uint32_t instr) {
...
namespace {
bool isLoadStoreRegisterOff(uint32_t instr) {
...
namespace {
bool isLoadStoreRegisterUnsigned(uint32_t instr) {
  return (instr & 0x3b000000) == 0x39000000;
}

// Rt is always in bit position 0 - 4.
uint32_t getRt(uint32_t instr) { return (instr & 0x1f); }

// Rn is always in bit position 5 - 9.
uint32_t getRn(uint32_t instr) { return (instr >> 5) & 0x1f; }
} // namespace
...
bool isV8SingleRegisterNonStructureLoadStore(uint32_t instr) {
...
// in v8.1.
namespace {
bool isV8NonStructureLoad(uint32_t instr) {
...
namespace {
bool hasWriteback(uint32_t instr) {
...
namespace {
bool doesLoadStoreWriteToReg(uint32_t instr, uint32_t reg) {
...
//
// Sequence 2 from the Software Developers Errata Notice is not scanned for:
// it is exceedingly rare in compiler-emitted code and the bookkeeping cost
// of detecting it dominates any expected benefit.
...
namespace {
bool is843419ErratumSequence(uint32_t instr1, uint32_t instr2,
                             uint32_t instr4) {
  if (!isADRP(instr1))
    return false;
...
namespace {
uint64_t scanCortexA53Errata843419(InputSection *isec, uint64_t &off,
                                   uint64_t limit) {
...
namespace {
void implementPatch(uint64_t adrpAddr, uint64_t patcheeOffset,
                    InputSection *isec,
...
AArch64Err843419Patcher::patchInputSectionDescription(
  ...
  for (InputSection *isec : isd.sections) {
    //  The linker doesn't use the erratum sequence in SyntheticSections.
