...
#include "Linker/ELF/CallGraphSort.h"
...
namespace {
template <class ELFT> class OutputWriter {
public:
...
private:
  ...
  SmallVector<PhdrEntry *, 0> buildSegmentMap(Partition &part);
  void addPhdrForSection(Partition &part, unsigned shType, unsigned pType,
                         unsigned pFlags);
  void computeFileLayout();
  void computeFileLayoutBinary();
  void commitSegmentHeaders(Partition &part);
  void validateLayout();
...
void elf::addReservedSymbols() {
  StringRef gotSymName = "_GLOBAL_OFFSET_TABLE_";
...
void resolveSymbolVisibility() {
  ...
  if (config->hasDynSymTab) {
    parallelForEach(symbols, [](Symbol *sym) {
...
bool elf::canHaveMemtagGlobals() {
  return config->emachine == EM_AARCH64 &&
         ...
         (config->shared || needsInterpSection());
}

OutputSection *findSection(StringRef name, unsigned partition = 1) {
...
template <class ELFT> void elf::createSyntheticSections() {
  ...
  for (Partition &part : partitions) {
    ...
    if (config->buildId != BuildIdStyle::None) {
      part.buildId = std::make_unique<FormIdSection>();
      add(*part.buildId);
    }
...
template <class ELFT> void OutputWriter<ELFT>::run() {
  ...
  if (config->printMemoryUsage)
    script->printMemoryUsage(linker::outs());
  ...
  {
    llvm::TimeTraceScope timeScope("Write image");
...
void markUsedLocalSymbolsImpl(ObjFile<ELFT> *file, llvm::ArrayRef<RelTy> rels) {
...
bool shouldKeepInSymtab(const Defined &sym) {
  if (sym.isSection())
    return false;
...
bool linker::elf::includeInSymtab(const Symbol &b) {
...
// Bit-flag ranking: each flag encodes one placement decision.
enum RankFlags {
...
unsigned getSectionRank(OutputSection &osec) {
  ...
  // -T sections first for explicit VA assignment.
  ...
  // Permission order: R, RX, RXW, RW(RELRO), RW(non-RELRO).
...
bool compareSections(const SectionCommand *aCmd, const SectionCommand *bCmd) {
...
// Static executables need iplt boundary symbols for IRELATIVE resolution.
template <class ELFT> void OutputWriter<ELFT>::addRelIpltSymbols() {
  if (config->isPic)
    return;
...
// countLeadingZeros.
int getRankProximity(OutputSection *a, SectionCommand *b) {
...
bool shouldSkip(SectionCommand *cmd) {
  if (auto *assign = dyn_cast<SymbolAssignment>(cmd))
    return assign->name != ".";
  return false;
}

// Place orphan sections near sections with the closest rank.
SmallVectorImpl<SectionCommand *>::iterator
findOrphanPos(SmallVectorImpl<SectionCommand *>::iterator b,
  ...
  // When the orphan would land at the tail, append it past any remaining
  // commands. Scripts that pin the file start but leave the tail (e.g. all
  // the non-alloc sections) loose rely on this placement.
...
void maybeShuffle(DenseMap<const InputSectionBase *, int> &order) {
...
void sortISDBySectionOrder(InputSectionDescription *isd,
  ...
  // Insert hot sections at midpoint to minimize thunk distance.
  size_t insPt = 0;
...
void sortSection(OutputSection &osec,
                 const DenseMap<const InputSectionBase *, int> &order) {
...
template <class ELFT> void OutputWriter<ELFT>::sortInputSections() {
...
template <class ELFT> void OutputWriter<ELFT>::sortSections() {
...
template <class ELFT> void OutputWriter<ELFT>::sortOrphanSections() {
...
bool compareByFilePosition(InputSection *a, InputSection *b) {
...
template <class ELFT> void OutputWriter<ELFT>::resolveShfLinkOrder() {
...
void finalizeSynthetic(SyntheticSection *sec) {
...
template <class ELFT>
void OutputWriter<ELFT>::finalizeAddressDependentContent() {
  llvm::TimeTraceScope timeScope("Finalize address dependent content");
  ThunkCreator tc;
  AArch64Err843419Patcher a64p;
  script->assignAddresses();
  // SHF_LINK_ORDER does not require precise addresses, but does require the
...
// option is used.
template <class ELFT> void OutputWriter<ELFT>::optimizeBasicBlockJumps() {
...
template <class ELFT> void OutputWriter<ELFT>::prepareLayout() {
...

  {
    ...
    // Pre-compute bindings in parallel before serial symtab insertion.
    parallelForEach(symbols, [](Symbol *sym) {
...

  {
    ...
    // Finalize independent synthetic sections in parallel.
    {
      ...
      finPar(in.symTabShndx.get());
      finPar(in.shStrTab.get());
      finPar(in.strTab.get());
      finPar(in.got.get());
      finPar(in.igotPlt.get());
      finPar(in.gotPlt.get());
      finPar(in.relaIplt.get());
      finPar(in.relaPlt.get());
      finPar(in.plt.get());
      finPar(in.iplt.get());
      finPar(in.partIndex.get());
    }

    // Partition-specific sections: dynSymTab first (hash tables depend on it),
    // then the rest in parallel, dynamic section last.
    for (Partition &part : partitions) {
      if (part.relaDyn) {
...

      {
        ...
        finPar(part.verDef.get());
        finPar(part.ehFrameHdr.get());
        finPar(part.verSym.get());
        finPar(part.verNeed.get());
      }
  ...
  //    they cannot jump to arbitrary addresses in memory. For example, AArch64
  //    branch instructions can target only +-128 MiB from PC. It is a linker's
  ...
  //    address of InputSections. For example, android packed relocations
  //    sections content.
  //
  ...
  parallelForEach(outputSections, [](OutputSection *sec) { sec->finalize(); });
...
// readable.
template <class ELFT> void OutputWriter<ELFT>::checkExecuteOnly() {
...
template <class ELFT> void OutputWriter<ELFT>::addStartEndSymbols() {
...
template <class ELFT>
void OutputWriter<ELFT>::addStartStopSymbols(OutputSection &osec) {
...
bool needsPtLoad(OutputSection *sec) {
...
uint64_t computeFlags(uint64_t flags) {
  if (config->omagic)
...
template <class ELFT>
SmallVector<PhdrEntry *, 0>
OutputWriter<ELFT>::buildSegmentMap(Partition &part) {
  ...
  for (OutputSection *sec : outputSections) {
    ...
    // Partition 255 = end marker, force into main partition.
    ...
    // Start a new PT_LOAD on flag change, LMA region change, or
    // NOBITS → PROGBITS transition.
...
template <class ELFT>
void OutputWriter<ELFT>::addPhdrForSection(Partition &part, unsigned shType,
...
// such section.
template <class ELFT> void OutputWriter<ELFT>::fixSectionAlignments() {
...
uint64_t computeFileOffset(OutputSection *os, uint64_t off) {
...
template <class ELFT> void OutputWriter<ELFT>::computeFileLayoutBinary() {
...
template <class ELFT>
void OutputWriter<ELFT>::commitSegmentHeaders(Partition &part) {
  for (PhdrEntry *p : part.phdrs) {
...
void checkOverlap(StringRef name, std::vector<SectionOffset> &sections,
                  bool isVirtualAddr) {
...
template <class ELFT> void OutputWriter<ELFT>::validateLayout() {
...
template <class ELFT> void OutputWriter<ELFT>::allocateOutputBuffer() {
  uint64_t maxSize = INT64_MAX;
...
// Pad executable segment tails with trap instructions for safety.
template <class ELFT> void OutputWriter<ELFT>::writeTrapInstr() {
  for (Partition &part : partitions) {
    // Fill the last page.
    for (PhdrEntry *p : part.phdrs)
      if (p->p_type == PT_LOAD && (p->p_flags & PF_X))
...
template <class ELFT> void OutputWriter<ELFT>::writeSections() {
  llvm::TimeTraceScope timeScope("Write sections");

  // RELA targets carry full addends in relocation entries, so all
  // sections can be written in a single parallel pass.  REL targets
...
// Parallel Merkle-tree hash: split into 1 MB chunks, hash each in
// parallel, then hash the concatenated per-chunk hashes.
void computeHash(
  ...
  std::vector<ArrayRef<uint8_t>> chunks = splitIntoChunks(data, 1024 * 1024);
...
template <class ELFT> void OutputWriter<ELFT>::computeContentHash() {
  if (!mainPart->buildId || !mainPart->buildId->getParent())
    return;

  if (config->buildId == BuildIdStyle::Hexstring) {
    for (Partition &part : partitions)
  ...
  case BuildIdStyle::Uuid:
    if (int ec = llvm::getRandomBytes(buildId.get(), hashSize))
      error("entropy source failure: " + Twine(strerror(ec)));
    break;
  default:
    llvm_unreachable("unknown BuildIdStyle");
  }
  for (Partition &part : partitions)
