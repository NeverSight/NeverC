...
uint32_t AArch64::getThunkSectionSpacing() const {
  // For AArch64 the only branches we are permitted to thunk have a range of
  // +/- 128 MiB
...
namespace {
void write32AArch64Addr(uint8_t *l, uint64_t imm) {
...
void or32le(uint8_t *p, int32_t v) { write32le(p, read32le(p) | v); }

// Update the immediate field in a AARCH64 ldr, str, and add instruction.
void or32AArch64Imm(uint8_t *l, uint64_t imm) {
...
void writeSMovWImm(uint8_t *loc, uint32_t imm) {
...
template <class ELFT>
void addTaggedSymbolReferences(InputSectionBase &sec,
...
void linker::elf::createTaggedSymbols(
    const SmallVector<ELFFileBase *, 0> &files) {
