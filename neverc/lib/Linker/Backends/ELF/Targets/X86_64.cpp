...
namespace {
JmpInsnOpcode getJmpInsnType(const uint8_t *first, const uint8_t *second) {
  if (*second == 0xe9)
    return J_JMP_32;
...
namespace {
unsigned getRelocationWithOffset(const InputSection &is, uint64_t offset) {
...
bool isRelocationForJmpInsn(Relocation &R) {
  return R.type == R_X86_64_PLT32 || R.type == R_X86_64_PC32 ||
         R.type == R_X86_64_PC8;
}

bool isFallThruRelocation(InputSection &is, InputFile *file,
...
namespace {
JmpInsnOpcode invertJmpOpcode(const JmpInsnOpcode opcode) {
  switch (opcode) {
  case J_JE_32:
    return J_JNE_32;
  case J_JNE_32:
...
namespace {
void relaxTlsGdToLe(uint8_t *loc, const Relocation &rel, uint64_t val) {
...
void relaxTlsGdToIe(uint8_t *loc, const Relocation &rel, uint64_t val) {
  if (rel.type == R_X86_64_TLSGD) {
    // Convert
    //   .byte 0x66
    //   leaq x@tlsgd(%rip), %rdi
...
namespace {
void relaxTlsIeToLe(uint8_t *loc, const Relocation &, uint64_t val) {
  uint8_t *inst = loc - 3;
  uint8_t reg = loc[-1] >> 3;
...
void relaxTlsLdToLe(uint8_t *loc, const Relocation &rel, uint64_t val) {
  const uint8_t inst[] = {
...
namespace {
void relaxGot(uint8_t *loc, const Relocation &rel, uint64_t val);
...
namespace {
void relaxGotNoPic(uint8_t *loc, uint64_t val, uint8_t op, uint8_t modRm) {
