...
namespace {
ELFKind getELFKind(MemoryBufferRef mb, StringRef archiveName) {
  unsigned char size;
  ...
  if (size == ELFCLASS32)
    report("32-bit ELF is not supported");
  if (size != ELFCLASS64)
    report("corrupted ELF file: invalid file class");

  size_t bufSize = mb.getBuffer().size();
  if (bufSize < sizeof(Elf64_Ehdr))
...
namespace {
bool isCompatible(InputFile *file) {
  if (!file->isElf() && !isa<BitcodeFile>(file))
    return true;

  // Only x86/x64 and AArch64 are supported.
...
void elf::parseFile(InputFile *file) { dispatchByFormat(doParseFile, file); }

// Concatenates arguments to construct a string representing an error location.
namespace {
std::string createFileLineMsg(StringRef path, unsigned line) {
...
template <class ELFT>
std::string getSrcMsgAux(ObjFile<ELFT> &file, const Symbol &sym,
...
// the various ways that a library can be specified. This ELF extension
// is a form of autolinking and is called `dependent libraries`. It is currently
// unique to LLVM and the NeverC linker.
namespace {
void addDependentLibrary(StringRef specifier, const InputFile *f) {
...
template <class ELFT>
void handleSectionGroup(ArrayRef<InputSectionBase *> sections,
...
template <class ELFT> DWARFCache *ObjFile<ELFT>::getDwarf() {
  llvm::call_once(initDwarf, [this]() {
    dwarf = std::make_unique<DWARFCache>(std::make_unique<DWARFContext>(
        std::make_unique<LinkerDwarfObj<ELFT>>(this), "",
...
const Elf_Shdr *findSection(ArrayRef<Elf_Shdr> sections, uint32_t type) {
...
template <class ELFT> void ObjFile<ELFT>::parse(bool ignoreComdats) {
  ...
  // parseFile loop, so concurrent DenseMap writes are not possible.
  //
...
template <class ELFT> static uint32_t readAndFeatures(const InputSection &sec) {
  ...
  while (!data.empty()) {
    ...
    while (!desc.empty()) {
      ...
      desc = desc.slice(alignTo<8>(size));
    }
...
InputSectionBase *ObjFile<ELFT>::createInputSection(uint32_t idx,
  ...
  if (name.starts_with(".n")) {
    ...
    // Therefore, we always add PT_GNU_STACK unless it is
...
template <class ELFT>
// ===----------------------------------------------------------------------===
// ObjFile: symbol initialization & postprocessing
// ===----------------------------------------------------------------------===
...
template <class ELFT>
void ObjFile<ELFT>::prepareSectionsAndLocals(bool ignoreComdats) {
  if (!justSymbols)
...
// Tentative definitions have two possible resolutions: either treat them as
// definitive (so archive members are only pulled when some other symbol
...
SmallVector<const void *, 0> parseVerdefs(const uint8_t *base,
...
namespace {
template <typename ELFT>
uint64_t getAlignment(ArrayRef<typename ELFT::Shdr> sections,
...
namespace {
ELFKind getBitcodeELFKind(const Triple &t) {
  return t.isLittleEndian() ? ELF64LEKind : ELF64BEKind;
}

uint16_t getBitcodeMachineKind(StringRef path, const Triple &t) {
  switch (t.getArch()) {
  case Triple::aarch64:
    return EM_AARCH64;
  case Triple::x86_64:
    return EM_X86_64;
  default:
    error(path + ": unsupported target architecture (triple: " + t.str() + ")");
    return EM_NONE;
  }
}

uint8_t getOsAbi(const Triple &t) { return ELF::ELFOSABI_NONE; }
} // namespace
...
namespace {
uint8_t mapVisibility(GlobalValue::VisibilityTypes gvVisibility) {
  switch (gvVisibility) {
...
void createBitcodeSymbol(Symbol *&sym, const std::vector<bool> &keptComdats,
...
void BitcodeFile::parse() {
  ...
  // NeverC frontend. The IR symbol table is the only signal available before
  // LTO finishes code generation; the .neverc.overrides section is parsed
  // separately when LTO-produced object files re-enter the link. Marker
  // names are already linker-mangled (emit side prepends "\01" to suppress
  // double mangling), so we compare against the linker-level name.
  //
  // Markers are extern_weak declarations with no section and no uses in IR.
  // They stay visible in the LTO IR symbol table, but we do not insert them
  ...
  // ObjFile<ELFT>::initializeSymbols. Skip override markers entirely:
  // BitcodeCompiler::add() will hand LTO a no-op resolution for their
  // slots, and they will not appear in the final symbol table.
  auto isMarker = [&](const lto::InputFile::Symbol &irSym) {
    return irSym.getName().starts_with(overridePrefix);
  };
  for (auto [i, irSym] : llvm::enumerate(obj->symbols()))
    if (!irSym.isUndefined() && !isMarker(irSym))
...
void BitcodeFile::postParse() {
  for (auto [i, irSym] : llvm::enumerate(obj->symbols())) {
    // Override marker slots are intentionally left nullptr in parse().
    if (!symbols[i])
      continue;
