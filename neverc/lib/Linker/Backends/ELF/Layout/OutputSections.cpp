...
namespace {
bool canMergeToProgbits(unsigned type) {
  ...
  if (LLVM_UNLIKELY(type != isec->type)) {
    if (hasInputSections || typeIsSet) {
      if (typeIsSet || !canMergeToProgbits(type) ||
        ...
        // NOLOAD sections use SHT_NOBITS; other mismatches are errors.
        if (type != SHT_NOBITS) {
...
void OutputSection::finalizeInputSections() {
  ...
  for (SectionCommand *cmd : commands) {
    ...
    for (InputSectionBase *s : isd->sectionBases) {
      ...
      // Match by flags + entsize; SHF_STRINGS also requires same alignment.
...
namespace {
void nopInstrFill(uint8_t *buf, size_t size) {
  if (size == 0)
    return;
  unsigned i = 0;
  if (size == 0)
...
void fill(uint8_t *buf, size_t size, const std::array<uint8_t, 4> &filler) {
  size_t i = 0;
  for (; i + 4 < size; i += 4)
...
template <class ELFT> void OutputSection::maybeCompress() {
  ...
  auto shardsIn =
      splitIntoChunks(ArrayRef<uint8_t>(buf.get(), size), shardSize);
...
namespace {
void writeInt(uint8_t *buf, uint64_t data, uint64_t size) {
  if (size == 1)
    *buf = data;
  else if (size == 2)
    write16(buf, data);
...
namespace {
void finalizeShtGroup(OutputSection *os, InputSection *section) {
...
bool compCtors(const InputSection *a, const InputSection *b) {
...
void OutputSection::checkDynRelAddends(const uint8_t *bufStart) {
  ...
  parallelFor(0, sections.size(), [&](size_t i) {
    // Only check synthetic reloc sections, skip passthrough input relocs.
