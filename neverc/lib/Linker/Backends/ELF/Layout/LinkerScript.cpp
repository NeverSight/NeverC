...
namespace {
bool isSectionPrefix(StringRef prefix, StringRef name) {
...
namespace {
void expandMemoryRegion(MemoryRegion *memRegion, uint64_t size,
                        StringRef secName) {
...
// ===----------------------------------------------------------------------===
// Section processing & insert commands
// ===----------------------------------------------------------------------===
...
namespace {
inline StringRef getFilename(const InputFile *file) {
...
namespace {
bool matchConstraints(ArrayRef<InputSectionBase *> sections,
                      ConstraintKind kind) {
...
void sortSections(MutableArrayRef<InputSectionBase *> vec,
                  SortSectionPolicy k) {
...
namespace {
void sortInputSections(MutableArrayRef<InputSectionBase *> vec,
                       SortSectionPolicy outer, SortSectionPolicy inner) {
...
LinkerScript::computeInputSections(const InputSectionDescription *cmd,
  ...
  for (const SectionPattern &pat : cmd->sectionPatterns) {
    ...
    for (size_t i = 0, e = sections.size(); i != e; ++i) {
      ...
      // don't double-place them; keep linker-synthesised SHT_REL[A] alone
      // so custom layout still works.
...
void LinkerScript::discardSynthetic(OutputSection &) {}
...
namespace {
OutputSection *findByName(ArrayRef<SectionCommand *> vec, StringRef name) {
...
OutputDesc *createSection(InputSectionBase *isec, StringRef outsecName) {
  OutputDesc *osd = script->createOutputSection(outsecName, "<internal>");
  osd->osec.recordSection(isec);
  return osd;
}

OutputDesc *addInputSec(StringMap<TinyPtrVector<OutputSection *>> &map,
...
namespace {
OutputSection *findFirstSection(PhdrEntry *load) {
...
namespace {
bool isDiscardable(const OutputSection &sec) {
  if (sec.name == "/DISCARD/")
    return true;
...
namespace {
void maybePropagatePhdrs(OutputSection &sec, SmallVector<StringRef, 0> &phdrs) {
  if (sec.phdrs.empty()) {
    // Only propagate program headers to allocated sections; non-allocated
    // sections never end up in a PT_LOAD anyway.
    if (sec.flags & SHF_ALLOC)
      sec.phdrs = phdrs;
  } else {
    phdrs = sec.phdrs;
...
void LinkerScript::adjustOutputSections() {
  // Sections that hold only symbol assignments still need a real output
  // section so the script's address arithmetic stays well-defined. The
  // tricky part is inheriting the right flags/type; without that we'd
  // accidentally reshape which PT_LOAD covers each region.
  ...
  // Generic scripts often define more output sections than a given link
  // actually fills. Drop the trivially empty ones so the output isn't
  // cluttered with zero-size sections.
...
namespace {
uint64_t computeBase(uint64_t min, bool allocateHeaders) {
...
// ===----------------------------------------------------------------------===
// Header allocation & final adjustments
// ===----------------------------------------------------------------------===
...
namespace {
std::optional<size_t> getPhdrIndex(ArrayRef<PhdrsCommand> vec, StringRef name) {
...
namespace {
void checkMemoryRegion(const MemoryRegion *region, const OutputSection *osec,
                       uint64_t addr) {
