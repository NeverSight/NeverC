...
namespace {
std::optional<std::string> getLinkerScriptLocation(const Symbol &sym) {
...
namespace {
std::string getLocation(InputSectionBase &s, const Symbol &sym, uint64_t off) {
...
void elf::reportRangeError(uint8_t *loc, const Relocation &rel, const Twine &v,
  ...
  if (rel.sym) {
    if (!rel.sym->isSection())
      hint = "; references '" + linker::toString(*rel.sym) + '\'';
  ...
  errorOrWarn(errPlace.loc + "relocation " + linker::toString(rel.type) +
...
void elf::reportRangeError(uint8_t *loc, int64_t v, int n, const Symbol &sym,
  ...
  if (!sym.getName().empty())
    hint = "; references '" + linker::toString(sym) + '\'' +
...
// Represent the set of RelExprs as a constant 64-bit bitmask for fast
// membership testing.
...
bool isAbsoluteValue(const Symbol &sym) {
  return isAbsolute(sym) || sym.isTls();
}

// Returns true if Expr refers a PLT entry.
bool needsPlt(RelExpr expr) {
  return oneof<R_PLT, R_PLT_PC, R_PLT_GOTPLT>(expr);
}
} // namespace

bool linker::elf::needsGot(RelExpr expr) {
...
namespace {
bool isRelExpr(RelExpr expr) {
  return oneof<R_PC, R_GOTREL, R_GOTPLTREL, R_AARCH64_PAGE_PC, R_RELAX_GOT_PC>(
      expr);
}
} // namespace

namespace {
RelExpr toPlt(RelExpr expr) {
  switch (expr) {
  case R_PC:
    return R_PLT_PC;
  case R_ABS:
...
namespace {
template <class ELFT>
SmallSet<SharedSymbol *, 4> getSymbolsAt(SharedSymbol &ss) {
...
// location.
namespace {
void replaceWithDefined(Symbol &sym, SectionBase &sec, uint64_t value,
                        uint64_t size) {
  Symbol old = sym;
...
// file of sym.
namespace {
const Symbol *getAlternativeSpelling(const Undefined &sym,
                                     std::string &pre_hint,
...
namespace {
void reportUndefinedSymbol(const UndefinedDiag &undef, bool correctSpelling) {
...
// ===----------------------------------------------------------------------===
// Symbol resolution & GOT/PLT allocation
// ===----------------------------------------------------------------------===
...
void addRelativeReloc(InputSectionBase &isec, uint64_t offsetInSec, Symbol &sym,
...
void addPltEntry(PltSection &plt, GotPltSection &gotPlt,
...
namespace {
void addTpOffsetGotEntry(Symbol &sym) {
  in.got->addEntry(sym);
  uint64_t off = sym.getGotOffset();
...
// library.
namespace {
bool canDefineSymbolInExecutable(Symbol &sym) {
...
bool RelocationScanner::isStaticLinkTimeConstant(RelExpr e, RelType type,
  ...
  if (oneof<R_GOTPLT, R_GOT_OFF, R_RELAX_HINT, R_AARCH64_GOT_PAGE_PC, R_GOT_PC,
            R_GOTONLY_PC, R_GOTPLTONLY_PC, R_PLT_PC, R_PLT_GOTPLT,
...
namespace {
unsigned handleTlsRelocation(RelType type, Symbol &sym, InputSectionBase &c,
                             uint64_t offset, int64_t addend, RelExpr expr) {
  ...
  //
  // AArch64 does not support TLS relaxations, however, we can omit the DTPMOD
  // dynamic relocations and resolve them at link time because they are
  ...
  if (oneof<R_GOT, R_GOTPLT, R_GOT_PC, R_AARCH64_GOT_PAGE_PC, R_GOT_OFF,
    ...
    if (toExecRelax && isLocalInExecutable) {
      ...
      // R_GOT needs a relative relocation for PIC.
...
template <class ELFT, class RelTy> void RelocationScanner::scanOne(RelTy *&i) {
  ...
  // marker relocations, e.g. R_*_NONE. Don't error on them.
  ...
  } else if (oneof<R_GOTONLY_PC, R_GOTREL>(expr)) {
    in.got->hasGotOffRel.store(true, std::memory_order_relaxed);
  }
...
namespace {
bool handleNonPreemptibleIfunc(Symbol &sym, uint16_t flags) {
  ...
  //   point to the PLT entry instead of the resolver. We also store
...
void elf::postScanRelocations() {
  auto fn = [](Symbol &sym) {
    ...
    if (flags & NEEDS_COPY) {
      if (sym.isObject()) {
        dispatchByFormat(addCopyRelSymbol, cast<SharedSymbol>(sym));
...
namespace {
bool mergeCmp(const InputSection *a, const InputSection *b) {
...
//
// We have a synthetic ThunkSection that can hold many Thunks.
...
//
// The implementation of Thunks is split across these areas
...
// Thunks that must be placed immediately before the callee section are modelled
// by Thunks that return the section they must precede with
// getTargetInputSection(). We can assume that the caller is in range of the
// Thunk.
//
// Range extension thunks must be placed within range of the caller. These
// thunks can always reach the callee as they use an indirect jump via a
// register that has no range restrictions.
//
// Thunk placement algorithm:
// For ThunkSections with a specific target; the placement is explicit, it has
// to be before getTargetInputSection().
//
...
//
// For the first pass, we pre-create one or more ThunkSections per
...
ThunkSection *ThunkCreator::addThunkSection(OutputSection *os,
  ...
  if (config->fixCortexA53Errata843419 && !isd->sections.empty()) {
    ...
    // has an assertion that what the linker represents as an
    // InputSectionDescription does not exceed 4 KiB even if the overall
    // OutputSection is > 128 Mib. We use the heuristic of rounding up the size
    // when both of the following conditions are true: 1.) The OutputSection is
    // larger than the ThunkSectionSpacing. This
...
namespace {
bool isThunkSectionCompatible(InputSection *source, SectionBase *target) {
...
std::pair<Thunk *, bool> ThunkCreator::getThunk(InputSection *isec,
  ...
  const int64_t keyAddend = rel.addend;
...
// AArch64, then any future change in offset between caller and callee risks a
