...
#include "Linker/ELF/ScriptLexer.h"
...
namespace {
ExprValue add(ExprValue a, ExprValue b) {
  moveAbsRight(a, b);
...
ExprValue bitAnd(ExprValue a, ExprValue b) {
  moveAbsRight(a, b);
...
ExprValue bitOr(ExprValue a, ExprValue b) {
  moveAbsRight(a, b);
  return {a.sec, a.forceAbsolute,
          (a.getValue() | b.getValue()) - a.getSecAddr(), a.loc};
}
...
// ===----------------------------------------------------------------------===
// Linker script command dispatch
// ===----------------------------------------------------------------------===
...
namespace {
std::pair<ELFKind, uint16_t> parseBfdName(StringRef s) {
...
void ScriptParser::readOutputFormat() {
  ...
  if (config->emachine != EM_AARCH64 && config->emachine != EM_X86_64)
    setError("unsupported output format in this build");
}

// ===----------------------------------------------------------------------===
// PHDR & section directives
// ===----------------------------------------------------------------------===
...
void ScriptParser::readTarget() {
  // TARGET(foo) is an alias for "--format foo". Accept only "elf*" and
  // "binary" -- everything else maps to the same set of output formats
  // we already understand on the command line.
  expect("(");
  StringRef tok = unquote(next());
  expect(")");
...
namespace {
Expr checkAlignment(Expr e, std::string &loc) {
  return [=] {
...
// Reads a `=<fillexp>` expression and returns the value as a 32-bit
// big-endian number. Symbols and arbitrary-length blobs are not supported
// and arithmetic expressions must be parenthesised to disambiguate from
// the following section list, e.g. `=(0x90+3)`.
...
namespace {
std::optional<uint64_t> parseInt(StringRef tok) {
  // Hexadecimal
  uint64_t val;
...
namespace {
std::optional<uint64_t> parseFlag(StringRef tok) {
...
namespace {
void checkIfExists(const OutputSection &osec, StringRef location) {
  if (osec.location.empty() && script->errorOnMissingSection)
    error(location + ": undefined section " + osec.name);
}

bool isValidSymbolName(StringRef s) {
  auto valid = [](char c) {
...
// ===----------------------------------------------------------------------===
// Version declarations & memory regions
// ===----------------------------------------------------------------------===
...
// Reads an "extern "C"" directive in a version script.
SmallVector<SymbolVersion, 0> ScriptParser::readVersionExtern() {
  StringRef tok = next();
  if (tok != "\"C\"")
    setError("unknown language (only extern \"C\" is supported)");
  expect("{");

  SmallVector<SymbolVersion, 0> ret;
  while (!errorCount() && peek() != "}") {
    StringRef tok = next();
    ret.push_back({unquote(tok), !tok.starts_with("\"") && hasWildcard(tok)});
    if (consume("}"))
      return ret;
    expect(";");
  }
