...
public:
  ...
  void run() {
    ErrorOr<SmallString<256>> exeOrErr = sys::findProgramByName(prog);
    if (auto ec = exeOrErr.getError())
      fatal("unable to find " + prog + " in PATH: " + ec.message());
    StringRef exe = saver().save(StringRef(*exeOrErr));
...
uint32_t parseSectionAttributes(StringRef s) {
  uint32_t ret = 0;
  for (char c : s.lower()) {
    switch (c) {
    case 'd':
...
void LinkerDriver::parseSwaprun(StringRef arg) {
  do {
    ...
    // To catch trailing commas, e.g. `--swaprun=cd,`
...
public:
  TemporaryFile(StringRef prefix, StringRef extn, StringRef contents = "") {
    SmallString<128> s;
    if (auto ec =
            sys::fs::createTemporaryFile("neverc-linker-" + prefix, extn, s))
...
std::string LinkerDriver::createDefaultXml() {
  ...
  // Intentionally no XML attribute validation — matches link.exe behavior.
...
LinkerDriver::createManifestXmlWithExternalMt(StringRef defaultXml) {
  ...
  // Merge user-supplied manifests with Microsoft's tool. NeverC does not build
  // LLVM's libxml2-based internal manifest merger.
...
void writeResFileHeader(char *&buf) {
...
void writeResEntryHeader(char *&buf, size_t manifestSize, int manifestID) {
...
StringRef undecorate(COFFLinkerContext &ctx, StringRef sym) {
  if (sym.starts_with("_") && sym.contains('@'))
    return sym;
  return sym.starts_with("_") ? sym.substr(1) : sym;
}

// Convert stdcall/fastcall symbols to unsuffixed form.
StringRef killAt(StringRef sym, bool prefix) {
  if (sym.empty())
    return sym;
  ...
  // Fastcall: replace leading @ with _ when prefixes are used.
  sym = sym.substr(1);
  if (prefix)
...
StringRef exportSourceName(ExportSource s) {
  switch (s) {
...
// ===----------------------------------------------------------------------===
// Mismatch checking & resource conversion
// ===----------------------------------------------------------------------===
...
// In-process cvtres: convert .res files to COFF object.
MemoryBufferRef LinkerDriver::convertResToCOFF(ArrayRef<MemoryBufferRef> mbs,
                                               ArrayRef<ObjFile *> objs) {
  object::WindowsResourceParser parser(false);
...
// ===----------------------------------------------------------------------===
// Option table & argument parsing
// ===----------------------------------------------------------------------===

// Prefix string literals used in Options.td
...
constexpr llvm::opt::OptTable::Info infoTable[] = {
...
opt::InputArgList ArgParser::parse(ArrayRef<const char *> argv) {
  ...
  // fatalWarnings is now set from LinkerDriverConfig before parse() is
  // called; /WX has been removed from Options.td.
...
static StringRef normalizeMsvcDirective(StringRef tok) {
  ...
  auto [opt, val] = s.split(':');
  if (val.empty())
    return saver().save(Twine("--") + opt);
  return saver().save(Twine("--") + opt + "=" + val);
}
