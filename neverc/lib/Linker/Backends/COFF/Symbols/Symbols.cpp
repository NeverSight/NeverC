...
// Returns a symbol name for an error message. NeverC is C-only so there is no
// C++ name mangling to undo; we only strip the `__imp_` dllimport prefix.
namespace {
...
namespace {
Chunk *makeImportThunk(COFFLinkerContext &ctx, DefinedImportData *s,
                       uint16_t machine) {
  if (machine == AMD64)
