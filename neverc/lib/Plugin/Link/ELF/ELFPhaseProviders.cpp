...
Error phaseError(const Twine &Message) {
  return createStringError(errc::invalid_argument,
                           "ELF link phase adapter: " + Message);
}

} // namespace

Error ELFLinkGraphAdapter::advanceTo(NevercLinkState State) {
  if (!Graph)
  ...
  while (Graph->state() < State) {
    ...
    if (Error E = (*Pipeline)->setBuiltinGraphProvider(
      ...
      return joinErrors(
          phaseError("could not install native ELF phase provider"),
          std::move(E));

    auto Output =
        (*Pipeline)->execute(Graph, Transition->OutputState);
    if (!Output)
    ...
    if (Error E =
            applyDelta(*LastNative, **Output, Transition->OutputState))
