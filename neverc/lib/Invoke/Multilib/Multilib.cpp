...
llvm::raw_ostream &neverc::driver::operator<<(llvm::raw_ostream &OS,
                                              const Multilib &M) {
  M.print(OS);
  return OS;
}
...
namespace {
const llvm::VersionTuple MultilibVersionCurrent(1, 0);
...
template <> struct llvm::yaml::MappingTraits<MultilibSet::FlagMatcher> {
  ...
  static std::string validate(IO &io, MultilibSet::FlagMatcher &M) {
    ...
    if (!Regex.isValid(RegexError))
      return std::string(RegexError.data(), RegexError.size());
    if (M.Flags.empty())
...
MultilibSet::parseYaml(llvm::MemoryBufferRef Input,
  ...
  YamlInput >> MS;
  if (YamlInput.error())
    return std::error_code(YamlInput.error(), std::generic_category());
