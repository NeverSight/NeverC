...
ARRAY_QUERY_PATCHES = (('clang/lib/Sema/SemaExprCXX.cpp',
...
PARTIAL_ARGUMENT_BOUNDS_BEFORE = '''  TemplateParameterList *TemplateParams =
...
'''
PARTIAL_ARGUMENT_BOUNDS_AFTER = '''  TemplateParameterList *TemplateParams =
      PrimaryTemplate->getTemplateParameters();
  for (unsigned I = 0, N = TemplateParams->size(); I != N; ++I) {
...
class PartialSpecializationArgumentSourceTests(unittest.TestCase):
    def test_argument_bound_repair_checks_pinned_source_states(self):
        script = Path(__file__).resolve().with_name("IsolateSymbols.py")
        function = next(node for node in ast.parse(script.read_text()).body
                        if isinstance(node, ast.FunctionDef) and
                        node.name == "fix_partial_specialization_argument_bounds")
        namespace = {}
        exec(compile(ast.Module(body=[function], type_ignores=[]), str(script), "exec"), namespace)
        repair = namespace[function.name]
        with tempfile.TemporaryDirectory(prefix="neverc-partial-argument-source-") as temporary:
            path = Path(temporary) / "SemaTemplate.cpp"
            path.write_text(PARTIAL_ARGUMENT_BOUNDS_BEFORE)
            repair(path)
            self.assertEqual(path.read_text(), PARTIAL_ARGUMENT_BOUNDS_AFTER)
            repair(path)
            self.assertEqual(path.read_text(), PARTIAL_ARGUMENT_BOUNDS_AFTER)
            for name, contents in {
                "missing": None,
                "empty": "",
                "duplicate original": PARTIAL_ARGUMENT_BOUNDS_BEFORE * 2,
                "duplicate rewritten": PARTIAL_ARGUMENT_BOUNDS_AFTER * 2,
                "mixed": PARTIAL_ARGUMENT_BOUNDS_BEFORE + PARTIAL_ARGUMENT_BOUNDS_AFTER,
                "drift": PARTIAL_ARGUMENT_BOUNDS_BEFORE.replace("I != N", "I < N"),
                "orphan marker": (PARTIAL_ARGUMENT_BOUNDS_BEFORE +
                                  "// NeverC invalid pack frontiers"),
            }.items():
                with self.subTest(state=name):
                    if contents is None:
                        path.unlink()
    ...
    def test_pinned_dimension_repairs_are_atomic_and_idempotent(self):
        ...
        with tempfile.TemporaryDirectory(prefix="neverc-array-query-source-") as temporary:
            ...
            def snapshot():
                return {str(path.relative_to(root)): path.read_bytes()
                        for path in root.rglob("*") if path.is_file()}
            reset([0] * len(ARRAY_QUERY_PATCHES))
            repair(root)
            ...
            for index in range(len(ARRAY_QUERY_PATCHES)):
                states = [0] * len(ARRAY_QUERY_PATCHES)
                states[index] = 1
                reset(states)
                prior = snapshot()
                with self.assertRaises(SystemExit):
...
'''
        ...
        # The real patch step runs the separate argument-bound repair on the
        # same SemaTemplate.cpp fixture after the source-retention group.
        explicit_source_original += PARTIAL_ARGUMENT_BOUNDS_BEFORE
        explicit_source_expected += PARTIAL_ARGUMENT_BOUNDS_AFTER
