...
class FunctionLowering {
  ...
  Expression utilityOperation(const CallExpr *Call, UtilityOperation Operation,
    ...
    auto StringFor = [&](QualType Type) {
      return approvedUtilityStringRecord(
          A.S, A.Sources, Type.isNull() ? nullptr : Type->getAsCXXRecordDecl(),
          A.Context);
    };
    ...
    case UtilityOperation::AlgorithmBinarySearch: {
      ...
      if (Call->getNumArgs() == 4) {
        ...
        if (Upper) {
          ...
          if (Search && SDKForward) {
            SDKReverse = approvedRangeAlgorithmComparator(
                A.S, A.Sources, Call, 3, Value, Element, A.Context);
            if (!SDKReverse)
              reject(L, "algorithm binary_search",
                     "The standard comparator lacks the reverse comparison.");
          }
        }
    ...
    case UtilityOperation::TupleApply: {
      const auto Tuple = approvedUtilityTupleLikeSource(
          A.S, A.Sources, Call->getArg(1)->getType(), A.Context, true, true);
      if (!Tuple)
      ...
      if (MemberCallable) {
        ...
        if (MemberCallable->ObjectWrapper) {
          Receiver = snapshot(
              ReferenceMember(std::move(ReceiverStorage),
                              *MemberCallable->ObjectWrapper),
              L);
        } else if (MemberCallable->ObjectIsPointer) {
          Receiver = snapshot(std::move(ReceiverStorage), L);
        } else {
          Receiver = snapshot(
              address(std::move(ReceiverStorage),
                      MemberCallable->Object->getType(), L),
              L);
        }
        if (MemberCallable->Method) {
          ...
          if (HasRecordResult) {
            ...
            if (Result.getString("type") != type(Method->getReturnType(), L))
              reject(L, "utility tuple apply result",
                     "Call and result destination types differ.");
            Arguments.push_back(snapshot(
                address(Result, Method->getReturnType().getUnqualifiedType(),
                        L),
                L));
          } else if (Destination) {
            reject(L, "utility tuple apply result",
          ...
          Arguments.push_back(snapshot(
              cast(std::move(Receiver), type(Method->getThisType(), L), L),
              L));
      ...
      if (ReferenceCallable) {
        ...
        const auto *Method =
            ReferenceCallable->Kind ==
                ...
                ? ReferenceCallable->Method
                : nullptr;
        const auto *Prototype =
            !Method &&
                    ReferenceCallable->FunctionPointerType
        ...
        if (Method) {
          ...
          CallArguments.push_back(snapshot(
              cast(std::move(Referent), type(Method->getThisType(), L), L),
              L));
    ...
    case UtilityOperation::PairGetSecond: {
      ...
      auto Element = fieldStorage(
          ...
          Operation == UtilityOperation::PairGetFirst ? Pair->First
                                                      : Pair->Second,
          L);
      const auto *Field = Operation == UtilityOperation::PairGetFirst
                              ? Pair->First
                              : Pair->Second;
      return Field->getType()->isReferenceType()
                 ? dereference(std::move(Element), L)
    ...
    case UtilityOperation::StringAssignOperatorList: {
      ...
      const auto LongFlag = String->AlternateLayout
                                ? uint64_t(1)
                                      << (A.Context.getTypeSize(A.Context.getSizeType()) - 1)
                                : uint64_t(1);
      auto Word = [&](const Expression &Address, const char *Name) {
        return Expression{
            {"kind", "member"},
            {"type", type(llvm::StringRef(Name) == PointerName
                              ? String->PointerType : A.Context.getSizeType(),
                          L)},
            {"name", Name},
            {"args", json::Array{dereference(json::Object(Address), L)}},
            {"loc", A.loc(L)}};
      };
      auto ShortData = [&](const Expression &Address,
                           llvm::StringRef ResultType) {
        auto Pointer = cast(cast(json::Object(Address), "ptr:void", L),
                            ResultType, L);
        return String->AlternateLayout
                   ? std::move(Pointer)
                   : binary("+", std::move(Pointer),
                            quantity(1, DifferenceType, L), ResultType, L);
      };
      auto IsLong = [&](Expression Flag) {
        return binary("!=",
                      binary("&", std::move(Flag),
                             quantity(LongFlag, SizeType, L), SizeType, L),
                      quantity(0, SizeType, L), "bool", L);
      };
      ...
      if (Fill) {
        ...
        auto SourceAddress = snapshot(
            ...
            L);
        auto SourceFirst = snapshot(Word(SourceAddress, FlagName), L);
        auto SourceData = temporary(ConstPointerType, L);
        const auto Long = labelName(), Short = labelName(), Ready = labelName();
        branch(IsLong(json::Object(SourceFirst)), Long, Short, L);
        label(Long, L);
        assign(NewSize, Word(SourceAddress, "nct_string_word1"), L);
        assign(SourceData,
               cast(Word(SourceAddress, PointerName), ConstPointerType, L), L);
        jump(Ready, L);
        label(Short, L);
        assign(NewSize,
               String->AlternateLayout
        ...
        if (Operation == UtilityOperation::StringAssignPointer) {
          ...
          assign(NewSize, quantity(0, SizeType, L), L);
          const auto Check = labelName(), Advance = labelName(),
                     Scanned = labelName();
          jump(Check, L);
          label(Check, L);
      ...
      auto First = snapshot(Word(Receiver, FlagName), L);
      auto OldLong = temporary("bool", L);
      auto Capacity = temporary(SizeType, L);
      auto Data = temporary(PointerType, L);
      const auto Long = labelName(), Short = labelName(), Ready = labelName();
      branch(IsLong(json::Object(First)), Long, Short, L);
      label(Long, L);
      ...
      auto WriteBytes = [&](Expression TargetData) {
        ...
        assign(dereference(json::Object(Target), L),
               Fill ? json::Object(*CharacterArgument)
      ...
      const auto Grow = labelName(), Reuse = labelName(), Done = labelName();
      branch(binary(">", json::Object(NewSize), json::Object(Capacity),
                    "bool", L), Grow, Reuse, L);
      label(Grow, L);
      ...
      branch(binary(">", json::Object(NewSize),
      ...
      assign(NewData, cast(std::move(Allocation), PointerType, L), L);
      WriteBytes(json::Object(NewData));
      ...
      DeleteArgs.push_back(cast(json::Object(Data),
                                type(Delete->getParamDecl(0)->getType(), L),
                                L));
      if (Delete->getNumParams() == 2)
        DeleteArgs.push_back(cast(binary("+", json::Object(Capacity),
                                         quantity(1, SizeType, L), SizeType, L),
                                  type(Delete->getParamDecl(1)->getType(), L),
                                  L));
      ...
      jump(Install, L);
      label(Install, L);
      assign(Word(Receiver, PointerName), json::Object(NewData), L);
      assign(Word(Receiver, FlagName),
             binary("|", json::Object(AllocationBytes),
                    quantity(LongFlag, SizeType, L), SizeType, L), L);
      assign(Word(Receiver, "nct_string_word1"), json::Object(NewSize), L);
      jump(Done, L);
      label(Reuse, L);
      WriteBytes(json::Object(Data));
      ...
      assign(Word(Receiver, FlagName),
             binary("|", binary("&", Word(Receiver, FlagName),
                                std::move(Mask), SizeType, L),
    ...
    case UtilityOperation::StringShrinkToFit: {
      const auto *Object = MemberObject();
      auto String = Object ? StringFor(Object->getType())
                           : std::optional<UtilityStringRecord>();
      if (!Object || !String)
        reject(L, "string shrink_to_fit",
               "The selected std::string layout is unavailable.");
      auto Receiver =
          snapshot(address(lvalue(Object), Object->getType(), L), L);
      const auto SizeType = type(A.Context.getSizeType(), L);
      ...
      const uint64_t LongFlag =
          String->AlternateLayout ? uint64_t(1) << (SizeBits - 1) : uint64_t(1);
      const bool UsesLowBit =
          String->AlternateLayout != A.Context.getTargetInfo().isLittleEndian();
      auto Word = [&](const char *Name) {
        return Expression{
            {"kind", "member"},
            {"type", type(llvm::StringRef(Name) == PointerName
      ...
      branch(binary("!=",
             ...
             Long, Done, L);
      label(Long, L);
      auto Size = snapshot(Word("nct_string_word1"), L);
      auto Capacity =
          snapshot(binary("-",
                          binary("-", json::Object(First),
                                 quantity(LongFlag, SizeType, L), SizeType, L),
                          quantity(1, SizeType, L), SizeType, L),
                   L);
      auto TargetCapacity = temporary(SizeType, L);
      const auto Short = labelName(), Recommend = labelName();
      const auto Recommended = labelName();
      branch(binary("<=", json::Object(Size),
                    quantity(String->ShortCapacity, SizeType, L), "bool", L),
             Short, Recommend, L);
      label(Short, L);
      assign(TargetCapacity, quantity(String->ShortCapacity, SizeType, L), L);
      jump(Recommended, L);
      label(Recommend, L);
      // Match the pinned libc++ __recommend(size()) rounding and its first
      // long-allocation adjustment for the selected string layout.
      assign(TargetCapacity,
             binary("-",
                    binary("*",
                           binary("/",
                    ...
                    quantity(1, SizeType, L), SizeType, L),
             L);
      const auto Adjust = labelName(), Unadjusted = labelName();
      branch(binary("==", json::Object(TargetCapacity),
                    quantity(String->ShortCapacity + 1, SizeType, L), "bool",
                    L),
             Adjust, Unadjusted, L);
      label(Adjust, L);
      assign(TargetCapacity,
             binary("+", json::Object(TargetCapacity),
                    quantity(UsesLowBit ? 2 : 1, SizeType, L), SizeType, L),
             L);
      jump(Unadjusted, L);
      label(Unadjusted, L);
      jump(Recommended, L);
      label(Recommended, L);
      const auto Shrink = labelName();
      branch(binary("!=", json::Object(TargetCapacity), json::Object(Capacity),
                    "bool", L),
             Shrink, Done, L);
      label(Shrink, L);
      auto OldData = snapshot(cast(Word(PointerName), PointerType, L), L);
      auto NewData = temporary(PointerType, L);
      const auto Inline = labelName(), Allocate = labelName();
      const auto Copy = labelName();
      branch(binary("<=", json::Object(TargetCapacity),
                    quantity(String->ShortCapacity, SizeType, L), "bool", L),
             Inline, Allocate, L);
      label(Inline, L);
      ...
      const auto *New = A.allocatorHeapFunction(true, A.Context.CharTy, L);
      json::Array NewArgs;
      NewArgs.push_back(binary("+", json::Object(TargetCapacity),
                               quantity(1, SizeType, L), SizeType, L));
      chargeCall(NewArgs, L);
      auto Allocation = temporary(type(New->getReturnType(), L), L);
      Body.push_back(json::Object{{"op", "call"},
      ...
      auto Count = temporary(SizeType, L);
      assign(SourceCurrent, json::Object(OldData), L);
      assign(TargetCurrent, json::Object(NewData), L);
      assign(Count, quantity(0, SizeType, L), L);
      const auto Check = labelName(), CopyByte = labelName();
      ...
      const auto *Delete = A.allocatorHeapFunction(false, A.Context.CharTy, L);
      json::Array DeleteArgs;
      DeleteArgs.push_back(cast(json::Object(OldData),
                                type(Delete->getParamDecl(0)->getType(), L),
                                L));
      if (Delete->getNumParams() == 2)
        DeleteArgs.push_back(cast(binary("+", json::Object(Capacity),
      ...
      Body.push_back(json::Object{{"op", "call"},
                                  ...
                                  {"args", std::move(DeleteArgs)},
                                  {"loc", A.loc(L)}});
      const auto CommitShort = labelName(), CommitLong = labelName();
      branch(binary("<=", json::Object(TargetCapacity),
                    quantity(String->ShortCapacity, SizeType, L), "bool", L),
             CommitShort, CommitLong, L);
      label(CommitShort, L);
      auto Mask =
          String->AlternateLayout
              ? quantity((uint64_t(1) << (SizeBits - 8)) - 1, SizeType, L)
      ...
      auto Encoded =
          String->AlternateLayout
              ? binary("<<", json::Object(Size),
                       quantity(SizeBits - 8, SizeType, L), SizeType, L)
              : binary("*", json::Object(Size), quantity(2, SizeType, L),
                       SizeType, L);
      assign(Word(FlagName),
             binary("|",
                    binary("&", Word(FlagName), std::move(Mask), SizeType, L),
    ...
    case UtilityOperation::StringEraseIterator: {
      ...
      if (Operation == UtilityOperation::StringAppendString) {
        ...
        auto SourceWord = [&](const char *Name) {
          ...
          return Expression{
              {"kind", "member"},
              {"type",
               type(PointerWord ? String->PointerType : A.Context.getSizeType(),
                    L)},
              {"name", Name},
              {"args",
               json::Array{dereference(json::Object(SourceAddress), L)}},
              {"loc", A.loc(L)}};
        };
        auto SourceFirst = snapshot(SourceWord(FlagName), L);
        auto SourceSize = temporary(SizeType, L);
        auto SourceData = temporary(ConstPointerType, L);
        const auto Long = labelName(), Short = labelName(), Ready = labelName();
        const auto LongFlag =
            String->AlternateLayout
        ...
        branch(binary("!=",
               ...
               Long, Short, L);
        label(Long, L);
        assign(SourceSize, SourceWord("nct_string_word1"), L);
        assign(SourceData, cast(SourceWord(PointerName), ConstPointerType, L),
               L);
        jump(Ready, L);
        label(Short, L);
        assign(
        ...
        auto ShortPointer =
            cast(cast(json::Object(SourceAddress), "ptr:void", L),
                 ConstPointerType, L);
        assign(SourceData,
               String->AlternateLayout ? std::move(ShortPointer)
                                       : binary("+", std::move(ShortPointer),
      ...
      if (Operation == UtilityOperation::StringPushBack ||
        ...
        auto Data = temporary(PointerType, L);
        const auto Long = labelName(), Short = labelName(), Ready = labelName();
        branch(binary("!=", binary("&", json::Object(First),
                                    quantity(LongFlag, SizeType, L), SizeType, L),
        ...
        if (Operation != UtilityOperation::StringPopBack) {
          ...
          auto TargetCurrent = temporary(PointerType, L);
          auto Count = temporary(SizeType, L);
          assign(SourceCurrent, json::Object(Data), L);
          assign(TargetCurrent, json::Object(NewData), L);
          assign(Count, quantity(0, SizeType, L), L);
        ...
        assign(dereference(binary("+", std::move(Data),
      ...
      if (Operation == UtilityOperation::StringSize ||
        ...
        jump(Done, L);
        label(Done, L);
        if (Operation == UtilityOperation::StringEmpty)
          return binary("==", std::move(Result), quantity(0, SizeType, L),
                        "bool", L);
        return Result;
      }
    ...
    case UtilityOperation::VectorEmplace: {
      ...
      auto Iterator = approvedUtilityWrapIteratorRecord(
          A.S, A.Sources, Call->getArg(0)->getType()->getAsCXXRecordDecl(),
          A.Context);
      if (!Object || !Vector || !Result || !Iterator)
        reject(L, "vector insert",
               "The selected std::vector iterator layout is unavailable.");
      auto Receiver =
          snapshot(address(lvalue(Object), Object->getType(), L), L);
      auto PositionValue = snapshot(expression(Call->getArg(0)), L);
      auto PositionPointer = snapshot(
          fieldStorage(std::move(PositionValue), Iterator->Current, L), L);
      const auto SizeType = type(A.Context.getSizeType(), L);
      const auto DifferenceType = type(A.Context.getPointerDiffType(), L);
      ...
      auto Size = temporary(SizeType, L);
      auto Capacity = temporary(SizeType, L);
      auto Offset = temporary(DifferenceType, L);
      assign(Size, quantity(0, SizeType, L), L);
      assign(Capacity, quantity(0, SizeType, L), L);
      assign(Offset, quantity(0, DifferenceType, L), L);
      ...
      assign(Capacity,
             ...
             L);
      assign(Offset,
             binary("-", json::Object(PositionPointer),
      ...
      const auto Nonempty = labelName(), Positioned = labelName();
      branch(cast(json::Object(Begin), "bool", L), Nonempty, Positioned, L);
      label(Nonempty, L);
      assign(Position,
             binary("+", json::Object(Begin), json::Object(Offset), PointerType,
                    L),
             L);
      jump(Positioned, L);
      label(Positioned, L);
      const auto InPlace = labelName(), Grow = labelName(), Done = labelName();
      const auto Nonzero = labelName();
      branch(binary("==", json::Object(Count), quantity(0, SizeType, L), "bool",
                    L),
             Done, Nonzero, L);
      label(Nonzero, L);
      auto Needed = snapshot(
          binary("+", json::Object(Size), json::Object(Count), SizeType, L), L);
      branch(
    ...
    case UtilityOperation::VectorAssignList: {
      ...
      if (Operation == UtilityOperation::VectorAssignFill) {
        ...
        if (Operation == UtilityOperation::VectorAssignList ||
          ...
          const auto SourceType = Call->getArg(0)->getType();
          auto Wrapped = approvedUtilityWrapIteratorRecord(
              A.S, A.Sources, SourceType->getAsCXXRecordDecl(), A.Context);
          Expression RangeEnd;
          if (Wrapped) {
      ...
      jump(Measured, L);
      label(Measured, L);
      const auto Reuse = labelName(), Grow = labelName(), Done = labelName();
      branch(
      ...
      const auto ReuseCheck = labelName(), ReuseFill = labelName();
      const auto ReuseDone = labelName();
      jump(ReuseCheck, L);
      label(ReuseCheck, L);
      ...
      const auto *New = A.allocatorHeapFunction(true, Vector->ElementType, L);
      json::Array NewArgs;
      NewArgs.push_back(binary("*", json::Object(Count),
                               quantity(ElementBytes, SizeType, L), SizeType,
                               L));
      chargeCall(NewArgs, L);
      auto Allocation = temporary(type(New->getReturnType(), L), L);
      ...
      auto GrowIndex = temporary(SizeType, L);
      assign(GrowIndex, quantity(0, SizeType, L), L);
      const auto GrowCheck = labelName(), GrowFill = labelName();
      const auto GrowDone = labelName();
      jump(GrowCheck, L);
      label(GrowCheck, L);
      branch(
          binary("<", json::Object(GrowIndex), json::Object(Count), "bool", L),
    ...
    case UtilityOperation::VectorResize:
    case UtilityOperation::VectorResizeFill: {
      const auto *Object = MemberObject();
      auto Vector = Object ? VectorFor(Object->getType())
                           : std::optional<UtilityVectorRecord>();
      if (!Object || !Vector)
        reject(L, "vector capacity modifier",
               "The selected std::vector layout is unavailable.");
      auto Receiver =
          snapshot(address(lvalue(Object), Object->getType(), L), L);
      ...
      jump(Measured, L);
      label(Measured, L);
      const auto Allocate = labelName(), Fill = labelName(), Done = labelName();
      auto TargetSize = temporary(SizeType, L);
      auto NewCapacity = temporary(SizeType, L);
      auto TargetBegin = temporary(PointerType, L);
      auto TargetCurrent = temporary(PointerType, L);
      auto Grew = temporary("bool", L);
      if (Operation == UtilityOperation::VectorReserve) {
        const auto Grow = labelName();
        branch(binary(">", json::Object(Requested), json::Object(Capacity),
                      "bool", L), Grow, Done, L);
        label(Grow, L);
        assign(TargetSize, json::Object(Size), L);
        assign(NewCapacity, json::Object(Requested), L);
        jump(Allocate, L);
      } else {
        const auto Shrink = labelName(), Expand = labelName();
        branch(binary("<", json::Object(Requested), json::Object(Size),
                      "bool", L), Shrink, Expand, L);
        label(Shrink, L);
        const auto Zero = labelName(), Nonzero = labelName();
        branch(binary("==", json::Object(Requested),
        ...
        label(Existing, L);
        assign(TargetSize, json::Object(Requested), L);
        assign(TargetBegin, json::Object(Begin), L);
        assign(TargetCurrent, json::Object(End), L);
        assign(Grew, boolean(false, L), L);
        jump(Fill, L);
        label(Grow, L);
        assign(TargetSize, json::Object(Requested), L);
        assign(NewCapacity, json::Object(Requested), L);
        const auto Double = labelName(), Recommended = labelName();
        branch(binary(">", binary("*", json::Object(Capacity),
                                  quantity(2, SizeType, L), SizeType, L),
                      json::Object(Requested), "bool", L), Double,
               Recommended, L);
        label(Double, L);
        assign(NewCapacity,
               binary("*", json::Object(Capacity), quantity(2, SizeType, L),
      ...
      auto TargetEnd = snapshot(binary("+", json::Object(TargetBegin),
                                       cast(json::Object(TargetSize),
                                            DifferenceType, L), PointerType,
                                       L), L);
      const auto FillCheck = labelName(), Append = labelName();
      const auto Filled = labelName();
      jump(FillCheck, L);
      label(FillCheck, L);
      ...
      jump(FillCheck, L);
      label(Filled, L);
      const auto Release = labelName(), InPlace = labelName();
      branch(json::Object(Grew), Release, InPlace, L);
      label(InPlace, L);
      assign(Member("nct_vector_end"), json::Object(TargetCurrent), L);
      jump(Done, L);
      label(Release, L);
      const auto FreeOld = labelName(), Commit = labelName();
      branch(cast(json::Object(Begin), "bool", L), FreeOld, Commit, L);
      ...
      jump(Commit, L);
      label(Commit, L);
      assign(Member("nct_vector_begin"), json::Object(TargetBegin), L);
      assign(Member("nct_vector_end"), json::Object(TargetCurrent), L);
      assign(Member("nct_vector_capacity"),
    ...
    case UtilityOperation::StringConcat: {
      auto String = StringFor(Call->getType());
      if (!String || Call->getNumArgs() != 2)
        reject(L, "string concatenation",
               "The selected std::string result is unavailable.");
      const auto SizeType = type(A.Context.getSizeType(), L);
      const auto DifferenceType = type(A.Context.getPointerDiffType(), L);
      const auto PointerType = type(String->PointerType, L);
      const auto ConstPointerType =
      ...
      for (unsigned I = 0; I != 2; ++I) {
        ...
        Operands.push_back(
            StringFor(Arg->getType())
                ? snapshot(address(lvalue(Arg), Arg->getType(), L), L)
                : snapshot(expression(Arg), L));
      }
      auto Span = [&](unsigned I) {
        const auto *Arg = Call->getArg(I);
        if (StringFor(Arg->getType()))
          return ReadStringAt(json::Object(Operands[I]), *String);
        if (Arg->getType()->isPointerType())
          return ReadCStringAt(json::Object(Operands[I]));
        return std::pair<Expression, Expression>{
            cast(address(json::Object(Operands[I]), A.Context.CharTy, L),
                 ConstPointerType, L),
            quantity(1, SizeType, L)};
      };
      auto [LeftData, LeftSize] = Span(0);
      auto [RightData, RightSize] = Span(1);
      auto Length = snapshot(binary("+", json::Object(LeftSize),
                                    json::Object(RightSize), SizeType, L),
                             L);
      auto Place = Destination ? std::move(*Destination)
                               : objectTemporary(Call->getType(), L);
      Destination.reset();
      if (Place.getString("type") != type(Call->getType(), L))
        reject(L, "string concatenation",
               "The concatenation destination type differs from the result.");
      const char *FlagName =
          String->AlternateLayout ? "nct_string_word2" : "nct_string_word0";
      const char *PointerName =
          String->AlternateLayout ? "nct_string_word0" : "nct_string_word2";
      ...
      const auto *Function = Call->getDirectCallee();
      const bool RvalueLeft =
          Function &&
          Function->getParamDecl(0)->getType()->isRValueReferenceType();
      const bool RvalueRight =
          Function &&
          Function->getParamDecl(1)->getType()->isRValueReferenceType();
      if (RvalueLeft || RvalueRight) {
        const unsigned TargetIndex = RvalueLeft ? 0 : 1;
        auto TargetAddress = json::Object(Operands[TargetIndex]);
        auto TargetData =
            RvalueLeft ? json::Object(LeftData) : json::Object(RightData);
        auto TargetSize =
            RvalueLeft ? json::Object(LeftSize) : json::Object(RightSize);
        auto TargetWord = [&](const char *Name) {
          return Expression{{"kind", "member"},
                            {"type", type(llvm::StringRef(Name) == PointerName
                                              ? String->PointerType
                                              : A.Context.getSizeType(),
        ...
        const auto LongFlag =
            String->AlternateLayout
                ? uint64_t(1)
                      << (A.Context.getTypeSize(A.Context.getSizeType()) - 1)
                : uint64_t(1);
        auto First = snapshot(TargetWord(FlagName), L);
        auto OldLong = snapshot(
            binary("!=",
                   binary("&", json::Object(First),
                          quantity(LongFlag, SizeType, L), SizeType, L),
                   quantity(0, SizeType, L), "bool", L),
            L);
        auto Capacity = temporary(SizeType, L);
        const auto LongCapacity = labelName(), ShortCapacity = labelName(),
                   CapacityReady = labelName();
        branch(json::Object(OldLong), LongCapacity, ShortCapacity, L);
        label(LongCapacity, L);
        assign(Capacity,
               binary("-",
                      binary("-", json::Object(First),
                             quantity(LongFlag, SizeType, L), SizeType, L),
               ...
               L);
        jump(CapacityReady, L);
        label(ShortCapacity, L);
        assign(Capacity, quantity(String->ShortCapacity, SizeType, L), L);
        jump(CapacityReady, L);
        label(CapacityReady, L);
        auto CopyBytes = [&](Expression To, Expression From,
                             Expression Amount) {
          auto Target = temporary(PointerType, L);
          auto Source = temporary(ConstPointerType, L);
          auto Count = temporary(SizeType, L);
          assign(Target, std::move(To), L);
          assign(Source, std::move(From), L);
          assign(Count, quantity(0, SizeType, L), L);
          const auto Check = labelName(), Write = labelName(),
                     Done = labelName();
          jump(Check, L);
          label(Check, L);
          branch(binary("<", json::Object(Count), std::move(Amount), "bool", L),
                 Write, Done, L);
          label(Write, L);
          assign(dereference(json::Object(Target), L),
                 dereference(json::Object(Source), L), L);
        ...
        const auto Grow = labelName(), Reuse = labelName(), Move = labelName();
        branch(binary(">", json::Object(Length), json::Object(Capacity), "bool",
                      L),
               Grow, Reuse, L);
        label(Grow, L);
        auto DesiredCapacity = temporary(SizeType, L);
        assign(DesiredCapacity,
               binary("*", json::Object(Capacity), quantity(2, SizeType, L),
                      SizeType, L),
               L);
        const auto UseLength = labelName(), DesiredReady = labelName();
        branch(binary(">", json::Object(Length), json::Object(DesiredCapacity),
                      "bool", L),
               UseLength, DesiredReady, L);
        label(UseLength, L);
        assign(DesiredCapacity, json::Object(Length), L);
        jump(DesiredReady, L);
        ...
        auto NewData = temporary(PointerType, L);
        assign(NewData, cast(std::move(Allocation), PointerType, L), L);
        CopyBytes(json::Object(NewData), json::Object(LeftData),
                  json::Object(LeftSize));
        CopyBytes(binary("+", json::Object(NewData),
                         cast(json::Object(LeftSize), DifferenceType, L),
                         PointerType, L),
                  json::Object(RightData), json::Object(RightSize));
        assign(dereference(binary("+", json::Object(NewData),
                                  cast(json::Object(Length), DifferenceType, L),
                                  PointerType, L),
                           L),
               quantity(0, type(A.Context.CharTy, L), L), L);
        const auto Release = labelName(), Install = labelName();
        ...
        label(Release, L);
        const auto *Delete =
            A.allocatorHeapFunction(false, A.Context.CharTy, L);
        json::Array DeleteArgs;
        DeleteArgs.push_back(cast(json::Object(TargetData),
                                  type(Delete->getParamDecl(0)->getType(), L),
                                  L));
        if (Delete->getNumParams() == 2)
          DeleteArgs.push_back(
              cast(binary("+", json::Object(Capacity), quantity(1, SizeType, L),
                          SizeType, L),
                   type(Delete->getParamDecl(1)->getType(), L), L));
        chargeCall(DeleteArgs, L);
        Body.push_back(json::Object{{"op", "call"},
        ...
        jump(Install, L);
        label(Install, L);
        assign(TargetWord(PointerName), json::Object(NewData), L);
        assign(TargetWord("nct_string_word1"), json::Object(Length), L);
        assign(TargetWord(FlagName),
               binary("|", json::Object(AllocationBytes),
                      quantity(LongFlag, SizeType, L), SizeType, L),
               L);
        jump(Move, L);
        label(Reuse, L);
        auto Data = cast(json::Object(TargetData), PointerType, L);
        if (RvalueLeft) {
          CopyBytes(binary("+", json::Object(Data),
                           cast(json::Object(TargetSize), DifferenceType, L),
                           PointerType, L),
                    json::Object(RightData), json::Object(RightSize));
        } else {
          auto Prefix = temporary(ConstPointerType, L);
          assign(Prefix, json::Object(LeftData), L);
          if (StringFor(Call->getArg(0)->getType()) ||
              Call->getArg(0)->getType()->isPointerType()) {
            const auto CheckUpper = labelName(), AdjustPrefix = labelName(),
                       Shift = labelName();
            branch(binary(">=", bitCast(json::Object(Prefix), SizeType, L),
                          bitCast(json::Object(Data), SizeType, L), "bool", L),
                   CheckUpper, Shift, L);
            label(CheckUpper, L);
            branch(binary("<=", bitCast(json::Object(Prefix), SizeType, L),
                          bitCast(binary("+", json::Object(Data),
                                         cast(json::Object(TargetSize),
                                              DifferenceType, L),
                                         PointerType, L),
                                  SizeType, L),
                          "bool", L),
                   AdjustPrefix, Shift, L);
            label(AdjustPrefix, L);
            assign(Prefix,
                   binary("+", json::Object(Prefix),
                          cast(json::Object(LeftSize), DifferenceType, L),
                          ConstPointerType, L),
                   L);
            jump(Shift, L);
          ...
          auto ShiftIndex = temporary(SizeType, L);
          assign(ShiftIndex, quantity(0, SizeType, L), L);
          const auto ShiftCheck = labelName(), ShiftByte = labelName(),
                     Shifted = labelName();
          jump(ShiftCheck, L);
          label(ShiftCheck, L);
          branch(binary("<=", json::Object(ShiftIndex),
                        json::Object(TargetSize), "bool", L),
                 ShiftByte, Shifted, L);
          label(ShiftByte, L);
          auto Offset = binary("-", json::Object(TargetSize),
                               json::Object(ShiftIndex), SizeType, L);
          assign(dereference(
                     binary("+", json::Object(Data),
                            cast(binary("+", json::Object(Offset),
                                        json::Object(LeftSize), SizeType, L),
                                 DifferenceType, L),
                            PointerType, L),
                     L),
                 dereference(binary("+", json::Object(Data),
                                    cast(std::move(Offset), DifferenceType, L),
                                    PointerType, L),
                             L),
                 L);
          assign(ShiftIndex,
          ...
          jump(ShiftCheck, L);
          label(Shifted, L);
          CopyBytes(json::Object(Data), json::Object(Prefix),
                    json::Object(LeftSize));
        }
        assign(dereference(binary("+", json::Object(Data),
                                  cast(json::Object(Length), DifferenceType, L),
        ...
        assign(TargetWord(FlagName),
               binary("|",
                      binary("&", TargetWord(FlagName), std::move(Mask),
                             SizeType, L),
                      std::move(Encoded), SizeType, L),
               L);
        jump(Move, L);
        label(Move, L);
        const char *Names[] = {"nct_string_word0", "nct_string_word1",
                               "nct_string_word2"};
        std::vector<Expression> Values;
        Values.reserve(3);
        for (const char *Name : Names)
          Values.push_back(snapshot(TargetWord(Name), L));
        for (unsigned I = 0; I != 3; ++I)
          assign(Word(Names[I]), json::Object(Values[I]), L);
        for (const char *Name : Names)
          initializeZero(TargetWord(Name),
                         llvm::StringRef(Name) == PointerName
                             ? String->PointerType
                             : A.Context.getSizeType(),
                         L);
        return Place;
      ...
      branch(binary("<=", json::Object(Length),
             ...
             Short, Long, L);
      label(Short, L);
      auto ShortPointer = cast(
          cast(address(json::Object(Place), Call->getType(), L), "ptr:void", L),
          PointerType, L);
      assign(
          Word(FlagName),
          String->AlternateLayout
      ...
      assign(Word(PointerName), json::Object(Data), L);
      const auto LongFlag =
          String->AlternateLayout
              ? uint64_t(1)
                    << (A.Context.getTypeSize(A.Context.getSizeType()) - 1)
      ...
      label(Copy, L);
      auto Target = temporary(PointerType, L);
      assign(Target, json::Object(Data), L);
      auto CopySpan = [&](Expression Bytes, Expression Size) {
        auto Source = temporary(ConstPointerType, L);
        auto Count = temporary(SizeType, L);
        assign(Source, std::move(Bytes), L);
        assign(Count, quantity(0, SizeType, L), L);
        ...
        jump(Check, L);
        label(Check, L);
        branch(binary("<", json::Object(Count), std::move(Size), "bool", L),
               Write, Done, L);
        label(Write, L);
        assign(dereference(json::Object(Target), L),
               dereference(json::Object(Source), L), L);
      ...
      CopySpan(std::move(LeftData), std::move(LeftSize));
    ...
    case UtilityOperation::StringSubstr: {
      ...
      auto String = Object ? StringFor(Object->getType())
                           : std::optional<UtilityStringRecord>();
      if (!Object || !String)
        reject(L, "string substring",
               "The selected std::string layout is unavailable.");
      auto Receiver =
          snapshot(address(lvalue(Object), Object->getType(), L), L);
      auto Argument = [&](unsigned Index) {
        const auto *Value = Call->getArg(Index);
        if (const auto *Default = dyn_cast<CXXDefaultArgExpr>(Value))
          Value = selectedDefaultArgument(Default, A.Context);
        if (!Value)
          reject(L, "string substring",
                 "The selected substring argument is unavailable.");
        return snapshot(expression(Value), L);
      };
      auto Position = Argument(0);
      auto Requested = Argument(1);
      auto [SourceData, SourceSize] =
          ReadStringAt(json::Object(Receiver), *String);
      const auto SizeType = type(A.Context.getSizeType(), L);
      const auto DifferenceType = type(A.Context.getPointerDiffType(), L);
      const auto ConstPointerType =
          type(A.Context.getPointerType(A.Context.CharTy.withConst()), L);
      ...
      if (Place.getString("type") != type(Call->getType(), L))
        reject(L, "string substring",
               "The substring destination type differs from the result.");
      const char *FlagName =
          String->AlternateLayout ? "nct_string_word2" : "nct_string_word0";
      const char *PointerName =
          String->AlternateLayout ? "nct_string_word0" : "nct_string_word2";
      ...
      const auto Valid = labelName(), Invalid = labelName(),
                 LengthReady = labelName();
      branch(binary("<=", json::Object(Position), json::Object(SourceSize),
                    "bool", L),
             Valid, Invalid, L);
      label(Valid, L);
      auto Remaining = temporary(SizeType, L);
      assign(Remaining,
             binary("-", json::Object(SourceSize), json::Object(Position),
                    SizeType, L),
             L);
      ...
      label(Invalid, L);
      assign(Length, quantity(0, SizeType, L), L);
      assign(Position, json::Object(SourceSize), L);
      jump(LengthReady, L);
      label(LengthReady, L);
      const auto Offset = labelName(), Positioned = labelName();
      branch(binary("!=", json::Object(Position), quantity(0, SizeType, L),
      ...
      if (String->AlternateLayout) {
        assign(
            ...
            binary("<<", json::Object(Length),
                   ...
                   SizeType, L),
            L);
        assign(Data, std::move(ShortPointer), L);
      } else {
        assign(Word(FlagName),
               binary("*", json::Object(Length), quantity(2, SizeType, L),
                      SizeType, L),
      ...
      assign(
          Word(FlagName),
          binary("|", json::Object(AllocationBytes),
                 quantity(String->AlternateLayout
                              ? uint64_t(1) << (A.Context.getTypeSize(
                                                    A.Context.getSizeType()) -
                                                1)
                              : uint64_t(1),
                          SizeType, L),
      ...
      auto Target = temporary(PointerType, L);
      auto Count = temporary(SizeType, L);
      assign(Source, json::Object(SourceData), L);
      assign(Target, json::Object(Data), L);
      assign(Count, quantity(0, SizeType, L), L);
      const auto Check = labelName(), Write = labelName(), Done = labelName();
      jump(Check, L);
    ...
    case UtilityOperation::StringCopy:
    case UtilityOperation::StringViewCopy: {
      const auto *Object = MemberObject();
      const bool ViewCopy = Operation == UtilityOperation::StringViewCopy;
      auto String = !ViewCopy && Object ? StringFor(Object->getType())
                                        : std::optional<UtilityStringRecord>();
      auto View = ViewCopy && Object ? StringViewFor(Object->getType())
                                     : std::optional<UtilityStringViewRecord>();
      if (!Object || (!String && !View))
        reject(L, "string copy", "The selected string layout is unavailable.");
      auto Receiver =
          snapshot(address(lvalue(Object), Object->getType(), L), L);
      auto Argument = [&](unsigned Index) {
        const auto *Value = Call->getArg(Index);
        if (const auto *Default = dyn_cast<CXXDefaultArgExpr>(Value))
          Value = selectedDefaultArgument(Default, A.Context);
        if (!Value)
          reject(L, "string copy",
                 "The selected copy argument is unavailable.");
        return snapshot(expression(Value), L);
      };
      auto Target = Argument(0);
      auto Requested = Argument(1);
      auto Position = Argument(2);
      const auto SizeType = type(A.Context.getSizeType(), L);
      const auto DifferenceType = type(A.Context.getPointerDiffType(), L);
      const auto ConstPointerType =
          type(A.Context.getPointerType(A.Context.CharTy.withConst()), L);
      const auto PointerType =
          type(A.Context.getPointerType(A.Context.CharTy), L);
      auto Data = temporary(ConstPointerType, L);
      auto Size = temporary(SizeType, L);
      if (String) {
        auto [Bytes, Length] = ReadStringAt(json::Object(Receiver), *String);
        assign(Data, std::move(Bytes), L);
        assign(Size, std::move(Length), L);
      } else {
        assign(
            Data,
            fieldStorage(dereference(json::Object(Receiver), L), View->Data, L),
            L);
        assign(
            Size,
            fieldStorage(dereference(json::Object(Receiver), L), View->Size, L),
            L);
      }
      auto Copied = temporary(SizeType, L);
      assign(Copied, quantity(0, SizeType, L), L);
      const auto Valid = labelName(), Done = labelName();
      branch(
          binary("<=", json::Object(Position), json::Object(Size), "bool", L),
          Valid, Done, L);
      ...
      label(Ready, L);
      auto Source = temporary(ConstPointerType, L);
      assign(Source, json::Object(Data), L);
      const auto Offset = labelName(), Positioned = labelName();
      branch(binary("!=", json::Object(Position), quantity(0, SizeType, L),
                    "bool", L),
             Offset, Positioned, L);
      ...
      label(Positioned, L);
      auto Cursor = temporary(PointerType, L);
      assign(Cursor, json::Object(Target), L);
      const auto Check = labelName(), Write = labelName();
      jump(Check, L);
      label(Check, L);
      branch(
          binary("<", json::Object(Copied), json::Object(Remaining), "bool", L),
          Write, Done, L);
      label(Write, L);
      ...
      assign(Source,
             ...
             L);
      assign(Copied,
             binary("+", json::Object(Copied), quantity(1, SizeType, L),
                    SizeType, L),
             L);
      jump(Check, L);
      label(Done, L);
    ...
    case UtilityOperation::StringViewSubstr: {
      const auto *Object = MemberObject();
      auto View = Object ? StringViewFor(Object->getType())
                         : std::optional<UtilityStringViewRecord>();
      if (!Object || !View)
        reject(L, "string view substring",
               "The selected std::string_view layout is unavailable.");
      auto Receiver =
          snapshot(address(lvalue(Object), Object->getType(), L), L);
      auto Argument = [&](unsigned Index) {
      ...
      if (Place.getString("type") != type(Call->getType(), L))
        reject(L, "string view substring",
               "The substring destination type differs from the result.");
      auto NewSize = temporary(SizeType, L);
      const auto Valid = labelName(), Invalid = labelName(), Done = labelName();
      branch(
          binary("<=", json::Object(Position), json::Object(Size), "bool", L),
          Valid, Invalid, L);
      label(Valid, L);
      ...
      branch(binary("<", json::Object(Requested), json::Object(Remaining),
             ...
             UseRequested, UseRemaining, L);
      label(UseRequested, L);
      assign(NewSize, json::Object(Requested), L);
      jump(LengthReady, L);
      label(UseRemaining, L);
      assign(NewSize, json::Object(Remaining), L);
      jump(LengthReady, L);
      label(LengthReady, L);
      const auto Offset = labelName(), Positioned = labelName();
      branch(binary("!=", json::Object(Position), quantity(0, SizeType, L),
      ...
      assign(fieldStorage(json::Object(Place), View->Data, L),
             json::Object(Data), L);
      assign(fieldStorage(json::Object(Place), View->Size, L),
             json::Object(NewSize), L);
      return Place;
    }
  ...
  Expression call(const CallExpr *Call,
    ...
    if (A.S.coreV2()) {
      ...
      if (auto Assignment = approvedUtilityStringAssignment(
              A.S, A.Sources, dyn_cast<CXXOperatorCallExpr>(Call), A.Context)) {
        if (Destination)
          reject(L, "string assignment",
                 "std::string assignment cannot initialize a record result.");
        ...
        const char *PointerName = String->AlternateLayout
                                      ? "nct_string_word0" : "nct_string_word2";
        auto WordType = [&](const char *Name) {
          return llvm::StringRef(Name) == PointerName
                     ? String->PointerType : A.Context.getSizeType();
        };
        auto Member = [&](const Expression &Address, const char *Name) {
          return Expression{{"kind", "member"},
                            ...
                            {"name", Name},
                            {"args", json::Array{dereference(
                                         json::Object(Address), L)}},
                            {"loc", A.loc(L)}};
        };
        auto LeftWord = [&](const char *Name) {
          return Member(LeftAddress, Name);
        };
        auto RightWord = [&](const char *Name) {
          return Member(RightAddress, Name);
        };
        auto IsLong = [&](Expression Flag) {
          return binary("!=", binary("&", std::move(Flag),
                                      quantity(LongFlag, SizeType, L),
                                      SizeType, L),
                        quantity(0, SizeType, L), "bool", L);
        };
        ...
        auto ReleaseLeft = [&] {
          auto Flag = snapshot(LeftWord(FlagName), L);
          const auto Release = labelName(), Done = labelName();
          branch(IsLong(json::Object(Flag)), Release, Done, L);
          label(Release, L);
          const auto *Delete =
              A.allocatorHeapFunction(false, A.Context.CharTy, L);
          json::Array Args;
          Args.push_back(cast(LeftWord(PointerName),
                              type(Delete->getParamDecl(0)->getType(), L), L));
          if (Delete->getNumParams() == 2)
            Args.push_back(cast(binary("-", json::Object(Flag),
                                       quantity(LongFlag, SizeType, L),
                                       SizeType, L),
                                type(Delete->getParamDecl(1)->getType(), L), L));
          chargeCall(Args, L);
        ...
        const auto Same = labelName(), Apply = labelName(), End = labelName();
        branch(binary("==", cast(json::Object(LeftAddress), "ptr:void", L),
                      cast(json::Object(RightAddress), "ptr:void", L),
                      "bool", L), Same, Apply, L);
        label(Same, L);
        jump(End, L);
        label(Apply, L);
        if (*Assignment == UtilityStringAssignment::Move) {
          ReleaseLeft();
          const auto BytePointerType = std::string("ptr:u8");
          auto SourceBytes = temporary(BytePointerType, L);
          auto TargetBytes = temporary(BytePointerType, L);
          auto Count = temporary(SizeType, L);
          assign(SourceBytes,
                 cast(cast(json::Object(RightAddress), "ptr:void", L),
                      BytePointerType, L), L);
          assign(TargetBytes,
                 cast(cast(json::Object(LeftAddress), "ptr:void", L),
                      BytePointerType, L), L);
          assign(Count, quantity(0, SizeType, L), L);
          ...
          label(Check, L);
          branch(binary("<", json::Object(Count),
                        quantity(A.Context.getTypeSizeInChars(
                                     A.Context.getRecordType(String->Record))
                                     .getQuantity(),
                                 SizeType, L), "bool", L), Advance, Done, L);
          label(Advance, L);
          assign(dereference(json::Object(TargetBytes), L),
                 dereference(json::Object(SourceBytes), L), L);
          assign(SourceBytes,
                 binary("+", json::Object(SourceBytes),
                        quantity(1, DifferenceType, L), BytePointerType, L), L);
          assign(TargetBytes,
                 binary("+", json::Object(TargetBytes),
                        quantity(1, DifferenceType, L), BytePointerType, L), L);
          assign(Count,
                 binary("+", json::Object(Count), quantity(1, SizeType, L),
                        SizeType, L), L);
          ...
          for (const char *Name : {"nct_string_word0", "nct_string_word1",
                                   "nct_string_word2"})
            initializeZero(RightWord(Name), WordType(Name), L);
        } else {
          auto SourceSize = temporary(SizeType, L);
          auto SourceData = temporary(ConstPointerType, L);
          auto SourceFlag = snapshot(RightWord(FlagName), L);
          const auto SourceLong = labelName(), SourceShort = labelName(),
                     SourceReady = labelName();
          branch(IsLong(json::Object(SourceFlag)), SourceLong, SourceShort, L);
          label(SourceLong, L);
          assign(SourceSize, RightWord("nct_string_word1"), L);
          assign(SourceData, cast(RightWord(PointerName), ConstPointerType, L),
                 L);
          jump(SourceReady, L);
          label(SourceShort, L);
          assign(SourceSize,
          ...
          jump(SourceReady, L);
          label(SourceReady, L);
          auto OldFlag = snapshot(LeftWord(FlagName), L);
          auto OldLong = snapshot(IsLong(json::Object(OldFlag)), L);
          auto Capacity = temporary(SizeType, L);
          auto TargetData = temporary(PointerType, L);
          const auto TargetLong = labelName(), TargetShort = labelName(),
                     TargetReady = labelName();
          branch(json::Object(OldLong), TargetLong, TargetShort, L);
          label(TargetLong, L);
          assign(Capacity,
                 binary("-", binary("-", json::Object(OldFlag),
                                    quantity(LongFlag, SizeType, L),
                                    SizeType, L),
                        quantity(1, SizeType, L), SizeType, L), L);
          assign(TargetData, LeftWord(PointerName), L);
          jump(TargetReady, L);
          label(TargetShort, L);
          assign(Capacity, quantity(String->ShortCapacity, SizeType, L), L);
          assign(TargetData, ShortData(LeftAddress, PointerType), L);
          jump(TargetReady, L);
          label(TargetReady, L);
          const auto Reuse = labelName(), Grow = labelName(), Copy = labelName();
          branch(binary("<=", json::Object(SourceSize), json::Object(Capacity),
                        "bool", L), Reuse, Grow, L);
          label(Reuse, L);
          const auto ReuseLong = labelName(), ReuseShort = labelName();
          branch(json::Object(OldLong), ReuseLong, ReuseShort, L);
          label(ReuseLong, L);
          assign(LeftWord("nct_string_word1"), json::Object(SourceSize), L);
          jump(Copy, L);
          label(ReuseShort, L);
          assign(LeftWord(FlagName),
                 String->AlternateLayout
                     ? binary("<<", json::Object(SourceSize),
                              quantity(A.Context.getTypeSize(A.Context.getSizeType()) - 8,
                                       SizeType, L), SizeType, L)
                     : binary("*", json::Object(SourceSize),
                              quantity(2, SizeType, L), SizeType, L), L);
          ...
          Body.push_back(json::Object{{"op", "call"},
                                      ...
                                      {"loc", A.loc(L)}});
          ReleaseLeft();
          assign(TargetData, cast(std::move(Allocation), PointerType, L), L);
          assign(LeftWord(PointerName), json::Object(TargetData), L);
          assign(LeftWord("nct_string_word1"), json::Object(SourceSize), L);
          assign(LeftWord(FlagName),
                 binary("|", json::Object(AllocationBytes),
                        quantity(LongFlag, SizeType, L), SizeType, L), L);
          ...
          auto TargetCurrent = temporary(PointerType, L);
          auto Count = temporary(SizeType, L);
          assign(SourceCurrent, json::Object(SourceData), L);
          assign(TargetCurrent, json::Object(TargetData), L);
          assign(Count, quantity(0, SizeType, L), L);
          const auto Check = labelName(), Advance = labelName(), Finish = labelName();
          jump(Check, L);
          label(Check, L);
          branch(binary("<", json::Object(Count), json::Object(SourceSize),
      ...
      if (auto Assignment = approvedUtilityVectorAssignment(
              A.S, A.Sources, dyn_cast<CXXOperatorCallExpr>(Call), A.Context)) {
        if (Destination)
          reject(L, "vector assignment",
                 "std::vector assignment cannot initialize a record result.");
        ...
        auto LeftMember = [&](const char *Name) {
          return Member(LeftAddress, Name);
        };
        auto RightMember = [&](const char *Name) {
          return Member(RightAddress, Name);
        };
        const auto PointerType = type(Vector->PointerType, L);
        const auto DifferenceType = type(A.Context.getPointerDiffType(), L);
        ...
        const uint64_t ElementBytes =
            A.Context.getTypeSizeInChars(Vector->ElementType).getQuantity();
        auto ElementCount = [&](const Expression &Begin, const Expression &End) {
          auto Count = temporary(SizeType, L);
          assign(Count, quantity(0, SizeType, L), L);
          const auto Measure = labelName(), Done = labelName();
          branch(cast(json::Object(Begin), "bool", L), Measure, Done, L);
          label(Measure, L);
          assign(Count,
                 cast(binary("-", json::Object(End), json::Object(Begin),
                             DifferenceType, L), SizeType, L), L);
        ...
        auto Release = [&] {
          ...
          Args.push_back(cast(json::Object(Begin),
                              type(Delete->getParamDecl(0)->getType(), L), L));
          if (Delete->getNumParams() == 2) {
            auto Capacity = snapshot(LeftMember("nct_vector_capacity"), L);
            auto Count = ElementCount(Begin, Capacity);
            Args.push_back(cast(
          ...
          for (const char *Name : {"nct_vector_begin", "nct_vector_end",
                                   "nct_vector_capacity"})
            initializeZero(LeftMember(Name), Vector->PointerType, L);
        };
        if (*Assignment == UtilityVectorAssignment::Move) {
          Release();
          for (const char *Name : {"nct_vector_begin", "nct_vector_end",
                                   "nct_vector_capacity"}) {
            auto Pointer = snapshot(RightMember(Name), L);
            assign(LeftMember(Name), std::move(Pointer), L);
            initializeZero(RightMember(Name), Vector->PointerType, L);
          }
          return Left;
        }
        const auto Same = labelName(), Copy = labelName(), Done = labelName();
        const auto RightPointerType = type(
            A.Context.getPointerType(Call->getArg(1)->getType()), L);
        branch(binary("==", cast(json::Object(LeftAddress), RightPointerType,
                                 L), json::Object(RightAddress), "bool", L),
               Same, Copy, L);
        label(Same, L);
        jump(Done, L);
        label(Copy, L);
        auto SourceBegin = snapshot(RightMember("nct_vector_begin"), L);
        auto SourceEnd = snapshot(RightMember("nct_vector_end"), L);
        auto TargetBegin = snapshot(LeftMember("nct_vector_begin"), L);
        auto TargetCapacity = snapshot(LeftMember("nct_vector_capacity"), L);
        auto SourceSize = ElementCount(SourceBegin, SourceEnd);
        auto Capacity = ElementCount(TargetBegin, TargetCapacity);
        const auto Reuse = labelName(), Grow = labelName(), Fill = labelName();
        branch(binary("<=", json::Object(SourceSize), json::Object(Capacity),
                      "bool", L), Reuse, Grow, L);
        auto NewBegin = temporary(PointerType, L);
        label(Reuse, L);
        ...
        label(Grow, L);
        auto NewCapacity = temporary(SizeType, L);
        assign(NewCapacity, json::Object(SourceSize), L);
        const auto Double = labelName(), Allocate = labelName();
        branch(binary(">", binary("*", json::Object(Capacity),
                                  quantity(2, SizeType, L), SizeType, L),
                      json::Object(SourceSize), "bool", L), Double, Allocate,
               L);
        label(Double, L);
        assign(NewCapacity,
               binary("*", json::Object(Capacity), quantity(2, SizeType, L),
        ...
        json::Array Args;
        Args.push_back(binary("*", json::Object(NewCapacity),
                              quantity(ElementBytes, SizeType, L), SizeType,
                              L));
        chargeCall(Args, L);
        auto Allocation = temporary(type(New->getReturnType(), L), L);
        Body.push_back(json::Object{{"op", "call"},
                                    ...
                                    {"loc", A.loc(L)}});
        assign(NewBegin, cast(std::move(Allocation), PointerType, L), L);
        assign(LeftMember("nct_vector_begin"), json::Object(NewBegin), L);
        assign(LeftMember("nct_vector_capacity"),
               binary("+", json::Object(NewBegin),
                      cast(json::Object(NewCapacity), DifferenceType, L),
                      PointerType, L), L);
        jump(Fill, L);
        ...
        auto SourceCurrent = temporary(PointerType, L);
        auto TargetCurrent = temporary(PointerType, L);
        assign(SourceCurrent, std::move(SourceBegin), L);
        assign(TargetCurrent, json::Object(NewBegin), L);
        const auto Check = labelName(), Advance = labelName(), Finish = labelName();
        jump(Check, L);
        label(Check, L);
        ...
        jump(Check, L);
        label(Finish, L);
        assign(LeftMember("nct_vector_end"), std::move(TargetCurrent), L);
        jump(Done, L);
        label(Done, L);
        return Left;
      }
  ...
  void construct(Expression Place, QualType T, const CXXConstructExpr *C,
    ...
    if (auto Kind = approvedUtilityStringConstruction(A.S, A.Sources, C,
                                                      A.Context)) {
      auto String = approvedUtilityStringRecord(
          A.S, A.Sources, T->getAsCXXRecordDecl(), A.Context);
      if (!String)
      ...
      if (*Kind == UtilityStringConstruction::Default) {
        for (const char *Name : {"nct_string_word0", "nct_string_word1",
                                 "nct_string_word2"})
      ...
      if (*Kind == UtilityStringConstruction::Copy ||
        ...
        if (*Kind == UtilityStringConstruction::Move) {
          const auto BytePointerType = std::string("ptr:u8");
          auto SourceBytes = temporary(BytePointerType, L);
          auto TargetBytes = temporary(BytePointerType, L);
          auto Count = temporary(SizeType, L);
          ...
          label(Check, L);
          branch(binary("<", json::Object(Count),
                        quantity(A.Context.getTypeSizeInChars(T).getQuantity(),
                                 SizeType, L), "bool", L), Advance, Done, L);
          label(Advance, L);
          assign(dereference(json::Object(TargetBytes), L),
                 dereference(json::Object(SourceBytes), L), L);
          ...
          for (const char *Name : {"nct_string_word0", "nct_string_word1",
                                   "nct_string_word2"})
            initializeZero(SourceWord(Name), WordType(Name), L);
          return;
        }
        auto Flag = snapshot(SourceWord(String->AlternateLayout
                                            ? "nct_string_word2"
                                            : "nct_string_word0"), L);
        const auto LongFlag = String->AlternateLayout
                                  ? uint64_t(1)
        ...
        branch(binary("!=", binary("&", json::Object(Flag),
                      ...
                      quantity(0, SizeType, L), "bool", L), Long, Short, L);
        label(Long, L);
        assign(Length, SourceWord("nct_string_word1"), L);
        assign(Input, cast(SourceWord(String->AlternateLayout
                                          ? "nct_string_word0"
                                          : "nct_string_word2"),
                           ConstPointerType, L), L);
        jump(Ready, L);
        ...
        const auto Check = labelName(), Advance = labelName(), Scanned = labelName();
        jump(Check, L);
        label(Check, L);
        branch(binary("!=", dereference(json::Object(Cursor), L),
                      quantity(0, type(A.Context.CharTy, L), L), "bool", L),
    ...
    if (auto Kind = approvedUtilityVectorConstruction(A.S, A.Sources, C,
      ...
      if (*Kind == UtilityVectorConstruction::Copy ||
          *Kind == UtilityVectorConstruction::Move) {
        auto SourceAddress = snapshot(address(lvalue(C->getArg(0)),
                                              C->getArg(0)->getType(), L), L);
        ...
        if (*Kind == UtilityVectorConstruction::Move) {
          for (const char *Name : {"nct_vector_begin", "nct_vector_end",
                                   "nct_vector_capacity"}) {
            auto Pointer = snapshot(SourceMember(Name), L);
            assign(Member(Name), std::move(Pointer), L);
            initializeZero(SourceMember(Name), Vector->PointerType, L);
          }
          return;
        }
        ...
        branch(binary("!=", json::Object(OldBegin), json::Object(OldEnd),
                      "bool", L), Copy, Done, L);
        label(Copy, L);
        const auto SizeType = type(A.Context.getSizeType(), L);
        const auto DifferenceType = type(A.Context.getPointerDiffType(), L);
        const auto PointerType = type(Vector->PointerType, L);
        auto Count = snapshot(cast(binary("-", json::Object(OldEnd),
                                          json::Object(OldBegin), DifferenceType,
                                          L), SizeType, L), L);
        const uint64_t ElementBytes =
            A.Context.getTypeSizeInChars(Vector->ElementType).getQuantity();
        const auto *New =
        ...
        auto NewEnd = snapshot(binary("+", json::Object(NewBegin),
                                      cast(json::Object(Count), DifferenceType,
                                           L), PointerType, L), L);
        assign(Member("nct_vector_begin"), json::Object(NewBegin), L);
        assign(Member("nct_vector_end"), json::Object(NewEnd), L);
        assign(Member("nct_vector_capacity"), std::move(NewEnd), L);
        auto SourceCurrent = temporary(PointerType, L);
        auto TargetCurrent = temporary(PointerType, L);
...
public:
  ...
  FunctionLowering(Adapter &A, const CXXRecordDecl *R)
    ...
    const bool StringDestructor =
        approvedUtilityStringDestructor(A.S, A.Sources, D, A.Context);
    ...
    if (approvedUtilityStringRecord(A.S, A.Sources, DestroyedRecord,
                                    A.Context) && !StringDestructor)
      reject(D ? D->getLocation() : DestroyedRecord->getLocation(),
             "string destructor",
             "The pinned std::string destructor definition is required.");
    if (D && !UniquePtrDestructor && !StringDestructor && !VectorDestructor && !D->isImplicit() &&
    ...
    if (DestroyedRecord) {
      ...
      if (auto Unique = approvedUtilityUniquePtrRecord(
        ...
        deallocateUniquePtr(std::move(Pointer), json::Object(*ThisPointer),
                            *Unique, L);
      } else if (auto String = approvedUtilityStringRecord(
                     A.S, A.Sources, DestroyedRecord, A.Context)) {
        const auto SizeType = type(A.Context.getSizeType(), L);
        auto WordType = [&](const char *Name) {
          const bool PointerWord = String->AlternateLayout
        ...
        branch(binary("!=", binary("&", json::Object(First),
                                    quantity(LongFlag, SizeType, L), SizeType, L),
                      quantity(0, SizeType, L), "bool", L), Release, Done, L);
        label(Release, L);
        const auto *Deallocate =
            A.allocatorHeapFunction(false, A.Context.CharTy, L);
        json::Array Args;
        Args.push_back(cast(Word(String->AlternateLayout
                                     ? "nct_string_word0"
                                     : "nct_string_word2"),
                            type(Deallocate->getParamDecl(0)->getType(), L),
                            L));
        if (Deallocate->getNumParams() == 2)
          Args.push_back(cast(binary("-", json::Object(First),
                                     quantity(LongFlag, SizeType, L), SizeType, L),
                              type(Deallocate->getParamDecl(1)->getType(), L),
                              L));
        chargeCall(Args, L);
        Body.push_back(json::Object{{"op", "call"},
