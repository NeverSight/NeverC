    ...
    if (D && D->getDefinition() && !D->isUnion()) {
      ...
      if (approvedFunctionalObjectRecord(S, Sources, D, Context)) {
        ...
        reject(L, "standard library record",
               ...
               "layouts with matching std::default_delete or an admitted "
               "empty custom deleter are supported.",
               "TR0203");
        return {};
        ...
        return {};
      } else if (approvedUtilityStringRecord(S, Sources, D, Context) &&
                 !requireUtilityString(D, L, Depth + 1)) {
        return {};
...
bool Adapter::requireUtilityString(const CXXRecordDecl *Record,
                                   SourceLocation Location, unsigned Depth) {
  if (Depth > 64) {
    reject(Location, "string type", "Nested std::string types exceed the protocol limit.");
    return false;
  }
  const auto String = approvedUtilityStringRecord(S, Sources, Record, Context);
  if (!String) {
    reject(Location, "standard library record",
           "Only the pinned std::string layout is admitted.", "TR0203");
    return false;
  }
  if (!RequiredUtilityStrings.insert(String->Record->getCanonicalDecl()).second)
    return true;
  if (type(Context.getSizeType(), Location, false, Depth + 1).empty() ||
      type(String->PointerType, Location, false, Depth + 1).empty())
    return false;
  S.Module["memory_lifetimes"] = true;
  Records.push_back(const_cast<CXXRecordDecl *>(String->Record));
  return true;
}
...
bool Adapter::requireUtilityUniquePtr(const CXXRecordDecl *Record,
  ...
  if (!Unique) {
    reject(Location, "standard library record",
           ...
           "layouts with matching std::default_delete or an admitted empty "
           "custom deleter are supported.",
           "TR0203");
    return false;
  }
  ...
  if (type(Unique->PointerType, Location, false, Depth + 1).empty())
    return false;
  if (Unique->CustomDeleter) {
    if (type(Context.getRecordType(Unique->Deleter.Record), Location, false,
             Depth + 1)
            .empty())
      return false;
  } else {
...
bool Adapter::requireFunctionalObject(const CXXRecordDecl *Record,
                                      SourceLocation Location,
                                      unsigned Depth) {
  if (Depth > 64) {
    reject(Location, "functional object type",
           "Nested standard function-object types exceed the protocol limit.");
    return false;
  }
  const auto Object =
  ...
  const auto *Canonical = Object->Record->getCanonicalDecl();
  if (!RequiredFunctionalObjects.insert(Canonical).second)
    return true;
  Records.push_back(const_cast<CXXRecordDecl *>(Object->Record));
  return true;
}
...
json::Object Adapter::zero(QualType T, SourceLocation L) {
  ...
  if (const auto *R = T->getAsCXXRecordDecl()) {
    ...
    } else if (auto String = approvedUtilityStringRecord(S, Sources, R,
                                                        Context)) {
      for (unsigned I = 0; I != 3; ++I)
        Args.push_back(zero((String->AlternateLayout ? I == 0 : I == 2)
                                ? String->PointerType
                                : Context.getSizeType(), L));
  ...
  void checkConstruction(const CXXConstructExpr *C, SourceLocation L) {
    ...
    if (A.S.coreV2() &&
        (approvedFunctionalObjectConstruction(A.S, A.Sources, C, A.Context) ||
         ...
         approvedUtilityStringConstruction(A.S, A.Sources, C, A.Context) ||
        ...
        if (approvedUtilityNulloptExpression(A.S, A.Sources, C, A.Context) ||
          ...
          // by authenticated optional operations.
                   ...
                   approvedUtilityUniquePtrConstruction(A.S, A.Sources, C,
                                                        A.Context) ||
                   approvedUtilityStringConstruction(A.S, A.Sources, C,
                                                     A.Context) ||
...
static void orderCoreV2Records(Adapter &A) {
  ...
  auto Order = [&](auto &&Self, std::size_t I, unsigned Depth) -> void {
    ...
    if (UtilityTuple) {
      ...
      DependencyTypes.emplace_back(A.Context.getSizeType(),
                                   UtilityString->Record->getLocation());
      DependencyTypes.emplace_back(UtilityString->PointerType,
                                   UtilityString->Record->getLocation());
...
void Adapter::run(llvm::ArrayRef<ExplicitFunctionInstantiationSource> Directives,
  ...
  for (const auto *R : Records) {
    ...
    const auto *Base = S.coreV2() && !FunctionalObject &&
                               ...
                               !UtilityUniquePtr && !UtilityString &&
                               !UtilityVector &&
    ...
    if (FunctionalObject) {
      ...
      for (unsigned I = 0; I != 3; ++I)
        Fields.push_back(json::Object{
            {"name", "nct_string_word" + std::to_string(I)},
            {"type", type((UtilityString->AlternateLayout ? I == 0 : I == 2)
                              ? UtilityString->PointerType
                              : Context.getSizeType(), R->getLocation())}});
    } else if (UtilityVector) {
    ...
    if (S.coreV2()) {
      ...
      } else if (UtilityString || UtilityVector) {
        const auto PointerBits = UtilityString
                                     ? Context.getTypeSize(Context.getSizeType())
                                     : Context.getTypeSize(UtilityVector->PointerType);
        for (unsigned I = 0; I != 3; ++I)
      ...
      if (S.owns(SM, L) &&
          (!Angled ||
           (Name != "type_traits" && Name != "cstdint" && Name != "limits" &&
            ...
            Name != "string_view" && Name != "string" && Name != "vector"))) {
        reject(L, "include",
               ...
               "#include <string_view>, #include <string> and #include "
               "<vector> entries are "
