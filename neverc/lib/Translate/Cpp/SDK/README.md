...
conversions keep the exact unqualified type and mutation requires recursive
assignability. Programs using scalar pair
...
outputs accept checked direct scalar conversions. The copy-producing equality
and predicate algorithms below accept the same conversions for their writable
outputs.
...
`partition`, `stable_partition`, `partition_copy` and `partition_point`.
Checked function-pointer binary predicates with directly convertible by-value
scalar parameters additionally admit `adjacent_find`, three- and four-iterator
...
`transform_inclusive_scan` forms and `transform_exclusive_scan`, accept
function pointers with directly convertible scalar results and by-value scalar
parameters. `gcd` and `lcm` lower for non-boolean built-in integer
combinations through 64 bits. None of these operations requires platform
headers or a libc++ runtime. Callable objects, reference parameters or results,
record callback results and the remaining C++17 numeric components stay outside
...
headers are authenticated without platform headers. Typed and transparent
...
`double` specializations authenticate the same `__scalar_hash` base plus the
...
exact same-type local copy-initializer chains are authenticated and erased with
the wrapper carrier. A local automatic member pointer with one exact direct address of an
...
member pointers, `mem_fn` copies from parameters, reassigned `mem_fn` objects,
base adjustments,
