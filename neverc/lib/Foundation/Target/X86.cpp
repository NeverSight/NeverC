...
namespace {
constexpr Builtin::Info BuiltinInfoX86[] = {
...
bool X86TargetInfo::initFeatureMap(
  ...
  // Only x86-64 is supported for this target; SSE2 is always present.
  ...
  for (const auto &Feature : FeaturesVec) {
    ...
    if (F.size() > 1 && F.drop_front(1).starts_with("avx10.")) {
      if (F[0] == '+') {
        HasAVX10 = true;
        if (F.ends_with("512"))
          HasAVX10_512 = true;
        LastAVX10 = Feature;
      } else if (HasAVX10 && F == "-avx10.1-256") {
        HasAVX10 = false;
        HasAVX10_512 = false;
      } else if (HasAVX10_512 && F == "-avx10.1-512") {
        HasAVX10_512 = false;
      }
      UpdatedAVX10FeaturesVec.push_back(Feature);
      continue;
    } else if (!HasAVX512F && F.starts_with("+avx512")) {
      HasAVX512F = true;
      LastAVX512 = Feature;
    } else if (HasAVX512F && F == "-avx512f") {
      HasAVX512F = false;
    } else if (HasEVEX512 != FE_TRUE && F == "+evex512") {
      HasEVEX512 = FE_TRUE;
      continue;
    } else if (HasEVEX512 != FE_FALSE && F == "-evex512") {
      HasEVEX512 = FE_FALSE;
      continue;
    }
...
bool X86TargetInfo::handleTargetFeatures(std::vector<std::string> &Features,
  ...
  // x87-backed long double needs x87 code generation support.
...
void X86TargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  case CK_Clearwaterforest:
    // Recent primary x86 CPUs intentionally share this legacy macro.
...
namespace {
llvm::X86::ProcessorFeatures getFeature(llvm::StringRef Name) {
...
namespace {
unsigned x86MatchAsmCCConstraint(const char *Name) {
...
bool X86TargetInfo::validateAsmConstraint(
  ...
  case '@':
    ...
    if (auto Len = x86MatchAsmCCConstraint(Name)) {
      Name += Len - 1;
      Info.setAllowsRegister();
      return true;
    }
...
std::string X86TargetInfo::convertConstraint(const char *&Constraint) const {
  switch (*Constraint) {
  case '@':
    if (auto Len = x86MatchAsmCCConstraint(Constraint)) {
...
void X86TargetInfo::fillValidCPUList(
...
llvm::ArrayRef<TargetInfo::AddlRegName>
