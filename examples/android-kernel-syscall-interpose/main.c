...
/*
 * NeverC syscall-interpose demo — sys_call_table pointer replacement.
 *
 * Interposes __NR_openat by swapping the function pointer in sys_call_table.
...
static long interpose_openat(const struct pt_regs *regs)
{
	char buf[256];
	long ret;
	int pid;
...
static int neverc_krt_syscall_interpose_init(void)
{
	int ret;
	...
	ret = neverc_krt_syscall_replace(__NR_openat,
					 (neverc_krt_syscall_fn_t)interpose_openat,
					 &orig_openat);
	if (ret) {
	...
	neverc_krt_log_info("openat interposed\n");
	return 0;
}

static void neverc_krt_syscall_interpose_exit(void)
{
	if (orig_openat)
...
module_init(neverc_krt_syscall_interpose_init);
module_exit(neverc_krt_syscall_interpose_exit);

MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("NeverC");
MODULE_DESCRIPTION("NeverC syscall-interpose demo (sys_call_table)");

NEVERC_KRT_DEFINE_MODULE("neverc_krt_syscall_interpose");
