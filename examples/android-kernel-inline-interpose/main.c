...
/*
 * NeverC function-interpose demo.
 *
 * Demonstrates neverc_krt_interpose_register:
 *  - Interpose do_faccessat at function entry
...
#define NEVERC_KRT_LOG_TAG "neverc_krt_interpose_demo"
#include <nvk_log.h>
...
/* ----------------------------------------------------------------
 * Interpose handler: logs faccessat calls and forwards to original.
 * ---------------------------------------------------------------- */

static struct neverc_krt_interpose_ref faccessat_ref;
static void *orig_faccessat;

static long interpose_faccessat(void *orig, void *a0, void *a1,
...
static int neverc_krt_interpose_demo_init(void)
{
	void *target;
	int ret;
	...
	ret = neverc_krt_interpose_register(target, (void *)interpose_faccessat,
				       0, &orig_faccessat, &faccessat_ref);
	if (ret) {
		neverc_krt_log_err("interpose_register failed: %d\n", ret);
		return ret;
	}

	neverc_krt_log_info("interposed do_faccessat (priority=0)\n");
	return 0;
}

static void neverc_krt_interpose_demo_exit(void)
{
	neverc_krt_interpose_unregister(&faccessat_ref);
	neverc_krt_log_info("unloaded\n");
}

module_init(neverc_krt_interpose_demo_init);
module_exit(neverc_krt_interpose_demo_exit);

MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("NeverC");
MODULE_DESCRIPTION("NeverC function-interpose demo");

NEVERC_KRT_DEFINE_MODULE("neverc_krt_interpose_demo");
