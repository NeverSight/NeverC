...
| [字符串加密 (xorstr)](builtins/xorstr/README.zh-CN.md) | `-fencrypt-call-strings` | 逐实例加密、强制 late 封口、逐调用点最终展开与 volatile 栈清零 |
...
| [`neverc translate`](translate/README.zh-CN.md) | 将 C++ 转译为 NeverC |
...
| [发布二进制与 `--strip`](release-builds/README.zh-CN.md) | 删除非运行时符号与源码调试信息，并对 `.ko` 进行内核感知的结构化符号重命名（不是 hash，也不是 encryption） |
