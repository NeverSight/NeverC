...
Un plugin que registre su propio destino puede en cambio aportar una retrollamada `PlanCallingConvention` en su `NevercCallingConventionDescriptor` y producir planes directamente, saltándose la capa de spec. Véase [Destino, MC, ensamblador, objetos](plugin-api-target-mc-object.md#abi-y-convenciones-de-llamada).
...
Vea [`PluginIR.h`] para la tabla de núcleo usada arriba, [`PluginTarget.h`] para `NevercCallingConventionDescriptor` y [`Schema/PhaseSchema.json`] para la fase `neverc.ir.pass.post_opt` a la que se engancha el pase.
