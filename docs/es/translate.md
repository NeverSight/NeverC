...
Core v2 admite registros anidados con nombre y sin plantillas de clase, incluidos tipos privados o protegidos expuestos mediante alias o fábricas permitidos. Cada objeto conserva su almacenamiento y receptor; las referencias explícitas al objeto exterior, la identidad de tipo por ámbito, copias y movimientos de miembros y arrays, destrucción e iteradores anidados mantienen su semántica habitual. Las dependencias por valor se emiten primero. Los registros anidados anónimos, las plantillas de clase, la herencia y la STL completa siguen pendientes; la validación nativa requiere el CI de la revisión que lo implementa.
...
Core v2 admite espacios de nombres inline, alias, directivas using y declaraciones using resueltas en espacios de nombres y bloques. Las funciones, variables, tipos y enumeradores sin ámbito importados conservan su identidad, las reglas de búsqueda, los controles de acceso y la vida de los objetos. Los espacios inline conservan la búsqueda combinada con el padre y la búsqueda dependiente de argumentos en ambas direcciones, incluso al anidarlos o reabrirlos. La importación de miembros de clase, los constructores heredados, las plantillas de clase y las importaciones de enumeradores o sintaxis inline anidada explícita de C++20 quedan fuera de esta etapa; C++/STL completo sigue pendiente.
...
Core v2 admite plantillas de funciones libres con hasta 64 parámetros de tipo sin paquetes y argumentos de tipo compatibles. Deducción, instanciación y especialización explícitas, tipos predeterminados, argumentos predeterminados resueltos, recursión e importaciones producen funciones tipadas ordinarias. Cada instancia conserva tipos locales y almacenamiento estático escalar propios, incluso entre plantillas primarias con firmas idénticas. Los cuerpos genéricos y valores predeterminados diferidos se revisan al instanciarse; se revisa todo cuerpo materializado. Las llamadas con solo firma dentro de sizeof/noexcept aún exigen definición. Plantillas de clases, miembros, operadores y amigas, parámetros no tipo, paquetes, cabeceras estándar y STL completo siguen pendientes. La validación nativa requiere el CI de la revisión correspondiente.
