**Idiomas**: [English](../dyncode-compiler.md) | [简体中文](../zh-CN/dyncode-compiler.md) | [繁體中文](../zh-TW/dyncode-compiler.md) | [日本語](../ja/dyncode-compiler.md) | [한국어](../ko/dyncode-compiler.md) | [Français](../fr/dyncode-compiler.md) | [Deutsch](../de/dyncode-compiler.md) | [Español](dyncode-compiler.md) | [Italiano](../it/dyncode-compiler.md) | [Русский](../ru/dyncode-compiler.md) | [العربية](../ar/dyncode-compiler.md)
...
- [Tutorial de ensamblaje ARM64 (AArch64) — Perspectiva DynCode](dyncode-compiler-arm64-assembly-tutorial.md)
- [Arquitectura multiplataforma de NeverC DynCode — Resumen](dyncode-compiler-cross-platform-architecture.md)
- [Diseño de pasadas IR — Principios, pipeline y ejemplos antes/después](dyncode-compiler-ir-pass-design.md)
- [Soporte de dyncode modo kernel (Ring-0)](dyncode-compiler-kernel-mode-dyncode.md)
- [Diseño de pasadas MIR — Principios y puntos de interpose](dyncode-compiler-mir-pass-design.md)
- [Pipeline de DynCode, MIR y Estrategia PIC (Notas de Diseño)](dyncode-compiler-pipeline-and-pic.md)
- [Guía de extensión de plataforma](dyncode-compiler-platform-extension-guide.md)
- [Compilador de dyncode — Seguimiento de progreso](dyncode-compiler-progress.md)
- [Hoja de ruta](dyncode-compiler-roadmap.md)
...
Los ocho triples (OS, arch) comparten **el mismo conjunto de passes**. Las diferencias están en `TargetDesc.cpp` y tres ramas de extractores. Nueva plataforma = una fila + un case por extractor. `ExecutionLevel` es ortogonal: `User` → syscall / PEB; `Kernel` desactiva ambos e inyecta `KernelImportPass` para reescribir llamadas externas mediante shims. Véase [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md).
...
| `-fdyncode-obfuscate=<spec>` | Pasa a interposes de plugin **nivel IR** vía la [API de Plugins](plugin-api.md). No-op sin plugin cargado. Ver [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes). |
| `-fdyncode-mir-obfuscate=<spec>` | Pasa a interposes **nivel MIR** (`RunBeforePreEmit` / `RunAfterPreEmit`). Por defecto `-fdyncode-obfuscate=` si no se define. Ver [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-interposes-de-ofuscación-del-usuario). |
...
## Ofuscación y extensión de plugins

El pipeline dyncode en sí solo garantiza que «el código se ejecute correctamente». La ofuscación, el polimorfismo, los codificadores por etapas y funciones similares de la capa de estrategia **no están integrados a propósito** — los proporcionan plugins fuera del árbol a través de la [API de Plugins](plugin-api.md).

El pipeline expone **11 puntos de enganche** en tres capas, todos accesibles vía la C Plugin API (`NEVERC_INTERPOSE_SC_*`):

**Nivel IR (6 interposes)**:
...
**Nivel flujo de bytes (2 interposes)**:
...
Consulta la [documentación de la API de Plugins](plugin-api.md) para la lista completa de interposes, registro de pases y ejemplos de código.

- IR-level: [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes).
- MIR-level: [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-interposes-de-ofuscación-del-usuario)
---
