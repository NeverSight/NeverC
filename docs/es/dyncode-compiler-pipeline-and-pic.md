**Idiomas**: [English](../dyncode-compiler-pipeline-and-pic.md) | [简体中文](../zh-CN/dyncode-compiler-pipeline-and-pic.md) | [繁體中文](../zh-TW/dyncode-compiler-pipeline-and-pic.md) | [日本語](../ja/dyncode-compiler-pipeline-and-pic.md) | [한국어](../ko/dyncode-compiler-pipeline-and-pic.md) | [Français](../fr/dyncode-compiler-pipeline-and-pic.md) | [Deutsch](../de/dyncode-compiler-pipeline-and-pic.md) | [Español](dyncode-compiler-pipeline-and-pic.md) | [Italiano](../it/dyncode-compiler-pipeline-and-pic.md) | [Русский](../ru/dyncode-compiler-pipeline-and-pic.md) | [العربية](../ar/dyncode-compiler-pipeline-and-pic.md)

[← Compilador de dyncode](dyncode-compiler.md)

# Pipeline de DynCode, MIR y Estrategia PIC (Notas de Diseño)

Este documento describe los compromisos de diseño en el modo dyncode de NeverC a través de la cadena **IR → optimización LLVM → backend MIR → archivo objeto → extracción/parcheo**, y su relación con la política de **PIC predeterminado a nivel de compilador**. Los detalles de implementación son autoritativos en el código fuente y los comentarios en inglés.

## 1. Por qué forzar PIC por defecto (incluyendo compilación no-dyncode)

El extractor de dyncode asume que las referencias a símbolos externos en el fragmento ejecutable caen en relocalizaciones **relativas a PC** o resolubles intra-`.text`, no en direcciones absolutas codificadas o pools de constantes que requieren un cargador para llenar `.data`.
...
- La compilación C normal y la compilación `-fdyncode` comparten los mismos hábitos de relocalización, reduciendo la carga cognitiva "funciona normalmente, falla bajo dyncode".
...
Esta política no distingue si `-fdyncode` está habilitado o si el contexto es user/kernel. Incluso si el usuario pasa `-fno-pic` / `-static` / `-mkernel` / `-mdynamic-no-pic`, `ParsePICArgs()` mantiene `Reloc::PIC_`, usando las mismas suposiciones relativas a PC para compilación normal, dyncode modo usuario y dyncode modo kernel.

## 2. División de trabajo IR y MIR en dos fases

### 2.1 Capa IR (`registerDynCodePasses`)
...
2. **`DynCodeMIRPrepPass`**: Elimina pseudos que generarían secciones laterales `.eh_frame` / `.pdata` / `.xray_*`, haciendo que el flujo de instrucciones antes de AsmPrinter sea lo más cercano a "código puro" posible.
...
**Principio**: Si las secuencias de instrucciones nativas aún tienen problemas, corregir en MIR (especialmente alrededor de `DynCodeMIRPrepPass`); **la extracción y el parcheo son la última red de seguridad**, evitando duplicar la misma lógica en las capas COFF/ELF/Mach-O.

Los nombres de opcode MIR no se dispersan en el flujo de control del pass; `DynCodeMIRPrepPass` usa la tabla `(pattern, role, opcode)` de `Tables/MIRRewriteOpcodes.def` vía `TargetInstrInfo::getName()`. Al agregar sustituciones de instrucciones amigables con dyncode, se prefiere agregar entradas de tabla y reescrituras MIR pequeñas; solo recurrir a cambios de selección de instrucciones del backend `.td` cuando sea necesario, con el fallback de formato de objeto a nivel de extractor como último recurso.

> Nota: `DynCodeMIRPrepPass` solo se registra cuando `-fdyncode` está habilitado. Los programas normales no deben eliminar globalmente CFI/EH, ya que esto rompería el manejo normal de excepciones y la información de depuración.

Tanto los callbacks globales de IR como de MIR usan un patrón de **registrar una vez, leer la instantánea actual de `DynCodeOptions` en tiempo de ejecución**. Esto soporta procesos de compilador embebido de larga duración: cuando el mismo proceso primero compila dyncode y luego C normal, la compilación C normal no hereda los passes IR/MIR anteriores; al compilar múltiples TUs de dyncode consecutivamente, los registros duplicados de callbacks globales no apilan el mismo conjunto de passes múltiples veces.
...
Ambas fuentes funcionan sin registro ni variables de entorno VS proporcionadas por el SO, habilitando la compilación cruzada de dyncode Windows desde macOS / Linux.

## 5. Puntos de ofuscación y extensión

- **Interposes IR**: 6 puntos de enganche IR (`NEVERC_INTERPOSE_SC_BEFORE_PREP` a `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) vía la [API de Plugins](plugin-api.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` es el verdadero último punto de inyección IR — los passes registrados aquí no tienen passes subsecuentes que modifiquen su salida. 11 puntos de interpose en total (6 IR + 3 MIR + 2 flujo de bytes).
- **Ofuscación MIR**: `RunBeforePreEmit` / `RunAfterPreEmit` son interposes MIR de granularidad media; `RunAfterFinalMIR` es el **verdadero último** interpose MIR (extensión fork agrega `RegisterTargetPassConfigPostPreEmitCallbackFn` invocado después de `addPreEmitPass2()`). `-fdyncode-mir-obfuscate=` especifica MIR spec por separado; por defecto usa IR spec si no se establece.
- **Interposes de flujo de bytes**: `RunPostExtract` es el interpose **pre**-finalize; `RunPostFinalize` es el interpose **post**-finalize (último momento antes de escritura a disco, NeverC no audita más).
- **Tamaño / alineación / relleno**: `-fdyncode-max-length=`, `-fdyncode-align=`, `-fdyncode-pad=` se ejecutan al final de finalize; el driver rechaza configuraciones contradictorias.
...
El modo dyncode introduce `-mdyncode-context=user|kernel` como la segunda dimensión del pipeline, superpuesta al triple:
...
- **Modo kernel**:
  ...
  - `<neverc/dyncode/kernel.h>` expone `neverc_kern_resolve_t`, `neverc_kern_hash()` y firmas relacionadas del lado kernel; los shims de modo usuario (`<windows.h>`, `<unistd.h>`, etc.) rechazan en modo kernel vía `#error`.

Ver [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md) para detalles.
...
| Tema | Enfoque |
|------|---------|
| PIC predeterminado | Todas las toolchains `isPICDefaultForced()==true`, alineado con suposiciones dyncode |
| Corregir primero en IR | Constantes, saltos indirectos, intrínsecos de memoria eliminados en IR cuando es posible |
| Red de seguridad MIR | `DynCodeMIRPrepPass` + interposes pre/post, luego extracción/parcheo de objeto como último recurso |
| Minimizar codificación dura | `TargetDesc` + `Options.td.h` dirigido por tablas |
| Dos dimensiones user/kernel | `-fdyncode` × `-mdyncode-context={user,kernel}`; cada (OS, arch, level) es una fila en `describeTriple()` |
...
Los headers shim (`sys/mman.h`, `fcntl.h`, etc.) exponen constantes que deben coincidir con el ABI del kernel objetivo, ya que los stubs de syscall de dyncode pasan estos valores directamente al kernel sin traducción de libc.
