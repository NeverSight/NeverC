...
Un plugin que registre su propio destino puede en cambio aportar una retrollamada `PlanCallingConvention` en su `NevercCallingConventionDescriptor` y producir planes directamente, saltándose la capa de spec. Véase [Destino, MC, ensamblador, objetos](target-mc-object.md#abi-y-convenciones-de-llamada).
