**Idiomas**: [English](../dyncode-compiler-platform-extension-guide.md) | [简体中文](../zh-CN/dyncode-compiler-platform-extension-guide.md) | [繁體中文](../zh-TW/dyncode-compiler-platform-extension-guide.md) | [日本語](../ja/dyncode-compiler-platform-extension-guide.md) | [한국어](../ko/dyncode-compiler-platform-extension-guide.md) | [Français](../fr/dyncode-compiler-platform-extension-guide.md) | [Deutsch](../de/dyncode-compiler-platform-extension-guide.md) | [Español](dyncode-compiler-platform-extension-guide.md) | [Italiano](../it/dyncode-compiler-platform-extension-guide.md) | [Русский](../ru/dyncode-compiler-platform-extension-guide.md) | [العربية](../ar/dyncode-compiler-platform-extension-guide.md)

[← Compilador de dyncode](dyncode-compiler.md)

# Guía de extensión de plataforma

Este documento explica cómo extender el compilador de dyncode a nuevas plataformas objetivo. Actualmente soportado: **arm64 / x86_64 en macOS / Linux / Android / Windows** (8 triples), cada uno con contextos independientes **User** / **Kernel** (16 variantes en total). Agregar una nueva plataforma típicamente requiere unos cientos de líneas de código.
...
| `DriverInjectFlags` | Flags específicos de plataforma en modo dyncode | null → sin inyección |
...
2. Agregar case en switch de despacho de `DynCodeExtractor.cpp`
3. Escribir `<Format>Extractor.cpp` (siguiendo estructura de `ELFExtractor.cpp`)

### 4. Agregar Loader (solo herramienta de prueba)

Referencia [`tests/neverc/dyncode/loader_linux.c`] y `loader_windows.c`. Típicamente: `mmap(RWX) → memcpy → icache flush → call`.
...
La pipeline dyncode expone 11 interposes vía `Pipeline.h::ObfuscationInterposes` para bibliotecas de ofuscación de terceros:
...
// Registrar vía Plugin API: interposes NEVERC_INTERPOSE_SC_* (ver docs de plugin-api)
```
...
El patching MIR integrado también es dirigido por tablas: `Tables/MIRRewritePatterns.def` registra nombres de diagnóstico de patrones, filtros de arquitectura y nombres de helpers; `Tables/MIRRewriteOpcodes.def` registra nombres de opcodes del backend. Al agregar nuevas formas de backend compatibles con dyncode, preferir entradas de tabla y helpers estrechos sobre dispersar ramas específicas de objetivo en el cuerpo del pass.
