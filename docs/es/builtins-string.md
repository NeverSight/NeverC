**Idiomas**: [English](../builtins-string.md) | [简体中文](../zh-CN/builtins-string.md) | [繁體中文](../zh-TW/builtins-string.md) | [日本語](../ja/builtins-string.md) | [한국어](../ko/builtins-string.md) | [Français](../fr/builtins-string.md) | [Deutsch](../de/builtins-string.md) | [Español](builtins-string.md) | [Italiano](../it/builtins-string.md) | [Русский](../ru/builtins-string.md) | [العربية](../ar/builtins-string.md)

[← Sistema de Runtime Integrado de NeverC](builtins.md) · [Documentación NeverC](README.md)
...
**Activación:** Pase `-fbuiltin-string` en tiempo de compilación (desactivado por defecto). Se activa automáticamente cuando:
- El archivo de entrada tiene extensión `.nc` (ver [documentación de extensión `.nc`](nc-extension.md))
- El modo `-fdyncode` está activo

```bash
neverc hello.nc -o hello                # automático — .nc lo activa
neverc -fbuiltin-string main.c -o main  # flag explícito para archivos .c
```
...
#### Limpieza automática de tipos compuestos

El compilador detecta y libera recursivamente los miembros `string` en arrays, estructuras y combinaciones anidadas:
...
La capa CodeGen recorre el árbol de tipos para generar llamadas de limpieza por elemento. Solo se liberan las cadenas owned (`cap != 0`); las vistas se omiten.
...
`.encrypt()` proporciona cifrado XOR en tiempo de compilación para literales de cadena. El texto plano nunca se almacena en el binario — se cifra en tiempo de compilación y se descifra en tiempo de ejecución mediante una función `always_inline`, haciendo que las herramientas de análisis estático (ej. `strings`) no puedan extraer el contenido original.

### Uso

```c
string secret = "API_KEY_12345".encrypt();
printf("%s\n", secret.c_str());  // imprime "API_KEY_12345" en tiempo de ejecución

// pero `strings ./binary` NO encontrará "API_KEY_12345"
```
...
`.encrypt()` soporta automáticamente todos los tipos de literal — el compilador convierte literales wide/UTF-16/UTF-32 a UTF-8 antes del cifrado:
...
```c
string s = "hello";
string e = s.encrypt();  // ERROR: .encrypt() can only be applied to string literals
```

- `.encrypt()` **no acepta argumentos**:

```c
string e = "hello".encrypt(42);  // ERROR: .encrypt() takes no arguments
```

- El cifrado doble es rechazado:

```c
string e = "hello".encrypt().encrypt();  // ERROR: .encrypt() can only be applied to string literals
```
