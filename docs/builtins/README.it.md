...
| [**`xorstr`**](xorstr/README.it.md) | `-fencrypt-call-strings` | Disattivato | Crittografia per istanza, sigillatura tardiva obbligatoria, espansione per call site e pulizia volatile dello stack |
| [**`strhash`**](strhash/README.it.md) | `-fstrhash-algo` / `-fstrhash-fold` | Disattivato | Hash di stringhe a tempo di compilazione, stesso algoritmo a runtime, fold IR opzionale |
...
> **Nota:** `xorstr` non usa il modello di bitcode incorporato. `semaBuiltinNeverCXorstr` in `SemaCheckingBuiltinNeverC.cpp` abbassa la macro esplicita. `EncryptCallStringsPass` e `XorStrCleanupPass` sigillano letterali automatici e storage in chiaro prima dell'IPO e dopo ogni fase IR tardiva ordinaria o di plugin. `FinalizeXorStrPass` ricifra ed espande i decoder espliciti solo a un vero confine di codice macchina nativo, quindi rimuove il grafo helper condiviso. La [documentazione xorstr](xorstr/README.it.md) descrive il design e il contratto di riproducibilità.

> **Nota:** `strhash` non usa il modello di bitcode incorporato. [`NC_STRHASH(s)`](strhash/README.it.md) si riduce a una costante in Sema; `-fstrhash-fold` abilita `StrHashFoldPass`. Vedi la [documentazione strhash](strhash/README.it.md).
