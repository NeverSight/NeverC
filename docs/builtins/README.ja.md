...
| 組み込み | フラグ | デフォルト | 説明 |
|---------|--------|----------|------|
| [**`string`**](../builtin-string/README.ja.md) | `-fbuiltin-string` | オフ | 値セマンティクスの文字列型。ドットコールメソッド、自動メモリ管理、ネイティブ UTF-8 対応 |
| [**mimalloc**](mimalloc/README.ja.md) | `-fbuiltin-mimalloc` | オフ | 高性能メモリアロケータ。`malloc`/`free`/`calloc`/`realloc` を透過的に置換 |
...
| **プリプロセッサマクロ** | `__NEVERC_BUILTIN_STRING__` | `__NEVERC_MIMALLOC__` |
