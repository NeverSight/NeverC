...
Éditeur de liens intégré · Pipeline dyncode · Runtimes intégrés (`string` · `mimalloc` · `xorstr` · `strhash`)
...
- **Extensible sans friction** — Un seul en-tête C, 130 phases de compilation nommées, et vous avez un [plugin compilateur](../plugin-api/README.fr.md) capable d'intervenir à toute étape — de l'optimisation IR à la sortie binaire finale — sans connaître LLVM.
...
- **[Dépouillement des versions](../release-builds/README.fr.md)** — `--strip` / `-s` intégré retire des images ELF, Mach-O et PE/COFF finales les symboles non requis à l'exécution et le débogage source, avec renommage structurel des symboles `.ko` adapté au noyau (ni hash ni encryption)
...
- **[Runtimes intégrés](../builtins/README.fr.md)** — runtimes LLVM bitcode intégrés au compilateur : [`string`](../builtins/string/README.fr.md) (chaîne à sémantique de valeur, gestion mémoire automatique), [`mimalloc`](../builtins/mimalloc/README.fr.md) (remplacement transparent d'allocateur haute performance, activé par défaut hors cibles noyau et freestanding), [`xorstr`](../builtins/xorstr/README.fr.md) (chiffrement par instance, scellement tardif obligatoire et développement natif par site d'appel) et [`strhash`](../builtins/strhash/README.fr.md) (hachage de chaînes à la compilation, même algorithme à l'exécution)
- **[API Plugin](../plugin-api/README.fr.md)** — ABI C pure pour les greffons hors arbre ; SDK à en-tête unique, zéro dépendance LLVM/CRT, couvrant les phases pilote, préprocesseur, AST, IR, MIR, MC, objet, édition de liens, LTO et dyncode
...
Une installation release peut mettre à jour le compilateur et les runtimes de compilation croisée déjà installés comme une seule unité versionnée :

```bash
neverc update                 # dernière release complète
neverc update v3389.1.2       # version exacte, y compris un retour en arrière
```

`neverc upgrade` est un alias. NeverC résout un seul tag release concret et ne réinstalle que
les runtimes déjà présents, tous épinglés sur la version cible du compilateur. Avant toute
modification des fichiers actifs, les archives requises sont téléchargées, vérifiées par SHA256,
extraites et validées. Un échec de préparation ou de vérification laisse l’installation intacte,
...
Lors de la copie ou de l’adaptation de code, y compris avec l’aide d’une IA ou d’un LLM ou dans le cadre d’une réutilisation liée à LLVM, respectez la licence applicable et conservez les mentions de droits d’auteur, de licence et d’attribution requises. Par ailleurs, nous demandons aux personnes utilisant NeverC comme référence de citer le projet et la source d’origine ; cette demande du projet ne constitue pas une condition de licence supplémentaire.

Consultez les [mentions](../../NOTICE), le [guide d’attribution (anglais/chinois)](../../ATTRIBUTION.md) et la [référence bibliographique lisible par machine](../../CITATION.cff).
