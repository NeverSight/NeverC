...
- **真のクロスコンパイル** — macOS や Linux から Windows 実行ファイルや shellcode をビルド——VM 不要、デュアルブート不要、SDK 探し不要。Windows SDK はコンパイラに内蔵。
- **[プラグインシステム](../plugin-api/README.ja.md)、ヘッダー1つで完結** — 純粋な C API、20+ のフックポイントが IR・MIR・Binary・Linker の全段階をカバー。任意の言語でプラグインを書き、コンパイルパイプラインのほぼすべての実行ポイントに介入可能——LLVM ヘッダー不要。
