...
- **[Release-Stripping](../release-builds/README.de.md)** — integriertes `--strip` / `-s` entfernt nicht zur Laufzeit nötige Symbole und Quelldebuginformationen aus finalen ELF-, Mach-O- und PE/COFF-Images und benennt `.ko`-Symbole kernelgerecht strukturell um (kein hash und keine encryption)
