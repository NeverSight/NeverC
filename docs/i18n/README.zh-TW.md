...
[文件索引](../README.zh-TW.md) · [Shellcode 指南](../shellcode-compiler/README.zh-TW.md) · [內建字串](../builtin-string/README.zh-TW.md)

</div>

---

> **說明：** GitHub 倉庫首頁固定顯示英文 `README.md`，不會依瀏覽器語言自動切換。請用上方語言連結；進入 [文件](../README.zh-TW.md) 或 [shellcode 指南](../shellcode-compiler/README.zh-TW.md) 後，請透過頁內語言欄與麵包屑保持同一語言。
...
- **[Shellcode 編譯器](../shellcode-compiler/README.zh-TW.md)** — 多階段 IR/MIR 管線、跨平台擷取、匯入/系統呼叫降級、核心模式、壞位元組稽核與外掛架構
- **整合連結器** — 單一二進位內完成 COFF、ELF、Mach-O 連結，無需外部 `ld` 或 `link.exe`
- **交叉編譯** — 在 macOS/Linux 上建置 Windows PE，支援捆綁 MSVC SDK
- **[內建 `string` 型別](../builtin-string/README.zh-TW.md)** — 值語義字串，支援點號方法語法、自動記憶體管理與原生 UTF-8 後備
