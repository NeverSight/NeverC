...
`neverc run -O2 -fbuiltin-string hello.c`. Параметры компилятора указываются перед
...
`neverc run hello.c -O2 -fbuiltin-string --`. Временный исполняемый файл наследует
