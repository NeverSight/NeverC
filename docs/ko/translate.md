...
Core v2는 중첩 레코드와 다차원 배열을 포함한 암시적 및 명시적 default 이동 생성과 대입도 지원합니다. 멤버는 선언 및 요소 순서에 따라 C++가 선택한 복사나 이동을 실행합니다. 생성된 이동은 기본 멤버 초기화를 반복하지 않으며 trivial 연산은 저장된 포인터 값을 보존합니다. 원본은 정상 소멸되고 사용하지 않거나 trivial인 생성 연산에는 가상의 본문이 필요하지 않습니다. 기존 인라인 암시적 trivial 연산의 임시 객체 동작은 유지하며 일반 임시 참조 바인딩은 아직 허용하지 않습니다. 완전한 C++／STL 지원은 개발 중입니다.
...
Core v2는 살아 있는 객체의 일반 변환 함수도 지원합니다. 암시적·명시적 정수, 열거형, 포인터 변환과 조건식의 명시적 `bool`, 참조 및 객체 반환을 처리합니다. 변환마다 선택된 멤버 함수를 한 번 호출합니다. 참조는 별칭을 유지하고 객체 prvalue는 실제 목적지를 직접 초기화하며, 참조에서 값으로 변환할 때는 선택된 복사나 이동을 수행합니다. const 및 참조 한정자, constexpr, noexcept는 C++17 선택 규칙을 따릅니다. 임시 참조 수명, 템플릿 및 전체 STL은 개발 중입니다.

Core v2는 임시 객체 수신자와 전체 표현식이 끝날 때까지 살아 있는 스칼라·레코드 임시 참조 인수를 지원하며 생성자, 메서드, 연산자 및 변환 함수에 적용됩니다. 평가마다 실제 저장 공간을 사용하고 객체 결과의 최종 목적지와 참조 별칭을 유지합니다. 호출과 피호출 함수의 매개변수 소멸이 끝난 뒤 생성 역순으로 임시 객체를 정리하며 조건 분기와 루프의 반복 평가도 처리합니다. 임시 레코드의 배열 하위 객체도 지원합니다. 독립 배열 임시 값, 자동·정적 참조 수명 연장, 템플릿 및 전체 STL은 개발 중입니다.
