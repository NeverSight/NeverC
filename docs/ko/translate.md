...
Core v2는 산술, 비교, 첨자, 역참조, 증가, 함수 객체와 일반 대입 시그니처를 포함한 일반 멤버 및 비멤버 연산자 오버로드를 지원합니다. 선택된 함수, 참조 별칭과 객체 결과를 유지합니다. 연산자 표기는 C++17 평가 순서를 따르며 오버로드된 논리 연산자는 두 피연산자를 모두 평가합니다. 비멤버 대입 연산자는 매개변수를 오른쪽부터 초기화한 뒤 역순으로 소멸시킵니다. 변환 함수, 템플릿, 메모리 할당과 전체 STL은 계속 개발 중입니다.
