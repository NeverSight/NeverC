...
Core v2는 원본 매개변수가 `R&` 또는 `const R&`인 일반 사용자 정의 복사 생성자와 복사 대입 연산자를 지원합니다. 실제 대상에서 복사하며 부수 효과와 반환 참조를 유지합니다. 대입 연산자 구문은 오른쪽 피연산자를 먼저 평가하고, 명시적 `operator=` 호출은 수신 객체를 먼저 평가합니다. default 복사 연산와 암시적 비자명 복사는 아직 지원하지 않습니다.

Core v2는 컴파일러가 생성하거나 default로 지정한 기본 생성자와 default 소멸자도 지원하며 중첩 레코드 및 배열 멤버를 처리합니다. 실제 대상에서 선언 순서대로 초기화하고, 값 초기화는 C++ 규칙이 요구할 때만 0 초기화를 수행합니다. 클래스 밖의 default 지정에 따른 초기화 차이도 유지합니다. 사용하지 않거나 평가되지 않는 식에만 나타나는 생성자에 별도 함수 본문을 요구하지 않습니다. 기본 멤버 초기화, 암시적 비자명 복사, 이동 연산과 전체 STL은 계속 개발 중입니다.
