...
Core v2는 살아 있는 `R&&` 또는 `const R&&`를 받는 일반 사용자 정의 이동 생성자와 이동 대입을 실행합니다. 생성은 실제 대상 저장소를 사용하며 대입은 원본 변경, 피연산자 평가 순서와 반환된 `R&` 별칭을 보존합니다. `&`／`&&` 한정 대입도 지원합니다. 이름이 있는 오른값 참조는 왼값 오버로드를 선택하고 명시적 생성자는 초기화 규칙을 따릅니다. 이동은 원본 수명을 끝내지 않으며 원본과 대상은 각각 정상 소멸됩니다. 생성／default 이동, 새 임시 객체 참조 바인딩, 예외, 템플릿과 전체 STL은 개발 중입니다.
