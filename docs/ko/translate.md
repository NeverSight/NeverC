...
Core v2는 지원되는 단순 집합체의 이름 있는 비가상 멤버 함수를 지원합니다. const 오버로드, lvalue 한정 메서드, 정적 메서드 및 `this`를 포함합니다. 호출은 원래 객체의 동일성을 유지하며 인수보다 먼저 수신 객체를 평가합니다. 임시 객체의 비정적 호출, 특수 멤버 함수, 상속, 템플릿 및 전체 STL은 아직 지원하지 않습니다.
