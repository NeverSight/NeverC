...
Core v2는 표준 레이아웃과 지원되는 복사 연산을 갖는 레코드의 일반 사용자 정의 생성자도 지원합니다. 지역 객체, 필드와 배열 요소는 최종 저장 위치에 직접 생성되며 필드는 선언 순서대로 초기화됩니다. 위임 생성자, 이동 생성자와 예외는 아직 지원하지 않습니다.
...
Core v2는 허용된 필드의 기본 멤버 초기화를 지원하며 앞선 멤버 접근, 일반 호출, 중첩 레코드와 배열을 처리합니다. 선택된 기본값의 `this`는 실제 소유 객체를 가리키고 명시적 집합체 초기화 항목은 호출자의 `this`를 유지합니다. 명시적 초기화는 해당 멤버의 기본값을 대체합니다. 암시적 또는 default 복사와 대입은 기본값을 다시 실행하지 않지만 사용자 복사 생성자는 생략한 멤버의 기본값을 사용할 수 있습니다. 생성자 멤버 초기화와 집합체 초기화는 각자의 임시 객체 정리 경계를 유지합니다. 이동, 템플릿과 전체 STL은 개발 중입니다.
