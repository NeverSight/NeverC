...
Core v2는 원본 매개변수가 `R&` 또는 `const R&`인 일반 사용자 정의 복사 생성자와 복사 대입 연산자를 지원합니다. 실제 대상에서 복사하며 부수 효과와 반환 참조를 유지합니다. 대입 연산자 구문은 오른쪽 피연산자를 먼저 평가하고, 명시적 `operator=` 호출은 수신 객체를 먼저 평가합니다. default 복사 대입과 암시적 비자명 복사 대입은 아직 지원하지 않습니다.

Core v2는 컴파일러가 생성하거나 default로 지정한 기본 생성자와 default 소멸자도 지원하며 중첩 레코드 및 배열 멤버를 처리합니다. 실제 대상에서 선언 순서대로 초기화하고, 값 초기화는 C++ 규칙이 요구할 때만 0 초기화를 수행합니다. 클래스 밖의 default 지정에 따른 초기화 차이도 유지합니다. 사용하지 않거나 평가되지 않는 식에만 나타나는 생성자에 별도 함수 본문을 요구하지 않습니다. 기본 멤버 초기화, 암시적 비자명 복사 대입, 이동 연산과 전체 STL은 계속 개발 중입니다.

Core v2는 암시적 및 default 복사 생성자를 지원하며 중첩 레코드와 다차원 배열도 처리합니다. 선택된 멤버 복사 생성자를 실제 대상에서 선언 및 요소 순서대로 호출하고 원본 배열 주소는 한 번만 평가합니다. 자명한 복사는 저장된 포인터 값을 그대로 유지합니다. 값 전달과 원본 객체 반환은 복사 부수 효과를 유지하며 순수 우측값의 직접 전달에는 추가 복사가 없습니다. 생성된 복사 대입, 이동 및 나머지 C++／STL 기능은 개발 중입니다.
