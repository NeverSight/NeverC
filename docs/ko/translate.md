...
Core v2는 암시적 및 default 복사 대입도 지원합니다. 멤버와 배열 요소의 선택된 대입 연산 및 순서를 유지하며 생성된 자명한 배열 복사는 외부 메모리 복사 호출 없이 타입이 있는 요소 대입으로 변환합니다. 자명한 대입은 저장된 포인터를 유지하고 실제 수신 객체의 참조를 반환합니다. 연산자 구문에서는 원본 참조를 먼저, 명시적 멤버 호출에서는 수신 객체를 먼저 확정한 후 부수 효과가 끝나면 원본 값을 읽습니다. 대입 때문에 객체를 추가로 생성하거나 소멸하지 않습니다. 기본 멤버 초기화, 이동, 템플릿과 전체 STL은 개발 중입니다.
