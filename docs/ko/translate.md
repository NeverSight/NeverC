...
Core v2는 이미 살아 있는 객체의 오른값 참조를 지원합니다. 스칼라, 포인터, 레코드와 배열의 별칭, 참조 매개변수와 반환값, 조건식 xvalue 및 일반 `&&` 한정 메서드를 포함합니다. `static_cast<R&&>(live)`는 같은 객체를 유지하며 이름이 있는 오른값 참조 변수는 여전히 왼값입니다. 오버로드와 기존 복사 동작은 Clang의 선택을 따릅니다. 참조는 추가 정리 소유자를 만들지 않습니다. 새 임시 객체 바인딩, 수명 연장과 미구현 이동 연산은 이번 단계에 포함되지 않습니다.

Core v2는 살아 있는 `R&&` 또는 `const R&&`를 받는 일반 사용자 정의 이동 생성자와 이동 대입을 실행합니다. 생성은 실제 대상 저장소를 사용하며 대입은 원본 변경, 피연산자 평가 순서와 반환된 `R&` 별칭을 보존합니다. `&`／`&&` 한정 대입도 지원합니다. 이름이 있는 오른값 참조는 왼값 오버로드를 선택하고 명시적 생성자는 초기화 규칙을 따릅니다. 이동은 원본 수명을 끝내지 않으며 원본과 대상은 각각 정상 소멸됩니다. 새 임시 객체 참조 바인딩, 예외, 템플릿과 전체 STL은 개발 중입니다.
