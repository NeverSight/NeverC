**언어**: [English](../../dyncode-compiler/platform-extension-guide.md) | [简体中文](../../zh-CN/dyncode-compiler/platform-extension-guide.md) | [繁體中文](../../zh-TW/dyncode-compiler/platform-extension-guide.md) | [日本語](../../ja/dyncode-compiler/platform-extension-guide.md) | [한국어](platform-extension-guide.md) | [Français](../../fr/dyncode-compiler/platform-extension-guide.md) | [Deutsch](../../de/dyncode-compiler/platform-extension-guide.md) | [Español](../../es/dyncode-compiler/platform-extension-guide.md) | [Italiano](../../it/dyncode-compiler/platform-extension-guide.md) | [Русский](../../ru/dyncode-compiler/platform-extension-guide.md) | [العربية](../../ar/dyncode-compiler/platform-extension-guide.md)

[← DynCode 컴파일러](README.md)

# 플랫폼 확장 가이드

이 문서는 dyncode 컴파일러를 새로운 타겟 플랫폼으로 확장하는 방법을 설명합니다. 현재 지원: **macOS / Linux / Android / Windows의 arm64 / x86_64** (8개 triple), 각각 독립된 **User** / **Kernel** 컨텍스트 (총 16개 변형). 새 플랫폼 추가는 보통 수백 줄의 코드로 충분합니다.
...
| `DriverInjectFlags` | dyncode 모드에서 주입되는 플랫폼별 플래그 | null → 주입 없음 |
...
2. `lib/Headers/windows.h`의 dyncode 분기에 선언 추가
3. pass 변경 불필요
...
2. `DynCodeExtractor.cpp`의 디스패치 switch에 case 추가
...
- **ABI 차이**: Win64(rcx/rdx/r8/r9) vs System V AMD64(rdi/rsi/rdx/rcx/r8/r9)는 완전히 다른 인수 레지스터. NeverC 프론트엔드 레이어에서 처리; dyncode 파이프라인은 관여 불필요.
...
dyncode 파이프라인은 `Pipeline.h::ObfuscationInterposes`를 통해 서드파티 난독화 라이브러리에 11개 훅을 노출합니다:
...
// Plugin API로 등록: NEVERC_INTERPOSE_SC_* 훅 (plugin-api 문서 참조)
```
