...
자체 타깃을 등록하는 플러그인은 대신 `NevercCallingConventionDescriptor`에 `PlanCallingConvention` 콜백을 제공해 spec 계층을 건너뛰고 plan을 직접 만들 수도 있습니다. [타깃, MC, 어셈블리, 오브젝트](plugin-api-target-mc-object.md#abi와-호출-규약)를 참고하세요.
