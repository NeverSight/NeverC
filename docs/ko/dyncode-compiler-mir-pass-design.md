**언어**: [English](../dyncode-compiler-mir-pass-design.md) | [简体中文](../zh-CN/dyncode-compiler-mir-pass-design.md) | [繁體中文](../zh-TW/dyncode-compiler-mir-pass-design.md) | [日本語](../ja/dyncode-compiler-mir-pass-design.md) | [한국어](dyncode-compiler-mir-pass-design.md) | [Français](../fr/dyncode-compiler-mir-pass-design.md) | [Deutsch](../de/dyncode-compiler-mir-pass-design.md) | [Español](../es/dyncode-compiler-mir-pass-design.md) | [Italiano](../it/dyncode-compiler-mir-pass-design.md) | [Русский](../ru/dyncode-compiler-mir-pass-design.md) | [العربية](../ar/dyncode-compiler-mir-pass-design.md)

[← DynCode 컴파일러](dyncode-compiler.md)

# MIR Pass 설계 — 원칙과 훅 포인트

> [ir-pass-design.md](dyncode-compiler-ir-pass-design.md)의 동반 문서. IR 계층은 IR 수준에서 명백히 재배치를 생성하는 구조를 제거합니다. MIR 계층은 명령 선택과 레지스터 할당 후의 **캐치올**로, 코드 생성이 도입한 의사/메타데이터 명령을 제거하고 서드파티 난독화 pass가 최종 명령 수준 변환을 수행할 훅 포인트를 노출합니다.
>
...
하지만 LLVM 백엔드는 **IR → MIR 하강** 중 dyncode가 수용할 수 없는 추가 구조를 도입합니다:
...
- **내장 의사 명령 제거 추가**: `isDynCodeStripPseudo` switch에 1 case 추가.
- **내장 MIR 리라이트 추가**: `tryRewriteXxx` 작성 후 `MIRRewritePatterns.def` + `MIRRewriteOpcodes.def`에 추가.
- **서드파티 난독화**: [Plugin API](plugin-api.md) (`NEVERC_INTERPOSE_SC_*` 훅)로 등록.

## 7. DynCodeExtractor와의 관계
