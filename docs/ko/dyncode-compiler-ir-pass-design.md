**언어**: [English](../dyncode-compiler-ir-pass-design.md) | [简体中文](../zh-CN/dyncode-compiler-ir-pass-design.md) | [繁體中文](../zh-TW/dyncode-compiler-ir-pass-design.md) | [日本語](../ja/dyncode-compiler-ir-pass-design.md) | [한국어](dyncode-compiler-ir-pass-design.md) | [Français](../fr/dyncode-compiler-ir-pass-design.md) | [Deutsch](../de/dyncode-compiler-ir-pass-design.md) | [Español](../es/dyncode-compiler-ir-pass-design.md) | [Italiano](../it/dyncode-compiler-ir-pass-design.md) | [Русский](../ru/dyncode-compiler-ir-pass-design.md) | [العربية](../ar/dyncode-compiler-ir-pass-design.md)

[← DynCode 컴파일러](dyncode-compiler.md)

# IR Pass 설계 — 원칙, 파이프라인, 전후 비교

> 이 문서는 dyncode 컴파일 파이프라인의 각 pass의 **설계 근거**를 설명합니다. 구현 세부사항은 `.cpp` 소스의 영문 주석에 있습니다.

---

## 0. 핵심 아이디어

dyncode의 목표 한 문장: **`.o`에서 재배치가 될 모든 것을 제거하고, `mmap(RWX)` + `memcpy` + `blr`할 수 있는 순수한 명령 스트림만 남긴다.**
...
| DynCodeExtractor | `.o` 스캔 최종 감사 + flat `.bin` 출력 | 후처리 |
...
## 9. 난독화 훅
11개 훅 포인트. [mir-pass-design.md §3](dyncode-compiler-mir-pass-design.md#3-사용자-난독화-훅) 참조.

## 10. 2단계 설계 근거
Phase 1은 원본 IR 정리. LLVM 최적화 후 Phase 2는 최적화기가 도입한 새 구조를 정리.

## 11. KernelImportPass (ring-0만)
미해결 extern → 암호화 주소 캐시 포함 리졸버 간접 호출로 자동 리라이트. `(resolver, cookie)` 암시적 매개변수 주입. 각 API 해석 결과는 암호화 후 캐시되며, 이후 호출은 캐시 사용 (lock-free `cmpxchg`). 암호화 메커니즘은 WinPEBImportPass와 공유 ([§4.1](#41-주소-캐시-암호화) 참조). [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md) 참조.
...
셸코드 내 `malloc`/`free`/`calloc`/`realloc` 호출을 하이브리드 할당 전략으로 재작성 (기본 활성화, `-fdyncode-heap-arena` / `-fno-dyncode-heap-arena`로 제어):
...
## 14. 오류 진단 철학
각 하드 오류는 정확히 **1개의 실행 가능한 진단**을 생성. `__neverc_dyncode_hard_error` 메타데이터 센티넬이 캐스케이드를 방지. 사용자에게 1개의 명확한 오류와 수정이 표시되고, 3개의 캐스케이드 메시지는 나오지 않습니다.
