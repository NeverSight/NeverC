**언어**: [English](../vbs-enclave.md) | [简体中文](../zh-CN/vbs-enclave.md) | [繁體中文](../zh-TW/vbs-enclave.md) | [日本語](../ja/vbs-enclave.md) | [한국어](vbs-enclave.md) | [Français](../fr/vbs-enclave.md) | [Deutsch](../de/vbs-enclave.md) | [Español](../es/vbs-enclave.md) | [Italiano](../it/vbs-enclave.md) | [Русский](../ru/vbs-enclave.md) | [العربية](../ar/vbs-enclave.md)
...
크로스 호스트 링크 단계에서는 서명되지 않고 처리되지 않은 엔클레이브 DLL을 생성합니다. VEIID 처리, SignTool 서명 및 `CreateEnclave`/`LoadEnclaveImage`를 통한 실제 로드는 계속 Windows에서만 수행할 수 있으므로 macOS 또는 Linux에서 링크한 DLL을 마지막 세 단계용 Windows 패키징 또는 테스트 머신으로 옮기십시오. 런타임 설치와 검색에 관한 내용은 [타깃 런타임](runtime.md)을 참조하십시오.
...
`/GUARD:MIXED`를 `/GUARD:EHCONT`와 함께 사용하면 EH 연속 대상 테이블도 4바이트 RVA와 값이 0인 메타데이터 1바이트로 구성된 5바이트 항목을 사용합니다.
...
7. `GetProcAddress`로 엔클레이브 내보내기 함수를 찾고, `CallEnclave`를 통해 실행한 뒤 반환값을 검증합니다. CFG로 보호된 경로와 레거시 간접 호출 경로를 포함해 반복 호출합니다.
8. `TerminateEnclave`와 `DeleteEnclave`로 엔클레이브를 종료하고 해제하며, 두 작업의 성공 여부를 확인합니다.
...
x64 런타임 프로브는 Microsoft 이미지를 먼저 실행한 다음 두 NeverC 후보를 실행합니다. `GetProcAddress`로 내보내기 함수를 찾고, `CallEnclave`를 반복 호출하여 CFG로 보호된 경로와 레거시 간접 호출 경로를 실행하고 반환값을 검사합니다. `PASS`에는 종료와 해제의 성공을 포함한 모든 수명 주기 단계의 증거가 올바른 순서로 필요합니다. 로드와 초기화의 성공만으로는 충분하지 않습니다. 이 워크플로에서 ARM64는 정적 검증 및 차등 링크로 다루며, 정규 런타임 프로브는 계속 x64만 대상으로 합니다.

2026-09-11에 네이티브 Windows 11 ARM64에서 별도의 검증이 통과했습니다([실행 34554067852](https://github.com/NeverSight/NeverC/actions/runs/34554067852)). `c46e3a4cc4b3732823a7241a43bca6c44b0f0b74`에서 빌드된 두 NeverC ARM64 DLL을 재사용하여 새로 빌드한 MSVC 참조 이미지와 함께 테스트했습니다. 48개 수명 주기 단계와 결과 검증을 포함한 12회 호출이 모두 통과했으며, 엔클레이브 종료·해제와 서명 인증서 정리도 성공했습니다. 이는 해당 고정 산출물을 검증한 것이며, NeverC 컴파일러를 새로 빌드한 결과나 상시 ARM64 런타임 CI 게이트를 의미하지 않습니다.

선택적 런타임 프로브는 함수 실행 전에 알려진 환경 준비 조건을 충족할 수 없는 경우에만 `SKIP`을 보고할 수 있습니다. 충돌, 시간 초과, 단계 증거 누락이나 순서 오류 및 기능 오류는 Microsoft 참조 이미지에서도 모두 `FAIL`입니다. 구성된 셀프 호스팅 VBS runner에서는 런타임 성공을 필수 게이트로 지정할 수 있습니다.
