...
x64 런타임 프로브는 Microsoft 이미지를 먼저 실행한 다음 두 NeverC 후보를 실행합니다. `GetProcAddress`로 내보내기 함수를 찾고, `CallEnclave`를 반복 호출하여 CFG로 보호된 경로와 레거시 간접 호출 경로를 실행하고 반환값을 검사합니다. `PASS`에는 종료와 해제의 성공을 포함한 모든 수명 주기 단계의 증거가 올바른 순서로 필요합니다. 로드와 초기화의 성공만으로는 충분하지 않습니다. ARM64는 정적 검증 및 차등 링크 대상이며 런타임 실행 대상은 아닙니다.
