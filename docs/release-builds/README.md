...
Use `--strip` when producing an executable or shared library for distribution.
Its short alias is `-s`; both spellings have identical behavior.
...
- `--strip` supports final linked executables and shared libraries.
- NeverC rejects it with `-c`, `-r`, `--emit-static-lib`, or `-fdyncode` instead
  of silently producing an unstripped non-final artifact.
