...
NeverC は LLVM bitcode として埋め込まれた組み込みランタイムで標準 C を拡張します。各 `-fbuiltin-<name>` フラグで制御。
...
| [組み込み mimalloc](builtins/mimalloc/README.ja.md) | `-fbuiltin-mimalloc` | `malloc`/`free`/`calloc`/`realloc` の透過的高性能アロケータオーバーライド |
