**Languages**: [English](dyncode-compiler-mir-pass-design.md) | [简体中文](zh-CN/dyncode-compiler-mir-pass-design.md) | [繁體中文](zh-TW/dyncode-compiler-mir-pass-design.md) | [日本語](ja/dyncode-compiler-mir-pass-design.md) | [한국어](ko/dyncode-compiler-mir-pass-design.md) | [Français](fr/dyncode-compiler-mir-pass-design.md) | [Deutsch](de/dyncode-compiler-mir-pass-design.md) | [Español](es/dyncode-compiler-mir-pass-design.md) | [Italiano](it/dyncode-compiler-mir-pass-design.md) | [Русский](ru/dyncode-compiler-mir-pass-design.md) | [العربية](ar/dyncode-compiler-mir-pass-design.md)

[← DynCode Compiler](dyncode-compiler.md)

# MIR Pass Design — Principles and Interpose Points

> Companion document to [ir-pass-design.md](dyncode-compiler-ir-pass-design.md). The IR layer eliminates constructs that are visibly relocation-producing at the IR level. The MIR layer serves as a **catch-all** after instruction selection and register allocation, stripping codegen-introduced pseudo/metadata instructions and exposing interpose points for third-party obfuscation passes to perform final instruction-level transformations.
>
...
But the LLVM backend introduces additional constructs during **IR → MIR lowering** that dyncode cannot accommodate:
...
Additionally, MIR interposes serve a critical purpose: **enabling third-party instruction-level obfuscation** (instruction substitution, register renaming) that IR cannot express (IR only has virtual registers and abstract instructions).

---

## 1. Integration with LLVM (Native Interposes)
...
The callback does not capture `Opts`. It reads the current `DynCodeOptions` snapshot at runtime, preventing stale configuration when the same process compiles both dyncode and regular C.
...
After pseudo stripping, `MIRPrepPass` runs a rewrite pass to replace codegen-selected but dyncode-unfriendly instruction patterns with equivalent dyncode-friendly forms, without modifying LLVM `.td` files.
...
## 3. User Obfuscation Interposes

`ObfuscationInterposes` exposes **11 interpose points**: 6 IR-level, 3 MIR-level, 2 byte-level:
...
```
[IR PassBuilder]
  ├─ RunBeforePrep       (user interpose)
  ├─ ZeroRelocPass(Prep)
  ├─ RunAfterPrep        (user interpose)
  ├─ IndirectBrPass / MemIntrinPass / CompilerRtPass
  ├─ SyscallStubPass / WinPEBImportPass / KernelImportPass
  ├─ Data2TextPass #1
  ├─ RunBeforeInlining   (user interpose)
  │  (LLVM O-level: AlwaysInliner / SROA / SLP)
  ├─ RunAfterInlining    (user interpose)
  ├─ Data2TextPass #2 / ZeroReloc(Stackify)
  ├─ RunAfterStackify    (user interpose)
  └─ AllBlrPass          (opt)
        │
[Codegen (IR → MIR)]
  ├─ RunBeforePreEmit    (user interpose, CFI present)
  ├─ DynCodeMIRPrepPass  ← this document's focus
  └─ RunAfterPreEmit     (user interpose, CFI stripped)
        │
[AsmPrinter → Object File]
        │
[DynCodeExtractor]  ← byte-level fallback audit
  ├─ RunPostExtract   (user interpose, pure bytes)
  └─ flat .bin
```

The MIR layer handles **catch-all cleanup + obfuscation interpose points**, not business logic. The "write normal C, no dyncode tricks needed" promise is fulfilled by the 5+ IR passes.
...
**Adding a built-in pseudo strip**: add one case to `isDynCodeStripPseudo` switch.
...
**Third-party obfuscation library**: register via the [Plugin API](plugin-api.md) (`NEVERC_INTERPOSE_SC_*` interposes).

## 7. Relationship with DynCodeExtractor
