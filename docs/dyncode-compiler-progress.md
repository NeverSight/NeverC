**Languages**: [English](dyncode-compiler-progress.md) | [简体中文](zh-CN/dyncode-compiler-progress.md) | [繁體中文](zh-TW/dyncode-compiler-progress.md) | [日本語](ja/dyncode-compiler-progress.md) | [한국어](ko/dyncode-compiler-progress.md) | [Français](fr/dyncode-compiler-progress.md) | [Deutsch](de/dyncode-compiler-progress.md) | [Español](es/dyncode-compiler-progress.md) | [Italiano](it/dyncode-compiler-progress.md) | [Русский](ru/dyncode-compiler-progress.md) | [العربية](ar/dyncode-compiler-progress.md)
...
- [x] Directory & CMake skeleton (`nevercDynCode` library)
...
- [x] `DynCodeExtractor`: Mach-O `.o` → flat `.bin` with intra-section relocation patching
- [x] CLI options via generated [`neverc/include/neverc/Invoke/Options.td.h`] (`-fdyncode`, `-fdyncode-all-blr`, `-mdyncode-syscall`, `-fdyncode-keep-obj=`, `-fdyncode-entry=`)
...
- [x] Cross-platform `-mdyncode-syscall` semantics (replaces Darwin-only `-mdyncode-libsystem`)
- [x] Linux / Android syscall number tables (Darwin BSD 100+, Linux arm64 130+, Linux x86_64 150+)
- [x] `DynCodeExtractor` refactored into `MachOExtractor` / `ELFExtractor` / `COFFExtractor`
...
- [x] Multi-DLL Win32 API whitelist (~210 APIs across kernel32/ntdll/user32/ws2_32/advapi32/shell32)
...
- [x] MIR + byte-level obfuscation interposes (11 interposes across IR/MIR/byte-stream layers)
- [x] AArch64 non-Darwin `long double` auto-downgrade to binary64
- [x] DynCode shim headers: `<windows.h>`, `<unistd.h>`, `<fcntl.h>`, `<sys/stat.h>`, `<sys/mman.h>`, `<string.h>`, `<stdlib.h>`
...
- [x] `<neverc/dyncode/kernel.h>` for ring-0 entry conventions
...
- [x] Out-of-tree C Plugin API ([`NevercPluginAPI.h`]): 11 dyncode interpose points (`NEVERC_INTERPOSE_SC_*`)
