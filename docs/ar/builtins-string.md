...
**اللغات**: [English](../builtins-string.md) | [简体中文](../zh-CN/builtins-string.md) | [繁體中文](../zh-TW/builtins-string.md) | [日本語](../ja/builtins-string.md) | [한국어](../ko/builtins-string.md) | [Français](../fr/builtins-string.md) | [Deutsch](../de/builtins-string.md) | [Español](../es/builtins-string.md) | [Italiano](../it/builtins-string.md) | [Русский](../ru/builtins-string.md) | [العربية](builtins-string.md)

[→ نظام وقت التشغيل المدمج في NeverC](builtins.md) · [وثائق NeverC](README.md)
...
- ملف الإدخال بامتداد `.nc` (انظر [توثيق امتداد `.nc`](nc-extension.md))
- وضع `-fdyncode` نشط
...
2. **وقت التشغيل (المسار العام)**: الدالة `always_inline` باسم `__neverc_string_decrypt_literal` تفك التشفير بـ XOR في ذاكرة owned جديدة. النتيجة `string` عادية من نوع owned.
3. **وقت التشغيل (المسار السريع للمقارنة/البحث)**: عندما يظهر النص الحرفي المشفر مباشرة في تعبير مقارنة أو بحث، يعيد Sema الكتابة إلى متغير `__neverc_string_decrypt_*` بدون تخصيص ذاكرة، يفك التشفير ويقارن بايت ببايت (انظر "المقارنة المشفرة بدون تخصيص ذاكرة" أدناه)

### توليد المفتاح

- **افتراضي**: تحصل البذرة `0` داخل المترجم على إنتروبيا جديدة من نظام التشغيل لكل literal مشفر، ويفشل التجميع بصورة آمنة إن تعذر الحصول عليها. تنتج عمليات البناء المستقلة ciphertext مختلفًا.
- **تجاوز CLI**: تختار بذرة غير صفرية مثل `-fstring-encrypt-key=0xDEADBEEF` بذرة حتمية كاملة بعرض 64 بت. ويفصل nonce محلي لوحدة الترجمة بين الـ literals، ما يناسب البناء القابل للتكرار والاختبارات.
...
### المقارنة المشفرة بدون تخصيص ذاكرة

عندما يُستخدم نص حرفي مشفر مباشرة في تعبير مقارنة أو بحث، يتجاوز المترجم تلقائياً تخصيص الذاكرة. بدلاً من فك تشفير السلسلة بالكامل في الذاكرة ثم المقارنة، يتم فك التشفير والمقارنة بايت ببايت — لا يوجد النص الواضح بالكامل في الذاكرة أبداً.

**التعبيرات المُحسّنة** (جميعها بدون تخصيص ذاكرة عندما يكون أحد المعاملات `.encrypt()`):

| الفئة | التعبيرات |
|-------|----------|
| المساواة | `s == "key".encrypt()`، `s != "key".encrypt()` |
| العلاقي | `s < "key".encrypt()`، `s > "key".encrypt()`، `<=`، `>=` |
| البادئة/اللاحقة | `s.starts_with(...)`، `s.ends_with(...)` |
| الاحتواء | `s.contains(...)` |
| دون تمييز حالة | `s.eq_ic(...)`، `s.starts_with_ic(...)`، `s.ends_with_ic(...)`، `s.contains_ic(...)` |
| البحث | `s.find(...)`، `s.rfind(...)`، المتغيرات مع المواضع |
| العد | `s.count(...)` |
...
| العلم | الوصف |
|-------|-------|
| `-fstring-encrypt-key=<hex>` | تحديد بذرة الحماية الكاملة بعرض 64 بت؛ تستخدم `0` إنتروبيا جديدة من نظام التشغيل |
