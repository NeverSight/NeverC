...
2. تحويلات IR — التحضير، وخفض التفرّع غير المباشر، وخفض التعليمات الجوهرية
   للذاكرة (قبل الكومة)، وخفض زمن تشغيل السلاسل، وساحة الكومة، وخفض التعليمات
   الجوهرية للذاكرة (بعد الكومة)، و`compiler_rt` (قبل)، وخفض استيراد النداء
   النظامي وPEB والنواة، و`data_to_text` (قبل)، والتحسين المُضمَّن،
   و`compiler_rt` (بعد)، وإنهاء السلاسل، و`data_to_text` (بعد)، والتكديس،
   وتحويل كل شيء إلى `blr`، و`compiler_rt` (نهائي)، ثم التدقيق النهائي المختوم
   لـIR؛
...
- احفظ الحالة القابلة للتغيير في نطاقات العملية/الجلسة/المهمة التي يوفّرها
  المضيف؛ لا تستعمل أبدًا مفردة «الإضافة الحالية» أو «الخيارات الحالية».
...
- أعِد `NevercStatus` الأصلي؛ فإخفاق `REPLACE` المُعلَن لا يتراجع بصمت إلى
  المزوِّد المدمج.
