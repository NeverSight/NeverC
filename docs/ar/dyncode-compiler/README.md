...
- [دليل تجميع ARM64 (AArch64) — منظور DynCode](arm64-assembly-tutorial.md)
- [نظرة عامة على البنية متعددة المنصات لـ NeverC DynCode](cross-platform-architecture.md)
- [تصميم مرورات IR — المبادئ، المسار، وأمثلة قبل/بعد](ir-pass-design.md)
- [دعم dyncode وضع النواة (Ring-0)](kernel-mode-dyncode.md)
- [تصميم مرورات MIR — المبادئ ونقاط الخطاف](mir-pass-design.md)
- [مسار DynCode وMIR واستراتيجية PIC (ملاحظات التصميم)](pipeline-and-pic.md)
- [دليل توسيع المنصة](platform-extension-guide.md)
- [مُجمِّع dyncode — تتبع التقدم](progress.md)
...
| `-fdyncode-obfuscate=<spec>` | يُمرَّر إلى خطافات التشويش **مستوى IR** عبر [واجهة الإضافات](../plugin-api/README.md). no-op بدون إضافة. راجع [ir-pass-design.md §9](ir-pass-design.md#9-obfuscation-interposes). |
| `-fdyncode-mir-obfuscate=<spec>` | يُمرَّر إلى خطافات **مستوى MIR**. الافتراضي `-fdyncode-obfuscate=` إن لم يُحدَّد. راجع [mir-pass-design.md §3](mir-pass-design.md#3-خطافات-التشويش-من-المستخدم). |
...
مسار dyncode نفسه يضمن فقط «أن الكود يعمل بشكل صحيح». التشويش وتعدّد الأشكال والمُشفّرات المرحلية وميزات طبقة الاستراتيجية المشابهة **غير مضمّنة عمداً** — توفّرها إضافات خارج الشجرة عبر [واجهة الإضافات](../plugin-api/README.md).
...
راجع [وثائق واجهة الإضافات](../plugin-api/README.md) للاطلاع على قائمة نقاط الربط الكاملة وتسجيل المرورات وأمثلة الكود.

- IR-level: [ir-pass-design.md §9 — Obfuscation Interposes](ir-pass-design.md#9-obfuscation-interposes).
- MIR-level: [mir-pass-design.md §3 — User Obfuscation Interposes](mir-pass-design.md#3-خطافات-التشويش-من-المستخدم).
---
