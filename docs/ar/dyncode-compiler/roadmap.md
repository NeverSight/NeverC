...
- قيود الحجم / المحاذاة / الحشو (`-fdyncode-max-length=`، `-fdyncode-align=`، `-fdyncode-pad=`)
...
2. **واجهة الإضافات C خارج الشجرة** — واجهة إضافات C ABI خالصة ([`NevercPluginAPI.h`]) لمرورات IR وMIR وBinary وLinker المخصصة. تُسجَّل الإضافات في 11 نقطة ربط dyncode (`NEVERC_INTERPOSE_SC_*`). SDK بملف رأس واحد، صفر تبعيات LLVM/CRT. راجع [وثائق واجهة الإضافات](../plugin-api/README.md).
...
- `RunBeforePrep` — قبل أي مرور dyncode
...
يستدعي كل مستخرج `finalizeDynCodeBytes` قبل كتابة `.bin`:
...
الاستخدام وأمثلة الكود في [وثائق Plugin API](../plugin-api/README.md).
