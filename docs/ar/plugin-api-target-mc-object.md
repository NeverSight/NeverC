...
[اصطلاحات استدعاء مخصّصة](plugin-api-custom-callconv.md#الخطط-المُجسَّدة) و
...
يسجل محول ELF المدمج الحقائق الأصلية الدقيقة في امتدادات ذات وسوم: يحمل
`NCSE v2` فهرس القسم وعنوانه ونوعه وflags وإزاحة الملف وحجم المدخل؛ ويحمل
`NCSY v2` الحقول `st_info` و`st_other` كاملة و`st_size` وحالة صريحة تدل على
كون الاسم الأصلي فارغاً أو غير فارغ؛ ويحمل `NCRL v1` نوع إعادة التموضع الأصلي
واسمه الرسمي. لذلك يبقى رمز ELF العادي ذو الاسم الفارغ فارغاً، ولا يعاد
تسميته أبداً باسم اصطناعي مثل `$symbol.N`، بينما يظل الرمز المسمى حرفياً
`$symbol.N` اسماً عادياً غير فارغ. يستطيع تمرير الصورة الأصلية من دون تعديل
أن يحفظ الرموز المجهولة بدقة. أما الكتابة المدمجة التي يكون فيها الرسم هو
المصدر المعتمد فترفضها قبل فتح sink الإخراج، لأن صياغة MC المحمولة لا تستطيع
إعادة إنشاء مدخل جدول الرموز المجهول نفسه. تتطلب مراجعات Android canonical
release الحمولة الموسومة الدقيقة للإصدار الحالي، وتعيد تشغيل إسقاط الرسم
المستقر من تلك الحقائق الأصلية.
...
### سياسات Writer في Object Format 1.1

يعلن `NevercObjectFormatDescriptor.Header.Minor` قدرة الـprovider، ولا يبدّل وضع
المضيف بأكمله. ويظل descriptor بالإصدار 1.0 متوافقًا تمامًا مع probe وread
والكتابة الافتراضية العادية؛ إذ يتلقى writer الخاص به
`NevercObjectWriteRequest.Header.Minor == 0` و`Header.Flags == 0`. لا تعلن
minor 1 إلا إذا كان writer يفهم request flags الخاصة بـ1.1؛ فالكتابة العادية
تظل تحمل flags صفرية وتحافظ على سلوك الإخراج السابق لـ1.1.

يعرّف Object Format 1.1 البتات الآتية في
`NevercObjectWriteRequest.Header.Flags`:

- يتطلب `NEVERC_OBJECT_WRITE_CANONICAL_ELF_TABLES` قسمي `.strtab` و`.shstrtab`
  معياريين ومستقلين، مع إعادة تعيين كل الفهارس التابعة. وهذه عملية توحيد لجداول
  ELF وليست ربطًا قابلاً لإعادة التموضع: يبقى ترتيب الأقسام ومجموعات COMDAT
  وبيانات linker الوصفية والرموز المكررة والرموز المستعارة وسجلات relocation
  وكل محتوى (payload) خارج جداول الأسماء دون تغيير. وتُحفظ أقسام `SHT_STRTAB` الإضافية
  الصحيحة الخاصة بالتنسيق؛ ولا يُعاد بناء سوى جدول السلاسل الخاص بـ
  `SHT_SYMTAB` المحدد والجدول الذي يسميه `e_shstrndx`. ومع `DROP_DEBUG_INFO`،
  لا تُرشَّح إلا أقسام debug والبيانات الوصفية التي تشير إلى الفهارس المحذوفة
  لتلك الأقسام.
- يجعل `NEVERC_OBJECT_WRITE_ANDROID_KERNEL_RELEASE` ملف ELF النهائي المتسلسل هو
  المرجع أيضًا: يحذف mapping symbols التي أنشأها writer ويعيد توليد أسماء
  release من إحداثيات الأقسام المتسلسلة الفعلية.
- يطلب `NEVERC_OBJECT_WRITE_DROP_DEBUG_INFO` حذف أقسام debug ضمن إحدى سياستي ELF
  هاتين.

يمثل `NEVERC_OBJECT_WRITE_REQUEST_KNOWN_FLAGS` القناع الكامل للبتات المعروفة.
والتركيبات القانونية الوحيدة هي `0` و`CANONICAL_ELF_TABLES` و
`CANONICAL_ELF_TABLES | DROP_DEBUG_INFO` و
`CANONICAL_ELF_TABLES | ANDROID_KERNEL_RELEASE` والبتات الثلاثة معًا. ولا يصح
استخدام بت release أو debug من دون بت canonical.

يرفض المضيف أي تركيب مجهول أو غير قانوني، وأي طلب خاص إلى provider بالإصدار
minor-0، قبل فتح sink الإخراج. ويجب كذلك على writer بالإصدار 1.1 رفض أي flags
مجهولة أو غير قانونية يتلقاها بدلًا من تجاهلها. وبعد writer وأي معترض
`object.post_write`، تعيد
المصادقة الدلالية للمضيف و`object.final_verify` المختومة تدقيق البايتات
المتسلسلة وتكونان المرجع. وليست هذه flags وعدًا عامًا لكل صيغة خارجية. ويعني
minor 1 أن writer يفهم بروتوكول flags: يمكنه تنفيذ سياسة ELF المناسبة، أو إرجاع
