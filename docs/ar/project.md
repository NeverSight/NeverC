...
[التوثيق](README.md) · [دليل dyncode](dyncode-compiler.md) · [أوقات التشغيل المدمجة](builtins.md) · [واجهة الإضافات API](plugin-api.md) · [خارطة الطريق](roadmap.md)

</div>

---

> **ملاحظة:** يعرض GitHub دائمًا `README.md` (الإنجليزية) كصفحة رئيسية للمستودع (دون كشف تلقائي للغة). استخدم روابط اللغة أعلاه؛ في [التوثيق](README.md) و[دليل dyncode](dyncode-compiler.md) حافظ على نفس اللغة عبر شريط اللغة ومسار التنقل.
...
- **قابل للتوسيع بلا عوائق** — ملف رأس C وحيد و130 مرحلة ترجمة مسمّاة، وتحصل على [إضافة مُجمِّع](plugin-api.md) قادرة على التدخل في أي مرحلة — من تحسين IR إلى الإخراج الثنائي النهائي — دون معرفة LLVM.
...
- **[مُجمِّع dyncode](dyncode-compiler.md)** — مسار IR/MIR متعدد المراحل، استخراج متعدد المنصات، حل الاستيراد/استدعاءات النظام، وضع النواة، تدقيق البايتات المحظورة، بنية إضافات
...
- **[أوقات التشغيل المدمجة](builtins.md)** — أوقات تشغيل LLVM bitcode مدمجة في المترجم: [`string`](builtins-string.md) (سلسلة بدلالة القيمة، إدارة ذاكرة تلقائية) و[`mimalloc`](builtins-mimalloc.md) (تجاوز مخصص ذاكرة عالي الأداء شفاف، مُفعَّل افتراضيًا خارج أهداف النواة وfreestanding) و[`xorstr`](builtins-xorstr.md) (تشفير مستقل لكل مثيل، ختم متأخر إلزامي وتوسيع أصلي لكل موضع استدعاء) و[`strhash`](builtins-strhash.md) (تجزئة السلاسل وقت الترجمة بنفس الخوارزمية وقت التشغيل)
- **[واجهة الإضافات API](plugin-api.md)** — واجهة C ABI خالصة للإضافات خارج الشجرة؛ SDK بملف رأس واحد، صفر تبعيات LLVM/CRT، تغطي مراحل المُشغِّل والمعالج المسبق والشجرة النحوية وIR وMIR وMC والكائنات والربط وLTO وdyncode
