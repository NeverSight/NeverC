...
**تشفير ذاكرة التخزين المؤقت للعناوين** (§4.1، مشترك بين WinPEBImportPass وKernelImportPass): العناوين المحلولة تُشفَّر قبل التخزين عبر تفكيك حسابي بدون XOR `(a + b) - 2*(a & b)` + متغيرات وسيطة `volatile`. ثلاث دوال قابلة للتوصيل (`__sc_derive_key`، `__sc_ptr_encrypt`، `__sc_ptr_decrypt`). فتحات تخزين مؤقت لكل (DLL، API) في قسم `.text`. مسار سريع/بطيء مع `cmpxchg weak` آمن للخيوط. يمكن للمستخدم توفير تنفيذات خاصة (`always_inline`، معكوسات متبادلة، بدون استدعاءات خارجية). راجع [README.md §4.1–4.5](../dyncode-compiler-ir-pass-design.md#41-address-cache-encryption) للتفاصيل الكاملة.

11 خطاف تشويش. فلسفة التشخيص: خطأ واحد = تشخيص واحد قابل للتنفيذ. راجع [mir-pass-design.md §3](dyncode-compiler-mir-pass-design.md#3-خطافات-تشويش-المستخدم) و[kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md).
