...
**Languages**: [English](../builtins.md) | [简体中文](../zh-CN/builtins.md) | [繁體中文](../zh-TW/builtins.md) | [日本語](../ja/builtins.md) | [한국어](../ko/builtins.md) | [Français](../fr/builtins.md) | [Deutsch](../de/builtins.md) | [Español](../es/builtins.md) | [Italiano](../it/builtins.md) | [Русский](../ru/builtins.md) | [العربية](builtins.md)

[→ وثائق NeverC](README.md)
...
|--------|-------|-----------|-------|
| [**`string`**](builtins-string.md) | `-fbuiltin-string` | معطل | نوع سلسلة نصية بدلالة القيمة مع أساليب النقطة، إدارة تلقائية للذاكرة ودعم UTF-8 أصلي |
| [**`mimalloc`**](builtins-mimalloc.md) | `-fbuiltin-mimalloc` | **مفعّل** | مخصص ذاكرة عالي الأداء يستبدل بشفافية `malloc`/`free`/`calloc`/`realloc` |
| [**`xorstr`**](builtins-xorstr.md) | `-fencrypt-call-strings` | معطّل | تشفير مستقل لكل مثيل، ختم متأخر إلزامي، توسيع عند كل موضع استدعاء وتنظيف متطاير للمكدس |
| [**`strhash`**](builtins-strhash.md) | `-fstrhash-algo` / `-fstrhash-fold` | معطّل | تجزئة السلاسل وقت الترجمة بنفس الخوارزمية وقت التشغيل، طي IR اختياري |
...
> **ملاحظة:** لا يستخدم `xorstr` نموذج bitcode المضمّن. يخفض `semaBuiltinNeverCXorstr` في `SemaCheckingBuiltinNeverC.cpp` الماكرو الصريح. يختم `EncryptCallStringsPass` و`XorStrCleanupPass` الـ literals التلقائية ومخازن النص الصريح قبل IPO وبعد كل مرحلة IR متأخرة عادية أو خاصة بـ plugin. لا يعيد `FinalizeXorStrPass` تشفير المفككات الصريحة ويوسعها إلا عند حد حقيقي للشيفرة الآلية الأصلية، ثم يحذف رسم المساعدات المشترك. تصف [وثائق xorstr](builtins-xorstr.md) التصميم وعقد قابلية إعادة البناء.

> **ملاحظة:** `strhash` لا يستخدم أيضاً نموذج bitcode المضمّن. [`NC_STRHASH(s)`](builtins-strhash.md) يُطوى إلى ثابت في Sema؛ `-fstrhash-fold` يفعّل `StrHashFoldPass`. انظر [وثائق strhash](builtins-strhash.md).
