...
- [ARM64 (AArch64) Assembly Tutorial — DynCode Perspective](dyncode-compiler-arm64-assembly-tutorial.md)
- [NeverC DynCode Cross-Platform Architecture Overview](dyncode-compiler-cross-platform-architecture.md)
- [IR Pass Design — Principles, Pipeline, and Before/After Examples](dyncode-compiler-ir-pass-design.md)
- [Kernel-Mode (Ring-0) DynCode Support](dyncode-compiler-kernel-mode-dyncode.md)
- [MIR Pass Design — Principles and Interpose Points](dyncode-compiler-mir-pass-design.md)
- [DynCode Pipeline, MIR & PIC Strategy (Design Notes)](dyncode-compiler-pipeline-and-pic.md)
- [Platform Extension Guide](dyncode-compiler-platform-extension-guide.md)
- [DynCode Compiler — Progress Tracker](dyncode-compiler-progress.md)
...
All eight (OS, arch) triples are driven by **the same set of passes**. Differences are isolated in `TargetDesc.cpp` table entries + three extractor arch switches. Adding a new platform = fill one more table row + add one case in each extractor. The `ExecutionLevel` dimension is orthogonal: `User` uses the user-mode syscall / PEB pipeline; `Kernel` disables both and injects `KernelImportPass` to rewrite extern calls through resolver shims. See [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md).
...
| Option | Description |
|--------|-------------|
| `-fdyncode` | Enable dyncode compilation mode. |
| `-fno-dyncode` | Cancel a preceding `-fdyncode`. |
| `-fdyncode-all-blr` | Aggressive mode: indirect-ize intra-module direct calls to `blr xN` / `call *rax`, eliminating all relative branch relocations. Not needed for normal use. |
| `-mdyncode-syscall` | Explicitly enable syscall stubs (already default under `-fdyncode` for Darwin/Linux/Android; mainly for expressing intent or script compat). |
| `-mdyncode-libsystem` | Darwin legacy alias for `-mdyncode-syscall`. |
| `-mdyncode-win-peb-import` | Explicitly enable Windows PEB import (already default under `-fdyncode` + Windows triple). |
| `-fdyncode-keep-obj=<path>` | Copy intermediate object file to `<path>` for native disassembler audit. |
...
| `-fdyncode-obfuscate=<spec>` | Passed through to registered **IR-level** plugin interposes via the [Plugin API](plugin-api.md). No-op when no plugin is loaded. See [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes). |
| `-fdyncode-mir-obfuscate=<spec>` | Passed through to **MIR-level** obfuscation interposes (`RunBeforePreEmit` / `RunAfterPreEmit`). Falls back to `-fdyncode-obfuscate=` value if unset. See [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-user-obfuscation-interposes). |
...
1. DynCode load address must be 4 KB aligned — the natural behavior of `mmap` / `VirtualAlloc`; all loader code already complies.
...
The dyncode pipeline itself only ensures "the code runs correctly". Obfuscation, polymorphism, staged encoders, and similar strategy-layer features are **intentionally not built-in** — they are provided by out-of-tree plugins through the [Plugin API](plugin-api.md).

The pipeline exposes **11 interpose points** across three layers, all accessible via the C Plugin API (`NEVERC_INTERPOSE_SC_*`):
...
See the [Plugin API documentation](plugin-api.md) for the full interpose list, pass registration, and code examples.

- IR-level design: [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes).
- MIR-level design: [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-user-obfuscation-interposes)
---
