...
2. преобразования IR — prepare, понижение косвенных переходов, понижение
   интринсик памяти (до кучи), понижение строкового рантайма, арена кучи,
   понижение интринсик памяти (после кучи), `compiler_rt` (pre), понижение
   импортов syscall/PEB/ядра, `data_to_text` (pre), встраивающая оптимизация,
   `compiler_rt` (post), финализация строк, `data_to_text` (post), stackify,
   all-`blr`, `compiler_rt` (final) и запечатанная финальная проверка IR;
