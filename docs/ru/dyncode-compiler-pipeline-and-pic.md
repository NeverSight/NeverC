**Языки**: [English](../dyncode-compiler-pipeline-and-pic.md) | [简体中文](../zh-CN/dyncode-compiler-pipeline-and-pic.md) | [繁體中文](../zh-TW/dyncode-compiler-pipeline-and-pic.md) | [日本語](../ja/dyncode-compiler-pipeline-and-pic.md) | [한국어](../ko/dyncode-compiler-pipeline-and-pic.md) | [Français](../fr/dyncode-compiler-pipeline-and-pic.md) | [Deutsch](../de/dyncode-compiler-pipeline-and-pic.md) | [Español](../es/dyncode-compiler-pipeline-and-pic.md) | [Italiano](../it/dyncode-compiler-pipeline-and-pic.md) | [Русский](dyncode-compiler-pipeline-and-pic.md) | [العربية](../ar/dyncode-compiler-pipeline-and-pic.md)

[← Компилятор dyncode](dyncode-compiler.md)

# Конвейер dyncode, MIR и стратегия PIC (заметки по проектированию)

Этот документ описывает проектные компромиссы в режиме dyncode NeverC по цепочке **IR → оптимизация LLVM → бэкенд MIR → объектный файл → извлечение/патчинг** и связь с политикой **PIC по умолчанию на уровне компилятора**. Детали реализации авторитетны в исходном коде и английских комментариях.

## 1. Почему PIC по умолчанию принудителен (включая не-dyncode)

Экстрактор dyncode предполагает, что ссылки на внешние символы попадают на **PC-относительные** или внутри-`.text` разрешимые перемещения, а не на жёстко закодированные абсолютные адреса или пулы констант, требующие загрузчика для заполнения `.data`.
...
- Обычная компиляция C и компиляция `-fdyncode` разделяют одни привычки перемещений, снижая когнитивную нагрузку «работает нормально, ломается под dyncode».
...
Эта политика не различает, включён ли `-fdyncode` или контекст user/kernel. Даже при передаче `-fno-pic` / `-static` / `-mkernel` / `-mdynamic-no-pic`, `ParsePICArgs()` сохраняет `Reloc::PIC_`.

## 2. Двухфазное разделение труда IR и MIR

### 2.1 Слой IR (`registerDynCodePasses`)
...
**Принцип**: Задачи, решаемые в IR структурно, исправляются сначала в IR (пулы констант, BlockAddress, проваливание `memcpy` в libc, проваливание `__int128 /` в `__udivti3` и т.д.), упрощая байтовый поток для бэкенда и экстрактора. Для сценариев с высокой когнитивной нагрузкой, но безопасно интернализуемых, драйвер проактивно вводит правила (например, `long double` AArch64 Linux / Android / Windows понижается до binary64 в режиме dyncode). Только конструкции, не поддерживаемые без runtime, активируют диагностику MIR/экстрактора.

### 2.2 Слой MIR (`registerDynCodeMachinePasses`)
...
2. **`DynCodeMIRPrepPass`**: Удаляет псевдоинструкции, которые порождали бы `.eh_frame` / `.pdata` / `.xray_*`.
...
Имена опкодов MIR не разбросаны в потоке управления прохода; `DynCodeMIRPrepPass` использует таблицу `Tables/MIRRewriteOpcodes.def`. При добавлении dyncode-дружественных подстановок предпочтительны записи в таблице; изменения выбора инструкций `.td` — крайний случай.

> Примечание: `DynCodeMIRPrepPass` регистрируется только при `-fdyncode`. Обычные программы не должны глобально удалять CFI/EH.

Глобальные коллбэки IR и MIR используют паттерн **регистрация один раз, чтение текущего снапшота `DynCodeOptions` во время выполнения**.
...
Оба работают без реестра или переменных среды VS, обеспечивая кросс-компиляцию dyncode Windows из macOS / Linux.

## 5. Точки обфускации и расширения

- **IR-хуки**: 6 точек подключения IR (`NEVERC_INTERPOSE_SC_BEFORE_PREP` — `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) через [API плагинов](plugin-api.md). 11 хуков всего (6 IR + 3 MIR + 2 байтовый поток).
- **MIR обфускация**: `RunBeforePreEmit` / `RunAfterPreEmit` / `RunAfterFinalMIR`. `-fdyncode-mir-obfuscate=` для отдельного MIR spec.
- **Хуки байтового потока**: `RunPostExtract` (пре-финализация) и `RunPostFinalize` (пост-финализация).
- **Размер / выравнивание / заполнение**: `-fdyncode-max-length=`, `-fdyncode-align=`, `-fdyncode-pad=`.
...
`-mdyncode-context=user|kernel` как второе измерение конвейера:

- **Режим пользователя**: Конвейер PEB walk / syscall stub.
- **Режим ядра**: `SyscallStubPass` / `WinPEBImportPass` досрочно возвращают; `KernelImportPass` переписывает неразрешённые extern вызовы; `<neverc/dyncode/kernel.h>` предоставляет типы ядра.

См. [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md).

## 7. Слой совместимости Windows POSIX

### 7.1 Проблема

Кроссплатформенный код на C обычно использует `write(fd, buf, n)`, `read(fd, buf, n)`, `exit(code)` и т.д. На платформах Unix `SyscallStubPass` заменяет их встроенными syscall. На Windows у этих имён POSIX нет соответствующего Win32 API, что вызывает ошибки «неразрешённая релокация».

### 7.2 Цель проектирования

**Ноль осведомлённости пользователя**: тот же исходник C компилируется для всех 8 целевых тройок без `#ifdef _WIN32` или ручных вызовов Win32 API.

### 7.3 Реализация

`WinPEBImportPass` реализует трёхфазную обработку:

1. **Фаза 1 — сканирование POSIX**: сканирует несопоставленные extern-объявления по таблице совместимости POSIX.
2. **Фаза 2 — генерация мостовых обёрток**: `Win32PosixCompat.def` направляет имена POSIX к построителям обёрток, генерирующим обёртки `always_inline` (напр. `write` → `GetStdHandle` + `WriteFile`, `mmap` → `VirtualAlloc` с отображением защиты, `exit` → `ExitProcess` и т.д.). Покрыто 13 групп функций POSIX.
3. **Фаза 3 — разрешение PEB**: Win32 API, на которые ссылаются обёртки, разрешаются через обычный резолвер PEB walk.

### 7.4 Расширяемость

Добавление новых функций совместимости POSIX: добавления только псевдонимов меняют лишь `Win32PosixCompat.def`; новая семантика требует небольшого IR-построителя + одной записи в таблице. Операции с состоянием, такие как `open→CreateFileA`, которым нужны таблицы времени жизни fd/handle, намеренно не эмулируются.
...
| Страховочная сеть MIR | `DynCodeMIRPrepPass` + хуки до/после |
| Минимизация хардкода | `TargetDesc` + `Options.td.h` табличные |
| Два измерения user/kernel | `-fdyncode` × `-mdyncode-context={user,kernel}` |
