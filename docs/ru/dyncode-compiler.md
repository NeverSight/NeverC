**Языки**: [English](../dyncode-compiler.md) | [简体中文](../zh-CN/dyncode-compiler.md) | [繁體中文](../zh-TW/dyncode-compiler.md) | [日本語](../ja/dyncode-compiler.md) | [한국어](../ko/dyncode-compiler.md) | [Français](../fr/dyncode-compiler.md) | [Deutsch](../de/dyncode-compiler.md) | [Español](../es/dyncode-compiler.md) | [Italiano](../it/dyncode-compiler.md) | [Русский](dyncode-compiler.md) | [العربية](../ar/dyncode-compiler.md)

[← Индекс документации](README.md) · [← Проект NeverC](project.md)

# NeverC — компилятор dyncode

Компилирует исходный код C напрямую в плоский бинарный dyncode: **позиционно-независимый, без релокаций, без секции данных**.

## Руководства

- [Учебник по ассемблеру ARM64 (AArch64) — Перспектива DynCode](dyncode-compiler-arm64-assembly-tutorial.md)
- [Кроссплатформенная архитектура NeverC DynCode — Обзор](dyncode-compiler-cross-platform-architecture.md)
- [Проектирование проходов IR — Принципы, конвейер и примеры до/после](dyncode-compiler-ir-pass-design.md)
- [Поддержка dyncode режима ядра (Ring-0)](dyncode-compiler-kernel-mode-dyncode.md)
- [Проектирование проходов MIR — Принципы и точки хуков](dyncode-compiler-mir-pass-design.md)
- [Конвейер dyncode, MIR и стратегия PIC (заметки по проектированию)](dyncode-compiler-pipeline-and-pic.md)
- [Руководство по расширению платформ](dyncode-compiler-platform-extension-guide.md)
- [Компилятор dyncode — отслеживание прогресса](dyncode-compiler-progress.md)
- [Дорожная карта](dyncode-compiler-roadmap.md)
...
| Опция | Описание |
|--------|-------------|
| `-fdyncode` | Включить режим компиляции dyncode. |
| `-fno-dyncode` | Отменить предыдущий `-fdyncode`. |
| `-fdyncode-all-blr` | Агрессивный режим: косвенные вызовы `blr xN` / `call *rax`, убирает относительные branch reloc. Обычно не нужен. |
| `-mdyncode-syscall` | Явно включить syscall stubs (по умолчанию с `-fdyncode` на Darwin/Linux/Android). |
| `-mdyncode-libsystem` | Устаревший алиас Darwin для `-mdyncode-syscall`. |
| `-mdyncode-win-peb-import` | Явно включить импорт PEB Windows (по умолчанию с `-fdyncode` + Windows triple). |
| `-fdyncode-keep-obj=<path>` | Копировать объект в `<path>` для нативного дизассемблера. |
| `-fdyncode-entry=<name>` | Переопределить вход (`main`, `_main`, `dyncode_entry`, `_dyncode_entry`). |
...
| `-fdyncode-obfuscate=<spec>` | В **IR-level** хуки плагинов через [API плагинов](plugin-api.md). No-op без загруженного плагина. См. [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes). |
| `-fdyncode-mir-obfuscate=<spec>` | В **MIR-level** interposes. Fallback `-fdyncode-obfuscate=`. См. [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-пользовательские-хуки-обфускации). |
...
| libc write/exit | `write(1, s, 3);` | Да (с `-mdyncode-syscall`) | Обёртка syscall |
| POSIX includes | `#include <unistd.h>` | Да (режим dyncode переключается на shim) | Драйвер внедряет `__NEVERC_DYNCODE__` |
| Win32 API | `WriteFile(h, buf, n, &w, 0);` | Да (с `-mdyncode-win-peb-import`) | PEB-walk thunk |
| Windows SDK includes | `#include <windows.h>` | Да (режим dyncode переключается на shim) | Лёгкие shim-заголовки |
| Custom entry name | `int dyncode_main(...)` | Да (с `-fdyncode-entry=...`) | Проброс драйвером |
...
│   │   ├── MmapABI.h                          # Общие константы mmap (prot/flags)
│   │   ├── DynCodeIRHelpers.h               # Общие IR-утилиты (getSizeType и т.д.)
...
1. Адрес загрузки dyncode должен быть выровнен на 4 КБ — естественное поведение `mmap` / `VirtualAlloc`; тестовые загрузчики уже соблюдают это.
...
Конвейер dyncode сам по себе лишь гарантирует, что «код выполняется корректно». Обфускация, полиморфизм, поэтапные кодировщики и подобные возможности уровня стратегии **намеренно не встроены** — они предоставляются внедеревными плагинами через [API плагинов](plugin-api.md).
...
Полный список точек подключения, регистрацию проходов и примеры кода см. в [документации API плагинов](plugin-api.md).

- IR-level: [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes).
- MIR-level: [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-пользовательские-хуки-обфускации)
---
