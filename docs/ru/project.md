...
[Документация](README.md) · [Руководство dyncode](dyncode-compiler/README.md) · [Встроенные среды выполнения](builtins/README.md) · [API плагинов](plugin-api/README.md) · [Дорожная карта](roadmap.md)

</div>

---

> **Примечание:** GitHub всегда показывает на главной репозитория `README.md` (английский), без автоопределения языка браузера. Используйте ссылки языков выше; в [документации](README.md) и [руководстве dyncode](dyncode-compiler/README.md) сохраняйте ту же локаль через языковую панель и хлебные крошки.
...
- **Расширяемый без порога вхождения** — Единственный C-заголовок, 130 именованных фаз компиляции — и у вас [плагин компилятора](plugin-api/README.md), способный вмешаться на любом этапе от оптимизации IR до финального бинарного вывода — без знания LLVM.
...
- **[Компилятор dyncode](dyncode-compiler/README.md)** — многоступенчатый конвейер IR/MIR, кроссплатформенное извлечение, разрешение импортов/syscall, режим ядра, аудит запрещённых байт, архитектура плагинов
- **Интегрированный линкер** — COFF, ELF и Mach-O в одном бинарнике; внешние `ld` и `link.exe` не нужны
- **[Удаление релизных символов](release-builds.md)** — встроенные `--strip` / `-s` удаляют ненужные во время выполнения символы и исходную отладку из итоговых ELF, Mach-O и PE/COFF и структурно переименовывают символы `.ko` с учётом ядра (это не hash и не encryption)
- **Кросс-компиляция** — Windows PE, Linux ELF, macOS Mach-O и Android ELF с любого хоста со встроенными SDK платформ
- **[Встроенные среды выполнения](builtins/README.md)** — встроенные в компилятор LLVM bitcode среды: [`string`](builtins/string.md) (строковый тип с семантикой значения, автоуправление памятью), [`mimalloc`](builtins/mimalloc.md) (прозрачная замена аллокатора высокой производительности, включена по умолчанию вне ядерных и freestanding-целей), [`xorstr`](builtins/xorstr.md) (шифрование каждого экземпляра, обязательное позднее запечатывание и нативное разворачивание в каждой точке вызова) и [`strhash`](builtins/strhash.md) (хеширование строк на этапе компиляции с тем же алгоритмом во время выполнения)
- **[API плагинов](plugin-api/README.md)** — чистый C ABI для внедеревных плагинов; SDK с одним заголовком, ноль зависимостей LLVM/CRT, охватывающий фазы драйвера, препроцессора, AST, IR, MIR, MC, объектного файла, компоновки, LTO и dyncode
- **[Расширение `.nc`](nc-extension.md)** — используйте `.nc` для автоматического включения всех возможностей NeverC (`string`, целочисленные типы в стиле Rust) без дополнительных флагов
...
> **Примечание:** Встроенный тип **`string`** требует **`-fbuiltin-string`** для файлов `.c`. Он автоматически включается для [**файлов `.nc`**](nc-extension.md) и в режиме **`-fdyncode`**.
...
Подробности: **[индекс документации](README.md)** — дизайн, матрица платформ, справочник CLI, примеры. Полные собираемые примеры: **[examples](examples.md)**.
...
Компилирует во временный исполняемый файл, запускает его на **локальном хосте** и удаляет — по аналогии с `go run`. Чтобы сохранить бинарник, используйте `neverc ... -o out`.
...
| Тема | Поведение |
|------|-----------|
| Разделение по умолчанию | Флаги компилятора до первого `.c`/`.nc`; подряд идущие исходники компилируются вместе; остальное передаётся в `main` |
| Явный `--` | До `--` — компилятору, после — программе (когда после исходников идут флаги линковки) |
| Рабочий каталог | Запуск в текущем каталоге — относительные пути как у обычного бинарника |
| Окружение и I/O | Наследует окружение; stdin/stdout/stderr подключены к временному процессу |
| Код выхода | Возвращает код программы; при ошибке компиляции — код компилятора, программа не запускается |
| Артефакт | Хранится в `neverc-run-*` и удаляется после выполнения |
...
Полный справочник: [`neverc runtime` →](runtime.md) · [`neverc update` →](update.md) · [`neverc build` / `make` →](build.md).

## Сборка из исходников

Требования, команды сборки, кросс-компиляция под Windows, настройка PATH и переключение между release и in-tree сборкой — см. **[Локальная разработка](local-dev.md)**.
...
При копировании или адаптации кода, в том числе с помощью ИИ/LLM или для повторного использования, связанного с LLVM, соблюдайте применимую лицензию и сохраняйте обязательные уведомления об авторских правах, лицензии и авторстве. Отдельно мы просим всех, кто использует NeverC как источник для справки, указывать проект и первоисточник; эта просьба проекта не является дополнительным условием лицензии.

См. [уведомления](../../NOTICE), [руководство по указанию источников](attribution.md) и [машиночитаемые данные для цитирования](../../CITATION.cff).
