...
[Документация](README.md) · [Руководство dyncode](dyncode-compiler.md) · [Встроенные среды выполнения](builtins.md) · [API плагинов](plugin-api.md) · [Дорожная карта](roadmap.md)

</div>

---

> **Примечание:** GitHub всегда показывает на главной репозитория `README.md` (английский), без автоопределения языка браузера. Используйте ссылки языков выше; в [документации](README.md) и [руководстве dyncode](dyncode-compiler.md) сохраняйте ту же локаль через языковую панель и хлебные крошки.
...
- **Расширяемый без порога вхождения** — Единственный C-заголовок, 130 именованных фаз компиляции — и у вас [плагин компилятора](plugin-api.md), способный вмешаться на любом этапе от оптимизации IR до финального бинарного вывода — без знания LLVM.
...
- **[Компилятор dyncode](dyncode-compiler.md)** — многоступенчатый конвейер IR/MIR, кроссплатформенное извлечение, разрешение импортов/syscall, режим ядра, аудит запрещённых байт, архитектура плагинов
