**Languages**: [English](../builtins.md) | [简体中文](../zh-CN/builtins.md) | [繁體中文](../zh-TW/builtins.md) | [日本語](../ja/builtins.md) | [한국어](../ko/builtins.md) | [Français](../fr/builtins.md) | [Deutsch](../de/builtins.md) | [Español](../es/builtins.md) | [Italiano](../it/builtins.md) | [Русский](builtins.md) | [العربية](../ar/builtins.md)

[← Документация NeverC](README.md)
...
|-----------|------|-------------|----------|
| [**`string`**](builtins-string.md) | `-fbuiltin-string` | Выкл. | Строковый тип с семантикой значения, методы через точку, автоматическое управление памятью и нативная поддержка UTF-8 |
| [**`mimalloc`**](builtins-mimalloc.md) | `-fbuiltin-mimalloc` | **Вкл.** | Высокопроизводительный аллокатор памяти, прозрачно заменяющий `malloc`/`free`/`calloc`/`realloc` |
| [**`xorstr`**](builtins-xorstr.md) | `-fencrypt-call-strings` | Выкл. | Шифрование для каждого экземпляра, обязательное позднее запечатывание, разворачивание в точке вызова и volatile-очистка стека |
| [**`strhash`**](builtins-strhash.md) | `-fstrhash-algo` / `-fstrhash-fold` | Выкл. | Хеширование строк на этапе компиляции, тот же алгоритм во время выполнения, опциональный IR-fold |
...
> **Примечание:** `xorstr` не использует модель встроенного bitcode. `semaBuiltinNeverCXorstr` в `SemaCheckingBuiltinNeverC.cpp` понижает явный макрос. `EncryptCallStringsPass` и `XorStrCleanupPass` запечатывают автоматические литералы и открытое хранилище до IPO и после каждой поздней обычной или plugin-IR-фазы. `FinalizeXorStrPass` перешифровывает и разворачивает явные декодеры только на реальной границе нативного машинного кода, после чего удаляет общий граф помощников. Дизайн и контракт воспроизводимости описаны в [документации xorstr](builtins-xorstr.md).

> **Примечание:** `strhash` также не использует модель встроенного bitcode. [`NC_STRHASH(s)`](builtins-strhash.md) сворачивается в константу в Sema; `-fstrhash-fold` включает `StrHashFoldPass`. См. [документацию strhash](builtins-strhash.md).
...
├── lib/Headers/neverc/
│   ├── xorstr/xorstr.h / xorstr/xorstr_impl.inc # макросы NC_XORSTR / NEVERC_XORSTR
