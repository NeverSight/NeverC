**Языки**: [English](../../builtins/string.md) | [简体中文](../../zh-CN/builtins/string.md) | [繁體中文](../../zh-TW/builtins/string.md) | [日本語](../../ja/builtins/string.md) | [한국어](../../ko/builtins/string.md) | [Français](../../fr/builtins/string.md) | [Deutsch](../../de/builtins/string.md) | [Español](../../es/builtins/string.md) | [Italiano](../../it/builtins/string.md) | [Русский](string.md) | [العربية](../../ar/builtins/string.md)

[← Встроенная система времени выполнения NeverC](README.md) · [Документация NeverC](../README.md)
...
**Включение:** Передайте `-fbuiltin-string` при компиляции (по умолчанию отключено). Автоматическое включение в следующих случаях:
- Входной файл имеет расширение `.nc` (см. [документацию расширения `.nc`](../nc-extension.md))
- Активен режим `-fdyncode`

```bash
neverc hello.nc -o hello                # автоматически — .nc включает
neverc -fbuiltin-string main.c -o main  # явный флаг для .c файлов
```
...
## Шифрование строк во время компиляции

`.encrypt()` обеспечивает XOR-шифрование строковых литералов во время компиляции. Открытый текст никогда не хранится в бинарном файле — он шифруется при компиляции и расшифровывается во время выполнения через функцию `always_inline`, что делает невозможным извлечение исходного содержимого инструментами статического анализа (например, `strings`).

### Использование

```c
string secret = "API_KEY_12345".encrypt();
printf("%s\n", secret.c_str());  // выводит "API_KEY_12345" во время выполнения

// но `strings ./binary` НЕ найдёт "API_KEY_12345"
```

### Принцип работы

1. **Время компиляции**: Sema перехватывает `.encrypt()` на литерале, XOR-шифрует каждый байт и сохраняет шифротекст в `.rodata`
2. **Время выполнения (общий путь)**: `always_inline` функция `__neverc_string_decrypt_literal` расшифровывает XOR в свежевыделенный owned буфер. Результат — обычная owned строка `string`.
3. **Время выполнения (быстрый путь сравнения/поиска)**: когда зашифрованный литерал появляется непосредственно в выражении сравнения или поиска, Sema переписывает его в вариант `__neverc_string_decrypt_*` с нулевым выделением памяти, который расшифровывает и сравнивает побайтово (см. «Сравнение с расшифровкой без выделения памяти» ниже)

### Генерация ключа

- **По умолчанию**: seed `0` получает в компиляторе свежую энтропию ОС для каждого шифруемого литерала и безопасно завершает компиляцию ошибкой, если энтропия недоступна. Независимые сборки дают разный ciphertext.
- **Переопределение через CLI**: ненулевой seed, например `-fstring-encrypt-key=0xDEADBEEF`, задаёт детерминированный полный 64-битный seed. Локальный для единицы трансляции nonce разделяет литералы, что подходит для воспроизводимых сборок и тестов.

### Поддержка всех типов литералов

`.encrypt()` автоматически поддерживает все типы строковых литералов — компилятор преобразует wide/UTF-16/UTF-32 литералы в UTF-8 перед шифрованием:
...
### Сравнение с расшифровкой без выделения памяти

Когда зашифрованный литерал используется непосредственно в выражении сравнения или поиска, компилятор автоматически обходит выделение памяти в куче. Вместо расшифровки всей строки в память и последующего сравнения, расшифровка и сравнение выполняются побайтово — открытый текст никогда полностью не присутствует в памяти.

**Оптимизированные выражения** (все с нулевым выделением, когда один операнд — `.encrypt()`):

| Категория | Выражения |
|-----------|----------|
| Равенство | `s == "key".encrypt()`, `s != "key".encrypt()` |
| Реляционные | `s < "key".encrypt()`, `s > "key".encrypt()`, `<=`, `>=` |
| Префикс/Суффикс | `s.starts_with(...)`, `s.ends_with(...)` |
| Вхождение | `s.contains(...)` |
| Без учёта регистра | `s.eq_ic(...)`, `s.starts_with_ic(...)`, `s.ends_with_ic(...)`, `s.contains_ic(...)` |
| Поиск | `s.find(...)`, `s.rfind(...)`, варианты с позицией |
| Подсчёт | `s.count(...)` |
...
```c
string s = "hello";
string e = s.encrypt();  // ОШИБКА: .encrypt() can only be applied to string literals
```

- `.encrypt()` **не принимает аргументов**:

```c
string e = "hello".encrypt(42);  // ОШИБКА: .encrypt() takes no arguments
```

- Двойное шифрование отклоняется:

```c
string e = "hello".encrypt().encrypt();  // ОШИБКА: .encrypt() can only be applied to string literals
```
...
| Флаг | Описание |
|------|----------|
| `-fstring-encrypt-key=<hex>` | Задать полный 64-битный seed защиты; `0` использует свежую энтропию ОС |
