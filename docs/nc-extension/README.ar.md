...
```bash
# تلقائي — لا حاجة لـ -fbuiltin-string أو -fneverc-types
...
بالنسبة لملفات `.c`، تظل هذه الأعلام اختيارية: يجب على المستخدم تمرير `-fneverc-types` و/أو `-fbuiltin-string` صراحةً.
