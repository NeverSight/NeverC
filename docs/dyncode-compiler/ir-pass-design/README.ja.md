...
未解決 extern → 暗号化アドレスキャッシュ付きリゾルバ間接呼出に自動リライト。`(resolver, cookie)` 暗黙パラメータ注入。各 API の解決結果は暗号化後キャッシュされ、後続呼出はキャッシュを使用（lock-free `cmpxchg`）。暗号化メカニズムは WinPEBImportPass と共有（[§4.1](#41-アドレスキャッシュ暗号化) 参照）。[kernel-mode-dyncode.md](../kernel-mode-dyncode/README.ja.md) 参照。
