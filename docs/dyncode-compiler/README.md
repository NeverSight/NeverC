...
The dyncode pipeline itself only ensures "the code runs correctly". Obfuscation, polymorphism, staged encoders, and similar strategy-layer features are **intentionally not built-in** — they are provided by out-of-tree plugins through the [Plugin API](../plugin-api/README.md).
