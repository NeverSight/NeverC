...
- **IR フック**：[Plugin API](../../plugin-api/README.ja.md) 経由で 6 つの IR フックポイント（`NEVERC_INTERPOSE_SC_BEFORE_PREP` 〜 `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`）を提供。`NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` は真の最終 IR 注入可能ポイント——ここに登録した pass の後に出力を変更する pass はない。計 11 フック（6 IR + 3 MIR + 2 バイトストリーム）。
...
詳細は [kernel-mode-dyncode.md](../kernel-mode-dyncode/README.ja.md) を参照。
