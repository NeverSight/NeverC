...
- **Interposes IR**: 6 puntos de enganche IR (`NEVERC_INTERPOSE_SC_BEFORE_PREP` a `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) vía la [API de Plugins](../../plugin-api/README.es.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` es el verdadero último punto de inyección IR — los passes registrados aquí no tienen passes subsecuentes que modifiquen su salida. 11 puntos de interpose en total (6 IR + 3 MIR + 2 flujo de bytes).
...
Ver [kernel-mode-dyncode.md](../kernel-mode-dyncode/README.es.md) para detalles.
