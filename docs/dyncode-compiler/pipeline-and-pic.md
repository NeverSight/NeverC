...
- **IR interposes**: 6 interpose points (`NEVERC_INTERPOSE_SC_BEFORE_PREP` through `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) accessible via the [Plugin API](../plugin-api/README.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` is the true last IR-dimension injectable point — passes registered here have no subsequent passes that can modify their output. 11 interpose points total (6 IR + 3 MIR + 2 byte-stream).
...
See [kernel-mode-dyncode.md](kernel-mode-dyncode.md) for details.
