...
> Documento compagno di [ir-pass-design.md](../ir-pass-design/README.it.md). Il livello IR elimina costrutti che producono visibilmente rilocazioni. Il livello MIR serve come **rete di sicurezza** dopo selezione istruzioni e allocazione registri: rimuove pseudo-istruzioni/metadati introdotti dal codegen ed espone punti interpose per pass di offuscamento terzi.

---

## 0. Perché serve un livello MIR

Il backend LLVM introduce costrutti aggiuntivi durante **IR → MIR lowering**: pseudo CFI/EH_LABEL, stub XRay/patchable, metadati sanitizer, fixup MC. I interpose MIR abilitano l'**offuscamento a livello istruzione** (sostituzione, rinomina registri).
...
11 punti interpose: 6 IR + 3 MIR + 2 livello byte.
...
- **Aggiunta rimozione pseudo**: Un case in `isDynCodeStripPseudo`.
- **Aggiunta riscrittura MIR**: Scrivere `tryRewriteXxx` + file `.def`.
- **Terze parti**: [API Plugin](../../plugin-api/README.it.md) (interpose `NEVERC_INTERPOSE_SC_*`).
