**Languages**: [English](dyncode-compiler-kernel-mode-dyncode.md) | [简体中文](zh-CN/dyncode-compiler-kernel-mode-dyncode.md) | [繁體中文](zh-TW/dyncode-compiler-kernel-mode-dyncode.md) | [日本語](ja/dyncode-compiler-kernel-mode-dyncode.md) | [한국어](ko/dyncode-compiler-kernel-mode-dyncode.md) | [Français](fr/dyncode-compiler-kernel-mode-dyncode.md) | [Deutsch](de/dyncode-compiler-kernel-mode-dyncode.md) | [Español](es/dyncode-compiler-kernel-mode-dyncode.md) | [Italiano](it/dyncode-compiler-kernel-mode-dyncode.md) | [Русский](ru/dyncode-compiler-kernel-mode-dyncode.md) | [العربية](ar/dyncode-compiler-kernel-mode-dyncode.md)

[← DynCode Compiler](dyncode-compiler.md)

# Kernel-Mode (Ring-0) DynCode Support

`-fdyncode` originally covered only ring-3 payloads (PEB walk, `svc`/`syscall` stubs, libSystem bridging). Ring-0 payloads (Windows drivers, Linux/Android kernel modules, macOS kexts) cannot simply reuse the ring-3 ABI: TEB/PEB does not exist, syscall instructions are user-to-kernel traps (drivers should not issue them), and x86_64 additionally requires a different code model and disabling the red zone.

## 1. Core Switch: `-mdyncode-context={user,kernel}`
...
- **Kernel mode**:
  ...
  - Injects `-D__NEVERC_DYNCODE_KERNEL__=1` so user-mode shim headers (`<windows.h>` / `<unistd.h>` / etc.) issue `#error`, preventing accidental inclusion.

`-mdyncode-context=` is a strict enum: only `user` or `kernel`. Invalid values must fail at the driver stage to prevent typos from generating bytecode at the wrong privilege level.
...
The user does not need to manually write resolver/cookie parameters — `KernelImportPass` injects them at the entry front. The loader passes them when calling the dyncode. Pure computation payloads or payloads that explicitly accept `neverc_kern_resolve_t` do not trigger this automatic prepend.
...
FNV-1a 64-bit, identical to `neverc_kern_hash()` in `<neverc/dyncode/kernel.h>`:
```c
...
Under automatic resolver rewriting, the first two parameters are always `(resolver, cookie)`. If the user explicitly declares `dyncode_entry(neverc_kern_resolve_t resolver, void *cookie, ...)` and calls the resolver themselves, the entry signature is preserved as-is.
...
2. **MIR layer**: `DynCodeMIRPrepPass::auditExternalReferences` re-audits
...
- **User mode**: when driver code is accidentally used in ring-3, the extractor emits `"'<name>' is a <os> kernel-only helper ... pass -mdyncode-context=kernel"`.
...
|------|---------------------|----------------------|
| User-mode `-fdyncode` | `<windows.h>` / `<unistd.h>` / `<fcntl.h>` / `<sys/stat.h>` / `<sys/mman.h>` / `<string.h>` / `<stdlib.h>` | `<neverc/dyncode/kernel.h>` |
| Kernel-mode `-mdyncode-context=kernel` | `<neverc/dyncode/kernel.h>` / `<string.h>` / `<stdlib.h>` / `<stddef.h>` / `<stdint.h>` (pure type headers) | `<windows.h>` / `<unistd.h>` / `<fcntl.h>` / `<sys/stat.h>` / `<sys/mman.h>` |

`<neverc/dyncode/kernel.h>` exposes:
...
neverc -fdyncode -mdyncode-context=kernel \
       -target aarch64-linux-gnu dyncode.c -o sc.bin
```
...
This is the **hosting driver's** code, compiled separately from the dyncode.

## 8. Roadmap

| Phase | Status | Content |
|-------|--------|---------|
| Kernel context switch + platform flags | Done | `-mdyncode-context=kernel`, `KernelInjectFlags`, pass gates |
...
| Resolver-based payload | Done | `<neverc/dyncode/kernel.h>` + stress test coverage |
