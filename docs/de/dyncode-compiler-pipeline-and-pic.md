**Sprachen**: [English](../dyncode-compiler-pipeline-and-pic.md) | [简体中文](../zh-CN/dyncode-compiler-pipeline-and-pic.md) | [繁體中文](../zh-TW/dyncode-compiler-pipeline-and-pic.md) | [日本語](../ja/dyncode-compiler-pipeline-and-pic.md) | [한국어](../ko/dyncode-compiler-pipeline-and-pic.md) | [Français](../fr/dyncode-compiler-pipeline-and-pic.md) | [Deutsch](dyncode-compiler-pipeline-and-pic.md) | [Español](../es/dyncode-compiler-pipeline-and-pic.md) | [Italiano](../it/dyncode-compiler-pipeline-and-pic.md) | [Русский](../ru/dyncode-compiler-pipeline-and-pic.md) | [العربية](../ar/dyncode-compiler-pipeline-and-pic.md)

[← DynCode-Compiler](dyncode-compiler.md)

# DynCode-Pipeline, MIR & PIC-Strategie (Design-Notizen)

Dieses Dokument beschreibt die Design-Kompromisse in NeverCs DynCode-Modus über die **IR → LLVM-Optimierung → Backend MIR → Objektdatei → Extraktion/Patching** Kette und die Beziehung zur **compilerweiten Standard-PIC**-Richtlinie. Implementierungsdetails sind im Quellcode und den englischen Kommentaren maßgeblich.

## 1. Warum standardmäßig PIC erzwingen (auch bei Nicht-DynCode)

Der DynCode-Extraktor nimmt an, dass Referenzen auf externe Symbole im ausführbaren Fragment auf **PC-relative** oder intra-`.text` auflösbaren Relocations landen, nicht auf hartcodierten absoluten Adressen oder Konstantenpools, die einen Loader zum Füllen von `.data` benötigen.
...
- Normale C-Kompilierung und `-fdyncode`-Kompilierung teilen dieselben Relocation-Gewohnheiten, was die kognitive Last „funktioniert normal, bricht unter DynCode" reduziert.
...
Diese Richtlinie unterscheidet nicht, ob `-fdyncode` aktiviert ist oder ob der Kontext user/kernel ist. Selbst wenn der Benutzer `-fno-pic` / `-static` / `-mkernel` / `-mdynamic-no-pic` übergibt, behält `ParsePICArgs()` `Reloc::PIC_` bei und verwendet die gleichen PC-relativen Annahmen für normale Kompilierung, Benutzermodul-DynCode und Kernelmodul-DynCode.

## 2. Zwei-Phasen IR- und MIR-Arbeitsteilung

### 2.1 IR-Schicht (`registerDynCodePasses`)
...
2. **`DynCodeMIRPrepPass`**: Entfernt Pseudos, die `.eh_frame` / `.pdata` / `.xray_*` Nebensektionen erzeugen würden, sodass der Befehlsstrom vor AsmPrinter möglichst nahe an „purem Code" ist.
...
**Prinzip**: Wenn native Befehlssequenzen noch Probleme haben, in MIR beheben (besonders um `DynCodeMIRPrepPass`); **Extraktion und Patching sind das letzte Sicherheitsnetz**, um Logikduplikation über COFF/ELF/Mach-O-Schichten zu vermeiden.

MIR-Opcode-Namen werden nicht in der Pass-Kontrollfluss verstreut; `DynCodeMIRPrepPass` verwendet die `Tables/MIRRewriteOpcodes.def` `(pattern, role, opcode)`-Tabelle über `TargetInstrInfo::getName()`. Beim Hinzufügen dyncode-freundlicher Befehlssubstitutionen werden Tabelleneinträge und schmale MIR-Rewrites bevorzugt; nur bei Bedarf auf Backend-`.td`-Befehlsauswahländerungen zurückfallen, Extraktor-Level-Objektformat-Fallback als letztes Mittel.

> Hinweis: `DynCodeMIRPrepPass` wird nur bei aktiviertem `-fdyncode` registriert. Normale Programme dürfen CFI/EH nicht global entfernen, da dies die reguläre Ausnahmebehandlung und Debug-Informationen zerstören würde.

Sowohl IR- als auch MIR-globale Callbacks verwenden ein **einmal registrieren, zur Laufzeit den aktuellen `DynCodeOptions`-Snapshot lesen**-Muster. Dies unterstützt langlebige eingebettete Compiler-Prozesse: Wenn derselbe Prozess zuerst DynCode und dann normales C kompiliert, erbt die normale C-Kompilierung nicht die vorherigen IR/MIR-Passes; bei aufeinanderfolgender Kompilierung mehrerer DynCode-TUs stapeln doppelte globale Callback-Registrierungen nicht den gleichen Pass-Satz mehrfach.
...
1. **Integriertes SDK** (Standard): NeverC bündelt ein vollständiges Windows SDK und WDK in `runtime/`. Header liegen in `runtime/windows/shared/`, architekturspezifische Bibliotheken in `runtime/windows/{x64,arm64}/`. Build-Layout:
   ...
   build-neverc/runtime/windows/x64/msvc/     (x64 Bibliotheken)
   build-neverc/runtime/windows/arm64/msvc/   (arm64 Bibliotheken)
   ```

2. **Explizites VS-Stil-Sysroot** (optional): Bei vorhandenem `VC/Tools/MSVC/<version>/...` + `Windows Kits/10/...`-Verzeichnisbaum über `-vctoolsdir=<path>` oder `-winsysroot=<path>` verweisen. Dieser Pfad hat Vorrang vor dem integrierten SDK.

Beide Quellen funktionieren ohne Registry oder OS-bereitgestellte VS-Umgebungsvariablen, was Windows-DynCode-Cross-Kompilierung von macOS / Linux ermöglicht.

## 5. Obfuskation und Erweiterungspunkte

- **IR-Interposes**: 6 IR-Interpose-Punkte (`NEVERC_INTERPOSE_SC_BEFORE_PREP` bis `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) über die [Plugin-API](plugin-api.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` ist der echte letzte IR-Injektionspunkt — nach hier registrierten Passes gibt es keine nachfolgenden Passes. 11 Interpose-Punkte insgesamt (6 IR + 3 MIR + 2 Byte-Stream).
- **MIR-Obfuskation**: `RunBeforePreEmit` / `RunAfterPreEmit` sind MIR-Interposes mittlerer Granularität; `RunAfterFinalMIR` ist der **echte letzte** MIR-Interpose (Fork-Erweiterung fügt `RegisterTargetPassConfigPostPreEmitCallbackFn` hinzu, aufgerufen nach `addPreEmitPass2()`). `-fdyncode-mir-obfuscate=` spezifiziert MIR-Spec separat; Standard ist IR-Spec wenn nicht gesetzt.
- **Byte-Stream-Interposes**: `RunPostExtract` ist der Finalize-**Pre**-Interpose; `RunPostFinalize` ist der Finalize-**Post**-Interpose (letzter Moment vor Festplattenschreiben, NeverC prüft danach nicht mehr).
- **Größe / Ausrichtung / Padding**: `-fdyncode-max-length=`, `-fdyncode-align=`, `-fdyncode-pad=` werden am Ende von Finalize ausgeführt; der Treiber lehnt widersprüchliche Konfigurationen ab.
...
Der DynCode-Modus führt `-mdyncode-context=user|kernel` als zweite Pipeline-Dimension ein, über dem Triple geschichtet:
...
- **Kernelmodus**:
  ...
  - `<neverc/dyncode/kernel.h>` exponiert `neverc_kern_resolve_t`, `neverc_kern_hash()` und verwandte Kernel-Signaturen; Benutzermodus-Shims (`<windows.h>`, `<unistd.h>` etc.) lehnen im Kernelmodus via `#error` ab.

Siehe [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md) für Details.
...
| Thema | Ansatz |
|-------|--------|
| Standard-PIC | Alle Toolchains `isPICDefaultForced()==true`, an DynCode-Annahmen ausgerichtet |
| Zuerst in IR beheben | Konstanten, indirekte Sprünge, Speicher-Intrinsics möglichst in IR eliminieren |
| MIR-Sicherheitsnetz | `DynCodeMIRPrepPass` + Pre/Post-Interposes, dann Objekt-Extraktion/Patching als letztes Mittel |
| Hardcoding minimieren | `TargetDesc` + `Options.td.h` tabellengesteuert |
| User/Kernel zwei Dimensionen | `-fdyncode` × `-mdyncode-context={user,kernel}`; jedes (OS, arch, level) ist eine Zeile in `describeTriple()` |
...
Shim-Header (`sys/mman.h`, `fcntl.h` etc.) exponieren Konstanten, die zum Ziel-Kernel-ABI passen müssen, da DynCode-Syscall-Stubs diese Werte direkt ohne libc-Übersetzung an den Kernel übergeben.
