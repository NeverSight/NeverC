...
- **IR-Interposes**: 6 IR-Interpose-Punkte (`NEVERC_INTERPOSE_SC_BEFORE_PREP` bis `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) über die [Plugin-API](../plugin-api/README.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` ist der echte letzte IR-Injektionspunkt — nach hier registrierten Passes gibt es keine nachfolgenden Passes. 11 Interpose-Punkte insgesamt (6 IR + 3 MIR + 2 Byte-Stream).
...
Siehe [kernel-mode-dyncode.md](kernel-mode-dyncode.md) für Details.
