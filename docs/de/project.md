...
> **Hinweis:** Der eingebaute **`string`**-Typ erfordert **`-fbuiltin-string`** für `.c`-Dateien. Er wird automatisch für [**`.nc`-Dateien**](nc-extension.md) und im **`-fdyncode`**-Modus aktiviert.
...
Ausführliche Designnotizen, Plattformmatrix, CLI-Referenz und Beispiele: **[Dokumentationsindex](README.md)**. Vollständig kompilierbare Beispiele: **[examples](examples.md)**.
...
Vollständige Befehlsreferenz: [`neverc runtime` →](runtime.md) · [`neverc update` →](update.md) · [`neverc build` / `make` →](build.md).

## Aus den Quellen bauen

Voraussetzungen, Build-Befehle, Cross-Kompilierung nach Windows, PATH-Einrichtung und Wechsel zwischen Release-Installation und In-Tree-Build — siehe **[Lokale Entwicklung](local-dev.md)**.
...
Siehe die [Hinweise](../../NOTICE), den [Leitfaden zur Quellenangabe](attribution.md) und die [maschinenlesbaren Zitierangaben](../../CITATION.cff).
