...
Ein Plugin, das ein eigenes Ziel registriert, kann stattdessen einen `PlanCallingConvention`-Callback an seinem `NevercCallingConventionDescriptor` bereitstellen und Pläne direkt erzeugen, wodurch die Spec-Schicht entfällt. Siehe [Target, MC, Assembly, Objekte](plugin-api-target-mc-object.md#abi-und-aufrufkonventionen).
...
Die oben verwendete Kerntabelle steht in [`PluginIR.h`], `NevercCallingConventionDescriptor` in [`PluginTarget.h`], die Phase `neverc.ir.pass.post_opt`, an die sich der Pass hängt, in [`Schema/PhaseSchema.json`].
