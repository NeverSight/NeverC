**Sprachen**: [English](../builtins-string.md) | [简体中文](../zh-CN/builtins-string.md) | [繁體中文](../zh-TW/builtins-string.md) | [日本語](../ja/builtins-string.md) | [한국어](../ko/builtins-string.md) | [Français](../fr/builtins-string.md) | [Deutsch](builtins-string.md) | [Español](../es/builtins-string.md) | [Italiano](../it/builtins-string.md) | [Русский](../ru/builtins-string.md) | [العربية](../ar/builtins-string.md)

[← NeverC Integriertes Laufzeitsystem](builtins.md) · [NeverC Dokumentation](README.md)

# NeverC eingebauter `string`-Typ

## Übersicht

NeverC bietet einen eingebauten `string`-Werttyp für C. Er vereint das Interface-Design von C++ `std::string` mit den Unicode-Fähigkeiten von Qt `QString` und ermöglicht sichere und effiziente String-Operationen in C-Code.

**Aktivierung:** `-fbuiltin-string` beim Kompilieren angeben (standardmäßig deaktiviert). Automatische Aktivierung in folgenden Fällen:
- Die Eingabedatei hat die Erweiterung `.nc` (siehe [`.nc`-Erweiterungsdokumentation](nc-extension.md))
- Der `-fdyncode`-Modus ist aktiv

```bash
neverc hello.nc -o hello                # automatisch — .nc aktiviert es
neverc -fbuiltin-string main.c -o main  # explizites Flag für .c-Dateien
```
...
### Besitzmodell: Owned vs Borrowed
...
#### ASCII Groß-/Kleinschreibung ignorieren
...
#### Verbrauchen + neuen Wert zurückgeben
...
## Kompilierzeit-Stringverschlüsselung

`.encrypt()` bietet XOR-Verschlüsselung zur Kompilierzeit für String-Literale. Der Klartext wird niemals im Binary gespeichert — er wird zur Kompilierzeit verschlüsselt und zur Laufzeit über eine `always_inline`-Funktion entschlüsselt, sodass statische Analysetools (z.B. `strings`) den Originalinhalt nicht extrahieren können.
...
// aber `strings ./binary` findet "API_KEY_12345" NICHT
```
...
`.encrypt()` unterstützt automatisch alle String-Literal-Typen — der Compiler faltet Wide-/UTF-16-/UTF-32-Literale vor der Verschlüsselung zu UTF-8:
...
```c
string s = "hello";
string e = s.encrypt();  // FEHLER: .encrypt() can only be applied to string literals
```

- `.encrypt()` akzeptiert **keine Argumente**:

```c
string e = "hello".encrypt(42);  // FEHLER: .encrypt() takes no arguments
```

- Doppelte Verschlüsselung wird abgelehnt:

```c
string e = "hello".encrypt().encrypt();  // FEHLER: .encrypt() can only be applied to string literals
```
...
## Hosted-Modus: Vorkompilierte Bitcode-Architektur
...
### 4-Schichten-Dispatch-Priorität
...
### Schritte zum Hinzufügen einer neuen Runtime-Funktion
