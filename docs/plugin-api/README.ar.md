...
2. **حماية الإصدار (للواجهات المستقبلية)**: عندما تحصل vtable على حقول جديدة بعد الإصدار، احمِ الاستدعاءات بـ `NEVERC_API_FN` فقط إذا كان الإضافة يجب أن تعمل على مضيفين قديمين يفتقرون لهذه الحقول. إدخالات vtable الحالية لا تحتاج حماية.
