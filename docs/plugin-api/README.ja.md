...
- **Binary Pass**：生バイト操作（dyncode 抽出、バイナリパッチ）。
...
| フック | レベル | 説明 |
|--------|-------|------|
| `NEVERC_INTERPOSE_PRE_OPT` | IR | LLVM 最適化パス前 |
| `NEVERC_INTERPOSE_POST_OPT` | IR | LLVM 最適化パス後 |
| `NEVERC_INTERPOSE_PIPELINE_START` | IR | パイプラインの最初 |
| `NEVERC_INTERPOSE_PIPELINE_LAST` | IR | IR パイプラインの最後 |
| `NEVERC_INTERPOSE_BEFORE_CODEGEN_PREEMIT` | MIR | pre-emit マシンパス前 |
| `NEVERC_INTERPOSE_AFTER_CODEGEN_FINAL_MIR` | MIR | 全マシンパス後 |

### DynCode / LTO / リンカーフロー

DynCode フックは `NEVERC_INTERPOSE_SC_*`、LTO フックは `NEVERC_INTERPOSE_LTO_*`、リンカーフックは `NEVERC_INTERPOSE_LINK_*` プレフィックスを使用します。
