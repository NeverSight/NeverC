...
API ввода-вывода (см. [Source и ввод-вывод](source.ru.md)); именно так хост
подтверждает, что файлы, которые задание заявило записанными, действительно
существуют с указанными дайджестами. `GetJobResult` читает зафиксированный
результат и, как и выбор цепочки, сообщает `BuiltinProviderUsed`.
