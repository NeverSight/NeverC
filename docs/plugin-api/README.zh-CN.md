...
| `NevercNamedMDRef` | 命名 Metadata 节点 |
| `NevercComdatRef` | COMDAT 组 |
...
| `NEVERC_FOR_EACH_GLOBAL(api, m, var)` | 遍历全局变量 |
| `NEVERC_FOR_EACH_ALIAS(api, m, var)` | 遍历别名 |
...
| `NEVERC_FOR_EACH_SYMBOL(api, var)` | 遍历链接器符号 |
| `NEVERC_FOR_EACH_SECTION(api, var)` | 遍历链接器节区 |
| `NEVERC_ALLOC_ARRAY(api, type, count)` | 类型化堆分配 |
| `NEVERC_CALLOC_ARRAY(api, type, count)` | 类型化零初始化堆分配 |
| `NEVERC_REALLOC_ARRAY(api, ptr, type, count)` | 类型化堆重分配 |
| `NEVERC_COLLECT_FUNCTIONS(api, m, count)` | 批量收集所有函数到数组 |
| `NEVERC_COLLECT_DEFINED_FUNCTIONS(api, m, count)` | 批量收集已定义函数 |
...
| `NEVERC_STRMAP_NEW(api, cap)` | 创建指定容量的 StrMap |
| `NEVERC_INTMAP_NEW(api, cap)` | 创建指定容量的 IntMap |
...
| `NEVERC_CLAMP(v, lo, hi)` | 将值限制在 `[lo, hi]` 范围 |
