...
A plugin that registers its own target can instead supply a `PlanCallingConvention` callback on its `NevercCallingConventionDescriptor` and produce plans directly, skipping the spec layer. See [Target, MC, assembly, object](plugin-api-target-mc-object.md#abi-and-calling-conventions).
