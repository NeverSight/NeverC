...
`/GUARD:MIXED`는 보호된 객체 파일과 레거시 객체 파일이 섞인 입력에 CFG 출력을 활성화합니다. 보호되지 않은 객체에 필요한, 보수적으로 수집된 주소 취득 대상이 포함됩니다. 이는 별도의 컴파일러 계측 모드도 아니고 새로운 PE `GuardFlags` 비트도 아닙니다.
...
링커는 x86-64 및 ARM64 COFF 엔클레이브 이미지를 지원합니다. 게시된 구성 포인터는 검증하지만 `IMAGE_ENCLAVE_CONFIG` 내부의 버전 관리 필드에 추가 정책을 적용하지는 않습니다.
