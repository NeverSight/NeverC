...
`/GUARD:MIXED` は、保護されたオブジェクトファイルと従来のオブジェクトファイルが混在する入力に対して CFG 出力を有効にします。保護されていないオブジェクトに必要な、保守的に収集されたアドレス取得対象を含めます。これは独立したコンパイラーの計装モードでも、新しい PE `GuardFlags` ビットでもありません。
...
リンカーは x86-64 および ARM64 の COFF エンクレーブイメージをサポートします。公開された構成ポインターは検証しますが、`IMAGE_ENCLAVE_CONFIG` 内のバージョン管理されたフィールドに追加のポリシーは課しません。
