...
`/GUARD:MIXED` 为受保护对象文件和旧式对象文件的混合输入启用 CFG 输出。它会包含未受保护对象所需的、以保守方式收集的地址被获取目标；它不是独立的编译器插桩模式，也不是新的 PE `GuardFlags` 位。
...
链接器支持 x86-64 和 ARM64 COFF enclave 映像。它会验证已发布的配置指针，但不会对 `IMAGE_ENCLAVE_CONFIG` 内部带版本的字段施加额外策略。
