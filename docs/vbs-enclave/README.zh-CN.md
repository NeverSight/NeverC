...
`/GUARD:MIXED` 为受保护对象文件和旧式对象文件的混合输入启用 CFG 输出。它会生成 5 字节的 GFID 和 GIAT 条目：4 字节 RVA 加 1 字节元数据；当前普通目标的元数据为零。其 `GuardFlags` 包含 CFG、受保护的 delay-IAT 和条目大小位。旧式对象通过保守扫描重定位来提供地址被获取目标，同时排除展开元数据。
