...
`/GUARD:MIXED` 會為受保護與舊式目的檔混合輸入啟用 CFG 輸出。它會納入未受保護目的檔所需、以保守方式收集的位址被取得目標；它不是獨立的編譯器插樁模式，也不是新的 PE `GuardFlags` 位元。
...
連結器支援 x86-64 與 ARM64 COFF enclave 映像。它會驗證已發佈的設定指標，但不會對 `IMAGE_ENCLAVE_CONFIG` 內部帶版本的欄位施加額外原則。
