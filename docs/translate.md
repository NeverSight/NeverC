...
Core v2 also supports implicit and explicitly defaulted copy assignment. Members and array elements retain their selected assignment operations and order; generated trivial array copies become typed element assignments with no external memory-copy call. Trivial assignment preserves stored pointers and returns the actual receiver. Operator syntax captures the source reference before the receiver; explicit member syntax captures the receiver first. Source values are read after those effects, and assignment introduces no extra constructed or destroyed object. Default member initializers, moves, templates and full STL remain in development.
