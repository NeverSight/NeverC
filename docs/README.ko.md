...
| [문자열 암호화 (xorstr)](builtins/xorstr/README.ko.md) | `-fencrypt-call-strings` | 인스턴스별 암호화, 필수 late sealing, 호출 지점별 최종 전개, volatile 스택 정리 |
...
| [릴리스 바이너리와 `--strip`](release-builds/README.ko.md) | 런타임에 필요 없는 심볼과 소스 디버그를 제거하고 `.ko`에는 커널 인식 구조적 심볼 이름 변경을 적용합니다(hash 또는 encryption 아님) |
...
| 문서 | 설명 |
|------|------|
| [VBS 엔클레이브 DLL](vbs-enclave/README.ko.md) | Microsoft 호환 VBS 엔클레이브 이미지를 링크·검증·처리·서명·로드 |
