...
[`Schema/PhaseSchema.json`] です。実行可能なカバレッジ契約は
[`coverage.json`] です。
...
規範的な宣言は [`PluginDynCode.h`] を、読み取り専用のフェーズトレーサは
[`pluginsdk/examples/DynCodeTracePlugin.c`] を、charset エンコーダは
[`pluginsdk/examples/DynCodeEncoderPlugin.c`] を参照してください。
