...
Core v2 は値渡しのレコード引数ごとに独立したオブジェクトを作り、レコードの戻り値を呼び出し側の格納先に直接書き込みます。コンストラクターとメソッドにも同じ規則を適用し、ソースで必要なコピーと参照の別名関係は維持します。これは対応する自明なレコード型に対する NeverC の選択であり、C++17 の全実装に共通するアドレス保証ではありません。非自明なコピー／ムーブ、破棄、クリーンアップ、STL 全般への対応は開発中です。
