...
core v2 は固定長のローカル配列と配列フィールド、多次元の添字アクセス、配列へのポインターと参照にも対応します。部分初期化では省略されたスカラー要素をゼロ初期化し、レコード要素には選択された初期化を適用します。要素の初期化順序とエイリアス関係を保持します。配列長と初期化の展開量には上限があります。グローバル配列、可変長配列、要素の非自明なデストラクターは未対応です。
...
Core v2 は、標準レイアウトでコピーと破棄が自明なレコード型の通常のユーザー定義コンストラクターにも対応します。ローカル変数、フィールド、配列要素は最終的な格納位置で直接構築し、フィールドは宣言順に初期化します。明示的に default 指定されたコンストラクター、委譲、ユーザー定義のコピー／ムーブとデストラクター、クリーンアップ、例外は未対応です。
