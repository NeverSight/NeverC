...
Core v2 は、対応する単純な集成体の名前付き非仮想メンバー関数を扱えます。const オーバーロード、左辺値修飾メソッド、静的メソッド、`this` に対応し、元のオブジェクトの同一性を保ち、引数より先に呼び出し対象を評価します。一時オブジェクトへの非静的呼び出し、特殊メンバー関数、継承、テンプレート、STL 全般は未対応です。
