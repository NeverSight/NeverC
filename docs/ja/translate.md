...
Core v2 は標準レイアウトで対応するコピー操作を持つレコード型の通常のユーザー定義コンストラクターにも対応します。ローカル変数、フィールド、配列要素は最終的な格納位置で直接構築し、フィールドは宣言順に初期化します。明示的に default 指定されたコンストラクター、委譲、ムーブコンストラクター、例外は未対応です。
...
Core v2 はソース引数が `R&` または `const R&` の通常のユーザー定義コピーコンストラクターとコピー代入演算子に対応します。実際の格納先でコピーし、副作用と返された参照を保持します。代入演算子の構文では右辺を先に評価し、明示的な `operator=` 呼び出しでは呼び出し対象を先に評価します。default 指定の特殊メンバーと暗黙の非自明なコピーは未対応です。
