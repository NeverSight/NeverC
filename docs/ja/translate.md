...
Core v2 は生存する `R&&` または `const R&&` をソースとする通常のユーザー定義ムーブコンストラクターとムーブ代入を実行します。構築は実際の格納先を使い、代入はソースの変更、評価順序、返された `R&` の参照先を保持します。`&`／`&&` 修飾の代入も対応します。名前付き右辺値参照は左辺値のオーバーロードを選び、明示的コンストラクターは初期化規則に従います。ムーブ後もソースと格納先は通常どおり破棄されます。生成／default ムーブ、新しい一時オブジェクトへの参照束縛、例外、テンプレートと完全な STL は開発中です。
