...
Core v2 はソース引数が `R&` または `const R&` の通常のユーザー定義コピーコンストラクターとコピー代入演算子に対応します。実際の格納先でコピーし、副作用と返された参照を保持します。代入演算子の構文では右辺を先に評価し、明示的な `operator=` 呼び出しでは呼び出し対象を先に評価します。default 指定のコピー代入と暗黙の非自明なコピー代入は未対応です。

Core v2 は、生成されたデフォルトコンストラクター、default 指定のデフォルトコンストラクターとデストラクターにも対応し、入れ子のレコードや配列メンバーも扱います。実際の格納先で宣言順に初期化し、値初期化では C++ が要求する場合だけゼロ初期化を行います。クラス外での default 指定に固有の初期化規則も保持します。未使用または未評価式だけに現れるコンストラクターに、余分な関数本体を要求しません。デフォルトメンバー初期化子、暗黙の非自明なコピー代入、ムーブと STL 全般は引き続き開発中です。

Core v2 は暗黙および default 指定のコピーコンストラクターに対応し、入れ子のレコードや多次元配列も扱います。選択されたメンバーコピーを宣言順・要素順に実際の格納先で実行し、コピー元配列のアドレスは一度だけ評価します。自明なコピーでは格納済みポインターの値を保持します。値渡しと元のオブジェクトからの戻り値はコピーの副作用を保持し、純粋右辺値の直接転送では追加コピーを行いません。生成されるコピー代入、ムーブと残りの C++／STL 機能は開発中です。
