...
Core v2 は暗黙および default 指定のコピー代入にも対応します。メンバーと配列要素は選択された代入と順序を保持し、生成された自明な配列コピーは外部のメモリーコピー呼び出しを使わず型付きの要素代入になります。自明な代入は格納済みポインターを保持し、実際の呼び出し対象への参照を返します。演算子構文ではソース参照を先に、明示的なメンバー呼び出しでは対象を先に確定し、その副作用の後でソース値を読みます。代入のために追加の構築や破棄を行いません。デフォルトメンバー初期化子、ムーブ、テンプレートと STL 全般は開発中です。
