...
Core v2 は標準レイアウトでコピーが自明なレコード型の通常のユーザー定義コンストラクターにも対応します。ローカル変数、フィールド、配列要素は最終的な格納位置で直接構築し、フィールドは宣言順に初期化します。明示的に default 指定されたコンストラクター、委譲、ユーザー定義のコピー／ムーブ、例外は未対応です。
...
Core v2 は通常のユーザー定義デストラクターと、正常終了時の暗黙のメンバー破棄に対応します。ローカルオブジェクト、フィールド、配列要素は逆順に破棄し、一時オブジェクトは必要な値を保存してから完全式の終わりに破棄します。return、分岐、ループ、break、continue で必要な破棄を行います。値引数は呼び出された関数の終了時に破棄し、戻り値のオブジェクトは呼び出し側が管理します。明示的なデストラクター呼び出し、記述された例外仕様、静的オブジェクトの破棄、例外の巻き戻しは未対応です。非自明なコピー／ムーブと STL 全般は開発中です。
