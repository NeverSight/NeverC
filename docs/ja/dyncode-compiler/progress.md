**言語**: [English](../../dyncode-compiler/progress.md) | [简体中文](../../zh-CN/dyncode-compiler/progress.md) | [繁體中文](../../zh-TW/dyncode-compiler/progress.md) | [日本語](progress.md) | [한국어](../../ko/dyncode-compiler/progress.md) | [Français](../../fr/dyncode-compiler/progress.md) | [Deutsch](../../de/dyncode-compiler/progress.md) | [Español](../../es/dyncode-compiler/progress.md) | [Italiano](../../it/dyncode-compiler/progress.md) | [Русский](../../ru/dyncode-compiler/progress.md) | [العربية](../../ar/dyncode-compiler/progress.md)

[← DynCode コンパイラ](README.md)

# DynCode コンパイラ — 進捗トラッカー

## ステージ 0 — macOS arm64 MVP（完了）

- [x] ディレクトリ＆ CMake スケルトン（`nevercDynCode` ライブラリ）
...
**テスト**：108/108 dyncode アサーション合格。バイナリサイズ：`add` 8B、`fib` 64B、`hello` 64B、`big_const` 632B。

## ステージ 1 — Linux / Android / Windows クロスプラットフォーム（完了）

- [x] `TargetDesc` 抽象化：テーブル駆動のプラットフォーム差異
- [x] クロスプラットフォーム `-mdyncode-syscall` セマンティクス（Darwin 専用の `-mdyncode-libsystem` を置き換え）
- [x] Linux / Android syscall 番号テーブル（Darwin BSD 100+、Linux arm64 130+、Linux x86_64 150+）
- [x] `DynCodeExtractor` を `MachOExtractor` / `ELFExtractor` / `COFFExtractor` にリファクタ
...
- [x] DynCode シムヘッダー：`<windows.h>`、`<unistd.h>`、`<fcntl.h>`、`<sys/stat.h>`、`<sys/mman.h>`、`<string.h>`、`<stdlib.h>`
...
**テスト**：743+ dyncode アサーション、8 トリプルすべてで合格。NeverC 全体のテストスイート：1000+ テスト合格。

## ステージ 2 — 印字可能 / 英数字エンコーダー（予定）

- [ ] ARM64 印字可能 dyncode エンコーダー（0x20–0x7e 命令サブセット）
- [ ] x86_64 英数字エンコーダー
