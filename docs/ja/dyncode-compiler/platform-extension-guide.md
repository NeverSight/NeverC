**言語**: [English](../../dyncode-compiler/platform-extension-guide.md) | [简体中文](../../zh-CN/dyncode-compiler/platform-extension-guide.md) | [繁體中文](../../zh-TW/dyncode-compiler/platform-extension-guide.md) | [日本語](platform-extension-guide.md) | [한국어](../../ko/dyncode-compiler/platform-extension-guide.md) | [Français](../../fr/dyncode-compiler/platform-extension-guide.md) | [Deutsch](../../de/dyncode-compiler/platform-extension-guide.md) | [Español](../../es/dyncode-compiler/platform-extension-guide.md) | [Italiano](../../it/dyncode-compiler/platform-extension-guide.md) | [Русский](../../ru/dyncode-compiler/platform-extension-guide.md) | [العربية](../../ar/dyncode-compiler/platform-extension-guide.md)

[← DynCode コンパイラ](README.md)

# プラットフォーム拡張ガイド

本ドキュメントは dyncode コンパイラを新しいターゲットプラットフォームに拡張する方法を説明する。現在サポート：**macOS / Linux / Android / Windows 上の arm64 / x86_64**（8 つの triple）、各々独立した **User** / **Kernel** コンテキスト（計 16 バリアント）。新プラットフォーム追加は通常数百行のコード。
...
| `DriverInjectFlags` | dyncode モードで注入するプラットフォーム固有フラグ | null → 注入なし |
...
2. `lib/Headers/windows.h` の dyncode ブランチに宣言を追加
3. pass 変更不要
...
2. `DynCodeExtractor.cpp` のディスパッチ switch に case 追加
...
- **ABI 差分**：Win64（rcx/rdx/r8/r9）vs System V AMD64（rdi/rsi/rdx/rcx/r8/r9）で引数レジスタが完全に異なる。NeverC フロントエンド層で処理；dyncode パイプラインは関知不要。
...
dyncode パイプラインは `Pipeline.h::ObfuscationInterposes` 経由で 11 フックをサードパーティ難読化ライブラリに公開：
...
// Plugin API 経由で登録：NEVERC_INTERPOSE_SC_* フック（plugin-api ドキュメント参照）
```
