**言語**: [English](../roadmap.md) | [简体中文](../zh-CN/roadmap.md) | [繁體中文](../zh-TW/roadmap.md) | [日本語](roadmap.md) | [한국어](../ko/roadmap.md) | [Français](../fr/roadmap.md) | [Deutsch](../de/roadmap.md) | [Español](../es/roadmap.md) | [Italiano](../it/roadmap.md) | [Русский](../ru/roadmap.md) | [العربية](../ar/roadmap.md)

[← ドキュメント索引](README.md)

# NeverC ロードマップ

このドキュメントは、既存の dyncode コンパイラと組み込みランタイムを超えた NeverC プロジェクトの主要な計画方向を概説します。
...
- **DynCode 互換** — フリースタンディングモードで意味のあるパッケージ（例：`crypto`、`encoding`、`bytes`）は `-fdyncode` で動作
...
- dyncode コンパイル経験（位置独立、最小ランタイムコード）はオンチェーンプログラムの制約に直接マッピング
