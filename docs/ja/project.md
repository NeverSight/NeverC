...
> **注：** 組み込み **`string`** 型は `.c` ファイルでは **`-fbuiltin-string`** が必要です。[**`.nc` ファイル**](nc-extension.md) または **`-fdyncode`** モードでは自動的に有効になります。
...
詳細な設計、プラットフォームマトリクス、CLI リファレンス、例は **[ドキュメント索引](README.md)** を参照。ビルド可能なサンプルは **[examples](examples.md)** を参照。
...
Release インストールでは、コンパイラとインストール済みのクロスコンパイル runtime を一つのバージョン単位として同期更新できます：

```bash
neverc update                 # 最新の完全な release へ更新
neverc update v3389.1.2       # 指定したバージョンへ切り替え（ダウングレードも可）
```

`neverc upgrade` は同義コマンドです。NeverC は一つの具体的な release タグを解決し、
すでに存在する runtime だけを再インストールして、すべてをコンパイラの対象バージョンに固定します。
必要な全アーカイブは既存ファイルを変更する前にダウンロード、SHA256 検証、展開、内容検証されます。
ステージングや検証の失敗では現在のインストールは変わらず、コミット失敗時は自動的にロールバックします。
runtime release に問題がある場合は `neverc update <古いバージョン>` を実行すると、
コンパイラとインストール済み runtime をまとめて戻せます。

コマンドの詳細は [`neverc runtime` →](runtime.md) · [`neverc update` →](update.md) · [`neverc build` / `make` →](build.md)。

## ソースからビルド

ビルド要件、ビルドコマンド、Windows クロスコンパイル、PATH 設定、release インストールとツリー内ビルドの切り替えについては **[ローカル開発](local-dev.md)** を参照してください。
...
AI/LLM の支援による再利用や LLVM 関連の再利用を含め、コードをコピー・改変する際は、適用されるライセンスに従い、必要な著作権・ライセンス・帰属表示を保持してください。これとは別に、NeverC を参考にする場合は、プロジェクトと元の出典の引用をお願いします。これはプロジェクトからの引用のお願いであり、追加のライセンス条件ではありません。

[告知事項](../../NOTICE)、[帰属表示ガイド](attribution.md)、[機械可読の引用情報](../../CITATION.cff)を参照してください。
