...
自前のターゲットを登録するプラグインは、代わりに `NevercCallingConventionDescriptor` に `PlanCallingConvention` コールバックを提供し、spec の層を飛ばして直接 plan を生成することもできます。[ターゲット、MC、アセンブリ、オブジェクト](plugin-api-target-mc-object.md#abi-と呼び出し規約) を参照してください。
