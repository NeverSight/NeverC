**言語**: [English](../vbs-enclave.md) | [简体中文](../zh-CN/vbs-enclave.md) | [繁體中文](../zh-TW/vbs-enclave.md) | [日本語](vbs-enclave.md) | [한국어](../ko/vbs-enclave.md) | [Français](../fr/vbs-enclave.md) | [Deutsch](../de/vbs-enclave.md) | [Español](../es/vbs-enclave.md) | [Italiano](../it/vbs-enclave.md) | [Русский](../ru/vbs-enclave.md) | [العربية](../ar/vbs-enclave.md)
...
クロスホストのリンク段階では、未署名かつ未処理のエンクレーブ DLL が生成されます。VEIID 処理、SignTool 署名、および `CreateEnclave`/`LoadEnclaveImage` による実際の読み込みは、引き続き Windows でのみ実行できます。そのため、macOS または Linux でリンクした DLL は、最後の 3 段階を実行する Windows のパッケージングマシンまたはテストマシンへ移動してください。ランタイムのインストールと検出については、[ターゲットランタイム](runtime.md)を参照してください。
...
`/GUARD:MIXED` を `/GUARD:EHCONT` と組み合わせる場合、EH 継続ターゲットテーブルも 5 バイトのエントリ（4 バイトの RVA と、それに続く値 0 のメタデータバイト）を使用します。
...
7. `GetProcAddress` でエンクレーブのエクスポート関数を解決し、`CallEnclave` を通じて実行して戻り値を検証します。CFG で保護された間接呼び出し経路と従来の間接呼び出し経路を含め、繰り返し呼び出します。
8. `TerminateEnclave` と `DeleteEnclave` でエンクレーブを終了・解放し、両方の操作が成功したことを確認します。
...
x64 ランタイムプローブは Microsoft イメージを最初に実行し、その後に両方の NeverC 候補を実行します。`GetProcAddress` でエクスポート関数を解決し、`CallEnclave` を繰り返し呼び出して、CFG で保護された経路と従来の間接呼び出し経路を実行し、戻り値を検証します。`PASS` には、終了と解放の成功を含む全ライフサイクル段階の証跡が正しい順序で必要です。読み込みと初期化の成功だけでは不十分です。このワークフローでは ARM64 を静的検証と差分リンクでカバーしており、通常のランタイムプローブは引き続き x64 のみが対象です。

2026-09-11 に、ネイティブ Windows 11 ARM64 上での個別の検証が成功しました（[実行 34554067852](https://github.com/NeverSight/NeverC/actions/runs/34554067852)）。`c46e3a4cc4b3732823a7241a43bca6c44b0f0b74` からビルド済みの 2 つの NeverC ARM64 DLL を再利用し、新たにビルドした MSVC 参照イメージとともにテストしました。48 のライフサイクル段階と、結果検証を伴う 12 回の呼び出しがすべて成功し、エンクレーブの終了・解放と署名証明書のクリーンアップも成功しました。これは対象の固定成果物を検証したものであり、NeverC コンパイラの新規ビルドや恒常的な ARM64 実行時 CI ゲートを意味するものではありません。

任意のランタイムプローブで `SKIP` を報告できるのは、関数実行前に既知の環境準備条件を満たせない場合だけです。クラッシュ、タイムアウト、段階の証跡の欠落や順序違反、機能上のエラーは、Microsoft 参照イメージも含めて `FAIL` になります。構成済みのセルフホステッド VBS runner では、ランタイムの成功を必須ゲートにできます。
