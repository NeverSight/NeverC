...
x64 ランタイムプローブは Microsoft イメージを最初に実行し、その後に両方の NeverC 候補を実行します。`GetProcAddress` でエクスポート関数を解決し、`CallEnclave` を繰り返し呼び出して、CFG で保護された経路と従来の間接呼び出し経路を実行し、戻り値を検証します。`PASS` には、終了と解放の成功を含む全ライフサイクル段階の証跡が正しい順序で必要です。読み込みと初期化の成功だけでは不十分です。ARM64 は静的検証と差分リンクの対象であり、実行時検証の対象ではありません。
