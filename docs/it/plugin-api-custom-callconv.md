...
Un plugin che registra un proprio target può invece fornire una callback `PlanCallingConvention` sul suo `NevercCallingConventionDescriptor` e produrre piani direttamente, saltando il livello della spec. Vedi [Target, MC, assembly, oggetti](plugin-api-target-mc-object.md#abi-e-convenzioni-di-chiamata).
