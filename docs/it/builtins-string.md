**Lingue**: [English](../builtins-string.md) | [简体中文](../zh-CN/builtins-string.md) | [繁體中文](../zh-TW/builtins-string.md) | [日本語](../ja/builtins-string.md) | [한국어](../ko/builtins-string.md) | [Français](../fr/builtins-string.md) | [Deutsch](../de/builtins-string.md) | [Español](../es/builtins-string.md) | [Italiano](builtins-string.md) | [Русский](../ru/builtins-string.md) | [العربية](../ar/builtins-string.md)

[← Sistema Runtime Integrato di NeverC](builtins.md) · [Documentazione NeverC](README.md)
...
**Attivazione:** Passare `-fbuiltin-string` in fase di compilazione (disattivato di default). Attivazione automatica nei seguenti casi:
- Il file di input ha estensione `.nc` (vedi [documentazione estensione `.nc`](nc-extension.md))
- La modalità `-fdyncode` è attiva

```bash
neverc hello.nc -o hello                # automatico — .nc lo attiva
neverc -fbuiltin-string main.c -o main  # flag esplicito per file .c
```
...
#### Pulizia automatica dei tipi composti

Il compilatore rileva e rilascia ricorsivamente i membri `string` in array, strutture e combinazioni annidate:
...
Il livello CodeGen attraversa l'albero dei tipi per generare chiamate di cleanup per ogni elemento. Solo le stringhe owned (`cap != 0`) vengono rilasciate; le viste vengono ignorate.
...
## Crittografia delle stringhe a tempo di compilazione

`.encrypt()` fornisce la crittografia XOR a tempo di compilazione per i letterali stringa. Il testo in chiaro non viene mai memorizzato nel binario — viene crittografato a tempo di compilazione e decrittografato a runtime tramite una funzione `always_inline`, rendendo impossibile l'estrazione del contenuto originale da parte degli strumenti di analisi statica (es. `strings`).
...
// ma `strings ./binary` NON troverà "API_KEY_12345"
```
...
- **Default**: il seed `0` ottiene nel compilatore nuova entropia dal sistema operativo per ogni letterale cifrato e fallisce in modo sicuro se non è disponibile. Build indipendenti producono ciphertext diverso.
- **Override CLI**: un seed non zero, come `-fstring-encrypt-key=0xDEADBEEF`, seleziona un seed completo e deterministico a 64 bit. Un nonce locale alla translation unit separa i letterali, rendendolo adatto a build riproducibili e test.

### Tutti i tipi di letterale supportati

`.encrypt()` supporta automaticamente tutti i tipi di letterale — il compilatore converte i letterali wide/UTF-16/UTF-32 in UTF-8 prima della crittografia:
...
- `.encrypt()` può essere applicato **solo** ai letterali stringa. Chiamarlo su una variabile produce un errore di compilazione:

```c
string s = "hello";
string e = s.encrypt();  // ERRORE: .encrypt() can only be applied to string literals
```

- `.encrypt()` **non accetta argomenti**:

```c
string e = "hello".encrypt(42);  // ERRORE: .encrypt() takes no arguments
```

- La doppia crittografia viene rifiutata:

```c
string e = "hello".encrypt().encrypt();  // ERRORE: .encrypt() can only be applied to string literals
```
...
`ENCRYPT_BYTE` usa XOR per impostazione predefinita:

```c
((char)((unsigned char)(byte) ^ (unsigned char)((key) >> (8 * ((idx) % sizeof(size_t))))))
```

`DECRYPT_BYTE` usa per impostazione predefinita una **decomposizione aritmetica senza istruzioni XOR** — calcola `a ^ b` come `(a + b) - (a & b) - (b & a)` con intermediari `volatile` per impedire la ri-ottimizzazione di LLVM in `xor`. Può essere rafforzata con pass di offuscamento MBA (Mixed Boolean-Arithmetic).
...
| Flag | Descrizione |
|------|-------------|
| `-fstring-encrypt-key=<hex>` | Impostare il seed di protezione completo a 64 bit; `0` usa nuova entropia del sistema operativo |

### Parametri configurabili

| Macro | Default | Descrizione |
|-------|---------|-------------|
| `NEVERC_STRING_ENCRYPT_BYTE(byte, key, idx)` | XOR con byte chiave rotanti | Operazione di crittografia per byte (compilazione) |
| `NEVERC_STRING_DECRYPT_BYTE(byte, key, idx)` | XOR con byte chiave rotanti | Operazione di decrittografia per byte (runtime); deve essere l'inverso di `ENCRYPT_BYTE` |

### Stringhe crittografate in array e struct

`.encrypt()` funziona negli inizializzatori aggregati. I membri `string` owned vengono rilasciati automaticamente all'uscita dallo scope (vedi [Pulizia automatica dei tipi composti](#pulizia-automatica-dei-tipi-composti)):
...
### Compatibilità modalità DynCode

La crittografia delle stringhe funziona in tutte le modalità di compilazione, incluso dyncode (`-fdyncode`). In modalità dyncode, la decrittografia utilizza l'allocatore arena locale. Sono supportati sia il contesto utente che kernel (`-mdyncode-context=kernel`).
