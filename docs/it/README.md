...
| Documento | Descrizione |
|-----------|-------------|
| [`neverc run`](run.md) | Compila, esegue localmente ed elimina un binario temporaneo (stile `go run`) |
| [`neverc translate`](translate.md) | Tradurre C++ in NeverC |
| [`neverc update`](update.md) | Aggiornare o declassare un'installazione release (compilatore + runtime installati a un tag) |
| [`neverc runtime`](runtime.md) | Installare, elencare, aggiornare o rimuovere sysroot di cross-compilazione |
| [`neverc build` / `neverc make`](build.md) | Driver compatibile GNU Make per Makefile di esempi e progetti |
| [Binari di rilascio e `--strip`](release-builds.md) | Rimuove simboli non necessari e debug sorgente, con rinomina strutturale dei simboli `.ko` consapevole del kernel (non è hash né encryption) |
...
| Documento | Descrizione |
|-----------|-------------|
| [DLL di enclave VBS](vbs-enclave.md) | Collegare, convalidare, elaborare, firmare e caricare immagini di enclave VBS compatibili con Microsoft |
...
## Attribuzione e citazione delle fonti

**[guida all’attribuzione →](attribution.md)**
