**Lingue**: [English](../dyncode-compiler.md) | [简体中文](../zh-CN/dyncode-compiler.md) | [繁體中文](../zh-TW/dyncode-compiler.md) | [日本語](../ja/dyncode-compiler.md) | [한국어](../ko/dyncode-compiler.md) | [Français](../fr/dyncode-compiler.md) | [Deutsch](../de/dyncode-compiler.md) | [Español](../es/dyncode-compiler.md) | [Italiano](dyncode-compiler.md) | [Русский](../ru/dyncode-compiler.md) | [العربية](../ar/dyncode-compiler.md)

[← Indice documentazione](README.md) · [← Progetto NeverC](project.md)

# NeverC Compilatore dyncode

Compila sorgenti C direttamente in dyncode binario piatto **indipendente dalla posizione, zero rilocazioni, senza sezione dati**.

## Guide

- [Tutorial assembly ARM64 (AArch64) — Prospettiva DynCode](dyncode-compiler-arm64-assembly-tutorial.md)
- [Architettura multipiattaforma NeverC DynCode — Panoramica](dyncode-compiler-cross-platform-architecture.md)
- [Progettazione dei pass IR — Principi, pipeline ed esempi prima/dopo](dyncode-compiler-ir-pass-design.md)
- [Supporto dyncode modalità kernel (Ring-0)](dyncode-compiler-kernel-mode-dyncode.md)
- [Progettazione dei pass MIR — Principi e punti interpose](dyncode-compiler-mir-pass-design.md)
- [Pipeline DynCode, MIR e Strategia PIC (Note di progettazione)](dyncode-compiler-pipeline-and-pic.md)
- [Guida all'estensione della piattaforma](dyncode-compiler-platform-extension-guide.md)
- [Compilatore dyncode — Tracciamento progressi](dyncode-compiler-progress.md)
- [Roadmap](dyncode-compiler-roadmap.md)

---

## Obiettivi principali

1. **Scrivi C normale** — nessun trucco specifico per dyncode.
...
4. **Opzioni CLI via TableGen** — ogni `-fdyncode-*` è registrato in [`neverc/include/neverc/Invoke/Options.td.h`] (nessun confronto di stringhe hardcoded). Errori di battitura → did-you-mean; `--help` elenca tutto.
5. **Vincoli di output verificabili** — `-fdyncode-bad-bytes=` / `-fdyncode-bad-byte-profile=` analizzano il `.bin` finale dopo il interpose post-extract e rifiutano l'output in caso di byte vietato, con offset, byte e contesto.
...
| Opzione | Descrizione |
|--------|-------------|
| `-fdyncode` | Attiva la modalità compilazione dyncode. |
| `-fno-dyncode` | Annulla un `-fdyncode` precedente. |
| `-fdyncode-all-blr` | Modalità aggressiva: chiamate dirette rese indirette in `blr xN` / `call *rax`, elimina tutte le reloc di salto relative. Non necessaria in uso normale. |
| `-mdyncode-syscall` | Abilita esplicitamente stub syscall (predefinito con `-fdyncode` su Darwin/Linux/Android; intento o compat script). |
| `-mdyncode-libsystem` | Alias legacy Darwin di `-mdyncode-syscall`. |
| `-mdyncode-win-peb-import` | Abilita esplicitamente import PEB Windows (predefinito con `-fdyncode` + triple Windows). |
| `-fdyncode-keep-obj=<path>` | Copia l'oggetto intermedio in `<path>` per audit con disassemblatore nativo. |
| `-fdyncode-entry=<name>` | Sostituisce il simbolo di ingresso predefinito (`main`, `_main`, `dyncode_entry`, `_dyncode_entry`). |
| `-fdyncode-bad-bytes=<hex-list>` | Elenco di byte vietati separati da virgole. Scansione del `.bin` finale dopo post-extract; errore senza file scritto se hit. |
| `-fdyncode-bad-byte-profile=<name>` | Profili integrati : `null`, `c-string`, `http-newline`, `line`, `whitespace`, `ascii-control`. Combinabile con `-fdyncode-bad-bytes=`. |
| `-fdyncode-obfuscate=<spec>` | Passa agli interpose plugin **livello IR** via l'[API Plugin](plugin-api.md). No-op senza plugin caricato. Vedi [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes). |
| `-fdyncode-mir-obfuscate=<spec>` | Passa agli interpose **livello MIR** (`RunBeforePreEmit` / `RunAfterPreEmit`). Predefinito `-fdyncode-obfuscate=` se non impostato. Vedi [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-interpose-di-offuscamento-utente). |
...
[`neverc/include/neverc/DynCode/Pipeline/TargetDesc.h`] definisce `TargetDesc` per ogni combinazione (OS, arch) :
...
| libc write/exit | `write(1, s, 3);` | Sì (con `-mdyncode-syscall`) | Syscall wrapper |
| POSIX includes | `#include <unistd.h>` | Sì (modalità dyncode passa automaticamente allo shim) | Driver injects `__NEVERC_DYNCODE__` |
| Win32 API | `WriteFile(h, buf, n, &w, 0);` | Sì (con `-mdyncode-win-peb-import`) | PEB-walk thunk |
| Windows SDK includes | `#include <windows.h>` | Sì (modalità dyncode passa automaticamente allo shim) | Lightweight shim headers |
| Custom entry name | `int dyncode_main(...)` | Sì (con `-fdyncode-entry=...`) | Driver pass-through |
...
│   │   ├── MmapABI.h                          # Costanti mmap condivise (prot/flags)
│   │   ├── DynCodeIRHelpers.h               # Utilità IR comuni (getSizeType, ecc.)
...
## Offuscamento ed estensione tramite plugin

La pipeline dyncode di per sé garantisce solo che «il codice funzioni correttamente». Offuscamento, polimorfismo, encoder a stadi e funzionalità simili dello strato strategico **non sono integrati di proposito** — sono forniti da plugin fuori dall'albero tramite l'[API Plugin](plugin-api.md).

La pipeline espone **11 punti di aggancio** su tre livelli, tutti accessibili tramite l'API C Plugin (`NEVERC_INTERPOSE_SC_*`):

**Livello IR (6 interpose)**:
...
**Livello flusso byte (2 interpose)**:
...
Consulta la [documentazione dell'API Plugin](plugin-api.md) per l'elenco completo degli interpose, la registrazione dei pass ed esempi di codice.

- IR-level: [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes).
- MIR-level: [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-interpose-di-offuscamento-utente)
---
...
- **La whitelist di funzioni esterne** copre solo syscall comuni Darwin BSD / Linux / Android (~80+) + API Win32 (~190). stdio e interfacce runtime pesanti non sono incluse — lo dyncode non può incorporare l'intera macchina a stati di stdio.
