...
x64 运行时探测会先执行 Microsoft 映像，再执行两个 NeverC 候选映像。它使用 `GetProcAddress` 解析导出函数，通过 `CallEnclave` 重复调用，覆盖受 CFG 保护和旧式的间接调用路径，并检查返回值。`PASS` 要求完整且顺序正确的生命周期阶段证据，包括成功终止和释放；仅加载和初始化成功并不够。在此工作流中，ARM64 的覆盖范围是静态验证和差分链接；常规运行时探测仍仅针对 x64。

2026-09-11，单独进行的原生 Windows 11 ARM64 验收已通过（[运行记录 34554067852](https://github.com/NeverSight/NeverC/actions/runs/34554067852)）。该次验收复用了从 `c46e3a4cc4b3732823a7241a43bca6c44b0f0b74` 构建的两个 NeverC ARM64 DLL，并与新构建的 MSVC 参考映像一同测试。48 个生命周期阶段和 12 次带结果校验的调用全部通过，enclave 终止、释放及签名证书清理也均成功。这验证的是这批固定产物，不是新构建的 NeverC 编译器，也不代表已建立永久 ARM64 运行时 CI 门禁。
