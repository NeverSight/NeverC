...
- **IR 钩子**：通过 [Plugin API](plugin-api.md) 提供 6 个 IR 钩子点（`NEVERC_INTERPOSE_SC_BEFORE_PREP` 至 `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`）。`NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` 是真正的最后 IR 维度可注入点——注册在此的 pass 之后没有后续 pass 能修改其输出。共 11 个钩子（6 IR + 3 MIR + 2 字节流）。
...
详见 [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md)。
