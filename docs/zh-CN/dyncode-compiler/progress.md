...
- [x] 目录与 CMake 骨架（`nevercDynCode` 库）
...
**测试**：108/108 dyncode 断言通过。二进制大小：`add` 8B、`fib` 64B、`hello` 64B、`big_const` 632B。

## 阶段 1 — Linux / Android / Windows 跨平台（已交付）

- [x] `TargetDesc` 抽象：表驱动的平台差异
- [x] 跨平台 `-mdyncode-syscall` 语义（替代仅 Darwin 的 `-mdyncode-libsystem`）
- [x] Linux / Android 系统调用号表（Darwin BSD 100+、Linux arm64 130+、Linux x86_64 150+）
- [x] `DynCodeExtractor` 重构为 `MachOExtractor` / `ELFExtractor` / `COFFExtractor`
...
- [x] DynCode 垫片头文件：`<windows.h>`、`<unistd.h>`、`<fcntl.h>`、`<sys/stat.h>`、`<sys/mman.h>`、`<string.h>`、`<stdlib.h>`
...
**测试**：743+ dyncode 断言，全部通过（覆盖 8 个 triple）。NeverC 总测试套件：1000+ 测试通过。

## 阶段 2 — 可打印 / 字母数字编码器（计划中）

- [ ] ARM64 可打印 dyncode 编码器（0x20–0x7e 指令子集）
- [ ] x86_64 字母数字编码器
