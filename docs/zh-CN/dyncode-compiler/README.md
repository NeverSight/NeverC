...
- [ARM64 (AArch64) 汇编教程 — DynCode 视角](arm64-assembly-tutorial.md)
- [NeverC DynCode 跨平台架构概览](cross-platform-architecture.md)
- [IR Pass 设计 — 原则、流水线与前后对比](ir-pass-design.md)
- [内核模式（Ring-0）DynCode 支持](kernel-mode-dyncode.md)
- [MIR Pass 设计 — 原则与钩子点](mir-pass-design.md)
- [DynCode 流水线、MIR 与 PIC 策略（设计笔记）](pipeline-and-pic.md)
- [平台扩展指南](platform-extension-guide.md)
- [DynCode 编译器 — 进度追踪](progress.md)
- [路线图](roadmap.md)
...
全部 8 个 (OS, arch) triple 由**同一套 pass** 驱动。差异隔离在 `TargetDesc.cpp` 表项与三个提取器架构分支。新增平台 = 多填一行表 + 各提取器多一个 case。`ExecutionLevel` 维度正交：`User` 走用户态 syscall / PEB 流水线；`Kernel` 禁用二者并注入 `KernelImportPass`，通过 resolver shim 重写 extern 调用。见 [kernel-mode-dyncode.md](kernel-mode-dyncode.md)。
...
| `-fdyncode-obfuscate=<spec>` | 传递给通过 [Plugin API](../plugin-api/README.md) 注册的 **IR 级**插件钩子。未加载插件时为 no-op。见 [ir-pass-design.md 第 9 节](ir-pass-design.md#9-混淆钩子)。 |
| `-fdyncode-mir-obfuscate=<spec>` | 传递给 **MIR 级**混淆钩子（`RunBeforePreEmit` / `RunAfterPreEmit`）。未设置时回退到 `-fdyncode-obfuscate=`。见 [mir-pass-design.md 第 3 节](mir-pass-design.md#3-用户混淆钩子)（User Obfuscation Interposes）。 |
...
DynCode 流水线本身只保证「代码能正确运行」。混淆、多态、分阶段编码器等策略层功能**故意不内置**——它们通过 [Plugin API](../plugin-api/README.md) 以树外插件的形式提供。
...
详见 [Plugin API 文档](../plugin-api/README.md)了解完整钩子列表、pass 注册和代码示例。

- IR 层设计：[ir-pass-design.md 第 9 节](ir-pass-design.md#9-混淆钩子)。
- MIR 层设计：[mir-pass-design.md 第 3 节](mir-pass-design.md#3-用户混淆钩子)。
