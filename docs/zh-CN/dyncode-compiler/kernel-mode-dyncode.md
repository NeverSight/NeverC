...
## 1. 核心开关：`-mdyncode-context={user,kernel}`
...
1. **IR 层**：pass 输出诊断
2. **MIR 层**：`DynCodeMIRPrepPass::auditExternalReferences` 重审计
3. **提取器**：拒绝所有未解析重定位
...
|------|-------------|-------------|
| 用户态 | `<windows.h>` / `<unistd.h>` / 等 | `<neverc/dyncode/kernel.h>` |
| 内核态 | `<neverc/dyncode/kernel.h>` / `<string.h>` / `<stdlib.h>` / `<stddef.h>` / `<stdint.h>` | `<windows.h>` / `<unistd.h>` / 等 |
