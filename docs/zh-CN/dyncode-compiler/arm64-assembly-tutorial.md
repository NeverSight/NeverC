...
**为什么 dyncode 必须避免 `bl`**：`bl` 使用 26 位立即偏移，由链接器填充。对外部符号会产生 `ARM64_RELOC_BRANCH26` 重定位。dyncode 没有链接器，所以必须替换为 `blr`。
...
整个指令流 100% 在 `__TEXT,__text` 内。`.o` 重定位表除段内分支外为空 — 这就是"真正的 dyncode"。
