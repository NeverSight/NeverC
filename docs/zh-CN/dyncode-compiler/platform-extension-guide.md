...
| `DriverInjectFlags` | dyncode 模式下注入的平台特定标志 | null → 不注入 |
...
2. 在 `lib/Headers/windows.h` 的 dyncode 分支中添加声明
3. 无需修改 pass
...
2. 在 `DynCodeExtractor.cpp` 的分发 switch 中添加一个 case
...
- **ABI 差异**：Win64（rcx/rdx/r8/r9）与 System V AMD64（rdi/rsi/rdx/rcx/r8/r9）有完全不同的参数寄存器。这在 NeverC 前端层处理；dyncode 流水线无需关心。
...
dyncode 流水线通过 `Pipeline.h::ObfuscationInterposes` 暴露 11 个钩子供第三方混淆库使用：
...
// 通过 Plugin API 注册：NEVERC_INTERPOSE_SC_* 钩子（见 plugin-api 文档）
```
