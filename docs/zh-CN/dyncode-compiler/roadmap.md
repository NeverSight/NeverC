**语言**: [English](../../dyncode-compiler/roadmap.md) | [简体中文](roadmap.md) | [繁體中文](../../zh-TW/dyncode-compiler/roadmap.md) | [日本語](../../ja/dyncode-compiler/roadmap.md) | [한국어](../../ko/dyncode-compiler/roadmap.md) | [Français](../../fr/dyncode-compiler/roadmap.md) | [Deutsch](../../de/dyncode-compiler/roadmap.md) | [Español](../../es/dyncode-compiler/roadmap.md) | [Italiano](../../it/dyncode-compiler/roadmap.md) | [Русский](../../ru/dyncode-compiler/roadmap.md) | [العربية](../../ar/dyncode-compiler/roadmap.md)
...
- 大小 / 对齐 / 填充约束（`-fdyncode-max-length=`、`-fdyncode-align=`、`-fdyncode-pad=`）
- IR、MIR 和字节流三层共 11 个混淆钩子

## 已完成（2026-04）

1. **大小 / 对齐 / 填充约束** — 内置功能。`-fdyncode-max-length=`、`-fdyncode-align=`、`-fdyncode-pad=` 在 `finalizeDynCodeBytes` 末尾执行。驱动会拒绝矛盾的配置（例如填充字节在坏字节集中，或无 align/max-length 时使用 pad）。

2. **树外 C 插件 API** — 纯 C ABI 插件接口（[`NevercPluginAPI.h`]），用于自定义 IR、MIR、Binary 和 Linker pass。插件通过 11 个 dyncode 钩子点（`NEVERC_INTERPOSE_SC_*`）注册。单头文件 SDK，零 LLVM/CRT 依赖。详见 [Plugin API 文档](../plugin-api/README.md)。
...
- `RunBeforePrep` — 在任何 dyncode pass 之前
- `RunAfterPrep` — 链接统一之后
...
每个提取器在写入 `.bin` 之前调用 `finalizeDynCodeBytes`：
...
用法和代码示例见 [Plugin API 文档](../plugin-api/README.md)。
