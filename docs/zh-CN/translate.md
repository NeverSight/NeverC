...
Core v2 支持命名空间别名，以及命名空间和代码块内的 using 指令与已解析的 using 声明。导入的函数、变量、类型和无作用域枚举值保留原有身份、名称查找规则、访问检查及对象生命周期。类成员导入、继承构造函数、模板、内联命名空间及 C++20 枚举导入尚未在此阶段支持；完整 C++／STL 仍未完成。
