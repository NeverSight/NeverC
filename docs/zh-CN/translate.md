...
core v2 还增加了固定长度的局部数组与数组字段、多维下标访问，以及数组指针和引用。部分初始化将省略的标量元素置零，记录元素按其选定的初始化方式处理；元素初始化保留源码顺序与别名关系。数组长度与初始化展开量有上限。全局数组、变长数组及元素的非平凡析构仍未支持。
...
Core v2 还支持具有标准布局、平凡复制和析构的记录类型的普通用户构造函数。局部对象、记录字段与数组元素直接在最终存储位置构造，字段按声明顺序初始化。显式 default 的构造函数、委托构造、用户定义的复制／移动构造和析构、清理及异常仍未支持。

Core v2 为每个按值传递的记录参数创建独立对象，并将记录返回值直接写入调用者的目标位置。构造函数和成员函数采用相同规则；源代码要求的复制仍会执行，引用仍保留别名。这是 NeverC 对已接纳平凡记录类型选择的行为，并非 C++17 对所有实现的地址保证。非平凡复制／移动、析构、清理和完整 STL 仍在开发中。
