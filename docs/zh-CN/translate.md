...
Core v2 支持对已支持的固定数组和源文件中定义的范围执行 C++17 范围 `for`，通过已解析的成员函数或 ADL 查找调用 `begin/end`。值或引用循环变量、记录类型迭代器及不同类型的结束迭代器遵循普通调用和生命周期规则。范围初始化及 `begin/end` 只执行一次；每次迭代的对象在递增或退出前析构，覆盖 `continue`、`break` 和 `return`。模板、标准头文件、STL 容器、结构化绑定和 C++20 范围初始化语句尚不在本次支持范围内。原生验证以实现版本的 CI 为准。
...
Core v2 支持具名非模板嵌套记录，包括通过合法别名或工厂函数暴露的私有/受保护类型。每个嵌套对象保留自己的存储和接收者；显式外层引用、作用域类型身份、成员/数组复制与移动、析构及嵌套迭代器沿用普通语义。按值依赖类型先输出。匿名嵌套记录、模板、继承及完整 STL 仍待实现；原生验证以实现版本的 CI 为准。
...
Core v2 支持内联命名空间、命名空间别名，以及命名空间和代码块内的 using 指令与已解析的 using 声明。导入的函数、变量、类型和无作用域枚举值保留原有身份、名称查找规则、访问检查及对象生命周期。内联命名空间保留与父命名空间合并的查找及双向实参依赖查找，并支持嵌套和重开。类成员导入、继承构造函数、模板、C++20 枚举导入及显式嵌套 inline 语法尚未在此阶段支持；完整 C++／STL 仍未完成。

Core v2 支持已解析、非模板的 C++17 if constexpr，仅为选中的分支生成运行时代码，并保留初始化语句、条件变量存储及作用域清理。两个源分支仍会接受检查；当前配置仍要求已声明的函数和全局变量具有定义，即使只在被丢弃分支中使用。依赖模板的分支和 C++23 if consteval 尚未支持；完整 C++／STL 仍未完成。
