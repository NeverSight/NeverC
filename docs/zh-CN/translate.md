...
Core v2 现支持普通自动局部引用延长临时对象的生命周期，包括 `const T& r{T{...}}` 和 `T&& r = T{...}`。标量、枚举、指针和记录临时值使用实际存储；C++ 规定延长生命周期时，指向成员或数组元素的引用会保留完整记录对象。花括号及等号加花括号初始化保留同一个对象。对象在引用所属作用域结束时析构，覆盖条件、循环和提前退出；初始化器内其他临时对象仍在各自完整表达式结束时清理。别名不增加所有者，后续复制或移动保留选定操作。静态／全局／线程局部引用、引用字段、独立数组临时值、模板和完整 STL 仍在开发中。
