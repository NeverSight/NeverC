...
Core v2 还支持隐式及显式 default 的复制赋值。成员和数组元素保留选定的赋值操作及其顺序，生成的平凡数组复制转为有类型的元素赋值，无需外部内存复制调用。平凡赋值保留指针字段原值并返回实际接收对象。运算符语法先捕获源引用，显式成员调用先捕获接收对象；源值在这些副作用完成后读取，赋值不会额外构造或析构对象。默认成员初始化器、移动操作、模板与完整 STL 仍在开发中。
