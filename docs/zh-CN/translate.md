...
Core v2 支持普通成员及自由重载运算符，包括算术、比较、下标、解引用、自增、函数对象与一般赋值签名。调用保留选定函数、引用别名和对象结果。运算符语法遵循 C++17 求值顺序，重载逻辑运算符会求值两个操作数。自由赋值运算符统一从右向左初始化参数，再按逆序析构。转换函数、模板、内存分配和完整 STL 仍在开发中。
