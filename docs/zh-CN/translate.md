...
Core v2 支持具有定义的整数、布尔及枚举静态数据成员，包括 inline/constexpr 与类外定义，可采用常量或零初始化。所有实例共享同一份带类型存储。接收对象的副作用与临时对象清理得到保留；静态成员引用不随临时接收对象销毁而失效。目前即使只读取非 inline 常量的值，也要求当前源文件中有定义。动态初始化、其他静态类型及完整 STL 仍待实现；原生验证以实现版本的 CI 为准。
