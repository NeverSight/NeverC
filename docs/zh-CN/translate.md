...
Core v2 支持在其他条件均受支持的标准布局类中使用 `private/protected` 数据成员。内置 Clang 在翻译前检查访问权限；合法的成员函数、构造函数、工厂函数、默认参数和生成的复制/移动操作使用同一份带类型的成员存储。类外非法访问仍报告 C++ 诊断。访问标签属于源语言规则，不提供运行时保密能力。混合访问级别的非标准布局类、模板友元、继承、嵌套记录和不支持的字段类型仍受限制。原生结果以实现版本的 CI 为准。
