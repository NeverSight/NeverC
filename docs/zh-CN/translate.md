...
Core v2 还支持隐式及显式 default 移动构造和移动赋值，包括嵌套记录与多维数组。成员按声明和元素顺序执行 C++ 选中的复制或移动。生成的移动不重跑默认成员初始化器，平凡操作保留已存储的指针值。源对象仍正常析构，未使用或平凡的生成操作不需要虚构函数体。已有内联隐式平凡操作保留其临时对象行为；普通临时引用绑定仍未开放。完整 C++／STL 支持仍在推进。
...
Core v2 也支持现存对象上的普通转换函数：隐式与显式整数、枚举和指针转换，条件中的显式 `bool`，以及引用或对象结果。每次转换执行一次选定的成员函数。引用保留别名；对象纯右值直接初始化实际目标，从引用初始化值则保留选定的复制或移动。const／引用限定、constexpr 与 noexcept 遵循 C++17 选择规则。临时引用的生命周期、模板和完整 STL 仍在开发中。

Core v2 现支持临时对象接收者，以及存活到整条表达式结束的标量／记录临时引用实参，覆盖构造函数、方法、运算符和转换函数。每次求值都有实际存储，对象结果保留最终目标，引用保留别名。调用及被调用函数的参数析构完成后，临时对象按构造逆序清理，并正确处理条件分支和循环中的重复求值。临时记录对象的数组子对象也受支持。独立数组临时值、自动／静态引用的生命周期延长、模板和完整 STL 仍在开发中。
