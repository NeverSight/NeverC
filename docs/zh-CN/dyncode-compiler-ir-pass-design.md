...
| DynCodeExtractor | 扫描 `.o` 进行最终审计 + 输出 flat `.bin` | 后处理 |
...
- `_Thread_local` → 静默降级为 static（dyncode 在宿主的调用线程上运行）
...
已解析的 API 地址在存入缓存全局变量前经过加密，防止简单的内存扫描在 dyncode 地址空间中发现已解析的函数指针。
...
由 `-fdyncode-all-blr` 激活。将剩余模块内直接调用转为通过 volatile 槽 + `blr xN` / `call *rax` 的间接调用，消除所有相对分支重定位。正常使用不需要，因为提取器会修补段内分支。

---

## 9. 混淆钩子

DynCode 为第三方混淆 pass 提供 11 个混淆钩子点（6 个 IR 级、3 个 MIR 级、2 个字节级）。完整清单详见 [mir-pass-design.md 第 3 节](dyncode-compiler-mir-pass-design.md#3-用户混淆钩子)。

关键设计原则：
- **注册位置是契约**：越早注册越有更广的内置 PIC 修复覆盖
- **钩子不解释 spec 字符串**：`-fdyncode-obfuscate=<spec>` 透传给 `DynCodeOptions::ObfuscateSpec`；混淆库定义自有 DSL
...
当 `-mdyncode-context=kernel` 时激活。自动重写未解析的 extern 直接调用为带加密地址缓存的解析器间接调用：
...
处理 NeverC 的内置 `string` 类型方法。dyncode 模式下使用 `string` 值时，此 pass 将堆分配的运行时调用改写为栈分配的 arena 变体，确保零外部依赖。

---

## 13. HeapArenaPass

将 dyncode 中的 `malloc`/`free`/`calloc`/`realloc` 调用改写为混合分配策略（默认开启，通过 `-fdyncode-heap-arena` / `-fno-dyncode-heap-arena` 控制）：
...
每个硬错误产出恰好**一条可操作诊断**。模块级 `__neverc_dyncode_hard_error` 元数据哨兵防止级联噪声：一旦 `reportError` 设置标志，后续阶段（ZeroReloc Stackify 验证、提取器审计）静默提前返回。
