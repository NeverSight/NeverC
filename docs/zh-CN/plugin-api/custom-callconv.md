...
如果插件注册了自己的目标，也可以在其 `NevercCallingConventionDescriptor` 上提供 `PlanCallingConvention` 回调直接产出 plan，跳过 spec 这一层。参见[目标、MC、汇编与目标文件](target-mc-object.md#abi-与调用约定)。
