...
当 `-fdyncode` 活跃时，编译器遵循此固定顺序。**每个阶段都有对应的混淆钩子**用于第三方 pass 插入：
...
2. **dyncode 没有外部重定位和数据段**。每个 pass 消除一类"需要加载器"的引用；提取器执行字节级兜底审计。
...
- 普通（非 dyncode）编译也使用 PIC，确保行为一致性。

## 4. User / Kernel 正交维度

ExecutionLevel 与 (OS, arch) 正交：

- **User**（`-mdyncode-context=user`，默认）：经典 ring-3 载荷
...
- **Kernel**（`-mdyncode-context=kernel`）：ring-0 载荷
  ...
  - 用户仍只写普通 C。`<neverc/dyncode/kernel.h>` 提供 `neverc_kern_resolve_t` 和 `neverc_kern_hash()`。

## 5. 用户态"普通 C"支持矩阵

使用 `-fdyncode` 时，以下常见 C 模式**无需用户感知即直接支持**：
...
| POSIX 头文件 | `<unistd.h>` / `<fcntl.h>` | DynCode shim 头 |
| Win32 头文件 | `<windows.h>` | DynCode shim 头 |
...
2. **DynCode 友好指令重写**：`MIRRewritePatterns.def` + `MIRRewriteOpcodes.def` 表驱动。
...
11 个 `ObfuscationInterposes` 字段（6 IR + 3 MIR + 2 字节级）覆盖每个流水线阶段：
