...
[文档索引](README.md) · [DynCode 指南](dyncode-compiler/README.md) · [内置运行时](builtins/README.md) · [插件 API](plugin-api/README.md) · [路线图](roadmap.md)

</div>

---

> **说明：** GitHub 仓库首页固定展示英文 `README.md`，不会根据浏览器语言自动切换。请用上方语言链接进入对应版本；进入 [文档](README.md) 或 [dyncode 指南](dyncode-compiler/README.md) 后，请继续通过页内语言栏与面包屑保持同一语言。
...
- **零门槛可扩展** — 单个 C 头文件、130 个具名编译阶段，就能写出[编译器插件](plugin-api/README.md)，介入从 IR 优化到最终产物输出的任何阶段——不需要懂 LLVM。
- **安全研究开箱即用** — DynCode 编译、编译期字符串加密、跨平台 PE 生成全部原生集成在编译器中——不需要靠外部脚本拼凑。

## 特性

- **[DynCode 编译器](dyncode-compiler/README.md)** — 多阶段 IR/MIR 流水线、跨平台提取、导入/系统调用降级、内核模式、坏字节审计与插件架构
- **集成链接器** — 单一二进制内完成 COFF、ELF、Mach-O 链接，无需外部 `ld` 或 `link.exe`
- **[发布剥离](release-builds.md)** — 内置 `--strip` / `-s`，从最终 ELF、Mach-O 与 PE/COFF 映像中删除非运行时符号和源码级调试信息，并对 `.ko` 进行内核感知的结构化符号重命名（不是 hash，也不是 encryption）
- **交叉编译** — 从任意宿主构建 Windows PE、Linux ELF、macOS Mach-O 和 Android ELF，内置各平台 SDK
- **[内置运行时](builtins/README.md)** — 嵌入编译器的 LLVM bitcode 运行时：[`string`](builtins/string.md)（值语义字符串，自动内存管理）、[`mimalloc`](builtins/mimalloc.md)（透明高性能分配器覆盖，内核与 freestanding 目标之外默认开启）、[`xorstr`](builtins/xorstr.md)（逐实例编译期加密、强制 late 封口与逐调用点原生展开）和 [`strhash`](builtins/strhash.md)（编译期字符串哈希，与运行时算法一致）
- **[插件 API](plugin-api/README.md)** — 纯 C ABI 的树外插件接口；单头文件 SDK，零 LLVM/CRT 依赖，覆盖驱动、预处理、AST、IR、MIR、MC、目标文件、链接、LTO、dyncode 各阶段
- **[`.nc` 扩展名](nc-extension.md)** — 使用 `.nc` 文件扩展名自动启用所有 NeverC 功能（`string`、Rust 风格整数类型），无需额外标志
...
> **说明：** 内置 **`string`** 在 `.c` 文件中需要显式加 **`-fbuiltin-string`**。使用 [**`.nc` 文件**](nc-extension.md) 或 **`-fdyncode`** 模式时自动启用。
...
详细设计说明、平台矩阵、CLI 参考与示例见 **[文档索引](README.md)**。更多完整可构建示例见 **[examples](examples.md)**。
...
完整命令说明见 [`neverc runtime` →](runtime.md) · [`neverc update` →](update.md) · [`neverc build` / `make` →](build.md)。

## 从源码构建

构建依赖、构建命令、Windows 交叉编译、PATH 设置，以及在 release 安装与本地源码构建之间切换，详见 **[本地开发](local-dev.md)**。
...
复制或改编代码时，包括借助 AI/LLM 或在 LLVM 相关项目中复用，必须遵守适用的许可证，并保留其要求的版权、许可证和署名声明。此外，我们请求以 NeverC 为参考的使用者注明项目及原始出处；这是项目的引用请求，不是额外的许可条件。

请参阅[声明](../../NOTICE)、[署名与引用指南](attribution.md)和[机器可读的引用信息](../../CITATION.cff)。
