...
- [ARM64 (AArch64) 汇编教程 — DynCode 视角](dyncode-compiler-arm64-assembly-tutorial.md)
- [NeverC DynCode 跨平台架构概览](dyncode-compiler-cross-platform-architecture.md)
- [IR Pass 设计 — 原则、流水线与前后对比](dyncode-compiler-ir-pass-design.md)
- [内核模式（Ring-0）DynCode 支持](dyncode-compiler-kernel-mode-dyncode.md)
- [MIR Pass 设计 — 原则与钩子点](dyncode-compiler-mir-pass-design.md)
- [DynCode 流水线、MIR 与 PIC 策略（设计笔记）](dyncode-compiler-pipeline-and-pic.md)
- [平台扩展指南](dyncode-compiler-platform-extension-guide.md)
- [DynCode 编译器 — 进度追踪](dyncode-compiler-progress.md)
- [路线图](dyncode-compiler-roadmap.md)
...
全部 8 个 (OS, arch) triple 由**同一套 pass** 驱动。差异隔离在 `TargetDesc.cpp` 表项与三个提取器架构分支。新增平台 = 多填一行表 + 各提取器多一个 case。`ExecutionLevel` 维度正交：`User` 走用户态 syscall / PEB 流水线；`Kernel` 禁用二者并注入 `KernelImportPass`，通过 resolver shim 重写 extern 调用。见 [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md)。
...
| `-fdyncode-obfuscate=<spec>` | 传递给通过 [Plugin API](plugin-api.md) 注册的 **IR 级**插件钩子。未加载插件时为 no-op。见 [ir-pass-design.md 第 9 节](dyncode-compiler-ir-pass-design.md#9-混淆钩子)。 |
| `-fdyncode-mir-obfuscate=<spec>` | 传递给 **MIR 级**混淆钩子（`RunBeforePreEmit` / `RunAfterPreEmit`）。未设置时回退到 `-fdyncode-obfuscate=`。见 [mir-pass-design.md 第 3 节](dyncode-compiler-mir-pass-design.md#3-用户混淆钩子)（User Obfuscation Interposes）。 |
...
│   │   ├── MmapABI.h                          # 共享 mmap 常量 (prot/flags)
...
DynCode 流水线本身只保证「代码能正确运行」。混淆、多态、分阶段编码器等策略层功能**故意不内置**——它们通过 [Plugin API](plugin-api.md) 以树外插件的形式提供。

流水线暴露 **11 个钩子点**，全部通过 C Plugin API（`NEVERC_INTERPOSE_SC_*`）访问：

**IR 层（6 个钩子）**：
- `NEVERC_INTERPOSE_SC_BEFORE_PREP` — 任何 dyncode pass 之前
- `NEVERC_INTERPOSE_SC_AFTER_PREP` — 链接属性已统一（internal + always_inline）
- `NEVERC_INTERPOSE_SC_BEFORE_INLINING` — AlwaysInliner 前最后机会
- `NEVERC_INTERPOSE_SC_AFTER_INLINING` — IR 已压缩为单一大函数
- `NEVERC_INTERPOSE_SC_AFTER_STACKIFY` — 最终 IR 形态，下一步为代码生成
- `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` — AllBlrPass 之后，真正的最后 IR 钩子

**MIR 层（3 个钩子）**：
- `NEVERC_INTERPOSE_SC_BEFORE_PREEMIT` — 寄存器已分配，**CFI/EH 伪指令仍在**
- `NEVERC_INTERPOSE_SC_AFTER_PREEMIT` — **内置 MIRPrepPass 已剥离伪指令**，最接近 AsmPrinter 将看到的字节形态；适合指令级混淆/寄存器重命名
- `NEVERC_INTERPOSE_SC_AFTER_FINAL_MIR` — 真正的最后 MIR 钩子，在 LLVM `addPreEmitPass2()` 之后、AsmPrinter 之前

**字节流层（2 个钩子）**：
- `NEVERC_INTERPOSE_SC_POST_EXTRACT` — 提取器完成段内重定位修补与数据段审计之后、写入 `.bin` 之前。用于整包加密、垃圾字节插入或自定义头。
- `NEVERC_INTERPOSE_SC_POST_FINALIZE` — 全部 finalize 步骤之后；NeverC 不再审计。

详见 [Plugin API 文档](plugin-api.md)了解完整钩子列表、pass 注册和代码示例。

- IR 层设计：[ir-pass-design.md 第 9 节](dyncode-compiler-ir-pass-design.md#9-混淆钩子)。
- MIR 层设计：[mir-pass-design.md 第 3 节](dyncode-compiler-mir-pass-design.md#3-用户混淆钩子)。
