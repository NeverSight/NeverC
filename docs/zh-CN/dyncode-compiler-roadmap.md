...
2. **树外 C 插件 API** — 纯 C ABI 插件接口（[`NevercPluginAPI.h`]），用于自定义 IR、MIR、Binary 和 Linker pass。插件通过 11 个 dyncode 钩子点（`NEVERC_INTERPOSE_SC_*`）注册。单头文件 SDK，零 LLVM/CRT 依赖。详见 [Plugin API 文档](plugin-api.md)。
...
用法和代码示例见 [Plugin API 文档](plugin-api.md)。
