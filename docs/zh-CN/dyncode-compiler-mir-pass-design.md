**语言**: [English](../dyncode-compiler-mir-pass-design.md) | [简体中文](dyncode-compiler-mir-pass-design.md) | [繁體中文](../zh-TW/dyncode-compiler-mir-pass-design.md) | [日本語](../ja/dyncode-compiler-mir-pass-design.md) | [한국어](../ko/dyncode-compiler-mir-pass-design.md) | [Français](../fr/dyncode-compiler-mir-pass-design.md) | [Deutsch](../de/dyncode-compiler-mir-pass-design.md) | [Español](../es/dyncode-compiler-mir-pass-design.md) | [Italiano](../it/dyncode-compiler-mir-pass-design.md) | [Русский](../ru/dyncode-compiler-mir-pass-design.md) | [العربية](../ar/dyncode-compiler-mir-pass-design.md)

[← DynCode 编译器](dyncode-compiler.md)

# MIR Pass 设计 — 原则与钩子点

> 与 [ir-pass-design.md](dyncode-compiler-ir-pass-design.md) 配套。IR 层消除在 IR 级别明显产生重定位的构造。MIR 层作为指令选择和寄存器分配后的**兜底层**，剥离代码生成引入的伪指令/元数据指令，并暴露钩子点供第三方混淆 pass 进行最终指令级变换。
>
> 实现：`neverc/lib/DynCode/MIR/MIRPrepPass.cpp` + `Pipeline.cpp`。
> 钩子接口：[`neverc/include/neverc/DynCode/Pipeline/Pipeline.h`]。
...
但 LLVM 后端在 **IR → MIR 降低** 过程中引入了 dyncode 无法容纳的额外构造：
...
回调不捕获 `Opts`。它在运行时读取当前 `DynCodeOptions` 快照，防止同一进程同时编译 dyncode 和普通 C 时使用过期配置。
...
伪指令剥离后，`MIRPrepPass` 运行重写 pass，将代码生成选择的但对 dyncode 不友好的指令模式替换为等价的 dyncode 友好形式，而无需修改 LLVM `.td` 文件。
...
`ObfuscationInterposes` 暴露 **11 个钩子点**：6 个 IR 级、3 个 MIR 级、2 个字节级：
...
static void myMirObfInit() {
  ...
  // 通过 Plugin API 注册：NEVERC_INTERPOSE_SC_BEFORE_PREEMIT / AFTER_PREEMIT / AFTER_FINAL_MIR
}
```
...
[Codegen (IR → MIR)]
  ...
  ├─ DynCodeMIRPrepPass  ← 本文档重点
  └─ RunAfterPreEmit     (用户钩子, CFI 已剥离)
        │
[AsmPrinter → 目标文件]
        │
[DynCodeExtractor]  ← 字节级兜底审计
  ├─ RunPostExtract   (用户钩子, 纯字节)
  └─ flat .bin
```

MIR 层处理**兜底清理 + 混淆钩子点**，而非业务逻辑。"写普通 C，无需 dyncode 技巧"的承诺由 5+ 个 IR pass 实现。
...
**添加内置伪指令剥离**：在 `isDynCodeStripPseudo` switch 中添加一个 case。
...
**第三方混淆库**：通过 [Plugin API](plugin-api.md)（`NEVERC_INTERPOSE_SC_*` 钩子）注册。

## 7. 与 DynCodeExtractor 的关系
...
此原则使 MIR 层几乎不受后端 ISA 升级影响。唯一的维护是："TargetOpcode 中有新伪指令吗？如果 dyncode 不需要它，添加一个 case。"
