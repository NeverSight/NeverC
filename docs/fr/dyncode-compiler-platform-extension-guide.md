**Langues**: [English](../dyncode-compiler-platform-extension-guide.md) | [简体中文](../zh-CN/dyncode-compiler-platform-extension-guide.md) | [繁體中文](../zh-TW/dyncode-compiler-platform-extension-guide.md) | [日本語](../ja/dyncode-compiler-platform-extension-guide.md) | [한국어](../ko/dyncode-compiler-platform-extension-guide.md) | [Français](dyncode-compiler-platform-extension-guide.md) | [Deutsch](../de/dyncode-compiler-platform-extension-guide.md) | [Español](../es/dyncode-compiler-platform-extension-guide.md) | [Italiano](../it/dyncode-compiler-platform-extension-guide.md) | [Русский](../ru/dyncode-compiler-platform-extension-guide.md) | [العربية](../ar/dyncode-compiler-platform-extension-guide.md)

[← Compilateur dyncode](dyncode-compiler.md)

# Guide d'extension de plateforme

Ce document explique comment étendre le compilateur dyncode à de nouvelles plateformes cibles. Actuellement supporté : **arm64 / x86_64 sur macOS / Linux / Android / Windows** (8 triples), chacun avec des contextes indépendants **User** / **Kernel** (16 variantes au total). L'ajout d'une nouvelle plateforme nécessite typiquement quelques centaines de lignes de code.
...
| `DriverInjectFlags` | Flags spécifiques plateforme en mode dyncode | null → pas d'injection |
...
2. Ajouter case dans switch dispatch de `DynCodeExtractor.cpp`
3. Écrire `<Format>Extractor.cpp` (suivant la structure de `ELFExtractor.cpp`)

### 4. Ajouter Loader (outil de test uniquement)

Référence [`tests/neverc/dyncode/loader_linux.c`] et `loader_windows.c`. Typiquement : `mmap(RWX) → memcpy → icache flush → call`.

### 5. Mettre à jour les tests

Ajouter un test de compilation croisée dans [`tests/neverc/DynCodeCrossTargetTests.cpp`].
...
La pipeline dyncode expose 11 interposes via `Pipeline.h::ObfuscationInterposes` pour les bibliothèques d'obfuscation tierces :
...
// Enregistrer via l'API Plugin : interposes NEVERC_INTERPOSE_SC_* (voir docs plugin-api)
```

Utilisation au niveau MIR :
```cpp
...
Le patching MIR intégré est aussi piloté par table : `Tables/MIRRewritePatterns.def` enregistre les noms de diagnostic de motifs, les filtres d'architecture et les noms de helpers ; `Tables/MIRRewriteOpcodes.def` enregistre les noms d'opcodes du backend. Lors de l'ajout de nouvelles formes de backend compatibles dyncode, préférer les entrées de table et les helpers étroits plutôt que de disperser des branches spécifiques à la cible dans le corps de la passe.
