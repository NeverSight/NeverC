...
- **Interposes IR** : 6 points d'accroche IR (`NEVERC_INTERPOSE_SC_BEFORE_PREP` à `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) via l'[API Plugin](plugin-api.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` est le vrai dernier point d'injection IR — aucune passe après. 11 points de interpose total (6 IR + 3 MIR + 2 flux d'octets).
...
Voir [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md) pour les détails.
