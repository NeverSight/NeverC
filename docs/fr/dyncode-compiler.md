**Langues**: [English](../dyncode-compiler.md) | [简体中文](../zh-CN/dyncode-compiler.md) | [繁體中文](../zh-TW/dyncode-compiler.md) | [日本語](../ja/dyncode-compiler.md) | [한국어](../ko/dyncode-compiler.md) | [Français](dyncode-compiler.md) | [Deutsch](../de/dyncode-compiler.md) | [Español](../es/dyncode-compiler.md) | [Italiano](../it/dyncode-compiler.md) | [Русский](../ru/dyncode-compiler.md) | [العربية](../ar/dyncode-compiler.md)

[← Index documentation](README.md) · [← Projet NeverC](project.md)

# NeverC Compilateur dyncode

Compilez du C directement en dyncode binaire plat **indépendant de la position, sans relocalisation, sans section de données**.

## Guides

- [Tutoriel assemblage ARM64 (AArch64) — Perspective DynCode](dyncode-compiler-arm64-assembly-tutorial.md)
- [Architecture multiplateforme NeverC DynCode — Vue d'ensemble](dyncode-compiler-cross-platform-architecture.md)
- [Conception des passes IR — Principes, pipeline et exemples avant/après](dyncode-compiler-ir-pass-design.md)
- [Support dyncode mode noyau (Ring-0)](dyncode-compiler-kernel-mode-dyncode.md)
- [Conception des passes MIR — Principes et points de interpose](dyncode-compiler-mir-pass-design.md)
- [Pipeline DynCode, MIR et Stratégie PIC (Notes de conception)](dyncode-compiler-pipeline-and-pic.md)
- [Guide d'extension de plateforme](dyncode-compiler-platform-extension-guide.md)
- [Compilateur dyncode — Suivi de progression](dyncode-compiler-progress.md)
- [Feuille de route](dyncode-compiler-roadmap.md)

---

## Objectifs principaux

1. **Écrivez du C normal** — aucune astuce spécifique au dyncode.
...
4. **Options CLI via TableGen** — chaque `-fdyncode-*` est enregistré dans [`neverc/include/neverc/Invoke/Options.td.h`] (pas de correspondance de chaînes en dur). Typos → did-you-mean ; `--help` liste tout.
5. **Contraintes de sortie vérifiables** — `-fdyncode-bad-bytes=` / `-fdyncode-bad-byte-profile=` analysent le `.bin` final après le interpose post-extract et rejettent la sortie en cas d'octet interdit, avec offset, octet et contexte.
...
| Option | Description |
|--------|-------------|
| `-fdyncode` | Active le mode compilation dyncode. |
| `-fno-dyncode` | Annule un `-fdyncode` précédent. |
| `-fdyncode-all-blr` | Mode agressif : appels directs indirectisés en `blr xN` / `call *rax`, supprime toutes les relocs de branche relative. Inutile en usage normal. |
| `-mdyncode-syscall` | Active explicitement les stubs syscall (défaut avec `-fdyncode` sur Darwin/Linux/Android ; intention ou compat scripts). |
| `-mdyncode-libsystem` | Alias legacy Darwin de `-mdyncode-syscall`. |
| `-mdyncode-win-peb-import` | Active explicitement l'import PEB Windows (défaut avec `-fdyncode` + triple Windows). |
| `-fdyncode-keep-obj=<path>` | Copie l'objet intermédiaire vers `<path>` pour audit désassembleur natif. |
| `-fdyncode-entry=<name>` | Remplace le symbole d'entrée par défaut (`main`, `_main`, `dyncode_entry`, `_dyncode_entry`). |
...
| `-fdyncode-obfuscate=<spec>` | Vers les interposes plugin **niveau IR** via l'[API Plugin](plugin-api.md). No-op sans plugin chargé. Voir [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes). |
| `-fdyncode-mir-obfuscate=<spec>` | Vers les interposes **niveau MIR** (`RunBeforePreEmit` / `RunAfterPreEmit`). Par défaut `-fdyncode-obfuscate=`. Voir [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-interposes-dobfuscation-utilisateur). |
...
[`neverc/include/neverc/DynCode/Pipeline/TargetDesc.h`] définit `TargetDesc` pour chaque combinaison (OS, arch) :
...
│   │   ├── MmapABI.h                          # Constantes mmap partagées (prot/flags)
│   │   ├── DynCodeIRHelpers.h               # Utilitaires IR communs (getSizeType, etc.)
...
Le pipeline dyncode lui-même garantit seulement que « le code s'exécute correctement ». L'obfuscation, le polymorphisme, les encodeurs par étapes et autres fonctionnalités de la couche stratégie **ne sont volontairement pas intégrés** — ils sont fournis par des plugins hors arbre via l'[API Plugin](plugin-api.md).
...
Voir la [documentation de l'API Plugin](plugin-api.md) pour la liste complète des interposes, l'enregistrement des passes et des exemples de code.

- IR-level: [ir-pass-design.md §9 — Obfuscation Interposes](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes).
- MIR-level: [mir-pass-design.md §3 — User Obfuscation Interposes](dyncode-compiler-mir-pass-design.md#3-interposes-dobfuscation-utilisateur)
---
