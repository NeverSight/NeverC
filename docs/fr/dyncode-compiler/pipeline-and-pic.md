**Langues**: [English](../../dyncode-compiler/pipeline-and-pic.md) | [简体中文](../../zh-CN/dyncode-compiler/pipeline-and-pic.md) | [繁體中文](../../zh-TW/dyncode-compiler/pipeline-and-pic.md) | [日本語](../../ja/dyncode-compiler/pipeline-and-pic.md) | [한국어](../../ko/dyncode-compiler/pipeline-and-pic.md) | [Français](pipeline-and-pic.md) | [Deutsch](../../de/dyncode-compiler/pipeline-and-pic.md) | [Español](../../es/dyncode-compiler/pipeline-and-pic.md) | [Italiano](../../it/dyncode-compiler/pipeline-and-pic.md) | [Русский](../../ru/dyncode-compiler/pipeline-and-pic.md) | [العربية](../../ar/dyncode-compiler/pipeline-and-pic.md)

[← Compilateur dyncode](README.md)

# Pipeline DynCode, MIR et Stratégie PIC (Notes de conception)

Ce document décrit les compromis de conception dans le mode dyncode de NeverC à travers la chaîne **IR → optimisation LLVM → backend MIR → fichier objet → extraction/correction**, et sa relation avec la politique de **PIC par défaut au niveau compilateur**. Les détails d'implémentation font foi dans le code source et les commentaires en anglais.

## 1. Pourquoi forcer PIC par défaut (y compris la compilation non-dyncode)

L'extracteur de dyncode suppose que les références aux symboles externes dans le fragment exécutable sont des relocalisations **relatives au PC** ou résolubles intra-`.text`, et non des adresses absolues codées en dur ou des pools de constantes nécessitant un chargeur pour remplir `.data`.
...
- La compilation C normale et la compilation `-fdyncode` partagent les mêmes habitudes de relocalisation, réduisant la charge cognitive « fonctionne normalement, casse sous dyncode ».
...
Cette politique ne distingue pas si `-fdyncode` est activé ou si le contexte est user/kernel. Même si l'utilisateur passe `-fno-pic` / `-static` / `-mkernel` / `-mdynamic-no-pic`, `ParsePICArgs()` maintient `Reloc::PIC_`, utilisant les mêmes hypothèses relatives au PC pour la compilation normale, le dyncode mode utilisateur et le dyncode mode noyau.

## 2. Division du travail IR et MIR en deux phases

### 2.1 Couche IR (`registerDynCodePasses`)
...
2. **`DynCodeMIRPrepPass`** : Supprime les pseudos qui généreraient des sections latérales `.eh_frame` / `.pdata` / `.xray_*`, rendant le flux d'instructions aussi proche que possible de « code pur » avant AsmPrinter.
...
**Principe** : Si les séquences d'instructions natives ont encore des problèmes, corriger en MIR (surtout autour de `DynCodeMIRPrepPass`) ; **l'extraction et le patching sont le dernier filet de sécurité**, évitant de dupliquer la même logique dans les couches COFF/ELF/Mach-O.

Les noms d'opcode MIR ne sont pas dispersés dans le flux de contrôle de la passe ; `DynCodeMIRPrepPass` utilise la table `(pattern, role, opcode)` de `Tables/MIRRewriteOpcodes.def` via `TargetInstrInfo::getName()`. Pour les substitutions d'instructions amicales avec dyncode, préférer l'ajout d'entrées de table et de petites réécritures MIR ; ne recourir aux modifications de sélection d'instructions backend `.td` que si nécessaire, le repli au niveau extracteur étant le dernier recours.

> Note : `DynCodeMIRPrepPass` n'est enregistré que lorsque `-fdyncode` est activé. Les programmes normaux ne doivent pas supprimer globalement CFI/EH, cela briserait la gestion normale des exceptions et les informations de débogage.

Les callbacks globaux IR et MIR utilisent tous deux un modèle **enregistrer une fois, lire l'instantané `DynCodeOptions` courant à l'exécution**. Cela supporte les processus compilateur embarqué de longue durée : quand le même processus compile d'abord du dyncode puis du C normal, la compilation C normale n'hérite pas des passes IR/MIR précédentes ; lors de compilations consécutives de plusieurs TUs dyncode, les enregistrements en double ne s'empilent pas.
...
1. **SDK intégré** (par défaut) : NeverC embarque un Windows SDK et WDK complet dans `runtime/`. Les en-têtes sont dans `runtime/windows/shared/`, les bibliothèques par architecture dans `runtime/windows/{x64,arm64}/`. Disposition après build :

   ```
   build-neverc/bin/neverc
   build-neverc/runtime/windows/shared/msvc/  (en-têtes)
   build-neverc/runtime/windows/x64/msvc/     (bibliothèques x64)
   build-neverc/runtime/windows/arm64/msvc/   (bibliothèques arm64)
   ```

2. **Sysroot explicite style VS** (optionnel) : Si vous avez un arbre `VC/Tools/MSVC/<version>/...` + `Windows Kits/10/...`, le pointer via `-vctoolsdir=<path>` ou `-winsysroot=<path>`. Ce chemin a priorité sur le SDK intégré.

Les deux sources fonctionnent sans registre ni variables d'environnement VS fournies par l'OS, permettant la compilation croisée de dyncode Windows depuis macOS / Linux.

## 5. Points d'obfuscation et d'extension

- **Interposes IR** : 6 points d'accroche IR (`NEVERC_INTERPOSE_SC_BEFORE_PREP` à `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR`) via l'[API Plugin](../plugin-api/README.md). `NEVERC_INTERPOSE_SC_AFTER_FINAL_IR` est le vrai dernier point d'injection IR — aucune passe après. 11 points de interpose total (6 IR + 3 MIR + 2 flux d'octets).
- **Obfuscation MIR** : `RunBeforePreEmit` / `RunAfterPreEmit` sont des interposes MIR de granularité moyenne ; `RunAfterFinalMIR` est le **vrai dernier** interpose MIR. `-fdyncode-mir-obfuscate=` spécifie le spec MIR séparément ; utilise le spec IR par défaut si non défini.
- **Interposes de flux d'octets** : `RunPostExtract` est le interpose **pré**-finalize ; `RunPostFinalize` est le interpose **post**-finalize (dernier moment avant écriture sur disque).
- **Taille / alignement / remplissage** : `-fdyncode-max-length=`, `-fdyncode-align=`, `-fdyncode-pad=` s'exécutent en fin de finalize ; le driver rejette les configurations contradictoires.
...
Le mode dyncode introduit `-mdyncode-context=user|kernel` comme seconde dimension du pipeline, superposée au triple :
...
- **Mode noyau** :
  ...
  - `<neverc/dyncode/kernel.h>` expose `neverc_kern_resolve_t`, `neverc_kern_hash()` ; les shims mode utilisateur rejettent via `#error`.

Voir [kernel-mode-dyncode.md](kernel-mode-dyncode.md) pour les détails.
...
| Sujet | Approche |
|-------|----------|
| PIC par défaut | Toutes toolchains `isPICDefaultForced()==true`, aligné avec les hypothèses dyncode |
| Corriger d'abord en IR | Constantes, sauts indirects, intrinsèques mémoire éliminés en IR si possible |
| Filet de sécurité MIR | `DynCodeMIRPrepPass` + interposes pré/post, puis extraction/patching comme dernier recours |
| Minimiser le codage dur | `TargetDesc` + `Options.td.h` piloté par table |
| Deux dimensions user/kernel | `-fdyncode` × `-mdyncode-context={user,kernel}` ; chaque (OS, arch, level) est une ligne dans `describeTriple()` |
...
Les en-têtes shim (`sys/mman.h`, `fcntl.h`, etc.) exposent des constantes devant correspondre à l'ABI du noyau cible, car les stubs syscall de dyncode passent ces valeurs directement au noyau sans traduction libc.
