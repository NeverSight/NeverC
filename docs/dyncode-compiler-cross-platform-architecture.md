...
- [README.md](dyncode-compiler.md) — overview, CLI options, quick start
- [ir-pass-design.md](dyncode-compiler-ir-pass-design.md) — IR-layer pass responsibilities and examples
- [mir-pass-design.md](dyncode-compiler-mir-pass-design.md) — MIR-layer prep pass + obfuscation interposes
- [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md) — kernel context design details
- [platform-extension-guide.md](dyncode-compiler-platform-extension-guide.md) — step-by-step guide for adding new platforms
...
When `-fdyncode` is active, the compiler follows this fixed order. **Every stage has a corresponding obfuscation interpose** for third-party pass insertion:
...
```
  ...
  └──────────────┬────────────────────────────┘
                 ...
                 ▼
   PipelineStartEP:
    ① RunBeforePrep         ← extensible interpose
    ② ZeroRelocPass (Prep)
    ③ RunAfterPrep          ← extensible interpose
    ④ IndirectBrPass
    ...
    ⑫ RunBeforeInlining     ← extensible interpose

   LLVM AlwaysInliner / SROA / SLPVectorize / InstCombine

   OptimizerLastEP:
    ⑫ RunAfterInlining      ← extensible interpose (IR fully flattened)
    ⑬ Data2TextPass phase 2
    ⑭ ZeroRelocPass (Stackify)
    ⑮ RunAfterStackify      ← extensible interpose
    ⑯ AllBlrPass            (optional -fdyncode-all-blr)

                 │  LLVM IR → MIR → Register Allocation
                 ▼
   TargetPassConfig.addMachinePasses, before addPreEmitPass:
    ⑰ RunBeforePreEmit      ← MIR interpose, CFI pseudos still present
    ⑱ DynCodeMIRPrepPass  ← includes MIRRewritePatterns/Opcodes tables
    ⑲ RunAfterPreEmit       ← MIR interpose, closest to AsmPrinter

                 │  Mach-O / ELF / COFF object file
                 ▼
   DynCodeExtractor dispatcher → MachO / ELF / COFF extractor
...
2. **DynCode has no external relocations and no data sections**. Each pass eliminates one class of "loader-required" references; the extractor performs byte-level fallback auditing.
...
- Regular (non-dyncode) compilation also uses PIC, ensuring behavioral consistency.

## 4. User / Kernel Orthogonal Dimension

ExecutionLevel is orthogonal to (OS, arch):

- **User** (`-mdyncode-context=user`, default): classic ring-3 payload
...
- **Kernel** (`-mdyncode-context=kernel`): ring-0 payload
  ...
  - Users still just write normal C. `<neverc/dyncode/kernel.h>` provides `neverc_kern_resolve_t` and `neverc_kern_hash()`.

## 5. User-Mode "Normal C" Support Matrix

With `-fdyncode`, the following common C patterns are **directly supported** without user awareness:
...
| POSIX headers | `<unistd.h>` / `<fcntl.h>` | DynCode shim headers |
| Win32 headers | `<windows.h>` | DynCode shim headers |
...
2. **DynCode-friendly instruction rewriting**: `MIRRewritePatterns.def` + `MIRRewriteOpcodes.def` table-driven (e.g., AArch64 CPI FP → FMOV imm; x86_64 MOVSS/MOVSD +0.0 → xorps).
...
## 8. Obfuscation Interpose Points

11 `ObfuscationInterposes` fields (6 IR + 3 MIR + 2 byte-level) cover every pipeline stage:
...
| `RunAfterFinalIR` | True last IR interpose; your pass must be PIC-clean |
...
| `RunAfterFinalMIR` | True last MIR interpose; after all LLVM backend passes |
