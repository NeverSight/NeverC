...
| WinPEBImportPass | Win32 extern → PEB 모듈 워크 + PE 익스포트 리졸버 | PipelineStart |
| KernelImportPass | Ring-0 extern → 리졸버 지원 간접 호출 (커널만) | PipelineStart |
...
미해결 extern → 리졸버 지원 간접 호출로 자동 리라이트. `(resolver, cookie)` 암시적 매개변수 주입. [kernel-mode-shellcode.md](../kernel-mode-shellcode/README.ko.md) 참조.
