...
## 9. 難読化フック
11 フックポイント（`NEVERC_HOOK_SC_*`）。[Plugin API — フックポイント](../../plugin-api/README.ja.md#5-フックポイント) 参照。

## 10. 二段階設計の根拠
Phase 1 は元の IR をクリーン。LLVM 最適化後、Phase 2 は最適化器が導入した新構造をクリーン。

## 11. KernelImportPass（ring-0 のみ）
未解決 extern → 暗号化アドレスキャッシュ付きリゾルバ間接呼出に自動リライト。`(resolver, cookie)` 暗黙パラメータ注入。各 API の解決結果は暗号化後キャッシュされ、後続呼出はキャッシュを使用（lock-free `cmpxchg`）。暗号化メカニズムは WinPEBImportPass と共有（[§4.1](#41-アドレスキャッシュ暗号化) 参照）。[kernel-mode-shellcode.md](../kernel-mode-shellcode/README.ja.md) 参照。
...
shellcode 内の `malloc`/`free`/`calloc`/`realloc` 呼び出しをハイブリッド割り当て戦略に書き換え（デフォルト有効、`-fshellcode-heap-arena` / `-fno-shellcode-heap-arena` で制御）：
