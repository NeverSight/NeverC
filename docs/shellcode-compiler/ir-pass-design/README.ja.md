...
未解決 extern → 暗号化アドレスキャッシュ付きリゾルバ間接呼出に自動リライト。`(resolver, cookie)` 暗黙パラメータ注入。各 API の解決結果は暗号化後キャッシュされ、後続呼出はキャッシュを使用（lock-free `cmpxchg`）。[kernel-mode-shellcode.md](../kernel-mode-shellcode/README.ja.md) 参照。
