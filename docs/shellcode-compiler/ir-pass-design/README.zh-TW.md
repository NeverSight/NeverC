...
| WinPEBImportPass | Win32 extern → PEB 模組遍歷 + PE 匯出解析器 | PipelineStart |
| KernelImportPass | Ring-0 extern → 解析器支援的間接呼叫（僅核心模式） | PipelineStart |
...
## 4. WinPEBImportPass
Win32 extern → PEB walk 解析器。約 190 API，6 DLL。Windows POSIX 相容：13 函式群組桥接。
...
自動重寫 extern 直接呼叫為解析器支援的間接呼叫，注入 `(resolver, cookie)` 前綴參數。見 [kernel-mode-shellcode.md](../kernel-mode-shellcode/README.zh-TW.md)。
