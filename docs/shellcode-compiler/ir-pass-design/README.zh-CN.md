...
| WinPEBImportPass | Win32 extern → PEB 模块遍历 + PE 导出解析器 | PipelineStart |
| KernelImportPass | Ring-0 extern → 解析器支持的间接调用（仅内核模式） | PipelineStart |
...
1. 生成 `always_inline` 包装器调用 `__neverc_win_resolve(dll_hash, api_hash)`
2. 解析器是真实的 PEB → Ldr → InMemoryOrderModuleList 遍历器，带 ROR-13 哈希匹配
3. 每个平台只有一条内联汇编指令：`movq %gs:0x60, $0`（x86_64）/ `ldr $0, [x18, #0x60]`（arm64）
4. 所有列表遍历、PE 头解析和哈希比较都是纯 LLVM IR
...
当 `-mshellcode-context=kernel` 时激活。自动重写未解析的 extern 直接调用为解析器支持的间接调用：

1. 模块含需要解析的 extern 直接调用时，入口签名前置 `(resolver, cookie)` 隐式参数
2. 每个调用点：`fn_ptr = resolver(FNV1a_hash(name), cookie); ((T*)fn_ptr)(args...)`
