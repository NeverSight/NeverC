...
2. **Внедеревный C-плагин API** — Чистый C ABI интерфейс плагинов (`NevercPluginAPI.h`) для пользовательских IR, MIR, Binary и Linker проходов. Плагины регистрируются в 11 точках подключения shellcode (`NEVERC_HOOK_SC_*`). SDK с одним заголовком, ноль зависимостей LLVM/CRT. См. [документацию API плагинов](../../plugin-api/README.ru.md).
