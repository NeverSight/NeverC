...
2. **واجهة الإضافات C خارج الشجرة** — واجهة إضافات C ABI خالصة (`NevercPluginAPI.h`) لمرورات IR وMIR وBinary وLinker المخصصة. تُسجَّل الإضافات في 11 نقطة ربط shellcode (`NEVERC_HOOK_SC_*`). SDK بملف رأس واحد، صفر تبعيات LLVM/CRT. راجع [وثائق واجهة الإضافات](../../plugin-api/README.ar.md).
