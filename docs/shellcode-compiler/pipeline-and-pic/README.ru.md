...
- **IR обфускация**: Через `setShellcodeObfuscationHooks`; 11 хуков (6 IR + 3 MIR + 2 байтовый поток).
...
- **SDK плагинов Finalize**: `Plugin.h` предоставляет `registerBadByteRewriteStrategy` и `registerCharsetEncoder`. См. [plugin-interface.md §2–§3](../plugin-interface/README.ru.md#2-bad-byte-rewriter-badbyterewritestrategy).
