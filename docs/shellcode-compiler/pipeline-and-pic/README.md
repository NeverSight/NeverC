...
- **Kernel mode**:
  ...
  - `<neverc/shellcode/kernel.h>` exposes `neverc_kern_resolve_t`, `neverc_kern_hash()`, and related kernel-side signatures; user-mode shims (`<windows.h>`, `<unistd.h>`, etc.) reject via `#error` in kernel mode.
