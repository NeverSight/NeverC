...
- **Modo kernel**:
  ...
  - `<neverc/shellcode/kernel.h>` expone `neverc_kern_resolve_t`, `neverc_kern_hash()` y firmas relacionadas del lado kernel; los shims de modo usuario (`<windows.h>`, `<unistd.h>`, etc.) rechazan en modo kernel vía `#error`.
