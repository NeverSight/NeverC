...
1. **SDK empaquetado con el árbol de compilación** (recomendado): Usuarios y scripts de prueba tratan `build-neverc/sdk` como raíz del SDK. NeverC auto-detecta `sdk/msvc/` dentro del directorio de instalación e inyecta rutas include/lib en `MSVCToolChain::AddNeverCSystemIncludeArgs` / `Linker::ConstructJob`. Diseño típico:
...
- **SDK de plugin Finalize**: `Plugin.h` expone `registerBadByteRewriteStrategy` (cadena de estrategias de reescritura de bytes prohibidos a nivel de instrucción) y `registerCharsetEncoder` (registro de charset con nombre). Ver [plugin-interface.md §2–§3](../plugin-interface/README.es.md#2-bad-byte-rewriter-badbyterewritestrategy).
