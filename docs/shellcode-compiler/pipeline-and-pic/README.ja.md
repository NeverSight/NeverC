...
1. **ビルドツリーにバンドルされた SDK**（推奨）：ユーザーとテストスクリプトが `build-neverc/sdk` を SDK ルートとして扱う。NeverC はインストールディレクトリ内の `sdk/msvc/` を自動検出し、`MSVCToolChain::AddClangSystemIncludeArgs` / `Linker::ConstructJob` で include/lib パスを注入。典型レイアウト：
