...
NeverC предоставляет чистый C ABI для внедеревных pass-плагинов. Плагин — это разделяемая библиотека (`.dll` / `.so` / `.dylib`), которая регистрирует пользовательские проходы в назначенных точках подключения конвейера. Один заголовочный файл, ноль зависимостей от LLVM/CRT.
