...
Dieses Dokument skizziert die wichtigsten geplanten Richtungen des NeverC-Projekts jenseits des bestehenden DynCode-Compilers und der integrierten Laufzeiten.
...
- **DynCode-kompatibel** — Pakete, die im Freestanding-Modus sinnvoll sind (z. B. `crypto`, `encoding`, `bytes`), funktionieren mit `-fdyncode`
...
- Die DynCode-Kompilierungserfahrung (positionsunabhängig, minimale Laufzeit) lässt sich direkt auf On-Chain-Programmeinschränkungen übertragen
