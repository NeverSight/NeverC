...
如果外掛註冊了自己的目標，也可以在其 `NevercCallingConventionDescriptor` 上提供 `PlanCallingConvention` 回呼直接產出 plan，跳過 spec 這一層。參見[目標、MC、組譯與目的檔](plugin-api-target-mc-object.md#abi-與呼叫慣例)。
