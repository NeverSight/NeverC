**Languages**: [English](../builtins.md) | [简体中文](../zh-CN/builtins.md) | [繁體中文](builtins.md) | [日本語](../ja/builtins.md) | [한국어](../ko/builtins.md) | [Français](../fr/builtins.md) | [Deutsch](../de/builtins.md) | [Español](../es/builtins.md) | [Italiano](../it/builtins.md) | [Русский](../ru/builtins.md) | [العربية](../ar/builtins.md)
...
| 內建功能 | 旗標 | 預設 | 描述 |
|---------|------|------|------|
| [**`string`**](builtins-string.md) | `-fbuiltin-string` | 關閉 | 值語義字串型別，支援點呼叫方法、自動記憶體管理和原生 UTF-8 |
| [**`mimalloc`**](builtins-mimalloc.md) | `-fbuiltin-mimalloc` | **開啟** | 高效能記憶體配置器，透明替換 `malloc`/`free`/`calloc`/`realloc` |
| [**`xorstr`**](builtins-xorstr.md) | `-fencrypt-call-strings` | 關閉 | 逐實例編譯期加密、強制 late 封口、逐呼叫點最終展開與 volatile 堆疊清零 |
| [**`strhash`**](builtins-strhash.md) | `-fstrhash-algo` / `-fstrhash-fold` | 關閉 | 編譯期字串雜湊，執行時演算法一致，可選 IR 常數摺疊 |
...
> **說明：** `xorstr` 不走嵌入式 bitcode 模型。顯式巨集 [`NC_XORSTR(s)` / `NEVERC_XORSTR(s)`](builtins-xorstr.md) 由 `SemaCheckingBuiltinNeverC.cpp` 中的 `semaBuiltinNeverCXorstr` 降級。`EncryptCallStringsPass` 與 `XorStrCleanupPass` 會在 IPO 前及每個一般或 plugin late IR 階段後封口自動字面值並清除明文堆疊槽；`FinalizeXorStrPass` 僅在真正的原生機器碼邊界重新加密並逐呼叫點展開顯式解碼器，隨後移除共享輔助圖。完整設計與可重現性約定見 [xorstr 文件](builtins-xorstr.md)。

> **說明：** `strhash` 同樣不走嵌入式 bitcode 模型。[`NC_STRHASH(s)`](builtins-strhash.md) 在 Sema 摺疊為整數常數；`-fstrhash-fold` 啟用 `StrHashFoldPass`。詳見 [strhash 文件](builtins-strhash.md)。
...
| **DynCode 模式** | 自動啟用，arena 重寫 | 被抑制（HeapArenaPass 處理堆積分配） |
...
| `-fdyncode-mode` | 抑制 `mimalloc` | 由 HeapArenaPass 替代（基於 arena） |
| `-ffreestanding` | 抑制 `mimalloc` | 無 libc 可覆蓋 |

`string` 內建有自己的抑制邏輯（dyncode 流水線中 arena 重寫替換堆積分配）。

### HeapArenaPass（DynCode 堆積分配）

當 `-fdyncode-mode` 啟用時，`mimalloc` 被抑制，但 `malloc`/`free`/`calloc`/`realloc` 呼叫會被 `HeapArenaPass` 自動改寫（預設開啟）。該 Pass 使用混合策略：

- **小分配（≤ 64 KB）**：從與 `string` 內建執行時共享的堆疊上 arena 分配（bump allocator + free list 重用）。
- **大分配（> 64 KB）或 arena OOM**：回退到 OS 分配器：
  - **Windows**：`malloc`/`free` 透過 PEB walk 從 `msvcrt.dll` 解析（`-mdyncode-win-peb-import`）。
  - **Linux / macOS / Android**：`mmap`/`munmap` 內聯為原生系統呼叫（`-mdyncode-syscall`）。
  - **未啟用匯入 Pass**：僅使用 arena；OOM 傳回 `NULL`。

透過驅動標誌控制：

```bash
neverc -fdyncode test.c -o test.bin                     # HeapArenaPass 開啟（預設）
neverc -fdyncode -fno-dyncode-heap-arena test.c       # HeapArenaPass 關閉（原始行為）
```
