...
2. **樹外 C 外掛 API** — 純 C ABI 外掛介面（[`NevercPluginAPI.h`]），用於自訂 IR、MIR、Binary 和 Linker pass。外掛透過 11 個 dyncode 掛鈎點（`NEVERC_INTERPOSE_SC_*`）註冊。單一標頭檔 SDK，零 LLVM/CRT 相依性。詳見 [Plugin API 文件](../plugin-api/README.md)。
...
用法和程式碼範例見 [Plugin API 文件](../plugin-api/README.md)。
