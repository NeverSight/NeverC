...
- [x] 目錄與 CMake 骨架（`nevercDynCode` 程式庫）
...
- [x] `DynCodeExtractor`：Mach-O `.o` → 扁平 `.bin`，含節內重定位修補
- [x] CLI 選項（透過產生的 [`neverc/include/neverc/Invoke/Options.td.h`]）：`-fdyncode`、`-fdyncode-all-blr`、`-mdyncode-syscall`、`-fdyncode-keep-obj=`、`-fdyncode-entry=`
...
**測試**：108/108 dyncode 斷言通過。二進位大小：`add` 8B、`fib` 64B、`hello` 64B、`big_const` 632B。

## 階段 1 — Linux / Android / Windows 跨平台（已交付）

- [x] `TargetDesc` 抽象：表驅動的平台差異
- [x] 跨平台 `-mdyncode-syscall` 語意（替代僅 Darwin 的 `-mdyncode-libsystem`）
- [x] Linux / Android 系統呼叫號表（Darwin BSD 100+、Linux arm64 130+、Linux x86_64 150+）
- [x] `DynCodeExtractor` 重構為 `MachOExtractor` / `ELFExtractor` / `COFFExtractor`
...
- [x] 多 DLL Win32 API 白名單（~210 API，跨 kernel32/ntdll/user32/ws2_32/advapi32/shell32）
...
- [x] DynCode 墊片標頭檔：`<windows.h>`、`<unistd.h>`、`<fcntl.h>`、`<sys/stat.h>`、`<sys/mman.h>`、`<string.h>`、`<stdlib.h>`
...
- [x] `<neverc/dyncode/kernel.h>` 用於 ring-0 進入點慣例
- [x] 進入點偏移零強制（`placeEntryFirst`）
- [x] Finalize 管線：壞位元組重寫器 SDK + 字元集編碼器 SDK + 大小約束
- [x] 樹外 C 外掛 API（[`NevercPluginAPI.h`]）：11 個 dyncode 掛鈎點（`NEVERC_INTERPOSE_SC_*`）
- [x] x86_64 `-mno-implicit-float` 注入（防止後端 SSE 常數池溢出）
- [x] 跨平台 loader（macOS/Linux/Windows）

**測試**：743+ dyncode 斷言，全部通過（涵蓋 8 個 triple）。NeverC 總測試套件：1000+ 測試通過。

## 階段 2 — 可列印 / 字母數字編碼器（計畫中）

- [ ] ARM64 可列印 dyncode 編碼器（0x20–0x7e 指令子集）
- [ ] x86_64 字母數字編碼器
