...
Core v2 支援一般使用者解構函式，以及正常離開時的隱含成員解構。區域物件、紀錄欄位與陣列元素以反向順序解構；暫存物件在完整運算式結束時清理，先保存需要使用的值。回傳、分支、迴圈、break 與 continue 均執行對應清理。以值傳遞的參數在被呼叫函式結束時解構，回傳物件由呼叫者管理。明確解構呼叫、手寫例外規格、靜態物件解構與例外展開仍未支援。
...
Core v2 支援指向既有存活物件的右值參考，包括純量、指標、記錄與陣列別名、參考參數／回傳值、條件亡值及一般 `&&` 限定方法。`static_cast<R&&>(live)` 等轉換保留同一個物件，具名右值參考變數仍是左值。多載與既有複製行為遵循 Clang 的選擇，參考不會增加清理擁有者。本階段尚不支援繫結新的暫存物件、延長生命週期或未實作的移動操作。

Core v2 支援以存活的 `R&&` 或 `const R&&` 為來源的一般使用者移動建構與移動指定。建構使用實際目標；指定保留來源修改、運算元求值順序與回傳的 `R&` 別名，並支援 `&`／`&&` 限定的接收物件。具名右值參考仍選擇左值多載，明確建構函式保留其初始化規則。移動不會結束來源物件的生命週期，來源與目標仍各自正常解構。產生／default 移動、新暫存物件的參考繫結、例外、樣板與完整 STL 仍在開發中。
