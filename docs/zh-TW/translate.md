...
Core v2 支援一般成員及自由多載運算子，包括算術、比較、下標、解參考、遞增、函式物件與一般指定簽章。呼叫保留選定函式、參考別名及物件結果。運算子語法遵循 C++17 求值順序，多載邏輯運算子會求值兩個運算元。自由指定運算子統一由右向左初始化參數，再依反向順序解構。轉換函式、樣板、記憶體配置與完整 STL 仍在開發中。
