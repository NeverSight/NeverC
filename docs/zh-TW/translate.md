...
Core v2 支援以存活的 `R&&` 或 `const R&&` 為來源的一般使用者移動建構與移動指定。建構使用實際目標；指定保留來源修改、運算元求值順序與回傳的 `R&` 別名，並支援 `&`／`&&` 限定的接收物件。具名右值參考仍選擇左值多載，明確建構函式保留其初始化規則。移動不會結束來源物件的生命週期，來源與目標仍各自正常解構。新暫存物件的參考繫結、例外、樣板與完整 STL 仍在開發中。
