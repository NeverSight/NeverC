...
Core v2 支援一般使用者解構函式，以及正常離開時的隱含成員解構。區域物件、紀錄欄位與陣列元素以反向順序解構；暫存物件在完整運算式結束時清理，先保存需要使用的值。回傳、分支、迴圈、break 與 continue 均執行對應清理。以值傳遞的參數在被呼叫函式結束時解構，回傳物件由呼叫者管理。明確解構呼叫、手寫例外規格、靜態物件解構與例外展開仍未支援。隱含非平凡複製、移動操作與完整 STL 仍在開發中。

Core v2 支援來源參數為 `R&` 或 `const R&` 的一般使用者複製建構與複製指定函式。複製直接作用於實際目標，保留函式的副作用與回傳參考。指定運算子語法先求值右運算元，明確呼叫 `operator=` 時先求值接收物件。明確 default 的複製賦值與隱含非平凡複製賦值仍未支援。

Core v2 也支援編譯器產生或明確 default 的預設建構函式，以及 default 解構函式，包括巢狀紀錄與陣列成員。預設建構使用實際目標位置，並依宣告順序初始化成員；值初始化只執行 C++ 規則要求的清零。類別外 default 保留其不同的初始化規則。未使用或僅出現在未求值運算式中的 default 建構不要求額外產生函式本體。預設成員初始化器、隱含非平凡複製賦值、移動操作與完整 STL 仍在開發中。

Core v2 支援隱含及明確 default 的複製建構，包括巢狀紀錄與多維陣列。選定的成員複製建構依宣告及元素順序在實際目標位置執行，來源陣列位址只計算一次。平凡複製保留已儲存的指標欄位原值。按值傳參與從來源物件回傳保留複製副作用，直接轉送純右值不會額外複製。自動產生的複製賦值、移動操作及其餘 C++／STL 功能仍在開發中。
