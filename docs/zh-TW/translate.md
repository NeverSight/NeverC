...
Core v2 也支援隱含及明確 default 的複製指定。成員與陣列元素保留選定的指定操作及其順序，產生的平凡陣列複製轉為具型別的元素指定，無需外部記憶體複製呼叫。平凡指定保留指標欄位原值並回傳實際接收物件。運算子語法先擷取來源參考，明確成員呼叫先擷取接收物件；來源值在這些副作用完成後讀取，指定不會額外建構或解構物件。預設成員初始化器、移動操作、樣板與完整 STL 仍在開發中。
