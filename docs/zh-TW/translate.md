...
Core v2 也支援具有標準布局及受支援複製操作的紀錄型別的一般使用者建構函式。區域物件、紀錄欄位與陣列元素直接在最終儲存位置建構，欄位依宣告順序初始化。委派建構、移動建構及例外仍未支援。
...
Core v2 支援已准入欄位的預設成員初始化器，包括存取先前成員、一般呼叫、巢狀記錄與陣列。選中的預設值使用實際所屬物件作為 `this`，明確的聚合初始化項保留呼叫者的 `this`；明確初始化會覆蓋對應成員的預設值。隱含或 default 複製及指定不重跑預設值，使用者複製建構函式可為省略的成員選用預設值。建構成員初始化與聚合初始化各自保留暫存物件的清理邊界。移動操作、樣板與完整 STL 仍在開發中。
