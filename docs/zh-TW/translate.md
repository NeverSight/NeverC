...
Core v2 現支援普通自動區域參考延長暫存物件的生命週期，包括 `const T& r{T{...}}` 與 `T&& r = T{...}`。純量、列舉、指標及記錄暫存值使用實際儲存；C++ 規定延長生命週期時，指向成員或陣列元素的參考會保留完整記錄物件。大括號及等號加大括號初始化保留同一個物件。物件在參考所屬範圍結束時解構，涵蓋條件、迴圈與提前離開；初始化器內其他暫存物件仍在各自完整運算式結束時清理。別名不增加擁有者，後續複製或移動保留選定操作。靜態／全域／執行緒區域參考、參考欄位、獨立陣列暫存值、樣板與完整 STL 仍在開發中。
