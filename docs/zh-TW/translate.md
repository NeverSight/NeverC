...
Core v2 也支援隱含及明確 default 移動建構與移動指定，包括巢狀記錄與多維陣列。成員依宣告與元素順序執行 C++ 選中的複製或移動。產生的移動不重跑預設成員初始化器，平凡操作保留已儲存的指標值。來源物件仍正常解構，未使用或平凡的產生操作不需要虛構函式主體。既有內聯隱含平凡操作保留其暫存物件行為；一般暫存參考繫結仍未開放。完整 C++／STL 支援仍在推進。
...
Core v2 也支援現存物件上的一般轉換函式：隱式與顯式整數、列舉及指標轉換，條件中的顯式 `bool`，以及參考或物件結果。每次轉換執行一次選定的成員函式。參考保留別名；物件純右值直接初始化實際目標，從參考初始化值則保留選定的複製或移動。const／參考限定、constexpr 與 noexcept 遵循 C++17 選擇規則。暫存參考的生命週期、樣板與完整 STL 仍在開發中。

Core v2 現支援暫存物件接收者，以及存活到完整運算式結束的純量／記錄暫存參考引數，涵蓋建構函式、方法、運算子與轉換函式。每次求值都有實際儲存，物件結果保留最終目標，參考保留別名。呼叫及被呼叫函式的參數解構完成後，暫存物件依建構反向順序清理，並正確處理條件分支與迴圈中的重複求值。暫存記錄物件的陣列子物件也受支援。獨立陣列暫存值、自動／靜態參考的生命週期延長、樣板與完整 STL 仍在開發中。
