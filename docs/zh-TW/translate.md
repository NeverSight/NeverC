...
Core v2 支援來源參數為 `R&` 或 `const R&` 的一般使用者複製建構與複製指定函式。複製直接作用於實際目標，保留函式的副作用與回傳參考。指定運算子語法先求值右運算元，明確呼叫 `operator=` 時先求值接收物件。明確 default 的複製操作與隱含非平凡複製仍未支援。

Core v2 也支援編譯器產生或明確 default 的預設建構函式，以及 default 解構函式，包括巢狀紀錄與陣列成員。預設建構使用實際目標位置，並依宣告順序初始化成員；值初始化只執行 C++ 規則要求的清零。類別外 default 保留其不同的初始化規則。未使用或僅出現在未求值運算式中的 default 建構不要求額外產生函式本體。預設成員初始化器、隱含非平凡複製、移動操作與完整 STL 仍在開發中。
