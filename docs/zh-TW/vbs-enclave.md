**語言**: [English](../vbs-enclave.md) | [简体中文](../zh-CN/vbs-enclave.md) | [繁體中文](vbs-enclave.md) | [日本語](../ja/vbs-enclave.md) | [한국어](../ko/vbs-enclave.md) | [Français](../fr/vbs-enclave.md) | [Deutsch](../de/vbs-enclave.md) | [Español](../es/vbs-enclave.md) | [Italiano](../it/vbs-enclave.md) | [Русский](../ru/vbs-enclave.md) | [العربية](../ar/vbs-enclave.md)
...
跨主機連結階段會產生未簽署、未經處理的 enclave DLL。VEIID 處理、SignTool 簽署，以及透過 `CreateEnclave`/`LoadEnclaveImage` 實際載入仍只能在 Windows 上進行，因此請將在 macOS 或 Linux 上連結的 DLL 移至 Windows 封裝機或測試機，以完成最後三個階段。有關執行階段的安裝與探索，請參閱[目標執行階段](runtime.md)。
...
當 `/GUARD:MIXED` 與 `/GUARD:EHCONT` 組合使用時，EH 延續目標資料表也使用 5 位元組項目：4 位元組 RVA 後接 1 個值為零的中繼資料位元組。
...
7. 使用 `GetProcAddress` 解析 enclave 匯出函式，透過 `CallEnclave` 進入該函式並驗證傳回值。重複呼叫，涵蓋受 CFG 保護與舊式的間接呼叫路徑。
8. 使用 `TerminateEnclave` 與 `DeleteEnclave` 終止並釋放 enclave，檢查兩個操作均成功。
...
x64 執行階段探測會先執行 Microsoft 映像，再執行兩個 NeverC 候選映像。它使用 `GetProcAddress` 解析匯出函式，透過 `CallEnclave` 重複呼叫，涵蓋受 CFG 保護與舊式的間接呼叫路徑，並檢查傳回值。`PASS` 要求完整且順序正確的生命週期階段證據，包括成功終止與釋放；僅載入和初始化成功並不足夠。在此工作流程中，ARM64 的涵蓋範圍是靜態驗證與差異連結；例行執行階段探測仍僅針對 x64。

2026-09-11，單獨進行的原生 Windows 11 ARM64 驗收已通過（[執行記錄 34554067852](https://github.com/NeverSight/NeverC/actions/runs/34554067852)）。該次驗收重用了從 `c46e3a4cc4b3732823a7241a43bca6c44b0f0b74` 建置的兩個 NeverC ARM64 DLL，並與新建置的 MSVC 參考映像一同測試。48 個生命週期階段和 12 次含結果驗證的呼叫全部通過，enclave 終止、釋放及簽署憑證清理也均成功。這驗證的是這批固定產物，不是新建置的 NeverC 編譯器，也不代表已建立永久 ARM64 執行階段 CI 閘道。

選擇性的執行階段探測僅能在函式執行前，因已識別的環境準備條件不可用而回報 `SKIP`。當機、逾時、階段證據缺漏或順序錯誤及功能錯誤均為 `FAIL`，Microsoft 參考映像也不例外。設定妥當的自託管 VBS runner 可以將執行階段成功設為強制閘道。
