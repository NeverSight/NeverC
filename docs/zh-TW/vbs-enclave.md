...
x64 執行階段探測會先執行 Microsoft 映像，再執行兩個 NeverC 候選映像。它使用 `GetProcAddress` 解析匯出函式，透過 `CallEnclave` 重複呼叫，涵蓋受 CFG 保護與舊式的間接呼叫路徑，並檢查傳回值。`PASS` 要求完整且順序正確的生命週期階段證據，包括成功終止與釋放；僅載入和初始化成功並不足夠。ARM64 的涵蓋範圍是靜態驗證與差異連結，不包含實際執行。
