**語言**: [English](../dyncode-compiler.md) | [简体中文](../zh-CN/dyncode-compiler.md) | [繁體中文](dyncode-compiler.md) | [日本語](../ja/dyncode-compiler.md) | [한국어](../ko/dyncode-compiler.md) | [Français](../fr/dyncode-compiler.md) | [Deutsch](../de/dyncode-compiler.md) | [Español](../es/dyncode-compiler.md) | [Italiano](../it/dyncode-compiler.md) | [Русский](../ru/dyncode-compiler.md) | [العربية](../ar/dyncode-compiler.md)

[← 文件索引](README.md) · [← NeverC 專案主頁](project.md)
...
- [ARM64 (AArch64) 組語教學 — DynCode 視角](dyncode-compiler-arm64-assembly-tutorial.md)
- [NeverC DynCode 跨平台架構概覽](dyncode-compiler-cross-platform-architecture.md)
- [IR Pass 設計 — 原則、流水線與前後對比](dyncode-compiler-ir-pass-design.md)
- [核心模式（Ring-0）DynCode 支援](dyncode-compiler-kernel-mode-dyncode.md)
- [MIR Pass 設計 — 原則與掛鉤點](dyncode-compiler-mir-pass-design.md)
- [DynCode 流水線、MIR 與 PIC 策略（設計筆記）](dyncode-compiler-pipeline-and-pic.md)
- [平台擴充指南](dyncode-compiler-platform-extension-guide.md)
- [DynCode 編譯器 — 進度追蹤](dyncode-compiler-progress.md)
- [路線圖](dyncode-compiler-roadmap.md)

---

## 核心目標

1. **使用者编写普通 C** — 无需 dyncode 专用技巧。
2. **全自动流水线** — `static int counter = 0`、`const char s[] = "..."`、递归函数、`write/exit/read/...` 以及大型常量数组均在内部处理，无需修改使用者代码。
...
| Triple | 格式 | 使用者态 syscall | Ring-0 解析器 | 状态 |
|--------|--------|-------------------|-----------------|--------|
| `arm64-apple-macos*` | Mach-O | `svc #0x80` (Darwin BSD) | `DarwinXNUKextShim` | 原生 loader 往返 + 内核解析器已覆盖 |
| `x86_64-apple-macos*` | Mach-O | `syscall` (BSD class mask `0x2000000`) | `DarwinXNUKextShim` | 編譯 + 提取通过；x86_64 `__text` 无重定位预期 |
| `aarch64-linux-gnu` | ELF | `svc #0` (x8 = nr) | `LinuxKallsymsShim` | 編譯 + 提取 + 内核解析器通过 |
| `x86_64-linux-gnu` | ELF | `syscall` (rax = nr) | `LinuxKallsymsShim` | 編譯 + 提取 + 内核解析器通过 |
| `aarch64-linux-android*` | ELF | 同 Linux arm64 | `LinuxKallsymsShim` (GKI) | 編譯 + 提取通过 |
| `x86_64-linux-android*` | ELF | 同 Linux x86_64 | `LinuxKallsymsShim` (GKI) | 編譯 + 提取通过 |
| `aarch64-pc-windows-msvc` | PE/COFF | **PEB 遍历** (`ldr xN, [x18, #0x60]`) | `WindowsKernelResolverShim` | 使用者态 PEB 读字节哨兵 `32 40 f9` 已验证；ring-0 使用 loader 解析器 |
| `x86_64-pc-windows-msvc` | PE/COFF | **PEB 模块遍历 + PE 导出表查找** | `WindowsKernelResolverShim` | 使用者态解析器为完整 IR 级 PEB 遍历；ring-0 不复用 PEB |

全部 8 个 (OS, arch) triple 由**同一套 pass** 驱动。差异隔离在 `TargetDesc.cpp` 表项与三个提取器架构分支。新增平台 = 多填一行表 + 各提取器多一个 case。`ExecutionLevel` 维度正交：`User` 走使用者态 syscall / PEB 流水线；`Kernel` 禁用二者并注入 `KernelImportPass`，通过 resolver shim 重写 extern 调用。见 [kernel-mode-dyncode.md](dyncode-compiler-kernel-mode-dyncode.md)。
...
| 选项 | 說明 |
|------|------|
| `-fdyncode` | 启用 dyncode 編譯模式。 |
| `-fno-dyncode` | 取消先前的 `-fdyncode`。 |
| `-fdyncode-all-blr` | 激进模式：将模块内直接调用间接化为 `blr xN` / `call *rax`，消除所有相对分支重定位。常规使用不需要。 |
| `-mdyncode-syscall` | 显式启用 syscall stub（Darwin/Linux/Android 下 `-fdyncode` 已默认；主要用于表达意图或脚本兼容）。 |
| `-mdyncode-libsystem` | Darwin 遗留别名，等同 `-mdyncode-syscall`。 |
| `-mdyncode-win-peb-import` | 显式启用 Windows PEB 导入（`-fdyncode` + Windows triple 已默认）。 |
| `-fdyncode-keep-obj=<path>` | 将中间对象文件复制到 `<path>` 供原生反汇编器审计。 |
...
| `-fdyncode-obfuscate=<spec>` | 傳遞給透過 [Plugin API](plugin-api.md) 註冊的 **IR 級**外掛掛鈎。未載入外掛時為 no-op。見 [ir-pass-design.md 第 9 節](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes)。 |
| `-fdyncode-mir-obfuscate=<spec>` | 传递给 **MIR 级**混淆钩子（`RunBeforePreEmit` / `RunAfterPreEmit`）。未设置时回退到 `-fdyncode-obfuscate=`。见 [mir-pass-design.md 第 3 節](dyncode-compiler-mir-pass-design.md#3-使用者混淆掛鉤)（User Obfuscation Interposes）。 |

---

## 架构概览

流水线分为**与目標无关的 IR pass + 目標相关的提取器**：
...
SyscallStubPass 与 WinPEBImportPass 均根据 TargetDesc 字段生成 InlineAsm。編譯器后端仍使用 TableGen 定义的指令模式。新增目標即在 `describeTriple` 中**多一行**，并在各提取器 switch 中**多一个 case**。
...
| libc write/exit | `write(1, s, 3);` | 是（需 `-mdyncode-syscall`） | Syscall 包装 |
| POSIX 头文件 | `#include <unistd.h>` | 是（dyncode 模式自动切换 shim） | 驱动注入 `__NEVERC_DYNCODE__` |
| Win32 API | `WriteFile(h, buf, n, &w, 0);` | 是（需 `-mdyncode-win-peb-import`） | PEB 遍历 thunk |
| Windows SDK 头文件 | `#include <windows.h>` | 是（dyncode 模式自动切换 shim） | 轻量 shim 头 |
| 自定义入口名 | `int dyncode_main(...)` | 是（需 `-fdyncode-entry=...`） | 驱动透传 |
...
│   │   ├── MmapABI.h                          # 共享 mmap 常數 (prot/flags)
│   │   ├── DynCodeIRHelpers.h               # 公共 IR 工具 (getSizeType 等)
...
1. DynCode 加载地址须 4 KB 对齐 — `mmap` / `VirtualAlloc` 的自然行为；所有 loader 代码已满足。
2. 调用约定遵循目標 OS 原生 ABI：
...
DynCode 流水線本身只保證「程式碼能正確執行」。混淆、多型、分階段編碼器等策略層功能**故意不內建**——它們透過 [Plugin API](plugin-api.md) 以樹外外掛的形式提供。
...
完整掛鈎清單、pass 註冊與程式碼範例，詳見 [Plugin API 文件](plugin-api.md)。

- IR 层：[ir-pass-design.md 第 9 節](dyncode-compiler-ir-pass-design.md#9-obfuscation-interposes)（Obfuscation Interposes）。
- MIR 层：[mir-pass-design.md 第 3 節](dyncode-compiler-mir-pass-design.md#3-使用者混淆掛鉤)（User Obfuscation Interposes）。

---

## 当前限制

- **支援 8 种 (OS, arch) 组合**（见上表）。其他 triple（RISC-V、PowerPC、32 位 x86、大端 ARM 等）在 `describeTriple()` 被拒绝并提示完整支援列表。每个 (OS, arch) 行有独立的 `User` / `Kernel` 上下文，共 16 种 (OS, arch, level) 变体。
- **Windows PEB 遍历已完整实现多 DLL 分派**。`__neverc_win_resolve` 接受 `(dll_hash, api_hash)` 对。当前白名单覆盖 kernel32.dll（約 125 API）、ntdll.dll（約 26）、user32.dll（約 13）、ws2_32.dll（約 23）、advapi32.dll（約 16）、shell32.dll（約 6）。新增 API = `Tables/Win32Apis.def` 一行 + `lib/Headers/windows.h` 一個宣告。
- **外部函数白名单**仅覆盖 Darwin BSD / Linux / Android 常见 syscall（约 80+）与 Win32 API（约 190）。stdio 等重度运行时接口未包含 — dyncode 无法嵌入完整 stdio 状态机。
- 不支援 C++ / ObjC / CUDA — NeverC 设计上仅支援 C。
