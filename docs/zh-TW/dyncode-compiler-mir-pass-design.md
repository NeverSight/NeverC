...
但 LLVM 後端在 **IR → MIR 降低** 過程中引入了 dyncode 無法容納的額外構造：
...
回呼不擷取 `Opts`。它在執行時讀取當前 `DynCodeOptions` 快照，防止同一程序同時編譯 dyncode 和普通 C 時使用過期配置。
...
[Codegen (IR → MIR)]
  ...
  ├─ DynCodeMIRPrepPass  ← 本文檔重點
  └─ RunAfterPreEmit     (使用者掛鉤, CFI 已剝離)
        │
[AsmPrinter → 目的檔]
        │
[DynCodeExtractor]  ← 位元組級兜底稽核
  ├─ RunPostExtract   (使用者掛鉤, 純位元組)
  └─ flat .bin
```

MIR 層處理**兜底清理 + 混淆掛鉤點**，而非業務邏輯。「寫普通 C，無需 dyncode 技巧」的承諾由 5+ 個 IR pass 實現。
...
**新增內建偽指令剝離**：在 `isDynCodeStripPseudo` switch 中新增一個 case。
...
**第三方混淆庫**：透過 [Plugin API](plugin-api.md)（`NEVERC_INTERPOSE_SC_*` 掛鈎）註冊。

## 7. 與 DynCodeExtractor 的關係
