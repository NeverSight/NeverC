...
## 1. 核心開關：`-mdyncode-context={user,kernel}`

- **使用者模式**（預設）：維持現有 PEB / syscall stub 流水線。
- **核心模式**：停用 SyscallStubPass / WinPEBImportPass、注入核心標誌、啟用 KernelImportPass、注入 `__NEVERC_DYNCODE_KERNEL__`。
