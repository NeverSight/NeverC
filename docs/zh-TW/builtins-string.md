**語言**: [English](../builtins-string.md) | [简体中文](../zh-CN/builtins-string.md) | [繁體中文](builtins-string.md) | [日本語](../ja/builtins-string.md) | [한국어](../ko/builtins-string.md) | [Français](../fr/builtins-string.md) | [Deutsch](../de/builtins-string.md) | [Español](../es/builtins-string.md) | [Italiano](../it/builtins-string.md) | [Русский](../ru/builtins-string.md) | [العربية](../ar/builtins-string.md)

[← NeverC 內建執行時系統](builtins.md) · [NeverC 文件](README.md)
...
- 輸入檔案為 `.nc` 副檔名（參見 [`.nc` 副檔名文件](nc-extension.md)）
- `-fdyncode` 模式啟動時
...
- **預設**：seed `0` 會讓編譯器為每個加密字面值取得新的作業系統隨機熵；熵無法取得時安全失敗。獨立建置會產生不同 ciphertext。
- **CLI 覆蓋**：使用非零 `-fstring-encrypt-key=0xDEADBEEF` 指定具決定性的完整 64 位元 seed；翻譯單元本地 nonce 會分離各字面值，適用於可重現建置與測試。
...
| 旗標 | 說明 |
|------|------|
| `-fstring-encrypt-key=<hex>` | 覆寫完整 64 位元保護 seed；`0` 使用新的作業系統隨機熵 |
